		...
		if (player.tailType === AppearanceDefs.TAIL_TYPE_CAT && player.earType === AppearanceDefs.EARS_CAT &&
			...
			if (player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_HOOFED || player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_CENTAUR) {
				...
				if (player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_CENTAUR) {
					EngineCore.outputText("  You feel woozy and collapse on your side.  When you wake, you're no longer a centaur and your body has returned to a humanoid shape.", false);
				}
	...
	Mutations.reptilum = function(player) {
		...
		if (player.lib < 100 && changes < changeLimit && Utils.rand(3) === 0) {
			...
			if (player.cocks.length > 0 && (player.gender !== 3 || Utils.rand(2) === 0)) {
				...
				EngineCore.outputText("puddling in your featureless crotch for a split-second before it slides into your " + Descriptors.assDescript() + ".  You want to be fucked, filled, and perhaps even gain a proper gender again.  Through the lust you realize your sex-drive has been permanently increased.", false);
			}
	...
	Mutations.demonChanges = function(player) {
		...
		if ((player.faceType !== AppearanceDefs.FACE_HUMAN || player.skinType !== AppearanceDefs.SKIN_TYPE_PLAIN) && Utils.rand(3) === 0) {
			...
			if (player.faceType !== AppearanceDefs.FACE_HUMAN) {
				...
				EngineCore.outputText("\n\n", false);
				if (player.skinType === AppearanceDefs.SKIN_TYPE_FUR) {
					EngineCore.outputText("Your skin suddenly feels itchy as your fur begins falling out in clumps, <b>revealing inhumanly smooth skin</b> underneath.", false);
				} else if (player.skinType === AppearanceDefs.SKIN_TYPE_SCALES) {
					EngineCore.outputText("Your scales begin to itch as they begin falling out in droves, <b>revealing your inhumanly smooth " + player.skinTone + " skin</b> underneath.", false);
				}
				player.skinType = AppearanceDefs.SKIN_TYPE_PLAIN;
				player.skinDesc = "skin";
			}
