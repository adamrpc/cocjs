...
angular.module('cocjs').factory('Mutations', function (CoC, kFLAGS, EngineCore, StatusAffects, Utils, PerkLib, CockTypesEnum, Descriptors, AppearanceDefs, Appearance) {
	function Mutations() {
	...
	Mutations.prototype.ceruleanPotion = function(player) {
		...
		if (player.gender == 0 && CoC.getInstance().flags[kFLAGS.CERULEAN_POTION_NEUTER_ATTEMPTED] > 0) {
			...
			if (player.findStatusAffect(StatusAffects.RepeatSuccubi) >= 0) {
				EngineCore.outputText("You look forwards to tonight's encounter.", false);
			} else { //First timer huh?
				EngineCore.outputText("You do not notice any real effects.  Did the merchant con you?", false);
			}
		}
	...
	Mutations.prototype.vitalityTincture = function(player) {
		...
		if (EngineCore.HPChange(50, false)) {
			EngineCore.outputText("  Any aches, pains and bruises you have suffered no longer hurt and you feel much better.", false);
		}
		if ( Utils.rand(3) == 0) {
			EngineCore.outputText(player.modTone(95, 3), false);
		}
	};
	//Scholar's Tea
	Mutations.prototype.scholarsTea = function(player) {
		...
		if ( Utils.rand(3) == 0) {
			EngineCore.outputText(player.modTone(15, 1), false);
		}
	...
	Mutations.prototype.incubiDraft = function(tainted,player) {
		...
		EngineCore.outputText("The draft is slick and sticky, ", true);
		if (player.cor <= 33) {
			EngineCore.outputText("just swallowing it makes you feel unclean.", false);
		} else if(player.cor <= 66) {
			EngineCore.outputText("reminding you of something you just can't place.", false);
		} else {
			EngineCore.outputText("deliciously sinful in all the right ways.", false);
		}
		if (player.cor >= 90) {
			EngineCore.outputText("  You're sure it must be distilled from the cum of an incubus.", false);
		}
		var effect = 0;
		//Lowlevel changes
		if (changesLevel < 50) {
			if (player.cocks.length == 1) {
				...
				if (effect < 0.5) {
					EngineCore.outputText("  It stops almost as soon as it starts, growing only a tiny bit longer.", false);
				} else if(effect < 1) {
					EngineCore.outputText("  It grows slowly, stopping after roughly half an inch of growth.", false);
				} else if(effect <= 2) {
					EngineCore.outputText("  The sensation is incredible as more than an inch of lengthened dick-flesh grows in.", false);
				} else {
					EngineCore.outputText("  You smile and idly stroke your lengthening " + Descriptors.cockDescript(0) + " as a few more inches sprout.", false);
				}
				if (tainted) {
				...
				if (player.cocks[0].cockType != CockTypesEnum.DEMON) {
					EngineCore.outputText("  With the transformation complete, your " + Descriptors.cockDescript(0) + " returns to its normal coloration.", false);
				} else {
					EngineCore.outputText("  With the transformation complete, your " + Descriptors.cockDescript(0) + " throbs in an almost happy way as it goes flaccid once more.", false);
				}
				...
				if (effect < 0.5) {
					...
					EngineCore.outputText("  You smile and idly stroke your lengthening " + Descriptors.cockDescript(shortestCockIndex) + " as a few more inches sprout.", false);
				}
				EngineCore.outputText("  With the transformation complete, your " + Descriptors.multiCockDescriptLight() + " return to their normal coloration.", false);
				...
				EngineCore.outputText("The skin seems to fold back as a fully formed demon-cock bursts forth from your loins, drizzling hot cum everywhere as it orgasms.  Eventually the orgasm ends as your " + Descriptors.cockDescript(0) + " fades to a more normal " + player.skinTone + " tone.", false);
				if (tainted) {
			...
			if (player.cocks.length > 1) {
				...
				//Display the degree of thickness change.
				if (maxThicknessChange >= 1) {
					EngineCore.outputText("  Your cock spreads rapidly, swelling an inch or more in girth, making it feel fat and floppy.", false);
				}else if (maxThicknessChange <= 0.5) {
					EngineCore.outputText("  Your cock feels swollen and heavy. With a firm, but gentle, squeeze, you confirm your suspicions. It is definitely thicker.", false);
				}
				if (maxThicknessChange > 0.5 && maxLengthChange < 1) {
					EngineCore.outputText("  Your cock seems to swell up, feeling heavier. You look down and watch it growing fatter as it thickens.", false);
				}
				if (tainted) {
					EngineCore.dynStats("lib", 3, "sen", 5, "lus", 10, "cor", 3);
				} else {
		...
		if ( Utils.rand(4) == 0 && tainted) {
			EngineCore.outputText(player.modFem(5, 2), false);
		}
		if ( Utils.rand(4) == 0 && tainted) {
	...
	Mutations.prototype.growDemonCock = function(growCocks, player) {
		...
		EngineCore.outputText("\n\nYou shudder as a pressure builds in your crotch, peaking painfully as a large bulge begins to push out from your body.  ", false);
		if (growCocks == 1) {
			EngineCore.outputText("The skin seems to fold back as a fully formed demon-cock bursts forth from your loins, drizzling hot cum everywhere as it orgasms.  In time it fades to a more normal coloration and human-like texture.  ", false);
		} else {
			EngineCore.outputText("The skin bulges obscenely, darkening and splitting around " + Utils.num2Text(growCocks) + " of your new dicks.  For an instant they turn a demonic purple and dribble in thick spasms of scalding demon-cum.  After, they return to a more humanoid coloration.  ", false);
		}
		if (growCocks > 4) {
			EngineCore.outputText("Your tender bundle of new cocks feels deliciously sensitive, and you cannot stop yourself from wrapping your hands around the slick demonic bundle and pleasuring them.\n\nNearly an hour later, you finally pull your slick body away from the puddle you left on the ground.  When you look back, you notice it has already been devoured by the hungry earth.", false);
		}
		player.orgasm();
	};
		...
		if (player.hairColor == "sandy blonde") {
			...
			if (player.biggestLactation() > 0) {
				...
				if (player.biggestLactation() < 2) {
					EngineCore.outputText("powerful jets of milk spray from your nipples, spraying thick streams over the ground.  You moan at the sensation and squeeze your tits, hosing down the tainted earth with an offering of your milk.  You blush as the milk ends, quite embarassed with your increased milk production.  ", false);
					...
					EngineCore.outputText("thick hoses of milk erupt from your aching nipples, forming puddles on the ground.  You smile at how well you're feeding the earth, your milk coating the ground faster than it can be absorbed.  The constant lactation is pleasurable... in a highly erotic way, and you find yourself moaning and pulling on your nipples, your hands completely out of control.  In time you realize the milk has stopped, and even had time to soak into the dirt.  You wonder at your strange thoughts and pull your hands from your sensitive nipples.  ", false);
				} else {
				...
				EngineCore.outputText("A pleasurable release suddenly erupts from your nipples!  Twin streams of milk are spraying from your breasts, soaking into the ground immediately.  It stops all too soon, though a voice in your head assures you that you can lactate quite often now.  ", false);
	...
	Mutations.prototype.minotaurCum = function(player) {
		...
		EngineCore.outputText("  Unbidden, your hand brings the bottle to your lips, and the heady taste fills your mouth as you convulsively swallow the entire bottle.", false);
		//-Raises lust by 10.
		//-Raises sensitivity
		...
		if (player.cocks.length > 0) {
			...
			EngineCore.outputText("our " + Descriptors.multiCockDescriptLight() + " aches, flooding with blood until it's bloating and trembling.", false);
		}
		if (player.hasVagina()) {
			EngineCore.outputText("  Your " + Descriptors.clitDescript() + " engorges, ", false);
			if (player.clitLength < 3) {
				EngineCore.outputText("parting your lips.", false);
			} else {
				EngineCore.outputText("bursting free of your lips and bobbing under its own weight.", false);
			}
			if (player.vaginas[0].vaginalWetness <= AppearanceDefs.VAGINA_WETNESS_NORMAL) {
				EngineCore.outputText("  Wetness builds inside you as your " + Descriptors.vaginaDescript(0) + " tingles and aches to be filled.", false);
			} else if (player.vaginas[0].vaginalWetness <= AppearanceDefs.VAGINA_WETNESS_SLICK) {
				EngineCore.outputText("  A trickle of wetness escapes your " + Descriptors.vaginaDescript(0) + " as your body reacts to the desire burning inside you.", false);
			} else if (player.vaginas[0].vaginalWetness <= AppearanceDefs.VAGINA_WETNESS_DROOLING) {
				EngineCore.outputText("  Wet fluids leak down your thighs as your body reacts to this new stimulus.", false);
			} else {
				EngineCore.outputText("  Slick fluids soak your thighs as your body reacts to this new stimulus.", false);
			}
		}
		//(Minotaur fantasy)
	...
	Mutations.prototype.minotaurBlood = function(player) {
		...
		//Set up output
		EngineCore.outputText("You drink the bubbling red fluid, tasting the tangy iron after-taste.", true);
		//STATS
		//Strength h
		if ( Utils.rand(3) == 0) {
		...
		if (player.lowerBody != AppearanceDefs.LOWER_BODY_TYPE_HOOFED && player.lowerBody != AppearanceDefs.LOWER_BODY_TYPE_CENTAUR) {
			if (changes < changeLimit && Utils.rand(3) == 0) {
				...
				if (player.skinType != AppearanceDefs.SKIN_TYPE_FUR) {
					EngineCore.outputText("  A fine coat of fur grows out below your waist, itching briefly as it fills in.");
				}
				EngineCore.outputText("<b>  You now have hooves in place of your feet!</b>", false);
		...
		if (!CoC.getInstance().flags[kFLAGS.HYPER_HAPPY]) {
			...
			if (player.vaginas.length > 0) {
				if (player.vaginas[0].vaginalLooseness > AppearanceDefs.VAGINA_LOOSENESS_TIGHT) {
					...
					if (player.cocks.length == 0) {
						EngineCore.outputText("  Strangely, your clit seems to have resisted the change, and is growing larger by the moment... shifting into the shape of a small ribbed minotaur-like penis!  <b>You now have a horse-cock!</b>", false);
						player.createCock();
			...
			if (changes < changeLimit && player.bRows() > 1 && Utils.rand(3) == 0) {
				...
				if (player.bRows() >= 3) {
					EngineCore.outputText("abdomen", false);
				} else {
					EngineCore.outputText("chest", false);
				}
				EngineCore.outputText(". The " + Descriptors.nippleDescript(player.breastRows.length - 1) + "s even fade until nothing but ", false);
				if (player.skinType == AppearanceDefs.SKIN_TYPE_FUR) {
					EngineCore.outputText(player.hairColor + " " + player.skinDesc, false);
				} else {
					EngineCore.outputText(player.skinTone + " " + player.skinDesc, false);
				}
				...
				if (player.breastRows.length == 1) {
					...
					_.forEach(player.breastRows, function(breastRow, index) {
						if (breastRow.breastRating >= 1) {
							...
							EngineCore.outputText("our " + Descriptors.breastDescript(index) + " shrink, dropping to " + player.breastCup(index) + "s.", false);
						}
					});
					if (changedBreasts == 2) {
		...
		if (changes < changeLimit && Utils.rand(2) == 0 && player.cocks.length > 0) {
			...
			if (selectedCock != -1) {
				...
				if (player.cocks[selectedCock].cockThickness < 5) {
					...
					EngineCore.outputText("  To your delight and surprise, you discover it has grown slightly thicker as well!", false);
				} else { //Just length...
					//Increase by 2 + Utils.rand(8), and store the actual amount in result
					result = player.increaseCock(selectedCock, 2 + Utils.rand(8));
					//Comment on length changes
		...
		if (player.cocks.length > 0 && Utils.rand(2) == 0 && changes < changeLimit) {
			...
			if (selectedCock != -1) {
				...
				EngineCore.outputText("<b>  You now have a");
				if (player.horseCocks() > 1) {
					EngineCore.outputText("nother")
				}
				EngineCore.outputText(" horse-penis.</b>", false);
				changes++;
			}
		}
		//Males go into rut
		...
		if (player.earType == AppearanceDefs.EARS_COW && player.lowerBody == AppearanceDefs.LOWER_BODY_TYPE_HOOFED && player.tallness >= 90
				...
				if (player.hornType == AppearanceDefs.HORNS_COW_MINOTAUR) {
					...
					if (player.vaginas.length > 0) {
						if (player.horns > 4) {
							...
							EngineCore.outputText("Your headache clears as lust washes through you unnaturally.  You feel as if you haven't cum in months.", false);
						...
						if (player.horns < 3) {
							EngineCore.outputText("They are the size of tiny nubs.", false);
						} else if (player.horns < 6) {
							EngineCore.outputText("They are similar to what you would see on a young bull.", false);
						} else if (player.horns < 12) {
							EngineCore.outputText("They look like the horns on a grown bull, big enough and dangerous enough to do some damage.", false);
						} else if (player.horns < 20) {
							EngineCore.outputText("They are large and wicked looking.", false);
						} else {
							EngineCore.outputText("They are huge, heavy, and tipped with dangerous points.", false);
						}
		...
		if ( Utils.rand(4) == 0) {
			EngineCore.outputText(player.modFem(5, 10), false);
		}
		if ( Utils.rand(4) == 0) {
			EngineCore.outputText(player.modTone(85, 3), false);
		}
		if ( Utils.rand(4) == 0) {
			EngineCore.outputText(player.modThickness(70, 4), false);
		}
		//Default
		if (changes == 0) {
			EngineCore.outputText("\n\nMinotaur-like vitality surges through your body, invigorating and arousing you!\n", false);
			if (player.balls > 0) {
				EngineCore.outputText("Your balls feel as if they've grown heavier with the weight of more sperm.\n", false);
				player.hoursSinceCum += 200;
			}
	...
	Mutations.prototype.equinum = function(player) {
		...
		//Set up output
		EngineCore.outputText("You down the potion, grimacing at the strong taste.", true);
					...
					if (player.cockTotal() > 0) {
						...
						if (player.horseCocks() > 0) {
							...
							if (player.gender == 0 || player.gender == 1) {
								EngineCore.outputText("He is clearly a male, but you are somewhat confused as you can see not one but three bulges where his manhood would be.\n\n", false);
							} else if (player.gender == 2) {
								EngineCore.outputText("She is clearly a female, as you can see her six breasts jiggle as she walks towards you, small stains appearing on her shirt where her nipples are.\n\n", false);
							} if (player.gender == 3) {
		...
		if ( Utils.rand(2) == 0) {
			...
			if (player.tou >= 75) {
				...
				if (player.gender == 1 || player.gender == 3) {
					EngineCore.outputText("stallion's.", false);
				} else {
					EngineCore.outputText("mare's.", false);
				}
			} else { //NOT MAXXED
		...
		if ((player.gender == 1 || player.gender == 3) && Utils.rand(1.5) == 0 && changes < changeLimit) {
			...
			if ((player.horseCocks() + player.demonCocks()) < player.cocks.length) {
				...
				if (player.cocks.length == 1) {
					...
					if (hasChange) {
						EngineCore.outputText("  <b>Your penis has transformed into a horse's!</b>", false);
					}
				} else { //MULTICOCK
					EngineCore.dynStats("lib", 5, "sen", 4, "lus", 35);
					cockIndex = player.addHorseCock();
					...
					if (cockIndex == -1) {
						CoC_Settings.error("");
						EngineCore.outputText("FUKKKK ERROR NO COCK XFORMED", true);
					}
					//Already have a sheath
					if (player.horseCocks() > 1 || player.dogCocks() > 0) {
						EngineCore.outputText("  Your sheath tingles and begins growing larger as the cock's base shifts to lie inside it.", false);
					} else {
						EngineCore.outputText("  You feel a tightness near the base where your skin seems to be bunching up.  A sheath begins forming around your " + Descriptors.cockDescript(cockIndex) + "'s root, tightening and pulling your " + Descriptors.cockDescript(cockIndex) + " inside its depths.", false);
					}
					player.increaseCock(cockIndex, Utils.rand(4) + 4);
					EngineCore.outputText("  The shaft suddenly explodes with movement, growing longer and developing a thick flared head leaking steady stream of animal-cum.", false);
					EngineCore.outputText("  <b>You now have a horse-cock.</b>", false);
				}
				...
				if (cockModifications > 2) {
					EngineCore.outputText("Your " + Descriptors.cockDescript(cockModified) + " tightens painfully, inches of taut horse-flesh pouring out from your sheath as it grows longer.  Thick animal-pre forms at the flared tip, drawn out from the pleasure of the change.", false);
				} else if (cockModifications > 1) {
					EngineCore.outputText("Aching pressure builds within your sheath, suddenly releasing as an inch or more of extra dick flesh spills out.  A dollop of pre beads on the head of your enlarged " + Descriptors.cockDescript(cockModified) + " from the pleasure of the growth.", false);
				} else {
					EngineCore.outputText("A slight pressure builds and releases as your " + Descriptors.cockDescript(cockModified) + " pushes a bit further out of your sheath.", false);
				}
				changes++;
			}
			...
			if (Utils.rand(2) == 0 && changes < changeLimit && player.horseCocks() > 0) {
				...
				player.cocks[lessThickCockIndex].thickenCock(.5);
				...
				if (player.cor + player.lib < 50) {
					EngineCore.outputText(" to have such a splendid tool.  You idly daydream about cunts and pussies, your " + Appearance.cockNoun(CockTypesEnum.HORSE) + " plowing them relentlessly, stuffing them pregnant with cum", false);
				} else if (player.cor + player.lib < 80) {
					EngineCore.outputText(" to be this way... You breath the powerful animalistic scent and fantasize about fucking centaurs night and day until their bellies slosh with your cum", false);
				} else if (player.cor + player.lib <= 125) {
					EngineCore.outputText(" to be a rutting stud.  You ache to find a mare or centaur to breed with.  Longing to spend your evenings plunging a " + Appearance.cockNoun(CockTypesEnum.HORSE) + " deep into their musky passages, dumping load after load of your thick animal-cum into them.  You'd be happy just fucking horsecunts morning, noon, and night.  Maybe somewhere there is a farm needing a breeder..", false);
				} else {
					EngineCore.outputText(" to whinny loudly like a rutting stallion.  Your " + Appearance.cockNoun(CockTypesEnum.HORSE) + " is perfect for fucking centaurs and mares.  You imagine the feel of plowing an equine pussy deeply, bottoming out and unloading sticky jets of horse-jizz into its fertile womb.  Your hand strokes your horsecock of its own accord, musky pre dripping from the flared tip with each stroke.  Your mind wanders to the thought of you with a harem of pregnant centaurs.", false);
				}
				EngineCore.outputText(".", false);
				if (player.cor < 30) {
					EngineCore.outputText("  You shudder in revulsion at the strange thoughts and vow to control yourself better.", false);
				} else if (player.cor < 60) {
					EngineCore.outputText("  You wonder why you thought such odd things, but they have a certain appeal.", false);
				} else if (player.cor < 90) {
					EngineCore.outputText("  You relish your twisted fantasies, hoping to dream of them again.", false);
				} else {
					EngineCore.outputText("  You flush hotly and give a twisted smile, resolving to find a fitting subject to rape and relive your fantasies.", false);
				}
		...
		if (player.gender == 2 || player.gender == 3) {
			...
			if (player.vaginas.length == 1) {
				...
				if (player.vaginas[leastWetVaginaIndex].vaginalWetness <= VAGINA_WETNESS_NORMAL && changes < changeLimit && Utils.rand(2) == 0) {
				...
				if (player.vaginas[0].vaginalLooseness <= VAGINA_LOOSENESS_GAPING && changes < changeLimit && Utils.rand(2) == 0) {
		...
		if (changes < changeLimit && Utils.rand(6) == 0 && player.lowerBody == AppearanceDefs.LOWER_BODY_TYPE_HOOFED) {
			...
			if (player.gender > 0) {
				EngineCore.outputText("  After taking a moment to get used to your new body, you notice that your genitals now reside between the back legs on your centaur body.", false);
			}
		...
		if (changes < changeLimit && Utils.rand(5) == 0 && player.eyeType > AppearanceDefs.EYES_HUMAN) {
			if (player.eyeType == AppearanceDefs.EYES_BLACK_EYES_SAND_TRAP) {
				...
				if (player.eyeType == AppearanceDefs.EYES_FOUR_SPIDER_EYES) {
					EngineCore.outputText("  Your multiple, arachnid eyes are gone!</b>", false);
				}
				EngineCore.outputText("  <b>You have normal, humanoid eyes again.</b>", false);
			}
		...
		if (player.earType != AppearanceDefs.EARS_HORSE && player.tailType == AppearanceDefs.TAIL_TYPE_HORSE && changes < changeLimit && Utils.rand(3) == 0) {
			...
			player.earValue = 0;
			EngineCore.outputText("<b>You now have horse ears.</b>", false);
			changes++;
		}
		//Tail - no-prereq
		if (player.tailType != AppearanceDefs.TAIL_TYPE_HORSE && Utils.rand(2) == 0 && changes < changeLimit) {
			...
			EngineCore.outputText("  <b>You now have a horse-tail.</b>", false);
		...
		if ( Utils.rand(3) == 0) {
			EngineCore.outputText(player.modTone(60, 1), false);
		}
		//FAILSAFE CHANGE
		if (changes == 0) {
	...
	Mutations.prototype.succubiMilk = function(tainted, player) {
		...
		if (player.cor < 35) {
			EngineCore.outputText("You wonder why in the gods' names you would drink such a thing, but you have to admit, it is the best thing you have ever tasted.", true);
		} else if (player.cor < 70) {
			EngineCore.outputText("You savor the incredible flavor as you greedily gulp it down.", true);
			if (player.gender == 2 || player.gender == 3) {
				EngineCore.outputText("  The taste alone makes your " + Descriptors.vaginaDescript(0) + " feel ", false);
				if (player.vaginas[0].vaginalWetness == AppearanceDefs.VAGINA_WETNESS_DRY) {
					EngineCore.outputText("tingly.", false);
					...
					EngineCore.outputText("sloppy and wet.", false);
				} else if (player.vaginas[0].vaginalWetness == AppearanceDefs.VAGINA_WETNESS_SLICK) {
					EngineCore.outputText("sopping and juicy.", false);
				} else if (player.vaginas[0].vaginalWetness >= AppearanceDefs.VAGINA_WETNESS_DROOLING) {
					EngineCore.outputText("dripping wet.", false);
				}
			} else if (player.hasCock()) {
				EngineCore.outputText("  You feel a building arousal, but it doesn't affect your cock.", false);
			}
		} else {
			EngineCore.outputText("You pour the milk down your throat, chugging the stuff as fast as you can.  You want more.", true);
			if (player.gender == 2 || player.gender == 3) {
				EngineCore.outputText("  Your " + Descriptors.vaginaDescript(0), false);
				if (player.vaginas.length > 1) {
					EngineCore.outputText(" quiver in orgasm, ", false);
				} else if (player.vaginas.length == 1) {
					EngineCore.outputText(" quivers in orgasm, ", false);
				}
				if (player.vaginas[0].vaginalWetness == AppearanceDefs.VAGINA_WETNESS_DRY) {
					EngineCore.outputText("becoming slightly sticky.", false);
				} else if (player.vaginas[0].vaginalWetness == AppearanceDefs.VAGINA_WETNESS_NORMAL) {
					EngineCore.outputText("leaving your undergarments sticky.", false);
				} else if (player.vaginas[0].vaginalWetness == AppearanceDefs.VAGINA_WETNESS_WET) {
					EngineCore.outputText("wet with girlcum.", false);
				} else if (player.vaginas[0].vaginalWetness == AppearanceDefs.VAGINA_WETNESS_SLICK) {
					EngineCore.outputText("staining your undergarments with cum.", false);
				} else if (player.vaginas[0].vaginalWetness == AppearanceDefs.VAGINA_WETNESS_DROOLING) {
					EngineCore.outputText("leaving cunt-juice trickling down your leg.", false);
				} else if (player.vaginas[0].vaginalWetness >= AppearanceDefs.VAGINA_WETNESS_SLAVERING) {
					EngineCore.outputText("spraying your undergarments liberally with slick girl-cum.", false);
				}
				player.orgasm();
			} else if (player.gender != 0) {
				if (player.cocks.length == 1) {
					EngineCore.outputText("  You feel a strange sexual pleasure, but your " + Descriptors.multiCockDescript() + " remains unaffected.", false);
				} else {
					EngineCore.outputText("  You feel a strange sexual pleasure, but your " + Descriptors.multiCockDescript() + " remain unaffected.", false);
				}
			}
		}
		if (tainted) {
		...
		if (changesLevel <= 75) {
			...
			if (player.breastRows.length == 0) {
				EngineCore.outputText("A perfect pair of B cup breasts, complete with tiny nipples, form on your chest.", false);
				player.createBreastRow();
				player.breastRows[0].breasts = 2;
			...
			if (!CoC.getInstance().flags[kFLAGS.HYPER_HAPPY]) {
				...
				if (player.cocks.length > 0) {
					...
					temp3 += player.increaseCock(longestCockIndex, ( Utils.rand(3) + 1) * -1);
					EngineCore.outputText("\n\n", false);
			...
			if (player.vaginas.length > 0) {
				...
				if (player.vaginas[0].vaginalWetness == AppearanceDefs.VAGINA_WETNESS_SLAVERING) {
					if (player.vaginas.length == 1) {
						EngineCore.outputText("Your " + Descriptors.vaginaDescript(0) + " gushes fluids down your leg as you spontaneously orgasm.", false);
					} else {
						EngineCore.outputText("Your " + Descriptors.vaginaDescript(0) + "s gush fluids down your legs as you spontaneously orgasm, leaving a thick puddle of pussy-juice on the ground.  It is rapidly absorbed by the earth.", false);
					}
					player.orgasm();
					if (tainted) {
						EngineCore.dynStats("cor", 1);
					...
					if (player.vaginas.length == 1) {
						EngineCore.outputText("Your pussy feels hot and juicy, aroused and tender.  You cannot resist as your hands dive into your " + Descriptors.vaginaDescript(0) + ".  You quickly orgasm, squirting fluids everywhere.  <b>You are now a squirter</b>.", false);
					}
					if (player.vaginas.length > 1) {
						EngineCore.outputText("Your pussies feel hot and juicy, aroused and tender.  You cannot resist plunging your hands inside your " + Descriptors.vaginaDescript(0) + "s.  You quiver around your fingers, squirting copious fluids over yourself and the ground.  The fluids quickly disappear into the dirt.", false);
					}
					player.orgasm();
					if (tainted) {
						EngineCore.dynStats("cor", 1);
					...
					if (player.vaginas.length == 1) {
						EngineCore.outputText("You feel a sudden trickle of fluid down your leg.  You smell it and realize it's your pussy-juice.  Your " + Descriptors.vaginaDescript(0) + " now drools lubricant constantly down your leg.", false);
					}
					if (player.vaginas.length > 1) {
						EngineCore.outputText("You feel sudden trickles of fluids down your leg.  You smell the stuff and realize it's your pussies-juices.  They seem to drool lubricant constantly down your legs.", false);
					}
				} else if (player.vaginas[0].vaginalWetness == AppearanceDefs.VAGINA_WETNESS_WET) {
					EngineCore.outputText("You flush in sexual arousal as you realize how moist your cunt-lips have become.  Once you've calmed down a bit you realize they're still slick and ready to fuck, and always will be.", false);
				} else if (player.vaginas[0].vaginalWetness == AppearanceDefs.VAGINA_WETNESS_NORMAL) {
					if (player.vaginas.length == 1) {
						EngineCore.outputText("A feeling of intense arousal passes through you, causing you to masturbate furiously.  You realize afterwards that your " + Descriptors.vaginaDescript(0) + " felt much wetter than normal.", false);
					} else {
						EngineCore.outputText("A feeling of intense arousal passes through you, causing you to masturbate furiously.  You realize afterwards that your " + Descriptors.vaginaDescript(0) + " were much wetter than normal.", false);
					}
				} else if (player.vaginas[0].vaginalWetness == AppearanceDefs.VAGINA_WETNESS_DRY) {
					EngineCore.outputText("You feel a tingling in your crotch, but cannot identify it.", false);
				}
			...
			if (player.skinTone == "blue" || player.skinTone == "purple" || player.skinTone == "indigo" || player.skinTone == "shiny black") {
				if (player.vaginas.length > 0) {
					...
					if (player.clitLength > 3 && player.findPerk(PerkLib.BigClit) < 0) {
						EngineCore.outputText("  After some time it shrinks, returning to its normal aroused size.  You guess it can't get any bigger.", false);
					}
					if (player.clitLength > 5 && player.findPerk(PerkLib.BigClit) >= 0) {
						EngineCore.outputText("  Eventually it shrinks back down to its normal (but still HUGE) size.  You guess it can't get any bigger.", false);
					}
		...
		if (tainted) {
			EngineCore.outputText(player.modFem(100, 2), false);
			if ( Utils.rand(3) == 0) {
				EngineCore.outputText(player.modTone(15, 2), false);
			}
		} else {
			EngineCore.outputText(player.modFem(90, 1), false);
			if ( Utils.rand(3) == 0) {
				EngineCore.outputText(player.modTone(20, 2), false);
			}
		}
		player.genderCheck();
	};
...
//5-Bulbous Pepper (+ball size or fresh balls)
	Mutations.prototype.caninePepper = function(type, player) {
		...
		if (type == 0) {
			if ( Utils.rand(100) < 15) {
				...
				EngineCore.outputText("The pepper tastes particularly potent, searingly hot and spicy.", false);
			} else {
				EngineCore.outputText("The pepper is strangely spicy but very tasty.", false);
			}
		} else if (type == 1) { //Oversized pepper
			crit = Utils.rand(20) / 10 + 2;
			EngineCore.outputText("The pepper is so large and thick that you have to eat it in several large bites.  It is not as spicy as the normal ones, but is delicious and flavorful.", false);
		}else if (type == 2) { //Double Pepper
			crit = Utils.rand(20) / 10 + 2;
			EngineCore.outputText("The double-pepper is strange, looking like it was formed when two peppers grew together near their bases.", false);
		} else if (type == 3) { //Black Pepper
			crit = Utils.rand(20) / 10 + 2;
			EngineCore.outputText("This black pepper tastes sweet, but has a bit of a tangy aftertaste.", false);
		} else if (type == 4) { //Knotty Pepper
			crit = Utils.rand(20) / 10 + 2;
			EngineCore.outputText("The pepper is a bit tough to eat due to the swollen bulge near the base, but you manage to cram it down and munch on it.  It's extra spicy!", false);
		} else if (type == 5) { //Bulbous Pepper
			crit = Utils.rand(20) / 10 + 2;
			EngineCore.outputText("You eat the pepper, even the two orb-like growths that have grown out from the base.  It's delicious!", false);
		}
		//OVERDOSE Bad End!
		if (type <= 0 && crit > 1 && player.skinType == AppearanceDefs.SKIN_TYPE_FUR && player.faceType == AppearanceDefs.FACE_DOG && player.earType == AppearanceDefs.EARS_DOG && player.lowerBody == AppearanceDefs.LOWER_BODY_TYPE_DOG && player.tailType == AppearanceDefs.TAIL_TYPE_DOG && Utils.rand(2) == 0 && player.findStatusAffect(StatusAffects.DogWarning) >= 0) {
			if (Utils.rand(2) == 0) {
				...
				if (player.findPerk(PerkLib.MarblesMilk) >= 0) {
					EngineCore.outputText("All you know is that there is a scent on the wind, it is time to hunt, and at the end of the day you need to come home for your milk.", false);
				} else {
					EngineCore.outputText("All you know is that there is a scent on the wind, and it is time to hunt.", false);
				}
			} else {
			...
			if (player.cor < 50) {
				EngineCore.outputText("  You shake your head, blushing hotly.  Where did that thought come from?", false);
			}
		}
		...
		if (player.inte > 30 && Utils.rand(3) == 0 && changes < changeLimit && type != 3) {
			...
			EngineCore.outputText("\n\nYou feel ", false);
			if (crit > 1) {
				EngineCore.outputText("MUCH ", false);
			}
			EngineCore.outputText("dumber.", false);
			changes++;
		}
		//-Remove feather-arms (copy this for goblin ale, mino blood, equinum, canine pepps, demon items)
		if (changes < changeLimit && player.armType == AppearanceDefs.ARM_TYPE_HARPY && Utils.rand(4) == 0) {
		...
		if (type == 2) {
			...
			if (player.cockTotal() > 0) {
				...
				if (player.dogCocks() > 0) {
					...
					EngineCore.outputText("\n\n", false);
					if (knotChange < 0.06) {
						EngineCore.outputText("Your " + Appearance.cockNoun(CockTypesEnum.DOG) + " feels unusually tight in your sheath as your knot grows.", false);
					} else if (knotChange <= 0.12) {
						EngineCore.outputText("Your " + Appearance.cockNoun(CockTypesEnum.DOG) + " pops free of your sheath, thickening nicely into a bigger knot.", false);
					} else {
						EngineCore.outputText("Your " + Appearance.cockNoun(CockTypesEnum.DOG) + " surges free of your sheath, swelling thicker with each passing second.  Your knot bulges out at the base, growing far beyond normal.", false);
					}
		...
		if (player.cocks.length > 0) {
			...
			if (player.dogCocks() < player.cocks.length && ((changes < changeLimit && Utils.rand(1.6)) || type == 1) == 0) {
				...
				if (player.cocks[nonDogCockIndex].cockType == CockTypesEnum.HUMAN) {
					...
					if (player.cocks[nonDogCockIndex].cockLength > 6) player.cocks[nonDogCockIndex].cockLength -= 2;
					else player.cocks[nonDogCockIndex].cockLength -= 0.5;
			...
			if (player.cumMultiplier < 2 && Utils.rand(2) == 0 && changes < changeLimit) {
				...
				if ((player.findPerk(PerkLib.MessyOrgasms) >= 0 ? 2 : 1.5) < player.cumMultiplier + 0.05 * crit) {
					...
					if (crit > 1) {
						EngineCore.outputText("  A bit of milky pre dribbles from your " + Descriptors.multiCockDescriptLight() + ", pushed out by the change.", false);
					}
				}
				changes++;
			}
			...
			if (type == 1) {
				...
				if (player.cocks.length == 1) {
					...
					EngineCore.dynStats("sen", 1, "lus", 10);
					if (player.cocks[modifiedCockIndex].cockThickness <= 2) player.cocks[modifiedCockIndex].thickenCock(1);
				}
				if (cockModifications > 2) {
		...
		if (player.breastRows.length > 0) {
			...
			if (player.breastRows[0].breastRating > 0 && player.vaginas.length > 0) {
				...
				if (player.breastRows.length < 3 && Utils.rand(2) == 0 && changes < changeLimit) {
					...
					if (player.vaginas.length > 0 && player.breastRows[0].breastRating <= player.breastRows.length) {
						...
						player.breastRows[0].breastRating += 2;
						EngineCore.outputText(player.breastCup(0) + " size.  But it doesn't stop there, you feel a tightness beginning lower on your torso...", false);
						changes++;
					}
					//Had 1 row to start
					if (player.breastRows.length == 2) {
						...
						EngineCore.outputText("  A sensitive nub grows on the summit of each new tit, becoming a new nipple.", false);
					...
					if (crit > 1) {
						if (crit > 2) {
							EngineCore.outputText("  You heft your new chest experimentally, exploring the new flesh with tender touches.  Your eyes nearly roll back in your head from the intense feelings.", false);
							EngineCore.dynStats("sen", 6, "lus", 15, "cor", 0)
						} else {
							EngineCore.outputText("  You touch your new nipples with a mixture of awe and desire, the experience arousing beyond measure.  You squeal in delight, nearly orgasming, but in time finding the willpower to stop yourself.", false);
			...
			EngineCore.outputText("  A sensitive nub grows on the summit of each tit, becoming a new nipple.", false);
			player.createBreastRow();
			player.breastRows[0].breastRating = 2;
			player.breastRows[0].breasts = 2;
		...
		if (changes < changeLimit && player.dogScore() >= 3 && Utils.rand(4) == 0) {
			changes++;
			EngineCore.outputText("\n\n", false);
			EngineCore.outputText("Images and thoughts come unbidden to your mind, overwhelming your control as you rapidly lose yourself in them, daydreaming of... ", false);
			//cawk fantasies
			if (player.gender <= 1 || (player.gender == 3 && Utils.rand(2) == 0)) {
				EngineCore.outputText("bounding through the woods, hunting with your master.  Feeling the wind in your fur and the thrill of the hunt coursing through your veins intoxicates you.  You have your nose to the ground, tracking your quarry as you run, until a heavenly scent stops you in your tracks.", false);
				...
				if (player.cor < 33 || !player.hasCock()) {
					...
					EngineCore.outputText("  Heart pounding, your shaft pops free of its sheath on instinct, as you take off after the new scent.  Caught firmly in the grip of a female's heat, you ignore your master's cry as you disappear into the wild, " + Appearance.cockNoun(CockTypesEnum.DOG) + " growing harder as you near your quarry.  You burst through a bush, spotting a white-furred female.  She drops, exposing her dripping fem-sex to you, the musky scent of her sex channeling straight through your nose and sliding into your " + Appearance.cockNoun(CockTypesEnum.DOG) + ".", false);
					EngineCore.dynStats("lus", 5 + player.lib / 20);
					//Break 2
					if (player.cor < 66) {
						EngineCore.outputText("\nYou blink a few times, the fantasy fading as you master yourself.  That daydream was so strange, yet so hot.", false);
					} else {
						EngineCore.outputText("  Unable to wait any longer, you mount her, pressing your bulging knot against her vulva as she yips in pleasure. The heat of her sex is unreal, the tight passage gripping you like a vice as you jackhammer against her, biting her neck gently in spite of the violent pounding.", false);
						EngineCore.dynStats("lus", 5 + player.lib / 20);
						//break3
						if (player.cor < 80) {
							if (player.vaginas.length > 0) {
				...
				EngineCore.outputText("wagging your dripping " + Descriptors.vaginaDescript(0) + " before a pack of horny wolves, watching their shiny red doggie-pricks practically jump out of their sheaths at your fertile scent.", false);
				EngineCore.dynStats("lus", 5 + player.lib / 20);
				//BREAK 1
				if (player.cor < 33) {
					EngineCore.outputText("\nYou shake your head to clear the unwanted fantasy from your mind, repulsed by it.", false);
				} else {
					EngineCore.outputText("  In moments they begin their advance, plunging their pointed beast-dicks into you, one after another.  You yip and howl with pleasure as each one takes his turn knotting you.", false);
					EngineCore.dynStats("lus", 5 + player.lib / 20);
					//BREAK 2
					if (player.cor <= 66) {
						EngineCore.outputText("\nYou blink a few times, the fantasy fading as you master yourself.  That daydream was so strange, yet so hot.", false);
					} else {
						EngineCore.outputText("  The feeling of all that hot wolf-spooge spilling from your overfilled snatch and running down your thighs is heavenly, nearly making you orgasm on the spot.  You see the alpha of the pack is hard again, and his impressive member is throbbing with the need to breed you.", false);
						EngineCore.dynStats("lus", 5 + player.lib / 20);
						//break3
						if (player.cor < 80) {
							EngineCore.outputText("\nYou reluctantly pry your hand from your aching " + Descriptors.vaginaDescript(0) + " as you drag yourself out of your fantasy.", false);
						} else {
							EngineCore.outputText("  You growl with discomfort as he pushes into your abused wetness, stretching you tightly, every beat of his heart vibrating through your nethers.  With exquisite force, he buries his knot in you and begins filling you with his potent seed, impregnating you for sure. Your knees give out as your fantasy nearly brings you to orgasm, the sudden impact with the ground jarring you from your daydream.", false);
							EngineCore.dynStats("lus", 5 + player.lib / 20);
						}
					}
				}
			} else {
				EngineCore.outputText("wagging your [asshole] before a pack of horny wolves, watching their shiny red doggie-pricks practically jump out of their sheaths at you after going so long without a female in the pack.");
				EngineCore.dynStats("lus", 5 + player.lib / 20);
				//BREAK 1
				if (player.cor < 33) {
					EngineCore.outputText("\nYou shake your head to clear the unwanted fantasy from your mind, repulsed by it.", false);
					...
					if (player.cor <= 66) {
						...
						EngineCore.outputText("  The feeling of all that hot wolf-spooge spilling from your overfilled ass and running down your thighs is heavenly, nearly making you orgasm on the spot.  You see the alpha of the pack is hard again, and his impressive member is throbbing with the need to spend his lust on you.", false);
						EngineCore.dynStats("lus", 5 + player.lib / 20);
						//break3
						if (player.cor < 80) {
		...
		if ( Utils.rand(2) == 0 && player.earType != AppearanceDefs.EARS_DOG && player.tailType == AppearanceDefs.TAIL_TYPE_DOG && changes < changeLimit) {
			...
			player.earValue = 2;
			EngineCore.outputText("<b>You now have dog ears.</b>", false);
			changes++;
		}
		...
		if ( Utils.rand(3) == 0 && changes < changeLimit && player.tailType != AppearanceDefs.TAIL_TYPE_DOG) {
			...
			EngineCore.outputText("<b>You now have a dog-tail.</b>", false);
		}
		if ( Utils.rand(4) == 0 && player.gills && changes < changeLimit) {
			EngineCore.outputText("\n\nYour chest itches, and as you reach up to scratch it, you realize your gills have withdrawn into your skin.", false);
			player.gills = false;
	...
	Mutations.prototype.impFood = function(player) {
		...
		if (player.cocks.length > 0) {
			...
			if ( Utils.rand(30) == 0 && player.skinTone != "red") {
				...
				EngineCore.outputText("begins to lose its color, fading until you're as white as an albino.  Then, starting at the crown of your head, a reddish hue rolls down your body in a wave, turning you completely " + player.skinTone + ".", false);
			}
			return;
		} else {
			EngineCore.outputText("The food tastes... corrupt, for lack of a better word.\n", false);
	...
	Mutations.prototype.succubisDelight = function(tainted, player) {
		...
		EngineCore.outputText("You uncork the bottle and drink down the strange substance, struggling to down the thick liquid.", true);
		//low corruption thoughts
		if (player.cor < 33) {
			EngineCore.outputText("  This stuff is gross, why are you drinking it?", false);
		}
		//high corruption
		if (player.cor >= 66) {
			EngineCore.outputText("  You lick your lips, marvelling at how thick and sticky it is.", false);
		}
		//Corruption increase
	...
	Mutations.prototype.succubisDream = function(player) {
		...
		if (player.cor < 50 || Utils.rand(2)) {
			...
			EngineCore.dynStats("cor", temp + 2);
			changes++;
		}
		//NEW BALLZ
		if (player.balls < 4) {
		...
		if (player.balls > 0 && Utils.rand(3) == 0) {
			EngineCore.outputText(player.modFem(12, 5), false);
		}
	};
//butt expansion
	Mutations.prototype.brownEgg = function(large, player) {
		...
		if (!large) {
			...
			EngineCore.outputText("Your " + Descriptors.buttDescript() + " wobbles, nearly throwing you off balance as it grows much bigger!", true);
			player.buttRating += 2 + Utils.rand(3);
		}
		if ( Utils.rand(3) == 0) {
			if (large) {
				EngineCore.outputText(player.modThickness(100, 8), false);
			} else {
				EngineCore.outputText(player.modThickness(95, 3), false);
			}
		}
	};
//hip expansion
	Mutations.prototype.purpleEgg = function(large, player) {
		...
		if (!large || player.hipRating > 20) {
			...
			EngineCore.outputText("You stagger wildly as your hips spread apart, widening by inches.  When the transformation finishes you feel as if you have to learn to walk all over again.", false);
			player.hipRating += 2 + Utils.rand(2);
		}
		if ( Utils.rand(3) == 0) {
			if (large) {
				EngineCore.outputText(player.modThickness(80, 8), false);
			} else {
				EngineCore.outputText(player.modThickness(80, 3), false);
			}
		}
	};
//Femminess
		...
		if (!large) {
			...
			if (player.balls > 0) {
				if (player.ballSize > 15) {
					player.ballSize -= 8;
					EngineCore.outputText("Your scrotum slowly shrinks, settling down at a MUCH smaller size.  <b>Your " + Descriptors.ballsDescriptLight() + " are much smaller.</b>\n\n", false);
				} else {
					player.balls = 0;
					player.ballSize = 1;
					EngineCore.outputText("Your scrotum slowly shrinks, eventually disappearing entirely!  <b>You've lost your balls!</b>\n\n", false);
				}
			}
			//Fertility boost
			if (player.vaginas.length > 0 && player.fertility < 40) {
				EngineCore.outputText("You feel a tingle deep inside your body, just above your " + Descriptors.vaginaDescript(0) + ", as if you were becoming more fertile.\n\n", false);
				player.fertility += 5;
			}
		} else { //LARGE
			//Remove a dick
			...
			if (player.vaginas.length > 0 && player.fertility < 70) {
				EngineCore.outputText("You feel a powerful tingle deep inside your body, just above your " + Descriptors.vaginaDescript(0) + ". Instinctively you know you have become more fertile.\n\n", false);
				player.fertility += 10;
			}
		}
		if ( Utils.rand(3) == 0) {
			if (large) {
				EngineCore.outputText(player.modFem(100, 8), false);
			} else {
				EngineCore.outputText(player.modFem(95, 3), false);
			}
		}
	};
//Maleness
	Mutations.prototype.blueEgg = function(large, player) {
		...
		if (!large) {
			...
			//Kill pussies!
			if (player.vaginas.length > 0) {
				EngineCore.outputText("Your vagina clenches in pain, doubling you over.  You slip a hand down to check on it, only to feel the slit growing smaller and smaller until it disappears, taking your clit with it!\n\n", false);
			...
			//Kill extra boobages
			if (player.bRows() > 1) {
				EngineCore.outputText("Your back relaxes as extra weight vanishes from your chest.  <b>Your lowest " + Descriptors.breastDescript(player.bRows() - 1) + " have vanished.</b>", false);
			...
			if (player.buttRating > 5) {
				EngineCore.outputText("Muscles firm and tone as you feel your " + Descriptors.buttDescript() + " become smaller and tighter.", false);
			...
			if (player.hipRating > 5) {
				EngineCore.outputText("Feeling the sudden burning of lactic acid in your " + Descriptors.hipDescript() + ", you realize they have slimmed down and firmed up some.", false);
				player.hipRating -= 2;
			}
			//Shrink tits!
		...
		if ( Utils.rand(3) == 0) {
			if (large) {
				EngineCore.outputText(player.modFem(0, 8), false);
			} else {
				EngineCore.outputText(player.modFem(5, 3), false);
			}
		}
	};
//Nipplezzzzz
	...
	Mutations.prototype.blackRubberEgg = function(large, player) {
		...
		if (!large) {
			...
			if ((player.skinAdj != "smooth" && player.skinAdj != "latex" && player.skinAdj != "rubber") || player.skinDesc != "skin") {
				...
				if (player.skinType == AppearanceDefs.SKIN_TYPE_PLAIN) {
					EngineCore.outputText(" loses its blemishes, becoming flawless smooth skin.", false);
				} else if (player.skinType == AppearanceDefs.SKIN_TYPE_FUR) {
					EngineCore.outputText(" falls out in clumps, revealing smooth skin underneath.", false);
				} else if (player.skinType == AppearanceDefs.SKIN_TYPE_SCALES) {
					EngineCore.outputText(" begins dropping to the ground in a pile around you, revealing smooth skin underneath.", false);
				} else {
					EngineCore.outputText(" shifts and changes into flawless smooth skin.", false);
				}
			...
			if (player.skinDesc == "skin" && player.skinAdj == "smooth") {
				...
				if ( Utils.rand(2) == 0) {
					...
					EngineCore.outputText("a layer of pure latex.  ", false);
				} else {
					player.skinDesc = "skin";
				...
				if (player.cor < 66) {
					...
					EngineCore.outputText("You feel like some kind of sexy " + player.skinDesc + " love-doll.", false);
				}
	...
	Mutations.prototype.hairDye = function(color, player) {
		...
		if (player.hairLength == 0) {
			EngineCore.outputText("You rub the dye into your bald head, but it has no effect.", true);
			return;
		}
		EngineCore.outputText("You rub the dye into your " + Descriptors.hairDescript() + ", then use a bucket of cool lakewater to rinse clean a few minutes later.  ", true);
		player.hairColor = color;
		EngineCore.outputText("You now have " + Descriptors.hairDescript() + ".", false);
		if (player.lust > 50) {
	...
	Mutations.prototype.lactaid = function(player) {
		...
		if (player.biggestLactation() < 1) {
			EngineCore.outputText("\n\n", false);
			EngineCore.outputText("You feel your " + Descriptors.nippleDescript(0) + "s become tight and engorged.  A single droplet of milk escapes each, rolling down the curves of your breasts.  <b>You are now lactating!</b>", false);
			_.forEach(player.breastRows, function(breastRow) {
				breastRow.lactationMultiplier += 2;
			});
		} else { //Boost lactation
			EngineCore.outputText("\n\n", false);
			EngineCore.outputText("Milk leaks from your " + Descriptors.nippleDescript(0) + "s in thick streams.  You're lactating even more!", false);
			_.forEach(player.breastRows, function(breastRow) {
	...
	Mutations.prototype.useMarbleMilk = function(player) {
		...
		if (player.statusAffectv2(StatusAffects.Marble) < 30 && player.statusAffectv3(StatusAffects.Marble) == 0) {
			EngineCore.outputText("You gulp down the bottle's contents; Marble makes some good tasting milk.\n\n", false);
		} else if (player.statusAffectv3(StatusAffects.Marble) <= 0) { //[before the player is addicted, Addiction < 50]
			EngineCore.outputText("You gulp down the bottle's contents; Marble makes some really good tasting milk.\n\n", false);
		} else if (player.statusAffectv3(StatusAffects.Marble) > 0) {
			//[player is completely addicted]
			if (player.findPerk(PerkLib.MarblesMilk) >= 0) {
				EngineCore.outputText("You gulp down the bottle's contents; it's no substitute for the real thing, but it's a nice pick me up.\n\n", false);
			} else {
				//[player is no longer addicted]
				if (player.findPerk(PerkLib.MarbleResistant) >= 0) {
					EngineCore.outputText("You gulp down the bottle's contents; you're careful not to get too attached to the taste.\n\n", false);
				} else { //[player is addicted]
					EngineCore.outputText("You gulp down the bottle's contents; you really needed that.\n\n", false);
				}
			}
		}
		...
		if (player.findStatusAffect(StatusAffects.MarbleWithdrawl) >= 0) {
			...
			EngineCore.outputText("You no longer feel the symptoms of withdrawal.\n\n", false);
		}
	...
	Mutations.prototype.laBova = function(tainted, enhanced, player) {
		...
		if (enhanced) {
			EngineCore.outputText("Pro Bova", false);
		} else {
			EngineCore.outputText("La Bova", false);
		}
		EngineCore.outputText(".  The drink has an odd texture, but is very sweet.  It has a slight aftertaste of milk.", false);
		...
		if (player.cocks.length > 0 && Utils.rand(2) == 0 && !CoC.getInstance().flags[kFLAGS.HYPER_HAPPY]) {
			...
			if (player.cocks[longestCockIndex].cockLength < 2) {
				...
				if (player.cockTotal() == 1 && !player.hasVagina()) {
					EngineCore.outputText("Your " + Descriptors.cockDescript(0) + " suddenly starts tingling.  It's a familiar feeling, similar to an orgasm.  However, this one seems to start from the top down, instead of gushing up from your loins.  You spend a few seconds frozen to the odd sensation, when it suddenly feels as though your own body starts sucking on the base of your shaft.  Almost instantly, your cock sinks into your crotch with a wet slurp.  The tip gets stuck on the front of your body on the way down, but your glans soon loses all volume to turn into a shiny new clit.", false);
					if (player.balls > 0) {
						EngineCore.outputText("  At the same time, your " + Descriptors.ballsDescriptLight() + " fall victim to the same sensation; eagerly swallowed whole by your crotch.", false);
					}
					EngineCore.outputText("  Curious, you touch around down there, to find you don't have any exterior organs left.  All of it got swallowed into the gash you now have running between two fleshy folds, like sensitive lips.  It suddenly occurs to you; <b>you now have a vagina!</b>", false);
					player.balls = 0;
					player.ballSize = 1;
					player.createVagina();
					player.clitLength = 0.25;
		...
		if (enhanced) {
			...
			if (player.breastRows[0].nipplesPerBreast == 1) {
				...
				if (player.breastRows.length >= 2 && player.breastRows[1].nipplesPerBreast == 1) {
					EngineCore.outputText("A moment later your second row of " + Descriptors.breastDescript(1) + " does the same.  <b>You have sixteen nipples now!</b>", false);
				}
				if (player.breastRows.length >= 3 && player.breastRows[2].nipplesPerBreast == 1) {
					EngineCore.outputText("Finally, your ");
					if (player.bRows() == 3) {
						EngineCore.outputText("third row of " + Descriptors.breastDescript(2) + " mutates along with its sisters, sprouting into a wonderland of nipples.", false);
					} else if (player.bRows() >= 4) {
						EngineCore.outputText("everything from the third row down mutates, sprouting into a wonderland of nipples.", false);
					}
					EngineCore.outputText("  <b>You have a total of " + Utils.num2Text(player.totalNipples()) + " nipples.</b>");
				}
				...
				if ((player.nippleLength < 1.5 && tainted) || (!tainted && player.nippleLength < 1)) {
					EngineCore.outputText("  Your " + Descriptors.nippleDescript(0) + "s swell up, growing larger to accommodate your increased milk flow.", false);
		...
		if (tainted && player.lowerBody != AppearanceDefs.LOWER_BODY_TYPE_HOOFED && player.earType == AppearanceDefs.EARS_COW) {
			if (changes < changeLimit && Utils.rand(3) == 0) {
				...
				EngineCore.outputText("  A coat of beastial fur springs up below your waist, itching as it fills in.<b>  You now have hooves in place of your feet!</b>", false);
				player.lowerBody = AppearanceDefs.LOWER_BODY_TYPE_HOOFED;
				EngineCore.dynStats("cor", 0);
				changes++;
			}
		...
		if ( Utils.rand(3) == 0) {
			EngineCore.outputText(player.modFem(79, 3), false);
		}
		if ( Utils.rand(3) == 0) {
			EngineCore.outputText(player.modThickness(70, 4), false);
		}
	...
	Mutations.prototype.lustDraft = function(fuck, player) {
		...
		EngineCore.outputText("You drink the ", true);
		if (fuck) {
			EngineCore.outputText("red", false);
		} else {
			EngineCore.outputText("pink", false);
		}
		EngineCore.outputText(" potion, and its unnatural warmth immediately flows to your groin.", false);
		...
		if (player.lust >= 100 && !CoC.getInstance().isInCombat()) {
			...
			if (player.cocks.length > 0 && player.vaginas.length > 0) {
				EngineCore.outputText("At the same time your ", false);
			}
			if (player.vaginas.length > 0) {
				EngineCore.outputText(Descriptors.vaginaDescript(0) + " soaks your thighs.  ", false);
			}
			if (player.gender == 0) {
				EngineCore.outputText("body begins to quiver with orgasmic bliss.  ", false);
			}
			EngineCore.outputText("Once you've had a chance to calm down, you notice that the explosion of pleasure you just experienced has rocked you to your core.  You are a little hornier than you were before.", false);
	...
	Mutations.prototype.goblinAle = function(player) {
		...
		EngineCore.outputText("You drink the ale, finding it to have a remarkably smooth yet potent taste.  You lick your lips and sneeze, feeling slightly tipsy.", true);
		EngineCore.dynStats("lus", 15);
		//Stronger
		if (player.str > 50) {
			EngineCore.dynStats("str", -1);
		...
		if (player.skinTone != "green" && player.skinTone != "grayish-blue" && player.skinTone != "dark green" && player.skinTone != "pale yellow" && changes < changeLimit && Utils.rand(2) == 0) {
			...
			if (player.skinType == AppearanceDefs.SKIN_TYPE_FUR) {
				...
				EngineCore.outputText(player.skinDesc, false);
			}
			EngineCore.outputText(" turned " + player.skinTone + ".  No way!  It's staying, it really changed color!", false);
		}
		//Face!
	...
	Mutations.prototype.gooGasmic = function(player) {
		...
		//libido up to 80
		if (player.lib < 80) {
			EngineCore.dynStats("lib", (.5 + (90 - player.lib) / 10), "lus", player.lib / 2);
		...
		if (player.hairType != 3) {
			...
			if (player.hairColor != "green" && player.hairColor != "purple" && player.hairColor != "blue" && player.hairColor != "cerulean" && player.hairColor != "emerald") {
				EngineCore.outputText("  Stranger still, the hue of your semi-liquid hair changes to ");
				...
				EngineCore.outputText(player.hairColor + ".");
			}
			EngineCore.dynStats("lus", 10);
			return;
			...
			if (player.skinTone != "green" && player.skinTone != "purple" && player.skinTone != "blue" && player.skinTone != "cerulean" && player.skinTone != "emerald") {
				EngineCore.outputText("  Stranger still, your skintone changes to ");
				...
				EngineCore.outputText(player.skinTone + "!");
			}
			return;
		}
		...
		if (player.skinAdj == "slimy" && player.skinDesc == "skin" && player.lowerBody != AppearanceDefs.LOWER_BODY_TYPE_GOO) {
			...
			if (player.tallness < 36) {
				player.tallness = 36;
				EngineCore.outputText("  The goo firms up and you return to your previous height.  It would truly be hard to get any shorter than you already are!", false);
			}
		...
		if ( Utils.rand(2) == 0) {
			EngineCore.outputText(player.modFem(85, 3), false);
		}
		if ( Utils.rand(2) == 0) {
			EngineCore.outputText(player.modThickness(20, 3), false);
		}
		if ( Utils.rand(2) == 0) {
	...
	Mutations.prototype.sharkTooth = function(type, player) {
		...
		if (type == 0) {
			EngineCore.outputText("You have no idea why, but you decide to eat the pointed tooth. To your surprise, it's actually quite brittle, turning into a fishy-tasting dust. You figure it must just be a tablet made to look like a shark's tooth.", true);
		} else if (type == 1) {
			EngineCore.outputText("You have no idea why, but you decide to eat the pointed, glowing tooth. To your surprise, it's actually quite brittle, crumbling into a fishy-tasting dust. Maybe it's just a tablet made to look like a shark's tooth.", true);
		}
		//STATS
		...
		if (type == 1 && (!player.hasCock()) && changes < changeLimit && Utils.rand(3) == 0) {
			...
			if (player.balls == 0) {
				EngineCore.outputText(" and a pair of balls", false);
				player.balls = 2;
				player.ballSize = 2;
			}
			EngineCore.outputText("!", false);
		...
		if (((player.skinTone != "rough gray" && player.skinTone != "orange and black striped") || player.skinType != AppearanceDefs.SKIN_TYPE_PLAIN) && Utils.rand(7) == 0 && changes < changeLimit) {
			...
			if (player.skinType == AppearanceDefs.SKIN_TYPE_FUR || player.skinType == AppearanceDefs.SKIN_TYPE_SCALES) {
				EngineCore.outputText("Your " + player.skinDesc + " falls out, collecting on the floor and exposing your supple skin underneath.  ", false);
			} else if (player.skinType == AppearanceDefs.SKIN_TYPE_GOO) {
				EngineCore.outputText("Your gooey skin solidifies, thickening up as your body starts to solidy into a more normal form. ", false);
			} else if (type == 0) {
				EngineCore.outputText("Your skin itches and tingles becoming slightly rougher and turning gray.  ", false);
			}
			if (type == 0) {
				EngineCore.outputText("You abruptly stop moving and gasp sharply as a shudder goes up your entire frame. Your skin begins to shift and morph, growing slightly thicker and changing into a shiny grey color. Your skin now feels oddly rough too, comparable to that of a marine mammal. You smile and run your hands across your new shark skin.", false);
				player.skinType = AppearanceDefs.SKIN_TYPE_PLAIN;
				player.skinDesc = "skin";
				player.skinTone = "rough gray";
				changes++;
			} else {
				EngineCore.outputText("Your skin begins to tingle and itch, before rapidly shifting to a shiny orange color, marked by random black stripes. You take a quick look in a nearby pool of water, to see your skin has morphed in appearance and texture to become more like a tigershark!", false);
				player.skinType = AppearanceDefs.SKIN_TYPE_PLAIN;
				player.skinDesc = "skin";
...
//9)  Transformation Item - Snake Oil (S. Oil)
	...
	Mutations.prototype.snakeOil = function(player) {
		...
		if (player.spe < 70 && Utils.rand(2) == 0) {
			...
			if (player.spe < 40) {
				EngineCore.outputText("  Of course, you're nowhere near as fast as that.", false);
			}
			changes++;
		}
		//Removes wings
		...
		if (changes == 0 && player.tongueType == AppearanceDefs.TONUGE_SNAKE && player.faceType != AppearanceDefs.FACE_SNAKE_FANGS && Utils.rand(3) == 0 && changes < changeLimit) {
			...
			if (player.faceType != AppearanceDefs.FACE_HUMAN && player.faceType != AppearanceDefs.FACE_SHARK_TEETH && player.faceType != AppearanceDefs.FACE_BUNNY && player.faceType != AppearanceDefs.FACE_SPIDER_FANGS) {
				EngineCore.outputText("  As the change progresses, your " + player.face() + " reshapes.  The sensation is far more pleasant than teeth cutting into gums, and as the tingling transformation completes, <b>you've gained with a normal-looking, human visage.</b>");
			}
		...
		if (changes == 0 && player.faceType == AppearanceDefs.FACE_SNAKE_FANGS && player.lowerBody != AppearanceDefs.LOWER_BODY_TYPE_NAGA && Utils.rand(4) == 0 && changes < changeLimit) {
			...
			if (player.balls > 0 && player.ballSize > 10) {
				EngineCore.outputText("  You're happy not to have to drag those testicles around with you anymore.", false);
			}
			EngineCore.outputText("  But then, scales start to form on the surface of your skin, slowly becoming visible, recoloring all of your body from the waist down in a snake-like pattern. The feeling is... not that bad actually, kind of like callous, except on your whole lower body. The transformation complete, you get up, standing on your newly formed snake tail. You can't help feeling proud of this majestic new body of yours.", false);
	...
	Mutations.prototype.Hummus = function(player) {
		EngineCore.outputText("", true);
		if (debug) {
			EngineCore.outputText("You're about to eat the humus when you see it has bugs in it. Not wanting to eat bugged humus or try to debug it you throw it into the portal and find something else to eat.", false);
	...
	Mutations.prototype.catTransformation = function(player) {
		...
		//Text go!
		EngineCore.outputText("", true);
		EngineCore.outputText("You take a bite of the fruit and gulp it down. It's thick and juicy and has an almost overpowering sweetness. Nevertheless, it is delicious and you certainly could use a meal.  You devour the fruit, stopping only when the hard, nubby pit is left; which you toss aside.", false);
		...
		if ( Utils.rand(4) == 0 && changes < changeLimit) {
			...
			if (player.inte < 15) {
				...
				if ( Utils.rand(2) == 0) {
					EngineCore.outputText("somewhere and ", false);
					EngineCore.outputText(Utils.randomChoice("toss a ball around or something", "play with some yarn", "take a nap and stop worrying"), false);
				} else {
					EngineCore.outputText("in the sun and let your troubles slip away", false);
				}
				EngineCore.outputText(".", false);
		...
		if (player.lib < 80 && changes < changeLimit && Utils.rand(4) == 0) {
			...
			if (player.catCocks() > 0) {
				...
				if (player.cor < 33) {
					EngineCore.outputText("You need to control yourself better.", false);
				} else if (player.cor < 66) {
					EngineCore.outputText("You're not sure how you feel about the fantasy.", false);
				} else {
					EngineCore.outputText("You hope to find a willing partner to make this a reality.", false);
				}
			} else {
				...
				if (player.lust > 60) {
					EngineCore.outputText("even more ", false);
				}
				EngineCore.outputText("turned on.", false);
			}
		...
		if ( Utils.rand(4) == 0 && changes < changeLimit) {
			...
			if (player.goIntoHeat(false)) {
				if (intensified) {
					...
					if (player.skinType == AppearanceDefs.SKIN_TYPE_FUR) {
						EngineCore.outputText("underneath your fur ", false);
					}
					EngineCore.outputText("as images and fantasies ", false);
					if (player.cor < 50) {
						EngineCore.outputText("assault ", false);
					} else {
						EngineCore.outputText("fill ", false);
					}
					EngineCore.outputText(" your mind.  Lithe cat-boys with their perfect, spine-covered cocks line up behind you, and you bend over to present your needy pussy to them.  You tremble with the desire to feel the exotic texture of their soft barbs rubbing your inner walls, smearing your " + Descriptors.vaginaDescript(0) + " with their cum as you're impregnated.  Shivering, you recover from the fantasy and pull your fingers from your aroused sex.  <b>It would seem you've gone into heat!</b>", false);
				}
				changes++;
			}
		}
					...
					if (breastRow.breastRating > breastChangeThreshold) {
						...
						if (breastRow.breastRating > 10) {
							...
							if (breastChanges == 0) {
								...
								EngineCore.outputText("  The change moves down to your " + Utils.num2Text2(index + 1) + " row of " + Descriptors.breastDescript(0) + ". They shrink greatly, losing a couple cup-sizes.", false);
							}
						} else { //Small change
							...
							if (breastChanges == 0) {
								...
								EngineCore.outputText("  Your " + Utils.num2Text2(index + 1) + " row of " + Descriptors.breastDescript(index) + " gives a tiny jiggle as it shrinks, losing some off its mass.", false);
							}
						}
		...
		if (player.cockTotal() > 0 && player.catCocks() < player.cockTotal() && changes < changeLimit && Utils.rand(4) == 0) {
			...
			EngineCore.outputText("\n\nYour " + Descriptors.cockDescript(i) + " swells up with near-painful arousal and begins to transform.  It turns pink and begins to narrow until the tip is barely wide enough to accommodate your urethra.  Barbs begin to sprout from its flesh, if you can call the small, fleshy nubs barbs. They start out thick around the base of your " + Appearance.cockNoun(CockTypesEnum.HUMAN) + " and shrink towards the tip. The smallest are barely visible. <b>Your new feline dong throbs powerfully</b> and spurts a few droplets of cum.  ", false);
			if (!player.hasSheath()) {
				EngineCore.outputText("Then, it begins to shrink and sucks itself inside your body.  Within a few moments, a fleshy sheath is formed.", false);
				if (player.balls > 0) {
					EngineCore.outputText("  Thankfully, your balls appear untouched.", false);
				}
			} else {
				EngineCore.outputText("Then, it disappears back into your sheath.", false);
			}
		...
		if (player.catCocks() > 0 && Utils.rand(3) == 0 && changes < changeLimit && !CoC.getInstance().flags[kFLAGS.HYPER_HAPPY]) {
			...
			var hasBigCatCockIndex = _.indexOf(player.cocks, _.find(player.cocks, function(cock) { return cock.cockType == CockTypesEnum.CAT && cock.cockLength > 6; }));
			if (hasBigCatCock) {
				//lose 33% size until under 10, then lose 2" at a time
				if (player.cocks[hasBigCatCockIndex].cockLength > 16) {
					EngineCore.outputText("\n\nYour " + Descriptors.cockDescript(hasBigCatCockIndex) + " tingles, making your sheath feel a little less tight.  It dwindles in size, losing a full third of its length and a bit of girth before the change finally stops.", false);
					player.cocks[hasBigCatCockIndex].cockLength *= 0.66;
				} else if (player.cocks[hasBigCatCockIndex].cockLength > 6) {
					EngineCore.outputText("\n\nYour " + Descriptors.cockDescript(hasBigCatCockIndex) + " tingles and withdraws further into your sheath.  If you had to guess, you'd say you've lost about two inches of total length and perhaps some girth.", false);
					player.cocks[hasBigCatCockIndex].cockLength -= 2;
				}
				if (player.cocks[hasBigCatCockIndex].cockLength / 5 < player.cocks[hasBigCatCockIndex].cockThickness && player.cocks[hasBigCatCockIndex].cockThickness > 1.25) {
					player.cocks[hasBigCatCockIndex].cockThickness = player.cocks[hasBigCatCockIndex].cockLength / 6;
				}
				//Check for any more!
				var hasMore = false;
				...
				EngineCore.outputText("  Although the package is smaller, it feels even more sensitive – as if it retained all sensation of its larger size in its smaller form.", false);
				EngineCore.dynStats("sen", 5);
				//Make note of other dicks changing
				if (hasMore) {
					EngineCore.outputText("  Upon further inspection, all your " + Appearance.cockNoun(CockTypesEnum.CAT) + "s have shrunk!", false);
				}
				changes++;
			}
		}
		...
		if (player.tailType == AppearanceDefs.TAIL_TYPE_CAT && player.earType == AppearanceDefs.EARS_CAT &&
			...
			if (player.lowerBody == AppearanceDefs.LOWER_BODY_TYPE_HOOFED || player.lowerBody == AppearanceDefs.LOWER_BODY_TYPE_CENTAUR) {
				...
				if (player.lowerBody == AppearanceDefs.LOWER_BODY_TYPE_CENTAUR) EngineCore.outputText("  You feel woozy and collapse on your side.  When you wake, you're no longer a centaur and your body has returned to a humanoid shape.", false);
		...
		if (changes < changeLimit) {
			if ( Utils.rand(2) == 0) {
				EngineCore.outputText(player.modThickness(5, 2), false);
			}
			if ( Utils.rand(2) == 0) {
				EngineCore.outputText(player.modTone(76, 2), false);
			}
			if (player.gender < 2) {
				if ( Utils.rand(2) == 0) {
					EngineCore.outputText(player.modFem(65, 1), false);
				} else {
					EngineCore.outputText(player.modFem(85, 2), false);
				}
			}
		}
	};
	Mutations.prototype.reptilum = function(player) {
		...
		//clear screen
		EngineCore.outputText("", true);
		EngineCore.outputText("You uncork the vial of fluid and drink it down.  The taste is sour, like a dry wine with an aftertaste not entirely dissimilar to alcohol.  Instead of the warmth you'd expect, it leaves your throat feeling cold and a little numb.", false);
		//Statistical changes:
		...
		if (player.lib < 100 && changes < changeLimit && Utils.rand(3) == 0) {
			...
			if (player.cocks.length > 0 && (player.gender != 3 || Utils.rand(2) == 0)) {
				EngineCore.outputText("filling ", false);
				if (player.cocks.length > 1) {
					EngineCore.outputText("each of ", false);
				}
				EngineCore.outputText("your " + Descriptors.multiCockDescriptLight() + " with the desire to breed.  You get a bit hornier when you realize your sex-drive has gotten a boost.", false);
			} else if (player.hasVagina()) { //(COOCH)
				EngineCore.outputText("puddling in your " + Descriptors.vaginaDescript(0) + ".  An instinctive desire to mate and lay eggs spreads through you, increasing your lust and boosting your sex-drive.", false);
			} else { //(TARDS)
				EngineCore.outputText("puddling in your featureless crotch for a split-second before it slides into your " + assDescript() + ".  You want to be fucked, filled, and perhaps even gain a proper gender again.  Through the lust you realize your sex-drive has been permanently increased.", false);
			}
		...
		if (player.lizardCocks() == 0 && player.cockTotal() > 0 && changes < changeLimit && Utils.rand(4) == 0) {
			...
			EngineCore.outputText("to tip, undulating and convulsing as its color lightens, darkens, and finally settles on a purplish hue.  Your " + Appearance.cockNoun(CockTypesEnum.HUMAN) + " resolves itself into a bulbous form, with a slightly pointed tip.  The 'bulbs' throughout its shape look like they would provide an interesting ride for your sexual partners, but the perverse, alien pecker ", false);
			if (player.cor < 33) {
				EngineCore.outputText("horrifies you.", false);
			} else if (player.cor < 66) {
				EngineCore.outputText("is a little strange for your tastes.", false);
			} else {
				EngineCore.outputText("looks like it might be more fun to receive than use on others.  ", false);
				if (player.hasVagina()) {
					EngineCore.outputText("Maybe you could find someone else with one to ride?", false);
				} else {
					EngineCore.outputText("Maybe you should test it out on someone and ask them exactly how it feels?", false);
				}
			}
			EngineCore.outputText("  <b>You now have a bulbous, lizard-like cock.</b>", false);
		...
		if (player.cockTotal() > 1 && player.lizardCocks() > 0 && player.cockTotal() > player.lizardCocks() && Utils.rand(4) == 0 && changes < changeLimit) {
			...
			if (player.cockTotal() == 2) {
				...
				EngineCore.outputText("another one of your dicks", false);
			}
			EngineCore.outputText(" starts to change into the strange reptilian shape you've grown familiar with.  It warps visibly, trembling and radiating pleasurable feelings back to you as the transformation progresses.  ", false);
			if (player.cumQ() < 50) {
				EngineCore.outputText("pre-cum oozes from the tip", false);
			} else if (player.cumQ() < 700) {
				EngineCore.outputText("Thick pre-cum rains from the tip", false);
			} else {
				EngineCore.outputText("A wave of pre-cum splatters on the ground", false);
			}
			EngineCore.outputText(" from the pleasure of the change.  In moments <b>you have a bulbous, lizard-like cock.</b>", false);
		...
		if (player.lizardCocks() == 1 && player.cocks.length == 1 && Utils.rand(4) == 0 && changes < changeLimit) {
			...
			if (player.skinType == AppearanceDefs.SKIN_TYPE_FUR || player.skinType == AppearanceDefs.SKIN_TYPE_SCALES) {
				EngineCore.outputText(" and shedding " + player.skinDesc, false);
			}
		...
		if (player.lizardCocks() == player.cockTotal() && changes < changeLimit && player.findStatusAffect(StatusAffects.Infested) >= 0) {
			...
			if (player.balls > 1) {
				EngineCore.outputText("  The remaining " + Utils.num2Text(player.balls - 1) + " slither out the pre-stretched holes with ease, though the last one hangs from your tip for a moment before dropping to the ground.", false);
			}
			EngineCore.outputText("  The white creature joins its kin on the ground and slowly slithers away.  Perhaps they prefer mammals? In any event, <b>you are no longer infected with worms</b>.", false);
		...
		if (player.biggestTitSize() >= 1 && player.gender == 1 && changes < changeLimit && Utils.rand(3) == 0) {
			...
			//(BOTH – no new PG)
			EngineCore.outputText("  With the change in weight and gravity, you find it's gotten much easier to move about.", false);
		...
		if (player.biggestLactation() >= 1 && changes < changeLimit && Utils.rand(4) == 0) {
			...
			EngineCore.outputText(" nipples relax.  It's a strange feeling, and you pull back your top to touch one.  It feels fine, though there doesn't seem to be any milk leaking out.  You give it a squeeze and marvel when nothing ", false);
			if (player.hasFuckableNipples()) {
				EngineCore.outputText("but sexual fluid ", false);
			}
			EngineCore.outputText("escapes it.  <b>You are no longer lactating.</b>  That makes sense, only mammals lactate!  Smiling, you muse at how much time this will save you when cleaning your gear.", false);
		...
		if (player.averageNipplesPerBreast() > 1 && changes < changeLimit && Utils.rand(4) == 0) {
			...
			if (player.biggestTitSize() < 1) {
				EngineCore.outputText("'breast'.", false);
			} else {
				EngineCore.outputText("breast.", false);
			}
			changes++;
		...
		if (player.hasVagina() && player.findPerk(PerkLib.Oviposition) < 0 && changes < changeLimit && Utils.rand(5) == 0 && player.lizardScore() > 3) {
			...
			EngineCore.outputText("(<b>Perk Gained: Oviposition</b>)", false);
		...
		if (player.lowerBody != AppearanceDefs.LOWER_BODY_TYPE_LIZARD && changes < changeLimit && Utils.rand(5) == 0) {
			...
			EngineCore.outputText("  <b>You have reptilian legs and claws!</b>", false);
		...
		if (player.skinType != AppearanceDefs.SKIN_TYPE_SCALES && player.earType == AppearanceDefs.EARS_LIZARD && player.tailType == AppearanceDefs.TAIL_TYPE_LIZARD && player.lowerBody == AppearanceDefs.LOWER_BODY_TYPE_LIZARD && changes < changeLimit && Utils.rand(5) == 0) {
			...
			if (player.skinType == AppearanceDefs.SKIN_TYPE_FUR) {
				...
				EngineCore.outputText(player.skinTone + " scales.</b>", false);
			}
	...
	Mutations.prototype.neonPinkEgg = function(pregnantChange, player) {
		...
		if (!pregnantChange) {
			EngineCore.outputText("You eat the neon pink egg, and to your delight it tastes sweet, like candy.  In seconds you've gobbled down the entire thing, and you lick your fingers clean before you realize you ate the shell – and it still tasted like candy.", false);
		}
		...
		if (pregnantChange) {
			...
			if (player.buttPregnancyType == PregnancyStore.PREGNANCY_BUNNY) {
				EngineCore.outputText("backdoor ", false);
			}
			if (player.buttPregnancyType == PregnancyStore.PREGNANCY_BUNNY && player.pregnancyType == PregnancyStore.PREGNANCY_BUNNY) {
				EngineCore.outputText("rumble", false);
			} else {
				EngineCore.outputText("rumbles", false);
			}
			EngineCore.outputText(" oddly, and you have a hunch that something's about to change</b>.", false);
		}
		//STATS CHANGURYUUUUU
		//Boost speed (max 80!)
			...
			if (player.lib < 60) {
				...
				if (!player.hasCock() || (player.gender == 3 && Utils.rand(2) == 0)) {
					if (player.lib < 30) {
						...
						if (player.cor < 25) {
							EngineCore.outputText("You're repulsed by such shameful thoughts.", false);
						} else if (player.cor < 60) {
							EngineCore.outputText("You worry that this place is really getting to you.", false);
						} else if (player.cor < 90) {
							EngineCore.outputText("You pant a little and wonder where the nearest fertile male is.", false);
						} else {
							EngineCore.outputText("You grunt and groan with desire and disappointment.  You should get bred soon!", false);
						}
					} else {
				...
				if (player.hasCock()) {
					if (player.lib < 30) {
						...
						if ( Utils.rand(2) == 0) {
							EngineCore.outputText("female hare until she's immobilized by all her eggs", false);
						} else {
							EngineCore.outputText("herm rabbit until her sack is so swollen that she's forced to masturbate over and over again just to regain mobility", false);
						}
						EngineCore.outputText(". ", false);
						if (player.cor < 25) {
							EngineCore.outputText("You're repulsed by such shameful thoughts.", false);
							...
							EngineCore.outputText("You pant a little and wonder where the nearest fertile female is.", false);
						} else {
							EngineCore.outputText("You grunt and groan with desire and disappointment.  Gods you need to fuck!", false);
						}
					} else {
				...
				if (player.skinType > AppearanceDefs.SKIN_TYPE_PLAIN) {
					EngineCore.outputText(" through your " + player.skinDesc, false);
				}
				EngineCore.outputText(".  ", false);
				if (player.gender == 1) {
					EngineCore.outputText("Compression tightens down on " + Descriptors.sMultiCockDesc() + " as it strains against your " + player.armorName + ".  You struggle to fight down your heightened libido, but it's hard – so very hard.", false);
				} else if (player.gender == 0) {
					EngineCore.outputText("Sexual hunger seems to gnaw at your " + Descriptors.assholeDescript() + ", demanding it be filled, but you try to resist your heightened libido.  It's so very, very hard.", false);
				} else if (player.gender == 2) {
					EngineCore.outputText("Moisture grows between your rapidly-engorging vulva, making you squish and squirm as you try to fight down your heightened libido, but it's hard – so very hard.", false);
				} else {
					EngineCore.outputText("Steamy moisture and tight compression war for your awareness in your groin as " + Descriptors.sMultiCockDesc() + " starts to strain against your " + player.armorName + ".  Your vulva engorges with blood, growing slicker and wetter.  You try so hard to fight down your heightened libido, but it's so very, very hard.  The urge to breed lingers in your mind, threatening to rear its ugly head.", false);
				}
			} else { //MEGALIBIDO
				...
				if (player.cor < 33) {
					EngineCore.outputText("You sigh, trying not to give in completely.", false);
				} else if (player.cor < 66) {
					EngineCore.outputText("You pant and groan, not sure how long you'll even want to resist.", false);
				} else {
					EngineCore.outputText("You smile and wonder if you can ", false);
					if (player.lib < 100) {
						EngineCore.outputText("get your libido even higher.", false);
					} else {
						EngineCore.outputText("find someone to fuck right now.", false);
					}
				}
			}
		}
		...
		if (player.sens < 60 && changes < changeLimit && Utils.rand(3) == 0) {
			...
			//(low)
			if ( Utils.rand(3) != 2) {
				EngineCore.outputText("The feeling of small breezes blowing over your " + player.skinDesc + " gets a little bit stronger.  How strange.  You pinch yourself and nearly jump when it hurts a tad more than you'd think. You've gotten more sensitive!", false);
				EngineCore.dynStats("sen", 5);
			} else { //(BIG boost 1/3 chance)
				EngineCore.dynStats("sen", 15);
				EngineCore.outputText("Every movement of your body seems to bring heightened waves of sensation that make you woozy.  Your " + player.armorName + " rubs your " + Descriptors.nippleDescript(0) + "s deliciously", false);
				if (player.hasFuckableNipples()) {
					EngineCore.outputText(", sticking to the ", false);
					if (player.biggestLactation() > 2) {
						EngineCore.outputText("milk-leaking nipple-twats", false);
					} else {
						EngineCore.outputText("slippery nipple-twats", false);
					}
				} else if (player.biggestLactation() > 2) {
					EngineCore.outputText(", sliding over the milk-leaking teats with ease", false);
				} else {
					EngineCore.outputText(" catching on each of the hard nubs repeatedly", false);
				}
				EngineCore.outputText(".  Meanwhile, your crotch... your crotch is filled with such heavenly sensations from ", false);
				if (player.gender == 1) {
					EngineCore.outputText(Descriptors.sMultiCockDesc() + " and your ", false);
					if (player.balls > 0) {
					...
					EngineCore.outputText("your " + Descriptors.vaginaDescript(0) + " and " + Descriptors.clitDescript(), false);
				} else if (player.gender == 3) {
					EngineCore.outputText(Descriptors.sMultiCockDesc() + ", ", false);
					if (player.balls > 0) {
						EngineCore.outputText(Descriptors.ballsDescriptLight() + ", ", false);
					}
					EngineCore.outputText(Descriptors.vaginaDescript(0) + ", and " + Descriptors.clitDescript(), false);
				} else { //oh god genderless
					EngineCore.outputText("you " + Descriptors.assholeDescript(), false);
				}
				EngineCore.outputText(" that you have to stay stock-still to keep yourself from falling down and masturbating on the spot.  Thankfully the orgy of tactile bliss fades after a minute, but you still feel way more sensitive than your previous norm.  This will take some getting used to!", false);
			}
		}
		...
		if (changes < changeLimit && Utils.rand(2) == 0) {
			...
			if (buffer != "") {
				EngineCore.outputText(buffer, false);
				changes++;
			}
		}
		...
		if (player.hasVagina() && player.findPerk(PerkLib.BunnyEggs) < 0 && changes < changeLimit && Utils.rand(4) == 0 && player.bunnyScore() > 3) {
			EngineCore.outputText("\n\nDeep inside yourself there is a change.  It makes you feel a little woozy, but passes quickly.  Beyond that, you aren't sure exactly what just happened, but you are sure it originated from your womb.\n\n", false);
			EngineCore.outputText("(<b>Perk Gained: Bunny Eggs</b>)", false);
		...
		if ((player.balls > 0 || player.hasCock()) && player.cumQ() < 3000 && Utils.rand(3) == 0 && changeLimit > 1) {
			...
			if (player.balls > 0) {
				...
				if (player.cumQ() < 50) {
					...
					if (player.hasCock()) {
						EngineCore.outputText("  In the process, you brush " + Descriptors.sMultiCockDesc() + " and discover a bead of pre leaking at the tip.", false);
					}
					...
					if (player.hasCock()) {
						EngineCore.outputText(Descriptors.SMultiCockDesc() + " leaks and dribbles pre-cum down your " + player.legs() + " as your body's cum production kicks up even higher.", false);
					} else {
						EngineCore.outputText("You wince, feeling pent up and yet unable to release.  You really wish you had a cock right about now.", false);
					}
					...
					if (player.hasCock()) {
						EngineCore.outputText("A thick trail of slime leaks from " + Descriptors.sMultiCockDesc() + " down your " + player.leg() + ", pooling below you.", false);
					} else {
						EngineCore.outputText("You grunt, feeling terribly pent-up and needing to release.  Maybe you should get a penis to go with these balls...", false);
					}
					EngineCore.outputText("  It's quite obvious that your cum production has gone up again.", false);
		...
		if (player.lowerBody != AppearanceDefs.LOWER_BODY_TYPE_BUNNY && changes < changeLimit && Utils.rand(5) == 0 && player.earType == AppearanceDefs.EARS_BUNNY) {
			...
			if (player.isTaur()) {
				...
				if (player.tailType > AppearanceDefs.TAIL_TYPE_NONE) {
					EngineCore.outputText(", nearly smashing your tail flat", false);
				} else {
					EngineCore.outputText(" hard enough to sting", false);
				}
				EngineCore.outputText(" while the change works its way through you.  Once it finishes, <b>you discover that you now have fuzzy bunny feet and legs</b>!", false);
			}
			changes++;
		...
		if (player.earType == AppearanceDefs.EARS_BUNNY && player.faceType != AppearanceDefs.FACE_BUNNY && Utils.rand(3) == 0 && changes < changeLimit) {
			...
			//Human(ish) face
			if (player.faceType == AppearanceDefs.FACE_HUMAN || player.faceType == AppearanceDefs.FACE_SHARK_TEETH) {
				EngineCore.outputText("You catch your nose twitching on its own at the bottom of your vision, but as soon as you focus on it, it stops.  A moment later, some of your teeth tingle and brush past your lips, exposing a white pair of buckteeth!  <b>Your face has taken on some rabbit-like characteristics!</b>", false);
			} else { //Crazy furry TF shit
				EngineCore.outputText("You grunt as your " + player.face() + " twists and reforms.  Even your teeth ache as their positions are rearranged to match some new, undetermined order.  When the process finishes, <b>you're left with a perfectly human looking face, save for your constantly twitching nose and prominent buck-teeth.</b>", false);
			}
	...
	Mutations.prototype.goldenSeed = function(type, player) {
		...
		//Generic eating text:
		EngineCore.outputText("", true);
		EngineCore.outputText("You pop the nut into your mouth, chewing the delicious treat and swallowing it quickly.  No wonder harpies love these things so much!", false);
		//****************
		//Stats:
		//****************
		...
		if ((player.lib < 90 || Utils.rand(3) == 0) && Utils.rand(3) == 0 && changes < changeLimit) {
			...
			if (player.lib < 40) {
				...
				if (player.hasVagina()) {
					EngineCore.outputText(" The moistness of your " + Descriptors.vaginaDescript() + " seems to agree.", false);
				} else if (player.hasCock()) {
					EngineCore.outputText(" The hardness of " + Descriptors.sMultiCockDesc() + " seems to agree.", false);
				}
				EngineCore.dynStats("lus", 5);
		...
		if (changes < changeLimit && player.breastRows[0].breastRating < 2 && Utils.rand(3) == 0) {
			...
			if (player.breastRows[0].breastRating < 1) {
				EngineCore.outputText("  <b>You have breasts now!</b>", false);
			}
			player.breastRows[0].breastRating = 2;
		}
		//****************
		...
		if (player.femininity < 85 && changes < changeLimit && Utils.rand(3) == 0) {
			changes++;
			EngineCore.outputText(player.modFem(85, 3 + Utils.rand(5)), false);
		}
		...
		if ((player.skinTone != "tan" && player.skinTone != "olive" && player.skinTone != "dark" && player.skinTone != "light") && changes < changeLimit && Utils.rand(5) == 0) {
			...
			if (player.skinType == AppearanceDefs.SKIN_TYPE_FUR) {
				EngineCore.outputText("the skin under your " + player.hairColor + " " + player.skinDesc, false);
			} else {
				EngineCore.outputText("your " + player.skinDesc, false);
			}
			EngineCore.outputText(" has changed to become ", false);
			player.skinTone = Utils.randomChoice("tan", "olive", "dark", "light");
			EngineCore.outputText(player.skinTone + " colored.</b>", false);
		}
		...
		if (player.thickness > 25 && changes < changeLimit && Utils.rand(3) == 0) {
			EngineCore.outputText(player.modThickness(25, 3 + Utils.rand(4)), false);
			changes++;
		}
		//Remove odd eyes
		if (changes < changeLimit && Utils.rand(5) == 0 && player.eyeType > AppearanceDefs.EYES_HUMAN) {
			if (player.eyeType == AppearanceDefs.EYES_BLACK_EYES_SAND_TRAP) {
				...
				EngineCore.outputText("\n\nYou blink and stumble, a wave of vertigo threatening to pull your " + player.feet() + " from under you.  As you steady and open your eyes, you realize something seems different.  Your vision is changed somehow.", false);
				if (player.eyeType == AppearanceDefs.EYES_FOUR_SPIDER_EYES) EngineCore.outputText("  Your multiple, arachnid eyes are gone!</b>", false);
				EngineCore.outputText("  <b>You have normal, humanoid eyes again.</b>", false);
			}
			player.eyeType = AppearanceDefs.EYES_HUMAN;
			changes++;
		...
		if (player.lowerBody != AppearanceDefs.LOWER_BODY_TYPE_HARPY && changes < changeLimit && (type == 1 || player.tailType == AppearanceDefs.TAIL_TYPE_HARPY) && Utils.rand(4) == 0) {
			...
			changes++;
			//(cont)
			EngineCore.outputText("While humanoid in shape, they have two large, taloned toes on the front and a single claw protruding from the heel. The entire ensemble is coated in " + player.hairColor + " feathers from ankle to hip, reminding you of the bird-women of the mountains. <b>You now have harpy legs!</b>", false);
		}
		//-Feathery Tail
	...
	Mutations.prototype.kangaFruit = function(type, player) {
		...
		if (player.hasCock()) {
			...
			if (player.kangaCocks() < player.cockTotal() && (type == 1 && Utils.rand(2) == 0) && changes < changeLimit) {
				...
				if (player.hasSheath()) {
					EngineCore.outputText("your sheath", false);
				} else {
					EngineCore.outputText("a sheath that forms at the base of it", false);
				}
				EngineCore.outputText(".  <b>You now have a kangaroo-penis!</b>", false);
				var firstNonKangarooCock = _.find(player.cocks, function(cock) { return player.cocks[cockIdx].cockType != CockTypesEnum.KANGAROO; });
		...
		if (player.tailType != AppearanceDefs.TAIL_TYPE_KANGAROO && changes < changeLimit && Utils.rand(4) == 0 && (type != 1 || player.earType == AppearanceDefs.EARS_KANGAROO)) {
			...
			if (player.tailType == AppearanceDefs.TAIL_TYPE_NONE) {
				...
				if (player.tailType == AppearanceDefs.TAIL_TYPE_BEE_ABDOMEN) {
					EngineCore.outputText("hollow stinger drops out of the end, taking the venom organ with it.", false);
				} else {
					EngineCore.outputText("spinnerets drop out of the end, taking the last of your webbing with it.", false);
				}
				EngineCore.outputText("  By the time you're back to yourself, the insectile carapace has fallen off completely, leaving you with a long, thick, fleshy tail in place of your proud, insectile abdomen.  <b>You now have a kangaroo tail!</b>  You wipe the errant spittle from your mouth as you idly bob your new tail about.", false);
...
//[Giant Chocolate Cupcake] – 500 gems
	Mutations.prototype.giantChocolateCupcake = function(player) {
		...
		EngineCore.outputText("Gods, you feel heavy!  You waddle slightly as your body begins thickening, swelling until you feel as wide as a house.  Lethargy spreads through your limbs, and you're forced to sit still a little while until you let out a lazy burp.\n\n", false);
	...
	Mutations.prototype.sweetGossamer = function(type, player) {
		...
		//Consuming Text
		if (type == 0) {
			EngineCore.outputText("You wad up the sweet, pink gossamer and eat it, finding it to be delicious and chewy, almost like gum.  Munching away, your mouth generates an enormous amount of spit until you're drooling all over yourself while you devour the sweet treat.", false);
		} else if (type == 1) {
			EngineCore.outputText("You wad up the sweet, black gossamer and eat it, finding it to be delicious and chewy, almost like licorice.  Munching away, your mouth generates an enormous amount of spit until you're drooling all over yourself while you devour the sweet treat.", false);
		}
		//*************
		//Stat Changes
		//*************
		...
		if (player.str > 70 && Utils.rand(3) == 0) {
			...
			if (player.spiderScore() < 4) {
				EngineCore.outputText("Wait, you're not a spider, that doesn't make any sense!", false);
			} else {
				EngineCore.outputText("Well, maybe you should put your nice, heavy abdomen to work.", false);
			}
			EngineCore.dynStats("str", -1);
			changes++;
		}
		...
		if (player.hasCock() && changes < changeLimit && Utils.rand(4) == 0) {
			...
			if (thickenSomthing == 1) {
				...
				EngineCore.outputText(" out, you look closely.  ", false);
				if (player.cockTotal() == 1) {
					EngineCore.outputText("It's", false);
				} else {
					EngineCore.outputText("They're", false);
				}
				EngineCore.outputText(" definitely thicker.", false);
				changes++;
			}
		}
		...
		if (player.skinType != AppearanceDefs.SKIN_TYPE_PLAIN && (player.earType == AppearanceDefs.EARS_HUMAN || player.earType == AppearanceDefs.EARS_ELFIN) && Utils.rand(4) == 0 && changes < changeLimit) {
			...
			EngineCore.outputText(" falling to the ground, revealing flawless, almost pearly-white skin underneath.  <b>You now have pale white skin.</b>", false);
	...
	Mutations.prototype.broBrew = function(player) {
		...
		if (player.findPerk(PerkLib.BimboBody) >= 0) {
			EngineCore.outputText("The stuff hits you like a giant cube, nearly staggering you as it begins to settle.", false);
			if (player.tallness < 77) {
				player.tallness = 77;
				EngineCore.outputText(".. Did the ground just get farther away?  You glance down and realize, you're growing!  Like a sped-up flower sprout, you keep on getting taller until finally stopping around... six and a half feet, you assume.  Huh.  You didn't expect that to happen!", false);
			}
			if (player.tone < 100) {
				EngineCore.outputText("  A tingling in your arm draws your attention just in time to see your biceps and triceps swell with new-found energy, skin tightening until thick cords of muscle run across the whole appendage.  Your other arm surges forward with identical results.  To compensate, your shoulders and neck widen to bodybuilder-like proportions while your chest and abs tighten to a firm, statuesque physique.  Your " + player.legs() + " and glutes are the last to go, bulking up to proportions that would make any female martial artist proud.  You feel like you could kick forever with legs this powerful.", false);
				player.tone = 100;
			}
			EngineCore.outputText("\n\n", false);
			//female
			if (!player.hasCock()) {
				EngineCore.outputText("The beverage isn't done yet, however, and it makes it perfectly clear with a building pleasure in your groin.  You can only cry in ecstasy and loosen the bottoms of your " + player.armorName + " just in time for a little penis to spring forth.  You watch, enthralled, as blood quickly stiffens the shaft to its full length – then keeps on going!  Before long, you have a quivering 10-inch maleness, just ready to stuff into a welcoming box.", false);
				player.createCock();
				player.cocks[0].cockLength = 10;
				player.cocks[0].cockThickness = 2;
				if (player.balls == 0) {
					EngineCore.outputText("  Right on cue, two cum-laden testicles drop in behind it, their contents swirling and churning.", false);
					player.balls = 2;
					player.ballSize = 3;
				}
				EngineCore.outputText("\n\n", false);
			} else if (player.balls == 0) {
				EngineCore.outputText("A swelling begins behind your man-meat, and you're assailed with an incredibly peculiar sensation as two sperm-filled balls drop into a newly-formed scrotum.  Frikkin' sweet!\n\n", false);
				player.balls = 2;
				player.ballSize = 3;
			}
			EngineCore.outputText("Finally, you feel the transformation skittering to a halt, leaving you to openly roam your new chiseled and sex-ready body.  So what if you can barely form coherent sentences anymore?  A body like this does all the talking you need, you figure!", false);
			if (player.inte > 35) {
				player.inte = 35;
			...
			if (player.findPerk(PerkLib.BimboBrains) >= 0) {
				EngineCore.outputText("<b>(Lost Perks - Bimbo Brains, Bimbo Body)\n", false);
			} else {
				EngineCore.outputText("<b>(Lost Perk - Bimbo Body)\n", false);
			}
			...
			EngineCore.outputText("(Gained Perks - Futa Form, Futa Faculties)</b>", false);
			player.genderCheck();
			return;
		}
		//HP restore for bros!
		if (player.findPerk(PerkLib.BroBody) >= 0 || player.findPerk(PerkLib.FutaForm) >= 0) {
			EngineCore.outputText("You crack open the can and guzzle it in a hurry.  Goddamn, this shit is the best.  As you crush the can against your forehead, you wonder if you can find a six-pack of it somewhere?\n\n", false);
			fatigue(-33);
			EngineCore.HPChange(100, true);
			return;
		}
		EngineCore.outputText("Well, maybe this will give you the musculature that you need to accomplish your goals.  You pull on the tab at the top and hear the distinctive snap-hiss of venting, carbonating pressure.  A smoky haze wafts from the opened container, smelling of hops and alcohol.  You lift it to your lips, the cold, metallic taste of the can coming to your tongue before the first amber drops of beer roll into your waiting mouth.  It tingles, but it's very, very good.  You feel compelled to finish it as rapidly as possible, and you begin to chug it.  You finish the entire container in seconds.\n\n", false);
		EngineCore.outputText("A churning, full sensation wells up in your gut, and without thinking, you open wide to release a massive burp. It rumbles through your chest, startling birds into flight in the distance.  Awesome!  You slam the can into your forehead hard enough to smash the fragile aluminum into a flat, crushed disc.  Damn, you feel stronger already", false);
		if (player.inte > 50) {
			EngineCore.outputText(", though you're a bit worried by how much you enjoyed the simple, brutish act", false);
		}
		EngineCore.outputText(".\n\n", false);
		//(Tits b' gone)
		if (player.biggestTitSize() >= 1) {
			EngineCore.outputText("A tingle starts in your " + Descriptors.nippleDescript(0) + "s before the tight buds grow warm, hot even.  ", false);
			if (player.biggestLactation() >= 1) {
				EngineCore.outputText("Somehow, you know that the milk you had been producing is gone, reabsorbed by your body.  ", false);
			}
			EngineCore.outputText("They pinch in towards your core, shrinking along with your flattening " + Descriptors.allChestDesc() + ".  You shudder and flex in response.  Your chest isn't just shrinking, it's reforming, sculping itself into a massive pair of chiseled pecs.  ", false);
			if (player.breastRows.length > 1) {
				EngineCore.outputText("The breasts below vanish entirely.  ", false);
			...
			player.removeStatusAffect(StatusAffects.Feeder);
			player.removePerk(PerkLib.Feeder);
			EngineCore.outputText("All too soon, your boobs are gone.  Whoa!\n\n", false);
		}
		EngineCore.outputText("Starting at your hands, your muscles begin to contract and release, each time getting tighter, stronger, and more importantly - larger.  The oddness travels up your arms, thickens your biceps, and broadens your shoulders.  Soon, your neck and chest are as built as your arms.  You give a few experimental flexes as your abs ", false);
		if (player.tone >= 70) {
			EngineCore.outputText("further define themselves", false);
		} else {
			EngineCore.outputText("become extraordinarily visible", false);
		}
		EngineCore.outputText(".  The strange, muscle-building changes flow down your " + player.legs() + ", making them just as fit and strong as the rest of you.  You curl your arm and kiss your massive, flexing bicep.  You're awesome!\n\n", false);
		EngineCore.outputText("Whoah, you're fucking ripped and strong, not at all like the puny weakling you were before.  Yet, you feel oddly wool-headed.  Your thoughts seem to be coming slower and slower, like they're plodding through a marsh.  You grunt in frustration at the realization.  Sure, you're a muscle-bound hunk now, but what good is it if you're as dumb as a box of rocks?  Your muscles flex in the most beautiful way, so you stop and strike a pose, mesmerized by your own appearance.  Fuck thinking, that shit's for losers!\n\n", false);
		//(has dick less than 10 inches)
		if (player.hasCock()) {
			if (player.cocks[0].cockLength < 10) {
				EngineCore.outputText("As if on cue, the familiar tingling gathers in your groin, and you dimly remember you have one muscle left to enlarge.  If only you had the intelligence left to realize that your penis is not a muscle.  In any event, your " + Descriptors.cockDescript(0) + " swells in size, ", false);
				if (player.cocks[0].cockThickness < 2.75) {
					EngineCore.outputText("thickening and ", false);
					player.cocks[0].cockThickness = 2.75;
				}
				EngineCore.outputText("lengthening until it's ten inches long and almost three inches wide.  Fuck, you're hung!  ", false);
				player.cocks[0].cockLength = 10;
			}
			//Dick already big enough! BALL CHECK!
			if (player.balls > 0) {
				EngineCore.outputText("Churning audibly, your " + Descriptors.sackDescript() + " sways, but doesn't show any outward sign of change.  Oh well, it's probably just like, getting more endurance or something.", false);
			} else {
				EngineCore.outputText("Two rounded orbs drop down below, filling out a new, fleshy sac above your " + player.legs() + ".  Sweet!  You can probably cum buckets with balls like these.", false);
				player.balls = 2;
				player.ballSize = 3;
			}
			EngineCore.outputText("\n\n", false);
		} else { //(No dick)
			EngineCore.outputText("You hear a straining, tearing noise before you realize it's coming from your underwear.  Pulling open your " + player.armorName + ", you gasp in surprise at the huge, throbbing manhood that now lies between your " + Descriptors.hipDescript() + ".  It rapidly stiffens to a full, ten inches, and goddamn, it feels fucking good.  You should totally find a warm hole to fuck!", false);
			if (player.balls == 0) {
				EngineCore.outputText("  Two rounded orbs drop down below, filling out a new, fleshy sac above your " + player.legs() + ".  Sweet!  You can probably cum buckets with balls like these.", false);
			}
			EngineCore.outputText("\n\n", false);
		...
		//(Pussy b gone)
		if (player.hasVagina()) {
			EngineCore.outputText("At the same time, your " + Descriptors.vaginaDescript(0) + " burns hot, nearly feeling on fire.  You cuss in a decidedly masculine way for a moment before the pain fades to a dull itch.  Scratching it, you discover your lady-parts are gone.  Only a sensitive patch of skin remains.\n\n", false);
			player.removeVagina(0, 1);
		}
		player.genderCheck();
		//(below max masculinity)
		if (player.femininity > 0) {
			EngineCore.outputText("Lastly, the change hits your face.  You can feel your jawbones shifting and sliding around, your skin changing to accommodate your face's new shape.  Once it's finished, you feel your impeccable square jaw and give a wide, easy-going grin.  You look awesome!\n\n", false);
			player.modFem(0, 100);
		}
		EngineCore.outputText("You finish admiring yourself and adjust your " + player.armorName + " to better fit your new physique.  Maybe there's some bitches around you can fuck.  Hell, as good as you look, you might have other dudes wanting you to fuck them too, no homo.\n\n", false);
		...
		EngineCore.outputText("<b>(Bro Body - Perk Gained!)\n", false);
		EngineCore.outputText("(Bro Brains - Perk Gained!)</b>\n", false);//int to 20.  max int 50)
		if (player.findPerk(PerkLib.Feeder) >= 0) {
			EngineCore.outputText("<b>(Perk Lost - Feeder!)</b>\n", false);
			player.removePerk(PerkLib.Feeder);
		}
		if (player.inte > 21) {
			player.inte = 21;
...
//Bottle of Ectoplasm Text
	Mutations.prototype.ectoplasm = function(player) {
		...
		if (player.lib < 100 && Utils.rand(3) == 0 && changes < changeLimit) {
			...
			EngineCore.outputText(" a trace amount of the ghost girl's lust is transferred into you.  How horny IS she, you have to wonder...", false);
			EngineCore.dynStats("lib", 1);
			if (player.lib < 50) {
				EngineCore.dynStats("lib", 1);
			}
		...
		if (changes < changeLimit && player.findPerk(PerkLib.Incorporeality) < 0 && (player.skinTone == "white" || player.skinTone == "sable") && player.hairType == 2) {
			...
			EngineCore.outputText("<b>(Gained Perk:  Incorporeality</b>)", false);
			player.createPerk(PerkLib.Incorporeality, 0, 0, 0, 0);
		}
		//Effect Script 8: 100% chance of healing
		if (changes == 0) {
			EngineCore.outputText("You feel strangely refreshed, as if you just gobbled down a bottle of sunshine.  A smile graces your lips as vitality fills you.  ", false);
	...
	Mutations.prototype.isabellaMilk = function(player) {
		...
		if (player.fatigue > 0) {
			EngineCore.outputText("  You feel much less tired! (-33 fatigue)", false);
		}
		fatigue(-33);
	};
...
//tooltip:
	Mutations.prototype.shriveledTentacle = function(player) {
		...
		if (player.hairType == 4 && player.hairLength < 36 &&
			...
			EngineCore.outputText("\n\nAs you laboriously chew the rubbery dried anemone, your head begins to feel heavier.  Using your newfound control, you snake one of your own tentacles forward; holding it out where you can see it, the first thing you notice is that it appears quite a bit longer.  <b>Your head-tentacles are now " + Utils.num2Text(temp) + " inches longer!</b>", false);
	...
	Mutations.prototype.numbRocks = function(player) {
		...
		if ( Utils.rand(5) == 0) {
			if (player.findStatusAffect(StatusAffects.Dysfunction) < 0) {
				...
				EngineCore.outputText(" numbs up too.  You give yourself a gentle touch, but are quite disturbed when you realize you can barely feel it.  You can probably still fuck something to get off, but regular masturbation is out of the question...", false);
		...
		if (player.findPerk(PerkLib.ThickSkin) < 0 && player.sens < 30 && Utils.rand(4) == 0) {
			EngineCore.outputText("Slowly, ", false);
			if (player.skinType == AppearanceDefs.SKIN_TYPE_PLAIN) {
				EngineCore.outputText("your skin", false);
			} else {
				EngineCore.outputText("the skin under your " + player.skinDesc, false);
			}
			EngineCore.outputText(" begins to feel duller, almost... thicker.  You pinch yourself and find that your epidermis feels more resistant to damage, almost like natural armor!\n<b>(Thick Skin - Perk Gained!)</b>", false);
...
//2. Sensitivity Draft
	Mutations.prototype.sensitivityDraft = function(player) {
		...
		if ( Utils.rand(4) == 0 && player.findStatusAffect(StatusAffects.LustyTongue) < 0) {
			EngineCore.outputText("The constant tingling in your mouth grows and grows, particularly around your lips, until they feel as sensitive as ", false);
			if (player.hasVagina()) {
				EngineCore.outputText("your", false);
			} else {
				EngineCore.outputText("a woman's", false);
			}
			EngineCore.outputText(" lower lips.  You'll have to be careful not to lick them!", false);
			//(Lustytongue status)
		...
		if (player.lust > 70 || player.lib > 70) {
			EngineCore.outputText("You shiver and think of how much better it'll make sex and masturbation.", false);
		} else {
			EngineCore.outputText("You worry it'll make it harder to resist the attentions of a demon.", false);
		}
	...
	Mutations.prototype.foxTF = function(enhanced, player) {
		...
		if (!enhanced) {
			...
			EngineCore.outputText("You pop the cap on the enhanced \"Vixen's Vigor\" and decide to take a swig of it.  Perhaps it will make you as cunning as the crude fox Lumi drew on the front?");
		}
		var changes = 0;
		var changeLimit = 1;
		if (enhanced) {
		...
		if (player.faceType == AppearanceDefs.FACE_FOX && player.tailType == AppearanceDefs.TAIL_TYPE_FOX && player.earType == AppearanceDefs.EARS_FOX && player.lowerBody == AppearanceDefs.LOWER_BODY_TYPE_FOX && player.skinType == AppearanceDefs.SKIN_TYPE_FUR && Utils.rand(3) == 0) {
			if (CoC.getInstance().flags[kFLAGS.FOX_BAD_END_WARNING] == 0) {
				...
				EngineCore.outputText("\n\nYou scarf down the ");
				if (enhanced) {
					EngineCore.outputText("fluid ");
				} else {
					EngineCore.outputText("berries ");
				}
				EngineCore.outputText("with an uncommonly voracious appetite, taking particular enjoyment in the succulent, tart flavor.  As you carefully suck the last drops of ochre juice from your fingers, you note that it tastes so much more vibrant than you remember.  Your train of thought is violently interrupted by the sound of bones snapping, and you cry out in pain, doubling over as a flaming heat boils through your ribs.");
				...
				if (player.tailVenom > 1) {
					EngineCore.outputText("  Your tails thrash around violently as they begin to fuse painfully back into one, the fur bristling back out with a flourish.");
				}
		...
		if (player.hairColor != "golden-blonde" && player.hairColor != "reddish-orange" && player.hairColor != "silver" && player.hairColor != "white" && player.hairColor != "red" && player.hairColor != "black" && changes < changeLimit && Utils.rand(4) == 0) {
			var hairTemp:int = Utils.rand(10);
			if (hairTemp < 5) {
		...
		if (changes < changeLimit && Utils.rand(3) == 0 && player.dogCocks() < player.cocks.length) {
			var nonDogCocks = _.filter(player.cocks, function(cock) { return player.cocks[counter].cockType != CockTypesEnum.DOG; });
			if (nonDogCocks.length != 0) {
				...
				if (player.cocks[selectedNonDogCockIndex].cockType == CockTypesEnum.HUMAN) {
					...
					EngineCore.outputText("\n\nYour " + Appearance.cockNoun(CockTypesEnum.HORSE) + " shrinks, the extra equine length seeming to shift into girth.  The flared tip vanishes into a more pointed form, a thick knotted bulge forming just above your sheath.  <b>You now have a dog-cock.</b>", false);
					//Tweak length/thickness.
					if (player.cocks[selectedNonDogCockIndex].cockLength > 6) player.cocks[selectedNonDogCockIndex].cockLength -= 2;
					else player.cocks[selectedNonDogCockIndex].cockLength -= 0.5;
		...
		if (changes < changeLimit && player.balls > 0 && player.ballSize > 4 && Utils.rand(3) == 0) {
			...
			changes++;
			EngineCore.outputText("  You now have a [balls].");
		}
		//Sprouting more!
		if (changes < changeLimit && enhanced && player.bRows() < 4 && player.breastRows[player.bRows() - 1].breastRating > 1) {
			...
			if (!enhanced) {
				EngineCore.outputText("berry ");
			} else {
				EngineCore.outputText("drink ");
			}
			EngineCore.outputText("settles deeper inside you.  A second later, the unpleasant gut-gurgle passes, and you let out a tiny burp of relief.  Before you finish taking a few breaths, there's an itching below your " + Descriptors.allChestDesc() + ".  You idly scratch at it, but gods be damned, it hurts!  You peel off part of your " + player.armorName + " to inspect the unwholesome itch, ");
			if (player.biggestTitSize() >= 8) {
				EngineCore.outputText("it's difficult to see past the wall of tits obscuring your view.");
			} else {
				EngineCore.outputText("it's hard to get a good look at.");
			}
			EngineCore.outputText("  A few gentle prods draw a pleasant gasp from your lips, and you realize that you didn't have an itch - you were growing new nipples!");
		...
		_.forEach(player.breastRows, function(breastRow, index) {
			...
			if (index > 0 && breastRow.breastRating <= (player.breastRows[index - 1].breastRating - 1) && changes < changeLimit && Utils.rand(2) == 0) {
				...
				EngineCore.outputText("  You do a quick measurement and determine that your " + Utils.num2Text2(index + 1) + " row of breasts are now " + player.breastCup(index) + "s.");
				if (!tits) {
					tits = true;
					changes++;
				}
		...
		if ((enhanced || player.earType == AppearanceDefs.EARS_FOX) && player.lowerBody != AppearanceDefs.LOWER_BODY_TYPE_FOX && changes < changeLimit && Utils.rand(5) == 0) {
			...
			if (player.isTaur()) {
				...
				if (player.lowerBody == AppearanceDefs.LOWER_BODY_TYPE_BEE) {
					EngineCore.outputText(", while a hideous cracking fills the air");
				}
				EngineCore.outputText(".  When the spasms subside and you can once again stand, <b>you find that your legs have been changed to those of a fox!</b>");
			}
		...
		if (player.skinType == AppearanceDefs.SKIN_TYPE_FUR && player.faceType != AppearanceDefs.FACE_FOX && changes < changeLimit && Utils.rand(5) == 0) {
			...
			if (EngineCore.silly()) {
				EngineCore.outputText("  And they called you crazy...");
			}
			changes++;
	...
	Mutations.prototype.godMead = function(player) {
		...
		if ( Utils.rand(3) == 0) {
			...
			if (EngineCore.silly()) {
				EngineCore.outputText("  Maybe you should run around shirtless, drink, and fight!  Saxton Hale would be proud.");
			}
			EngineCore.dynStats("str", 1);
		} else { //Tough:
	...
	Mutations.prototype.sheepMilk = function(player) {
		...
		fatigue(-30);
		EngineCore.dynStats("lib", -.25, "lus", -10, "cor", -0.5);
	};
...
//Consume:
	Mutations.prototype.foxJewel = function(mystic, player) {
		...
		if (((mystic && Utils.rand(2) == 0) || (!mystic && Utils.rand(4) == 0)) && changes < changeLimit && player.femininity != 50) {
			EngineCore.outputText(player.modFem(50, 2), false);
			changes++;
		}
		...
		if (player.hasVagina() && ((mystic && Utils.rand(2) == 0) || (!mystic && Utils.rand(3) == 0)) && player.statusAffectv1(StatusAffects.BonusVCapacity) < 200 && changes < changeLimit) {
			...
			if (EngineCore.silly()) {
				EngineCore.outputText("  <b>Your vagina has become a universe unto itself, capable of accepting colossal insertions beyond the scope of human comprehension!</b>");
			} else {
				EngineCore.outputText("  <b>Your vagina is now capable of accepting even the most ludicrously sized insertions with no ill effects.</b>");
			}
		...
		if (player.skinType == AppearanceDefs.SKIN_TYPE_FUR || player.skinType == AppearanceDefs.SKIN_TYPE_SCALES && ((mystic) || (!mystic && Utils.rand(2) == 0))) {
			...
			if (player.skinType == AppearanceDefs.SKIN_TYPE_FUR) {
				EngineCore.outputText("  You stare in horror as you pull your fingers away holding a handful of " + player.hairColor + " fur!  Your fur sloughs off your body in thick clumps, falling away to reveal patches of bare, " + player.skinTone + " skin.");
			} else if (player.skinType == AppearanceDefs.SKIN_TYPE_SCALES) {
				EngineCore.outputText("  You stare in horror as you pull your fingers away holding a handful of dried up scales!  Your scales continue to flake and peel off your skin in thick patches, revealing the tender " + player.skinTone + " skin underneath.");
			}
			EngineCore.outputText("  Your skin slowly turns raw and red under your severe scratching, the tingling sensations raising goosebumps across your whole body.  Over time, the itching fades, and your flushed skin resolves into a natural-looking ");
			player.skinType = AppearanceDefs.SKIN_TYPE_PLAIN;
			player.skinAdj = "";
			player.skinDesc = "skin";
			if (!mystic && player.skinTone != "tan" && player.skinTone != "olive" && player.skinTone != "light") {
				var skinTemp:int = Utils.rand(3);
				if (skinTemp == 0) {
					player.skinTone = "tan";
				} else if (skinTemp == 1) {
			...
			EngineCore.outputText(player.skinTone + " complexion.");
			EngineCore.outputText("  <b>You now have " + player.skin() + "!</b>");
			changes++;
			...
			EngineCore.outputText(player.skin() + "!</b>");
			changes++;
...
//Flavour Description: A round, opaque glass vial filled with a clear, viscous fluid.  It has a symbol inscribed on it, a circle with a cross and arrow pointing out of it in opposite directions.  It looks and smells entirely innocuous.
	Mutations.prototype.trapOil = function(player) {
		...
		EngineCore.outputText("You pour some of the oil onto your hands and ");
		if (player.cor < 30) {
			EngineCore.outputText("hesitantly ");
		} else if (player.cor > 70) {
			EngineCore.outputText("eagerly ");
		}
		EngineCore.outputText("rub it into your arms and chest.  The substance is warm, coating and ever so slightly numbing; it quickly sinks into your skin, leaving you feeling smooth and sleek.");
		//Speed Increase:
		...
		if (player.lib < 70 && player.hasVagina() && Utils.rand(3) == 0 && changes < changeLimit) {
			...
			EngineCore.dynStats("lib", 2);
			if (player.lib < 30) EngineCore.dynStats("lib", 2);
			changes++;
		}
		//Body Mass Loss:
		...
		if (player.hipRating >= 10 && Utils.rand(4) == 0 && changes < changeLimit) {
			...
			player.hipRating--;
			if (player.hipRating > 15) player.hipRating -= 2 + Utils.rand(3);
			changes++;
		}
		...
		if (player.longestCockLength() >= 3.5 && player.hasCock() && Utils.rand(2) == 0 && changes < changeLimit) {
			...
			EngineCore.outputText(" incredibly sensitive and retract into your body.  Anxiously you pull down your underclothes to examine your nether regions.  To your relief ");
			if (player.cockTotal() == 1) {
				EngineCore.outputText("it is");
			} else {
				EngineCore.outputText("they are");
			}
			EngineCore.outputText(" still present, and as you touch ");
			if (player.cockTotal() == 1) {
				EngineCore.outputText("it");
			} else {
				EngineCore.outputText("them");
			}
			EngineCore.outputText(", the sensitivity fades, however - a blush comes to your cheeks - ");
			if (player.cockTotal() == 1) {
				EngineCore.outputText("it seems");
			} else {
				EngineCore.outputText("they seem");
			}
			EngineCore.outputText(" to have become smaller.");
			_.forEach(player.cocks, function (cock, index) {
		...
		if (player.hasVagina() && Utils.rand(4) == 0 && changes < changeLimit) {
			...
			if (player.fertility >= 30) {
				EngineCore.outputText("It feels like your overcharged reproductive organs have simmered down a bit.");
			} else if (player.fertility >= 5) { //Average fertility:
				EngineCore.outputText("You feel like you have dried up a bit inside; you are left feeling oddly tranquil.");
			} else { //[Low/No fertility:
				EngineCore.outputText("Although the numbness makes you feel serene, the trap oil has no effect upon your ");
				if (player.fertility > 0) {
					EngineCore.outputText("mostly ");
				}
				EngineCore.outputText("sterile system.");
				//[Low/No fertility + Trap/Corruption  >70:
				if (player.cor > 70) {
					EngineCore.outputText("  For some reason the fact that you cannot function as nature intended makes you feel helpless and submissive.  Perhaps the only way to be a useful creature now is to find a dominant, fertile being willing to plow you full of eggs? You shake the alien, yet oddly alluring thought away.");
				}
			}
		...
		if (player.hasVagina() && player.vaginaType() != 5 && Utils.rand(4) == 0 && changes < changeLimit) {
			...
			//(Wet:
			if (player.wetness() >= 3) {
				EngineCore.outputText("  Your natural lubrication makes it gleam invitingly.");
			}
			//(Corruption <50:
			if (player.cor < 50) {
				EngineCore.outputText("  After a few cautious touches you decide it doesn't feel any different- it does certainly look odd, though.");
			} else {
				EngineCore.outputText("  After a few cautious touches you decide it doesn't feel any different - the sheer bizarreness of it is a big turn on though, and you feel it beginning to shine with anticipation at the thought of using it.");
			}
			EngineCore.outputText("  <b>Your vagina is now ebony in color.</b>");
			EngineCore.dynStats("sen", 2, "lus", 10);
			player.vaginaType(5);
			changes++;
		}
...
//>When Used
	Mutations.prototype.purpleFruitEssrayle = function(player) {
		...
		if (player.averageNipplesPerBreast() < 4) {
			EngineCore.outputText("  At first you think nothing has changed, but a second look confirms that your breasts now sport the same quartet of cow-like nipples the bovine plant-girl bears.");
...
//A dried fig with two lobes and thin dark rings just below its stem.  The skin is wrinkly and it looks vaguely like a bulging scrotum.
	Mutations.prototype.ringtailFig = function(player) {
		...
		if ( Utils.rand(4) == 0 && player.thickness < 80 && changes < changeLimit) {
			EngineCore.outputText(player.modThickness(80, 2), false);
			changes++;
		}
		//bodypart changes:
		...
		if (player.earType == AppearanceDefs.EARS_RACCOON && player.lowerBody != AppearanceDefs.LOWER_BODY_TYPE_RACCOON && changes < changeLimit && Utils.rand(4) == 0) {
			...
			if (player.isNaga()) {
				...
				if ((player.lowerBody == AppearanceDefs.LOWER_BODY_TYPE_HUMAN || player.lowerBody == AppearanceDefs.LOWER_BODY_TYPE_DEMONIC_HIGH_HEELS || player.lowerBody == AppearanceDefs.LOWER_BODY_TYPE_DEMONIC_CLAWS) && player.skinType != AppearanceDefs.SKIN_TYPE_FUR) {
					EngineCore.outputText("The sensation of walking around on what feels like a second pair of hands is so weird that you miss noticing the itchy fur growing in over your legs...  ");
				}
				EngineCore.outputText("<b>You now have raccoon paws!</b>");
			}
		...
		if (player.faceType != AppearanceDefs.FACE_RACCOON_MASK && player.faceType != AppearanceDefs.FACE_RACCOON && Utils.rand(4) == 0 && changes < changeLimit) {
			...
			if (player.faceType == AppearanceDefs.FACE_HUMAN || player.faceType == AppearanceDefs.FACE_SHARK_TEETH || player.faceType == AppearanceDefs.FACE_SNAKE_FANGS || player.faceType == AppearanceDefs.FACE_BUNNY) {
				...
				if (player.faceType == AppearanceDefs.FACE_SHARK_TEETH || player.faceType == AppearanceDefs.FACE_SNAKE_FANGS || player.faceType == AppearanceDefs.FACE_BUNNY) {
					EngineCore.outputText("Your prominent teeth chatter noisily at first, then with diminishing violence, until you can no longer feel them jutting past the rest!  ");
				}
				EngineCore.outputText("Shaking your head a bit, you wait for your energy to return, then examine your appearance.  ");
				...
				if (((player.skinTone == "ebony" || player.skinTone == "black") && (player.skinType == AppearanceDefs.SKIN_TYPE_PLAIN || player.skinType == AppearanceDefs.SKIN_TYPE_GOO)) || ((player.hairColor == "black" || player.hairColor == "midnight") && (player.skinType == AppearanceDefs.SKIN_TYPE_FUR || player.skinType == AppearanceDefs.SKIN_TYPE_SCALES))) {
					EngineCore.outputText("Nothing seems different at first.  Strange... you look closer and discover a darker, mask-line outline on your already inky visage.  <b>You now have a barely-visible raccoon mask.</b>");
				} else {
					EngineCore.outputText("A dark, almost black mask shades the " + player.skinFurScales() + " around your eyes and over the topmost portion of your nose, lending you a criminal air!  <b>You now have a raccoon mask!</b>");
				}
				...
				if (((player.hairColor == "black" || player.hairColor == "midnight") && (player.skinType == AppearanceDefs.SKIN_TYPE_FUR || player.skinType == AppearanceDefs.SKIN_TYPE_SCALES))) {
					EngineCore.outputText("your face apparently returned to normal shape, albeit still covered in " + player.skinFurScales() + ".  You look closer and discover a darker, mask-line outline on your already inky visage.  <b>You now have a barely-visible raccoon mask on your otherwise normal human face.</b>");
					...
					EngineCore.outputText("your face apparently returned to normal shape.  You look closer and discover a darker, mask-line outline on your already inky visage.  <b>You now have a barely-visible raccoon mask on your normal human face.</b>");
				} else {
					EngineCore.outputText("your face returned to human dimensions, but shaded by a black mask around the eyes and over the nose!  <b>You now have a humanoid face with a raccoon mask!</b>");
				}
			}
...
//maybe some pregnancy-accelerating thing
	Mutations.prototype.demonChanges = function(player) {
		...
		if (player.tailType != AppearanceDefs.TAIL_TYPE_DEMONIC && player.horns > 0) {
			if (player.tailType != AppearanceDefs.TAIL_TYPE_NONE) {
				EngineCore.outputText("\n\n", false);
				if (player.tailType == AppearanceDefs.TAIL_TYPE_SPIDER_ADBOMEN || player.tailType == AppearanceDefs.TAIL_TYPE_BEE_ABDOMEN) {
					EngineCore.outputText("You feel a tingling in your insectile abdomen as it stretches, narrowing, the exoskeleton flaking off as it transforms into a flexible demon-tail, complete with a round spaded tip.  ", false);
				} else {
					EngineCore.outputText("You feel a tingling in your tail.  You are amazed to discover it has shifted into a flexible demon-tail, complete with a round spaded tip.  ", false);
				}
				EngineCore.outputText("<b>Your tail is now demonic in appearance.</b>", false);
			} else {
		...
		if (player.horns == 0 || ( Utils.rand(player.horns + 3) == 0)) {
			if (player.horns < 12 && (player.hornType == HORNS_NONE || player.hornType == HORNS_DEMON)) {
				EngineCore.outputText("\n\n", false);
				if (player.horns == 0) {
					EngineCore.outputText("A small pair of demon horns erupts from your forehead.  They actually look kind of cute.  <b>You have horns!</b>", false);
				} else {
					EngineCore.outputText("Another pair of demon horns, larger than the last, forms behind the first row.", false);
				}
			...
			} else if (player.hornType > HORNS_DEMON) { //Text for shifting horns
				EngineCore.outputText("\n\n", false);
				EngineCore.outputText("Your horns shift, shrinking into two small demonic-looking horns.", false);
				player.horns = 2;
				player.hornType = HORNS_DEMON;
				EngineCore.dynStats("cor", 3);
			}
		...
		if ((player.faceType != AppearanceDefs.FACE_HUMAN || player.skinType != AppearanceDefs.SKIN_TYPE_PLAIN) && Utils.rand(3) == 0) {
			...
			if (player.faceType != AppearanceDefs.FACE_HUMAN) {
				EngineCore.outputText("\n\n", false);
				EngineCore.outputText("Your visage twists painfully, returning to a more normal human shape, albeit with flawless skin.  <b>Your face is human again!</b>", false);
				player.faceType = AppearanceDefs.FACE_HUMAN;
			} else if (player.skinType != AppearanceDefs.SKIN_TYPE_PLAIN) { //De-fur
				EngineCore.outputText("\n\n", false);
				if (player.skinType == AppearanceDefs.SKIN_TYPE_FUR) EngineCore.outputText("Your skin suddenly feels itchy as your fur begins falling out in clumps, <b>revealing inhumanly smooth skin</b> underneath.", false);
				if (player.skinType == AppearanceDefs.SKIN_TYPE_SCALES) EngineCore.outputText("Your scales begin to itch as they begin falling out in droves, <b>revealing your inhumanly smooth " + player.skinTone + " skin</b> underneath.", false);
				player.skinType = AppearanceDefs.SKIN_TYPE_PLAIN;
				player.skinDesc = "skin";
			}
		}
		//Demon tongue
		if (player.tongueType == TONUGE_SNAKE && Utils.rand(3) == 0) {
		...
		if (player.wingType != AppearanceDefs.WING_TYPE_BAT_LIKE_LARGE && Utils.rand(8) == 0 && player.cor >= 50) {
			...
			if (player.wingType == AppearanceDefs.WING_TYPE_BAT_LIKE_TINY && player.cor >= 75) {
				EngineCore.outputText("\n\n", false);
				EngineCore.outputText("Your small demonic wings stretch and grow, tingling with the pleasure of being attached to such a tainted body.  You stretch over your shoulder to stroke them as they unfurl, turning into full-sized demon-wings.  <b>Your demonic wings have grown!</b>", false);
				...
				EngineCore.outputText("\n\n", false);
				EngineCore.outputText("The muscles around your shoulders bunch up uncomfortably, changing to support the new bat-like wings growing from your back.  You twist your head as far as you can for a look and realize your fin has changed into ", false);
				EngineCore.outputText("small ", false);
				player.wingType = AppearanceDefs.WING_TYPE_BAT_LIKE_TINY;
				player.wingDesc = "tiny, bat-like";
				EngineCore.outputText("bat-like demon-wings!", false);
			} else if (player.wingType == AppearanceDefs.WING_TYPE_BEE_LIKE_SMALL || player.wingType == AppearanceDefs.WING_TYPE_BEE_LIKE_LARGE) {
				EngineCore.outputText("\n\n", false);
				EngineCore.outputText("The muscles around your shoulders bunch up uncomfortably, changing to support your wings as you feel their weight increasing.  You twist your head as far as you can for a look and realize they've changed into ", false);
				...
				EngineCore.outputText("<b>bat-like demon-wings!</b>", false);
			} else if (player.wingType == AppearanceDefs.WING_TYPE_NONE) { //No wings
				EngineCore.outputText("\n\n", false);
				EngineCore.outputText("A knot of pain forms in your shoulders as they tense up.  With a surprising force, a pair of small demonic wings sprout from your back, ripping a pair of holes in the back of your " + player.armorName + ".  <b>You now have tiny demonic wings</b>.", false);
				player.wingType = AppearanceDefs.WING_TYPE_BAT_LIKE_TINY;
				player.wingDesc = "tiny, bat-like";
			}
		}
	...
	Mutations.prototype.princessPucker = function(player) {
		...
		if (player.hairColor != "pink") {
			if ( Utils.rand(5) == 0) {
				EngineCore.outputText("A slight tingle across your scalp draws your attention to your hair.  It seems your " + player.hairColor + " is rapidly gaining a distinctly pink hue, growing in from the roots!\n\n");
	...
	Mutations.prototype.ferretTF = function(player) {
			...
			if(longestCock >= 0 && Utils.rand(2) == 0 && changes < changeLimit) {
				if(player.cocks[longestCock].cockLength > 6 && !CoC.getInstance().flags[kFLAGS.HYPER_HAPPY]) {
					...
					if(player.cocks[longestCock].cockLength/6 >= player.cocks[longestCock].cockThickness) {
						EngineCore.outputText("  Luckily, it doen’t seem to have lost its previous thickness.");
					} else {
		...
		if(player.skinType != AppearanceDefs.SKIN_TYPE_FUR && player.earType == AppearanceDefs.EARS_FERRET && player.tailType == AppearanceDefs.TAIL_TYPE_FERRET && player.lowerBody == AppearanceDefs.LOWER_BODY_FERRET && Utils.rand(4) == 0 && changes < changeLimit) {
			...
			EngineCore.outputText("  <b>You now have " + player.hairColor + " fur!</b>");
			player.skinType = AppearanceDefs.SKIN_TYPE_FUR;
			changes++;
		}
		//Tail TFs!
