	...
	PlayerEvents.prototype.timeChange = function() {
		...
		if( CoC.player.hasCock() && CoC.player.cocks[ 0 ].cockType === CockTypesEnum.BEE ) { //All the hourly bee cock checks except the 'seek out the bee girl' check. That's in timeChangeLarge
			...
			if( CoC.player.cocks[ 0 ].cockLength < 25 || CoC.player.cocks[ 0 ].cockThickness < 4 ) {
				EngineCore.outputText( 'Your ' + CoC.player.cockDescript( 0 ) + ' quivers for a moment before growing slightly ' );
				if( CoC.player.cocks[ 0 ].cockLength < 25 && CoC.player.cocks[ 0 ].cockThickness < 4 ) {
					EngineCore.outputText( 'longer and thicker' );
				} else {
					EngineCore.outputText( CoC.player.cocks[ 0 ].cockLength < 25 ? 'longer again' : 'wider again' );
				}
			...
			EngineCore.outputText( 'The desire to find the bee girl that gave you this cursed ' + CoC.player.cockDescript( 0 ) + ' and have her spread honey all over it grows with each passing minute\n' );
	...
	PlayerEvents.prototype.timeChangeLarge = function() {
		...
		if( this.checkedDream++ === 0 && CoC.time.hours === 3 ) { //You can only have one dream each night
			...
			if( CoC.player.hasCock() && CoC.player.findPerk( PerkLib.SpiderOvipositor ) >= 0 && (CoC.player.eggs() >= 20 && Utils.rand( 6 ) === 0) ) { //Drider dreams proc
				...
				EngineCore.outputText( Descriptors.cockDescript( 0 ) + ' therein; you lower yourself onto ' + CoC.player.mf( 'him', 'her' ) + ' over and over again, spearing your eager pussy with ' + CoC.player.mf( 'him', 'her' ) + ' prick' );
				if( CoC.player.hasVagina() ) {
