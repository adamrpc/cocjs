...
angular.module( 'cocjs' ).factory( 'Naga', function( $log, CoC, Monster, Utils, StatusAffects, Appearance, AppearanceDefs, WeightedDrop, ConsumableLib, Combat, EngineCore, MainView, PerkLib ) {
	...
	Naga.prototype.nagaPoisonBiteAttack = function() {
		...
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.NagaVenom ) < 0 ) {
			...
			if( CoC.getInstance().player.spe > 4 ) {
				//stats(0,0,-3,0,0,0,0,0);
				CoC.getInstance().player.spe -= 3;
				MainView.statsView.showStatDown( 'spe' );
				// speUp.visible = false;
				// speDown.visible = true;
				CoC.getInstance().player.createStatusAffect( StatusAffects.NagaVenom, 3, 0, 0, 0 );
			} else {
				CoC.getInstance().player.createStatusAffect( StatusAffects.NagaVenom, 0, 0, 0, 0 );
				CoC.getInstance().player.takeDamage( 5 + Utils.rand( 5 ) );
			}
			CoC.getInstance().player.takeDamage( 5 + Utils.rand( 5 ) );
		} else {
			...
			if( CoC.getInstance().player.spe > 3 ) {
	...
	Naga.prototype.nagaConstrict = function() {
		...
		CoC.getInstance().player.createStatusAffect( StatusAffects.NagaBind, 0, 0, 0, 0 );
		CoC.getInstance().player.takeDamage( 2 + Utils.rand( 4 ) );
		Combat.combatRoundOver();
	};
	...
	Naga.prototype.nagaTailWhip = function() {
		...
		//[if evaded]
		if( (CoC.getInstance().player.findPerk( PerkLib.Evade ) && Utils.rand( 6 ) === 0) ) {
		...
		} else if( CoC.getInstance().player.spe > Utils.rand( 300 ) ) {
			...
			var damage = 10;
			if( CoC.getInstance().player.armorDef < 10 ) {
				damage += 10 - CoC.getInstance().player.armorDef;
			}
			damage += Utils.rand( 3 );
