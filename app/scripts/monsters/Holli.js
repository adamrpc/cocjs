...
angular.module( 'cocjs' ).factory( 'Holli', function( SceneLib, MainView, CockTypesEnum, CoC, Monster, Utils, StatusAffects, Appearance, AppearanceDefs, Combat, EngineCore ) {
	...
	Holli.prototype.escapeFailWithHolli = function() {
		...
		if( CoC.player.canFly() ) {
			MainView.outputText( 'beat your wings and ' );
		}
		MainView.outputText( 'try to escape, but ' + this.short + ' wraps one of her writhing roots around your [leg], slamming you to the ground and tying you up with several more!  "<i>And just where do you think you\'re going, my little meat?</i>" she hisses.  Her bark splits open, exposing her body, and a green shaft snakes out of her crotch, sprouting thorns and blooming into a rose at the tip.  She holds the drooling blossom over your [face] as she forces your mouth open with her roots!' );
	...
	Holli.prototype.fuckinJamanjiFlowerDarts = function() {
		...
		if( Combat.combatMiss() || Combat.combatEvade() || Combat.combatFlexibility() || Combat.combatMisdirect() ) {
			MainView.outputText( '  Nimbly, you step aside and let the darts whistle by.' );
		}
		//Hit;
		else {
			MainView.outputText( '  The darts find flesh, and you feel yourself slowing down drastically; all you want to do as the plant woman\'s poison takes you is fuck and sleep.  "<i>Just give up,</i>" Holli coos.  "<i>Think how good it would be to fall into my arms and ' );
			if( CoC.player.hasCock() ) {
				MainView.outputText( 'come inside me' );
			} else {
				MainView.outputText( 'have me inside you' );
			}
			MainView.outputText( ', forever...</i>"' );
	...
	Holli.prototype.holliConstrictAttack = function() {
		...
		//Miss;
		if( Combat.combatMiss() || Combat.combatEvade() || Combat.combatFlexibility() || Combat.combatMisdirect() ) {
			MainView.outputText( '  It\'s a narrow thing, but you manage to avoid the roots - one of them almost grabs you, but you duck aside and let it find only its neighbor.' );
		}
		//Hit;
		else {
			MainView.outputText( '  She latches onto you with a painful smack and several more root tentacles join the first; as she pulls you close, her bark opens and a long, phallic stalk extends from her crotch, wrapped in thorns and flowering with a rose!  It caresses your face, then dangles the blossom above your mouth, dripping her sap.  Several of the roots pry your jaws apart, forcing you to drink the tainted fluids from her pseudo-cock!  "<i>What do you think of my little sap rose?</i>"' );
	...
	Holli.prototype.struggleOutOfHolli = function() {
		...
		if( CoC.player.tongueType > AppearanceDefs.TONUGE_HUMAN ) {
			MainView.outputText( 'You can\'t move an arm nor a [leg] to bat the flower away... but she\'s literally holding your mouth open.  Your long tongue rolls out, gripping and ripping out several of the petals on the end of her stalk!  Holli screams and her roots slacken, allowing you to batter your way out of them.' );
		...
		//else if normal str-based success;
			...
			MainView.outputText( 'You manage to force the roots open when the distracted Holli begins to stroke her plant-shaft, pulling out of the bindings just as a drop of sap oozes out and falls where you were standing.  You\'re free!' );
			//sap rose pls go;
			CoC.player.removeStatusAffect( StatusAffects.HolliConstrict );
		}
		//Struggle Fail/Wait;
		else {
			MainView.outputText( 'You try to escape the entangling roots, but cannot break their grip!  ' );
