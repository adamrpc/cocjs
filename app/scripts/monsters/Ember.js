...
angular.module( 'cocjs' ).factory( 'Ember', function( kFLAGS, MainView, SceneLib, CockTypesEnum, PerkLib, CoC, Monster, Utils, StatusAffects, Appearance, AppearanceDefs, Combat ) {
	...
	Ember.prototype.emberReactsToLustiness = function() {
		...
		MainView.outputText( 'The dragon moans, weaving softly from side to side, eyes glazed and tongue lolling at the intimate prospect of sex... but then, to your surprise, ' + this.emberMF( 'he', 'she' ) + ' visibly shakes it off and recomposes ' + this.emberMF( 'him', 'her' ) + 'self, frowning at you.' );
		...
		if( CoC.player.lib >= 50 ) {
			MainView.outputText( '  How boring.' );
		}
		this.gems = 0;
		this.XP = 0;
		this.HP = 0;
	...
	Ember.prototype.emberAttack = function() {
		...
		//Miss/dodge;
			...
			MainView.outputText( 'You dodge aside at the last second and Ember\'s claws whistle past you.' );
		} else {
			var damage = Math.ceil( (this.str + this.weaponAttack) - Utils.rand( CoC.player.tou ) - CoC.player.armorDef );
			if( damage <= 0 ) {
				MainView.outputText( 'Ember\'s claws scrape noisily but harmlessly off your [armor].' );
			} else {
				damage = CoC.player.takeDamage( damage );
				MainView.outputText( 'Ember\'s claws rip into you, leaving stinging wounds.' );
	...
	Ember.prototype.embersSupahSpecialDragonBreath = function() {
		if( this.findStatusAffect( StatusAffects.Blind ) >= 0 && Utils.rand( 2 ) === 0 ) {
			...
			if( Combat.combatMiss() || Combat.combatEvade() || Combat.combatFlexibility() || Combat.combatMisdirect() ) {
				MainView.outputText( '  It\'s a narrow thing, but you manage to throw yourself aside at the last moment.  Fortunately, the energy whirling around and tearing up the soil blinds Ember to your escape until you have recovered and are ready to keep fighting.' );
			} else {
				MainView.outputText( '  The pain as the deadly combination washes over you is indescribable.  It\'s a miracle that you endure it, and even Ember looks amazed to see you still standing.' );
	...
	Ember.prototype.emberTailSlap = function() {
		...
		if( Combat.combatMiss() || Combat.combatEvade() || Combat.combatFlexibility() || Combat.combatMisdirect() || Utils.rand( 2 ) === 0 ) {
			...
			if( Utils.rand( 2 ) === 0 ) {
				MainView.outputText( 'duck under' );
			} else {
				MainView.outputText( 'leap over' );
			}
			MainView.outputText( ' the tail at the last moment, causing Ember to lose control of ' + this.emberMF( 'his', 'her' ) + ' own momentum and stumble.' );
		} else {
			var damage = Math.ceil( (this.str + this.weaponAttack + 100) - Utils.rand( CoC.player.tou ) - CoC.player.armorDef );
			MainView.outputText( '  The tail slams into you with bone-cracking force, knocking you heavily to the ground even as the spines jab you wickedly.  You gasp for breath in pain and shock, but manage to struggle to your feet again.' );
			damage = CoC.player.takeDamage( damage );
			MainView.outputText( ' (' + damage + ')' );
		}
		Combat.combatRoundOver();
