...
angular.module( 'cocjs' ).factory( 'SecretarialSuccubus', function( AbstractSuccubus, Appearance, WeightedDrop, CoC, EngineCore, DungeonCore, Utils, AppearanceDefs, Monster, Combat, StatusAffects, ConsumableLib ) {
	...
	SecretarialSuccubus.prototype.defeated = function( hpVictory ) {
		if( CoC.getInstance().player.gender > 0 ) {
			var dildo = (CoC.getInstance().player.hasKeyItem( 'Deluxe Dildo' ) >= 0 ? DungeonCore.succubusGetsDildoed : null);
			if( hpVictory ) {
				...
				EngineCore.choices( 'Yes', DungeonCore.succubusVictoryRape, 'Dildo Rape', dildo, '', null, '', null, 'No', Combat.cleanupAfterCombat );
