...
angular.module( 'cocjs' ).factory( 'Kitsune', function( CoC, PerkLib, Monster, Utils, StatusAffects, Appearance, AppearanceDefs, WeightedDrop, Combat, EngineCore, ConsumableLib, SandWitch, kFLAGS, CockTypesEnum ) {
	...
	Kitsune.prototype.kitsuneEntwine = function() {
		EngineCore.outputText( 'The kitsune closes in on you with a mischievous glint in her eyes.  You raise your guard, keeping your eyes trained on her to ensure that she doesn\'t try to pull anything.  Suddenly, you feel something coiling around your ' + CoC.getInstance().player.leg() + ', and let out a yelp as you are suddenly lifted into the air, entangled in the kitsune\'s tails!' );
		...
		EngineCore.dynStats( 'lus', 10 + CoC.getInstance().player.sens / 8 );
	...
	Kitsune.prototype.kitsuneStruggle = function() {
		...
		//Success
		if( Utils.rand( 20 ) + CoC.getInstance().player.str / 20 + this.statusAffectv1( StatusAffects.PCTailTangle ) >= 12 ) {
	...
	Kitsune.prototype.illusionKitsuneAttack = function() {
		...
		if( CoC.getInstance().player.inte < 30 ) {
			resist = Math.round( CoC.getInstance().player.inte );
		} else {
			resist = 30;
		}
		...
		if( CoC.getInstance().player.findPerk( PerkLib.HistoryReligious ) >= 0 && CoC.getInstance().player.cor < 20 ) {
			resist += 20 - CoC.getInstance().player.cor;
		}
	...
	Kitsune.prototype.kitsuneSealAttack = function() {
		...
		if( select === 0 ) {
			...
			CoC.getInstance().player.createStatusAffect( StatusAffects.Sealed, 4, 0, 0, 0 );
		} else if( select === 1 ) {
			//Tease:
			...
			CoC.getInstance().player.createStatusAffect( StatusAffects.Sealed, 4, 1, 0, 0 );
		}
		//Spells:
		else if( select === 2 ) {
			...
			CoC.getInstance().player.createStatusAffect( StatusAffects.Sealed, 4, 2, 0, 0 );
		}
		//Items:
		else if( select === 3 ) {
			...
			CoC.getInstance().player.createStatusAffect( StatusAffects.Sealed, 4, 3, 0, 0 );
		}
		//Run:
		else if( select === 4 ) {
			EngineCore.outputText( '"<i>Tsk tsk, leaving so soon?</i>"  the kitsune says, popping up in front of you suddenly as you attempt to make your escape.  Before you can react, she draws a small circle on your chest with her fingertip, leaving behind a glowing rune made of crackling blue flames.  You try to run the other way, but your ' + CoC.getInstance().player.legs() + ' won\'t budge!\n\n"<i>Sorry baby, you\'ll just have to stay and play~.</i>" she says in a singsong tone, appearing in front of you again.  <b>The kitsune\'s spell prevents your escape!</b>  You\'ll have to tough it out until the spell wears off.' );
			CoC.getInstance().player.createStatusAffect( StatusAffects.Sealed, 4, 4, 0, 0 );
		}
		//P.Special:
		else if( select === 5 ) {
			...
			CoC.getInstance().player.createStatusAffect( StatusAffects.Sealed, 4, 5, 0, 0 );
		}
		//M.Special:
		else {
			...
			CoC.getInstance().player.createStatusAffect( StatusAffects.Sealed, 4, 6, 0, 0 );
		}
	...
	Kitsune.prototype.kitSuneTeases = function() {
		...
		EngineCore.dynStats( 'lus', 5 + CoC.getInstance().player.sens / 7 );
		Combat.combatRoundOver();
	};
	...
	SandWitch.prototype.init = function( that, args ) {
		...
		if( Utils.rand( 3 ) !== 2 ) {
			CoC.getInstance().flags[ kFLAGS.redheadIsFuta ] = 1;
		}
		that.a = 'a ';
		that.short = 'kitsune';
		...
		// this.plural = false;
		if( args[ 0 ] === 'red' && CoC.getInstance().flags[ kFLAGS.redheadIsFuta ] === 1 ) {
