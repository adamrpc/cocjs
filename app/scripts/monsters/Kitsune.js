...
angular.module( 'cocjs' ).factory( 'Kitsune', function( SceneLib, MainView, CoC, PerkLib, Monster, Utils, StatusAffects, Appearance, AppearanceDefs, WeightedDrop, Combat, EngineCore, ConsumableLib, SandWitch, kFLAGS, CockTypesEnum ) {
	...
	Kitsune.prototype.kitsuneStruggle = function() {
		...
		//Struggle:
		MainView.outputText( 'You struggle against the kitsune\'s tails with all your might, desperately trying to free yourself before she has her way with you.' );
		//Success
		if( Utils.rand( 20 ) + CoC.player.str / 20 + this.statusAffectv1( StatusAffects.PCTailTangle ) >= 12 ) {
			MainView.outputText( '  Summoning up reserves of strength you didn\'t know you had, you wrench yourself free of her tails, pushing her away.\n\n' );
			this.removeStatusAffect( StatusAffects.PCTailTangle );
			this.doAI();
		}
		//Failure - +5-10 LUST
		else {
			MainView.outputText( '  Despite your valiant efforts, your wriggling only serves to get you deeper entangled in the fluffy tails, eliciting an amused giggle from the kitsune.' );
	...
	Kitsune.prototype.kitsuneWait = function() {
		MainView.clearOutput();
		MainView.outputText( 'Happily, you slump deeper into the fluffy tails, eliciting an amused giggle from the kitsune.' );
		if( EngineCore.silly() ) {
			MainView.outputText( '  You\'re so glad you got to touch fluffy tail.' );
		}
		MainView.outputText( '\n\nShe licks her lips, running her hands along you wherever she can find exposed flesh.  Her fingertips leave small trails of dazzling blue that make you flush with lust - you must escape her grasp soon or else you will be like putty in her hands!' );
		EngineCore.dynStats( 'lus', 5 + CoC.player.sens / 10 );
		Combat.combatRoundOver();
	...
	Kitsune.prototype.foxFireAttack = function() {
		MainView.outputText( 'The kitsune makes a small circle in the air with her fingers, conjuring up a pale blue flame into her palm with the sound of flint striking against steel.  Pursing her lips, she blows it toward you with a kiss.' );
	...
	Kitsune.prototype.illusionKitsuneAttack = function() {
		MainView.outputText( 'You struggle to keep your eyes on the kitsune, ghostly laughter echoing all around you as you turn to and fro, trying to track her movements.  It almost seems like the edges of reality are blurring around her, severely distorting your perceptions and making it hard to follow her.  It\'s going to be much harder to hit her if she keeps this up!' );
	...
	Kitsune.prototype.kitsuneSealAttack = function() {
		...
		//Attack:
		if( select === 0 ) {
			MainView.outputText( 'The kitsune playfully darts around you, grinning coyly.  She somehow slips in under your reach, and before you can react, draws a small circle on your chest with her fingertip.  As you move to strike again, the flaming runic symbol she left on you glows brightly, and your movements are halted mid-swing.' );
		...
		//M.Special:
			...
			MainView.outputText( 'You jump with surprise as the kitsune appears in front of you, grinning coyly.  As she draws a small circle on your forehead with her fingertip, you find that you suddenly can\'t remember how to use any of your magical skills!' );
			MainView.outputText( '\n\n"<i>Oh no darling, </i>I\'m<i> the one with all the tricks here.</i>"' );
	...
	Kitsune.prototype.kitSuneTeases = function() {
		...
		if( select === 0 ) {
			MainView.outputText( 'You rub your eyes, suddenly seeing triple as you find yourself in the midst of a crowd of kitsune doppelgangers.  They run their hands all over you, teasing and doting on you as their tails caress every inch of your body.  Taken by surprise, you forget to fight back until they have already dispersed, blending back into a single fox-woman.' );
		} else if( select === 1 ) {
			MainView.outputText( 'Bending forward, the kitsune runs her hands down over her breasts, jiggling them enticingly and squeezing them together.  Hooking a finger in her robes, she slides it down, tugging them aside until her nipples are just barely covered, and with a teasing smirk, pulls them back up, leaving you wanting.' );
		} else if( select === 2 ) {
			MainView.outputText( 'Turning her back to you, the kitsune fans out her tails, peering back as she lifts the hem of her robe to expose her plump hindquarters.  Her tails continually shift and twist, blocking your view, but it only serves to make you want it even <i>more</i>, licking your lips in anticipation.' );
		}//Redhead only:
		else {
