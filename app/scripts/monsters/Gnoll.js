...
angular.module( 'cocjs' ).factory( 'Gnoll', function( MainView, SceneLib, ChainedDrop, ConsumableLib, CoC, EngineCore, Monster, Utils, AppearanceDefs, StatusAffects, Appearance, Combat, PerkLib ) {
	...
	Gnoll.prototype.gnollAttackText = function() {
		...
		//Determine if dodged!;
			...
			if( CoC.player.spe - this.spe < 8 ) {
				...
				MainView.outputText( 'You deftly avoid ' + this.a + this.short + '\'s slow ' + this.weaponVerb + '.\n', false );
			}
		}
		//Determine if evaded;
		...
		//Determine if cat'ed;
			...
			if( damage <= 0 ) {
				...
				if( this.short === 'harpy' ) {
					MainView.outputText( 'The harpy dives at you with her foot-talons, but you deflect the attack, grasp onto her leg, and swing her through the air, tossing her away from you before she has a chance to right herself.', false );
				}
			...
			//everyone else;
				...
				//Gnoll Attack #1;
				if( attack === 0 ) {
					MainView.outputText( 'The gnoll leaps forward, her jaws slamming shut across your upper arm.  She twists away before you can touch her, laughing the entire time.', false );
					damage += 10;
				}
				//Gnoll Attack #2;
				else if( attack === 1 ) {
					MainView.outputText( 'With a shudder and lurch, the gnoll barrels forward into your gut, the claws of her free hand raking across your belly.', false );
					damage += 3;
				}
				//Gnoll Attack #3;
				else if( attack === 2 ) {
					MainView.outputText( 'The gnoll tumbles to the ground, then comes up with a handful of sand.  The sand goes in your face; the club goes into your cheek.  Ow.', false );
					damage += 13;
				}
				//Gnoll Attack #4;
				else if( attack === 3 ) {
					MainView.outputText( 'The hyena girl giggles and darts forward, teeth snapping.  Spittle flies everywhere, and the snapping teeth find purchase, drawing red lines across your body.', false );
					damage += 8;
				}
				//Gnoll Attack #5;
				else if( attack === 4 ) {
					MainView.outputText( 'With a mocking laugh, the gnoll brings her club high and then down in a savage strike that catches you across the temple.', false );
					damage += 25;
				}
				//Gnoll Attack #6;
				else {
					MainView.outputText( 'The gnoll waves her club threateningly, but it\'s her foot that snaps up from the dusty plain to connect with your gut.', false );
				}
	...
	Gnoll.prototype.gnollTease = function() {
		...
		//Gnoll Tease #1;
		if( tease === 0 ) {
			MainView.outputText( 'The gnoll takes a moment to stretch her sleek, athletic body.  Her free hand runs up her side and she leers knowingly at you.', false );
			bonus += 5;
		}
		//Gnoll Tease #2;
		else if( tease === 1 ) {
			MainView.outputText( 'With one hand, the hyena girl grasps her eight-inch clitoris and strokes it.  "<i>I know you\'re curious!</i>" she laughs.  "<i>You want to try this.</i>"', false );
			bonus += 5;
		}
		//Gnoll Tease #3;
		else if( tease === 2 ) {
			MainView.outputText( 'The gnoll bounds forward, but instead of clobbering you she slides her lithe body against yours.  "<i>We don\'t have to fight,</i>" she titters.  "<i>It\'s lots easier if I just fuck you.</i>"', false );
			bonus += 10;
		}
		//Gnoll Tease #4;
		else if( tease === 3 ) {
			MainView.outputText( 'The gnoll slides her fingers down the length of her pseudo-penis and collects the cream that drips from its end.  With two steps, she\'s inside your guard, but all she does is wave her hand in front of your nose.  The reek of sex nearly bowls you over.', false );
			bonus += 12;
		}
		//Gnoll Tease #5;
		else if( tease === 4 ) {
			MainView.outputText( '"<i>I love outlanders,</i>" the gnoll confides in you as she circles.  "<i>You have such interesting cries when you get fucked in a new way.</i>"  She laughs, and the sound is far louder than it has any right to be.\n\n', false );
		}
		//Gnoll Tease #6;
		else {
			MainView.outputText( 'The gnoll dances forward, then back, her whole body alive with sensual movement.  She catches the way you watch her and smirks, throwing in a hip-shake just for you.', false );
			bonus += 6;
		}
