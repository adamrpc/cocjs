...
angular.module( 'cocjs' ).factory( 'Izumi', function( $log, CoC, kFLAGS, EngineCore, Monster, Utils, AppearanceDefs, StatusAffects, Appearance, Combat ) {
	...
	Izumi.prototype.performCombatAction = function() {
		...
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.Chokeslam ) >= 0 ) {
			$log.debug( 'ChokeSlam Rounds to Damage: ' + CoC.getInstance().player.statusAffectv1( StatusAffects.Chokeslam ) );
			CoC.getInstance().player.addStatusValue( StatusAffects.Chokeslam, 1, -1 );
			if( CoC.getInstance().player.statusAffectv1( StatusAffects.Chokeslam ) <= 0 ) {
		...
		// Handle groundpound
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.Groundpound ) >= 0 ) {
			CoC.getInstance().player.addStatusValue( StatusAffects.Groundpound, 1, -1 );
			if( CoC.getInstance().player.statusAffectv1( StatusAffects.Groundpound ) <= 0 ) {
		...
		if( (this.HPRatio() <= 0.25) && (this.findStatusAffect( StatusAffects.UsedTitsmother ) <= -1) ) {
			...
			if( CoC.getInstance().player.findStatusAffect( StatusAffects.Groundpound ) <= -1 ) {
	...
	Izumi.prototype.straightJab = function() {
		EngineCore.outputText( 'Quick as a flash, this.Izumi lashes out with her free hand, aiming for your head.' );
		var damage = Math.ceil( (this.str + 175) - Utils.rand( CoC.getInstance().player.tou ) - CoC.getInstance().player.armorDef );
		if( Combat.combatMiss() || Combat.combatEvade() || Combat.combatFlexibility() || Combat.combatMisdirect() ) {
			EngineCore.outputText( '  You deftly dodge under the lightning-quick punch.' );
		} else if( damage <= 0 ) {
			EngineCore.outputText( '  You lash out and manage to deflect the blow before it can connect.' );
		} else {
			EngineCore.outputText( '  Her fist connects with your chin with a mighty crack, sending you sailing across the cave.  this.Izumi smirks at you as you' );
			if( CoC.getInstance().player.isNaga() ) {
				EngineCore.outputText( ' raise back up onto your [legs]' );
			} else {
				EngineCore.outputText( ' stand' );
			}
			EngineCore.outputText( ' and dust yourself off.' );
	...
	Izumi.prototype.roundhouseKick = function() {
		EngineCore.outputText( 'Izumi leaps backwards onto one foot, spinning around and unleashing a thundering roundhouse kick.  Luckily, you manage to duck just in time, avoiding what surely would have been a monstrously powerful blow.  Unfortunately, as Izumi’s leg scythes through the air over your head, you find your gaze naturally following the line of her thigh muscles until you’re staring directly up the fluttering folds of Izumi’s increasingly impractical kimono.\n\n' );
		if( CoC.getInstance().player.cor >= 50 || CoC.getInstance().player.lib >= 50 || CoC.getInstance().player.sens >= 50 ) {
			EngineCore.outputText( 'You fall backwards and stagger away, already feeling a flush of warmth colouring your cheeks, trying to drag your mind back to the fight and away from... other things.' );
			EngineCore.dynStats( 'lus', 10 + CoC.getInstance().player.lib / 10 );
		} else {
			EngineCore.outputText( 'You furrow a brow at the Oni\'s ineffectual attack, not entirely sure if she was intending to hurt you or turn you on.  Her thighs did look rather tantalizing though...' );
	...
	Izumi.prototype.chokeSlam = function() {
		if( Combat.combatMiss() || Combat.combatEvade() || Combat.combatFlexibility() || Combat.combatMisdirect() ) {
			EngineCore.outputText( 'Izumi surges towards you, closing the distance between you within the blink of an eye. You narrowly avoid her crushing grip, twisting away from her grasp at the last moment.  The enormous Oni lets loose a deep, satisfied laugh.' );
		} else {
			EngineCore.outputText( 'Izumi surges towards you, smashing aside your guard and seizing you by the throat in the blink of an eye.  Lifting you above her head, you can only struggle to breathe as the enormous Oni grins at you like some sort of prize.' );
			CoC.getInstance().player.createStatusAffect( StatusAffects.Chokeslam, 3, 0, 0, 0 );
	...
	Izumi.prototype.chokeSlamStruggle = function() {
		...
		var brokeFree;
		if( Utils.rand( CoC.getInstance().player.str ) > this.str / 2 ) {
			brokeFree = true;
		}
		if( brokeFree ) {
			...
			EngineCore.outputText( 'Izumi\'s grip around your throat continues to strangle the breath from your lungs as she holds you aloft.  Your fingers tighten in turn around the Oni\'s wrist, fighting against her' );
			if( CoC.getInstance().player.str < 90 ) {
				EngineCore.outputText( ' immense' );
			} else {
				EngineCore.outputText( ' impressive' );
			}
			EngineCore.outputText( ' strength, in an attempt to free yourself from her crushing embrace, without success.' );
			CoC.getInstance().player.takeDamage( 75 + Utils.rand( 15 ) );
			this.doAI();
		}
	};
	// OH HEY ITS A THING
	Izumi.prototype.chokeSlamWait = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.PC_FETISH ] >= 2 ) {
			EngineCore.outputText( ' and to be honest, the grip isn\'t an entirely unplesent experience, either.  If only Izumi would stop playing around and just <i>take you</i> already.' );
	...
	Izumi.prototype.chokeSlamDamage = function() {
		EngineCore.outputText( 'With a grunt of effort, this.Izumi hauls you through the air, her iron-like grip around your throat providing the perfect anchor to propel you towards the ground.  Before you have a chance to react, the Oni drives you into the unforgiving stone lining the floor of the cave.\n\n' );
		EngineCore.outputText( 'The hit is extreme enough to leave you dazed for a moment, splayed out across the floor.  When you rouse yourself back to full consciousness a few seconds later, the cave is still echoing with the sound of the impact, a testament to the strength of the Oni - and your resilience.' );
		var damage = Math.ceil( (this.str + 225) - Utils.rand( CoC.getInstance().player.tou ) - CoC.getInstance().player.armorDef );
	...
	Izumi.prototype.chokeSlamEscape = function() {
		...
		EngineCore.outputText( 'Scrabbling desperately against her wrist, you narrow your eyes at the Oni woman’s superior expression,' );
		if( CoC.getInstance().player.isBiped() ) {
			EngineCore.outputText( ' raise a [leg] and kick her roundly' );
		} else if( CoC.getInstance().player.isNaga() ) {
			EngineCore.outputText( ' raise your tail and slap her solidly' );
		} else {
			EngineCore.outputText( ' and slap her square' );
		}
		EngineCore.outputText( ' in the face.  this.Izumi drops you, staggering back in surprise.  “Ow!”  She actually yelps, covering her face with her hands.\n\n' );
		EngineCore.outputText( 'You drop to the ground and roll away, expecting some form of retribution.  this.Izumi glares at you from behind her hand for a moment, then snickers.  Slowly, she drops back into her fighting stance and gestures for your bout to continue.' );
		this.cleanupChokeslam();
		this.HP -= 50 + Utils.rand( CoC.getInstance().player.str );
		Combat.combatRoundOver();
	};
	...
	Izumi.prototype.cleanupChokeslam = function() {
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.Chokeslam ) >= 0 ) {
			...
			CoC.getInstance().player.removeStatusAffect( StatusAffects.Chokeslam );
		}
	};
	// Groundslam, does damage and slows the player if they don't dodge the hit
	Izumi.prototype.groundPound = function() {
		EngineCore.outputText( 'Izumi raises one mighty foot and slams it to the ground with a victorious yell.  The ground itself actually shakes below your feet, threatening to knock you off balance.\n\n' );
		if( Combat.combatMiss() || Combat.combatEvade() || Combat.combatFlexibility() || Combat.combatMisdirect() ) // TODO: ensure this is correct
		{
			EngineCore.outputText( 'Leaping to the side, you manage to steady yourself against the wall, keeping your footing.' );
		} else {
			EngineCore.outputText( 'The rumbling actually knocks you off your feet, sprawling on the ground and banging your head.  As the shaking subsides, you pull yourself upright, but you feel a little unsteady on your [feet] after the disorienting impact.' );
			var spdReducedBy = Math.ceil( CoC.getInstance().player.spe * 0.25 );
			CoC.getInstance().player.createStatusAffect( StatusAffects.Groundpound, 3, spdReducedBy, 0, 0 );
	...
	Izumi.prototype.cleanupGroundpound = function() {
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.Groundpound ) >= 0 ) {
			...
			CoC.getInstance().player.spe += CoC.getInstance().player.statusAffectv2( StatusAffects.Groundpound );
			if( CoC.getInstance().player.spe > 100 ) {
				CoC.getInstance().player.spe = 100;
			}
			CoC.getInstance().player.removeStatusAffect( StatusAffects.Groundpound );
	...
	Izumi.prototype.titSmother = function() {
		...
		EngineCore.outputText( 'With a sudden burst of speed, the Oni woman bullrushes you, slapping aside your hasty defence.  You brace yourself for a powerful impact, but rather than strike you she instead thrusts her arm straight past your head.  Bemused, you turn your head to follow her fist, just in time to see her crook her elbow and yank you back towards her - hard.  Pulled right off your [feet] by the sudden strike, you slam' );
		if( CoC.getInstance().player.hasMuzzle() ) {
			EngineCore.outputText( ' muzzle-' );
		} else {
			EngineCore.outputText( ' face-' );
		}
		EngineCore.outputText( 'first into Izumi - specifically, into her chest.  Shocked by suddenly having your face rammed into the pillowy soft expanse of Izumi’s bust, you rear back only to be slammed straight back into the mountainous expanse by Izumi’s arm.' );
		CoC.getInstance().player.createStatusAffect( StatusAffects.Titsmother, 0, 0, 0, 0 );
		EngineCore.dynStats( 'lus', (CoC.getInstance().player.lib / 15) + 5 + Utils.rand( 5 ) );
		Combat.combatRoundOver();
	};
	// Remove the effect post-combat
	Izumi.prototype.cleanupTitsmother = function() {
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.Titsmother ) >= 0 ) {
			CoC.getInstance().player.removeStatusAffect( StatusAffects.Titsmother );
	...
	Izumi.prototype.titSmotherStruggle = function() {
		...
		var brokeFree;
		if( Utils.rand( CoC.getInstance().player.str ) > this.str / 4 ) {
			brokeFree = true;
		}
		if( brokeFree ) {
			...
			if( Utils.rand( 2 ) === 0 ) {
				...
				if( CoC.getInstance().player.hasCock() ) {
					...
					EngineCore.outputText( ' just makes things even more unbearable, as the harder' );
					if( CoC.getInstance().player.totalCocks() > 1 ) {
						EngineCore.outputText( ' they get' );
					} else {
						EngineCore.outputText( ' it gets' );
					}
					EngineCore.outputText( ', the more insistently your' );
					if( CoC.getInstance().player.totalCocks() > 1 ) {
						EngineCore.outputText( ' erections throb' );
					} else {
						EngineCore.outputText( ' erection throbs' );
					}
					EngineCore.outputText( ', pressed up against Izumi’s stomach muscles.  Her muscles ripple and undulate as she struggles to keep you in her grip, abs flexing, bumping, encircling your insistent erection' );
					if( CoC.getInstance().player.totalCocks() > 1 ) {
						EngineCore.outputText( 's' );
					}
					EngineCore.outputText( ', stimulating you even further.  You realize in a flash of panic that if you don’t get out of this soon, you may actually... ' );
				} else {
					EngineCore.outputText( 'Izumi’s bust encloses you on all sides, leaving you feeling like you’re trapped in some kind of breast sarcophagus.  The heat radiating from the soft flesh combines with the scent of whatever strange drug Izumi had been smoking, now hanging around her like some heady perfume.' );
				}
			}
			EngineCore.dynStats( 'lus', CoC.getInstance().player.lib / 15 + 5 + Utils.rand( 5 ) );
			this.doAI();
	...
	Izumi.prototype.titSmotherEscape = function() {
		...
		if( CoC.getInstance().player.str < 90 ) {
			...
			EngineCore.outputText( 'Locking your arms against Izumi’s shoulders, you heave with all your might against the musclebound Oni girl’s choke hold.  You can feel her arm straining to hold you, struggling to resist, giving ground....' );
			if( CoC.getInstance().player.isBiped() ) {
				EngineCore.outputText( '  As soon as you can, you hike up your legs and place your feet firmly on Izumi’s stomach, adding your leg muscles to the effort.' );
			}
			EngineCore.outputText( '  this.Izumi grits her teeth and growls as she pulls with all her might, trying to force your limbs to give way, but to no avail - with a final thrust, this.Izumi lets out a yelp as you knock her arm aside and leap away.  this.Izumi rolls her arm around a little, massaging her shoulder as she regards you, thoughtfully.  Then she reaches up and fans at her face with one hand, grinning that suggestive grin.\n\n' );
		}
		EngineCore.outputText( '“Oh my,” she purrs, lasciviously. “Aren’t you the impressive one?  Keep surprising me like that and I might just forget about this handicap...”' );
		this.cleanupTitsmother();
		this.HP -= (15 + Utils.rand( CoC.getInstance().player.str ));
		Combat.combatRoundOver();
	};
	...
	Izumi.prototype.titSmotherWait = function() {
		...
		EngineCore.dynStats( 'lus', CoC.getInstance().player.lib / 10 + 5 + Utils.rand( 5 ) );
		if( CoC.getInstance().flags[ kFLAGS.PC_FETISH ] >= 2 ) {
			EngineCore.outputText( ' and to be honest, her grip isn\'t an entirely unplesent experience, either.  If only Izumi would stop playing around and just <i>take you</i> already.' );
			EngineCore.dynStats( 'lus', 5 );
		} else {
			EngineCore.outputText( '.' );
		}
