...
angular.module( 'cocjs' ).factory( 'Ceraph', function( $log, SceneLib, kFLAGS, PerkLib, EventParser, CockTypesEnum, CoC, Monster, Utils, StatusAffects, Appearance, AppearanceDefs, Combat, EngineCore ) {
	...
	Ceraph.prototype.ceraphSpecial1 = function() {
		...
		if( this.findStatusAffect( StatusAffects.Uber ) < 0 ) {
			...
			//FIRE!;
				...
				//(Avoid!);
				if( CoC.getInstance().flags[ kFLAGS.IN_COMBAT_USE_PLAYER_WAITED_FLAG ] === 1 ) {
	...
	Ceraph.prototype.ceraphSpecial2 = function() {
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.Bound ) < 0 ) {
			...
			CoC.getInstance().player.createStatusAffect( StatusAffects.Bound, 2 + Utils.rand( 5 ), 0, 0, 0 );
		}
		//[SPECIAL WHILE PC RESTRAINED];
		else {
			if( Utils.rand( 2 ) === 0 ) {
				EngineCore.outputText( 'Ceraph cuddles up against you, embracing you tenderly.  Her more-than-ample bosom crushes against your flank, and her demonic prick grinds and rubs against your ' + CoC.getInstance().player.skinDesc + ', smearing it with her juices.  Her hands slide over your bound form, sneaking underneath your ' + CoC.getInstance().player.armorName + ' to caress you more intimately while you\'re at her mercy.', false );
				EngineCore.dynStats( 'lus', 9 + CoC.getInstance().player.sens / 10 );
			}
			//[SPECIAL 2 WHILE PC RESTRAINED];
				...
				EngineCore.dynStats( 'lus', 5 + CoC.getInstance().player.cor / 10 + CoC.getInstance().player.lib / 20 );
			}
		}
		Combat.combatRoundOver();
	};
	...
	Ceraph.prototype.ceraphBindingStruggle = function() {
		...
		if( CoC.getInstance().player.statusAffectv1( StatusAffects.Bound ) - 1 <= 0 ) {
			...
			CoC.getInstance().player.removeStatusAffect( StatusAffects.Bound );
			Combat.combatRoundOver();
			return;
		} else {
			...
			CoC.getInstance().player.addStatusValue( StatusAffects.Bound, 1, -1 );
			//Strong characters break free faster;
			if( CoC.getInstance().player.str > 65 && Utils.rand( CoC.getInstance().player.str ) > 45 ) {
	...
	Ceraph.prototype.ceraphBoundWait = function() {
		EngineCore.outputText( '', true );
		EngineCore.outputText( 'Why bother resisting?  The feeling of the leather wrapped tightly around you, digging into your ' + CoC.getInstance().player.skinDesc + ', is intoxicating.', false );
		if( CoC.getInstance().flags[ kFLAGS.PC_FETISH ] >= 2 ) {
		...
		EngineCore.dynStats( 'lus', CoC.getInstance().player.lib / 20 + 5 + Utils.rand( 5 ) );
		EngineCore.outputText( '\n\n', false );
		this.doAI();
	};
	...
	Ceraph.prototype.ceraphSpecial3 = function() {
		...
		} else if( CoC.getInstance().player.findPerk( PerkLib.Misdirection ) >= 0 && Utils.rand( 100 ) < 15 && CoC.getInstance().player.armorName === 'red, high-society bodysuit' ) {
