...
angular.module( 'cocjs' ).factory( 'Ceraph', function( MainView, $log, SceneLib, kFLAGS, PerkLib, CockTypesEnum, CoC, Monster, Utils, StatusAffects, Appearance, AppearanceDefs, Combat, EngineCore ) {
	...
	Ceraph.prototype.ceraphSpecial1 = function() {
		...
		if( this.findStatusAffect( StatusAffects.Uber ) < 0 ) {
			if( Utils.rand( 2 ) === 0 ) {
				...
				MainView.outputText( 'Ceraph titters, "<i>Let me show you the true power of an Omnibus.</i>"\n\n', false );
			}
			MainView.outputText( 'Despite her sultry tease, you can tell she\'s starting to build up to something big...', false );
			...
			if( this.statusAffectv1( StatusAffects.Uber ) === 0 ) {
				...
				if( Utils.rand( 2 ) === 0 ) {
					MainView.outputText( 'The demonic hermaphrodite begins forging demonic symbols in the air before her, each glowing brilliant pink before they blur away in a haze.', false );
				} else {
					MainView.outputText( 'The demonette makes obscene motions with her hands, as if masturbating an imaginary cock or vagina while her hands are wreathed in pink flames.', false );
				}
				MainView.outputText( '  <b>She\'s about to unleash something huge!</b>', false );
				if( CoC.player.inte > 50 ) {
					MainView.outputText( '  You should probably wait so you\'ll have a chance to avoid whatever\'s coming.', false );
				}
			}
			//FIRE!;
			else {
	...
	Ceraph.prototype.ceraphSpecial2 = function() {
		if( CoC.player.findStatusAffect( StatusAffects.Bound ) < 0 ) {
			...
			if( CoC.flags[ kFLAGS.PC_FETISH ] >= 2 ) {
				MainView.outputText( '  Gods this turns you on!', false );
	...
	Ceraph.prototype.ceraphBindingStruggle = function() {
		...
		MainView.outputText( 'You wriggle in the tight binding, trying your best to escape.  ', false );
		if( CoC.player.statusAffectv1( StatusAffects.Bound ) - 1 <= 0 ) {
			MainView.outputText( 'With a mighty twist and stretch, the whip gives and uncurls from you all at once.  You\'ve regained your freedom', false );
			if( CoC.flags[ kFLAGS.PC_FETISH ] >= 2 ) {
				MainView.outputText( ', though you miss the tight leathery embrace', false );
			}
			...
			if( CoC.flags[ kFLAGS.PC_FETISH ] >= 2 ) {
				MainView.outputText( '  You get nice and hot from being so effectively restrained, maybe you should just accept it?', false );
			}
			...
			if( CoC.player.str > 65 && Utils.rand( CoC.player.str ) > 45 ) {
				MainView.outputText( '  Though you didn\'t break free, it seems like your mighty struggles loosened the whip slightly...', false );
				CoC.player.addStatusValue( StatusAffects.Bound, 1, -1 );
			}
		}
		MainView.outputText( '\n\n', false );
	...
	Ceraph.prototype.ceraphBoundWait = function() {
		MainView.outputText( '', true );
		MainView.outputText( 'Why bother resisting?  The feeling of the leather wrapped tightly around you, digging into your ' + CoC.player.skinDesc + ', is intoxicating.', false );
		if( CoC.flags[ kFLAGS.PC_FETISH ] >= 2 ) {
			MainView.outputText( '  You squirm inside the bindings as you get more and more turned on, hoping that Ceraph will strip away your armor and force you to parade around as her bound, naked pet.', false );
			EngineCore.dynStats( 'lus', 5 );
		}
	...
	Ceraph.prototype.ceraphSpecial3 = function() {
		...
		var damage = 0;
		MainView.outputText( 'The demoness weaves her whip in the air until you can practically hear it slithering like a snake, cutting the air as it weaves back and forth, still magically alight with flames.  In a blink she lashes out twice in quick succession!\n', false );
		//First hit!;
		...
		//Determine damage - str modified by enemy toughness!;
			...
			if( damage <= 0 ) {
				...
				if( Utils.rand( CoC.player.armorDef + CoC.player.tou ) < CoC.player.armorDef ) {
					MainView.outputText( 'Your ' + CoC.player.armorName + ' absorb and deflect every ' + this.weaponVerb + ' from ' + this.a + this.short + '.', false );
				} else {
			...
			if( damage > 20 ) {
				MainView.outputText( this.getCapitalA() + this.short + ' <b>mutilates</b> you with ' + this.pronoun3 + ' powerful ' + this.weaponVerb + '! (' + damage + ')', false );
			}
		}
