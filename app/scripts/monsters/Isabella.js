...
angular.module( 'cocjs' ).factory( 'Isabella', function( SceneLib, PerkLib, CoC, Monster, Utils, StatusAffects, Appearance, AppearanceDefs, Combat, EngineCore ) {
	...
	Isabella.prototype.isabellaAttack = function() {
		...
		//Determine if cat'ed;
			...
			var damage;
			damage = Math.round( (this.weaponAttack + this.str + 20) - Utils.rand( CoC.getInstance().player.tou + CoC.getInstance().player.armorDef ) );
			if( damage < 0 ) {
	...
	Isabella.prototype.isabellaStun = function() {
		...
		//Determine if cat'ed;
			...
			var damage = 0;
			damage = Math.round( (this.weaponAttack + this.str) - Utils.rand( CoC.getInstance().player.tou + CoC.getInstance().player.armorDef ) );
			if( damage < 0 ) {
				...
				CoC.getInstance().player.createStatusAffect( StatusAffects.IsabellaStunned, 0, 0, 0, 0 );
			}
		}
		Combat.combatRoundOver();
	};
	Isabella.prototype.isabellaThroatPunch = function() {
		...
		//Determine if cat'ed;
			...
			var damage;
			damage = Math.round( this.str - Utils.rand( CoC.getInstance().player.tou + CoC.getInstance().player.armorDef ) );
			if( damage <= 0 ) {
				...
				CoC.getInstance().player.createStatusAffect( StatusAffects.ThroatPunch, 2, 0, 0, 0 );
			}
		}
		Combat.combatRoundOver();
	};
	...
	Isabella.prototype.performCombatAction = function() {
		...
		else if( CoC.getInstance().player.hasSpells() && CoC.getInstance().player.findStatusAffect( StatusAffects.ThroatPunch ) < 0 && Utils.rand( 3 ) === 0 ) {
			this.isabellaThroatPunch();
		}
		//if PC isn't stunned, 1/4 chance of stun;
		else if( CoC.getInstance().player.findStatusAffect( StatusAffects.IsabellaStunned ) < 0 && Utils.rand( 4 ) === 0 ) {
			this.isabellaStun();
		} else {
			this.isabellaAttack();
		}
	...
	Isabella.prototype.defeated = function() {
		CoC.getInstance().isabellaScene.defeatIsabella();
	};
