...
angular.module( 'cocjs' ).factory( 'Hel', function( MainView, SceneLib, $log, kFLAGS, Appearance, ArmorLib, CoC, EngineCore, Monster, Utils, PerkLib, AppearanceDefs, StatusAffects, ChainedDrop, Combat, ConsumableLib) {
	...
	Hel.prototype.helAttack = function() {
		...
		//Determine damage - str modified by enemy toughness!;
			...
			//Take Damage;
				...
				MainView.outputText( 'The salamander lunges at you, sword swinging in a high, savage arc.  You attempt to duck her attack, but she suddenly spins about mid-swing, bringing the sword around on a completely different path.  It bites deep into your flesh, sending you stumbling back. (' + damage + ')', false );
			}
			if( damage > 0 ) {
	...
	Hel.prototype.helAttack2 = function() {
		...
		if( damage <= 0 ) {
			...
			//Due to toughness or amor...;
			if( Utils.rand( CoC.player.armorDef + CoC.player.tou ) < CoC.player.armorDef ) {
				MainView.outputText( 'The salamander\'s tail-swipe harmlessly deflects off your armor!', false );
			} else {
				MainView.outputText( 'The salamander\'s tail-swipe hits you but fails to move or damage you.', false );
			}
		}
		//Take Damage;
		else {
			MainView.outputText( 'The salamander rushes at you, knocking aside your defensive feint and sliding in past your guard.  She lashes out at your feet with her tail, and you can feel the heated wake of the fiery appendage on your ensuing fall toward the now-smouldering grass. (' + damage + ')', false );
		}
		if( damage > 0 ) {
			if( this.lustVuln > 0 && CoC.player.armorName === 'barely-decent bondage straps' ) {
				MainView.outputText( '\n' + this.getCapitalA() + this.short + ' brushes against your exposed skin and jerks back in surprise, coloring slightly from seeing so much of you revealed.', false );
