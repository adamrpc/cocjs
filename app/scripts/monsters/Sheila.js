...
angular.module( 'cocjs' ).factory( 'Sheila', function( SceneLib, MainView, kFLAGS, WeightedDrop, ConsumableLib, ChainedDrop, StatusAffects, PerkLib, CoC, Monster, Utils, AppearanceDefs, Combat, EngineCore ) {
	...
	Sheila.prototype.sheilaFrogPunch = function() {
		...
		//Miss:;
			...
			if( CoC.player.lib > 40 ) {
				MainView.outputText( ', though not before you have a chance to stick your tongue in her cleavage!' );
			} else {
				MainView.outputText( '.' );
			}
			MainView.outputText( '  Blushing pinkly, she crosses her arms over her chest as she resumes her distance.' );
	...
	Sheila.prototype.suspiciousGlint = function() {
		...
		//Miss:;
			...
			MainView.outputText( 'Sheila\'s eyes glint suspiciously as she proclaims her affection for you and begs you to look into them, but you keep your head down and focus on her feet.  You can feel her stare boring holes into you, but eventually she abandons the attempt.' );
		}
		//Hit:;
		else {
			MainView.outputText( 'Sheila\'s eyes glint suspiciously, and you feel your mind slowing down and your body heating up as you meet her lascivious gaze.  Too late you look away, but the damage is done; her fantasies of ' );
			if( !CoC.player.hasCock() ) {
				MainView.outputText( 'burying her drooling tail inside you until it squirts' );
			} else {
				MainView.outputText( 'riding your dick to the hilt' );
			}
	...
	Sheila.prototype.tittyMonsterAttack = function() {
		MainView.outputText( 'Sheila giggles and strokes her ' + SceneLib.sheilaScene.sheilaCup() + ' breasts, trying to entice you.' );
		//results, no new pg;
		//[(sheila corruption < 20; 'miss');
		if( SceneLib.sheilaScene.sheilaCorruption() < 20 ) {
			MainView.outputText( '  But with nothing there for her to work with, it\'s a lot like being teased by a dressmaker\'s mannequin.' );
	...
	Sheila.prototype.splashAttackLookOutShellEvolveIntoGyrados = function() {
		MainView.outputText( 'Sheila waits patiently, staring at you and stroking her dark, spaded tail with its opposite.  A line of the always-oozing oil falls from the slit, pooling in the smooth brown coil; she unwinds it rapidly, flinging the liquid at your face playfully.  ' );
		//results, no new PG;
		//Hit:;
		if( !Combat.combatMiss() && !Combat.combatEvade() && !Combat.combatMisdirect() && !Combat.combatFlexibility() ) {
			MainView.outputText( 'It lands on target, and you\'re forced to close your eyes lest it get in them!' );
	...
	Sheila.prototype.lickEmAndStickEm = function() {
		MainView.outputText( 'Sheila\'s voice gets closer, becoming disarmingly apologetic as you scrub furiously at your face in darkness.  "<i>Oh, my.  I didn\'t mean to get that in your eyes... let me help clean you up, love.</i>"  Your face is gently gripped between her hands and pulled down, then the demon begins passing her tongue over you affectionately, wiping the fluid away with long, ticklish licks as you wait for the other shoe to fall.' );
		MainView.outputText( '"<i>All better,</i>" Sheila announces.  With her thumb, she gingerly pushes one eyelid up before you can pull away, proving her claim - and causing you to look right into her own glowing, purple iris.  A fantasy invades your mind, one where ' );
		if( CoC.player.hasCock() ) {
			MainView.outputText( '[oneCock] fucks Sheila to the base while her tail snakes around and penetrates your [vagOrAss]' );
		} else {
			MainView.outputText( 'you take Sheila from behind by plunging her spaded tail into your [vagina] as she lies face-down on the ground with her ass pushed in the air' );
		}
		MainView.outputText( '.' );
		//results, no new pg;
		//[(int check passed);
