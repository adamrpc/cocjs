...
angular.module( 'cocjs' ).factory( 'Minerva', function( SceneLib, MainView, CoC, kFLAGS, EngineCore, Monster, Utils, AppearanceDefs, StatusAffects, Appearance, Combat, WeightedDrop, ConsumableLib ) {
	function Minerva() {
	...
	Minerva.prototype.minervaBite = function() {
		MainView.outputText( 'The siren paces around you in circles, waiting for the right moment to strike.  Unexpectedly quick thanks to her clawed feet, she propels herself toward you at full speed.  Her maw opens wide to chomp on you, showing off multiple rows of glinting, razor-sharp teeth.' );
		...
		if( Combat.combatMiss() || Combat.combatEvade() || Combat.combatFlexibility() || Combat.combatMisdirect() ) {
			MainView.outputText( '  You get out of the way just in time, this.Minerva making a loud chomping sound as she only catches the air.' );
		}
		//[else block]
		else if( damage <= 0 ) {
			MainView.outputText( '  Your hands lash out, knocking her head to the side as she tries to bite you.  With the bite deflected, this.Minerva makes a loud chomping sound as she only bites the air.' );
		}//[if attack lands]
		else {
			MainView.outputText( '  Her teeth dig right into your arm!  It\'s a bit of a struggle, but you\'re able to free yourself.  The damage doesn\'t look too serious. ' );
	...
	Minerva.prototype.minervaKnowsKungfu = function() {
		MainView.outputText( 'The blue beauty flaps her wings and launches herself into the air.  Once she\'s gained as much altitude as she can, she dive-bombs you, her demon-clawed feet leading the attack.' );
		var damage = Math.ceil( (this.str + this.weaponAttack + 100) - Utils.rand( CoC.player.tou ) - CoC.player.armorDef );
		this.spe -= 70;
		if( Combat.combatMiss() || Combat.combatEvade() || Combat.combatFlexibility() || Combat.combatMisdirect() ) {
			MainView.outputText( '  You jump out of the landing zone just in time, piles of dirt exploding in all directions as Minerva slams into the ground.' );
		}
		//[else block]
		else if( damage <= 0 ) {
			MainView.outputText( '  Steadying yourself, you reach up, grabbing hold of Minerva as she attempts to land a heavy blow on you.  Grunting hard, you pull against her and toss the siren aside completely, halting her attack.' );
		}//[if attack lands]
		else {
			MainView.outputText( '  She hits you square in the chest, knocking you to the ground as her entire weight lands on you.  The bombshell of a woman jumps off your chest, ready to keep fighting.' );
			damage = CoC.player.takeDamage( damage );
			MainView.outputText( ' (' + damage + ')' );
		}
		this.spe += 70;
	...
	Minerva.prototype.tailWhip = function() {
		...
		MainView.outputText( 'She runs at you, holding the weapon like she\'s about to chop into your side.  You brace yourself, but when she\'s only a few feet away, she starts to turn her body.' );
		//[else block]
		if( damage <= 0 ) {
			MainView.outputText( '  Lashing out with a fierce kick you intercept the tail-whip, your [foot] impacting against her strong appendage and totally neutralizing its momentum.' );
		}//[if attack lands]
		else {
			MainView.outputText( '  Her shark tail whacks you, knocking you to the ground.  You quickly struggle back into position' );
			if( CoC.player.armorDef > 0 ) {
				MainView.outputText( ', but your defense has been reduced' );
			}
			MainView.outputText( '!' );
			damage = CoC.player.takeDamage( damage );
			MainView.outputText( ' (' + damage + ')' );
	...
	Minerva.prototype.minervaUsesHalberdStab = function() {
		MainView.outputText( 'Minerva charges at you, brandishing her halberd\'s sharp tip toward you.' );
		var damage = Math.ceil( (this.str + this.weaponAttack) - Utils.rand( CoC.player.tou ) );
		if( Combat.combatMiss() || Combat.combatEvade() || Combat.combatFlexibility() || Combat.combatMisdirect() ) {
			MainView.outputText( '  You sidestep the attack just as she thrusts the point past your face.' );
		}//[else block]
		else if( damage < 0 ) {
			MainView.outputText( '  With all your strength, you swing your [weapon], the blow landing on the side of Minerva\'s halberd and deflecting the goring strike away from you.' );
		}//[if attack lands]
		else {
			MainView.outputText( '  She pierces you right in the shoulder!  You wince in pain and step back, out of her reach again.' );
			damage = CoC.player.takeDamage( damage );
			MainView.outputText( ' (' + damage + ')' );
		}
		SceneLib.combatScene.combatRoundOver();
	...
	Minerva.prototype.minervaUsesHalberdCHOP = function() {
		MainView.outputText( 'She moves in close, practically right in front of you and raises the halberd.' );
		var damage = Math.ceil( (this.str + 100) - Utils.rand( CoC.player.tou ) - CoC.player.armorDef );
		if( Combat.combatMiss() || Combat.combatEvade() || Combat.combatFlexibility() || Combat.combatMisdirect() ) {
			MainView.outputText( '  You get out of the way quickly, her attack chopping deeply into the earth. ' );
		}//[else block]
		else if( damage < 0 ) {
			MainView.outputText( '  In a mad show of pure skill, you lift your hands, clamping them down on the cheeks of the halberd blade and stop Minerva\'s attack cold, bewildering the siren in the process.' );
		}//[if attack lands]
		else {
			MainView.outputText( '  You don\'t have time to avoid the downward chop and the axe head lands right in your shoulder blade!  You cry out in pain, but you can still move your arm despite the brutal blow.' );
			damage = CoC.player.takeDamage( damage );
			MainView.outputText( ' (' + damage + ')' );
		}
		SceneLib.combatScene.combatRoundOver();
	...
	Minerva.prototype.sirensSong = function() {
		...
		//[part 2]
			...
			MainView.outputText( 'Her hum becomes a song.  A magnificent song without words, a sound that should be impossible for any human, or creature for that matter, to make naturally.' );
			//If wait:
			if( CoC.flags[ kFLAGS.IN_COMBAT_USE_PLAYER_WAITED_FLAG ] === 1 ) {
				MainView.outputText( '  You cover your ears before she even opens her lips, wary of its power.  Judging by the dim feeling of pleasure simmering through you with the little sound you\'re picking up regardless, it probably was for the better.' );
			}//No wait - insta loss:
			else {
				MainView.outputText( '  Your mind clouds over as the song flows through your ears and fills your mind with sweet bliss.  You lower your [weapon] and dreamily walk into the siren\'s sweet embrace.  You absent-mindedly disrobe yourself as you move in closer, the song getting louder with each step you take, until you finally bury yourself into the siren\'s soft bosom and she wraps her feathery arms around your body.  She stops singing her beautiful song and whispers into your ear, "<i>You\'re all mine now.</i>"' );
				CoC.player.lust = 100;
			}
