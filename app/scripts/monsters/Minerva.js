...
angular.module( 'cocjs' ).factory( 'Minerva', function( $log, CoC, kFLAGS, EngineCore, Monster, Utils, AppearanceDefs, StatusAffects, Appearance, Combat, WeightedDrop, ConsumableLib ) {
	...
	Minerva.prototype.minervaBite = function() {
		...
		var damage = Math.ceil( (this.str + 85) - Utils.rand( CoC.getInstance().player.tou ) - CoC.getInstance().player.armorDef );
	...
	Minerva.prototype.tailWhip = function() {
		var damage = Math.ceil( (this.str + 35) - Utils.rand( CoC.getInstance().player.tou ) - CoC.getInstance().player.armorDef );
	...
	Minerva.prototype.minervaUsesHalberdCHOP = function() {
		...
		var damage = Math.ceil( (this.str + 100) - Utils.rand( CoC.getInstance().player.tou ) - CoC.getInstance().player.armorDef );
		if( Combat.combatMiss() || Combat.combatEvade() || Combat.combatFlexibility() || Combat.combatMisdirect() ) {
	...
	Minerva.prototype.bootyShortInYoFaceSon = function() {
		...
		EngineCore.dynStats( 'lus', 20 + CoC.getInstance().player.lib / 10 + Utils.rand( 5 ) );
		Combat.combatRoundOver();
	};
