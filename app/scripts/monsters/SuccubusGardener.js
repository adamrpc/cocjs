...
angular.module( 'cocjs' ).factory( 'SuccubusGardener', function( MainView, Appearance, AppearanceDefs, kFLAGS, EngineCore, CoC, Monster, StatusAffects, Utils, Combat ) {
	...
	SuccubusGardener.prototype.cleanupEffects = function() {
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.Tentagrappled ) ) {
			CoC.getInstance().player.removeStatusAffect( StatusAffects.Tentagrappled );
		}
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.ShowerDotEffect ) ) {
			CoC.getInstance().player.removeStatusAffect( StatusAffects.ShowerDotEffect );
		}
	};
	SuccubusGardener.prototype.defeated = function( hpVictory ) {
		this.cleanupEffects();
		CoC.getInstance().succubusGardenerScenes.fuckUpTheGardener( hpVictory );
	};
	SuccubusGardener.prototype.won = function( hpVictory ) {
		this.cleanupEffects();
		CoC.getInstance().succubusGardenerScenes.surrenderToTheGardener( hpVictory );
	};
	SuccubusGardener.prototype.performCombatAction = function() {
		...
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.ShowerDotEffect ) >= 0 ) {
		...
		if( this.HPRatio() <= 0.6 ) {
			...
			this.tentagrapple();
		} else if( CoC.getInstance().player.findStatusAffect( StatusAffects.GardenerSapSpeed ) < 0 && this.findStatusAffect( StatusAffects.VineHealUsed ) >= 0 ) {
			this.sapSpeed();
		} else {
	...
	SuccubusGardener.prototype.tentagrapple = function() {
		...
		if( Combat.combatMiss() || Combat.combatEvade() || Combat.combatFlexibility() || Combat.combatMisdirect() ) {
			...
			CoC.getInstance().player.createStatusAffect( StatusAffects.Tentagrappled, 0, 0, 0, 0 );
		}
	};
	SuccubusGardener.prototype.grappleStruggle = function() {
		EngineCore.clearOutput();
		var numRounds = CoC.getInstance().player.statusAffectv1( StatusAffects.Tentagrappled );
		if( Utils.rand( CoC.getInstance().player.str ) > this.str / (1 + (numRounds / 2)) ) {
			...
			CoC.getInstance().player.addStatusValue( StatusAffects.Tentagrappled, 1, 1 );
	...
	SuccubusGardener.prototype.sicem = function() {
		...
		for( var i = 0; i < 10; i++ ) {
			if( !(Combat.combatMiss() || Combat.combatEvade() || Combat.combatFlexibility() || Combat.combatMisdirect()) ) {
				damage += 2 + Utils.rand( 1 + CoC.getInstance().player.lib / 20 ) + Utils.rand( 1 + CoC.getInstance().player.sens / 20 );
			}
		}
		if( damage >= 0 ) {
			var sL = CoC.getInstance().player.lust;
			EngineCore.dynStats( 'lus+', damage );
			sL = Math.round( CoC.getInstance().player.lust - sL );
	...
	SuccubusGardener.prototype.corruptiveShower = function() {
		...
		if( Combat.combatMiss() || Combat.combatEvade() || Combat.combatFlexibility() || Combat.combatMisdirect() ) {
			...
			//Fail;
			if( CoC.getInstance().player.findStatusAffect( StatusAffects.ShowerDotEffect ) < 0 ) {
				CoC.getInstance().player.createStatusAffect( StatusAffects.ShowerDotEffect, 3, 0, 0, 0 );
			} else {
				CoC.getInstance().player.addStatusValue( StatusAffects.ShowerDotEffect, 1, 3 );
			}
	...
	SuccubusGardener.prototype.showerDotEffect = function() {
		...
		CoC.getInstance().player.addStatusValue( StatusAffects.ShowerDotEffect, 1, -1 );
		//Dot effect;
		if( CoC.getInstance().player.lust < 50 ) {
			...
			EngineCore.outputText( 'You whimper as the insidious plant-sperm works on your vulnerable ' + CoC.getInstance().player.skinDesc + ', building pernicious desires in tiny, insistent increments. It\'s getting harder to focus... harder not to think about how good all those tentacles would feel in you and on you, caressing your most intimate places.\n\n' );
			...
			if( CoC.getInstance().player.hasCock() ) {
				EngineCore.outputText( ', even while your rigid cock' + ((CoC.getInstance().player.cocks.length > 1) ? 's are' : ' is') + ' drizzling ropes of pre unimpeded.' );
			} else if( CoC.getInstance().player.hasVagina() ) {
				EngineCore.outputText( ' soaked twat threatens to ' + ((CoC.getInstance().player.wetness() === 4) ? 'drown' : 'further soak') + ' your [legs].' );
			} else {
		...
		if( CoC.getInstance().player.statusAffectv1( StatusAffects.ShowerDotEffect ) < 0 ) {
			CoC.getInstance().player.removeStatusAffect( StatusAffects.ShowerDotEffect );
	...
	SuccubusGardener.prototype.motorboat = function() {
		...
		if( CoC.getInstance().player.tallness <= this.tallness - 6 ) {
			EngineCore.outputText( ', somehow short enough in spite of the height differences.' );
		} else if( CoC.getInstance().player.tallness >= this.tallness + 6 ) {
	...
	SuccubusGardener.prototype.sapSpeed = function() {
		...
		// 20%?;
		var speedSapped = CoC.getInstance().player.spe * 0.2;
		CoC.getInstance().player.spe -= speedSapped;
		CoC.getInstance().player.createStatusAffect( StatusAffects.GardenerSapSpeed, speedSapped, 0, 0, 0 );
	...
	SuccubusGardener.prototype.tasteTheEcstasy = function() {
		...
		//Fail strength check!;
		if( Utils.rand( CoC.getInstance().player.str - 30 ) + 30 > this.str ) {
			...
			EngineCore.dynStats( 'lus', (8 + Math.ceil( CoC.getInstance().player.lib / 20 + CoC.getInstance().player.cor / 25 )), 'cor+', 5 );
		} else {
