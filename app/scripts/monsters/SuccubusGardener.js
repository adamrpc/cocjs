...
angular.module( 'cocjs' ).factory( 'SuccubusGardener', function( MainView, Appearance, AppearanceDefs, kFLAGS, EngineCore, CoC, Monster, StatusAffects, Utils, Combat ) {
	...
	SuccubusGardener.prototype.vineHeal = function() {
		...
		if( this.findStatusAffect( StatusAffects.Stunned ) < 0 ) {
			...
			if( this.lustVuln <= 0.15 ) {
				...
				MainView.outputText( ' <b>She\'s definitely leering at you now.</b>' );
			} else {
				MainView.outputText( ' <b>There\'s no disguising the lecherous look on her face while she eye-humps you.</b>' );
			}
			if( this.lustVuln <= 0.3 ) {
				MainView.outputText( ' Whatever is in that healing nectar must be weakening her self-control.' );
			}
		} else {
			MainView.outputText( 'Acting on some unseen signal, the forest of tentacles lurches into action, surrounding its insensate mistress in a veil of wiggling green. A bundle of slime-dripping vines can be spotted through gaps in the verdant veil, hanging over their demonic caretaker with remarkable care, leaking gooey strands of pink into her mouth. Her throat works to swallow it, and when she emerges a second later, her injuries have vanished and her eyes' );
			if( this.lustVuln <= 0.15 ) {
				MainView.outputText( ' are slightly glassy.' );
			} else if( this.lustVuln <= 0.45 ) {
				MainView.outputText( ' are even glassier.' );
			} else {
				MainView.outputText( ' are dilated and a little slow.' );
			}
			if( this.lustVuln <= 0.3 ) {
				MainView.outputText( ' Whatever is in that healing nectar must be weakening her self-control.' );
			}
	...
	SuccubusGardener.prototype.tentagrapple = function() {
		...
		MainView.outputText( 'A web of interwoven vines lashes out from behind the succubus, somehow leaving her untouched by the wave of advancing greenery. They\'re trying to grab you!' );
		if( Combat.combatMiss() || Combat.combatEvade() || Combat.combatFlexibility() || Combat.combatMisdirect() ) {
			//Dodge;
			MainView.outputText( ' You slip aside at the last moment, barely avoiding being wrapped in the squirming mass. It snaps back, perhaps at the limits of its reach, leaving you once more eye to eye with the alluring gardener.' );
		} else {
			//Do not dodddddddggggeee;
			MainView.outputText( ' You twist to the side, but one snags you by the wrist. Another loops around your [leg], and an avalanche of slime-oozing tentacles falls across the rest of you, wrapping you up in snug coils. Their grip is equal parts iron grip and lover\'s caress. You\'d better struggle free before they really start to work on you!' );
	...
	SuccubusGardener.prototype.squeeze = function() {
		if( Utils.rand( 2 ) === 0 ) {
			MainView.outputText( 'The vines encircling you squeeze you tighter, twisting and sliding across your [skinFurScales] as they press more tightly around you. It gets hard to breathe, but at the same time, some of them are inside your [armor], gliding across your most sensitive places with oiled ease that\'s made all the more arousing by the force behind it.' );
		} else {
			MainView.outputText( 'You\'re intimately aware of the vegetative masses pressing down on you from every angle, lavishing you with attentions so forceful that they threaten to squeeze the very breathe from your body. It\'s impossible to ignore. You do your best to breathe and ignore the undulated affections, but even you can\'t deny the way that it makes your heart beat faster.' );
		}
		CoC.player.addStatusValue( StatusAffects.Tentagrappled, 1, 1 );
	...
	SuccubusGardener.prototype.sicem = function() {
		...
		if( damage >= 0 ) {
			...
			MainView.outputText( ' The sinuous plant-based tentacles lash at you like a dozen tiny whips! Preparing for stinging pain, you\'re somewhat taken aback when they pull back at the last moment, sensually caressing your most sensitive places! (' + sL + ')' );
		} else {
			MainView.outputText( ' The sinuous plant-based tentacles strike at you like a dozen tiny whips, but, somehow, you manage to avoid every single one of their lashes!' );
		}
	};
	SuccubusGardener.prototype.corruptiveShower = function() {
		MainView.outputText( 'The succubus lifts her hands up in the air, saying, <i>“Why not taste a sampling of the pleasures I offer?”</i> Above her, a canopy of corrupt, snarled greenery forms, oozing unmistakable sexual fluids - both male and female. Splatters of jism and pussy juice fall like curtains of corruptive rain, their scent lacing the air with their heady musk.' );
		if( Combat.combatMiss() || Combat.combatEvade() || Combat.combatFlexibility() || Combat.combatMisdirect() ) {
			//Dodge;
	...
	SuccubusGardener.prototype.showerDotEffect = function() {
		...
		if( CoC.player.lust < 50 ) {
			MainView.outputText( 'The tentacles\' sex-juices are still covering you - still slowly arousing you. You\'ve got a good handle on it for now.\n\n' );
		} else if( CoC.player.lust < 60 ) {
			MainView.outputText( 'You try to wipe some of the fragrant seed from your palm, but all you succeed in doing is smearing it into your [hips].' );
			if( CoC.player.cor < 50 ) {
				MainView.outputText( ' You\'re ashamed to admit' );
			} else {
				MainView.outputText( ' You\'re a little irritated to admit' );
			}
			MainView.outputText( ' that it\'s starting to feel really good.\n\n' );
		} else if( CoC.player.lust < 70 ) {
			MainView.outputText( 'You groan at the warm slipperiness enveloping your [skinFurScales] as the tainted tentacles\' fluids go to work on you. There\'s nothing you can do but try to endure it. If only it didn\'t feel so... hot to be drenched in. If you wind up losing, you hope she\'ll do this again....\n\n' );
		} else if( CoC.player.lust < 80 ) {
			MainView.outputText( 'You whimper as the insidious plant-sperm works on your vulnerable ' + CoC.player.skinDesc + ', building pernicious desires in tiny, insistent increments. It\'s getting harder to focus... harder not to think about how good all those tentacles would feel in you and on you, caressing your most intimate places.\n\n' );
		} else if( CoC.player.lust < 90 ) {
			MainView.outputText( 'You shudder in place, stumbling dazedly as your ardor rises to a fever pitch. Soon, you\'re going to wind up too turned-on to resist, and when that happens, those tentacles are going to take you. The worst part? It\'s starting to sound really, really... really good to you. Not struggling, no tension... just giving in to what your body craves and loving it.\n\n' );
		} else if( CoC.player.lust < 100 ) {
			MainView.outputText( 'Ohhhh, you\'re close now. You can feel the need hammering inside of you, soaking in through your [skinFurScales] to stoke the fires between your [legs] into a blazing inferno, one you couldn\'t resist even if you wanted to. Then... then you\'ll be free to cum. You shake your head. Gotta hold it together' );
			if( CoC.player.hasCock() ) {
				MainView.outputText( ', even while your rigid cock' + ((CoC.player.cocks.length > 1) ? 's are' : ' is') + ' drizzling ropes of pre unimpeded.' );
			} else if( CoC.player.hasVagina() ) {
				MainView.outputText( ' soaked twat threatens to ' + ((CoC.player.wetness() === 4) ? 'drown' : 'further soak') + ' your [legs].' );
			} else {
				MainView.outputText( '.' );
			}
		} else {
		...
		if( CoC.player.statusAffectv1( StatusAffects.ShowerDotEffect ) < 0 ) {
			CoC.player.removeStatusAffect( StatusAffects.ShowerDotEffect );
			MainView.outputText( '<b>The lust-inducing effects of the plant-spunk feel like they\'ve dissipated...</b>\n\n' );
		}
	};
	SuccubusGardener.prototype.taunt = function() {
		MainView.outputText( '<i>“How do you expect to defeat me, [name],”</i> the green-thumbed temptress asks with a quirk of her head. <i>“You are but one, and we are many. You have the frailties and weaknesses of a soul. I have power and experience beyond your comprehension. What is there for you to do but willingly submit?”</i> She purses her puffy lips, thinking. <i>“If you submit willingly, I\'ll allow you to lay your head between my breasts while my plants feed on you. It\'ll be quite the experience.”</i>' );
	};
	SuccubusGardener.prototype.motorboat = function() {
		MainView.outputText( '<i>“Oh fuck it,”</i> the demoness growls, stalking forward. <i>“We both need this, don\'t we, pet?”</i> She slips inside your guard, pressing her pendulous melons against your face' );
		if( CoC.player.tallness <= this.tallness - 6 ) {
			MainView.outputText( ', somehow short enough in spite of the height differences.' );
		} else if( CoC.player.tallness >= this.tallness + 6 ) {
			MainView.outputText( ', somehow tall enough in spite of the height differences.' );
		}

		MainView.outputText( ' They\'re so soft and pillowy that you can\'t help but enjoy the feel of them on your skin, and you take a deep, contented breath before remembering where you are and struggling out of the creamy valley.' );
