...
angular.module( 'cocjs' ).factory( 'AbstractSpiderMorph', function( MainView, kFLAGS, WeaponLib, CoC, EngineCore, Monster, Utils, StatusAffects, Combat, PerkLib ) {
	...
	AbstractSpiderMorph.prototype.getBitten = function() {
		...
		if( CoC.player.spe < 2 && CoC.player.findStatusAffect( StatusAffects.Web ) >= 0 ) {
			...
			if( CoC.player.hasCock() ) {
				EngineCore.outputText( CoC.player.SMultiCockDesc() + ' turns rock hard and squirts weakly, suddenly so aroused that it starts soaking your ' + CoC.player.armorName, false );
				if( CoC.player.hasVagina() ) {
					EngineCore.outputText( ' along with your ' + CoC.player.vaginaDescript(), false );
				}
				EngineCore.outputText( '.  ', false );
			} else if( CoC.player.hasVagina() ) {
				EngineCore.outputText( 'Your ' + CoC.player.vaginaDescript() + ' grows wet as hell and so sensitive that every step and movement reminds you of the powerful need for something between your sopping nether-lips.  ', false );
			}
			EngineCore.outputText( 'While ' + this.mf( 'his', 'her' ) + ' venom pours into you, the spider-' + this.mf( 'boy', 'girl' ) + ' reaches into your gear to play with your ' + CoC.player.nippleDescript( 0 ) + ', and you moan like a whore from the dual stimulation of ' + this.mf( 'his', 'her' ) + ' venom and nipple-play.\n\n', false );
			if( this.hasVagina() ) {
	...
	AbstractSpiderMorph.prototype.spiderDisarm = function() {
		EngineCore.outputText( this.getCapitalA() + this.short + ' shifts and sprays webbing, aiming a tight strand of it at your ' + CoC.player.weaponName + '.  ', false );
		//Blind dodge change;
		if( this.findStatusAffect( StatusAffects.Blind ) >= 0 && Utils.rand( 3 ) < 2 ) {
		...
		//Determine if cat'ed;
			...
			EngineCore.outputText( 'You don\'t react fast enough and the sticky webbing pulls your ' + CoC.player.weaponName + ' out of your grip, gluing it to a nearby tree.  There\'s no way to get it back right now, you\'ll have to fight bare-handed!', false );
