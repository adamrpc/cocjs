...
angular.module( 'cocjs' ).factory( 'MinotaurMob', function( SceneLib, CoC, kFLAGS, EngineCore, Monster, Utils, AppearanceDefs, StatusAffects, Combat, CockTypesEnum, Descriptors ) {
	...
	MinotaurMob.prototype.precumTease = function() {
		var teased = false;
		var damage = 0;
		var oldLust = CoC.getInstance().player.lust;
		...
		if( Utils.rand( 2 ) === 0 ) {
			...
			if( Utils.rand( 5 ) > 0 ) {
				...
				damage = 7 + CoC.getInstance().player.lib / 20;
			}
			//crit)
			else {
				...
				damage = 13 + CoC.getInstance().player.lib / 20;
			}
		}
		...
		if( Utils.rand( 2 ) === 0 ) {
			...
			if( Utils.rand( 5 ) > 0 ) {
				...
				damage = 3 + CoC.getInstance().player.lib / 30;
			} else {
				...
				if( CoC.getInstance().player.spe / 5 + Utils.rand( 20 ) > 20 ) {
					...
					damage = 6 + CoC.getInstance().player.lib / 20;
				} else {
					...
					damage = 15 + CoC.getInstance().player.lib / 20;
				}
			}
		...
		if( !teased || Utils.rand( 3 ) === 0 ) {
			...
			damage = 9 + CoC.getInstance().player.lib / 20;
		}
		EngineCore.dynStats( 'lus', damage );
		damage = CoC.getInstance().player.lust - oldLust;
		...
		//(Medium damage taken)
			...
			EngineCore.outputText( 'The powerful pheromones and scents hanging in the air around you make your body flush hotly.  Your ' + CoC.getInstance().player.nippleDescript( 0 ) + 's grow harder', false );
			if( CoC.getInstance().player.lust > 70 ) {
				EngineCore.outputText( ', though you didn\'t think such a thing was possible', false );
			} else {
				EngineCore.outputText( ', feeling like two bullets scraping along the inside of your ' + CoC.getInstance().player.armorName, false );
			}
	...
	MinotaurMob.prototype.init = function( that, args ) {
		...
		that.long = Utils.Num2Text( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00326 ] ) + ' shaggy beastmen stand around you in a loose circle.  Their postures aren\'t exactly threatening.  If anything, they seem to be standing protectively around you, as if their presence would somehow shelter you from the rest of the mountain.  All of their features share a brotherly similarity, though there\'s still a fair bit of differences between your minotaur sons.  One of them is a head above the rest, a massive hulk of muscle so big he seems to dwarf the rest.  In stark contrast, a feminine minitaur keeps his distance in the rear.' + (CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00326 ] >= 20 ? '  The tribe constantly makes hoots and cat-calls, fully expecting to be fucking you soon.' : '');
		that.plural = true;
		that.pronoun1 = 'they';
		that.pronoun2 = 'them';
		that.pronoun3 = 'their';
		...
		var bonusHP = 340 + 50 * (CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00326 ] - 3);
		var lustVuln = 0.45;
		if( (CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00326 ] - 3) * 2 > 13 ) {
			lustVuln = 0.3;
		} else {
			lustVuln -= (CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00326 ] - 3) * 0.02;
		}
		that.bonusHP = bonusHP;
		that.lust = 30;
		that.lustVuln = lustVuln;
		that.temperment = Monster.TEMPERMENT_LUSTY_GRAPPLES;
		var level = 11 + Math.round( (CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00326 ] - 3) / 2 );
		if( level > 14 ) {
			level = 14;
		}
		that.level = level;
