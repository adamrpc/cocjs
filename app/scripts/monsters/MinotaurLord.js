...
angular.module( 'cocjs' ).factory( 'MinotaurLord', function( SceneLib, MainView, CoC, WeaponLib, Combat, EngineCore, CockTypesEnum, Utils, AppearanceDefs, Monster, StatusAffects ) {
	...
	MinotaurLord.prototype.minotaurDrankMalk = function() {
		MainView.outputText( 'The minotaur lord snorts audibly and turns to look at his mistress.  "<i>What is it, Fido, boy?  You thirsty?</i>"  The hulking minotaur nods.' );
		//Success:*;
		...
		//Failure:*;
			...
			MainView.outputText( '"<i>Well too bad!  We\'re all out of milk... but don\'t worry, my dear pet, I\'ll let you drink as much as you want after you\'re done with this bitch.</i>"  The succubus replies, idly checking her elongated nails.' );
	...
	MinotaurLord.prototype.minotaurLordEntangle = function() {
		...
		if( Combat.combatMiss() || Combat.combatEvade() || Combat.combatFlexibility() || Combat.combatMisdirect() ) {
			MainView.outputText( 'You leap over the clumsy swing, allowing the chain to fly harmlessly underneath you!' );
		} else {
			MainView.outputText( 'You try to avoid it, but you\'re too slow, and the chain slaps into your hip, painfully bruising you with the strength of the blow, even through your armor.  The inertia carries the back half of the whip around you, and in a second, the chain has you all wrapped up with your arms pinned to your sides and your movement restricted.' );
	...
	MinotaurLord.prototype.minotaurCumPress = function() {
		...
		MainView.outputText( 'You tumble to your knees a few feet away, compulsively licking it up.  Once it\'s gone, ' );
		if( CoC.player.lust > 99 ) {
			MainView.outputText( 'you rise up, horny and hungry for more.' );
		} else {
			MainView.outputText( 'you realize what you\'ve been doing.  Your embarrassment gives you the strength to re-adopt your fighting pose, but it\'s hard with how rigid' );
			if( CoC.player.lust >= 80 ) {
				MainView.outputText( ' and drippy' );
			}
	...
	MinotaurLord.prototype.minotaurPrecumTease = function() {
		...
		if( Utils.rand( 2 ) === 0 ) {
			MainView.outputText( ' slapping into your face before you can react!  You wipe the slick snot-like stuff out of your eyes and nose, ' );
			if( CoC.player.lust >= 70 ) {
				MainView.outputText( 'swallowing it into your mouth without thinking.  You greedily guzzle the potent, narcotic aphrodisiac down, even going so far as to lick it from each of your fingers in turn, sucking every drop into your waiting gullet.' );
			} else {
				MainView.outputText( 'feeling your heart hammer lustily.' );
			}
			EngineCore.dynStats( 'lus', 15 + CoC.player.lib / 8 + CoC.player.sens / 8 );
		} else {
			MainView.outputText( ' right past your head, but the smell alone is enough to make you weak at the knees.' );
			MainView.outputText( '  The animalistic scent of it seems to get inside you, the musky aroma burning a path of liquid heat to your groin, stiffening your horse-cock to absurd degrees.' );
			EngineCore.dynStats( 'lus', 11 + CoC.player.lib / 10 );
		}
		//(1);
		if( CoC.player.lust <= 75 ) {
			MainView.outputText( '  You shiver with need, wanting nothing more than to bury your face under that loincloth and slurp out every drop of goopey goodness.' );
		} else {
