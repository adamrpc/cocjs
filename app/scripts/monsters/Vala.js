...
angular.module( 'cocjs' ).factory( 'Vala', function( SceneLib, MainView, ConsumableLib, CoC, Appearance, kFLAGS, Combat, EngineCore, Utils, WeightedDrop, AppearanceDefs, Monster, StatusAffects ) {
	function Vala() {
	...
	Vala.prototype.valaSpecial1 = function() {
		MainView.outputText( 'Vala dabs at one of her wounds and swoons.  Is she actually getting off from the wounds?  Damn she\'s damaged!  Vala licks the blood from her fingers, winks, and blows pink mist from her mouth.', false );
		//Lightly wounded.;
		if( this.HPRatio() > 0.7 ) {
			MainView.outputText( '  The sweet-smelling cloud rapidly fills the room, but the volume of mist is low enough that you don\'t end up breathing in that much of it.  It does make your pulse quicken in the most pleasant way though...', false );
			EngineCore.dynStats( 'lus', 5 + CoC.player.lib / 20 );
		} else if( this.HPRatio() > 0.4 ) {
			MainView.outputText( '  The rose-colored vapor spreads throughout the room, forcing you to breathe it in or pass out from lack of air.  It smells sweet and makes your head swim with sensual promises and your crotch tingle with desire.  Panicked by the knowledge that you\'re being drugged, you gasp, but it only draws more of the rapidly disappating cloud into your lungs, fueling your lust.', false );
	...
	Vala.prototype.valaCombatDialogue = function() {
		if( this.findStatusAffect( StatusAffects.Vala ) < 0 ) {
			...
			if( this.statusAffectv1( StatusAffects.Vala ) === 1 ) {
				MainView.outputText( '"<i>If you won\'t fuck Bitch, you must not be a master,</i>" she realizes, the fight invigorating her lust-deadened brain. "<i>You get to be a pet for the masters, too!</i>"', false );
			} else if( this.statusAffectv1( StatusAffects.Vala ) === 2 ) {
				MainView.outputText( '"<i>If the masters like you, maybe they will let Bitch keep you for herself! Won\'t you like that?</i>"', false );
			} else if( this.statusAffectv1( StatusAffects.Vala ) === 3 ) {
				MainView.outputText( '"<i>We obey the masters. They fed Bitch until she became big enough to please them. The masters love their pets so much, you\'ll see.</i>"', false );
			} else if( this.statusAffectv1( StatusAffects.Vala ) === 4 ) {
				MainView.outputText( '"<i>Thoughts are so hard. Much easier to be a toy slut. Won\'t you like being a toy? All that nasty memory fucked out of your head.</i>"', false );
			} else if( this.statusAffectv1( StatusAffects.Vala ) === 5 ) {
				MainView.outputText( '"<i>Bitch has given birth to many of the masters\' children. She will teach you to please the masters. Maybe you can birth more masters for us to fuck?</i>"', false );
			} else {
				MainView.outputText( '"<i>Bitch loves when her children use her as their fathers did. Sluts belong to them. Slut love them. You will love them too!</i>"', false );
			}
		}
	};
