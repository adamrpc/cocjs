...
angular.module( 'cocjs' ).factory( 'FetishZealot', function( $log, MainView, SceneLib, CoC, EngineCore, Monster, Utils, AppearanceDefs, StatusAffects, Combat, WeightedDrop, ConsumableLib, WeaponLib, ArmorLib ) {
	...
	FetishZealot.prototype.combatRoundUpdate = function() {
		...
		//Talk abouts it mang!
		if( changed ) {
			MainView.outputText( 'The fetish zealot\'s clothing shifts and twists, until he is wearing ' + this.armorName + '.\n\n', false );
		}
	...
	FetishZealot.prototype.zealotSpecial1 = function() {
		...
		if( this.armorName === RELIGIOUS_CLOTHES ) {
			...
			MainView.outputText( 'The zealot cries out "<i>Child, are you ready to present your offering to the holy rod?</i>" while indicating his cock sliding between his robes.  The whole scene leaves you distracted for a few moments and significantly aroused.', false );
		}
		//A pirate costume; 
		if( this.armorName === PIRATE_CLOTHES ) {
			...
			MainView.outputText( 'The zealot turns to the side holding his prosthetic towards you and doing something that sends the devices spinning and clicking.  <i>So that\'s how that would work...<i> you find yourself thinking for a few moments before realizing that he had both distracted and aroused you.', false );
		}
		//Military attire;
		if( this.armorName === MILITARY_CLOTHES ) {
			...
			MainView.outputText( 'He suddenly barks, "<i>Let\'s see those genitals, soldier!</i>" ', false );
			//[CoC.player is corrupt] 
			if( CoC.player.cor > 50 ) {
				MainView.outputText( 'You eagerly cry out "<i>Yes, sir!</i>" and show yourself off to the best of your ability.  The whole act is extremely arousing.', false );
			}//[CoC.player is not corrupt]
			else {
				MainView.outputText( 'You have no idea why, but you promptly display yourself in the most provocative way possible.  After a moment you realize what you\'re doing and quickly stop, flushed with embarrassment and arousal.', false );
			}
		}
		//Gimp gear;
		if( this.armorName === LEATHER_CLOTHES ) {
			...
			MainView.outputText( 'The Zealot turns around and gives you a full view of his tight leather clad body.  He smacks his ass and says "<i>You like what you see, don\'t you ' + CoC.player.mf( 'stud', 'slut' ) + '?</i>"  You can\'t help but be incredibly aroused by the scene.', false );
		}
