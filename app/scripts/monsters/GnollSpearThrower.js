...
angular.module( 'cocjs' ).factory( 'GnollSpearThrower', function( MainView, Descriptors, EventParser, ChainedDrop, ConsumableLib, CoC, EngineCore, Monster, Utils, AppearanceDefs, StatusAffects, Appearance, Combat, PerkLib ) {
	...
	GnollSpearThrower.prototype.hyenaPhysicalAttack = function() {
		...
		if( damage <= 0 ) {
			...
			if( Utils.rand( CoC.getInstance().player.armorDef + CoC.getInstance().player.tou ) < CoC.getInstance().player.armorDef ) {
				EngineCore.outputText( 'The gnoll before you suddenly charges, almost too fast to see.  Twin fists slam into your ' + CoC.getInstance().player.armorName + ' with enough force to stagger you, but the force is absorbed without doing any real damage.  As jaws powerful enough to crush bone flash at your neck, you are able to twist to the side, letting the furious hyena slip by you.', false );
			} else {
		...
		if( damage > 0 ) {
			if( this.short === 'fetish zealot' ) {
				...
				EngineCore.dynStats( 'lus', (CoC.getInstance().player.lib / 20 + Utils.rand( 4 ) + 1) );
			}
	...
	GnollSpearThrower.prototype.hyenaJavelinAttack = function() {
		...
		else if( CoC.getInstance().player.armorDef > 10 && Utils.rand( 2 ) === 0 ) {
			EngineCore.outputText( 'The gnoll pulls a long, dark wooden javelin from over her shoulder.  Her spotted arm strikes forward, launching the missile through the air.  The spear flashes through the air but hits at an angle, sliding off your ' + CoC.getInstance().player.armorName + ' without doing any damage.  It disappears into the grass.', false );
		} else if( CoC.getInstance().player.findPerk( PerkLib.Resolute ) >= 0 && CoC.getInstance().player.tou >= 75 ) {
		...
		//<Hyena Attack 2 – Javelin – Successful – Player Entangled>;
			...
			damage = CoC.getInstance().player.takeDamage( 25 + Utils.rand( 20 ) );
			EngineCore.outputText( 'The gnoll pulls a long, black javelin from over her shoulder.  Her spotted arm strikes forward, launching the missile through the air.  You attempt to dive to the side, but are too late.  The powerful shaft slams, hard, into your back.  Pain radiates from the powerful impact.  Instead of piercing you, however, the tip seems to explode into a sticky goo that instantly bonds with your ' + CoC.getInstance().player.armorName + '.  The four foot, heavy shaft pulls down on you awkwardly, catching at things and throwing your balance off.  You try to tug the javelin off of you but find that it has glued itself to you.  It will take time and effort to remove; making it impossible to do while a dominant hyena stalks you. (' + damage + ')', false );
			if( CoC.getInstance().player.findStatusAffect( StatusAffects.GnollSpear ) < 0 ) {
				CoC.getInstance().player.createStatusAffect( StatusAffects.GnollSpear, 0, 0, 0, 0 );
			}
			slow = 15;
			while( slow > 0 && CoC.getInstance().player.spe > 2 ) {
				slow--;
				CoC.getInstance().player.addStatusValue( StatusAffects.GnollSpear, 1, 1 );
	...
	GnollSpearThrower.prototype.hyenaSnapKicku = function() {
		...
		//Determine damage - str modified by enemy toughness!;
			...
			damage = CoC.getInstance().player.biggestTitSize();
			if( damage > 20 ) {
				damage = 20;
			}
			damage += Math.ceil( this.str - Math.random() * (CoC.getInstance().player.tou) - CoC.getInstance().player.armorDef );
			if( damage > 0 ) {
				damage = CoC.getInstance().player.takeDamage( damage );
			}
			//No damage;
	...
	GnollSpearThrower.prototype.hyenaArousalAttack = function() {
		...
		if( CoC.getInstance().player.cor + CoC.getInstance().player.lib > chance + 50 ) {
			...
			EngineCore.dynStats( 'lus', (25 + CoC.getInstance().player.lib / 20 + CoC.getInstance().player.sens / 5) );
		}
		//<Hyena Attack 4 – Arousal Attack – Mildly Successful>;
		else if( 20 + CoC.getInstance().player.cor + CoC.getInstance().player.lib > chance ) {
			...
			EngineCore.dynStats( 'lus', (15 + CoC.getInstance().player.lib / 20 + CoC.getInstance().player.sens / 5) );
		}
	...
	GnollSpearThrower.prototype.defeated = function() {
		if( this.short === 'alpha gnoll' ) {
			...
			EngineCore.menu();
			EngineCore.addButton( 0, 'Fuck', CoC.getInstance().urtaQuest.winRapeHyenaPrincess );
	...
	GnollSpearThrower.prototype.won = function( hpVictory, pcCameWorms ) {
		if( this.short === 'alpha gnoll' ) {
			CoC.getInstance().urtaQuest.loseToGnollPrincessAndGetGangBanged();
		} else if( pcCameWorms ) {
