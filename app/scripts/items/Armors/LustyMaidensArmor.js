...
angular.module( 'cocjs' ).run( function( MainView, ArmorLib, SceneLib, AppearanceDefs, kFLAGS, Minotaur, MinotaurMob, Combat, Armor, PerkLib, EngineCore, CoC ) {
	...
	LustyMaidensArmor.prototype.canUse = function() {
		...
		if( CoC.player.hasCock() || CoC.player.balls > 0 ) {
			MainView.outputText( 'that it won\'t fit you ' );
			if( CoC.player.hasCock() ) {
				MainView.outputText( 'or your ' + CoC.player.multiCockDescriptLight() );
			} else {
				MainView.outputText( 'or your [balls]' );
			}
			MainView.outputText( ' at all!  <b>You put your old gear back on with a sigh</b>.' );
			return false;
		} else if( !CoC.player.hasVagina() ) {
			MainView.outputText( 'that it will dig uncomfortably into your featureless groin.  <b>You put your old gear back on with a sigh</b>.' );
			return false;
		}
		MainView.outputText( 'your [vagina] is prominently displaying your camel-toe for all to see.' );
		...
		if( CoC.player.vaginas[ 0 ].virgin ) {
			MainView.outputText( 'a tight seal over your chastity, displaying your womanly status while guarding your maidenhead at the same time.  A smug, smile tugs at the corners of your mouth - who would take your virginity when they can tit-fuck your tits or fuck your butt?' );
			if( CoC.player.cor < 33 ) {
				MainView.outputText( '  Wait, that isn\'t right...' );
			}
		} else {
			MainView.outputText( 'a tight seal over your previously-claimed cunt.  Regret fills you when you realize you could have kept your chastity intact simply by servicing the lusty studs and monsters with your ass and tits.' );
		}
		if( CoC.player.wetness() >= 3 ) {
			MainView.outputText( '  The moisture you normally drip seems to soak right into the gusset instead of running down your [legs] like normal, giving you a much more chaste appearance in spite of the lewd garments that even now seem to shape your femininity and [butt] into perfectly arousing shapes.' );
		}
	...
	LustyMaidensArmor.prototype.lustyMaidenPaizuri = function( player, monster ) {
		...
		if( !player.hasVirginVagina() ) {
			MainView.outputText( 'a gift too fine for a selfish brute like you' );
		} else {
			MainView.outputText( 'sacred and to be shared only with a cherished loved one' );
		}
		MainView.outputText( '!  Now, I\'m going to squeeze all the impure thoughts out of you through your cock, so you just lie there and focus on letting them out all over my breasts.</i>"' );
		...
		if( monster.balls > 0 ) {
			MainView.outputText( ' and fondle ' + monster.pronoun3 + ' balls one-handed, squeezing the virile orbs to try and coax more of ' + monster.pronoun3 + ' dirty, perverted thoughts to distill into salty seed' );
		} else if( monster.short === 'anemone' ) {
			MainView.outputText( ' and stroke ' + monster.pronoun3 + ' taint, even brushing over the featureless spot where an asshole would be, if she had one, to try and coax more of ' + monster.pronoun3 + ' dirty, perverted thoughts to distill into salty seed' );
		} else {
			MainView.outputText( ' and stroke ' + monster.pronoun3 + ' taint, even brushing close to ' + monster.pronoun3 + ' asshole to try and coax more of ' + monster.pronoun3 + ' dirty, perverted thoughts to distill into salty seed' );
		}
		MainView.outputText( '.  A startled moan slips out of ' + monster.pronoun3 + ' lips, but you\'re just getting warmed up.  You dive down onto ' + monster.pronoun3 + ' ' + monster.cockDescriptShort( 0 ) + ', taking the ' + monster.cockHead() + ' straight into your mouth with a smooth gulp.' );
		if( monster.cockArea( 0 ) >= 80 ) {
			MainView.outputText( '  It\'s so big and strong that it pushes right into your throat, stretching out your neck in the shape of the intruding cock.' );
		}
		MainView.outputText( '  The strong, pulsing cock feels so good inside your mouth, like it belongs there, and you can\'t help but think that you\'re doing a good deed by helping ' + monster.a + monster.short + ' empty every last perverse desire onto your purifying breasts.' );
		...
		MainView.outputText( ' and sensitive, scraping around the nebulous inner lining of your bikini and occasionally catching on the metal that feels even warmer than normal.  Behind you, your [butt] is bouncing happily to the rhythm your corruption-devouring breasts have set, the thong digging tightly into your [vagina] in the most exquisite way.  You feel so hot and sensual, but still secure in the knowledge that you won\'t have to worry about such a creature ravaging your ' );
		if( player.hasVirginVagina() ) {
			MainView.outputText( 'maidenhead' );
		} else {
			MainView.outputText( 'sloppy gash' );
		}
		MainView.outputText( '.  Still, you\'re not sure how much hotter you can get before you\'re cumming all over your g-string, letting your own dark thoughts seep into your magical underwear.' );
