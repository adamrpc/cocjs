	...
	BimboLiqueur.prototype.useItem = function() {
		if( CoC.getInstance().player.findPerk( PerkLib.BroBody ) >= 0 ) {
			...
			if( CoC.getInstance().player.tallness < 77 ) {
				...
				CoC.getInstance().player.tallness = 77;
			}
			...
			if( !CoC.getInstance().player.hasVagina() ) {
				EngineCore.outputText( '  Before you can even take a breath, an extremely peculiar sensation emanates from your crotch.  You can\'t see through your ' + CoC.getInstance().player.armorName + ', but you can certainly feel the vagina splitting ' + (CoC.getInstance().player.balls > 0 ? 'from behind your testicles' : 'your groin') + '.  Luckily, the cunt-forming doesn\'t yield any discomfort - on the contrary, you feel yourself falling farther into your chemically-dulled, libido-fueled rut.' );
				if( CoC.getInstance().player.hipRating < 12 || CoC.getInstance().player.buttRating < 12 ) {
			...
			CoC.getInstance().player.removePerk( PerkLib.BroBody );
			CoC.getInstance().player.removePerk( PerkLib.BroBrains );
			CoC.getInstance().player.createPerk( PerkLib.FutaFaculties, 0, 0, 0, 0 );
			CoC.getInstance().player.createPerk( PerkLib.FutaForm, 0, 0, 0, 0 );
			...
			if( CoC.getInstance().player.hairColor !== 'platinum blonde' ) {
				EngineCore.outputText( 'The first change that you notice is to your ' + CoC.getInstance().player.hairDescript() + '.  It starts with a tingling in your scalp and intensifies ' );
				if( CoC.getInstance().player.hairLength < 36 ) {
					...
					CoC.getInstance().player.hairLength = 36;
				} else {
					EngineCore.outputText( 'as your hair grows thicker and heavier.' );
				}
				EngineCore.outputText( '  You grab a lock of the silken strands and watch open-mouthed while streaks so blonde they\'re almost white flow down the ' + CoC.getInstance().player.hairColor + ' hair.  It goes faster and faster until your hair has changed into perfectly bimbo-blonde, flowing locks.\n\n' );
			...
			if( !CoC.getInstance().player.hasVagina() ) {
				...
				if( CoC.getInstance().player.isTaur() ) {
					EngineCore.outputText( 'Wait!? Wet? You wish you could touch yourself between the ' + CoC.getInstance().player.legs() + ', but you can tell from the fluid running down your hind-legs just how soaked your new vagina is.' );
				} else {
					EngineCore.outputText( 'Wait!?  Wet?  You touch yourself between the ' + CoC.getInstance().player.legs() + ' and groan when your fingers sink into a sloppy, wet cunt.' );
				}
			} else {
				if( CoC.getInstance().player.isTaur() ) {
			...
			if( CoC.getInstance().player.hasCock() ) {
				...
				while( CoC.getInstance().player.hasCock() ) {
			...
			EngineCore.outputText( 'You sigh and run one hand over your ' + CoC.getInstance().player.nippleDescript( 0 ) + 's' );
			if( CoC.getInstance().player.breastRows[ 0 ].breastRating < 10 ) {
				CoC.getInstance().player.breastRows[ 0 ].breastRating += 5 + Utils.rand( 5 );
				EngineCore.outputText( ', surprised at how large and rounded your expanding breasts have become while fresh tit-flesh continues to spill out around your needy fingers.  They feel so supple and soft, but when you let them go, they still sit fairly high and firm on your chest.  The newer, more generous, ' + CoC.getInstance().player.breastCup( 0 ) + ' cleavage has you moaning with how sensitive it is, pinching a nipple with one hand ' );
			} else {
				CoC.getInstance().player.breastRows[ 0 ].breastRating += 5 + Utils.rand( 5 );
				EngineCore.outputText( ', admiring how sensitive they\'re getting.  The big breasts start getting bigger and bigger, soft chest-flesh practically oozing out between your fingers as the squishy mammaries sprout like weeds, expanding well beyond any hand\'s ability to contain them.  The supple, ' + CoC.getInstance().player.breastCup( 0 ) + ' boobs still manage to sit high on your chest, almost gravity defying in their ability to generate cleavage.  You pinch a nipple with one hand ' );
			}
			EngineCore.dynStats( 'sen', 20 );
			EngineCore.outputText( 'while the other toys with the juicy entrance of your folds.  Mmmm, it, like, feels too good not to touch yourself, and after being worried about getting all dumb and stuff, you need to relax.  Thinking is hard, but sex is so easy and, like, natural!  You lean back and start grunting as you plunge four fingers inside yourself, plowing your ' + CoC.getInstance().player.vaginaDescript( 0 ) + ' like no tomorrow.  By now, your ' + CoC.getInstance().player.clitDescript() + ' is throbbing, and you give it an experimental ' );
			if( CoC.getInstance().player.clitLength >= 3 ) {
				EngineCore.outputText( 'jerk ' );
			} else {
				EngineCore.outputText( 'caress ' );
			}
			EngineCore.outputText( 'that makes your ' + CoC.getInstance().player.legs() + ' give out as you cum, splattering female fluids as you convulse nervelessly on the ground.\n\n' );
			...
			if( CoC.getInstance().player.hipRating < 12 || CoC.getInstance().player.buttRating < 12 ) {
				...
				if( CoC.getInstance().player.hipRating < 12 && CoC.getInstance().player.buttRating < 12 ) {
					...
					CoC.getInstance().player.buttRating = 12;
					CoC.getInstance().player.hipRating = 12;
				} else if( CoC.getInstance().player.hipRating < 12 ) {
			...
			if( CoC.getInstance().player.tone > 0 ) {
				...
				if( CoC.getInstance().player.str >= 30 ) {
			...
			//FULL ON BITCHFACE;
			CoC.getInstance().player.modFem( 100, 100 );
			//Body;
			//Tease/Seduce Boost;
