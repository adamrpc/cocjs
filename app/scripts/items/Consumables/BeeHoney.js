...
angular.module( 'cocjs' ).factory( 'BeeHoney', function( CockTypesEnum, CoC, StatusAffects, AppearanceDefs, Descriptors, Utils, PregnancyStore, kFLAGS, Consumable, PerkLib, EngineCore ) {
	...
	BeeHoney.prototype.canUse = function() {
		if( this.value === SPECIAL_HONEY_VALUE && CoC.getInstance().player.statusAffectv1( StatusAffects.Exgartuan ) === 1 ) { //Exgartuan doesn't like the special honey
	...
	BeeHoney.prototype.useItem = function() {
		var player = CoC.getInstance().player;
		...
		if( (pure || special) && CoC.getInstance().player.pregnancyType === PregnancyStore.PREGNANCY_FAERIE ) { //Pure or special honey can reduce the corruption of a phouka baby
			if( CoC.getInstance().flags[ kFLAGS.PREGNANCY_CORRUPTION ] > 1 ) { //Child is phouka, hates pure honey
			...
			} else if( CoC.getInstance().flags[ kFLAGS.PREGNANCY_CORRUPTION ] < 1 ) { //Child is faerie, loves pure honey
			...
			CoC.getInstance().flags[ kFLAGS.PREGNANCY_CORRUPTION ]--;
			if( pure ) {
		...
		if( changes < changeLimit && pure ) {
			...
			EngineCore.dynStats( 'cor', -(1 + (CoC.getInstance().player.cor / 20)) );
			//Libido Reduction;
			if( CoC.getInstance().player.cor > 0 && changes < changeLimit && Utils.rand( 1.5 ) === 0 && CoC.getInstance().player.lib > 40 ) {
				EngineCore.outputText( ' and settling your overcharged sex-drive a bit.' );
				EngineCore.dynStats( 'lib', -3, 'lus', -20 );
				changes++;
			} else if( CoC.getInstance().player.cor > 0 ) {
				EngineCore.outputText( '.' );
			}
		}
		//bee item corollary:;
		if( changes < changeLimit && CoC.getInstance().player.hairType === 4 && Utils.rand( 2 ) === 0 ) {
			...
			CoC.getInstance().player.hairType = 0;
			changes++;
		}
		//(removes tentacle hair status, restarts hair growth if not prevented by reptile status);
		//Intelligence Boost;
		if( changes < changeLimit && Utils.rand( 2 ) === 0 && CoC.getInstance().player.inte < 80 ) {
			EngineCore.dynStats( 'int', 0.1 * (80 - CoC.getInstance().player.inte) );
		...
		//Appearance Stuff;
		//Hair Color;
		if( changes < changeLimit && (CoC.getInstance().player.hairColor !== 'shiny black' && CoC.getInstance().player.hairColor !== 'black and yellow') && CoC.getInstance().player.hairLength > 10 && Utils.rand( 5 ) === 0 ) {
			EngineCore.outputText( '\n\nYou feel your scalp tingling, and you grab your hair in a panic, pulling a strand forward.  ' );
			if( Utils.rand( 9 ) === 0 ) {
				CoC.getInstance().player.hairColor = 'black and yellow';
			} else {
				CoC.getInstance().player.hairColor = 'shiny black';
			}
			EngineCore.outputText( 'Your hair is now ' + CoC.getInstance().player.hairColor + ', just like a bee-girl\'s!' );
			changes++;
		}
		//Hair Length;
		if( changes < changeLimit && CoC.getInstance().player.hairLength < 25 && Utils.rand( 3 ) === 0 ) {
			EngineCore.outputText( '\n\nFeeling a bit off-balance, you discover your hair has lengthened, ' );
			CoC.getInstance().player.hairLength += Utils.rand( 4 ) + 1;
			EngineCore.outputText( 'becoming ' + Descriptors.hairDescript() + '.' );
			changes++;
		}
		//-Remove extra breast rows;
		if( changes < changeLimit && CoC.getInstance().player.bRows() > 2 && Utils.rand( 3 ) === 0 && !CoC.getInstance().flags[ kFLAGS.HYPER_HAPPY ] ) {
			changes++;
			EngineCore.outputText( '\n\nYou stumble back when your center of balance shifts, and though you adjust before you can fall over, you\'re left to watch in awe as your bottom-most ' + Descriptors.breastDescript( CoC.getInstance().player.breastRows.length - 1 ) + ' shrink down, disappearing completely into your ' );
			if( CoC.getInstance().player.bRows() >= 3 ) {
				EngineCore.outputText( 'abdomen' );
			} else {
				EngineCore.outputText( 'chest' );
			}
			EngineCore.outputText( '. The ' + Descriptors.nippleDescript( CoC.getInstance().player.breastRows.length - 1 ) + 's even fade until nothing but ' );
			if( CoC.getInstance().player.skinType === AppearanceDefs.SKIN_TYPE_FUR ) {
				EngineCore.outputText( CoC.getInstance().player.hairColor + ' ' + CoC.getInstance().player.skinDesc );
			} else {
				EngineCore.outputText( CoC.getInstance().player.skinTone + ' ' + CoC.getInstance().player.skinDesc );
			}
		...
		//Antennae;
		if( changes < changeLimit && CoC.getInstance().player.antennae === AppearanceDefs.ANTENNAE_NONE && CoC.getInstance().player.horns === 0 && Utils.rand( 3 ) === 0 ) {
			EngineCore.outputText( '\n\nYour head itches momentarily as two floppy antennae sprout from your ' + Descriptors.hairDescript() + '.' );
			CoC.getInstance().player.antennae = AppearanceDefs.ANTENNAE_BEE;
			changes++;
		}
		//Horns;
		if( changes < changeLimit && CoC.getInstance().player.horns > 0 && Utils.rand( 3 ) === 0 ) {
		...
		//Bee Legs;
		if( changes < changeLimit && CoC.getInstance().player.lowerBody !== AppearanceDefs.LOWER_BODY_TYPE_BEE && CoC.getInstance().player.lowerBody !== AppearanceDefs.LOWER_BODY_TYPE_CENTAUR && Utils.rand( 4 ) === 0 ) {
			...
			CoC.getInstance().player.lowerBody = AppearanceDefs.LOWER_BODY_TYPE_BEE;
			changes++;
		}
		//-Nipples reduction to 1 per tit.;
		if( CoC.getInstance().player.averageNipplesPerBreast() > 1 && changes < changeLimit && Utils.rand( 4 ) === 0 ) {
			EngineCore.outputText( '\n\nA chill runs over your ' + Descriptors.allBreastsDescript() + ' and vanishes.  You stick a hand under your ' + CoC.getInstance().player.armorName + ' and discover that your extra nipples are missing!  You\'re down to just one per ' );
			...
			for( var temp = 0; temp < CoC.getInstance().player.breastRows.length; temp++ ) {
				CoC.getInstance().player.breastRows[ temp ].nipplesPerBreast = 1;
			}
		}
		//Gain oviposition!;
		if( changes < changeLimit && CoC.getInstance().player.findPerk( PerkLib.BeeOvipositor ) < 0 && CoC.getInstance().player.tailType === AppearanceDefs.TAIL_TYPE_BEE_ABDOMEN && Utils.rand( 2 ) === 0 ) {
			...
			CoC.getInstance().player.createPerk( PerkLib.BeeOvipositor, 0, 0, 0, 0 );
			changes++;
		}
		//Bee butt - 66% lower chance if already has a tail;
		if( changes < changeLimit && CoC.getInstance().player.tailType !== AppearanceDefs.TAIL_TYPE_BEE_ABDOMEN && (CoC.getInstance().player.tailType === AppearanceDefs.TAIL_TYPE_NONE || Utils.rand( 1.5 ) === 0) && Utils.rand( 4 ) === 0 ) {
			...
			CoC.getInstance().player.tailType = AppearanceDefs.TAIL_TYPE_BEE_ABDOMEN;
			CoC.getInstance().player.tailVenom = 10;
			CoC.getInstance().player.tailRecharge = 2;
			changes++;
		}
		//Venom Increase;
		if( changes < changeLimit && CoC.getInstance().player.tailType === AppearanceDefs.TAIL_TYPE_BEE_ABDOMEN && CoC.getInstance().player.tailRecharge < 15 && Utils.rand( 2 ) ) {
			if( CoC.getInstance().player.tailRecharge < 5 ) {
				CoC.getInstance().player.tailRecharge += 1;
			}
			if( CoC.getInstance().player.tailRecharge < 10 ) {
				CoC.getInstance().player.tailRecharge += 1;
			}
			if( CoC.getInstance().player.tailRecharge < 15 ) {
				CoC.getInstance().player.tailRecharge += 1;
			}
			CoC.getInstance().player.tailVenom += 50;
		...
		if( changes < changeLimit && CoC.getInstance().player.wingType === AppearanceDefs.WING_TYPE_BEE_LIKE_SMALL && Utils.rand( 4 ) ) {
			changes++;
			CoC.getInstance().player.wingType = AppearanceDefs.WING_TYPE_BEE_LIKE_LARGE;
			CoC.getInstance().player.wingDesc = 'large bee-like';
		...
		if( changes < changeLimit && (CoC.getInstance().player.wingType === AppearanceDefs.WING_TYPE_NONE || CoC.getInstance().player.wingType === AppearanceDefs.WING_TYPE_SHARK_FIN) && Utils.rand( 4 ) ) {
			changes++;
			if( CoC.getInstance().player.wingType === AppearanceDefs.WING_TYPE_SHARK_FIN ) {
				EngineCore.outputText( '\n\nYou feel an itching on your large back-fin as something begins growing there.  You twist and contort yourself, trying to scratch and bring yourself relief, and failing miserably.  A sense of relief erupts from you as you feel something new grow out from your fin.  You hastily remove the top portion of your ' + CoC.getInstance().player.armorName + ' and marvel as a pair of small bee-like wings sprout from your back, replacing the fin that once grew there.  Tenderly flexing your new muscles, you find you can flap them quite fast.  Unfortunately you can\'t seem to flap your little wings fast enough to fly, but they would certainly slow a fall.  A few quick modifications to your ' + CoC.getInstance().player.armorName + ' later and you are ready to continue your journey with <b>your new bee wings</b>.' );
			} else {
				EngineCore.outputText( '\n\nYou feel an itching between your shoulder-blades as something begins growing there.  You twist and contort yourself, trying to scratch and bring yourself relief, and failing miserably.  A sense of relief erupts from you as you feel something new grow out from your body.  You hastily remove the top portion of your ' + CoC.getInstance().player.armorName + ' and marvel as a pair of small bee-like wings sprout from your back.  Tenderly flexing your new muscles, you find you can flap them quite fast.  Unfortunately you can\'t seem to flap your little wings fast enough to fly, but they would certainly slow a fall.  A few quick modifications to your ' + CoC.getInstance().player.armorName + ' later and you are ready to continue your journey with <b>your new bee wings</b>.' );
			}
			CoC.getInstance().player.wingType = AppearanceDefs.WING_TYPE_BEE_LIKE_SMALL;
			CoC.getInstance().player.wingDesc = 'small bee-like';
		}
		//Melt demon wings!;
		if( changes < changeLimit && (CoC.getInstance().player.wingType === AppearanceDefs.WING_TYPE_BAT_LIKE_TINY || CoC.getInstance().player.wingType === AppearanceDefs.WING_TYPE_BAT_LIKE_LARGE) ) {
			changes++;
			...
			CoC.getInstance().player.wingDesc = '';
		}
		if( Utils.rand( 4 ) === 0 && CoC.getInstance().player.gills && changes < changeLimit ) {
			EngineCore.outputText( '\n\nYour chest itches, and as you reach up to scratch it, you realize your gills have withdrawn into your skin.' );
			CoC.getInstance().player.gills = false;
			changes++;
		}
		if( special ) {
			if( !CoC.getInstance().player.hasCock() ) {
				...
				CoC.getInstance().player.cocks[ 0 ].cockLength = Utils.rand( 3 ) + 8;
				CoC.getInstance().player.cocks[ 0 ].cockThickness = 2;
				EngineCore.dynStats( 'sen', 10 );
			} else if( CoC.getInstance().player.cocks.length > 1 ) {
				var biggest = CoC.getInstance().player.biggestCockIndex();
				EngineCore.outputText( '\n\nThe effects of the honey move towards your groin, and into your ' + CoC.getInstance().player.multiCockDescriptLight() + ', causing them to stand at attention.  They quiver for a moment, and feel rather itchy.  Suddenly you are overwhelmed with pleasure as <b>your ' + CoC.getInstance().player.cockDescript( biggest ) + ' is absorbed into your ' + CoC.getInstance().player.cockDescript( 0 ) + '!</b>  You grab onto the merging cock and pump it with your hands as it increases in size and you cum in pleasure.  Your ' + CoC.getInstance().player.cockDescript( 0 ) + ' seems a lot more sensative now...' );
				CoC.getInstance().player.cocks[ 0 ].cockLength += 5 * Math.sqrt( 0.2 * CoC.getInstance().player.cocks[ biggest ].cArea() );
				CoC.getInstance().player.cocks[ 0 ].cockThickness += Math.sqrt( 0.2 * CoC.getInstance().player.cocks[ biggest ].cArea() );
				CoC.getInstance().player.removeCock( biggest, 1 );
				EngineCore.dynStats( 'sen', 5 );
			} else if( CoC.getInstance().player.cocks[ 0 ].cArea() < 100 ) {
				EngineCore.outputText( '\n\nYour ' + CoC.getInstance().player.cockDescript( 0 ) + ' suddenly becomes rock hard and incredibly sensitive to the touch.  You pull away your ' + CoC.getInstance().player.armorName + ', and start to masturbate furiously as it rapidly swells in size.  When the change finally finishes, you realize that your ' + CoC.getInstance().player.cockDescript( 0 ) + ' has both grown much longer and wider!  <b>' );
				if( CoC.getInstance().player.cocks[ 0 ].cArea() <= 20 ) {
					EngineCore.outputText( 'It now swings as low as your knees!' );
				} else if( CoC.getInstance().player.cocks[ 0 ].cArea() <= 50 ) {
				...
				CoC.getInstance().player.cocks[ 0 ].cockType = CockTypesEnum.BEE;
				CoC.getInstance().player.cocks[ 0 ].cockLength += 5;
				CoC.getInstance().player.cocks[ 0 ].cockThickness += 1;
				EngineCore.dynStats( 'sen', 15 );
			} else {
				EngineCore.outputText( '\n\nThe effects of the honey don’t seem to focus on your groin this time, but you still feel your ' + CoC.getInstance().player.cockDescript( 0 ) + ' grow slightly under your ' + CoC.getInstance().player.armorName + '.' );
				CoC.getInstance().player.cocks[ 0 ].cockLength += 0.1 * Utils.rand( 10 ) + 1;
				CoC.getInstance().player.cocks[ 0 ].cockThickness += 0.1 * Utils.rand( 2 ) + 0.1;
			...
			if( CoC.getInstance().player.cor >= 5 ) {
				...
				var corLoss = Math.min( 0.1 * CoC.getInstance().player.cor + 5, CoC.getInstance().player.cor );
			...
			if( CoC.getInstance().player.femininity >= 60 || CoC.getInstance().player.femininity <= 40 ) {
				EngineCore.outputText( '\n\nYour face shifts in shape, becoming more androgynous.' );
				if( CoC.getInstance().player.femininity >= 60 ) {
					CoC.getInstance().player.femininity -= 3;
				} else {
					CoC.getInstance().player.femininity += 3;
				}
			}
			EngineCore.dynStats( 'lust', 0.2 * CoC.getInstance().player.lib + 5 );
		}
		return false;
	};
	return BeeHoney;
