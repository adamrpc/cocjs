...
angular.module( 'cocjs' ).run( function( MainView, ConsumableLib, CockTypesEnum, CoC, StatusAffects, AppearanceDefs, Descriptors, Utils, PregnancyStore, kFLAGS, Useable, PerkLib, EngineCore ) {
	...
	BeeHoney.prototype.canUse = function() {
		if( this.value === SPECIAL_HONEY_VALUE && CoC.player.statusAffectv1( StatusAffects.Exgartuan ) === 1 ) { //Exgartuan doesn't like the special honey
			MainView.outputText( 'You uncork the bottle only to hear Exgartuan suddenly speak up.  <i>“Hey kid, this beautiful cock here doesn’t need any of that special bee shit.  Cork that bottle up right now or I’m going to make it so that you can’t drink anything but me.”</i>  You give an exasperated sigh and put the cork back in the bottle.' );
			return false;
		}
		return true;
	};
	BeeHoney.prototype.useItem = function() {
		...
		//Drink text;
		if( special ) {
			MainView.outputText( 'You uncork the bottle and pour the incredibly strong smelling concentrated honey down your throat.  Its taste is also mighty intense.  All at once you feel the effects of the substance start to course through your body.' );
		} else {
			{ //Text for normal or pure
			}
			MainView.outputText( 'Opening the crystal vial, you are greeted by a super-concentrated wave of sweet honey-scent.  It makes you feel lightheaded.  You giggle and lick the honey from your lips, having drank down the syrupy elixir without a thought.' );
		}
		...
		if( changes < changeLimit && pure ) {
			...
			if( CoC.player.cor > 80 ) {
				MainView.outputText( 'Your head aches, as if thunder was echoing around your skull.  ' );
			} else if( CoC.player.cor > 60 ) {
				MainView.outputText( 'You feel a headache forming just behind your eyes.  In no time flat it reaches full strength.  ' );
			} else if( CoC.player.cor > 40 ) {
				MainView.outputText( 'A wave of stinging pain slices through your skull.  ' );
			} else if( CoC.player.cor > 20 ) {
				MainView.outputText( 'A prickling pain spreads throughout your skull.  ' );
			} else {
				MainView.outputText( 'You feel a mildly unpleasant tingling inside your skull.  ' );
			}
			if( CoC.player.cor > 0 ) {
				MainView.outputText( 'It quickly passes, leaving you more clearheaded' );
			}
			EngineCore.dynStats( 'cor', -(1 + (CoC.player.cor / 20)) );
			//Libido Reduction;
			if( CoC.player.cor > 0 && changes < changeLimit && Utils.rand( 1.5 ) === 0 && CoC.player.lib > 40 ) {
				MainView.outputText( ' and settling your overcharged sex-drive a bit.' );
		...
		if( changes < changeLimit && (CoC.player.hairColor !== 'shiny black' && CoC.player.hairColor !== 'black and yellow') && CoC.player.hairLength > 10 && Utils.rand( 5 ) === 0 ) {
			...
			MainView.outputText( 'Your hair is now ' + CoC.player.hairColor + ', just like a bee-girl\'s!' );
			changes++;
		}
		//Hair Length;
		if( changes < changeLimit && CoC.player.hairLength < 25 && Utils.rand( 3 ) === 0 ) {
			...
			MainView.outputText( 'becoming ' + Descriptors.hairDescript() + '.' );
			changes++;
		}
		...
		if( changes < changeLimit && CoC.player.bRows() > 2 && Utils.rand( 3 ) === 0 && !CoC.flags[ kFLAGS.HYPER_HAPPY ] ) {
			...
			if( CoC.player.bRows() >= 3 ) {
				MainView.outputText( 'abdomen' );
			} else {
				MainView.outputText( 'chest' );
			}
			MainView.outputText( '. The ' + Descriptors.nippleDescript( CoC.player.breastRows.length - 1 ) + 's even fade until nothing but ' );
			if( CoC.player.skinType === AppearanceDefs.SKIN_TYPE_FUR ) {
				MainView.outputText( CoC.player.hairColor + ' ' + CoC.player.skinDesc );
			} else {
				MainView.outputText( CoC.player.skinTone + ' ' + CoC.player.skinDesc );
			}
			MainView.outputText( ' remains. <b>You\'ve lost a row of breasts!</b>' );
		...
		if( special ) {
			if( !CoC.player.hasCock() ) {
				...
				if( CoC.player.cocks[ 0 ].cArea() <= 20 ) {
					MainView.outputText( 'It now swings as low as your knees!' );
				} else if( CoC.player.cocks[ 0 ].cArea() <= 50 ) {
					MainView.outputText( 'While erect, your massive member fills the lower half of your vision.' );
				} else {
					MainView.outputText( 'Your member is now simply huge, you wonder what in the world could actually take your massive size now?' );
				}
