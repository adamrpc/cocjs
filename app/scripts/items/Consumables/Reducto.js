...
angular.module( 'cocjs' ).run( function( MainView, ConsumableLib, SceneLib, CockTypesEnum, CoC, Utils, Useable, EngineCore ) {
	...
	Reducto.prototype.useItem = function() {
		...
		MainView.outputText( 'You ponder the paste in your hand and wonder what part of your body you would like to shrink.  What will you use it on?' );
	...
	Reducto.prototype.reductoBalls = function() {
		MainView.clearOutput();
		MainView.outputText( 'You smear the foul-smelling paste onto your ' + CoC.player.sackDescript() + '.  It feels cool at first but rapidly warms to an uncomfortable level of heat.\n\n' );
		CoC.player.ballSize -= Utils.rand( 4 ) + 2;
		if( CoC.player.ballSize < 1 ) {
			CoC.player.ballSize = 1;
		}
		MainView.outputText( 'You feel your scrotum shift, shrinking down along with your ' + CoC.player.ballsDescriptLight() + '.  Within a few seconds the paste has been totally absorbed and the shrinking stops.' );
		EngineCore.dynStats( 'lib', -2, 'lus', -10 );
		SceneLib.inventory.itemGoNext();
	};
	Reducto.prototype.reductoBreasts = function() {
		MainView.clearOutput();
		MainView.outputText( 'You smear the foul-smelling ointment all over your ' + CoC.player.allBreastsDescript() + ', covering them entirely as the paste begins to get absorbed into your ' + CoC.player.skinDesc + '.\n' );
	...
	Reducto.prototype.reductoButt = function() {
		MainView.clearOutput();
		MainView.outputText( 'You smear the foul-smelling paste onto your ' + CoC.player.buttDescript() + '.  It feels cool at first but rapidly warms to an uncomfortable level of heat.\n\n' );
		if( CoC.player.buttRating >= 15 ) {
			CoC.player.buttRating -= (3 + Math.ceil( CoC.player.buttRating / 3 ));
			MainView.outputText( 'Within seconds you feel noticeably lighter, and a quick glance shows your ass is significantly smaller.' );
		} else if( CoC.player.buttRating >= 10 ) {
			CoC.player.buttRating -= 3;
			MainView.outputText( 'You feel much lighter as your ' + CoC.player.buttDescript() + ' jiggles slightly, adjusting to its smaller size.' );
		} else {
			CoC.player.buttRating -= Utils.rand( 3 ) + 1;
			if( CoC.player.buttRating < 1 ) {
				CoC.player.buttRating = 1;
			}
			MainView.outputText( 'After a few seconds your ' + CoC.player.buttDescript() + ' has shrunk to a much smaller size!' );
		}
		EngineCore.dynStats( 'lib', -2, 'lus', -10 );
		SceneLib.inventory.itemGoNext();
	};
	Reducto.prototype.reductoClit = function() {
		MainView.clearOutput();
		MainView.outputText( 'You carefully apply the paste to your ' + CoC.player.clitDescript() + ', being very careful to avoid getting it on your ' + CoC.player.vaginaDescript( 0 ) + '.  It burns with heat as it begins to make its effects known...\n\n' );
		...
		MainView.outputText( 'Your ' + CoC.player.clitDescript() + ' shrinks rapidly, dwindling down to almost half its old size before it finishes absorbing the paste.' );
	...
	Reducto.prototype.reductoCock = function() {
		...
		if( CoC.player.cocks[ 0 ].cockType === CockTypesEnum.BEE ) {
			MainView.outputText( 'The gel produces an odd effect when you rub it into your ' + CoC.player.cockDescript( 0 ) + '.  It actually seems to calm the need that usually fills you.  In fact, as your ' + CoC.player.cockDescript( 0 ) + ' shrinks, its skin tone changes to be more in line with yours and the bee hair that covered it falls out.  <b>You now have a human cock!</b>' );
			CoC.player.cocks[ 0 ].cockType = CockTypesEnum.HUMAN;
		} else {
			MainView.outputText( 'You smear the repulsive smelling paste over your ' + CoC.player.multiCockDescriptLight() + '.  It immediately begins to grow warm, almost uncomfortably so, as your ' + CoC.player.multiCockDescriptLight() + ' begins to shrink.\n\n' );
			if( CoC.player.cocks.length === 1 ) {
				MainView.outputText( 'Your ' + CoC.player.cockDescript( 0 ) + ' twitches as it shrinks, disappearing steadily into your ' + (CoC.player.hasSheath() ? 'sheath' : 'crotch') + ' until it has lost about a third of its old size.' );
				CoC.player.cocks[ 0 ].cockLength *= 2 / 3;
				CoC.player.cocks[ 0 ].cockThickness *= 2 / 3;
			} else {
				{ //MULTI
				}
				MainView.outputText( 'Your ' + CoC.player.multiCockDescriptLight() + ' twitch and shrink, each member steadily disappearing into your ' + (CoC.player.hasSheath() ? 'sheath' : 'crotch') + ' until they\'ve lost about a third of their old size.' );
	...
	Reducto.prototype.reductoHips = function() {
		MainView.clearOutput();
		MainView.outputText( 'You smear the foul-smelling paste onto your [hips].  It feels cool at first but rapidly warms to an uncomfortable level of heat.\n\n' );
		if( CoC.player.hipRating >= 15 ) {
			CoC.player.hipRating -= (3 + Math.ceil( CoC.player.hipRating / 3 ));
			MainView.outputText( 'Within seconds you feel noticeably lighter, and a quick glance at your hips shows they\'ve gotten significantly narrower.' );
		} else if( CoC.player.hipRating >= 10 ) {
			CoC.player.hipRating -= 3;
			MainView.outputText( 'You feel much lighter as your [hips] shift slightly, adjusting to their smaller size.' );
		} else {
			CoC.player.hipRating -= Utils.rand( 3 ) + 1;
			if( CoC.player.hipRating < 1 ) {
				CoC.player.hipRating = 1;
			}
			MainView.outputText( 'After a few seconds your [hips] have shrunk to a much smaller size!' );
		}
		EngineCore.dynStats( 'lib', -2, 'lus', -10 );
		SceneLib.inventory.itemGoNext();
	};
	Reducto.prototype.reductoNipples = function() {
		MainView.clearOutput();
		MainView.outputText( 'You rub the paste evenly over your ' + CoC.player.nippleDescript( 0 ) + 's, being sure to cover them completely.\n\n' );
		//Shrink;
		if( CoC.player.nippleLength / 2 < 0.25 ) {
			MainView.outputText( 'Your nipples continue to shrink down until they stop at 1/4" long.' );
			CoC.player.nippleLength = 0.25;
		} else {
			MainView.outputText( 'Your ' + CoC.player.nippleDescript( 0 ) + 's get smaller and smaller, stopping when they are roughly half their previous size.' );
	...
	Reducto.prototype.reductoCancel = function() {
		MainView.clearOutput();
		MainView.outputText( 'You put the salve away.\n\n' );
