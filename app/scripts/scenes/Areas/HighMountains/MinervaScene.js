...
angular.module( 'cocjs' ).run( function( MainView, SceneLib, EngineCore, CoC, kFLAGS, Combat, Utils, AppearanceDefs, StatusAffects, Descriptors, ConsumableLib, Minerva ) {
	...
	MinervaScene.prototype.minervaVCapacity = function() {
		return 90;
	};
	MinervaScene.prototype.minervaACapacity = function() {
		return 100;
	};
	...
	//Basic intros
	//1-1 First Encounter.
	MinervaScene.prototype.firstMinervaEncounter = function() {
		MainView.clearOutput();
		MainView.outputText( 'Returning to the mountains, you begin to climb; the usual search for loot and, perhaps, some harpies driving your upward efforts.  Perhaps you\'ll run into' );
		//If Sophie Bimbo=0:
		if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00282 ] === 0 && CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00283 ] === 0 && !SceneLib.sophieFollowerScene.sophieFollower() ) {
			MainView.outputText( ' that matronly harpy, Sophie' );
		} else {
			MainView.outputText( ' something interesting' );
		}
		MainView.outputText( ', or get some good plunder from some of the local beasts' );
		//if minotaur cum addict:
		if( CoC.player.minotaurAddicted() ) {
			MainView.outputText( '.  With any luck, you might even find a hunky minotaur to scratch that burning itch in your mind' );
		}
	...
	MinervaScene.prototype.ignoreMinervasPath = function() {
		MainView.clearOutput();
		MainView.outputText( 'Deciding to play it safe, you turn away from the new path and continue your search elsewhere.' );
		// PC returns to camp.
	...
	MinervaScene.prototype.takeMinervasPath = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 95 );
		MainView.outputText( 'Knowing that a new path could lead to great things, you can\'t help but wonder where this one could lead.' );
		...
		if( CoC.player.tallness <= 96 ) {
			MainView.outputText( ' one hand' );
		} else {
			MainView.outputText( ' both hands' );
		}
		MainView.outputText( ', bodily swinging you out over the abyss, with only her grip keeping you from plunging back down the mountainside.' );
		//Low corruption:
		if( CoC.player.cor < 50 ) {
			MainView.outputText( '  "<i>All right, you!  Who are you and what are you doing here?!  I don\'t get any visitors besides those horny harpies, so what are you after?  Hm?' );
		}//High corruption:
		else {
			MainView.outputText( '  "<i>All right, you corrupted beast!  Who are you and what are you doing here?  Your presence is not welcome in this sanctuary... answer quickly or I\'ll drop you to your death!' );
		}
		//Demon Score=3>:
		if( CoC.player.demonScore() >= 3 ) {
			MainView.outputText( '  Filthy demon!  Come to finish your work, have you?!  You won\'t take me without a fight!  Maybe if you beg for your life I\'ll give you a chance. Speak!  Convince me to not simply kill you here and now!' );
		}
		MainView.outputText( '</i>" the creature demands, clearly threatening to drop you if you don\'t answer to her liking.' );
		//Goo:
		if( CoC.player.isGoo() ) {
			...
			if( CoC.player.cor >= 75 ) {
				MainView.outputText( '... "<i>yet</i>", you think to yourself.' );
			}
		...
		if( CoC.player.cor > 66 ) {
			MainView.outputText( '  She is wise not to fully trust you.' );
		}
		MainView.outputText( '\n\nWith' );
		if( CoC.player.isGoo() ) {
		...
		MainView.outputText( ' relieved sigh, you take in the sight of the strange shark... harpy... harpy-shark... sharpy?  What an odd combination!  Looking the woman up and down, you take in her whole appearance.' );
		MainView.outputText( '\n\nShe is quite tall, roughly eight and a half feet by your estimate, covered in navy blue skin that looks like that of a shark girl, and with fiery red-orange hair that frames her feminine face and hangs down in long braids.  The harpy traits are just as obvious large pair of bird-like wings extends from her back, their feathers sharing the same burning hue as her hair.  Those same feathers also run down her arms like warm, feathered sleeves, while a long shark tail drapes down from her lower back, completing her unusual appearance.' );
	...
	MinervaScene.prototype.repeatEncounterMinerva = function() {
		...
		if( !this.minervaRomanced() ) {
			MainView.outputText( '  "<i>Ah, [name], you have returned!  What can I do for you?</i>"' );
		} else {
	...
	MinervaScene.prototype.minervaThirdPlusEncounter = function() {
		...
		//No romance:
		if( !this.minervaRomanced() ) {
			MainView.outputText( '  "<i>Welcome back, [name]!  It\'s good to see you again.  Have you come to give me some company, or would you like to partake in some of the fruit?  Or are you maybe here to sate your thirst?</i>"' );
		} else {
	...
	MinervaScene.prototype.goForASkyRideWithMinervaHeartBaring = function() {
		...
		//Goo:
		if( CoC.player.isGoo() ) {
			MainView.outputText( ' while you\'re barely able to maintain your slimy cohesion' );
		}
	...
	//[Reject]
	MinervaScene.prototype.rejectMinervasLove = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 95 );
		MainView.outputText( 'You stare at Minerva awkwardly as she waits for your response, her electric blue eyes looking at you with hope.  ' );
		if( CoC.player.cor < 45 ) {
			MainView.outputText( 'The look in the siren\'s eyes only makes this harder for you as you look into them; you can\'t help but feel bad that you really don\'t feel anything special for her.' );
		} else {
			MainView.outputText( 'You can tell she really wants this - the siren is almost desperate for your attention.  Unfortunately for the love starved herm, she won\'t find the love she craves with you.' );
		}
		MainView.outputText( '  You look away from her and pull back a bit before speaking.  You confess to her that, while she is a nice person, you do not feel that sort of bond with her, and you apologize if she was misled by your actions, but that you would like to be friends with her still.' );
	...
	MinervaScene.prototype.talkingToMinervaAboutBackstory = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 95 );
		//PART 1
		if( CoC.flags[ kFLAGS.MINERVA_BACKSTORY ] === 0 ) {
		...
		//PART 3
			...
			//No Romance:
			if( !this.minervaRomanced() ) {
				MainView.outputText( '  "<i>You\'re a good friend to have, and you know that, [name].</i>"' );
			}//Romance:
			else {
				MainView.outputText( '  "<i>You\'re a wonderful lover, you know.  I couldn\'t be happier that you found me.</i>"' );
			}
			// PC returns to camp.
	...
	//3-2 Talk Scene 2 - talks about the spring
	//-repeatable
	MinervaScene.prototype.talkAboutTheSpringWithMinerva = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 95 );
	...
	//3-4 Talk Scene 4 - talks about her corruption
	//- romance yes only or if you trick her into thinking you love her. - Repeatable until Minerva is purified or corrupted
	MinervaScene.prototype.talkToMinervaAboutHerCorruption = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 95 );
		MainView.outputText( 'Telling Mineva that you just want to talk brings a smile to her face, the siren eager to speak with you.  Guiding you over to a soft spot of moss by the spring, she sits next to you and looks into the pond, her smile slowly fading as she looks into the clear waters.' );
		...
		if( CoC.flags[ kFLAGS.MINERVA_BACKSTORY_LEARNED ] === 1 ) {
			MainView.outputText( '  You remember my firstborn, the shark girl I had?  Well... not long after she reached maturity, she... well, she got a bit amorous...  She seduced me and I... I mated with her.  I mated with her over and over that night.  I enjoyed it... it was like I couldn\'t get enough...  I probably got her pregnant...  So shameful...  I impregnated my own daughter and made her bear my grandchild,' );
		}
		MainView.outputText( '</i>" she declares sourly as she stares into the pure waters of the spring.' );
	...
	//3-5 Motherhood
	//-talks about how she wishes to be a real mother, have an actual loving family and not made from being raped- repeatable
	MinervaScene.prototype.minervaMotherhood = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 95 );
		...
		if( CoC.player.gender === 1 ) {
			MainView.outputText( ' and those goblins' );
		}//PC vagina:
		else if( CoC.player.gender === 2 ) {
			MainView.outputText( ' and that bee-girl' );
		}
		//PC herm:
		if( CoC.player.gender === 3 ) {
			MainView.outputText( ', goblins and that bee-girl' );
		}
		MainView.outputText( ', if the stories were real, I\'m sure you gave them plenty of children.  I wish to one day have kids... to have a great, loving family to call my own.</i>"' );
		MainView.outputText( '\n\nShaking your head, you grin and shuffle over to Minerva.  Sitting down with her, you' );
		//Romance:
		if( this.minervaRomanced() ) {
			MainView.outputText( ' hug her tightly and kiss her cheek, telling her that she will have children one day, and perhaps you will have to see about helping her with that \'problem.\'' );
		}//No romance:
		else {
			MainView.outputText( ' place a hand on her shoulder and give her a comforting smile before telling her that she will have that one day - you\'re sure of it.' );
		}
		MainView.outputText( ' Hearing your words brings a smile to her black glossy lips.  "<i>Thank you so much; that means a lot to me, really,</i>" she coos, her hands gingerly stroking yours before pulling away.  "<i>But... I guess we have been here for a while.  I\'m sure you have a great deal of work to do as such a brave champion.</i>"' );
	...
	MinervaScene.prototype.minervaSexMenu = function( display ) {
		if( display === undefined || display ) {
			MainView.clearOutput();
			MainView.spriteSelect( 95 );
			MainView.outputText( 'You tell Minerva that, this time, you\'d like to have sex with her.' );
			//Default, first time:
			if( CoC.flags[ kFLAGS.TIMES_MINERVA_SEXED ] === 0 ) {
				...
				//PC cock, add:
				if( CoC.player.hasCock() ) {
					MainView.outputText( ' just... not in my vagina, all right?  That\'s... special.' );
				}
				MainView.outputText( '  Did you have anything specific in mind?' );
				//PC female, add: '
				if( CoC.player.hasVagina() ) {
					MainView.outputText( '  Would you mind how we do it?  You\'re the one asking, so... would you want me to fuck your ass?  Or... do you want me to try to fit between those sexy hips of yours?' );
				}
			...
			//Dick Too Big - PC exceed capacity
				...
				if( CoC.player.hasVagina() ) {
					MainView.outputText( '  Since you don\'t seem to mind... would you like me to fuck your ass or to squeeze into your cute cunny?' );
				}
				MainView.outputText( '</i>"' );
			}
			//Romanced:
				...
				//PC cock, add:
				if( CoC.player.hasCock() ) {
					MainView.outputText( '  Since we\'re together, I\'ll even let you have my pussy, I bet you like the sound of that!' );
				}
				//PC female, add:
				if( CoC.player.hasVagina() ) {
					MainView.outputText( '  I\'d be happy to pound that cute butt of yours and stuff your hot pussy - which would you rather have, my dear?' );
				}
				MainView.outputText( '</i>"' );
			}
		}
	...
	MinervaScene.prototype.fuckMinervasAsshole = function() {
		...
		if( CoC.player.hasVagina() ) {
			MainView.outputText( ' and your [vagina]' );
		}
		MainView.outputText( ' to the open air.' );
		...
		MainView.outputText( ' to erectness, and you begin to teasingly drag [oneCock] through the crack of her curvy, spankable ass, hesitating to actually penetrate her until you\'re ready.  Your vexation brings a brighter blush to her cheeks, her tail squirming gently against you as she takes the hint.  Her hips start pushing back and forth, using her soft rounded rump to massage [eachCock], wanting to get you just as needy as she is.' );
	...
	MinervaScene.prototype.minervaCowgirlSex = function() {
		...
		CoC.flags[ kFLAGS.TIMES_MINERVA_SEXED ]++;
		MainView.outputText( 'You look the attractive siren up and down and' );
		if( CoC.player.tallness <= 60 ) {
			MainView.outputText( ', with some trepidation, given the difference in size,' );
		}
		MainView.outputText( ' ask if Minerva would like to give her female parts some pleasure, promising to allow her control of the pace.' );
		//First time:
		if( CoC.flags[ kFLAGS.TIMES_MINERVA_COWGIRLED ] === 0 ) {
			MainView.outputText( '  Minerva looks unsure, the siren biting her lip and looking a little away from you, "<i>I don\'t know.  That\'s... special...  After the things I have been through, I don\'t want to just do it with anyone.  You... </i>have<i> been really nice to me though... I guess I can trust you.</i>"' );
		}
		//Repeated:
		else {
			MainView.outputText( '  Minerva runs a hand through her hair and smiles, "<i>Well... the last time was quite nice, I think it could be fun.  Sure, let\'s do it.</i>"' );
		}
		...
		if( CoC.player.cumQ() >= 500 ) {
			...
			if( CoC.player.balls > 0 ) {
				MainView.outputText( ' your balls and' );
			}
			MainView.outputText( ' the base of your ' + Descriptors.cockDescript( x ) + ', stinging you again and again like an overzealous jellyfish, pumping more of her aphrodisiac venom into your genitals in mind-numbing quantities.' );
			if( CoC.player.hasVagina() ) {
				MainView.outputText( '  A few even slither down into your [vagina] to sting you there as well, making sure you are, at least temporarily, turned into a sex-hungry beast.' );
			}
		}
		...
		if( CoC.player.cumQ() < 500 ) {
			MainView.outputText( ', Minerva\'s belly visibly bloated by the time you are finished' );
		}
		MainView.outputText( '.' );
		if( CoC.player.cumQ() < 500 ) {
		...
		if( CoC.player.cumQ() >= 500 && CoC.player.cumQ() < 4000 ) {
			...
			if( this.minervaRomanced() ) {
				MainView.outputText( ' with your child' );
			}
			MainView.outputText( '.' );
		}
		//Very High:
		if( CoC.player.cumQ() >= 4000 ) {
			...
			if( !this.minervaRomanced() ) {
				MainView.outputText( '  "<i>Oohhh...I\'m so... so big... It\'s almost like you\'ve gotten me pregnant.</i>"' );
			} else {
				MainView.outputText( '  "<i>Oohhh... I\'m so... so big... cumming so much, I didn\'t know you were so desperate to get me pregnant.</i>"' );
			}
		}
		//Low-to-moderate:
		if( CoC.player.cumQ() >= 500 ) {
	...
	MinervaScene.prototype.minervaLapSex = function() {
		...
		//PC herm:
		if( CoC.player.gender === 3 ) {
			MainView.outputText( '  One of her hands trail down to your hard herm ' + Descriptors.cockDescript( x ) + ', stroking you softly.' );
		}
		...
		//PC herm:
		if( CoC.player.hasCock() ) {
			MainView.outputText( ', your own throbbing phallus' );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 'es' );
			}
			MainView.outputText( ' cumming alongside,' );
			//{Low-moderate cum:
			if( CoC.player.cumQ() < 500 ) {
				MainView.outputText( ' gushing your cum all over both of your bellies' );
			}//High cum volume:
			else if( CoC.player.cumQ() < 4000 ) {
				MainView.outputText( ' splattering thick splooge all over you and Minerva, caking your torsos in fertile jizz' );
			}//Very high cum:
			else {
				MainView.outputText( ' exploding out onto your unsuspecting bodies, thick ropes of starkly white sperm gushing over your bodies, quickly dousing both of you in buckets of cum and pooling on the ground' );
			}
		}
		MainView.outputText( '.' );
	...
	MinervaScene.prototype.goDownOnAHermAndLoveItYouDirtySlutYou = function() {
		...
		if( CoC.player.tallness < 96 ) {
			MainView.outputText( 'your fingers not even able to fit around the herm\'s hefty girth' );
		} else {
			MainView.outputText( 'your fingers barely wrapping around the meaty member, the fat thing throbbing gently' );
		}
		MainView.outputText( ' as you stroke it slowly.  Your touches are clearly adding to your partner\'s pleasure, her thick masculinity pulsating under your grip, her tentacles reacting to your presence, the dexterous things curling around your hands and fingers, touching you gently, but not stinging you.' );
		...
		if( CoC.player.tongueType === AppearanceDefs.TONUGE_DEMONIC || CoC.player.tongueType === AppearanceDefs.TONUGE_DRACONIC ) {
			MainView.outputText( 'Your deliciously long tongue slithers deeper and deeper, reaching sensitive walls and nerves that only the deepest penetrations would reach.  The deep licking makes Minerva shudder and let out a long drawn out moan, "<i>O-oh!  Your tongue is so... licking me so deep...</i>"' );
		}
		MainView.outputText( '  The intimate vaginal kiss is working wonders for the siren.  Her sweet, slick juices leak over your tongue, giving you plenty of the peachy flavor that you crave.  As you tongue your lover\'s pussy and tease her clit, your hands work her thick, pulsating cock, pre-cum slowly coating your hands and making it easy for you to squeeze tighter and stroke her faster.' );
	...
	MinervaScene.prototype.noBallsMinervaAndContinuation = function( clear ) {
		...
		MainView.spriteSelect( 95 );
		MainView.outputText( 'Your careful ministrations soon have the poor siren shuddering and squirming, looking about ready to pop from your skilled hands and mouth.' );
		if( clear !== undefined && !clear ) {
			MainView.outputText( '  Retreating from her throbbing balls, you' );
		} else {
			MainView.outputText( '  You' );
		}
		MainView.outputText( ' move back up to her now soaking snatch, your tongue eagerly lapping up all the tasty juices from her lips before delving deep into her refreshingly cool cunt, hungrily licking up every slick drop of her succulent sweetness.  The act of worming your tongue back inside her hungry hole again drives Minerva ever closer to the brink of her climax, each slurp and stroke of your tongue and hands pushing the near-climactic herm right to the edge and sending her off into the blissful, pleasure-filled abyss of a mind-blowing double orgasm.' );
		...
		if( CoC.player.tongueType === AppearanceDefs.TONUGE_DEMONIC ) {
			MainView.outputText( ', your perversely prehensile tongue slowly retracting from her quivering quim, rubbing and teasing as it comes out and nearly sending the overly sensitive herm into another climax' );
		}
		MainView.outputText( '.' );
	...
	MinervaScene.prototype.letMinervaSuckYouOff = function() {
		...
		if( CoC.player.isNaga() ) {
			MainView.outputText( ' lounge against the rock, spreading open your protective slit and letting your ' + Descriptors.cockDescript( x ) + ' hang free.' );
		} else {
			MainView.outputText( ' sit down, spreading your [legs] and opening up the crotch of your [armor] to the air, letting your ' + Descriptors.cockDescript( x ) + ' hang free.' );
		}
		...
		//If balls:
		if( CoC.player.balls > 0 ) {
			MainView.outputText( '  Her hand on your hip moves down between your legs, sliding inwards to gently caress and massage your cum-filled [balls] to increase your pleasure even more, her hands trying their best to get your nuts worked up and prepare a good load for her at the end.' );
		}
		...
		//If balls:
		if( CoC.player.balls > 0 ) {
			MainView.outputText( ' before licking and teasing your [balls]' );
		}
		MainView.outputText( '.' );
		...
		if( CoC.player.cockArea( x ) >= 50 ) {
			MainView.outputText( ', your huge cock deep inside her mouth, and soon far down her throat.  With some difficulty, her slightly angular nose presses down against your crotch, every inch of your ' + Descriptors.cockDescript( x ) + ' trapped inside her cool, wet mouth and throat' );
		}
		MainView.outputText( '.' );
		...
		if( CoC.player.balls > 0 ) {
			MainView.outputText( ', while your [balls] lurch and swell, rising up as you reach your climactic explosion' );
		}
		MainView.outputText( '.' );
	...
	MinervaScene.prototype.fuckMinervaWithHerHandsBehindHerBack = function() {
		...
		//If balls:
		if( CoC.player.balls > 0 ) {
			MainView.outputText( '  As you continue to pound the siren, your [balls] swing between your legs, the sperm-filled orbs churning with your fertility, preparing to fill the needy siren with every drop you can muster when you let loose your eventual orgasm.' );
		}
		...
		//Holy shit!:
			...
			if( CoC.player.balls > 0 ) {
				MainView.outputText( '  Minerva groans and lets out mindless gurgles as you unload your overproductive balls into her.' );
			}
			MainView.outputText( '  To even your amazement, her hungry womb is able to contain much of your sperm before she can take no more, each blast of potent cum now gushing from between your bodies, the floor soon looking like someone dumped a barrel full of raw milk on the ground before you\'re finally done.' );
	...
	MinervaScene.prototype.pcGetsEatenOutByMinerva = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 95 );
		MainView.outputText( 'Thinking about just what kind of heated, heart-pounding, lusty fun you want to have with your siren lover, your eyes look the shark-like harpy up and down, taking in her features.  Starting with her long legs, and moving up to her wide, flared hips, your eyes travel past the thick bulge in her shorts, going up her scarred belly to the siren\'s perky DD-cup breasts before finally reaching her face.' );
		...
		if( CoC.player.hasCock() ) {
			MainView.outputText( ', and, perhaps, your more male parts while she\'s down there' );
		}
		MainView.outputText( '.' );
		...
		if( CoC.player.balls > 0 ) {
			MainView.outputText( ', one of her hands lifting your [balls] up before giving each of the fertile nuts a kiss' );
		}
		MainView.outputText( ' and moving down to your sweet cunt, just waiting for her attentions.' );
		...
		//If dick:
		if( CoC.player.hasVagina() ) {
			MainView.outputText( 'Thanks to her handy stimulation, your musky maleness throbs and dilates along your femininity, the thick cockvein feeding your bloating member as your seed rushes out, splattering up and onto Minerva\'s back, defiling her smooth skin with your fertility.  ' );
		}
		MainView.outputText( 'Her mouth is closed over your hot quaking cunt, catching every drop and sucking hard on you as she slowly retracts her perversely prehensile tongue.  Minerva\'s mind wanders as she muses your unique flavors, her tongue still squirming inside you, making sure to lick up as much of your feminine juices as she can before finally pulling out from your body and letting go of you.' );
	...
	MinervaScene.prototype.beatUpDatSharpie = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 95 );
		//Win by damage
		if( CoC.monster.HP < 1 ) {
	...
	MinervaScene.prototype.eatSomethingYouCunt = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 95 );
		//Acquiring:
		MainView.outputText( 'You walk over to the fruit trees surrounding the spring, examining the strange treats.  You decide that one ripe fruit, one resembling a peach, is the best choice, and pluck it from the tree.  Thanking Minerva for letting you have it, you stow it away safely and head back to camp.\n\n' );
		SceneLib.inventory.takeItem( ConsumableLib.PURPEAC, SceneLib.camp.returnToCampUseOneHour );
	};
	/*
	 Calming Mint
	 Acquiring are many strange herbs growing around the spring, fed by whatever power resides in the water.  Finally, you locate a sprig of something that resembles mint, but silver in color, and decide to pluck it.  Stowing it carefully amongst your belongings, you thank Minerva for sharing the contents of her 'garden' with you and then head back to camp.
	 C. Mint - Inventory
	 ...
	 */
	//Spring Water
	MinervaScene.prototype.getADrinkYouBitch = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 95 );
		MainView.outputText( 'You ask Minerva if she\'s okay with allowing you to drink from her spring.  She replies with a nod, a smile and a casual wave towards the pool.  Making it quite clear that you can drink your fill.' );
		//[Bottle] [Drink]
		//[Drink]
	 ...
	 */
	SceneLib.registerScene( 'minervaScene', new MinervaScene() );
} );