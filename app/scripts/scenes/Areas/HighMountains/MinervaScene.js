...
angular.module( 'cocjs' ).run( function( EngineCore, CoC, kFLAGS, Combat, Utils, AppearanceDefs, StatusAffects, Descriptors, ConsumableLib, EventParser, Minerva ) {
	...
	MinervaScene.prototype.firstMinervaEncounter = function() {
		...
		//If Sophie Bimbo=0:
		if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00282 ] === 0 && CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00283 ] === 0 && !CoC.getInstance().scenes.sophieFollowerScene.sophieFollower() ) {
	...
	MinervaScene.prototype.takeMinervasPath = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 95 );
		EngineCore.outputText( 'Knowing that a new path could lead to great things, you can\'t help but wonder where this one could lead.' );
		...
		EngineCore.outputText( ' relieved sigh, you take in the sight of the strange shark... harpy... harpy-shark... sharpy?  What an odd combination!  Looking the woman up and down, you take in her whole appearance.' );
	...
	MinervaScene.prototype.repeatEncounterMinerva = function() {
		...
		if( !this.minervaRomanced() ) {
			EngineCore.outputText( '  "<i>Ah, [name], you have returned!  What can I do for you?</i>"' );
		} else {
			EngineCore.outputText( '  "<i>Mmm... my lover has come to see me!  Oh, how I have missed you, my sweet!  Won\'t you join me for a bath?  Or... is there anything else I can do for you?</i>"' );
		}
		//[Talk] [Sex] [Eat] [Fight] [Leave]
		EngineCore.menu();
	...
	MinervaScene.prototype.minervaThirdPlusEncounter = function() {
		...
		//No romance:
		if( !this.minervaRomanced() ) {
			EngineCore.outputText( '  "<i>Welcome back, [name]!  It\'s good to see you again.  Have you come to give me some company, or would you like to partake in some of the fruit?  Or are you maybe here to sate your thirst?</i>"' );
		} else {
			EngineCore.outputText( '  "<i>Darling!  Coming to visit me all the way out here!  How sweet of you!</i>"' );
		}
	...
	MinervaScene.prototype.rejectMinervasLove = function() {
		...
		EngineCore.outputText( '  You look away from her and pull back a bit before speaking.  You confess to her that, while she is a nice person, you do not feel that sort of bond with her, and you apologize if she was misled by your actions, but that you would like to be friends with her still.' );
	...
	MinervaScene.prototype.talkingToMinervaAboutBackstory = function() {
		...
		//PART 3
			...
			//No Romance:
			if( !this.minervaRomanced() ) {
				EngineCore.outputText( '  "<i>You\'re a good friend to have, and you know that, [name].</i>"' );
			}//Romance:
			else {
	...
	MinervaScene.prototype.talkToMinervaAboutHerCorruption = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 95 );
		EngineCore.outputText( 'Telling Mineva that you just want to talk brings a smile to her face, the siren eager to speak with you.  Guiding you over to a soft spot of moss by the spring, she sits next to you and looks into the pond, her smile slowly fading as she looks into the clear waters.' );
	...
	MinervaScene.prototype.minervaMotherhood = function() {
		...
		//Romance:
		if( this.minervaRomanced() ) {
			EngineCore.outputText( ' hug her tightly and kiss her cheek, telling her that she will have children one day, and perhaps you will have to see about helping her with that \'problem.\'' );
		}//No romance:
		else {
			EngineCore.outputText( ' place a hand on her shoulder and give her a comforting smile before telling her that she will have that one day - you\'re sure of it.' );
		}
	...
	MinervaScene.prototype.minervaCowgirlSex = function() {
		...
		//Repeated:
			...
			EngineCore.outputText( '  Minerva runs a hand through her hair and smiles, "<i>Well... the last time was quite nice, I think it could be fun.  Sure, let\'s do it.</i>"' );
		}
		...
		if( CoC.getInstance().player.cumQ() >= 500 && CoC.getInstance().player.cumQ() < 4000 ) {
			...
			if( this.minervaRomanced() ) {
				EngineCore.outputText( ' with your child' );
			}
			EngineCore.outputText( '.' );
		}
		//Very High:
		if( CoC.getInstance().player.cumQ() >= 4000 ) {
			...
			if( !this.minervaRomanced() ) {
				EngineCore.outputText( '  "<i>Oohhh...I\'m so... so big... It\'s almost like you\'ve gotten me pregnant.</i>"' );
			} else {
				EngineCore.outputText( '  "<i>Oohhh... I\'m so... so big... cumming so much, I didn\'t know you were so desperate to get me pregnant.</i>"' );
			}
		}
		//Low-to-moderate:
		if( CoC.getInstance().player.cumQ() >= 500 ) {
	...
	MinervaScene.prototype.minervaLapSex = function() {
		...
		if( CoC.getInstance().player.hasCock() ) {
			...
			if( CoC.getInstance().player.cumQ() < 500 ) {
				...
				EngineCore.outputText( ' exploding out onto your unsuspecting bodies, thick ropes of starkly white sperm gushing over your bodies, quickly dousing both of you in buckets of cum and pooling on the ground' );
			}
		}
		EngineCore.outputText( '.' );
	...
	MinervaScene.prototype.noBallsMinervaAndContinuation = function( clear ) {
		...
		EngineCore.spriteSelect( 95 );
		EngineCore.outputText( 'Your careful ministrations soon have the poor siren shuddering and squirming, looking about ready to pop from your skilled hands and mouth.' );
		if( clear !== undefined && !clear ) {
			EngineCore.outputText( '  Retreating from her throbbing balls, you' );
		} else {
			EngineCore.outputText( '  You' );
		}
		EngineCore.outputText( ' move back up to her now soaking snatch, your tongue eagerly lapping up all the tasty juices from her lips before delving deep into her refreshingly cool cunt, hungrily licking up every slick drop of her succulent sweetness.  The act of worming your tongue back inside her hungry hole again drives Minerva ever closer to the brink of her climax, each slurp and stroke of your tongue and hands pushing the near-climactic herm right to the edge and sending her off into the blissful, pleasure-filled abyss of a mind-blowing double orgasm.' );
	...
	MinervaScene.prototype.getADrinkYouBitch = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 95 );
		EngineCore.outputText( 'You ask Minerva if she\'s okay with allowing you to drink from her spring.  She replies with a nod, a smile and a casual wave towards the pool.  Making it quite clear that you can drink your fill.' );
		//[Bottle] [Drink]
		//[Drink]
	 ...
	 */
	CoC.getInstance().registerScene( 'minervaScene', new MinervaScene() );
} );