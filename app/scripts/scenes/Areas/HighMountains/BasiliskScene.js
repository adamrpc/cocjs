...
angular.module( 'cocjs' ).run( function( SceneLib, EngineCore, CoC, kFLAGS, Basilisk, Combat, Descriptors, StatusAffects, PerkLib, PregnancyStore, Utils, AppearanceDefs, MainView ) {
	...
	BasiliskScene.prototype.defeatBasilisk = function() {
		...
		//Player HP victory:
		if( CoC.monster.HP < 1 ) {
			MainView.outputText( 'Unable to stand anymore, the basilisk shakily sinks down on one knee, drops its head and looks at the ground, evidently demonstrating submission.', false );
		}//Player Lust victory:
		else {
			MainView.outputText( 'No longer able to control its raging erection, the basilisk closes its eyes and sinks to one knee.  It would probably be attempting to signal its submission to you if it weren\'t furiously masturbating its long, purple cock, which has emerged straining from the creature\'s genital slit.', false );
		}
	...
	BasiliskScene.prototype.defaultBasiliskRape = function() {
		MainView.outputText( 'Working briskly, the basilisk tears off your ' + CoC.player.armorName + ' until you are entirely naked.  It then rummages through your pockets; it carelessly discards everything it finds without apparent interest.  It grabs a handful of gems from your purse and then prowls back to you.\n\n', false );
		//Male/Herm:
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'With surprising gentleness and deftness, the basilisk rubs your ' + Descriptors.cockDescript( 0 ) + ' with one palm', false );
			if( CoC.player.hasVagina() ) {
				MainView.outputText( ' and sticks some of the smaller fingers of its other hand in your ' + Descriptors.vaginaDescript( 0 ), false );
			}
			MainView.outputText( ', thankfully angling its sickle claw away.  You can\'t do anything against it, and some of its mental compulsion remains; a backwash of erotic images from your past fill your head, and you can\'t even grit your teeth as the gentle, insistent pressure brushing your prick makes you rock hard.  It stops when you are erect and then, with the very faintest of smiles playing over its cruel mouth, leaves. You\'re naked, your ' + Descriptors.cockDescript( 0 ) + ' is begging for release; you\'re utterly helpless... you can only hope that the spell will wear off, and before anything else in the mountain finds you.\n\n', false );
		}
		//Female:
		else if( CoC.player.hasVagina() ) {
			MainView.outputText( 'With surprising gentleness and deftness, the basilisk slips the smaller fingers of one hand into your ' + Descriptors.vaginaDescript( 0 ) + ', and carefully flicks at your ' + Descriptors.clitDescript() + ' with the other, thankfully holding its sickle claws away from you.  You can\'t do anything against it, and some of its mental compulsion remains; a backwash of erotic images from your past fill your head, and you can\'t even grit your teeth as the gentle, insistent caresses make you wet.  It stops when you are beading moisture involuntarily onto its hand and then, with the very faintest of smiles playing over its cruel mouth, leaves. You\'re naked, your ' + Descriptors.vaginaDescript( 0 ) + ' begs to be filled; you\'re utterly helpless. You can only hope that its spell will wear off, and before anything else in the mountain finds you...\n\n', false );
		}
		//Genderless:
		else {
		...
		//More to go here?
		var scene = Utils.rand( 5 );
		if( scene === 0 ) {
			this.basiliskAdvantageNobody();
		} else if( scene === 1 ) {
			this.basiliskAdvantageHarpy();
		} else if( scene === 2 ) {
			this.basiliskAdvantageImp();
		} else if( scene === 3 ) {
			this.basiliskAdvantageGoblin();
		} else {
	...
	BasiliskScene.prototype.basiliskHasVagEggStuff = function() {
		...
		if( CoC.player.inHeat ) {
			MainView.outputText( '  You are more aware than ever of an invisible scent simmering off you, of your wet vagina clenching and wetting itself in anticipation, your body begging this male creature to fulfil its genetic objective upon you.  Your eyes have betrayed you, your body is betraying you, and whatever else you are is a tiny, ignored voice screaming in between.', false );
		}
		MainView.outputText( '  The basilisk suddenly breaks away and kneels down in front of you.  Out of sight of your petrified eyes you cannot see what it is doing; however a moment later, you can feel, as a warm, sticky sensation slavers over your abdomen.', false );
		//(egg preg:
	...
	BasiliskScene.prototype.basiliskBirth = function() {
		...
		if( CoC.player.findPerk( PerkLib.BasiliskWomb ) ) {
			...
			MainView.outputText( '.  You place the egg back down and gather them all up, moving them closer to the campfire to stay warm while you recover from your exertions.' );
	...
	BasiliskScene.prototype.basiliskAdvantageHarpy = function() {
		...
		//Genderless:
			...
			MainView.outputText( 'You feel something build at the back of your mind.  Involuntary sensations prickle your skin as, once again, the intermittent mental backwash that the basilisk\'s hypnosis has forced upon your mind hits you.  Memory after memory of sexually-charged encounters, daydreams or fantasies crowd your consciousness.  It is made worse, much worse by the pheromones the harpy has pushed into your mouth; your body rides the chemical glow at the same time as imaginings of soft skin, tight muscle and musk overload your senses.  The denial the petrification has forced upon you is becoming unbearable.\n\n', false );
			...
			MainView.outputText( 'The harpy manages to cum twice on your hand, gobbling with excitement as she spatters your arm with her juices.  During this time you are forced to ride the potent cocktail of hypnotic sexual compulsion and the harpy\'s golden lipstick again and again, until you feel you would have collapsed in a pool of steaming sex long ago if your knees would only allow it.  Once she is finished with you the harpy clambers down, taking care to wipe her leaking twat on your naked front as she does so, before flapping off with a winsome smirk, entirely ignoring your own plight.\n\n', false );
			MainView.outputText( 'After another ten or twenty minutes of being forced to stand still and savor your own shameful memories, you find with great relief you can begin to move your fingers again.  Eventually with some effort you manage to work power into each corner of your body and finally shake free of the basilisk\'s curse; quickly, you redress before anything else finds you and, still reeking of harpy sex, you begin to make your way back down the mountain.  You think woozily that maybe you should consider yourself lucky that nothing actually fucked you whilst you were in your helpless state, but your body thinks the exact opposite, and you really, really need to get back to camp and sort yourself out.', false );
			EngineCore.dynStats( 'lus=', 100 );
		}
	};
	...
	BasiliskScene.prototype.basiliskAdvantageGoblin = function() {
		...
		//Unsexed:
			...
			MainView.outputText( '"<i>Hey you!  Yeah you, shit-for-brains!  Wanna free assfuck?  Come and get it then, you ugly, dumb muscle-bound moron!</i>"  A moment later you feel a slap on your calf as the goblin runs past you, giggling.  "<i>Have fun, smartass!</i>"  A huge, angry bellow vibrates the air around you and the earth shakes as something big approaches you from behind.  You strain with every sinew of your being to escape, but you are, as ever, glued in place.  You manage a raspy moan as the enraged minotaur grasps you roughly by both arms, his animal musk filling your nostrils. <i>Fucking goblins.</i>\n\n', false );
			...
			MainView.outputText( '\n\n', false );
			MainView.outputText( 'Eventually, after another thirty or so minutes of being forced to stand there and savor the cum trickling down your legs, you find with great relief you can begin to move your fingers again. With some effort you manage to work power into each corner of your body and finally shake free of the basilisk\'s curse; quickly, you shake the aching out of your ' + CoC.player.legs() + ' and re-dress before anything else finds you and woozily begin to make your way back down the mountain, trying to ignore the feeling of ooze dripping out of you.', false );
			CoC.player.orgasm();
			CoC.player.slimeFeed();
		}
		//Female:
	...
	BasiliskScene.prototype.basiliskAdvantageMinotaur = function() {
		...
		MainView.outputText( '  You begin to pant from exertion and the overpowering sensation of the fuck as you feel first one ring and then a second push past your sphincter, then out again, then in and out as the minotaur picks up the pace.  As rut takes hold of him, he picks you up by your arms and uses his strength to force you up and down his dick, using you as a cocksleeve to sate his animalistic desires.  Your ass is slick with his pre-cum by now however, and the drugged slime has bumped you upwards into a hazy high; only the height of his downward thrust when his dick is almost completely buried in your bowels brings you out of it, the painful intensity of it dragging you down of your cloud with a gasp.\n\n', false );
		MainView.outputText( 'Eventually the minotaur\'s balls swell against your ' + Descriptors.buttDescript() + ' and with a long, satisfied moo, it reaches its peak, holding you down so its cum jets forth deep inside you.  ', false );
		if( CoC.player.gender === 1 ) {
			MainView.outputText( 'The sensual high it has already instilled in you amplifies by tenfold; the warm, oozing fluid finds its mark and you moan as you spontaneously ejaculate, ropes of your own cum spattering on the ground and your own nerveless ' + CoC.player.feet() + '.', false );
		} else if( CoC.player.gender === 2 ) {
			MainView.outputText( 'The sensual high it has already instilled in you amplifies by tenfold; the warm, oozing fluid finds its mark and you moan as you spontaneously orgasm, your juices spattering your thighs and calves.', false );
		} else if( CoC.player.gender === 3 ) {
			MainView.outputText( 'The sensual high it has already instilled in you amplifies by tenfold; the warm, oozing fluid finds its mark and you moan as you spontaneously orgasm, ropes of your own jizz spattering on the ground and girl cum dripping down your thighs.', false );
		} else {
			MainView.outputText( 'The sensual high it has already instilled in you amplifies by tenfold as the warm, oozing fluid finds its mark and you moan as you spontaneously orgasm, your anus helplessly milking the creature for all it can get.', false );
		}
		MainView.outputText( '\n\n', false );
		MainView.outputText( 'The minotaur holds onto you until he has finished spurting his last into you, before abruptly setting you back down on your frozen feet, pulling his slimy, receding member out of your abused anus and with a satisfied snort, takes his leave.  You feel his cum drooling out of you and down your ' + Descriptors.hipDescript() + ', but in your hazy, druggy state the feeling is almost sensual.', false );
		//(Addict:
	...
	BasiliskScene.prototype.driderPCEggLaysBasilisk = function() {
		...
		if( CoC.monster.HP < 1 ) {
			MainView.outputText( 'exhaustion' );
		} else {
			MainView.outputText( 'arousal' );
		}
		MainView.outputText( ', the basilisk is still striving to meet your eyes with its own.  Clenching your teeth, you avert your eyes and stomp forward blindly, colliding roughly with the spined lizard, knocking him flat on his back on the rocky ground.' );
		...
		if( CoC.player.hasVagina() ) {
			MainView.outputText( 'already-dripping pussy' );
		}
		if( CoC.player.gender === 3 ) {
		...
		MainView.outputText( ' up against his half-open mouth, smearing your free-running juices across his scaled face.  You wait a few seconds, just enjoying the feeling of his hot breath against your oversensitive sex, before you state your intentions in an imperious tone.  "<i>Lick.</i>"' );
		...
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'wrap tightly around your waiting prick' );
		} else {
			MainView.outputText( 'burrow deep into your needy slit' );
		}
		MainView.outputText( '.  You coo in delight at the sensation, and begin to grind yourself against his face as you wait for your egg-laying to begin.' );
		...
		if( CoC.player.fertilizedEggs() === 0 ) {
			MainView.outputText( 'un' );
		}
		MainView.outputText( 'fertilized orbs, providing you with a moment\'s warning before the ecstasy-inducing ripple of an egg sliding deep into him rolls your eyes backwards in pleasure.  You twitch and squirm atop the bound basilisk, mashing your ' );
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'dick' );
		} else {
			MainView.outputText( 'cunt' );
		}
		MainView.outputText( ' against his face while your egg-laden tube pumps away, each pulse forcing another egg through his squeezing ring and sending trills of intoxicating euphoria up your spine.  You lose yourself in the act, your heartbeat racing in time with contractions of your ovipositor, spraying ' );
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'semen' );
		} else {
			MainView.outputText( 'femcum' );
		}
		MainView.outputText( ' across your partner\'s face and neck.' );
		...
		if( CoC.player.cor >= 50 ) {
			MainView.outputText( ' before heading off towards your camp. The harpies will probably free him from your webs... eventually.' );
		} else {
			MainView.outputText( '.  You reach up and slice him free from the webbing, carefully placing him on the ground below.  The blindfold stays, though.  You\'re kind, not stupid.  With your good deed for the day complete, you gather yourself up and head back to camp.' );
		}
		CoC.player.dumpEggs();
		CoC.player.orgasm();
		Combat.cleanupAfterCombat();
	...
	BasiliskScene.prototype.layBeeEggsInABasilisk = function() {
		...
		if( CoC.player.isTaur() || CoC.player.gender === 0 ) {
			...
			MainView.outputText( '; rubbing enthusiastically, you start at the base but end in a gentle stroke at the sensitive tip.  With you doing your best to milk as much pre as you can, the viscous ooze that dribbles out of your dong' );
			if( CoC.player.totalCocks() ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ' lands with soft pitters on the overgrown lizard\'s back, the intense mountain sun baking the gunk onto the scales.  You sigh in languor, but remember you\'re doing this for a reason.' );
		}
		//(else if pc has vag:
		else {
		...
		if( CoC.player.isTaur() ) {
			MainView.outputText( 'above' );
		} else {
			MainView.outputText( 'behind' );
		}
		MainView.outputText( ' the desperately-squirming lizard, delighting in the fear-scented air around the beast as your ovipositor slaps his cheeks in search of his tight little black hole.  Aside from the difference in feel, you know once you hit the sweet spot by the rigid jolt from the basilisk as his worst fear is confirmed.  You openly laugh this time, and enjoy the feel of the lizard squirming under you as your bulbous appendage painstakingly, slowly works its way inside his anus.  Dry and tight, you really have to force your way, but the feeling is amazing, and it just keeps getting better.  It probably doesn\'t feel too great for the basilisk, but that\'s not exactly your problem.  The slit at the end of your ovipositor widens, leaking golden liquid, and the squirming basilisk halts its struggles and relaxes as the narcotics in the slime work their way into the bloodstream.  You begin humming in a fashion similar to the giant bees you now resemble as you enjoy the blissful sensation of the eggs progressing through the long tubular organ, stretching the reptile\'s pucker to accommodate your ' );
		if( CoC.player.fertilizedEggs() ) {
			MainView.outputText( 'fertilized offspring' );
		} else {
			MainView.outputText( 'burdensome load' );
		}
		MainView.outputText( '.' );
		...
		if( CoC.player.isTaur() || CoC.player.gender === 0 ) {
			...
			if( CoC.player.gender === 0 ) {
				MainView.outputText( 'no needy cock or cunt demanding a caress - except the basilisk\'s, and you care not a bit about that one.' );
			} else if( CoC.player.isTaur() ) {
				MainView.outputText( 'even if you wanted to reach your genitals, your arms resolutely refuse to bend that way, so you focus on the slippery, silky feelings travelling along your ovipositor.' );
			}
		}
		if( CoC.player.hasCock() ) {
			MainView.outputText( '\n\nYour seeping member' );
