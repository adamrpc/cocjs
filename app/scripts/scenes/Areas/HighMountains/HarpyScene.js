...
angular.module( 'cocjs' ).run( function( SceneLib, MainView, EngineCore, CoC, kFLAGS, Descriptors, Utils, AppearanceDefs ) {
	...
	HarpyScene.prototype.harpyVictoryuuuuu = function() {
		...
		if( CoC.monster.HP < 1 ) {
			MainView.outputText( 'The harpy screams out in one last, pained cry before her wings give way, the feathered woman collapsing into a weary heap.', true );
		}//(Enemy defeated by lust)
		else {
			MainView.outputText( 'The harpy can\'t contain her lust anymore and crumples to the ground before you, on her knees with her plush, heavy ass resting on her feet. She coos pathetically, with one hand between her legs furiously fingering herself, and the other pressed against your crotch, a needy look in her eyes.', false );
		}
		...
		if( CoC.player.hasVagina() ) {
			if( CoC.player.isNaga() ) {
				MainView.outputText( '  If you weren\'t a naga, you could scissor her.' );
			} else {
	...
	HarpyScene.prototype.harpyGooGenderlessLoss = function() {
		MainView.outputText( '', true );
		MainView.outputText( 'The triumphant harpy looks down at your goopy form, ready to take you.  She seems a little confused though, and begins poking at your gelatinous body.\n\n', false );
		MainView.outputText( '"<i>Penis?</i>" she eventually asks.\n\n', false );
		MainView.outputText( 'You tell her no.\n\n', false );
		MainView.outputText( '"<i>Pussy?</i>"\n\n', false );
		MainView.outputText( 'Nope.\n\n', false );
		MainView.outputText( '"<i>Oh...</i>"\n\n', false );
		MainView.outputText( 'She grabs a handful of you and releases it, watching your body jiggle about. This seems to amuse her greatly and she plunges both hands inside you then shakes them about. Retracting them, she takes a few steps back and gives you an evil grin, then lets out a blood-curdling shriek.\n\n', false );
		MainView.outputText( 'Two more harpies flap down beside her and the trio huddles together, periodically pointing at you.  Most of what they\'re saying is inaudible to you, though you\'re fairly sure you heard \'goop\' somewhere in there.  You\'re not given much time to stew on it though, as they seem to agree on something and approach.\n\n', false );
		MainView.outputText( 'The smallest of the bunch walks right up to you and punches your chest, laughing and chittering as your body undulates in response.  The other two stick their arms inside you and begin shaking them about, also laughing.  This goes on for quite a while, with the bird women trying various positions and levels of force (including a particularly odd one in which two attempted to shove your entire body up the third\'s vagina; not exactly a situation you want to find yourself in again...). Eventually they get bored of you and flap off, leaving your exhausted body in a pile until you get enough energy to pull yourself together and head back.', false );
	};
	...
	HarpyScene.prototype.harpyDamageLoss = function() {
		...
		if( CoC.player.hasCock() && x >= 0 ) {
			...
			MainView.outputText( 'your ' + Descriptors.multiCockDescriptLight() + ' flopping from side to side from the ministrations, you see not one, but THREE deranged, randy bird-girls, all squabbling over your genitals.\n\n', false );
			//[(Male/shemale)
			if( !CoC.player.hasVagina() ) {
				MainView.outputText( 'The hierarchy is eventually settled, it seems, because two of those heavy-hipped bird babes wrap their lips around your ' + Descriptors.cockDescript( x ) + ', sucking and pecking at the hot flesh', false );
				if( CoC.player.balls > 0 ) {
					MainView.outputText( ', craning their necks to slurp and massage your ' + Descriptors.ballsDescriptLight() + ' eagerly, trying to coax up your cum production as much as they can', false );
				}
				MainView.outputText( '. The third, estranged harpy leaps at your torso, straddling your navel with her weighty rear, knocking the breath out of you and leaning down, ', false );
				//(If breasts)
				if( CoC.player.biggestTitSize() >= 1 ) {
					MainView.outputText( 'her petite bosom grinding across your ' + CoC.player.allBreastsDescript() + ', her nipples tickling at your ' + Descriptors.nippleDescript( 0 ) + ', ', false );
				}
				MainView.outputText( 'her golden lips glinting in the light as they make for your own.\n\n', false );
			}
			//[(Herm)
			else {
				MainView.outputText( 'The hierarchy is settled between the warring bird-girls, and two of them grasp onto your thighs with their clawed hands, those gold-lipped mouths starting to hungrily suckle and taste every inch of your ' + Descriptors.cockDescript( x ) + ', their squawking, feathered heads occasionally diving down to probe their short, stubby tongues into your ' + Descriptors.vaginaDescript( 0 ) + ', sending sharp spikes of pleasure rippling through your body, ', false );
				//(If balls)
				if( CoC.player.balls > 0 ) {
					MainView.outputText( 'their hands moving up to grope and fondle your ' + Descriptors.ballsDescriptLight() + ', massaging them to boost your cum production as much as they can, ', false );
				}
				MainView.outputText( 'tiny tingles left on your body at every spot their gilded lips caress. The third harpy dives at your chest, her thick hips SLAMMING into your stomach, knocking the breath out of you', false );
				//(If breasts)
				if( CoC.player.biggestTitSize() >= 2 ) {
					MainView.outputText( ', causing your ' + CoC.player.allBreastsDescript() + ' to bounce and sway violently', false );
				}
				MainView.outputText( '.  She slowly leans down, her eyes locked on yours, her lips glinting a malicious gold', false );
				//(if breasts)
				if( CoC.player.biggestTitSize() >= 2 ) {
					MainView.outputText( ' her toned chest and pert breasts squishing your ' + Descriptors.breastDescript( 0 ) + ' beneath them', false );
				}
				MainView.outputText( '.\n\n', false );
			}
			//Apply harpy status.
		...
		//[(Female)
			...
			MainView.outputText( 'The hierarchy is determined, and two of the harpies dive forward, burying their fat golden lips into your ' + Descriptors.vaginaDescript( 0 ) + ', their short, stubby tongues probing and exploring every inch of your feminine love-tunnel.  Together they circle and push at your ' + Descriptors.clitDescript() + ' sensuously, leaving a sharp tingle at every little point their plump golden lips brush against.  Growing more and more horny, each harpy probes one hand behind herself to toy with her drooling pussy, coaxing aroused squawks and moans from the group as they tend to you. The leading harpy makes a reckless dive for your chest, intent on claiming her prize before anyone else can take it from her. Careering down upon your stomach, her immense, wobbly ass pounds into you, knocking the breath out of your lungs', false );
			//(If breasts)
			if( CoC.player.biggestTitSize() >= 3 ) {
				MainView.outputText( ' and causing your ' + CoC.player.allBreastsDescript() + ' to jiggle, slapping into one another', false );
			}
			MainView.outputText( '. She leans slowly over you, her gilded lips glistening with saliva as they approached yours', false );
			//{(If breasts)
			if( CoC.player.biggestTitSize() >= 2 ) {
				MainView.outputText( ', her petite chest squashing your ' + Descriptors.breastDescript( 0 ) + ' against hers', false );
			}
			MainView.outputText( '.\n\n', false );
			MainView.outputText( 'When the harpy\'s lips press against yours, your vision starts to swim. She kisses you ferally, fuelled by a primeval lust.  Her short, sharp tongue slaps yours all over your mouth, tickling and scratching across your lips. As you are forced into the crushing, vicious kiss, you notice that the pleasure shooting up and down your body seems somehow sharp, and your lust is beginning to swell and surge around your mind.  Everywhere their lips have brushed on you feels tingly and pleasurable... those golden lips... they must be drugged! Even as you realize this, it is too late, and your tense muscles collapse in the overwhelming feel of it, your libido burning hot even as your body feels so weary.\n\n', false );
	...
	HarpyScene.prototype.victoryHarpyGetsHerPussyRaped = function() {
		...
		MainView.outputText( 'You slowly slip your ' + Descriptors.multiCockDescriptLight() + ' into her hot, clutching depths, inch by inch, seeming to take an eternity as all the sensations of her rough, ribbed pussy ', false );
		if( CoC.player.totalCocks() > 1 ) {
			MainView.outputText( 'and viciously-clenching ass ', false );
		}
		MainView.outputText( 'pulse through you. She throws her head back and moans out in sheer, slutty enjoyment as you plumb her luxurious depths, different from anything you\'ve experienced before in its firmness and the powerful, brutal sensations it is giving to you.\n\n', false );
		//(If balls)
		if( CoC.player.balls > 0 ) {
			MainView.outputText( 'You eventually bottom out, your ' + Descriptors.ballsDescriptLight() + ' coming to rest between her powerful, gelatinous thighs, her soft feathers tickling at your ' + Descriptors.sackDescript() + ' maddeningly.\n\n', false );
		}//(If balls-less)
		else {
			MainView.outputText( 'Leaning low over her back, you adjust to drive yourself ever deeper, forcing the whole length of your ' + Descriptors.cockDescript( x ) + ' into her, the soft down on her gelatinous thighs prickling at your thighs maddeningly.\n\n', false );
		}
		if( CoC.player.gender === 1 ) {
			//(Male, no breasts)
			if( CoC.player.biggestTitSize() < 1 ) {
				MainView.outputText( 'Taking care not to waste your opportunity with an immense butt by cumming too soon, you slowly start to push and pull her fat, quivering cheeks up and down your ' + Descriptors.cockDescript( x ) + ', feeling her rough walls and clutching, gasping convulsions massage your manhood', false );
				if( CoC.player.totalCocks() > 1 ) {
					MainView.outputText( 's', false );
				}
				MainView.outputText( ' along ', false );
				if( CoC.player.totalCocks() > 1 ) {
					MainView.outputText( 'their', false );
				} else {
					MainView.outputText( 'the', false );
				}
				MainView.outputText( ' whole length, already bringing you close to the edge of a bone-shaking orgasm.\n\n', false );
			}
			//[(Male w/ breasts)
			else {
				MainView.outputText( 'You laugh in glee as the bird\'s groping, clutching hole', false );
				if( CoC.player.totalCocks() > 1 ) {
					MainView.outputText( 's massage', false );
				} else {
					MainView.outputText( ' massages', false );
				}
				MainView.outputText( ' your ' + Descriptors.cockDescript( x ), false );
				if( CoC.player.totalCocks() > 1 ) {
					MainView.outputText( ' and ' + Descriptors.cockDescript( y ), false );
				}
				MainView.outputText( ' while you knead over your ' + CoC.player.allBreastsDescript() + ' hungrily.  You start to pound her tight, harsh hole roughly, groaning and licking your lips, trying to master the power of the sensations her hot, clenching hole are giving to you. You lean down low over her back, your ' + CoC.player.allBreastsDescript() + ' pressing against the tickly, prickling feathers, as you bite down onto her shoulder.  Your ' + Descriptors.hipDescript() + ' raise and drop mercilessly into that jiggling, jelly-like ass, giving a noticeable bouncing effect from it as the squishing flesh catapults your rear back into the air with every lewd, slapping thrust.\n\n', false );
			}
		}
		//[(Herm)
		else if( CoC.player.gender === 3 ) {
			MainView.outputText( 'Grinning and massaging your conquest\'s round, soft ass in anticipation, you gently begin to thrust your ' + Descriptors.hipDescript() + ', moaning loudly as her rough, grasping walls drag over the sensitive flesh of your ' + Descriptors.multiCockDescriptLight() + '. Your ' + Descriptors.vaginaDescript( 0 ) + ' practically spurts fluids out, dribbling down the inside of your thighs as you pump in and out of the helpless harpy.  You love every second you get to spend in her harsh inner walls.\n\n', false );
		}
		MainView.outputText( 'It seems the exposed girl is enjoying herself just as much.  Every tiny motion of your ' + Descriptors.hipDescript() + ' causes her to writhe and moan, her moans punctuated with birdlike squawks as she pushes her huge egg-bearing hips back at you in sheer, undisguised desire to have you fuck her harder and deeper.\n\n', false );
		MainView.outputText( 'You decide to give her what she so clearly wants and plant both your hands onto her massive bottom, squashing and groping it around your impaling ' + Descriptors.multiCockDescriptLight() + ' as you speed up the pace of your thrusts, driving the full length of your ' + Descriptors.cockDescript( x ), false );
		if( CoC.player.totalCocks() > 1 ) {
			MainView.outputText( ' and ' + Descriptors.cockDescript( y ), false );
		}
		MainView.outputText( ' into her grinding, overwhelming hole', false );
		if( CoC.player.totalCocks() > 1 ) {
			MainView.outputText( 's', false );
		}
		MainView.outputText( '.  ', false );
		//{(If balls)
		if( CoC.player.balls > 0 ) {
			MainView.outputText( 'Your ' + Descriptors.sackDescript() + ' slaps wetly into her immense, wobbling thighs.  ', false );
		}
	...
	HarpyScene.prototype.winAndRapeHarpyAnally = function() {
		...
		MainView.outputText( 'Grinning to yourself and stripping free of your ' + CoC.player.armorName + ', you lean down and grasp the harpy\'s pointed chin, turning it from side to side, your eyes roaming across her sharp, beautiful features as you inspect your prize. Her body just begs to be touched!  You gaze at those marvelous, tickly feathers coating her slender chest and those awe-inspiring hips, each supporting a fat, jiggling buttock that\'s nearly as large as a centaur\'s whole rear!  Without hesitation, the stricken harpy raises herself and starts to press that sweet, feathery body against yours, squawking and crooning pathetically.', false );
		if( CoC.player.biggestTitSize() >= 2 ) {
			MainView.outputText( ' Her petite bust squishes against your ' + CoC.player.allBreastsDescript() + '. Her downy tits tickle, and grind against your chest maddeningly, tickling and scratching them gently as the lust-fueled bird-girl seems to ache for you.', false );
		}
		MainView.outputText( '\n\n', false );
		MainView.outputText( 'Not wishing to keep her fantastic butt waiting, you grasp onto her shoulders and pull her back with you, laying back against the hard ground with the squawking avian slut grinding and bouncing her bean-bag ass against your ' + Descriptors.multiCockDescriptLight() + '.  Blood flows through your ' + Descriptors.cockDescript( x ) + ' as it is sandwiched between her hot, caressing, luxurious ass cheeks, and you grind the head brutally against her impossibly tight pucker, which seems to make her panic a bit. She starts squawking and flapping her arm-wings in a vain attempt to escape your grasp, and the imminent butt-fucking she is about to receive! You smirk to yourself as the fluids from her womanhood drool and spill over your entrapped ' + Descriptors.cockDescript( x ) + ', your slowly-pumping ' + Descriptors.hipDescript() + ' smearing her own sticky sexual juices over those downy butt cheeks.\n\n', false );
		MainView.outputText( 'The bird-girl\'s attempts to escape only turn you on more, and you grab HARD onto those gelatinous orbs, guiding your ' + Descriptors.cockDescript( x ) + ' into her gloriously tight and hot rear entrance; it feels harsh and rough, practically scratching every inch of your member as it\'s driven hard against her resisting inner walls.  She\'s far tighter and hotter than any human girl could possibly be! Your ' + Descriptors.cockDescript( x ) + ' pushes so viciously and painfully into her clenching tight depths that the harpy squeals and cries out, trying to fly away from you again.  However, she only succeeds in clenching and waving her enormous ass, sending viciously sharp sensations through your blood-engorged dick. Inch by inch you force your ' + Descriptors.cockDescript( x ) + ' into her, her movements getting more frantic as you plumb her most lustful and forbidden hole.  Finally, you bottom out and stop for breath, her powerful, jiggling thighs pressing against yours.', false );
		//(if balls)
		if( CoC.player.balls > 0 ) {
			MainView.outputText( '  Your ' + Descriptors.ballsDescriptLight() + ' come to rest against her thick, wiggly rear.', false );
		}
		MainView.outputText( '\n\n', false );
		MainView.outputText( 'The harpy girl seems to freeze above you, wincing and trilling in pain. You are bordering on pain yourself; her crushing ass is so tight and so hot it nearly burns your ' + Descriptors.cockDescript( x ) + '! You smirk and raise your hand, before bringing it down onto her blubbery feminine rear.  With a loud, ringing slap, her ass jiggles as you scream "<i>Giddy-up!</i>"\n\n', false );
		//[(Males, no breasts)
		if( CoC.player.biggestTitSize() < 1 && !CoC.player.hasVagina() ) {
			MainView.outputText( 'Her pupils constrict, and the feathery fat-reared fowl squawks loudly, going completely crazy atop you! She writhes and bounces this way and that, growing more and more desperate and insane as her pain increases.  Your ' + Descriptors.cockDescript( x ) + ' slams this way and that into her rough, gripping walls, as moans and grunts escape your lips. She flaps frantically from side to side, almost slipping her clutching rump free of your ' + Descriptors.cockDescript( x ) + ' before you grasp hard onto her meaty thighs, forcing yourself back into the distressed harpy with a lewd grin.  It is clear that some part of her is loving this brutal intrusion; her feminine fluids are spilling free, coating your torso with her strong scent.\n\n', false );
		}
		//[(Herms/Shemales)
		else {
			MainView.outputText( 'Her pupils constrict, and the feathery fat-reared fowl squawks loudly, going completely crazy atop you! Struggling to escape from your impaling shaft, she drives her clutching, ribbed inner walls into the sides of your throbbing dick, her gelatinous ass wobbling this way and that', false );
			if( CoC.player.biggestTitSize() >= 2 ) {
				MainView.outputText( ', the motions causing your ' + CoC.player.allBreastsDescript() + ' to bounce and sway wildly, slapping into one another lewdly, adding to your growing lust', false );
			}
			MainView.outputText( '. You keep a firm grip on her rump and thighs, ensuring that every time she nearly wriggles her hot anus free of your cock, you can drive your hot meat straight back into her lush depths, laughing at her helplessness.', false );
			//(If herm)
			if( CoC.player.gender === 3 ) {
				MainView.outputText( '  Her struggles are driving you wild, and your ' + Descriptors.vaginaDescript( 0 ) + ' is seeping fluids, growing more and more aroused as she bounces and slamming that immense, rippling rear into you, the force squishing your moist lips together, releasing yet more sticky juices and adding an extra little tone of pleasure.', false );
				//{(If balls)
				if( CoC.player.balls > 0 ) {
					MainView.outputText( '  She even makes your ' + Descriptors.ballsDescript() + ' slap against your ' + Descriptors.vaginaDescript( 0 ) + ', a hot, moist SPLAT accompanying every tiny motion the harpy makes.', false );
				}
			}
			MainView.outputText( '\n\n', false );
		}
		MainView.outputText( 'With an ear-splitting squawk, the impaled harpy\'s drooling pussy absolutely explodes.  It fires huge streams of her sticky feminine cum onto your ever-thrusting ' + Descriptors.hipDescript() + ', her muscles locking in ecstasy, before the exhausted bird-girl collapses over you, ', false );
		//(if breasts)
		if( CoC.player.biggestTitSize() >= 2 ) {
			MainView.outputText( 'her chest mashing against yours as she becomes still, ', false );
		}
	...
	HarpyScene.prototype.WinOnHarpyAndOralRape = function() {
		...
		MainView.outputText( 'You stand threateningly over the helpless harpy, stroking yourself through your ' + CoC.player.armorName + ' before grasping onto her chin, turning it this way and that, inspecting your prize thoroughly. Those golden lips glisten in the light as she trills feebly, her eyes looking soulfully up at you.\n\n', false );
		MainView.outputText( 'Grinning lewdly you squeeze her cheeks, the stricken harpy opening her mouth obediently. Her tongue is short and stubby like that of a normal bird, glistening with saliva and looking terribly inviting. You strip off the lower portion of your ' + CoC.player.armorName, false );
		//(if pussy)
		if( CoC.player.hasVagina() ) {
			MainView.outputText( ', fluids seeping from your plush ' + Descriptors.vaginaDescript( 0 ) + ' all the while', false );
		}
		//(if cock)
		if( CoC.player.hasCock() ) {
			MainView.outputText( ', ', false );
	...
	HarpyScene.prototype.harpyScissorSurprise = function() {
		...
		MainView.outputText( 'You stare down at the defeated harpy, watching as ' );
		if( CoC.monster.HP < 1 ) {
			MainView.outputText( 'her arms tremble from the effort of trying to keep her body from collapsing to the floor.' );
		} else {
			MainView.outputText( 'she leans back from her kneeling position, spreading her legs and digging her fingers into her soaking cunt. Her other hand darts up to her ample bosom, giving it a hard squeeze before starting to tease her erect nipple, pinching and flicking as she continues to fingerfuck herself ever more forcefully.' );
		}
		...
		if( CoC.monster.HP < 1 ) {
			MainView.outputText( 'vulnerable' );
		} else {
			MainView.outputText( 'lust-mad' );
		}
		MainView.outputText( ' harpy.' );
		...
		if( CoC.monster.HP < 1 ) {
			MainView.outputText( 'worry' );
		} else {
			MainView.outputText( 'desire' );
		}
		MainView.outputText( ', but before she can even begin to talk you grab the mess of short hair atop her head and yank her forwards, roughly grinding her face into your soaked pussy[if (isHerm = true) ", leaving your stiff cock resting on her head where the stimulation from her feathery hair causes it to steadily drip pre-cum down the back of her neck"].  She struggles furiously, her hands shooting up to your thighs as she tries to push away from your slick, aching entrance.  Your grip is far too strong and she\'s too exhausted from the fight though, and you demonstrate the pointlessness of her attempts at freedom by slowly dragging her face up and down against your [vagina], smearing even more femcum over her increasingly distraught visage.' );
	...
	HarpyScene.prototype.spoidahsLegEggsInHarpeis = function() {
		...
		MainView.outputText( 'Perched above the downed bird-bitch, you look her over as you weigh up your options.  Your eyes slide across her petite breasts, tracing her curves until your gaze rests upon her flared hips and curvaceous ass.  Unfortunately your appreciation of the finer things in life is interrupted by the feather-brained eyecandy; she simply will not stop squawking.' );
		...
		if( CoC.player.tallness > 78 ) {
			...
			MainView.outputText( 'forward' );
		} else {
			MainView.outputText( 'down' );
		}
		MainView.outputText( ', eyes closed and lips puckered seductively, ready to deliver a heady dose of her drugged lipstick.' );
		...
		if( CoC.player.gender > 0 ) {
			...
			MainView.outputText( ', enjoying the view.' );
		}
	...
	HarpyScene.prototype.clitFuckAHarpy = function() {
		MainView.clearOutput();
		MainView.outputText( 'Eyeing the hapless bird-woman up and down, you feel your lusts kindling.  Moisture seeps from your [vagina] as you watch the harpy ' );
		if( CoC.monster.lust > 99 ) {
			MainView.outputText( 'moan and masturbate, dipping her fingers into her honey-pot with wild abandon.' );
		} else {
			MainView.outputText( 'moan and struggle to rise, jiggling in the cutest way each time she slumps down, defeated.' );
		}
		MainView.outputText( '  You wrench her thick thighs apart to get a better view of her avian snatch.  Her lips are shrouded in a downy, feathery fuzz, but you can easily make out the heavy, lust-glossed labia.  The thought of tasting her dripping honey makes your mouth water and inside your [armor], your [clit] has emerged from its hood, straining as blood rushes to your loins.   The sensitive organ is so swollen that it\'s more lady-cock than joy-buzzer, the thick hood resembling an animal\'s sheath more than a natural flesh fold.' );
		...
		if( CoC.player.wetness() < 3 ) {
			MainView.outputText( '  Runnels of your lubricant are dripping down your [legs] in streams,' );
		} else if( CoC.player.wetness() < 4 ) {
			MainView.outputText( '  Streams of lubricant are soaking down your [legs] in waves,' );
		} else {
			MainView.outputText( '  Squirts of lubricant are gushing down your [legs] to puddle on the floor,' );
		}
		MainView.outputText( ' evidence of just how hungry your clenching, empty hole is.' );
		...
		if( CoC.player.wetness() < 3 ) {
			MainView.outputText( '  A fresh gush of fluid washes out of your pussy at the peak of your pleasure even though you\'re far from a squirter' );
		} else if( CoC.player.wetness() < 4 ) {
			MainView.outputText( '  A fresh spout of girlspunk washes out of your pussy at the peak of your pleasure' );
		} else {
			MainView.outputText( '  A torrential splash of girlspunk shoots out of your pussy in a river at the peak of your pleasure' );
		}
		MainView.outputText( ', timed with a full-body seizure that causes you and your partner to slump down into sticky pile.' );
		...
		if( CoC.player.tou / 20 + Utils.rand( 20 ) < 6 ) {
			MainView.outputText( ' your endurance gives out, and you slip into unconsciousness.  This time it seems you met your sexual match.' );
			CoC.monster.lust = 98;
			CoC.monster.HP = 2;
			CoC.player.lust = 100;
			CoC.flags[ kFLAGS.COMBAT_BONUS_XP_VALUE ] = CoC.monster.XP;
