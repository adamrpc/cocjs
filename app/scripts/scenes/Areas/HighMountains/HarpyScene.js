...
angular.module( 'cocjs' ).run( function( SceneLib, EngineCore, CoC, kFLAGS, Combat, Descriptors, Utils, AppearanceDefs ) {
	...
	HarpyScene.prototype.harpyVictoryuuuuu = function() {
		...
		var scissor = null;
		var clitFuck = null;
		if( CoC.getInstance().player.hasVagina() && CoC.getInstance().player.clitLength >= 3.5 ) {
			clitFuck = this.clitFuckAHarpy;
		}
		if( CoC.getInstance().player.hasVagina() ) {
			if( CoC.getInstance().player.isNaga() ) {
				EngineCore.outputText( '  If you weren\'t a naga, you could scissor her.' );
			} else {
	...
	HarpyScene.prototype.harpyLossU = function() {
		...
		if( CoC.getInstance().player.hasCock() ) {
			x = CoC.getInstance().player.cockThatFits( CoC.getInstance().monster.vaginalCapacity() );
		}
		//Genderless people get boned
		if( CoC.getInstance().player.gender === 0 && CoC.getInstance().player.isGoo() ) {
		...
		//No fitu
			...
			EngineCore.outputText( 'Though you\'ve been defeated by the harpy, it doesn\'t seem she wants anything to do with you, and gives you a whack upside your head before departing.', true );
			Combat.cleanupAfterCombat();
		}
	};
	HarpyScene.prototype.harpyGooGenderlessLoss = function() {
		EngineCore.outputText( '', true );
		EngineCore.outputText( 'The triumphant harpy looks down at your goopy form, ready to take you.  She seems a little confused though, and begins poking at your gelatinous body.\n\n', false );
		EngineCore.outputText( '"<i>Penis?</i>" she eventually asks.\n\n', false );
		EngineCore.outputText( 'You tell her no.\n\n', false );
		EngineCore.outputText( '"<i>Pussy?</i>"\n\n', false );
		EngineCore.outputText( 'Nope.\n\n', false );
		EngineCore.outputText( '"<i>Oh...</i>"\n\n', false );
		EngineCore.outputText( 'She grabs a handful of you and releases it, watching your body jiggle about. This seems to amuse her greatly and she plunges both hands inside you then shakes them about. Retracting them, she takes a few steps back and gives you an evil grin, then lets out a blood-curdling shriek.\n\n', false );
		EngineCore.outputText( 'Two more harpies flap down beside her and the trio huddles together, periodically pointing at you.  Most of what they\'re saying is inaudible to you, though you\'re fairly sure you heard \'goop\' somewhere in there.  You\'re not given much time to stew on it though, as they seem to agree on something and approach.\n\n', false );
		EngineCore.outputText( 'The smallest of the bunch walks right up to you and punches your chest, laughing and chittering as your body undulates in response.  The other two stick their arms inside you and begin shaking them about, also laughing.  This goes on for quite a while, with the bird women trying various positions and levels of force (including a particularly odd one in which two attempted to shove your entire body up the third\'s vagina; not exactly a situation you want to find yourself in again...). Eventually they get bored of you and flap off, leaving your exhausted body in a pile until you get enough energy to pull yourself together and head back.', false );
	};
	//Requires pussy or cock small enough for harpy!
	HarpyScene.prototype.harpyLossLust = function() {
		EngineCore.outputText( '', true );
		//Merauder wroted.
		var x = -1;
		if( CoC.getInstance().player.hasCock() ) {
		...
		EngineCore.outputText( 'You collapse in a broken heap, panting as your body feels hot and heavy with desire.  The wanton blood coarsing through your veins is a testament to the sumptuous shapes of the harpy\'s lithe bodies. Turning your aching head to look at the ever-circling harpy, you see her mad-looking eyes narrow as she gains height, flapping her two pairs of wings frantically.\n\n', false );
		EngineCore.outputText( 'She lets out a psychotic cry that seems to echo through the area around you, carrying into the hills and back again. You could swear that more cries come back than the one that started the rolling echo, but before you can dwell on this, the crazed bird-girl dives at you with incredible speed. She lands elegantly in front of you, teasing you by obscuring parts of her body with a fan of feathers. When she gets close enough, the horny harpy plucks at your ' + CoC.getInstance().player.armorName + ', attempting to remove it.\n\n', false );
		EngineCore.outputText( 'It\'s not just her claws moving across your body! From behind and from the side, three more ornately plumed ladies have descended and are running their talons curiously across the last trappings holding your gear to your body. When these are undone, your ' + CoC.getInstance().player.armorName + ' falls to the ground. You cover your ' + CoC.getInstance().player.allBreastsDescript() + ' and crotch as the foursome leer at you unnervingly with their pitch black eyes and yellow pupils. The sensation is not so bad however, being surrounded by these fair and elegant creatures. When the four spread their wings, they screen your body from the outside world in a palette of colorful plumage. The one that took you down lets a single lock of your ' + Descriptors.hairDescript() + ' fall across her talons admiringly', false );
		//if long hair
		if( CoC.getInstance().player.hairLength > 12 ) {
			EngineCore.outputText( ' and moves it out of your face', false );
		}
		EngineCore.outputText( '.  When she smirks, you know you\'re in for quite a ride.\n\n', false );
		//{If Player has only a pussy} Fan of feathers:
		if( CoC.getInstance().player.hasVagina() && x < 0 ) {
			EngineCore.outputText( 'Grabbing your sedate body by ankles and wrists, the four yank you roughly towards a large stone slab, upon which they lie you down, warbling and squawking. Three hold you down and as though they\'re trying to avoid other harpies getting in on the fun, the three to your side spread their wings wide and cover you up in a feathery dome.  While you are pinned down, the fourth climbs in on top of you as you struggle, taking it amazingly slow. She extends an open hand, revealing the long talons that constitute her fingers and runs the tip of one across your bare chest, leaving a red trail. It makes you gasp a little. The nail would be sharp enough to carve your flesh if she didn\'t put it down with such care. She seems to like the reaction of both you and your ' + CoC.getInstance().player.skinDesc + ' and chirps bemused.  ', false );
			if( CoC.getInstance().player.skinType === AppearanceDefs.SKIN_TYPE_PLAIN ) {
				EngineCore.outputText( 'Shortly after, she repeats her journey, this time by folding her wing towards you and almost like soothing the trek, swishing the tip of her tallest feather across it, evoking a thankful moan. The downey barbules are some of the softest materials you\'ve yet encountered. The harpies must not be familiar with sensitive skin like yours and they\'re exhilarated with the discovery.  ', false );
			}
			EngineCore.outputText( 'Within moments, the three harpies holding you down join in on the fun, running their razor digits carefully across your body', false );
			if( CoC.getInstance().player.skinType === AppearanceDefs.SKIN_TYPE_PLAIN ) {
				EngineCore.outputText( ' and quickly alleviating the pain by brushing their wings across your hide', false );
			}
			EngineCore.outputText( '.  You instantly stop struggling, no matter how insane the sensation is. One wrong move and you\'d lance yourself on their talons even if the harpies did not intend it. It\'s hard to lie still however. Every inch of your body is beset by the hair-raising scratch of their razor sharp nails', false );
			if( CoC.getInstance().player.skinType === AppearanceDefs.SKIN_TYPE_PLAIN ) {
				EngineCore.outputText( ', always followed by a soothing brush of their feathers', false );
			}
			EngineCore.outputText( '. They line the curve of your ' + Descriptors.breastDescript( 0 ) + ', brush around your aurela, and harden your ' + Descriptors.nippleDescript( 0 ) + 's. They trace your inner thigh, and not even your ' + CoC.getInstance().player.feet() + ' are safe. It does not tickle, but your nerves are lit up like a Christmas tree and a tear wells up in your eyes as you\'re forced to lie still in the face of such a soft, yet overpowering sensation.\n\n', false );
			EngineCore.outputText( 'Suddenly, there\'s a different feeling altogether. You look down and through the feathered fans, spot one of the harpies with her face between your spread legs. She looks at you, a sultry expression in her black-yellow eyes as her black, stout tongue runs up and down your ' + Descriptors.vaginaDescript( 0 ) + '. You close your eyes and focus on that, instead of the maddening sensation of the rolling waves of talons', false );
			if( CoC.getInstance().player.skinType === AppearanceDefs.SKIN_TYPE_PLAIN ) {
				EngineCore.outputText( ' across your reddening skin', false );
			}
			EngineCore.outputText( '. Her little black tongue feels divine as it bumps up against your ' + Descriptors.clitDescript() + '. WHen you let out a small, alleviated coo, the other harpies once again follow suit. The two previously holding your arms, bend down and put their stiff lips on top of your antagonized nipples, rewarding them after having brushed past them so many times.\n\n', false );
			EngineCore.outputText( 'You feel your ' + CoC.getInstance().player.legs() + ' being lifted up and out of the way. Feathered arms curl around either side of your thighs to keep them in place as the harpy teasing and suckling your ' + Descriptors.clitDescript() + ' gives the last harpy enough room to scoot in below her. A teasing of soft harpy hair feathers against your ' + Descriptors.buttDescript() + ' is the only warning you receive before you feel the wet warmth of that final black tongue pressing against your ' + Descriptors.assholeDescript() + '.\n\n', false );
			EngineCore.outputText( 'It doesn\'t take long for you to reach climax when the harpies continue to titillate your skin with their digits while suckling your pleasure button and nipples. You count yourself fortunate that they have the sense to retract their talons when you shudder and convulse upon the slab.\n\n', false );
			EngineCore.outputText( 'When you recover a few hours later, the harpies are gone. All they\'ve left you with is a sloppy cunt and a few downey feathers clinging to your nipples and inner thigh.', false );
		}
		//{When player has a cock} Airborne sex!
		//(Reminds me of what my own Succubus does in my novel)
		else {
			EngineCore.outputText( 'The ladies flap their wings and you\'re surprised by their grace and power. Three of them grab you by the shoulders with their hawk-like feet and lift you up effortlessly and with good speeds like mere prey. When the ground is removed beneath your feet, your heart jumps, but it leaves you completely at the mercy of the harpies who begin to fly up higher and higher. By the time you begin to protest, you\'re already fifty feet up into the air. If they\'d drop you now, your fall would be fatal. It only makes the way in which they begin to toy with you all the more frightening.\n\n', false );
			EngineCore.outputText( 'They drop you on purpose, only for you to be caught again mid-air by one of the harpies. Using the momentum, your sadistic savior then throws you around like a sling for others to catch, which they do crying with joy, before passing you along again with dizzying speeds and heights. The ground below rushes towards and away from you in rapid succession. After five times, they squabble and two of them nearly pull you apart between them. It is then however, that a third gets an idea. Using the fact that your body is suspended between two others pretty stably, she flies into you and latches on by crossing her legs and arms around your hips and waist. There, high up in the sky and in intimate embrace, the other two harpies still quarreling, she smiles at you while straddling you. In an unknown language of whistles and chirps, she assures you it\'s going to be all right and reaches underneath her to fondle your cock. With the warmth of her body so near to yours and now armed with wings of your own in the form of the harpy on top of you, it works. Held between three digits, your ' + Descriptors.cockDescript( x ) + ' assumes its erect shape. It makes the latched harpy give a pleased warble, and with a quick slip, she pushes her womanhood against it until you sink into her.\n\n', false );
			EngineCore.outputText( 'The little slit is a bit dry, but her shaft is silky smooth and pliable. With no effort, she starts to hump you, running her arid canal across your ' + Descriptors.cockDescript( x ) + ' and holds on tight. Within minutes, the efforts of the eager little lithe parachute latched on front of your body makes you wheeze. Her entire body bucks and bends frantically on top of yours. Another harpy comes flying up and demands a turn of her own, but cannot find an opening. Instead, she chooses to fly underneath your chafing bodies and starts licking the embracing nethers enthusiastically. ', false );
			if( CoC.getInstance().player.balls > 0 ) {
				EngineCore.outputText( 'You can feel her little tongue rolling across your balls and the base of your ', false );
			} else {
				EngineCore.outputText( 'You can feel her little tongue rolling across the base of your ', false );
			}
			EngineCore.outputText( Descriptors.cockDescript( x ) + ' whenever the one on top of you doesn\'t take you up to the hilt.\n\n', false );
			EngineCore.outputText( 'You were just about to climax, when the harpies keeping you airborn decide they want their own turn. Spurned, they let go of you, sending you to plummet back to earth with one of the harpies still fucking on top of you. This time, the other three don\'t seem bothered to save you anymore.\n\n', false );
			EngineCore.outputText( 'The ground rapidly aproaches, but the harpy fucking you doesn\'t even seem to notice. She simply continues to dry hump, a stupored stare of sex in her eyes. You try to alert her, but as she starts squeezing your cock within the hawk-like confines of her birdhole, she doesn\'t even attempt to flap her wings. With just a few feet remaining, you finally orgasm and release your load into her eager cunt, yelling and flailing your arms as textured details of the mountain\'s granite ground rushes towards you. As you fill her up with your last desperate throes, it seems to do it for the bird-lady. With the last ten feet remaining, she disconnects, spreads her wings and uses the velocity to tumble forcefully and hurl you away horizontally. With a faint arc, you skip across the floor for over a dozen feet.\n\n', false );
			EngineCore.outputText( 'In your last conscious moment before you pass out from the hit, you hear the harpy sisters cackling above you.', false );
		}
		Combat.cleanupAfterCombat();
	...
	//No genderless folks.
	HarpyScene.prototype.harpyDamageLoss = function() {
		EngineCore.outputText( '', true );
		var x = -1;
		if( CoC.getInstance().player.hasCock() ) {
			x = CoC.getInstance().player.cockThatFits( CoC.getInstance().monster.vaginalCapacity() );
		...
		if( y >= CoC.getInstance().player.cocks.length ) {
			y -= 2;
		}
		if( y === x && CoC.getInstance().player.totalCocks() > 1 ) {
			if( y === 0 ) {
				y++;
			} else {
				y--;
		...
		EngineCore.outputText( 'You collapse in a broken heap, gasping for breath as your wounds sting and throb, a testament to the sharpness of the harpy\'s claws. Turning your aching head to look at the ever-circling harpy, you see her mad-looking eyes narrow as she gains height, flapping her two pairs of wings frantically.\n\n', false );
		EngineCore.outputText( 'She lets out a psychotic cry that seems to echo throughout the area around you, carrying into the hills and back again. You could swear that more cries come back than the one that started the rolling echo, but before you can dwell on this, the crazed bird-girl dives at you with incredible speed, her thick frame colliding with you. You are sent spinning and rolling across the ground, your wounds singing a hymn of pain and weariness. After a few complex moments, the pair of you are dumped in a heap on the floor, with the horny harpy clawing brutally at your ' + CoC.getInstance().player.armorName + ', tearing it away from your body in no time at all.\n\n', false );
		//[(Male, shemale, herm)
		if( CoC.getInstance().player.hasCock() && x >= 0 ) {
			...
			EngineCore.outputText( 'your ' + Descriptors.multiCockDescriptLight() + ' flopping from side to side from the ministrations, you see not one, but THREE deranged, randy bird-girls, all squabbling over your genitals.\n\n', false );
			//[(Male/shemale)
			if( !CoC.getInstance().player.hasVagina() ) {
				EngineCore.outputText( 'The hierarchy is eventually settled, it seems, because two of those heavy-hipped bird babes wrap their lips around your ' + Descriptors.cockDescript( x ) + ', sucking and pecking at the hot flesh', false );
				if( CoC.getInstance().player.balls > 0 ) {
					EngineCore.outputText( ', craning their necks to slurp and massage your ' + Descriptors.ballsDescriptLight() + ' eagerly, trying to coax up your cum production as much as they can', false );
				}
				EngineCore.outputText( '. The third, estranged harpy leaps at your torso, straddling your navel with her weighty rear, knocking the breath out of you and leaning down, ', false );
				//(If breasts)
				if( CoC.getInstance().player.biggestTitSize() >= 1 ) {
					EngineCore.outputText( 'her petite bosom grinding across your ' + CoC.getInstance().player.allBreastsDescript() + ', her nipples tickling at your ' + Descriptors.nippleDescript( 0 ) + ', ', false );
				}
				EngineCore.outputText( 'her golden lips glinting in the light as they make for your own.\n\n', false );
			}
			//[(Herm)
			else {
				EngineCore.outputText( 'The hierarchy is settled between the warring bird-girls, and two of them grasp onto your thighs with their clawed hands, those gold-lipped mouths starting to hungrily suckle and taste every inch of your ' + Descriptors.cockDescript( x ) + ', their squawking, feathered heads occasionally diving down to probe their short, stubby tongues into your ' + Descriptors.vaginaDescript( 0 ) + ', sending sharp spikes of pleasure rippling through your body, ', false );
				//(If balls)
				if( CoC.getInstance().player.balls > 0 ) {
					EngineCore.outputText( 'their hands moving up to grope and fondle your ' + Descriptors.ballsDescriptLight() + ', massaging them to boost your cum production as much as they can, ', false );
				}
				EngineCore.outputText( 'tiny tingles left on your body at every spot their gilded lips caress. The third harpy dives at your chest, her thick hips SLAMMING into your stomach, knocking the breath out of you', false );
				//(If breasts)
				if( CoC.getInstance().player.biggestTitSize() >= 2 ) {
					EngineCore.outputText( ', causing your ' + CoC.getInstance().player.allBreastsDescript() + ' to bounce and sway violently', false );
				}
				EngineCore.outputText( '.  She slowly leans down, her eyes locked on yours, her lips glinting a malicious gold', false );
				//(if breasts)
				if( CoC.getInstance().player.biggestTitSize() >= 2 ) {
					EngineCore.outputText( ' her toned chest and pert breasts squishing your ' + Descriptors.breastDescript( 0 ) + ' beneath them', false );
				}
		...
		//[(Female)
			...
			EngineCore.outputText( 'The hierarchy is determined, and two of the harpies dive forward, burying their fat golden lips into your ' + Descriptors.vaginaDescript( 0 ) + ', their short, stubby tongues probing and exploring every inch of your feminine love-tunnel.  Together they circle and push at your ' + Descriptors.clitDescript() + ' sensuously, leaving a sharp tingle at every little point their plump golden lips brush against.  Growing more and more horny, each harpy probes one hand behind herself to toy with her drooling pussy, coaxing aroused squawks and moans from the group as they tend to you. The leading harpy makes a reckless dive for your chest, intent on claiming her prize before anyone else can take it from her. Careering down upon your stomach, her immense, wobbly ass pounds into you, knocking the breath out of your lungs', false );
			//(If breasts)
			if( CoC.getInstance().player.biggestTitSize() >= 3 ) {
				EngineCore.outputText( ' and causing your ' + CoC.getInstance().player.allBreastsDescript() + ' to jiggle, slapping into one another', false );
			}
			EngineCore.outputText( '. She leans slowly over you, her gilded lips glistening with saliva as they approached yours', false );
			//{(If breasts)
			if( CoC.getInstance().player.biggestTitSize() >= 2 ) {
				EngineCore.outputText( ', her petite chest squashing your ' + Descriptors.breastDescript( 0 ) + ' against hers', false );
			}
			EngineCore.outputText( '.\n\n', false );
			EngineCore.outputText( 'When the harpy\'s lips press against yours, your vision starts to swim. She kisses you ferally, fuelled by a primeval lust.  Her short, sharp tongue slaps yours all over your mouth, tickling and scratching across your lips. As you are forced into the crushing, vicious kiss, you notice that the pleasure shooting up and down your body seems somehow sharp, and your lust is beginning to swell and surge around your mind.  Everywhere their lips have brushed on you feels tingly and pleasurable... those golden lips... they must be drugged! Even as you realize this, it is too late, and your tense muscles collapse in the overwhelming feel of it, your libido burning hot even as your body feels so weary.\n\n', false );
		...
		if( x >= 0 && CoC.getInstance().player.cockTotal() === 1 ) {
			EngineCore.outputText( 'Another powerful sensation shoots through you, forcing your back to tense from the simple, pure pleasure, and further forcing your face to lick and taste her moist cunt. You\'re blinded completely by the huge ass-cheeks stifling your face, and can only feel the hot, rough walls of a harpy\'s pussy as it descends on your ' + Descriptors.cockDescript( x ) + ', slipping all the way down to the base as the slutty harpy squawks in a perfect duet with her face-riding sister. The clutching tightness engulfs your whole length as she slips down to the base', false );
			if( CoC.getInstance().player.balls > 0 ) {
				EngineCore.outputText( ', her huge feathery bottom coming to rest on your ' + Descriptors.ballsDescriptLight(), false );
			}
			EngineCore.outputText( '. Even as you come to bear this tremendous pleasure, you feel your hands wrenched carelessly from the big butt that\'s grinding against your face, and a small feathery pair of hands guides them to their goal.  Your fingers slip into the hot, clutching tightness of two harpy holes, the sensations made even more vivid by the blinding faceful of ass.  A sharp-clawed foot steps dominantly on your navel, digging the claws into your ' + CoC.getInstance().player.skinDesc + '. The two harpies whose asses are pinning you to the ground start to bounce their full weight brutally on your already broken body in unison.  The sheer, vast weight of their hips batters every inch of you, twin feathery punchbags slamming into your cheeks and onto your crotch. Were it not for the hot, moist hole that\'s coating your mouth, you would be screaming in pain', false );
			if( CoC.getInstance().player.balls >= 0 ) {
				EngineCore.outputText( '.  Your ' + Descriptors.ballsDescriptLight() + ' bounce with such force as to fetch the cock-speared harpy a stinging slap on the rear with every brutal, mechanical fuck', false );
				//{(If herm)
				if( CoC.getInstance().player.gender === 3 ) {
					EngineCore.outputText( ' before swinging back to spanking your ' + Descriptors.vaginaDescript( 0 ) + ' wetly.', false );
				} else {
					EngineCore.outputText( '.', false );
				}
			} else if( CoC.getInstance().player.biggestTitSize() >= 2 ) {
				EngineCore.outputText( ' while your ' + CoC.getInstance().player.allBreastsDescript() + ' are shaking and jiggling around helplessly.', false );
			} else {
				EngineCore.outputText( '.', false );
			}
			EngineCore.outputText( '  It\'s anyone\'s guess how long you\'ll be able to maintain consciousness while being ridden to exhaustion.\n\n', false );
		}
		//[(Male, Herm, Shemale, Multi-cock)
		else if( x >= 0 && CoC.getInstance().player.cockTotal() > 1 ) {
			EngineCore.outputText( 'The squabbling hasn\'t gone on for very long this time, and you know why.  You steel yourself beneath the tremendous quivering rumpcheeks; you can\'t help but gasp for breath, moaning sluttily as your ' + Descriptors.cockDescript( x ) + ' and ' + Descriptors.cockDescript( y ) + ' are crushed by the tight walls of a pair of unbelievable harpy pussies.  As the face-riding bird-girl releases a thick, creamy load of feminine juices onto your face, she leans forward and moans out in bliss.  You\'re allowed to peek between her thighs at the action on your crotch. One of the monstrous-assed harpies is bent down, her hands grasping onto your calves as she slips her plump-cheeked pussy down onto your ' + Descriptors.cockDescript( y ) + '.  Her sister is back-to-back with her, and her own sweet, rough cunt grinds down onto your ' + Descriptors.cockDescript( x ) + '. Just as you catch the glimpse of this amazing sight, your vision is obscured once more by the pudgy cheeks of the third girl, her rear shifting back and smearing that cum-stained vagina against your mouth. All three girls, once they\'ve established their positions, start to mercilessly pound their chunky buns against your body.  The girls on your two cocks are pumping in opposition.  Just as one clutching, roasting pussy releases its grip on your meat, the other slams down forcefully, giving you no time to catch your breath, even as the fat-assed fowl on your face bounces and jiggled. Your head is savagely slammed against the ground with each wet, splattering rebound.\n\n', false );
		}
		//[(Female)
		else {
			EngineCore.outputText( 'The harpy whose immense ass is pinning your face to the ground raises her dripping cunt from your hungry lips, turning with her feet placed on either side of your head.  She shakes her feathered rear gently with her tail swaying in the breeze. Without so much as a warning, the hungry harpy drops that massive ass HARD onto your face, the impact cushioned pleasantly by the sweet, jelly-like flesh of her honeypot slapping onto your mouth.  Her tight rear passage squishes against your nose, ticklish feathers pressing at your cheeks while her powerful thighs clutch at the sides of your head.  You feel the tense, feathery bodies of the other two harpies clutching at your flanks, their hands sweeping over every inch of your body in pure lust.  ', false );
			//{(If breasts)
			if( CoC.getInstance().player.biggestTitSize() >= 2 ) {
				EngineCore.outputText( 'Their hungry feminine mouths creep beneath the body of the harpy atop you and grab hard onto your ' + Descriptors.breastDescript( 0 ) + '.  They pull them towards their mouths, those gold lips wrapping themselves around your ' + Descriptors.nippleDescript( 0 ) + 's to suckle gently.  ', false );
			}
			//{(If lactating)
			if( CoC.getInstance().player.biggestLactation() >= 1 ) {
				EngineCore.outputText( 'You hear some subtle slurps around the big feathery earmuffs crushing your head, and feel your ' + Descriptors.breastDescript( 0 ) + ' begin to drain, sending motherly shivers through your whole being.  The monster-girls drink and drink, those stubby tongues circling each nipple while the drugged golden lips make you tingle sensually.  ', false );
			}
			EngineCore.outputText( 'Their hands find their way to your ' + Descriptors.vaginaDescript( 0 ) + ', and two sets of fingers start to explore your lush, velvety depths.  They seem intent on making their captive quiver and scream in ecstasy beneath the stifling birdy bottom. The riding harpy leans down, ', false );
			//{(If breasts)
			if( CoC.getInstance().player.biggestTitSize() >= 2 ) {
				EngineCore.outputText( 'her belly squishing your ' + CoC.getInstance().player.allBreastsDescript() + ' beneath her weight, ', false );
			}
			EngineCore.outputText( 'and her drug-laced mouth joins the two intruding fingers in exploring your moist, feminine sex. Even as your ' + Descriptors.vaginaDescript( 0 ) + ' lets out its first wave of sticky girl-cum, your body tightens from the strength of the orgasm.  The harpy\'s stubby tongue drives into you, licking up your hot, sticky fluids.  Those tingly drugs even slip past your nether lips, seeming to warm you from the inside out while her insane birdy tongue thrashes around in your ' + Descriptors.vaginaDescript( 0 ) + '. The other bird-girls\' practiced fingers don\'t let up in the slightest.\n\n', false );
		}
		//[(Male, Herm, Shemale)
		if( x >= 0 && CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( 'They ride you brutally, switching position every now and then to make sure all three harpies get the satisfaction they need. You lose track of the amount of times you\'ve spurted thick, creamy cum into those hot, rough passages', false );
			//{(If herm)
			if( CoC.getInstance().player.gender === 3 ) {
				EngineCore.outputText( ', and the number of times your ' + Descriptors.vaginaDescript( 0 ) + ' has dripped your sweet feminine cum over your thighs', false );
			}
			EngineCore.outputText( '. Every part of your being screams in pain from the hours of relentless hammering by the harpies\' immense, wobbling rears; you think a few bones might even have broken.  Yet every time you came close to blacking out from the pain, the three harpy girls had squawk and trill loudly, abandoning their actions to all lick and slurp at your ' + Descriptors.multiCockDescriptLight() + '.  Their drug-laced kisses pull you back from the brink and allow them to continue their relentless pounding!', false );
		}
		//[(Female)
		else {
			EngineCore.outputText( 'The harpies\' fingers and that firm, buffeting tongue have driven you to orgasm over and over again.  You\'re lying in an immense pool of your own sexual fluids as the bird-girls\' lips pump you full of those lust-inducing drugs, keeping you from blacking out by just a smidgeon.  Those hips batter your head, slamming it repetitively into the ground and dazing you.  The girls rotate, making sure they all have a nice, long turn riding your face and nibbling on your breasts.  ', false );
			//{(If lactating)
			if( CoC.getInstance().player.biggestLactation() >= 1 ) {
				EngineCore.outputText( '  Their bellies seem a little pudgier than when they\'d started, a result of the endless drinking from your ' + Descriptors.nippleDescript( 0 ) + 's.  ', false );
			}
			EngineCore.outputText( '  At long last, all three bird-girls raise their drooling, swollen fuck-holes above your head.  Each of them bares their buxom backsides and feverishly rubs their delicate hands over their clits until, in unison, they cry out with a bone-crushing orgasms.  All three girls\' eager cunts spurt out hot feminine cum over your face, neck and chest.  The effects of their drug-laced kisses start to wear off and your vision swims. You catch a last glimpse of the harpies going through the bags and pockets of your long-discarded ' + CoC.getInstance().player.armorName + ' before the blissful embrace of unconsciousness finally takes you.\n\n', false );
		}
		//[Male, Shemale, Herm]
		if( x >= 0 && CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( 'After countless hours of remorseless, relentless poundings from three heavy bird-butts, even the effects of their drug-laced kisses aren\'t enough to stave off unconsciousness.  You struggle for breath beneath a pair of crushing, feathery thighs as ', false );
			if( CoC.getInstance().player.totalCocks() > 1 ) {
				EngineCore.outputText( 'each of ', false );
			}
			EngineCore.outputText( 'your ' + Descriptors.multiCockDescriptLight() + ' shakes and quivers under the attentions of the merciless bird-girls, releasing a powerful blast of hot, sticky seed, coating the three cruel harpies.  They\'re still rubbing and probing their pussies with delicate fingers while you cum. You hear some angry squawking and a few syllables of recognizable language before you finally give in to unconsciousness, fading into a warm dream of soft, velvet thighs...\n\n', false );
		}
		EngineCore.outputText( 'After a few hours, when you wake, every muscle in your body is aching as though you\'ve just run a marathon. Looking down at your ' + CoC.getInstance().player.legs() + ' in a weary haze, you see signs that even after you\'d blacked out, your body had continued to be abused by the three lust-crazed harpies.', false );
		EngineCore.fatigue( 20 );
		Combat.cleanupAfterCombat();
		CoC.getInstance().player.orgasm();
		EngineCore.dynStats( 'str', -1, 'tou', -1, 'lib', 1, 'sen', 2 );
	};
	HarpyScene.prototype.victoryHarpyGetsHerPussyRaped = function() {
		var x = -1;
		if( CoC.getInstance().player.hasCock() ) {
			x = CoC.getInstance().player.cockThatFits( CoC.getInstance().monster.vaginalCapacity() );
		}
		...
		EngineCore.outputText( '', true );
		EngineCore.outputText( 'You step over the now-submissive feathered beauty and grin, your ' + Descriptors.hipDescript() + ' swaying from side to side idly as you eye up the sweet curves of your latest conquest. Stripping off your ' + CoC.getInstance().player.armorName + ' and tossing it aside carelessly, you waste no time with words, pushing her roughly down to the ground face-down and rubbing your ' + Descriptors.multiCockDescriptLight() + ' in anticipation. She raises her magnificently-plumed head and turns it, looking at you pathetically with a glint of fear in her eye, squawking and crooning as if pleading with you. This passes unnoticed to you, however, as the sight of her thick, fluffy ass and meaty thighs has captured your attention FAR more effectively.\n\n', false );
		EngineCore.outputText( 'Wrapping your arms around those immense, motherly hips, you drag her rump up so that it sways and jiggles in the air, forcing the big-butted bird-girl into the doggy position. Her cheeks burn red with embarrassment, but the way she is jiggling and shaking her blubbery bottom at you tells you she wants it just as much as you do. Finally getting a really good view of it, your eyes widen as you run your hands over every inch of her bouncy rear - it\'s more like standing behind a horse than a harpy! An ass this fine isn\'t to be wasted, and so you lean down and bury your ' + CoC.getInstance().player.face() + ' between those luxurious orbs, planting your mouth firmly over her drooling pussy and tasting her sweet juices, suckling and kissing around her beautifully-formed clit and breathing her intoxicating feminine scent in with enjoyment, letting it fill your lungs as your expert ministrations coax a plethora of moans and subtle squawks from your partner.\n\n', false );
		EngineCore.outputText( 'But as sumptuous as her enormous rump is, you can\'t resist taking her right then and there. The scent from her pussy has taken a serious effect on you as, with an almost feral roar, you break away from her drooling velvet pussy and grasp onto those immense hips, which feel as soft as digging your hands into the most luxurious cushions you can imagine. Guiding her sweet, dribbling fuck-hole onto your ' + Descriptors.cockDescript( x ) + ', you aim to impale the disheveled harpy with all your might, pulling hard on her waist to bury yourself as deep as you can.\n\n', false );
		//(Added bit for multi-dicks)
		if( CoC.getInstance().player.totalCocks() > 1 ) {
			EngineCore.outputText( 'As your head creeps past her tight entrance, your ' + Descriptors.cockDescript( y ) + ' brushes past her hot, clenched anus and a wicked idea forms in your brain. Pulling your ' + Descriptors.cockDescript( x ) + ' free for the time being, you grasp your two lengths of meat in one hand and smear them enthusiastically against her slick honeypot, coating them in slippery-sweet feminine fluids, before trying a second time. You squeeze her plump rump and force those two hot, throbbing cocks deep inside her holes, letting out a breathless moan which is echoed by the stricken harpy as she tries to adjust to the sudden intrusion.\n\n', false );
		}
		EngineCore.outputText( 'You slowly slip your ' + Descriptors.multiCockDescriptLight() + ' into her hot, clutching depths, inch by inch, seeming to take an eternity as all the sensations of her rough, ribbed pussy ', false );
		if( CoC.getInstance().player.totalCocks() > 1 ) {
			EngineCore.outputText( 'and viciously-clenching ass ', false );
		}
		EngineCore.outputText( 'pulse through you. She throws her head back and moans out in sheer, slutty enjoyment as you plumb her luxurious depths, different from anything you\'ve experienced before in its firmness and the powerful, brutal sensations it is giving to you.\n\n', false );
		//(If balls)
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( 'You eventually bottom out, your ' + Descriptors.ballsDescriptLight() + ' coming to rest between her powerful, gelatinous thighs, her soft feathers tickling at your ' + Descriptors.sackDescript() + ' maddeningly.\n\n', false );
		}//(If balls-less)
		else {
			EngineCore.outputText( 'Leaning low over her back, you adjust to drive yourself ever deeper, forcing the whole length of your ' + Descriptors.cockDescript( x ) + ' into her, the soft down on her gelatinous thighs prickling at your thighs maddeningly.\n\n', false );
		}
		if( CoC.getInstance().player.gender === 1 ) {
			//(Male, no breasts)
			if( CoC.getInstance().player.biggestTitSize() < 1 ) {
				EngineCore.outputText( 'Taking care not to waste your opportunity with an immense butt by cumming too soon, you slowly start to push and pull her fat, quivering cheeks up and down your ' + Descriptors.cockDescript( x ) + ', feeling her rough walls and clutching, gasping convulsions massage your manhood', false );
				if( CoC.getInstance().player.totalCocks() > 1 ) {
					EngineCore.outputText( 's', false );
				}
				EngineCore.outputText( ' along ', false );
				if( CoC.getInstance().player.totalCocks() > 1 ) {
					EngineCore.outputText( 'their', false );
				} else {
					EngineCore.outputText( 'the', false );
				}
				EngineCore.outputText( ' whole length, already bringing you close to the edge of a bone-shaking orgasm.\n\n', false );
			}
			//[(Male w/ breasts)
			else {
				EngineCore.outputText( 'You laugh in glee as the bird\'s groping, clutching hole', false );
				if( CoC.getInstance().player.totalCocks() > 1 ) {
					EngineCore.outputText( 's massage', false );
				} else {
					EngineCore.outputText( ' massages', false );
				}
				EngineCore.outputText( ' your ' + Descriptors.cockDescript( x ), false );
				if( CoC.getInstance().player.totalCocks() > 1 ) {
					EngineCore.outputText( ' and ' + Descriptors.cockDescript( y ), false );
				}
				EngineCore.outputText( ' while you knead over your ' + CoC.getInstance().player.allBreastsDescript() + ' hungrily.  You start to pound her tight, harsh hole roughly, groaning and licking your lips, trying to master the power of the sensations her hot, clenching hole are giving to you. You lean down low over her back, your ' + CoC.getInstance().player.allBreastsDescript() + ' pressing against the tickly, prickling feathers, as you bite down onto her shoulder.  Your ' + Descriptors.hipDescript() + ' raise and drop mercilessly into that jiggling, jelly-like ass, giving a noticeable bouncing effect from it as the squishing flesh catapults your rear back into the air with every lewd, slapping thrust.\n\n', false );
			}
		}
		//[(Herm)
		else if( CoC.getInstance().player.gender === 3 ) {
			EngineCore.outputText( 'Grinning and massaging your conquest\'s round, soft ass in anticipation, you gently begin to thrust your ' + Descriptors.hipDescript() + ', moaning loudly as her rough, grasping walls drag over the sensitive flesh of your ' + Descriptors.multiCockDescriptLight() + '. Your ' + Descriptors.vaginaDescript( 0 ) + ' practically spurts fluids out, dribbling down the inside of your thighs as you pump in and out of the helpless harpy.  You love every second you get to spend in her harsh inner walls.\n\n', false );
		}
		EngineCore.outputText( 'It seems the exposed girl is enjoying herself just as much.  Every tiny motion of your ' + Descriptors.hipDescript() + ' causes her to writhe and moan, her moans punctuated with birdlike squawks as she pushes her huge egg-bearing hips back at you in sheer, undisguised desire to have you fuck her harder and deeper.\n\n', false );
		EngineCore.outputText( 'You decide to give her what she so clearly wants and plant both your hands onto her massive bottom, squashing and groping it around your impaling ' + Descriptors.multiCockDescriptLight() + ' as you speed up the pace of your thrusts, driving the full length of your ' + Descriptors.cockDescript( x ), false );
		if( CoC.getInstance().player.totalCocks() > 1 ) {
			EngineCore.outputText( ' and ' + Descriptors.cockDescript( y ), false );
		}
		EngineCore.outputText( ' into her grinding, overwhelming hole', false );
		if( CoC.getInstance().player.totalCocks() > 1 ) {
			EngineCore.outputText( 's', false );
		}
		EngineCore.outputText( '.  ', false );
		//{(If balls)
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( 'Your ' + Descriptors.sackDescript() + ' slaps wetly into her immense, wobbling thighs.  ', false );
		}
		EngineCore.outputText( 'She screams, and thrusts her fat rear back against you, its impact cushioned by the sheer amount of meat on her wide hips.\n\n', false );
		EngineCore.outputText( 'For what seems like hours the pair of you go on, the harpy screaming in ecstasy as she cums over and over again, spattering obscene amounts of sticky feminine cum onto your ' + CoC.getInstance().player.legs() + '. The slippery cum coats the ground beneath you, covering a wider and wider area as your brutal pounding becomes more and more intense.  Finally you throw your head back, scream out in orgasm and pump the whining, quivering harpy girl\'s love tunnel full of your hot, sticky cum.  You keep thrusting yourself in and out to get as much of that hard, grueling pleasure as you can out of her sweet, plump bottom.  It bounces against your crotch each time, as though you were fucking a pair of warm velvet pillows...\n\n', false );
		EngineCore.outputText( 'Once your flow of seed subsides, you pull free of the fat-reared harpy.  She collapses into an orgasm-wracked pile on the ground, her plump ass and tender thighs waving in the air for whomever comes along after you.  You wipe yourself down and continue on your way, pleased with the brutal fuck and looking forward to your NEXT encounter...', false );
		Combat.cleanupAfterCombat();
		CoC.getInstance().player.orgasm();
	};
	HarpyScene.prototype.winAndRapeHarpyAnally = function() {
		...
		EngineCore.outputText( 'Grinning to yourself and stripping free of your ' + CoC.getInstance().player.armorName + ', you lean down and grasp the harpy\'s pointed chin, turning it from side to side, your eyes roaming across her sharp, beautiful features as you inspect your prize. Her body just begs to be touched!  You gaze at those marvelous, tickly feathers coating her slender chest and those awe-inspiring hips, each supporting a fat, jiggling buttock that\'s nearly as large as a centaur\'s whole rear!  Without hesitation, the stricken harpy raises herself and starts to press that sweet, feathery body against yours, squawking and crooning pathetically.', false );
		if( CoC.getInstance().player.biggestTitSize() >= 2 ) {
			EngineCore.outputText( ' Her petite bust squishes against your ' + CoC.getInstance().player.allBreastsDescript() + '. Her downy tits tickle, and grind against your chest maddeningly, tickling and scratching them gently as the lust-fueled bird-girl seems to ache for you.', false );
		}
		EngineCore.outputText( '\n\n', false );
		EngineCore.outputText( 'Not wishing to keep her fantastic butt waiting, you grasp onto her shoulders and pull her back with you, laying back against the hard ground with the squawking avian slut grinding and bouncing her bean-bag ass against your ' + Descriptors.multiCockDescriptLight() + '.  Blood flows through your ' + Descriptors.cockDescript( x ) + ' as it is sandwiched between her hot, caressing, luxurious ass cheeks, and you grind the head brutally against her impossibly tight pucker, which seems to make her panic a bit. She starts squawking and flapping her arm-wings in a vain attempt to escape your grasp, and the imminent butt-fucking she is about to receive! You smirk to yourself as the fluids from her womanhood drool and spill over your entrapped ' + Descriptors.cockDescript( x ) + ', your slowly-pumping ' + Descriptors.hipDescript() + ' smearing her own sticky sexual juices over those downy butt cheeks.\n\n', false );
		EngineCore.outputText( 'The bird-girl\'s attempts to escape only turn you on more, and you grab HARD onto those gelatinous orbs, guiding your ' + Descriptors.cockDescript( x ) + ' into her gloriously tight and hot rear entrance; it feels harsh and rough, practically scratching every inch of your member as it\'s driven hard against her resisting inner walls.  She\'s far tighter and hotter than any human girl could possibly be! Your ' + Descriptors.cockDescript( x ) + ' pushes so viciously and painfully into her clenching tight depths that the harpy squeals and cries out, trying to fly away from you again.  However, she only succeeds in clenching and waving her enormous ass, sending viciously sharp sensations through your blood-engorged dick. Inch by inch you force your ' + Descriptors.cockDescript( x ) + ' into her, her movements getting more frantic as you plumb her most lustful and forbidden hole.  Finally, you bottom out and stop for breath, her powerful, jiggling thighs pressing against yours.', false );
		//(if balls)
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( '  Your ' + Descriptors.ballsDescriptLight() + ' come to rest against her thick, wiggly rear.', false );
		}
		EngineCore.outputText( '\n\n', false );
		EngineCore.outputText( 'The harpy girl seems to freeze above you, wincing and trilling in pain. You are bordering on pain yourself; her crushing ass is so tight and so hot it nearly burns your ' + Descriptors.cockDescript( x ) + '! You smirk and raise your hand, before bringing it down onto her blubbery feminine rear.  With a loud, ringing slap, her ass jiggles as you scream "<i>Giddy-up!</i>"\n\n', false );
		//[(Males, no breasts)
		if( CoC.getInstance().player.biggestTitSize() < 1 && !CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( 'Her pupils constrict, and the feathery fat-reared fowl squawks loudly, going completely crazy atop you! She writhes and bounces this way and that, growing more and more desperate and insane as her pain increases.  Your ' + Descriptors.cockDescript( x ) + ' slams this way and that into her rough, gripping walls, as moans and grunts escape your lips. She flaps frantically from side to side, almost slipping her clutching rump free of your ' + Descriptors.cockDescript( x ) + ' before you grasp hard onto her meaty thighs, forcing yourself back into the distressed harpy with a lewd grin.  It is clear that some part of her is loving this brutal intrusion; her feminine fluids are spilling free, coating your torso with her strong scent.\n\n', false );
		}
		//[(Herms/Shemales)
		else {
			EngineCore.outputText( 'Her pupils constrict, and the feathery fat-reared fowl squawks loudly, going completely crazy atop you! Struggling to escape from your impaling shaft, she drives her clutching, ribbed inner walls into the sides of your throbbing dick, her gelatinous ass wobbling this way and that', false );
			if( CoC.getInstance().player.biggestTitSize() >= 2 ) {
				EngineCore.outputText( ', the motions causing your ' + CoC.getInstance().player.allBreastsDescript() + ' to bounce and sway wildly, slapping into one another lewdly, adding to your growing lust', false );
			}
			EngineCore.outputText( '. You keep a firm grip on her rump and thighs, ensuring that every time she nearly wriggles her hot anus free of your cock, you can drive your hot meat straight back into her lush depths, laughing at her helplessness.', false );
			//(If herm)
			if( CoC.getInstance().player.gender === 3 ) {
				EngineCore.outputText( '  Her struggles are driving you wild, and your ' + Descriptors.vaginaDescript( 0 ) + ' is seeping fluids, growing more and more aroused as she bounces and slamming that immense, rippling rear into you, the force squishing your moist lips together, releasing yet more sticky juices and adding an extra little tone of pleasure.', false );
				//{(If balls)
				if( CoC.getInstance().player.balls > 0 ) {
					EngineCore.outputText( '  She even makes your ' + Descriptors.ballsDescript() + ' slap against your ' + Descriptors.vaginaDescript( 0 ) + ', a hot, moist SPLAT accompanying every tiny motion the harpy makes.', false );
				}
			}
			EngineCore.outputText( '\n\n', false );
		}
		EngineCore.outputText( 'With an ear-splitting squawk, the impaled harpy\'s drooling pussy absolutely explodes.  It fires huge streams of her sticky feminine cum onto your ever-thrusting ' + Descriptors.hipDescript() + ', her muscles locking in ecstasy, before the exhausted bird-girl collapses over you, ', false );
		//(if breasts)
		if( CoC.getInstance().player.biggestTitSize() >= 2 ) {
			EngineCore.outputText( 'her chest mashing against yours as she becomes still, ', false );
		}
		EngineCore.outputText( 'submitting to your will. Growling, you start to fuck her in earnest and grab a handful of her squishy rear in each hand, pounding her up and down on your ' + Descriptors.cockDescript( x ) + '. Though the harpy has collapsed, her body is still panicking, forcing her rough inner passage to clutch and massage your shaft, feeling harsh and grating, but oh-so wonderful! Pathetic squawks and trills of pain escape her golden lips as you roughly pound her tender ass, and her hips pound back against you, ass bouncing on your lap with a dull SLAP every time, which spatters pre-cum across your legs as well as her enormous thighs.\n\n', false );
		EngineCore.outputText( 'Throwing your head back and crying out in sheer lust-consumed ecstasy, you hilt your ' + Descriptors.cockDescript( x ) + ' deep inside her thick, tight anus, releasing a flood of hot, sticky seed into her butt. The sudden surge of cum into her rough depths causes the exhausted harpy to tense up once more, and she releases a second, more pathetic wave of her fluids, a little less than last time. Her creamy feminine cum leaks out over your thighs as you lock with her. Your eyes roll back from the intensity of your orgasm, and you bite down on your bottom lip; for what seems like weeks you cum into her plump rump, her hands pressed on your chest as if trying to push you away. Eventually, your flow subsides, and the harpy collapses on top of you, sound asleep.\n\n', false );
		EngineCore.outputText( 'Still leaking seed, you pull free of her gaping ass, and dump the worthless bird-slut on the floor in a heap. Passed out and woefully exposed, her lush holes await whatever horny beast or demon will come alone after you.', false );
		Combat.cleanupAfterCombat();
		CoC.getInstance().player.orgasm();
	};

	HarpyScene.prototype.WinOnHarpyAndOralRape = function() {
		EngineCore.outputText( '', true );
		var x = -1;
		if( CoC.getInstance().player.hasCock() ) {
			x = CoC.getInstance().player.cockThatFits( CoC.getInstance().monster.analCapacity() );
		}
		var y = x + 1;
		if( y === x && CoC.getInstance().player.totalCocks() > 1 ) {
			//If second cock is out of bounds, choose one in bounds
		...
		EngineCore.outputText( 'You stand threateningly over the helpless harpy, stroking yourself through your ' + CoC.getInstance().player.armorName + ' before grasping onto her chin, turning it this way and that, inspecting your prize thoroughly. Those golden lips glisten in the light as she trills feebly, her eyes looking soulfully up at you.\n\n', false );
		EngineCore.outputText( 'Grinning lewdly you squeeze her cheeks, the stricken harpy opening her mouth obediently. Her tongue is short and stubby like that of a normal bird, glistening with saliva and looking terribly inviting. You strip off the lower portion of your ' + CoC.getInstance().player.armorName, false );
		//(if pussy)
		if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( ', fluids seeping from your plush ' + Descriptors.vaginaDescript( 0 ) + ' all the while', false );
		}
		//(if cock)
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( ', ', false );
			...
			EngineCore.outputText( 'your ' + Descriptors.multiCockDescriptLight() + ' jumps to attention, throbbing and begging to be touched', false );
		}
		EngineCore.outputText( '. Grasping onto the harpy\'s head-feathers, you lean down close and wrap your lips around her golden mouth, passionately driving your tongue into the wet opening.  You explore every little corner of her moist, tender orifice. Her tongue presses back against yours, tentatively at first', false );
		//(if boobs)
		if( CoC.getInstance().player.biggestTitSize() >= 2 ) {
			EngineCore.outputText( ', but soon she presses her toned, petite chest against your ' + CoC.getInstance().player.allBreastsDescript() + ', that stubby tongue tangling around yours lustily.', false );
		} else {
			EngineCore.outputText( ', but soon she is pressing against you, rubbing her toned body against yours, driving her tongue back against yours in a haze of lust, turning her head quickly and sharply, birdlike.', false );
		}
		EngineCore.outputText( '\n\n', false );
		//[(Male, shemale, herm, one cock)
		if( CoC.getInstance().player.cockTotal() === 1 ) {
			EngineCore.outputText( 'Breaking the kiss, your lips tingle sharply as you realize her golden lipstick is drugged! The heat flows to your groin, your ' + Descriptors.multiCockDescriptLight() + ' swelling noticeably and growing even more sensitive.  Even the slightest breeze feels like a lover\'s hands passing across the tender flesh. Almost apologetically, the harpy leans in to wrap her hands around your ' + Descriptors.cockDescript( 0 ) + ' those drugged lips brushing sensuously across your cock-tip. Her beautifully formed hands give your shaft a squeeze, sending a wave of pleasure through you that brings you to your knees.  You throw both hands out behind you as you lean away from her, pushing your ' + Descriptors.hipDescript() + ' towards her. The submissive bird-girl waggles her huge, jiggling ass behind her, softly guiding the very tip of your ' + Descriptors.cockDescript( 0 ) + ' between those tainted lips, that tingling sensation slipping down your hyper-sensitive member. Her mouth is warm, soft and suckling while her short tongue is flicking frantically across the underside of your shaft, the pleasure being harsh but oh-so intense.', false );
			//(if herm)
			if( CoC.getInstance().player.gender === 3 ) {
				EngineCore.outputText( '  Her expert fingers tease up the sides of your thighs, coming to softly brush across your moist nether lips, her fingers spreading your ' + Descriptors.vaginaDescript( 0 ) + ' wide before plunging into the warmth within, adding another sweet sensation of pleasure to the amazing orchestra of sensations her lips were giving to you.', false );
			}
			//(If balls)
			if( CoC.getInstance().player.balls > 0 ) {
				EngineCore.outputText( '  Her hands creep down to softly squeeze and massage your ' + Descriptors.ballsDescriptLight() + ', the harpy\'s eager head bobbing back and forth along the sweet tasting shaft of your ' + Descriptors.cockDescript( 0 ) + ', sending waves of powerful, overwhelming pleasure resonating through your body.', false );
			}
			EngineCore.outputText( '\n\n', false );
		}
		//[(Male, Shemale, Herm, multicock)
		else if( CoC.getInstance().player.cockTotal() > 1 ) {
			EngineCore.outputText( 'You break the kiss, the soft tingle lingering on your lips for longer than it should... much longer. Her golden lipstick is drugged! The tingle spreads over your whole body', false );
			//if breasts)
			if( CoC.getInstance().player.biggestTitSize() >= 2 ) {
				EngineCore.outputText( ', and your ' + CoC.getInstance().player.allBreastsDescript() + ' jiggle slightly from the sheer intensity of the pins-and-needles feeling coating your body.  Your ' + Descriptors.nippleDescript( 0 ) + 's perk up and harden.', false );
				if( CoC.getInstance().player.biggestLactation() >= 1 ) {
					EngineCore.outputText( '  Milk dribbles out gently onto the areola, trickling sensually between your cleavage.', false );
				}
			} else {
				EngineCore.outputText( '.', false );
			}
			EngineCore.outputText( '  Surging down your body, an alien sensation focuses on your crotch, the pinpricks conglomerating on your ' + Descriptors.multiCockDescriptLight() + ', and you fall to your knees, hunched over and quivering from the powerful orgasmic sensations. The big-bottomed harpy creeps forward on her knees; her eyes gaze expressively at your face. One arm reaches out to wrap her sweet hand around your ' + Descriptors.cockDescript( 0 ) + '. As she pumps it gently, the pleasure is magnified tenfold by the lingering effects of those treacherous golden lips.', false );
			//(If balls)
			if( CoC.getInstance().player.balls > 0 ) {
				EngineCore.outputText( '  She moves that hand to softly squeeze and pet at your ' + Descriptors.ballsDescriptLight() + ', massaging those bloated cum-factories with expertise. You can feel a distinct rumble as your ' + Descriptors.sackDescript() + ' really ups its production, almost feeling heavier.', false );
			}
			//(If herm)
			if( CoC.getInstance().player.gender === 3 ) {
				EngineCore.outputText( '  Her other perfect hand creeps beneath your tense ' + Descriptors.multiCockDescriptLight() + ' to the ' + Descriptors.vaginaDescript( 0 ) + ' that\'s nestled between your thighs. At the first touch of her hand, an explosive ecstasy shoots through you, forcing you to tense up and arch your back.  You grit your teeth while puissant waves of orgasmic glee flood through you.', false );
			}
			EngineCore.outputText( '\n\n', false );
		}
		//(Female)
		if( CoC.getInstance().player.gender === 2 ) {
			EngineCore.outputText( 'Tearing your lips away from hers, you feel a distinct, sharp tingle across your mouth and cheeks as you realized that her shining, gilded lipstick was drugged! The tingle spreads through your body, prickling across your whole body, flowing inexorably in a wave of sensation towards your groin, the ticklish feeling concentrating on your ' + Descriptors.vaginaDescript( 0 ) + '. You fall to your knees at the sheer unbridled pleasure shooting up and down your body, the thick hipped harpy crawling towards you meekly, applying a gentle pressure to your shoulder and pressing your back to the ground, trilling and squawking quietly, her perfectly formed hand reaching to your overwhelmed pussy. At the slightest first touch of her hand an explosive ecstasy shoots through you, forcing you to tense up and arch your back, teeth gritted. ', false );
			//(If breasts)
			if( CoC.getInstance().player.biggestTitSize() >= 2 ) {
				EngineCore.outputText( 'You grasp onto your ' + CoC.getInstance().player.allBreastsDescript() + ' violently, kneading and tweaking your ' + Descriptors.nippleDescript( 0 ) + ' in a horny haze as the harpy\'s fingers tease around your pussy.  ', false );
			}
			EngineCore.outputText( 'She parts your thighs gently, her eyes focused on yours as her hands come together to tease at your ' + Descriptors.vaginaDescript( 0 ) + ', gently lowering her magnificently-plumed head to plant those drug-laced lips on your sweet, seeping pussy. Her stubby tongue tickles around your ' + Descriptors.clitDescript() + ' before plunging hungrily into your warm depths. The heightened sensitivity of your ' + Descriptors.vaginaDescript( 0 ) + ' means that every tiny flick of the bird\'s tongue coaxes a violent muscular spasm as you struggle to deal with the overwhelming sensations, her expert ministrations keeping you just on the edge of orgasm, but never driving you over the edge.\n\n', false );
		}
		//[(Male, Shemale, Herm, multicock, cont. from last paragraph)
		if( CoC.getInstance().player.cockTotal() > 1 ) {
			EngineCore.outputText( 'The harpy isn\'t finished yet; she grasps your ' + Descriptors.cockDescript( 0 ) + ' with one hand and takes your ' + Descriptors.cockDescript( 1 ) + ' in the other, pressing them against one another. Little pinpricks of pleasure course through your groin.  She forces both of those quivering cocks into her aphrodisiac-tainted lips, working them into her well-practiced throat. As the suction mashes them together pleasurably, her tight throat\'s grip on your dicks forces them to grind together. The feelings are just incredible, especially since her drugged lips are still forcing chemicals into your body, magnifying the intensity of the blissful sensations by the second.\n\n', false );
		}
		//(Male, Shemale, Herm, one cock)
		if( CoC.getInstance().player.cockTotal() === 1 ) {
			EngineCore.outputText( 'The horny harpy slips one hand between her fat thighs, digging her fingers into her plush, leaking pussy', false );
			//(if herm)
			if( CoC.getInstance().player.gender === 3 ) {
				EngineCore.outputText( ', her other hand doing the exact same to your own ' + Descriptors.vaginaDescript( 0 ) + ', digging those perfect fingers deep into the wet, hyper-sensitive flesh, spreading those lips wide', false );
			}
			EngineCore.outputText( '.  Her mouth greedily gobbles up inch after inch of your ' + Descriptors.cockDescript( 0 ) + ', until at last she bottoms out, pressing her nose against your groin, those tainted lips tingling all the way down, increasing the intensity of the pleasure by the second. It takes a surprisingly short time of the harpy bobbing her head, that stubby tongue wetly caressing the sweet shaft of your ' + Descriptors.cockDescript( 0 ) + ', before you let out a vast, quaking moan, blasting a much larger load than normal into her gentle throat. She doesn\'t stop though, quite the opposite in fact, as her attentions to your dick became even more aggressive. Her submissive side all but vanishes as her golden lips brush up and down the hot shaft of that ' + Descriptors.cockDescript( 0 ) + '.  ', false );
			//(If balls)
			if( CoC.getInstance().player.balls > 0 && CoC.getInstance().player.cocks[ 0 ].cockLength <= 16 ) {
				EngineCore.outputText( 'She even deep throats you, taking your length into her expertly-controlled mouth, that tongue creeping out to tickle at your ' + Descriptors.ballsDescriptLight() + '.  ', false );
			}
			EngineCore.outputText( 'More slutty trills and squawks creep from her tainted lips, muffled by the girth of meat plugged so perfectly into her sucking, slurping mouth. For nearly an hour she keeps milking you of your seed, and you cum a full six times into her incredible mouth before she finally pops free, gasping for her breath with her fat, wobbling thighs coated in her feminine cum. How many times she orgasmed while giving herself to you was a mystery, but she bears down on you with a smile and a slightly-bulging belly. You flinch as she dives at your head, but it was merely to deliver a soft, drugged kiss to your forehead, which lulls you into sleep just as she turns on her heel and takes wing.\n\n', false );
		}
		//(Female)
		if( CoC.getInstance().player.gender === 2 ) {
			EngineCore.outputText( 'Her approach changes quickly however, as her stubby tongue soon starts to flick rapidly across your ' + Descriptors.clitDescript() + ' the shuddering, powerful sensations rendering you powerless on the ground before the horny harpy. More aphrodisiac slowly seeps into your body as you gasp, your hand reaching down to mash her finely-feathered face into your ' + Descriptors.vaginaDescript( 0 ) + ', forcing the big-bottomed bird to probe deeper into your cunt. For an hour you keep her face buried between your thighs, your ' + Descriptors.hipDescript() + ' raising and falling uncontrollably, her tongue thrashing around inside your lush depths more and more desperately, compelling you to cum over and over onto her beautiful face. For the final time you explode, releasing a massive load of sticky girl-cum onto her face before releasing her, the harpy recoiling. Her tongue licks its way across those golden lips, her pudgy thighs coated with her own cum. Your vision swims as you wonder whether this thick-hipped beauty came from the simple act of serving you so submissively, as she clambers over your body to plant a long, loving kiss on your lips, tangling her stumpy bird-tongue with yours as the last dose of those tainted lips slips into your brain, carrying you off to sleep. You are left with just one last glimpse of her fine, feathered ass before blacking out.\n\n', false );
		}
		//[(Male, herm, shemale, multi-dick)
		if( CoC.getInstance().player.cockTotal() > 1 ) {
			EngineCore.outputText( 'The lusty harpy continues to suck and swirl your cocks in her tight, hot mouth and throat, your ' + Descriptors.cockDescript( 0 ) + ' and ' + Descriptors.cockDescript( 1 ) + ' brushing across one another, lubricated by her slippery saliva. The pair of shafts stretch her jaw to the absolute limit, with ropes of pre-cum and saliva drooling from her golden lips', false );
			//(if balls)
			if( CoC.getInstance().player.balls > 0 ) {
				EngineCore.outputText( ', dribbling all over your ' + Descriptors.sackDescript(), false );
			}
			EngineCore.outputText( '.  You cum over and over again under the powerful ministrations of her drooling mouth, the horny harpy\'s tainted lips coaxing even more sticky seed out of you than usual.', false );
			//(If herm)
			if( CoC.getInstance().player.gender === 3 ) {
				EngineCore.outputText( '  Her fingers never stop their sweet teasing of your ' + Descriptors.vaginaDescript( 0 ) + ', using all four fingers on her hand to plumb your velvet depths.', false );
			}
	...
	HarpyScene.prototype.harpyScissorSurprise = function() {
		...
		if( CoC.getInstance().monster.HP < 1 ) {
			...
			EngineCore.outputText( 'she leans back from her kneeling position, spreading her legs and digging her fingers into her soaking cunt. Her other hand darts up to her ample bosom, giving it a hard squeeze before starting to tease her erect nipple, pinching and flicking as she continues to fingerfuck herself ever more forcefully.' );
		}
		...
		if( CoC.getInstance().monster.HP < 1 ) {
			EngineCore.outputText( 'vulnerable' );
		} else {
			EngineCore.outputText( 'lust-mad' );
		}
		EngineCore.outputText( ' harpy.' );
		...
		if( CoC.getInstance().monster.HP < 1 ) {
			EngineCore.outputText( 'worry' );
		} else {
			EngineCore.outputText( 'desire' );
		}
		EngineCore.outputText( ', but before she can even begin to talk you grab the mess of short hair atop her head and yank her forwards, roughly grinding her face into your soaked pussy[if (isHerm = true) ", leaving your stiff cock resting on her head where the stimulation from her feathery hair causes it to steadily drip pre-cum down the back of her neck"].  She struggles furiously, her hands shooting up to your thighs as she tries to push away from your slick, aching entrance.  Your grip is far too strong and she\'s too exhausted from the fight though, and you demonstrate the pointlessness of her attempts at freedom by slowly dragging her face up and down against your [vagina], smearing even more femcum over her increasingly distraught visage.' );
	...
	HarpyScene.prototype.spoidahsLegEggsInHarpeis = function() {
		...
		} else if( CoC.getInstance().player.tallness === 78 ) {
			EngineCore.outputText( 'forward' );
		} else {
			EngineCore.outputText( 'down' );
		}
		EngineCore.outputText( ', eyes closed and lips puckered seductively, ready to deliver a heady dose of her drugged lipstick.' );
		...
		if( CoC.getInstance().player.gender > 0 ) {
			...
			EngineCore.outputText( ', enjoying the view.' );
		}
	...
	HarpyScene.prototype.clitFuckAHarpy = function() {
		...
		if( CoC.getInstance().player.wetness() < 3 ) {
			EngineCore.outputText( '  Runnels of your lubricant are dripping down your [legs] in streams,' );
		} else if( CoC.getInstance().player.wetness() < 4 ) {
			EngineCore.outputText( '  Streams of lubricant are soaking down your [legs] in waves,' );
		} else {
			EngineCore.outputText( '  Squirts of lubricant are gushing down your [legs] to puddle on the floor,' );
		}
		EngineCore.outputText( ' evidence of just how hungry your clenching, empty hole is.' );
		...
		if( CoC.getInstance().player.wetness() < 3 ) {
			EngineCore.outputText( '  A fresh gush of fluid washes out of your pussy at the peak of your pleasure even though you\'re far from a squirter' );
		} else if( CoC.getInstance().player.wetness() < 4 ) {
			EngineCore.outputText( '  A fresh spout of girlspunk washes out of your pussy at the peak of your pleasure' );
		} else {
			EngineCore.outputText( '  A torrential splash of girlspunk shoots out of your pussy in a river at the peak of your pleasure' );
		}
		EngineCore.outputText( ', timed with a full-body seizure that causes you and your partner to slump down into sticky pile.' );
		...
		if( CoC.getInstance().player.tou / 20 + Utils.rand( 20 ) < 6 ) {
			EngineCore.outputText( ' your endurance gives out, and you slip into unconsciousness.  This time it seems you met your sexual match.' );
			CoC.getInstance().monster.lust = 98;
			CoC.getInstance().monster.HP = 2;
			CoC.getInstance().player.lust = 100;
			CoC.getInstance().flags[ kFLAGS.COMBAT_BONUS_XP_VALUE ] = CoC.getInstance().monster.XP;
			...
			EngineCore.outputText( ' the harpy\'s eyes close, and she slips into unconsciousness.  You give her round bottom a little pat before you depart, still shivering from the cascades of pleasure it brought you.' );
			CoC.getInstance().player.orgasm();
