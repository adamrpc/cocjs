...
angular.module( 'cocjs' ).run( function( SceneLib, EngineCore, CoC, kFLAGS, Combat, Descriptors, Utils, AppearanceDefs ) {
	...
	HarpyScene.prototype.harpyVictoryuuuuu = function() {
		...
		var scissor = null;
		var clitFuck = null;
		if( CoC.player.hasVagina() && CoC.player.clitLength >= 3.5 ) {
	...
	HarpyScene.prototype.harpyLossLust = function() {
		...
		if( CoC.player.hasVagina() && x < 0 ) {
			...
			EngineCore.outputText( 'You feel your ' + CoC.player.legs() + ' being lifted up and out of the way. Feathered arms curl around either side of your thighs to keep them in place as the harpy teasing and suckling your ' + Descriptors.clitDescript() + ' gives the last harpy enough room to scoot in below her. A teasing of soft harpy hair feathers against your ' + Descriptors.buttDescript() + ' is the only warning you receive before you feel the wet warmth of that final black tongue pressing against your ' + Descriptors.assholeDescript() + '.\n\n', false );
	...
	HarpyScene.prototype.harpyDamageLoss = function() {
		...
		if( y >= CoC.player.cocks.length ) {
			y -= 2;
		}
		if( y === x && CoC.player.totalCocks() > 1 ) {
			if( y === 0 ) {
				y++;
			} else {
				y--;
		...
		if( CoC.player.hasCock() && x >= 0 ) {
			...
			if( !CoC.player.hasVagina() ) {
				...
				if( CoC.player.biggestTitSize() >= 1 ) {
					EngineCore.outputText( 'her petite bosom grinding across your ' + CoC.player.allBreastsDescript() + ', her nipples tickling at your ' + Descriptors.nippleDescript( 0 ) + ', ', false );
				}
			...
			//[(Herm)
				...
				if( CoC.player.biggestTitSize() >= 2 ) {
					EngineCore.outputText( ', causing your ' + CoC.player.allBreastsDescript() + ' to bounce and sway violently', false );
				}
		...
		//[(Female)
			...
			if( CoC.player.biggestTitSize() >= 3 ) {
				EngineCore.outputText( ' and causing your ' + CoC.player.allBreastsDescript() + ' to jiggle, slapping into one another', false );
			}
		...
		if( x >= 0 && CoC.player.cockTotal() === 1 ) {
			...
			if( CoC.player.balls >= 0 ) {
				...
				EngineCore.outputText( ' while your ' + CoC.player.allBreastsDescript() + ' are shaking and jiggling around helplessly.', false );
			} else {
				EngineCore.outputText( '.', false );
			}
		...
		else if( x >= 0 && CoC.player.cockTotal() > 1 ) {
		...
		//[(Female)
			...
			//{(If breasts)
			if( CoC.player.biggestTitSize() >= 2 ) {
				EngineCore.outputText( 'her belly squishing your ' + CoC.player.allBreastsDescript() + ' beneath her weight, ', false );
			}
		...
		EngineCore.outputText( 'After a few hours, when you wake, every muscle in your body is aching as though you\'ve just run a marathon. Looking down at your ' + CoC.player.legs() + ' in a weary haze, you see signs that even after you\'d blacked out, your body had continued to be abused by the three lust-crazed harpies.', false );
	...
	HarpyScene.prototype.victoryHarpyGetsHerPussyRaped = function() {
		...
		if( CoC.player.gender === 1 ) {
			...
			//[(Male w/ breasts)
				...
				EngineCore.outputText( ' while you knead over your ' + CoC.player.allBreastsDescript() + ' hungrily.  You start to pound her tight, harsh hole roughly, groaning and licking your lips, trying to master the power of the sensations her hot, clenching hole are giving to you. You lean down low over her back, your ' + CoC.player.allBreastsDescript() + ' pressing against the tickly, prickling feathers, as you bite down onto her shoulder.  Your ' + Descriptors.hipDescript() + ' raise and drop mercilessly into that jiggling, jelly-like ass, giving a noticeable bouncing effect from it as the squishing flesh catapults your rear back into the air with every lewd, slapping thrust.\n\n', false );
			}
		}
		//[(Herm)
	...
	HarpyScene.prototype.winAndRapeHarpyAnally = function() {
		...
		//[(Males, no breasts)
		if( CoC.player.biggestTitSize() < 1 && !CoC.player.hasVagina() ) {
		...
		//[(Herms/Shemales)
			...
			if( CoC.player.biggestTitSize() >= 2 ) {
				EngineCore.outputText( ', the motions causing your ' + CoC.player.allBreastsDescript() + ' to bounce and sway wildly, slapping into one another lewdly, adding to your growing lust', false );
			}
	...
	HarpyScene.prototype.WinOnHarpyAndOralRape = function() {
		...
		var x = -1;
		if( CoC.player.hasCock() ) {
			x = CoC.player.cockThatFits( CoC.monster.analCapacity() );
		}
		var y = x + 1;
		if( y === x && CoC.player.totalCocks() > 1 ) {
			//If second cock is out of bounds, choose one in bounds
		...
		EngineCore.outputText( 'You stand threateningly over the helpless harpy, stroking yourself through your ' + CoC.player.armorName + ' before grasping onto her chin, turning it this way and that, inspecting your prize thoroughly. Those golden lips glisten in the light as she trills feebly, her eyes looking soulfully up at you.\n\n', false );
		EngineCore.outputText( 'Grinning lewdly you squeeze her cheeks, the stricken harpy opening her mouth obediently. Her tongue is short and stubby like that of a normal bird, glistening with saliva and looking terribly inviting. You strip off the lower portion of your ' + CoC.player.armorName, false );
		//(if pussy)
		if( CoC.player.hasVagina() ) {
		...
		//(if boobs)
		if( CoC.player.biggestTitSize() >= 2 ) {
			EngineCore.outputText( ', but soon she presses her toned, petite chest against your ' + CoC.player.allBreastsDescript() + ', that stubby tongue tangling around yours lustily.', false );
		} else {
		...
		//[(Male, Shemale, Herm, multicock)
			...
			//if breasts)
			if( CoC.player.biggestTitSize() >= 2 ) {
				EngineCore.outputText( ', and your ' + CoC.player.allBreastsDescript() + ' jiggle slightly from the sheer intensity of the pins-and-needles feeling coating your body.  Your ' + Descriptors.nippleDescript( 0 ) + 's perk up and harden.', false );
				if( CoC.player.biggestLactation() >= 1 ) {
		...
		if( CoC.player.gender === 2 ) {
			...
			//(If breasts)
			if( CoC.player.biggestTitSize() >= 2 ) {
				EngineCore.outputText( 'You grasp onto your ' + CoC.player.allBreastsDescript() + ' violently, kneading and tweaking your ' + Descriptors.nippleDescript( 0 ) + ' in a horny haze as the harpy\'s fingers tease around your pussy.  ', false );
			}
		...
		if( CoC.player.cockTotal() === 1 ) {
			...
			//(If balls)
			if( CoC.player.balls > 0 && CoC.player.cocks[ 0 ].cockLength <= 16 ) {
	...
	HarpyScene.prototype.clitFuckAHarpy = function() {
		...
		if( CoC.player.tou / 20 + Utils.rand( 20 ) < 6 ) {
