...
angular.module( 'cocjs' ).run( function( SceneLib, MainView, kFLAGS, CoC, AppearanceDefs, PerkLib, StatusAffects, EngineCore, Descriptors, Combat, HellHound, PregnancyStore, CockTypesEnum ) {
	...
	/*
	 ...
	 Raped by style, using both dicks if the foe has two holes.
	 Rape:
	...
	HellHoundScene.prototype.hellhoundRapesPlayer = function() {
		...
		if( CoC.player.lust >= 100 ) {
			MainView.outputText( 'Overcome by sexual desire, you submit to the hungry hellhound.\n\n', true );
		} else {
			MainView.outputText( 'Too weak to continue fighting, you fall to your knees.\n\n', true );
		}
		MainView.outputText( 'The heads of the hellhound grin with desire as he rises onto his hind legs. As the hellhound stands before you, your eyes widen at the sight of its equipment pair of eight-inch long black rods, one above the other, with a sack containing a quad of flaming balls underneath. As the hound swaggers closer, you gulp at the thought of what those fiery balls are going to be pumping into you. You are helpless against the hellhound as he pushes you onto your ', false );
		if( CoC.player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_CENTAUR ) {
			MainView.outputText( 'hocks and knees', false );
		} else {
			MainView.outputText( 'hands and knees', false );
		}
		MainView.outputText( ' and positions himself behind you. He grips you with his clawed hands on your shoulders and easily holds you in place. The powerful creature is easily able to get past your ' + CoC.player.armorName + ' and you feel one of his rods brush against your ' + Descriptors.buttDescript() + '.  A shudder shoots through your body as he brings his two heads down to either side of your own, his hot canine breath blowing past your ears. He stays there for a few moments, completely still... before suddenly pulling his heads back and ', false );
		if( CoC.player.vaginas.length > 0 && CoC.player.lowerBody !== AppearanceDefs.LOWER_BODY_TYPE_NAGA ) {
			MainView.outputText( 'slamming his twin dicks into your ' + Descriptors.vaginaDescript( 0 ) + ' and your ' + Descriptors.assholeDescript() + '.  ', false );
		} else {
			MainView.outputText( 'slamming his lower dick into your ' + Descriptors.assholeDescript() + ', while rubbing the other between the cheeks of your ' + Descriptors.buttDescript() + '.  ', false );
		}
		if( CoC.player.vaginas.length > 0 && CoC.player.lowerBody !== AppearanceDefs.LOWER_BODY_TYPE_NAGA ) {
			if( CoC.player.vaginalCapacity() < CoC.monster.cockArea( 0 ) ) {
				MainView.outputText( 'You feel an intense mixture of sensations in your lower body as your ' + Descriptors.assholeDescript() + ' feels like it is being torn apart, while your ' + Descriptors.vaginaDescript( 0 ) + ' is filled with an intense pleasure at being filled with the creature\'s large member.  ', false );
			} else {
				MainView.outputText( 'Your lower body explodes with pain as the hellhound forces himself in too quickly for either your ' + Descriptors.assholeDescript() + ' or your ' + Descriptors.vaginaDescript( 0 ) + ' to handle.  ', false );
			}
			if( CoC.player.vaginas[ 0 ].virgin ) {
				MainView.outputText( '<b>You are no longer a virgin!  </b>', false );
			}
			if( CoC.player.cuntChange( CoC.monster.cockArea( 0 ), false ) ) {
				MainView.outputText( 'The beast howls as your ' + Descriptors.vaginaDescript( 0 ) + ' is stretched to accommodate the large shaft.  ', false );
			}
			MainView.outputText( 'The hellhound pants for a few seconds before continuing.  ', false );
			if( CoC.player.vaginas[ 0 ].vaginalWetness < AppearanceDefs.VAGINA_WETNESS_WET ) {
				MainView.outputText( 'The pause gives your ' + Descriptors.vaginaDescript( 0 ) + ' time to moisten, ensuring the next thrust won\'t be quite as rough.  ', false );
			}
			MainView.outputText( 'This time the beast starts pumping in and out more gently, only a short distance at a time as the hellhound continues panting.  As the pain in your ' + Descriptors.assholeDescript() + ' fades, you start feeling pleasure to match the sensations rising from your ' + Descriptors.vaginaDescript( 0 ) + '. Each thrust of the beast\'s twin manhoods bring you closer and closer to your peak.\n\n', false );
			//Cum
			MainView.outputText( 'As you reach your climax, the beast howls and you feel its incredibly hot seed pour into you.  ', false );
			if( CoC.player.cor < 40 ) {
				MainView.outputText( 'You moan as your insides begin to heat up. The uncomfortably hot sensation only grows as more and more of its fiery seed is pumped into your body. After what feels like an eternity, the beast pulls out of you. He gives your ' + Descriptors.vaginaDescript( 0 ) + ' and your ' + Descriptors.assholeDescript() + ' a single extended lick with its long dog-like tongue before running off out of sight. The tainted heat inside you proves to be too much and you pass out. After some time passes, you wake up to find the corrupt warmth inside you has thankfully faded away. You\'re able to stand up again, but the damage is done and the creature\'s seed has left you feeling rather weak.', false );
			} else {
				MainView.outputText( 'His flaming seed brings about a pleasure you had not expected; your insides feel like they are burning with passion and power.  It is an incredible and fiery experience, one that you don\'t think you could have had if it wasn\'t for the power of corruption that you\'ve gained since you got here. Too soon, the beast pulls out of you.  He gives your ' + Descriptors.vaginaDescript( 0 ) + ' and your ' + Descriptors.assholeDescript() + ' a single extended lick with his long dog-like tongue before he runs off. You quickly look over and manage to catch a glimpse of its tail before it disappears from view, just before your body falls into a deep sleep. When you wake, you can still feel the afterglow of the hot seed inside you.', false );
			}
			//Preggers chance!
			CoC.player.knockUp( PregnancyStore.PREGNANCY_HELL_HOUND, PregnancyStore.INCUBATION_HELL_HOUND, 101 );
		} else {
			MainView.outputText( 'Your lower body explodes with pain as the hellhound forces himself in too quickly for your ' + Descriptors.assholeDescript() + ' to handle. He pauses and pants for a few seconds, but you barely have enough time to catch your breath before the beast starts thrusting in and out again. However, this time the beast starts pumping in and out more gently, only a short distance at a time as the hellhound continues panting.  As the pain in your ' + Descriptors.assholeDescript() + ' fades, you start feeling strong pleasure from it. Each thrust of the beast\'s lower manhood, combined with the upper one tickling and teasing your ' + Descriptors.buttDescript() + ' above it brings you closer and closer to your peak.\n\n', false );
			//Cum
			MainView.outputText( 'As you reach your climax, the beast howls and you feel its incredibly hot seed pour into you and onto your back.  ', false );
			if( CoC.player.cor < 40 ) {
				MainView.outputText( 'You moan while trying to break free to get the heat off your back... but the beast keeps a firm grip on you as more and more of its fiery seed is pumped into and onto your body. After what feels like an eternity, the beast pulls out of you. He gives your ' + Descriptors.assholeDescript() + ' a single extended lick with his long dog-like tongue before running off out of sight. You are finally able to roll around and get the heat off of your back, but you can do nothing about the tainted warmth inside you. The heat proves to be too much and you pass out. After some time passes, you wake up to find the fire inside you has thankfully faded away. You\'re able to stand up again, but the damage is done and the creature\'s seed has left you feeling rather weak.', false );
			} else {
	...
	HellHoundScene.prototype.hellHoundGetsRaped = function() {
		...
		if( CoC.player.gender === 0 ) {
			MainView.outputText( 'The two heads stare at your lack of sexual endowments for a few seconds before looking up at you sadly.  With a whine, the hellhound wanders off.  Feeling rather unsatisfied, you think that you should probably fix your lack of sexual \'parts\'...', false );
		} else {
			CoC.player.orgasm();
		}
		Combat.cleanupAfterCombat();
	...
	HellHoundScene.prototype.HellHoundMasterEncounter = function() {
		MainView.outputText( '', true );
		//Initial encounter (A)
		...
		if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00141 ] === 0 ) {
			MainView.outputText( 'As you\'re wandering the mountains, you feel an odd presence in the back of your mind.  It calls out to you with promise of power and safety, but part of you feels a bit apprehensive at the prospect of giving yourself over to it.  Do you let the presence take over?', false );
	...
	//choose no (B)
	HellHoundScene.prototype.declineCrazyPresences = function() {
		MainView.outputText( '', true );
		MainView.outputText( 'You force the presence out of your mind.  You feel almost a bit lost after it disappears, but giving yourself over to foreign control can never be a good idea, can it?', false );
	...
	HellHoundScene.prototype.declineGivingAwayLethicite = function() {
		MainView.outputText( '', true );
		MainView.outputText( 'You push the presence out of your mind.  Maybe later you\'ll collect the hellfire, but for now you\'d rather keep the lethicite.', false );
	...
	//Regular Lethicite
	HellHoundScene.prototype.giveLethicitePiece = function() {
		MainView.outputText( '', true );
		//Give a single piece of Lethicite, get hellfire
		MainView.outputText( 'You produce the pink crystal and hand it to the master.  As the crystal touches his hand, he gives a low growl of pleasure and holds the crystal up to his eyes.  "<i>Ah yes, excellent!  Thank you so much for this tasty treat of power.</i>"\n\n', false );
		//if (PC gave ⅔ Merae's crystal)
		if( CoC.player.keyItemv2( 'Marae\'s Lethicite' ) === 1 ) {
			MainView.outputText( 'He looks at the crystal for a moment, before biting off half of it and giving the other half back to you.  "<i>This is enough for the hellfire, I think.  Go ahead and keep the rest, my favorite fan!</i>"\n\n', false );
	...
	HellHoundScene.prototype.giveALLTHELETHICITES = function() {
		...
		if( CoC.player.dogCocks() >= 2 ) {
			...
			if( CoC.player.totalCocks() > 2 || CoC.player.cockThatFits( 60 ) < 0 ) {
				MainView.outputText( 'He then reaches around your waist and takes a hold of your ' + Descriptors.multiCockDescriptLight() + '.  "<i>Before we get started, let\'s make sure you\'re just right for Cremera.</i>"  He then reforms your body to have twin doggy pricks of appropriately sized.  "<i>Now.</i>"\n\n', false );
			...
			if( CoC.player.vaginalCapacity() < 45 ) {
				...
				MainView.outputText( 'our two cunts quickly stretch larger to accommodate the intruders.\n\n', false );
			}
			MainView.outputText( 'You let out a moan of pleasure from the pleasure of being filled.  The hound doesn\'t hesitate at all, and just thrusts into you with abandon.  You can\'t believe the feral feeling of it all, and that your twin womanhoods manage to so easily take the members and flow around them.\n\n', false );
			MainView.outputText( 'In moments, you feel Cimer\'s members explode inside you with their wonderful hot seed, and his new knots swell to tie the two of you together.  Though, something feels wrong, you didn\'t feel an orgasm yourself!  You give a whimper and try to thrust against Cimer\'s pair of dog pricks inside you.  Suddenly there is a loud smack, and you feel like Cimer was just propelled inside you even further!  It\'s an incredible feeling, almost like he is pushing past your ' + Descriptors.buttDescript() + ', but you still can\'t cum!  "<i>This isn\'t enough!  Further inside, further!</i>"  You hear the master yell.  Once again you feel Cimer being forced further and further inside you, again accentuated by a loud smacking noise.  Again, you find yourself unable to achieve sweet release, but you hardly have time to register that, as there is another burst and a smack, and another, and another.  It almost feels like your ' + Descriptors.buttDescript() + ' is being smacked...\n\n', false );
			MainView.outputText( 'After what feels like hours of painful pleasure without release, you\'re smacked on your rear one last time, and it feels like Cimer bursts through your body completely.  This finally frees the floodgates, and you feel like you\'re releasing everything in your body in the most powerful orgasm you\'ve ever felt.  You pant for a moment and look down in front of you.  You can see your ' + CoC.player.allBreastsDescript() + ' and your pregnant belly in front of you. You also see a package of demonic dicks sticking out in front of you, with a quad of hot balls resting underneath.  In front of you is a large pool of white steaming fluid, with a pink crystal resting in the middle.\n\n', false );
	...
	HellHoundScene.prototype.hellHoundPropahRape = function() {
		...
		//--- ELSE (CORRUPTION >= 20) ---
			...
			//--- ELSE (CORRUPTION >= 40) ---
			MainView.outputText( 'His magical natural flames fill your body with fierce warmth, arousing you further. You pick yourself up, letting the hellhound\'s cocks nearly escape your nethers just to sit down again, taking the duo of dog members to the hilt. Grabbing his hind legs like the handlebars of an exercise machine, you bring yourself into position for a good amount of training. Gradually, you pick up the pace, rocking your hips up and down, enjoying the awesome double penetration.', false );
			if( CoC.player.biggestTitSize() > 1 ) {
				MainView.outputText( '  Your ' + Descriptors.chestDesc() + ' jiggle in exquisite ways along with your bumping, grinding motions.', false );
			}
			MainView.outputText( '  Riding the hound for a couple of minutes, you feel the dick in your ' + Descriptors.assholeDescript() + ' extending, giving you a good stretching. Its brother in your ' + Descriptors.vaginaDescript( 0 ) + ' does the same, pushing more and more girl-fluids out of your slippery slit while stimulating your sensitive clit from the inside. You lean forward, running your hands through your fuckpet\'s dense midnight black fur, feeling his animal body warmth. The hellhound is moaning and panting beneath you, unsure whether to fear you or to enjoy what you are doing to him.\n\n', false );
			//--- IF PC HAS PENIS ---
			if( CoC.player.hasCock() ) {
				MainView.outputText( 'His canine cock in your anus stimulates your prostate perfectly. His exotic black dong applies pressure in parts of your body you wouldn\'t expect to get such pleasure from.', false );
				//--- IF ONE PENIS ---
				if( CoC.player.totalCocks() === 1 ) {
					MainView.outputText( '  Soon your own ' + Descriptors.cockDescript( 0 ) + ' reacts, quickly growing into a full throbbing erection. You leave your member alone, and it keeps flapping against the hellhound\'s belly while you ride him.', false );
				}//--- ELSE ---
				else {
					MainView.outputText( '  Soon ' + Descriptors.sMultiCockDesc() + ' reacts, quickly growing into a throbbing erection.  You leave your members alone, and they flap against the hellhound\'s belly while you ride him.', false );
				}
			}
			//--- IF PC HAS BALLS ---
			if( CoC.player.balls > 0 ) {
				MainView.outputText( '  Whenever you lower your hips to your mates belly, your ' + Descriptors.ballsDescriptLight() + ' touches his. The magical flames fill your scrotum with unnatural heat but no pain. You literally feel the cum in your balls boiling upwards, building up immense pressure.', false );
			}
			MainView.outputText( '\n\n', false );
			MainView.outputText( 'Way too early, the two doggie pricks stuffed inside you start to twitch, unloading four balls worth of spunk into you. In ecstasy, the hellhound\'s eyes look in four different directions. The dual eruptions in your love-canal and rectum feel amazing as they pulse in and out of sync, dumping cum into your body over and over.  The hellhound\'s contractions ebb away as he finishes, but you\'re not done yet. You slowly start tilting your hips back and forth, continuing to fiercely fuck the prime member while the other rests nearly motionless in your ass. Soon the hellhound whines from the unwanted stimulation of his now sensitive pricks, but he is too defeated to put up much of a struggle.\n\n', false );
			MainView.outputText( 'Slowly but steadily you bring yourself to the verge of orgasm. The hellhound under you squirms and tries to get free, his hind legs flailing through the air behind your back. You keep riding his magnificent member, grinding it up and down in your moist cave. Finally you feel the familiar rippling of your vaginal walls, milking his cock and intensifying your pleasant sensations. Clamping down on his members, you keep moving your hips in a circling motion, working your clit, vagina and butt in equal measure. The sensations eventually grant you an intense orgasm while the hellhound\'s still-distended members apply the perfect pressure to all of your sensitive spots and prolong your bliss.', false );
			//--- IF PC HAS PENIS ---
			if( CoC.player.totalCocks() > 0 ) {
				// --- IF ONE PENIS ---
				if( CoC.player.cockTotal() === 1 ) {
					MainView.outputText( 'Your ' + Descriptors.cockDescript( 0 ) + ' twitches as it ejects its sticky load.', false );
				}//--- ELSE ---
				else {
					MainView.outputText( 'Your ' + Descriptors.multiCockDescriptLight() + ' twitch as they eject their sticky loads.', false );
				}
				MainView.outputText( '  Keeping up your motions, you scatter your cum across the hellhound\'s body and the ground nearby.', false );
