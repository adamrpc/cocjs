...
angular.module( 'cocjs' ).run( function( SceneLib, kFLAGS, CoC, AppearanceDefs, PerkLib, StatusAffects, EngineCore, Descriptors, Combat, HellHound, PregnancyStore, CockTypesEnum ) {
	...
	HellHoundScene.prototype.hellhoundEncounter = function() {
		EngineCore.outputText( 'You hear a fiery howl as a demonic, two-headed beast-man leaps out in front of you!', true );
		...
		//--- ELSE (CORRUPTION >= 20) ---
			...
			if( CoC.player.hasCock() ) {
				...
				if( CoC.player.totalCocks() === 1 ) {
					...
					EngineCore.outputText( '  Soon ' + Descriptors.sMultiCockDesc() + ' reacts, quickly growing into a throbbing erection.  You leave your members alone, and they flap against the hellhound\'s belly while you ride him.', false );
				}
			}
			//--- IF PC HAS BALLS ---
			...
			if( CoC.player.totalCocks() > 0 ) {
				...
				if( CoC.player.cockTotal() === 1 ) {
					...
					EngineCore.outputText( 'Your ' + Descriptors.multiCockDescriptLight() + ' twitch as they eject their sticky loads.', false );
				}
