...
angular.module( 'cocjs' ).run( function( SceneLib, MainView, kFLAGS, CoC, AppearanceDefs, PerkLib, CockTypesEnum, Minotaur, Appearance, StatusAffects, EngineCore, Descriptors, Utils, Combat, PregnancyStore ) {
	...
	MinotaurScene.prototype.minoVictoryRapeChoices = function() {
		...
		if( CoC.flags[ kFLAGS.MINOTAUR_CUM_ADDICTION_STATE ] >= 1 ) {
			MainView.outputText( 'Smiling down at your vanquished foe, you feel a familiar hunger growing within you.  What do you do?', true );
		}
		//Not an addict
		else if( (CoC.player.lust >= 33 && CoC.player.gender > 0) || feedposit === 'Lay Eggs' ) {
			if( CoC.monster.lust > 99 ) {
				MainView.outputText( 'You smile in satisfaction as the ' + CoC.monster.short + ' drops down on all fours and begins masturbating feverishly.  Sadly you realize your own needs have not been met.  Of course you could always fuck the eager bull...\n\nWhat do you do?', true );
			} else {
				MainView.outputText( 'You smile in satisfaction as the ' + CoC.monster.short + ' collapses, unable to continue fighting.  Sadly you realize your own needs have not been met.  Of course you could always rape the poor thing...\n\nWhat do you do?', true );
			}
		}
		...
		//No rape, no feeder
			...
			if( CoC.monster.lust > 99 ) {
				MainView.outputText( 'You smile in satisfaction as the ' + CoC.monster.short + ' drops down on all fours and begins masturbating feverishly.', true );
			} else {
	...
	MinotaurScene.prototype.rapeMinotaurTentacles = function() {
		...
		if( CoC.monster.HP <= 0 ) {
			MainView.outputText( 'The minotaur collapses backwards after your last blow. Defeated and tired, he can barely stand up as you approach.  ', false );
		} else {
			MainView.outputText( 'The minotaur drops his arm', false );
			if( CoC.monster.weaponName === 'axe' ) {
				MainView.outputText( ', letting his axe clatter to the floor', false );
			}
			MainView.outputText( '. Transfixed by your ' + Descriptors.multiCockDescript() + ' in front of his face, he barely notices as you walk closer.  ', false );
		}
		MainView.outputText( 'You grab his arm firmly, making certain there will be no retaliation during the pleasure. By the look on his face, and the state of his torn, rolled up loincloth, though, you doubt that will be a problem.  ', false );
		if( CoC.player.cor < 25 ) {
			MainView.outputText( 'You cry, petting his hair, smoothing it across his horns, justifying to yourself that you are raping this beast because this is how this world works as you maneuver your ' + Descriptors.multiCockDescript() + ' towards his tailhole.  ', false );
		}
		if( CoC.player.cor >= 25 && CoC.player.cor < 50 ) {
			MainView.outputText( 'You smoothly guide your ' + Descriptors.multiCockDescript() + ' toward his tailhole.  ', false );
		}
		if( CoC.player.cor >= 50 && CoC.player.cor < 80 ) {
			MainView.outputText( 'You roughly grab him, slamming him close to your ' + Descriptors.multiCockDescript() + ' - just how this beast likes it.  ', false );
		}
		if( CoC.player.cor >= 80 ) {
			MainView.outputText( 'You grin and roughly pull his horns towards your ' + Descriptors.multiCockDescript() + ', ready to roughly ride this beast.  ', false );
		}
		//Mixed dix
		if( CoC.player.cockTotal() !== CoC.player.tentacleCocks() ) {
			...
			MainView.outputText( 'Your ' + Descriptors.cockDescript( temp2 ) + ' slams into his tailhole, spreading it wider as you thrust in and out of the opening, relishing in the feeling of the rough, yet lubricated insides of the minotaur.  ', false );
		}
		//Not mismatched
		else {
			temp2 = 0;
			MainView.outputText( 'One of your prehensile dicks coils in on itself as it nears the minotaur\'s tailhole, readying itself. It then suddenly springs itself into his fleshy orifice, wiggling and writhing in deeper and deeper until all ' + Utils.num2Text( Math.ceil( CoC.player.cocks[ 0 ].cockLength ) ) + ' inches of it have submerged itself inside his intestines. You and he both shudder as your cock starts weaving from side to side, making slithering motions inside him.  ', false );
		}
		//Vaginassss
		if( CoC.player.vaginas.length >= 1 ) {
			MainView.outputText( 'With your ' + Descriptors.cockDescript( temp2 ) + ' successfully embedded in the minotaur, you twist around, positioning your ' + Descriptors.vaginaDescript( 0 ) + ' to take the minotaur\'s enormous length into yourself.  You feel his member deliciously slide in, and feel the beast\'s hot breath radiate as he starts panting in lust.  ', false );
		...
		if( CoC.player.vaginas.length > 1 && !mismatched && CoC.player.cocks.length > 2 ) {
			MainView.outputText( 'Your tentacle cocks writhe upon themselves, each turning to an unattended ' + Descriptors.vaginaDescript( 1 ) + ' on your body, and quickly filling up the cavernous depths. You groan in ecstasy at the self-fucking you are receiving as all your pussies start leaking their wonderful fluids.  ', false );
		}
		//more dicks than available holes
		if( (CoC.player.tentacleCocks() === 2 || CoC.player.tentacleCocks() === 3) ) {
			MainView.outputText( 'Your next tentacle dick, saddened by not having anything to writhe around, turns to the minotaurs protruding member. It darts out from your crotch, wrapping around his monstrous meat and constricting, like a snake might around its prey. Your tongue lolls out as you feel that cock twist and grab his dick, pulsing with both your blood and the beast\'s twitching member.  ', false );
		}
		if( CoC.player.tentacleCocks() > 3 ) {
			MainView.outputText( 'Your remaining tentacle dicks wave around the minotaur, tending to his balls, weaving around his limbs, and generally rubbing and throbbing all over him, spreading pre-cum around and through him, leaving both of you moaning in pleasure.  ', false );
		}
		//Cum
	...
	MinotaurScene.prototype.rapeMinotaurTentacle = function() {
		...
		MainView.outputText( '', true );
		MainView.outputText( 'You stand before the defeated minotaur and your tentacle dick begins to lash back and forth quickly, showing your excitement.  You see the hardness of his dick, and decide to do something about it... for both of you.  ', false );
		MainView.outputText( 'You come up behind him and wrap your tentacle dick around his huge member, squeezing him tightly at his dick\'s base pump even more blood to it.  Then you wrap yourself around his massive erection a few more times and you\'re ready. You jerk him up and down, varying the pace, place, and strength of your grip, milking his dick for all he is worth.  ', false );
		MainView.outputText( 'The minotaur\'s balls begin to spasm, so you wrap the base of your tentacle around him tightly, preventing his ' + CoC.monster.cockDescriptShort( 0 ) + ' from cumming, driving the minotaur mad with desire.  Your own ' + Descriptors.cockDescript( x ) + ' delights in the feeling of the fur of his balls and the hardness of his ' + CoC.monster.cockDescriptShort( 0 ) + '.  ', false );
		//Reset for upcoming stuff
		var temp = 0;
		//if multi with pony dick!
		if( CoC.player.horseCocks() === 1 ) {
			MainView.outputText( 'But it isn\'t enough for you. You take your pony prick, and with your hands, pull his anus wide. The flare at your tip takes some effort to get in, but once it\'s inside, you ram right to the hilt, knocking the wind from the minotaur with a solid grunt.  ', false );
			temp = 1;
		}
		//if multi with dogdick
		if( CoC.player.dogCocks() === 1 && temp !== 1 ) {
			temp = 1;
			MainView.outputText( 'But it isn\'t enough for you. You take your bitch knotter, and with your hands, pull his anus wide. You slide in easily, right up to your knot, but he\'s too tight for you to slip that in.  ', false );
		}
		//If single leftover dick
		if( CoC.player.cocks.length === 2 && temp !== 1 ) {
			temp = 1;
			MainView.outputText( 'But you avail yourself of his free rear end, to further your own delights. A quick push, and you\'re in, buried to your hilt.  ', false );
		}
		if( CoC.player.cocks.length > 2 && temp !== 1 ) {
			temp = 1;
			MainView.outputText( 'It\'s not enough! Your other dicks demand attention too! Fortunately, he has a large hole for you to abuse... you stuff as many of your ' + Descriptors.multiCockDescript() + ' as you can, the rest hanging underneath him, slapping against his sack. ' );
		}
		if( CoC.player.clitLength > 6 ) {
			MainView.outputText( 'Eager for more stimulation, you muse that your clit\'s big enough to fuck with, so why not!? You take your fingers, spit on them, and slide them into place, opening him up wider to accept your ' + Descriptors.clitDescript() + '. It\'s so sensitive, sliding into that tight hole, and the sensations are driving you wild as you hump against him with your dick-like clit.  ', false );
		}
		//Free pussy!
		if( CoC.player.vaginas.length > 0 ) {
			temp = 2;
			MainView.outputText( 'You\'re a woman, and it\'s about time you got some pussy action too!  You slide your ' + Descriptors.vaginaDescript( 0 ) + ' onto ' + CoC.player.cockDescript( x ) + '-wrapped minotaur-prick, further driving the beast mad with lust.  He plaintatively groans, struggling to get his dick free or to cum, you aren\'t quite sure.  ', false );
			MainView.outputText( 'Eventually, the moment is right.  You smile as you feel a boiling heat building in your groin, surging through your ' + Descriptors.multiCockDescript() + ' in a tide of white-hot pleasure.  You groan and shiver, lost in the pleasure until you begin to calm down.  ', false );
			if( CoC.player.cumQ() < 50 ) {
				MainView.outputText( 'Your cock made quite a mess of things, and when you pull him out, your ' + Descriptors.vaginaDescript( 0 ) + ' makes a wet sucking sound.  ', false );
			}
			if( CoC.player.cumQ() >= 50 && CoC.player.cumQ() < 400 ) {
				MainView.outputText( 'Your cock made quite a mess of things inside your ' + Descriptors.vaginaDescript( 0 ) + '.  As you pull free, cum drips freely down your thighs.  The heavenly scent of minotaur spunk fills your nose, and begin scooping the mixed jizz up to sample.  ', false );
			}
			if( CoC.player.cumQ() >= 400 ) {
				MainView.outputText( 'Your cock made quite a mess of things inside your ' + Descriptors.vaginaDescript( 0 ) + '.  As you pull free, a river of cum pours from betwixt your thighs.  The heavenly scent of minotaur spunk fills your nose, and you begin scooping the mixed jizz up to sample.  ', false );
			}
		}
		//Free ass!
		else {
			temp = 2;
			MainView.outputText( 'You\'re feeling a little kinky so you step in front of him, offering him a little bit more, pushing yourself down onto his meaty dick.  You slowly push down, enjoying every inch as it travels down your ' + Descriptors.assholeDescript() + ', the pre leaking from it acting as a lubricant as his tentacle-wrapped cock works its way inside you.  ', false );
			//Ass cum
			MainView.outputText( 'Eventually, the moment is right.  You smile as you feel a boiling heat building in your groin, surging through your ' + Descriptors.multiCockDescript() + ' in a tide of white-hot pleasure.  You groan and shiver, lost in the pleasure until you begin to calm down.  ', false );
			if( CoC.player.cumQ() < 50 ) {
				MainView.outputText( 'Your cock in his ass made a mess of things, and when you pull out, it makes a wet sucking sound, trying to keep you inside.  ', false );
			}
			if( CoC.player.cumQ() >= 50 && CoC.player.cumQ() < 400 ) {
				MainView.outputText( 'Your cock in his ass spills forth, creating a puddle which will mat his fur later.  ', false );
			}
			if( CoC.player.cumQ() >= 400 ) {
				MainView.outputText( 'Your cock is blasted free from his ass by the force of your ejaculation, splattering rivers of spoo over the ground.  A river of the stickiness pours from his violated asshole, sticking in the fur of his thighs.  ', false );
			}
		}
		//Cum tiem
		if( temp < 2 ) {
	...
	MinotaurScene.prototype.AddictNagaOnMinotaur = function() {
		...
		//[if(CoC.monster.lust >= 99)
		if( CoC.monster.lust > 99 ) {
			MainView.outputText( 'You slither towards the fallen monster, wishing to taste his addictive semen.  He\'s currently engaged in stroking his enormous shaft up and down with both of his hands, but offers little resistance when you push him over to a sitting position.  You pull his hands away and replace them with yours, running your fingers up and down along his thickness, working yourself up with anticipation.  He leans back and exhales loudly, clearly appreciative.\n\n', false );
		} else {
			MainView.outputText( 'You slither towards the fallen monster, wishing to taste his addictive semen.  You notice that, despite the beating you just gave him, his huge cock is fully erect.  Delighted, you gently run both of your hands up and down its long shaft.  He exhales loudly, signaling his appreciation.\n\n', false );
		}
		MainView.outputText( 'A bit of gooey pre-cum begins to gather at the tip of his thick, horse-like cock.  Your tongue flits out instinctively, and you taste its scent in the air.  It\'s too much for you to handle, and you are overcome with an absolute need to fill yourself with his nectar.  You throw yourself onto his crotch, grabbing his thighs and rubbing his cock along your torso as you position your mouth over his length, taking care to keep your long fangs out of the way.  You violently pull yourself to him, forcing more of him than you thought you could handle down your throat.  You feel his pre lubricating your insides, numbing you to the pain and allowing you to take him deeper.  You take one hand from his leg and place it on the still exposed bit of his shaft, stroking what you could not take in your mouth. His animal grunting grows louder as you take just a bit more of him with each thrust downwards.  Mad with lust, you remove your other hand from his thigh, balancing on your lengthy tail and his cock, and use it to ', false );
		//if(CoC.player.cocks.length > 0)
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'furiously stroke your own ' + Descriptors.multiCockDescriptLight() + '. Your muffled vocalizations of pleaure weakly join his own.\n\n', false );
		} else {
			MainView.outputText( 'part the folds of your ' + Descriptors.vaginaDescript( 0 ) + ', sliding it inside and masturbating with a furious vigor. Your muffled vocalizations of pleaure weakly join his own.\n\n', false );
		}
	...
	MinotaurScene.prototype.minoRapeIntro = function() {
		...
		if( CoC.monster.HP <= 0 ) {
			MainView.outputText( 'You roll the barely conscious beast over, lifting his ass up above his knees.  The loincloth flops open, giving you a perfect view of his hardening member.  In most ways it looks like a horse\'s dick, though it seems to be textured with multiple rings of prepuce along its length.  The scent boiling off that marvelously thickening endowment is pleasant yet musky, and overpoweringly strong.', true );
			//Vagina or not flavor texts
			if( CoC.player.vaginas.length > 0 ) {
				MainView.outputText( '  Glancing back at you with hope in his eyes, the minotaur\'s ' + CoC.monster.cockDescriptShort( 0 ) + ' twitches as he lays his gaze upon your ' + Descriptors.vaginaDescript( 0 ) + '.', false );
			} else {
				MainView.outputText( '  Glancing back at you with eyes full of despair, the minotaur seems to realize you won\'t be helping the predicament his ' + CoC.monster.ballsDescriptLight() + ' are in.', false );
			}
		}
		//Monster lost to lust
		else {
			MainView.outputText( 'You rip off the minotaur\'s loincloth with a savage grin, slapping his ass and admiring the jiggle of his ' + CoC.monster.ballsDescriptLight() + ' as they wobble from the force of your blow.  The cow-man moos softly with desire, ropes of pre splattering the ground from his arousal.   The musk pouring off him and his puddle of slick pre-cum is overpowering, making it difficult to think as you feel your blood rush to your groin ', false );
			//NIPPULAR ENJOYMENT
			if( CoC.player.totalNipples() > 1 ) {
				MainView.outputText( ' and nipples', false );
			}
			MainView.outputText( '.', false );
			//Vagina or not flavor texts
			if( CoC.player.vaginas.length > 0 ) {
				MainView.outputText( '  Glancing back at you with hope in its eyes, the minotaur\'s ' + CoC.monster.cockDescriptShort( 0 ) + ' twitches as he lays his gaze upon your ' + Descriptors.vaginaDescript( 0 ) + '.', false );
			} else {
				MainView.outputText( '  Glancing back at you with eyes full of despair, the minotaur doubts you\'ll help relieve the predicament his ' + CoC.monster.ballsDescriptLight() + ' are in.', false );
			}
		}
	};
	...
	MinotaurScene.prototype.minoPheromones = function() {
		MainView.spriteSelect( 44 );
		MainView.outputText( 'The minotaur smiles at you and lifts his loincloth, flicking it at you.  Thick ropes of pre-cum fly through the air, ', false );
		//sometimes get hit with the pre for stronger effect!
		if( Utils.rand( 3 ) === 0 ) {
			MainView.outputText( 'slapping into your face before you can react!  You wipe the slick snot-like stuff out of your eyes and nose, ', false );
			if( CoC.player.lust > 75 ) {
				MainView.outputText( 'swallowing it into your mouth without thinking.  ', false );
				EngineCore.dynStats( 'lus', 10 + CoC.player.lib / 10 );
			} else {
				MainView.outputText( 'feeling your heart beat with desire as your tongue licks the residue from your lips.  ', false );
				EngineCore.dynStats( 'lus', 5 + CoC.player.lib / 20 );
			}
		} else {
			MainView.outputText( 'right past your head.  ', false );
		}
	...
	MinotaurScene.prototype.getRapedByMinotaur = function( autoRape ) {
		...
		if( !autoRape ) {
			...
			if( CoC.player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_CENTAUR ) {
				MainView.outputText( 'against your buttocks', false );
			} else {
				MainView.outputText( 'into the small of your back', false );
			}
			MainView.outputText( ' as it grows larger and harder, smearing its pre-cum into your skin and making you shiver.  ', false );
			//High str escape
		...
		//Normal RAEP
		if( CoC.player.vaginas.length > 0 ) {
			MainView.outputText( 'The bull-man roughly grabs your hair and begins rubbing the flared head of his penis along your ' + Descriptors.vaginaDescript( 0 ) + '.  ', false );
			if( CoC.player.averageVaginalWetness() < 2 ) {
				MainView.outputText( 'You aren\'t very wet, and fear the damage this beast will inflict on your ' + Descriptors.vaginaDescript( 0 ) + '.  ', false );
			} else {
				MainView.outputText( 'You\'re shamefully wet by this point, and your knees are ready to buckle.  ', false );
			}
			this.minoCumAddiction( 5 );
		}
		//BUTTSECKS!
		else {
			MainView.outputText( 'The bull-man roughly grabs your hair and repositions himself to press against your asshole, slowly building the pressure until suddenly, the flared head pops into you.  ', false );
		...
		if( CoC.player.biggestTitSize() > 0 && CoC.player.mostBreastsPerRow() > 1 && CoC.player.breastRows.length > 0 ) {
			MainView.outputText( 'He manhandles your tits as he does so, almost violently squeezing and stretching them to his enjoyment.  ', false );
			if( CoC.player.biggestLactation() > 1.5 ) {
				MainView.outputText( 'He then gives a grunt in what you take to be approval as your milk begins to squirt out. He licks a milk-coated finger in satisfaction.  ', false );
			}
		}
		if( CoC.player.cockTotal() === 1 ) {
			MainView.outputText( 'The bull-man bends forward a little, and grabs your ' + Descriptors.cockDescript( 0 ) + ' in a crushing grip. He makes short jerking motions as he keeps thrusting into you.', false );
		}
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 'The bull-man bends forward a little, and grabs one of your ' + Descriptors.cockDescript( 0 ) + 's in a crushing grip. He makes short jerking motions as he keeps thrusting into you.', false );
		}
		if( CoC.player.vaginas.length > 0 ) {
			if( CoC.player.vaginalCapacity() < CoC.monster.cockArea( 0 ) ) {
		...
		if( CoC.player.vaginas.length > 0 ) {
			if( CoC.player.averageVaginalWetness() >= 2 ) {
				if( CoC.player.averageVaginalWetness() < 4 ) {
					MainView.outputText( 'You squirm and quiver, orgasming from the beast\'s rough usage.  ', false );
				}
				if( CoC.player.averageVaginalWetness() === 4 ) {
					MainView.outputText( 'You squirm and quiver, orgasming from the beast\'s rough usage, soaking him with your ' + Descriptors.vaginaDescript( 0 ) + '.  ', false );
				}
				if( CoC.player.averageVaginalWetness() === 5 ) {
					MainView.outputText( 'You orgasm on his massive rod, splattering the beast with girlcum.  ', false );
				}
				CoC.player.orgasm();
				EngineCore.dynStats( 'lib', 0.5, 'sen', 1, 'cor', 1 );
			}
			if( CoC.player.averageVaginalWetness() < 2 ) {
				MainView.outputText( 'You gasp in pain, your cunt rubbed raw by the rough and violent penetration.  ', false );
		...
		if( CoC.player.cockTotal() > 0 && (CoC.player.sens + Utils.rand( 40 ) > 50) ) {
			MainView.outputText( 'You orgasm, ', false );
			if( CoC.player.cumQ() < 25 ) {
				MainView.outputText( 'spurting your seed helplessly from the brutal rape.  ', false );
			}
			if( CoC.player.cumQ() >= 25 && CoC.player.cumQ() < 250 ) {
				MainView.outputText( 'squirting thick ropes of cum over the cave.  ', false );
			}
			if( CoC.player.cumQ() >= 250 && CoC.player.cumQ() < 500 ) {
				MainView.outputText( 'erupting potent ropes of seed in thick bursts, splattering the cave walls and floors.  ', false );
			}
			if( CoC.player.cumQ() >= 500 ) {
				MainView.outputText( 'erupting a thick torrent of seed that seems to go on forever, spurred by the constant pressure of the huge minotaur cock.  You paint the cave wall with cum, the beast egging you on as it roughly jacks your ' + Descriptors.cockDescript( 0 ) + '.  You are vaguely aware of your orgasm dragging on and on, until eventually your orgasm stops, leaving a sizable puddle of cum on the floor.  ', false );
			}
			CoC.player.orgasm();
			EngineCore.dynStats( 'lib', 0.5, 'sen', 1, 'cor', 1 );
		}
		MainView.outputText( 'The bull-man relaxes for a moment, then shoves you off of him and to the cold ground. You pass out as a strange sense of euphoria washes over you while copious quantities of monstrous cum escape your distended ', false );
		if( CoC.player.hasVagina() ) {
	...
	MinotaurScene.prototype.minoGetsTitFucked = function() {
		...
		MainView.outputText( '', true );
		MainView.outputText( 'The helpless minotaur lays in the dirt, ', false );
		if( CoC.monster.HP < 1 ) {
			MainView.outputText( 'defeated.  ', false );
		} else {
			MainView.outputText( 'masturbating uncontrollably.  ', false );
		}
		MainView.outputText( 'He\'s yours for the moment, and for right now, you want to see if that throbbing horse-cock will splatter its heavenly spunk all over your ' + CoC.player.allBreastsDescript() + ' and ' + CoC.player.face() + '.  You really could use a fix, just enough to fuzz over all the worry and stress you\'ve had to deal with.\n\n', false );
		if( CoC.monster.lust > 99 ) {
			MainView.outputText( 'You push his hands away and y', false );
		} else {
			MainView.outputText( 'Y', false );
		}
		MainView.outputText( 'ou lean over him, feeling your generous breast-flesh jiggle, your ' + Descriptors.nippleDescript( 0 ) + 's tingling with desire as they rub across the horse-like minotaur dong.   The beast\'s pre starts forming into large drops along his flared head, and you waste no time smearing it through your cleavage, lubricating the space between your tits and breathing deeply of its perfect scent.', false );
		if( CoC.player.totalCocks() > 0 ) {
			MainView.outputText( '  Your ' + Descriptors.cockDescript( 0 ) + ' grows harder and more aroused than ever before while your ', false );
		} else {
			MainView.outputText( '  Your body aches with arousal while your ', false );
		}
		if( CoC.player.hasVagina() ) {
			MainView.outputText( Descriptors.vaginaDescript( 0 ) + ' grows hotter and hotter, begging to be taken.', false );
		} else {
			MainView.outputText( Descriptors.assholeDescript() + ' puckers slightly, as if begging to be taken by another monstrous beast.', false );
		}
		MainView.outputText( '\n\n', false );
		MainView.outputText( 'He moos out pitifully, clearly understanding what you\'re up to and frustrated he won\'t get to sate his desire in a tight mortal ', false );
		if( CoC.player.hasVagina() ) {
			MainView.outputText( 'cunt.  ', false );
		} else {
			MainView.outputText( 'ass.  ', false );
		}
		MainView.outputText( 'Out to prove him wrong by letting him release all over your face, you wrap your ' + CoC.player.allBreastsDescript() + ' tightly around him and begin sliding your body along his length.  The constant drizzling pre-cum easily lubricates you, and soon your orbs are shiny and slick with his drug-like fluid.\n\n', false );
		...
		MainView.outputText( 'The minotaur\'s balls clench tightly as the bull-man throws his head back and screams out in pleasure and relief, "<i>MooooOOOOOOooooOOOOO!</i>"\n\n', false );
		MainView.outputText( 'You squeeze tightly and press down, watching his dick-head flare out, nearly doubling size before it unloads, drenching your face and hair with his thick narcotic cream.   You manage to get your lips against it in time for the next blast, and nearly drown when it overfills your mouth.   You pull back, coughing, starting to get high, and swaying woozily as more and more cream spatters your body, continuing to be squeezed out by your slick heaving breasts.\n\n', false );
		MainView.outputText( 'The bull-man relaxes at last, drenching you with one last wave before he passes out.  You stagger up and nearly trip over your legs, giggling and drunk on cow-spunk as you noisily slurp down the fluids drenching your face and hair.  You black out for a while...\n\n', false );
		MainView.outputText( 'You wake up an hour later, unsure how you made it back to your camp, light-headed and euphoric with the lingering after-affects.  You also find a bottle on the ground next to you, filled to the brim with the fluid.  You don\'t remember filling it up, but you\'re already itching to pop it open and guzzle it down.  You manage to resist the urge.  Besides, you\'re still feeling GREAT from your last \'sample\'.', false );
		this.minoCumAddiction( 10 );
		CoC.monster.short = 'tit-fucked Minotaur';
		CoC.player.orgasm();
	...
	MinotaurScene.prototype.takeMinoCumDirectly = function() {
		MainView.spriteSelect( 44 );
		MainView.outputText( '', true );
		MainView.outputText( 'Relishing your victory, you breathe deeply, shivering and stretching.  ', false );
		if( CoC.monster.HP < 1 ) {
			MainView.outputText( 'The beast-man has fallen on his back, comatose and insensate, though his shaft has risen to full erectness, displacing his loincloth.', false );
		} else {
			MainView.outputText( 'The beast-man is on his back with his legs splayed, masturbating hopelessly.  His anguished moans tell you all you need to know – he\'ll never get off alone.', false );
		}
		MainView.outputText( '  You smile, blushing in anticipation, readying yourself for the coming bliss.\n\n', false );
		MainView.outputText( 'You shed your ' + CoC.player.armorName + ' and step over its waist, trembling slightly with anticipation of your next fix.  Taking his flared drug-nozzle in hand, you hold it steady and lower yourself down, feeling his slippery narcotic pre-cum smearing over your hole.  ', false );
		//Too tight!
		if( CoC.player.analCapacity() < CoC.monster.cockArea( 0 ) ) {
			MainView.outputText( 'You have to push harder and harder to get it inside you, even with the copious lube\'s drug-like affects relaxing your sphincter.  The penetration is slow, but inch by inch you force it inside you.', false );
		} else {
			MainView.outputText( 'Your muscles instantly relax on contact with the pre, and you feel the massive tool sliding up your spacious asshole with ease.', false );
		}
		if( !CoC.player.buttChange( CoC.monster.cockArea( 0 ), true ) ) {
			MainView.outputText( '  ', false );
		}
		MainView.outputText( 'The minotaur\'s eyes open wide as he lets out a lustful \'<i>Moooooo</i>\'.  He doesn\'t struggle or even move – you\'re giving him exactly what he wants, after all.\n\n', false );
		MainView.outputText( 'You bottom out, sighing happily as you feel his hot fluids leaking inside you, radiating pure pleasure out from your core.   Bestial hands grab your ' + Descriptors.nippleDescript( 0 ) + 's and begin gently twisting them, taking surprising care for all their strength.  Spurred on by the feelings radiating from your ', false );
		if( CoC.player.biggestTitSize() < 1 ) {
			MainView.outputText( 'chest', false );
		} else {
			MainView.outputText( Descriptors.breastDescript( 0 ), false );
		}
		MainView.outputText( ' and hungry for even more bliss, you begin bouncing atop the defeated beast, repeatedly squeezing and relaxing yourself around him, trying to wring out the maximum possible amount of fluid.\n\n', false );
		...
		if( CoC.flags[ kFLAGS.MINOTAUR_CUM_ADDICTION_TRACKER ] < 50 ) {
			...
			if( CoC.flags[ kFLAGS.MINOTAUR_CUM_ADDICTION_STATE ] > 0 ) {
				...
				if( CoC.player.cor > 60 ) {
					MainView.outputText( 'but miss the tingling apathy and fuzziness that filled your head every time one of them was squirting inside you.', false );
				} else {
					MainView.outputText( 'and resolve to give the beasts a wide berth from now on.', false );
				}
	...
	MinotaurScene.prototype.minoAddictionBadEnd2 = function() {
		MainView.spriteSelect( 44 );
		MainView.outputText( '', true );
		MainView.outputText( 'You grin and stumble up the mountain, inhaling deep breaths from moment to moment to make sure you\'re on the right track.  The scent gets stronger and stronger, making you ', false );
		if( CoC.player.hasVagina() ) {
			...
			MainView.outputText( 'shiver ', false );
		}
	...
	MinotaurScene.prototype.minotaurBlowjob = function() {
		...
		MainView.outputText( 'You step towards the minotaur, ', false );
		if( !CoC.player.isTaur() ) {
			MainView.outputText( 'peeling away your ' + CoC.player.armorName + ' and ', false );
		}
		MainView.outputText( 'revealing your ' + Descriptors.multiCockDescriptLight() + ' to the defeated bull.  The beast looks to you in both arousal and shock, mouth parting slightly at the idea of another creature taking charge.  Within the musky stench of its dripping shaft you nearly lose yourself, and ', false );
		if( !CoC.player.isTaur() ) {
			MainView.outputText( 'taking the bull by the horns', false );
		} else {
			MainView.outputText( 'pinning his horns in the crook of your forelegs', false );
		}
		MainView.outputText( ', you guide its mouth closer to your member as if on autopilot.  "<i>Let\'s see just how good you are at milking,</i>" you blurt; the minotaur moos in defiance but is soon silenced when the head of your ' + Appearance.cockNoun( CoC.player.cocks[ x ].cockType ) + ' presses into its bovine mouth.\n\n', false );
		...
		MainView.outputText( ' leaving him hungry for more.\n\n', false );
		MainView.outputText( 'At this realization he attempts to pull away, to save himself from the ordeal, but in his debilitated state his efforts are as weak as a kitten\'s. Held firmly by the horns, the bull-man slowly succumbs.  Unable to control himself in his lust, he licks at the cock within his mouth wildly, his thick cow-like tongue roughly wrapping around your length as you move yourself further within his sucking mouth.  From the combined effects of the creature\'s musk and maw you start to lose yourself as well, ', false );
		if( !CoC.player.isTaur() ) {
			MainView.outputText( 'finding your hands leaving his horns for the back of his head ', false );
		}
		MainView.outputText( 'as your hips begin to thrust into his face.  In your daze you glance down, to see the minotaur roughly and futilely stroking his shaft, his eyes full of shame and desperation - full of wanting to not only release his pent-up arousal, but to taste your thick cum as well.\n\n', false );
		MainView.outputText( 'You soon oblige the bull; your effort to fuck his face and the rapid bobbing of his head along your member quickly pass what little restraint you care to exercise on the monster\'s behalf.  Slamming the muzzle into your groin and leaning over his back, you begin layering the minotaur\'s tongue and tonsils with bursts of your seed.  Reaching down with ', false );
		if( !CoC.player.isTaur() ) {
			MainView.outputText( 'your hand', false );
		} else {
			MainView.outputText( 'a forehoof', false );
		}
	...
	MinotaurScene.prototype.layEggsInAMinotaurSpiderLike = function() {
		...
		MainView.outputText( 'The defeated minotaur turns and tries to flee.  You quickly fling a cord of sticky webbing that wraps around his massive horns, hauling him up short.  With a panicked moo, he begins to thrash his head, desperately trying to free himself.  You scurry forward and tackle the man-bull, forcing him onto his back.  Extruding more webbing, you wrap his wrists and bind them to his horns, then force his muscular legs upward, bending him almost double before lashing his legs to his horns as well.' );
		...
		if( !EngineCore.silly() ) {
			MainView.outputText( 'his bovine rosebud.' );
		} else {
			MainView.outputText( 'the entrance to his O.K. Corral.' );
		}
		MainView.outputText( '  Grinning, you lift your abdomen high behind you, then snap it forward, forcing more than half of your long ovipositor deep into your beef-bitch.' );
		if( CoC.player.gender > 0 ) {
			MainView.outputText( '\n\n' );
			if( CoC.player.hasCock() ) {
				MainView.outputText( 'Forcing [oneCock] between his bound thighs, you frot his flared bull-cock, your mixed pre soaking into the fur of his belly as you thrust against him.' );
			}
			if( CoC.player.hasVagina() ) {
				MainView.outputText( 'Grasping his bull-cock, you pull it against your pussy, rubbing your juices up and down his impressive length as your lips and clit grind on him, adding to your pleasure.' );
			}
		}
		//(If male or herm:
		MainView.outputText( '\n\n' );
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'As your orgasm washes over you, you spray your cum, ' );
			if( CoC.player.cumQ() < 100 ) {
				MainView.outputText( 'spattering' );
			} else if( CoC.player.cumQ() < 500 ) {
				MainView.outputText( 'drenching' );
			} else {
				MainView.outputText( 'nearly drowning' );
			}
			MainView.outputText( ' your helpless prey.  ' );
		}
		if( CoC.player.hasVagina() ) {
			MainView.outputText( 'Your femcum sprays across his cock.  ' );
		}
		MainView.outputText( 'Simultaneously, your ovipositor pulses and throbs, forcing egg after egg between his beefy buns and deep into him.  The ' );
		if( CoC.player.gender > 0 ) {
			MainView.outputText( 'combination of the scent of your fluids and the ' );
		}
		MainView.outputText( 'sensation of eggs forcing their way past his sphincter proves too much for your bovine bitch to resist, and he helplessly sprays his musky cum, stream after stream jetting up his chest and across his face as his eyes roll back in his head.' );
		//[(mino addict)
		if( CoC.flags[ kFLAGS.MINOTAUR_CUM_ADDICTION_STATE ] >= 1 ) {
			MainView.outputText( '  The smell is too much for you as well, and as you come a second time, you point his flare at your mouth, gulping as much of his seed as you can get.' );
	...
	MinotaurScene.prototype.layBeeEggsInCowbutt = function() {
		MainView.clearOutput();
		MainView.outputText( 'Looking down at the defeated minotaur' );
		if( CoC.monster.lust > 99 ) {
			MainView.outputText( ', stroking his flared cock desperately' );
		}
		MainView.outputText( ' you feel a throbbing sensation in your apiary appendage.  A leering grin spreads across your face as you force the minotaur onto his stomach, trapping his musky manhood beneath him.' );
		if( !CoC.player.isTaur() ) {
			MainView.outputText( '  Wrapping his tail around one hand, you lift his furred flanks up' );
		} else {
			MainView.outputText( '  Straddling him with all four legs, you slide forward' );
		}
		MainView.outputText( ' while aiming your ovipositor.' );
		...
		if( CoC.player.cor < 50 ) {
			MainView.outputText( 'You hesitate, feeling a surge of pity for the once-mighty creature, but your new instincts drive your egg-layer further into his prone body.  ' );
		} else {
			MainView.outputText( 'Grinning cruelly, you grasp him by the horns and haul back with both hands until the minotaur is forced to look you in the eye.  You lean in close to his snout and buzz mockingly, "<i>Whoz the bitch now?  Iz it you?  Yezz, it izz!</i>"  ' );
		}
		MainView.outputText( 'You begin to thrust back and forth, your strong forward strokes punctuated by grunting moos from your huge but helpless brood-bull.' );
		MainView.outputText( '\n\nYour body, long-ready to lay, quickly achieves a climax; ' );
		//(if male or herm:
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'you thrust your cock' );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ' through the air harder and faster, drenching his ' );
			//(if cum volume low:
			if( CoC.player.cumQ() < 250 ) {
				MainView.outputText( 'backside with your cum' );
			} else if( CoC.player.cumQ() < 500 ) {
				MainView.outputText( 'back with your cum, with ropes dripping slowly off his horns and down his face' );
			} else {
				MainView.outputText( 'back with your cum, before it covers him and spreads across the floor of his cave.' );
			}
		}
		//(if female:
		else if( CoC.player.hasVagina() ) {
			MainView.outputText( 'you rub your pussy hard against the minotaur\'s muscular ass, marking him with the scent of your fem-cum.' );
		}
	...
	MinotaurScene.prototype.getMinoHawtDawged = function() {
		...
		if( CoC.player.lust > 99 ) {
			MainView.outputText( 'You collapse to your knees, hands racing to your crotch and your ' );
			//(If Male or Herm:
			if( CoC.player.hasCock() ) {
				MainView.outputText( '[cock biggest], which you frantically try to stroke and rub through your [armor], its throbbing arousal clearly evident.' );
			}
			//If Herm:
			if( CoC.player.gender === 3 ) {
				MainView.outputText( '  Your other hand races to your ' );
			}
			//(If Female or Herm:
			if( CoC.player.hasVagina() ) {
				MainView.outputText( '[vagina], which is dripping and drooling your arousal down the insides of your [armor].  You manage to apply some pressure to your [clit] through your [armor], which sends a shock of pleasure through you.' );
			}
		}
		//If PC lost by damage, and Minotaur has axe:
		else if( CoC.monster.weaponName === 'axe' ) {
			MainView.outputText( 'The last great blow from the Minotaur\'s giant, man-slaughtering axe is just a little off, thankfully - the flat of the axe, rather than the blade, hits you upside the flat of your head, sending you tumbling into the dirt.' );
		}//(If PC lost by damage, and Minotaur does not:
		else {
			MainView.outputText( 'The last great blow from the Minotaur\'s giant, beefy fist hits you upside your head, the blow strong enough to send you tumbling into the dirt.' );
		}
		...
		if( CoC.player.hasCock() ) {
			MainView.outputText( '[eachCock] twitching beneath you, painting the floor with dribbles of your pre.' );
		}
		if( CoC.player.gender === 3 ) {
			MainView.outputText( '  You also feel ' );
		}
		if( CoC.player.hasVagina() ) {
			MainView.outputText( 'your [vagina] clenching and unclenching as your entire hips try to push against the Minotaur\'s searching tongue, dripping with his spit - along with your own juices.' );
		}
