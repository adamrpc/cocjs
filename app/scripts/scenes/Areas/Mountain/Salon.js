...
angular.module( 'cocjs' ).run( function( $rootScope, CoC, PerkLib, Appearance, Descriptors, kFLAGS, Utils, StatusAffects, PregnancyStore, EngineCore, ConsumableLib ) {
	function Salon() {
	...
	Salon.prototype.timeChange = function() {
		CoC.getInstance().flags[ kFLAGS.SALON_PAID ] = 0;
		if( CoC.getInstance().time.hours > 23 ) {
			if( CoC.getInstance().flags[ kFLAGS.LYNNETTE_CARRYING_COUNT ] === 0 || CoC.getInstance().flags[ kFLAGS.LYNNETTE_PREGNANCY_CYCLE ] !== 4 ) {
				CoC.getInstance().flags[ kFLAGS.LYNNETTE_PREGNANCY_CYCLE ]++;
			}
			if( CoC.getInstance().flags[ kFLAGS.LYNNETTE_PREGNANCY_CYCLE ] === 7 ) {
				CoC.getInstance().flags[ kFLAGS.LYNNETTE_PREGNANCY_CYCLE ] = 0;
			}
		}
		return false;
	};
	...
	Salon.prototype.hairDresser = function() {
		...
		EngineCore.doYesNo( this.salonGreeting, CoC.getInstance().scenes.camp.returnToCampUseOneHour );
	};
	Salon.prototype.salonGreeting = function() {
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.HairdresserMeeting ) >= 0 ) {
			...
			CoC.getInstance().player.createStatusAffect( StatusAffects.HairdresserMeeting, 0, 0, 0, 0 );
	...
	Salon.prototype.salonFavoritesPaymentMenu = function() {
		...
		var minoCum = null;
		if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00142 ] > 0 ) {
			minoCum = this.goblinHairDresserFacefuck;
		}
		EngineCore.menu();
		if( CoC.getInstance().flags[ kFLAGS.LYNNETTE_PREGNANCY_CYCLE ] >= 4 && CoC.getInstance().player.hasCock() ) {
	...
	Salon.prototype.buyMinoCum = function() {
		if( CoC.getInstance().player.gems < 60 ) {
			EngineCore.outputText( 'You can\'t afford any minotaur cum right now!', true );
			if( CoC.getInstance().flags[ kFLAGS.SALON_PAID ] === 0 ) {
	...
	Salon.prototype.salonPurchaseMenu = function() {
		CoC.getInstance().flags[ kFLAGS.SALON_PAID ] = 1;
		...
		if( CoC.getInstance().player.hairLength > 2 ) {
			cutShort2 = this.cutShort;
		}
		if( CoC.getInstance().player.hairLength > 13 ) {
			cutMedium2 = this.cutMedium;
		}
		if( CoC.getInstance().player.hairLength >= 26 ) {
			cutLong2 = this.cutLong;
		}
		if( CoC.getInstance().player.hairLength < CoC.getInstance().player.tallness ) {
			lengthening = this.hairGrow;
		}
		if( CoC.getInstance().player.femininity < 100 && CoC.getInstance().player.gender === 2 ) {
			mudFacial2 = this.mudFacial;
		} else if( CoC.getInstance().player.femininity < 85 && (CoC.getInstance().player.gender === 0 || CoC.getInstance().player.gender === 3) ) {
			mudFacial2 = this.mudFacial;
		} else if( CoC.getInstance().player.femininity < 70 && CoC.getInstance().player.gender === 1 ) {
			mudFacial2 = this.mudFacial;
		} else if( CoC.getInstance().player.femininity < 100 && CoC.getInstance().player.findPerk( PerkLib.Androgyny ) >= 0 ) {
			mudFacial2 = this.mudFacial;
		}
		if( CoC.getInstance().player.femininity > 0 && CoC.getInstance().player.gender === 1 ) {
			sandFacial2 = this.sandFacial;
		} else if( CoC.getInstance().player.femininity > 30 && CoC.getInstance().player.gender === 2 ) {
			sandFacial2 = this.sandFacial;
		} else if( CoC.getInstance().player.femininity > 20 && (CoC.getInstance().player.gender === 0 || CoC.getInstance().player.gender === 3) ) {
			sandFacial2 = this.sandFacial;
		} else if( CoC.getInstance().player.femininity > 0 && CoC.getInstance().player.findPerk( PerkLib.Androgyny ) >= 0 ) {
			sandFacial2 = this.sandFacial;
		}
	...
	Salon.prototype.hairDresserGreeting = function() {
		...
		if( CoC.getInstance().player.cockTotal() === 0 ) {
			...
			EngineCore.outputText( '  I\'ll even do you the favor of letting you blow it in my mouth, I\'ve already got a bun in the oven.  So what do you say?  Want a spooge and a haircut?  Or would you rather go get your payment from one of the gloryholes in the back, you ' + CoC.getInstance().player.mf( 'kinky boy', 'naughty girl' ) + '?</i>"\n\n', false );
	...
	Salon.prototype.hairDresserRepeatGreeting = function() {
		...
		if( Utils.rand( 5 ) === 0 && (CoC.getInstance().player.findPerk( PerkLib.MinotaurCumAddict ) >= 0 || CoC.getInstance().flags[ kFLAGS.MINOTAUR_CUM_ADDICTION_STATE ] > 0) ) {
			this.minotaurCumBukkakeInSalon();
			return;
		}
		//Had babies announcement!;
		if( CoC.getInstance().flags[ kFLAGS.LYNNETTE_PREGNANCY_CYCLE ] === 4 && CoC.getInstance().flags[ kFLAGS.LYNNETTE_CARRYING_COUNT ] > 0 ) {
			...
			EngineCore.outputText( 'at you. "<i>Hey there, honey. Come for some more service or to check up on the kids?</i>" She pats her now-flat midsection. "<i>I popped them out last night. We had ' + Utils.num2Text( CoC.getInstance().flags[ kFLAGS.LYNNETTE_CARRYING_COUNT ] ) + ' kids... all girls, of course.</i>" She tucks a lock of beautifully styled hair behind an elfin ear. "<i>Now you be sure and let me know when you want to contribute to my baby-bank again.</i>"' );
			if( CoC.getInstance().flags[ kFLAGS.LYNNETTE_BABY_COUNT ] > 0 ) {
			...
			CoC.getInstance().flags[ kFLAGS.LYNNETTE_BABY_COUNT ] += CoC.getInstance().flags[ kFLAGS.LYNNETTE_CARRYING_COUNT ];
			CoC.getInstance().flags[ kFLAGS.LYNNETTE_CARRYING_COUNT ] = 0;
			//If favorite!;
			if( CoC.getInstance().flags[ kFLAGS.LYNNETTE_ANNOUNCED_APPROVAL ] === 1 ) {
		...
		//Favorite Announcement;
		else if( CoC.getInstance().flags[ kFLAGS.LYNNETTE_APPROVAL ] >= 100 && CoC.getInstance().flags[ kFLAGS.LYNNETTE_ANNOUNCED_APPROVAL ] === 0 ) {
			...
			CoC.getInstance().flags[ kFLAGS.LYNNETTE_ANNOUNCED_APPROVAL ] = 1;
			//Custom menu with options to 'pay anyway';
			this.favoriteSalonMenu();
			return;
		}
		//Favorite Greeting ;
		else if( CoC.getInstance().flags[ kFLAGS.LYNNETTE_ANNOUNCED_APPROVAL ] === 1 ) {
			...
			else if( CoC.getInstance().flags[ kFLAGS.LYNNETTE_PREGNANCY_CYCLE ] < 4 ) {
		...
		//Non-Pregnant Greetings:;
			...
			if( CoC.getInstance().flags[ kFLAGS.LYNNETTE_MET_UNPREGNANT ] === 0 ) {
				CoC.getInstance().flags[ kFLAGS.LYNNETTE_MET_UNPREGNANT ] = 1;
	...
	Salon.prototype.gloryholeMinotaur = function() {
		...
		EngineCore.outputText( 'Driven on by the minotaur\'s reactions, you take more and more of the minotaur cock into your mouth. As you work your mouth around the first ring, you open up the top section of your ' + CoC.getInstance().player.armorName + ', freeing up your ' + Descriptors.allBreastsDescript() + '. With each pass, you\'re able to take more and more of the bull dick down your throat. You caress your ' + Appearance.biggestBreastSizeDescript() + ' while moaning around the dick. When you get your mouth around the second ring, the minotaur\'s cock throbs sharply, and his cum explodes in your mouth. Your eyes bulge in shock, and you pull off the cock as quickly as you can. You swallow the first several shots of thick, rich minotaur cum, while the rest shoots all over your face and tits. You ' );
		...
		EngineCore.outputText( 'Your head spins from the minotaur\'s musk, and you idly mop up and swallow the cum on your ' + CoC.getInstance().player.face() + '. A goblin aide comes in with a bowl, and gently scrapes the cum off your tits with a smooth, flat rock. Once you\'re cleaned up and you\'re dressed, the aide leads you back to Lynnette.\n\n', false );
	...
	Salon.prototype.goblinHairDresserFacefuck = function() {
		EngineCore.spriteSelect( 38 );
		EngineCore.outputText( 'Lynnette licks her lips and practically tears her way into your ' + CoC.getInstance().player.armorName + ', having your crotch exposed in seconds.  Your ' + Descriptors.cockDescript( 0 ) + ' flops out immediately, slapping her on the nose as it grows hard.  She wraps both hands around you and begins pumping with practiced ease, flicking her tongue over your crown and wrapping her lips ', true );
	...
	Salon.prototype.cutLong = function() {
		...
		EngineCore.outputText( 'Lynnette and her daughters crowd around you with razor-sharp scissors, effortlessly paring down your ' + Descriptors.hairDescript() + '.  When they\'ve finished, you\'re left with ', true );
		CoC.getInstance().player.hairLength = 25;
		EngineCore.outputText( Descriptors.hairDescript() + '.', false );
		EngineCore.doNext( CoC.getInstance().scenes.camp.returnToCampUseOneHour );
	};
	Salon.prototype.hairGrow = function() {
		...
		CoC.getInstance().player.hairLength += temp;
		EngineCore.outputText( Utils.num2Text( temp ) + ' more inches of ' + CoC.getInstance().player.hairColor + ' hair.', false );
		EngineCore.doNext( CoC.getInstance().scenes.camp.returnToCampUseOneHour );
	};
	...
	Salon.prototype.minotaurCumBukkakeInSalon = function() {
		...
		EngineCore.outputText( 'Tied down by the cum-spurting flare locked in your gullet, you pump on the other two dicks with feverish speed and sway on your ' + CoC.getInstance().player.feet() + ' as the narcotic spooge intoxicates your already-addled mind.  You can feel each muscular contraction pulsing through the bestial shaft while it finishes depositing the heavy, sticky load, and your eyes cross from the viscous inebriation that\'s pooling in your belly.  Drizzles of pre-cum soak into your arms and palms, drawn out from the frenzied pumping of your fists.  They won\'t come from just a hand though.  They need something... tighter.\n\n', false );
		...
		EngineCore.outputText( '\n\n', false );
		EngineCore.outputText( 'You lower and raise yourself, bouncing up and down on your ' + CoC.getInstance().player.legs() + ' to enhance the ', false );
		if( CoC.getInstance().player.biggestTitSize() >= 2 ) {
		...
		//ADD PREG CHECK;
		//Preggers chance!;
		CoC.getInstance().player.knockUp( PregnancyStore.PREGNANCY_MINOTAUR, PregnancyStore.INCUBATION_MINOTAUR, 70 );
		...
		//(buttchange here: 90);
		CoC.getInstance().player.buttChange( 90, true, false );
		...
		EngineCore.outputText( '\n\n', false );
		EngineCore.outputText( 'The strength goes completely out of your ' + CoC.getInstance().player.legs() + ', but you manage to hold yourself up long enough for your stud to flex his cock inside you and fill up your backdoor with more potent addiction.  You slide off, nerveless and still orgasming as jizz rains on your back from the abandoned cock, rolling off you to add to the ever-widening puddle of fluids on the floor.  Smiling vacantly and addled on more minotaur-cum than any one person has reason to handle, you stumble up on all fours and crawl to the next one.  More...', false );
		//[NEXT];
		CoC.getInstance().player.orgasm();
	...
	Salon.prototype.minotaurSalonFollowUp = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00142 ] === 0 ) {
			...
			CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00142 ]++;
			//text goez here;
			...
			EngineCore.outputText( 'You\'re woken up with a slap to the face, and still muzzy from your cum-induced glory-hole orgy, you stagger up to your ' + CoC.getInstance().player.feet() + '.  Lynnette shakes her stinging palm and apologizes, "<i>Sorry I have to keep doing that, but you\'re damned hard to wake after you go on these binges!  Anyways, you\'ve earned some hair treatment if you want it.  Of course you could always buy more of your favorite fluid...</i>"\n\n', false );
		}
		//Menu;
		EngineCore.outputText( 'Lynnette offers and explains their options, "<i>So what\'ll it be hun?  We could cut it down or give you a lengthening treatment. Or you can get a hair-dye to use on your own.  Just remember to come back in a few days for a touchup.</i>"', false );
		this.salonPurchaseMenu();
	...
	Salon.prototype.mudFacial = function() {
		EngineCore.spriteSelect( 38 );
		EngineCore.outputText( '', true );
		EngineCore.outputText( 'You sit back in a comfortable chair and pull on a lever to recline it.  The goblins buzz around you, gathering up \'special mud\'.  You close your eyes, letting them plaster your ' + CoC.getInstance().player.face() + ' with the stuff in hopes that it will improve your complexion as much as you\'ve been promised.  A pair of cucumber slices are laid out on your eyes, obscuring your view.\n\n', false );
		...
		CoC.getInstance().player.modFem( 100, 10 );
		EngineCore.doNext( CoC.getInstance().scenes.camp.returnToCampUseOneHour );
	};
	Salon.prototype.sandFacial = function() {
		EngineCore.spriteSelect( 38 );
		EngineCore.outputText( '', true );
		EngineCore.outputText( 'You sit back in a comfortable chair and pull on a lever to recline it.  The goblins buzz around you, gathering up \'special sand\'.  You close your eyes, letting them splatter your ' + CoC.getInstance().player.face() + ' with the rough, textured goop.  It doesn\'t feel very good, but that won\'t matter if it makes you as handsome as it\'s supposed to.\n\n', false );
		EngineCore.outputText( 'After a while the goblin girls come back and clean the stuff from your face. (+10 masculinity)', false );
		CoC.getInstance().player.modFem( 0, 10 );
		EngineCore.doNext( CoC.getInstance().scenes.camp.returnToCampUseOneHour );
	};
	/*
	...
	Salon.prototype.fuckLynnette = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.LYNNETTE_FUCK_COUNTER ] === 0 ) {
			...
			EngineCore.outputText( '.' );
			//IF FAIL!;
			if( CoC.getInstance().player.hoursSinceCum < 24 ) {
		...
		EngineCore.outputText( '\n\nYour thrusts transform from powerful lunges into a frenzied jackhammering so powerful that the impact ripples along her considerable thighs and ass.  Even her squirting tits shake and jostle with the pussy-plowing impacts as you mount the goblin like a rutting beast, battering your ' + CoC.getInstance().player.cockHead( x ) + ' ' );
		...
		if( CoC.getInstance().player.cockTotal() > 1 ) {
			...
			if( CoC.getInstance().player.longestCockLength() <= 12 ) {
		...
		EngineCore.outputText( '\n\nTiny hands latch onto your shoulders as you pour your distilled lusts into the lucky woman\'s womb, digging deeply into your ' + CoC.getInstance().player.skinFurScales() + '. Lynnette\'s lips crash back against yours, her tongue burrowing into your mouth before she traps your lower lip, biting it painfully hard. Her eyes roll back, and her body shakes with so much pleasure that she almost looks to be having a seizure.  Your cock doesn\'t care about that any more than her sperm-hungry uterus does, the two organs far too busy exchanging genetic material to care what state their owners are in.' );
		...
		if( CoC.getInstance().player.cumQ() < 1000 ) {
			...
			EngineCore.outputText( '\n\nThere\'s so much cum flowing out of your ' + CoC.getInstance().player.cockHead( x ) + ' that Lynnette\'s once-taut tummy immediately plumps up with spermy weight, filled into a jiggly paunch by your first few blasts.  As you continue to flood her womb with your slurry of baby-making goodness, her belly continues to rise up between you, eventually arcing up into a gravid dome.  Somehow, her body holds it all inside, and only a small trickle of spunk leaks out of her luscious slit as you finish.' );
		...
		//'UBUR';
			...
			if( CoC.getInstance().flags[ kFLAGS.LYNNETTE_ANNOUNCED_APPROVAL ] === 0 ) {
		...
		CoC.getInstance().flags[ kFLAGS.LYNNETTE_FUCK_COUNTER ]++;
		CoC.getInstance().player.orgasm();
		CoC.getInstance().flags[ kFLAGS.LYNNETTE_CARRYING_COUNT ] = 3 + Utils.rand( 3 );
		if( CoC.getInstance().player.cumQ() >= 1000 ) {
			CoC.getInstance().flags[ kFLAGS.LYNNETTE_CARRYING_COUNT ] += 1 + Utils.rand( 3 );
		}
		if( CoC.getInstance().player.cumQ() >= 2000 ) {
			CoC.getInstance().flags[ kFLAGS.LYNNETTE_CARRYING_COUNT ] += 1 + Utils.rand( 3 );
		}
		if( CoC.getInstance().player.cumQ() >= 3000 ) {
			CoC.getInstance().flags[ kFLAGS.LYNNETTE_CARRYING_COUNT ] += 2 + Utils.rand( 3 );
		}
	...
	Salon.prototype.lynnetteApproval = function( arg ) {
		if( arg !== undefined && arg !== 0 ) {
			CoC.getInstance().flags[ kFLAGS.LYNNETTE_APPROVAL ] += arg;
		}
		if( CoC.getInstance().flags[ kFLAGS.LYNNETTE_APPROVAL ] < -100 ) {
			CoC.getInstance().flags[ kFLAGS.LYNNETTE_APPROVAL ] = -100;
		} else if( CoC.getInstance().flags[ kFLAGS.LYNNETTE_APPROVAL ] > 100 ) {
			CoC.getInstance().flags[ kFLAGS.LYNNETTE_APPROVAL ] = 100;
		}
		return CoC.getInstance().flags[ kFLAGS.LYNNETTE_APPROVAL ];
	};
	CoC.getInstance().registerScene( 'salon', new Salon() );
} );