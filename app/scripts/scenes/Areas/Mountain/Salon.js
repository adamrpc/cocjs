...
angular.module( 'cocjs' ).run( function( SceneLib, MainView, $rootScope, CoC, PerkLib, Appearance, Descriptors, kFLAGS, Utils, StatusAffects, PregnancyStore, EngineCore, ConsumableLib ) {
	...
	Salon.prototype.hairDresser = function() {
		MainView.outputText( 'While exploring the mountain, you find a cleverly concealed doorway.  From inside you can hear the sound of blades being sharpened.  Do you enter the doorway?', true );
	...
	Salon.prototype.buyMinoCum = function() {
		if( CoC.player.gems < 60 ) {
			MainView.outputText( 'You can\'t afford any minotaur cum right now!', true );
			...
			CoC.player.gems -= 60;
			MainView.outputText( 'You happily give Lynnette 60 gems and pick up the bottle full of glistening, heavenly cum.  ', true );
	...
	Salon.prototype.hairDresserGreeting = function() {
		...
		if( CoC.player.cockTotal() === 0 ) {
			MainView.outputText( '  You look like you don\'t got any of your own, but we\'ve got glory holes in the back if you need to get some.  Just don\'t swallow too much, ok?</i>"\n\nShe shows you to the back of the cave, which is boarded-up.  There are about 20 holes in boards, and most are empty.  While you watch, a few new dicks slide in, and just as quickly the goblin\'s daughters commence sucking and fucking them.  There are only a few you could take a crack at - do you blow one (and if so which one)?', false );
		} else {
			MainView.outputText( '  I\'ll even do you the favor of letting you blow it in my mouth, I\'ve already got a bun in the oven.  So what do you say?  Want a spooge and a haircut?  Or would you rather go get your payment from one of the gloryholes in the back, you ' + CoC.player.mf( 'kinky boy', 'naughty girl' ) + '?</i>"\n\n', false );
			MainView.outputText( 'In the very back of the salon you can see a boarded-up wall with holes cut in it, some of which are currently plugged by various monstrous penises.  Do you let the goblin blow you, or do you go suck your payment from one of them?', false );
		}
	...
	Salon.prototype.hairDresserRepeatGreeting = function() {
		...
		if( CoC.flags[ kFLAGS.LYNNETTE_PREGNANCY_CYCLE ] === 4 && CoC.flags[ kFLAGS.LYNNETTE_CARRYING_COUNT ] > 0 ) {
			MainView.outputText( 'As soon as you enter the Salon, Lynnette is beaming ' );
			if( CoC.player.tallness >= 48 ) {
				MainView.outputText( 'up ' );
			}
			MainView.outputText( 'at you. "<i>Hey there, honey. Come for some more service or to check up on the kids?</i>" She pats her now-flat midsection. "<i>I popped them out last night. We had ' + Utils.num2Text( CoC.flags[ kFLAGS.LYNNETTE_CARRYING_COUNT ] ) + ' kids... all girls, of course.</i>" She tucks a lock of beautifully styled hair behind an elfin ear. "<i>Now you be sure and let me know when you want to contribute to my baby-bank again.</i>"' );
		...
		//Non-Pregnant Greetings:;
			...
			if( CoC.flags[ kFLAGS.LYNNETTE_MET_UNPREGNANT ] === 0 ) {
				...
				MainView.outputText( 'Lynnette\'s familiar face greets you at the door to her salon once more, and she waves you in with a beatific smile plastered upon her face. Clinging to her like a second skin, the businessgoblin\'s dress highlights her surprisingly flat midsection; she\'s not pregnant!' );
		...
		//Standard repeats;
			...
			MainView.outputText( 'Lynnette the goblin answers the door and lets you in, waving you deeper into her shop.  Her shining black dress barely contains her fertile-hips and jiggling chest as she greets you, "<i>Welcome back, honey!  ', true );
			if( CoC.player.cockTotal() === 0 ) {
				MainView.outputText( 'I see you still can\'t make payment for our services on your own.  No worries, the glory holes in the back are pretty popular with the demons and monsters, just go back there and catch some spooge and we\'ll see about helping with your hair, ok?</i>"\n\n(There are a number of dicks in the glory hole, which do you want to deal with (if you\'re willing at all)?', false );
			} else {
				MainView.outputText( 'Will you be getting our normal services?  Just one BJ per hair treatment!  Or would you rather get it from the gloryholes in back?</i>"', false );
			}
		}
		this.salonPaymentMenu();
	};
	...
	Salon.prototype.gloryholeDoggie = function() {
		CoC.player.slimeFeed();
		MainView.outputText( '', true );
		MainView.outputText( 'You sigh and kneel down to bring yourself level with the dog-dick hanging out of the wall.  It\'s pointed at the tip with a swollen circular bulb at the base.  As a matter of fact, the dog-dick\'s owner must be pretty excited to be here - it\'s dripping cum and the knot has swollen so large that it can\'t fit back through the hole.\n\n', true );
		if( CoC.player.cor < 33 ) {
			MainView.outputText( 'It\'s a struggle to force your lips apart and take the strange thing in your mouth.  You feel like a freak, but make yourself get to work servicing the male who\'ll be covering your hair treatment today.  It tastes salty and sweaty and has a potent musky scent that excites you in spite of your better judgment.  You find yourself starting to get into it as you lick and slurp, humming with pleasure.  You reach up to touch the knot and the dog-cock jumps in your mouth, spurting a wad of thick cum into your throat.  You pull back most of the way and pump the cock, collecting the jism in your mouth.\n\n', false );
			EngineCore.dynStats( 'lus', 15, 'cor', 0.25 );
		} else if( CoC.player.cor < 66 ) {
			MainView.outputText( 'You open wide and take as much of the pointed dick as you can into your mouth.  A few times you nearly gag yourself, but you manage to get most of the way down to the thick swollen knot.  You start sliding back and forth, breathing in the musky animalistic scent of your \'client\' and become aroused by the situation more and more.  You reach up and caress the knot, squeezing gently and stroking the dick until it unloads a thick wad of cum into your mouth.  You pull back and catch the fruits of your labor in your mouth, milking the dick with your hands until it\'s finished and the dick pops back through the hole.\n\n', false );
			EngineCore.dynStats( 'lus', 25, 'cor', 0.25 );
		} else {
			MainView.outputText( 'You throw yourself onto the swollen dog-cock and easily engulf the whole thing, spreading your lips wide around the thick knot.  You can feel it bouncing the uvula in your throat around but you manage to supress your gag reflex like a good slut while you work the sweaty dick, giving it a thorough tongue bath.  You wrap your lips around the base of the knot, feeling it start to stretch your jaws apart.  You pull back before it can lock you like that, and squeeze the knot in your hands.  It cums, unloading thick wads of gooey cum into your mouth.\n\n', false );
			EngineCore.dynStats( 'lus', 30, 'cor', 0.3 );
		}
		MainView.outputText( 'A young goblin comes by with a bowl for you to make your payment into.  You spit out the gunk and wipe your mouth, as the goblin carries the seed away.  You notice a trail of clear drops on the ground behind her.  She must be anticipating something...', false );
		EngineCore.doNext( this, this.hairDressingMainMenu );
	};
	Salon.prototype.gloryholeIncubus = function() {
		CoC.player.slimeFeed();
		MainView.outputText( '', true );
		MainView.outputText( 'You kneel down in front of the throbbing demonic dick, ready to earn your pay.\n\n', false );
		if( CoC.player.cor < 33 ) {
			MainView.outputText( 'You picked this dick hoping that a male lust demon would be the most pleasurable to service, and judging by the pleasant spicy scent the member gives off, you\'re right.  You open wide and take it inside your mouth, feeling the ridges around the crown pressing back against your lips. A drop of pre-cum hits your tongue and it tastes divine, spurring you on to please the tainted member.  You grab it with both hands and pump it, feeling the corrupted nodules under your fingers as you lick and suck on the head, eager for more.  You get your reward soon enough, and you eagerly start drinking down the copious waves of fluid.  Thankfully, the incubus\' orgasm seems to last forever, and when he finishes, you have a full belly and a mouth full of cream.  You manage to resist this last swallow, as you still need to pay.\n\n', false );
			EngineCore.dynStats( 'lus', 1, 'cor', 1.5 );
			CoC.player.lust = 99;
		} else {
			MainView.outputText( 'You take the corrupted cock in your hands and guide it into your waiting mouth, eager to try a walk on the wild side.  The incubus-cock does not disappoint, quickly dripping a steady flow of wonderful tasting pre-cum that only whets your appetite for the demon\'s cum.  You lean forwards, taking most of the bumpy purple dick inside your mouth and running your tongue all over it, paying special attention to the ring of nodules around the bottom edge of his crown.  You\'re quickly rewarded with a blast of heavenly demonic seed.  You gulp it down, craving more of it, and thankfully the incubus has plenty for you.  You\'re shivering and horny by the time he finishes, but thankfully you have enough willpower to keep a mouthful of cum for payment.\n\n', false );
	...
	Salon.prototype.goblinHairDresserFacefuck = function() {
		...
		if( CoC.player.cocks[ 0 ].cockThickness >= 4 ) {
			...
			MainView.outputText( 'around your first few inches', false );
		}
		MainView.outputText( '.  She slips her tongue down your urethra, and arches her back, popping the straps on her dress and letting her oversized tits engulf your shaft.  Milk sputters and dribbles from the lactating goblin, lubricating her breasts and hands as she pleasures every inch of you.\n\n', false );
		MainView.outputText( 'It\'s impossible to hold back while receiving such expertly given stimulation, and you blow your load, bulging her cheeks out like a squirrel\'s.', false );
		if( CoC.player.cumQ() >= 250 ) {
			MainView.outputText( ' She can\'t hold it all and suddenly starts swallowing as runnels of it escape her nose and a wave of it rolls down her lips.', false );
		}
		MainView.outputText( '  In time it ends, and she pops back, spitting most of the cum into a funnel.  It washes down a pipe and you have to wonder if there\'s some horny goblin girl at the other end with her cunt spread wide.', false );
	...
	Salon.prototype.cutShort = function() {
		...
		MainView.outputText( 'Lynnette and her daughters crowd around you with razor-sharp scissors, effortlessly paring down your ' + Descriptors.hairDescript() + '.  When they\'ve finished, you\'re left with ', true );
		CoC.player.hairLength = 1;
		MainView.outputText( Descriptors.hairDescript() + '.', false );
	...
	Salon.prototype.hairGrow = function() {
		...
		MainView.outputText( 'Lynnette grabs a bottle and squirts a white fluid into your hair.  You really hope it isn\'t your payment.  But it must not be, as within short order you feel the added weight of ', true );
		var temp = Utils.rand( 3 ) + 3;
		CoC.flags[ kFLAGS.HAIR_GROWTH_STOPPED_BECAUSE_LIZARD ] = 0;
		CoC.player.hairLength += temp;
		MainView.outputText( Utils.num2Text( temp ) + ' more inches of ' + CoC.player.hairColor + ' hair.', false );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	...
	Salon.prototype.minotaurCumBukkakeInSalon = function() {
		...
		if( CoC.player.hasVagina() || CoC.player.hasCock() ) {
			...
			if( CoC.player.hasCock() ) {
				if( CoC.player.cumQ() < 100 ) {
					...
					MainView.outputText( 'pre-cum a few inches wide ', false );
				} else {
					MainView.outputText( 'pre-cum over a foot wide ', false );
				}
			}
			MainView.outputText( 'winds over the rough-hewn floor behind you, clearly marking your passage to the overpowering musk.', false );
		}
		MainView.outputText( '\n\n', false );
		...
		MainView.outputText( 'You hear a deep, strangled sigh as more and more delicious pre floods your mouth, lighting your senses up with a fireworks show of pleasure', false );
		if( CoC.player.gender > 0 ) {
			MainView.outputText( ' and increasing the size of the puddle you\'re leaving on the floor', false );
		}
		MainView.outputText( '.  In between gulps of the fragrant fluid, you suck the swollen rod deeper and deeper into your mouth, gleefully suppressing your gag reflex as it pushes aside your uvula.  You work your throat, feverishly swallowing to suck it down.  A flare briefly distends your neck as it\'s taken deeper and deeper inside your form until your lips are puckered through the hole and you\'re sniffing the minotaur\'s pheromone-laced crotch.\n\n', false );
		MainView.outputText( 'The beast pulls back and starts pounding the hole, letting out deep, rumbling sighs of pleasure with each throat abusing glory-hole fuck.  If it weren\'t for his constant, bubbling preseed turning your throat to a slippery rut-hole you\'d be rubbed raw, but instead you continue to lean forward, sniffing at his matted pubes every time he crushes them against your dick-stretched lips.  The other horny studs grunt with displeasure, but you reach out with your hands and grab hold two eager brutes\' throbbing horsecocks.  Three out-of-sync heartbeats hammer through your hands and throat while you do your best to get them all off.\n\n', false );
		MainView.outputText( 'You let your eyes drift closed and lash your tongue back and forth across the minotaur\'s lowest medial ring, feeling his flare bulge larger inside you in response.  The beast-man batters at the wall, grunting in equal measures of passion and pent-up desire as his cock\'s tip stretches wide and locks itself inside you.  His urethra bulges in rhythmic contractions, stretching your lips around the already swollen shaft and signaling to your musk-addled mind that you\'re about to get what you desire.  You hum with blissful delight as your belly begins to gurgle, accepting long bursts of sticky minotaur drug.\n\n', false );
		MainView.outputText( 'Tied down by the cum-spurting flare locked in your gullet, you pump on the other two dicks with feverish speed and sway on your ' + CoC.player.feet() + ' as the narcotic spooge intoxicates your already-addled mind.  You can feel each muscular contraction pulsing through the bestial shaft while it finishes depositing the heavy, sticky load, and your eyes cross from the viscous inebriation that\'s pooling in your belly.  Drizzles of pre-cum soak into your arms and palms, drawn out from the frenzied pumping of your fists.  They won\'t come from just a hand though.  They need something... tighter.\n\n', false );
		MainView.outputText( 'You start moaning in drug-induced bliss, but your vocalized pleasure is interrupted by the squelching slurp of the softening shaft being pulled from your dick-puckered lips.  It drips a rope of cum over your mouth and chin as it pulls free from the wall, leaving behind one vacancy among the swarm of ready minotaur dicks.  You lick up your stud\'s leavings and purr in bliss, reaching through the hole to cup the departing minotaur\'s balls teasingly.  He grunts and walks out of your grip – sated for now.  You curl your fingers into a come-hither gesture, retract your arm through the sweat-lubricated glory-hole, and wait for one of the other minotaurs to come over.\n\n', false );
		MainView.outputText( 'The beast who kept your left hand so busy repositions himself at the now-vacant opening, and you decide to reward him for moving so quickly.  You lick the last of the salty cream from your lips and muse that he isn\'t the only one getting a reward, but the monstrous cow-man doesn\'t need to know that.', false );
		if( CoC.player.biggestTitSize() >= 2 ) {
			MainView.outputText( '  You wrap the pillowy flesh of your ' + Descriptors.allBreastsDescript() + ' around the new member, pleasantly surprised by its girth and wide, already-flared tip.  Maybe you could have gotten him off with your hands after all?  ', false );
			if( CoC.player.biggestTitSize() < 6 ) {
				MainView.outputText( 'Even so, you can\'t quite get your breasts the entire way around him, so you make up for it by pressing it harder into you with your busy hands.  ', false );
			}
			MainView.outputText( 'Runnels of pre cover the shaft and squish wetly between your tits, turning your body into a slip-n-slide that reeks of hot, sticky minotaur sex.', false );
		} else {
			MainView.outputText( 'You crush the member tightly against your chest, smearing your torso with the copious pre until you\'re like a hot, wet slip-n-slide of minotaur sex.  You make up for it by wrapping your arms around his prodigious thickness and squeeze him tightly with your hands, stroking him off with your whole body with enough tightness to make the poor mino\' think he\'s in some poor adventurer\'s asshole.', false );
		}
		MainView.outputText( '\n\n', false );
		MainView.outputText( 'You lower and raise yourself, bouncing up and down on your ' + CoC.player.legs() + ' to enhance the ', false );
		if( CoC.player.biggestTitSize() >= 2 ) {
			MainView.outputText( 'tit-fucking ', false );
		} else {
			MainView.outputText( 'full-body handjob ', false );
		}
		MainView.outputText( 'you\'re giving out.  In such a position, you\'re given the perfect view to watch as your strokes draw forth large bubbles of pre, and before you can lose your high, you latch onto the minotaur\'s vulnerable urethra and suck, tonguing in wide circles around it since you can\'t open wide enough to accommodate his flare.  Of course, all the attention just makes him flare wider, not just at the tip, but through the whole shaft.  ', false );
		if( CoC.player.biggestTitSize() >= 2 ) {
			MainView.outputText( 'The sudden girth change sends an enticing ripple through your ' + Descriptors.allBreastsDescript() + ' that\'s pleasurable enough to make you moan into his steadily-widening urethra.  ', false );
		}
		MainView.outputText( 'You pull off and bounce faster, lost to your lust and the haze of sex-musk permeating the air, intent on seeing just how much this huge stud can spray onto you.\n\n', false );
		MainView.outputText( 'The minotaur does not disappoint.  His hole dilates from the size of the approaching cum-blast, and you sink down his shaft slowly until it\'s aimed directly at your face.  You close your eyes and feel the first explosion splatter over your ' + Descriptors.hairDescript() + ' and forehead.  The next takes you full in the face, making it difficult to breathe through the mask of drug-like goo, but a few quick licks gives you a fix and makes it easy to breathe again.  On and on, the minotaur pumps fat ropes of spooge over your body until you\'re a syrupy, sticky mess that reeks of minotaur pheromones so strongly that dizziness overwhelms you and you fall free of the still-orgasming mino-cock, taking a few final blasts of seed on your ' + Descriptors.chestDesc() + ' and crotch.  Your hands instinctively shovel a few loads into your ' + Appearance.assholeOrPussy(), false );
		if( CoC.player.hasVagina() ) {
			MainView.outputText( ' while the animal part of your brain hopes it makes you pregnant with an equally girthy son', false );
		}
		MainView.outputText( '.\n\n', false );
		//ADD PREG CHECK;
		//Preggers chance!;
		CoC.player.knockUp( PregnancyStore.PREGNANCY_MINOTAUR, PregnancyStore.INCUBATION_MINOTAUR, 70 );
		MainView.outputText( 'Giggling, you stagger over to the next cock in line and turn around, possessed with the idea of taking its spooge in the most direct way possible – anally.   You pull your butt-cheeks apart and lean back, surprising one of the horny beasts with the warmth of your ' + Descriptors.assholeDescript() + ' as you slowly relax, spreading over his flare.  He actually squirts ropes of something inside of you, but you\'ve been around minotaurs enough to know that it can\'t be cum, at least not yet.  The slippery gouts of preseed make it nice and easy to rock back and spear yourself on the first few inches, ', false );
		if( CoC.player.analCapacity() < 80 ) {
			MainView.outputText( 'delighting in the drug-numbed pain of stretching yourself beyond your normal capacity.', false );
		} else if( CoC.player.analCapacity() < 140 ) {
			MainView.outputText( 'delighting in the feeling of perfect fullness.', false );
		} else {
			MainView.outputText( 'delighting in realizing that you could take far larger than even this virile specimen!', false );
		}
		//(buttchange here: 90);
		...
		if( !CoC.player.hasCock() ) {
			MainView.outputText( 'drags through your body, touching sensitive nerves you didn\'t even know you had until you cum, shuddering and shaking like a wanton whore.', false );
		} else {
			MainView.outputText( 'presses on your prostate as it squeezes by, making ' + Descriptors.sMultiCockDesc() + ' drip and spurt freely until you can bear it no longer and cum, shaking and shuddering like a wanton whore.  Jizz drips and pours from ' + Descriptors.sMultiCockDesc() + ' in a steady stream that pools on the floor, slowly rolling towards a drain that doubtless empties into a tank or greedy goblin cunt.', false );
		}
		MainView.outputText( '\n\n', false );
		MainView.outputText( 'The strength goes completely out of your ' + CoC.player.legs() + ', but you manage to hold yourself up long enough for your stud to flex his cock inside you and fill up your backdoor with more potent addiction.  You slide off, nerveless and still orgasming as jizz rains on your back from the abandoned cock, rolling off you to add to the ever-widening puddle of fluids on the floor.  Smiling vacantly and addled on more minotaur-cum than any one person has reason to handle, you stumble up on all fours and crawl to the next one.  More...', false );
		//[NEXT];
		CoC.player.orgasm();
	...
	Salon.prototype.minotaurSalonFollowUp = function() {
		...
		if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00142 ] === 0 ) {
			...
			//text goez here;
			MainView.outputText( 'Lynnette slaps your face, waking you from your stupor.  What?  Where are you?  You look around and realize you\'re strapped into a barber\'s chair and caked with white goop, but why?\n\n', false );
			MainView.outputText( 'The goblin answers before you can vocalize your query, "<i>Baby, you\'re so cum-hungry you make my daughters look like chaste virgins!  I haven\'t seen anyone go to town on minotaurs like that since my mother passed away, Marae guide her soul.  Normally we don\'t have that much use for the minotaurs outside of a bit of recreational drug-play, but you milked out so much cum that we can start selling it!  Isn\'t that great!?</i>"\n\n', false );
			MainView.outputText( 'You nod, a little shocked by your behavior, but already licking your lips in anticipation.  Lynnette smiles and says, "<i>You can buy some for the usual price, plus 60 gems, but for right now, how about some hair service?  It\'s on the house!</i>"', false );
		} else {
			MainView.outputText( 'You\'re woken up with a slap to the face, and still muzzy from your cum-induced glory-hole orgy, you stagger up to your ' + CoC.player.feet() + '.  Lynnette shakes her stinging palm and apologizes, "<i>Sorry I have to keep doing that, but you\'re damned hard to wake after you go on these binges!  Anyways, you\'ve earned some hair treatment if you want it.  Of course you could always buy more of your favorite fluid...</i>"\n\n', false );
		}
		//Menu;
		MainView.outputText( 'Lynnette offers and explains their options, "<i>So what\'ll it be hun?  We could cut it down or give you a lengthening treatment. Or you can get a hair-dye to use on your own.  Just remember to come back in a few days for a touchup.</i>"', false );
		this.salonPurchaseMenu();
	...
	Salon.prototype.fuckLynnette = function() {
		...
		if( CoC.flags[ kFLAGS.LYNNETTE_FUCK_COUNTER ] === 0 ) {
			...
			MainView.outputText( ' so that she can slip a manicured hand inside your underclothes. She handles [eachCock] ' );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 'in turn ' );
			}
			MainView.outputText( 'with the skill of a practiced snake-charmer before ' );
			if( CoC.player.balls === 0 ) {
				MainView.outputText( 'pressing down near the [sheath]' );
			} else {
				MainView.outputText( 'curling her petite fingers around your [sack] to heft it' );
			}
			MainView.outputText( '.' );
			//IF FAIL!;
		...
		//Repeat cum within 24 hours check;
			...
			MainView.outputText( 'Lynnette licks her lips at your decision and sashays forward, her immense hips rocking with enough sensuality that her plush ass is practically having a sexquake.  "<i>You\'ll understand if we do a quick check then, right, [name]?</i>" She doesn\'t wait for an answer, pressing herself against you so that you can get a good feel of her soft, sopping-wet tits conform to the shape of your body, letting the moist heat that seems to radiate from every pore of Lynnette\'s petite shape soak into your exposed ' + CoC.player.skinFurScales() + '.  Her hand dives right into your underwear without asking, seizing tight around [oneCock] to heft it.' );
			MainView.outputText( '\n\nYou give up an appreciative groan, [eachCock] twitching in lewd enjoyment as ' );
			if( CoC.player.lust <= 75 ) {
				MainView.outputText( 'it fills to full size almost immediately' );
			} else {
				MainView.outputText( 'it\'s fully-erect state makes itself known' );
			}
			MainView.outputText( '. Lynnette strokes a few times to make sure you\'re suitably engorged and then shifts lower, squeezing ' );
			if( CoC.player.balls === 0 ) {
				MainView.outputText( 'around your [sheath] to get a feel for how pent-up you are' );
			} else {
				MainView.outputText( 'gently at your [balls], weighing them in her palm to get a feel for just how pent-up you are' );
			}
			MainView.outputText( '.' );
			//NOT ENOUGH!;
			if( CoC.player.hoursSinceCum < 24 ) {
		...
		MainView.outputText( ' with hunger in her eyes.' );
		...
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 'es have' );
		} else {
			MainView.outputText( ' has' );
		}
		MainView.outputText( ' begun to bead clear droplets of precum. They run down the underside of your distended, eager cumvein' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ' and race down towards your ' );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( '[balls]' );
		} else {
			MainView.outputText( '[sheath]' );
		}
		MainView.outputText( ', leaving a glossy, sexual shine in their wake, and they\'re only coming on faster and faster the longer you gaze upon Lynnette\'s emerald twat and inhale her prick-stiffening scent.' );
		...
		MainView.outputText( ' won\'t take no for an answer.' );
		...
		if( CoC.player.cockArea( x ) > 50 ) {
			MainView.outputText( '  Her tummy bulges with the outline of your swollen mass, clearly displaying the shape of your bitch-breaking dong as it travels up ' );
			if( CoC.player.cockArea( x ) <= 80 ) {
				MainView.outputText( 'her belly.' );
			} else {
				MainView.outputText( 'into her ribcage, penetrating farther than any lesser race could ever hope to handle.' );
			}
		}
		MainView.outputText( ' The breeder grabs hold of your [hips] and tugs as hard as her meager strength will allow.  ' );
		if( CoC.player.cockArea( x ) <= 90 ) {
			MainView.outputText( 'Slapping against her hard enough to release a splash of girlish cum, you hilt yourself completely' );
			if( CoC.player.balls > 0 ) {
				MainView.outputText( ', [balls] resting on her soaked, juicy ass-cheeks.' );
			}
		} else {
			MainView.outputText( 'Eventually stopping, you look down to realize that she hasn\'t taken all of you. Going any farther would likely hurt her and ruin the fun, so you\'ll have to make to with partial strokes.' );
			if( CoC.player.balls > 0 ) {
				MainView.outputText( '  Her feet wrap around your [sack] to squeeze your [balls] affectionately, compensating quite nicely for her own shortcomings.' );
			}
		}
		...
		if( CoC.player.balls > 0 ) {
			MainView.outputText( ', making your sack feel tight and your testes bloated and tender.' );
		} else {
			MainView.outputText( ', making your gut clench and spasm with near-orgasmic contractions.' );
		}
		MainView.outputText( '  There\'s a tsunami of sperm building up inside you to the point where holding it in is actually starting to hurt you, but at the same time, your body refuses to give in and climax yet either.' );
		...
		if( CoC.player.cockArea( x ) > 24 ) {
			MainView.outputText( 'up against her slightly-yielding cervix' );
		} else {
			MainView.outputText( 'in as close to the cervix as you possibly can' );
		}
		MainView.outputText( ', grunting and growling as you lose yourself in the feel of her soaked, velvet tunnel.  Your focus collapses entirely down to the feel of that slick tunnel around you and how far you can push into it, how strongly you can make it climax and squeeze around you.' );
		...
		if( CoC.player.cumQ() >= 3000 ) {
			MainView.outputText( ', even if ' );
			if( CoC.player.cumQ() >= 10000 ) {
				MainView.outputText( 'most' );
			} else {
				MainView.outputText( 'some' );
			}
			MainView.outputText( ' of it is on the bed at this point' );
		}
		MainView.outputText( '.  Only then does she release you, allowing you to slide out' );
		if( CoC.player.hasKnot( x ) ) {
			MainView.outputText( ' with a wet-sounding \'pop\'.  Your knot does a terrific job of gaping her lips during its exit, and you cannot help but marvel at how well you\'ve broken this bitch in.  A shame she\'ll probably tighten up again in short order' );
		} else {
			MainView.outputText( ' with a gush of girlspunk to chase you' );
		}
		MainView.outputText( '.  She groans, ' );
		//BELOW ONE LITER!;
		if( CoC.player.cumQ() < 1000 ) {
			MainView.outputText( '"<i>[name], that was fun, but you barely even came!  How am I supposed to birth a dozen daughters at once if you barely coat the surface of my womb!?</i>"  She shudders.  "<i>Go on. I\'ve got to find some drugs to fix this, else I won\'t get a decent brood out of this term.</i>"\n\nYou leave her feeling a little underappreciated.  Maybe you could find some cum-boosting supplements out there?  Still, you are owed a haircut...' );
			this.lynnetteApproval( -2 );
		}
		//'GOOD';
		else if( CoC.player.cumQ() <= 10000 ) {
			if( CoC.flags[ kFLAGS.LYNNETTE_ANNOUNCED_APPROVAL ] === 1 ) {
				MainView.outputText( '"<i>Wow, [name].  You cum like a minotaur... and without drugging me out of my mind to boot!  Gods, I can feel the little swimmers hunting down my eggs already.</i>"  She giggles and retrieves a vial, downing it in a single gulp.  "<i>I better make sure there\'s enough eggs for them all, huh?</i>"\n\nYou leave with a happy smile. It seems Lynnette approves of you as a mate.' );
			}
			//'GOOD + LYNNETTE FAVORITE';
			else {
				MainView.outputText( '"<i>Mmmm, [name], I love the way you pack me full.</i>"  She tosses back a vial of fertility-boosting chemicals and sighs.  "<i>Do me a favor and come back soon.  I\'d rather be pouring out swarms of your babies than a slurry of weakling sluts.</i>"' );
			}
