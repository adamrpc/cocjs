...
angular.module( 'cocjs' ).run( function( SceneLib, CoC, kFLAGS, Utils, EngineCore, EventParser ) {
	...
	Swamp.prototype.exploreSwamp = function() {
		...
		if( !SceneLib.kihaFollower.followerKiha() && CoC.getInstance().player.cor < 60 && CoC.getInstance().flags[ kFLAGS.KIHA_AFFECTION_LEVEL ] >= 1 && CoC.getInstance().flags[ kFLAGS.HEL_FUCKBUDDY ] > 0 && CoC.getInstance().player.hasCock() && CoC.getInstance().flags[ kFLAGS.KIHA_AND_HEL_WHOOPIE ] === 0 ) {
