...
angular.module( 'cocjs' ).run( function( $log, CoC, kFLAGS, Utils, StatusAffects, PregnancyStore, EngineCore, XmasBitch, AppearanceDefs, PerkLib, Imp, Goblin, ComsumableLib, Combat, XmasJackFrost, OnLoadVariables, XmasGatsNotAnAngel ) {
	...
	Mountain.prototype.exploreMountain = function() {
		...
		if( Utils.rand( 100 ) <= 77 ) {
			if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00141 ] < 3 ) {
		...
		if( Utils.rand( 10 ) === 0 && CoC.getInstance().player.findStatusAffect( StatusAffects.HairdresserMeeting ) < 0 ) {
			chooser = 4;
		}
		...
		if( CoC.getInstance().player.findPerk( PerkLib.MinotaurCumAddict ) >= 0 && Utils.rand( 10 ) === 0 ) {
			chooser = 1;
		}
		...
		if( chooser === 1 ) {
			EngineCore.spriteSelect( 44 );
			if( CoC.getInstance().player.findStatusAffect( StatusAffects.TF2 ) < 0 && CoC.getInstance().player.level <= 1 && CoC.getInstance().player.str <= 40 ) {
				if( EngineCore.silly() ) {
					...
					EngineCore.outputText( 'Crossing over the treacherous mountain paths, you walk past an ominous cave.  The bones and the smell of death convince you to hasten your pace.  However, as you walk by, you hear a deep bellow and a snort as a monstrous man with a bull\'s head steps out.  With hell in his eyes and a giant ax in his hand, he begins to approach you in clear rage.  As he comes out into the light, you see that he is completely naked and sports a monstrous erection as angry as the minotaur himself, freely leaking a steady stream of pre-cum as he stalks you.\n\n', true );
					EngineCore.outputText( 'You stumble in your attempt to escape and realize that you are completely helpless.  The minotaur towers over you and heaves his ax for a <i>coup de grace</i>.  As he readies the blow, a monstrous explosion rocks the entire mountainside, causing the bull-man to stumble before he can finish you off. You look around, bewildered, trying to understand this strange new turn of events, and notice a group of maybe half a dozen people approaching from further up the path.  They appear to be a motley crew clad in blue and carrying monstrous weapons.  The tallest man holds a weapon made of multiple rotating tubes, and begins spinning the barrels.  A second later, while screaming in a language you do not understand, a rain of lead begins shredding the minotaur into a cloud of blood and flesh.\n\n', false );
					EngineCore.outputText( 'An equally imposing black man with a patch over one eye begins firing canisters at the beast, which explode violently.  "<i>Ya ragged-arsed beast man!</i>" he taunts.  "<i>Ye should pick on someone yer own size, BOY-O! HEHEHE!</i>"\n\n', false );
					EngineCore.outputText( 'Coming up the path next is a freak of a person clad in a contained shiny suit with a weapon that burns with flame.  He freely walks into the explosions and gunfire and begins igniting the beast.\n\n', false );
					EngineCore.outputText( '"<i>MRPHHUMFHRUFH!!!!! HUMFHUFMMRUF!</i>" the freak mumbles through his mask.\n\n', false );
					EngineCore.outputText( '"<i>I like me steak well done, ye crazy bugger!</i>" yells the black man.\n\n', false );
					EngineCore.outputText( 'The beast collapses in a charred and bloody heap.   As you stand back up, you hear a strange noise behind you.  You turn around to find a well-dressed man wearing a ski mask and smoking a cigarette.  "<i>Don\'t you know ze mountains are dangereuse,</i>" the man says with a thick accent.  "<i>You will get FUCKED up here if you are not careful.</i>"\n\n', false );
					EngineCore.outputText( 'You thank the man and his team, but they brush off your gratitude.  "<i>Non, non!</i>" the man with the accent says.  "<i>As zey say, everyone gets ONE.</i>" With that, he touches the watch on his wrist and disappears.  The rest of the group continues on their way.\n\n', false );
					EngineCore.outputText( 'As they leave, the giant with the chain gun yells in a horribly accented manner, "<i>YOU LEAVE SANDVICH ALONE! SANDVICH IS MINE!</i>"\n\n', false );
					EngineCore.outputText( 'With that, another hail of bullets break the scene as they walk away, leaving you safe from the minotaur, but utterly baffled as to what in hell just happened.', false );
				} else {
					EngineCore.outputText( 'Crossing over the treacherous mountain paths, you walk past an ominous cave.  The bones and the smell of death convince you to hasten your pace.  However, as you walk by, you hear a deep bellow and a snort as a monstrous man with a bull\'s head steps out.  With hell in his eyes and a giant ax in his hand, he begins to approach you in clear rage.  As he comes out into the light, you see that he is completely naked and sports a monstrous erection as angry as the minotaur himself, freely leaking a steady stream of pre-cum as he stalks you.\n\n', true );
			...
			//Mino gangbang
			if( CoC.getInstance().player.findStatusAffect( StatusAffects.MinoPlusCowgirl ) < 0 || Utils.rand( 10 ) === 0 ) {
				if( CoC.getInstance().flags[ kFLAGS.HAS_SEEN_MINO_AND_COWGIRL ] === 1 && CoC.getInstance().player.horns > 0 && CoC.getInstance().player.hornType === AppearanceDefs.HORNS_COW_MINOTAUR && CoC.getInstance().player.earType === AppearanceDefs.EARS_COW && CoC.getInstance().player.tailType === AppearanceDefs.TAIL_TYPE_COW && CoC.getInstance().player.lactationQ() >= 200 && CoC.getInstance().player.biggestTitSize() >= 3 && CoC.getInstance().player.minotaurAddicted() ) {
					//PC must be a cowmorph (horns, legs, ears, tail, lactating, breasts at least C-cup)
					//Must be addicted to minocum
					EngineCore.outputText( 'As you pass a shadowy cleft in the mountainside, you hear the now-familiar call of a cowgirl echoing from within.  Knowing what\'s in store, you carefully inch closer and peek around the corner.' );
					...
					if( CoC.getInstance().player.hasVagina() ) {
						EngineCore.outputText( 'dampens' );
					} else {
						EngineCore.outputText( 'twinges' );
					}
					EngineCore.outputText( ', aching to be filled' );
					if( CoC.getInstance().player.hasCock() ) {
						EngineCore.outputText( ', while [eachCock] rises to attention, straining at your [armor]' );
					}
					EngineCore.outputText( '.' );
				...
				CoC.getInstance().flags[ kFLAGS.HAS_SEEN_MINO_AND_COWGIRL ] = 1;
				if( CoC.getInstance().player.findStatusAffect( StatusAffects.MinoPlusCowgirl ) < 0 ) {
					CoC.getInstance().player.createStatusAffect( StatusAffects.MinoPlusCowgirl, 0, 0, 0, 0 );
				} else {
					CoC.getInstance().player.addStatusValue( StatusAffects.MinoPlusCowgirl, 1, 1 );
				}
				EngineCore.outputText( 'As you pass a shadowy cleft in the mountainside, you hear the sounds of a cow coming out from it. Wondering how a cow got up here, but mindful of this land\'s dangers, you cautiously sneak closer and peek around the corner.\n\n', true );
				...
				EngineCore.outputText( 'A part of your mind registers bits of clothing tossed aside and the heady scent of impending sex in the air, but your attention is riveted on the actions of the pair. The cow-girl turns and places her hands on a low ledge, causing her to bend over, her ample ass facing the minotaur. The minotaur closes the distance between them in a single step.\n\n', false );
		...
		if( chooser === 2 ) {
			...
			if( CoC.getInstance().player.findStatusAffect( StatusAffects.WormsOn ) >= 0 && CoC.getInstance().player.findStatusAffect( StatusAffects.Infested ) < 0 ) {
				if( CoC.getInstance().player.findStatusAffect( StatusAffects.WormsHalf ) >= 0 && Utils.rand( 2 ) === 0 ) {
					if( CoC.getInstance().player.cor < 90 ) {
						EngineCore.outputText( 'Your hike in the mountains, while fruitless, reveals pleasant vistas and provides you with good exercise and relaxation.', true );
						EngineCore.dynStats( 'tou', 0.25, 'spe', 0.5, 'lus', CoC.getInstance().player.lib / 10 - 15 );
					} else {
	...
	Mountain.prototype.joinBeingAMinoCumSlut = function() {
		...
		if( CoC.getInstance().player.tallness <= 96 ) {
			EngineCore.outputText( ' despite your immense height' );
		}
		EngineCore.outputText( ' as they look you up and down.  The entire area goes silent, even the goblins and the imps that are no doubt watching seem to be holding their breath, wondering what will happen to you.' );
		...
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( 'ignoring your cock' );
			if( CoC.getInstance().player.cockTotal() > 1 ) {
				EngineCore.outputText( 's' );
			}
			EngineCore.outputText( ' entirely, ' );
		}
		EngineCore.outputText( 'slipping a finger into your moistening ' );
		if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( 'pussy' );
		} else {
			EngineCore.outputText( 'asshole' );
		}
		EngineCore.outputText( '.  A low moan escapes your lips as a second finger slips in while the busty bovine woman\'s thumb ' );
		if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( 'swirls around your clitoris' );
		} else {
			EngineCore.outputText( 'presses against your perineum' );
		}
		EngineCore.outputText( '.' );
		...
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( ', once again ignoring your manhood completely' );
		}
		EngineCore.outputText( ', dives tongue first into your wet ' );
		if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( 'quim' );
		} else {
			EngineCore.outputText( 'back door' );
		}
		EngineCore.outputText( '.  The movement is so quick that you can\'t even suppress the sudden, perverted moan that leaves your lips... a moan that sounds shockingly like a cow\'s moo.  The surprise at your sudden bovine outburst quickly dissipates as the cow-girl\'s large tongue dips in and out of your ' );
		if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( 'sodden box' );
		} else {
			EngineCore.outputText( 'moist butthole' );
		}
		EngineCore.outputText( '.  Any remaining fears of joining this very public sex show are gone, and you wonder why you didn\'t join in sooner.' );
		...
		if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( 'cow-pussy' );
		} else {
			EngineCore.outputText( 'ass' );
		}
		EngineCore.outputText( ' right there,</i>" he groans, more of his bombastic cock slipping deep inside you.  The minotaur hooks an arm under each of your knees, lifting up your lower body, pressing even deeper.  Powerful sensations drift up from your ' );
		if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( 'g-spot' );
		} else {
			EngineCore.outputText( 'prostate' );
		}
		EngineCore.outputText( ' as the minotaur\'s wide flare strokes it through your ' );
		if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( 'vaginal' );
		} else {
			EngineCore.outputText( 'anal' );
		}
		EngineCore.outputText( ' walls.  Biting your lip with barely contained pleasure, you bring your hands to your breasts, playing with your milk-sodden nipples in between each orgasmic thrust of the bull\'s hips.' );
		...
		EngineCore.outputText( 'tongue inside her, eagerly licking out and guzzling down the remnants of the minotaur\'s present.' );
		...
		EngineCore.outputText( ' increases slowly, and you can feel your limbs tingling at the prospect of your mino-cum-induced orgasm.' );
		...
		EngineCore.outputText( ' like a Christmas tree.  You\'re acutely aware of every single movement, every pulse, every little bit of contact between you and the huge cock living inside you.' );
		...
		EngineCore.outputText( '.  The moment that long-awaited jism hits your walls, it\'s like another lightning bolt hits.  It travels up your spine and sets your entire brain aglow.  Ecstasy wrapped in bliss with a side of euphoric rapture consumes your thoughts.  Your vision goes white, pearly white like the seed filling your body, and your lips part as a primal "<i>moo</i>" slips out.' );
		...
		EngineCore.outputText( 'womb with his divine dick.' );
		...
		EngineCore.outputText( 'breasts around his monolithic shaft.  Your faces meet and her soft lips press against yours, each of you earnestly pressing your tongues into the other\'s mouths, swapping the juices you\'ve collected over the past hour or so.  The bull beneath you groans, awakening to the feeling of four breasts surrounding his love muscle.' );
		...
		if( CoC.getInstance().flags[ kFLAGS.PC_FETISH ] > 0 ) {
			EngineCore.outputText( '  A thrill runs through you.  Even though you were brought to such a satisfying climax, the whole thought that goblins and imps were watching you and getting off on it... it just makes you hornier than you were before.' );
	...
	Mountain.prototype.continueMinoVoyeurism = function() {
		EngineCore.outputText( 'They go at it for nearly an hour, oblivious to you watching them, before their intensity heightens as they near orgasm. The results are almost explosive, both of them crying out as they begin twitching uncontrollably. Clinging desperately to the cow-girl\'s ass, the minotaur pumps so much cum into her depths that it begins spurting out. This accidental lubrication releases his grip and the pair collapse to the ground. Yet the minotaur isn\'t finished, his man-milk spraying into the air almost like his still-erect dick is a hose and splattering down onto both of them.\n\n', true );
		EngineCore.outputText( 'As you look at the two cum-covered creatures laying their in their exhausted sex-induced stupors, the minotaur\'s thick horse-cock now slowly deflating, you realize that you\'ve been touching yourself.  You make yourself stop ', false );
		//[low corruption]
		if( CoC.getInstance().player.cor < 33 ) {
			EngineCore.outputText( 'in disgust.', false );
		} else if( CoC.getInstance().player.cor < 66 ) {
			EngineCore.outputText( 'in confusion.', false );
		} else {
			EngineCore.outputText( 'reluctantly.', false );
		}
		...
		if( CoC.getInstance().player.statusAffectv1( StatusAffects.MinoPlusCowgirl ) === 0 ) {
			EngineCore.outputText( '  Apparently this isn\'t an uncommon show, and the locals enjoy it immensely.', false );
		}
		//Lust!
