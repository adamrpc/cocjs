...
angular.module( 'cocjs' ).run( function( SceneLib, MainView, ArmorLib, Utils, Satyr, PregnancyStore, kFLAGS, CoC, EngineCore, Descriptors ) {
	...
	SatyrScene.prototype.satyrEncounter = function( location ) {
		...
		//Pregnant PCs can't get this encounter.;
			...
			MainView.outputText( 'You wander through the ' );
			if( location === 0 ) {
				MainView.outputText( 'grassy plains' );
			} else {
				MainView.outputText( 'sodden expanse of the swamp' );
			}
			MainView.outputText( ' when you hear strange music emanating not far from where you are.  Do you investigate?' );
			//[Yes][No];
			if( location === 0 ) {
	...
	SatyrScene.prototype.keepDrinking = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 98 );
		MainView.outputText( 'You grin at the satyr\'s encouragement and continue drinking, setting on a slower pace so you won\'t spill any more; shortly you pass him the empty skin and ask for more.' );
		...
		MainView.outputText( '.  This must be the work of that satyr!  Mentally, you remind yourself to watch out for him next time.  You clean yourself up as best as you can and redress, then wobble your way towards your camp, trying to stifle the pain, in your head and elsewhere, along the way.' );
	...
	SatyrScene.prototype.trickZeSatyr = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 98 );
		MainView.outputText( 'You come up with a plan and pretend to start drinking again; once you notice the satyr is distracted, you quickly spill most of your drink on the floor and return an empty skin to him.' );
	...
	SatyrScene.prototype.loseToSatyr = function() {
		...
		if( CoC.player.lust > 99 ) {
			MainView.outputText( 'You pant as you begin masturbating furiously, too horny to care about anything the grinning satyr before you has in mind.' );
		} else {
			MainView.outputText( 'You try to steady yourself, clutching your body in pain as the satyr grins at you.' );
		}
		...
		if( CoC.player.isNaga() ) {
			MainView.outputText( 'pins down your serpentine body' );
		} else if( CoC.player.isBiped() ) {
			MainView.outputText( 'spreads your [legs]' );
		} else {
			MainView.outputText( 'pins you down' );
		}
		MainView.outputText( ' and reaches forward, probing for a vagina.  ' );
		if( CoC.player.hasVagina() ) {
			MainView.outputText( 'Upon finding one, he grins lustfully, his already impressively erect cock rising to full flag.' );
		} else {
			MainView.outputText( 'Finding you lacking that trait, he grunts disdainfully and probes roughly for an asshole, which he locates right between your buttcheeks where it belongs, then jerks himself with several strokes to help coax his masculinity to full erection.' );
		}
		MainView.outputText( 'He grabs your [ass] and roughly squeezes your cheeks, kneeling down while lifting you up so he can impale your [vagOrAss].' );
		...
		if( (CoC.player.hasVagina() && CoC.player.vaginalCapacity() < CoC.monster.cockArea( 0 )) || (!CoC.player.hasVagina() && CoC.player.analCapacity() < CoC.monster.cockArea( 0 )) ) {
			MainView.outputText( ', not even bothered by the fact that his cock doesn\'t even fit inside you' );
		}
		MainView.outputText( '.' );
		//[cunt/buttchange];
		if( CoC.player.hasVagina() ) {
		...
		if( CoC.player.hasCock() ) {
			MainView.outputText( '  [EachCock] throbs as it send strings of cum flying straight into the satyr\'s chest, painting it white.' );
		}
		if( CoC.player.hasVagina() ) {
			MainView.outputText( '  A veritable flood of girlcum escapes the small space between your vaginal walls and the satyr\'s massive shaft, covering his balls and legs with your female fluids.' );
		}
		...
		MainView.outputText( '; your belly begins to swell and bulge as he pumps liters of semen into you.' );
		...
		if( CoC.player.cor < 50 ) {
			MainView.outputText( 'shamefully ' );
		} else {
			MainView.outputText( 'shamelessly ' );
		}
		MainView.outputText( ' gather and re-dress in your garments and head back to camp, cum still dribbling from you as you go.' );
	...
	SatyrScene.prototype.defeatASatyr = function() {
		...
		if( CoC.monster.lust > 99 ) {
			MainView.outputText( 'The satyr collapses to its caprine knees, bleating in dismay as it paws frantically at its huge cock, oblivious to everything in its need to get off.  Already, pre-cum is fountaining from the goat-man\'s shaft, his jerking motions smearing the pungent sexual fluid across the crown.' );
		}
		//HP Victory;
		else {
			MainView.outputText( 'Beaten and dazed, the satyr collapses to its caprine knees, shaking his head in a futile attempt to recover himself from the brutal trouncing you\'ve just given him.  The combination of the blows and his previous drunken state mean he\'s quite incapable of getting back, however.' );
		}
		var butt = null;
		var faces = null;
	...
	SatyrScene.prototype.femaleTakesAdvantageOfSatyr = function() {
		...
		if( CoC.player.hasCock() ) {
			MainView.outputText( ', your own cock belching its usual load across his stomach and the surroundings' );
		}
		MainView.outputText( '.' );
	...
	SatyrScene.prototype.malesTakeAdvantageOfSatyrs = function() {
		...
		if( CoC.player.cumQ() < 250 ) {
			MainView.outputText( '  He tries to milk you for far more than you can give him, and at one point you feel like his anal muscles are going to tear your dick off, until finally they go lax and lets you slip out.' );
		}//(Medium Cum Amount);
		else if( CoC.player.cumQ() < 1000 ) {
			MainView.outputText( '  You dump far more than you usually do; the satyr\'s clenching asshole makes sure to drain you of every little drop of cum you have until finally it goes lax and lets you slip out, leaving the goat-man with a pudgy belly.' );
		}//(High Cum Amount);
		else {
			MainView.outputText( '  You shoot jet after jet of cum, far faster than the satyr\'s ass can milk you for it, and still he doesn\'t seem to stop trying.  His rippling muscles draw your cum inside his bowels in big bubbles.  When his ass finally goes lax, you\'re propelled halfway out of him by the force of your own backflowing seed; the satyr\'s ass looks like a wreck, and his belly is overflowing with so much spunk you\'re surprised it didn\'t come out of his mouth.' );
		}
	...
	SatyrScene.prototype.willinglyBoneSatyr = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 98 );
		MainView.outputText( 'The satyr eyes you up and down hungrily; his hands move to grope your [chest], gently tweaking your [nipples], then he moves down towards your ' );
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'hardening maleness, giving it a few strokes to bring it to full mast, then he moves on to your ' );
		}
		if( CoC.player.balls > 0 ) {
			MainView.outputText( '[balls], rolling each of your orbs around with reverence at the life-giving load they produce; finally he addresses your ' );
		}
		//((if PC has a pussy);
		if( CoC.player.hasVagina() ) {
			MainView.outputText( '[vagina], spreading your labia wide to see the interior of your rapidly moistening walls.' );
		} else {
			MainView.outputText( '[ass], spreading your cheeks apart to gaze at your winking rosebud.' );
		}

		MainView.outputText( '"<i>Wonderful!  With a [vagOrAss] like this I\'m sure you\'ll make a great ' + CoC.player.mf( 'father', 'mother' ) + ' for our children,</i>" he says, leaning closer to sniff at your ' );
		if( CoC.player.hasVagina() ) {
			MainView.outputText( '[vagina], licking all over your netherlips and tasting your juices.' );
		} else {
			MainView.outputText( '[butt], licking all around your hole to prepare it for what\'s coming.' );
		}
		...
		if( !CoC.player.hasVagina() ) {
			MainView.outputText( 'clenching butt' );
		} else {
			MainView.outputText( 'spasming vagina' );
		}
		MainView.outputText( '.  It\'s like someone shoved a hose into you and opened the nozzle, filling you with an endless stream of hot spooge; you can feel it pooling inside you, inflating your belly with his gigantic load.' );
		...
		if( CoC.player.hasVagina() ) {
			MainView.outputText( 'pussy gushes fluids down his massive manhood' );
		}
		if( CoC.player.gender === 3 ) {
			MainView.outputText( ' and ' );
		}
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'cock spurts cream across your intertwined forms' );
		}
		MainView.outputText( '.  Moaning softly and heaving in great lungfuls of breath, you sink back down onto him, feeling well and truly sated.' );
		...
		MainView.outputText( '; making slow, short thrusts to ensure you\'ve got all the seed that you need.  Panting, he grins at you from his prone position.  "<i>Tell me then... did you like it?  Think that was enough baby batter to put a little satyr inside you?  Because if not I can go again.</i>"  He grins confidently.' );
		...
		if( !CoC.player.hasVagina() && CoC.flags[ kFLAGS.SATYR_KIDS ] === 0 ) {
			...
			if( CoC.player.hasCock() ) {
				MainView.outputText( 'males' );
			} else {
				MainView.outputText( 'neuters' );
			}
			MainView.outputText( ' like you, lover.</i>"' );
	...
	SatyrScene.prototype.satyrBirth = function( vag ) {
		...
		if( vag && !CoC.player.hasVagina() ) {
			MainView.outputText( 'newly grown vagina' );
		...
		MainView.outputText( ', a gripping, writhing sensation as if something is thrashing around inside you and wants to get out.  Looks like it\'s time for whatever is inside you to emerge.' );
		...
		MainView.outputText( '; it\'s adorned with a pair of miniscule horns.  You groan in pain as its shoulders stretch you even wider, but the worst part of it is over and, within moments, you are delivered of your child.  ' );
		if( vag ) {
			CoC.player.cuntChange( 40, true, false, true );
		} else {
			CoC.player.buttChange( 40, true, false, true );
		}
		MainView.outputText( 'Long minutes pass as your body recovers from the strain, bouncing back with unnatural vitality from the exhaustion and pain of giving birth, but finally you are ready to look at it.' );
		...
		if( CoC.player.cor < 33 ) {
			MainView.outputText( ' and more than a bit disgusted' );
		}
		MainView.outputText( ', you slip into a short, fitful sleep.' );
		//badabingbadaboom;
