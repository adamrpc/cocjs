...
angular.module( 'cocjs' ).run( function( ArmorLib, Utils, Satyr, PregnancyStore, kFLAGS, Combat, CoC, EngineCore, Descriptors ) {
	...
	SatyrScene.prototype.satyrEncounter = function( location ) {
		...
		if( Utils.rand( 2 ) === 0 || CoC.getInstance().player.pregnancyIncubation > 0 || CoC.getInstance().player.buttPregnancyIncubation > 0 || CoC.getInstance().player.gender === 0 ) {
	...
	SatyrScene.prototype.consensualSatyrFuck = function( loc ) {
		...
		var trick = null;
		//(if High Int);
		if( CoC.getInstance().player.inte > 60 && CoC.getInstance().player.lust <= 99 ) {
	...
	SatyrScene.prototype.keepDrinking = function() {
		...
		EngineCore.outputText( '\n\nYou can\'t resist the soft melody and begin to feel sleepy; you stop your drinking and eating to yawn... now that you\'ve stopped, you feel awfully hot.  You strip off your [armor] without a second thought, not even bothering to protect your modesty.  Once you feel the cool air against your ' + CoC.getInstance().player.skinFurScales() + ' you sigh and lay to relax for a nap.' );
	...
	SatyrScene.prototype.loseToSatyr = function() {
		...
		if( (CoC.getInstance().player.hasVagina() && CoC.getInstance().player.vaginalCapacity() < CoC.getInstance().monster.cockArea( 0 )) || (!CoC.getInstance().player.hasVagina() && CoC.getInstance().player.analCapacity() < CoC.getInstance().monster.cockArea( 0 )) ) {
	...
	SatyrScene.prototype.defeatASatyr = function() {
		...
		var bikiniTits = null;
		if( CoC.getInstance().player.hasVagina() && CoC.getInstance().player.biggestTitSize() >= 4 && CoC.getInstance().player.armor === ArmorLib.LMARMOR ) {
	...
	SatyrScene.prototype.willinglyBoneSatyr = function() {
		...
		EngineCore.outputText( '"<i>Wonderful!  With a [vagOrAss] like this I\'m sure you\'ll make a great ' + CoC.getInstance().player.mf( 'father', 'mother' ) + ' for our children,</i>" he says, leaning closer to sniff at your ' );
		if( CoC.getInstance().player.hasVagina() ) {
		...
		if( !CoC.getInstance().player.hasVagina() && CoC.getInstance().flags[ kFLAGS.SATYR_KIDS ] === 0 ) {
	...
	SatyrScene.prototype.satyrPreggo = function() {
		if( CoC.getInstance().player.hasVagina() ) {
			CoC.getInstance().player.knockUp( PregnancyStore.PREGNANCY_SATYR, PregnancyStore.INCUBATION_SATYR ); //Satyrs can now fertilize eggs for ovipositing players
		} else {
			CoC.getInstance().player.buttKnockUp( PregnancyStore.PREGNANCY_SATYR, PregnancyStore.INCUBATION_SATYR, 1, 1 );
		}
	};
	//Birthing (Z);
	//Baby is always male.;
	SatyrScene.prototype.satyrBirth = function( vag ) {
		...
		if( vag && !CoC.getInstance().player.hasVagina() ) {
		...
		if( vag ) {
			CoC.getInstance().player.cuntChange( 40, true, false, true );
		} else {
			CoC.getInstance().player.buttChange( 40, true, false, true );
		}
