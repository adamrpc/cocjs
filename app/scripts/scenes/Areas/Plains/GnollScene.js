...
angular.module( 'cocjs' ).run( function( SceneLib, MainView, Combat, Gnoll, Appearance, CockTypesEnum, ConsumableLib, CoC, EngineCore, Descriptors, Utils ) {
	...
	GnollScene.prototype.getRapedByGnoll = function() {
		...
		MainView.outputText( '', true );
		MainView.outputText( 'The sound of the gnoll\'s mocking laughter grates in your ears as you collapse down on your knees before her.  She circles you with the last scrap of her wariness and then surges forward to knock you over, exposing your ', false );
		...
		if( CoC.player.gender > 0 ) {
			...
			MainView.outputText( 'flat, featureless groin.  ', false );
		}
		//  (if cockTotal=0 AND vaginas=1);
		if( !CoC.player.hasCock() ) {
			if( CoC.player.hasVagina() ) {
				MainView.outputText( 'The gnoll looks a little disappointed at the void between your legs.  "<i>Aw, I was hoping for something substantial,</i>" she titters.  That doesn\'t stop her from hefting her engorged clit-dick and slapping its tip onto the entrance of your ' + Descriptors.vaginaDescript( 0 ) + '.  With one jittery lurch forward, she shoves her clit up inside you, her pubic mound mashing up against your ' + Descriptors.clitDescript() + '.', false );
				CoC.player.cuntChange( 9, true, true, false );
				MainView.outputText( '\n\n', false );
			}
			//(if cockTotal=0 AND vaginas=0);
			else {
				MainView.outputText( 'The gnoll actually stops laughing for a moment when she takes in your featureless crotch.  "<i>Well.  That\'s a new one,</i>" she mutters.  She then takes two bobbing steps up the length of your body and rudely shoves her thumb into your mouth.  Her other hand guides her giant clitoris in after it, and you are in no position to stop her.\n\n', false );
			}
		}
		//(if cockTotal>0 AND ;
		if( CoC.player.cockTotal() > 0 ) {
			...
			if( CoC.player.thickestCockThickness() > 2 ) {
				...
				if( CoC.player.cockTotal() > 1 ) {
					MainView.outputText( 'The gnoll fishes into your ' + Descriptors.multiCockDescriptLight() + ' and quickly snaps up the thickest one.  ', false );
				}//(if cockTotal=1);
				else {
					MainView.outputText( 'The gnoll\'s hands dart down to your crotch and grabs ' + Descriptors.cockDescript( 0 ) + '.  ', false );
				}
				MainView.outputText( '"<i>Yes, this will do nicely,</i>" she says with relish, pumping your ' + Appearance.cockNoun( CoC.player.cocks[ CoC.player.thickestCock() ].cockType ) + ' until it plumps up, filling with blood.  She cantilevers her body over yours, knees bent sharply, and brings the tip of her massive clitoris up against the head of your dick.  The moment a dollop of pre forms at your urethra, her hips surge forward, sinking her giant clit down the length of your ' + Appearance.cockNoun( CoC.player.cocks[ CoC.player.thickestCock() ].cockType ) + '.  ', false );
				MainView.outputText( 'You gasp at the feeling of her hot skin rippling down the interior of your dick, and all she can do is laugh as she plunges it deeper and deeper into you.\n\n', false );
			}
			//(if cockTotal>0 AND thickestCock()<2");
			else {
				//(if cockTotal>1);
				if( CoC.player.cockTotal() > 1 ) {
					MainView.outputText( '"<i>This will have to do,</i>" she says with the barest trace of disappointment as she grabs your thickest cock.  ', false );
				}//(if cockTotal=1);
				else {
					MainView.outputText( '"<i>This will have to do,</i>" she says as she grabs your cock.  ', false );
				}
				MainView.outputText( 'She runs her rough hand up and down its length until it begins to plump up.  She cantilevers her body over yours, knees bent sharply, and brings the tip of her massive clitoris up against the head of your dick.  The moment a dollop of pre forms at your urethra, her hips surge forward, sinking her giant clit down the length of your ' + Appearance.cockNoun( CoC.player.cocks[ CoC.player.thickestCock() ].cockType ) + '.  ', false );
				MainView.outputText( 'You scream in pain as she forces her bizarre pseudo-penis down the length of your dick.  In horror, you watch as the bulge of her anatomy\'s invasion of yours slowly descends towards your ', false );
				if( CoC.player.balls > 0 ) {
					MainView.outputText( 'balls', false );
				} else {
					MainView.outputText( 'groin', false );
				}
				MainView.outputText( '.  All she can do is laugh as she plunges it deeper and deeper into you.\n\n', false );
				//(increase thickness of thickestCock());
				CoC.player.cocks[ CoC.player.thickestCock() ].cockThickness += 0.25;
			}
		}
		MainView.outputText( 'In no time whatsoever she falls into an easy rhythm, pistoning her obscene girl-dick in and out of you.  At first, you can do little more than gurgle and squirm under the assault, but all too soon you feel the telltale signs of your own arousal building.  You find yourself moving in sympathy with her thrusts, at least as best you can given the circumstances.', false );
		//(if breastRating>C);
		if( CoC.player.biggestTitSize() >= 3 ) {
			MainView.outputText( '  Your ' + CoC.player.allBreastsDescript() + ' bounce up and down as the gnoll grinds her anatomy into yours, slick with sweat under the hot sun.  Your ' + Descriptors.nippleDescript( 0 ) + 's tighten and flush as your whole body submits to the rough fuck administered by the hyena girl.', false );
		}
		//(if cockTotal>0);
		if( CoC.player.cockTotal() > 0 ) {
			MainView.outputText( '  Your hips begin to buck as your orgasm builds, but the gnoll slams her hands down on your sides, pinning you to the hardscrabble ground.  "<i>Best if you don\'t move too much, lover,</i>" she laughs, even as she quickens her pace.  Immediately your ', false );
			if( CoC.player.balls > 0 ) {
				MainView.outputText( Descriptors.sackDescript(), false );
			} else {
				MainView.outputText( 'body', false );
			}
			MainView.outputText( ' seizes tighter and you can feel your cum churning inside you.\n\n', false );
			MainView.outputText( 'When you finally explode, the effect is strange but not uncomfortable.  You feel your hot seed surging up out of you, but then it seems to disappear just at the base of your dick.  Blearily you look down at your crotch and see that where your sensation ends is where the tip of her clitoris has penetrated.  You are pumping cum directly into her pseudo-penis.\n\n"<i>That\'s it!</i>" she giggles, bouncing up and down atop you.  "<i>Give it to momma!  I\'ll suck the cum right out of you!</i>"  Finally, just as your own orgasm is fading, the gnoll screams up at the empty sky, her giant clitoris lurching within the confines of your dick.\n\n', false );
		}
		//(if cockTotal<0);
		else {
			MainView.outputText( '  You squirm in the dirt as your orgasm builds and the gnoll simply giggles.  Heat seems to flash across the length of your body, and then every muscle spasms and twitches all at once.  At the same time, the gnoll screams up at the empty sky, her giant clitoris lurching inside you.\n\n', false );
		}
	...
	GnollScene.prototype.defeatHyena = function() {
		...
		//Gnoll Defeat (HP)	;
		if( CoC.monster.HP < 1 ) {
			MainView.outputText( 'The hyena girl collapses backwards onto the dusty ground, unable to continue the fight.', false );
		}
		//Gnoll Defeat (Lust);
		else {
			MainView.outputText( 'The hyena girl heaves a giant breath and takes a single step backwards.  Her club thumps to the ground and her hands fall to her shaft, stroking along its rough length.', false );
		}
	...
	GnollScene.prototype.dickDownGnollClit = function() {
		MainView.outputText( '', true );
		MainView.outputText( 'This is not an opportunity you can pass up.  You roughly roll her onto her back and pull her long clit up to a vertical position.  She moans softly, and the rough skin beneath your fingers pulses as her arousal brings it harder and taller.  The end bloats larger, fuller, until finally it looks nearly wide enough.  You don\'t wait any further; you position the head of your dick against the end of hers and jam it inside.\n\n', false );
		MainView.outputText( 'Inch by inch, you sink your dick down into the warm tunnel of her monstrous clitoris.  The interior, you are happy to find, is not as rough as the exterior, and in fact is just slick enough to make your invasion possible.  After a few thrusts, you find it easiest to adopt a sort of reverse-cowgirl position, facing her feet as you roll your hips forward and shove more and more of your dick inside her.   Beneath you, the gnoll is clutching at the ground and making little submissive whimpers.\n\n', false );
		var x = CoC.player.longestCock();
		//(if biggestCockLength()>9);
		if( CoC.player.cocks[ x ].cockLength > 9 ) {
			MainView.outputText( 'Finally you can feel your cockhead push free into a deeper, wetter place.  You look down at her strange anatomy, the entire length of her tube bulging thicker to accommodate your ' + Descriptors.cockDescript( x ) + '.  You\'ve made it all the way to her vagina!\n\n', false );
		} else {
			MainView.outputText( 'Soon you feel the end of her clitoral tunnel butting up against your crotch.  You are as far in as you\'ll get.  You look down the length of her strange anatomy, the tube bulging thicker to accomodate your ' + Descriptors.cockDescript( x ) + '.  There are still inches between your cockhead and the end of her clit-dick.\n\n', false );
		}

		MainView.outputText( 'Now you begin to roll your hips in a steady rhythm, running your dick up and down her long clitoral tunnel.  It\'s like the deepest, tightest cunt you\'ve ever had, gripping every inch of you.', false );
		//(if balls>0);
		if( CoC.player.balls > 0 ) {
			MainView.outputText( '  It doesn\'t take long before your ' + Descriptors.ballsDescriptLight() + ' begin twitching, pumping cum down the length of your encased cock.', false );
		}
		MainView.outputText( '  Your orgasm is quick and hard.  Your entire body seems to tremble under the hot sun and your thrusting doubles and then redoubles, stuffing your dick down hers like mad.\n\n', false );
		//(if cumQ()<25);
		if( CoC.player.cumQ() < 25 ) {
			MainView.outputText( 'Your ' + Descriptors.cockDescript( x ) + ' convulses, pumping down the length of her clitoris.  She squirms and howls beneath you, hissing for more.', false );
		}//(if cumQ()>25 AND cumQ()<500);
		else if( CoC.player.cumQ() < 500 ) {
			MainView.outputText( 'Your ' + Descriptors.cockDescript( x ) + ' convulses, pumping cum into her.  Her clitoris bulges even more as the jism backs up.  The warm, wet, sticky insides swirl around your dick.', false );
		}//(if cumQ()>500);
		else {
			MainView.outputText( 'Your ' + Descriptors.cockDescript( x ) + ' convulses, pouring cum into her.  Her clitoris bulges even more as the jism backs up, the skin pulling tight and shiny as the pressure builds.  Finally gouts of cum begin spurting back out the tip of her clitoris, splashing against your crotch.', false );
		}
	...
	GnollScene.prototype.dickInGnollAss = function() {
		...
		if( Utils.rand( 2 ) === 0 ) {
			MainView.outputText( 'The gnoll must be taught a lesson, but you\'re staying the hell away from her freaky anatomy.  You roughly roll her onto her belly and pull her lean ass up into the air.  You line up your ' + Descriptors.cockDescript( x ) + ' and ram it home into her tiny puckered entrance, eliciting a half-conscious gasp from the hyena girl.\n\n', false );
			//(if cockTotal>1);
			if( CoC.player.totalCocks() > 1 && CoC.player.cockArea( x ) < CoC.monster.analCapacity() && y !== -1 ) {
				MainView.outputText( 'With a smirk, you pound away for a few minutes to get her loosened up.  Then you reach down and slap another ' + Appearance.cockNoun( CockTypesEnum.HUMAN ) + ' alongside the first.  With a grunt and a thrust, you shove both of them inside.', false );
				//(if cockTotal>2);
				if( CoC.player.totalCocks() > 2 && CoC.player.cockArea( x ) + CoC.player.cockArea( y ) < CoC.monster.analCapacity() ) {
					MainView.outputText( '  A few thrusts more, and you slow again.  As you gather up your third cock, the gnoll whimpers quietly, fearing what she knows will come next.  You slowly push forward, sinking your rigid meat into her ass.  When all three are in, it\'s very slow going, but you don\'t let up.', false );
				}
				//(if cockTotal>3);
				if( CoC.player.cockTotal() > 3 && CoC.player.cockArea( 0 ) + CoC.player.cockArea( 1 ) + CoC.player.cockArea( 2 ) < CoC.monster.analCapacity() ) {
					MainView.outputText( '  From there it\'s some time before her anus is stretched wide enough for more.  By now you can feel your orgasm on the horizon, so you\'re less careful with your fourth ' + Descriptors.cockDescript( y ) + '.  You slap it into the bundle of dicks and then ram it home.', false );
				}
				//(if cockTotal>4);
				if( CoC.player.cockTotal() > 4 && CoC.player.cockArea( 0 ) + CoC.player.cockArea( 1 ) + CoC.player.cockArea( 2 ) + CoC.player.cockArea( 3 ) < CoC.monster.analCapacity() ) {
					MainView.outputText( '  And then the next.', false );
				}
				//(if cockTotal>5);
				if( CoC.player.cockTotal() > 5 && CoC.player.cockArea( 0 ) + CoC.player.cockArea( 1 ) + CoC.player.cockArea( 2 ) + CoC.player.cockArea( 4 ) + CoC.player.cockArea( 5 ) < CoC.monster.analCapacity() ) {
					MainView.outputText( '  And the next.', false );
				}
				//(if cockTotal>6);
				if( CoC.player.cockTotal() > 6 && CoC.player.cockArea( 0 ) + CoC.player.cockArea( 1 ) + CoC.player.cockArea( 2 ) + CoC.player.cockArea( 4 ) + CoC.player.cockArea( 5 ) + CoC.player.cockArea( 6 ) < CoC.monster.analCapacity() ) {
					MainView.outputText( '  And the next, until all of your ' + Descriptors.multiCockDescriptLight() + ' are inside her wide-spread ass.', false );
				}
				MainView.outputText( '\n\n', false );
			}
			MainView.outputText( '"<i>So how do you like it?</i>" you grunt, "<i>when a little too much is shoved up a hole that\'s a little too small?</i>"  You pound away, hard and rough, until her cries turn into squeals and then into moans.  Finally, she simply passes out.  You slap your hips against her bony ass for a few more thrusts and cum, long and hard, into her rectum.\n\n', false );
			MainView.outputText( 'You let her slump to the ground, leaking your cum into the sun-caked ground, and head home.', false );
		} else {
			MainView.outputText( 'Your mind is made up; this gnoll needs to learn her place.  You eye her freakish pseudo-phallus for a moment, then shake your head, roughly grabbing her by the bony hips and flopping her onto her belly.  She lets out a little barking whine when you firmly smack her bony rear, lowering her chin to the ground and raising her hips submissively in recognition of your dominance.\n\n', false );
			MainView.outputText( 'You stroke your ' + Descriptors.cockDescript( x ) + ' eagerly, bringing yourself to full mast and squeezing out a few drops of pre to pool against her puckered anus, leaning over and adding a little bit of saliva for good measure.  Sliding your shaft between her flat ass cheeks, you make sure to get the tip nice and lubed up, then you drop her hips down and ram yourself home in one go.\n\n', false );
			MainView.outputText( 'The gnoll lets out a squeal that sounds halfway like a maniacal laugh, sliding forward in the dirt a little.  Your ' + Descriptors.cockDescript( x ) + ' throbs wonderfully, fully engulfed by the tight passage of the gnoll\'s anus', false );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( ' while the remainder of your endowments slide along her bare buttocks', false );
			}
			MainView.outputText( ', and you begin to thrust forward and back, digging your fingers into her hips.\n\n', false );
			...
			MainView.outputText( 'Your hands slide further down her sides, and you bear down on her as you feel your climax building.  Increasing momentum, you piston into her ass with a desperate vigor, moaning your animalistic pleasure into her ear', false );
			if( CoC.player.cumQ() >= 500 ) {
				MainView.outputText( ' as your voluminous pre-cum turns her innards into a sloppy fuckhole', false );
			}
			MainView.outputText( '.\n\n', false );
			//(Low cumQ):;
			if( CoC.player.cumQ() <= 250 ) {
				MainView.outputText( 'Pressing your fingertips into her sides, you let out a moan into the open air, spurting cum deep and hard into her warm rectum', false );
				if( CoC.player.cockTotal() > 1 ) {
					MainView.outputText( ' while your other endowment', false );
					if( CoC.player.cockTotal() > 2 ) {
						MainView.outputText( 's soak', false );
					} else {
						MainView.outputText( ' soaks', false );
					}
					MainView.outputText( ' her back and rear with a few sticky streams', false );
				}
				MainView.outputText( '.  You pull out with an audible *schlick!*, and the gnoll slumps forward to the ground, panting a bit as she begins to shamelessly play with herself.\n\n', false );
			}
			//Med CumQ):;
			else if( CoC.player.cumQ() <= 500 ) {
				MainView.outputText( 'Gripping her sides tightly, you let out a bellowing moan that echos through the grasslands, ' + Descriptors.sMultiCockDesc() + ' swelling noticeably as you pump thick ribbons of spunk into the gnoll\'s innards', false );
				if( CoC.player.cockTotal() > 1 ) {
					MainView.outputText( ', unleashing a torrent of semen that mats down the short fur on her back and rear as', false );
				}
				MainView.outputText( '.  The muscular ring of her anus bears down on your ' + Descriptors.cockDescript( x ) + ' as you pull out, making an audible *schlick!*, and the gnoll drops to the ground in exhaustion, shamelessly playing with herself in an effort to get off.\n\n', false );
			}
			//(High CumQ):;
			else {
				MainView.outputText( 'You bear down on the limber hyena girl with all your force, groaning through clenched teeth as ' + Descriptors.sMultiCockDesc() + ' distends with the pressure of your virile load.  Her puckered anus stretches around the swell of your seed, and she lets out a shuddering, laughing moan as her belly begins to distend with your thick jism.  The involuntary spasms of her rectum milk your ' + Descriptors.cockDescript( x ) + ' for every drop, clenching your member tightly', false );
				if( CoC.player.cockTotal() > 1 ) {
					if( CoC.player.cockTotal() > 2 ) {
						MainView.outputText( ' while your remaining endowments soak her back thoroughly, covering her in a thick blanket of spunk', false );
					} else {
						MainView.outputText( ' while your remaining endowment soaks her back thoroughly, covering her in a thick blanket of spunk', false );
					}
				}
				MainView.outputText( '.  Her anus makes an audible *schlick!* as you pull back, and the gnoll rolls off your member, cradling her gravid, cum-filled belly as she begins to shamelessly finger herself.\n\n', false );
			}
	...
	GnollScene.prototype.suckGnollClit = function() {
		MainView.outputText( '', true );
		MainView.outputText( 'Since you first saw this strange anatomy, you knew it needed to be inside you.  You roughly roll her onto her back and pull her long clit up to a vertical position.  She moans softly, and the rough skin beneath your fingers pulses as her arousal brings it harder and taller.  The end bloats larger and fuller, until it might be too wide for your purposes.  Without losing any more time, you lean forward and stuff the clit\'s end into your mouth.\n\n', false );
		MainView.outputText( 'The gnoll yelps in surprise and then moans appreciatively as your mouth and tongue roam across her sensitive skin.  Her hips squirm against the hard-baked dirt, tentatively pushing her clit deeper into your mouth.\n\n', false );
		MainView.outputText( 'You\'re only too happy to oblige her, slowly and carefully swallowing the strange member down your throat.  Inch by inch you draw it deeper into yourself, eliciting increasing groans from the hyena girl.  Finally your nose buries itself in the rough, curly hair of her crotch, your chin rubbing up against the fake scrotum formed out of her distorted labia.\n\n', false );
		MainView.outputText( 'Her legs twitch around your ears as you continue to swallow, even though there is no more to go.  The muscles in your throat gently massage the length of the monstrous clitoris, which plumps even further as more blood rushes into it.  You can feel its tumescent bulk all down your throat and halfway into your chest.\n\n', false );
	...
	GnollScene.prototype.takeGnollClit = function() {
		MainView.outputText( '', true );
		MainView.outputText( 'You roughly roll her onto her back and pull her long clit up to a vertical position.  She moans softly, and the rough skin beneath your fingers pulses as her arousal brings it harder and taller.  The end bloats larger, fuller, until it might be too wide for your purposes.  You line up your ' + Descriptors.vaginaDescript( 0 ) + ' and lower yourself onto the long, narrow faux-cock.', false );
		CoC.player.cuntChange( 9, true, true, false );
		MainView.outputText( '\n\n', false );
		MainView.outputText( 'It slithers up your love canal with alacrity, and you quickly find yourself sitting on the gnoll\'s haunches.  You pump your hips up and down, relishing the feel of the thin spike slipping around within you.  This was not, however, what her anatomy was designed to do, and you can feel her monstrous clitoris flagging within you, collapsing down and denying you your well-deserved fucking.\n\n', false );
		MainView.outputText( '"<i>Hey!</i>" you shout, and reach forward to slap the gnoll across the face.  Abruptly the clit-dick hardens inside you, even as the girl\'s face registers little response.  With a smirk, you try it again.  A second hard slap brings the hyena\'s pseudocock ramrod straight up inside you, and you roll your hips atop her appreciatively.\n\n', false );
	...
	GnollScene.prototype.yoDawgHyenaBallz2 = function() {
		...
		MainView.outputText( 'Her bucking pauses for a split-second and you can feel her fuzzy balls throbbing against your ' + Descriptors.buttDescript() + ', the stimulation of her fucking filling them with fresh loads, eager to spill into your violated hole.  But, without a passage to let loose the torrent of her sperm with, the gnoll\'s balls merely swell with unspent passion, lewdly quivering against', false );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( ' your distended sac, ballooning with the fatigued flood of the prostate-milking the girl is giving you', false );
		} else {
			MainView.outputText( ' your sore loins, soaked with the slimy river of the girl\'s excitement', false );
		}
		MainView.outputText( '.  "<i>N-no! I can\'t... quite... arg!</i>" She trembles, unable to cum, before mindlessly starting anew, pistoning into your ' + Descriptors.assholeDescript() + ' with renewed vigor.  "<i>I just! Have to! Fucking! Cum!</i>" she grunts from between clenched teeth as she slams into you, the waterfall of lubrication from her pussy soaking your ' + CoC.player.skinFurScales() + ' as she wetly slaps her hips into yours in her increasingly desperate assault.\n\n', false );
		MainView.outputText( 'Again and again, she rushes toward a release, and every time, her balls slosh with overflowing lust, unable to climax yet urging her to greater depravity until even words are too difficult to manage.  Your ravaged body aches under the endless rape.  It\'s all you can do to work your hands under your hips to masturbate in the blissful pauses while the gnoll\'s muscles clench in near orgasm, before she slams you back to the dirt for another round.  You lose track of time as your world narrows to the endlessly thrusting shaft of the hyena-girl\'s massive clitoris and the merciless weight of her ball-slapping, spunk-swollen pouch. Every time you climax, it fuels her mad frenzy until you silently beg your body to shut out the overwhelming sensations, to no avail.  Day fades into night and night into day as the amazon fucks your spasming, shuddering pucker without thought, or pity, or release.\n\n', false );
		//[NEXT];
	...
	GnollScene.prototype.yoDawgHyenaBallz3 = function() {
		MainView.outputText( '', true );
		MainView.outputText( 'Finally, you feel the gnoll\'s clit slide out of your anus as the overly-endowed girl collapses at your side.  Broken utterly, she pants with depraved exhaustion, no strength left to continue pounding your body into the dirt.  Even now, she strokes her massive clit with one spotted paw as the other helplessly massages the basketball-sized testicles that audibly churn with gallons of the girl\'s jizz.  Finding a reserve of strength you didn\'t know you had, you rise tenderly and gather your belongings, creeping away before the hyena-girl can get her second wind.', false );
