...
angular.module( 'cocjs' ).run( function( SceneLib, MainView, CoC, kFLAGS, Utils, StatusAffects, PregnancyStore, EngineCore, WeaponLib ) {
	...
	Desert.prototype.mirageDesert = function() {
		...
		MainView.outputText( 'While exploring the desert, you see a shimmering tower in the distance.  As you rush towards it, it vanishes completely.  It was a mirage!   You sigh, depressed at wasting your time.', true );
		EngineCore.dynStats( 'lus', -15 );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	Desert.prototype.walkingDesertStatBoost = function() {
		MainView.clearOutput();
		MainView.outputText( 'You walk through the shifting sands for an hour, finding nothing.\n\n', true );
		//Chance of boost === 50%
		if( Utils.rand( 2 ) === 0 ) {
			//50/50 strength/toughness
			if( Utils.rand( 2 ) === 0 && CoC.player.str < 50 ) {
				MainView.outputText( 'The effort of struggling with the uncertain footing has made you stronger.', false );
				EngineCore.dynStats( 'str', 0.5 );
			}
			//Toughness
			else if( CoC.player.tou < 50 ) {
				MainView.outputText( 'The effort of struggling with the uncertain footing has made you tougher.', false );
