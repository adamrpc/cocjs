...
angular.module( 'cocjs' ).run( function( CoC, Utils, StatusAffects, EngineCore, AppearanceDefs, Descriptors, FetishZealot, Combat ) {
	...
	FetishZealotScene.prototype.zealotBoat = function() {
		if( CoC.getInstance().player.statusAffectv1( StatusAffects.FetishOn ) === 1 ) {
			this.zealotRepeat();
			return;
		}
		CoC.getInstance().player.changeStatusValue( StatusAffects.FetishOn, 1, 1 );
	...
	FetishZealotScene.prototype.zealotRepeat = function() {
		EngineCore.outputText( '', true );
		EngineCore.outputText( 'While exploring, you hear someone cry out behind you "<i>This is sacred land!  You WILL be punished for trespassing!</i>"  It seems you\'ve managed to stumble upon whatever land this zealot has been tasked to guard, and now you must fight him.', false );
		Combat.startCombat( new FetishZealot() );
		EngineCore.spriteSelect( 20 );
	};
	//Raping the player
	FetishZealotScene.prototype.zealotLossRape = function() {
		EngineCore.outputText( '', true );
		var broseph = CoC.getInstance().player.mf( 'dude', 'chick' );
		//Pre Rape Scene - lose by hp
		if( CoC.getInstance().player.HP < 1 ) {
			EngineCore.outputText( 'You collapse from the pain of the zealot\'s attacks.  You feel your head swimming from the aphrodisiac; it seems to be having a stronger and stronger effect on you.  Soon your head is swimming with images of fetish scenes of all kinds.  The zealot walks up to you and puts his hand on your forehead.  Suddenly, all the images cascade into one.\n\n', false );
		}//Pre Rape Scene – lose by lust
		else {
			EngineCore.outputText( 'The constant images of fetishes overwhelm you and you fall to your knees, unable to resist the temptation. The zealot walks up to you and puts his hand on your forehead.  Suddenly all the images cascade into one.\n\n', false );
		}
		//Student rape
		...
		EngineCore.outputText( 'You stand up facing the chalkboard in your classroom.  For some reason you think you should be bothered by this, but you\'re in your classroom just after class.  Nothing seems to be out of place or missing.  You turn back to the rest of the classroom and notice that everyone but one boy seems to have left already.  You recognize that he is the cute nervous boy with the rich parents.  You shudder at the thought of his parents; last week they got a teacher fired for yelling at him to speak up.  ', false );
		...
		if( broseph === 'dude' ) {
			...
			if( CoC.getInstance().player.armorValue <= 0 ) {
				CoC.getInstance().player.modArmorName = 'formal vest, tie, and crotchless pants';  //can you think of a better way of putting this?
			}
			EngineCore.outputText( 'You smooth down your detached pants, and look at your exposed dick for a few moments, wondering if there was anything you said that may have upset him.  ', false );
		}
		//no its a girl, or it looks like one
		else {
			//Set armour to female teacher outfit with no back side.
			if( CoC.getInstance().player.armorValue === 0 ) {
				CoC.getInstance().player.modArmorName = 'backless female teacher\'s clothes';  // again, change this if you've got a better name
			}
			EngineCore.outputText( 'You smooth out your half-skirt, trying to busy yourself as you try to remember if there was anything you said to upset him.  ', false );
		}
		EngineCore.outputText( '"<i>Professor?</i>"  You hear him call out to you.  You look up and see him standing in front of your desk looking at the ground nervously.  "<i>Yes, what is it?</i>" you reply.  ', false );
		//same as before
		if( broseph === 'dude' ) {
			EngineCore.outputText( 'He starts to look up, but then his gazes fixes directly on your ', false );
			if( CoC.getInstance().player.cockTotal() > 0 ) {
				EngineCore.outputText( Descriptors.multiCockDescriptLight() + '.  You feel yourself grow hard ', false );
				if( CoC.getInstance().player.hasVagina() ) {
					EngineCore.outputText( 'and wet ', false );
				}
				EngineCore.outputText( 'under his gaze, ', false );
			} else {
				EngineCore.outputText( 'bare crotch, ', false );
			}
			EngineCore.outputText( 'and you silently wish that the dress code allowed for a small flap for covering your ', false );
			if( CoC.getInstance().player.gender === 0 ) {
				EngineCore.outputText( 'lack of ', false );
			} else {
				EngineCore.outputText( 'genitals.  ', false );
			}
		} else {
			EngineCore.outputText( 'He starts to look up, but his gazes fixes on your skirt, just barely covering your ', false );
			if( CoC.getInstance().player.hasVagina() ) {
			...
			EngineCore.outputText( 'from this angle.  You feel hot under his gaze ', false );
			if( CoC.getInstance().player.cockTotal() > 0 ) {
				EngineCore.outputText( ', and your ' + Descriptors.multiCockDescriptLight() + ' start', false );
				if( CoC.getInstance().player.cockTotal() === 1 ) {
					EngineCore.outputText( 's', false );
				}
				EngineCore.outputText( ' to get hard, as your ', false );
			}
			EngineCore.outputText( 'cleft starts to moisten.  ', false );
		}
		EngineCore.outputText( 'You clear your throat nervously and he starts, finally raising his eyes to your face.\n\n', false );
		EngineCore.outputText( '"<i>Ah, professor,</i>" he says again, "<i>I just got a ' + CoC.getInstance().player.mf( 'boyfriend', 'girlfriend' ) + ' and I have no idea how to have sex.  Could you please give me a lesson?</i>"  You stare at him in disbelief.  This boy still hadn\'t had a ' + CoC.getInstance().player.mf( 'man', 'woman' ) + ' at this school?  It would almost be laughable, if the threat of his parents didn\'t loom in the back of your mind.\n\n', false );
		EngineCore.outputText( '"<i>Ok,</i>"  you say nervously, "<i>I can give you a lesson on pleasing ' + CoC.getInstance().player.mf( 'men', 'women' ) + '.</i>"  He jumps up excitedly and runs around to the other side of the desk saying "<i>Ok, what do I have to do?</i>"  Feeling a bit put on the spot, you stop for a moment to take in the situation.  ', false );
		//again, is that a guy?  Or just look like one?
		if( broseph === 'dude' ) {
			EngineCore.outputText( 'You\'re standing in front of your student behind your desk in your empty classroom.  You are in regular teacher\'s attire open vest and tie over your otherwise bare ' + CoC.getInstance().player.allBreastsDescript() + ', with formal dress pant legs attached to your vest with suspenders, plus your black dress shoes for dancing.  Your student is wearing the standard male student uniform, a collection of pieces of material held together with straps, that will all fall away easily when one gets pulled; but otherwise gives full coverage.\n\n', false );
			...
			if( CoC.getInstance().player.gender !== 0 ) {
				...
				EngineCore.outputText( 'You take a hold of his nicely sized balls in one hand and the base of his cock in the other.  You give the tip a few licks before running your tongue down its length, making sure to coat every bit of it in your saliva along the way.  His aroused groans let you know that you are giving a good demonstration.  You give his balls a soft rub as you stick the end inside your mouth and run your tongue around it; the taste of his pre is almost like candy to you.\n\n', false );
				//now the favor is returned
				EngineCore.outputText( 'You stand up and indicate to him that it is now his turn, and while he is disappointed that the blowjob ended before he came, he still eagerly kneels down in front of your exposed ' + Descriptors.multiCockDescriptLight() + ' and licks his lips eagerly.  He tries to imitate your performance on his manhood onto your ' + Descriptors.cockDescript( 0 ) + ', but he sometimes messes up the order, or does something too fast or slow.  Fortunately, he is a good student, and under your guidance he is soon sucking cock like a pro.  He tickles you with his tongue in just the right way, and gives just the right amount of attention to each part of your length.  ', false );
				if( CoC.getInstance().player.hasVagina() ) {
					EngineCore.outputText( 'You slip one of your fingers inside your ' + Descriptors.vaginaDescript( 0 ) + ', wishing that it could get some attention, but this lesson is about pleasing <i>men</i>.  ', false );
				}
				EngineCore.outputText( 'You feel yourself getting close to the edge and tell him to stop.  He looks up at you uncertain for a moment, and you tell him "<i>It\'s time for the main event.</i>"\n\n', false );
				//jerking off contest
				EngineCore.outputText( 'He eagerly stands up and wraps his hands around his fine cock, you quickly try to do the same with your ' + Descriptors.multiCockDescript() + '.  "<i>I won\'t lose to you so easily, professor!</i>" he declares to you, "<i>Don\'t act so confidently on your first time.</i>" you reply.  Then the two of you are madly jerking yourselves off in a mad effort to get off before the other, your respective cocks already moist from the blowjobs they just received.  ', false );
				//just one cock
				if( CoC.getInstance().player.totalCocks() === 1 ) {
					EngineCore.outputText( 'You piston your hands on your ' + Descriptors.cockDescript( 0 ) + ', ', false );
				}
				//nope two
				else if( CoC.getInstance().player.totalCocks() === 2 ) {
					EngineCore.outputText( 'With one hand on each of your ' + Descriptors.multiCockDescriptLight() + ', you piston them like mad, ', false );
				}
				//oh no, is more than that
				else {
					EngineCore.outputText( 'Rapidly moving your hands between each rod in your ' + Descriptors.multiCockDescriptLight() + ', you manage to piston each of them at an incredible rate, ', false );
				}
				EngineCore.outputText( 'pushing your pleasure to incredible levels at an impressive rate.  Though, to your amazement, you hear your student give a gasp of pleasure just before you yourself are pushed over the edge.  The student had surpassed the teacher.\n\n', false );
			}
		} else {
			EngineCore.outputText( 'You\'re standing in front of your student behind your desk in your empty classroom.  You are in your regular dress clothes; a formal backless blouse and short semi skirt that only blocks the front from view, plus your two inch tall high heels.  Your student is wearing the standard male student uniform, a collection of pieces of material held together with straps, that will all fall away easily when one gets pulled; but otherwise gives full coverage.\n\n', false );
			...
			if( CoC.getInstance().player.gender !== 0 ) {
				...
				EngineCore.outputText( 'Throughout the whole conversation, your student gets more and more excited before finally saying "<i>Can we do it now?!</i>" and pulling his waist strings, causing his crotch piece to fall open.  At the site of his impressive erection, you eagerly kneel down in front of him as he says "<i>I want you, I want you to suck my cock <b>so</b> much!</i>"  You waste no time in obliging him.\n\n', false );
				//blowjob
				EngineCore.outputText( 'You take a hold of his nicely sized balls in one hand and the base of his cock in the other.  You give the tip a few licks before running your tongue down its length,  making sure to coat every bit of it in your saliva along the way.  His aroused groans let you know that you are giving a good demonstration.  You give his balls a soft rub as you stick the end inside your mouth and run your tongue around it; the taste of his pre is almost like candy to you.\n\n', false );
				//cunalinguss
				EngineCore.outputText( 'You stand up and indicate that it is now his turn, and while he is disappointed that the blowjob ended before he came, he still eagerly kneels down in front of your ' + Descriptors.vaginaDescript( 0 ) + '.  You gingerly lift up your skirt ', false );
				if( CoC.getInstance().player.balls > 0 ) {
					EngineCore.outputText( 'and move your ' + Descriptors.ballsDescriptLight() + ' out of the way ', false );
				}
				EngineCore.outputText( 'so he has access to your ' + Descriptors.vaginaDescript( 0 ) + '.  He sets to work, and you give him careful instructions on where to lick, how hard, and such.  He easily catches on, and soon he is probing all the right places, and giving the right amount of attention to every part of you.  ', false );
				if( CoC.getInstance().player.cockTotal() > 0 ) {
					EngineCore.outputText( 'You give your ' + Descriptors.multiCockDescriptLight() + ' a few gentle strokes, wishing he would give that a little attention too, but this is a lesson on how to please a <i>woman</i>.  ', false );
				}
				EngineCore.outputText( 'You feel yourself getting close to the edge and tell him to stop.  He looks up at you uncertain for a moment, and you tell him "<i>It\'s time for the main event.</i>"\n\n', false );
				//prick in the twat
				EngineCore.outputText( 'He eagerly stands up and starts to push you back against your desk, his fine cock already inside your ' + Descriptors.vaginaDescript( 0 ) + '.  He pushes you onto your back and grabs ahold of your legs, starting to make strong thrusts deep inside you, his cock seeming to fit your ' + Descriptors.vaginaDescript( 0 ) + ' perfectly.  Any pretense of professionalism is lost in the moment, as all you can care about is the feeling of his exquisite cock perfectly filling you, pushing in and out.  All too soon, you hear him gasp, and the wonderful feeling of sweet release fills you.  ', false );
				if( CoC.getInstance().player.cockTotal() > 0 ) {
					EngineCore.outputText( 'Your ' + Descriptors.multiCockDescriptLight() + ' spasms and covers you with a liberal amount of your own fluids.  ', false );
				}
				EngineCore.outputText( 'After a moment, your student pulls out of you and helps you back up.\n\n', false );
			}
		}
		//Wrapping things up
		if( CoC.getInstance().player.gender !== 0 ) {
			EngineCore.outputText( 'The two of you stand looking at each other again for a moment.  Then you help your student get his clothes back together.  Once you have him fixed up, you notice that he is looking at the ground again.  "<i>Professor?  I have a confession to make.</i>" he says very quietly.  "<i>What is it?</i>" you ask him.  "<i>I lied to you about getting a ' + CoC.getInstance().player.mf( 'boyfriend', 'girlfriend' ) + ', I actually just wanted you.</i>"  Before you have a chance to respond, he quickly gives you a kiss on the mouth, before rushing out the door, blushing furiously.  You can\'t help but smile at how he is growing up.\n\n', false );
		}
		if( CoC.getInstance().player.gender === 0 ) {
			EngineCore.outputText( 'Throughout the whole conversation, your student gets more and more excited.  Half way though he starts masturbating, and by the end he is ready to go over the edge.  "<i>Thank you so much for the lesson professor, gaahh.</i>" he tells you as he begins to cum. The sight of him cumming in front of you is enough to drive you to your own orgasm.  Breathing a sigh of relief, you let your student out of the room and go back to cleaning up.\n\n', false );
		}
		...
		//continue on to Cultist bad end.  I'll expand these later when where doing the cathedral.
			...
			EngineCore.outputText( 'A few hours later your mind finally returns to reality.  You look around and realize that you are no longer in the same place you were before, and the zealot is nowhere to be seen.', false );
		}
		...
		if( CoC.getInstance().player.armorValue === 0 ) {
			EngineCore.outputText( '\n\nYou notice that you still have the ' + CoC.getInstance().player.armorName + ' from the fantasy that your mind was trapped in, with no sign of your original clothes.', false );
		}
		//If armour is not replaced
		else {
			EngineCore.outputText( '\n\nYou find that your ' + CoC.getInstance().player.armorName + ' are back to normal, and there is no sign of the strange clothes you were wearing before.', false );
		}
		...
		EngineCore.dynStats( 'lus', CoC.getInstance().player.cor / 20 + CoC.getInstance().player.lib / 10 );
		Combat.cleanupAfterCombat();
	};
	FetishZealotScene.prototype.zealotDefeated = function() {
		//Defeated by health
		if( CoC.getInstance().monster.HP < 1 ) {
			EngineCore.outputText( 'The zealot collapses from his wounds, too hurt to continue controlling his powers.', false );
		}//Defeated by lust
		else {
			EngineCore.outputText( 'The zealot quivers for a moment before collapsing, his desires becoming too great for even him to control.', false );
		}
	...
	FetishZealotScene.prototype.zealotWinRape = function() {
		...
		EngineCore.outputText( 'The zealot\'s attire seems to have settled on an outfit similar to those commonly worn by members of religious orders, though you aren\'t too surprised to see that it has a slit running down the front and back of the outfit that gives you full access to his sizable cock and asshole.\n\n', false );
		//If player has a dick, chose one of these at random if the player has both
		if( CoC.getInstance().player.cockTotal() > 0 && (CoC.getInstance().player.gender !== 3 || Utils.rand( 2 ) === 0) ) {
			EngineCore.outputText( 'As you move towards him, he drops onto all fours with his head down and ass in the air.  He seems to have started making a prayer me my lord, for I have failed to protect your holdings and will now accept your punishment by being violated by the one who defeated me.</i>" ', false );
			if( CoC.getInstance().player.cor < 50 ) {
				EngineCore.outputText( 'You stop and stare at him for a moment, in complete disbelief at this bizarre \'prayer\'.  You consider just leaving him alone, but sensing your hesitation, the zealot looks up to you with a horrified expression.  "<i>You must violate me!</i>"  He cries out to you, "<i>Please, let me finish my prayer, put your ' + Descriptors.cockDescript( 0 ) + ' in my ass.</i>"  Well, he did ask you...\n\n', false );
			} else {
				EngineCore.outputText( 'You chuckle at him, he <i>accepts</i> his punishment?  Oh, you are going to enjoy this so much.\n\n', false );
			}
			EngineCore.outputText( 'You remove your ' + CoC.getInstance().player.armorName + ' and stride up behind him, and grab his rear end to line up your ' + Descriptors.cockDescript( 0 ) + ' with his loose hole as he continues his prayer a man will violate my ass, like so many others have done in your holy worship.  Woe is me, to be treated like this by someone who doesn\'t follow us.</i>" You shake your head at this absurdity and plunge your ' + Descriptors.cockDescript( 0 ) + ' inside his waiting hole.  Amazingly, his hole somehow manages to fit you perfectly.  Since he seems to have no problem taking you, you waste no time in getting the anal rape on.\n\n', false );
			EngineCore.outputText( '"<i>My shame brings the one within me their pleasure; such a tragedy has befallen me.</i>" he continues.  "<i>Shut up and take it like a man!</i>" you tell him, and start fucking him more and more roughly.  You reach around him and grab his balls, and start to grip them painfully.    He isn\'t perturbed, and continues his prayers between his gasps, The horror, I\'m being tortur- ah, while being raped ungh, and I\'m loving every moment.  Oug!</i>"  Having had enough, you squeeze his sack hard, at the same time as you cum inside his ass.\n\n', false );
			EngineCore.outputText( 'Your lusts sated for now, you rise up off of him and put your ' + CoC.getInstance().player.armorName + ' back on.  You decide to leave him lying there, still doubled over in pain from the damage you did to his balls.\n\n', false );
		}
