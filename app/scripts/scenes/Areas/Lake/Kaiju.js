...
angular.module( 'cocjs' ).run( function( SceneLib, MainView, kFLAGS, ConsumableLib, CoC, AppearanceDefs, PerkLib, StatusAffects, EngineCore, Descriptors ) {
	function Kaiju() {
	}
	...
	Kaiju.prototype.kaijuCock = function() {
		return (CoC.flags[ kFLAGS.KAIJU_COCK ] === 1);
	};
	//[If no]
	...
	Kaiju.prototype.talkToKaiju = function() {
		...
		if( CoC.flags[ kFLAGS.KAIJU_TALK_CYCLE ] === 1 ) {
			//Dialog one:
		...
		//Dialog two:
		else if( CoC.flags[ kFLAGS.KAIJU_TALK_CYCLE ] === 2 ) {
		...
		} else if( CoC.flags[ kFLAGS.KAIJU_TALK_CYCLE ] === 3 ) {
			//Dialog three:
	...
	Kaiju.prototype.peekAtSomePhatAssKaijuButt = function() {
		...
		//[Lust is increased]
		EngineCore.dynStats( 'lus', 10 + CoC.player.lib / 3 );
		//[If Venus is corrupt/herm]
		if( this.kaijuCock() ) {
	...
	Kaiju.prototype.mockDatTurtleGirl = function() {
		...
		if( CoC.player.balls > 0 && CoC.player.ballSize >= 6 ) {
	...
	Kaiju.prototype.helpNewFutaKaijuGetOff = function() {
		...
		if( CoC.player.cor < 50 && CoC.player.lib < 75 ) {
	...
	Kaiju.prototype.yesTurnKaijuFuta = function() {
		...
		while( counter > 0 ) {
			...
			} else if( CoC.player.hasItem( ConsumableLib.INCUBID ) ) {
	...
	Kaiju.prototype.badEndPartTwo = function() {
		...
		else if( CoC.player.tongueType > AppearanceDefs.TONUGE_HUMAN ) {
		...
		if( this.kaijuCock() ) {
			...
			CoC.player.buttChange( 500, true, true, false );
