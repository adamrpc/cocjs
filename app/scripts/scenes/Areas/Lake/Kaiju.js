...
angular.module( 'cocjs' ).run( function( SceneLib, MainView, kFLAGS, ConsumableLib, CoC, AppearanceDefs, PerkLib, StatusAffects, EngineCore, Descriptors ) {
	...
	Kaiju.prototype.noMeetingKaijus = function() {
		MainView.clearOutput();
		MainView.outputText( 'You continue rowing on, away from the hilly island.' );
	...
	Kaiju.prototype.letKaijuHaveWayWithYou = function() {
		...
		if( CoC.player.hasCock() ) {
			MainView.outputText( ', [eachCock] stiffening in your [armor] from the stimulation as your whole body is used in what seems more and more like a massive boob job' );
		}
		MainView.outputText( '.' );
		if( CoC.player.hasVagina() ) {
			MainView.outputText( '  Your [vagina] moistens from your body\'s growing arousal.' );
		}
		...
		if( CoC.player.gender === 3 ) {
			...
			MainView.outputText( 'bare groin' );
		}
		MainView.outputText( '.  "<i>Let\'s have some fun!</i>"' );
		//[if player has cock]
		if( CoC.player.hasCock() ) {
			...
			//[if PC has balls]
			if( CoC.player.balls > 0 ) {
				MainView.outputText( '  Your [balls] begin to ache as you slowly empty your load onto green flesh.' );
			}
			CoC.player.orgasm();
		}
	...
	Kaiju.prototype.flirtWithKaiju = function() {
		...
		if( CoC.player.biggestTitSize() >= 1 ) {
			MainView.outputText( '  Your [fullChest] slide across the slick yet firm vaginal flesh of her surprisingly dexterous cunt, your [nipples] stiffening from the constant teasing massage.' );
		}
		//[If PC has cock]
		if( CoC.player.hasCock() ) {
			...
			MainView.outputText( ' throbbing from the sexual stimulants of her aphrodisiac moisture and the pressing walls of her cunt flesh, pre forming at your tip' );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( '.' );
		...
		if( CoC.player.sens < 50 ) {
			...
			//[If PC has cock]
			if( CoC.player.hasCock() ) {
				MainView.outputText( '  [EachCock] spasms as you release your load onto her pussy walls, making them just a bit more slick with your contribution.' );
			}
			if( CoC.player.hasVagina() ) {
				MainView.outputText( '  Your own pussy releases its own feminine juices, soaking your thighs even further in wetness.' );
			}
		...
		if( this.kaijuCock() ) {
			...
			if( CoC.player.gender > 0 ) {
				...
				if( CoC.player.biggestTitSize() >= 1 ) {
					MainView.outputText( '  You press your [chest] against her cock flesh as well, giving her penis as much a boob job as such a behemoth prick can be given.' );
				}
				if( CoC.player.lactationQ() >= 200 ) {
					MainView.outputText( '  Milk spurts and flows from your [nipples], making your breast slick and shiny and helping you slide more easily along the throbbing meat-pole.' );
				}
			}
			...
			if( CoC.player.gender > 0 ) {
				...
				MainView.outputText( '  Undeterred, you continue grinding upon the spurting pole until you reach your own smaller, far less impressive orgasm, releasing your own ' );
				if( CoC.player.hasVagina() ) {
					MainView.outputText( 'feminine juices' );
				}
				if( CoC.player.gender === 3 ) {
					MainView.outputText( ' and ' );
				}
				if( CoC.player.hasCock() ) {
					MainView.outputText( 'thick cum' );
				}
				MainView.outputText( '.  You look out upon the lake to see much of it has taken on a remarkably more milky white color.  Even for a giant that was a prodigious amount of cum she unleashed!' );
	...
	Kaiju.prototype.fuckThisGiantYouDumbCunt = function() {
		...
		if( CoC.player.sens >= 50 ) {
			...
			if( this.kaijuCock() ) {
				...
				//[If vagina and/or cock is present]
				if( CoC.player.gender > 0 ) {
					MainView.outputText( 'You grind your privates against her member, determined to get yourself off as well.  ' );
				}
				//[If PC has boobs]
				if( CoC.player.biggestTitSize() >= 1 ) {
					MainView.outputText( 'You press your [fullChest] against her cock flesh as well, giving her penis as much a boob job as such a behemoth prick can be given.' );
				}
				if( CoC.player.lactationQ() >= 200 ) {
					MainView.outputText( '  Milk spurts and flows from your [nipples], making your breast slick and shiny and helping you slide more easily along the throbbing meat-pole.' );
				}
	...
	Kaiju.prototype.urethraFuckDatGiantCock = function() {
		...
		if( CoC.player.hasCock() ) {
			MainView.outputText( ' and your ' + Descriptors.multiCockDescriptLight() );
		}
		MainView.outputText( '.  "<i>Wow, you look absolutely gorgeous in my cock!</i>" the giantess says with glee, perverse desire in her eyes as she shoves you in further up to your [chest], her trembling, squeezing cock seeming to give you an almost relaxing massage.  You can barely believe it, but this unnatural giantess has as much control over her urethral walls as the most whorish succubus has over her cunt!  Only your arms and head are free of her cock as her unusual member starts squeezing and rippling all along your body.  It seems her dick wants to give you a happy ending.' );
		...
		if( CoC.player.hasCock() ) {
			MainView.outputText( '  So much jizz fills your ass it can’t help but to press in on your prostate, making [eachCock] stiffen with a seriously growing urge.' );
		}
		if( CoC.player.hasVagina() ) {
			MainView.outputText( '  Your ass isn’t alone in the feeling of being flooded as your [vagina] and womb are packed with the hermaphrodite’s sperm.' );
		}
		MainView.outputText( '  You manage to reach your own orgasmic climax right before the pressure finally pops you out of her cock like a cork from a wine bottle.  You black out somewhere above the lake.' );
	...
	Kaiju.prototype.talkToKaiju = function() {
		...
		//Dialog two:
			...
			if( CoC.player.findStatusAffect( StatusAffects.DungeonShutDown ) >= 0 && CoC.player.findStatusAffect( StatusAffects.FactoryOverload ) < 0 ) {
				...
				MainView.outputText( '  But the pouring of liquids has been stopped for some reason, but not before there was one final, huge burst of tainted fluids. It was so strong and changed me even further, making me grow this,</i>" she says as she rises further out of the lake\'s waters, revealing a large green cock.' );
			}
	...
	Kaiju.prototype.peekAtSomePhatAssKaijuButt = function() {
		...
		MainView.outputText( '.  She pumps her clenched fist furiously as deep into herself as she can reach, going faster and faster as she quickly reaches orgasm, her pussy juices squirting out around her fist, soaking her forearm in the feminine fluid.  She slowly pulls her hand out with an audible \'Plop\' and brings both hands back to her breasts, massaging her giant sized fuck pillows until two thick steady streams of milk begin flowing out her nipples and down her breasts.' );
		//[Lust is increased]
	...
	Kaiju.prototype.mockDatTurtleGirl = function() {
		...
		if( CoC.player.balls > 0 && CoC.player.ballSize >= 6 ) {
			MainView.outputText( 'Your [balls] rest on the hermaphrodite\'s hand, bouncing loudly with each thunderous stroke from the speed and force she is employing to get herself off.  ' );
		}
		//[end balls] [If player has cock]
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'You can feel your [armor] getting tighter as [eachCock] begins to stiffen under your clothing from all the constant pressure of the cock squeezing down on your member.  ' );
		}
		//[If player has vagina]
		if( CoC.player.hasVagina() ) {
			MainView.outputText( 'You begin to feel a wetness between your legs as your cunt begins leaking as your arousal spikes from the intense rubbing the herm\'s penis is giving your whole body.  ' );
		}
		//[If genderless]
		if( CoC.player.gender === 0 ) {
			MainView.outputText( 'Your cheeks blush in arousal from all this heavy rubbing.  ' );
		}
		if( CoC.player.cor < 33 ) {
			MainView.outputText( 'You can\'t believe that you\'re being aroused by this.  Aren\'t you suppose to be a champion?' );
		} else if( CoC.player.cor < 66 ) {
			MainView.outputText( 'Wow, this is arousing you.  You wonder if you really are a champion.' );
		} else {
			MainView.outputText( 'You just want to get off now.' );
		}
	...
	Kaiju.prototype.helpNewFutaKaijuGetOff = function() {
		...
		//[If PC has cock]
		if( CoC.player.hasCock() ) {
			MainView.outputText( '  [EachCock] quickly hardens now as it is rubbed up against the giant\'s massive shaft.' );
		}
		if( CoC.player.hasVagina() ) {
			MainView.outputText( '  The warm heat that radiates off of the titanic member seems to seep into your nether region, making your [vagina] leak out moisture onto the massive prick.' );
		}
		MainView.outputText( '  She squirms under your touch, falling on her back and wriggling almost uncontrollably.  You\'re quite surprised at the power you hold over such a behemoth.' );
		...
		if( CoC.player.cor < 50 && CoC.player.lib < 75 ) {
			MainView.outputText( '  You knead the cock head with your hands, massaging it quickly and roughly as you grind up against it, rock back and forth on the giant member.  Some of the pre dribbles down and you begin to use it as sticky massage oil, working it into the cock flesh.  The cooing and lustful moaning coming from the giantess increases in intensity with your every movement.' );
		} else {
	...
	Kaiju.prototype.yesTurnKaijuFuta = function() {
		...
		MainView.outputText( 'You wouldn’t have offered if you didn’t want to see a giant green futa-cock dangling between her legs.  Your declaration seems to have deepened her blush to almost beet red, but the lustful look in her eyes betrays her desire.' );
	...
	Kaiju.prototype.badEndPartTwo = function() {
		...
		if( CoC.player.tongueType === AppearanceDefs.TONUGE_SNAKE ) {
			...
			MainView.outputText( '  The long tongue in your mouth slowly crawls around the nipple tip clenched between your lips, wrapping around the milky nip and squeezing to urge a dollop of milk out.' );
		...
		if( this.kaijuCock() ) {
			...
			if( CoC.player.hasCock() ) {
				MainView.outputText( '  You prostate is pressed down like a button, and there’s just no letting up!' );
			}
			if( CoC.player.hasVagina() ) {
				MainView.outputText( '  Even if your cunt is empty, there is no free space left there, your cunt walls forced together like glue.' );
			}
		...
		MainView.outputText( '  Inches slide in at a time as it spasms inside of you, attempting to hit every sensitive spot your ass may have as it slowly tries to stretch you out.\n\n' );
		//[If PC has a cock]
		if( CoC.player.hasCock() ) {
			MainView.outputText( '[EachCock] stiffens as the tree tendril pushes against your prostate, forcing a thick glob of pre up your shaft.  ' );
		}
		if( CoC.player.hasVagina() ) {
		...
		MainView.outputText( '  The next sinuous cock threads itself between your legs, up your belly, past your chest to lightly poke at your lips.  You open your mouth to shout when the cock in your [asshole] hits a sensitive spot and the cock at your mouth slides right in, dripping seed into your mouth, your tongue suddenly awash in a salty, yet pleasantly sweet flavor.' );
		if( CoC.player.biggestTitSize() >= 1 ) {
			MainView.outputText( '  Another tendril snakes its way up your abdomen to nestle between your [chest], slicking your mounds with pre as it slides between your tits, giving itself a titty-fuck with your titties!' );
		}
