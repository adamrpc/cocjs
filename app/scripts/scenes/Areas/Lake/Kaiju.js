...
angular.module( 'cocjs' ).run( function( kFLAGS, ConsumableLib, CoC, AppearanceDefs, PerkLib, StatusAffects, EngineCore, Descriptors, EventParser ) {
	...
	//First encounter
	//Boat
	Kaiju.prototype.kaijuMeeting = function() {
		EngineCore.clearOutput();
		if( CoC.getInstance().flags[ kFLAGS.KAIJU_MEETINGS ] === 0 ) {
			...
			if( CoC.getInstance().player.findStatusAffect( StatusAffects.FactoryOverload ) >= 0 && !this.kaijuCock() ) {
				this.kaijuGrowsWangus();
			} else if( CoC.getInstance().flags[ kFLAGS.KAIJU_BAD_END_COUNTER ] >= 5 ) {
	...
	Kaiju.prototype.kaijuCock = function() {
		return (CoC.getInstance().flags[ kFLAGS.KAIJU_COCK ] === 1);
	};
	//[If no]
	Kaiju.prototype.noMeetingKaijus = function() {
		EngineCore.clearOutput();
	...
	Kaiju.prototype.insultTheKaijuFirstMeeting = function() {
		...
		if( this.kaijuCock() ) {
			...
			EngineCore.outputText( 'In perhaps not your brightest idea, you decide to make fun of the giant green girl in front of you, mocking her obvious insecurity over her figure.  The word \'fat\' barely exits your mouth before her face goes red in anger and you realize your mistake.' );
			...
			if( CoC.getInstance().player.findStatusAffect( StatusAffects.KnowsWhitefire ) >= 0 && (CoC.getInstance().player.findPerk( PerkLib.BloodMage ) < 0 && CoC.getInstance().player.fatigue + EngineCore.spellCost( 30 ) > 100) ) {
	...
	Kaiju.prototype.corruptKaijuInsertion = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 103 );
		EngineCore.outputText( 'She practically shoves your face into her dark green cock-head, your nose snug against the long slit.  It smells of the sexual corruption of the lake, of demonic jizz and tainted aphrodisiacs.  "<i>Well go on,</i>" she says, "<i>lick it!</i>"' );
	...
	Kaiju.prototype.complimentKaiju = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 103 );
		EngineCore.outputText( 'You decide to compliment her figure, stating it is proportional and very pleasing.' );
	...
	Kaiju.prototype.letKaijuHaveWayWithYou = function() {
		...
		if( CoC.getInstance().player.gender === 3 ) {
			...
			EngineCore.outputText( 'bare groin' );
		}
		EngineCore.outputText( '.  "<i>Let\'s have some fun!</i>"' );
		//[if player has cock]
		if( CoC.getInstance().player.hasCock() ) {
	...
	Kaiju.prototype.flirtWithKaiju = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 103 );
		EngineCore.outputText( 'You decide to try and flirt up the giantess.  You tell her that her figure is perfect and statuesque, and that her curves are quite magnificent to behold.  It seems to work if her spreading look of pleasure is any indication.' );
		...
		if( CoC.getInstance().player.biggestTitSize() >= 1 ) {
			EngineCore.outputText( '  Your [fullChest] slide across the slick yet firm vaginal flesh of her surprisingly dexterous cunt, your [nipples] stiffening from the constant teasing massage.' );
		}
		//[If PC has cock]
		if( CoC.getInstance().player.hasCock() ) {
			...
			EngineCore.outputText( ' throbbing from the sexual stimulants of her aphrodisiac moisture and the pressing walls of her cunt flesh, pre forming at your tip' );
			if( CoC.getInstance().player.cockTotal() > 1 ) {
				EngineCore.outputText( 's' );
			}
			EngineCore.outputText( '.' );
		...
		if( CoC.getInstance().player.sens < 50 ) {
			...
			//[If PC has cock]
			if( CoC.getInstance().player.hasCock() ) {
				EngineCore.outputText( '  [EachCock] spasms as you release your load onto her pussy walls, making them just a bit more slick with your contribution.' );
			}
			if( CoC.getInstance().player.hasVagina() ) {
				EngineCore.outputText( '  Your own pussy releases its own feminine juices, soaking your thighs even further in wetness.' );
			}
		...
		if( this.kaijuCock() ) {
			...
			if( CoC.getInstance().player.gender > 0 ) {
				...
				if( CoC.getInstance().player.biggestTitSize() >= 1 ) {
					EngineCore.outputText( '  You press your [chest] against her cock flesh as well, giving her penis as much a boob job as such a behemoth prick can be given.' );
				}
				if( CoC.getInstance().player.lactationQ() >= 200 ) {
					EngineCore.outputText( '  Milk spurts and flows from your [nipples], making your breast slick and shiny and helping you slide more easily along the throbbing meat-pole.' );
				}
			}
			...
			if( CoC.getInstance().player.gender > 0 ) {
				...
				EngineCore.outputText( '  Undeterred, you continue grinding upon the spurting pole until you reach your own smaller, far less impressive orgasm, releasing your own ' );
				if( CoC.getInstance().player.hasVagina() ) {
				...
				if( CoC.getInstance().player.hasCock() ) {
					EngineCore.outputText( 'thick cum' );
				}
				EngineCore.outputText( '.  You look out upon the lake to see much of it has taken on a remarkably more milky white color.  Even for a giant that was a prodigious amount of cum she unleashed!' );
	...
	Kaiju.prototype.repeatKaijuEncounter = function() {
		...
		if( CoC.getInstance().player.itemCount( ConsumableLib.INCUBID ) + CoC.getInstance().player.itemCount( ConsumableLib.P_DRAFT ) >= 10 && !this.kaijuCock() ) {
	...
	Kaiju.prototype.kaijuRepeatBoobHug = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 103 );
		EngineCore.outputText( 'You decide it would be fun to play with her titanic titties for a while.' );
		...
		if( CoC.getInstance().player.hasCock() ) {
			...
			EngineCore.outputText( ' rest on her perky pink naughty bit.  She coos in delight as her hand slowly rubs your private parts along her firm nipple.  She begins to bounce ever so slightly, which at her size means a verifiable mountain of boob begins smacking heavily against your lower body.  "<i>Oh yes,</i>" she says, moaning in ecstasy, "<i>fuck my breast!</i>"  You don\'t really seem to have a choice in the matter as she picks up the pace, almost violently slamming you against her smooth green skin and stiff pink nipple.  Despite the relatively soft flesh of her mammary, it is still hundreds of pounds of tit flesh.  As if unsatisfied with just smothering you in boob she takes one green pinkie finger and begins to spank you in a manner she probably believes to be quite soft but lands quite hard on your [butt].  A small number of spankings begin to leave your ass red and tingling, each strike pushing your ' + Descriptors.multiCockDescriptLight() + ' further into her soft breast and firm nipple.  "<i>Please, cum on me!</i>" she commands, and almost as if on cue your body obeys, jizz flying and coating her breast with as much sperm as your body can produce.' );
			if( CoC.getInstance().player.balls > 0 ) {
				EngineCore.outputText( '  Your [balls] begin to ache as you slowly empty your load onto green flesh.' );
			}
			CoC.getInstance().player.orgasm();
	...
	Kaiju.prototype.fuckThisGiantYouDumbCunt = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 103 );
		EngineCore.outputText( 'You decide it’d be fun to fuck the giantess, even if at her size you’re really more of a living dildo than a rutting stud.  She seems pretty happy with the thought if her spreading look of pleasure is any indication.' );
		...
		if( CoC.getInstance().player.sens >= 50 ) {
			...
			if( this.kaijuCock() ) {
				...
				//[If vagina and/or cock is present]
				if( CoC.getInstance().player.gender > 0 ) {
					EngineCore.outputText( 'You grind your privates against her member, determined to get yourself off as well.  ' );
				}
				//[If PC has boobs]
				if( CoC.getInstance().player.biggestTitSize() >= 1 ) {
					EngineCore.outputText( 'You press your [fullChest] against her cock flesh as well, giving her penis as much a boob job as such a behemoth prick can be given.' );
				}
				if( CoC.getInstance().player.lactationQ() >= 200 ) {
					EngineCore.outputText( '  Milk spurts and flows from your [nipples], making your breast slick and shiny and helping you slide more easily along the throbbing meat-pole.' );
				}
	...
	Kaiju.prototype.urethraFuckDatGiantCock = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 103 );
		EngineCore.outputText( 'You look at her with an inquisitive stare.  You’re interested in that big green cock of hers, but how can you use such a giant tool?' );
		...
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( ' and your ' + Descriptors.multiCockDescriptLight() );
		}
		EngineCore.outputText( '.  "<i>Wow, you look absolutely gorgeous in my cock!</i>" the giantess says with glee, perverse desire in her eyes as she shoves you in further up to your [chest], her trembling, squeezing cock seeming to give you an almost relaxing massage.  You can barely believe it, but this unnatural giantess has as much control over her urethral walls as the most whorish succubus has over her cunt!  Only your arms and head are free of her cock as her unusual member starts squeezing and rippling all along your body.  It seems her dick wants to give you a happy ending.' );
		...
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( '  So much jizz fills your ass it can’t help but to press in on your prostate, making [eachCock] stiffen with a seriously growing urge.' );
		}
		if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( '  Your ass isn’t alone in the feeling of being flooded as your [vagina] and womb are packed with the hermaphrodite’s sperm.' );
		}
		EngineCore.outputText( '  You manage to reach your own orgasmic climax right before the pressure finally pops you out of her cock like a cork from a wine bottle.  You black out somewhere above the lake.' );
	...
	Kaiju.prototype.talkToKaiju = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 103 );
		CoC.getInstance().flags[ kFLAGS.KAIJU_TALK_CYCLE ]++;
		CoC.getInstance().flags[ kFLAGS.KAIJU_BAD_END_COUNTER ]--;
		EngineCore.outputText( '"<i>Oh?  You want to talk?</i>" she says, a bit taken by surprise, "<i>What about?</i>"' );
		//(One of the available lines of dialog plays.)
		if( CoC.getInstance().flags[ kFLAGS.KAIJU_TALK_CYCLE ] === 1 ) {
			//Dialog one:
		...
		//Dialog two:
		else if( CoC.getInstance().flags[ kFLAGS.KAIJU_TALK_CYCLE ] === 2 ) {
			...
			if( CoC.getInstance().player.findStatusAffect( StatusAffects.DungeonShutDown ) >= 0 && CoC.getInstance().player.findStatusAffect( StatusAffects.FactoryOverload ) < 0 ) {
				EngineCore.outputText( '  But the flow of yummy fluids seems to have been stopped.' );
			}//[If the Factory was shut down and Marae corrupted]
			else if( this.kaijuCock() ) {
				EngineCore.outputText( '  But the pouring of liquids has been stopped for some reason, but not before there was one final, huge burst of tainted fluids. It was so strong and changed me even further, making me grow this,</i>" she says as she rises further out of the lake\'s waters, revealing a large green cock.' );
			}
			EngineCore.outputText( '\n\nYou thank the giantess for her explanation as you row away.' );
		} else if( CoC.getInstance().flags[ kFLAGS.KAIJU_TALK_CYCLE ] === 3 ) {
			//Dialog three:
	...
	Kaiju.prototype.peekAtSomePhatAssKaijuButt = function() {
		...
		EngineCore.outputText( '.  She pumps her clenched fist furiously as deep into herself as she can reach, going faster and faster as she quickly reaches orgasm, her pussy juices squirting out around her fist, soaking her forearm in the feminine fluid.  She slowly pulls her hand out with an audible \'Plop\' and brings both hands back to her breasts, massaging her giant sized fuck pillows until two thick steady streams of milk begin flowing out her nipples and down her breasts.' );
		//[Lust is increased]
		EngineCore.dynStats( 'lus', 10 + CoC.getInstance().player.lib / 3 );
		//[If Venus is corrupt/herm]
		if( this.kaijuCock() ) {
	...
	Kaiju.prototype.mockDatTurtleGirl = function() {
		...
		if( CoC.getInstance().player.balls > 0 && CoC.getInstance().player.ballSize >= 6 ) {
			EngineCore.outputText( 'Your [balls] rest on the hermaphrodite\'s hand, bouncing loudly with each thunderous stroke from the speed and force she is employing to get herself off.  ' );
		}
		//[end balls] [If player has cock]
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( 'You can feel your [armor] getting tighter as [eachCock] begins to stiffen under your clothing from all the constant pressure of the cock squeezing down on your member.  ' );
		}
		//[If player has vagina]
		if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( 'You begin to feel a wetness between your legs as your cunt begins leaking as your arousal spikes from the intense rubbing the herm\'s penis is giving your whole body.  ' );
		}
		//[If genderless]
		if( CoC.getInstance().player.gender === 0 ) {
			EngineCore.outputText( 'Your cheeks blush in arousal from all this heavy rubbing.  ' );
		}
		if( CoC.getInstance().player.cor < 33 ) {
			EngineCore.outputText( 'You can\'t believe that you\'re being aroused by this.  Aren\'t you suppose to be a champion?' );
		} else if( CoC.getInstance().player.cor < 66 ) {
			EngineCore.outputText( 'Wow, this is arousing you.  You wonder if you really are a champion.' );
		} else {
			EngineCore.outputText( 'You just want to get off now.' );
		}
	...
	Kaiju.prototype.helpNewFutaKaijuGetOff = function() {
		...
		//[If PC has cock]
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( '  [EachCock] quickly hardens now as it is rubbed up against the giant\'s massive shaft.' );
		}
		if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( '  The warm heat that radiates off of the titanic member seems to seep into your nether region, making your [vagina] leak out moisture onto the massive prick.' );
		}
		EngineCore.outputText( '  She squirms under your touch, falling on her back and wriggling almost uncontrollably.  You\'re quite surprised at the power you hold over such a behemoth.' );
		...
		if( CoC.getInstance().player.cor < 50 && CoC.getInstance().player.lib < 75 ) {
			EngineCore.outputText( '  You knead the cock head with your hands, massaging it quickly and roughly as you grind up against it, rock back and forth on the giant member.  Some of the pre dribbles down and you begin to use it as sticky massage oil, working it into the cock flesh.  The cooing and lustful moaning coming from the giantess increases in intensity with your every movement.' );
		} else {
	...
	Kaiju.prototype.yesTurnKaijuFuta = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 103 );
		CoC.getInstance().flags[ kFLAGS.KAIJU_BAD_END_COUNTER ] += 2;
		var counter = 10;
		while( counter > 0 ) {
			counter--;
			if( CoC.getInstance().player.hasItem( ConsumableLib.P_DRAFT ) ) {
				CoC.getInstance().player.consumeItem( ConsumableLib.P_DRAFT );
			} else if( CoC.getInstance().player.hasItem( ConsumableLib.INCUBID ) ) {
				CoC.getInstance().player.consumeItem( ConsumableLib.INCUBID );
			}
		}
		EngineCore.outputText( 'You wouldn’t have offered if you didn’t want to see a giant green futa-cock dangling between her legs.  Your declaration seems to have deepened her blush to almost beet red, but the lustful look in her eyes betrays her desire.' );
	...
	Kaiju.prototype.kaijuBadEndToyBOOSH = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 103 );
		EngineCore.outputText( 'As you are rowing through the lake you once more stumble upon the jolly green giantess Venus, who seems to be in the throes of pleasure as she masturbates furiously in some shallow waters, rubbing her large ' );
		//[if Venus is female]
		if( !this.kaijuCock() ) {
	...
	Kaiju.prototype.badEndPartTwo = function() {
		...
		if( this.kaijuCock() ) {
			...
			CoC.getInstance().player.buttChange( 500, true, true, false );
			...
			if( CoC.getInstance().player.hasCock() ) {
				EngineCore.outputText( '  You prostate is pressed down like a button, and there’s just no letting up!' );
			}
			if( CoC.getInstance().player.hasVagina() ) {
				EngineCore.outputText( '  Even if your cunt is empty, there is no free space left there, your cunt walls forced together like glue.' );
			}
		...
		EngineCore.outputText( '  Inches slide in at a time as it spasms inside of you, attempting to hit every sensitive spot your ass may have as it slowly tries to stretch you out.\n\n' );
		//[If PC has a cock]
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( '[EachCock] stiffens as the tree tendril pushes against your prostate, forcing a thick glob of pre up your shaft.  ' );
		}
		if( CoC.getInstance().player.hasVagina() ) {
		...
		EngineCore.outputText( '  The next sinuous cock threads itself between your legs, up your belly, past your chest to lightly poke at your lips.  You open your mouth to shout when the cock in your [asshole] hits a sensitive spot and the cock at your mouth slides right in, dripping seed into your mouth, your tongue suddenly awash in a salty, yet pleasantly sweet flavor.' );
		if( CoC.getInstance().player.biggestTitSize() >= 1 ) {
			EngineCore.outputText( '  Another tendril snakes its way up your abdomen to nestle between your [chest], slicking your mounds with pre as it slides between your tits, giving itself a titty-fuck with your titties!' );
		}
