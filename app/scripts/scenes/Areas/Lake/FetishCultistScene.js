...
angular.module( 'cocjs' ).run( function( MainView, SceneLib, CoC, Utils, StatusAffects, EngineCore, AppearanceDefs, FetishCultist, Descriptors, kFLAGS, Combat, CockTypesEnum ) {
			...
			if( CoC.player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_CENTAUR && CoC.player.balls > 0 && CoC.player.totalCocks() > 0 && Utils.rand( 4 ) < 3 ) {
				if( CoC.player.cocks[ 0 ].cockType === CockTypesEnum.HORSE ) {
					...
					if( CoC.player.inte <= 10 ) {
						...
						if( CoC.player.armorValue > 0 ) {
							EngineCore.outputText( 'You find your ' + CoC.player.armorName + ' back on your body with no sign of the strange clothes you were wearing before.  ', false );
						} else {
							EngineCore.outputText( 'You are still wearing the ' + CoC.player.armorName + ' that she gave you, and there is no sign of your old clothes.  ', false );
						}
		...
		if( Utils.rand( 2 ) === 0 && !sexed ) {
			...
			if( CoC.player.biggestTitSize() > 1 && !changed ) {
				...
				if( CoC.player.breastRows.length === 2 ) {
					EngineCore.outputText( 'Once about a quarter of the fluid has been injected into the first breast, she moves on to the second and you feel a similar sharp pain followed by the glowing pleasure.  She continues this until all of your ' + CoC.player.allBreastsDescript() + ' have been injected.  ', false );
				}
				//[if the player has three rows of breasts]
				if( CoC.player.breastRows.length > 2 ) {
					EngineCore.outputText( 'Once about a sixth of the fluid has been injected into the first breast, she moves on to the second and you feel a similar sharp pain followed by the glowing pleasure.  She continues this until all of your ' + CoC.player.allBreastsDescript() + ' have been injected.  ', false );
				}
				EngineCore.outputText( 'Soon the glowing pleasure fills your breasts completely and you feel an orgasm shoot through your body as your ' + CoC.player.allBreastsDescript(), false );
				//[if breasts are already lactating]
				if( CoC.player.biggestLactation() >= 1 ) {
			...
			if( CoC.player.vaginas.length > 0 && CoC.player.cocks.length > 0 ) {
				EngineCore.outputText( 'You see your ' + CoC.player.armorName + ' becoming a nice, comfortable outfit of a white shirt and black pants that make you look well-suited to work in a manor... if they didn\'t have an exposed crotch and butt that puts your ' + Descriptors.multiCockDescriptLight() + ', your ' + Descriptors.vaginaDescript( 0 ) + ', and your ' + Descriptors.buttDescript() + ' on full display.  Any undergarments you had are now gone.\n\n', false );
				if( CoC.player.armorValue === 0 ) {
				...
				if( CoC.player.cocks.length > 0 && (CoC.player.vaginas.length === 0 || Utils.rand( 2 ) === 0) ) {
					EngineCore.outputText( '  You see that your ' + CoC.player.armorName + ' have become a nice looking, if simple, frilled white shirt and bow tie along with black pants and matching shoes.  They make you look like you would be at home in a manor attending the needs of a noble, if it were not for the flaps on the front and back of your pants that give easy access to your ' + Descriptors.multiCockDescriptLight(), false );
				...
				//(variant 2: maid, requires vagina)
					...
					EngineCore.outputText( 'You see that your ' + CoC.player.armorName + ' have become an apron over a plain dress with high stockings and simple black shoes.  They make you look like you would be at home keeping a manor clean and tidy, if it were not for the rather short skirt and lack of undergarments that give easy access to your ' + Descriptors.vaginaDescript( 0 ), false );
			...
			if( CoC.player.gender === 0 ) {
				EngineCore.outputText( 'You see that your ' + CoC.player.armorName + ' have become a nice and comfortable outfit of a shirt white and black pants, that make you look well suited to work in a manor.  However, you notice that there is a flap on your pants that gives easy access to your ' + Descriptors.buttDescript() + '.\n\n', false );
				if( CoC.player.armorValue === 0 ) {
	...
	FetishCultistScene.prototype.playerRapesCultist = function() {
		...
		if( CoC.monster.armorName === 'pervy nun\'s clothing' ) {
			...
			//(if player has a vagina)
				...
				EngineCore.outputText( 'A few minutes later, you close your ' + CoC.player.armorName + ' back up, and look back at the cultist.  She is lying on the ground in her farm hand outfit with a look of utter bliss on her face.  Satisfied, you continue on your way.', false );
				CoC.player.orgasm();
			}
			if( CoC.player.gender === 0 ) {
	...
	FetishCultistScene.prototype.fetishCultistHasAMilkFetish = function() {
		EngineCore.outputText( '', true );
		EngineCore.outputText( 'You remove your ' + CoC.player.armorName + ', standing naked in front of the beaten fetish-girl with your lactating ' + Descriptors.breastDescript( 0 ) + ' dripping milk. You pick the cultist up and sit down on the ground with her resting on your lap, her soft warm ass up against your thighs. You hold the nipple up to her mouth; she suckles on it without the slightest sign of rebellion.\n\n', false );
		...
		EngineCore.outputText( '"<i>Thank you ' + CoC.player.mf( 'Master', 'Mistress' ) + ' for letting me have some of your special milk after doing all my chores.</i>"\n\n', false );
		EngineCore.outputText( 'What is she talking about? ' + CoC.player.mf( 'Master', 'Mistress' ) + '? Chores? What the hell is going on? Your mind races with confusion as you try to figure out what\'s happening. The scene around you then changes back to the lake. The cultist, who looks like she normally would, sees your confusion.\n\n', false );
		...
		EngineCore.outputText( 'Soon her thong becomes wet with arousal and her mouth comes off your ' + Descriptors.nippleDescript( 0 ) + ' to ask, "<i>Please ' + CoC.player.mf( 'Master', 'Mistress' ) + ', will you fix the aching in my loins?</i>"\n\n', false );
