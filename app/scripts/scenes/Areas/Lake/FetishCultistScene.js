...
angular.module( 'cocjs' ).run( function( SceneLib, CoC, Utils, StatusAffects, EngineCore, AppearanceDefs, FetishCultist, Descriptors, EventParser, kFLAGS, Combat, CockTypesEnum ) {
	...
	FetishCultistScene.prototype.cultistRapesYou = function() {
		CoC.getInstance().monster.lust = 1;
		CoC.getInstance().monster.HP = 10;
		var changed = false;
		var changedCock = false;
		var changedBoobs = false;
		var sexed = false;
		EngineCore.outputText( 'The thoughts of fetishes have become so strong that they override everything else.  You see the cultist moving towards you, but you can\'t possibly fight any more.  She reaches out and touches you, and you fall to the ground as the multitudes of thoughts cascade into one.\n\n', false );
		...
		if( CoC.getInstance().player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_CENTAUR || CoC.getInstance().player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_NAGA ) {
			...
			if( CoC.getInstance().player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_CENTAUR && CoC.getInstance().player.balls > 0 && CoC.getInstance().player.totalCocks() > 0 && Utils.rand( 4 ) < 3 ) {
				if( CoC.getInstance().player.cocks[ 0 ].cockType === CockTypesEnum.HORSE ) {
					...
					if( CoC.getInstance().player.inte <= 10 ) {
						...
						EngineCore.outputText( 'A few hours later your mind finally returns to reality.  You look around, but can see no sign of the cultist that you saw earlier.  ', false );
						if( CoC.getInstance().player.armorValue > 0 ) {
							EngineCore.outputText( 'You find your ' + CoC.getInstance().player.armorName + ' back on your body with no sign of the strange clothes you were wearing before.  ', false );
						} else {
							EngineCore.outputText( 'You are still wearing the ' + CoC.getInstance().player.armorName + ' that she gave you, and there is no sign of your old clothes.  ', false );
						}
						EngineCore.outputText( 'The ordeal has also left you with a slightly dulled mind, and some of the desire you felt still lingers.', false );
			...
			EngineCore.outputText( 'You look up and find that you\'re behind a set of bars.  You panic for a moment before hearing someone whistling and turn towards the source of the sound, a feeling of dread falling over you as you recognize the tune.  The zoo keeper turns the corner; she is a busty girl wearing a tiny brown tube top with a name tag that says "Featsy\'s Slutty Beast Stables - Stable-Mistress".  She has a similarly colored brown cap, and a belt around her otherwise bare waist and long legs.  She is spinning a keyring around her finger while whistling her tune.  She stops at your cell and turns to look at you with a smile on her face, catching the keys in her hand.\n\n', false );
			//Centaur
			...
			//Naga
				...
				if( CoC.getInstance().player.armorValue === 0 ) {
					CoC.getInstance().player.modArmorName = 'headdress, necklaces, and many body-chains';
				}
				//Check for bad-end start!
				if( CoC.getInstance().player.inte <= 10 ) {
					this.cultistBadEnd();
		...
		if( Utils.rand( 2 ) === 0 && !sexed ) {
			...
			if( CoC.getInstance().player.cocks.length > 0 && !changed ) {
				EngineCore.outputText( 'The smile disappears from her lips and she says, "<i>Now if I understand it correctly, you are beset by random panic attacks due to unnaturally low levels of cum production.  To address this issue, I will be giving you a special injection at the site.</i>"' );
			...
			if( CoC.getInstance().player.biggestTitSize() > 1 && !changed ) {
				EngineCore.outputText( 'The smile disappears from her lips and she says, \'Now if I understand it correctly, you are beset by random panic attacks due to unnaturally low levels of lactation.  To address this issue, I will be giving you a special injection at the site.\'  She briefly steps out of the room before returning with a syringe.  \'It\'ll be ok, the good news is that the pain for this injection will quickly subside, just take nice easy breaths and you\'ll be fine.  In fact, you might find this quite enjoyable after the first hurdle.\'  She steps over to you and carefully pushes the syringe into your first breast.  You feel a sharp pain from the needle going in, but after a moment you start to feel a glowing pleasure spread throughout your breast.  ', false );
				//[if player only has one row of breasts]
				if( CoC.getInstance().player.breastRows.length === 1 ) {
					EngineCore.outputText( 'Once about half of the fluid has been injected into your first breast, she moves on to the other and you feel a similar sharp pain followed by the glowing pleasure.  ', false );
				}
				//[if the player has two rows of breasts]
				if( CoC.getInstance().player.breastRows.length === 2 ) {
					EngineCore.outputText( 'Once about a quarter of the fluid has been injected into the first breast, she moves on to the second and you feel a similar sharp pain followed by the glowing pleasure.  She continues this until all of your ' + CoC.getInstance().player.allBreastsDescript() + ' have been injected.  ', false );
				}
				//[if the player has three rows of breasts]
				if( CoC.getInstance().player.breastRows.length > 2 ) {
					EngineCore.outputText( 'Once about a sixth of the fluid has been injected into the first breast, she moves on to the second and you feel a similar sharp pain followed by the glowing pleasure.  She continues this until all of your ' + CoC.getInstance().player.allBreastsDescript() + ' have been injected.  ', false );
				}
				EngineCore.outputText( 'Soon the glowing pleasure fills your breasts completely and you feel an orgasm shoot through your body as your ' + CoC.getInstance().player.allBreastsDescript(), false );
				//[if breasts are already lactating]
				if( CoC.getInstance().player.biggestLactation() >= 1 ) {
					EngineCore.outputText( ' start lactating even more.  ', false );
				}//[if breasts are not lactating]
				else {
					EngineCore.outputText( ' start lactating.  ', false );
				}
				EngineCore.outputText( 'Nodding approvingly, the nurse says  "<i>Good, for now I\'ve got to go take care of another patient. Don\'t you do anymore running about until I get back, ok?</i>" as she turns away and goes out the door.', false );
				CoC.getInstance().player.boostLactation( 1.5 );
				changed = true;
				EngineCore.dynStats( 'sen', 3 );
				changedBoobs = true;
			...
			if( CoC.getInstance().player.vaginas.length > 0 && !changed ) {
			...
			if( CoC.getInstance().player.gender === 0 && !changed ) {
			...
			EngineCore.outputText( 'Her outfit shifts into that of a noble woman, and she looks down at you.', false );
			if( CoC.getInstance().player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_CENTAUR ) {
				EngineCore.outputText( '  It dawns to your fogged mind that you now have a human-looking lower body, though your sexual organs are unchanged.', false );
				if( CoC.getInstance().player.cockTotal() > 0 ) {
					EngineCore.outputText( '  As you see your ' + Descriptors.multiCockDescriptLight() + ', \'incredibly large\' is the only qualifier you can think of.', false );
				}
			}
			...
			if( CoC.getInstance().player.vaginas.length > 0 && CoC.getInstance().player.cocks.length > 0 ) {
				EngineCore.outputText( 'You see your ' + CoC.getInstance().player.armorName + ' becoming a nice, comfortable outfit of a white shirt and black pants that make you look well-suited to work in a manor... if they didn\'t have an exposed crotch and butt that puts your ' + Descriptors.multiCockDescriptLight() + ', your ' + Descriptors.vaginaDescript( 0 ) + ', and your ' + Descriptors.buttDescript() + ' on full display.  Any undergarments you had are now gone.\n\n', false );
				if( CoC.getInstance().player.armorValue === 0 ) {
					CoC.getInstance().player.modArmorName = 'crotch-revealing clothes';
				}
				EngineCore.outputText( '"<i>Toy, I am feeling aroused,</i>" she says to you in an authoritative voice. "<i>I want you to please me.</i>"\n\n', false );
				EngineCore.outputText( '"<i>Yes, mistress,</i>" you reply without hesitation, your thoughts unable to comprehend anything besides this \'game\'.  You could swear that you were in the sitting room of a manor.  "<i>How do you wish to be pleased?</i>"\n\n', false );
				EngineCore.outputText( '"<i>I\'m glad to see you remember your place.  I wish you to first rub my feet, then move up my legs, and finally, start relieving the tension in my lower lips with your capable hands.</i>"\n\n', false );
				EngineCore.outputText( 'Eager to please your mistress, now reclining on a lovely chair, you crawl to her, remove her shoes, and start to massage her feet.  Your hands seem to move with the will of some outside force and they expertly work to please your mistress.  Pushed on by her approving groans, you work your way up her legs, removing clothing as you go, until you reach her hips.  There, to your mistress\' great delight, you run your hands over her hips and butt, and then start to rub her lower lips and clit.\n\n', false );
				EngineCore.outputText( 'After a minute of this, she stops you and says, "<i>Good; I am now ready to have you inside of me, toy.  Lie down and start fingering your ' + Descriptors.vaginaDescript( 0 ) + ' while I use the ' + Descriptors.multiCockDescriptLight() + ' that I keep you here for.</i>"\n\n', false );
				EngineCore.outputText( 'With your hands slightly trembling in anticipation, you lie down and grab at your very erect ' + Descriptors.multiCockDescriptLight() + ' just before you slip your fingers into your ' + Descriptors.vaginaDescript( 0 ) + ', ready to please your mistress the best way you can.  You watch your mistress rise off her chair and slowly lower herself onto your ' + Descriptors.cockDescript( 0 ) + ', moaning in pleasure.  Her interior is so familiar and so wonderful.  You increase the pace that your fingers plunge into your depths, moaning in the way you know your mistress loves. She starts to rock back and forth, your ' + Descriptors.cockDescript( 0 ) + ' bringing her to the height of ecstasy. ', false );
				//[if player has 2 dicks]
				if( CoC.getInstance().player.cocks.length === 2 ) {
					EngineCore.outputText( 'Your ' + Descriptors.cockDescript( 1 ) + ' rubs against her skin pleasantly.  ', false );
				}
				//[if player has more than 2 dicks]
				if( CoC.getInstance().player.cocks.length > 2 ) {
					EngineCore.outputText( 'The rest of your ' + Descriptors.multiCockDescriptLight() + ' rubs against her skin pleasantly.  ', false );
				}
				EngineCore.outputText( 'Thanks to your extended foreplay, your mistress quickly comes to a climax that reverberates through both your bodies, causing you to release yourself into your mistress and cover your fingers with your other fluids.', false );
				//[if player has multiple dicks]
				if( CoC.getInstance().player.cocks.length > 1 ) {
					EngineCore.outputText( '  You take special care to make sure that the rest of your cum does not get on your mistress in any way.', false );
				}
				...
				if( CoC.getInstance().player.cocks.length > 0 && (CoC.getInstance().player.vaginas.length === 0 || Utils.rand( 2 ) === 0) ) {
					EngineCore.outputText( '  You see that your ' + CoC.getInstance().player.armorName + ' have become a nice looking, if simple, frilled white shirt and bow tie along with black pants and matching shoes.  They make you look like you would be at home in a manor attending the needs of a noble, if it were not for the flaps on the front and back of your pants that give easy access to your ' + Descriptors.multiCockDescriptLight(), false );
					if( CoC.getInstance().player.vaginas.length ) {
						EngineCore.outputText( ', your ' + Descriptors.vaginaDescript( 0 ) + ',', false );
					}
					EngineCore.outputText( ' and your ' + Descriptors.buttDescript() + '.  Any undergarments you had are now gone.\n\n', false );
					if( CoC.getInstance().player.armorValue === 0 ) {
						CoC.getInstance().player.modArmorName = 'cute servant\'s clothes';
					}
					EngineCore.outputText( '"Boy, I am feeling aroused." she says to you in an authoritative voice. "I want you to please me."\n\n', false );
					EngineCore.outputText( '"Yes, mistress." you reply without hesitation, your thoughts unable to comprehend anything besides this "game". You could swear that you were in the sitting room of a manor.  "How do you wish to be pleased?"\n\n', false );
					EngineCore.outputText( '"I\'m glad to see you remember your place.  I wish you to first rub my feet, then move up my legs, and finally, start relieving the tension in my lower lips with your capable hands."\n\n', false );
					...
					EngineCore.outputText( 'After a minute of this, she stops you and says, "Good, I am now ready to have you inside of me boy.  Show me your ' + Descriptors.multiCockDescriptLight() + ' that I keep you here for."\n\n', false );
					EngineCore.outputText( 'With a cry of joy you lift the front flap on your pants revealing your very erect ' + Descriptors.multiCockDescriptLight() + ' and lie on the ground ready to please your mistress the best way you can.  You watch your mistress rise off her chair and slowly lower herself onto your ' + Descriptors.cockDescript( 0 ) + ' moaning in pleasure.  Her interior is so familiar and so wonderful.  ', false );
					if( CoC.getInstance().player.vaginas.length > 0 ) {
						EngineCore.outputText( 'You feel desire start to well up in your ' + Descriptors.vaginaDescript( 0 ) + ', but you resist the urge to act on it; that\'s not why your mistress keeps you. ', false );
					}
					EngineCore.outputText( 'She starts to rock back and forth, your ' + Descriptors.cockDescript( 0 ) + ' bringing her to the height of pleasure. ', false );
					if( CoC.getInstance().player.cocks.length === 2 ) {
						EngineCore.outputText( 'Your ' + Descriptors.cockDescript( 1 ) + ' rubs against her skin pleasantly.  ', false );
					}
					if( CoC.getInstance().player.cocks.length > 2 ) {
						EngineCore.outputText( 'The rest of your ' + Descriptors.multiCockDescriptLight() + ' rub against her skin pleasantly.  ', false );
					}
					EngineCore.outputText( 'Thanks to your extended foreplay, your mistress quickly comes to a climax that reverberates through both your bodies, causing you to release yourself into your mistress.  ', false );
					if( CoC.getInstance().player.cocks.length > 1 ) {
						EngineCore.outputText( 'You take special care to make sure that the rest of your cum does not get on your mistress in any way.  ', false );
					}
					EngineCore.outputText( '\n\nSatisfied, your mistress rises off of you and dresses.  She tells you that she is going for a walk, and that you should continue your duties now.  ', false );
				...
				//(variant 2: maid, requires vagina)
					...
					EngineCore.outputText( 'You see that your ' + CoC.getInstance().player.armorName + ' have become an apron over a plain dress with high stockings and simple black shoes.  They make you look like you would be at home keeping a manor clean and tidy, if it were not for the rather short skirt and lack of undergarments that give easy access to your ' + Descriptors.vaginaDescript( 0 ), false );
					...
					EngineCore.outputText( ' and your ' + Descriptors.buttDescript() + '.\n\n', false );
					if( CoC.getInstance().player.armorValue === 0 ) {
						CoC.getInstance().player.modArmorName = 'maid\'s clothes';
					}
					EngineCore.outputText( '"Girl, I am feeling aroused." she says to you in an authoritative voice. \'I want you to please me.\'\n\n', false );
					EngineCore.outputText( '"Yes, mistress." you reply without hesitation, your thoughts unable to comprehend anything besides this \'game\'.  You could swear that you were in the sitting room of a manor.  "How do you wish to be pleased?"\n\n', false );
					EngineCore.outputText( '"I\'m glad to see you remember your place.  I wish you to first rub my feet, then move up my legs, and finally, start relieving the tension in my lower lips with your capable hands."\n\n', false );
					EngineCore.outputText( 'Eager to please your mistress, now reclining on a lovely chair, you crawl to her, remove her shoes, and start to massage her feet.  Your hands seem to move with the will of some outside force and they expertly work to please your mistress.  Pushed on by her approving groans, you work your way up her legs, removing clothing as you go, until you reach her hips.  There, to your mistress\' great delight, you run your hands over her hips, butt, and then start to rub her hips, lower lips, and clit.\n\n', false );
					EngineCore.outputText( 'After a minute of this, she says, "Good.  Now finger yourself before me, girl.  Show me that ' + Descriptors.vaginaDescript( 0 ) + ' that I keep you here for."\n\n', false );
					EngineCore.outputText( 'You lay back on the ground and raise your skirt, showing off your engorged ' + Descriptors.clitDescript() + ' and ' + Descriptors.vaginaDescript( 0 ) + '.  With a pleased squeal you plunge your fingers into your waiting ' + Descriptors.vaginaDescript( 0 ) + ' and start wriggling them around in the way that your mistress loves.  ', false );
					//[if player has a dick]
					if( CoC.getInstance().player.cocks.length > 0 ) {
						EngineCore.outputText( 'You feel a desire rising in your erect ' + Descriptors.multiCockDescriptLight() + ' but you ignore it; that\'s not what your mistress told you to play with.  ', false );
					}
					EngineCore.outputText( 'Thanks to the extended foreplay, the sight of you quickly brings your mistress to climax, a climax that quickly fills you as well, causing you to cum all over your fingers.\n\n', false );
					EngineCore.outputText( 'Satisfied, your mistress rises and dresses,  She tells you that she is going for a walk, and that you should continue your duties now.  ', false );
				}
			}
			//(variant 4: gender-less servant, requires that you have no dick or vagina)
			if( CoC.getInstance().player.gender === 0 ) {
				EngineCore.outputText( 'You see that your ' + CoC.getInstance().player.armorName + ' have become a nice and comfortable outfit of a shirt white and black pants, that make you look well suited to work in a manor.  However, you notice that there is a flap on your pants that gives easy access to your ' + Descriptors.buttDescript() + '.\n\n', false );
				if( CoC.getInstance().player.armorValue === 0 ) {
					CoC.getInstance().player.modArmorName = 'servant\'s clothes';
				}
				EngineCore.outputText( '"Servant, I am feeling aroused." she says to you in an authoritative voice. \'I want you to please me.\'\n\n', false );
				EngineCore.outputText( '"Yes, mistress." you reply without hesitation, your thoughts unable to comprehend anything besides this \'game\'.  You could swear that you were in the sitting room of a manor.  "How do you wish to be pleased?"\n\n', false );
				EngineCore.outputText( '"I\'m glad to see you remember your place.  I wish you to first rub my feet, then move up my legs, and finally, start relieving the tension in my lower lips with your capable hands."\n\n', false );
				...
				EngineCore.outputText( 'After a few minute of this, your expert fingers bring your mistress\' body to a full climax.  A climax that you too feel, bringing with it the relief that only your mistress can give you.\n\n', false );
				EngineCore.outputText( 'Satisfied, your mistress rises and dresses.  She tells you that she is going for a walk, and that you should continue your duties now.  ', false );
			}
			//Check for bad-end start!
			if( CoC.getInstance().player.inte <= 10 ) {
	...
	FetishCultistScene.prototype.playerRapesCultist = function() {
		...
		if( CoC.getInstance().monster.armorName === 'pervy nun\'s clothing' ) {
			...
			if( CoC.getInstance().player.cor < 40 ) {
				EngineCore.outputText( 'You are rather taken aback by this odd declaration, but in the end you decide the hell, I\'m horny.</i>\n\n', false );
			} else {
				EngineCore.outputText( 'You nod to yourself thinking right, I am a sex god.</i>\n\n', false );
			}
			EngineCore.outputText( 'You stride over to her and loosen your ' + CoC.getInstance().player.armorName + ' and let them drop to the ground before sitting down in front of her.  "<i>Show me your affections, then maybe I\'ll be willing to take you,</i>" you tell her.  She looks over your naked body with a sort of reverence before bowing her head and saying, "<i>Of course my lord.</i>"  With that, she gets to work on you.\n\n', false );
			EngineCore.outputText( 'She takes one of your arms and begins kissing it, from your shoulder to your hand.  Each kiss sends a short quiver of pleasure through you, one you could swear the cultist was also feeling.  When she gets to your hands she kisses each of your fingers, then takes each one into her mouth in turn, sucking on them and licking them with passion.  Her lustful acts are intense, and you can feel yourself getting close to your peak, just from her playing with your fingers.  With a shudder from the sensations, you haltingly tell her, "<i>Good, now the other arm.</i>"  With no hesitation, she complies.  Before too long, the fingers on your other hand are being teased and cleaned by her mouth, and again you are brought to the brink.  You shakily pull your hand back and tell her that she has done enough; you are willing to take her now.\n\n', false );
			//(if player has a dick)
			if( CoC.getInstance().player.cocks.length > 0 && (CoC.getInstance().player.gender !== 3 || Utils.rand( 2 )) ) {
				EngineCore.outputText( 'She meekly leans back onto the ground and parts her legs with her hands, giving you a clear view of her needy hole through the large opening in her clothing.  "<i>My lord, please fill me!</i>" she begs you.  You stand and stride over to her.  "<i>Yes little girl, accept my divine rod', false );
				if( CoC.getInstance().player.cocks.length > 1 ) {
					EngineCore.outputText( 's', false );
				}
				EngineCore.outputText( ' in your pussy,</i>" you reply.  ', false );
				if( CoC.getInstance().player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_CENTAUR ) {
					EngineCore.outputText( 'You command her to get on all fours, then cover her and push ', false );
				} else {
					EngineCore.outputText( 'You grab hold of and lift her legs, then kneel down and push ', false );
				}
				if( CoC.getInstance().player.cocks.length > 1 ) {
					EngineCore.outputText( 'the first of ', false );
				}
				EngineCore.outputText( 'your ', false );
				EngineCore.outputText( Descriptors.multiCockDescriptLight() + ' into her waiting love hole.  ', false );
				if( CoC.getInstance().player.cocks.length > 1 ) {
					EngineCore.outputText( 'You thrust once, before using one of your hands to guide the second of your ' + Descriptors.multiCockDescriptLight() + ' inside her.  You are rather surprised at how easily she seems to accommodate your size', false );
					if( CoC.getInstance().player.cocks.length > 2 ) {
						EngineCore.outputText( ' and decide to push the ', false );
						if( CoC.getInstance().player.cocks.length === 3 ) {
							EngineCore.outputText( 'last', false );
						}
						if( CoC.getInstance().player.cocks.length > 3 ) {
							EngineCore.outputText( 'rest', false );
						}
						EngineCore.outputText( ' in the same way.  ', false );
					} else {
						EngineCore.outputText( '.  ', false );
					}
				}
				EngineCore.outputText( 'Fully inside her, you begin to fuck her in earnest, and her eager moans soon fill the air.  Within moments the cultist climaxes unexpectedly quickly, and the feeling soon rushes through you as well, culminating in an incredibly huge orgasm that shakes your whole body. You ejaculate deep into her welcoming cunt, filling it up completely', false );
				if( CoC.getInstance().player.cumQ() < 50 ) {
					EngineCore.outputText( '.', false );
				}
				if( CoC.getInstance().player.cumQ() >= 50 && CoC.getInstance().player.cumQ() < 200 ) {
					EngineCore.outputText( ' before streams of cum begin to run down her thighs.', false );
				}
				if( CoC.getInstance().player.cumQ() >= 200 ) {
					EngineCore.outputText( ' before torrents of cum begin to run down her legs and onto the ground.', false );
				}
				EngineCore.outputText( '  You pull out and the cultist slumps onto the ground contentedly, massaging her well-fucked, cum-filled pussy.  Looking at the cultist you wonder if she was able to make you spontaneously orgasm just from having an orgasm herself, or if she has the power of a real sex god?\n\n', false );
			}
			//(if player has a vagina)
			else if( CoC.getInstance().player.vaginas.length > 0 && CoC.getInstance().player.lowerBody !== AppearanceDefs.LOWER_BODY_TYPE_CENTAUR ) {
				EngineCore.outputText( 'She meekly lies back on the ground and spreads her legs, one slightly in the air.  You have a clear view of her needy hole through the large opening in her clothing.  "<i>My lord, please scissor me!</i>" she begs you.  You stand and stride over to her. "<i>Yes little girl, accept my divine lips on your pussy,</i>" you reply.  You grab the leg she had in the air and pull yourself between her legs, pressing your ' + Descriptors.vaginaDescript( 0 ) + ' against her own.  With a firm tug on her leg, you begin to push and rub your pussies together in earnest, and her eager moans soon fill the air.  The cultist climaxes unexpectedly quickly, and the feeling soon rushes through you as well.  You pull back and the cultist slumps onto the ground contentedly.  Looking at the cultist you wonder if she was able to make you spontaneously orgasm just from having an orgasm herself, or if she has the power of a real sex god?\n\n', false );
			}
			//Horsey
			if( CoC.getInstance().player.vaginas.length > 0 && CoC.getInstance().player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_CENTAUR ) {
				EngineCore.outputText( 'She meekly looks at your equine lower body, obviously unused to it, and tentatively asks if she can lick your pussy. Smiling, you present your buttocks to her and say, "<i>Yes little girl, you can pleasure my divine lips with your tongue.</i>" She happily gets to it, and you hear eager licking and sucking sounds as a warmth diffuses in your nether parts. She sure knows how to handle a cunt, and you find yourself cumming very quickly, as an incredibly huge orgasm shakes your whole body, sending girl-cum all over her face. You turn around and see the cultist with a look of utter bliss on her face, as if she\'s just been blessed.  You wonder from the quality of the orgasm she just gave you if she has the power of a real sex god.\n\n', false );
			}
			//(after either)
			EngineCore.outputText( 'You get dressed and notice that the cultist is still lying on the ground in the perverted nun outfit, with a look of utter bliss on her face.  ', false );
			EngineCore.outputText( 'Satisfied, you continue on your way.', false );
			CoC.getInstance().player.orgasm();
		} else if( CoC.getInstance().monster.armorName === 'swimsuit' ) {
			//(scene requires that the PC not be genderless)
			//foreplay
			EngineCore.outputText( 'You advance on the defeated cultist, whose outfit seems to have fixated on such a ridiculously skimpy swimming suit that she might as well be naked.  She looks up at you with a worried expression as you take a closer look at her.  The cultist slowly starts to speak, ""<i>Ah, hey there ' + CoC.getInstance().player.mf( 'mister', 'miss' ) + ', what a surprise to see you here!</i>"  Embarrassed, she tries to make her incredibly revealing swimming suit somewhat less obscene while blushing furiously.  "<i>Oh mercy me, just thinking about how much of me that you can see is turning me on.</i>"  You arch an eyebrow this girl an exhibitionist?\n\n', false );
			EngineCore.outputText( 'You slowly ask her why, if she likes it so much, is she covering herself up?  She looks around a little nervously for a moment before pulling her hands away and giving you a good look at her body.  You see her breathing becoming more rapid.  "<i>Oh fuck yeah, look at me in this slutty outfit,</i>" she says, starting to shiver.  She giggles and does a stretch while "accidentally" getting part of her suit to slip off.  "<i>Oops, you can totally see my nipple now, can\'t you?</i>" she says breathlessly, her crotch now visibly soaked with her arousal.\n\n', false );
			//if (PC is an exhibitionist)
			if( CoC.getInstance().flags[ kFLAGS.PC_FETISH ] > 0 ) {
				EngineCore.outputText( 'Given your experience with Ceraph, you can definitely understand why she is having so much fun showing off her body.  In fact, you think you\'ll start to do the same.', false );
			} else {
				EngineCore.outputText( 'You don\'t really understand what could be so arousing about showing off your body to others, but you decided you were going to rape her, so you might as well strip down now.', false );
			}
			EngineCore.outputText( '  At the sight of your nude body, the nearly naked woman gives another shiver.  "<i>Oh sweet cocks, you look so sexy,</i>" she says running her hands over her heaving breasts.  "<i>Please, can we fuck?</i>" she begs you.\n\n', false );
			//corruption check!
			if( CoC.getInstance().player.cor < 33 ) {
				EngineCore.outputText( 'Well, she obviously wants this, so you guess there isn\'t anything wrong with enjoying what\'s on offer, is there?\n\n', false );
			} else if( CoC.getInstance().player.cor < 66 ) {
				EngineCore.outputText( 'You think to yourself how awesome this is.  It isn\'t often a defeated foe is so eager.\n\n', false );
			} else {
				EngineCore.outputText( 'You\'re almost disappointed at how easy she is making this; but hey, a fuck is a fuck, so what do you care?\n\n', false );
			}
			//end of the foreplay
			//sex to be written here
			EngineCore.outputText( 'The cultist turns around, presenting her rear end to you, looking over her shoulder.  ', false );
			if( CoC.getInstance().player.hasCock() ) {
				EngineCore.outputText( 'Her eyes lock onto your very erect maleness.  "<i>I got that thing that hard?  Oh fuck yes!</i>"  She wraps her arms around her body and rocks back and forth.  ', false );
				//if (PC has a vagina)
				if( CoC.getInstance().player.hasVagina() ) {
					EngineCore.outputText( 'Then, she hesitates for some reason, before saying, "<i>Um, I\'m not sure why I\'m asking this since you\'ve got a good tool there already, but... I\'ve got some vibrators, if you want to play with them instead...</i>"\n\n', false );
				} else {
					EngineCore.outputText( '"<i>I want this so damn much!</i>"\n\n', false );
				}
			}
			//if you ain't got a cock, break out the vibrators
			else {
				EngineCore.outputText( '"<i>Uh, hey, I\'ve got some toys that we could play with,</i>" she says a bit nervously.  "<i>Do you want to play with some vibrators?</i>"\n\n', false );
			}
			...
			if( CoC.getInstance().player.cor < 40 ) {
				EngineCore.outputText( 'You stop, thinking that maybe you should let her go, but she grabs your hand and gives you a look of desperation.  Deciding to finish the job, you push her to her knees and the look of fear returns to her face.  ', false );
			} else {
				EngineCore.outputText( 'With a wicked smile, you push her to her knees.  ', false );
			}
			//(if player has a dick)
			if( CoC.getInstance().player.cocks.length > 0 && (CoC.getInstance().player.gender !== 3 || Utils.rand( 2 )) ) {
				EngineCore.outputText( 'Telling her that she is going to please you whether she wants to or not, you reveal your ' + Descriptors.multiCockDescriptLight() + '.  She screams for help and starts struggling against your grip, but you tell her that no one is going to save her now.  Easily overpowering her, you force her to the ground with no care for her comfort and start tearing off her clothes.  Once you have a clear view of her love hole, you look back up at her face and see that she has stopped struggling and is now crying.  ', false );
				//[if not corrupted]
				if( CoC.getInstance().player.cor < 40 ) {
					EngineCore.outputText( 'Feeling uncomfortable at this, you start to pull away, but her expression turns instantly into a look of pure horror and her body becomes completely tense.  As you move back to her, her body relaxes and her expression returns to the tear-stricken face.  Obviously this is an act and she would be far more bothered by you if you left her without finishing the job.  ', false );
				}
				EngineCore.outputText( 'Filled with new determination, you push your ' + Descriptors.cockDescript( 0 ) + ' into her love hole and start to rape her roughly.  Despite the tears, she starts moaning, clearly enjoying the rough treatment.  A little while into the rape, she starts to move against you, almost lovingly.  It is quite clear that she has a lot of experience, but is giving you free reign to do whatever you want.   Feeling in complete control, you decide that this game isn\'t too bad.  Before long you reach your orgasm and you blow your load deep inside her.', false );
			...
			//(if player has a vagina)
				...
				EngineCore.outputText( 'Telling her that she is going to please you whether she wants to or not, you reveal your ' + Descriptors.vaginaDescript( 0 ) + '.  Pushing your ' + Descriptors.clitDescript() + ' to her mouth, you order her to lick you if she wants you to leave her mother alone.  ', false );
				EngineCore.outputText( 'She screams for help and starts struggling against your grip, but you tell her that no one is going to save her now.  You feel her tongue go in, and she hesitantly gives a lick.  You pull her head back by the hair and look into her eyes.  She is crying!  ', false );
				if( CoC.getInstance().player.cor < 40 ) {
					EngineCore.outputText( 'Feeling uncomfortable at this, you start to pull away, but her expression turns instantly into a look of pure horror and her body becomes completely tense.  As you move back to her, her body relaxes and her expression returns to the tear stricken face.  Obviously this is an act and she would be far more bothered by you if you left her without finishing the job.  ', false );
				}
				EngineCore.outputText( 'With new determination, you yell at her to lick harder, and to push in as much as she can, or her mother will die.  This time when she starts licking you, she does it almost lovingly.  It is quite clear that she has a lot of experience and, regardless of her words, she obviously wants this.  Feeling in complete control, you think that this game isn\'t too bad.  Under her expert flicks, you quickly reach a climax.  You pull her head back and tell her that now she has to clean you up.  You specify that she can\'t just lick this time, she has to drink your excretions.\n\n', false );
				EngineCore.outputText( 'A few minutes later, you close your ' + CoC.getInstance().player.armorName + ' back up, and look back at the cultist.  She is lying on the ground in her farm hand outfit with a look of utter bliss on her face.  Satisfied, you continue on your way.', false );
				CoC.getInstance().player.orgasm();
			}
			if( CoC.getInstance().player.gender === 0 ) {
				EngineCore.outputText( 'You make her rub your body down, but you don\'t really have a means to rape her.  Afterwards you do feel better, but didn\'t get any real release.  Disappointed, you continue on your way.', false );
			}
		}
		Combat.cleanupAfterCombat();
	};
	...
	FetishCultistScene.prototype.plugSwimsuitVag = function() {
		...
		if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( 'You decline the vibrators; you\'d much rather take her incredibly wet pussy.  ', false );
		} else {
			EngineCore.outputText( 'You tell her you\'re gonna fuck her.  ', false );
		}
		EngineCore.outputText( 'She nods at you and turns around shivering even harder.  You step forward and put your hands on her shoulders, then start to run them over her body, while your ' + Descriptors.multiCockDescriptLight() + ' pokes her in the back.  "<i>Oh fuck me, I need you to fuck me now!</i>" she screams.  Grinning, you push her to the ground and grip her pert, bouncing breasts.  You push her bathing suit to the side so that you have access to her waiting snatch.  You assure her that you\'ll be fucking her all right, all while you tease and play with her exposed body.\n\n', false );
		EngineCore.outputText( 'When you push your ' + Descriptors.cockDescript( x ) + ' inside her, she goes absolutely insane underneath you.  She squirms and wriggles all over your ' + Descriptors.cockDescript( x ) + ', as you ', false );
		if( CoC.getInstance().player.cockArea( x ) < 50 ) {
			EngineCore.outputText( 'push further and further until you\'re all the way inside her.', false );
		} else if( CoC.getInstance().player.cockArea( x ) < 200 ) {
			EngineCore.outputText( 'push further and further, until to your great delight, she manages to take all of your massive member without any trouble!', false );
		} else {
			EngineCore.outputText( 'manage to get your monster cock further and further inside her.  To your immense shock, she manages to take the whole thing, without it seeming to warp her body at all!', false );
		}
		EngineCore.outputText( '  Her slick interior seems to fit your shaft perfectly, and with the way she is squirming around you, you\'re getting one hell of a ride.\n\n', false );
		EngineCore.outputText( 'After all the foreplay, the two of you are already reaching your peaks thanks to her incredible, tight cunt and its ministrations to your member.  The cultist is screaming in pleasure, telling you to look at her, to fondle her, and to fuck her; you happily oblige her.  Her interior contracts about your ' + Descriptors.cockDescript( x ) + ' as you loose the proof of your orgasm deep inside her.', false );
		if( CoC.getInstance().player.cockTotal() > 1 ) {
			EngineCore.outputText( '  Your remaining cock', false );
			if( CoC.getInstance().player.cockTotal() === 2 ) {
				EngineCore.outputText( ' lets loose its load', false );
			} else {
				EngineCore.outputText( 's let loose their loads', false );
			}
			EngineCore.outputText( ' onto her back and legs, ', false );
			if( CoC.getInstance().player.cumQ() < 50 ) {
				EngineCore.outputText( 'adding a little white decoration', false );
			} else if( CoC.getInstance().player.cumQ() < 200 ) {
				EngineCore.outputText( 'adding a liberal coating of white to her body', false );
			} else {
				EngineCore.outputText( 'drenching her in your cum', false );
			}
			EngineCore.outputText( '.', false );
		}
		EngineCore.outputText( '\n\n', false );
		EngineCore.outputText( 'You stand up and clean yourself off, thoroughly satisfied with the encounter.  The cultist, on the other hand, collapses in a quivering pile of pleasure on the ground.', false );
	...
	FetishCultistScene.prototype.fetishCultistHasAMilkFetish = function() {
		...
		if( CoC.getInstance().player.hasCock() ) {
			...
			EngineCore.outputText( 'our ' + Descriptors.multiCockDescriptLight() + ' becomes fully erect as it rubs up against her warm ass cheeks, which is enough to fulfill its desire.   Her cum bubbles into the palm of your hand while your cum drizzles her ass with sticky whiteness.\n\n', false );
		}
		//[ELSE Pussy]
		else if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( 'Your pussy becomes fully aroused as you continue to play with each other. Her nice, warm ass against it is enough to fulfill its desire as you feel her cum in the palm of your hand.  You get off underneath her, twitching and dripping with release.\n\n', false );
		}
		//[Genderless]
		else {
			EngineCore.outputText( 'Your entire body tenses up, like you\'re approaching an orgasm. The release of this tension flows through your body as she cums into the palm of your hand. You sigh with relief as the phantom orgasm lingers.\n\n', false );
		}
		EngineCore.outputText( 'You both keep playing with each other as you moan with ecstasy, having one orgasm after the other. You and the cultist stop after the 10th simultaneous climax, both of you soaked with milk, spit, cum, and sweat. The cultist cuddles up to you and falls asleep, smelling of sex. You sit her down on the rocking chair and make your way out of the fantasy. It\'s difficult, as if each orgasm made the fantasy more and more real to you.  You focus hard... ', false );
		//[Low Int]
		if( CoC.getInstance().player.inte < 33 ) {
			EngineCore.outputText( 'but nothing happens. You even try to open the door of the cabin but it\'s locked. You poke around, when a brilliant idea comes to you. You place your hand in your mouth and bite down hard. The intense pain brings you back to reality.  ', false );
			//(Minus 5hp)
			CoC.getInstance().player.takeDamage( 5 );
		}
		//[Med Int]
		else if( CoC.getInstance().player.inte < 66 ) {
			EngineCore.outputText( 'your head turns red, veins bulging. The scenery very slowly turns back to the lake. You maintain your intense concentration, as even one second of slacking would pull you back into the fantasy. Eventually you\'re back in reality.  ', false );
		}
		//[High Int]
		else {
			EngineCore.outputText( 'and everything goes back to normal.  ', false );
		}
		EngineCore.outputText( 'You see the cultist is sleeping on the ground, still in the fantasy she created.', false );
