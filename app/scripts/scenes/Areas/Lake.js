...
angular.module( 'cocjs' ).run( function( SceneLib, $log, CoC, Utils, EngineCore, kFLAGS, StatusAffects, Combat, Descriptors, AppearanceDefs, PregnancyStore, GreenSlime, ConsumableLib, CockTypesEnum ) {
	...
	Lake.prototype.exploreLake = function() {
		...
		//Egg chooser
		if( Utils.rand( 100 ) < 25 && CoC.getInstance().player.pregnancyIncubation > 1 && CoC.getInstance().player.pregnancyType === PregnancyStore.PREGNANCY_OVIELIXIR_EGGS ) {
		...
		//==================================================
		//COMMON EVENTS
		if( CoC.getInstance().player.level < 2 || CoC.getInstance().player.spe < 50 ) {
			choice[ choice.length ] = 0;
		}
		choice[ choice.length ] = 1;
		choice[ choice.length ] = 2;
		//Fetish cultist not encountered till level 2
		if( CoC.getInstance().player.level >= 2 && CoC.getInstance().player.findStatusAffect( StatusAffects.DungeonShutDown ) >= 0 ) {
			choice[ choice.length ] = 3;
		}
		//Slimes/Ooze = level >= 2
		if( CoC.getInstance().player.level >= 2 ) {
			choice[ choice.length ] = 4;
		}
		//Izma
		if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00230 ] > 0 && (CoC.getInstance().player.exploredLake >= 10) && (CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00233 ] === 0 || CoC.getInstance().player.findStatusAffect( StatusAffects.Infested ) < 0) && CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00238 ] <= 0 ) {
		...
		if( dickDraggingChances > 0 && CoC.getInstance().player.longestCockLength() >= CoC.getInstance().player.tallness - 10 && CoC.getInstance().player.totalCockThickness() >= 8 ) {
		...
		//Sword Discovery
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.TookBlessedSword ) < 0 && CoC.getInstance().player.findStatusAffect( StatusAffects.BSwordBroken ) < 0 ) {
		...
		//Chance of dick-dragging! 10% + 10% per two foot up to 30%
			...
			//OOZE!
				...
				CoC.getInstance().flags[ kFLAGS.TIMES_MET_OOZE ]++;
				EngineCore.spriteSelect( 25 );
				//High int starts on even footing.
				if( CoC.getInstance().player.inte >= 25 ) {
					EngineCore.outputText( 'A soft shuffling sound catches your attention and you turn around, spotting an amorphous green mass sliding towards you!  Realizing it\'s been spotted, the ooze\'s mass surges upwards into a humanoid form with thick arms and wide shoulders.  The beast surges forward to attack!', true );
			...
			EngineCore.outputText( 'Your quick walk along the lakeshore feels good.', true );
			...
			if( CoC.getInstance().player.cor > 30 || CoC.getInstance().player.lust > 60 || CoC.getInstance().player.lib > 40 ) {
				EngineCore.outputText( 'Your imaginings increasingly seem to turn ', false );
			} else {
				EngineCore.dynStats( 'int', 1 );
			}
			if( (CoC.getInstance().player.cor > 30 && CoC.getInstance().player.cor < 60) || (CoC.getInstance().player.lust > 60 && CoC.getInstance().player.lust < 90) || (CoC.getInstance().player.lib > 40 && CoC.getInstance().player.lib < 75) ) {
				EngineCore.outputText( 'to thoughts of sex.', false );
				EngineCore.dynStats( 'lus', (5 + CoC.getInstance().player.lib / 10) );
				displayed = true;
			}
			if( ((CoC.getInstance().player.cor >= 60) || (CoC.getInstance().player.lust >= 90) || (CoC.getInstance().player.lib >= 75)) && !displayed ) {
		...
		//Find whitney or equinum
			...
			//Find Whitney
				...
				if( CoC.getInstance().player.findStatusAffect( StatusAffects.MetWhitney ) >= 0 ) {
					...
					if( CoC.getInstance().player.statusAffectv1( StatusAffects.MetWhitney ) > 1 ) {
	...
	Lake.prototype.ottahGirl = function() {
		EngineCore.clearOutput();
		CoC.getInstance().flags[ kFLAGS.MET_OTTERGIRL ]++;
		//First Time
		if( CoC.getInstance().flags[ kFLAGS.MET_OTTERGIRL ] === 1 ) {
			EngineCore.outputText( 'Your exploration of the lakeside takes you further than you\'ve gone before.  The water here is almost completely still, its waters ' );
			if( CoC.getInstance().player.findStatusAffect( StatusAffects.FactoryOverload ) < 0 ) {
				EngineCore.outputText( 'crystal clear, giving you a stunning view of the lakebed' );
			} else {
				EngineCore.outputText( 'only slightly clouded, giving you an obscured view of the lakebed' );
			}
			EngineCore.outputText( '.  Fish dart to and fro within the waters, caring little for your explorations above the waves.' );
			...
			EngineCore.outputText( '\n\n"<i>Fish\'n\'fuck,</i>" she replies, simply.  "<i>I fish, you fuck. Ya ain\'t dense, are you ' + CoC.getInstance().player.mf( 'boy', 'girl' ) + '?</i>"' );
			EngineCore.outputText( '\n\nThat\'s it?  She doesn\'t even know you and she\'s just offering it up like that?' );
			//Silly Mode (EngineCore.silly()) EngineCore.outputText('  No tragic backstory to go through? No annoying combat encounter? Just meet and fuck?  My god, what has this world come to?');
			EngineCore.outputText( '  You don\'t even know her name!' );
		...
		//Repeats
			...
			EngineCore.outputText( 'Your explorations of the lake lead you back to Callu, the otter girl. She sits lazily on the beach; fishing rod in hand, as usual. She gives a friendly wave as you approach, and pats the sandy patch of lakeside next to her.' );
		...
		if( CoC.getInstance().player.lust < 33 ) {
			...
			if( CoC.getInstance().player.hasCock() ) {
				if( CoC.getInstance().player.shortestCockLength() > 48 ) {
	...
	Lake.prototype.ottergirlLikesDongs = function() {
		...
		var notTooBigCocks = _.filter( CoC.getInstance().player.cocks, function( cock ) {
			return cock.cockLength < 48;
		} );
		var x = notTooBigCocks.length > 0 ? _.indexOf( CoC.getInstance().player.cocks, _.maxBy( notTooBigCocks, function( cock ) {
		...
		else if( CoC.getInstance().player.cocks[ x ].cockLength < 10 ) {
		...
		EngineCore.outputText( '  She leisurely stretches out across your stomach and chest, letting her cunt come to rest right in front of your face.' );
		EngineCore.outputText( '\n\nYou feel slender but powerful fingers wrap around your cock, followed shortly after by a pair of lips. They encircle your ' + CoC.getInstance().player.cockHead( x ) + ' and suck, creating a delightful tingling sensation that travels down your cock and into your core.' );
		EngineCore.outputText( '\n\n"<i>Hey darlin\', better get to lickin\', we want this ' );
		//{(lil dicks)
		if( CoC.getInstance().player.cocks[ x ].cockLength < 6 ) {
			EngineCore.outputText( 'little, wanna-be cock' );
		} else {
			EngineCore.outputText( 'bad boy' );
		}
		EngineCore.outputText( ' to slip right in, don\'t we?</i>"  Callu murrs back at you.  You most certainly do, so you lean your head forward ever-so-slightly, extending your tongue and lapping at her delicate pussy lips.  In no time at all they become puffy and flushed, blossoming outwards like a perverse flower.  You run your tongue up and down each and every fold, occasionally stopping to flick over her rapidly hardening clitoris.' );
		...
		EngineCore.outputText( '  She pays particularly close attention ' );

		//[equine]
		if( CoC.getInstance().player.cocks[ x ].cockType === CockTypesEnum.HORSE ) {
			EngineCore.outputText( 'to your flare, sucking, teasing and ' );
		}//[canine]
		else if( CoC.getInstance().player.hasKnot( x ) ) {
			EngineCore.outputText( 'to the base of your cock, planting sloppy kisses on your knot, ' );
		}//[demonic]
		else if( CoC.getInstance().player.cocks[ x ].cockType === CockTypesEnum.DEMON ) {
			EngineCore.outputText( 'to the demonic nodules ringing your cock, ' );
		}//[anemone]
		else if( CoC.getInstance().player.cocks[ x ].cockType === CockTypesEnum.ANEMONE ) {
			EngineCore.outputText( 'to the little wriggling tentacles ringing the head and base of your cock, ' );
		} else {
			EngineCore.outputText( 'to the sensitive little spot on the underside of the head, ' );
		}
		EngineCore.outputText( 'lavishing it with attention.  Precum and saliva practically pour down the length of your shaft, tickling your ' );
		if( CoC.getInstance().player.balls > 0 && CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( 'balls and cunt' );
		...
		EngineCore.outputText( 'girth within her she settles down on your lap, stretching her legs out before retrieving her fishing rod.  "<i>Now don\'t you go movin\' about, darlin\',</i>" Callu says over her shoulder.  "<i>Don\'t wanna go scarin\' the fish away.  I\'ll let ya go after I catch a few good ones.</i>"' );
		...
		if( CoC.getInstance().player.cumQ() >= 250 ) {
			EngineCore.outputText( '  There\'s so much of it, ' );
			if( CoC.getInstance().player.cumQ() < 500 ) {
				EngineCore.outputText( 'some of it begins to dribble down your cock, forming a puddle right under your ass cheeks' );
			} else if( CoC.getInstance().player.cumQ() < 1000 ) {
				EngineCore.outputText( 'it begins to spray out of the edges of your cock, like water coming out of a blocked faucet' );
			} else if( CoC.getInstance().player.cumQ() < 2000 ) {
				EngineCore.outputText( 'Callu\'s stomach begins to visibly inflate, even from your point of view' );
			} else {
				EngineCore.outputText( 'Callu\'s stomach inflates to a huge degree. She suddenly looks to be about eight months pregnant, though she doesn\'t seem bothered by this in the least' );
			}
			EngineCore.outputText( '.' );
		}
		EngineCore.outputText( '  Her womb greedily takes everything it can, until you fall back onto the ground, exhausted.' );
		EngineCore.outputText( '\n\nTo your surprise, Callu simply picks up where she left off' );
		if( CoC.getInstance().player.cumQ() >= 2000 ) {
			EngineCore.outputText( ', despite the huge belly she now sports' );
		}
		EngineCore.outputText( '.  Gyrations, thrusts and the constant cadence of her cunt work together to keep you unbearably hard.  Apparently she\'s not satisfied.' );
		...
		if( CoC.getInstance().player.hasKnot( x ) ) {
			EngineCore.outputText( 'fails, held fast by the knot tying the two of you together. She looks at you in surprise, but eventually smiles and leans back down. The two of you cuddle for half an hour, until your knot deflates enough to let her *pop* off of it. She stands and ' );
		}
		EngineCore.outputText( 'slips her bikini bottoms into a canvas bag.' );
	...
	Lake.prototype.ottersForGals = function() {
		...
		if( CoC.getInstance().player.hasVagina() ) {
			...
			EngineCore.outputText( 'unnatural bare crotch' );
		}
		EngineCore.outputText( ' down to your pucker, she leaves a trail of sloppy smooches.  You mirror her movements, attempting to give her the same experience she\'s giving you.  A low murr escapes her lips, and she squirms above you slightly as your tongue hits the right spots.' );
		...
		if( CoC.getInstance().player.hasFuckableNipples() ) {
			EngineCore.outputText( 'You even slip a finger or two inside, stretching your nipple-cunts out with deliciously pleasurable results.  ' );
		}
		EngineCore.outputText( 'Combined with Callu\'s tender tongue ' );
		if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( 'paying lip service to your wet cunt' );
		} else {
			EngineCore.outputText( 'doing a cave dive in your rear' );
		}
		EngineCore.outputText( ', you can\'t hold out much longer.  All the tingling in your body seems to get forced through your veins, coalescing in a single spot within your groin.  The pressure builds and builds as orgasmic energies begin overflowing.  Your legs and arms tremble, your head wobbles uncertainly, and you can\'t even guess at what your spine is attempting to do.' );
		EngineCore.outputText( '\n\nThe pleasure within you finally bursts outwards, shooting through every nerve, inflaming every fiber of your being.  ' );
		if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( 'Your snatch clenches and clamps down on thin air, flooding Callu\'s face with your feminine juices in a tasty, refreshing spray.' );
		} else {
			EngineCore.outputText( 'Your asshole clenches and spasms randomly, aching to be filled by something, anything in your quest for release.' );
		}
		EngineCore.outputText( '  The orgasmic bliss makes you collapse forwards, dropping you onto all fours. However, your blonde lover grips your thighs firmly, clearly intent on fully repaying her debt.' );
