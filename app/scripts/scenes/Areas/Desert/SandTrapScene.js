...
angular.module( 'cocjs' ).run( function( MainView, SceneLib, CoC, Utils, StatusAffects, EngineCore, AppearanceDefs, Descriptors, PregnancyStore, kFLAGS, SandTrap ) {
	...
	SandTrapScene.prototype.encounterASandTarp = function() {
		...
		CoC.flags[ kFLAGS.TIMES_ENCOUNTERED_SAND_TRAPS ]++;
		if( CoC.flags[ kFLAGS.TIMES_ENCOUNTERED_SAND_TRAPS ] === 1 ) {
			//femininity <= 65:
			if( CoC.player.femininity <= 65 ) {
				MainView.outputText( 'You are exploring the endlessly undulating landscape of the desert when you hear an agonized cry for help over the nearest dune.  You quickly scramble up it and are confronted by the sight of a small, flat lake of exceptionally fine sand, overlooked on each side by tall dunes.  Trapped in the middle of it is a young woman with black hair, up to her armpits in the sand.  "<i>Oh, thank the Gods!</i>" she cries, spotting you.  "<i>I\'m sinking, please help me!</i>"  The word \'quicksand\' flashes through your mind like fire and you take a nervous step back, attempting to quickly evaluate the situation.' );
				if( CoC.player.inte > 65 ) {
					MainView.outputText( '  Something about this scene puts you on edge.  How come you\'ve never encountered quicksand in this desert before, with no water and its steady, stubborn, but not violent winds?  There\'s something odd about the woman, too - although she can\'t be a sand witch with that hair, you can\'t quite make her features out clearly through the heat haze.' );
				}
				MainView.outputText( '  You think you could just about reach her with your hand if you were careful.  If you\'re going to save her, it has to be now; even in the short time you\'ve been here she has sunk down to her collarbone.  She stares at you with plaintive despair.' );
			} else {
				MainView.outputText( 'You are exploring the endlessly undulating landscape of the desert when you hear an agonised cry for help over the nearest dune.  You quickly scramble up it and are confronted by the sight of a small, flat lake of exceptionally fine sand, overlooked on each side by tall dunes.  Trapped in the middle of it is a young man with black hair, up to his armpits in the sand.  "<i>Oh, thank the Gods!</i>" he cries, spotting you.  "<i>I\'m sinking, please help me!</i>"  The word \'quicksand\' flashes through your mind like fire and you take a nervous step back, attempting to quickly evaluate the situation.  ' );
				if( CoC.player.inte > 65 ) {
			...
			//Speed check fail:
				...
				SceneLib.combatScene.startCombat( new SandTrap() );
	...
	SandTrapScene.prototype.saveTheSandTarps = function() {
		...
		//(femininity <= 65):
		if( CoC.player.femininity <= 65 ) {
			MainView.outputText( 'You carefully kneel at the edge of the quicksand and stretch your hand out towards the stricken woman, commanding her to grab hold.   With effort she pulls an arm clear of the treacherous powder and, straining, reaches out to grab the very tips of your fingers... before, with a triumphant sneer, pulling as hard as she can.  Thrown off-balance, you fall face first into the quicksand.  Coughing, you right yourself and struggle desperately to get back up the side, but you have no leverage, and your thrashing causes you to sink waist deep into what feels like dry, impossibly heavy water.' );
		...
		//(else fem > 65):
			...
			MainView.outputText( 'You carefully kneel at the edge of the quicksand and stretch your hand out towards the stricken man, commanding him to grab hold.   With effort he pulls an arm clear of the treacherous powder and, straining, reaches out to grab the very tips of your fingers... before, with a triumphant sneer, pulling as hard as he can.  Thrown off-balance, you fall face first into the quicksand.  Coughing, you right yourself and struggle desperately to get back up the side, but you have no leverage, and your thrashing causes you to sink waist deep into what feels like dry, impossibly heavy water.' );
	...
	SandTrapScene.prototype.dontSaveTheTarps = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 97 );
		MainView.outputText( 'You carefully step backwards from the quicksand, not taking your eyes off the stricken ' );
		if( CoC.player.femininity <= 65 ) {
			MainView.outputText( 'woman' );
		} else {
			MainView.outputText( 'man' );
		}
		MainView.outputText( '.  "<i>Please!</i>" ' );
		if( CoC.player.femininity <= 65 ) {
			MainView.outputText( 'she' );
		} else {
			MainView.outputText( 'he' );
		}
		MainView.outputText( ' cries, neck sinking into the drift.  "<i>Don\'t let me die like this... oh, very well.  Have it YOUR way!</i>"  The ' );
		if( CoC.player.femininity <= 65 ) {
			MainView.outputText( 'woman' );
		} else {
			MainView.outputText( 'man' );
		}
		MainView.outputText( ' suddenly thrusts upwards and plunges a fist into the fluid sand surrounding ' );
		if( CoC.player.femininity <= 65 ) {
			MainView.outputText( 'her' );
		} else {
			MainView.outputText( 'him' );
		}
		MainView.outputText( '.  The dune you are standing on caves inwards, turning into a shifting slide to the morass below you.  You stumble and hop backwards as fast as you can, managing to stay just ahead of the ' );
		if( CoC.player.femininity <= 65 ) {
			MainView.outputText( 'woman' );
		} else {
			MainView.outputText( 'man' );
		}
		MainView.outputText( '\'s spell.  In front of you, sand collapses downwards into a great conical depression, at the epicentre of which is your supposed rescuee.  With ' );
		if( CoC.player.femininity <= 65 ) {
			MainView.outputText( 'her' );
		} else {
			MainView.outputText( 'his' );
		}
		MainView.outputText( ' midriff exposed and glamour gone, the creature looks quite different; six eyes as black as its hair look at you hungrily from its beautiful androgynous face, whilst four slender arms trail patterns in the sand around its willowy, flat-chested midsection.  It wriggles its body tauntingly at you, making the sand beneath it move ponderously.  You glimpse insect chitin beneath its flat humanoid belly; the buried half of this creature must be monstrous.' );
	...
	SandTrapScene.prototype.pcBeatsATrap = function() {
		...
		//PC HP victory:
		if( CoC.monster.HP < 1 ) {
			MainView.outputText( 'The sandtrap groans and collapses forwards into the dirt.  With the creature no longer controlling it, the sand underneath you feels a great deal more stable... climbing out won\'t be too difficult.' );
		}//PC lust victory:
		else {
			MainView.outputText( 'The sand around you stops sinking.  Overpowered with lust, the sandtrap moans and gives up control over itself and its pit to desperately run one set of hands over its flat chest whilst the other pushes into the sand to masturbate... whatever it has down there.  With your foe in this state, climbing out won\'t be so difficult now.' );
		}
		...
		if( CoC.player.hasCock() && CoC.player.str >= 60 ) {
		...
		if( CoC.player.lust >= 33 ) {
			...
			EngineCore.choices( 'Naga3Some', this, nagaThreeSome, 'UseYourCock', this, putYourDickInIt, 'RideVaginal', this, rideDatSantTrap, 'Handjob', this, useSandTarpsHand, '', null, null,
				'', null, null, '', null, null, '', null, null, 'B.Titfuck', null, bikiniTits, 'Leave', SceneLib.combatScene, SceneLib.combatScene.cleanupAfterCombat );
		} else {
	...
	SandTrapScene.prototype.dickwieldersLoseToSandTarps = function() {
		...
		else if( CoC.player.cocks[ 0 ].cockLength < 15 ) {
		...
		if( CoC.player.cumQ() > 1000 ) {
			MainView.outputText( '  The sandtrap gasps and coos as you ejaculate over and over again, and beneath you feel your cum wet the sands as it beads around the clenching grip it has you in.' );
		}
		if( CoC.player.hasVagina() ) {
			MainView.outputText( '  Ignored entirely, your ' + Descriptors.vaginaDescript( 0 ) + ' quivers and orgasms in tandem, flexing and wetting fruitlessly against the dry water pressing against it.' );
		}
		...
		if( !CoC.player.isTaur() ) {
			MainView.outputText( 'armpits' );
		} else {
			MainView.outputText( 'forelegs' );
		}
		MainView.outputText( '.  You try and turn your head to see what it is doing but it gently but insistently pushes your face into the yielding sand.  "<i>You arrrrhe a fine little drone.  I wwill thank you for yourrh hard work... with the fruits of ssame hard work.</i>" What does it mean by...?' );
		...
		MainView.outputText( '  The creature pushes your head down, making soothing shushing noises as it feeds more of its bulbous length into your ass.' );
		if( CoC.player.analCapacity() < 40 ) {
			MainView.outputText( '  Although the oil which coats your thoughts and reflexes with sensual supplication makes things easier, the feeling is still incredibly intense, and you grit your teeth as something round travels down the creature\'s ovipositor and stretches your tight hole wide.' );
		} else {
			MainView.outputText( '  Your accommodating ass feels like it was made for this purpose, and with the creature\'s oil soothing your thoughts and reflexes you savor the creature taking you like this as if it were your quim.  You moan as something round travels down the creature\'s ovipositor and into your wet ass, the sensation making [eachCock] hard again.' );
		}
		...
		if( CoC.player.hasVagina() ) {
			MainView.outputText( ' and cunt' );
		}
		MainView.outputText( ' flexing out more moisture into the sand beneath you.  The oil-induced serenity and the sexual marathon the sandtrap has put you through are too much and, even with the creature still relentlessly pumping away at you, you pass out.' );
		...
		CoC.monster.createStatusAffect( StatusAffects.Fertilized, 0, 0, 0, 0 );
	...
	SandTrapScene.prototype.chicksLoseToSandTarp = function() {
		...
		MainView.outputText( '  The prong base rubs against your [clit] rhythmically and suddenly you find yourself orgasming in tandem with it, crying out as the creature groans and ejaculates its delicious ooze deliriously into you.' );
		...
		if( CoC.player.analCapacity() < 30 ) {
			MainView.outputText( 'Although the oil which coats your thoughts and reflexes with sensual supplication makes things easier, the feeling is still incredibly intense, and you grit your teeth as something round travels down the creature\'s ovipositor and stretches your tight ass wide.' );
		} else {
			MainView.outputText( 'Your accommodating ass feels like it was made for this purpose, and with the creature\'s oil soothing your thoughts and reflexes you savor the creature taking you like this as if it were your second quim.  You moan as something round travels down the creature\'s ovipositor and into your wet ass.' );
		}
		MainView.outputText( '  Slowly the creature begins to pick up its rhythm again, sliding in and out of your pussy as it breeds you.' );
	...
	SandTrapScene.prototype.genderlessLoss = function() {
		...
		MainView.outputText( '  Slowly the creature begins to pick up its rhythm again, sliding in and out of your butt as it breeds you.' );
	...
	SandTrapScene.prototype.birfSandTarps = function() {
		...
		if( CoC.player.gender === 1 ) {
			MainView.outputText( '  You roll your eyes to the sky and moan, [eachCock] growing hard as you push out the egg.' );
			//[(mans and qualified horses only)]
			if( !CoC.player.isTaur() || (CoC.player.tallness * (5 / 6) < CoC.player.cocks[ CoC.player.longestCock() ].cockLength) ) {
				MainView.outputText( '  Your oily hands descend upon your cock, and you massage your shaft as you feel the pressure in your bowels intensify again.' );
			}
		}
		//Female:
		else if( CoC.player.gender === 2 ) {
			MainView.outputText( '  You roll your eyes to the sky and moan, your ' + Descriptors.vaginaDescript( 0 ) + ' moistening as you push out the egg.' );
			//[(no fukken horses from here)]
			if( !CoC.player.isTaur() ) {
				MainView.outputText( '  Your oily hands push softly into your cleft, fingering your needy ' + Descriptors.clitDescript() + ' as you feel the pressure in your bowels intensify again.' );
			}
		}
		//Herm:
		else if( CoC.player.gender === 3 ) {
			MainView.outputText( '  You roll your eyes to the sky and moan, [eachCock] growing hard and your [vagina] moistening as you push out the egg.' );
			//[(no horses)]
			if( !CoC.player.isTaur() ) {
				MainView.outputText( '  Your oily hands descend upon your genitals and you begin to slowly pump your shaft and finger your needy ' + Descriptors.clitDescript() + ' as you feel the pressure in your bowels intensify again.' );
			}
		}
	...
	SandTrapScene.prototype.useSandTarpsHands = function() {
		...
		if( CoC.player.gender === 1 ) {
			...
			if( CoC.player.cocks[ 0 ].cockLength < 6 ) {
				...
				MainView.outputText( '  Your small size doesn\'t seem to bother it; if anything, judging from the red rising in its thin cheeks and its shortening breath, the girliness of your cock actually seems to excite it.  It pauses and, whilst you are both panting lightly, it slowly and gently circles your head with its index finger whilst it still holds the entirety of your cock with the rest of its hand.' );
			}
			//Cock 6-16 inches:
			...
			if( CoC.player.cockTotal() === 1 ) {
				MainView.outputText( '  The sandtrap pumps you faster and faster until its slippery hands are a blur of motion.  It clutches onto your butt cheeks so tightly you hazily realize the indents of its slender digits will be with you for hours afterwards.' );
			...
			//Three cocks:
				...
				if( CoC.player.balls > 0 ) {
					MainView.outputText( '  It grins up at you shyly and, with its final hand, cups your [balls].  ]You laugh with an almost hysterical edge to the sky at how mindboggling getting a triple handjob feels, and the trap giggles slightly as well despite itself.' );
				}
		...
		//Female:
			...
			if( CoC.player.tone < 60 ) {
				MainView.outputText( '  It picks up the pace gradually and soon it is doubly frigging you as fast as its fingers will allow, its other hands sinking deep into your soft, pillowy behind as it clutches onto you.' );
			} else {
				MainView.outputText( '  It picks up the pace gradually and soon it is doubly frigging you as fast as its fingers will allow, its other hands clutching onto your tight, muscled behind so tightly you hazily realize the indents of its slender digits will be with you for hours afterwards.' );
			}
		...
		//Herm:
			...
			if( CoC.player.cocks[ 0 ].cockLength < 6 ) {
				...
				MainView.outputText( '  Your small size doesn\'t seem to bother it; if anything, judging from the red rising in its thin cheeks and its shortening breath, the girliness of your cock actually seems to excite it.  It pauses and, while you are both panting lightly, it slowly and gently circles your head with its index finger, still holding the entirety of your cock with the rest of its hand.' );
			}
			//Cock 6-16 inches:
			else if( CoC.player.cocks[ 0 ].cockLength < 16 ) {
				if( CoC.player.balls === 0 ) {
	...
	SandTrapScene.prototype.rideDatSandTarpLikeIts1999 = function() {
		...
		if( CoC.player.hasCock() ) {
			MainView.outputText( '  [EachCock] flexes in delicious concert with your quim, spurting jizz onto the trap\'s chest and face.' );
		}
	...
	SandTrapScene.prototype.stickWangInSandgina = function() {
		...
		if( CoC.player.balls > 0 ) {
			MainView.outputText( ', makes your ' + Descriptors.ballsDescriptLight() + ' swell in readiness,' );
		}
		MainView.outputText( ' and fills you with a deep, red urge; you grit your teeth in frustration at its essentially male upper half.  Its female half must be what is buried beneath the sand.  <i>Well,</i> says your dick.  <i>You\'ve come this far...</i>' );
		...
		//8 inches to 35:
		else if( CoC.player.cocks[ x ].cockLength < 35 ) {
			...
			if( CoC.player.balls > 0 ) {
				MainView.outputText( '  Your ' + Descriptors.ballsDescriptLight() + ' slap against the creature\'s wet opening.  ' );
			}
			MainView.outputText( '  Any coherence flies straight out of the sandtrap\'s head and it can do nothing but whine, buzz and moan as you begin to thrust in and out of it, penetrating it to its very core, the [cockHead] bumping against strange bulbous objects which give and push back against your sensitive end delightfully.' );
		}
	...
	SandTrapScene.prototype.nagaThreesomeWithSandTrap = function() {
		...
		if( CoC.player.hasCock() ) {
			...
			if( CoC.player.cocks[ 0 ].cockLength < 14 ) {
				...
				if( CoC.player.balls > 0 ) {
					MainView.outputText( 'slapping your ' + Descriptors.ballsDescriptLight() + ' into its nose, forcing your cock all the way to the top of its tight throat.' );
				}
			}
			//14 inches or more:
			else {
				...
				if( CoC.player.skinType === AppearanceDefs.SKIN_TYPE_PLAIN ) {
					MainView.outputText( 'beading sweat, ' );
				}
				MainView.outputText( 'pistoning into its submissive opening with fervor, ' );
				if( CoC.player.balls > 0 ) {
					MainView.outputText( 'your ' + Descriptors.ballsDescriptLight() + ' swinging heavily in front of the sandtrap\'s nose, ' );
				}
				MainView.outputText( 'pushing as much of your lubricated dick down the creature\'s gullet as you can.' );
			}
			...
			if( CoC.player.cocks[ 0 ].cockLength >= 14 ) {
				MainView.outputText( '  You\'re not sure which is the more pleasing sight; her, or the rhythmic bulging in the sandtrap\'s neck.' );
			}
			MainView.outputText( '  Mindlessly, the sandtrap\'s four arms clutch and stroke the warm scales which swaddle its upper body as the two of you continue to fuck it.' );
		...
		//Female:
			...
			if( CoC.player.cumQ() > 1000 ) {
				MainView.outputText( '  The long, sensuous tongue bath has pushed your body into overdrive and the waterfall you unleash is even more uncontrollable than the last; you wonder dizzily if you haven\'t just lost control of your bladder.  The hypnotised sandtrap gladly drinks what you give it, but it\'s far too much and soon it is coughing your girlcum back up, through its nose and back onto your still contracting ' + Descriptors.vaginaDescript( 0 ) + '.' );
			}
		}
	...
	SandTrapScene.prototype.sandTrapBadEnd = function() {
		...
		if( CoC.player.hasCock() ) {
			MainView.outputText( '[EachCock] hardens and your thin frame feels aflame, barely understood impulses deep within you begging you to walk down and submit to the Sandtrap\'s will.  ' );
		}//Female:
		else if( CoC.player.hasVagina() ) {
			MainView.outputText( 'Your [vagina] moistens and your thin frame feels aflame, barely understood impulses deep within you begging you to walk down and submit to the Sandtrap\'s will.  ' );
		}
	...
	SandTrapScene.prototype.sandTrapBadEndFight = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 97 );
		MainView.outputText( 'You shake yourself out of your fog, clench your [weapon] and grit your teeth.  You are your own person, dammit, and you will show this creature just what that means!  The Sandtrap slowly lowers its arms and looks at you with almost parental disappointment as you march into its pit with the obvious intention of beating the crap out of it.  "<i>You want to dance again, Flytrap?</i>" it sighs.  "<i>I would prefer that you look at yourself and accept what you are, but I will force you to accept it if that is what you wish.</i>"' );
	...
	SandTrapScene.prototype.leaveSandTrapBadEnd = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 97 );
		MainView.outputText( 'With some effort you break your stare with the Sandtrap, turn away and step back towards camp, resolving to leave this disturbing scenario with its disturbing thoughts behind.' );
	...
	SandTrapScene.prototype.desztiny = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 97 );
		MainView.outputText( 'The Sandtrap is right.  What your body and subconscious are telling you is so right.  At your acceptance of what the creature is saying warmth floods through your body; your mind bubbles as blood rushes towards your skin, making you feel incredibly sensitive, incredibly sexual.  ' );
		//Male:
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'Your cock strains upwards and you moan; feeling as sensitive as you do, you are both desperate to be touched and desperate not to be.  ' );
		}//Female:
		else if( CoC.player.hasVagina() ) {
			MainView.outputText( 'You feel but barely hear the drip of moisture on the sand, both your [asshole] and [vagina] beading with oily need.  ' );
		}
		MainView.outputText( 'It seems incredible, as you stumble slowly downwards, feeling every grain of sand underneath your receptive feet, that barely seconds ago you were even considering not doing this.  It dully occurs to you that there was a reason for that, that you had a purpose.  What is your purpose? You feel like you are shedding whatever that was like an old skin as you walk downwards, discarding an old, directionless life, becoming something newer and raw and purposeful.  You practically throw yourself into the waiting embrace of the Sandtrap, moaning softly as you press your hands against its supple body and feel its quartet of hands slowly explore your back, your nape, your thin wings, your [butt].  You feel like you\'ve come home.' );
		MainView.outputText( '\n\n"<i>Good ' + CoC.player.mf( 'boy', 'girl' ) + ',</i>" says the Sandtrap, giggling slightly as it caresses you.  "<i>Of course, that word doesn\'t mean anything to you anymore.</i>"  You blearily look into its thin, beautiful face to ask what it means by that, and the creature takes the opportunity to plunge its tongue into your unresisting mouth.' );
	...
	SandTrapScene.prototype.sandTrapBadEndFinale = function() {
		...
		if( CoC.player.gender > 0 ) {
			...
			if( CoC.player.hasCock() ) {
				MainView.outputText( 'cock harden' );
			} else {
				MainView.outputText( 'vagina dribble' );
			}
		}
		MainView.outputText( '.  Yes.  You remember your quest.' );
		...
		if( CoC.player.hasCock() ) {
			MainView.outputText( ' and masturbating [oneCock] desperately' );
		} else if( CoC.player.hasVagina() ) {
			MainView.outputText( ' and fingering your [vagina] desperately' );
		}
		MainView.outputText( '.  But, of course, as a fresh dominant urge takes you and drags you reluctantly from your needy genitals, there is much still left to be done.' );
