...
angular.module( 'cocjs' ).run( function( CoC, Utils, StatusAffects, EngineCore, AppearanceDefs, Descriptors, EventParser, PregnancyStore, kFLAGS, Combat, SandTrap ) {
	...
	SandTrapScene.prototype.encounterASandTarp = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.SANDTRAP_LOSS_REPEATS ] >= 2 && CoC.getInstance().player.eyeType === AppearanceDefs.EYES_BLACK_EYES_SAND_TRAP && CoC.getInstance().player.wingType === AppearanceDefs.WING_TYPE_GIANT_DRAGONFLY ) {
			this.sandTrapBadEnd();
			return;
		}
		CoC.getInstance().flags[ kFLAGS.TIMES_ENCOUNTERED_SAND_TRAPS ]++;
		if( CoC.getInstance().flags[ kFLAGS.TIMES_ENCOUNTERED_SAND_TRAPS ] === 1 ) {
			//femininity <= 65:
			if( CoC.getInstance().player.femininity <= 65 ) {
				EngineCore.outputText( 'You are exploring the endlessly undulating landscape of the desert when you hear an agonized cry for help over the nearest dune.  You quickly scramble up it and are confronted by the sight of a small, flat lake of exceptionally fine sand, overlooked on each side by tall dunes.  Trapped in the middle of it is a young woman with black hair, up to her armpits in the sand.  "<i>Oh, thank the Gods!</i>" she cries, spotting you.  "<i>I\'m sinking, please help me!</i>"  The word \'quicksand\' flashes through your mind like fire and you take a nervous step back, attempting to quickly evaluate the situation.' );
				if( CoC.getInstance().player.inte > 65 ) {
					EngineCore.outputText( '  Something about this scene puts you on edge.  How come you\'ve never encountered quicksand in this desert before, with no water and its steady, stubborn, but not violent winds?  There\'s something odd about the woman, too - although she can\'t be a sand witch with that hair, you can\'t quite make her features out clearly through the heat haze.' );
				}
				EngineCore.outputText( '  You think you could just about reach her with your hand if you were careful.  If you\'re going to save her, it has to be now; even in the short time you\'ve been here she has sunk down to her collarbone.  She stares at you with plaintive despair.' );
			} else {
				EngineCore.outputText( 'You are exploring the endlessly undulating landscape of the desert when you hear an agonised cry for help over the nearest dune.  You quickly scramble up it and are confronted by the sight of a small, flat lake of exceptionally fine sand, overlooked on each side by tall dunes.  Trapped in the middle of it is a young man with black hair, up to his armpits in the sand.  "<i>Oh, thank the Gods!</i>" he cries, spotting you.  "<i>I\'m sinking, please help me!</i>"  The word \'quicksand\' flashes through your mind like fire and you take a nervous step back, attempting to quickly evaluate the situation.  ' );
				if( CoC.getInstance().player.inte > 65 ) {
			...
			if( CoC.getInstance().player.spe / 10 + Utils.rand( 20 ) >= 16 ) {
	...
	SandTrapScene.prototype.saveTheSandTarps = function() {
		...
		//(femininity <= 65):
		if( CoC.getInstance().player.femininity <= 65 ) {
			EngineCore.outputText( 'You carefully kneel at the edge of the quicksand and stretch your hand out towards the stricken woman, commanding her to grab hold.   With effort she pulls an arm clear of the treacherous powder and, straining, reaches out to grab the very tips of your fingers... before, with a triumphant sneer, pulling as hard as she can.  Thrown off-balance, you fall face first into the quicksand.  Coughing, you right yourself and struggle desperately to get back up the side, but you have no leverage, and your thrashing causes you to sink waist deep into what feels like dry, impossibly heavy water.' );
		...
		//(else fem > 65):
			...
			EngineCore.outputText( 'You carefully kneel at the edge of the quicksand and stretch your hand out towards the stricken man, commanding him to grab hold.   With effort he pulls an arm clear of the treacherous powder and, straining, reaches out to grab the very tips of your fingers... before, with a triumphant sneer, pulling as hard as he can.  Thrown off-balance, you fall face first into the quicksand.  Coughing, you right yourself and struggle desperately to get back up the side, but you have no leverage, and your thrashing causes you to sink waist deep into what feels like dry, impossibly heavy water.' );
		...
		Combat.startCombat( new SandTrap() );
		CoC.getInstance().monster.changeStatusValue( StatusAffects.Level, 1, 2 );
		EngineCore.doNext( EventParser.playerMenu );
	};
	//[Don't Save]:
	SandTrapScene.prototype.dontSaveTheTarps = function() {
		...
		EngineCore.outputText( '.  "<i>Please!</i>" ' );
		if( CoC.getInstance().player.femininity <= 65 ) {
			EngineCore.outputText( 'she' );
		} else {
			EngineCore.outputText( 'he' );
		}
		EngineCore.outputText( ' cries, neck sinking into the drift.  "<i>Don\'t let me die like this... oh, very well.  Have it YOUR way!</i>"  The ' );
		if( CoC.getInstance().player.femininity <= 65 ) {
			EngineCore.outputText( 'woman' );
		} else {
			EngineCore.outputText( 'man' );
		}
		EngineCore.outputText( ' suddenly thrusts upwards and plunges a fist into the fluid sand surrounding ' );
		if( CoC.getInstance().player.femininity <= 65 ) {
			EngineCore.outputText( 'her' );
		} else {
			EngineCore.outputText( 'him' );
		}
		EngineCore.outputText( '.  The dune you are standing on caves inwards, turning into a shifting slide to the morass below you.  You stumble and hop backwards as fast as you can, managing to stay just ahead of the ' );
		if( CoC.getInstance().player.femininity <= 65 ) {
			EngineCore.outputText( 'woman' );
		} else {
			EngineCore.outputText( 'man' );
		}
		EngineCore.outputText( '\'s spell.  In front of you, sand collapses downwards into a great conical depression, at the epicentre of which is your supposed rescuee.  With ' );
		if( CoC.getInstance().player.femininity <= 65 ) {
			EngineCore.outputText( 'her' );
		} else {
	...
	SandTrapScene.prototype.sandtrapmentLoss = function( clear ) {
		if( CoC.getInstance().flags[ kFLAGS.SANDTRAP_LOSS_REPEATS ] >= 2 && CoC.getInstance().player.eyeType === AppearanceDefs.EYES_BLACK_EYES_SAND_TRAP && CoC.getInstance().player.wingType === AppearanceDefs.WING_TYPE_GIANT_DRAGONFLY ) {
			this.loseLastFightWithSandTrap();
			return;
		}
		CoC.getInstance().flags[ kFLAGS.SANDTRAP_LOSS_REPEATS ]++;
		if( clear ) {
			EngineCore.clearOutput();
		} else {
			EngineCore.outputText( '\n\n' );
	...
	SandTrapScene.prototype.pcBeatsATrap = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 97 );
		CoC.getInstance().flags[ kFLAGS.SANDTRAP_LOSS_REPEATS ] = 0;
		//PC HP victory:
		if( CoC.getInstance().monster.HP < 1 ) {
			EngineCore.outputText( 'The sandtrap groans and collapses forwards into the dirt.  With the creature no longer controlling it, the sand underneath you feels a great deal more stable... climbing out won\'t be too difficult.' );
		}//PC lust victory:
		else {
			EngineCore.outputText( 'The sand around you stops sinking.  Overpowered with lust, the sandtrap moans and gives up control over itself and its pit to desperately run one set of hands over its flat chest whilst the other pushes into the sand to masturbate... whatever it has down there.  With your foe in this state, climbing out won\'t be so difficult now.' );
		}
		var nagaThreeSome = null;
		var putYourDickInIt = null;
		var rideDatSantTrap = null;
		...
		if( CoC.getInstance().player.isNaga() && CoC.getInstance().player.findStatusAffect( StatusAffects.Naga ) >= 0 && CoC.getInstance().player.gender > 0 && CoC.getInstance().player.faceType === AppearanceDefs.FACE_SNAKE_FANGS ) {
			nagaThreeSome = this.nagaThreesomeWithSandTrap;
		}
		//Requires: Penis and str requirement
		if( CoC.getInstance().player.hasCock() && CoC.getInstance().player.str >= 60 ) {
	...
	SandTrapScene.prototype.dickwieldersLoseToSandTarps = function() {
		...
		else if( CoC.getInstance().player.cocks[ 0 ].cockLength < 15 ) {
		...
		if( !CoC.getInstance().player.isTaur() ) {
			EngineCore.outputText( 'armpits' );
		} else {
			EngineCore.outputText( 'forelegs' );
		}
		EngineCore.outputText( '.  You try and turn your head to see what it is doing but it gently but insistently pushes your face into the yielding sand.  "<i>You arrrrhe a fine little drone.  I wwill thank you for yourrh hard work... with the fruits of ssame hard work.</i>" What does it mean by...?' );
		...
		EngineCore.outputText( '  The creature pushes your head down, making soothing shushing noises as it feeds more of its bulbous length into your ass.' );
		if( CoC.getInstance().player.analCapacity() < 40 ) {
			EngineCore.outputText( '  Although the oil which coats your thoughts and reflexes with sensual supplication makes things easier, the feeling is still incredibly intense, and you grit your teeth as something round travels down the creature\'s ovipositor and stretches your tight hole wide.' );
		} else {
			EngineCore.outputText( '  Your accommodating ass feels like it was made for this purpose, and with the creature\'s oil soothing your thoughts and reflexes you savor the creature taking you like this as if it were your quim.  You moan as something round travels down the creature\'s ovipositor and into your wet ass, the sensation making [eachCock] hard again.' );
		}
		...
		if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( ' and cunt' );
		}
		EngineCore.outputText( ' flexing out more moisture into the sand beneath you.  The oil-induced serenity and the sexual marathon the sandtrap has put you through are too much and, even with the creature still relentlessly pumping away at you, you pass out.' );
		...
		CoC.getInstance().monster.createStatusAffect( StatusAffects.Fertilized, 0, 0, 0, 0 );
	...
	SandTrapScene.prototype.chicksLoseToSandTarp = function() {
		...
		if( CoC.getInstance().monster.findStatusAffect( StatusAffects.Fertilized ) >= 0 ) {
			EngineCore.outputText( '  "<i>A cute little wwworker hasz fallen into my home.  Wwwell, that isz ok.  Fortunately for hwyou, I already have some eggsz ready.  Hhyou will get to be a queen! Iszn\'t that exciting?</i>"' );
		} else {
			EngineCore.outputText( '  "<i>A cute little wwworker hasz fallen into my home, but I have no fertile eggsz to givve it.  Wwwell, that isz ok.  Hwwe can still have fun, can\'t we?</i>"' );
		}
		...
		EngineCore.outputText( '  The prong base rubs against your [clit] rhythmically and suddenly you find yourself orgasming in tandem with it, crying out as the creature groans and ejaculates its delicious ooze deliriously into you.' );
		...
		if( CoC.getInstance().player.analCapacity() < 30 ) {
			EngineCore.outputText( 'Although the oil which coats your thoughts and reflexes with sensual supplication makes things easier, the feeling is still incredibly intense, and you grit your teeth as something round travels down the creature\'s ovipositor and stretches your tight ass wide.' );
		} else {
			EngineCore.outputText( 'Your accommodating ass feels like it was made for this purpose, and with the creature\'s oil soothing your thoughts and reflexes you savor the creature taking you like this as if it were your second quim.  You moan as something round travels down the creature\'s ovipositor and into your wet ass.' );
		}
		EngineCore.outputText( '  Slowly the creature begins to pick up its rhythm again, sliding in and out of your pussy as it breeds you.' );
	...
	SandTrapScene.prototype.genderlessLoss = function() {
		...
		if( CoC.getInstance().monster.findStatusAffect( StatusAffects.Fertilized ) >= 0 ) {
			EngineCore.outputText( '"<i>Not a worker OR a drone! Wwwell, that isz ok.  Even ants who hhhave nothing can be someone for somebody.  And hhyou can be mommy for my childrrhen!</i>"' );
		} else {
			EngineCore.outputText( '"<i>Not a worker OR a drone! Wwwell, that isz ok.  Hwwe can still have fun, can\'t we?</i>"' );
		}
		...
		EngineCore.outputText( '  Slowly the creature begins to pick up its rhythm again, sliding in and out of your butt as it breeds you.' );
	...
	SandTrapScene.prototype.sandTrapPregChance = function() {
		...
		if( CoC.getInstance().monster.findStatusAffect( StatusAffects.Fertilized ) >= 0 ) {
			CoC.getInstance().player.buttKnockUp( PregnancyStore.PREGNANCY_SANDTRAP_FERTILE, PregnancyStore.INCUBATION_SANDTRAP, 1, 1 );
		} else {
			CoC.getInstance().player.buttKnockUp( PregnancyStore.PREGNANCY_SANDTRAP, PregnancyStore.INCUBATION_SANDTRAP, 1, 1 );
		}
	};
	SandTrapScene.prototype.birfSandTarps = function() {
		EngineCore.spriteSelect( 97 );
		EngineCore.outputText( '\nYour eyes widen as a gout of oil suddenly gushes from your ass.  Before panic can set in, an incredible light-headedness overtakes you.  Dreamily, you discard your [armor] and squat.  More oil oozes out of you, and in your hazy euphoria, you scoop some of it up and rub it dreamily into your ' + Descriptors.nippleDescript( 0 ) + 's.  Part of you is disgusted at yourself, questioning what you are doing, but that is one voice in a million-strong chorus crooning you into total relaxation... the oil clings to your skin and seems to radiate warmth and softness.  Something round stretches your rectum wide, but in your state the sensation is practically orgasmic.' );
		//[Male:
		if( CoC.getInstance().player.gender === 1 ) {
			EngineCore.outputText( '  You roll your eyes to the sky and moan, [eachCock] growing hard as you push out the egg.' );
			//[(mans and qualified horses only)]
			if( !CoC.getInstance().player.isTaur() || (CoC.getInstance().player.tallness * (5 / 6) < CoC.getInstance().player.cocks[ CoC.getInstance().player.longestCock() ].cockLength) ) {
				EngineCore.outputText( '  Your oily hands descend upon your cock, and you massage your shaft as you feel the pressure in your bowels intensify again.' );
			}
		}
		//Female:
		else if( CoC.getInstance().player.gender === 2 ) {
			EngineCore.outputText( '  You roll your eyes to the sky and moan, your ' + Descriptors.vaginaDescript( 0 ) + ' moistening as you push out the egg.' );
			//[(no fukken horses from here)]
			if( !CoC.getInstance().player.isTaur() ) {
				EngineCore.outputText( '  Your oily hands push softly into your cleft, fingering your needy ' + Descriptors.clitDescript() + ' as you feel the pressure in your bowels intensify again.' );
			}
		}
		//Herm:
		else if( CoC.getInstance().player.gender === 3 ) {
			EngineCore.outputText( '  You roll your eyes to the sky and moan, [eachCock] growing hard and your [vagina] moistening as you push out the egg.' );
			//[(no horses)]
			if( !CoC.getInstance().player.isTaur() ) {
				EngineCore.outputText( '  Your oily hands descend upon your genitals and you begin to slowly pump your shaft and finger your needy ' + Descriptors.clitDescript() + ' as you feel the pressure in your bowels intensify again.' );
			}
		}
	...
	SandTrapScene.prototype.useSandTarpsHands = function() {
		...
		if( CoC.getInstance().player.gender === 1 ) {
			...
			if( CoC.getInstance().player.cocks[ 0 ].cockLength < 6 ) {
				...
				if( CoC.getInstance().player.balls === 0 ) {
					EngineCore.outputText( '\n\nThe sandtrap only needs one hand to pump your diminutive cock; it barely moves its wrist as it jerks you, occasionally wriggling its oily fingers along the bottom of your ' + Descriptors.cockDescript( 0 ) + ' to stimulate you even further.' );
				} else {
					EngineCore.outputText( '\n\nThe sandtrap only needs one hand to pump your diminutive cock; it barely moves its wrist as it jerks you, occasionally wriggling its oily fingers along the bottom of your ' + Descriptors.cockDescript( 0 ) + ' to incite you even further, whilst it uses its free hand to gently cup your [balls].' );
				}
				EngineCore.outputText( '  Your small size doesn\'t seem to bother it; if anything, judging from the red rising in its thin cheeks and its shortening breath, the girliness of your cock actually seems to excite it.  It pauses and, whilst you are both panting lightly, it slowly and gently circles your head with its index finger whilst it still holds the entirety of your cock with the rest of its hand.' );
			}
			//Cock 6-16 inches:
			else if( CoC.getInstance().player.cocks[ 0 ].cockLength < 16 ) {
				//[no balls:
				if( CoC.getInstance().player.balls === 0 ) {
					EngineCore.outputText( '\n\nThe sandtrap glides its hand up and down your ' + Descriptors.cockDescript( 0 ) + ', picking up a warm smooth rhythm.' );
				}//Balls:
				else {
					EngineCore.outputText( '\n\nThe sandtrap glides its hand up and down your ' + Descriptors.cockDescript( 0 ) + ', picking up a warm smooth rhythm as it gently cups your [balls] with its free hand.]  It slows down intermittently to move its palm around your length, letting every inch of your dick feel the tips of its delicate fingers.' );
				}
			}
			//Cock more than 16 inches:
			else {
				EngineCore.outputText( '\n\nThe sandtrap quickly comes to the realisation it needs both hands for this.  Still cupping your behind, it moves one oily hand up the top end of your massive shaft, while the other works the other half.  You sigh with deep satisfaction, enjoying the all-too-rare sensation of having every inch of your ' + Descriptors.cockDescript( 0 ) + ' immersed in pleasure.  The creature screws its face up and looks away in distaste at the size of your straining dick, which is in danger of poking it in the nose.  As you enjoy the warm, slippery sensation inundating you, you find you couldn\'t care less what it thinks.' );
			}
			//number of cock fork
			//One cock:
			if( CoC.getInstance().player.cockTotal() === 1 ) {
				EngineCore.outputText( '  The sandtrap pumps you faster and faster until its slippery hands are a blur of motion.  It clutches onto your butt cheeks so tightly you hazily realize the indents of its slender digits will be with you for hours afterwards.' );
			...
			//Two cocks:
				...
				EngineCore.outputText( '  As the sandtrap begins to pump you faster, your disregarded ' + Descriptors.cockDescript( 1 ) + ' bobs in hard, eager concert with your ' + Descriptors.cockDescript( 0 ) + '.  You gasp as the trap slows down momentarily to grasp your second sex.  It is clumsy at first as it moves its lubricated palms in tandem, then with increasing confidence as it finds a position and rhythm that suits it, until it is jerking you off furiously, clutching your butt cheeks so tightly you hazily realize the indents of its slender digits will be with you for hours afterwards.' );
				EngineCore.outputText( '\n\nThe warm, oily double sensation is unbearably good and although you try to hold on to savor it as long as possible, it isn\'t long before you reach a toe-curling peak and, with a wordless bark, spurt out line after line of jizz.  The trap flinches slightly when your first cumshots hit it in the face and chest, but then goes back to gently rubbing you, guiding you up and down your warm, slippery orgasm.' );
			}
			//Three cocks:
			else if( CoC.getInstance().player.cockTotal() === 3 ) {
				EngineCore.outputText( '\n\nAs the sandtrap begins to pump you faster, your two other, disregarded cocks bob in hard, eager concert with your ' + Descriptors.cockDescript( 0 ) + '.  You gasp as the trap slows down momentarily to reach one of its hands down to grasp your second sex, and then stops entirely as  one of its hands clutching your ass releases and snakes back around to take hold of your ' + Descriptors.cockDescript( 2 ) + '.  It is clumsy at first as it works its way around the rather complex arrangement you have down below, but it moves its lubricated palms with increasing confidence, until it is jerking all three of your cocks in rhythm.' );
				if( CoC.getInstance().player.balls > 0 ) {
					EngineCore.outputText( '  It grins up at you shyly and, with its final hand, cups your [balls].  ]You laugh with an almost hysterical edge to the sky at how mindboggling getting a triple handjob feels, and the trap giggles slightly as well despite itself.' );
				}
			...
			//Four cocks:
			else if( CoC.getInstance().player.cockTotal() === 4 ) {
				EngineCore.outputText( '\n\nAs the sandtrap begins to pump you faster, your three other, disregarded cocks bob in hard, eager concert with your ' + Descriptors.cockDescript( 0 ) + '.  You gasp as the trap slows down momentarily to reach one of its hands down to grasp your second sex, and then stops entirely as both of its hands clutching your ass release and snake back around to take hold of your ' + Descriptors.cockDescript( 2 ) + ' and ' + Descriptors.cockDescript( 3 ) + '.  It is clumsy at first as it works its way around the rather complex arrangement you have down below, but it moves its lubricated palms with increasing confidence, until it is jerking all four of your cocks in rhythm.  You laugh into the sky with an almost hysterical edge at how mindbogglingly good getting a quadruple handjob feels; it glances up at you and giggles shyly as you look into each other\'s eyes and share a thought.  "<i>At last, I\'ve found somebody who complements my body perfectly.</i>' );
		...
		//Female:
			...
			if( CoC.getInstance().player.tone < 60 ) {
				EngineCore.outputText( '  It picks up the pace gradually and soon it is doubly frigging you as fast as its fingers will allow, its other hands sinking deep into your soft, pillowy behind as it clutches onto you.' );
			} else {
				EngineCore.outputText( '  It picks up the pace gradually and soon it is doubly frigging you as fast as its fingers will allow, its other hands clutching onto your tight, muscled behind so tightly you hazily realize the indents of its slender digits will be with you for hours afterwards.' );
			}
		...
		//Herm:
			...
			if( CoC.getInstance().player.cocks[ 0 ].cockLength < 6 ) {
				...
				EngineCore.outputText( '  Your small size doesn\'t seem to bother it; if anything, judging from the red rising in its thin cheeks and its shortening breath, the girliness of your cock actually seems to excite it.  It pauses and, while you are both panting lightly, it slowly and gently circles your head with its index finger, still holding the entirety of your cock with the rest of its hand.' );
			}
			//Cock 6-16 inches:
			else if( CoC.getInstance().player.cocks[ 0 ].cockLength < 16 ) {
				if( CoC.getInstance().player.balls === 0 ) {
					...
					EngineCore.outputText( '\n\nThe sandtrap glides its hand up and down your ' + Descriptors.cockDescript( 0 ) + ', picking up a warm smooth rhythm as it gently cups your [balls] with its free hand.]  It slows down intermittently to move its fingers around your length, letting every inch of your dick feel the tips of its delicate fingers.' );
				}
			} else {
			...
			//More than three cocks:
				...
				EngineCore.outputText( '\n\nAs the sandtrap begins to pump you faster, your many other, disregarded cocks begin to harden in eager concert with your ' + Descriptors.cockDescript( 0 ) + '.  You gasp as the trap slows down momentarily to reach both of its hands clutching your ass back around to take hold of your ' + Descriptors.cockDescript( 1 ) + ' and ' + Descriptors.cockDescript( 2 ) + '.  It is clumsy at first as it works its way around the rather complex arrangement you have down below, but it moves its lubricated palms with increasing confidence, until it is jerking three of your cocks whilst fingering you in rhythm.  You laugh into the sky with an almost hysterical edge at how mindbogglingly good getting a quadruple handjob feels, and the trap giggles slightly as well despite itself.  Hazily you wonder if it is too much to wish that this creature, with its excellent wrist action, had just one or two more hands to its name.' );
	...
	SandTrapScene.prototype.rideDatSandTarpLikeIts1999 = function() {
		...
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( '  [EachCock] flexes in delicious concert with your quim, spurting jizz onto the trap\'s chest and face.' );
		}
	...
	SandTrapScene.prototype.stickWangInSandgina = function() {
		...
		EngineCore.outputText( '\n\nYou take a moment to look your strange conquest over.  Even up close you can\'t tell from its thin, fey beauty whether it is male or female.  Although it is affecting to look angry at you, its sculpted cheeks are quite flushed and there is a definite undercurrent of desire in its eyes; you suspect your display of strength and dominance appeals to it on some perverse, animalistic level.  Still smiling victoriously, you bend your face into it, deliberately invading its personal space, silently demonstrating who is in charge here.  It can\'t hold your gaze and, blushing, looks downwards.  This submissive demureness sends heat rushing towards your ' + Descriptors.multiCockDescriptLight() + '; the desire to fuck builds in you, to plough, to sink your cock into this strange, eerily beautiful creature, to establish once and for all who is giving and who is receiving around here... the only question is, how?  Its slim, androgyne upper body is devoid of obvious sexual features.  Your eyes roam steadily downwards.  Where its insect half meets its human half, where its cock should be, it instead has a genital slit; from this is emerging a long, black prong, dripping with clear oil.  Your show of dominance is turning this creature on, and it is powerless to disguise it.  It looks up at you with timid hopefulness... but you are in no mood for that.  There is a definite air of femininity to this creature, a pheromone scent which speaks directly to your ' + Descriptors.cockDescript( 0 ) + ', makes you hard' );
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( ', makes your ' + Descriptors.ballsDescriptLight() + ' swell in readiness,' );
		}
		EngineCore.outputText( ' and fills you with a deep, red urge; you grit your teeth in frustration at its essentially male upper half.  Its female half must be what is buried beneath the sand.  <i>Well,</i> says your dick.  <i>You\'ve come this far...</i>' );
		...
		//8 inches to 35:
		else if( CoC.getInstance().player.cocks[ x ].cockLength < 35 ) {
	...
	SandTrapScene.prototype.nagaThreesomeWithSandTrap = function() {
		...
		//First encounter:
		if( CoC.getInstance().flags[ kFLAGS.SANDTRAP_NAGA_3SOME ] === 0 ) {
		...
		//Female:
			...
			EngineCore.outputText( '\n\nThe naga is soon deep within the sandtrap\'s oil fuge and she slows her thrusting down as it relaxes her and amplifies what she can feel, smiling euphorically as she slowly rotates on the creature\'s leaking pseudo-cock.  You pace yourself in sympathy; having already reached one ecstatic high you\'re more than happy to simply sit back and enjoy the sandtrap\'s worshipful ministrations, tending to every corner of your ' + Descriptors.vaginaDescript( 0 ) + '.  You\'ve also had enough oil rubbed into you to feel quite dreamy yourself; the sand, the sun and even your own vagina seem bigger and more encompassing than they were before.' );
			...
			if( CoC.getInstance().player.cumQ() > 1000 ) {
				EngineCore.outputText( '  The long, sensuous tongue bath has pushed your body into overdrive and the waterfall you unleash is even more uncontrollable than the last; you wonder dizzily if you haven\'t just lost control of your bladder.  The hypnotised sandtrap gladly drinks what you give it, but it\'s far too much and soon it is coughing your girlcum back up, through its nose and back onto your still contracting ' + Descriptors.vaginaDescript( 0 ) + '.' );
			}
		}
	...
	SandTrapScene.prototype.sandTrapBadEnd = function() {
		...
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( '[EachCock] hardens and your thin frame feels aflame, barely understood impulses deep within you begging you to walk down and submit to the Sandtrap\'s will.  ' );
		}//Female:
		else if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( 'Your [vagina] moistens and your thin frame feels aflame, barely understood impulses deep within you begging you to walk down and submit to the Sandtrap\'s will.  ' );
		}
	...
	//Both go to:
	SandTrapScene.prototype.sandTrapBadEndFinale = function() {
		EngineCore.spriteSelect( 97 );
		...
		if( CoC.getInstance().player.gender > 0 ) {
			...
			if( CoC.getInstance().player.hasCock() ) {
				EngineCore.outputText( 'cock harden' );
			} else {
				EngineCore.outputText( 'vagina dribble' );
			}
		}
		EngineCore.outputText( '.  Yes.  You remember your quest.' );
		...
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( ' and masturbating [oneCock] desperately' );
		} else if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( ' and fingering your [vagina] desperately' );
		}
		EngineCore.outputText( '.  But, of course, as a fresh dominant urge takes you and drags you reluctantly from your needy genitals, there is much still left to be done.' );
