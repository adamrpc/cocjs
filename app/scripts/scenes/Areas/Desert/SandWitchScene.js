...
angular.module( 'cocjs' ).run( function( SceneLib, MainView, $rootScope, $log, CoC, Utils, StatusAffects, EngineCore, AppearanceDefs, Descriptors, PregnancyStore, kFLAGS, SandWitch, PerkLib ) {
	...
	SandWitchScene.prototype.encounter = function() {
		...
		if( CoC.player.statusAffectv1( StatusAffects.Exgartuan ) === 1 && CoC.player.cockArea( 0 ) > 100 && CoC.player.statusAffectv2( StatusAffects.Exgartuan ) === 0 ) {
			...
			MainView.outputText( 'The sandwitch ' );
			if( CoC.player.cor < 50 ) {
				MainView.outputText( 'and you both turn crimson', false );
			} else {
	...
	SandWitchScene.prototype.allowSandWitchMagic = function() {
		...
		if( CoC.player.hairColor === 'sandy blonde' ) {
			...
			if( CoC.player.breastRows[ 0 ].nipplesPerBreast < 1 ) {
				MainView.outputText( 'Two dark spots appear on your chest, rapidly forming into sensitive nipples.  ', false );
				CoC.player.breastRows[ 0 ].nipplesPerBreast = 1;
				EngineCore.dynStats( 'sen', 2, 'lus', 1 );
			}
			if( CoC.player.biggestLactation() > 0 ) {
				...
				if( CoC.player.biggestLactation() < 2 ) {
					MainView.outputText( 'powerful jets of milk spray from your nipples, spraying thick streams over the desert sands.  You moan at the sensation and squeeze your tits, hosing down the tainted earth with an offering of your milk.  You blush as the milk ends, quite embarassed with your increased milk production.  ', false );
				}
			...
			if( CoC.player.biggestLactation() === 0 ) {
				MainView.outputText( 'A pleasurable release suddenly erupts from your nipples!  Streams of milk are spraying from your breasts, soaking into the sand immediately.  It stops all too soon, though the witch assures you that you can lactate quite often now.  ', false );
				CoC.player.boostLactation( 1 );
				EngineCore.dynStats( 'lib', 0.5, 'sen', 1, 'lus', 10 );
			}
			MainView.outputText( 'The sand-witch smiles and thanks you for your offering.  You notice her dress is damp in four spots on the front.  ', false );
			if( CoC.sand === 0 ) {
				MainView.outputText( 'You wonder at what her robes conceal as she vanishes into the dunes.', false );
			}
			if( CoC.sand === 1 ) {
				if( CoC.player.cor <= 33 ) {
					MainView.outputText( 'You are glad to avoid servicing her again as she vanishes into the dunes.', false );
				} else if( CoC.player.cor <= 66 ) {
					MainView.outputText( 'You wonder if you should\'ve resisted and tried for some sex as she departs.', false );
				} else {
					MainView.outputText( 'You wish you had said no, so you could fuck with her and her magnificent quartet of breasts some more.', false );
				}
			}
		} else {
			MainView.outputText( 'She smiles wickedly and intones, "<i>nuf erutuf rof riah ydnas, nus tresed eht sa ydnas.</i>"\n\nYou feel a tingling in your scalp, and realize your hair has become a sandy blonde!', false );
	...
	SandWitchScene.prototype.sandwitchRaped = function() {
			...
			//Really corrupt
				...
				if( CoC.player.cockTotal() === 1 ) {
					...
					if( CoC.player.dogCocks() > 0 && CoC.player.cocks[ 0 ].knotMultiplier > 1.4 ) {
						...
						MainView.outputText( 'You place your ' + CoC.player.foot() + ' in the wet sand, pulling her hair and making her look you in the eyes. The sand witch looks shocked at your display of power, only turning you on more. You undo your ' + CoC.player.armorName + ' and grope her nethers, only to find a surprise. She has ' + Utils.num2Text( CoC.player.cocks.length + 1 ) + ' pussies and each seems to be made to fit you exactly. Pushing her to the ground, you bring the seeping head of your ' + Descriptors.cockDescript( 0 ) + ' to one of her nethers, slowly easing into her depths.  ', false );
						MainView.outputText( 'Thrusting deep, you hit bottom, and marvel at the sensations as you feel the head of your ' + Descriptors.cockDescript( 0 ) + ' rub against the end of her slick cunts, only making you lust after her more. As though responding to your will, your ' + Descriptors.cockDescript( 0 ) + ' grows just a tiny bit more in length and thickness.  ', false );
						MainView.outputText( 'With a satisfied growl of lust from yourself, and a screech of pain from the sand witch, you break past her cervix and into her womb. Pulling out, you rub yourself against her wet outer lips, bringing yourself closer and closer to your peak as the sand witch struggles, her eyes turning wide with fright as she starts to understand you hold more power than her.  ', false );
						MainView.outputText( 'With a bestial cry of lust, you shove your shaft deep within one of her vaginas, past her cervix and into her womb where you blast your potent, corrupted, demon-tainted seed.  ', false );
						...
						MainView.outputText( 'Finally finished, you pull your ' + Descriptors.cockDescript( 0 ) + ' from one of her well-used slits, your ' + Descriptors.cockDescript( 0 ) + ' drooling cum even now. With a grin, you walk away, happy with your little encounter.  ', false );
						CoC.player.orgasm();
					}
				}
				//Multiprick
				if( CoC.player.cockTotal() > 1 ) {
					...
					MainView.outputText( 'Using the corruption in your blood, you place your ' + CoC.player.foot() + ' upon the budding shoots, and wipe them from existence. The sand witch looks shocked at your display of power, only turning you on more. You undo your loin cloth and grope her nethers, only to find a surprise. She has ' + Utils.num2Text( CoC.player.cocks.length + 1 ) + ' loose wet cunts and each seems to be made to fit you exactly. Pushing her to the ground, you bring the seeping heads of your ' + Descriptors.multiCockDescriptLight() + ' to her nethers, slowly easing into her depths.  ', false );
					...
					MainView.outputText( 'With a satisfied growl of lust from yourself, and a screech of pain from the sand witch, you break past her cervixes and into her wombs. Pulling the heads of your ' + Descriptors.multiCockDescriptLight() + ' from her wombs, you rub and rut at the outside of her cunts, bringing yourself closer and closer to your peak as the sand witch struggles, her eyes turning wide with fright as she starts to understand you hold more power than her.  ', false );
					MainView.outputText( 'With a bestial cry of lust, you shove your shafts deep within her vaginas, past her cervices and into her womb where you blast your potent, corrupted, demon-tainted seed.  ', false );
					//A few dicks...
		...
		//Femfem
			...
			MainView.outputText( 'You smile at the sand witch, trying to seem enticing. A tiny smile plays across her face even though she supports a worried frown. She reaches up to you, and you allow her to gently pull you down to her. You marvel at her multiple breasts, each perfectly rounded. Each has a nipple approximately two inches long, enough to suck if you wanted to.  ', false );
			MainView.outputText( 'Your attention wanders down to the damp sand underneath her legs, and her musky scent hits your nose.\n\n', false );
			this.sandwitchBewbs();
		}
	};
	SandWitchScene.prototype.sandwitchBewbs = function() {
		...
		MainView.outputText( 'You burp, and flush scarlet, but the sand witch lays a hand against your face, leans close, and kisses your lips sweetly. Her tongue snakes out and dives into your mouth. She pulls your face against hers, grinding her breasts against your chest. You can feel fresh milk dribbling against your ' + CoC.player.skinDesc + ' and the scents are driving you wild with lust.  ', false );
		MainView.outputText( 'She breaks the kiss and smiles at you, whispering. Your eyes slowly close as you grow drowsy; you realize too late that she\'s casting a spell. You fall asleep, the sand witch\'s milk in your stomach churning, gurgling.  ', false );
	...
	SandWitchScene.prototype.sandwitchSex = function() {
		...
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( Utils.num2Text( CoC.player.cockTotal() ), false );
		} else {
			MainView.outputText( 'two', false );
		}
		MainView.outputText( ' of them!  The sand witch appears frightened, but as you gently tease her lips with your fingers, she slowly starts to breathe faster, starting to moan as her legs spread themselves against her better judgment. Grinning in satisfaction, you bring your rigid ' + Descriptors.cockDescript( 0 ) + ' against her lips, and push slowly, gently.  ', false );
		MainView.outputText( 'The sand witch cries out, her hands grabbing your arms, pressing hard against your ' + CoC.player.skinDesc + ' as you slowly and teasingly ease your ' + Descriptors.cockDescript( 0 ) + ' into her tight depths. You almost feel like a virgin, your ' + Descriptors.cockDescript( 0 ) + ' throbbing, the need to cum almost beyond your ability to control.  ', false );
		MainView.outputText( 'You rest against her chest, her large breasts squirting milk against your own chest as your weight appears to excite her enough to make her lactate, if what you\'re doing already wasn\'t enough.  ', false );
		MainView.outputText( 'You kiss the sand witch, getting accustomed to her unbelievable tightness. You hump backwards and forwards, grinning like a fool with his first girlfriend. Not even a succubus would be able to rival her lush depths.  ', false );
		MainView.outputText( 'You finally get serious and ease yourself into fucking the sand witch, slowly gaining speed and depth as your groins slap together with such ferocity that a normal person would be bruised upon their next waking. You pound the sand witch, making her scream in ecstasy, her already tight depths turning vice-like as you grunt your lust, cumming and filling her depths. Your ', false );
		if( CoC.player.balls >= 2 ) {
			MainView.outputText( 'balls ache and your ', false );
		}
		MainView.outputText( Descriptors.cockDescript( 0 ) + ' throbs with need, even as you empty your lust into the defeated sand witch.', false );
	...
	SandWitchScene.prototype.sandwitchCentaurBoning = function() {
		...
		//[win via HP]
		if( CoC.monster.HP < 1 ) {
			MainView.outputText( 'and torn in many places. ', false );
		}//[win via lust]
		else {
			MainView.outputText( 'and her fingers squeezing her nipples as lust overwhelms her. ', false );
		}
		//[has breasts]
		if( CoC.player.biggestTitSize() > 3 ) {
			MainView.outputText( 'As you approach, her eyes drift up to your ' + CoC.player.allBreastsDescript() + ' and she ', false );
			//[win via HP]
			if( CoC.monster.HP < 1 ) {
				MainView.outputText( 'is unable to stop herself from slowly licking her lips. ', false );
			}//[win via lust]
			else {
				MainView.outputText( 'sensually licks her parted lips. ', false );
			}
		}
		//[lactating]
		if( CoC.player.biggestLactation() > 1 ) {
			MainView.outputText( 'Her eyes widen when she sees the beads of milk already dripping from your ' + Descriptors.nippleDescript( 0 ) + 's as her hand drops between her legs and she begins to slowly masturbate.', false );
		}
		MainView.outputText( '\n\n', false );
		//[Corruption < 70]
		if( CoC.player.cor < 70 ) {
			MainView.outputText( 'You smile as you approach', false );
			//[has breasts]
			if( CoC.player.biggestTitSize() > 0 ) {
				MainView.outputText( '; your ' + Descriptors.nippleDescript( 0 ) + 's  become stiff', false );
			}
			//[cock only]
			if( CoC.player.totalCocks() > 0 && !CoC.player.hasVagina() ) {
				MainView.outputText( ' and your ' + Descriptors.cockDescript( 0 ) + ' hardens.', false );
			}
			//[cunt only]
			if( CoC.player.hasVagina() && CoC.player.totalCocks() === 0 ) {
				MainView.outputText( ' and your ' + Descriptors.vaginaDescript( 0 ) + ' begins to drip with moisture.', false );
			}
			//[cock and cunt]
			if( CoC.player.hasVagina() && CoC.player.totalCocks() > 0 ) {
				MainView.outputText( ' and your ' + Descriptors.cockDescript( 0 ) + ' hardens as your ' + Descriptors.vaginaDescript( 0 ) + ' begins to drip with moisture.', false );
			}
			if( CoC.player.gender === 0 ) {
				MainView.outputText( ' her.', false );
			}
			MainView.outputText( ' She seems hesitant at first, but soon approaches and begins to run her hands along your stomach and your ' + Descriptors.hipDescript(), false );
			//[has breasts]
			if( CoC.player.biggestTitSize() > 0 ) {
				MainView.outputText( ', eyes fixed on your ' + CoC.player.allBreastsDescript(), false );
			}
			MainView.outputText( '.\n\n', false );
			...
			MainView.outputText( '\n\n', false );
			MainView.outputText( 'Delicate hands reach around to ', false );
			//[has breasts]
			if( CoC.player.biggestTitSize() > 0 ) {
				MainView.outputText( 'fondle your ' + CoC.player.allBreastsDescript() + ' and ', false );
			}
			MainView.outputText( 'tease your ' + Descriptors.nippleDescript( 0 ) + 's, and you can feel her grinding herself against your strong shoulders. ', false );
			//[orb penetrated player during combat]
			MainView.outputText( 'With a start you realize that the witch\'s orb is still inside you as it suddenly begins to pulse within your ' + CoC.player.assholeOrPussy() + ', causing you to cry out and hasten your pace across the dunes. ', false );
			MainView.outputText( 'You lose track of time and location as you ride, feeling her orgasm over and over against you, her milk pouring down your back and spilling onto the sands. ', false );
			//[has breasts]
			if( CoC.player.biggestTitSize() > 0 ) {
				//[lactating]
				if( CoC.player.biggestLactation() >= 1 ) {
					MainView.outputText( 'Your own ' + Descriptors.nippleDescript( 0 ) + 's  are leaking as well, ', false );
					//[light-med lactation]
					if( CoC.player.biggestLactation() < 3 ) {
						MainView.outputText( 'small streams running out ', false );
					}//[heavy lactation]
					else {
						MainView.outputText( 'torrents gushing out ', false );
					}
					MainView.outputText( 'under her expert hands. ', false );
				}
			}
			MainView.outputText( 'You can somehow feel her arousal pouring through her milk and into your skin, filling you with her own sensations and making your entire body vibrate like a sexual tuning fork.  As the sensations grow to be overwhelming, you feel yourself stumbling and slowing until you finally come to a stop. Panting, you listen to the sand witch\'s voice whispering sweet nothings into your ear... but the nothings seem to be having an effect.\n\n', false );
			//[GoTo: SW-1]
	...
	SandWitchScene.prototype.SWCentaurMore = function( argument ) {
		...
		//[SW_1]
		if( argument === 1 ) {
			MainView.outputText( 'You realize too late that she has been casting a spell for the past few minutes. Before you can react, everything grows dark.', false );
		...
		if( argument === 2 ) {
			...
			if( CoC.player.cockTotal() > 0 || (CoC.player.gender === 3 && Utils.rand( 2 ) === 0) ) {
				MainView.outputText( 'Finished with your games, pre-cum starts pooling under your throbbing ' + Descriptors.cockDescript( 0 ) + '. ', false );
				//[largest cock is wide]
				if( CoC.player.cocks[ 0 ].cockThickness >= 3 ) {
					if( CoC.player.hasKnot( 0 ) ) {
						MainView.outputText( 'Seeing its sheer size combined with your swollen knot causes her to gasp in terror and try to run again.  ', false );
					} else {
						MainView.outputText( 'Seeing its sheer size causes her to gasp in fear and turn to run again.  ', false );
					}
				}
				MainView.outputText( 'You grab the witch and knock her down into the sand, quickly lowering your ' + Descriptors.cockDescript( 0 ) + ' to be against her buttocks. ', false );
				//[1 cock, non-tentacle]
				if( CoC.player.cockTotal() === 1 && CoC.player.tentacleCocks() === 0 ) {
					MainView.outputText( 'With a single thrust, you push deep into one of her cunts, ', false );
				}
				//[1 cock, tentacle]
				if( CoC.player.cockTotal() === 1 && CoC.player.tentacleCocks() === 1 ) {
					MainView.outputText( 'Your ' + Descriptors.cockDescript( 0 ) + ' caresses her anus teasingly, causing her to whimper in a mixture of arousal and denial.  With a single savage thrust, you push past her clenched muscles, ', false );
				}
				//[two cocks]
				if( CoC.player.cockTotal() === 2 ) {
					MainView.outputText( 'After aligning your ' + Descriptors.multiCockDescriptLight() + ' to her twin cunts, you push yourself deeply into her, ', false );
				}
				//[3+ cocks]
				if( CoC.player.cockTotal() >= 3 ) {
					MainView.outputText( 'Lining up two of your cocks to her twin cunts and another to her anus, you thrust into her without pre-amble.  Her anal muscles try to keep you out, but they are no match for the strength of your legs.  You tear into her, ', false );
				}
				MainView.outputText( 'eliciting a scream as your hind quarters push her forcefully over the sand. ', false );
				//[largest cock is wide]
				if( CoC.player.cocks[ 0 ].cockThickness >= 3 ) {
					MainView.outputText( 'It is hard to believe just how tight she is, though if her cries serve as any indication, she will not be after you are through with her.  Turned on even more, you thrust in with increasing vigor and try to widen her as much as possible. ', false );
				}
				//[2+ cocks]
				if( CoC.player.cockTotal() >= 2 ) {
					MainView.outputText( 'The feeling of your ' + Descriptors.multiCockDescriptLight() + ' touching each other through the narrow layers of her body causes you to grunt in pleasure, experiencing the sensation anew during every thrust. ', false );
				}
				//[largest cock is long, non-tentacle]
				if( CoC.player.cocks[ 0 ].cockLength >= 12 ) {
					if( CoC.player.cockTotal() === 1 ) {
						MainView.outputText( 'Her body surrenders more and more as your ' + Descriptors.multiCockDescriptLight() + ' presses against her cervix. ', false );
					} else {
						MainView.outputText( 'Her body surrenders more and more as your ' + Descriptors.multiCockDescriptLight() + '  press against her cervixes. ', false );
					}
					MainView.outputText( 'Her mouth opens in a soundless and agonizing cry when you finally push past. ', false );
				}
				//[1 cock, tentacle]
				if( CoC.player.tentacleCocks() === 1 && CoC.player.cockTotal() === 1 ) {
					MainView.outputText( 'Her body surrenders more and more as your ' + Descriptors.cockDescript( 0 ) + ' pushes deep into her bowels, snaking its way further into her body as it fucks her insides. ', false );
				}
				MainView.outputText( 'It feels like you have been pushing yourself into her for hours', false );
				//[largest cock is wide and/or long]
				if( CoC.player.cocks[ 0 ].cockLength >= 12 || CoC.player.cocks[ 0 ].cockThickness >= 3 ) {
					MainView.outputText( '; her cries have long since died down to whimpers. ', false );
				}
				//[largest cock is not wide and/or long]
				else {
					MainView.outputText( ', and her cries have long since turned into groans of pleasure. ', false );
				}
				MainView.outputText( 'You cannot hold off your orgasm any longer. Your ' + Descriptors.multiCockDescriptLight() + ' explode', false );
				if( CoC.player.cockTotal() === 1 ) {
				...
				if( CoC.player.cumQ() >= 250 ) {
					MainView.outputText( 'gushing massive amounts of your cum ', false );
				}//[regular cum production]
				else {
					MainView.outputText( 'pushing your sperm ', false );
				}
				MainView.outputText( 'deep into her ', false );
				//[1 cock, tentacle]
				if( CoC.player.cockTotal() === 1 && CoC.player.tentacleCocks() === 1 ) {
					...
					if( CoC.player.cocks[ 0 ].cockLength >= 12 || CoC.player.cocks[ 0 ].cockThickness >= 3 ) {
						...
						//[2+ cocks]
						if( CoC.player.cockTotal() >= 2 ) {
							MainView.outputText( 'cunts', false );
						}
					}
					//[largest cock is wide and/or long]
					else {
						...
						if( CoC.player.cockTotal() >= 3 ) {
							MainView.outputText( ' and bowels', false );
						}
					}
				}
				MainView.outputText( '.  With a satisfied groan, you pull out and let your ' + Descriptors.multiCockDescriptLight() + ' dribble the last remnants of your cum over the ravished witch. Satisfied, you ride off into the desert.', false );
			}
			//[% no cocks]
			else {
				MainView.outputText( 'Finished with your games ', false );
				//[has cunt]
				if( CoC.player.hasVagina() ) {
					MainView.outputText( 'and your ' + Descriptors.vaginaDescript( 0 ) + ' dripping with desire', false );
				}
				MainView.outputText( ', you push the witch unceremoniously to the ground and deliver a slap to her breasts.  She cries out in pain as milk splashes out onto the sand.  A cruel smile is brought to your face and you start slapping them even harder, alternatively smacking her pair of cunts for good measure.  Surprisingly, her cries of pain begin to turn into moans of pleasure with every slap.  She might even be pushing into the blows slightly, though it is difficult to tell.  After a sizable pool of milk and her juices has drained into the sands beneath her, you reach down and start to drive your fingers hard into one of the sand witch\'s cunts.  The first few thrusts are enough to violently bring her to orgasm.  You trot forward slowly until you are over her and the witch suddenly hops up. Driving her face into your ', false );
				//[has cunt]
				if( CoC.player.hasVagina() ) {
					MainView.outputText( Descriptors.vaginaDescript( 0 ) + ', she fingers herself even harder than you had been a moment ago.  Her lips lock unto your ' + Descriptors.clitDescript() + ' and she suckles on it, drawing a pained gasp from you and causing your pussy juices to gush over her face. You feel ', false );
					//[cunt size is small]
					if( CoC.player.vaginalCapacity() < 10 ) {
						MainView.outputText( 'one of her fingers ', false );
					}//[cunt size is normal]
					else if( CoC.player.vaginalCapacity() < 20 ) {
						MainView.outputText( 'several of her fingers ', false );
					}//[cunt size is large]
					else {
						MainView.outputText( 'a fist ', false );
					}
					MainView.outputText( 'pushing deep into your ' + Descriptors.vaginaDescript( 0 ) + ', and you\'re surprised as ', false );
					//[anus size is small]
					if( CoC.player.analCapacity() < 10 ) {
						MainView.outputText( 'her tongue ', false );
					}//[anus size is normal]
					else if( CoC.player.analCapacity() < 20 ) {
						MainView.outputText( 'two of her fingers ', false );
					}//[anus size is large]
					else {
						MainView.outputText( 'a fist ', false );
					}
					MainView.outputText( 'penetrates your ' + Descriptors.assholeDescript(), false );
					MainView.outputText( '.', false );
				}
				//[does not has cunt]
				else {
					MainView.outputText( Descriptors.assholeDescript() + ', she fingers herself even harder than you had been a moment ago.  Her tongue drives into your ' + Descriptors.assholeDescript() + ', licking enthusiastically as she squeezes her nipples. ', false );
					//[anus size is small]
					if( CoC.player.analCapacity() < 10 ) {
						MainView.outputText( 'The sensations are intense, and you could almost swear that her tongue is tracing a pattern as her mouth mutters strange sounds. ', false );
					}//[anus size is normal]
					else if( CoC.player.analCapacity() < 20 ) {
						MainView.outputText( 'Unsatisfied with just her buffet, you soon feel her fingers thrust hard into you and hear her muttering what sounds like obscene words of depraved lust. ', false );
					}//[anus size is large]
					else {
						MainView.outputText( 'Unsatisfied with just her buffet, the witch grabs one of her breasts and squirts milk into your massive hole.  You realize she was lubricating it even as you feel her entire hand driving into you.  The sensations are too intense, and your screams of pleasure drown out her lustful muttering. ', false );
					}
					MainView.outputText( 'Her mumbling finishes even as an orgasm overwhelms you.', false );
					MainView.outputText( '\n\n', false );
					//[SW_1]
		...
		if( argument === 3 ) {
			...
			//[STR > 50]
			if( CoC.player.str > 50 ) {
				MainView.outputText( 'pick her up and toss her onto your back. ', false );
			}//[STR < 50]
			else {
				MainView.outputText( 'offer her your hand and help her onto your back. ', false );
			}
			MainView.outputText( 'Your strong hooves carry you across the clear desert at a light trot.  Without her robes to protect her, you can feel the witch\'s bare pussies grinding against your back as she presses her quad of breasts against your shoulders.  Her moans intensify more and more as you pick up the pace.', false );
		}
		return false;
	};
	//Knot sand witch
	...
	SandWitchScene.prototype.beatSandwitch = function() {
		...
		if( CoC.monster.lust > 99 ) {
			...
			MainView.outputText( 'You smile in satisfaction as the ' + CoC.monster.short + ' drops down on all fours and struggles to rise.  Sadly you realize your own needs have not been met.  Of course, you could always fuck the witch...', true );
		}
		MainView.outputText( '  Of course, just taunting, teasing, and humiliating her for her arrogance would be equally amusing, <b>but it would give her plenty of time to turn the tables...</b>' );
		var temp2 = null;
		var temp3 = null;
		if( EngineCore.silly() ) {
	...
	SandWitchScene.prototype.missingoSex = function() {
		...
		MainView.outputText( 'You want to embarrass her further, complete her humiliation upon the sands she once thought to be her domain. You tear her robes into little shreds, exposing her four generously sized breasts. Your eyes follow her dri-^ling nipples, the pupils bouncing as they do. "Wh-what are you going to do? Rape me?!" cries the witch, her tone equal parts terrified and wishful. Her thighs attempt to close, though you\'re sure it\'s more t$ grind them together than any atte-pt at preserving her modesty. Leaning down, y-u open up to let your tongue lap at her swollen clit. She arches her back and moans, thrusting her hips towa-ds you as bes- she can.\n\n', false );
		MainView.outputText( '0)w is t$e time. But no, she won\'t get the dick. She hasn\'t EAR*ED the dick. She isn\'t even wor$h your " + Descriptors.vaginaDescript(0) + " yet. No n- no, you need -o warm that ho_ cunt up. You whistle to g-t her attention, and as you do, you present your -i%-s ^a_=e* up to*--h-r. Her eyes wi_en, the wo$an\'s jaw going slack. "N-no.. not t-e whole thing." you nod _n respo-se to her plea. "The WHO-E THING?!"\n\n', false );
		MainView.outputText( 'Now she begins to panic. She shake- her head rapidly as you draw bac- your %s99=, and thrust them towards her >pen sl1t...', false );
	...
	SandWitchScene.prototype.missingoSex2 = function() {
		...
		MainView.outputText( '"Still..." she says, ##ok%7g at you slyly. "Why don\'t you (ull b=c( and try that again...."', false );
		//[N*xt]
	...
	SandWitchScene.prototype.missingoSex3 = function() {
		...
		MainView.outputText( '"@_i@_..." s_e says, ##o@%7g @t yo_ s_@_y. "W_@ d_n\'t @o_ (ull b=c( an_ @r_ _h_@ a_@in...."\n\n', false );
		//[@*xt]
	...
	SandWitchScene.prototype.missingoSex4 = function() {
		...
		MainView.outputText( '"@_o@_..." o_o oooo, ##o@%7o @o oo_ o_@_o. "o_@ o_o\'o @o_ (ooo o=o( oo_ @o_ _o_@ o_@oo...."', false );
		//[Fuck this!]
	...
	SandWitchScene.prototype.missingoSex6 = function() {
		...
		MainView.outputText( '', true );
		MainView.outputText( 'You successfully restore your game. Everything\'s intact. At least, you thought it was. You must have messed up the password somewhere along the line, but oddly enough, the only difference is the clock being set an hour ahead and a few extra gems and EXP in your pocket.\n\n', false );
		MainView.outputText( 'It is like fucking christmas up in here.', false );
	...
	SandWitchScene.prototype.ovipositSandWitches = function() {
		MainView.clearOutput();
		MainView.outputText( 'As you glance down at the ' );
		if( CoC.monster.HP < 1 ) {
			MainView.outputText( 'bruised and battered' );
		} else {
			MainView.outputText( 'lusty' );
		}
		MainView.outputText( ' sand witch, you feel the familiar tug of arousal in your loins.  Yet your current... situation offers quite the opportunity to relieve some tension through the use of your \'other\' sexual appendage.  Licking your lips in anticipation, you lower yourself down to the obstinate spell caster, who immediately reacts with ' );
		if( CoC.monster.HP < 1 ) {
			MainView.outputText( 'fright as she clamors to get away.' );
		} else {
			MainView.outputText( 'an uneasy curiosity to see what exactly you have planned.' );
		}
		...
		MainView.outputText( 'The sand witch slows to a dead stop as she assesses your intentions, which are made all the more clear as you disrobe and toss your [armor] aside, exposing your ' );
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'hardened  ' + Descriptors.multiCockDescriptLight() );
		}
		...
		MainView.outputText( ' for her viewing consumption.  Making quite the persuasive argument of non-hostility as you do, the sand witch finally relents, removing her cloak and exposing her four-breasted, ' );
		if( CoC.player.totalCocks() > 1 ) {
			MainView.outputText( Utils.num2Text( CoC.player.cockTotal() + 1 ) );
		} else {
			MainView.outputText( 'two' );
		}
		MainView.outputText( '-sexed form, allowing the sun to glisten off her bare body.' );
		...
		if( CoC.player.canOvipositBee() ) {
			MainView.outputText( 'bee-morphs' );
		} else {
			MainView.outputText( 'driders' );
		}
		MainView.outputText( ' can do to their partners and unsure if this is really what she wants.  The poor girl seems to need a little \'encouragement\'... of course, you could just skip all that regardless if you really wanted.' );
	...
	SandWitchScene.prototype.laySomeEggsInThatWitchFinally = function() {
		...
		MainView.outputText( 'Before long the intense need to culminate the act grows too lucid to ignore, and with a hum of satisfaction you finally bore your ovipositor through the tight ' );
		if( cunt ) {
			MainView.outputText( 'folds of her vulva' );
		} else {
			MainView.outputText( 'ring of her anus' );
		}
		MainView.outputText( '.  Like something out of a fever dream, the bizarre appendage you wield burrows further and further into her ' );
		if( cunt ) {
		...
		if( cunt ) {
			MainView.outputText( 'ribbed contours of her pussy' );
		} else {
			MainView.outputText( 'tight fit of her innards' );
		}
		MainView.outputText( ' enrapturing your egg-tube with every passing inch.  You begin the process of lubricating the antsy witch\'s ' );
		if( cunt ) {
			MainView.outputText( 'love-' );
		} else {
			MainView.outputText( 'anal ' );
		}
		MainView.outputText( 'canal for the delivery of your young; the witch barely contains her moans and utter glee at being filled with such a hot, humid substance.  In perverted delight the sand witch coos between kisses, her moans betraying a curiosity to explore just how pleasant mating this way can truly be.' );
		...
		if( cunt ) {
			MainView.outputText( 'opening of your lover\'s cervix' );
		} else {
			MainView.outputText( 'deepest recesses of her sphincter' );
		}
		MainView.outputText( '.  The joining of oviduct to ' );
		if( cunt ) {
			MainView.outputText( 'womb' );
		} else {
			MainView.outputText( 'anus' );
		}
		MainView.outputText( ' now complete, you prepare yourself for the inevitable convulsion of orgasm along your bizarre appendage, grinning down at the sand witch with a smirk that can only convey that the fun\'s about to begin.  With a sudden, violent quake your ovipositor rumbles with the first of many eggs, squeezing your unborn brood down your shaft and pushing ' );
		if( cunt ) {
			MainView.outputText( 'at the waiting lips of the sand witch\'s vagina' );
		} else {
			MainView.outputText( ' through her rectal walls' );
		}
		MainView.outputText( '.  With a meticulous push, the forerunner of your young manages to infiltrate her passage, working its way valiantly into her depths with every convulsion.' );
		...
		if( cunt ) {
			MainView.outputText( 'into her waiting womb' );
		} else {
			MainView.outputText( 'as far as you can safely tuck it' );
		}
		MainView.outputText( ', nestled and sound until the scramble for life can begin.' );
		MainView.outputText( '\n\n"<i>More... give me more...</i>" the sand witch whimpers, caressing your ' );
		//(Bee, non-horse:
		if( CoC.player.canOvipositBee() && !CoC.player.isTaur() ) {
			MainView.outputText( 'torso' );
		} else {
			MainView.outputText( 'pelvis' );
		}
		MainView.outputText( ' to spur you on.  Of course you don\'t have a problem with fulfilling her wishes; the neglect of your throbbing sex' );
		if( CoC.player.cockTotal() > 1 || CoC.player.gender > 2 ) {
			MainView.outputText( 'es' );
		}
		MainView.outputText( ' is the only point of dissatisfaction, a point you make clear when you ' );
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'take hold of your ' + Descriptors.multiCockDescriptLight() + ' and bring it' );
		} else {
			MainView.outputText( 'bring your ' + Descriptors.vaginaDescript( 0 ) );
		}
		MainView.outputText( ' to her face.  The cute stare of her half-glazed eyes turns to the lunge of lips toward your sex, the sand witch driven like a woman possessed in her efforts to get the next batch going...' );
		MainView.outputText( '\n\nFor what feels like an eternity you sit atop the pliant sorceress, having her tend to your sex' );
		if( CoC.player.cockTotal() > 1 || CoC.player.gender === 3 ) {
			MainView.outputText( 'es' );
		}
		MainView.outputText( ' while you slowly pump her full of eggs, her girlish little moans resonating through your body as she orally stimulates your ' );
		if( CoC.player.hasCock() ) {
		...
		MainView.outputText( '.  At long last her enthusiastic efforts bring you to a more \'traditional\' release, lathering ' );
		var facial = Utils.rand( 2 );
		if( facial === 0 ) {
			MainView.outputText( 'her facial features with proof of your climax' );
		} else if( facial === 1 ) {
			MainView.outputText( 'the interior of her mouth with sweltering streams of your cum' );
		}
		MainView.outputText( '.  Having finally achieved release, and also having run out of eggs, you pull yourself free of the desert woman with a loud pop, the ' );
		if( CoC.player.canOvipositBee() ) {
			MainView.outputText( 'honey-like' );
		} else {
			MainView.outputText( 'goopy' );
		}
		MainView.outputText( ' lubricant drooling out of her in the absence of your intimate plug.  Utterly overwhelmed and exhausted, the sand witch resigns herself to gathering strength in the bright, blistering gaze of the desert sun.  Smiling to yourself, you take to re-donning your discarded [armor], remarking as you leave that you look forward to the next encounter; a sentiment the sorceress must agree with as she weakly waves to you, holding her stomach with her arms and filled with motherly delight at the thought of carrying your brood.' );
		//Give her ze eggs!
	...
	SandWitchScene.prototype.sammitchBirthsDriders = function() {
		...
		if( CoC.player.cor < 50 ) {
			MainView.outputText( 'Intending to help' );
		} else {
			MainView.outputText( 'Curious' );
		}
		MainView.outputText( ', you slide down the dune that you are on and come to a stop next to the distressed woman.  However, upon seeing her face you realize that this is the very sand witch that you had a hot and steamy tumble with!' );
	...
	SandWitchScene.prototype.helpZeWithBirfBabies = function() {
		...
		if( CoC.player.isTaur() || CoC.player.isDrider() ) {
			...
			if( CoC.player.isDrider() ) {
				MainView.outputText( 'skitter' );
			} else {
				MainView.outputText( 'gallop' );
			}
			MainView.outputText( ' across the dry desert sea...' );
		}
	...
	SandWitchScene.prototype.sandwitchBirthsYourMonstrosities = function() {
		...
		if( CoC.player.cor < 60 ) {
			MainView.outputText( ', with a quick kiss on the lips' );
		}
		MainView.outputText( ', whisper in her ear that you\'ll see her again...' );
		//[(corr >= 60)
		if( CoC.player.cor >= 60 ) {
	...
	SandWitchScene.prototype.sandwitchSpanking = function() {
		MainView.clearOutput();
		MainView.outputText( 'Feeling euphoric after your clear victory, you lazily circle the panting witch and laugh at her.  "<i>Look at you,</i>" you taunt, "<i>A mighty witch of the sands, laid low by a helpless traveller.' );
		if( CoC.monster.HP < 1 ) {
			MainView.outputText( '  You can\'t even stand up!' );
		} else {
			MainView.outputText( '  You can\'t even stop masturbating, can you?' );
		}
		MainView.outputText( '</i>"  You kick her hand away, and she tumbles down into a distraught heap, crying softly' );
		if( CoC.monster.lust > 99 ) {
			MainView.outputText( ' but still masturbating' );
		}
		MainView.outputText( '.  It\'s time for some fun!' );
		...
		if( CoC.player.cor < 33 ) {
			MainView.outputText( 'and though you feel kind of bad about using her this way, she likely would have done worse to you.' );
		} else if( CoC.player.cor < 66 ) {
			MainView.outputText( 'and though you aren\'t sure you ought to be using her this way, she likely would have done worse to you.' );
		} else {
			MainView.outputText( 'and though you used to find things like this distasteful, you don\'t really give a shit anymore.' );
		}
		MainView.outputText( '  You tweak and tease with a goofy smile, laughing as you aim one of the tits up into the witch\'s face, splattering her with her own alabaster juices.  You\'re so involved in it that you fail to notice the increasingly angry gaze that settles over your captive\'s face.  Her tits are just so goddamn fun to tease that you don\'t really waste any time paying attention to her from the neck up.  That\'s a mistake.' );
		...
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'Worse still, [eachCock] is now unprotected and pressed against the rough stone by your body-weight.  It\'s less than pleasant.  ' );
		...
		MainView.outputText( '.  Fingers tickle the ' + CoC.player.skin() + ' of your bottom, and then, without warning, your [butt] is slapped!  The sound of the hit rings out across the desert, hard enough to make moisture bead at the corners of your eyes.' );
		...
		if( CoC.player.cor < 50 ) {
			MainView.outputText( 'She sounds almost fanatical in her dedication to her order.' );
		} else {
			MainView.outputText( 'What the hell is she on about?  Does she actually think her little torture session is going change your attitude?' );
		}
