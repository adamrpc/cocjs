...
angular.module( 'cocjs' ).run( function( SceneLib, $rootScope, $log, CoC, Utils, StatusAffects, EngineCore, AppearanceDefs, Descriptors, EventParser, PregnancyStore, kFLAGS, Combat, SandWitch, PerkLib ) {
	...
	SandWitchScene.prototype.timeChange = function() {
		...
		$log.debug( '\nSand Witch time change is ' + CoC.getInstance().time.hours + ', incubation: ' + this.pregnancy.incubation + ', event: ' + this.pregnancy.event );
	...
	SandWitchScene.prototype.encounter = function() {
		...
		if( CoC.getInstance().player.statusAffectv1( StatusAffects.Exgartuan ) === 1 && CoC.getInstance().player.cockArea( 0 ) > 100 && CoC.getInstance().player.statusAffectv2( StatusAffects.Exgartuan ) === 0 ) {
			EngineCore.outputText( '\n\nThe ' + CoC.getInstance().player.armorName + ' covering your lower half hits the ground, as if yanked down by magic.  Your ' + Descriptors.cockDescript( 0 ) + ' pulsates darkly, growing rigid in seconds as the demon within you takes over.  It barks, "<i>Fuck, how about I cast my spell on you baby?</i>"\n\n' );
			...
			EngineCore.outputText( ' as you yank your ' + CoC.getInstance().player.armorName + ' back into place.  You\'re in charge here, not some possessed appendage!   Exgartuan yells something snide, but it\'s muffled too much to understand.  You look up in time to sidestep an attack from the Sand Witch.  It looks like you\'ll have to fight her!' );
	...
	SandWitchScene.prototype.allowSandWitchMagic = function() {
		EngineCore.spriteSelect( 50 );
		EngineCore.clearOutput();
		if( CoC.getInstance().player.hairColor === 'sandy blonde' ) {
			...
			if( CoC.getInstance().player.breastRows.length === 0 || CoC.getInstance().player.biggestTitSize() === 0 ) {
				...
				if( CoC.getInstance().player.breastRows[ 0 ].nipplesPerBreast < 1 ) {
					CoC.getInstance().player.breastRows[ 0 ].nipplesPerBreast = 1;
				}
				EngineCore.dynStats( 'sen', 2, 'lus', 1 );
			}
			if( CoC.getInstance().player.biggestTitSize() >= 1 && CoC.getInstance().player.biggestTitSize() <= 2 ) {
			...
			if( CoC.getInstance().player.biggestLactation() > 0 ) {
				...
				if( CoC.getInstance().player.biggestLactation() >= 2 && CoC.getInstance().player.biggestLactation() <= 2.6 ) {
				...
				if( CoC.getInstance().player.biggestLactation() > 2.6 && CoC.getInstance().player.biggestLactation() < 3 ) {
				...
				if( CoC.getInstance().player.biggestLactation() < 3 ) {
					CoC.getInstance().player.boostLactation( 0.75 );
	...
	SandWitchScene.prototype.sandwitchRape = function() {
		...
		if( CoC.getInstance().player.lust > 99 ) {
			//BAD END START
			if( CoC.getInstance().player.biggestTitSize() >= 9 && CoC.getInstance().player.biggestLactation() >= 3 && CoC.getInstance().player.cor >= 25 ) {
			...
			if( CoC.getInstance().player.hairColor.indexOf( 'sandy blonde' ) !== -1 ) {
				...
				if( CoC.getInstance().player.biggestTitSize() > 0 && CoC.getInstance().player.biggestTitSize() < 3 ) {
					EngineCore.outputText( 'Your breasts suddenly balloon outwards, stopping as they reach a perfectly rounded C-cup.  ', false );
					CoC.getInstance().player.breastRows[ 0 ].breastRating = 3;
					EngineCore.dynStats( 'sen', 1, 'lus', 1 );
				}
				...
				if( CoC.getInstance().player.biggestLactation() > 0 ) {
					...
					if( CoC.getInstance().player.biggestLactation() < 3 ) {
						CoC.getInstance().player.boostLactation( 0.7 );
						EngineCore.outputText( 'Your breasts feel fuller... riper... like your next milking could be even bigger.  ', false );
					}
					EngineCore.dynStats( 'lib', 1, 'sen', 4, 'lus', 15 );
				}
		...
		if( CoC.getInstance().player.gender === 1 || CoC.getInstance().player.gender === 3 ) {
			...
			//Really corrupt
				...
				if( CoC.getInstance().player.cockTotal() === 1 ) {
					//DOGGIES
					if( CoC.getInstance().player.dogCocks() > 0 && CoC.getInstance().player.cocks[ 0 ].knotMultiplier > 1.4 ) {
						this.knotSandwitch();
					} else {
						EngineCore.outputText( 'You place your ' + CoC.getInstance().player.foot() + ' in the wet sand, pulling her hair and making her look you in the eyes. The sand witch looks shocked at your display of power, only turning you on more. You undo your ' + CoC.getInstance().player.armorName + ' and grope her nethers, only to find a surprise. She has ' + Utils.num2Text( CoC.getInstance().player.cocks.length + 1 ) + ' pussies and each seems to be made to fit you exactly. Pushing her to the ground, you bring the seeping head of your ' + Descriptors.cockDescript( 0 ) + ' to one of her nethers, slowly easing into her depths.  ', false );
				...
				if( CoC.getInstance().player.cockTotal() > 1 ) {
					...
					EngineCore.outputText( 'Using the corruption in your blood, you place your ' + CoC.getInstance().player.foot() + ' upon the budding shoots, and wipe them from existence. The sand witch looks shocked at your display of power, only turning you on more. You undo your loin cloth and grope her nethers, only to find a surprise. She has ' + Utils.num2Text( CoC.getInstance().player.cocks.length + 1 ) + ' loose wet cunts and each seems to be made to fit you exactly. Pushing her to the ground, you bring the seeping heads of your ' + Descriptors.multiCockDescriptLight() + ' to her nethers, slowly easing into her depths.  ', false );
	...
	SandWitchScene.prototype.sandwitchSex = function() {
		...
		if( CoC.getInstance().player.cockTotal() > 1 ) {
			EngineCore.outputText( Utils.num2Text( CoC.getInstance().player.cockTotal() ), false );
		} else {
		...
		EngineCore.outputText( 'The sand witch cries out, her hands grabbing your arms, pressing hard against your ' + CoC.getInstance().player.skinDesc + ' as you slowly and teasingly ease your ' + Descriptors.cockDescript( 0 ) + ' into her tight depths. You almost feel like a virgin, your ' + Descriptors.cockDescript( 0 ) + ' throbbing, the need to cum almost beyond your ability to control.  ', false );
		...
		EngineCore.outputText( '\n\nWith a satisfied smirk, you lay atop your prize, your ' + Descriptors.cockDescript( 0 ) + ' still twitching within her depths. One of the sand witch\'s hands rubs at your head, teasing your ' + CoC.getInstance().player.skinDesc + '. Like a fresh youth, you fall asleep, ' + Descriptors.cockDescript( 0 ) + ' buried deep in the sand witch.  ', false );
	...
	SandWitchScene.prototype.sandwitchCentaurBoning = function() {
		...
		if( CoC.getInstance().player.biggestTitSize() > 3 ) {
			EngineCore.outputText( 'As you approach, her eyes drift up to your ' + CoC.getInstance().player.allBreastsDescript() + ' and she ', false );
			//[win via HP]
			if( CoC.getInstance().monster.HP < 1 ) {
		...
		if( CoC.getInstance().player.cor < 70 ) {
			...
			//[has breasts]
			if( CoC.getInstance().player.biggestTitSize() > 0 ) {
				EngineCore.outputText( ', eyes fixed on your ' + CoC.getInstance().player.allBreastsDescript(), false );
			}
			EngineCore.outputText( '.\n\n', false );
			...
			//[has breasts]
			if( CoC.getInstance().player.biggestTitSize() > 0 ) {
				EngineCore.outputText( 'fondle your ' + CoC.getInstance().player.allBreastsDescript() + ' and ', false );
			}
			...
			EngineCore.outputText( 'With a start you realize that the witch\'s orb is still inside you as it suddenly begins to pulse within your ' + CoC.getInstance().player.assholeOrPussy() + ', causing you to cry out and hasten your pace across the dunes. ', false );
	...
	SandWitchScene.prototype.SWCentaurMore = function( argument ) {
		...
		if( argument === 2 ) {
			...
			if( CoC.getInstance().player.cockTotal() > 0 || (CoC.getInstance().player.gender === 3 && Utils.rand( 2 ) === 0) ) {
				...
				//[1 cock, non-tentacle]
				if( CoC.getInstance().player.cockTotal() === 1 && CoC.getInstance().player.tentacleCocks() === 0 ) {
					EngineCore.outputText( 'With a single thrust, you push deep into one of her cunts, ', false );
				}
				//[1 cock, tentacle]
				if( CoC.getInstance().player.cockTotal() === 1 && CoC.getInstance().player.tentacleCocks() === 1 ) {
				...
				//[1 cock, tentacle]
				if( CoC.getInstance().player.tentacleCocks() === 1 && CoC.getInstance().player.cockTotal() === 1 ) {
				...
				if( CoC.getInstance().player.cocks[ 0 ].cockLength >= 12 || CoC.getInstance().player.cocks[ 0 ].cockThickness >= 3 ) {
			...
			//[% no cocks]
				...
				if( CoC.getInstance().player.hasVagina() ) {
					...
					else if( CoC.getInstance().player.vaginalCapacity() < 20 ) {
					...
					//[anus size is small]
					if( CoC.getInstance().player.analCapacity() < 10 ) {
	...
	SandWitchScene.prototype.knotSandwitch = function() {
		...
		if( CoC.getInstance().player.cumQ() >= 25 && CoC.getInstance().player.cumQ() < 100 ) {
		...
		if( CoC.getInstance().player.cumQ() >= 100 && CoC.getInstance().player.cumQ() < 500 ) {
	...
	SandWitchScene.prototype.beatSandwitch = function() {
		...
		if( CoC.getInstance().monster.lust > 99 ) {
			EngineCore.outputText( 'You smile in satisfaction as the ' + CoC.getInstance().monster.short + ' drops down on all fours and begins masturbating feverishly.  Sadly you realize your own needs have not been met.  Of course you could always fuck the horny witch...\n\nDo you rape her?', true );
		} else {
			EngineCore.outputText( 'You smile in satisfaction as the ' + CoC.getInstance().monster.short + ' drops down on all fours and struggles to rise.  Sadly you realize your own needs have not been met.  Of course, you could always fuck the witch...', true );
		}
	...
	SandWitchScene.prototype.ovipositSandWitches = function() {
		...
		if( CoC.getInstance().player.totalCocks() > 1 ) {
			EngineCore.outputText( Utils.num2Text( CoC.getInstance().player.cockTotal() + 1 ) );
		} else {
	...
	SandWitchScene.prototype.laySomeEggsInThatWitchFinally = function() {
		...
		//(Bee, non-horse:
		if( CoC.getInstance().player.canOvipositBee() && !CoC.getInstance().player.isTaur() ) {
		...
		if( CoC.getInstance().player.cockTotal() > 1 || CoC.getInstance().player.gender > 2 ) {
			EngineCore.outputText( 'es' );
		}
	...
	SandWitchScene.prototype.sandwitchSpanking = function() {
		...
		if( CoC.getInstance().player.findPerk( PerkLib.FireLord ) >= 0 || CoC.getInstance().player.findPerk( PerkLib.Hellfire ) >= 0 || CoC.getInstance().player.findPerk( PerkLib.Dragonfire ) >= 0 ) {
		...
		EngineCore.outputText( '.  Fingers tickle the ' + CoC.getInstance().player.skin() + ' of your bottom, and then, without warning, your [butt] is slapped!  The sound of the hit rings out across the desert, hard enough to make moisture bead at the corners of your eyes.' );
