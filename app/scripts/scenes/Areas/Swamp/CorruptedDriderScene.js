...
angular.module( 'cocjs' ).run( function( SceneLib, MainView, CorruptedDrider, AppearanceDefs, UseableLib, StatusAffects, Appearance, Utils, PregnancyStore, kFLAGS, Combat, CoC, EngineCore, Descriptors ) {
	...
	CorruptedDriderScene.prototype.driderEncounter = function() {
		...
		//First time;
		if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00277 ] === 0 ) {
	...
	CorruptedDriderScene.prototype.defeatDriderIntro = function() {
		EngineCore.spriteSelect( 77 );
		EngineCore.outputText( '', true );
		EngineCore.outputText( 'All eight of the drider\'s black and ' + CoC.monster.hairColor + ' striped legs collapse under her weight while she struggles ', false );
		if( CoC.monster.HP < 1 ) {
		...
		if( CoC.player.cockThatFits( CoC.monster.analCapacity() ) >= 0 ) {
		...
		if( CoC.player.hasCock() && CoC.player.lust >= 33 ) {
			...
			if( CoC.player.cockThatFits( 12 ) >= 0 ) {
		...
		var rideOvi = null;
		var bikiniTits = null;
		if( CoC.player.hasVagina() && CoC.player.biggestTitSize() >= 4 && CoC.player.armor ) {
	...
	CorruptedDriderScene.prototype.loseToDrider = function() {
		...
		if( CoC.player.gender === 0 || (CoC.player.gender > 0 && Utils.rand( 3 ) === 0) ) {
	...
	CorruptedDriderScene.prototype.winDriderPCDickInSpiderCunt = function() {
		...
		EngineCore.outputText( 'Eager to plumb the depths of this greedy spider\'s snatch, you practically tear your way out of your ' + CoC.player.armorName + ' in your hurry to get naked.  Through it all ', false );
		if( CoC.monster.lust > 99 ) {
			EngineCore.outputText( 'she looks ', false );
			if( CoC.player.tallness > 72 ) {
				EngineCore.outputText( 'up ', false );
			}
			EngineCore.outputText( 'at you with her ' + CoC.monster.skinTone + ' eyes opened wide and her tongue hanging from her mouth, panting lewdly.  Carapace-clad digits sink into her dripping honey-pot over and over while she watches, deep in her own lusty haze.  She retracts her engorged demon-cock so as to have better access to that feminine hole.', false );
		} else {
			EngineCore.outputText( 'she looks ', false );
			if( CoC.player.tallness > 72 ) {
				EngineCore.outputText( 'up ', false );
			}
			EngineCore.outputText( 'at you with a look of disgust in her ' + CoC.monster.skinTone + ' eyes.  She folds her arms across her many-bosomed, quadratically endowed chest with disdain', false );
			if( CoC.monster.nipplesPierced > 0 ) {
		...
		EngineCore.outputText( '  The more you fuck the arachnid hussy\'s wanton slit, the more animated she becomes, until she finally breaks her wordless silence by crying, "<i>Yes ' + CoC.player.mf( 'master', 'mistress' ) + ', dominate me!  I\'ve been such a naughty bitch.  Use me like your cheap whore!  Fuck my holes full of cum, and leave me defiled in a puddle of spunk-laced mud!</i>"\n\n', false );
		EngineCore.outputText( 'Invigorated by the rising tide of desire inside her, the drider pulls her torso up, crushing her tits into your palms so hard that her flesh bulges out through your fingers.  You smile down at the slutty spider and squeeze them hard enough to give her a taste of pain in response.  She pants, and her four ' + CoC.monster.skinTone + '-hued eyes gaze at you, so full of desire that you have a hard time not cumming on the spot while you lock onto those half-lidded, luminous orbs.  You let go of her breasts and reach back to squeeze the smooth skin of her butt, feeling the joint between her human and arachnid halves just under her curvy derriere.\n\n', false );
		...
		EngineCore.outputText( ' breasts mash against your ' + Descriptors.allChestDesc() + ', sweaty skin sliding back and forth and drawing gasps from each of you every time one of her nipples brushes one of yours.  She coos, "<i>Please fuck my slutty drider-cunt harder, ' + CoC.player.mf( 'master', 'mistress' ) + '!  It was so naughty and cruel when I tried to rape you.  Don\'t let me cum... deny me over and over until I\'m your good little spider-pet that\'ll do anything for release!</i>"  She emphasizes every sentence by clamping her quim down on your ' + Descriptors.cockDescript( x ) + ', the dozens of lubricant-soaked folds rolling back and forth, milking you until you cannot help but ejaculate inside her.\n\n', false );
		...
		EngineCore.outputText( '\n\n', false );
		EngineCore.outputText( 'While you\'re distracted by the repeated contrast of tension and relief emanating from ' + Descriptors.sMultiCockDesc() + ', the drider leans up and kisses your neck, humming against your throat as you squirt the last of your orgasmic goop into her vagina.  You slump into her arms, spent by the effort of fucking her so fast and cumming so hard.  As you lie there, her lips sucking hard enough to leave hickeys on your neck with every crushing, passionate kiss, you feel the patterns of glossy fluid tingling on your ' + CoC.player.skin() + '.  A jolt of fear runs up your spine, and you try to push away from her envenomed affections.  It doesn\'t work!  You can barely lift your arms, let alone pull out of the amorous arachnid\'s embrace!\n\n', false );
		EngineCore.outputText( '"<i>Mmmmm,</i>" hums the drider into your collarbone, smearing more of her toxic lip-gloss onto you as she goes.  "<i>Such a good ' + CoC.player.mf( 'master', 'mistress' ) + '!  You punished me so well!  Now just relax... I\'ve been dripping out a special venom that\'ll help you teach me the rest of my lesson, and I need to make sure that every... single... drop... slips inside that sexy bloodstream of yours.</i>"\n\n', false );
		...
		if( CoC.player.cor > 80 && CoC.player.lib > 50 ) {
			...
			EngineCore.outputText( 'No, that\'s not right.  You\'re not some rutting beast, you\'re the champion.  Sure, you\'ve got your ' + Descriptors.cockDescript( x ) + ' locked hilt-deep inside her squirming slit, but that doesn\'t mean you\'re some kind of ' + CoC.player.mf( 'stud', 'slut' ) + ' who jumps on a cunt without a second thought, does it?  You blink your eyes while you try to think.  She does have a point though, if you hadn\'t been in such a hurry for some sex, you wouldn\'t be sagging into her tits and feeling so awfully relaxed.\n\n', false );
		}
		...
		EngineCore.outputText( '\n\n', false );
		EngineCore.outputText( 'While it seems to go on and on, your orgasm eventually ends, leaving you to hang limp while your still-hard dick is massaged by a much wetter spider-cunt.  The ' + CoC.monster.hairColor + '-haired beauty dreamily says, "<i>Mmmm... good job, pet.  Now, let that nice hard dick do the thinking for you and keep fucking my hot cunt until it goes dry.  Maybe a few hours after you finish, you\'ll be able to remember that you used to be a ' + CoC.player.mf( 'man', 'woman' ) + ' before you became my livestock.</i>"\n\n', false );
		EngineCore.outputText( 'The blissful arachnid rolls onto her side with you still locked inside her and falls asleep, her ' + CoC.monster.hairColor + ' tresses ', false );
		if( CoC.player.cumQ() <= 1000 ) {
	...
	CorruptedDriderScene.prototype.winVSDriderTakeItsCockInCunt = function() {
		...
		} else if( CoC.player.vaginalCapacity() < CoC.monster.cockArea( 0 ) * 2 ) {
	...
	CorruptedDriderScene.prototype.victoryVSDriderRideOviVaginal = function() {
		...
		CoC.player.slimeFeed();
		EngineCore.outputText( 'Eyeing the small lump of the spider\'s retracted ovipositor, you resolve to have a little fun with it, even if it means giving the spider exactly what she wants in a way that she doesn\'t.  You tear off your ' + CoC.player.armorName + ' while the drider watches with ', false );
		if( CoC.monster.HP < 1 ) {
		...
		EngineCore.outputText( 'Like this, you\'re treated to a perfect view of ALL of the web-woman\'s endowments.  Her four D-cup tits flatten slightly against her torso now that she\'s on her back, hanging slightly to the side.  At her crotch, you can see her glistening, black-lipped sex puffing up, the tip of her large button already peeking out of her tempting box.  Just below that, the black and ' + CoC.monster.hairColor + '-striped exoskeleton of her spider-half starts, merging down into an bulbous, arachnid shape.  At the very bottom of it all is the huge bulk of her abdomen, covered with numerous spinnerets and with a wide, mostly-retracted organ near the back.\n\n', false );
	...
	CorruptedDriderScene.prototype.victoryVSDriderRideOviAnal = function() {
		...
		if( CoC.player.hasCock() ) {
			EngineCore.outputText( 'while your ' + Descriptors.multiCockDescriptLight() + ' dribbles white, wasted goo onto the black and ' + CoC.monster.hairColor + ' striped carapace below', false );
		} else if( CoC.player.hasVagina() ) {
			EngineCore.outputText( 'while your ' + Descriptors.vaginaDescript( 0 ) + ' dribbles fem-slime onto the black and ' + CoC.monster.hairColor + ' striped carapace below', false );
		} else {
	...
	CorruptedDriderScene.prototype.victoryVSDriderStickDickInSpinneret = function() {
		...
		EngineCore.outputText( 'Any satisfaction the drider could have gleaned is quickly replaced with complete trepidation as you swivel her spinneret toward you, grabbing your ' + Descriptors.cockDescript( x ) + ' and lining it up with the onyx protrusion.  She manages to choke out a protest before you plunge into the depths, the tight recesses squeezing you wonderfully.  A strangled, gurgly scream peals from the oversensitized girl, both her arms and legs flailing about.  You push up to your pelvis into her spinneret, then slowly—so slowly—draw back, the sensation leaving the drider to claw deep divots into the mossy ground.  You draw out to the ' + CoC.player.cockHead( x ) + ', then thrust back in, taking your sweet time with the motions.  Her eyes roll back into her head, and another gurgle rumbles in her throat.  You have to wonder if she can even form coherent thoughts, in this state.  No matter; she insulted you, after all.  She begins hissing something through her clenched teeth, and while unintelligible at first, you can eventually make out, "<i>Fuck you... fuck you... fuck you...</i>" like some sort of vulgar mantra.\n\n', false );
		...
		if( CoC.player.biggestTitSize() >= 3 ) {
			EngineCore.outputText( '  The semi-sticky stuff soaks into the cleavage of your ' + CoC.player.allBreastsDescript() + ', the feeling surprisingly enjoyable.  You even knead the web in a bit, biting your bottom lip from the sensation.', false );
		}
	...
	CorruptedDriderScene.prototype.loseVSDriderGetAnalledOvi = function() {
		...
		if( CoC.player.tallness < 66 ) {
			EngineCore.outputText( CoC.player.face() + ', sliding down across your cheeks and along your lips', false );
		} else {
	...
	CorruptedDriderScene.prototype.loseVSDriderAndVaginallyImpreggleOvi = function() {
		...
		if( CoC.player.isBiped() || CoC.player.isGoo() ) {
		...
		if( !CoC.player.isNaga() ) {
			EngineCore.outputText( 'pull your own ' + CoC.player.feet() + ' apart', false );
		} else {
		...
		if( !CoC.player.isNaga() ) {
			...
			EngineCore.outputText( CoC.player.legs() + ' and pinning you in that prone, separated position', false );
		} else {
		...
		if( !CoC.player.isTaur() ) {
			...
			if( CoC.player.biggestTitSize() > 4 ) {
				EngineCore.outputText( 'With pain and then a pop, your ' + CoC.player.allBreastsDescript() + ' squash out to the sides as she presses your torso into your knees.  ', false );
			}
		...
		if( CoC.player.hasCock() && CoC.monster.nipplesPierced > 0 ) {
	...
	CorruptedDriderScene.prototype.loseVSDriderAndGetFuckedANDAnalImpreggegity = function() {
		...
		if( CoC.player.lust > 99 ) {
			EngineCore.outputText( 'Unable to resist the desire coursing through your body, you strip off your ' + CoC.player.armorName + ' and start stroking ' + Descriptors.sMultiCockDesc() + ', displaying it in the hope that it will draw the drider\'s attention, while at the same time just trying to get yourself off and attain release.', false );
		} else {
			EngineCore.outputText( 'You slump down, too beat up to mount an effective defense, let alone stay up on your ' + CoC.player.feet() + '.  The drider skitters over to you and effortlessly peels you out of your ' + CoC.player.armorName + ', exposing your ', false );
			...
			EngineCore.outputText( ' to the air.  She squeezes ' + CoC.player.oMultiCockDesc() + ' in her hand and pumps it, smiling while you stiffen and twitch precipitously in her grasp.', false );
		}
