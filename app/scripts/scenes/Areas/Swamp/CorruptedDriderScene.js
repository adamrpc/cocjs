...
angular.module( 'cocjs' ).run( function( CorruptedDrider, AppearanceDefs, UsableLib, StatusAffects, Appearance, Utils, PregnancyStore, kFLAGS, Combat, CoC, EngineCore, Descriptors ) {
	...
	CorruptedDriderScene.prototype.driderEncounter = function() {
		...
		//repeat intro:;
			...
			EngineCore.outputText( 'pulls you back halfway, but as you open your mouth to yell, the ground gives a quiet thump.  Just barely in your periphery, you see a white apparition with a yawning mouth stretching out to bite, and your reflexes take over, dropping your weight and rolling you away from it.\n\n', false );
			EngineCore.outputText( '"<i>Clever little pet,</i>" the phantom spits, resolving into the shape of a drider as your eyes focus and your adrenaline haze clears.  Raising a cupped hand to her ghoulishly grinning mouth, she laughs into it; a minute shivering in the air reveals the taut threads of silk attached to her finger.  The vibrations travel down them, shaking several silk-tied leaves and producing the raspy, laughing audience from before.  "<i>It\'d be so much easier on you if you\'d just walked into the trap, sweetness.</i>"\n\n', false );
			EngineCore.outputText( 'You\'ve yet to meet a wild drider that let you walk away without some sadistic display of power, and this one looks to be no different.', false );
		}
		CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00277 ]++;
	...
	CorruptedDriderScene.prototype.defeatDriderIntro = function() {
		EngineCore.spriteSelect( 77 );
		EngineCore.outputText( '', true );
		EngineCore.outputText( 'All eight of the drider\'s black and ' + CoC.getInstance().monster.hairColor + ' striped legs collapse under her weight while she struggles ', false );
		if( CoC.getInstance().monster.HP < 1 ) {
			EngineCore.outputText( 'to try to continue the fight, her movements slowed by pain and weakness.', false );
		} else {
			EngineCore.outputText( 'to masturbate her swollen pussy-lips and the thick, juicy demon-cock that\'s sprouted in place of her clit.  She\'s so focused on getting off that she utterly gives up on the idea of fighting back, let alone dominating you as she seemed to plan originally.', false );
		}
		EngineCore.outputText( '  The spider-herm lies in a miserable, defeated heap.', false );
		var buttFuckBUTTFUCKBUTTTFUCKBUTTFUCK = null;
		if( CoC.getInstance().player.cockThatFits( CoC.getInstance().monster.analCapacity() ) >= 0 ) {
			buttFuckBUTTFUCKBUTTTFUCKBUTTFUCK = this.buttFuckADriderOhBaby;
		}
		var vagFuck = null;
		var fuckSpinner = null;
		var careful = null;
		if( CoC.getInstance().player.hasCock() && CoC.getInstance().player.lust >= 33 ) {
			if( CoC.getInstance().player.cockThatFits( CoC.getInstance().monster.vaginalCapacity() ) >= 0 ) {
				...
				if( CoC.getInstance().player.tailType === AppearanceDefs.TAIL_TYPE_SPIDER_ADBOMEN || CoC.getInstance().player.hasItem( UsableLib.T_SSILK ) ) {
			...
			if( CoC.getInstance().player.cockThatFits( 12 ) >= 0 ) {
		...
		var rideOvi = null;
		var bikiniTits = null;
		if( CoC.getInstance().player.hasVagina() && CoC.getInstance().player.biggestTitSize() >= 4 && CoC.getInstance().player.armor ) {
	...
	CorruptedDriderScene.prototype.loseToDrider = function() {
		...
		if( CoC.getInstance().player.gender === 0 || (CoC.getInstance().player.gender > 0 && Utils.rand( 3 ) === 0) ) {
			select[ select.length ] = 2;
		}
		//SELECT SCENE HERE;
		var choice = select[ Utils.rand( select.length ) ];
	...
	CorruptedDriderScene.prototype.winDriderPCDickInSpiderCunt = function() {
		...
		EngineCore.outputText( 'Eager to plumb the depths of this greedy spider\'s snatch, you practically tear your way out of your ' + CoC.getInstance().player.armorName + ' in your hurry to get naked.  Through it all ', false );
		if( CoC.getInstance().monster.lust > 99 ) {
			...
			EngineCore.outputText( 'at you with her ' + CoC.getInstance().monster.skinTone + ' eyes opened wide and her tongue hanging from her mouth, panting lewdly.  Carapace-clad digits sink into her dripping honey-pot over and over while she watches, deep in her own lusty haze.  She retracts her engorged demon-cock so as to have better access to that feminine hole.', false );
		} else {
			EngineCore.outputText( 'she looks ', false );
			if( CoC.getInstance().player.tallness > 72 ) {
				EngineCore.outputText( 'up ', false );
			}
			EngineCore.outputText( 'at you with a look of disgust in her ' + CoC.getInstance().monster.skinTone + ' eyes.  She folds her arms across her many-bosomed, quadratically endowed chest with disdain', false );
			if( CoC.getInstance().monster.nipplesPierced > 0 ) {
				EngineCore.outputText( ', the chains jingling loudly as she does so', false );
			}
			EngineCore.outputText( '.  In spite of keeping a stiff upper lip about it, her lower ones visibly quiver while you strip.', false );
		}
		EngineCore.outputText( '  You saunter over to the limp-legged drider and run your hands down her inhuman, arachnid body, exploring the smooth, shiny carapace.\n\n', false );
		EngineCore.outputText( 'The corrupted monster-girl sighs and shudders at your touches, as if she can somehow feel your finger-tips through the rock-hard exoskeleton.  From the way she\'s trying to hold in her exhalations, you can only guess that it must feel wonderful, so you keep it up, circling and touching every part of her exotic anatomy.  By the time you\'ve circled around her entire body, she\'s ', false );
		if( CoC.getInstance().monster.HP < 1 ) {
			EngineCore.outputText( 'giving up little, lewd moans', false );
		} else {
			EngineCore.outputText( 'breathing out lurid moans', false );
		}
		EngineCore.outputText( ' of pleasure and her ', false );
		if( CoC.getInstance().monster.nipplesPierced > 0 ) {
			EngineCore.outputText( 'chain-pierced-', false );
		}
		EngineCore.outputText( 'nipples and clit look as hard as her natural body-armor.  You boldly grope one of her ', false );
		if( CoC.getInstance().monster.nipplesPierced > 0 ) {
			EngineCore.outputText( 'pierced ', false );
		}
		EngineCore.outputText( 'breasts, feeling the supple, soft flesh shift while you roughly palm it and rub your thumb down a dark nipple.  You let the sensitive bud go and watch it spring back up, enjoying the drider\'s gasp of surprised pleasure for the brief moment before she muffles herself.\n\n', false );
		EngineCore.outputText( 'Still fondling one of the four nicely-sized tits, you push the spider-bitch back, admiring the drider\'s flexibility as she leans back over her inhuman lower body, giving you an even better view of her intoxicatingly sweet-smelling pussy.  You take ' + CoC.getInstance().player.oMultiCockDesc() + ' in hand and rub the ' + CoC.getInstance().player.cockHead( x ) + ' along the slick lips of her pussy, shuddering slightly as her wetness rubs the sensitive glans.  Gasping, you slowly spread her molten-hot pussy around your girth, reveling in the tight embrace of her slick walls.  The drider squirms in your grip with her breasts bouncing deliciously and whole body trembling.  Spreading as easily as her cunt, the drider\'s venom-glossed lips slowly open into a wordless \'o\' of rapturous pleasure.\n\n', false );
		EngineCore.outputText( 'Half an inch at a time, you sink ever-deeper inside the seemingly bottomless pit that is the spider-girl\'s steamy sex.  Reveling in the taming of the aggressive predator\'s sticky snatch, you postpone the eventual hilting as long as possible.  For her part, she doesn\'t seem to mind you having your way with her.  All eight of her spindly legs are scrabbling pitifully in the dirt, doing little more than nervelessly convulsing in uncontrolled pleasure.  The spider-woman\'s arms close behind your back, the sharp tips of her nails trying to claw deep furrows into your ' + CoC.getInstance().player.skinFurScales() + '.  At long last, your waist presses against her own to feel the oozing slipperiness of her nethers.  Your ' + Descriptors.cockDescript( x ) + ' is totally buried in her tunnel, and now that you\'re holding still, you can feel the tiny folds inside her vagina slowly moving and gliding along your length, stroking your ' + Descriptors.cockDescript( x ) + ' like dozens of tongues.\n\n', false );
		EngineCore.outputText( 'Compelled by your body\'s instincts, you draw back your ' + Descriptors.hipDescript() + ' to expose the bulk of your pussy-soaked shaft.  The spider-woman moans disappointedly, her voice filled with so much wanton desire that your ' + CoC.getInstance().player.cockHead( x ) + ' twitches in response.  You fondle the drider\'s lower pair of breasts with both hands, circling and squeezing the delectable mounds while you begin to repeatedly impale her squelching pussy, watching the upper row of pale breasts bounce and jiggle above your busy fingers.', false );
		if( CoC.getInstance().monster.nipplesPierced > 0 ) {
			EngineCore.outputText( '  The chains binding her nipples make a delightful clinking noise with each squeeze and stroke.', false );
		}
		EngineCore.outputText( '\n\n', false );
		EngineCore.outputText( 'Meanwhile, the drider\'s swollen clit prods at your loins with each penetrating thrust, leaving lines of sticky spider-juice all over your crotch.  ', false );
		if( CoC.getInstance().player.cockTotal() > 1 ) {
			EngineCore.outputText( 'Your extra penis', false );
			if( CoC.getInstance().player.cockTotal() > 2 ) {
				EngineCore.outputText( 'es fan out to the side, forgotten while you give yourself over to the silken caresses of her glorious vagina.', false );
			} else {
				EngineCore.outputText( ' sticks out to the side, forgotten while you give yourself over to the silken caresses of her glorious vagina.', false );
			}
		} else {
			EngineCore.outputText( 'Your ' + Descriptors.cockDescript( x ) + ' seems to bulge inside her, giving itself over to the silken caresses of her glorious vagina.', false );
		}
		EngineCore.outputText( '  The more you fuck the arachnid hussy\'s wanton slit, the more animated she becomes, until she finally breaks her wordless silence by crying, "<i>Yes ' + CoC.getInstance().player.mf( 'master', 'mistress' ) + ', dominate me!  I\'ve been such a naughty bitch.  Use me like your cheap whore!  Fuck my holes full of cum, and leave me defiled in a puddle of spunk-laced mud!</i>"\n\n', false );
		EngineCore.outputText( 'Invigorated by the rising tide of desire inside her, the drider pulls her torso up, crushing her tits into your palms so hard that her flesh bulges out through your fingers.  You smile down at the slutty spider and squeeze them hard enough to give her a taste of pain in response.  She pants, and her four ' + CoC.getInstance().monster.skinTone + '-hued eyes gaze at you, so full of desire that you have a hard time not cumming on the spot while you lock onto those half-lidded, luminous orbs.  You let go of her breasts and reach back to squeeze the smooth skin of her butt, feeling the joint between her human and arachnid halves just under her curvy derriere.\n\n', false );
		EngineCore.outputText( 'Heaving mightily, you use the submissive spider\'s ass-cheeks as hand-holds for a fast, hard fuck.  Her four ', false );
		if( CoC.getInstance().monster.nipplesPierced > 0 ) {
			EngineCore.outputText( 'pierced', false );
		} else {
			EngineCore.outputText( 'white-skinned', false );
		}
		EngineCore.outputText( ' breasts mash against your ' + Descriptors.allChestDesc() + ', sweaty skin sliding back and forth and drawing gasps from each of you every time one of her nipples brushes one of yours.  She coos, "<i>Please fuck my slutty drider-cunt harder, ' + CoC.getInstance().player.mf( 'master', 'mistress' ) + '!  It was so naughty and cruel when I tried to rape you.  Don\'t let me cum... deny me over and over until I\'m your good little spider-pet that\'ll do anything for release!</i>"  She emphasizes every sentence by clamping her quim down on your ' + Descriptors.cockDescript( x ) + ', the dozens of lubricant-soaked folds rolling back and forth, milking you until you cannot help but ejaculate inside her.\n\n', false );
		EngineCore.outputText( 'Your back arches in orgasmic pleasure, fingers squeezing her butt tightly enough that your pale lover\'s derriere will show your hand-prints for some time.  She shudders slightly as your ' + Descriptors.cockDescript( x ) + ' is forced as far inside her lewdly-textured vagina as possible, milking your penis with repeated strokes while your urethra bulges with the first load.  You release what feels like a torrent into her passage; the heavy spurt is quickly swallowed by the drider, disappearing deep inside her as if drawn away on an unseen current.  The sudden suction and increase in the movement of her folds makes your next rope of jizz feel even bigger, but it too disappears into the drider\'s seemingly-bottomless body.', false );
		if( CoC.getInstance().player.cumQ() > 1000 ) {
			EngineCore.outputText( '  Amazingly, you actually manage to cum enough to fill her massive womb, and after the first few squirts, the cum slowly bubbles out around your shaft with each fresh deposit of jism.', false );
		}
		EngineCore.outputText( '\n\n', false );
		EngineCore.outputText( 'While you\'re distracted by the repeated contrast of tension and relief emanating from ' + Descriptors.sMultiCockDesc() + ', the drider leans up and kisses your neck, humming against your throat as you squirt the last of your orgasmic goop into her vagina.  You slump into her arms, spent by the effort of fucking her so fast and cumming so hard.  As you lie there, her lips sucking hard enough to leave hickeys on your neck with every crushing, passionate kiss, you feel the patterns of glossy fluid tingling on your ' + CoC.getInstance().player.skin() + '.  A jolt of fear runs up your spine, and you try to push away from her envenomed affections.  It doesn\'t work!  You can barely lift your arms, let alone pull out of the amorous arachnid\'s embrace!\n\n', false );
		EngineCore.outputText( '"<i>Mmmmm,</i>" hums the drider into your collarbone, smearing more of her toxic lip-gloss onto you as she goes.  "<i>Such a good ' + CoC.getInstance().player.mf( 'master', 'mistress' ) + '!  You punished me so well!  Now just relax... I\'ve been dripping out a special venom that\'ll help you teach me the rest of my lesson, and I need to make sure that every... single... drop... slips inside that sexy bloodstream of yours.</i>"\n\n', false );
		EngineCore.outputText( 'You wiggle futilely, using the last of your strength to try to break free, but it\'s no use - she\'s too strong, or the venom has made you too weak.  Dimly, you\'re aware of your ' + Descriptors.cockDescript( x ) + ' pulsating inside her, hard as iron and burning hot with lust, even though you just came.  The spider-woman continues to kiss you, and with every peck of her lips, you feel yourself sagging deeper into her embrace until you lay your head down on her breast, your only movements the slight twitches of your body as it tries to deal with the poison-induced need to copulate.  You feel the warmth of her breath on your neck when she stops, cradling you in her arms.\n\n', false );
		EngineCore.outputText( 'The drider cocks her head to the side so you can see her luminous ' + CoC.getInstance().monster.skinTone + ' eyes boring into you while she whispers, "<i>Don\'t try to fight it, dear.  While there\'s enough of my normal venom in there to keep your ' + Descriptors.cockDescript( x ) + ' hard enough to cum a few more times for me, the real magic is in the secondary chemicals my demonic friends were kind enough to teach me how to excrete.  You can feel it relaxing your muscles can\'t you?  Well, the beauty is that it isn\'t just your muscles relaxing... your mind is getting nice and relaxed too.  So open and pliable that you\'ll soon be a horny puddle of putty for me to play with.</i>"\n\n', false );
		EngineCore.outputText( 'Gods, she\'s right!  You started to nod while she was talking, automatically agreeing with every sibilant syllable.  Trying to shake your head in denial, you only succeed at bouncing the jiggling tit-flesh beneath you.  You have to resist!  You beat her!  She can\'t do this!\n\n', false );
		...
		if( CoC.getInstance().player.cor > 80 && CoC.getInstance().player.lib > 50 ) {
			EngineCore.outputText( 'Of course it\'s the truth, the cunt\'s clearly got a good grasp on the obvious. Unwilling to lose your grip on dominance, however, you try to keep struggling to get the upper hand.\n\n', false );
		} else {
			EngineCore.outputText( 'No, that\'s not right.  You\'re not some rutting beast, you\'re the champion.  Sure, you\'ve got your ' + Descriptors.cockDescript( x ) + ' locked hilt-deep inside her squirming slit, but that doesn\'t mean you\'re some kind of ' + CoC.getInstance().player.mf( 'stud', 'slut' ) + ' who jumps on a cunt without a second thought, does it?  You blink your eyes while you try to think.  She does have a point though, if you hadn\'t been in such a hurry for some sex, you wouldn\'t be sagging into her tits and feeling so awfully relaxed.\n\n', false );
		}

		EngineCore.outputText( 'The drider croons, "<i>Come on, don\'t delude yourself.  If you weren\'t such a dominating, rape-hungry beast you wouldn\'t be nuzzling against my tits and giving me little nods of agreement.  Honestly, how could you ever think of yourself as anything different when your brain just does whatever your dick tells it to?  You just got off and might have even knocked me up, but your penis is even harder than before.</i>"\n\n', false );
		if( CoC.getInstance().player.cor > 80 && CoC.getInstance().player.lib > 50 ) {
			EngineCore.outputText( 'You grumble out a denial.  You\'re not dumb, you just like to take charge and fuck. There\'s no reason to resist jumping on a nice wet pussy and making it squirt while you pack it full... but you can\'t quite get your limbs to move.\n\n', false );
		} else {
			EngineCore.outputText( 'You mumble out a denial.  You\'re not a beast... you just like to take charge and fuck.  It\'s just so hard to resist jumping on a nice wet pussy and making it squirt while you pack it full... no!  That\'s not what...\n\n', false );
		}

		EngineCore.outputText( '"<i>Oooh, are you leaking out more pre-cum already?  You\'re insatiable!  I bet you start thinking about your next fuck before you even finish getting off, don\'t you?  You can deny it all you want, but I felt your dick puff up a little bit more as soon as I said it.  No matter what you think, your cock is a dumb, happy little tattle-tale that always tells the girls exactly what you\'re thinking.</i>"', false );
		if( CoC.getInstance().player.statusAffectv1( StatusAffects.Exgartuan ) === 1 ) {
			EngineCore.outputText( '  Exgartuan mumbles telepathicaly, "<i>Fuck yes I do, but I wanna see how this plays out.</i>"', false );
		}
		EngineCore.outputText( '  She adds, "<i>Face the music - there\'s nothing you love more than defeating someone and letting your dick tell you what to do.  You dumb... domineering... beast...</i>"\n\n', false );
		EngineCore.outputText( 'Gods!  It\'s getting so hard to think with that tight cunt clutching at your cock and the soft, feminine enunciations constantly interrupting your thoughts.  How can she concentrate with the stink of sex hanging so thick in the air?  Your ' + Descriptors.cockDescript( x ) + ' is so horny, why aren\'t you fucking her yet?  You shudder weakly and remember ', false );
		if( CoC.getInstance().player.cor > 80 && CoC.getInstance().player.lib > 50 ) {
			EngineCore.outputText( 'that it\'s because of her own venom paralyzing your muscles so effectively.\n\n', false );
		} else {
			EngineCore.outputText( 'there was something wrong about what she was saying... something important.\n\n', false );
		}

		EngineCore.outputText( 'The drider pauses to gaze into your eyes with all four of hers.  She runs a hand through your hair while making gentle shushing noises at you before she resumes her diatribe.  "<i>There there, I know how eager you are to fuck me into my place.  I\'m just another wanton harlot waiting to be forced on her back and turned into a fresh cock-sleeve for you, aren\'t I?  You recognize the absolute truth of my words.  Every time I talk you can feel the words bouncing around inside that empty head of yours.</i>"\n\n', false );
		EngineCore.outputText( 'You nod, actually moving your head against her pillowy breasts as you feel your strength starting to return.  Why is she wasting so much time telling you things you already know?  This bitch\'s cunt is like a white-hot furnace of lust, and your cock is so tired of having to sit still.  You can\'t wait to recover from the venom and fuck her so hard she\'ll black out from the ensuing orgasm.  Then you\'ll fuck her a few more times until your cock finally gets tired of her sloppy, cream-pied pussy.\n\n', false );
		EngineCore.outputText( '"<i>Oooh, that dick is just getting harder and harder isn\'t it?  I bet it feels like it\'s about to burst and cum, even though you\'re not able to punish me like you want to yet.  Maybe you secretly get turned on by being denied?  Oh!  I just felt your dick jump!  So you want to go around, letting your dick tell you to dominate every horny cunt you see, but the thing that turns you on more than anything else is having a woman turn the tables on you, isn\'t it?</i>"\n\n', false );
		EngineCore.outputText( 'You moan, "<i>N-no...</i>" but a small burst of pre-cum chooses that moment to erupt from your ' + Descriptors.cockDescript( x ) + ', still nestled inside the squirming spider-cunt.  It feels heavenly, and while you try to struggle in your weakened state, the warm embrace of her pussy\'s many cock-milking folds is turning your penis into a cum-drooling behemoth on the edge of exploding.  Oh gods, is she going to do it?  Damn, you want to put her in her place, but you\'re so weak right now.  Even worse, her pussy has you so close, so very close.\n\n', false );
		EngineCore.outputText( 'The chitinous, oh-so-right bitch pulls your head up to gaze deeply into her eyes.  "<i>You have a horny, naughty cock, and you always listen to it.  This rebellious member gets you into so much trouble, but you\'ll never stop listening to it.  You\'ll keep chasing every wet pussy you see, because you\'re just a horny animal.  A rutting, horny animal.  But I know your secret.  Your dick really wants a girl to turn the tables on you, paralyze you, and tease your cock mercilessly.  It feels so good to let me touch your body, stroke it, tease it, and squeeze it.  You won\'t even fight any more.  My pussy has your cock so close, so very close, and you\'re going to cum when I tell you.  You\'ll blow your load on command for me, my servile, secretly submissive mount.</i>"\n\n', false );
		EngineCore.outputText( 'A plaintive whining, almost exactly like that of an animal, vocalizes subconsciously.  Your ' + Descriptors.cockDescript( x ) + ' feels afire, constantly leaking pre-cum while you\'re held immobile in your spidery mistress\' arms; you lick your lips and start to beg, mumbling out your desire to cum for her, anything for her, so long as she lets your dick orgasm again and again.  You want to climax so bad, but you want her to control your cock so much more.  It\'s always getting you in trouble, and it feels so good to let her use you as her cum-drooling mount.\n\n', false );
		EngineCore.outputText( 'The spider-woman giggles and surprises you by kissing you on the lips, holding you against her while her narrow tongue smears her venom all over your lips and tongue, staining them purple.  You exhale happily through your nose, feeling perfectly, utterly relaxed and yet aroused beyond any reasonable measure.  The squeezing caresses around your dick keep you happily squirting heavy dollops of pre-cum.  You\'ve no doubt it must look like some kind of over-stuffed sausage by this point, bloated beyond reason.\n\n', false );
		EngineCore.outputText( 'When she pulls away, her face is covered in a huge blush, and she\'s panting out loud and interrupting her words with quiet moans as she talks, "<i>That\'s a good ' + CoC.getInstance().player.mf( 'boy', 'girl' ) + '.  Even though you\'re a stupid pussy-slave, bound to let your cock lead you into every hungry snatch you see, you know how to obey a smarter woman\'s cunt, don\'t you?  Let me hear you beg like the animal you are.  Whine for mistress while her cunt fucks your dick, show me with every fiber of your being how much you love to submit, to obey, and to beg to be allowed to cum.</i>"\n\n', false );
		EngineCore.outputText( 'She lets your head hang limply, and all you can do is whine, and try to look up at her with the most pathetic expression in your eyes.  You\'re beaten - she\'s so much smarter than you and she\'s got you by your dick.  Why didn\'t you think?  ', false );
		if( CoC.getInstance().player.cor > 80 && CoC.getInstance().player.lib > 50 ) {
			EngineCore.outputText( 'Gods it was a bad idea not to bind her lips... ', false );
		} else {
			EngineCore.outputText( 'Gods this was a bad idea... ', false );
		}
		EngineCore.outputText( 'still, at least she\'s controlling your rebellious dick, teaching the little prick a lesson while letting you feel so good.\n\n', false );
		EngineCore.outputText( 'The drider moans louder and responds, "<i>Yes, oooh, good ' + CoC.getInstance().player.mf( 'boy', 'girl' ) + '.  You\'re so hard, so very hard.  You\'re oozing out cum even though you\'re not allowed to fuck me, and you love it.  I bet you don\'t even mind not getting to orgasm - so long as you\'re helpless and hilt-deep in a cunt, you\'re happy, aren\'t you?  Oohhh, I\'m gonna... gonna... cum soon.  Luckily for you, I want you to squirt inside me.  I want that cock creaming my pussy as soon as I... ohhhh... so good... s-soon as I command it...  You\'ll do that for me, won\'t you?  You\'ll obey me completely, utterly, and pump every drop of your jism into my soon-to-be-pregnant pussy, right?</i>"\n\n', false );
		EngineCore.outputText( 'You mewl out an affirmation, forgetting how to speak with all the venom pumping through you and the mind-crushing levels of arousal pouring out of your ' + Descriptors.cockDescript( x ) + '.\n\n', false );
		EngineCore.outputText( 'The spider-woman shudders from head to all eight of her \'toes\', her four eyelids closing while her mouth gasps out, "<i>Cum for me.</i>"\n\n', false );
		EngineCore.outputText( 'The first spurt of pent-up seed splatters the spider\'s womb before you even realize she gave you permission, painting the squirming walls white.  You growl and cum, releasing spurt after spurt of spunk while her pussy seems to drink it all down', false );
		if( CoC.getInstance().player.cumQ() > 1000 ) {
			EngineCore.outputText( ', even though you\'ve packed it full once already', false );
		}
		EngineCore.outputText( '.  Her legs skitter and tremble, her pussy convulses, and your dick gets the squeezing, sucking orgasm of a lifetime.  It feels like you\'re pumping out two or three orgasms worth of spooge, each dick-stretching explosion of cream dilating your slit almost painfully wide.', false );
		if( CoC.getInstance().player.cumQ() > 1000 ) {
			EngineCore.outputText( '  The stuff dribbles out around your tool along with a mix of girl-lube, too much even for her prodigious pussy to contain.', false );
			if( CoC.getInstance().player.cumQ() > 2000 ) {
				EngineCore.outputText( '  The flow thickens into a river, and soon the puddle of cum looks more like a pond, turning the dirt into a soup of muddy cum.', false );
			}
		}
		EngineCore.outputText( '\n\n', false );
		EngineCore.outputText( 'While it seems to go on and on, your orgasm eventually ends, leaving you to hang limp while your still-hard dick is massaged by a much wetter spider-cunt.  The ' + CoC.getInstance().monster.hairColor + '-haired beauty dreamily says, "<i>Mmmm... good job, pet.  Now, let that nice hard dick do the thinking for you and keep fucking my hot cunt until it goes dry.  Maybe a few hours after you finish, you\'ll be able to remember that you used to be a ' + CoC.getInstance().player.mf( 'man', 'woman' ) + ' before you became my livestock.</i>"\n\n', false );
		EngineCore.outputText( 'The blissful arachnid rolls onto her side with you still locked inside her and falls asleep, her ' + CoC.getInstance().monster.hairColor + ' tresses ', false );
		if( CoC.getInstance().player.cumQ() <= 1000 ) {
			EngineCore.outputText( 'splayed across the ground', false );
		} else {
			EngineCore.outputText( 'soaking in the white puddle of spooge', false );
		}
	...
	CorruptedDriderScene.prototype.winDriderPCDickInSpiderCunt2 = function() {
		...
		EngineCore.spriteSelect( 77 );
		EngineCore.outputText( '', true );
		EngineCore.outputText( '<b>Sometime Later...</b>\n', false );
		EngineCore.outputText( 'Pistoning your hips violently, you continue to ravage the unconscious spider, fucking her in a brutal, beast-like manner that centers entirely upon your own pleasure.  You cum, dumping a fresh batch of jizz into her already-stuffed twat, grinning when a few more rivulets of ooze roll over her inflamed pussy-lips to drip into the growing puddle.  Still hard, ' + Descriptors.sMultiCockDesc() + ' continues to twitch, hungry for more, and you don\'t deny it.  You grab the arachnid bitch and stuff your dick into her mouth, shivering while her venom seeps into your dick, keeping you hard and ready to blow.  Whatever special, immobilizing venom she had earlier, it seems exhausted, and the only response your body has to the onslaught of toxin is to erupt, spouting a huge load of semen into her mouth.\n\n', false );
		EngineCore.outputText( 'A low growl rips through your throat as you pull back and plaster her face white, coating the caked-on residue of a few previous loads with fresh moisture.  You shift position and force your ' + Descriptors.cockDescript( x ) + ' between her two pairs of breasts, sandwiching it in there for the titty-fuck of a lifetime.  Pistoning back and forth, humping the glorious orbs relentlessly, you give yourself over completely to your sore dick, letting your world dissolve into the feelings of each of the spider-woman\'s orifices around your member.\n\n', false );
		EngineCore.outputText( '<b>Even later...</b>\n', false );
		EngineCore.outputText( 'You collapse into the dirt, not even caring that it stinks of dried cum and girl-jizz.  You\'ve completely emptied out every speck of sperm from your ' + Descriptors.ballsDescriptLight() + ', and you even kept going for a while after that.  It wasn\'t until ' + Descriptors.sMultiCockDesc() + ' went soft at last that you found enough peace to lie down.  ', false );
		if( CoC.getInstance().player.cor > 80 && CoC.getInstance().player.lib > 50 ) {
			EngineCore.outputText( 'The longer you sit there, the more your strength returns to you, and the more the submissiveness placed into you by her venom recedes. Still, you\'ve tired yourself out too much to leave at the moment.', false );
		} else {
			EngineCore.outputText( 'The longer you sit there, the more you remember of yourself, and the more you forget the animal, the rut-beast that the spider-woman placed inside you.', false );
		}
		EngineCore.outputText( '  You shiver and wrap your arms around her, snuggling against her while a primal, buried part of your brain insists that she\'s YOURS.  You fall asleep like that, resting dreamlessly even through your mate\'s waking and departure.', false );
		EngineCore.hideUpDown();
		CoC.getInstance().monster.lust = 98;
		CoC.getInstance().monster.HP = 2;
		CoC.getInstance().player.lust = 100;
		CoC.getInstance().flags[ kFLAGS.COMBAT_BONUS_XP_VALUE ] = CoC.getInstance().monster.XP;
		Combat.cleanupAfterCombat();
		CoC.getInstance().player.orgasm();
		EngineCore.dynStats( 'lib', 1, 'sen', 1 );
	};
	...
	CorruptedDriderScene.prototype.victoryVSDriderRideOviVaginal = function() {
		...
		CoC.getInstance().player.slimeFeed();
		EngineCore.outputText( 'Eyeing the small lump of the spider\'s retracted ovipositor, you resolve to have a little fun with it, even if it means giving the spider exactly what she wants in a way that she doesn\'t.  You tear off your ' + CoC.getInstance().player.armorName + ' while the drider watches with ', false );
		if( CoC.getInstance().monster.HP < 1 ) {
			EngineCore.outputText( 'hate', false );
		} else {
		...
		EngineCore.outputText( 'Like this, you\'re treated to a perfect view of ALL of the web-woman\'s endowments.  Her four D-cup tits flatten slightly against her torso now that she\'s on her back, hanging slightly to the side.  At her crotch, you can see her glistening, black-lipped sex puffing up, the tip of her large button already peeking out of her tempting box.  Just below that, the black and ' + CoC.getInstance().monster.hairColor + '-striped exoskeleton of her spider-half starts, merging down into an bulbous, arachnid shape.  At the very bottom of it all is the huge bulk of her abdomen, covered with numerous spinnerets and with a wide, mostly-retracted organ near the back.\n\n', false );
		EngineCore.outputText( 'The spider\'s look of ', false );
		if( CoC.getInstance().monster.HP < 1 ) {
			EngineCore.outputText( 'hate fades slightly when she sees you eyeing her ovipositor, and with a sigh of annoyance, allows the segmented organ to slowly slide out from her body.  It grows to just over two feet in length, swaying sinuously in the air, though you can tell it isn\'t completely engorged yet.  Still, your ' + Descriptors.vaginaDescript(), false );
			if( CoC.getInstance().player.hasCock() ) {
				EngineCore.outputText( ' and ' + Descriptors.sMultiCockDesc() + ' twitch', false );
			} else {
				EngineCore.outputText( ' twitches', false );
			}
			EngineCore.outputText( ' at the thought of trying to pack all of that meaty, alien pole inside you.\n\n', false );
		} else {
			EngineCore.outputText( 'lust only deepens when she sees you eyeing her ovipositor, and with a twinkle in her eyes, she releases a hiss of pleasure.  The organ pulsates once, then practically explodes before your eyes, lancing just over two feet into the air and waving back and forth sinuously, like a snake.  In a way, the tubular, segmented shaft reminds you of a monstrously thick cock, something that might even give a minotaur pause. Your ' + Descriptors.vaginaDescript(), false );
			if( CoC.getInstance().player.hasCock() ) {
				EngineCore.outputText( ' and  ' + Descriptors.sMultiCockDesc() + ' twitch', false );
			} else {
				EngineCore.outputText( ' twitches', false );
			}
			EngineCore.outputText( ' in anticipation - you can\'t wait to feel that thing going inside you!\n\n', false );
		}
		EngineCore.outputText( 'You sit on top of the chitinous abdomen and grab it in your hands, letting it pulse and squirm in response to your touches.  The copious slime it\'s covered in lets your fingers easily slide down its length.  Each stroke of the odd, feminine member elicits a pleasured groan from your defeated opponent, and you quickly surmise that she must like this just as much as you do using your own genitals.  Pumping and stroking it, you watch with perverse fascination as the ridges grow firmer, hardening in your hand.  A viscous green slime bubbles out of the wide slot at its tip as the spider-girl\'s moans reach a crescendo.  It\'s as lubricated as it\'s going to get.\n\n', false );
		EngineCore.outputText( 'Shifting back a bit, you place the entrance to your ' + Descriptors.vaginaDescript() + ' against the wiggling tip.  The drider sighs and warm green goop gushes inside you eagerly, spreading your canal wide and making you gasp with pleasure.  Subtly, you begin sliding your hips, seeking to soak in more sensation for your slit.  Cooing in delight, the drider grabs at her four ', false );
		if( CoC.getInstance().monster.nipplesPierced > 0 ) {
			EngineCore.outputText( 'pierced ', false );
		}
		EngineCore.outputText( 'tits and begins to grope herself, squeezing hard on her breasts and tugging viciously at her pert, black nipples.  The segmented ovipositor wiggles excitably and plunges forward to bury the first few inches inside your sloppy snatch, the tube dilating and contracting inside you with alien rhythms.', false );
		CoC.getInstance().player.cuntChange( 30, true, true, false );
		EngineCore.outputText( '  You gasp, arching your back and sliding your ' + Descriptors.vaginaDescript() + ' forward, as if your quim could somehow devour the entire length on its own.\n\n', false );
		EngineCore.outputText( 'Once you get about halfway down the drider\'s ovipositor, it stops sliding in so easily, leaving you to hump away at the drider\'s abdomen in a futile quest for further penetration.  You mewl in disappointment and prepare to pull back and fuck the portion you\'ve already taken like no tomorrow.  The flexible shaft undulates, then pistons forward, spear-like.  You feel your ' + Descriptors.vaginaDescript() + ' stretching wide, then relaxing as one of the segmented rings nestles into your canal\'s sopping embrace.  A fresh gush of green slime sprays out of your tightly-stretched labia, strings of the stuff shooting fanwise over the drider\'s glossy, black abdomen.', false );
		CoC.getInstance().player.cuntChange( 50, true, true, false );
		EngineCore.outputText( '\n\n', false );
		EngineCore.outputText( 'Throughout it all, the spider-girl is still mauling her tits, the once-perky nipples now distended and sore from the constant tugging, twisting, and flicking she subjects them to.  The skin of her breasts is rosy and pink, with imprints of her fingers highlighted in red where she\'s squeezed herself too hard.  In between exhaling blissful moans of contentment, the drider pants, "<i>Just like that... yeah.  Ride my fat, egg-filled cock like that, you slut...</i>"\n\n', false );
		EngineCore.outputText( 'You look back in irritation, though your displeasure is stolen away by another fierce push of the woman\'s massive organ.  It stretches you so nicely while it fills you with sloppy green goo, the excess squeezing out from your lips in three squirts as another fat ring pops inside you.  You shudder and look down, watching as the ovipositor begins to wriggle and squirm, twisting about inside you in a relentless drive to impale your ' + Descriptors.vaginaDescript() + ' with its entire length.  Even before you finish adjusting to the last one, the next segment is thrown against you, squeezing out gushes of the verdant egg-lube while it works its way inside you.  Though you\'ve almost got the entire thing lodged deep in your twat at this point, it still manages to squirm back and forth, working half of the ring in at a time, until it too pops into place, stuffing you utterly.\n\n', false );
		EngineCore.outputText( 'Shuddering in what you suppose must be climax, the drider screams and twists, bucking at the air and threatening to dislodge you from your perch.  You hold onto her slippery chitin as well as you can, but you get far more support from the girthy, widening ovipositor inside you.  Each of the thick rings is pressing tightly into your walls, while the circular tip engorges and presses against your cervix, sealing against it in a perfect kiss.  Slowly, you feel your belly growing warm, your uterus filling with the green goo, now with no vent for the pressure.  A few wet, gurgling moments later you start to feel full, but thankfully, the fluid-filling flow tapers off to a slow trickle.  At the same time, the spider stops her gyrations, her whole body holding utterly, completely still.  You look down at her, seeing an orgasmic, slack-jawed expression grow on her face until her eyes begin to roll back.  In that same second, you feel a burgeoning, spherical pressure sliding up your soaking tunnel, rubbing along your walls from your lips all the way to now-stretching cervix.\n\n', false );
		EngineCore.outputText( 'There\'s a moment of terrific, agonizing pain that forces you farther onto your back, and then it vanishes, the only signal of its passage a muffled slosh coming from your belly.  You shiver, feeling strange, unexpected bliss welling up from your loins, just in time for the next egg to traverse your ovipositor-gaped, goo-spewing cunny.  Even before this one can squeeze through your abused cervix, another backs up behind... and another... and another.  Your poor, stretched-out pussy clenches, shooting spasms of pleasure through your whole body that make you grunt and moan.  Lost in your orgasm, you throw your head back to howl, only to find the drider\'s cock, graduated from its larval clitoris, twitching and spewing its foul steaming load directly where you just laid your head.  Several ropes of the demonic jizz land in your ' + Descriptors.hairDescript() + ' and even patter on your forehead, but you can\'t be put out to care right now.', false );
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( '  Your own ' + Descriptors.multiCockDescriptLight() + ' retaliate', false );
			if( CoC.getInstance().player.cockTotal() === 1 ) {
				EngineCore.outputText( 's', false );
			}
			EngineCore.outputText( ' automatically, dumping ', false );
			if( CoC.getInstance().player.cumQ() < 500 ) {
				EngineCore.outputText( 'scores of', false );
			} else {
				EngineCore.outputText( 'unending', false );
			}
			EngineCore.outputText( ' jizz-ropes on your groin and the base of the drider\'s slimy stalk.', false );
		}
		EngineCore.outputText( '  The sounds coming from your throat are gutteral, animalistic, and utterly inhuman, but you\'re too busy cumming... cumming so hard with each orb dropping into your already-packed womb, lost in motherly bliss.\n\n', false );
		EngineCore.outputText( 'Vocalizing like an animal, you climax over and over in a seemingly endless rut, swiveling your hips while your muscles milk the ovipositor, practically pulling the eggs into your hungry womb.  When the last one is packed tightly into your uterus, your ' + Descriptors.vaginaDescript() + ' keeps milking it, a hollow ring of disappointment penetrating your fog as you realize the pleasure has come to an end.  The thick, egg-laying organ slowly shrivels, shrinking down enough that it slides free.  As it\'s pulled out, a river of slime comes with it, released to pour out of your overstuffed cooch.\n\n', false );
		EngineCore.outputText( 'Still in the aftershocks of your orgasm, your muscles go limp, leaving you to slide off the drider into a puddle of your own juices on the ground.  The slime piles out of your punished pussy and pools in small declivities in the ground; you keep trickling more from your loins to swell the tides.  Next to you, the spider-woman is gasping and shuddering, still spurting tainted fluid all over herself from her demonic cock while bursts of lady-cream erupt from her sloppy spider-cunt.  She\'s stopped abusing her breasts and her arms lie immobile beside her; she\'s cumming utterly hands-free from the pleasure of thoroughly impregnating your womb.\n\n', false );
		EngineCore.outputText( 'You watch her for a time, until her orgasms finally die down.  By that point, you\'ve recovered enough to get dressed.  You walk away a little bow-legged, with a new, heavy weight in your belly, just waiting to be born.', false );
		//GET KNOCKED UP;
		//Pregnancy Goes Here;
		CoC.getInstance().player.knockUp( PregnancyStore.PREGNANCY_DRIDER_EGGS, PregnancyStore.INCUBATION_DRIDER, 151 );
		CoC.getInstance().player.slimeFeed();
	...
	CorruptedDriderScene.prototype.victoryVSDriderRideOviAnal = function() {
		...
		EngineCore.outputText( 'The spider\'s look of ', false );
		if( CoC.getInstance().monster.HP < 1 ) {
			EngineCore.outputText( 'hate fades slightly when she sees you eyeing her ovipositor, and with a sigh of annoyance, allows the segmented organ to slowly slide out from her body.  It grows to just over two feet in length, swaying sinuously in the air, though you can tell it isn\'t completely engorged yet.  Still, your ' + Descriptors.assholeDescript(), false );
			if( CoC.getInstance().player.hasCock() ) {
				EngineCore.outputText( ' and ' + Descriptors.sMultiCockDesc() + ' twitch', false );
			} else {
				EngineCore.outputText( ' twitches', false );
		...
		EngineCore.outputText( 'You lean over and grab it in your hands, letting it pulse and squirm in response to your touches.  The copious slime it\'s covered in lets your fingers easily slide down its length.  Each stroke of the odd, feminine member elicits a pleasured groan from your defeated opponent, and you quickly surmise that she must like this just as much as you do using your own genitals.  Pumping and stroking it, you watch with perverse fascination as the ridges grow firmer, hardening in your hand.  A viscous green slime bubbles out of the wide slot at its tip as the spider-girl\'s moans reach a crescendo.  It\'s as lubricated as it\'s going to get.\n\n', false );
		EngineCore.outputText( 'Climbing over the drider\'s bulbous body, you bend over to present your ' + Descriptors.assDescript() + ' to the twitching ovipositor, beckoning her to slide inside your twitching ' + Descriptors.assholeDescript() + ' as you stand atop its owner.  The sprawled-out spider looks up past all of her body to watch you, leaving you waiting.  Nothing happens at first.  You\'re just there, ass in the air and waiting to be violated, encouraging it on your own terms, but she doesn\'t do anything at all.  She simply watches you.  It isn\'t until a scowl of irritation makes its way across your face that she acts.\n\n', false );
		EngineCore.outputText( 'The ovipositor lurches over, bending and flexing to reach up to your exposed hole, barely touching you while it explores your cheeks.   You pant and grab your ' + Descriptors.buttDescript() + ', pulling yourself wide to expose the clenching, dark circle that is your pucker.  It rubs across your hand before finally finding its way into your anal cleavage, ridges sliding between your backside and flopping against your ring while it blindly tries to get adjusted.  The moist, dripping slime it exudes quickly turns your ass into a green-glazed slip-\'n-slide.  A sudden shock of sensation alerts you that yes, the ovipositor has finally found your asshole.\n\n', false );
		EngineCore.outputText( 'The tentacle-like tube prods forward, leaking ooze inside you as the first ring contracts to squeeze inside you.  You gasp when it pops into place, barely adjusting before the next ridge slides into the slightly-loosened sphincter.', false );
		CoC.getInstance().player.buttChange( 30, true, true, false );
		EngineCore.outputText( '  Each segment somehow feels larger than the one preceding it, though it seems impossible given how widely stretched you feel by the time the fourth is inside you.', false );
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( '  The warm organ brushes past your prostate, the slippery caress making ' + Descriptors.sMultiCockDesc() + ' dribble a ', false );
			if( CoC.getInstance().player.cumQ() < 500 ) {
				EngineCore.outputText( 'drop', false );
			} else {
				EngineCore.outputText( 'stream', false );
			}
			EngineCore.outputText( ' of pre-cum down its length.', false );
		}
		if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( '  Your ' + Descriptors.vaginaDescript( 0 ) + ' quivers', false );
			if( CoC.getInstance().player.wetness() >= 3 ) {
				EngineCore.outputText( ' and drips', false );
			}
			EngineCore.outputText( ' sympathetically with the sensation, and your ' + Descriptors.clitDescript() + ' peeks from the wet lips.', false );
		}
		EngineCore.outputText( '  Yet you are not denied the anal pleasure you crave, as the widening bands now filling you are each pushing out harder than the one before.  At the stimulation, her own demonic clit swells as if posessed, nodulates, and hardens into dick form, poking up from her pussy and drooling a slime of pre-cum onto her abdomen.  The second ring rolls over your anal g-spot and you gasp, nearly losing your grip.  The third knocks your ' + CoC.getInstance().player.legs() + ' out from under you, and you collapse atop your partner, squeezing oozing dick', false );
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( 's', false );
		}
		EngineCore.outputText( ' between you', false );
		if( CoC.getInstance().player.clitLength >= 4 && CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( ', and pressing your monster chick-stick into it; the gooey heat sends a tingle of shock up your spine and your pussy convulses', false );
		}
		EngineCore.outputText( '.', false );
		CoC.getInstance().player.buttChange( 50, true, true, false );
		EngineCore.outputText( '\n\n', false );
		EngineCore.outputText( 'Shivering, you glance back up at the defeated woman; she has a passionate, prideful look plastered on her face, though it seems to waver each time another inch of ovipositor pushes past your stretched-out ring.\n\n', false );
		//([not mans];
		if( !CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( '"<i>I never would\'ve thought you were an anal-pregnancy slut,</i>" hisses the spider in pleasure, flexing her organ harder while it curls through your guts, so deep inside you that you wonder how much further she can go.', false );
		} else {
			EngineCore.outputText( '"<i>Just because I have an exoskeleton doesn\'t mean I can\'t feel you dripping on me.  I thought I was the corrupt bitch here, but you... you take the cake.</i>"', false );
		}
		EngineCore.outputText( '\n\n', false );
		EngineCore.outputText( 'With a final hard shove, the last few segments pop inside you', false );
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( ', nearly crushing your prostate from the tight squeeze', false );
		}
		EngineCore.outputText( '.  ', false );
		if( !CoC.getInstance().player.hasCock() && CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( 'You gasp and dribble a few strands of girl-goo, the contractions of your achingly empty pussy intensifying.  ', false );
		}
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( 'You gasp and squirt a few weak ropes of cum, having a miniature orgasm on the spot.  ', false );
		}
		EngineCore.outputText( 'Writhing happily, you sigh ', false );
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( 'while your ' + Descriptors.multiCockDescriptLight() + ' dribbles white, wasted goo onto the black and ' + CoC.getInstance().monster.hairColor + ' striped carapace below', false );
		} else if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( 'while your ' + Descriptors.vaginaDescript( 0 ) + ' dribbles fem-slime onto the black and ' + CoC.getInstance().monster.hairColor + ' striped carapace below', false );
		} else {
			EngineCore.outputText( 'while your fingers tweak your ' + Descriptors.nippleDescript( 0 ) + 's and your starved sphincter sucks at its slimy, squirming stuffing', false );
		}
		EngineCore.outputText( '.  Without warning, the ovipositor dilates, pulling your ' + Descriptors.assholeDescript() + ' even wider.  An explosion of warmth goes off in your guts, and you realize that your term as an incubator has begun with a sense of perverse delight.\n\n', false );
		EngineCore.outputText( 'The large, round egg pushes its way up your butt, squeezes past your ring', false );
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( ' and your prostate', false );
		}
		EngineCore.outputText( ', and travels deep inside you, bulging your belly the slightest bit once it\'s deposited.  With a groan of deep satisfaction, you start humping the fluid-slicked chitin beneath you, barely registering the long, seemingly unending series of eggs that infiltrate your ass', false );
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( ' and milk your prostate as they push past', false );
		}
		EngineCore.outputText( '.\n\n', false );
		EngineCore.outputText( 'The stream of slime only spreads as the stimulation of the sliding spheres slewing into your sphincter sets off', false );
		if( CoC.getInstance().player.gender > 0 ) {
			EngineCore.outputText( ', simultaneously, ', false );
			if( CoC.getInstance().player.hasCock() ) {
				EngineCore.outputText( 'a storm of semen-squirting spurts from your shaft', false );
				if( CoC.getInstance().player.cockTotal() > 1 ) {
					EngineCore.outputText( 's', false );
				}
			} else if( CoC.getInstance().player.hasVagina() ) {
				EngineCore.outputText( 'a siege of spray-scattering spasms from your snatch', false );
			}
			EngineCore.outputText( ' and', false );
		}
		EngineCore.outputText( ' squirts of slippery spooge from her shuddering spunk-stick.  While you shriek, squirm and shiver shamelessly, you see the spider-slut suspended in the same, similarly a slave to the sensation of shoving and slamming her stalk into your straining sacrum and stuffing you with her strange spawn.\n\n', false );
		EngineCore.outputText( 'Gradually, you\'re lifted off the carapace by your inflating abdomen.  Your belly gurgles, making you burp as you\'re filled with enough eggs to make you feel uncomfortable and compressed.  The ' + CoC.getInstance().player.skinFurScales() + ' of your stomach is drum-tight by the time you slide off the wilting ovipositor, and the bottom portion of it is soaked in the last of your partner\'s orgasm.  You look positively pregnant as you stagger upright and make for your garments, trying to ignore the exhausted drider\'s jibes.\n\n', false );
		EngineCore.outputText( 'By the time you finish getting dressed, she\'s fallen into a fitful slumber.  What an ordeal!', false );
		//GET KNOCKED UP;
		CoC.getInstance().player.buttKnockUp( PregnancyStore.PREGNANCY_DRIDER_EGGS, PregnancyStore.INCUBATION_DRIDER - 200, 10 ); //Butt pregnancy goes faster
		CoC.getInstance().player.slimeFeed();
		CoC.getInstance().player.orgasm();
		EngineCore.dynStats( 'cor', 0.5 );
		Combat.cleanupAfterCombat();
	...
	CorruptedDriderScene.prototype.loseVSDriderGetAnalledOvi = function() {
		EngineCore.spriteSelect( 77 );
		EngineCore.outputText( '', true );
		EngineCore.outputText( 'You fall to the ground, defeated by the agile arachnid-woman as she slowly advances towards you, her engorged clit growing larger and larger as it throbs in your direction.  You\'ve managed to find yourself crumpled face down in the dirt, your ' + Descriptors.buttDescript() + ' in full display of the aroused drider while she grins lustfully at your broken figure.  Grunting, you begin to pull yourself off the ground, only to hear your attacker \'tsk\' before feeling the warm stickiness of her web launch upon your limbs, gluing your hands to your ankles and leaving you completely vulnerable.  You struggle against the bindings, but to no avail.  Lifting your head, you turn to see the drider stroking her now fully-enlarged demon cock, replacing the hardened clit that once adorned her moist pussy.\n\n', false );
		EngineCore.outputText( '"<i>You just had to make it harder for yourself, didn\'t you?</i>"\n\n', false );
		EngineCore.outputText( 'You grunt, too exhausted to move any further.  She chuckles at your disinterest, bending down to rest her thick member in between your cheeks and slowly grind along the cleft of your ass.  You feel the tiny nubs of her demonic cock rub along your eager asshole, pre-cum lubricating your hot entrance as she moans in unrequited pleasure.  Gritting your teeth, you force yourself to ', false );
		if( CoC.getInstance().player.HP < 1 ) {
			EngineCore.outputText( 'relax in order to reduce the discomfort', false );
		} else {
			EngineCore.outputText( 'calm your racing thoughts and resist showing how eager you are', false );
		}
		EngineCore.outputText( ' as she begins to enter inside of you.\n\n', false );
		EngineCore.outputText( 'Inch by inch, the bumps and curves of her nine-inch demonic dick massage your puckered hole as she plunges further in, working slowly to savor the feeling of being inside of you.  With a hearty grunt she fails to control her lust, driving in at full force and filling you with her corrupted member.', false );
		CoC.getInstance().player.buttChange( CoC.getInstance().monster.cockArea( 0 ), true, true, false );
		EngineCore.outputText( '\n\n', false );
		EngineCore.outputText( '"<i>Ungh, if I knew your ass was this eager to s-swallow my cock I wouldn\'t have wasted my time tying you up...</i>"  She gasps, clearly the more impassioned of the two of you as she manages to bottom out inside of your ' + Descriptors.buttDescript() + '.  You have very little time to regain your breath as the horny drider-woman begins to thrust in and out of your ' + Descriptors.assholeDescript() + ' with fevered abandon, tongue lolling out of her open mouth while you\'re forced to bounce against her crotch, your ', false );
		if( CoC.getInstance().player.balls > 0 ) {
	...
	CorruptedDriderScene.prototype.loseVSDriderAndVaginallyImpreggleOvi = function() {
		...
		//[LUST: Desperately (jerking your ' + Descriptors.cockDescript(0) + "<i>/grinding your fingers against your ' + Descriptors.clitDescript() + "<i>/and shamelessly groping your ' + Appearance.biggestBreastSizeDescript() + "<i>) you find yourself unable to continue fighting.  The carnal sexuality of the drider - her full chest, her gorgeous lips, and her pristine and gorgeous skin - has overwhelmed you.  Even if your mind was acutely aware that defeating your enemy was a necessity, your body would rebel. You need release.  More than that, you need her to provide it.] [BEATDOWN: With unceremonious indignity, you fall onto your ' + Descriptors.buttDescript() + "<i>.  It seems that you overestimated your capabilities, or underestimated the drider's strength, or perhaps both.  You have lost, and the expression that twinkles in her four eyes suggests that what awaits you is the violation you have come to expect from defeat.  Though weary, you steel yourself for what is to come. You may be her next victim, but you don't have to enjoy it.];
		EngineCore.outputText( 'The drider ignores you at first, lifting her head and glancing from side to side.  It\'s unclear why - perhaps she is worried that another predator will have been attracted by the sounds of combat, and attempt to steal her prey away.  Whatever her reasons she\'s apparently satisfied with what she sees.  Her gaze returns to you, eyes twinkling maliciously.  "<i>Aren\'t you <b>precocious</b>,</i>" she chuckles, walking slowly towards your prone body.  "<i>Such an aggressive little morsel.</i>"  A single strong spider leg places itself on your chest, pushing you ', false );
		if( CoC.getInstance().player.isBiped() || CoC.getInstance().player.isGoo() ) {
			EngineCore.outputText( 'down onto your back', false );
		} else {
			EngineCore.outputText( 'over, onto your side and then to your back', false );
		}
		EngineCore.outputText( '.  It seems that, regardless of their slenderness, she can still put an impressive amount of weight behind her legs.  They ', false );
		if( !CoC.getInstance().player.isNaga() ) {
			EngineCore.outputText( 'pull your own ' + CoC.getInstance().player.feet() + ' apart', false );
		} else {
			EngineCore.outputText( 'scrape inquisitively at your scales', false );
		}
		EngineCore.outputText( ', her body climbing atop you.  Before you can realize her plans her spinneret is at work, ', false );
		if( !CoC.getInstance().player.isNaga() ) {
			EngineCore.outputText( 'coating your ', false );
			if( CoC.getInstance().player.isTaur() ) {
				EngineCore.outputText( 'rear ', false );
			}
			EngineCore.outputText( CoC.getInstance().player.legs() + ' and pinning you in that prone, separated position', false );
		} else {
			EngineCore.outputText( 'spraying webbing across your tail and weighing it down to pin it against the earth', false );
		}
		EngineCore.outputText( '.  She turns with the eerie speed of the arachnids of Mareth, quickly performing the same acts to your ', false );
		if( !CoC.getInstance().player.isTaur() ) {
			EngineCore.outputText( 'wrists', false );
		} else {
			EngineCore.outputText( 'forelegs', false );
		}
		EngineCore.outputText( ' and leaving you pinned and spread-eagled on the ground.  "<i>That should stem that.</i>"\n\n', false );
		EngineCore.outputText( 'Turning, she makes as though to bite you, before pausing.  A thought runs through her head, and if the wicked curve of her dark lips are any indication, she likes it.  She gathers webbing on her fingers and bends down, inspecting your features carefully.  Placing said fingers on your chin she forces it open, quickly and expertly weaving sticky strands to connect your jaw and your chest.  The strands are just short enough to keep your chin held down, while strong enough that you can\'t seem to pull at them.  ', false );
		if( !CoC.getInstance().player.isTaur() ) {
			EngineCore.outputText( 'Her fingers then move to the sides of your head, lacing webbing through your hair and sticking it firmly to the ground. More vulnerable than ever, you can\'t lift your head without ripping your hair from your scalp in chunks.', false );
		} else {
			EngineCore.outputText( 'Her fingers then reach behind your head, pulling your upper body off of the damp dirt and stretching sticky strands between your skull and your immobile forelegs.  ', false );
			if( CoC.getInstance().player.biggestTitSize() > 4 ) {
				EngineCore.outputText( 'With pain and then a pop, your ' + CoC.getInstance().player.allBreastsDescript() + ' squash out to the sides as she presses your torso into your knees.  ', false );
			}
			EngineCore.outputText( 'Reaching back for more webbing, she ties your arms behind your back in a bundle of white thread.  Now pretzeled into an uncomfortable curled shape, you can\'t pull your torso away from your forelegs without wrenching your head off of your neck.  You feel completely helpless.', false );
		}
		EngineCore.outputText( '  "<i>Now, isn\'t that just precious,</i>" the drider chuckles again.  She leans in low, her fingers trickling over your thighs like the steps of... well, of small spiders.  You gurgle in response, mouth held open uncomfortably.  She roams slowly up your body.', false );
		if( CoC.getInstance().player.hasCock() && CoC.getInstance().monster.nipplesPierced > 0 ) {
			EngineCore.outputText( '  Chains brush over ' + Descriptors.sMultiCockDesc() + ' as her chest passes your crotch.', false );
		}
		EngineCore.outputText( '  Her fangs trail teasingly along your belly, her fingers crawling up your sides.', false );
		if( CoC.getInstance().player.biggestTitSize() > 3 ) {
			EngineCore.outputText( '  Venom dribbles down the cleavage of your ' + Appearance.biggestBreastSizeDescript() + ' as she approaches your vulnerable face.  Her own quartet of matronly tits comes to rest on them as her torso appears over their horizon.', false );
		}
		EngineCore.outputText( '\n\n', false );
		EngineCore.outputText( 'Drawing ever closer, at last her eyes blink slowly in front of yours.  Her lips seem to be pouting, then smirking, then - oh.  She\'s swirling something around in her mouth.  Finally it opens up above yours, her fangs extending.  Though you flinch in preparation for a bite, none comes - instead venom drips off her teeth and lips, every drop felt as it lands on your tongue.  You gurgle once more in protestation at this torture, but her hand pets the skin of your forehead maternally to remind you of your powerlessness.  Sadistic delight plays across her eyes as she watches your restrained struggles.  As the venom seeps, a heat grows.  Though subtle at first, you soon recognize it  Raw, burning need.  The poison coursing through you makes little effect on your heart, but a world of difference to your crotch.', false );
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( '  Worse still, its blood-swollen protuberance is met with no reaction from the drider.', false );
		}
		EngineCore.outputText( '  The slow drip serves no purpose - at least, none that serves you.  She is not teasing your body, nor whispering delicious torments.  All it does is force you to tremble more and more as she watches, knowing the effect that her venom will have. For no reason beyond her own sadistic amusement, she forces your body to put on the display of a helpless victim losing more and more control of their lust. You feel like a toy.\n\n', false );
		EngineCore.outputText( 'And the more you drink, the less you mind that.\n\n', false );
		EngineCore.outputText( 'Eventually she decides you\'ve had enough, or perhaps grows impatient.  Her mouth closes, and her body rises.  Yours, incapable of either, merely burns.  The entirety of your body desires her, and your mind has a difficult time coming up with counterarguments.  Venom flows through you, commandeering your sensibilities and trading them in for a desire thus far unquenched.  She stands tall above you, straddling your body with her arachnid behind.  Something moist and ridged stretches out, caressing your thighs.  The degree to which you hope it\'s a cock is nearly immeasurable.  "<i>I think you\'ll make a delectable little nest, morsel,</i>" the drider croons.  "<i>Let\'s start.</i>"  Though you can\'t shift your head down to look at what she has in her behind, you can feel it.  It slaps messily against your thighs, searching out your ' + Descriptors.vaginaDescript() + '.  The caress the tentacle gives your slit is mockingly affectionate, but any illusions of gentility that may have persisted are soon ended by the thrust that delves greedily and deeply into your ' + Descriptors.vaginaDescript() + '.  If there is any small mercy, it\'s that the insatiable lust flooding your mind may wind up sated after all.\n\n', false );
		EngineCore.outputText( 'The thick, vile appendage twists inside of you as it advances.  Slick and obscene in its girth, each individual segmented ridge is palpable against your walls.  The prick practically breathes inside of you, pulsing with life and the ravenous desires that embody a phallus.  As it pushes it twists about in an attempt to wriggle in deeper, inadvertently showing the flexibility of its form.  "<i>How many eggs have you lain, snack?</i>" the drider coos.  Her fingers rub her sizeable swollen clit forebodingly.  It grows increasingly large, acquiring veins and bulges where no clit should.  "<i>How swollen has your belly grown?</i>"  Her tongue flicks across her venomous full lips.  "<i>Time to enjoy it again.</i>"  You can\'t see the tentacle shoved into your ' + Descriptors.vaginaDescript() + ', but you are increasingly certain that it is in not, in fact, a dick.', false );
		CoC.getInstance().player.cuntChange( 50, true, true, false );
		EngineCore.outputText( '  With her body in the way, you cannot clearly see what transpires in your loins, yet a sight emerges before your eyes that dominates your mind. Her clit has finished growing, and is no longer a clit.\n\n', false );
		EngineCore.outputText( 'From her crotch a cock has erupted, demonic and terrible.  It drips with sweat and musk, dribbling onto your collarbone.  The position you\'re in has never seemed more ominous - or more exciting.  Her cockhead smacks against your lower lip, pre-cum bouncing off and lightly spattering on your forehead.  Venom-addled, breathing in her fragrant spunk sends a quake up your body.  Inadvertently your hips start to seize, your ' + Descriptors.vaginaDescript() + ' shuddering in surprising pleasure.', false );
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( '  Even your ' + Descriptors.cockDescript( 0 ) + ' spurts some pre-cum of its own.', false );
		}
		EngineCore.outputText( '  There\'s some shame in having a small orgasm from nothing more than the scent of sperm, but you place the blame solely on your intoxicated state.\n\n', false );
		//It would take a creature with truly fucked up biology to evoke such a reaction otherwise.;
		EngineCore.outputText( 'She grinds her prick against your open mouth, massaging the underside of the infernal shaft with your lips.  The steaming heat of the cock this close to your face makes you sweat, a low sheen of moisture on your forehead adding to the heady sensation of lust.  Pushing past your nose, the scent of her demon dick only grows more intense.  "<i>Open up, morsel,</i>" the drider coos, then snickers.  She lowers her upper body, angles herself, and then calmly slides her prick into your mouth.  A low purr rises from her throat, cockhead grinding into your palate.  "<i>So warm,</i>" she hisses affectionately, "<i>so wet.  Have you hungered for my domination, snack?</i>"  Her shaft presses firmly against your tongue.  "<i>Have you hungered for me?</i>"\n\n', false );
		EngineCore.outputText( 'You suspect she is not waiting for an answer, given that her prick pushes at the entrance to your throat.  Horny and helpless, you start sucking.  It seems like a good idea, and the obscenity of the act arouses you.  Chitin-covered fingers wriggle into your hair as she moans, and her heady breath seems echoed by the beast lodged between your legs, long since settled but preparing for its obscene duty.  The appendage is buried as deep as it can go, practically kissing your womb.  This intimacy is soon interrupted by the equivalent of an obscene tongue.  The bulge comes slowly; it pushes aside the walls of your ' + Descriptors.vaginaDescript() + ' without mercy, the ovipositor forcing your hole into an all-access VIP club for its fertilized cargo.  No matter how your pussy contracts, there\'s only one direction those eggs are going right now  Belching into your belly, the first egg pushes all the way and sits heavy inside of you. Already the entrance of your ' + Descriptors.vaginaDescript() + ' can feel the next one coming, but her thrust distracts you.\n\n', false );
		EngineCore.outputText( 'The drider presses you to her crotch', false );
		if( !CoC.getInstance().player.isTaur() ) {
			EngineCore.outputText( ', cutting your head loose from the imposed bondage with sharp, hard fingertips', false );
		}
		EngineCore.outputText( '. Throat stretched and jaw gaped, no matter how much you try to hold it you gag.  You spit, and you struggle, and you gag.  A throaty exhalation above you lets you know that she doesn\'t care.  The webbing at your wrists holds strong as you tug reflexively; but her hips rock forward, knocking your nose into lusciously pale skin.  Venom drools from her open mouth, catching on her tits before eventually finding its way into your hair, landing in small spatters.  She fucks <i>deep</i>, never leaving your throat as she bounces your head.  Though you can occasionally catch a breath through your nose, it\'s not quite enough.  The corners of your vision fade in and out as your eyelids flutter, finding it more and more difficult to stay awake.  Even the lust inflicted upon you seems strained and near being forgotten as you struggle.\n\n', false );
		EngineCore.outputText( 'Another egg shoves itself into your womb.  The next is already halfway down your cunt and another is at your entrance.  They are coming faster, and you don\'t know when they\'ll stop.  Small twitches shake your head as your oxygen-deprived and unconscious state threatens to overtake you.  You\'re so close to the edge that it takes you a moment to notice when she rips her prick out of your mouth and lets you drop.  Dredged-up slime splatters across your chin and chest, drooling from her cock.  Hacking and gasping desperately you fill your lungs, jolted back to wakefulness.\n\n', false );
		EngineCore.outputText( 'Before you can protest or ground yourself, she takes your hair and shoves herself back in.  The taste of the back of your throat sits on your tongue as she pushes past, back into the filthy deepthroat.  She\'s obviously done talking to you, using you purely as a vehicle to get herself off now.  A repository for her eggs and a hole to dump her lusts in - add it to your resume.  Her webbing is too strong to rip, and her venom too intoxicating.  Even if you could fight off both of those aspects, you\'re not sure you could pull away from the ovipositor dug into you.\n\n', false );
		EngineCore.outputText( 'You don\'t mind, though.\n\n', false );
		EngineCore.outputText( 'You like it.  The venom coursing through your veins makes sure of that.  It\'s not an intense feeling, not an overwhelming need to fuck fuck fuck now forever always, but you like this sex.  You like this drider.  You like her cock in your throat, smearing the thickest of your saliva over your lips.  You like the weight of that fifth egg being added to the pile in your womb - and then, there\'s six. You like your helplessness, the restrained limbs occasionally spasming in a desperate need to move.  It\'s sedentary and unusual, a sexual experience you don\'t frequently have.  In some ways the slothful use of your ' + Descriptors.vaginaDescript() + ' would be more at home against your lips, while the aggressive rape of your throat is expected for your crotch.  As far as this drider seems to be concerned, the two may as well have swapped places.\n\n', false );
		EngineCore.outputText( 'You take another deep gasp of breath as you lay against the ground.  You hadn\'t noticed that she had stolen so much of your air again.  Perhaps you passed out.  The deprivation and the venom have already slowed your mind enough that you can\'t be certain, but she picks your head back up, and back on her prick you go.  Each time you feel a little weaker, a little more exhausted.  Your jaw is slacker than it was before, the energy to squeeze and suck bleeding out of you and splattered across your face every time she dredges up the depths of your throat.  She just lets your head drop now, whenever she thinks you\'re near passing out.  It hurts to ', false );
		if( !CoC.getInstance().player.isTaur() ) {
			EngineCore.outputText( 'land against the ground', false );
		} else {
			EngineCore.outputText( 'strain your neck suddenly with the dead weight of it', false );
		}
		EngineCore.outputText( ', and you nearly spit up, but the drider\'s abuse remains welcome.\n\n', false );
		EngineCore.outputText( 'Her thick cock feels like it might be stretching your throat less now, but you don\'t think it\'s gotten any smaller.  Your throat is just being as worn out as any sex hole, as gaped as any slut\'s cunt.  Your tongue is nearly limp in your mouth, and even your gagging is quieter.  Her crotch pushes up your nose every time you slap into it.  Wet noises echo in your head, and it sometimes takes a bit too long to remember they\'re coming from it.  She violates your mouth, and violates you.\n\n', false );
		EngineCore.outputText( 'The cum explodes into your throat unexpectedly - but then, you can\'t even remember when she started fucking your mouth.  How could you have known how close she was?  The drider moans loudly, slapping into and grasping your cheeks with hard fingers as she pulls you down to blow her foul load deep into your maw.  You think you can feel it landing directly into your stomach.  Hair is pulled, and scratches dig into your scalp.  Shuddering and seizing a few last times, with a sigh she lets you go.  Cum keeps flowing as you fall back, drenching your mouth and then shooting across your features.  It pools out the corner of your mouth when your head slumps to the side.  The last few spurts drip across your ear, the drider squeezing them out carefully with her dark fingers.  She wriggles her behind and slowly pulls her ovipositor out, leaving the sticky weights in your womb as a reminder of her actions.  You haven\'t cum yet.  She doesn\'t care.\n\n', false );
		EngineCore.outputText( 'She steps over you, satisfied and still groaning slightly.  Not fully retracted, her ovipositor drags over your belly and chest as she departs, smearing your pussy juices over you.\n\n', false );
		EngineCore.outputText( 'You can tell that the bindings at your limbs are already slackening, but you\'re so weak, so exhausted - simply being able to breathe seems like a delicacy that had not been afforded you until just this moment.  You can\'t leave, just yet.  You just need... to rest... a little...', false );
	...
	CorruptedDriderScene.prototype.loseVSDriderAndGetFuckedANDAnalImpreggegity = function() {
		...
		if( CoC.getInstance().player.lust > 99 ) {
			EngineCore.outputText( 'Unable to resist the desire coursing through your body, you strip off your ' + CoC.getInstance().player.armorName + ' and start stroking ' + Descriptors.sMultiCockDesc() + ', displaying it in the hope that it will draw the drider\'s attention, while at the same time just trying to get yourself off and attain release.', false );
		} else {
			EngineCore.outputText( 'You slump down, too beat up to mount an effective defense, let alone stay up on your ' + CoC.getInstance().player.feet() + '.  The drider skitters over to you and effortlessly peels you out of your ' + CoC.getInstance().player.armorName + ', exposing your ', false );
			if( CoC.getInstance().player.lust < 50 ) {
				EngineCore.outputText( 'soft', false );
			} else {
			...
			EngineCore.outputText( ' to the air.  She squeezes ' + CoC.getInstance().player.oMultiCockDesc() + ' in her hand and pumps it, smiling while you stiffen and twitch precipitously in her grasp.', false );
		}
		EngineCore.outputText( '  With a strength beyond what her lithe frame would imply, the spider-woman\'s shiny, exoskeletal arms slip under your shoulders and lift you up, hugging you into both pairs of breasts', false );
		if( CoC.getInstance().monster.nipplesPierced > 0 ) {
			EngineCore.outputText( ', the crossed chains digging into your ' + Descriptors.chestDesc(), false );
		}
		EngineCore.outputText( '.  The warmth of her nodule-covered cock pulsates on your belly, smearing you with a trail of pre-cum while she arranges your entwined bodies.\n\n', false );
		EngineCore.outputText( 'Lines of webbing burst from the arachnid abdomen, snapping into tree limbs before pulling taut.  The drider\'s eight legs clitter and clack, four of them folding around your back while your world rotates 90 degrees.  The two of you are pulled into the trees by the sticky strands, hanging sideways, almost upside-down, in the leaves.  The four-eyed monster-girl blinks asynchronously and smiles, shifting you slightly to bring ' + CoC.getInstance().player.oMultiCockDesc() + ' against her moist lips.  You exhale, an intense, animalistic thrill shivering through you from the feeling of having your ' + Descriptors.cockDescript( x ) + ' lodged in a woman\'s entrance.  She leans down and bites into your neck, releasing a surge of toxin into your bloodstream.  It boils like fire, making you twitch and squirm in her many-limbed clutches.  The pain fades quickly, but in its place it leaves behind a molten, liquid lust, filling ' + Descriptors.sMultiCockDesc() + ' to its absolute maximum size, as hard as it can possibly be.\n\n', false );
		EngineCore.outputText( 'The ' + CoC.getInstance().monster.hairColor + '-haired dominatrix pulls back, a single drop of blood clinging to one of her fangs while she hisses, "<i>Mmm, fresh meat is always so tasty... not to mention... firm...</i>"\n\n', false );
		EngineCore.outputText( 'You look at her with fear, but your ' + Descriptors.hipDescript() + ' start to twitch against her, rubbing your ' + CoC.getInstance().player.cockHead( x ) + ' up and down the slippery slit.  A slow exhale of pleasure escapes the spider\'s purple lips while her legs squeeze tighter against you, trying to push your ' + Descriptors.cockDescript( x ) + ' home.  The drider gasps when at last, you slip between her juicy outer labia, slurping deep into her cunny with a lurid squelch.  Finally held within her depths, your muscles all slacken, save for those you need to pump your hips against her, futilely trying to fuck her hot quim even though you have zero leverage.\n\n', false );
		EngineCore.outputText( '"<i>So eager!  I\'m not going to let you fuck me, but I do want your sperm.  After all, I\'ll need to fertilize some more eggs.</i>"\n\n', false );
		EngineCore.outputText( 'Wait, eggs?  While you try to chew over the thought, you\'re battling with your raging, venom-enhanced libido in order to think about something other than how good her pussy feels.  Fuck it!  You go back to grinding against your mistress, nuzzling at her neck while she twists her spider-half around you, holding you tightly in her chitin and bringing her abdomen up behind you.  A weird, wet appendage rubs over your exposed ' + Descriptors.buttDescript() + ', slapping at your cheeks until it finds the cleft between them.  Sliding deeper, it bumps into your sphincter, spurts something warm, wet, and sticky against it, then pushes through.', false );
		CoC.getInstance().player.buttChange( 8, true, true, false );
		EngineCore.outputText( '  You shudder from the anal violation and twist against the legs holding you in place, but with your ' + Descriptors.cockDescript( x ) + ' locked inside her, it\'s a useless endeavor.  It isn\'t shaped like a dick, and besides, that\'s currently dripping on your belly.  Oh gods... she mentioned eggs.\n\n', false );
		EngineCore.outputText( 'The strange organ keeps forcing its way deeper, slipping the steadily-widening, rib-like segments of its bulk into your ' + Descriptors.assholeDescript() + '.  ', false );
		EngineCore.outputText( '  Once it gets deep enough to push on your prostate, an upsurge of pleasure hits you hard enough to make ' + Descriptors.sMultiCockDesc() + ' dribble.  You moan against the drider\'s neck, pulsing pathetically inside the spider-woman\'s slit.  You know you\'ll soon be giving her exactly what she wants - all of it - and there\'s nothing you can do about it.\n\n', false );
		EngineCore.outputText( 'Another inch slides inside you, filling you uncomfortably full.  You shiver, clutching at your spidery mistress while she prepares both to deposit and to withdraw from the bank of you.   While you hang there, on the very edge of orgasm, it stops going deeper.  You\'ve been given a reprieve, but it doesn\'t last long.  The drider kisses your forehead, her venomy lips spiking the sensitivity of your skin once again.  Suddenly, the fat tube in your ' + Descriptors.assholeDescript() + ' is dilating, stretching you wider.  Goo races up it, pumping into your guts, paving the way for the bigger bulk of the coming egg.  It stops at your pucker, battering against your muscles\' resistance until it pops inside you, slamming into your prostate so hard that a huge rope of dick-milk erupts into the spider-woman\'s wet vagina.', false );
		CoC.getInstance().player.buttChange( 50, true, true, false );
		EngineCore.outputText( '\n\n', false );
		EngineCore.outputText( 'The egg travels further up your butt, eventually flowing out the end of her stalk-like organ to rest deep in your digestive tract, ensconced in a bubble of slime.  Before you can adjust to this new sensation, another egg slips through your now-widened ring.  It squishes past your anal g-spot, rubbing it just as hard as the first, but before it even finishes traveling up your intestines, a third egg is forced inside you.  ' + Descriptors.SMultiCockDesc() + ' begins to cum wildly, squirting and leaking a torrent of jizz.  The first few ropes blast out with incredible force, hosing down her cunt', false );
		if( CoC.getInstance().player.cockTotal() > 1 ) {
			EngineCore.outputText( ' and belly', false );
		}
		EngineCore.outputText( '.  Then, the constant application and release of pressure on your prostate utterly destroys the orgasmic timing of your muscles\' contractions, making ' + Descriptors.sMultiCockDesc() + ' release a weak, but constant trickle of cum.\n\n', false );
		EngineCore.outputText( 'You quickly lose track of how many of the drider\'s eggs have gone inside you.  Feeling the numerous bulges in your abdomen, you give up entirely.  Your ass is going to be full of eggs and you\'ve probably cum enough for her to make a whole \'nother batch.  Shuddering while the ovipositor is withdrawn, you sag, exhausted, barely realizing that the drider is hosing down your ' + Descriptors.chestDesc() + ' with her arachnid herm-spunk, the nodules ringing her dick pulsing in time with the blasts of thick spooge.  Some sticks to your chin, but you\'re so drained from your rough treatment that you barely react.\n\n', false );
		EngineCore.outputText( '"<i>Shhh, shhh... it\'s all right.  You\'ll be such a good mother for my young, and an even better father for the eggs you just fertilized.  Maybe if we ever meet again you can tell me how they looked when they were born.  For now, rest up.  I want you to take good care of our babies,</i>" instructs the drider while she lowers herself towards the ground.  She sets you down on a soft spot, giving you a blanket of moss to recover in.  You struggle to stay awake but quickly slump into a deep slumber.', false );
		//(slimefeed);
		CoC.getInstance().player.slimeFeed();
		//GET KNOCKED UP;
		CoC.getInstance().player.buttKnockUp( PregnancyStore.PREGNANCY_DRIDER_EGGS, PregnancyStore.INCUBATION_DRIDER - 200, 10 ); //Butt pregnancy goes faster
	...
	CorruptedDriderScene.prototype.birthSpiderEggsFromAnusITSBLEEDINGYAYYYYY = function() {
		...
		EngineCore.outputText( '\n\n', false );
		EngineCore.outputText( 'You shift position slightly, if only to get more comfortable, and relax as the eggs work their way through your guts.  With each egg\'s passage, your sphincter seems to be looser, more pliant, and more sensitive.  You blush and squeeze, hurrying the process to make yourself feel even better, and egg after egg splats wetly into the growing pile, each one slick and soaked in slime.  One after another, the heavy orbs exit your body, until the last one is sliding out of your rectum and you\'re having a strange anal orgasm that feels as relaxing as it is pleasant.\n\n', false );
		EngineCore.outputText( 'Clenching and relaxing, your slime-dripping asshole slowly narrows, gaping less visibly while you shudder and sigh, so happy and blissful that you don\'t even care about the crackling egg-shells hatching beneath you.  ', false );
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( Descriptors.SMultiCockDesc() + ' stays limp, but drools a heavy flow of semen, puddling up a nice thick batch of your pleasure alongside your goo-pile.  ', false );
		}
		if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( 'Your pussy clenches and ', false );
			if( CoC.getInstance().player.wetness() < 3 ) {
				EngineCore.outputText( 'drips', false );
			} else if( CoC.getInstance().player.wetness() < 4 ) {
				EngineCore.outputText( 'dribbles', false );
			} else if( CoC.getInstance().player.wetness() < 5 ) {
				EngineCore.outputText( 'gives a tiny squirt', false );
			} else {
				EngineCore.outputText( 'gushes a river of lady-spunk', false );
			}
			EngineCore.outputText( ' into the dirt, getting off along with your ' + Descriptors.assholeDescript() + '.  ', false );
		}
		EngineCore.outputText( 'Once you come down from your high, you roll to the side and watch the tiny driders clambering out of their egg shells.\n\n', false );
		EngineCore.outputText( 'Your newborn children glance at you, chittering happily before they scurry off in the direction of the swamp.\n', false );
		CoC.getInstance().player.orgasm();
	};
	...
	CorruptedDriderScene.prototype.driderPregVagBirth = function() {
		...
		EngineCore.outputText( '\n\n', false );
		EngineCore.outputText( 'After what seems like hours, you have lost count of the eggs pushed out.  ', false );
		if( CoC.getInstance().player.isNaga() ) {
			EngineCore.outputText( 'At last, when no more eggs seem to be coming, you flop over onto your back, careful not to crush any of your dormant offspring.', false );
		} else {
			EngineCore.outputText( 'Looking between your legs you can see another slick, green egg slide from your body to join the sizable pile behind you.  At last, when no more seem to be following it, you flop over onto your back, careful not to crush any of your dormant offspring.', false );
		}
		...
		if( CoC.getInstance().player.eyeType !== AppearanceDefs.EYES_FOUR_SPIDER_EYES ) {
			EngineCore.outputText( '  Suddenly she flops forward, leaning against your eyebrows and causing you to cross your eyes in an attempt to focus on her.  She seems confused by the way your eyes are different to hers.', false );
		}
		if( CoC.getInstance().player.isNaga() ) {
			EngineCore.outputText( '  A few baby driders crawl ponderously across your scaled tail.  You give your tail a little wiggle and suddenly they go wild crawling around it and letting out the faintest, cute little squeals.', false );
		} else {
			EngineCore.outputText( '  A few baby driders sit on each of your legs, facing each other.  Some gesture and squeak at each other in wonder while others pull faces.', false );
		}
		EngineCore.outputText( '\n\n', false );
		EngineCore.outputText( 'Nearby, two more of your little girls are in a stand-off, as each skitters back and forth in front of the other, ready to charge.  Suddenly one leaps forward and grabs the other before attempting to bite her arm.  Of course the attack is completely harmless - they are still much too young to have grown their fangs.  The other then turns the tables on her sister by grappling her sister to the ground, and soon, they\'re both letting out tiny giggles.\n\n', false );
		EngineCore.outputText( 'You lie back and your exhaustion finally begins to get the better of you when combined with the contentment from seeing your babies enjoying themselves.  Your eyelids begin to droop, and before you know it, you\'ve slipped into unconsciousness.  You awaken later on, a sticky feeling between your legs to go with your wet pussy', false );
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( ' and raging hard on', false );
		}
		EngineCore.outputText( '.  After remembering where you are, the next thing you notice is the absence of your drider children.  Only a pile of empty, broken egg shells and some messy webbing in the trees remain.  As you stand up and prepare to move on, the sun reflecting off something in the corner of your eye catches your attention.\n\n', false );
		EngineCore.outputText( 'Between two nearby trees is a large spider web.  While not out of the ordinary, when the sunlight hits it at a certain angle the gossamer strands spell out a simple message YOU MOMMY.</i>"', false );
		CoC.getInstance().player.orgasm();
	};
	...
	CorruptedDriderScene.prototype.buttFuckADriderOhBaby = function() {
		...
		EngineCore.outputText( 'Now that she\'s calmed down, you give her a hard shove, putting her on her drider-belly so you\'re not nearly thrown off her every other moment.  With that done, you put your hands on her human back, pushing her forward and exposing her human-like bubble-butt, seated just above where woman meets spider. Greedily, you pull her cheeks apart, revealing the tiny star of her puckered asshole.  You pull your ' + Descriptors.cockDescript( x ) + ' out of your ' + CoC.getInstance().player.armorName + ', already hardening from the thought of you pounding her tight little backdoor.\n\n', false );
		EngineCore.outputText( '"<i>Oh, you want that hole, do you? How very kinky,</i>" the drider chides, but you aren\'t fooled by her tone – you can see her anus relaxing, preparing itself for your arrival.  With a grin, you grab your ' + Descriptors.cockDescript( x ) + ' and push it against her tight hole, leaning into the penetration until you feel it slip past her sphincter and into her hot, snug passage.  As you slide more and more of your cockmeat into her, you reach up from her cheeks and grope her breasts, squeezing her pert nipples between your fingers as you finally bottom out.  Since you can go no further up her butt, you start to piston your hips forward, slapping your hips to her ass with short, forceful strokes.\n\n', false );
		EngineCore.outputText( 'You\'re pleased to see the drider\'s tongue loll out of her mouth, her eyes rolled up into her head. Clearly she\'s enjoying this!  Encouraged, you start to fuck her butt harder, hammering into her ass with every bit of force your powerful muscles can summon up.  You\'re rewarded with a sharp, high moan from the drider.  Once she starts, she doesn\'t stop.  A symphony of moans, grunts, and pleasured screams erupt from the spider-girl, all in concert with your rough anal ravishing.\n\n', false );
		EngineCore.outputText( 'Without warning, the drider cums.  She cries out and grabs her fully-engaged corrupted cock, hosing down the trees in front of her with spider-cum as her butt squeezes down on your ' + Descriptors.cockDescript( x ) + ', milking your dick with her anal passage.  You pick up the tempo of your fucking to break-pelvis speeds, ramming your cock into her backdoor and letting loose a torrent of your spunk, shooting it deep inside her', false );
		//(if Vagina: ;
		if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( ' while your unused cunt spurts girlcum across her carapace', false );
			if( CoC.getInstance().player.cockTotal() > 1 ) {
				EngineCore.outputText( ' and', false );
			}
		...
		if( CoC.getInstance().player.cockTotal() > 1 ) {
			EngineCore.outputText( ' your spare cock', false );
			if( CoC.getInstance().player.cockTotal() === 2 ) {
				EngineCore.outputText( ' coats', false );
			} else {
				EngineCore.outputText( 's coat', false );
			}
			EngineCore.outputText( ' her human back with a thick white layer of spooge', false );
		}
		EngineCore.outputText( '.\n\n', false );
		EngineCore.outputText( 'Finally done, you pull out of the drider, collapsing on her spider-back with a contented sigh.  Completely insensate after the anal ravaging you just gave her, the drider poses no threat as you leisurely gather ' + Descriptors.sMultiCockDesc() + ' back into your ' + CoC.getInstance().player.armorName + ' and lower yourself off her cum-soaked abdomen.  You give her a final, playful ass slap as you dismount before heading back to camp.', false );
		if( EngineCore.silly() ) {
	...
	CorruptedDriderScene.prototype.driderVagSmartFuck = function() {
		...
		var y = x + 1;
		EngineCore.outputText( 'You keep your distance from the downed spidermorph, still wary of her dripping, toxin-coated lips despite her weakened state and your own rising desire to breed her.  Trailing your eyes over her sweaty, splayed body you feel a familiar rush below the belt, your hardening [cock ' + y + '] pressing against the inside of your [armor] as her breasts rise and fall seductively with each panting breath.  Desire threatens to overpower your caution as your gaze lingers on the vulnerable woman before you, her netherlips ' );
		if( CoC.getInstance().monster.HP < 1 ) {
			EngineCore.outputText( 'parted slightly and glistening with beads of sweat from the adrenaline of battle.' );
		} else {
			EngineCore.outputText( 'stuffed with her fingers, femcum streaming down the front of her spider-half as she desperately plays with herself.' );
		}
		EngineCore.outputText( '  The need to bury your aching length inside that puffy snatch is almost unbearable, but you don\'t want to risk exposing yourself to any of her tricks, especially considering how close you\'d be to her mouth as you take her.  Racking your brain for a solution, the mass of webbing that covers the canopy around this area catches your eye.' );
		if( CoC.getInstance().player.tailType === AppearanceDefs.TAIL_TYPE_SPIDER_ADBOMEN ) {
			...
			if( CoC.getInstance().player.isDrider() ) {
				...
				EngineCore.outputText( 'saunter' );
			}
			EngineCore.outputText( ' over to the collapsed drider.' );
			this.driderTiedUpPartTwo( false );
		}
		//(Champion has some Tough Spider Silk on them);
		else if( CoC.getInstance().player.hasItem( UsableLib.T_SSILK ) ) {
	...
	CorruptedDriderScene.prototype.driderTiedUpPartTwo = function( useItem ) {
		...
		if( !useItem ) {
			EngineCore.outputText( 'Shifting yourself, you withdraw your hand from her quivering slit, reaching back to collect another handful of stretchy silk.' );
		} else {
			EngineCore.outputText( 'Reaching back into your pack, you grab the remainder of the bundle of silk that you tied the girl\'s hands with.  Looking over the mass of white-grey strands, you smile as you see that there seems to be more than enough left for what you have in mind.' );
		}
	...
	CorruptedDriderScene.prototype.driderTiedUpPartThree = function() {
		...
		EngineCore.outputText( '\n\nDriven on by the sight of the thoroughly defeated girl, you move your hands to grip her sides and start pistoning your bodies together.  She can only moan pathetically while you use her, the way your length presses against her sensitive insides all too much for her exhausted frame.  Hammering into her, you dart a thumb across to sweep over her throbbing clit, bringing about a particularly loud cry of pleasure that makes your [cock ' + y + '] jump delightfully even as she clenches especially tightly around you.  You feel yourself getting close, the mixture of such a tight, wet cunt and your unbelieveable arousal leaving you smacking into her pelvis with a single goal in mind her with your potent seed.' );
	...
	CorruptedDriderScene.prototype.driderTiedUpPartFour = function() {
		...
		//(if (30 =< corruption < 70);
			...
			EngineCore.outputText( 'You make sure she\'s comfortable before checking her for anything that could help your quest.  She stirs slightly as your hands caress her, eyes fluttering drunkenly as she tries to focus on you.  Something in the way her eyes bore into you fills you with a sense of shame at having taken advantage of her and you quickly sever her bonds, reasoning that she\'ll still be too weak to try anything.' );
