...
angular.module( 'cocjs' ).run( function( MainView, SceneLib, $rootScope, $log, FemaleSpiderMorph, ConsumableLib, AppearanceDefs, Utils, PregnancyStore, kFLAGS, Combat, CoC, EngineCore, Descriptors ) {
	...
	FemaleSpiderMorphScene.prototype.defeatFemale = function() {
		...
		if( !CoC.isInCombat() ) {
			...
			EngineCore.outputText( ' undress, tossing your ' + CoC.player.armorName + ' to the side to fully expose yourself, just like your soon-to-be lover.', false );
	...
	FemaleSpiderMorphScene.prototype.spiderMorphFemaleRidesACawk = function() {
		...
		//(Noncombat Intro:) ;
		if( !CoC.isInCombat() ) {
			EngineCore.outputText( 'You shuck your ' + CoC.player.armorName + ' and toss it aside, feeling ' + Descriptors.sMultiCockDesc() + ' ', false );
			if( CoC.player.lust < 70 ) {
	...
	FemaleSpiderMorphScene.prototype.femaleSpiderMorphTooBigWebRape = function() {
		...
		//(Consensual);
		if( !CoC.isInCombat() ) {
			EngineCore.outputText( 'You hastily remove your ' + CoC.player.armorName + ' and toss it aside, glad to be able to let ' + Descriptors.sMultiCockDesc() + ' flop out and breathe.  The spider-girl\'s eyes widen as she takes in ALL of your ', false );
			if( CoC.player.lust < 70 ) {
			...
			EngineCore.outputText( '"<i>W-wow, you\'re a big ' + CoC.player.mf( 'boy', 'girl' ) + ', aren\'t you?  Why don\'t you lie down and get comfortable while I get you restrained... I wouldn\'t want that beast between your legs to kill me!</i>" she says.\n\n', false );
		...
		//(COMBAT LOSS);
			...
			if( CoC.player.lust > 99 ) {
				EngineCore.outputText( 'stick your hands into your groin, fondling ' + CoC.player.oMultiCockDesc() + ' with need', false );
			} else {
	...
	FemaleSpiderMorphScene.prototype.fSpiderMorphRape = function() {
		EngineCore.outputText( '', true );
		EngineCore.spriteSelect( 73 );
		EngineCore.outputText( 'You laugh at her and push her down with your ' + CoC.player.legs() + ', enjoying the view of the pale maiden\'s unblemished skin and dark, fetishy-looking exoskeleton when she bounces in swamp loam.  Her legs scissor closed nervously, a weak effort to conceal her sex from you', false );
		if( CoC.player.lust > 99 ) {
	...
	FemaleSpiderMorphScene.prototype.fSpiderMorphRapeDude = function() {
		...
		EngineCore.outputText( '', true );
		EngineCore.spriteSelect( 73 );
		EngineCore.outputText( 'You push the spider-girl onto her back, getting a good long look at your conquest before you take off your ' + CoC.player.armorName + '.  At the sight of your ' + Descriptors.multiCockDescriptLight() + ', she ', false );
		if( CoC.monster.lust <= 99 ) {
	...
	FemaleSpiderMorphScene.prototype.evilSpiderGirlVictoryAnal = function() {
		...
		EngineCore.outputText( '', true );
		EngineCore.spriteSelect( 73 );
		EngineCore.outputText( 'You peel off your ' + CoC.player.armorName + ' while you gaze disdainfully down at your prize.  ', false );
		if( CoC.monster.lust > 99 ) {
