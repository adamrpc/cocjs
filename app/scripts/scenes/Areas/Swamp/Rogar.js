...
angular.module( 'cocjs' ).run( function( SceneLib, kFLAGS, AppearanceDefs, StatusAffects, CockTypesEnum, PerkLib, ConsumableLib, CoC, EngineCore, Descriptors ) {
	function Rogar() {
	}
	...
	//Dirt Mc Girt flag: state 0 - CoC.getInstance().player has not had the opportunity to tongue-bathe Ro'; 1 - CoC.getInstance().player gently turned down the chance to bathe him; 2 - CoC.getInstance().player bathed Ro';
	...
	Rogar.prototype.encounterRogarSwamp = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.ROGAR_PHASE ] === 0 ) {
		...
		//((Ro'gar phase = 1)) (edited);
		else if( CoC.getInstance().flags[ kFLAGS.ROGAR_PHASE ] === 1 ) {
			EngineCore.outputText( 'With map in hand you wander the swamp for some time, squinting at the rough scrawls Ro\'gar had scribed for you.  You can\'t help but stop and scratch your head as you begin to wonder if you are hopelessly lost.\n\n', false );
			//((intellect less than 30));
			if( CoC.getInstance().player.inte <= 30 ) {
			...
			//((high femininity or breasts >=B-cup, libido less than 50));
				...
				EngineCore.outputText( 'You grin as you chat with Ro\'gar mindlessly.  ', false );
				//[(int <30) ;
				if( CoC.getInstance().player.inte < 30 ) {
					EngineCore.outputText( 'Ro\'gar apologizes for wrestling for his territory in your sight.', false );
				}
				//(int>=30) ;
				else {
					EngineCore.outputText( 'Ro\'gar apologizes for how messy his hut is.', false );
				}
			...
			//((androgynous or masculine and breasts <= A-cup, libido less than 50));
			else if( CoC.getInstance().player.lib < 50 ) {
				EngineCore.outputText( 'You grin as you chat with Ro\'gar mindlessly.  ', false );
				//[(int<30);
				if( CoC.getInstance().player.inte < 30 ) {
					EngineCore.outputText( 'Ro\'gar apologizes for wrestling for his territory in your sight.', false );
				...
				if( CoC.getInstance().player.inte < 30 ) {
					EngineCore.outputText( 'leftovers from that fight in the swamp still linger on his body.', false );
				} else {
					EngineCore.outputText( 'it makes you wonder what Ro\'gar was up to.', false );
				}
				EngineCore.outputText( '  Ro\'gar follows your gaze, then his eyes glint oddly as he snickers.  "<i>Care ta... return the favor?</i>"  He slyly winks at you, eyes flicking from you to the bucket and back again.', false );
				//goto sexy choices;
			}
		...
		//Not sure what determines this yet;
		else if( CoC.getInstance().flags[ kFLAGS.ROGAR_PHASE ] === 2 ) {
			...
			if( CoC.getInstance().player.biggestTitSize() >= 2 ) {
				...
				CoC.getInstance().flags[ kFLAGS.ROGAR_PHASE ] = 3;
			}
			//((androgynous or masculine and breasts <= A-cup));
			else {
				EngineCore.outputText( 'Remembering your way from memory, you find yourself on Ro\'gar\'s doorstep. You gently knock with your fist, and soon you hear footsteps on the other side of the crude door. It opens with a snap and you are greeted with a wide grin of jagged, razor-like teeth.\n\n', false );
				//(Dirt Mc Girt = 2);
				if( CoC.getInstance().flags[ kFLAGS.ROGAR_DIRT ] === 2 ) {
					EngineCore.outputText( '"<i>' + CoC.getInstance().player.short + ', y\'all took off on me real quick like,</i>" he says through a snicker.  The joke falls flat and the orc seems to blush lime green as you stare at him uncomprehendingly.  He rubs the back of his neck with a hand before continuing.  "<i>Sorry about that.  I was plumb tuckered out afterwards.</i>"\n\n', false );
				}
				//(merge sex and non-sex, new PG);
				EngineCore.outputText( '"<i>Come on in.</i>"  Ro\'gar extends a hand, welcoming you inside.  You quickly take a seat and fold your arms', false );
				if( CoC.getInstance().player.lib > 50 ) {
					EngineCore.outputText( ' over your musk-fueled, growing ', false );
					if( CoC.getInstance().player.hasCock() ) {
						EngineCore.outputText( 'bulge', false );
					} else {
						EngineCore.outputText( 'nipples', false );
					}
				}
				EngineCore.outputText( '.  "<i>I been meanin\' ta speak with ya,</i>" Ro\'gar starts, from across the table.  He rests his arms on it and turns his head to face you before he continues.  "<i>I\'ve been plannin\' to git away from this here place for some time now, an\' I reckon if\'n you didn\'t come along I\'d have just taken off without a clue.</i>"  He lets out a sigh before continuing. "<i>I\'ve just been lookin\' fer a place to settle in an\' be around people that\'d be acceptin\' of a feller like me.  Y\'ever find a place like that?</i>"  Ro\'gar turns his whole body to face you, an expectant look in his eyes.\n\n', false );
				//(If player hasn't been to Tel'Adre);
				if( CoC.getInstance().player.statusAffectv1( StatusAffects.TelAdre ) <= 0 ) {
	...
	Rogar.prototype.dontWaitForRogar = function() {
		EngineCore.outputText( '', true );
		EngineCore.spriteSelect( 86 );
		EngineCore.outputText( 'Time passes with no sign of Ro\'gar.  You decide it would be a waste of time to sit and wait for him, and head off in hopes of finding something interesting.\n\n', false );
		EngineCore.outputText( 'Instead, you find only more mud.  You return to camp.', false );
		//<set Crying Game = 1>;
		CoC.getInstance().flags[ kFLAGS.ROGAR_DISABLED ] = 1;
		EngineCore.doNext( SceneLib.camp.returnToCampUseOneHour );
	};
	//((Yes));
	Rogar.prototype.waitForChunkyOrcLoe = function() {
		...
		EngineCore.spriteSelect( 86 );
		if( CoC.getInstance().flags[ kFLAGS.ROGAR_PHASE ] === 0 ) {
			EngineCore.outputText( 'It feels like several hours pass with no sign of Ro\'gar, but you decide to wait and use what little cloth you have to clean yourself off.  Eventually you hear the slow trod of heavy footsteps as Ro\'gar comes into view with a smile on his face and a bucket and towel in his hands.  He sets the steaming bucket down as he draws near.  "<i>Here ya go!</i>" he says with a grunt, dipping the towel in the bucket.  He rubs your body with the towel to wipe away the mud for you, seeming not to realize - or perhaps, not minding - the intimacy of the gesture.\n\n', false );
			//((If libido is less than 50));
			if( CoC.getInstance().player.lib < 50 ) {
				EngineCore.outputText( 'The hulking orc is surprisingly gentle as he cleans you like a young child.  The warm waters and careful touch soothe your tired muscles.  Your skin becomes textured with goose bumps as the tickles of droplets roll down your body and the filth is washed down to the waters below.  You find yourself sighing in relaxation from the treatment.  A smile curls your lips as you look down at Ro\'gar, and he returns the pleased look with a toothy smirk.\n\n', false );
			}
			//((If libido is 50 or more));
			else {
				EngineCore.outputText( '  Something about the burly orc rubbing you down with warm water excites you.', false );
				if( CoC.getInstance().player.hasVagina() ) {
					EngineCore.outputText( '  Your ' + Descriptors.vaginaDescript( 0 ) + ' starts to quiver as the warm water rolls over its lips, moistening in excitement as your skin is rubbed over by strong hands.  Your chest heaves as it relaxes to his touch, your ' + Descriptors.nippleDescript( 0 ) + 's becoming stiff under his rubbing.', false );
					EngineCore.outputText( '  Even your ' + CoC.getInstance().player.feet() + ' and fingers curl as you smell the sweat and musk of Ro\'gar\'s closeness, the animalistic scent arousing you.', false );
				} else if( CoC.getInstance().player.hasCock() ) {
					EngineCore.outputText( '  ' + Descriptors.SMultiCockDesc() + ' stirs to life, making it obvious how much you are enjoying the rub down, then begins to drool with lust.  Through lidded eyes you faintly make out Ro\'gar\'s toothy smirk becoming a hungry grin.', false );
				}
				EngineCore.dynStats( 'lus', 5 + CoC.getInstance().player.lib / 20 + CoC.getInstance().player.sens / 20 );
				EngineCore.outputText( '\n\n', false );
			}
			EngineCore.outputText( '"<i>Clean as a whistle.</i>"  Ro\'gar chuckles as he drops the towel in the bucket with a splash.  His face beams with pride at his work, chest puffing out as he looks you over.  He eyes you up and down again, and your curious look just makes him snicker.  "<i>Don\'t meet too many friendly folk \'round here, but it would be nice to have a ' + CoC.getInstance().player.mf( 'feller', 'lass' ) + ' to talk with.</i>"  After a few moments of awkward rummaging through his pocket he pulls out a scrunched up piece of paper and hands it to you.  "<i>This\'s so you can find me.</i>"  Ro\'gar grins from ear to pointy ear.  You unfold the paper, and see a crudely drawn map that vaguely explains how to get to Ro\'gar\'s hut.  You wonder if making this map was why he took so long.  "<i>Y\'all take care now, y\'hear!</i>" he hollers, turning summarily and walking away, waving over his shoulder at you as he tromps through his natural habitat.  You wander back to your camp, wondering if you should see him again.\n\n', false );
			...
			CoC.getInstance().flags[ kFLAGS.ROGAR_PHASE ] = 1;
			EngineCore.doNext( SceneLib.camp.returnToCampUseTwoHours );
		}
	};
	//sexy choices;
	...
	Rogar.prototype.noSlowBroIDontWantPokeSex = function() {
		...
		CoC.getInstance().flags[ kFLAGS.ROGAR_DIRT ] = 1;
		CoC.getInstance().flags[ kFLAGS.ROGAR_PHASE ] = 2;
		EngineCore.doNext( SceneLib.camp.returnToCampUseTwoHours );
	};
	//((Ok));
	Rogar.prototype.okayBroLetsHaveAGayCarwash = function() {
		...
		EngineCore.outputText( ' aches, each step of the way rubbing the sensitive', false );
		if( CoC.getInstance().player.gender > 0 ) {
			EngineCore.outputText( ', drooling', false );
		}
		EngineCore.outputText( ' parts against the fabric of your clothes.\n\n', false );
		...
		CoC.getInstance().flags[ kFLAGS.ROGAR_DIRT ] = 2;
		CoC.getInstance().flags[ kFLAGS.ROGAR_PHASE ] = 2;
		if( CoC.getInstance().player.inte < 30 ) {
			EngineCore.doNext( SceneLib.camp.returnToCampUseTwoHours );
		}
	...
	Rogar.prototype.rogarThirdPhase = function() {
		...
		if( (CoC.getInstance().player.biggestTitSize() >= 2) ) {
			if( CoC.getInstance().flags[ kFLAGS.ROGAR_WARNING ] === 0 ) {
				EngineCore.outputText( 'Judging the patron\'s figure based on how imposingly filled out the cloak is, he or she is clearly very muscled.  You spy a green hand reach for a can and a spark of recognition hits you.  You\'re completely unsure how to break the ice, but tug on the hem of the cloak.  The figure turns toward you, and you\'re greeted with the green, toothy smile of Ro\'gar, the orc from the swamp!  "<i>H\'lo, ' + CoC.getInstance().player.short + '!  I\'m s\'rprised to see ya here...</i>"\n\n', false );
				EngineCore.outputText( 'You greet him warmly, but he seems a bit nervous around you.  "<i>Well... after all ya tol\' me about yer adventures, I, ah, thought I\'d hit the road m\'self.  An\' so ya find me here.  Good ta see ya again, but I\'m just finishin\' up my drink afore I head on out.</i>"  With this, he tips back the contents of his can and flashes you his standard grin, then departs.  He seemed a bit uneasy around you.', false );
				//set Ro'roh Raggy flag to 1;
				CoC.getInstance().flags[ kFLAGS.ROGAR_WARNING ] = 1;
			}
		...
		//((androgynous or masculine and breasts <= A-cup));
			...
			EngineCore.outputText( 'After taking a few moments to look over the familiar cloth of the cloak, your lips curl into a small smile.  Walking up quietly, ', false );
			//[(if Ro'roh Raggy flag >=1);
			if( CoC.getInstance().flags[ kFLAGS.ROGAR_WARNING ] >= 1 ) {
				EngineCore.outputText( 'you tap him on the shoulder.  He turns around, then assumes a pleasant smile.  "<i>Well, h\'lo.  Who might you be?</i>"  Momentarily confused, it quickly comes to you that you were quite different at your prior meetings; you reintroduce yourself with a small bow.  "<i>Wow.  ' + CoC.getInstance().player.short + ', ya look completely different than before!</i>"  You nod, slightly abashed at having so much attention drawn to your changes, and point out that this world can have some unusual effects on people.  He nods eagerly.  "<i>Well, ya look jus\' great.  How \'bout havin\' a sit and chattin\' with me fer a while?</i>"\n\n', false );
			}
			//(else if Ro'roh Raggy < 1 and player height >= 5 feet);
			else if( CoC.getInstance().player.tallness >= 60 ) {
				EngineCore.outputText( 'you carefully wrap your arms around the figure\'s head and cover his eyes with your hands.  A playful giggle escapes your lips as you murmur for him to "<i>guess who.</i>"  A familiar voice growls, but quickly relaxes as orcish hands cup over yours.  "<i>Dag nabbit ' + CoC.getInstance().player.short + ', don\'t sneak up on a feller like that.</i>"  Ro\'gar\'s low tone fills your ears as he turns to face you with a wide grin.\n\n', false );
			}
			//else if Ro'roh Raggy < 1 and player height < 5 feet);
			else {
				EngineCore.outputText( 'you gently tug on the figure\'s cloak, mentioning aloud that it seemed to have held up all right with your sewing job.  The figure\'s back straightens sharply and a deep snicker is heard.  "<i>Ya did a mighty fine job, ' + CoC.getInstance().player.short + '.  Been waitin\' for ya.</i>"  Ro\'gar turns and grins at you.\n\n', false );
			}
			//((Paths combine here));
			EngineCore.outputText( 'Returning the grin with your own, you take a seat next to Ro\'gar.  "<i>It was some trip hoofin\' it all the way here, but I reckon it was worth it.</i>"  His deep, familiar tone soothes your mind.  Ro\'gar grips the can off the bar and drinks heavily before exhaling loudly, the container thudding as he rests it on the counter. "<i>Would have dried up in that sun if not fer this here cloak.</i>"  His head turns to flash a grin from under the hood.  You carefully reach a hand up to pull it back, wanting to see his face.  Ro\'gar flushes lime green at that and turns his head away to scratch at his cheek with a calloused finger.', false );
			//[(if playerskincolor = green);
			if( CoC.getInstance().player.skinTone === 'green' ) {
				EngineCore.outputText( '  "<i>Hey... Have y\'ever had somebody randomly run up an\' ask ya ta \'Waaagh!\'?</i>" he mutters quietly.  You give him a perplexed look.  The orc laughs off your confusion dismissively.  "<i>Nothin\', nothin\'.</i>"', false );
			}
			EngineCore.outputText( '\n\n', false );
			EngineCore.outputText( '"<i>Things\'ve been really swell since I got here,</i>" he continues, artlessly changing the subject.  "<i>Got my own place, been workin\' mah rear off... not to mention the gym.  I\'ve been hittin\' that baby when I\'m not drinkin\' here.</i>"  He shifts the cloak behind himself and begins flexing for you, confident in his stature as he gives you a show.  It\'s apparent as he flexes that he\'s even bigger than he was in the swamp.\n\n', false );
			//variable ending;
			//[(if Dirt Mc Girt = 0);
			if( CoC.getInstance().flags[ kFLAGS.ROGAR_DIRT ] === 0 ) {
				EngineCore.outputText( 'You chat for a while longer before separating.  Ro\'gar extracts a promise from you to come drink with him again before waving you off with his standard grin.', false );
			...
			//(else if Dirt Mc Girt = 2);
				...
				EngineCore.outputText( 'Your heart flutters as you realize he is still only wearing the ragged shorts beneath his hooded cloak, and your eyes rake over his musculature as he models.  "<i>How\'s about you an\' I go visit the baths sometime?</i>"  Ro\'gar asks coyly, relaxing his body as he takes another drink.  "<i>You been nothin\' but good ta me ever since I met ya, ' + CoC.getInstance().player.short + '.  I do ya one favor and you been a real friend ever since.</i>"  Ro\'gar\'s tone grows lower and more serious as a calloused finger rubs over his damp can.  You rest your arms against the polished wood of the bar as you listen.  "<i>Yer special ta me.</i>" he mutters quietly, just loud enough for you to hear him.  You smile warmly and rest a hand against his arm, coaxing him to turn his head to look at you.  His tongue slides over his lips as he gives you a seductive stare, with his hand reaching between your legs.', false );
				//[(female or unsexed);
				if( CoC.getInstance().player.gender === 2 || CoC.getInstance().player.gender === 0 ) {
					...
					EngineCore.outputText( '  ' + Descriptors.SMultiCockDesc() + ' gets a caress from him as he explores, arousing it to mild stiffness.  "<i>' + CoC.getInstance().player.short + ',</i>" he grunts.  "<i>Ah hope ah\'ll be seein\' more of ya.</i>"  Ro\'gar breaks the contact and tips back the remains of his drink while you ', false );
					if( CoC.getInstance().player.cor < 50 ) {
						EngineCore.outputText( 'blush', false );
					} else {
						EngineCore.outputText( 'fantasize about those strong hands touching every part of you', false );
					}
					EngineCore.outputText( '.  I drink here whenever I\'m not werkin\' out or on a job.  I\'ll be waitin\' for you.</i>"  He lowers his eyelids suggestively at you, then gets up; you smile dumbly as he leaves the bar, watching him go with a blush in your cheeks', false );
					//[(If player has a tail);
					if( CoC.getInstance().player.hasLongTail() > 0 && CoC.getInstance().player.isNaga() ) {
						EngineCore.outputText( ', and you feel your tail flicking rapidly in excitement', false );
					}
					EngineCore.outputText( '.\n\n', false );
	...
	Rogar.prototype.rogarPhaseFour = function() {
		...
		//(high fem or breasts >=B-cup and Ro'roh Raggy =1);
		else if( (CoC.getInstance().player.biggestTitSize() >= 2) && CoC.getInstance().flags[ kFLAGS.ROGAR_WARNING ] === 1 ) {
		...
		//(andro or masculine with tits <= A);
			...
			EngineCore.outputText( 'Ro\'gar is tipping his drink back when he catches sight of you, and waves you over.  "<i>Ah, good ta see ya, ' + CoC.getInstance().player.short + '!', false );
			//[(Ro'roh Raggy = 1);
			if( CoC.getInstance().flags[ kFLAGS.ROGAR_WARNING ] === 1 ) {
				EngineCore.outputText( 'Yer lookin\' much better today; ya been workin\' out?</i>"  You smile at the compliment, and ', false );
			} else {
				EngineCore.outputText( '</i>"  You ', false );
			}
			EngineCore.outputText( 'wave the bartender over for another round.  "<i>So,</i>" he begins once you\'ve finished your drinks and she\'s out of earshot, "<i>I was wonderin\' if ya would like to hang out at my place later?  It ain\'t much, but it\'d be a lot more... private... than here.</i>"  He colors slightly and waggles an eyebrow at you, then slides a hand into your groin.', false );
			//[(if F or U);
			if( CoC.getInstance().player.gender === 2 || CoC.getInstance().player.gender === 0 ) {
				...
				if( CoC.getInstance().flags[ kFLAGS.ROGAR_DIRT ] === 1 ) {
					EngineCore.outputText( '</i>"I know ya probably ain\'t ever imagined hangin\' out with a feller like me, but... I could really use the company tonight.</i>"  ', false );
				}
				EngineCore.outputText( 'He\'s clearly in the mood for some fun; do you want to head back to his place and do something?', false );
	...
	Rogar.prototype.rogarFuckMenu = function() {
		...
		var getTied = null;
		var rubdub = null;
		if( CoC.getInstance().player.hasItem( ConsumableLib.BROBREW ) ) {
	...
	Rogar.prototype.fuckRogarsButtPussyBoyCuntManMoundSissySlitQueerQuim = function() {
		...
		//((If cock is between 18 and 4 inches));
			...
			if( CoC.getInstance().player.cocks[ 0 ].cockType === CockTypesEnum.CAT ) {
				EngineCore.outputText( '  You grind around in his green pucker, the barbs working over Ro\'gar\'s inner walls.  "<i>W-what in tarnation?</i>" Ro\'gar grunts as you watch him squirm under you.  You buck your hips, digging deeper into orcflesh with each thrust, his muscled ass tensing with each slam of your hips.', false );
			}
			//(If horse cock);
			else if( CoC.getInstance().player.cocks[ 0 ].cockType === CockTypesEnum.HORSE ) {
				EngineCore.outputText( '  The flared head of your ' + Descriptors.cockDescript( 0 ) + ' nestles in Ro\'gar\'s hole.  His breath quickens as you push in, the girthy, flared head stretching Ro\'gar\'s insides impossibly.  You simply thrust and buck you way deeper within the green pucker to force more of your lengthy shaft past that olive rosebud each time.\n\n', false );
			}
			//(If dog cock);
			else if( CoC.getInstance().player.hasKnot( 0 ) ) {
				EngineCore.outputText( '  Your pointed tip slips in easily and you begin to plunge into Ro\'gar\'s depths, and the burly orc gives satisfied grunts as his strong hand reaches down to touch himself.  You begin to fill his orc hole, the pre-slicked cock working as a lubing tool as you begin to build a nice rhythm within Ro\'gar.  Your knot bulges as your climax approaches and begins to ram into Ro\'gar\'s green pucker, the tight ring unable to allow entry as you swell bigger and bigger.', false );
			}//(If other cock);
			else {
				EngineCore.outputText( '  Breaking Ro\'gar\'s orc hole open, you thrust deep within his hot, tight depths.  The burly orc pants with heated breaths, humid air fluttering out of his tusked mouth.  His musclegut heaves, each breath punctuating the thrusts of your hips.  You plow his ass roughly, pulling out until only the head remains inside him before plunging back in further than before.  Ro\'gar grunts low in his chest with each bit of progress made, his strong, calloused hand stroking his drooling cock.  "<i>Mmmf, ' + CoC.getInstance().player.short + ',</i>" he groans as you finally hilt yourself within him.', false );
			}
	...
	Rogar.prototype.takeDatBroBrewFromDaBigMeanOlOrc = function() {
		...
		if( CoC.getInstance().player.findPerk( PerkLib.BroBody ) < 0 ) {
			...
			if( CoC.getInstance().player.inte > 35 ) {
				CoC.getInstance().player.inte = 35;
			...
			if( CoC.getInstance().player.biggestTitSize() >= 1 ) {
				if( CoC.getInstance().player.breastRows.length > 1 ) {
					while( CoC.getInstance().player.breastRows.length > 1 ) {
						CoC.getInstance().player.removeBreastRow( CoC.getInstance().player.breastRows.length - 1, 1 );
					}
				}
			...
			if( CoC.getInstance().player.hasCock() ) {
				if( CoC.getInstance().player.cocks[ 0 ].cockLength < 10 ) {
					if( CoC.getInstance().player.cocks[ 0 ].cockThickness < 2.75 ) {
			...
			if( CoC.getInstance().player.femininity > 0 ) {
				CoC.getInstance().player.modFem( 0, 100 );
			}
			//max tone.  Thickness + 50;
			if( CoC.getInstance().player.tone < 100 ) {
				CoC.getInstance().player.modTone( 100, 100 );
			}
			if( CoC.getInstance().player.thickness < 100 ) {
				CoC.getInstance().player.modThickness( 100, 50 );
			}
			if( CoC.getInstance().player.inte > 21 ) {
				CoC.getInstance().player.inte = 21;
			}
		...
		if( changed ) {
			EngineCore.outputText( '  You realize you look much stronger, bigger, and sexier', false );
			//(if dick grew);
			if( cocked ) {
				EngineCore.outputText( ', and you smile to yourself as your now-larger endowment rubs against garments not suited to it', false );
			}
			EngineCore.outputText( '.  Who needs book smarts anyway?  Nerds.', false );
			//lose lust, gain bro traits, slimefeed;
			CoC.getInstance().player.createPerk( PerkLib.BroBrains, 0, 0, 0, 0 );
			CoC.getInstance().player.createPerk( PerkLib.BroBody, 0, 0, 0, 0 );
	...
	Rogar.prototype.kinkyWithDaOrc = function() {
		...
		EngineCore.outputText( 'over the threshold, and then look around the apartment.  He points out the small kitchen area and then, rather sheepishly, the bedroom.  You\'re examining it when something catches your eye...\n\n', false );
		EngineCore.outputText( 'You bend over the bed to grip a small bottle lying in the soft fabric of the sheets.  Turning it over, you tilt your head as you read the label.  It\'s... a bottle of lubricant.  Suddenly, there is a loud thud behind you and the bottle drops from your hand as your torso is pressed into the cushioned surface of the bed.  Something presses into your back.  "<i>Gotcha...</i>" the familiar voice snickers in your ear.  Ro\'gar\'s head leans over your shoulder and a long, orcish tongue licks your cheek.  "<i>Was waitin\' for this...</i>" he purrs; the resonance of his voice rumbles against your ear.  Ro\'gar chuckles and grips around your wrists.  "<i>Now that yer here, it\'s time ta break in this here new bed with sumthin\' I\'ve been cravin\' since I washed ya down in that swamp, boy.</i>"  Ro\'gar\'s tone suddenly takes a demanding, hissing tone.  His tongue flicks over your ear as he speaks.  Your skin begins to grow goosebumps as shivers crawl down your spine.\n\n', false );
		EngineCore.outputText( 'He yanks your wrists behind your back, ignoring your faint gasps.  With a rustle of cloth, your wrists are bound together behind your back tightly; your fingers feel over the bonds but you struggle against them in vain.  Looking over your shoulder in ', false );
		if( CoC.getInstance().player.lib < 33 ) {
			EngineCore.outputText( 'feigned confusion ', false );
		} else if( CoC.getInstance().player.lib > 66 ) {
			EngineCore.outputText( 'clear lust ', false );
		}
		EngineCore.outputText( 'at Ro\'gar, you only see his lust-ridden face dipping down to kiss at your lips.  His tusks press against your cheeks with a demanding snarl and his tongue into your mouth; your ' + Descriptors.buttDescript() + ' receives a smack, forcing you to yelp.  His tongue quickly invades, and his free hand unfastens your ' + CoC.getInstance().player.armorName + ' to strip you forcefully.  Rough, calloused fingers dance over your body and fasten onto your ' + Descriptors.nippleDescript( 0 ) + 's, coaxing moans from your throat with each pinch.  Your pleased noises are muffled by Ro\'gar\'s mouth, his tongue and lips hushing each whimper and gasp you breathe.  A bare, firm chest presses against your exposed back, and you pant as the heat from his body burns against you, only intensified by the thin layer of oil on his skin.  Ro\'gar\'s blue eyes watch you like a predator as your face flushes.  He breaks the kiss and cups his hands under your chin to hold your face in place.  "<i>Such a perdy face yer makin\', ' + CoC.getInstance().player.short + '.  Yer lookin\' at me like a proper slut, an\' it\'s makin\' me hard seein\' ya so needy to be bred.</i>"  He growls in his throat as his mouth nips over your neck, leaving red marks along your skin where his tusks rub against you.  You can feel the oil and sweat smearing over your back, and your senses go wild.  The room is soon filled with your needy whimpers as ' + Descriptors.sMultiCockDesc() + ' oozes and throbs against the sheets, making the fabric damp with your lust.\n\n', false );
		...
		if( CoC.getInstance().player.ass.analLooseness < 3 ) {
			EngineCore.outputText( ' with some resistance, but the relentless finger rubs over your walls as he begins to pull your rim open, forcing another finger inside and making you bite your lip from the stinging sensation.  Working the pair of fingers together inside of you, he slowly moves them apart and stretches your hole wide.', false );
		} else if( CoC.getInstance().player.ass.analLooseness < 5 ) {
			EngineCore.outputText( ' and rubs over your walls as he pulls your rim open, forcing another finger inside and making you moan like a bitch in heat.  Working the pair of fingers together inside of you, he slowly moves them apart and stretches your hole wide.', false );
		}
		EngineCore.outputText( '  Fingers and ' + CoC.getInstance().player.feet() + ' curl in pleasure as your boycunt is forced open by Ro\'gar\'s steady coaxing.', false );
		...
		if( CoC.getInstance().player.ass.analLooseness < 3 ) {
			EngineCore.outputText( 'the tight hole.  Your teeth clench as he stretches you wide around his length, though he continues without hesitation, and finally you cry out with the sting of the bulbous head squeezing into your tiny hole, though it falls on deaf ears as Ro\'gar merely groans at your tightness.', false );
		} else if( CoC.getInstance().player.ass.analLooseness < 5 ) {
			EngineCore.outputText( 'your well-used hole. You moan with need as he stretches you wide around his length, continuing without hesitation until he\'s inside.', false );
		}
		CoC.getInstance().player.buttChange( 25, true, true, false );
	...
	Rogar.prototype.rubdubWithDaOrcWithoutDucky = function() {
		...
		EngineCore.outputText( '', true );
		EngineCore.spriteSelect( 86 );
		EngineCore.outputText( 'By the time you reach Ro\'gar he has already reached his bedroom, his shorts on the floor. You eye over his olive green skin, all of it laid bare before you including his limp orc cock hanging over his hefty nutsack.\n\n', false );
		EngineCore.outputText( '"<i>I purchased these oils to relax my muscles after spending so much time in the gym.</i>" Your orc lover motions to a few bottle of oils on his night stand. "<i>How about you give me a hand... or two.</i>" Ro\'gar grins at you, his eyebrows twitching suggestively. You need no further provocation as you strip from your ' + CoC.getInstance().player.armorName + ', you let it fall into a pile on the floor as you make for the bottle. Ro\'gar grins and rolls onto his stomach.\n\n', false );
		EngineCore.outputText( 'The bottles pop open easily and you pour a generous amount over your hand, you spill even more over Ro\'gar back. He gasps feeling the cool liquid crawl down the spine of his back. Though it\'s not long after you warm his skin with your hands. Your fingers spread and smear the oils over his damp skin. You crawl up and straddle Ro\'gar\'s hips, sitting on his muscled ass as you begin to rub the oils in.\n\n', false );
		EngineCore.outputText( 'Ro\'gar\'s skin under your fingertips feels stiff, his muscles resisting your insistent touches. Slowly you can feel them relax, they smooth out and grow warm under your palms. It\'s not long until Ro\'gar is purring loud enough for you to hear. With a pleased growl Ro\'gar shifts under your body briefly before letting you continue.\n\n', false );
		EngineCore.outputText( '"<i>That\'s the spot.</i>" Ro\'gar grunts as you work your thumbs over a certain spot near his shoulders. You curve your fingers around his broad shoulder and gently pull the muscles to stretch away from the neck. Ro\'gar\'s mouth bellows a moan as his head falls to the sheets. The oils mixing with the strange liquid secreted from the orc\'s skin making a curious concoction. You can feel your hands feeling so warm and soothed, the muscles in your hands relaxed and comforted. As you look along Ro\'gar glistening back you wonder how it would feel to have such a large area affected by the slurry.\n\n', false );
		EngineCore.outputText( 'You ask Ro\'gar to flip over as you get off of his hips to get more oils. By the time you turn around your orc lover has already laid out on his back. Your eyes take in the sight of your muscled lover laid out for you, his muscled limbs bulging in anticipation, his swollen cock growing larger before your eyes. He grins at you as you pour oils over his chest, the cool liquid making him shiver as it drips down along his center. The clear oil crawling in between his pectorals, down his treasure trail and dipping into his navel before filling the gaps in his abdominals.\n\n', false );
		...
		EngineCore.outputText( '"<i>You\'re good at this ' + CoC.getInstance().player.short + ', have you done this before?</i>" Ro\'gar purrs as you work over his abdominals, tracing over every gap with your fingertips, the oils glistening over your lover\'s body. Ro\'gar\'s hips buck gently, bumping your ' + Descriptors.buttDescript() + ' with his throbbing cock with a chuckle, his mouth curls into a coy smile. You watch as Ro\'gar reaches his arms back, resting his head on his hands as he bares his underarm and pits.\n\n', false );
