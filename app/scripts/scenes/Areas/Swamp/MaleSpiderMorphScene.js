...
angular.module( 'cocjs' ).run( function( MaleSpiderMorph, EventParser, ConsumableLib, Utils, PregnancyStore, Combat, CoC, EngineCore, Descriptors, OnLoadVariables ) {
	...
	MaleSpiderMorphScene.prototype.defeatSpiderBoy = function() {
		...
		if( CoC.getInstance().player.hasCock() ) {
			...
			if( CoC.getInstance().player.biggestCockArea() > CoC.getInstance().monster.analCapacity() ) {
	...
	MaleSpiderMorphScene.prototype.loseToMaleSpiderMorph = function() {
		...
		if( CoC.getInstance().player.gender === 0 ) {
			EngineCore.spriteSelect( 74 );
			EngineCore.outputText( 'The raven-haired spider-guy tears off your ' + CoC.getInstance().player.armorName + ', and disgusted by what he finds, he kicks you in the head, knocking you out.', true );
	...
	MaleSpiderMorphScene.prototype.victoryButtFuck = function() {
		...
		EngineCore.outputText( ' against your ' + CoC.getInstance().player.armorName + ', eager to take the dark hole.\n\n', false );
		...
		EngineCore.outputText( '  Meanwhile, you grab ' + CoC.getInstance().player.oMultiCockDesc() + ' and line it up with the spider-boy\'s twitching rectum, rubbing the ' + CoC.getInstance().player.cockHead( x ) + ' over the tight little hole while you wait for it to slowly loosen.  A fresh dollop of pre-cream bubbles out of his cum-frothing foreskin in response, letting you know that in spite of his protests, your reluctant lover\'s body loves the anal stimulation.\n\n', false );
		EngineCore.outputText( 'After a little more gentle pressure and a few more gentle caresses of his testicles, the defeated male finally relaxes enough for you to put the first inch inside him.  His tunnel is so hot that it feels like your ' + Descriptors.cockDescript( x ) + ' is going to melt inside his sweltering embrace, and much worse, his sphincter clamps down hard enough to trap your ' + CoC.getInstance().player.cockHead( x ) + ' inside his toasty tail-hole.  Holding tightly to his ass and balls, you yank your clenching, anal slut down hard, forcing him to stretch while the rest of your ' + Descriptors.cockDescript( x ) + ' burrows deep inside him', false );
	...
	MaleSpiderMorphScene.prototype.victoryCowgirlRidingOnSpiderBoi = function() {
		...
		EngineCore.outputText( '', true );
		EngineCore.spriteSelect( 74 );
		EngineCore.outputText( 'You hastily remove your ' + CoC.getInstance().player.armorName + ' to bare yourself in preparation for the coming pleasure.  Throughout it all, the spider-boy doesn\'t react.  He\'s too busy ', false );
		...
		EngineCore.outputText( ' to summon a response.  How irritating.  You push him over with your ' + CoC.getInstance().player.foot() + ' to roll him onto his back, giving you the access you need to get at his ', false );
		if( CoC.getInstance().monster.HP < 1 ) {
		...
		if( CoC.getInstance().player.cor < 33 ) {
			...
			EngineCore.outputText( 'paying no heed to the thick flows of spunk that drip down your ' + CoC.getInstance().player.legs() + '.', false );
		}
		CoC.getInstance().player.slimeFeed();
		//Pregnancy Goes Here;
		CoC.getInstance().player.knockUp( PregnancyStore.PREGNANCY_SPIDER, PregnancyStore.INCUBATION_SPIDER, 151 );
		CoC.getInstance().player.orgasm();
		Combat.cleanupAfterCombat();
	};
	...
	MaleSpiderMorphScene.prototype.spiderBoyLossMaleButtfuck = function() {
		...
		if( CoC.getInstance().player.isTaur() ) {
			...
			EngineCore.outputText( CoC.getInstance().player.legs() + ' up and out of the way', false );
		}
		...
		EngineCore.outputText( '  Pumping his hips slowly, the spider-morph watches a few more weak trickles of seed squirt out.  He gives your ass a hearty smack and muses out loud, "<i>That\'s a good ' + CoC.getInstance().player.mf( 'boy', 'girl' ) + '.  You make a', false );
		if( CoC.getInstance().player.analCapacity() >= 40 ) {
	...
	MaleSpiderMorphScene.prototype.loseToSpiderBoyVagFucked = function() {
		...
		EngineCore.outputText( ' you, your ' + CoC.getInstance().player.legs() + ' grow shaky, then give out entirely.  You flop down on your back', false );
		if( CoC.getInstance().player.lust > 99 ) {
		...
		CoC.getInstance().player.slimeFeed();
		CoC.getInstance().player.orgasm();
		CoC.getInstance().player.knockUp( PregnancyStore.PREGNANCY_SPIDER, PregnancyStore.INCUBATION_SPIDER );
		Combat.cleanupAfterCombat();
	};
