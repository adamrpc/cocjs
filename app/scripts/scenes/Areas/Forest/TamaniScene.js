...
angular.module( 'cocjs' ).run( function( SceneLib, $rootScope, $log, PregnancyStore, kFLAGS, CoC, Utils, EngineCore, Descriptors, Combat, Tamani, AppearanceDefs, Appearance, CockTypesEnum, PerkLib ) {
	...
	TamaniScene.prototype.timeChange = function() {
		...
		$log.debug( '\nTamani time change is ' + CoC.getInstance().time.hours + ', incubation: ' + this.pregnancy.incubation + ', event: ' + this.pregnancy.event );
	...
	TamaniScene.prototype.tamaniGivesBirth = function() {
		if( this.pregnancy.type === PregnancyStore.PREGNANCY_PLAYER ) { //Don't want drider eggs to add to her daughers
			CoC.getInstance().flags[ kFLAGS.TAMANI_NUMBER_OF_DAUGHTERS ] += CoC.getInstance().flags[ kFLAGS.TAMANI_PREGNANCY_COUNT ];
			CoC.getInstance().flags[ kFLAGS.TAMANI_PREGNANCY_COUNT ] = 0;
			CoC.getInstance().flags[ kFLAGS.TAMANI_TIMES_IMPREGNATED ]++;
		}
		this.pregnancy.knockUpForce(); //Clear Pregnancy
	};
	TamaniScene.prototype.tamaniChest = function() {
		var descript = '';
		switch( CoC.getInstance().flags[ kFLAGS.TAMANI_TIMES_IMPREGNATED ] ) {
			case -1:
			case  0:
	...
	TamaniScene.prototype.tamaniFemaleEncounter = function() {
		...
		//[Umm OK?] [No]
		EngineCore.choices( 'Umm OK?', this.tamaniFemaleYes, 'No', this.tamaniFemaleNo, 'PreferGirls', this.preferTamaniFemdom, '', null, '', null );
	};
	//(Umm OK?)
	...
	TamaniScene.prototype.tamaniMaleFirstEncounter = function() {
		EngineCore.spriteSelect( 56 );
		CoC.getInstance().flags[ kFLAGS.TAMANI_MET ] = 1; //Indicates you've met her as a male at least once
		EngineCore.outputText( '', true );
		...
		//[Fuck Her] [Refuse]
		EngineCore.choices( 'Fuck Her', this.tamaniFirstTimeConsentual, 'Refuse', this.tamaniFirstTimeRefusal, '', null, '', null, '', null );
	};
	...
	TamaniScene.prototype.tamaniFirstTimeConsentual = function() {
		...
		this.tamaniKnockUp();
		EngineCore.outputText( '', true );
		EngineCore.outputText( 'You almost can\'t believe your good fortune.  Finally you meet a creature willing to talk instead of just launching into violent rape.   Her direct advances were so crude and overtly sexual that you felt yourself stiffening before she could even finish her offer.   Your decision was made by the tent in your ' + CoC.getInstance().player.armorName + '.  You\'ll give Tamani exactly what you both want.\n\n', false );
		EngineCore.outputText( 'Her hips sway seductively as she approaches with her right hand dipping into the moist honeypot of her sex.  You disrobe, throwing your ' + CoC.getInstance().player.armorName + ' to the side before you reach down and lift her up, pressing her curvy body against your ', false );
		...
		//(FITS)
		if( CoC.getInstance().player.cockArea( 0 ) <= 90 ) {
			EngineCore.outputText( 'Tamani breaks the kiss and gives you a coy smile as she shimmies down your body, dropping her moist cunt onto your ' + Descriptors.cockDescript( 0 ) + '\'s ' + CoC.getInstance().player.cockHead() + '.  She swings her hips in a little circle, teasing you with her moist entrance as your drug-enhanced pre-cum bubbles and drools around her lips, mixing with her own copious fluids as it flows down your length', false );
			if( CoC.getInstance().player.balls > 0 ) {
		...
		//(TOO BIG)
			...
			EngineCore.outputText( 'Tamani breaks your sloppy kiss and shimmies down your body, clutching tightly to your ' + Descriptors.cockDescript( 0 ) + ' and ' + CoC.getInstance().player.skinDesc + ' as she settles down lower on your groin.  The goblin somehow manages to turn herself around so that is she is hanging upside-down, with her legs and arms clutching tightly to your member while her tongue ', false );
			if( CoC.getInstance().player.hasSheath() ) {
				...
				if( CoC.getInstance().player.balls > 0 ) {
					...
					EngineCore.outputText( 'licks the sensitive ' + CoC.getInstance().player.skinTone + ' ' + CoC.getInstance().player.skinDesc + ' of your inner thighs', false );
				}
			}
			...
			if( CoC.getInstance().player.balls > 0 && CoC.getInstance().player.hoursSinceCum > 200 ) {
	...
	TamaniScene.prototype.tamaniFirstTimeRefusal = function() {
		EngineCore.spriteSelect( 56 );
		EngineCore.outputText( '', true );
		EngineCore.outputText( 'Tamani\'s eyes widen in surprise, "<i>Don\'t let the size fool you, big ' + CoC.getInstance().player.boyGirl() + '. I can take more than you think,</i>" she says while her hands begins playing with her box, "<i>Are you sure you don\'t want to just let off a little steam?</i>"\n\n', false );
		//[Fuck Her (Goes to fuck her - consensual first time)]
		//[No means no]
		EngineCore.choices( 'Fuck Her', this.tamaniFirstTimeConsentual, 'No', this.tamaniSecondRefusal, '', null, '', null, '', null );
	...
	TamaniScene.prototype.tamaniMaleRepeatEncounter = function() {
		...
		if( this.pregnancy.isPregnant || CoC.getInstance().flags[ kFLAGS.TAMANI_NUMBER_OF_DAUGHTERS ] > 0 ) {
			EngineCore.outputText( 'While exploring, you\'re startled by the feeling of tiny hands stroking the insides of your thighs.  You look down and find Tamani there, grinning wolfishly,  "<i>Ready for another fuck, big ' + CoC.getInstance().player.boyGirl() + '?"\n\n', false );
		}//(ELSE)
		else {
		...
		//[Let Her – Get dommed]
		//[No – starts fight]
		EngineCore.choices( 'Take Her', this.tamaniSexWon, 'Let Her', this.tamaniSexLetHer, 'No', this.tamaniStartFight, '', null, '', null );
	};
	...
	TamaniScene.prototype.tamaniSexLetHer = function() {
		...
		if( CoC.getInstance().isInCombat() ) {
			...
			if( CoC.getInstance().player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_CENTAUR ) {
				...
				EngineCore.outputText( 'Tamani sidles up to you and pats your flank, cooing, "<i>There there horsey, everything\'s going to be okay.  Momma Tamani\'s going to take really good care of you.</i>"  Before you know it your ' + CoC.getInstance().player.armorName + ' is on the ground and the goblin is underneath you, climbing up to your crotch.\n\n', false );
			}
			//Not taur
			else {
				if( CoC.getInstance().player.HP < 1 ) {
		...
		//Voluntary submission
			...
			if( CoC.getInstance().player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_CENTAUR ) {
				...
				EngineCore.outputText( '"<i>Just lie there and take it ' + CoC.getInstance().player.mf( 'stud', 'slut' ) + ',</i>" she commands.\n\n', false );
			}
		}
		//Taur-zilla
		if( CoC.getInstance().player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_CENTAUR ) {
			...
			if( CoC.getInstance().player.cocks[ 0 ].cockLength <= 18 ) {
				...
				EngineCore.outputText( '"<i>Go on then,</i>" she breathes eventually. "<i>I guess you\'ve had enough. Cum for Mistress Tamani, ' + CoC.getInstance().player.mf( 'stud', 'slut' ) + '. Fill me to the brim.</i>" With this she finally works herself all the way to the bottom of your shaft, and with surprising strength grips your stomach and begins to pound her plump behind against your inner thighs with increasing force.\n\n', false );
		...
		//Not-taurs
			...
			if( CoC.getInstance().player.cockArea( 0 ) <= 90 ) {
				...
				if( CoC.getInstance().player.cumQ() < 250 ) {
					EngineCore.outputText( '.  I really enjoy our fucks ' + CoC.getInstance().player.mf( 'stud', 'hun' ) + ', and I can\'t wait to \'bump\' into you again.', false );
				} else {
			...
			//(Doesnt fit)
				...
				EngineCore.outputText( ' a rough slap.  Whimpering, you still your rebellious hips and allow her to play you like the world\'s most sensitive fiddle.  Tamani pulls your ' + CoC.getInstance().player.cockHead() + ' up and rocks herself against it, openly moaning as you dribble pre into her sopping gash.\n\n', false );
				...
				if( CoC.getInstance().player.hasVagina() ) {
					...
					CoC.getInstance().player.cuntChange( CoC.getInstance().player.vaginalCapacity() * 0.95, true );
					...
					CoC.getInstance().player.buttChange( CoC.getInstance().player.analCapacity() * 0.95, true );
	...
	TamaniScene.prototype.tamaniSexLost = function() {
		...
		EngineCore.outputText( 'Tamani pours oil over your abused backside, letting it soak in and start to numb the pain.  She pours quite a different type of oil into her hands and starts massaging you, letting the alchemical mixture make your skin tingle and enhance the sensation of pleasure while numbing away the pain of your cock being kept so... tightly bound.  Your arms and ' + CoC.getInstance().player.legs() + ' tremble from supporting yourself at such an awkward angle, struggling not to relax at the sudden influx of pleasure.  The feelings bring you to the edge, and you feel your body\'s internal muscles clenching, but the tight vibrating ring blocks your cum and your orgasm, leaving you backed up and desperate.\n\n', false );
		...
		EngineCore.outputText( ' and asks, "<i>So are you sorry for making me wait for my cream ' + CoC.getInstance().player.mf( 'stud', 'slut' ) + '?  If you are, just say <b>I\'m sorry mistress Tamani, please milk my cock whenever you want</b> and then beg me to cum.</i>"\n\n', false );
	...
	TamaniScene.prototype.tamaniSexWon = function() {
		EngineCore.spriteSelect( 56 );
		this.tamaniKnockUp();
		var x = CoC.getInstance().player.cockThatFits( 90 );
		if( x === -1 ) {
			x = CoC.getInstance().player.biggestCockIndex();
		}
		EngineCore.outputText( '', true );
		if( CoC.getInstance().player.cockArea( x ) <= 90 ) {
			...
			EngineCore.outputText( 'It\'s too tempting of a target to resist.  You open your ' + CoC.getInstance().player.armorName + ' and allow your ' + Descriptors.multiCockDescriptLight() + ' to flop free.  You\'re already hard from the enticing display, and in a moment you\'re pressing against her lust-slicked pussy', false );
			...
			EngineCore.outputText( '\n\nTamani winks, sucking on her finger as she staggers up.  She coos, "<i>Mmmm, Tamani loves it when her ' + CoC.getInstance().player.mf( 'sexy lover', 'stud' ) + ' dominates her.  Do you think your ', false );
			if( CoC.getInstance().player.balls > 0 ) {
		...
		//Too big? Jerk off with feet and bukkake
			...
			EngineCore.outputText( '\n\n', false );
			EngineCore.outputText( 'Tamani winks, blinking the cum out of an eye as you finish up, releasing her seed-covered form.  She coos, "<i>Mmmm, Tamani loves it when you dominate her, ' + CoC.getInstance().player.mf( 'stud', 'sexy' ) + '.  Do you think your ', false );
			if( CoC.getInstance().player.balls > 0 ) {
				EngineCore.outputText( 'sexy balls have ', false );
			} else {
				EngineCore.outputText( 'sweet cock has', false );
	...
	TamaniScene.prototype.tamaniPregnantEncounter = function() {
		...
		//[Fuck Her] [Refuse]
		EngineCore.choices( 'Fuck Her', this.tamaniPregnantFuck, 'Refuse', this.tamaniPregnantRefusal, '', null, '', null, '', null );
	};
	//[Refuse]
	...
	TamaniScene.prototype.tamaniPregnantFuck = function() {
		...
		//(Very large:
		if( CoC.getInstance().flags[ kFLAGS.TAMANI_TIMES_IMPREGNATED ] > 6 ) {
		...
		if( CoC.getInstance().player.cockArea( 0 ) <= 50 ) {
			...
			EngineCore.outputText( '"<i>Thanks for the good fuck, ' + CoC.getInstance().player.mf( 'stud', 'lover' ) + ',</i>" she says, smiling cruelly as you find yourself getting hard again from her drug-laced kisses.  Tamani doesn\'t hang around to help though; she waddles off, cradling her belly and smiling.\n\n', false );
			CoC.getInstance().player.orgasm();
			EngineCore.dynStats( 'lus', 25 );
		}
		//(Doesn't Fit)
		else {
			EngineCore.outputText( '"<i>Sorry ' + CoC.getInstance().player.mf( 'stud', 'lover' ) + ', you\'re just too big and <b>hard</b> for me.   Don\'t worry, I\'ll help you unload everything straight into my sweet cunt,</i>" she apologizes, ', false );
			if( CoC.getInstance().player.balls > 0 ) {
			...
			CoC.getInstance().flags[ kFLAGS.TAMANI_NUMBER_OF_DAUGHTERS ] += 3;
	...
	TamaniScene.prototype.tamaniPoopsOutBabies = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.TAMANI_PREGNANCY_COUNT ] > 2 ) {
			EngineCore.outputText( '  Moments later a third appears', false );
			if( CoC.getInstance().flags[ kFLAGS.TAMANI_PREGNANCY_COUNT ] > 3 ) {
				EngineCore.outputText( ', a fourth', false );
				if( CoC.getInstance().flags[ kFLAGS.TAMANI_PREGNANCY_COUNT ] === 5 ) {
		...
		EngineCore.outputText( '  She looks up at you and whispers breathily, "<i>Come fuck me a little later, I\'ll give you even more beautiful little girls.  These ' + Utils.num2Text( CoC.getInstance().flags[ kFLAGS.TAMANI_PREGNANCY_COUNT ] ) + ' will be fully grown in a week or two and out man-hunting, but none of them will please you like me, I promise.</i>"\n\n', false );
	...
	TamaniScene.prototype.tamaniKnockUp = function() {
		...
		CoC.getInstance().flags[ kFLAGS.TAMANI_PREGNANCY_COUNT ] = 2;
		var cum = CoC.getInstance().player.cumQ();
		//Breeder perk is awesome
		if( CoC.getInstance().player.findPerk( PerkLib.MaraesGiftStud ) >= 0 ) {
			CoC.getInstance().flags[ kFLAGS.TAMANI_PREGNANCY_COUNT ] += 3;
		}
		if( cum >= 50 && Utils.rand( 2 ) === 0 ) {
			CoC.getInstance().flags[ kFLAGS.TAMANI_PREGNANCY_COUNT ]++;
		}
	...
	TamaniScene.prototype.encounterTamani = function() {
		...
		//Dudezillaz:
		else if( CoC.getInstance().flags[ kFLAGS.TAMANI_MET ] === 0 ) {
	...
	TamaniScene.prototype.getRapedByTamaniYouHypnoSlut = function() {
		...
		var primary = CoC.getInstance().player.cockThatFits( 65 );
		var secondary = 0;
		var cocks = CoC.getInstance().player.totalCocks();
		EngineCore.outputText( 'Tamani crooks her finger and you come running, already feeling yourself straining against your ' + CoC.getInstance().player.armorName + ' for a chance to fuck your wife and mistress.  She giggles, "<i>Ohhh, how sweet, you\'re so ready to do your duty.</i>"\n\n', false );
		EngineCore.outputText( 'The goblin traces her fingers over the bulge in your ' + CoC.getInstance().player.armorName + ' before reaching inside of it to give you a tender squeeze.  You sigh, happy to have earned such tender treatment from your lovely wife.  Automatically, your hands undo your ' + CoC.getInstance().player.armorName + ', removing the troublesome garment so as not to interfere with your lovemaking.   Tamani gives you another squeeze and teases, "<i>Oh baby, you\'re hooked on Tamani\'s pussy, aren\'t you?  I can tell, you\'re practically trembling.  Let Tamani help take care of that for you.</i>"\n\n', false );
		EngineCore.outputText( 'She smirks and bends over, looking back up at you from between her legs.  She offers, "<i>Go ahead and stick it in.  A horny ' + CoC.getInstance().player.mf( 'boy', 'herm' ) + ' like you needs to have that delicious cock inside a nice, tight wife like myself.</i>"\n\n', false );
		//[FITS]
		if( primary >= 0 ) {
			...
			EngineCore.outputText( Descriptors.cockDescript( 0 ) + '.  Her warmth spreads over your ' + CoC.getInstance().player.cockHead( primary ) + ' slowly, making you twitch and drip inside her, lubricating her already dripping channel with even more slick fluids.  With a mighty heave you push inside her, feeling yourself penetrating deep inside her, directly into her fertile womb.  Tamani rubs your ' + CoC.getInstance().player.legs() + ' with her hands, giving you a massage as you bottom out against her, feeling her juices stain your ', false );
			if( CoC.getInstance().player.hasSheath() ) {
			...
			EngineCore.outputText( '  You don\'t stop until your ' + CoC.getInstance().player.cockHead( primary ) + ' is the only thing inside her.\n\n', false );
			...
			EngineCore.outputText( ' ' + CoC.getInstance().player.skinDesc + ' impacting her ass fill the air.  You fuck your wife harder and faster with every stroke, knowing you won\'t last more than a few more thrusts before you\'re painting her womb white.  Your wife is too cummed out to care or respond, ', false );
			if( cocks === 1 ) {
			...
			if( CoC.getInstance().player.hasKnot( primary ) ) {
				EngineCore.outputText( 'Your knot balloons inside her, locking her in place', false );
			} else if( CoC.getInstance().player.cocks[ primary ].cockType === CockTypesEnum.HORSE ) {
			...
			if( CoC.getInstance().player.cumQ() < 50 ) {
				...
				if( CoC.getInstance().player.cumQ() >= 1000 && CoC.getInstance().player.cumQ() < 4000 ) {
		...
		//[DOESNT FIT]
			...
			EngineCore.outputText( Descriptors.cockDescript( 0 ) + '.  Her warmth spreads over the ' + CoC.getInstance().player.cockHead() + ' of your ' + Descriptors.cockDescript( 0 ) + ', but will go no further.  You tug harder on her bouncing backside, but you\'re simply too large to fit inside your wife\'s beautiful pussy.  She turns around and places a small hand ', false );
			if( CoC.getInstance().player.biggestTitSize() < 1 ) {
			...
			EngineCore.outputText( CoC.getInstance().player.mf( 'strong man\'s', 'sexy lover\'s' ) + ' ' + Appearance.cockNoun( CockTypesEnum.HUMAN ) + '?</i>"\n\n', false );
	...
	TamaniScene.prototype.tamaniBeaten = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.TIMES_OVIPOSITED_TAMANI ] === 0 ) {
			EngineCore.outputText( 'my baby-' + CoC.getInstance().player.mf( 'daddy', 'momma' ) );
		} else {
		...
		EngineCore.outputText( '\n\nTamani exhales, "<i>Eyes up here, ' + CoC.getInstance().player.mf( 'stud', 'breeder' ) + '.  I want to watch your face while you fuck me,</i>" in a tone that promises delights beyond imagining.  "<i>Put it in my pussy and give me your babies.  There\'s nothing like fucking a woman who\'ll take your cock straight into her womb and let you bathe her eggs with cum' );
		if( CoC.getInstance().flags[ kFLAGS.TIMES_OVIPOSITED_TAMANI ] > 0 ) {
		...
		CoC.getInstance().flags[ kFLAGS.TIMES_OVIPOSITED_TAMANI ]++;
	...
	TamaniScene.prototype.preferTamaniFemdom = function() {
		...
		EngineCore.menu();
		EngineCore.addButton( 0, 'Accept', this.acceptTamaniFacesits );
		EngineCore.addButton( 1, 'Refuse', this.declineZeFacesits );
	};
	//(('Refuse' choice))
