...
angular.module( 'cocjs' ).run( function( SceneLib, MainView, $rootScope, $log, PregnancyStore, kFLAGS, CoC, Utils, EngineCore, Descriptors, Combat, Tamani, AppearanceDefs, Appearance, CockTypesEnum, PerkLib ) {
	...
	TamaniScene.prototype.tamaniFemaleYes = function() {
		...
		if( CoC.player.cor < 50 ) {
			MainView.outputText( 'Gross.\n\n', false );
		} else {
			MainView.outputText( 'Getting cock-slapped would\'ve been kind of hot...\n\n', false );
		}
		MainView.outputText( 'The goblin leaves you with a warning, "<i>Be careful, it likes to leak aphrodisiacs like crazy.  Believe me, those are FUN to get addicted to.  Oh, and remember – Tamani owns all the cocks around here, so if you ever grow one, come pay your dues!</i>"\n\n', false );
		MainView.outputText( '(<b>Deluxe Dildo acquired!</b>)', false );
	...
	TamaniScene.prototype.tamaniFemaleNo = function() {
		...
		if( CoC.player.cor < 33 ) {
			MainView.outputText( 'What a slut.', false );
		} else if( CoC.player.cor < 66 ) {
			MainView.outputText( 'How odd.', false );
		} else {
			MainView.outputText( 'You hope she misses a few.', false );
		}
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	...
	TamaniScene.prototype.tamaniMaleFirstEncounter = function() {
		...
		MainView.outputText( 'The goblin makes you an offer that\'s difficult to turn down, "<i>Hey there stud, want to fuck me pregnant?  I promise my box will milk your dick dry.  Just let Tamani take care of all your boners OK?</i>"', false );
		//[Fuck Her] [Refuse]
	...
	TamaniScene.prototype.tamaniFirstTimeConsentual = function() {
		...
		MainView.outputText( 'Her hips sway seductively as she approaches with her right hand dipping into the moist honeypot of her sex.  You disrobe, throwing your ' + CoC.player.armorName + ' to the side before you reach down and lift her up, pressing her curvy body against your ', false );
		if( CoC.player.biggestTitSize() > 1 ) {
			MainView.outputText( Descriptors.allBreastsDescript(), false );
		} else {
			MainView.outputText( 'chest', false );
		}
		MainView.outputText( '.  She wraps her tiny arms around your neck and kisses you passionately, letting her tongue slither through your lips.   The two of you french kiss hard, virtually tongue-fucking each other.\n\n', false );
		MainView.outputText( 'She breaks the kiss and smiles, licking the shining purple lipstick she wears as she whispers in your ear, "<i>Mmmhmm, I knew your juicy cock just couldn\'t resist a wet and ready pussy like mine.  I made sure to lace my lipstick with fertility enhancing chemicals too, so we\'ll be nice and messy.</i>"\n\n', false );
		...
		if( CoC.player.balls > 0 ) {
			MainView.outputText( 'Your ' + Descriptors.ballsDescriptLight() + ' swell with seed, spurring your desire to new heights.', false );
		} else {
			MainView.outputText( 'Something inside you swells up with seed, spurring your desire to new heights.', false );
		}
		MainView.outputText( '  You NEED to fuck her pussy full – NOW.\n\n', false );
		//(FITS)
		if( CoC.player.cockArea( 0 ) <= 90 ) {
			MainView.outputText( 'Tamani breaks the kiss and gives you a coy smile as she shimmies down your body, dropping her moist cunt onto your ' + Descriptors.cockDescript( 0 ) + '\'s ' + CoC.player.cockHead() + '.  She swings her hips in a little circle, teasing you with her moist entrance as your drug-enhanced pre-cum bubbles and drools around her lips, mixing with her own copious fluids as it flows down your length', false );
			if( CoC.player.balls > 0 ) {
				MainView.outputText( ' and drips from your ' + Descriptors.ballsDescriptLight(), false );
			}
			MainView.outputText( '.  She stops and teases, "<i>Ready to stuff me full of your cream?  I just KNOW I\'ll get pregnant from such a purrfect mate.</i>"\n\n', false );
			MainView.outputText( 'Tamani doesn\'t wait for an answer – she pauses until you\'re about to reply, then drops her weight down, fully impaling herself and turning the beginnings of your reply into a babbled moan.  She plants her feet on your thighs and her arms around your back and begins bouncing up and down rapidly, squeezing and contracting, milking your ' + Descriptors.cockDescript( 0 ) + ' in her tight wet walls the entire time. Your inner abdominal muscles begin clenching and squeezing, sending a wave of heat through your groin as your baby-batter begins its journey towards the goblin\'s womb.\n\n', false );
			MainView.outputText( 'You grab her with both hands and slam her down, taking her to the hilt', false );
			if( CoC.player.cockArea( 0 ) > 30 ) {
				MainView.outputText( ' and watching her belly bulge from your size', false );
			}
			MainView.outputText( '.  She twists violently, practically thrashing in your arms as spunk begins pouring into her womb, making her belly start to bloat.  The goblin babbles incoherently with each blast of cum, stretching tighter and tighter around you as her pussy works to hold in every drop of spunk.  Her belly bloats a bit more, until the pressure is too much to bear and jism begins spurting around her opening, splattering into a puddle on the ground.\n\n', false );
			MainView.outputText( 'All good things eventually end, and with a sigh you pull the insensate goblin slut free of your ' + Descriptors.cockDescript( 0 ) + ', watching a river of whiteness drain from between her thighs.  You set her down and the escaping jism suddenly stops, the remainder held inside by some kind of reflex.  Tamani giggles and pats her still pregnant-looking belly, "<i>Wasn\'t the sample nice?  Come see me when your dick has had a chance to recover and we can do this again, and again, and again.  You\'re practically hooked already aren\'t you ' + CoC.player.mf( 'stud', 'hun' ) + '?</i>"\n\n', false );
			MainView.outputText( 'It takes a moment to put your ' + CoC.player.armorName + ' back on and make ready to leave, but somehow you know this isn\'t the last you\'ve seen of this goblin.', false );
			//[CORRUPT]
			if( CoC.player.cor > 66 ) {
				MainView.outputText( '  Your ' + Descriptors.cockDescript( 0 ) + ' twitches at the thought, ready and wanting more.', false );
			}
		}
		//(TOO BIG)
		else {
	...
	TamaniScene.prototype.tamaniMaleRepeatEncounter = function() {
		...
		if( this.pregnancy.isPregnant || CoC.flags[ kFLAGS.TAMANI_NUMBER_OF_DAUGHTERS ] > 0 ) {
			MainView.outputText( 'While exploring, you\'re startled by the feeling of tiny hands stroking the insides of your thighs.  You look down and find Tamani there, grinning wolfishly,  "<i>Ready for another fuck, big ' + CoC.player.boyGirl() + '?"\n\n', false );
		}//(ELSE)
		else {
			MainView.outputText( 'While exploring, you\'re startled by the feeling of tiny hands stroking the insides of your thighs.  You look down and find Tamani the goblin there, grinning with desire, "<i>Ready to stuff me with cum?  I\'m not taking no for an answer this time.</i>"\n\n', false );
		}
	...
	TamaniScene.prototype.tamaniSexLetHer = function() {
		...
		if( CoC.isInCombat() ) {
			...
			if( CoC.player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_CENTAUR ) {
				if( CoC.player.HP < 1 ) {
					MainView.outputText( 'You stumble, hooves weaving drunkenly as you try to keep your feet.  ', false );
				} else {
					MainView.outputText( 'You stumble, hooves weaving drunkenly as ' + Descriptors.sMultiCockDesc() + ' ache and drool with need.  ', false );
				}
				MainView.outputText( 'Tamani sidles up to you and pats your flank, cooing, "<i>There there horsey, everything\'s going to be okay.  Momma Tamani\'s going to take really good care of you.</i>"  Before you know it your ' + CoC.player.armorName + ' is on the ground and the goblin is underneath you, climbing up to your crotch.\n\n', false );
			}
			//Not taur
			else {
				if( CoC.player.HP < 1 ) {
					MainView.outputText( 'You collapse, unable to fight back any longer.  For her part, Tamani seems disappointed with the outcome, but approaches your prone form in spite of your sorry appearance.', false );
				} else {
					MainView.outputText( 'You tear off your clothes in a frenzy of lust and stagger towards Tamani, openly fondling your groin.   She smirks and pushes you hard, throwing you off balance and onto your back.', false );
				}
				MainView.outputText( '  She wiggles her ' + this.tamaniChest() + ', and despite your efforts to control your body, ', false );
				if( CoC.player.cockTotal() === 1 ) {
					MainView.outputText( 'an erection emerges', false );
				} else {
					MainView.outputText( 'erections sprout up like weeds', false );
				}
				MainView.outputText( '.  She strips you down, giving you a smirk as she says, "<i>Just lie there and I\'ll take what I need, babe.</i>"\n\n', false );
			}
		}
		...
		if( CoC.player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_CENTAUR ) {
			...
			//If >18 Inches
				...
				if( CoC.player.balls === 0 ) {
					...
					MainView.outputText( 'You find yourself wondering vaguely where a goblin learns these kinds of gymnastics, before the goblin in question drives all thoughts out of your head by beginning to mash her soft soles into your ' + Descriptors.ballsDescriptLight() + '.\n\n', false );
				}
				MainView.outputText( 'Tamani has her way with you like this for what seems like hours, crawling up and down your massive cock', false );
				if( CoC.player.balls > 0 ) {
					MainView.outputText( ' and ' + Descriptors.ballsDescriptLight(), false );
				}
				MainView.outputText( ', sucking, tonguing, fondling, and foot rubbing you with horrible accuracy, slapping against you and squealing as she gets off over and over until your underside is drenched in her juices, but drawing away every time you get close to your own release, playing you like the world\'s most sensitive fiddle until your human half is drenched with sweat. You stamp your back hooves in deep agitation and fill the forest with bellowing shouts. There is nothing you can do; attached securely to your nether regions as she is, you couldn\'t reach her or otherwise knock her off even if you wanted to. She is evidently enjoying every minute it, laughing cruelly every time she manages to draw a scream from you, spurring her on to invent some new way of agonizing your poor cock. There is something about this situation which is driving you wild; that you can\'t do anything about what a creature many times smaller than you is doing to you, and that you willingly allowed it to happen, is a shamefully powerful sensation.\n\n', false );
				MainView.outputText( '"<i>Go on then,</i>" she breathes eventually. "<i>I guess you\'ve had enough. Cum for Mistress Tamani, ' + CoC.player.mf( 'stud', 'slut' ) + '. Cover me from head to toe.</i>" As she says this, she walks her hands down your belly, positions herself directly over your head, uses her own lubrication to rub her feet furiously down your ' + Descriptors.cockDescript( 0 ) + ' and then, with one last evil giggle, sticks her tongue directly into your urethra.\n\n', false );
				//Low cum:
				if( CoC.player.cumQ() < 1200 ) {
					MainView.outputText( 'With a ragged sigh, you reach your peak. Your first blast of jizz goes straight into Tamani\'s mouth, but you feel the goblin nimbly and hastily changing her position so that the very tip of your ' + Descriptors.cockDescript( 0 ) + ' can feel her moist snatch. Being teased for so long makes the sensation incredible, and it feels like entire minutes go by whilst you stand there, sweat dripping off you as you cum over and over again until your cock is doing nothing but flexing mindlessly.\n\n', false );
					MainView.outputText( 'When you have finished, you feel an intense feeling of relief as Tamani finally lets go of your underside and drops to the floor. She happily paws off the jizz coating her face and attempts to stuff more into her dripping cunt as she waddles around to face you. "<i>Who knew that riding a horsie could be so much fun! We\'re going to have to do that again, stud. Next time I might make it a gallop instead of a canter. Won\'t that be nice? Until next time, big ' + CoC.player.mf( 'boy', 'girl' ) + '.</i>" She swaggers off, leaving you to wonder if you\'ll ever be in a fit enough state to let her near your cock again, and where the nearest whereabouts of a gallon of water is.\n\n', false );
				}
				//High cum:
				else {
					MainView.outputText( 'With a bellowing roar, you cum with mind blowing force. Your churning balls blast out a river of gooey fluid, the first glorious arc of which flies straight past your human front and paints the tree in front of you. Totally insensate, you can do nothing for entire minutes but stand there and cum, over and over again, sweat dripping off your frame, until the entire area underneath you is covered with your spooge.\n\n', false );
					MainView.outputText( 'When you finally come to your senses, you wearily but with intense satisfaction clop around to take in the lake of jizz you have created. ', false );
					if( CoC.player.balls > 0 ) {
						MainView.outputText( 'Your ' + Descriptors.ballsDescriptLight() + ' ache and you feel very thirsty indeed.', false );
					} else {
						MainView.outputText( 'You feel very thirsty indeed.', false );
					}
					MainView.outputText( ' It\'s only when you manage to drag your mind away from your own privations that you suddenly wonder where Tamani went. She isn\'t still attached to you, is she? It is then that a small lump in the center of the vast puddle in front of you stands up and begins to happily wipe herself down, taking care to stuff as much of the seed plastering her into herself as she can.\n\n', false );
					MainView.outputText( '"<i>Wheeeeeeeee,</i>" she says, with a grin which almost splits her face in half. "<i>Who knew that riding a horsie could be so much fun! We\'re going to have to do that again, stud. Next time I might make it a gallop instead of a canter. Won\'t that be nice? Until next time, big  ' + CoC.player.mf( 'boy', 'girl' ) + '.</i>" She swaggers off, leaving you to wonder if you\'ll ever be in a fit enough state to let her near your cock again, and where the nearest whereabouts of a gallon of water is.\n\n', false );
				}
			}
			MainView.outputText( 'After a little while you redress, but the scent of horny goblin stays with you for hours.', false );
			//Combat end:
			if( CoC.isInCombat() ) {
		...
		//Not-taurs
			...
			MainView.outputText( 'She gets down on all fours, crawling up your legs until her gloss-covered lips reach your ' + Descriptors.multiCockDescriptLight() + '.   Warm wetness slides along your length as the gobbo-slut gets you slick and wet with her saliva.   It has quite the effect on you, turning your ' + Descriptors.cockDescript( 0 ) + ' into a hard trembling pleasure-center.  You sigh, enjoying the attention but more than ready to release your pent-up sexual need into the willing girl.\n\n', false );
			//(fits)
			if( CoC.player.cockArea( 0 ) <= 90 ) {
				MainView.outputText( 'Tamani turns about, dragging her wet gash across you and giving you a nice view of her plump butt-cheeks.  The soft skin of her palms squeezes around you as she lifts you up, positioning your ' + Descriptors.cockDescript( 0 ) + ' against her womanhood.   She grinds against your ' + CoC.player.cockHead() + ', her folds slowly parting to take you.  Slowly, inch after inch of dickflesh sinks into her amazingly elastic yet tight pussy.  ', false );
				if( CoC.player.cocks[ 0 ].cockLength >= 12 ) {
					MainView.outputText( 'Her body visibly stretches around you, and you silently thank whatever gods or demons adapted goblins to be able to fulfill this role.  ', false );
				}
				MainView.outputText( 'Moaning like a whore, she easily slips the rest of the way down, bottoming out her sopping-wet fuck-tunnel.\n\n', false );
				MainView.outputText( 'You gasp in pain and surprise as Tamani pinches and tugs ', false );
				if( CoC.player.totalNipples() === 2 ) {
					MainView.outputText( 'both', false );
				} else {
					MainView.outputText( 'all', false );
				}
				MainView.outputText( ' of your ' + Descriptors.nippleDescript( 0 ) + 's.  She eases up her grip a bit when she sees how bad it hurts, but she does not release them.  The noisy squelching of her cunt fucking you draws your attention back to your groin.  Your body begins thrusting up to meet her, finally understanding it\'s on the receiving end of a passionate screw.  ', false );
				if( CoC.player.biggestTitSize() >= 2 ) {
					MainView.outputText( 'Your ' + Descriptors.allBreastsDescript() + ' bounce and jiggle obscenely as the goblin twists and pulls, abusing your nipples.', false );
				} else {
					MainView.outputText( 'Your ' + Descriptors.nippleDescript( 0 ) + 's burn with pain as the goblin twists and pulls on them, abusing them.  Pleasure and pain mix together, overwhelming you, reducing your world to little more than the slap of genital against genital and the painful torture happening to your chest.\n\n', false );
				}
				MainView.outputText( 'Tamani plays your body like a fiddle, keeping you near the edge as she noisily orgasms over your shaft again and again.  You can tell she\'s starting to tire, so she gives you a kind look and begins to pick up the tempo.   She whispers softly, "<i>Go ahead, cream my honeypot until I can barely walk.</i>"\n\n', false );
				MainView.outputText( 'That\'s all the encouragement you need.  ', false );
				if( CoC.player.balls > 0 ) {
					MainView.outputText( 'Your ' + Descriptors.ballsDescriptLight() + ' twitch powerfully, venting their contents, releasing a jet of hot cum into the goblin.', false );
				} else {
					MainView.outputText( 'Your body\'s internal muscles twitch, pushing a jet of hot cum into the goblin.', false );
				}
				MainView.outputText( '  Tamani wriggles happily, giving your nipples one last tweak before releasing them to play with her own.  Rope after rope of your goopey jism paints the inside of her canal, and the happy slut only begs for more the whole time.', false );
				if( CoC.player.cumQ() >= 250 ) {
					MainView.outputText( '  After a while you can feel the pressure building inside her, starting to give her a bit of a belly.  Tamani rubs it and begs, "<i>More!</i>"', false );
				}
				if( CoC.player.cumQ() >= 500 ) {
					MainView.outputText( '  Your body fulfills her requests and more, stuffing her until she\'s positively pregnant and your spunk is squirting from her entrance with each wave of sticky spooge.', false );
				} else {
					MainView.outputText( '  Sadly, you only manage a few more spurts before your orgasm ends.', false );
				}
				...
				if( CoC.player.cumQ() < 250 ) {
					MainView.outputText( '.  I really enjoy our fucks ' + CoC.player.mf( 'stud', 'hun' ) + ', and I can\'t wait to \'bump\' into you again.', false );
				} else {
					MainView.outputText( '.  You make sure and let your ', false );
					if( CoC.player.balls > 0 ) {
						MainView.outputText( Descriptors.ballsDescriptLight() + ' fill back up with cum, ok?', false );
					} else {
						MainView.outputText( Descriptors.cockDescript( 0 ) + ' fills back up with cum, ok?', false );
					}
				}
				...
				//(Noncombat end:
					...
					MainView.outputText( 'You lie there, recovering from the intense sex.  After a little while you manage to get up and redress, but the scent of horny goblin stays with you for hours.', false );
					CoC.player.orgasm();
					EngineCore.dynStats( 'lus', +10 );
					EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
				}
			...
			//(Doesnt fit)
				...
				MainView.outputText( 'Tamani turns about, dragging her wet gash across you and giving you a nice view of her plump butt-cheeks.  She looks over her shoulder and sucks on one of her lacquered fingernails, looking equal parts mischievous and sexy.   You rock your hips under her until she gives your ', false );
				if( CoC.player.balls > 0 ) {
					MainView.outputText( Descriptors.ballsDescriptLight(), false );
				} else {
					MainView.outputText( Descriptors.cockDescript( 0 ), false );
				}
				MainView.outputText( ' a rough slap.  Whimpering, you still your rebellious hips and allow her to play you like the world\'s most sensitive fiddle.  Tamani pulls your ' + CoC.player.cockHead() + ' up and rocks herself against it, openly moaning as you dribble pre into her sopping gash.\n\n', false );
				MainView.outputText( '"<i>Oooh, you\'re too big for little ol\' me,</i>" moans the goblin, easing back a bit, "<i>but I have something that might make you cum just as hard.   I\'ll just make sure to catch all your cum with my cunny!</i>"    She pulls out a hot-pink dildo and gives it a lick.  You watch in horror as a drop of thick fluid leaks from the tip.  No doubt Tamani intends for you to take take the entire thing in one way or another.\n\n', false );
				MainView.outputText( 'You pout, protesting meekly as she ', false );
				if( CoC.player.balls > 0 ) {
					MainView.outputText( 'lifts up your ' + Descriptors.ballsDescriptLight(), false );
				} else {
					MainView.outputText( 'leans forwards, looking between your legs', false );
				}
				MainView.outputText( ', and places the dildo\'s leaking tip against your ' + Appearance.assholeOrPussy() + '.  You feel it stretching you out, reacting with your body\'s own fluids to swell up inside you like a pervert\'s balloon as Tamani pushes it further and further inside you.', false );
				if( CoC.player.hasVagina() ) {
					MainView.outputText( '  ', false );
					CoC.player.cuntChange( CoC.player.vaginalCapacity() * 0.95, true );
					MainView.outputText( 'Your walls squeeze tightly around it in an unconscious milking motion as your body reacts to it as if it were a real dick.', false );
				} else {
					CoC.player.buttChange( CoC.player.analCapacity() * 0.95, true );
					MainView.outputText( 'Your body clamps down around it, resisting the strange intrusion as it swells up to fill you, pressing tightly against your prostate.', false );
				}
				MainView.outputText( '  A burning heat radiates through your body, making your ' + Descriptors.nippleDescript( 0 ) + ' tingle and grow ever more sensitive.\n\n', false );
				MainView.outputText( 'Tamani rocks back and forth, sliding herself against your ' + Descriptors.cockDescript( 0 ) + ', humping it lewdly as your veins rub her hard little nub of a clit.  She pants, making sure to keep giving you sultry looks over her shoulder, licking her glossy lips as she brutally teases you.  You can feel her getting off over and over as she grinds, sliding faster and faster on your shaft.   For your part, your ' + Descriptors.cockDescript( 0 ) + ' begins making a puddle of pre-cum on your belly as the dildo\'s drug-like cum affects you, making you feel horny and hot.\n\n', false );
				MainView.outputText( 'The kinky goblin dominatrix on your ' + Descriptors.cockDescript( 0 ) + ' leans even further forwards, angling the dildo in your ' + Appearance.assholeOrPussy() + ' upwards to squeeze your ', false );
				if( CoC.player.hasVagina() ) {
					MainView.outputText( 'cock\'s base ', false );
				} else {
					MainView.outputText( 'prostate ', false );
				}
				MainView.outputText( ' with delicious intensity.  You quiver underneath her, panting and moaning, totally subservient to the tiny girl giving your body the sexual workout of a life-time.  She slides back, her cunt squishing back and forth against your prick-head, driving you wild.  She coos, "<i>Go ahead and cum stud,</i>" while squeezing your ', false );
				if( CoC.player.balls > 0 ) {
					MainView.outputText( 'balls', false );
				} else {
					MainView.outputText( 'crotch', false );
				}
				MainView.outputText( ', "<i>let out all that hot sticky spunk for Mistress Tamani.</i>"\n\n', false );
				MainView.outputText( 'Your body immediately and happily complies, squirting a jet of thick white jism onto the goblin\'s jiggling ass-cheeks.  She slides down, aligning her cunt with your swollen cock-tip, taking the next squirt directly between her lips as she rubs against you.  Tamani slaps the dildo in your ' + Appearance.assholeOrPussy() + ' gently, squeezing out an incredibly thick gout of semen as she stretches you wider.  ', false );
				if( CoC.player.cumQ() >= 250 ) {
					MainView.outputText( 'You cum harder and harder, pushing her back a bit and splattering her belly with white cream.  She moans, rubbing it into her skin and twat', false );
					if( CoC.player.cumQ() >= 500 ) {
						MainView.outputText( ' as you keep cumming, splattering her tits and face, soaking the little goblin slut', false );
					}
					MainView.outputText( '.  ', false );
				}
				MainView.outputText( 'The dildo slips out of your abused ' + Appearance.assholeOrPussy() + ' forgotten in the heat of your lovemaking, leaking pink goo.\n\n', false );
				MainView.outputText( 'Tamani eases up off of you, dripping a mixture of sexual fluids and stretching as if she had just completed a long workout.  The slutty goblin winks at you and waves, "<i>Thanks for the cum.  Be sure and take some time to refill.  I wanna be soaked again next time!</i>"\n\n', false );
				//Combat end:
				if( CoC.isInCombat() ) {
					MainView.outputText( 'You black out, exhausted from the ordeal.', false );
					Combat.cleanupAfterCombat();
	...
	TamaniScene.prototype.tamaniSexLost = function() {
		...
		if( CoC.player.HP < 1 ) {
			MainView.outputText( 'lie there, unable to fight back.  ', false );
		} else {
			MainView.outputText( 'touch yourself, giving in to the tiny goblin\'s desire to breed.  ', false );
		}
		MainView.outputText( 'She advances, her bare feet padding across the soil as she rummages in a faded leather pouch for something.  Her eyes light up as she finds whatever it is she was looking for.  In her hand is a ribbed silver ring, covered on every surface with numerous carved runes.  You whimper and back up, knowing she intends to fit it over your ' + Descriptors.cockDescript( 0 ) + ', but Tamani only \'tsks\' and shakes her head.\n\n', false );
		MainView.outputText( '"<i>You were such a naughty boy, teasing me with that juicy dick and then pretending like you didn\'t want this.  I\'ll have to punish you for being bad, that\'s all there is to it,</i>" she explains, pushing your hands away and fitting the ring around your flare.  ', false );
		if( CoC.player.cocks[ 0 ].cockThickness >= 5 ) {
			MainView.outputText( 'Impossibly, despite your titanic girth, the ring somehow expands to squeeze onto you, though it\'s painfully tight.\n\n', false );
		} else {
			MainView.outputText( 'Somehow the ring fits onto you perfectly, as if it was made exactly for your ' + Descriptors.cockDescript( 0 ) + ', though it remains painfully tight.\n\n', false );
		}
		...
		MainView.outputText( 'She forces the ring down your entire length, not stopping until it bottoms out against the flesh of your groin.  In response, the obviously magical ring cinches even tighter and begins a slow vibration, providing pleasure but denying you the ability to orgasm.  Tamani wraps a single hand around your tight tender shaft, squeezing and pulling, using discomfort to guide you up onto your hands and knees.  She eases up on the pressure, allowing you a bit of pleasure once she has you where she wants you.  You look down between your arms ', false );
		if( CoC.player.biggestTitSize() >= 6 ) {
			MainView.outputText( 'but can\'t manage to see your bloated prick past the wall of titflesh.\n\n', false );
		} else {
			MainView.outputText( 'at your bloated prick, looking almost overinflated from all the blood that tight ring has forced into it.\n\n', false );
		}
		MainView.outputText( '<b>*SMACK*</b>  Tamani\'s hand cracks against your ' + Descriptors.buttDescript() + ' making you lurch and bite back a cry of pain.   "<i>Bad boys get punished!</i>" she yells as she brings her hand down again, slapping your other cheek.  You\'re sure there must be two tiny red hand-prints forming already. The kinetic impacts of her blows vibrate through your hips, traveling into your sensitive ' + Descriptors.cockDescript( 0 ) + ' and working with the magic-ring to spread equal measures of pain and pleasure through its length.\n\n', false );
		MainView.outputText( 'The spanking doesn\'t let up until your ass is cherry-red and tender.  Tamani gently pats it, making you wince in displeasure, and giggles, pleased with herself, "<i>Hehe, do you think I\'ve punished you enough?  I\'m sure you do, but what I really want to know is – have you learned your lesson?  I need to make sure you\'ll be a good boy and jump right into my honeypot next time I ask.  I hate wasting time fighting when we could be making love and breeding.</i>"\n\n', false );
		MainView.outputText( 'Tamani pours oil over your abused backside, letting it soak in and start to numb the pain.  She pours quite a different type of oil into her hands and starts massaging you, letting the alchemical mixture make your skin tingle and enhance the sensation of pleasure while numbing away the pain of your cock being kept so... tightly bound.  Your arms and ' + CoC.player.legs() + ' tremble from supporting yourself at such an awkward angle, struggling not to relax at the sudden influx of pleasure.  The feelings bring you to the edge, and you feel your body\'s internal muscles clenching, but the tight vibrating ring blocks your cum and your orgasm, leaving you backed up and desperate.\n\n', false );
		MainView.outputText( 'Your goblin mistress ', false );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( 'cups your ' + Descriptors.ballsDescriptLight() + ' tightly', false );
		} else {
			MainView.outputText( 'caresses you around the ring', false );
		}
		MainView.outputText( ' and asks, "<i>So are you sorry for making me wait for my cream ' + CoC.player.mf( 'stud', 'slut' ) + '?  If you are, just say <b>I\'m sorry mistress Tamani, please milk my cock whenever you want</b> and then beg me to cum.</i>"\n\n', false );
		MainView.outputText( 'You NEED to cum.  Her hands... they keep stroking you, and each time your body begins to orgasm it\'s held back, trapped within you painfully.  ', false );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( 'Your balls are getting VERY swollen and tender.', false );
		} else {
			MainView.outputText( 'Your body aches inside, feeling bloated and ready to pop.', false );
		}
		if( CoC.player.cor < 50 ) {
			MainView.outputText( '  You have no choice, you\'ll say what she wants you to say.\n\n', false );
		} else {
			MainView.outputText( '  Being denied like this was kind of fun, but you REALLY need to get off now, so you\'ll say what she wants you to say.\n\n', false );
		}
		MainView.outputText( '"<i>Mistress Tamani, I\'m so sorry!  Milk my cock dry any time you want!  Puhleeeeeaaaaase let me cum!  Please, I\'m so fucking horny, I\'ll do what you want, just give me release!</i>" you beg, ', false );
		if( CoC.player.cor > 50 ) {
			MainView.outputText( 'blushing hotly and doing your best to look like a submissive toy.\n\n', false );
		} else {
			MainView.outputText( 'blushing in shame and embarrassment, but meaning every word.\n\n', false );
		}
		MainView.outputText( 'She takes the empty bottle and presses it against your ' + CoC.player.cockHead() + ', caressing your length with loving affection with her free hand as she replies, "<i>Not exactly what I told you to say, but it DID sound heartfelt.  Let\'s let out all that backed up cum now, hrmmm?</i>"   Her free hand touches the ring and you feel the pressure relax a bit - still tight enough to keep you beyond-hard, but loose enough for you to cum.  She strokes and squeezes as your blocked orgasms slowly flow into the bottle, smiling as it thickens and begins spurting as pleasure overtakes you.  You hold yourself on all fours, red-assed and submissive as your goblin mistress milks you of the last of your cum.', false );
		if( CoC.player.cumQ() >= 250 ) {
			MainView.outputText( '  The bottle quickly overflows, but Tamani happily sets it aside and slides her own cunt under you, letting you pump the rest of your seed into her fertile womb.', false );
		}
		if( CoC.player.cumQ() >= 500 ) {
			MainView.outputText( '  She coos happily when you still manage to pump enough jism into her to bloat her belly, but even with your impressive fertility, the orgasm has to end.', false );
		}
		MainView.outputText( '\n\n', false );
	...
	TamaniScene.prototype.tamaniSexWon = function() {
		...
		if( CoC.player.cockArea( x ) <= 90 ) {
			...
			MainView.outputText( 'It\'s too tempting of a target to resist.  You open your ' + CoC.player.armorName + ' and allow your ' + Descriptors.multiCockDescriptLight() + ' to flop free.  You\'re already hard from the enticing display, and in a moment you\'re pressing against her lust-slicked pussy', false );
			if( CoC.player.totalCocks() > 1 ) {
				MainView.outputText( ' and tight asshole', false );
			}
			MainView.outputText( '.  You don\'t allow her any say in the matter, pushing forward as you feel the small girl\'s flesh yield around ', false );
			if( CoC.player.totalCocks() > 1 ) {
				MainView.outputText( 'each of ', false );
			}
			MainView.outputText( 'your girth', false );
			if( CoC.player.totalCocks() > 1 ) {
				MainView.outputText( 's', false );
			}
			MainView.outputText( '.  She squeals happily, clearly getting what she desires.  For a moment you feel disappointed in yourself, but the sensations of her tight hole', false );
			if( CoC.player.totalCocks() > 1 ) {
				MainView.outputText( 's', false );
			}
			MainView.outputText( ' clenching and squeezing around you quickly washes it away.\n\n', false );
			MainView.outputText( 'You rock back and forth methodically, treating Tamani like a tight cock-sleeve.  The goblin slut\'s hands rub her belly, not even attempting to pull her face out of the mud as she moans and giggles like a whore.  You keep working her cunt like a ', false );
			if( CoC.player.gender === 1 ) {
				MainView.outputText( 'man', false );
			} else {
				MainView.outputText( 'herm', false );
			}
			MainView.outputText( ' possessed, sawing in and out with brutal efficiency, the wet squelches of the slut\'s juices driving you to piston back and forth with even greater force.  She gurgles happily, her ', false );
			if( CoC.player.totalCocks() === 1 ) {
				MainView.outputText( 'pussy squeezing tightly as she cums hard.\n\n', false );
			} else {
				MainView.outputText( 'holes squeezing tightly as she cums hard.\n\n', false );
			}
			if( CoC.player.totalCocks() > 1 ) {
				MainView.outputText( 'Each of your ' + Descriptors.multiCockDescriptLight() + ' pulsates, spasming inside your goblin-flesh prison, spraying a bit of sticky goo into the happy slut.   You can hear her babbling, pleasure-drunk as she quivers around you, "<i>Fuck yes! Cu-ah-ahm in me!  Fuck fuckfuckfucKFUCKYEAH!  Oooh, that\'s it, put me in the dirt and show me who\'s boss!</i>"  The words seem to have the desired effect, helping you empty every ounce of cock-cream into the slut\'s tight holes.', false );
				if( CoC.player.cumQ() >= 250 ) {
					MainView.outputText( '  She pants, turning her head in the mud to watch as her belly visibly inflates, stuffed totally full of cum. ', false );
				}
				if( CoC.player.cumQ() >= 500 ) {
					MainView.outputText( '  In no time at all a river of jism pours from her, pooling below as you overwhelm her body\'s capacity to store spunk.', false );
				}
			} else {
				MainView.outputText( 'Your ' + Descriptors.multiCockDescriptLight() + ' pulsates, spasming inside your goblin-flesh prison, spraying a bit of sticky goo into the happy slut.   You can hear her babbling, pleasure-drunk as she quivers around you, "<i>Fuck yes! Cu-ah-ahm in me!  Fuck fuckfuckfucKFUCKYEAH!  Oooh, that\'s it, put me in the dirt and show me who\'s boss!</i>"  The words seem to have the desired effect, helping you empty every ounce of cock-cream into the slut\'s tight hole.', false );
				if( CoC.player.cumQ() >= 250 ) {
					MainView.outputText( '  She pants, turning her head in the mud to watch as her belly visibly inflates, stuffed totally full of cum. ', false );
				}
				if( CoC.player.cumQ() >= 500 ) {
			...
			if( CoC.player.balls > 0 ) {
				MainView.outputText( 'sexy balls have ', false );
			} else {
				MainView.outputText( 'sweet cock has ', false );
			}
			MainView.outputText( 'any more cream to give to me?</i>"\n\n', false );
		...
		//Too big? Jerk off with feet and bukkake
			...
			MainView.outputText( 'You throw Tamani on her back, too drunk on desire to care how it feels for the tiny slut.  There\'s no way she could ever take ', false );
			if( CoC.player.totalCocks() > 1 ) {
				MainView.outputText( 'any of your massive members', false );
			} else {
				MainView.outputText( 'your massive member', false );
			}
			MainView.outputText( ', so you grab her by the ankles and wrap her soft-soled feet about yourself.   You start jerking yourself off, using Tamani as a cute but expendable masturbation aid.  She chews a fingernail and massages her ' + this.tamaniChest() + ' as she watches you, doing her best to put on a show.  The little slut seems to like it.\n\n', false );
			MainView.outputText( 'Her feet start to grow slick with your sweat and pre-cum, sliding effortlessly along the length of your shaft as you continue to bring yourself towards orgasm.  You watch while Tamani dips her fingers into the slick folds of her hungry cunt, getting off on being used in such a perverse manner.  She coos, "<i>How do my feet feel ' + CoC.player.mf( 'stud', 'slut' ) + '?  Are they soft and slick when they slide on your cock?  Are you going to cum for Tamani and paint her white?  You are.  You know you are.  Cum for Tamani.</i>"\n\n', false );
			MainView.outputText( 'Oh gods, you are... it\'d be so easy to just release all over her tight little body, soaking her in cum from head to toe.  You know she\'d like it.  Hell, you\'d like it, but you want to make her wait.   Your hands keep sliding and squeezing, jerking her now-slippery soles with faster and faster strokes.  Holding back is TOO hard!  You need to release – the little slut\'s words ring true as you squeeze tightly, feeling warmth building in your crotch as your body begins to climax.\n\n', false );
			MainView.outputText( 'Tamani licks her lips and pushes with her legs, assisting you as you milk yourself with her feet, squeezing out the first jet of hot goblin-treat.   It spatters over the green girl\'s forehead, running into her pink highlights.  She opens wide, craning up to catch the next blast of salty seed in her dirty lipstick-coated mouth.   Swallowing like a pro, she leans up further, letting you coat her tits with cream.', false );
			if( CoC.player.cumQ() >= 250 ) {
				MainView.outputText( '  You continue working your ' + Descriptors.cockDescript( x ) + ' with her supple feet, watching your dripping spooge froth and bubble as you continue to pump more onto the sassy wench.', false );
			}
			if( CoC.player.cumQ() >= 500 ) {
				MainView.outputText( '  She sputters, blowing some of the caked up semen off her face so she can breathe.  The slut gathers up the goopy mess as you continue to paint her, alternatively devouring it with her mouth and shoveling drippy handfuls into her eager cunt.', false );
			}
			MainView.outputText( '\n\n', false );
			MainView.outputText( 'Tamani winks, blinking the cum out of an eye as you finish up, releasing her seed-covered form.  She coos, "<i>Mmmm, Tamani loves it when you dominate her, ' + CoC.player.mf( 'stud', 'sexy' ) + '.  Do you think your ', false );
			if( CoC.player.balls > 0 ) {
				MainView.outputText( 'sexy balls have ', false );
			} else {
				MainView.outputText( 'sweet cock has', false );
			}
			MainView.outputText( 'has any more cream to give to me?</i>"\n\n', false );
			MainView.outputText( 'She doesn\'t give you a chance to answer as she staggers up and strikes a sexy pose, "<i>Of course you do.  I\'ll be back for the rest later!</i>"\n\n', false );
			MainView.outputText( 'Tamani wiggles in the sexiest way as she leaves, arousing your body all over again...', false );
			CoC.player.orgasm();
	...
	TamaniScene.prototype.tamaniPoopsOutBabies = function() {
		...
		MainView.outputText( '\n\n', false );
		if( CoC.flags[ kFLAGS.TAMANI_PREGNANCY_COUNT ] === 2 ) {
			MainView.outputText( 'The twins drink deeply, growing taller before your eyes as Tamani comes out of her pregnancy induced orgasms.', false );
		} else {
			MainView.outputText( 'The goblin children drink deeply, squabbling for a chance at suckling the nipples as their mother recovers from her birthing-induced orgasms.', false );
		}
		MainView.outputText( '  She looks up at you and whispers breathily, "<i>Come fuck me a little later, I\'ll give you even more beautiful little girls.  These ' + Utils.num2Text( CoC.flags[ kFLAGS.TAMANI_PREGNANCY_COUNT ] ) + ' will be fully grown in a week or two and out man-hunting, but none of them will please you like me, I promise.</i>"\n\n', false );
		MainView.outputText( 'Tamani sighs and relaxes, enjoying the breastfeeding and waving you away.  You shrug and head back to camp, more than a little aroused.', false );
	...
	TamaniScene.prototype.getRapedByTamaniYouHypnoSlut = function() {
		...
		MainView.outputText( 'Tamani crooks her finger and you come running, already feeling yourself straining against your ' + CoC.player.armorName + ' for a chance to fuck your wife and mistress.  She giggles, "<i>Ohhh, how sweet, you\'re so ready to do your duty.</i>"\n\n', false );
		MainView.outputText( 'The goblin traces her fingers over the bulge in your ' + CoC.player.armorName + ' before reaching inside of it to give you a tender squeeze.  You sigh, happy to have earned such tender treatment from your lovely wife.  Automatically, your hands undo your ' + CoC.player.armorName + ', removing the troublesome garment so as not to interfere with your lovemaking.   Tamani gives you another squeeze and teases, "<i>Oh baby, you\'re hooked on Tamani\'s pussy, aren\'t you?  I can tell, you\'re practically trembling.  Let Tamani help take care of that for you.</i>"\n\n', false );
		MainView.outputText( 'She smirks and bends over, looking back up at you from between her legs.  She offers, "<i>Go ahead and stick it in.  A horny ' + CoC.player.mf( 'boy', 'herm' ) + ' like you needs to have that delicious cock inside a nice, tight wife like myself.</i>"\n\n', false );
		//[FITS]
		if( primary >= 0 ) {
			MainView.outputText( 'Tamani is completely right.  You grab her ass, feeling your fingers sink in to her supple flesh as you pull her back towards your ', false );
			if( cocks > 0 ) {
				MainView.outputText( 'main ', false );
			}
			MainView.outputText( Descriptors.cockDescript( 0 ) + '.  Her warmth spreads over your ' + CoC.player.cockHead( primary ) + ' slowly, making you twitch and drip inside her, lubricating her already dripping channel with even more slick fluids.  With a mighty heave you push inside her, feeling yourself penetrating deep inside her, directly into her fertile womb.  Tamani rubs your ' + CoC.player.legs() + ' with her hands, giving you a massage as you bottom out against her, feeling her juices stain your ', false );
			if( CoC.player.hasSheath() ) {
			...
			MainView.outputText( '\n\n', false );
			MainView.outputText( 'You wait a second, adjusting to the feeling of being completely contained by such a warm, wet hole before you slowly push her away.  The slick walls of her tunnel clench around you as you pull her back, massaging the parts of you still inside her.  ', false );
			if( cocks > 1 ) {
				if( primary > 0 ) {
					secondary = 0;
				} else {
				...
				MainView.outputText( 'She moans and pulls your ' + Descriptors.cockDescript( secondary ) + ' into her mouth, running her tongue around the tip and sliding her palm along the underside.', false );
			} else {
				MainView.outputText( 'She moans and pants, babbling about how good you are to her the whole time.', false );
			}
			MainView.outputText( '  You don\'t stop until your ' + CoC.player.cockHead( primary ) + ' is the only thing inside her.\n\n', false );
			MainView.outputText( '  The second thrust is even better than the first.  It\'s slippery, hot, and passionate, and the idea of slowing down or stopping for any reason is inconceivable.  Tamani pants with each thrust, pawing at her bouncing breasts, grabbing one by the nipple with a free hand and moaning loudly', false );
			if( cocks > 0 ) {
				MainView.outputText( ' around your ' + Descriptors.cockDescript( secondary ), false );
			}
			MainView.outputText( '. Her tiny form jiggles, ', false );
			if( CoC.player.tallness > 55 ) {
				MainView.outputText( 'lifting off the ground as you get into it, giving yourself over to her sex.', false );
			} else {
				MainView.outputText( 'nearly falling over as you give your body over to her sex.', false );
			}
			MainView.outputText( '  She visibly orgasms, shaking and trembling from head to toe as her pussy milks your ' + Descriptors.cockDescript( primary ) + '.  It only lasts a few seconds, though they feel like heaven, before her body goes nerveless.\n\n', false );
			MainView.outputText( 'Taking that as an opportunity, you pick up the pace, plunging yourself in and out of her with a renewed, almost vicious vigor.  The wet slaps of your juice-', false );
			if( CoC.player.skinType === AppearanceDefs.SKIN_TYPE_FUR ) {
				MainView.outputText( 'matted', false );
			} else {
				MainView.outputText( 'slicked', false );
			}
			MainView.outputText( ' ' + CoC.player.skinDesc + ' impacting her ass fill the air.  You fuck your wife harder and faster with every stroke, knowing you won\'t last more than a few more thrusts before you\'re painting her womb white.  Your wife is too cummed out to care or respond, ', false );
			if( cocks === 1 ) {
				MainView.outputText( 'her tongue hanging out as she pants nonsensical pleasure-noises.', false );
			} else {
				MainView.outputText( 'and her mouth is too full of ' + Descriptors.cockDescript( secondary ) + ' to do anything but gurgle out nonsensical pleasure-noises.', false );
			}
			MainView.outputText( '  Swelling wider inside the wet hole, your ' + Descriptors.cockDescript( primary ) + ' tingles and clenches, about to unload.\n\n', false );
			MainView.outputText( 'An intense orgasm rolls through you, making your body clench as your piston your ' + Descriptors.cockDescript( primary ) + ' forwards, burying it deeply inside the goblin-shaped cum-receptacle.  ', false );
			if( CoC.player.hasKnot( primary ) ) {
				MainView.outputText( 'Your knot balloons inside her, locking her in place', false );
			} else if( CoC.player.cocks[ primary ].cockType === CockTypesEnum.HORSE ) {
				MainView.outputText( 'Your flare widens, actually holding itself inside Tamani\'s womb', false );
			} else if( CoC.player.cocks[ primary ].cockType === CockTypesEnum.DEMON ) {
				MainView.outputText( 'The corrupted nodules along your ' + Descriptors.cockDescript( primary ) + ' pulsate in rippling, wave-like motions, massaging the interior of Tamani\'s twat', false );
			} else if( CoC.player.cocks[ primary ].cockType === CockTypesEnum.TENTACLE ) {
				MainView.outputText( 'Your tentacle-cock\'s tip widens, becoming mushroom-like inside Tamani\'s womb', false );
			} else {
				MainView.outputText( 'Your sensitive head swells slightly in time with the contractions of your orgasm', false );
			}
			MainView.outputText( ' as you begin to fill your wife\'s womb with seed.   The orgasm is intense', false );
			if( CoC.player.cumQ() < 50 ) {
				MainView.outputText( ', but brief, and after a few moments of spasming pleasure, the flood of cum slows to a trickle.', false );
			} else if( CoC.player.cumQ() < 300 ) {
				MainView.outputText( ' and voluminous, forcing more and more seed into Tamani\'s uterus until she sloshes with each new squirt.  After a few long moments of spasming pleasure, the flood of cum slows to a trickle.', false );
			} else {
				MainView.outputText( ' and massive, squirting thick gouts of semen directly into Tamani\'s waiting womb until each new explosion of spunk makes her slosh.  She moans, her belly bloating slightly until she\'s unable to take any more.', false );
				if( CoC.player.cumQ() < 1000 ) {
					MainView.outputText( '  Though your orgasm tapers off, the additional seed squirts from between her thighs, trickling down to a slow drip of viscous white goo.', false );
				} else {
					MainView.outputText( '  Trapped inside her and cumming as hard as ever, you raise the pressure inside the goblin until your seed is washing out of her cunt around you, puddling on the ground.', false );
				}
				if( CoC.player.cumQ() >= 1000 && CoC.player.cumQ() < 4000 ) {
					MainView.outputText( '  It does eventually end, but not until a sizable puddle has formed between the pair of you.', false );
				} else {
					MainView.outputText( '  It lasts seemingly forever, and by the time you\'re done, the two of you are standing in a puddle inches deep.', false );
				}
			}
			if( cocks > 1 ) {
				MainView.outputText( '  During it all Tamani\'s throat works to swallow every drop', false );
				if( CoC.player.cumQ() > 300 ) {
					MainView.outputText( ', but before long she\'s failing, with cum leaking out from the corners of her mouth.', false );
					if( CoC.player.cumQ() > 600 ) {
						MainView.outputText( '  A few seconds later she pulls back coughing, and the perky goblin takes the rest of your load in her face.', false );
					}
				} else {
					MainView.outputText( '.', false );
				}
			...
			MainView.outputText( '\n\n', false );
			MainView.outputText( 'Joined at the hips, the two of you manage to stay upright, each leaning into the other', false );
			if( CoC.player.tallness > 55 ) {
				MainView.outputText( ' in spite of the height differential', false );
			}
			MainView.outputText( '.  Tamani regains her senses and pushes you back, waddling slightly after the intense fuck.   White jism and clear feminine fluids drool down her thighs, and your mistress couldn\'t be happier.  She pets your still-dripping ' + Descriptors.cockDescript( primary ) + ' affectionately as she says, "<i>You\'re such a wonderful husband.  I can\'t wait to see how you get along with your new daughters!</i>"\n\n', false );
			MainView.outputText( 'A surge of pride spreads through you at her praise, and you lean down to give your wife a long french kiss before heading back to camp.  On the way back your head slowly clears, and you wonder what came over you back there?!', false );
			this.tamaniKnockUp();
			CoC.player.orgasm();
		...
		//[DOESNT FIT]
			...
			MainView.outputText( Descriptors.cockDescript( 0 ) + '.  Her warmth spreads over the ' + CoC.player.cockHead() + ' of your ' + Descriptors.cockDescript( 0 ) + ', but will go no further.  You tug harder on her bouncing backside, but you\'re simply too large to fit inside your wife\'s beautiful pussy.  She turns around and places a small hand ', false );
			if( CoC.player.biggestTitSize() < 1 ) {
				MainView.outputText( 'on your chest ', false );
			} else {
				MainView.outputText( 'between your ' + Descriptors.breastDescript( 0 ) + ' ', false );
			}
			MainView.outputText( ' and commands, "<i>Stop, you\'re just too big of a man for Tamani to do it the old fashioned way.</i>"\n\n', false );
			MainView.outputText( 'Disgruntled but obedient, you stop your vaginal assault and release your wife.  It seems she has another way for your aching ' + Descriptors.cockDescript( 0 ) + ' to perform its duties anyway.  Tamani turns around and pulls out some clear tubing, along with a floppy, flexible sheath while she explains, "<i>I\'ll just have to get you off and let you pump all your babies into me with this!</i>"\n\n', false );
			MainView.outputText( 'Her small, dextrous fingers wrap the upper half of your ', false );
			if( cocks > 1 ) {
				MainView.outputText( 'primary ', false );
			}
			MainView.outputText( Descriptors.cockDescript( 0 ) + ' inside the sheath, constricting it inside the tight, transparent material.  You sigh while you endure the treatment, disappointed you won\'t get to feel her clenching about you as you pour sperm into her fertile womb. She finishes up, slowly pushing the tube up her gorgeous, feminine hole until you\'re certain it\'ll be depositing your precious cargo into her deepest recesses. Tamani claps her hands together excitedly, "<i>I haven\'t had a man big enough to use this with until you.  Oh husband, you\'re going to love this, just lie back for Tamani, okay?</i>"\n\n', false );
			MainView.outputText( 'You do as you are told, like a good husband, and Tamani stays close, bound to you by a flexible, artificial umbilical.  She sits next to you, propping her feet up onto your ' + Descriptors.cockDescript( 0 ) + '.   The goblin doesn\'t have any shoes on her green-tinted feet, and in spite of the rough living conditions, they\'re softer than her hands, and nearly as dextrous.  You look over at her, questioningly at first, though it shifts to confused happiness as she begins stroking her left foot up and down your length.  Your wife shifts her right leg underneath you, and curls her toes around to interlock them with her left.\n\n', false );
			MainView.outputText( 'With both her feet locked together, wrapped around most of your sensitive ' + Appearance.cockNoun( CockTypesEnum.HUMAN ) + ' and sliding along it, your petite wife begins giving you the foot-job of a lifetime.  The awkward position and intense desire make the both of you break out in a sweat, and before long her slippery feet are lubricated enough for her to pick up the tempo.  You rock against her soles, gazing lovingly into her eyes as she fondles her clit and gives you a perverted wink.  Tamani exclaims, "<i>Oooh, I can feel you twitching under my toes!  Does Tamani\'s little feet feel good on her ', false );
			if( CoC.player.tallness > 60 ) {
				MainView.outputText( 'big ', false );
			}
			MainView.outputText( CoC.player.mf( 'strong man\'s', 'sexy lover\'s' ) + ' ' + Appearance.cockNoun( CockTypesEnum.HUMAN ) + '?</i>"\n\n', false );
			MainView.outputText( 'You assure her that they do like what she is doing – very much so – and ', false );
			if( CoC.player.cor < 33 ) {
				MainView.outputText( 'beg', false );
			} else if( CoC.player.cor < 66 ) {
				MainView.outputText( 'ask', false );
			} else {
				MainView.outputText( 'command', false );
			}
			MainView.outputText( ' her not to stop.  Tamani starts to giggle, but bursts into a moan as her fingers slide along her easily visible clit. She pants, "<i>Hurry up and come for Tamani baby, I wanna feel you pumping your cum inside me!</i>"\n\n', false );
			MainView.outputText( 'Obedient until the end, you obey your tiny mistress and reach your climax.  You gasp out loud, twitching between her feet as your cum slowly boils out from your body.  Tamani watches in fascination as your urethra bulges out, actually shifting the position of her feet to make room for your jism.  White goop rockets out of your urethra, pumping through the tube and into your wife\'s waiting gash.  She squeals and dribbles clear fluid underneath the tube, actually getting off from the feeling of being impregnated.', false );
			if( CoC.player.cumQ() > 250 ) {
				MainView.outputText( '  In no time, you\'ve filled her to capacity, and white mixes in with the clear sexual fluids that are dripping from the goblin\'s cunt.', false );
			}
			if( CoC.player.cumQ() > 700 ) {
				MainView.outputText( '  A few more blasts visibly bloat her belly, and Tamani pulls her hands away from her pussy to stroke the slowly rounding dome on her abdomen.', false );
			}
			if( CoC.player.cumQ() > 1500 ) {
				MainView.outputText( '  You just keep cumming, forcing the spooge to squirt out around the tube, puddling on the ground', false );
				if( CoC.player.cumQ() > 3000 ) {
					MainView.outputText( ', and by the time you finish, both of you are in a thick puddle of spunk', false );
				}
				MainView.outputText( '.', false );
			}
			if( cocks > 1 ) {
				if( cocks === 2 ) {
					MainView.outputText( '  Your other' + Descriptors.cockDescript( 1 ), false );
				} else {
					MainView.outputText( '  Each of your other ' + Descriptors.multiCockDescriptLight(), false );
				}
				MainView.outputText( ' splatters its own orgasmic juice everywhere, making a thorough mess of things, but neither of you seem to mind.', false );
			}
			MainView.outputText( '\n\n', false );
			MainView.outputText( 'Tamani smiles as she pulls the tube out and stands up.  White jism and clear feminine fluids drool down her thighs, and your mistress couldn\'t be happier.  She peels her device off your still-dripping ' + Descriptors.cockDescript( 0 ) + ', pats it affectionately, and says, "<i>You\'re such a wonderful husband.  I can\'t wait to see how you get along with your new daughters!</i>"\n\n', false );
			MainView.outputText( 'A surge of pride spreads through you at her praise, and you get up to give your wife a long french kiss before heading back to camp.  On the way back your head slowly clears, and you wonder what came over you back there?!', false );
			this.tamaniKnockUp();
			CoC.player.orgasm();
			EngineCore.dynStats( 'int', -0.5, 'sen', -1 );
		}
	...
	TamaniScene.prototype.tamaniBeaten = function() {
		MainView.clearOutput();
		MainView.outputText( 'With the goblin defeated, you\'re finally free to tend to your body\'s true desires, even if they may be different than the ones the goblin was intent on stoking.  You shed your [armor] like a second skin, shrugging out of the offending garments with ease.  With your nude form revealed, Tamani ' );
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'licks her lips in anticipation, eyes locked on [eachCock]' );
		} else {
			MainView.outputText( 'frowns in displeasure, noting your lack of a penis' );
		}
		MainView.outputText( '.  She\'s clueless to the growing size that dangles beneath your spider-half, the secret drider treasure that you usually keep retracted.  A dollop of green goo drips from the tip of the semi-hard organ, still hidden from the goblin matron.' );
		...
		if( CoC.flags[ kFLAGS.TIMES_OVIPOSITED_TAMANI ] > 0 ) {
			MainView.outputText( ', or in your case even more eggs' );
		}
		MainView.outputText( '.</i>"  Petite hands massage the goblin\'s perky, lust-thickened nipples as she talks and wiggles her bouncy bum in your direction.' );
		...
		if( CoC.player.hasCock() ) {
			MainView.outputText( '  White coats the goblin from her shoulders up, the only evidence of your male satisfaction.  You were so focused on the act of impregnation and the unique pleasure that it provides that you barely noticed [eachCock] erupting in sympathetic pleasure.' );
		}
	...
	TamaniScene.prototype.declineZeFacesits = function() {
		MainView.clearOutput();
		MainView.outputText( 'You tell her you\'re not interested.' );
	...
	TamaniScene.prototype.acceptTamaniFacesits = function() {
		MainView.clearOutput();
		MainView.outputText( 'You eye the goblin\'s wide hips again before you nod, anticipating the idea.' );
