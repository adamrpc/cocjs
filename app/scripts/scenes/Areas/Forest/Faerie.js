		...
		if( CoC.player.cockTotal() > 0 && (!CoC.player.hasVagina() || Utils.rand( 2 ) === 0) ) {
			...
			if( Utils.rand( CoC.player.spe / 2 ) + CoC.player.statusAffectv1( StatusAffects.FaerieFucked ) > 15 ) {
				if( CoC.player.statusAffectv1( StatusAffects.FaerieFucked ) < 5 ) {
					...
					EngineCore.outputText( 'It would be cute if she wasn\'t dressed up like such a slut.  You bet you could get her to help pleasure you, but she might not like it.  Or you could be a nice ' + CoC.player.guyGirl() + ' and let her go...\n\nDo you force her to pleasure you?', false );
	...
	Faerie.prototype.faerieRAEP = function() {
		...
		//Non-Taurs
			...
			//(medium) <= 0.1.25"
				...
				EngineCore.outputText( 'You spread your ' + CoC.player.legs() + ', careful not to dislodge the faerie as she releases the ' + Descriptors.clitDescript() + ' and stands up, placing her dripping gash against the tip.   A quick plunge later and she\'s bottomed out, pressing her hips into the opening of your ' + Descriptors.vaginaDescript( 0 ) + ' her feet slipping over the outer folds as she tries to maintain her balance.   You start rocking back and forth happily, bouncing the faerie up and down.  She moans, cute and barely audible, but sexy in a way that makes your sopping fuckhole even wetter.\n\n', false );
	...
	Faerie.prototype.faerieCaptureHJ = function() {
		...
		if( CoC.player.isTaur() ) {
			...
			if( CoC.player.statusAffectv1( StatusAffects.FaerieFucked ) < 10 ) {
				...
				EngineCore.outputText( 'The faerie burps and laughs drunkenly, patting the side of your ' + CoC.player.leg() + ' and slurring, "<i>Oh by Marae\'s ripe titsh!  I needed that.  Do you thhink you could catsch me again?  I love feeling your cum coating my body.</i>"\n\n', false );
			} else {
		...
		//Non-taurs
			...
			if( CoC.player.cumQ() > 250 ) {
				EngineCore.outputText( 'You cum uncontrollably, regretting your fertility as your body paints the inside of your ' + CoC.player.armorName + ' with goopy whiteness.  ', false );
