...
angular.module( 'cocjs' ).run( function( SceneLib, MainView, CoC, Utils, StatusAffects, EngineCore, AppearanceDefs, Descriptors, CockTypesEnum, PerkLib ) {
	...
	CorruptedGlade.prototype.flowerFun = function() {
		...
		if( CoC.player.hasCock() ) { //Sex scenes for those with cawks
			...
			MainView.outputText( 'You walk away, your lips and tongue feeling slightly puffy and sensitive, but none the worse for the wear.' );
	...
	CorruptedGlade.prototype.tentacleFun = function() {
		...
		if( CoC.player.vaginas.length > 0 && Utils.rand( 2 ) === 0 ) {
			...
			MainView.outputText( 'You saunter over to a dangling group of perverted looking vines, discarding your ' + CoC.player.armorName + ' along the way. Running your fingertips along the bulbous-tipped tentacle-like vines, you find one that looks ' );
			var tentacle = Utils.rand( 3 ); //Big, medium or small size tentacle
			var tentacleSize = 0;
			if( tentacle === 0 ) { //Small
				MainView.outputText( 'a little small for your ' );
			...
			if( tentacle === 1 ) { //Normal
				MainView.outputText( 'well suited to your ' );
				switch( CoC.player.vaginas[ 0 ].vaginalLooseness ) {
					case AppearanceDefs.VAGINA_LOOSENESS_TIGHT:
						tentacleSize = 3;
						break;
			...
			if( tentacle === 2 ) { //Large
				MainView.outputText( 'almost too big to cram in your ' );
				switch( CoC.player.vaginas[ 0 ].vaginalLooseness ) {
					case AppearanceDefs.VAGINA_LOOSENESS_TIGHT:
						tentacleSize = 6;
						break;
			...
			if( Utils.rand( 4 ) === 0 && CoC.player.buttRating <= 10 ) {
				MainView.outputText( 'A strange shifting occurs on your backside, making your ' + CoC.player.armorName + ' feel tight.  <b>Your butt has grown larger</b>, becoming a ' + Descriptors.buttDescript() + '.  ' );
			...
			if( Utils.rand( 3 ) === 0 && CoC.player.cocks.length > 0 && CoC.player.hairColor === 'green' ) {
				if( CoC.player.tentacleCocks() < CoC.player.cockTotal() ) {
					if( CoC.player.cocks.length === 1 ) { //Single cawks
						MainView.outputText( 'Your feel your ' + Descriptors.cockDescript( 0 ) + ' bending and flexing of its own volition... looking down, you see it morph into a green vine-like shape.  <b>You now have a tentacle cock!</b>  ' );
						//Set primary cock flag
						CoC.player.cocks[ 0 ].cockType = CockTypesEnum.TENTACLE;
					}
					if( CoC.player.cockTotal() > 1 ) { //multi
						MainView.outputText( 'Your feel your ' + Descriptors.multiCockDescriptLight() + ' bending and flexing of their own volition... looking down, you watch them morph into flexible vine-like shapes.  <b>You now have green tentacle cocks!</b>  ' );
	...
	CorruptedGlade.prototype.treeBoobFun = function() {
		...
		if( CoC.player.findStatusAffect( StatusAffects.LustyTongue ) < 0 ) {
			if( Utils.rand( 4 ) === 0 ) { //25% Chance of sensitive mouth status – increased lust gain/hour due to licking your lips
				...
				MainView.outputText( '  Thankfully, the feeling goes away after a few minutes, leaving you feeling just a bit more sensitive.' );
			}
		}
		if( Utils.rand( 2 ) === 0 ) { //50% Chance of breast-growth
			if( CoC.player.biggestTitSize() > 5 ) {
				MainView.outputText( '  As you redress, you feel a sense of added weight on your chest.  After a few moments you realize your already-formidable chest has grown even larger.' );
				CoC.player.growTits( 1, 1 + Utils.rand( 3 ), false, 1 );
			} else {
				if( CoC.player.biggestTitSize() === 0 ) { //No tits yet
					MainView.outputText( '  As you redress, you realize you have grown a pair of luscious breasts!  Perhaps this was a good idea after all...' );
					CoC.player.growTits( 1, 2 + Utils.rand( 3 ), false, 1 );
				} else {
					//Small tits growth
					MainView.outputText( '  As you redress, you realize your breasts have gotten quite a bit larger!  Maybe you can come back later when you aren\'t so full and plump them up a bit more.' );
					CoC.player.growTits( 1, 1 + Utils.rand( 3 ), false, 1 );
				}
			}
		}
