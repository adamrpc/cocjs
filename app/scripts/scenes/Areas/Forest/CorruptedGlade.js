...
angular.module( 'cocjs' ).run( function( CoC, Utils, StatusAffects, EngineCore, AppearanceDefs, Descriptors, CockTypesEnum, PerkLib ) {
	...
	CorruptedGlade.prototype.intro = function() {
		...
		if( CoC.getInstance().player.cor <= 33 ) { //disgusted reaction
			//Get plant-cum dripped on you if not fast and unlucky!
			if( CoC.getInstance().player.spe < 60 && Utils.rand( CoC.getInstance().player.spe + 50 ) < 50 ) {
		...
		} else if( CoC.getInstance().player.cor <= 66 ) { //intrigued reaction
			...
			EngineCore.dynStats( 'lus', 20 + CoC.getInstance().player.lib / 5, 'cor', 0.5 );
			EngineCore.doNext( CoC.getInstance().scenes.camp.returnToCampUseOneHour );
			...
			EngineCore.choices( 'Flowers', this.flowerFun, 'Vines', this.tentacleFun, 'Trees', this.treeBoobFun, '', null, 'Leave', CoC.getInstance().scenes.camp.returnToCampUseOneHour );
		}
	...
	CorruptedGlade.prototype.flowerFun = function() {
		...
		EngineCore.spriteSelect( 92 );
		if( CoC.getInstance().player.hasCock() ) { //Sex scenes for those with cawks
			if( CoC.getInstance().player.cocks.length === 1 ) { //Single Cawk
				EngineCore.outputText( 'You grin to yourself as you decide to see just how close to a pussy these perverted little flowers are.  The thick stem bends with ease as you grasp it and bend it towards your groin, your other hand fumbling to open your ' + CoC.getInstance().player.armorName + '.  In seconds you free yourself and gingerly bring the folds closer, the musky scent that fills the air rapidly bringing you to a full, throbbing hardness.  The first touch of petals to your skin slicks you with the flower\'s silky secretions, allowing you to easily slip between the petals.  Though the flower looks fairly deep, you quickly feel yourself bottom out inside the petal\'s slippery grip.  Shrugging, you decide to make the best of it and begin thrusting into the plant, enjoying the unusual sensations along the front-most parts of your ' + Descriptors.cockDescript( 0 ) + '.  As you pound away, you begin to notice a change in the rear of the flower.\n\n' );
			...
			EngineCore.outputText( 'You cum, and cum, and cum, the evidence of your pleasure devoured by the plant\'s sucking, squeezing gullet.  The orgasm drags on for what feels like forever, your ' + CoC.getInstance().player.legs() + ' eventually giving out, your hips the only muscle that seems to work as they twitch into the air, as if begging for more.  You are milked of a few last big spurts, at last collapsing.\n\n' );
			EngineCore.outputText( 'The tendrils encircling your genitals do not release; instead they pull tighter, one of ' + (CoC.getInstance().player.cocks.length > 1 ? 'each plant\'s tiny' : 'the tiny plant\'s') + ' appendages penetrating your urethra' + (CoC.getInstance().player.cocks.length > 1 ? 's' : '') + ', squirming up your cum slicked passage' + (CoC.getInstance().player.cocks.length > 1 ? 's' : '') + ' with uncomfortable slowness.  You lay there, too weak to resist it or fight, hoping that whatever the plants are doing won\'t hurt much.  You feel it twisting and coiling inside you... until it stops.  You feel a sharp pinch, and then it withdraws, seemingly satisfied.  The tendrils unwrap, allowing the plants to spring back up, exposing your still over-engorged and sensitive member' + (CoC.getInstance().player.cocks.length > 1 ? 's' : '') + '.\n\n' );
			EngineCore.outputText( 'You lay there for some time until your muscle control returns, your cock' + (CoC.getInstance().player.cocks.length > 1 ? 's' : '') + ' still slightly over-large ' + (CoC.getInstance().player.balls >= 2 ? 'and your ' + Descriptors.ballsDescriptLight() : 'and') + ' feeling sore from the exertion.  At least you hope it\'s just from the exertion and not from whatever the plant did.\n\n' );
			...
			if( CoC.getInstance().player.findPerk( PerkLib.MessyOrgasms ) >= 0 && CoC.getInstance().player.cumMultiplier < 3 ) {
				booster += 1;
			}
			CoC.getInstance().player.cumMultiplier += booster;
	...
	CorruptedGlade.prototype.tentacleFun = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 92 );
		if( CoC.getInstance().player.vaginas.length > 0 && Utils.rand( 2 ) === 0 ) {
			//Vaginal Variant 50% of the time
			EngineCore.outputText( 'You saunter over to a dangling group of perverted looking vines, discarding your ' + CoC.getInstance().player.armorName + ' along the way. Running your fingertips along the bulbous-tipped tentacle-like vines, you find one that looks ' );
			var tentacle = Utils.rand( 3 ); //Big, medium or small size tentacle
			var tentacleSize = 0;
			if( tentacle === 0 ) { //Small
				EngineCore.outputText( 'a little small for your ' );
				switch( CoC.getInstance().player.vaginas[ 0 ].vaginalLooseness ) {
			...
			if( CoC.getInstance().player.cuntChange( tentacleSize, true ) ) {
			...
			//Xforms
			if( Utils.rand( 3 ) === 0 && CoC.getInstance().player.hairColor !== 'green' ) { //Change hair to green sometimes
				...
				CoC.getInstance().player.hairColor = 'green';
			}
			if( Utils.rand( 4 ) === 0 && CoC.getInstance().player.hipRating <= 10 ) { //+hip up to 10
				EngineCore.outputText( 'A strange shifting occurs below your waist, making your ' + CoC.getInstance().player.armorName + ' feel tight.  <b>Your hips have grown larger</b>, becoming ' + Descriptors.hipDescript() + '.  ' );
			...
			if( CoC.getInstance().player.cocks.length > 0 ) { //Cocks (and maybe vagina)
				EngineCore.outputText( CoC.getInstance().player.cocks.length === 1 ? Descriptors.cockDescript( 0 ) : Descriptors.multiCockDescriptLight() );
			...
			EngineCore.dynStats( 'lus', 25 + CoC.getInstance().player.lib / 10, 'cor', 2 );
			//Change hair to green sometimes
			if( Utils.rand( 3 ) === 0 && CoC.getInstance().player.hairColor !== 'green' ) {
				EngineCore.outputText( 'You don\'t get far before you realize all the hair on your body has shifted to a verdant green color.  <b>You now have green hair.</b>  ' );
				CoC.getInstance().player.hairColor = 'green';
			}
			//+butt up to 10
			if( Utils.rand( 4 ) === 0 && CoC.getInstance().player.buttRating <= 10 ) {
				EngineCore.outputText( 'A strange shifting occurs on your backside, making your ' + CoC.getInstance().player.armorName + ' feel tight.  <b>Your butt has grown larger</b>, becoming a ' + Descriptors.buttDescript() + '.  ' );
				CoC.getInstance().player.buttRating += Utils.rand( 3 ) + 1;
			}
			//Rarely change one prick to a vine-like tentacle cock.
			if( Utils.rand( 3 ) === 0 && CoC.getInstance().player.cocks.length > 0 && CoC.getInstance().player.hairColor === 'green' ) {
				if( CoC.getInstance().player.tentacleCocks() < CoC.getInstance().player.cockTotal() ) {
					if( CoC.getInstance().player.cocks.length === 1 ) { //Single cawks
					...
					if( CoC.getInstance().player.cockTotal() > 1 ) { //multi
	...
	CorruptedGlade.prototype.treeBoobFun = function() {
		...
		CoC.getInstance().player.slimeFeed();
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.LustyTongue ) < 0 ) {
			if( Utils.rand( 4 ) === 0 ) { //25% Chance of sensitive mouth status – increased lust gain/hour due to licking your lips
				...
				CoC.getInstance().player.createStatusAffect( StatusAffects.LustyTongue, 24, 0, 0, 0 );
			} else {
		...
		if( Utils.rand( 2 ) === 0 ) { //50% Chance of breast-growth
			if( CoC.getInstance().player.biggestTitSize() > 5 ) {
				...
				CoC.getInstance().player.growTits( 1, 1 + Utils.rand( 3 ), false, 1 );
			} else {
				if( CoC.getInstance().player.biggestTitSize() === 0 ) { //No tits yet
					...
					CoC.getInstance().player.growTits( 1, 2 + Utils.rand( 3 ), false, 1 );
				} else {
					//Small tits growth
	...
	CoC.getInstance().registerScene( 'corruptedGlade', new CorruptedGlade() );
} );