...
angular.module( 'cocjs' ).run( function( SceneLib, $log, CoC, StatusAffects, Combat, TentacleBeast, Descriptors, PerkLib, AppearanceDefs, EngineCore, Utils, EventParser, kFLAGS) {
	...
	TentacleBeastScene.prototype.encounter = function() {
		...
		if( CoC.getInstance().player.gender === 0 && Utils.rand( 3 ) === 0 && !CoC.getInstance().player.isNaga() && !CoC.getInstance().player.isTaur() && !CoC.getInstance().player.isGoo() ) {
			...
			if( CoC.getInstance().player.cor > 50 && CoC.getInstance().player.cor <= 75 ) {
	...
	TentacleBeastScene.prototype.tentacleEntice = function() {
		...
		if( CoC.getInstance().player.gender === 1 || CoC.getInstance().player.gender === 3 ) {
			if( Utils.rand( 2 ) === 0 ) {
				...
				CoC.getInstance().monster.lust += 10 + Utils.rand( 5 );
			}
			//Failure:
			else {
	...
	TentacleBeastScene.prototype.tentacleLossRape = function() {
		...
		//Horsecock surprise!
		if( CoC.getInstance().player.horseCocks() > 0 && CoC.getInstance().player.cocks[ 0 ].cockLength > 15 && CoC.getInstance().player.cocks[ 0 ].cockThickness >= 3 ) {
			if( CoC.getInstance().player.cor < 75 && CoC.getInstance().player.lust < 100 ) {
		...
		if( CoC.getInstance().player.lust > 99 ) {
			var temp = CoC.getInstance().player.findStatusAffect( StatusAffects.TentacleBadEndCounter );
			if( temp < 0 ) {
				CoC.getInstance().player.createStatusAffect( StatusAffects.TentacleBadEndCounter, 0, 0, 0, 0 );
			} else {
				//count up
				CoC.getInstance().player.statusAffect( temp ).value1++;
				//Bad end
				if( CoC.getInstance().player.statusAffect( temp ).value1 >= 3 && CoC.getInstance().player.cor > 50 && CoC.getInstance().player.gender === 3 ) {
		...
		//Centaur madness!
			...
			if( CoC.getInstance().player.biggestTitSize() >= 1 && CoC.getInstance().player.lactationQ() > 0 && CoC.getInstance().player.lactationQ() < 1000 ) {
			...
			else if( CoC.getInstance().player.biggestTitSize() >= 1 && CoC.getInstance().player.lactationQ() >= 1000 ) {
			...
			if( CoC.getInstance().player.hasCock() && CoC.getInstance().player.cumQ() < 1500 && CoC.getInstance().player.ass.analLooseness < 4 ) {
			...
			// has cock, normal cum amount, anus === gaping:
			if( CoC.getInstance().player.hasCock() && CoC.getInstance().player.cumQ() < 1500 && CoC.getInstance().player.ass.analLooseness >= 0 ) {
		...
		//Milk surprise!
		if( CoC.getInstance().player.biggestLactation() >= 3.5 && CoC.getInstance().player.gender > 0 ) {
			CoC.getInstance().player.slimeFeed();
			...
			if( CoC.getInstance().player.gender === 2 ) {
				...
				if( CoC.getInstance().player.cuntChange( 20, true ) ) {
					EngineCore.outputText( '\n', false );
				}
				EngineCore.outputText( '\n', false );
			}//HERMS
		...
		if( CoC.getInstance().player.gender === 2 ) {
			...
			EngineCore.outputText( '\n\n' + (CoC.getInstance().player.totalBreasts() + 1) + ' thorny tendrils appear and pierce your breasts and groin. A sharp pain and a burning sensation tear through you, overriding the previous wave of pleasure. You feel fluids being injected into you and a distinctive, agonizing misery flows into your veins.  Your breasts and ', false );
		...
		if( CoC.getInstance().player.gender === 3 ) {
			...
			EngineCore.outputText( '  ' + Utils.Num2Text( (CoC.getInstance().player.totalNipples()) ) + ' thorny tentacles pierce your nipples, and you feel as if someone shot acid into your tits, which immediately begin to swell.', false );
			CoC.getInstance().player.growTits( 1, CoC.getInstance().player.breastRows.length, false, 2 );
	...
	TentacleBeastScene.prototype.tentacleRapeContinuation = function() {
		...
		if( CoC.getInstance().player.gender === 1 ) {
			...
			CoC.getInstance().player.buttChange( 50, true );
			...
			CoC.getInstance().player.cuntChange( CoC.getInstance().player.vaginalCapacity() * 0.76, true );
			...
			if( CoC.getInstance().player.cockTotal() === 1 ) {
				...
				EngineCore.outputText( 'A sharp tug tells you that the creature has sealed itself upon your ' + Descriptors.multiCockDescriptLight() + '. You see ' + CoC.getInstance().player.totalBreasts() + ' smaller tentacles latch onto your erect nipples. You feel milk begin to leak out as the creature makes a perfect seal around your areola. A thick, phallic tentacle probes underneath your trapped cocks until it finds your vaginal opening. You cry out as the member punches past your opening and bottoms out in your womb. The tentacle swells up until it completely fills your ' + Descriptors.vaginaDescript( 0 ) + '.' );
				CoC.getInstance().player.cuntChange( CoC.getInstance().player.vaginalCapacity() * 0.76, true, true, false );
	...
	TentacleBeastScene.prototype.tentacleRapeContinuationForFemales = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 100 );
		if( CoC.getInstance().player.vaginas.length === 1 ) { //single coochie
			...
			if( (CoC.getInstance().player.findPerk( PerkLib.BigClit ) >= 0 && CoC.getInstance().player.clitLength > 2) || CoC.getInstance().player.clitLength > 3 ) {
				EngineCore.outputText( 'almost ' + Utils.num2Text( Math.floor( CoC.getInstance().player.clitLength * 1.75 ) ) + ' inches in length. ' );
			...
			EngineCore.outputText( 'Satisfied, the creature drops you smartly and withdraws its limbs from you and lumbers away.  Covered completely in cum, you see that your ' + CoC.getInstance().player.vaginas.length + ' clits have swollen up to almost four inches in length.  Bruised and sore, you pass into unconsciousness, ' );
		}
		//Not too corrupt
		if( CoC.getInstance().player.cor < 75 ) {
		...
		if( CoC.getInstance().player.clitLength < 7 && CoC.getInstance().player.clitLength >= 3.5 && CoC.getInstance().player.findPerk( PerkLib.BigClit ) >= 0 ) {
			CoC.getInstance().player.clitLength += 0.1 + CoC.getInstance().player.cor / 100;
			EngineCore.outputText( 'Your massive clitty eventually diminishes, retaining a fair portion of its former glory.  It is now ' + Math.ceil( CoC.getInstance().player.clitLength * 10 ) / 10 + ' inches long when aroused, ' );
			if( CoC.getInstance().player.clitLength < 5 ) {
				EngineCore.outputText( 'like a tiny cock.' );
			}
			if( CoC.getInstance().player.clitLength >= 5 && CoC.getInstance().player.clitLength < 7 ) {
		...
		else if( CoC.getInstance().player.clitLength < 3.5 ) {
	...
	TentacleBeastScene.prototype.centaurGenderlessRetardation = function() {
		EngineCore.outputText( '', true );
		EngineCore.spriteSelect( 100 );
		if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00247 ] === 0 || CoC.getInstance().player.balls === 0 ) {
			CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00247 ] = 1;
			...
			if( CoC.getInstance().player.cor < 50 && CoC.getInstance().player.lust < 70 ) {
	...
	TentacleBeastScene.prototype.genderlessHilarityForNagaKenDolls = function() {
		...
		CoC.getInstance().player.takeDamage( 15 );
		if( CoC.getInstance().isInCombat() ) {
			Combat.cleanupAfterCombat();
		} else {
			EngineCore.doNext( SceneLib.camp.returnToCampUseOneHour );
