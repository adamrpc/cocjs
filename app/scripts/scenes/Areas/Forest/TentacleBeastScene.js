...
angular.module( 'cocjs' ).run( function( MainView, SceneLib, $log, CoC, StatusAffects, Combat, TentacleBeast, Descriptors, PerkLib, AppearanceDefs, EngineCore, Utils, kFLAGS) {
	...
	TentacleBeastScene.prototype.encounter = function() {
		...
		if( CoC.player.gender === 0 && Utils.rand( 3 ) === 0 && !CoC.player.isNaga() && !CoC.player.isTaur() && !CoC.player.isGoo() ) {
			...
			MainView.outputText( 'You see a massive, shambling form emerge from the underbrush. While first appearing to be a large shrub, it shifts its bulbous mass and reveals a collection of thorny tendrils and cephalopodic limbs. Sensing your presence, it lumbers at you, full speed, tentacles outstretched.\n\n', false );
			if( CoC.player.cor > 50 && CoC.player.cor <= 75 ) {
				MainView.outputText( 'You debate the merits of running from such a creature, and realize it\'s now too late to escape.  ', false );
			}
			if( CoC.player.cor > 75 ) {
				MainView.outputText( 'You smile and stride forward, welcoming the pleasure you expect from such a monster.  ', false );
			}
			//HILARIOUS NEUTER EVENT HERE
			if( CoC.player.cor < 75 ) {
				MainView.outputText( 'While you attempt to resist the abomination, its raw muscle mass is too much. ', false );
			}
			MainView.outputText( 'It pins you to the ground easily. You feel slimy tentacles run up and down your groin as the creature searches for whatever gonads it expected you to have. When it realizes that you have neither penis nor vagina, it smartly casts you to the ground in apparent disgust.\n\n"<i>WHAT THE FUCK IS THIS SHIT?!!</i>" The creature speaks in an unnervingly human voice.\n\n', false );
			MainView.outputText( 'Completely confused, all you can do is sit there in shock.\n\n"<i>Where are your naughty bits, goddammit!</i>" the creature bellows. "<i>Us tentacle creatures need to FEED!</i>"\n\n', false );
			MainView.outputText( 'You sheepishly state that you are gender-neutral and have no genitalia.\n\n"<i>You gotta be shitting me!!</i>" the monster bellows in contempt. "<i>Of all the motherfuckers I ambush, it has to be the ONE bastard I can\'t feed from! What am I supposed to do now, asshole?! I gotta eat!</i>"', false );
			MainView.outputText( 'At a loss for words, you meekly offer the creature some of your food you have packed for your journey. The creature slaps it out of your hand, almost breaking your wrist.\n\n"<i>I can\'t eat that shit!</i>" roars the abomination. "<i>Do I look like I have a fucking mouth to chew that with?! NOOOOOO! I feed off dicks and wayward women! Cum and tit milk! YOU have NEITHER!!!</i>"  ', false );
			MainView.outputText( 'The beast slaps you squarely on the ass as if to push you along. "<i>Get the fuck out of here!</i>" it screams. "<i>Get lost so I can hunt me a REAL meal!!!</i>"', false );
		...
		//Combat starter
		MainView.outputText( 'You see a massive, shambling form emerge from the underbrush.  While it resembles a large shrub, a collection of thorny tendrils and cephalopodic limbs sprout from its bulbous mass.  Sensing your presence, it lumbers at you, full speed, tentacles outstretched.\n\n', false );
		if( CoC.player.cor > 50 && CoC.player.cor <= 75 ) {
			MainView.outputText( 'You debate the merits of running from such a creature.\n\n', false );
		}
		if( CoC.player.cor > 75 ) {
		...
		if( CoC.player.cor > 50 ) {
			MainView.outputText( 'Do you joyfully submit or fight back?\n\n', false );
	...
	TentacleBeastScene.prototype.tentacleEntice = function() {
		...
		if( CoC.player.gender === 1 || CoC.player.gender === 3 ) {
			if( Utils.rand( 2 ) === 0 ) {
				MainView.outputText( 'In an effort to distract the creature, you begin gyrating your hips and swinging your penis in a shameless imitation of MeatSpin. The Tentacled Horror briefly pauses to observe your actions and rears similar to a posturing spider, considering your next actions.\n\n', false );
				CoC.monster.lust += 10 + Utils.rand( 5 );
			}
			//Failure:
			else {
				MainView.outputText( 'You grab your penis and shake it feverishly at the creature in order to distract it.  It swats a tentacle at you, forcing you to adroitly dodge the reprisal.  Apparently, the beast found you unimpressive.\n\n', false );
			}
		}
	...
	TentacleBeastScene.prototype.tentacleVictoryRape = function() {
		...
		if( CoC.player.gender === 1 || CoC.player.gender === 3 ) {
			...
			MainView.outputText( '"<i>You want dick milk, you freak?!</i>" you bellow in triumph. "<i>HERE YOU GO!</i>"\n\n', false );
			MainView.outputText( 'You impale the tentacle on your penis and begin humping wildly, treating the creature\'s armature as your own personal onahole. The creature squirms wildly, trying to escape your lust-driven strength. Replying with a swift kick with your free foot, the creature\'s resolve to escape wavers.\n\n', false );
			MainView.outputText( '"<i>Quit fuckin\' squirming and take your MEDICINE!</i>" you thunder as you cheap-shot the beast for good measure.\n\n', false );
			MainView.outputText( 'Feeling your peak approach, you allow your muscles into their rhythmic contractions as you unload your cum into the creature like a howitzer attacking a fort. Laughing like a maniac with each shot, you see your jizz leak out as the creature struggles to assimilate your \'donation\'.\n\n', false );
			MainView.outputText( 'Withdrawing your prick, you cast the beast aside and begin walking away both amused and satisfied. The beast stumbles weakly back into the wood it came from, clearly worse-off from the encounter.', false );
		}
		//Female:
		else {
			...
			if( CoC.player.cor >= 66 ) {
				MainView.outputText( 'Seizing the opportunity, you rush the monster while it is stopped. You grab the fattest phallic tentacle you can find and summarily shit-kick the beast onto the ground. Holding the creature down with one foot, you take the tentacle-cock and poise it in front of your dripping cunt.\n\n', false );
				MainView.outputText( 'Laughing like a true psychotic, you stuff the tentacle into your womb.', false );
				CoC.player.cuntChange( 20, true, true, false );
				MainView.outputText( '  Your vaginal muscles quickly go to work stroking, squeezing and kneading the appendage. The creature, more intent with escape than hammering your box, begins struggling. You summarily slug the beast as well as any professional pugilist to stop its throes.\n\n', false );
				MainView.outputText( '"<i>STOP STRUGGLING AND FUCK MY LITTLE PUSSY!</i>", you screech.\n\n', false );
				MainView.outputText( 'The sensation of the beast ejaculating immediately gets your attention. As your womb fills with its warm load, a brutal idea takes you. The beast responded after you hit it. Smirking like a devil, you turn the beast into a punching bag. With each strike, the beast sprays a batch of goo deep inside your body. The sheer force of the spray is working your hole into an ecstatic frenzy. As you orgasm, you slug the creature again, forcing another batch of semen to flush your womanhood. After an hour of this, you reach a multi-orgasmic peak and release. The creature twitches weakly as you pull the limp tentacle from your body. The excess spunk flows out like an overturned bucket, leaving one hell of a mess. You walk away satisfied. It is unclear whether the tentacled horror survived your lust... but who cares. Your satisfaction is all you cared about, anyway.', false );
			}
			//Rape Win Female-Low Corruption
			else {
				MainView.outputText( 'Seizing the opportunity, you rush the monster while it is stopped. You grab the fattest phallic tentacle you can find and summarily push the beast onto the ground. Holding the creature down with your body weight, you take the tentacle-cock and poise it in front of your dripping cunt.\n\n', false );
				MainView.outputText( 'You sit on the creature and begin using the tentacle as a living dildo. With your mass atop it, the creature cannot move or struggle, despite its lack of any attempts to do so. You push the limb deeper and deeper until you feel it bottom out against your cervix.', false );
				CoC.player.cuntChange( 20, true, true, false );
	...
	TentacleBeastScene.prototype.futaTentacleEpilogue = function() {
		...
		if( CoC.giacomo > 0 ) {
			...
			MainView.outputText( 'While the fog of your brain has yet to lift completely, you recognize the voice to be the seedy merchant, Giacomo.\n\n', false );
			MainView.outputText( '"<i>It is a good thing I happened to be out and about today.</i>", Giacomo says. "<i>I was testing out a new weapon to sell and I happened to see one of those nasty tentacle beasties. I had no idea they captured prey! Hell, you must have spent a few months inside that thing feeding it!</i>"\n\n', false );
			MainView.outputText( 'You attempt to say something, only to find yourself incapable of speaking. You feel the man\'s bony hands pick you up and set you down in what feels like his cart.\n\n', false );
			MainView.outputText( '"<i>Well, I can\'t be a total bastard all the time.</i>", Giacomo jingles. "<i>I guess I can drop you off at the next village I come to so you can recover. Isn\'t that absolutely nice of me! Even better! I will do this for free!!!</i>"\n\n', false );
			MainView.outputText( 'Giacomo giggles to himself at his cheaply bought humanitarianism. A part of you dreads what is to happen next as nothing about the merchant ever struck you as trustworthy. However, a day or so later, true to his word, he leaves you at the clinic in the first town he comes to. Your recovery takes the better part of the year. The healers and apothecaries purge you of all of your corruptions, save your transgendered status. However, the sheer stress on your body has effectively ended your adventuring lifestyle and you resign yourself to settle down to a comparatively mundane existence, broken by the occasional tryst with a villager curious about your genitalia.', false );
		}
		//[Never met Giacomo]
		else {
			MainView.outputText( '"<i>Will she live?</i>", says a soft and feminine voice.\n\n', false );
			MainView.outputText( '"<i>Yes, doctor. She will live.</i>", replies a gruff and clearly masculine voice.\n\n', false );
			MainView.outputText( '"<i>Is the beast dead</i>", queries the doctor.\n\n', false );
			MainView.outputText( '"<i>Dead several times over, madam.</i>", answers the man.\n\n', false );
			MainView.outputText( '"<i>We cannot leave this unfortunate woman out in the wild like this. Load her onto the wagon. We will take her back to the village. I am certain I can help this woman recover.</i>", the doctor states flatly.\n\n', false );
			MainView.outputText( 'Strong masculine hands easily lift your atrophied body and place you on a wooden slab. You feel the shaking of a cart as its movement assaults your stunted senses. After a while you notice the cart stops as it arrives at its destination. A cacophony of voices talk over one another as you feel a half a dozen people move you to what can only be a clinic. Many of the voices talk constantly as you are examined and various medicines are applied to you exhausted body. Your vision returns in a day or so, revealing that you are in a hospital and laborious effort from the staff allowed for your revival.\n\n', false );
			MainView.outputText( 'Your recovery takes the better part of the year. The healers and apothecaries purge you of all of your corruptions, save your transgendered status. However, the sheer stress on your body has effectively ended your adventuring lifestyle and you resign yourself to settle down to a comparatively mundane existence, broken by the occasional tryst with a villager curious about your genitalia, which you are more than happy to display.', false );
		}
		EngineCore.gameOver();
	};
	TentacleBeastScene.prototype.tentacleLossRape = function() {
		...
		if( CoC.player.gender === 0 ) {
			...
			if( CoC.player.isTaur() ) {
				...
				MainView.outputText( '"<i>Where are your naughty bits, goddammit!</i>" the creature bellows. "<i>Us tentacle creatures need to FEED!</i>"\n\n', false );
				MainView.outputText( 'You sheepishly state that you are gender neutral and have no genitalia.\n\n"<i>You gotta be shitting me!!</i>" the monster bellows in contempt. "<i>Of all the motherfuckers I ambush, it has to be the ONE bastard I can\'t feed from! What am I supposed to do now, asshole?! I gotta eat!</i>"', false );
		...
		//Centaur madness!
			...
			MainView.outputText( 'Tentacles wrap around your legs before you can stop them.  They continue to coil up your legs, spreading an uncomfortable warmth through your equine half.  Another tentacle wraps around your torso, spreading that same warmth and fuzzing your mind.  You grab one you can reach and attempt to tear it off of you, but two thinner, translucent feelers immobilize your arms, pulling them up behind your head.\n\n', false );
			CoC.player.slimeFeed();
			MainView.outputText( 'They test your body, slipping about over your form.  A small tentacle finds its way into your mouth, coiling about your tongue and down your throat.  It\'s careful not to make you choke, seemingly as curious about your innards as it is about your shell.  You\'re given little time to think though, as a surge of fluid is deposited into your stomach, making your desire to cum grow even more.  The sharp spines coiled about you act similarly, spreading warmth about them wherever they touch your ' + CoC.player.skin() + '.\n\n', false );
			// has at least 1 cock, engulfable:
			if( CoC.player.hasCock() ) {
				if( CoC.player.cockArea( CoC.player.smallestCockIndex() ) <= 50 ) {
					MainView.outputText( 'More aphrodisiac-toxin pours into you, causing ' + Descriptors.sMultiCockDesc() + ' to expand.  ', false );
			...
			if( CoC.player.hasVagina() ) {
				MainView.outputText( 'A squirming tentacle forces its way inside your ' + Descriptors.vaginaDescript( 0 ) + ', undulating and squirming as it works its way deeper and deeper.  Your body responds by pumping out more fluid, making the passage of the monstrous thing easier.', false );
				CoC.player.cuntChange( 32, true, true, false );
				if( CoC.player.hasCock() ) {
					if( CoC.player.cockArea( CoC.player.smallestCockIndex() ) <= 50 ) {
						MainView.outputText( '  Your humping appears to not affect the creatures continuing efforts, despite the force of your body.', false );
					}
				}
				MainView.outputText( '  You feel the beast bottom out against your uterus and cry out in pleasure, gyrating yourself about as fluid sprays behind you.\n\n', false );
			}
			// Breasts > Manly, non-lactating:
			if( CoC.player.biggestTitSize() >= 1 && CoC.player.lactationQ() <= 0 ) {
				MainView.outputText( 'Roving tentacles latch onto your ' + Descriptors.allBreastsDescript() + '; tiny spikes jabbing into each ' + Descriptors.nippleDescript( 0 ) + ' and injecting some sort of hot fluid.', false );
				if( CoC.player.totalBreasts() === 2 ) {
					MainView.outputText( '  The anus-like tips affix to them.', false );
				} else {
					MainView.outputText( '  The anus-like tips attach to one pair as more appear in order to take the others.', false );
				}
				MainView.outputText( '  You feel a gush of liquid leave your body as the translucent lengths of the tentacles turn stark white.  The fluid they inject has caused you to lactate!  They suckle at you incessantly and before long your nipples ache from overuse and your breasts have run completely dry.\n\n', false );
				CoC.player.boostLactation( 1.5 );
			}
			// Anus === gaping:
			if( CoC.player.ass.analLooseness >= 4 ) {
				MainView.outputText( 'Your ' + Descriptors.assholeDescript() + ' makes an inviting target for the squirming mass and it\'s quick to capitalize.  A particularly bulbous appendage slides deep inside, roiling about in a way that not even your well-trained hole has been treated to.', false );
				if( CoC.player.hasCock() ) {
					MainView.outputText( '  A series of undulating lumps pass over your prostate, pushing out a splash of pre-cum.', false );
				}
				MainView.outputText( '  You moan into the tentacle in your mouth appreciatevely at the beast\'s spectacular skill.\n\n', false );
			}
			...
			// Breasts > Manly, lactating, enough to overfill:
				...
				MainView.outputText( 'Roving tentacles latch onto your ' + Descriptors.allBreastsDescript() + ', tiny spikes jabbing into your ' + Descriptors.nippleDescript( 0 ) + ' and injecting some sort of hot fluid.  The pressure inside grows nearly unbearable as you feel your milk production increase.  To your relief, an anus-like tip attaches to each nipple.  They suckle at you incessantly and before long your nipples ache from overuse, but your breasts are still prepared to provide more milk!  The suction decreases as the beast before you becomes overfilled and eventually is forced to give up.\n\n', false );
				if( CoC.player.hasCock() ) {
					MainView.outputText( 'Your ' + Descriptors.cockDescript( 0 ) + ' explodes inside the creature, ', false );
					if( CoC.player.cumQ() <= 500 ) {
						MainView.outputText( 'pushing the creature to the edge of its fluid-containing abilities.', false );
					} else {
						MainView.outputText( 'quickly overfilling the tentacle attached to it; it explodes off of you, freeing your spunk to spray from both you and the retreating beast.  ', false );
					}
				}
				MainView.outputText( 'The tentacles holding you release, leaking fluids everywhere.  You delight in giving one of the larger ones a hard stomp, as a reminder not to trifle with you.', false );
				//end (victory)
				CoC.player.orgasm();
			...
			if( CoC.player.hasCock() ) {
				...
				MainView.outputText( 'The creature\'s desires are soon fulfilled as your ' + Descriptors.cockDescript( 0 ) + ' starts to swell.  ', false );
				//[has testicles:
				if( CoC.player.balls > 0 ) {
					MainView.outputText( 'Your ' + Descriptors.ballsDescriptLight() + ' tighten up against you in preparation for their inevitable release, ready to spray their boiling load into the beast.  ', false );
				}
				MainView.outputText( 'You rear up as a surge of euphoria races through you; your equine strength manages to overpower the tentacles holding your forelegs down for the briefest of moments needed to release your spunk into the suction of the tentacle, and you feel it get whisked out and down toward the writhing mass.\n\n', false );
			}
			// has vagina:
			if( CoC.player.hasVagina() ) {
				MainView.outputText( 'Your ' + Descriptors.vaginaDescript( 0 ) + ' ripples about the coiled intruder as you climax; fem-cum drips down the tentacle and fills the area with your musky scent.  You rear up as a surge of euphoria races through you, managing to overpower the tentacles holding your forelegs down for the briefest of moments.  But even with your forelegs free, the tentacle in your ' + Descriptors.vaginaDescript( 0 ) + ' remains, rippling with waves of seed that spray inside you in massive, hot globules.  The sticky substance flooding your love canal pushes you over the edge and you orgasm again, spraying more as you cry out in pleasure.\n\n', false );
			}
			// has cock, normal cum amount, anus < gaping:
			if( CoC.player.hasCock() && CoC.player.cumQ() < 1500 && CoC.player.ass.analLooseness < 4 ) {
				MainView.outputText( 'Just as you think it\'s over, another tentacle rams into your ' + Descriptors.assholeDescript() + ' and begins roughly massaging your prostate as it swells massively, causing another surge of cum to leave you, and another, and another.', false );
				CoC.player.buttChange( 40, true, true, false );
				MainView.outputText( '  It continues to violate your ass until you black out from exhaustion, the number of loads you\'ve released no longer countable.', false );
				//end (loss)
				CoC.player.orgasm();
			...
			if( CoC.player.hasVagina() ) {
				MainView.outputText( 'Just as you think it\'s over, a tentacle rams into your ' + Descriptors.assholeDescript() + ' and begins to swell massively, causing another surge of girlcum to leave you, and another, and another.', false );
				CoC.player.buttChange( 40, true, true, false );
				MainView.outputText( '  It continues to violate your ass until you black out from exhaustion, the number of times you\'ve orgasmed no longer countable.', false );
				//end (loss)
				CoC.player.orgasm();
				EngineCore.dynStats( 'tou', 1, 'int', -0.5, 'lib', 2, 'sen', 1, 'cor', 0.5 );
				if( CoC.isInCombat() ) {
			...
			if( CoC.player.hasCock() ) {
				MainView.outputText( 'You continue to pump more and more baby batter into the monster until, much to your surprise, it overwhelms the beast and comes surging back out to coat your ', false );
				if( CoC.player.balls > 0 ) {
					MainView.outputText( Descriptors.sackDescript() + ' and ', false );
				}
				MainView.outputText( 'hind legs.  When the creature tries to pull away you step forward awkwardly, forelegs still raised, and continue spraying your copious amount of seed directly into the main mass.  It writhes about beneath you, incapable of doing anything as its soggy, heavily-laden tentacles are now no match for your strength.\n\n', false );
				MainView.outputText( 'Eventually you', false );
				if( CoC.player.balls > 0 ) {
					MainView.outputText( 'r ' + Descriptors.ballsDescriptLight(), false );
				}
				MainView.outputText( ' empty and you turn around to leave, giving the spunk covered mass a swift kick as a reminder of your superiority.', false );
				//end (victory)
				CoC.player.orgasm();
				EngineCore.dynStats( 'tou', 0.5, 'spe', -0.5, 'int', -0.5, 'lib', 1, 'sen', 1, 'cor', 1 );
				CoC.monster.HP = 0;
		...
		if( CoC.player.biggestLactation() >= 3.5 && CoC.player.gender > 0 ) {
			...
			//Pussy
				...
				if( CoC.player.cockTotal() > 0 ) {
					MainView.outputText( Descriptors.vaginaDescript( 0 ) + ' and ' + Descriptors.multiCockDescriptLight(), false );
					MainView.outputText( ' begin ', false );
				}
				//Nope just pussy
				else {
		...
		if( CoC.player.gender === 1 ) {
			...
			MainView.outputText( 'It pins you to the ground easily. You immediately feel a sharp, horrible pain ', false );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 'at the base of your ' + Descriptors.multiCockDescriptLight() + '.', false );
			}
			MainView.outputText( '  You look down to see the end of a thorny tendril impaled in your pelvic region. Fiery pain courses through your veins as you feel the creature inject you with some sort of liquid. As the pain sears through you, ', false );
			if( CoC.player.cockTotal() === 1 ) {
				MainView.outputText( 'your member immediately becomes fully erect and pre-cum leaks liberally from your tip.', false );
			} else {
				MainView.outputText( 'your members immediately become fully erect, pre-cum drizzling from the tips.', false );
			}
		...
		if( CoC.player.gender === 2 ) {
			...
			MainView.outputText( 'Tentacles burst from the mass and bind your arms, legs, and midriff. ', false );
			if( CoC.player.cor < 75 ) {
				MainView.outputText( 'You struggle to break free, but the creature only constricts you further, ensuring your immobility. ', false );
			}
			MainView.outputText( 'A quick flex of the tentacles securing your legs leaves you spreadeagled before the maw of the horror.  ', false );
			if( CoC.player.cor < 75 ) {
				MainView.outputText( 'Fearing for your life, you scream and struggle for help, but only the apathetic sounds of nature respond.', false );
			}
			...
			if( CoC.player.vaginas.length === 1 ) {
				...
				MainView.outputText( 'clits ', false );
			}
			MainView.outputText( 'heat up and begin to swell. The pressure in your breasts is maddening and to your shock, you feel yourself leaking milk.', false );
		}
		if( CoC.player.gender === 3 ) {
			CoC.player.slimeFeed();
			...
			MainView.outputText( 'It pins you to the ground easily. You immediately feel a sharp, horrible pain at the base of your ', false );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 'cocks', false );
			} else {
				MainView.outputText( Descriptors.cockDescript( 0 ), false );
			}
			MainView.outputText( '.  You look down to see the end of a thorny tendril impaled in your pelvic region. Fiery pain courses through your veins as you feel the creature inject you with some sort of liquid. As the pain sears through you, your ', false );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( Descriptors.multiCockDescriptLight() + ' immediately become fully erect and leak pre-cum liberally from their tips.  ', false );
			} else {
				MainView.outputText( 'member immediately becomes fully erect and pre-cum leaks liberally from your tip.  ', false );
			}
			MainView.outputText( '  ' + Utils.Num2Text( (CoC.player.totalNipples()) ) + ' thorny tentacles pierce your nipples, and you feel as if someone shot acid into your tits, which immediately begin to swell.', false );
			...
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 'furious cocks.  ', false );
			} else {
				MainView.outputText( 'furious cock.  ', false );
			}
			MainView.outputText( 'You scream in shock and protest, but your cries fall upon deaf ears. The tentacle descends upon your ', false );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( Descriptors.multiCockDescriptLight() + ', now begging for release, and clamps down around your pubic mound, fully encapsulating your members.  ', false );
			} else {
				MainView.outputText( Descriptors.cockDescript( 0 ) + ', now begging for release, and clamps down upon your pubic mound, fully encapsulating your member.', false );
			}
		}
		//Call page 2!
	...
	TentacleBeastScene.prototype.tentacleRapeContinuation = function() {
		...
		if( CoC.player.gender === 1 ) {
			...
			if( CoC.player.cockTotal() === 1 ) {
				MainView.outputText( 'using a sucking-stroking motion on your helpless ' + Descriptors.multiCockDescriptLight() + '. The swelling of the ass tentacle pressures your prostate in a paradoxically pleasurable and painful manner. You realize, much to your terror, that this beast is MILKING you of your semen!', false );
			} else {
				MainView.outputText( 'using a sucking-stroking motion on your ' + Descriptors.multiCockDescriptLight() + '.  The swelling of the ass tentacle pressures your prostate in a paradoxical pleasurable and painful manner.  You realize, much to your terror, that this beast is MILKING you of your semen!', false );
			}
	...
	TentacleBeastScene.prototype.tentacleRapeContinuationForFemales = function() {
		...
		if( CoC.player.vaginas.length === 1 ) { //single coochie
			...
			if( (CoC.player.findPerk( PerkLib.BigClit ) >= 0 && CoC.player.clitLength > 2) || CoC.player.clitLength > 3 ) {
				...
				MainView.outputText( 'almost four inches in length.  Bruised and sore, you pass into unconsciousness ' );
			}
		} else {
			MainView.outputText( 'Satisfied, the creature drops you smartly and withdraws its limbs from you and lumbers away.  Covered completely in cum, you see that your ' + CoC.player.vaginas.length + ' clits have swollen up to almost four inches in length.  Bruised and sore, you pass into unconsciousness, ' );
		}
		//Not too corrupt
		if( CoC.player.cor < 75 ) {
			MainView.outputText( 'too intoxicated with lust to fume over your violation. ' );
		}//Very corrupt
		else {
		...
		if( CoC.player.clitLength < 7 && CoC.player.clitLength >= 3.5 && CoC.player.findPerk( PerkLib.BigClit ) >= 0 ) {
			...
			if( CoC.player.clitLength < 5 ) {
				MainView.outputText( 'like a tiny cock.' );
			}
			if( CoC.player.clitLength >= 5 && CoC.player.clitLength < 7 ) {
				MainView.outputText( 'like a slick throbbing cock.' );
			}
			if( CoC.player.clitLength >= 7 ) {
				MainView.outputText( 'like a big thick cock.' );
			}
		}
		//Grow clit if smaller than 3.5'
		else if( CoC.player.clitLength < 3.5 ) {
			MainView.outputText( 'In time your clit returns to a more normal size, but retains a bit of extra volume.' );
			CoC.player.clitLength += 0.2;
		}
		//Mention that clit doesn't grow if your big enough.
		else {
			MainView.outputText( 'In time it returns to its normal size, losing all the extra volume.' );
		}
	...
	TentacleBeastScene.prototype.centaurGenderlessRetardation = function() {
		...
		if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00247 ] === 0 || CoC.player.balls === 0 ) {
			...
			if( CoC.player.cor < 50 && CoC.player.lust < 70 ) {
				MainView.outputText( 'dread', false );
			} else {
				MainView.outputText( 'anticipation', false );
			}
			MainView.outputText( ', but stops when it reaches your haunches.  Another starts testing the same area, briefly touching your ' + Descriptors.assholeDescript() + ' but clearly not finding what it\'s looking for.\n\n', false );
			MainView.outputText( '"<i>WHAT THE FUCK IS WRONG WITH YOUR BODY?!</i>" yells out an unnervingly human voice.\n\n', false );
			MainView.outputText( 'Startled past horror, your mouth hangs wide open.\n\n', false );
			MainView.outputText( '"<i>Why the FUCK can\'t I find your juicy bits?</i>" the creature shrills.  "<i>I\'m so hungry I could risk stealing spoo from an army of goblins in heat!</i>"\n\n', false );
			MainView.outputText( 'You stammer out something about having no genitals, not thinking clearly enough to dissemble.\n\n', false );
			MainView.outputText( '"<i>Oh, you think this shit is FUNNY, don\'t you?</i>"  The voice has switched to a mocking tone.  "<i>I know, let\'s wander into the forest and fuck with the hungry creatures who want some nice, nutritious cum!  Let\'s make them work for my amusement!  It\'ll be fucking HILARIOUS!</i>"\n\n', false );
			MainView.outputText( 'A tentacle smacks your ' + Descriptors.buttDescript() + ' hard, and the voice returns to normal.\n\n', false );
			MainView.outputText( '"<i>I just caught a motherfucking HORSE, just to find out you haven\'t got anything for me to eat!  Do you have any idea how fucking hard it is to catch a horse!?</i>"\n\n', false );
			MainView.outputText( 'Feeling kind of ashamed now, you agree that horses are probably pretty hard to catch, but point out you\'re not <i>really</i> a horse, you\'re a centaur.  This is met by a stunned silence, which you, being unable to read the mood of the creature very well, decide to fill with your own voice.  You briefly explain the main differences between horses and centaurs, then mention that you weren\'t exactly <i>willing</i> prey; the monster certainly never asked you if it would be okay to feed from your genitalia, and that perhaps it should reconsider its strategy.\n\n', false );
			MainView.outputText( 'More silence.\n\n', false );
			MainView.outputText( 'Out of nowhere a tentacle slaps you in the face.\n\n', false );
			MainView.outputText( '"<i>FUCK you, you stupid horse!  Why don\'t you grow a pair?  Literally!</i>"\n\n', false );
			MainView.outputText( 'It raises its tentacles and slams them into you as one, dropping you to the ground, unconscious.  With that, the tentacles retract and the monster shambles off into the forest, mumbling something about burning.', false );
		}
		//(Followup scene, if pc has seen above at least once, is unsexed centaur and has balls: -Z)
			...
			MainView.outputText( ', slipping forward to probe between your haunches.  It arrives at and discovers your ' + Descriptors.sackDescript() + ' with some little ceremony, stroking and fondling it.\n\n', false );
			MainView.outputText( '"<i>Now THIS is what I\'m talking about!</i>" the creature\'s eerie voice sings out.  "<i>Daddy needs his medicine!</i>"\n\n', false );
			MainView.outputText( 'The tentacle, now joined by a second, hunts around your ' + Descriptors.ballsDescriptLight() + ', seeking any organs that might serve as a release valve for their contents.  You stare at it as it searches, quite certain you know what\'s coming next.\n\n', false );
			MainView.outputText( '"<i>No, no, no. Where the FUCK is it?</i>" the creature mumbles, frustration spiking the pitch of its voice.\n\n', false );
			MainView.outputText( 'You glibly explain that though you do in fact have \'a pair\', as requested, you\'re still very much genderless, without any sexual organs.\n\n', false );
			MainView.outputText( 'The tentacles cease movement as their owner digests your words; it begins to shake visibly, shedding leaf-litter as it does.\n\n', false );
			MainView.outputText( '"<i>You... literal-minded... PRICK!</i>" it howls, rounding on you with furious venom and making you flinch.  "<i>First of all, you\'re not GENDERLESS, you\'re UNSEXED!  Gender identity rolls up social and behavioral factors like masculine or feminine mannerisms, dress, and domestic roles; the only thing YOU are less is anything remotely USEFUL between your legs!  If you\'re going to be PEDANTIC, try at least to be right!</i>"\n\n', false );
			MainView.outputText( 'You quail, surprised at misguessing the character of its reaction.\n\n', false );
			MainView.outputText( '"<i>SECOND of all,</i>" it continues, "<i>it occurs to me that, in your misguided zeal, you\'ve forgotten that you, a: have BALLS, and b NO WAY to close your legs!  WHICH BRINGS ME TO C TO THE GROIN!</i>"\n\n', false );
			MainView.outputText( 'Your eyes bulge out as one of the feelers which had been still during your argument pulls away from your ' + Descriptors.sackDescript() + ' and then returns with a sharp slap; as your vision pinks over under the wave of nausea, the creature releases your legs and you collapse into what can only be assumed is a centaur fetal position.\n\n', false );
			MainView.outputText( '"<i>Q.E.D., MOTHERFUCKER!</i>" it shouts, gesticulating in the air wildly with its tentacles as it turns and clumps back into the dense brush.', false );
		}
	...
	TentacleBeastScene.prototype.genderlessHilarityForNagaKenDolls = function() {
		...
		MainView.outputText( '"<i>I\'m sorry, maybe I just worded the question badly.  Um, where do you keep your penis... esss and or vagina... ssss.</i>"  The words are followed up by prolonged hisses that may or may not represent the usual attempt to transmute one language to another by tacking new suffixes on.\n\n', false );
		MainView.outputText( 'Sensing an opportunity to get out of this situation, you respond with your own series of hisses and hand gestures as if to say you have no idea what the beast wants.  It responds with a sigh and you\'re released from its grip, landing on the ground in a bit of a heap.\n\n', false );
	...
	TentacleBeastScene.prototype.tentacularGenderGooTimes = function() {
		...
		MainView.outputText( '"<i>Uh huh... that so?  Well, so long as you have some substantial naughty bits, I\'ll be happy.</i>"\n\n', false );
		MainView.outputText( 'There\'s an awkward silence.\n\n', false );
		MainView.outputText( '"<i>You haven\'t got anything, have you?</i>"\n\n', false );
		MainView.outputText( 'You shake your head.\n\n', false );
		MainView.outputText( '"<i>Well, fuck.</i>"  A tentacle pokes you and you\'d guess the beast is watching you jiggle as it chuckles.  "<i>Maybe this isn\'t a total waste. I wonder, what do you taste like?</i>"\n\n', false );
		MainView.outputText( 'One of the larger tentacles extends and latches onto your base, its anus-like opening sucking gently at your gooey mass.  There follows a brief moment where you\'re not really afraid of the situation, but are instead mildly curious yourself what you taste like.\n\n', false );
