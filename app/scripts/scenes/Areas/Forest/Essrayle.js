...
angular.module( 'cocjs' ).run( function( MainView, SceneLib, $log, kFLAGS, Descriptors, ConsumableLib, CoC, Utils, EngineCore, AppearanceDefs ) {
	...
	Essrayle.prototype.essrayleMeetingI = function() {
		...
		if( CoC.flags[ kFLAGS.MET_ESSY ] === 0 ) {
			...
			EngineCore.outputText( '.  "<i>You must be one of those adventurers I\'ve been hearing about, am I right?</i>"' );
	...
	Essrayle.prototype.plantsForMe = function() {
		...
		EngineCore.outputText( ' as her hand begins to circle your chest.  "<i>Though you, that\'s another story.</i>"  She grins seductively as the air around you seems to thicken with a gentle pink mist.  Out of instinct, you look around, trying to discern the source - could she be one of the demons?' );
	...
	Essrayle.prototype.plantsForMe2 = function( BE ) {
		...
		//Yes
		if( BE === 1 ) {
			EngineCore.outputText( 'Essy grins, nodding.  "<i>Precisely.  I don\'t doubt they\'ll be even more fun given enough exposure here.</i>"\n\n' );
		}//If No=
		else if( BE === 2 ) {
			EngineCore.outputText( '"<i>Heh, suit yourself.</i>"\n\n' );
		}
		...
		if( BE === 1 ) {
			...
			EngineCore.outputText( ' been swallowed up as well.' );
	...
	Essrayle.prototype.hasPussyFeedEssy = function( newPage ) {
		...
		EngineCore.outputText( '.  With some effort, you take the plant girl\'s peak to its leaf-green hilt, the soft skin of her pliant breast squeezed against your [hips], teat-like nipple a pulsing cock-like shaft inside your [vagina].' );
	...
	Essrayle.prototype.noGimmeGiantNipplesEssy = function() {
		MainView.clearOutput();
		EngineCore.outputText( 'She shrugs her shoulders, giving one bundle of milk-drooling nipples a soft squeeze.  "<i>Eh, to each their own, I suppose.</i>"' );
		this.essyRewardEpilogueOUTTIES();
	};
	//[Either choice]
