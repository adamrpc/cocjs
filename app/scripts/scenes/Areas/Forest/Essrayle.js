...
angular.module( 'cocjs' ).run( function( MainView, SceneLib, kFLAGS, Descriptors, ConsumableLib, CoC, Utils, EngineCore, AppearanceDefs ) {
	...
	Essrayle.prototype.essrayleMeetingI = function() {
		...
		if( CoC.flags[ kFLAGS.MET_ESSY ] === 0 ) {
			...
			if( CoC.player.cor < 33 ) {
				MainView.outputText( 'hastily looking away from her nude body' );
			} else if( CoC.player.cor < 66 ) {
				MainView.outputText( 'looking away out of courtesy' );
			} else {
				MainView.outputText( 'letting your eyes roam over her as you search for even more depraved features on her nubile body' );
			}
			MainView.outputText( '.' );
			...
			MainView.outputText( '.  "<i>You must be one of those adventurers I\'ve been hearing about, am I right?</i>"' );
	...
	Essrayle.prototype.plantsForMe = function() {
		...
		if( CoC.player.breastRows[ 0 ].breastRating >= 1 ) {
			...
			if( CoC.player.bRows() > 1 ) {
				MainView.outputText( ' of your uppermost pair' );
			}
			MainView.outputText( '.  You moan, and this only seems to encourage her, as she takes your nipples between thumb and forefinger, twisting and turning them this way and that.  She giggles at how your moans seem to fluctuate by coincidence, continuing to tease them' );
			if( CoC.player.bRows() > 1 ) {
				MainView.outputText( ', steadily working over the multitude of fleshy mounds on your body' );
			}
			MainView.outputText( '.' );
			if( CoC.player.hasFuckableNipples() ) {
				MainView.outputText( '  Her fingers play with your nipples and press gently to tease at them.  She seems quite surprised as they sink in with minimal effort.  A grin spreads over her face as she begins to steadily thrust those digits in and out of your nipples, fingers swirling about the interiors all the while.' );
			}
	...
	Essrayle.prototype.plantsForMe2 = function( BE ) {
		...
		if( CoC.player.hasCock() && CoC.player.hasVagina() ) {
			MainView.outputText( 'find the dual sexes there.  Another hand joins the first, one gripping and tugging on your hardening cock as the other cups and squeezes over your pussy.' );
		} else if( CoC.player.hasCock() ) {
			MainView.outputText( 'circle about ' );
			...
			MainView.outputText( ', beginning to squeeze and slide about its length until your nursing is interrupted by moans of your own delight' );
		} else {
			MainView.outputText( 'stroke about the puffy, now-inflamed lips of your own flower, taking the lips between two fingers and stroking up and down across their length fully before switching to your moist core.' );
		}
		...
		MainView.outputText( ' and pulls you close, forcing your [legs] against her bulb.  They sink in slightly as you go down, reluctantly relinquishing her nipple from your mouth.  As it springs free, a thick glob of sap splatters over your lips and nose.  Essy giggles softly as she wipes them off with a hand.' );
		...
		MainView.outputText( 'and [legs], ' );
		if( !CoC.player.isTaur() ) {
			MainView.outputText( 'lifting you slowly into the air' );
		} else {
			MainView.outputText( 'forcing you to rear up' );
		}
		MainView.outputText( '.  Her lower hands grasp you, helping to raise you up until you\'re at the proper height, whence she abruptly plants her lips upon your crotch.' );
		//Cock=
		if( CoC.player.hasCock() ) {
			...
			if( CoC.player.breastRows[ 0 ].breastRating >= 1 ) {
				MainView.outputText( ' and ' + CoC.player.breastCup( 0 ) + ' breasts' );
			}
			MainView.outputText( '.  The opaque cups pump and suck, Essy humming to herself as the base of her tongue continues to skillfully tease over every hot spot near the [cockHead biggest] of your [cock biggest], her length slithering about it skillfully.' );
		}
		//(Pussy=
		else {
		...
		if( CoC.player.biggestTitSize() >= 1 ) {
			...
			if( CoC.player.lactationQ() >= 400 ) {
				MainView.outputText( 'your milk and ' );
			}
			MainView.outputText( 'your pleasure.  In time, their pumping increases in force and tempo just as her lips and tongue do.' );
		}
		...
		if( BE === 1 ) {
			...
			MainView.outputText( ' been swallowed up as well.' );
			...
			if( CoC.player.bRows() > 1 ) {
				MainView.outputText( '  She eventually dislodges with a wet pop, setting your new breasts to wobbling about as she moves down your body, repeating the gestures upon each set of breasts in return.' );
			}
			...
			if( CoC.player.bRows() > 1 ) {
				MainView.outputText( ', the top pair being largest, descending in size with each subsequent pair' );
			}
			MainView.outputText( '.' );
	...
	Essrayle.prototype.feedTrappedEssy = function() {
		MainView.clearOutput();
		MainView.outputText( 'Well, it\'s not like she\'s going anywhere right now, and she probably would love a good fuck from a friend anyway.' );
	...
	Essrayle.prototype.hasCockFeedEssy = function( newPage ) {
		...
		if( CoC.player.biggestCockLength() < 20 ) {
			MainView.outputText( 'her lower pair of wobbling udders' );
		} else {
			MainView.outputText( 'both sets of milk-bloated breasts' );
		}
		MainView.outputText( ' faster and faster, the hot slapping of her bountiful bosom against your [cock]' );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( ' and your swollen, heavy balls against her fecund belly' );
		}
		MainView.outputText( ' setting an inspiring drum beat to your feverish pace.' );
	...
	Essrayle.prototype.acceptEssyPrizes = function() {
		...
		if( CoC.player.biggestTitSize() <= 1 ) {
			MainView.outputText( 'four bulging tits erupt from your chest' );
		...
		//if two breasts:
			...
			MainView.outputText( 'two more bulging tits erupt from your chest' );
			if( CoC.player.breastRows[ 0 ].breastRating < 4 ) {
				MainView.outputText( ' while your top row expands' );
				CoC.player.breastRows[ 0 ].breastRating = 4;
			}
			if( CoC.player.bRows() === 1 ) {
				CoC.player.createBreastRow();
		...
		//if four+ breasts:
			...
			MainView.outputText( 'your [fullChest] swell larger and larger under the expanding magic of the spell' );
	...
	Essrayle.prototype.noGimmeGiantNipplesEssy = function() {
		MainView.clearOutput();
		MainView.outputText( 'She shrugs her shoulders, giving one bundle of milk-drooling nipples a soft squeeze.  "<i>Eh, to each their own, I suppose.</i>"' );
		this.essyRewardEpilogueOUTTIES();
	};
	//[Either choice]
	Essrayle.prototype.essyRewardEpilogueOUTTIES = function() {
	...
	Essrayle.prototype.askMotherToReleaseEssy = function() {
		...
		if( CoC.flags[ kFLAGS.SAND_WITCHES_COWED ] === 0 ) {
			MainView.outputText( 'You point out that the witches have a friend of yours trapped here with magic and you\'d like her released.' );
			...
			MainView.outputText( 'You command the Sand Mother to release your friend, this.Essrayle from her magical bondage.  The sorcerous queen looks about to protest, but after a brief pause, she admits, "<i>Fine, I\'ll see her released.  You can go pick her up.</i>"  She practically spits the last sentence.' );
			MainView.outputText( '\n\nThat\'ll do.' );
		}
