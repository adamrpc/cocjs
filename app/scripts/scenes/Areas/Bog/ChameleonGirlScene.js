...
angular.module( 'cocjs' ).run( function( CoC, kFLAGS, Utils, StatusAffects, PregnancyStore, EngineCore, Combat, ChameleonGirl, EventParser, Descriptors, ConsumableLib ) {
	...
	/*You can ignore this stuff, just sets a random color to the chameleon girl at each encounter
	 ...
	 /*FEN NOTE SKINADJ IN PLACE OF STRIPECOLOR*/
	//ENCOUNTER TEXT (Introduction; First Time) (Z edited)
	...
	ChameleonGirlScene.prototype.encounterChameleon = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.TIMES_MET_CHAMELEON ] === 0 ) {
			...
			if( CoC.getInstance().player.tallness > 76 ) {
			...
			//ENCOUNTER INTRO; REPEAT (Z edited)
			EngineCore.outputText( 'You work your way through the dense foliage of the bog, pushing aside branches and slogging through the thick mud in search of something new.  Feeling exhausted, you slow down and look for a place to rest; a small clearing with shallow water and firmer ground seems to fit the bill, and you sit back against a tree to catch your breath.  You\'re so soaked by now that you hardly notice the murky water beneath you and the slick mud on the trunk seeping into your ' + CoC.getInstance().player.armorName + '.' );
			EngineCore.outputText( '\n\nA sudden scrape sounds behind you!  You spin around in time to see the familiar shape of a chameleon girl peeling off the trees.  She approaches you with a menacing glare in her eyes, growling "<i>This is my bog!  Get out now or... or face the consequences!</i>"  You raise your ' + CoC.getInstance().player.weaponName + ' and prepare to defend yourself.' );
		}
		CoC.getInstance().flags[ kFLAGS.TIMES_MET_CHAMELEON ]++;
	...
	ChameleonGirlScene.prototype.loseToChameleonGirl = function() {
		...
		if( CoC.getInstance().player.lust > 99 ) {
			EngineCore.outputText( 'Losing control to your own growing arousal, you fall to your knees and desperately start working to get at your needy body beneath your ' + CoC.getInstance().player.armorName + '.' );
		}
		//Lose by HP
		else {
			EngineCore.outputText( 'Too weak to keep fighting, you stagger back and fall to your knees.' );
		}
		//Savin's Note what the fuck. Bracket off your bits and pieces and LABEL YOUR SCENES RIGHT. -Savin
	...
	ChameleonGirlScene.prototype.loseToChameleonWithCockAnBallsAnCunt = function() {
		...
		if( CoC.getInstance().player.cockArea( x ) < 40 ) {
			EngineCore.outputText( ' and occasionally lifting it to take the shaft between two dexterous toes and work it more delicately' );
		}
		EngineCore.outputText( '.  She blushes as she watches your breaths turn to pants from the pleasure.' );
	...
	ChameleonGirlScene.prototype.dudesLoseToChamChams = function() {
		var x = Utils.rand( CoC.getInstance().player.totalCocks() );
		EngineCore.spriteSelect( 89 );
		EngineCore.outputText( '\n\nRecognizing that you\'re no danger, her skin shifts back into its natural ' + CoC.getInstance().monster.skinTone + ' coloration as she walks towards you.  She swings her hips seductively as she splashes slowly through the water and licks her lips with a loud smack of her tongue, then reaches out and forcefully pulls your ' + CoC.getInstance().player.armorName + ' above your waist before you can even try to resist her advances, and pushes you back against a tree.  With a few quick pulls of fabric she\'s stripped her own clothes, and she tosses her thong over a low-hanging branch.  The other band of silk she uses to quickly bind your wrists.  Were you in any shape to fight back you could pull free, but you\'re too ' );
		if( CoC.getInstance().player.lust > 99 ) {
			EngineCore.outputText( 'overwhelmed by lust' );
		} else {
			EngineCore.outputText( 'battered' );
		}
		EngineCore.outputText( ' to even rip the makeshift binding.' );
		EngineCore.outputText( '\n\nShe makes short work of your ' + CoC.getInstance().player.armorName + ' in her haste to see her prize, then stands up and, still looking down, lets her tongue fall out of her mouth and find its way to [oneCock].  In an amazing display of control, she manages to wrap the warm pink appendage around your man-meat, sliding the slick length of her warm tongue up and down your shaft almost as if she were massaging you.  You shudder softly as she tickles the underside of your cock with just the tip of her tongue, trailing sticky saliva around its head.  Your member is soon erect and throbbing with need; satisfied, she slurps the slender length of muscle back into her mouth.' );
	...
	ChameleonGirlScene.prototype.defeatChameleonGirl = function() {
		...
		if( CoC.getInstance().monster.lust > 99 ) {
			EngineCore.outputText( 'Unable to control her arousal, the chameleon girl collapses to her knees and begins masturbating underneath her thong, having lost all capacity to fight you; she moans and throws her head back as her hand splashes in and out of the water she\'s kneeling in.  Her skin returns to its usual ' + CoC.getInstance().monster.skinTone + ' and then keeps going, shifting closer and closer to pink as her moans increase in both volume and volubility.' );
			if( CoC.getInstance().player.lust >= 33 ) {
				EngineCore.outputText( '  Do you help the horny girl to get you both off?  Or maybe you could punish her bad attitude with something from your bag...' );
			}
		}
		//-Win by HP
		else {
			EngineCore.outputText( 'Too weak to continue fighting, the chameleon girl drops to her knees, exhausted.  Her skin returns to its usual ' + CoC.getInstance().monster.skinTone + ', unable to maintain the camouflage.' );
			if( CoC.getInstance().player.lust >= 33 ) {
				EngineCore.outputText( '  Do you use the girl in her weakened state to get yourself off?  Or maybe you could punish her bad attitude with an item from your bags...' );
			}
		}
		...
		//let PC use item
		if( (CoC.getInstance().player.hasItem( ConsumableLib.SUCMILK ) || CoC.getInstance().player.hasItem( ConsumableLib.P_S_MLK )) && CoC.getInstance().player.hasCock() ) {
			item = this.useAnItemOnTheChamcham;
		} else if( CoC.getInstance().player.hasItem( ConsumableLib.SENSDRF ) && (CoC.getInstance().player.hasItem( ConsumableLib.L_DRAFT ) || CoC.getInstance().player.hasItem( ConsumableLib.F_DRAFT )) ) {
			item = this.useAnItemOnTheChamcham;
		}
	...
	ChameleonGirlScene.prototype.fuckDatChameleonAsACoolGuyGirlHerm = function() {
		...
		if( CoC.getInstance().player.cor < 50 ) {
			EngineCore.outputText( 'and guiltily ' );
		}
		EngineCore.outputText( 'roll her onto her back and get up to collect your things, rinsing yourself with the water and shaking the mud out before putting your ' + CoC.getInstance().player.armorName + ' back on and heading back to camp.' );
		CoC.getInstance().player.orgasm();
		Combat.cleanupAfterCombat();
	};
	//-Male (Z edited)
	...
	ChameleonGirlScene.prototype.manFucksChameleonWithBiggishWang = function() {
		...
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( 'your ' + Descriptors.ballsDescriptLight() + ' clench and ' );
		}
		EngineCore.outputText( 'the semen erupts from the end of your ' + Descriptors.cockDescript( x ) + ' spilling out hot spunk over her folded tongue.  She gasps audibly and arches her back as the thick ropes of your jizz coat her tongue, shoving her long fingers as far as she possibly can into her dripping pussy, and flashes so brightly pink she nearly glows.' );
		if( CoC.getInstance().player.cumQ() > 750 ) {
			EngineCore.outputText( '  The torrent of cum bursting from your cock just doesn\'t stop, and her eyes roll back into her head as your semen coats her face and runs off her tongue, drenching her breasts and belly in white.  Her head falls back and her tongue goes limp as she\'s sent into ecstasy by the flood.' );
		}
		...
		if( CoC.getInstance().player.cumQ() > 750 ) {
			EngineCore.outputText( '  Even the murky water around her is turned white as the pools of your cum on her body run off into it.' );
		}
		EngineCore.outputText( '  Satisfied, you put your ' + CoC.getInstance().player.armorName + ' back on and head back to camp.' );
	...
	ChameleonGirlScene.prototype.femaleHasWinSexWithChamCham = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 89 );
		EngineCore.outputText( 'You shed your ' + CoC.getInstance().player.armorName + ' and approach the fallen chameleon girl' );
		if( CoC.getInstance().monster.lust > 99 ) {
			EngineCore.outputText( ', her moans of pleasure growing more desperate as you approach' );
		}
		EngineCore.outputText( '.  You look her up and down, following the smooth curves of her thin body with your eyes.  Squatting down beside her, you take her weak body by the shoulders in one arm and trail your fingertips lightly up her torso with the other, feeling the soft skin of her belly.' );
		if( CoC.getInstance().monster.lust > 99 ) {
			EngineCore.outputText( '  She coos lustily at the sensation.' );
		} else {
			EngineCore.outputText( '  Her eyes widen, and she looks from your hand to your face and back again.' );
		}
		EngineCore.outputText( '  When your hand reaches the silk cloth covering her chest, you pull it off with a light tug and cast it off to the side.  You cup her small breast in your palm, and as you squeeze it her skin tone changes perceptibly, flashing to a much brighter shade of the colors it was.' );
		EngineCore.outputText( '\n\nCurious, you take her nipple between your index finger and thumb, pinching and twisting and pulling on it, eliciting a series of sharp gasps and squeals.  She arches her back into your supporting arm and her skin flashes wildly, long stripes of color shifting across her body with your every tweak.  You find her complete inability to hide what she\'s feeling amusing, but you\'re much too turned on to just sit there and play with her.  Releasing her breast, you take her arm and guide her hand to your own slick cunt.  She gets the idea and begins to work her fingers lightly in and out of your wet folds.  You squeeze her close and breathe in deep, enjoying the sensation of her long fingers plunging in and out of your ' + Descriptors.vaginaDescript( 0 ) + '.  It feels so good to have her touching you, and as you bask in the pleasure you lose your balance.  The two of you topple over backwards, landing in the shallow water with a splash.  Unsure what to do, the chameleon girl freezes up and looks almost apologetically into your eyes.  You laugh quietly at her confusion and guide her fingers back into motion with your hand.' );
		EngineCore.outputText( '\n\nNot ' );
		if( CoC.getInstance().player.cor < 50 ) {
			EngineCore.outputText( 'one to be selfish' );
		...
		if( CoC.getInstance().player.cor < 50 ) {
			EngineCore.outputText( 'and somewhat guiltily ' );
		}
		EngineCore.outputText( 'roll her onto her back and get up to collect your things, shaking out the mud and water before putting your ' + CoC.getInstance().player.armorName + ' back on and heading to camp.' );
		//send player back to camp, reset hours since cum, add gems and time
		CoC.getInstance().player.orgasm();
		Combat.cleanupAfterCombat();
	};
	...
	ChameleonGirlScene.prototype.useAnItemOnTheChamcham = function() {
		...
		//(Display Options Draft] [Succubi Milk](PC must have cock) [Lust&SensDrafts]
		//optionz go herez
		var milk = null;
		var drafts = null;
		if( (CoC.getInstance().player.hasItem( ConsumableLib.SUCMILK ) || CoC.getInstance().player.hasItem( ConsumableLib.P_S_MLK )) && CoC.getInstance().player.hasCock() ) {
		...
		if( CoC.getInstance().player.hasItem( ConsumableLib.SENSDRF ) && (CoC.getInstance().player.hasItem( ConsumableLib.L_DRAFT ) || CoC.getInstance().player.hasItem( ConsumableLib.F_DRAFT )) ) {
	...
	ChameleonGirlScene.prototype.giveTheChameleonASuccubiMilk = function() {
		...
		if( CoC.getInstance().player.hasLongTail() > 0 ) {
			EngineCore.outputText( ' with your tail,' );
		} else {
			EngineCore.outputText( ' by straddling her,' );
		}
		EngineCore.outputText( ' your cock aimed for her pillowy tits.  She pushes and squirms beneath you, but cannot get enough leverage to move you.  You take her breasts in your hands and shimmy up her torso towards them, squeezing them around your ' + Descriptors.cockDescript( x ) + '.' );
	...
	ChameleonGirlScene.prototype.doseDatChameleonWithLustAndSensitivityDrafts = function() {
		...
		EngineCore.outputText( '\n\nToo turned on to just sit there, she falls forward, catching herself on you, then looks up, pleading with her eyes.  You laugh mirthlessly at her grovelling form, and tell her to beg if she wants you.  "<i>Please!</i>" she squeals, her eyes watering, "<i>I need this!</i>"  She tugs insistently at your ' + CoC.getInstance().player.armorName + ', begging for you to fill her and crying out that she can\'t stand to feel so empty.  You grin, and wonder if the sensitivity draft has started to take effect.  Two fingers run lightly up her forearm produce shivers at the obviously amplified sensation, and a tweak of her nipple elicits a shrill shriek.  "<i>Why does it feel so good?!</i>"  She puts her face into your lap and begs, "<i>I don\'t even know what\'s going on, please, just fuck me, pleasepleaseplease...</i>" trailing off as she grinds her pussy into you.' );
		...
		if( !CoC.getInstance().player.hasCock() || CoC.getInstance().player.cockArea( CoC.getInstance().player.smallestCockIndex() ) > CoC.getInstance().monster.vaginalCapacity() ) {
		...
		CoC.getInstance().player.consumeItem( ConsumableLib.SENSDRF );
