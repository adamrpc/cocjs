...
angular.module( 'cocjs' ).run( function( SceneLib, CoC, kFLAGS, Utils, EngineCore, Combat, ChameleonGirl, EventParser, Descriptors, ConsumableLib ) {
	...
	ChameleonGirlScene.prototype.encounterChameleon = function() {
		...
		if( CoC.flags[ kFLAGS.TIMES_MET_CHAMELEON ] === 0 ) {
			...
			//ENCOUNTER INTRO; REPEAT (Z edited)
			EngineCore.outputText( 'You work your way through the dense foliage of the bog, pushing aside branches and slogging through the thick mud in search of something new.  Feeling exhausted, you slow down and look for a place to rest; a small clearing with shallow water and firmer ground seems to fit the bill, and you sit back against a tree to catch your breath.  You\'re so soaked by now that you hardly notice the murky water beneath you and the slick mud on the trunk seeping into your ' + CoC.player.armorName + '.' );
	...
	ChameleonGirlScene.prototype.loseToChameleonGirl = function() {
		...
		if( CoC.player.lust > 99 ) {
			EngineCore.outputText( 'Losing control to your own growing arousal, you fall to your knees and desperately start working to get at your needy body beneath your ' + CoC.player.armorName + '.' );
		}
		//Lose by HP
		else {
	...
	ChameleonGirlScene.prototype.dudesLoseToChamChams = function() {
		...
		EngineCore.outputText( '\n\nShe makes short work of your ' + CoC.player.armorName + ' in her haste to see her prize, then stands up and, still looking down, lets her tongue fall out of her mouth and find its way to [oneCock].  In an amazing display of control, she manages to wrap the warm pink appendage around your man-meat, sliding the slick length of her warm tongue up and down your shaft almost as if she were massaging you.  You shudder softly as she tickles the underside of your cock with just the tip of her tongue, trailing sticky saliva around its head.  Your member is soon erect and throbbing with need; satisfied, she slurps the slender length of muscle back into her mouth.' );
	...
	ChameleonGirlScene.prototype.defeatChameleonGirl = function() {
		...
		//-Win by HP
			...
			EngineCore.outputText( 'Too weak to continue fighting, the chameleon girl drops to her knees, exhausted.  Her skin returns to its usual ' + CoC.monster.skinTone + ', unable to maintain the camouflage.' );
			if( CoC.player.lust >= 33 ) {
		...
		//let PC use item
		if( (CoC.player.hasItem( ConsumableLib.SUCMILK ) || CoC.player.hasItem( ConsumableLib.P_S_MLK )) && CoC.player.hasCock() ) {
	...
	ChameleonGirlScene.prototype.fuckDatChameleonAsACoolGuyGirlHerm = function() {
		...
		EngineCore.outputText( 'roll her onto her back and get up to collect your things, rinsing yourself with the water and shaking the mud out before putting your ' + CoC.player.armorName + ' back on and heading back to camp.' );
		CoC.player.orgasm();
		Combat.cleanupAfterCombat();
	};
	//-Male (Z edited)
	...
	ChameleonGirlScene.prototype.manFucksChameleonWithBiggishWang = function() {
		...
		EngineCore.outputText( '  Satisfied, you put your ' + CoC.player.armorName + ' back on and head back to camp.' );
	...
	ChameleonGirlScene.prototype.femaleHasWinSexWithChamCham = function() {
		...
		EngineCore.outputText( 'roll her onto her back and get up to collect your things, shaking out the mud and water before putting your ' + CoC.player.armorName + ' back on and heading to camp.' );
		//send player back to camp, reset hours since cum, add gems and time
		CoC.player.orgasm();
		Combat.cleanupAfterCombat();
	};
	...
	ChameleonGirlScene.prototype.doseDatChameleonWithLustAndSensitivityDrafts = function() {
		...
		if( !CoC.player.hasCock() || CoC.player.cockArea( CoC.player.smallestCockIndex() ) > CoC.monster.vaginalCapacity() ) {
		...
		CoC.player.consumeItem( ConsumableLib.SENSDRF );
