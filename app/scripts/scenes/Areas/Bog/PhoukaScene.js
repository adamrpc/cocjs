...
angular.module( 'cocjs' ).run( function( $rootScope, CoC, kFLAGS, Utils, StatusAffects, PregnancyStore, EngineCore, Phouka, Combat, ConsumableLib, OnLoadVariables ) {
	...
	PhoukaScene.prototype.timeChange = function() {
		if( CoC.getInstance().player.statusAffectv1( StatusAffects.PhoukaWhiskeyAffect ) > 0 ) {
			CoC.getInstance().player.addStatusValue( StatusAffects.PhoukaWhiskeyAffect, 1, -1 ); //Count down hours until player is not drunk
			if( CoC.getInstance().player.statusAffectv1( StatusAffects.PhoukaWhiskeyAffect ) <= 0 ) {
				ConsumableLib.P_WHSKY.phoukaWhiskeyExpires( CoC.getInstance().player );
				return true;
			}
		}
		return false;
	...
	PhoukaScene.prototype.phoukaNameText = function( known, unknown ) { //Helper function, For any text that depends on whether or not the player knows what to call a phouka
		return (CoC.getInstance().flags[ kFLAGS.PHOUKA_LORE ] === 0 ? unknown : known);
	};
	...
	PhoukaScene.prototype.phoukaEncounter = function() { //General entry point for everything except halloween special encounter
		...
		var choiceChance = 0;
		if( CoC.getInstance().flags[ kFLAGS.PHOUKA_ENCOUNTER_STATUS ] === 0 ) { //Guarantee first Phouka encounter is with faerie fire
			CoC.getInstance().flags[ kFLAGS.TREACLE_MINE_YEAR_DONE ] = OnLoadVariables.date.fullYear - 1; //If you've never encountered phoukas before then we can safely set all this stuff here
			CoC.getInstance().flags[ kFLAGS.BIRTHS_PHOUKA ] = 0;
			CoC.getInstance().flags[ kFLAGS.BIRTHS_FAERIE ] = 0;
	...
	PhoukaScene.prototype.phoukaFaerieFire = function() { //In old Irish legends the Phouka are responsible for Will-o’-the-wisps. In the deep woods it’s Kitsune, but in the bog it’s a Phouka
		...
		if( CoC.getInstance().flags[ kFLAGS.PHOUKA_ENCOUNTER_STATUS ] !== 0 ) {
		...
		EngineCore.menu();
		if( CoC.getInstance().flags[ kFLAGS.PHOUKA_ENCOUNTER_STATUS ] !== 0 || CoC.getInstance().player.hasKeyItem( 'Traveler\'s Guide' ) >= 0 ) {
	...
	PhoukaScene.prototype.phoukaFaerieFireLeave = function() {
		...
		EngineCore.doNext( CoC.getInstance().scenes.camp.returnToCampUseOneHour );  //Return to camp, 1 hour used
	};
	PhoukaScene.prototype.phoukaStuck = function() {
		EngineCore.clearOutput();
		var hasAlcohol = CoC.getInstance().player.hasItem( ConsumableLib.P_WHSKY, 1 );
	...
	PhoukaScene.prototype.phoukaStuckOfferWhiskey = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.PHOUKA_LORE ] === 0 ) {
			CoC.getInstance().flags[ kFLAGS.PHOUKA_LORE ] = 1;
		} //Now you know what to call them
		CoC.getInstance().player.consumeItem( ConsumableLib.P_WHSKY, 1 );
	...
	PhoukaScene.prototype.phoukaTalk = function() {
		...
		if( CoC.getInstance().player.level < 10 ) {
			...
			EngineCore.outputText( ' if he really wants a taste of what you\'re going to give him.' );
		}
		if( CoC.getInstance().player.cor + Utils.rand( 125 ) <= 110 ) { //Players with 0% corruption have a 12% chance that the Phouka is willing to talk, players with 100% corruption have a 92% chance
			//The phouka is not in the mood for talk, start the fight gagged as punishment for trying to talk
			EngineCore.outputText( '  You open your mouth to say something more to the ' );
			if( CoC.getInstance().player.cor < 34 ) {
				EngineCore.outputText( 'vile little monster' );
			} else if( CoC.getInstance().player.cor < 67 ) {
				EngineCore.outputText( 'perverted ' + this.phoukaName() );
			} else {
				EngineCore.outputText( 'fuckable little ' + this.phoukaName() );
			}
			EngineCore.outputText( ' but he must not feel like listening.  He launches a ball of black mud at your face, sealing your mouth with sticky and [if (corruption <= 50)ewww -][if (corruption > 50)mmmm -] salty muck.' );
			Combat.startCombat( new Phouka( this.phoukaName() ) );
			CoC.getInstance().player.createStatusAffect( StatusAffects.WebSilence, 0, 0, 0, 0 );
		} else { //The phouka would rather talk
			EngineCore.outputText( '\n\nThe ' + this.phoukaName() );
			switch( CoC.getInstance().player.level < 10 ? Utils.rand( 8 ) : Utils.rand( 12 ) ) { //Low level characters don't get the chance to be offered whiskey
				case 0:
				case 1:
				case 2:
				case 3:
				...
				case 6: //Offers you a chance to submit, which leads to the various defeat sex scenes
					...
					if( CoC.getInstance().player.hasVagina() ) {
						...
						EngineCore.outputText( '  If it ain\'t then tough shite, cause that\'s what I want.”</i>' );
					}
					EngineCore.menu();
				...
				case 8: //Decides he doesn't want you
					EngineCore.outputText( ' takes a long look at you.  Finally he grins and tells you <i>“You know what?  You\'re not quite what I was lookin for.  Us phooka got varied tastes, ya know.”</i>  Then the phouka lazily flies off, whistling some strange tune to himself.\n\nYou wriggle against your bonds and after a few minutes you are free once more.  You decide that\'s enough excitement for now and head back to camp.' );
					if( CoC.getInstance().flags[ kFLAGS.PHOUKA_LORE ] === 0 ) {
						CoC.getInstance().flags[ kFLAGS.PHOUKA_LORE ] = 1;
					} //Now you know what to call them
					EngineCore.doNext( CoC.getInstance().scenes.camp.returnToCampUseOneHour );  //Return to camp, 1 hour used
	...
	PhoukaScene.prototype.phoukaDrinkWhilePregnant = function( playerOfferedTheBooze ) {
		if( (CoC.getInstance().player.pregnancyIncubation <= 100) || (CoC.getInstance().player.buttPregnancyIncubation <= 100) ) { //Pregnancy is obvious to the phouka
		...
		var acceptable = ConsumableLib.P_WHSKY.phoukaWhiskeyAcceptable( CoC.getInstance().player );
		if( acceptable < 0 ) {
			EngineCore.outputText( '\n\nYou think about the unborn child' + (acceptable <= -3 ? 'ren' : '') + ' you\'re carrying.' );
			if( acceptable === -4 ) {
				EngineCore.outputText( '  One of them seems to crave the whiskey, but the other is a more normal child.' );
			}
			EngineCore.outputText( '  Sure, some of the elders around the village said a beer or some light cider was all right for a woman heavy with child, but you doubt that whiskey would be acceptable.  You politely refuse the drink, hoping the phouka won\'t be offended.' );
		} else {
			EngineCore.outputText( '\n\nYou put one hand on your belly, about to refuse the drink.  Then you realize that yes, this is exactly what your ' );
			if( acceptable <= 2 ) {
				EngineCore.outputText( 'baby needs.  Some light kicks inside your ' + (acceptable === 2 ? 'distended colon' : 'womb') + ' suggest that your child knows' );
			} else {
				EngineCore.outputText( 'babies need.  Some light kicks inside your womb and your distended colon suggest that your children know' );
			}
	...
	PhoukaScene.prototype.phoukaDrinkAccept = function() { //In every path that leads here the character learns the creature is called a phouka, so no phoukaName() calls needed
		...
		switch( ConsumableLib.P_WHSKY.phoukaWhiskeyDrink( CoC.getInstance().player ) ) {
			...
			case 1: //Child is a phouka or satyr, loves alcohol
				EngineCore.outputText( 'A warm feeling in your belly lets you know your passenger enjoyed the meal as well.' );
				break;
		...
		ConsumableLib.P_WHSKY.phoukaWhiskeyAddStatus( CoC.getInstance().player );
		if( CoC.getInstance().player.tou < 30 ) {
		...
		if( CoC.getInstance().player.tou < 70 ) {
			EngineCore.outputText( '  It looks like the phouka is holding his liquor about as well as you are.' );
		} else {
			EngineCore.outputText( '  It seems you can handle your liquor better than the phouka.' );
		}
		EngineCore.outputText( '  Soon both of you start to share stories about your adventures in Mareth.' );
	...
	PhoukaScene.prototype.phoukaDrinkRefuse = function() { //In every path that leads here the character learns the creature is called a phouka, so no phoukaName() calls needed
		EngineCore.clearOutput();
		if( (CoC.getInstance().player.pregnancyIncubation === 0) && (CoC.getInstance().player.buttPregnancyIncubation === 0) && (CoC.getInstance().player.pregnancyIncubation <= 100) ) { //Pregnancy is obvious to the phouka
			...
			if( (CoC.getInstance().player.lust > 66) || ((CoC.getInstance().player.cor > 50) && (CoC.getInstance().player.lust > 33)) ) {
	...
	PhoukaScene.prototype.phoukaDrinkTalk = function( playerNotDrinking ) {
		if( CoC.getInstance().flags[ kFLAGS.PHOUKA_LORE ] === 1 ) {
			CoC.getInstance().flags[ kFLAGS.PHOUKA_LORE ]++;
		...
		} else if( CoC.getInstance().flags[ kFLAGS.PHOUKA_LORE ] === 2 ) {
			CoC.getInstance().flags[ kFLAGS.PHOUKA_LORE ]++;
		...
		} else if( CoC.getInstance().flags[ kFLAGS.PHOUKA_LORE ] === 3 ) {
			CoC.getInstance().flags[ kFLAGS.PHOUKA_LORE ] += 1;
		...
		} else if( CoC.getInstance().flags[ kFLAGS.PHOUKA_LORE ] === 4 ) {
		...
		var playerIsInebriated = (playerNotDrinking === false) && (CoC.getInstance().player.tou < 70);
		if( playerIsInebriated ) {
			...
			if( CoC.getInstance().player.lust < 33 ) {
				...
				if( CoC.getInstance().player.hasCock() ) {
					EngineCore.outputText( ', [eachCock] beginning to swell' + (CoC.getInstance().player.hasVagina() ? ' and your [vagina] starting to moisten' : '') + '.' );
				} else if( CoC.getInstance().player.hasVagina() ) {
					EngineCore.outputText( ', your [vagina] starting to moisten.' );
				} else {
					EngineCore.outputText( 'and you feel your sphincter twitch as your brain thinks about what you could do with this phouka now.' );
				} //Genderless
			}
			EngineCore.menu();
			EngineCore.addButton( 0, 'Leave', this.phoukaDrinkLeave );
	...
	PhoukaScene.prototype.phoukaHalloween = function() {
		CoC.getInstance().flags[ kFLAGS.TREACLE_MINE_YEAR_DONE ] = OnLoadVariables.date.fullYear;
		EngineCore.clearOutput();
		...
		if( CoC.getInstance().player.str < 40 ) {
			EngineCore.outputText( 'You find it very difficult to operate and have to squeeze it with your thighs, but you manage to pump a respectable amount before passing it on.' );
		} else if( CoC.getInstance().player.str < 80 ) {
			EngineCore.outputText( 'You find that treacle pumping is as hard as the phouka said, but you keep at it.  With what you pumped you must be as strong as they are.' );
		} else {
			EngineCore.outputText( 'You don\'t know why they said this was difficult.  You have no problem pumping the treacle; in fact they have to stop you partway through to change barrels.' );
		}
	...
	PhoukaScene.prototype.phoukaHalloweenLeave = function() {
		EngineCore.outputText( '\n\nFeeling you\'re better off not finding out you excuse yourself, telling them they can enjoy the treacle amongst themselves.  As you walk away you just catch the boss telling the others <i>“Watch out fer that one, ' + CoC.getInstance().player.mf( 'He', 'She' ) + '\'s a smart one.”</i>' );
		EngineCore.doNext( CoC.getInstance().scenes.camp.returnToCampUseOneHour );  //Return to camp, 1 hour used
	};
	...
	PhoukaScene.prototype.phoukaLeaveOnLustWin = function( newScreen ) {
		...
		EngineCore.outputText( 'The lusty ' + this.phoukaName() + ' continues to pound his cock into the earth as you prepare to go, oblivious to your presence.' );
	...
	PhoukaScene.prototype.phoukaPlayerWins = function( hpVictory ) {
		...
		if( hpVictory ) { //You win by physical damage, the phouka cheats and runs
			...
			if( CoC.getInstance().player.lust < 33 ) {
				...
				EngineCore.outputText( '  While he\'s busy, you extract yourself from the muck, and once you\'re free, you look over at the lusty ' + this.phoukaName() );
				if( CoC.getInstance().player.cor < 34 ) {
					EngineCore.outputText( '.  You could sate yourself just to be sure you\'re not distracted on the way home.' );
				} else if( CoC.getInstance().player.cor < 67 ) {
					EngineCore.outputText( '.  You could take advantage of the situation to slake your lust.' );
				} else {
	...
	PhoukaScene.prototype.phoukaPregUpdate = function() { //Belly size doesn't change, instead you get updates on what's going on
		...
		if( CoC.getInstance().player.pregnancyIncubation === 140 ) { //Stage 2
			if( CoC.getInstance().flags[ kFLAGS.PREGNANCY_CORRUPTION ] > 0 ) {
		...
		if( CoC.getInstance().player.pregnancyIncubation === 100 ) { //Stage 3 belly feels like it's full of liquid, more like a normal pregnancy. ');
			if( CoC.getInstance().flags[ kFLAGS.PREGNANCY_CORRUPTION ] > 0 ) {
			...
			EngineCore.outputText( ' You\'ve noticed that this pregnancy doesn\'t seem to be affecting your breasts. It\'s as if the child inside you has no use for your milk.\n' );
			return true;
		}
		if( CoC.getInstance().player.pregnancyIncubation === 60 ) { //Stage 4
			if( CoC.getInstance().flags[ kFLAGS.PREGNANCY_CORRUPTION ] > 6 ) {
		...
		if( CoC.getInstance().player.pregnancyIncubation === 36 ) { //Stage 5 though your belly remains the same size you somehow feel that your pregnancy is drawing to a close. ');
			if( CoC.getInstance().flags[ kFLAGS.PREGNANCY_CORRUPTION ] > 6 ) {
				EngineCore.outputText( 'Despite the small size of your belly you spend most of your time feeling deeply ill and you can\'t wait to force this thing out.  You can feel a constant dull pain from your womb and ovaries, probably the result of the tainted fluid inside you.  Only drinking alcohol settles your stomach.\n' );
			} else if( CoC.getInstance().flags[ kFLAGS.PREGNANCY_CORRUPTION ] > 0 ) {
				EngineCore.outputText( 'Despite the relatively small size of your belly, you spend most of your time feeling mildly ill and you can\'t wait to force this thing out.\n' );
			} else {
				EngineCore.outputText( 'Despite being pregnant you still don\'t feel restricted or slowed in any way.  It\'s as if the tiny life inside is living in harmony with you.\n' );
			}
			return true;
		}
		return false;
	...
	PhoukaScene.prototype.phoukaSexBunny = function( postCombat, lustLoss ) { //Player must have a vagina for bunny-morph sex
		...
		if( postCombat ) {
			...
			EngineCore.outputText( '.  You watch as the bunny\'s erection stiffens, your ' );
			if( lustLoss ) {
				EngineCore.outputText( 'mind so overcome with lust you can\'t' );
			} else {
				EngineCore.outputText( 'body too bruised to' );
			}
			EngineCore.outputText( ' resist any further.' );
		} else {
		...
		if( (CoC.getInstance().player.lust > 80) || (CoC.getInstance().player.cor > 50) ) {
			...
			EngineCore.outputText( ' You are powerless to stop the bunny-morph as he undoes the last few straps holding your [armor] in place.' );
		}
		...
		if( CoC.getInstance().player.hasVirginVagina() ) {
			EngineCore.outputText( 'You scream as the ' + this.phoukaName() + '\'s cock tears your hymen apart.  He stops thrusting and says <i>“A virgin? Here? Good goddess, missy, ya should have told me.”</i> In a lower tone he adds <i>“so I could have enjoyed it more.”</i> Your whole vagina burns as every fold is stretched and widened by the passage of the rabbit\'s cock.' );
		} else if( postCombat && (CoC.getInstance().player.vaginas[ 0 ].vaginalLooseness < 2) ) {
			EngineCore.outputText( 'You nearly pass out from the pain as the ' + this.phoukaName() + '\'s cock lengthens and thickens inside you.' );
		} else if( !postCombat || (CoC.getInstance().player.cor > 50) ) {
			EngineCore.outputText( 'Even as the invader lengthens and thickens you push back against it. <i>“Good girl. Good little fuck slut,”</i> whispers the ' + this.phoukaName() );
		}
		CoC.getInstance().player.cuntChange( 50, true );
	...
	PhoukaScene.prototype.phoukaSexGoat = function( postCombat, lustLoss ) { //Player does not have a vagina or has chosen anal
		...
		if( postCombat ) {
			...
			EngineCore.outputText( 'You tell the ' + this.phoukaName() + ' you\'re willing to let him take your ass in his goat form.  He smiles and lands near you.  His body warps into the form of a black furred goat morph.  ' );
			if( CoC.getInstance().player.cor <= 50 ) {
				EngineCore.outputText( 'You steel yourself for an asspounding while you remove your [armor].' );
			} else {
		...
		if( postCombat ) {
			EngineCore.outputText( 'He mounts you immediately, giving you no chance to adjust to his invading cock.  The goat-morph\'s first powerful thrust drives your face into the muck as his tries to force his narrow cockhead into your ass.' );
		} else {
			EngineCore.outputText( 'The goat rubs his cockhead against your pucker, spreading a thick layer of pre over your hole before slowly pressing home his cock.  His first few thrusts are gentle enough that you have the chance to spread your ass and get used to this invader.' );
		}
		EngineCore.outputText( '  Disturbingly you feel his front \'hooves\' split into hard little fingers that he wraps around your [if (isTaur = true)belly][if (isTaur = false)[chest]] to hold onto you even tighter.\n\n' );
		if( postCombat ) {
			EngineCore.outputText( 'The ' + this.phoukaName() + ' shows no interest in you except as a receptacle for his cum.  After stabbing his hips at you several times you feel the full length of his cock penetrate your pucker.' );
		} else {
			EngineCore.outputText( 'Once your [butt] has adjusted, the goat morph begins to thrust more quickly.  You are surprised when you feel his balls slapping against your [if (isNaga = true)tail][if (isNaga = false)inner thighs] - he can\'t already be in that deep, can he?' );
		}
		EngineCore.outputText( '  Once the goat\'s cock is inside, he stops moving and tenses up.  You feel his cock growing larger and larger until you fear your abused anus is going to split apart.' );
		CoC.getInstance().player.buttChange( 45, true );
		...
		if( postCombat ) {
			EngineCore.outputText( 'The goat-morph waits only a few seconds before unceremoniously yanking his deflating cock from your ass.' );
		} else {
			EngineCore.outputText( 'Realizing that you haven\'t yet cum, the goat morph continues to rock in and out of your ass.  The air fills with the sucking and slurping noises of the goat\'s seed alternately being sucked deeper or forced out of your ass.  Finally you reach your limit and cum[if (hasCock = true), your own cock turning the mud white beneath you].' );
		}
		...
		CoC.getInstance().player.orgasm();
		if( postCombat ) {
			EngineCore.outputText( '  While you\'re recovering the goat-morph reaches into your gem pouch and takes a handful.' );
		}
	...
	PhoukaScene.prototype.phoukaSexHorse = function( postCombat, lustLoss ) { //Player has a vagina
		...
		if( CoC.getInstance().player.isTaur() ) {
			EngineCore.outputText( 'He walks up behind you and mounts you.  You feel his front legs rubbing against your flanks and have to admit to yourself that this feels right.  The ' + this.phoukaName() + '\'s equine cock was built to force its way inside a mare[if (vagCapacity < 40), but it has difficulty with your tight cuntlips.  The stallion has to try twice more before his cock slides into place between your labia][if (vagCapacity >= 40) and it does just that].' );
		} else {
			if( CoC.getInstance().player.isNaga() ) {
				EngineCore.outputText( 'He walks in front of you, turns your way and then simply walks over you, forcing your upper body to bend backward over your tail.  Thankfully your snake-like spine means bending into a \'U\' shape isn\'t that uncomfortable.' );
			} else {
				EngineCore.outputText( 'He walks up behind you and then simply walks right over top of you, his chest knocking you to the ground.  You find yourself on all fours with a massive cockhead grinding against your ass.' );
			}
			EngineCore.outputText( '  You hear the ' + this.phoukaName() + ' say <i>“Now line me up with yer cunt lass, or I\'ll lay down on top of ya and have a nap.”</i> Not wanting to be crushed under that kind of bulk, you reach [if (isNaga = true)up][if (isNaga = false)behind you] and grab his member.  You place his cock at the entrance to your [vagina] and the stallion steps forward.  [if (vagCapacity < 50)His cock bends and then snaps free of your cunt, slapping against his belly. <i>“Again! And do it right this time!”</i> he shouts.  You line it up once more and press against it with your hands so it has nowhere to go but in.]' );
		}
		EngineCore.outputText( '  You\'re convinced you\'re going to be split in half by this monster horse cock.\n\n' );
		if( CoC.getInstance().player.hasVirginVagina() ) {
			EngineCore.outputText( 'You scream as the ' + this.phoukaName() + '\'s cock tears your hymen apart.  He stops mid-thrust and asks <i>“A virgin? Here? Good goddess missy ya should have told me.”</i> In a lower tone he adds <i>“so I could have enjoyed it more.”</i> Your whole vagina burns as every fold is stretched and widened by the passage of the ' + this.phoukaName() + '\'s huge horse cock.  You think he\'s delighted to have taken a woman from virgin to gaping in just one thrust.  ' );
		} else if( (postCombat) && (CoC.getInstance().player.vaginas[ 0 ].vaginalLooseness < 3) ) {
			EngineCore.outputText( 'You nearly pass out from the pain as the ' + this.phoukaName() + '\'s cock drives deeper and deeper inside you.  With such a wide head forcing its way in, you\'re sure the stretching will be permanent.  ' );
		} else if( !postCombat || (CoC.getInstance().player.cor > 50) ) {
			EngineCore.outputText( 'Even as the invader thrusts deeper you push back against it. <i>“Good girl, ya love horse cock, don\'t ya?”</i> whispers the ' + this.phoukaName() + '.  ' );
		}
		CoC.getInstance().player.cuntChange( 80, true );
		...
		if( (CoC.getInstance().player.lust > 80) || (CoC.getInstance().player.cor > 50) ) {
			EngineCore.outputText( 'You pant and moan, waiting for the inevitable flood from his cantaloupe-sized nuts.  You\'re ready to get off just from the feeling of being so stuffed, so violated.' );
		} else {
			EngineCore.outputText( 'You whimper in pain, hoping the ' + this.phoukaName() + ' will soon be done with you.  ' );
			if( CoC.getInstance().player.isTaur() ) {
				EngineCore.outputText( 'You feel so stretched and full.  What worries you is that the pain is giving way to pleasure.  Some base instinct of your centaur body is telling you that as a mare you were meant to be filled like this.  Your cunt was built for cocks like this so you could carry lots of little centaur foals.' );
			} else {
				EngineCore.outputText( 'You\'re so stretched that your blood flow is being cut off and you can\'t feel [if (isNaga = true)the end of your tail][if (isNaga = false)your legs] anymore.' );
			}
		}
		EngineCore.outputText( '  You feel those cum-stuffed balls twitch once, twice, three times.  The pressure must be building in the ' + this.phoukaName() + '\'s prostate.  He groans <i>“[if (isPregnant = true)Time te feed the baby.][if (isPregnant = false)Maybe today I\'ll leave ye a present.]”</i>  When he cums it\'s not a series of ripples moving up his urethra, it\'s a single long blast of stallion seed.' );
		this.phoukaSexPregnate( postCombat );
	};
	...
	PhoukaScene.prototype.phoukaSexFaerie = function( postCombat ) {
		...
		if( CoC.getInstance().player.hasVagina() ) {
			...
			CoC.getInstance().player.buttChange( 20, true );
			...
			if( CoC.getInstance().player.isTaur() ) {
				EngineCore.outputText( 'You sigh in frustration as the ' + this.phoukaName() + ' pulls free from your ass, obviously about to leave.  He notices your condition and thinks better of it. <i>“Just gimme a sec.  I’ll get ye off,”</i> he says. <i>“After all, I do want ye to come back fer more.”</i>\n\nHe starts jacking himself off, whispering something to himself under his breath.  When his cock looks nice and hard he grabs your flanks and drives the full length back inside you.  As he pistons into your rectum, you feel that denied orgasm building and at last you cum.' );
			} else {
				EngineCore.outputText( 'Not yet satisfied, you wait for the flood to stop, then begin to really ride your man-sized faerie.\n\nYou don\'t give him the chance to go soft.  Your [if (isNaga = true)tail][if (isNaga = false)legs] feel rubbery and your muscles are burning when you finally cum.' );
			}
			if( CoC.getInstance().player.hasCock() ) {
				EngineCore.outputText( '  [EachCock] fires several blasts of cum into the bog [if (isTaur = true)beneath you][if (isTaur = false)and all over your faerie partner].' );
			}
	...
	PhoukaScene.prototype.phoukaSexFaerieFemale = function( postCombat ) {
		...
		if( CoC.getInstance().player.biggestCockLength() < 8 ) {
			EngineCore.outputText( 'You push the last inch of your ' + Math.ceil( CoC.getInstance().player.biggestCockLength() ) + ' prick into the moaning slut.' );
		} else if( CoC.getInstance().player.biggestCockLength() < 15 ) {
			EngineCore.outputText( 'You feed the last inch of your ' + Math.ceil( CoC.getInstance().player.biggestCockLength() ) + ' cock into her.  Your cockhead bumps against her cervix and the slut beneath you begs for more.' );
		} else {
			EngineCore.outputText( 'You drive as much of your cock as you can into the little faerie slut, but there\'s just too much meat for her to take in.' );
			if( CoC.getInstance().player.cor <= 50 ) {
				EngineCore.outputText( '  It still feels amazing and you give her even more pleasure by rubbing your cockhead against her cervix.' );
			} else {
		...
		if( CoC.getInstance().player.cor <= 33 ) {
			EngineCore.outputText( 'You certainly don\'t want a phouka to learn where your campsite is.  You help her into the bushes and tell her she just needs to hide out for a few days.' );
		} else if( CoC.getInstance().player.cor <= 66 ) {
			EngineCore.outputText( 'You laugh and remind her that\'s what she\'s wanted to do to you.' );
		} else {
			EngineCore.outputText( 'Why shouldn\'t the other phouka get the chance to enjoy her?  Before she can fully recover you grab some tree limbs and vines.  You pin her to the ground and use them to tie her up tight.  With her arms and legs lashed together and her ass in the air she\'s an inviting target.' );
		}
		EngineCore.outputText( '  Satisfied, you march back to camp.' );
		EngineCore.dynStats( 'cor', Utils.rand( 1 ) + 3 ); //Extra two corruption for being enough of a pervert to want to fuck the phouka
		if( postCombat ) {
			Combat.clearStatuses( false );
			Combat.awardPlayer();  //This will provide loot and return to camp, 1 hour used
	...
	PhoukaScene.prototype.phoukaSexPregnate = function( postCombat ) { //Whether by horse, bunny or (male) faerie sex it all ends up here if the PC has a vagina
		if( CoC.getInstance().player.isPregnant() ) {
			...
			if( CoC.getInstance().player.cor > 50 ) {
				EngineCore.outputText( '  The feeling of all that seed flowing into you and then out of you again finally brings you to your own orgasm.[if (hasCock = true)  [EachCock] fires long strands of cum into the bog water beneath you.]' );
			} else {
		...
		CoC.getInstance().player.knockUp( PregnancyStore.PREGNANCY_FAERIE, PregnancyStore.INCUBATION_FAERIE );
		if( PhoukaScene.phoukaForm === PhoukaScene.PHOUKA_FORM_FAERIE ) {
		...
		if( (!postCombat) || (CoC.getInstance().player.cor > 50) ) {
			if( PhoukaScene.phoukaForm === PhoukaScene.PHOUKA_FORM_HORSE ) {
				...
				if( postCombat ) {
					EngineCore.outputText( 'The bastard ' + this.phoukaName() + ' laughs as you cum. <i>“Yeah, get nice and comfy while my seed takes root.  You lasses always love bein’ bred.”</i>' );
				} else {
					EngineCore.outputText( 'Perhaps he\'s being a considerate lover or perhaps he just wants to better his chances of knocking you up.' );
				}
			}
		} else {
			if( PhoukaScene.phoukaForm === PhoukaScene.PHOUKA_FORM_HORSE ) {
		...
		if( postCombat ) {
			EngineCore.outputText( ' rummages around in your gem pouch and then' );
		}
		EngineCore.outputText( ' takes to the air and hovers nearby as if waiting for something.\n\n' );
		if( CoC.getInstance().player.isPregnant() ) {
			var isFirstTime = (CoC.getInstance().flags[ kFLAGS.BIRTHS_PHOUKA ] === 0) && (CoC.getInstance().flags[ kFLAGS.BIRTHS_FAERIE ] === 0);
			CoC.getInstance().flags[ kFLAGS.PREGNANCY_CORRUPTION ] = 2; //Start off with a corrupt little phouka. It will take three drinks of pure honey to turn it into a faerie
			EngineCore.outputText( 'You\'re about to lift yourself up when you feel ' + (isFirstTime ? 'an unusual' : 'a familiar') + ' sensation inside your belly.  All that cum is hardening!  You can feel it set, almost like mortar left out in the sun!  You stand and some clear water drains out, your belly shrinking in size to that of a watermelon.  Whatever\'s left inside your belly is solid, like you\'re carrying a smooth ball of wood inside you.\n\nYou run a hand over your belly.  Thankfully it isn\'t any heavier than a normal pregnancy, nor does it hurt at all.\n\n<i>“Yes!”</i> shouts the ' + this.phoukaName() + '. <i>“I\'m gonna be a daddy!  Thanks fer letting me fill ya up girl, there’s plenty o’ space in there fer the little guy.  I gotta tell all the other phouka about ya' );
			if( CoC.getInstance().flags[ kFLAGS.PHOUKA_LORE ] === 0 ) {
				CoC.getInstance().flags[ kFLAGS.PHOUKA_LORE ] = 1;
			} //Now you know what to call them
			if( !postCombat || (CoC.getInstance().player.cor > 50) ) {
				EngineCore.outputText( ', yer a real prize slut' );
			}
			EngineCore.outputText( '.”</i> The phouka gains more altitude and flies off deeper into the bog, whistling happily to himself.  ' );
			if( isFirstTime ) {
				EngineCore.outputText( 'You have to wonder how long this pregnancy will last. If this were a human child people would judge you to be at least four months along.' );
			} else {
				EngineCore.outputText( 'At least you know what to expect from this form of pregnancy now.' );
			}
		} else {
			EngineCore.outputText( 'When you pull yourself up your belly begins to deflate and leak its gooey grey payload into the mud.  The ' + this.phoukaName() + ' says <i>“Ah well, there\'s always next time,”</i> and buzzes away to some other part of the bog.' );
		}
		this.phoukaSexPregnateEnd( postCombat );
	};
	PhoukaScene.prototype.phoukaSexPregnateEnd = function( postCombat ) { //Everything after the sex. Handles awards, gem loss and text for player leaving the bog
		...
		if( PhoukaScene.phoukaForm === PhoukaScene.PHOUKA_FORM_FAERIE ) { //In this case postCombat means you need an award because you must have won to get faerie sex
			...
			if( postCombat && CoC.getInstance().player.gems > 0 ) {
				EngineCore.outputText( '  While you\'re recovering the ' + this.phoukaName() + ' reaches into your gem pouch and takes a handful.' );
			}
			...
			EngineCore.outputText( '\n\nYou ' );
			if( PhoukaScene.phoukaForm === PhoukaScene.PHOUKA_FORM_HORSE && !CoC.getInstance().player.isTaur() ) {
				EngineCore.outputText( 'wait for the throbbing pain in your pelvis to subside.  Then you ' );
			}
			if( postCombat ) {
				EngineCore.outputText( 'pull your [if (isNaga = true)tail][if (isNaga = false)legs] free from the cool muck before you get chilled to the bone.' );
				Combat.cleanupAfterCombat();
			} else {
			...
			} else if( PhoukaScene.phoukaForm === PhoukaScene.PHOUKA_FORM_HORSE && CoC.getInstance().player.isTaur() ) {
	...
	CoC.getInstance().registerScene( 'phoukaScene', new PhoukaScene() );
} );