...
angular.module( 'cocjs' ).run( function( SceneLib, $rootScope, CoC, kFLAGS, Utils, StatusAffects, PregnancyStore, EngineCore, Phouka, Combat, ConsumableLib, OnLoadVariables ) {
		...
		if( playerIsInebriated ) {
			...
			if( CoC.player.lust < 33 ) {
				...
				if( CoC.player.hasCock() ) {
					EngineCore.outputText( ', [eachCock] beginning to swell' + (CoC.player.hasVagina() ? ' and your [vagina] starting to moisten' : '') + '.' );
	...
	PhoukaScene.prototype.phoukaSexFaerieFemale = function( postCombat ) {
		...
		if( CoC.player.biggestCockLength() < 8 ) {
			EngineCore.outputText( 'You push the last inch of your ' + Math.ceil( CoC.player.biggestCockLength() ) + ' prick into the moaning slut.' );
		} else if( CoC.player.biggestCockLength() < 15 ) {
			EngineCore.outputText( 'You feed the last inch of your ' + Math.ceil( CoC.player.biggestCockLength() ) + ' cock into her.  Your cockhead bumps against her cervix and the slut beneath you begs for more.' );
		} else {
