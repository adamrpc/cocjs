...
angular.module( 'cocjs' ).run( function( SceneLib, $rootScope, MainView, CoC, kFLAGS, Utils, StatusAffects, PregnancyStore, EngineCore, Phouka, Combat, ConsumableLib, OnLoadVariables ) {
	...
	PhoukaScene.prototype.timeChange = function() {
		if( CoC.player.statusAffectv1( StatusAffects.PhoukaWhiskeyAffect ) > 0 ) {
			CoC.player.addStatusValue( StatusAffects.PhoukaWhiskeyAffect, 1, -1 ); //Count down hours until player is not drunk
			if( CoC.player.statusAffectv1( StatusAffects.PhoukaWhiskeyAffect ) <= 0 ) {
				ConsumableLib.P_WHSKY.phoukaWhiskeyExpires( CoC.player );
				return true;
			}
		}
		return false;
	};
	PhoukaScene.prototype.timeChangeLarge = function() {
		return false;
	};
	//End of Interface Implementation
	PhoukaScene.prototype.phoukaNameText = function( known, unknown ) { //Helper function, For any text that depends on whether or not the player knows what to call a phouka
		return (CoC.flags[ kFLAGS.PHOUKA_LORE ] === 0 ? unknown : known);
	};
	PhoukaScene.prototype.phoukaName = function() { //Helper function, Handles the most use of phoukaNameText
	...
	PhoukaScene.prototype.phoukaEncounter = function() { //General entry point for everything except halloween special encounter
		...
		if( CoC.flags[ kFLAGS.PHOUKA_ENCOUNTER_STATUS ] === 0 ) { //Guarantee first Phouka encounter is with faerie fire
			...
			CoC.flags[ kFLAGS.BIRTHS_PHOUKA ] = 0;
			CoC.flags[ kFLAGS.BIRTHS_FAERIE ] = 0;
		} else if( (OnLoadVariables.date.month > 4) && (OnLoadVariables.date.month < 11) ) {
			choiceChance = Utils.rand( 7 );
		}//All through the late summer and autumn they might be masturbating on berries
		else {
			choiceChance = Utils.rand( 6 );
		}
		if( choiceChance < 2 ) {
	...
	PhoukaScene.prototype.phoukaStuckOfferWhiskey = function() {
		MainView.clearOutput();
		MainView.outputText( 'You pull out the bottle and ask the ' + this.phoukaName() + ' if he might be interested in a drink.  He looks at the bottle and says <i>“Well we both know where ya picked that up, hmmm?”</i>  Then he smiles and adds <i>“Course that’s some other phouka’s problem if he let ye beat him.”</i>' );
	...
	PhoukaScene.prototype.phoukaTalk = function() {
		MainView.clearOutput();
		MainView.outputText( 'You ask the ' + this.phoukaName() );
		if( CoC.player.level < 10 ) {
			MainView.outputText( ' if perhaps there\'s some way he could release you.' );
		} else {
			MainView.outputText( ' if he really wants a taste of what you\'re going to give him.' );
		}
		if( CoC.player.cor + Utils.rand( 125 ) <= 110 ) { //Players with 0% corruption have a 12% chance that the Phouka is willing to talk, players with 100% corruption have a 92% chance
			//The phouka is not in the mood for talk, start the fight gagged as punishment for trying to talk
			MainView.outputText( '  You open your mouth to say something more to the ' );
			if( CoC.player.cor < 34 ) {
				MainView.outputText( 'vile little monster' );
			} else if( CoC.player.cor < 67 ) {
				MainView.outputText( 'perverted ' + this.phoukaName() );
			} else {
			...
			MainView.outputText( '\n\nThe ' + this.phoukaName() );
			switch( CoC.player.level < 10 ? Utils.rand( 8 ) : Utils.rand( 12 ) ) { //Low level characters don't get the chance to be offered whiskey
				case 0:
				case 1:
				case 2:
				case 3:
				...
				case 6: //Offers you a chance to submit, which leads to the various defeat sex scenes
					MainView.outputText( ' considers for a moment and then says, <i>“You can\'t get away from me right now, so why don\'t you enjoy it?  Otherwise, maybe I bring in some of my friends an we all have at ya.”</i> The confidence in the ' + this.phoukaName() + '\'s gaze suggests he isn\'t bluffing.  He slowly circles you, hovering a few feet out of reach.  His eyes seem to be drinking in every detail of your body in a way that [if (corruption < 50)makes shivers run down your spine][if (corruption >= 50)makes your heart beat faster].\n\nThe ' + this.phoukaName() + ' continues, ' );
					if( CoC.player.hasVagina() ) {
						MainView.outputText( '<i>“Maybe yer inta bunnies, hrm?  Big bunny with a nice hard cock and strong legs, I can do that fer ya.  Fill that tight little muff o\' yers up with a big load o\' spunk!”\n\n“Or maybe ' );
					} else {
						MainView.outputText( '<i>“' );
					}
					MainView.outputText( 'I could change inta a goat for ya.  That suit yer fancy?  Is it a fond dream o\' yers to let a goat blow its load in yer cornhole?' );
					if( CoC.player.hasVagina() ) {
				...
				case 8: //Decides he doesn't want you
					MainView.outputText( ' takes a long look at you.  Finally he grins and tells you <i>“You know what?  You\'re not quite what I was lookin for.  Us phooka got varied tastes, ya know.”</i>  Then the phouka lazily flies off, whistling some strange tune to himself.\n\nYou wriggle against your bonds and after a few minutes you are free once more.  You decide that\'s enough excitement for now and head back to camp.' );
					if( CoC.flags[ kFLAGS.PHOUKA_LORE ] === 0 ) {
						CoC.flags[ kFLAGS.PHOUKA_LORE ] = 1;
					} //Now you know what to call them
					EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );  //Return to camp, 1 hour used
	...
	PhoukaScene.prototype.phoukaDrinkWhilePregnant = function( playerOfferedTheBooze ) {
		if( (CoC.player.pregnancyIncubation <= 100) || (CoC.player.buttPregnancyIncubation <= 100) ) { //Pregnancy is obvious to the phouka
		...
		var acceptable = ConsumableLib.P_WHSKY.phoukaWhiskeyAcceptable( CoC.player );
		if( acceptable < 0 ) {
			MainView.outputText( '\n\nYou think about the unborn child' + (acceptable <= -3 ? 'ren' : '') + ' you\'re carrying.' );
			if( acceptable === -4 ) {
				MainView.outputText( '  One of them seems to crave the whiskey, but the other is a more normal child.' );
			}
			MainView.outputText( '  Sure, some of the elders around the village said a beer or some light cider was all right for a woman heavy with child, but you doubt that whiskey would be acceptable.  You politely refuse the drink, hoping the phouka won\'t be offended.' );
		} else {
			MainView.outputText( '\n\nYou put one hand on your belly, about to refuse the drink.  Then you realize that yes, this is exactly what your ' );
			if( acceptable <= 2 ) {
				MainView.outputText( 'baby needs.  Some light kicks inside your ' + (acceptable === 2 ? 'distended colon' : 'womb') + ' suggest that your child knows' );
			} else {
				MainView.outputText( 'babies need.  Some light kicks inside your womb and your distended colon suggest that your children know' );
			}
	...
	PhoukaScene.prototype.phoukaDrinkAccept = function() { //In every path that leads here the character learns the creature is called a phouka, so no phoukaName() calls needed
		...
		if( CoC.player.tou < 70 ) {
			MainView.outputText( '  It looks like the phouka is holding his liquor about as well as you are.' );
		} else {
			MainView.outputText( '  It seems you can handle your liquor better than the phouka.' );
		}
		MainView.outputText( '  Soon both of you start to share stories about your adventures in Mareth.' );
	...
	PhoukaScene.prototype.phoukaDrinkRefuseLeave = function() { //In every path that leads here the character learns the creature is called a phouka, so no phoukaName() calls needed
		MainView.clearOutput();
		MainView.outputText( 'You extract yourself from the mud and get out of the bog before the phouka changes its mind.' );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );  //Return to camp, 1 hour used
	};
	PhoukaScene.prototype.phoukaDrinkRefusePregnantLeave = function() { //In every path that leads here the character learns the creature is called a phouka, so no phoukaName() calls needed
		MainView.clearOutput();
		MainView.outputText( '<i>“Ah fuck.  You\'re a killjoy.  Don\'t you know the best time to get porked is when you\'ve already got a bun in the oven?”</i>\n\nThe black bunny waves you away dismissively. <i>“Fine, yeah, go home.  I hope the little bastard hurts when it pops out.  I would have stretched you out but good.”</i>' );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );  //Return to camp, 1 hour used
	};
	PhoukaScene.prototype.phoukaDrinkRefuseAfterOffer = function() {
		MainView.clearOutput();
		MainView.outputText( 'The phouka looks disappointed but downs one of the cups and says <i>“How come chicks that’re knocked up are always such stick in the muds?”</i> He starts to sip the second cup, looks over at you and chuckles. <i>“I guess in yer case you’ve got a good excuse - ye are stuck in the mud!”</i> The phouka keeps drinking and it isn’t long before he starts to reminisce.' );
		this.phoukaDrinkTalk( true );
	};
	PhoukaScene.prototype.phoukaDrinkTalk = function( playerNotDrinking ) {
		if( CoC.flags[ kFLAGS.PHOUKA_LORE ] === 1 ) {
			CoC.flags[ kFLAGS.PHOUKA_LORE ]++;
		...
		} else if( CoC.flags[ kFLAGS.PHOUKA_LORE ] === 2 ) {
			CoC.flags[ kFLAGS.PHOUKA_LORE ]++;
		...
		} else if( CoC.flags[ kFLAGS.PHOUKA_LORE ] === 3 ) {
			CoC.flags[ kFLAGS.PHOUKA_LORE ] += 1;
		...
		} else if( CoC.flags[ kFLAGS.PHOUKA_LORE ] === 4 ) {
		...
		if( playerIsInebriated ) {
			...
			MainView.outputText( '  He lets out a little giggle and begins to shrink. Tendrils of black smoke rise from the bunny morph’s black fur as he shrinks back to the size and shape of a faerie.' );
			this.phoukaForm = this.PHOUKA_FORM_FAERIE;
			if( CoC.player.lust < 33 ) {
				MainView.outputText( '  It looks like he needs a nap and you take that as your cue to get out of here.' );
			} else {
				MainView.outputText( '\n\nYou look at the sleeping phouka' );
				if( CoC.player.hasCock() ) {
					MainView.outputText( ', [eachCock] beginning to swell' + (CoC.player.hasVagina() ? ' and your [vagina] starting to moisten' : '') + '.' );
				} else if( CoC.player.hasVagina() ) {
					MainView.outputText( ', your [vagina] starting to moisten.' );
				} else {
	...
	PhoukaScene.prototype.phoukaHalloween = function() {
		...
		if( CoC.player.str < 40 ) {
			MainView.outputText( 'You find it very difficult to operate and have to squeeze it with your thighs, but you manage to pump a respectable amount before passing it on.' );
		} else if( CoC.player.str < 80 ) {
			MainView.outputText( 'You find that treacle pumping is as hard as the phouka said, but you keep at it.  With what you pumped you must be as strong as they are.' );
		} else {
			MainView.outputText( 'You don\'t know why they said this was difficult.  You have no problem pumping the treacle; in fact they have to stop you partway through to change barrels.' );
		}
	...
	PhoukaScene.prototype.phoukaPlayerWins = function( hpVictory ) {
		...
		if( hpVictory ) { //You win by physical damage, the phouka cheats and runs
			...
			if( this.phoukaForm !== this.PHOUKA_FORM_FAERIE ) {
				MainView.outputText( '  As you watch, wisps of black smoke start to rise from him and his body starts to shrink.  In moments the ' );
				if( this.phoukaForm === this.PHOUKA_FORM_BUNNY ) {
					MainView.outputText( 'bunny morph' );
				} else if( this.phoukaForm === this.PHOUKA_FORM_GOAT ) {
					MainView.outputText( 'goat morph' );
				} else if( this.phoukaForm === this.PHOUKA_FORM_HORSE ) {
					MainView.outputText( 'stallion' );
				}
				MainView.outputText( ' has shrunk back down to the size and shape of a ' + this.phoukaNameText( 'phouka', 'faerie' ) + '.' );
				this.phoukaForm = this.PHOUKA_FORM_FAERIE;
			}
			if( CoC.player.lust < 33 ) {
				MainView.outputText( '  The threat dealt with you scoop, dig and finally extract yourself from the mire.  Your lower half is soaked and you long to sit in front of the fire at your camp.  Hefting your supplies you turn back the way you came.' );
				this.phoukaLeaveOnLustWin( false );
			} else {
				MainView.outputText( '  While he\'s busy, you extract yourself from the muck, and once you\'re free, you look over at the lusty ' + this.phoukaName() );
				if( CoC.player.cor < 34 ) {
					MainView.outputText( '.  You could sate yourself just to be sure you\'re not distracted on the way home.' );
				} else if( CoC.player.cor < 67 ) {
					MainView.outputText( '.  You could take advantage of the situation to slake your lust.' );
				} else {
	...
	PhoukaScene.prototype.phoukaPregUpdate = function() { //Belly size doesn't change, instead you get updates on what's going on
		...
		if( CoC.player.pregnancyIncubation === 140 ) { //Stage 2
			if( CoC.flags[ kFLAGS.PREGNANCY_CORRUPTION ] > 0 ) {
		...
		if( CoC.player.pregnancyIncubation === 100 ) { //Stage 3 belly feels like it's full of liquid, more like a normal pregnancy. ');
			if( CoC.flags[ kFLAGS.PREGNANCY_CORRUPTION ] > 0 ) {
			...
			MainView.outputText( ' You\'ve noticed that this pregnancy doesn\'t seem to be affecting your breasts. It\'s as if the child inside you has no use for your milk.\n' );
			return true;
		}
		if( CoC.player.pregnancyIncubation === 60 ) { //Stage 4
			if( CoC.flags[ kFLAGS.PREGNANCY_CORRUPTION ] > 6 ) {
		...
		if( CoC.player.pregnancyIncubation === 36 ) { //Stage 5 though your belly remains the same size you somehow feel that your pregnancy is drawing to a close. ');
			if( CoC.flags[ kFLAGS.PREGNANCY_CORRUPTION ] > 6 ) {
				MainView.outputText( 'Despite the small size of your belly you spend most of your time feeling deeply ill and you can\'t wait to force this thing out.  You can feel a constant dull pain from your womb and ovaries, probably the result of the tainted fluid inside you.  Only drinking alcohol settles your stomach.\n' );
			} else if( CoC.flags[ kFLAGS.PREGNANCY_CORRUPTION ] > 0 ) {
				MainView.outputText( 'Despite the relatively small size of your belly, you spend most of your time feeling mildly ill and you can\'t wait to force this thing out.\n' );
			} else {
				MainView.outputText( 'Despite being pregnant you still don\'t feel restricted or slowed in any way.  It\'s as if the tiny life inside is living in harmony with you.\n' );
			}
			return true;
		}
		return false;
	...
	PhoukaScene.prototype.phoukaSexBunny = function( postCombat, lustLoss ) { //Player must have a vagina for bunny-morph sex
		MainView.clearOutput();
		if( postCombat ) {
			MainView.outputText( 'As you collapse the ' + this.phoukaName() );
			if( this.phoukaForm === this.PHOUKA_FORM_BUNNY ) {
				MainView.outputText( ' hops over and stands in front of you' );
			} else {
				MainView.outputText( ' begins to shapeshift in front of you.  He changes into various forms, looks you over and finally changes into a black furred bunny-morph' );
			}
			MainView.outputText( '.  You watch as the bunny\'s erection stiffens, your ' );
			if( lustLoss ) {
				MainView.outputText( 'mind so overcome with lust you can\'t' );
			} else {
				MainView.outputText( 'body too bruised to' );
			}
			MainView.outputText( ' resist any further.' );
		} else {
			if( CoC.player.cor <= 50 ) {
				MainView.outputText( 'It might not be the wisest move, but you decide to let the ' + this.phoukaName() + ' have his way with you.  You tell him you would prefer a little bunny loving and the ' + this.phoukaName() + ' grins from ear to ear. <i>“Comin\' right up,”</i> he says' );
				if( this.phoukaForm !== this.PHOUKA_FORM_BUNNY ) {
					MainView.outputText( ', and morphs into a rabbit' );
				}
				MainView.outputText( '.  Hopefully he\'ll be gentle with a willing partner.' );
			} else {
				MainView.outputText( 'You can\'t think of any good reason to refuse a roll in the hay (or the mire) with this ' + this.phoukaName() + '.  You tell him that you want to feel his big bunny cock deep inside you.  The ' + this.phoukaName() + ' grins as you begin seductively stripping off you armor.' );
				if( this.phoukaForm !== this.PHOUKA_FORM_BUNNY ) {
					MainView.outputText( '  He goes through a short series of warm up exercises and then transforms himself into the rabbit you desire.' );
				}
			}
		}
		this.phoukaForm = this.PHOUKA_FORM_BUNNY;
		...
		if( (CoC.player.lust > 80) || (CoC.player.cor > 50) ) {
			MainView.outputText( ' You can only moan in response.  The thought of this rabbit stuffing you has your [vagina] leaking already.' );
		} else {
			MainView.outputText( ' You are powerless to stop the bunny-morph as he undoes the last few straps holding your [armor] in place.' );
		}
		...
		if( CoC.player.hasVirginVagina() ) {
			MainView.outputText( 'You scream as the ' + this.phoukaName() + '\'s cock tears your hymen apart.  He stops thrusting and says <i>“A virgin? Here? Good goddess, missy, ya should have told me.”</i> In a lower tone he adds <i>“so I could have enjoyed it more.”</i> Your whole vagina burns as every fold is stretched and widened by the passage of the rabbit\'s cock.' );
		} else if( postCombat && (CoC.player.vaginas[ 0 ].vaginalLooseness < 2) ) {
			MainView.outputText( 'You nearly pass out from the pain as the ' + this.phoukaName() + '\'s cock lengthens and thickens inside you.' );
		} else if( !postCombat || (CoC.player.cor > 50) ) {
			MainView.outputText( 'Even as the invader lengthens and thickens you push back against it. <i>“Good girl. Good little fuck slut,”</i> whispers the ' + this.phoukaName() );
		}
		CoC.player.cuntChange( 50, true );
	...
	PhoukaScene.prototype.phoukaSexBunnyChoice = function() {
	...
	PhoukaScene.prototype.phoukaSexGoat = function( postCombat, lustLoss ) { //Player does not have a vagina or has chosen anal
		...
		if( postCombat ) {
			if( lustLoss ) {
				MainView.outputText( 'Panting with need, you begin to strip off your [armor].  The ' + this.phoukaName() + ' watches the show with an evil smile.' );
			} else {
				MainView.outputText( 'Your body falls to the ground, too weak to fight back against the ' + this.phoukaName() + ' any longer.' );
			}
			if( this.phoukaForm !== this.PHOUKA_FORM_GOAT ) {
				MainView.outputText( '  As you lay there, the ' + this.phoukaName() + '\'s body stretches and morphs until it takes the form of a large, black furred goat-morph.' );
			}
		} else {
			MainView.outputText( 'You tell the ' + this.phoukaName() + ' you\'re willing to let him take your ass in his goat form.  He smiles and lands near you.  His body warps into the form of a black furred goat morph.  ' );
			if( CoC.player.cor <= 50 ) {
				MainView.outputText( 'You steel yourself for an asspounding while you remove your [armor].' );
			} else {
				MainView.outputText( 'Then you put on a show, taking your clothes off piece by piece.  You strip off your final undergarment, bend over and wink your asshole at the goat.' );
			}
		}
		...
		if( postCombat ) {
			MainView.outputText( 'He mounts you immediately, giving you no chance to adjust to his invading cock.  The goat-morph\'s first powerful thrust drives your face into the muck as his tries to force his narrow cockhead into your ass.' );
		} else {
			MainView.outputText( 'The goat rubs his cockhead against your pucker, spreading a thick layer of pre over your hole before slowly pressing home his cock.  His first few thrusts are gentle enough that you have the chance to spread your ass and get used to this invader.' );
		}
		MainView.outputText( '  Disturbingly you feel his front \'hooves\' split into hard little fingers that he wraps around your [if (isTaur = true)belly][if (isTaur = false)[chest]] to hold onto you even tighter.\n\n' );
		if( postCombat ) {
			MainView.outputText( 'The ' + this.phoukaName() + ' shows no interest in you except as a receptacle for his cum.  After stabbing his hips at you several times you feel the full length of his cock penetrate your pucker.' );
		} else {
			MainView.outputText( 'Once your [butt] has adjusted, the goat morph begins to thrust more quickly.  You are surprised when you feel his balls slapping against your [if (isNaga = true)tail][if (isNaga = false)inner thighs] - he can\'t already be in that deep, can he?' );
		}
		MainView.outputText( '  Once the goat\'s cock is inside, he stops moving and tenses up.  You feel his cock growing larger and larger until you fear your abused anus is going to split apart.' );
		CoC.player.buttChange( 45, true );
		...
		if( postCombat ) {
			MainView.outputText( 'The goat-morph waits only a few seconds before unceremoniously yanking his deflating cock from your ass.' );
		} else {
			MainView.outputText( 'Realizing that you haven\'t yet cum, the goat morph continues to rock in and out of your ass.  The air fills with the sucking and slurping noises of the goat\'s seed alternately being sucked deeper or forced out of your ass.  Finally you reach your limit and cum[if (hasCock = true), your own cock turning the mud white beneath you].' );
		}
		EngineCore.dynStats( 'cor', Utils.rand( 1 ) + (postCombat ? 1 : 3) ); //Extra two corruption for being enough of a pervert to want to fuck the phouka
		CoC.player.orgasm();
		if( postCombat ) {
			MainView.outputText( '  While you\'re recovering the goat-morph reaches into your gem pouch and takes a handful.' );
		}
	...
	PhoukaScene.prototype.phoukaSexHorse = function( postCombat, lustLoss ) { //Player has a vagina
		...
		if( postCombat ) {
			...
			if( this.phoukaForm === this.PHOUKA_FORM_HORSE ) {
				MainView.outputText( ' circles you in a slow trot, his top half shaping into a centaur form before finally stopping directly in front of you.' );
			} else {
				MainView.outputText( ' begins to shapeshift in front of you.  It grows and grows, finally taking the form of a massive, black, centaur stallion with an equally massive prick.' );
			}
		} else { //Phouka can’t be a horse if the player is choosing this. Must be either a faerie or a bunny.
			if( CoC.player.cor <= 50 ) {
				MainView.outputText( 'Perhaps this land is finally starting to get to you.  You tell the ' + this.phoukaName() + ' you want him to fuck you with a great big horse cock.  The ' + this.phoukaName() + '\'s eyes light up. <i>“Sure thing missy”</i> he says, and his form stretches into that of a huge, black furred, centaur stallion.  You hope he\'ll be gentle with a willing partner.' );
			} else {
				MainView.outputText( 'The more you think about it the more you decide you could use a real fucking.  You smile at the ' + this.phoukaName() + ', telling him you want the biggest cock he can muster.  The ' + this.phoukaName() + 'morphs into the form you expected, that of a black, centaur stallion [if (tallness <= 96)taller than you are][if (tallness > 96)that\'s about your height].  Maybe that tree trunk between his legs can scratch your itch.' );
			}
		}
		this.phoukaForm = this.PHOUKA_FORM_HORSE;
		MainView.outputText( '  On closer inspection the centaur is a bit unusual.  For a start, he has shiny green eyes.  Parts of his flanks look like thin wings pressed against its sides.  His nose is a little shorter and his face a lot more human than you would have expected.  He sees you looking him over and says <i>[if (isTaur = true)“I\'ve been waitin for this.  A nice big mare to fuck the daylights out of][if (isTaur = false)“Yer gonna be sorry ye weren\'t born with four legs and a cunt you could lose an arm in, princess].”</i>\n\n' );
		if( !postCombat || lustLoss ) {
			MainView.outputText( 'Considering the fire in your vagina that\'s crying out to be quenched, you don\'t care how big this horse\'s cock is.  You just need something to fill you NOW.' );
		} else {
			MainView.outputText( 'You\'re hardly in any condition to refuse.  This stallion is going to pound you with that meatstick no matter what you do[if (corruption <= 50), so you steel yourself for what is to come][if (corruption > 50).  The thought of being wrecked by such an enormous staff has your cunt dripping girl-cum already.]' );
		}
		...
		if( CoC.player.isTaur() ) {
			MainView.outputText( 'He walks up behind you and mounts you.  You feel his front legs rubbing against your flanks and have to admit to yourself that this feels right.  The ' + this.phoukaName() + '\'s equine cock was built to force its way inside a mare[if (vagCapacity < 40), but it has difficulty with your tight cuntlips.  The stallion has to try twice more before his cock slides into place between your labia][if (vagCapacity >= 40) and it does just that].' );
		} else {
			if( CoC.player.isNaga() ) {
				MainView.outputText( 'He walks in front of you, turns your way and then simply walks over you, forcing your upper body to bend backward over your tail.  Thankfully your snake-like spine means bending into a \'U\' shape isn\'t that uncomfortable.' );
			} else {
				MainView.outputText( 'He walks up behind you and then simply walks right over top of you, his chest knocking you to the ground.  You find yourself on all fours with a massive cockhead grinding against your ass.' );
			}
			MainView.outputText( '  You hear the ' + this.phoukaName() + ' say <i>“Now line me up with yer cunt lass, or I\'ll lay down on top of ya and have a nap.”</i> Not wanting to be crushed under that kind of bulk, you reach [if (isNaga = true)up][if (isNaga = false)behind you] and grab his member.  You place his cock at the entrance to your [vagina] and the stallion steps forward.  [if (vagCapacity < 50)His cock bends and then snaps free of your cunt, slapping against his belly. <i>“Again! And do it right this time!”</i> he shouts.  You line it up once more and press against it with your hands so it has nowhere to go but in.]' );
		}
		MainView.outputText( '  You\'re convinced you\'re going to be split in half by this monster horse cock.\n\n' );
		if( CoC.player.hasVirginVagina() ) {
			MainView.outputText( 'You scream as the ' + this.phoukaName() + '\'s cock tears your hymen apart.  He stops mid-thrust and asks <i>“A virgin? Here? Good goddess missy ya should have told me.”</i> In a lower tone he adds <i>“so I could have enjoyed it more.”</i> Your whole vagina burns as every fold is stretched and widened by the passage of the ' + this.phoukaName() + '\'s huge horse cock.  You think he\'s delighted to have taken a woman from virgin to gaping in just one thrust.  ' );
		} else if( (postCombat) && (CoC.player.vaginas[ 0 ].vaginalLooseness < 3) ) {
			MainView.outputText( 'You nearly pass out from the pain as the ' + this.phoukaName() + '\'s cock drives deeper and deeper inside you.  With such a wide head forcing its way in, you\'re sure the stretching will be permanent.  ' );
		} else if( !postCombat || (CoC.player.cor > 50) ) {
			MainView.outputText( 'Even as the invader thrusts deeper you push back against it. <i>“Good girl, ya love horse cock, don\'t ya?”</i> whispers the ' + this.phoukaName() + '.  ' );
		}
		CoC.player.cuntChange( 80, true );
	...
	PhoukaScene.prototype.phoukaSexFaerie = function( postCombat ) {
		...
		if( CoC.player.hasVagina() ) {
			...
			if( CoC.player.isTaur() ) {
				MainView.outputText( 'You sigh in frustration as the ' + this.phoukaName() + ' pulls free from your ass, obviously about to leave.  He notices your condition and thinks better of it. <i>“Just gimme a sec.  I’ll get ye off,”</i> he says. <i>“After all, I do want ye to come back fer more.”</i>\n\nHe starts jacking himself off, whispering something to himself under his breath.  When his cock looks nice and hard he grabs your flanks and drives the full length back inside you.  As he pistons into your rectum, you feel that denied orgasm building and at last you cum.' );
			} else {
				MainView.outputText( 'Not yet satisfied, you wait for the flood to stop, then begin to really ride your man-sized faerie.\n\nYou don\'t give him the chance to go soft.  Your [if (isNaga = true)tail][if (isNaga = false)legs] feel rubbery and your muscles are burning when you finally cum.' );
			}
			if( CoC.player.hasCock() ) {
				MainView.outputText( '  [EachCock] fires several blasts of cum into the bog [if (isTaur = true)beneath you][if (isTaur = false)and all over your faerie partner].' );
			}
	...
	PhoukaScene.prototype.phoukaSexFaerieNonCombat = function() {
		this.phoukaSexFaerie( false );
	};
	PhoukaScene.prototype.phoukaSexFaeriePostCombat = function() {
		this.phoukaSexFaerie( true );
	};
	PhoukaScene.prototype.phoukaSexFaerieFemale = function( postCombat ) {
		...
		if( CoC.player.biggestCockLength() < 8 ) {
			MainView.outputText( 'You push the last inch of your ' + Math.ceil( CoC.player.biggestCockLength() ) + ' prick into the moaning slut.' );
		} else if( CoC.player.biggestCockLength() < 15 ) {
			MainView.outputText( 'You feed the last inch of your ' + Math.ceil( CoC.player.biggestCockLength() ) + ' cock into her.  Your cockhead bumps against her cervix and the slut beneath you begs for more.' );
		} else {
			MainView.outputText( 'You drive as much of your cock as you can into the little faerie slut, but there\'s just too much meat for her to take in.' );
			if( CoC.player.cor <= 50 ) {
				MainView.outputText( '  It still feels amazing and you give her even more pleasure by rubbing your cockhead against her cervix.' );
			} else {
		...
		if( CoC.player.cor <= 33 ) {
			MainView.outputText( 'You certainly don\'t want a phouka to learn where your campsite is.  You help her into the bushes and tell her she just needs to hide out for a few days.' );
		} else if( CoC.player.cor <= 66 ) {
			MainView.outputText( 'You laugh and remind her that\'s what she\'s wanted to do to you.' );
		} else {
			MainView.outputText( 'Why shouldn\'t the other phouka get the chance to enjoy her?  Before she can fully recover you grab some tree limbs and vines.  You pin her to the ground and use them to tie her up tight.  With her arms and legs lashed together and her ass in the air she\'s an inviting target.' );
		}
	...
	PhoukaScene.prototype.phoukaSexFaerieFemalePostCombat = function() {
	...
	PhoukaScene.prototype.phoukaSexPregnate = function( postCombat ) { //Whether by horse, bunny or (male) faerie sex it all ends up here if the PC has a vagina
		if( CoC.player.isPregnant() ) {
			...
			if( CoC.player.cor > 50 ) {
				MainView.outputText( '  The feeling of all that seed flowing into you and then out of you again finally brings you to your own orgasm.[if (hasCock = true)  [EachCock] fires long strands of cum into the bog water beneath you.]' );
			} else {
				MainView.outputText( '  As the thick grey cum leaks from your cunt you finally give in and get yourself off.  There\'s no way you could have made it back to camp feeling like that.' );
			}
			if( this.phoukaForm === this.PHOUKA_FORM_FAERIE ) {
		...
		if( postCombat ) {
			MainView.outputText( ' rummages around in your gem pouch and then' );
		}
		MainView.outputText( ' takes to the air and hovers nearby as if waiting for something.\n\n' );
		if( CoC.player.isPregnant() ) {
			var isFirstTime = (CoC.flags[ kFLAGS.BIRTHS_PHOUKA ] === 0) && (CoC.flags[ kFLAGS.BIRTHS_FAERIE ] === 0);
			CoC.flags[ kFLAGS.PREGNANCY_CORRUPTION ] = 2; //Start off with a corrupt little phouka. It will take three drinks of pure honey to turn it into a faerie
			MainView.outputText( 'You\'re about to lift yourself up when you feel ' + (isFirstTime ? 'an unusual' : 'a familiar') + ' sensation inside your belly.  All that cum is hardening!  You can feel it set, almost like mortar left out in the sun!  You stand and some clear water drains out, your belly shrinking in size to that of a watermelon.  Whatever\'s left inside your belly is solid, like you\'re carrying a smooth ball of wood inside you.\n\nYou run a hand over your belly.  Thankfully it isn\'t any heavier than a normal pregnancy, nor does it hurt at all.\n\n<i>“Yes!”</i> shouts the ' + this.phoukaName() + '. <i>“I\'m gonna be a daddy!  Thanks fer letting me fill ya up girl, there’s plenty o’ space in there fer the little guy.  I gotta tell all the other phouka about ya' );
			if( CoC.flags[ kFLAGS.PHOUKA_LORE ] === 0 ) {
				CoC.flags[ kFLAGS.PHOUKA_LORE ] = 1;
			} //Now you know what to call them
			if( !postCombat || (CoC.player.cor > 50) ) {
				MainView.outputText( ', yer a real prize slut' );
			}
			MainView.outputText( '.”</i> The phouka gains more altitude and flies off deeper into the bog, whistling happily to himself.  ' );
			if( isFirstTime ) {
				MainView.outputText( 'You have to wonder how long this pregnancy will last. If this were a human child people would judge you to be at least four months along.' );
			} else {
				MainView.outputText( 'At least you know what to expect from this form of pregnancy now.' );
			}
		} else {
			MainView.outputText( 'When you pull yourself up your belly begins to deflate and leak its gooey grey payload into the mud.  The ' + this.phoukaName() + ' says <i>“Ah well, there\'s always next time,”</i> and buzzes away to some other part of the bog.' );
		}
		this.phoukaSexPregnateEnd( postCombat );
	};
	PhoukaScene.prototype.phoukaSexPregnateEnd = function( postCombat ) { //Everything after the sex. Handles awards, gem loss and text for player leaving the bog
		...
		if( this.phoukaForm === this.PHOUKA_FORM_FAERIE ) { //In this case postCombat means you need an award because you must have won to get faerie sex
			...
			if( postCombat && CoC.player.gems > 0 ) {
				MainView.outputText( '  While you\'re recovering the ' + this.phoukaName() + ' reaches into your gem pouch and takes a handful.' );
			}
			...
			if( this.phoukaForm === this.PHOUKA_FORM_HORSE && !CoC.player.isTaur() ) {
				MainView.outputText( 'wait for the throbbing pain in your pelvis to subside.  Then you ' );
			}
			if( postCombat ) {
			...
			if( CoC.player.cor <= 50 ) {
				MainView.outputText( 'As you trudge back to camp you have to wonder - why did you decide to visit the bog again?' );
			} else if( this.phoukaForm === this.PHOUKA_FORM_HORSE && CoC.player.isTaur() ) {
				MainView.outputText( '  It was quite the ride and you find yourself looking forward to your next trip into the bog.' );
			} else {
				MainView.outputText( 'Not what you expected, but at least you got off.' );
			}
		}
	};
	SceneLib.registerScene( 'phoukaScene', new PhoukaScene() );
} );