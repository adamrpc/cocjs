...
angular.module( 'cocjs' ).run( function( SceneLib, MainView, CoC, kFLAGS, PregnancyStore, EngineCore, AppearanceDefs ) {
	...
	FrogGirlScene.prototype.findTheFrogGirl = function() {
		...
		if( CoC.flags[ kFLAGS.TIMES_ENCOUNTERED_FROG ] === 0 ) {
			MainView.outputText( 'You begin your trudge through the bog, slogging through the soupy muck.  Insects buzz by every second, spinning in hectic spirals around your head.' );
			...
			MainView.outputText( 'While exploring the bog, you become aware of a familiar sound behind the buzzing— a musical chirping.  Step by step, you move toward it, pushing aside hanging curtains of gray moss until the dark trees of the swamp give way to the wide roots of mangroves.  The sporadic notes trickle into your ears, and you feel unusually relaxed as you move toward their source.' );
	...
	FrogGirlScene.prototype.followDatFrog = function() {
		MainView.clearOutput();
		MainView.outputText( 'You splash your way up the terrace of pools, shedding your [armor] as you do so, leaving your gear draped over the mangrove ridges.  She holds her arms out to you as you step naked into her pool.  She rises to meet you, and you notice that the water concealed a swollen, pregnant belly.' );
		...
		if( CoC.player.hasCock() ) {
			...
			MainView.outputText( 'your bare but sensitive groin' );
		}
	...
	FrogGirlScene.prototype.getFrogButtFilled = function() {
		MainView.clearOutput();
		MainView.outputText( 'You wake up an hour later and it takes a while for you remember anything.  Gradually, you piece the last hour together, remembering the frog girl, her hypnotic song, and hallucinogenic slime.  You’re floating on your back in a crystal-clear pool, and above you, between the mangrove canopies, Mareth’s sky drifting lazily far overhead.' );
	...
	FrogGirlScene.prototype.resistDatFrog = function() {
		MainView.clearOutput();
		MainView.outputText( 'You shake your head and drop to the ground, dunking your head in one of the crystal clear pools.  You emerge with your head clear, cold water dripping from your [hair], frowning at the frog girl.' );
	...
	FrogGirlScene.prototype.leaveFrogBe = function() {
		MainView.clearOutput();
		MainView.outputText( 'You shrug and leave the pouting frog girl in her pond, hopping back down the terrace of pools and walking back towards your camp, hoping that your [hair] will dry by the time you get back.' );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	//Question the frog girl
	FrogGirlScene.prototype.questDatFrogGirl = function() {
		MainView.clearOutput();
		MainView.outputText( 'You decide to question the pouting amphibian and take a seat at the edge of her pool, asking her why she just tried to lure you in.' );
	...
	FrogGirlScene.prototype.continueOnYourWay = function() {
		MainView.clearOutput();
		MainView.outputText( 'You nod in sympathy, patting the frog girl on her head, but decide not to inquire any further.  She sighs and sinks down further into the water, resigned to her maternal duties.' );
	...
	FrogGirlScene.prototype.carryBeeGirlsEggsVoluntarilyYouButtSlut = function() {
		MainView.clearOutput();
		MainView.outputText( 'Feeling sympathy for the frog girl, you volunteer to help with her burden.' );
	...
	FrogGirlScene.prototype.teachDatFrogALesson = function() {
		MainView.clearOutput();
		MainView.outputText( 'This frog bitch needs to be careful about who she lures in.' );
	...
	FrogGirlScene.prototype.lessonFollowup = function() {
		MainView.clearOutput();
		MainView.outputText( 'You wake up two hours later, floating alone in the pool, with a migraine and soggy clothes.  You slog your way out, clutching your head, and head back to camp.' );
		//[Toughness -1]
	...
	FrogGirlScene.prototype.superBonusFrogEggsInYerCooch = function() {
		MainView.clearOutput();
		MainView.outputText( '"<i>Wait, you want them where?</i>" asks the frog girl, incredulously.' );
		...
		if( CoC.player.tailType > AppearanceDefs.TAIL_TYPE_NONE ) {
			MainView.outputText( ' hooked over your tail,' );
		} else {
			MainView.outputText( ' clasped against your [ass],' );
		}
		MainView.outputText( ' smearing more and more of her slime against your body.' );
	...
	FrogGirlScene.prototype.layFrogEggs = function() {
		if( CoC.player.vaginas.length === 0 ) {
			MainView.outputText( 'You feel a terrible pressure in your groin... then an incredible pain accompanied by the rending of flesh.  You look down and behold vagina.  ', false );
			CoC.player.createVagina();
			CoC.player.genderCheck();
		}
		MainView.outputText( 'You shudder as you feel the familiar tingle of stimulant slime running down your thighs.' );
		...
		if( CoC.player.vaginas[ 0 ].vaginalWetness < AppearanceDefs.VAGINA_WETNESS_SLAVERING ) {
			...
			MainView.outputText( '  It rapidly becomes clear that it has.  <b>Your pussy is wetter than ever.</b>' );
		}
		if( CoC.player.hipRating < 25 ) {
			CoC.player.hipRating++;
			MainView.outputText( '  There\'s an extra thickness to your [hips] as well.' );
		}
		CoC.player.orgasm();
		EngineCore.dynStats( 'sen', 1 );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
