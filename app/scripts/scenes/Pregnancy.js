...
angular.module( 'cocjs' ).run( function( MainView, SceneLib, Mutations, StatusAffects, PregnancyStore, Descriptors, Utils, CockTypesEnum, CoC_Settings, PerkLib, AppearanceDefs, kFLAGS, CoC, EngineCore ) {
	...
	Pregnancy.prototype.updatePregnancy = function() {
		...
		if( CoC.player.pregnancyIncubation > 1 ) {
			...
			if( CoC.player.pregnancyType === PregnancyStore.PREGNANCY_IMP ) {
				...
				if( CoC.player.pregnancyIncubation === 216 ) {
					...
					if( CoC.player.cor < 40 ) {
						MainView.outputText( 'You are distressed by your unwanted pregnancy, and your inability to force this thing out of you.</b>', false );
					}
					if( CoC.player.cor >= 40 && CoC.player.cor < 75 ) {
						MainView.outputText( 'Considering the size of the creatures you\'ve fucked, you hope it doesn\'t hurt when it comes out.</b>', false );
					}
					if( CoC.player.cor >= 75 ) {
						MainView.outputText( 'You think dreamily about the monstrous cocks that have recently been fucking you, and hope that your offspring inherit such a pleasure tool.</b>', false );
					}
				...
				if( CoC.player.pregnancyIncubation === 72 ) {
					...
					if( CoC.player.cor < 40 ) {
						MainView.outputText( 'making it difficult to function.</b>', false );
					}
					if( CoC.player.cor >= 40 && CoC.player.cor < 75 ) {
						MainView.outputText( 'and you wonder how much longer you have to wait.</b>', false );
					}
					if( CoC.player.cor >= 75 ) {
						MainView.outputText( 'and you\'re eager to give birth, so you can get impregnated again by corrupted or monstrous cum filling out your eager womb.</b>', false );
					}
					MainView.outputText( '\n', false );
				...
				if( CoC.player.pregnancyIncubation === 48 ) {
					...
					if( CoC.player.cor < 40 ) {
						MainView.outputText( 'Afterwards you feel somewhat disgusted with yourself.</b>\n', false );
					}
					if( CoC.player.cor >= 40 && CoC.player.cor < 75 ) {
						MainView.outputText( 'You estimate you\'ll give birth in the next few days.</b>\n', false );
					}
					if( CoC.player.cor >= 75 ) {
						MainView.outputText( 'You find yourself daydreaming about birthing demons repeatedly, each time being re-impregnated by your hordes of lusty adolescent children.</b>\n', false );
					}
					displayedUpdate = true;
				}
			}
			...
			if( CoC.player.pregnancyType === PregnancyStore.PREGNANCY_MINOTAUR ) {
				...
				if( CoC.player.pregnancyIncubation === 72 ) {
					...
					if( CoC.player.cor >= 75 ) {
						MainView.outputText( 'and you\'re eager to give birth, so you can get impregnated again by monstrous cocks unloading their corrupted seed directly into your eager womb.</b>', false );
					}
					MainView.outputText( '\n', false );
					EngineCore.dynStats( 'spe', -3, 'lib', 1, 'sen', 1, 'lus', 4 );
					displayedUpdate = true;
				...
				if( CoC.player.pregnancyIncubation === 48 ) {
					...
					if( CoC.player.cor >= 75 ) {
						MainView.outputText( 'You find yourself daydreaming about birthing some huge monstrous beast, and raising it to fuck your wet pussy over and over.</b>\n', false );
					}
				}
				if( CoC.player.pregnancyIncubation === 32 || CoC.player.pregnancyIncubation === 64 || CoC.player.pregnancyIncubation === 85 || CoC.player.pregnancyIncubation === 150 ) {
					displayedUpdate = true;
			...
			if( CoC.player.pregnancyType === PregnancyStore.PREGNANCY_BUNNY ) {
				...
				if( CoC.player.pregnancyIncubation === 156 ) {
					...
					if( CoC.player.cor >= 40 && CoC.player.cor < 75 ) {
						MainView.outputText( 'You find yourself wondering what giving birth to bunny-girls is like.</b>', false );
					}
					if( CoC.player.cor >= 75 ) {
						MainView.outputText( 'You dreamily wonder if you could find a bunny willing to put more than two eggs inside you at once.</b>', false );
					}
					EngineCore.dynStats( 'spe', -1, 'lib', 1, 'sen', 1, 'lus', 2 );
					MainView.outputText( '\n', false );
					displayedUpdate = true;
				...
				if( CoC.player.pregnancyIncubation === 72 ) {
					...
					if( CoC.player.cor >= 75 ) {
						MainView.outputText( 'and you\'re eager to give birth so you\'ll be able to get pregnant again.</b>', false );
					}
					MainView.outputText( '\n', false );
					EngineCore.dynStats( 'spe', -3, 'lib', 1, 'sen', 1, 'lus', 4 );
					displayedUpdate = true;
				...
				if( CoC.player.pregnancyIncubation === 48 ) {
					...
					if( CoC.player.cor >= 75 ) {
						MainView.outputText( 'You find yourself daydreaming about birthing bunnies repeatedly, each time being re-impregnated with dozens of eggs from your lusty adolescent children.</b>\n', false );
					}
					displayedUpdate = true;
				}
			}
			...
			if( CoC.player.pregnancyType === PregnancyStore.PREGNANCY_MARBLE ) {
				...
				if( CoC.player.pregnancyIncubation === 72 ) {
					...
					if( CoC.player.cor >= 75 ) {
						MainView.outputText( 'and you\'re eager to give birth, so you can get fill your womb with a new charge.</b>', false );
					}
					MainView.outputText( '\n', false );
					EngineCore.dynStats( 'spe', -3, 'lib', 1, 'sen', 1, 'lus', 4 );
					displayedUpdate = true;
				...
				if( CoC.player.pregnancyIncubation === 85 ) {
					...
					if( CoC.player.biggestTitSize() <= 0 ) {
						MainView.outputText( 'your chest has begun to feel a bit odd.  Your run your hands over it to find that your breasts have grown to around C-cups at some point when you weren\'t paying attention!  ', false );
						CoC.player.breastRows[ 0 ].breastRating = 3;
					} else if( CoC.player.biggestTitSize() <= 1 ) {
						MainView.outputText( 'your breasts feel oddly tight in your top.  You put a hand to them and are startled when you find that they\'ve grown to C-cups!  ', false );
						CoC.player.breastRows[ 0 ].breastRating = 3;
					} else if( CoC.player.biggestTitSize() <= 10 ) {
						MainView.outputText( 'your breasts feel oddly full.  You grab them with your hands, and after a moment you\'re able to determine that they\'ve grown about a cup in size.  ', false );
						CoC.player.breastRows[ 0 ].breastRating++;
					} else {
						MainView.outputText( 'your breasts feel a bit odd.  You put a hand on your chest and start touching them.  ', false );
					}
					if( CoC.player.biggestLactation() < 1 ) {
						MainView.outputText( 'You gasp slightly in surprise and realize that you\'ve started lactating.', false );
						CoC.player.boostLactation( CoC.player.breastRows.length );
					} else {
						MainView.outputText( 'A few drips of milk spill out of your breasts, as expected.  Though, it occurs to you that there is more milk coming out than before.', false );
			...
			if( CoC.player.pregnancyType === PregnancyStore.PREGNANCY_MOUSE || CoC.player.pregnancyType === PregnancyStore.PREGNANCY_JOJO ) {
				...
				if( CoC.player.pregnancyIncubation === 48 ) {
					...
					if( CoC.player.cor >= 75 ) {
						MainView.outputText( 'You find yourself daydreaming about birthing hundreds of little babies, and lounging around while they nurse non-stop on your increasingly sensitive breasts.</b>\n', false );
					}
				}
				if( CoC.player.pregnancyIncubation === 32 || CoC.player.pregnancyIncubation === 64 || CoC.player.pregnancyIncubation === 85 || CoC.player.pregnancyIncubation === 150 ) {
					//Increase lactation!;
			...
			if( CoC.player.pregnancyType === PregnancyStore.PREGNANCY_AMILY ) {
				...
				if( CoC.player.pregnancyIncubation === 216 ) {
					...
					if( CoC.flags[ kFLAGS.AMILY_FOLLOWER ] === 1 ) {
						MainView.outputText( '  Amily smiles at you reassuringly. "<i>We do have litters, dear, this is normal.</i>"', false );
					}
					MainView.outputText( '</b>', false );
					MainView.outputText( '\n', false );
					EngineCore.dynStats( 'spe', -1, 'lib', 1, 'sen', 1, 'lus', 2 );
					displayedUpdate = true;
				}
			...
			if( CoC.player.pregnancyType === PregnancyStore.PREGNANCY_SPIDER || CoC.player.pregnancyType === PregnancyStore.PREGNANCY_DRIDER_EGGS ) {
				...
				if( CoC.player.pregnancyIncubation === 250 ) {
					...
					if( CoC.player.thickness > 60 && CoC.player.tone < 40 ) {
						MainView.outputText( ' even for you', false );
					}
					MainView.outputText( ', maybe you should cut back on all the food you\'ve been consuming lately?</b>\n', false );
					displayedUpdate = true;
				}
				if( CoC.player.pregnancyIncubation === 216 ) {
				...
				if( CoC.player.pregnancyIncubation === 72 ) {
					...
					if( CoC.player.cor < 50 ) {
						MainView.outputText( '  You shudder and shake your head, wondering why you\'re thinking such unusual things.', false );
					}
					MainView.outputText( '</b>\n', false );
					displayedUpdate = true;
				}
			...
			if( CoC.player.pregnancyType === PregnancyStore.PREGNANCY_ANEMONE ) {
				...
				if( CoC.player.pregnancyIncubation === 185 ) {
					...
					if( CoC.player.cor >= 40 && CoC.player.cor < 75 ) {
						MainView.outputText( 'Considering the possible fathers, you hope it isn\'t that big.</b>', false );
					}
					if( CoC.player.cor >= 75 ) {
						MainView.outputText( 'You think dreamily about the cocks that have recently been fucking you, and hope that your offspring inherit such a divine pleasure tool.</b>', false );
					}
					EngineCore.dynStats( 'spe', -1, 'lib', 1, 'sen', 1, 'lus', 2 );
					MainView.outputText( '\n', false );
					displayedUpdate = true;
				...
				if( CoC.player.pregnancyIncubation === 120 ) {
					...
					if( CoC.player.hasVagina() ) {
						MainView.outputText( ' and keeps your ' + Descriptors.vaginaDescript( 0 ) + ' aroused from the constant tingling in your womb', false );
					}
					MainView.outputText( '.</b>\n', false );
				...
				if( CoC.player.pregnancyIncubation === 72 ) {
					...
					if( CoC.player.cor < 40 ) {
						MainView.outputText( 'and constantly shifts and wriggles.  What manner of beast are you bringing into the world?</b>', false );
					}
					if( CoC.player.cor >= 40 && CoC.player.cor < 75 ) {
						MainView.outputText( 'and you wonder how much longer you have to wait.</b>', false );
					}
				...
				if( CoC.player.pregnancyIncubation === 48 ) {
					MainView.outputText( '\n<b>You rub your hands over your bulging belly, lost in the sensations of motherhood.  ', false );
					if( CoC.player.cor < 40 ) {
						MainView.outputText( 'Afterwards you feel somewhat disgusted with yourself, but horny.</b>\n', false );
					}
					if( CoC.player.cor >= 40 && CoC.player.cor < 75 ) {
						MainView.outputText( 'You estimate you\'ll give birth in the next few days.  You hope the birth is as erotically charged as the pregnancy has been.</b>\n', false );
					}
					if( CoC.player.cor >= 75 ) {
						MainView.outputText( 'You find yourself daydreaming  about birthing cilia-covered worms, orgasming each time their thousands of stingers brush by your clit and fill it full of sensation-enhancing drugs.</b>\n', false );
					}
			...
			//Frog Eggs;
				...
				if( CoC.player.pregnancyIncubation === 8 ) {
					...
					if( CoC.player.hasVagina() ) {
						...
						//pussy:;
						if( CoC.player.hasVagina() ) {
							MainView.outputText( '  Against your [vagina], the slime feels warm and cold at the same time, coaxing delightful tremors from your [clit].' );
						}//[balls:;
						else if( CoC.player.balls > 0 ) {
							MainView.outputText( '  Slathered in hallucinogenic frog slime, your balls tingle, sending warm pulses of pleasure all the way up into your brain.' );
						}
						//genderless: ;
						else {
							MainView.outputText( '  Your [vagina] begins twitching, aching for something to push through it over and over again.' );
						}
						MainView.outputText( '  Seated in your own slime, you moan softly, unable to keep your hands off yourself.' );
			...
			if( CoC.player.buttPregnancyType === PregnancyStore.PREGNANCY_FROG_GIRL ) {
				if( CoC.player.buttPregnancyIncubation === 8 ) {
					...
					//[cock:;
						...
						MainView.outputText( '  Splashing against the underside of your ' + CoC.player.multiCockDescriptLight() + ', the slime leaves a warm, oozy sensation that makes you just want to rub [eachCock] over and over and over again.' );
					}//genderless: ;
					else {
						MainView.outputText( '  Your asshole begins twitching, aching for something to push through it over and over again.' );
					}
					MainView.outputText( '  Seated in your own slime, you moan softly, unable to keep your hands off yourself.' );
					EngineCore.dynStats( 'lus=', 100, 'resisted', false );
					displayedUpdate = true;
			...
			if( CoC.player.buttPregnancyType === PregnancyStore.PREGNANCY_BEE_EGGS ) {
				...
				if( CoC.player.buttPregnancyIncubation === 20 ) {
					...
					if( CoC.player.cockTotal() > 0 ) {
						MainView.outputText( 'The aroma seems to permeate your very being, slowly congregating in your ', false );
					}
					if( CoC.player.cockTotal() === 1 ) {
						MainView.outputText( Descriptors.cockDescript( 0 ), false );
						if( CoC.player.horseCocks() === 1 ) {
							MainView.outputText( ', each inhalation making it bigger, harder, and firmer.  You suck in huge lungfuls of air, until your ' + Descriptors.cockDescript( 0 ) + ' is twitching and dripping, the flare swollen and purple.  ', false );
						}
						if( CoC.player.dogCocks() === 1 ) {
							MainView.outputText( ', each inhalation making it thicker, harder, and firmer.  You suck in huge lungfuls of air, desperate for more, until your ' + Descriptors.cockDescript( 0 ) + ' is twitching and dripping, its knot swollen to the max.  ', false );
						}
						if( CoC.player.normalCocks() === 1 ) {
							MainView.outputText( ', each inhalation making it bigger, harder, and firmer.  You suck in huge lungfuls of air, until your ' + Descriptors.cockDescript( 0 ) + ' is twitching and dripping, the head swollen and purple.  ', false );
						}
						//FAILSAFE FOR NEW COCKS;
						if( CoC.player.normalCocks() === 0 && CoC.player.dogCocks() === 0 && CoC.player.horseCocks() === 0 ) {
							MainView.outputText( ', each inhalation making it bigger, harder, and firmer.  You suck in huge lungfuls of air until your ' + Descriptors.cockDescript( 0 ) + ' is twitching and dripping.  ', false );
						}
					}
					if( CoC.player.cockTotal() > 1 ) {
						MainView.outputText( 'groin.  Your ' + CoC.player.multiCockDescriptLight() + ' fill and grow with every lungful of the stuff you breathe in.  You suck in great lungfuls of the tainted air, desperate for more, your cocks twitching and dripping with need.  ', false );
					}
					MainView.outputText( 'You smile knowing you couldn\'t stop from masturbating if you wanted to.\n', false );
			...
			if( CoC.player.buttPregnancyType === PregnancyStore.PREGNANCY_SANDTRAP || CoC.player.buttPregnancyType === PregnancyStore.PREGNANCY_SANDTRAP_FERTILE ) {
				if( CoC.player.buttPregnancyIncubation === 36 ) {
					...
					if( CoC.player.buttPregnancyType === PregnancyStore.PREGNANCY_SANDTRAP_FERTILE ) {
						MainView.outputText( ' bloated and full; the sensation isn\'t entirely unpleasant.' );
					} else {
						MainView.outputText( 'increasingly empty, as though some obstructions inside you were being broken down.' );
		...
		if( CoC.player.buttPregnancyIncubation === 1 && CoC.player.buttPregnancyType === PregnancyStore.PREGNANCY_BEE_EGGS ) {
			MainView.outputText( '\n', false );
			MainView.outputText( 'There is a sudden gush of honey-colored fluids from your ass.  Before panic can set in, a wonderful scent overtakes you, making everything ok.  ', false );
			if( CoC.player.cockTotal() > 0 ) {
				MainView.outputText( 'The muzzy feeling that fills your head seems to seep downwards, making your equipment hard and tight.  ', false );
			}
			if( CoC.player.vaginas.length > 0 ) {
				MainView.outputText( 'Your ' + Descriptors.vaginaDescript( 0 ) + ' becomes engorged and sensitive.  ', false );
			}
			MainView.outputText( 'Your hand darts down to the amber, scooping up a handful of the sticky stuff.  You wonder what your hand is doing as it brings it up to your mouth, which instinctively opens.  You shudder in revulsion as you swallow the sweet-tasting stuff, your mind briefly wondering why it would do that.  The stuff seems to radiate warmth, quickly pushing those nagging thoughts away as you scoop up more.\n\n', false );
			MainView.outputText( 'A sudden slip from below surprises you; a white sphere escapes from your anus along with another squirt of honey.  Your drugged brain tries to understand what\'s happening, but it gives up, your hands idly slathering honey over your loins.  The next orb pops out moments later, forcing a startled moan from your mouth.  That felt GOOD.  You begin masturbating to the thought of laying more eggs... yes, that\'s what those are.  You nearly cum as egg number three squeezes out.  ', false );
			if( CoC.player.averageLactation() >= 1 && CoC.player.biggestTitSize() > 2 ) {
				MainView.outputText( 'Seeking even greater sensation, your hands gather the honey and massage it into your ' + Descriptors.breastDescript( 0 ) + ', slowly working up to your nipples.  Milk immediately begins pouring out from the attention, flooding your chest with warmth.  ', false );
			}
			MainView.outputText( 'Each egg seems to come out closer on the heels of the one before, and each time your conscious mind loses more of its ability to do anything but masturbate and wallow in honey.\n\n', false );
			MainView.outputText( 'Some time later, your mind begins to return, brought to wakefulness by an incredibly loud buzzing...  You sit up and see a pile of dozens of eggs resting in a puddle of sticky honey.  Most are empty, but a few have hundreds of honey-bees emptying from them, joining the massive swarms above you.  ', false );
			if( CoC.player.cor < 35 ) {
				MainView.outputText( 'You are disgusted, but glad you were not stung during the ordeal.  You stagger away and find a brook to wash out your mouth with.', false );
			}
			if( CoC.player.cor >= 35 && CoC.player.cor < 65 ) {
				MainView.outputText( 'You are amazed you could lay so many eggs, and while the act was strange there was something definitely arousing about it.', false );
			}
			if( CoC.player.cor >= 65 && CoC.player.cor < 90 ) {
				MainView.outputText( 'You stretch languidly, noting that most of the drugged honey is gone.  Maybe you can find the Bee again and remember to bottle it next time.', false );
			}
			if( CoC.player.cor >= 90 ) {
				MainView.outputText( 'You lick your lips, savoring the honeyed residue on them as you admire your thousands of children.  If only every night could be like this...\n', false );
			}
			CoC.player.buttKnockUpForce(); //Clear Butt Pregnancy
			CoC.player.orgasm();
		...
		if( CoC.player.pregnancyType === PregnancyStore.PREGNANCY_ANEMONE && CoC.player.pregnancyIncubation === 1 ) {
			...
			//[(if pc has 0-9 existing cocks);
				...
				//[(if cocks);
				if( CoC.player.cockTotal() > 0 ) {
					MainView.outputText( ' below your ' + CoC.player.multiCockDescriptLight(), false );
				}
				MainView.outputText( '. A sharp pinch lances through the nerves in your groin and sends your hands to it reflexively.  This smaller pain, coupled with the adrenaline and dopamine that have finally chased the fog from your head, is enough to pull your thoughts into focus for another attempt to remove your strange, parasitic offspring.  You shift your grip and pull a few more times, but the thing doesn\'t budge.  The handling of it only serves to make the stalk thicken and become stiff; gradually you notice that you\'re feeling the sensation of your own pulling not from the skin at the point of attachment but from the stalk itself, and this realization is accompanied by the ring of tentacles opening and pulling back to reveal the crown of a penis!  <b>You have a new anemone-penis!</b>', false );
				//[(dick slot 1 exists);
				if( CoC.player.cockTotal() > 0 ) {
					MainView.outputText( '  The tentacles writhe around, rubbing against your ' + CoC.player.multiCockDescriptLight(), false );
				}//(doesn't exist);
				else {
					MainView.outputText( '  The tentacles curl inwards, rubbing on the head of your new blue pecker', false );
				}
				CoC.player.createCock( (4 + Utils.rand( 3 )), 1.2 );
				CoC.player.cocks[ CoC.player.cockTotal() - 1 ].cockType = CockTypesEnum.ANEMONE;
				MainView.outputText( ' and you quickly become fully erect from the aphrodisiac they inject.  Over and over the tentacles caress ' + CoC.player.sMultiCockDesc() + ' sensually, leaving behind a tingling trail of vibrant pleasure', false );
				//[(if no dick1 and no balls);
				if( CoC.player.totalCocks() === 1 && CoC.player.balls === 0 ) {
					MainView.outputText( '; you feel a pressure build below the shaft, near your asshole', false );
				}
				MainView.outputText( '.  As the venom and the rubbing work you to the edge of climax, your muscles clench and a ', false );
				if( CoC.player.cumQ() < 100 ) {
					MainView.outputText( 'glob', false );
				} else if( CoC.player.cumQ() < 500 ) {
					MainView.outputText( 'squirt', false );
				} else {
					MainView.outputText( 'spray', false );
				}
				MainView.outputText( ' of semen shoots from your new penis and lands on your ', false );
				//[(if boobs);
				if( CoC.player.biggestTitSize() >= 1 ) {
					MainView.outputText( Descriptors.allBreastsDescript() + ' and ', false );
				}
				MainView.outputText( 'stomach', false );
				//[(dick1 exists);
				if( CoC.player.cockTotal() > 1 ) {
					MainView.outputText( ', followed in short order by white squirts from ' + CoC.player.sMultiCockDesc() + ' remaining', false );
				}
				MainView.outputText( '.  Your ' + Descriptors.vaginaDescript( 0 ) + ' quivers and pulses as well, adding ', false );
				if( CoC.player.vaginas[ 0 ].vaginalWetness < AppearanceDefs.VAGINA_WETNESS_SLICK ) {
					...
					MainView.outputText( 'nearly a cupful of fluid', false );
				}
				MainView.outputText( ' from your female orgasm to the puddle on the ground below your ass.\n\n', false );
			...
			CoC.player.knockUpForce(); //Clear Pregnancy
			MainView.outputText( 'Exhausted by the \'birth\' and the climax, you slip into a doze.\n', false );
		}
		...
		if( CoC.player.pregnancyIncubation === 1 && CoC.player.pregnancyType === PregnancyStore.PREGNANCY_IMP ) {
			...
			MainView.outputText( 'A sudden gush of fluids erupts from your vagina - your water just broke.  You grunt painfully as you feel wriggling and squirming inside your belly, muscle contractions forcing it downwards.  ', false );
			if( CoC.player.cor < 50 ) {
				MainView.outputText( 'You rue the day you encountered that hateful imp.  ', false );
			}
			MainView.outputText( 'The pain begins to subside as your delivery continues... replaced with a building sensation of pleasure.  Arousal spikes through you as the contractions intensify, and as you feel something pass you have a tiny orgasm.\n\nYet you feel more within you, and the contractions spike again, pushing you to orgasm as you pass something else.  It repeats, over and over, nearly a dozen times you birth and orgasm.  After an eternity of procreation and pleasure, you sense your ordeal is over and collapse, unconscious.', false );
			...
			if( CoC.player.averageLactation() > 0 && CoC.player.averageLactation() < 5 ) {
				MainView.outputText( '  Your breasts won\'t seem to stop dribbling milk, lactating more heavily than before.', false );
				CoC.player.boostLactation( 0.5 );
			}
			//Lactate if large && not lactating;
			if( CoC.player.biggestTitSize() >= 3 && CoC.player.mostBreastsPerRow() > 1 && CoC.player.averageLactation() === 0 ) {
				MainView.outputText( '  As you ponder the implications, <b>you realize your breasts have been slowly lactating</b>.  You wonder how much longer it will be before they stop.', false );
				CoC.player.boostLactation( 1 );
			}
			CoC.player.boostLactation( 0.01 );
			//Enlarge if too small for lactation;
			if( CoC.player.biggestTitSize() === 2 && CoC.player.mostBreastsPerRow() > 1 ) {
				MainView.outputText( '  <b>Your breasts have grown to C-cups!</b>', false );
				CoC.player.growTits( 1, 1, false, 3 );
			}
			//Enlarge if really small!;
			if( CoC.player.biggestTitSize() === 1 && CoC.player.mostBreastsPerRow() > 1 ) {
				MainView.outputText( '  <b>Your breasts have grown to B-cups!</b>', false );
				CoC.player.growTits( 1, 1, false, 3 );
			}
		...
		if( CoC.player.pregnancyIncubation === 1 && CoC.player.pregnancyType === PregnancyStore.PREGNANCY_MARBLE ) {
			...
			if( CoC.flags[ kFLAGS.MARBLE_NURSERY_CONSTRUCTION ] < 100 ) {
				...
				MainView.outputText( 'Marble seems to understand what happened, but is really disappointed with you, "<i>Sweetie, why couldn\'t you wait until after I\'d finished the nursery...?</i>"', false );
				...
				//it's a boy!;
				{
					...
					MainView.outputText( 'The little boy is already starting to look like he is a few years old; he’s trotting around on his little hoofs.', false );
					//Increase the size of the PC’s hips, as per normal for pregnancies, increase birth counter;
					if( CoC.player.hipRating < 10 ) {
						CoC.player.hipRating++;
						MainView.outputText( 'After the birth your ' + CoC.player.armorName + ' fits a bit more snugly about your ' + Descriptors.hipDescript() + '.', false );
					}
					if( CoC.flags[ kFLAGS.MARBLE_BOYS ] === 0 )
					//has Marble had male kids before?;
					{
						MainView.outputText( 'You notice that Marble seems to be deep in thought, and you ask her what is wrong.  She starts after a moment and says, "<i>Oh sweetie, no, it\'s nothing really.  I just never thought that I\'d actually be able to father a son is all.  The thought never occurred to me.', false );
					}
					//Add to marble-kids:;
				...
				//it's a girl!;
				{
					...
					MainView.outputText( 'You give an almighty heave and finally manage to push the shoulders out. The rest is downhill from there.  Once you\'ve pushed the child completely out, Marble lays you down on the ground.  You rest there for a few moments, trying to catch your breath after the relatively difficult birthing.  When you finally have a chance to get up, you see that Marble has a small cowgirl cradled in her arms, suckling on her nipple.  You can hardly believe that you managed to push out a girl that big!  Marble seems to understand and tells you that the child is actually a fair bit bigger now than when she came out.\n\n', false );
					MainView.outputText( 'She helps you stand up and gives you the little girl to suckle for yourself.  ', false );
					if( CoC.player.statusAffectv4( StatusAffects.Marble ) >= 20 ) {
						MainView.outputText( 'As the child contentedly drinks from your ' + Descriptors.nippleDescript( 0 ) + ', Marble tells you, "<i>Sweetie, somehow I know that our kids won\'t have to worry about the addictive milk.  It will only make them healthy and strong.</i>"  You nod at her and put the child into the nursery.  ', false );
					} else {
						MainView.outputText( 'You put the child to your breast and let her drink down your milk.  You sigh in contentment and Marble says, "<i>See sweetie?  It\'s a really good feeling, isn\'t it?</i>"  You can\'t help but nod in agreement.  After a minute the little girl has had enough and you put her into the nursery.\n\n', false );
					}
					MainView.outputText( 'The little girl is already starting to look like she is a few years old; she\'s trotting around on her little hooves.', false );
					//Add to marble-kids:;
					CoC.flags[ kFLAGS.MARBLE_KIDS ]++;
				}
				//Increase the size of the PC's hips, as per normal for pregnancies, increase birth counter;
		...
		if( CoC.player.pregnancyIncubation === 1 && CoC.player.pregnancyType === PregnancyStore.PREGNANCY_AMILY ) {
			...
			MainView.outputText( '\n', false );
			if( CoC.player.vaginas.length === 0 ) {
				MainView.outputText( 'You feel a terrible pressure in your groin... then an incredible pain accompanied by the rending of flesh.  <b>You look down and behold a new vagina</b>.  ', false );
				CoC.player.createVagina();
				CoC.player.genderCheck();
			}
		...
		if( CoC.player.pregnancyType === PregnancyStore.PREGNANCY_OVIELIXIR_EGGS && CoC.player.pregnancyIncubation > 0 ) {
			...
			if( CoC.player.pregnancyIncubation === 1 ) {
				...
				//Large egg scene;
					...
					MainView.outputText( 'A sudden shift in the weight of your pregnant belly staggers you, dropping you to your knees.  You realize something is about to be birthed, and you shed your ' + CoC.player.armorName + ' before it can be ruined by what\'s coming.  A contraction pushes violently through your midsection, ', false );
					if( CoC.player.vaginas[ 0 ].vaginalLooseness < AppearanceDefs.VAGINA_LOOSENESS_LOOSE ) {
						MainView.outputText( 'stretching your tight cunt painfully, the lips opening wide ', false );
					}
					if( CoC.player.vaginas[ 0 ].vaginalLooseness >= AppearanceDefs.VAGINA_LOOSENESS_LOOSE && CoC.player.vaginas[ 0 ].vaginalLooseness <= AppearanceDefs.VAGINA_LOOSENESS_GAPING_WIDE ) {
						MainView.outputText( 'temporarily stretching your cunt-lips wide-open ', false );
					}
					if( CoC.player.vaginas[ 0 ].vaginalLooseness > AppearanceDefs.VAGINA_LOOSENESS_GAPING_WIDE ) {
						MainView.outputText( 'parting your already gaping lips wide ', false );
					}
					...
					if( CoC.player.clitLength > 5 ) {
						MainView.outputText( 'your massive cock-like clit, jacking it off with the slimy birthing fluids as lube.   It pulses and twitches in time with your heartbeats, its sensitive surface overloading your fragile mind with pleasure.  ', false );
					}
					if( CoC.player.clitLength > 2 && CoC.player.clitLength <= 5 ) {
						MainView.outputText( 'your large clit like a tiny cock, stroking it up and down between your slime-lubed thumb and fore-finger.  It twitches and pulses with your heartbeats, the incredible sensitivity of it overloading your fragile mind with waves of pleasure.  ', false );
					}
					if( CoC.player.clitLength <= 2 ) {
						MainView.outputText( 'your ' + Descriptors.vaginaDescript( 0 ) + ' by pulling your folds wide and playing with your clit.  Another egg pops free from your diminishing belly, accompanied by an audible burst of relief.  You make wet \'schlick\'ing sounds as you spread the slime around, vigorously frigging yourself.  ', false );
					}
					MainView.outputText( 'You cum hard, the big eggs each making your cunt gape wide just before popping free.  You slump down, exhausted and barely conscious from the force of the orgasm.  ', false );
					if( CoC.player.statusAffectv3( StatusAffects.Eggs ) >= 11 ) {
						MainView.outputText( 'Your swollen belly doesn\'t seem to be done with you, as yet another egg pushes its way to freedom.   The stimulation so soon after orgasm pushes you into a pleasure-stupor.  If anyone or anything discovered you now, they would see you collapsed next to a pile of eggs, your fingers tracing the outline of your ' + Descriptors.vaginaDescript( 0 ) + ' as more and more eggs pop free.  In time your wits return, leaving you with the realization that you are no longer pregnant.  ', false );
					}
