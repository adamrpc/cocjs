...
angular.module( 'cocjs' ).run( function( SceneLib, SandMother, PregnancyStore, MainView, CumWitch, Combat, ConsumableLib, Utils, PerkLib, StatusAffects, Descriptors, CockTypesEnum, OnLoadVariables, AppearanceDefs, kFLAGS, CoC, EngineCore ) {
	...
	DungeonSandWitch.prototype.leaveBoobsDungeon = function() {
		...
		MainView.outputText( 'You leave the door behind and take off through the desert back towards camp.' );
	...
	DungeonSandWitch.prototype.openZeDoorToParadize = function() {
		...
		if( CoC.flags[ kFLAGS.ENTERED_SANDWITCH_DUNGEON ] === 0 ) {
			MainView.outputText( 'You hesitantly touch the dark sphere, admiring its smooth, glossy finish.  Almost as soon as you come in contact with it, it recedes into the wall.  The doorway rumbles, a giant slab vanishing into the sandy depths, opening a portal to the inside.  Meticulous carvings inlaid with pearl depict large breasted witches in great quantity, and though the specific means of the glyphs are foreign to you, it\'s clear this place is some kind of sanctuary for sand witches.' );
			CoC.flags[ kFLAGS.ENTERED_SANDWITCH_DUNGEON ] = 1;
		}
		//Repeat;
		else {
			MainView.outputText( 'Just ahead is the familiar sight of the sand witches\' coven.  It\'s hewn from a sandstone archway buried in the side of a dune.  Pearl-inlays of big-breasted, lactating witches decorate the way inside, making it clear what you can expect to find inside.' );
		}
	...
	DungeonSandWitch.prototype.loseToSammitchMob = function() {
		if( CoC.player.hasCock() && CoC.player.biggestCockArea() >= 6 ) {
	...
	DungeonSandWitch.prototype.getMockedForSmallDongBySammitchMob = function() {
		...
		if( CoC.player.cumQ() < 250 ) {
			MainView.outputText( '  A little squirt of spooge dirties the witch\'s two fingers, creating a small web of spunk between them.  "<i>Heh, knew it</i>" the witch smugly says, giving you a pathetic look.' );
		} else if( CoC.player.cumQ() < 500 ) {
			MainView.outputText( '  Spurts of sweet release shoot from your [cock smallest], glopping the bitch-witch\'s hand.  "<i>Nothing too impressive</i>" she snidely remarks as she rubs your spunk off on the dirt floor.' );
		} else {
			MainView.outputText( '  Your [cock smallest] shoots out a barrage of cum onto the bitch\'s face, caking her disbelieving features into a thick layer of slimy goop as her fellow sisters laugh, pointing at her spunk-covered face.' );
		}
		...
		if( CoC.player.balls > 0 ) {
			MainView.outputText( '  Your [balls] didn\'t have enough time to make another full batch.' );
		}
		MainView.outputText( '  It\'s very watery, dripping down between her toes. Your cock has a dirty smudge of where her foot was.' );
		...
		if( CoC.player.isBiped() ) {
			...
			MainView.outputText( 'to your side' );
		}
		MainView.outputText( '.  She takes your dirty cock in her hand.  She must have been dared to do something.  Looking at her peers with pleading eyes, she says, "<i>Are you sure about this?</i>"  A small group of witches egg her on, telling her to "<i>do it.</i>"' );
		...
		if( CoC.player.balls > 0 && CoC.player.hasVagina() ) {
			MainView.outputText( '  She even cups your [balls] but moves to touch your [vagina] when she feels its wetness dripping off the testes.' );
		} else if( CoC.player.balls > 0 ) {
			MainView.outputText( '  She even cups your tender [balls] and lightly massages them.' );
		} else if( CoC.player.hasVagina() ) {
			MainView.outputText( '  She even lightly caresses the lips of your pussy.' );
		}
	...
	DungeonSandWitch.prototype.femaleGirlsLoseToSammitches = function() {
		...
		if( CoC.player.hasMuzzle() ) {
			MainView.outputText( '  She fucks your muzzle like a cock as it goes into one cunt while she plays with the other one.' );
		} else {
			MainView.outputText( '  She grinds her dampening pussies against your lips while her clits are pressed up against your nose.' );
		}
		...
		if( !CoC.player.hasVagina() ) {
			MainView.outputText( 'Amazingly, as the oil spreads over your crotch, the flesh splits, revealing a small, wet entrance with a little clit.' );
			CoC.player.createVagina();
			CoC.player.clitLength = 0.25;
		}
		MainView.outputText( 'Their warm streams run all over your form: down your [chest], off your [hips] and teasingly around your [vagina].  At this point, you want it so badly that you squeeze your chest, pinch your nipples and stroke your [clit].' );
		...
		if( !CoC.player.hasMuzzle() ) {
			MainView.outputText( 'grinding on your face' );
		} else {
			MainView.outputText( 'fucking your muzzle while she plays with herself' );
		}
		MainView.outputText( ' and the four others whining out their enjoyment, making your own pussy feel neglected and needing something to fill it.' );
		...
		if( CoC.player.biggestTitSize() >= 1 ) {
			MainView.outputText( 'between' );
		} else {
			MainView.outputText( 'on' );
		}
		MainView.outputText( ' your [chest].' );
		MainView.outputText( '\n\n"<i>Hey girls, hurry up and pick a cock already!  The ' );
		if( CoC.flags[ kFLAGS.SAND_WITCHES_FRIENDLY ] > 0 ) {
			MainView.outputText( 'slut' );
		} else {
			MainView.outputText( 'initiate' );
		}
		MainView.outputText( ' is getting a bit randy!</i>"' );
		if( CoC.flags[ kFLAGS.SAND_WITCHES_FRIENDLY ] > 0 ) {
		...
		EngineCore.menu();
		if( CoC.flags[ kFLAGS.SAND_WITCHES_FRIENDLY ] > 0 ) {
			EngineCore.addButton( 0, 'Next', this, this.sandWitchMobNotBadEnd );
		} else {
			EngineCore.addButton( 0, 'Next', this, this.sandWitchMobBadEnd );
		}
	};
	//[if for fun];
	DungeonSandWitch.prototype.sandWitchMobNotBadEnd = function() {
		MainView.clearOutput();
		MainView.outputText( 'You wake up hours later outside the dungeon with all your belongings, surprisingly not missing any gems.' );
		OnLoadVariables.dungeonLoc = 0; //Replaces	inDungeon = false;
		CoC.player.orgasm();
		EngineCore.dynStats( 'lib', 1, 'sen', -1 );
		Combat.cleanupAfterCombat();
	...
	DungeonSandWitch.prototype.memeberedFolksFindTrueWuv = function() {
		...
		if( CoC.player.cockTotal() === 2 ) {
			MainView.outputText( ' while another grabs your other one' );
		} else if( CoC.player.cockTotal() > 2 ) {
			MainView.outputText( ' while others latch onto the rest' );
		}
		MainView.outputText( ', and the stroking starts up again with an immediacy that\'s too much for your overstimulated body.  [EachCock] belches out a stream of white liquid love, straight into the crowd of giggling women, but they don\'t seem to mind you stickying their hands, robes, and skin with your spermy deposits.  It\'s an intriguing new experience for them - it must have been some time since they\'ve had the chance to dominate a penis so completely.  Moaning, you bounce along in an orgasmic haze, spurting the whole way into your new home, not registering your own arrival until the splooge-slicked palms release your ' + CoC.player.multiCockDescriptLight() + ' and leave you to blurt a small rope onto your own belly.' );
		...
		if( CoC.player.balls > 0 ) {
			MainView.outputText( ' while your [balls] start to work, clenching and relaxing as they brew a thick load of fuckbatter thanks to the chemical jump-start.' );
		} else {
			MainView.outputText( ' while your body starts to work, clenching and relaxing as it rushes to brew a thick load of fuckbatter thanks to the chemical jump-start.' );
		}
		...
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( '  Similarly textured cunnies playfully snake around your other phallus' );
			if( CoC.player.cockTotal() > 2 ) {
				MainView.outputText( 'es' );
			}
			MainView.outputText( ', gleefully slurping up your twitching hardness inside ' );
			if( CoC.player.cockTotal() === 2 ) {
				MainView.outputText( 'itself' );
			} else {
				MainView.outputText( 'themselves' );
			}
			MainView.outputText( '.' );
		}
		MainView.outputText( 'Slurping noisily, the cunt' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 's piston' );
		} else {
			MainView.outputText( ' piston' );
		}
		MainView.outputText( ' and bob, splattering ' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 'their' );
		} else {
			MainView.outputText( 'its' );
		}
		MainView.outputText( ' dripping saliva all over your exposed crotch.  Your [hips] lurch up to meet the hungry tentacles.  Feeding the creature\'s cum-lust is something your body seems driven to do at this point, and you gurgle in wordless delight at her ministrations.' );
	...
	DungeonSandWitch.prototype.yoYouBeatUpSomeSandWitchesYOUMONSTER = function() {
		...
		MainView.outputText( 'The sand witches all collapse to the floor in a vast puddle of milk and pussy juice, ' );
		if( CoC.monster.HP < 1 ) {
			MainView.outputText( 'nursing their wounds' );
		} else {
			MainView.outputText( 'frantically making out with each other, huddling into a squirming orgy on the floor' );
		}
		MainView.outputText( '.  You stand alone, the sole victor, triumphant against your enemies; you can easily continue deeper into the caves, now.' );
		...
		if( CoC.player.lust >= 33 ) {
			...
			if( CoC.player.hasCock() ) {
				EngineCore.addButton( 0, 'Fuck One', this, this.dicksHaveSexWithAWitch );
			}
			if( CoC.player.hasVagina() ) {
				EngineCore.addButton( 1, 'Forced Lick', this, this.forceCunnilingusRimjobClitAndNipple );
			}
		}
	...
	DungeonSandWitch.prototype.dicksHaveSexWithAWitch = function() {
		...
		if( CoC.monster.lust > 99 ) {
			MainView.outputText( 'masturbating companions' );
		} else {
			MainView.outputText( 'groaning companions' );
		}
		MainView.outputText( ' for advice but finds none, and with no other choice, she takes your hand and allows you to separate her from her kin.  You put a hand to her cheek and tell her that you won\'t hurt her.  She and her kind have incited a great deal of lust in you, and while you intend to vent it on her, you don\'t see why it has to be unpleasant for her.' );
		if( CoC.monster.lust > 99 ) {
			MainView.outputText( '  Judging by the omnipresent flush on her cheeks and the scent of female honey inundating the air, she\'s more than ready to help you with your problem.' );
		} else {
			MainView.outputText( '  Her cheekbones slowly color at the thought, turning almost entirely beet-red.  At the same time, her nipples seem to tighten beneath her robes, signalling her body\'s acceptance of your intent.' );
		}
		...
		MainView.outputText( '\n\nCircling behind the lactating spellcaster, you undress one-handed while you fondle her.  Her perky, hard nipple drips faster and faster as her breast is squeezed and fondled with increasing vigor, your touches spurred on by the burgeoning heat in your ' + CoC.player.multiCockDescriptLight() + '.  Discarding your [armor], you kiss her delicate shoulders and neck, lingering at the edge of her jaw before you suckle her earlobe briefly.  She shivers against you but doesn\'t pull away.  In fact, her back arches to press her more firmly against you.  The sorceress practically melts into your arms when you bring your other hand to bear on the other side of her chest.' );
		MainView.outputText( '\n\n[EachCock] rises up ' );
		if( CoC.player.tallness < 60 ) {
			MainView.outputText( 'through her buttcheeks' );
		} else {
			MainView.outputText( 'along her arched spine' );
		}
		MainView.outputText( ', fully engorged and ready for action.  For now, you\'re content to continue to molest your chosen fuck-toy, dipping a pair of fingers into her ' );
		if( CoC.monster.lust > 99 ) {
			MainView.outputText( 'well-soaked' );
		} else {
			MainView.outputText( 'rapidly-moistening' );
		}
		MainView.outputText( ' cunts in order to make sure she\'s prepared.  The witch bites her lip to stifle a lewd moan, so you drag the pads of your fingers across each of her clits simultaneously.  This shatters whatever restraint she was showing into a little more than slick cunt-juice.  Even now, it\'s dripping down her inner thighs.  She\'s properly moaning with your strokes now, and you judge she\'s ready.' );
		//DAT SHIT FITS;
		if( CoC.player.smallestCockArea() <= CoC.monster.vaginalCapacity() ) {
			...
			if( y >= 0 ) {
				MainView.outputText( ' and your ' + Descriptors.cockDescript( y ) + ' aimed at her other one' );
			}
			MainView.outputText( '.  Her knees buckle, but you\'re ready for it, and she slides straight onto your supporting shaft' );
			if( y >= 0 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ', impaling herself to the hilt.' );
			...
			if( y < 0 ) {
				MainView.outputText( 'pussy clings tight to you' );
			} else {
				MainView.outputText( 'pussies cling tightly to you' );
			}
			MainView.outputText( '.  Supported entirely by your body, the sand witch\'s muscles go slack (aside from the ones hugging your dick' );
			if( y >= 0 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ').  She\'s breathing nice and heavy as you lift her and swing your hips back, pulling most of your ' + Descriptors.cockDescript( x ) );
			if( y >= 0 ) {
				MainView.outputText( ' and ' + Descriptors.cockDescript( y ) );
			}
			MainView.outputText( ' out before reversing the motion nigh-instantly, slamming yourself in to the hilt.  The once proud enchantress is reduced to a quivering puddle of fuck by your jackhammering thrusts, so taken by the sensations in her quim' );
			if( y >= 0 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ' that her brains are practically drooling out of her cunts.' );
			...
			if( y < 0 ) {
				...
				MainView.outputText( 'double dongs' );
			}
			MainView.outputText( '.  Waves of rippling contractions sheath you in shimmering, ephemeral ecstasy.  Like something clicking into place in your brain, a biological prerogative asserts itself, unleashing a heat that boils up out of your core like lava from a volcano.  [EachCock] spasms and releases a flow of salty jism.  The witch must feel it, because as soon as your seed makes contact with her juicy cunt' );
			if( y >= 0 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ', her clingy lips seem to tighten around your girth to seal it in.' );
			if( CoC.player.cumQ() >= 400 ) {
				MainView.outputText( '  That fails to contain the totality of your virile offshoots, and ribbons of the stuff squirt out the sides of her lips.' );
				if( CoC.player.cumQ() < 2000 ) {
					MainView.outputText( '  You do manage to gift her with a nice little cum-paunch for her efforts, though.' );
				} else {
					MainView.outputText( '  You\'re cumming so fast that that venting doesn\'t help her much.  Slowly, her belly begins to bulge outward.  It stretches and wobbles as it fills with spunk, not stopping until she looks months pregnant.  The liquid way that it jiggles with every movement will reveal the contents of her middle to any who dares look at her.' );
				}
			}
			MainView.outputText( '\n\nSetting her down in front of her sisters, you let her slide off your ' );
			if( y >= 0 ) {
				MainView.outputText( 'twin cocks' );
			} else {
				MainView.outputText( Descriptors.cockDescript( x ) );
			}
			MainView.outputText( ' onto the floor as an example of just what you\'re capable of.  You can beat the best they have and fuck them into a twitching puddle in the dirt.  Though the mob seems cowed by your display of authority, they\'re still fucking themselves to the sight of what you\'ve done to their sister.  You make the rest of them take turns licking your spent spunk and caked-on cunt-juice from your phallus' );
			if( y >= 0 ) {
				MainView.outputText( 'es' );
			}
			MainView.outputText( ' before getting dressed and getting back to business.' );
		}
		//Too Big For Fucks;
		else {
			...
			if( CoC.player.cumQ() >= 500 ) {
				MainView.outputText( 'struggling to swallow it' );
			} else {
				MainView.outputText( 'swallowing it' );
			}
			MainView.outputText( '.  She squeezes her arms together, which pushes her boobs tighter around your orgasmically pulsating length.  It feels so good that it feels like a whole new orgasm kicks off in your [balls], even though you\'ve already started to cum!  ' );
			if( CoC.player.cumQ() < 250 ) {
				MainView.outputText( 'The cute woman swallows every single drop.  She even gives up a satisfied moan when she finishes.' );
			} else if( CoC.player.cumQ() < 1000 ) {
				MainView.outputText( 'The cute woman\'s cheeks balloon as she tries to contain it all.  She swallows admirably in spite of your heavy jizz-flow, only losing a few white trickles out the corners of her stretched mouth.' );
			} else {
				MainView.outputText( 'The cute woman\'s cheeks flood with your obscene cum-flow.  She tries to swallow it down, but no matter how fast her fevered gulps are, the stuff still floods in with such force that it squirts from the corners of her mouth.  She just gives up after a few moments and lets it run down her chin in a waterfall.  Most of the jism puddles on the floor below her, soaking into her ponytail and matting it with salty spunk.' );
			}
	...
	DungeonSandWitch.prototype.forceCunnilingusRimjobClitAndNipple = function() {
		MainView.clearOutput();
		MainView.outputText( 'You eye the defeated mob' );
		if( CoC.player.HP < 1 ) {
			MainView.outputText( ' through their torn-up robes, noting many large, dripping breasts and more than a few pairs of supple cunt-lips' );
		} else {
			MainView.outputText( ' through their squirming, orgiastic motions, noting their many large, dripping breasts and gushing multi-cunts' );
		}
		MainView.outputText( '.  They may ' );
		if( CoC.player.cor < 33 ) {
			MainView.outputText( 'be misguided in fighting you' );
		} else if( CoC.player.cor < 66 ) {
			MainView.outputText( 'be a little over-zealous in defending their home' );
		} else {
			MainView.outputText( 'be crappy fighters' );
		}
		MainView.outputText( ', but feeling a stirring in your nethers, you can think of at least one good use for them.  You start to strip out of your [armor], groping your butt with one hand; maybe there\'s a second purpose they can serve...' );
		...
		if( CoC.player.wetness() >= 4 ) {
			MainView.outputText( ', spraying your musky girl-goo across the pretty faces at your groin, soaking them all the way to their first rows of tits.  Your juices mix with the milk dripping from their nipples into a sticky alabaster treat that makes you lick your lips and shudder in delight. The seductress on your [vagina] gurgles and sighs as she tries to keep up with your copious lady-spunk, but the way you squirt, she seems doomed to failure.' );
		} else if( CoC.player.wetness() >= 2 ) {
			MainView.outputText( ', sluicing secretions down the lips of your would-be pussy-slave, forcing her to work her throat in order to try and keep up with it.  You\'re so goddamned wet, and the way your channel is clenching and undulating around her tongue just forces out more of your juice.' );
		} else {
			MainView.outputText( ', dribbling your slippery girl-cum straight into your would-be pussy-slave, forcing her to swallow your tangy flavor.  It\'s hard not to turn into a dripping mess with the way her tongue works your spasming pussy, but neither of you seems to mind.' );
		}
		if( CoC.player.hasCock() ) {
			MainView.outputText( '  Above, [eachCock] fires lances of proper, virile spunk onto the girls\' exposed backs to properly mark them.' );
			if( CoC.player.cumQ() >= 500 ) {
				MainView.outputText( '  It turns their bronzed skin into a soupy white, then dangles to the ground in thick ropes' );
				if( CoC.player.cumQ() >= 1500 ) {
					MainView.outputText( ', each of which begins pooling into a puddle' );
				}
				if( CoC.player.cumQ() >= 2500 ) {
					MainView.outputText( '.  Those puddles combine to form a lake of jism beneath these lusty slaves bodies, putting their combined milky tits to shame' );
				}
				MainView.outputText( '.' );
			}
		}
		...
		if( CoC.player.lactationQ() >= 200 ) {
			MainView.outputText( ' drink their fill' );
		}
		if( CoC.player.lactationQ() >= 1000 ) {
			MainView.outputText( ', or at least as much as they can handle of your lactic capacity,' );
		} else if( CoC.player.lactationQ() < 200 ) {
			MainView.outputText( ' slowly suckle you' );
		}
		MainView.outputText( ' while you come down from your high.  Why couldn\'t they have just done this to start?' );
	...
	DungeonSandWitch.prototype.gangrush = function() {
		MainView.outputText( 'The witches close ranks and advance with raised fists, intent on beating you into submission!\n' );
	...
	DungeonSandWitch.prototype.headbuttABitch = function() {
		MainView.outputText( 'The crowd parts, and a stockier, sturdier sorceress ambles out, fists up and head cocked back.  She makes to punch at you before pulling her fist at the last second, snapping her head forward in a powerful headbutt!  You barely have time to react!' );
		var damage = Math.round( (CoC.monster.str + CoC.monster.weaponAttack + 10) - Utils.rand( CoC.player.tou ) - CoC.player.armorDef );
		//Dodge;
	...
	DungeonSandWitch.prototype.sandstonesAreCool = function() {
		MainView.outputText( 'The sandstorm whirling around the room suddenly ceases, and all the tiny sand particles gather together into balls, growing into several smooth stones.  Then, all the sand stones fall to the ground and slither towards you.' );
		var bonus = 0;
		...
		if( !Combat.combatMiss() && !Combat.combatEvade() && !Combat.combatFlexibility() && !Combat.combatMisdirect() ) {
			...
			if( !CoC.player.isGoo() ) {
				...
				//If PC has 1 cock:;
				if( CoC.player.cockTotal() === 1 ) {
					MainView.outputText( '  A stone crawls onto your [cock].' );
				}
				//[If PC has multi-cocks: ;
				if( CoC.player.cockTotal() > 1 ) {
					MainView.outputText( '  A bunch of the stones crawl onto your ' + CoC.player.multiCockDescriptLight() + '.' );
				}
				if( CoC.player.hasCock() ) {
					bonus++;
				}
				//[If PC has cunt];
				if( CoC.player.hasVagina() ) {
					MainView.outputText( '  One stone slides up your inner thigh' );
					if( CoC.player.balls > 0 ) {
						MainView.outputText( ' behind your [sack]' );
					}
					MainView.outputText( ' and pops itself right into your [vagina]' );
					if( CoC.player.hasVirginVagina() ) {
						MainView.outputText( ', robbing you of your virginity as a trickle of blood runs down your [leg].' );
					}
					bonus++;
				}
				//[If PC has balls:;
				if( CoC.player.balls > 0 ) {
					MainView.outputText( '  A small set of stones settle on your [balls].' );
					bonus++;
				}
				MainView.outputText( '  ' + Utils.num2Text( CoC.player.totalNipples() ) + ' crawl up to your chest and over your top ' + Descriptors.nippleDescript( 0 ) + 's' );
				if( CoC.player.bRows() > 1 ) {
					...
					MainView.outputText( ' your middle ' + Descriptors.nippleDescript( 1 ) + 's' );
					bonus++;
				}
				if( CoC.player.bRows() > 2 ) {
					MainView.outputText( ', and your bottom ' + Descriptors.nippleDescript( 2 ) + 's' );
					bonus++;
				}
				MainView.outputText( '.' );
				MainView.outputText( '  The last stone travels up the back of your [legs] and slides right into your [asshole].' );
			...
			CoC.player.createStatusAffect( StatusAffects.LustStones, bonus, 0, 0, 0 );
			EngineCore.dynStats( 'lus', bonus * 2 + 5 + CoC.player.sens / 7 );
		}
		//[If attack misses];
		else {
	...
	DungeonSandWitch.prototype.sandStormAttack = function() {
		if( CoC.monster.short === 'sand witches' ) {
			MainView.outputText( 'The witches link their hands together and begin to chant together, lifting their voices high as loose sand trickles in from every corner, every doorway, even the ceiling.  "<i>Enevretni llahs tresed eht!</i>"  Swirling around the chamber, a cloud of biting, stinging sand clouds your vision and bites into your skin.  It\'s going to keep blinding you and hurting you every round!' );
		} else {
	...
	DungeonSandWitch.prototype.bukkakeAttack = function() {
		...
		MainView.outputText( 'The Cum Witch moans and daintily peels her robes away from her swollen cock-flesh.  A bubble of precum pops wetly from her urethra to splatter on the floor as her balls suddenly swell.  You look back up in time to see the telltale glow of magic surrounding her staff, but then she\'s thrusting her hips at you, lewdly humping the air as she unleashes rope after thick rope of potent jism in your direction!\n' );
	...
	DungeonSandWitch.prototype.genderConfusionAttack = function() {
		...
		MainView.outputText( 'Touching her alabaster staff to her brow, just under the brim of her hat, the Cum Witch makes a brief incantation and fixes you with her gaze.  Her eyes flash blindingly white, and then you feel her inside you, rifling through your memories, digging up memories of your childhood, your past, and throwing them against you.  ' );
		if( CoC.player.inte / 5 + Utils.rand( 20 ) + CoC.player.level / 2 < 18 ) {
			MainView.outputText( 'She batters your consciousness with conflicting memories of your gender, utterly dazing you.  How can you fight when you can barely tell who you are anymore?' );
			CoC.player.createStatusAffect( StatusAffects.Confusion, 0, 0, 0, 0 );
		} else {
	...
	DungeonSandWitch.prototype.cocknosisAttack = function() {
		MainView.outputText( 'Lifting her robes enticingly, the Cum Witch reveals her ' );
		if( CoC.monster.lust < 50 ) {
			MainView.outputText( 'half-hard' );
		} else if( CoC.monster.lust < 70 ) {
			MainView.outputText( 'hard' );
		} else if( CoC.monster.lust < 85 ) {
			MainView.outputText( 'throbbing' );
		} else {
			MainView.outputText( 'hard, dripping' );
		}
		MainView.outputText( ' cock.  She gently begins to sway her hips, bouncing back and forth with near-mechanical precision, her softly bobbing cock catching your eyes with its metronome-like precision.  She softly begins to speak, enunciating each word to the time and tune of her movements.' );
		...
		if( CoC.player.inte / 20 + Utils.rand( 20 ) >= 13 ) {
			MainView.outputText( 'You chuckle at her crude attempt to hypnotize you with her member.  She stomps her foot in irritation and drops her robes back into place.' );
		} else {
			MainView.outputText( 'The Witch smirks, though you\'re too focused on her cock to see, and she continues, "<i>Good ' + CoC.player.mf( 'boy', 'girl' ) + '.  Watch it sway.  You\'re so focused on my cock now that the world is just falling away around it, sinking into nothingness, leaving only that wonderful cock behind for you to watch.  And since it\'s filling your view, you can feel it filling your mind as well, can\'t you?</i>"' );
			MainView.outputText( '\n\nYou nod, your view rigidly attached to her equally rigid tool as you utterly and completely fixate on her penis, admiring its curves, its thickness, and the way it seems to pulsate happily for you whenever you look at it just right.  The Witch keeps talking, but it\'s her dick that\'s important.  You start to drool as your ' + CoC.player.skin() + ' flushes and your body heats.  Gods, her cock is gorgeous.  Reaching down, you begin to masturbate without thinking.  You don\'t know why, but it just feels like the right thing to do.' );
	...
	DungeonSandWitch.prototype.turnIntoASammitch = function() {
		MainView.clearOutput();
		MainView.outputText( 'Helplessly, you ' );
		if( CoC.player.lust > 99 ) {
			if( CoC.player.hasVagina() ) {
				MainView.outputText( 'dig your hands into your loins, frigging your hungry cunt so hard your juices splatter the Witch\'s robe.' );
			} else {
				MainView.outputText( 'dig your finger into your [asshole], frigging it like your life depended on it.' );
			}
		} else {
			MainView.outputText( 'try to rise, but all you manage to do is slip back down to the ground.' );
		}
		MainView.outputText( '  The Cum Sorceress smiles and releases her staff.  Instead of clattering to the ground, it dematerializes a piece at a time, fading away in a way that would be mesmerizing were you not otherwise occupied.  She hikes her robes up and pulls off her hat with them, shaking her surprisingly light, blonde hair free.  "<i>Now, all that got me pretty worked up, and I don\'t think the girls will mind if I give you your first sampling of cum witchery.  What do you think, hun?</i>" she asks, glancing back towards the comatose witch behind her.' );
		...
		if( CoC.player.pregnancyType === 0 ) {
			MainView.outputText( 'let\'s go ahead and get your first daughter inside you, shall we?' );
		} else {
			MainView.outputText( 'let\'s go ahead and have some fun, shall we?  We\'ll have to wait until you pop out whatever\'s in your womb before I give you your first witch child.' );
		}
		MainView.outputText( '  There will be plenty of time to make you one of us while we\'re waiting.</i>"' );
		if( !CoC.player.hasVagina() ) {
		...
		if( CoC.monster.lust < 65 ) {
			MainView.outputText( ' herself to full hardness, smiling when thick strings of cum begin to drizzle from her swollen cock-tip.' );
		} else if( CoC.monster.lust < 85 ) {
			MainView.outputText( ' herself until cum begins to drizzle from her swollen cock-tip.' );
		} else {
			MainView.outputText( ' herself, gathering up the constantly-dripping cum and smearing it all over her shaft.' );
		}
		...
		if( CoC.player.lust > 99 ) {
			MainView.outputText( ', even though your hand is knuckle-deep in your twat' );
		}
		MainView.outputText( '.' );
		...
		if( CoC.player.bRows() < 2 ) {
			MainView.outputText( 'get four big, milky tits' );
		} else if( CoC.player.lactationQ() < 200 ) {
			MainView.outputText( 'have your tits made into perfect milkers' );
		} else {
			MainView.outputText( 'get your four, big tits milked' );
		}
		MainView.outputText( ' or to get your first proper administration of witch-cum.' );
	...
	DungeonSandWitch.prototype.cumWitchCumPumpBadEnd = function() {
		MainView.clearOutput();
		MainView.outputText( 'Laid low by ' );
		if( CoC.player.HP < 1 ) {
			MainView.outputText( 'your wounds' );
		} else {
			MainView.outputText( 'by the lust coursing through your veins' );
		}
		MainView.outputText( ', you slump over against a desk, leaning heavily on it for support while ' );
		if( CoC.player.HP < 1 ) {
			MainView.outputText( 'you struggle to rise' );
		} else {
			MainView.outputText( 'you struggle to get your crotch' );
		}
		MainView.outputText( '.' );
		MainView.outputText( '\n\n"<i>Let me help you with that,</i>" the robed beauty whispers as she deftly removes your [armor], leaving you bare and exposed, naked to her casual caresses and lecherous looks.  "Such a lovely ' + CoC.player.mf( 'male', 'hermaphroditic' ) + ' specimen' );
		if( CoC.player.cumQ() > 1000 ) {
			if( CoC.player.balls > 0 ) {
				MainView.outputText( ' and such virile, cum-swollen testes.  An excellent breeder for sure!' );
			} else {
				MainView.outputText( ' and such a virile body!  An excellent breeder for sure!' );
			}
		} else {
			MainView.outputText( 'but with such a pathetic virility.' );
		}
		MainView.outputText( '  A little magic later, and you\'ll be quite suited to breeding daughters with me.  Perhaps if you do well enough, I\'ll allow you to seed MY womb.</i>"' );
		...
		if( CoC.player.HP < 1 ) {
			MainView.outputText( 'grunt and strain in response, still trying to fight, even though you know it\'s hopeless.' );
		} else {
			MainView.outputText( 'groan and touch yourself, trying to show her just how \'in the mood\' you are.' );
		}
		MainView.outputText( '  She affectionately ruffles your ' + Descriptors.hairDescript() + ' and tuts, "<i>Now, now, it\'s cute that you think you still have some agency here, but you really ought to just lay back and let me take worry about it!</i>"' );
		...
		MainView.outputText( '\n\nSnapping her fingers, the black-skinned spellcaster snares glowing bands of force around your chest, wrists, biceps, and [feet], effectively restraining you as efficiently as any torture rack.  You struggle briefly, though you cannot say whether to free or touch yourself.  Does it really matter?  Either way, you\'re bound, nude, helpless, and aroused.  Gentle touches rub the ' + CoC.player.skinFurScales() + ' of your [chest] as she reassuringly coos, "<i>Relax, my mighty friend.  You undoubtedly fought hard to get here, but now, all you need to do is relax.</i>"' );
		...
		MainView.outputText( '\n\nNext, the witch rubs her way back up to your [hips].  She suggests, "<i>I\'m going to work my way up your body, and with every part I rub, you\'re going to let out more and more of your pent up tension.</i>"  True to her words, your body is beginning to feel like putty from the waist down - a limp facsimile of its former self.  The softness has even infected your ' + CoC.player.multiCockDescriptLight() + ', robbing some of the former stiffness.  You sigh and inadvertently let your guard down.  You\'ve already lost, and if this woman wants to give you a massage before she has her way with you, why not enjoy it?' );
		...
		MainView.outputText( '\n\nYou strain to stay awake, but with the sensation of so many soothing caresses lingering on your ' + CoC.player.skinFurScales() + ' and the heavy weights pulling down... down.  You blink, getting your eyes half open. The next time they close, they stay closed, leaving you to dwell on the imaginary massage.' );
	...
	DungeonSandWitch.prototype.beACumPumpPartII = function() {
		...
		MainView.outputText( '.  Of course, the wicked witch only releases you at this point.  "<i>Do I look like one of the cum-hungry wenches around here?</i>"' );
		//add lust;
		...
		MainView.outputText( ' wilting from the force of your surprise and fear.' );
		...
		MainView.outputText( ', "<i>I did not think you would get this far.  Your will is truly mighty, [name], but the task I gave you is something I\'ve never been able to do.  Ten is as far as I ever got.  I would not have you damage yourself just yet.</i>  She pulls open the curtain and bellows, "<i>Next!</i>"' );
		...
		if( CoC.player.smallestCockArea() >= 100 ) {
			MainView.outputText( '  The penetration is eased somewhat by your shrinking dick.  You glance at the sand witch in awe, noting her glowing hands.  She gives you a knowing wink and turns, her spell complete, leaving you to enjoy yourself.' );
		}
		...
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 'Her voluminous butt is thoroughly smeared by the goo from the rest of your ' + CoC.player.multiCockDescriptLight() + ', shining with white from your magical virility.  ' );
		}
		MainView.outputText( 'She rides you with surprising skill for one who seems to be so hesitant about such a healthy jizz output, her four breasts squishing tight against [chest].  Her tight cunny squeezes and ripples around you, rapidly growing soaked with white, thoroughly spunk-lubed and hungry for more.' );
	...
	DungeonSandWitch.prototype.cumPumpVolumeThree = function() {
		...
		MainView.outputText( ' Marae.  Their abilities, numbers, and familiarity at dealing with demons lend them great success at battling Lethice\'s growing hordes, and within the span of another decade, the demons are exterminated.  The witches are hailed as saviors, and young females of all races seek to join them.  You, of course, inseminate all of them.' );
		EngineCore.gameOver();
	};
	...
	DungeonSandWitch.prototype.repeatLoseToCumWitchForDudes = function() {
		...
		if( CoC.player.HP < 1 ) {
			...
			if( CoC.player.lust < 40 ) {
				MainView.outputText( 'flaccid' );
			} else if( CoC.player.lust < 60 ) {
				MainView.outputText( 'semi-flaccid' );
			} else if( CoC.player.lust < 70 ) {
				MainView.outputText( 'semi-hard' );
			} else {
				MainView.outputText( 'turgid' );
			}
			MainView.outputText( ' weight.  Pumping her hand with deft strokes and watching your expression intently, the curvy woman works you into a pleasant, erotic warmth, excitement coursing through your body with such fervor that your remaining wounds seem insignificant in comparison.' );
		}
		//Lust:;
		else {
			MainView.outputText( 'Dropping down on your [legs], you rip off your [armor] and flop onto your back so that you can focus on abusing your genitalia.  [EachCock] is already hard enough to be leaking pre' );
			if( CoC.player.balls > 0 ) {
				MainView.outputText( ', and your [balls] are practically quaking with need' );
			}
			MainView.outputText( '.  The ebony futanari laughs at you as she confidently strides forward and discards her hat.  "<i>Did you really have any intention of fighting me, or did you ask the Sand Mother to send us out so that we could abuse your insatiable libido?  I don\'t know how you didn\'t wind up captured, but I suppose I can tend to your needs... this time.</i>"' );
			MainView.outputText( '\n\nKneeling next to you, the witch runs her hands across your ' + CoC.player.skinFurScales() + ' toward your groin.  She grabs hold of [oneCock], hefting the rigid weight as she gauges your size.  Her fingers are soon glossy with your dribbling pre-cum, and she slowly pumps you to make sure she has your undivided attention.  "<i>So helpless...  Still, hopefully this can produce a decent cum-shot.</i>"  Her eyes twinkle with mirth, and she finishes, "<i>If not, I can always encourage it.</i>"' );
		}
		...
		if( CoC.player.cockThatFits( CoC.monster.vaginalCapacity() ) < 0 ) {
			MainView.outputText( '\n\nAbruptly, the Cum Witch climbs atop your lust-wracked body.  Her robe is off in a flash, and you\'re finally able to appreciate her sweat-slicked, onyx-skinned body as she sits on your immense, oversized maleness, her form glistening in the desert sun.  Thumbing a finger just under your ' + CoC.player.cockHead( 0 ) + ', she teases your gigantic dong while her own hard prick flops down on top of it, just heavy enough to leave a cock-shaped impressed on your urethral bulge.  The omnipresent heat and the fight have ensured that both bodies are soaked with sweat and able to slip and slide over each other easily.  Utilizing this, the Cum Witch rocks her hips slowly, folds splayed and leaking over your cock.  Her dick is already dripping a slow flow of girl-jism onto your own, almost claiming it as her own.' );
			MainView.outputText( '\n\nGiggling, the onyx sperm-mage laughs as she slowly begins to frot with you.  "<i>Why have a dong this big if you can\'t fit it in anything?</i>"  She sensually caresses some of your copious cock before bending down and licking at the ' + CoC.player.cockHead( 0 ) + '.  You shiver and express a drop of pre-cum onto your own ' );
			if( CoC.player.biggestCockLength() < CoC.player.tallness / 2 ) {
				MainView.outputText( '[chest]' );
			} else if( CoC.player.biggestCockLength() < CoC.player.tallness / 1.6 ) {
				MainView.outputText( CoC.player.face() );
			} else {
				MainView.outputText( Descriptors.hairDescript() );
			}
			MainView.outputText( '.  The black spellcaster comments, "<i>I must admit it is fun to look at, but it doesn\'t look like it\'s ready to blow just yet.  How about a little encouragement?</i>"' );
			...
			MainView.outputText( 'your [nipples]' );
			if( CoC.player.hasFuckableNipples() ) {
				MainView.outputText( ', sinking inside your moist tit-holes almost immediately' );
			}
			MainView.outputText( '.  With two vibrators savaging your [nipples] and the black-hued beauty riding your cock like her own personal rocket, you can barely contain yourself.  Your back arches, and you try to buck your hips, to fuck her, her cock, whatever, but you\'re still too exhausted from the fight to shift her body weight.  You can do nothing but lie there while she uses you, humping your cock, her ebony length dragging its lurid pleasure across your ' + Descriptors.cockDescript( 0 ) + ' while you wriggle and writhe ecstatically.' );
			MainView.outputText( '\n\nAfter a while of continual teasing, the sable seductress leans over you and lets her huge breasts sweetly kiss on your elephantine mass.  She leans down as if to kiss you, but at the last moment, she swerves to the side, licking the nape of your neck up to your ear before breathily whispering promises into your ear, "<i>There\'s nothing like blowing off some steam out here in the sands after my work.  And to have such a... gifted ' + CoC.player.mf( 'boy', 'girl' ) + ' to play with is a treat in and of itself.</i>"  She grunts, sweaty balls bouncing on your [sheath].  "<i>You look so helpless for someone who\'s packing so much.  I look forward to seeing how you look with my cum hiding that expression.</i>"' );
			...
			if( CoC.player.balls > 0 ) {
				MainView.outputText( '[balls], rolling the ' );
				if( CoC.player.ballSize < 2 ) {
					MainView.outputText( 'petite ' );
				} else if( CoC.player.ballSize >= 5 ) {
					MainView.outputText( 'weighty ' );
				}
				MainView.outputText( 'orbs across her soles.' );
			} else if( CoC.player.hasVagina() ) {
				MainView.outputText( '[vagina], slipping a few toes through your folds while a big toe diddles your [clit].' );
			} else {
				MainView.outputText( 'taint, softly stroking the sensitive skin between your groin and [asshole].' );
			}
			...
			MainView.outputText( 'loud and firm enough for your dazzled mind to cogitate her words: "<i>Oh, my pretty bitch ' + CoC.player.mf( 'boy', 'girl' ) + ', feel the pressure building up inside you.  Higher and higher now... It\'s almost too much, isn\'t it?  You don\'t need to answer, just feel it grow thicker and hotter.  You\'re going to cum like a geyser, because of me, and you\'re going to love it.  You\'ll always want to cum for me, won\'t you?</i>"' );
			...
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( ' while your extra erection' );
				if( CoC.player.cockTotal() > 1 ) {
					MainView.outputText( 's spurt' );
				} else {
					MainView.outputText( ' spurt' );
				}
				MainView.outputText( ' some cream up the ebony arch of your mistress\'s back' );
			} else {
				MainView.outputText( ' from her bouncing, ebony erection' );
			}
			MainView.outputText( '.  At the same time, her clenching, pussy climaxes around you while you fill it.  You\'re spraying ropes of spooge into her so fast that it\'s backwashing out around your ' + Descriptors.cockDescript( 0 ) + ', the veins pumping almost as hard as your overactive reproductive system.' );
	...
	DungeonSandWitch.prototype.tooBigCumWitchLossFacial = function() {
		MainView.clearOutput();
		MainView.outputText( 'Ruefully, you tell her that you don\'t mind a little cum in exchange for having your own abilities enhanced.  The knowing grin that spreads across her face makes it seem like she knew the result was a foregone conclusion.' );
		if( CoC.player.cor < 33 ) {
			MainView.outputText( '  You shudder at the knowledge of what\'s going to happen to you.  Just why did you agree to this?' );
		} else if( CoC.player.cor < 66 ) {
			MainView.outputText( '  You catch yourself licking your lips as the knowledge of what is about to happen to you sinks in. Despite hastily stopping yourself, the Cum Witch still saw, and she smiles.' );
		} else {
			MainView.outputText( '  You give her a lewd look and lick your tongue across your lips as you anticipate the big, fat load she\'s going to feed you, hungry for wet, decadent pleasure regardless of inhibitions.' );
		}
		...
		if( CoC.player.hasFuckableNipples() ) {
			MainView.outputText( 'in' );
		} else {
			MainView.outputText( 'on' );
		}
		MainView.outputText( ' your [nipples].  It feels like there\'s so much blood being forced inside your erection that the sheer over-tumescence will make you burst.  The need is overwhelming.  You HAVE to cum, and you\'re so wet, so stained with spunk and juice that there\'s no time like now.  The Cum Witch\'s heels press in on your ' );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( '[balls], compressing them slightly as a tingle of magic lances into you, fattening them under her ministrations' );
		} else if( CoC.player.hasVagina() ) {
			MainView.outputText( '[vagina], digging into the slippery slit as a tingle of magic spreads through it and into your core, thickening some organ inside you' );
		} else {
			MainView.outputText( 'taint, rubbing it as a tingle of electric need phases through your ' + CoC.player.skinFurScales() + ' to assault your swelling prostate and seminal vesicles' );
		}
		MainView.outputText( ', and in that moment, you lose all ability to hold out.  You\'re cumming, and you\'re cumming now.' );
		...
		if( CoC.player.biggestCockLength() >= CoC.player.tallness / 1.6 ) {
			MainView.outputText( '  The Cum Witch smiles as she holds it, bending it to make sure the overlong shaft deposits its thick load on your ' + CoC.player.face() + ' where it belongs.' );
		}
		MainView.outputText( '  Your captor is still humping it even as you cum, moaning as the bulges of copious cream press on her smaller boner.  Her visage is alight with ecstasy, and her heavy nipples are dragging on your [chest] as she begins to grunt, her motions going jerky in her ecstasy.' );
	...
	DungeonSandWitch.prototype.tooBigCumWitchLossNoFacial = function() {
		...
		if( CoC.player.balls > 0 ) {
			MainView.outputText( ', just like your own' );
		}
		MainView.outputText( ', and she smiles cruelly as her feet press harder on you, suddenly tingling with supernatural forces.  Almost painful heat floods through your middle as she frots you harder and harder, inadvertently tit-fucking your immense erection all the way to orgasm.' );
		...
		if( CoC.player.ballSize > 5 ) {
			CoC.player.ballSize -= Math.round( CoC.player.ballSize * 0.333 );
		}
	...
	DungeonSandWitch.prototype.TDMsLoseToCumWitchScene = function() {
		...
		if( CoC.player.cockTotal() > 1 ) {
			...
			//if (cocks = 2);
			if( CoC.player.cockTotal() === 2 ) {
				MainView.outputText( 'your other phallus.' );
			}//[if (cocks > 2);
			else {
				MainView.outputText( 'the rest of your ' + CoC.player.multiCockDescriptLight() + '.' );
			}
			MainView.outputText( '  The futa on top of you continues to smile mysteriously, and spares not another moment of her attention on your genitals.' );
		}
		...
		if( CoC.player.balls > 0 ) {
			MainView.outputText( 'the one part of your manhood that you can still feel, your [sack].' );
		} else {
			MainView.outputText( 'a part of your body just behind and above the base of [eachCock], probably your prostate.' );
		}
		...
		if( EngineCore.silly() ) {
			MainView.outputText( 'in both body and in [balls].' );
		} else {
			MainView.outputText( 'in both body, and in spirit.' );
		}
	...
	DungeonSandWitch.prototype.cumWitchDefeated = function() {
		...
		CoC.flags[ kFLAGS.CUM_WITCH_DEFEATED ] = 1;
		EngineCore.menu();
		if( CoC.player.hasCock() ) {
			//*Male 'Too Big' Victory Sex;
			if( CoC.player.biggestCockArea() > CoC.monster.vaginalCapacity() ) {
				EngineCore.addButton( 0, 'Too Big Fuck', this, this.maleTooBigVictorySex );
			}
			//*Male Victory Sex;
			if( CoC.player.cockThatFits( CoC.monster.vaginalCapacity() ) >= 0 ) {
				EngineCore.addButton( 1, 'Fuck Her', this, this.menFuckUpSomeCumWitch );
			}
		}
		//Tentacle Victory Gangbang;
		//3+ Tentas;
		if( CoC.player.tentacleCocks() >= 3 ) {
			EngineCore.addButton( 2, 'Tentacles', this, this.tentacleVictoryGangbangCumWitch );
		}
		//Female Victory Sex;
		if( CoC.player.hasVagina() ) {
			EngineCore.addButton( 3, 'Ladysex', this, this.ladyVictorySex );
		}
		if( CoC.isInCombat() ) {
			if( CoC.monster.HP >= 1 ) {
	...
	DungeonSandWitch.prototype.menFuckUpSomeCumWitch = function() {
		...
		if( CoC.player.cockThatFits( CoC.monster.vaginalCapacity() ) < 0 ) {
			MainView.outputText( 'most of' );
		} else {
			MainView.outputText( 'all of the way' );
		}
		MainView.outputText( ' inside her, sheathed deep in her under-used twat.  Her tight passage fits around your ' + Descriptors.cockDescript( x ) + ' like a custom-made glove, a slippery warm embrace that threatens to rob you of your very reason.  The witch begins pumping her ebony cock along with the motions of your hips, throwing her head back in wordless pleasure that only a true hermaphrodite can experience.' );
		if( CoC.player.hasVagina() ) {
			MainView.outputText( '  Two can play at that game.' );
		}
		//Herm sexback;
		if( CoC.player.gender === 3 ) {
			...
			//cuntchange;
			CoC.player.cuntChange( CoC.monster.biggestCockArea(), true, true, false );
			...
			MainView.outputText( '\n\n' );
			if( CoC.player.balls > 0 ) {
				MainView.outputText( 'Your sweaty nutsacks slide over each other while you fuck, compressing together into a mass of sweaty orbs twitching against each other in a competition to disgorge their frothy load before the other.' );
			} else {
				MainView.outputText( 'Her sweaty nutsack glides over your ' + CoC.player.skinFurScales() + ' as you fuck, compressing and twitching against you as it begs to discharge its frothy load.' );
			}
			...
			if( CoC.player.cumQ() > 2000 ) {
				MainView.outputText( '  Of course, you outlast even the jizz-witch\'s magics, plugging her so full of semen that her belly rounds into a gravid orb while yours remains only slightly bloated.' );
			} else if( CoC.player.cumQ() > 1000 ) {
				MainView.outputText( '  Of course, you both manage to bulge each other\'s bellies with sexual discharge.' );
			} else {
				MainView.outputText( '  Of course, she cums far longer and harder than you, plugging your belly full of semen until it has a slightly fecund curve to it.' );
			}
		...
		//DP Males;
			...
			MainView.outputText( '\n\nA torrent of thick herm-cream spouts from the woman\'s neglected erection as you utterly abuse and humiliate her.  Taking this as your signal to go balls out, you twist that nipple sideways and rut her like a ' + CoC.player.mf( 'man', 'woman' ) + ' possessed, slapping both cocks into her spasming holes non-stop until you feel the heat surging in your loins, ready to explode inside her.' );
			...
			if( CoC.player.cumQ() < 250 ) {
				MainView.outputText( 'pours out of your dual cum-slits, making a sloppy mess of both holes.' );
			} else if( CoC.player.cumQ() < 500 ) {
				MainView.outputText( 'gushes out of your dual cum-slits, soaking both holes in and out with pearly white.' );
			} else if( CoC.player.cumQ() < 1000 ) {
				MainView.outputText( 'gushes out with incredible force from your dual cum-slits, quickly plugging both holes so full of seed that the witch\'s belly slowly rises, cum-inflated.' );
			} else {
				MainView.outputText( 'washes out of your dual cum-slits in a tidal wave, flooding the hungry holes with your virile seed.  You cum and cum, spunking up the witch\'s belly.  First it bulges slightly, but all too soon you have it rounded into a pregnant dome.' );
			}
			MainView.outputText( '  The bouncing girl-cock above the woman\'s gushing pussy does a fine job of painting her belly and bust white.  It spurts rope after rope onto her chocolate-toned flesh, and soon she looks more like an icing-drizzled tart than a formidable foe.' );
		...
		//Regular Dicking;
			...
			//(silly:) MainView.outputText(', and I don\'t want it.  I think it\'s messy');;
			...
			if( CoC.player.cor < 50 ) {
				MainView.outputText( 'and a hint of compassion' );
			} else {
				MainView.outputText( 'and finally deign to let her have some pleasure' );
			}
			MainView.outputText( '.  You gradually increase the tempo of your lovemaking and let your hand lazily drift lower, grabbing the witch by her soft, supple thigh.  Her skin is warm and flushed (or as flushed as ebony skin can get), and the higher your fingers reach, the damper it gets.  Gently, you prod around her lips and your own maleness to find the hardness of clit.' );
			...
			if( CoC.player.cumQ() < 700 ) {
				MainView.outputText( ', spurting with unceasing quantity until she\'s absolutely flooded with jizz and a little bulgy around the middle.' );
			} else {
				MainView.outputText( ', spooging up her cunny until it\'s completely flooded, her belly bloated into a rounded dome.' );
			}
			if( CoC.player.cumQ() >= 1500 ) {
				MainView.outputText( '  You keep going beyond that, but there\'s just no room, and spouts of semen spurt from the woman\'s poor, over-filled twat.' );
			}
			MainView.outputText( '\n\nThe witch sighs and idly rubs her ' );
			if( CoC.player.cumQ() >= 700 ) {
				MainView.outputText( 'swollen ' );
			}
			MainView.outputText( 'tummy, smearing her own wasted goo into her skin as you withdraw and re-dress.' );
			if( CoC.player.cor < 33 ) {
				MainView.outputText( '  You hope she learned something from the experience, but knowing most people in this strange land, she\'ll go right back to her rotten ways.' );
			} else if( CoC.player.cor < 66 ) {
				MainView.outputText( '  You wonder if she\'ll learn anything from this and shrug nonchalantly.  Who cares?' );
			} else {
				MainView.outputText( '  You smirk as you wonder if she\'ll learn anything from this.  You hope not - it\'s a fun lesson to teach.' );
			}
			CoC.player.orgasm();
		}
		if( OnLoadVariables.dungeonLoc ) {
	...
	DungeonSandWitch.prototype.maleTooBigVictorySex = function() {
		...
		MainView.outputText( '\n\nYou aren\'t done!  Not by a long shot!  This dark creature blessed you with unholy levels of virility, and you aren\'t going to waste it.  You stand away and butt your ' + CoC.player.cockHead( x ) + ' into her nether-lips, and the remainder of your orgasm is injected directly into the woman\'s waiting womb.  Her belly rounds into a nice, gravid bump that carries her cock up with it, and you\'re given the first chance to see the results of the cum witch\'s own orgasm.  The dark, glossy cock is absolutely soaked in sperm, while her balls are wreathed in churned up, frothy cum.  Once you\'ve made her uterus a swollen dome, you aim up and drizzle the last ropes atop her pathetic, half-limp dick.' );
		MainView.outputText( '\n\nExhausted at last, you pat your ' + Descriptors.cockDescript( x ) + ' affectionately.  You\'d wipe it off on the witch\'s hair, if it wasn\'t messier than the ' + CoC.player.skin() + ' you plan to clean.  She begins to lick her fingers and clean the stuff off her face.  You just laugh, and get dressed.  There\'s still much to do.' );
		CoC.player.orgasm();
		if( OnLoadVariables.dungeonLoc ) {
			if( CoC.isInCombat() ) {
				Combat.cleanupAfterCombat();
	...
	DungeonSandWitch.prototype.ladyVictorySex = function() {
		MainView.clearOutput();
		MainView.outputText( 'You disrobe, casting aside the garments with a feminine sigh.  As soon as you expose yourself, the witch\'s eyes twinkle happily.  She caresses her stiff tool and tweaks one of her nipples as she watches you.  "<i>If you wanted to get fucked by little old me, all you had to do was bend over, honey.</i>"' );
		...
		if( CoC.player.wetness() >= 5 ) {
			MainView.outputText( '  It gushes out of you in a soaking river, utterly drenching the futa\'s stomach, hips, and butt.' );
		}
		if( CoC.player.lactationQ() >= 50 ) {
			MainView.outputText( '  At the same time, milk spouts from your bosom to wash over your hermaphrodite.' );
		}
	...
	DungeonSandWitch.prototype.tentacleVictoryGangbangCumWitch = function() {
		...
		MainView.outputText( '  You shudder and grab the ebony sorceress\'s tits in your hands, squeezing her bronzed orbs tightly as you release works through you.  Jets of cum well up to plug the witch\'s pussy and anus simultaneously.  The spit-soaked, cum-splattered vine between her tits quivers meaningfully, and then unloads, squirting a thick wave of jizz onto her face.' );
		if( CoC.player.tentacleCocks() >= 4 ) {
			MainView.outputText( '  Of course, your fourth prick is not to be outdone, and blasts a gooey coating of cream across the witch\'s own pulsating prick, wreathing it in drizzles of syrupy white.' );
		}
		if( CoC.player.cumQ() >= 500 ) {
			MainView.outputText( '  Wave after wave of spunk washes into the tight holes and toned form of your foe, turning her into a complete mess.' );
		}
		if( CoC.player.cumQ() >= 1000 ) {
			MainView.outputText( '  A few more virile splurts see to her utterly inundated state, bloating her belly into a tightly-stretched dome.' );
		}
	...
	DungeonSandWitch.prototype.savinMakesAwesomeFemdom = function() {
		...
		//if Multiboob:;
		if( CoC.player.bRows() > 1 ) {
			MainView.outputText( 'She caresses each of your breasts, cupping each in turn, running her thumb over each nipple' );
			if( CoC.player.lactationQ() >= 200 ) {
				MainView.outputText( ' until milk streams down your chest, much to her delight' );
			}
			MainView.outputText( '.' );
		} else if( CoC.player.lactationQ() >= 200 ) {
			MainView.outputText( 'Her fingers deftly work across your [nipples], caressing out a trickle of milk, which she laps up with a long, languid motion that electrifies your sun-kissed skin.' );
		} else {
			MainView.outputText( 'She tweaks your [nipples] between her dexterous fingers, sending shivers of pleasure up your spine, but seems oddly discontent...  "<i>No milk for me? We\'ll have to do something about that...</i>"' );
		}
		MainView.outputText( '  Slowly, she works her way down from your [chest] to your belly, peeling off your [armor] as she goes to reveal more and more of your ' + CoC.player.skinFurScales() + ', never neglecting to run her hands all across you, stroking and kissing along every exposed inch until she comes to your groin.  Instinctively, you writhe in her grip as she tosses the last of you [armor] aside, leaving you wholly bare between her legs.' );
		//If PC has a cock:;
		if( CoC.player.hasCock() ) {
	...
	DungeonSandWitch.prototype.lionpaws = function( skipped ) {
		...
		if( skipped ) {
			MainView.outputText( '"<i>Really, [name]?</i>"  Sanura smirks and shakes her head.  "<i>I probably shouldn\'t do this sort of thing just because you ask for it, but your fascination with my paws is too cute to ignore.  Go on, then, strip for me and I\'ll get to it.</i>"  You oblige, happily tossing aside your armor.  [EachCock], already standing erect with anticipation, bounces freely before you.  The sphinx eyes ' );
			if( CoC.player.cockTotal() === 1 ) {
				MainView.outputText( 'it' );
			} else {
				MainView.outputText( 'them' );
			}
			MainView.outputText( ' lustfully, momentarily entranced by the swaying motions of your member' );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( '.' );
		}
		//[CoC.player requests big fluffy paws on their dick after solving a riddle];
		//[if event occurrence === 0];
		else if( CoC.flags[ kFLAGS.PAWJOBS ] === 0 ) {
			MainView.outputText( 'Sanura raises her eyebrows at your demand, her expression somewhere between surprise and curiosity.  "<i>Well, that\'s something of an odd request, but I suppose I\'m obliged if it\'s your wish.</i>"  Not bothering to give her a chance to consider your demand any further, you pull off your armor and cast it aside.  "<i>Can\'t say I\'ve even been asked for something like this before...  I apologize in advance for my inexperience, [name].</i>"  The sphinx looks down at her paws, obviously confused as to why you would ask for this in particular.' );
		}
		//[if event occurrence > 0];
		else {
			MainView.outputText( '"<i>Again?</i>" Sanura asks, cocking her head to the side.  "<i>They must feel better than I imagine... well then, get out of your clothes and I\'ll do as you ask.' );
			if( CoC.player.biggestCockArea() > 100 ) {
				MainView.outputText( '  It\'s not as if there\'s all that much else I can do for that monster of yours anyways.' );
			}
			MainView.outputText( '</i>" She raises a paw and examines it, as though trying to figure out just what it is about them that you enjoy so much.  You cough as a means of drawing her attention once you\'ve stripped off the last bit of your armor, shaking her from her reverie.' );
		}
		MainView.outputText( '\n\nSanura asks that you kneel, and you eagerly comply.  ' );
		//[if dick isn't already hard and PC knows what's coming];
		if( CoC.player.lust < 50 && CoC.flags[ kFLAGS.PAWJOBS ] === 0 ) {
			MainView.outputText( 'Even as you drop to your knees before Sanura, your cock' );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ' begin' );
			if( CoC.player.cockTotal() === 1 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ' to harden in anticipation while your mind wonders at just what this will feel like.  ' );
		}
		MainView.outputText( 'The sphinx bends too, so that her head is level with your crotch.  With a little bounce, she throws her large paws around your hips and roughly pulls you closer.  The guardian wastes no time, licking along the shaft' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ' of your ' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 'various ' );
		}
		MainView.outputText( 'prick' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ' with a soft, skilled tongue.' );
		//[if pc was soft before];
		if( CoC.player.lust < 50 && CoC.flags[ kFLAGS.PAWJOBS ] === 0 ) {
			MainView.outputText( '  It isn\'t long before her oral ministrations have your cock' );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ' standing erect, throbbing with hunger for more sensation.  You wonder as to what exactly she has planned, but you think it\'s shaping up to be something good.' );
		}
		//[continue];
		MainView.outputText( '  Her long, expert strokes methodically coat your member' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ' in a glistening veil of saliva. After a while, she pulls her head away and examines her work.' );
		MainView.outputText( '\n\nApparently deeming ' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 'them' );
		} else {
		...
		MainView.outputText( ' sufficiently lubricated, Sanura lifts a foreleg and carefully guides her paw towards your crotch.' );
		//[if PC doesn't expect it];
		if( CoC.player.lust < 50 && CoC.flags[ kFLAGS.PAWJOBS ] === 0 ) {
			MainView.outputText( '  You raise your eyebrows in surprise, but you\'re not in much of a position to do anything about her foot\'s advance.' );
		}
		MainView.outputText( '  She playfully taps at your cock, purring sensually as she sets it bouncing with a light bat.  After watching it for a moment, she hesitantly pushes forward so that her paw gingerly rests against ' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 'one of ' );
		}
		...
		if( !skipped && CoC.flags[ kFLAGS.PAWJOBS ] === 0 ) {
			MainView.outputText( 'It feels wonderful in a peculiar sort of way; though it\'s certainly not the treatment you expected to receive, you can definitely feel your arousal being pushed along and built up by her foot.  ' );
		}
		//[if PC has 2+ dicks];
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 'Not content to leave your other cocks neglected, the sphinx leans in and starts to lick and suckle once again at the pricks not beneath her paw.  You shudder at the feeling of her tongue darting from shaft to shaft and teasing your heads, sharper sensations punctuating her foot\'s more constant pleasuring.  ' );
		}
		MainView.outputText( 'The fuzzy warmth clouding your mind dazes you, and you sway slightly in place, letting her rock you back and forth with her motions.' );
		...
		if( CoC.flags[ kFLAGS.PAWJOBS ] > 0 ) {
			MainView.outputText( ' as always' );
		}
		MainView.outputText( ', [name].  Now get your clothes back on, I\'ve a temple to guard and a coat to clean.</i>"  At that, she turns and pads away from you, lying down in front of the door and licking at her' );
		//[if cum output is normal];
		if( CoC.player.cumQ() < 250 ) {
			MainView.outputText( ' leg.' );
		} else {
			MainView.outputText( ' stained-white fur in a futile attempt to get some of your cum off.' );
		}
		...
		if( skipped ) {
			...
			EngineCore.menu();
			EngineCore.addButton( 0, 'Enter', this, this.openZeDoorToParadize );
			EngineCore.addButton( 4, 'Leave', this, this.leaveBoobsDungeon );
		}
	};
	...
	DungeonSandWitch.prototype.riddleGameGo = function() {
		...
		MainView.outputText( '"<i>Oh!  You\'ll play my game?  Marvelous!  Well then, let\'s begin...</i>"' );
		//[NEXT];
		EngineCore.menu();
		EngineCore.addButton( 0, 'Next', this, this.riddlePicker );
	};
	DungeonSandWitch.prototype.riddlePicker = function() {
		var choices = [];
		if( CoC.flags[ kFLAGS.RIDDLE_ONE ] !== this.riddleOne && CoC.flags[ kFLAGS.RIDDLE_TWO ] !== this.riddleOne ) {
			choices[ choices.length ] = this.riddleOne;
		}
		if( CoC.flags[ kFLAGS.RIDDLE_ONE ] !== this.riddleTwo && CoC.flags[ kFLAGS.RIDDLE_TWO ] !== this.riddleTwo ) {
			choices[ choices.length ] = this.riddleTwo;
		}
		if( CoC.flags[ kFLAGS.RIDDLE_ONE ] !== this.riddleThree && CoC.flags[ kFLAGS.RIDDLE_TWO ] !== this.riddleThree ) {
			choices[ choices.length ] = this.riddleThree;
		}
		if( CoC.flags[ kFLAGS.RIDDLE_ONE ] !== this.riddleFour && CoC.flags[ kFLAGS.RIDDLE_TWO ] !== this.riddleFour ) {
			choices[ choices.length ] = this.riddleFour;
		}
		if( CoC.flags[ kFLAGS.RIDDLE_ONE ] !== this.riddleFive && CoC.flags[ kFLAGS.RIDDLE_TWO ] !== this.riddleFive ) {
			choices[ choices.length ] = this.riddleFive;
		}
		if( CoC.flags[ kFLAGS.RIDDLE_ONE ] !== this.riddleSix && CoC.flags[ kFLAGS.RIDDLE_TWO ] !== this.riddleTwo ) {
			choices[ choices.length ] = this.riddleSix;
		}
		if( CoC.flags[ kFLAGS.RIDDLE_ONE ] !== this.riddleSeven && CoC.flags[ kFLAGS.RIDDLE_TWO ] !== this.riddleSeven ) {
			choices[ choices.length ] = this.riddleSeven;
		}
		if( CoC.flags[ kFLAGS.RIDDLE_ONE ] !== this.riddleEight && CoC.flags[ kFLAGS.RIDDLE_TWO ] !== this.riddleEight ) {
			choices[ choices.length ] = this.riddleEight;
		}
		if( CoC.flags[ kFLAGS.RIDDLE_ONE ] !== this.riddleNine && CoC.flags[ kFLAGS.RIDDLE_TWO ] !== this.riddleNine ) {
			choices[ choices.length ] = this.riddleNine;
		}
		if( CoC.flags[ kFLAGS.RIDDLE_ONE ] !== this.riddleTen && CoC.flags[ kFLAGS.RIDDLE_TWO ] !== this.riddleTen ) {
			choices[ choices.length ] = this.riddleTen;
		}
		if( CoC.flags[ kFLAGS.RIDDLE_ONE ] !== this.riddleEleven && CoC.flags[ kFLAGS.RIDDLE_TWO ] !== this.riddleEleven ) {
			choices[ choices.length ] = this.riddleEleven;
		}
		if( CoC.flags[ kFLAGS.RIDDLE_ONE ] !== this.riddleTwelve && CoC.flags[ kFLAGS.RIDDLE_TWO ] !== this.riddleTwelve ) {
			choices[ choices.length ] = this.riddleTwelve;
		}
		if( CoC.flags[ kFLAGS.RIDDLE_ONE ] === 0 ) {
			CoC.flags[ kFLAGS.RIDDLE_ONE ] = choices[ Utils.rand( choices.length ) ];
			CoC.flags[ kFLAGS.RIDDLE_ONE ]();
		} else if( CoC.flags[ kFLAGS.RIDDLE_TWO ] === 0 ) {
			CoC.flags[ kFLAGS.RIDDLE_TWO ] = choices[ Utils.rand( choices.length ) ];
			CoC.flags[ kFLAGS.RIDDLE_TWO ]();
		} else {
			CoC.flags[ kFLAGS.RIDDLE_THREE ] = choices[ Utils.rand( choices.length ) ];
			CoC.flags[ kFLAGS.RIDDLE_THREE ]();
		}
	};
	...
	DungeonSandWitch.prototype.riddleOne = function() {
		MainView.clearOutput();
		MainView.outputText( '"<i>Hmm, here\'s an old one, but a good one, I think.  One of my favorites, but stop me if you know the answer already: \'I walk on four legs in the morning, two in the afternoon, and three in the evening.  What am I?\'</i>"' );
		EngineCore.menu();
		//[A Sphinx] (if PC int < 50 add this Centaur]) [A Man] (if PC int < 35 add this: [Stilts]) [Fuck it, Attack];
		EngineCore.addButton( 0, 'A Sphinx', this, this.riddleOneSphinx );
		EngineCore.addButton( 1, 'A Centaur', this, this.answerWrong );
		EngineCore.addButton( 2, 'A Man', this, this.answerCorrect );
		if( CoC.player.inte < 35 ) {
			EngineCore.addButton( 3, 'Stilts', this, this.answerWrong );
		}
		EngineCore.addButton( 4, 'Uh, ATTACK!', this, this.fuckItAttack );
	};
	//Special Occurrence: Pick [A Sphinx];
	DungeonSandWitch.prototype.riddleOneSphinx = function() {
		MainView.clearOutput();
		MainView.outputText( 'The sphinx narrows her eyes at you, crossing her arms over her chest.  "<i>You don\'t say?  Come on, step it up, [name].  Sorry, but that\'s wrong.  Let\'s try again.</i>"' );
		EngineCore.menu();
		if( CoC.flags[ kFLAGS.RIDDLE_ONE ] === this.riddleOne ) {
			CoC.flags[ kFLAGS.RIDDLE_ONE ] = 0;
		} else if( CoC.flags[ kFLAGS.RIDDLE_TWO ] === this.riddleOne ) {
			CoC.flags[ kFLAGS.RIDDLE_TWO ] = 0;
		} else if( CoC.flags[ kFLAGS.RIDDLE_THREE ] === this.riddleOne ) {
			CoC.flags[ kFLAGS.RIDDLE_THREE ] = 0;
		}
		EngineCore.addButton( 0, 'Next', this, this.riddlePicker );
	};
	...
	DungeonSandWitch.prototype.riddleTwo = function() {
		MainView.clearOutput();
		MainView.outputText( '"<i>Mmm, I\'ve always been fond of this one.  Delightfully simple, really - don\'t overthink it, dear.  \'I always run, yet never walk; I murmur often, yet never talk; and I\'ve a bed, yet never sleep.  What am I?\'</i>"' );
		//[A River] [A Whisper] (if PC int < 50 add this Nail]) (if PC int < 35 add this: [A Bunny Girl]) [Fuck it, Attack];
		EngineCore.menu();
		EngineCore.addButton( 0, 'A River', this, this.answerCorrect );
		EngineCore.addButton( 1, 'A Whisper', this, this.answerWrong );
		if( CoC.player.inte < 50 ) {
			EngineCore.addButton( 2, 'A Nail', this, this.answerWrong );
		}
		if( CoC.player.inte < 35 ) {
			EngineCore.addButton( 3, 'A Bunny-Girl', this, this.answerWrong );
		}
		EngineCore.addButton( 4, 'Uh, ATTACK!', this, this.fuckItAttack );
	};
	//RIDDLE 3;
	DungeonSandWitch.prototype.riddleThree = function() {
		MainView.clearOutput();
		MainView.outputText( '"<i>Ooh, this is a morbid one: \'Whoever makes me, sells me.  He who buys me, never uses me. And he who uses me, well, they\'ll never know it.  What am I?\'</i>"' );
		EngineCore.menu();
		//[Poison] (if PC int < 50 add this Condom]) (if PC int < 35 add this: [Arrows]) [A Coffin] [Fuck it, Attack];
		EngineCore.addButton( 0, 'Poison', this, this.answerWrong );
		if( CoC.player.inte < 50 ) {
			EngineCore.addButton( 1, 'A Condom', this, this.answerWrong );
		}
		if( CoC.player.inte < 35 ) {
			EngineCore.addButton( 2, 'Arrows', this, this.answerWrong );
		}
		EngineCore.addButton( 3, 'A Coffin', this, this.answerCorrect );
		EngineCore.addButton( 4, 'Uh, ATTACK!', this, this.fuckItAttack );
	};
	//RIDDLE 4;
	DungeonSandWitch.prototype.riddleFour = function() {
		MainView.clearOutput();
		MainView.outputText( '"<i>Here\'s a classic for you: \'I\'m always hungry, and must be fed always, lest I flicker away.  Yet I will always bite the hand that feeds me, if it touches me.  What am I?\'</i>"' );
		EngineCore.menu();
		//(if PC int < 35 add this Fairy]) [Fire] [A Dog] (if PC int < 50 add this: [Disease]) [Fuck it, Attack];
		if( CoC.player.inte < 35 ) {
			EngineCore.addButton( 0, 'A Fairy', this, this.answerWrong );
		}
		EngineCore.addButton( 1, 'Fire', this, this.answerCorrect );
		EngineCore.addButton( 2, 'A Dog', this, this.answerWrong );
		if( CoC.player.inte < 50 ) {
			EngineCore.addButton( 3, 'Disease', this, this.answerWrong );
		}
		EngineCore.addButton( 4, 'Uh, ATTACK!', this, this.fuckItAttack );
	};
	//RIDDLE 5;
	DungeonSandWitch.prototype.riddleFive = function() {
		MainView.clearOutput();
		MainView.outputText( '"<i>A favorite of my dear Goblin friends: \'When young, I am sweet in the sun; in middle age, I make you gay; but when I\'m old, I\'m more valuable than gold.  What am I?\'</i>"' );
		EngineCore.menu();
		//(if PC int < 50 add this) (if PC int < 35 add this: [Women]) [Wine] [Cheese] [Fuck it, Attack];
		if( CoC.player.inte < 50 ) {
			EngineCore.addButton( 0, 'Men', this, this.answerWrong );
		}
		if( CoC.player.inte < 35 ) {
			EngineCore.addButton( 1, 'Women', this, this.answerWrong );
		}
		EngineCore.addButton( 2, 'Wine', this, this.answerCorrect );
		EngineCore.addButton( 3, 'Cheese', this, this.answerWrong );
		EngineCore.addButton( 4, 'Uh, ATTACK!', this, this.fuckItAttack );
	};
	//RIDDLE 6;
	DungeonSandWitch.prototype.riddleSix = function() {
		MainView.clearOutput();
		MainView.outputText( '"<i>This riddle always makes me a bit sad, but... \'My life lasts but hours, and in service I\'m devoured.  Thin, I am quick, fat I am slow, and wind is ever my foe.  What am I?\'</i>"' );
		//(if PC int < 35 add this Goblin]) [A Candle] (if PC int < 50 add this: [A Boat]) [An Arrow] [Fuck it, Attack];
		EngineCore.menu();
		if( CoC.player.inte < 35 ) {
			EngineCore.addButton( 0, 'A Goblin', this, this.answerWrong );
		}
		EngineCore.addButton( 1, 'A Candle', this, this.answerCorrect );
		if( CoC.player.inte < 50 ) {
			EngineCore.addButton( 2, 'A Boat', this, this.answerWrong );
		}
		EngineCore.addButton( 3, 'An Arrow', this, this.answerWrong );
		EngineCore.addButton( 4, 'Uh, ATTACK!', this, this.fuckItAttack );
	};
	//RIDDLE 7;
	DungeonSandWitch.prototype.riddleSeven = function() {
		MainView.clearOutput();
		MainView.outputText( '"<i>In service to the Sand Witches, I can sympathize with this riddle\'s speaker: \'Each morning I appear to lie at your feet; all day I\'ll follow you no matter how fast you run, yet I\'ll nearly perish in the midday sun.  What am I?\'</i>"' );
		//[A Shadow] [A Dog] (if PC int < 35 add this) (if PC int < 50 add this: [The Breeze]) [Fuck it, Attack];
		EngineCore.menu();
		EngineCore.addButton( 0, 'A Shadow', this, this.answerCorrect );
		EngineCore.addButton( 1, 'A Dog', this, this.answerWrong );
		if( CoC.player.inte < 35 ) {
			EngineCore.addButton( 2, 'Water', this, this.answerWrong );
		}
		if( CoC.player.inte < 50 ) {
			EngineCore.addButton( 3, 'The Breeze', this, this.answerWrong );
		}
		EngineCore.addButton( 4, 'Uh, ATTACK!', this, this.fuckItAttack );
	};
	//RIDDLE 8;
	DungeonSandWitch.prototype.riddleEight = function() {
		MainView.clearOutput();
		MainView.outputText( '"<i>Tsk, a sad tale this, fleeting as a melody on the breeze: \'You\'ve heard me before, and will again, till fast I die - then you\'ll but summon me again.  What am I?\'</i>"' );
		//(if PC int < 35 add this Demon]) (if PC int < 50 add this: [Religion]) [An Idea] [An Echo] [Fuck it, Attack];
		EngineCore.menu();
		if( CoC.player.inte < 35 ) {
			EngineCore.addButton( 0, 'A Demon', this, this.answerWrong );
		}
		if( CoC.player.inte < 50 ) {
			EngineCore.addButton( 1, 'Religion', this, this.answerWrong );
		}
		EngineCore.addButton( 2, 'An Idea', this, this.answerWrong );
		EngineCore.addButton( 3, 'An Echo', this, this.answerCorrect );
		EngineCore.addButton( 4, 'Uh, ATTACK!', this, this.fuckItAttack );
	};
	//RIDDLE 9;
	DungeonSandWitch.prototype.riddleNine = function() {
		MainView.clearOutput();
		MainView.outputText( '"<i>Like a cat comes this riddle of many lives: \'Three lives have I, gentle enough to sooth the skin, light enough to caress the sky, or hard enough to shatter stone.  What am I?\'</i>"' );
		EngineCore.menu();
		//(if PC int < 35 add this Dick]) [Water] [A Voice] (if PC int < 50 add this: [Faith]) [Fuck it, Attack];
		if( CoC.player.inte < 35 ) {
			EngineCore.addButton( 0, 'A Dick', this, this.answerWrong );
		}
		EngineCore.addButton( 1, 'Water', this, this.answerCorrect );
		EngineCore.addButton( 2, 'A Voice', this, this.answerWrong );
		if( CoC.player.inte < 50 ) {
			EngineCore.addButton( 3, 'Faith', this, this.answerWrong );
		}
		EngineCore.addButton( 4, 'Uh, ATTACK!', this, this.fuckItAttack );
	};
	//RIDDLE 10;
	DungeonSandWitch.prototype.riddleTen = function() {
		MainView.clearOutput();
		MainView.outputText( '"<i>Devious, this: \'Whoever makes me, tells it not; he who takes me, knows it not; and he who knows me, takes me not.  What am I?\'</i>"' );
		//[Disease] [Counterfeits] (if PC int < 35 add this in a River]) (if PC int < 50 add this: [Tattered Scrolls]) [Fuck it, Attack];
		EngineCore.menu();
		EngineCore.addButton( 0, 'Disease', this, this.answerWrong );
		EngineCore.addButton( 1, 'Counterfeits', this, this.answerCorrect );
		if( CoC.player.inte < 35 ) {
			EngineCore.addButton( 2, 'Piss In A River', this, this.answerWrong );
		}
		if( CoC.player.inte < 50 ) {
			EngineCore.addButton( 3, 'Tattered Scrolls', this, this.answerWrong );
		}
		EngineCore.addButton( 4, 'Uh, ATTACK!', this, this.fuckItAttack );
	};
	//RIDDLE 11;
	DungeonSandWitch.prototype.riddleEleven = function() {
		MainView.clearOutput();
		MainView.outputText( '"<i>The lovely Naga that roams this desert knows my speaker well: \'A box without hinges, lock or key, yet I\'ve golden treasure within me.  What am I?\'</i>"' );
		//(if PC int < 35 add this Treasure Chest]) (if PC int < 50 add this: [Pure Honey]) [Eggs] [Booze Bottles] [Fuck it, Attack];
		EngineCore.menu();
		if( CoC.player.inte < 35 ) {
			EngineCore.addButton( 0, 'Teasure Chest', this, this.answerWrong );
		}
		if( CoC.player.inte < 50 ) {
			EngineCore.addButton( 1, 'Pure Honey', this, this.answerWrong );
		}
		EngineCore.addButton( 2, 'Eggs', this, this.answerCorrect );
		EngineCore.addButton( 3, 'Booze Bottles', this, this.answerWrong );
		EngineCore.addButton( 4, 'Uh, ATTACK!', this, this.fuckItAttack );
	};
	//RIDDLE 12;
	DungeonSandWitch.prototype.riddleTwelve = function() {
		MainView.clearOutput();
		MainView.outputText( '"<i>Here we\'ve one that speaks to the stars and the earth at once: \'One by one we fall from the heavens, down into the depths of the past; our world is ever upturned, so that yet some time will last.  What are we?\'</i>"' );
		//(if PC int < 50 add this Stars]) (if PC int < 35 add this: [Angels]) [Sand] [Rain] [Fuck it, Attack];
		EngineCore.menu();
		if( CoC.player.inte < 50 ) {
			EngineCore.addButton( 0, 'Fallen Stars', this, this.answerWrong );
		}
		if( CoC.player.inte < 35 ) {
			EngineCore.addButton( 1, 'Angels', this, this.answerWrong );
		}
		EngineCore.addButton( 2, 'Sand', this, this.answerCorrect );
		EngineCore.addButton( 3, 'Rain', this, this.answerWrong );
		EngineCore.addButton( 4, 'Uh, ATTACK!', this, this.fuckItAttack );
	};
	...
	DungeonSandWitch.prototype.answerCorrect = function() {
		...
		if( CoC.flags[ kFLAGS.RIDDLE_THREE ] === 0 ) {
			MainView.outputText( 'The sphinx-girl sighs, "<i>That\'s... correct.  Not bad, I suppose.  Well, we\'re not done yet... I\'ve still got some tricks up my sleeves.  Er, so to speak.</i>"' );
			EngineCore.menu();
			EngineCore.addButton( 0, 'Next', this, this.riddlePicker );
		} else {
			//BEAT THE SPHINX AT HER OWN GAME (First Time);
			if( CoC.flags[ kFLAGS.BEATEN_SANURA_COUNT ] === 0 ) {
				MainView.outputText( '"<i>W-what!?</i>"  the sphinx gasps as you correctly answer the last of her riddles.   "<i>I-I don\'t... but how?  No one\'s ever had the wits to answer all three riddles!</i>"' );
			...
			//BEAT THE SPHINX AT HER OWN GAME (REPEAT);
				...
				MainView.outputText( '"<i>Tsk, I lose again!</i>" Sanura pouts, crossing her arms.  "<i>I suppose you\'ll just have to give me a victory ravishing, won\'t you?  Go easy, though... you\'ve already damaged my pride so....</i>"' );
			}
			CoC.flags[ kFLAGS.BEATEN_SANURA_COUNT ]++;
			MainView.outputText( '\n\nWhat do you do?' );
			//(Display Options: [Fuck Her] [Door] [Leave]);
			EngineCore.menu();
			EngineCore.addButton( 0, 'North Door', this, this.openZeDoorToParadize );
			EngineCore.addButton( 1, 'Fuck Her', this, this.fuckDatSphinx );
			EngineCore.addButton( 4, 'Leave', this, this.leaveBoobsDungeon );
		}
	};
	...
	DungeonSandWitch.prototype.answerWrong = function() {
		MainView.clearOutput();
		MainView.outputText( '"<i>That\'s... wrong, I\'m afraid,</i>" she says, a proud smile playing across her lips.  "<i>Wrong, wrong, wrong.  Well, perhaps </i>I<i> was wrong about </i>you<i> after all...  Here I thought you\'d be some fun.  Still, though, at least your body will serve me well, I think.  Yes, you\'ll do nicely.  Go on then, love, strip out of your [armor] for me.  A deal\'s a deal, after all...</i>"' );
		//(Display Options: [Submit] [Fuck it, Attack]);
		//(Submit goes to appropriate loss scene);
		EngineCore.menu();
		EngineCore.addButton( 0, 'Submit', this, this.sphinxSubmissionOptions );
		EngineCore.addButton( 1, 'Uh, ATTACK!', this, this.fuckItAttack );
	};
	//FUCK IT, ATTACK (First Time);
	DungeonSandWitch.prototype.fuckItAttack = function() {
		MainView.clearOutput();
		MainView.outputText( 'To hell with this. You ready your [weapon] to beat your way through the damned sphinx, but as soon as you make a threatening move, the half-lion makes a shrill "<i>EEEP</i>" and throws her hands up in surrender. "<i>Please, th-there\'s no need for violence! I\'ve no loyalty to these witches, they </i>force<i> me to guard the door. I\'ve no desire to fight you; I\'ll not stop you if you wish to enter the lair.</i>"' );
	...
	DungeonSandWitch.prototype.getBlown = function( submit ) {
		...
		if( CoC.player.balls > 0 ) {
			MainView.outputText( 'balls empty' );
		} else {
			MainView.outputText( 'body empties' );
		}
		MainView.outputText( ' out onto her eager face.  As you feel yourself winding down from the magically-induced orgasm, however, a strange blue sigil appears in the air, encircling ' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 'each of your ' );
		} else {
			MainView.outputText( 'your ' );
		}
		MainView.outputText( 'dripping cock' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( '.  "<i>You do want to go again, yes?  I think I\'ve got enough magic to keep that nice [cock biggest] hard for a little fun with my new hands....</i>"' );
		...
		MainView.outputText( ' - the salty liquid flows freely onto the desert sands from Sanura\'s open mouth as your ' );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( '[balls] churn' );
		} else {
			MainView.outputText( 'body churns' );
		}
		MainView.outputText( ' out a seemingly endless supply of seed.  Your breath comes ragged as more and more spills out of you, your newly-potent manhood making you feel lightheaded as it expels an endless tide of cum.  Slowly, gently, Sanura leans in, wrapping her lips around the tip of your [cock biggest], lapping up the river of semen that pours into her mouth.' );
		//If Multi-cock = 3;
		if( CoC.player.cockTotal() >= 3 ) {
			...
			if( CoC.player.cockTotal() === 2 ) {
				...
				MainView.outputText( 'two of your ' );
			}
			MainView.outputText( 'other ' + Utils.num2Text( CoC.player.cockTotal() - 1 ) + ' members, wrapping tightly around their meaty lengths.  You groan throatily as tingling digits slowly work their way up your shafts, making your heart flutter wildly with the alien sensation.' );
			//AND If Multicock = 5;
			if( CoC.player.cockTotal() >= 5 ) {
				MainView.outputText( '  You barely have time to adjust to the eldritch hands jerking off your extra cocks when Sanura\'s own come into play, wading into the jumble of wangs growing from your crotch to grab another pair of pricks.  With five cocks encircled in hands, magic, and mouth, Sanura slowly begins to synchronize her movements, gently stroking your many cocks.' );
			}
		}
		...
		MainView.outputText( ' geysering into the air, raining back down in a pool around you and the now cum-covered sphinx.' );
	...
	DungeonSandWitch.prototype.forceDildos = function() {
		MainView.clearOutput();
		MainView.outputText( 'You start to peel out of your [armor], but a twirled finger from Sanura has you turn to face the stone pillar and brace yourself.  With silent grace, the half-cat slinks up to you, wandering hands moving to explore your body, her slender fingers tracing along the supple curves of your [butt] and [hips], slowly moving toward your [chest] with slow, teasing motions, her every shift sending shivers up your spine.  Your breath catches as her fingers brush your [nipples], gently squeezing them' );
		//if MilkyTits:;
		if( CoC.player.lactationQ() >= 200 ) {
			MainView.outputText( ' until a gentle trickle of your motherly fluids pours out across her hands.  With a girlish giggle, the sphinx slips her head under your arm, lapping at your milky teats.  A throaty purr escapes her lips before you give her a little push, getting her back to the task at hand' );
		}
		MainView.outputText( '.' );
	...
	DungeonSandWitch.prototype.fuckDatLionPussah = function() {
		MainView.clearOutput();
		MainView.outputText( 'You disrobe, tossing your [armor] aside into a small pile.  You stretch your muscles in the dry desert air and exult in the warm rays beating down on your ' + CoC.player.skinFurScales() + ' and ' + CoC.player.multiCockDescriptLight() + '.  Sanura pads around you, taking in your appearance with her chestnut-colored eyes before clicking her tongue approvingly.  Her leonine tail swishes across the top of your [cock biggest], stroking it with the silky soft tuft of fur at its tip.  You shiver at the contact, unsure of what to think.  Your penis, on the other hand, has no such conflictions, and immediately begins to rise.' );
		//(Small dicks) ;
		if( CoC.player.biggestCockArea() < 6 ) {
		...
		else if( CoC.player.biggestCockArea() < 30 ) {
			...
			MainView.outputText( ' with the little tuft of fur.  "<i>That bad boy will be just perfect.</i>"' );
		} else {
		...
		MainView.outputText( '\n\nYour manhood spikes upwards at the attention, becoming painfully hard as the sphinx continues to survey your body.  Her tail flicks over your ' + CoC.player.skinFurScales() + ' gently, stopping briefly to tantalize ' );
		if( CoC.player.totalNipples() === 2 ) {
			MainView.outputText( 'both' );
		} else {
			MainView.outputText( 'each' );
		}
		MainView.outputText( ' of your nipples, which stiffen in response, just like your cock.  You can\'t help but think there\'s some sort of magic at work here.  Her tail passes between your thighs, gracefully tickling your ' );
		if( CoC.player.hasVagina() ) {
			MainView.outputText( 'damp cunt, which suddenly tightens up and practically gushes with slick juices.' );
		} else {
			MainView.outputText( 'asshole, which reflexively tightens, sending a shiver up your spine.' );
		}
		...
		if( CoC.player.hasKnot( CoC.player.biggestCockIndex() ) ) {
			MainView.outputText( ', your knot slipping into place with a moist <b>pop</b>, ensuring you\'ll be locked together for the foreseeable future' );
		}
		MainView.outputText( '.  Jet after jet of cum erupts from your [cock], painting the sphinx\'s inner walls white.' );
		//(low-average cum);
		if( CoC.player.cumQ() < 250 ) {
		...
		if( CoC.player.hasKnot( CoC.player.biggestCockIndex() ) ) {
			MainView.outputText( 'which proves somewhat uncomfortable, given your knot is still lodged firmly within her, ' );
		}
		MainView.outputText( 'panting wildly.  It takes you several minutes to recover, the sphinx even longer, ' );
		if( CoC.player.hasKnot( CoC.player.biggestCockIndex() ) ) {
			MainView.outputText( 'and even longer still for your knot to deflate enough to slip out of her, ' );
		}
		MainView.outputText( 'but you finally gather the strength to haul yourself up onto your [feet].' );
	...
	DungeonSandWitch.prototype.sphinxSubmissionOptions = function() {
		...
		MainView.outputText( 'Having lost the Riddle Game, you do as the sphinx-girl commands: you pull off your [armor], leaving yourself bare under the desert sun. "<i>Now then, my dull little pet, what shall I do with you...</i>"' );
		EngineCore.menu();
		//SUBMISSION:;
		EngineCore.addButton( 0, 'Get Pegged', this, this.fenPutsHisShittyFetishInYoSphinx );
		//Reqs Penis;
		if( CoC.player.hasCock() ) {
			EngineCore.addButton( 1, 'Get Blown', this, this.getBlown, true );
			EngineCore.addButton( 2, 'Pawjob', this, this.lionpaws, true );
		}
	};
	//Horsecock Symphony;
	DungeonSandWitch.prototype.fenPutsHisShittyFetishInYoSphinx = function() {
		MainView.clearOutput();
		MainView.outputText( 'Starting to disrobe, ' );
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'you flop your ' + CoC.player.multiCockDescriptLight() + ' free in the dry desert heat, allowing ' );
			if( CoC.player.cockTotal() === 1 ) {
				MainView.outputText( 'it' );
			} else {
				MainView.outputText( 'them' );
			}
			MainView.outputText( ' to sway back and forth' );
		} else if( CoC.player.hasVagina() ) {
			MainView.outputText( 'you expose your [vagina] to the hot desert air, allowing it to moisten' );
		} else {
			MainView.outputText( 'you expose your [butt], allowing it to redden' );
		}
		MainView.outputText( ' as the omnipresent heat and seductive sphinx have their way with your heightened libido.  Sanura snaps her palm up in front of your face as your [armor] hits the ground, landing with a heavy thump.' );
		...
		if( CoC.player.analCapacity() >= 100 ) {
			...
			MainView.outputText( '\n\n"<i>Mmmm, I knew you\'d be a good ' + CoC.player.mf( 'boy', 'girl' ) + '.</i>"' );
		}
		//Medium capacity! 40+!;
			...
			MainView.outputText( '\n\nElectrically charged balls the size of large melons plop against your [butt] as she bottoms out.  "<i>Mmmm, I knew you\'d be a good ' + CoC.player.mf( 'boy', 'girl' ) + '.</i>" Sanura purrs, patting you on the head.' );
		}
		//Lowish Capacity;
		else {
		...
		if( CoC.player.cockTotal() > 0 ) {
			MainView.outputText( '  Pre-cum oozes from [eachCock] in steady strings that dangle onto the dirt, painting a swirled pattern beneath you as your prostate is pushed hard enough to squeeze trickles of your salty \'milk\' from you.' );
		}
		...
		if( CoC.player.cockTotal() > 0 ) {
			MainView.outputText( '  Whenever her flare slides past your prostate, the wide, blunt head squeezes it hard enough to express a huge strand of pre.' );
		}
		MainView.outputText( '  After a few such thrusts, her bouncing, fake balls rise up in their faux, fleshy sack.  Her swollen dick-tip engorges more than you thought possible inside you, wide enough for you to touch through your belly and marvel at the size.  Quivering from head to four lion-like toes, Sanura cums.' );
		...
		if( CoC.player.cockTotal() > 0 ) {
			MainView.outputText( '  When she crosses the lump that is your well used prostate, you lose control and shudder with a nice, long cum of your own, your arms giving out and dumping you face-down on the ground while you spurt milky whiteness onto your chest, chin, and then the ground.' );
		} else {
			MainView.outputText( '  When she\'s almost out, you give in and cum, quivering around her magical breeding tool while it finishes inseminating your guts.' );
		}
		MainView.outputText( '  The head is so swollen that she actually struggles to pop it back out of your [asshole], bouncing her hips back a few times with little success' );
		if( CoC.player.cockTotal() > 0 ) {
			MainView.outputText( ' and dragging you through your fresh, milky puddle' );
		}
		MainView.outputText( '.  Eventually, she gives a mighty, sphincter-stretching lurch and ejects from your [butt], painting it white on the way out.' );
	...
	DungeonSandWitch.prototype.sandWitchMotherFriendlyMenu = function() {
		...
		if( CoC.flags[ kFLAGS.SAND_WITCH_LEAVE_ME_ALONE ] === 0 ) {
			EngineCore.addButton( 0, 'StopAttacking', this, this.leaveAloneSendLackeysToggle );
		} else {
			EngineCore.addButton( 0, 'StartAttacking', this, this.leaveAloneSendLackeysToggle );
		}
		EngineCore.addButton( 1, 'SandMothers', this, this.askHowSandMothersAreChosen );
		EngineCore.addButton( 2, 'Cum Witches', this, this.discussCumWitches );
		EngineCore.addButton( 3, 'CovenStatus', this, this.currentStateOfInterwebs );
		EngineCore.addButton( 4, 'History', this, this.sandWitchHistory );
		if( CoC.flags[ kFLAGS.SAND_WITCH_LOOT_TAKEN ] < 5 ) {
			EngineCore.addButton( 5, 'Get LaBova', this, this.getLaBova );
			EngineCore.addButton( 6, 'Get Lactaid', this, this.getLactaidFromWitches );
		}
		if( (CoC.flags[ kFLAGS.ESSY_MET_IN_DUNGEON ] > 0 && CoC.flags[ kFLAGS.TOLD_MOTHER_TO_RELEASE_ESSY ] === 0) || (CoC.flags[ kFLAGS.MET_MILK_SLAVE ] > 0 && !isNaN( CoC.flags[ kFLAGS.MILK_NAME ] )) ) {
			EngineCore.addButton( 7, 'Free Slaves', this, this.slavesDiscussion );
		}
		if( CoC.player.lust >= 33 ) {
			EngineCore.addButton( 8, 'Sex', this, this.sexWithFriendlySandMother );
		}
	...
	DungeonSandWitch.prototype.slavesDiscussion = function() {
		...
		if( CoC.flags[ kFLAGS.ESSY_MET_IN_DUNGEON ] > 0 && CoC.flags[ kFLAGS.TOLD_MOTHER_TO_RELEASE_ESSY ] === 0 ) {
			EngineCore.addButton( 0, 'Essrayle', SceneLib.essrayle, SceneLib.essrayle.askMotherToReleaseEssy );
		}
		if( CoC.flags[ kFLAGS.MET_MILK_SLAVE ] > 0 && !isNaN( CoC.flags[ kFLAGS.MILK_NAME ] ) ) {
			EngineCore.addButton( 1, 'Milk-Slave', this, this.freeSlaves );
		}
	};
	...
	DungeonSandWitch.prototype.freeSlaves = function() {
		MainView.clearOutput();
		MainView.outputText( 'Thinking back to the poor, mind-broken tittymonster of a bath girl you met here in the witches\' coven, you tell the Sand Mother it\'s about time to free her and any other slaves she\'s got hidden away.' );
		...
		//[Yes] [No] [Gimme her];
		EngineCore.menu();
		EngineCore.addButton( 0, 'Yes', this, this.yesDemandMilkRelease );
		EngineCore.addButton( 1, 'No', this, this.noDemandMilkRelease );
		EngineCore.addButton( 2, 'Gimme Her', this, this.gimmeDatDeliciousMilkWaifuINeedMoreWaifusCauseTheTwoCowslutsWerentEnoughForMyInsatiableLacticLustandDesire );
	};
	//Yes (No Change);
	DungeonSandWitch.prototype.yesDemandMilkRelease = function() {
		MainView.clearOutput();
		MainView.outputText( 'You tell the Sand Mother that everyone deserves freedom, even if they\'re addle-minded, or useful.  She can\'t just <i>keep people</i> because she feels like they\'re better off in her care.' );
	...
	DungeonSandWitch.prototype.gimmeDatDeliciousMilkWaifuINeedMoreWaifusCauseTheTwoCowslutsWerentEnoughForMyInsatiableLacticLustandDesire = function() {
		MainView.clearOutput();
		MainView.outputText( 'You have a better idea: give the girl to you.  You can care for and protect her in your camp, but also give her at least as much freedom as is safe in these dire times.' );
		...
		EngineCore.menu();
		//[2 Expensive][Buy Her];
		EngineCore.addButton( 0, '2 Expensive', this, this.TwoExpensive4Me );
		if( CoC.player.gems >= 2000 ) {
			EngineCore.addButton( 1, 'Buy Her', this, this.BuyHer );
		}
	};
	DungeonSandWitch.prototype.TwoExpensive4Me = function() {
		MainView.clearOutput();
		MainView.outputText( 'You explain that you can\'t afford that much.' );
	...
	DungeonSandWitch.prototype.BuyHer = function() {
		...
		MainView.outputText( 'You hand over two thousand of your hard-earned gems.' );
		...
		EngineCore.menu();
		EngineCore.addButton( 0, 'Next', SceneLib.milkWaifu, SceneLib.milkWaifu.arriveWithLacticWaifuAtCamp );
	};
	DungeonSandWitch.prototype.sexWithFriendlySandMother = function() {
		EngineCore.menu();
		//friendly cunt fuck:;
		if( CoC.player.hasVagina() && CoC.player.lust >= 33 ) {
			EngineCore.addButton( 0, 'GetMilkFill', this, this.lesboMilkFilling );
		}
		//dick fucking, must fit!;
		if( CoC.player.hasCock() && CoC.player.lust >= 33 ) {
			if( CoC.player.cockThatFits( CoC.monster.vaginalCapacity() ) >= 0 ) {
				EngineCore.addButton( 1, 'Fuck Her', this, this.friendlySandMotherFuck );
			}
		}
		EngineCore.addButton( 4, 'Back', this, this.sandWitchMotherFriendlyMenu );
	};
	...
	DungeonSandWitch.prototype.sandMotherPOMenu = function() {
		...
		if( CoC.flags[ kFLAGS.SAND_WITCH_LEAVE_ME_ALONE ] === 0 ) {
			EngineCore.addButton( 0, 'StopAttacking', this, this.unfriendlyWitchToggle );
		} else {
			EngineCore.addButton( 0, 'StartAttacking', this, this.unfriendlyWitchToggle );
		}
		if( CoC.player.lust >= 33 ) {
			if( CoC.player.hasVagina() ) {
				EngineCore.addButton( 1, 'Scissor', this, this.scissorAndDrink );
			}
			if( CoC.player.tentacleCocks() >= 5 ) {
				EngineCore.addButton( 2, 'TentacleBang', this, this.tentacleGangBang );
			}
			if( CoC.player.hasCock() ) {
				if( CoC.player.cockThatFits( CoC.monster.vaginalCapacity() ) >= 0 ) {
					EngineCore.addButton( 3, 'Fuck Her', this, this.fuckTheSandMothersCunt );
				}
			}
		}
		if( CoC.flags[ kFLAGS.SAND_WITCH_LOOT_TAKEN ] < 5 ) {
			EngineCore.addButton( 5, 'Get LaBova', this, this.takeLaBovaOrLactaid, false );
			EngineCore.addButton( 6, 'Get Lactaid', this, this.takeLaBovaOrLactaid );
		}
		if( CoC.flags[ kFLAGS.ESSY_MET_IN_DUNGEON ] > 0 && CoC.flags[ kFLAGS.TOLD_MOTHER_TO_RELEASE_ESSY ] === 0 ) {
			EngineCore.addButton( 7, 'Essrayle', SceneLib.essrayle, SceneLib.essrayle.askMotherToReleaseEssy );
		}
		EngineCore.addButton( 9, 'Leave', null, MainView.playerMenu );
	};
	...
	DungeonSandWitch.prototype.sandMotherStuffGOA = function() {
		...
		//*Encounter Repeat:;
		if( CoC.flags[ kFLAGS.SAND_WITCHES_FRIENDLY ] > 0 ) {
			MainView.outputText( 'The Sand Mother slightly inclines her head in your direction as you enter her throne room.  She is every bit as imposing as you recall, or at least as imposing as a woman with four watermelon-sized jugs on her chest can be.  "<i>You have returned to us, far traveler.  Is there something you desire from the coven of the sands?  Simply ask for what you need, and we shall aid you against the demon Queen.</i>"  The lactation-obsessed arch-enchantress idly pushes a whitish-blonde curl out of her eyes as she awaits your reply.' );
			//Display Friendly Choices;
			this.sandWitchMotherFriendlyMenu();
			return;
		}
		//*Encounter Repeat: Cowed;
		else if( CoC.flags[ kFLAGS.SAND_MOTHER_DEFEATED ] > 0 ) {
			MainView.outputText( 'The Sand Mother glares down at you from atop her throne, but does not dare lift a finger against you, not after her previous defeat.  She hisses, "<i>You\'ve already plundered my coven!  What more do you want from us?</i>"  even as her chest heaves in a panic.  Four mountainous peaks jut from the witch\'s silken robe... you have a few ideas, and the Queen Witch seems to have some of her own.' );
			//Resisting Options;
			...
			if( CoC.flags[ kFLAGS.SAND_WITCH_LOOT_TAKEN ] >= 5 ) {
				MainView.outputText( ', though right now it sits empty.  You\'ll need to wait for them to restock it' );
			} else {
				MainView.outputText( ', and noone could stop you from taking them' );
			}
			MainView.outputText( '.' );
		...
		//The Sand Mother gazes at you uncertainly as you return, her defeat at your hands still fresh in her memory.  The bleach-blond matriarch looks to have recovered from the fight, but she lamely queries, "<i>Well, you've proven yourself mightier than my magic.  All that remains is a choice.  Do you show your true colors, pawn of demons, or do you tread the path of virtue?</i>"  Sure, she puts on a brave face, but the nervous tics at the taut corners of her mouth reveal her worry. ;
			...
			if( CoC.player.cor >= 66 ) {
				MainView.outputText( 'You stink of corruption, but I still sense a soul within you.  You are not a demon... yet.' );
			} else {
				MainView.outputText( 'You do not stink of corruption over-much, like many in this land, but that does not mean you are not an agent of the Lethice\'s.' );
			}
			MainView.outputText( '  Explain your actions, heathen, or be scourged from this chamber like one of Lethice\'s demons.</i>"' );
			...
			//[Explain] [Fight];
			EngineCore.menu();
			EngineCore.addButton( 0, 'Explain', this, this.explainYourSelfToZeSandBitch );
			EngineCore.addButton( 1, 'Fight', this, this.fightTheSandWitch );
		}
	};
	//*Explain to Sand Mother;
	DungeonSandWitch.prototype.explainYourSelfToZeSandBitch = function() {
		MainView.clearOutput();
		MainView.outputText( 'You explain that you came here through a portal from a place called Ingnam, sent as a champion to defend your land from the demons.  Recounting your travels and experiences here, you confidently explain your discovery of this coven, and your misconceptions about the sand witches.  They seemed as lust crazed as the rest of the lands, and you had assumed them to be in league with the demons you found so frequently.' );
		...
		//(No new PG. Corruption is less than intellect and no major asshole moves made:) ;
			...
			MainView.outputText( '  "<i>Your tale has the ring of truth to it, [name].  I am the Sand Mother of this coven, and it seems we are not enemies after all.  If you like I can instruct my daughters to let you pass through the sands unhindered.</i>"  She spreads her arms graciously and asks, "<i>Was there anything you would like to discuss?  It has been an age since I\'ve had a new arrival to converse with.</i>"\n\n(+200 XP)' );
			//(Award XP);
			CoC.player.XP += 200;
	...
	DungeonSandWitch.prototype.gigaFire2 = function() {
		...
		//Not interrupted:;
		if( CoC.monster.statusAffectv1( StatusAffects.Gigafire ) < 10 ) {
			MainView.outputText( 'The Sand Mother grabs her scepter in both hands, combining the flames that wreath them into an immense, blinding conflagration.  She points at you, and the fire washes out in a wave like a serpent, twisting at you as you try to avoid it, doubling back on itself whenever it misses.  It\'s unavoidable!  You\'re enveloped in the consuming fire!' );
			damage *= 3;
		}
		//Interrupted:;
		else {
	...
	DungeonSandWitch.prototype.sandMotherTelekinesis = function() {
		MainView.outputText( 'She narrows her eyes at you, and an immense, magical pressure reaches out, taking hold of you.  It spins you in the air before slamming you into the walls!' );
		var sizeMod = 100;
		var thickMod = CoC.player.thickness / 100 + 0.5;
		sizeMod += CoC.player.tallness * thickMod;
		if( sizeMod < 140 ) {
			MainView.outputText( '  You immediately wish you weren\'t so small, as you\'re sure she couldn\'t have flung a heavier champion nearly as easily.' );
		} else if( sizeMod >= 200 ) {
			MainView.outputText( '  You\'re glad for your size, as she couldn\'t seem to accelerate you into the stone as fast as she\'d like.  Sometimes eating plenty pays off.' );
		}
	...
	DungeonSandWitch.prototype.defeatTheSandMother = function() {
		...
		MainView.outputText( 'Panting ' );
		if( CoC.player.HP < 1 ) {
			MainView.outputText( 'in exhaustion' );
		} else {
			MainView.outputText( 'with lust' );
		}
		MainView.outputText( ', the Sand Mother sinks back into her throne with sweat and milk staining her silky robes.  She groans softly, the milk-stains slowly spreading as you consider your options.  This woman dared to stand against you.  What do you do?' );
		//[Fuck Her Cunt] [ScissorAnDrink] [Talk Her Down];
		EngineCore.menu();
		EngineCore.addButton( 0, 'TalkHerDown', this, this.talkDownTheMother );
		if( CoC.player.hasVagina() ) {
			EngineCore.addButton( 1, 'Scissor', this, this.scissorAndDrink );
		}
		if( CoC.player.hasCock() && CoC.player.cockThatFits( CoC.monster.vaginalCapacity() ) >= 0 ) {
			EngineCore.addButton( 2, 'Fuck Her', this, this.fuckTheSandMothersCunt );
		}
		if( CoC.player.tentacleCocks() >= 5 ) {
			EngineCore.addButton( 3, 'TentacleFuck', this, this.tentacleGangBang );
		}
	};
	...
	DungeonSandWitch.prototype.tentacleGangBang = function() {
		...
		if( CoC.player.tentacleCocks() >= 6 ) {
			...
			if( CoC.player.tentacleCocks() >= 7 ) {
				MainView.outputText( 'untended serpentine shafts to please' );
			} else {
				MainView.outputText( 'an untended serpentine shaft to please' );
			}
			MainView.outputText( ', but all the holes are full!  You take the ' );
			if( CoC.player.tentacleCocks() === 6 ) {
				MainView.outputText( 'leftover one and thread it' );
			} else {
				MainView.outputText( 'leftover ones and thread them' );
			}
			MainView.outputText( ' through the dick-dominated enchantrix\'s fingers.  By now, her body has completely and utterly betrayed her, and she grabs hold like a cock-hungry slut, immediately stroking up and down, jacking it like a pro.  A muffled sigh of contentment shivers up your throat-fucking cock, alerting you to your depraved foe\'s prick-addled state.  You begin to pump your quartette of buried cocks into their respective holes with renewed vigor.  The titty-fucking vine pistons through its curvy path along with them, sending the witch\'s tits jiggling and squirting wildly.  The only thing' );
			if( CoC.player.tentacleCocks() === 6 ) {
				MainView.outputText( ' held still is the one getting a hand job, and you only hold it still' );
			} else {
				MainView.outputText( 's held still are the two getting hand jobs, and you only hold them still' );
			}
			MainView.outputText( ' so that part of you can relax and enjoy your captive Queen\'s enthusiastic wanking.' );
		}
		//(10+);
	...
	DungeonSandWitch.prototype.fuckTheSandMothersCunt = function() {
		...
		if( y >= 0 ) {
			MainView.outputText( ', grabbing a second cock as well, intent on penetrating both of her sopping entrances' );
		}
		MainView.outputText( '.  The moment her labia caress your ' );
		if( y >= 0 ) {
			MainView.outputText( 'cockheads' );
		} else {
			MainView.outputText( CoC.player.cockHead( x ) );
		}
		MainView.outputText( ', she freezes as she realizes what she\'s doing, and she tries to pull away.' );
		...
		if( y >= 0 ) {
			MainView.outputText( 'tools' );
		} else {
			MainView.outputText( Descriptors.cockDescript( x ) );
		}
		MainView.outputText( '.  She moans out loud, throwing her bleached tresses around wildly in pleasure while inch after inch of cock snakes into her passage' );
		if( y >= 0 ) {
			MainView.outputText( 's' );
		}
		...
		MainView.outputText( '.  Semen floods out of you from the force of your orgasm, creaming the juicy cunt' );
		if( y >= 0 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ' as ' );
		if( y < 0 ) {
			MainView.outputText( 'it clamps down' );
		} else {
			MainView.outputText( 'they clamp down' );
		}
		MainView.outputText( ', squirting femlube and leftover spunk across your [feet].' );
		if( CoC.player.cumQ() >= 500 ) {
			MainView.outputText( '  Again and again, you flood the poor spellcaster\'s twat' );
			if( y >= 0 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ', shooting so much that it bubbles out in thick globs to fall wetly on the ground.' );
			if( CoC.player.cumQ() >= 2000 ) {
				MainView.outputText( '  They fall faster and faster as you cum, the witch\'s belly bloating, rounding out to a ridiculous degree.  Her legs are drenched with the leaking spooge.' );
			}
		}
		MainView.outputText( '  Once you\'ve pumped every ounce of your virile fluid into the waiting receptacle' );
		if( y >= 0 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ', you pull, admiring the salty, mark you\'ve left on the once proud woman.' );
	...
	DungeonSandWitch.prototype.scissorAndDrink = function() {
		...
		if( CoC.player.tallness < 85 ) {
			MainView.outputText( ' larger' );
		} else if( CoC.player.tallness > 105 ) {
			MainView.outputText( ' smaller' );
		}
		MainView.outputText( ' woman, moist, slick bodies entwined as you both grope and grind.' );
		...
		MainView.outputText( '\n\nHer doubled labias have grown so hot that they\'re almost steaming against your [leg].  She keeps shifting to press them against you, leaving hot, slippery, cunt-shaped stamps of clear moisture on your ' + CoC.player.skinFurScales() + '.  Groaning with your own unfulfilled needs, you drink a deep draught of delectable milk and bring your own burning-hot furnace of need to bear on your once-foe.  Her creamy thigh feels like heaven against your vulva, and you lewdly grind your [clit] against her supple leg until you can feel her moisture starting to drip into your honeypot.  Your traitorous muscles lurch your sordid snatch straight into the Sand Mother\'s copious, clenching crotch, and both of your minds are buried under the avalanche of raw, sensuous pleasure.' );
		...
		if( CoC.player.wetness() >= 5 ) {
			MainView.outputText( 'explodes out in a cunt-shattering spray' );
		} else if( CoC.player.wetness() >= 4 ) {
			MainView.outputText( 'gushes out in a heavy spray' );
		} else if( CoC.player.wetness() >= 3 ) {
			MainView.outputText( 'pours out in a thick, wet river' );
		} else if( CoC.player.wetness() >= 2 ) {
			MainView.outputText( 'drools out in a steady trickles' );
		} else {
			MainView.outputText( 'drips out during the cunt-shattering convulsions' );
		}
		MainView.outputText( '.  Bulging obscenely, your cheeks barely contain the flood of milk, and you swallow it without thinking, acting entirely on instinct.' );
		MainView.outputText( '\n\nTwo strong arms encircle the back of your head and press your ' + CoC.player.face() + ' deeper into the gushing chest-flesh until you\'re left but no choice but to suckle and swallow while the fluttering cooches finish bathing each other in liquified orgasm.  You hum in super-sensitive bliss - well fed and sated, trembling every few seconds whenever your [clit] catches on a fold or the other two rigid buttons.  The Sand Mother\'s sculpted body slowly goes limper and limper under your attentions until she\'s on her back and your grinding draws a few, last trembling sparks of pleasure from her body.' );
	...
	DungeonSandWitch.prototype.loseToSandMotherBadEnd = function() {
		MainView.clearOutput();
		MainView.outputText( 'Stumbling back into a wall, you try to hold your ' );
		if( CoC.player.HP < 1 ) {
			MainView.outputText( 'battered body aloft, but your wounds are too great, robbing you of strength.' );
		} else {
			MainView.outputText( 'rebellious body aloft, but your lust is too great, turning your body into a quivering wreck.' );
		}
		MainView.outputText( '  You ' );
		if( CoC.player.HP < 1 ) {
			MainView.outputText( 'spitefully watch the Sand Mother approach, only able to admire her sculpted form as she towers over your prone form.' );
		} else {
			MainView.outputText( 'hungrily watch the Sand Mother approach, only able to touch yourself as you gaze up at her sculpted form' );
		}
		MainView.outputText( '.  She sniffs in disdain as she looks down at you.  "<i>Disappointing.  Lethice sends a weakling like you after us, and the sisters do not even slow you.  Their atonement shall be severe.</i>"  Her eyes, blue as the desert sky, consider you again, and she mutters, "<i>First, there is the matter of you, isn\'t there?</i>"' );
		...
		if( CoC.player.cor < 66 ) {
			MainView.outputText( 'aren\'t corrupt' );
		} else {
			MainView.outputText( 'don\'t serve the demons' );
		}
		MainView.outputText( ', looking up at her pleadingly ' );
		if( CoC.player.cockTotal() === 1 ) {
			MainView.outputText( 'while she gleefully pumps you, smearing your leaky juices all over your shaft.' );
		} else {
			MainView.outputText( 'while she gleefully moves to your second cock, smearing it with your first dick\'s pre-cum, messing your ' + CoC.player.multiCockDescriptLight() + ' with each penis\'s leaky drippings.' );
		}
		...
		if( CoC.player.cumQ() >= 500 ) {
			MainView.outputText( '  Wave after wave of jism washes across you, thoroughly drenching you with your spooge, the alabaster spunk soaking you from crown to waist with sticky slickness.' );
		}
		if( CoC.player.cumQ() >= 3000 ) {
			MainView.outputText( '  A puddle pools around you, deepening nicely as your boundless virility does its work, inching up your body until you feel as if you\'ll float away in your own pearly jism.' );
		}
		MainView.outputText( '\n\nJust as you\'re finishing, dribbling out a few weak spurts of cum, another robed figure enters the chamber.  Despite the penetrating scent of your ropey orgasmic goo filling the room, the smell of jizz intensifies at the new arrival, doubling in magnitude as the robed woman approaches you.  The Sand Mother wipes her foot off on your [leg] and whispers, "<i>Good ' + CoC.player.mf( 'boy', 'girl' ) + '.  Now relax, we\'ll make this feel good.</i>"  She turns away from you to greet the ebony sister.  This one isn\'t just dark-skinned or well-tanned... she\'s virtually midnight black.  Worse, her robes are spattered with whitish stains, and a protrusion that would put most men\'s erections to shame tents them with unabashed eagerness.  ' );
		if( CoC.flags[ kFLAGS.CUM_WITCH_DEFEATED ] > 0 ) {
			MainView.outputText( 'The Cum Witch is here, and she seems to have recovered from her defeat!' );
		} else {
			MainView.outputText( 'This must be the Cum Witch!' );
		}
		...
		if( CoC.player.balls > 0 ) {
			MainView.outputText( 'testes' );
		} else {
			MainView.outputText( 'prostate' );
		}
		MainView.outputText( ' emptying orgasm, you shudder and try to pull away, but there\'s nowhere to go.  Gentle hands place themselves to either side of your head, immobilizing you, and the lusty hermaphrodite begins to chant, punctuating her arcane words with slow pumps of her body across your cum-slicked middle' );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( ', two sets of slippery balls bouncing and sliding across each other' );
			if( EngineCore.silly() ) {
				MainView.outputText( '.  Great, now it\'s gay' );
			}
		}
		MainView.outputText( '.' );
		...
		if( !CoC.player.hasVagina() ) {
			MainView.outputText( 'innards' );
		} else {
			MainView.outputText( 'folds' );
		}
		MainView.outputText( ' thickens slightly, and the heaving balls below jiggle with liquid weight, swelling larger against you.  They grow heavier and heavier, until they slosh with audible, barely contained need, each as large as one of your butt-cheeks and three times as soft.' );
		...
		MainView.outputText( ', pumping into you with enough force to audible splash and churn about, gently thickening your middle.  A crystalline lattice-work erects from the mental syrup, climbing higher into a brand new frame of reference, filled with the thoughts and experiences of others - no, of your past.  New memories fill in the gaps while your gut rises, forming a small bump that would be easily mistaken for pregnancy.' );
	...
	DungeonSandWitch.prototype.loseToSandMother = function() {
		MainView.clearOutput();
		MainView.outputText( 'The world grows dizzy as your ' );
		if( CoC.player.HP < 1 ) {
			MainView.outputText( 'heavily punished and aching body is filled with too much agony to continue.' );
		} else {
			MainView.outputText( 'hot and aroused body fills with unbearable urges that finally distract you from fighting any further.' );
		}
		MainView.outputText( '  Instead, you fall backwards against the wall ' );
		if( CoC.player.HP < 1 ) {
			MainView.outputText( 'and grit your teeth in anger as the Sand Mother approaches.' );
		} else {
			MainView.outputText( 'panting deeply, and your blood races as the Sand Mother approaches.' );
		}
		MainView.outputText( '  You can not help but admire her incredible physique as she towers over you, looking down upon your body with disdain in her fierce eyes.  "<i>Lethice should have known better than to send such a pathetic imbecile like you to battle us.  I can hardly believe you managed to get by so many of our sisters.</i>"  She sighs.  "<i>Though I suppose it\'d be a greater waste if we just killed you.</i>"' );
		...
		if( !CoC.player.hasVagina() ) {
			MainView.outputText( 'Remarkably, a divot appears on your groin, slowly dividing into two puffy lips with a moist crease between.  The fresh mons of your new pussy glistens with immediate arousal as the Sand Mother\'s magic molds your flesh with a simple spell.  ' );
			CoC.player.createVagina();
			CoC.player.clitLength = 0.25;
		}
		MainView.outputText( 'Your [vagina] is eyed momentarily as it ' );
		if( CoC.player.wetness() <= 2 ) {
			MainView.outputText( 'grows moist.' );
		} else if( CoC.player.wetness() <= 4 ) {
			MainView.outputText( 'drips with arousal.' );
		} else {
			MainView.outputText( 'drools obscenely.' );
		}
		MainView.outputText( '  "<i>We can certainly improve upon this.</i>"  You attempt to voice opposition but she quickly silences you.  "<i>No! Understand here and now, you live by our mercy.  Your body belongs to the sand.  Your old life is over.  From now on, your failed life will serve a greater purpose.</i>"' );
		MainView.outputText( '\n\nYou try to resist but ' );
		if( CoC.player.HP < 1 ) {
			MainView.outputText( 'the pain of your abused body' );
		} else {
			MainView.outputText( 'lustful urges fill your senses and' );
		}
		MainView.outputText( ' hinders all of your attempts to fight back.  You try to argue that you aren\'t a servant of the demons, that you were fighting against them for your own people just as she is, but every time you open your mouth she forces it closed again with her magic.  "<i>I am not interested in the lies of a demonic servant.  It is time we dealt with you once and for all.</i>"' );
		...
		if( CoC.player.bRows() === 1 ) {
			MainView.outputText( '  Plus an extra set of these to ensure you produce as much milk as the rest of your soon-to-be sisters.' );
			CoC.player.createBreastRow();
		}
		//if player has four breasts;
		else if( CoC.player.bRows() === 2 ) {
			MainView.outputText( '  Thankfully you\'re already endowed with enough breasts to begin immediate production of milk.' );
		}//if player has six breasts;
		else {
			MainView.outputText( '  Hmmm. We\'ve never had a sister blessed with more than four breasts. I wonder if perhaps we can use your unusual anatomy to our advantage.' );
		}
		MainView.outputText( '  She squeezes a [nipple] and a yelp of pleasure escapes you.  "<i>I see you like that idea. Excellent.</i>"' );
		//if breasts are smaller than a normal sand witch's;
		if( CoC.player.biggestTitSize() < 3 ) {
			MainView.outputText( '  "<i>Of course we\'ll have to grow them a bit to suit our needs.</i>"' );
		}//if breasts are larger than a normal sand witch's;
		else if( CoC.player.biggestTitSize() >= 14 ) {
			MainView.outputText( '  "<i>Mmmm, and with such massive ones I\'m sure we can expect you to produce more than most of our sisters could.</i>"' );
		}
		MainView.outputText( '\n\nThe Sand Mother\'s fingers slide around your labia, teasing them, occasionally fingering your entrance and thumbing your clit.  A light sweat breaks out all over your ' + CoC.player.skinFurScales() + '.  "<i>Yes, you\'re really starting to enjoy that idea, aren\'t you?</i>"  You start to nod before regaining your senses. "<i>I see.  You still need persuasion.</i>"  She pinches your clit and an orgasm starts shivering inside your [hips].  "<i>Of course that was never in doubt.  I could never completely trust you, even if I could bring you around to our way of thinking.</i>"' );
		...
		if( CoC.player.biggestTitSize() < 5 ) {
			MainView.outputText( 'jiggle' );
		} else if( CoC.player.biggestTitSize() < 10 ) {
			MainView.outputText( 'bounce' );
		} else {
			MainView.outputText( 'quake' );
		}
		MainView.outputText( ' with every twitch of her fingers inside you.' );
		if( CoC.player.lactationQ() >= 50 ) {
			MainView.outputText( '  Streams of milk are already starting to pour down the swells of your tits.' );
		}
		MainView.outputText( '  Her fingers start pressing deeper, playing upon your erogenous zones like an instrument, strumming your body\'s sense of pleasure to new heights.  She plays your cunt until you cum again and again around palm and gush your juice down her arm.  Everything below your waist abandons your control and submits itself to her administrations.  You cum and all you can do is grope your own tits as you ride the waves of pleasure that saturate your flesh.' );
		...
		if( CoC.flags[ kFLAGS.CUM_WITCH_DEFEATED ] === 1 ) {
			MainView.outputText( '  It is obviously the Cum Witch you encountered, and she has clearly regained her strength!' );
		} else {
			MainView.outputText( 'This must be the Cum Witch the Sand Mother sent for!' );
		}
		...
		if( CoC.player.bRows() <= 2 ) {
			MainView.outputText( ', admiring the extra set of breasts you already possess' );
		} else {
			MainView.outputText( ', admiring how you\'ve managed to grow ' + Utils.num2Text( CoC.player.bRows() ) + ' rows of breasts instead of just two on your panting torso' );
		}
		MainView.outputText( '.' );
		...
		CoC.player.orgasm();
		EngineCore.menu();
		EngineCore.addButton( 0, 'Next', this, this.sandMotherBadEndsLadiesEpilogue );
	};
	//Epilogue:;
	DungeonSandWitch.prototype.sandMotherBadEndsLadiesEpilogue = function() {
		MainView.clearOutput();
		MainView.outputText( 'The Champion awoke to a body changed. ' + CoC.player.mf( 'His', 'Her' ) + ' body had been transformed in to that of a Sand Witch\'s.' );
		//if player had six breasts;
		if( CoC.player.bRows() > 2 ) {
			MainView.outputText( '  Although unlike the other Sand Witches, this one sported extra breasts which she used to her utmost advantage.' );
		}
		//if player had gigantic breasts;
		if( CoC.player.biggestTitSize() >= 22 ) {
			MainView.outputText( '  She was unrivaled in her ability to produce milk thanks to the natural enormity of her breasts that her other sisters couldn\'t come close to.  They were so large that they often impeded her movement, which she admittedly didn\'t mind so much.' );
		}
	...
	DungeonSandWitch.prototype.friendlySandMotherFuck = function() {
		...
		var y = CoC.player.cockThatFits2( CoC.monster.vaginalCapacity() );
		//First Time:;
		if( CoC.flags[ kFLAGS.TIMES_FRIENDLY_FUCKED_SAND_MOTHER ] === 0 ) {
			MainView.outputText( 'Casting your gaze around to ensure privacy, you lower your brows conspiringly and inquire into the Sand Mother\'s love life.' );
		...
		if( !CoC.player.isTaur() ) {
			MainView.outputText( 'holding [oneCock] in ' );
			if( y >= 0 ) {
				MainView.outputText( 'one' );
			} else {
				MainView.outputText( 'your' );
			}
			MainView.outputText( ' hand' );
			if( y >= 0 ) {
				MainView.outputText( ' and another in the other' );
			}
			MainView.outputText( ', the meaty cock-flesh wobbling, veins pulsing just under the surface as you nose ' );
			if( y < 0 ) {
				MainView.outputText( 'it' );
			} else {
				MainView.outputText( 'them' );
			}
			MainView.outputText( ' up against the twin, wet pussies.' );
		} else {
			MainView.outputText( 'dick' );
			if( y >= 0 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ' bobbing under your equine belly, the meaty cock-flesh wobbling with veins pulsing just under the surface as ' );
			if( y < 0 ) {
				MainView.outputText( 'it noses' );
			} else {
				MainView.outputText( 'they nose' );
			}
			MainView.outputText( ' up against the twin, wet pussies.' );
		}
		if( CoC.player.tallness >= 90 ) {
			MainView.outputText( '  You have to kneel down to make it work, tall as you are.  ' );
		} else if( CoC.player.tallness <= 48 ) {
			MainView.outputText( '  You can barely reach her cunt standing, short as you are.  ' );
		}
		MainView.outputText( 'Her lube-glossed gates spread around your ' + CoC.player.cockHead( x ) );
		if( y >= 0 ) {
			MainView.outputText( ' and other cock-tip' );
		}
		MainView.outputText( ', the hot moisture gobbling up your inches, clinging hot and wet to the oversensitive, pulsating dick' );
		if( y >= 0 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( '.  The large woman sighs with relief as you scratch her unspoken, hidden itch.  Your [hips] push forward with such fervor that you doubt you could stop if you wanted, pressing hard on her sensitive, innermost folds with ' );
		if( y >= 0 ) {
			MainView.outputText( 'both your cocks' );
		} else {
			MainView.outputText( 'your ' + Descriptors.cockDescript( x ) );
		}
		MainView.outputText( '.' );
		//HOARSES:;
		if( CoC.player.isTaur() ) {
			...
			if( CoC.player.tallness < 78 ) {
				MainView.outputText( 'her shoulders' );
			} else {
				MainView.outputText( 'the top of her throne' );
			}
			MainView.outputText( ' and thrust, pumping yourself hard into her with animalistic delight.  The Sand Mother shudders beneath your bestial bulk as her hands begin to tug at her nipples through her soaked, not-so-concealing robes.  She whimpers and begins to kiss at your human abdomen, tonguing your belly button as she\'s mounted on your bitch-breaking erection' );
			if( y >= 0 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( '.  Grabbing a lock of platinum in your fingers, you keep a firm hold on her while you let your lower half\'s inhuman ardor take control of your body.' );
		}
		//NOTAHOARSE:;
		else {
			MainView.outputText( '\n\nGroaning in delight, you rest your hands on her ' );
			if( CoC.player.tallness < 60 ) {
				MainView.outputText( 'bottom tits' );
			} else if( CoC.player.tallness < 84 ) {
				MainView.outputText( 'top tits' );
			} else {
				MainView.outputText( 'shoulders' );
			}
			MainView.outputText( ' and thrust, powerfully pumping your manhood through her lube-gilded channel' );
			if( y >= 0 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( '.  The Sand Mother shudders before you, hands tugging her nipples through her soaked, not-so-concealing robes.  She\'s whimpering needily, and ' );
			if( CoC.player.tallness < 72 ) {
				MainView.outputText( 'you kiss her to silence her, tongue pumping to the same timing that drives your hips.' );
			} else {
				MainView.outputText( 'you suckle one of the heavy, milk-laden tits to enhance her pleasure' );
			}
			MainView.outputText( '.  Her fingers roam through your ' + Descriptors.hairDescript() + ' as she allows her passion to overwhelm her.  Splattering noisily, her juices are dripping and squirting around your hard-thrusting dong' );
			if( y >= 0 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ', replaced by fresh secretions as fast as you squeeze them out of her.' );
		}
		//Both;
		...
		MainView.outputText( ' lurches into convulsions, too hot, tight, and sensuous for you to endure.  For a moment, you try your damnedest to hold out, to ride the wave of pleasure for another few minutes before you release into the agonizingly sweet puss' );
		if( y < 0 ) {
			MainView.outputText( 'y' );
		} else {
			MainView.outputText( 'ies' );
		}
		MainView.outputText( '.' );
		if( CoC.player.hasVagina() ) {
			MainView.outputText( '  Your own [vagina] quivers weakly, insides grinding against insides, squeezing every drop of juice from your ' );
			if( CoC.player.wetness() >= 4 ) {
				MainView.outputText( 'squirting ' );
			}
			MainView.outputText( 'passage.' );
		}
		if( CoC.player.lactationQ() >= 200 ) {
			MainView.outputText( '  Escaping from your [nipples], milk ' );
			if( CoC.player.lactationQ() < 200 ) {
				MainView.outputText( 'leaks' );
			} else if( CoC.player.lactationQ() < 1000 ) {
				MainView.outputText( 'drips' );
			} else {
				MainView.outputText( 'erupts' );
			}
			MainView.outputText( ', splattering the throne with a fresh, lactic offering.  If you weren\'t mindlessly jamming yourself into her cunt, you\'d wonder if she approves of your offering, but every neuron is busy sending your body to a mind-blowing climax.' );
		}
		MainView.outputText( '  You grunt as the heat inside your clenching [balls] radiates through your groin and towards [eachCock].' );
		//Newline;
		MainView.outputText( '\n\n' );
		//More than 2 dongs or one doesn't fit;
		if( CoC.player.cockTotal() > 2 || (CoC.player.cockTotal() === 2 && y < 0) ) {
			MainView.outputText( 'White love-batter fires in thick ropes onto her robes from your unbound penis' );
			if( CoC.player.cockTotal() > 3 || (CoC.player.cockTotal() === 3 && y < 0) ) {
				MainView.outputText( 'es' );
			}
			MainView.outputText( '.  One after another, the syrupy strands mix in with the dripping milk to make an alabaster drapery of sex-juice.  ' );
		}
		//light ejaculate;
		if( CoC.player.cumQ() < 250 ) {
			MainView.outputText( 'You spend your passion in numerous spurts, firing straight into the deepest depths of the Sand Mother\'s pussy.  Her slick walls squeeze you repeatedly, pulling you further inside, caressing you to encourage your orgasm to gift her with greater quantities of jism. As you continue to spurt, you realize that her hungry cunt' );
			if( y >= 0 ) {
				MainView.outputText( 's are' );
			} else {
				MainView.outputText( ' is' );
			}
			MainView.outputText( ' successful in some capacity, because you squirt until it\'s almost painful, finishing with ' );
			if( y < 0 ) {
				MainView.outputText( 'a fat white drop that\'s squeezed from your tip' );
			} else {
				MainView.outputText( 'fat white drops that are squeezed from your tips' );
			}
			MainView.outputText( '.' );
		}
		//medium ejaculate;
		...
		//high ejaculate;
			...
			if( y >= 0 ) {
				MainView.outputText( 's go' );
			} else {
				MainView.outputText( ' goes' );
			}
			MainView.outputText( ' straight into her womb' );
			if( y >= 0 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ', rounding her belly to pregnant proportions.  She moans as she\'s filled beyond capacity with cum, only getting relief when the pressure in her packed womb' );
			if( y >= 0 ) {
				MainView.outputText( 's grow' );
			} else {
				MainView.outputText( ' grows' );
			}
			MainView.outputText( ' so high that jets of spooge spray out around you.  That doesn\'t stop you - you keep forcing more thick ejaculate into her until she\'s a nonsensical, babbling mess and you\'re totally spent.' );
		}
		//END;
	...
	DungeonSandWitch.prototype.lesboMilkFilling = function() {
		...
		//CuntChange 3;
		CoC.player.cuntChange( 3, true, true, false );
		MainView.outputText( '\n\nYou ' );
		if( CoC.player.isTaur() ) {
			MainView.outputText( 'squeeze your own [nipples]' );
		} else {
			MainView.outputText( 'grab hold of her head' );
		}
		MainView.outputText( ' as you begin to pull away and thrust back, languidly fucking yourself on her gushing spout while it floods your pouty, dripping pussy and eager womb with lactic moisture.' );
		if( !CoC.player.isTaur() ) {
			MainView.outputText( '  Somehow, she\'s able to pull her hand away from your hip while still holding you aloft.  A glowing, arm-shaped silhouette remains, even as she diverts her attention elsewhere.' );
		}
		MainView.outputText( '  \'Schluck-schluck-schluck.\'  You can hear her pushing her fingers deep into her neglected womanhoods, and judging by the doubled, wet sounds of penetration, she must be jamming two fingers in her doubled cunts.' );
		...
		if( CoC.player.pregnancyIncubation === 0 ) {
			MainView.outputText( 'Your poor womb is flooding with the stuff.  It\'s getting so full that you\'re starting to feel like you should be labelled \'cream-filled\', and a tiny bump has begun to swell on your midsection in response.' );
		} else {
			MainView.outputText( 'Your poor pregnant womb is battered with the stuff, but since you\'re already full, jets of hot cream are spraying out from your [vagina] around nipple-shaped milk-injectors.' );
		}
		//CuntChange 6;
		CoC.player.cuntChange( 6, true, true, false );
		MainView.outputText( '\n\n"<i>Yesssss,</i>" you find yourself hissing, ' );
		if( CoC.player.isTaur() ) {
			MainView.outputText( 'wishing you could rub your swollen belly' );
		} else {
			MainView.outputText( 'rubbing your swollen belly' );
		}
		...
		if( CoC.player.pregnancyIncubation === 0 ) {
			MainView.outputText( 'You can feel your midsection going round, wobbling wildly.  POP!  Your belly-button has turned into an outie, accompanied by a wobbling, comforting weight inside you.  Four months... five, no - six... you give up on counting, but with every second that passes, you\'re getting bigger and bigger, visually matching any of the pregnant women you saw growing up.' );
		} else {
			MainView.outputText( 'You can feel it fountaining out of you, backwashing out to splatter the witch with wetness, her skin going glossy under the tide of white.  If you weren\'t already pregnant, you\'d probably be ballooning up with her lactic cargo, a rounded, milk-filled cream-balloon.' );
		}
	...
	DungeonSandWitch.prototype.sandWitchHistory = function() {
		MainView.clearOutput();
		MainView.outputText( 'You inquire about the history of the sand witches, and how they came to be.  The Sand Mother gives you a benign smile and offers, "<i>With pleasure.  It is so rare that I get to recount the story to one not of our order.</i>"  She utters a few spidery words that seem to ring in your ears, and a luminous, floating illusion appears before her, all floating shapeless colors.  As she begins to talk, the blobs shift into the actors in her tale, allowing you to view the story as it happens.' );
	...
	DungeonSandWitch.prototype.currentStateOfInterwebs = function() {
		MainView.clearOutput();
		MainView.outputText( 'You inquire as to the current state of the sand witches, and this coven in particular. The Queen Witch\'s brow creases in thought as she considers your request, tapping her chin and gazing into the intricate, swirling lights above.' );
		...
		if( CoC.flags[ kFLAGS.HEL_HARPY_QUEEN_DEFEATED ] === 0 ) {
			MainView.outputText( 'All that remains is to ready those still maturing and wait on our allies to finish growing their numbers.  We shall be carried into the Demon Queen\'s fortress on the wings of phoenixes, to fight as glorious a battle as has ever been fought!' );
		} else {
			MainView.outputText( 'All that remains is for us to find new aerial allies or a method of easy, sustained flight.  The phoenixes were smashed apart by fiery demons, their lair raided and destroyed by an interloper.  If only things had been different... Calais was so close.' );
		}
		MainView.outputText( '</i>"' );
		...
		if( CoC.flags[ kFLAGS.HEL_HARPY_QUEEN_DEFEATED ] > 0 ) {
			MainView.outputText( '  Of course, this is all contingent on us finding an ally that can breach the mountaintop citadel\'s walls.' );
		}
		MainView.outputText( '  Was there something else you needed to ask about?</i>"' );
		//[friendly options];
		this.sandWitchMotherFriendlyMenu();
	};
	//*Cum Witches;
	...
	DungeonSandWitch.prototype.discussCumWitches = function() {
		MainView.clearOutput();
		MainView.outputText( 'You inquire as to why they have cum witches.' );
		...
		if( CoC.player.cor + CoC.player.lib > 100 ) {
			MainView.outputText( ', though potentially fun' );
		}
		MainView.outputText( '.  With a little bit of a lecherous smile, you ask if she ever tried out for it.' );
		...
		if( CoC.flags[ kFLAGS.MORE_CUM_WITCHES ] === 1 ) {
			...
			if( CoC.flags[ kFLAGS.CUM_WITCHES_FIGHTABLE ] === 0 ) {
				MainView.outputText( '  If you asked her to send them out hunting for recruits, you could potentially fight and fuck them in the deserts sands in the future.' );
				EngineCore.addButton( 0, 'Send Out', this, this.sendOutCumWitch );
			} else {
				MainView.outputText( 'Many of them are prowling the desert sands even now, hunting for recruits.  You could wind up having to fight them if you go out.  It\'s possible she might keep them away from you, if you request it.' );
				EngineCore.addButton( 0, 'KeepThemIn', this, this.keepCumWitchesIn );
			}
		} else {
			...
			EngineCore.addButton( 0, 'Bolster Them', this, this.moreCumWitchesPlease );
		}
		//[Send Them Out] [Back];
		//[Keep Them In] [Back];
		//[Bolster Numbers] [Back];
		EngineCore.addButton( 4, 'Back', null, MainView.playerMenu );
	};
	//Send them out:;
	DungeonSandWitch.prototype.sendOutCumWitch = function() {
		MainView.clearOutput();
		MainView.outputText( 'You request she put the cum witches\' boundless virility and desire to work for the coven, letting some of them out to venture the sands in search of new recruits.  (<b>From now on, there\'s a chance you\'ll encounter cum witches in the desert.</b>)' );
	...
	DungeonSandWitch.prototype.keepCumWitchesIn = function() {
		MainView.clearOutput();
		MainView.outputText( 'You request she put a hold on sending out the cum witches.  Their aggressive, sexual recruitment methods are getting in your way, and for now, it would be best if they were out of your way.' );
	...
	DungeonSandWitch.prototype.moreCumWitchesPlease = function() {
		...
		MainView.outputText( 'You ask her if it wouldn\'t be more humane to simply create a few more cum witches, allowing them to split their duties and actually have time to serve in other ways, either as normal sisters or perhaps helping with the recruitment.' );
		...
		MainView.outputText( '\n\nSmiling serenely, the coven leader answers, "<i>No, no my dear.  [name] here had an idea.  ' + CoC.player.mf( 'He', 'She' ) + ' suggested we add to your ranks, to give you more time to help the coven in ways besides reproduction.  Perhaps through magic, or even going out to patrol and recruit newcomers.  Would you have sisters to free up some of your time, or remain the only one to fill your sisters\' wombs?</i>"' );
		...
		CoC.flags[ kFLAGS.MORE_CUM_WITCHES ] = 1;
		this.sandWitchMotherFriendlyMenu();
	...
	DungeonSandWitch.prototype.askHowSandMothersAreChosen = function() {
		MainView.clearOutput();
		MainView.outputText( 'You ask the Sand Mother how her people choose who will be a Sand Mother of a new coven.' );
		...
		if( CoC.player.race() === 'human' ) {
			MainView.outputText( '\'re a human' );
		} else {
			MainView.outputText( ' were a human' );
		}
		MainView.outputText( '.  Our people are notorious for forming gangs and cliques, groups focused around a leader\'s charisma.  It\'s only natural that has continued in Mareth.  It would not surprise me to learn that even the demons have their own factions and political malcontents, though I imagine they deal with them far more harshly than we.</i>"' );
	...
	DungeonSandWitch.prototype.leaveAloneSendLackeysToggle = function() {
		...
		//Leave Alone;
		if( CoC.flags[ kFLAGS.SAND_WITCH_LEAVE_ME_ALONE ] === 0 ) {
			MainView.outputText( 'You request that the scouts in the desert stop bothering you with their barbed offers of magic.  Fighting them is not something you want to have to do.  It\'s a waste of effort on both sides you that feel should come to an end.' );
			...
			CoC.flags[ kFLAGS.SAND_WITCH_LEAVE_ME_ALONE ] = 1;
		}
		//Send Lackeys;
		else {
			MainView.outputText( 'You mention that you wouldn\'t mind the witches trying to recruit you as they used to.' );
	...
	DungeonSandWitch.prototype.getLactaidFromWitches = function() {
		...
		MainView.outputText( 'You ' );
		if( CoC.player.cor < 33 ) {
			MainView.outputText( 'politely request' );
		} else if( CoC.player.cor < 66 ) {
			MainView.outputText( 'request' );
		} else {
			MainView.outputText( 'none-too-politely request' );
		}
		MainView.outputText( ' a dose of Lactaid from her coven.' );
	...
	DungeonSandWitch.prototype.getLaBova = function() {
		MainView.clearOutput();
		MainView.outputText( 'You ask for some La Bova' );
		if( CoC.player.cor < 33 ) {
			MainView.outputText( ' with a blush' );
		} else if( CoC.player.cor < 66 ) {
			MainView.outputText( ' a little uncertainly' );
		} else {
			MainView.outputText( ' with a knowing smile' );
		}
		MainView.outputText( '.' );
		...
		MainView.outputText( '\n\nYou shake your head' );
		if( CoC.player.cor > 66 ) {
			MainView.outputText( ', though the idea does seem to nestle into your imagination quite alluringly' );
		}
		MainView.outputText( '.' );
	...
	DungeonSandWitch.prototype.unfriendlyWitchToggle = function() {
		...
		if( CoC.flags[ kFLAGS.SAND_WITCH_LEAVE_ME_ALONE ] === 0 ) {
			...
			if( CoC.player.cor < 50 ) {
				MainView.outputText( '  You probably wouldn\'t do something like that, at least not without provocation!/  You could see how she would think that.' );
			} else {
				MainView.outputText( '  As if that would stop you.' );
			}
			MainView.outputText( '  Slapping her on the shoulder, you congratulate her on seeing good sense.  Her eyes narrow dangerously as she twists away from your touch.' );
	...
	DungeonSandWitch.prototype.pullLever = function() {
		MainView.clearOutput();
		MainView.outputText( 'There is a loud rumbling from the direction of the cavernous commons...' );
	...
	DungeonSandWitch.prototype.takeFertilePills = function() {
		...
		if( CoC.player.findStatusAffect( StatusAffects.Contraceptives ) < 0 ) {
			...
			MainView.outputText( 'It doesn\'t take you long to figure out that the pink pill should cancel the effects of your contraceptives.  You pop it into your mouth and swallow, feeling a tingle near your crotch after a moment.  You should be capable of bearing children again' );
			if( !CoC.player.hasVagina() ) {
				MainView.outputText( ', should you ever grow a vagina' );
			}
			MainView.outputText( '.' );
	...
	DungeonSandWitch.prototype.takeBarrenPills = function() {
		...
		if( CoC.player.findStatusAffect( StatusAffects.Contraceptives ) >= 0 ) {
			...
			MainView.outputText( 'You figure one of these brown pills should render you barren, and you pop it into your mouth, not wanting to be impregnated.' );
			if( CoC.player.pregnancyIncubation > 0 ) {
				MainView.outputText( '  Of course, you\'re already pregnant, and this doesn\'t seem to be doing anything about THAT.' );
			}
			MainView.outputText( '  You do feel an emptiness in your midsection, reassuring you that the pill did its job.' );
			if( !CoC.player.hasVagina() ) {
				MainView.outputText( '  Now if you ever re-grow a vagina, you should be fine.' );
			}
	...
	DungeonSandWitch.prototype.friendlyCumWitchBlessing = function() {
		MainView.clearOutput();
		MainView.outputText( 'You ask the Cum Witch if she could use her magic to gift you with some kind of blessing since she has such an affinity for sexualized magics and fertility.  ' );
		if( CoC.player.cor < 33 ) {
			MainView.outputText( 'Blushing, ' );
		} else if( CoC.player.cor < 66 ) {
			MainView.outputText( 'Nervously, ' );
		} else {
			MainView.outputText( 'Boldly, ' );
		}
		MainView.outputText( 'you remove your armor, figuring whatever blessing she\'s going to give is going to be at least a little bit sexual.' );
		...
		EngineCore.menu();
		if( CoC.player.hasCock() ) {
			EngineCore.addButton( 0, 'Virility', this, this.cumWitchBlessed, true );
		}
		if( CoC.player.hasVagina() ) {
			EngineCore.addButton( 1, 'Fertility', this, this.cumWitchBlessed, false );
		}
		EngineCore.addButton( 4, 'Nevermind', this, this.changeMindAboutBlessings );
	};
	//*Nevermind ✓Kirbu;
	DungeonSandWitch.prototype.changeMindAboutBlessings = function() {
		MainView.clearOutput();
		MainView.outputText( 'Thinking better of it, you grab your [armor] and get dressed, telling the jizz-obsessed enchantress that you don\'t need her gifts for now.' );
	...
	DungeonSandWitch.prototype.cumWitchBlessed = function( virility ) {
		...
		if( CoC.player.tongueType > AppearanceDefs.TONUGE_HUMAN ) {
			MainView.outputText( '  Your inhuman tongue slides further and further out, following the witch\'s bulging, pre-cum filled urethra towards her balls where it can curl around them, embracing them with slick, slippery saliva.  You can taste her unique flavor on her skin, and it makes you shudder with arousal.' );
		}
		...
		if( virility ) {
			MainView.outputText( '[eachCock] drip and drool your own seed onto the floor.  The heat inside you is so hot that you can\'t keep your ardor penned up, and it\'s slowly boiling out of your [balls] in a pleasure-filled orgasm that provides everything but relief.  You\'re sure that touching yourself would give you a proper orgasm, but you keep your hands where they belong - on her balls.' );
		} else {
			MainView.outputText( 'your [vagina] ' );
			if( CoC.player.wetness() < 3 ) {
				MainView.outputText( 'drip' );
			} else if( CoC.player.wetness() < 4 ) {
				MainView.outputText( 'drool' );
			} else if( CoC.player.wetness() < 5 ) {
				MainView.outputText( 'leak' );
			} else {
				MainView.outputText( 'spray' );
			}
			MainView.outputText( ' your juicy secretions onto the ground below.  Your lust has risen to such unbearable levels that you\'re clenching and dripping in orgasmic delight without any release.  You\'re sure just a touch on your clit would have you cumming your brains out, but you keep your hands on her balls.' );
		}
		...
		MainView.outputText( ' that reminds you that you got exactly you asked for, even if she made you like it a little bit more than you would have liked...' );
		MainView.outputText( '<b>' );
		if( virility ) {
			MainView.outputText( '\n\n(Perk Unlocked: Magical Virility - 200 mLs more cum per orgasm and enhanced virility.)' );
			CoC.player.createPerk( PerkLib.MagicalVirility, 0, 0, 0, 0 );
		} else {
			MainView.outputText( '\n\n(Perk Unlocked: Magical Fertility - 10% higher chance of pregnancy and increased pregnancy speed.)' );
			CoC.player.createPerk( PerkLib.MagicalFertility, 0, 0, 0, 0 );
		}
		CoC.flags[ kFLAGS.BEEN_BLESSED_BY_CUM_WITCH ] = 1;
		CoC.player.orgasm();
	...
	DungeonSandWitch.prototype.knockUpSomeDoubleStuffedSandWitches = function() {
		...
		if( CoC.player.hasCock() && CoC.player.cockThatFits( 50 ) >= 0 && CoC.player.lust >= 33 ) {
			EngineCore.addButton( 0, 'Fuck One', this, this.fuckOneSandWitch );
		}
		//[TWO AT ONCE] *Requires at least two Cocks of appropriate size and enough lust.;
		if( CoC.player.cockThatFits( 50 ) >= 0 && CoC.player.cockThatFits2( 50 ) >= 0 && CoC.player.lust >= 33 ) {
			EngineCore.addButton( 1, 'Fuck Two', this, this.fuckTwoPregWitches );
		}
		//[FUCK EM ALL] *Requires at least one Cock of appropriate size and enough lust.;
		if( CoC.player.cockThatFits( 50 ) >= 0 && CoC.player.lust >= 33 ) {
			EngineCore.addButton( 2, 'Fuck\'EmAll', this, this.fuckAllThePregWitches );
		}
	...
	DungeonSandWitch.prototype.fuckOneSandWitch = function() {
		...
		if( CoC.player.tentacleCocks() === 0 ) {
			MainView.outputText( 'You grab the tanned shoulders of the closest pregnant witch and flip her over on to her back, trapping her beneath the weight of her belly and breasts while you grab both of her ankles.  Her dirty blonde, sweat-soaked hair lies in waves around her head on the floor as she pants heavily in sexual desperation.' );
		}//{else if player has a tentacle cock};
		else {
			MainView.outputText( 'Your tentacle cock lashes outwards, extending as it goes, and wraps around the ankle of the closest girl. It flips her over on to her back, trapping her own torso beneath the weight of her heavy pregnancy and her own four massive leaking breasts.' );
		}
		//{if player has two tentacle cocks};
		if( CoC.player.tentacleCocks() > 1 ) {
			MainView.outputText( '  Another tentacle cock reaches out for the girl\'s other ankle, and together your cocks spread her tanned legs wide, revealing her large double pussy bloated from her pregnancy.' );
		} else {
			MainView.outputText( '  You reach forward with a hand to grab her other ankle and with your cock and arm you spread her tanned legs wide to reveal her large double pussy, bloated from her pregnancy.' );
		}
		...
		if( CoC.player.tentacleCocks() >= 2 && CoC.player.cockTotal() >= 3 && CoC.player.cocks[ x ].cockType !== CockTypesEnum.TENTACLE ) {
		...
		else if( CoC.player.tentacleCocks() >= 2 && CoC.player.cocks[ x ].cockType !== CockTypesEnum.TENTACLE && CoC.player.cockThatFits2( 50 ) >= 0 && CoC.player.cockTotal() >= 4 ) {
		...
		MainView.outputText( '  The witch\'s moans and grunts of pleasure arouse the other witches in the room as they watch with the utmost jealously.  Some grab their nipples while others try to finger their pussies; whining and getting off on their envious voyeurism as they watch you fuck their sister.' );
		//{all} ;
		...
		if( multi ) {
			MainView.outputText( 's constantly pound' );
		} else {
			MainView.outputText( ' constantly pounds' );
		}
		MainView.outputText( ' into her nethers. Drool escapes her lips and drips down her cheek even as her eyes roll back in her head. Talk about an easy lay.' );
		//{if player has large but not extremely massive breasts} ;
		if( CoC.player.biggestTitSize() >= 4 && CoC.player.biggestTitSize() < 15 ) {
			...
			if( CoC.player.pregnancyIncubation > 0 && CoC.player.pregnancyIncubation < 200 ) {
				MainView.outputText( '  Each time it does so your own breasts slap against your swollen midsection.' );
			}
			//{if player has four or more large breasts and is not pregnant};
			else if( CoC.player.bRows() >= 2 ) {
				MainView.outputText( '  Your lower pair of breasts slap against the pregnant girl\'s belly with each thrust.' );
			}
		}
		//else if player has extremely huge breasts;
			...
			if( CoC.player.bRows() === 1 ) {
				MainView.outputText( '  Your two heavy milk sacs sway and at times you must rest them on top of the poor witch\'s belly to keep from prematurely exhausting yourself.' );
			} else if( CoC.player.bRows() >= 2 ) {
				MainView.outputText( '  Resting on top of each other and on top of the witch\'s legs, your giant globes of tit flesh quake each time you hips move.' );
			}
			MainView.outputText( '  The witches around the room can\'t help but gawk at how much more endowed your mammaries are than even their own, and how the wrong forwards or backwards movement could result in either you or the witch you\'re fucking to be suddenly smothered under the weight of your ridiculously engorged tits.' );
		}
		MainView.outputText( '\n\nIt isn\'t long before the sensations of naked wet flesh sliding along your ' );
		if( multi ) {
			MainView.outputText( 'shafts' );
		} else {
			MainView.outputText( 'shaft' );
		}
		MainView.outputText( ' forces you to reach your own orgasm.' );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( '  Your [balls] tense with the anticipation of releasing the fruit of your loins into her waiting womb.' );
		}
		MainView.outputText( '  With both hands you reach up and grab the fat nipples of the witch\'s lower breasts, tugging on them like reins as you try to thrust deeper.  "<i>Ung! Ah!</i>" The witch bites her lower lip in pleasure and pain as you roughly handle her leaking teats.' );
		//{if player has huge or massive breasts} ;
		if( CoC.player.biggestTitSize() >= 15 ) {
			MainView.outputText( '  The weight of your own tits bears heavily against your arms as you try to keep the motion of your hips constant while pulling on your lover\'s milk knobs.' );
		}//{if player's tits can lactate};
		else {
			MainView.outputText( '  Each of your [nipples] begin spraying milk over the witch as your body begins its crescendo.' );
		}
		...
		MainView.outputText( '.  The girl squeals as she feels you thicken within her, on the verge of releasing your torrent.  Her hands claw at the floor as if seeking any foundation to ground her senses, but it is to no avail as your bounty of cum explodes within her.' );
		...
		if( CoC.player.hasVagina() ) {
			...
			if( CoC.player.wetness() >= 4 ) {
				MainView.outputText( '  Gobs of pussy juice splash against yours and the witch\'s legs as your orgasm forces your cunt to squirt over and over again.' );
			}
			if( CoC.player.balls > 0 ) {
				MainView.outputText( '  Your [sack] is soaking wet by the time your pussy finishes cumming.' );
			}
		}
	...
	DungeonSandWitch.prototype.fuckTwoPregWitches = function() {
		...
		MainView.outputText( 'These hormonally overwrought witches are each little more than five huge fleshy orbs of tits and bellies, desperately crawling towards you on hands and knees as their stares fixate on your ' + Descriptors.cockDescript( y ) + '.  ' );
		//{if player has two long tentacle cocks} ;
		if( CoC.player.tentacleCocks() >= 2 ) {
			MainView.outputText( 'Almost of their own accord your tentacle cocks lash out, wrapping around the arms of the two closest girls, lifting them to their knees only to roll them backwards and on to their sides with their backs to each other.  ' );
		} else {
			MainView.outputText( 'It takes little effort to grab the closest two witches and pull them up only to push them down again and on to their sides with their backs pressed against each other.  ' );
		}
		MainView.outputText( 'The two witches lay before you with their heaving breasts and bellies jutting out to either side while their asses are mashed against each other.  You grab an ankle from each of them and raise their legs up to expose the dual pairs of pussies that they possess.' );
		//{for non-serpent lower body};
		if( !CoC.player.isNaga() ) {
			MainView.outputText( 'You straddle ' );
		} else {
			MainView.outputText( 'You entwine your serpentine tail around ' );
		}
		MainView.outputText( 'their two legs that remain on the floor while allowing their raised legs to ' );
		if( CoC.player.biggestTitSize() >= 15 ) {
			MainView.outputText( 'slide between your cleavage' );
		}//{for small or non-breasted characters};
		else {
			MainView.outputText( 'rest against your torso and shoulders' );
		}
		MainView.outputText( '.' );
		if( CoC.player.pregnancyIncubation > 0 && CoC.player.pregnancyIncubation < 200 ) {
			MainView.outputText( '  Each of these raised legs carefully bend at the knee to accomodate your own pregnant belly that looms over their prone figures.' );
		}
		...
		if( CoC.player.cocks[ x ].cockLength < 20 && CoC.player.cocks[ y ].cockLength < 20 ) {
		...
		//{if player lactates};
		if( CoC.player.lactationQ() >= 200 ) {
			MainView.outputText( '  Your own breasts begin spraying down the two pregnant sluts with your own dairy fluids.' );
		}
		//{if player has large but not huge breasts} ;
		if( CoC.player.biggestTitSize() >= 5 && CoC.player.biggestTitSize() < 20 ) {
			MainView.outputText( '  Each thrust in to the eager cunts causes your breasts to bounce up and down.' );
		} else if( CoC.player.biggestTitSize() >= 20 ) {
			MainView.outputText( '  Each thrust in to the eager cunts causes your massive breasts to sway back and forth, slapping against your own sides.' );
		}
		//{if player has six huge breasts};
		if( CoC.player.bRows() >= 3 ) {
			if( CoC.player.breastRows[ 2 ].breastRating >= 20 ) {
				MainView.outputText( '  Your lower beach ball sized breasts are large enough to rest on the floor to either side as you fuck, and both witches eventually lower the legs they\'ve had raised so as to hook them around your massive mammaries and hug your flesh even closer to their thighs.' );
			}
		}
		if( CoC.player.balls > 0 ) {
			MainView.outputText( '  The closer your shafts come to their climax the more tense your balls become, preparing to pump their loads through your cocks.' );
		}
		...
		if( CoC.player.cumQ() < 500 ) {
			MainView.outputText( '  The pair of girls yelp as they feel the hot seed splash forth inside them.  Millions of sperm swim in to their fertile wombs, seeking to turn them into doubly pregnant sluts.' );
		}//{else if cumHigh};
		else if( CoC.player.cumQ() < 1000 ) {
			MainView.outputText( '  The slutty pair of milk bags cry out in sensual surprise as they feel extraordinary amounts of cum pumping directly into their baby makers, knocking them up with the double pregnancy they\'ve desperately wanted. Their bellies bulge with all of the cum you\'ve deposited within them.' );
		}//{else if cumVeryHigh} ;
		else if( CoC.player.cumQ() < 3000 ) {
			MainView.outputText( '  The two piles of tits and bellies scream in pleasured insanity as their empty wombs are assaulted with a surging torrent of jizz.  You can feel the heavy blasts of cum surging within your shafts as they flood forth in to slutty orifices that have swallowed your manhoods.  Your brain burns with ecstasy as your genitals pump each womb over and over again, forcing them to swell far beyond what they\'re intended to take from cum alone.  By the time your cocks feel satisfied each witch looks as if she\'s already full term with a second child, the cum filled womb creating a distinct second orb next to the first pregnancy.' );
		}//{else if cumExtreme};
		else {
			MainView.outputText( '  The eyes of every witch in the room open wide in shock as they watch their sister\'s bellies swell obscenely.  Your cocks feel as if they\'re trying to wrest control of your body from you as liters of hot sperm pump forcefully directly in to the extra fertile wombs of each slutty witch.  The two quad-breasted whores convulse in brain damaging pleasure, causing their huge milk filled tits to quake and bounce everywhere. A white flood of ejaculate erupts from their pussies around your cocks as their overfilled wombs fail to contain your bounty of baby batter.' );
		}
		//{if player has 4+ cocks ie; did anal};
		if( CoC.player.cockTotal() >= 4 ) {
			MainView.outputText( '  Streams of cum drip from their abused anuses as you finish them off.' );
		}
	...
	DungeonSandWitch.prototype.fuckAllThePregWitches = function() {
		...
		if( CoC.player.cockTotal() === 1 || CoC.player.cockThatFits2( 50 ) < 0 ) {
		...
		else if( CoC.player.cockThatFits2( 50 ) >= 0 && CoC.player.cockTotal() < 3 ) {
		...
		MainView.outputText( ' thoroughly swallowed up by the juicy confines of the witch' );
		if( multi ) {
			MainView.outputText( 'es' );
		}
		MainView.outputText( ', you begin rocking your hips to and fro, sliding yourself in and out of ' );
		if( !multi ) {
			MainView.outputText( 'her' );
		} else {
			MainView.outputText( 'their' );
		}
		MainView.outputText( ' velvety wet depths.  ' );
		if( !multi ) {
			MainView.outputText( 'Her' );
		} else {
			MainView.outputText( 'Their' );
		}
		MainView.outputText( ' blissful cries of pleasure fill the room, joined with a chorus of slurping sounds as you thrust your sensitive shafts ever deeper and harder.  ' );
		if( !multi ) {
			MainView.outputText( 'Her' );
		} else {
			MainView.outputText( 'Their' );
		}
		MainView.outputText( ' quadruple breasts jiggle and bounce along the floor and ' );
		if( !multi ) {
			MainView.outputText( 'her' );
		} else {
			MainView.outputText( 'their' );
		...
		if( !multi ) {
			MainView.outputText( 'belly sways' );
		} else {
			MainView.outputText( 'bellies sway' );
		}
		MainView.outputText( ' with each ramming motion of your cock' );
		if( multi ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( '.  The muscles in ' );
		if( !multi ) {
			MainView.outputText( 'her' );
		} else {
			MainView.outputText( 'their' );
		}
		MainView.outputText( ' thighs and calves tense with pleasure as you push ' );
		if( !multi ) {
			MainView.outputText( 'her body' );
		} else {
			MainView.outputText( 'their bodies' );
		}
		MainView.outputText( ' to the limit.' );
		//{if player has non-huge tits};
		if( CoC.player.biggestTitSize() > 3 && CoC.player.biggestTitSize() < 10 ) {
			MainView.outputText( '  Your tits bounce as you enthusiastically grind away.' );
		}//{else if player has huge tits};
		else if( CoC.player.biggestTitSize() >= 10 && CoC.player.biggestTitSize() < 20 ) {
			MainView.outputText( '  The heavy weight of your huge tits causes them to sway with every thrust of your hips.' );
		}//{if player has really huge tits} ;
		else if( CoC.player.biggestTitSize() >= 20 ) {
			MainView.outputText( '  The immense swells of your mammaries press down upon the ass of the girl in front of you.' );
		}
		//{if player has four non-huge tits} ;
		if( CoC.player.bRows() >= 2 ) {
			if( CoC.player.breastRows[ 1 ].breastRating >= 3 ) {
				MainView.outputText( '  Your second row of breasts slap against the ass cheeks of the girl in front of you.' );
			}
		}
		//{if player has six huge but not really huge tits} ;
		if( CoC.player.bRows() >= 3 ) {
			if( CoC.player.breastRows[ 2 ].breastRating >= 3 && CoC.player.breastRows[ 2 ].breastRating < 20 ) {
				MainView.outputText( '  Your lowermost row of breasts become squashed against the witch' );
				if( multi ) {
					MainView.outputText( 's' );
				}
				MainView.outputText( '\'s muscular thighs.' );
			} else if( CoC.player.breastRows[ 2 ].breastRating >= 20 ) {
				MainView.outputText( '  Your lowermost row of breasts come to rest on the floor as you lean forwards to fuck, and the witch' );
				if( multi ) {
					MainView.outputText( 'es' );
				}
				MainView.outputText( ' can almost support ' );
				if( multi ) {
					MainView.outputText( 'their' );
				} else {
					MainView.outputText( 'her' );
				}
				MainView.outputText( ' thighs against your obscene mammaries when they lean back against the constant ramming of your cock.' );
			}
		}
		//{if player lactates};
		if( CoC.player.lactationQ() >= 200 ) {
			MainView.outputText( '  After only a short while of mindless rutting your tits begin leaking, and in short order your glistening nipples start spraying your milk over ' );
			if( !multi ) {
				MainView.outputText( 'the witch\'s tanned backside' );
			} else {
				MainView.outputText( 'the collective tanned bodies you\'re impaling' );
			}
			MainView.outputText( '.  A few other witches in the room eagerly step forth to suckle your [nipples] as they await their turn for impregnating.' );
		}
		MainView.outputText( '\n\nThe cum inside your loins starts to move, filling the base of your shaft' );
		if( multi ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ' as you feel your orgasm creep up inside you.  The sucking sensations of the ' );
		if( !multi ) {
			MainView.outputText( 'girl\'s cunt on your cock' );
		} else {
			MainView.outputText( 'the girls\' cunts on your cocks' );
		}
		MainView.outputText( ' starts to become overwhelming, and before you realize it you\'re on the verge of releasing your seed.  The ' );
		if( !multi ) {
			MainView.outputText( 'pussy' );
		} else {
			MainView.outputText( 'pussies' );
		}
		MainView.outputText( ' you\'re pounding seem' );
		if( !multi ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ' to sense the imminent release of the sperm ' );
		if( !multi ) {
			MainView.outputText( 'it craves' );
		} else {
			MainView.outputText( 'they crave' );
		}
		MainView.outputText( ' and strong contractions begin sucking on your shaft' );
		if( multi ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ' even more fervently.  You can\'t help but give ' );
		if( !multi ) {
			MainView.outputText( 'it the load of baby batter it craves' );
		} else {
			MainView.outputText( 'them the loads of baby batter they crave' );
		}
		MainView.outputText( '.' );
		MainView.outputText( '\n\nThe ' );
		if( !multi ) {
			MainView.outputText( 'underside of your shaft swells' );
		} else {
			MainView.outputText( 'undersides of your shafts swell' );
		}
		MainView.outputText( ' as your bounty of man-seed floods your full length and stretches the ' );
		if( !multi ) {
			MainView.outputText( 'witch\'s cunt' );
		} else {
			MainView.outputText( 'witches\' cunts' );
		}
		MainView.outputText( ' even wider.  Pressing as far forwards as possible you align the ' );
		if( !multi ) {
			MainView.outputText( 'tip of your tool against her abused cervix' );
		} else {
			MainView.outputText( 'tips of your tools against their weakened cervixes' );
		}
		MainView.outputText( ' just as your cum begins erupting out of you.' );
		//{if cumNormal-cumHigh};
		if( CoC.player.cumQ() < 1000 ) {
			MainView.outputText( '\n\nYou plaster the insides of ' );
			if( !multi ) {
				MainView.outputText( 'her pussy' );
			} else {
				MainView.outputText( 'their pussies' );
			}
			MainView.outputText( ' with no small amount squirting directly in to ' );
			if( !multi ) {
				MainView.outputText( 'her womb' );
			} else {
				MainView.outputText( 'their wombs' );
			}
			MainView.outputText( '.  When you finally pull out ' );
			if( !multi ) {
				MainView.outputText( 'a stream' );
			} else {
				MainView.outputText( 'streams' );
			}
			MainView.outputText( ' of cum are left behind.' );
		}
		//{else if cumVeryHigh-cumExtreme]};
		else {
			MainView.outputText( '\n\nThe horny ' );
			if( !multi ) {
				MainView.outputText( 'witch isn\'t' );
			} else {
				MainView.outputText( 'witches aren\'t' );
			}
			MainView.outputText( ' prepared for the deluge that explodes from your genitals.  Liter after liter pours forth, filling up ' );
			if( !multi ) {
				MainView.outputText( 'her empty womb like a balloon' );
			} else {
				MainView.outputText( 'their empty wombs like a bunch of balloons' );
			}
			MainView.outputText( ' and causing your eyes to roll back in your head from the sheer pleasured sensation of emptying yourself.  When ' );
			if( !multi ) {
				MainView.outputText( 'her belly' );
			} else {
				MainView.outputText( 'their bellies' );
			}
			MainView.outputText( ' swell as large as the neighboring pregnancies and stretch marks start appearing up and down each side you finally pull out and allow your cock' );
			if( multi ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ' to finish spraying ' );
			if( !multi ) {
				MainView.outputText( 'its' );
			} else {
				MainView.outputText( 'their' );
			}
			MainView.outputText( ' final loads over the ' );
			if( !multi ) {
				MainView.outputText( 'whorish witch\'s back' );
			} else {
				MainView.outputText( 'collective grouping of the witches\' asses' );
			}
			MainView.outputText( '.' );
		}
		//{all};
	...
	DungeonSandWitch.prototype.milkBathsAhoy = function() {
		...
		MainView.outputText( '  You could probably masturbate in the tub if you wanted to, or maybe pull the dusky milk-maid in for company....  What do you do?' );
		//{If can masturbate [Drink & Masturbate] [Milk Girl] [Relax]};
		//{otherwise [next] to relaxing};
		EngineCore.fatigue( -10 );
		EngineCore.menu();
		EngineCore.addButton( 0, 'Next', this, this.finishMilkBath );
		if( CoC.player.gender > 0 ) {
			EngineCore.addButton( 1, 'DrinkNFap', this, this.drinkNFap );
		}
		EngineCore.addButton( 2, 'Milk Girl', this, this.grabTheMilkGirl );
	};
	//[Next] (Relax);
	DungeonSandWitch.prototype.finishMilkBath = function() {
		MainView.clearOutput();
		MainView.outputText( 'You sit in the tub for a while, letting the fragrant fluids soak into your ' + CoC.player.skinFurScales() + '.  Yet, you have work to do, and eventually, you tire of relaxing in the sand witches\' endless white bounty.  You pull out the tub\'s plug and climb out, finding a towel on the wall.  Thankfully, the milk doesn\'t seem to leave behind any residue, and you feel clean and refreshed, if a bit horny.' );
		//(+Lust, -Fatigue);
	...
	DungeonSandWitch.prototype.grabTheMilkGirl = function() {
		MainView.clearOutput();
		MainView.outputText( 'You call out to the milk slave before she can slink away, and wade over to the side of the tub, leaving your face a few scant inches from her massive, milk-laden jugs. She cocks her head to the side, obviously unused to the attention, and quietly whispers, "<i>' + CoC.player.mf( 'M-master?', 'M-mistress?' ) + '</i>"' );
		...
		if( CoC.player.biggestTitSize() >= 8 ) {
			MainView.outputText( ', pressing deep into your own huge rack, your nipples flicking across her own leaky tits' );
		}
		MainView.outputText( '.' );
		...
		if( CoC.player.gender === 1 ) {
			...
			MainView.outputText( '[cock] and [vagina]' );
		} else {
			MainView.outputText( 'sexless crotch, still burning hotly with your desires' );
		}
		MainView.outputText( '.  After a time, though, you give the girl a gentle little push, having her rest her arms and tits on the edge of the pool.  You shift around behind her, cupping up handfuls of her milk.  You start to pour it down her back and shoulders, getting her nice and soaked in her own sweet cream before you close in, starting to massage her back, rubbing it in nice and slow until she\'s shivering quietly.  She moans under her breath as your fingers sink into her soft, yielding flesh, gently kneading her shoulders and hips, giving special attention to her full, round ass, tentatively slipping a few fingers around her leg to caress along her slick vulva and the bud of her clit.' );
		MainView.outputText( '\n\nShe tenses when you brush against her, ' );
		if( CoC.player.cor < 70 ) {
			MainView.outputText( 'and you\'re quick to pull back, not wanting to force her, but to your surprise and delight, she reaches back and takes your hand in hers, moving you back to finger her' );
		}
		// if Corrupt> 70:;
		else {
			MainView.outputText( 'and grinning with lusty fervor, you push harder, slipping your fingers into her with ease, her milk providing the perfect lubricant to penetrate her.  The slave girl trembles at your sexual advance, but either does not want to stop you out of well-trained fear, or just doesn\'t want you to stop' );
		}
		MainView.outputText( '.  Before you can get too far, though, the slave girl turns on a heel, her huge rack pushing you back through the milky pool and then against the rim.  You\'re dazed for only a brief second before her breasts press firmly into your back, pressed so hard that a new streak of milk pours from her teats, wetting your back much as you did hers.  You relax against the rim as the slave cups up handfuls of milk, rubbing it into your own hair and shoulders, deft fingers massaging every muscle in your back with the skill of the greatest masseuses, and you can feel the tension bleeding from your muscles. You yawn powerfully, resting your chin on your arms and letting the milky girl massage you, coating your ' + CoC.player.skinFurScales() + ' in her rich, delicious milk.' );
		//{If PC has a dick: ;
		if( CoC.player.hasCock() ) {
			...
			if( CoC.player.balls > 0 ) {
				MainView.outputText( ', her other hand cupping your [balls], rolling the ' + Utils.num2Text( CoC.player.balls ) + ' orbs in her palm with surprising dexterity' );
			}
			MainView.outputText( '.  Leaning over the two titanic teats between you, she traces a line of kisses down your back, licking up stray drops of milk between affectionate caresses.' );
		}
		//{If PC has cooch:;
		if( CoC.player.hasVagina() ) {
			...
			if( CoC.player.biggestTitSize() >= 1 ) {
				MainView.outputText( '  She cups your breasts, having to reach so far around both your rack and hers that she\'s straining her arms to rub your [nipples], but she does so valiantly, stroking them with her incredibly deft fingers.' );
			}
			if( CoC.player.lactationQ() >= 200 ) {
				MainView.outputText( '  A spurt of milk escapes your own full jugs, joining the pool-full of your new friend\'s.  She gasps with surprise and delight, quickly nuzzling herself into your back and going to work, milking you just as she would herself, letting the hefty flow of your motherly fluids pour into the pool, odd trickles smearing down your chest, staining your chest as white as her own.' );
			}
		}
	...
	DungeonSandWitch.prototype.ladyFucks = function() {
		MainView.clearOutput();
		MainView.outputText( 'You turn around in the milky pool, pulling the cute little slave tight against you.  She gasps with surprise, but settles as soon as you press your lips to hers, your hands wandering across her huge teats and supple, milky body.  She nuzzles up against you, her head resting on your [chest] as you hold her against yourself, stroking her dark hair.  After a few moments of such a simple pleasure, the little slave girl shifts her cheek along your breast, wrapping her full, dusky lips around your [nipple].  You let out a long moan as she suckles gently' );
		if( CoC.player.lactationQ() >= 200 ) {
			MainView.outputText( ', drawing out a trickle of milk from your motherly reserves.  She gulps deeply, smiling up at you as a trickle of your milk runs down her chin, dripping into the pool of her own' );
		}
		MainView.outputText( '.  Her hand slips up your body, brushing your vulva and [clit] before cupping your other breast, delicate fingers wrapping around your [nipple].  With deft, practiced motion, she works your teats between her fingers, working your breast like she might her own' );
		if( CoC.player.lactationQ() >= 200 ) {
			MainView.outputText( ', milking you with skill beyond anything you\'ve ever experienced before; and why not, when her entire existence revolves around that self-same skill?' );
		} else {
			MainView.outputText( '.' );
		}
		...
		if( CoC.player.lactationQ() >= 200 ) {
			MainView.outputText( ', spraying milk all over yourself and the milkmaid who caused your explosive pleasure' );
		}
		MainView.outputText( '.  You run your fingers through the girl\'s hair, urging her sexual skills on as your chest heaves and quavers, riding out the massive boobgasm as fem-cum spurts from your cunt and into the milky pool below.' );
		MainView.outputText( '\n\nYour entire body feels deeply refreshed, her milk having soaked into your body and making you feel fresh and revitalized, and every muscle seems to have relaxed thanks to your blissful coitus.  You start to thank the milk girl for the pleasurable company, but when you open your mouth, she presses her lips to yours for a long, tongue-filled kiss.  Chuckling to yourself, you hold the girl as tightly as her udders will allow, turning her to the side to let her nuzzle her cheek into your [chest], kissing the top of her head before the two of you climb from the pool.  You have to help her out, her massive extra weight nearly dragging her back in except for your quick reflexes.  You gather your [armor] and ruffle the milk slave\'s hair before turning back to the task at hand.' );
		//[+Lust, +HP, -Fatigue];
		CoC.player.orgasm();
		EngineCore.fatigue( -15 );
	...
	DungeonSandWitch.prototype.drinkNFap = function() {
		MainView.clearOutput();
		MainView.outputText( '"<i>Wait,</i>" you call out to the ebony woman, letting the milk obscure your hands as you begin to masturbate, "<i>I want a drink.</i>"  Sheepishly, the milk slave obliging shifts to drag her tits back into place.' );
		...
		if( CoC.player.hasVagina() ) {
			MainView.outputText( '[vagina] and idly stroke your puffy, lust-engorged vulva' );
		} else {
			MainView.outputText( '[cock] and idly stroke the turgid mass' );
		}
		MainView.outputText( ', inadvertently drawing a lewd moan from yourself.  The fat nipple stuffed in your mouth does an adequate job of muffling your pleasurable vocalizations' );
		if( CoC.player.hasCock() ) {
			MainView.outputText( ', but it does little to hide the swelling of [eachCock] - you have the milk for that' );
		}
		MainView.outputText( '.' );
		...
		//(DA HERMS) ;
		if( CoC.player.gender === 3 ) {
			MainView.outputText( '  Your [cock] throbs painfully in your hand, so hot and hard that you\'re sure you must have begun to leak precum, but any fluid is swiftly washed away by the ever-present milk.' );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( '  You make sure to fondle each of your members equally, caressing, squeezing, and stroking to the tempo of your swelling passion.' );
			}
			MainView.outputText( '  With your off-hand, you rub your cream-lubricated fingers through your sodden gash, the flesh parting easily to allow a few of your questing fingers inside.  Delicious bliss unfolds from your [clit] as it pushes free of its hood, fully engorging, faintly throbbing from aching need.  You brush the button a few times before going back to fingering your box, yet you make sure to strum your thumb across your clit every few moments to keep yourself as close to peak as possible.  Truly, being a hermaphrodite is bliss.' );
		}
		//(DA SCHLICKS);
		else if( CoC.player.gender === 2 ) {
			MainView.outputText( '  Your pussy juices mix freely with the tub\'s white-colored \'waters\', allowing your cream-lubed fingers to plunge into your [vagina] with ease.  You stroke your lips and caress the interior of your birth canal with the intimate familiarity of a skilled lover, playing with your body until you feel your control slipping, so wound up with lust that you feel like an over-tightened guitar string vibrating out of control.' );
		}
		//(DA DUDES);
		else {
			MainView.outputText( '  Your [cock] throbs painfully in your hand, so hot and hard that you\'re sure you must have begun to leak precum, but any fluid is swiftly washed away by the ever-present milk.  ' );
			if( CoC.player.cockTotal() > 1 ) {
				...
				MainView.outputText( '  With your offhand, you cradle your [sack], hefting your [balls] as you feel your desire churning to new levels.' );
			}
		}
		//(TOGETHER);
		...
		if( CoC.player.hasCock() ) {
			MainView.outputText( '  [EachCock] releases its own gooey load, sputtering and spurting to add to the pearly deluge.' );
			if( CoC.player.cumQ() >= 500 ) {
				MainView.outputText( '  With every torrent of seed you release, you can see it lift partway out of the tub, propelled by your incredible virility towards the nearest female specimen.' );
			}
			if( CoC.player.cumQ() >= 3000 ) {
				MainView.outputText( '  Soon, the tub\'s fluid contents break out of their confines to stain your companion\'s dusky flesh white, an alabaster glaze that would entice you to further feats of debauchery were it not for the pleased contentment your maleness now radiates.' );
			}
		}
		if( CoC.player.hasVagina() ) {
			MainView.outputText( '  Meanwhile, your juiced-up cunny feels like it\'s doing backflips inside you, contorting and squeezing as it gushes with fluid, feminine joy.  One brush on your [clit] knocks your [legs] out from underneath you, but thankfully, you float out the rest of your orgasm.' );
		}
	...
	DungeonSandWitch.prototype.sandPregUpdate = function() {
		...
		if( CoC.player.pregnancyIncubation === 280 ) {
			...
			if( CoC.player.biggestLactation() < 1 ) {
				MainView.outputText( 'starting to lactate' );
			} else {
				MainView.outputText( 'lactating more' );
			}
			MainView.outputText( '?  Your belly is even bulging a little bit more than food can explain.\n' );
			_.forEach( _.filter( CoC.player.breastRows, function( breastRow ) {
				return breastRow.breastRating < 1;
			} ), function( breastRow ) {
				breastRow.breastRating = 1;
			} );
			while( CoC.player.biggestLactation() < 1.5 ) {
		...
		if( CoC.player.pregnancyIncubation === 180 ) {
			...
			while( CoC.player.biggestLactation() < 2 ) {
				CoC.player.boostLactation( 0.5 );
			}
			return true;
		}
		...
		if( CoC.player.pregnancyIncubation === 120 ) {
			...
			while( CoC.player.biggestLactation() < 3 ) {
				CoC.player.boostLactation( 0.5 );
			}
			return true;
		}
		...
		if( CoC.player.pregnancyIncubation === 72 ) {
			...
			while( CoC.player.biggestLactation() < 4.5 ) {
				CoC.player.boostLactation( 0.5 );
			}
			return true;
		}
		...
		if( CoC.player.pregnancyIncubation === 48 ) {
			...
			while( CoC.player.biggestLactation() < 5.5 ) {
				CoC.player.boostLactation( 0.5 );
			}
			return true;
		}
