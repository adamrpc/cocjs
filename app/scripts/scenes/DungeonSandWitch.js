...
angular.module( 'cocjs' ).run( function( SceneLib, SandMother, PregnancyStore, MainView, CumWitch, Combat, Appearance, ConsumableLib, Utils, PerkLib, StatusAffects, Descriptors, CockTypesEnum, OnLoadVariables, AppearanceDefs, kFLAGS, CoC, EngineCore ) {
	...
	DungeonSandWitch.prototype.sandstonesAreCool = function() {
		...
		if( !Combat.combatMiss() && !Combat.combatEvade() && !Combat.combatFlexibility() && !Combat.combatMisdirect() ) {
			...
			if( !CoC.player.isGoo() ) {
				...
				if( CoC.player.cockTotal() > 1 ) {
					EngineCore.outputText( '  A bunch of the stones crawl onto your ' + CoC.player.multiCockDescriptLight() + '.' );
				}
				if( CoC.player.hasCock() ) {
					bonus++;
				}
				...
				EngineCore.outputText( '  ' + Utils.num2Text( CoC.player.totalNipples() ) + ' crawl up to your chest and over your top ' + Descriptors.nippleDescript( 0 ) + 's' );
	...
	DungeonSandWitch.prototype.cocknosisAttack = function() {
		...
		if( CoC.player.inte / 20 + Utils.rand( 20 ) >= 13 ) {
			...
			EngineCore.outputText( 'The Witch smirks, though you\'re too focused on her cock to see, and she continues, "<i>Good ' + CoC.player.mf( 'boy', 'girl' ) + '.  Watch it sway.  You\'re so focused on my cock now that the world is just falling away around it, sinking into nothingness, leaving only that wonderful cock behind for you to watch.  And since it\'s filling your view, you can feel it filling your mind as well, can\'t you?</i>"' );
	...
	DungeonSandWitch.prototype.beACumPumpPartII = function() {
		...
		if( CoC.player.cockTotal() > 1 ) {
			EngineCore.outputText( 'Her voluminous butt is thoroughly smeared by the goo from the rest of your ' + CoC.player.multiCockDescriptLight() + ', shining with white from your magical virility.  ' );
		}
	...
	DungeonSandWitch.prototype.repeatLoseToCumWitchForDudes = function() {
		...
		if( CoC.player.cockThatFits( CoC.monster.vaginalCapacity() ) < 0 ) {
			...
			if( CoC.player.biggestCockLength() < CoC.player.tallness / 2 ) {
				...
				EngineCore.outputText( CoC.player.face() );
			} else {
			...
			EngineCore.outputText( 'loud and firm enough for your dazzled mind to cogitate her words: "<i>Oh, my pretty bitch ' + CoC.player.mf( 'boy', 'girl' ) + ', feel the pressure building up inside you.  Higher and higher now... It\'s almost too much, isn\'t it?  You don\'t need to answer, just feel it grow thicker and hotter.  You\'re going to cum like a geyser, because of me, and you\'re going to love it.  You\'ll always want to cum for me, won\'t you?</i>"' );
	...
	DungeonSandWitch.prototype.tooBigCumWitchLossFacial = function() {
		...
		if( CoC.player.balls > 0 ) {
			...
			EngineCore.outputText( 'taint, rubbing it as a tingle of electric need phases through your ' + CoC.player.skinFurScales() + ' to assault your swelling prostate and seminal vesicles' );
		}
		...
		if( CoC.player.biggestCockLength() >= CoC.player.tallness / 1.6 ) {
			EngineCore.outputText( '  The Cum Witch smiles as she holds it, bending it to make sure the overlong shaft deposits its thick load on your ' + CoC.player.face() + ' where it belongs.' );
		}
	...
	DungeonSandWitch.prototype.TDMsLoseToCumWitchScene = function() {
		...
		if( CoC.player.cockTotal() > 1 ) {
			...
			if( CoC.player.cockTotal() === 2 ) {
				...
				EngineCore.outputText( 'the rest of your ' + CoC.player.multiCockDescriptLight() + '.' );
			}
	...
	DungeonSandWitch.prototype.menFuckUpSomeCumWitch = function() {
		...
		if( CoC.player.gender === 3 ) {
			...
			if( CoC.player.balls > 0 ) {
				...
				EngineCore.outputText( 'Her sweaty nutsack glides over your ' + CoC.player.skinFurScales() + ' as you fuck, compressing and twitching against you as it begs to discharge its frothy load.' );
			}
	...
	DungeonSandWitch.prototype.getBlown = function( submit ) {
		...
		if( CoC.player.cockTotal() >= 3 ) {
			...
			EngineCore.outputText( 'other ' + Utils.num2Text( CoC.player.cockTotal() - 1 ) + ' members, wrapping tightly around their meaty lengths.  You groan throatily as tingling digits slowly work their way up your shafts, making your heart flutter wildly with the alien sensation.' );
			//AND If Multicock = 5;
	...
	DungeonSandWitch.prototype.loseToSandMotherBadEnd = function() {
		...
		if( CoC.player.cockTotal() === 1 ) {
			...
			EngineCore.outputText( 'while she gleefully moves to your second cock, smearing it with your first dick\'s pre-cum, messing your ' + CoC.player.multiCockDescriptLight() + ' with each penis\'s leaky drippings.' );
		}
	...
	DungeonSandWitch.prototype.loseToSandMother = function() {
		...
		if( CoC.player.bRows() <= 2 ) {
			...
			EngineCore.outputText( ', admiring how you\'ve managed to grow ' + Utils.num2Text( CoC.player.bRows() ) + ' rows of breasts instead of just two on your panting torso' );
		}
		EngineCore.outputText( '.' );
	...
	DungeonSandWitch.prototype.friendlySandMotherFuck = function() {
		...
		EngineCore.outputText( 'Her lube-glossed gates spread around your ' + CoC.player.cockHead( x ) );
		if( y >= 0 ) {
	...
	DungeonSandWitch.prototype.grabTheMilkGirl = function() {
		...
		if( CoC.player.hasCock() ) {
			...
			if( CoC.player.balls > 0 ) {
				EngineCore.outputText( ', her other hand cupping your [balls], rolling the ' + Utils.num2Text( CoC.player.balls ) + ' orbs in her palm with surprising dexterity' );
			}
