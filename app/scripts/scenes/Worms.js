...
angular.module( 'cocjs' ).run( function( MainView, SceneLib, StatusAffects, $log, WormMass, Descriptors, kFLAGS, Utils, Combat, PerkLib, CoC, EngineCore ) {
	...
	Worms.prototype.wormsOn = function() {
		MainView.clearOutput();
		MainView.outputText( 'You actually think it\'s kind of a hot idea, and wonder if such creatures actually exist in this land as you make your way back to camp.' );
		CoC.player.createStatusAffect( StatusAffects.WormsOn, 0, 0, 0, 0 );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	Worms.prototype.wormsPartiallyOn = function() {
		MainView.clearOutput();
		MainView.outputText( 'You shrug and keep walking, not sure how you feel about the strange sign.' );
		CoC.player.createStatusAffect( StatusAffects.WormsOn, 0, 0, 0, 0 );
	...
	Worms.prototype.wormsOff = function() {
		MainView.clearOutput();
		MainView.outputText( 'You shudder in revulsion and figure the sign to be the result of someone\'s perverted fantasy.' );
	...
	Worms.prototype.wormsRun = function() {
		...
		if( CoC.player.spe > Utils.rand( 35 ) ) {
			MainView.outputText( 'Your instincts overwhelm you and you immediately turn around and run like hell in the opposite direction. You look behind you as your heart feels as if it is about to burst only to discover that the creature did not follow you. You take a moment to catch your breath and consider yourself fortunate.' );
			EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
		} else {
	...
	Worms.prototype.wormsEntice = function() {
		...
		if( Utils.rand( 2 ) === 0 ) {
			if( CoC.player.lust < 50 ) {
				MainView.outputText( 'The worm colony stands before you and begins secreting a significant amount of slime. You are perplexed as to why the worms have done this. You shrug your shoulders and remain on guard.\n', false );
			} else {
		...
		//SUCCESS;
		if( CoC.player.lust < 50 ) {
			MainView.outputText( 'The worm colony stands before you and begins secreting a significant amount of slime. Inexplicably, you find that your ' + Descriptors.cockDescript( 0 ) + ' is already erect and is throbbing. The erection is quite meddlesome and you find yourself distracted by the unwanted arousal.\n', false );
			EngineCore.dynStats( 'lus', 10 + CoC.player.lib / 20 + CoC.player.cor / 20 );
		} else {
	...
	Worms.prototype.playerInfest = function() {
		...
		if( CoC.monster.short === 'Izma' ) {
			...
			MainView.outputText( 'With a great squeeze, you will your body to push out a load of semen filled with worms.  Izma\'s eyes widen in shock as she dives and rolls away from the torrent.  "<i>What the fuck!</i>" she yells. "<i>Is that what that smell was?  That\'s disgusting!  Get away from me ' + CoC.player.short + ', I DON\'T want what you\'ve got.  Don\'t talk to me again while you\'ve got those nasty things!</i>"  Izma grabs her locker between her hands and wades into the lake, swimming away by means of her tail while holding the books out of the water.', true );
			//(set Izmacounter to 0);
			CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00230 ] = 0;
		...
		EngineCore.fatigue( 40, 2 );
		MainView.outputText( 'With a great squeeze, you will your body to push out a load of semen filled with worms. You moan in pleasure and release as your fluids launch at your enemy. Worms and cum fly through the air in an attempt to coat your opponent with your new pets.  ', true );
		//Viable target?;
		if( CoC.monster.short === 'minotaur' || CoC.monster.short === 'imp' ) {
		...
		} else if( CoC.monster.short === 'anemone' ) {
			//Infest vs. anemone:;
			MainView.outputText( 'The white mess takes the anemone square in the torso, and the worms nearest her blue shaft waste no time in heading for it.  You both watch curiously as the lead worms ascend her dick, only to run headlong into the tentacles and slip off, stunned.  Eyes twinkling, the girl scoops up a handful of the squirming load still on her stomach and slurps it down greedily.  She smacks her lips and says, "<i>Chewy!</i>"\n', false );
			CoC.monster.HP += 20;
	...
	Worms.prototype.nightTimeInfestation = function() {
		...
		MainView.outputText( ' open and vulnerable to the piles of spunk-hungry parasites, and as you look on in shock, you see four of them slide into your worm-packed tunnel' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ', one after another!  No!  You reach for your [cock biggest] to somehow try and stop the infestation, but it\'s a futile gesture.  The worm pile on your [legs] is as big as the rest of you.  Worse still, you doubt you can pull out the ones that have already crawled inside.' );
		...
		if( CoC.player.cocks[ CoC.player.biggestCockIndex() ].cockThickness >= 6 ) {
			MainView.outputText( 'as tight around the girth as you can' );
		} else {
			MainView.outputText( 'tightly around your girth' );
		}
		MainView.outputText( ', hoping to contain the squiggling menace in the upper reaches of your erection.' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( '  It\'s a vain effort, as your other penis' );
			if( CoC.player.cockTotal() > 2 ) {
				MainView.outputText( 'es are' );
			} else {
				MainView.outputText( ' is' );
			}
			MainView.outputText( 'already turning into ' );
			if( CoC.player.cockTotal() === 2 ) {
				MainView.outputText( 'an ' );
			}
			MainView.outputText( 'infestation highway' );
			if( CoC.player.cockTotal() > 2 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( '.  ' );
		...
		MainView.outputText( ' to absorb the sensual nutrition.  Some of them even slop off the sides after the dripping whiteness, to get to it before it absorbs into the brittle, wasted earth.  You cum and cum, almost without end, yet each thick jet is slurped into the writhing mass immediately, leaving behind just the parasitic, clear slime.' );
