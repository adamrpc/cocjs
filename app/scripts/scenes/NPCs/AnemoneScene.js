...
angular.module( 'cocjs' ).run( function( Anemone, LustyMaidensArmor, ItemType, WeaponLib, Weapon, $rootScope, Descriptors, PerkLib, ImageManager, CockTypesEnum, Appearance, AppearanceDefs, EventParser, ArmorLib, CoC, kFLAGS, Utils, StatusAffects, PregnancyStore, EngineCore, Combat, ConsumableLib ) {
	...
	AnemoneScene.prototype.timeChange = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.ANEMONE_KID ] > 0 ) {
			if( CoC.getInstance().flags[ kFLAGS.KID_ITEM_FIND_HOURS ] < 20 ) {
				CoC.getInstance().flags[ kFLAGS.KID_ITEM_FIND_HOURS ]++;
			}
			if( CoC.getInstance().flags[ kFLAGS.KID_SITTER ] === 0 && CoC.getInstance().flags[ kFLAGS.MARBLE_KIDS ] >= 5 && CoC.getInstance().time.hours > 10 && CoC.getInstance().time.hours < 18 && Utils.rand( 4 ) === 0 ) {
				this.kidABabysitsCows();
				needNext = true;
			}
			if( CoC.getInstance().flags[ kFLAGS.KID_SITTER ] === 1 && CoC.getInstance().time.hours > 10 && CoC.getInstance().time.hours < 18 && Utils.rand( 4 ) === 0 ) {
				CoC.getInstance().flags[ kFLAGS.KID_SITTER ] = 2;
			} else if( CoC.getInstance().flags[ kFLAGS.KID_SITTER ] === 2 ) {
				CoC.getInstance().flags[ kFLAGS.KID_SITTER ] = 1;
			}
		}
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.AnemoneArousal ) >= 0 ) {
			if( CoC.getInstance().player.pregnancyIncubation > 1 ) {
				CoC.getInstance().player.removeStatusAffect( StatusAffects.AnemoneArousal );
	...
	AnemoneScene.prototype.mortalAnemoneeeeee = function() {
		...
		EngineCore.outputText( 'You are fighting an anemone!', false );
	...
	AnemoneScene.prototype.defeatAnemone = function() {
		...
		if( CoC.getInstance().monster.HP < 1 ) {
			EngineCore.outputText( 'The anemone\'s knees buckle and she collapses, planting her hands behind her with a splash.  You stand over her, victorious.\n\n', false );
		}
		//win by lust:;
		else {
			EngineCore.outputText( 'The anemone slumps down and begins masturbating, stroking her cock furiously.  You think you can detect something like desperation in her opaque eyes.  It doesn\'t look like she\'ll trouble you anymore.\n\n', false );
		}
		if( CoC.getInstance().player.lust >= 33 ) {
			EngineCore.outputText( 'You could always have your way with her.  If you do, which parts do you use to do the deed?', false );
			...
			if( CoC.getInstance().player.hasVagina() && CoC.getInstance().player.clitLength >= 4 ) {
				anal = this.anemoneButtPlugginz;
			} else if( CoC.getInstance().player.hasCock() && CoC.getInstance().player.cockThatFits( 48 ) >= 0 ) {
				anal = this.anemoneButtPlugginz;
			}
			...
			var bikiniTits = null;
			if( CoC.getInstance().player.hasVagina() && CoC.getInstance().player.biggestTitSize() >= 4 && CoC.getInstance().player.armor instanceof LustyMaidensArmor ) {
	...
	AnemoneScene.prototype.victoryButtholeRape = function() {
		EngineCore.spriteSelect( 4 );
		EngineCore.outputText( '', true );
		EngineCore.outputText( ImageManager.showImage( 'anemone-getanal' ), false );
		EngineCore.outputText( 'You look over the anemone in front of you.  Your attention focuses on her blue shaft; those smaller tentacles should have plenty of pleasing venom in them as well.  Stripping off your ' + CoC.getInstance().player.armorName + ', you approach her and push her backwards.  Her gills slide off her breasts and float at her sides. revealing a pair of cute nipples.  You take the opportunity to stroke the shaft of her penis and rub her vagina a bit, soaking up some venom and making your hands tingle.\n\n', false );
		EngineCore.outputText( 'Quite soon you can hardly stand your own arousal and your lover\'s cock is nice and hard.  Straddling the anemone, you position your ' + Descriptors.assholeDescript() + ' over her colorful shaft and gradually lower yourself towards it.  The florid crown slips into your hole, delivering the expected shock, and a gasp from behind you is accompanied by the anemone\'s hands moving to your hips.', false );
		//[butt hymen removal];
		if( !CoC.getInstance().player.buttChange( CoC.getInstance().monster.cockArea( 0 ), true ) ) {
			EngineCore.outputText( '  ', false );
		}
		EngineCore.outputText( 'Despite your anticipatory stiffening you find yourself trembling and your muscles weakening, but by a superb effort you manage to concentrate and lower yourself gently, savoring the slow crawl of the warmth up your ' + Descriptors.assholeDescript() + '.  You reach the base of the anemone\'s short shaft soon, and pause for a minute; looking over your shoulder at the anemone, you notice her biting her lower lip impatiently.  Making a mental note of her cute expression to fuel your imagination, you turn forward and, putting your hands down for support, you begin to rise and fall on her erect penis.\n\n', false );
		EngineCore.outputText( 'With your ' + Descriptors.assholeDescript() + ' heated up from the aphrodisiac already, the friction is enough to warm it to fever pitch.  Over and over you impale yourself on the girl\'s rod, dragging trails of venom and heat up and down your rectum.', false );
		if( CoC.getInstance().player.cockTotal() > 0 ) {
			EngineCore.outputText( '  One hand involuntarily moves to your ' + Descriptors.cockDescript( 0 ) + ' and begins stroking, smearing the copious pre-cum forced out by the prostate stimulation over ' + Descriptors.sMultiCockDesc() + '.', false );
		}
		//[(if vag and nococks);
		else if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( '  You lift one hand up to your ' + Descriptors.vaginaDescript( 0 ) + ' and begin jilling yourself off.  This works to satisfy you for a while, but eventually you want more and grab a brace of tentacles floating in the water beside you, shoving them into your greedy pussy and smearing them around. This provokes a lusty moan from you and a giggle from your lover.', false );
		}
		EngineCore.outputText( '  As you work your ' + Descriptors.assholeDescript() + ' on the tool, something happens to push your peak closer at a startling pace...\n\n', false );
		EngineCore.outputText( 'Your blue lover, restless now and uncontent to simply lie there anymore, begins to use her own hands and hips to pump in time with you, doubling the pace of the fuck.  The fervid intensity of her strokes doesn\'t leave any time for the venom to disperse before the next thrust arrives, making it feel as though your ' + Descriptors.assholeDescript() + ' is filled with one huge, hot cock that nevertheless slides in and out even as it remains motionless.  The sensation pushes you to orgasm quickly, your ' + Descriptors.assholeDescript() + ' clamping down on the anemone\'s penis', false );
		if( CoC.getInstance().player.cockTotal() > 0 ) {
			EngineCore.outputText( ' as ' + Descriptors.sMultiCockDesc() + ' twitches and ejaculates in a ', false );
			...
			EngineCore.outputText( ' of semen', false );
			//[(if vag and cox);
			if( CoC.getInstance().player.hasVagina() ) {
				EngineCore.outputText( ' and your pussy spasms', false );
			}
		} else if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( ' and your ' + Descriptors.vaginaDescript( 0 ) + ' spasms around the tentacles and your fingers', false );
		}
		EngineCore.outputText( '.  The anemone must have been right on the edge with you, because after a few more thrusts in your hungry asshole she achieves her own climax and shoots several strings of cool, thick semen into you.  You collapse backward against your partner and she idly caresses your ' + Descriptors.nippleDescript( 0 ) + '.  After several minutes of relaxing in the warm water, you sit up and push yourself off of the anemone\'s limp penis, which drags a string of semen out of your ' + Descriptors.assholeDescript() + ' and prompts ', false );
		//[(dix);
		if( CoC.getInstance().player.totalCocks() > 0 || CoC.getInstance().player.gender === 0 ) {
			EngineCore.outputText( 'a ', false );
		} else if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( 'another ', false );
		}
		EngineCore.outputText( ' giggle from the blue girl.  Standing up, you gather your gear and blow her a kiss before you leave.  She darkens in color, her camouflage reflex causing her to \'blush\' in discomfort at this display of affection.', false );
	...
	AnemoneScene.prototype.rapeAnemoneWithDick = function() {
		EngineCore.spriteSelect( 4 );
		EngineCore.outputText( '', true );
		EngineCore.outputText( ImageManager.showImage( 'anemone-male-fuck' ), false );
		if( CoC.getInstance().player.cockThatFits( 36 ) >= 0 ) {
			var x = CoC.getInstance().player.cockThatFits( 36 );
			EngineCore.outputText( 'Rubbing yourself through your ' + CoC.getInstance().player.armorName + ', you look over the anemone; your attention wanders down her torso to the blue slit between her legs', false );
			//[(lust victory);
			if( CoC.getInstance().monster.lust > 99 ) {
				EngineCore.outputText( ', which she\'s currently diddling with the hand she\'s not using to stroke her cock', false );
			}
			EngineCore.outputText( '.  Unfastening your garments, you stroke ' + Descriptors.sMultiCockDesc() + ' to full hardness and approach her.  The anemone looks up at you, still somewhat befogged; then, as you stand over her, she leans forward and opens her mouth invitingly.\n\n', false );
			EngineCore.outputText( 'You smile at how eager she is for you, but shake your head.  The anemone closes her mouth and looks at you quizzically.  <i>"No?"</i> she asks.  Only then does she follow your gaze down to her pussy.  The skin on her face darkens a bit as she realizes your intention... which turns out to be a blush, by the looks of the shy glance she gives you next!  <i>"Umm."</i>  The anemone\'s fingers', false );
			//[(HP defeat);
			if( CoC.getInstance().monster.HP < 1 ) {
				EngineCore.outputText( ' move to the lips of her vagina and', false );
			}
			EngineCore.outputText( ' pull apart her feathery labia, showing a velvety interior.  <i>"Ok..."</i> she says haltingly.  You accept the invitation in a hurry, kneeling down and holding onto her hips, then guiding your ' + Descriptors.cockDescript( x ) + ' into her.\n\n', false );
			EngineCore.outputText( 'After a moment of savoring the sensation, you push all of the way in, provoking a moan and a widening of the eyes from your partner.  ', false );
			//[(multicox);
			if( CoC.getInstance().player.cockTotal() > 1 ) {
				EngineCore.outputText( 'As you push all the way into her, your other dick', false );
				if( CoC.getInstance().player.cockTotal() > 2 ) {
					EngineCore.outputText( 's', false );
				}
				EngineCore.outputText( ' rub', false );
				if( CoC.getInstance().player.cockTotal() === 2 ) {
					EngineCore.outputText( 's', false );
				}
				EngineCore.outputText( ' up against the feelers lining her pussy.  Unexpectedly, they also contain the venomous nematocysts of her tentacles and in that single stroke ' + Descriptors.sMultiCockDesc() + ' is throbbing hard and squeezing pre-cum all over her groin.  She reaches down and plays with it', false );
				if( CoC.getInstance().player.cockTotal() > 2 ) {
					EngineCore.outputText( ', and them,', false );
				}
				EngineCore.outputText( ' as you start pumping.  ', false );
			}
			EngineCore.outputText( 'The fuck begins in earnest as you demonstrate all the techniques you know or can imagine; the anemone seems to be more impressed as you go on, cooing and moaning to goad you further while wrapping her hands and hair around your hips for chemical encouragement.  Her small tits bounce a little', false );
			//[(if PC boobs > A);
			if( CoC.getInstance().player.biggestTitSize() >= 1 ) {
				EngineCore.outputText( ' in time with your own', false );
			}
			EngineCore.outputText( '; here and there one of the bounces brush her gills away, giving you a tantalizing view of nipple.', false );
			//[(if balls);
			...
			EngineCore.outputText( '\n\n', false );
			EngineCore.outputText( 'The anemone\'s eyes roll back in her head as she reaches her climax first, hips shaking and penis squirting a glob of semen that drools down the side.  ', false );
			//[(Flexibility cat perk);
			if( CoC.getInstance().player.findPerk( PerkLib.Flexibility ) >= 0 ) {
				EngineCore.outputText( 'You lean down and take the anemone\'s cock in your own mouth as she continues orgasming, swallowing the cool, slippery jism -- it tastes not salty and fishy as you\'d hoped but somewhat faintly like algae.  The anemone, recovering her wits a bit, looks at you blankly, as though she can\'t fathom why anyone would want to drink <i>her</i> ejaculate instead of the other way around.  Your eyes twinkle mirthfully in response as you suck and swallow the last of her jizz.  ', false );
			}
			EngineCore.outputText( 'Your own orgasm takes a bit longer, but the convulsing walls of her pussy do their best to help you arrive and the feelers along her labia writhe against your ' + Descriptors.multiCockDescriptLight() + ' in the same tempo, injecting the last of their venom.  With a deep, final thrust, ' + Descriptors.sMultiCockDesc() + ' erupts in her pussy.  ', false );
			//[(big skeet);
			if( CoC.getInstance().player.cumQ() > 500 ) {
				EngineCore.outputText( 'You continue to pour into her even after her convulsions stop, stretching her belly', false );
				//[(super skeet);
				if( CoC.getInstance().player.cumQ() > 2000 ) {
					EngineCore.outputText( ' to enormous dimensions.  <i>"Ohh..."</i> she moans, as her waist distends to over four times its former slender diameter', false );
				}
				EngineCore.outputText( '.  She looks ', false );
				if( CoC.getInstance().player.cumQ() < 2000 ) {
					EngineCore.outputText( 'thoroughly', false );
				} else {
					EngineCore.outputText( 'monstrously', false );
				}
				EngineCore.outputText( ' pregnant when you\'ve finished, her little blue dick poking out below a swollen stomach... not a bad look, actually.  You get a little turned on at the idea.  ', false );
			}
			EngineCore.outputText( 'After the last of your cum has been squeezed out, you pull your ' + Descriptors.cockDescript( 0 ) + ' out and rinse it off in the lakewater.  You gather your gear while the anemone holds her belly and smiles placidly, staring into the sky.', false );
		...
		//Too-big male: -requires cockarea > 36;
			...
			EngineCore.outputText( '.  Unfastening your garments, you stroke ' + Descriptors.sMultiCockDesc() + ' to full hardness and approach her.  The anemone looks up at you, still somewhat befogged; then, as you stand ', false );
			if( CoC.getInstance().player.tallness > 48 ) {
				EngineCore.outputText( 'over', false );
			} else {
				EngineCore.outputText( 'before', false );
			}
			EngineCore.outputText( ' her, her eyes widen as she beholds the sheer amount of cock you have.\n\n', false );
			EngineCore.outputText( 'You smile at how stunned she is by you, and waggle your erect ' + Descriptors.multiCockDescriptLight() + ' from side-to-side.  The anemone obediently watches it swing, like a hypnotist\'s pendulum; her mouth reflexively opens as all her conscious thought is temporarily sidetracked.  You push on the shaft with one hand and move the tip down relative to her body, watching bemused as the girl tries to keep her mouth in line with it until it goes too low for her neck and snaps her out of her daze.  She closes her mouth and looks at you quizzically.  <i>"No?"</i> she asks.  You answer by pushing forward slightly, bumping the head of your ' + Descriptors.cockDescript( 0 ) + ' against her <i>mons pubis</i>.  The skin on her face darkens a bit as she realizes your intention... which turns out to be a blush, by the looks of the shy glance she gives you next!  <i>"Umm."</i>  She gives it a moment of concerned study before her natural instincts take over and the anemone\'s fingers', false );
			//[(HP defeat);
			if( CoC.getInstance().monster.HP < 1 ) {
				EngineCore.outputText( ' move to the lips of her vagina and', false );
			}
			EngineCore.outputText( ' pull apart her feathery labia, showing a velvety interior.  <i>"Ok..."</i> she says, clearly unsure of the wisdom of this.  You motion for her to lay back and lift her pussy as high as possible to reduce the angle, then attempt to guide your ' + Descriptors.cockDescript( 0 ) + ' into her.', false );
			//[(PC height > 7' and non-centaur);
			if( CoC.getInstance().player.tallness > 84 ) {
				EngineCore.outputText( '  Finally, after having gone so far as to kneel down to bring yourself in line, you begin pushing your way in.', false );
			}
			EngineCore.outputText( '\n\n', false );
			EngineCore.outputText( 'The first few inches are slow going, as you try to stretch the blue girl\'s roughly human-sized pussy around your superhuman girth.  She sports a worried expression as you struggle to push the crown of your ' + Appearance.cockNoun( CockTypesEnum.HUMAN ) + ' in without bending your shaft.  The girl\'s body proves more elastic than you\'d expected, though; with each shove her cunt takes more width without suffering any obvious ill effect.  Eventually you get the head of your ' + Descriptors.cockDescript( 0 ) + ' into her, and you give an experimental thrust to test her depths. You slide surprisingly far into her, your huge ' + Appearance.cockNoun( CockTypesEnum.HUMAN ) + ' making a bump in her pelvis; the anemone has abandoned her worry and is blissfully tweaking her nipples.  Taking the anemone\'s legs under your arms, you begin pumping at her, savoring the sensation of having at least part of your unwieldy tool inside someone.  Your blue partner gives out cute moans as you fuck her, prompting a grin from you, but most of your attention is focused on maintaining the angle of your ' + Descriptors.cockDescript( 0 ) + '.  As you focus on the thrusts, you gradually notice the bump sliding further up her pelvis on each push... it\'s nearly up to her stomach now!  You quickly make up your mind to test her unusual biology, choking up on your grip of her legs and pushing hard with each thrust in.  The anemone is now tracking the progress of the bump representing your ' + Appearance.cockNoun( CockTypesEnum.HUMAN ) + ' as intently as you are; as your head pushes up her chest between her breasts she presses them together against the ridge, massaging them against the shaft through her skin.', false );
			//[(cocklength > 60");
			if( CoC.getInstance().player.cocks[ 0 ].cockLength > 60 ) {
				EngineCore.outputText( '  The imagery and the stimulation inspire you to increase your efforts, and you push even harder into her until your ' + Descriptors.cockDescript( 0 ) + ' slides its way into her throat, expanding her slender neck to twice the normal width.  The anemone\'s mouth gapes open reflexively as if to gag as the pressure forces her head back, but she doesn\'t stop moaning or rubbing herself against you.  This seems to be as deep as you can get; the tip of your cock is now right up against the base of her chin.', false );
			}
			EngineCore.outputText( '\n\n', false );
			EngineCore.outputText( 'Inordinately pleased at having gotten your ' + Descriptors.cockDescript( 0 ) + ' so far in, you begin thrusting grandly, alternating huge back-and-forth pumps with hip gyrations that press you against the sides of your partner.  The anemone', false );
			//[(dick > 60");
			if( CoC.getInstance().player.cocks[ 0 ].cockLength > 60 ) {
				EngineCore.outputText( ', despite being unable to look anywhere except over her head thanks to the giant ridge running up the front of her body,', false );
			}
			EngineCore.outputText( ' has clearly been enjoying the treatment; soon she begins twitching in orgasm and her pussy spasms against the base of your ' + Appearance.cockNoun( CockTypesEnum.HUMAN ) + ', flicking its venomous feelers into it, as she kneads her breasts in her hands.  Her body attempts to convulse, bending your ' + Descriptors.cockDescript( 0 ) + ' slightly, while her neglected dick gives a little squirt of pearly semen which lands on the raised line in the center of her body and makes a little trail as it rolls down.  The pressure of her twisting and the sensation of her pussy lapping at your shaft with aphrodisiac is enough to send you over the edge as well.  Your ' + Descriptors.cockDescript( 0 ) + ' twitches as you unload into your blue partner', false );
			//[(big skeet);
			if( CoC.getInstance().player.cumQ() > 500 ) {
				EngineCore.outputText( ' until it puffs out her cheeks', false );
			}
			EngineCore.outputText( '.', false );
			//[(mega skeet);
			if( CoC.getInstance().player.cumQ() > 1500 ) {
				EngineCore.outputText( '  Eventually she can\'t hold in the sheer volume of your ejaculate, and it erupts from her mouth in a white spray.  Spurt after spurt goes into and then out of her, dribbling out of her slack mouth and down her face until her hair is covered with seed.', false );
			}
			EngineCore.outputText( '  She takes the opportunity to squeeze along the length of your cock, pushing out as much semen as you have to offer as she moans in orgiastic delight at the fluid injection.', false );
			EngineCore.outputText( '\n\n', false );
			EngineCore.outputText( 'Eventually both you and she are spent and limp, and you draw your ' + Descriptors.cockDescript( 0 ) + ' out of her, making an extended, wet sucking noise.  As you pull up your gear and make your way up the beach, ', false );
			//[(normal/big skeet);
			if( CoC.getInstance().player.cumQ() < 500 ) {
				EngineCore.outputText( 'her hands are still dazedly playing with the space between her breasts where your cock used to rest.', false );
			}//[(mega skeet);
			else {
				EngineCore.outputText( 'she continues to sputter and cough up bubbles of your spunk.', false );
			}
			//(pass 1 hour, reset lust to min or min+10 if big or greater skeet);
			CoC.getInstance().player.orgasm();
			Combat.cleanupAfterCombat();
	...
	AnemoneScene.prototype.rapeAnemoneWithPussy = function() {
		EngineCore.spriteSelect( 4 );
		EngineCore.outputText( '', true );
		EngineCore.outputText( ImageManager.showImage( 'anemone-female-fuck' ), false );
		EngineCore.outputText( 'As you review your handiwork, the stirrings in your feminine side focus your attention on the anemone\'s penis.  Those smaller tentacles on it should have plenty of pleasing venom in them as well.  You make up your mind to put them to use for you.\n\n', false );
		EngineCore.outputText( 'The anemone looks vacantly up at you as you approach.  Reaching forward, you take her cock in your hand', false );
		//[(lust victory);
		if( CoC.getInstance().monster.lust > 99 ) {
			EngineCore.outputText( ' after brushing hers aside', false );
		}
		EngineCore.outputText( ' and begin to fondle the crown, with its slippery tentacles.  As expected, her venom flows into your hand, imparting a sensation of heat that slides up your arm and diffuses into a gentle warmth.  After a few rubs, you lean down and carefully take her penis into your mouth.  It tastes of the lakewater and heats your mouth as it did your hand; ', false );
		//[(HP victory);
		if( CoC.getInstance().monster.HP < 1 ) {
			EngineCore.outputText( 'you can feel it harden as ', false );
		}
		EngineCore.outputText( 'you caress it with your tongue before pulling it out and giving it a squeeze.  The blue girl shivers as a drop of pre-cum is forced out.\n\n', false );
		EngineCore.outputText( 'Next, you take the time to strip off your ' + CoC.getInstance().player.armorName + ', making sure to give a good show; the anemone entertains herself by stroking her erect prick and smearing around the pre-cum, grinning as she watches you.', false );
		//[(breastrow0>C-cup);
		if( CoC.getInstance().player.biggestTitSize() > 2 ) {
			EngineCore.outputText( '  You give special attention to the presentation of your ' + Descriptors.breastDescript( 0 ) + ', removing your top with tantalizing slowness, letting each breast slip out and hang between you like fruit ripe to be taken, then making sure to rub them seductively to arouse the both of you further.', false );
		}
		//(hipsize=girly or better);
		if( CoC.getInstance().player.hipRating > 6 ) {
			EngineCore.outputText( '  You make good use of your ' + Descriptors.hipDescript() + ', too, giving a little shimmy to show off your pronounced curves.', false );
		}
		EngineCore.outputText( '  By the time you\'re finished, the anemone\'s crotch glistens with fluid from both her sexes; it\'s probably as wet as it was when she was underwater.  You lean into the anemone and give her a deep kiss, ', false );
		//[(breast0>C);
		if( CoC.getInstance().player.biggestTitSize() > 2 ) {
			EngineCore.outputText( 'making sure to let your ' + CoC.getInstance().player.allBreastsDescript() + ' rub up against hers, ', false );
		}
		EngineCore.outputText( 'then pull apart from her and ', false );
		//[(goddamn centaur);
		if( CoC.getInstance().player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_CENTAUR ) {
			EngineCore.outputText( 'turn away, kneeling down to display your animalistic, musky pussy readily.', false );
		} else {
			EngineCore.outputText( 'recline back on your ' + CoC.getInstance().player.legs() + '. Spreading your thighs, you reach down with two fingers and pull apart your ' + Descriptors.vaginaDescript( 0 ) + ' welcomingly; it\'s the last act in your sexual performance.', false );
		}
		EngineCore.outputText( '\n\n', false );
		EngineCore.outputText( 'The anemone wastes no time in assessing your intention and crawls forward onto you, returning your kiss with equal passion.  ', false );
		//[(no centaur);
		if( CoC.getInstance().player.lowerBody !== AppearanceDefs.LOWER_BODY_TYPE_CENTAUR ) {
			EngineCore.outputText( 'You take her by the upper arms and pull her on top of you as you lie back in the sun-warmed shallows.  ', false );
		}
		EngineCore.outputText( 'Her hair drapes over you as she lines her penis up with your ' + Descriptors.vaginaDescript( 0 ) + ', delivering heat to your body, but this is dwarfed by the sensation of her entry as she pushes her cock in for the first time.  ', false );
		CoC.getInstance().player.cuntChange( CoC.getInstance().monster.cockArea( 0 ), true );
		EngineCore.outputText( 'The penetration combines with the aphrodisiac being injected straight into your hungry pussy to produce a feeling like euphoria.  Unable to focus your thoughts any longer, you allow the anemone to take the lead as she begins pumping into you, coating your labia with a mixture of her pre-cum and your own secretion.  Soon you\'re moaning lustily with complete disregard for anything except the pleasure between you as your lover ups the pace; ', false );
		//[(biped);
		if( CoC.getInstance().player.lowerBody !== AppearanceDefs.LOWER_BODY_TYPE_CENTAUR ) {
			EngineCore.outputText( 'as she thrusts hard and fast, her hair whips back and forth over your ', false );
			//[(breasts>manly);
			if( CoC.getInstance().player.biggestTitSize() >= 1 ) {
				EngineCore.outputText( Descriptors.breastDescript( 0 ) + ' and ', false );
			}
			EngineCore.outputText( Descriptors.nippleDescript( 0 ) + 's,', false );
		}
		//(centaur);
		else {
			EngineCore.outputText( 'as she pushes deeply into your cunt, her hair flies forward past your upper body, brushing along your skin.  On one pass you grab some and use it as a pleasure tool, rubbing it vigorously on your ', false );
			//[(breasts>manly);
			if( CoC.getInstance().player.biggestTitSize() >= 1 ) {
				EngineCore.outputText( Descriptors.breastDescript( 0 ) + ' and ', false );
			}
		...
		EngineCore.outputText( ' spreading heat along your chest to nearly match your vagina\'s.\n\n', false );
		EngineCore.outputText( 'The overwhelming sensations drive you over the edge and your ' + Descriptors.vaginaDescript( 0 ) + ' contracts hungrily around the heat radiating from the anemone\'s cock.  As your orgasming pussy ', false );
		//(squirter);
		if( CoC.getInstance().player.vaginas[ 0 ].vaginalWetness >= AppearanceDefs.VAGINA_WETNESS_DROOLING ) {
			EngineCore.outputText( 'soaks her crotch with juice and ', false );
		}
		EngineCore.outputText( 'wrings her penis, the blue shaft responds enthusiastically; she pushes deeply into you as it begins spasming and squirting its load.  Your partner\'s mouth hangs open as you squeeze the cum out of her; with all her muscle control taken away, her head hangs limply', false );
		if( CoC.getInstance().player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_CENTAUR ) {
			EngineCore.outputText( ' on your back', false );
		}
		//[(notits);
		else if( CoC.getInstance().player.biggestTitSize() < 1 ) {
			EngineCore.outputText( ' on your chest', false );
		} else {
			EngineCore.outputText( ' between your breasts', false );
		}
		...
		EngineCore.outputText( 'Having scratched your itch, you give her another kiss, catching her by surprise.  She smiles shyly at you as you gather up your clothes, then slips into the water as you start to dress again.', false );
		this.anemonePreg();
	...
	AnemoneScene.prototype.loseToAnemone = function() {
		...
		if( CoC.getInstance().player.gender === 0 ) {
			EngineCore.outputText( 'Shivering, you slump before the anemone and begin trying to remove your ' + CoC.getInstance().player.armorName + '.  She claps and peals happily, splashing over to you.  Pushing your trembling hands out of the way, she unfastens your garments and pulls them free of you... and then stops.  You look the girl in the face uncomprehendingly and she answers your gaze with a look of equal confusion.  Your head turns to follow her as she looks you up and down and even makes a circle around you, inspecting every inch and chewing her lip anxiously.', false );
			//[(lactating);
			if( CoC.getInstance().player.biggestLactation() > 1 ) {
				EngineCore.outputText( '  For a moment the examination stops at the dribble of milk leaking from your ' + Descriptors.nippleDescript( 0 ) + 's.  With one finger she collects a bit and tastes it, only to grimace and stick her tongue out.', false );
			}
			EngineCore.outputText( '  Back at the front, the anemone motions questioningly toward your ornament-free groin with open palms.  You follow her gesture down to your spartan nethers, then look back up.  Her bottom lip is quivering and -- yes, it looks like water is beginning to well up in her eyes.\n\n', false );
			EngineCore.outputText( 'Hurriedly you begin to compose an explanation for your anatomy, and you get as far as telling her that you have no genitalia thanks to events since your arrival before she bursts into tears.  ', false );
			//[(low cor);
			if( CoC.getInstance().player.cor < 33 ) {
				EngineCore.outputText( 'You reach out instinctively to comfort her, but ', false );
			}
			//(high cor);
			else {
				EngineCore.outputText( 'You smirk, amused by the turnabout, until ', false );
			}
			EngineCore.outputText( 'the anemone lashes out with a slap that knocks the expression off your face and makes your eardrums sing.  <i>"Dumb!"</i> she shouts, turning sharply; her tentacles lash past you as she about-faces.  She dives down below the surface and kicks water into your face spitefully as she swims away.  You sputter and rub your jaw a bit, then stand up and walk dizzily back to camp.', false );
		...
		if( CoC.getInstance().player.cockTotal() > 0 ) {
			...
			if( CoC.getInstance().player.cockThatFits( 36 ) >= 0 ) {
				...
				if( CoC.getInstance().player.cumQ() < 50 ) {
					EngineCore.outputText( 'A droplet', false );
				} else if( CoC.getInstance().player.cumQ() < 250 ) {
					EngineCore.outputText( 'A squirt', false );
				} else {
					EngineCore.outputText( 'A steady flow', false );
				}
				EngineCore.outputText( ' of pre-cum follows up the inside in the wake of her stroke.  She touches her finger to the tip of your sensitive urethra and draws it away, stretching a string of your fluid through the air.  Putting the finger in her mouth, she savors the taste of you; the string manages to transfer to her bottom lip before she breaks it with a flick of her tongue.\n\n', false );
				EngineCore.outputText( 'She pushes you back on your haunches and leans over your groin.  Her hair-tentacles slither forward over her shoulders and drop', false );
				//[(normal);
				if( CoC.getInstance().player.lowerBody !== AppearanceDefs.LOWER_BODY_TYPE_CENTAUR ) {
					EngineCore.outputText( ' into your lap,', false );
				}
				//(shitty taur);
				else {
					EngineCore.outputText( ' downwards, onto your hams,', false );
				}
				EngineCore.outputText( ' delivering lances of venom into your lower body.  The tingle warms your groin and more pre-cum leaks out of ' + Descriptors.sMultiCockDesc() + '.  Her eyes lock onto a glistening dollop and she drops down quickly, enfolding the head of your ' + Descriptors.cockDescript( x ) + ' in her cool mouth.  Her tongue dances around the crown of your ' + Appearance.cockNoun( CockTypesEnum.HUMAN ) + ', relieving it of the sticky pre.  Looking ', false );
				if( CoC.getInstance().player.tallness > 48 ) {
					EngineCore.outputText( 'up', false );
				} else {
					EngineCore.outputText( 'down', false );
				}
				EngineCore.outputText( ' at you, you can see a smile in the lines around her eyes even though her mouth is locked around your ' + Appearance.cockNoun( CockTypesEnum.HUMAN ) + '.  ', false );
				//[(if dual/multi-cox);
				if( CoC.getInstance().player.cockTotal() > 1 ) {
					//[(cock2 doesn't exist);
					if( CoC.getInstance().player.cockTotal() === 2 ) {
						EngineCore.outputText( 'Your other dick rubs', false );
					}//(cock2 exists);
					else {
						EngineCore.outputText( 'The rest of your ' + Descriptors.multiCockDescriptLight() + ' rub', false );
					}
					EngineCore.outputText( ' against her cheek, smearing slick wetness on her blue face.', false );
				}
				EngineCore.outputText( '\n\n', false );
				EngineCore.outputText( 'Her hands come up from the water and push two sheaves of her long, dangling hair into your ' + Descriptors.multiCockDescriptLight() + '.  Wrapping these bundles of tentacles around your ' + Descriptors.cockDescript( x ) + ', she clasps them in place with one hand and begins sliding them up and down your length.  Your ' + Appearance.cockNoun( CockTypesEnum.HUMAN ) + ' begins feeling hotter and hotter from the injections and the friction of her hair, secreting more pre-cum into her obliging mouth.', false );
				//[(if vag);
				if( CoC.getInstance().player.hasVagina() ) {
					EngineCore.outputText( '  Her other hand slips', false );
					if( CoC.getInstance().player.vaginalCapacity() < 15 ) {
						EngineCore.outputText( ' a few fingers', false );
					} else if( CoC.getInstance().player.vaginalCapacity() < 30 ) {
						EngineCore.outputText( ' partway', false );
					} else {
						EngineCore.outputText( ' all the way', false );
					}
					EngineCore.outputText( ' into your ' + Descriptors.vaginaDescript( 0 ) + ', sending a tingle through your lower lips and exposing your clitoris.', false );
					//[(if clit > 5");
					if( CoC.getInstance().player.clitLength > 5 ) {
						EngineCore.outputText( '  Having achieved this, she pulls her hand out and wraps another bundle of tentacles around your ' + Descriptors.clitDescript() + ', then begins jerking it off in time to her efforts on your ' + Appearance.cockNoun( CockTypesEnum.HUMAN ) + '.  Your eyes roll back in your head and your mouth gapes involuntarily at the rough stimulation of your swollen chick-stick.', false );
					}
				}
				EngineCore.outputText( '\n\n', false );
				EngineCore.outputText( 'The heat rubbing on your cock', false );
				if( CoC.getInstance().player.hasVagina() ) {
					EngineCore.outputText( ' and clit', false );
				}
				EngineCore.outputText( ' quickly gets to you, and the first orgasm begins to work its way up your ' + Descriptors.multiCockDescriptLight() + '.  Your ' + Descriptors.cockDescript( x ) + ' lets fly into the anemone girl\'s mouth', false );
				//[(big skeet);
				if( CoC.getInstance().player.cumQ() > 500 ) {
					EngineCore.outputText( ', bowing out her cheeks', false );
				}
				//[(cray-cray skeet);
				if( CoC.getInstance().player.cumQ() > 2000 ) {
					EngineCore.outputText( ' and squirting out her nose in thick ribbons', false );
				}
				//[(multi-dix);
				if( CoC.getInstance().player.totalCocks() > 1 ) {
					...
					if( CoC.getInstance().player.totalCocks() === 2 ) {
						EngineCore.outputText( 'your cocks shoot', false );
					} else {
						EngineCore.outputText( 'the rest of your ' + Descriptors.multiCockDescriptLight() + ' shoot', false );
					}
					EngineCore.outputText( ' all over her face and hair, ', false );
					//[(small skeet);
					if( CoC.getInstance().player.cumQ() < 500 ) {
						EngineCore.outputText( 'drawing a pattern like a musical score on her blue skin', false );
					}//(big skeet);
					else if( CoC.getInstance().player.cumQ() < 2000 ) {
						EngineCore.outputText( 'painting her skin white as she flinches and shuts her eyes tightly', false );
					}//(cray-cray skeet);
					else {
						EngineCore.outputText( 'whitewashing her entire upper body and running off until a fan of milky color spreads through the water around you', false );
					}
				}
				EngineCore.outputText( '.  The anemone swallows greedily as she pumps each stroke into her mouth', false );
				//[(big or > skeet);
				if( CoC.getInstance().player.cumQ() >= 500 ) {
					EngineCore.outputText( ', her taut blue belly distending as it fills', false );
				}
				EngineCore.outputText( '.\n\n', false );
				EngineCore.outputText( 'After a grateful moment of rest as the anemone swallows your issue, her hands begin pumping once again.  Oh god!  Your ' + Appearance.cockNoun( CockTypesEnum.HUMAN ) + ' quickly returns to erectness under the renewed siege of aphrodisiac', false );
				//[(multi);
				if( CoC.getInstance().player.cockTotal() > 1 ) {
					if( CoC.getInstance().player.cockTotal() === 2 ) {
						EngineCore.outputText( ' and your other ' + Descriptors.cockDescript( 1 ) + ' follows suit', false );
					} else {
						EngineCore.outputText( ' and your other pricks follow suit', false );
					}
				}
				EngineCore.outputText( '.  The blue girl continues to stroke your privates with her tentacle hair, flicking your urethra with her tongue, until you\'ve come twice more.  Nor does she display any intention of stopping there, but mercifully you black out and collapse into the water.  Half-frowning, the anemone shrugs and pushes your insensible form up onto the sandy shore.', false );
			...
			//loss, too-big male (cock > 5' width or > 48' length):;
				...
				EngineCore.outputText( '\n\n', false );
				EngineCore.outputText( 'Taking a moment to transfer her now-free hand to your other hip, the anemone girl then begins to pump her stiff pecker into your ' + Descriptors.assholeDescript() + ', pausing after every few strokes to gyrate her hips a bit, massaging your prostate with her feelers and smearing venom into it.  The stimulation brings you to your limit in minutes; your dick twitches spastically', false );
				//[(if balls);
				if( CoC.getInstance().player.balls > 0 ) {
					EngineCore.outputText( ' and your ' + Descriptors.sackDescript() + ' tightens up', false );
				}
				EngineCore.outputText( '.  This doesn\'t escape your blue lover\'s notice, and she quickly stops pumping.  Left at the edge of orgasm, you panic and look over your shoulder at her.  Judging by her grinning demeanour, she seems to be up to something diabolical.  You stare at her confusedly until you feel a new heat at the base of your ' + Descriptors.multiCockDescriptLight() + '. Glancing down, you see that her tentacle-hair has wrapped around ' + Descriptors.sMultiCockDesc() + ' and is squeezing tightly!  Pleased with the arrangement, the anemone begins pumping and rubbing your prostate again, spreading new warmth through your ' + Descriptors.assholeDescript() + '.  Your delayed climax finally arrives, but the <i>de facto</i> cockring', false );
				if( CoC.getInstance().player.cockTotal() > 1 ) {
					EngineCore.outputText( 's', false );
				}
				EngineCore.outputText( ' prevent' );
				if( CoC.getInstance().player.cockTotal() === 1 ) {
					EngineCore.outputText( 's' );
				}
				EngineCore.outputText( ' any semen from escaping!  The sensations swell and fade as your orgasm passes fruitlessly, your blue partner fucking away as merrily as ever.\n\n', false );
				EngineCore.outputText( 'For nearly an hour the anemone continues her performance, even going so far as to caress your swollen ' + Descriptors.multiCockDescriptLight() + ' with her unoccupied tentacles.  Several more orgasms arrive and desert you without bringing any relief from the pressure on your ', false );
				//[(if balls);
				if( CoC.getInstance().player.balls > 0 ) {
					EngineCore.outputText( Descriptors.ballsDescriptLight() + ' and ', false );
				}
				EngineCore.outputText( Descriptors.multiCockDescriptLight() + '.  Eventually you get to the point where you can\'t take it anymore, and when you feel the next orgasm drawing close you straighten up and begin ', false );
				//[(man);
				if( CoC.getInstance().player.lowerBody !== AppearanceDefs.LOWER_BODY_TYPE_CENTAUR ) {
					EngineCore.outputText( 'clawing at your tormentor\'s tentacles, trying to pry them from ' + Descriptors.sMultiCockDesc() + ' by main force.', false );
				}
				//(horse);
				else {
					EngineCore.outputText( 'bucking and stamping the ground, wanting to shake the tentacles loose but unable to reach them with your hands.', false );
				}
				EngineCore.outputText( '  Looking a bit irritated that you want to bring her fun to an end, the anemone nevertheless relents and releases her visegrip on your ' + Descriptors.multiCockDescriptLight() + '.  As the joy of seeing the way to your release cleared overtakes you, the anemone avails herself of your distraction to grab your arms and pull you toward her while pushing your ' + CoC.getInstance().player.legs() + ' out from under you.  The two of you fall backward into the shallow water as ' + Descriptors.sMultiCockDesc() + ' begins unloading its immense backup of semen in a high arc.  The ', false );
				//[(skeet amount);
				if( CoC.getInstance().player.cumQ() < 500 ) {
					EngineCore.outputText( 'strings', false );
				} else if( CoC.getInstance().player.cumQ() < 2000 ) {
					EngineCore.outputText( 'streams', false );
				} else {
					EngineCore.outputText( 'gouts', false );
				}
				EngineCore.outputText( ' of jism ', false );
				//[(height <4' and non-horse);
				if( CoC.getInstance().player.tallness < 48 && CoC.getInstance().player.lowerBody !== AppearanceDefs.LOWER_BODY_TYPE_CENTAUR ) {
					EngineCore.outputText( 'fly over your head, and turning behind you, you see the anemone trying to catch them with an open mouth and her tongue out.', false );
				} else if( CoC.getInstance().player.tallness < 84 && CoC.getInstance().player.lowerBody !== AppearanceDefs.LOWER_BODY_TYPE_CENTAUR ) {
					EngineCore.outputText( 'catch the air and rain down on both your faces, splashing quietly where they hit water.', false );
				} else {
					EngineCore.outputText( ' land right on your', false );
					//[(if breasts);
					if( CoC.getInstance().player.biggestTitSize() >= 1 ) {
						EngineCore.outputText( ' breasts and', false );
					}
					EngineCore.outputText( ' face.  You hear the anemone giggling as you flinch from the white shower.', false );
				}
				EngineCore.outputText( '  After several minutes of climax with you shooting more jism than you thought possible and the anemone banging out an accompaniment on your ' + Descriptors.assholeDescript() + ', you finally begin to wind down.  The anemone, clearly turned on by the impressive amount of ejaculate, unloads her own blue cock into your asshole.  Her semen, lower in temperature than yours, forms a little pocket of cool inside your ' + Descriptors.buttDescript() + '.  She idly swishes her tentacles in the', false );
				//[(big skeet);
				if( CoC.getInstance().player.cumQ() >= 500 ) {
					EngineCore.outputText( ' semen-colored', false );
				}
				EngineCore.outputText( ' water around her as you push out your last load and slip into a doze.\n\n', false );
				EngineCore.outputText( 'Pushing your inert form off of her dick, she slips out from under you and sits up beside.  ', false );
				//[(height <4' non-centaur);
				if( CoC.getInstance().player.tallness < 48 && CoC.getInstance().player.lowerBody !== AppearanceDefs.LOWER_BODY_TYPE_CENTAUR ) {
					EngineCore.outputText( 'She looks you over, then bends down and drinks up as much of the semen floating in the water as she can find nearby.', false );
				} else {
					EngineCore.outputText( 'She leans over you and begins licking the semen off your body, not stopping until you\'re clean (if slightly sticky).', false );
				}
				EngineCore.outputText( '  Having fed, she grins mischievously and grabs your ' + Descriptors.cockDescript( 0 ) + ', then tows your floating body to the shoreline with it.  She rolls you onto the sand and then swims for deep water, vanishing.', false );
	...
	AnemoneScene.prototype.minoCumForAnemonieeeeez = function() {
		EngineCore.spriteSelect( 4 );
		EngineCore.outputText( 'The initial surprise subsides to wooly-headedness and a feeling of mild arousal as the stingers in her tentacles find exposed flesh.  In panic of drowning you pull free of the ropy mass and backpaddle away from the girl until your ' + CoC.getInstance().player.feet() + ' reassuringly touch the shallows of the lakebed.  As you shake your head to clear the haze, you notice a few of your items have fallen out of your pouches and are floating in the water.  The anemone has picked one in particular up and is examining it; a bottle of minotaur cum.  Her eyes light up in recognition as the fluid sloshes back and forth and she looks beseechingly at you, cradling it next to her cheek.  "<i>Gimme?</i>" she asks, trying to look as sweet as possible.\n\n', false );
		//[(PC not addicted);
		if( CoC.getInstance().flags[ kFLAGS.MINOTAUR_CUM_ADDICTION_STATE ] === 0 ) {
			EngineCore.outputText( 'Do you want to make a present of the bottle?', false );
		}
		//(PC addicted but sated);
		else if( CoC.getInstance().flags[ kFLAGS.MINOTAUR_CUM_ADDICTION_STATE ] === 1 ) {
			EngineCore.outputText( 'You\'re still riding high from your last dose; do you want to share your buzz with the girl? It might lead to something fun...', false );
		}
		//(PC addicted but in withdrawal);
		else {
			EngineCore.outputText( 'Oh hell no, you\'re not going to give that bottle away when you haven\'t even gotten your own fix yet! You raise your ' + CoC.getInstance().player.weaponName + ' and advance on the girl with a wild look in your eyes. She shivers a bit at your expression and drops the bottle with a splash, then recovers her composure and backs up a few steps. You grab the floating bottle, and the rest of your stuff, quickly.', false );
	...
	AnemoneScene.prototype.giveMino = function() {
		...
		EngineCore.outputText( 'You nod at the girl and she smiles and responds with a very quiet "<i>Yay.</i>"  As you pick up the rest of your stuff, she takes the top off of the bottle and chugs it like a champ, without even stopping to breathe.  Her eyes widen a bit as the drug hits her system, then narrow into a heavy-lidded stare.  Dropping the bottle with a splash, she falls to her knees with another.  She looks at you and licks her lips as she begins playing with her nipples. Obviously, she\'s feelin\' good.  ', false );
		//[(lust<30);
		if( CoC.getInstance().player.lust < 30 ) {
			EngineCore.outputText( 'Watching as her fondling devolves into outright masturbation, your own ', false );
			if( CoC.getInstance().player.cockTotal() > 0 ) {
				EngineCore.outputText( Descriptors.cockDescript( 0 ) + ' becomes a little erect', false );
			} else if( CoC.getInstance().player.hasVagina() ) {
				EngineCore.outputText( Descriptors.vaginaDescript( 0 ) + ' aches a bit with need', false );
			} else {
				EngineCore.outputText( Descriptors.assholeDescript() + ' begins to tingle with want', false );
			}
			EngineCore.outputText( '.  You shake off the feeling and head back to camp, leaving her to her fun.', false );
		}
		//(lust>30);
		else {
			...
			EngineCore.outputText( 'As her fondling devolves into genuine masturbation you realize this would be a good opportunity to take care of your own lusts.  If you do, how will you do it?', false );
			var cockRape = null;
			var vaginaRape = null;
			//Normal male: -requires dick of area < 36;
			if( CoC.getInstance().player.cockTotal() > 0 ) {
	...
	AnemoneScene.prototype.anemoneButtPlugginz = function() {
		...
		if( CoC.getInstance().monster.lust > 99 ) {
			EngineCore.outputText( ' as she plays with herself' );
		}
		EngineCore.outputText( '.  Nice, but not what you\'re looking for...  ' );
		if( !CoC.getInstance().player.isTaur() ) {
			EngineCore.outputText( 'Opening your [armor] a bit, you stroke ' );
			if( CoC.getInstance().player.hasCock() ) {
				EngineCore.outputText( '[oneCock]' );
			} else {
				EngineCore.outputText( 'your ' + Descriptors.vaginaDescript( 0 ) );
			}
			EngineCore.outputText( ' as y' );
		} else {
			EngineCore.outputText( 'Y' );
		}
		EngineCore.outputText( 'ou circle around behind her.  The anemone looks over her shoulder at you as you size her up.  There... that\'s what you wanted to see.  Tilting the girl forward with a hand on her shoulder, you lower yourself to get a better look at her backside.' );
		...
		if( CoC.getInstance().player.hasCock() && CoC.getInstance().player.cockThatFits( 48 ) >= 0 ) {
			if( !CoC.getInstance().player.isTaur() ) {
				EngineCore.outputText( 'you fish your [cockFit 48] out of your garments and rub it' );
			} else {
				EngineCore.outputText( 'you rub your [cockFit 48]' );
			}
			EngineCore.outputText( ' between the smooth blue curves' );
		} else {
			if( !CoC.getInstance().player.isTaur() ) {
				EngineCore.outputText( 'you finger your pussy' );
			} else {
				EngineCore.outputText( 'you imagine the tightness of her hole' );
			}
			EngineCore.outputText( ' until your ' + Descriptors.clitDescript() + ' starts to fill with blood and peeks from its hood' );
		}
		EngineCore.outputText( ', prompting a giggle from her' );
		if( CoC.getInstance().player.hasCock() && CoC.getInstance().player.cockThatFits( 48 ) >= 0 ) {
			EngineCore.outputText( ' as you smear a dribble of pre-cum into the crack of her ass' );
		}
		EngineCore.outputText( '.  Putting her hands down, she tries to angle her rear up to bring you in line with her pussy; you give her a harder smack and force her back down, much to her confusion.  The blue profile appears over her shoulder again, this time with a worried expression.  Squeezing her ass once with open palms, you lean down and plant a kiss on it. A nervous titter issues from the blue girl as you pull open her cheeks.' );
		...
		EngineCore.outputText( ' already drooping indolently toward the water.  She turns and looks at you out of the corner of her eye, her face a rictus of suspense.' );
	...
	AnemoneScene.prototype.anemoneQuoteUnquoteAnal = function() {
		EngineCore.clearOutput();
		var dick = (CoC.getInstance().player.cockThatFits( 48 ) >= 0 && CoC.getInstance().player.hasCock());
		var x = 0;
		EngineCore.outputText( ImageManager.showImage( 'anemone-bj' ), false );
		if( dick ) {
			x = CoC.getInstance().player.cockThatFits( 48 );
		}
		EngineCore.outputText( 'You\'re in the mood for anal and anal you shall have.  ' );
		if( !CoC.getInstance().player.isTaur() ) {
			EngineCore.outputText( 'Ever a purist, you stroke your ' );
			if( dick ) {
				EngineCore.outputText( Descriptors.cockDescript( x ) );
			} else {
				EngineCore.outputText( Descriptors.clitDescript() );
			}
			EngineCore.outputText( ' until it protrudes from the hole in your ' + CoC.getInstance().player.armorName + ', returned to full erectness.  ' );
		}
		EngineCore.outputText( 'The anemone regards you intently as you approach her and ' );
		if( !CoC.getInstance().player.isTaur() ) {
			EngineCore.outputText( 'seize her head' );
		} else {
			EngineCore.outputText( 'mount her head' );
			if( !dick ) {
				EngineCore.outputText( ', angling your clit forward along your stomach by pinning it against the anemone\'s forehead' );
			}
		}
		EngineCore.outputText( '; her hair delivers tingles of venom where it brushes your skin even as her own hands dawdle playfully along your ' + CoC.getInstance().player.skin() + '.  Without another word, you force her head into your groin as you ram your distended ' );
		if( !dick ) {
			EngineCore.outputText( 'chick-' );
		}
		EngineCore.outputText( 'prick into her eager face.' );
		...
		EngineCore.outputText( ' first with surprise, then with relish.  As you slide past her lips, your ' );
		if( dick ) {
			EngineCore.outputText( '[cockHead ' + (x + 1) + ']' );
		} else {
			EngineCore.outputText( 'tip' );
		}
		EngineCore.outputText( ' is embraced by the rippling walls of her throat, already trying to milk you for spunk' );
		if( !dick ) {
			EngineCore.outputText( ' that won\'t come' );
		}
		EngineCore.outputText( '.  The tightness seems to adjust to the ' );
		if( dick ) {
			EngineCore.outputText( 'girth of your prick' );
		} else {
			EngineCore.outputText( 'girl-th of your swollen button' );
		}
		EngineCore.outputText( '; the anemone looks up at you with ' );
		if( CoC.getInstance().player.isTaur() ) {
			EngineCore.outputText( 'unseen ' );
		}
		EngineCore.outputText( 'eyes a-twinkle' );
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( ', and her hair reaches forward to caress your [sack], delivering lancets of venom through the thin skin that send your arousal, and your production, into overdrive' );
			if( !CoC.getInstance().player.hasCock() ) {
				EngineCore.outputText( '.  As your [balls] swell with blood and seed, you can\'t help but groan; there\'ll be nowhere for the largess to go, and it will be with you until your body reabsorbs it - or you make an outlet' );
			}
		}
		EngineCore.outputText( '.  Your hips take over, thrusting into her brutally and knocking her head back and forth.  Her tentacles fly wildly, brushing all along your stomach and hips as you pound her mouth, leaving little stripes of heat on your ' + CoC.getInstance().player.skin() + ' that seep into your body and only make you want to come even more.' );
		...
		EngineCore.outputText( ' suddenly, stroking vigorously even as it smears a burning wave of arousal along your length.  "<i>F-fuck,</i>" you moan, pushed beyond your control by the coup.' );
		if( dick ) {
			EngineCore.outputText( '  Your [cockFit 48] begins to ejaculate, filling the blue girl\'s mouth with your seed; she sucks it down greedily, swallowing every drop.' );
			if( CoC.getInstance().player.cumQ() >= 1000 ) {
				EngineCore.outputText( '  You push so much cum into her that her belly actually rounds from the volume, transforming from a sleek, flat midriff into a barely-contained ball of sloshing liquid' );
			}
			if( CoC.getInstance().player.cumQ() >= 2000 ) {
				EngineCore.outputText( '; even when her stomach skin can stretch no further, your body won\'t stop filling her, and the bulge expands up her esophagus, pushing out her chest and throat until she\'s full to the brim and rather reminiscent of a big, blue pear' );
			}
			if( CoC.getInstance().player.cumQ() >= 1000 ) {
				EngineCore.outputText( '.' );
			}
			...
			if( CoC.getInstance().player.cumQ() >= 1000 ) {
				EngineCore.outputText( 'bobbing roundly in the water as she tries to make her way from the shallows, ' );
			}
			EngineCore.outputText( 'and head back to camp.' );
		} else {
			EngineCore.outputText( '  Your clit, packed with nerves, shivers and sets off your orgasm as she strokes, and you bury it into her throat as your head rolls back.  The anemone flinches, unnoticed by you, as your vagina ' );
			if( CoC.getInstance().player.wetness() < 4 ) {
				EngineCore.outputText( 'drools its lube along your length.  ' );
			} else {
				EngineCore.outputText( 'squirts obscenely, coating your length and her face with your female orgasm.  ' );
			}
			EngineCore.outputText( 'Your body shivers as near-painful sensations wrack you, engulfed in the anemone\'s questing passage' );
			if( CoC.getInstance().player.balls > 0 ) {
				EngineCore.outputText( '.  Your lover, hopeful, watches and waits for your testicles to rise into your groin and unload their cargo' );
			}
			if( CoC.getInstance().player.cockTotal() === 0 ) {
				EngineCore.outputText( '; she even tries to push them up herself with her hands, as if that would make your clit produce the semen inside them' );
			}
			EngineCore.outputText( '.' );
			if( CoC.getInstance().player.hasCock() ) {
				EngineCore.outputText( '  Your clit remains unproductive; your male orgasm squirts out uselessly around her head, mingling with the lake water.  The girl\'s expression upon seeing this is pained, and she tries but fails to get free to follow the arcing white ropes, sending further spasms of pleasure through your sensitive distaff staff.' );
			}
			EngineCore.outputText( '  Eventually your body calms enough to respond to your impulses, and you carefully draw your [clit] out of your lover\'s throat.' );
			EngineCore.outputText( '\n\n"<i>No food,</i>" she whines, pouting.  Negligent and uncaring, you shake your head' );
			if( CoC.getInstance().player.hasCock() ) {
				EngineCore.outputText( ', pointing to the squiggles of your seed floating lazily in the lakewater,' );
			}
			EngineCore.outputText( ' and leave her.' );
		}
	...
	AnemoneScene.prototype.hotdogTheAnemone = function() {
		...
		EngineCore.outputText( 'Well... it\'s the spirit of the thing that counts, right?  That blue butt still does look pretty tempting.  You force the anemone forward again and squeeze them together; she giggles and tries once more to push her vagina toward you, but you push it down again and jam your ' );
		if( dick ) {
			EngineCore.outputText( '[cockFit 48]' );
		} else {
			EngineCore.outputText( '[clit]' );
		}
		EngineCore.outputText( ' between her small, round cheeks, seizing one in each hand and forcing them to sleeve as much of your length as possible.  The girl looks back at you, her face a picture of confusion, but you do not even care.  Her cool ass feels otherworldly as you thrust through it, poking your tip out of the crack and then pulling back again; ' );
		if( dick ) {
			EngineCore.outputText( 'pre-cum' );
		} else {
			EngineCore.outputText( 'your juice' );
		}
		EngineCore.outputText( ' drools onto her at one end of your motion, filling your little canyon of fun with a river of hot lubrication that you smear liberally throughout.' );
		EngineCore.outputText( '\n\n"<i>Hey,</i>" you call, "<i>you could be helping.  Use your tentacles or something.</i>"' );
		EngineCore.outputText( '\n\nThe anemone, ' );
		if( dick ) {
			EngineCore.outputText( 'excited by the feel and sight of your pre-cum' );
		} else {
			EngineCore.outputText( 'looking almost bored' );
		}
		EngineCore.outputText( ', obliges and snakes several toward you; they stop just inside the range of your furthest stroke, flitting tantalizingly as if begging you to push into them.  Wary at first, you do so and they gently caress ' );
		if( dick ) {
			EngineCore.outputText( 'your [cockHead ' + (x + 1) + ']' );
		} else {
			EngineCore.outputText( 'the nerve-packed end of your [clit]' );
		}
		EngineCore.outputText( ', responding almost autonomously to wrap around it.  The grip is not enough to keep you from pulling back out, though, and they relinquish your shaft, leaving the tip covered in a warm lattice of venom that soaks through you.  A shudder wracks you, and you eagerly push in again, sliding first into her cheeks, then down the slick passageway made by your lube, and finally into the body-heating embrace of her tentacles.  As your ' );
		if( dick ) {
			EngineCore.outputText( 'now-throbbing prick' );
		} else {
			EngineCore.outputText( 'swollen, red clit' );
		}
		EngineCore.outputText( ' wicks more of her venom into you, your hips begin to disobey your will, thrusting faster with each dose and pushing the anemone\'s face into the lakebed; elbows crooked and fingers splayed, she still can\'t raise her head against the vigor of your onslaught.' );
		...
		EngineCore.outputText( ', you come.  Your body shakes and you nearly fall atop your lover; ' );
		if( dick ) {
			EngineCore.outputText( 'your swollen, painfully hard prick fountains with cum, coating the anemone\'s back and hair in white.  The venom in your system draws out your orgasm to incredible lengths, and ' + Descriptors.sMultiCockDesc() + ' ejaculates over and over until you feel lightheaded and woozy.' );
			if( CoC.getInstance().player.cumQ() >= 500 ) {
				EngineCore.outputText( '  When you\'ve finished, not a single place on the anemone\'s backside is still blue; the giddy girl is shoving your semen into her mouth with her tentacles and hands, swallowing almost as much lakewater as jizz.' );
			}
		} else {
			EngineCore.outputText( 'your pussy clamps down, trying and failing to find something to squeeze and ' );
			if( CoC.getInstance().player.wetness() < 4 ) {
				EngineCore.outputText( 'drooling its lube down your thighs.' );
			} else {
				EngineCore.outputText( 'squirting cum behind you to rain onto your partner\'s calves and the surface of the lake.' );
			}
			EngineCore.outputText( '  The anemone sighs impatiently as you twitch atop her, waiting for you to finish.' );
		}
		...
		if( dick ) {
			EngineCore.outputText( 'dares not move from her knees, balancing your freshly-shot load on her back as she tries to push it toward her face with her tentacles.' );
		} else {
			EngineCore.outputText( 'looks indolently at you as you go.' );
		}
	...
	AnemoneScene.prototype.anemoneGetsLayedByBeePositor = function() {
		...
		if( EngineCore.silly() ) {
			EngineCore.outputText( 'duck ' );
		}
		EngineCore.outputText( 'face,</i>" you reply.' );
		...
		//[(sens>=50 or horse);
		if( CoC.getInstance().player.sens >= 50 || CoC.getInstance().player.isTaur() ) {
			...
			if( CoC.getInstance().player.hasCock() ) {
				...
				if( CoC.getInstance().player.totalCocks() === 1 ) {
					EngineCore.outputText( 'male part erupts' );
				} else {
					EngineCore.outputText( 'male parts erupt' );
				}
				EngineCore.outputText( ' in sympathy, lancing semen over your partner.  Surprisingly, the anemone doesn\'t even notice, so absorbed is she in experimenting with your modified stinger.' );
			}
			//[(Lev3 eggs);
			if( CoC.getInstance().player.eggs() > 40 ) {
				EngineCore.outputText( '  Eventually there are so many eggs ' );
				if( CoC.getInstance().player.hasCock() && CoC.getInstance().player.cumQ() > 1000 ) {
					EngineCore.outputText( 'and so much cum ' );
				}
				EngineCore.outputText( 'that they begin to collect around her, clumping and layering around her midriff like white, slimy algae on a pier post.' );
			}
			EngineCore.outputText( '  As the last rolls from you, the anemone raises your wilting ovipositor to her lips once again, eagerly drinking the final sweet secretions that fall out in long, slimy strings.' );
			EngineCore.outputText( '\n\n"<i>Weird,</i>" she repeats, tugging at the shriveling organ as it tries to recede into your slit.' );
			EngineCore.outputText( '"<i>Ugh...</i>" you respond, weak in the [legs].  With a jerk, you pull your abdomen away from her grasp, flinching as her skin rubs your sensitive slit, then turn to leave.  She catches your hand, looking almost concerned.' );
			...
			if( CoC.getInstance().player.eggs() < 20 ) {
				EngineCore.outputText( 'It doesn\'t take long before you finish, pushing your cargo down her passageway in a neat, orderly line.' );
			} else {
				EngineCore.outputText( 'So many come that you can see them under her skin, a myriad of tiny bumps' );
				if( CoC.getInstance().player.eggs() >= 40 ) {
					EngineCore.outputText( '; these same bumps multiply upward as your long-suffering abdomen pushes out line upon line of eggs, and soon you can feel them pressing back against the tip of your ovipositor.  You squeeze the girl\'s head in your hands, holding her against the base, and concentrate; slowly, the deposited eggs give way to their siblings, stretching her elastic stomach and chest wide' );
				}
				EngineCore.outputText( '.' );
			}
			...
			if( EngineCore.silly() && CoC.getInstance().player.fertilizedEggs() > 1 ) {
				EngineCore.outputText( '  You briefly amuse yourself imagining her carrying your eggs to term and having them hatch in her mouth, so that when she talks, she shoots bees.  Nicholas Cage would be proud.' );
			}
			EngineCore.outputText( '  Gathering your things, you ' + CoC.getInstance().player.mf( 'laugh', 'giggle' ) + ' at her and depart.' );
		}
	...
	AnemoneScene.prototype.spiderOvipositAnAnemone = function() {
		...
		if( CoC.getInstance().player.HP < 1 ) {
			EngineCore.outputText( 'eyes unfocused with fatigue' );
		} else {
			EngineCore.outputText( 'two fingers plunging eagerly into her pussy' );
		}
		EngineCore.outputText( ', you advance on her.  Your abdomen bends forward and your egg-laying organ slides out, dripping slime into the water with little \'plit\' noises; the girl, fascinated despite the strangeness of it, sits up and creeps forward to touch it.' );
		...
		//[(cock);
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( ', and your [armor] tightens as [eachCock] swells' );
		} else if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( ', and the inside of your [armor] dampens with the lube your [vagina] leaves as it clenches around empty air' );
		}
		EngineCore.outputText( '; you have to push yourself to finish working before you can lose yourself to it.  Completing your restraints, you release her tentacles.  They bob behind her in one mass like a long, cute ponytail, and only the ends are free to wiggle.  When she realizes this, her behavior changes dramatically.' );
		...
		if( CoC.getInstance().player.cor < 60 ) {
			EngineCore.outputText( ', even though her adorable and slightly surreal puppy dog eyes implore you otherwise' );
		}
		EngineCore.outputText( '.  "<i>Let me just finish what I need to do first.</i>"' );
		...
		if( CoC.getInstance().player.gender > 0 ) {
			EngineCore.outputText( 'go for your ' );
			if( CoC.getInstance().player.hasCock() ) {
				EngineCore.outputText( 'cock' );
			} else {
		...
		EngineCore.outputText( ' you holding the bag... of eggs, as it were.  Your steel your resolve and stride atop her, pinning the loose, wiggling end of her ponytail harmlessly against your chitinous underside and forcing her hands underneath her by lowering some of your weight onto her back.' );
		...
		//[(sexed);
		if( CoC.getInstance().player.gender > 0 ) {
			EngineCore.outputText( 'while unfastening your [armor] with one hand, ' );
		}
		EngineCore.outputText( '"<i>there is definitely more.</i>"  The next two eggs slip into her as you speak, sending convulsions through her body.  Her pussy spasms again and enfolds your ovipositor even more tightly; you\'re ready for the sensation this time, and allow it to resonate through you, forcing out your own wet orgasm.  You hold her face as you unburden yourself' );
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( ' and cover her hair with yet more white strings from your twitching manhood' );
			if( CoC.getInstance().player.cockTotal() > 1 ) {
				EngineCore.outputText( 's' );
			}
		}
		//(egg level 3);
		if( CoC.getInstance().player.eggs() >= 40 ) {
			EngineCore.outputText( '; so many eggs pump into her that she gives a little start when her distended belly touches the lukewarm water below' );
		}
		EngineCore.outputText( '... finally you let her go when you\'re completely empty, pulling your stalk from her with a lewd sucking noise.  A little bit of green goo drools from her pussy as she slumps over on her side, and you make ready to leave her there - bloated and pregnant, with a squiggle of her semen floating in the water next to her.' );
	...
	AnemoneScene.prototype.anemoneKidBirthPtII = function() {
		...
		EngineCore.outputText( 'You awake and look skyward to the sun for a hint at the time.  What greets you is more of an eclipse; a shape impedes your view.  As your eyes adjust to the light, it resolves into an upside-down blue-eyed, blue-skinned face wreathed with snubby, shoulder-length tentacles of purple and green hue.  The silence continues as you stare into it, until you move to push yourself off the hard ground.  At the first sign of activity, the head disappears into the water barrel with a sloshing sound.  You push yourself to your ' + CoC.getInstance().player.feet() + ' and look back toward it; the eyes, now right side-up, peek over the rim bashfully atop a set of blue fingers.' );
		...
		if( CoC.getInstance().player.tallness > 48 ) {
			EngineCore.outputText( ' even as the face angles up to maintain eye contact' );
		}
		EngineCore.outputText( '.  Peering into the barrel, you can make out a little, though most of her décolletage is obscured by a pair of gangly knees.  The previously-noted head sits atop a pair of narrow, thin shoulders bearing equally wiry-looking arms.  She raises one hand and gives you a little finger wave.' );
	...
	AnemoneScene.prototype.anemoneBarrelDescription = function() {
		if( CoC.getInstance().time.hours < 6 ) {
			//(morning)
			EngineCore.outputText( 'Kid A is sleeping in her barrel right now.' );
		}
		else if( CoC.getInstance().time.hours <= 10 ) {
			EngineCore.outputText( 'Kid A stands next to her barrel, refilling it from one of your waterskins.  A second full skin is slung over her shoulder.  She gives you a grin.\n\n' );
		} else if( CoC.getInstance().flags[ kFLAGS.KID_SITTER ] > 1 ) {
			EngineCore.outputText( 'Kid A is absent from her barrel right now, dragooned into babysitting again.\n\n' );
		} else if( CoC.getInstance().time.hours < 16 ) {
			//(midday)
			EngineCore.outputText( 'Kid A is deep in her barrel with the lid on top, hiding from the midday sun.\n\n' );
		} else if( CoC.getInstance().time.hours < 22 ) {
			//(night hours)
			EngineCore.outputText( 'Kid A is peeking out of her barrel.  Whenever you make eye contact she breaks into a smile; otherwise she just stares off into the distance, relaxing.\n\n' );
		} else {
			EngineCore.outputText( 'Kid A is here, seated demurely on the rim of her barrel and looking somewhat more purple under the red moon.  She glances slyly at you from time to time.\n\n' );
		}
	};
	...
	AnemoneScene.prototype.approachAnemoneBarrel = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.KID_ITEM_FIND_HOURS ] >= 16 ) {
			EngineCore.outputText( 'An item sits next to it, left there by the anemone as a present to you.  Or \'rent\', if you choose to think of it that way.  ' );
			item = this.getAnemoneItem;
		}
		//[(if Kid A has been given a weapon);
		if( CoC.getInstance().flags[ kFLAGS.ANEMONE_WEAPON_ID ] !== 0 ) {
			EngineCore.outputText( 'She has ' + ItemType.lookupItem( CoC.getInstance().flags[ kFLAGS.ANEMONE_WEAPON_ID ] ).longName + ' sitting next to it.  ' );
			//EngineCore.outputText('Kid A pokes her head out and smiles at you.  What do you need from her?');;
			weaponT = 'Take Weapon';
			weaponB = this.takeOutOfAnemone;
		}
		//(if babysitting);
		if( CoC.getInstance().flags[ kFLAGS.KID_SITTER ] > 1 ) {
			EngineCore.outputText( 'Kid A is away right now, helping season some of Marble\'s children.  If you wanted to leave or take an item, she would discover it when she got back.  ' );
		} else if( this.kidAXP() < 25 ) {
			EngineCore.outputText( 'Kid A sinks below the rim, peering nervously at you.  ' );
		} else if( this.kidAXP() < 75 ) {
			EngineCore.outputText( 'Kid A pokes her head out and smiles at you.  ' );
		} else {
			EngineCore.outputText( 'Kid A leans her head and shoulders out of the barrel and puts her elbows on the rim, staring unabashedly at you as she rests her chin on her hands.  ' );
		}
		EngineCore.outputText( 'What do you need from her?' );
		//[(if N.Watch is toggled on);
		if( CoC.getInstance().flags[ kFLAGS.ANEMONE_WATCH ] > 0 ) {
		...
		if( CoC.getInstance().flags[ kFLAGS.KID_SITTER ] <= 1 ) {
			if( CoC.getInstance().flags[ kFLAGS.ANEMONE_WEAPON_ID ] !== 0 && CoC.getInstance().player.fatigue <= 90 ) {
				EngineCore.addButton( 3, 'Tutor', this.tutorAnemoneKid );
			} else if( CoC.getInstance().player.fatigue > 90 ) {
			...
			var sex = false;
			if( CoC.getInstance().flags[ kFLAGS.HAD_KID_A_DREAM ] > 0 && CoC.getInstance().flags[ kFLAGS.ANEMONE_KID ] >= 2 ) {
				if( this.kidAXP() >= 40 ) {
					if( CoC.getInstance().player.lust >= 33 ) {
						if( CoC.getInstance().player.hasCock() ) {
							if( CoC.getInstance().player.cockThatFits( 60 ) >= 0 ) {
								sex = true;
							} else {
	...
	AnemoneScene.prototype.getAnemoneItem = function() {
		...
		var choice;
		var itype;
		EngineCore.outputText( 'You reach down and pick up her present.  Today, she\'s left you ' );
		if( this.kidAXP() === 0 ) {
		...
		EngineCore.outputText( itype.longName + '.' );
	...
	AnemoneScene.prototype.giveAnemoneWeapon = function() {
		...
		var foundItem = false;
		for( var x = 0; x < 5; x++ ) {
			if( CoC.getInstance().player.itemSlots[ x ].quantity > 0 && giveableToAnemone( CoC.getInstance().player.itemSlots[ x ].itype ) ) {
				EngineCore.addButton( x, CoC.getInstance().player.itemSlots[ x ].itype.shortName + ' x' + CoC.getInstance().player.itemSlots[ x ].quantity, this.placeInAnemone, x );
				foundItem = true;
			}
		}
		if( !foundItem ) {
	...
	AnemoneScene.prototype.placeInAnemone = function( slot ) {
		...
		CoC.getInstance().flags[ kFLAGS.ANEMONE_WEAPON_ID ] = CoC.getInstance().player.itemSlots[ slot ].itype.id;
	...
	AnemoneScene.prototype.takeOutOfAnemone = function() {
		...
		var itype = ItemType.lookupItem( CoC.getInstance().flags[ kFLAGS.ANEMONE_WEAPON_ID ] );
		if( CoC.getInstance().flags[ kFLAGS.ANEMONE_WATCH ] > 0 ) {
	...
	AnemoneScene.prototype.anemoneWatchToggle = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.ANEMONE_WATCH ] > 0 ) {
			EngineCore.outputText( 'You tell Kid A not to worry about guarding at night.  She nods in your direction.' );
			CoC.getInstance().flags[ kFLAGS.ANEMONE_WATCH ] = 0;
		} else {
			if( CoC.getInstance().flags[ kFLAGS.ANEMONE_WEAPON_ID ] === 0 ) {
				EngineCore.outputText( 'You\'re not really going to set this featherweight against the night barehanded, are you?!  Her hair hasn\'t even grown out!  You\'d better hand her some kind of weapon.' );
			} else {
				EngineCore.outputText( 'Kid A smiles and cheers, "<i>Guard!</i>"' );
				CoC.getInstance().flags[ kFLAGS.ANEMONE_WATCH ] = 1;
			}
		}
		EngineCore.doNext( this.approachAnemoneBarrel );
	};
	...
	AnemoneScene.prototype.tutorAnemoneKid = function() {
		...
		//[(KidXP < 33)];
		if( this.kidAXP() < 33 ) {
			EngineCore.outputText( 'holding ' + ItemType.lookupItem( CoC.getInstance().flags[ kFLAGS.ANEMONE_WEAPON_ID ] ).longName + ' protectively across her chest.' );
		} else {
			EngineCore.outputText( 'taking up an attentive stance with ' + ItemType.lookupItem( CoC.getInstance().flags[ kFLAGS.ANEMONE_WEAPON_ID ] ).longName + ' in her hands.' );
		}

		EngineCore.outputText( '  You spend some time instructing Kid A in the finer points of the equipment you\'ve provided her with, and then finish up with a practice duel.' );
		//duel effects by weapon, output in new PG;
		//[Pipe] or [Wizard Staff] or [Eldritch Staff];
		if( CoC.getInstance().flags[ kFLAGS.ANEMONE_WEAPON_ID ] === WeaponLib.PIPE.id ||
			CoC.getInstance().flags[ kFLAGS.ANEMONE_WEAPON_ID ] === WeaponLib.W_STAFF.id ||
			CoC.getInstance().flags[ kFLAGS.ANEMONE_WEAPON_ID ] === WeaponLib.E_STAFF.id ) {
			EngineCore.outputText( '\n\nThough she acts like she\'s not serious and pulls her swings more often than not, the heft of the ' );
			if( CoC.getInstance().flags[ kFLAGS.ANEMONE_WEAPON_ID ] === WeaponLib.PIPE.id ) {
				EngineCore.outputText( 'pipe' );
			} else {
				EngineCore.outputText( 'stick' );
			}
			EngineCore.outputText( ' is still enough to bruise you a bit.' );
		}
		//(HP - 5, KidXP + 1);
		//[Riding Crop];
		else if( CoC.getInstance().flags[ kFLAGS.ANEMONE_WEAPON_ID ] === WeaponLib.RIDINGC.id ) {
			...
			if( CoC.getInstance().player.lib < 50 ) {
				...
				EngineCore.outputText( 'isn\'t' );
			}
			EngineCore.outputText( ' enough to keep you from thinking dirty thoughts about grabbing her naughty, teasing face and mashing it into your crotch.' );
		...
		//[Lust Dagger];
		else if( CoC.getInstance().flags[ kFLAGS.ANEMONE_WEAPON_ID ] === WeaponLib.L_DAGGR.id ) {
		...
		//[Beautiful Sword];
		else if( CoC.getInstance().flags[ kFLAGS.ANEMONE_WEAPON_ID ] === WeaponLib.B_SWORD.id ) {
		...
		else if( CoC.getInstance().flags[ kFLAGS.ANEMONE_WEAPON_ID ] === WeaponLib.RRAPIER.id ||
			CoC.getInstance().flags[ kFLAGS.ANEMONE_WEAPON_ID ] === WeaponLib.JRAPIER.id ) {
		...
		else if( CoC.getInstance().flags[ kFLAGS.ANEMONE_WEAPON_ID ] === WeaponLib.L__AXE.id ||
			CoC.getInstance().flags[ kFLAGS.ANEMONE_WEAPON_ID ] === WeaponLib.L_HAMMR.id ||
			CoC.getInstance().flags[ kFLAGS.ANEMONE_WEAPON_ID ] === WeaponLib.WARHAMR.id ||
			CoC.getInstance().flags[ kFLAGS.ANEMONE_WEAPON_ID ] === WeaponLib.L__AXE.id ) {
		...
		else if( CoC.getInstance().flags[ kFLAGS.ANEMONE_WEAPON_ID ] === WeaponLib.KATANA.id ||
			CoC.getInstance().flags[ kFLAGS.ANEMONE_WEAPON_ID ] === WeaponLib.S_BLADE.id ) {
		...
		//[Spear];
		else if( CoC.getInstance().flags[ kFLAGS.ANEMONE_WEAPON_ID ] === WeaponLib.SPEAR.id ) {
		...
		//[Whip] or [Succubi's Whip];
		else if( CoC.getInstance().flags[ kFLAGS.ANEMONE_WEAPON_ID ] === WeaponLib.WHIP.id ||
			CoC.getInstance().flags[ kFLAGS.ANEMONE_WEAPON_ID ] === WeaponLib.SUCWHIP.id ) {
			...
			if( CoC.getInstance().flags[ kFLAGS.ANEMONE_WEAPON_ID ] === WeaponLib.SUCWHIP.id ) {
				EngineCore.outputText( '  The demonic enchantment chooses then to activate, and her color deepens as her lust peaks, as does your own.' );
			}
			EngineCore.outputText( '  You feel a point digging into your groin as her prick hardens and her struggles cease; she begins to moan openly in arousal.  As she relaxes, the coils of the whip finally loosen enough for you to extricate yourself.' );
		...
		else if( CoC.getInstance().flags[ kFLAGS.ANEMONE_WEAPON_ID ] === WeaponLib.S_GAUNT.id ||
			CoC.getInstance().flags[ kFLAGS.ANEMONE_WEAPON_ID ] === WeaponLib.H_GAUNT.id ) {
		...
		//[Wingstick];
		else if( CoC.getInstance().flags[ kFLAGS.ANEMONE_WEAPON_ID ] === ConsumableLib.W_STICK.id ) {
		...
		//[Dragonshell Shield];
		else if( CoC.getInstance().flags[ kFLAGS.ANEMONE_WEAPON_ID ] === WeaponLib.DRGNSHL.id ) {
		...
		//[White Book];
		else if( CoC.getInstance().flags[ kFLAGS.ANEMONE_WEAPON_ID ] === ConsumableLib.W__BOOK.id ) {
		...
		//[Black Book];
		else if( CoC.getInstance().flags[ kFLAGS.ANEMONE_WEAPON_ID ] === ConsumableLib.B__BOOK.id ) {
			...
			if( CoC.getInstance().player.hasCock() ) {
				EngineCore.outputText( 'tighten' );
			} else if( CoC.getInstance().player.hasVagina() ) {
				EngineCore.outputText( 'dampen' );
			} else {
				EngineCore.outputText( 'become a hindrance' );
			}
			EngineCore.outputText( '... though the part between her own legs is still pointed at you.' );
			//(lust + 10, KidXP + 2);
			EngineCore.dynStats( 'lus', 20 );
			this.kidAXP( 4 );
		}
		//[Scarred Blade](NYI);
			...
			if( CoC.getInstance().player.cor <= 70 ) {
				EngineCore.outputText( '  The blade\'s edge flashes toward you as well, when you try to pick it up.  After a few frustrated attempts, it becomes clear that you\'ll have to abandon it for now.' );
			}
		...
		//[Any new weapon added without text written for it, or any custom item name set by a save editor];
			...
			EngineCore.outputText( '\n\nFor the life of her, Kid A can\'t seem to grasp how to use the ' + ItemType.lookupItem( CoC.getInstance().flags[ kFLAGS.ANEMONE_WEAPON_ID ] ).longName + ' you\'ve provided her with.  You have to interrupt the practice for explanations so many times that you don\'t actually get to do any sparring.' );
		...
		if( CoC.getInstance().player.HP < 1 ) {
			...
			if( CoC.getInstance().flags[ kFLAGS.ANEMONE_WEAPON_ID ] !== WeaponLib.S_GAUNT.id &&
				CoC.getInstance().flags[ kFLAGS.ANEMONE_WEAPON_ID ] !== WeaponLib.H_GAUNT.id ) {
				EngineCore.outputText( 'the thump of something hitting the ground and ' );
			}
			EngineCore.outputText( 'the anemone gasp, and then the world slips away from you.' );
		...
		if( CoC.getInstance().flags[ kFLAGS.HAD_KID_A_DREAM ] > 0 && this.kidAXP() >= 40 && CoC.getInstance().player.lust > 99 ) {
			if( this.kidASex() ) {
				return;
			}//nothing fits;
			//if KidXP >= 40 and lust > 99 after tutor and PC has only huge dicks of area >= 60 or hasn't got shit;
			else if( this.kidAXP() >= 40 && CoC.getInstance().player.lust > 99 ) {
				...
				if( CoC.getInstance().player.hasCock() ) {
					EngineCore.outputText( 'something too enormous' );
				} else {
					EngineCore.outputText( 'nothing useful' );
				}
	...
	AnemoneScene.prototype.kidASex = function( cont ) {
		...
		if( CoC.getInstance().flags[ kFLAGS.ANEMONE_KID ] === 2 && CoC.getInstance().player.hasCock() && CoC.getInstance().player.cockThatFits( 36 ) >= 0 ) {
			x = CoC.getInstance().player.cockThatFits( 36 );
			EngineCore.outputText( '\n\n' );
			EngineCore.outputText( ImageManager.showImage( 'anemone-kid-male-vagfuck' ), false );
			EngineCore.outputText( 'You collapse onto your back, panting your arousal into the dry air.  Shyly at first but with increasing confidence as you fail to react, your daughter slips a hand into your clothes and down to your crotch.  She bites her lip and blushes as her hand reaches the neck of your ' + Appearance.cockNoun( CoC.getInstance().player.cocks[ x ].cockType ) + ', then her resolve appears to crystallize as she yanks it free from your ' + CoC.getInstance().player.armorName + '. At first, nothing happens and your erect cock just bobs through the empty air, but then you feel soft fingertips guiding it and a warm, tight squeeze at the end of your dick.  Raising your head in surprise, you see the anemone has mounted you!  She blushes and looks away at the eye contact, but her mouth opens in a sly grin as she begins to shift her hips, sliding your ' + Descriptors.cockDescript( x ) + ' deeper into herself.' );
			...
			//[(big skeet)];
			if( CoC.getInstance().player.cumQ() >= 1000 ) {
				EngineCore.outputText( '  As you pour into her, the sensation of being stuffed with semen sets off a second orgasm in the quivering girl, and her own prick drools weakly onto your stomach' );
			}
			if( CoC.getInstance().player.cumQ() >= 2000 ) {
				EngineCore.outputText( '; even a second orgasm from your daughter isn\'t enough to match one of yours in length.  Your magnificently productive ' + Appearance.cockNoun( CoC.getInstance().player.cocks[ x ].cockType ) + ' fills her until her belly is inflated so far that she can\'t contain more, and it begins to breach the seal her pussy makes around you, even as tight as it is, to squirt out in lazy globs as you deposit the last of your spunk' );
			}
			if( CoC.getInstance().player.cumQ() >= 1000 ) {
				EngineCore.outputText( '.' );
			}
			//[(multi)];
			if( CoC.getInstance().player.cockTotal() > 1 ) {
				EngineCore.outputText( '  The loads from your other shaft' );
				if( CoC.getInstance().player.cockTotal() > 2 ) {
					EngineCore.outputText( 's' );
				}
				EngineCore.outputText( ' ooze down your [legs], directed there by her weight.' );
			}
		...
		//if KidXP >= 40 and lust > 99 after tutor and PC has wang of area <60, set that cock to x variable, also set biggest cock not yet used to y variable if multicocked;
		else if( CoC.getInstance().player.hasCock() && CoC.getInstance().player.cockThatFits( 60 ) >= 0 ) {
			x = CoC.getInstance().player.cockThatFits( 60 );
			y = -1;
			var temp = 0;
			while( temp < CoC.getInstance().player.cockTotal() ) {
				if( temp !== x ) {
					y = temp;
					break;
				}
			...
			EngineCore.outputText( '\n\n' );
			EngineCore.outputText( ImageManager.showImage( 'anemone-kid-male-bj' ), false );
			EngineCore.outputText( 'You collapse onto your back, panting your arousal into the dry air.  Shyly at first but with increasing confidence as you fail to react, your daughter slips a hand into your clothes and down to your crotch.  She bites her lip and blushes as her hand reaches the neck of your ' + Appearance.cockNoun( CoC.getInstance().player.cocks[ x ].cockType ) + ', then her resolve appears to crystallize as she yanks it free from your ' + CoC.getInstance().player.armorName + '.  A sudden wetness at the end of your dick raises your head in surprise; the mop-topped girl is kneeling over you with the crown in her mouth and, as you make eye contact, turns the deepest shade of blue you\'ve ever seen on her.' );
			...
			if( y >= 0 || CoC.getInstance().player.balls > 0 || CoC.getInstance().player.hasVagina() ) {
				EngineCore.outputText( '\n\nYou feel her bashful fingers sneak into your ' + CoC.getInstance().player.armorName + ' again, and they quickly locate your ' );
				if( y >= 0 ) {
					EngineCore.outputText( Descriptors.cockDescript( y ) );
				...
				//[(if cock or sack)];
				if( y >= 0 || CoC.getInstance().player.balls > 0 ) {
					EngineCore.outputText( ', pulling it free from your armor as well' );
				}
				EngineCore.outputText( '.  Her dextrous digits are emboldened as your head lolls in the dirt, staring skyward, and quickly begin to caress their find.' );
				//[(cock);
				if( y >= 0 ) {
					EngineCore.outputText( '  Another hand quickly joins the first, stroking the length of your second shaft as it ' );
					if( CoC.getInstance().player.cocks[ y ].cockLength >= 36 ) {
						EngineCore.outputText( 'towers over her head' );
					} else {
						EngineCore.outputText( 'quivers and twitches into her venomous locks' );
					}
					EngineCore.outputText( '.  Every time it brushes her hair, another jolt shoots to the base of your spine and both of your cocks force out another glob of precum which is quickly slurped up.' );
				}
				//[(sac);
				else if( CoC.getInstance().player.balls > 0 ) {
					EngineCore.outputText( '  One hand cups below your ' + Descriptors.ballsDescript() + ' and lifts them up to her chin, where the other, formerly tracing gentle circles along your shaft, grabs a tentacle from her head and begins to tickle.' );
				}
				//[(vag);
				else if( CoC.getInstance().player.hasVagina() ) {
					EngineCore.outputText( '  Her fingers part your labia and reveal your ' + Descriptors.clitDescript() + ', ' );
					if( CoC.getInstance().player.clitLength >= 6 ) {
						EngineCore.outputText( 'working it free as well and allowing her to tease and stroke it, cock-like, with her hands and hair.' );
					} else {
						EngineCore.outputText( 'in order to give the sensitive button their attention.' );
					}
					EngineCore.outputText( '  Her other hand slips into your garments after the first, following it down to your groin and slipping into your pussy to pump back and forth gently.' );
				}
			}
			EngineCore.outputText( '\n\nHer attentions on your groin reach a peak as ' );
			if( CoC.getInstance().player.balls > 0 ) {
				EngineCore.outputText( 'your balls tighten up' );
			} else {
				EngineCore.outputText( 'your body tenses' );
			}
			EngineCore.outputText( '; seeing this, she dives down onto your shaft, forcing the head past the ginger care of lips and tongue into her welcoming throat.  Her hair drapes all around your groin, touching exposed flesh as she shakes her head and undulates her throat to coax out your climax.  Your body reacts before you can, and your hips thrust up into her face as your ejaculate erupts.' );
			//[(if multicock or sac)];
			if( CoC.getInstance().player.balls > 0 || y >= 0 ) {
				EngineCore.outputText( '  As her eyes twinkle, the hands on your ' );
				if( y >= 0 ) {
					EngineCore.outputText( Descriptors.cockDescript( y ) );
				} else {
					EngineCore.outputText( Descriptors.sackDescript() );
				}
				EngineCore.outputText( ' renew their efforts, squeezing out squirts of semen with measured strokes and squeezes.' );
			}
			//[(vag);
			else if( CoC.getInstance().player.hasVagina() ) {
				EngineCore.outputText( '  As her eyes twinkle, a hand plunges all the way into your pussy, and your lonely muscles clamp down reflexively on the invader, attempting to wring her fingers.' );
			}
			EngineCore.outputText( '  She swallows greedily as you unload into her throat' );
			if( y >= 0 ) {
				EngineCore.outputText( ' and onto her hair' );
			}
			EngineCore.outputText( ', the oral contractions squeezing the head of your penis and setting your body to shivering.' );
			//[(big cum);
			if( CoC.getInstance().player.cumQ() >= 500 ) {
				EngineCore.outputText( '  Squirt after huge squirt disappears into the starveling\'s esophagus, sliding without fail down into her belly, until ' );
				if( CoC.getInstance().player.cumQ() < 1000 ) {
					EngineCore.outputText( 'your climax is spent.' );
				}//(mega skeet);
				else {
					EngineCore.outputText( 'she can hold no more and it begins to drool from her mouth; still she tries her best to swallow more and more, despite the limits of her body.  Obligingly, you grab her head and hold it to you, venom tingling in your fingertips, as the last half of your orgasm empties into her.  With her face mashed into your crotch and escape cut off, the semen has nowhere to go except down the throat, expanding her taut stomach and pushing her skinny butt into the air as she balloons against your legs.  Her eyes widen and look up at you as the pressure increases, and a hum travels down your prick as she attempts to protest.' );
				}
			}
			...
			if( CoC.getInstance().player.cumQ() < 1000 ) {
				EngineCore.outputText( 'then gets to her feet and returns to her barrel to digest the meal.' );
			} else {
				EngineCore.outputText( 'but can barely move from the spot thanks to the incredible distension of her belly.  She manages to drag herself over to the barrel with a great shuffling of arms and legs but cannot by any means get into it, and settles for slumping beside it to occasionally sprinkle water on herself while she deflates.' );
			}
		...
		//if KidXP >= 40 and lust > 99 after tutor and PC has vag and no cox;
			...
			//[(if pc is loose);
			if( CoC.getInstance().player.vaginas[ 0 ].vaginalLooseness >= AppearanceDefs.VAGINA_LOOSENESS_GAPING ) {
				EngineCore.outputText( '  You can barely feel her little shaft in your stretched cunt, but the chemical stimulation from the tentacles stroking your insides goes a long way toward making up for that.' );
			}
			EngineCore.outputText( '  Emboldened, she picks up your legs haltingly, then begins to work herself in and out of your depths.' );
			...
			//[(clitsize<6)];
			if( CoC.getInstance().player.clitLength < 6 ) {
				EngineCore.outputText( '  The little button gets rubbed by her probing hand even as she resumes thrusting, accelerating your imminent peaking.' );
			} else if( CoC.getInstance().player.clitLength < 36 ) {
				EngineCore.outputText( '  The monstrous chick-stick bobs in the air as she pounds you harder; the breeze alone would be enough to arouse you further, but the anemone grabs it and begins jacking the nerve-laden stalk off like a dick, which sends your back into spasms.' );
			} else {
				EngineCore.outputText( '  The incredible length of your clitoris is such that the pace of the fuck slows to a crawl as the anemone puzzles out what to do with it; finally falling back on her racial instinct, she pops it into her mouth and begins to caress the tip gently with her teeth and tongue as her short hair reaches around to tickle the shaft.  Your body flops weakly, too wracked by the sensation from your clit to allow conscious control.' );
			}
	...
	AnemoneScene.prototype.reallyEvictDaAnemone = function() {
		...
		EngineCore.outputText( '"<i>Um... hi!</i>" your anemone returns, enthusiastically this time, as she answers the physical contact by tugging one of the newcomer\'s tentacles.' );
	...
	AnemoneScene.prototype.kidADreams = function() {
		...
		//if male:;
		if( CoC.getInstance().player.hasCock() && (!CoC.getInstance().player.hasVagina() || CoC.getInstance().player.femininity < 50) ) {
			EngineCore.outputText( ImageManager.showImage( 'anemone-kid-male-masti' ), false );
			...
			if( CoC.getInstance().player.isTaur() ) {
				EngineCore.outputText( 'thigh' );
			} else {
				EngineCore.outputText( 'cheek' );
			}
			EngineCore.outputText( ', she leans in to kiss you, softly whispering your name over and over.  "<i>[name]... [name]...</i>"' );
			EngineCore.outputText( '\n\nYour eyes snap open suddenly to see the ubiquitous red moon overhead.  A ' );
			if( !CoC.getInstance().player.isTaur() ) {
				EngineCore.outputText( 'soft hand strokes your face and a ' );
			}
			EngineCore.outputText( 'slight weight rests against your crotch; as you look down, your anemone is there with her pussy pressed to the erection bulging under your gear, frozen in mid-rub and blushing to the point that you can barely make her out.' );
			EngineCore.outputText( '\n\n"<i>Um... hi?</i>"  She slowly backs away into the night, leaving you to try to sleep with your awakened libido.' );
		}
		//if female:;
		else {
			EngineCore.outputText( ImageManager.showImage( 'anemone-kid-female-masti' ), false );
			...
			if( CoC.getInstance().player.cor >= 66 ) {
				EngineCore.outputText( ' and, with a sigh, you grab her head and force it down to the mess, compelling her to lick it up.' );
			} else {
				EngineCore.outputText( ' and neither of you says a word as she backs away slowly on her knees.' );
			}
			EngineCore.outputText( '  Sighing, you turn over and attempt to return to sleep despite the pervading smell of semen.' );
		}
		EngineCore.dynStats( 'lus', 50 + CoC.getInstance().player.sens / 2, 'resisted', false );
		EngineCore.doNext( EventParser.playerMenu );
	};
	...
	AnemoneScene.prototype.repeatCowSitting = function() {
		EngineCore.outputText( 'Marble looks up from her work and regards you warmly.  "<i>I appreciate the offer, [name].  I\'ve already got someone on it, but if you want to go help, I\'m sure there\'s something you could do.</i>"  She gestures in a direction over your shoulder.' );
		...
		EngineCore.outputText( '\n\n"<i>Yes, mummy,</i>" your daughter answers.  "<i>I let ' + CoC.getInstance().player.mf( 'him', 'her' ) + ' eat me right up.</i>"  Laughing, Marble leads the little girl off and you make your way back.  Kid A is dragging herself to her water barrel, looking at the ground in a frazzle.  As you pass by, she makes unblinking eye contact for a long time, then eventually acknowledges you with a curt nod.' );
		EngineCore.outputText( '\n\n"<i>...Sweetie.</i>"' );
	};
	...
	AnemoneScene.prototype.kidAWatchesSharks = function() {
		...
		if( this.kidAXP() < 40 || CoC.getInstance().flags[ kFLAGS.ANEMONE_WEAPON_ID ] === 0 ) {
		...
		else if( this.kidAXP() < 40 && (CoC.getInstance().flags[ kFLAGS.ANEMONE_WEAPON_ID ] === ConsumableLib.B__BOOK.id || CoC.getInstance().flags[ kFLAGS.ANEMONE_WEAPON_ID ] === ConsumableLib.W__BOOK.id) ) {
			...
			if( CoC.getInstance().flags[ kFLAGS.ANEMONE_WEAPON_ID ] === ConsumableLib.W__BOOK.id ) {
				EngineCore.outputText( 'blinds them with a sudden flash of white light' );
			} else {
				EngineCore.outputText( 'turns deep blue as the various shark-girls clap their thighs together and look away from one another in embarrassment' );
			}
			EngineCore.outputText( '.  Before any of them can react, she dives into the water and fills the waterskins, then hightails it away.' );
		}
		//(else KidXP < 75);
		else if( this.kidAXP() < 75 ) {
			EngineCore.outputText( '\n\nThe sharks look up from their play, baring teeth at the anemone.  She steels herself, then holds her ' + ItemType.lookupItem( CoC.getInstance().flags[ kFLAGS.ANEMONE_WEAPON_ID ] ).longName + ' menacingly in front of her as best she can while she steps into the stream and fills her waterskins.  Occasionally one of the shark-girls will feint at the anemone, who accordingly turns the weapon toward the aggressor until she retreats.  Once the skins are filled, Kid A hangs them over her shoulders and backs away, still facing the sharks with her weapon out.' );
		}
		//(else);
		else {
	...
	AnemoneScene.prototype.goblinNightAnemone = function() {
		...
		if( CoC.getInstance().player.tallness > 48 ) {
			EngineCore.outputText( 'tall' );
		} else {
			EngineCore.outputText( 'short' );
		}
		EngineCore.outputText( ', ' + CoC.getInstance().player.mf( 'studly', 'gorgeous' ) + ' ' + CoC.getInstance().player.race() + '; told us that ' + CoC.getInstance().player.mf( 'he', 'she' ) + '\'d be able to knock us all up like ' + CoC.getInstance().player.mf( 'he', 'she' ) + ' did her, and a few of the little goblin tramps outside the family have seen one in this camp.  We\'re going to get our babies.</i>"  Kid A remains silent, but shakes her head uncertainly, holding her equipment closer to her chest.' );
		...
		EngineCore.outputText( '\n\nThough you can\'t see Kid A\'s expression from behind, she\'s probably steeling her face into as stern a mask as she\'s capable of, judging by the way she assumes an aggressive stance - albeit one that\'s still trying to protect her body as much as possible with ' + ItemType.lookupItem( CoC.getInstance().flags[ kFLAGS.ANEMONE_WEAPON_ID ] ).longName + '.  As the anemone takes a step forward, the goblins all take a step back, except for the leader.  The two are now staring at each other; one looking up, the other down.' );
	...
	/*
	 ...
	 -sting with hair (combines both bee-sting effects, but weaker than either one separately) rush ' + CoC.getInstance().monster.short + ', whipping your hair around like a genie, and manage to land a few swipes with your tentacles.  As the venom infiltrates ' + CoC.getInstance().monster.pronoun3 + ' body, ' + CoC.getInstance().monster.pronoun1 + ' twitches and begins to move more slowly, hampered half by paralysis and half by arousal."
	 (decrease speed/str, increase lust)
	 -miss a sting
	 'You rush ' + CoC.getInstance().monster.short + ', whipping your hair around to catch it with your tentacles, but ' + CoC.getInstance().monster.pronoun1 + ' easily dodges.  Oy, you hope you didn\'t just give yourself whiplash.'
