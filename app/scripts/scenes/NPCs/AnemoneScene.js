...
angular.module( 'cocjs' ).run( function( MainView, SceneLib, Anemone, ItemType, WeaponLib, Weapon, $rootScope, Descriptors, PerkLib, ImageManager, CockTypesEnum, Appearance, AppearanceDefs, ArmorLib, CoC, kFLAGS, Utils, StatusAffects, PregnancyStore, EngineCore, Combat, ConsumableLib ) {
	...
	AnemoneScene.prototype.rapeAnemoneWithDick = function() {
		...
		if( CoC.player.cockThatFits( 36 ) >= 0 ) {
			...
			EngineCore.outputText( 'Rubbing yourself through your ' + CoC.player.armorName + ', you look over the anemone; your attention wanders down her torso to the blue slit between her legs', false );
			//[(lust victory);
	...
	AnemoneScene.prototype.rapeAnemoneWithPussy = function() {
		...
		EngineCore.outputText( 'Next, you take the time to strip off your ' + CoC.player.armorName + ', making sure to give a good show; the anemone entertains herself by stroking her erect prick and smearing around the pre-cum, grinning as she watches you.', false );
		//[(breastrow0>C-cup);
		...
		//[(breast0>C);
		if( CoC.player.biggestTitSize() > 2 ) {
			EngineCore.outputText( 'making sure to let your ' + CoC.player.allBreastsDescript() + ' rub up against hers, ', false );
		}
		...
		if( CoC.player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_CENTAUR ) {
			...
			EngineCore.outputText( 'recline back on your ' + CoC.player.legs() + '. Spreading your thighs, you reach down with two fingers and pull apart your ' + Descriptors.vaginaDescript( 0 ) + ' welcomingly; it\'s the last act in your sexual performance.', false );
		}
		EngineCore.outputText( '\n\n', false );
	...
	AnemoneScene.prototype.loseToAnemone = function() {
		...
		//loss rape, vaginal (only full females):;
			...
			EngineCore.outputText( ImageManager.showImage( 'anemone-female-fuck' ), false );
			EngineCore.outputText( 'Shivering, you fall to your knees before the anemone and begin trying to remove your ' + CoC.player.armorName + '.  She claps and peals happily, splashing over to you.  Pushing your trembling hands out of the way, she unfastens your garments and pulls them free of you, but her bright expression dims a bit when she sees only your ' + Descriptors.vaginaDescript( 0 ) + '.', false );
			//[(lactation);
			if( CoC.player.biggestLactation() > 1 ) {
	...
	AnemoneScene.prototype.anemoneQuoteUnquoteAnal = function() {
		...
		if( !CoC.player.isTaur() ) {
			...
			EngineCore.outputText( ' until it protrudes from the hole in your ' + CoC.player.armorName + ', returned to full erectness.  ' );
		}
		...
		EngineCore.outputText( '; her hair delivers tingles of venom where it brushes your skin even as her own hands dawdle playfully along your ' + CoC.player.skin() + '.  Without another word, you force her head into your groin as you ram your distended ' );
		if( !dick ) {
	...
	AnemoneScene.prototype.anemoneGetsLayedByBeePositor = function() {
		...
		if( CoC.player.sens >= 50 || CoC.player.isTaur() ) {
			...
			EngineCore.outputText( '  Gathering your things, you ' + CoC.player.mf( 'laugh', 'giggle' ) + ' at her and depart.' );
		}
		CoC.player.dumpEggs();
		CoC.player.orgasm();
		Combat.cleanupAfterCombat();
	...
	AnemoneScene.prototype.approachAnemoneBarrel = function() {
		...
		if( CoC.flags[ kFLAGS.ANEMONE_WEAPON_ID ] !== 0 ) {
			EngineCore.outputText( 'She has ' + ItemType.lookupItem( CoC.flags[ kFLAGS.ANEMONE_WEAPON_ID ] ).longName + ' sitting next to it.  ' );
	...
	AnemoneScene.prototype.tutorAnemoneKid = function() {
		...
		//[(KidXP < 33)];
		if( this.kidAXP() < 33 ) {
			EngineCore.outputText( 'holding ' + ItemType.lookupItem( CoC.flags[ kFLAGS.ANEMONE_WEAPON_ID ] ).longName + ' protectively across her chest.' );
		} else {
			EngineCore.outputText( 'taking up an attentive stance with ' + ItemType.lookupItem( CoC.flags[ kFLAGS.ANEMONE_WEAPON_ID ] ).longName + ' in her hands.' );
		}
