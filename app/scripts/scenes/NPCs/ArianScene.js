...
angular.module( 'cocjs' ).run( function( MainView, SceneLib, UseableLib, $rootScope, Descriptors, ImageManager, AppearanceDefs, CoC, kFLAGS, Utils, StatusAffects, EngineCore, ConsumableLib ) {
	/*Design Notes
	 ...
	 Flags and variables Arian's current "<i>health</i>", the bigger the better!
	 ...
	 Health Thresholds health: You may only Talk.
	 ...
	 AnalXP Thresholds Anal Virgin. Same as <33, except there is a virgin blurb.
	...
	ArianScene.prototype.timeChangeLarge = function() {
		return false;
	};
	...
	ArianScene.prototype.arianChest = function() {
		...
		//Name 'dose titays;
		buffer += Utils.randomChoice('tits', 'tits', 'tits', 'breasts', 'breasts', 'breasts', 'pillows', 'pillows', 'boobs', 'boobs');
		return buffer;
	};
	...
	ArianScene.prototype.meetArian = function() {
	...
	ArianScene.prototype.helpArianWhenYouMeetHim = function() {
		...
		MainView.outputText( 'You approach the hooded figure with caution, asking if they\'re all right; it feels a little silly to say that, but you can\'t think of much else to say.' );
	...
	ArianScene.prototype.visitThePark = function() {
		MainView.clearOutput();
		MainView.outputText( 'As you enter the ragged remnants of the park, you spot the sickly lizan, Arian, sitting at his usual bench, and greet him.  "<i>Oh, hello there [name].  Good to see you.</i>"  He waves lazily.' );
		//Visit 1;
		if( CoC.flags[ kFLAGS.ARIAN_PARK ] === 1 ) {
			MainView.outputText( '\n\nFeeling ' );
			if( CoC.player.cor < 50 ) {
				MainView.outputText( 'curious' );
			} else {
				MainView.outputText( 'bored' );
			}
			MainView.outputText( ', you decide to ask him what his story is.' );
			...
			if( CoC.player.hasSpells() ) {
				MainView.outputText( '  You thought spellcasting merely took fatigue and the proper mindset, not life force, and you express that sentiment to the lizan.' );
			}
		...
		//Visit 3;
			...
			if( CoC.player.cor > 66 ) {
				MainView.outputText( ', whilst privately thinking to yourself how useless that looks - no wonder they can\'t fight the demons if this is the best they\'re capable of' );
			}
			MainView.outputText( '.' );
			...
			if( CoC.player.cor >= 66 ) {
				MainView.outputText( ', keeping your real reasons for bothering with him to yourself' );
			}
			MainView.outputText( '.' );
	...
	ArianScene.prototype.visitAriansHouse = function() {
		...
		if( this.arianFollower() ) {
			MainView.outputText( 'You approach the enchanted tent and slip easily inside the doors to the luxurious interior.  ' );
			var temp = Utils.rand( 10 );
			if( temp === 0 ) {
				MainView.outputText( 'However, Arian isn\'t here right now, so you instead make yourself comfortable on the couch.  After a few minutes, Arian [Arian emself] walks in through the entrance.  "<i>Oh, [name].  I wasn\'t aware you were here... have you been waiting for long?</i>" [Arian ey] asks.  You tell [Arian em] not very long.  "<i>That\'s good to hear.  So, what can I do for you?</i>" [Arian ey] asks, with a smile.' );
			} else if( temp === 1 ) {
				MainView.outputText( 'Inside, the lizan is sitting at a table, fastidiously drinking from a cup of something hot while pouring over an arcane-looking text.  You politely cough to draw [Arian eir] attention and [Arian ey] looks at you, smiling.  "<i>Hello, [name].  I was just catching up on my studies.  Can I offer you a cup of tea, or maybe something else?</i>" [Arian ey] asks.' );
			} else if( temp <= 2 ) {
				MainView.outputText( 'The lizan is currently busy tinkering with some occult-looking paraphernalia when you find [Arian em].  You politely cough to attract [Arian eir] attention, then do so again when [Arian ey] fails to heed that.  It\'s only on the third attempt that [Arian ey] looks up apologetically from [Arian eir] work.  "<i>Ah, [name]; I\'m sorry, but I was preoccupied with something.</i>" [Arian ey] states in an apologetic tone, indicating the mess on [Arian eir] desk.  "<i>Was there something you wanted?</i>" [Arian ey] asks.' );
			} else if( temp <= 4 ) {
				MainView.outputText( 'The smell of fresh cooking fills the air and you can see Arian happily sitting down at his couch with a plate of something just cooked.  "<i>Oh, [name]; I was just about to eat, can I offer you a dish?  Or if you\'d rather do something else, this can wait,</i>" the lizan tells you with a smile.' );
			} else if( temp <= 6 ) {
				...
				MainView.outputText( 'A strange smell hits your nose as you enter the tent; it takes you a few moments, but then you place it, your ears pricking as you hear Arian letting out some very familiar groans of release.  With a smirk, you sneak up and lean over the couch, looking right into the eyes of Arian, [Arian eir] fingers still glistening with ' );
				if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] > 0 ) {
					if( CoC.flags[ kFLAGS.ARIAN_VAGINA ] > 0 ) {
					...
					MainView.outputText( 'femjizz' );
				}
				MainView.outputText( ' and, indeed, still hovering over [Arian eir] ' );
				if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] > 0 ) {
					if( CoC.flags[ kFLAGS.ARIAN_VAGINA ] > 0 ) {
						MainView.outputText( 'twin sexual slits' );
					} else {
				...
				MainView.outputText( '.  For a long moment, [Arian ey] just stares back at you; if lizans could blush, you\'re certain [Arian ey]\'d be red as a beet.  "<i>[name]! I was- I was just....  Oh, this is embarrassing,</i>" [Arian ey] mutters, looking at [Arian eir] feet.' );
			...
			if( CoC.flags[ kFLAGS.ARIAN_PARK ] === 4 ) {
				...
				if( CoC.player.cor < 33 ) {
					...
					MainView.outputText( 'indifferently' );
				}
				MainView.outputText( ' ask why he\'s thanking you.' );
				...
				if( !CoC.player.isTaur() ) {
					MainView.outputText( 'pull up a chair' );
				} else {
					MainView.outputText( 'seat your tauric body on the floor' );
				}
				MainView.outputText( '.  You rack your brains for polite conversation, and finally ask how he\'s been since you saw him last.' );
				...
				//20-29 health:;
					...
					if( CoC.flags[ kFLAGS.ARIAN_VIRGIN ] === 0 && CoC.flags[ kFLAGS.ARIAN_S_DIALOGUE ] === 2 ) {
						...
						EngineCore.addButton( 3, 'Leave', this, this.leaveFappingArian );
						return;
					}
					//(else);
					else {
				...
				//50-74 health;
					...
					if( !CoC.player.isTaur() ) {
						MainView.outputText( 'take the offered chair' );
					} else {
						MainView.outputText( 'the offered chair' );
					}
					MainView.outputText( '.' );
				...
				//75-100 health:;
					...
					if( !CoC.player.isTaur() ) {
						MainView.outputText( ' and offers you the chair nearby' );
					}
					MainView.outputText( '.  Taking another for [Arian em]self.  "<i>I love when you come visit, [name].  So, what are we going to do today?</i>" [Arian ey] asks, expectantly.' );
				}
				//(Display Options);
				this.arianHomeMenu();
			}
	...
	ArianScene.prototype.arianHomeMenu = function() {
		...
		if( CoC.flags[ kFLAGS.ARIAN_S_DIALOGUE ] === 0 && this.arianHealth() >= 10 ) {
			...
			EngineCore.addButton( 0, 'Next', this, this.arianStoryDialogue2 );
		} else if( CoC.flags[ kFLAGS.ARIAN_S_DIALOGUE ] === 2 && this.arianHealth() >= 30 ) {
			EngineCore.addButton( 0, 'Next', this, this.arianDialogue3 );
		} else if( CoC.flags[ kFLAGS.ARIAN_S_DIALOGUE ] === 3 && this.arianHealth() >= 50 ) {
			EngineCore.addButton( 0, 'Next', this, this.arianImbue );
		} else if( CoC.flags[ kFLAGS.ARIAN_S_DIALOGUE ] === 4 && this.arianHealth() >= 75 ) {
			EngineCore.addButton( 0, 'Next', this, this.arianPlot4 );
		} else if( CoC.flags[ kFLAGS.ARIAN_S_DIALOGUE ] === 5 && this.arianHealth() >= 100 ) {
			EngineCore.addButton( 0, 'Next', this, this.arianPlot5 );
		}//If no story dialogue;
		else {
			EngineCore.addButton( 0, 'Talk', this, this.talkToArianChoices );
			if( CoC.flags[ kFLAGS.ARIAN_S_DIALOGUE ] >= 2 ) {
				EngineCore.addButton( 1, 'Sex', this, this.arianSexMenu );
			}
			if( CoC.flags[ kFLAGS.ARIAN_S_DIALOGUE ] >= 1 ) {
				EngineCore.addButton( 3, 'Give Item', this, this.giveArianAnItem );
			}
			if( CoC.player.hasKeyItem( 'Arian\'s Talisman' ) >= 0 || CoC.player.hasKeyItem( 'Arian\'s Charged Talisman' ) >= 0 ) {
				EngineCore.addButton( 2, 'Talisman', this, this.imbueTalisman );
			}
			if( CoC.flags[ kFLAGS.ARIAN_S_DIALOGUE ] >= 5 ) {
				EngineCore.addButton( 4, 'Treat Corr.', this, this.treatCorruption );
			}
			if( CoC.time.hours >= 17 && this.arianFollower() ) {
				EngineCore.addButton( 8, 'Sleep With', this, this.sleepWithArian, true );
			}
			if( CoC.flags[ kFLAGS.SLEEP_WITH ] === 'Arian' ) {
				EngineCore.addButton( 8, 'NoSleepWith', this, this.dontSleepWithArian );
			}
			if( !this.arianFollower() ) {
	...
	ArianScene.prototype.dontSleepWithArian = function() {
		MainView.clearOutput();
		MainView.outputText( 'You decide not to sleep with Arian at night, for now.' );
		CoC.flags[ kFLAGS.SLEEP_WITH ] = '';
		this.arianHomeMenu();
	};
	//[=Eavesdrop=];
	ArianScene.prototype.eavesDropOnArian = function() {
		MainView.clearOutput();
	...
	ArianScene.prototype.peepOnArian = function() {
		MainView.clearOutput();
		MainView.outputText( 'Curious, you decide to take a little peek through the lock; you press yourself against it as best you can, looking through into the bedroom beyond.  True to what your ears heard, the sickly albino\'s health has improved enough for him to focus on more... carnal matters.  Naked from the waist down, he sits on the edge of his bed, groinal slit disgorging a single, average-sized phallus.  Maybe 6 inches long, it\'s a bright purple-red color, covered in strange lumps' );
		if( CoC.player.lizardCocks() > 0 ) {
			MainView.outputText( ' just like yours' );
		}
		MainView.outputText( ', though this isn\'t stopping him from enthusiastically stroking himself off.' );
	...
	//[=Leave=];
	ArianScene.prototype.leaveFappingArian = function() {
		MainView.clearOutput();
		MainView.outputText( 'You decide to let Arian have some privacy and leave for the moment... after all, what the lizan mage does in his free time is not really your business....' );
	...
	ArianScene.prototype.bargeInOnArian = function() {
		MainView.clearOutput();
		MainView.outputText( 'With a wry smirk you turn the knob and find that Arian\'s door is unlocked; without missing a beat, you open the door and step in right in time to see a sticky rope of pre paint Arian\'s slender belly as he scrambles to cover himself up.' );
		...
		//(else) //if PC is a herm;
			...
			MainView.outputText( '\n\nArian hides his face once more inside his covers and says in a whisper, "<i>Yes....</i>"' );
			MainView.outputText( '\n\nYou pull the covers off his face and say, "<i>Well... we\'ll have to fix that then.</i>"  You slip off his bed and begin stripping off your [armor].  Arian shyly does the same, stripping off his robes until he is laying in his bed, completely naked.' );
			MainView.outputText( '\n\nOnce you toss your [armor] on the floor, however, Arian\'s eyes widen as he realizes you\'re not entirely female; he eyes your ' + Descriptors.multiCockDescriptLight() + ' and the moistening pussy between your legs with equal parts wonder and arousal.' );
	...
	//[=Like Male=];
	ArianScene.prototype.hermsLikeMaleArian = function() {
		MainView.clearOutput();
		MainView.outputText( 'You tell him that\'s not the case for you; you don\'t have a problem with him being a guy.  In fact, you think he looks very cute, earning you a nervous smile.  Arian relaxes, letting you look over his body and decide what you want to do....' );
		...
		EngineCore.addButton( 1, 'Get Penetrated', this, this.getPenetratedByArianAndHisHitlerMustache );
	};
	//[=Prefer Female=];
	ArianScene.prototype.hermsLikeFemaleArian = function() {
		MainView.clearOutput();
		MainView.outputText( 'You tell him that while you do like to play with guys once in a while, you prefer girls.' );
	...
	//[=Don't mind=];
	ArianScene.prototype.youDontMindBeingGayForArian = function() {
		MainView.clearOutput();
		MainView.outputText( 'You tell him that you don\'t have a problem with males, as long as they\'re cute.  You smile at him.  "<i>You... do you really think I\'m cute?</i>"' );
	...
	//[=Like Girls=];
	ArianScene.prototype.youLikeGirlsNotSickLizardDudes = function() {
		MainView.clearOutput();
		MainView.outputText( 'You tell him that you prefer females.... Arian looks at you expectantly.  "<i>So... if I was a girl... then you wouldn\'t mind?</i>"' );
	...
	ArianScene.prototype.arianStoryDialogue1 = function() {
		this.arianHealth( 1 );
		MainView.clearOutput();
		MainView.outputText( 'You feel like you\'d like to know a bit more about Arian, so you ask if he would mind sharing some of [Arian eir] history with you.  After all, as a survivor from at least the early days of the demon war, and a wizard to boot, he\'s got to have some stories up [Arian eir] voluminous sleeves.' );
		...
		MainView.outputText( '\n\nYou comment that it does seem kind of odd that a self-professed bookworm would be so desperate to get out and stretch [Arian eir] legs, now that he mentions it.  Arian laughs at that.  "<i>Yes, quite a change wouldn\'t you say?</i>"  Then, with a sigh, he says, "<i>It\'s ironic actually.  Since I couldn\'t go outside, all I had were the books; and the books had become my world read them, loved them, lived in them, and mastered them. With the time I spent reading, it was quite an easy path towards wizardhood.  As soon as I was of age, I joined the academy formally, and during my testing I was shown to have skills greater than or on par with my testers. In the end, they didn\'t see a need to instruct me, so they declared me a master of the art and assigned me my pupils and Boon.</i>"' );
	...
	//Can sex Arian.;
	ArianScene.prototype.arianStoryDialogue2 = function() {
		MainView.clearOutput();
		this.arianHealth( 1 );
		MainView.outputText( 'You look Arian over, remarking that he seems to be getting better after all.' );
	...
	//=Drop It=;
	ArianScene.prototype.arianStory2DropIt = function() {
		MainView.clearOutput();
		MainView.outputText( 'Though you do feel a little curious, you decide to stop making him uncomfortable, and tell him that it\'s okay, you\'ll let him get some sleep now.' );
	...
	ArianScene.prototype.arianStoryPry = function() {
		MainView.clearOutput();
		MainView.outputText( 'Oh, no, you\'re not letting him wriggle out of this that easily.  You playfully tap [Arian eir] nose and tell him he should come clean and confess' );
		if( CoC.player.cor < 40 ) {
			MainView.outputText( '; he\'ll sleep better with the burden off [Arian eir] conscience' );
		}
		MainView.outputText( '.' );
	...
	//Will Teach Magic;
	ArianScene.prototype.arianDialogue3 = function() {
		MainView.clearOutput();
		this.arianHealth( 1 );
		MainView.outputText( 'Before you can say anything, Arian asks you, "<i>[name], I\'ve been wondering....  Do you have any interest in magic?  You\'ve done so much for me; I believe I should return the favor somehow.</i>"' );
		//ArianSDialogue++;;
		CoC.flags[ kFLAGS.ARIAN_S_DIALOGUE ]++;
		//[Yes] [No];
		EngineCore.menu();
	...
	//=Yes=;
	ArianScene.prototype.yesArianShouldMagicTeach = function() {
		MainView.clearOutput();
	...
	//=No=;
	ArianScene.prototype.noArianShouldntMagicTeach = function() {
		MainView.clearOutput();
		MainView.outputText( 'You think it over for a moment, and then tell Arian that while you are flattered by the offer and willing to consider it, you can\'t say that you want to study magic right this moment.  You\'d like to discuss it at some other time, please.' );
	...
	ArianScene.prototype.arianImbue = function() {
		MainView.clearOutput();
		this.arianHealth( 1 );
		MainView.outputText( 'Before you can say anything, Arian gasps, "<i>Oh, [name].  I have a surprise for you.</i>"  Arian says with a smile.' );
		...
		MainView.outputText( '\n\nYou reach out and gently take it from [Arian em], carefully opening the package.  A part of you briefly wonders if it might be an engagement ring, then dismisses the thought - surely not, not even here in Tel\'Adre.  Once the package is open, you gaze upon a silver necklace design is intricate and exotic - very beautiful.  Held by its unusual chain lies a small silver plate with a rune adorning the center, although you don\'t recognize the rune.' );
		MainView.outputText( '\n\nArian smiles at you.  "<i>Do you like it?  I made it myself.</i>"' );
		MainView.outputText( '\n\nYou study the fascinating piece, and tell [Arian em] the honest truth beautiful.  You never would have expected [Arian em] to be such a crafts[Arian man].  Arian blushes at your flattery.  "<i>Thanks, I\'m glad you like it.  But let me explain - that is not a common necklace; it\'s actually a magical talisman.  I wanted to give you something that would be useful in your adventures,</i>" Arian explains.' );
	...
	//=Yes=;
	ArianScene.prototype.yesPlotSexArian = function() {
		MainView.clearOutput();
		MainView.outputText( 'You approach the awkwardly amorous lizan and place your arms around [Arian eir] neck.  Leaning in close, you whisper into [Arian eir] ear that [Arian ey] only had to ask.' );
		//(Display Sex Menu);
		this.arianSexMenu( false );
	};
	//=No=;
	ArianScene.prototype.noPlotSexNauArian = function() {
		MainView.clearOutput();
		MainView.outputText( 'You apologize to the lizan, telling [Arian em] that you aren\'t in the mood right now....' );
	...
	ArianScene.prototype.arianPlot4 = function() {
		MainView.clearOutput();
		this.arianHealth( 1 );
		MainView.outputText( 'Before you can say anything, Arian says, "<i>Oh, I have good news, [name]!</i>"' );
	...
	ArianScene.prototype.arianPlot5 = function() {
		MainView.clearOutput();
		this.arianHealth( 1 );
		MainView.outputText( 'Before you can say anything, Arian stops you.  "<i>I\'ve been meaning to ask you something, [name].  I\'ve been feeling a lot better lately; in fact, I may be even better than I was before.</i>"  Arian blushes.' );
		...
		if( SceneLib.camp.companionsCount() > 1 ) {
			MainView.outputText( '  There are other people living with you already, so what\'s one more?  Right?' );
		}
	...
	//[=Accept=];
	ArianScene.prototype.acceptArianMovingIntoCamp = function() {
		MainView.clearOutput();
	...
	//[=Deny=];
	ArianScene.prototype.denyAriansMoveIn = function() {
		MainView.clearOutput();
		MainView.outputText( 'You tell Arian you\'d like some time to think about it.  Arian looks disappointed at first, but smiles at you all the same.  "<i>I understand... no pressure....  So, what are we going to do today?</i>"' );
		//(Display Options);
		this.arianHomeMenu();
	};
	//Talk;
	ArianScene.prototype.talkToArianChoices = function() {
		...
		if( CoC.flags[ kFLAGS.ARIAN_S_DIALOGUE ] >= 3 ) {
			EngineCore.addButton( 1, 'Teach Magic', this, this.arianMagicLessons );
		}
		if( !this.arianFollower() && CoC.flags[ kFLAGS.ARIAN_S_DIALOGUE ] >= 6 ) {
			EngineCore.addButton( 4, 'Invite2Camp', this, this.inviteArianToCamp );
		}
		...
		EngineCore.addButton( 9, 'Back', this, this.arianHomeMenu );
	};
	//Magic:;
	//Magic Lessons, teaches white magic and increases int. Up to 100.;
	//Gain a pretty nice boost, 4 lessons per day, only.;
	ArianScene.prototype.arianMagicLessons = function() {
		MainView.clearOutput();
		this.arianHealth( 1 );
		MainView.outputText( 'You ask Arian if [Arian ey] wouldn\'t mind giving you some magic lessons.' );
		//(if ArianMLesson >= 4);
		...
		if( CoC.player.inte < 25 ) {
			...
			if( !this.arianFollower() ) {
				MainView.outputText( 'make your way back to your camp.' );
			} else {
				MainView.outputText( 'return to your duties.' );
			}
		}
		//(else if PC int < 50 //Teach Charge Weapon if PC doesn't know.;
			...
			MainView.outputText( '\n\n"<i>So remember overcharge your conduit or the results could be disastrous....  Just ask Boon,</i>" Arian chuckles.' );
			...
			if( !this.arianFollower() ) {
				MainView.outputText( ', making your way back to camp.' );
			} else {
				MainView.outputText( ', exiting [Arian eir] tent and going about your business.' );
			}
			...
			if( CoC.player.findStatusAffect( StatusAffects.KnowsCharge ) < 0 ) {
				...
				MainView.outputText( ' away, you start pondering Arian\'s lesson of the day.  You wonder if you could perhaps apply the channeling principle to strengthen your [weapon] in combat.' );
			...
			MainView.outputText( '<b>Game Note isn\'t canon and will be re-written soonish.</b>' );
			...
			if( !this.arianFollower() ) {
				MainView.outputText( 'begin the trek home.' );
			} else {
	...
	ArianScene.prototype.arianSexingTalk = function() {
		MainView.clearOutput();
		this.arianHealth( 1 );
		MainView.outputText( 'You smirk knowingly at [Arian em] and ask how [Arian ey] feels about sex now that [Arian ey]\'s had [Arian eir] first time?' );
	...
	//[=Yes=];
	ArianScene.prototype.yesYouButtslutIllFuckYou = function() {
		MainView.clearOutput();
	...
	//[=No=];
	ArianScene.prototype.goddamnitNoYouButtSlut = function() {
		MainView.clearOutput();
		MainView.outputText( 'You apologize, but you really can\'t do that right now.  Arian looks a bit disappointed, but smiles at you all the same.  "<i>Oh... okay.  Next time then?</i>"' );
		MainView.outputText( '\n\nYou nod.' );
		this.sexTalkFinish( false );
	};
	ArianScene.prototype.sexTalkFinish = function( newl ) {
		if( newl ) {
			MainView.clearOutput();
		} else {
			MainView.outputText( '\n\n' );
		}
		MainView.outputText( 'Satisfied with your little chat, you pat the lizan\'s head and excuse yourself, heading back to camp.' );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	//Invite to Camp:;
	//Only available if ArianHealth === 100.;
	ArianScene.prototype.inviteArianToCamp = function() {
		MainView.clearOutput();
		MainView.outputText( 'You ask the lizan if [Arian ey] still really wants to leave [Arian eir] comfortable home in the city and come out to live with you in your little camp in the wastelands?' );
	...
	ArianScene.prototype.takeYerLizardHomePartII = function() {
		MainView.clearOutput();
		MainView.outputText( 'Upon arriving at the camp, the first thing Arian notices is the shimmering portal.  "<i>Is this... where you came from?</i>" Arian asks.' );
		...
		if( SceneLib.camp.followersCount() > 1 ) {
			MainView.outputText( ' and your other companions' );
		}
		MainView.outputText( '.' );
	...
	ArianScene.prototype.arianSexMenu = function( output ) {
		if( output === undefined || output ) {
			MainView.clearOutput();
			MainView.outputText( 'You ask Arian if [Arian ey] feels strong enough to do a little lovemaking.' );
			//(if ArianHealth < 30);
		...
		if( CoC.player.hasCock() ) {
			...
			if( CoC.player.cockThatFits( 50 ) >= 0 ) {
				EngineCore.addButton( 0, 'Anal - Pitch', this, this.giveArianAnal );
			}
			//Get Blown:;
			//PC must have a cock.;
			EngineCore.addButton( 2, 'Get Blown', this, this.getBlownByArian );
			//Penetrate:;
			//Arian must be herm/female.;
			//PC must have a cock that fits (cock area 50 or less);
			if( CoC.player.cockThatFits( CoC.flags[ kFLAGS.ARIAN_CAPACITY ] ) >= 0 && CoC.flags[ kFLAGS.ARIAN_VAGINA ] > 0 ) {
				EngineCore.addButton( 4, 'Fuck Vagina', this, this.penetrateArian );
			}
			//Double Pen Arian:;
			...
			if( CoC.player.cockThatFits( 50 ) >= 0 && CoC.player.cockThatFits2( 50 ) >= 0 && CoC.flags[ kFLAGS.ARIAN_VAGINA ] > 0 ) {
				EngineCore.addButton( 8, 'Double Pen', this, this.doublePenetrateArian );
			}
			//Docking;
			...
			if( CoC.player.smallestCockArea() <= 30 && CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] < 3 && CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] > 0 ) {
				EngineCore.addButton( 7, 'Docking', this, this.arianDocking );
			}
		}
		//Get Anal:;
		//Arian must have a cock.;
		if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] > 0 ) {
			EngineCore.addButton( 1, 'Anal - Catch', this, this.getButtWreckedByArian );
		}
		//Blow:;
		//Arian must have a cock.;
		if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] > 0 ) {
			EngineCore.addButton( 3, 'Blow ' + this.arianMF( 'Him', 'Her' ), this, this.suckAriansDick );
		}
		...
		if( CoC.player.hasVagina() ) {
			...
			//Arian must have a cock.;
			if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] > 0 ) {
				EngineCore.addButton( 5, 'Get Fucked', this, this.getPenetratedByArianAndHisHitlerMustache );
			}
			if( CoC.flags[ kFLAGS.ARIAN_VAGINA ] > 0 && (CoC.player.hasKeyItem( 'Dildo' ) >= 0 || CoC.player.hasKeyItem( 'Deluxe Dildo' ) >= 0) ) {
				EngineCore.addButton( 6, 'Dildo Fun', this, this.arianDildoFun );
			}
		}
		EngineCore.addButton( 9, 'Back', this, this.arianHomeMenu );
	};
	...
	ArianScene.prototype.giveArianAnal = function() {
		...
		if( CoC.flags[ kFLAGS.ARIAN_VAGINA ] > 0 ) {
			MainView.outputText( ImageManager.showImage( 'arianfemale-home-giveArianAnal' ) );
		} else {
			MainView.outputText( ImageManager.showImage( 'arianmale-home-giveArianAnal' ) );
		}
		...
		MainView.outputText( 'You tell Arian that, if [Arian ey]\'s willing, you\'d like to take [Arian em] from behind.' );
		//AnalXP < 33;
		...
		if( CoC.flags[ kFLAGS.ARIAN_ANAL_XP ] < 33 ) {
			...
			if( CoC.flags[ kFLAGS.ARIAN_VAGINA ] < 1 ) {
				...
				MainView.outputText( ' isn\'t going to make it any smaller either.</i>"  Arian fidgets and shudders as you stroke [Arian eir] behind.' );
			}
			...
			if( CoC.player.balls === 0 ) {
				MainView.outputText( 'the base of your cock' );
			} else {
				MainView.outputText( 'the bottom of your balls' );
			}
			MainView.outputText( ' and you warn Arian that you\'re going to cum inside [Arian em] if you keep going.  Arian drools in pained pleasure, for a moment you wonder if [Arian ey] even heard you.  "<i>Cum.  Cum inside me.  I want it... all of it! Ah!</i>"' );
			...
			if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] > 0 ) {
				MainView.outputText( 'spewing forth large gobs of cum of [Arian eir] own' );
			}
			if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] > 0 && CoC.flags[ kFLAGS.ARIAN_VAGINA ] > 0 ) {
				MainView.outputText( ' and ' );
			}
			if( CoC.flags[ kFLAGS.ARIAN_VAGINA ] > 0 ) {
				MainView.outputText( 'wetting both your and her legs with a healthy serving of lizan pussy juice' );
			}
			MainView.outputText( '.  You shudder and gasp until, utterly spent, you can\'t resist sinking down atop Arian and bearing [Arian em] into the comforting embrace of the bed.  You lay there, recovering your strength from your most delightful exertions, wriggling to give the lizan a proper snuggling while you lay there.' );
		...
		//Loose, sensitive enough to make Arian scream in pleasure. (It's like a tight vagina, feels like one too, minor pain);
			...
			if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] > 0 ) {
				...
				if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
					MainView.outputText( ' pair of dripping cocks' );
				} else {
			...
			if( CoC.flags[ kFLAGS.ARIAN_VAGINA ] > 0 ) {
				MainView.outputText( ' a sopping wet pussy' );
			}
			MainView.outputText( ' greet' );
			...
			MainView.outputText( ' your touch.  What a little slut Arian is turning into; [Arian ey]\'s already raring to go, and even knowing it\'s going to be [Arian eir] ass that\'s getting fucked, too!' );
			...
			if( CoC.flags[ kFLAGS.ARIAN_VAGINA ] < 1 ) {
				...
				MainView.outputText( 'nether-pussy' );
			}
			MainView.outputText( '.  "<i>Yesssss.... Oh, sometimes it hurts a bit, but the feeling, the pleasure, the pain.... It\'s heavenly.</i>"' );
			...
			if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] > 0 ) {
				if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] === 0 ) {
					MainView.outputText( 'cock spews its load' );
				} else {
					MainView.outputText( 'twin cocks spew their loads' );
				}
				MainView.outputText( ' on the bedsheets' );
				if( CoC.flags[ kFLAGS.ARIAN_VAGINA ] > 0 ) {
					MainView.outputText( ' and [Arian eir] ' );
				}
			}
			if( CoC.flags[ kFLAGS.ARIAN_VAGINA ] > 0 ) {
				MainView.outputText( ' contracting pussy paints your lower body with lizan femcum' );
			}
			MainView.outputText( '.  Arian is only capable of moaning and shuddering as [Arian eir] powerful orgasm rocks the poor lizan to [Arian eir] core.  The extra tightness of [Arian eir] contracting butthole increases the friction on your ' + Descriptors.cockDescript( x ) + ', pushing you ever closer to the climax.' );
			...
			if( CoC.player.balls === 0 ) {
				MainView.outputText( 'base of your spine' );
			} else {
				MainView.outputText( 'depths of your balls' );
			}
			MainView.outputText( '.  Arian, completely blissed out, lays limply on [Arian eir] bed, [Arian eir] butt held up by your gripping hands.  With a final deep thrust you finally go over the edge.' );
			//(Low Cum Amount);
			if( CoC.player.cumQ() < 250 ) {
		...
		//Very Loose, sensitive enough to make Arian cum just from insertion. (Feels better than anything else. Yep, Arian really becomes a buttslut at this point);
			...
			if( CoC.flags[ kFLAGS.ARIAN_VAGINA ] > 0 ) {
				MainView.outputText( 'wet gash is positively leaking lizan juices' );
			}
			if( CoC.flags[ kFLAGS.ARIAN_VAGINA ] > 0 && CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] > 0 ) {
				MainView.outputText( ' and [Arian eir] ' );
			}
			if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] > 0 ) {
				MainView.outputText( 'erect cock is already leaking pre like a sieve' );
			}
			MainView.outputText( '.  You can hardly believe how far Arian has come; you ask [Arian em] if it really feels that good?  "<i>More than you can... ah... imagine, now fill me up!</i>" [Arian ey] demands with a sense of urgency.' );
			...
			if( CoC.flags[ kFLAGS.ARIAN_VAGINA ] > 0 ) {
				MainView.outputText( 'her pussy clenches, spilling a flood of juices; ' );
			}
			if( CoC.flags[ kFLAGS.ARIAN_VAGINA ] > 0 && CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] > 0 ) {
				MainView.outputText( ' and ' );
			}
			if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] > 0 ) {
				MainView.outputText( 'his cock throbs and shoots rope after rope of cum onto the bedsheets' );
			}
			MainView.outputText( '. Overwhelmed by your sudden intrusion, Arian collapses forward, burying [Arian eir] face on [Arian eir] pillow and dragging you on top of [Arian em].  "<i>Ah... [name], you feel so good.  It\'s amazing.  I never thought buttsex could ever feel this good.</i>"' );
			...
			if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] > 0 ) {
				if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
					MainView.outputText( 'cocks are still rock hard, and surprisingly, still leaking ropes of cum' );
				} else {
					MainView.outputText( 'cock is still rock hard, and surprisingly, still leaking ropes of cum' );
				}
				if( CoC.flags[ kFLAGS.ARIAN_VAGINA ] > 0 ) {
			...
			if( CoC.flags[ kFLAGS.ARIAN_VAGINA ] > 0 ) {
				MainView.outputText( ' pussy squirting juices against your [legs] with each shuddering impact of your [hips]' );
			}
			MainView.outputText( '.  "<i>Ah!  Yes!  More, give me more!  Harder!  Faster!</i>" Arian pleads, before biting on [Arian eir] pillow in pleasure.' );
			...
			if( CoC.flags[ kFLAGS.ARIAN_VAGINA ] > 0 ) {
				MainView.outputText( '  Her cunt grips the air and blasts one last jet of wetness at your crotch.' );
			}
			if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] > 0 ) {
				MainView.outputText( '  [Arian Eir] throbbing cock flexes and shoots one last rope of lizan-cum against your belly.' );
			}
			MainView.outputText( '  [Arian Eir] ass grips you forcefully, forming a vacuum and finally loosens.  "<i>That... that wash da best,</i>"  Arian slurs, before finally collapsing on [Arian eir] bed.' );
			...
			if( CoC.flags[ kFLAGS.ARIAN_ASS_CHAT ] === 0 ) {
				...
				if( 9999 === 9999 ) {
					MainView.outputText( ' as hypocritical as that may be' );
				}
				MainView.outputText( '... besides, you think [Arian ey]\'s kind of sexy when [Arian ey] gets like that.  At that Arian perks up.  "<i>Really?</i>"  Yes, really, you reply.  Arian smiles happily at you.  "<i>So... do you want to go again?</i>"' );
				...
				if( !this.arianFollower() ) {
					MainView.outputText( 'back to Tel\'Adre\'s streets, and from there back to camp.' );
				} else {
					MainView.outputText( 'out of [Arian eir] tent.' );
				}
	...
	ArianScene.prototype.getBlownByArian = function() {
		...
		if( CoC.flags[ kFLAGS.ARIAN_HAS_BLOWN ] === 0 ) {
			...
			if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] === 3 ) {
				MainView.outputText( '  You notice Arian\'s exposed shaft slowly hardening at your invitation.' );
			}
			MainView.outputText( '  "<i>I... Can I really?</i>"' );
		...
		MainView.outputText( ' pot belly.  [Arian Ey] lets go of your dick with a sigh of pleasure and lays back on the bed.  "<i>Ah... I\'m full.</i>"' );
		MainView.outputText( '\n\nYou tell [Arian em] that [Arian ey] looks full' );
		if( CoC.player.cumQ() >= 1500 ) {
			MainView.outputText( ', in fact, you\'re amazed [Arian ey] didn\'t pop with how much [Arian ey] drank' );
		}
		MainView.outputText( '; is [Arian ey] sure [Arian ey]\'s all right?' );
		...
		if( CoC.player.cumQ() >= 1000 ) {
			MainView.outputText( ' which, considering the fact [Arian ey] looks like a mother ready to birth a toddler from the size of [Arian eir] belly, isn\'t as easy as you\'d think' );
		}
		MainView.outputText( '.  Letting the lizan use your shoulder, you pull [Arian eir] sheets up and help [Arian em] back in the bed, where [Arian ey] flops down with a protest of springs' );
		if( CoC.player.cumQ() >= 250 ) {
			MainView.outputText( ', [Arian eir] gut audibly sloshing as the cum inside is churned by the impact' );
		}
		MainView.outputText( '.' );
		...
		if( !this.arianFollower() ) {
			MainView.outputText( ' back to camp, stopping only to notify Laika and Boon that Arian is sleeping.' );
		} else {
			MainView.outputText( ' out of Arian\'s tent.' );
		}
		CoC.player.orgasm();
	...
	ArianScene.prototype.penetrateArian = function() {
		...
		CoC.flags[ kFLAGS.ARIAN_VIRGIN ] += 1;
		this.arianHealth( 3 );
		MainView.outputText( ImageManager.showImage( 'arianfemale-home-penetrate' ) );
		MainView.outputText( 'You admire the transgendered lizan\'s body, from her feminized features, down past her [arian chest], all the way to her shapely thighs.  You tell Arian that the change looks very good on her; few boys would really be so naturally pretty when turned into a ' );
		if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] > 0 ) {
		...
		MainView.outputText( '.  "<i>Err... readier?</i>"  She asks, smiling nervously.' );
		...
		if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] === 3 ) {
			MainView.outputText( ', her exposed cock' );
			if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ' hardening at what she\'s about to do' );
		} else if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] > 0 ) {
			MainView.outputText( ', her bulbous cock' );
			if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
				MainView.outputText( 's peeking out of their' );
			} else {
				MainView.outputText( ' peeking out of its' );
			}
			MainView.outputText( ' hiding place, already at full mast in anticipation' );
		}
		MainView.outputText( '.  Arian swallows audibly.  "<i>What now?</i>" she asks, already panting in lust.' );
		...
		MainView.outputText( ' lizan, until you have managed to hilt yourself inside of her.' );
		...
		if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] > 0 ) {
			...
			if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
				MainView.outputText( 'twin shafts' );
			} else {
				MainView.outputText( 'shaft' );
			}
			MainView.outputText( ' bobbing just out of your field of vision, leaking pre all over your belly.' );
		}
		...
		if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] > 0 ) {
			MainView.outputText( '  Her cock' );
			if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
				MainView.outputText( 's twitch as they spurt ' );
			} else {
				MainView.outputText( ' twitches as it spurts ' );
			}
			MainView.outputText( 'a small rope of pre over her belly, anticipating what\'s to come as she braces herself for the inevitable pounding you\'re about to give her.' );
		}
		...
		if( CoC.player.hasVagina() ) {
			MainView.outputText( ' A wash of fluids spills down your body from your cunt as it spasms in release and y' );
		} else {
			MainView.outputText( 'Y' );
		}
		MainView.outputText( 'ou unleash your essence right into the lizan\'s burning womb.' );
		//(Low Cum amount);
		if( CoC.player.cumQ() < 250 ) {
		...
		if( CoC.player.cumQ() < 250 ) {
			MainView.outputText( 'lean' );
		} else if( CoC.player.cumQ() < 1000 ) {
			MainView.outputText( 'full' );
		} else {
			MainView.outputText( 'overfilled' );
		}
		MainView.outputText( ' belly.  "<i>Stay with me, like this, a little longer?</i>" Arian asks.' );
	...
	ArianScene.prototype.getButtWreckedByArian = function() {
		...
		this.arianHealth( 3 );
		if( CoC.flags[ kFLAGS.ARIAN_VAGINA ] > 0 ) {
			MainView.outputText( ImageManager.showImage( 'arianfemale-home-getArianAnal' ) );
		} else {
			MainView.outputText( ImageManager.showImage( 'arianmale-home-getArianAnal' ) );
		}
		MainView.outputText( 'With a lick of your lips with your tongue, you ask how Arian would like to be on the pitching end of anal?' );
		//(if AnalXP < 33);
		if( CoC.flags[ kFLAGS.ARIAN_ANAL_XP ] < 33 ) {
		...
		if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] >= 3 ) {
			MainView.outputText( 'exposed cock' );
			if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
				MainView.outputText( 's harden' );
			} else {
				MainView.outputText( ' hardens' );
			}
			MainView.outputText( ' to full mast.' );
		} else {
			MainView.outputText( this.arianMF( 'his', 'her' ) + ' cock' );
			if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
				MainView.outputText( 's peek out of their' );
			} else {
				MainView.outputText( ' peeks out of its' );
			}
			MainView.outputText( ' hiding place, rising to full mast.' );
		}
		MainView.outputText( '\n\nYou sashay yourself towards the prone lizan, straddling [Arian eir] legs and reaching out to grasp [Arian eir] ' );
		if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
			MainView.outputText( 'primary ' );
		}
		MainView.outputText( 'cock.  Slowly and gently you begin to stroke its strange, knobbly surface, your fingers eliciting moans and gasps from Arian as [Arian ey] shudders under your touch.  Precum begins to ooze from the reptilian prick\'s head, and you purposefully rub it into [Arian eir] shaft as a kind of makeshift lubricant.' );
		MainView.outputText( '\n\n"<i>Ohm... T-this feels great, [name], b-but if you keep this up, I won\'t be able to hold back!</i>" Arian pants, [Arian eir] shaft' );
		if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ' throbbing against your hands.' );
		...
		if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
			MainView.outputText( '  [Arian Eir] other shaft twiches and sprays your ' );
		...
		if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
			MainView.outputText( 'stroking [Arian eir] other cock and sliding it between your ass cheeks, ' );
		}
		MainView.outputText( 'caressing the base of [Arian eir] tail.  With a devious grin, you begin to gently but insistently slide your finger into [Arian eir] ' );
		if( CoC.flags[ kFLAGS.ARIAN_ANAL_XP ] > 66 ) {
			MainView.outputText( 'eager ' );
		}
		MainView.outputText( 'ass, probing for [Arian eir] prostrate...' );
		MainView.outputText( '\n\nThe reaction is almost instantaneous; Arian moans into your kiss and you feel [Arian eir] ' );
		if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
			MainView.outputText( 'pair of lizan-cocks practically jump' );
		} else {
			MainView.outputText( 'lizan-cock practically jump' );
		}
		MainView.outputText( ' back to full mast, even surprising you with Arian\'s eagerness.' );
		...
		if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
			MainView.outputText( 'sliding [Arian eir] neglected prick through your ass-cheeks, ' );
		}
		MainView.outputText( 'clenching your inner walls to grip and feel the excitingly unusual lumpy, bulbous texture of [Arian eir] shaft inside you.  You moan and gasp, telling [Arian em] that [Arian ey] feels so unique, and yet [Arian ey] stimulates you so wonderfully.' );
		...
		if( CoC.player.hasVagina() ) {
			MainView.outputText( 'cunt dripping with arousal' );
		}
		if( CoC.player.hasCock() && CoC.player.hasVagina() ) {
			MainView.outputText( ' and ' );
		}
		if( CoC.player.hasCock() ) {
			...
			MainView.outputText( ' drooling precum onto [Arian eir] belly' );
		}
		MainView.outputText( '.  You\'re so close now, you can feel it.  You tell Arian you\'re going to cum, and beg [Arian em] to cum with you.' );
		...
		MainView.outputText( ', signaling [Arian eir] oncoming orgasm.  [Arian Ey] grabs the sheets on [Arian eir] bed and curls [Arian eir] toes.  "<i>...[name]!</i>"  Jet after jet of lizan spunk paints your bowels white' );
		if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
			MainView.outputText( ', as well as your butt' );
		}
		MainView.outputText( ', as Arian climaxes' );
		if( CoC.flags[ kFLAGS.ARIAN_VAGINA ] > 0 ) {
			MainView.outputText( ', her own pussy clenching and drizzling juices on the matted sheets beneath the two of you.' );
		} else {
			MainView.outputText( '.' );
		}
		...
		if( CoC.player.hasCock() ) {
			MainView.outputText( '  [EachCock] spasms and shoots rope after rope of cum, painting Arian\'s [arian chest] white, as well as the groaning lizan\'s face.' );
		}
		if( CoC.player.hasVagina() ) {
			MainView.outputText( '  Your [pussy], though unattended, joins in the perverted display, leaking copious amount of fluids on top of the prone lizan.' );
		}
		...
		//(if AnalXP <= 100);
			...
			if( CoC.player.hasCock() ) {
				...
				if( CoC.player.cockThatFits( 50 ) >= 0 && CoC.player.hasCock() ) {
					...
					MainView.outputText( '.  You\'re too big to fit inside Arian\'s ass, though.' );
				}
				EngineCore.addButton( 1, 'No', SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
				return;
			} else {
	...
	ArianScene.prototype.suckAriansDick = function() {
		...
		this.arianHealth( 3 );
		if( CoC.flags[ kFLAGS.ARIAN_VAGINA ] > 0 ) {
			MainView.outputText( ImageManager.showImage( 'arianfemale-home-suckariandick' ) );
		} else {
			MainView.outputText( ImageManager.showImage( 'arianmale-home-suckariandick' ) );
		}
		MainView.outputText( 'You make a show of lewdly licking your lips and ask Arian if [Arian ey]\'d be willing to let you have a little taste of lizan essence...?' );
		...
		if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] > 3 ) {
			if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
				MainView.outputText( 'exposed cocks hardening to full mast' );
			} else {
				MainView.outputText( 'exposed cock hardening to full mast' );
			}
		} else {
			MainView.outputText( 'cock' );
			if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
				MainView.outputText( 's emerging from their' );
			} else {
				MainView.outputText( ' emerging from its' );
			}
			MainView.outputText( ' hiding place, already fully erect' );
			MainView.outputText( '.' );
		}
		...
		MainView.outputText( 'cock, marveling at the strange textures of its bumpy, knobbly, bulbous surface.  It feels so strange to you, and you continue to stroke it eagerly.' );
		...
		MainView.outputText( 'shaft' );
		if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
			MainView.outputText( 's tremble' );
		} else {
			MainView.outputText( ' trembles' );
		}
		MainView.outputText( ' and throb' );
		if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] === 0 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ' against your hand; pre quickly forming on the tip' );
		if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ' only to slowly slide down the bulbous surface of Arian\'s lizan prick' );
		if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( '.  You continue to slide your hand along the increasingly slick surface, playfully asking if your hand really feels that good to [Arian em]?' );
		...
		MainView.outputText( ' a long, loving, slurpy lick.  The taste is surprising; a sort of sugary-tinted spice, not the usual salt of another creature.  It\'s actually quite pleasant, and you find yourself running your tongue up and down [Arian eir] prick as if it were some kind of candied stick, eagerly sucking and slobbering as you coax more of [Arian eir] yummy goo from its strangely-shaped tip.' );
		...
		MainView.outputText( ' one more time and erupts into your waiting mouth' );
		if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
			MainView.outputText( ' and chin' );
		}
		MainView.outputText( '.  [Arian Eir] legs close around your back, effectively keeping you from moving too far away as [Arian ey] finishes giving you all of [Arian eir] load.' );
		...
		if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
			MainView.outputText( ', oblivious to the second cock spewing cum all over your [chest] in your eagerness' );
		}
		MainView.outputText( '.  After [Arian ey] finally finishes, you continue to lick and suckle for a few minutes longer, making sure you\'ve lapped up every last drop.  Then and then alone do you lift your head, flirtatiously licking your lips to tease the now-spent lizan.' );
		...
		MainView.outputText( ' rolling your neck in pleasure as [Arian ey] rubs it with [Arian eir] reptilian tail.' );
		...
		if( CoC.flags[ kFLAGS.ARIAN_VAGINA ] > 0 ) {
			MainView.outputText( ' and tickle her cunt with a finger' );
		}
		MainView.outputText( ' for emphasis, kiss [Arian em] gently on the tip of [Arian eir] nose, and then get your clothes together, planning on getting dressed and heading back out again.' );
	...
	ArianScene.prototype.getPenetratedByArianAndHisHitlerMustache = function() {
		...
		this.arianHealth( 3 );
		if( CoC.flags[ kFLAGS.ARIAN_VAGINA ] > 0 ) {
			MainView.outputText( ImageManager.showImage( 'arianfemale-home-getpenetrated' ) );
		} else {
			MainView.outputText( ImageManager.showImage( 'arianmale-home-getpenetrated' ) );
		}
		MainView.outputText( 'You look at Arian\'s ' );
		if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] < 3 ) {
			MainView.outputText( 'slit, and think of what it\'s hiding in there' );
		} else {
			MainView.outputText( 'exposed cock' );
			if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
				MainView.outputText( 's' );
		...
		MainView.outputText( '; then you turn to look at Arian and ask [Arian em] how would [Arian ey] feel about giving [Arian eir] ' );
		if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
			MainView.outputText( 'twin cocks' );
		} else {
			MainView.outputText( 'cock' );
		}
		MainView.outputText( ' a bit of a workout?' );
		...
		if( CoC.player.isBiped() ) {
			MainView.outputText( 'spread your [legs] and ' );
		}
		MainView.outputText( 'expose your [vagina] to [Arian eir] viewing pleasure.  The reaction is almost instantaneous; Arian\'s ' );
		if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] >= 3 ) {
			MainView.outputText( 'exposed' );
		} else {
			MainView.outputText( 'hidden' );
		}
		if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
			MainView.outputText( ' twin shafts immediately fill' );
		} else {
			MainView.outputText( ' shaft immediately fills' );
		}
		MainView.outputText( ' with blood, growing fully erect in ' );
		if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
			MainView.outputText( 'their' );
		} else {
			MainView.outputText( 'its' );
		}
		MainView.outputText( ' bulbous, throbbing glory.' );
		...
		MainView.outputText( ' and get ready to thrust in; otherwise neither of you are going to be feeling good any time soon.' );
		MainView.outputText( '\n\nSnapping to [Arian eir] senses, Arian quickly ' );
		MainView.outputText( 'kneels between your [legs]' );
		MainView.outputText( ', aligning the tip of ' );
		if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
			MainView.outputText( 'each of ' );
		}
		MainView.outputText( this.arianMF( 'his', 'her' ) + ' ' );
		MainView.outputText( 'shaft' );
		if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ' with your [vagina]' );
		if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
			MainView.outputText( ' and [asshole]' );
		}
		MainView.outputText( '.  [Arian Ey] looks at you, waiting for you to confirm that [Arian ey] should indeed get going.  You smile and nod' );
		if( CoC.player.hasLongTail() ) {
			MainView.outputText( ', looping your tail around [Arian eir] waist' );
		}
		MainView.outputText( '.' );
		...
		if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
			...
			if( CoC.player.hasLongTail() ) {
				MainView.outputText( ', further encouraging [Arian em] to do so, by looping your tail around [Arian eir] waist' );
			}
			MainView.outputText( '.  The lizan doesn\'t need much encouragement to comply, easily snuggling against you, breast to breast, and sighing softly.  "<i>I\'m sorry.  Sometimes I wonder why you bother doing something like this, with someone like me,</i>" [Arian ey] says, a hint of sadness in [Arian eir] voice.' );
		...
		MainView.outputText( ' sit inside you.' );
		...
		if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
			MainView.outputText( 'twin bulbous shafts feel' );
		} else {
			MainView.outputText( 'bulbous shaft feels' );
		}
		MainView.outputText( ' exquisite, and ' );
		if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
			MainView.outputText( 'their' );
		} else {
			MainView.outputText( 'its' );
		}
		MainView.outputText( ' bumps massage your entrance' );
		if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ' unlike anything else.  You praise the lizan on being a quick study, letting go of [Arian eir] tail and grabbing onto [Arian eir] scaly shoulders instead, giving [Arian em] a few more directions so [Arian ey] can catch your most sensitive spots.' );
		...
		MainView.outputText( 'with [Arian eir] sticky wet seed.' );
		...
		MainView.outputText( ' swelling inside you, the lizan grabbing your shoulders and starting to thrust [Arian em]self back into you once more.' );
		...
		if( CoC.flags[ kFLAGS.ARIAN_VAGINA ] === 0 ) {
			MainView.outputText( '[Arian ey] himself ' );
		} else {
			MainView.outputText( 'she ' );
		}
		MainView.outputText( 'is being overwhelmed by, spraying your ' );
		if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] === 0 ) {
			MainView.outputText( 'cavity' );
		} else {
			MainView.outputText( 'cavities' );
		}
		MainView.outputText( ' with a second helping of lizan spunk, dredging up every last drop of jizz left in [Arian eir] internal balls before, with a groan, [Arian ey] sinks down atop you.  Arian\'s orgasm pushes you over the edge, and you find your pussy ' );
		if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
			MainView.outputText( 'and ass ' );
		}
		MainView.outputText( 'contracting, trying ' );
		if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] === 0 ) {
			MainView.outputText( 'its' );
		} else {
			MainView.outputText( 'their' );
		}
		MainView.outputText( ' best to milk the poor lizan of all [Arian ey] is worth, until finally with one last spasm, you slump down and release the lizan shaft' );
		if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ' deeply embedded inside you, some of your mixed juices running down to mat Arian\'s bed.' );
	...
	ArianScene.prototype.doublePenetrateArian = function() {
		...
		CoC.flags[ kFLAGS.ARIAN_VIRGIN ] += 1;
		this.arianHealth( 3 );
		MainView.outputText( ImageManager.showImage( 'arianfemale-home-doublepenetrate' ) );
		...
		MainView.outputText( 'You look over your feminine lizard lover, and feel your ' + Descriptors.multiCockDescriptLight() + ' starting to stir in your [armor].  Since you have enough tools for the job, and Arian has enough holes, you ask if Arian would be willing to let you fuck her in both her ass and pussy at the same time?' );
		...
		MainView.outputText( ' ass and dripping wet vagina; a pair of clawed hands reach behind to spread her butt open for you.  "<i>Okay, I\'m ready.</i>"  Arian says, looking behind at you.' );
		...
		//(Medium Cum Amount);
			...
			MainView.outputText( ' of their load; a load Arian is not only pleased to accept, but also eager to relieve you of every single stray drop off.  The tightness of her ass, pressing down on your ' + Descriptors.cockDescript( y ) + ', her pussy milking on your ' + Descriptors.cockDescript( x ) + '.  How could anyone refuse such an invitation?  You let yourself go, stuffing the eager lizan with more cum than you thought yourself capable of producing.' );
			MainView.outputText( '\n\n"<i>M-more,</i>" Arian pleads, even as her belly starts to distend.' );
		}
		//(High Cum Amount);
		else {
			MainView.outputText( '\n\nYou cum with such force, that if Arian\'s ass wasn\'t clamping down on your ' + Descriptors.cockDescript( y ) + ' so tightly, you\'d be sure she was going to get pushed off.  "<i>So much cum!  Yesssss!</i>" Arian moans, as you quickly give her usually lithe belly a very blatant bump.  Even though your prodigious amount of cum is enough to completely fill the eager lizan-girl, her ass and pussy work overtime to ensure you\'re completely spent; and you have no desire to resist.' );
		...
		if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_PENETRATION_CHAT ] === 0 ) {
			...
			if( !this.arianFollower() ) {
				MainView.outputText( 'bedchambers' );
			} else {
				MainView.outputText( 'tent' );
			}
			MainView.outputText( '.' );
	...
	ArianScene.prototype.arianDocking = function() {
		...
		this.arianHealth( 3 );
		if( CoC.flags[ kFLAGS.ARIAN_VAGINA ] > 0 ) {
			MainView.outputText( ImageManager.showImage( 'arianfemale-home-docking' ) );
		} else {
			MainView.outputText( ImageManager.showImage( 'arianmale-home-docking' ) );
		}
		MainView.outputText( 'You set your eyes on Arian\'s genital slit, and then smile at [Arian em].  You ask how [Arian ey]\'d feel about ‘hiding\' your cock, rather than [Arian eirs], inside [Arian eir] slit?' );
		...
		//(if ArianDblCock === 0);
		if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] === 0 ) {
			MainView.outputText( 'didn\'t [Arian ey] always want to have two dicks?  Well, now it\'s [Arian eir] chance to find out what it\'d be like.' );
		} else {
			MainView.outputText( '[Arian ey]\'s already hiding two in there... an extra cock won\'t make such a huge difference, would it?' );
		}
		...
		MainView.outputText( 'pecker' );
		if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ' from your prying eyes.' );
		...
		MainView.outputText( ' of Arian\'s dick' );
		if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( '.  You brush your fingers against the tip' );
		if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ', seeing what kind of reaction that will elicit from your scaly lover.' );
		...
		if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] === 0 ) {
			MainView.outputText( ' brushing up against [Arian eir] own lizan pecker' );
		} else {
			MainView.outputText( 'sandwiched between [Arian eir] two lizard dicks' );
		}
		MainView.outputText( ', the friction is absolutely incredible, sending sparks of pleasure cascading along your shaft.  You moan in delicious lust and tell Arian that this is absolutely incredible.' );
		...
		MainView.outputText( 'slides across your intruding shaft as you go, and [Arian eir] sheathe-walls grip and squeeze as best they can, leaving you moaning and panting like a bitch in heat.  Arian\'s moans of pleasure join yours, [Arian eir] twitching bulbous shaft' );
		if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ' massaging your [cock smallest] almost like a vagina would.  Shaky hands grab a hold of your shoulders, helping you down and up on Arian\'s male slit.  "<i>Oh, Marae!  It feels like I\'m getting an internal handjob... it feEels so... so good!  H - harder... almost cumming!</i>" [Arian ey] pleads, as [Arian ey] breathes hotly on your face; panting like a bitch in heat.' );
		...
		MainView.outputText( ' out before cumming.' );
		...
		if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
			MainView.outputText( 'pair of cummy masts emerge' );
		} else {
			MainView.outputText( 'cummy mast emerges' );
		}
		MainView.outputText( ' to feebly shoot a couple more ropes of jism, before returning limply to ' );
		if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
			MainView.outputText( 'their' );
		} else {
			MainView.outputText( 'its' );
		}
		MainView.outputText( ' hiding place.  "<i>Ahhh,</i>" is all Arian says, before slumping down.' );
	...
	ArianScene.prototype.giveArianAnItem = function() {
		MainView.clearOutput();
		MainView.outputText( 'Thinking about the many items in your possession, you ask if Arian would be willing to take something for you?' );
		...
		if( CoC.flags[ kFLAGS.ARIAN_S_DIALOGUE ] === 0 && this.arianHealth() >= 10 ) {
			...
			if( CoC.player.hasItem( ConsumableLib.VITAL_T ) ) {
				EngineCore.addButton( 0, 'Vital Tinct', this, this.arianVitalityTincture );
			}
			if( CoC.flags[ kFLAGS.ARIAN_HEALTH ] >= 20 ) {
				if( CoC.player.hasItem( ConsumableLib.P_DRAFT ) ) {
					EngineCore.addButton( 1, 'P. Incubi D', this, this.giveIncubusDraftToArian );
				}
				if( CoC.player.hasItem( ConsumableLib.P_S_MLK ) ) {
					EngineCore.addButton( 2, 'P. Suc.Milk', this, this.succubiMilkForArian );
				}
				if( CoC.player.hasItem( ConsumableLib.LACTAID ) ) {
					EngineCore.addButton( 3, 'Lactaid', this, this.giveArianLactaid );
				}
				if( CoC.player.hasItem( ConsumableLib.REDUCTO ) ) {
					EngineCore.addButton( 4, 'Reducto', this, this.giveArianReducto );
				}
				if( CoC.player.hasItem( ConsumableLib.REPTLUM ) ) {
					EngineCore.addButton( 5, 'Reptilum', this, this.giveArianReptilum );
				}
			}
			EngineCore.addButton( 9, 'Back', this, this.arianHomeMenu );
		}
	...
	ArianScene.prototype.arianVitalityTincture = function() {
		MainView.clearOutput();
		MainView.outputText( 'Fishing around amongst your pockets, you withdraw a vial of that strange potion Giacomo peddles and offer it to the sickly lizan, explaining it will bolster [Arian eir] constitution and fill [Arian em] with permanent vitality.' );
	...
	ArianScene.prototype.giveIncubusDraftToArian = function() {
		...
		MainView.outputText( 'Fishing around in your pockets, your hand closes on the vial of purified incubus draft.  You offer this to Arian, asking ' );
		if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] > 0 ) {
			MainView.outputText( 'if [Arian ey] would like to make [Arian eir] cock bigger' );
		} else {
			MainView.outputText( 'if she\'d like to have a touch of her old malehood back' );
		}
		MainView.outputText( '.' );
		...
		if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] === 0 ) {
			...
			if( CoC.player.hasVagina() ) {
				MainView.outputText( '[vagina] and ' );
			}
			...
			if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] === 0 ) {
				MainView.outputText( 'shaft emerges from its hiding place.  It throbs and grows, settling in its' );
			} else {
				MainView.outputText( 'pair of shafts emerge from their hiding place.  They throb and grow, settling in their' );
			}
			MainView.outputText( ' new size as Arian moans and cums all over [Arian em]self.' );
			...
			if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
				MainView.outputText( 's back into their' );
			} else {
				MainView.outputText( ' back into its' );
			}
			...
			if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] === 2 ) {
				...
				MainView.outputText( ' until two thirds are inside, then it doesn\'t looks like [Arian ey] can take in anymore.' );
				if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
				...
				if( CoC.flags[ kFLAGS.ARIAN_VAGINA ] > 0 ) {
					...
					MainView.outputText( ' starts hardening, no longer able to hide Arian\'s excitement inside her protective slit.' );
				} else {
	...
	ArianScene.prototype.succubiMilkForArian = function() {
		...
		MainView.outputText( 'Fishing out the bottle of purified demon\'s milk, you ask if Arian is willing to get ' );
		if( CoC.flags[ kFLAGS.ARIAN_VAGINA ] > 0 ) {
			MainView.outputText( 'further ' );
		}
		MainView.outputText( 'in touch with [Arian eir] feminine side.  For you?' );
		...
		if( CoC.flags[ kFLAGS.ARIAN_VAGINA ] === 0 ) {
			...
			MainView.outputText( ' having already softened and retracted' );
			//if ArianCockSize >= 3:;
			if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] === 3 ) {
				MainView.outputText( ' as far as it\'ll go' );
			}
			MainView.outputText( ' into its protective slit.' );
			...
			if( CoC.flags[ kFLAGS.ARIAN_BREASTS ] === 2 ) {
				...
				if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] >= 3 ) {
					MainView.outputText( ' and her exposed shaft begins to grow hard' );
				}
				MainView.outputText( '.' );
			} else {
				...
				if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] === 3 ) {
					MainView.outputText( ' and you see her exposed shaft slowly rising to point at you' );
				}
				MainView.outputText( '.' );
		...
		//(else If ArianCockSize > 1) //Dick shrinkage.;
			...
			MainView.outputText( '.  "<i>Oh... that\'s good.  Right?</i>" Arian looks at you nervously.  You think it over, and confess that while you are more attracted to women than men, you won\'t force Arian to become fully female if she doesn\'t want to.' );
	...
	ArianScene.prototype.giveArianLactaid = function() {
		...
		MainView.outputText( 'Your hand closes around the vial of lactation-inducing potion that is Lactaid.  You almost reject it automatically, but then you stop and think.  There\'s odder things in this world, after all.  You remove the vial and ask Arian if [Arian ey] would be willing to let you see what lizan milk tastes like.' );
		...
		if( CoC.flags[ kFLAGS.ARIAN_BREASTS ] === 0 ) {
			...
			if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] === 3 ) {
				MainView.outputText( ', her exposed cock growing hard as she flushes with arousal' );
			}
			MainView.outputText( '.  Her hands grip the sheets and you watch her ' );
			if( CoC.flags[ kFLAGS.ARIAN_BREASTS ] === 1 ) {
				MainView.outputText( 'perky' );
			} else if( CoC.flags[ kFLAGS.ARIAN_BREASTS ] === 2 ) {
				MainView.outputText( 'luscious' );
			} else {
				MainView.outputText( 'pillowy' );
			}
			MainView.outputText( ' breasts grow before your eyes.  "<i>My breasts... d-do something, [name]!</i>" Arian pleads.  Unsure of what you should be doing, you grab her breasts and begin kneading them as they grow in your hands; her erect nipples poking your palms.  "<i>Ah... that feels good!  Don\'t stop!</i>"' );
	...
	ArianScene.prototype.giveArianReducto = function() {
		MainView.clearOutput();
		...
		if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] > 0 ) {
			EngineCore.addButton( 1, 'Cock', this, this.useReductoOnArianCocks );
		}
		EngineCore.addButton( 2, 'Asshole', this, this.useReductoOnAriansAsshole );
	};
	//Breasts:;
	//Cannot go flat;
	ArianScene.prototype.useReductoOnAriansBreasts = function() {
		...
		MainView.outputText( 'You point at Arian\'s ' );
		if( CoC.flags[ kFLAGS.ARIAN_BREASTS ] === 1 ) {
			MainView.outputText( 'small' );
		} else if( CoC.flags[ kFLAGS.ARIAN_BREASTS ] === 2 ) {
			MainView.outputText( 'generous' );
		} else {
			MainView.outputText( 'pillowy' );
		}
		MainView.outputText( ' breasts.  "<i>Okay... I suppose it would be good to lose a bit of weight up here and spare my back.</i>"  Arian smiles, opening the tube of Reducto and her robes; then squeezing the contents of the paste her hands.' );
		...
		if( CoC.flags[ kFLAGS.ARIAN_BREASTS ] > 1 ) {
			...
			MainView.outputText( 'The two of you watch Arian\'s breasts, but nothing seems to be changing.  You reach forward to feel them, and indeed they haven\'t shrunk down at all.' );
	...
	ArianScene.prototype.useReductoOnArianCocks = function() {
		MainView.clearOutput();
		CoC.player.consumeItem( ConsumableLib.REDUCTO );
		MainView.outputText( 'You point at [Arian eir] crotch, mentioning that you\'d like [Arian em] to be smaller.' );
		...
		if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] === 3 ) {
			...
			if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
				MainView.outputText( 'pair of exposed lizard cocks' );
			} else {
				MainView.outputText( 'exposed lizard cock' );
			}
			MainView.outputText( ' into full erection.  Finally [Arian ey] begins applying the paste.' );
			MainView.outputText( '\n\nThe changes are almost immediate; Arian groans and watches as [Arian eir] oversized dick' );
			if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ' slowly shrink' );
			if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] === 0 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ' to a more manageable size.  Once the transformation is complete Arian tries to fully retract [Arian eir] cock' );
			if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
				MainView.outputText( 's back into their' );
			} else {
				MainView.outputText( ' back into its' );
			}
			MainView.outputText( ' hiding place; it\'s a tight fit, but it fits.' );
			...
			if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
				MainView.outputText( 's from their tight hiding place' );
			} else {
				MainView.outputText( ' from its tight hiding place' );
			}
			MainView.outputText( ', and into full erection.  Finally [Arian ey] begins applying the Reducto.' );
			MainView.outputText( '\n\nIt takes mere moments for the changes to occur; Arian groans in slight discomfort as [Arian eir] cock' );
			if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
				MainView.outputText( 's return to their' );
			} else {
				MainView.outputText( ' returns to its' );
			}
			MainView.outputText( ' original size.  Once the transformation is complete, [Arian ey] retracts ' );
			if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
				MainView.outputText( 'them' );
			} else {
				MainView.outputText( 'it' );
			}
			MainView.outputText( ' back into [Arian eir] genital slit.' );
			...
			MainView.outputText( 'Then, [Arian ey] coaxes [Arian eir] twin reptilian-peckers out of their hiding place.  Finally [Arian ey] applies the paste to both shafts in turn.' );
	...
	ArianScene.prototype.useReductoOnAriansAsshole = function() {
		MainView.clearOutput();
		CoC.player.consumeItem( ConsumableLib.REDUCTO );
		MainView.outputText( 'You ask Arian to hand the tube of reducto back over to you, telling [Arian em] that you want to make [Arian em] a little tighter when you do [Arian em] from behind.  The lizard-' );
		if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] > 0 ) {
			if( CoC.flags[ kFLAGS.ARIAN_VAGINA ] === 0 ) {
				MainView.outputText( 'man' );
			} else {
		...
		MainView.outputText( ' does as you ask, allowing you to smear your fingers generously with the shrinking cream, even as [Arian ey] anticipates your next request and removes [Arian eir] clothes before lying down, bum up in the air and tail slowly swishing to and fro, [Arian eir] anus exposed and waiting for your ministrations.' );
		//(if AnalXP === 1);
		...
		//(else if AnalXP < 33);
			...
			if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] > 0 ) {
				MainView.outputText( ', [Arian eir] cock' );
				if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
					MainView.outputText( 's suddenly jolting from their sheathe' );
				} else {
					MainView.outputText( ' suddenly jolting from its sheathe' );
				}
				if( CoC.flags[ kFLAGS.ARIAN_VAGINA ] > 0 ) {
					MainView.outputText( ' and ' );
				}
	...
	ArianScene.prototype.giveArianReptilum = function() {
		...
		MainView.outputText( 'Fingering the vial of reptilium, you smirk to yourself.  Quickly wiping it off your face, you instruct Arian to close [Arian eir] eyes and open [Arian eir] mouth, as you have a special surprise for [Arian em].' );
		...
		if( !CoC.player.hasVagina() && !CoC.player.hasCock() ) {
			MainView.outputText( 'genderless crotch' );
		}
		MainView.outputText( '.  "<i>Hmm, that looks much tastier than candy actually.</i>"  [Arian Ey] licks your ' );
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'cock from base to tip' );
		} else if( CoC.player.hasVagina() ) {
			MainView.outputText( 'pussy all the way from the back to the tip of your [clit]' );
		} else {
			MainView.outputText( 'your sweaty crotch' );
		}
		MainView.outputText( '.  "<i>Yummy!  I could use seconds, but roleplay time is over; let\'s... masturbate each other!</i>"  [Arian Ey] begins stroking [Arian eir] ' );
		if( CoC.flags[ kFLAGS.ARIAN_VAGINA ] > 0 ) {
			MainView.outputText( ImageManager.showImage( 'arianfemale-home-mutualmasturbation' ) );
		} else {
			MainView.outputText( ImageManager.showImage( 'arianmale-home-mutualmasturbation' ) );
		}
		if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] === 1 ) {
			MainView.outputText( 'quickly erecting lizan cock' );
			if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
				MainView.outputText( 's' );
			}
			if( CoC.flags[ kFLAGS.ARIAN_VAGINA ] > 0 ) {
		...
		if( CoC.flags[ kFLAGS.ARIAN_VAGINA ] > 0 ) {
			MainView.outputText( 'slavering vagina' );
		}
		MainView.outputText( '.' );
		...
		if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] > 0 ) {
			MainView.outputText( 'shooting thick ropes of cum up into the air to land on [Arian eir] ' );
			if( CoC.flags[ kFLAGS.ARIAN_BREASTS ] === 0 ) {
				MainView.outputText( 'chest' );
			} else if( CoC.flags[ kFLAGS.ARIAN_BREASTS ] === 1 ) {
				MainView.outputText( 'perky little breasts' );
			} else if( CoC.flags[ kFLAGS.ARIAN_BREASTS ] === 2 ) {
				MainView.outputText( 'soft breasts' );
			} else {
				MainView.outputText( 'pillowy breasts' );
			}
			if( CoC.flags[ kFLAGS.ARIAN_VAGINA ] > 0 ) {
		...
		if( CoC.flags[ kFLAGS.ARIAN_VAGINA ] > 0 ) {
			MainView.outputText( 'her vagina doing its best to soak her bed with lizan femcum' );
		}
		MainView.outputText( '.' );
	...
	ArianScene.prototype.treatCorruption = function() {
		MainView.clearOutput();
		MainView.outputText( 'You ask Arian if [Arian ey] thinks [Arian ey] can help you reduce some of the taint that has infected your soul.' );
		if( CoC.flags[ kFLAGS.ARIAN_TREATMENT ] === 1 ) {
			...
			if( CoC.player.cor === 0 ) {
				MainView.outputText( ' - in fact, you\'re quite sure that Arian has purified you entirely' );
			}
			MainView.outputText( '.  You thank the lizan for [Arian eir] magical treatment.' );
	...
	ArianScene.prototype.imbueTalisman = function() {
		MainView.clearOutput();
		MainView.outputText( 'You tell Arian that, if it\'s not too much trouble, you\'d like [Arian em] to ' );
		if( CoC.player.hasKeyItem( 'Arian\'s Talisman' ) >= 0 ) {
			MainView.outputText( 'place a spell in the enchanted talisman [Arian ey] created for you' );
		} else {
			MainView.outputText( 'change the spell in the talisman [Arian ey] created for you' );
		}
		...
		/*The list Spell: 2x Wet Cloth and 2x Vitality T. - Heals the PC, no chance for failure.
		 Lust Reduction Spell Lust Draft and 1x Fuck Draft. - Reduces the PC's current lust, no chance for failure.
		 Shielding Spell Black Chitin and 1x Tough Silk. - Increases defense for the duration of the battle.
		 Dispelling Spell White Book and 1x Black Book - Cancels all magical and alchemical effects currently affecting the PC, beneficial or not. (Does not cancel the effects of demon's lust aura.)
		 Immolation Spell Goblin Ale and 1x Sweet Gossamer. - Deals damage over time.
		...
		if( CoC.player.hasItem( ConsumableLib.GOB_ALE, 2 ) && CoC.player.hasItem( ConsumableLib.S_GOSSR ) ) {
			EngineCore.addButton( 1, 'Immolation', this, this.arianSpellPlace, 'Immolation Spell' );
		}
		EngineCore.addButton( 9, 'Back', this, this.arianHomeMenu );
	};
	ArianScene.prototype.arianSpellPlace = function( spell ) {
		MainView.clearOutput();
		MainView.outputText( 'You tell Arian that you want [Arian em] to place the ' + spell + ' spell in your talisman for you.' );
	...
	//Sleep With Arian;
	ArianScene.prototype.sleepWithArian = function( newl ) {
		if( newl ) {
			MainView.clearOutput();
		}
		CoC.flags[ kFLAGS.SLEEP_WITH ] = 'Arian';
		MainView.outputText( 'Tired after a whole day of adventuring, you decide to retire and catch some shut-eye.  While going through the day\'s events, you recall Arian had offered to let you stay in [Arian eir] tent and sleep with [Arian em] in [Arian eir] bed.  Your tired body could surely use a soft bed today, and maybe a certain lizan to keep you company too.  With that in mind, you head to [Arian eir] tent.' );
		...
		if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] > 0 ) {
			MainView.outputText( 'You can clearly see [Arian eir] exposed cock' );
			if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ' stiffening at the sight.  ' );
		}
		if( CoC.flags[ kFLAGS.ARIAN_VAGINA ] > 0 ) {
			MainView.outputText( 'You can tell that [Arian ey] is aroused by the little show, despite [Arian eir] body doing a good job of hiding it.  ' );
		}
		MainView.outputText( 'You just let [Arian em] squirm, pretending you don\'t see [Arian eir] reactions, and then, indicating the bed, you ask which of you should get in first.' );
	...
	ArianScene.prototype.listenToLowAnalXPArian = function() {
		...
		MainView.outputText( 'You turn you head to try and catch what Arian might be dreaming about.' );
	...
	ArianScene.prototype.listenToMediumAnalXPArian = function() {
		...
		if( CoC.player.hasCock() ) {
			...
			MainView.outputText( ' still worming its way into [Arian eir] fuckable ass, but it really does look like [Arian ey]\'s fast asleep.  Weird.  "<i>So many muffins... all [name] flavored...</i>"  [Arian ey] calms down and begins snoring lightly.' );
			...
			if( CoC.player.cor >= 66 ) {
				MainView.outputText( ', despite the silly lizan being deserving of it' );
			}
			MainView.outputText( '.  But as payback, [Arian ey]\'s going to sleep with you inside, not that it seems like it\'ll be much of a problem to [Arian em]... Too bad you can\'t hold an erection all the time while asleep, so you know eventually you\'ll slip out...' );
		} else {
	...
	ArianScene.prototype.TeaseHighAnalXPArian = function() {
		MainView.clearOutput();
		if( CoC.flags[ kFLAGS.ARIAN_VAGINA ] > 0 ) {
			MainView.outputText( ImageManager.showImage( 'arianfemale-camp-dreamingArian' ) );
		} else {
			MainView.outputText( ImageManager.showImage( 'arianmale-camp-dreamingArian' ) );
		}
		CoC.flags[ kFLAGS.ARIAN_MORNING ] = 1;
	...
	ArianScene.prototype.wakeUpAfterArianSleep = function() {
		...
		if( CoC.player.hasCock() ) {
			...
			if( CoC.flags[ kFLAGS.ARIAN_ANAL_XP ] < 33 ) {
				...
				MainView.outputText( 'Your eyes open, and you suddenly realize that whatever dreams you were having... are now gone.  You make note of the still sleeping lizan right next to you and recall last night\'s events.  You wonder if Arian will remember any of that...' );
				...
				if( CoC.flags[ kFLAGS.ARIAN_VAGINA ] > 0 ) {
					MainView.outputText( this.arianMF( 'his', 'her' ) + ' pussy is squirting her juices all over your lower body' );
					if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] > 0 ) {
						MainView.outputText( ' and ' );
					} else {
						MainView.outputText( '; ' );
					...
					MainView.outputText( this.arianMF( 'his', 'her' ) + ' ' );
					if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
						MainView.outputText( 'twin shafts are' );
					} else {
						MainView.outputText( 'shaft is' );
					}
					MainView.outputText( ' squirting white ropes of lizan cum all over the bedsheets;' );
				}
				MainView.outputText( ' what a naughty little lizan.' );
				MainView.outputText( '\n\nYou can\'t resist playfully teasing Arian that [Arian ey] is such a buttslut.  ' );
				if( !CoC.player.hasVagina() ) {
					MainView.outputText( 'Did [Arian ey] always know [Arian ey] wanted a man to top [Arian em], or is this development new to [Arian em]?' );
				} else {
					MainView.outputText( 'Does [Arian ey] really get off on having a girl with a dick all her own fuck [Arian em] like a girl, hmm?' );
				}
			...
			if( CoC.flags[ kFLAGS.ARIAN_ANAL_XP ] < 33 ) {
				...
				if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] > 0 ) {
					...
					MainView.outputText( 'bulbous shaft' );
					if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
						MainView.outputText( 's begin' );
					} else {
						MainView.outputText( ' begins' );
					}
					MainView.outputText( ' sporting an erection' );
					if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] < 3 ) {
						MainView.outputText( ', poking ' );
						if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] === 1 ) {
							MainView.outputText( 'its tip' );
						} else {
							MainView.outputText( 'their tips' );
						}
						MainView.outputText( ' out of Arian\'s genital slit' );
					}
					MainView.outputText( '.' );
				} else if( CoC.flags[ kFLAGS.ARIAN_VAGINA ] > 0 ) {
					MainView.outputText( 'and her moist pussy, begins forming a stream of juices that gently run its course through the inside of Arian\'s thighs, evaporating before hitting the floor due to the lizan\'s currently elevated body heat.' );
				}
				MainView.outputText( '  [Arian Ey] pants, eyes glazed.  "<i>Uhh... [name]...</i>"' );
	...
	ArianScene.prototype.arianEggingEvent = function() {
		...
		if( CoC.flags[ kFLAGS.ARIAN_EGG_CHAT ] === 0 ) {
			...
			MainView.outputText( 'As you are about to enter Arian\'s tent, you hear a moan emanate from within.  Those aren\'t moans of pleasure though.  They are moans of discomfort.  Wondering if the lizan is all right, you decide to enter her tent.' );
			...
			MainView.outputText( 'As you are about to enter Arian\'s tent, you hear a moan emanate from within.  Seems like it\'s that time of the month again...' );
		...
		EngineCore.addButton( 3, 'Pink', this, this.pickAnEggArian, 'pink' );
		EngineCore.addButton( 4, 'White', this, this.pickAnEggArian, 'white' );
		EngineCore.addButton( 5, 'Rubber', this, this.pickAnEggArian, 'rubbery black' );
		EngineCore.addButton( 6, 'No Thanks', this, this.leaveEggs );
	};
	//Pick a color;
	ArianScene.prototype.pickAnEggArian = function( color ) {
		if( color === undefined ) {
			color = 'pink';
		}
		MainView.clearOutput();
		CoC.flags[ kFLAGS.ARIAN_EGG_COLOR ] = color;
		MainView.outputText( 'You tell Arian you\'d like her to make you a ' + color + ' egg.' );
	...
	ArianScene.prototype.leaveEggs = function() {
		MainView.clearOutput();
		MainView.outputText( 'You tell her that you don\'t want any colored eggs from her this month.  The lizan nods, deciding it\'s not necessary for her to spell out that this means she\'ll just produce ordinary eggs and eat them for breakfast after she\'s laid them.  "<i>So, do you want something?</i>" she asks.' );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	...
	ArianScene.prototype.arianLaysEggs = function() {
		...
		MainView.outputText( 'As you approach the tent, you wonder if Arian\'s ready to lay those eggs...  Your contemplations are promptly interrupted as you hear a pained moan coming from inside.  Seems like Arian is in labor!  Without delay, you rush inside, to be greeted by the sight of naked lizan-' );
		if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] > 0 ) {
			MainView.outputText( 'herm' );
		} else {
			MainView.outputText( 'girl' );
		}
		MainView.outputText( ' squatting over a wooden basin, one hand between her legs, massaging her dripping treasure, while the other is rubbing her belly to try and coax the eggs out of her.' );
	...
	ArianScene.prototype.arianDildoFun = function() {
		...
		if( CoC.flags[ kFLAGS.TIMES_ARIAN_DILDOED ] === 0 ) {
			MainView.outputText( 'Looking over your reptilian girlfriend, your eyes go down her body' );
			if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] > 0 ) {
				MainView.outputText( ', past her cock' );
				if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
					MainView.outputText( 's' );
				}
			} else {
			...
			MainView.outputText( ' to her dripping wet pussy, clearly yearning for some action.  Reflecting that your own love-lips feel the same way, you decide to try something fun and present to Arian the dildo that you ' );
			if( CoC.player.hasKeyItem( 'Dildo' ) >= 0 ) {
				MainView.outputText( 'bought from Giacomo' );
			} else {
				MainView.outputText( 'got from Tamani' );
			}
			MainView.outputText( ', asking if she\'s willing to give this a try.' );
			...
			MainView.outputText( 'You present your dildo to Arian again, telling her that this time, you want to play with your toys - after all, she enjoyed it so much the first time.' );
		...
		if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] > 0 ) {
			MainView.outputText( 'Ignoring her cock' );
			if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ', y' );
		...
		MainView.outputText( 'ou slide first one finger into her slick pussy and then another, gently caressing her inner labia to see if you can find her sweet spots.  Arian\'s eyes open in surprise as the first pang of pleasure hits her, she moans into your kiss.  Her tongue darts to dance with your own in excitement.' );
		...
		if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] > 0 ) {
			MainView.outputText( 'brushing it tantalizingly across the sensitive skin of her pre-oozing cock' );
			if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ' before' );
		}
		MainView.outputText( ' brushing it against her outer lips, stroking it up and down each labia, rubbing it against her clitoris, gently pushing the very tip of it into her cunt only to then withdraw it before it can really start to stretch her cunt.' );
		...
		if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] > 0 ) {
			MainView.outputText( 'Her cock' );
			if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_COCK ] > 0 ) {
				MainView.outputText( 's throb' );
			} else {
				MainView.outputText( 'throbs' );
			}
			MainView.outputText( ' throb as a jet of whiteness escape them.  ' );
		}
		MainView.outputText( 'You can feel it... the suddenness of your penetration brought the poor lizan mage to an instant orgasm.' );
