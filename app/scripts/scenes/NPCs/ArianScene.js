...
angular.module( 'cocjs' ).run( function( MainView, SceneLib, UseableLib, $rootScope, Descriptors, ImageManager, AppearanceDefs, CoC, kFLAGS, Utils, StatusAffects, EngineCore, ConsumableLib ) {
	...
	ArianScene.prototype.visitThePark = function() {
		...
		if( CoC.flags[ kFLAGS.ARIAN_PARK ] === 1 ) {
			...
			EngineCore.outputText( ', you decide to ask him what his story is.' );
	...
	ArianScene.prototype.visitAriansHouse = function() {
		...
		if( this.arianFollower() ) {
			EngineCore.outputText( 'You approach the enchanted tent and slip easily inside the doors to the luxurious interior.  ' );
			var temp = Utils.rand( 10 );
			if( temp === 0 ) {
				EngineCore.outputText( 'However, Arian isn\'t here right now, so you instead make yourself comfortable on the couch.  After a few minutes, Arian [Arian emself] walks in through the entrance.  "<i>Oh, [name].  I wasn\'t aware you were here... have you been waiting for long?</i>" [Arian ey] asks.  You tell [Arian em] not very long.  "<i>That\'s good to hear.  So, what can I do for you?</i>" [Arian ey] asks, with a smile.' );
			} else if( temp === 1 ) {
				EngineCore.outputText( 'Inside, the lizan is sitting at a table, fastidiously drinking from a cup of something hot while pouring over an arcane-looking text.  You politely cough to draw [Arian eir] attention and [Arian ey] looks at you, smiling.  "<i>Hello, [name].  I was just catching up on my studies.  Can I offer you a cup of tea, or maybe something else?</i>" [Arian ey] asks.' );
			} else if( temp <= 2 ) {
				EngineCore.outputText( 'The lizan is currently busy tinkering with some occult-looking paraphernalia when you find [Arian em].  You politely cough to attract [Arian eir] attention, then do so again when [Arian ey] fails to heed that.  It\'s only on the third attempt that [Arian ey] looks up apologetically from [Arian eir] work.  "<i>Ah, [name]; I\'m sorry, but I was preoccupied with something.</i>" [Arian ey] states in an apologetic tone, indicating the mess on [Arian eir] desk.  "<i>Was there something you wanted?</i>" [Arian ey] asks.' );
			} else if( temp <= 4 ) {
				EngineCore.outputText( 'The smell of fresh cooking fills the air and you can see Arian happily sitting down at his couch with a plate of something just cooked.  "<i>Oh, [name]; I was just about to eat, can I offer you a dish?  Or if you\'d rather do something else, this can wait,</i>" the lizan tells you with a smile.' );
			} else if( temp <= 6 ) {
				...
				EngineCore.outputText( '.  For a long moment, [Arian ey] just stares back at you; if lizans could blush, you\'re certain [Arian ey]\'d be red as a beet.  "<i>[name]! I was- I was just....  Oh, this is embarrassing,</i>" [Arian ey] mutters, looking at [Arian eir] feet.' );
			...
			if( CoC.flags[ kFLAGS.ARIAN_PARK ] === 4 ) {
				...
				EngineCore.outputText( ' ask why he\'s thanking you.' );
				...
				EngineCore.outputText( '.  You rack your brains for polite conversation, and finally ask how he\'s been since you saw him last.' );
	...
	ArianScene.prototype.eavesDropOnArian = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'You sidle up to the door, pressing your ear against the wood and start to listen intently.' );
	...
	ArianScene.prototype.leaveFappingArian = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'You decide to let Arian have some privacy and leave for the moment... after all, what the lizan mage does in his free time is not really your business....' );
	...
	ArianScene.prototype.bargeInOnArian = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'With a wry smirk you turn the knob and find that Arian\'s door is unlocked; without missing a beat, you open the door and step in right in time to see a sticky rope of pre paint Arian\'s slender belly as he scrambles to cover himself up.' );
	...
	ArianScene.prototype.hermsLikeMaleArian = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'You tell him that\'s not the case for you; you don\'t have a problem with him being a guy.  In fact, you think he looks very cute, earning you a nervous smile.  Arian relaxes, letting you look over his body and decide what you want to do....' );
	...
	ArianScene.prototype.hermsLikeFemaleArian = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'You tell him that while you do like to play with guys once in a while, you prefer girls.' );
	...
	ArianScene.prototype.youDontMindBeingGayForArian = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'You tell him that you don\'t have a problem with males, as long as they\'re cute.  You smile at him.  "<i>You... do you really think I\'m cute?</i>"' );
	...
	ArianScene.prototype.youLikeGirlsNotSickLizardDudes = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'You tell him that you prefer females.... Arian looks at you expectantly.  "<i>So... if I was a girl... then you wouldn\'t mind?</i>"' );
	...
	ArianScene.prototype.arianStoryDialogue1 = function() {
		this.arianHealth( 1 );
		EngineCore.clearOutput();
		EngineCore.outputText( 'You feel like you\'d like to know a bit more about Arian, so you ask if he would mind sharing some of [Arian eir] history with you.  After all, as a survivor from at least the early days of the demon war, and a wizard to boot, he\'s got to have some stories up [Arian eir] voluminous sleeves.' );
	...
	ArianScene.prototype.arianStoryDialogue2 = function() {
		EngineCore.clearOutput();
		this.arianHealth( 1 );
		EngineCore.outputText( 'You look Arian over, remarking that he seems to be getting better after all.' );
	...
	ArianScene.prototype.noArianShouldntMagicTeach = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'You think it over for a moment, and then tell Arian that while you are flattered by the offer and willing to consider it, you can\'t say that you want to study magic right this moment.  You\'d like to discuss it at some other time, please.' );
	...
	ArianScene.prototype.arianImbue = function() {
		EngineCore.clearOutput();
		this.arianHealth( 1 );
		EngineCore.outputText( 'Before you can say anything, Arian gasps, "<i>Oh, [name].  I have a surprise for you.</i>"  Arian says with a smile.' );
	...
	ArianScene.prototype.yesPlotSexArian = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'You approach the awkwardly amorous lizan and place your arms around [Arian eir] neck.  Leaning in close, you whisper into [Arian eir] ear that [Arian ey] only had to ask.' );
		//(Display Sex Menu);
		this.arianSexMenu( false );
	};
	//=No=;
	ArianScene.prototype.noPlotSexNauArian = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'You apologize to the lizan, telling [Arian em] that you aren\'t in the mood right now....' );
	...
	ArianScene.prototype.arianPlot4 = function() {
		EngineCore.clearOutput();
		this.arianHealth( 1 );
		EngineCore.outputText( 'Before you can say anything, Arian says, "<i>Oh, I have good news, [name]!</i>"' );
	...
	ArianScene.prototype.arianPlot5 = function() {
		EngineCore.clearOutput();
		this.arianHealth( 1 );
		EngineCore.outputText( 'Before you can say anything, Arian stops you.  "<i>I\'ve been meaning to ask you something, [name].  I\'ve been feeling a lot better lately; in fact, I may be even better than I was before.</i>"  Arian blushes.' );
	...
	ArianScene.prototype.acceptArianMovingIntoCamp = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'You tell Arian you\'d be delighted to have [Arian em] move in with you.  Arian\'s face lights up like a kid\'s who\'s been given a bucket of candy.  "<i>Really!?  Great!  I\'ll pack my stuff and we can go right away!</i>"' );
	...
	ArianScene.prototype.denyAriansMoveIn = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'You tell Arian you\'d like some time to think about it.  Arian looks disappointed at first, but smiles at you all the same.  "<i>I understand... no pressure....  So, what are we going to do today?</i>"' );
		//(Display Options);
		this.arianHomeMenu();
	};
	//Talk;
	...
	ArianScene.prototype.arianMagicLessons = function() {
		...
		if( CoC.player.inte < 25 ) {
			...
			if( !this.arianFollower() ) {
				EngineCore.outputText( 'make your way back to your camp.' );
			} else {
				EngineCore.outputText( 'return to your duties.' );
			}
		}
		//(else if PC int < 50 //Teach Charge Weapon if PC doesn't know.;
			...
			if( !this.arianFollower() ) {
				EngineCore.outputText( ', making your way back to camp.' );
			} else {
				EngineCore.outputText( ', exiting [Arian eir] tent and going about your business.' );
			}
			...
			if( !this.arianFollower() ) {
				EngineCore.outputText( 'begin the trek home.' );
			} else {
	...
	ArianScene.prototype.arianSexingTalk = function() {
		EngineCore.clearOutput();
		this.arianHealth( 1 );
		EngineCore.outputText( 'You smirk knowingly at [Arian em] and ask how [Arian ey] feels about sex now that [Arian ey]\'s had [Arian eir] first time?' );
	...
	ArianScene.prototype.yesYouButtslutIllFuckYou = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'How could you refuse such a request?  You tell [Arian em] to strip and get ready.' );
	...
	ArianScene.prototype.goddamnitNoYouButtSlut = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'You apologize, but you really can\'t do that right now.  Arian looks a bit disappointed, but smiles at you all the same.  "<i>Oh... okay.  Next time then?</i>"' );
		EngineCore.outputText( '\n\nYou nod.' );
	...
	ArianScene.prototype.takeYerLizardHomePartII = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'Upon arriving at the camp, the first thing Arian notices is the shimmering portal.  "<i>Is this... where you came from?</i>" Arian asks.' );
	...
	ArianScene.prototype.giveArianAnal = function() {
		...
		//Loose, sensitive enough to make Arian scream in pleasure. (It's like a tight vagina, feels like one too, minor pain);
			...
			if( this.arianFollower() ) {
				EngineCore.outputText( 'tent' );
			} else {
				EngineCore.outputText( 'bedchamber' );
			}
		...
		//Very Loose, sensitive enough to make Arian cum just from insertion. (Feels better than anything else. Yep, Arian really becomes a buttslut at this point);
			...
			if( CoC.flags[ kFLAGS.ARIAN_ASS_CHAT ] === 0 ) {
				...
				if( 9999 === 9999 ) {
					EngineCore.outputText( ' as hypocritical as that may be' );
				}
				EngineCore.outputText( '... besides, you think [Arian ey]\'s kind of sexy when [Arian ey] gets like that.  At that Arian perks up.  "<i>Really?</i>"  Yes, really, you reply.  Arian smiles happily at you.  "<i>So... do you want to go again?</i>"' );
				...
				if( !this.arianFollower() ) {
					EngineCore.outputText( 'back to Tel\'Adre\'s streets, and from there back to camp.' );
				} else {
	...
	ArianScene.prototype.getBlownByArian = function() {
		...
		if( !this.arianFollower() ) {
			EngineCore.outputText( ' back to camp, stopping only to notify Laika and Boon that Arian is sleeping.' );
		} else {
	...
	ArianScene.prototype.penetrateArian = function() {
		...
		if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] === 3 ) {
			...
			EngineCore.outputText( ' hiding place, already at full mast in anticipation' );
		}
		EngineCore.outputText( '.  Arian swallows audibly.  "<i>What now?</i>" she asks, already panting in lust.' );
		...
		EngineCore.outputText( ' lizan, until you have managed to hilt yourself inside of her.' );
		...
		if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] > 0 ) {
			...
			EngineCore.outputText( ' bobbing just out of your field of vision, leaking pre all over your belly.' );
		}
		...
		if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] > 0 ) {
			...
			EngineCore.outputText( 'a small rope of pre over her belly, anticipating what\'s to come as she braces herself for the inevitable pounding you\'re about to give her.' );
		}
	...
	ArianScene.prototype.getButtWreckedByArian = function() {
		...
		EngineCore.outputText( ' back to full mast, even surprising you with Arian\'s eagerness.' );
	...
	ArianScene.prototype.suckAriansDick = function() {
		...
		EngineCore.outputText( 'You make a show of lewdly licking your lips and ask Arian if [Arian ey]\'d be willing to let you have a little taste of lizan essence...?' );
		...
		if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] > 3 ) {
			...
			EngineCore.outputText( ' hiding place, already fully erect' );
			EngineCore.outputText( '.' );
		}
		...
		EngineCore.outputText( ' rolling your neck in pleasure as [Arian ey] rubs it with [Arian eir] reptilian tail.' );
	...
	ArianScene.prototype.doublePenetrateArian = function() {
		...
		if( CoC.flags[ kFLAGS.ARIAN_DOUBLE_PENETRATION_CHAT ] === 0 ) {
			...
			if( !this.arianFollower() ) {
				EngineCore.outputText( 'bedchambers' );
			} else {
				EngineCore.outputText( 'tent' );
			}
			EngineCore.outputText( '.' );
	...
	ArianScene.prototype.arianDocking = function() {
		...
		EngineCore.outputText( 'You set your eyes on Arian\'s genital slit, and then smile at [Arian em].  You ask how [Arian ey]\'d feel about ‘hiding\' your cock, rather than [Arian eirs], inside [Arian eir] slit?' );
		...
		EngineCore.outputText( ' hiding place.  "<i>Ahhh,</i>" is all Arian says, before slumping down.' );
	...
	ArianScene.prototype.arianVitalityTincture = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'Fishing around amongst your pockets, you withdraw a vial of that strange potion Giacomo peddles and offer it to the sickly lizan, explaining it will bolster [Arian eir] constitution and fill [Arian em] with permanent vitality.' );
	...
	ArianScene.prototype.giveArianLactaid = function() {
		...
		if( CoC.flags[ kFLAGS.ARIAN_BREASTS ] === 0 ) {
			...
			EngineCore.outputText( ' breasts grow before your eyes.  "<i>My breasts... d-do something, [name]!</i>" Arian pleads.  Unsure of what you should be doing, you grab her breasts and begin kneading them as they grow in your hands; her erect nipples poking your palms.  "<i>Ah... that feels good!  Don\'t stop!</i>"' );
	...
	ArianScene.prototype.giveArianReducto = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'Eyeing Arian up and down, you fish your pouches for a tube of Reducto.  Once you\'ve found it, you hand it over to Arian and tell [Arian em] you\'d like [Arian em] to reduce something for you.' );
	...
	ArianScene.prototype.arianSpellPlace = function( spell ) {
		EngineCore.clearOutput();
		EngineCore.outputText( 'You tell Arian that you want [Arian em] to place the ' + spell + ' spell in your talisman for you.' );
		...
		if( CoC.player.hasCock() ) {
			...
			if( CoC.flags[ kFLAGS.ARIAN_ANAL_XP ] < 33 ) {
				...
				if( CoC.flags[ kFLAGS.ARIAN_VAGINA ] > 0 ) {
					...
					EngineCore.outputText( ' squirting white ropes of lizan cum all over the bedsheets;' );
				}
	...
	ArianScene.prototype.arianEggingEvent = function() {
		...
		if( CoC.flags[ kFLAGS.ARIAN_EGG_CHAT ] === 0 ) {
			...
			EngineCore.outputText( 'As you are about to enter Arian\'s tent, you hear a moan emanate from within.  Seems like it\'s that time of the month again...' );
	...
	ArianScene.prototype.arianDildoFun = function() {
		...
		if( CoC.flags[ kFLAGS.TIMES_ARIAN_DILDOED ] === 0 ) {
			...
			EngineCore.outputText( ', asking if she\'s willing to give this a try.' );
		...
		EngineCore.outputText( 'ou slide first one finger into her slick pussy and then another, gently caressing her inner labia to see if you can find her sweet spots.  Arian\'s eyes open in surprise as the first pang of pleasure hits her, she moans into your kiss.  Her tongue darts to dance with your own in excitement.' );
		...
		EngineCore.outputText( ' brushing it against her outer lips, stroking it up and down each labia, rubbing it against her clitoris, gently pushing the very tip of it into her cunt only to then withdraw it before it can really start to stretch her cunt.' );
		...
		if( CoC.flags[ kFLAGS.ARIAN_COCK_SIZE ] > 0 ) {
			...
			EngineCore.outputText( ' throb as a jet of whiteness escape them.  ' );
		}
		EngineCore.outputText( 'You can feel it... the suddenness of your penetration brought the poor lizan mage to an instant orgasm.' );
