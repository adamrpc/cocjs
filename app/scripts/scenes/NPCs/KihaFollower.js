...
angular.module( 'cocjs' ).run( function( Kiha, SpiderMorphMob, ConsumableLib, PerkLib, EventParser, Descriptors, AppearanceDefs, Appearance, CockTypesEnum, CoC, kFLAGS, Utils, StatusAffects, EngineCore, Combat ) {
	...
	KihaFollower.prototype.followerKiha = function() {
		if( CoC.getInstance().flags[ kFLAGS.KIHA_CORRUPTION_BITCH ] === 1 ) {
			return false;
		}
		if( CoC.getInstance().flags[ kFLAGS.KIHA_FOLLOWER ] > 0 ) {
			return true;
		}
		return false;
	};
	KihaFollower.prototype.kihaAffection = function( changes ) {
		...
		if( CoC.getInstance().flags[ kFLAGS.KIHA_AFFECTION_LEVEL ] === 2 ) {
			CoC.getInstance().flags[ kFLAGS.KIHA_AFFECTION ] += changes;
		}
		if( CoC.getInstance().flags[ kFLAGS.KIHA_AFFECTION ] > 100 ) {
			CoC.getInstance().flags[ kFLAGS.KIHA_AFFECTION ] = 100;
		}
		return CoC.getInstance().flags[ kFLAGS.KIHA_AFFECTION ];
	};
	//Introduction;
	KihaFollower.prototype.kihaSpiderEventIntro = function() {
		...
		EngineCore.outputText( '"<i>Bullshit!  You\'re working for the demons!</i>" she howls.  "<i>First you try and make me go soft, tell me I don\'t need to fight, and then suddenly you\'re sneaking into my home.  Well, no more!</i>"  Her axe swings around to point straight at you.  "<i>I\'ve killed the Demon Queen\'s agents before, and if I have to, I\'ll kill you too, ' + CoC.getInstance().player.short + '!</i>"\n\n', false );
		...
		CoC.getInstance().monster.createStatusAffect( StatusAffects.spiderfight, 0, 0, 0, 0 );
	};
	...
	KihaFollower.prototype.loseKihaPreSpiderFight = function() {
		...
		EngineCore.choices( 'Warn Kiha', this.warnKihaOfHerImpendingDemise, 'Let Them', this.letTheSpidersHaveTheirWayWithKiha, '', null, '', null, '', null );
	};
	...
	KihaFollower.prototype.playerBeatsUpKihaPreSpiderFight = function() {
		...
		EngineCore.outputText( 'To your surprise, Kiha hangs her head and whispers, "<i>Just go, ' + CoC.getInstance().player.short + '.  You\'ve already beaten me, and they\'ll punish me for my weakness...  I deserve whatever\'s coming to me.  So JUST GO!</i>"\n\n', false );
		...
		EngineCore.choices( 'Help Kiha', this.helpKihaAgainstSpoidahs, 'Leave Her', this.leaveKihaToSpoidahHorde, '', null, '', null, '', null );
	};
	//Warn Kiha (Z);
	KihaFollower.prototype.warnKihaOfHerImpendingDemise = function() {
		...
		EngineCore.outputText( 'She smirks.  "<i>Come on, ' + CoC.getInstance().player.short + '.  What do you think I am, stupid?</i>"\n\n', false );
		...
		CoC.getInstance().monster.createStatusAffect( StatusAffects.MissFirstRound, 0, 0, 0, 0 );
	...
	KihaFollower.prototype.letTheSpidersHaveTheirWayWithKiha = function() {
		EngineCore.outputText( '', true );
		EngineCore.spriteSelect( 72 );
		CoC.getInstance().flags[ kFLAGS.KIHA_AFFECTION_LEVEL ] = -1;
	...
	KihaFollower.prototype.helpKihaAgainstSpoidahs = function() {
		...
		EngineCore.outputText( '"<i>W-what are... what do you think you\'re doing, ' + CoC.getInstance().player.short + '?</i>" she snaps, recoiling away from you.  "<i>I told you to run!</i>"\n\n', false );
	...
	KihaFollower.prototype.beatSpiderMob = function() {
		...
		EngineCore.outputText( '"<i>Why, ' + CoC.getInstance().player.short + '?</i>" she asks, her voice barely more than a whisper.  "<i>Why... why did you help me?  I tried to hurt you, and you just... turned around and saved me.  I don\'t get it.</i>"[pg]', false );
		...
		EngineCore.outputText( '[pg]"<i>H-hey, ' + CoC.getInstance().player.short + '!</i>"  Kiha suddenly shouts, causing you to turn at the shore.  "<i>D-don\'t forget... um, I mean... remember where this place is, all right?  Friends, uh, visit each other sometimes, I guess.</i>"', false );
	...
	KihaFollower.prototype.kihaFriendlyGreeting = function( output ) {
		...
		EngineCore.spriteSelect( 72 );
		if( output && CoC.getInstance().flags[ kFLAGS.KIHA_AFFECTION_LEVEL ] === 1 && CoC.getInstance().flags[ kFLAGS.KIHA_TALK_STAGE ] >= 7 ) {
		...
		if( output && CoC.getInstance().flags[ kFLAGS.KIHA_TALK_STAGE ] === 6 && CoC.getInstance().player.cor <= 50 ) {
			...
			EngineCore.outputText( '[pg]"<i>Bah!  You just watch, ' + CoC.getInstance().player.short + '.  The queen bitch isn\'t just sitting on her ass; she won\'t rest until the whole world\'s ruined!  The demons already ruined the seasons, caused an endless drought.  Who\'s to say they can\'t shoot a giant dick into the sky, or make it rain corrupted cum forever, until we\'re all slavering monsters?  What\'s to stop them, huh?</i>"', false );
			...
			EngineCore.outputText( '[pg]"<i>Then why... why haven\'t we done something, ' + CoC.getInstance().player.short + '?  Why couldn\'t we just have gone... together?</i>"', false );
			...
			if( CoC.getInstance().flags[ kFLAGS.KIHA_NEED_SPIDER_TEXT ] === 1 ) {
				...
				CoC.getInstance().flags[ kFLAGS.KIHA_NEED_SPIDER_TEXT ] = 0;
			}
			...
			EngineCore.outputText( '[pg]"<i>Oh, uh, hey, ' + CoC.getInstance().player.short + ',</i>" she says leaning on her greataxe.  "<i>It\'s... good to see you again, I guess.  Did you, uh, want something?</i>"', false );
		}
		var talk = null;
		if( CoC.getInstance().flags[ kFLAGS.KIHA_TALK_STAGE ] < 6 ) {
	...
	KihaFollower.prototype.winSparWithKiha = function() {
		...
		if( !this.followerKiha() ) {
			...
			EngineCore.outputText( '[pg]Even though it was just a mock match, you can clearly see the dragoness took the loss as a personal failure.  She flops back down, unable to rise again, and curses at herself.  "<i>If... if I can\'t defeat you, ' + CoC.getInstance().player.short + ', how the hell am I supposed to beat Lethice?</i>"', false );
	...
	KihaFollower.prototype.sparWithFriendlyKihaLose = function() {
		...
		if( !this.followerKiha() ) {
			...
			EngineCore.outputText( '[pg]"<i>Bam. You\'re dead!</i>" she laughs, giving you a little pop on the chin before slinging it back over her shoulder.  "<i>Come on, ' + CoC.getInstance().player.short + '!</i>"  she jeers, "<i>How the hell do you think you\'re going to beat the Demon Queen if you can\'t even beat me, huh?</i>"', false );
	...
	KihaFollower.prototype.talkToFriendlyKiha = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.KIHA_TALK_STAGE ] <= 3 ) {
			CoC.getInstance().flags[ kFLAGS.KIHA_TALK_STAGE ] = 3;
			...
			EngineCore.outputText( '[pg]God dammit, Kiha.', false );
		} else if( CoC.getInstance().flags[ kFLAGS.KIHA_TALK_STAGE ] === 4 ) {
		...
		//Talk to Friendly Kiha - Third Time (Z);
		else if( CoC.getInstance().flags[ kFLAGS.KIHA_TALK_STAGE ] === 5 ) {
			...
			EngineCore.outputText( '[pg]"<i>Dammit, ' + CoC.getInstance().player.short + '!</i>"  she hisses, waving away the smoke.  "<i>I\'m fucking done talking!  Why the hell do you keep this shit up, huh?  All this talk and talk and talk!  You keep coming here, talking your precious morals and acting like you ACTUALLY give a shit about me.  Well, fucking stop!</i>"', false );
	...
	KihaFollower.prototype.kihaXSalamander = function() {
		...
		EngineCore.outputText( '[pg]She rises to her feet, leaning heavily on her greataxe.  "<i>Well, well,</i>"  she sneers, a thin grin on her lips.  "<i>Coming to visit me, ' + CoC.getInstance().player.short + '?  How thoughtful.</i>"', false );
		EngineCore.outputText( '[pg]You attempt to explain that you were just exploring, but before you can finish half a sentence, Kiha swings her axe up into a fighting pose.  "<i>Fuck that,</i>"  the dragoness growls.  "<i>I\'m in the mood for a fight, so come on, ' + CoC.getInstance().player.short + '!  Put \'em up!</i>"', false );
		...
		EngineCore.choices( 'Lie There', this.lieThere, 'Jump Them', this.jumpDaBitches, 'GTFO', this.GTFO, '', null, '', null );
	};
	//this.GTFO (Z);
	...
	KihaFollower.prototype.lieThere = function() {
		...
		EngineCore.outputText( '[pg]"<i>OH HELL NO, you scaly bitch,</i>"  Hel growls, leering down at Kiha as the dragoness leaps to her feet, axe raised.  "<i>You do NOT fucking touch my ' + CoC.getInstance().player.short + ' and get away with it.  You hear me?</i>"', false );
		...
		EngineCore.outputText( '[pg]"<i>Oh, ' + CoC.getInstance().player.mf( 'he', 'she' ) + ' belongs to you, is that it?</i>"', false );
		...
		EngineCore.outputText( '[pg]"<i>Shitter muscle,</i>" Hel says with a sneer.  "<i>And to top it all off... You\'ve got small tits.  Why the hell would ' + CoC.getInstance().player.short + ' want those little things over THESE,</i>" she laughs, cupping her big E-cups for emphasis.', false );
		EngineCore.outputText( '[pg]"<i>Oh, like ' + CoC.getInstance().player.short + ' would even look twice at those floppy things!</i>" Kiha growls, grabbing her own D-cups defensively.  "<i>' + CoC.getInstance().player.mf( 'He', 'She' ) + ' OBVIOUSLY prefers smaller, perkier boobs.  Don\'t you, ' + CoC.getInstance().player.short + '?</i>"', false );
		EngineCore.outputText( '[pg]You start to stammer an answer, but before you know it, Hel\'s put a foot on your chest, pushing you just a little deeper into the still-hot mud.  "<i>How the hell would YOU know what ' + CoC.getInstance().player.short + ' likes, huh?  <i>I</i> know exactly what makes ' + CoC.getInstance().player.mf( 'him', 'her' ) + ' tick!</i>"', false );
		...
		EngineCore.outputText( '[pg]Kiha yells in feral outrage.  "<i>HOW THE FUCK DARE YOU!?</i>"  she screams, storming over.  "<i>Get your filthy feet off ' + CoC.getInstance().player.short + '!</i>"', false );
		...
		EngineCore.outputText( '[pg]Suddenly, Hel rises from the dirt beside you, her tail flaming dangerously.  "<i>Oh, FUCK YOU!</i>"  Hel cries, stamping back to stand beside Kiha.  "<i>How the FUCK would you know ANYTHING about what ' + CoC.getInstance().player.mf( 'he', 'she' ) + ' likes?  You think you know?  I\'ll fucking SHOW YOU!</i>"', false );
		...
		EngineCore.outputText( '[pg]"<i>See?</i>"  Kiha shouts, grabbing at the stain you\'ve left on her thighs, "<i>' + CoC.getInstance().player.short + ' obviously likes me better - ' + CoC.getInstance().player.mf( 'he', 'she' ) + ' dropped ' + CoC.getInstance().player.mf( 'his', 'her' ) + ' cum RIGHT next to my vag.  ' + CoC.getInstance().player.mf( 'He', 'She' ) + ' probably wants to knock me up even, don\'t you, ' + CoC.getInstance().player.short + '?</i>"', false );
		EngineCore.outputText( '[pg]"<i>Oh, is that right?</i>"  Hel laughs, giving her ample hips a shake forceful enough to dislodge your spooge, hitting Kiha full on the face.  "<i>' + CoC.getInstance().player.short + ' put a load right in my asscheeks - ' + CoC.getInstance().player.mf( 'he', 'she' ) + ' gave me the last AND the dirtiest load.  What do you get? Oh, that\'s right, a vag shot.  How unique!</i>"', false );
		...
		EngineCore.outputText( '[pg]"<i>Like her better!  Ha!  Come on, ' + CoC.getInstance().player.short + ', you and I both know you like ME better.  AND that I give the best footsies.</i>"', false );
		...
		EngineCore.outputText( '[pg]Happily, Kiha finally wipes the mud off her dark face, glaring at Hel, who quickly does the same.  The dragoness huffs indignantly.  "<i>I guess if ' + CoC.getInstance().player.short + ' is all right with you...</i>"' );
		...
		EngineCore.outputText( '[pg]"<i>Fine,</i>"  Kiha says, putting up a cocky grin.  "<i>If ' + CoC.getInstance().player.short + ' can tolerate you, then I guess... you\'re welcome in my swamp, I guess.</i>"', false );
	...
	KihaFollower.prototype.jumpDaBitches = function() {
		...
		EngineCore.outputText( '[pg]After what she just tried to pull, you tell her to sit down and shut up.  Behind you, Hel squirms around until you grab her arm and drag her over to sit by Kiha.  "<i>' + CoC.getInstance().player.short + '!  I was trying to help you!</i>"  You just scowl and grab both scaly girls by the hair, dragging them up to kneel in front of you.  They squirm and struggle in your grasp, until you give the both of them a hand slap.  That shuts them up for the moment.', false );
		...
		} else if( CoC.getInstance().player.cocks[ 0 ].cockLength > 6 ) {
		...
		if( CoC.getInstance().player.cockArea( 0 ) < 36 ) {
	...
	KihaFollower.prototype.kihaAdmitsSheLikesYourWang = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 72 );
		if( CoC.getInstance().flags[ kFLAGS.KIHA_ADMITTED_WARM_FEELINZ ] === 0 ) {
			CoC.getInstance().flags[ kFLAGS.KIHA_ADMITTED_WARM_FEELINZ ] = 1;
	...
	KihaFollower.prototype.lovinHugKiha = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 72 );
		CoC.getInstance().flags[ kFLAGS.KIHA_AFFECTION_LEVEL ] = 2;
		CoC.getInstance().flags[ kFLAGS.KIHA_AFFECTION ] = 0;
		...
		if( CoC.getInstance().player.tallness >= 95 ) {
		...
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.doNext( this.lovingHugDickings );
		} else if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.doNext( this.lovingHugsGirlFuckSex );
		} else {
			EngineCore.doNext( this.lovingHugsForRetards );
		}
	};
	...
	KihaFollower.prototype.lovingHugDickings = function() {
		...
		} else if( CoC.getInstance().player.biggestCockArea() > 9 ) {
		...
		if( CoC.getInstance().player.biggestCockArea() <= 9 ) {
		...
		if( CoC.getInstance().player.cocks[ CoC.getInstance().player.biggestCockIndex() ].cockLength >= 36 ) {
		...
		if( CoC.getInstance().player.horseScore() > 4 || CoC.getInstance().player.dogScore() > 4 || CoC.getInstance().player.catScore() > 4 ) {
		...
		EngineCore.outputText( ' your ' + CoC.getInstance().player.cockHead( x ) + ' butting up against the entrance of her womb. Kiha turns and bites your shoulder while her tail spirals around the two of you, forcing you into the most intimate of embraces.  Her whole body quakes once, twice, and then goes into tiny, rhythmic convulsions.  A low, pleasured moan hisses out Kiha\'s mouth as her mouth disengages from your flesh, and you\'re able to see her eyes roll part-way back from the intensity of it all.' );
	...
	KihaFollower.prototype.warmLoverKihaIntro = function( output ) {
		...
		if( output === undefined || output ) {
			...
			if( this.followerKiha() ) {
				if( CoC.getInstance().flags[ kFLAGS.KIHA_NEEDS_TO_REACT_TO_HORSECOCKING ] === 1 ) {
				...
				EngineCore.addButton( 1, 'Hug', this.hugFriendWarmKiha );
				EngineCore.addButton( 3, 'Sex', this.kihaSexMenu );
				EngineCore.addButton( 4, 'Spar', this.sparWithKiha );
				if( CoC.getInstance().flags[ kFLAGS.KIHA_CAMP_WATCH ] > 0 ) {
					EngineCore.addButton( 8, 'Stop Guard', this.guardMyCampKiha );
				} else {
					EngineCore.addButton( 8, 'Guard Camp', this.guardMyCampKiha );
				}
				EngineCore.addButton( 9, 'Leave', leave );
				return;
			}
		...
		if( !this.followerKiha() && CoC.getInstance().flags[ kFLAGS.KIHA_MOVE_IN_OFFER ] === 1 ) {
		...
		//-[Leave]);
		EngineCore.choices( 'Hang Out', this.hangOutWithKiha, 'Hug', this.hugFriendWarmKiha, 'InviteCamp', campo, 'Sex', this.kihaSexMenu, 'Spar', this.sparWithKiha,
	...
	KihaFollower.prototype.hangOutWithKiha = function() {
		...
		//Hang Out 2;
			...
			//[It's Good] [Blech];
			EngineCore.choices( 'It\'s Good', this.itsGood, 'Blech', this.blechKihaYourCooking, '', null, '', null, '', null );
			return;
		}
		//Hang Out 3;
		else {
	...
	KihaFollower.prototype.kihaSexMenu = function( display, allowBack ) {
		...
		if( this.followerKiha() ) {
			...
			if( CoC.getInstance().player.hasItem( ConsumableLib.INCUBID ) || CoC.getInstance().player.hasItem( ConsumableLib.P_DRAFT ) ) {
				if( display ) {
			...
			//Req: Cock;
			if( CoC.getInstance().player.hasCock() ) {
				if( CoC.getInstance().player.cockThatFits( 67 ) >= 0 ) {
				...
				if( CoC.getInstance().player.cockThatFits( 200 ) >= 0 && (CoC.getInstance().player.minLust() > 50 || CoC.getInstance().player.lib > 80 || CoC.getInstance().player.hasItem( ConsumableLib.L_DRAFT )) ) {
					//Dick also can't be that small.;
					if( CoC.getInstance().player.cockArea( CoC.getInstance().player.cockThatFits( 200 ) ) >= 40 ) {
						horse = this.boneTheShitOutofKihaHolesWithHorsecock;
					} else if( display ) {
						if( CoC.getInstance().player.minLust() > 50 || CoC.getInstance().player.lib > 80 ) {
		...
		} else if( display && CoC.getInstance().player.hasCock() ) {
		...
		if( CoC.getInstance().player.hasCock() ) {
			...
			if( CoC.getInstance().player.cockThatFits( 94 ) >= 0 ) {
	...
	KihaFollower.prototype.boneTheShitOutofKihaHolesWithHorsecock = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 72 );
		var c = CoC.getInstance().player.cumQ();
		var x = _.indexOf( CoC.getInstance().player.cocks, _.find( CoC.getInstance().player.cocks, function( cock ) {
			return cock.cockType === CockTypesEnum.HORSE && CoC.getInstance().player.cockArea( cock ) >= 40;
		} ) );
		if( x === -1 ) {
			x = CoC.getInstance().player.cockThatFits( 200 );
		}
		var y = x + 1;
		...
		if( CoC.getInstance().player.cocks[ x ].cockType === CockTypesEnum.HORSE || CoC.getInstance().player.isTaur() ) {
		...
		if( c <= 250 ) {
			...
			if( CoC.getInstance().player.cocks[ x ].cockType.Index > 0 && CoC.getInstance().player.cocks[ x ].cockType.Index < 10 ) {
				EngineCore.outputText( 'animal ' );
			}
		...
		if( CoC.getInstance().player.cocks[ x ].cockType.Index > 0 ) {
			if( CoC.getInstance().player.isTaur() ) {
		...
		if( CoC.getInstance().player.cocks[ x ].cockType === CockTypesEnum.HORSE ) {
			...
			EngineCore.outputText( CoC.getInstance().player.cockHead( x ) + ' of your ' + Descriptors.cockDescript( x ) );
		}
		...
		if( CoC.getInstance().flags[ kFLAGS.KIHA_CERVIXGINITY_TAKEN ] === 0 ) {
			EngineCore.outputText( ' to steal her womb cherry' );
			CoC.getInstance().flags[ kFLAGS.KIHA_CERVIXGINITY_TAKEN ]++;
		} else {
		...
		if( CoC.getInstance().player.balls ) {
		...
		EngineCore.outputText( '.  Of course you\'re big, you think, you\'re the alpha, the ' + CoC.getInstance().player.mf( 'stud', 'breeder' ) + ', the one in charge.  You ' );
		if( CoC.getInstance().player.isTaur() ) {
		...
		EngineCore.outputText( '[pg]You circle and lower yourself in front of her face, the perfect height, and you rest your cum-soaked shaft on her face, the ' + CoC.getInstance().player.cockHead( x ) + ' drooping into her hair to leak seed into the reddish tresses.  Kiha sniffs your ' );
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( '[balls]' );
		...
		//(FIRST TIME ONLY:  ;
		if( CoC.getInstance().flags[ kFLAGS.KIHA_HORSECOCK_FUCKED ] === 0 ) {
			...
			if( CoC.getInstance().player.lib < 80 && CoC.getInstance().player.minLust() < 50 ) {
			...
			CoC.getInstance().flags[ kFLAGS.KIHA_NEEDS_TO_REACT_TO_HORSECOCKING ] = 1;
		}
		//(REPEAT:  ;
		else {
	...
	KihaFollower.prototype.kihaReactsToHorseDicking = function() {
		...
		CoC.getInstance().flags[ kFLAGS.KIHA_NEEDS_TO_REACT_TO_HORSECOCKING ] = 2;
		//{MENU HERE};
	...
	KihaFollower.prototype.kihaPlaysWithBigassCocksFemDomAhoy = function() {
		...
		if( CoC.getInstance().player.cocks[ x ].cockLength > 48 ) {
		...
		EngineCore.outputText( '[pg]Kiha comments, "<i>That\'s it, that\'s a good ' + CoC.getInstance().player.mf( 'boy', 'girl' ) + ',</i>" just in time to watch the first globules of semen blast into the air.  You clench and unload another geyser of sperm into the air before the first rains down over Kiha, splattering over her hair and face' );
		if( CoC.getInstance().player.cumQ() >= 1000 ) {
	...
	KihaFollower.prototype.kihaGirlGirlSex = function() {
		...
		if( CoC.getInstance().player.clitLength >= 10 ) {
	...
	KihaFollower.prototype.savinTheAnalForKiha = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 72 );
		var x = CoC.getInstance().player.cockThatFits( 94 );
		if( x < 0 ) {
			x = 0;
		}
		...
		if( CoC.getInstance().player.cocks[ x ].cockLength < 3 ) {
		...
		if( CoC.getInstance().flags[ kFLAGS.TIMES_KIHA_ANALED ] >= 5 ) {
	...
	KihaFollower.prototype.giveKihaIncubusDraft = function() {
		...
		EngineCore.outputText( '[pg]Kiha staggers back, dropping her greataxe and gripping her stomach as the draft sloshes through her system.  You grin as a sudden squirt of fem-lube erupts from her bare pussy, followed by a sudden twitch and shift of the flesh just above her soaked cunt.  "<i>W-what did you give me, ' + CoC.getInstance().player.short + '?  You... you idiot!</i>" she screams as the skin above her groin violently erupts, a long, hard human-like prick bursting forth into the light.  Kiha lets out a wail of pleasure as her newly-formed prick takes shape, already jetting great white globs of cum onto her thighs and legs.  Her legs give, and Kiha collapses, squirming as her new cock twitches and spasms between her once-powerful legs.' );
		...
		CoC.getInstance().player.buttChange( 14, true, true, false );
	...
	KihaFollower.prototype.fuckKihaWithATentacle = function() {
		...
		var tentacles = _.map( _.filter( _.map( CoC.getInstance().player.cocks, function( cock, index ) {
		...
		if( tentacles.length > 1 ) {
			...
			if( CoC.getInstance().player.cockArea( tentacles[1] - 1 ) < 24 ) {
	...
	KihaFollower.prototype.kihaOffersToMoveIn = function() {
		...
		CoC.getInstance().flags[ kFLAGS.KIHA_MOVE_IN_OFFER ] = 1;
		this.warmLoverKihaIntro( false );
	};
	//Invite Kiha to Camp;
	...
	KihaFollower.prototype.kihaBitchesOutCorruptPCs = function() {
		...
		if( !this.followerKiha() ) {
			...
			if( CoC.getInstance().flags[ kFLAGS.KIHA_AFFECTION_LEVEL ] === 1 ) {
		...
		CoC.getInstance().flags[ kFLAGS.KIHA_CORRUPTION_BITCH ] = 1;
	...
	KihaFollower.prototype.kihaUnbitchesUncorruptedFolks = function() {
		...
		CoC.getInstance().flags[ kFLAGS.KIHA_CORRUPTION_BITCH ] = 2;
	...
	KihaFollower.prototype.dominateKihasFaceWithStuffAndStuffOrSomethingIDunnoWhyImStillWritingThis = function() {
		...
		EngineCore.outputText( '[pg]"<i>If you want have your way with me, you\'ll need to earn it, just like anyone else,</i>" Kiha explains.  She narrows her eyes at you and questions, "<i>The question is, are you ' + CoC.getInstance().player.mf( 'man', 'woman' ) + ' enough to take what you want?  I wouldn\'t want you to get hurt.</i>"' );
		EngineCore.outputText( '[pg]That seems like a challenge.  Will you rise to it, or will you back down?' );
		//[Back down]    [Fight for position];
		EngineCore.choices( 'Back Down', this.beABitchDumbass, 'FightForDom', this.fightForDominanceWithDragonCunnies, '', null, '', null, '', null );
	};
	//[Back down];
	...
	KihaFollower.prototype.fightForDominanceWithDragonCunnies = function() {
		...
		//[Leads to a fight];
		Combat.startCombat( new Kiha() );
		CoC.getInstance().monster.createStatusAffect( StatusAffects.DomFight, 0, 0, 0, 0 );
	};
	//[PC loses the fight];
	...
	KihaFollower.prototype.pcWinsDomFight = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.KIHA_CHOKED_OUT_PC ] > 0 ) {
		...
		if( select === 0 || !CoC.getInstance().player.hasVagina() ) {
			...
			if( Utils.rand( 2 ) === 0 ) {
				EngineCore.outputText( '[pg]"<i>Ha!  The Champion still has some fight in ' + CoC.getInstance().player.mf( 'him', 'her' ) + '!  Keep me here if you can, wimp!  I WILL pin you down and cum all over your face!  You\'ll lap it all up like a good little ' + CoC.getInstance().player.mf( 'boy', 'girl' ) + ' to bo- UGH!</i>"' );
			}
			//Kiha response one (requires cock): ;
			else {
				EngineCore.outputText( '[pg]"<i>Ha!  The Champion still has some fight in ' + CoC.getInstance().player.mf( 'him', 'her' ) + '!  Keep me here if you can, wimp!  I WILL pin you down and make you beg to plant your seed in my womb!  Just wa-OOhhOhhh...</i>"' );
			}
		}
		...
		if( CoC.getInstance().player.gender !== 2 && CoC.getInstance().player.balls === 0 ) {
		...
		if( select === 0 ) {
			EngineCore.outputText( 'works her way on top of your pelvis and impales herself on your ' + Descriptors.cockDescript( x ) + ', grinning like a mad fool as she does so.  "<i>Told you!</i>"  She remarks. "<i>Was that really the best you could do?  With the hair pulling?</i>"  She sighs.  "<i>I guess I have to teach you a thing or two...</i>"  Lowering herself down in between savage slams of her rump against your pelvis, the cool and arrogant woman takes the ' + CoC.getInstance().player.skin() + ' of your neck and pierces it with her canines, causing you to cry out as the sting of her bite flows through you.  Humming her approval, she carries on with the coarse treatment of your entrapped cock, mashing and working forth a crescendo of skin on skin slapping noises.' );
		}
		//Kiha ensnares the PC between her legs for cunnilingus (universal):;
		else if( select === 1 ) {
			EngineCore.outputText( 'quickly wraps her legs around your head and rolls your body into a belly down prone.  "<i>Eat it ' + CoC.getInstance().player.mf( 'big boy', 'you delectable slut' ) + '!  I\'ve got you in a hold you can\'t possibly escape from!  Lap it up bitch!</i>"  For the time being you comply, if only to give you time to find a way out of this as you probe her cunt with your tongue.  "<i>Yeeeaahhhah, just like that.  Good ' + CoC.getInstance().player.mf( 'boy', 'girl' ) + ', lick your mistress\'s pussy!</i>"  She coos authoritatively, her thigh tensing up with every stroke.  The dragon woman even has the nerve to play with your hair as she keeps you within her tight embrace.' );
		}
		...
		//Random Scene three of three dominant cunnilingus, with a reversal:;
			...
			if( (CoC.getInstance().player.hasCock() && Utils.rand( 2 )) || !CoC.getInstance().player.hasVagina() ) {
				...
				EngineCore.outputText( ' only inches away from Kiha\'s face.  Her body perks up as it dawns on her what you have in mind, and she arcs her head back for your impending release.  "<i>Come on... come on... yeah, cum baby, I want it so bad...</i>" she whines in between pants of lust, impatiently longing for your orgasm.  A few moments later you feel the warmth of seed gushing from your ' + CoC.getInstance().player.cockHead( x ) + ', strings of sperm slamming against Kiha\'s pretty features.  She gasps in satisfaction as your cum rains down on her, moaning as your ejaculate coats her cheeks and brow.  Not one to let jism go to waste, you take her by the back of the head and press your ' + Descriptors.cockDescript( x ) + ' against her lips; Kiha yields her mouth to your whims and hums in bliss as more seed flows into her throat and maw.' );
				//Cum multiplier low: ;
				...
				EngineCore.outputText( '[pg]Helpless to offer any resistance, and perhaps unwilling to resist, even if she could, Kiha lays back and accepts that you have won this "<i>battle</i>", licking, suckling and kissing your ' + Descriptors.clitDescript() + ' as you grind against her.  The sensation of her long, reptilian tongue pressing against and sliding sensuously over your nether-lips and clit is incredibly intense and satisfying, and it isn\'t long until you bellow a ' + CoC.getInstance().player.mf( 'manly', 'girlish little' ) + ' cry and cum hard, your muscles tensing with pleasure as you drench the dragoness\' face in lady-cum.  She readily licks it all up as it flows, moaning as she swallows your sexual essence.' );
			}
		}
	...
	KihaFollower.prototype.guardMyCampKiha = function() {
		EngineCore.clearOutput();
		if( CoC.getInstance().flags[ kFLAGS.KIHA_CAMP_WATCH ] > 0 ) {
			CoC.getInstance().flags[ kFLAGS.KIHA_CAMP_WATCH ] = 0;
			...
			CoC.getInstance().flags[ kFLAGS.KIHA_CAMP_WATCH ] = 1;
		}
		EngineCore.menu();
