...
angular.module( 'cocjs' ).run( function( SceneLib, MainView, BreastStore, PerkLib, StatusAffects, ConsumableLib, Descriptors, Appearance, CockTypesEnum, CoC, kFLAGS, Utils, EngineCore ) {
	...
	LatexGirl.prototype.nameZeLatexGoo = function() {
		...
		if( CoC.flags[ kFLAGS.GOO_NAME ] === 'Cattleya' ) {
			...
			EngineCore.outputText( CoC.flags[ kFLAGS.GOO_NAME ] + ', huh?  I can live with that, I suppose,' );
		}
	...
	LatexGirl.prototype.approachLatexy = function() {
		...
		if( this.gooHappiness() < 10 ) {
			EngineCore.outputText( CoC.flags[ kFLAGS.GOO_NAME ] + ' scowls up at your approach, her unhappiness clearly visible in her ' + CoC.flags[ kFLAGS.GOO_EYES ] + ' eyes.  You doubt her solid onyx face could be any more morose.' );
		}//(Sub 20);
		else if( this.gooHappiness() < 20 ) {
			EngineCore.outputText( CoC.flags[ kFLAGS.GOO_NAME ] + ' glares at you as you approach, clearly unhappy with the situation.  She looks like she\'s having a pretty terrible time.' );
		}//(Sub 30) ;
		else if( this.gooHappiness() < 30 ) {
			EngineCore.outputText( CoC.flags[ kFLAGS.GOO_NAME ] + ' frowns at you as you approach, more than a little displeased to see you.  She doesn\'t seem to be having a very good time.' );
		}//(Sub 40) ;
		else if( this.gooHappiness() < 40 ) {
			EngineCore.outputText( CoC.flags[ kFLAGS.GOO_NAME ] + ' knits her eyebrows in consternation at your approach, visibly less than happy.  It doesn\'t look like she\'s thrilled to be here.' );
		}//(Sub 50);
		else if( this.gooHappiness() < 50 ) {
			EngineCore.outputText( CoC.flags[ kFLAGS.GOO_NAME ] + ' glances up at you when you approach, visibly bored.  She looks like she needs some cheering up.' );
		}//(Sub 60);
		else if( this.gooHappiness() < 60 ) {
			EngineCore.outputText( CoC.flags[ kFLAGS.GOO_NAME ] + ' gives you a wan smile as you approach, looking a tad bored.  Still, the glitter in her eyes tells you that she\'s still a good ways from unhappy.' );
		}//(Sub 70);
		else if( this.gooHappiness() < 70 ) {
			EngineCore.outputText( CoC.flags[ kFLAGS.GOO_NAME ] + ' cracks a small grin at the sight of you, happy to see you.  It\'s good to see her having a positive outlook.' );
		}//(Sub 80) ;
		else if( this.gooHappiness() < 80 ) {
			EngineCore.outputText( CoC.flags[ kFLAGS.GOO_NAME ] + ' openly smiles at you as you approach, in good spirits, it seems.  Her sensual, fetishy skin reflects the light as she smiles, looking truly beautiful.' );
		}//(Sub 90) ;
		else if( this.gooHappiness() < 90 ) {
			EngineCore.outputText( CoC.flags[ kFLAGS.GOO_NAME ] + ' grins at you, revealing rows of glittering black teeth.  She playfully regards you with happy, ' + CoC.flags[ kFLAGS.GOO_EYES ] + ' eyes.' );
		}//(Sub 100);
		else if( this.gooHappiness() < 100 ) {
			EngineCore.outputText( CoC.flags[ kFLAGS.GOO_NAME ] + ' bounces up at your approach, mouth splitting into a wide smile.  Her glittering, ' + CoC.flags[ kFLAGS.GOO_EYES ] + ' eyes twinkle with genuine pleasure.' );
		}//(MAX) ;
		else {
			EngineCore.outputText( CoC.flags[ kFLAGS.GOO_NAME ] + ' jumps up on her feet at your approach, gleefully clapping her hands as you close in on her.  Her ' + CoC.flags[ kFLAGS.GOO_EYES ] + ' eyes and ecstatic visage make it clear how unbelievably happy she is.' );
		}
		...
		//{Regardless} ;
		EngineCore.outputText( '  You estimate ' + CoC.flags[ kFLAGS.GOO_NAME ] + '\'s chest would fit a ' + Appearance.breastCup( this.gooTitSize() ) + ' bra, were she to wear one.' );
		//Dicknips:;
		...
		if( CoC.flags[ kFLAGS.GOO_DICK_LENGTH ] > 0 ) {
			EngineCore.outputText( 'Below her waist, a ' + Utils.num2Text( Math.round( CoC.flags[ kFLAGS.GOO_DICK_LENGTH ] * 10 ) / 10 ) + '-inch, ' + this.gooCock() + ' hangs, soft and shiny.  ' );
		}
		...
		if( this.gooObedience() < 10 ) {
			EngineCore.outputText( CoC.flags[ kFLAGS.GOO_NAME ] + ' gives you a look of pure defiance, ready to disobey at the slightest notice.' );
		}//{sub 20};
		else if( this.gooObedience() < 20 ) {
			EngineCore.outputText( CoC.flags[ kFLAGS.GOO_NAME ] + ' idly twirls a raven lock of hair as she awaits your comment, a rebellious look glinting in her ' + CoC.flags[ kFLAGS.GOO_EYES ] + ' eyes.' );
		}//{sub 30};
		else if( this.gooObedience() < 30 ) {
			EngineCore.outputText( CoC.flags[ kFLAGS.GOO_NAME ] + ' smirks at you with a rebellious cast on her features.' );
		}//{sub 40};
		else if( this.gooObedience() < 40 ) {
			EngineCore.outputText( CoC.flags[ kFLAGS.GOO_NAME ] + ' bats charcoal eyelashes at you as she watches you with ' + CoC.flags[ kFLAGS.GOO_EYES ] + ', fiery eyes, still far from obedient.' );
		}//{sub 50};
		else if( this.gooObedience() < 50 ) {
			EngineCore.outputText( CoC.flags[ kFLAGS.GOO_NAME ] + ' glances downwards for a moment, averting her eyes until she remembers to look up, defying her own submission.' );
		}//{sub 60};
		else if( this.gooObedience() < 60 ) {
			EngineCore.outputText( CoC.flags[ kFLAGS.GOO_NAME ] + ' casts her eyes down but peeks up through her charcoal eyelashes to regard you.' );
		}//{sub 70};
		else if( this.gooObedience() < 70 ) {
			EngineCore.outputText( CoC.flags[ kFLAGS.GOO_NAME ] + ' keeps her ' + CoC.flags[ kFLAGS.GOO_EYES ] + ' eyes cast down at the ground in deference to you, though she nervously draws in the dirt with a finger while she awaits a command.' );
		}//{sub 80};
		else if( this.gooObedience() < 80 ) {
			EngineCore.outputText( CoC.flags[ kFLAGS.GOO_NAME ] + ' fixes her gaze down obediently, her cheeks flushing purplish.  Is she enjoying submitting to you?' );
		}//{sub 90);
		else if( this.gooObedience() < 90 ) {
			EngineCore.outputText( CoC.flags[ kFLAGS.GOO_NAME ] + ' prostrates herself on the ground before you, tilting her head back so that she can fix her eyes on your waist, unfit to meet your gaze.' );
		}//{sub 100);
		else if( this.gooObedience() < 100 ) {
			EngineCore.outputText( CoC.flags[ kFLAGS.GOO_NAME ] + ' submissively prostrates herself before you, raising her onyx rump high as her forehead lowers.  Her cheeks and rump both manage to blush purple through her latex skin, clearly aroused by her own submissiveness.' );
		}//{MAX};
		else {
			EngineCore.outputText( CoC.flags[ kFLAGS.GOO_NAME ] + ' plants herself on all fours before you, kissing at your [feet].  Her smooth butt lightens along with her sides and cheeks, blushing violet with lust from the heat of her own submission.  Lewdly, she greets you, "<i>[Master].</i>"' );
		}
	...
	LatexGirl.prototype.changeGooDick = function() {
		...
		if( this.gooObedience() >= 60 ) {
			...
			if( CoC.flags[ kFLAGS.GOO_DICK_LENGTH ] === 0 ) {
				EngineCore.outputText( 'ask ' + CoC.flags[ kFLAGS.GOO_NAME ] + ' if she\'s ever considered growing a dick for fun.' );
			} else {
		...
		//High Happiness + Whatever Obedience(F);
			...
			EngineCore.outputText( 'You ask ' + CoC.flags[ kFLAGS.GOO_NAME ] + ' if ' );
			if( CoC.flags[ kFLAGS.GOO_DICK_LENGTH ] === 0 ) {
	...
	LatexGirl.prototype.latexyEatsADickItem = function( item ) {
		...
		if( CoC.flags[ kFLAGS.GOO_DICK_LENGTH ] === 0 ) {
			...
			if( item === ConsumableLib.GROPLUS ) {
				...
				EngineCore.outputText( 'Her ' + this.gooCock() + ' trembles, and starts to shrink inward, disappearing into her polished abdomen.  The effect is so startling profound that you have to do a double-take.  ' + CoC.flags[ kFLAGS.GOO_NAME ] + '\'s penis is barely two thirds its original size!\n\nShe giggles, "<i>Not a fan of the big boys, huh?</i>"' );
	...
	LatexGirl.prototype.changeLatexyTits = function( arg ) {
		...
		//To DDish {large} sized:;
			...
			if( this.gooTitClass( this.gooTitSize() ) < 3 ) {
				...
				EngineCore.outputText( CoC.flags[ kFLAGS.GOO_NAME ] + ' groans in disappointment, "<i>I liked em big!  Well, you\'re the boss...</i>"  She clenches her fists and closes her eyes.  Nothing happens at first, but after a moment, her ' + this.gooTits() + ' tighten up.  Over the span of a few dozen seconds, ' + CoC.flags[ kFLAGS.GOO_NAME ] + ' is reduced to having something like a DD-cup - a nice but fairly normal size, as ordered.' );
			}
		}
		//To C {medium} sized:;
	...
	LatexGirl.prototype.feedLatexyCumIndirectly = function() {
		...
		EngineCore.outputText( 'Glancing down at ' + CoC.flags[ kFLAGS.GOO_NAME ] + ', you tell her that it\'s feeding time.  She turns her gaze upward, more than a little eagerly' );
	...
	LatexGirl.prototype.minotaurCumFeedingGoo = function( nice ) {
		...
		EngineCore.outputText( '.  Striking ' + CoC.flags[ kFLAGS.GOO_EYES ] + ' eyes imploringly beg at you, as if to say, "<i>Can I have it now?</i>"' );
		//{HARD - skip if nice.};
		if( !nice ) {
			...
			if( this.gooObedience() >= 70 ) {
				EngineCore.outputText( '  ' + CoC.flags[ kFLAGS.GOO_NAME ] + ' immediately drops to the ground on all fours, rubbing her head against your [foot] and begging, "<i>Please [Master], may your latex fuckpet have some of that wonderful cum?</i>"  It seems she can be obedient after all.' );
			}//[Semi obedient:];
			else if( this.gooObedience() >= 40 ) {
				EngineCore.outputText( '  ' + CoC.flags[ kFLAGS.GOO_NAME ] + ' gives a trifling protest, "<i>Do I have to?  That cum looks really delicious!  I promise, I\'ll be good!  I\'ll let you fuck me any way you want.  I\'ll even fuck you, if you want me to!</i>"' );
			...
			//[Not obedient:];
				...
				EngineCore.outputText( '  ' + CoC.flags[ kFLAGS.GOO_NAME ] + ' protests, "<i>But you captured me!  You ought to at least have the decency to give me some of that yummy food!</i>"' );
	...
	LatexGirl.prototype.feedLatexyCumDirectly = function() {
		...
		//HUNGRY: ;
		if( this.gooFluid() < 33 ) {
			EngineCore.outputText( '  A rumble makes ' + CoC.flags[ kFLAGS.GOO_NAME ] + '\'s tight belly tremble.  She brings a hand up to cover her featureless belly, giving you a nervous grin as she leans forward.  "<i>How did you know I was so hungry' );
		...
		//INTERMEDIATE: ;
			...
			EngineCore.outputText( '  A smirk spreads over ' + CoC.flags[ kFLAGS.GOO_NAME ] + '\'s visage.  She licks her lips and leans forward, saying, "<i>Mmm...  I could go for a snack' );
			if( this.gooObedience() > 50 ) {
				EngineCore.outputText( ', [Master]' );
			}
			EngineCore.outputText( '.</i>"' );
		...
		//NOT HUNGRY:  ;
			...
			EngineCore.outputText( '  A lazy smile reveals itself on ' + CoC.flags[ kFLAGS.GOO_NAME ] + '\'s face.  She giggles, her lush body jiggling with liquid weight as she moves.  The latex woman comments, "<i>Mmm...  I\'m pretty full.</i>"  She licks her lips and continues in a gluttonous purr, "<i>You can always feed me more though' );
			if( this.gooObedience() > 50 ) {
				EngineCore.outputText( ', [Master]' );
			}
			EngineCore.outputText( '.</i>"' );
	...
	LatexGirl.prototype.maleDirectFeedLatexGooGoneWrong = function() {
		...
		EngineCore.outputText( '.  ' + CoC.flags[ kFLAGS.GOO_NAME ] + ' looks up at your seizing body, smiling wide around her ' );
		if( CoC.player.biggestCockArea() > 80 ) {
	...
	LatexGirl.prototype.feedLatexyGirlCumDirect = function() {
		...
		if( CoC.player.hasCock() ) {
			...
			EngineCore.outputText( '  When ' + CoC.flags[ kFLAGS.GOO_NAME ] + ' glances down, you sashay to better show your [vagina].' );
		}
		...
		EngineCore.outputText( 'outer lips with excitement, dilating slightly as she leans closer.  ' + CoC.flags[ kFLAGS.GOO_NAME ] + '\'s nostrils flare as she breathes deeply of your scent, inhaling thick lungfuls of your feminine odor, the heady musk of an aroused woman.  Her mouth hangs open slightly, as if in awe of what she sees, and undirected by her conscious mind, she lazily drifts closer.' );
	...
	LatexGirl.prototype.femalePCDomFucksLatexGoo = function() {
		...
		if( this.gooObedience() < 60 ) {
			...
			EngineCore.outputText( ', ' + CoC.flags[ kFLAGS.GOO_NAME ] + ' ' );
		...
		if( CoC.flags[ kFLAGS.GOO_DICK_LENGTH ] > 0 ) {
			EngineCore.outputText( '  Her ' + this.gooCock() + ' practically bursts from within, distending to accommodate its lurid latex discharge.  Most of it splatters against ' + CoC.flags[ kFLAGS.GOO_NAME ] + '\'s skin and rolls down her flesh in a march of onyx droplets.' );
		}
	...
	LatexGirl.prototype.femalePCDomFucksLatexGooFuta = function() {
		...
		if( CoC.flags[ kFLAGS.GOO_DICK_LENGTH ] * CoC.flags[ kFLAGS.GOO_DICK_LENGTH ] / 6 > CoC.player.vaginalCapacity() ) {
			...
			EngineCore.outputText( '  Soon, your [butt] comes to rest against ' + CoC.flags[ kFLAGS.GOO_NAME ] + '\'s oily skin, her erection nestled nicely inside you.' );
		}
		...
		if( CoC.player.hasCock() ) {
			EngineCore.outputText( '  White erupts from your ' + Descriptors.multiCockDescriptLight() + ' with abandon.  You don\'t even care that it stains ' + CoC.flags[ kFLAGS.GOO_NAME ] + '\'s ' + this.gooTits() + '.  It feels so good and she looks so pretty with strands of icing spattering over top of her.' );
		}
	...
	LatexGirl.prototype.malePCDomFucksLatexGoo = function() {
		...
		if( CoC.player.cumQ() >= 500 ) {
			EngineCore.outputText( '  Squirts of spooge run out of her nostrils after a particularly potent blast, but ' + CoC.flags[ kFLAGS.GOO_NAME ] + ' doesn\'t mind or even break her gaze.' );
		}
		if( CoC.player.cumQ() >= 1000 ) {
		...
		EngineCore.outputText( '</i>"  A hand that appears to be wearing a latex glove smears the juices that hang from ' + CoC.flags[ kFLAGS.GOO_NAME ] + '\'s mound up and through her crack, coating her asshole with your leftover sperm.' );
	...
	LatexGirl.prototype.feedLatexySomeMilk = function() {
		...
		if( this.gooFluid() >= 66 ) {
			...
			EngineCore.outputText( '  Before you\'ve even completed your offer, ' + CoC.flags[ kFLAGS.GOO_NAME ] + ' is on you like a moth to a flame, eagerly pulling at your clothes to try and free your teats.' );
		}
