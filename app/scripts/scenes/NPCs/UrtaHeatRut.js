...
angular.module( 'cocjs' ).run( function( SceneLib, PregnancyStore, AppearanceDefs, Descriptors, CoC, kFLAGS, EngineCore ) {
	...
	UrtaHeatRut.prototype.urtaQuestDone = function() {
		return (CoC.getInstance().flags[ kFLAGS.URTA_QUEST_STATUS ] === 1);
	};
	//[Approach Sober Urta];
	UrtaHeatRut.prototype.approachSoberUrtaHeatRutProc = function() {
		...
		} else if( CoC.getInstance().player.inRut && CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( ' Males stare at you with a hint of challenge in their eyes, whilst females eye you attentively. Herms don\'t seem sure whether they want to fight you or fuck you.' );
		}//(Player in heat;
		else if( CoC.getInstance().player.inHeat && CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( ' Males, and individuals who look like females at first, until you spot the telltale bulge of at least one cock in garments or hanging freely between their legs, stare at you with obvious desire as you go, more than a few erections starting to form in your wake.' );
		}
		...
		if( CoC.getInstance().player.inRut && CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( '  Urta is right next to you, and she has already proven herself a good lay' );
		} else {
			EngineCore.outputText( '  You keep reminding yourself what Urta is packing between her legs and how much precious semen it can give, and can\'t help but want to try that now' );
		}

		EngineCore.outputText( '.  Not to mention, with or without the monster between her legs, her body is sexy as hell.' );
		EngineCore.outputText( '\n\n"<i>Well, you do help me with my own releases, so I suppose that\'s fair....' );
		if( CoC.getInstance().player.inHeat && CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( '  You... smell good, so I at least hope you\'re not here to tease me.' );
		}
		EngineCore.outputText( '</i>"  She leans closer to you, and ' );
		if( CoC.getInstance().player.inRut && CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( 'your eyes are quickly drawn to her full breasts as she whispers something to you. You\'re too hypnotized to care.' );
		} else {
		...
		if( CoC.getInstance().player.inRut && CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( ' focus is on the hips of the fox-morph' );
		} else {
			EngineCore.outputText( ' focus on catching a glimpse of that amazing cock, and you keep imagining what she can do with it' );
		}
		EngineCore.outputText( '.  Your aggressive hormonal state demands that you ' );
		if( CoC.getInstance().player.inRut && CoC.getInstance().player.inHeat && CoC.getInstance().player.hasCock() && CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( ' put a baby in her, then let her put a baby in you' );
		} else if( CoC.getInstance().player.inRut && CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( ' put a baby in her so she can push it out of those beautiful hips' );
		} else {
			EngineCore.outputText( ' put that beautiful big cock into your needy [vagina]' );
		}
		//[If Urta's fertility quest wasnât done];
		if( !this.urtaQuestDone() ) {
			EngineCore.outputText( ', even if something keeps reminding you that\'s impossible.' );
		} else {
			EngineCore.outputText( '.' );
		}
		...
		if( CoC.getInstance().player.inRut && CoC.getInstance().player.inHeat && CoC.getInstance().player.hasCock() && CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( 'mixed genitals' );
		} else if( CoC.getInstance().player.inRut && CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( '[cock biggest]' );
		} else {
			EngineCore.outputText( '[vagina]' );
		}
		EngineCore.outputText( ' against her backside.' );
		...
		//Player in rut:;
			...
			if( CoC.getInstance().player.hasVagina() ) {
				EngineCore.outputText( '  Your ' + Descriptors.multiCockDescriptLight() + ' and [vagina] are filling your clothes with mixed juices' );
			} else {
				EngineCore.outputText( '  Your [cock biggest] begins to seep precum into your clothes' );
			}
			EngineCore.outputText( ', so you decide to dispose of them as you force Urta against a wall.' );
			...
			if( CoC.getInstance().player.hasVagina() ) {
				EngineCore.outputText( ' and your pussy dripping and eager' );
			}
			EngineCore.outputText( '.' );
			EngineCore.menu();
			if( CoC.getInstance().player.cockThatFits( SceneLib.urta.urtaCapacity() ) >= 0 ) {
	...
	UrtaHeatRut.prototype.notInterestedInUburDrunkUrtaRuts = function() {
		...
		if( CoC.getInstance().player.inRut && CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( '[EachCock] twitches in protest at you leaving a willing lay behind.  ' );
		}
		//[Medium lust increase] ;
		//[If the PC has a snake tongue];
		if( CoC.getInstance().player.tongueType === AppearanceDefs.TONUGE_SNAKE ) {
			EngineCore.outputText( 'You instinctively taste the air, and Urta suddenly seems utterly delicious. ' );
		}
		//Rut: ;
		if( CoC.getInstance().player.inRut && CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( 'Your ' + Descriptors.multiCockDescriptLight() + ' hardens in your pants as you still force yourself to leave.' );
		}//[Medium lust increase];
		else {
			EngineCore.outputText( 'Your condition does seem kind of protesting against leaving such a willing lay behind, but it isn\'t time to be indulging it.' );
		}
	...
	UrtaHeatRut.prototype.interestedInUburDrunkurtaRuts = function() {
		...
		if( CoC.getInstance().player.inRut && CoC.getInstance().player.inHeat && CoC.getInstance().player.hasCock() && CoC.getInstance().player.hasVagina() ) {
			...
			EngineCore.outputText( '.  "<i>Fuuuck, but you smell so goood... Goddess, I don\'t know which part of you I want more!  Gimme some of that, sexy... please!</i>" she begs you desperately.' );
	...
	UrtaHeatRut.prototype.drunkUrtaIntroPartDuex = function( chosenSex, newl ) {
		...
		if( CoC.getInstance().player.balls > 0 && chosenSex === 1 ) {
			EngineCore.outputText( 'your [balls], rolling them in her hand and squeezing a bit too roughly.' );
		} else if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( 'your [clit], rubbing it until it\'s ' );
			if( chosenSex === 1 ) {
				EngineCore.outputText( 'the very same state [eachCock] seems to be in so often recently, ' );
			}
			EngineCore.outputText( 'erect and throbbing.' );
		} else {
			EngineCore.outputText( 'your [hips], stroking along them towards [eachCock] in an agonizing display of teasing.' );
		}
		...
		if( chosenSex === 1 ) {
			EngineCore.outputText( ', and [eachCock] already gets hard in your [armor].' );
		} else {
			EngineCore.outputText( ', and you feel the inside of your [armor] rapidly slickened by your [vagina]\'s excitement.' );
		}
		...
		//Player in both : ;
		if( CoC.getInstance().player.inRut && CoC.getInstance().player.inHeat && CoC.getInstance().player.hasCock() && CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( 'and she proceeds to maul her breasts, pre-cum seeping copiously from her cock and her snatch drooling over her balls and onto the floor.' );
		} else if( chosenSex === 1 ) {
			EngineCore.outputText( 'but she doesn\'t seem to mind it for the moment.  Rather, the fur between her thighs appears to be rapidly soaking.' );
		} else {
			EngineCore.outputText( 'which she seems more than intent to push into you, although you do see a bit of female lubricant sliding down her balls.' );
		}
		...
		EngineCore.outputText( ' for me</i>"  Moments later, as you feel your naked [butt] against the table, her eyes dart to your ' );
		if( chosenSex === 1 ) {
			EngineCore.outputText( Descriptors.multiCockDescriptLight() );
		} else {
			EngineCore.outputText( '[vagina]' );
		}
		EngineCore.outputText( '.  Her smile opens wider as Urta presents you with a sexy face that seems a mix of positively delighted and predatory.' );
	...
	UrtaHeatRut.prototype.keepTeasingDatHornyFawkes = function() {
		...
		EngineCore.outputText( 'slowly beginning to stroke and caress your ' + Descriptors.multiCockDescriptLight() + ' for the benefit of those who bear vaginas.' );
		...
		if( CoC.getInstance().player.isGoo() || CoC.getInstance().player.isNaga() ) {
			EngineCore.outputText( 'what passes for knees on your [legs]' );
		} else {
		...
		if( CoC.getInstance().player.cor < 50 ) {
			...
			EngineCore.outputText( 'walk triumphantly' );
		}
	...
	UrtaHeatRut.prototype.soberUrtaSatingPCHeat = function() {
		...
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( ' by shoving your ' + Descriptors.multiCockDescriptLight() );
			if( CoC.getInstance().player.hasCock() && CoC.getInstance().player.balls > 0 ) {
				EngineCore.outputText( ' and [balls]' );
			}
			EngineCore.outputText( ' out of the way' );
		} else if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( ' by shoving your [balls] out of the way' );
		}
		EngineCore.outputText( '.  Slowly, you feel the wide flared head of her magnificent pillar of flesh press onto your opening, spreading your vaginal lips and making your [clit] harden in anticipation just as your [butt] shivers, awaiting the intrusion.  Slowly, Urta\'s giant member starts to enter you.' );
		if( CoC.getInstance().player.vaginalCapacity() >= 100 ) {
			EngineCore.outputText( '  Your hungry gash eagerly welcomes her, Urta easily sliding herself home.' );
		} else if( CoC.getInstance().player.vaginalCapacity() > 60 ) {
			EngineCore.outputText( '  You feel it stretch you in the most pleasuring of ways, filling you with warm, throbbing flesh as she pushes it deeper and deeper.' );
		} else {
			EngineCore.outputText( '  You wince in pain at being stretched so much, but you grow wetter and wetter as Urta slowly pushes herself inside your depths, your [vagina] slowly getting wetter and warmer, accepting the intruder.  Pleasure starts to override pain and you let out a gasp.' );
		}
		...
		if( CoC.getInstance().player.vaginalCapacity() >= 60 ) {
			EngineCore.outputText( 'Urta\'s erection seems completely at home in your depths, completely embraced by the wet walls of your throbbing [vagina].' );
		} else {
			EngineCore.outputText( 'You moan again, feeling Urta\'s member stretch, harden and throb, blood running through it.  You feel complete, and your heat is ignited further by the sensation of something so wonderful and big filling you.' );
		}
		...
		if( CoC.getInstance().player.hasCock() ) {
			...
			EngineCore.outputText( 'clitoris, ' );
			if( CoC.getInstance().player.clitLength < 2.5 ) {
				EngineCore.outputText( 'rubbing the hard, stimulated sensitive spot between her fingers' );
			} else {
				EngineCore.outputText( 'running her fingers over your entire big clit before stroking it like she\'d do with a cock' );
			}
		}
		EngineCore.outputText( ', letting you have a few moments to get used to her mammoth member and accommodate it.' );
		...
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( ' Cum rushes out of [eachCock] and onto the soft fur of Urta\'s abdomen, then starts leaking out slowly in time with her thrusts.' );
		}
		EngineCore.outputText( ' Your cunt clamps down tighter onto Urta as you orgasm,' );
		if( CoC.getInstance().player.wetness() >= 4 ) {
			EngineCore.outputText( ' pussy juice shooting out around her member, coating Urta\'s fur and soaking her balls and the base of her cock.' );
		} else {
			EngineCore.outputText( ' love juices flowing liberally out of you, giving the skin of Urta\'s cock a visible shine and spreading onto her balls.' );
		}
		...
		if( CoC.getInstance().player.wetness() >= 4 ) {
			EngineCore.outputText( 'shooting out all over the place' );
		} else {
			EngineCore.outputText( 'running down her member and the crack of your [butt], coating the two of you in the proof of your passion' );
		}
		EngineCore.outputText( '.' );
		...
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( 'your own member responding alike to Urta\'s, shooting out semen, ' );
		}
		EngineCore.outputText( 'your [vagina] ' );
		if( CoC.getInstance().player.wetness() >= 4 ) {
			EngineCore.outputText( 'once again setting off to coat Urta in your juices.' );
		} else {
			EngineCore.outputText( 'squeezing on her, trying to milk her of her seed.' );
		}
		...
		//[Hermaphrodite];
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( '[eachCock] shoots its seed onto the floor of the alley, helplessly' );
		} else {
			EngineCore.outputText( 'your [vagina] squeezes and tugs on her shooting member, leaking its own juices all over her member, the intensity of the orgasm shaking you right into your core' );
		}
		EngineCore.outputText( '.' );
		...
		if( !this.urtaQuestDone() ) {
			...
			EngineCore.outputText( '  She grabs your hand and helps you stand up.' );
		}
	...
	UrtaHeatRut.prototype.sateRutWithSoberUrtaButHuegDicked = function() {
		...
		if( !SceneLib.urta.urtaLove() ) {
			...
			if( CoC.getInstance().player.cumQ() <= 50 ) {
				...
				EngineCore.outputText( 'splashing her face and tits with your cum' );
			} else {
				EngineCore.outputText( 'soaking the shocked fox-morph in your jism' );
			}
			EngineCore.outputText( '; though you still feel the urge to breed fertile females, your lust is momentarily sated.  Urta wipes ' );
			if( CoC.getInstance().player.cumQ() >= 500 ) {
				EngineCore.outputText( 'halfheartedly ' );
			}
			EngineCore.outputText( 'at the cum on her faces and gives you a friendly smile.  "<i<There, that should feel better; I know what it\'s like to be all pent up with no release.</i>"  She rises to her feet, still holding your cock, and gives it a squeeze.  "<i>In the meantime, if you want to really cut loose, I suggest you go and find something to shrink this down.</i>"' );
		...
		//Lover Urta Path/;
			...
			if( CoC.getInstance().player.cumQ() <= 50 ) {
				EngineCore.outputText( 'She swallows it all without complaint, licking her lips clean of any stray droplets once you remove it from her mouth.' );
			} else if( CoC.getInstance().player.cumQ() <= 250 ) {
				EngineCore.outputText( 'Her cheeks bulge at the amount you pump into her, but she\'s able to swallow with an admirable amount of control, gasping for breath.' );
			} else {
				EngineCore.outputText( 'Rivulets of cum burst from between her lips, but she stubbornly sucks and swallows as much as she can, her stomach starting to bulge from the sudden influx of spooge.  When you stop, she is drooling cum and gasping for breath, but she meets your eyes with pride, even as she gives a wet belch.' );
			}
	...
	UrtaHeatRut.prototype.sateRutWithSoberUrta = function() {
		...
		if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( ', trigging a mutual explosion from your [vagina]' );
		}
		...
		if( this.urtaQuestDone() ) {
			...
			EngineCore.outputText( '  I\'d rather do something like this in the privacy of my home than in the middle of the street.</i>"' );
	...
	UrtaHeatRut.prototype.drunkenUrtaFucksPCInHeat = function() {
		...
		if( !this.urtaQuestDone() ) {
			EngineCore.outputText( '  You know that she can\'t give you babies... but, honestly, at this moment, you couldn\'t care less. Her wonderful stallion cock is there and ready and it\'s all for you.' );
		} else {
			EngineCore.outputText( '  You dimly recall that Urta could be virile, but she\'s right there, her huge mare-cock just dripping to plunge into your ripe [vagina] and knock you up.  You don\'t care anymore; you want her babies!' );
		}
		...
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( ', impatiently nudging aside your ' + Descriptors.multiCockDescriptLight() );
			if( CoC.getInstance().player.balls > 0 ) {
				EngineCore.outputText( ' and your [balls]' );
			}
			EngineCore.outputText( ' to get to the feminine orifice below' );
		}
		EngineCore.outputText( '. You cry out in joy as your [vagina] is filled, her soft fur brushing against your inner walls before she opens her mouth as wide as she can and sticks out her tongue. The long, broad, vulpine appendage slides out and up your depths, caressing the walls, lapping up your juices, ' );
		if( CoC.getInstance().player.clitLength < 2.5 ) {
			EngineCore.outputText( 'playing with your [clit]' );
		} else {
			EngineCore.outputText( 'lapping around your huge pleasure-button, slurping it into her mouth where she suckles from it' );
		}
		EngineCore.outputText( ', and making you thrash and scream until you think you\'re going to cum from the pussy-eating alone.  ' );
		if( CoC.getInstance().player.wetness() >= 4 ) {
			EngineCore.outputText( 'As if to emphasize this, your squelching, slurping pussy suddenly sprays fem-cum all over Urta\'s face, making her pull herself free with a shocked shake of her head.' );
		} else {
			EngineCore.outputText( 'As if sensing your impending orgasm, Urta removes her head from your [vagina], giving you a cocky grin and shaking her head.' );
		}
		...
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( 'nor even your ' + Descriptors.multiCockDescriptLight() );
		}
		EngineCore.outputText( ', despite usually trying to make it just as good for you.  She begins to thrust into you with even greater force than before.  You groan and whimper under her; partly from the pleasures of being taken so roughly, so triumphantly, and partly to goad her on, to make her feel even more aroused and alpha-male-like.' );
		...
		//CoC.getInstance().player returns to camp and loses 4 hours//;
	...
	UrtaHeatRut.prototype.sateRutWithDrunkUrtaWithHugeDick = function() {
		...
		EngineCore.outputText( 'between your [hips]' );
		EngineCore.outputText( '.' );
		...
		if( CoC.getInstance().player.biggestCockArea() < 150 ) {
			EngineCore.outputText( 'compete with minotaurs or something?' );
		} else {
			EngineCore.outputText( 'set a world record or something?' );
		}
		EngineCore.outputText( '</i>"\n\n' );
		...
		if( CoC.getInstance().player.cumQ() <= 50 ) {
			EngineCore.outputText( 'You cum all over your [chest] as Urta blows thick ropes of vixen-spunk all over your cock and upper body, a single shot landing on your lips.' );
	...
	UrtaHeatRut.prototype.drunkUrtaRidesARutPCsCock = function() {
		...
		EngineCore.outputText( ', and her own balls resting comfortably on your abdomen.  She grits her teeth as you feel her pussy slowly tighten around and massage your ' + Descriptors.cockDescript( x ) + ', and you groan in response.  Urta starts to bounce up and down on your member, smacking her thighs and ass into your body, not minding her own giant equine member flopping around and occasionally smacking you over your chest.  She lets her tongue flick out into the air for a moment and moans in delight, before picking up the pace and starting to pump your cock in and out of her hungry passage furiously.' );
		...
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( 'feel your balls churning' );
		} else {
			EngineCore.outputText( 'recognize the tingling sensation from your ' + Descriptors.cockDescript( x ) + ' to your spine' );
		}
		EngineCore.outputText( ', a telltale sign that your penis is ready to unload the potent spunk inside the herm-nympho bouncing on top of you.  You try to announce this, but she doesn\'t slow down.  Moments later, ' );
		//[Normal cum production];
		if( CoC.getInstance().player.cumQ() <= 50 ) {
			EngineCore.outputText( 'you unload your thick spunk into her pussy' );
		} else if( CoC.getInstance().player.cumQ() <= 250 ) {
			EngineCore.outputText( 'you erupt inside her, filling her with wave after wave of warm, thick semen' );
		} else {
			EngineCore.outputText( 'you blow massive surges of spunk inside her continuously.  The fox\'s stomach expands, rivulets of semen running down onto the table as she continues her ride on top of you' );
		}
		EngineCore.outputText( '.  As you discharge, the fox\'s pussy starts squeezing and contracting, and you recognize the hermaphrodite\'s own orgasm.  Thicker white liquid suddenly drips from the tip of her cock quite copiously and occasionally shoots out into the air.' );
		...
		if( CoC.getInstance().player.hasKnot( x ) ) {
			EngineCore.outputText( 'You slowly feel the blood flow to your erect penis, and the knot at the base of it seems to start to expand' );
		} else {
			EngineCore.outputText( 'You don\'t mind staying like this for much, much longer, and the hermaphrodite on top of you seems to share that point of view' );
		}
		EngineCore.outputText( '.  Summoning your slightly exhausted strength, you pull her hips down deeper into your own before starting to return the hermaphrodite\'s shakes and bounces with thrusts of your own.  It intensifies the sensations, making your lover yelp in surprise before smiling widely and releasing low, content moans and purrs from the depths of her throat.' );
		...
		if( CoC.getInstance().player.hasKnot( x ) ) {
			EngineCore.outputText( 'knot surge with more blood and your erection twitch, ensuring that you\'re completely trapped inside her until you\'re empty.' );
		} else {
			EngineCore.outputText( 'tip expand as you\'re getting ready to blow another load.' );
		}
		...
		if( CoC.getInstance().player.cumQ() >= 500 && CoC.getInstance().player.cumQ() <= 1500 ) {
			...
			if( CoC.getInstance().player.balls > 0 ) {
				EngineCore.outputText( '[balls]' );
			} else if( CoC.getInstance().player.hasVagina() && CoC.getInstance().player.clitLength >= 3.5 ) {
				EngineCore.outputText( 'cock-like clit' );
			} else if( CoC.getInstance().player.hasVagina() ) {
				EngineCore.outputText( 'pussy lips' );
			} else {
				EngineCore.outputText( 'inside of your thighs' );
			}
			EngineCore.outputText( ' with your own thick white semen.  Then, she slides your softening erection back into herself again.' );
		}
		...
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( '  Your [balls] don\'t seem to be completely empty yet' );
		} else {
			EngineCore.outputText( '  It doesn\'t appear to be the end of what your body can produce' );
		}
		EngineCore.outputText( ', and your vulpine lover squeals with delight as she feels you harden inside her.  She smacks your [chest] with her huge, half-hard member.' );
		...
		if( !CoC.getInstance().player.hasVagina() && CoC.getInstance().player.balls === 0 ) {
			EngineCore.outputText( ' the inside of your thighs' );
		} else if( CoC.getInstance().player.balls > 0 ) {
		...
		if( CoC.getInstance().player.balls > 0 ) {
			...
			EngineCore.outputText( '[vagina] and [clit]' );
		} else {
		...
		if( CoC.getInstance().player.cumQ() <= 200 ) {
			EngineCore.outputText( '  The table already seems like it\'ll be stained white by Urta\'s efforts alone, and your own fluid doesn\'t help the situation.' );
		} else if( CoC.getInstance().player.cumQ() < 1000 ) {
			EngineCore.outputText( '  The two of you are already fucking in a small pool of your mixed semen sauce, making naughty sounds heard all over the bar, and her vagina, dripping with your fluids, only makes the pool seem bigger.' );
		} else if( CoC.getInstance().player.cumQ() >= 2000 ) {
			EngineCore.outputText( '  Your mixed sperm is covering the table and the floor, and woe to anyone who will have to clean this place up.  Despite the ridiculous amount of fluids around, she seems set on the idea that the two of you are barely mid-way through the act.' );
		}
		...
		if( CoC.getInstance().player.cumQ() >= 1000 ) {
			EngineCore.outputText( '  Well, maybe you can, but let\'s make some twins!' );
		}
		EngineCore.outputText( '  Keep going!</i>"' );
		...
		else if( CoC.getInstance().player.tallness <= 93 ) {
		...
		if( CoC.getInstance().player.tallness <= 93 ) {
			EngineCore.outputText( 'making you lick, kiss and suck on her flared, wide tip' );
		} else {
		...
		if( CoC.getInstance().player.cor > 35 || CoC.getInstance().flags[ kFLAGS.PC_FETISH ] > 0 ) {
			EngineCore.outputText( 'You tell her it was actually a great experience to have such passionate sex publicly.  She blushes at that suggestion.' );
		}
		...
		if( !SceneLib.urta.urtaLove() ) {
			...
			EngineCore.outputText( '\n\nYou ' + CoC.getInstance().player.mf( 'chuckle', 'giggle' ) + ' and she punches your shoulder playfully, but keeps her face from blushing more.  She seems pleased that she brought herself to that declaration.' );
		}
		//[Both end here];
