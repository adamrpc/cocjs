...
angular.module( 'cocjs' ).run( function( MainView, SceneLib, Utils, PregnancyStore, AppearanceDefs, Descriptors, CoC, kFLAGS, EngineCore ) {
	...
	UrtaPregs.prototype.urtaPregAppearance = function() {
		switch( SceneLib.urta.pregnancy.event ) {
			case 1:
			case 2:
				MainView.outputText( 'Urta is sitting at her usual table, looking more clear-eyed and sober than usual.  She looks extremely happy, tail wagging gently behind her, and she gives you a thrilled look when she sees you, smiling blissfully and patting her belly in a meaningful gesture.' );
				break;
			case 3:
			case 4:
			case 5:
			case 6:
			case 7:
				MainView.outputText( 'Urta has moved from her usual table to a more private stall, apparently for the comfort of its padded seats.  Her bulging belly makes it obvious she\'s pregnant - and the wide grin that seems fixed to her face makes it obvious she\'s ecstatic about it.' );
				break;
			case 8:
			case 9:
				MainView.outputText( 'Urta\'s pregnancy has caused her to outgrow the stall; she\'s moved herself to an entirely new position in a quiet, out of the way part of the bar.  Looking at the sheer size of her, she\'s doubtlessly going to pop any day now.' );
		}
	};
	//In Urta Menu;
		...
		switch( SceneLib.urta.pregnancy.event ) {
			case 1:
				...
				if( CoC.flags[ kFLAGS.URTA_TIME_SINCE_LAST_CAME ] === 0 ) {
					MainView.outputText( '  Her cock audibly clonks against the table\'s underside, but she just grins at the sound, fully confident in who she is now.  "<i>Wanna come back to my place, lover' + CoC.player.mf( '-boy', '' ) + '?  They say sex is good for the baby - and really good for the mommy...</i>"  She croons, teasingly rubbing your [legs] with her foot.' );
					this.urtaSexMenu();
				} else {
					MainView.outputText( '  The lack of cock-on-wood sounds or motion on Urta\'s part makes it clear she\'s not feeling amorous right this moment.  Breaking the kiss, she licks her glossy lips clean and looks at you with a good-natured smirk.  "<i>Feeling up to talking for a little while?</i>"' );
			...
			case 7:
				...
				break;
			case 8:
				MainView.outputText( 'Urta\'s finally outgrown even her booth, her belly now looking like she swallowed a prize-winning watermelon whole.  She\'s taken to sitting in a shapeless, very well-stuffed couch in an out-of-the-way nook of the bar.  Her hands stroke her gut and her erect dick at more or less the same time, precum dripping noisily into a bucket positioned under her erection.  She sees you coming and her cock bounces eagerly, belching another spurt of precum into the bucket from a surge in arousal.' );
	...
	UrtaPregs.prototype.IAintGotTimeForUrtaBirfs = function() {
		MainView.clearOutput();
		MainView.outputText( 'The centaur looks perplexed at your refusal.  "<i>I - what?  Really?</i>"' );
	...
	UrtaPregs.prototype.getUrtaLetter = function() {
		...
		if( CoC.flags[ kFLAGS.URTA_LATESTBORN_GENDER ] === 1 ) {
			...
			MainView.outputText( 'hermaphrodite daughter' );
		}
		MainView.outputText( ', though.\n' );
	...
	UrtaPregs.prototype.goVisitUrtaBirfs = function( withUrta ) {
		...
		CoC.flags[ kFLAGS.NEW_BABY_ANNOUNCED ] = 0;
		if( !withUrta ) {
			MainView.outputText( 'The trip to the clinic where Urta is giving birth to your child is a blur; all that matters is getting there as fast as possible.  The centaur abandons you at the hospital doors, and you make your own way inside, approaching the counter inside.' );
		}
		...
		if( gender === 2 ) {
			...
			MainView.outputText( 'boy...?  Well... isn\'t that unusual, looks like you actually gave birth to a healthy herm!' );
		}
		...
		if( gender === 1 || gender === 3 ) {
			if( Utils.rand( 2 ) === 0 ) {
				...
				MainView.outputText( 'he\'s a perfectly normal little fox,</i>" the doctor notes, and you can catch a quick glimpse of a somewhat bigger than normal vulpine sheathe on your ' );
				if( gender === 1 ) {
					MainView.outputText( 'son' );
				} else {
					MainView.outputText( 'daughter' );
				}
				MainView.outputText( '\'s groin from where you are.' );
				CoC.flags[ kFLAGS.URTA_LATESTBORN_COCKTYPE ] = 2;
			}
			//[Horse Dick:;
			else {
				...
				MainView.outputText( 'he takes after ' );
				if( gender === 3 ) {
					MainView.outputText( 'her' );
				} else {
					MainView.outputText( 'his' );
				}
				MainView.outputText( ' mother,</i>" the doctor comments.  Snatching a quick glance reveals the leathery sheath of an equine phallus swaying between the newborn\'s legs.  Although infantile, it already promises that ' );
				if( gender === 1 ) {
					MainView.outputText( 'he' );
				} else {
					MainView.outputText( 'she' );
				}
				MainView.outputText( ' is going to be quite a big ' );
				if( gender === 1 ) {
					MainView.outputText( 'boy when he grows up.' );
				} else {
					MainView.outputText( 'girl when she grows up.' );
				}
				CoC.flags[ kFLAGS.URTA_LATESTBORN_COCKTYPE ] = 1;
			}
		}
		...
		MainView.outputText( ' to the breast, watching with joyous awe as the little fox-morph roots for her black nipple and then latches on.  Tears begin to roll down her cheeks at the sight.  "<i>Our ' );
		if( gender >= 2 ) {
			MainView.outputText( 'daughter' );
		} else {
			MainView.outputText( 'son' );
		}
		MainView.outputText( '... look, [name], ' );
		//[she's];
		if( gender >= 2 ) {
			MainView.outputText( 'she\'s' );
		} else {
			MainView.outputText( 'he\'s' );
		}
		MainView.outputText( ' really here; our beautiful, beautiful little baby...</i>" She sobs, ' );
		if( this.urtaKids() === 0 ) {
			MainView.outputText( 'overwhelmed by the fact she\'s a mother at long last' );
		} else {
			MainView.outputText( 'still as delighted as ever to have a new addition to your growing brood' );
		}
		MainView.outputText( '.' );
		...
		MainView.outputText( ' mother\'s breast.  "<i>Looks like my work here is done for now... I\'ll be back later to give a quick check-up on the baby, but from what I can see you have nothing to worry about.  Excuse me then.</i>"  The doctor gets up and leaves the room, letting you have some privacy with the fox-herm.' );
		//1st time:;
		if( CoC.flags[ kFLAGS.URTA_TIMES_BIRTHED ] === 0 ) {
	...
	UrtaPregs.prototype.goAfterUrtaBirf = function() {
		MainView.clearOutput();
		MainView.outputText( 'You apologize, but there are pressing matters you need to attend to today... but you\'ll make it up to her somehow, you promise.  Urta looks disappointed, but sighs sadly and nods her head.  "<i>Of course.  You\'re still a busy adventurer and all that.  Well, don\'t worry; I\'ll make sure this little one gets home and settled in safely.</i>"' );
		CoC.time.hours++;
	...
	UrtaPregs.prototype.stayAfterUrtaBirf = function() {
		MainView.clearOutput();
		MainView.outputText( 'Of course, if that\'s what she wants, you\'d be happy to oblige.  The vixen smiles at you thankfully.  "<i>You won\'t regret it, [name]; I have an eventful day planned for us,</i>" she finishes with a mischievous grin and a giggle of delight.' );
	...
	UrtaPregs.prototype.wakeUpWithUrtaAfterStaying = function() {
		...
		//PC Wakes with Urta;
		MainView.outputText( 'As the morning sun shines on the blinds, you open your eyes.  Then you remember the events of the last day.  You spent the whole day with Urta, not having sex, just walking together and buying stuff for your newborn ' );
		if( CoC.flags[ kFLAGS.URTA_KIDS_MALES ] === 1 ) {
			MainView.outputText( 'son' );
		} else {
			MainView.outputText( 'daughter' );
		}
		MainView.outputText( ', then you went home and spent the better part of the evening making out with Urta which, of course, led to hot vixen sex in her bedroom.  After that... you probably fell asleep on her bed, and considering the current state of affairs, she didn\'t mind it one bit.' );
		...
		if( CoC.player.cor < 50 ) {
			MainView.outputText( 'gently,' );
		} else {
			MainView.outputText( 'tightly,' );
		}
		MainView.outputText( ' earning a ' );
		if( CoC.player.cor < 50 ) {
			MainView.outputText( 'whimpering moan' );
		} else {
			MainView.outputText( 'gasping shudder' );
		}
		MainView.outputText( ' from the vixen, then begin stroking her and pushing her to the side.  The fox grumbles and shudders, rolling over to the side.  "<i>Mmm... [name], while that\'s nice, I don\'t really wanna have to do laundry right after getting up,</i>"  she mumbles in a sleepy voice.' );
		...
		if( CoC.flags[ kFLAGS.URTA_TIMES_BIRTHED ] === 1 ) {
			MainView.outputText( 'You look at her questioningly; she\'s breast-feeding, so she shouldn\'t be drinking alcohol.  "<i>Oh, don\'t worry; even I don\'t drink booze in the morning.  It\'s a little pick-me-up of my own invention; totally non-alcoholic.  It\'s mostly fruit juice with a few spices thrown in.</i>"  Urta laughs.  ' );
		}
		MainView.outputText( '  Your only reply is to reach behind her balls and gently insert a wandering finger into her pussy.' );
		...
		if( this.urtaKids() === 1 ) {
			MainView.outputText( 'a child' );
		} else {
			MainView.outputText( 'children' );
		}
		MainView.outputText( ' to look after now; I can\'t be leaving broken glass around, now can I?</i>"  She puts her hands behind her head and stretches, perhaps not so coincidentally thrusting out her naked breasts.  "<i>Mmm... now I feel awake.  Come on, lover; you\'re clearly in the mood, and last night was wonderful... ready for round two, hmm?</i>"  She croons, wriggling herself over to sit right next to you and leaning onto your shoulder.' );
		...
		if( this.urtaKids() === 1 ) {
			...
			MainView.outputText( 'newest ' );
		}
		MainView.outputText( 'little monster,</i>" she notes, already walking off to the nursery, indifferent to any need for clothes.  After all, it\'s not like the baby will mind that she\'s not wearing pants while feeding ' );
		if( this.urtaKids() === 1 ) {
			MainView.outputText( 'him' );
		} else {
			MainView.outputText( 'her' );
		...
		MainView.outputText( '.' );
		if( this.urtaKids() === 2 ) {
			MainView.outputText( '  The other kid is staying at Edryn\'s, so there\'s no risk of you two needing to answer... complicated questions.' );
		}
		if( this.urtaKids() > 2 ) {
			MainView.outputText( '  The other kids are staying at Edryn\'s, so there\'s no risk of you two needing to answer... complicated questions.' );
		}
	...
	UrtaPregs.prototype.drinkSomeUrtaPostBirthTitMilk = function() {
		MainView.clearOutput();
		MainView.outputText( 'You decide to ask if Urta wouldn\'t mind you getting breakfast before going.  Your hand wanders to caress her unoccupied breast.  She moans appreciatively and then giggles.  "<i>Naughty [name]... but, I\'m okay with that.  Just let me finish up with this little one first; I can\'t juggle the two of you at the same time.</i>"' );
		...
		MainView.outputText( ' together... but that\'s a poor substitute if you don\'t ever come back.  Please, be careful with yourself, [name],</i>" she tells you solemnly.  You kiss her goodbye, giving her shaft a teasing grope then step out of the room.' );
	...
	UrtaPregs.prototype.urtaPregooUpdates = function() {
		...
		//4: ;
			...
			if( SceneLib.urta.pregnancy.type !== PregnancyStore.PREGNANCY_PLAYER ) {
				MainView.outputText( 'sounding somewhat jealous that it\'s you carrying and not her.' );
			} else {
				MainView.outputText( 'sounding a little smug as she caresses her own gravid tummy.' );
			}
			MainView.outputText( '  You shake your head, but considering how far you are into the pregnancy, it shouldn\'t take long... if Urta keeps visiting, you\'re pretty sure it\'ll be kicking the next time she comes around.' );
		...
		//5;
			...
			if( this.urtaKids() === 0 ) {
				MainView.outputText( ' Oh, [name], we really are going to be parents...' );
			}
			MainView.outputText( '</i>" She babbles happily, tail wagging so enthusiastically you wonder if it\'s going to fall off.' );
	...
	UrtaPregs.prototype.noHelpForUrtaPreggoPC = function() {
		MainView.clearOutput();
		MainView.outputText( 'You caress her cheek before saying a flat, "<i>No</i>".  Urta actually gives you puppy dogs eyes at that, then lets out a wounded-sounding huff.  "<i>All right, [name]; I guess I better get back to work, then.  Now you take care of yourself; you\'ve got a little one on board, now.</i>"  She chastises you.' );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	//[=Yes=];
	UrtaPregs.prototype.getUrtaHelpPreggoPC = function() {
		MainView.clearOutput();
		MainView.outputText( 'Good, you hope this means she\'ll be having plenty of time, because you sure could use a fair share of vixen right now.  "<i>As long as you want me, I\'ll be here.</i>"  Urta laughs already starting to fiddle with the straps of her armor.  "<i>Do you have any preferences for what we do?</i>"' );
	...
	UrtaPregs.prototype.getUrtaSexWhenPreggoz = function() {
		MainView.clearOutput();
		MainView.outputText( 'Well... you could use a break from minding your passenger, so how does she plan to help you take your mind off your current predicament?  You ask with a knowing smile.' );
	...
	UrtaPregs.prototype.getAnUrtaMassageWhenPreggo = function() {
		MainView.clearOutput();
		MainView.outputText( 'You tell her that a backrub sounds fine, but you\'d rather not lie down.  It could hurt the baby.  "<i>Right, sorry, what was I thinking?  Well, I think I can be flexible...</i>"  Urta grins, cracking her fingers. "<i>Now, let\'s just find somewhere comfortable to put you first...</i>"' );
	...
	UrtaPregs.prototype.declineUrtaLevelSixPreggoHelps = function() {
		MainView.clearOutput();
		MainView.outputText( 'You thank Urta for the offer, but you\'ll have to decline.  You still have business to take care of, so you\'ll just rest for a while longer then be on your way.  Besides, it wouldn\'t look good for her to be absent for too long, she has a city to take care of.' );
	...
	UrtaPregs.prototype.acceptUrtaLevelSixPreggoHelps = function() {
		MainView.clearOutput();
		MainView.outputText( 'You\'re feeling really tired right now, and besides, spending a whole day with Urta would be nice, so you gladly accept her offer.' );
	...
	UrtaPregs.prototype.PCGivesBirf = function() {
		...
		if( gender === 1 ) {
			...
			MainView.outputText( ' sheath between his legs that looks like it\'s going to be bigger than normal when he grows up.' );
			CoC.flags[ kFLAGS.URTA_KIDS_MALES ]++;
		...
		//Herm:;
			...
			MainView.outputText( ' sheath between her legs that looks like it\'s going to be bigger than normal when she grows up.' );
			CoC.flags[ kFLAGS.URTA_KIDS_HERMS ]++;
		...
		MainView.outputText( ' out here.  I know you wouldn\'t let anything bad happen to ' );
		if( gender >= 2 ) {
			MainView.outputText( 'her' );
		} else {
			MainView.outputText( 'him' );
		}
		MainView.outputText( ', but, well, you can\'t really take care of ' );
		if( gender >= 2 ) {
			MainView.outputText( 'her' );
		} else {
			MainView.outputText( 'him' );
		}
		MainView.outputText( ' properly in this wilderness, now can you?</i>"' );
		...
		MainView.outputText( ' was brought up in Tel\'adre.  Urta smiles thankfully and gives both of you one particularly eager cuddle.  "<i>Thank you for understanding, [name].  ' );
		//[She];
		if( gender >= 2 ) {
			MainView.outputText( 'She' );
		} else {
		...
		MainView.outputText( ' will be brought up safely, I promise.</i>"  With that, she carefully bundles up the sleeping child and quietly vanishes into the wilderness, leaving you to take yourself off to bed for a proper rest.\n' );
	...
	UrtaPregs.prototype.urtaIsAPregnantCopScene = function() {
		...
		if( SceneLib.edryn.pregnancy.isPregnant ) {
			MainView.outputText( '  Edryn\'s pregnant belly isn\'t helping her, either.' );
		}
	...
	UrtaPregs.prototype.urtaSpecialDeliveries = function() {
		MainView.clearOutput();
		MainView.outputText( 'As you enter through the gates, Edryn clops up to you, a small basket swinging from the crook of one arm' );
		if( SceneLib.edryn.pregnancy.isPregnant ) {
			MainView.outputText( ' and her pregnant belly jiggling from the motions' );
		}
		MainView.outputText( ', a faint smile on her face.  "<i>Hey there, [name]; can you do me a favor, please?  Urta\'s been getting deliveries to her place, but I got an unexpected call, so I\'m too busy to take her groceries to her today - you\'re her ' + CoC.player.mf( 'boyfriend', 'girlfriend' ) + '; can you please take this for me?</i>"  The centauress asks.' );
		...
		if( SceneLib.edryn.pregnancy.isPregnant ) {
			MainView.outputText( 'as quickly as a pregnant mare can, anyway,' );
		}
		MainView.outputText( ' leaving you to head on to Urta\'s house.' );
		...
		if( SceneLib.urta.pregnancy.event >= 7 ) {
			MainView.outputText( 'heavily ' );
		}
		MainView.outputText( 'pregnant vixen is snoring softly, laying on her side under the covers, forming a sizable bump in the soft, fluffy quilt that practically envelops her, almost invisible amidst the big plump cushions that serve to keep her comfortable as she sleeps. ' );
	...
	UrtaPregs.prototype.talkToUrtaAboutHerHouse = function() {
		MainView.clearOutput();
		MainView.outputText( 'You\'ve been wondering why Urta keeps her old apartment if she has a nicer and bigger house now.  It would be much better for her to put the lease money towards something more productive than keeping that apartment...' );
		if( CoC.player.cor < 50 ) {
			MainView.outputText( ' despite it having some emotional value to you.' );
		}
		...
		if( this.urtaKids() > 1 ) {
			MainView.outputText( 'or two ' );
		}
		MainView.outputText( 'pipes up asking what we\'re doing.</i>"  She shakes her head with amused smirk.  "<i>No, I\'d rather keep the old place for our little love nest; we can have our fun there, then come home and be all respectable around the kids.</i>"' );
	...
	UrtaPregs.prototype.yesUrtaHouseSex = function() {
		MainView.clearOutput();
		MainView.outputText( 'You smile seductively at her and raise a brow.' );
		...
		if( CoC.player.lib > 50 ) {
			MainView.outputText( '  Sometimes you almost give up on waiting to get to her apartment and instead strip and do her right then and there... but it wouldn\'t bode well for the Watch captain to get a fine for public indecency.' );
		}
	...
	UrtaPregs.prototype.noUrtaHouseSex = function() {
		MainView.clearOutput();
		MainView.outputText( 'You smile at her and apologize, you\'re just not in the mood right now.' );
	...
	UrtaPregs.prototype.talkAboutLiana = function() {
		MainView.clearOutput();
		MainView.outputText( 'You tell Urta that you need to talk with her about something.' );
		...
		if( this.urtaKids() > 1 ) {
			MainView.outputText( 'ren have' );
		} else {
			MainView.outputText( ' has' );
		}
		MainView.outputText( ' a special place in her heart.</i>"' );
	...
	UrtaPregs.prototype.preggoUrtaSmexOrSomething = function() {
		MainView.clearOutput();
		MainView.outputText( 'You let Urta know that you\'re for helping her vent some steam, if that\'s what she wants.' );
		MainView.outputText( '\n\nUrta grins lecherously, precum ' );
		switch( SceneLib.urta.pregnancy.event ) {
			case 7:
				MainView.outputText( 'dripping wetly onto the floor' );
				break;
			case 8:
			case 9:
				MainView.outputText( 'slopping into the filling bucket' );
				break;
			default:
				MainView.outputText( 'staining her dress' );
		}
	...
	UrtaPregs.prototype.massagePregnantUrtasBelly = function() {
		...
		if( CoC.flags[ kFLAGS.TIMES_MASSAGED_URTA_BELLY ] === 0 ) {
			MainView.outputText( '  She looks at you and her newly cream-adorned belly contemplatively.  "<i>I never knew my sweet tooth would bring me to this...</i>"' );
		}
		...
		MainView.outputText( 'tongue hang and begin licking the cream off her breasts' );
		if( CoC.player.tongueType > AppearanceDefs.TONUGE_HUMAN ) {
			MainView.outputText( ', a task that is made far easier due to your tongue\'s ability to stretch to a surprising length' );
		}
		MainView.outputText( '.  You make sure to pay extra attention to her cream covered nubs.' );
	...
	UrtaPregs.prototype.rubUrtaPussy = function() {
		MainView.clearOutput();
		MainView.outputText( 'You decide that by "her," she could only mean her pussy.  So you stretch your arm, as if reaching for her shaft, then find your way under her furry balls to her waiting slit, gently touching and massaging her labia, teasing her clit with little pinches, forcing her wetness out of her slavering honeypot.  She lets out a surprised yelp, then laughs, "<i>Not actually what I wanted... ooh!  But I\'m not complaining...</i>"' );
	...
	UrtaPregs.prototype.urtaMassageClimax = function() {
		...
		//Low Cor:;
		if( CoC.player.cor < 33 ) {
			MainView.outputText( '  You walk up to her and carefully remove her condom, tying it up with a knot to prevent any of her seed from escaping, then dispose of it.' );
		}
		//Medium Cor:;
		else if( CoC.player.cor < 66 ) {
			MainView.outputText( 'Well, whatever, she\'s probably too sleepy to do anything about it either way.' );
		}
		//High Cor:;
		else {
			MainView.outputText( 'You walk up to her with a smirk and remove her condom, then pour down its contents over her belly, chuckling to yourself.  Now that\'s a great cream topping for your lovely fox-herm.' );
		}
		MainView.outputText( '  You walk out of her house, taking care to lock the door on your way out.' );
	...
	UrtaPregs.prototype.urtaBoobWorshipScene = function() {
		MainView.clearOutput();
		MainView.outputText( 'You gaze at Urta\'s breasts, heavy with their milky load, a few droplets already escaping her black nipples.  "<i>Oh, you like these, do you?  You like a nice, big pair of foxy-boobs, huh?</i>"  She jokes, jiggling them for your amusement, before wincing at their sensitivity.' );
		...
		if( CoC.player.faceType === AppearanceDefs.FACE_SHARK_TEETH || CoC.player.faceType === AppearanceDefs.FACE_SNAKE_FANGS || CoC.player.faceType === AppearanceDefs.FACE_SPIDER_FANGS ) {
			MainView.outputText( ', always minding your sharp teeth' );
		}
		MainView.outputText( '.  She yelps loudly, but makes no move to push you off, so she\'s obviously enjoying it.  You alternate between biting and pulling, never repeating the same motion twice.  "<i>I- I\'m gonna...!</i>"  She gasps out, bucking and wriggling under you.  You smile wickedly and bite her nipple one last time, pulling and sucking on it.' );
		...
		if( CoC.player.hasCock() ) {
			MainView.outputText( ', tasting a bit of yourself,' );
		}
		MainView.outputText( ' and tell her you\'ll be back to check up on her later.  She gives you a sleepy smile and nods absently, already practically asleep.  You quickly find your [armor] and don them, leaving Urta to rest for a spell in her apartment.' );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	//Nurse;
	UrtaPregs.prototype.nurseFromMommaUrtasBooBees = function() {
		MainView.clearOutput();
		MainView.outputText( 'You look at Urta\'s breasts, full of milk and dripping, and absentmindedly lick your lips. Urta follows your gaze and gives you a knowing smile.  "<i>You know, no matter how much I milk myself, I always seem to fill up again right away... I wonder if that\'s part of Taoth\'s so-called parting gift,</i>" she muses to herself.' );
		//1st Time:;
		...
		MainView.outputText( 'tongue.  Urta lets out a sigh of pleasure, yipping softly as your tongue tantalizes her over-sensitive black nipples.  Noticing that she seemed to enjoy the licking you keep doing so, slathering her soft mounds in a thin layer of saliva, alternating between breasts.' );
		if( CoC.player.tongueType > AppearanceDefs.TONUGE_HUMAN ) {
			MainView.outputText( '  Every few licks you coil your tongue around one of her nipples and gently pull at it, milking a thin strand of white goodness straight on your tongue, much to the fox-herm\'s delight.' );
		}
	...
	UrtaPregs.prototype.titWorshipAndStroke = function() {
		MainView.clearOutput();
		MainView.outputText( 'You smile mischievously around Urta\'s breast and eagerly grab her shaft with a hand, careful not to hurt her, but roughly enough for her to feel what you just did.  Urta lets out a surprised groan.  "<i>Lover?  What are you...?</i>"  She moans again as your fingers shift on her distended dick.' );
	...
	UrtaPregs.prototype.ignoreUrtaBonerWhenWorship = function() {
		MainView.clearOutput();
		MainView.outputText( 'You have other things to worry about right now, but you\'re glad she seems to be enjoying your ministrations.' );
		this.finishTitWorshipWivUrta();
	};
	...
	UrtaPregs.prototype.milkPregnantUrta = function() {
		MainView.clearOutput();
		MainView.outputText( 'You tell her that you feel like playing with her toys.  Urta smiles and nods.  "<i>Sure, though I prefer a more... personal touch... myself.  They\'re in that cupboard over there,</i>" she finishes, pointing it out.' );
	...
	UrtaPregs.prototype.goHomeHorsecock = function() {
		MainView.clearOutput();
		MainView.outputText( 'You nod at Urta and gather your clothes, redressing in an instant.  Before going you ask if Urta needs anything.  She smiles and shakes her head, a soft smile on her face.  You bid her a good rest and leave her apartment.' );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	//[=Stay=];
	UrtaPregs.prototype.stayForHorseDickings = function() {
		MainView.clearOutput();
		MainView.outputText( 'You yawn yourself, and suggest that maybe Urta would like some company.  You\'re kinda sleepy yourself.  She smirks and reclines on the bed, "<i>Help yourself, lover - there\'s plenty of me to cuddle,</i>" she giggles softly as she pats her swollen belly.' );
	...
	UrtaPregs.prototype.urtaPregCravings = function() {
		MainView.clearOutput();
		MainView.outputText( 'You decide to ask Urta if she has any particular cravings right now.  The pregnant fox visibly thinks about, then surreptitiously eyes your crotch.' );
		//noDick:;
		if( !CoC.player.hasCock() ) {
		...
		if( CoC.player.balls > 0 ) {
			MainView.outputText( ' with your [balls]' );
		}
		MainView.outputText( '.  You smile apologetically down at her and ask her if she\'s found out what she\'s been craving yet.  "<i>What do you think?</i>"  She mumbles sarcastically, a playful twinkle in her eye, then starts noisily slurping and sucking, tongue frenziedly lapping at your intruding dick and her eyes rolling shut in obvious enjoyment as she gulps down the precum she\'s eagerly coaxing from you.' );
		MainView.outputText( '\n\nYou moan and try your best not to just grab onto her head and face-fuck her.  ' );
		if( CoC.player.cockTotal() > 2 ) {
			MainView.outputText( 'The rest of your ' + Descriptors.multiCockDescriptLight() + ' are as erect as your [cock biggest] dribbling pre along their lengths as they slap Urta during her frenzied bobs.  ' );
		}
		MainView.outputText( 'You tell Urta that you\'re getting close; if she keeps up with this you won\'t last much longer.' );
		...
		if( CoC.player.balls > 0 || CoC.player.hasVagina() ) {
			MainView.outputText( ', a hand playing expertly with ' );
			if( CoC.player.balls > 0 ) {
				MainView.outputText( 'your balls' );
				if( CoC.player.hasVagina() ) {
					MainView.outputText( ' and another with ' );
				}
			}
			if( CoC.player.hasVagina() ) {
				MainView.outputText( 'your pussy' );
			}
		}
	...
	UrtaPregs.prototype.getBlownByPregnantUrta = function() {
		MainView.clearOutput();
		MainView.outputText( 'If she wants it so bad, she can have it, you think to yourself, bucking against her hand as you finally feel ' );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( 'your balls churn and ' );
		}
		MainView.outputText( 'cum travelling down your urethra, expanding your shaft with the force of their passage, and finally jetting out of your [cockHead biggest] to arch gracefully through the air and splatter against Urta\'s face.' );
		...
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( '  Even if it did, you wouldn\'t be able to do anything about your other cock' );
			if( CoC.player.cockTotal() > 2 ) {
				MainView.outputText( 's as they join' );
			} else {
				MainView.outputText( ' as it joins' );
			}
			MainView.outputText( ' in on the onslaught.' );
		}
		...
		//(Low Cum Amount);
		if( CoC.player.cumQ() < 250 ) {
			MainView.outputText( '  Soon, all too soon, you\'re spent.  Urta is left with strands of cum covering her naked body, only a few having made it inside her open maw.  You pant as you slump down, satisfied with the outcome.' );
	...
	UrtaPregs.prototype.holdOnYouAintGettingYerCumYetYouNaughtyFox = function() {
		MainView.clearOutput();
		MainView.outputText( 'You\'re not going to give in so easily.  If she wants it she\'d best work hard for it.  You smile at her mischievously, even as you hump against her grip.' );
		...
		if( CoC.player.cockTotal() > 1 ) {
			...
			if( CoC.player.cockTotal() > 2 ) {
				MainView.outputText( 's bulging ominously as they too prepare to deposit their loads on the willing fox-herm.' );
			} else {
				MainView.outputText( ' bulging ominously as it too prepares to deposit its load on the willing fox-herm.' );
			}
		}
		MainView.outputText( '  Urta eagerly gulps and slurps, doing her best to swallow every drop.' );
		//(Low Cum Amount);
		if( CoC.player.cumQ() < 250 ) {
			MainView.outputText( '\n\nYou feed her all you can, ' );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 'even splashing some cum over her face as your other cock' );
				if( CoC.player.cockTotal() === 2 ) {
					MainView.outputText( ' convulses' );
				} else {
					MainView.outputText( 's convulse' );
				}
				MainView.outputText( ' in orgasm, ' );
			}
			MainView.outputText( 'but ultimately you reach your limit and can do nothing but slump as she continues sucking on your [cock biggest] like a straw, hungry for your male milk.' );
		...
		//(Medium Cum Amount);
			...
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 'Your other cock' );
				if( CoC.player.cockTotal() === 2 ) {
					MainView.outputText( ' contributes' );
				} else {
					MainView.outputText( 's contribute' );
				}
				MainView.outputText( ' by making a complete mess of her as ' );
				if( CoC.player.cockTotal() > 2 ) {
					MainView.outputText( 'they randomly spatter' );
				} else {
					MainView.outputText( 'it randomly spatters' );
				}
				MainView.outputText( ' her with cum, not that she seems to mind.' );
			}
			MainView.outputText( '  You lay down and relax, letting the cock-hungry fox-herm suck you dry as you continue pumping your male milk.  By the time she\'s done her belly looks a little larger, and you know that some of that bloating is not entirely due to the baby.  You smile happily at the vixen hungrily slurping on your [cock biggest] despite there being nothing more for her to drink.' );
		...
		//(High Cum Amount);
			...
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( '  Your other cock' );
				if( CoC.player.cockTotal() > 2 ) {
					MainView.outputText( 's shoot their loads' );
				} else {
					MainView.outputText( ' shoots its load' );
				}
				MainView.outputText( ' erratically, some of it splattering against Urta\'s face, but most of it winds up making a complete mess of the room.' );
			}
			...
			if( CoC.player.hasKnot( CoC.player.biggestCockIndex() ) ) {
				MainView.outputText( ', just above your knot,' );
			}
			MainView.outputText( ' with both hands, trying to pinch it shut long enough to inhale a couple of deep breaths, then goes back to trying to drink every last drop; she must really have a hankering for what you\'re packing.' );
		...
		if( CoC.player.balls > 0 ) {
			...
			MainView.outputText( 'yourself' );
		}
		MainView.outputText( ' drained of your cum.' );
		...
		if( CoC.player.cumQ() >= 500 ) {
			MainView.outputText( ', audibly gurgling' );
		}
		MainView.outputText( ' belly, clearly feeling tired even though she wasn\'t the one being pleasured this time.' );
	...
	UrtaPregs.prototype.weirdUrtaCravingsConclusion = function() {
		MainView.clearOutput();
		MainView.outputText( 'You have the distinct feeling that someone is groping you and something is wiggling inside your mouth... actually it feels like someone is lapping you inside your mouth.  You open your eyes to gaze at Urta\'s green eyes, as she continues kissing you.  She holds your [face] with both hands, closing her eyes as she deepens the kiss, then slowly lets go, opening her eyes and smiling.  "<i>Morning, lover; sleep well?</i>"  She laughs.' );
		MainView.outputText( '\n\nYou nod, then notice that she\'s ' );
		if( CoC.player.cumQ() < 1000 ) {
			MainView.outputText( 'completely clean.' );
		} else {
			MainView.outputText( 'lacking the extra bit of paunch you gave her earlier.' );
		}
		MainView.outputText( '  You gaze at her questioningly.  "<i>I woke up before you and went and... shall we say cleaned myself up a little,</i>" she replies, flicking her ears in the direction of the shower.' );
	...
	UrtaPregs.prototype.urtaRaepsJoo = function() {
		...
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'Every time her swollen tip rubs against your prostate you feel droplets of pre escape your dilating cum-hole.  ' );
		}
		if( CoC.player.hasVagina() ) {
			MainView.outputText( 'Each slap of her balls against your [vagina] sends electric waves of pleasure that expand throughout your body.  ' );
		}
		MainView.outputText( 'You moan audibly, making it pretty clear you\'re getting off from this kind of treatment.  "<i>See?  I knew you\'d enjoy this too, now that I\'m not so tense...</i>"  She draws out the last word as she makes a particularly deep thrust inside of you.  "<i>Mmm...  I can feel your sexy ass squeezing my dick; you\'re enjoying this, really, aren\'t you?</i>"' );
		...
		if( CoC.player.cockTotal() > 0 ) {
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( '  [EachCock] tenses in unison' );
			} else {
				MainView.outputText( '  [EachCock] throbs, tensing in preparation' );
			}
			//(Low Cum Amount);
			if( CoC.player.cumQ() < 250 ) {
				MainView.outputText( ' and you unload on the floor, spilling rope after rope of cum, forming a small puddle where you stand.  An undeniable testament to your enjoyment of the kinkiness of the act, however small.' );
			}//(Medium Cum Amount);
			else if( CoC.player.cumQ() < 1000 ) {
				MainView.outputText( ' and you deliver your payload right there on the floor.  Jet after jet splash against it, and you have no choice but to continue moaning and spilling your cum as Urta remains firmly lodged in your depths, no longer able to move now that you\'re squeezing her shaft so deliciously tight.  By the time you\'re done, the puddle you\'ve formed is big enough that you can see your own face on the reflection, enraptured in pleasure and enjoyment of Urta\'s equine meatstick.' );
			}//(High Cum Amount);
			else {
				MainView.outputText( ' and you let your prodigious load escape the confines of your ' );
				if( CoC.player.balls > 0 ) {
					MainView.outputText( 'churning balls' );
				} else {
					MainView.outputText( 'being' );
				}
				MainView.outputText( ', splashing against the floor with such force that you form veritable streams of enjoyment in the rapidly expanding lake of perversion.  The pleasure of the kinky act, aided by your earlier drink, ensures you feel nothing but ultimate happiness for this brief moment.  By the time you\'ve spilled the last drop, the lake has reached Urta\'s knees, and you pity the poor soul in charge of cleaning the back rooms.' );
			}
		}
		if( CoC.player.hasVagina() ) {
			...
			MainView.outputText( ' juices that paint Urta\'s balls, which are resting flush against your [vagina].  The spilled juice slides down Urta\'s balls' );
			if( CoC.player.wetness() >= 4 ) {
				MainView.outputText( ' to join your previous discharge on' );
			} else {
				MainView.outputText( ' and down towards' );
			}
			MainView.outputText( ' the floor, where it gathers in a ' );
			MainView.outputText( 'puddle of its own.' );
		}
		//Genderless:;
		if( CoC.player.gender === 0 ) {
			...
			MainView.outputText( ' anal muscles milk Urta\'s shaft much like a pussy would.' );
			if( CoC.player.ass.analWetness > 0 ) {
				MainView.outputText( ' It even secretes its own wetness to slide lovingly down the fox-herm\'s balls and drip on the floor, an undeniable proof of your enjoyment.' );
			}
		}
		...
		if( puddle > 0 ) {
			if( puddle === 1 ) {
				...
				MainView.outputText( 'reasonable' );
			} else {
				MainView.outputText( 'enormous' );
			}
			MainView.outputText( ' puddle of cum you created' );
		} else {
			MainView.outputText( 'floor' );
		}
		MainView.outputText( ' and close your eyes, letting sleep take you over even as the fox behind you drools on your back and continues to pound your ass raw.' );
	...
	UrtaPregs.prototype.declineARimJob = function() {
		MainView.clearOutput();
		MainView.outputText( 'You tell her that you appreciate the offer, but you\'re going to sit this one out.  Though she tries to hide it, she looks relieved at your refusal; she probably isn\'t really comfortable with that level of kinkiness.  Instead, she eagerly captures your lips with hers, murmuring into your mouth as she devours your kiss, tongue sliding between your lips to caress your own tongue.' );
		//Both choices converge here.;
	...
	UrtaPregs.prototype.acceptUrtaRimJobbies = function() {
		MainView.clearOutput();
		MainView.outputText( 'Well... she is responsible for your state, and her broad vulpine tongue must feel amazing on your sensitive ass... so you nod in acceptance of her offer.  "<i>All right then... you do know you\'re the only person I\'d ever do this for, right?</i>" Urta comments.  "<i>Now, roll around and present yourself to me; you can\'t expect a pregnant lady to do a lot of bending, now can you?</i>"  She smirks.' );
		//Not Centaur:;
		if( !CoC.player.isTaur() ) {
		...
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( '  Especially not if your ' + Descriptors.multiCockDescriptLight() + ' have anything to say about it, throbbing at the mere feeling of Urta\'s wet tongue.' );
		} else if( CoC.player.cockTotal() === 1 ) {
			MainView.outputText( '  Especially not if your [cock] has anything to say about it, throbbing at the mere feeling of Urta\'s wet tongue.' );
		} else if( CoC.player.hasVagina() ) {
			MainView.outputText( '  Especially not if your [pussy] has any say in the matter, winking at Urta, hoping for a licking of its own.' );
		}
		MainView.outputText( '  She slowly begins deepening the pseudo-kiss, pressing more and more of her wet tongue against you until she is taking slow, languid licks up and down the lengths of the chasm, slathering your burning skin with her cooling goo.' );
	...
	UrtaPregs.prototype.visitKidsFirstTime = function() {
		MainView.clearOutput();
		MainView.outputText( 'You ask Urta if she can take you to see your child' );
		if( this.urtaKids() > 1 ) {
			MainView.outputText( 'ren' );
		}
		MainView.outputText( '.' );
		MainView.outputText( '\n\n"<i>You want to see ' );
		if( this.urtaKids() === 1 ) {
			MainView.outputText( 'our child' );
		} else {
			MainView.outputText( 'our babies' );
		}
		MainView.outputText( '?</i>"  Urta asks, tail wagging excitedly at the thought.  "<i>Why, of course, [name] - ' + this.urtaKidsText( 'he\'ll', 'she\'ll', 'they\'ll' ) + ' be so happy to see ' + this.urtaKidsText( 'his', 'her', 'their' ) + ' ' + CoC.player.mf( 'father', 'other-mother' ) + '!  Come on, let\'s get going!</i>"  She insists, immediately getting up from her seat' );
		if( CoC.flags[ kFLAGS.URTA_TIME_SINCE_LAST_CAME ] === 0 ) {
			MainView.outputText( ', erection already diminishing as she forgets about her horniness in her parental pride' );
		}
		MainView.outputText( '.  You waste no time in following her; indeed, she seems liable to leave you behind in her excitement to go and see her offspring.' );
		...
		MainView.outputText( '.  One door leads out to the back courtyard, while another looks like it closes off stairs leading down to a basement.' );
	...
	UrtaPregs.prototype.newBabyComing = function() {
		MainView.clearOutput();
		MainView.outputText( 'You announce to your ' );
		if( this.urtaKids() === 1 ) {
			MainView.outputText( 'child' );
		} else {
			MainView.outputText( Utils.num2Text( this.urtaKids() ) + ' kids' );
		}
		MainView.outputText( ' that you and Urta have something important to tell them.' );
		//If UrtaKids <8:;
		if( this.urtaKids() < 8 ) {
			MainView.outputText( '\n\nAs ' );
			if( this.urtaKids() === 1 ) {
				...
				MainView.outputText( ' approaches' );
			} else {
				MainView.outputText( 'they gather' );
			}
			MainView.outputText( ' and curiously look' );
			if( this.urtaKids() === 1 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ' at you, Urta gives ' );
			if( this.urtaKids() > 1 ) {
			...
			MainView.outputText( ' a loving smile.' );
			//Both Preg:;
			...
			MainView.outputText( '\n\nYour child' );
			if( this.urtaKids() > 1 ) {
				MainView.outputText( 'ren are' );
			} else {
				MainView.outputText( ' is' );
			}
			MainView.outputText( ' silent for a moment, digesting this information.  Then ' );
			if( this.urtaKids() === 1 ) {
				MainView.outputText( 'a small fluffy bundle is' );
			} else {
				MainView.outputText( 'several small fluffy bundles are' );
			}
			MainView.outputText( ' doing ' );
			if( this.urtaKids() === 1 ) {
				MainView.outputText( 'its' );
			} else {
				MainView.outputText( 'their' );
			}
			MainView.outputText( ' best to hug you both; ' );
			if( this.urtaKids() > 1 ) {
			...
			MainView.outputText( ' clearly delighted at the prospect.  You and Urta smile at each other over ' );
			if( this.urtaKids() > 1 ) {
				MainView.outputText( 'their' );
			} else if( CoC.flags[ kFLAGS.URTA_FIRSTBORN_GENDER ] >= 2 ) {
				MainView.outputText( 'her' );
			...
			MainView.outputText( ', then return the embrace, pleased that ' );
			if( this.urtaKids() > 1 ) {
				MainView.outputText( 'they' );
			} else if( CoC.flags[ kFLAGS.URTA_FIRSTBORN_GENDER ] >= 2 ) {
				MainView.outputText( 'she' );
			...
			MainView.outputText( ' excited at the prospect.' );
		}
		//If UrtaKids =>8:;
		else {
	...
	UrtaPregs.prototype.hugAttackScene = function() {
		MainView.clearOutput();
		MainView.outputText( 'Before you can exchange any further pleasantries the sound of running padded feet fills the house.  Lianna simply looks at the approaching foxling' );
		if( this.urtaKids() > 1 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ' from the safety of the kitchen, leaving you to handle the incoming charge.  It takes a bit of effort to keep from losing your balance as your brood tackles you, hugging you in affection.  "<i>All right, all right, let your ' + CoC.player.mf( 'father', 'other-mother' ) + ' go, or do you not want to play?</i>" Urta says, smirking with amusement as she closes the front door.  With a groan and a bit of prying the little fox' );
		if( this.urtaKids() > 1 ) {
			MainView.outputText( 'es do' );
		} else {
			MainView.outputText( ' does' );
		}
		MainView.outputText( ' eventually clamor' + this.urtaKidsText( 's', 's', '' ) + ' off of you, but not before taking your hands in ' + this.urtaKidsText( 'his', 'her', 'their' ) + ' own.  ' );
		if( this.urtaKids() > 1 ) {
			MainView.outputText( 'They' );
		} else if( CoC.flags[ kFLAGS.URTA_FIRSTBORN_GENDER ] >= 2 ) {
			MainView.outputText( 'She' );
		...
		MainView.outputText( ' insistently pull' );
		if( this.urtaKids() === 1 ) {
			MainView.outputText( 's' );
		}
	...
	UrtaPregs.prototype.noPlayTimeForKids = function() {
		MainView.clearOutput();
		MainView.outputText( 'You shake your head, telling your family that you\'re sorry but you have to go.  "<i>Really?  All right... but, you should at least hang around long enough to take some on the go.</i>"  Urta insists.  You tell her you\'ll be fine, but you\'ll definitely stop by to eat with them sometime.  Urta gives a melodramatic sigh and rolls her eyes, then nods and smiles.  "<i>All right... just make sure you do.</i>"  She tells you, then gives you a quick hug and a kiss.  "<i>We worry about you out there; come home safe to us.</i>"  She says, then lets you go.' );
	...
	UrtaPregs.prototype.stayToPlay = function() {
		MainView.clearOutput();
		MainView.outputText( 'You nod your head in acceptance.  Urta smiles in delight.  "<i>Take a seat, please, make yourself comfortable - I\'ll go and get it,</i>" she says happily, tail wagging as she saunters off to the kitchen.' );
		//1st time:;
	...
	UrtaPregs.prototype.bathtime = function() {
		MainView.clearOutput();
		MainView.outputText( 'You tell Lianna you just wanted to check in on your ' );
		if( this.urtaKids() > 1 ) {
			MainView.outputText( 'kids' );
		} else if( CoC.flags[ kFLAGS.URTA_FIRSTBORN_GENDER ] >= 2 ) {
			MainView.outputText( 'daughter' );
		} else {
			MainView.outputText( 'son' );
	...
	UrtaPregs.prototype.giveTheKidsABath = function() {
		...
		if( this.urtaKids() === 1 ) {
			if( CoC.flags[ kFLAGS.URTA_FIRSTBORN_GENDER ] === 1 ) {
				MainView.outputText( 'your son' );
			} else {
				MainView.outputText( 'your daughter' );
			}
		} else {
			MainView.outputText( 'the kids' );
		}
		MainView.outputText( '.</i>"  You contemplate what to do, then remove your outer layer of clothes, carefully placing them on a shelf in case they get splashed.  Now you just need to wait for Lianna to return.' );
		...
		if( this.urtaKids() === 1 ) {
			...
			MainView.outputText( 'army of kids' );
		}
		MainView.outputText( '.  "<i>All right, ' );
		if( this.urtaKids() > 1 ) {
			MainView.outputText( 'everyone ' );
		}
		MainView.outputText( 'strip up and hop in.</i>"  ' );
		if( this.urtaKids() === 1 ) {
			if( CoC.flags[ kFLAGS.URTA_FIRSTBORN_GENDER ] === 1 ) {
				MainView.outputText( 'He' );
			} else {
		...
		if( CoC.flags[ kFLAGS.URTA_KIDS_MALES ] > 0 ) {
			...
			MainView.outputText( '.' );
			if( CoC.flags[ kFLAGS.URTA_FIRSTBORN_GENDER ] === 1 ) {
				MainView.outputText( '  Your firstborn is ' );
				if( CoC.flags[ kFLAGS.URTA_KIDS_MALES ] === 1 ) {
					MainView.outputText( 'the only one,' );
				} else {
					MainView.outputText( 'among them,' );
				}
				MainView.outputText( ' sporting a' );
				if( CoC.flags[ kFLAGS.URTA_FIRSTBORN_COCKTYPE ] === 2 ) {
					MainView.outputText( ' vulpine penis' );
				} else {
					MainView.outputText( 'n equine phallus, similar to what Urta has' );
				}
				MainView.outputText( '.' );
			}
			if( CoC.flags[ kFLAGS.URTA_LATESTBORN_GENDER ] === 1 && this.urtaKids() > 1 ) {
				MainView.outputText( '  Your newest is ' );
				if( CoC.flags[ kFLAGS.URTA_LATESTBORN_GENDER ] === 1 ) {
					MainView.outputText( 'also ' );
				}
				MainView.outputText( 'a boy, with a' );
				if( CoC.flags[ kFLAGS.URTA_LATESTBORN_COCKTYPE ] === 2 ) {
					MainView.outputText( ' vulpine penis' );
				} else {
					MainView.outputText( 'n equine phallus, just like his mom\'s' );
				}
				MainView.outputText( '.' );
			}
			MainView.outputText( '  You have a feeling ' );
			if( CoC.flags[ kFLAGS.URTA_KIDS_MALES ] > 1 ) {
				MainView.outputText( 'they\'re' );
			} else {
				MainView.outputText( 'he\'s' );
			}
			MainView.outputText( ' going to be quite developed when ' );
			if( CoC.flags[ kFLAGS.URTA_KIDS_MALES ] > 1 ) {
				MainView.outputText( 'they hit' );
			} else {
				MainView.outputText( 'he hits' );
			}
			MainView.outputText( ' puberty, if not as big as Urta.  Urta is quite proud of ' );
			if( CoC.flags[ kFLAGS.URTA_KIDS_MALES ] > 1 ) {
				MainView.outputText( 'them' );
			} else {
				MainView.outputText( 'him' );
			}
			MainView.outputText( ', and often boasts that ' );
			if( CoC.flags[ kFLAGS.URTA_KIDS_MALES ] > 1 ) {
				MainView.outputText( 'they' );
			} else {
				MainView.outputText( 'he' );
			}
			MainView.outputText( ' will grow up to be ' );
			if( CoC.flags[ kFLAGS.URTA_KIDS_MALES ] > 1 ) {
				MainView.outputText( 'strong fighters like she is.' );
			} else {
				MainView.outputText( 'a strong fighter like she is.' );
			}
		}
		...
		if( CoC.flags[ kFLAGS.URTA_KIDS_HERMS ] > 0 ) {
			...
			if( CoC.flags[ kFLAGS.URTA_FIRSTBORN_GENDER ] === 3 ) {
				MainView.outputText( '  Your firstborn is a hermaphrodite, sporting a' );
				if( CoC.flags[ kFLAGS.URTA_FIRSTBORN_COCKTYPE ] === 2 ) {
					MainView.outputText( ' vulpine penis' );
				} else {
					MainView.outputText( 'n equine phallus, a peculiarity she inherited from her mother' );
				}
				MainView.outputText( '.' );
			}
			if( CoC.flags[ kFLAGS.URTA_LATESTBORN_GENDER ] === 3 && this.urtaKids() > 1 ) {
				MainView.outputText( '  Your latest addition is ' );
				if( CoC.flags[ kFLAGS.URTA_FIRSTBORN_GENDER ] === 3 ) {
					MainView.outputText( 'also ' );
				}
				MainView.outputText( 'a herm, with a' );
				if( CoC.flags[ kFLAGS.URTA_LATESTBORN_COCKTYPE ] === 2 ) {
					MainView.outputText( ' vulpine penis' );
				} else {
					MainView.outputText( 'n equine phallus.  No doubt such a characteristic was passed from her mother' );
				}
				MainView.outputText( '.' );
			}
			MainView.outputText( '  You have a feeling ' );
			...
			if( CoC.flags[ kFLAGS.URTA_KIDS_HERMS ] > 1 ) {
				...
				MainView.outputText( 'she hits' );
			}
			MainView.outputText( ' puberty, if maybe not quite as big as Urta.  Urta particularly dotes on ' );
			if( CoC.flags[ kFLAGS.URTA_KIDS_HERMS ] > 1 ) {
				MainView.outputText( 'them' );
			} else {
				MainView.outputText( 'her' );
			}
			MainView.outputText( ', maybe because she remembers how much her parents hated her for being a herm.' );
		}
		...
		if( CoC.flags[ kFLAGS.URTA_KIDS_FEMALES ] > 0 ) {
			...
			MainView.outputText( '.' );
			if( CoC.flags[ kFLAGS.URTA_FIRSTBORN_GENDER ] === 2 ) {
				MainView.outputText( '  Your firstborn is a girl.' );
			}
			if( CoC.flags[ kFLAGS.URTA_LATESTBORN_GENDER ] === 2 && this.urtaKids() > 1 ) {
				...
				MainView.outputText( 'a girl.' );
			}
			MainView.outputText( '  Urta loves to play with ' );
			if( CoC.flags[ kFLAGS.URTA_KIDS_FEMALES ] > 1 ) {
				MainView.outputText( 'them' );
			} else {
				MainView.outputText( 'her' );
			}
			MainView.outputText( ', and swears ' );
			if( CoC.flags[ kFLAGS.URTA_KIDS_FEMALES ] > 1 ) {
				MainView.outputText( 'they will' );
			} else {
				MainView.outputText( 'she will' );
			}
			MainView.outputText( ' be quite the heartbreaker' );
			if( CoC.flags[ kFLAGS.URTA_KIDS_FEMALES ] > 1 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ' when ' );
			if( CoC.flags[ kFLAGS.URTA_KIDS_FEMALES ] > 1 ) {
				MainView.outputText( 'they grow' );
			} else {
				MainView.outputText( 'she grows' );
			}
		...
		if( this.urtaKids() > 1 ) {
			MainView.outputText( 'nearest ' );
		}
		MainView.outputText( 'furry little form of your child and gently tug them over to you.  ' );
		if( CoC.flags[ kFLAGS.URTA_LATESTBORN_GENDER ] >= 2 ) {
			MainView.outputText( 'She' );
		} else {
			MainView.outputText( 'He' );
		}
		MainView.outputText( ' kicks and splashes with ' );
		if( CoC.flags[ kFLAGS.URTA_LATESTBORN_GENDER ] >= 2 ) {
			MainView.outputText( 'her' );
		} else {
			MainView.outputText( 'his' );
		}
		MainView.outputText( ' little legs, but smiles up at you, clearly willing to cooperate, giving you no problem as you spurt a generous dollop of the soapy goo into ' );
		if( CoC.flags[ kFLAGS.URTA_LATESTBORN_GENDER ] >= 2 ) {
			MainView.outputText( 'her' );
		} else {
			MainView.outputText( 'his' );
		}
		MainView.outputText( ' hair and start to massage it... now, just how do you shampoo somebody covered from head to toe in fur...?' );
		...
		MainView.outputText( ' flicks ' );
		if( CoC.flags[ kFLAGS.URTA_LATESTBORN_GENDER ] >= 2 ) {
			MainView.outputText( 'her' );
		} else {
			MainView.outputText( 'his' );
		}
		MainView.outputText( ' tail, splashing Lianna with water.  The skunkette, however, fails to display any reaction at this.' );
		...
		MainView.outputText( '\'s chest, tickling ' );
		if( CoC.flags[ kFLAGS.URTA_LATESTBORN_GENDER ] >= 2 ) {
			MainView.outputText( 'her' );
		} else {
			MainView.outputText( 'him' );
		}
		MainView.outputText( ' sides as she does so.  The little fox giggles and squirms, clearly finding the actions quite ticklish, and you have to hold on a little harder to keep ' );
		if( CoC.flags[ kFLAGS.URTA_LATESTBORN_GENDER ] >= 2 ) {
			MainView.outputText( 'her' );
		} else {
			MainView.outputText( 'him' );
		}
		MainView.outputText( ' from escaping.  "<i>Now all you have to do is rinse off.</i>"  She grabs a small bucket nearby, scooping up some water.  "<i>Close your eyes, dear.</i>"  Your child does as Lianna instructs; this is evidently a familiar enough routine that ' );
		if( CoC.flags[ kFLAGS.URTA_LATESTBORN_GENDER ] >= 2 ) {
			MainView.outputText( 'she' );
		} else {
			MainView.outputText( 'he' );
		}
		MainView.outputText( ' has no intention of winding up with soap in ' );
		if( CoC.flags[ kFLAGS.URTA_LATESTBORN_GENDER ] >= 2 ) {
			MainView.outputText( 'her' );
		} else {
			MainView.outputText( 'his' );
		}
		MainView.outputText( ' eyes.  Lianna turns the bucket, letting the water cascade over ' );
		if( CoC.flags[ kFLAGS.URTA_LATESTBORN_GENDER ] >= 2 ) {
			MainView.outputText( 'her' );
		} else {
			MainView.outputText( 'his' );
		}
		MainView.outputText( ' head.  Your little fox squeals and then shakes wildly to fling the water off of ' );
		if( CoC.flags[ kFLAGS.URTA_LATESTBORN_GENDER ] >= 2 ) {
			MainView.outputText( 'her' );
		} else {
			MainView.outputText( 'his' );
		}
		MainView.outputText( ' head, kicking up a cascade of water as ' );
		if( CoC.flags[ kFLAGS.URTA_LATESTBORN_GENDER ] >= 2 ) {
			MainView.outputText( 'she' );
		} else {
			MainView.outputText( 'he' );
		}
		MainView.outputText( ' does so' );
		if( this.urtaKids() > 2 ) {
			MainView.outputText( ', to the delight of ' );
			if( CoC.flags[ kFLAGS.URTA_LATESTBORN_GENDER ] >= 2 ) {
				MainView.outputText( 'her' );
			} else {
				MainView.outputText( 'his' );
			}
			MainView.outputText( ' siblings, who are safely watching from the other side of the bath' );
		}
		MainView.outputText( '.' );
		...
		MainView.outputText( ' hair is done, shaking some of the water out of your own face.' );
		//1 child only:;
		if( this.urtaKids() === 1 ) {
			...
			MainView.outputText( ' tail.  Get out of the tub.</i>"  The skunkette instructs.  The young fox nods and scrabbles at the side of the bath, hauling ' );
			if( CoC.flags[ kFLAGS.URTA_LATESTBORN_GENDER ] >= 2 ) {
				MainView.outputText( 'her' );
			} else {
				MainView.outputText( 'him' );
			}
			MainView.outputText( 'self up and out so Lianna can start tending to ' );
			if( CoC.flags[ kFLAGS.URTA_LATESTBORN_GENDER ] >= 2 ) {
				MainView.outputText( 'her' );
			} else {
				MainView.outputText( 'his' );
			}
			MainView.outputText( ' tail.  You promptly make yourself useful and start scrubbing as well...' );
		} else {
			MainView.outputText( '\n\n"<i>Good, now who\'s next?</i>"  "<i>Me!</i>"  ' );
			if( this.urtaKids() > 2 ) {
				MainView.outputText( 'One of your other children' );
			} else {
				MainView.outputText( 'Your other child' );
			}
			MainView.outputText( ' pipes up.  You let go of the ' );
			if( CoC.flags[ kFLAGS.URTA_LATESTBORN_GENDER ] >= 2 ) {
				MainView.outputText( 'girl' );
			} else {
				MainView.outputText( 'boy' );
			}
			MainView.outputText( ' you\'re currently holding so ' );
			if( CoC.flags[ kFLAGS.URTA_LATESTBORN_GENDER ] >= 2 ) {
				MainView.outputText( 'she' );
			} else {
				MainView.outputText( 'he' );
			}
			MainView.outputText( ' can swim away, allowing the new volunteer to take ' );
			if( CoC.flags[ kFLAGS.URTA_LATESTBORN_GENDER ] >= 2 ) {
				MainView.outputText( 'her' );
			} else {
				MainView.outputText( 'his' );
			}
			MainView.outputText( ' place.  You promptly make yourself useful and start scrubbing as well...' );
		}
	...
	UrtaPregs.prototype.noBathPleaseUrtaLian = function() {
		MainView.clearOutput();
		MainView.outputText( 'You politely refuse her invitation.  "<i>Okay, but can you wash mine then?</i>" she asks.  You give the idea a moment\'s thought and then agree; it\'s a harmless offer, right?  "<i>Thanks,</i>" she replies, getting out of the tub and sitting on a small wooden bench nearby.  "<i>Use the flower extract shampoo, please.</i>"  You nod your head in understanding as you climb out after her, taking up the indicated shampoo and approaching her before asking how much you should use.  "<i>Plenty,</i>" is her reply.' );
	...
	UrtaPregs.prototype.getABackWashFromLianna = function() {
		MainView.clearOutput();
		MainView.outputText( 'You tell Lianna that if she is willing, you wouldn\'t object to her washing your back.' );
	...
	UrtaPregs.prototype.takeTheKidsForWalk = function() {
		MainView.clearOutput();
		MainView.outputText( '"<i>You\'d like to see ' + this.urtaKidsText( 'him', 'her', 'them' ) + '?  Sure, but first I need to get ' + this.urtaKidsText( 'him', 'her', 'them' ) + ' ready.</i>"   You ask her where she plans on taking ' + this.urtaKidsText( 'him', 'her', 'them' ) + '.' );
		...
		MainView.outputText( ' are suitably dressed for a traipse around a desert city, you usher your kit' );
		if( this.urtaKids() > 1 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ' out the door and into the street.  "<i>Can ' + this.urtaKidsText( 'I', 'I', 'we' ) + ' get some ice cream?!</i>"' );
		if( this.urtaKids() > 1 ) {
			MainView.outputText( '  "<i>Yeah, can we ' + CoC.player.mf( 'daddy', 'mommy' ) + '?!</i>"' );
		}
		MainView.outputText( '  Your child' );
		if( this.urtaKids() > 1 ) {
			MainView.outputText( 'ren plead' );
		} else {
			MainView.outputText( ' pleads' );
		}
		MainView.outputText( '.  Well, the vendors down at the market would make for a suitably far destination, which should give Lianna plenty of time for herself, and you yourself wouldn\'t mind some ice cream either at that, so it\'s decided.  "<i>Yay! You\'re the best ' + CoC.player.mf( 'dad', 'mom' ) + ' ever!</i>" ' );
		if( this.urtaKids() > 1 ) {
			MainView.outputText( 'a chorus of excited cheers sounds out.' );
		} else {
			MainView.outputText( 'an excited cheer sounds out.' );
		}
		MainView.outputText( '  It\'s hard not to want to live up to that title with such ' );
		if( this.urtaKids() === 1 ) {
			MainView.outputText( 'a happy face' );
		} else {
			MainView.outputText( 'happy faces' );
		}
		MainView.outputText( ' staring up at you.' );
		...
		MainView.outputText( ' pick out flavours while the shopkeeper scoops them up onto wafer cones.  Once he\'s done, you thank the wolf and leave the shop followed by your ' );
		if( this.urtaKids() > 10 ) {
			MainView.outputText( 'army of ' );
		}
		MainView.outputText( 'child' );
		if( this.urtaKids() > 1 ) {
			MainView.outputText( 'ren' );
		...
		MainView.outputText( ' don\'t bother waiting or savouring their ice cream and neither do you, it\'s damned hot walking around this desert.  There\'s less cause for mischief too, ' + this.urtaKidsText( 'his', 'her', 'their' ) + ' face' );
		if( this.urtaKids() === 1 ) {
			MainView.outputText( 'buried in cone' );
		} else {
			MainView.outputText( 's buried in their cones' );
		}
		MainView.outputText( ' along with your own, greedily licking up melting droplets.  There\'s just a bit of cone left as you return to Urta\'s house, and you quickly polish that off, upon entering.  "<i>Oh, hey.  You\'re back.</i>"  Lianna greets you in her usual inexpressive fashion.' );
		MainView.outputText( '\n\n"<i>How did it go?</i>"' );
		...
		MainView.outputText( ' towards the bathroom.  There is a ' + this.urtaKidsText( 'shrill protest', 'shrill protest', 'chorus of shrill protests' ) + ', but ' + this.urtaKidsText( 'he', 'she', 'they' ) + ' clearly understand' );
		if( this.urtaKids() === 1 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ' that Lianna is not one to be trifled with and so ' + this.urtaKidsText( 'he', 'she', 'they' ) + ' march' );
		if( this.urtaKids() === 1 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ' off to the bathroom.  You can\'t help but note how good Lianna is at making kids do what she says.' );
	...
	UrtaPregs.prototype.visitTheHouse = function() {
		...
		/*{if (Urta is pregnant at stage 5 or above) && (random chance) >= 192 && Utils.rand(2) === 0) {
		 ...
		 MainView.outputText(' for a walk.  Be back soon!  - Lianna');
	...
	UrtaPregs.prototype.talkToLianaAboutHerJob = function() {
		MainView.clearOutput();
		MainView.outputText( 'You ask her about her jobs, both for Urta and what she intends to do afterwards.' );
	...
	UrtaPregs.prototype.talkToLianaAboutHerResearch = function() {
		MainView.clearOutput();
		MainView.outputText( 'You ask her why she\'s so eager to get samples from you, a human.  What sort of research is she doing that she needs human samples?' );
		...
		if( CoC.player.cor >= 50 ) {
			MainView.outputText( 'however half-heartedly it may be, ' );
		}
		MainView.outputText( 'and ask if that\'s what pushed her to studying humans and Marethians.' );
	...
	UrtaPregs.prototype.talkAboutLiannasLubeSpray = function() {
		...
		MainView.outputText( 'You ask why it is that she produces a lubricating liquid rather than a noxious one, like you would expect of a skunk.' );
	...
	UrtaPregs.prototype.lubeSpraySideEffects = function() {
		MainView.clearOutput();
		MainView.outputText( 'So, if she\'s experimented on herself in the past, that must have gone wrong for her at least once or twice in the past, you comment.' );
		...
		if( CoC.player.cor < 33 ) {
			MainView.outputText( 'mild' );
		} else if( CoC.player.cor < 66 ) {
			MainView.outputText( 'some' );
		} else {
			MainView.outputText( 'great' );
		}
		MainView.outputText( ' reluctance.' );
	...
	UrtaPregs.prototype.sexWithLianna = function() {
		MainView.clearOutput();
		MainView.outputText( '"<i>So you want to help me with my experiments?  Great!</i>"  She takes your hand and basically drags you into her room.  You allow her to lead you, wondering what she has in mind.' );
		...
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( '  Your lower garments suddenly feel much tighter.' );
		}
	...
	UrtaPregs.prototype.refuseGenderlessInjection = function() {
		MainView.clearOutput();
		MainView.outputText( 'Lianna\'s face falls at your refusal, but she sighs.  "<i>All right, I can understand; I mean, you must have your reasons.  Still, if ever you get a gender, remember that I\'d be happy to use some of your seed, please.</i>"' );
	...
	UrtaPregs.prototype.acceptARandomGenderFromASkunk = function() {
		MainView.clearOutput();
		MainView.outputText( 'You decide that it\'s worthwhile to go along and indicate that you\'ll let Lianna experiment on you with her "genderiser".' );
		...
		//{if Centaur: ;
		if( CoC.player.isTaur() ) {
			MainView.outputText( '  Or at least hold on to something, this might sting a little and I don\'t need you kicking down anything in here.' );
		}
		MainView.outputText( '</i>"' );
		...
		MainView.outputText( '  You really should be more surprised, but, sadly, this is kind of predictable; you were more hoping than anything that this would be an oral delivery, or at least an injection.' );
		...
		//if PC becomes Female:;
			...
			if( CoC.player.biggestTitSize() < 1 ) {
				...
				if( CoC.player.nippleLength < 0.5 ) {
					MainView.outputText( '  Your nipples erect into small half-inch nubs, complementing the transformation.' );
		...
		//if PC becomes Herm:;
			...
			if( CoC.player.biggestTitSize() < 3 ) {
				...
				MainView.outputText( ' swelling into a soft C-Cup.' );
				if( CoC.player.nippleLength < 0.5 ) {
					MainView.outputText( '  Your nipples erect into small, half-inch nubs, complementing the transformation.' );
					CoC.player.nippleLength = 0.5;
				}
				MainView.outputText( '  You watch the soft mounds in your chest, unable to do anything but pant.' );
	...
	UrtaPregs.prototype.getEggsHarvested = function() {
		MainView.clearOutput();
		MainView.outputText( '"<i>All right, let\'s get this show on the road.</i>"  She ducks out into her closet once more, and comes out carrying what looks like a dildo and a lollipop.  "<i>So, let me explain.  It\'s pretty simple really.</i>"  She takes the dildo in her hand.  "<i>This is a special dildo I had custom-made for this experiment, It will pleasure you at the same time it lets me harvest your pussy juice and hopefully one of your ovum.  All you have to do is sit back and relax.  Any questions?</i>"' );
		...
		if( CoC.flags[ kFLAGS.LIANNA_HAVESTED_LADIES ] === 0 ) {
			MainView.outputText( 'confusion, wondering what she\'s playing at' );
			CoC.flags[ kFLAGS.LIANNA_HAVESTED_LADIES ]++;
		} else {
			MainView.outputText( 'remembrance, knowing what\'s coming next' );
		}
		MainView.outputText( '.  She unceremoniously begins shoving it inside without even a hint of pleasure or discomfort; the dildo slides in without resistance, spreading her buttcheeks apart and penetrating her with a wet squelch.  You watch the display, not sure whether or not you should find this kinky.' );
		...
		if( CoC.player.vaginalCapacity() >= 50 ) {
			MainView.outputText( '  "<i>Hmm, I might not have had to lube it up after all...</i>"  The skunkette remarks.' );
		}
		MainView.outputText( '  She aligns the tip of the dildo with you and without saying a word she begins pushing in.  Instinctively you moan and try to spread yourself as wide as possible.' );
		...
		if( CoC.player.looseness() >= 4 ) {
			MainView.outputText( '; your loose pussy doesn\'t hurt, either' );
		}
		MainView.outputText( '.  Finally, you can feel it stop inside of you, the tip pushing against your cervix, which is what has brought it to a halt.' );
		...
		if( CoC.player.hasCock() ) {
			MainView.outputText( ' except for the ' );
			...
			MainView.outputText( ' splatters of cum from your neglected penis' );
		}
		MainView.outputText( '; the dildo has absorbed it all.  You can feel it clenching tighter, shrinking down so that it no longer fills you up so perfectly, leaving you panting in release as Lianna gently frees it from inside you.' );
	...
	UrtaPregs.prototype.liannaHandjobbies = function() {
		...
		MainView.outputText( '"<i>All right, let\'s get this show on the road.</i>"  She ducks out into her closet once more, and comes out carrying what looks like a condom and a lollipop.  "<i>Here\'s how this works.  You put on this condom and I milk your seed out of you.  Simple, yes?</i>"' );
		...
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( '  "<i>However I think milking just one is good enough.</i>"' );
		}
		MainView.outputText( '  She motions to the bed.' );
		...
		if( CoC.player.cockTotal() > 1 ) {
			...
			MainView.outputText( ', effectively binding them at their base.  You groan hollowly at this; though this certainly will mean less mess, you\'re not so sure you\'re going to enjoy the sensation of trying to cum through the ' + Utils.num2Text( CoC.player.cockTotal() - 1 ) + ' cocks that have been blocked up.  Satisfied with her work, she looks at your [cock biggest].' );
		}
		...
		if( CoC.player.balls > 0 ) {
			MainView.outputText( 'occasionally stopping to caress your [balls], ' );
		}
		MainView.outputText( 'while the other massages your [cockHead biggest], rolling the steadily growing bubble of pre around as she kneads it.' );
