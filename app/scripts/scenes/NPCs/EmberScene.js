...
angular.module( 'cocjs' ).run( function( $log, WeaponLib, PregnancyStore, CockTypesEnum, Combat, Ember, PerkLib, $rootScope, Descriptors, ImageManager, AppearanceDefs, EventParser, CoC, kFLAGS, Utils, StatusAffects, EngineCore, ConsumableLib ) {
	...
	EmberScene.prototype.emberCampMenu = function() {
		...
		if( this.emberAffection() <= 25 ) {
			EngineCore.outputText( 'Ember sighs as you approach, and doesn\'t even look you in the eye before speaking.  "<i>What do you want?</i>"' );
		}//Moderate Affection:;
		else if( this.emberAffection() <= 75 ) {
			EngineCore.outputText( 'Ember fidgets as ' + this.emberMF( 'his', 'her' ) + ' tail starts to sway from side to side, ' + this.emberMF( 'he', 'she' ) + ' looks at you and asks, "<i>What is it?</i>"' );
		} else {
			EngineCore.outputText( 'Ember\'s eyes light up as you close in on ' + this.emberMF( 'him', 'her' ) + ', and ' + this.emberMF( 'he', 'she' ) + ' smiles nervously.  "<i>Y-Yes?</i>"' );
		}
		//OPTIONS HERE;
		//[APPEARANCE];
		...
		EngineCore.choices( 'Appearance', this.embersAppearance, 'Talk', this.talkToEmber, 'DrinkBlood', this.bloodForTheBloodGod, 'Drink Milk', milk, 'Get Egg', egg,
			'Sex', this.emberSexMenu, 'Spar', this.decideToSparEmbra, '', null, '', null, 'Back', CoC.getInstance().scenes.camp.campFollowers );
	};
	...
	EmberScene.prototype.emberSexMenu = function( output ) {
		...
		//EngineCore.choices('Catch Anal',this.catchAnal,'Pitch Anal',pitchAnal,'Blow Ember',blowEmber,'Get Blown',getBlown,'Eat Her Out',eatOut,'Get Eaten Out',getEatenOut,'Penetrate Her',penetrateHer,'Get Penetrated',getPenetrated,'',0,'Leave',this.emberCampMenu);;
		EngineCore.menu();
		EngineCore.addButton( 0, 'Catch Anal', catchAnal_ );
		EngineCore.addButton( 1, 'Pitch Anal', pitchAnal );
		EngineCore.addButton( 2, 'Blow Ember', blowEmber );
		EngineCore.addButton( 3, 'Get Blown', getBlown );
		EngineCore.addButton( 4, 'Eat Her Out', eatOut );
	...
	EmberScene.prototype.destroyBabyEmberYouMonster = function() {
		...
		//[Eat][Leave];
		EngineCore.choices( 'Eat It', this.eatEmbersYolkLikeAnEvenBiggerDick, '', null, '', null, '', null, 'Leave', CoC.getInstance().scenes.camp.returnToCampUseOneHour );
	};
	//[=Eat=];
	...
	EmberScene.prototype.emberCampDesc = function() {
		...
		//NOT AN EGG! HAHA!;
			...
			if( choice === 0 ) {
				EngineCore.outputText( 'Ember is lying in front of ' + this.emberMF( 'his', 'her' ) + ' excavated den, watching the camp and resting.  Every once in a while ' + this.emberMF( 'his', 'her' ) + ' eyes dart in your direction, before ' + this.emberMF( 'he', 'she' ) + ' quickly looks away.\n\n' );
			} else if( choice === 1 ) {
				EngineCore.outputText( 'Ember doesn\'t seem to be around ' + this.emberMF( 'his', 'her' ) + ' excavated den... it doesn\'t take long before Ember lands in front of it and then takes off again, apparently occupied with darting around in the sky for fun.\n\n' );
			} else {
				EngineCore.outputText( 'Ember appears to be fighting to stay awake; sometimes ' + this.emberMF( 'he', 'she' ) + ' falls into a doze, but quickly snaps back to consciousness.\n\n' );
			}
		}
	};
	//Followers Descriptor (Z);
	EmberScene.prototype.emberFollowerDesc = function() {
		EngineCore.outputText( 'The mysterious egg you found in the cave sits in the grass nest you assembled for it; it is three feet tall and nearly two feet in circumference.  The nest itself isn\'t very pretty, but at least it\'s sturdy enough to keep the egg from rolling around.\n\n' );
	};
	...
	EmberScene.prototype.emberEggInteraction = function() {
		...
		if( CoC.getInstance().player.lust >= 33 && (CoC.getInstance().flags[ kFLAGS.EMBER_EGG_FLUID_COUNT ] < 5 || CoC.getInstance().flags[ kFLAGS.EMBER_JACKED_ON ] === 0) ) {
			...
			//[Yes][No];
			//[= Yes =];
			EngineCore.doYesNo( this.masturbateOntoAnEgg, this.dontEggFap );
		...
		if( hatch !== null ) {
			EngineCore.choices( 'Hatch', hatch, 'Blood', this.giveEmberBludSausages, 'IncubiDraft', draft, 'Pure Draft', pDraft, 'Succubi Milk', milk, 'Pure Milk', pMilk, 'Hair Serum', hair, 'Ovi Elixir', ovi, 'Lactaid', lactaid, 'Back', this.leaveWithoutUsingAnEmberItem );
		} else if( fap !== null ) {
			EngineCore.choices( 'Masturbate', fap, 'Blood', this.giveEmberBludSausages, 'IncubiDraft', draft, 'Pure Draft', pDraft, 'Succubi Milk', milk, 'Pure Milk', pMilk, 'Hair Serum', hair, 'Ovi Elixir', ovi, 'Lactaid', lactaid, 'Back', this.leaveWithoutUsingAnEmberItem );
		} else {
			EngineCore.choices( 'Hatch', hatch, 'Blood', this.giveEmberBludSausages, 'IncubiDraft', draft, 'Pure Draft', pDraft, 'Succubi Milk', milk, 'Pure Milk', pMilk, 'Hair Serum', hair, 'Ovi Elixir', ovi, 'Lactaid', lactaid, 'Back', this.leaveWithoutUsingAnEmberItem );
		}
	};
	...
	EmberScene.prototype.dontEggFap = function() {
		...
		EngineCore.outputText( '  The egg\'s rhythm suddenly changes; as if it were excited by something - something that you have brought near it.' );
	...
	EmberScene.prototype.hatchZeMuzzles = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.EMBER_ROUNDFACE ] === 0 ) {
			...
			//Female Anthro (Z);
				...
				EngineCore.outputText( ' the monster\'s crotch; there, you see that the fine scales actually separate to reveal a slick-looking pussy.  She\'s clearly a female, with no noteworthy \'additions\' that you can see.' );
			}
			//Herm Anthro (Z);
			else {
				EngineCore.outputText( '\n\nIt\'s huge, standing 7 feet tall at the very least.  Its build is lean and slender, with powerful arms and legs that end in reptilian claws, complete with splay-toed feet capped by menacing talons.' );
				...
				EngineCore.outputText( 'the monster\'s crotch; there, you see the scales part in two places.  The lower opening is unmistakably a pussy; but from the slit just above it suddenly distends a 16-inch penis unlike anything you\'ve ever seen before, bearing a rounded, elongated head and a series of ridges that give it an almost segmented look.  Beneath it, a pair of apple-sized balls fall into place heavily, leaving you no doubt that she is a hermaphrodite.' );
			}
		}
		//Boring version;
		else {
			...
			//Herm Monstergirl (Z);
				...
				EngineCore.outputText( ' and apple-size nuts held inside a fleshy sack slung under - the ensemble hangs nearly level with her knees.  She... or he? is obviously a hermaphrodite.' );
			}
		}
		//Aftermath (Z);
		EngineCore.doNext( this.meetEmberAftermath );
	};
	//Aftermath (Z);
	...
	EmberScene.prototype.embersAppearance = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.EMBER_ROUNDFACE ] === 0 ) {
			...
			switch( this.pregnancy.event ) {
				case 2:
					EngineCore.outputText( '  At the moment there\'s a slight pudgyness to her golden belly.' );
					break;
				case 3:
					EngineCore.outputText( '  Her golden belly is a little larger and firmer than usual.' );
					break;
				case 4:
				case 5:
					EngineCore.outputText( '  Her golden belly has grown quite a bit.  Ember often rests her hand on it, especially when she sees you looking at it.' );
					break;
				case 6:
		...
		//Dragon-girl Appearance (By Radar) (Z);
			...
			EngineCore.outputText( ' dragon-' + this.emberMF( 'boy', 'girl' ) + ', with slender limbs and a thin frame; ' + this.emberMF( 'he', 'she' ) + ' refuses to wear any kind of clothing and exposes ' + this.emberMF( 'him', 'her' ) + 'self naturally to the world.  ' + this.emberMF( 'He', 'She' ) + ' sports a rather human looking face, but with several key differences.  Where a normal set of human eyes would be located, instead a pair of orange, reptilian eyes stares back at you, filled with immeasurable pride and ferocity.' );
			...
			EngineCore.outputText( '.  Just below ' + this.emberMF( 'his', 'her' ) + ' collarbone, in the middle of ' + this.emberMF( 'his', 'her' ) + ' chest, you see what looks like a small, golden, heart-shaped scale; adorning the chest like a birthmark of some sort.' );
		...
		EngineCore.doNext( this.emberCampMenu );
	};
	//Talk;
	EmberScene.prototype.talkToEmber = function() {
		...
		//Talk about Yourself;
		EngineCore.choices( 'Dragons', this.talkToEmberAboutDragonzzz, 'Exploring', this.discussExplorationWithEmber, 'Yourself', this.talkToEmberAboutYourself, '', null, 'Back', this.emberCampMenu );
	};
	...
	EmberScene.prototype.discussExplorationWithEmber = function() {
		...
		if( choice === 0 ) {
			...
			if( subChoice === 0 ) {
				EngineCore.outputText( 'imps. Only a few imps, and they seemed very nervous. I\'m guessing that whatever naturally lives there is something that they really don\'t want to get involved with.' );
			} else if( subChoice === 1 ) {
				EngineCore.outputText( 'two goblins in the foothills, arguing with each other. One was saying that the other shouldn\'t go up into the hills, as apparently the minotaurs that live there are too much for an untrained girl like her to take without being split open. The second goblin just laughed at her, called her a \'wimp\' and told her that she\'s going to get herself \'some juicy bull-cock and tasty mino-spooge\'. Ugh, disgusting little creatures.' );
			} else if( subChoice === 2 ) {
				EngineCore.outputText( 'a pair of muscle-bound bull-men beating on each other with their bare fists. They spent over an hour smashing each other into a bloody pulp, and then the winner promptly started fucking the loser up the ass.  I had seen more than enough by that point, so I left.' );
			} else if( subChoice === 3 ) {
			...
			if( subChoice === 0 ) {
				EngineCore.outputText( 'a whole tribe of imps, just lounging around in a glade, jerking themselves off or squabbling over food.  Nasty little things, but easily dispatched.' );
			} else if( subChoice === 1 ) {
				EngineCore.outputText( 'a  goblin with a huge pregnant belly, laughing to herself and swilling down that ale they brew, slopping it all over herself.  Little hedonists.' );
			} else if( subChoice === 2 ) {
			...
			else if( subChoice === 3 && !CoC.getInstance().scenes.tamaniScene.pregnancy.isPregnant ) {
			...
			if( subChoice === 0 ) {
				EngineCore.outputText( 'a woman with four big breasts, squeezing milk out of her tits and into the sand.  I didn\'t know breasts could hold that much milk!' );
			} else if( subChoice === 1 ) {
	...
	EmberScene.prototype.talkToEmberAboutYourself = function() {
		...
		//(If PC Str >= 90);
			...
			EngineCore.outputText( '  "<i>With muscles like these, you could easily lift anything you wanted... even me...</i>" Ember trails off dreamily, almost purring.  "<i>N-Not that I\'d want you to carry me in your arms or anything...</i>"' );
			//(+2 points);
			points += 2;
		}
		...
		//(If PC Tou >= 90);
			...
			EngineCore.outputText( '"<i>Your body\'s as tough as rock, you should show off more frequently...  N-Not that I\'d want you to go naked!  You should always wear something to protect yourself,</i>" ' + this.emberMF( 'he', 'she' ) + ' quickly adds.' );
			//(+1 Affection);
		...
		//(If PC Int >= 90);
			...
			EngineCore.outputText( 'Ember\'s eyes widen.  "<i>I see great wisdom in your eyes...</i>"  Ember\'s face grows more absent by the minute as ' + this.emberMF( 'he', 'she' ) + ' looks at you in deep thought, until the trance breaks.  ' + this.emberMF( 'He', 'She' ) + ' blows at your face, brushing a claw on your forehead.  "<i>Y-You had something in your face... I wasn\'t staring into your steely eyes.</i>"' );
			//(+2 points);
			points += 2;
		}
		...
		//(If PC Spd >= 90);
			...
			EngineCore.outputText( 'You easily catch the pebble and throw it back at Ember, surprising ' + this.emberMF( 'him', 'her' ) + '.  "<i>With reflexes like these, you could even...</i>" Ember mumbles.  "<i>D-Don\'t even think about it!</i>" ' + this.emberMF( 'he', 'she' ) + ' snaps.' );
			//(+2 points;
			points += 2;
		}
		...
		if( points < 2 ) {
			EngineCore.outputText( '"<i>This is not good at all; it\'s a miracle you even managed to survive in this world thus far.  With the things I\'ve seen roaming the land...</i>" Ember trails off, concerned.  "<i>You should stay in the camp; I\'ll help you train.</i>"  You don\'t much like the assessment.' );
		}
		//(else if points < 6);
		else if( points < 6 ) {
			EngineCore.outputText( '"<i>You\'re doing fine, but make sure you don\'t slack off, and keep training.</i>"  You thank Ember for sharing ' + this.emberMF( 'his', 'her' ) + ' thoughts.' );
		}
		//(else);
		else {
			EngineCore.outputText( '"<i>You\'re quite a catch... if you strolled down the street you\'d have dragons fawning all over you...</i>"  Realizing what ' + this.emberMF( 'he', 'she' ) + ' just said, Ember coughs furiously.  "<i>I-I mean... lesser dragons might fawn all over you.  You don\'t meet my standards!</i>"' );
			this.emberAffection( 5 );
		}
		EngineCore.doNext( CoC.getInstance().scenes.camp.returnToCampUseOneHour );
	};
	...
	EmberScene.prototype.manEmberBitchesAboutPCPregnancy = function() {
		...
		if( this.emberAffection() <= 25 ) {
			...
			EngineCore.outputText( 'Ember stares coldly at your gravid midriff as icebreaker after icebreaker from you falls flat.  You ask what\'s wrong.  "<i>Nothing.  Nothing is wrong,</i>" Ember states, flatly.' );
		}//(High Affection);
		else {
			EngineCore.outputText( 'The expression Ember gives you is very cold.  "<i>What is the meaning of this!?</i>" ' + this.emberMF( 'he', 'she' ) + ' says, motioning towards your bloated belly.  "<i>You\'re supposed to help <b>me</b> breed more dragons; not slut around with trash!</i>"  Ember blows out an indignant puff of smoke and walks away.' );
		}
	};
	//Scene appears when selecting [Talk];
	...
	EmberScene.prototype.noMoDagronBlud = function() {
		...
		if( this.emberAffection() < 75 ) {
			...
			EngineCore.outputText( 'You decide to stop for now and pull away.  Ember licks ' + this.emberMF( 'his', 'her' ) + ' lips, draws ' + this.emberMF( 'his', 'her' ) + ' tongue back into ' + this.emberMF( 'his', 'her' ) + ' mouth and purrs with delight.  When ' + this.emberMF( 'he', 'she' ) + ' realizes what ' + this.emberMF( 'he', 'she' ) + '\'s doing, though, ' + this.emberMF( 'he', 'she' ) + ' sobers.  "<i>D-Don\'t get any strange ideas...</i>"' );
	...
	EmberScene.prototype.emberTFs = function() {
		...
		if( changes < changeLimit && CoC.getInstance().player.dragonCocks() < CoC.getInstance().player.totalCocks() && Utils.rand( 3 ) === 0 ) {
			...
			EngineCore.outputText( '  <b>You now have a dragon penis.</b>' );
	...
	EmberScene.prototype.emberIsAnEggFactory = function() {
		...
		if( this.emberAffection() <= 25 ) {
			...
			EngineCore.outputText( '.  One of her hands gently teases her clit, as another traces her outer labia; she bites her lips, trying to stifle her moans of pleasure, but it\'s useless... every touch brings a sigh.' );
		...
		//(Medium Affection);
			...
			//[Watch][Fob Off];
			EngineCore.choices( 'Watch', this.watchMediumAffectionEmberEggLay, 'Don\'t Watch', this.dontWatchEmberLayEgg, '', null, '', null, '', null );
		}
		//(High Affection);
		else {
	...
	EmberScene.prototype.getMilkFromEmber = function() {
		...
		if( this.emberAffection() <= 25 ) {
			...
			EngineCore.outputText( '.  ' + this.emberMF( 'He', 'She' ) + ' lets out a cute gasp of pleasure, eyes darting furiously around to look at anything but you. You beam a smile and ' + this.emberMF( 'he', 'she' ) + ' turns her head away in embarrassment.  "<i>T-They\'re not getting any emptier like this, y\'know!?</i>"' );
		...
		//(High Affection);
			...
			if( CoC.getInstance().flags[ kFLAGS.EMBER_GENDER ] === 1 || CoC.getInstance().flags[ kFLAGS.EMBER_GENDER ] === 3 ) {
				...
				EngineCore.outputText( 'small beads of moisture gathering on her honeypot' );
			}
			EngineCore.outputText( '.  "<i>Ok... I suppose I could use some help draining myself, but no funny moves!</i>"' );
			...
			if( CoC.getInstance().flags[ kFLAGS.EMBER_GENDER ] === 1 || CoC.getInstance().flags[ kFLAGS.EMBER_GENDER ] === 3 ) {
				...
				EngineCore.outputText( 'her gently-dripping cunt, sliding in between the lips to stroke the wet interior' );
			}
			EngineCore.outputText( '.' );
			...
			EngineCore.outputText( '.  Ember squirms and moans, humming softly, while you suckle and tease, delighting as much in the pleasure you are giving as in the delicious milk cascading down your thirsty throat.' );
	...
	EmberScene.prototype.beatEmberSpar = function() {
		...
		//Medium affection;
			...
			EngineCore.outputText( 'Ember lies on ' + this.emberMF( 'his', 'her' ) + ' back, while you stand over the defeated dragon triumphantly.  You extend a helping hand, intent on helping ' + this.emberMF( 'him', 'her' ) + ' up; ' + this.emberMF( 'he', 'she' ) + ' takes it.' );
		...
		//High affection;
			...
			EngineCore.outputText( 'Ember lies on ' + this.emberMF( 'his', 'her' ) + ' back, while you stand over the defeated dragon triumphantly.  You extend a helping hand, intent on helping ' + this.emberMF( 'him', 'her' ) + ' up; ' + this.emberMF( 'he', 'she' ) + ' gladly accepts your help.' );
	...
	EmberScene.prototype.catchAnal = function() {
		...
		EngineCore.outputText( ' length of ' + this.emberMF( 'his', 'her' ) + ' prick between your ' + Descriptors.buttDescript() + ' cheeks, letting you feel the refreshing coolness of ' + this.emberMF( 'his', 'her' ) + ' pre dripping from the tip.  "<i>This... isn\'t going to work,</i>" ' + this.emberMF( 'he', 'she' ) + ' growls.' );
		...
		EngineCore.outputText( '\n\nOkay then; ' );
		if( EngineCore.silly() ) {
			EngineCore.outputText( 'ASSUMING DIRECT CONTROL.' );
		} else {
			EngineCore.outputText( 'time to take control.' );
		}
		...
		EngineCore.outputText( 'belly, and start to rise and sink, pistoning your ass up and down. You have total control over the sex, and you intend to use that.' );
	...
	EmberScene.prototype.suckEmberCock = function() {
		...
		EngineCore.outputText( ' lower belly' );
		if( this.pregnancy.event > 3 ) {
			EngineCore.outputText( ', though the gravid orb resists you' );
		}
	...
	EmberScene.prototype.stickDickInKnifeDrawer = function() {
		...
		EngineCore.outputText( ' and begins licking you from base to tip in slow, almost torturous, strokes.  "<i>Mmm.  So tasty...</i>"  Sparks of pleasure surge up your body from the dragon\'s ministrations, and you find yourself biting back words because you\'re not sure if you want ' + this.emberMF( 'him', 'her' ) + ' to go faster or to keep at it like this.' );
	...
	EmberScene.prototype.stickItInEmbersButt = function() {
		...
		//(else str >=80);
			...
			EngineCore.outputText( 'grab ' + this.emberMF( 'his', 'her' ) + ' tail, pulling the dragon off ' + this.emberMF( 'his', 'her' ) + ' feet and throwing ' + this.emberMF( 'him', 'her' ) + ' to the ground as you roll over... and causing ' + this.emberMF( 'him', 'her' ) + ' to drop them everywhere as ' + this.emberMF( 'he', 'she' ) + ' falls.' );
			...
			//New;
			EngineCore.outputText( '"<i>What do you think you\'re doing!?</i>" Ember yells at you, obviously mad at your little stunt, one hand reaching behind to prevent any more cum from spilling.' );
	...
	EmberScene.prototype.slurpDraggieCunnies = function() {
		...
		if( this.emberAffection() <= 25 ) {
			...
			if( !EngineCore.silly() ) {
				EngineCore.outputText( 'with me' );
			} else {
				EngineCore.outputText( 'on my affection meter' );
			}
			EngineCore.outputText( ' just because you\'re offering to do this,</i>" the dragon hastily adds.' );
		}
		//(Medium Affection);
		else if( this.emberAffection() < 75 ) {
		...
		if( EngineCore.silly() ) {
			EngineCore.outputText( '  My meter\'s not that full!' );
		}
		EngineCore.outputText( '</i>"' );
	...
	EmberScene.prototype.penetrateEmbrah = function() {
		...
		EngineCore.outputText( 'Your eyes are drawn to the dragon\'s crotch, and you ask if she has any feminine itches she\'d like scratched.' );
		//(Low Affection);
		if( this.emberAffection() <= 25 ) {
		...
		if( CoC.getInstance().player.cockThatFits( this.emberVaginalCapacity() ) === -1 ) {
			...
			EngineCore.outputText( ' and then come back!</i>"  Ember turns on her heels and walks away, moodier than usual.' );
			EngineCore.doNext( CoC.getInstance().scenes.camp.returnToCampUseOneHour );
			return;
		}
		...
		EngineCore.outputText( 'smile and, with a flamboyant grace, ' );
		...
		if( CoC.getInstance().flags[ kFLAGS.EMBER_GENDER ] === 1 || CoC.getInstance().flags[ kFLAGS.EMBER_GENDER ] === 3 ) {
			...
			EngineCore.outputText( ' prick slaps wetly against your belly with each thrust, smearing pre on you as the engorged tool bounces back and forth' );
		}
		EngineCore.outputText( '.' );
		...
		if( CoC.getInstance().player.cumQ() < 250 ) {
			...
			EngineCore.outputText( 'finished, Ember doesn\'t only look more than a few months pregnant; she looks absolutely huge, like her water could break at any moment!' );
		}
	...
	EmberScene.prototype.getPenetratedByEmberLastSexSceneWoooo = function() {
		...
		EngineCore.outputText( ' dick between Ember\'s legs, and you lick your lips hungrily.  Your ' + Descriptors.vaginaDescript() + ' throbs eagerly as you tell Ember you have certain itches you need to scratch... is Ember enough of a ' + this.emberMF( 'man', 'herm' ) + ' to scratch them for you?' );
		//(Low Affection);
		if( this.emberAffection() <= 25 ) {
		...
		//(High Affection);
			...
			EngineCore.outputText( ' at the mere mention of what you expect ' + this.emberMF( 'him', 'her' ) + ' to do.  "<i>I\'m more than enough to handle two... no... three of you!  And if you want proof all you have to do is ask!</i>" Ember boasts proudly.' );
		}
		...
		if( CoC.getInstance().player.hasCock() ) {
			...
			EngineCore.outputText( ' your draconic partner.' );
		}
	...
	EmberScene.prototype.getPenetratedByEmberLastSexSceneWooooPartII = function() {
		...
		EngineCore.outputText( '.  "<i>You used me like a fucktoy!</i>"' );
		...
		if( this.emberAffection() < 75 ) {
			EngineCore.outputText( this.emberMF( 'his', 'her' ) + ' cock was unlike anything else in this world?' );
		} else {
			EngineCore.outputText( this.emberMF( 'he', 'she' ) + ' could handle two or even three of you at the same time?' );
		}
		EngineCore.outputText( '  You expected more resistance if that was the case.' );
	...
	EmberScene.prototype.getKnockedUpByEmbrahBroBaby = function() {
		...
		EngineCore.outputText( 'Your contracting walls milk the dragon\'s sensitive cock, trying to coax even more seed out of your panting dragon lover.  "<i>I-it\'s time,</i>" Ember mutters, crashing into your wet bedroll and thrusting deep inside you.' );
	...
	EmberScene.prototype.breedEmberPregnantAsIfThereWasAnyOtherKindOfBreeding = function() {
		...
		EngineCore.outputText( 'face, nipping lightly at her lip, rolling your tongue over hers and savoring her inhuman tastes; Ember answers in kind with a kiss of her own, purring and moaning into your mouth.  Her vibrating breasts massage your hands as you knead, lift, and then roll them about.' );
		//(if Ember has a dick);
		if( this.emberHasCock() ) {
		...
		if( this.emberHasCock() ) {
			EngineCore.outputText( 'her cock spurts a few jets of cum onto your [chest]; ' );
		}
		EngineCore.outputText( 'her eyes roll to the back of her head.  With a muffled roar she finally caves, a veritable wave of dragon juices crashing into your lower body and pooling on your bedroll, filling the whole tent with the delicious smell of sex and pheromones.' );
		//(If Ember has a dick);
		if( this.emberHasCock() ) {
			EngineCore.outputText( '  Ember\'s cock wets the side of the tent with continuous jets of cum, only adding to the already overpowering smell of sex as she empties herself on top of you.' );
		}
		...
		if( this.emberHasCock() ) {
			EngineCore.outputText( 'her spent cock throb; ' );
		}
		EngineCore.outputText( 'her eyes shut tight in pleasure as she reaches her second orgasm.  You muffle her groan of pleasure with one of your own.  A second wave of dragon fluids joins the first in wetting your lower body, along with your bedroll.' );
		if( this.emberHasCock() ) {
			EngineCore.outputText( '  Little gobs of cum spill from her cock to slide down her shaft and gather on your belly button.' );
		}
		EngineCore.outputText( '  Her tail coils around you, helping her stay in position as she melts on top of you like putty.' );
		...
		if( CoC.getInstance().player.cumQ() < 250 ) {
			...
			EngineCore.outputText( '  You dump a huge load of jism into Ember\'s awaiting belly, but her contracting walls seem to milk you for ever more!  Ember is filled to the brim, her belly already slightly distended, but you still continue to fill her with your seed.  Ember\'s tightly sealed pussy lips means the cum has nowhere to go, so instead it gathers in her womb, distending her belly until she looks at least a few months pregnant; and by the look of pleasure on Ember\'s panting face, you don\'t think she minds it.' );
		}

		EngineCore.outputText( '\n\nWith a gasp and a sigh of utter relief, you slump into Ember\'s arms, collapsing onto her ' );
		if( this.emberHasCock() ) {
			EngineCore.outputText( 'cum-slick ' );
		}
		EngineCore.outputText( 'belly and laying there to regain your strength, ' );
		if( this.emberHasCock() ) {
			EngineCore.outputText( 'dismissive of the spent cock lying sandwiched between you as you start ' );
		}
		EngineCore.outputText( 'feeling the heat in her gut that marks the beginning of a child.' );
	...
	EmberScene.prototype.emberBreedingAfterMathWatchOutForRadioactiveFallout = function( emberPregged ) {
		...
		EngineCore.outputText( '  "<i>You\'re welcome, but this is the only time I\'ll clean up after you!  I\'m not your personal maid!</i>"' );
		...
		if( !emberPregged ) {
			EngineCore.outputText( 'touch your belly' );
		} else {
			EngineCore.outputText( 'point at Ember\'s belly' );
		}
		EngineCore.outputText( ', and you ask the dragon if ' + this.emberMF( 'he', 'she' ) + ' thinks it "<i>took</i>".' );
		...
		if( emberPregged ) {
			EngineCore.outputText( 'pumped into me...' );
		} else {
			EngineCore.outputText( 'had me pump into you...' );
		}
		EngineCore.outputText( '</i>" Ember trails off, and you can see ' + this.emberMF( 'his', 'her' ) + ' hand move to ' + this.emberMF( 'his', 'her' ) + ' crotch to caress ' + this.emberMF( 'his', 'her' ) + ' ' );
		if( emberPregged ) {
		...
		EngineCore.outputText( ' deepens, and ' + this.emberMF( 'he', 'she' ) + ' frowns at you.  "<i>A-are you questioning my virility!?  Well, if you want, I can go again, right now!</i>"' );
	...
	EmberScene.prototype.emberGivesBirth = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.EMBER_OVIPOSITION ] === 0 ) {
			...
			EngineCore.outputText( '.  It\'s a beautiful strong, healthy little ' );
			...
			EngineCore.outputText( ' in your arms, uncaring of the stains as the fluid drenching its little body soaks into your [armor], gently soothing ' );
			if( roll < 40 ) {
				EngineCore.outputText( 'him' );
			} else {
				EngineCore.outputText( 'her' );
			}
			EngineCore.outputText( '.  Then, seeing Ember is recovering from her recent exertions, you grin proudly at her and gently hold your ' );
			if( roll < 40 ) {
				EngineCore.outputText( 'son' );
			} else {
				EngineCore.outputText( 'daughter' );
			}
			EngineCore.outputText( ' out for her to take.' );
			...
			if( this.emberChildren() > 1 ) {
				EngineCore.outputText( '  Your other draconic offspring quietly peek into the lair, having vacated the place when Ember went into labor.  They smile at the sight of their new sibling, gently slipping in to curl up beside the mother and new child.' );
			}
			EngineCore.outputText( '  With a contented expression on your face, you leave Ember and your ' );
			if( this.emberChildren() <= 1 ) {
				EngineCore.outputText( 'new child' );
			} else {
				EngineCore.outputText( 'newly expanded draconic brood' );
			}
	...
	EmberScene.prototype.giveBirthToEmberKids = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.EMBER_OVIPOSITION ] === 0 ) {
			...
			if( CoC.getInstance().flags[ kFLAGS.EMBER_MILK ] > 0 ) {
				...
				EngineCore.outputText( ' myself, but ' );
				if( roll < 40 ) {
					EngineCore.outputText( 'he' );
				} else {
					EngineCore.outputText( 'she' );
				}
				EngineCore.outputText( ' wanted yours...</i>"' );
			}
			...
			EngineCore.outputText( ' face and starts to cry softly.  You gently help ' );
			if( roll < 40 ) {
				EngineCore.outputText( 'him' );
			} else {
				EngineCore.outputText( 'her' );
			}
			EngineCore.outputText( ' up onto your shoulder and gently pat ' );
			if( roll < 40 ) {
				EngineCore.outputText( 'him' );
			} else {
				EngineCore.outputText( 'her' );
			}
			EngineCore.outputText( ' on the back between ' );
			if( roll < 40 ) {
				EngineCore.outputText( 'his' );
			} else {
				EngineCore.outputText( 'her' );
			}
			EngineCore.outputText( ' little wings, eliciting a burp that clearly leaves ' );
			if( roll < 40 ) {
				EngineCore.outputText( 'him' );
			} else {
				EngineCore.outputText( 'her' );
			}
			EngineCore.outputText( ' feeling better.  ' );
			if( roll < 40 ) {
				EngineCore.outputText( 'He' );
			} else {
				EngineCore.outputText( 'She' );
	...
	EmberScene.prototype.highAffectionEmberLustFuck = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.EMBER_GENDER ] === 1 ) {
			...
			EngineCore.outputText( ' moans throatily and gives you a rapturous look.  "<i>I love you, [name]... gods, I love you so much, and I\'m so happy I can finally say that to your face,</i>" she tells you.' );
		...
		//else if Ember has a pussy and the PC more than one cock:;
			...
			EngineCore.outputText( ' of those?  I-I know it\'s not exactly part of the pose and all, but...</i>"' );
			...
			EngineCore.outputText( ' moans throatily and gives you a rapturous look.  "<i>I love you, [name]... gods, I love you so much, and I\'m so happy I can finally say that to your face,</i>" she tells you.  Then she starts looking less in love and more in lust.  "<i>But I also just love these cocks; these two wonderful breeding rods - come on, mate!  Breed your horny dragon!</i>" she commands lustfully.' );
		}
		...
		if( CoC.getInstance().flags[ kFLAGS.EMBER_GENDER ] >= 2 ) {
			...
			EngineCore.outputText( ' tight ass' );
		}
		...
		//[Yes] [No];
		EngineCore.menu();
		EngineCore.addButton( 0, 'Yes', this.stayWithEmberAfterLustFuck );
		EngineCore.addButton( 1, 'No', this.noStayingForCuddlesPostLustFuck );
	};
	//[=No=];
	...
	EmberScene.prototype.noStayingForCuddlesPostLustFuck = function() {
		...
		EngineCore.outputText( ' and then gather your things before heading off to wash yourself down.' );
		EngineCore.doNext( CoC.getInstance().scenes.camp.returnToCampUseOneHour );
	};
	//[=Yes=];
	EmberScene.prototype.stayWithEmberAfterLustFuck = function() {
		...
		//else if Ember is female:;
			...
			//[Frotting] [Penetrate];
			EngineCore.menu();
			EngineCore.addButton( 0, 'Frotting', this.frottingWithFrottingEmberHerm );
			EngineCore.addButton( 1, 'Penetrate', this.penetrateEmberHerm );
		}
	};
	//[=Frotting=];
	...
	CoC.getInstance().registerScene( 'emberScene', new EmberScene() );
} );