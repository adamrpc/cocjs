...
angular.module( 'cocjs' ).run( function( MainView, SceneLib, Holli, PerkLib, $log, CockTypesEnum, PregnancyStore, Combat, Descriptors, AppearanceDefs, CoC, kFLAGS, Utils, StatusAffects, EngineCore ) {
	...
	//VARS;
	/*Plant Sprout Notes sapling plants near corner of camp after 2nd Marae meeting.
	 ...
	 Phase 4 Notes:
	 -The boobs fade into the tree and the tentacles retract, then the trunk 'blooms', exposing a humanoid woman with green skin.
	 -Dark Green Hair
	...
	HolliScene.prototype.treeMenu = function( output ) {
		...
		var fuck = null;
		var burnIt = null;
		if( CoC.flags[ kFLAGS.FOUGHT_HOLLI ] > 0 ) {
			...
			if( output ) {
				MainView.outputText( 'Before you is a large, waist-high plant of questionable origin.  A huge, violet bloom sits atop a strong, central stalk, and the obscene flower looks positively female in every sense, right down to a clit-shaped bump and moist, gently parted lips.  Inside that central passage, you can see a bevy of wiggling staman surrounding a larger, flared stalk.  The scent that wafts out of that opening is sweet and floral, but undoubtedly sexual.  It looks like an organic sex toy and is clearly corrupt.  It wouldn\'t be hard to get rid of right now, assuming you don\'t let it grow larger.' );
			}
			if( CoC.player.hasCock() && CoC.player.cockThatFits( 100 ) >= 0 && CoC.player.lust >= 33 ) {
			...
			if( output ) {
				...
				if( CoC.flags[ kFLAGS.HOLLI_SUBMISSIVE ] === 1 ) {
					MainView.outputText( '\n\n"<i>Welcome back, [name].  ' );
					if( CoC.flags[ kFLAGS.TIMES_FUCKED_FLOWER ] > 0 && Utils.rand( 3 ) === 0 ) {
						MainView.outputText( 'Did you want to fertilize me again?  My flower is ready and waiting...' );
					} else if( CoC.flags[ kFLAGS.TIMES_RIDDEN_FLOWER ] > 0 && Utils.rand( 3 ) === 0 ) {
						MainView.outputText( 'Did you want to use my tentacles again?  They\'ve been restless without you...' );
					} else if( Utils.rand( 2 ) === 0 ) {
						MainView.outputText( 'Did you come for a drink?  I\'ve built up lots of nectar for you.  Surely, a sip would be refreshing for you...' );
					} else {
						MainView.outputText( 'How may I serve you?  Come, give me a command so that I can fulfill it for you.' );
					}
					MainView.outputText( '</i>"' );
				} else {
					MainView.outputText( '\n\n"<i>Welcome back, [name].  ' );
					if( CoC.flags[ kFLAGS.TIMES_FUCKED_FLOWER ] > 0 && Utils.rand( 3 ) === 0 ) {
						MainView.outputText( 'Did you want to fertilize me again?  Gardening is such exhausting work, but so rewarding...' );
					} else if( CoC.flags[ kFLAGS.TIMES_RIDDEN_FLOWER ] > 0 && Utils.rand( 2 ) === 0 ) {
						MainView.outputText( 'Did you come back to play again?  My tentacles are so restless.' );
					} else if( Utils.rand( 2 ) === 0 ) {
						MainView.outputText( 'Did you come for a drink?  I\'ve built up lots of nectar for you.  Surely a hungry adventurer like yourself needs a sip, right?' );
					} else {
						MainView.outputText( 'Do you want to play?  Come on, taste Mother Marae\'s finest creation.' );
					}
					MainView.outputText( '</i>"' );
				}
				...
				if( CoC.flags[ kFLAGS.HOLLI_FRUIT ] > 0 ) {
					if( CoC.flags[ kFLAGS.HOLLI_FRUIT ] === 1 ) {
						...
						MainView.outputText( '\n\n' + Utils.Num2Text( CoC.flags[ kFLAGS.HOLLI_FRUIT ] ) + ' large, purple fruits hang from Holli\'s branches, twisting softly in the breeze.  They look succulent and ripe.' );
					}
					//Optional Addition: ;
					if( CoC.flags[ kFLAGS.HOLLI_FRUIT_EXPLAINED ] > 0 ) {
						MainView.outputText( '  A very small, strange part of you actually feels proud to have made something like that with the tree-woman.' );
					}
				}
			}
	...
	HolliScene.prototype.helpWithFarm = function() {
		MainView.clearOutput();
		MainView.outputText( 'You ask if a plant goddess might not be able to help the farm you recently acquired.' );
		...
		if( CoC.flags[ kFLAGS.HOLLI_FUCKED_TODAY ] === 0 ) {
	...
	HolliScene.prototype.noPlzDontFuckWithFarm = function() {
		MainView.clearOutput();
		MainView.outputText( '“<i>Such a fickle mortal,</i>” she sighs. “<i>But I am a kind goddess. Ask me for it anytime, assuming you’ve... performed the rites.</i>”' );
		this.treeMenu( false );
	};
	HolliScene.prototype.fertilizeHolli = function( cock ) {
		//20% chance per sexing.  Up to bonus 20% for jizz or fertility! Max 62%.;
		if( cock === undefined ) {
			cock = true;
		}
		var odds = 20;
		if( cock && CoC.player.hasCock() ) {
			odds += CoC.player.cumQ() / 300;
			if( odds > 40 ) {
				odds = 40;
			}
		...
		if( !cock && CoC.player.hasVagina() ) {
			odds += CoC.player.totalFertility() / 5;
			if( odds > 40 ) {
				odds = 40;
			}
		...
		if( odds >= Utils.rand( 101 ) ) {
			CoC.flags[ kFLAGS.HOLLI_FRUIT ] += 1 + Utils.rand( Math.floor( odds / 10 ) );
			$log.info( 'Holli pregged!' );
		}
	};
	...
	HolliScene.prototype.getASprout = function() {
		...
		if( CoC.flags[ kFLAGS.CORRUPT_MARAE_FOLLOWUP_ENCOUNTER_STATE ] === 2 ) {
			MainView.outputText( '  She even promised to plant a tree in your camp...' );
		}
		MainView.outputText( '  You look closer and see purple veins on the undersides of the leaves, throbbing with what can only be the tainted liquids that suffuse much of this planet.  Fully grown, it might spread its corruption to your camp.\n\n<b>Do you destroy it?</b>' );
	...
	HolliScene.prototype.letZeFuckingSproutLive = function() {
		MainView.clearOutput();
		MainView.outputText( 'Looking down at the sapling, you stay your wrath.  It may be corrupt, but it hasn\'t done anything to harm you just yet.  You give it a little pat on the uppermost leaves and leave it be.  It\'s not like it\'s going anywhere.' );
	...
	HolliScene.prototype.fuckPlantGrowsToLevel2 = function() {
		...
		if( CoC.player.gender === 0 ) {
			MainView.outputText( '  Sucks to be it!  You don\'t waste time with stupid shit, like genitals.' );
		}
		MainView.outputText( '  Is this Marae\'s idea of a gift?' );
		if( CoC.player.gender > 0 ) {
			MainView.outputText( '  Well, you could give it a whirl... just being around it seems to slowly turn you on.' );
		}
		MainView.outputText( '  Of course, destroying it would be the safest option.' );
	...
	HolliScene.prototype.fuckFuckingFuckFlowerP2 = function() {
		...
		MainView.outputText( 'Undoing your [armor], you approach the pulsating plant with one thought in mind it.  At the sight of [eachCock], the petals flutter happily, their surface glimmering with moisture in an instant, slick and wet for your pleasure.  You lean down to inhale the sweet yet sensual nature of the flower\'s aroma, letting it tickle your nostrils as you idly fondle [oneCock].  Your maleness quickly assumes your favorite state - hard and sensitive, ready to plunge into the nearest set of wet lips with reckless abandon.  At the same time, you can see the wriggling tendrils inside the flower\'s vase waving about hungrily.' );
		MainView.outputText( '\n\nYou laugh in amusement; who are you to deny it the creamy treat it so richly deserves?  Letting go of your ' + Descriptors.cockDescript( x ) + ', you reach around the soft petals to the harder, central bud and slide yourself through the honeyed embraced into the pulsating, tentacled flower-core.  Juicy warmth snuggles about your dick, the velvet-soft chamber immediately tightening up while the petals fold in, crossing over one another until they form a tube long enough to swallow you whole.  Gentle suction begins to rhythmically tug at your ' + CoC.player.cockHead( x ) + ', swelling you larger and more sensitive inside.  Immediately, the tiny tendrils you saw swarming around earlier reach up to massage your cock, slithering across your skin like tiny, sinuous snakes.' );
		...
		MainView.outputText( ' and form a sticky puddle.  Now moaning, you grab the stem and start to pump the sheath up and down, using the plant like the cock-toy it was meant to be.  Lewd, wet squishes ring out through the air,' );
		if( CoC.player.cor < 33 ) {
			MainView.outputText( ' and you timidly look around, hoping you haven\'t attracted an audience.' );
		} else if( CoC.player.cor < 66 ) {
			MainView.outputText( ' and you worriedly glance around, hoping yet terrified that someone will walk in on it.' );
		} else {
			MainView.outputText( ' and you glance around, hoping someone will walk in on the sexy show you\'re making.' );
		}
		MainView.outputText( '\n\nNow panting with unbearable lust and undeniable levels of pleasure, you give up all sense of propriety and resort to fucking wildly, [hips] jumping inches into the air.  The flower\'s interior is a slick, silky heaven for your ' + Descriptors.cockDescript( x ) + ', a sucking, squeezing hole of seemingly infinite pleasure.  Inside its gluttonous gullet, the numerous tentacles twist around, the longest ones circling the base of your shaft' );
		if( CoC.player.hasSheath() ) {
			MainView.outputText( ' inside your violated sheath' );
		}
		MainView.outputText( '.  With so much blood trapped in your ' + Descriptors.cockDescript( x ) + ', it\'s bigger and harder than ever, twitching valiantly against its restraints with every beat of your heart.' );
		MainView.outputText( '\n\nOne of the tentacles circles your ' + CoC.player.cockHead( x ) + ' for a moment, and without much warning or pause, it lances forward to bury itself into your vulnerable cum-slit.  Your preconceptions are turned on their heads when there\'s a complete lack of pain from the abrupt penetration.  If anything, it actually feels kind of good... like there\'s a warm, slippery finger caressing your penis from the inside out.  It slithers in and out of you, pumping you inside, burrowing pleasure into your center even while stroking you from without.  Your body, burning from the exertion of fucking the flower\'s pod and the constantly-rising tidal wave of lust, begins to twitch spasmodically.' );
		MainView.outputText( '\n\nThe internal suction spikes as your dick bloats larger, and with a roar of bestial pleasure, you arch your back, hands pushing your ' + Descriptors.cockDescript( x ) + ' as deep into the plant\'s gullet as possible.  ' );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( 'Your [balls] gurgle, a surprisingly happy sound, and cling tight to your loins.  ' );
		}
		MainView.outputText( 'Inside you, you can feel the liquid heat of your spurting release exploding out, melting everything before it with liquid, orgasmic bliss.  A low moan bursts from your lips when the sounding tentacle pulls out of your sloppy cock-slot, and the rushing tide of bubbling cum follows it, bursting into the sweet, suckling nectar-pot.' );
		if( CoC.player.cumQ() > 500 ) {
			MainView.outputText( '  You pour forth your prodigious essence without pause or fail, and the plant immediately begins to drink it down, the stem bulging with rounded balls of sperm that it carries underground.' );
		}
		if( CoC.player.cumQ() > 1500 ) {
			MainView.outputText( '  Soon, the meager stem cannot swallow it all.  Spunk froths and squirts out around the base of your tool for a moment, and then a veritable tide of the stuff gushes out to soak  you.' );
		}
		...
		CoC.flags[ kFLAGS.TIMES_FUCKED_FLOWER ] = 1;
	...
	HolliScene.prototype.rideDatFuckingFukkFlowerP2 = function() {
		...
		if( CoC.player.clitLength > 4 ) {
			MainView.outputText( '  Your heavy buzzer is so big that the flexible plant-cock has plenty of room to curl around it, and if you could see it, your clit would look almost like a candy-cane (if one set of stripes was jerking off the cane beneath).' );
		}
		if( CoC.player.cockTotal() > 0 ) {
			MainView.outputText( '  [EachCock] soon has its own partner twisting around it, a pussy-slicked vine to jerk and twine about its girth.' );
		}
		MainView.outputText( '\n\nWith nothing to occupy your roving hands, you let them slide up your ' + CoC.player.skinFurScales() + ' to your ' );
		if( CoC.player.bRows() > 1 ) {
			MainView.outputText( 'lowest row of tits, cupping them before groping to your top row of breasts' );
		} else {
			MainView.outputText( '[chest]' );
		}
		MainView.outputText( ', caressing the' );
		if( CoC.player.biggestTitSize() > 1 ) {
			MainView.outputText( 'ir' );
		}
		MainView.outputText( ' swell as you search for the lust-tightened skin of your [nipples].  They seem almost as sensitive as little clits, and as soon as you find them, you immediately set to circling them slowly, occasionally tugging and twisting when a particularly powerful sensation works through your pleasure-wracked body.' );
		if( CoC.player.lactationQ() > 50 ) {
			MainView.outputText( '  One after the other, they begin to ' );
			if( CoC.player.lactationQ() < 150 ) {
				MainView.outputText( 'trickle' );
			} else if( CoC.player.lactationQ() < 500 ) {
				MainView.outputText( 'leak' );
			} else {
				MainView.outputText( 'spray torrents of' );
			}
			MainView.outputText( ' milk.' );
		}
		MainView.outputText( '  You moan in unabashed delight, your mouth hanging open in a mindless, blissful \'o\' in between vocalizations.  If your friends back home could see you now, writhing in the dirt, face open in simple, animal pleasure, and perhaps worst of all, fucking a plant, you\'d die of ' );
		if( CoC.flags[ kFLAGS.PC_FETISH ] === 0 && CoC.player.cor + CoC.player.lib < 150 ) {
			MainView.outputText( 'embarrassment' );
		} else {
		...
		if( CoC.player.hasCock() ) {
			MainView.outputText( '  [EachCock] explodes in release, pumping heavy flows of seed into the petals, even as y' );
		} else {
			MainView.outputText( 'Y' );
		}
		MainView.outputText( 'our quivering pussy, already sopping wet, releases the last of your juices in a soaked ' );
		if( CoC.player.cumQ() < 50 ) {
			MainView.outputText( 'drizzle' );
		...
		MainView.outputText( '.  The tentacles inside you curl in on themselves and slowly pull back from the well-creamed quim, scraping out as much fluid as possible from your oversensitive honey-pot.  The binding on your [clit] releases, which of course sets off a whole new wave of rippling pleasure-contractions in your cunt.' );
		...
		EngineCore.dynStats( 'cor', 1 );
		EngineCore.fatigue( 5 );
		CoC.flags[ kFLAGS.TIMES_RIDDEN_FLOWER ] = 1;
		if( CoC.flags[ kFLAGS.FUCK_FLOWER_GROWTH_COUNTER ] < 1000 ) {
			CoC.flags[ kFLAGS.FUCK_FLOWER_GROWTH_COUNTER ] += 4;
		}
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	...
	HolliScene.prototype.flowerGrowsToP3 = function() {
		...
		CoC.flags[ kFLAGS.FUCK_FLOWER_LEVEL ] = 3;
		CoC.flags[ kFLAGS.FUCK_FLOWER_GROWTH_COUNTER ] = 0;
		MainView.outputText( '.\n\n<b>What do you do?</b>' );
		var fuck = null;
		var ride = null;
	...
	HolliScene.prototype.fuckTheFlower = function() {
		...
		if( CoC.player.hasVagina() ) {
			MainView.outputText( '  Wetness builds in your [vagina] as well, but you\'re focusing on your maleness just now.  Your neglected twat will have to wait its turn.' );
		}
		MainView.outputText( '  A feverish sweat breaks out on your brow, though whether from your own arousal, the plant\'s heat, or some chemical in its juices, you cannot tell.' );
		MainView.outputText( '\n\nBefore you can pull back, the hiss of something cutting through the air with lightning speed hits your eardrums.  Almost at the same time, tentacles (stamen?) snap around your neck, head, and ears, going taut in an instant.  Your ' + CoC.player.face() + ' is pulled roughly into the glossy hole, the soft folds of the petals smearing your nose and mouth with their saccharine sweetness.  You cannot help but breathe in more deeply as you struggle for your freedom, and with every struggle, every struggling gasp, [eachCock] grows hotter and harder, pulsating with tactile hunger.  With each moment that passes the motivation for your struggles shifts from freedom to fucking.  You don\'t care about getting away so much as getting your dick in this aromatic fuck-pot.' );
		...
		MainView.outputText( '\n\nRacking your brain for ideas, you momentarily slump into a temporary acceptance of your position, sagging heavily into the strong, fibrous tentacles.  Perhaps that\'s what the unnatural creature wants, for as soon as you let it have control, the tendrils swing you closer, close enough for [oneCock] to kiss the warm, wet hole.  Momentum carries you away from the promise of greater pleasure, your ' + Descriptors.cockDescript( x ) + '\'s tip nicely moistened by lubricants and its own heavy bead of liquid pleasure.  When you swing back, a few inches are lucky enough to nestle inside, and you find yourself moaning without meaning to.' );
		MainView.outputText( '\n\nYou\'re swung to and fro, held aloft by a shrubbery harness seemingly intent on teasing you.  First, you get a taste of heaven, but only on your ' + CoC.player.cockHead( x ) + '.  Then, you\'re left to flutter in the breeze, the only sexual sensation available the cool wind fluttering across your whetted tip.  Over and over, the plant teases you until you lose track of time.  The closest thing to a watch you have is the gentle throb in your [balls] and its gradually strengthening intensity.  Oh, how it aches!  You feel taut and stretched like a harp string being tuned, only the musician just keeps cranking away, ratcheting up the pressure until you feel about to snap.  Your one comfort is the momentary bliss of the flower\'s sweet kiss, an exciting pleasure that\'s slowly being turned into an aching hell.' );
		MainView.outputText( '\n\nDuring a particularly shameless moan, you realize that you\'re dipping a little deeper with each pendulous stream.  Luckily, the corrupt tree\'s arboreal lust must be rising like your own, and after a few slow swings, your ' + Descriptors.cockDescript( x ) + ' is slipping three quarters of the way in' );
		if( CoC.player.cockArea( x ) >= 150 ) {
			MainView.outputText( ', sliding down the trunk as easily if the entire inside was a giant pussy just waiting to get fucked' );
		}
		MainView.outputText( '.' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( '  Your other penis' );
			if( CoC.player.cockTotal() > 2 ) {
				MainView.outputText( 'es rub' );
			} else {
				MainView.outputText( ' rubs' );
			}
			MainView.outputText( ' sensually across the moist outer blossom, drooling their clear pre-cum over the petals.' );
		}
		MainView.outputText( '  Schliiick.  You slide out.  Slorp-pop.  You slide in, this time to the ' );
		if( CoC.player.hasSheath() ) {
		...
		if( CoC.player.balls > 0 ) {
			MainView.outputText( ' with your gurgling balls slapping against the flower\'s edge' );
		}
		MainView.outputText( ', accompanied by a small squirt of sweet lubricants.' );
		MainView.outputText( '\n\nYou\'ve been kept on edge for so long that even this brief taste of liquid-soft snugness has your body clenching and warm bliss mounting inside of you.  You wrap as much of your [legs] around the trunk below as possible and give into your pleasure, thrusting your ' + Descriptors.cockDescript( x ) + ' against the petalled muff each time the vines draw you away.  You rut like a demon\'s pet, half-mad with pleasure and desire so powerful it obviates rational thought.  The ecstasy rises like an all consuming tide, tinting your vision pink as your eyes roll back and [eachCock] thickens.' );
		MainView.outputText( '\n\nJizz blasts out of your ' + Descriptors.multiCockDescriptLight() + ' with uncommon force, painting the flower\'s interior with white' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( ', as your multiple endowments allow you to drizzle the purple bloom\'s exterior with alabaster cream' );
		}
		MainView.outputText( '.' );
		if( CoC.player.cumQ() > 500 ) {
			MainView.outputText( '  Pearly ropes big enough to fill a flask lurch out from your cum-slit, propelled by your quivering orgasmic muscles deep into the trunk, and you swear you can hear the pitch of your sexual pumping rising in tone, changed by the flooded vessel.' );
		}
		if( CoC.player.cumQ() > 1500 ) {
			MainView.outputText( '  A wave of your thick gunk rolls out from under your pumping cock - you\'ve filled the tree beyond even its capacity, and the thick puddle forming beneath your [feet] is all the proof you need of your virility.' );
		}
		if( CoC.player.hasVagina() ) {
			MainView.outputText( '  Your hungry cunt, long denied in this exchange, trembles as it ' );
			if( CoC.player.wetness() < 4 ) {
				MainView.outputText( 'leaks' );
			} else {
				MainView.outputText( 'sprays' );
			}
			MainView.outputText( ' your own female fluids in sympathy.' );
		}
		MainView.outputText( '  After what feels like forever, the spurting seed no longer flows from your long-abused dick, and as your member softens, the tentacles swing you away, setting you down softly.' );
		...
		EngineCore.fatigue( 5 );
		if( CoC.flags[ kFLAGS.FUCK_FLOWER_GROWTH_COUNTER ] < 1000 ) {
			CoC.flags[ kFLAGS.FUCK_FLOWER_GROWTH_COUNTER ] += 5;
		}
	...
	HolliScene.prototype.drinkThePlantGirlsSap = function() {
		...
		if( EngineCore.silly() ) {
			MainView.outputText( ', tree hugger that you are' );
		}
		MainView.outputText( '.  The thick \'milk\' quickly fills your body with energy, though it runs out nearly as soon as it started.' );
	...
	HolliScene.prototype.rideTheWalrusP3 = function() {
		...
		MainView.outputText( 'You eye the tentacles hiding amongst the leafy branches above, writhing obscenely around each other as they snake through the foilage.  You shed your [armor] into a worthless pile and bare your body, cupping your [chest] enticingly as you whistle up at the tree.  Your calls seem to fall on deaf ears; the whirling vines stay put up in their perch.  They\'re ignoring your delicious, feminine flesh!  Irritated, you stomp up to the trunk\'s flower and plunge your fist inside it.  Fresh fluid gushes out over your armor, releasing a spray of sweet, musky lubricants.  You blindly feel around inside the pliant plant-pussy with your fingers, giggling to yourself whenever the soft cavity clenches tightly to your arm.  Whatever this is, it can feel pleasure.' );
		...
		MainView.outputText( '\n\nAt some point, you are set back down on the ground and sprayed with a fresh batch of seed.  You rest there for a while, until you can catch your breath and move without pain.  When you start trying to clean up, you realize the puddle of cum is completely gone, likely swallowed back up the tree\'s hungry roots, and the only mess left is your sodden ' + CoC.player.skinDesc + '.  You get dressed and stumble away a little drunkenly, your [vagina] raw and oozing.' );
	...
	HolliScene.prototype.treePhaseFourGo = function() {
		...
		if( CoC.flags[ kFLAGS.TIMES_FUCKED_FLOWER ] + CoC.flags[ kFLAGS.TIMES_RIDDEN_FLOWER ] > 0 ) {
			MainView.outputText( 'Did you enjoy feeding me?  You should have seen the expression on your face when you were cumming in me.  Your eyes were rolling back and your mouth was just hanging open.  Do you know what it made me want to do?' );
		} else {
			MainView.outputText( 'Did you enjoy watching me grow?  You should have fed me.  It was so much harder to grow without proper nutrients.  Do you know what I thought about during all the time I spent waiting?  Do you know what it made me want to do?' );
		}
		MainView.outputText( '</i>"' );
	...
	HolliScene.prototype.JojoTransformAndRollOut = function() {
		...
		MainView.outputText( '"<i>[name], come here please.</i>"' );
		...
		if( CoC.player.cor < 25 ) {
			MainView.outputText( 'Not willing to lie to your friend, you also admit that she seems to have changed after your actions in the factory - that, though you were trying to render the tanks permanently inoperable, the spillage you created pushed the cthonic spirit over the edge.' );
		} else {
			MainView.outputText( 'Though, you do leave out the precise, needless details of what caused the change.' );
		}
		MainView.outputText( '  The monk\'s eyes widen in surprise and surmise.' );
		MainView.outputText( '\n\n"<i>Unbelievable...</i>" he blurts.  "<i>' );
		if( CoC.flags[ kFLAGS.TIMES_FUCKED_FLOWER ] + CoC.flags[ kFLAGS.TIMES_RIDDEN_FLOWER ] === 0 ) {
			MainView.outputText( 'To think that I must now consider Marae herself my enemy.  Forgive me, but this place is not safe for us anymore if it has attracted the attention of such a creature.  For now, I will return to the forest.  I know that you believe your mission keeps you here, but I urge you to stay away from that thing and watch the portal from an adequate distance.' );
		}
		//(totally deflowered that flower);
		else {
			MainView.outputText( 'And even knowing that this could be the case, you still chose to fornicate with it?  [name]... I cannot say that I am not disappointed.  This place is not safe for me anymore.  For now, I will return to the forest.' );
		}
		MainView.outputText( '</i>"' );
		MainView.outputText( 'Jojo looks at you sadly; any argument you try to make is met with a resigned shake of his head.  "<i>If the creature truly is the demonic offspring of a corrupted earth spirit, doubtless it will be powerful and the wrath of its parent severe.' );
		if( CoC.player.level < 20 ) {
			MainView.outputText( '  Nonetheless, if you find yourself in a position where you must confront this demon or perish, call upon me.  I will render what aid I can, though I prefer it not be in vain.' );
		} else {
			MainView.outputText( '  Though, a mortal of your stature and resources may be able to destroy her and escape the reprisal.  I would counsel against direct confrontation until you are sure of victory, but if you must fight, then call upon me and you will have what aid I can give.' );
		}
		MainView.outputText( '</i>"  So saying, he presses a small talisman into your hand.' );
		MainView.outputText( '"<i>Until then, you will find me in the forest if you need my assistance.</i>"  Jojo makes a polite bow and departs.' );
		MainView.outputText( '(<b>Jojo is no longer a companion!</b>)' );
	...
	HolliScene.prototype.amilyHatesTreeFucking = function() {
		MainView.clearOutput();
		MainView.outputText( 'Your mousey cohabitant tramps up to you, irritation plain on her expression.' );
	...
	HolliScene.prototype.stayQuietWhileAmilyBitchesAboutFuckingArborday = function() {
		MainView.clearOutput();
		MainView.outputText( 'Amily berates you for some while longer, then eyeballs you.  Just as you begin to speak, she cuts you off again.' );
	...
	HolliScene.prototype.slapAmilysWhoreFace = function() {
		MainView.clearOutput();
		MainView.outputText( 'You slap Amily across the ' );
		...
		if( CoC.flags[ kFLAGS.AMILY_NOT_FURRY ] === 0 ) {
			MainView.outputText( 'mouse-' );
		}
		MainView.outputText( 'girl says, eyes wet with tears, "<i>but I don\'t.  Goodbye.</i>"' );
	...
	HolliScene.prototype.fuckHolliInZeFlowerPuss = function() {
		...
		MainView.outputText( ' ' + CoC.player.mf( 'boy', 'girl' ) + ',</i>" the arboreal tramp suggests with a husky coo, "<i>I got you some sugar.</i>"  She pinches her nipples and gasps, dark streams of sugary juices dripping wetly into the soil as she beckons you closer.' );
		...
		if( CoC.player.cockTotal() > 1 ) {
			if( CoC.player.cockTotal() === 2 ) {
				MainView.outputText( '  Your other penis splays' );
			} else {
				MainView.outputText( 'The rest of your penises splay' );
			}
			MainView.outputText( ' to the side, smearing furrows of the stuff around as they slide over the butter-smooth skin.' );
		}
		MainView.outputText( ' The demon dryad\'s hand reaches below to ' );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( 'cradle your [balls], gently massaging your sack' );
		} else {
			MainView.outputText( 'gently caress your ' );
			if( CoC.player.hasSheath() ) {
				MainView.outputText( 'sheath' );
			} else {
				MainView.outputText( 'loins' );
			}
		}
		MainView.outputText( ' encouragingly.  You respond by edging your [hips] upward, more effectively presenting your shaft to be pleasured.' );
		MainView.outputText( '\n\nSighing happily, the immobile beauty ' );
		if( CoC.player.cockArea( x ) < 100 ) {
			MainView.outputText( 'leans down and kisses the tip, opening her glossy green puckers to slurp it down, sucking just hard enough to make your ' + CoC.player.cockHead( x ) + ' tingle with erotic warmth.' );
		} else {
			MainView.outputText( 'kisses up and down your huge, towering shaft, slobbering her glossy green puckers all over your urethral bulge, making it even more of a wet mess.' );
		}
		MainView.outputText( '  You grab hold of the edges of her tits and squeeze, smushing them tight around yourself and sliding them up and down.  The plant-woman\'s twinkling golden eyes meet your own while she pleasures your ' + Descriptors.cockDescript( x ) + ', and her hands come to rest over top of your own, assisting you with the quick tit-fuck.' );
		...
		MainView.outputText( ', slapping wetly against you.  Soon, the slick texture of her soft melons has you throbbing eagerly and dribbling hot trails of pre-cum.  You\'re going to blow any second now, and the hot bloom of pleasure within you is spreading further and further.  Those gold and black eyes look up imploringly while the pale green cheeks hollow hungrily, and you release ecstatically, arching your back as you pound the heavy tits down your length.' );
		MainView.outputText( '\n\nA geyser of spunk rockets out into ' );
		if( CoC.player.cockArea( x ) >= 100 ) {
			MainView.outputText( 'the air, where it hangs for a moment before splattering down atop the plant-girl\'s leaf-colored hair and face, forming a gossamer shroud.' );
		} else {
			MainView.outputText( 'her mouth, which works noisily to swallow the hot load even as you inject it into her.  After a few swallows, she leans back and lets you shoot it up into the air, so that it can fall down over her face and hair, into a gossamer shroud.' );
		}
		//highish cum:  ;
		if( CoC.player.cumQ() >= 500 ) {
			MainView.outputText( '  You keep blasting heavy ropes of seed until her body and trunk are painted with goo, and then you cum some more, smattering spunk until she looks more like a gooey waterfall than a fey creature.' );
		}
		if( CoC.player.cumQ() >= 1500 ) {
			MainView.outputText( '  Before you know it, there\'s a lake around the two of you and a small river running downhill away from camp.' );
		}
		MainView.outputText( '  A mischievous grin breaks out under the goopy facial, followed by an unnaturally long tongue that pulls gobs of the stuff into the dryad\'s hungry mouth.  Once she can see again, the girl gives her tits a squeeze and affectionately kisses your cock, practically worshipping it with sloppy-sweet licks.' );
		MainView.outputText( '\n\nYou break the embrace with a happy sigh and admire the state of your cock drips with milky syrup and still pulses with the pleasant aftershocks of your climax.  The pressure of your lust is dimmed, though a hint of it remains in your [balls], and you\'re still fairly hard.' );
		...
		MainView.outputText( '.  [EachCock], instead of slamming into the bark painfully, slots into something warm and wet, pulsating and pliant.  A clingy, tight hold that ripples gently around your member' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 's, massaging the bundle of cocks with uncommon ferver' );
		} else {
			MainView.outputText( ', massing your tool with uncommon fervor' );
		}
		MainView.outputText( '.  Your grunt of irritation vanishes as soon as your tender sex is forced into the heavenly hole, and with the soft breasts pressing into ' );
		if( CoC.player.biggestTitSize() >= 2 ) {
			MainView.outputText( 'your own' );
		} else {
			MainView.outputText( 'your [chest]' );
		}
		MainView.outputText( ', you give up and kiss the dryad full on the lips, biting the lower one roughly in irritation.' );
		MainView.outputText( '\n\nHolli deprived you of choice, but if you have to fuck, it\'s going to be by your rules.  You slide your fingers through the seam between her hips and the tree, finding a squeezable, pert behind to grab hold of.  Then, you pull your hips back, and still biting her lip, you slam them home, powerfully delivering your ' + Descriptors.multiCockDescriptLight() + ' into her slobbering, nectar-filled gash.  You let her lip go and menacingly growl at her, plowing her fields with animalistic vigor.' );
	...
	HolliScene.prototype.haveAMapleSyrupSnack = function() {
		MainView.clearOutput();
		MainView.outputText( 'You approach Holli with a hungry smile plastered across your [face], licking your chops as you eye the arboreal slut with unrestrained desire.  Of course, your gaze fixes on her heavy chest, with its distended, dripping nipples.  They leak sweet sap as you stare; the demon seems eager to feel your mouth on her slick, almond-colored buds.  Looking at you ' );
		if( CoC.flags[ kFLAGS.HOLLI_SUBMISSIVE ] === 0 ) {
			MainView.outputText( 'knowingly, Holli asks, "<i>Come for a little pick-me-up?  Well, go on; drink deeply of my nectar, </i>champion."  The last word is inundated with malicious, mocking tones.' );
		} else {
			MainView.outputText( 'eagerly, Holli asks, "<i>Care for a little pick-me-up?  They\'re getting very engorged, and I\'m sure it would be a refreshing snack for you.</i>"' );
		}
	...
	HolliScene.prototype.level4RideHollisTentacruels = function() {
		...
		MainView.outputText( '\n\nYou fold your arms impatiently across your [chest] and nod, waiting for her to get her phallic tendrils in position.  It takes you completely off guard when a half-dozen tentacles snap across your front and another bundle sweeps your [legs] out, spinning you to look face down at the ground, supported horizontally on a bed of taut, straining tentacles.  More loop over you, under you, and around you, cinching tight to your wrists and [feet].  They pull them out while other prehensile plants swing down to support you, occasionally looping around you, their moist skin easily gliding over your ' + CoC.player.skinFurScales() + '.  All that, and she\'s not even fucking you yet!' );
		MainView.outputText( '\n\nYou try to glare up at her, but when you look up, all you get is a faceful of flower-cunt.  Sneezing, you shake your head and try to pull away, but you have no control over your position at this point.  All you manage to do is smear feminine slime over your cheeks and get a fresh deposit of pollen on your nose, which brings on a fresh fit of sneezes.  "<i>This isn\'t what I ahh-ahhh-CHOOO!  0...asked for!</i>" you bellow before one of the thick, pulsating folds can obscure your mouth.' );
		...
		if( EngineCore.silly() ) {
			MainView.outputText( '  She proceeds to sing about Jack Sparrow\'s awesome adventures for a solid ten minutes, then goes on as if nothing had happened.' );
		}
		...
		if( CoC.player.lactationQ() === 0 ) {
			MainView.outputText( 'droplets of milk dripping from your nipples.  When did you start lactating?' );
		} else {
			MainView.outputText( 'steady streams of milk erupting from your nipples.  When did you start lactating this hard?' );
		}
		...
		if( !CoC.player.isBiped() ) {
			MainView.outputText( 'you\'d be walking bowlegged for a week, if you had legs.' );
		} else {
			MainView.outputText( 'you\'ll be walking bowlegged for a week!' );
		}
		//empty lust, boost corruption, slimefeed(), +1 cor, -3 sens, +.5 libido;
		CoC.player.orgasm();
		EngineCore.dynStats( 'lib', 0.5, 'sen', 2, 'cor', 1 );
	...
	HolliScene.prototype.eatHolliFruit = function() {
		MainView.clearOutput();
		MainView.outputText( 'You reach up into Holli\'s branches and pluck ' );
		if( CoC.flags[ kFLAGS.HOLLI_FRUIT ] === 1 ) {
			MainView.outputText( 'the fruit' );
		} else {
			MainView.outputText( 'one of the fruits' );
		}
		MainView.outputText( '.  ' );
		if( CoC.flags[ kFLAGS.HOLLI_FRUIT_EXPLAINED ] === 0 ) {
			MainView.outputText( 'Holli smiles over at you and asks, "<i>Enjoying the fruits of our union?</i>"' );
			...
			CoC.flags[ kFLAGS.HOLLI_FRUIT_EXPLAINED ]++;
		}
		MainView.outputText( 'Biting into it, sweet juices seem to explode from the flesh, dribbling down your chin.  It tastes like a dessert and you chow down, happily munching away.  In no time flat, you\'re down to just a core.  You toss it and wipe your [face] clean, then burp.  Damn, that was good!' );
		//TF CHANCES;
		if( Utils.rand( 2 ) === 0 && CoC.player.cockTotal() > CoC.player.tentacleCocks() ) {
			var choices = [];
			...
			MainView.outputText( '\n\nYour ' + Utils.num2Text2( choice + 1 ) + ' penis itches, and you idly scratch at it.  As you do, it begins to grow longer and longer, all the way to the ground before you realize something is wrong.  You pull open your [armor] and look down, discovering your ' + Descriptors.cockDescript( choice ) + ' has become a tentacle!  As you watch, it shortens back up; it\'s colored green except for a purplish head, and evidence seems to suggest you can make it stretch out at will.  <b>You now have a' );
			if( CoC.player.tentacleCocks() > 0 ) {
				MainView.outputText( 'nother' );
			}
			MainView.outputText( ' tentacle-cock!</b>' );
			CoC.player.cocks[ choice ].cockType = CockTypesEnum.TENTACLE;
			CoC.player.cocks[ choice ].knotMultiplier = 1.3;
			EngineCore.dynStats( 'sen', 3, 'lus', 10 );
		}
		CoC.flags[ kFLAGS.HOLLI_FRUIT ]--;
		EngineCore.dynStats( 'cor', 1 );
	...
	HolliScene.prototype.askHolliToWatch = function() {
		...
		if( CoC.flags[ kFLAGS.HOLLI_DEFENSE_ON ] === 0 ) {
			...
			if( CoC.flags[ kFLAGS.THREATENED_HOLLI ] > 0 ) {
				MainView.outputText( '<b>  You could always just slap the cunt.</b>' );
			}
		...
		//Stop Guarding (edited)(C);
			...
			MainView.outputText( 'You tell Holli you\'d like her to stop watching at night.  She laughs, "<i>You just want a chance to come back and beg some more, don\'t you?  I thought you were supposed to be tough shit, not a simpering little submissive.</i>"' );
	...
	HolliScene.prototype.begHolli4Watches = function() {
		MainView.clearOutput();
		MainView.outputText( 'You swallow your pride and drop down prostrate on the ground.  Sighing, you grudgingly say, "<i>Please, Holli, guard my camp at night.  I don\'t want to be raped by monsters.</i>"' );
	...
	HolliScene.prototype.domUpSomeHolli = function() {
		MainView.clearOutput();
		CoC.flags[ kFLAGS.HOLLI_FUCKED_TODAY ] = 1;
		MainView.outputText( 'You swagger up to the tree and put your hand on the bark beside Holli.  Glancing over, she quips, "<i>Is that supposed to be intimidating?</i>"  The dryad smirks ingratiatingly.  "<i>I\'m the offshoot of a demon goddess.  I don\'t think you thought this through.</i>"' );
		...
		var domPowah = CoC.player.level;
		domPowah += CoC.player.tallness / 12;
		...
		//[Next] - Holli's Menu;
		CoC.flags[ kFLAGS.HOLLI_SUBMISSIVE ] = 1;
		this.treeMenu( false );
	};
	//Guard Camp;
	HolliScene.prototype.askBrokenHolliToGuard = function() {
		MainView.clearOutput();
		MainView.outputText( 'Holli bows her head when you mention her guarding camp.  She asks, "<i>Shall I keep watch for foes to drain or let them into camp for you to have the pleasure of dealing with, [name]?  <b>I\'ve been ' );
		if( CoC.flags[ kFLAGS.HOLLI_DEFENSE_ON ] === 1 ) {
			MainView.outputText( 'keeping watch' );
		} else {
			MainView.outputText( 'sleeping, instead' );
		}
		MainView.outputText( '.</i>"' );
	...
	HolliScene.prototype.toggleBrokenHolliGuard = function() {
		MainView.clearOutput();
		if( CoC.flags[ kFLAGS.HOLLI_DEFENSE_ON ] === 0 ) {
			MainView.outputText( '"<i>All right, I\'ll get to have some fun then,</i>" Holli quips while rubbing her palms together in anticipation.' );
			MainView.outputText( '\n\nYou suppose you can sleep better with your pet demoness taking on the lesser whelps of this land.' );
			CoC.flags[ kFLAGS.HOLLI_DEFENSE_ON ] = 1;
		}
		//Guard Off;
		else {
			MainView.outputText( '"<i>All right, they\'re yours to smite or play with,</i>" Holli says with a wink.  You give her a glare, to which she responds with a shrug.  "<i>What?  I know how it is!  You mortals can\'t fight worth a damn when you\'re sleeping.</i>"' );
	...
	HolliScene.prototype.holliGetsDickDommed = function() {
		MainView.clearOutput();
		MainView.outputText( 'Tossing your [armor] aside, you expose your ' + Descriptors.multiCockDescriptLight() + '.  Holli\'s eyes fixate on [oneCock], her attention rapturously contained by a single phallus.  You grip your maleness in hand and bounce it up and down into your other palm.  The solid slaps of genital impacts echo off the surrounding rocks, each sending a pleasant tingle of pleasure through the root of your manhood.  Before long, you\'re completely erect and throbbingly hard, bouncing unbidden with each and every thrum of your heart.' );
		...
		if( CoC.player.biggestCockArea() > 80 ) {
			MainView.outputText( ', jaw stretching obscenely' );
		}
		MainView.outputText( '.' );
		...
		if( CoC.player.biggestCockArea() <= 80 ) {
			MainView.outputText( ', swallowed to the ' );
			if( CoC.player.balls > 0 ) {
				MainView.outputText( '[balls]' );
			} else {
				MainView.outputText( '[sheath]' );
			...
			MainView.outputText( ', swallowed as deep as Holli\'s distended neck can handle.' );
		}
		MainView.outputText( '  Gurgling, the demonic dryad flutters her lengthy eyelashes and looks up at your eyes, twinkling with happiness at her servitude.  You wiggle your crotch around, and it twists your [cock biggest] around her mouth and throat.  The tainted tree\'s tongue massages the underside of your length, embracing you with subtle moisture and frictionless pressure.' );
		MainView.outputText( '\n\nBubbling sexual pleasure begins to build up inside you, making your ' );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( '[sack] feel taut and ready to burst' );
		} else {
			MainView.outputText( 'body feel strung tight like an over-tuned guitar' );
		}
		MainView.outputText( '.  Holli pulls back, inciting a snarl of displeasure to vibrate from you, but as soon as she\'s withdrawn, her hands clasp onto the slick shaft and begin to pump it with vigorous strokes.  Her mouth settles back into place around your [cockHead biggest], circling it with her tongue while she bobs up and down on it, snaring your most sensitive place with overwhelming sensations.' );
		...
		if( CoC.player.biggestCockArea() > 80 ) {
			MainView.outputText( '  It bends slightly as the immense girth is forced inside her, aiming down, deep inside the trunk.' );
		}
		MainView.outputText( '  Holli flops into your [chest], twitching weakly and breathing heavily.  "<i>Fffuck, so good!</i>" she pants, nuzzling her cheek into your collarbone while her hands play across your shoulders.  You ruthlessly mount her flowery mound with fast motions.  Juices squirt out around your member at the apex of every stroke, raining down over the surrounding ground.  You grab Holli\'s deep green hair and pull her back.  Her face is wrecked by bliss, her mouth and eyes moving in uncoordinated twitches.' );
		...
		MainView.outputText( '\n\nThe abruptness of your orgasm startles you.  One moment there\'s the sanguine ecstasy of the surrounding fuck-flesh, and the next, there\'s an explosive volcano of lust boiling out of your ' + Descriptors.multiCockDescriptLight() + ', geysering fountains of fluid.' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( '  Excess jism spurts onto Holli\'s jiggling udders from below, spotting the viridian mounds with white.' );
			if( CoC.player.cumQ() >= 500 ) {
				MainView.outputText( '  The constant sprays turn them white in little time, dripping from the peaks of her nipples along with her sticky sap.' );
			}
		}
		if( CoC.player.cumQ() > 1000 ) {
			MainView.outputText( '  A trickle of sperm washes out of the demoness\'s entrance, somehow escaping the heavy suction.' );
		}
		if( CoC.player.cumQ() > 2000 ) {
			MainView.outputText( '  A moment later, that trickle turns into a river.' );
		}
		if( CoC.player.cumQ() > 3000 ) {
			MainView.outputText( '  You swear that her trunk must be absolutely stuffed with spunk.' );
		}
		if( CoC.player.cumQ() > 5000 ) {
			MainView.outputText( '  A spooge-spout sprays out as the pressure goes through the roof, confirming your suspicions in the messiest way possible.' );
		}
		MainView.outputText( '  Holli\'s tentacles cum with her, spraying her seed through her canopy in aimless ropes.  Most of it sticks to her branches and leaves, but small amounts of it trickle down to spatter the ground.  One of them lands on the back of your hand.  You frown and push it up against the tree-woman\'s mouth.  She licks you clean without question, still dazed from her orgasm.' );
	...
	HolliScene.prototype.fullOnTentacleTasticGangBangForHolli = function() {
		MainView.clearOutput();
		CoC.flags[ kFLAGS.HOLLI_FUCKED_TODAY ] = 1;
		MainView.outputText( 'You remove your [armor] and expose your writhing mass of wriggling tendrils to the tree-tart.  She places a hand over her mouth and exhales, "<i>Oh my, Mother Marae sure knows how to pick them, doesn\'t she?</i>"' );
		...
		if( EngineCore.silly() ) {
			MainView.outputText( ', inflatable-arm, flailing tube-cocks' );
		}
		MainView.outputText( '.  Below the orgy of arboreal affection, Holli\'s slick petals have spread wide.  You can actually see her interior twitching voraciously.  It opens and closes with pulsing hunger, vibrantly engorged.  As you watch, the fourth of your members lances forward of its own volition, spearing straight for that welcoming hole with no guidance from you.  It hits like a missile.  Two feet of that cock is gone in a moment, swallowed deep in the cunt-flower\'s bottomless depths.  A gush of girl-cum spurts out around the intruding member, and you can feel the muffled moan vibrate through the mouth-bound cock as she gives in to the four-way fucking.' );
		...
		if( CoC.player.balls > 0 ) {
			MainView.outputText( '  Your [balls] draw up tight to your groin and squeeze, rhythmically discharging their heavy loads.' );
		}
		...
		CoC.player.orgasm();
		EngineCore.dynStats( 'lib', 0.4, 'sen', -3, 'cor', 1 );
		CoC.flags[ kFLAGS.TIMES_FUCKED_FLOWER ]++;
		CoC.flags[ kFLAGS.HOLLI_FUCKED_TODAY ] = 1;
		this.fertilizeHolli();
		this.fertilizeHolli();
	...
	HolliScene.prototype.vaginalDomHollisTentacruels = function() {
		...
		if( !CoC.player.hasFuckableNipples() && CoC.player.lactationQ() < 50 ) {
			MainView.outputText( ' and pay special attention to your [nipples], tugging them in time with every sluggish thrust in your slit.' );
		} else if( CoC.player.lactationQ() >= 50 ) {
			MainView.outputText( ' and sigh at the ' );
			if( CoC.player.lactationQ() < 150 ) {
				MainView.outputText( 'squirts' );
			} else if( CoC.player.lactationQ() < 800 ) {
				MainView.outputText( 'sprays' );
			} else if( CoC.player.lactationQ() < 2000 ) {
				MainView.outputText( 'outpouring' );
			} else {
				MainView.outputText( 'thick fountains' );
			}
			MainView.outputText( ' of milk that escape with every tug, timed to match the sluggish thrusts at your slit.' );
		} else {
			MainView.outputText( ' and sink your fingers into your [nipples], fucking them to the sluggish tempo of the thrusting tentacle.' );
		}
		MainView.outputText( '  A loud groan escapes your mouth, and you rock your [hips] against the undulating tendril.  Your body\'s eagerness for more pleasure is unparalleled, but you control yourself as efficiently as you control the dryad.' );
		...
		if( CoC.player.clitLength >= 3.5 ) {
			MainView.outputText( ' for a moment before wrapping around and jerking on it' );
		}
		MainView.outputText( '.' );
	...
	HolliScene.prototype.threatenHolli = function() {
		MainView.clearOutput();
		MainView.outputText( 'You snarl and threaten to burn the barked bitch to the ground if she won\'t make herself useful after you ask politely.' );
		...
		if( CoC.player.level < 30 ) {
			MainView.outputText( 'Mighty as you are, Marae is likely still beyond your power.  Perhaps you should just ignore this demon for the time being.' );
		} else {
			MainView.outputText( 'Marae is likely beyond your power yet, but... you feel like she might not pressure you too hard if you can make a suitably credible threat.' );
		}
	...
	HolliScene.prototype.cutHerDown = function() {
		MainView.clearOutput();
		MainView.outputText( '"<i>Are you serious?  You really want to be broken that badly, my little toy?</i>"  The demon folds her arms over her hefty chest, her earlier, suggestive posture completely absent now.  "<i>You know my bark is as strong as any armor, right?  That Marae herself - the land itself - bolsters me?  Take that back right now, and I\'ll let you return to my good graces... if you put your face in the dirt and plead for the duty to pleasure me whenever I want.</i>"' );
	...
	HolliScene.prototype.defeatHolli = function() {
		...
		if( CoC.monster.findStatusAffect( StatusAffects.JojoIsAssisting ) < 0 ) {
			...
			MainView.outputText( 'hack at her trunk, laboriously peeling bark and making wedge-shaped cuts in it as you try to fell the abomination.  It takes nearly eight hours, but eventually the gnarled tree topples.  No sooner does it fall than it shrivels and turns to ash.' );
			if( CoC.time.hours + 7 >= 24 ) {
				CoC.time.days++;
			}
		...
		//Win with Jojo -Z;
			...
			MainView.outputText( 'The fire-blackened mouse cheers at you as the succubus groans and retreats completely into her bark.  "<i>Well done, [name]!  Help me carry tinder and we can burn this abomination to the ground!</i>"' );
	...
	HolliScene.prototype.recruitJojoToCamp = function() {
		MainView.clearOutput();
		MainView.outputText( 'Jojo nods eagerly at you when you invite him to share your camp.  "<i>Excellent idea.  If you\'ll excuse me, I wish to attempt to purify the spot where the creature grew, and then I will fetch my things.</i>"' );
	...
	HolliScene.prototype.enjoyYourBadEndBIYAAAATCH = function() {
		...
		if( CoC.player.gender === 1 || (Utils.rand( 2 ) === 0 && CoC.player.hasCock()) ) {
			MainView.outputText( '\n\nShe strips off your [armor] haphazardly, rending and shredding with vicious tugs and leaving deep lines where the clasps bite into your ' + CoC.player.skinFurScales() + ' before parting.  [OneCock] exposed, she lowers her mouth to the [cockHead biggest] and' );
			if( CoC.player.cocks[ CoC.player.biggestCockIndex() ].cockThickness > 6 ) {
				MainView.outputText( ', stretching her jaw wide open in a way that makes you wince,' );
			}
			MainView.outputText( ' engulfs you.  Holli\'s tongue plays over your slit and she looks up at you for just a moment to be sure you\'re watching.' );
			...
			if( CoC.player.biggestCockLength() > 24 ) {
				MainView.outputText( 'her neck ripples and extends grotesquely, and her head begins to migrate down all ' + Utils.num2Text( Math.round( CoC.player.biggestCockLength() ) ) + ' of your prick with alarming speed, pushing the stem inside along with it.  ' );
			} else {
				MainView.outputText( 'she sinks her head down to the base of your prick in one quick thrust, bringing the stem with it.  ' );
			}
			MainView.outputText( 'The sensation of having your entire [cock biggest] penetrated and sucked into her hot, wet, clinging throat pushes you back toward the edge and over, and it literally becomes a race for Holli to reach bottom before your nerves fire.  Her wet lips impact your crotch with a lewd splatter, but her tongue keeps going deep into your body, stopping only once it reaches your prostate.  A small bulge fires down the thin stem, forcing through your slit and into the shaft to lodge in your gland.  Holli\'s mouth and tongue withdraw once it is in place, and she grabs your cock in her hand, masturbating you vigorously.' );
			MainView.outputText( '\n\nIt\'s too much; your climax comes in a flood' );
			if( CoC.player.balls > 0 ) {
				MainView.outputText( ', and your [balls] draw close to your body to unload' );
			}
			MainView.outputText( '.  Your eyes roll as you flex and flex, but nothing comes out.  As you conclude your dry orgasm, you begin to feel a pressure on your prostate.' );
			...
			if( CoC.player.balls > 0 ) {
				MainView.outputText( 'migrating the other direction into your [balls], entangling and infiltrating them, filling them with the same warmth, then pressing out against your sack.  ' );
			} else {
				MainView.outputText( 'exploring the depths of your body and curling back against the surface of your ' + CoC.player.skin() + '.  ' );
			}
			MainView.outputText( 'Your stomach turns as the sprouts push their way through, crawling from your crotch down the outside of your [legs] to root in the dry earth.  The shoots thicken into fibrous roots, forming a trunk that supports you and pushes you closer to Holli and lower to the ground, until your writhing, tentacle-tipped prick is lined up with her pussy flower and within her easy reach.' );
		...
		//branch of vagina-having;
			...
			if( CoC.player.isTaur() ) {
				MainView.outputText( ' and turning you around' );
			}
			MainView.outputText( ', the demon buries her face in your [vagina], penetrating it with her wet, flexible tongue.' );
			...
			if( CoC.player.hasCock() ) {
				MainView.outputText( '  [OneCock] drools precum down its length and onto the flower, triggering another spasm and another gush from your prick.  Holli laughs as you continue to self-stimulate with your uncontrollable fluids.' );
			}
			...
			if( CoC.player.biggestTitSize() > 1 ) {
				MainView.outputText( 'wrapping around and under your breasts to keep them permanently jutting out in a lurid display' );
			} else {
				MainView.outputText( 'tracing a lattice of slender, criss-crossing ivy over your chest that hangs just far enough over your [nipples] that a stiff breeze will drag it into them' );
			}
			MainView.outputText( ', then continuing over your back and up your neck.  The tendrils thicken and multiply, occluding your forearms and lower [legs] in a casing of young bark and leaf buds, and Holli\'s own release you as yours take over the duty of holding you in place to receive whatever pleasure she forces upon you.  You\'re left in a salacious pose when the growth subsides, suspended several feet off the ground with your arms held over your head, your back arched in a memorial of your incredible orgasm, ' );
			if( CoC.player.biggestTitSize() > 1 ) {
				MainView.outputText( 'your breasts cupped and pressed together by vines, ' );
			}
			if( !CoC.player.isNaga() && !CoC.player.isGoo() ) {
				MainView.outputText( 'your thighs spread wide open, ' );
			}
			MainView.outputText( 'and your nectar-drooling flower-sex exposed with your [clit]' );
			if( CoC.player.hasCock() ) {
				MainView.outputText( ' and [eachCock]' );
			}
			MainView.outputText( ' bobbing in a state of permanent semi-arousal.' );
			...
			if( CoC.player.ass.analWetness > 0 ) {
				MainView.outputText( 'Betraying your depravity, the slick crack opens wide and your greedy, wet asshole practically welcomes the demon\'s entire hand, sucking it up and making you shiver with repressed delight.  "<i>Oh, mother!  It seems as though this is not a new lesson to you, is it?  My little whore, will you beg and moan as I fill you?</i>"' );
			} else if( CoC.player.analCapacity() < 30 ) {
				MainView.outputText( 'With some difficulty, she forces it into your pucker.  "<i>I think we\'ll just use this as our inroads, instead.  It may be slow going, but the journey is half the fun.</i>"' );
			} else {
				MainView.outputText( 'Despite your clenching, your loose hole proves no impediment to the demon\'s exploration.  "<i>Ah, how roomy you are!  I think I\'ll move right in.</i>"' );
			}
			...
			//[(iff buttslut);
			if( CoC.player.ass.analWetness > 0 ) {
				MainView.outputText( 'Try as you might, you can\'t see far enough down your back to determine what the demon has done.  The question answers itself, though, when she presses a shaft almost as thick as her leg into your [butt], nosing it into your wet, receptive asshole.  Despite yourself, you shiver in anticipation at being filled so completely.  ' );
			} else if( CoC.player.analCapacity() < 30 ) {
				MainView.outputText( 'Your jaw hits your collarbone as Holli angles the florid head of a long, narrow penis into your field of view.  Judging by how far it comes up her chest, it\'s almost a foot and half long, and it\'s drooling sticky sap all over your back.  With a twisted smile, the demon pushes it along your spine until it reaches your [butt], then shoves it toward the hole.  ' );
			} else {
				MainView.outputText( 'The demon presses up to your back, brushing her nipples against your skin and shoving her new growth between your thighs.  Looking down, you can see a fat, green shaft sticking from your crotch, glistening and dripping with sticky slime.  With another moan, Holli pulls it back from between your legs, then pushes apart your asscheeks and plunges it into your [asshole].  ' );
			}
			MainView.outputText( 'The sap oozing from her member coats your passage, ' );
			if( CoC.player.ass.analWetness > 0 ) {
				MainView.outputText( 'mingling with your own lubrication and ]easing her thrusts.  Holli\'s tempo picks up quickly, and soon you can feel her twitch inside you as she nears her climax.' );
			}
			...
			//[(buttslut or loose);
			if( CoC.player.ass.analWetness > 0 || CoC.player.analCapacity() > 30 ) {
				MainView.outputText( 'spilling a wave of her deposited fluids down your [legs].  ' );
			} else {
				MainView.outputText( 'and your once-tight pucker reflexively squeezes shut as best it can, trapping all but a few trickles of Holli\'s deposit inside you.  ' );
			}
			MainView.outputText( 'The freshly-grown shaft between Holli\'s legs begins to shrivel, withering and browning like a cut flower.  With an expression of mild disdain, she pulls it from her body and casts it aside, where it crumbles to pieces.' );
			...
			if( CoC.player.skinType === AppearanceDefs.SKIN_TYPE_FUR || CoC.player.skinType === AppearanceDefs.SKIN_TYPE_SCALES ) {
				MainView.outputText( ' under your ' + CoC.player.skinFurScales() );
			}
			MainView.outputText( '.  Panic intensifies as the sensation quickly focuses on your crotch, and when the tendrils holding you push you away from the demon\'s tree, your curiosity gets the better of you.  You look over your shoulder again, trying to see what\'s happening.  As you strain to watch, you feel hundreds of tiny, sharp pinches and small vines begin to creep from your body, just below your ass.  The vines merge and thread into thick trunks as they get closer to the ground, pushing deep into the dirt and mooring you to the spot.  Arousal builds in you as they join the web of roots linking the demon Marae and Holli, as does a swelling at your crotch.  Another cluster of thin vines push free from your groin, growing upward in a lattice and lashing your arms into place, ' );
			if( CoC.player.biggestTitSize() < 1 ) {
				MainView.outputText( 'folded atop your chest' );
			} else {
				MainView.outputText( 'crooked at your sides with your hands resting atop your [chest]' );
			}
			MainView.outputText( '.  They continue past your face, reaching over your head to form a canopy similar to Holli\'s own.  Struggle as you may, when they\'re done growing you\'re left unable to budge and locked in a rather artful pose, head turned over your shoulder with your [ass] thrust out' );
			if( CoC.player.biggestTitSize() >= 1 ) {
				MainView.outputText( ', your breasts exposed from the sides,' );
			}
			MainView.outputText( ' and your [legs] concealed below your buttcheeks by a thick tree trunk.' );
	...
	HolliScene.prototype.holliPlaysWithPenisesBadEnd = function() {
		...
		if( CoC.player.biggestTitSize() < 1 ) {
			MainView.outputText( 'masculine shape, it bears' );
		} else {
			MainView.outputText( 'feminine shape, it nonetheless bears' );
		}
		MainView.outputText( Descriptors.multiCockDescript() + ', standing erect and hard as iron; the brown-haired girl blushes and looks away.' );
		...
		MainView.outputText( '\n\nThe dryad gestures once again to her companion\'s ' + Descriptors.multiCockDescriptLight() + ', causing the brown-haired girl\'s mouth to hang open a little.  "<i>I can think of few better ways to demonstrate your vitality,</i>" Holli coos.  "<i>Do not feel indebted to us, but if you would share of yourself, we would be grateful.  And, in truth, is it not a hint of longing I see in your eyes?  Come, join with ' + CoC.player.mf( 'him', 'her' ) + ' for a while and clear your head.</i>"' );
		if( CoC.player.smallestCockArea() > 36 ) {
			MainView.outputText( '  As the dryad speaks, the silent figure\'s [cock smallest] shudders and begins to shrink, dwindling to a size that would fit ' + CoC.player.mf( 'him', 'her' ) + ' prospective lover.' );
		}
		...
		MainView.outputText( '\n\nBy the time the girl reaches her third orgasm, her partners are tense and near the breaking point; with an shudder and a stilling of the shaking branches, the silent one opens ' + CoC.player.mf( 'his', 'her' ) + ' mouth in mute ecstasy and ejaculates, ' );
		if( CoC.player.cocks.length > 1 ) {
			MainView.outputText( 'spraying the girl\'s stomach and tits with waste seed from ' + CoC.player.mf( 'his', 'her' ) + ' unused manhood and ' );
		}
		MainView.outputText( ' filling the Champion\'s pussy with so much semen it cascades from her stretched hole as Holli continues to pump.' );
		MainView.outputText( '\n\nThe dryad herself soon gives a cry and pushes her tits into the brown-haired girl\'s back as her climax comes.  A bulge ascends the green shaft, disappearing into the beat-up cunt of the Champion, and a trickle of golden sap leaks back down Holli\'s cock.  Pulling free, the dryad wipes her member off and folds it back into its bud; it shrinks and dwindles until nothing is left but her clitoris.  The silent one\'s branches lower the girl to the ground, ' + CoC.player.mf( 'his', 'her' ) + ' always-erect prick' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ' drooling aftershocks onto her breasts.' );
	...
	HolliScene.prototype.girlsGetANiceManToBadEnd = function() {
		...
		//[(if tits);
		if( CoC.player.biggestTitSize() > 1 ) {
			MainView.outputText( '[chest] cupped by vines and held out toward the young man, ' );
		} else {
			MainView.outputText( 'chest crisscrossed with a mesh of fine vines, ' );
		}
		MainView.outputText( 'its legs elevated and spread apart to display a ' );
		if( CoC.player.balls > 0 && CoC.player.ballSize > 10 && CoC.player.gender === 3 ) {
			MainView.outputText( Descriptors.multiCockDescriptLight() + ' with [balls] swinging below.' );
		}
		//(if H and ballsize < 10);
		else if( CoC.player.balls > 0 && CoC.player.ballSize <= 10 && CoC.player.gender === 3 ) {
			MainView.outputText( Descriptors.multiCockDescriptLight() + ' and [balls] that hang slightly over a [vagina].' );
		}
		//(if H and noballs);
		else if( CoC.player.gender === 3 ) {
			MainView.outputText( Descriptors.multiCockDescriptLight() + ' and a [vagina].' );
		} else {
			MainView.outputText( 'a wet, drooling cunt that practically begs to be filled.' );
		}
		MainView.outputText( '  Though its eyes are covered, the figure\'s mouth works constantly, biting and licking its lips as if beset by otherworldly amounts of lust.  Holli beckons the youth toward this tree.' );
		...
		if( CoC.player.ballSize > 10 && CoC.player.balls > 0 ) {
			MainView.outputText( 'lifting the hefty testicles to reveal a wet, eager pussy.' );
		} else {
			MainView.outputText( 'spreading the labia and exposing the eager, clenching hole inside.' );
		}
		...
		if( CoC.player.biggestTitSize() >= 1 ) {
			MainView.outputText( 'reaches up and grabs two handfuls of breast, passing his thumb over a nipple and watching the ' );
			if( CoC.player.gender === 2 ) {
				MainView.outputText( 'woman' );
			} else {
				MainView.outputText( 'hermaphrodite' );
			}
			MainView.outputText( '\'s mouth open and close in bliss.  ' );
		} else {
			MainView.outputText( 'leans down and kisses the [woman/hermaphrodite]\'s smooth chest, causing her to arch her back in pleasure.  ' );
		}
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'With a nervous glance downward, he slides one hand to her [cock smallest].  "<i>It\'s not gay if it\'s a chick\'s,</i>" he mumbles to himself, and then begins to stroke haltingly, playing his finger along the urethra and drawing a squirt of precum from his shivering partner.  ' );
		} else {
			MainView.outputText( 'His hand slides to the [clit] above her wet fuck-hole; collecting some of the frothing lube from their wet sex, ' );
			if( CoC.player.clitLength < 6 ) {
				MainView.outputText( 'he smears it over the little button and then begins to trace up and down the underside with a finger.  ' );
			} else {
				MainView.outputText( 'he coats the swollen organ with it and begins to jerk it off valiantly, like a cock.  ' );
			}
			MainView.outputText( '"<i>Does that feel good?</i>" he asks, upping the pace of his thrusts.  ' );
		}
		MainView.outputText( 'As if in reply, the ' );
		if( CoC.player.gender === 2 ) {
			MainView.outputText( 'woman' );
		} else {
			MainView.outputText( 'hermaphrodite' );
		}
		MainView.outputText( ' in the tree arches her back and her [vagina] clamps down on his tool' );
		if( CoC.player.hasCock() ) {
			MainView.outputText( '; ' );
			if( CoC.player.balls > 0 ) {
				MainView.outputText( 'her balls draw closer to her body and ' );
			}
			MainView.outputText( 'her trembling dick' );
			if( CoC.player.cockTotal() === 1 ) {
				MainView.outputText( ' throbs' );
			} else {
				MainView.outputText( 's throb' );
			}
			MainView.outputText( ' as she nears climax' );
		}
		MainView.outputText( '.' );
		//[(cock)];
		if( CoC.player.hasCock() ) {
			...
			MainView.outputText( ' toward her face; ' );
			if( CoC.player.biggestCockArea() < 20 ) {
				MainView.outputText( 'semen spurts and dribbles down her [chest], pooling in the combined laps and mingling with the sticky lube ringing her vagina.  ' );
				if( CoC.player.cumQ() >= 500 ) {
					MainView.outputText( 'So much so that by the time she finishes her climax, both her partner\'s legs and her trunk are covered in a sheen of milky white seed.  ' );
				}
			} else if( CoC.player.biggestCockArea() < 80 ) {
				MainView.outputText( 'her mouth is now full of cockhead, and she gulps as eagerly as she spurts, drooling lines of white from the corners of her lips.  ' );
				//[(big skeet)];
				if( CoC.player.cumQ() >= 500 ) {
					MainView.outputText( 'The waves of seed don\'t stop coming, pouring into her and swelling her belly out until its round bulge pushes the cock away from her mouth and the remaining squirts fall on her [chest].  ' );
				}
			}
			//else cock biggest > 80);
			else {
				MainView.outputText( 'her body is easily dwarfed by her male sex\'s massive size, and globs of sperm shoot into her canopy to splatter among her leaves and branches.  ' );
				if( CoC.player.cumQ() >= 1000 ) {
					MainView.outputText( 'The overproductive hermaphrodite\'s body pushes out so much seed that the entire canopy soon turns white and drips constantly as though it were in a tropical rainforest.  ' );
				}
			}
		}
		//(vagina and no cock);
			...
			if( CoC.player.wetness() < 3 ) {
				...
				MainView.outputText( 'rush' );
			} else {
				MainView.outputText( 'torrent' );
			}
			MainView.outputText( ' of female orgasm pours from her pussy, and her mouth opens wide in pleasure.  The tree branches surrounding the young man seem to close in, pushing him toward her.  Getting the message, he leans in and kisses his lover; her tongue probes his mouth with abandon as her body shakes out its fit and her [vagina] wrings his prick.  ' );
		}
		MainView.outputText( 'Pushed past his limit by her lewd display, the young man groans, shudders, and begins to unload his pent-up cargo.  Falling on top of his partner\'s slick ' );
		if( CoC.player.cockTotal() === 1 ) {
		...
		MainView.outputText( ', he goes limp and slack-jawed as his dick squirts, filling her vagina.' );
		...
		MainView.outputText( '\'s stomach.  "<i>In fact, she seems to have enjoyed it so much that she would have me bestow a blessing on you.</i>"' );
	...
	HolliScene.prototype.holliAndGenderlessSittingInATree = function() {
		...
		if( CoC.player.skinType !== AppearanceDefs.SKIN_TYPE_FUR && CoC.player.skinType !== AppearanceDefs.SKIN_TYPE_SCALES ) {
			MainView.outputText( CoC.player.skinTone + '-colored' );
		} else {
			MainView.outputText( 'covered in ' + CoC.player.skinFurScales() );
		}
		MainView.outputText( ' and wrapping its body around the tree in a way that makes determining its sex impossible.  No... it\'s not wrapping around the tree, but embedded in it up to the hips!' );
		...
		if( CoC.player.skinType === AppearanceDefs.SKIN_TYPE_SCALES ) {
			MainView.outputText( 'a quiver of arousal shakes its scales' );
		} else if( CoC.player.skinType === AppearanceDefs.SKIN_TYPE_FUR ) {
			MainView.outputText( 'its fur begins to prick up with arousal' );
		} else {
			MainView.outputText( 'a blush of lust colors its ' + CoC.player.skinFurScales() );
		}
		MainView.outputText( '...' );
	...
	HolliScene.prototype.amilyComesBack = function() {
		MainView.clearOutput();
		MainView.outputText( 'Amily arrives with her belongings over her shoulder and a smile on her face.  "<i>I knew you\'d do the right thing, [name].  I\'ll get my nest set back up.</i>"\n\n(<b>Amily has moved back in!  She can be found in the lovers tab.</b>)' );
		//CoC.flags[kFLAGS.AMILY_TREE_FLIPOUT] = 1;;
