...
angular.module( 'cocjs' ).run( function( Holli, PerkLib, $log, CockTypesEnum, PregnancyStore, Combat, Descriptors, AppearanceDefs, EventParser, CoC, kFLAGS, Utils, StatusAffects, EngineCore ) {
	function HolliScene() {
	}
	...
	HolliScene.prototype.fightHolli = function() {
		CoC.getInstance().flags[ kFLAGS.FOUGHT_HOLLI ] = 1;
	...
	HolliScene.prototype.treeMenu = function( output ) {
		...
		var fuck = null;
		var burnIt = null;
		if( CoC.getInstance().flags[ kFLAGS.FOUGHT_HOLLI ] > 0 ) {
			//If You Fight, Run, and Come Back To Her in the Menu -Z;
			EngineCore.outputText( 'The demon tree is still here, and the face peeking from it still stares daggers at you even from a distance.  It looks like forgiveness won\'t be forthcoming.' );
			...
			if( output ) {
				EngineCore.outputText( 'Before you is a large, waist-high plant of questionable origin.  A huge, violet bloom sits atop a strong, central stalk, and the obscene flower looks positively female in every sense, right down to a clit-shaped bump and moist, gently parted lips.  Inside that central passage, you can see a bevy of wiggling staman surrounding a larger, flared stalk.  The scent that wafts out of that opening is sweet and floral, but undoubtedly sexual.  It looks like an organic sex toy and is clearly corrupt.  It wouldn\'t be hard to get rid of right now, assuming you don\'t let it grow larger.' );
			}
			if( CoC.getInstance().player.hasCock() && CoC.getInstance().player.cockThatFits( 100 ) >= 0 && CoC.getInstance().player.lust >= 33 ) {
			...
			if( CoC.getInstance().player.findPerk( PerkLib.Dragonfire ) >= 0 || CoC.getInstance().player.findPerk( PerkLib.FireLord ) >= 0 || CoC.getInstance().player.findPerk( PerkLib.Hellfire ) >= 0 || CoC.getInstance().player.findStatusAffect( StatusAffects.KnowsWhitefire ) >= 0 ) {
				if( output ) {
			...
			if( output ) {
				...
				if( CoC.getInstance().flags[ kFLAGS.HOLLI_SUBMISSIVE ] === 1 ) {
					EngineCore.outputText( '\n\n"<i>Welcome back, [name].  ' );
					if( CoC.getInstance().flags[ kFLAGS.TIMES_FUCKED_FLOWER ] > 0 && Utils.rand( 3 ) === 0 ) {
						EngineCore.outputText( 'Did you want to fertilize me again?  My flower is ready and waiting...' );
					} else if( CoC.getInstance().flags[ kFLAGS.TIMES_RIDDEN_FLOWER ] > 0 && Utils.rand( 3 ) === 0 ) {
						EngineCore.outputText( 'Did you want to use my tentacles again?  They\'ve been restless without you...' );
					} else if( Utils.rand( 2 ) === 0 ) {
						EngineCore.outputText( 'Did you come for a drink?  I\'ve built up lots of nectar for you.  Surely, a sip would be refreshing for you...' );
					} else {
						EngineCore.outputText( 'How may I serve you?  Come, give me a command so that I can fulfill it for you.' );
					}
					EngineCore.outputText( '</i>"' );
				} else {
					EngineCore.outputText( '\n\n"<i>Welcome back, [name].  ' );
					if( CoC.getInstance().flags[ kFLAGS.TIMES_FUCKED_FLOWER ] > 0 && Utils.rand( 3 ) === 0 ) {
						EngineCore.outputText( 'Did you want to fertilize me again?  Gardening is such exhausting work, but so rewarding...' );
					} else if( CoC.getInstance().flags[ kFLAGS.TIMES_RIDDEN_FLOWER ] > 0 && Utils.rand( 2 ) === 0 ) {
						EngineCore.outputText( 'Did you come back to play again?  My tentacles are so restless.' );
					} else if( Utils.rand( 2 ) === 0 ) {
						EngineCore.outputText( 'Did you come for a drink?  I\'ve built up lots of nectar for you.  Surely a hungry adventurer like yourself needs a sip, right?' );
					} else {
						EngineCore.outputText( 'Do you want to play?  Come on, taste Mother Marae\'s finest creation.' );
					}
					EngineCore.outputText( '</i>"' );
				}
				//Extra line on description:  ;
				if( CoC.getInstance().flags[ kFLAGS.HOLLI_FRUIT ] > 0 ) {
					if( CoC.getInstance().flags[ kFLAGS.HOLLI_FRUIT ] === 1 ) {
						EngineCore.outputText( '\n\nA large, purple fruit hangs from Holli\'s branches, twisting softly in the breeze.  It looks succulent and ripe.' );
					} else {
						EngineCore.outputText( '\n\n' + Utils.Num2Text( CoC.getInstance().flags[ kFLAGS.HOLLI_FRUIT ] ) + ' large, purple fruits hang from Holli\'s branches, twisting softly in the breeze.  They look succulent and ripe.' );
					}
					//Optional Addition: ;
					if( CoC.getInstance().flags[ kFLAGS.HOLLI_FRUIT_EXPLAINED ] > 0 ) {
						EngineCore.outputText( '  A very small, strange part of you actually feels proud to have made something like that with the tree-woman.' );
					}
				}
			}
			if( CoC.getInstance().flags[ kFLAGS.HOLLI_SUBMISSIVE ] === 0 ) {
				...
				var guardT = '';
				if( CoC.getInstance().flags[ kFLAGS.HOLLI_DEFENSE_ON ] === 1 ) {
				...
				if( CoC.getInstance().player.tentacleCocks() >= 10 && CoC.getInstance().player.lust >= 33 ) {
	...
	HolliScene.prototype.helpWithFarm = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.HOLLI_FUCKED_TODAY ] === 0 ) {
	...
	HolliScene.prototype.fertilizeHolli = function( cock ) {
		...
		var odds = 20;
		if( cock && CoC.getInstance().player.hasCock() ) {
			odds += CoC.getInstance().player.cumQ() / 300;
			if( odds > 40 ) {
				odds = 40;
			}
			...
			if( CoC.getInstance().player.findPerk( PerkLib.ElvenBounty ) >= 0 ) {
				odds += 2;
			}
		}
		if( !cock && CoC.getInstance().player.hasVagina() ) {
			odds += CoC.getInstance().player.totalFertility() / 5;
			if( odds > 40 ) {
				odds = 40;
			}
		...
		if( odds >= Utils.rand( 101 ) ) {
			CoC.getInstance().flags[ kFLAGS.HOLLI_FRUIT ] += 1 + Utils.rand( Math.floor( odds / 10 ) );
	...
	HolliScene.prototype.getASprout = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.CORRUPT_MARAE_FOLLOWUP_ENCOUNTER_STATE ] === 2 ) {
			EngineCore.outputText( '  She even promised to plant a tree in your camp...' );
		}
		EngineCore.outputText( '  You look closer and see purple veins on the undersides of the leaves, throbbing with what can only be the tainted liquids that suffuse much of this planet.  Fully grown, it might spread its corruption to your camp.\n\n<b>Do you destroy it?</b>' );
		CoC.getInstance().flags[ kFLAGS.FUCK_FLOWER_LEVEL ] = 1;
		CoC.getInstance().flags[ kFLAGS.FUCK_FLOWER_GROWTH_COUNTER ] = 0;
		//[Yes] [No];
	...
	HolliScene.prototype.destroyDatFukkinTree = function() {
		...
		CoC.getInstance().flags[ kFLAGS.FUCK_FLOWER_KILLED ] = 1;
		EngineCore.doNext( CoC.getInstance().scenes.camp.returnToCampUseOneHour );
	};
	//[No] (edited);
	...
	HolliScene.prototype.fuckPlantGrowsToLevel2 = function() {
		EngineCore.clearOutput();
		CoC.getInstance().flags[ kFLAGS.FUCK_FLOWER_LEVEL ] = 2;
		CoC.getInstance().flags[ kFLAGS.FUCK_FLOWER_GROWTH_COUNTER ] = 0;
		...
		if( CoC.getInstance().player.gender === 0 ) {
			EngineCore.outputText( '  Sucks to be it!  You don\'t waste time with stupid shit, like genitals.' );
		}
		EngineCore.outputText( '  Is this Marae\'s idea of a gift?' );
		if( CoC.getInstance().player.gender > 0 ) {
			EngineCore.outputText( '  Well, you could give it a whirl... just being around it seems to slowly turn you on.' );
		}
		EngineCore.outputText( '  Of course, destroying it would be the safest option.' );
	...
	HolliScene.prototype.fuckFuckingFuckFlowerP2 = function() {
		...
		EngineCore.outputText( ' and form a sticky puddle.  Now moaning, you grab the stem and start to pump the sheath up and down, using the plant like the cock-toy it was meant to be.  Lewd, wet squishes ring out through the air,' );
		if( CoC.getInstance().player.cor < 33 ) {
			EngineCore.outputText( ' and you timidly look around, hoping you haven\'t attracted an audience.' );
		} else if( CoC.getInstance().player.cor < 66 ) {
			EngineCore.outputText( ' and you worriedly glance around, hoping yet terrified that someone will walk in on it.' );
		} else {
			EngineCore.outputText( ' and you glance around, hoping someone will walk in on the sexy show you\'re making.' );
		}
		...
		if( CoC.getInstance().player.hasSheath() ) {
			EngineCore.outputText( ' inside your violated sheath' );
		}
		EngineCore.outputText( '.  With so much blood trapped in your ' + Descriptors.cockDescript( x ) + ', it\'s bigger and harder than ever, twitching valiantly against its restraints with every beat of your heart.' );
		EngineCore.outputText( '\n\nOne of the tentacles circles your ' + CoC.getInstance().player.cockHead( x ) + ' for a moment, and without much warning or pause, it lances forward to bury itself into your vulnerable cum-slit.  Your preconceptions are turned on their heads when there\'s a complete lack of pain from the abrupt penetration.  If anything, it actually feels kind of good... like there\'s a warm, slippery finger caressing your penis from the inside out.  It slithers in and out of you, pumping you inside, burrowing pleasure into your center even while stroking you from without.  Your body, burning from the exertion of fucking the flower\'s pod and the constantly-rising tidal wave of lust, begins to twitch spasmodically.' );
		...
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( 'Your [balls] gurgle, a surprisingly happy sound, and cling tight to your loins.  ' );
		}
		EngineCore.outputText( 'Inside you, you can feel the liquid heat of your spurting release exploding out, melting everything before it with liquid, orgasmic bliss.  A low moan bursts from your lips when the sounding tentacle pulls out of your sloppy cock-slot, and the rushing tide of bubbling cum follows it, bursting into the sweet, suckling nectar-pot.' );
		if( CoC.getInstance().player.cumQ() > 500 ) {
			EngineCore.outputText( '  You pour forth your prodigious essence without pause or fail, and the plant immediately begins to drink it down, the stem bulging with rounded balls of sperm that it carries underground.' );
		}
		if( CoC.getInstance().player.cumQ() > 1500 ) {
			EngineCore.outputText( '  Soon, the meager stem cannot swallow it all.  Spunk froths and squirts out around the base of your tool for a moment, and then a veritable tide of the stuff gushes out to soak  you.' );
		}
		...
		CoC.getInstance().flags[ kFLAGS.TIMES_FUCKED_FLOWER ] = 1;
		if( CoC.getInstance().flags[ kFLAGS.FUCK_FLOWER_GROWTH_COUNTER ] < 1000 ) {
			CoC.getInstance().flags[ kFLAGS.FUCK_FLOWER_GROWTH_COUNTER ] += 4;
		}
	...
	HolliScene.prototype.rideDatFuckingFukkFlowerP2 = function() {
		...
		if( CoC.getInstance().player.clitLength > 4 ) {
			EngineCore.outputText( '  Your heavy buzzer is so big that the flexible plant-cock has plenty of room to curl around it, and if you could see it, your clit would look almost like a candy-cane (if one set of stripes was jerking off the cane beneath).' );
		}
		if( CoC.getInstance().player.cockTotal() > 0 ) {
			EngineCore.outputText( '  [EachCock] soon has its own partner twisting around it, a pussy-slicked vine to jerk and twine about its girth.' );
		}
		EngineCore.outputText( '\n\nWith nothing to occupy your roving hands, you let them slide up your ' + CoC.getInstance().player.skinFurScales() + ' to your ' );
		if( CoC.getInstance().player.bRows() > 1 ) {
			EngineCore.outputText( 'lowest row of tits, cupping them before groping to your top row of breasts' );
		} else {
			EngineCore.outputText( '[chest]' );
		}
		EngineCore.outputText( ', caressing the' );
		if( CoC.getInstance().player.biggestTitSize() > 1 ) {
			EngineCore.outputText( 'ir' );
		}
		EngineCore.outputText( ' swell as you search for the lust-tightened skin of your [nipples].  They seem almost as sensitive as little clits, and as soon as you find them, you immediately set to circling them slowly, occasionally tugging and twisting when a particularly powerful sensation works through your pleasure-wracked body.' );
		if( CoC.getInstance().player.lactationQ() > 50 ) {
			EngineCore.outputText( '  One after the other, they begin to ' );
			if( CoC.getInstance().player.lactationQ() < 150 ) {
				EngineCore.outputText( 'trickle' );
			} else if( CoC.getInstance().player.lactationQ() < 500 ) {
				EngineCore.outputText( 'leak' );
			} else {
				EngineCore.outputText( 'spray torrents of' );
			}
			EngineCore.outputText( ' milk.' );
		}
		EngineCore.outputText( '  You moan in unabashed delight, your mouth hanging open in a mindless, blissful \'o\' in between vocalizations.  If your friends back home could see you now, writhing in the dirt, face open in simple, animal pleasure, and perhaps worst of all, fucking a plant, you\'d die of ' );
		if( CoC.getInstance().flags[ kFLAGS.PC_FETISH ] === 0 && CoC.getInstance().player.cor + CoC.getInstance().player.lib < 150 ) {
			EngineCore.outputText( 'embarrassment' );
		} else {
		...
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( '  [EachCock] explodes in release, pumping heavy flows of seed into the petals, even as y' );
		} else {
			EngineCore.outputText( 'Y' );
		}
		EngineCore.outputText( 'our quivering pussy, already sopping wet, releases the last of your juices in a soaked ' );
		if( CoC.getInstance().player.cumQ() < 50 ) {
			EngineCore.outputText( 'drizzle' );
		...
		EngineCore.outputText( '.  The tentacles inside you curl in on themselves and slowly pull back from the well-creamed quim, scraping out as much fluid as possible from your oversensitive honey-pot.  The binding on your [clit] releases, which of course sets off a whole new wave of rippling pleasure-contractions in your cunt.' );
		...
		EngineCore.dynStats( 'cor', 1 );
		EngineCore.fatigue( 5 );
		CoC.getInstance().flags[ kFLAGS.TIMES_RIDDEN_FLOWER ] = 1;
		if( CoC.getInstance().flags[ kFLAGS.FUCK_FLOWER_GROWTH_COUNTER ] < 1000 ) {
			CoC.getInstance().flags[ kFLAGS.FUCK_FLOWER_GROWTH_COUNTER ] += 4;
		}
		EngineCore.doNext( CoC.getInstance().scenes.camp.returnToCampUseOneHour );
	...
	HolliScene.prototype.flowerGrowsToP3 = function() {
		...
		var burnIt = null;
		if( CoC.getInstance().player.findPerk( PerkLib.Dragonfire ) >= 0 || CoC.getInstance().player.findPerk( PerkLib.FireLord ) >= 0 || CoC.getInstance().player.findPerk( PerkLib.Hellfire ) >= 0 || CoC.getInstance().player.findStatusAffect( StatusAffects.KnowsWhitefire ) >= 0 ) {
			EngineCore.outputText( ', though you suppose you could burn it down with magical fire if you set your mind to it' );
			burnIt = this.torchP3Tree;
		}
		CoC.getInstance().flags[ kFLAGS.FUCK_FLOWER_LEVEL ] = 3;
		CoC.getInstance().flags[ kFLAGS.FUCK_FLOWER_GROWTH_COUNTER ] = 0;
		EngineCore.outputText( '.\n\n<b>What do you do?</b>' );
		var fuck = null;
		var ride = null;
	...
	HolliScene.prototype.fuckTheFlower = function() {
		...
		if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( '  Wetness builds in your [vagina] as well, but you\'re focusing on your maleness just now.  Your neglected twat will have to wait its turn.' );
		}
		EngineCore.outputText( '  A feverish sweat breaks out on your brow, though whether from your own arousal, the plant\'s heat, or some chemical in its juices, you cannot tell.' );
		EngineCore.outputText( '\n\nBefore you can pull back, the hiss of something cutting through the air with lightning speed hits your eardrums.  Almost at the same time, tentacles (stamen?) snap around your neck, head, and ears, going taut in an instant.  Your ' + CoC.getInstance().player.face() + ' is pulled roughly into the glossy hole, the soft folds of the petals smearing your nose and mouth with their saccharine sweetness.  You cannot help but breathe in more deeply as you struggle for your freedom, and with every struggle, every struggling gasp, [eachCock] grows hotter and harder, pulsating with tactile hunger.  With each moment that passes the motivation for your struggles shifts from freedom to fucking.  You don\'t care about getting away so much as getting your dick in this aromatic fuck-pot.' );
		...
		if( CoC.getInstance().player.cockArea( x ) >= 150 ) {
			EngineCore.outputText( ', sliding down the trunk as easily if the entire inside was a giant pussy just waiting to get fucked' );
		}
		EngineCore.outputText( '.' );
		if( CoC.getInstance().player.cockTotal() > 1 ) {
			EngineCore.outputText( '  Your other penis' );
			if( CoC.getInstance().player.cockTotal() > 2 ) {
				EngineCore.outputText( 'es rub' );
			} else {
				EngineCore.outputText( ' rubs' );
			}
			EngineCore.outputText( ' sensually across the moist outer blossom, drooling their clear pre-cum over the petals.' );
		}
		EngineCore.outputText( '  Schliiick.  You slide out.  Slorp-pop.  You slide in, this time to the ' );
		if( CoC.getInstance().player.hasSheath() ) {
		...
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( ' with your gurgling balls slapping against the flower\'s edge' );
		}
		EngineCore.outputText( ', accompanied by a small squirt of sweet lubricants.' );
		...
		if( CoC.getInstance().player.cockTotal() > 1 ) {
			EngineCore.outputText( ', as your multiple endowments allow you to drizzle the purple bloom\'s exterior with alabaster cream' );
		}
		EngineCore.outputText( '.' );
		if( CoC.getInstance().player.cumQ() > 500 ) {
			EngineCore.outputText( '  Pearly ropes big enough to fill a flask lurch out from your cum-slit, propelled by your quivering orgasmic muscles deep into the trunk, and you swear you can hear the pitch of your sexual pumping rising in tone, changed by the flooded vessel.' );
		}
		if( CoC.getInstance().player.cumQ() > 1500 ) {
			EngineCore.outputText( '  A wave of your thick gunk rolls out from under your pumping cock - you\'ve filled the tree beyond even its capacity, and the thick puddle forming beneath your [feet] is all the proof you need of your virility.' );
		}
		if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( '  Your hungry cunt, long denied in this exchange, trembles as it ' );
			if( CoC.getInstance().player.wetness() < 4 ) {
				EngineCore.outputText( 'leaks' );
			} else {
				EngineCore.outputText( 'sprays' );
			}
			EngineCore.outputText( ' your own female fluids in sympathy.' );
		}
		EngineCore.outputText( '  After what feels like forever, the spurting seed no longer flows from your long-abused dick, and as your member softens, the tentacles swing you away, setting you down softly.' );
		...
		EngineCore.fatigue( 5 );
		if( CoC.getInstance().flags[ kFLAGS.FUCK_FLOWER_GROWTH_COUNTER ] < 1000 ) {
			CoC.getInstance().flags[ kFLAGS.FUCK_FLOWER_GROWTH_COUNTER ] += 5;
		}
		CoC.getInstance().flags[ kFLAGS.TIMES_FUCKED_FLOWER ]++;
		CoC.getInstance().flags[ kFLAGS.HOLLI_FUCKED_TODAY ] = 1;
		EngineCore.doNext( CoC.getInstance().scenes.camp.returnToCampUseOneHour );
	};
	//Drink Sap (edited);
	HolliScene.prototype.drinkThePlantGirlsSap = function() {
		...
		if( EngineCore.silly() ) {
			EngineCore.outputText( ', tree hugger that you are' );
		}
	...
	HolliScene.prototype.rideTheWalrusP3 = function() {
		...
		EngineCore.outputText( '\n\nAt some point, you are set back down on the ground and sprayed with a fresh batch of seed.  You rest there for a while, until you can catch your breath and move without pain.  When you start trying to clean up, you realize the puddle of cum is completely gone, likely swallowed back up the tree\'s hungry roots, and the only mess left is your sodden ' + CoC.getInstance().player.skinDesc + '.  You get dressed and stumble away a little drunkenly, your [vagina] raw and oozing.' );
		...
		CoC.getInstance().flags[ kFLAGS.TIMES_RIDDEN_FLOWER ]++;
		CoC.getInstance().flags[ kFLAGS.HOLLI_FUCKED_TODAY ] = 1;
		EngineCore.doNext( CoC.getInstance().scenes.camp.returnToCampUseOneHour );
	};
	//Torch It (edited)(C);
	HolliScene.prototype.torchP3Tree = function() {
		...
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.KnowsWhitefire ) >= 0 ) {
			EngineCore.outputText( 'Closing your eyes to focus, you gather your energies, and unleash your white, supernatural flames upon the thing.' );
		}//(bromancefire);
		else if( CoC.getInstance().player.findPerk( PerkLib.FireLord ) >= 0 ) {
			EngineCore.outputText( 'Sucking in your breath, you inflate your chest and stir the embers of the jaguar demon\'s gift.  Dredging up an enormous pine-green fireball, you exhale and launch it at the tree.' );
		} else {
			EngineCore.outputText( 'Sucking in your breath, you inflate your chest and stir the embers of your fiery gift.  Dredging up an enormous fireball, you exhale and launch it at the tree.' );
		}
		EngineCore.outputText( '  It goes up by the torch, though the fire starts to fade sooner than you would expect.  You huff, but repeat the action, layering a new blaze atop the previous one; you put all your concentration into this new conflagration, stoking the inferno with every ounce of your ' );
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.KnowsWhitefire ) >= 0 ) {
			EngineCore.outputText( 'magical will' );
		} else {
			EngineCore.outputText( 'breath' );
		}
		EngineCore.outputText( '.' );
	...
	HolliScene.prototype.treePhaseFourGo = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.TIMES_FUCKED_FLOWER ] + CoC.getInstance().flags[ kFLAGS.TIMES_RIDDEN_FLOWER ] > 0 ) {
			EngineCore.outputText( 'Did you enjoy feeding me?  You should have seen the expression on your face when you were cumming in me.  Your eyes were rolling back and your mouth was just hanging open.  Do you know what it made me want to do?' );
		} else {
			EngineCore.outputText( 'Did you enjoy watching me grow?  You should have fed me.  It was so much harder to grow without proper nutrients.  Do you know what I thought about during all the time I spent waiting?  Do you know what it made me want to do?' );
		}
		EngineCore.outputText( '</i>"' );
	...
	HolliScene.prototype.JojoTransformAndRollOut = function() {
		...
		if( CoC.getInstance().player.cor < 25 ) {
			EngineCore.outputText( 'Not willing to lie to your friend, you also admit that she seems to have changed after your actions in the factory - that, though you were trying to render the tanks permanently inoperable, the spillage you created pushed the cthonic spirit over the edge.' );
		} else {
			EngineCore.outputText( 'Though, you do leave out the precise, needless details of what caused the change.' );
		}
		EngineCore.outputText( '  The monk\'s eyes widen in surprise and surmise.' );
		EngineCore.outputText( '\n\n"<i>Unbelievable...</i>" he blurts.  "<i>' );
		if( CoC.getInstance().flags[ kFLAGS.TIMES_FUCKED_FLOWER ] + CoC.getInstance().flags[ kFLAGS.TIMES_RIDDEN_FLOWER ] === 0 ) {
			EngineCore.outputText( 'To think that I must now consider Marae herself my enemy.  Forgive me, but this place is not safe for us anymore if it has attracted the attention of such a creature.  For now, I will return to the forest.  I know that you believe your mission keeps you here, but I urge you to stay away from that thing and watch the portal from an adequate distance.' );
		}
		//(totally deflowered that flower);
		else {
			EngineCore.outputText( 'And even knowing that this could be the case, you still chose to fornicate with it?  [name]... I cannot say that I am not disappointed.  This place is not safe for me anymore.  For now, I will return to the forest.' );
		}
		EngineCore.outputText( '</i>"' );
		EngineCore.outputText( 'Jojo looks at you sadly; any argument you try to make is met with a resigned shake of his head.  "<i>If the creature truly is the demonic offspring of a corrupted earth spirit, doubtless it will be powerful and the wrath of its parent severe.' );
		if( CoC.getInstance().player.level < 20 ) {
			EngineCore.outputText( '  Nonetheless, if you find yourself in a position where you must confront this demon or perish, call upon me.  I will render what aid I can, though I prefer it not be in vain.' );
		} else {
			EngineCore.outputText( '  Though, a mortal of your stature and resources may be able to destroy her and escape the reprisal.  I would counsel against direct confrontation until you are sure of victory, but if you must fight, then call upon me and you will have what aid I can give.' );
		}
		EngineCore.outputText( '</i>"  So saying, he presses a small talisman into your hand.' );
		EngineCore.outputText( '"<i>Until then, you will find me in the forest if you need my assistance.</i>"  Jojo makes a polite bow and departs.' );
		EngineCore.outputText( '(<b>Jojo is no longer a companion!</b>)' );
	...
	HolliScene.prototype.stayQuietWhileAmilyBitchesAboutFuckingArborday = function() {
		...
		CoC.getInstance().flags[ kFLAGS.AMILY_TREE_FLIPOUT ] = 1;
	...
	HolliScene.prototype.slapAmilysWhoreFace = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.AMILY_NOT_FURRY ] === 0 ) {
			EngineCore.outputText( 'mouse-' );
		}
		EngineCore.outputText( 'girl says, eyes wet with tears, "<i>but I don\'t.  Goodbye.</i>"' );
	...
	HolliScene.prototype.fuckHolliInZeFlowerPuss = function() {
		...
		EngineCore.outputText( ' ' + CoC.getInstance().player.mf( 'boy', 'girl' ) + ',</i>" the arboreal tramp suggests with a husky coo, "<i>I got you some sugar.</i>"  She pinches her nipples and gasps, dark streams of sugary juices dripping wetly into the soil as she beckons you closer.' );
		...
		if( CoC.getInstance().player.cockTotal() > 1 ) {
			if( CoC.getInstance().player.cockTotal() === 2 ) {
				EngineCore.outputText( '  Your other penis splays' );
			} else {
				EngineCore.outputText( 'The rest of your penises splay' );
			}
			EngineCore.outputText( ' to the side, smearing furrows of the stuff around as they slide over the butter-smooth skin.' );
		}
		EngineCore.outputText( ' The demon dryad\'s hand reaches below to ' );
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( 'cradle your [balls], gently massaging your sack' );
		} else {
			EngineCore.outputText( 'gently caress your ' );
			if( CoC.getInstance().player.hasSheath() ) {
				EngineCore.outputText( 'sheath' );
			} else {
				EngineCore.outputText( 'loins' );
			}
		}
		EngineCore.outputText( ' encouragingly.  You respond by edging your [hips] upward, more effectively presenting your shaft to be pleasured.' );
		EngineCore.outputText( '\n\nSighing happily, the immobile beauty ' );
		if( CoC.getInstance().player.cockArea( x ) < 100 ) {
			EngineCore.outputText( 'leans down and kisses the tip, opening her glossy green puckers to slurp it down, sucking just hard enough to make your ' + CoC.getInstance().player.cockHead( x ) + ' tingle with erotic warmth.' );
		} else {
			EngineCore.outputText( 'kisses up and down your huge, towering shaft, slobbering her glossy green puckers all over your urethral bulge, making it even more of a wet mess.' );
		}
		EngineCore.outputText( '  You grab hold of the edges of her tits and squeeze, smushing them tight around yourself and sliding them up and down.  The plant-woman\'s twinkling golden eyes meet your own while she pleasures your ' + Descriptors.cockDescript( x ) + ', and her hands come to rest over top of your own, assisting you with the quick tit-fuck.' );
		...
		EngineCore.outputText( ', slapping wetly against you.  Soon, the slick texture of her soft melons has you throbbing eagerly and dribbling hot trails of pre-cum.  You\'re going to blow any second now, and the hot bloom of pleasure within you is spreading further and further.  Those gold and black eyes look up imploringly while the pale green cheeks hollow hungrily, and you release ecstatically, arching your back as you pound the heavy tits down your length.' );
		EngineCore.outputText( '\n\nA geyser of spunk rockets out into ' );
		if( CoC.getInstance().player.cockArea( x ) >= 100 ) {
			EngineCore.outputText( 'the air, where it hangs for a moment before splattering down atop the plant-girl\'s leaf-colored hair and face, forming a gossamer shroud.' );
		} else {
			EngineCore.outputText( 'her mouth, which works noisily to swallow the hot load even as you inject it into her.  After a few swallows, she leans back and lets you shoot it up into the air, so that it can fall down over her face and hair, into a gossamer shroud.' );
		}
		//highish cum:  ;
		if( CoC.getInstance().player.cumQ() >= 500 ) {
			EngineCore.outputText( '  You keep blasting heavy ropes of seed until her body and trunk are painted with goo, and then you cum some more, smattering spunk until she looks more like a gooey waterfall than a fey creature.' );
		}
		if( CoC.getInstance().player.cumQ() >= 1500 ) {
			EngineCore.outputText( '  Before you know it, there\'s a lake around the two of you and a small river running downhill away from camp.' );
		}
		EngineCore.outputText( '  A mischievous grin breaks out under the goopy facial, followed by an unnaturally long tongue that pulls gobs of the stuff into the dryad\'s hungry mouth.  Once she can see again, the girl gives her tits a squeeze and affectionately kisses your cock, practically worshipping it with sloppy-sweet licks.' );
		...
		EngineCore.outputText( '.  [EachCock], instead of slamming into the bark painfully, slots into something warm and wet, pulsating and pliant.  A clingy, tight hold that ripples gently around your member' );
		if( CoC.getInstance().player.cockTotal() > 1 ) {
			EngineCore.outputText( 's, massaging the bundle of cocks with uncommon ferver' );
		} else {
			EngineCore.outputText( ', massing your tool with uncommon fervor' );
		}
		EngineCore.outputText( '.  Your grunt of irritation vanishes as soon as your tender sex is forced into the heavenly hole, and with the soft breasts pressing into ' );
		if( CoC.getInstance().player.biggestTitSize() >= 2 ) {
			EngineCore.outputText( 'your own' );
		} else {
			EngineCore.outputText( 'your [chest]' );
		}
		EngineCore.outputText( ', you give up and kiss the dryad full on the lips, biting the lower one roughly in irritation.' );
	...
	HolliScene.prototype.haveAMapleSyrupSnack = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.HOLLI_SUBMISSIVE ] === 0 ) {
			...
			EngineCore.outputText( 'eagerly, Holli asks, "<i>Care for a little pick-me-up?  They\'re getting very engorged, and I\'m sure it would be a refreshing snack for you.</i>"' );
		}
	...
	HolliScene.prototype.level4RideHollisTentacruels = function() {
		...
		EngineCore.outputText( '\n\nYou fold your arms impatiently across your [chest] and nod, waiting for her to get her phallic tendrils in position.  It takes you completely off guard when a half-dozen tentacles snap across your front and another bundle sweeps your [legs] out, spinning you to look face down at the ground, supported horizontally on a bed of taut, straining tentacles.  More loop over you, under you, and around you, cinching tight to your wrists and [feet].  They pull them out while other prehensile plants swing down to support you, occasionally looping around you, their moist skin easily gliding over your ' + CoC.getInstance().player.skinFurScales() + '.  All that, and she\'s not even fucking you yet!' );
		...
		if( EngineCore.silly() ) {
			EngineCore.outputText( '  She proceeds to sing about Jack Sparrow\'s awesome adventures for a solid ten minutes, then goes on as if nothing had happened.' );
		}
		...
		if( CoC.getInstance().player.lactationQ() === 0 ) {
			EngineCore.outputText( 'droplets of milk dripping from your nipples.  When did you start lactating?' );
		} else {
			EngineCore.outputText( 'steady streams of milk erupting from your nipples.  When did you start lactating this hard?' );
		}
		...
		if( !CoC.getInstance().player.isBiped() ) {
			EngineCore.outputText( 'you\'d be walking bowlegged for a week, if you had legs.' );
		} else {
			EngineCore.outputText( 'you\'ll be walking bowlegged for a week!' );
		}
	...
	HolliScene.prototype.eatHolliFruit = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.HOLLI_FRUIT ] === 1 ) {
			...
			EngineCore.outputText( 'one of the fruits' );
		}
		EngineCore.outputText( '.  ' );
		if( CoC.getInstance().flags[ kFLAGS.HOLLI_FRUIT_EXPLAINED ] === 0 ) {
			EngineCore.outputText( 'Holli smiles over at you and asks, "<i>Enjoying the fruits of our union?</i>"' );
			...
			CoC.getInstance().flags[ kFLAGS.HOLLI_FRUIT_EXPLAINED ]++;
		}
		EngineCore.outputText( 'Biting into it, sweet juices seem to explode from the flesh, dribbling down your chin.  It tastes like a dessert and you chow down, happily munching away.  In no time flat, you\'re down to just a core.  You toss it and wipe your [face] clean, then burp.  Damn, that was good!' );
		//TF CHANCES;
		if( Utils.rand( 2 ) === 0 && CoC.getInstance().player.cockTotal() > CoC.getInstance().player.tentacleCocks() ) {
			var choices = [];
			...
			EngineCore.outputText( ' tentacle-cock!</b>' );
			CoC.getInstance().player.cocks[ choice ].cockType = CockTypesEnum.TENTACLE;
			CoC.getInstance().player.cocks[ choice ].knotMultiplier = 1.3;
			EngineCore.dynStats( 'sen', 3, 'lus', 10 );
		}
		CoC.getInstance().flags[ kFLAGS.HOLLI_FRUIT ]--;
		EngineCore.dynStats( 'cor', 1 );
	...
	HolliScene.prototype.askHolliToWatch = function() {
		EngineCore.clearOutput();
		if( CoC.getInstance().flags[ kFLAGS.HOLLI_DEFENSE_ON ] === 0 ) {
			...
			if( CoC.getInstance().flags[ kFLAGS.THREATENED_HOLLI ] > 0 ) {
				EngineCore.outputText( '<b>  You could always just slap the cunt.</b>' );
			}
		...
		//Stop Guarding (edited)(C);
			...
			CoC.getInstance().flags[ kFLAGS.HOLLI_DEFENSE_ON ] = 0;
	...
	HolliScene.prototype.begHolli4Watches = function() {
		...
		CoC.getInstance().flags[ kFLAGS.HOLLI_DEFENSE_ON ] = 1;
		EngineCore.doNext( this.treeMenu );
	};
	...
	HolliScene.prototype.domUpSomeHolli = function() {
		...
		var domPowah = CoC.getInstance().player.level;
		domPowah += CoC.getInstance().player.tallness / 12;
		...
		//[Next] - Holli's Menu;
		CoC.getInstance().flags[ kFLAGS.HOLLI_SUBMISSIVE ] = 1;
		this.treeMenu( false );
	};
	//Guard Camp;
	HolliScene.prototype.askBrokenHolliToGuard = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.HOLLI_DEFENSE_ON ] === 1 ) {
			...
			EngineCore.outputText( 'sleeping, instead' );
		}
		EngineCore.outputText( '.</i>"' );
	...
	HolliScene.prototype.toggleBrokenHolliGuard = function() {
		...
		//Guard Off;
			...
			EngineCore.outputText( '"<i>All right, they\'re yours to smite or play with,</i>" Holli says with a wink.  You give her a glare, to which she responds with a shrug.  "<i>What?  I know how it is!  You mortals can\'t fight worth a damn when you\'re sleeping.</i>"' );
	...
	HolliScene.prototype.holliGetsDickDommed = function() {
		...
		if( CoC.getInstance().player.biggestCockArea() > 80 ) {
			EngineCore.outputText( ', jaw stretching obscenely' );
		}
		EngineCore.outputText( '.' );
		...
		if( CoC.getInstance().player.biggestCockArea() <= 80 ) {
			EngineCore.outputText( ', swallowed to the ' );
			if( CoC.getInstance().player.balls > 0 ) {
				EngineCore.outputText( '[balls]' );
			} else {
				EngineCore.outputText( '[sheath]' );
			...
			EngineCore.outputText( ', swallowed as deep as Holli\'s distended neck can handle.' );
		}
		EngineCore.outputText( '  Gurgling, the demonic dryad flutters her lengthy eyelashes and looks up at your eyes, twinkling with happiness at her servitude.  You wiggle your crotch around, and it twists your [cock biggest] around her mouth and throat.  The tainted tree\'s tongue massages the underside of your length, embracing you with subtle moisture and frictionless pressure.' );
		EngineCore.outputText( '\n\nBubbling sexual pleasure begins to build up inside you, making your ' );
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( '[sack] feel taut and ready to burst' );
		} else {
			EngineCore.outputText( 'body feel strung tight like an over-tuned guitar' );
		}
		EngineCore.outputText( '.  Holli pulls back, inciting a snarl of displeasure to vibrate from you, but as soon as she\'s withdrawn, her hands clasp onto the slick shaft and begin to pump it with vigorous strokes.  Her mouth settles back into place around your [cockHead biggest], circling it with her tongue while she bobs up and down on it, snaring your most sensitive place with overwhelming sensations.' );
		...
		if( CoC.getInstance().player.biggestCockArea() > 80 ) {
			EngineCore.outputText( '  It bends slightly as the immense girth is forced inside her, aiming down, deep inside the trunk.' );
		}
		EngineCore.outputText( '  Holli flops into your [chest], twitching weakly and breathing heavily.  "<i>Fffuck, so good!</i>" she pants, nuzzling her cheek into your collarbone while her hands play across your shoulders.  You ruthlessly mount her flowery mound with fast motions.  Juices squirt out around your member at the apex of every stroke, raining down over the surrounding ground.  You grab Holli\'s deep green hair and pull her back.  Her face is wrecked by bliss, her mouth and eyes moving in uncoordinated twitches.' );
		...
		if( CoC.getInstance().player.cockTotal() > 1 ) {
			EngineCore.outputText( '  Excess jism spurts onto Holli\'s jiggling udders from below, spotting the viridian mounds with white.' );
			if( CoC.getInstance().player.cumQ() >= 500 ) {
				EngineCore.outputText( '  The constant sprays turn them white in little time, dripping from the peaks of her nipples along with her sticky sap.' );
			}
		}
		if( CoC.getInstance().player.cumQ() > 1000 ) {
			EngineCore.outputText( '  A trickle of sperm washes out of the demoness\'s entrance, somehow escaping the heavy suction.' );
		}
		if( CoC.getInstance().player.cumQ() > 2000 ) {
			EngineCore.outputText( '  A moment later, that trickle turns into a river.' );
		}
		if( CoC.getInstance().player.cumQ() > 3000 ) {
			EngineCore.outputText( '  You swear that her trunk must be absolutely stuffed with spunk.' );
		}
		if( CoC.getInstance().player.cumQ() > 5000 ) {
			EngineCore.outputText( '  A spooge-spout sprays out as the pressure goes through the roof, confirming your suspicions in the messiest way possible.' );
		}
		EngineCore.outputText( '  Holli\'s tentacles cum with her, spraying her seed through her canopy in aimless ropes.  Most of it sticks to her branches and leaves, but small amounts of it trickle down to spatter the ground.  One of them lands on the back of your hand.  You frown and push it up against the tree-woman\'s mouth.  She licks you clean without question, still dazed from her orgasm.' );
	...
	HolliScene.prototype.fullOnTentacleTasticGangBangForHolli = function() {
		...
		if( EngineCore.silly() ) {
			EngineCore.outputText( ', inflatable-arm, flailing tube-cocks' );
		}
		EngineCore.outputText( '.  Below the orgy of arboreal affection, Holli\'s slick petals have spread wide.  You can actually see her interior twitching voraciously.  It opens and closes with pulsing hunger, vibrantly engorged.  As you watch, the fourth of your members lances forward of its own volition, spearing straight for that welcoming hole with no guidance from you.  It hits like a missile.  Two feet of that cock is gone in a moment, swallowed deep in the cunt-flower\'s bottomless depths.  A gush of girl-cum spurts out around the intruding member, and you can feel the muffled moan vibrate through the mouth-bound cock as she gives in to the four-way fucking.' );
		...
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( '  Your [balls] draw up tight to your groin and squeeze, rhythmically discharging their heavy loads.' );
		}
	...
	HolliScene.prototype.vaginalDomHollisTentacruels = function() {
		...
		if( !CoC.getInstance().player.hasFuckableNipples() && CoC.getInstance().player.lactationQ() < 50 ) {
			EngineCore.outputText( ' and pay special attention to your [nipples], tugging them in time with every sluggish thrust in your slit.' );
		} else if( CoC.getInstance().player.lactationQ() >= 50 ) {
			EngineCore.outputText( ' and sigh at the ' );
			if( CoC.getInstance().player.lactationQ() < 150 ) {
				EngineCore.outputText( 'squirts' );
			} else if( CoC.getInstance().player.lactationQ() < 800 ) {
				EngineCore.outputText( 'sprays' );
			} else if( CoC.getInstance().player.lactationQ() < 2000 ) {
				EngineCore.outputText( 'outpouring' );
			} else {
				EngineCore.outputText( 'thick fountains' );
			}
			EngineCore.outputText( ' of milk that escape with every tug, timed to match the sluggish thrusts at your slit.' );
		} else {
			EngineCore.outputText( ' and sink your fingers into your [nipples], fucking them to the sluggish tempo of the thrusting tentacle.' );
		}
		EngineCore.outputText( '  A loud groan escapes your mouth, and you rock your [hips] against the undulating tendril.  Your body\'s eagerness for more pleasure is unparalleled, but you control yourself as efficiently as you control the dryad.' );
		...
		if( CoC.getInstance().player.clitLength >= 3.5 ) {
			EngineCore.outputText( ' for a moment before wrapping around and jerking on it' );
		}
		EngineCore.outputText( '.' );
	...
	HolliScene.prototype.threatenHolli = function() {
		...
		if( CoC.getInstance().player.level < 30 ) {
			EngineCore.outputText( 'Mighty as you are, Marae is likely still beyond your power.  Perhaps you should just ignore this demon for the time being.' );
		} else {
			EngineCore.outputText( 'Marae is likely beyond your power yet, but... you feel like she might not pressure you too hard if you can make a suitably credible threat.' );
		}
		...
		if( (CoC.getInstance().player.gender > 0 && CoC.getInstance().player.findStatusAffect( StatusAffects.JojoNightWatch ) >= 0 && CoC.getInstance().player.findStatusAffect( StatusAffects.PureCampJojo ) >= 0) || CoC.getInstance().flags[ kFLAGS.ANEMONE_WATCH ] > 0 ) {
		...
		CoC.getInstance().flags[ kFLAGS.THREATENED_HOLLI ] = 1;
		EngineCore.doNext( CoC.getInstance().scenes.camp.returnToCampUseOneHour );
	};
	...
	HolliScene.prototype.cutHerDown = function() {
		...
		if( CoC.getInstance().player.weaponName === 'large axe' ) {
	...
	HolliScene.prototype.callDatJojo = function() {
		EngineCore.clearOutput();
		CoC.getInstance().player.removeKeyItem( 'Jojo\'s Talisman' );
		...
		CoC.getInstance().flags[ kFLAGS.FOUGHT_HOLLI ] = 1;
		Combat.startCombat( new Holli() );
		CoC.getInstance().monster.createStatusAffect( StatusAffects.JojoIsAssisting, 0, 0, 0, 0 );
	};
	...
	HolliScene.prototype.defeatHolli = function() {
		...
		CoC.getInstance().flags[ kFLAGS.FUCK_FLOWER_KILLED ] = 1;
		//Win without Jojo -Z;
		if( CoC.getInstance().monster.findStatusAffect( StatusAffects.JojoIsAssisting ) < 0 ) {
			EngineCore.outputText( 'Weary, the succubus godling folds into her tree, unwilling to allow you the sight of her defeat.  With her energy suppressed, you\'re free to ' );
			if( CoC.getInstance().player.weaponName !== 'large axe' ) {
				EngineCore.outputText( 'fetch an axe and ' );
			}
			EngineCore.outputText( 'hack at her trunk, laboriously peeling bark and making wedge-shaped cuts in it as you try to fell the abomination.  It takes nearly eight hours, but eventually the gnarled tree topples.  No sooner does it fall than it shrivels and turns to ash.' );
			if( CoC.getInstance().time.hours + 7 >= 24 ) {
				CoC.getInstance().time.days++;
			}
			CoC.getInstance().time.hours = (CoC.getInstance().time.hours + 7) % 24;
			EngineCore.statScreenRefresh();
		}
		//Win with Jojo -Z;
		else {
			EngineCore.outputText( 'The fire-blackened mouse cheers at you as the succubus groans and retreats completely into her bark.  "<i>Well done, [name]!  Help me carry tinder and we can burn this abomination to the ground!</i>"' );
			...
			if( CoC.getInstance().time.hours + 3 >= 24 ) {
				CoC.getInstance().time.days++;
			}
			CoC.getInstance().time.hours = (CoC.getInstance().time.hours + 3) % 24;
			EngineCore.statScreenRefresh();
			//[(if no Vapula)];
		...
		CoC.getInstance().player.createKeyItem( 'Holli\'s Ashes', 0, 0, 0, 0 );
	...
	HolliScene.prototype.enjoyYourBadEndBIYAAAATCH = function() {
		...
		EngineCore.outputText( '"<i>Oh, my little meat...</i>" Holli coos dangerously.  "<i>Was I not clear enough before?  You exist to worship me.  If you won\'t give your mind to me, I\'ll have your body, instead.</i>"  A floral tendril descends from her upper branches, slipping into your mouth and beginning to drool a nectar down your throat.  Forced to drink or drown, you gulp unhappily, feeling increasingly light-headed with each swallow.' );
		//FOOOOARK - choose with RNG for herms;
		//branch of penis-having;
		if( CoC.getInstance().player.gender === 1 || (Utils.rand( 2 ) === 0 && CoC.getInstance().player.hasCock()) ) {
			EngineCore.outputText( '\n\nShe strips off your [armor] haphazardly, rending and shredding with vicious tugs and leaving deep lines where the clasps bite into your ' + CoC.getInstance().player.skinFurScales() + ' before parting.  [OneCock] exposed, she lowers her mouth to the [cockHead biggest] and' );
			if( CoC.getInstance().player.cocks[ CoC.getInstance().player.biggestCockIndex() ].cockThickness > 6 ) {
				EngineCore.outputText( ', stretching her jaw wide open in a way that makes you wince,' );
			}
			EngineCore.outputText( ' engulfs you.  Holli\'s tongue plays over your slit and she looks up at you for just a moment to be sure you\'re watching.' );
			...
			if( CoC.getInstance().player.biggestCockLength() > 24 ) {
				EngineCore.outputText( 'her neck ripples and extends grotesquely, and her head begins to migrate down all ' + Utils.num2Text( Math.round( CoC.getInstance().player.biggestCockLength() ) ) + ' of your prick with alarming speed, pushing the stem inside along with it.  ' );
			} else {
				EngineCore.outputText( 'she sinks her head down to the base of your prick in one quick thrust, bringing the stem with it.  ' );
			}
			EngineCore.outputText( 'The sensation of having your entire [cock biggest] penetrated and sucked into her hot, wet, clinging throat pushes you back toward the edge and over, and it literally becomes a race for Holli to reach bottom before your nerves fire.  Her wet lips impact your crotch with a lewd splatter, but her tongue keeps going deep into your body, stopping only once it reaches your prostate.  A small bulge fires down the thin stem, forcing through your slit and into the shaft to lodge in your gland.  Holli\'s mouth and tongue withdraw once it is in place, and she grabs your cock in her hand, masturbating you vigorously.' );
			EngineCore.outputText( '\n\nIt\'s too much; your climax comes in a flood' );
			if( CoC.getInstance().player.balls > 0 ) {
				EngineCore.outputText( ', and your [balls] draw close to your body to unload' );
			}
			EngineCore.outputText( '.  Your eyes roll as you flex and flex, but nothing comes out.  As you conclude your dry orgasm, you begin to feel a pressure on your prostate.' );
			...
			if( CoC.getInstance().player.balls > 0 ) {
				EngineCore.outputText( 'migrating the other direction into your [balls], entangling and infiltrating them, filling them with the same warmth, then pressing out against your sack.  ' );
			} else {
				EngineCore.outputText( 'exploring the depths of your body and curling back against the surface of your ' + CoC.getInstance().player.skin() + '.  ' );
			}
			EngineCore.outputText( 'Your stomach turns as the sprouts push their way through, crawling from your crotch down the outside of your [legs] to root in the dry earth.  The shoots thicken into fibrous roots, forming a trunk that supports you and pushes you closer to Holli and lower to the ground, until your writhing, tentacle-tipped prick is lined up with her pussy flower and within her easy reach.' );
		...
		//branch of vagina-having;
			...
			if( CoC.getInstance().player.isTaur() ) {
				EngineCore.outputText( ' and turning you around' );
			}
			EngineCore.outputText( ', the demon buries her face in your [vagina], penetrating it with her wet, flexible tongue.' );
			...
			if( CoC.getInstance().player.hasCock() ) {
				EngineCore.outputText( '  [OneCock] drools precum down its length and onto the flower, triggering another spasm and another gush from your prick.  Holli laughs as you continue to self-stimulate with your uncontrollable fluids.' );
			}
			...
			if( CoC.getInstance().player.biggestTitSize() > 1 ) {
				EngineCore.outputText( 'wrapping around and under your breasts to keep them permanently jutting out in a lurid display' );
			} else {
				EngineCore.outputText( 'tracing a lattice of slender, criss-crossing ivy over your chest that hangs just far enough over your [nipples] that a stiff breeze will drag it into them' );
			}
			EngineCore.outputText( ', then continuing over your back and up your neck.  The tendrils thicken and multiply, occluding your forearms and lower [legs] in a casing of young bark and leaf buds, and Holli\'s own release you as yours take over the duty of holding you in place to receive whatever pleasure she forces upon you.  You\'re left in a salacious pose when the growth subsides, suspended several feet off the ground with your arms held over your head, your back arched in a memorial of your incredible orgasm, ' );
			if( CoC.getInstance().player.biggestTitSize() > 1 ) {
				EngineCore.outputText( 'your breasts cupped and pressed together by vines, ' );
			}
			if( !CoC.getInstance().player.isNaga() && !CoC.getInstance().player.isGoo() ) {
				EngineCore.outputText( 'your thighs spread wide open, ' );
			}
			EngineCore.outputText( 'and your nectar-drooling flower-sex exposed with your [clit]' );
			if( CoC.getInstance().player.hasCock() ) {
				EngineCore.outputText( ' and [eachCock]' );
			}
			EngineCore.outputText( ' bobbing in a state of permanent semi-arousal.' );
			...
			if( CoC.getInstance().player.ass.analWetness > 0 ) {
				EngineCore.outputText( 'Betraying your depravity, the slick crack opens wide and your greedy, wet asshole practically welcomes the demon\'s entire hand, sucking it up and making you shiver with repressed delight.  "<i>Oh, mother!  It seems as though this is not a new lesson to you, is it?  My little whore, will you beg and moan as I fill you?</i>"' );
			} else if( CoC.getInstance().player.analCapacity() < 30 ) {
				EngineCore.outputText( 'With some difficulty, she forces it into your pucker.  "<i>I think we\'ll just use this as our inroads, instead.  It may be slow going, but the journey is half the fun.</i>"' );
			} else {
				EngineCore.outputText( 'Despite your clenching, your loose hole proves no impediment to the demon\'s exploration.  "<i>Ah, how roomy you are!  I think I\'ll move right in.</i>"' );
			}
			...
			//[(iff buttslut);
			if( CoC.getInstance().player.ass.analWetness > 0 ) {
				EngineCore.outputText( 'Try as you might, you can\'t see far enough down your back to determine what the demon has done.  The question answers itself, though, when she presses a shaft almost as thick as her leg into your [butt], nosing it into your wet, receptive asshole.  Despite yourself, you shiver in anticipation at being filled so completely.  ' );
			} else if( CoC.getInstance().player.analCapacity() < 30 ) {
				EngineCore.outputText( 'Your jaw hits your collarbone as Holli angles the florid head of a long, narrow penis into your field of view.  Judging by how far it comes up her chest, it\'s almost a foot and half long, and it\'s drooling sticky sap all over your back.  With a twisted smile, the demon pushes it along your spine until it reaches your [butt], then shoves it toward the hole.  ' );
			} else {
				EngineCore.outputText( 'The demon presses up to your back, brushing her nipples against your skin and shoving her new growth between your thighs.  Looking down, you can see a fat, green shaft sticking from your crotch, glistening and dripping with sticky slime.  With another moan, Holli pulls it back from between your legs, then pushes apart your asscheeks and plunges it into your [asshole].  ' );
			}
			EngineCore.outputText( 'The sap oozing from her member coats your passage, ' );
			if( CoC.getInstance().player.ass.analWetness > 0 ) {
				EngineCore.outputText( 'mingling with your own lubrication and ]easing her thrusts.  Holli\'s tempo picks up quickly, and soon you can feel her twitch inside you as she nears her climax.' );
			}
			...
			//[(buttslut or loose);
			if( CoC.getInstance().player.ass.analWetness > 0 || CoC.getInstance().player.analCapacity() > 30 ) {
				EngineCore.outputText( 'spilling a wave of her deposited fluids down your [legs].  ' );
			} else {
				EngineCore.outputText( 'and your once-tight pucker reflexively squeezes shut as best it can, trapping all but a few trickles of Holli\'s deposit inside you.  ' );
			}
			EngineCore.outputText( 'The freshly-grown shaft between Holli\'s legs begins to shrivel, withering and browning like a cut flower.  With an expression of mild disdain, she pulls it from her body and casts it aside, where it crumbles to pieces.' );
			EngineCore.outputText( '\n\nThe fluids in your gut begin to mingle, sending a disquieting pinprick sensation through the skin' );
			if( CoC.getInstance().player.skinType === AppearanceDefs.SKIN_TYPE_FUR || CoC.getInstance().player.skinType === AppearanceDefs.SKIN_TYPE_SCALES ) {
				EngineCore.outputText( ' under your ' + CoC.getInstance().player.skinFurScales() );
			}
			EngineCore.outputText( '.  Panic intensifies as the sensation quickly focuses on your crotch, and when the tendrils holding you push you away from the demon\'s tree, your curiosity gets the better of you.  You look over your shoulder again, trying to see what\'s happening.  As you strain to watch, you feel hundreds of tiny, sharp pinches and small vines begin to creep from your body, just below your ass.  The vines merge and thread into thick trunks as they get closer to the ground, pushing deep into the dirt and mooring you to the spot.  Arousal builds in you as they join the web of roots linking the demon Marae and Holli, as does a swelling at your crotch.  Another cluster of thin vines push free from your groin, growing upward in a lattice and lashing your arms into place, ' );
			if( CoC.getInstance().player.biggestTitSize() < 1 ) {
				EngineCore.outputText( 'folded atop your chest' );
			} else {
				EngineCore.outputText( 'crooked at your sides with your hands resting atop your [chest]' );
			}
			EngineCore.outputText( '.  They continue past your face, reaching over your head to form a canopy similar to Holli\'s own.  Struggle as you may, when they\'re done growing you\'re left unable to budge and locked in a rather artful pose, head turned over your shoulder with your [ass] thrust out' );
			if( CoC.getInstance().player.biggestTitSize() >= 1 ) {
				EngineCore.outputText( ', your breasts exposed from the sides,' );
			}
			EngineCore.outputText( ' and your [legs] concealed below your buttcheeks by a thick tree trunk.' );
	...
	HolliScene.prototype.holliPlaysWithPenisesBadEnd = function() {
		...
		if( CoC.getInstance().player.biggestTitSize() < 1 ) {
			EngineCore.outputText( 'masculine shape, it bears' );
		} else {
			EngineCore.outputText( 'feminine shape, it nonetheless bears' );
		}
		EngineCore.outputText( Descriptors.multiCockDescript() + ', standing erect and hard as iron; the brown-haired girl blushes and looks away.' );
		...
		EngineCore.outputText( '\n\nThe dryad gestures once again to her companion\'s ' + Descriptors.multiCockDescriptLight() + ', causing the brown-haired girl\'s mouth to hang open a little.  "<i>I can think of few better ways to demonstrate your vitality,</i>" Holli coos.  "<i>Do not feel indebted to us, but if you would share of yourself, we would be grateful.  And, in truth, is it not a hint of longing I see in your eyes?  Come, join with ' + CoC.getInstance().player.mf( 'him', 'her' ) + ' for a while and clear your head.</i>"' );
		if( CoC.getInstance().player.smallestCockArea() > 36 ) {
			EngineCore.outputText( '  As the dryad speaks, the silent figure\'s [cock smallest] shudders and begins to shrink, dwindling to a size that would fit ' + CoC.getInstance().player.mf( 'him', 'her' ) + ' prospective lover.' );
		}
		...
		EngineCore.outputText( '\n\nBy the time the girl reaches her third orgasm, her partners are tense and near the breaking point; with an shudder and a stilling of the shaking branches, the silent one opens ' + CoC.getInstance().player.mf( 'his', 'her' ) + ' mouth in mute ecstasy and ejaculates, ' );
		if( CoC.getInstance().player.cocks.length > 1 ) {
			EngineCore.outputText( 'spraying the girl\'s stomach and tits with waste seed from ' + CoC.getInstance().player.mf( 'his', 'her' ) + ' unused manhood and ' );
		}
		EngineCore.outputText( ' filling the Champion\'s pussy with so much semen it cascades from her stretched hole as Holli continues to pump.' );
		EngineCore.outputText( '\n\nThe dryad herself soon gives a cry and pushes her tits into the brown-haired girl\'s back as her climax comes.  A bulge ascends the green shaft, disappearing into the beat-up cunt of the Champion, and a trickle of golden sap leaks back down Holli\'s cock.  Pulling free, the dryad wipes her member off and folds it back into its bud; it shrinks and dwindles until nothing is left but her clitoris.  The silent one\'s branches lower the girl to the ground, ' + CoC.getInstance().player.mf( 'his', 'her' ) + ' always-erect prick' );
		if( CoC.getInstance().player.cockTotal() > 1 ) {
			EngineCore.outputText( 's' );
		}
		EngineCore.outputText( ' drooling aftershocks onto her breasts.' );
	...
	HolliScene.prototype.girlsGetANiceManToBadEnd = function() {
		...
		//[(if tits);
		if( CoC.getInstance().player.biggestTitSize() > 1 ) {
			EngineCore.outputText( '[chest] cupped by vines and held out toward the young man, ' );
		} else {
			EngineCore.outputText( 'chest crisscrossed with a mesh of fine vines, ' );
		}
		EngineCore.outputText( 'its legs elevated and spread apart to display a ' );
		if( CoC.getInstance().player.balls > 0 && CoC.getInstance().player.ballSize > 10 && CoC.getInstance().player.gender === 3 ) {
			EngineCore.outputText( Descriptors.multiCockDescriptLight() + ' with [balls] swinging below.' );
		}
		//(if H and ballsize < 10);
		else if( CoC.getInstance().player.balls > 0 && CoC.getInstance().player.ballSize <= 10 && CoC.getInstance().player.gender === 3 ) {
			EngineCore.outputText( Descriptors.multiCockDescriptLight() + ' and [balls] that hang slightly over a [vagina].' );
		}
		//(if H and noballs);
		else if( CoC.getInstance().player.gender === 3 ) {
			EngineCore.outputText( Descriptors.multiCockDescriptLight() + ' and a [vagina].' );
		} else {
			EngineCore.outputText( 'a wet, drooling cunt that practically begs to be filled.' );
		}
		EngineCore.outputText( '  Though its eyes are covered, the figure\'s mouth works constantly, biting and licking its lips as if beset by otherworldly amounts of lust.  Holli beckons the youth toward this tree.' );
		...
		if( CoC.getInstance().player.ballSize > 10 && CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( 'lifting the hefty testicles to reveal a wet, eager pussy.' );
		} else {
			EngineCore.outputText( 'spreading the labia and exposing the eager, clenching hole inside.' );
		}
		...
		if( CoC.getInstance().player.biggestTitSize() >= 1 ) {
			EngineCore.outputText( 'reaches up and grabs two handfuls of breast, passing his thumb over a nipple and watching the ' );
			if( CoC.getInstance().player.gender === 2 ) {
				EngineCore.outputText( 'woman' );
			} else {
				EngineCore.outputText( 'hermaphrodite' );
			}
			EngineCore.outputText( '\'s mouth open and close in bliss.  ' );
		} else {
			EngineCore.outputText( 'leans down and kisses the [woman/hermaphrodite]\'s smooth chest, causing her to arch her back in pleasure.  ' );
		}
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( 'With a nervous glance downward, he slides one hand to her [cock smallest].  "<i>It\'s not gay if it\'s a chick\'s,</i>" he mumbles to himself, and then begins to stroke haltingly, playing his finger along the urethra and drawing a squirt of precum from his shivering partner.  ' );
		} else {
			EngineCore.outputText( 'His hand slides to the [clit] above her wet fuck-hole; collecting some of the frothing lube from their wet sex, ' );
			if( CoC.getInstance().player.clitLength < 6 ) {
				EngineCore.outputText( 'he smears it over the little button and then begins to trace up and down the underside with a finger.  ' );
			} else {
				EngineCore.outputText( 'he coats the swollen organ with it and begins to jerk it off valiantly, like a cock.  ' );
			}
			EngineCore.outputText( '"<i>Does that feel good?</i>" he asks, upping the pace of his thrusts.  ' );
		}
		EngineCore.outputText( 'As if in reply, the ' );
		if( CoC.getInstance().player.gender === 2 ) {
			EngineCore.outputText( 'woman' );
		} else {
			EngineCore.outputText( 'hermaphrodite' );
		}
		EngineCore.outputText( ' in the tree arches her back and her [vagina] clamps down on his tool' );
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( '; ' );
			if( CoC.getInstance().player.balls > 0 ) {
				EngineCore.outputText( 'her balls draw closer to her body and ' );
			}
			EngineCore.outputText( 'her trembling dick' );
			if( CoC.getInstance().player.cockTotal() === 1 ) {
				EngineCore.outputText( ' throbs' );
			} else {
				EngineCore.outputText( 's throb' );
			}
			EngineCore.outputText( ' as she nears climax' );
		}
		EngineCore.outputText( '.' );
		//[(cock)];
		if( CoC.getInstance().player.hasCock() ) {
			...
			EngineCore.outputText( ' toward her face; ' );
			if( CoC.getInstance().player.biggestCockArea() < 20 ) {
				EngineCore.outputText( 'semen spurts and dribbles down her [chest], pooling in the combined laps and mingling with the sticky lube ringing her vagina.  ' );
				if( CoC.getInstance().player.cumQ() >= 500 ) {
					EngineCore.outputText( 'So much so that by the time she finishes her climax, both her partner\'s legs and her trunk are covered in a sheen of milky white seed.  ' );
				}
			} else if( CoC.getInstance().player.biggestCockArea() < 80 ) {
				EngineCore.outputText( 'her mouth is now full of cockhead, and she gulps as eagerly as she spurts, drooling lines of white from the corners of her lips.  ' );
				//[(big skeet)];
				if( CoC.getInstance().player.cumQ() >= 500 ) {
					EngineCore.outputText( 'The waves of seed don\'t stop coming, pouring into her and swelling her belly out until its round bulge pushes the cock away from her mouth and the remaining squirts fall on her [chest].  ' );
				}
			}
			//else cock biggest > 80);
			else {
				EngineCore.outputText( 'her body is easily dwarfed by her male sex\'s massive size, and globs of sperm shoot into her canopy to splatter among her leaves and branches.  ' );
				if( CoC.getInstance().player.cumQ() >= 1000 ) {
					EngineCore.outputText( 'The overproductive hermaphrodite\'s body pushes out so much seed that the entire canopy soon turns white and drips constantly as though it were in a tropical rainforest.  ' );
				}
			}
		}
		//(vagina and no cock);
			...
			if( CoC.getInstance().player.wetness() < 3 ) {
				...
				EngineCore.outputText( 'rush' );
			} else {
				EngineCore.outputText( 'torrent' );
			}
			EngineCore.outputText( ' of female orgasm pours from her pussy, and her mouth opens wide in pleasure.  The tree branches surrounding the young man seem to close in, pushing him toward her.  Getting the message, he leans in and kisses his lover; her tongue probes his mouth with abandon as her body shakes out its fit and her [vagina] wrings his prick.  ' );
		}
		EngineCore.outputText( 'Pushed past his limit by her lewd display, the young man groans, shudders, and begins to unload his pent-up cargo.  Falling on top of his partner\'s slick ' );
		if( CoC.getInstance().player.cockTotal() === 1 ) {
		...
		EngineCore.outputText( ', he goes limp and slack-jawed as his dick squirts, filling her vagina.' );
		...
		EngineCore.outputText( '\'s stomach.  "<i>In fact, she seems to have enjoyed it so much that she would have me bestow a blessing on you.</i>"' );
	...
	HolliScene.prototype.holliAndGenderlessSittingInATree = function() {
		...
		if( CoC.getInstance().player.skinType !== AppearanceDefs.SKIN_TYPE_FUR && CoC.getInstance().player.skinType !== AppearanceDefs.SKIN_TYPE_SCALES ) {
			EngineCore.outputText( CoC.getInstance().player.skinTone + '-colored' );
		} else {
			EngineCore.outputText( 'covered in ' + CoC.getInstance().player.skinFurScales() );
		}
		EngineCore.outputText( ' and wrapping its body around the tree in a way that makes determining its sex impossible.  No... it\'s not wrapping around the tree, but embedded in it up to the hips!' );
		...
		if( CoC.getInstance().player.skinType === AppearanceDefs.SKIN_TYPE_SCALES ) {
			EngineCore.outputText( 'a quiver of arousal shakes its scales' );
		} else if( CoC.getInstance().player.skinType === AppearanceDefs.SKIN_TYPE_FUR ) {
			EngineCore.outputText( 'its fur begins to prick up with arousal' );
		} else {
			EngineCore.outputText( 'a blush of lust colors its ' + CoC.getInstance().player.skinFurScales() );
		}
		EngineCore.outputText( '...' );
	...
	CoC.getInstance().registerScene( 'holliScene', new HolliScene() );
} );