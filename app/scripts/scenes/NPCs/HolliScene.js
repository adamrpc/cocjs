...
angular.module( 'cocjs' ).run( function( MainView, SceneLib, Holli, PerkLib, $log, CockTypesEnum, PregnancyStore, Combat, Descriptors, AppearanceDefs, CoC, kFLAGS, Utils, StatusAffects, EngineCore ) {
	...
	HolliScene.prototype.fuckHolliInZeFlowerPuss = function() {
		...
		EngineCore.outputText( ' ' + CoC.player.mf( 'boy', 'girl' ) + ',</i>" the arboreal tramp suggests with a husky coo, "<i>I got you some sugar.</i>"  She pinches her nipples and gasps, dark streams of sugary juices dripping wetly into the soil as she beckons you closer.' );
		...
		if( CoC.player.cockArea( x ) < 100 ) {
			EngineCore.outputText( 'leans down and kisses the tip, opening her glossy green puckers to slurp it down, sucking just hard enough to make your ' + CoC.player.cockHead( x ) + ' tingle with erotic warmth.' );
		} else {
	...
	HolliScene.prototype.enjoyYourBadEndBIYAAAATCH = function() {
		...
		if( CoC.player.gender === 1 || (Utils.rand( 2 ) === 0 && CoC.player.hasCock()) ) {
			...
			if( CoC.player.biggestCockLength() > 24 ) {
				EngineCore.outputText( 'her neck ripples and extends grotesquely, and her head begins to migrate down all ' + Utils.num2Text( Math.round( CoC.player.biggestCockLength() ) ) + ' of your prick with alarming speed, pushing the stem inside along with it.  ' );
			} else {
			...
			if( CoC.player.balls > 0 ) {
				...
				EngineCore.outputText( 'exploring the depths of your body and curling back against the surface of your ' + CoC.player.skin() + '.  ' );
			}
		...
		//branch of vagina-having;
			...
			if( CoC.player.skinType === AppearanceDefs.SKIN_TYPE_FUR || CoC.player.skinType === AppearanceDefs.SKIN_TYPE_SCALES ) {
				EngineCore.outputText( ' under your ' + CoC.player.skinFurScales() );
			}
	...
	HolliScene.prototype.holliPlaysWithPenisesBadEnd = function() {
		...
		if( CoC.player.smallestCockArea() > 36 ) {
			EngineCore.outputText( '  As the dryad speaks, the silent figure\'s [cock smallest] shudders and begins to shrink, dwindling to a size that would fit ' + CoC.player.mf( 'him', 'her' ) + ' prospective lover.' );
		}
		...
		if( CoC.player.cocks.length > 1 ) {
			EngineCore.outputText( 'spraying the girl\'s stomach and tits with waste seed from ' + CoC.player.mf( 'his', 'her' ) + ' unused manhood and ' );
		}
	...
	HolliScene.prototype.holliAndGenderlessSittingInATree = function() {
		...
		if( CoC.player.skinType !== AppearanceDefs.SKIN_TYPE_FUR && CoC.player.skinType !== AppearanceDefs.SKIN_TYPE_SCALES ) {
			EngineCore.outputText( CoC.player.skinTone + '-colored' );
		} else {
			EngineCore.outputText( 'covered in ' + CoC.player.skinFurScales() );
		}
		...
		if( CoC.player.skinType === AppearanceDefs.SKIN_TYPE_SCALES ) {
			...
			EngineCore.outputText( 'a blush of lust colors its ' + CoC.player.skinFurScales() );
		}
		EngineCore.outputText( '...' );
