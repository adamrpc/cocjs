...
angular.module( 'cocjs' ).run( function( $rootScope, GooArmor, ArmorLib, PregnancyStore, Combat, Descriptors, AppearanceDefs, EventParser, CoC, kFLAGS, Utils, StatusAffects, EngineCore ) {
	...
	Valeria.prototype.valeriaAppearance = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.VELARIA_FUTA ] === 1 ) {
	...
	Valeria.prototype.pcWinsValeriaSparDefeat = function() {
		...
		if( CoC.getInstance().player.HP < 1 || CoC.getInstance().player.gender === 0 ) {
	...
	Valeria.prototype.valeriaDickToggle = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.VELARIA_FUTA ] === 1 ) {
			...
			CoC.getInstance().flags[ kFLAGS.VELARIA_FUTA ] = 0;
		}
		//[If Valeria doesn't have a dick:] ;
		else {
			CoC.getInstance().flags[ kFLAGS.VELARIA_FUTA ] = 1;
	...
	Valeria.prototype.valeriaGetFucked = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.VELARIA_FUTA ] === 0 ) {
	...
	Valeria.prototype.valeriaSexDominated = function() {
		EngineCore.spriteSelect( 79 );
		EngineCore.clearOutput();
		EngineCore.outputText( 'Making a show of playing hesitant and nervous, you tell Valeria you\'d like her to take charge.  A wide grin quickly spreads across her girlish features.  "<i>Mmm, feeling subby today, partner?  Good, good... You just let Valeria take good care of you, ' + CoC.getInstance().player.mf( 'handsome', 'cutie' ) + '.</i>"' );
		...
		if( CoC.getInstance().player.hasVagina() && (!CoC.getInstance().player.hasCock() || Utils.rand( 2 ) === 0) ) {
			...
			EngineCore.outputText( '\n\n"<i>Oh, you like that, do you?</i>" the armor-goo asks, smiling evilly.  "<i>Hmm, maybe if you\'re a good ' + CoC.getInstance().player.mf( 'boy', 'girl' ) + ', I\'ll let you get off, too</i>"  Still grinning, she begins to hammer her cock-like appendage into your pussy, fucking you fast and hard with her goo-dildo.' );
			...
			EngineCore.outputText( CoC.getInstance().player.mf( 'boy', 'girl' ) + ' ready to cum?</i>" she withdraws herself from you, leaving you feeling suddenly very, very empty.  "<i>I didn\'t say you could cum yet, slut! I\'m not done with you yet.</i>"' );
			...
			if( CoC.getInstance().player.vaginalCapacity() > 80 || CoC.getInstance().player.analCapacity() > 80 ) {
		...
		//(PC has Dick);
			...
			EngineCore.outputText( '\n\nYou gasp with pleasure as she starts to stroke [eachCock], jerking you off as she looms over you, grinning wickedly.  "<i>Oh, you like that do you?</i>" the armor-goo asks.  "<i>Well then, I might just let you get off, too... If you\'re a good ' + CoC.getInstance().player.mf( 'boy', 'girl' ) + '</i>"  She starts to increase her tempo, making you squirm and writhe as she wanks your ' + Descriptors.multiCockDescriptLight() + ', licking her lips as little bubbles of pre start to form.' );
			...
			EngineCore.outputText( ' ' + CoC.getInstance().player.mf( 'boy', 'girl' ) + ' ready to cum? Well, go on then - we\'ve got time for more.</i>"' );
			EngineCore.outputText( '\n\nYou cum, ropes of thick, white jizz shooting out of your cock and into the goo\'s waiting body.  Laughing, the goo-girl absorbs your cum, growing larger and larger as you feed her until she towers over you, her expanding breasts and belly now hanging over you.  "<i>Oh, that\'s good. Good, ' + CoC.getInstance().player.mf( 'boy', 'girl' ) + ', good.  Yes, let it all out, just like that... just like that,</i>" she coos, soaking your cum up until your orgasm finally passes.' );
			...
			EngineCore.outputText( '\n\n"<i>Oh, that\'s good... good, ' + CoC.getInstance().player.mf( 'boy', 'girl' ) + ', good.  Yes, let it all out, just like that... just like that,</i>" she moans, soaking your juices up until your orgasm finally passes.  Sated, she withdraws around your foot, leaving you a quivering mess on the ground.' );
	...
	Valeria.prototype.takeValeria = function() {
		...
		CoC.getInstance().player.armor.removeText();
		var item = CoC.getInstance().player.setArmor( ArmorLib.GOOARMR ); //Item is now the player's old armor
		if( item === null ) {
			EngineCore.doNext( EventParser.playerMenu );
		} else {
	...
	Valeria.prototype.valeriaAndGooThreeStuff = function() {
		...
		EngineCore.outputText( '\n\nYou splash ' + CoC.getInstance().player.face() + '-first into a cushiony mass of forgiving slime, but the momentum only carries you partway into the humanoid puddle. A semi-solid lump bumps off your cheek as you come to rest, floating inside a ' + CoC.getInstance().monster.gooColor() + ' prison. Shaking your head to clear your vision, you try to determine what you hit, but all you can see is a filmy blur, and you can\'t breathe either. Your cheeks puff out while you struggle to surface. As always, Valeria has your back. Her unsubtle drawl vibrates, "<i>I gotcha, partner,</i>" wetly against your eardrums as you are bodily rotated, slowly pushing your head through the ' + CoC.getInstance().monster.gooColor4() + ' surface while your body is thoroughly ensconced in slime.' );
		...
		EngineCore.outputText( '\n\nThe bluish' );
		if( CoC.getInstance().monster.skinTone !== 'blue' ) {
			EngineCore.outputText( ' ' + CoC.getInstance().monster.gooColor6() );
		}
	...
	Valeria.prototype.valeriaGooRapeII = function() {
		...
		CoC.getInstance().player.buttChange( CoC.getInstance().player.analCapacity() * 0.75, true, true, false );
		//Lay pipes in cooch! {reqiores non pregnant};
		if( !CoC.getInstance().player.isPregnant() && CoC.getInstance().player.hasVagina() ) {
			if( EngineCore.silly() ) {
			...
			//{cuntChange: MAXIMUM};
			CoC.getInstance().player.cuntChange( CoC.getInstance().player.vaginalCapacity() * 0.75, true, true, false );
		...
		//Fuckable nipples;
		if( CoC.getInstance().player.hasFuckableNipples() && CoC.getInstance().player.biggestTitSize() >= 5 ) {
			...
			EngineCore.outputText( '. Valeria takes this as an invitation, and ' + Utils.num2Text( CoC.getInstance().player.totalNipples() ) + ' phallic objects greedily push at your cunt-nipples, eagerly sliding themselves inside your welcome chest-pussies. Looking down, you can see the navy-blue outlines of the phallic tendrils waving and writhing as they push their way deep, embedding themselves tit-deep in blissful warmth.' );
		}
		//Dick;
		if( CoC.getInstance().player.hasCock() ) {
		...
		if( CoC.getInstance().player.biggestTitSize() >= 5 && CoC.getInstance().player.hasFuckableNipples() ) {
		...
		if( CoC.getInstance().player.biggestTitSize() >= 5 && CoC.getInstance().player.hasFuckableNipples() ) {
			for( var x = 0; x < CoC.getInstance().player.bRows(); x++ ) {
				CoC.getInstance().player.breastRows[ x ].breastRating += 3 + Utils.rand( 3 );
			}
			EngineCore.outputText( ' Your tits have grown much larger, ' + CoC.getInstance().player.breastCup( 0 ) + '-cups at least.' );
		}
		if( CoC.getInstance().player.hasCock() && CoC.getInstance().player.balls > 0 ) {
			CoC.getInstance().player.ballSize += 3 + Utils.rand( 2 );
		...
		CoC.getInstance().flags[ kFLAGS.TIMES_VALERIA_GOO_THREESOMED ]++;
		CoC.getInstance().player.orgasm();
		//v1 = time till birth.;
		//v2 = cock fill = 1, balls fill = 2;
		//v3 = cunt fill?;
		//v4 = tit fill?;
		CoC.getInstance().player.createStatusAffect( StatusAffects.GooStuffed, 10 + Utils.rand( 300 ), 0, 0, 0 );
		CoC.getInstance().player.buttKnockUpForce( PregnancyStore.PREGNANCY_GOO_STUFFED, 500 ); //Blocks other pregnancies - Way higher than GooStuffed status can last. Cleared when GooStuffed removed
		if( CoC.getInstance().player.hasVagina() ) {
			CoC.getInstance().player.changeStatusValue( StatusAffects.GooStuffed, 3, 1 );
			CoC.getInstance().player.knockUpForce( PregnancyStore.PREGNANCY_GOO_STUFFED, 500 ); //Blocks other pregnancies - Way higher than GooStuffed status can last. Cleared when GooStuffed removed
		}
		if( CoC.getInstance().player.hasCock() ) {
			if( CoC.getInstance().player.balls > 0 ) {
				CoC.getInstance().player.changeStatusValue( StatusAffects.GooStuffed, 2, 2 );
			} else {
				CoC.getInstance().player.changeStatusValue( StatusAffects.GooStuffed, 2, 1 );
			}
		}
		if( CoC.getInstance().player.biggestTitSize() >= 5 && CoC.getInstance().player.hasFuckableNipples() ) {
			CoC.getInstance().player.changeStatusValue( StatusAffects.GooStuffed, 4, 1 );
		}
		Combat.cleanupAfterCombat();
	};
	...
	Valeria.prototype.birthOutDatGooSlut = function() {
		...
		if( CoC.getInstance().player.statusAffectv4( StatusAffects.GooStuffed ) > 0 ) {
		...
		if( CoC.getInstance().player.hasCock() && CoC.getInstance().player.statusAffectv2( StatusAffects.GooStuffed ) > 0 ) {
			...
			if( CoC.getInstance().player.statusAffectv2( StatusAffects.GooStuffed ) >= 2 ) {
				EngineCore.outputText( '  Your balls keep their enhanced sized. Odd.' );
			}
		}
		if( CoC.getInstance().player.hasVagina() && CoC.getInstance().player.statusAffectv3( StatusAffects.GooStuffed ) > 0 ) {
		...
		CoC.getInstance().player.removeStatusAffect( StatusAffects.GooStuffed );
		CoC.getInstance().player.knockUpForce(); //Clear the false pregnancy
		CoC.getInstance().player.buttKnockUpForce(); //Clear the false pregnancy
	};
