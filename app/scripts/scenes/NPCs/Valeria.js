...
angular.module( 'cocjs' ).run( function( MainView, SceneLib, $rootScope, GooArmorMonster, ArmorLib, PregnancyStore, Descriptors, AppearanceDefs, CoC, kFLAGS, Utils, StatusAffects, EngineCore ) {
	...
	Valeria.prototype.valeriaFollower = function() {
		...
		MainView.outputText( 'You walk over to Valeria.  Seeing you approach, the armor-goo turns a slightly brighter shade of blue beneath her plates and grins.' );
	...
	Valeria.prototype.valeriaSpar = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 79 );
		MainView.outputText( 'You ask Valeria if she would be up for a bit of battle practice.' );
	...
	Valeria.prototype.pcWinsValeriaSparDefeat = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 79 );
		MainView.outputText( 'You collapse, ' );
		if( CoC.player.HP < 1 ) {
			MainView.outputText( 'in too much pain' );
		} else {
			MainView.outputText( 'too turned on' );
		}
		MainView.outputText( ' to fight any longer.  Valeria\'s on you in a heartbeat, her gooey greatsword pressed to your throat.' );
		...
		//[else; If PC lost via lust & has a gender: ;
			...
			MainView.outputText( '.  "<i>Well, since you\'re so... eager... I might as well get my daily fluids while we\'re here.</i>"' );
	...
	Valeria.prototype.followersValeriaSex = function( display ) {
		...
		if( display === undefined || display ) {
			MainView.clearOutput();
			MainView.outputText( 'With a lusty grin, you ask your gooey friend if ' );
			if( CoC.player.gender > 0 ) {
				MainView.outputText( 'she\'s interested in a little fluid exchange' );
			} else {
				MainView.outputText( 'she wouldn\'t mind helping you get some release, despite your genderlessness' );
			}
			MainView.outputText( '.' );
	...
	Valeria.prototype.valeriaDickToggle = function() {
		...
		if( CoC.flags[ kFLAGS.VELARIA_FUTA ] === 1 ) {
			MainView.outputText( 'Before you do anything, you ask Valeria if she wouldn\'t mind hiding her facsimile of a dick, at least when you aren\'t having sex.' );
		...
		//[If Valeria doesn't have a dick:] ;
			...
			MainView.outputText( 'Struggling to find a less-than-awkward way of phrasing this, you ask Valeria if she wouldn\'t mind growing a certain extra appendage for you.' );
	...
	Valeria.prototype.valeriaGetFucked = function() {
		MainView.spriteSelect( 79 );
		MainView.clearOutput();
		MainView.outputText( 'You disrobe and, ' );
		if( CoC.player.hasVagina() ) {
			MainView.outputText( 'stroking the slick lips of your [vagina], ask Valeria to fuck you.' );
		} else {
			MainView.outputText( 'posing seductively, ask Valeria to fuck you.' );
		}
		...
		if( CoC.player.isBiped() ) {
			MainView.outputText( 'and spreading your legs ' );
		}
		MainView.outputText( 'as Valeria ' );
		if( CoC.flags[ kFLAGS.VELARIA_FUTA ] === 0 ) {
			MainView.outputText( 'forms a thick, gooey cock' );
		} else {
			MainView.outputText( 'grabs her thick, gooey cock' );
		}
		MainView.outputText( ' and looms over you.' );
		...
		if( CoC.player.biggestTitSize() > 3 ) {
			MainView.outputText( 'bumping over your [chest] and stopping for a brief moment to tickle your sensitive tits with her goopy bottom before ' );
		}
		MainView.outputText( 'coming to a stop over your crotch.  She pours herself into your lap, ' );
		//[if PC has cock: "<i>;
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'her gooey ass devouring your ' + Descriptors.cockDescript( 0 ) + ' as she slides down your belly' );
		}
		if( CoC.player.hasCock() && CoC.player.hasVagina() ) {
			MainView.outputText( ' and ' );
		}
		if( CoC.player.hasVagina() ) {
			MainView.outputText( ' letting bits of her goo-body slither into your wet, waiting [vagina], ever so slightly peeling your walls apart' );
		}
		//If PC has no gender ;
		if( CoC.player.gender === 0 ) {
			MainView.outputText( 'poking the shaft of her cock gingerly, teasingly against your [asshole]' );
		}
		MainView.outputText( '.' );
		...
		if( CoC.player.hasCock() ) {
			MainView.outputText( ', as she rhythmically flexes her ass cheeks, shifting her fluidity around your shaft within her with a feeling like lotion-covered hands gently stroking you' );
		}
		MainView.outputText( '.' );
		//(If Female);
		if( !CoC.player.hasCock() && CoC.player.hasVagina() ) {
			...
			MainView.outputText( ' deeper inside you than ever before.  You feel streaks of goo pouring into you as she cums, pouring her essence into your womb ' );
			if( CoC.player.hasFuckableNipples() ) {
				MainView.outputText( 'and [chest] ' );
			}
			MainView.outputText( 'until slime overflows from inside you, pooling under her hips.  Filled beyond your capacity, you scream your pleasure and cum, clamping down on Valeria\'s cock' );
			if( CoC.player.hasFuckableNipples() ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ' until they literally pop inside you, flowing back out in goopy streaks.' );
		}
		//(If Male/Herm);
			...
			if( CoC.player.hasVagina() ) {
				...
				MainView.outputText( ', the double-attack\'s pleasure is so great that you barely even notice her creating a second cock above the first and pouring it into your unused [vagina], filling your last hole up with a firm, gooey cock' );
			}
			MainView.outputText( '.' );
			...
			MainView.outputText( ' literally popping as she climaxes, filling your stretched ass ' );
			if( CoC.player.hasVagina() ) {
				MainView.outputText( 'and vag ' );
			}
			MainView.outputText( 'with warm, sticky goop.' );
		}
		...
		if( CoC.player.gender > 0 ) {
			MainView.outputText( 'patting her belly full of your juices, ' );
		}
		MainView.outputText( 'and looms over you.  "<i>That was fun, partner,</i>" she says, leaning down to give you a wet peck on the cheek. "<i>Let\'s do that again soon, all right?</i>"' );
		CoC.player.orgasm();
	...
	Valeria.prototype.gooFlation = function( clearText ) {
		...
		if( clearText === undefined || clearText ) {
			clearText = true;
			MainView.clearOutput();
			MainView.outputText( '"<i>Just relax, partner,</i>" she grins, reaching up to run a hand through your [hair].  "<i>Just let ol\' Valeria take care of everything...</i>"' );
		...
		if( !CoC.player.hasVagina() ) {
			MainView.outputText( 'one lonely ' );
		}
		MainView.outputText( 'hole.  You gulp as the tentacle-fuck begins, her long tendrils pouring one by one into your [asshole], each so small and soft it easily slips inside you.  One piles onto another, tickling and teasing your anal walls as more and more of her slithers into you.' );
		...
		if( clearText ) {
			...
			MainView.outputText( 'and looms over you.  "<i>That was fun, partner,</i>" she says, leaning down to give you a wet peck on the cheek. "<i>Let\'s do that again soon, alright?</i>"' );
			CoC.player.orgasm();
			EngineCore.dynStats( 'sen', 1 );
			EngineCore.HPChange( 25, false );
			EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	...
	Valeria.prototype.valeriaSexDominated = function() {
		MainView.spriteSelect( 79 );
		MainView.clearOutput();
		MainView.outputText( 'Making a show of playing hesitant and nervous, you tell Valeria you\'d like her to take charge.  A wide grin quickly spreads across her girlish features.  "<i>Mmm, feeling subby today, partner?  Good, good... You just let Valeria take good care of you, ' + CoC.player.mf( 'handsome', 'cutie' ) + '.</i>"' );
		...
		if( CoC.player.hasVagina() && (!CoC.player.hasCock() || Utils.rand( 2 ) === 0) ) {
			...
			if( CoC.player.biggestTitSize() > 1 ) {
				...
				if( CoC.player.lactationQ() > 0 ) {
					MainView.outputText( '  To her delight, a spray of warm milk jets out of your sore nipples, milky white mixing into blue goo like oil in water.  "<i>Mmm, tasty!</i>" she teases, massaging more and more from you.' );
				}
			}
			...
			MainView.outputText( CoC.player.mf( 'boy', 'girl' ) + ' ready to cum?</i>" she withdraws herself from you, leaving you feeling suddenly very, very empty.  "<i>I didn\'t say you could cum yet, slut! I\'m not done with you yet.</i>"' );
			...
			if( CoC.player.vaginalCapacity() > 80 || CoC.player.analCapacity() > 80 ) {
				MainView.outputText( ' and your own receptibility' );
			}
			MainView.outputText( '.  Hugging your back, the goo-girl grabs at your [chest], squeezing your tits and pinching at your nipples.  "<i>Moan for me, bitch,</i>" Valeria growls into your ear, giving your [nipples] a particularly violent tug.  You let out a lewd, whorish moan.  The goo-girl grins, her tongue lashing out to lick your ear as she rewards you with a single long thrust into your depths, filling and stretching your [vagina] and [asshole] completely before withdrawing.' );
			...
			MainView.outputText( 'swirling around inside her.  "<i>We\'ll do this again sometime,</i>" she adds, walking off to another part of camp with a wink.' );
		}
		//(PC has Dick);
		else {
			...
			MainView.outputText( ' ' + CoC.player.mf( 'boy', 'girl' ) + ' ready to cum? Well, go on then - we\'ve got time for more.</i>"' );
	...
	Valeria.prototype.talkWithValeria = function() {
		MainView.spriteSelect( 79 );
		MainView.clearOutput();
		MainView.outputText( 'You ask Valeria if she wouldn\'t mind just talking for a little bit.' );
		...
		if( CoC.player.race() !== 'human' ) {
			MainView.outputText( ' were' );
		}
		MainView.outputText( '.  I just, I dunno, changed, I guess.</i>"' );
		MainView.outputText( '\n\nChanged?' );
	...
	Valeria.prototype.flirtWithValeria = function() {
		MainView.spriteSelect( 79 );
		MainView.clearOutput();
		MainView.outputText( 'You slip an arm around Valeria\'s slender shoulders and pull her close.  Patting your crotch, you tell her you\'re practically a gourmet chef for a girl like her.  She giggles and slips a hand easily into your [armor], ' );
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'brushing along the length of your rapidly-hardening cock' );
		} else {
			MainView.outputText( 'brushing a wet, warm knuckle across your [clit]' );
		}
		MainView.outputText( '.' );
	...
	Valeria.prototype.declineValeriasNeeds = function() {
		MainView.spriteSelect( 79 );
		MainView.clearOutput();
		MainView.outputText( 'You grimace and push the goo-girl away.  You\'ve got no interest in her corrupted \'needs,\' especially with a look like that on her face.  She gasps as you push her, nearly falling over; she catches herself and glowers angrily.' );
	...
	Valeria.prototype.valeriaAndGooThreeStuff = function() {
		MainView.clearOutput();
		MainView.outputText( 'You cautiously approach with Valeria\'s voice egging you on, "<i>We gonna fuck her? We\'re gonna fuck her, ain\'t we, [name]?</i>" She affectionately fondles your ' );
		var list = [ '[hips]' ];
		...
		list.push( '[asshole]' );
		list.push( '[nipples]' );
		MainView.outputText( Utils.formatStringArray( list ) + ' with liquid-soft caresses, almost ephemeral and yet still so perfectly all-consuming, filling every tiny imperfection in your [skin] with blue, gooey goodness and teasing whatever entrances she can get at. ' );
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'You sigh as [eachCock] stands at full erection. Bluish coating' );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ' barely thicker than a condom stretch' );
			if( CoC.player.cockTotal() === 1 ) {
				MainView.outputText( 'es' );
			}
			MainView.outputText( ' around ' );
			if( CoC.player.cockTotal() === 1 ) {
				MainView.outputText( 'it as it distends' );
			} else {
				MainView.outputText( 'them as they distend' );
			}
			MainView.outputText( ' Valeria\'s increasingly liquescent form. Dripping off the tip' );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ', sapphire slime falls in long gooey stands, yet the whole time you feel as if a dozen hands are dragging lubricant-coated feathers over the whole of your sensitive organ' );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( '.' );
		} else if( CoC.player.hasVagina() ) {
			MainView.outputText( 'You sigh your as [vagina] is tenderly caressed by what feels like dozens of lubricant-soaked feathers, tickling at the skin in a way that makes your entrance grow hot enough to make you wonder why Valeria hasn\'t been cooked into steam. The airtight seal over your mons is steadily shifting around and about to stimulate you. Sometimes, a narrow band of semi-solid goo slips between your lips like an oh-so-narrow thong, the slime mixing with your own suddenly-copious sexual juices to form a salacious, barely restrained soup of girlish need. She holds herself back from diving into your entrance... for now.' );
		} else {
			MainView.outputText( 'You sigh as your [asshole] is tenderly licked and tickled by what feels like dozens of inhumanly long tongues, slathering over it in such wide circles that your cheeks get a thorough lashing as well. Bands as firm and strong as leather straps slip into place around your [butt] to spread you wide open, and one of the slender, slime-crafted organs slithers deeper to taste your rosebud, never quite pushing in.' );
		}
		...
		MainView.outputText( '  She surprises you by contorting your body with pressure from an unexpected angle, tipping you head over heels, right toward the wide-eyed goo-girl you just managed to defeat!' );
		...
		MainView.outputText( '\n\nYou ' );
		if( CoC.flags[ kFLAGS.TIMES_VALERIA_GOO_THREESOMED ] === 0 ) {
			MainView.outputText( 'demand that she release you' );
		} else {
			MainView.outputText( 'playfully comment that she really ought to release you' );
		}
	...
	Valeria.prototype.valeriaGooRapeII = function() {
		MainView.clearOutput();
		MainView.outputText( 'The lump that bumped you earlier smacks off ' );
		if( CoC.player.biggestTitSize() >= 3 ) {
			MainView.outputText( 'the valley of your cleavage' );
		} else {
			MainView.outputText( 'your chest' );
		}
		MainView.outputText( ' once more, and this time you catch a glimpse of it out of the corner of your eye. It\'s the goo-girl\'s core, clasped in a fist of deep-blue. It pulses faintly as it\'s held there, just before you, while subtle vibrations and temperature fluctuations alert you to some kind of secret communications between the two women. Just what are they planning?' );
		MainView.outputText( '\n\nYou get your answer in the form of a sudden' );
		if( CoC.flags[ kFLAGS.TIMES_VALERIA_GOO_THREESOMED ] === 0 ) {
			MainView.outputText( ', unexpected' );
		} else {
			MainView.outputText( ' but expected' );
		}
		MainView.outputText( ' pressure at your [asshole]. It\'s so... so slick that you have to wonder how they can manage to push with such authority at your rearmost orifice, but they manage it all the same. You clench to deny them entrance, squeezing your sphincter shut tighter than locked door. Valeria\'s giggling voice coos from somewhere below, "<i>Oh, don\'t be a spoilsport. Come on, it\'ll be fun.</i>"' );
		if( CoC.flags[ kFLAGS.TIMES_VALERIA_GOO_THREESOMED ] === 0 ) {
			...
			MainView.outputText( 'You hold tight, keeping up your faux resistance until she yanks your cheeks open and twists her gooey reaming-tool into a sphincter-penetrating drill bit.' );
		}
		MainView.outputText( ' Spinning faster and faster, the intruding, cylindrical slime dilates your [asshole] to its maximum gape in four or five seconds, hardening its exterior into an butt-stuffing goo-pipe. You can feel it snaking around through your bowels, plumbing deeper inside you than you would have thought possible before stopping what feels like halfway through your large intestine.' );
		...
		if( CoC.player.hasFuckableNipples() && CoC.player.biggestTitSize() >= 5 ) {
			...
			if( CoC.player.hasVagina() ) {
				MainView.outputText( ' as your snatch' );
			} else {
				MainView.outputText( ' as the randiest slut\'s snatch' );
			}
			MainView.outputText( '. Valeria takes this as an invitation, and ' + Utils.num2Text( CoC.player.totalNipples() ) + ' phallic objects greedily push at your cunt-nipples, eagerly sliding themselves inside your welcome chest-pussies. Looking down, you can see the navy-blue outlines of the phallic tendrils waving and writhing as they push their way deep, embedding themselves tit-deep in blissful warmth.' );
		}
		//Dick;
		if( CoC.player.hasCock() ) {
			MainView.outputText( '\n\nValeria\'s voice calls out once more, "<i>You didn\'t think I\'d forget ' );
			if( CoC.player.cockTotal() === 1 ) {
				MainView.outputText( 'this little guy' );
			} else {
				MainView.outputText( 'these little guys' );
			}
			MainView.outputText( ', did you? ' );
			if( CoC.player.biggestCockArea() >= 50 ) {
				MainView.outputText( '...Maybe big guy is a bit more appropriate. Whatever. ' );
			}
			MainView.outputText( 'I was just saving the best for last. Hold on tight, tiger.</i>"' );
			...
			MainView.outputText( ' in place, and something even warmer than the slippery container' );
			if( CoC.player.totalCocks() > 1 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ' pushes at your head' );
			if( CoC.player.totalCocks() > 1 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ', circling your urethra' );
			if( CoC.player.totalCocks() > 1 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( '. You inadvertantly clench your muscles in a way that makes your ' + Descriptors.multiCockDescriptLight() + ' so very swollen, in turn making it that much easier for the gelatinous fiend to have her way with your cumslit' );
			if( CoC.player.totalCocks() > 1 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( '. You feel that warm, wet fluid slowly peel it open and push inside. It doesn\'t hurt at all, but it is a little weird having your ' + Descriptors.multiCockDescriptLight() + ' slowly stuffed in such a way. The warmth travels the whole way down your dick' );
			if( CoC.player.totalCocks() > 1 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ' to your middle, where you can feel it travel a bit further before it finally stops at your [balls].' );
		}
		//Merge;
		MainView.outputText( '\n\nGiggling, the sapphire slut does something that makes the body-wrapping sphere jiggle and contract slightly. ' );
		if( CoC.flags[ kFLAGS.TIMES_VALERIA_GOO_THREESOMED ] > 0 ) {
			MainView.outputText( 'You grin in eager anticipation, waiting to feel the flood of fluid that you know is coming. It won\'t take long.... You feel it in your butt first.' );
		} else {
			MainView.outputText( 'At first, you don\'t realize what it is. You\'re floating in a bit of a sexual daze by now and barely cogent of what\'s going on, but it does feel quite nice, whatever it is. The spreading heat in your posterior is what finally clues you in to what\'s happening.' );
		}
		MainView.outputText( ' Pumping deep into your bowels, the hardened protrusion is no longer simply stuffing you. It\'s pumping something slick and wet and oh so gooey straight into your [asshole], filling you up so strangely that you aren\'t sure what to make of beyond how... well, good it feels to be stuffed like this.' );
		//{Vagina!};
		if( CoC.player.hasVagina() ) {
			...
			MainView.outputText( ' your girlcum in feverish releases of ecstasy, repeatedly getting off on this perverse, doubled slime-flation.' );
		}
		//{titties};
		if( CoC.player.hasFuckableNipples() && CoC.player.biggestTitSize() >= 5 ) {
		...
		if( CoC.player.hasCock() ) {
			...
			MainView.outputText( ' in [eachCock] ' );
			if( CoC.player.totalCocks() > 1 ) {
				MainView.outputText( 'do not' );
			} else {
				MainView.outputText( 'does not' );
			}
			MainView.outputText( ' differ greatly from Valeria\'s other liquid attentions. ' );
			if( CoC.player.totalCocks() > 1 ) {
				MainView.outputText( 'They start' );
			} else {
				MainView.outputText( 'It starts' );
			}
			MainView.outputText( ' by thickening, stretching your cumvein' );
			if( CoC.player.totalCocks() > 1 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ' as wide as ' );
			if( CoC.player.totalCocks() > 1 ) {
				MainView.outputText( 'they' );
			} else {
				MainView.outputText( 'it' );
			}
			MainView.outputText( '\'ll go without pain and then rolling thick pulses of gelatinous matter into your urethra' );
			if( CoC.player.totalCocks() > 1 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( 's' );
			MainView.outputText( '. The sleeve' );
			if( CoC.player.totalCocks() > 1 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ' squeeze' );
			if( CoC.player.totalCocks() === 1 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ' down on your length' );
			if( CoC.player.totalCocks() > 1 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ' harder, starting to slide around in ways that would be impossible for a vagina, and you find yourself helplessly blowing your load straight into the dick-plug' );
			if( CoC.player.totalCocks() > 1 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ', spurting bliss that can\'t even hope to escape from your loins. You feel ' );
			if( CoC.player.balls === 0 ) {
				MainView.outputText( 'something expanding as you cum, filling up with spunk or slime or some mixture of the two' );
			} else {
				MainView.outputText( 'your [balls] expanding as you cum, filling up with spunk or slime or some mixture of the two, weighing down your sack with ever-increasing heaviness. Tender licks of tongue roll across the surface as you fill beyond completely' );
			}
			MainView.outputText( '.' );
		}
		//Butt fillinggasm;
		...
		if( CoC.player.biggestTitSize() >= 5 && CoC.player.hasFuckableNipples() ) {
			MainView.outputText( ', pausing to handle your hefty, swelling tits' );
		}
		MainView.outputText( ', moaning and groaning as Valeria\'s shape gradually coalesces back around you. The goo-girl\'s core vanishes from view and reappears down south, pressing against your [asshole] as the "pipe" that filled you so full melts into slime. You scream in anal ecstasy as it slips past your ring, lodging firmly in your butt while the jellied mass inside convulses, rubbing against your walls in all the right ways, almost like it\'s experiencing an orgasm all its own.' );
		...
		if( CoC.player.biggestTitSize() >= 5 && CoC.player.hasFuckableNipples() ) {
			...
			MainView.outputText( ' Your tits have grown much larger, ' + CoC.player.breastCup( 0 ) + '-cups at least.' );
		}
		if( CoC.player.hasCock() && CoC.player.balls > 0 ) {
			CoC.player.ballSize += 3 + Utils.rand( 2 );
			MainView.outputText( ' Your balls look positively swollen with libidinous juices.' );
		}
		MainView.outputText( ' Your belly is stuffed into a parody of a pregnant woman\'s' );
		if( CoC.player.hasVagina() ) {
			MainView.outputText( ', so gravid from both holes that a real mother would be shamed by your obscene "pregnancy."' );
		} else {
			MainView.outputText( '.' );
		}
	...
	Valeria.prototype.birthOutDatGooSlut = function() {
		...
		if( CoC.player.statusAffectv4( StatusAffects.GooStuffed ) > 0 ) {
			MainView.outputText( ' Your tits are even wobbling around wildly, shaking and jiggling obscenely inside your [armor] in a way that makes you your [nipples] more than a little leaky.' );
		}
		...
		MainView.outputText( 'bitch in heat. Gods, there\'s so much!' );
		if( CoC.player.statusAffectv4( StatusAffects.GooStuffed ) > 0 ) {
			MainView.outputText( '\n\n' );
			if( !CoC.player.hasFuckableNipples() ) {
				MainView.outputText( 'Stiffening up' );
			} else {
				MainView.outputText( 'Opening up' );
			}
			MainView.outputText( ', your [nipples], suddenly disgorge rivers of gelatinous slime, adding to growing puddle below with, odd-colored obscene lactation. You grab one with one hand and squeeze, shooting out huge streamers of milky-goo until your tit is sore and aching. You repeat on the other side' );
			if( CoC.player.bRows() > 1 ) {
				MainView.outputText( ' and the next row down' );
			}
			if( CoC.player.bRows() > 2 ) {
				MainView.outputText( ', and so on' );
			}
			MainView.outputText( ', working out every drop of thick, body-filling slime while your ass is still busy draining. Even after they empty, they don\'t seem to lose a single bit of the size they\'ve gained....' );
		}
