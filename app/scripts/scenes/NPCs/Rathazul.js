...
angular.module( 'cocjs' ).run( function( SceneLib, $rootScope, UsableLib, $log, PerkLib, Appearance, AppearanceDefs, ArmorLib, EventParser, CoC, kFLAGS, Utils, StatusAffects, EngineCore, ConsumableLib ) {
	...
	Rathazul.prototype.timeChange = function() {
		if( CoC.getInstance().flags[ kFLAGS.RATHAZUL_SILK_ARMOR_COUNTDOWN ] > 1 ) {
			CoC.getInstance().flags[ kFLAGS.RATHAZUL_SILK_ARMOR_COUNTDOWN ]--;
			if( CoC.getInstance().flags[ kFLAGS.RATHAZUL_SILK_ARMOR_COUNTDOWN ] < 1 ) {
				CoC.getInstance().flags[ kFLAGS.RATHAZUL_SILK_ARMOR_COUNTDOWN ] = 1;
			}
			if( CoC.getInstance().flags[ kFLAGS.RATHAZUL_SILK_ARMOR_COUNTDOWN ] > 300 ) {
				CoC.getInstance().flags[ kFLAGS.RATHAZUL_SILK_ARMOR_COUNTDOWN ] = 24;
			}
		}
		if( CoC.getInstance().flags[ kFLAGS.RATHAZUL_CAMP_INTERACTION_COUNTDOWN ] > 0 ) {
			CoC.getInstance().flags[ kFLAGS.RATHAZUL_CAMP_INTERACTION_COUNTDOWN ]--;
			if( CoC.getInstance().flags[ kFLAGS.RATHAZUL_CAMP_INTERACTION_COUNTDOWN ] < 0 ) {
				CoC.getInstance().flags[ kFLAGS.RATHAZUL_CAMP_INTERACTION_COUNTDOWN ] = 0;
			}
		}
		return false;
	};
	...
	Rathazul.prototype.encounterRathazul = function() {
		...
		//Introduction;
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.MetRathazul ) >= 0 ) {
			if( CoC.getInstance().player.findStatusAffect( StatusAffects.CampRathazul ) >= 0 ) {
			...
			CoC.getInstance().player.createStatusAffect( StatusAffects.MetRathazul, 0, 0, 0, 0 );
		}
		//Camp offer!;
		if( CoC.getInstance().player.statusAffectv2( StatusAffects.MetRathazul ) >= 3 && CoC.getInstance().player.statusAffectv3( StatusAffects.MetRathazul ) !== 1 && CoC.getInstance().player.cor < 75 ) {
			...
			CoC.getInstance().player.changeStatusValue( StatusAffects.MetRathazul, 3, 1 );
			return;
		}
	...
	Rathazul.prototype.campRathazul = function() {
		...
		if( Utils.rand( 6 ) === 0 && CoC.getInstance().flags[ kFLAGS.RATHAZUL_CAMP_INTERACTION_COUNTDOWN ] === 0 ) {
		...
		//Introduction;
		EngineCore.outputText( 'Rathazul looks up from his equipment and gives you an uncertain smile.\n\n"<i>Oh, don\'t mind me,</i>" he says, "<i>I\'m just running some tests here.  Was there something you needed, ' + CoC.getInstance().player.short + '?</i>"\n\n', true );
		//CoC.getInstance().player.createStatusAffect(StatusAffects.metRathazul,0,0,0,0);;
		offered = this.rathazulWorkOffer();
		if( !offered ) {
	...
	Rathazul.prototype.rathazulWorkOffer = function() {
		...
		var dyes = null;
		if( CoC.getInstance().player.hasItem( ConsumableLib.BLACKEG ) || CoC.getInstance().player.hasItem( ConsumableLib.L_BLKEG ) ) {
		...
		//Item crafting offer;
		if( CoC.getInstance().player.hasItem( UsableLib.GREENGL, 2 ) ) {
			...
			spoken = true;
			if( CoC.getInstance().player.hasItem( UsableLib.GREENGL, 5 ) ) {
				showArmorMenu = true;
				totalOffers++;
			} else {
		...
		//Item crafting offer;
		if( CoC.getInstance().player.hasItem( UsableLib.B_CHITN ) ) {
			...
			spoken = true;
			if( CoC.getInstance().player.hasItem( UsableLib.B_CHITN, 5 ) ) {
				showArmorMenu = true;
				totalOffers++;
			} else {
		...
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.CampRathazul ) >= 0 && CoC.getInstance().player.statusAffectv2( StatusAffects.MetRathazul ) >= 4 ) {
			...
			if( CoC.getInstance().flags[ kFLAGS.AMILY_MET_RATHAZUL ] >= 2 ) {
		...
		//SPOIDAH;
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.CampRathazul ) >= 0 && CoC.getInstance().player.hasItem( UsableLib.T_SSILK ) && CoC.getInstance().flags[ kFLAGS.RATHAZUL_SILK_ARMOR_COUNTDOWN ] + CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00275 ] === 0 ) {
			showArmorMenu = true;
			spoken = true;
			totalOffers++;
		...
		//Vines;
		if( CoC.getInstance().player.hasKeyItem( 'Marae\'s Lethicite' ) >= 0 && CoC.getInstance().player.keyItemv2( 'Marae\'s Lethicite' ) < 3 && CoC.getInstance().player.findStatusAffect( StatusAffects.DefenseCanopy ) < 0 && CoC.getInstance().player.findStatusAffect( StatusAffects.CampRathazul ) >= 0 ) {
		...
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.CampRathazul ) >= 0 ) {
			if( CoC.getInstance().flags[ kFLAGS.RATHAZUL_DEBIMBO_OFFERED ] === 0 && (SceneLib.sophieBimbo.bimboSophie() || CoC.getInstance().player.findPerk( PerkLib.BimboBrains ) >= 0 || CoC.getInstance().player.findPerk( PerkLib.FutaFaculties ) >= 0) ) {
				...
				if( CoC.getInstance().player.hasItem( ConsumableLib.SMART_T, 5 ) && CoC.getInstance().player.gems >= 250 ) {
					totalOffers++;
					debimbo = 1;
				} else if( !CoC.getInstance().player.hasItem( ConsumableLib.SMART_T, 5 ) ) {
	...
	Rathazul.prototype.rathazulDebimboOffer = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.RATHAZUL_DEBIMBO_OFFERED ] === 0 ) {
			...
			CoC.getInstance().flags[ kFLAGS.RATHAZUL_DEBIMBO_OFFERED ]++;
		}
		//Rath menu;
		EngineCore.menu();
	...
	Rathazul.prototype.rathazulArmorMenu = function() {
		EngineCore.spriteSelect( 49 );
		EngineCore.clearOutput();
		var beeArmor = (CoC.getInstance().player.hasItem( UsableLib.B_CHITN, 5 ) ? this.craftCarapace : null);
		var gelArmor = (CoC.getInstance().player.hasItem( UsableLib.GREENGL, 5 ) ? this.craftOozeArmor : null);
		var silk = null;
	...
	Rathazul.prototype.craftSilkArmor = function() {
		...
		//(Not enough webs: ;
		if( !CoC.getInstance().player.hasItem( UsableLib.T_SSILK, 5 ) ) {
	...
	Rathazul.prototype.commissionSilkArmorForReal = function() {
		...
		EngineCore.statScreenRefresh();
		CoC.getInstance().player.destroyItems( UsableLib.T_SSILK, 5 );
		EngineCore.menu();
	...
	Rathazul.prototype.collectRathazulArmor = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00275 ] === 1 ) {
		...
		//Robe;
		var itype;
		if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00275 ] === 2 ) {
	...
	Rathazul.prototype.buyReducto = function() {
		EngineCore.spriteSelect( 49 );
		EngineCore.clearOutput();
		var cost = (CoC.getInstance().flags[ kFLAGS.AMILY_MET_RATHAZUL ] >= 2 ? 50 : 100);
	...
	Rathazul.prototype.growLethiciteDefenseYesYesYes = function() {
		...
		CoC.getInstance().player.createStatusAffect( StatusAffects.DefenseCanopy, 0, 0, 0, 0 );
	...
	Rathazul.prototype.craftCarapace = function() {
		...
		if( CoC.getInstance().player.cockTotal() > 0 && CoC.getInstance().player.biggestCockArea() >= 40 ) {
