...
angular.module( 'cocjs' ).run( function( MainView, SceneLib, $rootScope, UseableLib, $log, PerkLib, Appearance, AppearanceDefs, ArmorLib, CoC, kFLAGS, Utils, StatusAffects, EngineCore, ConsumableLib ) {
	...
	Rathazul.prototype.encounterRathazul = function() {
		...
		if( CoC.player.statusAffectv2( StatusAffects.MetRathazul ) >= 3 && CoC.player.statusAffectv3( StatusAffects.MetRathazul ) !== 1 && CoC.player.cor < 75 ) {
			MainView.outputText( '"<i>You know, I think I might be able to do this worn-out world a lot more good from your camp than by wandering around this lake.  What do you say?</i>" asks the rat.\n\n(Move Rathazul into your camp?)', false );
		...
		if( !offered ) {
			MainView.outputText( 'He sighs dejectedly, "<i>I am not sure what I can do for you, youngling.  This world is fraught with unimaginable dangers, and you\'re just scratching the surface of them.</i>"\n\nYou nod and move on, leaving the depressed alchemist to his sadness.', false );
			EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
		}
	};
	Rathazul.prototype.rathazulMoveToCamp = function() {
		MainView.clearOutput();
		MainView.outputText( 'Rathazul smiles happily back at you and begins packing up his equipment.  He mutters over his shoulder, "<i>It will take me a while to get my equipment moved over, but you head on back and I\'ll see you within the hour.  Oh my, yes.</i>"\n\nHe has the look of someone experiencing hope for the first time in a long time.' );
		CoC.player.createStatusAffect( StatusAffects.CampRathazul, 0, 0, 0, 0 );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	Rathazul.prototype.rathazulMoveDecline = function() {
		MainView.clearOutput();
		MainView.outputText( 'Rathazul wheezes out a sigh, and nods.\n\n"<i>Perhaps I\'ll still be of some use out here after all,</i>" he mutters as he packs up his camp and prepares to head to another spot along the lake.' );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	...
	Rathazul.prototype.rathazulWorkOffer = function() {
		...
		if( CoC.player.hasItem( ConsumableLib.BLACKEG ) || CoC.player.hasItem( ConsumableLib.L_BLKEG ) ) {
			...
			spoken = true;
			MainView.outputText( 'He eyes the onyx egg in your inventory and offers a little advice.  "<i>Be careful with black eggs.  They can turn your skin to living latex or rubber.  The smaller ones are usually safer, but everyone reacts differently.  I\'d get rid of them, if you want my opinion.</i>"\n\n' );
		}
		//Item crafting offer;
		if( CoC.player.hasItem( UseableLib.GREENGL, 2 ) ) {
			...
			if( CoC.player.hasItem( UseableLib.GREENGL, 5 ) ) {
				...
				MainView.outputText( 'You realize you\'re still a bit short of gel.\n\n', false );
			}
		}
		//Item crafting offer;
		if( CoC.player.hasItem( UseableLib.B_CHITN ) ) {
			MainView.outputText( 'The elderly rat looks at you intently and offers, "<i>I see you\'ve gathered a piece of chitin from the giant bees of the forests.  If you bring me five pieces I could probably craft it into some tough armor.</i>"\n\n', false );
			spoken = true;
			if( CoC.player.hasItem( UseableLib.B_CHITN, 5 ) ) {
				showArmorMenu = true;
				totalOffers++;
			} else {
				MainView.outputText( '(You need five pieces of chitin for Rathazul to make you the chitinous armor.)\n\n', false );
			}
		}
		var pCounter = 0;
		...
		//Single Offer;
		if( pCounter === 1 ) {
			MainView.outputText( 'The rat mentions, "<i>I see you have at least one tainted item on you... for 20 gems I could remove most of the taint, making it a good deal safer to use.  Of course, who knows what kind of freakish transformations it would cause...</i>"\n\n', false );
			spoken = true;
			totalOffers++;
		}
		if( pCounter > 1 ) {
			MainView.outputText( 'The rat mentions, "<i>I see you have a number of demonic items on your person.  For 20 gems I could attempt to remove the taint from one of them, rendering it a good deal safer for consumption.  Of course it would not remove most of the transformative properties of the item...</i>"\n\n', false );
			spoken = true;
			totalOffers += 2;
		}
		//Offer dyes if offering something else.;
		if( CoC.player.gems >= 50 ) {
			MainView.outputText( 'Rathazul offers, "<i>Since you have enough gems to cover the cost of materials for my dyes as well, you could buy one of my dyes for your hair.  I will need 50 gems up-front.</i>"\n\n', false );
			spoken = true;
			totalOffers++;
			dyes = this.buyDyes;
		}
		...
		if( CoC.player.findStatusAffect( StatusAffects.CampRathazul ) >= 0 && CoC.player.statusAffectv2( StatusAffects.MetRathazul ) >= 4 ) {
			...
			if( CoC.flags[ kFLAGS.AMILY_MET_RATHAZUL ] >= 2 ) {
				...
				MainView.outputText( '100' );
			}
			MainView.outputText( ' gems for each jar of ointment you want.</i>"\n\n' );
			totalOffers++;
			spoken = true;
		...
		if( CoC.player.findStatusAffect( StatusAffects.CampRathazul ) >= 0 ) {
			if( CoC.flags[ kFLAGS.RATHAZUL_DEBIMBO_OFFERED ] === 0 && (SceneLib.sophieBimbo.bimboSophie() || CoC.player.findPerk( PerkLib.BimboBrains ) >= 0 || CoC.player.findPerk( PerkLib.FutaFaculties ) >= 0) ) {
				...
				MainView.outputText( 'You recall that Rathazul is willing to make something to cure bimbo liqueur for 250 gems and five Scholar\'s Teas.' );
				if( CoC.player.hasItem( ConsumableLib.SMART_T, 5 ) && CoC.player.gems >= 250 ) {
					totalOffers++;
					debimbo = 1;
				} else if( !CoC.player.hasItem( ConsumableLib.SMART_T, 5 ) ) {
					MainView.outputText( '  You should probably find some if you want that...' );
				} else {
					MainView.outputText( '  You need more gems to afford that, though.' );
				}
				MainView.outputText( '\n\n' );
			}
		}
	...
	Rathazul.prototype.rathazulPurifyIncubiDraft = function() {
		...
		if( CoC.player.gems < 20 ) {
			MainView.outputText( 'Rathazul says, "<i>You do not have enough gems for that service.</i>"' );
	...
	Rathazul.prototype.craftSilkArmor = function() {
		...
		MainView.outputText( 'You hand the bundled webbing to Rathazul carefully, lest you damage the elderly mouse.  He gives you a bemused smile and snatches the stuff from your grasp while he mutters, "<i>I\'m not falling apart you know.</i>"\n\n', false );
		//(Not enough webs: ;
		if( !CoC.player.hasItem( UseableLib.T_SSILK, 5 ) ) {
			MainView.outputText( 'The rat shakes his head and hands it back to you.  "<i>This isn\'t enough for me to make anything with.  I\'ll need at least five bundles of this stuff total, so you\'ll need to find more,</i>" he explains.\n\n', false );
			//(optional spider bonus: ;
			if( CoC.player.tailType === AppearanceDefs.TAIL_TYPE_SPIDER_ADBOMEN ) {
				MainView.outputText( 'You show him your spider-like abdomen in response, offering to produce more webbing for him.  Rathazul chuckles dryly, a sound that reminds you of hot wind rushing through a dead valley.  "<i>Dear child, this would never do.  Silk this tough can only be produced by a true-born spider.  No matter how you change yourself, you\'ll always be a human at heart.</i>"\n\n', false );
				MainView.outputText( 'The old rat shakes his head and adds, "<i>Well, now that I think about it, the venom of a red widow might be able to transform you until you are a spider to the core, but I have absolutely no idea what that would do to you.  If you ever try such a dangerous, reckless idea, let me know.  I want to have my notebooks handy, for SCIENCE!</i>"\n\n', false );
			}
			EngineCore.doNext( this, this.returnToRathazulMenu );
			return;
		}
		MainView.outputText( 'The rat limps over to his equipment, spider-silk in hand.  With efficient, practiced motions, he runs a few tests.  As he finishes, he sighs and explains, "<i>This will be harder than I thought.  The webbing is highly resistant to most of my alchemic reagents.  To even begin to work with such material I will need a number of rare, expensive elements.  I would need 500 gems to even start such a project.</i>"\n\n', false );
		MainView.outputText( 'You can\'t help but sigh when he names such a sizable figure.  Do you give him the 500 gems and spider-silk in order for him to create you a garment?', false );
		if( CoC.player.gems < 500 ) {
			MainView.outputText( '  <b>Wait... you don\'t even have 500 gems.  Damn.</b>', false );
			EngineCore.doNext( this, this.returnToRathazulMenu );
			return;
		}
		//[Yes] [No];
	...
	Rathazul.prototype.commissionSilkArmorForReal = function() {
		EngineCore.spriteSelect( 49 );
		MainView.outputText( '', true );
		MainView.outputText( 'You sort 500 gems into a pouch and toss them to Rathazul, along with the rest of the webbing.  The wizened alchemist snaps the items out of the air with lightning-fast movements and goes to work immediately.  He bustles about with enormous energy, invigorated by the challenging task before him.  It seems Rathazul has completely forgotten about you, but as you turn to leave, he calls out, "<i>What did you want me to make?  A mage\'s robe or some nigh-impenetrable armor?</i>"\n\n', false );
		CoC.player.gems -= 500;
		MainView.statsView.show();
	...
	Rathazul.prototype.declineSilkArmorCommish = function() {
		EngineCore.spriteSelect( 49 );
		MainView.outputText( '', true );
		MainView.outputText( 'You take the silk back from Rathazul and let him know that you can\'t spend 500 gems on a project like that right now.  He sighs, giving you a crestfallen look and a slight nod of his hooded muzzle.', false );
		EngineCore.doNext( this, this.returnToRathazulMenu );
	};
	Rathazul.prototype.chooseArmorOrRobes = function( robeType ) {
		EngineCore.spriteSelect( 49 );
		MainView.outputText( 'Rathazul grunts in response and goes back to work.  You turn back to the center of your camp, wondering if the old rodent will actually deliver the wondrous item that he\'s promised you.', true );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	...
	Rathazul.prototype.collectRathazulArmor = function() {
		EngineCore.spriteSelect( 49 );
		MainView.outputText( '', true );
		MainView.outputText( 'Rathazul beams and ejaculates, "<i>Good news everyone!  Your ', false );
		if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00275 ] === 1 ) {
			MainView.outputText( 'armor', false );
		} else {
			MainView.outputText( 'robe', false );
		}
		MainView.outputText( ' is finished!</i>"\n\n', false );
		//Robe;
		var itype;
		if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00275 ] === 2 ) {
			MainView.outputText( 'Hanging from a small rack is a long, flowing robe.  It glitters brightly in the light, the pearl-white threads seeming to shimmer and shine with every ripple the breeze blows through the soft fabric.  You run your fingers over the silken garment, feeling the soft material give at your touch.  There\'s a hood with a golden border embroidered around the edge.  For now, it hangs limply down the back, but it would be easy to pull up in order to shield the wearer\'s eyes from harsh sunlight or rainy drizzle.  The sleeves match the cowl, circled with intricate threads laid out in arcane patterns.\n\n', false );
			MainView.outputText( 'Rathazul gingerly takes down the garment and hands it to you.  "<i>Don\'t let the softness of the material fool you.  This robe is tougher than many armors, and the spider-silk\'s properties may even help you in your spell-casting as well.</i>"\n\n', false );
			itype = ArmorLib.SS_ROBE;
		}
		//(Armor);
		else {
			MainView.outputText( 'A glittering white suit of armor sits atop a crude armor rack, reflecting the light that plays across its surface beautifully.  You definitely didn\'t expect anything like this!  It looks nearly identical to a set of light platemail, though instead of having a cold metal surface, the armor feels slightly spongy, with just a little bit of give in it.\n\n', false );
			MainView.outputText( 'While you marvel at the strange equipment, Rathazul explains, "<i>When you said you wanted armor, I realized I could skip a few of the alchemical processes used to soften material.  The savings let me acquire a cheap metal set of armor to use as a base, and I molded half the armor around each piece, then removed it and created the outer, defensive layers with the rest of the webbing.  Unfortunately, I didn\'t have enough silk for a solid codpiece, but I did manage to make a you thin loincloth from the leftover scraps  - for modesty.</i>"\n\n', false );
	...
	Rathazul.prototype.buyDyes = function() {
		EngineCore.spriteSelect( 49 );
		MainView.clearOutput();
		MainView.outputText( 'Rathazul smiles and pulls forth several vials of colored fluids.  Which type of dye would you like?' );
		MainView.outputText( '\n\n<b>(-50 Gems)</b>' );
		CoC.player.gems -= 50;
		MainView.statsView.show();
		EngineCore.menu();
	...
	Rathazul.prototype.buyDyeNevermind = function() {
		EngineCore.spriteSelect( 49 );
		MainView.clearOutput();
		MainView.outputText( 'You change your mind about the dye, and Rathazul returns your gems.\n\n(<b>+50 Gems</b>)' );
		CoC.player.gems += 50;
		MainView.statsView.show();
		EngineCore.doNext( this, this.returnToRathazulMenu );
	};
	Rathazul.prototype.buyReducto = function() {
		...
		if( CoC.player.gems >= cost ) {
			MainView.outputText( 'Rathazul hands you the Reducto with a nod before returning to his work.\n\n' );
			CoC.player.gems -= cost;
			SceneLib.inventory.takeItem( ConsumableLib.REDUCTO, this.returnToRathazulMenu );
			MainView.statsView.show();
			CoC.player.addStatusValue( StatusAffects.MetRathazul, 2, 1 );
		} else {
			MainView.outputText( '"<i>I\'m sorry, but you lack the gems I need to make the trade,</i>" apologizes Rathazul.' );
			EngineCore.doNext( this, this.returnToRathazulMenu );
		}
	};
	Rathazul.prototype.growLethiciteDefense = function() {
		EngineCore.spriteSelect( 49 );
		MainView.clearOutput();
		MainView.outputText( 'Rathazul asks, "<i>Are you absolutely sure?  Growing this thorn canopy as a defense will use one third of the crystal\'s power.</i>"\n\n(Do you have Rathazul use the crystal to grow a defensive canopy?)' );
	...
	Rathazul.prototype.growLethiciteDefenseGuessNot = function() {
		EngineCore.spriteSelect( 49 );
		MainView.clearOutput();
		MainView.outputText( 'Rathazul nods sagely, "<i>That may be wise.  Perhaps there will be another use for this power.' );
		EngineCore.doNext( this, this.returnToRathazulMenu );
	};
	Rathazul.prototype.craftCarapace = function() {
		EngineCore.spriteSelect( 49 );
		MainView.outputText( 'The rat takes the scales and works on his bench for an hour while you wait.  Once he has finished, Ratzhul is beaming with pride, "<i>I think you\'ll be pleased. Go ahead and take a look.</i>"\n\nHe hands you the armor.  ', true );
		MainView.outputText( 'The plates shine and shimmer like black steel.  He has used the yellow chitin to add accents and embroidery to the plates with a level of detail and craftsmanship rarely seen back home. A yellow fur neck lining has been fashioned from hairs found on the pieces.  The armor includes a breastplate, shoulder guards, full arm guards, and knee high boots.  You notice there are no pants.  As you turn to ask him where the pants are, you see him scratching his head and hastily rustling in drawers.  He mutters under his breath, "<i>I\'m sorry, I\'m sorry, I got so focused on working on the pauldrons that I forgot to make any leg coverings!  Here, this should look good with it, and it won\'t restrict your movements.</i>"  He hands you a silken loincloth', false );
		if( CoC.player.gender >= 2 ) {
			MainView.outputText( ' with stockings and garters', false );
		}
		MainView.outputText( '.  He still manages to look somewhat pleased with himself in spite of the blunder, even bragging a little bit, "<i>Let me show you the different lengths of string I used.</i>"\n\n', false );
		if( CoC.player.cockTotal() > 0 && CoC.player.biggestCockArea() >= 40 ) {
			MainView.outputText( 'The silken material does little to hide the bulge of your groin, if anything it looks a little lewd.  Rathazul mumbles and looks away, shaking his head.\n\n', false );
		}
		if( CoC.player.biggestTitSize() >= 8 ) {
			MainView.outputText( 'Your ' + Appearance.biggestBreastSizeDescript() + ' barely fit into the breastplate, leaving you displaying a large amount of jiggling cleavage.\n\n', false );
		}
