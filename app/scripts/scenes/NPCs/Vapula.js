...
angular.module( 'cocjs' ).run( function( ConsumableLib, CockTypesEnum, PerkLib, $rootScope, PregnancyStore, Descriptors, AppearanceDefs, EventParser, CoC, kFLAGS, Utils, StatusAffects, EngineCore ) {
	function Vapula() {
	...
	Vapula.prototype.timeChange = function() {
		if( CoC.getInstance().time.hours > 23 ) {
			if( this.vapulaSlave() && CoC.getInstance().flags[ kFLAGS.FOLLOWER_AT_FARM_VAPULA ] === 0 ) {
				if( CoC.getInstance().flags[ kFLAGS.VAPULA_HAREM_FUCK ] === 0 ) {
					CoC.getInstance().flags[ kFLAGS.VAPULA_DAYS_SINCE_FED ]++;
				} else {
					CoC.getInstance().flags[ kFLAGS.VAPULA_DAYS_SINCE_FED ] = 0;
				}
			}
			if( CoC.getInstance().flags[ kFLAGS.VAPULA_FOLLOWER ] === 0.5 || CoC.getInstance().flags[ kFLAGS.VAPULA_FOLLOWER ] === 1.5 ) {
				CoC.getInstance().flags[ kFLAGS.VAPULA_FOLLOWER ]++;
			}
			CoC.getInstance().flags[ kFLAGS.DAYS_SINCE_LAST_DEMON_DEALINGS ]++;
		}
		if( this.vapulaSlave() && CoC.getInstance().player.hasKeyItem( 'Demonic Strap-On' ) < 0 && CoC.getInstance().player.gender === 2 && CoC.getInstance().flags[ kFLAGS.FOLLOWER_AT_FARM_VAPULA ] === 0 ) {
			this.vapulaGivesPCAPresent();
			return true;
		}
		return false;
	};
	Vapula.prototype.timeChangeLarge = function() {
		if( CoC.getInstance().flags[ kFLAGS.VAPULA_FOLLOWER ] >= 2.5 && CoC.getInstance().time.hours === 6 && CoC.getInstance().flags[ kFLAGS.FOLLOWER_AT_FARM_VAPULA ] === 0 ) {
			this.femaleVapulaRecruitmentPartII();
			return true;
		}
		if( CoC.getInstance().time.hours === 2 && this.vapulaSlave() && CoC.getInstance().flags[ kFLAGS.FOLLOWER_AT_FARM_VAPULA ] === 0 && CoC.getInstance().flags[ kFLAGS.VAPULA_DAYS_SINCE_FED ] >= 5 && (CoC.getInstance().player.hasCock() || (CoC.getInstance().player.hasKeyItem( 'Demonic Strap-On' ) >= 0 && CoC.getInstance().player.hasVagina())) ) {
	...
	Vapula.prototype.vapulaSlave = function() {
		return CoC.getInstance().flags[ kFLAGS.VAPULA_FOLLOWER ] === 1;
	};
	//Two nights after 'vagina enslave', if PC still meets initial requirements (else defer until she does)(Z);
	Vapula.prototype.femaleVapulaRecruitmentPartII = function() {
		...
		CoC.getInstance().flags[ kFLAGS.VAPULA_HAREM_FUCK ] = 1;
		//[Demonic dildo added];
		CoC.getInstance().player.createKeyItem( 'Demonic Strap-On', 0, 0, 0, 0 );
	...
	//Slave Vapula;
	//Camp lines;
	Vapula.prototype.vapulaSlaveFlavorText = function() {
		var choice = Utils.rand( 11 );
		if( choice === 0 ) {
			EngineCore.outputText( 'Vapula is currently resting on a very rough bed of leaves she\'s gathered herself.  She sleeps quietly, her hand absent-mindedly laid on her vagina and the other one resting on her breasts.  A thin trickle of her juice is leaking out of her crotch.' );
		}
		if( choice === 1 ) {
			EngineCore.outputText( 'Your succubus slut, Vaplua, is furiously stroking herself on her bed of leaves, her purple body shining in sweat.  She moans raggedly, often looking up to you with lust-consumed eyes.' );
		}
		if( choice === 2 ) {
			if( CoC.getInstance().flags[ kFLAGS.VAPULA_HAREM_FUCK ] === 0 ) {
				choice = 3;
			} else {
				EngineCore.outputText( 'Vapula is currently absent, probably getting screwed by some of the imps surrounding your camp in order to get her semen rations.  You\'re sure she would come back instantly if you ever needed to give her YOUR cum instead.' );
			}
		}
		if( choice === 3 ) {
			EngineCore.outputText( 'Vapula is busy drinking a vial of milk that looks very much like the kind you\'d usually find on imps.  Her purple cheeks seem to blush darker when she sees you; she awkwardly gulps the rest of the bottle, hiccups, and giggles softly, eyeing you with a coy smile and a lust-filled gaze.  The potion seems to have made her even more aroused than usual.' );
		}
		if( choice === 4 ) {
			EngineCore.outputText( 'Vapula is currently toying with ' );
			if( CoC.getInstance().player.hasKeyItem( 'Deluxe Dildo' ) >= 0 ) {
				EngineCore.outputText( 'your deluxe dildo' );
			} else {
				EngineCore.outputText( 'a dildo' );
			}
			EngineCore.outputText( ', pumping as fast as she can.  In her feverish display of lust she doesn\'t even notice you.' );
		}
		if( choice === 5 ) {
			if( CoC.getInstance().flags[ kFLAGS.VAPULA_HAREM_FUCK ] === 0 || !(CoC.getInstance().scenes.sophieBimbo.bimboSophie()) ) {
				choice = 6;
			}//if harem fucking is on);
			else {
				EngineCore.outputText( 'Vapula is fingering herself while eating out Sophie, your harpy slut.  The dumb avian matron keeps moaning like she\'s in heat while the succubus furiously assaults her lush fuck-hole; both of their pair of wings flap in rhythm as they feel their pleasure rising in waves.' );
			}
		}
		if( choice === 6 ) {
			if( CoC.getInstance().flags[ kFLAGS.VAPULA_HAREM_FUCK ] === 0 || CoC.getInstance().flags[ kFLAGS.IZMA_NO_COCK ] > 0 || !CoC.getInstance().scenes.izmaScene.izmaFollower() ) {
				choice = 7;
			}//if Izma has cock and harem fucking is on);
			else {
				EngineCore.outputText( 'Your succubus is currently very busy with Izma, your tigershark lover.  Izma is standing and hiding Vapula from your sight and you don\'t notice what Vapula is doing to her until you see her frantic bobbing movements, and hear Izma\'s hisses and growls of pleasure.  Obviously your succubus must get the cum she needs from somewhere.' );
			}
		}
		if( choice === 7 ) {
			//if C. Jojo and harem fucking is on);
			if( CoC.getInstance().flags[ kFLAGS.VAPULA_HAREM_FUCK ] === 0 || !CoC.getInstance().scenes.jojoScene.campCorruptJojo() ) {
				choice = 8;
			} else {
				EngineCore.outputText( 'You routinely hear screams of pleasure outside the camp, as well as some frantic male squeaks coming from Jojo, your ' );
				if( CoC.getInstance().scenes.jojoScene.tentacleJojo() ) {
					EngineCore.outputText( 'tentacled ' );
				}
				EngineCore.outputText( 'mouse pet.  Vapula seems to be having a lot of fun with your fuck-toy!' );
			}
		}
		if( choice === 8 ) {
			if( CoC.getInstance().flags[ kFLAGS.VAPULA_HAREM_FUCK ] === 0 || !(CoC.getInstance().scenes.amilyScene.amilyFollower() && CoC.getInstance().scenes.amilyScene.amilyCorrupt()) ) {
				choice = 9;
			}//if C. Amily and harem fucking on);
			else {
				EngineCore.outputText( 'Vapula is busy caressing your corrupted mousette, Amily while softly kissing her breasts.  Her fingers are buried in your fuck-toy\'s pussy, and Amily is moaning whorishly and doesn\'t even bother to stop when she looks at you, her helpless eyes betraying her lack of control over her own lust.' );
			}
		}
		if( choice === 9 ) {
			if( CoC.getInstance().player.findStatusAffect( StatusAffects.CampRathazul ) < 0 ) {
				...
				EngineCore.outputText( 'Vapula is having a very active conversation with Rathazul, the alchemist.  They seem to be discussing the chemical properties of demonic mixtures and the various taints that could occur from their ingestion.  The succubus is holding a bottle of a purplish white fluid that appears to be her own milk, probably waiting for an analysis.' );
			}
		}
		if( choice === 10 ) {
			EngineCore.outputText( 'Vapula is resting on her stomach, a buttplug shoved into her ass.  She doesn\'t seem to notice you watching her butt jiggle under the effect of her unconscious anal contractions.' );
		}
	};
	...
	Vapula.prototype.mouseWaifuFreakout = function( amily, jojo ) {
		...
		EngineCore.outputText( ' up to you, worried, as Vapula struts around the camp.' );
		EngineCore.outputText( '\n\n"<i>' + CoC.getInstance().player.short + ', what is this? Am I dreaming or did you actually bring a demon to your camp? What in the world is wrong with you?</i>"' );
		...
		if( CoC.getInstance().player.lib >= 50 ) {
			EngineCore.outputText( '"<i>Well, as long as she swallows, I don\'t care,</i>" you quip.' );
		}
		EngineCore.outputText( '\n\n"<i>That\'s it!</i>"  The ' );
		if( jojo && amily ) {
			EngineCore.outputText( 'mice are' );
		} else {
			EngineCore.outputText( 'mouse is' );
		}
		EngineCore.outputText( ' raving.  "<i>You\'ve clearly given in to her demonic lust.</i>"' );
		if( jojo ) {
			EngineCore.outputText( '\n\n"<i>I\'m leaving, ' + CoC.getInstance().player.short + ',</i>" Jojo says.  "<i>I only hope for your sake that you come to your senses soon... I will return to my place in the forest when you require assistance in freeing your soul of taint.</i>"' );
			CoC.getInstance().flags[ kFLAGS.JOJO_MOVE_IN_DISABLED ] = 1;
	...
	Vapula.prototype.callSlaveVapula = function( output ) {
		if( output === undefined || output ) {
			output = true;
			EngineCore.clearOutput();
			if( CoC.getInstance().flags[ kFLAGS.FOLLOWER_AT_FARM_VAPULA ] === 0 ) {
				...
				EngineCore.outputText( '\n\n"<i>Want to have some fun, ' + CoC.getInstance().player.short + '?</i>"' );
			} else {
				EngineCore.outputText( 'Vapula mooches over when you call her name, resentfully kicking an imp out of the way as she does. “<i>Yes, [master]?</i>” she says, with exaggerated sweetness.' );
		...
		if( CoC.getInstance().player.hasVagina() && CoC.getInstance().player.hasKeyItem( 'Demonic Strap-On' ) >= 0 ) {
			if( CoC.getInstance().player.lust >= 33 ) {
				fFeed = this.chixFeedVapulaBlehblehIVantToZuckYourSpooo;
			} else if( output && !CoC.getInstance().player.hasCock() ) {
				EngineCore.outputText( '\n\nYou aren\'t suitably aroused to feed Vapula right now.' );
			}
		}
		var threesome = null;
		...
		if( CoC.getInstance().flags[ kFLAGS.VAPULA_EARNED_A_SPANK ] > 0 && !CoC.getInstance().player.isTaur() && !CoC.getInstance().player.isDrider() ) {
		...
		EngineCore.choices( 'Appearance', this.fapulaFapfapfapAppearance, 'Talk', this.talkToVapulaForSomeReason, 'Feed', mFeed, 'Feed(Dildo)', fFeed, 'Threesome', threesome,
			'Spank', spank, '', null, '', null, '', null, 'Leave', CoC.getInstance().scenes.camp.campSlavesMenu );
		if( CoC.getInstance().flags[ kFLAGS.FOLLOWER_AT_FARM_VAPULA ] === 0 && CoC.getInstance().flags[ kFLAGS.FARM_CORRUPTION_STARTED ] === 1 ) {
		...
		if( CoC.getInstance().flags[ kFLAGS.FOLLOWER_AT_FARM_VAPULA ] === 1 && CoC.getInstance().flags[ kFLAGS.FOLLOWER_PRODUCTION_VAPULA ] === 0 ) {
	...
	Vapula.prototype.sendToFarm = function() {
		...
		CoC.getInstance().flags[ kFLAGS.FOLLOWER_AT_FARM_VAPULA ] = 1;
	...
	Vapula.prototype.backToCamp = function() {
		...
		CoC.getInstance().flags[ kFLAGS.FOLLOWER_AT_FARM_VAPULA ] = 0;
	...
	Vapula.prototype.harvestMilk = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.FARM_UPGRADES_REFINERY ] === 0 ) {
			EngineCore.outputText( '“<i>It’ll sure as hell beat whatever disgusting thing the bitch would have me doing instead. Why don’t you make her build a concentration machine? Lethice has loads; makes one dose of cream go so much further.</i>”' );
		} else {
			EngineCore.outputText( '“<i>As you insist, [master]. It’ll sure as hell beat working.</i>”' );
		}

		CoC.getInstance().flags[ kFLAGS.FOLLOWER_PRODUCTION_VAPULA ] = 1;
		EngineCore.doNext( CoC.getInstance().scenes.farmCorruption.rootScene );
	};
	Vapula.prototype.stopHarvest = function() {
		...
		CoC.getInstance().flags[ kFLAGS.FOLLOWER_PRODUCTION_VAPULA ] = 0;
		EngineCore.doNext( CoC.getInstance().scenes.farmCorruption.rootScene );
	};
	...
	Vapula.prototype.fapulaFapfapfapAppearance = function() {
		...
		if( CoC.getInstance().scenes.farmCorruption.hasTattoo( 'vapula' ) ) {
			EngineCore.outputText( '\n\n' );
			if( CoC.getInstance().scenes.farmCorruption.vapulaFullTribalTats() ) {
				...
				if( CoC.getInstance().scenes.farmCorruption.numTattoos( 'vapula' ) > 1 ) {
				...
				if( CoC.getInstance().flags[ kFLAGS.VAPULA_TATTOO_COLLARBONE ] !== 0 ) {
					EngineCore.outputText( CoC.getInstance().flags[ kFLAGS.VAPULA_TATTOO_COLLARBONE ] + '\n' );
				}
				if( CoC.getInstance().flags[ kFLAGS.VAPULA_TATTOO_SHOULDERS ] !== 0 ) {
					EngineCore.outputText( CoC.getInstance().flags[ kFLAGS.VAPULA_TATTOO_SHOULDERS ] + '\n' );
				}
				if( CoC.getInstance().flags[ kFLAGS.VAPULA_TATTOO_LOWERBACK ] !== 0 ) {
					EngineCore.outputText( CoC.getInstance().flags[ kFLAGS.VAPULA_TATTOO_LOWERBACK ] + '\n' );
				}
				if( CoC.getInstance().flags[ kFLAGS.VAPULA_TATTOO_BUTT ] !== 0 ) {
					EngineCore.outputText( CoC.getInstance().flags[ kFLAGS.VAPULA_TATTOO_BUTT ] + '\n' );
				}
			}
		}
	...
	Vapula.prototype.talkToVapulaForSomeReason = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( '"<i>You want to talk?  Well it\'s quite rare to see ' + CoC.getInstance().player.mf( 'studs', 'girls' ) + ' looking for conversation with the likes of me, but go ahead.  What do you want to talk about?</i>"' );
		...
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.RepeatSuccubi ) >= 0 ) {
			if( CoC.getInstance().flags[ kFLAGS.VAPULA_THREESOMES ] === 0 ) {
	...
	Vapula.prototype.toggleCeruleanVapulaAssist = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.VAPULA_THREESOMES ] === 0 ) {
			...
			CoC.getInstance().flags[ kFLAGS.VAPULA_THREESOMES ] = 1;
		}
		//Turn off;
		else {
			//Cerulean Threesome Off (Z);
			EngineCore.outputText( 'You inform Vapula that you don\'t want her to meddle with your night fucks anymore.  Your succubus shrugs.  "<i>Whatever.  As long as I get regular cum rations, I don\'t care much.  I\'ll do as you say.</i>"' );
			CoC.getInstance().flags[ kFLAGS.VAPULA_THREESOMES ] = 0;
		}
		//Return to talk menu;
	...
	Vapula.prototype.toggleVapulaHaremFucks = function() {
		EngineCore.clearOutput();
		//Stop Fucking Harem;
		if( CoC.getInstance().flags[ kFLAGS.VAPULA_HAREM_FUCK ] === 1 ) {
	...
	Vapula.prototype.feedVapulaACupOfJizz = function() {
		...
		EngineCore.outputText( ' with the other.  She sucks like a cock-hungry whore, treating your shaft like a straw as she slurps down your juices; her demonic throat feels incredibly hot and her saliva seems to make your ' + Descriptors.cockDescript( 0 ) + ' grow even harder; her lips squeeze your meat in the most exquisite manner, sending you into short spasms of pleasure every time.' );
		EngineCore.outputText( '\n\nShe works on it for what seems like hours.  You look down at your slut; she keeps staring at you with avid yet playful eyes, never breaking eye contact as she relentlessly throatfucks herself; you feel her lips curve a little as the shape of a ravenous smile appears on her face, and her eyes glitter with an insatiable need.  The naughty bitch! She knows she\'s going to get what she wants, whether you like it or not.  You wish you were able to control yourself, but the covetous succubus never lets go of your ' + Descriptors.cockDescript( 0 ) + ', clinging to it like an animal.' );
		EngineCore.outputText( '\n\nYour stamina is rapidly overwhelmed and you decide to reward her effort; you savagely press Vapula\'s head against your groin, effectively burying all ' + Utils.num2Text( Math.round( CoC.getInstance().player.cocks[ 0 ].cockLength ) ) + ' inches of your ' + Descriptors.cockDescript( 0 ) + ' in her waiting throat; her tongue keeps teasing your rod for a few seconds, and then you cum, brutally.  ' );
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( 'Your ' + Descriptors.ballsDescriptLight() + ' churn' );
		} else {
			EngineCore.outputText( 'Your ' + Descriptors.cockDescript( 0 ) + ' twitches' );
		}
		...
		if( CoC.getInstance().player.cumQ() >= 1500 ) {
			EngineCore.outputText( ' until she looks six months pregnant' );
		}
		EngineCore.outputText( '.  At last, the cum-flow spilling through your urethra starts to ebb and your ' + Descriptors.cockDescript( 0 ) + ' stops throbbing; only then does she removes your junk from her mouth with a loud POP.  Your ' + Descriptors.cockDescript( 0 ) + ' appears to be clean of any spooge: your cock-slut did a very good job.  Satisfied, you pat her head with your cock and let her digest her lavish meal.' );
	...
	Vapula.prototype.vapulaThreesomeMenu = function() {
		...
		//Option: Izma;
		var izma = null;
		if( CoC.getInstance().player.hasCock() || (CoC.getInstance().player.hasVagina() && CoC.getInstance().player.hasKeyItem( 'Demonic Strap-On' ) >= 0) ) {
			if( CoC.getInstance().scenes.jojoScene.campCorruptJojo() ) {
				jojo = this.vapulaJojoThreesomes;
			}
			if( CoC.getInstance().scenes.amilyScene.amilyFollower() && CoC.getInstance().scenes.amilyScene.amilyCorrupt() && CoC.getInstance().player.hasCock() ) {
				amily = this.vapulaAndAmilyThreesome;
			}
			if( CoC.getInstance().scenes.ceraphFollowerScene.ceraphIsFollower() && CoC.getInstance().player.hasCock() ) {
	...
	Vapula.prototype.vapulaCeraphThreesomePleaseBoth = function() {
		...
		EngineCore.outputText( 'You order your girls to turn around; you want to give them a good dicking.  They comply while giggling like a pair of whores, eager to get some cock up their holes.' );
		EngineCore.outputText( '\n\nYou push Vapula onto the floor, making her fall on her back, legs spread.  You then grab hold of Ceraph\'s shoulders and put her on all fours, her warm snatch facing Vapula\'s.  Already moist, the succubus and the omnibus naturally start caressing each other, kissing and groping each other\'s breasts.  You watch them giving in to their own depravity before joining the show; grabbing Ceraph\'s plump butt, you press the tip of your ' + Descriptors.cockDescript( x ) + ' against her backdoor.  The unexpected intrusion makes the omnibus cry, her gasps of joy only enticing you to push further, even at the cost of stretching her anal ring.  A few seconds later you\'re all the way in, enjoying the boiling and tight recesses of her pucker.  You roughly seize Ceraph\'s fuck-pillows and proceed to relentlessly assault her ass, making her yell under the conjugated efforts of your throbbing junk and the hot succubus fingering her from below.  Completely dominated by the pleasure you\'re inflicting on her, your slut starts muttering dirty words, often interrupted by sudden screams, "<i>Fuck... fuck... fuck my ass... ' + CoC.getInstance().player.mf( 'Master', 'Mistress' ) + '...feels so- AAAAAH! Oh FUCK YEAH! Harder, ' + CoC.getInstance().player.mf( 'Master', 'Mistress' ) + '! Fuck your bitch!</i>"' );
		EngineCore.outputText( '\n\nAnnoyed by the noise she\'s making, you slap her firm rump repeatedly, your hands imprinting marks on her red ass-cheeks.  Your buttslut doesn\'t seem to care, completely blissful from being used by her rightful ' + CoC.getInstance().player.mf( 'Master', 'Mistress' ) + '.  She groans, grinds harder and squirms in Vapula\'s embrace in her efforts to impale herself further on your ' + Descriptors.cockDescript( x ) + '.  The sight of your sluts\' love-tunnels mixing their fluids together as they squirt as well as the insane motion of Ceraph\'s wobbling ass sending the sweetest sensations to your rod prove too much for you; despite your best efforts to hold back, you blast Ceraph\'s ass with your baby-batter, the globs effectively packing her anal entrance' );
		if( CoC.getInstance().player.cumQ() >= 1500 ) {
			EngineCore.outputText( ' and swelling her belly until she looks ready to give birth to a full horde of imps' );
		}
		EngineCore.outputText( '.' );
		EngineCore.outputText( '\n\nYou feel your ' );
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( Descriptors.ballsDescriptLight() + ' twitch and your ' + Descriptors.cockDescript( x ) + ' pulsate as they unload their' );
		} else {
			EngineCore.outputText( 'your ' + Descriptors.cockDescript( x ) + ' twitch and pulsate as it unloads its' );
		}
		EngineCore.outputText( ' essence into your omnibus pet, bringing her to orgasm as she blubbers incoherently, "<i>Oh yessssssssss, just like that... Yeah... my ass... cum... yessssssssss...</i>"  Her voice goes so high pitched it trails off, letting you hear Vapula\'s soft moans below.  You quickly pull out your ' + Descriptors.cockDescript( x ) + ', still orgasming.  You hear Ceraph feebly protest at the sudden lack of cock filling her hole, "<i>What are you doing? Put it back! Put it back already!</i>"' );
		EngineCore.outputText( '\n\nWith another slap you tell her to shut up and learn to share; her partner hasn\'t received any cock yet, and you intend to remedy it.  Grabbing Vapula\'s thighs this time, you lunge toward her wet fuck-hole and bury your ' + Descriptors.cockDescript( x ) + ' to the hilt, ' );
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( Descriptors.ballsDescriptLight() + ' hitting her butt-hole with a faint thud' );
		} else {
			EngineCore.outputText( 'thudding up against her cushy butt-cheeks' );
		}
		EngineCore.outputText( '.  The purple sex-goddess squeals, your abrupt dick-digging utterly ravaging her cunt; her unholy warmth and impossibly tight confines are almost enough to make you cum instantly as her vaginal walls wrap in the most sensitive way around your rod, but you manage to hold off the flow of semen churning in your groin.' );
		//[if 2nd cock];
		...
		if( CoC.getInstance().player.cockTotal() > 4 ) {
			EngineCore.outputText( '  Sadly, your extra junk does not have any hole left to fill.  It just hangs there, rubbing against Ceraph\'s backside and teasing her all the more.' );
		}
		...
		//[if multicocks];
		if( CoC.getInstance().player.cockTotal() > 1 ) {
			EngineCore.outputText( '  She expertly jerks each of your rods in turn, gently polishing them as they grow back into an erect state.' );
		}
		EngineCore.outputText( '  She\'s obviously used to this kind of practice; even though [eachCock] felt numb after cumming so hard, you feel your lust build up again as your genitals throb once more with need.  You grab Ceraph\'s head and impale her on your crotch, forcefully throat-fucking her.' );
		...
		EngineCore.outputText( '  Your thoughts are quickly drowned in waves of pleasure, and soon your only thought is to ram more of your ' + Descriptors.cockDescript( x ) + ' down your sluts\' throats, pumping their heads alternatively around your shaft' );
		if( CoC.getInstance().player.cockTotal() > 1 ) {
			EngineCore.outputText( 's' );
		}
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( ' as the girls keep fondling your ' + Descriptors.ballsDescriptLight() );
		}
		EngineCore.outputText( ', bringing you extra tingles of pleasure to your body.' );
		...
		EngineCore.outputText( ', their lips doing their best to milk ' );
		if( CoC.getInstance().player.cockTotal() === 1 ) {
			EngineCore.outputText( 'it' );
		} else {
			EngineCore.outputText( 'them' );
		}
		EngineCore.outputText( ' of all ' );
		if( CoC.getInstance().player.cockTotal() === 1 ) {
			EngineCore.outputText( 'its ' );
		} else {
			EngineCore.outputText( 'their ' );
		}
		EngineCore.outputText( 'cream.  Once your junk feels completely empty, [oneCock] is released with a loud POP.' );
		EngineCore.outputText( '\n\nSatisfied, you kiss both of your sluts.  "<i>Thank you ' + CoC.getInstance().player.mf( 'Master', 'Mistress' ) + '.</i>" they say in unison, before you pat their heads and send them on their way; they will probably need to rest in order to digest their heavy meal.' );
		//lust set to 0;
		CoC.getInstance().player.orgasm();
	...
	Vapula.prototype.vapulaSophieThreeSome = function() {
		...
		EngineCore.outputText( 'You call Sophie insistently, panting in anticipation.  The blond-feathered bimbo doesn\'t come at first - perhaps the dumb slut forgot her name again - but as you grow more impatient you finally notice the platinum-headed woman, her enormous boobs bouncing hypnotically and her luscious thighs swaying lustfully as she walks.  She grins beatifically and coos: "<i>Yes?  Is there anything I can, like, do for... ooooh, but who\'s this?  She\'s sooooooo beautiful!  She\'s got, like, big boobs and stuff!</i>"  Vapula sighs in consternation.  "<i>What can you do with her, ' + CoC.getInstance().player.short + '? I\'m sure she\'s going to forget my presence by next hour.</i>"' );
		...
		EngineCore.outputText( '\n\nWithout ceremony you grab the avian slut\'s fuckpillows and eagerly sandwich your meat between them, pressing the tip of your ' + Descriptors.cockDescript( x ) + ' against her tight anus.  As soon as the first inches start sliding in, the bimbo slut flaps her wings frenetically as she squawks in pleasure, her anal muscles contracting on their own as an invitation to slide in further.  Before you even think of it ' );
		if( CoC.getInstance().player.cockArea( x ) > 75 ) {
			EngineCore.outputText( 'you\'ve filled the entirety of her colon' );
		} else {
			EngineCore.outputText( 'your cock is already buried to the hilt, its mammoth girth bulging between her broad hips' );
		}
		EngineCore.outputText( '; the bulge even grinds a bit against Vapula\'s own butt and the succubus reaches toward you, entranced with lust.  She wraps her arms around you, letting you feel her tender boobs being squeezed against your [chest] as she gives you a mad kiss filled with passion.  Your lips remain locked for a while as you explore each other\'s mouths with your tongues; you almost forget the burning hot and roughly tight hole.  It entraps your ' + Descriptors.cockDescript( x ) + ' as it continues massaging with the harpy\'s vice-like anal muscles.  The bimbo keeps wriggling arond your rod in order to get more of it inside, pleasantly smashing her fat rear against your groin.' );
		EngineCore.outputText( '\n\nStill not breaking the kiss, you groan silently and pull out your ' + Descriptors.cockDescript( x ) + ', all smeared in pre-cum.  Sophie whimpers and wails softly until you thrust all the way back in, literally jamming her with your junk.  Her pupils widen, her mouth opens and her gigantic tits keep swinging in the dirt as her whole body trembles and shudders from the brutal anal violation.  Her cry of pleasure is so high-pitched you barely hear her subsequent moan; satisfied, you start thrusting at a more convenient rhythm, enjoying the lush depths of the avian slut as the succubus keeps making out with you.  You keep butt-fucking Sophie, unable to focus on the ministrations of the expert succubus or the extremely warm recesses of the harpy\'s interior.  You regularly slap her plush, fat buttocks, sending ripples across her whole fleshy body. Needless to say, the bimbo\'s pussy is completely drenched in fem-spunk, her wide pussy drooling trickles after trickles of her juices, betraying her utter arousal.' );
		//[if 2nd cock];
		if( CoC.getInstance().player.cockTotal() > 1 ) {
		...
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( 'balls and massages your ' );
		}
		EngineCore.outputText( Descriptors.buttDescript() + '.' );
		//[if third cock];
		if( CoC.getInstance().player.totalCocks() > 2 ) {
			EngineCore.outputText( '  Using her extra flexibility, she bends over and start jerking your third pole, the sweet handjob adding to your pure cocktail of pleasure.' );
		}
		EngineCore.outputText( '  You can feel her hard nipples poke at your skin as they gently scrape across your torso.  Vapula hardly ever interrupts her embrace, only pulling back from your lips to take care of other intimate parts of your body.  You keep riding the bimbo senseless, making her squirm and squawk in ecstasy as your own cries of pleasure are muffled by Vapula\'s voracious lips.' );
		...
		CoC.getInstance().scenes.sophieScene.luststickApplication( 10 );
		CoC.getInstance().player.orgasm();
	...
	Vapula.prototype.vapulaSophieThreesomeCunts = function() {
		...
		EngineCore.outputText( '\n\nVapula sighs in consternation.  "<i>What can you do with her, ' + CoC.getInstance().player.short + '? I\'m sure she\'s going to forget who I am in an hour.</i>"' );
		...
		EngineCore.outputText( '\n\nWithout ceremony you grab the avian slut\'s fuckpillows and eagerly sandwich your dildo between them, pressing the tip of it against her tight anus.  As soon as the first inches start sliding in, the bimbo slut flaps her wings frenetically as she squawks in pleasure, her anal muscles contracting on their own as an invitation to slide in further.  Before you even think of it your strap-on is already buried to its leather hilt, its mammoth erection bulging between her broad hips; the bulge even grinds a bit against Vapula\'s own butt and the succubus reaches toward you, entranced with lust. She wraps her arms around you, her tender boobs against your own ' + Descriptors.allBreastsDescript() + ' as she gives you a mad kiss filled with passion. Your lips remain locked for a while as you explore each other\'s mouths with your tongue; you almost forget the tight hole wrapped around your dildo.  You are brought back to it by the bimbo wriggling around the rod rammed in her rear in an attempt to squeeze more of it into her, twisting your end around in your sopping ' + Descriptors.vaginaDescript( 0 ) + ' as she pleasantly mashes her fat rear against your groin.' );
		...
		EngineCore.outputText( '\n\nVapula shifts her attention to your breasts, her long tongue running lasciviously over your ' + Descriptors.nippleDescript( 0 ) + 's, tightly circling and gently teasing their tips until they are as sensitive and erect as any clitoris.  Her soft lips edge you further and further into your fuck daze, riding the bimbo senseless, making her squirm and squawk in ecstasy as your own cries of pleasure are muffled by Vapula\'s voracious lips.' );
	...
	Vapula.prototype.vapulaAndAmilyThreesome = function() {
		...
		EngineCore.outputText( '\n\nThe little mousy looks down awkwardly, intimidated by the towering succubus.  "<i>Y-yes ' + CoC.getInstance().player.mf( 'Master', 'Mistress' ) + '...</i>"' );
		...
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( ': her drooling pussy needs to be prepared because you\'re going to take her next' );
		}
		EngineCore.outputText( '.  Vapula smirks in excitement and eagerly shoves her tumescent pussy in front of the mousette\'s waiting lips; Amily keeps wagging her butt impatiently, looking forward to being used as a disposable anal-toy.  You wink at Vapula; she\'s going to like it.' );
		EngineCore.outputText( '\n\nGrabbing hold of your mouse\'s plush buttcheeks, you stretch her anal opening and in a brutal, powerful thrust, you slide all of your ' + Descriptors.cockDescript( x ) + ' down her interior.  Amily immediately squeaks, her mouth drooling and her whole body twitching as she adapts to the erect prick inside her.  Her cries of pleasure are suddenly muffled as Vapula, too turned on to care about your mousette, brutally grabs her head and squeezes her face between her legs.  In the meantime, you start pumping at her ass, effortlessly moving your hips backwards and forwards as your ' + Descriptors.cockDescript( x ) + ' ravages the mousegirl\'s insides.  The poor creature, already driven over the edge by your relentless anal pounding, is now forced to lick the succubus\' luscious pussy, her tongue being forced into probing her lush depths at each thrust received from behind.  She keeps moving back and forth between Vapula and you, roughly mishandled as both ends of her body are being abused.  However, this does not seem to decrease in the least the pleasure she\'s getting. She joyfully slams back her ass against your groin at every thrust, her insides always hungry for more dickflesh.  Your hips are quasi-immobile as she impales herself on her own in an insatiable cock-hunger.' );
	...
	Vapula.prototype.vapulaAssistsCeruleanSuccubus = function() {
		...
		EngineCore.outputText( '\n\nYou tell the blue woman not to worry about that and gently pat Vapula\'s head; the purple demoness coughs as she interrupts her divine blowjob and giggles when she notices one of her own kind.  She slowly gets up and is soon joined by your demon guest, awaiting your next orders.  The blue succubus and the purple succubus stand before you, their eyes filled with voracious lust and impatient need.  Two girls are eager for your cum, and there\'s only one ' + CoC.getInstance().player.mf( 'man', 'herm' ) + ' to satisfy them both... You don\'t know if you\'ll be up to the task, but it sure can\'t hurt to try.  Smiling to both, you remove your ' + CoC.getInstance().player.armorName + ' and lazily lie down, your erect ' + Descriptors.cockDescript( x ) + ' standing provocatively.  "<i>Well, come on ladies.  Please me.  Can\'t you see I need some satisfaction?</i>"' );
		EngineCore.outputText( '\n\nAt this invitation, both luscious creatures practically jump you.  In no time you\'re covered with blue and purple flesh, arms and lips caressing you everywhere but focusing on your most sensitive spots.  You shiver several times as the hell-girls give you tingles of pleasure that tease you almost unbearably.  You feel hands and tongues being pressed against your ' + Descriptors.cockDescript( x ) + ' and smearing it with saliva and pre-cum; you are kissed several times, your mouth is being padded with a flexible demonic tongue as your palate is filled with a sweet unholy taste; the scent of horny succubi makes you giddy and all the more aroused.  You feel nipples being stuffed in your mouth and you instinctively suck, gulping the succubi\'s essences like a newborn.' );
		EngineCore.outputText( '\n\nDrops of some fluid start falling on your face; looking up, you see nothing but a drooling, gushing pink pussy being lowered down.  Idly wondering whose it might be, you start probing your tongue into that moist vagina, eagerly sucking and licking as the whore atop you moans and bucks wildly; from her hoarse, ragged voice you can recognize Vapula.  Gods, she\'s such a horny succubus slut!  You keep eating her out until she screeches madly in pleasure, probably waking up every living creature around your camp.  At the same time, you feel your ' + Descriptors.cockDescript( x ) + ' is being handled and inserted into a nice warm snatch.  The brutal temperature change and amazing tightness drives you near the edge, and you actually feel your spooge flowing up and down your urethra as you contract your hips in a desperate effort to control yourself.  Your groin harshly hits the blue succubus\', and your ' + Descriptors.cockDescript( x ) + ' digs even deeper in her innermost depths.  She sighs in contentment and slowly whispers, "<i>Guests first.</i>"' );
		EngineCore.outputText( '\n\nShe starts riding you ferociously, clamping you down and gripping your hips with claw-like hands.  Her vaginal muscles implacably contract around your ' + Descriptors.cockDescript( x ) + ', acting as an boiling-hot and inescapable cock-ring.  Still trying your best to explore Vapula\'s love-tunnel, you reach around to get some of her tits, only to find the blue succubus\' hands pinching your slut\'s nipples, making her shriek in torturesome pleasure.  You both keep groping her voluptuous body, making her shudder under your abusive massages.  Your head is now entirely soaked with Vapula\'s juices - that whore seems to orgasm ridiculously often.  Every time you consider pulling back your tongue and gasping for some fresh air, her twat squirts another crazy jet of fem-spunk down your face, making your vision blurry and leaving you with no choice but to lick her interior clean.  However, your efforts only stimulate her all the more, leading you to wonder how she can generate such an absurd amount of vaginal juices.' );
		//[if multicock];
		if( CoC.getInstance().player.cockTotal() > 1 ) {
			EngineCore.outputText( '  She suddenly seems to have an idea: leaning down to reach your second cock, she starts licking it, accompanying the savage thrusting of the blue demoness as she slurps your extra junk, giving you even more pleasure.' );
		} else {
			EngineCore.outputText( '  The blue demoness leans down and starts licking Vapula\'s moist pussy, intertwining her flexible tongue with yours and pleasuring the purple slut even more.' );
		}
		...
		if( CoC.getInstance().player.tallness < 60 ) {
			EngineCore.outputText( ', enclosing your head in her cleavage' );
		}
		EngineCore.outputText( '.  "<i>That was fun! I can\'t wait to have another party like this! You should invite her more often!</i>"' );
	...
	Vapula.prototype.NTRSomeJojos = function() {
		...
		if( CoC.getInstance().player.hasCock() ) {
			...
			EngineCore.outputText( '"<i>Admit it, you\'d love to use that smoking hot body, wouldn\'t you?  Don\'t lie to me.  You\'d give up your soul in order to fuck her brains out.  Don\'t you want to plunge your dick inside her?  Don\'t you want to fuck my beautiful succubus wife?</i>"' );
			...
			EngineCore.outputText( '\n\n"<i>These hips... these thighs...</i>"  Grabbing hold of Vapula\'s waist, you shove your ' + Descriptors.cockDescript( x ) + ' between her legs and start grinding your junk against her vaginal entrance, teasing her in the most lustful manner. You keep grinding until your purple slut utters a lubricious moan.' );
			EngineCore.outputText( '\n\n"<i>No, really, her whole body is perfect.</i>"  You run your hands all over the succubus curves, pinching here and caressing here.  "<i>And gods, that tight anus!  Oh, Jojo, if you knew how cock-hungry she is...</i>"  With a wicked, wanton smile, you rail Vapula\'s ass with your ' + Descriptors.cockDescript( x ) + ', filling the entirety of her interior with your dickflesh as you watch the monk.  The poor mousy slut has completely given in to his lust; he has fallen on his knees and is now stroking himself feverishly, unable to break your gaze.  You start pounding Vapula\'s butt; with each thrust you grunt ostensibly, letting your mouse pet know how much you enjoy it.  As for Vapula, the slutty demoness is clearly enjoying her anal treatment, the sight of a poor bitch masturbating to her being all the more arousing to her.  She also stares at him with longing eyes, as if to provoke him with her nude, horny body being fucked by someone else.  Soon she is screaming in utter ecstasy and starts babbling meaningless dirty talk, "<i>Fuck, fuck me... oh yessssssssssssss... more!  MORE!</i>"  She\'s clearly acting as a naughty bitch!  You wonder if the presence of a shy creature is helping to release her inner wantonness... Shrugging it off, you decide to focus on the nice purple body impaling itself on your ' + Descriptors.cockDescript( x ) + ' and pound her harder, your meat releasing streams of sex juices as it pounds in and out of Vapula\'s pucker.' );
			if( CoC.getInstance().player.cockTotal() > 1 ) {
				...
				//[if third cock];
				if( CoC.getInstance().player.cockTotal() > 2 ) {
					EngineCore.outputText( '  Since the horny slut has a demonic vaginal capacity, you might as well fit more dick in it: in no time your third dick joins its colleague in this infernal vaginal assault.' );
				}
				EngineCore.outputText( '  ' );
			} else {
				EngineCore.outputText( '\n\n' );
			}
			EngineCore.outputText( 'You push in and out of her, claiming both her holes and snickering at the poor mouse thing that can get neither. With a mighty push, you put Vapula on all fours and lean down in order to touch more of her supple body.  Your purple slut is now dying with pleasure, her tongue hanging and her eyes staring behind Jojo as the monk squeaks pitifully in his pathetic self-stroking.' );
			...
			EngineCore.outputText( ' being filled with cock.' );
			...
			if( CoC.getInstance().scenes.jojoScene.tentacleJojo() ) {
				EngineCore.outputText( 'his tentacles start flying toward his ass, ' );
			}
	...
	Vapula.prototype.jojoButtFuckTrain = function() {
		...
		EngineCore.outputText( 'You tell Jojo it\'s his lucky day - you\'re going to have a joyful butt-fuck session with him and your purple pet.' );
		EngineCore.outputText( '\n\n"<i>Admit it, you\'d love to use that smoking hot body, wouldn\'t you?  Don\'t lie to me.  You\'d give up your soul in order to fuck her brains out.  Don\'t you want to plunge your dick inside her?  Don\'t you want to fuck my beautiful succubus wife?</i>"' );
		EngineCore.outputText( '\n\nJojo shakes his head in denial, but he can hardly keep any shred of credibility when his rock-hard mouse cock speaks for him.' );
		...
		EngineCore.outputText( '\n\nYou seize your ' + Descriptors.cockDescript( x ) + ' and press its tip against the mouse\'s sphincter, slowly pushing.  Startled by the sudden anal invasion, Jojo reflexively grabs Vapula\'s hips and inserts his rodent prick into her.  The simultaneous penetrations make your sluts squeak and moan in ecstasy, their bodies entranced as their anal walls provide them one thousand subtle sensations.  You grip his arms, using him as leverage to pull out, and slam it back home again.  Used like the fucktoy he truly is, Jojo doesn\'t resist; his dick moves of its own volition in and out of Vapula\'s interior as you relentlessly push him back and forth.  You secretly rejoice: his tight little ass is clearly adapting to your ' + Descriptors.cockDescript( x ) + ' and it somehow feels even better than usual.  You pound hard, making him groan and beg for more; you eagerly give him what he wants and stuff him full of your dickflesh.' );
		if( CoC.getInstance().player.cockArea( x ) > 100 ) {
			EngineCore.outputText( '  You can actually feel his internal organs being bumped against your meat as your ' + Descriptors.cockDescript( x ) + ' stretches his colon beyond its normal size and thrusts in and out like a charging mammoth.' );
		}
		...
		if( CoC.getInstance().scenes.jojoScene.tentacleJojo() ) {
			...
			if( CoC.getInstance().player.hasVagina() ) {
				EngineCore.outputText( '  Last but not least, a soft cock-knock at the entrance of your ' + Descriptors.vaginaDescript() + ' signals an incoming intrusion.  If your mouth weren\'t so full of cock you would sigh in relief as your aching pussy is thoroughly penetrated.' );
			}
		}
		//[if PC has extra long tentacle cock, variable is n1];
		var tentacles = _.filter( CoC.getInstance().player.cocks, function( cock, index ) {
			return cock.cockType === CockTypesEnum.TENTACLE && index !== x;
		} );
		if( tentacles.length > 0 ) {
			EngineCore.outputText( '\n\nA deliciously depraved idea crosses your mind; using your dick muscles, you move your ' + Descriptors.cockDescript( tentacles[ 0 ] ) + ' toward Vapula.  It squirms for a moment as you look for her wet snatch, poking her everywhere with your tip and teasing her with your accidental ministrations, but at last you find her vaginal entrance.  Using the rubbery flexibility of your plant-like prick, you brutally shove it down her love-tunnel, trying to force inside as many inches as you can.  You can feel her instinctively altering her own body in order to accommodate more dickflesh inside her, and you keep pushing until your ' + Descriptors.cockDescript( tentacles[ 0 ] ) + ' is stretched to maximum capacity.  Somehow your tentacle rod is entirely buried in Vapula\'s warm depths, and her unbelievable tightness is driving you mad with lust.  It twists on its on volition inside her, its vegetable-like texture being more sensitive and receptive to pleasure than a normal prick and the sensations it sends to you are abnormally exquisite.  You almost forgot the asshole you were pounding with your ' + Descriptors.cockDescript( x ) + ' but the way Jojo wriggles against it in order to fit more of your length inside him reminds you of your job.' );
		}
		//[if PC has another tentacle cock, variable is n2];
		if( tentacles.length > 1 ) {
			EngineCore.outputText( '\n\nSince your sluts still have free holes left, you decide to fill some more fuck-holes with your junk.  Guiding  your ' + Descriptors.cockDescript( tentacles[ 1 ] ) + ' toward Vapula\'s mouth, you think you could use a proper blowjob from a trained succubus and wait for her to let out another moan before filling your junk completely and thrusting up and down brutally, literally mouth-fucking her and treating her lips as a tight cock-ring.  Her expert tongue wraps around your girth, squeezing it and clinging to it like it\'s the most precious thing in her life.  She keeps jerking back and forth due to the hard butt-fucking she\'s receiving, and the jolts are transmitted to your ' + Descriptors.cockDescript( tentacles[ 1 ] ) + ' in the most teasing manner.' );
		}
		//[if PC has another tentacle cock, variable is n3];
		if( tentacles.length > 2 ) {
			EngineCore.outputText( '\n\nSince Vapula is already sucking you off, why not Jojo?  You still have appropriate junk left after all.  You promptly send your wriggling ' + Descriptors.cockDescript( tentacles[ 2 ] ) + ' and push it between the mousey\'s lips.  However, the slut won\'t open them!  You pound him ragingly, slamming the entirety of your ' + Descriptors.cockDescript( x ) + ' down his colon and bruising his firm butt until he lets out a deep howl of pleasure and pain mixed; you profit from the opening and stuff his mouth so full of your ' + Descriptors.cockDescript( tentacles[ 2 ] ) + ' you think he\'s going to choke.  However, his mouth seems to be accustomed to take monstrous lengths - probably a side-effect of his corruption - and he quickly accommodates to your size.  Although he can\'t provide a blowjob as sweet as Vapula, you can definitely tell he\'s skilled at sucking dick.  The cute little slut!  You ram his asshole faster to reward him; this increased butt-fucking pace seems to have the desired effect, and soon your ' + Descriptors.cockDescript( tentacles[ 2 ] ) + ' is throbbing in need as is bathes in the mousey\'s saliva.' );
		}
	...
	Vapula.prototype.vapulaAndIzma = function( girls ) {
		...
		if( girls !== undefined && !girls ) {
			EngineCore.outputText( 'You call your beta tigershark, telling her you\'ll have some use for her body.  Izma eagerly approaches you, although she frowns a bit when she sees the towering succubus accompanying you.  "<i>What can I do for my alp-0... wait, what are you doing with her?</i>"  Vapula only smirks at Izma\'s puzzled reaction.' );
			EngineCore.outputText( '\n\nYou gently explain that you intend to have some extra fun, that\'s why you brought this gorgeous creature to please you both.  Izma isn\'t really convinced, but still walks up to you, not willing to antagonize her alpha.  You tell to remove her cloth and turn around so you can have a clear view of her toned butt.  "<i>What do you have in mind, ' + CoC.getInstance().player.short + '?</i>"\n\n"<i>Shhh... just do as I say.</i>"  With a discreet gesture, you order Vapula to get on her knees in front of Izma; she\'s going to feed from your tigershark this time.  With a conniving smile, Vapula obeys and before your shark lover can react, you\'re already grinding your ' + Descriptors.cockDescript( x ) + ' against her muscular butt-cheeks while the purple succubus grabs hold of her 15 inch long pecker.  You rub the tip of your ' + Descriptors.cockDescript( x ) + ' against her butt crack.  "<i>W-wait! What are you guys doing? Are you going to fuck my... I don\'t want to-</i>"' );
			...
			EngineCore.outputText( '\n\nYour taunts don\'t seem to affect Izma; if anything, they only turn her on all the more.  You tease Izma\'s butthole with your hands and ' + Descriptors.cockDescript( x ) + ', stretching her fuckpillows to make room for your incoming junk.  The poor tigershark doesn\'t even resist anymore, completely overwhelmed by your teasing ministrations and Vapula\'s dexterous fondling.  Her shark tail is wagging in apprehension and excitement; her loose pussy is now freely gushing juices and her tight asshole is occasionally leaking some lubricant.  She\'s finally getting ready!' );
			EngineCore.outputText( '\n\nWith a roar of triumph, you grab her toned hips and force your ' + Descriptors.cockDescript( x ) + ' inside her, tearing her asshole and stretching it further.  Izma screeches, obviously not used to anal violation.  Her shark tail twists and wriggles nervously above her ass, nearly hitting you.  With one hand, you immobilize her inconvenient appendage; with your free hand, you grasp again on her toned buttocks, gripping the hard muscles as your herm lover contracts them; her anal walls are squeezing your ' + Descriptors.cockDescript( x ) + ' so tightly it actually hurts.  You tell Vapula to suck harder in order to relieve your tigershark\'s tension.  Your skilled succubus nods and starts clamping her pink lips around Izma\'s mammoth prick, effectively trapping it in an organic cock-ring.  At last you feel some of the intense pressure on your ' + Descriptors.cockDescript( x ) + ' being released, allowing you to pull out and slam back, almost bruising your groin as it mashes against her hard butt-muscles.  This time her ass completely gives in to your ferocious dick-assault.  You pull back and stuff her again, increasing the pace of your repeated thrusts as the both of you gain confidence.  Soon you\'re butt-fucking her senseless, devastating her rectal entrance with your towering rod.' );
			EngineCore.outputText( '\n\nNeedless to say, Izma\'s mind is being torn in pleasure; the sweet blowjob she\'s receiving from your voracious succubus contrasts with your rough anal ploughing and brings her the weirdest mix of sensation. She growls and moans, her hands grasping Vapula\'s head as its bobs up and down the length of her long prick.  Her pussy is dripping drop after drop of fem-juice.  You notice her asshole is getting wetter and wetter as you keep penetrating it; could it be that your tigershark lover is a buttslut in denial?  Excited at the idea of turning the prude tigershark into an eager anal beta, you intensify the pace of your butt-fucking, determined to bring her to ecstasy by the sole power of your ' + Descriptors.cockDescript( x ) + '. You want her to cum from her butt being devastated, certainly not from a petty blowjob, even from a succubus.  Vapula rapidly takes note of your sudden change of rhythm; she grins wickedly and starts taking Izma\'s whole dickflesh into her mouth, effortlessly deepthroating her.  Izma, being utterly crushed between her two lovers, squirms and thrashes savagely; it won\'t be long before she cums from your combined throat- and butt-fucking.' );
			EngineCore.outputText( '\n\nAs you give your tigershark lover another raging thrust, her groin hits Vapula\'s face, and the monstrously thick rod bumps against the back of her throat, making her cough unintentionally.  With a triumphal cry, you seize Izma\'s breasts, kneading them harshly as you pinch her nipples, caring little about her comfort in your fiery passion.  The extra stimulation is enough to make the tigershark cum, and her gaping pussy releases torrents of fem-spunk, soaking the floor with her juices.  Her orgasm is sending shivers all across her body and the way her body is trembling in your arms is enough to send you over the edge: you release a milky flood in her anus, packing her interior with seed; the contact of your goo being shot across her colon triggers her masculine orgasm, and she lets out a torrent of her own spunk into the succubus\' waiting stomach, unloading her load into your purple slut as you unload yours into her.  The flow of fluids flowing in and out of her body makes her thrill in ecstasy until your ' + Descriptors.cockDescript( x ) + ' stops vibrating, running out of spooge to shoot. Vapula keeps gluttonously sucking her subsistence out of Izma\'s traitorously long cock, squeezing her balls in order to extract as much semen as she can.' );
			EngineCore.outputText( '\n\nNodding in approval, you pull out your own ' + Descriptors.cockDescript( x ) + ' out of your tigershark\'s ass and tell Vapula to keep going.  With your rod still in hand, you walk around Vapula and grab hold of her plush, jiggly ass; without warning, you plunge your still-dripping rod into her tight entrance.  Unsurprisingly, the anal penetration is much easier with the purple succubus.  She jolts a bit but this doesn\'t reduce the intensity of her ministrations in the least; her tainted saliva appears to have an aphrodisiac effect on Izma\'s cock, never letting her go completely soft.  You pump your ' + Descriptors.cockDescript( x ) + ' in and out of Vapula\'s plump rump while she still sucks off your tigershark lover, letting her balls fill back up with cum.  The three of you go at it for what seems like hours, Vapula wobbling back and forth between your hard butt-fuck and Izma\'s dick.  Her asshole is somehow as tight as Izma\'s, but so wonderfully hot!  The sudden change of temperature is making you shiver in pleasure.  You keep pounding away, not really caring about a slutty creature used to butt-dickings.' );
			EngineCore.outputText( '\n\nYou stare at Izma, the two of you keeping eye contact as you both penetrate your fucktoy from both ends.  Then, at last, you wink at Izma as you both cum a second time, releasing simultaneously streams of spunk in the succubus\' horny body.  She happily devours all you\'ve got, her lips and anal-lips contracting mercilessly in order to milk both cocks fresh baby-batter.  You cum and cum, your ' + Descriptors.cockDescript( x ) + ' and Izma\'s pulsating in rhythm as they paint the succubus\' inner depths white.  You then pull out of her ass and walk toward Izma.  The poor tigershark is completely drained from the double-blowjob, and staggers her way toward the stream to get hydrated, pulling back her dick with a loud POP as it escapes from Vapula\'s needy lips.  The succubus looks disappointed to see her new sperm feeder running away, but you remind her your own ' + Descriptors.cockDescript( x ) + ' still needs to be cleaned. With a happy sigh, she works on your junk for a while, tingling you in the most pleasant way as her tongue runs across your dickflesh and slurps all the remaining goo. When you\'re as empty as she\'s full, you both rest side to side, recovering from the intense fuck.' );
		}
		//Vapula/Izma threesome (as female);
		else {
			EngineCore.outputText( 'You call your beta tigershark, telling her you\'ll have some use for her body.  Izma eagerly approaches you as you slip into your strap-on, only starting to frown when she sees the towering succubus accompanying you.  "<i>What can I do for my alp-0... wait, what are you doing with her?</i>"  Vapula only smirks at Izma\'s puzzled reaction.' );
			EngineCore.outputText( '\n\nYou gently explain that you intend to have some extra fun, and you\'ve brought this gorgeous creature to please you both.  Izma isn\'t really convinced, but still walks up to you, not willing to antagonize her alpha. You tell to remove her cloth and turn around so you can have a clear view of her toned butt.  "<i>What do you have in mind, ' + CoC.getInstance().player.short + '?</i>"\n\n"<i>Shhh... just do as I say.</i>"  With a discreet gesture, you order Vapula to get on her knees in front of Izma; she\'s going to feed from your tigershark this time.  With a conniving smile, Vapula obeys and before your shark lover can react, you\'re already grinding your obscene dildo against her muscular butt-cheeks while the purple succubus grabs hold of her 15 inch long pecker.  You rub the tip of your strap-on against her butt crack, sighing as the movement sends pleasurable vibrations down into your end.' );
			...
			EngineCore.outputText( '\n\nNodding in approval, you pull the dildo out of your tigershark\'s ass with a wet pop and tell Vapula to keep going.  With your rod still in hand, you walk around Vapula and grab hold of her plush, jiggly ass and without warning stuff your still-dripping strap-on in her ass; unsurprisingly, the anal penetration is much easier with the purple succubus.  She jolts a bit but this doesn\'t reduce the intensity of her ministrations in the least; her tainted saliva appears to have an aphrodisiac effect on Izma\'s cock, never letting her get completely soft.  You pump your dildo in and out of Vapula\'s plump rump, enjoying the pink end stroking your dripping ' + Descriptors.vaginaDescript( 0 ) + ' while she still sucks off your tigershark lover, letting her balls fill back up with cum.' );
			...
			EngineCore.outputText( '\n\nShe happily devours all you\'ve got, her lips and nether-lips contracting mercilessly in order to milk both cocks for fresh baby-batter.  The sight of the shameless cumbucket getting what she wants pushes you to a new height and you orgasm and orgasm, your end of the strap-on vibrating furiously in your streaming depths as the other end and Izma pulsate in rhythm and paint the succubus\' inner depths white.  You then pull out of her ass and walk toward Izma. The poor tigershark is completely drained from the double-blowjob, and staggers her way toward the stream to get hydrated, pulling back her dick with a loud POP as it escapes from Vapula\'s needy lips.  The succubus looks disappointed to see her new sperm feeder running away, but you remind her you still need to be cleaned.  With a happy sigh, she works on your ' + Descriptors.vaginaDescript( 0 ) + ' for a while, tingling you in the most pleasant way as her tongue runs slowly across your drooling sex and slurps up all the girl slime she can possibly tease out of you.  When you\'re as dry as she is full, you both rest side to side, recovering from the intense fuck.' );
		}
		CoC.getInstance().player.orgasm();
	...
	Vapula.prototype.teaseVapula = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.VAPULA_HAREM_FUCK ] === 0 && (CoC.getInstance().flags[ kFLAGS.VAPULA_DAYS_SINCE_FED ] === 0) ) {
			...
			CoC.getInstance().flags[ kFLAGS.VAPULA_TEASE_COUNT ]++;
		}
		//Second Tease;
		//Requires: Vapula not fucking harem, teased the day before;
		else if( CoC.getInstance().flags[ kFLAGS.VAPULA_HAREM_FUCK ] === 0 && CoC.getInstance().flags[ kFLAGS.VAPULA_DAYS_SINCE_FED ] === 1 ) {
			EngineCore.outputText( 'You smile as once again Vapula gets into position on her knees, and you buckle on the strap-on and stand over her.  This time you make the pink end grow long, closing your eyes as it inches up your tunnel, before slowly thrusting your ' + Descriptors.hipDescript() + ' backwards and forwards in front of your succubus, letting it shrink and grow so it climbs up and down your wet walls.' );
			...
			//[Hooves:;
			if( CoC.getInstance().player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_HOOFED || CoC.getInstance().player.isTaur() ) {
				EngineCore.outputText( '"<i>I would worship your feet, mistress.  I would clean your hooves with my tongue to a shine, tease out every scrap of dirt from every crevice.  And I would thank you for the privilege.</i>"' );
			}//[Naga:;
			...
			} else if( CoC.getInstance().player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_DOG || CoC.getInstance().player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_CAT || CoC.getInstance().player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_LIZARD || CoC.getInstance().player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_KANGAROO || CoC.getInstance().player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_FOX ) {
			...
			//[Insect:;
			else if( CoC.getInstance().player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_BEE || CoC.getInstance().player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_CHITINOUS_SPIDER_LEGS ) {
		...
		//Third Tease;
			...
			EngineCore.outputText( 'Once again you buckle up, sighing with pleasure as you sink the pink dildo into your moist hole, and once again you wait until Vapula gets onto her knees before you.  She looks at you silently and sullenly as you droop the monstrous purple end over her face.' );
			...
			if( CoC.getInstance().player.isNaga() ) {
				EngineCore.outputText( 'you slide your tail under and over her trunk, wrapping your coils around her and drawing her face into your stuffed crotch.' );
			}//Other:;
			else if( CoC.getInstance().player.isTaur() || CoC.getInstance().player.isGoo() || CoC.getInstance().player.isDrider() ) {
				EngineCore.outputText( 'you sit the monstrous half of your body down comfortably and draw her face into your stuffed cotch.' );
			} else {
				EngineCore.outputText( 'you open your [legs] and wrap them around her head, drawing her face into your stuffed crotch.' );
			}

			EngineCore.outputText( '\n\nShe works on it for what seems like hours.  You look down at your slut; she keeps staring at you with avid yet playful eyes, never breaking eye contact as she relentlessly throatfucks herself; you feel her lips curve a little as the shape of a ravenous smile appears on her face, and her eyes are glittering with an insatiable need.  The naughty bitch!  She knows she\'s going to get what she wants, whether you like it or not.  You wish you were able to control yourself, but through the sheer force of her obscene hunger the succubus is somehow able to control your end of the strap-on, making the dildo shrink, pulsate and push into your ' + Descriptors.vaginaDescript( 0 ) + ' involuntary, which soon has you panting, groaning, and thrusting back into her to sate your own growing need.' );
			EngineCore.outputText( '\n\nYour stamina is rapidly overwhelmed and you decide to reward her effort; you savagely press Vapula\'s head against your groin, burying all eight inches of your false cock into her waiting throat; in symphony the two of you will your throbbing dildo to expand to pack you mercilessly tight, making you cum brutally.  Your ' + Descriptors.clitDescript() + ' pulses as your vagina contracts ecstatically around the bulging sex toy; vaguely you feel release at the other end, and you hear Vapula loudly and shamelessly enjoy what the dildo pumps out.  You abandon yourself to the climax and keep thrusting your ' + Descriptors.hipDescript() + ' into her face, girlcum spurting around your harness.' );
	...
	Vapula.prototype.chicksFeedVapula = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.VAPULA_TEASE_COUNT ] > 1 ) {
			...
			EngineCore.outputText( 'You tell Vapula you\'re going to reward her for being such a good slut; pushing the pink end of the dildo into you with a sigh, you strap yourself into the harness so that the monstrous purple end bobs out in front of you.  You giggle as Vapula grabs hold of it and proceeds to lick it, coating the entirety of your artificial junk with her warm, sweet saliva.' );
		}
	...
	//Forcefeed;
	Vapula.prototype.vapulaForceFeeds = function() {
		if( CoC.getInstance().player.hasVagina() && CoC.getInstance().player.hasKeyItem( 'Demonic Strap-On' ) >= 0 && !CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( '\n<b>In the night...</b>\nArousal blooms in your dreams, and you are relentlessly pushed into increasingly juicier and filthier fantasies; feeling hopelessly wet, you half awaken and hazily slide a hand down towards your crotch, to try and find release and rest.  Your fingers touch leather and latex where your vagina should be; bewildered, you rise to see Vapula working around your crotch, tightening the final harnesses of your strap-on, locking you into the pink dildo throbbing in your ' + Descriptors.vaginaDescript( 0 ) + ' whilst she licks hungrily at the tip of the opposite end.  You try to sit up but the ferocious succubus pins you down ruthlessly, her strength increased by her state of hunger.  She smirks at you wickedly and whispers, "<i>So you don\'t want me to use any dick but the one I gave you, yet you won\'t feed me?  That\'s very bad of you, ' + CoC.getInstance().player.short + '... really, really bad...  Do you want to starve me?  Do you want me to beg you for cum?  You naughty slut, you better be ready because I\'m going to get what is rightfully mine.  I\'m HUNGRY!</i>"' );
			...
			CoC.getInstance().flags[ kFLAGS.VAPULA_DAYS_SINCE_FED ] = 0;
			CoC.getInstance().flags[ kFLAGS.VAPULA_TEASE_COUNT ] = 0;
			CoC.getInstance().flags[ kFLAGS.VAPULA_EARNED_A_SPANK ] = 1;
		}
		//[if no fuck harem is on and PC hasn't fed/fucked Vapula for 5 days that scene will trigger at night];
			...
			EngineCore.outputText( '\n\nYou are suddenly awakened by a gentle tingle at the base of your crotch.  As you open your eyes, you slowly make out in the surrounding darkness a purple shape bobbing up and down leisurely.  You realize that Vapula has profited from your unconsciousness to take your genitals by force!  You try to sit up but the ferocious succubus pins you down ruthlessly, her strength increased by her state of hunger.  She releases your junk with a wet POP and starts licking her lips, slurping back any drop of pre-cum that might have escaped her voracious mouth.  She smirks at you wickedly and whispers, "<i>So you don\'t want me to use any dick but yours, yet you won\'t feed me?  That\'s very bad of you, ' + CoC.getInstance().player.short + '... really, really bad... Do you want to starve me?  Do you want me to beg you for your cum?  You naughty stud, you better be ready because I\'m going to get what is rightfully mine.  I\'m HUNGRY!</i>"  With a wolfish groan, she starts sucking you off again, still pinning you down with one hand while the other tickles your crotch.' );
			EngineCore.outputText( '\n\nShe works on it for what seems like hours. You look down at your slut; she keeps staring at you with avid yet playful eyes, never breaking eye contact as she relentlessly throatfucks herself; you feel her lips curve a little as the shape of a ravenous smile appears on her face, and her eyes are glittering with an insatiable need. The naughty bitch!  She knows she\'s going to get what she wants, whether you like it or not.  You wish you were able to control yourself, but the covetous succubus never lets go of your ' + Descriptors.cockDescript( x ) + ', clinging to it like an animal.' );
			EngineCore.outputText( '\n\nYour stamina is rapidly overwhelmed and you decide to reward her effort; you savagely press Vapula\'s head against your groin, effectively burying all of your ' + Descriptors.cockDescript( x ) + ' in her waiting throat; her tongue keeps teasing your rod for a few seconds, and then you cum, brutally.' );
			if( CoC.getInstance().player.balls > 0 ) {
				EngineCore.outputText( '  Your ' + Descriptors.ballsDescriptLight() + ' churn' );
			} else {
				EngineCore.outputText( '  Your ' + Descriptors.cockDescript( x ) + ' twitches' );
			}
			EngineCore.outputText( ' as you release a sticky spooge fountain in her stomach.' );
			...
			if( CoC.getInstance().player.cumQ() >= 1500 ) {
				EngineCore.outputText( ' until she looks 6 months pregnant.  At last, the cum-flow spilling through your urethra starts to ebb and your ' + Descriptors.cockDescript( x ) + ' stops throbbing; only then does she removes your junk from her mouth with a loud POP.  Your tool appears to be clean of any spooge: your cock-slut did a very good job.  Satisfied, you pat her head with your cock and let her digest her lavish meal.' );
			}
			CoC.getInstance().player.orgasm();
			EngineCore.dynStats( 'cor', 1 );
			CoC.getInstance().flags[ kFLAGS.VAPULA_DAYS_SINCE_FED ] = 0;
	...
	Vapula.prototype.spankVapulaLikeABoss = function() {
		...
		if( CoC.getInstance().scenes.camp.companionsCount() > 2 ) {
			EngineCore.outputText( ', drawing the eyes of your other companions.' );
		} else if( CoC.getInstance().scenes.camp.companionsCount() === 2 ) {
			EngineCore.outputText( ', drawing the attention of your other companion.' );
		} else {
			EngineCore.outputText( ', echoing off the sentinel-like rocks that seem to ring your encampment.' );
		}
		EngineCore.outputText( '  Her tail goes wild at the impact, whipping around in a frenzy and actually managing to slap your cheek!  That bitch!' );
		...
		if( CoC.getInstance().player.findPerk( PerkLib.Sadist ) >= 0 ) {
			EngineCore.outputText( '  Dishing out all this suffering is getting you a little hot under the collar, and you squirm a bit in place in anticipation of the pain to come.' );
		}
		...
		//{+20ish lust};
		EngineCore.dynStats( 'lus', (10 + CoC.getInstance().player.lib / 7) );
		//{Sadist: + 20 lust};
		if( CoC.getInstance().player.findPerk( PerkLib.Sadist ) >= 0 ) {
			EngineCore.dynStats( 'lus', (10 + CoC.getInstance().player.lib / 7) );
		}
		CoC.getInstance().flags[ kFLAGS.VAPULA_EARNED_A_SPANK ] = 0;
		EngineCore.doNext( CoC.getInstance().scenes.camp.returnToCampUseOneHour );
	};
	CoC.getInstance().registerScene( 'vapula', new Vapula() );
} );