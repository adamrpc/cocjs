...
angular.module( 'cocjs' ).run( function( MainView, SceneLib, ConsumableLib, CockTypesEnum, PerkLib, $rootScope, PregnancyStore, Descriptors, AppearanceDefs, CoC, kFLAGS, Utils, StatusAffects, EngineCore ) {
	function Vapula() {
		var that = this;
	...
	Vapula.prototype.timeChange = function() {
		if( CoC.time.hours > 23 ) {
			if( this.vapulaSlave() && CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_VAPULA ] === 0 ) {
				if( CoC.flags[ kFLAGS.VAPULA_HAREM_FUCK ] === 0 ) {
					CoC.flags[ kFLAGS.VAPULA_DAYS_SINCE_FED ]++;
				} else {
					CoC.flags[ kFLAGS.VAPULA_DAYS_SINCE_FED ] = 0;
				}
			}
			if( CoC.flags[ kFLAGS.VAPULA_FOLLOWER ] === 0.5 || CoC.flags[ kFLAGS.VAPULA_FOLLOWER ] === 1.5 ) {
				CoC.flags[ kFLAGS.VAPULA_FOLLOWER ]++;
			}
			CoC.flags[ kFLAGS.DAYS_SINCE_LAST_DEMON_DEALINGS ]++;
		}
		if( this.vapulaSlave() && CoC.player.hasKeyItem( 'Demonic Strap-On' ) < 0 && CoC.player.gender === 2 && CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_VAPULA ] === 0 ) {
	...
	Vapula.prototype.timeChangeLarge = function() {
		if( CoC.flags[ kFLAGS.VAPULA_FOLLOWER ] >= 2.5 && CoC.time.hours === 6 && CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_VAPULA ] === 0 ) {
			this.femaleVapulaRecruitmentPartII();
			return true;
		}
		if( CoC.time.hours === 2 && this.vapulaSlave() && CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_VAPULA ] === 0 && CoC.flags[ kFLAGS.VAPULA_DAYS_SINCE_FED ] >= 5 && (CoC.player.hasCock() || (CoC.player.hasKeyItem( 'Demonic Strap-On' ) >= 0 && CoC.player.hasVagina())) ) {
	...
	Vapula.prototype.vapulaSlave = function() {
		return CoC.flags[ kFLAGS.VAPULA_FOLLOWER ] === 1;
	};
	...
	Vapula.prototype.vapulaSlaveFlavorText = function() {
		...
		if( choice === 0 ) {
			MainView.outputText( 'Vapula is currently resting on a very rough bed of leaves she\'s gathered herself.  She sleeps quietly, her hand absent-mindedly laid on her vagina and the other one resting on her breasts.  A thin trickle of her juice is leaking out of her crotch.' );
		}
		if( choice === 1 ) {
			MainView.outputText( 'Your succubus slut, Vaplua, is furiously stroking herself on her bed of leaves, her purple body shining in sweat.  She moans raggedly, often looking up to you with lust-consumed eyes.' );
		}
		if( choice === 2 ) {
			if( CoC.flags[ kFLAGS.VAPULA_HAREM_FUCK ] === 0 ) {
				choice = 3;
			} else {
				MainView.outputText( 'Vapula is currently absent, probably getting screwed by some of the imps surrounding your camp in order to get her semen rations.  You\'re sure she would come back instantly if you ever needed to give her YOUR cum instead.' );
			}
		}
		if( choice === 3 ) {
			MainView.outputText( 'Vapula is busy drinking a vial of milk that looks very much like the kind you\'d usually find on imps.  Her purple cheeks seem to blush darker when she sees you; she awkwardly gulps the rest of the bottle, hiccups, and giggles softly, eyeing you with a coy smile and a lust-filled gaze.  The potion seems to have made her even more aroused than usual.' );
		}
		if( choice === 4 ) {
			MainView.outputText( 'Vapula is currently toying with ' );
			if( CoC.player.hasKeyItem( 'Deluxe Dildo' ) >= 0 ) {
				MainView.outputText( 'your deluxe dildo' );
			} else {
				MainView.outputText( 'a dildo' );
			}
			MainView.outputText( ', pumping as fast as she can.  In her feverish display of lust she doesn\'t even notice you.' );
		}
		if( choice === 5 ) {
			if( CoC.flags[ kFLAGS.VAPULA_HAREM_FUCK ] === 0 || !(SceneLib.sophieBimbo.bimboSophie()) ) {
				choice = 6;
			}//if harem fucking is on);
			else {
				MainView.outputText( 'Vapula is fingering herself while eating out Sophie, your harpy slut.  The dumb avian matron keeps moaning like she\'s in heat while the succubus furiously assaults her lush fuck-hole; both of their pair of wings flap in rhythm as they feel their pleasure rising in waves.' );
			}
		}
		if( choice === 6 ) {
			if( CoC.flags[ kFLAGS.VAPULA_HAREM_FUCK ] === 0 || CoC.flags[ kFLAGS.IZMA_NO_COCK ] > 0 || !SceneLib.izmaScene.izmaFollower() ) {
				choice = 7;
			}//if Izma has cock and harem fucking is on);
			else {
				MainView.outputText( 'Your succubus is currently very busy with Izma, your tigershark lover.  Izma is standing and hiding Vapula from your sight and you don\'t notice what Vapula is doing to her until you see her frantic bobbing movements, and hear Izma\'s hisses and growls of pleasure.  Obviously your succubus must get the cum she needs from somewhere.' );
			}
		}
		if( choice === 7 ) {
			//if C. Jojo and harem fucking is on);
			if( CoC.flags[ kFLAGS.VAPULA_HAREM_FUCK ] === 0 || !SceneLib.jojoScene.campCorruptJojo() ) {
				choice = 8;
			} else {
				MainView.outputText( 'You routinely hear screams of pleasure outside the camp, as well as some frantic male squeaks coming from Jojo, your ' );
				if( SceneLib.jojoScene.tentacleJojo() ) {
					MainView.outputText( 'tentacled ' );
				}
				MainView.outputText( 'mouse pet.  Vapula seems to be having a lot of fun with your fuck-toy!' );
			}
		}
		if( choice === 8 ) {
			if( CoC.flags[ kFLAGS.VAPULA_HAREM_FUCK ] === 0 || !(SceneLib.amilyScene.amilyFollower() && SceneLib.amilyScene.amilyCorrupt()) ) {
				choice = 9;
			}//if C. Amily and harem fucking on);
			else {
				MainView.outputText( 'Vapula is busy caressing your corrupted mousette, Amily while softly kissing her breasts.  Her fingers are buried in your fuck-toy\'s pussy, and Amily is moaning whorishly and doesn\'t even bother to stop when she looks at you, her helpless eyes betraying her lack of control over her own lust.' );
			}
		}
		if( choice === 9 ) {
		...
		if( choice === 10 ) {
			MainView.outputText( 'Vapula is resting on her stomach, a buttplug shoved into her ass.  She doesn\'t seem to notice you watching her butt jiggle under the effect of her unconscious anal contractions.' );
		}
	};
	...
	Vapula.prototype.mouseWaifuFreakout = function( amily, jojo ) {
		...
		MainView.outputText( ' up to you, worried, as Vapula struts around the camp.' );
		MainView.outputText( '\n\n"<i>' + CoC.player.short + ', what is this? Am I dreaming or did you actually bring a demon to your camp? What in the world is wrong with you?</i>"' );
		...
		if( CoC.player.lib >= 50 ) {
			MainView.outputText( '"<i>Well, as long as she swallows, I don\'t care,</i>" you quip.' );
		}
		MainView.outputText( '\n\n"<i>That\'s it!</i>"  The ' );
		if( jojo && amily ) {
			MainView.outputText( 'mice are' );
		} else {
			MainView.outputText( 'mouse is' );
		}
		MainView.outputText( ' raving.  "<i>You\'ve clearly given in to her demonic lust.</i>"' );
		if( jojo ) {
			MainView.outputText( '\n\n"<i>I\'m leaving, ' + CoC.player.short + ',</i>" Jojo says.  "<i>I only hope for your sake that you come to your senses soon... I will return to my place in the forest when you require assistance in freeing your soul of taint.</i>"' );
			CoC.flags[ kFLAGS.JOJO_MOVE_IN_DISABLED ] = 1;
		...
		if( amily && jojo ) {
			MainView.outputText( ' have ' );
		} else {
			MainView.outputText( ' has ' );
		}
		MainView.outputText( 'moved out.' );
	...
	Vapula.prototype.callSlaveVapula = function( output ) {
		...
		if( CoC.player.hasVagina() && CoC.player.hasKeyItem( 'Demonic Strap-On' ) >= 0 ) {
			if( CoC.player.lust >= 33 ) {
				fFeed = this.chixFeedVapulaBlehblehIVantToZuckYourSpooo;
			} else if( output && !CoC.player.hasCock() ) {
				MainView.outputText( '\n\nYou aren\'t suitably aroused to feed Vapula right now.' );
			}
		}
		var threesome = null;
		...
		if( CoC.flags[ kFLAGS.VAPULA_EARNED_A_SPANK ] > 0 && !CoC.player.isTaur() && !CoC.player.isDrider() ) {
	...
	Vapula.prototype.sendToFarm = function() {
		MainView.clearOutput();
		MainView.outputText( 'You tell your succubus concubine that she is to head towards the lake, find a farm, present herself to the lady who works there and do as she says. Vapula laughs long and hard at this.' );
	...
	Vapula.prototype.backToCamp = function() {
		MainView.clearOutput();
		MainView.outputText( 'You tell her to head back to camp; there are things you need to do to her you can’t do whilst she’s here. Repeatedly. Vapula fist pumps the sky.' );
	...
	Vapula.prototype.harvestMilk = function() {
		MainView.clearOutput();
		MainView.outputText( 'You tell Vapula that you want her hooked up to a milking machine whenever possible; you need her fluids.' );
		...
		if( CoC.flags[ kFLAGS.FARM_UPGRADES_REFINERY ] === 0 ) {
			MainView.outputText( '“<i>It’ll sure as hell beat whatever disgusting thing the bitch would have me doing instead. Why don’t you make her build a concentration machine? Lethice has loads; makes one dose of cream go so much further.</i>”' );
		} else {
			MainView.outputText( '“<i>As you insist, [master]. It’ll sure as hell beat working.</i>”' );
		}
	...
	Vapula.prototype.stopHarvest = function() {
		MainView.clearOutput();
		MainView.outputText( 'You tell Vapula to stop producing succubus milk; you’re practically drowning in the stuff.' );
		...
		if( SceneLib.farmCorruption.hasTattoo( 'vapula' ) ) {
			...
			if( SceneLib.farmCorruption.vapulaFullTribalTats() ) {
				...
				if( CoC.flags[ kFLAGS.VAPULA_TATTOO_COLLARBONE ] !== 0 ) {
					MainView.outputText( CoC.flags[ kFLAGS.VAPULA_TATTOO_COLLARBONE ] + '\n' );
				}
				if( CoC.flags[ kFLAGS.VAPULA_TATTOO_SHOULDERS ] !== 0 ) {
					MainView.outputText( CoC.flags[ kFLAGS.VAPULA_TATTOO_SHOULDERS ] + '\n' );
				}
				if( CoC.flags[ kFLAGS.VAPULA_TATTOO_LOWERBACK ] !== 0 ) {
					MainView.outputText( CoC.flags[ kFLAGS.VAPULA_TATTOO_LOWERBACK ] + '\n' );
				}
				if( CoC.flags[ kFLAGS.VAPULA_TATTOO_BUTT ] !== 0 ) {
					MainView.outputText( CoC.flags[ kFLAGS.VAPULA_TATTOO_BUTT ] + '\n' );
				}
			}
		}
	...
	Vapula.prototype.talkToVapulaForSomeReason = function() {
		MainView.clearOutput();
		MainView.outputText( '"<i>You want to talk?  Well it\'s quite rare to see ' + CoC.player.mf( 'studs', 'girls' ) + ' looking for conversation with the likes of me, but go ahead.  What do you want to talk about?</i>"' );
	...
	Vapula.prototype.feedVapulaACupOfJizz = function() {
		MainView.clearOutput();
		MainView.outputText( 'You tell Vapula you\'re going to reward her for being such a good slut; idly stroking [oneCock] into erection, you command her to get on her knees so she can receive her meal.  The demoness eagerly complies and grabs hold of [oneCock].  She then proceeds to lick it, coating the entirety of your junk with her warm, sweet saliva. With an expert precision, she massages all your most sensitive points, pumping pre-cum up your urethra with ruthless efficiency.  As drops of your juices dribble from the tip of your ' + Descriptors.cockDescript( 0 ) + ' she voraciously slurps them with a gourmand\'s smile.' );
		...
		MainView.outputText( '\n\nYour stamina is rapidly overwhelmed and you decide to reward her effort; you savagely press Vapula\'s head against your groin, effectively burying all ' + Utils.num2Text( Math.round( CoC.player.cocks[ 0 ].cockLength ) ) + ' inches of your ' + Descriptors.cockDescript( 0 ) + ' in her waiting throat; her tongue keeps teasing your rod for a few seconds, and then you cum, brutally.  ' );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( 'Your ' + Descriptors.ballsDescriptLight() + ' churn' );
		} else {
			MainView.outputText( 'Your ' + Descriptors.cockDescript( 0 ) + ' twitches' );
		}
		...
		if( CoC.player.cumQ() >= 1500 ) {
			MainView.outputText( ' until she looks six months pregnant' );
		}
		MainView.outputText( '.  At last, the cum-flow spilling through your urethra starts to ebb and your ' + Descriptors.cockDescript( 0 ) + ' stops throbbing; only then does she removes your junk from her mouth with a loud POP.  Your ' + Descriptors.cockDescript( 0 ) + ' appears to be clean of any spooge: your cock-slut did a very good job.  Satisfied, you pat her head with your cock and let her digest her lavish meal.' );
		CoC.player.orgasm();
	...
	Vapula.prototype.vapulaThreesomeMenu = function() {
		MainView.clearOutput();
		MainView.outputText( 'Who do you invite?' );
		//Option: Amily;
		var amily = null;
		//Option: Ceraph;
		var ceraph = null;
		...
		//Option: Izma;
		var izma = null;
		if( CoC.player.hasCock() || (CoC.player.hasVagina() && CoC.player.hasKeyItem( 'Demonic Strap-On' ) >= 0) ) {
			if( SceneLib.jojoScene.campCorruptJojo() ) {
				jojo = this.vapulaJojoThreesomes;
			}
			if( SceneLib.amilyScene.amilyFollower() && SceneLib.amilyScene.amilyCorrupt() && CoC.player.hasCock() ) {
				amily = this.vapulaAndAmilyThreesome;
			}
			if( SceneLib.ceraphFollowerScene.ceraphIsFollower() && CoC.player.hasCock() ) {
	...
	Vapula.prototype.vapulaCeraphThreesomePleaseBoth = function() {
		...
		if( CoC.player.balls > 0 ) {
			MainView.outputText( Descriptors.ballsDescriptLight() + ' hitting her butt-hole with a faint thud' );
		} else {
			MainView.outputText( 'thudding up against her cushy butt-cheeks' );
		}
		MainView.outputText( '.  The purple sex-goddess squeals, your abrupt dick-digging utterly ravaging her cunt; her unholy warmth and impossibly tight confines are almost enough to make you cum instantly as her vaginal walls wrap in the most sensitive way around your rod, but you manage to hold off the flow of semen churning in your groin.' );
		//[if 2nd cock];
		...
		if( CoC.player.cockTotal() > 4 ) {
			MainView.outputText( '  Sadly, your extra junk does not have any hole left to fill.  It just hangs there, rubbing against Ceraph\'s backside and teasing her all the more.' );
		}
		...
		//[if multicocks];
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( '  She expertly jerks each of your rods in turn, gently polishing them as they grow back into an erect state.' );
		}
		MainView.outputText( '  She\'s obviously used to this kind of practice; even though [eachCock] felt numb after cumming so hard, you feel your lust build up again as your genitals throb once more with need.  You grab Ceraph\'s head and impale her on your crotch, forcefully throat-fucking her.' );
		...
		MainView.outputText( '  Your thoughts are quickly drowned in waves of pleasure, and soon your only thought is to ram more of your ' + Descriptors.cockDescript( x ) + ' down your sluts\' throats, pumping their heads alternatively around your shaft' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 's' );
		}
		if( CoC.player.balls > 0 ) {
			MainView.outputText( ' as the girls keep fondling your ' + Descriptors.ballsDescriptLight() );
		}
		MainView.outputText( ', bringing you extra tingles of pleasure to your body.' );
		...
		MainView.outputText( ', their lips doing their best to milk ' );
		if( CoC.player.cockTotal() === 1 ) {
			MainView.outputText( 'it' );
		} else {
			MainView.outputText( 'them' );
		}
		MainView.outputText( ' of all ' );
		if( CoC.player.cockTotal() === 1 ) {
			MainView.outputText( 'its ' );
		} else {
			MainView.outputText( 'their ' );
		}
		MainView.outputText( 'cream.  Once your junk feels completely empty, [oneCock] is released with a loud POP.' );
		MainView.outputText( '\n\nSatisfied, you kiss both of your sluts.  "<i>Thank you ' + CoC.player.mf( 'Master', 'Mistress' ) + '.</i>" they say in unison, before you pat their heads and send them on their way; they will probably need to rest in order to digest their heavy meal.' );
		//lust set to 0;
		CoC.player.orgasm();
	...
	Vapula.prototype.vapulaAndCeraphButtfuckTrainYeehaw = function() {
		...
		MainView.outputText( '\n\nYou pump more and more, growing more impatient and vicious as you feel your climax approaching.  You slap Ceraph\'s face, you toy with her nipples, you pinch Vapula\'s butt, you block Ceraph\'s thrusts midway, making both demonesses whimper in frustrated desire.  All the while, you keep butt-fucking Ceraph at a crazy pace, yelling like a ' + CoC.player.mf( 'man', 'herm' ) + ' possessed as you claim her impossibly tight confines with unequaled intensity.  You ride the butt-fuck train, submitting both girls to your will as their anal entrances keep being assaulted over and over again.  Their warm love-tunnels are literally steaming in arousal and their feminine juices are being spilled in a free flow of lust.' );
		//[if balls];
		if( CoC.player.balls > 0 ) {
			MainView.outputText( '  As you relentlessly smash your groin against Ceraph\'s plump rump, your ' + Descriptors.ballsDescriptLight() + ' repeatedly hit her quivering vagina; they are soon utterly coated in fem-spunk.' );
		}
		MainView.outputText( '  The entirety of your crotch ends up drenched with a various mix of sexual juices, and your ' + Descriptors.cockDescript( x ) + ' makes squelching noises at each thrust in and out of your omnibus\' little ass.' );
	...
	Vapula.prototype.vapulaSophieThreeSome = function() {
		...
		MainView.outputText( 'You call Sophie insistently, panting in anticipation.  The blond-feathered bimbo doesn\'t come at first - perhaps the dumb slut forgot her name again - but as you grow more impatient you finally notice the platinum-headed woman, her enormous boobs bouncing hypnotically and her luscious thighs swaying lustfully as she walks.  She grins beatifically and coos: "<i>Yes?  Is there anything I can, like, do for... ooooh, but who\'s this?  She\'s sooooooo beautiful!  She\'s got, like, big boobs and stuff!</i>"  Vapula sighs in consternation.  "<i>What can you do with her, ' + CoC.player.short + '? I\'m sure she\'s going to forget my presence by next hour.</i>"' );
		...
		if( CoC.player.balls > 0 ) {
			MainView.outputText( 'balls and massages your ' );
		}
		MainView.outputText( Descriptors.buttDescript() + '.' );
		//[if third cock];
		if( CoC.player.totalCocks() > 2 ) {
			MainView.outputText( '  Using her extra flexibility, she bends over and start jerking your third pole, the sweet handjob adding to your pure cocktail of pleasure.' );
		}
		MainView.outputText( '  You can feel her hard nipples poke at your skin as they gently scrape across your torso.  Vapula hardly ever interrupts her embrace, only pulling back from your lips to take care of other intimate parts of your body.  You keep riding the bimbo senseless, making her squirm and squawk in ecstasy as your own cries of pleasure are muffled by Vapula\'s voracious lips.' );
	...
	Vapula.prototype.vapulaAndAmilyThreesome = function() {
		...
		MainView.outputText( '\n\nThe little mousy looks down awkwardly, intimidated by the towering succubus.  "<i>Y-yes ' + CoC.player.mf( 'Master', 'Mistress' ) + '...</i>"' );
		...
		if( CoC.player.balls > 0 ) {
			MainView.outputText( ': her drooling pussy needs to be prepared because you\'re going to take her next' );
		}
		MainView.outputText( '.  Vapula smirks in excitement and eagerly shoves her tumescent pussy in front of the mousette\'s waiting lips; Amily keeps wagging her butt impatiently, looking forward to being used as a disposable anal-toy.  You wink at Vapula; she\'s going to like it.' );
		...
		if( CoC.player.balls > 0 ) {
			...
			if( CoC.player.cumQ() >= 1500 ) {
				MainView.outputText( '  Sticky globs of white goo are shot from the tip of your rod, effectively stuffing Vapula so full of your spooge that some of it drips out and rains down on Amily\'s face, sopping her with fluids. The succubus quivers in your arms, giving in to her lust before squirting a powerful spray of fem-spooge, splattering Amily even more.' );
			}
			MainView.outputText( '  Once your orgasm starts fading, you drop Vapula and tell her to imitate Amily; the dilligent succubus gets on her knees and quickly licks your ' + Descriptors.cockDescript( x ) + ', feeding her cum-thirst as she cleans your junk of its spooge.  Your two sluts work quickly, easily sucking up every bit of goo until your ' + Descriptors.cockDescript( x ) + ' eventually shrinks to a half-erect state, your lust consumed.' );
		}
	...
	Vapula.prototype.vapulaAssistsCeruleanSuccubus = function() {
		...
		//[if multicock];
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( '  She suddenly seems to have an idea: leaning down to reach your second cock, she starts licking it, accompanying the savage thrusting of the blue demoness as she slurps your extra junk, giving you even more pleasure.' );
		} else {
			MainView.outputText( '  The blue demoness leans down and starts licking Vapula\'s moist pussy, intertwining her flexible tongue with yours and pleasuring the purple slut even more.' );
		}
		...
		if( CoC.player.tallness < 60 ) {
			MainView.outputText( ', enclosing your head in her cleavage' );
		}
		MainView.outputText( '.  "<i>That was fun! I can\'t wait to have another party like this! You should invite her more often!</i>"' );
	...
	Vapula.prototype.NTRSomeJojos = function() {
		...
		if( CoC.player.hasCock() ) {
			...
			MainView.outputText( '"<i>Admit it, you\'d love to use that smoking hot body, wouldn\'t you?  Don\'t lie to me.  You\'d give up your soul in order to fuck her brains out.  Don\'t you want to plunge your dick inside her?  Don\'t you want to fuck my beautiful succubus wife?</i>"' );
			...
			if( CoC.player.cockTotal() > 1 ) {
				...
				//[if third cock];
				if( CoC.player.cockTotal() > 2 ) {
					MainView.outputText( '  Since the horny slut has a demonic vaginal capacity, you might as well fit more dick in it: in no time your third dick joins its colleague in this infernal vaginal assault.' );
				}
				MainView.outputText( '  ' );
			} else {
				MainView.outputText( '\n\n' );
			}
			MainView.outputText( 'You push in and out of her, claiming both her holes and snickering at the poor mouse thing that can get neither. With a mighty push, you put Vapula on all fours and lean down in order to touch more of her supple body.  Your purple slut is now dying with pleasure, her tongue hanging and her eyes staring behind Jojo as the monk squeaks pitifully in his pathetic self-stroking.' );
			...
			MainView.outputText( ' being filled with cock.' );
			...
			if( SceneLib.jojoScene.tentacleJojo() ) {
				MainView.outputText( 'his tentacles start flying toward his ass, ' );
			}
	...
	Vapula.prototype.jojoButtFuckTrain = function() {
		...
		MainView.outputText( 'You tell Jojo it\'s his lucky day - you\'re going to have a joyful butt-fuck session with him and your purple pet.' );
		MainView.outputText( '\n\n"<i>Admit it, you\'d love to use that smoking hot body, wouldn\'t you?  Don\'t lie to me.  You\'d give up your soul in order to fuck her brains out.  Don\'t you want to plunge your dick inside her?  Don\'t you want to fuck my beautiful succubus wife?</i>"' );
		MainView.outputText( '\n\nJojo shakes his head in denial, but he can hardly keep any shred of credibility when his rock-hard mouse cock speaks for him.' );
		...
		if( CoC.player.cockArea( x ) > 100 ) {
			MainView.outputText( '  You can actually feel his internal organs being bumped against your meat as your ' + Descriptors.cockDescript( x ) + ' stretches his colon beyond its normal size and thrusts in and out like a charging mammoth.' );
		}
		...
		if( SceneLib.jojoScene.tentacleJojo() ) {
			...
			if( CoC.player.hasVagina() ) {
				MainView.outputText( '  Last but not least, a soft cock-knock at the entrance of your ' + Descriptors.vaginaDescript() + ' signals an incoming intrusion.  If your mouth weren\'t so full of cock you would sigh in relief as your aching pussy is thoroughly penetrated.' );
			}
		}
		//[if PC has extra long tentacle cock, variable is n1];
		var tentacles = _.filter( CoC.player.cocks, function( cock, index ) {
		...
		//[if second multicock];
		if( tentacles.length > 0 ) {
			MainView.outputText( ', as well as Vapula\'s love-tunnel' );
		}
		if( tentacles.length > 1 ) {
			MainView.outputText( ' and throat, ' );
		}
		if( tentacles.length > 2 ) {
			MainView.outputText( 'and Jojo\'s too' );
		}
		MainView.outputText( '.  You feel that Jojo\'s release is coming too.  With a powerful thrust, he squirts his own mouse semen down Vapula\'s waiting ass' );
		//[if tentacle Jojo];
		if( SceneLib.jojoScene.tentacleJojo() ) {
			MainView.outputText( '; his tentacles absolutely drench your every hole with greenish mousey-cum' );
		}
		MainView.outputText( '.  Vapula\'s own orgasm also starts kicking in, the feeling of hot cum flooding her interior being too much for her horny body to bear.' );
	...
	Vapula.prototype.vapulaAndIzma = function( girls ) {
		...
		if( girls !== undefined && !girls ) {
			MainView.outputText( 'You call your beta tigershark, telling her you\'ll have some use for her body.  Izma eagerly approaches you, although she frowns a bit when she sees the towering succubus accompanying you.  "<i>What can I do for my alp-0... wait, what are you doing with her?</i>"  Vapula only smirks at Izma\'s puzzled reaction.' );
			MainView.outputText( '\n\nYou gently explain that you intend to have some extra fun, that\'s why you brought this gorgeous creature to please you both.  Izma isn\'t really convinced, but still walks up to you, not willing to antagonize her alpha.  You tell to remove her cloth and turn around so you can have a clear view of her toned butt.  "<i>What do you have in mind, ' + CoC.player.short + '?</i>"\n\n"<i>Shhh... just do as I say.</i>"  With a discreet gesture, you order Vapula to get on her knees in front of Izma; she\'s going to feed from your tigershark this time.  With a conniving smile, Vapula obeys and before your shark lover can react, you\'re already grinding your ' + Descriptors.cockDescript( x ) + ' against her muscular butt-cheeks while the purple succubus grabs hold of her 15 inch long pecker.  You rub the tip of your ' + Descriptors.cockDescript( x ) + ' against her butt crack.  "<i>W-wait! What are you guys doing? Are you going to fuck my... I don\'t want to-</i>"' );
		...
		//Vapula/Izma threesome (as female);
			...
			MainView.outputText( 'You call your beta tigershark, telling her you\'ll have some use for her body.  Izma eagerly approaches you as you slip into your strap-on, only starting to frown when she sees the towering succubus accompanying you.  "<i>What can I do for my alp-0... wait, what are you doing with her?</i>"  Vapula only smirks at Izma\'s puzzled reaction.' );
			MainView.outputText( '\n\nYou gently explain that you intend to have some extra fun, and you\'ve brought this gorgeous creature to please you both.  Izma isn\'t really convinced, but still walks up to you, not willing to antagonize her alpha. You tell to remove her cloth and turn around so you can have a clear view of her toned butt.  "<i>What do you have in mind, ' + CoC.player.short + '?</i>"\n\n"<i>Shhh... just do as I say.</i>"  With a discreet gesture, you order Vapula to get on her knees in front of Izma; she\'s going to feed from your tigershark this time.  With a conniving smile, Vapula obeys and before your shark lover can react, you\'re already grinding your obscene dildo against her muscular butt-cheeks while the purple succubus grabs hold of her 15 inch long pecker.  You rub the tip of your strap-on against her butt crack, sighing as the movement sends pleasurable vibrations down into your end.' );
	...
	Vapula.prototype.chixFeedVapulaBlehblehIVantToZuckYourSpooo = function() {
		MainView.clearOutput();
		MainView.outputText( 'You produce the demonic dildo and hold it by the hilt of its giant purple end, wagging it lazily by your side with a playful grin.  Vapula knows what that means; she is on her knees in an instant, staring at you with wolfish anticipation.' );
		//Feed/Tease;
		var tease = null;
		if( CoC.flags[ kFLAGS.VAPULA_HAREM_FUCK ] === 0 ) {
	...
	Vapula.prototype.teaseVapula = function() {
		...
		//Requires: Vapula not fucking harem, teased the day before;
			...
			//[Hooves:;
			if( CoC.player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_HOOFED || CoC.player.isTaur() ) {
				MainView.outputText( '"<i>I would worship your feet, mistress.  I would clean your hooves with my tongue to a shine, tease out every scrap of dirt from every crevice.  And I would thank you for the privilege.</i>"' );
			}//[Naga:;
			...
			} else if( CoC.player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_DOG || CoC.player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_CAT || CoC.player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_LIZARD || CoC.player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_KANGAROO || CoC.player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_FOX ) {
			...
			//[Insect:;
			else if( CoC.player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_BEE || CoC.player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_CHITINOUS_SPIDER_LEGS ) {
		...
		//Third Tease;
			...
			MainView.outputText( 'Once again you buckle up, sighing with pleasure as you sink the pink dildo into your moist hole, and once again you wait until Vapula gets onto her knees before you.  She looks at you silently and sullenly as you droop the monstrous purple end over her face.' );
			...
			if( CoC.player.isNaga() ) {
				MainView.outputText( 'you slide your tail under and over her trunk, wrapping your coils around her and drawing her face into your stuffed crotch.' );
			}//Other:;
			else if( CoC.player.isTaur() || CoC.player.isGoo() || CoC.player.isDrider() ) {
				MainView.outputText( 'you sit the monstrous half of your body down comfortably and draw her face into your stuffed cotch.' );
			} else {
				MainView.outputText( 'you open your [legs] and wrap them around her head, drawing her face into your stuffed crotch.' );
			}
	...
	Vapula.prototype.chicksFeedVapula = function() {
		...
		if( CoC.flags[ kFLAGS.VAPULA_TEASE_COUNT ] > 1 ) {
			MainView.outputText( 'You push the pink end of the dildo into you with a sigh, then strap yourself into the harness so that the monstrous purple end bobs out in front of you.  You stand over a tense Vapula and tease her with it first, lightly brushing her face with the tip for a while.  She swallows thickly and licks her lips as you caress her with your cum fountain, but makes no movements towards it.  Eventually you giggle at her furious concentration and tell her you\'re going to reward her for being such a good slut, before pushing the end against her lips.  A flood of ecstatic thank-you-mistresses are swiftly muffled out as the dildo disappears into her mouth.' );
		} else {
			MainView.outputText( 'You tell Vapula you\'re going to reward her for being such a good slut; pushing the pink end of the dildo into you with a sigh, you strap yourself into the harness so that the monstrous purple end bobs out in front of you.  You giggle as Vapula grabs hold of it and proceeds to lick it, coating the entirety of your artificial junk with her warm, sweet saliva.' );
		}
	...
	Vapula.prototype.vapulaForceFeeds = function() {
		...
		//[if no fuck harem is on and PC hasn't fed/fucked Vapula for 5 days that scene will trigger at night];
			...
			if( CoC.player.balls > 0 ) {
				...
				MainView.outputText( '  Your ' + Descriptors.cockDescript( x ) + ' twitches' );
			}
			MainView.outputText( ' as you release a sticky spooge fountain in her stomach.' );
			...
			if( CoC.player.cumQ() >= 1500 ) {
				MainView.outputText( ' until she looks 6 months pregnant.  At last, the cum-flow spilling through your urethra starts to ebb and your ' + Descriptors.cockDescript( x ) + ' stops throbbing; only then does she removes your junk from her mouth with a loud POP.  Your tool appears to be clean of any spooge: your cock-slut did a very good job.  Satisfied, you pat her head with your cock and let her digest her lavish meal.' );
			}
			CoC.player.orgasm();
			EngineCore.dynStats( 'cor', 1 );
			CoC.flags[ kFLAGS.VAPULA_DAYS_SINCE_FED ] = 0;
	...
	Vapula.prototype.spankVapulaLikeABoss = function() {
		...
		if( SceneLib.camp.companionsCount() > 2 ) {
			MainView.outputText( ', drawing the eyes of your other companions.' );
		} else if( SceneLib.camp.companionsCount() === 2 ) {
			MainView.outputText( ', drawing the attention of your other companion.' );
		} else {
			MainView.outputText( ', echoing off the sentinel-like rocks that seem to ring your encampment.' );
		}
