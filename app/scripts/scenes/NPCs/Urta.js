...
angular.module( 'cocjs' ).run( function( SceneLib, MainView, $rootScope, $log, ImageManager, Appearance, CockTypesEnum, PregnancyStore, CoC_Settings, PerkLib, Descriptors, AppearanceDefs, EventParser, CoC, kFLAGS, Utils, StatusAffects, EngineCore ) {
	//Urta variables;
	//CoC.flags[kFLAGS.TIMES_FUCKED_URTA] = times fucked;
	//CoC.flags[kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY] = urtas horsecock comfort level;
	//-1 = PC pissed her off;
	//0 = first timern;
	//1-10 = slightly comfortable;
	//11+ = flaunt it;
	//CoC.flags[kFLAGS.URTA_TIME_SINCE_LAST_CAME] = hours turned off;
	//0 = ready to rock!;
	//Urtas horsey bits =;
	//LxW = 20x3;
	//Luv Lock - CoC.flags[kFLAGS.URTA_PC_LOVE_COUNTER]:;
	//0 = normal, 1 = love, -1 = never love, CoC.flags[kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY] -1 = hate;
	/*Luv rating! - CoC.flags[kFLAGS.URTA_PC_AFFECTION_COUNTER]
	...
	Urta.prototype.timeChange = function() {
		var needNext = false;
		...
		$log.debug( '\nUrta time change: Time is ' + CoC.time.hours + ', incubation: ' + this.pregnancy.incubation + ', event: ' + this.pregnancy.event );
		if( CoC.flags[ kFLAGS.NEED_URTA_LETTER ] === 1 && CoC.time.hours === 6 ) {
			SceneLib.urtaPregs.getUrtaLetter(); //Urta Letters
		}
		if( this.pregnancy.incubation === 0 && (this.pregnancy.type === PregnancyStore.PREGNANCY_BEE_EGGS || PregnancyStore.PREGNANCY_DRIDER_EGGS) ) {
			...
			CoC.flags[ kFLAGS.URTA_EGGS ] = 0;
			CoC.flags[ kFLAGS.URTA_FERTILE_EGGS ] = 0;
		}
		if( CoC.flags[ kFLAGS.URTA_TIME_SINCE_LAST_CAME ] > 0 ) {
			CoC.flags[ kFLAGS.URTA_TIME_SINCE_LAST_CAME ]--; //Count down timer for urta's lust
		}
		if( CoC.flags[ kFLAGS.URTA_EGG_FORCE_EVENT ] > 0 ) {
			CoC.flags[ kFLAGS.URTA_EGG_FORCE_EVENT ]--; //Countdown to urta freakout
		}
		//Urta egg freak out;
		if( CoC.flags[ kFLAGS.URTA_EGG_FORCE_EVENT ] > 0 && CoC.time.hours > 6 && CoC.time.hours < 18 && CoC.flags[ kFLAGS.URTA_EGG_FORCE_EVENT ] < 12 ) {
		...
		if( CoC.flags[ kFLAGS.URTA_ANGRY_AT_PC_COUNTDOWN ] > 1 ) {
			CoC.flags[ kFLAGS.URTA_ANGRY_AT_PC_COUNTDOWN ]--;
			if( CoC.flags[ kFLAGS.URTA_ANGRY_AT_PC_COUNTDOWN ] < 1 ) {
				CoC.flags[ kFLAGS.URTA_ANGRY_AT_PC_COUNTDOWN ] = 1;
			}
		}
		if( CoC.time.hours > 23 ) {
			if( CoC.flags[ kFLAGS.URTA_TIME_SINCE_LAST_CAME ] === 0 ) {
				CoC.flags[ kFLAGS.URTA_CUM_NO_CUM_DAYS ]++;
			} else {
				CoC.flags[ kFLAGS.URTA_CUM_NO_CUM_DAYS ] = 0;
			}
			if( CoC.flags[ kFLAGS.URTA_PC_AFFECTION_COUNTER ] > 0 ) {
				CoC.flags[ kFLAGS.URTA_PC_AFFECTION_COUNTER ] -= 0.5;
				if( CoC.flags[ kFLAGS.URTA_PC_AFFECTION_COUNTER ] < 0 ) {
	...
	Urta.prototype.timeChangeLarge = function() {
		return false;
	};
	...
	Urta.prototype.urtaCapacity = function() {
		var bonus = CoC.flags[ kFLAGS.TIMES_RUT_FUCKED_URTAS_CUNT ] * 5;
		if( bonus > 40 ) {
			bonus = 40;
		}
		if( CoC.flags[ kFLAGS.URTA_TENTACLE_GAPED ] > 0 ) {
			return 500;
		}
		return 60 + bonus;
	};
	Urta.prototype.urtaLove = function( love ) {
		if( love === undefined ) {
			love = 0;
		}
		...
		if( love === 0 ) {
			return CoC.flags[ kFLAGS.URTA_PC_LOVE_COUNTER ] === 1;
		}
		if( love > 0 ) {
			CoC.flags[ kFLAGS.URTA_PC_AFFECTION_COUNTER ] += love;
		}
		if( love < 0 ) {
			CoC.flags[ kFLAGS.URTA_PC_AFFECTION_COUNTER ] -= love;
		}
		if( CoC.flags[ kFLAGS.URTA_PC_LOVE_COUNTER ] === 1 ) {
			...
			if( love > 0 && CoC.flags[ kFLAGS.AMILY_VISITING_URTA ] === 0 && SceneLib.amilyScene.amilyFollower() && CoC.flags[ kFLAGS.AMILY_FOLLOWER ] === 1 ) {
				CoC.flags[ kFLAGS.AMILY_NEED_TO_FREAK_ABOUT_URTA ] = 1;
			}
			return true;
		} else {
			return false;
	...
	Urta.prototype.urtaAvailableForSex = function() {
		return this.urtaFuckbuddy() && SceneLib.scylla.action !== SceneLib.scylla.SCYLLA_ACTION_FUCKING_URTA && CoC.flags[ kFLAGS.URTA_ANGRY_AT_PC_COUNTDOWN ] === 0;
	};
	Urta.prototype.urtaFuckbuddy = function() { //Returns true if Urta is either the player's fuckbuddy or lover
		if( SceneLib.urtaQuest.urtaBusy() || CoC.flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] === -1 ) {
			return false;
		}
		if( this.urtaJustFriends() ) {
			return false;
		}
		return (CoC.flags[ kFLAGS.TIMES_FUCKED_URTA ] > 0);
	};
	Urta.prototype.urtaJustFriends = function() {
		return CoC.flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] === 0 && CoC.flags[ kFLAGS.URTA_PC_LOVE_COUNTER ] === -1;
	};
	Urta.prototype.urtaAtBar = function() { //Is Urta physically at the Wet Bitch?
		return (!SceneLib.urtaQuest.urtaBusy() && CoC.flags[ kFLAGS.AMILY_VISITING_URTA ] !== 1 && CoC.time.hours < 15 && CoC.flags[ kFLAGS.KATHERINE_UNLOCKED ] !== 3);
	};
	Urta.prototype.urtaDrunk = function() {
		//Preg = no drinking!;
		...
		if( CoC.time.hours > 12 && CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00146 ] < 1 ) {
			return true;
		}
		if( CoC.time.hours > 8 && CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00146 ] >= 1 ) {
			return true;
		} else {
			return false;
		}
	};
	Urta.prototype.urtaOpenAboutEdryn = function() {
		//Did it come out from the scylla threesome?;
		if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00145 ] > 0 ) {
			return true;
		}
		//Did it come out from marble?;
		if( CoC.flags[ kFLAGS.URTA_KNOWS_PC_HAS_MARBLE_FOLLOWER ] > 0 ) {
			return true;
		}
		//Did it come out from discussions?;
		if( CoC.flags[ kFLAGS.URTA_OPEN_ABOUT_EDRYN ] > 0 ) {
			return true;
		}
		return false;
	};
	Urta.prototype.knockUpUrtaChance = function() { //Moved here from UrtaPregs since it needs to be accessed from outside Urta
		//GTFO IF PREGGO;
		if( this.pregnancy.isPregnant ) {
			return;
		}
		...
		var chance = 10;
		var temp = CoC.player.cumQ() / 25;
		if( temp > 40 ) {
			temp = 40;
		}
		chance += temp;
		//Bonus virility time!;
		temp = CoC.player.virilityQ() * 100;
		if( temp > 50 ) {
			temp = 50;
		}
		chance += temp;
		//FINAL ROLL!;
		if( chance > Utils.rand( 100 ) ) {
			this.pregnancy.knockUpForce( PregnancyStore.PREGNANCY_PLAYER, 384 );
			CoC.flags[ kFLAGS.URTA_PREGNANT_DELIVERY_SCENE ] = 0;
		}
	};
	Urta.prototype.urtaBarDescript = function() {
		MainView.outputText( '\n\n', false );
		//Urta PISSED;
		if( CoC.flags[ kFLAGS.URTA_ANGRY_AT_PC_COUNTDOWN ] > 1 ) {
			MainView.outputText( 'Urta is sitting at a table, swishing a bottle around and looking forlorn.  She glances up and sees you, then immediately turns away.', false );
			return false;
		}
		//	[PISSED URTA TALK];
		...
		//[Post Pissed Talk];
		if( CoC.flags[ kFLAGS.URTA_ANGRY_AT_PC_COUNTDOWN ] === 1 ) {
			MainView.outputText( 'Urta is sitting at a table, swishing a bottle around and looking forlorn.  She looks up at you and sighs, "<i>Look, I still don\'t think we\'ll ever be the same, but to be frank, we\'re great in the sack and I\'ve been lonely.  For now, I\'ll put the past behind me.</i>"', false );
			CoC.flags[ kFLAGS.URTA_ANGRY_AT_PC_COUNTDOWN ] = 0;
			return true;
		}
		//Raphael betrayed reward;
		if( CoC.flags[ kFLAGS.RAPHEAL_COUNTDOWN_TIMER ] === -1 && CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00148 ] === 0 ) {
			MainView.outputText( 'Urta has an ecstatic grin plastered across her muzzle, and it only gets wider when she sees you.  Perhaps you should see what all the fuss is about?', false );
			return true;
		}
		if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00350 ] === 1 ) {
			MainView.outputText( 'Urta is sitting at her usual table, holding her head in her hands.  She doesn\'t even seem to notice you.', false );
			return true;
		}
		//[URTA ASHAMED];
		if( CoC.flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] === -1 ) {
			MainView.outputText( 'You see Urta on a bench in the corner with her legs crossed nervously.  Her tail is curling tightly around her leg, hiding the massive surprise between her legs.  In her hand is a mostly empty bottle of whiskey.  She\'s wobbling back and forth in her seat, and keeping her head down, refusing to meet your gaze any time you spare a glance her way.  It looks like she wants nothing to do with you.', false );
			return true;
		}
		//Post Scylla Appearance;
		if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00145 ] === 0 && CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00143 ] > 0 ) {
			MainView.outputText( 'Urta\'s at her normal table, nursing a glass of water with nary a bottle of alcohol around.  She\'s fidgety and constantly glancing your way, but she never meets your eyes or acknowledges that she saw you.  Something is eating away at her.', false );
			return true;
		}
		//PREGNANT URTA;
		if( this.pregnancy.isPregnant ) {
			if( this.pregnancy.type === PregnancyStore.PREGNANCY_PLAYER ) {
				SceneLib.urtaPregs.urtaPregAppearance();
			} else {
				{ //Egg-Stuffed Urta replaces the normal 'display' for Urta in the Wet Bitch screen;
				}
				///This scene lasts for the duration of Urta's egg pregnancy, whatever that is;
				//Egg Level 1:;
				if( CoC.flags[ kFLAGS.URTA_EGGS ] < 20 ) {
					MainView.outputText( 'Urta is sitting quietly at her usual seat, sipping a mug of some non-alcoholic beer.  Though her clothes are still in place, you can just make out the bulging midriff stretching them taut, a consequence of all the eggs in her womb.' );
				} else if( CoC.flags[ kFLAGS.URTA_EGGS ] < 40 ) {
					MainView.outputText( 'Urta is sitting rather uncomfortably at her usual seat.  Her swollen stomach is so large it keeps her from wearing her usual outfit, instead forcing her into a mini-shirt and a skirt bottom, allowing her midriff to be bare.  She occasionally runs a hand over the expanse, as if she can\'t believe it\'s actually there.' );
				}
				//Egg Level 3:;
				else {
					MainView.outputText( 'Urta\'s bloated belly forces her to sit side-saddle at her usual table, the lumpy, egg-stuffed mass ballooning out in front of her.  Forced into a midriff-baring ensemble, she frequently touches it, occasionally seeming to trace an egg through the taut skin.' );
				}
			}
			return true;
		}
		...
		if( CoC.flags[ kFLAGS.URTA_TIMES_EGG_PREGGED ] > 0 && !this.pregnancy.isPregnant && CoC.flags[ kFLAGS.URTA_FLATBELLY_NOTICE ] === 0 ) {
			MainView.outputText( 'Urta is seated at her usual spot in her usual dress, eagerly chugging down mug after mug of booze; her belly is washboard-flat again, and she\'s clearly making up for lost time after having given up alcohol for her \'pregnancy\'.' );
			CoC.flags[ kFLAGS.URTA_FLATBELLY_NOTICE ] = 1;
			return true;
		}
		//[Love Urta Bar Appearance];
		if( CoC.flags[ kFLAGS.URTA_PC_LOVE_COUNTER ] === 1 ) {
			if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00146 ] === -1 || !this.urtaDrunk() ) {
				MainView.outputText( 'Urta is sitting at her usual table, sipping a glass of wine and wearing a form-fitting evening gown of shimmering black.  She looks up at you, happiness filling her eyes when she notices you entering the bar.', false );
			}//(DRUNK);
			else {
				MainView.outputText( 'Urta is sitting at her usual table wearing a shimmering black dress.  She\'s pretty clearly sloshed judging by her bleary gaze as she looks up at you.  The front of her dress visibly tents, and she waves you over with a lecherous grin.', false );
			}
			return true;
		}
		if( CoC.flags[ kFLAGS.TIMES_FUCKED_URTA ] === 0 && CoC.flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] > -1 ) {
			MainView.outputText( 'You see Urta on a bench in the corner with her feet propped up on a stool.  Her tail is coiled tightly around her left leg, just like every other time you\'ve seen her.  The pretty gray fox gives you a slight nod before turning her attention back to the large square bottle of whiskey in front of her.', false );
			return true;
		}
		//[URTA FUCKBUDDIED];
		if( CoC.flags[ kFLAGS.TIMES_FUCKED_URTA ] > 0 ) {
			MainView.outputText( 'You see Urta on a bench in the corner with her feet propped up on a stool.  She has a small mug on a table, quite different from the swill she chugged when you first met.  Her clothing is... far more risqué.  The top she\'s chosen for the moment is sheer to the extreme, making her proud black nipples visible to anyone who glances her way, and the short skirt she\'s wearing is nearly revealing her cunny.  ', false );
			if( CoC.flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] < 11 ) {
				MainView.outputText( 'Thankfully she still has her other addition trapped under her tail, for now.', false );
			} else {
				MainView.outputText( 'Her half-hard horse-cock is plainly visible to anyone who spares her a glance, barely contained by a pair of tightly woven fishnet stockings.  She\'s looking at you and wiggling a finger your way...', false );
			}
			return true;
		}
		CoC_Settings.error( '' );
		$log.error( 'URTA Error: Bar descript encountered unexpected scenario.' );
		return false;
	};

	Urta.prototype.urtaBarApproach = function() {
		this.urtaSprite();
		//Images are handled by ImageManager now. This was the old way of doing things.;
		//showImage(0);;
		var temp = null;
		var spank = null;
		if( (CoC.player.hasCock() && CoC.player.cockThatFits( this.urtaCapacity() ) >= 0) || CoC.player.hasKeyItem( 'Deluxe Dildo' ) >= 0 ) {
			spank = this.spankTheShitOutOfUrtaAndMakeHerCreamHerselfFromProstateStimulationAloneLikeTheHornyDrunkenSlutSheReallyIs;
		}
		MainView.clearOutput();
		//Raphael Reward;
		if( CoC.flags[ kFLAGS.RAPHEAL_COUNTDOWN_TIMER ] === -1 && CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00148 ] === 0 ) {
			CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00148 ] = 1;
			CoC.player.gems += 1000;
			MainView.statsView.show();
			MainView.outputText( ImageManager.showImage( 'urta-bar' ), false );
			MainView.outputText( 'Urta claps as you walk up to her with an exuberant smile spread across her vulpine face.  She proudly announces, "<i>You really turned in the Russet Rogue?!  I almost can\'t believe it.  Do you know how many women that rascal has fleeced out of their possessions?  The last count I made was over three dozen, and he even had the audacity to try it with me.  I\'m ashamed to admit it nearly worked, but once he saw ALL of me, well he couldn\'t keep up the facade.  I nearly caught him that night.</i>"\n\n', false );
			MainView.outputText( 'You smirk at her tale.  Knowing Raphael, seeing a horny Urta would\'ve made him white as a ghost.  She goes on to list some of his crimes, "<i>He\'s guilty of petty theft, disturbing the peace, robbery, lewd public behavior, lying under oath, defacing a public structure, destroying a public structure... I\'d probably have to get the paperwork to remember the rest.  In truth his shenanigans aren\'t usually a big deal individually, but they\'ve added up and garnered him a fairly sizable bounty.  He\'ll serve his punishment, and I hope he learns his lesson for a change.</i>"\n\n', false );
			MainView.outputText( 'Urta fishes out a rather sizable pouch and tosses it onto the table, the numerous gems inside tinkling loudly as it plops down in front of you.  You untie the silken cord that holds the leather bag closed and see hundreds of sparkling gems, perhaps even a thousand.  Wow!  Raphael must have pissed off the wrong people!\n\n', false );
			MainView.outputText( 'The gray fox knocks back a swig from her bottle and sighs as if you\'ve just removed one of the many weights hanging from her shoulders.  She looks back at you with a twinkle in her eyes, and you realize you\'ve definitely improved her opinion of you.  Sadly there isn\'t time for more chit-chat, and you head back to camp, your gem-pouch heavy with new weight.', false );
			//(+love score!);
			this.urtaLove( 3 );
		...
		if( SceneLib.katherineEmployment.canTalkToUrta() ) { //Katherine training discussion
			SceneLib.katherineEmployment.talkToUrta();
			return;
		}
		//PREGNANT URTA;
		if( this.pregnancy.type === PregnancyStore.PREGNANCY_PLAYER ) {
			SceneLib.urtaPregs.urtaPreggoApproached();
			return;
		}
		//HERE WE GOEZ!;
		if( CoC.flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] >= 5 && ((CoC.player.inRut && CoC.player.hasCock()) || (CoC.player.inHeat && CoC.player.hasVagina() )) ) {
			if( this.urtaDrunk() ) {
				SceneLib.urtaHeatRut.approachDrunkenUrta();
			} else {
				SceneLib.urtaHeatRut.approachSoberUrtaHeatRutProc();
			}
			return;
		}
		//[URTA FRIEND FUCKBUDDY BUT UNHORNY];
		if( CoC.flags[ kFLAGS.URTA_TIME_SINCE_LAST_CAME ] > 0 && !this.urtaDrunk() ) {
			MainView.outputText( ImageManager.showImage( 'urta-bar' ), false );
			//Cockblock marble convo;
			if( SceneLib.marbleScene.marbleAtCamp() && CoC.flags[ kFLAGS.URTA_KNOWS_PC_HAS_MARBLE_FOLLOWER ] === 0 ) {
				CoC.flags[ kFLAGS.URTA_KNOWS_PC_HAS_MARBLE_FOLLOWER ] = 1;
				this.marbleCockuBlockuUrta();
				return;
			}
			//Twu wuv talk;
			if( CoC.flags[ kFLAGS.URTA_PC_AFFECTION_COUNTER ] >= 30 && CoC.flags[ kFLAGS.URTA_PC_LOVE_COUNTER ] === 0 ) {
				this.UrtaTwuWuvOffer();
				return;
			}
			//MainView.outputText('You approach Urta, who gives you a friendly smile and begins talking with you.  Thanks to her unusual endowments, it\'s easy to tell she\'s not quite in the mood.  The conversation is still pleasant though, and the two of you knock back a few ales while Urta recounts some of the wilder scenarios she\'s encountered as the captain of Tel\'Adre\'s guard.', false);;
			this.QBsTalkExpack();
			return;
		}
		...
		if( CoC.flags[ kFLAGS.URTA_PC_LOVE_COUNTER ] === 1 ) {
			...
			if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00146 ] === -1 || !this.urtaDrunk() ) {
				MainView.outputText( ImageManager.showImage( 'urta-bar-drunk' ), false );
				MainView.outputText( 'You walk up to Urta and you see her pivot herself to the side.  Her dress visibly tents at the sight of you, and you smirk as you realize she shifted in position to avoid slamming her hardening cock into the bottom of the table.  You cuddle close and wrap an arm around your lover, giving her a quick kiss on the lips and fondling her through her dress.  The tent immediately darkens as a rush of pre soaks into the material, and Urta half-pants, half-asks, "<i>Oooh, lover, what do you have in mind today?</i>"\n\n', false );
				MainView.outputText( 'You could go back to her place, suck her off under the table, or eat her out under the table.', false );
				if( CoC.player.canOviposit() ) {
					if( CoC.flags[ kFLAGS.URTA_TIMES_EGG_PREGGED ] === 0 ) {
		...
		if( CoC.flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] === 0 && CoC.flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] > -1 ) {
			MainView.outputText( ImageManager.showImage( 'urta-bar-drunk' ), false );
			MainView.outputText( 'Urta smiles broadly and noisily slurps a few dribbles of whiskey from her shiny black lips, watching you approach.  She gives you a smoking-hot \'come-hither\' look and slurs, "<i>I didn\'t think I\'d get a chance to bump into you here, cutie.  Wanna dance?</i>"\n\n', false );
			MainView.outputText( 'Before you can answer, something thumps the table hard enough to shake it.  You look at Urta quizzically, and she shrugs through a blush so bright it\'s visible through her fur.  The lithe fox squirms in her seat, panting and gasping, "<i>Ummm, maybe later, I just realized I have to- OH MY GOD WHAT IS THAT?</i>"\n\n', false );
			MainView.outputText( 'You spend a moment looking over your shoulder, but can\'t for the life of you figure out what she was freaking out about.  She must be really drunk!  You turn back to scold her, but Urta has vanished!  She must have left in a hurry – she left her bottle of "<i>John Doe</i>" behind, and still half-full.  The bar\'s back-door swings closed, maybe she went that way.  What do you do?', false );
		...
		if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00146 ] !== -1 && this.urtaDrunk() ) {
			MainView.outputText( ImageManager.showImage( 'urta-bar-drunk' ), false );
			MainView.outputText( 'The drunken fox roughly gropes you and whispers in your ear, "<i>I NEED a good fuck right now, and it feels like someone wants a piece of Urta.  I can guess just which piece you\'re wanting too.  The only question is do I ', false );
			//CUT FOR NOW outputText('throw you over the table and fuck you,', false);;
			MainView.outputText( 'let the beast loose and bounce you on my lap while we drink, ', false );
			MainView.outputText( 'jerk off onto your face in front of everyone, or have to remember your ass for later when you bolt for the door.  What\'ll it be, hun?</i>"', false );
			temp = null;
			if( CoC.player.canOviposit() ) {
				if( CoC.flags[ kFLAGS.URTA_TIMES_EGG_PREGGED ] === 0 ) {
					MainView.outputText( '\n\nHmm... sounds like she wants to fuck almost as badly as you want to get these eggs out of you.  Besides, didn\'t Urta say to you once that she\'s basically barren and sterile?  You\'d be doing her a favor by letting her carry your eggs, wouldn\'t you?  Let\'s see if she\'s drunk enough to let you fill her full...' );
		...
		if( CoC.flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] >= 11 && Utils.rand( 2 ) === 0 ) {
			MainView.outputText( ImageManager.showImage( 'urta-bar' ), false );
			MainView.outputText( 'You approach Urta, who gives you a naughty wink and asks, "<i>Would you like to play with my little pony?</i>"  One of her hands is trailing along her leg, caressing her member as it hardens under her tight fishnet stockings.\n\n(You can suck her off under the table or go back to her place to fuck.)', false );
		} else {
			MainView.outputText( ImageManager.showImage( 'urta-bar' ), false );
			MainView.outputText( 'You approach Urta, who gives you a lascivious grin ', false );
			if( CoC.flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] < 11 ) {
				MainView.outputText( 'moments before you hear her horse-cock thump into the bottom of the table.  ', false );
			} else {
				MainView.outputText( 'and pets her already-growing horse-cock through her fishnet stockings.  ', false );
			}
			if( Utils.rand( 2 ) === 0 ) {
				MainView.outputText( 'Urta asks, "<i>I\'m really horny, do you think you could help me get off?  I guess we could go fuck at my place, or you could always just help me out under the table.</i>"  A blush colors her face through the gray fur as she suggests the latter option.', false );
			} else {
				MainView.outputText( 'She asks, "<i>Would you like to go back to my place and help me play with my \'little\' friend again?  Or maybe you could climb under the table and give me some relief?</i>"', false );
			}
		}
		if( CoC.player.canOviposit() ) {
			if( CoC.flags[ kFLAGS.URTA_TIMES_EGG_PREGGED ] === 0 ) {
	...
	Urta.prototype.drinkUrtasBoozePtTwo = function() {
		this.urtaSprite();
		MainView.outputText( '', true );
		MainView.outputText( 'With a happy sigh you ', false );
		if( CoC.player.hasVagina() && CoC.player.totalCocks() === 0 ) {
			MainView.outputText( 'squat down and ', false );
		}
		MainView.outputText( 'open up your ' + CoC.player.armorName + ', releasing the pressure onto a patch of grass.  It takes forever to stop, but when it does you feel like a million bucks.  You turn and start to close up your kit when you realize you aren\'t alone.  Urta is watching you, her lips parted hungrily.  She\'s still peeing, but what\'s truly shocking is the mammoth package she\'s got hanging between her legs.  It\'s a massive horse-shaft, patterned black and pink, slung down from between a pair of apple-sized balls.  Before your eyes the equine beast begins hardening, forcing the distracted fox to empty the last of her urine against a building\'s wall.  She blushes furiously, but the drunken fox doesn\'t seem to care about her shame...\n\n', false );
		MainView.outputText( 'She shakes her massive horse-meat more than a few times, shuddering lightly with pleasure as her tip flares a bit in response.  Urta stumbles over moaning, "<i>This ish all your fault!  You damned... sexy... mmm... letsh fuck!</i>"\n\n', false );
		MainView.outputText( 'The stumbling hermaphrodite is openly rubbing herself as she closes in on you.  What do you do?', false );
		//[RUN!] [LET HER];
	...
	Urta.prototype.drinkUrtasBoozeRun = function() {
		this.urtaSprite();
		MainView.outputText( '', true );
		MainView.outputText( 'You turn about and run, nearly smacking into the door frame in your haste to get away.  You hear a half-strangled sob behind you as the door swings closed.', false );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	...
	Urta.prototype.drinkUrtasBoozeLetHer = function() {
		...
		CoC.flags[ kFLAGS.URTA_TIME_SINCE_LAST_CAME ] = 5;
		EngineCore.hideUpDown();
		CoC.player.orgasm();
		EngineCore.dynStats( 'sen', -2 );
		//[LET HER];
		MainView.outputText( '', true );
		MainView.outputText( ImageManager.showImage( 'urta-behindbar-fuck' ), false );
		MainView.outputText( 'You nod at Urta, feeling a bit relaxed from all the whiskey and more than a little intrigued by the beast between the slender fox\'s legs.  She\'s on you before you know it, her softly furred hand squeezing one of your ', false );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( Descriptors.ballsDescriptLight(), false );
		} else if( CoC.player.biggestTitSize() > 1 ) {
			MainView.outputText( Appearance.biggestBreastSizeDescript(), false );
		} else {
			MainView.outputText( Descriptors.nippleDescript( 0 ) + 's', false );
		}
		MainView.outputText( ', as her tongue spears between your lips.  She practically mouth-rapes you as her trembling member grinds between your ' + CoC.player.legs() + ', smearing them with a generous helping of foxy horse-pre.  The horny drunk slurs into your ear, "<i>Gawdsh above, are you this much of a schlut for everyone with one of these?</i>"\n\n', false );
		MainView.outputText( 'With surprising strength, the inebriated fox pushes you against an old bench, flat onto your ', false );
		if( CoC.player.biggestTitSize() > 1 ) {
			MainView.outputText( 'tits', false );
		} else {
			MainView.outputText( 'belly', false );
		}
		MainView.outputText( '.  She pulls at your ' + CoC.player.armorName + ' until your crotch is completely exposed and vulnerable, revealing your ' + CoC.player.assholeOrPussy(), false );
		if( CoC.player.cockTotal() > 0 ) {
			MainView.outputText( ' and ' + Descriptors.multiCockDescript(), false );
		}
		MainView.outputText( '.  Urta snuggles close, brushing her nose along your thighs before plunging her tongue deep into your ' + CoC.player.assholeOrPussy() + ' in one powerful, drunken lick.  She slurps and licks, humming away until you\'re ', false );
		if( CoC.player.hasVagina() ) {
			MainView.outputText( 'wet', false );
		} else if( CoC.player.totalCocks() > 0 ) {
			MainView.outputText( 'suitably lubricated and leaking pre', false );
		} else {
			MainView.outputText( 'suitably lubricated', false );
		}
		MainView.outputText( '.\n\n', false );
		MainView.outputText( 'With a quick shift in position, she forces herself inside, her arms supporting her jiggling chest as she leans over you, dragging her nipples up your back.  Her fat shaft forces your body to spread wide.  The fat head and ring of prepuce strokes your inner walls, squeezing grunts of pleasure from you as the drunken fox forcefully begins fucking you, rutting like a beast.  You can feel her nipples harden, digging into your back and eliciting moans from their owner as they\'re repeatedly smashed against you.  Urta starts licking your neck where it joins your shoulder, gently nipping at your tender flesh as her tip begins to flare wider inside you.', false );
		CoC.player.cuntChange( 60, true, true, false );
		MainView.outputText( '\n\n', false );
		MainView.outputText( '"<i>Mmmm I shhoulda ushed a condom, you\'re gonna be a messssss!</i>" she cries as she climbs atop her sexual peak, mounting it just like she\'s mounting your ' + CoC.player.assholeOrPussy() + '.  You\'re spread wide, the equine member rammed tightly ', false );
		if( CoC.player.hasVagina() ) {
			MainView.outputText( 'against your cervix as the tip presses firmly against it', false );
		} else {
			MainView.outputText( 'into your rectum', false );
		}
		MainView.outputText( ', unloading a thick batch of cream directly into your ', false );
		if( CoC.player.hasVagina() ) {
			MainView.outputText( 'waiting womb', false );
		} else {
			MainView.outputText( 'abused asshole', false );
		}
		MainView.outputText( '.', false );
		if( CoC.player.pregnancyIncubation > 0 && CoC.player.pregnancyIncubation < 250 ) {
			MainView.outputText( '  Whatever\'s growing inside you is probably bathing in the stuff now.', false );
		} else if( CoC.player.hasVagina() ) {
			MainView.outputText( '  You briefly wonder if she\'s capable of knocking you up, and what the baby would look like.', false );
		} else {
			MainView.outputText( '  Your backside fills with warmth as her spunk seeps deep inside you.', false );
		}
		MainView.outputText( '  Another blast fills you to capacity, and you\'re mortified by the sensation of your body being shifted by your growing belly.\n\n', false );
		MainView.outputText( 'Urta sucks on the edge of your ear, still pumping away as she pours a bit more spooge into your now equine-sized cavity.  Your alcohol-dulled mind reels as your body caves in to the sticky fluid assault, cumming hard and wriggling around her shaft, moaning repeatedly as you clamp down tightly.  ', false );
		if( CoC.player.totalCocks() > 0 ) {
			...
			MainView.outputText( 'our ' + Descriptors.multiCockDescriptLight() + ' squirts hard, unloading a batch of sticky jism under your ', false );
			if( CoC.player.pregnancyIncubation > 0 && CoC.player.pregnancyIncubation < 100 ) {
				MainView.outputText( 'pregnancy swollen ', false );
			}
			MainView.outputText( 'belly, making you squish with each brutal thrust of your oversized lover\'s rod.', false );
		} else {
			if( CoC.player.hasVagina() ) {
				MainView.outputText( 'Your pussy dribbles and clenches, leaking Urta\'s whiteness as she claims it for her own.', false );
			} else {
				MainView.outputText( 'Your ' + Descriptors.assholeDescript() + ' clamps down tightly, spasming in pleasure around the invader.', false );
			}
		}
		if( CoC.player.totalCocks() > 0 ) {
			MainView.outputText( '  As you cum, more and more whiteness dribbles out until a puddle forms underneath you, showing Urta just how much you enjoyed the rough, drunken fuck.\n\n', false );
		} else {
			MainView.outputText( '\n\n', false );
		}

		MainView.outputText( 'She pulls out, dripping everywhere.  The fox leans down and proceeds to give your still quivering hole a tongue-bath, thoroughly cleaning her fluids from the entrance and teasing you until you clamp down, locking the rest inside.  With a gentle slap on your ' + Descriptors.buttDescript() + ' Urta says, "<i>Thankss for the quick fuck.  Maybe shometime you can ', false );
		if( CoC.player.cockTotal() > 0 ) {
			MainView.outputText( 'stick it in me', false );
		} else {
			MainView.outputText( 'come back for an even bigger load', false );
		}
		MainView.outputText( '.</i>"\n\n', false );
		MainView.outputText( 'The fox helps you to your feet and the two of you get dressed, stumbling a bit over each other and giggling happily.  Urta gives you a quick peck on the lips and whispers, "<i>Thanksh... for everything,</i>" before the two of you make your way back into the bar and go your separate ways.\n\n', false );
		if( CoC.player.hasVagina() ) {
	...
	Urta.prototype.urtaFollowedOutBack = function() {
		this.urtaSprite();
		MainView.outputText( '', true );
		MainView.outputText( ImageManager.showImage( 'urta-behindbar-masti' ), false );
		MainView.outputText( 'The backdoor opens out into a narrow alley.  Surprisingly, the alleyway is covered in dirt and grass.  Awnings from the buildings above cover the alley in shadow, making it difficult to see anything out here.  You can hear quiet gasps and a wet, repetitive sound.  Quietly approaching the source of the noise, you eventually spot the dark-colored fox.  Her tail is no longer wrapped around her leg.  Instead, it\'s splayed out behind her and her skirt is pressed up over her legs.  Urta\'s totally exposed, and it\'s plain to see that between the fox-girl\'s legs there\'s more than a little horse.  Both her hands are wrapped tightly around the quivering column of horse-flesh, stroking the pre-cum dribbling fuck-stick relentlessly as her apple-sized balls bounce under her, occasionally revealing a feminine gash drooling with clear nectar.\n\n', false );
		MainView.outputText( 'Urta moans and cries, "<i>Ooh, just go down! Please cum and go down!  Why won\'t you go awayyyyyy...</i>" eventually breaking off into blubbering sobbing, interrupted by the occasional involuntary gasp of pleasure.\n\n', false );
		//[Sneak away] [Watch] [Masturbate With Her] [Fuck Her];
		MainView.outputText( '(You could fuck her, masturbate with her, watch, or leave her like that.)', false );
	...
	Urta.prototype.urtaSneakAwayFromMasturbate = function() {
		this.urtaSprite();
		MainView.outputText( '', true );
		MainView.outputText( 'You sneak away from the depressing sight, and decide to head back to camp.', false );
	...
	Urta.prototype.dualUrtaMasturbation = function() {
		...
		CoC.flags[ kFLAGS.URTA_TIME_SINCE_LAST_CAME ] = 5;
		MainView.outputText( '', true );
		MainView.outputText( ImageManager.showImage( 'urta-behindbar-masti-dual' ), false );
		MainView.outputText( 'You come out of the shadows, shedding your ' + CoC.player.armorName + '.  ', false );
		if( CoC.player.cor < 60 ) {
			MainView.outputText( 'You blush furiously at exposing yourself in such a way', false );
		} else {
			MainView.outputText( 'Smiling seductively, you revel in exposing yourself so suddenly', false );
		}
		MainView.outputText( '.  Urta looks up at you in shock, blinking tears from her eyes and stammering, "<i>B-b-b-ut I have th-thish hideoush thing... Aren\'t you scared of it?</i>"\n\n', false );
		MainView.outputText( 'Gently wiping a tear from her muzzle, you comfort the distraught fox and sit down across from her, giving her a perfect view of your ', false );
		if( CoC.player.totalCocks() > 0 ) {
			...
			MainView.outputText( 'bare groin and ' + Descriptors.assholeDescript(), false );
		}
		MainView.outputText( '.  ', false );
		if( CoC.player.totalCocks() > 0 ) {
			MainView.outputText( 'The situation gives you all the spark you need to rise to your full size, ', false );
			if( CoC.player.cockArea( 0 ) < 40 ) {
				MainView.outputText( 'even if Urta\'s member dwarfs you.', false );
			} else if( CoC.player.cockArea( 0 ) < 70 ) {
				MainView.outputText( 'showing Urta you\'re just as big.', false );
			} else {
				MainView.outputText( 'proving to Urta just who has the bigger burden.', false );
			}
			MainView.outputText( '  ', false );
		}
		if( CoC.player.hasVagina() ) {
			if( CoC.player.vaginas[ 0 ].vaginalWetness > AppearanceDefs.VAGINA_WETNESS_SLICK ) {
				MainView.outputText( 'A trickle of wetness escapes your nether-lips, starting to puddle underneath you.', false );
			} else {
				MainView.outputText( 'Quickly engorging, your lower lips begin to part around your ' + Descriptors.clitDescript() + '.', false );
			}
			MainView.outputText( '  ', false );
		}
		MainView.outputText( 'The heat coursing through you quickly shows itself on your ', false );
		if( CoC.player.biggestTitSize() >= 1 ) {
			MainView.outputText( Descriptors.allBreastsDescript(), false );
		} else {
			MainView.outputText( 'chest', false );
		}
		MainView.outputText( ' as your ' + Descriptors.nippleDescript( 0 ) + 's become full and sensitive.\n\n', false );
		MainView.outputText( 'Urta relaxes, leaning against a wall as she returns to pleasuring her mismatched member.  Her eyes lock onto your groin as you join her, and she lets out a sigh that\'s equal parts relieved and inebriated.  Together, the two of you pleasure your bodies, each watching the other\'s squirming form and blissful expression.  ', false );
		if( CoC.player.totalCocks() === 1 ) {
			MainView.outputText( 'Your hands caress your ' + Descriptors.cockDescript( 0 ) + ' feverishly, tweaking every ounce of pleasure from the bloated rod.  ', false );
		}
		if( CoC.player.totalCocks() > 1 ) {
			MainView.outputText( 'Your hands caress your ' + Descriptors.multiCockDescript() + ' in turn, dancing back and forth from rod to rod as you attempt to play with every available pleasure outlet.  ', false );
		}
		if( CoC.player.hasVagina() ) {
			MainView.outputText( 'Delving deeply between your legs, you press forward into the source of your feminine pleasures, touching and rubbing, caressing your swollen button as you get closer and closer to orgasm.\n\n', false );
		}
		if( CoC.player.gender === 0 ) {
			MainView.outputText( 'Delving down between your legs, rubbing along your taint before you slowly penetrate your ' + Descriptors.assholeDescript() + ', pleasuring yourself in the only way possible.\n\n', false );
		}

		MainView.outputText( 'The cute fox-girl reaches her peak, crying out in a loud moan that echoes around the alley.  Her equine shaft trembles, the medial ring becoming more pronounced while the flat head flares wide mid-orgasm.  A blast of cum launches into the air, fans out overhead, and splatters down over you.  Urta looks horrified, and yanks herself back, succeeding only in depositing the next blast of spoogey goodness into her own face.  The situation would be comical if the foxy horse-goo wasn\'t such a great lubricant for your hands, and with that thought in your mind, you cum too.\n\n', false );
		if( CoC.player.totalCocks() > 0 ) {
			if( CoC.player.cumQ() < 250 ) {
				MainView.outputText( 'Jism spurts out onto the alley floor, forming a nice little puddle as ', false );
				if( CoC.player.totalCocks() > 1 ) {
					MainView.outputText( 'each of ', false );
				}
				MainView.outputText( 'your ' + Descriptors.multiCockDescriptLight() + ' empties itself.  ', false );
			} else if( CoC.player.cumQ() < 500 ) {
				MainView.outputText( 'Jism pours out onto the alley floor as your body releases the copious amount of cum it can produce.  It puddles before you, running off the stone into the grass.  ', false );
			} else if( CoC.player.cumQ() < 1000 ) {
				MainView.outputText( 'Jism erupts from ', false );
				if( CoC.player.totalCocks() > 1 ) {
					MainView.outputText( 'each of ', false );
				}
				MainView.outputText( 'your ' + Descriptors.multiCockDescriptLight() + ', blasting into the air before splattering onto the ground between you and Urta.  It forms a massive puddle, spreading as you pump gob after gob of cum into it until the sticky spooge is soaking your ' + CoC.player.legs() + ' and Urta too.', false );
			} else {
				MainView.outputText( 'Jism fountains from ', false );
				if( CoC.player.totalCocks() > 1 ) {
					MainView.outputText( 'each of ', false );
				}
				MainView.outputText( 'your ' + Descriptors.multiCockDescriptLight() + ', flooding the small alleyway with an unholy amount of cum.  In no time you\'ve painted Urta and yourself from head to toe, and the alleyway sloshes with the stuff as your production overwhelms the ground\'s ability to absorb liquid.  ', false );
			}
		}
		if( CoC.player.hasVagina() ) {
			MainView.outputText( 'Your sex squishes wetly around a finger, clenching down tightly', false );
			if( CoC.player.vaginas[ 0 ].vaginalWetness === AppearanceDefs.VAGINA_WETNESS_SLAVERING ) {
				MainView.outputText( ' and gushing everywhere', false );
			}
			MainView.outputText( '.  ', false );
		} else {
			MainView.outputText( 'Your sphincter clenches down tightly around your invading digit as you get off, and the rapt attention of your audience certainly helps.  ', false );
		}
		MainView.outputText( 'Sighing happily, you give your nipple a tweak as Urta finally finishes cumming, ', false );
		if( CoC.player.cumQ() >= 1000 ) {
			MainView.outputText( 'adding to the massive pool on the ground after ', false );
		}
		MainView.outputText( 'drenching herself thoroughly.\n\n', false );
		MainView.outputText( 'Urta lets out a VERY relieved-sounding sigh and murmurs, "<i>Oh godsh, thank you.  You don\'t know how hard it ish to hide thish beasht from EVERYONE.  You\'re a good friend for helping me with thish... let\'sh do thish again sometime, ok?</i>"\n\n', false );
		MainView.outputText( 'She staggers up to her feet and rapidly discards her sodden clothes.  With a resigned sigh, Urta pulls a loose stone out of the side of a building and removes a sheer skirt and diaphanous top from the cubby.  They slip on easily, though the new clothes look a bit more \'slutty\' than her previous pair, easily displaying her hard black nipples.  Urta replaces the stone, wobbling a bit unsteadily before she ', false );
		if( CoC.player.cumQ() >= 1000 ) {
			MainView.outputText( 'tip-toes through the spunk', false );
		} else {
			MainView.outputText( 'gets her balance and heads for the bar\'s backdoor', false );
		}
		MainView.outputText( '.  Her tail curls up around the softening horse-cock, hiding it from view as she blows you a sloppy kiss and disappears inside.\n\n', false );
		MainView.outputText( 'You waste no time cleaning up and stagger back inside after her, sated and happy with your new friend and her \'benefits\'.', false );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
		CoC.player.orgasm();
	};
	//[FUCK HER];
	Urta.prototype.urtaFuckHer = function( afterBefriending ) {
		this.urtaSprite();
		CoC.flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ]++;
		CoC.flags[ kFLAGS.TIMES_FUCKED_URTA ]++;
		CoC.flags[ kFLAGS.URTA_TIME_SINCE_LAST_CAME ] = 5;
		var cocks = CoC.player.cockTotal();
		if( !afterBefriending ) {
			MainView.clearOutput();
		}
		//(FUCK HER FEMALE/GENDERLESS);
		if( CoC.player.totalCocks() === 0 ) {
			MainView.outputText( ImageManager.showImage( 'urta-behindbar-female-fuckurta' ), false );
			CoC.player.slimeFeed();
			MainView.outputText( 'You come out of the shadows, shedding your ' + CoC.player.armorName + '.  ', false );
			if( CoC.player.cor < 50 ) {
				MainView.outputText( 'You blush furiously at exposing yourself in such a way' );
			} else {
				MainView.outputText( 'Smiling seductively; you revel in exposing yourself so suddenly' );
			}
			MainView.outputText( '.  Urta looks up at you in shock, blinking tears from her eyes and stammering, "<i>B-b-b-ut I have th-thish hideoush thing... Aren\'t you scared of it?</i>"\n\n', false );
			MainView.outputText( 'Gently wiping a tear from her muzzle, you answer, "<i>Not at all, but I think you\'d have more luck with a nice warm hole to slide that monster into.</i>"\n\n', false );
			MainView.outputText( 'Urta nods, numbed by the unexpected and forward nature of your offer.  She looks up hopefully and pries her hands away from the twitching behemoth in her loins, waiting to see how you intend to deal with it.  You approach her and turn, straddling your ' + CoC.player.legs() + ' over her, watching her expression as you lower yourself down, ', false );
			if( CoC.player.tailType === AppearanceDefs.TAIL_TYPE_HORSE || CoC.player.tailType === AppearanceDefs.TAIL_TYPE_DOG ) {
				MainView.outputText( 'swishing your tail across her taut nipples', false );
			} else {
				MainView.outputText( 'making her wait', false );
			}
			MainView.outputText( '.  Eventually the blunted tip of Urta\'s horse-cock presses against your ' + CoC.player.assholeOrPussy() + '.  Agonizingly, you force yourself to relax and lower yourself down on the mottled pink and black shaft.\n\n', false );
			MainView.outputText( 'The horse-cock spreads you wide, filling you past the point of fullness as you sink further and further along its length.  Urta groans happily, her hips twitching involuntarily and forcing more of the monster-sized cock inside you.', false );
			if( CoC.player.hasVagina() ) {
				if( CoC.player.vaginalCapacity() >= 60 ) {
					MainView.outputText( '  You sigh and sink down, taking it until the folds of her sheath are rubbing your entrance.', false );
				} else {
					MainView.outputText( '  You nearly cry out as you sink down, taking it until the folds of her sheath are rubbing your entrance, even though it hurts to accommodate it.', false );
				}
				CoC.player.cuntChange( 60, true, true, false );
			} else {
			...
			MainView.outputText( '  Urta babbles, "<i>Oh godsh yesh thank you so much-ohmigod it\'s SO HOT inside you.</i>"\n\n', false );
			MainView.outputText( 'You smirk over your shoulder and start bouncing up and down on her, slapping her balls with your ' + Descriptors.buttDescript() + ' each time you bottom out on the thick shaft.  You can feel her ring of prepuce rubbing your inner walls, pressing on nerves normally ignored.  Urta grunts underneath you and you feel her flare growing wider inside you, stretching you further and further with every bounce atop the equine-endowed slut.  Suddenly warmth is flooding into your ', false );
			if( CoC.player.hasVagina() ) {
				MainView.outputText( 'womb', false );
			} else {
				MainView.outputText( 'rectum', false );
			}
			MainView.outputText( ', and Urta\'s tongue is lolling out from her muzzle, drooling down the side of her face as she loses herself in orgasm.\n\n', false );
			MainView.outputText( 'The substantial flare forms a perfect plug inside you and your belly distends slightly as you\'re packed full of foxy-horse-cum.  Your own lusts keep your hips moving, grinding and squeezing the shaft inside you as it packs you full of Urta\'s seed.  Your orgasm explodes through your hips and your ' + CoC.player.legs() + ' give out underneath you, dropping you down onto the fox-girl as the pair of you cum together.  Whiteness begins squirting out around her shaft as her prick begins softening inside you.  You sigh happily, content but too nerveless to stand for the moment.  Peeking over your shoulder and seeing Urta\'s satisfied expression, you guess she feels about the same.\n\n', false );
			MainView.outputText( 'Once the horse-dick inside you has sufficiently softened, you stand up, gasping as a rush of spooge escapes from your ' + CoC.player.assholeOrPussy() + ' now that the \'plug\' is removed.  Urta gives you a cocky grin as she\'s soaked with her fluids, moments before she starts staggering back to her feet.  She wipes up with her clothes and tosses them into the garbage, then wobbles up to a loose brick in the wall and pulls it free, revealing a hidden compartment filled with sets of replacement clothes.  She removes a tight skirt and sheer top, both a bit more revealing than what she wore in the bar earlier, and hurriedly dresses herself.\n\n', false );
			MainView.outputText( 'Urta blows you a kiss as you put your own gear back together and flounces back to the bar, calling out, "<i>We should do thish again shoon!</i>"\n\n', false );
			MainView.outputText( '"<i>Yes indeed,</i>" you muse - the sooner the better.', false );
		} else {
			var x = CoC.player.cockThatFits( this.urtaCapacity() );
			var y = CoC.player.cockThatFits2( this.urtaCapacity() );
			if( x < 0 ) {
				x = 0;
			}
			MainView.outputText( ImageManager.showImage( 'urta-behindbar-male-fuckurta' ), false );
			MainView.outputText( 'You come out of the shadows, shedding your ' + CoC.player.armorName + '.  ', false );
			if( CoC.player.cor < 60 ) {
				MainView.outputText( 'You blush furiously at exposing yourself in such a way' );
			} else {
			...
			MainView.outputText( '.  Urta looks up at you in shock, blinking tears from her eyes and stammering, "<i>B-b-b-ut I have th-thish hideoush thing... Aren\'t you scared of it?</i>"\n\n', false );
			MainView.outputText( 'Gently wiping a tear from her muzzle, you answer, "<i>Not at all, but I think you\'d have more luck with something stuffed in your feminine half.</i>"\n\n', false );
			MainView.outputText( 'Urta gives you a shocked look, but nods and turns around, bending over an old bench and exposing her well-toned ass and glistening, black-lipped sex.  The invitation is all you need.  You wrap your arms around her, squeezing the downy-soft fur on her breasts and leveraging ', false );
			if( CoC.player.cockTotal() > 0 ) {
				if( CoC.player.cockTotal() > 2 ) {
					MainView.outputText( 'a pair of your ' + Descriptors.multiCockDescriptLight() + ' against her pussy and asshole.  ', false );
				} else if( CoC.player.cockTotal() === 2 ) {
					MainView.outputText( 'your ' + Descriptors.multiCockDescriptLight() + ' against her pussy and asshole.  ', false );
				} else if( CoC.player.cockTotal() === 1 ) {
					MainView.outputText( 'your ' + Descriptors.multiCockDescriptLight() + ' against her pussy.  ', false );
				}
			}
			//DOES IT FITZ;
			if( CoC.player.cockArea( x ) < this.urtaCapacity() ) {
				MainView.outputText( 'The wet entrance yields before you, accepting you like a velvet glove.  It grips you tightly, though the grip is meaningless with the flood of lubricant slipping over your shaft.  ', false );
				if( CoC.player.cockTotal() >= 2 ) {
					MainView.outputText( 'Her backdoor was rougher going – the tightly muscled entrance held you back for a moment until Urta relaxed completely, but now the tighter hole is nearly as easy to penetrate as her pussy.  ', false );
				}
				MainView.outputText( 'The fox is openly moaning and stroking her disproportional cock, dripping pre-cum everywhere as you do your level best to violate her ', false );
				if( CoC.player.cockTotal() === 1 ) {
					MainView.outputText( 'hole', false );
				} else {
					MainView.outputText( 'holes', false );
				}
				MainView.outputText( '.\n\n', false );
				MainView.outputText( 'She begins grinding against you, begging, "<i>Oh godsh yeeeessss don\'t shtop fucking me!  Give me more please!  PLEASE!</i>"\n\n', false );
				MainView.outputText( 'Nodding, you release one of her bouncing breasts in order to smack her back-side.  It barely jiggles, but the surprised moan that slips from the fox-herm\'s lips is all the encouragement you need.  You release the other breast and grab her hips and ass with both hands, slamming her back against you, only pausing to deliver another smack to one of her cheeks.  Urta wiggles happily, squealing after every slap, trying her best to steady her body while you brutally ', false );
				if( cocks === 1 ) {
					MainView.outputText( 'fuck', false );
				} else {
					MainView.outputText( 'double-penetrate', false );
				}
				MainView.outputText( ' her.\n\n', false );
				MainView.outputText( '"<i>Yesh oh yessssssSSSAHHHCUMMING,</i>" cries Urta as her body goes nerveless underneath you, held up entirely by your hands', false );
				if( cocks > 1 && y >= 0 ) {
					MainView.outputText( ', ' + Descriptors.cockDescript( x ) + ', and ' + Descriptors.cockDescript( y ) + '.', false );
				} else {
					MainView.outputText( ' and ' + Descriptors.cockDescript( x ) + '.', false );
				}
				MainView.outputText( '  Both her hands are locked tightly around her behemoth as it unloads onto the alley floor and walls.  The juicy tunnel clamps down on top of you, squeezing your ' + Descriptors.cockDescript( x ) + ' as if Urta\'s life depended on it.  It\'s as if every ounce of her being was focusing on clamping down on the invader, massaging, squeezing, and milking it, while the rest of her body goes entirely limp.', false );
				if( cocks > 1 && y >= 0 ) {
					MainView.outputText( '  Your ' + Descriptors.cockDescript( y ) + ' feels like it\'s about to explode inside her asshole when her sphincter clamps down tightly, acting like the world\'s tightest cock-ring.', false );
				}
				MainView.outputText( '\n\n', false );
				MainView.outputText( 'You fuck her savagely, abusing her spasming hole', false );
				if( cocks > 1 && y >= 0 ) {
					MainView.outputText( 's', false );
				}
				MainView.outputText( ' with every thrust until your own climax bursts over you like a wave.  Urta\'s drooling cunt ', false );
				if( cocks > 1 && y >= 0 ) {
					MainView.outputText( 'and tightly clenched ass seem', false );
				} else {
					MainView.outputText( 'seems', false );
				}
				MainView.outputText( ' to be trying to milk your body of every ounce of cum.  ', false );
				if( CoC.player.cumQ() < 250 ) {
					MainView.outputText( 'After a few passionate thrusts, you\'re spent, and you pull free, admiring the glaze of white that remains on her cunt-lips', false );
					if( cocks > 1 && y >= 0 ) {
						MainView.outputText( ' and slowly-closing pucker', false );
					}
					MainView.outputText( '.', false );
				} else if( CoC.player.cumQ() < 500 ) {
					MainView.outputText( 'After a few passionate thrusts, you can feel your cum packing Urta\'s womb ', false );
					if( cocks > 1 && y >= 0 ) {
						MainView.outputText( 'and gut ', false );
					}
					MainView.outputText( 'full of spooge, but all great things come to an end eventually.  You pull free, admiring the glaze of white that remains on her cunt-lips', false );
					if( cocks > 1 && y >= 0 ) {
						MainView.outputText( ' and slowly-closing pucker', false );
					}
					MainView.outputText( '.', false );
				} else {
					MainView.outputText( 'After a few passionate thrusts, Urta\'s womb ', false );
					if( cocks > 1 && y >= 0 ) {
						MainView.outputText( 'and belly are ', false );
					} else {
						MainView.outputText( 'is ', false );
					}
					MainView.outputText( 'so full of spooge that she looks positively pregnant, and you\'re expelled from her body by the sheer force of your orgasm.  You pause to admire your handiwork, watching whiteness drip from ', false );
					if( cocks > 1 && y >= 0 ) {
						MainView.outputText( 'both her holes.', false );
					} else {
						MainView.outputText( 'her abused hole.', false );
					}
				}
				...
				MainView.outputText( '.  Her tail curls up around the softening horse-cock, hiding it from view as she blows you a sloppy kiss and disappears inside.\n\n', false );
				MainView.outputText( 'You waste no time cleaning up and you stagger back inside after her, sated and happy with your new friend and her \'benefits\'.', false );
				this.knockUpUrtaChance();
			}
			//(TOO BIG);
			else {
				if( cocks === 1 ) {
					MainView.outputText( 'Urta\'s hole is just way too small to handle what you\'re packing, so you\'re forced to back off, much to her lament.  She moans, "<i>Noooo! Fuck me!</i>"\n\n', false );
					MainView.outputText( 'You pull her down into the grass and throw her on her back.  She looks confused and a little afraid, but that melts away when you spread your ' + CoC.player.legs() + ' across her and begin grinding your ' + Descriptors.multiCockDescriptLight() + ' against her sensitive dick.  Happy, the drunken fox begins pistoning against you, gathering her copious pre and smearing it over your ' + Descriptors.multiCockDescript() + '.  The two of you grind together, groin to groin, coating each other in a slippery mess.\n\n', false );
					MainView.outputText( 'Urta pants out, "<i>Ah ahhh ahhhh,</i>" and cums, her flare ballooning out until it\'s twice as wide as her shaft, a wave of white jism escaping from her horse-cock.  You gather some in your hand and slather it over your own ' + Descriptors.multiCockDescript() + ', and after a few moments of fevered masturbation, you start cumming with her.  Urta happily squirms against you, painting herself with jism and soaking her clothes with the stuff.  ', false );
					if( CoC.player.cumQ() < 250 ) {
						MainView.outputText( 'You join in, though your own contribution seems meager by comparison.', false );
					} else if( CoC.player.cumQ() < 500 ) {
						MainView.outputText( 'You join in, and manage to match her spurt for spurt, until she\'s lying in a thick puddle of the stuff.', false );
					} else {
						MainView.outputText( 'You join in, and make her orgasm look like a trickle compared to the flood of spunk you unleash onto her.  By the time the pair of you finish, she\'s soaked from head to toe, and lying in a nicely sized puddle of the stuff.', false );
					}
					...
					MainView.outputText( 'After a few moments she picks herself up and wipes herself up as much as she can with her sodden clothes.  Urta quickly gives up and removes a loose brick from the wall, revealing a hidden compartment with clothes and a few towels.  It seems she\'s quite prepared to make a mess in this alley.  She towels the worst of it off and gets dressed in a sheer tank-top and tight skirt before staggering back towards the bar.  Urta pauses and looks over her shoulder to blow you a sloppy kiss before she goes inside.\n\n', false );
					MainView.outputText( 'You waste no time cleaning up and mosey back inside after her, sated and happy with your new friend and her \'benefits\'.', false );
				}
				if( cocks > 1 ) {
					MainView.outputText( 'Urta\'s holes are just way too small to handle what you\'re packing, so you\'re forced to back off, much to her lament.  She moans, "<i>Noooo! Fuck me!</i>"\n\n', false );
					MainView.outputText( 'You pull her down into the grass and throw her on her back.  She looks confused and a little afraid, but that melts away when you spread your ' + CoC.player.legs() + ' across her and begin grinding each of your ' + Descriptors.multiCockDescriptLight() + ' against her sensitive dick.  Happy, the drunken fox begins pistoning against you, gathering her copious pre and smearing it over your ' + Descriptors.multiCockDescriptLight() + '.  The two of you grind together, groin to groin, coating each other in a slippery mess.\n\n', false );
					MainView.outputText( 'Urta pants out, "<i>Ah ahhh ahhhh,</i>" and cums, her flare ballooning out until it\'s twice as wide as her shaft, a wave of white jism escaping from her horse-cock.  You gather some in your hand and slather it over your own ' + Descriptors.multiCockDescriptLight() + ', and after a few moments of fevered masturbation, you start cumming with her.  Urta happily squirms against you, painting herself with jism and soaking her clothes with the stuff.  ', false );
					if( CoC.player.cumQ() < 250 ) {
						MainView.outputText( 'You join in, though your own contribution seems meager by comparison.', false );
					} else if( CoC.player.cumQ() < 500 ) {
						MainView.outputText( 'You join in, and manage to match her spurt for spurt, until she\'s lying in a thick puddle of the stuff.', false );
					...
					MainView.outputText( 'The drunken fox groans and continues spurting weakly, "<i>Oh godsh, letsh do this again sooon.  You don\'t know how much I needed thish!</i>"\n\n', false );
					MainView.outputText( 'After a few moments she picks herself up and wipes herself up as much as she can with her sodden clothes.  Urta quickly gives up and removes a loose brick from the wall, revealing a hidden compartment with clothes and a few towels.  It seems she\'s quite prepared to make a mess in this alley.  She towels the worst of it off and gets dressed in a sheer tank-top and tight skirt before staggering back towards the bar.  Urta pauses and looks over her shoulder to blow you a sloppy kiss before she goes inside.\n\n', false );
					MainView.outputText( 'You waste no time cleaning up and mosey back inside after her, sated and happy with your new friend and her \'benefits\'.', false );
				}
			}
	...
	//[Under Table BJ];
	Urta.prototype.blowUrtaUnderTable = function() {
		this.urtaSprite();
		CoC.player.slimeFeed();
		CoC.flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ]++;
		this.urtaLove( 1 );
		CoC.flags[ kFLAGS.URTA_TIME_SINCE_LAST_CAME ] = 2 + Utils.rand( 4 );
		MainView.outputText( '', true );
		MainView.outputText( ImageManager.showImage( 'urta-bar-bj' ), false );
		MainView.outputText( 'Urta\'s eyes widen in shock when you suggest taking her under the table.  She stammers, "<i>Uh, okay...</i>" and fills a glass with straight whiskey, fueling up on liquid courage while you crawl down.  You slip down under the table, pushing her legs apart', false );
		if( CoC.flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] >= 11 ) {
			MainView.outputText( ' and pulling down her fishnet to release the monster trapped inside it.  It snaps up immediately, bobbing before your face as it fills with Urta\'s booze-fueled lust.', false );
		} else {
			MainView.outputText( ' and pushing up her tight leather skirt, smiling at the sight of her trembling balls and juicy pussy.  Her tail unfurls around the equine member and it snaps up immediately, hanging before your face.', false );
		}
		MainView.outputText( '  It actually wiggles in time with the beating of her heart, bouncing up as it gets harder and harder, until it smacks your nose.\n\n', false );
		MainView.outputText( 'You hear the sound of her shot-glass slamming into the table with authority as she finishes her drink.  A bead of pre-cum bubbles out from Urta\'s urethra, signaling her readiness.  The horse-like penis gives off a salty scent, unique and not unpleasant.  Before you have a chance to change your mind, you lunge forward, opening your jaw as far as it will go and just barely taking her first few inches into your mouth.  The fox gasps out loud at the pleasure before she remembers to relax.  You can hear her pouring another drink, and in an effort to embarrass her, you begin sucking and licking around the edges of her flare, swallowing the surge of sweet-tasting pre-cum as it begins to fill your mouth.\n\n', false );
		MainView.outputText( 'Urta writhes in her seat, noisily spilling alcohol over the table.  You smile around the intruder in your mouth, gripping it with both hands and stroking it with tender caresses.  The gray fox slams down the bottle; though you can\'t tell if it\'s in irritation or due to difficulty controlling her muscles, and begins chugging down what she managed to get into her glass.  It seems Urta intends to drink until she doesn\'t care about any embarrassment you can force upon her.\n\n', false );
		MainView.outputText( 'Deciding you\'ve got to get her off before she can drink herself into a stupor, you ', false );
		if( CoC.player.cor < 70 ) {
			MainView.outputText( 'do your best to suppress your gag reflex and shove forwards, ramming her cock as deeply into your throat as you can.', false );
		} else {
			MainView.outputText( 'shove forwards, ramming her massive cock so far down your throat you can feel it dribbling pre-cum into your belly.', false );
		}

		MainView.outputText( '  Urta grunts, doing her best to muffle the noise as it escapes her lips, but you can feel her twitch and thicken inside you, beginning to drip with proof of her complete and total submission to your mouth.\n\n', false );
		MainView.outputText( 'You can hear chuckles from the bar\'s other occupants as they realize what\'s going on, and you blush, wishing there was a tablecloth to hide your activities from prying eyes.  The distraction does little to divert you from your chosen task, and you close your eyes, tasting your lover\'s throbbing shaft as it fills your mouth.  With both hands stroking, you bob up and down, completely centering your world around the pulsating horse-prick as it fills your mouth.\n\n', false );
		MainView.outputText( 'Without warning, Urta\'s legs wrap around your back, squeezing with her soft fur as she vices them closed.  You\'re forced to take her even more deeply, until the folds of her sheath are bumping your nose and chin and filling your nostrils with her heady musk.  You can\'t even breathe with such a massive intruder blocking your throat, but you do your best to get her off, knowing it\'s the quickest way to end it.  You suck hard and squeeze her balls, gurgling noisily around the sweaty shaft as you feel Urta give in.\n\n', false );
		MainView.outputText( 'A wave-like motion passes through your throat obstruction, stretching you wider than ever before.  It passes, leaving behind a feeling of warm fullness.  You feel another working its way into your mouth, and realize Urta is cumming, filling your belly with direct injections of horsey fox-seed. You desperately need to breathe, but you\'re trapped on your lover\'s maleness, and with nothing else to do, you give her twitching balls a squeeze, feeling her body twitch in response as she dumps a particularly heavy batch of spunk into you.  Happy that you could at least control how hard she came, you begin blacking out, barely noticing as Urta\'s orgasmic contractions die off.\n\n', false );
		MainView.outputText( 'Thumping down on your back, you\'re blissfully aware of the sweet air filling your lungs.  In a moment of reflection, you realize you can taste the salty flavor of Urta\'s seed on your lips and mouth, and the retracting horse-cock is ', false );
		if( CoC.player.biggestTitSize() >= 2 ) {
			MainView.outputText( 'dragging between your ' + Descriptors.allBreastsDescript(), false );
		} else {
			MainView.outputText( 'dragging across your chest', false );
		}
		MainView.outputText( ' as it begins to soften.  You wipe as much of her leavings from your ' + CoC.player.face() + ' as you can, licking the slippery glaze from your lips.  Before you get up, you make sure to give Urta\'s member a firm squeeze, punishment for forcing you to take her so deep.\n\n', false );
		MainView.outputText( 'Once you\'ve climbed out from under the table, you\'re VERY aware of the eyes of some of the nearby bar patrons on your back.  It seems your undercover act managed to draw more than a little attention.  Urta\'s cheeks burn bright-red under her gray fur, even though her eyes are a bit glassy and unfocused from the amount of alcohol she\'s ingested.  Sure that she\'ll remember the embarrassment, you give her a deep kiss, making her taste her residue on your lips.', false );
	...
	Urta.prototype.takeUrtaInTheButtPublically = function() {
		...
		if( CoC.player.cor < 30 && CoC.flags[ kFLAGS.PC_FETISH ] === 0 ) {
			MainView.outputText( 'No way!  You\'re not going to do that in front of EVERYONE.', false );
			EngineCore.doNext( SceneLib.telAdre, SceneLib.telAdre.barTelAdre );
			return;
		}
		CoC.player.slimeFeed();
		...
		this.urtaLove( 2 );
		CoC.flags[ kFLAGS.URTA_TIME_SINCE_LAST_CAME ] = 5;
		MainView.outputText( ImageManager.showImage( 'urta-public-fuck' ), false );
		if( CoC.player.cor < 60 && CoC.flags[ kFLAGS.PC_FETISH ] === 0 ) {
			MainView.outputText( 'You can\'t believe you\'re doing it, but you undo the lower half of your ' + CoC.player.armorName + '.  ', false );
		} else {
			MainView.outputText( 'You happily remove the lower half of your ' + CoC.player.armorName + ', more than a little excited at the prospect of putting on a show for the crowd.  ', false );
		}
		MainView.outputText( 'Urta casually flips up her skirt and ', false );
		if( CoC.flags[ kFLAGS.TIMES_FUCKED_URTA ] < 11 ) {
			MainView.outputText( 'unwinds her tail from around the beast', false );
		} else {
			MainView.outputText( 'fishes the beast out of her fishnets', false );
		}
		MainView.outputText( '.  Watching in horror, your eyes never leave the equine pillar as it grows to its full size.  Amazingly, none of the bar\'s patrons seem to notice the trembling horse-cock, even though it\'s beginning to leak with pre-cum.\n\n', false );
		MainView.outputText( 'You carefully shift your position, climbing in front of Urta.  With care, you grab the sensitive member just below the flare and begin rubbing it over your ' + Descriptors.buttDescript() + ' and ready ass-hole, lubing yourself up and trying to relax.  The impatient fox grabs you by your ' + Descriptors.hipDescript() + ' and pulls you down, forcing herself against your pucker.  ', false );
		//[GO TOO SMALL OR FITS, NO NEW PG];
		//[FITS];
		if( CoC.player.analCapacity() >= 60 ) {
			MainView.outputText( 'It\'s a tight fit, even with a backdoor as gaping as yours.  You\'re thankful for your looseness as you slide down into her lap, and Urta signals her appreciation of your capacity with a throaty sigh, "<i>Mmmm, you\'re good at this.</i>"\n\n', false );
		}
		//[DOESN'T FIT];
		else {
			tooBig = true;
			MainView.outputText( 'It\'s far too big for you, but that doesn\'t stop Urta from pushing it inside you.  You can feel yourself stretched around the invader, and it hurts enough that you almost scream out in pain, but inexorably, you\'re drawn down until your ' + Descriptors.buttDescript() + ' is resting on Urta\'s thighs.  Her thick prick keeps you stretched uncomfortably wide, and you can feel every twitch it makes.', false );
			//(STRETCH TEXT HERE);
			if( !CoC.player.buttChange( 60, true ) ) {
				MainView.outputText( '  ', false );
			}
			MainView.outputText( 'Surprisingly, the over-endowed fox notices your discomfort and hands you a narrow bottle from the table, "<i>Drink up, it\'ll help with the pain until you get used to me.</i>"\n\n', false );
			MainView.outputText( 'You look at the bottle suspiciously, reading the faded yellow label, "<i>Barkardi 151</i>".  Wincing from the pain, you suppose it couldn\'t hurt and you tip back the bottle, taking a hearty swig.  It burns like hellfire, and you nearly retch up the massive swallow before you get it down.  Amazingly, the pain in your throat and the bonfire in your belly nearly drowns out what\'s going on in your ' + Descriptors.assholeDescript() + '.  You actually start to relax, and Urta nibbles on your ear in between sips of ale.  She holds her mug up to your lips and invites you to share her drink and you do, grateful for any additional pain-killers.\n\n', false );
		}
		//[BOTH];
		if( CoC.player.totalCocks() > 0 ) {
			MainView.outputText( 'You start to grow hard from the pressure inside you, and there\'s nothing you can do to stop ', false );
			if( CoC.player.totalCocks() > 1 ) {
				MainView.outputText( 'each of ', false );
			}
			MainView.outputText( 'your ' + Descriptors.multiCockDescriptLight() + ' from swelling to full erectness.  A bead of pre forms at ', false );
			if( CoC.player.totalCocks() > 1 ) {
				MainView.outputText( 'each ', false );
			} else {
				MainView.outputText( 'the ', false );
			}
			MainView.outputText( 'tip, and begins leaking down the shaft', false );
			if( CoC.player.totalCocks() > 1 ) {
				MainView.outputText( 's', false );
			}
			MainView.outputText( ' of your ' + Descriptors.multiCockDescriptLight() + ', squeezed out from your prostate.  ', false );
		}
		if( CoC.player.hasVagina() ) {
			MainView.outputText( 'Heat spreads through your loins as your ' + Descriptors.vaginaDescript( 0 ) + ' engorges with blood, allowing your ' + Descriptors.clitDescript() + ' to ', false );
			if( CoC.player.clitLength < 2 ) {
				MainView.outputText( 'peep out', false );
			} else {
				MainView.outputText( 'grow to its full size', false );
			}
			MainView.outputText( '.  ', false );
			if( CoC.player.vaginas[ 0 ].vaginalWetness < AppearanceDefs.VAGINA_WETNESS_SLICK ) {
				MainView.outputText( 'Moisture beads on your lips, signaling just how turned on this total violation is making you.', false );
			} else if( CoC.player.vaginas[ 0 ].vaginalWetness < AppearanceDefs.VAGINA_WETNESS_SLAVERING ) {
				MainView.outputText( 'Moisture begins dripping onto Urta\'s thighs, signaling just how turned on this total violation is making you.', false );
			} else {
				MainView.outputText( 'Moisture puddles on the seat under Urta as your juices flood her thighs, revealing just how turned on this violation is making you.', false );
			}
			MainView.outputText( '  ', false );
		}
		//(NIPPLES:;
		MainView.outputText( 'Your ' + Descriptors.nippleDescript( 0 ) + 's tent up under your ' + CoC.player.armorName, false );
		if( CoC.player.hasFuckableNipples() && CoC.player.biggestLactation() >= 1 ) {
			MainView.outputText( ', leaking milky lubricants and aching to be penetrated.', false );
		} else if( CoC.player.hasFuckableNipples() ) {
			MainView.outputText( ', aching to be penetrated.', false );
		} else if( CoC.player.biggestLactation() >= 1.5 && CoC.player.biggestLactation() < 2.5 ) {
			MainView.outputText( ', leaking thin streams of milk that rapidly become visible to everyone around you.', false );
		} else if( CoC.player.biggestLactation() >= 2.5 ) {
			MainView.outputText( ', spurting thick streams of milk that soak your ' + CoC.player.armorName + ' and are visible to everyone in the bar.', false );
		} else {
			MainView.outputText( '.', false );
		}
		MainView.outputText( '\n\n', false );
		MainView.outputText( 'Soft-furred hands grip your ' + Descriptors.hipDescript() + ' firmly and begin to lift you up, making you gasp as her flare is dragged through your hind-end, eliciting what Urta must think are cute gasps.  You hear a \'murrrr\' of contentment in your ear and Urta releases you, twitching inside you as your weight slowly carries you back down into her lap.  Once your ' + Descriptors.buttDescript() + ' smacks into her thighs she grabs you again, dragging you back up her massive pole until you\'re looking down on the tables around you.  She stops and withdraws her support, guiding you back down her shaft and moaning when you take her ring of prepuce half-way down.\n\n', false );
		MainView.outputText( 'The eyes of everyone in the place are locked onto you, and ', false );
		if( tooBig ) {
			MainView.outputText( 'you find yourself giggling drunkenly, barely feeling any pain and arching your back like a slut for the crowd.', false );
		} else if( CoC.player.cor < 50 && CoC.flags[ kFLAGS.PC_FETISH ] === 0 ) {
			MainView.outputText( 'you find yourself whimpering, regretting the decision to climb on top of her, but aroused in spite of it.', false );
		} else {
			MainView.outputText( 'you find yourself sliding your hands under your ' + CoC.player.armorName + ' and pinching your ' + Descriptors.nippleDescript( 0 ) + 's, putting on a show for the crowd.', false );
		}

		MainView.outputText( '  Urta starts breathing hard into your ear, and you start flexing your ' + CoC.player.legs() + ' to help her lift you up.  Grateful for the effort, the fox-girl increases the pace by helping to pull you back down each time.  By now it\'s starting to feel pretty good, and her pre-cum is making your ' + Descriptors.assholeDescript() + ' so wet and slippery that you can slide quite quickly along the column of horse-meat.\n\n', false );
		MainView.outputText( 'Wet squelches echo through the bar as you\'re slammed into Urta\'s lap harder and harder.', false );
		if( CoC.player.biggestTitSize() >= 1 ) {
			MainView.outputText( '  Your ' + Descriptors.allBreastsDescript() + ' are jiggling and bouncing wildly, ', false );
			if( CoC.player.hasFuckableNipples() ) {
				MainView.outputText( 'dripping lubricants', false );
			} else if( CoC.player.biggestLactation() > 1 ) {
				MainView.outputText( 'spurting milk', false );
			} else {
				MainView.outputText( 'rubbing your tight nipples against your ' + CoC.player.armorName, false );
			}
			MainView.outputText( '.', false );
		}
		if( CoC.player.cockTotal() > 0 ) {
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( '  Steady drizzles of cum begin dripping from each of your ' + Descriptors.multiCockDescriptLight() + ' as she pounds away, rubbing hard against your prostate.', false );
			} else {
				MainView.outputText( '  Steady drizzles of cum begin dripping from your ' + Descriptors.multiCockDescriptLight() + ' as she pounds away, rubbing hard against your prostate.', false );
			}
		}
		MainView.outputText( '  You can\'t hold back, and start moaning quietly, trying your best to be silent and failing horribly.  For her part, your foxy lover seems equally incapable of suppressing her orgasm, and you feel her thick organ flexing inside you as it prepares to cum.\n\n', false );
		MainView.outputText( 'Her paws pull you down hard, crushing your ' + Descriptors.buttDescript() + ' against her as her sheath presses into your ' + Descriptors.assholeDescript() + '.  The invading flesh ripples from the base towards the tip, culminating in a flood of warmth that makes your belly gurgle, and at that moment you cum.  Your sphincter clamps down tightly around Urta\'s marvelous member, unintentionally providing the fox with an even better orgasm.  The next pulse of cum feels even larger, and you look down in shock when you realize you can see the outline of her shaft on the ' + CoC.player.skinDesc + ' of your belly.  As she cums you can see it getting less distinct as your body becomes more and more rounded.  A few blasts later you look positively pudgy, but you can feel her softening inside you.\n\n', false );
		MainView.outputText( 'Urta sighs and takes a sip of her drink, giving you a quick peck on the side of your neck.  You start to rise, but the drunken hermaphrodite grabs you and holds you down, "<i>Let\'s just stay like this a little longer...</i>"\n\n', false );
		MainView.outputText( 'You shrug and relax, cuddling with her while her massive load is plugged inside you.  She keeps you there for the better part of an hour, kissing you, nipping at your neck, and sharing her drinks with you.  When you finally do get a chance to leave, you\'re stumbling slightly from the alcohol and your asshole gapes and drips whiteness behind you.  You don\'t notice the trail of cum until you get back to your camp, and it brings a blush to your cheeks.', false );
		CoC.player.orgasm();
		EngineCore.dynStats( 'sen', -2 );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	...
	Urta.prototype.getAPublicFacialFromUrta = function() {
		...
		CoC.flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ]++;
		this.urtaLove( 2 );
		CoC.flags[ kFLAGS.URTA_TIME_SINCE_LAST_CAME ] = 3 + Utils.rand( 3 );
		MainView.outputText( '', true );
		MainView.outputText( ImageManager.showImage( 'urta-public-facial' ), false );
		MainView.outputText( 'Given the choices, it seems like just accepting the facial would be the best option.  Urta nods, grinning lasciviously as she pulls the table to the side, and gestures for you to get on the floor.  You kneel before her as she pulls back her ', false );
		if( CoC.flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] < 11 ) {
			MainView.outputText( 'skirt ', false );
		} else {
			MainView.outputText( 'stockings ', false );
		}
		MainView.outputText( 'and allows her horse-dick to flop free in front of you.  It pulses, growing bigger with each beat of the fox\'s heart as she watches you submit yourself to her arousal.  You feel the eyes of everyone around you as they watch the spectacle ', false );
		if( CoC.player.cor < 33 ) {
			MainView.outputText( 'and it makes you blush crimson', false );
		} else if( CoC.player.cor < 66 ) {
			MainView.outputText( 'and it makes you uncomfortable but horny', false );
		} else {
			MainView.outputText( 'and it makes you randy as hell', false );
		}
		MainView.outputText( '.\n\n', false );
		MainView.outputText( 'Two dexterous fur-covered hands wrap around her mottled shaft and begin pumping it.  Almost immediately the flattened-tip begins flaring wide and produces a dollop of fragrant equine pre-cum.  Urta\'s balls quiver happily as she plays with herself, and as you look up at her and open wide, ', false );
		if( CoC.player.cor < 75 ) {
			MainView.outputText( 'doing your best to play your part in the scene.', false );
		} else {
			MainView.outputText( 'hoping to encourage her to absolutely soak you with cum.', false );
		}
		MainView.outputText( '\n\nUrta looks down with a surprisingly shy expression and asks, "<i>Could you... lick the tip please?  For me?</i>"\n\n', false );
		if( CoC.player.cor < 50 ) {
			MainView.outputText( 'Hesitantly, you extend your tongue and lick the underside of her flat-tip.  ', false );
		} else {
			MainView.outputText( 'Eagerly, you lick around the outside edge of her flared tip, sticking your tongue out as far as you can to lick the underside once you finish.  ', false );
		}
		MainView.outputText( 'Your efforts are rewarded when Urta groans and spurts a dollop of pre-cum onto your tongue, and you can see her urethra twitching and pulsating as more of the clear fluid begins to leak out.  It drips down onto your tongue as the fox picks up her pace, feverishly masturbating her bloated shaft.  She says, "<i>Swallow that pre and get ready slut, you\'ll need an empty mouth to hold all this.</i>"\n\n', false );
		MainView.outputText( 'You swallow, tasting the sweet pre-cum as it slides down your throat.  Risking a glance to the side, you realize there\'s a crowd gathering around you, and more than a few dog-dicks and horse-cocks are being stroked.  ', false );
		if( CoC.player.cor > 50 ) {
			MainView.outputText( 'You hope the show will be arousing enough to get them off on you as well, and with renewed determination you turn back to Urta and give her balls a squeeze.', false );
		} else {
			MainView.outputText( 'You shudder in shame, and hope that the crowd will disperse. It\'s humiliating enough waiting to have Urta soak you with cum.', false );
		}
		MainView.outputText( '  She trembles and her tip flares wide, and you know she\'s starting to cum.\n\n', false );
		MainView.outputText( 'Her urethra dilates before your eyes and time seems to slow down as the first blob of horse-semen is launched towards you.  Blinking at the last second, you feel it splatter over your ' + CoC.player.face() + '.  You open your mouth wider, keeping your eyes closed but knowing you\'re expected to swallow some of it.  Urta\'s aim improves with the next blast, and it does land in your mouth.  You nearly gag from the sudden salty intrusion, and due to the volume of the seed a fair portion of it leaks out the sides of your mouth as you keep it open, expecting more.  You feel more cum land in your hair, then on your cheek, and finally on your neck and ', false );
		if( CoC.player.biggestTitSize() < 1 ) {
			MainView.outputText( 'chest.  ', false );
		} else {
			MainView.outputText( 'cleavage.  ', false );
		}
		MainView.outputText( 'Urta paints you with the stuff, until you\'re practically oozing cum.  It does eventually stop, and you wipe the jism from your eyes', false );
		if( CoC.player.cor > 60 ) {
			MainView.outputText( ', being sure to lick your fingers clean afterwards', false );
		}
		MainView.outputText( '.\n\n', false );
		if( CoC.player.cor > 50 ) {
			MainView.outputText( 'You could probably get the crowd to cover the rest of you.  Do you?', false );
			EngineCore.doYesNo( this, this.optionalUrtaBukkake, this, this.declineUrtaBukkake );
			return;
		} else {
			MainView.outputText( 'You blush and try to clean yourself up.  An embarrassed barmaid forces her way through the crowd to give you a towel, and starts shooing away the assembled masturbators.  You thank her and towel off Urta\'s thick seed, being more than a little aroused by the scene you caused.  The fox-girl is leaning back looking VERY satisfied as she ', false );
			if( CoC.flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] >= 11 ) {
				MainView.outputText( 'stuffs herself back into her fishnet', false );
			} else {
				MainView.outputText( 'curls her tail back around her member, concealing it from view', false );
			}
			MainView.outputText( '.', false );
	...
	Urta.prototype.optionalUrtaBukkake = function() {
		this.urtaSprite();
		CoC.player.slimeFeed();
		CoC.flags[ kFLAGS.URTA_TIME_SINCE_LAST_CAME ] -= 2;
		this.urtaLove( -1 );
		MainView.outputText( '', true );
		MainView.outputText( ImageManager.showImage( 'urta-bukkake' ), false );
		MainView.outputText( 'You pause to tear off your ' + CoC.player.armorName + ' and expose the rest of your body, and you reach out into the crowd, grabbing the nearest cock and caressing it as you beg, "<i>Come on and cum, she missed most of me!</i>"\n\n', false );
		MainView.outputText( 'Urta leans back with a smile, toying with her softening member as a ring of horny studs and herms encircles you, jerking themselves off furiously.  You take turns touching and caressing them, murmuring enticing words as they start cumming on you, one or two at a time.  As those that finish step away, they\'re replaced by newcomers, turned on by the cum-hungry slut in front of them.  You do your best to greet them with a wink, a stroke, and a slurp, and then you present your ', false );
		if( CoC.player.biggestTitSize() >= 1 ) {
			MainView.outputText( Descriptors.allBreastsDescript(), false );
		} else {
			MainView.outputText( Descriptors.nippleDescript( 0 ) + 's', false );
		}
		MainView.outputText( ' to a horny wolf just in time to take his hot load on your chest.\n\n', false );
		EngineCore.dynStats( 'lus', 15 + Utils.rand( 5 ) + CoC.player.lib / 10 );
		MainView.outputText( 'An hour later you\'re totally drenched in cum.  It\'s dripping off you from your hair down to your ' + CoC.player.feet() + ', and you love it.  You lick up the mixed spooge until you start to feel full, and a blushing barmaid comes forward to hand you a towel.  You give her a salty kiss on the lips, sending her running away before you towel off and dress yourself.', false );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	...
	Urta.prototype.goBackToUrtasForLuvinz = function() {
		this.urtaSprite();
		MainView.outputText( '', true );
		MainView.outputText( ImageManager.showImage( 'urta-home-image' ), false );
		if( CoC.flags[ kFLAGS.URTA_PC_LOVE_COUNTER ] === 1 ) {
			MainView.outputText( 'Urta gives you a warm smile and grabs you by the hand, pulling you out the Wet Bitch\'s backdoor and into Tel\'Adre\'s back-alleys.  Her dress does little to conceal the bulging mass underneath or the growing wet spot that forms at the tip of her impressive tent.  The long, body-hugging garment makes it difficult for Urta to walk in her current state, and you don\'t make it any easier for her, constantly teasing the fox as you navigate the back-alleys of Tel\'Adre.\n\n', false );
			MainView.outputText( 'Every chance you get, you brush a hand or a hip against her sensitive nether-bulge or lust-tightened nipples.  For someone with such a bad-ass reputation, Urta\'s quite easy to tease, and she makes adorable gasps and shuddering shakes every time you brush against her flare or surprise her with a tender caress.  She lets out cute whimpers as she shuffles onward, but she doesn\'t protest as her \'male\' half drenches her dress with more and more slimy pre-cum.\n\n', false );
			MainView.outputText( 'The door to her apartment looms before you and you push her inside.  You do your best to stifle an involuntary snicker as you close the door and see a trail of Urta\'s moist arousal on the street right up to the door.  The apartment is impeccably clean, though you see a few large sex-toys half-hidden about the place.  For someone as ashamed of her desires as Urta was, she sure is terrible at hiding her kinks.' );
			if( CoC.flags[ kFLAGS.URTA_PETPLAY_DONE ] === 0 && this.urtaLove() ) {
				MainView.outputText( '  <b>A new, shiny leather collar lies half-concealed under a cushion, as though its owner haphazardly tried to bury it in a rush.</b>' );
			}
			MainView.outputText( '  The busty fox has already retreated from the front-room, the half-soaked dress on the floor the only indication she was there with you a moment ago.\n\n', false );
			MainView.outputText( 'You follow a trail of pre-cum on the floor and the scent of copious female arousal deeper into the apartment, around a corner, and into Urta\'s relatively small bedroom.  Your vulpine lover is reclined on her bed, wearing nothing but lacy stockings and a latex cock-sheath.  ', false );
			if( CoC.player.totalCocks() > 0 ) {
				MainView.outputText( 'In her hand is another sheath, ready to gird your own equipment in a fashion similar to her own.  ', false );
			}
			EngineCore.doNext( this, this.urtaHomeLuvLuvinsMenu );
			return;
		}
		MainView.outputText( 'Urta smiles broadly and gives you a grateful peck on the cheek.  She makes like she\'s going to rise, then stirs uncomfortably, her eyes flashing down towards her crotch.  The fox settles back down and asks, "<i>Would you mind paying off my tab while I slip out of here?  You\'ve gotten me a bit too exposed to stand around trying to pay,</i>" as she fishes out a coin-purse.  Tossing a few gems and odd coins your way, your over-endowed lover gives you a longer kiss before pushing you away from the table.\n\n', false );
		...
		MainView.outputText( 'You assure her that you aren\'t offended', false );
		if( CoC.player.lib > 50 || CoC.player.cor > 50 ) {
			MainView.outputText( ' and are actually turned on by the thought of walking through the city with such a \'big\' lover hanging off your arm', false );
		}
		MainView.outputText( '.  Urta smiles weakly and hooks her arm through yours, leading you towards her apartment, making sure to stick to the more abandoned back-roads.  Her caution doesn\'t stop the pair of you from giving a few free shows, and if anything, each brief encounter with a passerby seems to be making her bigger and harder, revealing a half-buried exhibitionist streak.\n\n', false );
		this.goBackToUrtasForLuvinzII();
	};
	Urta.prototype.goBackToUrtasForLuvinzII = function() {
		MainView.outputText( 'The journey is blessedly brief, and within minutes you arrive at Urta\'s house.  She fiddles with the lock anxiously for a moment before finally getting a -CLICK- and unlocking it.  You sweep inside behind your host and look around, mildly shocked at the spartan surroundings.  Urta\'s apartment is tiny, cramped, and nearly devoid of possessions beyond weapons, armor, and sex-toys.', false );
		if( CoC.flags[ kFLAGS.PC_SEEN_URTA_SEX_TOYS ] === 0 ) {
			MainView.outputText( '  The last group is the most numerous of those three.', false );
			if( CoC.giacomo > 0 ) {
				MainView.outputText( '  You recognize some of the toys as Giacomo\'s wares, though there are a fair number that seem to be custom made, and others that are clearly living creatures.', false );
			}
			MainView.outputText( '  Looking over the things she had to use before she met you, it dawns on you just how lonely the poor fox has been.', false );
		} else {
			MainView.outputText( '  While the last group used to be the most numerous, it looks like Urta has cleaned house and only has a few of the toys left.  It seems that since meeting you she hasn\'t had to turn to her massive device collection like she used to.  It dawns on you just what a difference you\'ve made in Urta\'s life, and it brings a loving smile to your face.', false );
		}
		MainView.outputText( '\n\n', false );
		MainView.outputText( '"<i>Sexy, are you going to come get some?</i>" Urta calls out from the bedroom in the back.  You slap your forehead, angry for getting so distracted while a hot piece of ass is on the line.  Heading deeper into the apartment after her, you turn the corner to her bedroom and are floored by the sight.  Urta is totally nude save for a pair of black fishnet stockings and some kind of shining latex cock-sheath.  She\'s on all fours on top of a fur-lined bed, and wiggling her hind-end at you with her tail lifted in the air.  The scent of need pours off from her sex, and you find yourself as ', false );
		if( CoC.player.hasVagina() && CoC.player.totalCocks() > 0 ) {
			MainView.outputText( 'hard and wet as you\'ve ever been', false );
		} else if( CoC.player.totalCocks() > 0 ) {
			MainView.outputText( 'hard as you\'ve ever been', false );
		} else if( CoC.player.hasVagina() ) {
			MainView.outputText( 'wet as you\'ve ever been', false );
		} else {
			MainView.outputText( 'ready as you\'ve ever been to take such a monster', false );
		}
		MainView.outputText( '.\n\n', false );
		MainView.outputText( 'Urta is clearly looking to take a more passive role, and she begins stroking herself as she asks, "<i>How do you want to fuck, lover?</i>"', false );
		//[M: VAGINAL] [F: RIDE] [GENDERLESS/M: RIDE ANAL];
		var vaginal = null;
		var femaleRides = null;
		if( CoC.player.totalCocks() > 0 ) {
	...
	Urta.prototype.rideUrtaTenderFemale = function() {
		this.urtaSprite();
		CoC.flags[ kFLAGS.PC_SEEN_URTA_SEX_TOYS ]++;
		this.urtaLove( 1.5 );
		CoC.flags[ kFLAGS.TIMES_FUCKED_URTA ]++;
		CoC.flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ]++;
		CoC.flags[ kFLAGS.URTA_TIME_SINCE_LAST_CAME ] = 3 + Utils.rand( 2 );
		MainView.outputText( '', true );
		MainView.outputText( ImageManager.showImage( 'urta-home-female-ride' ), false );
		MainView.outputText( 'You climb into bed and playfully roll Urta over.  Her tits and horse-cock flop and jiggle from the change in position, and you watch mesmerized for a moment until Urta giggles girlishly, drawing you out of your reverie.\n\n', false );
		MainView.outputText( '"<i>Do you like them that much?</i>" she asks.\n\n', false );
		MainView.outputText( 'You nod ', false );
		if( CoC.player.cor < 50 || CoC.player.lib < 50 ) {
			MainView.outputText( 'sheepishly ', false );
		} else {
			MainView.outputText( 'eagerly ', false );
		}
		MainView.outputText( 'and grab ahold of her latex-wrapped member, giving it gentle strokes along the underside.  Urta moans loudly and twitches her hips, already eager for more, and the ', false );
		if( CoC.player.vaginas[ 0 ].vaginalWetness < AppearanceDefs.VAGINA_WETNESS_WET ) {
			MainView.outputText( 'growing moistness in your loins', false );
		} else if( CoC.player.vaginas[ 0 ].vaginalWetness < AppearanceDefs.VAGINA_WETNESS_DROOLING ) {
			MainView.outputText( 'sopping wetness in your loins', false );
		} else {
			MainView.outputText( 'flood of moisture leaking from your crotch', false );
		}
		MainView.outputText( ' signals your readiness to take her.', false );
		if( CoC.player.totalCocks() > 0 ) {
			if( CoC.player.totalCocks() === 1 ) {
				MainView.outputText( '  Your ' + Descriptors.multiCockDescriptLight() + ' is raging hard and trembling, but ignored for now.', false );
			} else {
				MainView.outputText( '  Each of your ' + Descriptors.multiCockDescriptLight() + ' is raging hard and trembling, but ignored for now.', false );
			}
		}
		MainView.outputText( '  Climbing atop the bed with her, you swivel your ' + Descriptors.hipDescript() + ' about, positioning her at your entrance and straddling your ' + CoC.player.legs() + ' around her.\n\n', false );
		MainView.outputText( 'You can see a bubble already forming at the tip of her hardness, signaling the unbridled lust your form has brought to your lover and you lower yourself with a sigh, feeling it spread your lips apart with delicious slowness as her flared tip begins penetrating you.  The horse-cock is so big and thick that it ', false );
		if( CoC.player.vaginalCapacity() < 60 ) {
			MainView.outputText( 'hurts quite a bit as it pushes further and further inside.  ', false );
			CoC.player.cuntChange( 60, true );
		} else {
			MainView.outputText( 'stretches you nice and wide, rubbing you tightly even though you\'re quite stretched already.  ', false );
			CoC.player.cuntChange( 60, true );
		}
		MainView.outputText( 'You gasp and sigh happily, sinking downwards slowly.\n\n', false );
		MainView.outputText( 'The ring of prepuce passes past your entrance ', false );
		if( CoC.player.vaginalCapacity() < 60 ) {
			MainView.outputText( 'and you bottom out there, unable to take in any more of her equine shaft.', false );
		} else {
			MainView.outputText( 'and you keep going, sliding down along the marvelous member until your cunt-lips are rubbing her sheath against your [clit].', false );
		}
		MainView.outputText( '\n\n', false );
		MainView.outputText( 'Urta growls and grabs you by the back of the neck, pulling your ' + CoC.player.face() + ' down for a sloppy kiss.  Her smooth canine-like tongue forces its way into your mouth and curls around yours as the two of you swap spit.  Your hips, spurred on by the passion, begin to rise and fall to the tempo of the kiss, and in seconds you\'re both gasping and moaning against each other\'s lips, lost in the throes of passion.  Deep inside you, you can feel the condom filling with Urta\'s incredibly copious pre-cum, but every time your hips drop down the pressure squeezes some of it down along her length until it escapes around the condom\'s edge, dripping into her sheath.\n\n', false );
		//(Big Tits:;
		if( CoC.player.biggestTitSize() >= 5 ) {
			MainView.outputText( 'Your ' + Descriptors.breastDescript( 0 ) + ' mash into the fox\'s own, your ' + Descriptors.nippleDescript( 0 ) + 's ', false );
			if( !CoC.player.hasFuckableNipples() ) {
				MainView.outputText( 'rubbing over her black-skinned buds.', false );
			} else {
				MainView.outputText( 'parting as her prominent black nipples slip inside your own.', false );
			}
			MainView.outputText( '  You squeal at the sudden pleasure, arching your back and furthering the sensation from your over-burdened chest.', false );
			if( CoC.player.totalCocks() > 0 ) {
				if( CoC.player.cocks[ 0 ].cockLength > 15 ) {
					if( CoC.player.totalCocks() === 1 ) {
						MainView.outputText( '  Your ' + Descriptors.multiCockDescriptLight() + ' pushes up between the jiggling breasts, slicking them with your pre-cum as it begins to get tit-fucked by both of your trembling forms.', false );
					} else {
						MainView.outputText( '  Each of your ' + Descriptors.multiCockDescriptLight() + ' pushes up between the jiggling breasts, slicking them with your pre-cum as they begin to get tit-fucked by both of your trembling forms.', false );
					}
				}
			}
			MainView.outputText( '\n\n', false );
		}
		MainView.outputText( 'You both cry out in orgiastic pleasure as your shaking ' + Descriptors.hipDescript() + ' force your tightly-stretched cunt over Urta\'s rubber-wrapped horse-prick again and again.  Your internal muscles spasm, clamping down tightly in sudden orgasm.  Urta\'s voice trembles in a low moan of satisfaction while her lower body twitches upwards with hard pumping motions.  Her dick trembles inside you, and you feel the latex around her girthy prick inflating with her seed and pressing against your cervix.\n\n', false );
		if( CoC.player.totalCocks() > 0 ) {
			if( CoC.player.cockTotal() === 1 ) {
				MainView.outputText( 'Your own ' + Descriptors.multiCockDescriptLight() + ' cums as well, blasting seed over both your ', false );
				if( CoC.player.cocks[ 0 ].cockLength > 15 && CoC.player.biggestTitSize() >= 5 ) {
					MainView.outputText( 'jiggling tits', false );
				} else {
					MainView.outputText( 'bellies', false );
				}
				MainView.outputText( ', glazing them in slippery goo.', false );
			} else {
				MainView.outputText( 'Each of your own ' + Descriptors.multiCockDescriptLight() + ' cums as well, blasting seed over both your ', false );
				if( CoC.player.cocks[ 0 ].cockLength > 15 && CoC.player.biggestTitSize() >= 5 ) {
					MainView.outputText( 'jiggling tits', false );
				} else {
					MainView.outputText( 'bellies', false );
		...
		MainView.outputText( 'Lying down on top of her, the two of you cuddle while joined at the hips, snuggling happily', false );
		if( CoC.player.totalCocks() > 0 ) {
			MainView.outputText( ' and ignoring the hot, wet mess your ' + Descriptors.multiCockDescriptLight() + ' ', false );
			if( CoC.player.totalCocks() === 1 ) {
				MainView.outputText( 'has', false );
			} else {
				MainView.outputText( 'have', false );
			}
			MainView.outputText( ' left between your nude bodies', false );
		}
		MainView.outputText( '.  The fox-girl gives you a long, happy kiss and holds you tightly in her arms before she lets you go and begins to pull out.  The loud -SCHLLLIIIICK- of her fat member being dragged through your nethers makes you blush, and the feeling of it robs you of any strength below the waist.  The latex stretches, the cum-bubble still trapped inside you and stretching the condom obscenely for a moment, until it jerks out with a sudden \'pop\'.  The relief is palpable, but your ' + Descriptors.vaginaDescript( 0 ) + ' hangs open, with lips parted loosely after today\'s incredible violation.\n\n', false );
		MainView.outputText( 'Urta sighs and languidly pulls off her condom, nearly getting cum everywhere before she ties it off.  It looks like it\'ll pop at any moment – there\'s probably over a liter of cum stored in it.  She hefts it and mocks like she\'s going to throw it at you, but stops and giggles before setting it down beside the bed.  Your foxy lover says, ', false );
		if( CoC.flags[ kFLAGS.TIMES_FUCKED_URTA ] <= 10 || CoC.flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] < 11 ) {
			MainView.outputText( '"<i>Lovemaking is so much of a hassle to clean up if I don\'t use one of these!  At least I don\'t have to use them for the same reason as most of Tel\'Adre.  It turns out I\'m sterile or damn close to it, so all that fun-juice is just for show.</i>"', false );
		} else {
			MainView.outputText( '</i>"<i>You\'re so good to me ' + CoC.player.short + '.  I wish my little pony here was fertile enough for me to put a bun in your oven, if you wanted of course.</i>"', false );
		}
		...
		MainView.outputText( 'You nod before you leave, feeling satisfied and a warm glow from something other than lust.', false );
	...
	Urta.prototype.dudeFuckingUrtasCooch = function() {
		...
		MainView.outputText( '', true );
		MainView.outputText( ImageManager.showImage( 'urta-home-male-fuck' ), false );
		...
		CoC.flags[ kFLAGS.URTA_TIME_SINCE_LAST_CAME ] = 2;
		MainView.outputText( 'You let her know that you want her just the way she is and climb into bed behind her.  She watches you apprehensively over her shoulder as you pull out your ' + Descriptors.multiCockDescriptLight() + ' and start lining yourself up to fuck her doggie-style.  ', false );
		if( cocks > 1 && y >= 0 ) {
			MainView.outputText( 'Your secondary ' + Descriptors.cockDescript( y ) + ' wobbles directly behind her back-door and Urta\'s eyes widen a bit when she realizes you\'re going to double-penetrate her.  ', false );
		}
		if( cocks > 2 ) {
			MainView.outputText( 'The remainder of your ' + Descriptors.multiCockDescriptLight() + ' fan out, ready to rub themselves against her butt-cheeks once you push inside.  ', false );
		}
		MainView.outputText( 'You can see your lover\'s condom starting to bloat at the tip as her horse-cock begins to flare within its confinement.  She trembles with anticipation before taking a deep breath and saying, "<i>I\'m ready... take me!</i>"\n\n', false );
		MainView.outputText( 'Obligingly, you ease your hips forward, reeling at the heat of her cunt-lips as they welcome your ' + Descriptors.cockDescript( 0 ) + '\'s ' + CoC.player.cockHead() + ' with slippery moisture.  ', false );
		if( cocks > 1 && y >= 0 ) {
			MainView.outputText( 'Your second ' + Appearance.cockNoun( CoC.player.cocks[ y ].cockType ) + ' soon bumps against her pucker and strains against it, pushing ever-so-slowly into her tight hole as she gradually relaxes before you.  ', false );
		}
		MainView.outputText( 'As you carefully breach the opening', false );
		if( cocks > 1 && y >= 0 ) {
			MainView.outputText( 's', false );
		}
		MainView.outputText( ', the going gets easier and easier for you.  You gently rock your [hips] forwards, sliding deep inside her until she\'s taken you to the hilt.  Once you\'re fully embedded within her, Urta wriggles happily beneath you, and you reach around to caress her drooping member, relishing in her pleasured yelps and moans.\n\n', false );
		MainView.outputText( 'You rock back and forth, picking up the tempo gradually.  At first the gray fox is only gasping happily, but as you go faster and faster, her gasps turn into quiet moans, then transform into ecstatic squeals.  ', false );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( 'Your balls loudly smack into her squelching-wet cunt and the soaked backside of HER balls', false );
		} else {
			MainView.outputText( 'Your crotch slaps loudly against her squelching-wet cunt and backside', false );
		}
		...
		MainView.outputText( 'Urta cums first.  Her cries of pleasure are muffled as she shoves her face into a pillow, screaming in ecstasy.  The warmth around your ' + Descriptors.cockDescript( x ) + ' seems to grow that much hotter as it squeezes you, wringing you tightly from root to tip.  ', false );
		if( cocks > 1 && y >= 0 ) {
			MainView.outputText( 'The intensity and pressure in her back-door is even worse, and your ' + Descriptors.cockDescript( y ) + ' is nearly in pain from her asshole\'s tight, pulsating grip.  ', false );
		}
		MainView.outputText( 'Her equine member trembles in your grasp as it releases its sticky payload into the thick, latex condom.  You pump away at her shaft, feeling the spunk inside distort the tube-like appendage until your orgasm shocks you into releasing it.\n\n', false );
		MainView.outputText( 'You spurt into Urta\'s sloppy box, creaming it until her juices begin to run in an off-white color.  ', false );
		if( cocks > 1 && y >= 0 ) {
			MainView.outputText( 'The ' + Descriptors.cockDescript( y ) + ' you\'ve crammed in her ass does not disappoint either, matching its brother squirt for squirt as it spends your seed with reckless abandon.  ', false );
		}
		if( cocks > 2 ) {
			MainView.outputText( 'Her butt and tail are covered with the stuff as each of your remaining ' + Descriptors.multiCockDescriptLight() + ' splatters her with spooge, covering her fur in white sludge.  ', false );
		}
		MainView.outputText( 'The fox\'s internal muscles happily continue to squeeze and milk you, prolonging your orgasm beyond its normal duration.', false );
		if( CoC.player.cumQ() >= 750 ) {
			MainView.outputText( '  Her body is quickly bloated thanks to your body\'s incredible fertility, but she seems able to take it just fine.', false );
		}
		MainView.outputText( '\n\n', false );
		MainView.outputText( 'The two of you collapse together in a shuddering pile of post-orgasmic bliss.  The shift in position frees your ' + Descriptors.multiCockDescriptLight() + ' from Urta with a noisy -SHHHLLUUUCK-, but you\'re too wiped out to care.  The fox-girl sighs happily and rolls over, planting a kiss on your nose before she works at removing her over-filled condom.  It nearly bursts when she ties it off, but she manages to gently set it down to the floor without incident. She looks at you happily and gives you a big hug, crushing her hard nipples against ', false );
		if( CoC.player.biggestTitSize() >= 1 ) {
			MainView.outputText( 'your own ' + Descriptors.breastDescript( 0 ), false );
		} else {
			MainView.outputText( 'your chest', false );
		}
		MainView.outputText( ' as she says, "<i>Thank you so much, lover.  I forget how good it can be in my... vagina.  Maybe we could do this again?</i>"\n\n', false );
		MainView.outputText( 'You give her a happy nod, and the two of you cuddle a bit longer before you leave to return to camp.', false );
		CoC.player.orgasm();
		EngineCore.dynStats( 'sen', -2 );
		this.knockUpUrtaChance();
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	...
	Urta.prototype.fuckItAndLeave = function() {
		...
		CoC.flags[ kFLAGS.URTA_TIME_SINCE_LAST_CAME ] = 36;
		this.urtaLove( -5 );
		MainView.outputText( '', true );
		MainView.outputText( 'You let Urta know that you\'ve changed your mind and will be leaving.  You go before she can object and the sound of quiet sobbing chases you out into the streets.', false );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	...
	Urta.prototype.tenderTakeItUpTheAssFromUrta = function() {
		...
		CoC.flags[ kFLAGS.TIMES_FUCKED_URTA ]++;
		CoC.flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ]++;
		CoC.flags[ kFLAGS.URTA_TIME_SINCE_LAST_CAME ] = 2 + Utils.rand( 3 );
		var cocks = CoC.player.totalCocks();
		MainView.outputText( ImageManager.showImage( 'urta-home-anal' ), false );
		MainView.outputText( 'You give Urta\'s backside a playful slap and ask her to roll over.  She does so with a hesitant look on her face, wondering what you\'re about.  When you spread her legs and work your way forward to straddle her, a beaming smile breaks across her face and she says, "<i>You really want me inside you, don\'t you ' + CoC.player.short + '?</i>"\n\n', false );
		MainView.outputText( 'Her cheeks color when you give her a nod and begin lowering your ' + Descriptors.buttDescript() + ' towards her pillar of horse-flesh. She gasps quietly when the tip begins to push against your ' + Descriptors.assholeDescript() + '.  ', false );
		if( CoC.player.analCapacity() >= 60 ) {
			MainView.outputText( 'You sigh and begin sliding down it, your previously-stretched backdoor easily accepting the gigantic intruder as it fills you.', false );
		} else {
			MainView.outputText( 'You grunt and begin sliding down it, feeling painfully stretched by the gigantic intruder but pressing on anyway, knowing full-well you\'ll be a bit looser by the time you take her whole length.', false );
			if( CoC.player.ass.analLooseness < 5 ) {
				if( CoC.player.ass.analLooseness === 0 ) {
					MainView.outputText( '<b>  Urta has taken your anal virginity.</b>', false );
				} else {
					MainView.outputText( '<b>  Your ass is stretched.</b>', false );
				}
		...
		if( CoC.player.totalCocks() > 0 ) {
			MainView.outputText( '  The bumps and veins keep rubbing against your prostate, and ', false );
			if( cocks > 1 ) {
				MainView.outputText( 'each of ', false );
			}
			MainView.outputText( 'your ' + Descriptors.multiCockDescriptLight() + ' hardens near instantly from the forced stimulation.', false );
		}
		MainView.outputText( '  Slowly but surely, it slips further and further inside you, until you\'ve bottomed out and have miraculously taken her entire shaft.\n\n', false );

		//(IF DICKS);
		if( cocks > 0 ) {
			MainView.outputText( 'For a few moments you sit on her deeply penetrating thickness, and ', false );
			if( cocks > 1 ) {
				MainView.outputText( 'each of ', false );
			}
			MainView.outputText( 'your ' + Descriptors.multiCockDescriptLight() + ' droops onto her belly, half-hard from the pressure on your prostate, but going a little soft from the lack of attention.', false );
			if( CoC.player.cocks[ 0 ].cockLength >= 15 ) {
				MainView.outputText( '  Urta winks and guides ', false );
				if( cocks > 1 ) {
					MainView.outputText( 'one of ', false );
				}
				MainView.outputText( 'your ' + Descriptors.multiCockDescriptLight() + ' between her tits', false );
				if( CoC.player.cocks[ 0 ].cockLength >= 20 && CoC.player.cocks[ 0 ].cockLength <= 28 ) {
					MainView.outputText( ', even taking time to lick and suck on the head', false );
				}
				MainView.outputText( '.', false );
				MainView.outputText( '  She begins giving you a slow tit-fuck, trying to encourage you to pick up the pace and start riding her.  It does the trick as your ' + Descriptors.hipDescript() + ' start rocking on their own in reaction to her efforts.  She licks harder, and you reward her by lifting yourself up and sliding back down.  Her massive girth squeezes a squirt of pre-cum from your prostate with every few strokes, splattering it on her muzzle.', false );
			}
			MainView.outputText( '\n\n', false );
		} else {
			MainView.outputText( 'For a few moments you sit like that, trying to adjust to the pressure in your backside.  After a few moments you can feel your muscles relaxing, allowing Urta to rest more comfortably inside you.  Flexing your ' + CoC.player.legs() + ', you pull yourself up off of her until the flared tip is just barely inside you, then you plunge back down.  Urta\'s mouth hangs open and her tongue lolls out, and you can feel the warmth of her leaking fluid inside of you, letting you know just how good your ' + Descriptors.assholeDescript() + ' feels to her.\n\n', false );
		}
		MainView.outputText( 'You bounce on top of your fox-lover\'s equine rod, spearing your ' + Descriptors.assholeDescript() + ' on it over and over.  ', false );
		if( cocks === 0 ) {
			MainView.outputText( 'Strange pleasure spreads from your sensitive opening, and you know that somehow you\'re going to cum from this.  ', false );
		} else {
			MainView.outputText( 'Powerful pleasure spreads through you as it bumps and rubs your prostate. You know you\'ll cum from this, and soon.  ', false );
		}
		MainView.outputText( 'Urta cries out and grabs you by the hips, yanking you down against her sheath.  Warmth blossoms inside you as her seed begins pumping into your bowels.  You can feel yourself filling up with her seed and you cum, clamping down around her intruder as it injects more and more foxy-horse-spunk into you.\n\n', false );
		//(DICKS);
		if( cocks > 0 ) {
			if( CoC.player.cocks[ 0 ].cockLength < 15 ) {
				MainView.outputText( 'Your orgasm erupts out onto her belly, splattering her fur with your sticky cum.  ', false );
			} else if( CoC.player.cocks[ 0 ].cockLength < 20 ) {
				MainView.outputText( 'Your orgasm erupts all over her tits, slicking them with your sticky white cum.  ', false );
			} else {
				MainView.outputText( 'Your orgasm erupts into her muzzle, filling her mouth with your sticky, white cum.  ', false );
			}
			MainView.outputText( '  Urta groans underneath you and begins caressing and squeezing you, milking you of every drop.', false );
			if( CoC.player.cumQ() >= 500 ) {
				MainView.outputText( '  Urta gulps down as much as she can but it begins leaking around her muzzle, and she eventually lets it pop free, giving up on swallowing it entirely.', false );
			}
			if( CoC.player.cumQ() >= 1000 && CoC.player.cumQ() < 1500 ) {
				MainView.outputText( '  Your output easily soaks her, and she gathers it up and slops it over her nipples, pinching and pulling on them.', false );
			}
			if( CoC.player.cumQ() >= 1500 ) {
				MainView.outputText( '  Your output soaks her and the bed with seed, leaving the two of you in a warm gooey puddle in the very center of the bed.', false );
			}
			MainView.outputText( '\n\n', false );
		}
		MainView.outputText( 'Urta pulls you down onto her breasts and gives you a long hug and a happy lick.\n\n', false );
		MainView.outputText( '"<i>Thank you soooo much lover, I needed that,</i>" she says, "<i>Now you better go get back to your business, I\'ve got to get cleaned up!', false );
		if( CoC.player.cumQ() > 750 ) {
			MainView.outputText( '  You really are a messy, ' + CoC.player.mf( 'boy', 'girl' ) + ' aren\'t you?', false );
		}
		MainView.outputText( '</i>"\n\n', false );
		MainView.outputText( 'She untangles herself from you, the filled end of her condom loudly popping free from your backside as she disengages herself from you.  The two of you do your best to clean up and get dressed, and you give her a quick but passionate kiss before ducking out the door to check up on your camp.', false );
		CoC.player.orgasm();
		EngineCore.dynStats( 'sen', -2 );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	...
	Urta.prototype.marbleCockuBlockuUrta = function() {
		this.urtaSprite();
		MainView.outputText( '', true );
		MainView.outputText( 'Urta gives you a serious look when you come up to her, "<i>I just got a report in from some of our scouts last night.  It looks like they found your camp.  There was a cow-girl there', false );
		//if nursery;
		if( CoC.flags[ kFLAGS.MARBLE_NURSERY_CONSTRUCTION ] === 100 ) {
			MainView.outputText( ' and a nursery', false );
		}
		MainView.outputText( '.', false );
		//(Not wuv'ed yet);
		if( CoC.flags[ kFLAGS.URTA_PC_LOVE_COUNTER ] === 0 ) {
			MainView.outputText( '  I know we aren\'t a couple or anything, but, is she something to you?  Are you lovers?</i>"\n\n', false );
			MainView.outputText( 'Urta\'s cheeks color sheepishly and she says, "<i>You don\'t have to answer, but whatever the truth is, you can tell me.</i>"\n\n', false );
		}
		//(WUVED);
		else {
			MainView.outputText( '  I know our paths keep us apart most of the time... and multiple lovers are pretty far from uncommon, but I need to know – do you two have something going on?</i>"\n\n', false );
			MainView.outputText( 'Urta\'s looking at you nervously, clearly dreading your answer.  How do you respond?\n\n', false );
		}
		MainView.outputText( 'Do you answer honestly, refuse to answer, or lie?', false );
		//[Honestly] [Refuse] [Lie];
	...
	//[Comfort];
	Urta.prototype.comfortUrtaAfterTellinTruuf = function() {
		this.urtaSprite();
		MainView.outputText( '', true );
		MainView.outputText( 'You let Urta know that you understand.  After all, in a world like this monogamy is next to impossible.  There\'s no shame in what she did with Edryn or your relationship with Marble.  The two of you share a heartfelt hug and resolve to continue your relationship.', false );
		//(If PC has Edryn (<3) & fits her req's);
		if( ((CoC.player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_CENTAUR && CoC.player.totalCocks() > 0) ||
			CoC.player.horseCocks() > 0 ||
			(CoC.player.totalCocks() > 0 && CoC.player.cor > 50)) &&
			CoC.player.statusAffectv1( StatusAffects.Edryn ) >= 4 ) {
			MainView.outputText( '  Urta even suggests that the two of you take turns at Edryn sometime.', false );
		}
		//Slight love loss;
		this.urtaLove( -4 );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	...
	Urta.prototype.toughenUpUrtaAfterTellingTruuf = function() {
		this.urtaSprite();
		MainView.outputText( '', true );
		MainView.outputText( 'You look at Urta and explain that she shouldn\'t take it too hard.  Both of you got into this scenario without expecting anything more from it.  You explain that though she is a good friend and GREAT in the sack, you don\'t see your relationship going to that level.  Urta seems ', false );
		if( CoC.flags[ kFLAGS.URTA_PC_LOVE_COUNTER ] === 1 ) {
			MainView.outputText( 'heartbroken', false );
		} else {
			MainView.outputText( 'saddened', false );
		}
		MainView.outputText( ' by the news', false );
		if( CoC.flags[ kFLAGS.URTA_PC_LOVE_COUNTER ] === 1 ) {
			MainView.outputText( ', and she has a defeated look in her eyes.', false );
			//(Luv set to 0 and blocked if in luv,;
			CoC.flags[ kFLAGS.URTA_PC_AFFECTION_COUNTER ] = 0;
			CoC.flags[ kFLAGS.URTA_PC_LOVE_COUNTER ] = -1;
		} else {
			MainView.outputText( ', but you spy a determined look in her eyes.', false );
	...
	Urta.prototype.walkoutOnUrtaAfterTellingTruff = function() {
		this.urtaSprite();
		MainView.outputText( '', true );
		MainView.outputText( 'You push away from the table and turn your back on Urta, enraged at her for cheating on you with Edryn, even though you cheated on her as well.  You ignore the hypocrisy in your actions and walk out, shattering Urta\'s heart in the process.  As you close the door of \'The Wet Bitch\' behind you, you see Urta, sobbing into a hanky while Edryn goes over to comfort her.  It figures.', false );
	...
	Urta.prototype.lietoUrtaAboutMarble = function() {
		this.urtaSprite();
		MainView.outputText( '', true );
		MainView.outputText( 'You let Urta know that the poor cow-girl needed a place to stay from the monsters that roam the wilds, and that your camp is the safest place.  In great detail you talk about sheltering her, but never taking advantage of her, and trying your best to do what\'s right.  Through the whole conversation Urta dabs at tears, and you assume them to be tears of relief.  The story suitably complete and air-tight, you finish and look at Urta.\n\n', false );
		MainView.outputText( '"<i>I misjudged you, ' + CoC.player.short + '.  I didn\'t think you\'d lie to me, and you did.  The scouts talked to her', false );
		if( CoC.flags[ kFLAGS.MARBLE_KIDS ] === 1 ) {
			MainView.outputText( ' and saw the kid', false );
		}
		if( CoC.flags[ kFLAGS.MARBLE_NURSERY_CONSTRUCTION ] > 1 ) {
			MainView.outputText( ' and saw the kids', false );
		}
		MainView.outputText( '!  I know you two are an item, and I can\'t believe you didn\'t own up to it!</i>" she cries, "<i>You better believe I\'m fucking pissed at you.  I could\'ve accepted sharing you, but lying to me like this...I won\'t ever trust you again.  Just...just go – I might be able to overlook this but it hurts too much right now.</i>"\n\n', false );
		MainView.outputText( 'There\'s nothing to do but go.  You\'re not sure if Urta will ever let you see her again.', false );
	...
	Urta.prototype.UrtaTwuWuvOffer = function() {
		this.urtaSprite();
		MainView.outputText( '', true );
		MainView.outputText( 'Urta looks nervously at you, fidgeting and shifting her sizable addition under the table.  She starts talking meekly, but her voice rises in confidence and intensity as she goes, "<i>I\'ve been thinking ' + CoC.player.short + ', we\'ve known each other for a while now.  When we\'re together, I don\'t have to worry about someone seeing something they shouldn\'t, or being made fun of, because it doesn\'t bother you and you don\'t care.  And what do I have to care about if you don\'t care about it?</i>"\n\n', false );
		MainView.outputText( 'She leans over and puts her hands on your shoulders, looking you in the eye as she continues, "<i>I\'ve never felt this comfortable around ANYONE before, and well, I think I love you.  Please, do you feel the same way about me?</i>"\n\n', false );
		MainView.outputText( 'Given how much time you\'ve spent with her, and the amazing sex, her declaration doesn\'t really surprise you.  Do you love her back?', false );
		//[Yes] [No];
	...
	Urta.prototype.noUDontLoveUrta = function() {
		...
		CoC.flags[ kFLAGS.URTA_PC_AFFECTION_COUNTER ] = 0;
		CoC.flags[ kFLAGS.URTA_PC_LOVE_COUNTER ] = -1;
		MainView.outputText( 'You gently remove her hands from your shoulders and let Urta know that you enjoy being around her, but that you aren\'t in love with her.  Her eyes choke up with tears, but she nods as you explain, wiping her eyes with a napkin.  Urta cries softly and murmurs, "<i>...I didn\'t want to hear that, but I understand.  Do you want to keep on as we have been though?</i>"\n\n', false );
		//[Yes] [No];
	...
	Urta.prototype.stayFuckbuddiesAfterShootingDown = function() {
		this.urtaSprite();
		MainView.outputText( 'Urta gives you a warm smile and a kiss on the cheek, "<i>Good.  Come see me soon, ok?</i>"\n\n', false );
	...
	Urta.prototype.turnDownFuckbuddiesANDLove = function() {
		this.urtaSprite();
		MainView.outputText( '', true );
		MainView.outputText( 'Urta bursts into tears and pushes back from the table.  She runs out of the bar, sobbing wildly.  It looks like you\'ve totally burned that bridge.  You may as well head back to camp...', false );
		//(BURN BRIDGES MOFO);
		CoC.flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] = -1;
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	...
	Urta.prototype.blowUrtaUnderTheTableLuv = function() {
		...
		MainView.outputText( 'Deciding you\'ve got to get her off and make her spill her drink, you ', false );
		if( CoC.player.cor < 50 ) {
			MainView.outputText( 'do your best to suppress your gag reflex and shove forwards, ramming her cock as deeply into your throat as you can.', false );
		} else {
			MainView.outputText( 'shove forwards, ramming her massive cock so far down your throat that you can feel it dribbling pre-cum into your belly.', false );
		}
		MainView.outputText( '  Urta grunts, doing her best to muffle the noise as it escapes her lips, but you can feel her twitch and thicken inside you, beginning to drip with proof of her complete and total submission to your mouth.\n\n', false );
		MainView.outputText( 'You can hear chuckles from the bar\'s other occupants as they realize what\'s going on, and you blush, ', false );
		if( CoC.player.cor < 50 && CoC.flags[ kFLAGS.PC_FETISH ] <= 0 ) {
			MainView.outputText( 'wishing there was a tablecloth to hide your activities from prying eyes', false );
		} else {
			MainView.outputText( 'wishing you had even more eyes watching your show', false );
		}
		MainView.outputText( '.  The distraction does little to divert you from your chosen task, and you close your eyes, tasting your lover\'s throbbing shaft as it fills your mouth.  With both hands stroking, you bob up and down, completely centering your world around the pulsating horse-prick as it fills your mouth.\n\n', false );
		MainView.outputText( 'Without warning, Urta\'s legs wrap around your back, squeezing with her soft fur as she vices them closed.  You\'re forced to take her even more deeply, until the folds of her sheath are bumping your nose and chin and filling your nostrils with her heady musk.  You can\'t even breathe with such a massive intruder blocking your throat, but you do your best to get her off, knowing it\'s the quickest way to end it.  You suck hard and squeeze her balls, gurgling noisily around the sweaty-shaft as you feel Urta give in.\n\n', false );
		MainView.outputText( 'A wave-like motion passes through your throat-obstruction, stretching you wider than ever before.  It passes, leaving behind a feeling of warm fullness.  You feel another working its way into your mouth, and realize Urta is cumming, filling your belly with direct injections of horsey fox-seed. You desperately need to breathe, but you\'re trapped on your lover\'s maleness, and with nothing else to do, you give her twitching balls a squeeze, feeling her body twitch in response as she dumps a particularly heavy batch of spunk into you.  Happy that you could at least control how hard she came, you begin blacking out, barely noticing as Urta\'s orgasmic contractions die off.\n\n', false );
		MainView.outputText( 'Thumping down on your back, you\'re blissfully aware of the sweet air filling your lungs.  In a moment of reflection, you realize you can taste the salty flavor of Urta\'s seed on your lips and mouth, and the retracting horse-cock is ', false );
		if( CoC.player.biggestTitSize() >= 1 ) {
		...
		MainView.outputText( ' as it begins to soften.  You wipe as much of her leavings from your ' + CoC.player.face() + ' as you can, licking the slippery glaze from your lips.  Before you get up, you make sure to give Urta\'s member a firm squeeze, scolding it for its forceful treatment of your mouth.\n\n', false );
		MainView.outputText( 'Once you\'ve climbed out from under the table, you\'re VERY aware of the eyes of some of the nearby bar patron\'s on your back.  It seems your undercover act managed to draw more than a little attention.  Urta\'s cheeks burn bright-red under her gray fur, though she has a loving smile on her face.  The pair of you share a tender kiss to hoots and catcalls from the audience.  Urta smiles and gushes, "<i>You give the BEST blowjobs... I mean, WOW, but maybe next time we should go back to my place and do something a little more... fulfilling for both of us.</i>"', false );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
		CoC.flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ]++;
	...
	//[RIDE VAGINAL];
	Urta.prototype.rideUrtasCoochLoveyDovey = function() {
		this.urtaSprite();
		...
		if( CoC.player.vaginalCapacity() < 40 ) {
			MainView.outputText( 'Urta smiles at first, but as you strip down and get ready her expression turns to one of care and worry.  She apologizes, "<i>I\'m sorry sweetheart, but I don\'t think you could take me like that without some serious pain.  Maybe after you\'ve loosened up that cute little snatch with some sex-toys I could give it a go... by Marae I want this, but you aren\'t ready for my... my thing.</i>"\n\n', false );
			EngineCore.doNext( this, this.urtaHomeLuvLuvinsMenu );
			return;
		}
		//(SHIT FITS);
		MainView.outputText( ImageManager.showImage( 'urta-home-female-fuck' ), false );
		MainView.outputText( 'Urta bursts into a wide smile at your suggestion and scooches over, making room for you on the bed.  ', false );
		if( CoC.player.cockTotal() > 0 ) {
			MainView.outputText( 'She tosses you ', false );
			if( CoC.player.cockTotal() === 1 ) {
				MainView.outputText( 'a latex sheath', false );
			} else {
				MainView.outputText( 'some latex sheaths', false );
			}
			MainView.outputText( ' to cover your ' + Descriptors.multiCockDescriptLight() + ' with, explaining that she doesn\'t want to have to mop up afterward, and you grudgingly agree.  ', false );
		}
		MainView.outputText( 'You climb in next to her, snuggling under the blankets and stroking her shaft, feeling yourself grow more and more ready as you tease your vulpine lover.  Urta doesn\'t move beyond rolling her eyes back and squirming her hips into your hand, trying to enhance the sensation.  She whines, "<i>I thought you were going to fuck meee-</i>"\n\n', false );
		MainView.outputText( 'Her cries are muted as you launch yourself over her, brutalizing her eager lips with a ferocious kiss.  You roughly tongue-fuck her as you adjust yourself, balancing yourself with one hand while your other guides her trembling shaft closer and closer to the entrance of your ' + Descriptors.vaginaDescript( 0 ) + '.  You sigh happily and rub the pre-filled bulb against your opening, ', false );
		if( CoC.player.vaginas[ 0 ].vaginalWetness < AppearanceDefs.VAGINA_WETNESS_WET ) {
			MainView.outputText( 'growing moist', false );
		} else if( CoC.player.vaginas[ 0 ].vaginalWetness < AppearanceDefs.VAGINA_WETNESS_DROOLING ) {
			MainView.outputText( 'growing wetter and wetter as you lubricate the tip of her condom', false );
		} else {
			MainView.outputText( 'soaking her condom-covered cock with your copious cunt-secretions', false );
		}
		MainView.outputText( '.  Urta\'s body goes limp under your sexual assault, totally caving in to the feelings you\'re stoking inside her.  The only thing she manages to keep up is the twisting of her hips as she tries to enhance the sensations radiating through her massive, flared prick.\n\n', false );
		MainView.outputText( 'You end the kiss by pulling back, sucking her lower lip into your mouth, and then releasing it with a pop.  Nearly crying with need, your horse-endowed lover begs, "<i>Ah, please don\'t stop!</i>"\n\n', false );
		MainView.outputText( 'Never one to disappoint, you press onwards and downwards, sliding deeper into the bed as you work the wide, flared tip against yourself.  It slowly stretches you wide and bows under your weight, though Urta doesn\'t voice a word of complaint or displeasure.  You swivel slightly to the side and it works its way inside you, one half of the tip sliding inside at a time.  The fox\'s mouth opens into an \'O\' of pleasure, letting her tongue escape as she begins panting from the pleasure.\n\n', false );
		MainView.outputText( 'Sinking down slowly and relishing the feeling of your passage being spread, you allow a low moan to escape your mouth.  Urta\'s hands lock onto her breasts and begin alternatively squeezing her soft mounds and fondling the black skin of her nipples.  The sensation of slow penetration is exquisite, growing within you while ever thicker horse-cock spears your womb.  You can feel the beat of Urta\'s heart racing through the shaft, swelling it slightly in time with each heartbeat.\n\n', false );
		MainView.outputText( 'You bottom-out, but immediately begin rocking your hips, working your lover\'s twitching rod deep inside your ' + Descriptors.vaginaDescript( 0 ) + '.  ', false );
		//(LOOSENESS MESSAGES GO HERE AS APPROPRIATE);
		CoC.player.cuntChange( 60, true );
		MainView.outputText( 'Urta\'s hands grab you firmly by the ' + Descriptors.assDescript() + ', reminding you just how strong the lithe fox is.  She begins working with you to lift your body up and slam it back down against her, alternating the feelings of emptiness and fullness within you.  Each time the little balloon that\'s formed on her flare gets larger, squishing each time she bottoms out.  As the pair of you rut faster and faster, the limited space inside you squeezes the excess pre-cum from the sheath, causing it to drip over her balls into the sheets.\n\n', false );
		MainView.outputText( 'Bending over, you plant another kiss full on her lips, and your vulpine lover just can\'t restrain herself anymore.  She moans into your mouth, her body shivering, her balls tightening up until you feel them press against your ass.  Her hands grip you hard and pull you down with incredible force, locking you against her as you feel her member swelling inside you, submitting to your ' + Descriptors.vaginaDescript( 0 ) + '\'s swelteringly hot embrace.  A burst of tightness fills you, growing as Urta squirms, unloading her massive horse-pole\'s seed into the latex balloon she\'s stuffed inside you.\n\n', false );
		MainView.outputText( 'The filling seed balloon stretches tight inside of you, ', false );
		if( CoC.player.vaginalCapacity() < 80 ) {
			MainView.outputText( 'until there is no more room for cum and it\'s squirting out around the base of her cock, soaking Urta and the bed with fragrant horse-cum.', false );
		} else {
			MainView.outputText( 'filling your body with a large bubble of hot horse-cum.', false );
		}
		MainView.outputText( '  Watching your lover submit to your body\'s pleasure, the sight pushes you over the deep edge with her, and your body begins milking the thick shaft inside you, clenching around it as you spasm happily.  ', false );
		if( CoC.player.cockTotal() > 0 ) {
			if( CoC.player.cockTotal() === 1 ) {
				MainView.outputText( 'Y', false );
			} else {
			...
			MainView.outputText( 'our ' + Descriptors.multiCockDescriptLight() + ' spurts, unloading waves of cream into the ', false );
			if( CoC.player.cockTotal() === 1 ) {
				MainView.outputText( 'condom Urta asked you to wear around it.', false );
				if( CoC.player.cumQ() >= 250 ) {
					if( CoC.player.cumQ() < 500 ) {
						MainView.outputText( '  The balloon inflates slightly, drooping down as your maleness empties its reserves.  ', false );
					} else if( CoC.player.cumQ() < 1000 ) {
						MainView.outputText( '  The balloon swells larger and larger as you pump it full, eventually wobbling around on Urta\'s belly as you finish filling it.', false );
					} else {
						MainView.outputText( '  The balloon swells larger and larger as you pump it full, wobbling obscenely on Urta\'s belly and slowly drooping down onto the sheets as the sheath reaches its maximum capacity.', false );
						if( CoC.player.cumQ() > 1500 ) {
							MainView.outputText( '  Your potent seed pops the condom, splattering Urta with your seed as it explosively fails.', false );
						}
					}
				}
			} else {
				MainView.outputText( 'condoms Urta asked you to wear around them.', false );
				if( CoC.player.cumQ() >= 250 ) {
					if( CoC.player.cumQ() < 500 ) {
						MainView.outputText( '  The balloons inflate slightly, drooping down as your malenesses empty their reserves.  ', false );
					} else if( CoC.player.cumQ() < 1000 ) {
						MainView.outputText( '  The balloons swell larger and larger as you pump them full, eventually wobbling around on Urta\'s belly as you finish filling them.', false );
					} else {
						MainView.outputText( '  The balloons swell larger and larger as you pump them full, wobbling obscenely on Urta\'s belly and slowly drooping down onto the sheets as the sheaths reach their maximum capacity.', false );
						if( CoC.player.cumQ() > 1500 ) {
							MainView.outputText( '  Your potent seed pops the condoms, splattering Urta with your seed as they explosively fail.', false );
						}
					}
				}
			}
	...
	//[RIDE ANAL];
	Urta.prototype.rideUrtaInButtAtHomeLove = function() {
		this.urtaSprite();
		this.urtaLove( 1 );
		CoC.flags[ kFLAGS.URTA_TIME_SINCE_LAST_CAME ] = 2 + Utils.rand( 2 );
		MainView.outputText( '', true );
		MainView.outputText( ImageManager.showImage( 'urta-home-anal-urta' ), false );
		MainView.outputText( 'Urta raises an eyebrow at the suggestion and blushes, though her horse-like member thickens and stiffens noticeably.  You give it a gentle slap and tease, "<i>Don\'t act like this isn\'t what you wanted!</i>"\n\n', false );
		MainView.outputText( 'She squirms and smiles nervously, "<i>It feels too good, even though it\'s dirty!  Besides, your sweet ass is irresistible!</i>"\n\n', false );
		if( CoC.player.totalCocks() > 0 ) {
			MainView.outputText( 'Even though you\'re going to be riding her, she insists you wear ', false );
			if( CoC.player.totalCocks() === 1 ) {
				MainView.outputText( 'a condom', false );
			} else {
				MainView.outputText( 'some condoms', false );
			}
			MainView.outputText( ' to help control the mess.  You grudgingly accept, girding your hardening shaft', false );
			if( CoC.player.totalCocks() > 1 ) {
				MainView.outputText( 's', false );
			}
			MainView.outputText( ' in tight latex.\n\n', false );
		}
		MainView.outputText( 'You climb on to the bed, but fur-covered hands grab hold of your ' + Descriptors.hipDescript() + ' and yank you down until something round and wide is knocking on your backdoor.  You can feel the latex sheath bubbling up, starting to give Urta\'s dick a more rounded tip and she begins pressing it into your backdoor.  The unexpected attempts at insertion surprised you and have kept you too tightly-clenched for the fox to batter her way inside.  She begs, "<i>Please let me in!  I can\'t wait anymore!</i>"\n\n', false );
		MainView.outputText( 'While giving her a knowing smile, you make a conscious attempt to relax your body\'s surprised rear-entrance.  Almost immediately you feel her slip inside, and then you\'re sliding down, sinking slowly towards your partner an inch at a time.  The expression on Urta\'s face is priceless – flitting between ecstasy and confusion as she struggles to deal with being engulfed by your ', false );
		if( CoC.player.analCapacity() < 60 ) {
			MainView.outputText( 'tight', false );
		} else {
			MainView.outputText( 'welcoming', false );
		}
		MainView.outputText( ', warm hole.  A groan escapes your mouth as you sink down, feeling fuller and fuller until her real sheath is resting just between your ass-cheeks.', false );
		CoC.player.buttChange( 60, true, true, false );
		if( CoC.player.totalCocks() > 0 ) {
			MainView.outputText( '  You can feel tiny dribbles force their way out of ', false );
			if( CoC.player.totalCocks() > 1 ) {
				MainView.outputText( 'each of ', false );
			}
			MainView.outputText( 'your urethra', false );
			if( CoC.player.totalCocks() > 1 ) {
				MainView.outputText( 's', false );
			}
			MainView.outputText( ' as your prostate is compressed.', false );
		}
		MainView.outputText( '\n\n', false );
		MainView.outputText( 'Urta recovers enough to meet your gaze and licks her lips hungrily.  Her hands release your ' + Descriptors.hipDescript() + ' and rump, sliding up your sides and in towards your ' + Descriptors.allBreastsDescript() + '.  You shiver as she circles your ' + Descriptors.nippleDescript( 0 ) + ', teasing the flesh around them while managing to avoid providing them with any stimulation.  She looks up at you expectantly with a patient look, as if waiting on something from you.\n\n', false );
		MainView.outputText( 'You shift your hips and are rewarded by a caress over your left ' + Descriptors.nippleDescript( 0 ) + ', but it stops almost immediately.  Catching on, you flex your ' + CoC.player.legs() + ' and lift yourself up until her flare is the only thing connecting you before sliding back down the pulsating shaft, squeezing your muscles around it.  Urta\'s hands reward you by pinching and tugging on your ' + Descriptors.nippleDescript( 0 ) + 's, ', false );
		if( CoC.player.hasFuckableNipples() ) {
			MainView.outputText( 'even slipping a finger inside their slippery depths, ', false );
		}
		MainView.outputText( 'spreading pleasure through the sensitive pleasure-buds.  Eager for more, you begin bouncing atop your vulpine lover, riding her as deeply as possible in return for a treasure-trove of sensation and lust.\n\n', false );
		MainView.outputText( 'Locked together by desire, love, and one swollen cock, you rut with her for what seems like forever.  You lose count of how many times your lover moans and screams in pleasure, and your own gasps of surprised enjoyment are too numerous to tabulate.  Urta\'s fur slowly becomes matted with sweat, and the salty scent of lovemaking fills the air.', false );
		if( CoC.player.totalCocks() > 0 ) {
			if( CoC.player.totalCocks() > 1 ) {
				MainView.outputText( '  Each of y', false );
			} else {
			...
			MainView.outputText( 'our ' + Descriptors.multiCockDescriptLight() + ' has filled its sheath with a golfball sized globule of pre-cum, and you can feel your body clenching and squeezing, your ', false );
			if( CoC.player.balls > 0 ) {
				MainView.outputText( 'balls', false );
			} else {
				MainView.outputText( 'prostate', false );
			}
			MainView.outputText( ' aching to unload.', false );
		}
		if( CoC.player.hasVagina() ) {
			MainView.outputText( '  Neglected and unused, your ' + Descriptors.vaginaDescript( 0 ) + ' ', false );
			if( CoC.player.vaginas[ 0 ].vaginalWetness < AppearanceDefs.VAGINA_WETNESS_DROOLING ) {
				MainView.outputText( 'dampens', false );
			} else {
				MainView.outputText( 'floods', false );
			}
			MainView.outputText( ' the fox underneath you with moisture, puffy and aching for the slightest touch.', false );
		}
		MainView.outputText( '\n\n', false );
		MainView.outputText( 'The lovemaking increases in tempo, slowly working its way to a feverish crescendo.  You\'re bouncing atop the fox wildly, and your lover cums hard.  As full as you felt with her inside you, it\'s nothing compared to being stuffed with her cock AND cum.  Each cum-filled contraction swells her member from base to flare, allowing you to feel it stretch your insides while it travels up your ' + Descriptors.assholeDescript() + '.  It dumps the load into the bubble of cum inside you, filling you with a creamy jizz balloon.\n\n', false );
		//(GENDERLESS ORGAZM);
		if( CoC.player.gender === 0 ) {
			MainView.outputText( 'You cry out with pleasure, feeling the closest thing you can to an orgasm.  Your muscles clench around the invading rod, milking it while the rest of your body trembles.  The strength drains from your ' + CoC.player.legs() + ' and arms and you twitch there, completely impaled on your lover\'s thick cock, happily and completely penetrated.\n\n', false );
		}
		//(EVERYBODY ELSE);
		else {
			MainView.outputText( 'You cry out with pleasure, feeling your body cum from the invading warmth in your ' + Descriptors.assholeDescript() + '.  Shivering as the strength drains from your arms and ' + CoC.player.legs() + ', you let the intruding and still cumming cock support your body.', false );
			if( CoC.player.hasVagina() ) {
				MainView.outputText( '  Your ' + Descriptors.vaginaDescript( 0 ) + ' clenches desperately, peaking without a single touch, but hungry for something to milk.', false );
				if( CoC.player.vaginas[ 0 ].vaginalWetness > AppearanceDefs.VAGINA_WETNESS_WET && CoC.player.vaginas[ 0 ].vaginalWetness < AppearanceDefs.VAGINA_WETNESS_SLAVERING ) {
					MainView.outputText( '  Fluids soak Urta\'s crotch with lust with feminine drool, though the fox barely seems to notice.', false );
				} else if( CoC.player.vaginas[ 0 ].vaginalWetness === AppearanceDefs.VAGINA_WETNESS_SLAVERING ) {
					MainView.outputText( '  An explosion of feminine fluids spatters Urta\'s crotch and bed, completely soaking her groin in your need, but the distracted fox barely notices.', false );
				}
			}
			if( CoC.player.totalCocks() > 0 ) {
				MainView.outputText( '  The warmth spreads deep inside you, firing off a series of contractions just inside your groin.  You can feel yourself about to cum, and look down just in time to see yourself squirting against your condom, deforming it from the pressure of the orgasm and slowly inflating it further.', false );
				if( CoC.player.cumQ() >= 750 && CoC.player.cumQ() < 1250 ) {
					MainView.outputText( '  By the time you\'re done the balloon is the size of a basketball and looking dangerously close to bursting.', false );
				} else if( CoC.player.cumQ() >= 1250 ) {
					MainView.outputText( '  By the time you\'re done, the balloon is the size of a watermelon and wobbling dangerously, looking like it could fail at any time!', false );
					if( CoC.player.cumQ() >= 1700 ) {
						MainView.outputText( '  You fill it fuller and fuller, until it\'s bigger than you thought possible.  All at once the material gives out, soaking Urta from the tits down in cum, while your ' + Descriptors.cockDescript( 0 ) + ' continues to pump even more onto her.  So much for not making a mess!', false );
					}
				}
			}
			MainView.outputText( '\n\n', false );
		...
		MainView.outputText( 'She comes out of her stupor and murmurs, "<i>Mmmm, that was nice,</i>" as she stretches.  With practiced motions she slides off her condom, ties it off, and tosses it into a waste-can half filled with the things.  ', false );
		if( CoC.player.cumQ() >= 1700 ) {
			MainView.outputText( 'Urta sighs at the mess and laughs, "<i>I guess we got a mess anyways, huh?</i>"', false );
		} else {
			MainView.outputText( 'Urta sighs contentedly and laughs, "<i>I guess I don\'t have much of a mess to clean for once!</i>"', false );
		}
	...
	Urta.prototype.oralFiestyUberExplosionUrta = function() {
		...
		if( CoC.player.cockArea( x ) >= 100 ) {
			MainView.outputText( 'Urta takes one look at your ' + Descriptors.cockDescript( x ) + ' and says, "<i>I\'m sorry babe, but there\'s no way I could take that monster in my mouth.  How about we do something else?</i>"\n\n', false );
			EngineCore.doNext( this, this.urtaHomeLuvLuvinsMenu );
			return;
		}
		MainView.outputText( ImageManager.showImage( 'urta-home-69', 'vert' ), false );
		if( CoC.player.gender === 2 || (CoC.player.gender === 3 && Utils.rand( 2 ) === 0) ) {
			MainView.outputText( 'You smile and let Urta know you want to sixty-nine her.  She raises an eyebrow and teases, "<i>I hope you don\'t mind having a full belly and a sore throat!</i>" as you disrobe and climb atop her.  You turn about, straddling your legs across her face and exposing the hungry lips of your sex.  Smirking over your shoulder, you snatch her discarded dress from the floor and tie the shimmering fabric into an impromptu cock-ring.  The gray fox shudders with excitement and you watch, enraptured, as her mottled horse-cock grows larger and larger, even flaring wide at the tip.\n\n', false );
			MainView.outputText( 'The musky scent of Urta\'s forearm-sized horse-prick wafts into your nostrils as it wobbles just below your ' + CoC.player.face() + ', visibly twitching and throbbing from all the extra blood.  She whines plaintively, nuzzling her moist nose against your ', false );
			if( CoC.player.vaginas[ 0 ].vaginalWetness < AppearanceDefs.VAGINA_WETNESS_DROOLING ) {
				MainView.outputText( 'equally wet', false );
			} else {
				MainView.outputText( 'sopping wet', false );
			}
			MainView.outputText( ' cunt.  Shivering from the air on your bare skin and the unexpected touches on your nethers, you open wide, extend your tongue, and give Urta\'s flare a long, slow lick.  You pause at her urethra, slip ever-so-slightly inside, and then pull out as you slather the flare in spit.\n\n', false );
			MainView.outputText( 'Urta eagerly gets to work, stabbing her tongue through your folds and into your ' + Descriptors.vaginaDescript( 0 ) + '.  Her efforts bring her muzzle flush against you as she pauses to slurp your pussy-juices over your ' + Descriptors.clitDescript() + '.  Her lips feel like liquid heat as they wrap around the incredibly sensitive bud, suckling it gently in between excursions into your passage.  Urta\'s slender fingers grab hold of your ' + Descriptors.buttDescript() + ' to support her as she tongue-fucks your ' + Descriptors.vaginaDescript( 0 ) + ' with abandon.\n\n', false );
			...
			if( CoC.player.cor > 50 ) {
				MainView.outputText( 'push forwards, feeling her shaft arch as it slides into your throat.  The medial ring slips past your lips and you can feel your throat bulging as you mouth-fuck the swollen horse-cock deep into your gut.  Warmth drips into your belly as the hermaphroditic fox\'s member closes on its sexual peak.\n\n', false );
			} else {
				MainView.outputText( 'grab hold of her cock and begin jerking her off into your mouth.  An immediate and copious gush of pre drips onto your tongue, showing you just how much the hermaphroditic fox loves the feeling.\n\n', false );
			}

			MainView.outputText( 'Not far from cumming yourself, you grind your ' + Descriptors.hipDescript() + ' on Urta\'s face, smearing her fur with your fragrant pussy-juices.  Her own moans of pleasure turn into noisy wet blubbering that vibrates through your ' + Descriptors.vaginaDescript( 0 ) + ', getting you off on the spot.  You squeal around the cock in your ', false );
			if( CoC.player.cor <= 50 ) {
				MainView.outputText( 'mouth', false );
			} else {
				MainView.outputText( 'throat', false );
			}
			MainView.outputText( ', clamping your ' + CoC.player.legs() + ' onto Urta\'s head as you orgasm.  ', false );
			if( CoC.player.vaginas[ 0 ].vaginalWetness > AppearanceDefs.VAGINA_WETNESS_DROOLING ) {
				MainView.outputText( 'Fluids rush from your folds, splattering her face with sex fluid.  The fox opens wide, gulping it down like someone in a desert who has just found an oasis.', false );
			} else {
				MainView.outputText( 'Wetness slowly coats her muzzle as your canal squeezes and wriggles with involuntary pleasure-spasms.  Urta never lets up, sucking and licking your ' + Descriptors.clitDescript() + ' in a rhythm that matches your own orgasmic contractions.', false );
			}
			if( CoC.player.totalCocks() > 0 ) {
				MainView.outputText( '  ', false );
				if( CoC.player.cockTotal() > 1 ) {
				...
				MainView.outputText( 'our ' + Descriptors.multiCockDescriptLight() + ' drools lamely, ', false );
				if( CoC.player.cumQ() < 50 ) {
					MainView.outputText( 'sputtering spunk onto Urta\'s neck.', false );
				} else if( CoC.player.cumQ() < 250 ) {
					MainView.outputText( 'soaking Urta\'s neck and chest with your spooge.', false );
				} else if( CoC.player.cumQ() < 700 ) {
					MainView.outputText( 'showering Urta\'s neck and body with spooge until her breasts are painted white and the bed has a small puddle soaking into it.', false );
				} else if( CoC.player.cumQ() < 1500 ) {
					MainView.outputText( 'soaking Urta with a torrent of seed that drenches her from the neck down and totally soaks the bed.', false );
				} else {
					MainView.outputText( 'soaking Urta with a torrent of seed that drenches her body and floods the bed, drips onto the floor, and slowly spreads into a puddle.', false );
					if( CoC.player.cumQ() > 10000 ) {
						MainView.outputText( '  The puddle thickens as you cum until it\'s nearly a foot deep.', false );
					}
				}
			}
			MainView.outputText( '\n\n', false );
			MainView.outputText( 'Too drunk with pleasure to register the telltale thickening of Urta\'s cock, you keep licking and sucking while you cum, drinking a steady stream of pre-cum.  Suddenly, your improvised cock-ring snaps and the straps fail from the massive pressure.  A torrent of jism ', false );
			if( CoC.player.cor > 50 ) {
				MainView.outputText( 'pours down your throat, bulging it into an obscene imprint of the horse-cock that\'s lodged inside.  Your eyes cross from pleasure and you slump down a bit, shoving your nose into Urta\'s sheath and allowing her to dump her sperm directly into your belly.  Gurgling noisily, your belly slowly expands outwards by about two inches, making you feel bloated and full.', false );
			} else {
				MainView.outputText( 'erupts into your mouth, instantly filling it with thick fox-cream.  You convulsively swallow in an attempt to keep up with it, but are quickly overwhelmed by the backed up semen-flood.  It drips from your mouth and forces open your throat, flowing directly into your belly.  Some even begins dripping out from your nose as you sway with a mixture of discomfort and pleasure atop your partner.  A few trembling squirts later you feel and look a little bloated, and your ' + CoC.player.face() + ' is thoroughly covered in foxy-horse-jizz.', false );
			}
			MainView.outputText( '\n\n', false );
			MainView.outputText( 'Both of you sputter for breath as you roll off of Urta.  She gasps and looks over at you with a smile, licking her sperm from your face and nuzzling her pussy-soaked muzzle against your cheek.  She pants, "<i>I suppose I should be mad at you for ruining the dress, but you got me off so hard that I think I gave you a few meals worth of my... um... cum.</i>"\n\n', false );
			MainView.outputText( 'The fox giggles and returns to cuddling and licking you.  You rest with a happy smile plastered on your ' + CoC.player.face() + ' from the experience.  After all the snuggling, you both rise and get dressed.  Urta finds a new dress in the closet and with a long goodbye kiss, you go your separate ways.\n\n', false );
			EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
			CoC.player.orgasm();
			EngineCore.dynStats( 'sen', -1 );
			return;
		...
		//(DUDEZILLAZ);
		MainView.outputText( 'Urta sits up and practically screams, "<i>Really,</i>" and squeals excitedly.  She claps and lies back as you climb into the bed, stopping only to yank off her cock-sheath and toss it into a nearby waste-bin.  You swing your ' + CoC.player.legs() + ' over her and get into position, aligning ', false );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( 'your ' + Descriptors.ballsDescriptLight() + ' and ', false );
		}
		if( CoC.player.totalCocks() > 1 ) {
			MainView.outputText( 'one of your ', false );
		} else {
			MainView.outputText( 'your ', false );
		}
		MainView.outputText( Descriptors.multiCockDescriptLight() + ' with Urta\'s muzzle.  Her rather sizable \'addition\' wobbles underneath you, blocking your vision as it wobbles back and forth under you.\n\n', false );
		MainView.outputText( 'Your foxy lover opens wide and slips her smooth, wide tongue all over your ', false );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( Descriptors.ballsDescript() + '.  Slithering over and around them, she bathes your sack in her spit before moving up to your ' + Descriptors.cockDescript( x ) + '.', false );
		} else {
			MainView.outputText( Descriptors.cockDescript( x ) + '.', false );
		}
		MainView.outputText( '  Her tongue curves and slurps at your shaft, swelteringly warm, wet, and tantalizingly slippery.  She tastes you from ', false );
		if( CoC.player.hasSheath() ) {
			MainView.outputText( 'sheath', false );
		} else {
			MainView.outputText( 'base', false );
		}
		MainView.outputText( ' to ' + CoC.player.cockHead( x ) + ', stopping to suckle the sensitive end briefly before breaking away for another long lick along the underside.  Her hands stroke along the sensitive ' + CoC.player.skinDesc + ' of your inner thighs, making your hips shiver and nearly pulling your ' + Descriptors.cockDescript( x ) + ' away from your lover\'s hungry mouth.\n\n', false );
		MainView.outputText( 'Whilst the fox is busy with your genitals, you begin to perform your duty, reciprocating the pleasure in turn.  You stretch your mouth wide, opening as far as you can to accommodate the mammoth beast before you.  It\'s difficult, but you manage to stretch wide enough to take her flare in your mouth.  It tastes vaguely salty like sweat, and in Urta\'s excited state it\'s easy to feel her flare widen and retract with each beat of her heart.  A dollop of pre-cum deposits itself on your tongue as the horny fox\'s nymphomaniac body reacts to you.  It tastes sweet, and you swallow as you work her deeper into your oral cavity.\n\n', false );
		MainView.outputText( 'Muffled, excited moans reach your ears as you work on your lover\'s pole.  The fox is clearly enjoying herself, and her sounds of pleasure cut off as she rams your ' + Descriptors.cockDescript( x ) + ' deep into her throat.  She gurgles noisily while her tongue slides as far down your shaft as possible, licking at your ', false );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( Descriptors.ballsDescriptLight(), false );
		} else if( CoC.player.hasVagina() ) {
			MainView.outputText( Descriptors.vaginaDescript( 0 ), false );
		...
		MainView.outputText( ' while her spittle froths around her lips.  She manages to start humming deep within her throat, vibrating your ' + Descriptors.cockDescript( x ) + '.  The obstruction in your mouth stifles your moan, but Urta hears the startled gurgle and slides away, then plunges back down, humming the entire time in between gasps for air.\n\n', false );
		MainView.outputText( 'Not to be outdone, you pull back for a quick gasp of air before ramming her back inside your mouth.  You push forwards, taking her to the back of your throat', false );
		if( CoC.player.cor < 60 ) {
			MainView.outputText( ' until your gag reflex forces you back.  Not being able to deep-throat her, you shift your weight onto your right arm and grab her just below her medial ring with your left.  You start vigorously jacking her off, running your tongue around her flare and over her sensitive tip, even darting it into her urethra to get at her sweet-tasting pre-cum.', false );
		} else {
			MainView.outputText( '.  You easily suppress your gag reflex and push forwards, feeling her cock arch slightly in the back of your mouth as it curves into your throat.  You extend your tongue as far past your lips as you can, licking down the shaft as you cram her further and further into your gullet.  Though you can\'t taste it, you can feel her urethra sending slow pulses of pre-cum inside of you, depositing it directly into your belly as you stuff your nose into her balls and tongue the skin around her sheath.', false );
		}
		MainView.outputText( '\n\n', false );
		MainView.outputText( 'If someone walked in on the both of you now they\'d see a breathless fox and you, each with the other\'s cock rammed into their face.  They\'d see each of you blushing bright and trembling, on the verge of orgasm and barely holding on as the other does their best to bring about an orgasm.  Wet slurps and panicked gasps fill the air while bodies twist and squirm together on the fur-covered bed.\n\n', false );
		MainView.outputText( 'You cum first, a victim of Urta\'s talented tongue and humming technique.  You\'d cry out with pleasure were you not so thoroughly stuffed, and the fox slams her face into your groin after the first blast, pushing you into her throat and noisily beginning to gulp down your spooge.  ', false );
		if( CoC.player.cumQ() >= 500 ) {
			MainView.outputText( 'She gurgles as she struggles to keep up with the flow, actually letting a bubbly white froth start to leak down the sides of her mouth.  ', false );
			if( CoC.player.cumQ() >= 1000 ) {
				MainView.outputText( 'It begins dripping from her nostrils as your explosive load overwhelms her ability to swallow spunk.  ', false );
			}
		}
		if( CoC.player.cumQ() >= 750 && CoC.player.cumQ() < 1500 ) {
			MainView.outputText( 'Her belly rounds out a little from all the cum as your orgasm begins to wind down.  ', false );
		}
		if( CoC.player.cumQ() >= 1500 ) {
			MainView.outputText( 'Her belly rounds out a little before she pulls back and you spurt all over her, soaking her face and head with more spooge than she could ever handle.  ', false );
		}
		MainView.outputText( 'Sated, you sigh, though Urta pushes back onto you and intently licks every ounce of jism from your body.\n\n', false );
		MainView.outputText( 'With only a few moments to recover from your own orgasm, you feel the fox stiffening, her apple-sized balls pulling up tightly against her body as she starts to orgasm.  A cock-bulging load of thick fox-cream forces your mouth wider before exploding ', false );
		if( CoC.player.cor < 60 ) {
			MainView.outputText( 'in your mouth, forcing you to compulsively swallow if you don\'t want to drown in it.', false );
		} else {
			MainView.outputText( 'down your throat, directly injecting it into your belly.', false );
		}
		MainView.outputText( '  More and more of the salty cream is forced into you, leaving you feeling full and bloated, with a salty aftertaste in your mouth.  The fox-girl\'s horse-cock does eventually stop cumming, though rather than stopping all at once it slows to a trickle over the course of a minute or two, and you slurp it down until it softens in your mouth.\n\n', false );
		...
		MainView.outputText( 'Urta blows you another kiss and says, "<i>Gotta go!  Be good, and don\'t do anyone I wouldn\'t, OK?  Love you!</i>"', false );
		CoC.player.orgasm();
		EngineCore.dynStats( 'sen', -1 );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	Urta.prototype.urtaHomeLuvLuvinsMenu = function() {
		MainView.clearOutput();
		this.urtaSprite();
		MainView.outputText( 'She caresses a nipple and visibly shivers as she says, "<i>You teased me the whole way here, and I\'m COMPLETELY ready for you, any way you could want.</i>"\n\n', false );
		MainView.outputText( 'She blushes and continues, "<i>So, what\'ll it be, lover?  A little injection of fox-cream or something else?</i>"', false );
		MainView.outputText( '\n\nHow do you want to fuck with the vixen?' );
		MainView.menu();
		if( CoC.player.hasVagina() ) {
			MainView.outputText( '  She could ride your vagina.' );
			EngineCore.addButton( 0, 'Ride Vag', this, this.rideUrtasCoochLoveyDovey );
		}
		MainView.outputText( '  She could fuck your ass.' );
		EngineCore.addButton( 1, 'Ride Ass', this, this.rideUrtaInButtAtHomeLove );
		if( CoC.player.hasCock() ) {
			MainView.outputText( '  Urta could 69 with you, let you fuck her pussy, or you could even fuck her cunt full of jizz and then lick her to a second cum.' );
		...
		if( CoC.player.gender > 0 ) {
			MainView.outputText( '  There\'s always the option to ask her to go condomless for a bit of extra fun.' );
	...
	Urta.prototype.urtasCoochiNeedsFuckedLove = function() {
		...
		MainView.outputText( '', true );
		MainView.outputText( ImageManager.showImage( 'urta-fills-her-condom' ), false );
		MainView.outputText( 'Urta gives you a knowing look as she spreads her legs and lifts her bulging sack to expose the glistening black lips of her sex.  She asks with a sultry expression, "<i>Is that what you need, lover, a slick, wet cock-sleeve to sheath yourself in?</i>"\n\n', false );
		MainView.outputText( 'You nod eagerly and advance, dropping your ' + CoC.player.armorName + ' by the wayside as you jump into bed.  She reaches around her male endowments, stroking along her lips to her puffy black clit.  Urta shivers in pleasure, spreading her legs as far as she possibly can, watching you with a hungry look as she licks her lips.  A few long strands of cunny-juice leak from her waiting hole onto the bed, practically begging you to slide inside and feel her nether-lips\' velvet embrace.  You sidle up to your lover, pressing your ', false );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 'main ', false );
		}
		MainView.outputText( Descriptors.multiCockDescriptLight() + ' at the moist opening, shivering at the warmth and desire as it wets your tip' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( '.\n\n', false );
		if( CoC.player.cockArea( x ) > this.urtaCapacity() ) {
			MainView.outputText( 'Urta whines a little as you press forward and scooches backwards, "<i>I\'m sorry ' + CoC.player.mf( 'stud', 'baby' ) + ', you\'re just too big for my poor little pussy to take.  Maybe we could do something else?</i>"\n\n', false );
			EngineCore.doNext( this, this.urtaHomeLuvLuvinsMenu );
			return;
		}
		MainView.outputText( ImageManager.showImage( 'urta-home-male-fuck' ), false );
		MainView.outputText( 'You push forwards, slowly sliding into the moist tunnel, feeling its warmth covering every inch of your ' + Descriptors.cockDescript( x ) + '. It caresses and squeezes you, surrounding you in perfect pleasure.  Urta stretches out languidly, her engorged horse-cock flopping down between her sizable breasts as it reaches a twitching, trembling hardness.  You bottom out, feeling the ', false );
		if( CoC.player.hasSheath() ) {
			MainView.outputText( 'sensitive skin of your sheath rubbing her exterior pussy-lips', false );
		} else {
			MainView.outputText( 'wetness of her pussy rubbing against your groin', false );
		}
		MainView.outputText( '. Urta moans, "<i>By Marae, you make my cunt feel good, lover.  I\'m getting so wet... fuuuck, I\'m going to need to change my furs and sheets again.</i>"\n\n', false );
		MainView.outputText( 'Unthinking, your ' + Descriptors.hipDescript() + ' begin to rock back and forth, sawing in and out of the fox\'s exotic pussy.  She wriggles and moans happily, losing her grip on her weighty balls and letting them slide over the top-side of your pussy-slicked ' + Appearance.cockNoun( CockTypesEnum.HUMAN ) + ' with each movement.  You ignore it, increasing the tempo and watching as your lover\'s jiggling breasts shift and bounce around her flared penis, eliciting uncontrolled gasps of pleasure and causing pre to form a bubble at the tip of her latex cock-sheath.  Every thrust is met by a visible bulging of Urta\'s urethra and an increase in the size of the balloon hanging from her dick\'s flare.\n\n', false );
		MainView.outputText( 'Wet sounding \'schliiiicks\' and noisy slaps fill the air as the sex takes on a violent, furious pace.  You slam yourself into the fox\'s hole, marveling at how nicely it accommodates and squeezes you.  The walls clench down on you from time to time, usually accompanied by a lewd moan as Urta\'s muscles involuntarily contract with pleasure.  She grabs her tits with both hands and squeezes them around her shaft, tit-fucking herself in perfect rhythm with your brutal thrusts.  Urta\'s condom starts filling faster – it\'s nearly the size of an apple now – and she\'s openly moaning and panting, on the verge of orgasm.\n\n', false );
		MainView.outputText( 'Your ' + Descriptors.cockDescript( 0 ) + ' is on the verge as well, ready to blow after fucking the dirty fox-girl so hard.  She cums first, shrieking in pleasure and throwing back her head, her hands clamping her jiggling breasts tightly around the rippling flesh of her massive horse-member as it begins unloading.  The surface of Urta\'s condom distorts as she fires a massive jet of her stuff into it, nearly doubling the size of the balloon.  Her hips wriggle and her pussy contracts around you, rippling along your length as the lusty fox\'s girl-cum squirts over your ', false );
		if( CoC.player.hasSheath() ) {
			MainView.outputText( 'sheath', false );
		...
		MainView.outputText( '.  You grab her ass and slam in a final time, reaching your own peak.\n\n', false );
		//[DAWG];
		if( CoC.player.hasKnot( x ) ) {
			MainView.outputText( 'Your knot flares wide, locking itself inside her convulsing cunt as your seed begins pumping into her womb.  Urta cries, "<i>oooooooOOOOHHhhh YESSSS... You\'ve plugged my cunt with your doggie dick!  It\'s squirting and pumping into my wooooomb...</i>"\n\n', false );
			MainView.outputText( 'You rock back and forth, unable to pull out yet stimulating the sensitive bulb of your knot.  Your ' + Descriptors.cockDescript( x ) + ' dumps its gooey cargo directly into your lover\'s stopped-up sex with barely audible squishing noises.  ', false );
			if( CoC.player.cumQ() < 250 ) {
			} else if( CoC.player.cumQ() < 700 ) {
				MainView.outputText( 'Her belly puffs up slightly by the time you finish, thanks to your body\'s copious cum production.  ', false );
			} else if( CoC.player.cumQ() < 1000 ) {
				MainView.outputText( 'Her belly swells larger and larger as you continue to pump your seemingly endless seed into her.  Urta groans, looking a few months pregnant by the time your overproductive ' + Appearance.cockNoun( CockTypesEnum.HUMAN ) + ' stops spewing inside her.  ', false );
			} else {
				MainView.outputText( 'Her belly swells larger and larger, bloating as you blast thick torrents of seed inside her, filling her completely thanks to the obstruction of your knot.  ', false );
				if( CoC.player.cumQ() < 3000 ) {
					MainView.outputText( 'By the time you finish, she looks VERY pregnant and is moaning with a mixture of pleasure and discomfort.  ', false );
				} else {
					MainView.outputText( 'Your seemingly never-ending orgasm pumps her up until she looks on the verge of giving birth, and you just keep going.  The pressure builds and Urta moans in discomfort and pleasure.  Eventually the pressure is too much, and your cum begins spraying out around your knot, soaking the pair of you as you both continue to cum hard.  ', false );
					if( CoC.player.cumQ() > 5000 ) {
						MainView.outputText( 'Not content with such an easy release, your ' + Descriptors.cockDescript( x ) + ' unloads the last of its cream until the bed and floor are both covered under inches of your canine cum.  ', false );
					}
				}
			}
			MainView.outputText( 'You finally feel your ' + Appearance.cockNoun( CockTypesEnum.HUMAN ) + '\'s knot shrinking and pull free with a loud and messy pop.  ', false );
			if( CoC.player.cumQ() >= 2000 ) {
				MainView.outputText( 'Spunk rushes out in a torrent, splattering everywhere.  ', false );
			}
			MainView.outputText( 'Urta\'s glazed pussy-lips clench reflexively at their sudden emptiness and you hear a very satisfied sigh from the far end of the bed.\n\n', false );
		}
		//[HORSE];
		else if( CoC.player.cocks[ x ].cockType === CockTypesEnum.HORSE ) {
			MainView.outputText( 'You feel your flare filling with blood as it ', false );
			if( CoC.player.cocks[ x ].cockLength >= 10 ) {
				MainView.outputText( 'prods against Urta\'s cervix.', false );
			} else {
				MainView.outputText( 'seats itself deep inside Urta\'s passage.', false );
			}
			MainView.outputText( '  Your crotch muscles clench in an unconscious rhythm as orgasm wracks your body.  You begin spurting hard inside her, splattering it into her cervix, even allowing it to flow into her womb.  Urta cries, "<i>ooooOOOHHHH YESSS... is this what you feel when I cum inside you?  YesYESYES- Ohhh I can feel the flare stroking and flexing inside me...</i>"\n\n', false );
			MainView.outputText( 'You rock back forth, unable to resist dragging the sensitive, flared tip and textured ring of your cock all along her pussy-walls, squirting seed with reckless abandon into her hot, cum-slicked cunt.  Wet squishing noises fill the apartment as the two of you rock the bed in twinned orgasms.  ', false );
			if( CoC.player.cumQ() < 250 ) {
			} else if( CoC.player.cumQ() < 700 ) {
				MainView.outputText( 'Her belly slowly rises as more and more cum slides into her womb, filling it.  Runners of the stuff leak down her thighs as it escapes around your bestial cock.  ', false );
			} else if( CoC.player.cumQ() < 1000 ) {
				MainView.outputText( 'Her belly swells larger and larger as you continue to pump your seemingly endless seed into her.  Urta groans, looking a few months pregnant by the time your over-productive ' + Appearance.cockNoun( CockTypesEnum.HUMAN ) + ' has finished spewing your seed inside her.  ', false );
			} else {
				MainView.outputText( 'Her belly swells larger and larger, bloating out as you blast thick torrents of seed inside her, stuffing her completely.  Every squirt of the stuff is met with a leaking back-blast that soaks her thighs and bed in your jism.  ', false );
				if( CoC.player.cumQ() < 3000 ) {
					MainView.outputText( 'By the time you\'ve finished, she looks heavily pregnant and cum soaks the bed.  ', false );
				} else {
					MainView.outputText( 'Your seemingly endless orgasm pumps more and more into her until she looks ready to give birth, and  you just keep going.  She moans in pleasure and discomfort as each blast of your horse-cum forces thick runners to squirt from between her thighs.  ', false );
				}
				if( CoC.player.cumQ() > 5000 ) {
					MainView.outputText( 'Not content with such an easy release, your ' + Descriptors.cockDescript( x ) + ' unloads the last of its cream until the bed and floor are covered in several inches of thick equine cum.  ', false );
				}
			}
			MainView.outputText( 'You finally feel your flare returning to normal and pull out with a prolonged \'sluuuuurp\'.  ', false );
			if( CoC.player.cumQ() >= 2000 ) {
				MainView.outputText( 'Spunk rushes out in a torrent, splattering everywhere.  ', false );
			}
			MainView.outputText( 'Urta\'s glazed pussy-lips clench reflexively at their sudden emptiness and you hear a very satisfied sigh from the far end of the bed.\n\n', false );
		...
		//[Everybody Else];
			...
			MainView.outputText( 'Your ' + Descriptors.cockDescript( 0 ) + ' clenches, filling her convulsing cunt as your seed begins pumping into her womb.  Urta cries, "<i>oooooooOOOOHHhhh YESSSS... It\'s so warm and wet.  Yes, lover!  Keep goo-oh-OH-innnngg!!</i>"\n\n', false );
			MainView.outputText( 'You rock back and forth, sliding your shaft through her spasming muscles, reveling and panting in orgiastic bliss.  Your ' + Descriptors.cockDescript( 0 ) + ' dumps its gooey cargo deep into your lover\'s canal with barely audible squishing noises.  ', false );
			if( CoC.player.cumQ() < 250 ) {
			} else if( CoC.player.cumQ() < 700 ) {
				MainView.outputText( 'Her belly puffs up slightly by the time you finish, thanks to your body\'s copious cum production.  ', false );
			} else if( CoC.player.cumQ() < 1000 ) {
				...
				MainView.outputText( 'Her belly swells larger and larger, bloating as you blast thick torrents of seed inside her, filling her completely while leftover jism squirts out around your ' + Descriptors.cockDescript( 0 ) + '.  ', false );
				if( CoC.player.cumQ() < 3000 ) {
					MainView.outputText( 'By the time you finish, she looks VERY pregnant and is moaning with a mixture of pleasure and discomfort.  ', false );
				} else {
					MainView.outputText( 'Your seemingly never-ending orgasm pumps her up until she looks on the verge of giving birth, and you just keep going.  Jism sprays out from her dripping cunt as you release a torrent of semen, splattering you, her, and the bed.  ', false );
				}
				if( CoC.player.cumQ() > 5000 ) {
					MainView.outputText( 'Not content with such an easy release, your ' + Descriptors.cockDescript( 0 ) + ' unloads the last of its cream until the bed and floor are both covered under inches of your gooey cum.  ', false );
				}
			}
			MainView.outputText( 'You finally feel your ' + Appearance.cockNoun( CockTypesEnum.HUMAN ) + ' deflating, and you pull free with a messy slurp.  ', false );
			if( CoC.player.cumQ() >= 2000 ) {
				MainView.outputText( 'Spunk rushes out in a torrent, splattering everywhere.  ', false );
			}
			MainView.outputText( 'Urta\'s glazed pussy-lips clench reflexively at their sudden emptiness and you hear a very satisfied sigh from the far end of the bed.\n\n', false );
		...
		//[RESUME];
		MainView.outputText( 'Urta calms down at last, though her member is still throbbing weakly as she peels her condom off.  The bloated cum-bubble at the tip is the size of a basketball, and she struggles to get it tied off and disposed of.  The fox tosses it into a waste-basket already full of the things and gives you a toothy smile, "<i>I think somebody\'s ' + Appearance.cockNoun( CockTypesEnum.HUMAN ) + ' loves my pussy almost as much as I love you!</i>"\n\n', false );
		MainView.outputText( 'She pulls you into her arms and gives you a long, wet kiss before breaking away to clean up.  Urta blows you a kiss and says, "<i>Thanks again, love!</i>"\n\n', false );
		MainView.outputText( 'You and Urta go your separate ways after that, both stinking of sex and with a sense of deep satisfaction.', false );
		this.knockUpUrtaChance();
		CoC.player.orgasm();
		EngineCore.dynStats( 'sen', -1 );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	...
	//[Back Room];
	Urta.prototype.scyllaAndUrtaSittingInATree = function() {
		this.urtaSprite();
		MainView.outputText( '', true );
		//Increment 'times caught with Scylla';
		CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00143 ]++;
		...
		MainView.menu();
		if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00147 ] === 0 && CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00145 ] > 0 ) {
			MainView.outputText( 'You meander towards the back-rooms and a canine waitress springs forward with a key clutched in her paw.  She blushes furiously and explains that she was tipped generously to hand it to you if you arrived.  You accept the proffered tool and smile.  It must be cheaper to pay someone to wait with a key than to fix the door each time the three of you meet up!  You snicker to yourself as you plunge the key into the lock\'s waiting receptacle, turn it, and push your way inside.  The scene in front of you is as arousing as it is familiar.\n\n', false );
			MainView.outputText( 'Urta is sitting down on a crate with her legs splayed and her torso leaning drunkenly in your direction.  Her tongue is hanging out and her eyes are lidded from alcohol as she notices you and gives another high-pitched whine, not of pain, but pleasure.  The source of her \'distress\' is a black-robed figure with curly black locks of hair that hang around her head like a halo and thick, glossy red lips.  Those same lips are currently wrapped firmly around Urta\'s twitching, lust-swollen horse-cock just above her sheath.  You can clearly see the slight bulge of Urta\'s medial ring highlighted through the corrupted nun\'s neck as she bobs up and down, so absorbed in feeding her addiction that she failed to notice your entrance.\n\n', false );
			MainView.outputText( 'The fox is giving you a lewd look as she cradles Scylla\'s head in her hands and pushes down.  She slurs, "<i>Oooooh YEAH!  Mmmmm, you made it ' + CoC.player.short + '!  It seems like each time Scylla does... does... oooh... this I cum bigger and harder.  MMmmmm so good... wanna watch?</i>"\n\n', false );
			MainView.outputText( 'Throughout the conversation she\'s weakly twitching her pelvis, rocking her hips against Scylla\'s face while her balls start to swell and churn in the nun\'s hands.  You close the door behind you carefully and drop the key on a crate.  It\'s time for some fun.\n\n', false );
			// Butans;
			if( CoC.player.cockThatFits( this.urtaCapacity() ) !== -1 && CoC.player.lowerBody !== AppearanceDefs.LOWER_BODY_TYPE_CENTAUR ) {
				...
				MainView.outputText( '(You could make Urta sit on YOUR lap for a change, or you could jerk off and watch.' );
			} else {
				MainView.outputText( '(You could jerk off and watch.' );
			}
			...
			MainView.outputText( '  Bonding with the use of Scylla\'s lipples is also an option.)' );
		} else {
			MainView.outputText( 'You meander by the Wet Bitch\'s back rooms, trying not to look like a creeper as you listen for ', false );
			if( !this.urtaLove() ) {
				MainView.outputText( 'Urta', false );
			} else {
				MainView.outputText( 'your love', false );
			}
			MainView.outputText( '\'s voice.  The first few doors are silent and the next one sounds to have a spirited game of strip-poker going on inside it.  Judging by the grunts and occasional mewling moan of pleasure, some poor girl has already lost.  One of the barmaids gives you a warning glance and you keep moving, sure to avoid the lewd game of chance as you hunt for the stray, well-endowed fox.\n\n', false );
			MainView.outputText( 'At first, the following entryway is silent, though you can see light underneath.  You\'re just about to move on from the portal when you hear Urta\'s voice give out a strained grunt.  Did some ', false );
			if( CoC.player.cor < 40 ) {
				MainView.outputText( 'corrupted ruffians', false );
			} else {
				MainView.outputText( 'demon agents', false );
			}
			MainView.outputText( ' somehow get the drop on her?  She whines, high-pitched and plaintive, almost as if she was wounded, and you cannot wait a moment later.  You lunge to the door, twist the handle, and find it locked.  Without a second thought, you throw your weight into the flimsy wood paneling and crash through to the last thing you expected to see!\n\n', false );
			MainView.outputText( 'Urta\'s sitting down on a crate with her legs akimbo and her torso leaning drunkenly in your direction.  Her tongue is hanging out and her eyes are lidded from alcohol as she notices you and gives another high-pitched whine, not of pain, but pleasure.  The source of her \'distress\' is a black-robed figure with curly black locks of hair that hang around her head like a halo and thick, glossy red lips.  Those same lips are currently wrapped firmly around Urta\'s twitching, lust-swollen horse-cock just above her sheath.  You can clearly see the slight bulge of Urta\'s medial ring highlighted through the corrupted nun\'s neck as she bobs up and down, so absorbed in feeding her addiction that she failed to notice you bursting through the door.\n\n', false );
			MainView.outputText( 'The fox is ', false );
			if( this.urtaLove() ) {
				MainView.outputText( 'weakly protesting', false );
			} else if( CoC.flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] >= 11 ) {
				MainView.outputText( 'giving you a lewd look', false );
			} else {
				MainView.outputText( 'blushing hard', false );
			}
			MainView.outputText( ' as she cradles Scylla\'s head in her hands and pushes down.  She slurs, "<i>', false );
			if( this.urtaLove() ) {
				MainView.outputText( 'I\'m... ahhhh... s-sorry love, but we both know she\'s been suuuu - oh yeah, keep licking - sucking you off before, and the hungry slut just couldn\'t take her eyes off my cock.  Why didn\'t you tell me ho - OH YES!  RIGHT THERE! - h-how good she was?', false );
			} else if( CoC.flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] >= 11 ) {
				MainView.outputText( 'Ohhh, you showed up a little late... mmmhmm yes, now lick... Oh, why didn\'t you tell me ho - OH YES!  RIGHT THERE! - h-how good of a cock-sucker this lewd-bodied bitch was?', false );
			} else {
				MainView.outputText( 'H-hi ' + CoC.player.short + '... I didn\'t think I\'d get to see you to- Oh yeah, like that!  Mmmm... OH!  I guess since we\'ve been intimate before you can watch...', false );
			}
			MainView.outputText( '</i>"\n\n', false );
			MainView.outputText( 'Throughout the conversation she\'s weakly twitching her pelvis, rocking her hips against Scylla\'s face while her balls start to swell and churn in the nun\'s hands.  Thankfully the doorway is at such an angle that the bulk of the bar can\'t see inside, and it helps that no one seems inclined to bother the three of you for now.  What do you do?\n\n', false );
			// Butans;
			if( CoC.player.cockThatFits( this.urtaCapacity() ) !== -1 && CoC.player.lowerBody !== AppearanceDefs.LOWER_BODY_TYPE_CENTAUR ) {
				EngineCore.addButton( 0, 'Lap', this, this.makeUrtaSitOnYourLapWithScylla );
				MainView.outputText( '(You could make Urta sit on YOUR lap for a change, or you could jerk off and watch.' );
			...
			EngineCore.addButton( 1, 'Jerk', this, this.watchTwoHotBitchesAndJerkIt );
			EngineCore.addButton( 2, 'LippleBond', this, this.lippleBondUrtaScylla );
			MainView.outputText( '  You could throw them the finger and leave.  Scylla also has two pairs of lips unoccupied - playing with them could lead to something interesting.  You could tell Urta you never want to see her again.)' );
	...
	Urta.prototype.urtaAndScyllaBoningLikeBitchesSober = function() {
		MainView.clearOutput();
		...
		if( !this.urtaLove() ) {
			MainView.outputText( 'fuck-buddy' );
		} else {
			MainView.outputText( 'lover' );
		}
		MainView.outputText( '\'s lusty slip-up and skittish responses, pinching her booty for good measure while you decide just how to get involved in the potential <i>ménage à trois</i>.\n\nWhat do you do?' );
		EngineCore.dynStats( 'lus', 10 + CoC.player.lib / 5, 'resisted', false );
		...
		if( CoC.player.cockThatFits( this.urtaCapacity() ) >= 0 ) {
			...
			MainView.outputText( '  You could <b>fuck the fox</b> while she gets her oral pleasure.' );
		}
		if( CoC.player.biggestCockArea() > this.urtaCapacity() ) {
			worship = this.dockWithUrtaUnderScyllasSoberSupervision;
			MainView.outputText( '  You could whip out your gigantic cock and let the blowjob queen <b>worship</b> a REAL dick.' );
		}
		MainView.outputText( '  Whatever you do, with Scylla involved, it\'s sure to get out of hand.' );
	...
	Urta.prototype.fuckUrtaWhileScyllaSucksSober = function() {
		MainView.clearOutput();
		var x = CoC.player.cockThatFits( this.urtaCapacity() );
		MainView.outputText( 'You peel out of your straining equipment to free your ' + Descriptors.multiCockDescriptLight() );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( ' and [balls]' );
		}
		MainView.outputText( ', hauling out your sweaty, engorged meat.  Scylla, absorbed as she is with cum-thirst, doesn\'t seem to notice, though her nose twitches as the scent of your musky aroma mingles with the sex-stained air.  You gently squeeze behind the fox, folding her bushy, soft tail to the side and giving you something firm to support yourself when you let her flop back into you.  Her smooth fur feels wonderful on your [chest] and she squirms and gyrates against Scylla\'s sucking mouth.  Fairly dripping with moisture, the fox\'s black-lipped sex grinds on you, resting atop [oneCock], and your length is soon soaked with her heady, animalistic fem-spunk.' );
		MainView.outputText( '\n\nUrta pants, "<i>Go on... nnn... j-just put it in already!</i>"  You readily oblige the over-endowed beast-woman by guiding your length into the clutching vice of her nethers, shivering at the onslaught of electric bliss that rockets up your ' + Descriptors.cockDescript( x ) + '.  You hilt inside her' );
		if( CoC.player.cockArea( x ) >= 40 ) {
			MainView.outputText( ', clearly bulging her stomach with your sizeable cock' );
		}
		MainView.outputText( ', sliding up into the slick, welcoming passage as if you always belonged.  Rivulets of liquid desire trickle from the stretched lips to run down ' );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( 'the front of your [sack]' );
		} else {
			MainView.outputText( 'your [legs]' );
		}
		MainView.outputText( '.  The gray-furred femme reaches back in an awkward hug, trying to hold you against her, speared together in the most intimate of embraces.' );
		...
		if( EngineCore.silly() ) {
			MainView.outputText( 'a cock and a sharp place.' );
		} else {
			MainView.outputText( 'your lusty bodies.' );
		}
		MainView.outputText( '  The velvet glove clasping your ' + Descriptors.cockDescript( x ) + ' tightens without warning, accompanied by a full-body quake.  Urta\'s eyes roll back as some invisible force works through her, culminating in a muscular clench that you feel will pop your ' + Descriptors.cockDescript( x ) + ' clean off your body.  You grunt, grabbing hold of the fox\'s tits for stability and wince from the almost painful tightness of her snatch.  The moment seems to drag on forever, but then, the vixen slumps back into your arms, allowing you to support her sagging weight.' );
		MainView.outputText( '\n\nBeyond your sight, the fox-girl\'s soft-furred sack begins to round out, growing larger in spite of the sharp tip prodding hard at her balls. The delicate, plush nuts swell bigger, big enough to pull the skin around them tight, big enough to pull Urta harder onto your ' + Descriptors.cockDescript( x ) + ', big enough to be painfully poked, on the verge of being pierced.  Urta\'s vocalizations take on a keening, banshee-like quality, but before any harm can be done, a rosy glow envelops the fox-herm\'s poor, brutalized sack.  Stranger still, the nun\'s ram-like protrusions shorten in response, actually withdrawing back into her skull, pushed back by your lover\'s impressive, seemingly magical testes.' );
		...
		MainView.outputText( '\n\nMoaning, Urta whimpers, "<i>S-so full... I - ungh - I... I can\'t hold it!  C-cuuuummmiiiiinnng!</i>"  Her hips jerk forward hard and fast enough that it nearly pulls the gushing pussy off your ' + Descriptors.cockDescript( x ) + ', but Scylla isn\'t so lucky.  Her nose is squished almost flat into the fox\'s sweaty middle-fur, chin deep in ballsack.  She doesn\'t seem to mind; in fact, when drizzles of frothing fluid appear at the crinkled corners of the nun\'s ruby lips, her eyes roll back, exposing the whites.  Meanwhile, Urta\'s less-than-colt-sized dong swells, but not as a return to its former glory.  No, if anything, her musky dick is still shrinking.  The sheer size of the cum-globs pumping through her urethra are visibly stretching her horse-cock, giving the illusion of increased size.' );
		...
		if( CoC.player.balls > 0 ) {
			MainView.outputText( 'r [balls]' );
		}
		MainView.outputText( ' give the warm snatch a dose of virile medicine.  Urta, perhaps stirred from her own orgasm by the jets of hot fluid flooding her nethers, reaches back to squeeze your butt and whimpers, "<i>Give it to me.</i>"' );
		...
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( '  The unbound portion of your package bobs in the air before coming to rest in Urta\'s ass-crack.  A second later, the vulpine butt is coated with sticky white icing.' );
		}
		if( CoC.player.lactationQ() > 50 ) {
			MainView.outputText( '  Your sensitive nipples let down, releasing their own heady flows to compliment your climactic pleasure, spraying into the gray fur relentlessly.' );
		}
		MainView.outputText( '  Throughout it all, your ' + Descriptors.cockDescript( x ) + ' continues to do its level best to fill the fox.  Fireworks of uncontrollable pleasure go off one after the other, alternatively numbing and locking your muscles as you release all of your pent-up lusts, desires, and needs into the welcoming cunt.' );
		if( CoC.player.cumQ() >= 500 ) {
			MainView.outputText( '  Urta\'s belly bloats from the thick deposit' );
			if( CoC.player.cumQ() >= 2000 ) {
				MainView.outputText( ', rounding more and more, a taut cum-tank that jiggles with your thick seed.' );
			} else if( CoC.player.cumQ() >= 1200 ) {
				MainView.outputText( ', rounding out into a swollen cum-bank that jiggles under the weight of your seed.' );
			} else if( CoC.player.cumQ() >= 750 ) {
				MainView.outputText( ', rounding out into a gravid bump that jiggles with liquid weight.' );
			} else {
				MainView.outputText( ', rounding out into a nicely-sized bump.' );
			}
			MainView.outputText( '  You run your hands across it as you give a last few pumps.' );
		}
		...
		} else if( CoC.player.tallness >= 48 ) {
			MainView.outputText( 'narrow middle' );
		} else {
			MainView.outputText( 'curvy hips' );
		}
		MainView.outputText( ' you watch the nun\'s throat bulge almost double wide with each swallow of seed.  Tremendous volleys of semen are pumping down her esophagus rapidfire, and yet the suckling woman barely shows any sign of being full.  A clear puddle has sprung up underfoot, centered below the cock-sucker\'s crotch, and you marvel at the evidence of Scylla\'s pleasure.  She must be cumming non-stop!' );
		...
		if( this.urtaLove() ) {
			MainView.outputText( '  You give your lover a wet kiss and a fondle before you go, snickering when it makes her even longer.' );
		} else {
			MainView.outputText( '  You give her a quick hug and depart, smiling when you realize the brief touch made her even longer.' );
		}
		this.knockUpUrtaChance();
		//-2 sens, -100 lust;
		CoC.player.orgasm();
	...
	Urta.prototype.dockWithUrtaUnderScyllasSoberSupervision = function() {
		...
		MainView.outputText( '\n\nScylla gestures at your ' + Descriptors.cockDescript( x ) + ' and commands, "<i>Go on, fuck it.  You know you want to.</i>"  The black restraints poof into phantasmal smoke.  At the same time, your lusty hermaphrodite ' );
		if( !this.urtaLove() ) {
			MainView.outputText( 'friend' );
		} else {
			MainView.outputText( 'lover' );
		}
		MainView.outputText( ' lunges forward, scattering the smokey haze in the split second it takes her to slam face-first into your dick.  A flat-tipped spear plunges through the goop plugging your urethra and into your cock, scattering streamings of stringy sexual juice in a fan around it.  To you, the penetration isn\'t painful in the slightest.  Your ' + Descriptors.cockDescript( x ) + ' is so big that the fox\'s own thick prick is a nice fit, and it feels like the slippery shaft is stroking you from the inside out, massaging nerve endings you didn\'t even know you had.' );
		MainView.outputText( '\n\nUrta grabs around the edges of your ' + CoC.player.cockHead( x ) + ' and pulls herself down, her fuzzy balls slapping wetly against the underside of your boner before pulling away again.  She starts kissing the top of your ' + Descriptors.cockDescript( x ) + ' with wild, unrestrained enjoyment.  Moving with the subtlety of a jackhammer, she pistons your inner hole wildly, grunting and moaning as she throws herself completely into debauchery.  The reluctant captain is fucking your immense member with abandon and shows no signs of stopping.  Scylla watches with detached amusement and idly climbs atop the column of jiggling dickmeat.  Her heavy udders press down on either side of your cock, but all she does is prop her face up on her hand and watch Urta fuck you hard enough to splatter pre-cum on everything in a five foot radius.  Thankfully, the movements cause your cock to flex and shift enough that the nun\'s inertia-bound breasts feel as if they\'re bouncing along the sides of your boner, unintentionally giving you a tit-fuck while your cock-pussy is inelegantly plowed.' );
		MainView.outputText( '\n\nUrta\'s apple-sized nuts pull tight against her body as she works your urethra, and somehow, she remembers that she ought to warn you.  "<i>Can\'t stop!  G-g-gonna cuuuUUUUUM!!!</i>" she screams, slamming her dick down with enough force to distort your ' + CoC.player.cockHead( x ) + '.  A moment later, you feel her swollen sack drag against your underside, and a warm explosion blooms inside your member.  The rushing white river runs through your length and into your body, giving you the oddest sensation of pleasant fullness and complete violation.' );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( '  Your [balls] quiver a little when the rushing seed reaches them, and you swear you hear them slosh with the horse-spunk pouring into their own reservoirs.' );
		} else {
			MainView.outputText( '  Your innards gurgle slightly as the rushing seed travels through them, and you swear you can feel your prostate stretching to accommodate being suddenly stuffed with horse-spunk.' );
		}
		...
		if( CoC.player.balls > 0 ) {
			MainView.outputText( ' while your enormous balls gurgle and begin to empty' );
		}
		MainView.outputText( '.  The titanic, room-filling cock lifts up off the ground by a few inches, supported by the thick bulge of cum that rushes from ' );
		if( CoC.player.hasSheath() ) {
			MainView.outputText( 'sheath' );
		} else {
			MainView.outputText( 'base' );
		}
		MainView.outputText( ' to tip in an instant. It passes through the tight crimson seal, distending the nuns neck for a moment before it settles in her belly.  She looks almost pregnant on a mix of horse and champion-spunk for a moment, but then her belly narrows and her tits swell - just in time to take the next gush of your demonically-blessed orgasm.  Scylla appears to be in heaven, and as she swallows, the darkness leaves her eyes.  What remains is wide-eyed irresistible pleasure, and the vibrations of orgasmic screams vibrate through your swollen ' + CoC.player.cockHead( x ) + ' as you dump wave after wave of spooge through the clinging orifice.  A shiny puddle of feminine fluid widens beneath the nun\'s orgasm-wracked frame, and just once, you wish you could stick your head under her robes and watch it happen.' );
		...
		CoC.flags[ kFLAGS.URTA_SCYLLA_BIG_DICK_TIMES_DONE ]++;
	...
	Urta.prototype.dockWithUrtaUnderScyllasSoberSupervision2 = function() {
		...
		if( !this.urtaLove() ) {
			MainView.outputText( 'waves ' );
		} else {
			MainView.outputText( 'kisses you ' );
		}
		MainView.outputText( 'goodbye, she says, "<i>Thank you.</i>"' );
	...
	Urta.prototype.flipUrtaTheBird = function() {
		this.urtaSprite();
		MainView.outputText( '', true );
		MainView.outputText( 'You curl your hand into a fist and extend your middle finger at the alcoholic and cum-slut.  You\'ve got better things to do than waste your time watching those two indulge their addictions.\n\n', false );
		...
		if( CoC.flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] < 0 ) {
			CoC.flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] = 0;
		}
	...
	Urta.prototype.watchTwoHotBitchesAndJerkIt = function() {
		this.urtaSprite();
		MainView.outputText( '', true );
		MainView.outputText( ImageManager.showImage( 'urta-scylla-masti' ), false );
		MainView.outputText( 'You open your ' + CoC.player.armorName + ' and sit down on a crate no more than a few feet away from the orally fixated couple, fishing out ' + Descriptors.sMultiCockDesc(), false );
		if( CoC.player.hasVagina() ) {
			MainView.outputText( ' and exposing your ' + Descriptors.vaginaDescript( 0 ), false );
		}
		MainView.outputText( '.  The nun\'s trademark demonic stubs commence sprouting, parting her hair until they rise up at least six inches from her forehead.  Meanwhile the poor, sexually frustrated vixen\'s balls are swelling up like balloons in Scylla\'s hands, growing so large that the fox\'s sack sags around the larger woman\'s hands.  The curly-haired oral slut tries to fellate and support Urta\'s growing size at the same time, and definitely appears to be struggling with it.\n\n', false );
		MainView.outputText( 'You stroke ' + Descriptors.sMultiCockDesc(), false );
		if( CoC.player.hasVagina() ) {
			MainView.outputText( ' and touch your ' + Descriptors.vaginaDescript( 0 ), false );
		}
		MainView.outputText( ', watching the puffed-up, fire engine-red cock-suckers bob on the trembling horse-shaft.  They slide up and down, up and down, then you see the hint of a tongue sliding around the cock and darting through the sheath.  Urta moans appreciatively at the act, and she turns her head to watch you as you masturbate to the exceptional scene before you.  Her eyes meet yours while her hands curl around Scylla\'s horns and pull the nun deeper into the sheath.  The fox loses her focus, her tongue rolls out, and her balls tremble and rise tight against her body.  She\'s cumming!\n\n', false );
		MainView.outputText( 'Scylla\'s neck is pulled tight as heavy bulges of cum work their way up the vixen\'s mismatched meat-pole, through the puffy, cock-sucking lips, and down the nun\'s throat into her hungry gullet.  Though only a few seconds pass, you watch Scylla\'s remarkably elastic midsection begin to bloat with what looks like a 7-month pregnancy.  She keeps her eyes closed the entire time, just silently suctioning down each plus-sized cream deposit as it\'s spurted out.  The only sounds in the room are Urta\'s half-whined moans of pleasure and the rapid slaps of your self-stimulation.\n\n', false );
		MainView.outputText( 'The gray-furred hermaphrodite\'s balls are shrinking with each blast, contracting and pausing as her body launches each volley, but they\'ve only lost about half their enhanced size and Scylla\'s getting rounder than she has any right to be.  The nun seems ready for this though, and hums in concentration.  Her belly stops growing immediately, but each of her already prodigious milkers begins to enlarge, sloshing noisily like a barrel being filled.  You nearly push yourself past the edge at the sight of the growing melons, but hold back – you don\'t want to blow until you see just how far this goes.\n\n', false );
		MainView.outputText( 'Urta\'s hands let go of the corrupted woman\'s horns and she slumps back against the wall, but the nun just wraps her arms around the blissed-out vixen and suckles harder.  You keep watching and stroking, entranced by the sight of the spooge bubbles vanishing into Scylla\'s neck and plumping up her tits.  Urta\'s eyes are rolled back and she\'s drooling a runnel of spittle down her tit, around one of her black nipples, and down her belly, but she keeps cumming, even though her balls are nearly back to normal.  The poor fox is utterly unable to cope with the sensations running through her drunken form, but she\'s clearly enjoying it.\n\n', false );
		MainView.outputText( 'Just as Scylla\'s tits start to drag against the floorboards, she pulls back, letting Urta blow a few long ropes of thick horse-cum over her face and hair.  The nun sighs happily and begins to gather it in her fingers, licking it up while the exhausted fox\'s cock slowly deflates, dripping a trail along the floor.  Urta recovers surprisingly quickly, blinking her eyes and pulling her tongue back into her mouth with a sigh of utter, sated contentment.  You held back for so long, but watching the nun pick pearly strands of animal cum from her hair and swallow it down like a whore is just too much.\n\n', false );
		MainView.outputText( 'You pump hard on ' + Descriptors.sMultiCockDesc(), false );
		if( CoC.player.totalCocks() > 1 ) {
			MainView.outputText( ' in turn', false );
		}
		MainView.outputText( ', shooting hot ropes of semen that drape over the satisfied girls.  After so much edging, and such a hot scene, your orgasm is far more intense than any normal masturbation session.  Your whole body trembles while it spurts and squirts, making sure both of the addicts get at least a little bit of your white goo on their bodies.  ', false );
		if( CoC.player.cumQ() >= 250 ) {
			MainView.outputText( 'A particularly coherent blast of seed splatters over Urta\'s tits, glazing her gray fur and black nipples with a thick layer of dripping spunk.  ', false );
		}
		if( CoC.player.cumQ() >= 450 ) {
			MainView.outputText( 'Another huge gout takes Scylla straight in the face, completely soaking her with yet more salty mess that drools onto the curve of her pendulous, black-clad tits.  ', false );
		}
		if( CoC.player.cumQ() >= 700 ) {
			MainView.outputText( 'A few more big blasts of the stuff fire off, spotting the pair with fat, sticky blotches.  ', false );
		}
		if( CoC.player.cumQ() >= 1000 ) {
			MainView.outputText( 'Your body is so productive that it utterly drenches both participants in the stuff.  Runnels of it drip over their curves while fat droplets pool into a', false );
			if( CoC.player.cumQ() >= 2000 ) {
				MainView.outputText( 'n inches-deep', false );
			}
			MainView.outputText( ' puddle on the ground.  ', false );
		}
		MainView.outputText( 'Finally spent, you slump back and smile.\n\n', false );
		MainView.outputText( 'Scylla looks shocked, as if she\'s seeing you for the first time, but her nose twitches and she starts to shovel your seed into her mouth with greedy abandon.  Even so, her body rapidly \'digests\' the fluid intake, and the nun starts to slim up before your eyes.  Urta wobbles up onto shaky legs and stumbles into you, giving you a wet kiss that tastes faintly of your salty leavings and then passionately hugs you.  She breaks the embrace and slurs, ', false );
		if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00143 ] === 1 ) {
			MainView.outputText( '"<i>We need to do thish again!</i>"', false );
		} else {
			MainView.outputText( '"<i>You\'re the besht ' + CoC.player.mf( 'man', 'woman' ) + ' a girl could ashk for.</i>"', false );
		}
		MainView.outputText( '  Before you can answer, she gives your ass a slap and finds a water barrel to wash up in.  Scylla finishes her \'meal\' and looks up at you, her alabaster skin turning completely crimson before she too runs off to clean up.\n\n', false );
		MainView.outputText( 'You smile happily and pick up your ' + CoC.player.armorName + '.  It\'s past time you checked up on your camp.', false );
		if( this.urtaLove() ) {
			MainView.outputText( '  You give your lover\'s back a wistful grin and muse over how good her taste in women is as you leave.', false );
		}
		CoC.player.orgasm();
		//+luv;
		this.urtaLove( 2 );
	...
	Urta.prototype.makeUrtaSitOnYourLapWithScylla = function() {
		...
		var y = CoC.player.cockThatFits2( this.urtaCapacity() );
		MainView.outputText( '', true );
		MainView.outputText( ImageManager.showImage( 'urta-scylla-lapfuck' ), false );
		MainView.outputText( 'You smirk at the drunken fox and inform her in no uncertain words that you\'ll do more than watch.  She blinks at you, her inebriated mind unable to process new thoughts as you stalk forwards and sit behind her on the crate', false );
		if( CoC.player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_NAGA ) {
			MainView.outputText( ', snaking your tail around her legs', false );
		} else if( CoC.player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_GOO ) {
			MainView.outputText( ', enveloping her legs with your goo', false );
		} else {
			MainView.outputText( ', straddling your ' + CoC.player.legs() + ' around her hips', false );
		}
		MainView.outputText( '.  You grab her narrow waist and pull her up, dragging Scylla\'s cock-locked head up with her and aligning your ' + Descriptors.cockDescript( x ) + ' with her backdoor before you start to lower her back down.  ', false );
		if( y > -1 ) {
			MainView.outputText( 'Your ' + Descriptors.cockDescript( y ) + ' even manages to get in line with her drooling, black-lipped pussy.  ', false );
		}
		if( (CoC.player.cockTotal() > 1 && y === -1) || CoC.player.cockTotal() > 2 ) {
			MainView.outputText( 'You allow your remaining, unused dickflesh to flop against her back, leaking pre-cum into her fur.  ', false );
		}
		MainView.outputText( 'You aren\'t sure if it\'s the alcohol in her or her own anal talents, but she slides down without a complaint, pulling Scylla\'s vacuum-like mouth down with her', false );
		if( y !== -1 ) {
			MainView.outputText( ' and squeezing you tightly with her sopping cunt', false );
		}
		MainView.outputText( '.\n\n', false );
		MainView.outputText( 'She grunts, "<i>', false );
		if( x !== -1 && y !== -1 ) {
			MainView.outputText( 'Oooh, and in both holes?  You naughty ' + CoC.player.mf( 'boy', 'girl' ) + ', you!', false );
		} else if( this.urtaLove() ) {
			MainView.outputText( 'Oooh yeah, fill me love!', false );
		} else if( Utils.rand( 2 ) === 0 ) {
			MainView.outputText( 'Oooh, you know how to treat a lady!', false );
		} else {
			MainView.outputText( 'MMmm, that\'s nice!', false );
		}
		MainView.outputText( '</i>"  She\'s actually trying to watch you over her shoulder now, completely ignoring the cock-addled nun as the latter bobs on her horse-meat.  You make the most of it and ', false );
		if( this.urtaLove() || CoC.player.cor <= 40 ) {
			MainView.outputText( 'kiss her full on the mouth', false );
		} else {
			MainView.outputText( 'bite her neck', false );
		}
		MainView.outputText( ' while you grab her soft, furred asscheeks and lift her up, feeling the exquisite texture of her pucker ', false );
		if( y !== -1 ) {
			MainView.outputText( 'and twat ', false );
		}
		MainView.outputText( 'massaging your manhood the whole way.  Before you go too high, you drop her, and let her weight impale her back on top of you.  Her rapidly expanding balls slap into your own and smear them with her pussy juice.\n\n', false );
		MainView.outputText( 'Urta\'s tail curls snugly around your waist, and the drunken fox takes over, lifting herself up even as Scylla\'s magic swells her balls past the size of grapefruits.  She openly moans when she reaches the apex, and smoothly lowers herself back down, squeezing and stroking you with her internal muscles while trying to endure the nun\'s incessant suckling.  The measured ', false );
		if( y !== -1 ) {
			MainView.outputText( 'double penetration', false );
		} else {
			MainView.outputText( 'anal fuck', false );
		}
		MainView.outputText( ' drives you wild, but being on the bottom as you are, there isn\'t much you can do aside from grabbing fistfuls of vixen-ass and slapping it for encouragement.  Urta gets the message and begins to bounce vigorously, mewling and moaning as her balls get large enough to reach the floor.  You lend a hand and play with her sheath, feeling the flesh underneath quake and tremble as the fox\'s orgasm hits.\n\n', false );
		MainView.outputText( 'Scylla\'s neck is pulled tight as Urta\'s urethra is stretched wide by huge bulges of cum.  You can actually feel them sliding past the fingers you\'ve dug into her sheath.  No sooner than one squeezes past is another on its way.  You wonder how Scylla can handle such incredible volumes and the only answer you can think of is magic.  Whatever the case, her swallowing gets noisier and Urta starts bouncing even faster atop your ' + Descriptors.cockDescript( x ), false );
		if( y !== -1 ) {
			MainView.outputText( ' and ' + Descriptors.cockDescript( y ), false );
		}
		MainView.outputText( '.  You pinch one of her shiny black nipples between your fingers as encouragement, and she actually howls with delight.  The herm vixen\'s cum-pumping cock starts spasming in your grip, the order of her previous cum blasts dissolving to turn her cock into a frenzied, constantly squirting spooge-hose.  Every spurt of jism that Scylla swallows is one less in the fox\'s magically-enhanced ballsack, and the unnatural, mighty orbs begin to shrink back towards their normal size with alarming rapidity.\n\n', false );
		MainView.outputText( 'Scylla\'s growing belly and tits smush around your ' + CoC.player.legs() + ', swelling further and further as she tries to devour all the creamy horse-spunk the fox is hosing into her.  Yet the touch of long, gentle fingers caressing your ' + Descriptors.sackDescript() + ' and the gradual inflation of your ' + Descriptors.ballsDescriptLight() + ' lets you know that the slutty nun\'s discovered another source of seed.  You groan as warmth and stimulation make your orbs churn and slosh, growing not just in size but also in raw, animal NEED.  Driven to near madness by the drunken fox\'s delicious backside and the building, backed-up cum in your sack, you grab hold of Urta with both hands and start jackhammering her asshole', false );
		if( y !== -1 ) {
			MainView.outputText( ' and pussy', false );
		}
		MainView.outputText( ', pounding the lithe fox as if your life depended on your orgasm.\n\n', false );
		MainView.outputText( 'Scylla\'s fingers don\'t stop their assault, and as Urta\'s softening cock flops free of the nun\'s ruby lips, smearing them with jizz, the nun takes each of your distended nuts between her palms, one at a time, and rubs it until you can feel the cold floor pressing against their over-sized bulk.  Urta\'s flagging cock reacts to the repeated strikes you\'re giving her prostate by returning to full hardness, though you wonder if Scylla\'s magic had a part in it as well.  Regardless, the nun lifts her habit to expose one lipped, milk-leaking nipple and slides the fox\'s girth inside.  Urta\'s eyes roll back and she begins to babble incoherently, the massive orgasm, followed by anal penetration and nipple-fucking, just too much for her poor battered psyche.\n\n', false );
		MainView.outputText( 'You cum, hard enough to make you cross your eyes and slam Urta\'s hips into you with painful force, even through the padding of her supple buttcheeks.  The first ass-filling wave of jism completely packs her backdoor, surrounding you in a sea of your own sticky moisture.', false );
		if( y > -1 ) {
			MainView.outputText( '  Meanwhile, your other ' + Appearance.cockNoun( CoC.player.cocks[ y ].cockType ) + ' utterly fills her womb, stuffing it so completely that the fox\'s belly rounds out.', false );
		}
		if( (CoC.player.cockTotal() > 1 && y === -1) || CoC.player.cockTotal() > 2 ) {
			MainView.outputText( '  A wave of the stuff splatters her back, glazing her from the shoulder-blades to the crack of her ass and nearly catching you in the chin. Maybe there is such a thing as too many cocks?', false );
		}
		MainView.outputText( '  Scylla\'s hands rub the fox\'s belly tenderly as she humps away at the lewd nun, using whatever magic she has to ensure you won\'t damage the drunken vixen as you continue to push larger and larger amounts of semen into her.  After three more blasts, she runs out of room inside her, and it starts squirting back out with each fresh stroke, soaking your waist, ' + CoC.player.legs() + ', and rapidly shrinking balls.  You lose track of time, knowing only the pleasure of orgasm and the feeling of your own spooge pumping into your lover until the unholy cumsplosion finally ends.\n\n', false );
		MainView.outputText( 'You regain your wits to the sight of a very pregnant-looking Urta and Scylla leaning on each other and trying to make their way to a water barrel to clean up.  The vixen is so full she looks like a mother in her ninth month of pregnancy with triplets, but as she starts to clean up Scylla gently presses on the fox\'s belly and devours the cum she squeezes out.  Urta shakes, but doesn\'t emit a sound beyond a few strangled half-moans of pleasure.  If the fresh horse-cum soaking the nun\'s habit is any indicator, Urta must have gotten off again during your orgasm.  Wild!\n\n', false );
		MainView.outputText( 'You grab a rag and wipe up before donning your ' + CoC.player.armorName + '.  Urta saunters over, looking like she\'s sobered up quite a bit.  She\'s got ', false );
		if( this.urtaLove() ) {
			MainView.outputText( 'a contended smile on her face, and gives you a long, slow kiss that practically radiates her love for you.', false );
		} else if( CoC.flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] < 11 ) {
			MainView.outputText( 'a nervous smile on her face, but gives you a quick peck all the same.', false );
		} else {
			MainView.outputText( 'a satisfied smile on her face, and crushes her lips against yours in a passionate kiss.', false );
		}
		MainView.outputText( '  She says, "<i>I think I\'ll need to help Scylla to her room... somehow whatever she did to me must have pulled all the alcohol out of me and into my cum.  Don\'t worry, I\'ll ', false );
		if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00143 ] <= 1 ) {
			MainView.outputText( 'pay for the door and ', false );
		}
		MainView.outputText( 'make sure she\'s ok.</i>"\n\n', false );
		MainView.outputText( 'Did her cock just twitch?  No, you scold yourself; it couldn\'t have, not after two orgasms like that.  Urta hooks her arm around the giggling nun\'s midsection and pulls the stumbling sister towards the doorway.  Scylla looks back at you and half-giggles, half-slurs, "<i>Tee-hee, thish ish so much fun!  I love you guysh!</i>"\n\n', false );
		MainView.outputText( 'You make your way out with your head low, but you do see Scylla and Urta walking up the stairs towards a truly private room.  They each look a little unsteady and neither of Scylla\'s hands can be seen, but you\'re sure they\'ll be ok.', false );
		CoC.player.orgasm();
		EngineCore.dynStats( 'lib', 1, 'sen', 1, 'cor', 0.5 );
		//+luv;
		this.urtaLove( 3 );
		//+comfort;
		CoC.flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] += 3;
		CoC.player.cumMultiplier += 2;
		CoC.player.ballSize += 1;
		if( y >= 0 ) {
			this.knockUpUrtaChance();
	...
	//[No Scylla];
	Urta.prototype.tellUrtaNoMoreScylla = function() {
		this.urtaSprite();
		MainView.outputText( '', true );
		CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00147 ] = 1;
		MainView.outputText( 'You let Urta know that you\'re willing to overlook this, but that Scylla is a slippery slope that\'s best avoided.  She nods, a sober expression on her vulpine face.  Once you finish, she replies, "<i>I understand, but you need to know – when \'it\' gets too much for me to handle... if you aren\'t around to help I\'m going to see Edryn.  S-she helps... if it wasn\'t for her I probably would\'ve given up and done something stupid a long time ago.  What we have is more important, but unless we can get together every couple days I\'ll NEED to visit her.  ', false );
		if( this.urtaLove() ) {
			MainView.outputText( 'Just please, don\'t doubt my love for you.  ', false );
		}
		MainView.outputText( 'Do I need to stop drinking too?', false );
	...
	Urta.prototype.tellUrtaMoreScyllaIsFine = function() {
		this.urtaSprite();
		MainView.outputText( '', true );
		MainView.outputText( 'You give Urta a raunchy, lewd smile and let her know that you\'re okay with her \'helping\' Scylla, and vice-versa, so long as they don\'t mind you popping in.  She smiles and gives you a wink, but her expression darkens after a moment of thought.  Urta interrupts to mention, "<i>There\'s something else.  If I can\'t find you or Scylla and I NEED to cum, I\'ll have to visit Edryn.  She\'s probably the only thing that\'s kept me sane, and if you aren\'t around to help with my needs I\'ll visit her.  ', false );
		if( this.urtaLove() ) {
			MainView.outputText( 'Just remember that YOU\'RE the one I love, not her.  ', false );
		} else {
			MainView.outputText( 'Me and her, we\'ve been partners in the guard for a long time, but the sex is just mechanical, to sate us.  You don\'t have to worry about her stealing me away.  ', false );
		}
		MainView.outputText( 'But what about my drinking or how I get when I\'m drunk?  It doesn\'t bother you, does it?', false );
		//[Drink More] [Drink Less] [No Change] [Leave Her];
		EngineCore.choices( 'Drink More', this, this.tellUrtaToBeADrunkenHussy, 'Drink Less', this, this.tellUrtaToStopBeingALush, 'No Change', this, this.tellUrtaToStayTheSame, 'Leave Her', this, this.leaveUrtaAfterScyllaConfrontation, '', null, null );
	};
	//[Leave Her];
	...
	Urta.prototype.tellUrtaToBeADrunkenHussy = function() {
		...
		if( CoC.player.cor < 33 ) {
			MainView.outputText( 'blush a little bit', false );
		} else if( CoC.player.cor < 66 ) {
			MainView.outputText( 'smile nervously', false );
		} else {
			MainView.outputText( 'grin widely', false );
		}
		MainView.outputText( ' and explain that when she gets drunk, lets her guard down, and gets a little more aggressive... you like it.  A lot.  Her ears perk up at your words, though her expression is a little uncertain while you explain it.  By the time you finish, something warm brushes by your ' + CoC.player.leg() + ' and gently \'thunks\' the table.  Clearly she\'s as into the idea as you.\n\n', false );
		MainView.outputText( 'Urta smiles, lewdly at first, though it carries a bit of a predatory glint as she waves down a waitress and orders a full bottle of Barkardi 151.  You give her a rueful smile, a stroke under the table, and a kiss just bursting with tongue before you conclude the conversation.  Urta\'s going to be a lot of fun from now on...', false );
	...
	Urta.prototype.tellUrtaToStayTheSame = function() {
		this.urtaSprite();
		MainView.outputText( '', true );
		MainView.outputText( 'You smile and ask her why you would ever want her to change?  You ', false );
		if( this.urtaLove() ) {
			MainView.outputText( 'love', false );
		} else {
			MainView.outputText( 'like', false );
		}
		MainView.outputText( ' everything about her.  She blushes hard when you tell her that, clearly enjoying the flattery and thrilled to hear that you\'re okay with how she chooses to blow off steam when she\'s not on the job.  The two of you joke around for a little longer, but before long, it\'s time you were on your way.  Urta\'s STILL blushing as she gives you a good-bye kiss – you must have earned some points with her today!', false );
		//(+love score);
		this.urtaLove( 5 );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	...
	Urta.prototype.itsOkayUrtaAmilyIsHot = function() {
		MainView.outputText( '', true );
		this.urtaSprite();
		MainView.outputText( 'You reach out, grab Urta\'s shoulders, and look at her until she raises her face to look back at you.  Before she can look away, you tell her that it\'s fine - you had hoped your lovers would come to love each other as you do.  Urta wipes away a tear and gives you a nervous smile when you suggest that perhaps the three of you could get together sometime.\n\n', false );
		MainView.outputText( 'Once you\'ve finished, Urta leans forwards and plants a salty, tear-stained kiss on your lips.  The two of you spend some more time bonding, before you realize you should probably check up on your camp and go.', false );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	//[Heart asplosion];
	...
	Urta.prototype.chastityBeltFun = function( cameFromSexMenu ) {
		...
		MainView.clearOutput();
		if( cameFromSexMenu === undefined || cameFromSexMenu ) {
			cameFromSexMenu = true;
			MainView.outputText( 'Eyeing your foxy lover, you tell her to lose the condom, you have something else planned.\n\n' );
			MainView.outputText( '“<i>Lose it?  What do you have in mind?</i>” Urta asks.  She looks past you towards her cupboard.  “<i>Oh, no, not that belt!  0...Do you really enjoy seeing me make such a mess of myself when I finally get it off?</i>” she protests.\n\n' );
	...
	//Approaching Urta:;
	//(Regular Urta);
	Urta.prototype.QBsTalkExpack = function() {
		MainView.clearOutput();
		this.urtaSprite();
		//(Lover Urta);
		if( this.urtaLove() ) {
			MainView.outputText( 'When you approach Urta, she smiles to see you.  However, there is no rapid swivelling in her seat to avoid banging her horse-cock against the table; evidently she\'s not in the mood for some sex right now.  You sit down beside her, and she happily loops her arm around you, planting a tender kiss on your lips, before almost apologetically stating, "<i>Hey there, lover.  Want to talk for a change?</i>" with a twinkle of good-natured mirth in her eyes.' );
		} else {
			MainView.outputText( 'When you approach Urta, you are almost surprised at the lack of an audible \'thump\'.  Seeing your confusion, the herm fox quips, "<i>What\'s with the look?</i>" while raising an eyebrow.  When you indicate that you had expected to hear her demonstrating her excitement to see you, she blushes and looks embarrassed.  "<i>Well... you do a pretty good job of letting my little pony blow off some steam, ya know, [name]?</i>"' );
		}
		//[Urta] [Edryn] [The Watch] [Back];
		this.urtaDialogueMenu();
	};
	Urta.prototype.friendsTalkExpack = function() {
		MainView.clearOutput();
		this.urtaSprite();
		MainView.outputText( 'You approack Urta\'s table and she motions for you to take a seat.  She finishes writing something on one of the pieces of paperwork scattered over the table, takes a sip of her drink and leans back in her chair.  “<i>Nice to see you again ' + CoC.player.short + '.  What can I do for you?</i>”' );
		this.urtaDialogueMenu();
	};
	...
	Urta.prototype.urtaDiscussesSelf = function() {
		MainView.clearOutput();
		this.urtaSprite();
		MainView.outputText( 'You tell Urta you\'d like to talk about her a little.' );
		if( !this.urtaLove() ) {
			MainView.outputText( '“<i>You wanna talk about me?</i>” she asks' + (this.urtaJustFriends() ? '' : ', blushing softly') + '.  “<i>Well, there isn\'t THAT much to tell, but what do you want to know?</i>”   She seems surprisingly pleased to have the chance to talk.' );
		} else {
			MainView.outputText( '“<i>Well, I don\'t think there\'s that much to tell,</i>” she says with a smile.  She idly blows a bang out of her face and exhales, “<i>Well, lover, I\'m an open book for you, what do you want to know?</i>”' );
		}
	...
	Urta.prototype.flirtWithUrta = function() {
		...
		if( CoC.flags[ kFLAGS.URTA_PC_LOVE_COUNTER ] === -1 ) {
			...
			MainView.outputText( 'You ask Urta if, living in Tel\'Adre, it\'s easier to resist the powerful feelings that come from living in this world.\n\n' );
			MainView.outputText( '“<i>I\'ve lived here my whole life, so I don\'t have anything else to compare with.  What do you mean by \'feelings\'?</i>”\n\n' );
			MainView.outputText( 'You tell her that in your world you never felt a deep urge to strip off all your clothes and ' + (CoC.player.cor < 75 ? 'make sweet love to' : 'slake your lust on') + ' the nearest person.\n\n' );
		} else {
			MainView.outputText( 'You tell her that you don’t have any problem with herms, in fact you find some of them quite attractive.\n\n' );
			MainView.outputText( 'Urta gulps down a bit more of her booze and you decide to have a little fun.  ' );
		}
		MainView.outputText( 'You quietly slide your ' + (CoC.player.isNaga() ? 'tail' : 'foot') + ' across under the table and rub it against the inside of her leg.  Urta practically jumps out of her seat, then tries to pretend nothing happened.\n\n' );
		MainView.outputText( 'You’re pretty sure most of the bar noticed, but you don’t care.  You keep rubbing and feel a thump as something large smacks against the underside of the table.  Urta drains the last of her drink and whispers, “<i>' + (CoC.flags[ kFLAGS.URTA_PC_LOVE_COUNTER ] === -1 ? 'What about Kath?  I... I wouldn\'t...' : 'Please - it’s so hard...') + '</i>”\n\n' );
		MainView.outputText( 'You never find out exactly what she was going to say.  When you start to rub your ' + (CoC.player.isNaga() ? 'tail' : 'foot') + ' in little circles against the root of her huge cock Urta loses her train of thought and just leans back in her chair, trying to keep from spraying her pent up cum across the floor.\n\n' );
		MainView.outputText( 'You ease up so she has a chance of understanding what you’re saying.  “<i>' + (CoC.flags[ kFLAGS.URTA_PC_LOVE_COUNTER ] === -1 ? 'I\'ll explain everything to Kath.  You want this as much as I do, right?' : 'Yes, some chicks, with their big dicks, just do it for me.') + '</i>”  You slide ' + (CoC.player.isNaga() ? 'the tip of your tail' : 'your toes') + ' a little lower and start moving it in little circles against her balls.\n\n' );
		MainView.outputText( 'Before Urta pops you get up from the table and tell her to meet you out back, right where things went off the rails that first time.  Urta nods eagerly and the edge of the table rises slightly.\n\n' );
		MainView.outputText( 'You wait outside, hiding in a shadowy corner.  You got Urta so excited that it’s a good fifteen minutes before she throws open the back door of the bar.  She looks hungry, yet unsure.  You’re guessing she’s had such terrible experiences with people in the past that despite your obvious interest she still can’t believe you want her.\n\n' );
		CoC.flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] = 0; //this.urtaFuckHer will raise it to 1
		CoC.flags[ kFLAGS.URTA_PC_LOVE_COUNTER ] = 0; //Reset both flags to put Urta back to her 'never met you before' state, ready for her first fuck
		this.urtaFuckHer( true ); //Call urtaFuckHer() to put you on track for much Urta loving
	};
	Urta.prototype.urtaFertilityDiscussion = function() {
		MainView.clearOutput();
		MainView.outputText( 'Urta traces a finger around the corner of her drink, "<i>So, what do you think, [name]?' );
		if( CoC.flags[ kFLAGS.URTA_FERTILE ] === 1 ) {
			MainView.outputText( '  Should I stay fertile?' );
		} else {
			MainView.outputText( '  How about I cut loose and go fertile?  Can you imagine me with a nice, round pregnant belly and a cock virile enough to get you pregnant just from looking at it?' );
		}
		if( CoC.flags[ kFLAGS.URTA_PREG_EVERYBODY ] === 1 ) {
			MainView.outputText( '  Or did you come to ask me not to be fertile and virile around anyone else?' );
		} else {
			MainView.outputText( '  Would you mind me being able to put a bun in Edryn\'s oven?  Imagine all the lil foxtaurs running around!' );
			if( CoC.flags[ kFLAGS.EDRYN_NUMBER_OF_KIDS ] > 0 ) {
				MainView.outputText( '  After all, you already had a turn!  Fair\'s fair!' );
			}
		}
		MainView.outputText( '</i>"  She winks at you before blowing you a playful raspberry.\n' );
		MainView.menu();
		if( CoC.flags[ kFLAGS.URTA_FERTILE ] === 1 ) {
	...
	Urta.prototype.urtaFertilityToggle = function() {
		MainView.clearOutput();
		if( CoC.flags[ kFLAGS.URTA_FERTILE ] === 1 ) {
			MainView.outputText( '"<i>Well, I guess that\'s okay.  But you better make it worth my while later, okay?</i>" Urta says with a half-hidden sigh.' );
			CoC.flags[ kFLAGS.URTA_FERTILE ] = 0;
		} else {
			MainView.outputText( 'Urta throws her hands up and cheers, "<i>Yesss!</i>"  Realizing that everyone is looking over at the two of you now, she gives a sheepish smirk and glances back at you.  "<i>I can\'t wait!</i>"' );
	...
	Urta.prototype.urtaPregOthersToggle = function() {
		MainView.clearOutput();
		if( CoC.flags[ kFLAGS.URTA_PREG_EVERYBODY ] === 1 ) {
			MainView.outputText( '"<i>Awww, are you sure?  Wouldn\'t you like ' );
			if( CoC.player.hasCock() ) {
				MainView.outputText( 'to help me make Edryn\'s belly bloat with our seed, and then play \'Guess Who Is The Father\'?' );
			} else if( CoC.player.hasVagina() ) {
				MainView.outputText( 'to have me get the both of you pregnant at the same time, so we can all enjoy it together?' );
			} else {
				MainView.outputText( 'see me put this thing to work in the way that it\'s meant to?' );
			}
			MainView.outputText( '</i>" Urta says, giving you a playful pinch.' );
			CoC.flags[ kFLAGS.URTA_PREG_EVERYBODY ] = 0;
		} else {
			MainView.outputText( 'Urta smiles salaciously, like a cat that got the cream.  Then, without warning, she kisses you square on the lips, passionately frenching you.  After nearly a full minute of frenzied kissing, she says, "<i>This is gonna be fun!</i>"' );
	...
	Urta.prototype.urtaDiscussesEdryn = function() {
		MainView.clearOutput();
		this.urtaSprite();
		MainView.outputText( 'You think for a moment, then tell her that you have questions about Edryn.' );
		if( !this.urtaLove() ) {
	...
	Urta.prototype.urtaDiscussesTheWatch = function() {
		MainView.clearOutput();
		this.urtaSprite();
		MainView.outputText( 'You tell Urta that you have a few questions about the Tel\'Adre city watch.' );
		if( !this.urtaLove() ) {
	...
	Urta.prototype.urtaDiscussesAlcholism = function() {
		MainView.clearOutput();
		this.urtaSprite();
		if( CoC.flags[ kFLAGS.DISCUSSED_URTA_ALCOHOLISM ] === 0 && CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00145 ] === 0 ) {
			MainView.outputText( 'You quietly inform Urta that you and she need to talk about her drinking habit.  She swallows nervously and insists, "<i>I - I only drink to try and keep my cock under control.</i>"' );
			...
			if( CoC.player.cor < 33 ) {
				MainView.outputText( 'and you feel a little bad for putting her through this' );
			} else if( CoC.player.cor < 66 ) {
				MainView.outputText( 'and you would feel bad if this talk weren\'t necessary' );
			} else {
				MainView.outputText( 'but you don\'t let up for a moment' );
			}
			MainView.outputText( '.  You insist that you aren\'t upset with her, that you\'re not blaming her for anything, but you need to talk to her about this matter.' );
			MainView.outputText( '[pg]Urta refuses to meet your eye for several long minutes.  Nervously, she begins tapping one clawed finger on the table.  "<i>I... okay.  The truth is that I did start drinking originally in the hopes it would make my erections subside and make me harder to arouse.  As you\'ve seen, it doesn\'t work like that - truth be told, I\'m kind of a horny drunk.</i>"' );
			MainView.outputText( '[pg]You ask why, then, she does it - why does she keep drinking?' );
			MainView.outputText( '[pg]"<i>Because it\'s an excuse for me to cut loose, alright?</i>" she barks indignantly.  Now she meets your eyes, staring into them with defiance.  "<i>I work day in and day out to keep this city from falling apart, and the whole time, I have to deal with this... this thing between my legs, undermining everything I say.  ' );
			if( this.urtaLove() ) {
				MainView.outputText( 'Lover' );
			} else {
				MainView.outputText( 'Cutey' );
			}
			MainView.outputText( ', you don\'t know what I\'ve been through.  In here, when I\'m off the clock, I can just have a bottle, hang out with my friends, and kind of... just... bask in everyone else\'s happiness.  It\'s not much, but it\'s better than sitting at home, bored and lonely.</i>"' );
			MainView.outputText( '[pg]She finally ceases her rambling dialogue and sighs loudly, shaking her head.  "<i>Maybe it was a bad idea, but it was my idea, you know?</i>"  Then, she looks up at you, gently.  "<i>Besides, I got to meet you, didn\'t I?  Even with all the bad, I gained you for a ' );
			if( !this.urtaLove() ) {
				MainView.outputText( 'friend.</i>"' );
			} else {
				MainView.outputText( 'lover.  I don\'t think I\'ve ever had anyone accept me like you have - even Edryn kind of holds back from me.  You... you\'re different.  Maybe it\'s because you\'re an off-worlder, I don\'t know, but meeting you has been the best thing to happen to me since I joined the guard.  Honestly... I feel like I could probably stop - I don\'t really need it anymore, though I do seem to wind up having a lot of fun when I do.</i>"' );
			}
			MainView.outputText( '[pg]Urta falls silent, waiting for you to speak.  Will you tell her to start drinking less?  Will you tell her that you are fine with her drinking habits, now that she\'s explained her startling change in behavior and her reasons for doing so?  Will you break up with her if she\'s that much of an alcoholic? Or do you want her to drink even more - perhaps you find her so much more fun to be with when she\'s blink stinking drunk?' );
			...
			MainView.outputText( 'You tell Urta that you want to discuss her newfound drinking habits.  The grey-furred fox-morph meets your gaze calmly.  "<i>Really?  What more do you have in mind?</i>" she asks.' );
			if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00146 ] === 1 ) {
				MainView.outputText( '  "<i>I can\'t drink any more than I already do - I\'m kind of pushing the laws to drink as much as I do already.</i>"  She burps loudly, then starts on another bottle.' );
			} else if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00146 ] === 0 ) {
				MainView.outputText( '  "<i>You said you didn\'t mind me drinking as much as I do... are you going to ask me to cut down?</i>" she asks, calmly and clearly assuming that\'s what you intend.' );
			} else {
				MainView.outputText( '  "<i>I said it before, I\'ll say it again, I\'m not going to stop drinking entirely - there\'s nothing wrong with a few cold ones to take the edge off.  I don\'t drink myself stupid any more; that should be enough for you,</i>" she replies, defensively.' );
			}
	...
	Urta.prototype.urtaDiscussAlcoholDrinkLess = function() {
		...
		if( this.pregnancy.type === PregnancyStore.PREGNANCY_PLAYER ) {
			MainView.outputText( ', once she\'s not pregnant anymore, of course' );
		}
		MainView.outputText( '.  Her eyes glitter with steely resolve, and she leans over the table to kiss you on the lips.  The two of you wrap up the conversation, with you hoping that you\'ve seen the last of drunken Urta.' );
	...
	//[=Family=];
	Urta.prototype.urtaDiscussesFamily = function() {
		MainView.clearOutput();
		this.urtaSprite();
		if( !this.urtaLove() ) {
			MainView.outputText( 'You tell Urta that you\'d like to learn about her family.  What are her parents like?  Does she have any siblings?  Is she the only hermaphrodite in the family?' );
			//(Regular;
			if( !this.urtaLove() ) {
				if( CoC.flags[ kFLAGS.URTA_FAMILY_TALK_ATTEMPTS ] === 0 ) {
	...
	//[=Sex/Romance=];
	Urta.prototype.urtaDiscussesSexAndRomance = function() {
		MainView.clearOutput();
		this.urtaSprite();
		MainView.outputText( 'You ask Urta if she\'ll tell you her romantic & sexual history.' );
		if( !this.urtaLove() ) {
	...
	Urta.prototype.urtaDiscussionTeaseAfterRomance = function() {
		MainView.clearOutput();
		this.urtaSprite();
		MainView.outputText( 'You thank her for telling you about her past like this but follow it up by asking her what she likes in the bedroom, doing your best to hide a mischievous grin.' );
	...
	//[=Employment=];
	Urta.prototype.urtaDiscussesEmployment = function() {
		MainView.clearOutput();
		this.urtaSprite();
		MainView.outputText( 'You ask how and when it was that Urta came to be part of the Tel\'Adre watch.' );
	...
	Urta.prototype.urtaDiscussesPrejudice = function() {
		MainView.clearOutput();
		this.urtaSprite();
		MainView.outputText( 'Hesitantly, you ask Urta if she has ever faced prejudice for her hermaphroditic nature.' );
	...
	Urta.prototype.urtaDiscussesEdrynHistory = function() {
		...
		this.urtaSprite();
		//[=History Together=];
		MainView.outputText( 'You ask Urta what her past with Edryn is.' );
	...
	Urta.prototype.urtaDiscussesWorkingWithEdryn = function() {
		MainView.clearOutput();
		this.urtaSprite();
		MainView.outputText( 'You tell Urta that you\'re curious about her working relationship with Edryn.  Do they work together often?' );
	...
	Urta.prototype.urtaDiscussesEdrynRomance = function() {
		...
		if( !this.urtaLove() ) {
			if( !this.urtaOpenAboutEdryn() ) {
				CoC.flags[ kFLAGS.URTA_OPEN_ABOUT_EDRYN ] = 1;
				MainView.outputText( 'You tell Urta that you\'ve noticed she and Edryn seem to hang out together a lot.  Furthermore, as a centauress, you figure Edryn\'s probably capable of handling what Urta has hanging between her legs.  You\'re curious; are they a couple?' );
			...
			//Subsequent Regular Variant:;
				...
				MainView.outputText( 'You give Urta a thoughtful look, and then ask how things are with Edryn these days.' );
				MainView.outputText( '"<i>Good,</i>" the fox-morph replies freely.  "<i>She\'ll never compare to you, but she is a truly wonderful fuck... I swear, there is nothing quite like feeling a mare-cunt juice itself around you...</i>"  She smirks at you, obviously enjoying the chance to make you squirm, but ' );
				if( CoC.player.cor < 66 ) {
					MainView.outputText( 'her manner is playful' );
				} else {
					MainView.outputText( 'her expression sobers when you make no response' );
				}
				MainView.outputText( '.' );
				//(If player can now sleep with Edryn for free:;
				if( CoC.player.statusAffectv1( StatusAffects.Edryn ) >= 5 ) {
					MainView.outputText( '  "<i>Of course, you know that firsthand, don\'t you?</i>" she grins.  "<i>Did hearing that I was having sex with her make you curious about what it was like?' );
					//(If player has fathered at least 1 child on Edryn:;
					if( CoC.flags[ kFLAGS.EDRYN_NUMBER_OF_KIDS ] > 0 ) {
						MainView.outputText( '  Still, I can\'t believe you actually knocked her up - I had to pick my jaw up off the floor when she told me that the foal kicking inside her ballooning gut was yours.  I mean, those herbs are supposed to be foolproof...</i>" She shakes her head, half in disbelief of your sheer virility, half in awe of it.' );
					} else {
						MainView.outputText( '</i>"' );
					}
					if( CoC.flags[ kFLAGS.URTA_FERTILE ] === 0 ) {
						MainView.outputText( '  "<i>Still, I hope you don\'t have any idle thoughts in your head about knocking me up; there\'s a difference between overwhelming contraceptives and sowing seed where the field\'s barren,</i>" she states, patting her flat belly for emphasis.  Despite how flippant she tries to sound when she says this, you can tell that it\'s something of a sore subject for her.' );
					}
				}
		...
		//Lover;
			...
			if( !this.urtaOpenAboutEdryn() ) {
				...
				MainView.outputText( 'With a smirk, you ask Urta how things have been between her and Edryn lately.' );
				...
				MainView.outputText( '"<i>Hmph!  So you should be!</i>" Urta says, sticking her nose up in the air in an indignant huff... but she can\'t keep a straight face for more than a few moments.  "<i>I still can\'t believe you took it so well when I told you - I really am a lucky girl to have someone like you, aren\'t I?</i>" she says.' );
				//(If Edryn is doing the player for free:;
				if( CoC.player.statusAffectv1( StatusAffects.Edryn ) >= 5 ) {
					MainView.outputText( '  "<i>Though I have to admit I was still a little jealous when Edryn told me you were having sex with her too.  Still, I guess that\'s just another thing we have in common, yeah?  A mutual taste in sexy ladies.</i>"  She laughs, and mockingly shapes her hands through the air as if caressing Edryn\'s sizable boobs.' );
				}
				//(If Edryn has had at least one baby:;
				if( CoC.flags[ kFLAGS.EDRYN_NUMBER_OF_KIDS ] > 0 ) {
					if( CoC.flags[ kFLAGS.URTA_QUEST_STATUS ] === 1 ) {
						if( SceneLib.urtaPregs.urtaKids() === 0 ) {
							MainView.outputText( '  "<i>...I still can\'t believe that you actually get her pregnant, though.  It\'s just not fair that Edryn\'s got little hooves running around, and after everything I went through, I don\'t have kits of my own... yet.</i>" She gives you a slightly predatory smile along with the last word.' );
						} else {
							MainView.outputText( '  "<i>...I still can\'t believe that you actually got her pregnant, though.  I guess I\'ll have to keep you busy in the sack if we\'re going to keep up, huh?</i>" She gives you a slightly predatory smile along with the last word.' );
						}
					} else {
						MainView.outputText( '  "<i>...I still can\'t believe that you actually got her pregnant, though. It\'s just not fair that Edryn\'s got little hooves running around, and you and I can\'t have any.</i>"  Urta sighs.' );
					}
				}
	...
	//[=Accept=];
	Urta.prototype.urtaDiscussesEdrynNonLoveAccept = function() {
		MainView.clearOutput();
		this.urtaSprite();
		MainView.outputText( 'You think about it long and hard, and you finally decide that you can\'t hold Urta\'s dalliances with Edryn against her.  From the sound of it, Urta honestly has more reason to be interested in the centauress than in you - she\'s known Edryn longer, she lost her virginity to her, they\'re partners and friends.  Yet, despite this, she insists she feels more of a connection to you, and she was honest about what she\'s been doing and why.  Plus it\'s not as if you can\'t understand how she feels about the temptation.  Most creatures you\'ve met in this weird world have either been very sexually tempting, or outright tried to make you have sex with them.' );
	...
	Urta.prototype.urtaDiscussesEdrynNonLoveREJECT = function() {
		MainView.clearOutput();
		this.urtaSprite();
		MainView.outputText( 'No matter how you look at it, you can\'t help but feel betrayed - Urta\'s been sleeping around behind your back.  If she didn\'t want this relationship to be monogamous, she should have told you before, and you tell her as much.' );
	...
	Urta.prototype.urtaDiscussesEdrynLoveBreakup = function() {
		MainView.clearOutput();
		this.urtaSprite();
		MainView.outputText( 'No matter how you look at it, you can\'t help but feel betrayed - Urta\'s been sleeping around behind your back.  If she didn\'t want this relationship to be monogamous, she should have told you before, and you tell her so.' );
	...
	Urta.prototype.urtaDiscussesWatchMembership = function() {
		MainView.clearOutput();
		this.urtaSprite();
		MainView.outputText( 'You tell Urta that you\'re curious about the Watch\'s membership.' );
	...
	//[=Role In The City=];
	Urta.prototype.urtaDiscussesWatchRole = function() {
		MainView.clearOutput();
		this.urtaSprite();
		MainView.outputText( 'You tell her that you\'re curious about what it is that the Watch actually does, on a day to day basis.' );
	...
	//[=Reach=];
	Urta.prototype.urtaDiscussesTheWatchsReach = function() {
		MainView.clearOutput();
		this.urtaSprite();
		MainView.outputText( 'You ask Urta just how far the Watch\'s authority spans from the city.' );
	...
	//[=Crime & Punishment=];
	Urta.prototype.urtaDiscussesCrimeAndPunishment = function() {
		MainView.clearOutput();
		this.urtaSprite();
		MainView.outputText( 'You explain to Urta that you\'re curious about the law in Tel\'Adre - what is a crime here, and what are the punishments that the Watch is likely to deal to a criminal?' );
	...
	Urta.prototype.spankTheShitOutOfUrtaAndMakeHerCreamHerselfFromProstateStimulationAloneLikeTheHornyDrunkenSlutSheReallyIs = function() {
		MainView.clearOutput();
		MainView.outputText( ImageManager.showImage( 'urta-bar-male-spank' ), false );
		MainView.outputText( 'You consider Urta\'s offers and, tempting as they are, decide they just aren\'t for you right now.' );
		...
		MainView.outputText( 'with each swat.  "<i>You\'re a rude bitch when you\'re drunk, Urta,</i>" you add, giving her another swat on the ass.  She tries to stand, but you roughly push her back against the table.  You give her a fourth spank for her insolence, and can only grin when you hear her cock slam up against the table again.' );
		...
		if( CoC.player.cockThatFits( this.urtaCapacity() ) >= 0 ) {
			var y = CoC.player.cockThatFits( this.urtaCapacity() ) + 1;
			...
			if( CoC.player.cumQ() < 250 ) {
				MainView.outputText( 'you\'ve spent your load' );
			} else if( CoC.player.cumQ() < 1000 ) {
				MainView.outputText( 'filled her utterly' );
			} else {
				MainView.outputText( 'streams of cum leak out of her ass, pooling at your [feet]' );
			}
			MainView.outputText( '.  Urta whines, desperate to cum, but you don\'t let her.  Not yet.' );
	...
	Urta.prototype.urtaDiscussessKatherine = function() {
		MainView.clearOutput();
		var affection = CoC.flags[ kFLAGS.KATHERINE_URTA_AFFECTION ];
		if( affection === 0 ) {
			{ //They’re just co-workers
			}
			MainView.outputText( 'You ask Urta how her new recruit is doing.\n\n' );
			MainView.outputText( '“<i>Oh Katherine?</i>” she asks.  “<i>Really well.  I mean she still needs to brush up on the laws of the land and a few other things, but she’s doing fine.  Plus it’s nice to have a cat in the watch.  I had thought about saving her for undercover work, but I figure it’s better for everyone to see a cat who’s a good upright citizen.  There really aren’t that many and maybe some of the ones in gangs will rethink things when they see her on patrol.</i>”' );
		} else if( affection === 1 ) {
			{ //They’ve never had sex together but each is aware you’re sleeping with the other if you're not in friends mode with Urta
			}
			MainView.outputText( '“<i>Heh, you really have a thing for ' + (SceneLib.katherine.hasCock() ? 'herms' : 'girls in need') + ', don’t ya?</i>” she says with a smile.  “<i>Not that I’m complaining.  Anyway, in terms of her work she still needs to brush up on the laws of the land and a few other things, but she’s doing fine.  Plus it’s nice to have a cat in the watch.  I had thought about saving her for undercover work, but I figure it’s better for everyone to see a cat who’s a good upright citizen.  There really aren’t that many and maybe some of the ones in gangs will rethink things when they see her on patrol.</i>”' );
		} else if( affection < 11 ) {
			{ //They’ve had drunken sex with you (can't get to this point in friend mode)
			}
			MainView.outputText( '“<i>Yeah Kath.  Well I can see why you like her.  Hot girl' + (SceneLib.katherine.hasCock() ? ' with a decent sized cock' : '') + ', gets a bit horny when she’s drunk.  What’s not to like?</i>”\n\n' );
			MainView.outputText( 'She takes a swig of her drink and gives you a hug.  “<i>So, I just want to say I’m not unhappy having sex with you.  I mean that’s always great.  But, um, I gotta admit it’s really fun when you get the three of us together.  If you decide to do that again 0... I’m in.</i>”' );
		} else if( affection < 32 ) {
			{ //Willing to have sober sex
			}
			MainView.outputText( '“<i>Mmmm yeah Kath,</i>” she says dreamily, then shakes her head and laughs.  “<i>Guess that about sums things up, huh?  Yeah I like Kath, I like her quite a bit.  She’s a good watch officer and she’s a good lover in a threesome.</i>”  Urta looks a little nervous, but presses on.  “<i>So... what do you think of the two of us, when you’re not around, you know?</i>”\n\n' );
			MainView.outputText( 'You could tell Urta not to have sex with Katherine except when you’re around, tell her it’s alright for them to have sex whenever they want or encourage them to fuck each other’s brains out.' );
			...
			MainView.outputText( '“<i>' + CoC.player.short + ', I can honestly say I never expected to have this much sex.  I love you, I love Katherine.  It’s a bit weird, but I’m not complaining and neither is she and I hope neither are you.  I’m not the same girl you found sitting alone in the bar.  I’m so happy now.</i>”' );
		}
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	Urta.prototype.urtaKathSexDont = function() {
		MainView.outputText( 'You give Urta a smile tell her that while you’re happy for her you’d really prefer if she waited for you to be around before banging Katherine.  You feel a little left out.\n\n' );
		MainView.outputText( 'Urta gives you a weak smile and says, “<i>Alright ' + CoC.player.short + ', I understand.  I’ll keep it in my pants.  Course I hope you’re up for a lot of three ways.  Girl’s got to get her fix.</i>”' );
	...
	Urta.prototype.urtaKathSexWhenever = function() {
		MainView.outputText( 'You put your arm around Urta and tell her that you don’t mind if your favorite girls need to blow off a little steam together.  As long as they don’t wear each other out that is.\n\n' );
		MainView.outputText( 'Urta lets out a relieved laugh and says, “<i>That’s good.  That’s good.  I’ll make sure not to wear our kitten out.</i>”' );
		CoC.flags[ kFLAGS.KATHERINE_URTA_DATE ] = SceneLib.katherine.KDATE_WHENEVER;
		EngineCore.doNext( this, this.urtaDialogueMenu );
	};
	Urta.prototype.urtaKathSexEncourage = function() {
		MainView.outputText( 'You ask Urta what she would say if you told her you flat out expect them to have sex when you’re not around.\n\n' );
		MainView.outputText( '“<i>Are you sure ' + CoC.player.short + '?  I mean I love the idea, I’m sure Kath will love it too, but you know what my appetite’s like.</i>”\n\n' );
		MainView.outputText( 'You give her a kiss and tell her you know very well.  Since you’re not in town all the time you expect Urta to see to Kath’s needs and Kath to hers.\n\n' );
		MainView.outputText( '“<i>Carte blanche?  Ok ' + CoC.player.short + ', but I’m warning you - your sex kitten is going to be very well fucked whenever you see her.</i>”' );
	...
	Urta.prototype.giveTheFoxSomeEggs = function() {
		...
		if( CoC.flags[ kFLAGS.PC_FETISH ] > 0 ) {
			MainView.outputText( 'as much fun as it might be, ' );
		}
		...
		if( CoC.player.isTaur() ) {
			MainView.outputText( 'positioning your centaur, ' );
		...
		if( CoC.player.gender > 0 ) {
			...
			MainView.outputText( 'starting to grow wet with arousal, matting her back with your sexual fluids, but you ignore them, too caught up in your need of release to bother with conventional pleasures.' );
		} else {
			MainView.outputText( '  The entirety of your sexual world begins and ends with your ovipositor, with the sensation of the mock-cock burrowing inexorably towards Urta\'s womb overwhelming you with pleasure.  Having no other sexual organs to distract you allows you to fully embrace it; you thrust harder and harder, desperate to ensure you have reached the womb before you start to lay.' );
		}
		...
		MainView.outputText( ' to keep Urta moist and slick and pliable.' );
		...
		var wetness = 0;
		if( CoC.player.hasVagina() ) {
			wetness += CoC.player.wetness() * 100;
		}
		if( CoC.player.hasCock() ) {
			wetness += CoC.player.cumQ();
		}
		if( wetness < 300 ) {
			MainView.outputText( 'splattering' );
		} else if( wetness < 500 ) {
			MainView.outputText( 'matting' );
		} else if( wetness < 1000 ) {
			MainView.outputText( 'soaking' );
		} else {
			MainView.outputText( 'flooding' );
		}
		MainView.outputText( ' the fox with your gushing ' );
		if( CoC.player.gender === 1 ) {
			MainView.outputText( 'cum' );
		} else if( CoC.player.gender === 2 ) {
			MainView.outputText( 'femcum' );
		} else {
			MainView.outputText( 'mixed sexual fluids' );
		}
		MainView.outputText( '.' );
		...
		if( CoC.player.gender > 0 ) {
			MainView.outputText( ' just as her back is from your own release' );
		}
		MainView.outputText( '.' );
		//[Egg Level 1:;
		if( CoC.player.eggs() < 20 ) {
			MainView.outputText( '  Urta\'s stomach starts to swell as your clutch of eggs fills her once-empty womb, stretching into a noticeable bulge, though not one that she couldn\'t hide if she wanted.' );
		}//[Egg Level 2:;
		else if( CoC.player.eggs() < 40 ) {
			MainView.outputText( '  As the eggs keep on coming, Urta\'s womb swells and bulges, crammed full of goo and eggs, growing larger and larger until Urta could easily pass for an expectant mother, ready to give birth any day now.' );
		}//[Egg Level 3:;
		else {
			MainView.outputText( '  You stuff Urta with eggs until you\'re wondering how many either of you can hold. Her belly just keeps getting rounder and rounder even as your eggs keep coming; soon, not only does she look like a woman pregnant with multiples, her skin is visibly stretched over the eggs, giving it a distinctively lumpy look rather than the round, smooth bulge she might have if your load was smaller.' );
		}
		...
		if( CoC.flags[ kFLAGS.URTA_EGG_FORCE_EVENT ] === 0 ) {
			CoC.flags[ kFLAGS.URTA_EGG_FORCE_EVENT ] = 48;
		}
		CoC.player.dumpEggs();
		CoC.player.orgasm();
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	...
	Urta.prototype.urtaChewsOutPC = function( newScreen ) {
		...
		MainView.outputText( 'Urta looks up at you and snorts fiercely; you almost expect steam to come flaring out of her nostrils, and her livid eyes are doing their best to burn holes straight through you.  "<i>There you are, [name]!  You have some nerve!</i>"' );
		...
		if( CoC.player.cor > 50 ) {
			MainView.outputText( ', especially given this may be the closest she\'ll ever get to experiencing pregnancy' );
		}
		MainView.outputText( '?' );
		...
		//[Unfertilised Eggs:;
		if( CoC.flags[ kFLAGS.URTA_FERTILE_EGGS ] === 0 ) {
			...
			if( CoC.player.lib > 49 && CoC.player.hasVagina() ) {
				MainView.outputText( '  Raising an eyebrow, you lean closer and inform her that you could make babies for her - she just needs to find you a big cock dripping with, and this is the important part, <i>fertile</i> seed.' );
	...
	Urta.prototype.repeatUrtaEgging = function() {
		MainView.clearOutput();
		MainView.outputText( ImageManager.showImage( 'urta-egg-repeat' ), false );
		MainView.outputText( 'With a smile, you ask if Urta would be willing to let you lay a few eggs in her womb again?' );
		...
		if( !this.urtaLove() ) {
			MainView.outputText( 'blatantly hanging out of her stockings and drooling pre-cum everywhere' );
		} else {
			MainView.outputText( 'vulgarly bulging her dress and rendering it sopping wet from her arousal' );
		}
		MainView.outputText( ', before sashaying in a drunkenly exaggerated fashion out towards the alleyway where you and she first began your relationship- and where you first \'impreggnated\' her.' );
		...
		if( CoC.player.gender > 0 ) {
			...
			MainView.outputText( ' are starting to grow wet with arousal, matting her back with your sexual fluids, but you ignore them, too caught up in your need of release to bother with conventional pleasures.' );
		} else {
	...
	//[Condomless];
	Urta.prototype.condomlessUrtaInHouseSmex = function() {
		MainView.clearOutput();
		MainView.outputText( 'You grab hold of the pre-filled bubble atop the fox\'s tip and yank, earning a moan as the latex sheath slides free, stretching it to an oblong mass before freeing her flare with an audible pop.  At the same time, you draw the second, unused condom from her unresisting grip and say, "<i>Why don\'t we make a little mess?</i>"' );
		//non lover:;
		if( !this.urtaLove() ) {
	...
	Urta.prototype.pegUrtasAssMessy = function() {
		...
		MainView.outputText( Descriptors.cockDescript( x ) + ' down onto the sheets, a scant few inches from Urta\'s puckered asshole.  The fox ' );
		if( this.urtaDrunk() ) {
			MainView.outputText( 'giggles drunkenly, "<i>You\'re sho dirty!</i>" while spreading those fishnet-clad legs further.' );
		} else if( CoC.flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] < 11 ) {
			MainView.outputText( 'softly begs, "<i>Be gentle, please,</i>" while spreading those fishnet-clad legs a little further apart.' );
		} else {
			MainView.outputText( 'eagerly coos, "<i>Feeling a little dirty, eh, lover?  Go on baby, you can fuck my ass anytime.</i>"  She spreads her fishnet-clad legs so wide they almost form an unbroken, horizontal line from padded toe to padded toe, tying her soft, poofy tail around a leg to keep it out of the way.' );
		}
		MainView.outputText( '  Scooting forward, you edge your ' + CoC.player.cockHead( x ) + ' up to her anus.  Her body heat washes over you as you enjoy the feel of her pucker\'s folded flesh pressing down on your most sensitive point, lubricated by the copious female fluid that drains from the sodden gash above.' );
		MainView.outputText( '\n\nYou begin to gently rock your [hips] forward, giving little, gentle pushes against the resistant muscle.  At first, you make no progress, simply forcing her anal ring to dilate, little by little.  Then, after a particularly vigorous stroke, your ' + CoC.player.cockHead( x ) + ' slides through the clinging ring and into the tight embrace of her slick anus.  You stop to allow her to adjust, admiring her ' );
		if( this.urtaDrunk() ) {
			MainView.outputText( 'drunken ' );
		}
		MainView.outputText( 'whimpers of pleasure.  However, you don\'t wait long, slowly pushing forward and exploring your vixen\'s every inner crevice with your eager rod.' );
		MainView.outputText( '\n\n"<i>It\'s ' );
		if( this.urtaDrunk() ) {
			MainView.outputText( 'sho' );
		} else {
			MainView.outputText( 'so' );
		}
		MainView.outputText( ' different than with a condom over a toy,</i>" the vulpine lass wonders aloud in delight.  She licks her lips and ' );
		if( CoC.flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] > 15 ) {
			MainView.outputText( 'casually' );
		} else if( CoC.flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] > 11 ) {
			MainView.outputText( 'confidently' );
		} else if( CoC.flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] > 4 ) {
			MainView.outputText( 'a little nervously' );
		} else {
			MainView.outputText( 'with a shaky hand' );
		}
		MainView.outputText( ' strokes a padded fingertip down the bulging length of her urethra, shivering when it bumps across the sensitive medial ring.  A few inches in, you feel a rounded bulge bump into you, and as soon as you press against it, Urta\'s cock lifts up and thickens, flooded with blood.  The fox trembles for a second as you glide by the obstruction, and without any warning at all, her shaft belches a thick spurt of clear pre-cum with enough force to glaze the underside of her tits.' );
		MainView.outputText( '\n\nSmiling at the lascivious display, you gather a handful of the balmy animal cream and circle it around one of her perky nipples, admiring how the black nub perks up into a puckered, sensitive tower.  Then, while still pushing into her, you repeat the action on the other side, with identical result.  Your ' + Descriptors.cockDescript( x ) + ' milks another heavy flow from the fox\'s prostate, this one tinged with just a little bit of off-white.  It puddles on her belly, already forgotten.  You\'re pushing forward without much worry for her comfort now, and blessedly, the fox doesn\'t seem to mind.  Indeed, with every lurch forward, her twat\'s drippings come faster and faster, matched by even larger discharges from the turgid member above.  ' );
		if( CoC.player.cockArea( x ) > this.urtaCapacity() ) {
			MainView.outputText( 'The aching pressure around your ' + Descriptors.cockDescript( x ) + ' doubles, and you realize she simply can\'t take any more.  Her anus is stretched wide, dilated to the extreme to accommodate this much of you.  There\'s little to do but plow her with what you can until she\'s fountaining white.' );
		} else if( CoC.player.hasKnot( x ) ) {
			MainView.outputText( 'The pleasure seems to double when your knot butts up against her furry rump, but her asshole isn\'t ready for that just yet.  This bitch needs a little more loving before you knot her.' );
		} else {
			MainView.outputText( 'The aching pleasure around your ' + Descriptors.cockDescript( x ) + ' redoubles as you hilt her, smashing your ' );
			if( !CoC.player.hasSheath() ) {
				MainView.outputText( 'groin ' );
			} else {
				MainView.outputText( 'sheath ' );
			}
			MainView.outputText( 'against her taut asshole with a heavy lurch.' );
		}
		MainView.outputText( '\n\nGroping the vixen\'s tits and thumbing her nipples, you have your way with her chest, torturously teasing her while your ' + Descriptors.cockDescript( x ) + ' rests immobile in her ass.  Her pussy-slicked sack rests against you, lewdly bouncing and pulsating with need, fairly ready to explode; her cock is equally aroused.  The rigid length is so hard that it hovers over her smooth abdominals without touching them, bobbing up and down while it exudes a steady drizzle of lust.  The pre-spunk is so thick on Urta\'s belly that her fur is matted and glossy from her hips to the underside of her tits, and ribbons of the stuff run down her sides to lubricate the sheets below.  Her hands push yours into her chest harder, smashing her tits almost flat against her.  At the same time, her hips squirm and writhe, grinding in lewd little circles against you, swirling your dick up against every part of her sensitive tail-hole.' );
		...
		if( CoC.player.hasKnot( x ) && CoC.player.cockArea( x ) < this.urtaCapacity() ) {
			MainView.outputText( '  You barely notice when your knot pops through the ebony ring, locking you inside her.  All you know is that while your strokes have gotten shorter, the pleasure has gotten even greater.' );
		}
		MainView.outputText( '\n\nYou pinch the nipple you\'ve been playing with and slam your ' + Descriptors.cockDescript( x ) + ' home with rump-shaking force.  It slides the fox backward in her puddle of pre, slicking the underside of her legs with her sexual moisture as she\'s dragged through it.  Barely suppressing a moan of your own, you shimmy forward again and voraciously tackle her asshole, pegging Urta with short, brutal strokes.  Eager to see her blow, you shift your angle and press up, slamming your ' + CoC.player.cockHead( x ) + ' into her prostate so hard you swear you must be flattening it.  Urta screams, her balls contracting powerfully.  Her dick, already iron-hard, seems to swell thicker, looking like a flat-tipped, over-inflated sausage.  Bulging obscenely, the underside of her cock swells with lusty intent, the distention moving up with muscular contractions until the vulpine girl\'s cum-slit is wide and filled with white.' );
		MainView.outputText( '\n\nThe onyx asshole wringes you with every ounce of force its owner can apply, convulsing into a tight cock-ring that' );
		if( CoC.player.hasKnot( x ) && CoC.player.cockArea( x ) < this.urtaCapacity() ) {
			MainView.outputText( ' only makes your knot bigger and harder.' );
		} else {
			MainView.outputText( ' works you so hard an irrational part of your mind worries your manhood will be pinched off.' );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( '  Good thing you have more.' );
			}
		}
		MainView.outputText( '  Then, the fox stops being a fox and turns into a fountain, spewing a wave of white up over her head and into the wall.  It hits with a wet \'splat\', flinging alabaster globs in every direction that burst like little bombs.  Spunk rains down over Urta, staining her bed and her in equal measure.  A lock of gunked-up hair flops across her face, temporarily blinding your orgasm-inundated lover.  Just when the dripping jizz slows and you think the worst is over, Urta goes off again, though this fountain of white has a little less force than the preceeding cumsplosion.  A wave of semen as thick as your wrist spurts out to take the fox in the face, plastering the rest of her lustrous hair to her forehead and glazing her visage with ivory lust.  Ringlets of her ' );
		if( CoC.flags[ kFLAGS.URTA_QUEST_STATUS ] < 0 ) {
			MainView.outputText( 'not-so-fertile ' );
		}
		MainView.outputText( 'fluid dangle precariously from her jawline as she tries to breathe, coughing and making bubbles from her white mask.' );
		...
		if( this.urtaDrunk() ) {
			MainView.outputText( ' and of course, alcohol' );
		}
		MainView.outputText( ', kissing your ' + CoC.player.cockHead( x ) + ' every time it peeks past her tits, relishing the taste of her own lust all over it.' );
		...
		if( CoC.player.cumQ() < 250 ) {
			MainView.outputText( ', swallowing with a smile and a wink.' );
		} else {
			MainView.outputText( ', but she\'s forced to close it when her cheeks are puffed beyond reason.' );
			if( CoC.player.cumQ() > 750 ) {
				MainView.outputText( '  You go on to wash her face with your own white sauce.' );
			}
			if( CoC.player.cumQ() >= 1200 ) {
				MainView.outputText( '  Your orgasm goes on and on, coating just about everything that Urta came on with yet another layer.' );
			}
			if( CoC.player.cumQ() > 3000 ) {
				MainView.outputText( '  She gasps in surprise when you flood the ' );
				if( CoC.player.cumQ() > 10000 ) {
					MainView.outputText( 'first few feet of the ' );
				}
				MainView.outputText( 'room, looking up at you in admiration through cum-coated eyelashes.' );
			}
		}
		MainView.outputText( '\n\nAs you finish emptying your [balls], you sag down into the fox\'s arms.  She plants a salty kiss on your lips ' );
		if( this.urtaLove() ) {
			MainView.outputText( 'and nuzzles against your cheek, just holding you for a few moments, lover to lover.' );
		} else {
			MainView.outputText( 'and sheepishly pulls back.' );
		}
		MainView.outputText( '  Climbing out of the alabaster-painted bed is no easy task, but the two of you manage, laughing and leaning on each other\'s arms.  Urta leads you to a cabinet in another room, and bends over to open it.  You barely notice her pulling out mops, buckets, and cleansers.  Instead, your vision is riveted to her asshole, still gaped lewdly in the shape of your ' + Descriptors.cockDescript( x ) + '.  When she comes up with an armload of cleaning supplies, you\'re ' );
		if( CoC.player.cor < 50 ) {
			MainView.outputText( 'blushing' );
		} else {
			MainView.outputText( 'smiling devilishly' );
		}
		MainView.outputText( '.' );
		...
		CoC.player.orgasm();
		EngineCore.dynStats( 'lib', 0.2, 'sen', -3 );
		CoC.flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] += 2;
		this.urtaLove( 2 );
		CoC.flags[ kFLAGS.TIMES_FUCKED_URTA ]++;
		CoC.player.slimeFeed();
	...
	Urta.prototype.UrtaGetsPeggedCleanUp = function() {
		MainView.clearOutput();
		if( CoC.player.cor >= 50 ) {
			MainView.outputText( 'You start to duck out, but the disapproving glare the fox gives you makes it clear that you won\'t be getting any more messy fun unless you stick around to clean it up this time.  Damnit.  ' );
		}
		MainView.outputText( 'With Urta\'s help, you slowly clean up the mess you two made, nearly clogging the plumbing a few times in the process.  New sheets, pillows, and furs are brought in, and you help her fix her bed.  The floor is mopped, the walls are scrubbed, and the shutters are opened to let in some fresh air.  Thankfully, just about everything is watertight and sealed, likely something she had done to ease situations such as this.  All told, between clean-up and sex, you spend a few hours with her.' );
		MainView.outputText( '\n\nTenting her apron (the only thing she\'s bothered to put on), Urta gives you a ' );
		if( !this.urtaLove() ) {
			MainView.outputText( 'wave' );
		} else {
			MainView.outputText( 'tender hug' );
		}
		MainView.outputText( ' and sends you on your way.' );
	...
	Urta.prototype.fillMeUpPleaseUrta = function() {
		MainView.clearOutput();
		MainView.outputText( ImageManager.showImage( 'urta-home-female-fucked' ), false );
		MainView.outputText( 'Lazily discarding the latex, you caress Urta\'s stocking-clad leg and coyly climb into bed.  She starts to reciprocate your affections, but you push her arm away, instructing, "<i>Let me... let me have this.</i>"' );
		...
		if( CoC.player.wetness() > 3 ) {
			MainView.outputText( '  You\'re so utterly soaked thanks to this place that a steady stream of vaginal lubricant drizzles over the meaty tool below.' );
		}
		if( CoC.player.hasCock() ) {
			MainView.outputText( '  Hard and erect, [eachCock] stands tall and proud, but ignored.  You have other plans.' );
		}
		MainView.outputText( '  She\'s ready.  You\'re ready.  It\'s time.' );
		...
		if( CoC.player.wetness() > 3 ) {
			MainView.outputText( ', though it\'s a drop in the bucket next to your copious moisture.  Hell, even now, rivers of your girl-goo are sliding down that spasming length, wreathing it in lady-spunk' );
		} else {
			MainView.outputText( ', deliciously staining your slit' );
		}
		MainView.outputText( '.' );
		//{Virgin PCs};
		if( CoC.player.vaginas[ 0 ].virgin ) {
			...
			if( CoC.player.tallness > 80 ) {
				...
				MainView.outputText( '[chest]' );
			}
			MainView.outputText( ' as she cranes her neck to meet you, eye to eye.  "<i>Really?</i>"  You nod, biting your lower lip as you continue your frenzied undulations upon her rod, working yourself up to taking her.' );
			...
			if( this.urtaDrunk() ) {
				MainView.outputText( 'drunkenly ' );
			}
			MainView.outputText( 'asks, "<i>Are you s' );
			if( this.urtaDrunk() ) {
				MainView.outputText( 'h' );
			}
			MainView.outputText( 'ure?  You don\'t have to...</i>"' );
			...
			if( CoC.player.hasCock() ) {
				MainView.outputText( '  Drool begins to leak from [eachCock], slicking the underside with pre-cum as the pleasure affects your maleness.' );
			}
			MainView.outputText( '  Before long, you\'ve taken Urta halfway, with her ring jutting up against your entrance.' );
			//{cuntChange 30};
		...
		if( CoC.flags[ kFLAGS.PC_FETISH ] > 0 ) {
			MainView.outputText( '  An exhibitionist thrill runs through you at that, making you even wetter.' );
		}
		...
		if( CoC.player.hasCock() ) {
			MainView.outputText( '  You don\'t even notice [eachCock] dripping on Urta\'s belly.' );
		}
		//HERMS:;
		if( CoC.player.hasCock() ) {
			...
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( ', followed in short order by the untouched portion of your ' + Descriptors.multiCockDescriptLight() );
			}
			MainView.outputText( '.  The next is your [vagina] going wild, convulsing from your entrance towards your womb with muscular contractions that milk Urta\'s cock relentlessly, causing it to grow bigger and harder inside you.  Urta whimpers slightly as you jizz over her belly' );
			if( CoC.player.cumQ() > 400 ) {
				MainView.outputText( ', soaking her' );
				if( CoC.player.cumQ() > 1000 ) {
					if( CoC.player.cumQ() < 2000 ) {
						MainView.outputText( ' and her bed' );
					} else {
						MainView.outputText( ', her bed, and damn near everything else' );
					}
				}
				MainView.outputText( ' with your ejaculate' );
			}
			MainView.outputText( '.  There\'s a bloom of warmth from below as the vixen\'s orgasm rushes towards you, and all you can do is cum your brains out in anticipation of being filled.' );
		}
		//NON-HERMS:;
		else {
		...
		if( CoC.player.biggestTitSize() >= 4 ) {
			MainView.outputText( ' and your [chest] rest heavily upon your cum-stuffed form' );
		}
		MainView.outputText( '.' );
		if( CoC.player.pregnancyIncubation > 0 && CoC.player.pregnancyIncubation < 150 ) {
			MainView.outputText( '  You may have been pregnant before, but you\'re beyond pregnant now... with cum AND child.' );
		}
		//{Goo:;
		if( CoC.player.isGoo() && CoC.player.vaginalCapacity() > 9000 ) {
			MainView.outputText( '  Successive eruptions make you bigger, but your flexible cunt soon grows so bloated that the onrushing injections mostly squirt back out, painting Urta, the bed, and your [legs] white with cum.' );
		} else {
			MainView.outputText( '  Successive eruptions do little to bloat you further - your body is stretched to capacity, so the onrushing injections only squirt back out through your entrance, painting Urta, the bed, and your [legs] white with cum.' );
		}
		...
		if( CoC.player.pregnancyIncubation > 0 && CoC.player.pregnancyIncubation < 150 ) {
			MainView.outputText( 'even more ' );
		}
		MainView.outputText( 'pregnant for a while, at least until you can pass all that filling!  She plants a salty kiss on your lips ' );
		if( this.urtaLove() ) {
			MainView.outputText( 'and nuzzles against your cheek, just holding you for a few moments, lover to lover.' );
		} else {
			MainView.outputText( 'and sheepishly pulls back.' );
		...
		if( CoC.player.cor < 33 ) {
			...
			MainView.outputText( 'grinning' );
		} else {
			MainView.outputText( 'openly leering' );
		}
		MainView.outputText( '.' );
	...
	Urta.prototype.cleanUpAfterUrtaCreamInjectsYou = function() {
		MainView.clearOutput();
		if( CoC.player.cor >= 50 ) {
			MainView.outputText( 'You start to duck out, but the disapproving glare the fox gives you makes it clear that you won\'t be getting any more messy fun unless you stick around to clean it up this time.  Damnit.  ' );
		}
	...
	Urta.prototype.lippleBondUrtaScylla = function() {
		...
		//[if (cocks > 1);
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( '  Without much warning, you suddenly feel Urta\'s hand move away from your own palm, the sensation of your [cock smallest] being wrapped by her fingers also assaulting you as she squeezes and tugs at your second cock affectionately.' );
		}
		//[if (cocks > 2);
		if( CoC.player.cockTotal() > 2 ) {
			MainView.outputText( '  The barrage of sensations is intensified when Scylla also gets a bit greedy, grabbing another tool and starting to jerk it off as well.' );
		}
		...
		//[if (cocks > 1);
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( ' either' );
		}
		MainView.outputText( ' the holy woman\'s ravenous lipples and inner, erotic tit-flesh in a mutually ecstatic, sensations-filled manner' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( ' or more delicately and less hungrily with a feminine hand' );
		}
		MainView.outputText( '.' );
		...
		//if (sensitivity < 29);
		if( CoC.player.sens < 29 ) {
			MainView.outputText( '  You can only tell so from her expression and from Scylla moaning and gasping sharply, but it appears Urta\'s cock is really just about ready to shoot off cum.' );
		}
		//if (sensitivity > 29);
		else {
			MainView.outputText( '  Scylla\'s titflesh slightly shifts from what is apparently the expansion of Urta\'s cock flare and the twitching of her massive shaft.  You\'re sensitive enough to notice.' );
		}
		//[if (sensitivity > 44);
		if( CoC.player.sens >= 44 ) {
			MainView.outputText( '  In fact, you can clearly feel the pressure inside Scylla\'s chest increasing as Urta\'s huge cock expands and twitches inside her next to your own, each such twitch sending a shiver of pleasure through both you and the nun.' );
		}
		...
		if( CoC.player.hasKnot( CoC.player.biggestCockIndex() ) ) {
			MainView.outputText( '  You\'re not making the problem any easier, in fact, your knot is halfway stuck inside her breast, surged and filled to full size right after your orgasm.' );
		}
		...
		if( CoC.player.hasKnot( CoC.player.biggestCockIndex() ) ) {
			MainView.outputText( ', your knot sufficiently deflated now' );
		}
		MainView.outputText( ', now sucking desperately at the air, looking for the tasty dick.' );
	...
	//The scene:;
	Urta.prototype.nagaOnUrtaMess = function() {
		MainView.clearOutput();
		MainView.outputText( ImageManager.showImage( 'urta-naga-fuck' ), false );
		MainView.outputText( 'You tell Urta that today, you intend to pleasure her a bit more thoroughly, intimately and in a slightly kinky way, and then maybe have her return the favor, condoms off, regardless of how messy it gets.  She raises an eyebrow, and then you ask her if she\'d ever thought about getting off on both ends with the use of your coils.  Her eyes dart open a bit and she seems flustered.' );
		...
		//[if (cocks = 0);
		if( CoC.player.cockTotal() === 0 ) {
			MainView.outputText( '  Urta runs her fingers over and gently pushes into your now revealed female slit. "<i>My lover\'s sweet, sweet pussy. The best in the whole world.</i>"' );
		}//if (cocks = 1);
		else if( CoC.player.cockTotal() === 1 ) {
			MainView.outputText( '  "<i>And here is my favorite cock in the whole world... Probably including mine,</i>" she giggles and winks at you.' );
		}//if (cocks = 2);
		else if( CoC.player.cockTotal() === 2 ) {
			MainView.outputText( '  "<i>Two delicious cocks, rather than just one...  You\'re trying to tell me you\'re the man here, aren\'t you, lover?</i>" she giggles and winks at you.' );
		} else if( CoC.player.cockTotal() === 3 ) {
			MainView.outputText( '  "<i>You want to have a spare one just in case or something?</i>" she giggles and winks at you.' );
		}//[if (cocks >= 4)];
		else {
			MainView.outputText( '  "<i>Interesting.  It looks like a bouquet.  A beautiful bouquet of cocks just for me,</i>" she giggles and winks at you.' );
		}

		MainView.outputText( '  Without further ado, Urta leans towards your sex to return the favor, her mouth pleasuring you just as you wrap more and more of your slightly looser coils around her member, having released her legs to allow her to move... and to allow your tail\'s tip to reach her needy vulpine pussy more easily from that position.' );
		...
		//MainView.outputText('  However, it requires no justification that your herm lover is further on her way to an orgasm then you are.');;
		...
		if( CoC.player.hasVagina() ) {
			...
			if( CoC.player.wetness() >= 4 ) {
				MainView.outputText( '  After a few convulsions, your femcum starts to shoot out of your pussy quite violently, adding to the mess your lover is making.' );
			}
		}
		//if (cocks > 0);
		if( CoC.player.cockTotal() > 0 ) {
			...
			//[if (cumHigh = true)];
			if( CoC.player.cumQ() >= 1000 && CoC.player.cumQ() < 2500 ) {
				MainView.outputText( '  Her eagerness does end up giving her a swollen tummy, but she\'s loving and hungry for you until the end, swallowing as much as she can, as quickly as she can.' );
			}//(if [cumExtreme = True];
			else if( CoC.player.cumQ() >= 2500 ) {
				MainView.outputText( '  Of course, your excessive load proves too much for the poor vixen to handle, and soon she is not only inflated, but painted in your cum as well, the mess the two of you are making definitely nothing to be scoffed at.' );
			}
		}
	...
	Urta.prototype.lickOutUrtaAtHome = function() {
		...
		MainView.outputText( '\n\nYou climb overtop of Urta and allow your ' + Descriptors.cockDescript( x ) + ' to press on into her pussy.  ' );
		if( CoC.player.cockArea( x ) >= this.urtaCapacity() / 2 ) {
			MainView.outputText( 'It fits you like a lubricated, stretchy little glove, ensuring every movement will caress her folds.' );
		} else {
			MainView.outputText( 'It\'s nice and roomy, allowing you full control over how you want to press on her inner folds.' );
		}
		MainView.outputText( '  You push the rest of the way into her cunt and admire the velvety texture of her walls as they close in around you, inner muscles flexing and massaging your ' + Descriptors.cockDescript( x ) + ' as she adjusts to your intrusion.' );
		if( CoC.player.cockArea( x ) >= 200 ) {
			MainView.outputText( '   Stretching ludicrously, her cunt dilates with surprising ease.  The elastic lips of her pussy cling oh-so-tightly to your girth as they\'re spread beyond reason.  Her hips even spread with only a modest bit of pressure and resistance as her passage is distorted, the shape of your ' + Descriptors.cockDescript( x ) + ' visible through her stretched belly as it pushes towards her tits.  You can even make out some of the larger veins.' );
		}
		MainView.outputText( '\n\nThe hermaphrodite lifts one of her ebony nipples up to her midnight-hued lips and suckles it eagerly, writhing in the delight.  You meet her hungry eyes as you gently begin to slide back and forth, reveling in the liquid-hot heat of the vulpine woman\'s snatch with a lusty moan, pumping your ' + Descriptors.cockDescript( x ) + ' in and out over and over.  Her secretions are running unchecked onto the bedsheets now, and neither of you care.  You grab hold of Urta\'s neck to pull her away from her nipple so that you can kiss her on the lips, and you flick your tongue into her mouth.  She demurely accepts the passionate tonguing before allowing her own to slide into yours, slithering to the tempo your [hips] set.' );
		...
		if( CoC.player.cor < 66 ) {
			MainView.outputText( 'You\'re wonderful' );
		} else {
			MainView.outputText( 'You\'re the most amazing fuck' );
		}
		MainView.outputText( '</i>"' );
		...
		if( CoC.player.cumQ() >= 500 ) {
			MainView.outputText( '  Spermy juices run out of her pussy in alabaster streams thick enough to obscure the black-lips they\'re travelling over.' );
		}
		if( CoC.player.cumQ() >= 1500 ) {
			MainView.outputText( '  The fox gives a surprised grin when her belly rounds slightly, though she seems concerned at how it makes her cum-filled prize wobble.' );
		}
		if( CoC.player.cumQ() >= 3000 ) {
			MainView.outputText( '  Your virile pecker just won\'t stop, and by the time you finish ejaculating, Urta is pregnant with sperm, her belly round with its belly-button protruding.' );
		}
		MainView.outputText( '\n\nSharing an intimate moment with your ' );
		if( this.urtaDrunk() ) {
			MainView.outputText( 'drunken ' );
		}
		MainView.outputText( 'lover, you kiss her again.  She returns it demurely, but midway through, you feel something slick and hard brushing up against you.  Urta whispers, "<i>Sorry,</i>" when you pull away to examine the sudden change.  Her pole is hardening again, surging up to rub the full prophylactic against your [chest].  You glance back at her, and all Urta has to say for herself is, "<i>Sorry, lover... feeling you cum inside me is wonderful, and well, I had time to recover.</i>"  She slowly pulls the rubber off her cock, careful not to jostle it too hard.  A second later, it\'s tied off and set aside.  With you still ' );
		if( CoC.player.hasKnot( x ) ) {
			MainView.outputText( 'knotted' );
		} else {
			MainView.outputText( 'resting' );
		}
		MainView.outputText( ' inside her juicy cunt, she asks if she needs another one, batting her eyelashes playfully.' );
		MainView.outputText( '\n\nShaking your head from side to side, you pull back' );
		if( CoC.player.hasKnot( x ) ) {
			MainView.outputText( ' in an attempt to withdraw.  Her entrance dilates slowly as your full, dog-like knot is pressed against her from inside, the pulsating, reddish flesh glistening with the mixed sexual lubricants.  You tug and tug, rocking your [hips] slowly to loosen her up, and eventually, you manage to pop out, drawing a moan of mixed anguish and pleasure from Urta\'s maw.' );
		} else {
			MainView.outputText( ' and withdraw.' );
		}
		if( CoC.player.cumQ() >= 1500 ) {
			MainView.outputText( '   Cum runs out of her packed womb in a wave, spreading all over the sheets down to her knees.  She shakes slightly as the cum runs out of her, rubbing her full middle protectively.' );
		} else {
			MainView.outputText( '  Cum drools out of her still-gaping slit in steady trickles, puddling on the sheets below.  She groans happily.  Clearly the fox is proud to have such a productive lover.' );
		}
		MainView.outputText( '  You take hold of the puffy, dick-stretched pussy lips and spread them around two fingers, collecting some of the warm sex-juice on your fingertips, then, you pull the dripping, creamy treat out and smear it over Urta\'s mouth.  She giggles as you apply the lip gloss to her and slowly runs her tongue across the shining coat of sex.  You respond by repeating the motion, this time laying it on thicker.' );

		MainView.outputText( '\n\n"<i>This is all the lipstick you\'ll need today, babe,</i>" you explain as you admire the way her cum-stained lips shine.  She\'s marked with your ejaculate and sure to have cock on her breath for hours.  Turning back to the glittering box below, you raise her balls and lean down under them, letting them fall into place on your ' + Descriptors.hairDescript() + ' once you\'re in position.  You blow on the dripping cunny, and Urta moans lustily in response.  Then, you promise, "<i>You\'re going to be satisfied after this.  Your dick and cunt are gonna cum so hard you won\'t be getting it up for days.</i>"' );
		...
		CoC.flags[ kFLAGS.URTA_TIME_SINCE_LAST_CAME ] += 12;
	...
	Urta.prototype.slurpFawkesCocksForFunAndInflation = function() {
		MainView.clearOutput();
		MainView.outputText( ImageManager.showImage( 'urta-home-bj' ), false );
		MainView.outputText( '"<i>Thanks, Lover! I need this. I\'ve been so busy, I haven\'t had any relief in days!</i>" Urta informs you as she lightly strokes herself to full erectness. It\'s a short trip, and in a few pumps, she\'s ready to go. You glance around the bar, making sure all possible prying eyes are occupied. Feeling satisfied with your findings, you slyly shuffle your way under the table.' );
		...
		if( CoC.player.pregnancyIncubation < 100 && CoC.player.pregnancyIncubation > 0 ) {
			MainView.outputText( ' than it already was' );
		}
		MainView.outputText( '.  You slide your way back into your seat with some difficulty, fighting the wriggling, bouncing mass that is your stomach over the now shrinking room between the seat and table.  By now, some of the patrons have realized what has happened, pointing, whispering, rubbing their stomachs, and pulling their hands away from each other to symbolize your new... asset.' );
		...
		MainView.outputText( '\n\nThe waitress approaches as Urta releases you, glancing around the bar at the commotion, "<i>Quite a scene ya caused, ' + CoC.player.mf( 'cutey', 'girly' ) + ',</i>" she looks at your stomach, "<i>Would you like anything to, uh... wash that down with?</i>"' );
		...
		this.urtaLove( 2 );
		if( Utils.rand( 2 ) === 0 ) {
			MainView.outputText( CoC.player.modThickness( 100, 2 ) );
		}
		if( Utils.rand( 2 ) === 0 ) {
			MainView.outputText( CoC.player.modTone( 0, 2 ) );
		}
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	...
	Urta.prototype.urtaAndRaphaelSurprise = function() {
		MainView.clearOutput();
		MainView.outputText( 'Letting your lips split into a mischievious grin, you lean across the table and whisper into Urta\'s fluffy ear.  "<i>Why don\'t you meet me in one of the back rooms with the lights off in about thirty minutes?  I\'ll see if I can get a friend, we can make a party of it.</i>"' );
		...
		if( CoC.player.hasCock() ) {
			MainView.outputText( ' and [eachCock] experience a twinge of excitement' );
		}
		MainView.outputText( '.  Grabbing a napkin and a pen from an accommodating waitress, you jot out a hasty missive:' );
		MainView.outputText( '\n\n<i>Dearest Rogue,' );
	...
	Urta.prototype.raphaelAndUrtaPartTwo = function() {
		MainView.clearOutput();
		MainView.outputText( ImageManager.showImage( 'urta-raphael-3some', 'vert' ), false );
		MainView.outputText( 'Narrow, familiar paws suddenly encircle your waist, and a puff of hot breath exhales past your ear.  "<i>Greetings, my supple swan.</i>"' );
		...
		if( CoC.player.tailType > AppearanceDefs.TAIL_TYPE_NONE ) {
			MainView.outputText( 'lift your tails and ' );
		}
		MainView.outputText( 'try to relax with her thick slab of penis so close to your backdoor, hoping she\'ll be gentle.  A drunken twinge of arousal knifes through your [vagina] as you imagine her forcefully raping your hind-end, and Raphael can certainly feel the convulsive fluttering of your love tunnel as you sink into your imaginings.  The gray vixen nips at your ear as she presses against your [asshole]' );
		if( CoC.player.analCapacity() < 30 ) {
			MainView.outputText( ', slowly stretching your tight sphincter with constant pressure.  You\'re not really loose enough back there for this to be entirely comfortable, but the incredible fullness her length provides has a way of muting the pain of stretching.' );
		} else if( CoC.player.analCapacity() < 60 ) {
			MainView.outputText( ', slowly sliding into your sphincter.  You groan into your other lover\'s mouth as your anus is claimed by the thick horse-pole, and you do your best to relax around the butthole-stretching shaft.' );
		} else {
			MainView.outputText( ', easily slipping inside your well-practiced anus.  You groan into your other lover\'s mouth as the thick cock claims your ready asshole, and you\'re glad your butthole was so well prepared to handle having a hot meat pole injected into it.' );
		}
		CoC.player.buttChange( 60, true, true, false );
	...
	Urta.prototype.urtaGooTesticleVoreRuinedOrgasms = function() {
		MainView.clearOutput();
		MainView.outputText( ImageManager.showImage( 'urta-goo-fuck' ), false );
		MainView.outputText( 'You tell your vixen lover that you\'ve got something a bit... special in mind this time.  You surge up onto the bed and gently flip Urta onto her back.  Her horsecock bobs enticingly in front of you, already as hard as ' );
		if( !EngineCore.silly() ) {
			MainView.outputText( 'a steel rod' );
		} else {
			MainView.outputText( 'Marethian algebra' );
		}
		MainView.outputText( '.' );
	...
	Urta.prototype.urtasRuinedOrgasmsFromGooPartII = function() {
		...
		MainView.outputText( 'You awaken the next morning to a panicked shout.  You bolt upright...  Or at least you try to.  Something is holding you tight, binding you in place.  You try to open your eyes, but nothing but blackness surrounds you.  Struggling, you try and wrench yourself free of your bindings.' );
		...
		var edrynAvailable = !SceneLib.edryn.pregnancy.isPregnant;
		if( edrynAvailable ) {
			MainView.outputText( 'set of hoofsteps' );
		} else {
			MainView.outputText( 'pair of footsteps' );
		}
		MainView.outputText( ' enter whatever room you\'re in. "<i>Hey, Urta, you haven\'t shown up for your shift today so...  Great googily moogily!  What happened to you?!</i>" a voice you ' );
		if( edrynAvailable ) {
			MainView.outputText( 'recognize as Edryn\'s' );
		} else {
			if( CoC.flags[ kFLAGS.MET_OLIVIA ] === 0 ) {
				MainView.outputText( 'recognize as Olivia\'s' );
			} else {
				MainView.outputText( 'don\'t recognize' );
			}
		}
		MainView.outputText( ' shouts.' );
		...
		if( edrynAvailable ) {
			...
			MainView.outputText( 'Olivia' );
		}
		MainView.outputText( ', I\'m going to need your help with this particular problem.</i>"' );
		MainView.outputText( '\n\nThere\'s a moment of silence, and then, "<i>' );
		if( edrynAvailable ) {
			MainView.outputText( 'All right, but I\'m still charging you for this one' );
		} else {
			MainView.outputText( 'Uhm, yes Captain, whatever you need' );
		}
		MainView.outputText( '.</i>"  You hear armor falling to the ground while Urta rises from wherever she was sitting; you\'ve been all over the house so many times it\'s hard to keep track of where you are.' );
		...
		MainView.outputText( '\'s cunt, if that wet squelching noise is any indication.  She thrusts all the way in inside in one go, making ' );
		if( edrynAvailable ) {
			MainView.outputText( 'Edryn moan in pleasure' );
		} else {
			MainView.outputText( 'Olivia shout with surprise' );
		}
		MainView.outputText( '.  Clearly, Urta\'s not messing around anymore.  She pulls out and thrusts back in, and with that thrust you sway, slapping into ' );
		if( edrynAvailable ) {
			MainView.outputText( 'Edryn' );
		} else {
			MainView.outputText( 'Olivia' );
		}
		MainView.outputText( '\'s underside.  The contact is electric.  The rough slap would have sent shivers down your spine, if you\'d had one.  It makes your body quake and convulse with gusto.' );
		...
		MainView.outputText( '.  Your entire, gooey body feels like one giant clitoris, so sensitive, so vulnerable to pleasure.  With every slap, your body begins to crave release even more.  The pleasure builds with every thrust and accompanying slap.  It grows to the brink of orgasm, and surges forward, with no end in sight.' );
		...
		MainView.outputText( '.  Your eyes search around, seeing nothing but darkness once more.  You extend your gooey self, trying to tell where you are.  Wherever it is, it\'s tight... and there seems to be some sort of plug holding you in.' );
		MainView.outputText( '\n\n<b>You\'re inside ' );
		if( edrynAvailable ) {
			MainView.outputText( 'Edryn' );
		} else {
		...
		MainView.outputText( ' voices her displeasure, "<i>What the... I look like I\'m pregnant!  With septuplets!</i>"' );
		MainView.outputText( '\n\nUrta lets out a nervous chuckle.  "<i>Sorry about that.  But this should be easier to deal with.  ' );
		if( edrynAvailable ) {
			MainView.outputText( 'All you need to do is push, right?' );
		} else {
			MainView.outputText( 'Let\'s just sit you back, and then all you need to do is push.' );
		}
		MainView.outputText( '</i>"  Your new host grunts in discomfort' );
		if( edrynAvailable ) {
			MainView.outputText( ', but obliges, sitting back on something, perhaps a bed.  You feel yourself sloshing around oddly as she lies on her back' );
		}
		MainView.outputText( '.  "<i>Now push,</i>" Urta says, encouragingly.' );
		...
		MainView.outputText( '.  The first thing out, thankfully is your head.  Bright light greets your eyes, almost stinging them.  You look around, seeing ' );
		if( edrynAvailable ) {
			MainView.outputText( 'Edryn clutching Urta\'s hand' );
		} else {
			MainView.outputText( 'Olivia lying back on the bed, clutching Urta\'s hand' );
		}
		MainView.outputText( ', panting heavily.  You surge outwards, first your shoulders, followed by one arm, and then the next.  The rest of your body comes out like a flood with one last push from your former host, sending you splattering onto the ground beneath you.  An exasperated, but relieved, sigh comes from ' );
		if( edrynAvailable ) {
			MainView.outputText( 'Edryn' );
		} else {
			MainView.outputText( 'Olivia' );
		}
		MainView.outputText( ', one that you share.' );
	...
	//if(CoC.flags[kFLAGS.URTA_PETPLAY_DONE] === 0 && this.urtaLove()) MainView.outputText('  <b>A new, shiny leather collar lies half-concealed under a cushion, as though its owner haphazardly tried to bury it in a rush.</b>');;
	...
	Urta.prototype.urtaPetPlayDeletedForeverBecauseThirdProovedMeWrongAboutDice = function() {
		MainView.clearOutput();
		//First Time;
		if( CoC.flags[ kFLAGS.URTA_PETPLAY_DONE ] > 0 ) {
			this.wearZeCollar();
			return;
		}
		MainView.outputText( 'You hold up the leather collar you found in the previous room, asking when she picked it up.' );
	...
	Urta.prototype.wearZeCollar = function() {
		MainView.clearOutput();
		if( CoC.flags[ kFLAGS.URTA_PETPLAY_DONE ] === 0 ) {
			MainView.outputText( 'You reach down into the trashcan, unclasp the collar and slip it on.  You tighten it until it fits snugly against your ' + CoC.player.skinFurScales() + ', but isn\'t otherwise uncomfortable.  With a satisfying <b>click</b>, the clasp snaps shut, so you know there\'s no going back.  Urta\'s mouth drops as she watches you do this, completely at a loss for words.  Her cock, on the other hand, hardens, knowing just what to do.' );
			MainView.outputText( '\n\nYou sink to your knees and then onto your hands in front of her, brazenly displaying your submission to the vixen goddess before you.  She leans back slightly, stunned by this action.  It takes her a minute to recover and pick her jaw up off the floor.  You dutifully wait, silently, until Urta issues a command.  She seems to sense this and clears her throat, clearly embarrassed and nervous.  "<i>Oh, um, good ' + CoC.player.mf( 'boy', 'girl' ) + ',</i>" she murmurs, patting your head.  You rub your face into her palm, which brings a smile to Urta\'s face.' );
			...
			if( CoC.player.hasCock() ) {
				MainView.outputText( 'harder' );
			}
			if( CoC.player.gender === 3 ) {
				MainView.outputText( ' and ' );
			}
			if( CoC.player.hasVagina() ) {
				MainView.outputText( 'wetter' );
			}
			MainView.outputText( ' at the touch, but you can\'t help but wonder about her words.  What else is there to being a good dog?' );
		...
		//Second Time;
			...
			if( CoC.player.tailType > AppearanceDefs.TAIL_TYPE_NONE ) {
				MainView.outputText( ', tail wagging' );
			}
			MainView.outputText( '.  Urta beams at you and pats your head lovingly.' );
		}
		//(Jump down to Scene Proper);
		//Repeats;
		else {
			MainView.outputText( 'You spy the collar sitting on a nearby dresser and pick it up.  You run your fingers over the engraved words on the collar\'s tag that reads \'<i>Property of Urta</i>\'.  Slipping it on, you also strip down until you\'re as nude as the day you were born, and kneel in front of your Owner' );
			if( CoC.player.tailType > AppearanceDefs.TAIL_TYPE_NONE ) {
				MainView.outputText( ', tail wagging' );
			}
			MainView.outputText( '.' );
		...
		if( CoC.player.tailType === AppearanceDefs.TAIL_TYPE_NONE ) {
			...
			if( CoC.player.tailType === AppearanceDefs.TAIL_TYPE_NONE && CoC.player.ass.analWetness > 0 ) {
				MainView.outputText( '  Despite your embarrassment, or perhaps because of it, you find your asshole getting wet with desire, practically salivating in anticipation of that thick plug.' );
			}
	...
	Urta.prototype.doggyStyle = function() {
		MainView.clearOutput();
		MainView.outputText( ImageManager.showImage( 'urta-home-petplay-doggy' ), false );
		MainView.outputText( '"<i>Hmm,</i>" Urta wonders aloud, bringing a finger up to her mouth in mock-thoughtfulness.  "<i>What should I do with my little pet now that ' + CoC.player.mf( 'he', 'she' ) + '\'s all collared up?</i>"  Your Owner walks around you slowly, assessing your body.' );
		MainView.outputText( '\n\n"<i><b>Sit</b>,</i>" she says, a commanding voice coming easily.  Before you can even think about it, you immediately find your butt touching the smooth, cool floor.  Urta smirks and pats you on the head.  "<i>Good pet.  <b>Down.</b></i>"  Again, you follow her instruction immediately, lowering your front to the ground while simultaneously lifting your ass into the air.  She circles around and kneels alongside of you.  "<i><b>Stay</b>,</i>" she commands.  You don\'t dare move as she leans in, running a furry hand along the ' + CoC.player.skinFurScales() + ' of your back, which you can\'t help but arch, and over your [butt].  Your ' );
		if( CoC.player.tailType > AppearanceDefs.TAIL_TYPE_NONE ) {
			MainView.outputText( 'tail' );
		} else {
			MainView.outputText( 'tailplug' );
		}
		MainView.outputText( ' wags of its own accord, happily swishing back and forth.  Urta\'s hand dips between your buttocks, over your ' );
		if( CoC.player.tailType > AppearanceDefs.TAIL_TYPE_NONE ) {
			MainView.outputText( 'asshole' );
		} else {
			MainView.outputText( 'plugged asshole' );
		}
		MainView.outputText( ', and between your thighs, ' );
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'grasping your [cock biggest]' );
		} else {
			MainView.outputText( 'her fingers slipping ever-so-slightly into the folds of your pussy' );
		}
		MainView.outputText( '.' );
		MainView.outputText( '\n\nYou moan appreciatively as she runs her fingers along it, her fingers encircling your ' );
		if( CoC.player.hasCock() ) {
			MainView.outputText( '[cockHead biggest]' );
		} else {
			MainView.outputText( '[clit]' );
		}
		MainView.outputText( ' while her other hand keeps your upper body firmly down.  With every stroke your body burns just a bit hotter, your breath coming a bit shorter.  ' );
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'Her thumb rubs against the sensitive underside of your cock' );
		} else {
			MainView.outputText( 'One finger slips into you, all the way down to the knuckle, and swivels around to caress your g-spot with expert accuracy' );
		}
		MainView.outputText( ', making you bite your lip in ecstasy.  You want to cry out right now, but you know you can\'t break character right now.  Not this early in.  The strokes come faster, and you can feel the orgasmic flames coming on; your limbs tingle and your legs are already wobbling.' );
		...
		MainView.outputText( '\n\n"<i>Oh, we can\'t have you coming too soon,</i>" she explains, slapping your ass lightly.  You flinch a little at the spanking, the erotic combination of pain and pleasure feeling oh-so good on your ' + CoC.player.skinFurScales() + '.  "<i>A good pet will only cum after their owner, after all.</i>"  You whine pitifully, hoping it won\'t take all that long at least.' );
		...
		if( CoC.player.hasCock() ) {
			MainView.outputText( '[cock biggest] does the same' );
		} else {
			MainView.outputText( '[vagina] grows slicker' );
		}
		MainView.outputText( '.  She really is going to tease you as long as she can, you think to yourself.' );
		...
		MainView.outputText( ', taking over where it left off just moments ago.  ' );
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'She wraps her fingers around your shaft, slowly pumping up and down, taking a moment to tease your [cockHead biggest] every few strokes.' );
		} else {
			MainView.outputText( 'She buries two fingers deep inside your cunt, followed by a third, each one twisting around to rub at your g-spot softly.' );
		}
		MainView.outputText( '  The hand on your chest switches to ' );
		if( CoC.player.totalNipples() === 2 ) {
			MainView.outputText( 'the ' );
		} else {
			MainView.outputText( 'an ' );
		}
		MainView.outputText( 'other nipple, tweaking and teasing that one while she simultaneously ' );
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'jerks you off' );
		} else {
			MainView.outputText( 'rubs you from the inside out' );
		}
		MainView.outputText( '.  Again comes the flame of orgasmic bliss, lapping at your tingling extremities.  Your body feels suddenly electric, so overcharged with sweet, sweet pleasure that you find it hard to think.  Low moans escape your throat, entirely unbidden, while ' );
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'pre-cum dribbles from your dick like a leaky faucet' );
		} else {
			MainView.outputText( 'your folds grow even more damp, constantly drizzling fluids that run down your ass crack, pooling on the floor like a lewd lake' );
		}
		if( CoC.player.hasCock() ) {
			MainView.outputText( ', and to top it all off, you find your [leg] inexplicably kicking in mid-air' );
		}
		MainView.outputText( '.' );
		...
		if( CoC.player.tailType === AppearanceDefs.TAIL_TYPE_NONE ) {
			MainView.outputText( ', while she grasps the plug in your ass firmly and pulls. It comes out easily with a wet <b>pop</b>, making your ass feel suddenly empty.  You guess it won\'t be empty for long, however' );
		}
		MainView.outputText( '.  Finally she rubs it against your [asshole], teasing you with every brush of skin against skin.' );
		...
		if( CoC.player.biggestTitSize() <= 5 ) {
			MainView.outputText( 'resting on your folded arms' );
		} else {
			MainView.outputText( 'resting on your enormous boobs' );
		}
		MainView.outputText( ', while your ass remains high in the air, all the better to be fucked by your hermaphroditic Owner.  Urta pulls out a few inches just to push in even deeper, and the feeling is exquisite as, with each thrust, you get the sensation of being filled just a little bit more.  By the time her large balls slap into your ' );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( 'own' );
		} else if( CoC.player.hasCock() ) {
			MainView.outputText( 'painfully erect member' );
		} else {
			MainView.outputText( 'slit' );
		}
		MainView.outputText( ', you are overwhelmed by the full, content feeling.  Your eyes roll back in their sockets, and it\'s hard to focus on anything but the twenty inch dick buried in your sensitive ' );
		if( !EngineCore.silly() ) {
			MainView.outputText( 'ass' );
		} else {
			MainView.outputText( 'boypussy' );
		}
		MainView.outputText( '.' );
		CoC.player.buttChange( 60, true, true, false );
		...
		MainView.outputText( ' in such a way that it tingles powerfully, fueling your impending orgasm even more.  Locked in this cadence, she pounds your ass for what seems like hours.  The air grows damp and sweat drips from the two of you, only adding to your desire.  It\'s not long after until your nostrils are assaulted by the salty scent of sex.  Your ' );
		if( CoC.player.hasCock() ) {
			MainView.outputText( '[cock] slaps against your belly roughly with every passing second, smearing thick globules of precum which drip down your belly, tickling your flesh like little balls of distilled pleasure' );
		}
		if( CoC.player.gender === 0 ) {
			MainView.outputText( ', while your' );
		}
		if( CoC.player.hasVagina() ) {
			MainView.outputText( ' unused pussy aches to be filled or touched, its lips puffy and red, sensitive to every hard slap of your foxy lover\'s balls' );
		}
		MainView.outputText( '.' );
		...
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'groin, sending a wave of tremors down your cock, which suddenly erupts, covering your belly and [chest] in warm gooey goodness.' );
			if( CoC.player.cumQ() >= 500 ) {
				MainView.outputText( '  It continues to jerk on its own, unloading its massive load until you\'re laying in a puddle of your own semen.' );
			}
			if( CoC.player.hasVagina() ) {
				MainView.outputText( '  It then travels along to your ' );
			}
		}
		if( CoC.player.hasVagina() ) {
			MainView.outputText( '[vagina], which, despite being devoid of cock, clamps down anyway, gushing with femcum, drenching your Owner\'s balls and legs in sticky, clear fluids.' );
		}
	...
	Urta.prototype.getAPetReward = function() {
		MainView.clearOutput();
		MainView.outputText( ImageManager.showImage( 'urta-home-petplay-reward' ), false );
		MainView.outputText( '"<i>Well, you\'ve been a good little pet so far,</i>" Urta says, patting you on the head. "<i>I think a little... reward is in order.</i>"  You quirk an eyebrow, wondering about that pause, but wag your ' );
		if( CoC.player.tailType > AppearanceDefs.TAIL_TYPE_NONE ) {
			MainView.outputText( 'tail' );
		} else {
			MainView.outputText( 'tailplug' );
		}
		MainView.outputText( ' enthusiastically.' );
		MainView.outputText( '\n\nUrta holds up a hand at you and says in a quite commanding voice, "<i>Sit.</i>"  You immediately drop your butt to the floor, getting a little thrill from obeying.  "<i>Good ' + CoC.player.mf( 'boy', 'girl' ) + '.  Now stay.</i>"  Once she\'s satisfied you\'re staying put, your Owner walks out of the room, her ass swaying tantalizingly as she disappears around the corner.  You wait, naked and with your legs and butt resting on the floor, for several minutes.  You find yourself actually getting a little ' );
		if( CoC.player.hasCock() ) {
		...
		MainView.outputText( ', oddly enough, thinking of what your \'reward\' could be.  Urta returns several minutes later, with one hand holding some kind of metal dish... a food bowl for a dog!' );
		if( EngineCore.silly() ) {
			MainView.outputText( '  In her other hand is what looks to be a jar of peanut butter.  What use could that have?' );
		}
		...
		if( EngineCore.silly() ) {
			MainView.outputText( ', though you wonder if perhaps that\'s what the peanut butter is for' );
		}
		MainView.outputText( '.  Your Owner stretches a foot out towards you, running her stocking-clad paw along your side.  The little tufts of fur protruding from the fishnet tickle your skin in a sensual fashion, and you can\'t stop your spine from stiffening at the contact.' );
		...
		if( CoC.player.biggestTitSize() < 1 ) {
			MainView.outputText( 'onto your chest' );
		} else {
			MainView.outputText( 'between your breasts' );
		}
		MainView.outputText( '.  She takes a moment to encircle one of your nipples, which stiffens to attention along with ' );
		if( CoC.player.totalNipples() === 2 ) {
			MainView.outputText( 'its twin' );
		} else {
			MainView.outputText( 'the others' );
		}
		MainView.outputText( ', before sending the paw down further where it makes contact with your ' );
		if( CoC.player.hasCock() ) {
			MainView.outputText( Descriptors.multiCockDescriptLight() );
		} else {
			MainView.outputText( '[vagina]' );
		}
		MainView.outputText( '.  There\'s nothing you can do to stifle the moan of pleasure that escapes your lips as your Owner\'s paw travels the length of your ' );
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'rapidly hardening shaft.' );
		} else {
			MainView.outputText( 'moistening slit.' );
		}
		...
		if( CoC.player.hasCock() ) {
			...
			MainView.outputText( 'cunt lips' );
		}
		MainView.outputText( ' with a paw, until you bite your lip and whine at her.  She gives you a little smirk and the foot retreats.  "<i>All right then, pet, come and get your bone,</i>" Urta gestures down at the half-erect cock sprouting from between her thighs' );
		//silly:;
		if( EngineCore.silly() ) {
			MainView.outputText( ', while she opens the jar of peanut butter and smears a dollop across the flat head.  Is... is that supposed to be enticement?  She really is treating you like a real dog' );
		}
		MainView.outputText( '.  You crawl towards her nervously, and start to reach up to grasp her cock, but again she mutters, "<i>Ah-ah, good doggies don\'t use their hands.</i>"' );
		MainView.outputText( '\n\nNodding, you extend your tongue, running it along the length of her hardening shaft' );
		if( EngineCore.silly() ) {
			MainView.outputText( ', licking all of the peanut butter off the tip' );
		}
		MainView.outputText( '.  Urta shivers and lets out a "<i>Mm,</i>" while her pillar of horsecock reaches its full size, clocking in at what has to be a good twenty inches.  ' );
		if( EngineCore.silly() ) {
			MainView.outputText( '  Your owner uses her fingers to smear another large dollop of peanut butter along her own shaft, giving you an almost sadistic smirk.' );
		}
		MainView.outputText( '  You smooch and lick your way down her shaft, lapping at every little vein while Urta leans back and enjoys your ministrations.  Little gasps of pleasure escape from her lips, while moans leave yours as you smooch and suckle your way down to the base of that towering horsecock.' );
		MainView.outputText( '\n\nYour foxy Owner\'s prick glistens by the time you finally take it into your mouth' );
		if( EngineCore.silly() ) {
			MainView.outputText( ', the peanut butter now long forgotten' );
		}
		MainView.outputText( ', stretching your jaw wide.  ' );
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'Your own cock remains almost painfully rock hard, bobbing awkwardly beneath you, dripping precum onto the floor and bowl.' );
		}
		if( CoC.player.hasVagina() ) {
			if( CoC.player.gender === 3 ) {
				MainView.outputText( '  Not to mention' );
			} else if( CoC.player.hasVagina() ) {
				MainView.outputText( '  Meanwhile' );
			}
			MainView.outputText( ', your pussy is as wet as it\'s ever been, constantly leaking juices that run down your legs, pooling on the floor.' );
		}
		MainView.outputText( '  You swirl your tongue around Urta\'s flare, paying homage to the horsecocked goddess.  Lapping at the slit for a moment, you slip your ' );
		...
		MainView.outputText( 'tongue along the vixen\'s thick cumvein, which pulses lightly.  Urta groans and runs a hand through your ' + Descriptors.hairDescript() + ', gripping it solidly.  You can\'t help as you\'re forced further onto the warm rod.' );
		...
		MainView.outputText( 'tantalizingly.' );
		...
		MainView.outputText( ' a little twirl with a soft, padded toe.  You can\'t help but moan into the bowl as you go, your legs shivering and stomach twisting with desire.  You continue to lap at the bowl eagerly downing mouthful after mouthful and being rewarded with more pleasure.  Even when you\'ve licked the bowl clean, you find yourself lowering your head further and slurping away the remnants that splattered onto the floor.  Urta seems to find this satisfactory, as she picks up the pace, ' );
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'jerking off your cock between two toes' );
		} else {
			MainView.outputText( 'rubbing your little love button like mad with one soft toe' );
		}
		MainView.outputText( ', until you just can\'t handle any more.' );
		...
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'The cock between your legs, meanwhile, stiffens, briefly becoming harder than you\'ve ever felt before, before blasting the floor with jet after jet of silky white cum.' );
		} else {
			MainView.outputText( 'Meanwhile, your vagina clamps on nothing, achingly, painfully empty.  Juices spray out onto Urta\'s foot and down your thighs, soaking the floor beneath you in yet more slippery femcum.' );
		}
		MainView.outputText( '  Your Owner doesn\'t just let it end there, however, she continues her paw-assault on your crotch, stroking and rubbing until you become little more than a gibbering mess of orgasm-infused flesh laying in a puddle of your own sexy goo.' );
		CoC.flags[ kFLAGS.URTA_TIME_SINCE_LAST_CAME ] += 2;
		CoC.flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] += 2;
		MainView.outputText( '...' );
		CoC.player.orgasm();
	...
	Urta.prototype.urtaTakesPCOnWalkies = function() {
		MainView.clearOutput();
		MainView.outputText( ImageManager.showImage( 'urta-home-petplay-walkies' ), false );
		MainView.outputText( '"<i>Ah, I know just what we should do!</i>" She claps her hands together and rummages through her dresser.  Urta throws on a simple little yellow sundress and makes her way to the door.  "<i>Come on, pet</i>" she says with a mischevious smirk, patting her leg and holding up a leather leash. "<i>It\'s time to go for a walk!</i>"' );
		...
		if( CoC.player.hasCock() ) {
			MainView.outputText( '[eachCock] grows hard beneath you' );
		}
		if( CoC.player.gender === 3 ) {
			MainView.outputText( ' and ' );
		}
		if( CoC.player.hasVagina() ) {
			MainView.outputText( 'your [vagina] moistens, ready to be on display' );
		}
		MainView.outputText( '.' );
		...
		MainView.outputText( ' a nervous little shake.  Urta smiles and opens the door wide, making you squint in the bright daylight.  You feel a tugging at your neck, and realize Urta\'s already set off.  You scamper off after her on all fours, not wanting to disappoint your Owner.  The two of you travel through some back alleys, thankfully free of strangers, until you come out into the main thoroughfare of the city.' );
		...
		if( CoC.player.biggestTitSize() >= 5 ) {
			MainView.outputText( 'the breasts hanging from your chest and ' );
		}
		MainView.outputText( 'your openly displayed rear end.  ' );
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'Pre-cum drizzles from the end of your painfully erect cock' );
		} else {
			MainView.outputText( 'Sweet juices flow from your cunt, drizzling down onto the ground' );
		}
		MainView.outputText( ', creating a trail of horniness alongside Urta\'s.' );
		...
		MainView.outputText( '.  You feel like you really are a pet, out for a walk with your Owner.  It\'s an odd, and slightly freeing revelation, and once you come to grips with it, your entire experience changes. You find yourself trotting ahead of Urta, happily displaying yourself to all who wish to see.' );
		...
		MainView.outputText( '\n\nYou ' + CoC.player.mf( 'laugh', 'giggle' ) + ' and confirm you had a great time as well.  Urta gives you a sheepish look, before asking if you\'d ever want to do that again.  You lean in for a kiss, and let that answer the question.' );
		EngineCore.dynStats( 'lus', 15 );
		CoC.flags[ kFLAGS.URTA_TIME_SINCE_LAST_CAME ] = 0;
		CoC.flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] += 5;
		if( CoC.flags[ kFLAGS.PC_FETISH ] > 0 ) {
			MainView.outputText( '  <b>You\'re so fucking turned on after exhibiting yourself in such a way that you\'re dripping EVERYWHERE.  Such a good doggie.</b>' );
			EngineCore.dynStats( 'lus=', 100, 'resisted', false );
		}
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
