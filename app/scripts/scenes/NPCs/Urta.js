...
angular.module( 'cocjs' ).run( function( SceneLib, $rootScope, $log, ImageManager, Appearance, CockTypesEnum, PregnancyStore, CoC_Settings, PerkLib, Descriptors, AppearanceDefs, EventParser, CoC, kFLAGS, Utils, StatusAffects, EngineCore ) {
	//Urta variables;
	...
	//11+ = flaunt it;
	//CoC.getInstance().flags[kFLAGS.URTA_TIME_SINCE_LAST_CAME] = hours turned off;
	//0 = ready to rock!;
	//Urtas horsey bits =;
	//LxW = 20x3;
	//Luv Lock - CoC.getInstance().flags[kFLAGS.URTA_PC_LOVE_COUNTER]:;
	//0 = normal, 1 = love, -1 = never love, CoC.getInstance().flags[kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY] -1 = hate;
	/*Luv rating! - CoC.getInstance().flags[kFLAGS.URTA_PC_AFFECTION_COUNTER]
	...
	Urta.prototype.timeChange = function() {
		...
		$log.debug( '\nUrta time change: Time is ' + CoC.getInstance().time.hours + ', incubation: ' + this.pregnancy.incubation + ', event: ' + this.pregnancy.event );
		if( CoC.getInstance().flags[ kFLAGS.NEED_URTA_LETTER ] === 1 && CoC.getInstance().time.hours === 6 ) {
		...
		if( this.pregnancy.incubation === 0 && (this.pregnancy.type === PregnancyStore.PREGNANCY_BEE_EGGS || PregnancyStore.PREGNANCY_DRIDER_EGGS) ) {
			...
			CoC.getInstance().flags[ kFLAGS.URTA_EGGS ] = 0;
			CoC.getInstance().flags[ kFLAGS.URTA_FERTILE_EGGS ] = 0;
		}
		if( CoC.getInstance().flags[ kFLAGS.URTA_TIME_SINCE_LAST_CAME ] > 0 ) {
			CoC.getInstance().flags[ kFLAGS.URTA_TIME_SINCE_LAST_CAME ]--; //Count down timer for urta's lust
		}
		if( CoC.getInstance().flags[ kFLAGS.URTA_EGG_FORCE_EVENT ] > 0 ) {
			CoC.getInstance().flags[ kFLAGS.URTA_EGG_FORCE_EVENT ]--; //Countdown to urta freakout
		}
		//Urta egg freak out;
		if( CoC.getInstance().flags[ kFLAGS.URTA_EGG_FORCE_EVENT ] > 0 && CoC.getInstance().time.hours > 6 && CoC.getInstance().time.hours < 18 && CoC.getInstance().flags[ kFLAGS.URTA_EGG_FORCE_EVENT ] < 12 ) {
		...
		if( CoC.getInstance().flags[ kFLAGS.URTA_ANGRY_AT_PC_COUNTDOWN ] > 1 ) {
			CoC.getInstance().flags[ kFLAGS.URTA_ANGRY_AT_PC_COUNTDOWN ]--;
			if( CoC.getInstance().flags[ kFLAGS.URTA_ANGRY_AT_PC_COUNTDOWN ] < 1 ) {
				CoC.getInstance().flags[ kFLAGS.URTA_ANGRY_AT_PC_COUNTDOWN ] = 1;
			}
		}
		if( CoC.getInstance().time.hours > 23 ) {
			if( CoC.getInstance().flags[ kFLAGS.URTA_TIME_SINCE_LAST_CAME ] === 0 ) {
				CoC.getInstance().flags[ kFLAGS.URTA_CUM_NO_CUM_DAYS ]++;
			} else {
				CoC.getInstance().flags[ kFLAGS.URTA_CUM_NO_CUM_DAYS ] = 0;
			}
			if( CoC.getInstance().flags[ kFLAGS.URTA_PC_AFFECTION_COUNTER ] > 0 ) {
				CoC.getInstance().flags[ kFLAGS.URTA_PC_AFFECTION_COUNTER ] -= 0.5;
				if( CoC.getInstance().flags[ kFLAGS.URTA_PC_AFFECTION_COUNTER ] < 0 ) {
					CoC.getInstance().flags[ kFLAGS.URTA_PC_AFFECTION_COUNTER ] = 0;
				}
			}
		}
		return needNext;
	...
	Urta.prototype.urtaCapacity = function() {
		var bonus = CoC.getInstance().flags[ kFLAGS.TIMES_RUT_FUCKED_URTAS_CUNT ] * 5;
		if( bonus > 40 ) {
			bonus = 40;
		}
		if( CoC.getInstance().flags[ kFLAGS.URTA_TENTACLE_GAPED ] > 0 ) {
			return 500;
		}
		return 60 + bonus;
	};
	Urta.prototype.urtaLove = function( love ) {
		...
		if( CoC.getInstance().flags[ kFLAGS.URTA_PC_LOVE_COUNTER ] === -1 ) {
			return false;
		}
		if( love === 0 ) {
			return CoC.getInstance().flags[ kFLAGS.URTA_PC_LOVE_COUNTER ] === 1;
		}
		if( love > 0 ) {
			CoC.getInstance().flags[ kFLAGS.URTA_PC_AFFECTION_COUNTER ] += love;
		}
		if( love < 0 ) {
			CoC.getInstance().flags[ kFLAGS.URTA_PC_AFFECTION_COUNTER ] -= love;
		}
		if( CoC.getInstance().flags[ kFLAGS.URTA_PC_LOVE_COUNTER ] === 1 ) {
			...
			if( love > 0 && CoC.getInstance().flags[ kFLAGS.AMILY_VISITING_URTA ] === 0 && SceneLib.amilyScene.amilyFollower() && CoC.getInstance().flags[ kFLAGS.AMILY_FOLLOWER ] === 1 ) {
				CoC.getInstance().flags[ kFLAGS.AMILY_NEED_TO_FREAK_ABOUT_URTA ] = 1;
			}
			return true;
		} else {
			return false;
	...
	Urta.prototype.urtaAvailableForSex = function() {
		return this.urtaFuckbuddy() && SceneLib.scylla.action !== SceneLib.scylla.SCYLLA_ACTION_FUCKING_URTA && CoC.getInstance().flags[ kFLAGS.URTA_ANGRY_AT_PC_COUNTDOWN ] === 0;
	};
	Urta.prototype.urtaFuckbuddy = function() { //Returns true if Urta is either the player's fuckbuddy or lover
		if( SceneLib.urtaQuest.urtaBusy() || CoC.getInstance().flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] === -1 ) {
			return false;
		}
		if( this.urtaJustFriends() ) {
			return false;
		}
		return (CoC.getInstance().flags[ kFLAGS.TIMES_FUCKED_URTA ] > 0);
	};
	Urta.prototype.urtaJustFriends = function() {
		return CoC.getInstance().flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] === 0 && CoC.getInstance().flags[ kFLAGS.URTA_PC_LOVE_COUNTER ] === -1;
	};
	Urta.prototype.urtaAtBar = function() { //Is Urta physically at the Wet Bitch?
		return (!SceneLib.urtaQuest.urtaBusy() && CoC.getInstance().flags[ kFLAGS.AMILY_VISITING_URTA ] !== 1 && CoC.getInstance().time.hours < 15 && CoC.getInstance().flags[ kFLAGS.KATHERINE_UNLOCKED ] !== 3);
	};
	Urta.prototype.urtaDrunk = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00146 ] === -1 ) {
			return false;
		}
		if( CoC.getInstance().time.hours > 12 && CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00146 ] < 1 ) {
			return true;
		}
		if( CoC.getInstance().time.hours > 8 && CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00146 ] >= 1 ) {
			return true;
		} else {
			return false;
		}
	...
	Urta.prototype.urtaOpenAboutEdryn = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00145 ] > 0 ) {
			return true;
		}
		//Did it come out from marble?;
		if( CoC.getInstance().flags[ kFLAGS.URTA_KNOWS_PC_HAS_MARBLE_FOLLOWER ] > 0 ) {
			return true;
		}
		//Did it come out from discussions?;
		if( CoC.getInstance().flags[ kFLAGS.URTA_OPEN_ABOUT_EDRYN ] > 0 ) {
			return true;
		}
		return false;
	};
	Urta.prototype.knockUpUrtaChance = function() { //Moved here from UrtaPregs since it needs to be accessed from outside Urta
		...
		//GTFO IF NOT FERTILE;
		if( CoC.getInstance().flags[ kFLAGS.URTA_FERTILE ] !== 1 ) {
			return;
		}
		//10% + up to 40% @ 1000mLs of cum, with bonus virility!;
		var chance = 10;
		var temp = CoC.getInstance().player.cumQ() / 25;
		if( temp > 40 ) {
			temp = 40;
		}
		chance += temp;
		//Bonus virility time!;
		temp = CoC.getInstance().player.virilityQ() * 100;
		if( temp > 50 ) {
			temp = 50;
		}
		chance += temp;
		...
		if( chance > Utils.rand( 100 ) ) {
			...
			CoC.getInstance().flags[ kFLAGS.URTA_PREGNANT_DELIVERY_SCENE ] = 0;
		}
	};
	Urta.prototype.urtaBarDescript = function() {
		...
		//Urta PISSED;
		if( CoC.getInstance().flags[ kFLAGS.URTA_ANGRY_AT_PC_COUNTDOWN ] > 1 ) {
			EngineCore.outputText( 'Urta is sitting at a table, swishing a bottle around and looking forlorn.  She glances up and sees you, then immediately turns away.', false );
			return false;
		}
		//	[PISSED URTA TALK];
		...
		//[Post Pissed Talk];
		if( CoC.getInstance().flags[ kFLAGS.URTA_ANGRY_AT_PC_COUNTDOWN ] === 1 ) {
			EngineCore.outputText( 'Urta is sitting at a table, swishing a bottle around and looking forlorn.  She looks up at you and sighs, "<i>Look, I still don\'t think we\'ll ever be the same, but to be frank, we\'re great in the sack and I\'ve been lonely.  For now, I\'ll put the past behind me.</i>"', false );
			CoC.getInstance().flags[ kFLAGS.URTA_ANGRY_AT_PC_COUNTDOWN ] = 0;
			return true;
		}
		//Raphael betrayed reward;
		if( CoC.getInstance().flags[ kFLAGS.RAPHEAL_COUNTDOWN_TIMER ] === -1 && CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00148 ] === 0 ) {
			EngineCore.outputText( 'Urta has an ecstatic grin plastered across her muzzle, and it only gets wider when she sees you.  Perhaps you should see what all the fuss is about?', false );
			return true;
		}
		if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00350 ] === 1 ) {
			EngineCore.outputText( 'Urta is sitting at her usual table, holding her head in her hands.  She doesn\'t even seem to notice you.', false );
			return true;
		}
		//[URTA ASHAMED];
		if( CoC.getInstance().flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] === -1 ) {
			EngineCore.outputText( 'You see Urta on a bench in the corner with her legs crossed nervously.  Her tail is curling tightly around her leg, hiding the massive surprise between her legs.  In her hand is a mostly empty bottle of whiskey.  She\'s wobbling back and forth in her seat, and keeping her head down, refusing to meet your gaze any time you spare a glance her way.  It looks like she wants nothing to do with you.', false );
			return true;
		}
		//Post Scylla Appearance;
		if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00145 ] === 0 && CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00143 ] > 0 ) {
			EngineCore.outputText( 'Urta\'s at her normal table, nursing a glass of water with nary a bottle of alcohol around.  She\'s fidgety and constantly glancing your way, but she never meets your eyes or acknowledges that she saw you.  Something is eating away at her.', false );
			return true;
		}
		//PREGNANT URTA;
		if( this.pregnancy.isPregnant ) {
			if( this.pregnancy.type === PregnancyStore.PREGNANCY_PLAYER ) {
				...
				//Egg Level 1:;
				if( CoC.getInstance().flags[ kFLAGS.URTA_EGGS ] < 20 ) {
					EngineCore.outputText( 'Urta is sitting quietly at her usual seat, sipping a mug of some non-alcoholic beer.  Though her clothes are still in place, you can just make out the bulging midriff stretching them taut, a consequence of all the eggs in her womb.' );
				} else if( CoC.getInstance().flags[ kFLAGS.URTA_EGGS ] < 40 ) {
					EngineCore.outputText( 'Urta is sitting rather uncomfortably at her usual seat.  Her swollen stomach is so large it keeps her from wearing her usual outfit, instead forcing her into a mini-shirt and a skirt bottom, allowing her midriff to be bare.  She occasionally runs a hand over the expanse, as if she can\'t believe it\'s actually there.' );
				}
				//Egg Level 3:;
				else {
					EngineCore.outputText( 'Urta\'s bloated belly forces her to sit side-saddle at her usual table, the lumpy, egg-stuffed mass ballooning out in front of her.  Forced into a midriff-baring ensemble, she frequently touches it, occasionally seeming to trace an egg through the taut skin.' );
				}
			}
			return true;
		}
		...
		if( CoC.getInstance().flags[ kFLAGS.URTA_TIMES_EGG_PREGGED ] > 0 && !this.pregnancy.isPregnant && CoC.getInstance().flags[ kFLAGS.URTA_FLATBELLY_NOTICE ] === 0 ) {
			EngineCore.outputText( 'Urta is seated at her usual spot in her usual dress, eagerly chugging down mug after mug of booze; her belly is washboard-flat again, and she\'s clearly making up for lost time after having given up alcohol for her \'pregnancy\'.' );
			CoC.getInstance().flags[ kFLAGS.URTA_FLATBELLY_NOTICE ] = 1;
			return true;
		}
		//[Love Urta Bar Appearance];
		if( CoC.getInstance().flags[ kFLAGS.URTA_PC_LOVE_COUNTER ] === 1 ) {
			if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00146 ] === -1 || !this.urtaDrunk() ) {
				EngineCore.outputText( 'Urta is sitting at her usual table, sipping a glass of wine and wearing a form-fitting evening gown of shimmering black.  She looks up at you, happiness filling her eyes when she notices you entering the bar.', false );
			}//(DRUNK);
			else {
				EngineCore.outputText( 'Urta is sitting at her usual table wearing a shimmering black dress.  She\'s pretty clearly sloshed judging by her bleary gaze as she looks up at you.  The front of her dress visibly tents, and she waves you over with a lecherous grin.', false );
			}
			return true;
		}
		if( CoC.getInstance().flags[ kFLAGS.TIMES_FUCKED_URTA ] === 0 && CoC.getInstance().flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] > -1 ) {
			EngineCore.outputText( 'You see Urta on a bench in the corner with her feet propped up on a stool.  Her tail is coiled tightly around her left leg, just like every other time you\'ve seen her.  The pretty gray fox gives you a slight nod before turning her attention back to the large square bottle of whiskey in front of her.', false );
			return true;
		}
		//[URTA FUCKBUDDIED];
		if( CoC.getInstance().flags[ kFLAGS.TIMES_FUCKED_URTA ] > 0 ) {
			EngineCore.outputText( 'You see Urta on a bench in the corner with her feet propped up on a stool.  She has a small mug on a table, quite different from the swill she chugged when you first met.  Her clothing is... far more risqué.  The top she\'s chosen for the moment is sheer to the extreme, making her proud black nipples visible to anyone who glances her way, and the short skirt she\'s wearing is nearly revealing her cunny.  ', false );
			if( CoC.getInstance().flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] < 11 ) {
				EngineCore.outputText( 'Thankfully she still has her other addition trapped under her tail, for now.', false );
			} else {
				EngineCore.outputText( 'Her half-hard horse-cock is plainly visible to anyone who spares her a glance, barely contained by a pair of tightly woven fishnet stockings.  She\'s looking at you and wiggling a finger your way...', false );
			}
			return true;
		}
		CoC_Settings.error( '' );
	...
	Urta.prototype.urtaBarApproach = function() {
		...
		var temp = null;
		var spank = null;
		if( (CoC.getInstance().player.hasCock() && CoC.getInstance().player.cockThatFits( this.urtaCapacity() ) >= 0) || CoC.getInstance().player.hasKeyItem( 'Deluxe Dildo' ) >= 0 ) {
			spank = this.spankTheShitOutOfUrtaAndMakeHerCreamHerselfFromProstateStimulationAloneLikeTheHornyDrunkenSlutSheReallyIs;
		}
		EngineCore.clearOutput();
		//Raphael Reward;
		if( CoC.getInstance().flags[ kFLAGS.RAPHEAL_COUNTDOWN_TIMER ] === -1 && CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00148 ] === 0 ) {
			CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00148 ] = 1;
			CoC.getInstance().player.gems += 1000;
			EngineCore.statScreenRefresh();
			EngineCore.outputText( ImageManager.showImage( 'urta-bar' ), false );
			EngineCore.outputText( 'Urta claps as you walk up to her with an exuberant smile spread across her vulpine face.  She proudly announces, "<i>You really turned in the Russet Rogue?!  I almost can\'t believe it.  Do you know how many women that rascal has fleeced out of their possessions?  The last count I made was over three dozen, and he even had the audacity to try it with me.  I\'m ashamed to admit it nearly worked, but once he saw ALL of me, well he couldn\'t keep up the facade.  I nearly caught him that night.</i>"\n\n', false );
			EngineCore.outputText( 'You smirk at her tale.  Knowing Raphael, seeing a horny Urta would\'ve made him white as a ghost.  She goes on to list some of his crimes, "<i>He\'s guilty of petty theft, disturbing the peace, robbery, lewd public behavior, lying under oath, defacing a public structure, destroying a public structure... I\'d probably have to get the paperwork to remember the rest.  In truth his shenanigans aren\'t usually a big deal individually, but they\'ve added up and garnered him a fairly sizable bounty.  He\'ll serve his punishment, and I hope he learns his lesson for a change.</i>"\n\n', false );
			EngineCore.outputText( 'Urta fishes out a rather sizable pouch and tosses it onto the table, the numerous gems inside tinkling loudly as it plops down in front of you.  You untie the silken cord that holds the leather bag closed and see hundreds of sparkling gems, perhaps even a thousand.  Wow!  Raphael must have pissed off the wrong people!\n\n', false );
		...
		//HERE WE GOEZ!;
		if( CoC.getInstance().flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] >= 5 && ((CoC.getInstance().player.inRut && CoC.getInstance().player.hasCock()) || (CoC.getInstance().player.inHeat && CoC.getInstance().player.hasVagina() )) ) {
			if( this.urtaDrunk() ) {
		...
		if( CoC.getInstance().flags[ kFLAGS.URTA_TIME_SINCE_LAST_CAME ] > 0 && !this.urtaDrunk() ) {
			EngineCore.outputText( ImageManager.showImage( 'urta-bar' ), false );
			//Cockblock marble convo;
			if( SceneLib.marbleScene.marbleAtCamp() && CoC.getInstance().flags[ kFLAGS.URTA_KNOWS_PC_HAS_MARBLE_FOLLOWER ] === 0 ) {
				CoC.getInstance().flags[ kFLAGS.URTA_KNOWS_PC_HAS_MARBLE_FOLLOWER ] = 1;
				this.marbleCockuBlockuUrta();
				return;
			}
			//Twu wuv talk;
			if( CoC.getInstance().flags[ kFLAGS.URTA_PC_AFFECTION_COUNTER ] >= 30 && CoC.getInstance().flags[ kFLAGS.URTA_PC_LOVE_COUNTER ] === 0 ) {
		...
		if( CoC.getInstance().flags[ kFLAGS.URTA_PC_LOVE_COUNTER ] === 1 ) {
			...
			if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00146 ] === -1 || !this.urtaDrunk() ) {
				EngineCore.outputText( ImageManager.showImage( 'urta-bar-drunk' ), false );
				EngineCore.outputText( 'You walk up to Urta and you see her pivot herself to the side.  Her dress visibly tents at the sight of you, and you smirk as you realize she shifted in position to avoid slamming her hardening cock into the bottom of the table.  You cuddle close and wrap an arm around your lover, giving her a quick kiss on the lips and fondling her through her dress.  The tent immediately darkens as a rush of pre soaks into the material, and Urta half-pants, half-asks, "<i>Oooh, lover, what do you have in mind today?</i>"\n\n', false );
				EngineCore.outputText( 'You could go back to her place, suck her off under the table, or eat her out under the table.', false );
				if( CoC.getInstance().player.canOviposit() ) {
					if( CoC.getInstance().flags[ kFLAGS.URTA_TIMES_EGG_PREGGED ] === 0 ) {
				...
				if( CoC.getInstance().flags[ kFLAGS.URTA_CUM_NO_CUM_DAYS ] >= 5 ) {
			...
			//[Horny Urta Talk – Drunk];
				...
				if( CoC.getInstance().flags[ kFLAGS.RAPHEAL_COUNTDOWN_TIMER ] === -2 && SceneLib.raphael.RaphaelLikes() && CoC.getInstance().flags[ kFLAGS.URTA_X_RAPHAEL_HAPPENED ] === 0 ) {
		...
		if( CoC.getInstance().flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] === 0 && CoC.getInstance().flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] > -1 ) {
			EngineCore.outputText( ImageManager.showImage( 'urta-bar-drunk' ), false );
			EngineCore.outputText( 'Urta smiles broadly and noisily slurps a few dribbles of whiskey from her shiny black lips, watching you approach.  She gives you a smoking-hot \'come-hither\' look and slurs, "<i>I didn\'t think I\'d get a chance to bump into you here, cutie.  Wanna dance?</i>"\n\n', false );
		...
		if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00146 ] !== -1 && this.urtaDrunk() ) {
			EngineCore.outputText( ImageManager.showImage( 'urta-bar-drunk' ), false );
			EngineCore.outputText( 'The drunken fox roughly gropes you and whispers in your ear, "<i>I NEED a good fuck right now, and it feels like someone wants a piece of Urta.  I can guess just which piece you\'re wanting too.  The only question is do I ', false );
			//CUT FOR NOW outputText('throw you over the table and fuck you,', false);;
			EngineCore.outputText( 'let the beast loose and bounce you on my lap while we drink, ', false );
			EngineCore.outputText( 'jerk off onto your face in front of everyone, or have to remember your ass for later when you bolt for the door.  What\'ll it be, hun?</i>"', false );
			temp = null;
			if( CoC.getInstance().player.canOviposit() ) {
				if( CoC.getInstance().flags[ kFLAGS.URTA_TIMES_EGG_PREGGED ] === 0 ) {
					EngineCore.outputText( '\n\nHmm... sounds like she wants to fuck almost as badly as you want to get these eggs out of you.  Besides, didn\'t Urta say to you once that she\'s basically barren and sterile?  You\'d be doing her a favor by letting her carry your eggs, wouldn\'t you?  Let\'s see if she\'s drunk enough to let you fill her full...' );
		...
		if( CoC.getInstance().flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] >= 11 && Utils.rand( 2 ) === 0 ) {
			EngineCore.outputText( ImageManager.showImage( 'urta-bar' ), false );
			EngineCore.outputText( 'You approach Urta, who gives you a naughty wink and asks, "<i>Would you like to play with my little pony?</i>"  One of her hands is trailing along her leg, caressing her member as it hardens under her tight fishnet stockings.\n\n(You can suck her off under the table or go back to her place to fuck.)', false );
		} else {
			EngineCore.outputText( ImageManager.showImage( 'urta-bar' ), false );
			EngineCore.outputText( 'You approach Urta, who gives you a lascivious grin ', false );
			if( CoC.getInstance().flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] < 11 ) {
				EngineCore.outputText( 'moments before you hear her horse-cock thump into the bottom of the table.  ', false );
			} else {
				EngineCore.outputText( 'and pets her already-growing horse-cock through her fishnet stockings.  ', false );
			}
			if( Utils.rand( 2 ) === 0 ) {
				EngineCore.outputText( 'Urta asks, "<i>I\'m really horny, do you think you could help me get off?  I guess we could go fuck at my place, or you could always just help me out under the table.</i>"  A blush colors her face through the gray fur as she suggests the latter option.', false );
			} else {
				EngineCore.outputText( 'She asks, "<i>Would you like to go back to my place and help me play with my \'little\' friend again?  Or maybe you could climb under the table and give me some relief?</i>"', false );
			}
		}
		if( CoC.getInstance().player.canOviposit() ) {
			if( CoC.getInstance().flags[ kFLAGS.URTA_TIMES_EGG_PREGGED ] === 0 ) {
	...
	Urta.prototype.drinkUrtasBoozePtTwo = function() {
		this.urtaSprite();
		EngineCore.outputText( '', true );
		EngineCore.outputText( 'With a happy sigh you ', false );
		if( CoC.getInstance().player.hasVagina() && CoC.getInstance().player.totalCocks() === 0 ) {
			EngineCore.outputText( 'squat down and ', false );
		}
		EngineCore.outputText( 'open up your ' + CoC.getInstance().player.armorName + ', releasing the pressure onto a patch of grass.  It takes forever to stop, but when it does you feel like a million bucks.  You turn and start to close up your kit when you realize you aren\'t alone.  Urta is watching you, her lips parted hungrily.  She\'s still peeing, but what\'s truly shocking is the mammoth package she\'s got hanging between her legs.  It\'s a massive horse-shaft, patterned black and pink, slung down from between a pair of apple-sized balls.  Before your eyes the equine beast begins hardening, forcing the distracted fox to empty the last of her urine against a building\'s wall.  She blushes furiously, but the drunken fox doesn\'t seem to care about her shame...\n\n', false );
	...
	Urta.prototype.drinkUrtasBoozeLetHer = function() {
		...
		CoC.getInstance().flags[ kFLAGS.TIMES_FUCKED_URTA ]++;
		CoC.getInstance().flags[ kFLAGS.URTA_TIME_SINCE_LAST_CAME ] = 5;
		EngineCore.hideUpDown();
		CoC.getInstance().player.orgasm();
		EngineCore.dynStats( 'sen', -2 );
		//[LET HER];
		EngineCore.outputText( '', true );
		EngineCore.outputText( ImageManager.showImage( 'urta-behindbar-fuck' ), false );
		EngineCore.outputText( 'You nod at Urta, feeling a bit relaxed from all the whiskey and more than a little intrigued by the beast between the slender fox\'s legs.  She\'s on you before you know it, her softly furred hand squeezing one of your ', false );
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( Descriptors.ballsDescriptLight(), false );
		} else if( CoC.getInstance().player.biggestTitSize() > 1 ) {
			EngineCore.outputText( Appearance.biggestBreastSizeDescript(), false );
		} else {
			EngineCore.outputText( Descriptors.nippleDescript( 0 ) + 's', false );
		}
		EngineCore.outputText( ', as her tongue spears between your lips.  She practically mouth-rapes you as her trembling member grinds between your ' + CoC.getInstance().player.legs() + ', smearing them with a generous helping of foxy horse-pre.  The horny drunk slurs into your ear, "<i>Gawdsh above, are you this much of a schlut for everyone with one of these?</i>"\n\n', false );
		EngineCore.outputText( 'With surprising strength, the inebriated fox pushes you against an old bench, flat onto your ', false );
		if( CoC.getInstance().player.biggestTitSize() > 1 ) {
			EngineCore.outputText( 'tits', false );
		} else {
			EngineCore.outputText( 'belly', false );
		}
		EngineCore.outputText( '.  She pulls at your ' + CoC.getInstance().player.armorName + ' until your crotch is completely exposed and vulnerable, revealing your ' + CoC.getInstance().player.assholeOrPussy(), false );
		if( CoC.getInstance().player.cockTotal() > 0 ) {
			EngineCore.outputText( ' and ' + Descriptors.multiCockDescript(), false );
		}
		EngineCore.outputText( '.  Urta snuggles close, brushing her nose along your thighs before plunging her tongue deep into your ' + CoC.getInstance().player.assholeOrPussy() + ' in one powerful, drunken lick.  She slurps and licks, humming away until you\'re ', false );
		if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( 'wet', false );
		} else if( CoC.getInstance().player.totalCocks() > 0 ) {
			EngineCore.outputText( 'suitably lubricated and leaking pre', false );
		} else {
			EngineCore.outputText( 'suitably lubricated', false );
		}
		EngineCore.outputText( '.\n\n', false );
		EngineCore.outputText( 'With a quick shift in position, she forces herself inside, her arms supporting her jiggling chest as she leans over you, dragging her nipples up your back.  Her fat shaft forces your body to spread wide.  The fat head and ring of prepuce strokes your inner walls, squeezing grunts of pleasure from you as the drunken fox forcefully begins fucking you, rutting like a beast.  You can feel her nipples harden, digging into your back and eliciting moans from their owner as they\'re repeatedly smashed against you.  Urta starts licking your neck where it joins your shoulder, gently nipping at your tender flesh as her tip begins to flare wider inside you.', false );
		CoC.getInstance().player.cuntChange( 60, true, true, false );
		EngineCore.outputText( '\n\n', false );
		EngineCore.outputText( '"<i>Mmmm I shhoulda ushed a condom, you\'re gonna be a messssss!</i>" she cries as she climbs atop her sexual peak, mounting it just like she\'s mounting your ' + CoC.getInstance().player.assholeOrPussy() + '.  You\'re spread wide, the equine member rammed tightly ', false );
		if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( 'against your cervix as the tip presses firmly against it', false );
		} else {
			EngineCore.outputText( 'into your rectum', false );
		}
		EngineCore.outputText( ', unloading a thick batch of cream directly into your ', false );
		if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( 'waiting womb', false );
		} else {
			EngineCore.outputText( 'abused asshole', false );
		}
		EngineCore.outputText( '.', false );
		if( CoC.getInstance().player.pregnancyIncubation > 0 && CoC.getInstance().player.pregnancyIncubation < 250 ) {
			EngineCore.outputText( '  Whatever\'s growing inside you is probably bathing in the stuff now.', false );
		} else if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( '  You briefly wonder if she\'s capable of knocking you up, and what the baby would look like.', false );
		} else {
			EngineCore.outputText( '  Your backside fills with warmth as her spunk seeps deep inside you.', false );
		}
		EngineCore.outputText( '  Another blast fills you to capacity, and you\'re mortified by the sensation of your body being shifted by your growing belly.\n\n', false );
		EngineCore.outputText( 'Urta sucks on the edge of your ear, still pumping away as she pours a bit more spooge into your now equine-sized cavity.  Your alcohol-dulled mind reels as your body caves in to the sticky fluid assault, cumming hard and wriggling around her shaft, moaning repeatedly as you clamp down tightly.  ', false );
		if( CoC.getInstance().player.totalCocks() > 0 ) {
			...
			EngineCore.outputText( 'our ' + Descriptors.multiCockDescriptLight() + ' squirts hard, unloading a batch of sticky jism under your ', false );
			if( CoC.getInstance().player.pregnancyIncubation > 0 && CoC.getInstance().player.pregnancyIncubation < 100 ) {
				EngineCore.outputText( 'pregnancy swollen ', false );
			}
			EngineCore.outputText( 'belly, making you squish with each brutal thrust of your oversized lover\'s rod.', false );
		} else {
			if( CoC.getInstance().player.hasVagina() ) {
				EngineCore.outputText( 'Your pussy dribbles and clenches, leaking Urta\'s whiteness as she claims it for her own.', false );
			} else {
				EngineCore.outputText( 'Your ' + Descriptors.assholeDescript() + ' clamps down tightly, spasming in pleasure around the invader.', false );
			}
		}
		if( CoC.getInstance().player.totalCocks() > 0 ) {
			EngineCore.outputText( '  As you cum, more and more whiteness dribbles out until a puddle forms underneath you, showing Urta just how much you enjoyed the rough, drunken fuck.\n\n', false );
		} else {
			EngineCore.outputText( '\n\n', false );
		}

		EngineCore.outputText( 'She pulls out, dripping everywhere.  The fox leans down and proceeds to give your still quivering hole a tongue-bath, thoroughly cleaning her fluids from the entrance and teasing you until you clamp down, locking the rest inside.  With a gentle slap on your ' + Descriptors.buttDescript() + ' Urta says, "<i>Thankss for the quick fuck.  Maybe shometime you can ', false );
		if( CoC.getInstance().player.cockTotal() > 0 ) {
			EngineCore.outputText( 'stick it in me', false );
		} else {
			EngineCore.outputText( 'come back for an even bigger load', false );
		}
		EngineCore.outputText( '.</i>"\n\n', false );
		EngineCore.outputText( 'The fox helps you to your feet and the two of you get dressed, stumbling a bit over each other and giggling happily.  Urta gives you a quick peck on the lips and whispers, "<i>Thanksh... for everything,</i>" before the two of you make your way back into the bar and go your separate ways.\n\n', false );
		if( CoC.getInstance().player.hasVagina() ) {
			CoC.getInstance().player.cuntChange( 60, true );
		} else {
			CoC.getInstance().player.buttChange( 60, true );
		}
		EngineCore.doNext( SceneLib.camp.returnToCampUseOneHour );
	};
	//[BACKDOOR];
	...
	Urta.prototype.dualUrtaMasturbation = function() {
		...
		CoC.getInstance().flags[ kFLAGS.URTA_TIME_SINCE_LAST_CAME ] = 5;
		EngineCore.outputText( '', true );
		EngineCore.outputText( ImageManager.showImage( 'urta-behindbar-masti-dual' ), false );
		EngineCore.outputText( 'You come out of the shadows, shedding your ' + CoC.getInstance().player.armorName + '.  ', false );
		if( CoC.getInstance().player.cor < 60 ) {
			EngineCore.outputText( 'You blush furiously at exposing yourself in such a way', false );
		} else {
			EngineCore.outputText( 'Smiling seductively, you revel in exposing yourself so suddenly', false );
		}
		EngineCore.outputText( '.  Urta looks up at you in shock, blinking tears from her eyes and stammering, "<i>B-b-b-ut I have th-thish hideoush thing... Aren\'t you scared of it?</i>"\n\n', false );
		EngineCore.outputText( 'Gently wiping a tear from her muzzle, you comfort the distraught fox and sit down across from her, giving her a perfect view of your ', false );
		if( CoC.getInstance().player.totalCocks() > 0 ) {
			...
			EngineCore.outputText( 'bare groin and ' + Descriptors.assholeDescript(), false );
		}
		EngineCore.outputText( '.  ', false );
		if( CoC.getInstance().player.totalCocks() > 0 ) {
			EngineCore.outputText( 'The situation gives you all the spark you need to rise to your full size, ', false );
			if( CoC.getInstance().player.cockArea( 0 ) < 40 ) {
				EngineCore.outputText( 'even if Urta\'s member dwarfs you.', false );
			} else if( CoC.getInstance().player.cockArea( 0 ) < 70 ) {
				EngineCore.outputText( 'showing Urta you\'re just as big.', false );
			} else {
				EngineCore.outputText( 'proving to Urta just who has the bigger burden.', false );
			}
			EngineCore.outputText( '  ', false );
		}
		if( CoC.getInstance().player.hasVagina() ) {
			if( CoC.getInstance().player.vaginas[ 0 ].vaginalWetness > AppearanceDefs.VAGINA_WETNESS_SLICK ) {
				EngineCore.outputText( 'A trickle of wetness escapes your nether-lips, starting to puddle underneath you.', false );
			} else {
				EngineCore.outputText( 'Quickly engorging, your lower lips begin to part around your ' + Descriptors.clitDescript() + '.', false );
			}
			EngineCore.outputText( '  ', false );
		}
		EngineCore.outputText( 'The heat coursing through you quickly shows itself on your ', false );
		if( CoC.getInstance().player.biggestTitSize() >= 1 ) {
			EngineCore.outputText( Descriptors.allBreastsDescript(), false );
		} else {
			EngineCore.outputText( 'chest', false );
		}
		EngineCore.outputText( ' as your ' + Descriptors.nippleDescript( 0 ) + 's become full and sensitive.\n\n', false );
		EngineCore.outputText( 'Urta relaxes, leaning against a wall as she returns to pleasuring her mismatched member.  Her eyes lock onto your groin as you join her, and she lets out a sigh that\'s equal parts relieved and inebriated.  Together, the two of you pleasure your bodies, each watching the other\'s squirming form and blissful expression.  ', false );
		if( CoC.getInstance().player.totalCocks() === 1 ) {
			EngineCore.outputText( 'Your hands caress your ' + Descriptors.cockDescript( 0 ) + ' feverishly, tweaking every ounce of pleasure from the bloated rod.  ', false );
		}
		if( CoC.getInstance().player.totalCocks() > 1 ) {
			EngineCore.outputText( 'Your hands caress your ' + Descriptors.multiCockDescript() + ' in turn, dancing back and forth from rod to rod as you attempt to play with every available pleasure outlet.  ', false );
		}
		if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( 'Delving deeply between your legs, you press forward into the source of your feminine pleasures, touching and rubbing, caressing your swollen button as you get closer and closer to orgasm.\n\n', false );
		}
		if( CoC.getInstance().player.gender === 0 ) {
			EngineCore.outputText( 'Delving down between your legs, rubbing along your taint before you slowly penetrate your ' + Descriptors.assholeDescript() + ', pleasuring yourself in the only way possible.\n\n', false );
		}

		EngineCore.outputText( 'The cute fox-girl reaches her peak, crying out in a loud moan that echoes around the alley.  Her equine shaft trembles, the medial ring becoming more pronounced while the flat head flares wide mid-orgasm.  A blast of cum launches into the air, fans out overhead, and splatters down over you.  Urta looks horrified, and yanks herself back, succeeding only in depositing the next blast of spoogey goodness into her own face.  The situation would be comical if the foxy horse-goo wasn\'t such a great lubricant for your hands, and with that thought in your mind, you cum too.\n\n', false );
		if( CoC.getInstance().player.totalCocks() > 0 ) {
			if( CoC.getInstance().player.cumQ() < 250 ) {
				EngineCore.outputText( 'Jism spurts out onto the alley floor, forming a nice little puddle as ', false );
				if( CoC.getInstance().player.totalCocks() > 1 ) {
					EngineCore.outputText( 'each of ', false );
				}
				EngineCore.outputText( 'your ' + Descriptors.multiCockDescriptLight() + ' empties itself.  ', false );
			} else if( CoC.getInstance().player.cumQ() < 500 ) {
				EngineCore.outputText( 'Jism pours out onto the alley floor as your body releases the copious amount of cum it can produce.  It puddles before you, running off the stone into the grass.  ', false );
			} else if( CoC.getInstance().player.cumQ() < 1000 ) {
				EngineCore.outputText( 'Jism erupts from ', false );
				if( CoC.getInstance().player.totalCocks() > 1 ) {
					EngineCore.outputText( 'each of ', false );
				}
				EngineCore.outputText( 'your ' + Descriptors.multiCockDescriptLight() + ', blasting into the air before splattering onto the ground between you and Urta.  It forms a massive puddle, spreading as you pump gob after gob of cum into it until the sticky spooge is soaking your ' + CoC.getInstance().player.legs() + ' and Urta too.', false );
			} else {
				EngineCore.outputText( 'Jism fountains from ', false );
				if( CoC.getInstance().player.totalCocks() > 1 ) {
					EngineCore.outputText( 'each of ', false );
				}
				EngineCore.outputText( 'your ' + Descriptors.multiCockDescriptLight() + ', flooding the small alleyway with an unholy amount of cum.  In no time you\'ve painted Urta and yourself from head to toe, and the alleyway sloshes with the stuff as your production overwhelms the ground\'s ability to absorb liquid.  ', false );
			}
		}
		if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( 'Your sex squishes wetly around a finger, clenching down tightly', false );
			if( CoC.getInstance().player.vaginas[ 0 ].vaginalWetness === AppearanceDefs.VAGINA_WETNESS_SLAVERING ) {
				EngineCore.outputText( ' and gushing everywhere', false );
			}
			EngineCore.outputText( '.  ', false );
		} else {
			EngineCore.outputText( 'Your sphincter clenches down tightly around your invading digit as you get off, and the rapt attention of your audience certainly helps.  ', false );
		}
		EngineCore.outputText( 'Sighing happily, you give your nipple a tweak as Urta finally finishes cumming, ', false );
		if( CoC.getInstance().player.cumQ() >= 1000 ) {
			EngineCore.outputText( 'adding to the massive pool on the ground after ', false );
		}
		EngineCore.outputText( 'drenching herself thoroughly.\n\n', false );
		EngineCore.outputText( 'Urta lets out a VERY relieved-sounding sigh and murmurs, "<i>Oh godsh, thank you.  You don\'t know how hard it ish to hide thish beasht from EVERYONE.  You\'re a good friend for helping me with thish... let\'sh do thish again sometime, ok?</i>"\n\n', false );
		EngineCore.outputText( 'She staggers up to her feet and rapidly discards her sodden clothes.  With a resigned sigh, Urta pulls a loose stone out of the side of a building and removes a sheer skirt and diaphanous top from the cubby.  They slip on easily, though the new clothes look a bit more \'slutty\' than her previous pair, easily displaying her hard black nipples.  Urta replaces the stone, wobbling a bit unsteadily before she ', false );
		if( CoC.getInstance().player.cumQ() >= 1000 ) {
			EngineCore.outputText( 'tip-toes through the spunk', false );
		} else {
			EngineCore.outputText( 'gets her balance and heads for the bar\'s backdoor', false );
		}
	...
	Urta.prototype.urtaFuckHer = function( afterBefriending ) {
		...
		CoC.getInstance().flags[ kFLAGS.TIMES_FUCKED_URTA ]++;
		CoC.getInstance().flags[ kFLAGS.URTA_TIME_SINCE_LAST_CAME ] = 5;
		var cocks = CoC.getInstance().player.cockTotal();
		if( !afterBefriending ) {
			EngineCore.clearOutput();
		}
		//(FUCK HER FEMALE/GENDERLESS);
		if( CoC.getInstance().player.totalCocks() === 0 ) {
			EngineCore.outputText( ImageManager.showImage( 'urta-behindbar-female-fuckurta' ), false );
			CoC.getInstance().player.slimeFeed();
			EngineCore.outputText( 'You come out of the shadows, shedding your ' + CoC.getInstance().player.armorName + '.  ', false );
			if( CoC.getInstance().player.cor < 50 ) {
				EngineCore.outputText( 'You blush furiously at exposing yourself in such a way' );
			} else {
				EngineCore.outputText( 'Smiling seductively; you revel in exposing yourself so suddenly' );
			}
			EngineCore.outputText( '.  Urta looks up at you in shock, blinking tears from her eyes and stammering, "<i>B-b-b-ut I have th-thish hideoush thing... Aren\'t you scared of it?</i>"\n\n', false );
			EngineCore.outputText( 'Gently wiping a tear from her muzzle, you answer, "<i>Not at all, but I think you\'d have more luck with a nice warm hole to slide that monster into.</i>"\n\n', false );
			EngineCore.outputText( 'Urta nods, numbed by the unexpected and forward nature of your offer.  She looks up hopefully and pries her hands away from the twitching behemoth in her loins, waiting to see how you intend to deal with it.  You approach her and turn, straddling your ' + CoC.getInstance().player.legs() + ' over her, watching her expression as you lower yourself down, ', false );
			if( CoC.getInstance().player.tailType === AppearanceDefs.TAIL_TYPE_HORSE || CoC.getInstance().player.tailType === AppearanceDefs.TAIL_TYPE_DOG ) {
				EngineCore.outputText( 'swishing your tail across her taut nipples', false );
			} else {
				EngineCore.outputText( 'making her wait', false );
			}
			EngineCore.outputText( '.  Eventually the blunted tip of Urta\'s horse-cock presses against your ' + CoC.getInstance().player.assholeOrPussy() + '.  Agonizingly, you force yourself to relax and lower yourself down on the mottled pink and black shaft.\n\n', false );
			EngineCore.outputText( 'The horse-cock spreads you wide, filling you past the point of fullness as you sink further and further along its length.  Urta groans happily, her hips twitching involuntarily and forcing more of the monster-sized cock inside you.', false );
			if( CoC.getInstance().player.hasVagina() ) {
				if( CoC.getInstance().player.vaginalCapacity() >= 60 ) {
					EngineCore.outputText( '  You sigh and sink down, taking it until the folds of her sheath are rubbing your entrance.', false );
				} else {
					EngineCore.outputText( '  You nearly cry out as you sink down, taking it until the folds of her sheath are rubbing your entrance, even though it hurts to accommodate it.', false );
				}
				CoC.getInstance().player.cuntChange( 60, true, true, false );
			} else {
			...
			EngineCore.outputText( '  Urta babbles, "<i>Oh godsh yesh thank you so much-ohmigod it\'s SO HOT inside you.</i>"\n\n', false );
			EngineCore.outputText( 'You smirk over your shoulder and start bouncing up and down on her, slapping her balls with your ' + Descriptors.buttDescript() + ' each time you bottom out on the thick shaft.  You can feel her ring of prepuce rubbing your inner walls, pressing on nerves normally ignored.  Urta grunts underneath you and you feel her flare growing wider inside you, stretching you further and further with every bounce atop the equine-endowed slut.  Suddenly warmth is flooding into your ', false );
			if( CoC.getInstance().player.hasVagina() ) {
				EngineCore.outputText( 'womb', false );
			} else {
				EngineCore.outputText( 'rectum', false );
			}
			EngineCore.outputText( ', and Urta\'s tongue is lolling out from her muzzle, drooling down the side of her face as she loses herself in orgasm.\n\n', false );
			EngineCore.outputText( 'The substantial flare forms a perfect plug inside you and your belly distends slightly as you\'re packed full of foxy-horse-cum.  Your own lusts keep your hips moving, grinding and squeezing the shaft inside you as it packs you full of Urta\'s seed.  Your orgasm explodes through your hips and your ' + CoC.getInstance().player.legs() + ' give out underneath you, dropping you down onto the fox-girl as the pair of you cum together.  Whiteness begins squirting out around her shaft as her prick begins softening inside you.  You sigh happily, content but too nerveless to stand for the moment.  Peeking over your shoulder and seeing Urta\'s satisfied expression, you guess she feels about the same.\n\n', false );
			EngineCore.outputText( 'Once the horse-dick inside you has sufficiently softened, you stand up, gasping as a rush of spooge escapes from your ' + CoC.getInstance().player.assholeOrPussy() + ' now that the \'plug\' is removed.  Urta gives you a cocky grin as she\'s soaked with her fluids, moments before she starts staggering back to her feet.  She wipes up with her clothes and tosses them into the garbage, then wobbles up to a loose brick in the wall and pulls it free, revealing a hidden compartment filled with sets of replacement clothes.  She removes a tight skirt and sheer top, both a bit more revealing than what she wore in the bar earlier, and hurriedly dresses herself.\n\n', false );
			EngineCore.outputText( 'Urta blows you a kiss as you put your own gear back together and flounces back to the bar, calling out, "<i>We should do thish again shoon!</i>"\n\n', false );
			EngineCore.outputText( '"<i>Yes indeed,</i>" you muse - the sooner the better.', false );
		} else {
			var x = CoC.getInstance().player.cockThatFits( this.urtaCapacity() );
			var y = CoC.getInstance().player.cockThatFits2( this.urtaCapacity() );
			if( x < 0 ) {
				x = 0;
			}
			EngineCore.outputText( ImageManager.showImage( 'urta-behindbar-male-fuckurta' ), false );
			EngineCore.outputText( 'You come out of the shadows, shedding your ' + CoC.getInstance().player.armorName + '.  ', false );
			if( CoC.getInstance().player.cor < 60 ) {
				EngineCore.outputText( 'You blush furiously at exposing yourself in such a way' );
			} else {
			...
			EngineCore.outputText( '.  Urta looks up at you in shock, blinking tears from her eyes and stammering, "<i>B-b-b-ut I have th-thish hideoush thing... Aren\'t you scared of it?</i>"\n\n', false );
			EngineCore.outputText( 'Gently wiping a tear from her muzzle, you answer, "<i>Not at all, but I think you\'d have more luck with something stuffed in your feminine half.</i>"\n\n', false );
			EngineCore.outputText( 'Urta gives you a shocked look, but nods and turns around, bending over an old bench and exposing her well-toned ass and glistening, black-lipped sex.  The invitation is all you need.  You wrap your arms around her, squeezing the downy-soft fur on her breasts and leveraging ', false );
			if( CoC.getInstance().player.cockTotal() > 0 ) {
				if( CoC.getInstance().player.cockTotal() > 2 ) {
					EngineCore.outputText( 'a pair of your ' + Descriptors.multiCockDescriptLight() + ' against her pussy and asshole.  ', false );
				} else if( CoC.getInstance().player.cockTotal() === 2 ) {
					EngineCore.outputText( 'your ' + Descriptors.multiCockDescriptLight() + ' against her pussy and asshole.  ', false );
				} else if( CoC.getInstance().player.cockTotal() === 1 ) {
					EngineCore.outputText( 'your ' + Descriptors.multiCockDescriptLight() + ' against her pussy.  ', false );
				}
			}
			//DOES IT FITZ;
			if( CoC.getInstance().player.cockArea( x ) < this.urtaCapacity() ) {
				EngineCore.outputText( 'The wet entrance yields before you, accepting you like a velvet glove.  It grips you tightly, though the grip is meaningless with the flood of lubricant slipping over your shaft.  ', false );
				if( CoC.getInstance().player.cockTotal() >= 2 ) {
					EngineCore.outputText( 'Her backdoor was rougher going – the tightly muscled entrance held you back for a moment until Urta relaxed completely, but now the tighter hole is nearly as easy to penetrate as her pussy.  ', false );
				}
				EngineCore.outputText( 'The fox is openly moaning and stroking her disproportional cock, dripping pre-cum everywhere as you do your level best to violate her ', false );
				if( CoC.getInstance().player.cockTotal() === 1 ) {
					EngineCore.outputText( 'hole', false );
				} else {
					EngineCore.outputText( 'holes', false );
				}
				EngineCore.outputText( '.\n\n', false );
				EngineCore.outputText( 'She begins grinding against you, begging, "<i>Oh godsh yeeeessss don\'t shtop fucking me!  Give me more please!  PLEASE!</i>"\n\n', false );
				EngineCore.outputText( 'Nodding, you release one of her bouncing breasts in order to smack her back-side.  It barely jiggles, but the surprised moan that slips from the fox-herm\'s lips is all the encouragement you need.  You release the other breast and grab her hips and ass with both hands, slamming her back against you, only pausing to deliver another smack to one of her cheeks.  Urta wiggles happily, squealing after every slap, trying her best to steady her body while you brutally ', false );
				if( cocks === 1 ) {
					EngineCore.outputText( 'fuck', false );
				} else {
					EngineCore.outputText( 'double-penetrate', false );
				}
				EngineCore.outputText( ' her.\n\n', false );
				EngineCore.outputText( '"<i>Yesh oh yessssssSSSAHHHCUMMING,</i>" cries Urta as her body goes nerveless underneath you, held up entirely by your hands', false );
				if( cocks > 1 && y >= 0 ) {
					EngineCore.outputText( ', ' + Descriptors.cockDescript( x ) + ', and ' + Descriptors.cockDescript( y ) + '.', false );
				} else {
					EngineCore.outputText( ' and ' + Descriptors.cockDescript( x ) + '.', false );
				}
				EngineCore.outputText( '  Both her hands are locked tightly around her behemoth as it unloads onto the alley floor and walls.  The juicy tunnel clamps down on top of you, squeezing your ' + Descriptors.cockDescript( x ) + ' as if Urta\'s life depended on it.  It\'s as if every ounce of her being was focusing on clamping down on the invader, massaging, squeezing, and milking it, while the rest of her body goes entirely limp.', false );
				if( cocks > 1 && y >= 0 ) {
					EngineCore.outputText( '  Your ' + Descriptors.cockDescript( y ) + ' feels like it\'s about to explode inside her asshole when her sphincter clamps down tightly, acting like the world\'s tightest cock-ring.', false );
				}
				EngineCore.outputText( '\n\n', false );
				EngineCore.outputText( 'You fuck her savagely, abusing her spasming hole', false );
				if( cocks > 1 && y >= 0 ) {
					EngineCore.outputText( 's', false );
				}
				EngineCore.outputText( ' with every thrust until your own climax bursts over you like a wave.  Urta\'s drooling cunt ', false );
				if( cocks > 1 && y >= 0 ) {
					EngineCore.outputText( 'and tightly clenched ass seem', false );
				} else {
					EngineCore.outputText( 'seems', false );
				}
				EngineCore.outputText( ' to be trying to milk your body of every ounce of cum.  ', false );
				if( CoC.getInstance().player.cumQ() < 250 ) {
					EngineCore.outputText( 'After a few passionate thrusts, you\'re spent, and you pull free, admiring the glaze of white that remains on her cunt-lips', false );
					if( cocks > 1 && y >= 0 ) {
						EngineCore.outputText( ' and slowly-closing pucker', false );
					}
					EngineCore.outputText( '.', false );
				} else if( CoC.getInstance().player.cumQ() < 500 ) {
					EngineCore.outputText( 'After a few passionate thrusts, you can feel your cum packing Urta\'s womb ', false );
					if( cocks > 1 && y >= 0 ) {
						EngineCore.outputText( 'and gut ', false );
					}
					EngineCore.outputText( 'full of spooge, but all great things come to an end eventually.  You pull free, admiring the glaze of white that remains on her cunt-lips', false );
					if( cocks > 1 && y >= 0 ) {
						EngineCore.outputText( ' and slowly-closing pucker', false );
					}
					EngineCore.outputText( '.', false );
				} else {
					EngineCore.outputText( 'After a few passionate thrusts, Urta\'s womb ', false );
					if( cocks > 1 && y >= 0 ) {
						EngineCore.outputText( 'and belly are ', false );
					} else {
						EngineCore.outputText( 'is ', false );
					}
					EngineCore.outputText( 'so full of spooge that she looks positively pregnant, and you\'re expelled from her body by the sheer force of your orgasm.  You pause to admire your handiwork, watching whiteness drip from ', false );
					if( cocks > 1 && y >= 0 ) {
						EngineCore.outputText( 'both her holes.', false );
					} else {
						EngineCore.outputText( 'her abused hole.', false );
					}
				}
				...
				EngineCore.outputText( '.  Her tail curls up around the softening horse-cock, hiding it from view as she blows you a sloppy kiss and disappears inside.\n\n', false );
				EngineCore.outputText( 'You waste no time cleaning up and you stagger back inside after her, sated and happy with your new friend and her \'benefits\'.', false );
				this.knockUpUrtaChance();
			}
			//(TOO BIG);
			else {
				if( cocks === 1 ) {
					EngineCore.outputText( 'Urta\'s hole is just way too small to handle what you\'re packing, so you\'re forced to back off, much to her lament.  She moans, "<i>Noooo! Fuck me!</i>"\n\n', false );
					EngineCore.outputText( 'You pull her down into the grass and throw her on her back.  She looks confused and a little afraid, but that melts away when you spread your ' + CoC.getInstance().player.legs() + ' across her and begin grinding your ' + Descriptors.multiCockDescriptLight() + ' against her sensitive dick.  Happy, the drunken fox begins pistoning against you, gathering her copious pre and smearing it over your ' + Descriptors.multiCockDescript() + '.  The two of you grind together, groin to groin, coating each other in a slippery mess.\n\n', false );
					EngineCore.outputText( 'Urta pants out, "<i>Ah ahhh ahhhh,</i>" and cums, her flare ballooning out until it\'s twice as wide as her shaft, a wave of white jism escaping from her horse-cock.  You gather some in your hand and slather it over your own ' + Descriptors.multiCockDescript() + ', and after a few moments of fevered masturbation, you start cumming with her.  Urta happily squirms against you, painting herself with jism and soaking her clothes with the stuff.  ', false );
					if( CoC.getInstance().player.cumQ() < 250 ) {
						EngineCore.outputText( 'You join in, though your own contribution seems meager by comparison.', false );
					} else if( CoC.getInstance().player.cumQ() < 500 ) {
						EngineCore.outputText( 'You join in, and manage to match her spurt for spurt, until she\'s lying in a thick puddle of the stuff.', false );
					} else {
						EngineCore.outputText( 'You join in, and make her orgasm look like a trickle compared to the flood of spunk you unleash onto her.  By the time the pair of you finish, she\'s soaked from head to toe, and lying in a nicely sized puddle of the stuff.', false );
					}
					...
					EngineCore.outputText( 'After a few moments she picks herself up and wipes herself up as much as she can with her sodden clothes.  Urta quickly gives up and removes a loose brick from the wall, revealing a hidden compartment with clothes and a few towels.  It seems she\'s quite prepared to make a mess in this alley.  She towels the worst of it off and gets dressed in a sheer tank-top and tight skirt before staggering back towards the bar.  Urta pauses and looks over her shoulder to blow you a sloppy kiss before she goes inside.\n\n', false );
					EngineCore.outputText( 'You waste no time cleaning up and mosey back inside after her, sated and happy with your new friend and her \'benefits\'.', false );
				}
				if( cocks > 1 ) {
					EngineCore.outputText( 'Urta\'s holes are just way too small to handle what you\'re packing, so you\'re forced to back off, much to her lament.  She moans, "<i>Noooo! Fuck me!</i>"\n\n', false );
					EngineCore.outputText( 'You pull her down into the grass and throw her on her back.  She looks confused and a little afraid, but that melts away when you spread your ' + CoC.getInstance().player.legs() + ' across her and begin grinding each of your ' + Descriptors.multiCockDescriptLight() + ' against her sensitive dick.  Happy, the drunken fox begins pistoning against you, gathering her copious pre and smearing it over your ' + Descriptors.multiCockDescriptLight() + '.  The two of you grind together, groin to groin, coating each other in a slippery mess.\n\n', false );
					EngineCore.outputText( 'Urta pants out, "<i>Ah ahhh ahhhh,</i>" and cums, her flare ballooning out until it\'s twice as wide as her shaft, a wave of white jism escaping from her horse-cock.  You gather some in your hand and slather it over your own ' + Descriptors.multiCockDescriptLight() + ', and after a few moments of fevered masturbation, you start cumming with her.  Urta happily squirms against you, painting herself with jism and soaking her clothes with the stuff.  ', false );
					if( CoC.getInstance().player.cumQ() < 250 ) {
						EngineCore.outputText( 'You join in, though your own contribution seems meager by comparison.', false );
					} else if( CoC.getInstance().player.cumQ() < 500 ) {
						EngineCore.outputText( 'You join in, and manage to match her spurt for spurt, until she\'s lying in a thick puddle of the stuff.', false );
					...
					EngineCore.outputText( 'The drunken fox groans and continues spurting weakly, "<i>Oh godsh, letsh do this again sooon.  You don\'t know how much I needed thish!</i>"\n\n', false );
					EngineCore.outputText( 'After a few moments she picks herself up and wipes herself up as much as she can with her sodden clothes.  Urta quickly gives up and removes a loose brick from the wall, revealing a hidden compartment with clothes and a few towels.  It seems she\'s quite prepared to make a mess in this alley.  She towels the worst of it off and gets dressed in a sheer tank-top and tight skirt before staggering back towards the bar.  Urta pauses and looks over her shoulder to blow you a sloppy kiss before she goes inside.\n\n', false );
					EngineCore.outputText( 'You waste no time cleaning up and mosey back inside after her, sated and happy with your new friend and her \'benefits\'.', false );
				}
			}
	...
	Urta.prototype.blowUrtaUnderTable = function() {
		...
		CoC.getInstance().flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ]++;
		this.urtaLove( 1 );
		CoC.getInstance().flags[ kFLAGS.URTA_TIME_SINCE_LAST_CAME ] = 2 + Utils.rand( 4 );
		EngineCore.outputText( '', true );
		EngineCore.outputText( ImageManager.showImage( 'urta-bar-bj' ), false );
		EngineCore.outputText( 'Urta\'s eyes widen in shock when you suggest taking her under the table.  She stammers, "<i>Uh, okay...</i>" and fills a glass with straight whiskey, fueling up on liquid courage while you crawl down.  You slip down under the table, pushing her legs apart', false );
		if( CoC.getInstance().flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] >= 11 ) {
			EngineCore.outputText( ' and pulling down her fishnet to release the monster trapped inside it.  It snaps up immediately, bobbing before your face as it fills with Urta\'s booze-fueled lust.', false );
		} else {
			EngineCore.outputText( ' and pushing up her tight leather skirt, smiling at the sight of her trembling balls and juicy pussy.  Her tail unfurls around the equine member and it snaps up immediately, hanging before your face.', false );
		}
		EngineCore.outputText( '  It actually wiggles in time with the beating of her heart, bouncing up as it gets harder and harder, until it smacks your nose.\n\n', false );
		EngineCore.outputText( 'You hear the sound of her shot-glass slamming into the table with authority as she finishes her drink.  A bead of pre-cum bubbles out from Urta\'s urethra, signaling her readiness.  The horse-like penis gives off a salty scent, unique and not unpleasant.  Before you have a chance to change your mind, you lunge forward, opening your jaw as far as it will go and just barely taking her first few inches into your mouth.  The fox gasps out loud at the pleasure before she remembers to relax.  You can hear her pouring another drink, and in an effort to embarrass her, you begin sucking and licking around the edges of her flare, swallowing the surge of sweet-tasting pre-cum as it begins to fill your mouth.\n\n', false );
		EngineCore.outputText( 'Urta writhes in her seat, noisily spilling alcohol over the table.  You smile around the intruder in your mouth, gripping it with both hands and stroking it with tender caresses.  The gray fox slams down the bottle; though you can\'t tell if it\'s in irritation or due to difficulty controlling her muscles, and begins chugging down what she managed to get into her glass.  It seems Urta intends to drink until she doesn\'t care about any embarrassment you can force upon her.\n\n', false );
		EngineCore.outputText( 'Deciding you\'ve got to get her off before she can drink herself into a stupor, you ', false );
		if( CoC.getInstance().player.cor < 70 ) {
			EngineCore.outputText( 'do your best to suppress your gag reflex and shove forwards, ramming her cock as deeply into your throat as you can.', false );
		} else {
			EngineCore.outputText( 'shove forwards, ramming her massive cock so far down your throat you can feel it dribbling pre-cum into your belly.', false );
		}

		EngineCore.outputText( '  Urta grunts, doing her best to muffle the noise as it escapes her lips, but you can feel her twitch and thicken inside you, beginning to drip with proof of her complete and total submission to your mouth.\n\n', false );
		EngineCore.outputText( 'You can hear chuckles from the bar\'s other occupants as they realize what\'s going on, and you blush, wishing there was a tablecloth to hide your activities from prying eyes.  The distraction does little to divert you from your chosen task, and you close your eyes, tasting your lover\'s throbbing shaft as it fills your mouth.  With both hands stroking, you bob up and down, completely centering your world around the pulsating horse-prick as it fills your mouth.\n\n', false );
		EngineCore.outputText( 'Without warning, Urta\'s legs wrap around your back, squeezing with her soft fur as she vices them closed.  You\'re forced to take her even more deeply, until the folds of her sheath are bumping your nose and chin and filling your nostrils with her heady musk.  You can\'t even breathe with such a massive intruder blocking your throat, but you do your best to get her off, knowing it\'s the quickest way to end it.  You suck hard and squeeze her balls, gurgling noisily around the sweaty shaft as you feel Urta give in.\n\n', false );
		EngineCore.outputText( 'A wave-like motion passes through your throat obstruction, stretching you wider than ever before.  It passes, leaving behind a feeling of warm fullness.  You feel another working its way into your mouth, and realize Urta is cumming, filling your belly with direct injections of horsey fox-seed. You desperately need to breathe, but you\'re trapped on your lover\'s maleness, and with nothing else to do, you give her twitching balls a squeeze, feeling her body twitch in response as she dumps a particularly heavy batch of spunk into you.  Happy that you could at least control how hard she came, you begin blacking out, barely noticing as Urta\'s orgasmic contractions die off.\n\n', false );
		EngineCore.outputText( 'Thumping down on your back, you\'re blissfully aware of the sweet air filling your lungs.  In a moment of reflection, you realize you can taste the salty flavor of Urta\'s seed on your lips and mouth, and the retracting horse-cock is ', false );
		if( CoC.getInstance().player.biggestTitSize() >= 2 ) {
			EngineCore.outputText( 'dragging between your ' + Descriptors.allBreastsDescript(), false );
		} else {
			EngineCore.outputText( 'dragging across your chest', false );
		}
		EngineCore.outputText( ' as it begins to soften.  You wipe as much of her leavings from your ' + CoC.getInstance().player.face() + ' as you can, licking the slippery glaze from your lips.  Before you get up, you make sure to give Urta\'s member a firm squeeze, punishment for forcing you to take her so deep.\n\n', false );
		...
		EngineCore.dynStats( 'sen', 1, 'lus', Utils.rand( 10 ) + 5 + CoC.getInstance().player.lib / 10 );
		EngineCore.doNext( SceneLib.camp.returnToCampUseOneHour );
	};
	//[Public Buttfucking];
	Urta.prototype.takeUrtaInTheButtPublically = function() {
		...
		EngineCore.outputText( '', true );
		var tooBig = false;
		if( CoC.getInstance().player.cor < 30 && CoC.getInstance().flags[ kFLAGS.PC_FETISH ] === 0 ) {
		...
		this.urtaLove( 2 );
		CoC.getInstance().flags[ kFLAGS.URTA_TIME_SINCE_LAST_CAME ] = 5;
		EngineCore.outputText( ImageManager.showImage( 'urta-public-fuck' ), false );
		if( CoC.getInstance().player.cor < 60 && CoC.getInstance().flags[ kFLAGS.PC_FETISH ] === 0 ) {
			EngineCore.outputText( 'You can\'t believe you\'re doing it, but you undo the lower half of your ' + CoC.getInstance().player.armorName + '.  ', false );
		} else {
			EngineCore.outputText( 'You happily remove the lower half of your ' + CoC.getInstance().player.armorName + ', more than a little excited at the prospect of putting on a show for the crowd.  ', false );
		}
		EngineCore.outputText( 'Urta casually flips up her skirt and ', false );
		if( CoC.getInstance().flags[ kFLAGS.TIMES_FUCKED_URTA ] < 11 ) {
			EngineCore.outputText( 'unwinds her tail from around the beast', false );
		} else {
			EngineCore.outputText( 'fishes the beast out of her fishnets', false );
		}
		EngineCore.outputText( '.  Watching in horror, your eyes never leave the equine pillar as it grows to its full size.  Amazingly, none of the bar\'s patrons seem to notice the trembling horse-cock, even though it\'s beginning to leak with pre-cum.\n\n', false );
		EngineCore.outputText( 'You carefully shift your position, climbing in front of Urta.  With care, you grab the sensitive member just below the flare and begin rubbing it over your ' + Descriptors.buttDescript() + ' and ready ass-hole, lubing yourself up and trying to relax.  The impatient fox grabs you by your ' + Descriptors.hipDescript() + ' and pulls you down, forcing herself against your pucker.  ', false );
		//[GO TOO SMALL OR FITS, NO NEW PG];
		//[FITS];
		if( CoC.getInstance().player.analCapacity() >= 60 ) {
			EngineCore.outputText( 'It\'s a tight fit, even with a backdoor as gaping as yours.  You\'re thankful for your looseness as you slide down into her lap, and Urta signals her appreciation of your capacity with a throaty sigh, "<i>Mmmm, you\'re good at this.</i>"\n\n', false );
		}
		//[DOESN'T FIT];
		else {
			tooBig = true;
			EngineCore.outputText( 'It\'s far too big for you, but that doesn\'t stop Urta from pushing it inside you.  You can feel yourself stretched around the invader, and it hurts enough that you almost scream out in pain, but inexorably, you\'re drawn down until your ' + Descriptors.buttDescript() + ' is resting on Urta\'s thighs.  Her thick prick keeps you stretched uncomfortably wide, and you can feel every twitch it makes.', false );
			//(STRETCH TEXT HERE);
			if( !CoC.getInstance().player.buttChange( 60, true ) ) {
				EngineCore.outputText( '  ', false );
			}
			EngineCore.outputText( 'Surprisingly, the over-endowed fox notices your discomfort and hands you a narrow bottle from the table, "<i>Drink up, it\'ll help with the pain until you get used to me.</i>"\n\n', false );
			EngineCore.outputText( 'You look at the bottle suspiciously, reading the faded yellow label, "<i>Barkardi 151</i>".  Wincing from the pain, you suppose it couldn\'t hurt and you tip back the bottle, taking a hearty swig.  It burns like hellfire, and you nearly retch up the massive swallow before you get it down.  Amazingly, the pain in your throat and the bonfire in your belly nearly drowns out what\'s going on in your ' + Descriptors.assholeDescript() + '.  You actually start to relax, and Urta nibbles on your ear in between sips of ale.  She holds her mug up to your lips and invites you to share her drink and you do, grateful for any additional pain-killers.\n\n', false );
		}
		//[BOTH];
		if( CoC.getInstance().player.totalCocks() > 0 ) {
			EngineCore.outputText( 'You start to grow hard from the pressure inside you, and there\'s nothing you can do to stop ', false );
			if( CoC.getInstance().player.totalCocks() > 1 ) {
				EngineCore.outputText( 'each of ', false );
			}
			EngineCore.outputText( 'your ' + Descriptors.multiCockDescriptLight() + ' from swelling to full erectness.  A bead of pre forms at ', false );
			if( CoC.getInstance().player.totalCocks() > 1 ) {
				EngineCore.outputText( 'each ', false );
			} else {
				EngineCore.outputText( 'the ', false );
			}
			EngineCore.outputText( 'tip, and begins leaking down the shaft', false );
			if( CoC.getInstance().player.totalCocks() > 1 ) {
				EngineCore.outputText( 's', false );
			}
			EngineCore.outputText( ' of your ' + Descriptors.multiCockDescriptLight() + ', squeezed out from your prostate.  ', false );
		}
		if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( 'Heat spreads through your loins as your ' + Descriptors.vaginaDescript( 0 ) + ' engorges with blood, allowing your ' + Descriptors.clitDescript() + ' to ', false );
			if( CoC.getInstance().player.clitLength < 2 ) {
				EngineCore.outputText( 'peep out', false );
			} else {
				EngineCore.outputText( 'grow to its full size', false );
			}
			EngineCore.outputText( '.  ', false );
			if( CoC.getInstance().player.vaginas[ 0 ].vaginalWetness < AppearanceDefs.VAGINA_WETNESS_SLICK ) {
				EngineCore.outputText( 'Moisture beads on your lips, signaling just how turned on this total violation is making you.', false );
			} else if( CoC.getInstance().player.vaginas[ 0 ].vaginalWetness < AppearanceDefs.VAGINA_WETNESS_SLAVERING ) {
				EngineCore.outputText( 'Moisture begins dripping onto Urta\'s thighs, signaling just how turned on this total violation is making you.', false );
			} else {
				EngineCore.outputText( 'Moisture puddles on the seat under Urta as your juices flood her thighs, revealing just how turned on this violation is making you.', false );
			}
			EngineCore.outputText( '  ', false );
		}
		//(NIPPLES:;
		EngineCore.outputText( 'Your ' + Descriptors.nippleDescript( 0 ) + 's tent up under your ' + CoC.getInstance().player.armorName, false );
		if( CoC.getInstance().player.hasFuckableNipples() && CoC.getInstance().player.biggestLactation() >= 1 ) {
			EngineCore.outputText( ', leaking milky lubricants and aching to be penetrated.', false );
		} else if( CoC.getInstance().player.hasFuckableNipples() ) {
			EngineCore.outputText( ', aching to be penetrated.', false );
		} else if( CoC.getInstance().player.biggestLactation() >= 1.5 && CoC.getInstance().player.biggestLactation() < 2.5 ) {
			EngineCore.outputText( ', leaking thin streams of milk that rapidly become visible to everyone around you.', false );
		} else if( CoC.getInstance().player.biggestLactation() >= 2.5 ) {
			EngineCore.outputText( ', spurting thick streams of milk that soak your ' + CoC.getInstance().player.armorName + ' and are visible to everyone in the bar.', false );
		} else {
			EngineCore.outputText( '.', false );
		}
		EngineCore.outputText( '\n\n', false );
		EngineCore.outputText( 'Soft-furred hands grip your ' + Descriptors.hipDescript() + ' firmly and begin to lift you up, making you gasp as her flare is dragged through your hind-end, eliciting what Urta must think are cute gasps.  You hear a \'murrrr\' of contentment in your ear and Urta releases you, twitching inside you as your weight slowly carries you back down into her lap.  Once your ' + Descriptors.buttDescript() + ' smacks into her thighs she grabs you again, dragging you back up her massive pole until you\'re looking down on the tables around you.  She stops and withdraws her support, guiding you back down her shaft and moaning when you take her ring of prepuce half-way down.\n\n', false );
		EngineCore.outputText( 'The eyes of everyone in the place are locked onto you, and ', false );
		if( tooBig ) {
			EngineCore.outputText( 'you find yourself giggling drunkenly, barely feeling any pain and arching your back like a slut for the crowd.', false );
		} else if( CoC.getInstance().player.cor < 50 && CoC.getInstance().flags[ kFLAGS.PC_FETISH ] === 0 ) {
			EngineCore.outputText( 'you find yourself whimpering, regretting the decision to climb on top of her, but aroused in spite of it.', false );
		} else {
			EngineCore.outputText( 'you find yourself sliding your hands under your ' + CoC.getInstance().player.armorName + ' and pinching your ' + Descriptors.nippleDescript( 0 ) + 's, putting on a show for the crowd.', false );
		}

		EngineCore.outputText( '  Urta starts breathing hard into your ear, and you start flexing your ' + CoC.getInstance().player.legs() + ' to help her lift you up.  Grateful for the effort, the fox-girl increases the pace by helping to pull you back down each time.  By now it\'s starting to feel pretty good, and her pre-cum is making your ' + Descriptors.assholeDescript() + ' so wet and slippery that you can slide quite quickly along the column of horse-meat.\n\n', false );
		EngineCore.outputText( 'Wet squelches echo through the bar as you\'re slammed into Urta\'s lap harder and harder.', false );
		if( CoC.getInstance().player.biggestTitSize() >= 1 ) {
			EngineCore.outputText( '  Your ' + Descriptors.allBreastsDescript() + ' are jiggling and bouncing wildly, ', false );
			if( CoC.getInstance().player.hasFuckableNipples() ) {
				EngineCore.outputText( 'dripping lubricants', false );
			} else if( CoC.getInstance().player.biggestLactation() > 1 ) {
				EngineCore.outputText( 'spurting milk', false );
			} else {
				EngineCore.outputText( 'rubbing your tight nipples against your ' + CoC.getInstance().player.armorName, false );
			}
			EngineCore.outputText( '.', false );
		}
		if( CoC.getInstance().player.cockTotal() > 0 ) {
			if( CoC.getInstance().player.cockTotal() > 1 ) {
				EngineCore.outputText( '  Steady drizzles of cum begin dripping from each of your ' + Descriptors.multiCockDescriptLight() + ' as she pounds away, rubbing hard against your prostate.', false );
			} else {
				EngineCore.outputText( '  Steady drizzles of cum begin dripping from your ' + Descriptors.multiCockDescriptLight() + ' as she pounds away, rubbing hard against your prostate.', false );
			}
		}
		EngineCore.outputText( '  You can\'t hold back, and start moaning quietly, trying your best to be silent and failing horribly.  For her part, your foxy lover seems equally incapable of suppressing her orgasm, and you feel her thick organ flexing inside you as it prepares to cum.\n\n', false );
		EngineCore.outputText( 'Her paws pull you down hard, crushing your ' + Descriptors.buttDescript() + ' against her as her sheath presses into your ' + Descriptors.assholeDescript() + '.  The invading flesh ripples from the base towards the tip, culminating in a flood of warmth that makes your belly gurgle, and at that moment you cum.  Your sphincter clamps down tightly around Urta\'s marvelous member, unintentionally providing the fox with an even better orgasm.  The next pulse of cum feels even larger, and you look down in shock when you realize you can see the outline of her shaft on the ' + CoC.getInstance().player.skinDesc + ' of your belly.  As she cums you can see it getting less distinct as your body becomes more and more rounded.  A few blasts later you look positively pudgy, but you can feel her softening inside you.\n\n', false );
		EngineCore.outputText( 'Urta sighs and takes a sip of her drink, giving you a quick peck on the side of your neck.  You start to rise, but the drunken hermaphrodite grabs you and holds you down, "<i>Let\'s just stay like this a little longer...</i>"\n\n', false );
	...
	Urta.prototype.getAPublicFacialFromUrta = function() {
		...
		CoC.getInstance().flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ]++;
		this.urtaLove( 2 );
		CoC.getInstance().flags[ kFLAGS.URTA_TIME_SINCE_LAST_CAME ] = 3 + Utils.rand( 3 );
		EngineCore.outputText( '', true );
		EngineCore.outputText( ImageManager.showImage( 'urta-public-facial' ), false );
		EngineCore.outputText( 'Given the choices, it seems like just accepting the facial would be the best option.  Urta nods, grinning lasciviously as she pulls the table to the side, and gestures for you to get on the floor.  You kneel before her as she pulls back her ', false );
		if( CoC.getInstance().flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] < 11 ) {
			EngineCore.outputText( 'skirt ', false );
		} else {
			EngineCore.outputText( 'stockings ', false );
		}
		EngineCore.outputText( 'and allows her horse-dick to flop free in front of you.  It pulses, growing bigger with each beat of the fox\'s heart as she watches you submit yourself to her arousal.  You feel the eyes of everyone around you as they watch the spectacle ', false );
		if( CoC.getInstance().player.cor < 33 ) {
			EngineCore.outputText( 'and it makes you blush crimson', false );
		} else if( CoC.getInstance().player.cor < 66 ) {
			EngineCore.outputText( 'and it makes you uncomfortable but horny', false );
		} else {
			EngineCore.outputText( 'and it makes you randy as hell', false );
		}
		EngineCore.outputText( '.\n\n', false );
		EngineCore.outputText( 'Two dexterous fur-covered hands wrap around her mottled shaft and begin pumping it.  Almost immediately the flattened-tip begins flaring wide and produces a dollop of fragrant equine pre-cum.  Urta\'s balls quiver happily as she plays with herself, and as you look up at her and open wide, ', false );
		if( CoC.getInstance().player.cor < 75 ) {
			EngineCore.outputText( 'doing your best to play your part in the scene.', false );
		} else {
			EngineCore.outputText( 'hoping to encourage her to absolutely soak you with cum.', false );
		}
		EngineCore.outputText( '\n\nUrta looks down with a surprisingly shy expression and asks, "<i>Could you... lick the tip please?  For me?</i>"\n\n', false );
		if( CoC.getInstance().player.cor < 50 ) {
			EngineCore.outputText( 'Hesitantly, you extend your tongue and lick the underside of her flat-tip.  ', false );
		} else {
			EngineCore.outputText( 'Eagerly, you lick around the outside edge of her flared tip, sticking your tongue out as far as you can to lick the underside once you finish.  ', false );
		}
		EngineCore.outputText( 'Your efforts are rewarded when Urta groans and spurts a dollop of pre-cum onto your tongue, and you can see her urethra twitching and pulsating as more of the clear fluid begins to leak out.  It drips down onto your tongue as the fox picks up her pace, feverishly masturbating her bloated shaft.  She says, "<i>Swallow that pre and get ready slut, you\'ll need an empty mouth to hold all this.</i>"\n\n', false );
		EngineCore.outputText( 'You swallow, tasting the sweet pre-cum as it slides down your throat.  Risking a glance to the side, you realize there\'s a crowd gathering around you, and more than a few dog-dicks and horse-cocks are being stroked.  ', false );
		if( CoC.getInstance().player.cor > 50 ) {
			EngineCore.outputText( 'You hope the show will be arousing enough to get them off on you as well, and with renewed determination you turn back to Urta and give her balls a squeeze.', false );
		} else {
			EngineCore.outputText( 'You shudder in shame, and hope that the crowd will disperse. It\'s humiliating enough waiting to have Urta soak you with cum.', false );
		}
		EngineCore.outputText( '  She trembles and her tip flares wide, and you know she\'s starting to cum.\n\n', false );
		EngineCore.outputText( 'Her urethra dilates before your eyes and time seems to slow down as the first blob of horse-semen is launched towards you.  Blinking at the last second, you feel it splatter over your ' + CoC.getInstance().player.face() + '.  You open your mouth wider, keeping your eyes closed but knowing you\'re expected to swallow some of it.  Urta\'s aim improves with the next blast, and it does land in your mouth.  You nearly gag from the sudden salty intrusion, and due to the volume of the seed a fair portion of it leaks out the sides of your mouth as you keep it open, expecting more.  You feel more cum land in your hair, then on your cheek, and finally on your neck and ', false );
		if( CoC.getInstance().player.biggestTitSize() < 1 ) {
			EngineCore.outputText( 'chest.  ', false );
		} else {
			EngineCore.outputText( 'cleavage.  ', false );
		}
		EngineCore.outputText( 'Urta paints you with the stuff, until you\'re practically oozing cum.  It does eventually stop, and you wipe the jism from your eyes', false );
		if( CoC.getInstance().player.cor > 60 ) {
			EngineCore.outputText( ', being sure to lick your fingers clean afterwards', false );
		}
		EngineCore.outputText( '.\n\n', false );
		if( CoC.getInstance().player.cor > 50 ) {
			...
			if( CoC.getInstance().flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] >= 11 ) {
				EngineCore.outputText( 'stuffs herself back into her fishnet', false );
			} else {
				EngineCore.outputText( 'curls her tail back around her member, concealing it from view', false );
			}
			EngineCore.outputText( '.', false );
			EngineCore.dynStats( 'lus', 5 + Utils.rand( 5 ) + CoC.getInstance().player.lib / 10 );
		}
		EngineCore.doNext( SceneLib.camp.returnToCampUseOneHour );
	};
	Urta.prototype.optionalUrtaBukkake = function() {
		this.urtaSprite();
		CoC.getInstance().player.slimeFeed();
		CoC.getInstance().flags[ kFLAGS.URTA_TIME_SINCE_LAST_CAME ] -= 2;
		this.urtaLove( -1 );
		EngineCore.outputText( '', true );
		EngineCore.outputText( ImageManager.showImage( 'urta-bukkake' ), false );
		EngineCore.outputText( 'You pause to tear off your ' + CoC.getInstance().player.armorName + ' and expose the rest of your body, and you reach out into the crowd, grabbing the nearest cock and caressing it as you beg, "<i>Come on and cum, she missed most of me!</i>"\n\n', false );
		EngineCore.outputText( 'Urta leans back with a smile, toying with her softening member as a ring of horny studs and herms encircles you, jerking themselves off furiously.  You take turns touching and caressing them, murmuring enticing words as they start cumming on you, one or two at a time.  As those that finish step away, they\'re replaced by newcomers, turned on by the cum-hungry slut in front of them.  You do your best to greet them with a wink, a stroke, and a slurp, and then you present your ', false );
		if( CoC.getInstance().player.biggestTitSize() >= 1 ) {
			EngineCore.outputText( Descriptors.allBreastsDescript(), false );
		} else {
			EngineCore.outputText( Descriptors.nippleDescript( 0 ) + 's', false );
		...
		EngineCore.dynStats( 'lus', 15 + Utils.rand( 5 ) + CoC.getInstance().player.lib / 10 );
		EngineCore.outputText( 'An hour later you\'re totally drenched in cum.  It\'s dripping off you from your hair down to your ' + CoC.getInstance().player.feet() + ', and you love it.  You lick up the mixed spooge until you start to feel full, and a blushing barmaid comes forward to hand you a towel.  You give her a salty kiss on the lips, sending her running away before you towel off and dress yourself.', false );
		EngineCore.doNext( SceneLib.camp.returnToCampUseOneHour );
	};
	...
	Urta.prototype.goBackToUrtasForLuvinz = function() {
		this.urtaSprite();
		EngineCore.outputText( '', true );
		EngineCore.outputText( ImageManager.showImage( 'urta-home-image' ), false );
		if( CoC.getInstance().flags[ kFLAGS.URTA_PC_LOVE_COUNTER ] === 1 ) {
			EngineCore.outputText( 'Urta gives you a warm smile and grabs you by the hand, pulling you out the Wet Bitch\'s backdoor and into Tel\'Adre\'s back-alleys.  Her dress does little to conceal the bulging mass underneath or the growing wet spot that forms at the tip of her impressive tent.  The long, body-hugging garment makes it difficult for Urta to walk in her current state, and you don\'t make it any easier for her, constantly teasing the fox as you navigate the back-alleys of Tel\'Adre.\n\n', false );
			EngineCore.outputText( 'Every chance you get, you brush a hand or a hip against her sensitive nether-bulge or lust-tightened nipples.  For someone with such a bad-ass reputation, Urta\'s quite easy to tease, and she makes adorable gasps and shuddering shakes every time you brush against her flare or surprise her with a tender caress.  She lets out cute whimpers as she shuffles onward, but she doesn\'t protest as her \'male\' half drenches her dress with more and more slimy pre-cum.\n\n', false );
			EngineCore.outputText( 'The door to her apartment looms before you and you push her inside.  You do your best to stifle an involuntary snicker as you close the door and see a trail of Urta\'s moist arousal on the street right up to the door.  The apartment is impeccably clean, though you see a few large sex-toys half-hidden about the place.  For someone as ashamed of her desires as Urta was, she sure is terrible at hiding her kinks.' );
			if( CoC.getInstance().flags[ kFLAGS.URTA_PETPLAY_DONE ] === 0 && this.urtaLove() ) {
				EngineCore.outputText( '  <b>A new, shiny leather collar lies half-concealed under a cushion, as though its owner haphazardly tried to bury it in a rush.</b>' );
			}
			EngineCore.outputText( '  The busty fox has already retreated from the front-room, the half-soaked dress on the floor the only indication she was there with you a moment ago.\n\n', false );
			EngineCore.outputText( 'You follow a trail of pre-cum on the floor and the scent of copious female arousal deeper into the apartment, around a corner, and into Urta\'s relatively small bedroom.  Your vulpine lover is reclined on her bed, wearing nothing but lacy stockings and a latex cock-sheath.  ', false );
			if( CoC.getInstance().player.totalCocks() > 0 ) {
		...
		EngineCore.outputText( 'You assure her that you aren\'t offended', false );
		if( CoC.getInstance().player.lib > 50 || CoC.getInstance().player.cor > 50 ) {
			EngineCore.outputText( ' and are actually turned on by the thought of walking through the city with such a \'big\' lover hanging off your arm', false );
		}
		EngineCore.outputText( '.  Urta smiles weakly and hooks her arm through yours, leading you towards her apartment, making sure to stick to the more abandoned back-roads.  Her caution doesn\'t stop the pair of you from giving a few free shows, and if anything, each brief encounter with a passerby seems to be making her bigger and harder, revealing a half-buried exhibitionist streak.\n\n', false );
		this.goBackToUrtasForLuvinzII();
	};
	Urta.prototype.goBackToUrtasForLuvinzII = function() {
		EngineCore.outputText( 'The journey is blessedly brief, and within minutes you arrive at Urta\'s house.  She fiddles with the lock anxiously for a moment before finally getting a -CLICK- and unlocking it.  You sweep inside behind your host and look around, mildly shocked at the spartan surroundings.  Urta\'s apartment is tiny, cramped, and nearly devoid of possessions beyond weapons, armor, and sex-toys.', false );
		if( CoC.getInstance().flags[ kFLAGS.PC_SEEN_URTA_SEX_TOYS ] === 0 ) {
			EngineCore.outputText( '  The last group is the most numerous of those three.', false );
			if( CoC.getInstance().giacomo > 0 ) {
				EngineCore.outputText( '  You recognize some of the toys as Giacomo\'s wares, though there are a fair number that seem to be custom made, and others that are clearly living creatures.', false );
			}
			EngineCore.outputText( '  Looking over the things she had to use before she met you, it dawns on you just how lonely the poor fox has been.', false );
		} else {
			EngineCore.outputText( '  While the last group used to be the most numerous, it looks like Urta has cleaned house and only has a few of the toys left.  It seems that since meeting you she hasn\'t had to turn to her massive device collection like she used to.  It dawns on you just what a difference you\'ve made in Urta\'s life, and it brings a loving smile to your face.', false );
		}
		EngineCore.outputText( '\n\n', false );
		EngineCore.outputText( '"<i>Sexy, are you going to come get some?</i>" Urta calls out from the bedroom in the back.  You slap your forehead, angry for getting so distracted while a hot piece of ass is on the line.  Heading deeper into the apartment after her, you turn the corner to her bedroom and are floored by the sight.  Urta is totally nude save for a pair of black fishnet stockings and some kind of shining latex cock-sheath.  She\'s on all fours on top of a fur-lined bed, and wiggling her hind-end at you with her tail lifted in the air.  The scent of need pours off from her sex, and you find yourself as ', false );
		if( CoC.getInstance().player.hasVagina() && CoC.getInstance().player.totalCocks() > 0 ) {
			EngineCore.outputText( 'hard and wet as you\'ve ever been', false );
		} else if( CoC.getInstance().player.totalCocks() > 0 ) {
			EngineCore.outputText( 'hard as you\'ve ever been', false );
		} else if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( 'wet as you\'ve ever been', false );
		} else {
			EngineCore.outputText( 'ready as you\'ve ever been to take such a monster', false );
		}
		EngineCore.outputText( '.\n\n', false );
		EngineCore.outputText( 'Urta is clearly looking to take a more passive role, and she begins stroking herself as she asks, "<i>How do you want to fuck, lover?</i>"', false );
		...
		if( CoC.getInstance().player.isGoo() && CoC.getInstance().player.skinType === AppearanceDefs.SKIN_TYPE_GOO ) {
			EngineCore.addButton( 4, 'Goo (Weird)', this.urtaGooTesticleVoreRuinedOrgasms );
		}
	};
	Urta.prototype.rideUrtaTenderFemale = function() {
		this.urtaSprite();
		CoC.getInstance().flags[ kFLAGS.PC_SEEN_URTA_SEX_TOYS ]++;
		this.urtaLove( 1.5 );
		CoC.getInstance().flags[ kFLAGS.TIMES_FUCKED_URTA ]++;
		CoC.getInstance().flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ]++;
		CoC.getInstance().flags[ kFLAGS.URTA_TIME_SINCE_LAST_CAME ] = 3 + Utils.rand( 2 );
		EngineCore.outputText( '', true );
		EngineCore.outputText( ImageManager.showImage( 'urta-home-female-ride' ), false );
		EngineCore.outputText( 'You climb into bed and playfully roll Urta over.  Her tits and horse-cock flop and jiggle from the change in position, and you watch mesmerized for a moment until Urta giggles girlishly, drawing you out of your reverie.\n\n', false );
		EngineCore.outputText( '"<i>Do you like them that much?</i>" she asks.\n\n', false );
		EngineCore.outputText( 'You nod ', false );
		if( CoC.getInstance().player.cor < 50 || CoC.getInstance().player.lib < 50 ) {
			EngineCore.outputText( 'sheepishly ', false );
		} else {
			EngineCore.outputText( 'eagerly ', false );
		}
		EngineCore.outputText( 'and grab ahold of her latex-wrapped member, giving it gentle strokes along the underside.  Urta moans loudly and twitches her hips, already eager for more, and the ', false );
		if( CoC.getInstance().player.vaginas[ 0 ].vaginalWetness < AppearanceDefs.VAGINA_WETNESS_WET ) {
			EngineCore.outputText( 'growing moistness in your loins', false );
		} else if( CoC.getInstance().player.vaginas[ 0 ].vaginalWetness < AppearanceDefs.VAGINA_WETNESS_DROOLING ) {
			EngineCore.outputText( 'sopping wetness in your loins', false );
		} else {
			EngineCore.outputText( 'flood of moisture leaking from your crotch', false );
		}
		EngineCore.outputText( ' signals your readiness to take her.', false );
		if( CoC.getInstance().player.totalCocks() > 0 ) {
			if( CoC.getInstance().player.totalCocks() === 1 ) {
				EngineCore.outputText( '  Your ' + Descriptors.multiCockDescriptLight() + ' is raging hard and trembling, but ignored for now.', false );
			} else {
				EngineCore.outputText( '  Each of your ' + Descriptors.multiCockDescriptLight() + ' is raging hard and trembling, but ignored for now.', false );
			}
		}
		EngineCore.outputText( '  Climbing atop the bed with her, you swivel your ' + Descriptors.hipDescript() + ' about, positioning her at your entrance and straddling your ' + CoC.getInstance().player.legs() + ' around her.\n\n', false );
		EngineCore.outputText( 'You can see a bubble already forming at the tip of her hardness, signaling the unbridled lust your form has brought to your lover and you lower yourself with a sigh, feeling it spread your lips apart with delicious slowness as her flared tip begins penetrating you.  The horse-cock is so big and thick that it ', false );
		if( CoC.getInstance().player.vaginalCapacity() < 60 ) {
			EngineCore.outputText( 'hurts quite a bit as it pushes further and further inside.  ', false );
			CoC.getInstance().player.cuntChange( 60, true );
		} else {
			EngineCore.outputText( 'stretches you nice and wide, rubbing you tightly even though you\'re quite stretched already.  ', false );
			CoC.getInstance().player.cuntChange( 60, true );
		}
		EngineCore.outputText( 'You gasp and sigh happily, sinking downwards slowly.\n\n', false );
		EngineCore.outputText( 'The ring of prepuce passes past your entrance ', false );
		if( CoC.getInstance().player.vaginalCapacity() < 60 ) {
			EngineCore.outputText( 'and you bottom out there, unable to take in any more of her equine shaft.', false );
		} else {
			EngineCore.outputText( 'and you keep going, sliding down along the marvelous member until your cunt-lips are rubbing her sheath against your [clit].', false );
		}
		EngineCore.outputText( '\n\n', false );
		EngineCore.outputText( 'Urta growls and grabs you by the back of the neck, pulling your ' + CoC.getInstance().player.face() + ' down for a sloppy kiss.  Her smooth canine-like tongue forces its way into your mouth and curls around yours as the two of you swap spit.  Your hips, spurred on by the passion, begin to rise and fall to the tempo of the kiss, and in seconds you\'re both gasping and moaning against each other\'s lips, lost in the throes of passion.  Deep inside you, you can feel the condom filling with Urta\'s incredibly copious pre-cum, but every time your hips drop down the pressure squeezes some of it down along her length until it escapes around the condom\'s edge, dripping into her sheath.\n\n', false );
		//(Big Tits:;
		if( CoC.getInstance().player.biggestTitSize() >= 5 ) {
			EngineCore.outputText( 'Your ' + Descriptors.breastDescript( 0 ) + ' mash into the fox\'s own, your ' + Descriptors.nippleDescript( 0 ) + 's ', false );
			if( !CoC.getInstance().player.hasFuckableNipples() ) {
				EngineCore.outputText( 'rubbing over her black-skinned buds.', false );
			} else {
				EngineCore.outputText( 'parting as her prominent black nipples slip inside your own.', false );
			}
			EngineCore.outputText( '  You squeal at the sudden pleasure, arching your back and furthering the sensation from your over-burdened chest.', false );
			if( CoC.getInstance().player.totalCocks() > 0 ) {
				if( CoC.getInstance().player.cocks[ 0 ].cockLength > 15 ) {
					if( CoC.getInstance().player.totalCocks() === 1 ) {
						EngineCore.outputText( '  Your ' + Descriptors.multiCockDescriptLight() + ' pushes up between the jiggling breasts, slicking them with your pre-cum as it begins to get tit-fucked by both of your trembling forms.', false );
					} else {
						EngineCore.outputText( '  Each of your ' + Descriptors.multiCockDescriptLight() + ' pushes up between the jiggling breasts, slicking them with your pre-cum as they begin to get tit-fucked by both of your trembling forms.', false );
					}
				}
			}
			EngineCore.outputText( '\n\n', false );
		}
		EngineCore.outputText( 'You both cry out in orgiastic pleasure as your shaking ' + Descriptors.hipDescript() + ' force your tightly-stretched cunt over Urta\'s rubber-wrapped horse-prick again and again.  Your internal muscles spasm, clamping down tightly in sudden orgasm.  Urta\'s voice trembles in a low moan of satisfaction while her lower body twitches upwards with hard pumping motions.  Her dick trembles inside you, and you feel the latex around her girthy prick inflating with her seed and pressing against your cervix.\n\n', false );
		if( CoC.getInstance().player.totalCocks() > 0 ) {
			if( CoC.getInstance().player.cockTotal() === 1 ) {
				EngineCore.outputText( 'Your own ' + Descriptors.multiCockDescriptLight() + ' cums as well, blasting seed over both your ', false );
				if( CoC.getInstance().player.cocks[ 0 ].cockLength > 15 && CoC.getInstance().player.biggestTitSize() >= 5 ) {
					EngineCore.outputText( 'jiggling tits', false );
				} else {
					EngineCore.outputText( 'bellies', false );
				}
				EngineCore.outputText( ', glazing them in slippery goo.', false );
			} else {
				EngineCore.outputText( 'Each of your own ' + Descriptors.multiCockDescriptLight() + ' cums as well, blasting seed over both your ', false );
				if( CoC.getInstance().player.cocks[ 0 ].cockLength > 15 && CoC.getInstance().player.biggestTitSize() >= 5 ) {
					EngineCore.outputText( 'jiggling tits', false );
				} else {
					EngineCore.outputText( 'bellies', false );
		...
		EngineCore.outputText( 'Lying down on top of her, the two of you cuddle while joined at the hips, snuggling happily', false );
		if( CoC.getInstance().player.totalCocks() > 0 ) {
			EngineCore.outputText( ' and ignoring the hot, wet mess your ' + Descriptors.multiCockDescriptLight() + ' ', false );
			if( CoC.getInstance().player.totalCocks() === 1 ) {
				EngineCore.outputText( 'has', false );
			} else {
				EngineCore.outputText( 'have', false );
			}
			EngineCore.outputText( ' left between your nude bodies', false );
		}
		EngineCore.outputText( '.  The fox-girl gives you a long, happy kiss and holds you tightly in her arms before she lets you go and begins to pull out.  The loud -SCHLLLIIIICK- of her fat member being dragged through your nethers makes you blush, and the feeling of it robs you of any strength below the waist.  The latex stretches, the cum-bubble still trapped inside you and stretching the condom obscenely for a moment, until it jerks out with a sudden \'pop\'.  The relief is palpable, but your ' + Descriptors.vaginaDescript( 0 ) + ' hangs open, with lips parted loosely after today\'s incredible violation.\n\n', false );
		EngineCore.outputText( 'Urta sighs and languidly pulls off her condom, nearly getting cum everywhere before she ties it off.  It looks like it\'ll pop at any moment – there\'s probably over a liter of cum stored in it.  She hefts it and mocks like she\'s going to throw it at you, but stops and giggles before setting it down beside the bed.  Your foxy lover says, ', false );
		if( CoC.getInstance().flags[ kFLAGS.TIMES_FUCKED_URTA ] <= 10 || CoC.getInstance().flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] < 11 ) {
			EngineCore.outputText( '"<i>Lovemaking is so much of a hassle to clean up if I don\'t use one of these!  At least I don\'t have to use them for the same reason as most of Tel\'Adre.  It turns out I\'m sterile or damn close to it, so all that fun-juice is just for show.</i>"', false );
		} else {
			EngineCore.outputText( '</i>"<i>You\'re so good to me ' + CoC.getInstance().player.short + '.  I wish my little pony here was fertile enough for me to put a bun in your oven, if you wanted of course.</i>"', false );
		}
		...
		EngineCore.outputText( 'You nod before you leave, feeling satisfied and a warm glow from something other than lust.', false );
		//if(CoC.getInstance().flags[kFLAGS.URTA_FERTILE] === 1) CoC.getInstance().player.knockUp(21,515,80);;
		CoC.getInstance().player.orgasm();
		EngineCore.dynStats( 'sen', -2 );
		EngineCore.doNext( SceneLib.camp.returnToCampUseOneHour );
	};
	...
	Urta.prototype.dudeFuckingUrtasCooch = function() {
		...
		var cocks = CoC.getInstance().player.totalCocks();
		CoC.getInstance().flags[ kFLAGS.PC_SEEN_URTA_SEX_TOYS ]++;
		CoC.getInstance().flags[ kFLAGS.TIMES_FUCKED_URTA ]++;
		this.urtaLove( 1.5 );
		CoC.getInstance().flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ]--;
		...
		CoC.getInstance().flags[ kFLAGS.URTA_TIME_SINCE_LAST_CAME ] = 2;
		EngineCore.outputText( 'You let her know that you want her just the way she is and climb into bed behind her.  She watches you apprehensively over her shoulder as you pull out your ' + Descriptors.multiCockDescriptLight() + ' and start lining yourself up to fuck her doggie-style.  ', false );
		if( cocks > 1 && y >= 0 ) {
			EngineCore.outputText( 'Your secondary ' + Descriptors.cockDescript( y ) + ' wobbles directly behind her back-door and Urta\'s eyes widen a bit when she realizes you\'re going to double-penetrate her.  ', false );
		}
		if( cocks > 2 ) {
			EngineCore.outputText( 'The remainder of your ' + Descriptors.multiCockDescriptLight() + ' fan out, ready to rub themselves against her butt-cheeks once you push inside.  ', false );
		}
		EngineCore.outputText( 'You can see your lover\'s condom starting to bloat at the tip as her horse-cock begins to flare within its confinement.  She trembles with anticipation before taking a deep breath and saying, "<i>I\'m ready... take me!</i>"\n\n', false );
		EngineCore.outputText( 'Obligingly, you ease your hips forward, reeling at the heat of her cunt-lips as they welcome your ' + Descriptors.cockDescript( 0 ) + '\'s ' + CoC.getInstance().player.cockHead() + ' with slippery moisture.  ', false );
		if( cocks > 1 && y >= 0 ) {
			EngineCore.outputText( 'Your second ' + Appearance.cockNoun( CoC.getInstance().player.cocks[ y ].cockType ) + ' soon bumps against her pucker and strains against it, pushing ever-so-slowly into her tight hole as she gradually relaxes before you.  ', false );
		}
		EngineCore.outputText( 'As you carefully breach the opening', false );
		if( cocks > 1 && y >= 0 ) {
			EngineCore.outputText( 's', false );
		}
		EngineCore.outputText( ', the going gets easier and easier for you.  You gently rock your [hips] forwards, sliding deep inside her until she\'s taken you to the hilt.  Once you\'re fully embedded within her, Urta wriggles happily beneath you, and you reach around to caress her drooping member, relishing in her pleasured yelps and moans.\n\n', false );
		EngineCore.outputText( 'You rock back and forth, picking up the tempo gradually.  At first the gray fox is only gasping happily, but as you go faster and faster, her gasps turn into quiet moans, then transform into ecstatic squeals.  ', false );
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( 'Your balls loudly smack into her squelching-wet cunt and the soaked backside of HER balls', false );
		} else {
			EngineCore.outputText( 'Your crotch slaps loudly against her squelching-wet cunt and backside', false );
		}
		...
		EngineCore.outputText( 'Urta cums first.  Her cries of pleasure are muffled as she shoves her face into a pillow, screaming in ecstasy.  The warmth around your ' + Descriptors.cockDescript( x ) + ' seems to grow that much hotter as it squeezes you, wringing you tightly from root to tip.  ', false );
		if( cocks > 1 && y >= 0 ) {
			EngineCore.outputText( 'The intensity and pressure in her back-door is even worse, and your ' + Descriptors.cockDescript( y ) + ' is nearly in pain from her asshole\'s tight, pulsating grip.  ', false );
		}
		EngineCore.outputText( 'Her equine member trembles in your grasp as it releases its sticky payload into the thick, latex condom.  You pump away at her shaft, feeling the spunk inside distort the tube-like appendage until your orgasm shocks you into releasing it.\n\n', false );
		EngineCore.outputText( 'You spurt into Urta\'s sloppy box, creaming it until her juices begin to run in an off-white color.  ', false );
		if( cocks > 1 && y >= 0 ) {
			EngineCore.outputText( 'The ' + Descriptors.cockDescript( y ) + ' you\'ve crammed in her ass does not disappoint either, matching its brother squirt for squirt as it spends your seed with reckless abandon.  ', false );
		}
		if( cocks > 2 ) {
			EngineCore.outputText( 'Her butt and tail are covered with the stuff as each of your remaining ' + Descriptors.multiCockDescriptLight() + ' splatters her with spooge, covering her fur in white sludge.  ', false );
		}
		EngineCore.outputText( 'The fox\'s internal muscles happily continue to squeeze and milk you, prolonging your orgasm beyond its normal duration.', false );
		if( CoC.getInstance().player.cumQ() >= 750 ) {
			EngineCore.outputText( '  Her body is quickly bloated thanks to your body\'s incredible fertility, but she seems able to take it just fine.', false );
		}
		EngineCore.outputText( '\n\n', false );
		EngineCore.outputText( 'The two of you collapse together in a shuddering pile of post-orgasmic bliss.  The shift in position frees your ' + Descriptors.multiCockDescriptLight() + ' from Urta with a noisy -SHHHLLUUUCK-, but you\'re too wiped out to care.  The fox-girl sighs happily and rolls over, planting a kiss on your nose before she works at removing her over-filled condom.  It nearly bursts when she ties it off, but she manages to gently set it down to the floor without incident. She looks at you happily and gives you a big hug, crushing her hard nipples against ', false );
		if( CoC.getInstance().player.biggestTitSize() >= 1 ) {
			EngineCore.outputText( 'your own ' + Descriptors.breastDescript( 0 ), false );
		} else {
			EngineCore.outputText( 'your chest', false );
		}
		EngineCore.outputText( ' as she says, "<i>Thank you so much, lover.  I forget how good it can be in my... vagina.  Maybe we could do this again?</i>"\n\n', false );
		EngineCore.outputText( 'You give her a happy nod, and the two of you cuddle a bit longer before you leave to return to camp.', false );
		CoC.getInstance().player.orgasm();
		EngineCore.dynStats( 'sen', -2 );
		this.knockUpUrtaChance();
		EngineCore.doNext( SceneLib.camp.returnToCampUseOneHour );
	...
	Urta.prototype.fuckItAndLeave = function() {
		...
		CoC.getInstance().flags[ kFLAGS.URTA_TIME_SINCE_LAST_CAME ] = 36;
		this.urtaLove( -5 );
		EngineCore.outputText( '', true );
	...
	Urta.prototype.tenderTakeItUpTheAssFromUrta = function() {
		...
		CoC.getInstance().flags[ kFLAGS.TIMES_FUCKED_URTA ]++;
		CoC.getInstance().flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ]++;
		CoC.getInstance().flags[ kFLAGS.URTA_TIME_SINCE_LAST_CAME ] = 2 + Utils.rand( 3 );
		var cocks = CoC.getInstance().player.totalCocks();
		EngineCore.outputText( ImageManager.showImage( 'urta-home-anal' ), false );
		EngineCore.outputText( 'You give Urta\'s backside a playful slap and ask her to roll over.  She does so with a hesitant look on her face, wondering what you\'re about.  When you spread her legs and work your way forward to straddle her, a beaming smile breaks across her face and she says, "<i>You really want me inside you, don\'t you ' + CoC.getInstance().player.short + '?</i>"\n\n', false );
		EngineCore.outputText( 'Her cheeks color when you give her a nod and begin lowering your ' + Descriptors.buttDescript() + ' towards her pillar of horse-flesh. She gasps quietly when the tip begins to push against your ' + Descriptors.assholeDescript() + '.  ', false );
		if( CoC.getInstance().player.analCapacity() >= 60 ) {
			EngineCore.outputText( 'You sigh and begin sliding down it, your previously-stretched backdoor easily accepting the gigantic intruder as it fills you.', false );
		} else {
			EngineCore.outputText( 'You grunt and begin sliding down it, feeling painfully stretched by the gigantic intruder but pressing on anyway, knowing full-well you\'ll be a bit looser by the time you take her whole length.', false );
			if( CoC.getInstance().player.ass.analLooseness < 5 ) {
				if( CoC.getInstance().player.ass.analLooseness === 0 ) {
					EngineCore.outputText( '<b>  Urta has taken your anal virginity.</b>', false );
				} else {
					EngineCore.outputText( '<b>  Your ass is stretched.</b>', false );
				}
				CoC.getInstance().player.ass.analLooseness++;
			}
		}
		//Butt Stretched used to determine how long since last enlargement;
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.ButtStretched ) < 0 ) {
			CoC.getInstance().player.createStatusAffect( StatusAffects.ButtStretched, 0, 0, 0, 0 );
		}//Reset the timer on it to 0 when restretched.;
		else {
			CoC.getInstance().player.changeStatusValue( StatusAffects.ButtStretched, 1, 0 );
		}
		if( CoC.getInstance().player.totalCocks() > 0 ) {
			EngineCore.outputText( '  The bumps and veins keep rubbing against your prostate, and ', false );
			if( cocks > 1 ) {
				EngineCore.outputText( 'each of ', false );
			}
			EngineCore.outputText( 'your ' + Descriptors.multiCockDescriptLight() + ' hardens near instantly from the forced stimulation.', false );
		}
		EngineCore.outputText( '  Slowly but surely, it slips further and further inside you, until you\'ve bottomed out and have miraculously taken her entire shaft.\n\n', false );

		//(IF DICKS);
		if( cocks > 0 ) {
			EngineCore.outputText( 'For a few moments you sit on her deeply penetrating thickness, and ', false );
			if( cocks > 1 ) {
				EngineCore.outputText( 'each of ', false );
			}
			EngineCore.outputText( 'your ' + Descriptors.multiCockDescriptLight() + ' droops onto her belly, half-hard from the pressure on your prostate, but going a little soft from the lack of attention.', false );
			if( CoC.getInstance().player.cocks[ 0 ].cockLength >= 15 ) {
				EngineCore.outputText( '  Urta winks and guides ', false );
				if( cocks > 1 ) {
					EngineCore.outputText( 'one of ', false );
				}
				EngineCore.outputText( 'your ' + Descriptors.multiCockDescriptLight() + ' between her tits', false );
				if( CoC.getInstance().player.cocks[ 0 ].cockLength >= 20 && CoC.getInstance().player.cocks[ 0 ].cockLength <= 28 ) {
					EngineCore.outputText( ', even taking time to lick and suck on the head', false );
				}
				EngineCore.outputText( '.', false );
				EngineCore.outputText( '  She begins giving you a slow tit-fuck, trying to encourage you to pick up the pace and start riding her.  It does the trick as your ' + Descriptors.hipDescript() + ' start rocking on their own in reaction to her efforts.  She licks harder, and you reward her by lifting yourself up and sliding back down.  Her massive girth squeezes a squirt of pre-cum from your prostate with every few strokes, splattering it on her muzzle.', false );
			}
			EngineCore.outputText( '\n\n', false );
		} else {
			EngineCore.outputText( 'For a few moments you sit like that, trying to adjust to the pressure in your backside.  After a few moments you can feel your muscles relaxing, allowing Urta to rest more comfortably inside you.  Flexing your ' + CoC.getInstance().player.legs() + ', you pull yourself up off of her until the flared tip is just barely inside you, then you plunge back down.  Urta\'s mouth hangs open and her tongue lolls out, and you can feel the warmth of her leaking fluid inside of you, letting you know just how good your ' + Descriptors.assholeDescript() + ' feels to her.\n\n', false );
		}
		EngineCore.outputText( 'You bounce on top of your fox-lover\'s equine rod, spearing your ' + Descriptors.assholeDescript() + ' on it over and over.  ', false );
		if( cocks === 0 ) {
			EngineCore.outputText( 'Strange pleasure spreads from your sensitive opening, and you know that somehow you\'re going to cum from this.  ', false );
		} else {
			EngineCore.outputText( 'Powerful pleasure spreads through you as it bumps and rubs your prostate. You know you\'ll cum from this, and soon.  ', false );
		}
		EngineCore.outputText( 'Urta cries out and grabs you by the hips, yanking you down against her sheath.  Warmth blossoms inside you as her seed begins pumping into your bowels.  You can feel yourself filling up with her seed and you cum, clamping down around her intruder as it injects more and more foxy-horse-spunk into you.\n\n', false );
		//(DICKS);
		if( cocks > 0 ) {
			if( CoC.getInstance().player.cocks[ 0 ].cockLength < 15 ) {
				EngineCore.outputText( 'Your orgasm erupts out onto her belly, splattering her fur with your sticky cum.  ', false );
			} else if( CoC.getInstance().player.cocks[ 0 ].cockLength < 20 ) {
				EngineCore.outputText( 'Your orgasm erupts all over her tits, slicking them with your sticky white cum.  ', false );
			} else {
				EngineCore.outputText( 'Your orgasm erupts into her muzzle, filling her mouth with your sticky, white cum.  ', false );
			}
			EngineCore.outputText( '  Urta groans underneath you and begins caressing and squeezing you, milking you of every drop.', false );
			if( CoC.getInstance().player.cumQ() >= 500 ) {
				EngineCore.outputText( '  Urta gulps down as much as she can but it begins leaking around her muzzle, and she eventually lets it pop free, giving up on swallowing it entirely.', false );
			}
			if( CoC.getInstance().player.cumQ() >= 1000 && CoC.getInstance().player.cumQ() < 1500 ) {
				EngineCore.outputText( '  Your output easily soaks her, and she gathers it up and slops it over her nipples, pinching and pulling on them.', false );
			}
			if( CoC.getInstance().player.cumQ() >= 1500 ) {
				EngineCore.outputText( '  Your output soaks her and the bed with seed, leaving the two of you in a warm gooey puddle in the very center of the bed.', false );
			}
			EngineCore.outputText( '\n\n', false );
		}
		EngineCore.outputText( 'Urta pulls you down onto her breasts and gives you a long hug and a happy lick.\n\n', false );
		EngineCore.outputText( '"<i>Thank you soooo much lover, I needed that,</i>" she says, "<i>Now you better go get back to your business, I\'ve got to get cleaned up!', false );
		if( CoC.getInstance().player.cumQ() > 750 ) {
			EngineCore.outputText( '  You really are a messy, ' + CoC.getInstance().player.mf( 'boy', 'girl' ) + ' aren\'t you?', false );
		}
		EngineCore.outputText( '</i>"\n\n', false );
	...
	Urta.prototype.marbleCockuBlockuUrta = function() {
		this.urtaSprite();
		EngineCore.outputText( '', true );
		EngineCore.outputText( 'Urta gives you a serious look when you come up to her, "<i>I just got a report in from some of our scouts last night.  It looks like they found your camp.  There was a cow-girl there', false );
		//if nursery;
		if( CoC.getInstance().flags[ kFLAGS.MARBLE_NURSERY_CONSTRUCTION ] === 100 ) {
			EngineCore.outputText( ' and a nursery', false );
		}
		EngineCore.outputText( '.', false );
		//(Not wuv'ed yet);
		if( CoC.getInstance().flags[ kFLAGS.URTA_PC_LOVE_COUNTER ] === 0 ) {
			EngineCore.outputText( '  I know we aren\'t a couple or anything, but, is she something to you?  Are you lovers?</i>"\n\n', false );
			EngineCore.outputText( 'Urta\'s cheeks color sheepishly and she says, "<i>You don\'t have to answer, but whatever the truth is, you can tell me.</i>"\n\n', false );
		}
		//(WUVED);
		else {
			EngineCore.outputText( '  I know our paths keep us apart most of the time... and multiple lovers are pretty far from uncommon, but I need to know – do you two have something going on?</i>"\n\n', false );
			EngineCore.outputText( 'Urta\'s looking at you nervously, clearly dreading your answer.  How do you respond?\n\n', false );
		}
	...
	Urta.prototype.comfortUrtaAfterTellinTruuf = function() {
		this.urtaSprite();
		EngineCore.outputText( '', true );
		EngineCore.outputText( 'You let Urta know that you understand.  After all, in a world like this monogamy is next to impossible.  There\'s no shame in what she did with Edryn or your relationship with Marble.  The two of you share a heartfelt hug and resolve to continue your relationship.', false );
		...
		if( ((CoC.getInstance().player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_CENTAUR && CoC.getInstance().player.totalCocks() > 0) ||
			CoC.getInstance().player.horseCocks() > 0 ||
			(CoC.getInstance().player.totalCocks() > 0 && CoC.getInstance().player.cor > 50)) &&
			CoC.getInstance().player.statusAffectv1( StatusAffects.Edryn ) >= 4 ) {
			EngineCore.outputText( '  Urta even suggests that the two of you take turns at Edryn sometime.', false );
		}
		//Slight love loss;
		this.urtaLove( -4 );
		EngineCore.doNext( SceneLib.camp.returnToCampUseOneHour );
	...
	Urta.prototype.toughenUpUrtaAfterTellingTruuf = function() {
		this.urtaSprite();
		EngineCore.outputText( '', true );
		EngineCore.outputText( 'You look at Urta and explain that she shouldn\'t take it too hard.  Both of you got into this scenario without expecting anything more from it.  You explain that though she is a good friend and GREAT in the sack, you don\'t see your relationship going to that level.  Urta seems ', false );
		if( CoC.getInstance().flags[ kFLAGS.URTA_PC_LOVE_COUNTER ] === 1 ) {
			EngineCore.outputText( 'heartbroken', false );
		} else {
			EngineCore.outputText( 'saddened', false );
		}
		EngineCore.outputText( ' by the news', false );
		if( CoC.getInstance().flags[ kFLAGS.URTA_PC_LOVE_COUNTER ] === 1 ) {
			EngineCore.outputText( ', and she has a defeated look in her eyes.', false );
			//(Luv set to 0 and blocked if in luv,;
			CoC.getInstance().flags[ kFLAGS.URTA_PC_AFFECTION_COUNTER ] = 0;
			CoC.getInstance().flags[ kFLAGS.URTA_PC_LOVE_COUNTER ] = -1;
		} else {
			EngineCore.outputText( ', but you spy a determined look in her eyes.', false );
	...
	Urta.prototype.walkoutOnUrtaAfterTellingTruff = function() {
		this.urtaSprite();
		EngineCore.outputText( '', true );
		EngineCore.outputText( 'You push away from the table and turn your back on Urta, enraged at her for cheating on you with Edryn, even though you cheated on her as well.  You ignore the hypocrisy in your actions and walk out, shattering Urta\'s heart in the process.  As you close the door of \'The Wet Bitch\' behind you, you see Urta, sobbing into a hanky while Edryn goes over to comfort her.  It figures.', false );
	...
	Urta.prototype.lietoUrtaAboutMarble = function() {
		this.urtaSprite();
		EngineCore.outputText( '', true );
		EngineCore.outputText( 'You let Urta know that the poor cow-girl needed a place to stay from the monsters that roam the wilds, and that your camp is the safest place.  In great detail you talk about sheltering her, but never taking advantage of her, and trying your best to do what\'s right.  Through the whole conversation Urta dabs at tears, and you assume them to be tears of relief.  The story suitably complete and air-tight, you finish and look at Urta.\n\n', false );
		EngineCore.outputText( '"<i>I misjudged you, ' + CoC.getInstance().player.short + '.  I didn\'t think you\'d lie to me, and you did.  The scouts talked to her', false );
		if( CoC.getInstance().flags[ kFLAGS.MARBLE_KIDS ] === 1 ) {
			EngineCore.outputText( ' and saw the kid', false );
		}
		if( CoC.getInstance().flags[ kFLAGS.MARBLE_NURSERY_CONSTRUCTION ] > 1 ) {
			EngineCore.outputText( ' and saw the kids', false );
		}
		EngineCore.outputText( '!  I know you two are an item, and I can\'t believe you didn\'t own up to it!</i>" she cries, "<i>You better believe I\'m fucking pissed at you.  I could\'ve accepted sharing you, but lying to me like this...I won\'t ever trust you again.  Just...just go – I might be able to overlook this but it hurts too much right now.</i>"\n\n', false );
		EngineCore.outputText( 'There\'s nothing to do but go.  You\'re not sure if Urta will ever let you see her again.', false );
		//(Urta pissed for 48 hours, then make-up scene.  Love forever blocked);
		CoC.getInstance().flags[ kFLAGS.URTA_PC_LOVE_COUNTER ] = -1;
		CoC.getInstance().flags[ kFLAGS.URTA_PC_AFFECTION_COUNTER ] = 0;
		CoC.getInstance().flags[ kFLAGS.URTA_ANGRY_AT_PC_COUNTDOWN ] = 48;
		EngineCore.doNext( SceneLib.camp.returnToCampUseOneHour );
	};
	//[TWU WUV TALK];
	Urta.prototype.UrtaTwuWuvOffer = function() {
		this.urtaSprite();
		EngineCore.outputText( '', true );
		EngineCore.outputText( 'Urta looks nervously at you, fidgeting and shifting her sizable addition under the table.  She starts talking meekly, but her voice rises in confidence and intensity as she goes, "<i>I\'ve been thinking ' + CoC.getInstance().player.short + ', we\'ve known each other for a while now.  When we\'re together, I don\'t have to worry about someone seeing something they shouldn\'t, or being made fun of, because it doesn\'t bother you and you don\'t care.  And what do I have to care about if you don\'t care about it?</i>"\n\n', false );
	...
	Urta.prototype.stayFuckbuddiesAfterShootingDown = function() {
		...
		CoC.getInstance().flags[ kFLAGS.URTA_TIME_SINCE_LAST_CAME ] = 0;
		EngineCore.doNext( SceneLib.camp.returnToCampUseOneHour );
	};
	//[no];
	...
	Urta.prototype.eatUrtaOutNomNomPussy = function() {
		...
		EngineCore.outputText( 'Finished at last, Urta\'s member rapidly deflates, the scent of her cum on your ' + Descriptors.hairDescript() + ' and her juice on your ' + CoC.getInstance().player.face() + ' lingers.  You slide up into the booth next to your blissed out lover.', false );
	...
	Urta.prototype.giveUrtaCumGlass = function() {
		...
		CoC.getInstance().flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ]++;
		EngineCore.outputText( '', true );
		EngineCore.outputText( 'You give her a kiss to distract her and push the glass into her hand.\n\n', false );
	...
	Urta.prototype.drinkUrtasCumGlass = function() {
		...
		this.urtaLove( 1 );
		EngineCore.outputText( '', true );
		EngineCore.outputText( 'You give her a good look at the full glass and tip it back, gulping down her thick goop, letting it run out of the corners of your mouth.  Urta looks on, dumbfounded while you guzzle her remaining seed from the cup.  You set it down and burp, leaning over to give her a wet kiss, letting her taste both her sexes on your tongue.  She breaks the kiss and says, "<i>Oh by Marae, I love you ' + CoC.getInstance().player.short + '.  Thank you so much.</i>"\n\n', false );
		EngineCore.outputText( 'You smile knowingly and leave, intent on cleaning up a little back at camp.', false );
	...
	Urta.prototype.blowUrtaUnderTheTableLuv = function() {
		...
		EngineCore.outputText( 'Deciding you\'ve got to get her off and make her spill her drink, you ', false );
		if( CoC.getInstance().player.cor < 50 ) {
			EngineCore.outputText( 'do your best to suppress your gag reflex and shove forwards, ramming her cock as deeply into your throat as you can.', false );
		} else {
			EngineCore.outputText( 'shove forwards, ramming her massive cock so far down your throat that you can feel it dribbling pre-cum into your belly.', false );
		}
		EngineCore.outputText( '  Urta grunts, doing her best to muffle the noise as it escapes her lips, but you can feel her twitch and thicken inside you, beginning to drip with proof of her complete and total submission to your mouth.\n\n', false );
		EngineCore.outputText( 'You can hear chuckles from the bar\'s other occupants as they realize what\'s going on, and you blush, ', false );
		if( CoC.getInstance().player.cor < 50 && CoC.getInstance().flags[ kFLAGS.PC_FETISH ] <= 0 ) {
			EngineCore.outputText( 'wishing there was a tablecloth to hide your activities from prying eyes', false );
		} else {
			EngineCore.outputText( 'wishing you had even more eyes watching your show', false );
		}
		EngineCore.outputText( '.  The distraction does little to divert you from your chosen task, and you close your eyes, tasting your lover\'s throbbing shaft as it fills your mouth.  With both hands stroking, you bob up and down, completely centering your world around the pulsating horse-prick as it fills your mouth.\n\n', false );
		EngineCore.outputText( 'Without warning, Urta\'s legs wrap around your back, squeezing with her soft fur as she vices them closed.  You\'re forced to take her even more deeply, until the folds of her sheath are bumping your nose and chin and filling your nostrils with her heady musk.  You can\'t even breathe with such a massive intruder blocking your throat, but you do your best to get her off, knowing it\'s the quickest way to end it.  You suck hard and squeeze her balls, gurgling noisily around the sweaty-shaft as you feel Urta give in.\n\n', false );
		EngineCore.outputText( 'A wave-like motion passes through your throat-obstruction, stretching you wider than ever before.  It passes, leaving behind a feeling of warm fullness.  You feel another working its way into your mouth, and realize Urta is cumming, filling your belly with direct injections of horsey fox-seed. You desperately need to breathe, but you\'re trapped on your lover\'s maleness, and with nothing else to do, you give her twitching balls a squeeze, feeling her body twitch in response as she dumps a particularly heavy batch of spunk into you.  Happy that you could at least control how hard she came, you begin blacking out, barely noticing as Urta\'s orgasmic contractions die off.\n\n', false );
		EngineCore.outputText( 'Thumping down on your back, you\'re blissfully aware of the sweet air filling your lungs.  In a moment of reflection, you realize you can taste the salty flavor of Urta\'s seed on your lips and mouth, and the retracting horse-cock is ', false );
		if( CoC.getInstance().player.biggestTitSize() >= 1 ) {
		...
		EngineCore.outputText( ' as it begins to soften.  You wipe as much of her leavings from your ' + CoC.getInstance().player.face() + ' as you can, licking the slippery glaze from your lips.  Before you get up, you make sure to give Urta\'s member a firm squeeze, scolding it for its forceful treatment of your mouth.\n\n', false );
	...
	Urta.prototype.rideUrtasCoochLoveyDovey = function() {
		...
		EngineCore.outputText( '', true );
		//(TOO SMALL);
		if( CoC.getInstance().player.vaginalCapacity() < 40 ) {
		...
		//(SHIT FITS);
		EngineCore.outputText( ImageManager.showImage( 'urta-home-female-fuck' ), false );
		EngineCore.outputText( 'Urta bursts into a wide smile at your suggestion and scooches over, making room for you on the bed.  ', false );
		if( CoC.getInstance().player.cockTotal() > 0 ) {
			EngineCore.outputText( 'She tosses you ', false );
			if( CoC.getInstance().player.cockTotal() === 1 ) {
				EngineCore.outputText( 'a latex sheath', false );
			} else {
				EngineCore.outputText( 'some latex sheaths', false );
			}
			EngineCore.outputText( ' to cover your ' + Descriptors.multiCockDescriptLight() + ' with, explaining that she doesn\'t want to have to mop up afterward, and you grudgingly agree.  ', false );
		}
		...
		EngineCore.outputText( 'Her cries are muted as you launch yourself over her, brutalizing her eager lips with a ferocious kiss.  You roughly tongue-fuck her as you adjust yourself, balancing yourself with one hand while your other guides her trembling shaft closer and closer to the entrance of your ' + Descriptors.vaginaDescript( 0 ) + '.  You sigh happily and rub the pre-filled bulb against your opening, ', false );
		if( CoC.getInstance().player.vaginas[ 0 ].vaginalWetness < AppearanceDefs.VAGINA_WETNESS_WET ) {
			EngineCore.outputText( 'growing moist', false );
		} else if( CoC.getInstance().player.vaginas[ 0 ].vaginalWetness < AppearanceDefs.VAGINA_WETNESS_DROOLING ) {
			EngineCore.outputText( 'growing wetter and wetter as you lubricate the tip of her condom', false );
		} else {
			EngineCore.outputText( 'soaking her condom-covered cock with your copious cunt-secretions', false );
		}
		EngineCore.outputText( '.  Urta\'s body goes limp under your sexual assault, totally caving in to the feelings you\'re stoking inside her.  The only thing she manages to keep up is the twisting of her hips as she tries to enhance the sensations radiating through her massive, flared prick.\n\n', false );
		EngineCore.outputText( 'You end the kiss by pulling back, sucking her lower lip into your mouth, and then releasing it with a pop.  Nearly crying with need, your horse-endowed lover begs, "<i>Ah, please don\'t stop!</i>"\n\n', false );
		EngineCore.outputText( 'Never one to disappoint, you press onwards and downwards, sliding deeper into the bed as you work the wide, flared tip against yourself.  It slowly stretches you wide and bows under your weight, though Urta doesn\'t voice a word of complaint or displeasure.  You swivel slightly to the side and it works its way inside you, one half of the tip sliding inside at a time.  The fox\'s mouth opens into an \'O\' of pleasure, letting her tongue escape as she begins panting from the pleasure.\n\n', false );
		EngineCore.outputText( 'Sinking down slowly and relishing the feeling of your passage being spread, you allow a low moan to escape your mouth.  Urta\'s hands lock onto her breasts and begin alternatively squeezing her soft mounds and fondling the black skin of her nipples.  The sensation of slow penetration is exquisite, growing within you while ever thicker horse-cock spears your womb.  You can feel the beat of Urta\'s heart racing through the shaft, swelling it slightly in time with each heartbeat.\n\n', false );
		EngineCore.outputText( 'You bottom-out, but immediately begin rocking your hips, working your lover\'s twitching rod deep inside your ' + Descriptors.vaginaDescript( 0 ) + '.  ', false );
		//(LOOSENESS MESSAGES GO HERE AS APPROPRIATE);
		CoC.getInstance().player.cuntChange( 60, true );
		EngineCore.outputText( 'Urta\'s hands grab you firmly by the ' + Descriptors.assDescript() + ', reminding you just how strong the lithe fox is.  She begins working with you to lift your body up and slam it back down against her, alternating the feelings of emptiness and fullness within you.  Each time the little balloon that\'s formed on her flare gets larger, squishing each time she bottoms out.  As the pair of you rut faster and faster, the limited space inside you squeezes the excess pre-cum from the sheath, causing it to drip over her balls into the sheets.\n\n', false );
		EngineCore.outputText( 'Bending over, you plant another kiss full on her lips, and your vulpine lover just can\'t restrain herself anymore.  She moans into your mouth, her body shivering, her balls tightening up until you feel them press against your ass.  Her hands grip you hard and pull you down with incredible force, locking you against her as you feel her member swelling inside you, submitting to your ' + Descriptors.vaginaDescript( 0 ) + '\'s swelteringly hot embrace.  A burst of tightness fills you, growing as Urta squirms, unloading her massive horse-pole\'s seed into the latex balloon she\'s stuffed inside you.\n\n', false );
		EngineCore.outputText( 'The filling seed balloon stretches tight inside of you, ', false );
		if( CoC.getInstance().player.vaginalCapacity() < 80 ) {
			EngineCore.outputText( 'until there is no more room for cum and it\'s squirting out around the base of her cock, soaking Urta and the bed with fragrant horse-cum.', false );
		} else {
			EngineCore.outputText( 'filling your body with a large bubble of hot horse-cum.', false );
		}
		EngineCore.outputText( '  Watching your lover submit to your body\'s pleasure, the sight pushes you over the deep edge with her, and your body begins milking the thick shaft inside you, clenching around it as you spasm happily.  ', false );
		if( CoC.getInstance().player.cockTotal() > 0 ) {
			if( CoC.getInstance().player.cockTotal() === 1 ) {
				EngineCore.outputText( 'Y', false );
			} else {
			...
			EngineCore.outputText( 'our ' + Descriptors.multiCockDescriptLight() + ' spurts, unloading waves of cream into the ', false );
			if( CoC.getInstance().player.cockTotal() === 1 ) {
				EngineCore.outputText( 'condom Urta asked you to wear around it.', false );
				if( CoC.getInstance().player.cumQ() >= 250 ) {
					if( CoC.getInstance().player.cumQ() < 500 ) {
						EngineCore.outputText( '  The balloon inflates slightly, drooping down as your maleness empties its reserves.  ', false );
					} else if( CoC.getInstance().player.cumQ() < 1000 ) {
						EngineCore.outputText( '  The balloon swells larger and larger as you pump it full, eventually wobbling around on Urta\'s belly as you finish filling it.', false );
					} else {
						EngineCore.outputText( '  The balloon swells larger and larger as you pump it full, wobbling obscenely on Urta\'s belly and slowly drooping down onto the sheets as the sheath reaches its maximum capacity.', false );
						if( CoC.getInstance().player.cumQ() > 1500 ) {
							EngineCore.outputText( '  Your potent seed pops the condom, splattering Urta with your seed as it explosively fails.', false );
						}
					}
				}
			} else {
				EngineCore.outputText( 'condoms Urta asked you to wear around them.', false );
				if( CoC.getInstance().player.cumQ() >= 250 ) {
					if( CoC.getInstance().player.cumQ() < 500 ) {
						EngineCore.outputText( '  The balloons inflate slightly, drooping down as your malenesses empty their reserves.  ', false );
					} else if( CoC.getInstance().player.cumQ() < 1000 ) {
						EngineCore.outputText( '  The balloons swell larger and larger as you pump them full, eventually wobbling around on Urta\'s belly as you finish filling them.', false );
					} else {
						EngineCore.outputText( '  The balloons swell larger and larger as you pump them full, wobbling obscenely on Urta\'s belly and slowly drooping down onto the sheets as the sheaths reach their maximum capacity.', false );
						if( CoC.getInstance().player.cumQ() > 1500 ) {
							EngineCore.outputText( '  Your potent seed pops the condoms, splattering Urta with your seed as they explosively fail.', false );
						}
					}
				}
			}
		...
		//if(CoC.getInstance().flags[kFLAGS.URTA_FERTILE] === 1) CoC.getInstance().player.knockUp(21,515,80);;
		CoC.getInstance().player.orgasm();
		if( CoC.getInstance().player.sens > 30 ) {
	...
	Urta.prototype.rideUrtaInButtAtHomeLove = function() {
		...
		CoC.getInstance().flags[ kFLAGS.URTA_TIME_SINCE_LAST_CAME ] = 2 + Utils.rand( 2 );
		EngineCore.outputText( '', true );
		EngineCore.outputText( ImageManager.showImage( 'urta-home-anal-urta' ), false );
		EngineCore.outputText( 'Urta raises an eyebrow at the suggestion and blushes, though her horse-like member thickens and stiffens noticeably.  You give it a gentle slap and tease, "<i>Don\'t act like this isn\'t what you wanted!</i>"\n\n', false );
		EngineCore.outputText( 'She squirms and smiles nervously, "<i>It feels too good, even though it\'s dirty!  Besides, your sweet ass is irresistible!</i>"\n\n', false );
		if( CoC.getInstance().player.totalCocks() > 0 ) {
			EngineCore.outputText( 'Even though you\'re going to be riding her, she insists you wear ', false );
			if( CoC.getInstance().player.totalCocks() === 1 ) {
				EngineCore.outputText( 'a condom', false );
			} else {
				EngineCore.outputText( 'some condoms', false );
			}
			EngineCore.outputText( ' to help control the mess.  You grudgingly accept, girding your hardening shaft', false );
			if( CoC.getInstance().player.totalCocks() > 1 ) {
				EngineCore.outputText( 's', false );
			}
			EngineCore.outputText( ' in tight latex.\n\n', false );
		}
		...
		EngineCore.outputText( 'While giving her a knowing smile, you make a conscious attempt to relax your body\'s surprised rear-entrance.  Almost immediately you feel her slip inside, and then you\'re sliding down, sinking slowly towards your partner an inch at a time.  The expression on Urta\'s face is priceless – flitting between ecstasy and confusion as she struggles to deal with being engulfed by your ', false );
		if( CoC.getInstance().player.analCapacity() < 60 ) {
			EngineCore.outputText( 'tight', false );
		} else {
			EngineCore.outputText( 'welcoming', false );
		}
		EngineCore.outputText( ', warm hole.  A groan escapes your mouth as you sink down, feeling fuller and fuller until her real sheath is resting just between your ass-cheeks.', false );
		CoC.getInstance().player.buttChange( 60, true, true, false );
		if( CoC.getInstance().player.totalCocks() > 0 ) {
			EngineCore.outputText( '  You can feel tiny dribbles force their way out of ', false );
			if( CoC.getInstance().player.totalCocks() > 1 ) {
				EngineCore.outputText( 'each of ', false );
			}
			EngineCore.outputText( 'your urethra', false );
			if( CoC.getInstance().player.totalCocks() > 1 ) {
				EngineCore.outputText( 's', false );
			}
			EngineCore.outputText( ' as your prostate is compressed.', false );
		}
		EngineCore.outputText( '\n\n', false );
		EngineCore.outputText( 'Urta recovers enough to meet your gaze and licks her lips hungrily.  Her hands release your ' + Descriptors.hipDescript() + ' and rump, sliding up your sides and in towards your ' + Descriptors.allBreastsDescript() + '.  You shiver as she circles your ' + Descriptors.nippleDescript( 0 ) + ', teasing the flesh around them while managing to avoid providing them with any stimulation.  She looks up at you expectantly with a patient look, as if waiting on something from you.\n\n', false );
		EngineCore.outputText( 'You shift your hips and are rewarded by a caress over your left ' + Descriptors.nippleDescript( 0 ) + ', but it stops almost immediately.  Catching on, you flex your ' + CoC.getInstance().player.legs() + ' and lift yourself up until her flare is the only thing connecting you before sliding back down the pulsating shaft, squeezing your muscles around it.  Urta\'s hands reward you by pinching and tugging on your ' + Descriptors.nippleDescript( 0 ) + 's, ', false );
		if( CoC.getInstance().player.hasFuckableNipples() ) {
			EngineCore.outputText( 'even slipping a finger inside their slippery depths, ', false );
		}
		EngineCore.outputText( 'spreading pleasure through the sensitive pleasure-buds.  Eager for more, you begin bouncing atop your vulpine lover, riding her as deeply as possible in return for a treasure-trove of sensation and lust.\n\n', false );
		EngineCore.outputText( 'Locked together by desire, love, and one swollen cock, you rut with her for what seems like forever.  You lose count of how many times your lover moans and screams in pleasure, and your own gasps of surprised enjoyment are too numerous to tabulate.  Urta\'s fur slowly becomes matted with sweat, and the salty scent of lovemaking fills the air.', false );
		if( CoC.getInstance().player.totalCocks() > 0 ) {
			if( CoC.getInstance().player.totalCocks() > 1 ) {
				EngineCore.outputText( '  Each of y', false );
			} else {
			...
			EngineCore.outputText( 'our ' + Descriptors.multiCockDescriptLight() + ' has filled its sheath with a golfball sized globule of pre-cum, and you can feel your body clenching and squeezing, your ', false );
			if( CoC.getInstance().player.balls > 0 ) {
				EngineCore.outputText( 'balls', false );
			} else {
				EngineCore.outputText( 'prostate', false );
			}
			EngineCore.outputText( ' aching to unload.', false );
		}
		if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( '  Neglected and unused, your ' + Descriptors.vaginaDescript( 0 ) + ' ', false );
			if( CoC.getInstance().player.vaginas[ 0 ].vaginalWetness < AppearanceDefs.VAGINA_WETNESS_DROOLING ) {
				EngineCore.outputText( 'dampens', false );
			} else {
				EngineCore.outputText( 'floods', false );
			}
			EngineCore.outputText( ' the fox underneath you with moisture, puffy and aching for the slightest touch.', false );
		}
		EngineCore.outputText( '\n\n', false );
		EngineCore.outputText( 'The lovemaking increases in tempo, slowly working its way to a feverish crescendo.  You\'re bouncing atop the fox wildly, and your lover cums hard.  As full as you felt with her inside you, it\'s nothing compared to being stuffed with her cock AND cum.  Each cum-filled contraction swells her member from base to flare, allowing you to feel it stretch your insides while it travels up your ' + Descriptors.assholeDescript() + '.  It dumps the load into the bubble of cum inside you, filling you with a creamy jizz balloon.\n\n', false );
		//(GENDERLESS ORGAZM);
		if( CoC.getInstance().player.gender === 0 ) {
			EngineCore.outputText( 'You cry out with pleasure, feeling the closest thing you can to an orgasm.  Your muscles clench around the invading rod, milking it while the rest of your body trembles.  The strength drains from your ' + CoC.getInstance().player.legs() + ' and arms and you twitch there, completely impaled on your lover\'s thick cock, happily and completely penetrated.\n\n', false );
		}
		//(EVERYBODY ELSE);
		else {
			EngineCore.outputText( 'You cry out with pleasure, feeling your body cum from the invading warmth in your ' + Descriptors.assholeDescript() + '.  Shivering as the strength drains from your arms and ' + CoC.getInstance().player.legs() + ', you let the intruding and still cumming cock support your body.', false );
			if( CoC.getInstance().player.hasVagina() ) {
				EngineCore.outputText( '  Your ' + Descriptors.vaginaDescript( 0 ) + ' clenches desperately, peaking without a single touch, but hungry for something to milk.', false );
				if( CoC.getInstance().player.vaginas[ 0 ].vaginalWetness > AppearanceDefs.VAGINA_WETNESS_WET && CoC.getInstance().player.vaginas[ 0 ].vaginalWetness < AppearanceDefs.VAGINA_WETNESS_SLAVERING ) {
					EngineCore.outputText( '  Fluids soak Urta\'s crotch with lust with feminine drool, though the fox barely seems to notice.', false );
				} else if( CoC.getInstance().player.vaginas[ 0 ].vaginalWetness === AppearanceDefs.VAGINA_WETNESS_SLAVERING ) {
					EngineCore.outputText( '  An explosion of feminine fluids spatters Urta\'s crotch and bed, completely soaking her groin in your need, but the distracted fox barely notices.', false );
				}
			}
			if( CoC.getInstance().player.totalCocks() > 0 ) {
				EngineCore.outputText( '  The warmth spreads deep inside you, firing off a series of contractions just inside your groin.  You can feel yourself about to cum, and look down just in time to see yourself squirting against your condom, deforming it from the pressure of the orgasm and slowly inflating it further.', false );
				if( CoC.getInstance().player.cumQ() >= 750 && CoC.getInstance().player.cumQ() < 1250 ) {
					EngineCore.outputText( '  By the time you\'re done the balloon is the size of a basketball and looking dangerously close to bursting.', false );
				} else if( CoC.getInstance().player.cumQ() >= 1250 ) {
					EngineCore.outputText( '  By the time you\'re done, the balloon is the size of a watermelon and wobbling dangerously, looking like it could fail at any time!', false );
					if( CoC.getInstance().player.cumQ() >= 1700 ) {
						EngineCore.outputText( '  You fill it fuller and fuller, until it\'s bigger than you thought possible.  All at once the material gives out, soaking Urta from the tits down in cum, while your ' + Descriptors.cockDescript( 0 ) + ' continues to pump even more onto her.  So much for not making a mess!', false );
					}
				}
			}
			EngineCore.outputText( '\n\n', false );
		...
		EngineCore.outputText( 'She comes out of her stupor and murmurs, "<i>Mmmm, that was nice,</i>" as she stretches.  With practiced motions she slides off her condom, ties it off, and tosses it into a waste-can half filled with the things.  ', false );
		if( CoC.getInstance().player.cumQ() >= 1700 ) {
			EngineCore.outputText( 'Urta sighs at the mess and laughs, "<i>I guess we got a mess anyways, huh?</i>"', false );
		} else {
			EngineCore.outputText( 'Urta sighs contentedly and laughs, "<i>I guess I don\'t have much of a mess to clean for once!</i>"', false );
		}
	...
	Urta.prototype.oralFiestyUberExplosionUrta = function() {
		...
		if( CoC.getInstance().player.gender === 2 || (CoC.getInstance().player.gender === 3 && Utils.rand( 2 ) === 0) ) {
			EngineCore.outputText( 'You smile and let Urta know you want to sixty-nine her.  She raises an eyebrow and teases, "<i>I hope you don\'t mind having a full belly and a sore throat!</i>" as you disrobe and climb atop her.  You turn about, straddling your legs across her face and exposing the hungry lips of your sex.  Smirking over your shoulder, you snatch her discarded dress from the floor and tie the shimmering fabric into an impromptu cock-ring.  The gray fox shudders with excitement and you watch, enraptured, as her mottled horse-cock grows larger and larger, even flaring wide at the tip.\n\n', false );
			EngineCore.outputText( 'The musky scent of Urta\'s forearm-sized horse-prick wafts into your nostrils as it wobbles just below your ' + CoC.getInstance().player.face() + ', visibly twitching and throbbing from all the extra blood.  She whines plaintively, nuzzling her moist nose against your ', false );
			if( CoC.getInstance().player.vaginas[ 0 ].vaginalWetness < AppearanceDefs.VAGINA_WETNESS_DROOLING ) {
				EngineCore.outputText( 'equally wet', false );
			} else {
				EngineCore.outputText( 'sopping wet', false );
			}
			EngineCore.outputText( ' cunt.  Shivering from the air on your bare skin and the unexpected touches on your nethers, you open wide, extend your tongue, and give Urta\'s flare a long, slow lick.  You pause at her urethra, slip ever-so-slightly inside, and then pull out as you slather the flare in spit.\n\n', false );
			EngineCore.outputText( 'Urta eagerly gets to work, stabbing her tongue through your folds and into your ' + Descriptors.vaginaDescript( 0 ) + '.  Her efforts bring her muzzle flush against you as she pauses to slurp your pussy-juices over your ' + Descriptors.clitDescript() + '.  Her lips feel like liquid heat as they wrap around the incredibly sensitive bud, suckling it gently in between excursions into your passage.  Urta\'s slender fingers grab hold of your ' + Descriptors.buttDescript() + ' to support her as she tongue-fucks your ' + Descriptors.vaginaDescript( 0 ) + ' with abandon.\n\n', false );
			...
			if( CoC.getInstance().player.cor > 50 ) {
				EngineCore.outputText( 'push forwards, feeling her shaft arch as it slides into your throat.  The medial ring slips past your lips and you can feel your throat bulging as you mouth-fuck the swollen horse-cock deep into your gut.  Warmth drips into your belly as the hermaphroditic fox\'s member closes on its sexual peak.\n\n', false );
			} else {
				EngineCore.outputText( 'grab hold of her cock and begin jerking her off into your mouth.  An immediate and copious gush of pre drips onto your tongue, showing you just how much the hermaphroditic fox loves the feeling.\n\n', false );
			}

			EngineCore.outputText( 'Not far from cumming yourself, you grind your ' + Descriptors.hipDescript() + ' on Urta\'s face, smearing her fur with your fragrant pussy-juices.  Her own moans of pleasure turn into noisy wet blubbering that vibrates through your ' + Descriptors.vaginaDescript( 0 ) + ', getting you off on the spot.  You squeal around the cock in your ', false );
			if( CoC.getInstance().player.cor <= 50 ) {
				EngineCore.outputText( 'mouth', false );
			} else {
				EngineCore.outputText( 'throat', false );
			}
			EngineCore.outputText( ', clamping your ' + CoC.getInstance().player.legs() + ' onto Urta\'s head as you orgasm.  ', false );
			if( CoC.getInstance().player.vaginas[ 0 ].vaginalWetness > AppearanceDefs.VAGINA_WETNESS_DROOLING ) {
				EngineCore.outputText( 'Fluids rush from your folds, splattering her face with sex fluid.  The fox opens wide, gulping it down like someone in a desert who has just found an oasis.', false );
			} else {
				EngineCore.outputText( 'Wetness slowly coats her muzzle as your canal squeezes and wriggles with involuntary pleasure-spasms.  Urta never lets up, sucking and licking your ' + Descriptors.clitDescript() + ' in a rhythm that matches your own orgasmic contractions.', false );
			}
			if( CoC.getInstance().player.totalCocks() > 0 ) {
				EngineCore.outputText( '  ', false );
				if( CoC.getInstance().player.cockTotal() > 1 ) {
				...
				EngineCore.outputText( 'our ' + Descriptors.multiCockDescriptLight() + ' drools lamely, ', false );
				if( CoC.getInstance().player.cumQ() < 50 ) {
					EngineCore.outputText( 'sputtering spunk onto Urta\'s neck.', false );
				} else if( CoC.getInstance().player.cumQ() < 250 ) {
					EngineCore.outputText( 'soaking Urta\'s neck and chest with your spooge.', false );
				} else if( CoC.getInstance().player.cumQ() < 700 ) {
					EngineCore.outputText( 'showering Urta\'s neck and body with spooge until her breasts are painted white and the bed has a small puddle soaking into it.', false );
				} else if( CoC.getInstance().player.cumQ() < 1500 ) {
					EngineCore.outputText( 'soaking Urta with a torrent of seed that drenches her from the neck down and totally soaks the bed.', false );
				} else {
					EngineCore.outputText( 'soaking Urta with a torrent of seed that drenches her body and floods the bed, drips onto the floor, and slowly spreads into a puddle.', false );
					if( CoC.getInstance().player.cumQ() > 10000 ) {
						EngineCore.outputText( '  The puddle thickens as you cum until it\'s nearly a foot deep.', false );
					}
				}
			}
			EngineCore.outputText( '\n\n', false );
			EngineCore.outputText( 'Too drunk with pleasure to register the telltale thickening of Urta\'s cock, you keep licking and sucking while you cum, drinking a steady stream of pre-cum.  Suddenly, your improvised cock-ring snaps and the straps fail from the massive pressure.  A torrent of jism ', false );
			if( CoC.getInstance().player.cor > 50 ) {
				EngineCore.outputText( 'pours down your throat, bulging it into an obscene imprint of the horse-cock that\'s lodged inside.  Your eyes cross from pleasure and you slump down a bit, shoving your nose into Urta\'s sheath and allowing her to dump her sperm directly into your belly.  Gurgling noisily, your belly slowly expands outwards by about two inches, making you feel bloated and full.', false );
			} else {
				EngineCore.outputText( 'erupts into your mouth, instantly filling it with thick fox-cream.  You convulsively swallow in an attempt to keep up with it, but are quickly overwhelmed by the backed up semen-flood.  It drips from your mouth and forces open your throat, flowing directly into your belly.  Some even begins dripping out from your nose as you sway with a mixture of discomfort and pleasure atop your partner.  A few trembling squirts later you feel and look a little bloated, and your ' + CoC.getInstance().player.face() + ' is thoroughly covered in foxy-horse-jizz.', false );
			}
			EngineCore.outputText( '\n\n', false );
			...
			EngineCore.outputText( 'The fox giggles and returns to cuddling and licking you.  You rest with a happy smile plastered on your ' + CoC.getInstance().player.face() + ' from the experience.  After all the snuggling, you both rise and get dressed.  Urta finds a new dress in the closet and with a long goodbye kiss, you go your separate ways.\n\n', false );
			EngineCore.doNext( SceneLib.camp.returnToCampUseOneHour );
			CoC.getInstance().player.orgasm();
			EngineCore.dynStats( 'sen', -1 );
			return;
		...
		//(DUDEZILLAZ);
		EngineCore.outputText( 'Urta sits up and practically screams, "<i>Really,</i>" and squeals excitedly.  She claps and lies back as you climb into the bed, stopping only to yank off her cock-sheath and toss it into a nearby waste-bin.  You swing your ' + CoC.getInstance().player.legs() + ' over her and get into position, aligning ', false );
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( 'your ' + Descriptors.ballsDescriptLight() + ' and ', false );
		}
		if( CoC.getInstance().player.totalCocks() > 1 ) {
			EngineCore.outputText( 'one of your ', false );
		} else {
			EngineCore.outputText( 'your ', false );
		}
		EngineCore.outputText( Descriptors.multiCockDescriptLight() + ' with Urta\'s muzzle.  Her rather sizable \'addition\' wobbles underneath you, blocking your vision as it wobbles back and forth under you.\n\n', false );
		EngineCore.outputText( 'Your foxy lover opens wide and slips her smooth, wide tongue all over your ', false );
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( Descriptors.ballsDescript() + '.  Slithering over and around them, she bathes your sack in her spit before moving up to your ' + Descriptors.cockDescript( x ) + '.', false );
		} else {
			EngineCore.outputText( Descriptors.cockDescript( x ) + '.', false );
		}
		EngineCore.outputText( '  Her tongue curves and slurps at your shaft, swelteringly warm, wet, and tantalizingly slippery.  She tastes you from ', false );
		if( CoC.getInstance().player.hasSheath() ) {
			EngineCore.outputText( 'sheath', false );
		} else {
			EngineCore.outputText( 'base', false );
		}
		EngineCore.outputText( ' to ' + CoC.getInstance().player.cockHead( x ) + ', stopping to suckle the sensitive end briefly before breaking away for another long lick along the underside.  Her hands stroke along the sensitive ' + CoC.getInstance().player.skinDesc + ' of your inner thighs, making your hips shiver and nearly pulling your ' + Descriptors.cockDescript( x ) + ' away from your lover\'s hungry mouth.\n\n', false );
		EngineCore.outputText( 'Whilst the fox is busy with your genitals, you begin to perform your duty, reciprocating the pleasure in turn.  You stretch your mouth wide, opening as far as you can to accommodate the mammoth beast before you.  It\'s difficult, but you manage to stretch wide enough to take her flare in your mouth.  It tastes vaguely salty like sweat, and in Urta\'s excited state it\'s easy to feel her flare widen and retract with each beat of her heart.  A dollop of pre-cum deposits itself on your tongue as the horny fox\'s nymphomaniac body reacts to you.  It tastes sweet, and you swallow as you work her deeper into your oral cavity.\n\n', false );
		EngineCore.outputText( 'Muffled, excited moans reach your ears as you work on your lover\'s pole.  The fox is clearly enjoying herself, and her sounds of pleasure cut off as she rams your ' + Descriptors.cockDescript( x ) + ' deep into her throat.  She gurgles noisily while her tongue slides as far down your shaft as possible, licking at your ', false );
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( Descriptors.ballsDescriptLight(), false );
		} else if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( Descriptors.vaginaDescript( 0 ), false );
		...
		EngineCore.outputText( ' while her spittle froths around her lips.  She manages to start humming deep within her throat, vibrating your ' + Descriptors.cockDescript( x ) + '.  The obstruction in your mouth stifles your moan, but Urta hears the startled gurgle and slides away, then plunges back down, humming the entire time in between gasps for air.\n\n', false );
		EngineCore.outputText( 'Not to be outdone, you pull back for a quick gasp of air before ramming her back inside your mouth.  You push forwards, taking her to the back of your throat', false );
		if( CoC.getInstance().player.cor < 60 ) {
			EngineCore.outputText( ' until your gag reflex forces you back.  Not being able to deep-throat her, you shift your weight onto your right arm and grab her just below her medial ring with your left.  You start vigorously jacking her off, running your tongue around her flare and over her sensitive tip, even darting it into her urethra to get at her sweet-tasting pre-cum.', false );
		} else {
			EngineCore.outputText( '.  You easily suppress your gag reflex and push forwards, feeling her cock arch slightly in the back of your mouth as it curves into your throat.  You extend your tongue as far past your lips as you can, licking down the shaft as you cram her further and further into your gullet.  Though you can\'t taste it, you can feel her urethra sending slow pulses of pre-cum inside of you, depositing it directly into your belly as you stuff your nose into her balls and tongue the skin around her sheath.', false );
		}
		EngineCore.outputText( '\n\n', false );
		EngineCore.outputText( 'If someone walked in on the both of you now they\'d see a breathless fox and you, each with the other\'s cock rammed into their face.  They\'d see each of you blushing bright and trembling, on the verge of orgasm and barely holding on as the other does their best to bring about an orgasm.  Wet slurps and panicked gasps fill the air while bodies twist and squirm together on the fur-covered bed.\n\n', false );
		EngineCore.outputText( 'You cum first, a victim of Urta\'s talented tongue and humming technique.  You\'d cry out with pleasure were you not so thoroughly stuffed, and the fox slams her face into your groin after the first blast, pushing you into her throat and noisily beginning to gulp down your spooge.  ', false );
		if( CoC.getInstance().player.cumQ() >= 500 ) {
			EngineCore.outputText( 'She gurgles as she struggles to keep up with the flow, actually letting a bubbly white froth start to leak down the sides of her mouth.  ', false );
			if( CoC.getInstance().player.cumQ() >= 1000 ) {
				EngineCore.outputText( 'It begins dripping from her nostrils as your explosive load overwhelms her ability to swallow spunk.  ', false );
			}
		}
		if( CoC.getInstance().player.cumQ() >= 750 && CoC.getInstance().player.cumQ() < 1500 ) {
			EngineCore.outputText( 'Her belly rounds out a little from all the cum as your orgasm begins to wind down.  ', false );
		}
		if( CoC.getInstance().player.cumQ() >= 1500 ) {
			EngineCore.outputText( 'Her belly rounds out a little before she pulls back and you spurt all over her, soaking her face and head with more spooge than she could ever handle.  ', false );
		}
		EngineCore.outputText( 'Sated, you sigh, though Urta pushes back onto you and intently licks every ounce of jism from your body.\n\n', false );
		EngineCore.outputText( 'With only a few moments to recover from your own orgasm, you feel the fox stiffening, her apple-sized balls pulling up tightly against her body as she starts to orgasm.  A cock-bulging load of thick fox-cream forces your mouth wider before exploding ', false );
		if( CoC.getInstance().player.cor < 60 ) {
			EngineCore.outputText( 'in your mouth, forcing you to compulsively swallow if you don\'t want to drown in it.', false );
		} else {
			EngineCore.outputText( 'down your throat, directly injecting it into your belly.', false );
		}
		EngineCore.outputText( '  More and more of the salty cream is forced into you, leaving you feeling full and bloated, with a salty aftertaste in your mouth.  The fox-girl\'s horse-cock does eventually stop cumming, though rather than stopping all at once it slows to a trickle over the course of a minute or two, and you slurp it down until it softens in your mouth.\n\n', false );
	...
	Urta.prototype.urtaHomeLuvLuvinsMenu = function() {
		...
		/*if(CoC.getInstance().player.gender === 3) EngineCore.choices('Ride Vag',this.rideUrtasCoochLoveyDovey,'Ride Anal',this.rideUrtaInButtAtHomeLove,'69',this.oralFiestyUberExplosionUrta,'Vag Fuck',this.urtasCoochiNeedsFuckedLove,'No Condoms',this.condomlessUrtaInHouseSmex);
		 ...
		 if(CoC.getInstance().player.gender === 0) EngineCore.choices('Ride Vag',0,'Ride Anal',this.rideUrtaInButtAtHomeLove,'69',0,'',0,'',0);*/
		...
		if( CoC.getInstance().flags[ kFLAGS.URTA_PETPLAY_DONE ] >= 0 && CoC.getInstance().player.gender > 0 ) {
	...
	Urta.prototype.urtasCoochiNeedsFuckedLove = function() {
		...
		EngineCore.outputText( '.  You grab her ass and slam in a final time, reaching your own peak.\n\n', false );
		//[DAWG];
		if( CoC.getInstance().player.hasKnot( x ) ) {
			EngineCore.outputText( 'Your knot flares wide, locking itself inside her convulsing cunt as your seed begins pumping into her womb.  Urta cries, "<i>oooooooOOOOHHhhh YESSSS... You\'ve plugged my cunt with your doggie dick!  It\'s squirting and pumping into my wooooomb...</i>"\n\n', false );
			EngineCore.outputText( 'You rock back and forth, unable to pull out yet stimulating the sensitive bulb of your knot.  Your ' + Descriptors.cockDescript( x ) + ' dumps its gooey cargo directly into your lover\'s stopped-up sex with barely audible squishing noises.  ', false );
			if( CoC.getInstance().player.cumQ() < 250 ) {
			} else if( CoC.getInstance().player.cumQ() < 700 ) {
				EngineCore.outputText( 'Her belly puffs up slightly by the time you finish, thanks to your body\'s copious cum production.  ', false );
			} else if( CoC.getInstance().player.cumQ() < 1000 ) {
				EngineCore.outputText( 'Her belly swells larger and larger as you continue to pump your seemingly endless seed into her.  Urta groans, looking a few months pregnant by the time your overproductive ' + Appearance.cockNoun( CockTypesEnum.HUMAN ) + ' stops spewing inside her.  ', false );
			} else {
				EngineCore.outputText( 'Her belly swells larger and larger, bloating as you blast thick torrents of seed inside her, filling her completely thanks to the obstruction of your knot.  ', false );
				if( CoC.getInstance().player.cumQ() < 3000 ) {
					EngineCore.outputText( 'By the time you finish, she looks VERY pregnant and is moaning with a mixture of pleasure and discomfort.  ', false );
				} else {
					EngineCore.outputText( 'Your seemingly never-ending orgasm pumps her up until she looks on the verge of giving birth, and you just keep going.  The pressure builds and Urta moans in discomfort and pleasure.  Eventually the pressure is too much, and your cum begins spraying out around your knot, soaking the pair of you as you both continue to cum hard.  ', false );
					if( CoC.getInstance().player.cumQ() > 5000 ) {
						EngineCore.outputText( 'Not content with such an easy release, your ' + Descriptors.cockDescript( x ) + ' unloads the last of its cream until the bed and floor are both covered under inches of your canine cum.  ', false );
					}
				}
			}
			EngineCore.outputText( 'You finally feel your ' + Appearance.cockNoun( CockTypesEnum.HUMAN ) + '\'s knot shrinking and pull free with a loud and messy pop.  ', false );
			if( CoC.getInstance().player.cumQ() >= 2000 ) {
				EngineCore.outputText( 'Spunk rushes out in a torrent, splattering everywhere.  ', false );
			}
			EngineCore.outputText( 'Urta\'s glazed pussy-lips clench reflexively at their sudden emptiness and you hear a very satisfied sigh from the far end of the bed.\n\n', false );
		}
		//[HORSE];
		else if( CoC.getInstance().player.cocks[ x ].cockType === CockTypesEnum.HORSE ) {
			EngineCore.outputText( 'You feel your flare filling with blood as it ', false );
			if( CoC.getInstance().player.cocks[ x ].cockLength >= 10 ) {
				EngineCore.outputText( 'prods against Urta\'s cervix.', false );
			} else {
				EngineCore.outputText( 'seats itself deep inside Urta\'s passage.', false );
			}
			EngineCore.outputText( '  Your crotch muscles clench in an unconscious rhythm as orgasm wracks your body.  You begin spurting hard inside her, splattering it into her cervix, even allowing it to flow into her womb.  Urta cries, "<i>ooooOOOHHHH YESSS... is this what you feel when I cum inside you?  YesYESYES- Ohhh I can feel the flare stroking and flexing inside me...</i>"\n\n', false );
			EngineCore.outputText( 'You rock back forth, unable to resist dragging the sensitive, flared tip and textured ring of your cock all along her pussy-walls, squirting seed with reckless abandon into her hot, cum-slicked cunt.  Wet squishing noises fill the apartment as the two of you rock the bed in twinned orgasms.  ', false );
			if( CoC.getInstance().player.cumQ() < 250 ) {
			} else if( CoC.getInstance().player.cumQ() < 700 ) {
				EngineCore.outputText( 'Her belly slowly rises as more and more cum slides into her womb, filling it.  Runners of the stuff leak down her thighs as it escapes around your bestial cock.  ', false );
			} else if( CoC.getInstance().player.cumQ() < 1000 ) {
				EngineCore.outputText( 'Her belly swells larger and larger as you continue to pump your seemingly endless seed into her.  Urta groans, looking a few months pregnant by the time your over-productive ' + Appearance.cockNoun( CockTypesEnum.HUMAN ) + ' has finished spewing your seed inside her.  ', false );
			} else {
				EngineCore.outputText( 'Her belly swells larger and larger, bloating out as you blast thick torrents of seed inside her, stuffing her completely.  Every squirt of the stuff is met with a leaking back-blast that soaks her thighs and bed in your jism.  ', false );
				if( CoC.getInstance().player.cumQ() < 3000 ) {
					EngineCore.outputText( 'By the time you\'ve finished, she looks heavily pregnant and cum soaks the bed.  ', false );
				} else {
					EngineCore.outputText( 'Your seemingly endless orgasm pumps more and more into her until she looks ready to give birth, and  you just keep going.  She moans in pleasure and discomfort as each blast of your horse-cum forces thick runners to squirt from between her thighs.  ', false );
				}
				if( CoC.getInstance().player.cumQ() > 5000 ) {
					EngineCore.outputText( 'Not content with such an easy release, your ' + Descriptors.cockDescript( x ) + ' unloads the last of its cream until the bed and floor are covered in several inches of thick equine cum.  ', false );
				}
			}
			EngineCore.outputText( 'You finally feel your flare returning to normal and pull out with a prolonged \'sluuuuurp\'.  ', false );
			if( CoC.getInstance().player.cumQ() >= 2000 ) {
				EngineCore.outputText( 'Spunk rushes out in a torrent, splattering everywhere.  ', false );
			}
			EngineCore.outputText( 'Urta\'s glazed pussy-lips clench reflexively at their sudden emptiness and you hear a very satisfied sigh from the far end of the bed.\n\n', false );
		...
		//[Everybody Else];
			...
			EngineCore.outputText( 'Your ' + Descriptors.cockDescript( 0 ) + ' clenches, filling her convulsing cunt as your seed begins pumping into her womb.  Urta cries, "<i>oooooooOOOOHHhhh YESSSS... It\'s so warm and wet.  Yes, lover!  Keep goo-oh-OH-innnngg!!</i>"\n\n', false );
			EngineCore.outputText( 'You rock back and forth, sliding your shaft through her spasming muscles, reveling and panting in orgiastic bliss.  Your ' + Descriptors.cockDescript( 0 ) + ' dumps its gooey cargo deep into your lover\'s canal with barely audible squishing noises.  ', false );
			if( CoC.getInstance().player.cumQ() < 250 ) {
			} else if( CoC.getInstance().player.cumQ() < 700 ) {
				EngineCore.outputText( 'Her belly puffs up slightly by the time you finish, thanks to your body\'s copious cum production.  ', false );
			} else if( CoC.getInstance().player.cumQ() < 1000 ) {
				...
				EngineCore.outputText( 'Her belly swells larger and larger, bloating as you blast thick torrents of seed inside her, filling her completely while leftover jism squirts out around your ' + Descriptors.cockDescript( 0 ) + '.  ', false );
				if( CoC.getInstance().player.cumQ() < 3000 ) {
					EngineCore.outputText( 'By the time you finish, she looks VERY pregnant and is moaning with a mixture of pleasure and discomfort.  ', false );
				} else {
					EngineCore.outputText( 'Your seemingly never-ending orgasm pumps her up until she looks on the verge of giving birth, and you just keep going.  Jism sprays out from her dripping cunt as you release a torrent of semen, splattering you, her, and the bed.  ', false );
				}
				if( CoC.getInstance().player.cumQ() > 5000 ) {
					EngineCore.outputText( 'Not content with such an easy release, your ' + Descriptors.cockDescript( 0 ) + ' unloads the last of its cream until the bed and floor are both covered under inches of your gooey cum.  ', false );
				}
			}
			EngineCore.outputText( 'You finally feel your ' + Appearance.cockNoun( CockTypesEnum.HUMAN ) + ' deflating, and you pull free with a messy slurp.  ', false );
			if( CoC.getInstance().player.cumQ() >= 2000 ) {
				EngineCore.outputText( 'Spunk rushes out in a torrent, splattering everywhere.  ', false );
			}
			EngineCore.outputText( 'Urta\'s glazed pussy-lips clench reflexively at their sudden emptiness and you hear a very satisfied sigh from the far end of the bed.\n\n', false );
		...
		//[RESUME];
		EngineCore.outputText( 'Urta calms down at last, though her member is still throbbing weakly as she peels her condom off.  The bloated cum-bubble at the tip is the size of a basketball, and she struggles to get it tied off and disposed of.  The fox tosses it into a waste-basket already full of the things and gives you a toothy smile, "<i>I think somebody\'s ' + Appearance.cockNoun( CockTypesEnum.HUMAN ) + ' loves my pussy almost as much as I love you!</i>"\n\n', false );
		EngineCore.outputText( 'She pulls you into her arms and gives you a long, wet kiss before breaking away to clean up.  Urta blows you a kiss and says, "<i>Thanks again, love!</i>"\n\n', false );
		EngineCore.outputText( 'You and Urta go your separate ways after that, both stinking of sex and with a sense of deep satisfaction.', false );
		this.knockUpUrtaChance();
		CoC.getInstance().player.orgasm();
		EngineCore.dynStats( 'sen', -1 );
		EngineCore.doNext( SceneLib.camp.returnToCampUseOneHour );
	...
	Urta.prototype.scyllaAndUrtaSittingInATree = function() {
		...
		CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00143 ]++;
		...
		if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00147 ] === 0 && CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00145 ] > 0 ) {
			...
			// Butans;
			if( CoC.getInstance().player.cockThatFits( this.urtaCapacity() ) !== -1 && CoC.getInstance().player.lowerBody !== AppearanceDefs.LOWER_BODY_TYPE_CENTAUR ) {
			...
			if( !this.urtaLove() ) {
				EngineCore.outputText( 'Urta', false );
			} else {
				EngineCore.outputText( 'your love', false );
			}
			EngineCore.outputText( '\'s voice.  The first few doors are silent and the next one sounds to have a spirited game of strip-poker going on inside it.  Judging by the grunts and occasional mewling moan of pleasure, some poor girl has already lost.  One of the barmaids gives you a warning glance and you keep moving, sure to avoid the lewd game of chance as you hunt for the stray, well-endowed fox.\n\n', false );
			EngineCore.outputText( 'At first, the following entryway is silent, though you can see light underneath.  You\'re just about to move on from the portal when you hear Urta\'s voice give out a strained grunt.  Did some ', false );
			if( CoC.getInstance().player.cor < 40 ) {
				EngineCore.outputText( 'corrupted ruffians', false );
			} else {
				EngineCore.outputText( 'demon agents', false );
			}
			EngineCore.outputText( ' somehow get the drop on her?  She whines, high-pitched and plaintive, almost as if she was wounded, and you cannot wait a moment later.  You lunge to the door, twist the handle, and find it locked.  Without a second thought, you throw your weight into the flimsy wood paneling and crash through to the last thing you expected to see!\n\n', false );
			EngineCore.outputText( 'Urta\'s sitting down on a crate with her legs akimbo and her torso leaning drunkenly in your direction.  Her tongue is hanging out and her eyes are lidded from alcohol as she notices you and gives another high-pitched whine, not of pain, but pleasure.  The source of her \'distress\' is a black-robed figure with curly black locks of hair that hang around her head like a halo and thick, glossy red lips.  Those same lips are currently wrapped firmly around Urta\'s twitching, lust-swollen horse-cock just above her sheath.  You can clearly see the slight bulge of Urta\'s medial ring highlighted through the corrupted nun\'s neck as she bobs up and down, so absorbed in feeding her addiction that she failed to notice you bursting through the door.\n\n', false );
			EngineCore.outputText( 'The fox is ', false );
			if( this.urtaLove() ) {
				EngineCore.outputText( 'weakly protesting', false );
			} else if( CoC.getInstance().flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] >= 11 ) {
				EngineCore.outputText( 'giving you a lewd look', false );
			} else {
				EngineCore.outputText( 'blushing hard', false );
			}
			EngineCore.outputText( ' as she cradles Scylla\'s head in her hands and pushes down.  She slurs, "<i>', false );
			if( this.urtaLove() ) {
				EngineCore.outputText( 'I\'m... ahhhh... s-sorry love, but we both know she\'s been suuuu - oh yeah, keep licking - sucking you off before, and the hungry slut just couldn\'t take her eyes off my cock.  Why didn\'t you tell me ho - OH YES!  RIGHT THERE! - h-how good she was?', false );
			} else if( CoC.getInstance().flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] >= 11 ) {
				EngineCore.outputText( 'Ohhh, you showed up a little late... mmmhmm yes, now lick... Oh, why didn\'t you tell me ho - OH YES!  RIGHT THERE! - h-how good of a cock-sucker this lewd-bodied bitch was?', false );
			} else {
				EngineCore.outputText( 'H-hi ' + CoC.getInstance().player.short + '... I didn\'t think I\'d get to see you to- Oh yeah, like that!  Mmmm... OH!  I guess since we\'ve been intimate before you can watch...', false );
			}
			EngineCore.outputText( '</i>"\n\n', false );
	...
	Urta.prototype.urtaAndScyllaBoningLikeBitchesSober = function() {
		...
		if( EngineCore.silly() ) {
			EngineCore.outputText( 'For some reason, it makes you think of mythical balloonicorns.  ' );
		}
		...
		if( !this.urtaLove() ) {
			EngineCore.outputText( 'fuck-buddy' );
		} else {
			EngineCore.outputText( 'lover' );
		}
		EngineCore.outputText( '\'s lusty slip-up and skittish responses, pinching her booty for good measure while you decide just how to get involved in the potential <i>ménage à trois</i>.\n\nWhat do you do?' );
		EngineCore.dynStats( 'lus', 10 + CoC.getInstance().player.lib / 5, 'resisted', false );
		//Get worshipped requires gigantic dong;
		var worship = null;
		var fuck = null;
		if( CoC.getInstance().player.cockThatFits( this.urtaCapacity() ) >= 0 ) {
			fuck = this.fuckUrtaWhileScyllaSucksSober;
			EngineCore.outputText( '  You could <b>fuck the fox</b> while she gets her oral pleasure.' );
		}
		if( CoC.getInstance().player.biggestCockArea() > this.urtaCapacity() ) {
			worship = this.dockWithUrtaUnderScyllasSoberSupervision;
			EngineCore.outputText( '  You could whip out your gigantic cock and let the blowjob queen <b>worship</b> a REAL dick.' );
		}
	...
	Urta.prototype.fuckUrtaWhileScyllaSucksSober = function() {
		EngineCore.clearOutput();
		var x = CoC.getInstance().player.cockThatFits( this.urtaCapacity() );
		EngineCore.outputText( 'You peel out of your straining equipment to free your ' + Descriptors.multiCockDescriptLight() );
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( ' and [balls]' );
		}
		EngineCore.outputText( ', hauling out your sweaty, engorged meat.  Scylla, absorbed as she is with cum-thirst, doesn\'t seem to notice, though her nose twitches as the scent of your musky aroma mingles with the sex-stained air.  You gently squeeze behind the fox, folding her bushy, soft tail to the side and giving you something firm to support yourself when you let her flop back into you.  Her smooth fur feels wonderful on your [chest] and she squirms and gyrates against Scylla\'s sucking mouth.  Fairly dripping with moisture, the fox\'s black-lipped sex grinds on you, resting atop [oneCock], and your length is soon soaked with her heady, animalistic fem-spunk.' );
		...
		if( CoC.getInstance().player.cockArea( x ) >= 40 ) {
			EngineCore.outputText( ', clearly bulging her stomach with your sizeable cock' );
		}
		EngineCore.outputText( ', sliding up into the slick, welcoming passage as if you always belonged.  Rivulets of liquid desire trickle from the stretched lips to run down ' );
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( 'the front of your [sack]' );
		} else {
			EngineCore.outputText( 'your [legs]' );
		}
		EngineCore.outputText( '.  The gray-furred femme reaches back in an awkward hug, trying to hold you against her, speared together in the most intimate of embraces.' );
		...
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( 'r [balls]' );
		}
		EngineCore.outputText( ' give the warm snatch a dose of virile medicine.  Urta, perhaps stirred from her own orgasm by the jets of hot fluid flooding her nethers, reaches back to squeeze your butt and whimpers, "<i>Give it to me.</i>"' );
		...
		if( CoC.getInstance().player.cockTotal() > 1 ) {
			EngineCore.outputText( '  The unbound portion of your package bobs in the air before coming to rest in Urta\'s ass-crack.  A second later, the vulpine butt is coated with sticky white icing.' );
		}
		if( CoC.getInstance().player.lactationQ() > 50 ) {
			EngineCore.outputText( '  Your sensitive nipples let down, releasing their own heady flows to compliment your climactic pleasure, spraying into the gray fur relentlessly.' );
		}
		EngineCore.outputText( '  Throughout it all, your ' + Descriptors.cockDescript( x ) + ' continues to do its level best to fill the fox.  Fireworks of uncontrollable pleasure go off one after the other, alternatively numbing and locking your muscles as you release all of your pent-up lusts, desires, and needs into the welcoming cunt.' );
		if( CoC.getInstance().player.cumQ() >= 500 ) {
			EngineCore.outputText( '  Urta\'s belly bloats from the thick deposit' );
			if( CoC.getInstance().player.cumQ() >= 2000 ) {
				EngineCore.outputText( ', rounding more and more, a taut cum-tank that jiggles with your thick seed.' );
			} else if( CoC.getInstance().player.cumQ() >= 1200 ) {
				EngineCore.outputText( ', rounding out into a swollen cum-bank that jiggles under the weight of your seed.' );
			} else if( CoC.getInstance().player.cumQ() >= 750 ) {
				EngineCore.outputText( ', rounding out into a gravid bump that jiggles with liquid weight.' );
			} else {
				EngineCore.outputText( ', rounding out into a nicely-sized bump.' );
			}
			EngineCore.outputText( '  You run your hands across it as you give a last few pumps.' );
		}
		...
		if( CoC.getInstance().player.tallness >= 78 ) {
			...
			EngineCore.outputText( 'narrow middle' );
		} else {
			EngineCore.outputText( 'curvy hips' );
		}
		EngineCore.outputText( ' you watch the nun\'s throat bulge almost double wide with each swallow of seed.  Tremendous volleys of semen are pumping down her esophagus rapidfire, and yet the suckling woman barely shows any sign of being full.  A clear puddle has sprung up underfoot, centered below the cock-sucker\'s crotch, and you marvel at the evidence of Scylla\'s pleasure.  She must be cumming non-stop!' );
		...
		if( this.urtaLove() ) {
			EngineCore.outputText( '  You give your lover a wet kiss and a fondle before you go, snickering when it makes her even longer.' );
		} else {
			EngineCore.outputText( '  You give her a quick hug and depart, smiling when you realize the brief touch made her even longer.' );
		}
		this.knockUpUrtaChance();
		//-2 sens, -100 lust;
		CoC.getInstance().player.orgasm();
	...
	Urta.prototype.dockWithUrtaUnderScyllasSoberSupervision = function() {
		...
		EngineCore.outputText( 'Unable to contain yourself, you sigh heavily and release your [armor].  Your immense package grows bigger with each beat of your heart, the blood-swollen veins pulsing hotly as they pump your cock to its full, obscene proportions.  It slowly undulates in the air as it fills to capacity with your lusty lifeblood, and the unmistakable aroma of your masculine ' );
		if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( 'half\'s ' );
		}
		EngineCore.outputText( 'musk fills the air.' );
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( '  Your [sack] swings free, bouncing off the back of [oneCock] when it thickens precipitously.' );
		}
		EngineCore.outputText( '  Sighing with relief, you gently stroke your length as you eye the two beauties meaningfully.' );
		EngineCore.outputText( '\n\nUrta whimpers at the view, and she whispers, "<i>[name], you aren\'t going to put that in me are you?  You\'d rip me in half, ' + CoC.getInstance().player.mf( 'hunk', 'babe' ) + '!</i>"' );
		...
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( ' while your enormous balls gurgle and begin to empty' );
		}
		EngineCore.outputText( '.  The titanic, room-filling cock lifts up off the ground by a few inches, supported by the thick bulge of cum that rushes from ' );
		if( CoC.getInstance().player.hasSheath() ) {
			EngineCore.outputText( 'sheath' );
		} else {
			EngineCore.outputText( 'base' );
		}
		EngineCore.outputText( ' to tip in an instant. It passes through the tight crimson seal, distending the nuns neck for a moment before it settles in her belly.  She looks almost pregnant on a mix of horse and champion-spunk for a moment, but then her belly narrows and her tits swell - just in time to take the next gush of your demonically-blessed orgasm.  Scylla appears to be in heaven, and as she swallows, the darkness leaves her eyes.  What remains is wide-eyed irresistible pleasure, and the vibrations of orgasmic screams vibrate through your swollen ' + CoC.getInstance().player.cockHead( x ) + ' as you dump wave after wave of spooge through the clinging orifice.  A shiny puddle of feminine fluid widens beneath the nun\'s orgasm-wracked frame, and just once, you wish you could stick your head under her robes and watch it happen.' );
		...
		CoC.getInstance().flags[ kFLAGS.URTA_SCYLLA_BIG_DICK_TIMES_DONE ]++;
	...
	Urta.prototype.dockWithUrtaUnderScyllasSoberSupervision2 = function() {
		...
		if( !this.urtaLove() ) {
			EngineCore.outputText( 'waves ' );
		} else {
			EngineCore.outputText( 'kisses you ' );
		}
		EngineCore.outputText( 'goodbye, she says, "<i>Thank you.</i>"' );
	...
	Urta.prototype.flipUrtaTheBird = function() {
		this.urtaSprite();
		EngineCore.outputText( '', true );
		EngineCore.outputText( 'You curl your hand into a fist and extend your middle finger at the alcoholic and cum-slut.  You\'ve got better things to do than waste your time watching those two indulge their addictions.\n\n', false );
		...
		if( CoC.getInstance().flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] < 0 ) {
			CoC.getInstance().flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] = 0;
		}
	...
	Urta.prototype.watchTwoHotBitchesAndJerkIt = function() {
		this.urtaSprite();
		EngineCore.outputText( '', true );
		EngineCore.outputText( ImageManager.showImage( 'urta-scylla-masti' ), false );
		EngineCore.outputText( 'You open your ' + CoC.getInstance().player.armorName + ' and sit down on a crate no more than a few feet away from the orally fixated couple, fishing out ' + Descriptors.sMultiCockDesc(), false );
		if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( ' and exposing your ' + Descriptors.vaginaDescript( 0 ), false );
		}
		EngineCore.outputText( '.  The nun\'s trademark demonic stubs commence sprouting, parting her hair until they rise up at least six inches from her forehead.  Meanwhile the poor, sexually frustrated vixen\'s balls are swelling up like balloons in Scylla\'s hands, growing so large that the fox\'s sack sags around the larger woman\'s hands.  The curly-haired oral slut tries to fellate and support Urta\'s growing size at the same time, and definitely appears to be struggling with it.\n\n', false );
		EngineCore.outputText( 'You stroke ' + Descriptors.sMultiCockDesc(), false );
		if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( ' and touch your ' + Descriptors.vaginaDescript( 0 ), false );
		}
		EngineCore.outputText( ', watching the puffed-up, fire engine-red cock-suckers bob on the trembling horse-shaft.  They slide up and down, up and down, then you see the hint of a tongue sliding around the cock and darting through the sheath.  Urta moans appreciatively at the act, and she turns her head to watch you as you masturbate to the exceptional scene before you.  Her eyes meet yours while her hands curl around Scylla\'s horns and pull the nun deeper into the sheath.  The fox loses her focus, her tongue rolls out, and her balls tremble and rise tight against her body.  She\'s cumming!\n\n', false );
		EngineCore.outputText( 'Scylla\'s neck is pulled tight as heavy bulges of cum work their way up the vixen\'s mismatched meat-pole, through the puffy, cock-sucking lips, and down the nun\'s throat into her hungry gullet.  Though only a few seconds pass, you watch Scylla\'s remarkably elastic midsection begin to bloat with what looks like a 7-month pregnancy.  She keeps her eyes closed the entire time, just silently suctioning down each plus-sized cream deposit as it\'s spurted out.  The only sounds in the room are Urta\'s half-whined moans of pleasure and the rapid slaps of your self-stimulation.\n\n', false );
		EngineCore.outputText( 'The gray-furred hermaphrodite\'s balls are shrinking with each blast, contracting and pausing as her body launches each volley, but they\'ve only lost about half their enhanced size and Scylla\'s getting rounder than she has any right to be.  The nun seems ready for this though, and hums in concentration.  Her belly stops growing immediately, but each of her already prodigious milkers begins to enlarge, sloshing noisily like a barrel being filled.  You nearly push yourself past the edge at the sight of the growing melons, but hold back – you don\'t want to blow until you see just how far this goes.\n\n', false );
		EngineCore.outputText( 'Urta\'s hands let go of the corrupted woman\'s horns and she slumps back against the wall, but the nun just wraps her arms around the blissed-out vixen and suckles harder.  You keep watching and stroking, entranced by the sight of the spooge bubbles vanishing into Scylla\'s neck and plumping up her tits.  Urta\'s eyes are rolled back and she\'s drooling a runnel of spittle down her tit, around one of her black nipples, and down her belly, but she keeps cumming, even though her balls are nearly back to normal.  The poor fox is utterly unable to cope with the sensations running through her drunken form, but she\'s clearly enjoying it.\n\n', false );
		EngineCore.outputText( 'Just as Scylla\'s tits start to drag against the floorboards, she pulls back, letting Urta blow a few long ropes of thick horse-cum over her face and hair.  The nun sighs happily and begins to gather it in her fingers, licking it up while the exhausted fox\'s cock slowly deflates, dripping a trail along the floor.  Urta recovers surprisingly quickly, blinking her eyes and pulling her tongue back into her mouth with a sigh of utter, sated contentment.  You held back for so long, but watching the nun pick pearly strands of animal cum from her hair and swallow it down like a whore is just too much.\n\n', false );
		EngineCore.outputText( 'You pump hard on ' + Descriptors.sMultiCockDesc(), false );
		if( CoC.getInstance().player.totalCocks() > 1 ) {
			EngineCore.outputText( ' in turn', false );
		}
		EngineCore.outputText( ', shooting hot ropes of semen that drape over the satisfied girls.  After so much edging, and such a hot scene, your orgasm is far more intense than any normal masturbation session.  Your whole body trembles while it spurts and squirts, making sure both of the addicts get at least a little bit of your white goo on their bodies.  ', false );
		if( CoC.getInstance().player.cumQ() >= 250 ) {
			EngineCore.outputText( 'A particularly coherent blast of seed splatters over Urta\'s tits, glazing her gray fur and black nipples with a thick layer of dripping spunk.  ', false );
		}
		if( CoC.getInstance().player.cumQ() >= 450 ) {
			EngineCore.outputText( 'Another huge gout takes Scylla straight in the face, completely soaking her with yet more salty mess that drools onto the curve of her pendulous, black-clad tits.  ', false );
		}
		if( CoC.getInstance().player.cumQ() >= 700 ) {
			EngineCore.outputText( 'A few more big blasts of the stuff fire off, spotting the pair with fat, sticky blotches.  ', false );
		}
		if( CoC.getInstance().player.cumQ() >= 1000 ) {
			EngineCore.outputText( 'Your body is so productive that it utterly drenches both participants in the stuff.  Runnels of it drip over their curves while fat droplets pool into a', false );
			if( CoC.getInstance().player.cumQ() >= 2000 ) {
				EngineCore.outputText( 'n inches-deep', false );
			}
			EngineCore.outputText( ' puddle on the ground.  ', false );
		}
		EngineCore.outputText( 'Finally spent, you slump back and smile.\n\n', false );
		EngineCore.outputText( 'Scylla looks shocked, as if she\'s seeing you for the first time, but her nose twitches and she starts to shovel your seed into her mouth with greedy abandon.  Even so, her body rapidly \'digests\' the fluid intake, and the nun starts to slim up before your eyes.  Urta wobbles up onto shaky legs and stumbles into you, giving you a wet kiss that tastes faintly of your salty leavings and then passionately hugs you.  She breaks the embrace and slurs, ', false );
		if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00143 ] === 1 ) {
			EngineCore.outputText( '"<i>We need to do thish again!</i>"', false );
		} else {
			EngineCore.outputText( '"<i>You\'re the besht ' + CoC.getInstance().player.mf( 'man', 'woman' ) + ' a girl could ashk for.</i>"', false );
		}
		EngineCore.outputText( '  Before you can answer, she gives your ass a slap and finds a water barrel to wash up in.  Scylla finishes her \'meal\' and looks up at you, her alabaster skin turning completely crimson before she too runs off to clean up.\n\n', false );
		EngineCore.outputText( 'You smile happily and pick up your ' + CoC.getInstance().player.armorName + '.  It\'s past time you checked up on your camp.', false );
		if( this.urtaLove() ) {
			EngineCore.outputText( '  You give your lover\'s back a wistful grin and muse over how good her taste in women is as you leave.', false );
		}
		CoC.getInstance().player.orgasm();
		//+luv;
		this.urtaLove( 2 );
	...
	Urta.prototype.makeUrtaSitOnYourLapWithScylla = function() {
		...
		var y = CoC.getInstance().player.cockThatFits2( this.urtaCapacity() );
		EngineCore.outputText( '', true );
		EngineCore.outputText( ImageManager.showImage( 'urta-scylla-lapfuck' ), false );
		EngineCore.outputText( 'You smirk at the drunken fox and inform her in no uncertain words that you\'ll do more than watch.  She blinks at you, her inebriated mind unable to process new thoughts as you stalk forwards and sit behind her on the crate', false );
		if( CoC.getInstance().player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_NAGA ) {
			EngineCore.outputText( ', snaking your tail around her legs', false );
		} else if( CoC.getInstance().player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_GOO ) {
			EngineCore.outputText( ', enveloping her legs with your goo', false );
		} else {
			EngineCore.outputText( ', straddling your ' + CoC.getInstance().player.legs() + ' around her hips', false );
		}
		EngineCore.outputText( '.  You grab her narrow waist and pull her up, dragging Scylla\'s cock-locked head up with her and aligning your ' + Descriptors.cockDescript( x ) + ' with her backdoor before you start to lower her back down.  ', false );
		if( y > -1 ) {
			EngineCore.outputText( 'Your ' + Descriptors.cockDescript( y ) + ' even manages to get in line with her drooling, black-lipped pussy.  ', false );
		}
		if( (CoC.getInstance().player.cockTotal() > 1 && y === -1) || CoC.getInstance().player.cockTotal() > 2 ) {
			EngineCore.outputText( 'You allow your remaining, unused dickflesh to flop against her back, leaking pre-cum into her fur.  ', false );
		}
		EngineCore.outputText( 'You aren\'t sure if it\'s the alcohol in her or her own anal talents, but she slides down without a complaint, pulling Scylla\'s vacuum-like mouth down with her', false );
		if( y !== -1 ) {
			EngineCore.outputText( ' and squeezing you tightly with her sopping cunt', false );
		}
		EngineCore.outputText( '.\n\n', false );
		EngineCore.outputText( 'She grunts, "<i>', false );
		if( x !== -1 && y !== -1 ) {
			EngineCore.outputText( 'Oooh, and in both holes?  You naughty ' + CoC.getInstance().player.mf( 'boy', 'girl' ) + ', you!', false );
		} else if( this.urtaLove() ) {
			EngineCore.outputText( 'Oooh yeah, fill me love!', false );
		} else if( Utils.rand( 2 ) === 0 ) {
			EngineCore.outputText( 'Oooh, you know how to treat a lady!', false );
		} else {
			EngineCore.outputText( 'MMmm, that\'s nice!', false );
		}
		EngineCore.outputText( '</i>"  She\'s actually trying to watch you over her shoulder now, completely ignoring the cock-addled nun as the latter bobs on her horse-meat.  You make the most of it and ', false );
		if( this.urtaLove() || CoC.getInstance().player.cor <= 40 ) {
			EngineCore.outputText( 'kiss her full on the mouth', false );
		} else {
			EngineCore.outputText( 'bite her neck', false );
		}
		EngineCore.outputText( ' while you grab her soft, furred asscheeks and lift her up, feeling the exquisite texture of her pucker ', false );
		if( y !== -1 ) {
			EngineCore.outputText( 'and twat ', false );
		}
		EngineCore.outputText( 'massaging your manhood the whole way.  Before you go too high, you drop her, and let her weight impale her back on top of you.  Her rapidly expanding balls slap into your own and smear them with her pussy juice.\n\n', false );
		EngineCore.outputText( 'Urta\'s tail curls snugly around your waist, and the drunken fox takes over, lifting herself up even as Scylla\'s magic swells her balls past the size of grapefruits.  She openly moans when she reaches the apex, and smoothly lowers herself back down, squeezing and stroking you with her internal muscles while trying to endure the nun\'s incessant suckling.  The measured ', false );
		if( y !== -1 ) {
			EngineCore.outputText( 'double penetration', false );
		} else {
			EngineCore.outputText( 'anal fuck', false );
		}
		EngineCore.outputText( ' drives you wild, but being on the bottom as you are, there isn\'t much you can do aside from grabbing fistfuls of vixen-ass and slapping it for encouragement.  Urta gets the message and begins to bounce vigorously, mewling and moaning as her balls get large enough to reach the floor.  You lend a hand and play with her sheath, feeling the flesh underneath quake and tremble as the fox\'s orgasm hits.\n\n', false );
		EngineCore.outputText( 'Scylla\'s neck is pulled tight as Urta\'s urethra is stretched wide by huge bulges of cum.  You can actually feel them sliding past the fingers you\'ve dug into her sheath.  No sooner than one squeezes past is another on its way.  You wonder how Scylla can handle such incredible volumes and the only answer you can think of is magic.  Whatever the case, her swallowing gets noisier and Urta starts bouncing even faster atop your ' + Descriptors.cockDescript( x ), false );
		if( y !== -1 ) {
			EngineCore.outputText( ' and ' + Descriptors.cockDescript( y ), false );
		}
		EngineCore.outputText( '.  You pinch one of her shiny black nipples between your fingers as encouragement, and she actually howls with delight.  The herm vixen\'s cum-pumping cock starts spasming in your grip, the order of her previous cum blasts dissolving to turn her cock into a frenzied, constantly squirting spooge-hose.  Every spurt of jism that Scylla swallows is one less in the fox\'s magically-enhanced ballsack, and the unnatural, mighty orbs begin to shrink back towards their normal size with alarming rapidity.\n\n', false );
		EngineCore.outputText( 'Scylla\'s growing belly and tits smush around your ' + CoC.getInstance().player.legs() + ', swelling further and further as she tries to devour all the creamy horse-spunk the fox is hosing into her.  Yet the touch of long, gentle fingers caressing your ' + Descriptors.sackDescript() + ' and the gradual inflation of your ' + Descriptors.ballsDescriptLight() + ' lets you know that the slutty nun\'s discovered another source of seed.  You groan as warmth and stimulation make your orbs churn and slosh, growing not just in size but also in raw, animal NEED.  Driven to near madness by the drunken fox\'s delicious backside and the building, backed-up cum in your sack, you grab hold of Urta with both hands and start jackhammering her asshole', false );
		if( y !== -1 ) {
			EngineCore.outputText( ' and pussy', false );
		}
		EngineCore.outputText( ', pounding the lithe fox as if your life depended on your orgasm.\n\n', false );
		EngineCore.outputText( 'Scylla\'s fingers don\'t stop their assault, and as Urta\'s softening cock flops free of the nun\'s ruby lips, smearing them with jizz, the nun takes each of your distended nuts between her palms, one at a time, and rubs it until you can feel the cold floor pressing against their over-sized bulk.  Urta\'s flagging cock reacts to the repeated strikes you\'re giving her prostate by returning to full hardness, though you wonder if Scylla\'s magic had a part in it as well.  Regardless, the nun lifts her habit to expose one lipped, milk-leaking nipple and slides the fox\'s girth inside.  Urta\'s eyes roll back and she begins to babble incoherently, the massive orgasm, followed by anal penetration and nipple-fucking, just too much for her poor battered psyche.\n\n', false );
		EngineCore.outputText( 'You cum, hard enough to make you cross your eyes and slam Urta\'s hips into you with painful force, even through the padding of her supple buttcheeks.  The first ass-filling wave of jism completely packs her backdoor, surrounding you in a sea of your own sticky moisture.', false );
		if( y > -1 ) {
			EngineCore.outputText( '  Meanwhile, your other ' + Appearance.cockNoun( CoC.getInstance().player.cocks[ y ].cockType ) + ' utterly fills her womb, stuffing it so completely that the fox\'s belly rounds out.', false );
		}
		if( (CoC.getInstance().player.cockTotal() > 1 && y === -1) || CoC.getInstance().player.cockTotal() > 2 ) {
			EngineCore.outputText( '  A wave of the stuff splatters her back, glazing her from the shoulder-blades to the crack of her ass and nearly catching you in the chin. Maybe there is such a thing as too many cocks?', false );
		}
		EngineCore.outputText( '  Scylla\'s hands rub the fox\'s belly tenderly as she humps away at the lewd nun, using whatever magic she has to ensure you won\'t damage the drunken vixen as you continue to push larger and larger amounts of semen into her.  After three more blasts, she runs out of room inside her, and it starts squirting back out with each fresh stroke, soaking your waist, ' + CoC.getInstance().player.legs() + ', and rapidly shrinking balls.  You lose track of time, knowing only the pleasure of orgasm and the feeling of your own spooge pumping into your lover until the unholy cumsplosion finally ends.\n\n', false );
		EngineCore.outputText( 'You regain your wits to the sight of a very pregnant-looking Urta and Scylla leaning on each other and trying to make their way to a water barrel to clean up.  The vixen is so full she looks like a mother in her ninth month of pregnancy with triplets, but as she starts to clean up Scylla gently presses on the fox\'s belly and devours the cum she squeezes out.  Urta shakes, but doesn\'t emit a sound beyond a few strangled half-moans of pleasure.  If the fresh horse-cum soaking the nun\'s habit is any indicator, Urta must have gotten off again during your orgasm.  Wild!\n\n', false );
		EngineCore.outputText( 'You grab a rag and wipe up before donning your ' + CoC.getInstance().player.armorName + '.  Urta saunters over, looking like she\'s sobered up quite a bit.  She\'s got ', false );
		if( this.urtaLove() ) {
			EngineCore.outputText( 'a contended smile on her face, and gives you a long, slow kiss that practically radiates her love for you.', false );
		} else if( CoC.getInstance().flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] < 11 ) {
			EngineCore.outputText( 'a nervous smile on her face, but gives you a quick peck all the same.', false );
		} else {
			EngineCore.outputText( 'a satisfied smile on her face, and crushes her lips against yours in a passionate kiss.', false );
		}
		EngineCore.outputText( '  She says, "<i>I think I\'ll need to help Scylla to her room... somehow whatever she did to me must have pulled all the alcohol out of me and into my cum.  Don\'t worry, I\'ll ', false );
		if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00143 ] <= 1 ) {
			EngineCore.outputText( 'pay for the door and ', false );
		}
		EngineCore.outputText( 'make sure she\'s ok.</i>"\n\n', false );
		EngineCore.outputText( 'Did her cock just twitch?  No, you scold yourself; it couldn\'t have, not after two orgasms like that.  Urta hooks her arm around the giggling nun\'s midsection and pulls the stumbling sister towards the doorway.  Scylla looks back at you and half-giggles, half-slurs, "<i>Tee-hee, thish ish so much fun!  I love you guysh!</i>"\n\n', false );
		EngineCore.outputText( 'You make your way out with your head low, but you do see Scylla and Urta walking up the stairs towards a truly private room.  They each look a little unsteady and neither of Scylla\'s hands can be seen, but you\'re sure they\'ll be ok.', false );
		CoC.getInstance().player.orgasm();
	...
	Urta.prototype.tellUrtaNoMoreScylla = function() {
		this.urtaSprite();
		EngineCore.outputText( '', true );
		CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00147 ] = 1;
		EngineCore.outputText( 'You let Urta know that you\'re willing to overlook this, but that Scylla is a slippery slope that\'s best avoided.  She nods, a sober expression on her vulpine face.  Once you finish, she replies, "<i>I understand, but you need to know – when \'it\' gets too much for me to handle... if you aren\'t around to help I\'m going to see Edryn.  S-she helps... if it wasn\'t for her I probably would\'ve given up and done something stupid a long time ago.  What we have is more important, but unless we can get together every couple days I\'ll NEED to visit her.  ', false );
		if( this.urtaLove() ) {
			EngineCore.outputText( 'Just please, don\'t doubt my love for you.  ', false );
		}
	...
	Urta.prototype.tellUrtaMoreScyllaIsFine = function() {
		this.urtaSprite();
		EngineCore.outputText( '', true );
		EngineCore.outputText( 'You give Urta a raunchy, lewd smile and let her know that you\'re okay with her \'helping\' Scylla, and vice-versa, so long as they don\'t mind you popping in.  She smiles and gives you a wink, but her expression darkens after a moment of thought.  Urta interrupts to mention, "<i>There\'s something else.  If I can\'t find you or Scylla and I NEED to cum, I\'ll have to visit Edryn.  She\'s probably the only thing that\'s kept me sane, and if you aren\'t around to help with my needs I\'ll visit her.  ', false );
		if( this.urtaLove() ) {
			EngineCore.outputText( 'Just remember that YOU\'RE the one I love, not her.  ', false );
		} else {
			EngineCore.outputText( 'Me and her, we\'ve been partners in the guard for a long time, but the sex is just mechanical, to sate us.  You don\'t have to worry about her stealing me away.  ', false );
		}
	...
	Urta.prototype.tellUrtaToBeADrunkenHussy = function() {
		...
		if( CoC.getInstance().player.cor < 33 ) {
			EngineCore.outputText( 'blush a little bit', false );
		} else if( CoC.getInstance().player.cor < 66 ) {
			EngineCore.outputText( 'smile nervously', false );
		} else {
			EngineCore.outputText( 'grin widely', false );
		}
		EngineCore.outputText( ' and explain that when she gets drunk, lets her guard down, and gets a little more aggressive... you like it.  A lot.  Her ears perk up at your words, though her expression is a little uncertain while you explain it.  By the time you finish, something warm brushes by your ' + CoC.getInstance().player.leg() + ' and gently \'thunks\' the table.  Clearly she\'s as into the idea as you.\n\n', false );
		...
		CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00146 ] = 1;
	...
	Urta.prototype.tellUrtaToStayTheSame = function() {
		this.urtaSprite();
		EngineCore.outputText( '', true );
		EngineCore.outputText( 'You smile and ask her why you would ever want her to change?  You ', false );
		if( this.urtaLove() ) {
			EngineCore.outputText( 'love', false );
		} else {
			EngineCore.outputText( 'like', false );
		}
	...
	Urta.prototype.chastityBeltFun = function( cameFromSexMenu ) {
		...
		if( cameFromSexMenu === undefined || cameFromSexMenu ) {
			...
			EngineCore.outputText( 'Eyeing your foxy lover, you tell her to lose the condom, you have something else planned.\n\n' );
			EngineCore.outputText( '“<i>Lose it?  What do you have in mind?</i>” Urta asks.  She looks past you towards her cupboard.  “<i>Oh, no, not that belt!  0...Do you really enjoy seeing me make such a mess of myself when I finally get it off?</i>” she protests.\n\n' );
	...
	Urta.prototype.QBsTalkExpack = function() {
		...
		//(Lover Urta);
		if( this.urtaLove() ) {
			EngineCore.outputText( 'When you approach Urta, she smiles to see you.  However, there is no rapid swivelling in her seat to avoid banging her horse-cock against the table; evidently she\'s not in the mood for some sex right now.  You sit down beside her, and she happily loops her arm around you, planting a tender kiss on your lips, before almost apologetically stating, "<i>Hey there, lover.  Want to talk for a change?</i>" with a twinkle of good-natured mirth in her eyes.' );
		} else {
			EngineCore.outputText( 'When you approach Urta, you are almost surprised at the lack of an audible \'thump\'.  Seeing your confusion, the herm fox quips, "<i>What\'s with the look?</i>" while raising an eyebrow.  When you indicate that you had expected to hear her demonstrating her excitement to see you, she blushes and looks embarrassed.  "<i>Well... you do a pretty good job of letting my little pony blow off some steam, ya know, [name]?</i>"' );
		}
	...
	Urta.prototype.friendsTalkExpack = function() {
		EngineCore.clearOutput();
		this.urtaSprite();
		EngineCore.outputText( 'You approack Urta\'s table and she motions for you to take a seat.  She finishes writing something on one of the pieces of paperwork scattered over the table, takes a sip of her drink and leans back in her chair.  “<i>Nice to see you again ' + CoC.getInstance().player.short + '.  What can I do for you?</i>”' );
		this.urtaDialogueMenu();
	};
	Urta.prototype.urtaDialogueMenu = function() {
		...
		} else if( CoC.getInstance().flags[ kFLAGS.FIRST_TIME_AT_URTA_HOUSE ] > 0 ) {
	...
	Urta.prototype.urtaDiscussesSelf = function() {
		EngineCore.clearOutput();
		this.urtaSprite();
		EngineCore.outputText( 'You tell Urta you\'d like to talk about her a little.' );
		if( !this.urtaLove() ) {
			EngineCore.outputText( '“<i>You wanna talk about me?</i>” she asks' + (this.urtaJustFriends() ? '' : ', blushing softly') + '.  “<i>Well, there isn\'t THAT much to tell, but what do you want to know?</i>”   She seems surprisingly pleased to have the chance to talk.' );
		} else {
			EngineCore.outputText( '“<i>Well, I don\'t think there\'s that much to tell,</i>” she says with a smile.  She idly blows a bang out of her face and exhales, “<i>Well, lover, I\'m an open book for you, what do you want to know?</i>”' );
		}
		...
		} else if( CoC.getInstance().flags[ kFLAGS.URTA_QUEST_STATUS ] === 0.5 ) {
		...
		} else if( CoC.getInstance().flags[ kFLAGS.URTA_QUEST_STATUS ] === 1 ) {
	...
	Urta.prototype.flirtWithUrta = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.URTA_PC_LOVE_COUNTER ] === -1 ) {
			...
			EngineCore.outputText( 'You ask Urta if, living in Tel\'Adre, it\'s easier to resist the powerful feelings that come from living in this world.\n\n' );
			EngineCore.outputText( '“<i>I\'ve lived here my whole life, so I don\'t have anything else to compare with.  What do you mean by \'feelings\'?</i>”\n\n' );
			EngineCore.outputText( 'You tell her that in your world you never felt a deep urge to strip off all your clothes and ' + (CoC.getInstance().player.cor < 75 ? 'make sweet love to' : 'slake your lust on') + ' the nearest person.\n\n' );
		} else {
			EngineCore.outputText( 'You tell her that you don’t have any problem with herms, in fact you find some of them quite attractive.\n\n' );
			EngineCore.outputText( 'Urta gulps down a bit more of her booze and you decide to have a little fun.  ' );
		}
		EngineCore.outputText( 'You quietly slide your ' + (CoC.getInstance().player.isNaga() ? 'tail' : 'foot') + ' across under the table and rub it against the inside of her leg.  Urta practically jumps out of her seat, then tries to pretend nothing happened.\n\n' );
		EngineCore.outputText( 'You’re pretty sure most of the bar noticed, but you don’t care.  You keep rubbing and feel a thump as something large smacks against the underside of the table.  Urta drains the last of her drink and whispers, “<i>' + (CoC.getInstance().flags[ kFLAGS.URTA_PC_LOVE_COUNTER ] === -1 ? 'What about Kath?  I... I wouldn\'t...' : 'Please - it’s so hard...') + '</i>”\n\n' );
		EngineCore.outputText( 'You never find out exactly what she was going to say.  When you start to rub your ' + (CoC.getInstance().player.isNaga() ? 'tail' : 'foot') + ' in little circles against the root of her huge cock Urta loses her train of thought and just leans back in her chair, trying to keep from spraying her pent up cum across the floor.\n\n' );
		EngineCore.outputText( 'You ease up so she has a chance of understanding what you’re saying.  “<i>' + (CoC.getInstance().flags[ kFLAGS.URTA_PC_LOVE_COUNTER ] === -1 ? 'I\'ll explain everything to Kath.  You want this as much as I do, right?' : 'Yes, some chicks, with their big dicks, just do it for me.') + '</i>”  You slide ' + (CoC.getInstance().player.isNaga() ? 'the tip of your tail' : 'your toes') + ' a little lower and start moving it in little circles against her balls.\n\n' );
		EngineCore.outputText( 'Before Urta pops you get up from the table and tell her to meet you out back, right where things went off the rails that first time.  Urta nods eagerly and the edge of the table rises slightly.\n\n' );
		EngineCore.outputText( 'You wait outside, hiding in a shadowy corner.  You got Urta so excited that it’s a good fifteen minutes before she throws open the back door of the bar.  She looks hungry, yet unsure.  You’re guessing she’s had such terrible experiences with people in the past that despite your obvious interest she still can’t believe you want her.\n\n' );
		CoC.getInstance().flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] = 0; //this.urtaFuckHer will raise it to 1
		CoC.getInstance().flags[ kFLAGS.URTA_PC_LOVE_COUNTER ] = 0; //Reset both flags to put Urta back to her 'never met you before' state, ready for her first fuck
	...
	Urta.prototype.urtaFertilityDiscussion = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'Urta traces a finger around the corner of her drink, "<i>So, what do you think, [name]?' );
		if( CoC.getInstance().flags[ kFLAGS.URTA_FERTILE ] === 1 ) {
			EngineCore.outputText( '  Should I stay fertile?' );
		} else {
			EngineCore.outputText( '  How about I cut loose and go fertile?  Can you imagine me with a nice, round pregnant belly and a cock virile enough to get you pregnant just from looking at it?' );
		}
		if( CoC.getInstance().flags[ kFLAGS.URTA_PREG_EVERYBODY ] === 1 ) {
			EngineCore.outputText( '  Or did you come to ask me not to be fertile and virile around anyone else?' );
		} else {
			EngineCore.outputText( '  Would you mind me being able to put a bun in Edryn\'s oven?  Imagine all the lil foxtaurs running around!' );
			if( CoC.getInstance().flags[ kFLAGS.EDRYN_NUMBER_OF_KIDS ] > 0 ) {
				EngineCore.outputText( '  After all, you already had a turn!  Fair\'s fair!' );
			}
		}
		EngineCore.outputText( '</i>"  She winks at you before blowing you a playful raspberry.\n' );
		EngineCore.menu();
		if( CoC.getInstance().flags[ kFLAGS.URTA_FERTILE ] === 1 ) {
	...
	Urta.prototype.urtaFertilityToggle = function() {
		EngineCore.clearOutput();
		if( CoC.getInstance().flags[ kFLAGS.URTA_FERTILE ] === 1 ) {
			EngineCore.outputText( '"<i>Well, I guess that\'s okay.  But you better make it worth my while later, okay?</i>" Urta says with a half-hidden sigh.' );
			CoC.getInstance().flags[ kFLAGS.URTA_FERTILE ] = 0;
		} else {
			EngineCore.outputText( 'Urta throws her hands up and cheers, "<i>Yesss!</i>"  Realizing that everyone is looking over at the two of you now, she gives a sheepish smirk and glances back at you.  "<i>I can\'t wait!</i>"' );
			CoC.getInstance().flags[ kFLAGS.URTA_FERTILE ] = 1;
		}
		EngineCore.menu();
	...
	Urta.prototype.urtaPregOthersToggle = function() {
		EngineCore.clearOutput();
		if( CoC.getInstance().flags[ kFLAGS.URTA_PREG_EVERYBODY ] === 1 ) {
			EngineCore.outputText( '"<i>Awww, are you sure?  Wouldn\'t you like ' );
			if( CoC.getInstance().player.hasCock() ) {
				EngineCore.outputText( 'to help me make Edryn\'s belly bloat with our seed, and then play \'Guess Who Is The Father\'?' );
			} else if( CoC.getInstance().player.hasVagina() ) {
				EngineCore.outputText( 'to have me get the both of you pregnant at the same time, so we can all enjoy it together?' );
			} else {
				EngineCore.outputText( 'see me put this thing to work in the way that it\'s meant to?' );
			}
			EngineCore.outputText( '</i>" Urta says, giving you a playful pinch.' );
			CoC.getInstance().flags[ kFLAGS.URTA_PREG_EVERYBODY ] = 0;
		} else {
			EngineCore.outputText( 'Urta smiles salaciously, like a cat that got the cream.  Then, without warning, she kisses you square on the lips, passionately frenching you.  After nearly a full minute of frenzied kissing, she says, "<i>This is gonna be fun!</i>"' );
			CoC.getInstance().flags[ kFLAGS.URTA_PREG_EVERYBODY ] = 1;
		}
		EngineCore.menu();
		EngineCore.addButton( 0, 'Back', this.urtaFertilityDiscussion );
	};
	...
	Urta.prototype.urtaDiscussesEdryn = function() {
		EngineCore.clearOutput();
		this.urtaSprite();
		EngineCore.outputText( 'You think for a moment, then tell her that you have questions about Edryn.' );
		if( !this.urtaLove() ) {
	...
	Urta.prototype.urtaDiscussesTheWatch = function() {
		EngineCore.clearOutput();
		this.urtaSprite();
		EngineCore.outputText( 'You tell Urta that you have a few questions about the Tel\'Adre city watch.' );
		if( !this.urtaLove() ) {
	...
	Urta.prototype.urtaDiscussesAlcholism = function() {
		EngineCore.clearOutput();
		this.urtaSprite();
		if( CoC.getInstance().flags[ kFLAGS.DISCUSSED_URTA_ALCOHOLISM ] === 0 && CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00145 ] === 0 ) {
			EngineCore.outputText( 'You quietly inform Urta that you and she need to talk about her drinking habit.  She swallows nervously and insists, "<i>I - I only drink to try and keep my cock under control.</i>"' );
			...
			if( CoC.getInstance().player.cor < 33 ) {
				EngineCore.outputText( 'and you feel a little bad for putting her through this' );
			} else if( CoC.getInstance().player.cor < 66 ) {
				EngineCore.outputText( 'and you would feel bad if this talk weren\'t necessary' );
			} else {
				EngineCore.outputText( 'but you don\'t let up for a moment' );
			}
			EngineCore.outputText( '.  You insist that you aren\'t upset with her, that you\'re not blaming her for anything, but you need to talk to her about this matter.' );
			EngineCore.outputText( '[pg]Urta refuses to meet your eye for several long minutes.  Nervously, she begins tapping one clawed finger on the table.  "<i>I... okay.  The truth is that I did start drinking originally in the hopes it would make my erections subside and make me harder to arouse.  As you\'ve seen, it doesn\'t work like that - truth be told, I\'m kind of a horny drunk.</i>"' );
			EngineCore.outputText( '[pg]You ask why, then, she does it - why does she keep drinking?' );
			EngineCore.outputText( '[pg]"<i>Because it\'s an excuse for me to cut loose, alright?</i>" she barks indignantly.  Now she meets your eyes, staring into them with defiance.  "<i>I work day in and day out to keep this city from falling apart, and the whole time, I have to deal with this... this thing between my legs, undermining everything I say.  ' );
			if( this.urtaLove() ) {
				EngineCore.outputText( 'Lover' );
			} else {
				EngineCore.outputText( 'Cutey' );
			}
			EngineCore.outputText( ', you don\'t know what I\'ve been through.  In here, when I\'m off the clock, I can just have a bottle, hang out with my friends, and kind of... just... bask in everyone else\'s happiness.  It\'s not much, but it\'s better than sitting at home, bored and lonely.</i>"' );
			EngineCore.outputText( '[pg]She finally ceases her rambling dialogue and sighs loudly, shaking her head.  "<i>Maybe it was a bad idea, but it was my idea, you know?</i>"  Then, she looks up at you, gently.  "<i>Besides, I got to meet you, didn\'t I?  Even with all the bad, I gained you for a ' );
			if( !this.urtaLove() ) {
				EngineCore.outputText( 'friend.</i>"' );
			} else {
				EngineCore.outputText( 'lover.  I don\'t think I\'ve ever had anyone accept me like you have - even Edryn kind of holds back from me.  You... you\'re different.  Maybe it\'s because you\'re an off-worlder, I don\'t know, but meeting you has been the best thing to happen to me since I joined the guard.  Honestly... I feel like I could probably stop - I don\'t really need it anymore, though I do seem to wind up having a lot of fun when I do.</i>"' );
			}
			...
			if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00146 ] === 1 ) {
				EngineCore.outputText( '  "<i>I can\'t drink any more than I already do - I\'m kind of pushing the laws to drink as much as I do already.</i>"  She burps loudly, then starts on another bottle.' );
			} else if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00146 ] === 0 ) {
				EngineCore.outputText( '  "<i>You said you didn\'t mind me drinking as much as I do... are you going to ask me to cut down?</i>" she asks, calmly and clearly assuming that\'s what you intend.' );
			} else {
	...
	Urta.prototype.urtaDiscussAlcoholDrinkMore = function() {
		...
		CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00146 ] = 1;
		CoC.getInstance().flags[ kFLAGS.DISCUSSED_URTA_ALCOHOLISM ] = 1;
		EngineCore.doNext( SceneLib.camp.returnToCampUseOneHour );
	};
	//[=Drink Less=];
	Urta.prototype.urtaDiscussAlcoholDrinkLess = function() {
		...
		if( this.pregnancy.type === PregnancyStore.PREGNANCY_PLAYER ) {
			EngineCore.outputText( ', once she\'s not pregnant anymore, of course' );
		}
		EngineCore.outputText( '.  Her eyes glitter with steely resolve, and she leans over the table to kiss you on the lips.  The two of you wrap up the conversation, with you hoping that you\'ve seen the last of drunken Urta.' );
	...
	Urta.prototype.urtaDiscussesFamily = function() {
		...
		this.urtaSprite();
		if( !this.urtaLove() ) {
			EngineCore.outputText( 'You tell Urta that you\'d like to learn about her family.  What are her parents like?  Does she have any siblings?  Is she the only hermaphrodite in the family?' );
			//(Regular;
			if( !this.urtaLove() ) {
				if( CoC.getInstance().flags[ kFLAGS.URTA_FAMILY_TALK_ATTEMPTS ] === 0 ) {
	...
	Urta.prototype.urtaDiscussesSexAndRomance = function() {
		EngineCore.clearOutput();
		this.urtaSprite();
		EngineCore.outputText( 'You ask Urta if she\'ll tell you her romantic & sexual history.' );
		if( !this.urtaLove() ) {
	...
	Urta.prototype.urtaDiscussionTeaseAfterRomance = function() {
		EngineCore.clearOutput();
		this.urtaSprite();
		EngineCore.outputText( 'You thank her for telling you about her past like this but follow it up by asking her what she likes in the bedroom, doing your best to hide a mischievous grin.' );
	...
	Urta.prototype.urtaDiscussesEmployment = function() {
		EngineCore.clearOutput();
		this.urtaSprite();
		EngineCore.outputText( 'You ask how and when it was that Urta came to be part of the Tel\'Adre watch.' );
	...
	Urta.prototype.urtaDiscussesPrejudice = function() {
		EngineCore.clearOutput();
		this.urtaSprite();
		EngineCore.outputText( 'Hesitantly, you ask Urta if she has ever faced prejudice for her hermaphroditic nature.' );
	...
	Urta.prototype.urtaDiscussesEdrynHistory = function() {
		...
		this.urtaSprite();
		//[=History Together=];
		EngineCore.outputText( 'You ask Urta what her past with Edryn is.' );
	...
	Urta.prototype.urtaDiscussesWorkingWithEdryn = function() {
		EngineCore.clearOutput();
		this.urtaSprite();
		EngineCore.outputText( 'You tell Urta that you\'re curious about her working relationship with Edryn.  Do they work together often?' );
	...
	Urta.prototype.urtaDiscussesEdrynRomance = function() {
		...
		if( !this.urtaLove() ) {
			if( !this.urtaOpenAboutEdryn() ) {
				CoC.getInstance().flags[ kFLAGS.URTA_OPEN_ABOUT_EDRYN ] = 1;
				EngineCore.outputText( 'You tell Urta that you\'ve noticed she and Edryn seem to hang out together a lot.  Furthermore, as a centauress, you figure Edryn\'s probably capable of handling what Urta has hanging between her legs.  You\'re curious; are they a couple?' );
			...
			//Subsequent Regular Variant:;
				...
				EngineCore.outputText( 'You give Urta a thoughtful look, and then ask how things are with Edryn these days.' );
				EngineCore.outputText( '"<i>Good,</i>" the fox-morph replies freely.  "<i>She\'ll never compare to you, but she is a truly wonderful fuck... I swear, there is nothing quite like feeling a mare-cunt juice itself around you...</i>"  She smirks at you, obviously enjoying the chance to make you squirm, but ' );
				if( CoC.getInstance().player.cor < 66 ) {
					EngineCore.outputText( 'her manner is playful' );
				} else {
					EngineCore.outputText( 'her expression sobers when you make no response' );
				}
				EngineCore.outputText( '.' );
				//(If player can now sleep with Edryn for free:;
				if( CoC.getInstance().player.statusAffectv1( StatusAffects.Edryn ) >= 5 ) {
					EngineCore.outputText( '  "<i>Of course, you know that firsthand, don\'t you?</i>" she grins.  "<i>Did hearing that I was having sex with her make you curious about what it was like?' );
					//(If player has fathered at least 1 child on Edryn:;
					if( CoC.getInstance().flags[ kFLAGS.EDRYN_NUMBER_OF_KIDS ] > 0 ) {
						EngineCore.outputText( '  Still, I can\'t believe you actually knocked her up - I had to pick my jaw up off the floor when she told me that the foal kicking inside her ballooning gut was yours.  I mean, those herbs are supposed to be foolproof...</i>" She shakes her head, half in disbelief of your sheer virility, half in awe of it.' );
					} else {
						EngineCore.outputText( '</i>"' );
					}
					if( CoC.getInstance().flags[ kFLAGS.URTA_FERTILE ] === 0 ) {
						EngineCore.outputText( '  "<i>Still, I hope you don\'t have any idle thoughts in your head about knocking me up; there\'s a difference between overwhelming contraceptives and sowing seed where the field\'s barren,</i>" she states, patting her flat belly for emphasis.  Despite how flippant she tries to sound when she says this, you can tell that it\'s something of a sore subject for her.' );
					}
				}
		...
		//Lover;
			...
			if( !this.urtaOpenAboutEdryn() ) {
				...
				EngineCore.outputText( '"<i>Hmph!  So you should be!</i>" Urta says, sticking her nose up in the air in an indignant huff... but she can\'t keep a straight face for more than a few moments.  "<i>I still can\'t believe you took it so well when I told you - I really am a lucky girl to have someone like you, aren\'t I?</i>" she says.' );
				//(If Edryn is doing the player for free:;
				if( CoC.getInstance().player.statusAffectv1( StatusAffects.Edryn ) >= 5 ) {
					EngineCore.outputText( '  "<i>Though I have to admit I was still a little jealous when Edryn told me you were having sex with her too.  Still, I guess that\'s just another thing we have in common, yeah?  A mutual taste in sexy ladies.</i>"  She laughs, and mockingly shapes her hands through the air as if caressing Edryn\'s sizable boobs.' );
				}
				//(If Edryn has had at least one baby:;
				if( CoC.getInstance().flags[ kFLAGS.EDRYN_NUMBER_OF_KIDS ] > 0 ) {
					if( CoC.getInstance().flags[ kFLAGS.URTA_QUEST_STATUS ] === 1 ) {
						if( SceneLib.urtaPregs.urtaKids() === 0 ) {
							EngineCore.outputText( '  "<i>...I still can\'t believe that you actually get her pregnant, though.  It\'s just not fair that Edryn\'s got little hooves running around, and after everything I went through, I don\'t have kits of my own... yet.</i>" She gives you a slightly predatory smile along with the last word.' );
						} else {
							EngineCore.outputText( '  "<i>...I still can\'t believe that you actually got her pregnant, though.  I guess I\'ll have to keep you busy in the sack if we\'re going to keep up, huh?</i>" She gives you a slightly predatory smile along with the last word.' );
						}
					} else {
						EngineCore.outputText( '  "<i>...I still can\'t believe that you actually got her pregnant, though. It\'s just not fair that Edryn\'s got little hooves running around, and you and I can\'t have any.</i>"  Urta sighs.' );
					}
				}
	...
	Urta.prototype.urtaDiscussesEdrynNonLoveREJECT = function() {
		EngineCore.clearOutput();
		this.urtaSprite();
		EngineCore.outputText( 'No matter how you look at it, you can\'t help but feel betrayed - Urta\'s been sleeping around behind your back.  If she didn\'t want this relationship to be monogamous, she should have told you before, and you tell her as much.' );
	...
	Urta.prototype.urtaDiscussesEdrynLoveTolerate = function() {
		...
		CoC.getInstance().flags[ kFLAGS.PC_DISLIKES_URTA_AND_EDRYN_TOGETHER ] = 1;
		EngineCore.doNext( SceneLib.camp.returnToCampUseOneHour );
	};
	//[=Breakup=];
	Urta.prototype.urtaDiscussesEdrynLoveBreakup = function() {
		EngineCore.clearOutput();
		this.urtaSprite();
		EngineCore.outputText( 'No matter how you look at it, you can\'t help but feel betrayed - Urta\'s been sleeping around behind your back.  If she didn\'t want this relationship to be monogamous, she should have told you before, and you tell her so.' );
	...
	Urta.prototype.urtaDiscussesWatchMembership = function() {
		EngineCore.clearOutput();
		this.urtaSprite();
		EngineCore.outputText( 'You tell Urta that you\'re curious about the Watch\'s membership.' );
	...
	Urta.prototype.urtaDiscussesWatchRole = function() {
		EngineCore.clearOutput();
		this.urtaSprite();
		EngineCore.outputText( 'You tell her that you\'re curious about what it is that the Watch actually does, on a day to day basis.' );
	...
	Urta.prototype.urtaDiscussesTheWatchsReach = function() {
		EngineCore.clearOutput();
		this.urtaSprite();
		EngineCore.outputText( 'You ask Urta just how far the Watch\'s authority spans from the city.' );
	...
	Urta.prototype.urtaDiscussesCrimeAndPunishment = function() {
		EngineCore.clearOutput();
		this.urtaSprite();
		EngineCore.outputText( 'You explain to Urta that you\'re curious about the law in Tel\'Adre - what is a crime here, and what are the punishments that the Watch is likely to deal to a criminal?' );
	...
	Urta.prototype.spankTheShitOutOfUrtaAndMakeHerCreamHerselfFromProstateStimulationAloneLikeTheHornyDrunkenSlutSheReallyIs = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( ImageManager.showImage( 'urta-bar-male-spank' ), false );
		EngineCore.outputText( 'You consider Urta\'s offers and, tempting as they are, decide they just aren\'t for you right now.' );
		...
		EngineCore.outputText( 'with each swat.  "<i>You\'re a rude bitch when you\'re drunk, Urta,</i>" you add, giving her another swat on the ass.  She tries to stand, but you roughly push her back against the table.  You give her a fourth spank for her insolence, and can only grin when you hear her cock slam up against the table again.' );
		...
		if( CoC.getInstance().player.cockThatFits( this.urtaCapacity() ) >= 0 ) {
			var y = CoC.getInstance().player.cockThatFits( this.urtaCapacity() ) + 1;
			...
			if( CoC.getInstance().player.cumQ() < 250 ) {
				EngineCore.outputText( 'you\'ve spent your load' );
			} else if( CoC.getInstance().player.cumQ() < 1000 ) {
				EngineCore.outputText( 'filled her utterly' );
			} else {
				EngineCore.outputText( 'streams of cum leak out of her ass, pooling at your [feet]' );
			}
			EngineCore.outputText( '.  Urta whines, desperate to cum, but you don\'t let her.  Not yet.' );
	...
	Urta.prototype.urtaDiscussessKatherine = function() {
		...
		if( affection === 0 ) {
			...
			EngineCore.outputText( '“<i>' + CoC.getInstance().player.short + ', I can honestly say I never expected to have this much sex.  I love you, I love Katherine.  It’s a bit weird, but I’m not complaining and neither is she and I hope neither are you.  I’m not the same girl you found sitting alone in the bar.  I’m so happy now.</i>”' );
		}
		EngineCore.doNext( SceneLib.camp.returnToCampUseOneHour );
	};
	Urta.prototype.urtaKathSexDont = function() {
		...
		CoC.getInstance().flags[ kFLAGS.KATHERINE_URTA_DATE ] = SceneLib.katherine.KDATE_LITTLE;
	...
	Urta.prototype.urtaKathSexWhenever = function() {
		...
		CoC.getInstance().flags[ kFLAGS.KATHERINE_URTA_DATE ] = SceneLib.katherine.KDATE_WHENEVER;
		EngineCore.doNext( this.urtaDialogueMenu );
	};
	Urta.prototype.urtaKathSexEncourage = function() {
		...
		CoC.getInstance().flags[ kFLAGS.KATHERINE_URTA_DATE ] = SceneLib.katherine.KDATE_LOTS;
		EngineCore.doNext( this.urtaDialogueMenu );
	};
	...
	Urta.prototype.giveTheFoxSomeEggs = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.PC_FETISH ] > 0 ) {
			EngineCore.outputText( 'as much fun as it might be, ' );
		}
		...
		if( CoC.getInstance().player.gender > 0 ) {
			...
			EngineCore.outputText( 'starting to grow wet with arousal, matting her back with your sexual fluids, but you ignore them, too caught up in your need of release to bother with conventional pleasures.' );
		} else {
			EngineCore.outputText( '  The entirety of your sexual world begins and ends with your ovipositor, with the sensation of the mock-cock burrowing inexorably towards Urta\'s womb overwhelming you with pleasure.  Having no other sexual organs to distract you allows you to fully embrace it; you thrust harder and harder, desperate to ensure you have reached the womb before you start to lay.' );
		}
		...
		if( CoC.getInstance().player.findPerk( PerkLib.BeeOvipositor ) >= 0 ) {
			EngineCore.outputText( 'honey' );
		} else {
			EngineCore.outputText( 'spider-goo' );
		}
		EngineCore.outputText( ' to keep Urta moist and slick and pliable.' );
		...
		var wetness = 0;
		if( CoC.getInstance().player.hasVagina() ) {
			wetness += CoC.getInstance().player.wetness() * 100;
		}
		if( CoC.getInstance().player.hasCock() ) {
			wetness += CoC.getInstance().player.cumQ();
		}
		if( wetness < 300 ) {
			EngineCore.outputText( 'splattering' );
		} else if( wetness < 500 ) {
			EngineCore.outputText( 'matting' );
		} else if( wetness < 1000 ) {
			EngineCore.outputText( 'soaking' );
		} else {
			EngineCore.outputText( 'flooding' );
		}
		EngineCore.outputText( ' the fox with your gushing ' );
		if( CoC.getInstance().player.gender === 1 ) {
		...
		if( CoC.getInstance().player.gender > 0 ) {
			EngineCore.outputText( ' just as her back is from your own release' );
		}
		EngineCore.outputText( '.' );
		//[Egg Level 1:;
		if( CoC.getInstance().player.eggs() < 20 ) {
			EngineCore.outputText( '  Urta\'s stomach starts to swell as your clutch of eggs fills her once-empty womb, stretching into a noticeable bulge, though not one that she couldn\'t hide if she wanted.' );
		}//[Egg Level 2:;
		else if( CoC.getInstance().player.eggs() < 40 ) {
			EngineCore.outputText( '  As the eggs keep on coming, Urta\'s womb swells and bulges, crammed full of goo and eggs, growing larger and larger until Urta could easily pass for an expectant mother, ready to give birth any day now.' );
		}//[Egg Level 3:;
		else {
			EngineCore.outputText( '  You stuff Urta with eggs until you\'re wondering how many either of you can hold. Her belly just keeps getting rounder and rounder even as your eggs keep coming; soon, not only does she look like a woman pregnant with multiples, her skin is visibly stretched over the eggs, giving it a distinctively lumpy look rather than the round, smooth bulge she might have if your load was smaller.' );
		}
		...
		CoC.getInstance().flags[ kFLAGS.URTA_EGGS ] = CoC.getInstance().player.eggs();
		CoC.getInstance().flags[ kFLAGS.URTA_FERTILE_EGGS ] = CoC.getInstance().player.fertilizedEggs();
		CoC.getInstance().flags[ kFLAGS.URTA_TIMES_EGG_PREGGED ]++;
		if( CoC.getInstance().player.findPerk( PerkLib.BeeOvipositor ) >= 0 ) {
		...
		CoC.getInstance().flags[ kFLAGS.URTA_FLATBELLY_NOTICE ] = 0;
		//First time, tag for triggering freakout!;
		if( CoC.getInstance().flags[ kFLAGS.URTA_EGG_FORCE_EVENT ] === 0 ) {
			CoC.getInstance().flags[ kFLAGS.URTA_EGG_FORCE_EVENT ] = 48;
		}
	...
	Urta.prototype.urtaChewsOutPC = function( newScreen ) {
		...
		EngineCore.outputText( 'Urta looks up at you and snorts fiercely; you almost expect steam to come flaring out of her nostrils, and her livid eyes are doing their best to burn holes straight through you.  "<i>There you are, [name]!  You have some nerve!</i>"' );
		...
		if( CoC.getInstance().player.cor > 50 ) {
			EngineCore.outputText( ', especially given this may be the closest she\'ll ever get to experiencing pregnancy' );
		}
		EngineCore.outputText( '?' );
		...
		//[Unfertilised Eggs:;
		if( CoC.getInstance().flags[ kFLAGS.URTA_FERTILE_EGGS ] === 0 ) {
			...
			if( CoC.getInstance().player.lib > 49 && CoC.getInstance().player.hasVagina() ) {
				EngineCore.outputText( '  Raising an eyebrow, you lean closer and inform her that you could make babies for her - she just needs to find you a big cock dripping with, and this is the important part, <i>fertile</i> seed.' );
				EngineCore.outputText( '\n\nUrta shifts in her seat.  "<i>Er, well.  I, ah... I\'ll keep... an eye out for one.</i>"' );
			}
		}
		//[Giant Bee:;
		else if( CoC.getInstance().player.findPerk( PerkLib.BeeOvipositor ) >= 0 ) {
		...
		if( CoC.getInstance().player.findPerk( PerkLib.SpiderOvipositor ) >= 0 ) {
			EngineCore.outputText( 'spider' );
		} else {
			EngineCore.outputText( 'little bee' );
		}
		EngineCore.outputText( '...</i>"\n' );
		//(Technically finished, now, but waiting Fen's okay before adding repeatable version);
		CoC.getInstance().flags[ kFLAGS.URTA_EGG_FORCE_EVENT ] = -1;
		if( newScreen ) {
			EngineCore.doNext( SceneLib.camp.returnToCampUseOneHour );
		}
	};
	...
	Urta.prototype.repeatUrtaEgging = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( ImageManager.showImage( 'urta-egg-repeat' ), false );
		EngineCore.outputText( 'With a smile, you ask if Urta would be willing to let you lay a few eggs in her womb again?' );
		...
		if( !this.urtaLove() ) {
			EngineCore.outputText( 'blatantly hanging out of her stockings and drooling pre-cum everywhere' );
		} else {
			EngineCore.outputText( 'vulgarly bulging her dress and rendering it sopping wet from her arousal' );
		}
		EngineCore.outputText( ', before sashaying in a drunkenly exaggerated fashion out towards the alleyway where you and she first began your relationship- and where you first \'impreggnated\' her.' );
		...
		if( CoC.getInstance().player.gender > 0 ) {
			...
			EngineCore.outputText( ' are starting to grow wet with arousal, matting her back with your sexual fluids, but you ignore them, too caught up in your need of release to bother with conventional pleasures.' );
		} else {
	...
	Urta.prototype.condomlessUrtaInHouseSmex = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'You grab hold of the pre-filled bubble atop the fox\'s tip and yank, earning a moan as the latex sheath slides free, stretching it to an oblong mass before freeing her flare with an audible pop.  At the same time, you draw the second, unused condom from her unresisting grip and say, "<i>Why don\'t we make a little mess?</i>"' );
		//non lover:;
		if( !this.urtaLove() ) {
		...
		if( CoC.getInstance().player.gender > 0 && CoC.getInstance().player.isNaga() ) {
	...
	Urta.prototype.pegUrtasAssMessy = function() {
		...
		EngineCore.outputText( Descriptors.cockDescript( x ) + ' down onto the sheets, a scant few inches from Urta\'s puckered asshole.  The fox ' );
		if( this.urtaDrunk() ) {
			EngineCore.outputText( 'giggles drunkenly, "<i>You\'re sho dirty!</i>" while spreading those fishnet-clad legs further.' );
		} else if( CoC.getInstance().flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] < 11 ) {
			EngineCore.outputText( 'softly begs, "<i>Be gentle, please,</i>" while spreading those fishnet-clad legs a little further apart.' );
		} else {
			EngineCore.outputText( 'eagerly coos, "<i>Feeling a little dirty, eh, lover?  Go on baby, you can fuck my ass anytime.</i>"  She spreads her fishnet-clad legs so wide they almost form an unbroken, horizontal line from padded toe to padded toe, tying her soft, poofy tail around a leg to keep it out of the way.' );
		}
		EngineCore.outputText( '  Scooting forward, you edge your ' + CoC.getInstance().player.cockHead( x ) + ' up to her anus.  Her body heat washes over you as you enjoy the feel of her pucker\'s folded flesh pressing down on your most sensitive point, lubricated by the copious female fluid that drains from the sodden gash above.' );
		EngineCore.outputText( '\n\nYou begin to gently rock your [hips] forward, giving little, gentle pushes against the resistant muscle.  At first, you make no progress, simply forcing her anal ring to dilate, little by little.  Then, after a particularly vigorous stroke, your ' + CoC.getInstance().player.cockHead( x ) + ' slides through the clinging ring and into the tight embrace of her slick anus.  You stop to allow her to adjust, admiring her ' );
		if( this.urtaDrunk() ) {
			EngineCore.outputText( 'drunken ' );
		}
		EngineCore.outputText( 'whimpers of pleasure.  However, you don\'t wait long, slowly pushing forward and exploring your vixen\'s every inner crevice with your eager rod.' );
		EngineCore.outputText( '\n\n"<i>It\'s ' );
		if( this.urtaDrunk() ) {
			EngineCore.outputText( 'sho' );
		} else {
			EngineCore.outputText( 'so' );
		}
		EngineCore.outputText( ' different than with a condom over a toy,</i>" the vulpine lass wonders aloud in delight.  She licks her lips and ' );
		if( CoC.getInstance().flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] > 15 ) {
			EngineCore.outputText( 'casually' );
		} else if( CoC.getInstance().flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] > 11 ) {
			EngineCore.outputText( 'confidently' );
		} else if( CoC.getInstance().flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] > 4 ) {
			EngineCore.outputText( 'a little nervously' );
		} else {
			EngineCore.outputText( 'with a shaky hand' );
		}
		EngineCore.outputText( ' strokes a padded fingertip down the bulging length of her urethra, shivering when it bumps across the sensitive medial ring.  A few inches in, you feel a rounded bulge bump into you, and as soon as you press against it, Urta\'s cock lifts up and thickens, flooded with blood.  The fox trembles for a second as you glide by the obstruction, and without any warning at all, her shaft belches a thick spurt of clear pre-cum with enough force to glaze the underside of her tits.' );
		...
		if( CoC.getInstance().player.cockArea( x ) > this.urtaCapacity() ) {
			EngineCore.outputText( 'The aching pressure around your ' + Descriptors.cockDescript( x ) + ' doubles, and you realize she simply can\'t take any more.  Her anus is stretched wide, dilated to the extreme to accommodate this much of you.  There\'s little to do but plow her with what you can until she\'s fountaining white.' );
		} else if( CoC.getInstance().player.hasKnot( x ) ) {
			EngineCore.outputText( 'The pleasure seems to double when your knot butts up against her furry rump, but her asshole isn\'t ready for that just yet.  This bitch needs a little more loving before you knot her.' );
		} else {
			EngineCore.outputText( 'The aching pleasure around your ' + Descriptors.cockDescript( x ) + ' redoubles as you hilt her, smashing your ' );
			if( !CoC.getInstance().player.hasSheath() ) {
				EngineCore.outputText( 'groin ' );
			} else {
				EngineCore.outputText( 'sheath ' );
			}
			EngineCore.outputText( 'against her taut asshole with a heavy lurch.' );
		}
		...
		if( CoC.getInstance().player.hasKnot( x ) && CoC.getInstance().player.cockArea( x ) < this.urtaCapacity() ) {
			EngineCore.outputText( '  You barely notice when your knot pops through the ebony ring, locking you inside her.  All you know is that while your strokes have gotten shorter, the pleasure has gotten even greater.' );
		}
		EngineCore.outputText( '\n\nYou pinch the nipple you\'ve been playing with and slam your ' + Descriptors.cockDescript( x ) + ' home with rump-shaking force.  It slides the fox backward in her puddle of pre, slicking the underside of her legs with her sexual moisture as she\'s dragged through it.  Barely suppressing a moan of your own, you shimmy forward again and voraciously tackle her asshole, pegging Urta with short, brutal strokes.  Eager to see her blow, you shift your angle and press up, slamming your ' + CoC.getInstance().player.cockHead( x ) + ' into her prostate so hard you swear you must be flattening it.  Urta screams, her balls contracting powerfully.  Her dick, already iron-hard, seems to swell thicker, looking like a flat-tipped, over-inflated sausage.  Bulging obscenely, the underside of her cock swells with lusty intent, the distention moving up with muscular contractions until the vulpine girl\'s cum-slit is wide and filled with white.' );
		EngineCore.outputText( '\n\nThe onyx asshole wringes you with every ounce of force its owner can apply, convulsing into a tight cock-ring that' );
		if( CoC.getInstance().player.hasKnot( x ) && CoC.getInstance().player.cockArea( x ) < this.urtaCapacity() ) {
			EngineCore.outputText( ' only makes your knot bigger and harder.' );
		} else {
			EngineCore.outputText( ' works you so hard an irrational part of your mind worries your manhood will be pinched off.' );
			if( CoC.getInstance().player.cockTotal() > 1 ) {
				EngineCore.outputText( '  Good thing you have more.' );
			}
		}
		EngineCore.outputText( '  Then, the fox stops being a fox and turns into a fountain, spewing a wave of white up over her head and into the wall.  It hits with a wet \'splat\', flinging alabaster globs in every direction that burst like little bombs.  Spunk rains down over Urta, staining her bed and her in equal measure.  A lock of gunked-up hair flops across her face, temporarily blinding your orgasm-inundated lover.  Just when the dripping jizz slows and you think the worst is over, Urta goes off again, though this fountain of white has a little less force than the preceeding cumsplosion.  A wave of semen as thick as your wrist spurts out to take the fox in the face, plastering the rest of her lustrous hair to her forehead and glazing her visage with ivory lust.  Ringlets of her ' );
		if( CoC.getInstance().flags[ kFLAGS.URTA_QUEST_STATUS ] < 0 ) {
			EngineCore.outputText( 'not-so-fertile ' );
		}
		EngineCore.outputText( 'fluid dangle precariously from her jawline as she tries to breathe, coughing and making bubbles from her white mask.' );
		...
		if( this.urtaDrunk() ) {
			EngineCore.outputText( ' and of course, alcohol' );
		}
		EngineCore.outputText( ', kissing your ' + CoC.getInstance().player.cockHead( x ) + ' every time it peeks past her tits, relishing the taste of her own lust all over it.' );
		...
		if( CoC.getInstance().player.cumQ() < 250 ) {
			EngineCore.outputText( ', swallowing with a smile and a wink.' );
		} else {
			EngineCore.outputText( ', but she\'s forced to close it when her cheeks are puffed beyond reason.' );
			if( CoC.getInstance().player.cumQ() > 750 ) {
				EngineCore.outputText( '  You go on to wash her face with your own white sauce.' );
			}
			if( CoC.getInstance().player.cumQ() >= 1200 ) {
				EngineCore.outputText( '  Your orgasm goes on and on, coating just about everything that Urta came on with yet another layer.' );
			}
			if( CoC.getInstance().player.cumQ() > 3000 ) {
				EngineCore.outputText( '  She gasps in surprise when you flood the ' );
				if( CoC.getInstance().player.cumQ() > 10000 ) {
					EngineCore.outputText( 'first few feet of the ' );
				}
				EngineCore.outputText( 'room, looking up at you in admiration through cum-coated eyelashes.' );
			}
		}
		EngineCore.outputText( '\n\nAs you finish emptying your [balls], you sag down into the fox\'s arms.  She plants a salty kiss on your lips ' );
		if( this.urtaLove() ) {
			EngineCore.outputText( 'and nuzzles against your cheek, just holding you for a few moments, lover to lover.' );
		} else {
			EngineCore.outputText( 'and sheepishly pulls back.' );
		}
		EngineCore.outputText( '  Climbing out of the alabaster-painted bed is no easy task, but the two of you manage, laughing and leaning on each other\'s arms.  Urta leads you to a cabinet in another room, and bends over to open it.  You barely notice her pulling out mops, buckets, and cleansers.  Instead, your vision is riveted to her asshole, still gaped lewdly in the shape of your ' + Descriptors.cockDescript( x ) + '.  When she comes up with an armload of cleaning supplies, you\'re ' );
		if( CoC.getInstance().player.cor < 50 ) {
			EngineCore.outputText( 'blushing' );
		} else {
			EngineCore.outputText( 'smiling devilishly' );
		}
		EngineCore.outputText( '.' );
	...
	Urta.prototype.UrtaGetsPeggedCleanUp = function() {
		EngineCore.clearOutput();
		if( CoC.getInstance().player.cor >= 50 ) {
			EngineCore.outputText( 'You start to duck out, but the disapproving glare the fox gives you makes it clear that you won\'t be getting any more messy fun unless you stick around to clean it up this time.  Damnit.  ' );
		}
		EngineCore.outputText( 'With Urta\'s help, you slowly clean up the mess you two made, nearly clogging the plumbing a few times in the process.  New sheets, pillows, and furs are brought in, and you help her fix her bed.  The floor is mopped, the walls are scrubbed, and the shutters are opened to let in some fresh air.  Thankfully, just about everything is watertight and sealed, likely something she had done to ease situations such as this.  All told, between clean-up and sex, you spend a few hours with her.' );
	...
	Urta.prototype.fillMeUpPleaseUrta = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( ImageManager.showImage( 'urta-home-female-fucked' ), false );
		EngineCore.outputText( 'Lazily discarding the latex, you caress Urta\'s stocking-clad leg and coyly climb into bed.  She starts to reciprocate your affections, but you push her arm away, instructing, "<i>Let me... let me have this.</i>"' );
		...
		if( CoC.getInstance().player.wetness() > 3 ) {
			EngineCore.outputText( '  You\'re so utterly soaked thanks to this place that a steady stream of vaginal lubricant drizzles over the meaty tool below.' );
		}
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( '  Hard and erect, [eachCock] stands tall and proud, but ignored.  You have other plans.' );
		}
		EngineCore.outputText( '  She\'s ready.  You\'re ready.  It\'s time.' );
		...
		if( CoC.getInstance().player.wetness() > 3 ) {
			EngineCore.outputText( ', though it\'s a drop in the bucket next to your copious moisture.  Hell, even now, rivers of your girl-goo are sliding down that spasming length, wreathing it in lady-spunk' );
		} else {
			EngineCore.outputText( ', deliciously staining your slit' );
		}
		EngineCore.outputText( '.' );
		//{Virgin PCs};
		if( CoC.getInstance().player.vaginas[ 0 ].virgin ) {
			...
			EngineCore.outputText( ' as she cranes her neck to meet you, eye to eye.  "<i>Really?</i>"  You nod, biting your lower lip as you continue your frenzied undulations upon her rod, working yourself up to taking her.' );
			...
			EngineCore.outputText( 'asks, "<i>Are you s' );
			if( this.urtaDrunk() ) {
				EngineCore.outputText( 'h' );
			}
			EngineCore.outputText( 'ure?  You don\'t have to...</i>"' );
			...
			if( CoC.getInstance().player.hasCock() ) {
				EngineCore.outputText( '  Drool begins to leak from [eachCock], slicking the underside with pre-cum as the pleasure affects your maleness.' );
			}
			EngineCore.outputText( '  Before long, you\'ve taken Urta halfway, with her ring jutting up against your entrance.' );
			//{cuntChange 30};
		...
		if( CoC.getInstance().flags[ kFLAGS.PC_FETISH ] > 0 ) {
			EngineCore.outputText( '  An exhibitionist thrill runs through you at that, making you even wetter.' );
		}
		...
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( '  You don\'t even notice [eachCock] dripping on Urta\'s belly.' );
		}
		//HERMS:;
		if( CoC.getInstance().player.hasCock() ) {
			...
			if( CoC.getInstance().player.cockTotal() > 1 ) {
				EngineCore.outputText( ', followed in short order by the untouched portion of your ' + Descriptors.multiCockDescriptLight() );
			}
			EngineCore.outputText( '.  The next is your [vagina] going wild, convulsing from your entrance towards your womb with muscular contractions that milk Urta\'s cock relentlessly, causing it to grow bigger and harder inside you.  Urta whimpers slightly as you jizz over her belly' );
			if( CoC.getInstance().player.cumQ() > 400 ) {
				EngineCore.outputText( ', soaking her' );
				if( CoC.getInstance().player.cumQ() > 1000 ) {
					if( CoC.getInstance().player.cumQ() < 2000 ) {
						EngineCore.outputText( ' and her bed' );
					} else {
						EngineCore.outputText( ', her bed, and damn near everything else' );
					}
				}
				EngineCore.outputText( ' with your ejaculate' );
			}
			EngineCore.outputText( '.  There\'s a bloom of warmth from below as the vixen\'s orgasm rushes towards you, and all you can do is cum your brains out in anticipation of being filled.' );
		}
		//NON-HERMS:;
		else {
		...
		if( CoC.getInstance().player.biggestTitSize() >= 4 ) {
			EngineCore.outputText( ' and your [chest] rest heavily upon your cum-stuffed form' );
		}
		EngineCore.outputText( '.' );
		if( CoC.getInstance().player.pregnancyIncubation > 0 && CoC.getInstance().player.pregnancyIncubation < 150 ) {
			EngineCore.outputText( '  You may have been pregnant before, but you\'re beyond pregnant now... with cum AND child.' );
		}
		//{Goo:;
		if( CoC.getInstance().player.isGoo() && CoC.getInstance().player.vaginalCapacity() > 9000 ) {
			EngineCore.outputText( '  Successive eruptions make you bigger, but your flexible cunt soon grows so bloated that the onrushing injections mostly squirt back out, painting Urta, the bed, and your [legs] white with cum.' );
		} else {
			EngineCore.outputText( '  Successive eruptions do little to bloat you further - your body is stretched to capacity, so the onrushing injections only squirt back out through your entrance, painting Urta, the bed, and your [legs] white with cum.' );
		}
		...
		if( CoC.getInstance().player.pregnancyIncubation > 0 && CoC.getInstance().player.pregnancyIncubation < 150 ) {
			EngineCore.outputText( 'even more ' );
		}
		EngineCore.outputText( 'pregnant for a while, at least until you can pass all that filling!  She plants a salty kiss on your lips ' );
		if( this.urtaLove() ) {
			EngineCore.outputText( 'and nuzzles against your cheek, just holding you for a few moments, lover to lover.' );
		} else {
			EngineCore.outputText( 'and sheepishly pulls back.' );
		...
		if( CoC.getInstance().player.cor < 33 ) {
			...
			EngineCore.outputText( 'grinning' );
		} else {
			EngineCore.outputText( 'openly leering' );
		}
		EngineCore.outputText( '.' );
		...
		CoC.getInstance().player.slimeFeed();
		if( CoC.getInstance().flags[ kFLAGS.URTA_FERTILE ] === 1 ) {
			CoC.getInstance().player.knockUp( PregnancyStore.PREGNANCY_URTA, PregnancyStore.INCUBATION_URTA, 25 );
		}
		CoC.getInstance().player.orgasm();
		EngineCore.dynStats( 'lib', 0.2, 'sen', -3 );
	};
	...
	Urta.prototype.lippleBondUrtaScylla = function() {
		...
		//[if (cocks > 1);
		if( CoC.getInstance().player.cockTotal() > 1 ) {
			EngineCore.outputText( '  Without much warning, you suddenly feel Urta\'s hand move away from your own palm, the sensation of your [cock smallest] being wrapped by her fingers also assaulting you as she squeezes and tugs at your second cock affectionately.' );
		}
		//[if (cocks > 2);
		if( CoC.getInstance().player.cockTotal() > 2 ) {
			EngineCore.outputText( '  The barrage of sensations is intensified when Scylla also gets a bit greedy, grabbing another tool and starting to jerk it off as well.' );
		}
		...
		//[if (cocks > 1);
		if( CoC.getInstance().player.cockTotal() > 1 ) {
			EngineCore.outputText( ' either' );
		}
		EngineCore.outputText( ' the holy woman\'s ravenous lipples and inner, erotic tit-flesh in a mutually ecstatic, sensations-filled manner' );
		if( CoC.getInstance().player.cockTotal() > 1 ) {
			EngineCore.outputText( ' or more delicately and less hungrily with a feminine hand' );
		}
		EngineCore.outputText( '.' );
		...
		//if (sensitivity < 29);
		if( CoC.getInstance().player.sens < 29 ) {
			EngineCore.outputText( '  You can only tell so from her expression and from Scylla moaning and gasping sharply, but it appears Urta\'s cock is really just about ready to shoot off cum.' );
		}
		//if (sensitivity > 29);
		else {
			EngineCore.outputText( '  Scylla\'s titflesh slightly shifts from what is apparently the expansion of Urta\'s cock flare and the twitching of her massive shaft.  You\'re sensitive enough to notice.' );
		}
		//[if (sensitivity > 44);
		if( CoC.getInstance().player.sens >= 44 ) {
			EngineCore.outputText( '  In fact, you can clearly feel the pressure inside Scylla\'s chest increasing as Urta\'s huge cock expands and twitches inside her next to your own, each such twitch sending a shiver of pleasure through both you and the nun.' );
		}
		...
		//[if (sensitivity < 59)];
		if( CoC.getInstance().player.sens < 59 ) {
		...
		if( CoC.getInstance().player.hasKnot( CoC.getInstance().player.biggestCockIndex() ) ) {
			EngineCore.outputText( '  You\'re not making the problem any easier, in fact, your knot is halfway stuck inside her breast, surged and filled to full size right after your orgasm.' );
		}
		...
		if( CoC.getInstance().player.hasKnot( CoC.getInstance().player.biggestCockIndex() ) ) {
			EngineCore.outputText( ', your knot sufficiently deflated now' );
		}
		EngineCore.outputText( ', now sucking desperately at the air, looking for the tasty dick.' );
		...
		if( CoC.getInstance().player.cumQ() < 20000 ) {
			CoC.getInstance().player.cumMultiplier += 2;
		}
		EngineCore.doNext( SceneLib.camp.returnToCampUseOneHour );
	};
	...
	Urta.prototype.nagaOnUrtaMess = function() {
		...
		//[if (cocks = 0);
		if( CoC.getInstance().player.cockTotal() === 0 ) {
			EngineCore.outputText( '  Urta runs her fingers over and gently pushes into your now revealed female slit. "<i>My lover\'s sweet, sweet pussy. The best in the whole world.</i>"' );
		}//if (cocks = 1);
		else if( CoC.getInstance().player.cockTotal() === 1 ) {
			EngineCore.outputText( '  "<i>And here is my favorite cock in the whole world... Probably including mine,</i>" she giggles and winks at you.' );
		}//if (cocks = 2);
		else if( CoC.getInstance().player.cockTotal() === 2 ) {
			EngineCore.outputText( '  "<i>Two delicious cocks, rather than just one...  You\'re trying to tell me you\'re the man here, aren\'t you, lover?</i>" she giggles and winks at you.' );
		} else if( CoC.getInstance().player.cockTotal() === 3 ) {
			EngineCore.outputText( '  "<i>You want to have a spare one just in case or something?</i>" she giggles and winks at you.' );
		}//[if (cocks >= 4)];
		else {
			EngineCore.outputText( '  "<i>Interesting.  It looks like a bouquet.  A beautiful bouquet of cocks just for me,</i>" she giggles and winks at you.' );
		}

		EngineCore.outputText( '  Without further ado, Urta leans towards your sex to return the favor, her mouth pleasuring you just as you wrap more and more of your slightly looser coils around her member, having released her legs to allow her to move... and to allow your tail\'s tip to reach her needy vulpine pussy more easily from that position.' );
		...
		//EngineCore.outputText('  However, it requires no justification that your herm lover is further on her way to an orgasm then you are.');;
		...
		if( CoC.getInstance().player.hasVagina() ) {
			...
			if( CoC.getInstance().player.wetness() >= 4 ) {
				EngineCore.outputText( '  After a few convulsions, your femcum starts to shoot out of your pussy quite violently, adding to the mess your lover is making.' );
			}
		}
		//if (cocks > 0);
		if( CoC.getInstance().player.cockTotal() > 0 ) {
			...
			//[if (cumHigh = true)];
			if( CoC.getInstance().player.cumQ() >= 1000 && CoC.getInstance().player.cumQ() < 2500 ) {
				EngineCore.outputText( '  Her eagerness does end up giving her a swollen tummy, but she\'s loving and hungry for you until the end, swallowing as much as she can, as quickly as she can.' );
			}//(if [cumExtreme = True];
			else if( CoC.getInstance().player.cumQ() >= 2500 ) {
				EngineCore.outputText( '  Of course, your excessive load proves too much for the poor vixen to handle, and soon she is not only inflated, but painted in your cum as well, the mess the two of you are making definitely nothing to be scoffed at.' );
			}
		}
	...
	Urta.prototype.lickOutUrtaAtHome = function() {
		...
		if( CoC.getInstance().player.cockArea( x ) >= this.urtaCapacity() / 2 ) {
			EngineCore.outputText( 'It fits you like a lubricated, stretchy little glove, ensuring every movement will caress her folds.' );
		} else {
			EngineCore.outputText( 'It\'s nice and roomy, allowing you full control over how you want to press on her inner folds.' );
		}
		EngineCore.outputText( '  You push the rest of the way into her cunt and admire the velvety texture of her walls as they close in around you, inner muscles flexing and massaging your ' + Descriptors.cockDescript( x ) + ' as she adjusts to your intrusion.' );
		if( CoC.getInstance().player.cockArea( x ) >= 200 ) {
			EngineCore.outputText( '   Stretching ludicrously, her cunt dilates with surprising ease.  The elastic lips of her pussy cling oh-so-tightly to your girth as they\'re spread beyond reason.  Her hips even spread with only a modest bit of pressure and resistance as her passage is distorted, the shape of your ' + Descriptors.cockDescript( x ) + ' visible through her stretched belly as it pushes towards her tits.  You can even make out some of the larger veins.' );
		}
		...
		if( CoC.getInstance().player.cor < 66 ) {
			EngineCore.outputText( 'You\'re wonderful' );
		} else {
			EngineCore.outputText( 'You\'re the most amazing fuck' );
		}
		EngineCore.outputText( '</i>"' );
		...
		if( CoC.getInstance().player.cumQ() >= 500 ) {
			EngineCore.outputText( '  Spermy juices run out of her pussy in alabaster streams thick enough to obscure the black-lips they\'re travelling over.' );
		}
		if( CoC.getInstance().player.cumQ() >= 1500 ) {
			EngineCore.outputText( '  The fox gives a surprised grin when her belly rounds slightly, though she seems concerned at how it makes her cum-filled prize wobble.' );
		}
		if( CoC.getInstance().player.cumQ() >= 3000 ) {
			EngineCore.outputText( '  Your virile pecker just won\'t stop, and by the time you finish ejaculating, Urta is pregnant with sperm, her belly round with its belly-button protruding.' );
		}
		EngineCore.outputText( '\n\nSharing an intimate moment with your ' );
		if( this.urtaDrunk() ) {
			EngineCore.outputText( 'drunken ' );
		}
		EngineCore.outputText( 'lover, you kiss her again.  She returns it demurely, but midway through, you feel something slick and hard brushing up against you.  Urta whispers, "<i>Sorry,</i>" when you pull away to examine the sudden change.  Her pole is hardening again, surging up to rub the full prophylactic against your [chest].  You glance back at her, and all Urta has to say for herself is, "<i>Sorry, lover... feeling you cum inside me is wonderful, and well, I had time to recover.</i>"  She slowly pulls the rubber off her cock, careful not to jostle it too hard.  A second later, it\'s tied off and set aside.  With you still ' );
		if( CoC.getInstance().player.hasKnot( x ) ) {
			EngineCore.outputText( 'knotted' );
		} else {
			EngineCore.outputText( 'resting' );
		}
		EngineCore.outputText( ' inside her juicy cunt, she asks if she needs another one, batting her eyelashes playfully.' );
		EngineCore.outputText( '\n\nShaking your head from side to side, you pull back' );
		if( CoC.getInstance().player.hasKnot( x ) ) {
			EngineCore.outputText( ' in an attempt to withdraw.  Her entrance dilates slowly as your full, dog-like knot is pressed against her from inside, the pulsating, reddish flesh glistening with the mixed sexual lubricants.  You tug and tug, rocking your [hips] slowly to loosen her up, and eventually, you manage to pop out, drawing a moan of mixed anguish and pleasure from Urta\'s maw.' );
		} else {
			EngineCore.outputText( ' and withdraw.' );
		}
		if( CoC.getInstance().player.cumQ() >= 1500 ) {
			EngineCore.outputText( '   Cum runs out of her packed womb in a wave, spreading all over the sheets down to her knees.  She shakes slightly as the cum runs out of her, rubbing her full middle protectively.' );
		} else {
			EngineCore.outputText( '  Cum drools out of her still-gaping slit in steady trickles, puddling on the sheets below.  She groans happily.  Clearly the fox is proud to have such a productive lover.' );
		}
		EngineCore.outputText( '  You take hold of the puffy, dick-stretched pussy lips and spread them around two fingers, collecting some of the warm sex-juice on your fingertips, then, you pull the dripping, creamy treat out and smear it over Urta\'s mouth.  She giggles as you apply the lip gloss to her and slowly runs her tongue across the shining coat of sex.  You respond by repeating the motion, this time laying it on thicker.' );
		...
		CoC.getInstance().flags[ kFLAGS.URTA_TIME_SINCE_LAST_CAME ] += 12;
		...
		if( CoC.getInstance().player.lib > 90 ) {
			EngineCore.dynStats( 'lib', -1 );
		}
		EngineCore.doNext( SceneLib.camp.returnToCampUseOneHour );
	};
	Urta.prototype.slurpFawkesCocksForFunAndInflation = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( ImageManager.showImage( 'urta-home-bj' ), false );
		EngineCore.outputText( '"<i>Thanks, Lover! I need this. I\'ve been so busy, I haven\'t had any relief in days!</i>" Urta informs you as she lightly strokes herself to full erectness. It\'s a short trip, and in a few pumps, she\'s ready to go. You glance around the bar, making sure all possible prying eyes are occupied. Feeling satisfied with your findings, you slyly shuffle your way under the table.' );
		...
		if( CoC.getInstance().player.pregnancyIncubation < 100 && CoC.getInstance().player.pregnancyIncubation > 0 ) {
			EngineCore.outputText( ' than it already was' );
		}
		EngineCore.outputText( '.  You slide your way back into your seat with some difficulty, fighting the wriggling, bouncing mass that is your stomach over the now shrinking room between the seat and table.  By now, some of the patrons have realized what has happened, pointing, whispering, rubbing their stomachs, and pulling their hands away from each other to symbolize your new... asset.' );
		...
		EngineCore.outputText( '\n\nThe waitress approaches as Urta releases you, glancing around the bar at the commotion, "<i>Quite a scene ya caused, ' + CoC.getInstance().player.mf( 'cutey', 'girly' ) + ',</i>" she looks at your stomach, "<i>Would you like anything to, uh... wash that down with?</i>"' );
	...
	Urta.prototype.urtaAndRaphaelSurprise = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'Letting your lips split into a mischievious grin, you lean across the table and whisper into Urta\'s fluffy ear.  "<i>Why don\'t you meet me in one of the back rooms with the lights off in about thirty minutes?  I\'ll see if I can get a friend, we can make a party of it.</i>"' );
		...
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( ' and [eachCock] experience a twinge of excitement' );
		}
		EngineCore.outputText( '.  Grabbing a napkin and a pen from an accommodating waitress, you jot out a hasty missive:' );
		EngineCore.outputText( '\n\n<i>Dearest Rogue,' );
	...
	Urta.prototype.raphaelAndUrtaPartTwo = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( ImageManager.showImage( 'urta-raphael-3some', 'vert' ), false );
		EngineCore.outputText( 'Narrow, familiar paws suddenly encircle your waist, and a puff of hot breath exhales past your ear.  "<i>Greetings, my supple swan.</i>"' );
		...
		EngineCore.outputText( '\n\nStronger, more muscular arms wrap around you and Raphael, firmly compressing your bodies together.  His dagger-like erection narrowly misses your [vagina], instead wetly grinding across your lips and [clit], pressing on your belly where it can leak a trickle of liquid need onto your ' + CoC.getInstance().player.skinFurScales() + '.  A slightly slurred voice demurely coos, "<i>Your friend has a nice butt!</i>" as a slimy, slippery horsecock is pressed against your back' );
		if( CoC.getInstance().player.tallness >= 60 ) {
			EngineCore.outputText( ', grinding through your [butt]\'s ass-cleavage without an ounce of hesitation' );
		}
		EngineCore.outputText( '.' );
		...
		if( CoC.getInstance().player.tailType > AppearanceDefs.TAIL_TYPE_NONE ) {
			EngineCore.outputText( 'lift your tails and ' );
		}
		EngineCore.outputText( 'try to relax with her thick slab of penis so close to your backdoor, hoping she\'ll be gentle.  A drunken twinge of arousal knifes through your [vagina] as you imagine her forcefully raping your hind-end, and Raphael can certainly feel the convulsive fluttering of your love tunnel as you sink into your imaginings.  The gray vixen nips at your ear as she presses against your [asshole]' );
		if( CoC.getInstance().player.analCapacity() < 30 ) {
			EngineCore.outputText( ', slowly stretching your tight sphincter with constant pressure.  You\'re not really loose enough back there for this to be entirely comfortable, but the incredible fullness her length provides has a way of muting the pain of stretching.' );
		} else if( CoC.getInstance().player.analCapacity() < 60 ) {
			EngineCore.outputText( ', slowly sliding into your sphincter.  You groan into your other lover\'s mouth as your anus is claimed by the thick horse-pole, and you do your best to relax around the butthole-stretching shaft.' );
		} else {
			EngineCore.outputText( ', easily slipping inside your well-practiced anus.  You groan into your other lover\'s mouth as the thick cock claims your ready asshole, and you\'re glad your butthole was so well prepared to handle having a hot meat pole injected into it.' );
		}
		CoC.getInstance().player.buttChange( 60, true, true, false );
		...
		CoC.getInstance().flags[ kFLAGS.URTA_X_RAPHAEL_HAPPENED ]++;
		EngineCore.doNext( SceneLib.camp.returnToCampUseOneHour );
	};
	...
	Urta.prototype.urtaGooTesticleVoreRuinedOrgasms = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( ImageManager.showImage( 'urta-goo-fuck' ), false );
		EngineCore.outputText( 'You tell your vixen lover that you\'ve got something a bit... special in mind this time.  You surge up onto the bed and gently flip Urta onto her back.  Her horsecock bobs enticingly in front of you, already as hard as ' );
		if( !EngineCore.silly() ) {
			EngineCore.outputText( 'a steel rod' );
		} else {
			EngineCore.outputText( 'Marethian algebra' );
		}
		EngineCore.outputText( '.' );
	...
	Urta.prototype.urtasRuinedOrgasmsFromGooPartII = function() {
		...
		EngineCore.outputText( 'You awaken the next morning to a panicked shout.  You bolt upright...  Or at least you try to.  Something is holding you tight, binding you in place.  You try to open your eyes, but nothing but blackness surrounds you.  Struggling, you try and wrench yourself free of your bindings.' );
		...
		if( edrynAvailable ) {
			EngineCore.outputText( 'set of hoofsteps' );
		} else {
			EngineCore.outputText( 'pair of footsteps' );
		}
		EngineCore.outputText( ' enter whatever room you\'re in. "<i>Hey, Urta, you haven\'t shown up for your shift today so...  Great googily moogily!  What happened to you?!</i>" a voice you ' );
		if( edrynAvailable ) {
			EngineCore.outputText( 'recognize as Edryn\'s' );
		} else {
			if( CoC.getInstance().flags[ kFLAGS.MET_OLIVIA ] === 0 ) {
				EngineCore.outputText( 'recognize as Olivia\'s' );
			} else {
				EngineCore.outputText( 'don\'t recognize' );
			}
		}
		EngineCore.outputText( ' shouts.' );
		...
		if( edrynAvailable ) {
			...
			EngineCore.outputText( 'Olivia' );
		}
		EngineCore.outputText( ', I\'m going to need your help with this particular problem.</i>"' );
		EngineCore.outputText( '\n\nThere\'s a moment of silence, and then, "<i>' );
		if( edrynAvailable ) {
			EngineCore.outputText( 'All right, but I\'m still charging you for this one' );
		} else {
			EngineCore.outputText( 'Uhm, yes Captain, whatever you need' );
		}
		EngineCore.outputText( '.</i>"  You hear armor falling to the ground while Urta rises from wherever she was sitting; you\'ve been all over the house so many times it\'s hard to keep track of where you are.' );
		...
		EngineCore.outputText( '\'s cunt, if that wet squelching noise is any indication.  She thrusts all the way in inside in one go, making ' );
		if( edrynAvailable ) {
			EngineCore.outputText( 'Edryn moan in pleasure' );
		} else {
			EngineCore.outputText( 'Olivia shout with surprise' );
		}
		EngineCore.outputText( '.  Clearly, Urta\'s not messing around anymore.  She pulls out and thrusts back in, and with that thrust you sway, slapping into ' );
		if( edrynAvailable ) {
			EngineCore.outputText( 'Edryn' );
		} else {
			EngineCore.outputText( 'Olivia' );
		}
		EngineCore.outputText( '\'s underside.  The contact is electric.  The rough slap would have sent shivers down your spine, if you\'d had one.  It makes your body quake and convulse with gusto.' );
		...
		EngineCore.outputText( '.  Your entire, gooey body feels like one giant clitoris, so sensitive, so vulnerable to pleasure.  With every slap, your body begins to crave release even more.  The pleasure builds with every thrust and accompanying slap.  It grows to the brink of orgasm, and surges forward, with no end in sight.' );
		...
		EngineCore.outputText( '.  Your eyes search around, seeing nothing but darkness once more.  You extend your gooey self, trying to tell where you are.  Wherever it is, it\'s tight... and there seems to be some sort of plug holding you in.' );
		EngineCore.outputText( '\n\n<b>You\'re inside ' );
		if( edrynAvailable ) {
			EngineCore.outputText( 'Edryn' );
		} else {
		...
		EngineCore.outputText( '\'s womb!</b>' );
		...
		EngineCore.outputText( ' voices her displeasure, "<i>What the... I look like I\'m pregnant!  With septuplets!</i>"' );
		EngineCore.outputText( '\n\nUrta lets out a nervous chuckle.  "<i>Sorry about that.  But this should be easier to deal with.  ' );
		if( edrynAvailable ) {
			EngineCore.outputText( 'All you need to do is push, right?' );
		} else {
			EngineCore.outputText( 'Let\'s just sit you back, and then all you need to do is push.' );
		}
		EngineCore.outputText( '</i>"  Your new host grunts in discomfort' );
		if( edrynAvailable ) {
			EngineCore.outputText( ', but obliges, sitting back on something, perhaps a bed.  You feel yourself sloshing around oddly as she lies on her back' );
		}
		EngineCore.outputText( '.  "<i>Now push,</i>" Urta says, encouragingly.' );
		...
		EngineCore.outputText( '.  The first thing out, thankfully is your head.  Bright light greets your eyes, almost stinging them.  You look around, seeing ' );
		if( edrynAvailable ) {
			EngineCore.outputText( 'Edryn clutching Urta\'s hand' );
		} else {
			EngineCore.outputText( 'Olivia lying back on the bed, clutching Urta\'s hand' );
		}
		EngineCore.outputText( ', panting heavily.  You surge outwards, first your shoulders, followed by one arm, and then the next.  The rest of your body comes out like a flood with one last push from your former host, sending you splattering onto the ground beneath you.  An exasperated, but relieved, sigh comes from ' );
		if( edrynAvailable ) {
			EngineCore.outputText( 'Edryn' );
		} else {
			EngineCore.outputText( 'Olivia' );
		}
		EngineCore.outputText( ', one that you share.' );
	...
	//if(CoC.getInstance().flags[kFLAGS.URTA_PETPLAY_DONE] === 0 && this.urtaLove()) EngineCore.outputText('  <b>A new, shiny leather collar lies half-concealed under a cushion, as though its owner haphazardly tried to bury it in a rush.</b>');;
	...
	Urta.prototype.urtaPetPlayDeletedForeverBecauseThirdProovedMeWrongAboutDice = function() {
		EngineCore.clearOutput();
		//First Time;
		if( CoC.getInstance().flags[ kFLAGS.URTA_PETPLAY_DONE ] > 0 ) {
			this.wearZeCollar();
			return;
		}
		EngineCore.outputText( 'You hold up the leather collar you found in the previous room, asking when she picked it up.' );
	...
	Urta.prototype.ignoreUrtasCollar = function() {
		CoC.getInstance().flags[ kFLAGS.URTA_PETPLAY_DONE ] = -1;
		EngineCore.clearOutput();
		if( this.urtaLove() ) {
	...
	Urta.prototype.wearZeCollar = function() {
		EngineCore.clearOutput();
		if( CoC.getInstance().flags[ kFLAGS.URTA_PETPLAY_DONE ] === 0 ) {
			EngineCore.outputText( 'You reach down into the trashcan, unclasp the collar and slip it on.  You tighten it until it fits snugly against your ' + CoC.getInstance().player.skinFurScales() + ', but isn\'t otherwise uncomfortable.  With a satisfying <b>click</b>, the clasp snaps shut, so you know there\'s no going back.  Urta\'s mouth drops as she watches you do this, completely at a loss for words.  Her cock, on the other hand, hardens, knowing just what to do.' );
			EngineCore.outputText( '\n\nYou sink to your knees and then onto your hands in front of her, brazenly displaying your submission to the vixen goddess before you.  She leans back slightly, stunned by this action.  It takes her a minute to recover and pick her jaw up off the floor.  You dutifully wait, silently, until Urta issues a command.  She seems to sense this and clears her throat, clearly embarrassed and nervous.  "<i>Oh, um, good ' + CoC.getInstance().player.mf( 'boy', 'girl' ) + ',</i>" she murmurs, patting your head.  You rub your face into her palm, which brings a smile to Urta\'s face.' );
			...
			if( CoC.getInstance().player.hasCock() ) {
				EngineCore.outputText( 'harder' );
			}
			if( CoC.getInstance().player.gender === 3 ) {
				EngineCore.outputText( ' and ' );
			}
			if( CoC.getInstance().player.hasVagina() ) {
				EngineCore.outputText( 'wetter' );
			}
			EngineCore.outputText( ' at the touch, but you can\'t help but wonder about her words.  What else is there to being a good dog?' );
		...
		//Second Time;
			...
			if( CoC.getInstance().player.tailType > AppearanceDefs.TAIL_TYPE_NONE ) {
				EngineCore.outputText( ', tail wagging' );
			}
			EngineCore.outputText( '.  Urta beams at you and pats your head lovingly.' );
		}
		//(Jump down to Scene Proper);
		//Repeats;
		else {
			EngineCore.outputText( 'You spy the collar sitting on a nearby dresser and pick it up.  You run your fingers over the engraved words on the collar\'s tag that reads \'<i>Property of Urta</i>\'.  Slipping it on, you also strip down until you\'re as nude as the day you were born, and kneel in front of your Owner' );
			if( CoC.getInstance().player.tailType > AppearanceDefs.TAIL_TYPE_NONE ) {
				EngineCore.outputText( ', tail wagging' );
			}
			EngineCore.outputText( '.' );
		...
		if( CoC.getInstance().player.tailType === AppearanceDefs.TAIL_TYPE_NONE ) {
			...
			if( CoC.getInstance().player.tailType === AppearanceDefs.TAIL_TYPE_NONE && CoC.getInstance().player.ass.analWetness > 0 ) {
				EngineCore.outputText( '  Despite your embarrassment, or perhaps because of it, you find your asshole getting wet with desire, practically salivating in anticipation of that thick plug.' );
			}
		...
		CoC.getInstance().flags[ kFLAGS.URTA_PETPLAY_DONE ]++;
	...
	Urta.prototype.doggyStyle = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( ImageManager.showImage( 'urta-home-petplay-doggy' ), false );
		EngineCore.outputText( '"<i>Hmm,</i>" Urta wonders aloud, bringing a finger up to her mouth in mock-thoughtfulness.  "<i>What should I do with my little pet now that ' + CoC.getInstance().player.mf( 'he', 'she' ) + '\'s all collared up?</i>"  Your Owner walks around you slowly, assessing your body.' );
		EngineCore.outputText( '\n\n"<i><b>Sit</b>,</i>" she says, a commanding voice coming easily.  Before you can even think about it, you immediately find your butt touching the smooth, cool floor.  Urta smirks and pats you on the head.  "<i>Good pet.  <b>Down.</b></i>"  Again, you follow her instruction immediately, lowering your front to the ground while simultaneously lifting your ass into the air.  She circles around and kneels alongside of you.  "<i><b>Stay</b>,</i>" she commands.  You don\'t dare move as she leans in, running a furry hand along the ' + CoC.getInstance().player.skinFurScales() + ' of your back, which you can\'t help but arch, and over your [butt].  Your ' );
		if( CoC.getInstance().player.tailType > AppearanceDefs.TAIL_TYPE_NONE ) {
			EngineCore.outputText( 'tail' );
		} else {
			EngineCore.outputText( 'tailplug' );
		}
		EngineCore.outputText( ' wags of its own accord, happily swishing back and forth.  Urta\'s hand dips between your buttocks, over your ' );
		if( CoC.getInstance().player.tailType > AppearanceDefs.TAIL_TYPE_NONE ) {
			EngineCore.outputText( 'asshole' );
		} else {
			EngineCore.outputText( 'plugged asshole' );
		}
		EngineCore.outputText( ', and between your thighs, ' );
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( 'grasping your [cock biggest]' );
		} else {
			EngineCore.outputText( 'her fingers slipping ever-so-slightly into the folds of your pussy' );
		}
		EngineCore.outputText( '.' );
		EngineCore.outputText( '\n\nYou moan appreciatively as she runs her fingers along it, her fingers encircling your ' );
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( '[cockHead biggest]' );
		} else {
			EngineCore.outputText( '[clit]' );
		}
		EngineCore.outputText( ' while her other hand keeps your upper body firmly down.  With every stroke your body burns just a bit hotter, your breath coming a bit shorter.  ' );
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( 'Her thumb rubs against the sensitive underside of your cock' );
		} else {
			EngineCore.outputText( 'One finger slips into you, all the way down to the knuckle, and swivels around to caress your g-spot with expert accuracy' );
		}
		EngineCore.outputText( ', making you bite your lip in ecstasy.  You want to cry out right now, but you know you can\'t break character right now.  Not this early in.  The strokes come faster, and you can feel the orgasmic flames coming on; your limbs tingle and your legs are already wobbling.' );
		...
		EngineCore.outputText( '\n\n"<i>Oh, we can\'t have you coming too soon,</i>" she explains, slapping your ass lightly.  You flinch a little at the spanking, the erotic combination of pain and pleasure feeling oh-so good on your ' + CoC.getInstance().player.skinFurScales() + '.  "<i>A good pet will only cum after their owner, after all.</i>"  You whine pitifully, hoping it won\'t take all that long at least.' );
		...
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( '[cock biggest] does the same' );
		} else {
			EngineCore.outputText( '[vagina] grows slicker' );
		}
		EngineCore.outputText( '.  She really is going to tease you as long as she can, you think to yourself.' );
		...
		EngineCore.outputText( ', taking over where it left off just moments ago.  ' );
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( 'She wraps her fingers around your shaft, slowly pumping up and down, taking a moment to tease your [cockHead biggest] every few strokes.' );
		} else {
			EngineCore.outputText( 'She buries two fingers deep inside your cunt, followed by a third, each one twisting around to rub at your g-spot softly.' );
		}
		EngineCore.outputText( '  The hand on your chest switches to ' );
		if( CoC.getInstance().player.totalNipples() === 2 ) {
			EngineCore.outputText( 'the ' );
		} else {
			EngineCore.outputText( 'an ' );
		}
		EngineCore.outputText( 'other nipple, tweaking and teasing that one while she simultaneously ' );
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( 'jerks you off' );
		} else {
			EngineCore.outputText( 'rubs you from the inside out' );
		}
		EngineCore.outputText( '.  Again comes the flame of orgasmic bliss, lapping at your tingling extremities.  Your body feels suddenly electric, so overcharged with sweet, sweet pleasure that you find it hard to think.  Low moans escape your throat, entirely unbidden, while ' );
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( 'pre-cum dribbles from your dick like a leaky faucet' );
		} else {
			EngineCore.outputText( 'your folds grow even more damp, constantly drizzling fluids that run down your ass crack, pooling on the floor like a lewd lake' );
		}
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( ', and to top it all off, you find your [leg] inexplicably kicking in mid-air' );
		}
		EngineCore.outputText( '.' );
		...
		if( CoC.getInstance().player.tailType === AppearanceDefs.TAIL_TYPE_NONE ) {
			EngineCore.outputText( ', while she grasps the plug in your ass firmly and pulls. It comes out easily with a wet <b>pop</b>, making your ass feel suddenly empty.  You guess it won\'t be empty for long, however' );
		}
		EngineCore.outputText( '.  Finally she rubs it against your [asshole], teasing you with every brush of skin against skin.' );
		...
		EngineCore.outputText( ' in such a way that it tingles powerfully, fueling your impending orgasm even more.  Locked in this cadence, she pounds your ass for what seems like hours.  The air grows damp and sweat drips from the two of you, only adding to your desire.  It\'s not long after until your nostrils are assaulted by the salty scent of sex.  Your ' );
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( '[cock] slaps against your belly roughly with every passing second, smearing thick globules of precum which drip down your belly, tickling your flesh like little balls of distilled pleasure' );
		}
		if( CoC.getInstance().player.gender === 0 ) {
			EngineCore.outputText( ', while your' );
		}
		if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( ' unused pussy aches to be filled or touched, its lips puffy and red, sensitive to every hard slap of your foxy lover\'s balls' );
		}
		EngineCore.outputText( '.' );
		...
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( 'groin, sending a wave of tremors down your cock, which suddenly erupts, covering your belly and [chest] in warm gooey goodness.' );
			if( CoC.getInstance().player.cumQ() >= 500 ) {
				EngineCore.outputText( '  It continues to jerk on its own, unloading its massive load until you\'re laying in a puddle of your own semen.' );
			}
			if( CoC.getInstance().player.hasVagina() ) {
				EngineCore.outputText( '  It then travels along to your ' );
			}
		}
		if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( '[vagina], which, despite being devoid of cock, clamps down anyway, gushing with femcum, drenching your Owner\'s balls and legs in sticky, clear fluids.' );
		}
	...
	Urta.prototype.getAPetReward = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( ImageManager.showImage( 'urta-home-petplay-reward' ), false );
		EngineCore.outputText( '"<i>Well, you\'ve been a good little pet so far,</i>" Urta says, patting you on the head. "<i>I think a little... reward is in order.</i>"  You quirk an eyebrow, wondering about that pause, but wag your ' );
		if( CoC.getInstance().player.tailType > AppearanceDefs.TAIL_TYPE_NONE ) {
			EngineCore.outputText( 'tail' );
		} else {
			EngineCore.outputText( 'tailplug' );
		}
		EngineCore.outputText( ' enthusiastically.' );
		EngineCore.outputText( '\n\nUrta holds up a hand at you and says in a quite commanding voice, "<i>Sit.</i>"  You immediately drop your butt to the floor, getting a little thrill from obeying.  "<i>Good ' + CoC.getInstance().player.mf( 'boy', 'girl' ) + '.  Now stay.</i>"  Once she\'s satisfied you\'re staying put, your Owner walks out of the room, her ass swaying tantalizingly as she disappears around the corner.  You wait, naked and with your legs and butt resting on the floor, for several minutes.  You find yourself actually getting a little ' );
		if( CoC.getInstance().player.hasCock() ) {
		...
		EngineCore.outputText( ', oddly enough, thinking of what your \'reward\' could be.  Urta returns several minutes later, with one hand holding some kind of metal dish... a food bowl for a dog!' );
		if( EngineCore.silly() ) {
			EngineCore.outputText( '  In her other hand is what looks to be a jar of peanut butter.  What use could that have?' );
		}
		...
		if( EngineCore.silly() ) {
			EngineCore.outputText( ', though you wonder if perhaps that\'s what the peanut butter is for' );
		}
		EngineCore.outputText( '.  Your Owner stretches a foot out towards you, running her stocking-clad paw along your side.  The little tufts of fur protruding from the fishnet tickle your skin in a sensual fashion, and you can\'t stop your spine from stiffening at the contact.' );
		...
		if( CoC.getInstance().player.biggestTitSize() < 1 ) {
			EngineCore.outputText( 'onto your chest' );
		} else {
			EngineCore.outputText( 'between your breasts' );
		}
		EngineCore.outputText( '.  She takes a moment to encircle one of your nipples, which stiffens to attention along with ' );
		if( CoC.getInstance().player.totalNipples() === 2 ) {
			EngineCore.outputText( 'its twin' );
		} else {
			EngineCore.outputText( 'the others' );
		}
		EngineCore.outputText( ', before sending the paw down further where it makes contact with your ' );
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( Descriptors.multiCockDescriptLight() );
		} else {
			EngineCore.outputText( '[vagina]' );
		}
		EngineCore.outputText( '.  There\'s nothing you can do to stifle the moan of pleasure that escapes your lips as your Owner\'s paw travels the length of your ' );
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( 'rapidly hardening shaft.' );
		} else {
			EngineCore.outputText( 'moistening slit.' );
		}
		...
		if( CoC.getInstance().player.hasCock() ) {
			...
			EngineCore.outputText( 'cunt lips' );
		}
		EngineCore.outputText( ' with a paw, until you bite your lip and whine at her.  She gives you a little smirk and the foot retreats.  "<i>All right then, pet, come and get your bone,</i>" Urta gestures down at the half-erect cock sprouting from between her thighs' );
		//silly:;
		if( EngineCore.silly() ) {
			EngineCore.outputText( ', while she opens the jar of peanut butter and smears a dollop across the flat head.  Is... is that supposed to be enticement?  She really is treating you like a real dog' );
		}
		EngineCore.outputText( '.  You crawl towards her nervously, and start to reach up to grasp her cock, but again she mutters, "<i>Ah-ah, good doggies don\'t use their hands.</i>"' );
		EngineCore.outputText( '\n\nNodding, you extend your tongue, running it along the length of her hardening shaft' );
		if( EngineCore.silly() ) {
			EngineCore.outputText( ', licking all of the peanut butter off the tip' );
		}
		EngineCore.outputText( '.  Urta shivers and lets out a "<i>Mm,</i>" while her pillar of horsecock reaches its full size, clocking in at what has to be a good twenty inches.  ' );
		if( EngineCore.silly() ) {
			EngineCore.outputText( '  Your owner uses her fingers to smear another large dollop of peanut butter along her own shaft, giving you an almost sadistic smirk.' );
		}
		EngineCore.outputText( '  You smooch and lick your way down her shaft, lapping at every little vein while Urta leans back and enjoys your ministrations.  Little gasps of pleasure escape from her lips, while moans leave yours as you smooch and suckle your way down to the base of that towering horsecock.' );
		EngineCore.outputText( '\n\nYour foxy Owner\'s prick glistens by the time you finally take it into your mouth' );
		if( EngineCore.silly() ) {
			EngineCore.outputText( ', the peanut butter now long forgotten' );
		}
		EngineCore.outputText( ', stretching your jaw wide.  ' );
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( 'Your own cock remains almost painfully rock hard, bobbing awkwardly beneath you, dripping precum onto the floor and bowl.' );
		}
		if( CoC.getInstance().player.hasVagina() ) {
			if( CoC.getInstance().player.gender === 3 ) {
				EngineCore.outputText( '  Not to mention' );
			} else if( CoC.getInstance().player.hasVagina() ) {
				EngineCore.outputText( '  Meanwhile' );
			}
			EngineCore.outputText( ', your pussy is as wet as it\'s ever been, constantly leaking juices that run down your legs, pooling on the floor.' );
		}
		EngineCore.outputText( '  You swirl your tongue around Urta\'s flare, paying homage to the horsecocked goddess.  Lapping at the slit for a moment, you slip your ' );
		if( CoC.getInstance().player.tongueType === AppearanceDefs.TONUGE_SNAKE ) {
			EngineCore.outputText( 'serpentine ' );
		} else if( CoC.getInstance().player.tongueType === AppearanceDefs.TONUGE_DEMONIC ) {
			EngineCore.outputText( 'demonic ' );
		} else if( CoC.getInstance().player.tongueType === AppearanceDefs.TONUGE_DRACONIC ) {
			EngineCore.outputText( 'draconic ' );
		}
		EngineCore.outputText( 'tongue along the vixen\'s thick cumvein, which pulses lightly.  Urta groans and runs a hand through your ' + Descriptors.hairDescript() + ', gripping it solidly.  You can\'t help as you\'re forced further onto the warm rod.' );
		...
		EngineCore.outputText( 'tantalizingly.' );
		...
		EngineCore.outputText( ' a little twirl with a soft, padded toe.  You can\'t help but moan into the bowl as you go, your legs shivering and stomach twisting with desire.  You continue to lap at the bowl eagerly downing mouthful after mouthful and being rewarded with more pleasure.  Even when you\'ve licked the bowl clean, you find yourself lowering your head further and slurping away the remnants that splattered onto the floor.  Urta seems to find this satisfactory, as she picks up the pace, ' );
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( 'jerking off your cock between two toes' );
		} else {
			EngineCore.outputText( 'rubbing your little love button like mad with one soft toe' );
		}
		EngineCore.outputText( ', until you just can\'t handle any more.' );
		...
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( 'The cock between your legs, meanwhile, stiffens, briefly becoming harder than you\'ve ever felt before, before blasting the floor with jet after jet of silky white cum.' );
		} else {
			EngineCore.outputText( 'Meanwhile, your vagina clamps on nothing, achingly, painfully empty.  Juices spray out onto Urta\'s foot and down your thighs, soaking the floor beneath you in yet more slippery femcum.' );
		}
		EngineCore.outputText( '  Your Owner doesn\'t just let it end there, however, she continues her paw-assault on your crotch, stroking and rubbing until you become little more than a gibbering mess of orgasm-infused flesh laying in a puddle of your own sexy goo.' );
		CoC.getInstance().flags[ kFLAGS.URTA_TIME_SINCE_LAST_CAME ] += 2;
		CoC.getInstance().flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] += 2;
		EngineCore.outputText( '...' );
		CoC.getInstance().player.orgasm();
	...
	Urta.prototype.urtaTakesPCOnWalkies = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( ImageManager.showImage( 'urta-home-petplay-walkies' ), false );
		EngineCore.outputText( '"<i>Ah, I know just what we should do!</i>" She claps her hands together and rummages through her dresser.  Urta throws on a simple little yellow sundress and makes her way to the door.  "<i>Come on, pet</i>" she says with a mischevious smirk, patting her leg and holding up a leather leash. "<i>It\'s time to go for a walk!</i>"' );
		...
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( '[eachCock] grows hard beneath you' );
		}
		if( CoC.getInstance().player.gender === 3 ) {
			EngineCore.outputText( ' and ' );
		}
		if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( 'your [vagina] moistens, ready to be on display' );
		}
		EngineCore.outputText( '.' );
		...
		EngineCore.outputText( ' a nervous little shake.  Urta smiles and opens the door wide, making you squint in the bright daylight.  You feel a tugging at your neck, and realize Urta\'s already set off.  You scamper off after her on all fours, not wanting to disappoint your Owner.  The two of you travel through some back alleys, thankfully free of strangers, until you come out into the main thoroughfare of the city.' );
		...
		if( CoC.getInstance().player.biggestTitSize() >= 5 ) {
			EngineCore.outputText( 'the breasts hanging from your chest and ' );
		}
		EngineCore.outputText( 'your openly displayed rear end.  ' );
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( 'Pre-cum drizzles from the end of your painfully erect cock' );
		} else {
			EngineCore.outputText( 'Sweet juices flow from your cunt, drizzling down onto the ground' );
		}
		EngineCore.outputText( ', creating a trail of horniness alongside Urta\'s.' );
		...
		EngineCore.outputText( '.  You feel like you really are a pet, out for a walk with your Owner.  It\'s an odd, and slightly freeing revelation, and once you come to grips with it, your entire experience changes. You find yourself trotting ahead of Urta, happily displaying yourself to all who wish to see.' );
		...
		EngineCore.outputText( '\n\nYou ' + CoC.getInstance().player.mf( 'laugh', 'giggle' ) + ' and confirm you had a great time as well.  Urta gives you a sheepish look, before asking if you\'d ever want to do that again.  You lean in for a kiss, and let that answer the question.' );
		EngineCore.dynStats( 'lus', 15 );
		CoC.getInstance().flags[ kFLAGS.URTA_TIME_SINCE_LAST_CAME ] = 0;
