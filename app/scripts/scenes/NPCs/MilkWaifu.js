...
angular.module( 'cocjs' ).run( function( MainView, EventParser, StatusAffects, Descriptors, CoC, kFLAGS, Utils, EngineCore ) {
	...
	//New Variables;
	//' + CoC.getInstance().flags[kFLAGS.MILK_NAME] + ': This slut's name. Can't call her Bath Slut forever. Unless you do.;
	...
	MilkWaifu.prototype.milkSlave = function() {
		return _.isString( CoC.getInstance().flags[ kFLAGS.MILK_NAME ] );
	};
	...
	MilkWaifu.prototype.nameZeMilkBath = function() {
		...
		EngineCore.clearOutput();
		CoC.getInstance().flags[ kFLAGS.MILK_NAME ] = MainView.nameBox.value;
		MainView.nameBox.value = '';
		MainView.nameBox.visible = false;
		//Call her Bath Slut (You Asshole);
		if( MainView.nameBox.value === 'Bath Slut' ) {
			EngineCore.outputText( 'Fuck it, Bath Slut it is.  At least she won\'t get confused.' );
		} else if( MainView.nameBox.value === 'Biscuit' ) {
			EngineCore.outputText( 'Fuck it, you may as well butter her buns!' );
		}
		//Variable: ' + CoC.getInstance().flags[kFLAGS.MILK_NAME] + ';
		//Having Named the Girl (Didn't name her Bath Slut);
		else {
			EngineCore.outputText( 'You crouch down beside the newly-named girl, brushing a few stray strands of her dark hair from her cheeks.  "<i>No more Bath Slut.  You\'re ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' now.  ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '\'s your name.</i>"' );
			EngineCore.outputText( '\n\n"<i>' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '</i>" she hesitantly repeats.  Looks like she gets it!  "<i>' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '!</i>" she says, more confidently as you encourage her.' );
		}
		//Arrival: Part 2 (PC has Rath in camp);
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.CampRathazul ) >= 0 && CoC.getInstance().player.statusAffectv2( StatusAffects.MetRathazul ) >= 4 ) {
			EngineCore.outputText( '\n\nAs you finish deciding on what to call ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ', you hear footsteps shuffling over to the two of you.  Looking up, you see the old rat alchemist Rathazul approaching, nose buried in an ancient-looking tome.  "<i>Good news, [name]!</i>" he calls, just before tripping over the prone milkmaid, going sprawling across the ground.' );
			EngineCore.outputText( '\n\n"<i>Gah!  Help, I can\'t get up!</i>" he shouts, flailing around until you rush over and pull him to his feet.' );
			EngineCore.outputText( '\n\n"<i>Ah, thank you, youngling.  But...  egad, [name], what have you dragged in this time?</i>" he mumbles, fishing a pair of spectacles out of his pocket to examine ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '.  "<i>Why, it\'s a girl...  a very, um, busty girl at that.  Gods be good, how the devil does she stand with all that...  those...  those things on her chest?</i>"' );
			EngineCore.outputText( '\n\n"<i>Milk time?</i>" ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' mewls, reaching back to rub her full, round ass where Rath bumped into her.  Sheepishly, you explain that she doesn\'t stand, per se.  You\'re not sure what the Sand Witches did to her, but she\'s not quite capable of taking care of herself.' );
			EngineCore.outputText( '\n\n"<i>I see.  Poor dear,</i>" Rath says, shakily kneeling down beside ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '.  He brushes her cheek sympathetically, a sorrowful smile playing across his aged features.  "<i>She\'s hardly the first to be changed so drastically by these awful times, my friend.  Aside from counseling and lessons, I do not know what I can do to help a mind that\'s been subjected to the horrors this girl has doubtless seen, but at the very least, my Reducto concoction may be able to help her live with some semblance of normality.</i>"' );
			...
			EngineCore.outputText( '\n\nWith that, Rath hobbles off back to his little laboratory, leaving you with ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '.' );
			EngineCore.outputText( '\n\n"<i>Rat nice,</i>" she mumbles, shifting her giant teats around beneath her.' );
			//{Plot Item gained: 'Super Reducto'};
			CoC.getInstance().player.createKeyItem( 'Super Reducto', 0, 0, 0, 0 );
	...
	MilkWaifu.prototype.ratducto = function() {
		...
		EngineCore.outputText( '\n\n"<i>Ah, thank you, youngling.  But...  egad, [name], what have you dragged in this time?</i>" he mumbles, fishing a pair of spectacles out of his pocket to examine ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '.  "<i>Why, it\'s a girl...  a very, um, busty girl at that.  Gods be good, how the devil does she stand with all that...  those...  those things on her chest?</i>"' );
		EngineCore.outputText( '\n\n"<i>Milk time?</i>" ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' mewls, reaching back to rub her full, round ass where Rath bumped into her.  Sheepishly, you explain that she doesn\'t stand, per se.  You\'re not sure what the Sand Witches did to her, but she\'s not quite capable of taking care of herself.' );
		EngineCore.outputText( '\n\n"<i>I see.  Poor dear,</i>" Rath says, shakily kneeling down beside ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '.  He brushes her cheek sympathetically, a sorrowful smile playing across her aged features.  "<i>She\'s hardly the first to be changed so drastically by these awful times, my friend.  Aside from counseling and lessons, I do not know what I can do to help a mind that\'s been subjected to the horrors this girl has doubtless seen, but at the very least, my Reducto concoction may be able to help her live with some semblance of normality.</i>"' );
		EngineCore.outputText( '\n\nRath reaches into his robes, and produces a huge vial of the stuff - easily a few dozen normal doses worth.  "<i>I\'d been saving this up for the next time Canine Peppers got into the food supply, but I think she needs it more.  Here, [name],</i>" he says, handing the vial to you.  "<i>You have more, uh, experience in applying it than I do.  And she\'ll want a gentle touch, I\'m sure.</i>"' );
		EngineCore.outputText( '\n\nWith that, Rath hobbles off back to his little laboratory, leaving you with ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '.' );
		EngineCore.outputText( '\n\n"<i>Rat nice,</i>" she mumbles, shifting her giant teats around beneath her.' );
		//{Plot Item gained: 'Super Reducto'};
	...
	MilkWaifu.prototype.milkyMenu = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.FOLLOWER_AT_FARM_BATH_GIRL ] === 0 ) {
			EngineCore.outputText( 'You wander over to ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '\'s pool, and find the dusky girl sitting at its rim, ' );
			if( CoC.getInstance().flags[ kFLAGS.MILK_SIZE ] === 0 ) {
				EngineCore.outputText( 'lying face-down on her massive rack, her plump little ass sticking up in the air for all to see.  Seeing you approach, she brightens up and shifts her titanic bust around to face you, all bright-eyes and smiles.  "<i>Milk time?</i>"' );
			} else if( CoC.getInstance().flags[ kFLAGS.MILK_SIZE ] === 1 ) {
				EngineCore.outputText( 'arms crossed under her hefty, milky bust to support their still-sizable weight.  She smiles as you approach, able to stand up under her own power to give you a hug, milky tits pressed tight against you.  "<i>I-is it milk time?</i>" she asks, cupping her tits for you.' );
			} else {
				EngineCore.outputText( 'her long, tanned legs dangling into the tub.  She jumps up when you approach, and though still a bit unsteady with such easy movements, she\'s quick to leap into your arms, nuzzling into your neck and pressing her firm, perky DD-cup breasts against you, little trickles of milk staining your chest through the little shirt she\'s wearing.  "<i>[name],</i>" she purrs happily.' );
			}
		} else {
			if( CoC.getInstance().flags[ kFLAGS.MILK_SIZE ] === 0 ) {
				EngineCore.outputText( '[bathgirlname] shakes vaguely out of her boob daze as you pick your way over to her.' );
				EngineCore.outputText( '\n\n“<i>Bath time?</i>”' );
			} else {
				EngineCore.outputText( '[bathgirlname] smiles at you as you pick your way over to her.' );
				EngineCore.outputText( '\n\n“<i>Hello [name]. Is there something you need?</i>”' );
			}
		}
		if( CoC.getInstance().flags[ kFLAGS.FOLLOWER_AT_FARM_BATH_GIRL ] === 0 && CoC.getInstance().flags[ kFLAGS.FARM_CORRUPTION_STARTED ] === 1 ) {
			if( CoC.getInstance().flags[ kFLAGS.MILK_SIZE ] === 0 ) {
		...
		if( CoC.getInstance().flags[ kFLAGS.FOLLOWER_AT_FARM_BATH_GIRL ] === 1 && CoC.getInstance().flags[ kFLAGS.MILK_SIZE ] === 0 ) {
		...
		if( CoC.getInstance().flags[ kFLAGS.MILK_SIZE ] < 2 && CoC.getInstance().player.hasKeyItem( 'Super Reducto' ) >= 0 ) {
			EngineCore.addButton( 3, 'Reducto', this.superReductoUsage );
		}
		if( CoC.getInstance().flags[ kFLAGS.MILK_SIZE ] > 0 && CoC.getInstance().player.lust >= 33 && CoC.getInstance().player.hasCock() ) {
			EngineCore.addButton( 4, 'Titfuck', this.titFuckDatMilkSlut );
		}
		if( CoC.getInstance().flags[ kFLAGS.FOLLOWER_AT_FARM_BATH_GIRL ] === 0 && CoC.getInstance().flags[ kFLAGS.MILK_SIZE ] > 0 && CoC.getInstance().flags[ kFLAGS.FARM_CORRUPTION_STARTED ] === 1 ) {
	...
	MilkWaifu.prototype.milkWriteFuAppearance = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( '' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' is a five-foot-five human female, with darkly tanned skin and ebony hair.  ' );
		if( CoC.getInstance().flags[ kFLAGS.MILK_SIZE ] >= 2 ) {
		...
		EngineCore.outputText( 'She has a human face with rich, smooth skin dotted with the faintest trace of freckles.  Her hair is long and full, hanging down her back to her waist.  She\'s got a curvaceous body, bust aside, with supple hips and thighs, and a big butt you can just sink your fingers into.' );
		//{If GIANT boobs: ;
		if( CoC.getInstance().flags[ kFLAGS.MILK_SIZE ] === 0 ) {
			EngineCore.outputText( '\n\nHer most distinguishing feature, though, is her massive bust.  Enough to drag her down onto all fours without support, ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '\'s rack is absolutely gigantic.  Her areola are the size of plates, constantly dribbling motherly fluids with every breath she takes.  The slightest touch is enough to get a stream flowing from ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '\'s titanic teats, enough to fill her entire pool with more to spare.  Those can\'t be too comfortable, you reckon, even if they are almost mesmerizing in their beauty and potential for sexual pleasure.' );
		} else if( CoC.getInstance().flags[ kFLAGS.MILK_SIZE ] === 1 ) {
			EngineCore.outputText( '\n\nThough you\'ve reduced them <i>considerably</i>, ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '\'s most eye-catching feature is still her tremendous bosom.  Those huge tits of hers would look more normal on a cowgirl than a human girl, and a rather petite one at that.  Her areola are palm-sized and tipped with a pair of prominent nipples, always eager to release their motherly fluids.  She can easily fill a tub with those things, and she alone could provide enough to feed a small village, producing more than the all the cows of Ingnam combined every day.' );
		} else {
			EngineCore.outputText( '\n\nThanks to your intervention, ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '\'s breasts aren\'t so cripplingly large.  Reduced to a DD-cup, you\'ve even managed to scrape together some clothes that actually fit her.  In her current state, you might even call her bust perky, her milk-laden breasts both firm and yet so delightfully soft; you can easily sink your fingers into them, or bury your face' );
			if( CoC.getInstance().player.hasCock() ) {
				EngineCore.outputText( ' or cock' );
			}
		...
		if( CoC.getInstance().scenes.farmCorruption.hasTattoo( 'milky' ) ) {
			...
			if( CoC.getInstance().scenes.farmCorruption.milkyFullTribalTats() ) {
				...
				if( CoC.getInstance().flags[ kFLAGS.MILKY_TATTOO_COLLARBONE ] !== 0 ) {
					EngineCore.outputText( CoC.getInstance().flags[ kFLAGS.MILKY_TATTOO_COLLARBONE ] + '\n' );
				}
				if( CoC.getInstance().flags[ kFLAGS.MILKY_TATTOO_SHOULDERS ] !== 0 ) {
					EngineCore.outputText( CoC.getInstance().flags[ kFLAGS.MILKY_TATTOO_SHOULDERS ] + '\n' );
				}
				if( CoC.getInstance().flags[ kFLAGS.MILKY_TATTOO_LOWERBACK ] !== 0 ) {
					EngineCore.outputText( CoC.getInstance().flags[ kFLAGS.MILKY_TATTOO_LOWERBACK ] + '\n' );
				}
				if( CoC.getInstance().flags[ kFLAGS.MILKY_TATTOO_BUTT ] !== 0 ) {
					EngineCore.outputText( CoC.getInstance().flags[ kFLAGS.MILKY_TATTOO_BUTT ] + '\n' );
				}
			}
		}
		EngineCore.menu();
	...
	MilkWaifu.prototype.superReductoUsage = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'You tell ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' that it\'s about time she got rid of those massive tits of hers.  She cocks her head to the side, looking at you without comprehension.  You fish the giant vial of Reducto Rath gave you from your pack and hold it up for her to examine.  She hesitantly takes it from your hand and rolls it around, sniffing at the foul-smelling stuff.  She turns up her nose at it.' );
		EngineCore.outputText( '\n\n"<i>Come on, you\'ll feel a lot better afterwards, I promise,</i>" you say, crouching down beside her, locking onto ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '\'s big brown eyes.' );
		EngineCore.outputText( '\n\nIf by nothing else than your soothing tone, ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' stops fussing about the Reducto and does her best to curl up in your lap, giving you free access to her big ol\' rack.  You tussle her hair and start thinking about how to go about this.' );
		//If GIANT BOOBS:;
		if( CoC.getInstance().flags[ kFLAGS.MILK_SIZE ] === 0 ) {
			...
			EngineCore.outputText( '\n\nYou shift ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' around in your lap, giving yourself the best angle on her giant tits.  That done, you lather up with Reducto, covering your hands with the stuff before reaching around and grabbing your friend\'s tits, smearing the first dollops around her leaky nipples.  She winces, shivering as the cool substance smears onto her teats, but almost immediately you can see it going to work: her breasts shudder, flesh quivering as they begin to shrink like balloons.' );
			EngineCore.outputText( '\n\n"<i>[name]!</i>" ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' cries out, whimpering and moaning as you massage her tits, rubbing a good amount of reducto all around her bust.  Milk pours out of her like a pair of hoses, spraying her motherly fluids everywhere as her carrying capacity shrinks and shrinks.  Down and down she goes, breasts shrinking and leaking until they\'re about HHH-cups - big enough for a cowgirl, but a lot smaller than they were.' );
			EngineCore.outputText( '\n\n"<i>That\'s enough,</i>" you say, leaning around to put your hands into the spray of milk ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '\'s making, washing the reducto from your hands.  When you\'re done, you wipe her teats off with spare milk, rubbing off the reducto until her dusky skin is as bare as can be, nice and shiny.' );
			EngineCore.outputText( '\n\nExperimentally, ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' tries to stand.  She nearly falls forward, leaden down by her still-prodigious bosoms, but you catch her, steadying the lightened girl until she\'s standing on her own power, swaying slightly, but standing.' );
			...
			if( CoC.getInstance().scenes.isabellaFollowerScene.isabellaFollower() ) {
				...
				EngineCore.outputText( ' pause' );
			} else if( CoC.getInstance().player.findStatusAffect( StatusAffects.CampMarble ) >= 0 ) {
				EngineCore.outputText( ', enough to give Marble pause' );
			}
			EngineCore.outputText( '.  She seems much happier now, and is still able to produce enough milk to drown you; it\'s a happy balance.  Still, she still has to support them everywhere, and you can\'t find any clothes that fit her particular shape, thanks to her human stature.  That said, you\'ve got enough Reducto left to bring her down to a pair of nice, firm DDs.  She\'ll still be nice and milky, though perhaps not enough to bathe in, and with a more reasonable bust size, you might actually be able to find a bra somewhere that will fit her.' );
			//Down to DD Cups;
			EngineCore.outputText( '\n\nYou shift ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' around in your lap, giving yourself the best angle on her giant tits.  That done, you lather up with Reducto, covering your hands with the stuff before reaching around and grabbing your friend\'s tits, smearing the first dollops around her leaky nipples.  She winces, shivering as the cool substance smears onto her teats, but almost immediately you can see it going to work: her breasts shudder, flesh quivering as they begin to shrink like balloons.' );
			EngineCore.outputText( '\n\n"<i>[name]!</i>" ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' cries out, whimpering and moaning as you massage her tits, rubbing a goodly amount of reducto all around her bust.  Milk pours out of her like a pair of hoses, spraying her motherly fluids everywhere as her carrying capacity shrinks and shrinks.  Down and down she goes, breasts shrinking and leaking more and more.  You smear more Reducto on her, using every last drop you\'ve got to help the poor milky slave girl.  She cries out again as her bust seems to explode with milk, a white tidal wave pouring from her shrinking nipples, nearly blinding you.' );
			EngineCore.outputText( '\n\nBy the time you can see again, you and ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' are whiter than snow, completely covered in milk.  You look down, and see that your hands are cupping a beautiful, firm pair of DD breasts.  She\'s stopped shrinking right where you predicted, leaving her with a shapely bust that any girl could be proud of.  And more importantly, one that she could easily move around with.  Wiping the milk from your eyes, you stroke ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '\'s hair, gently urging her to try and stand.  Shakily, she allows herself to be stood up, and you let her go.' );
			EngineCore.outputText( '\n\n' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' nearly falls, but catches herself at the last moment, waving her arms like a bird\'s to steady herself as she adjusts to her new center of gravity.  She takes a few experimental steps, and finds herself relatively unburdened.  Smiling from ear to ear, the girl turns to you and shouts out, "<i>I...  I can WALK!</i>" before leaping into your arms, crying tears of joy as she holds you tight against herself, perky tits dribbling milk onto your chest.  It seems no matter how flat she gets, ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' can\'t stop producing milk; but now she doesn\'t have those massive tits to deal with, and for now, that\'s enough.  The poor slave girl nuzzles her cheek into your neck, shivering as she sobs, fingers clutching at your [armor].' );
			EngineCore.outputText( '\n\nYou let her cry it out for a good long while, stroking her hair and whispering soothing words until finally, she steps back, drying her eyes.  ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' rubs her temples, groaning with a brief moment of pain before looking up at you with bright brown eyes and whispering, "<i>Thank you...  so very much, [name].</i>"' );
			...
			//{If Rath is at camp: ;
			EngineCore.outputText( 'Rathazul approaches, clearly having seen the preceeding events.  He smiles warmly at the two of you, clearly happy to have helped.  Wordlessly, the old rat produces a folded set of garments: a cloth shirt with intricate embroidery, and a pair of expensive-looking women\'s underthings, handing them over to ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '.  She looks from the gift of clothing to the alchemist, and taking them, plants an affectionate kiss on his old brow.  He chuckles, blushing.  "<i>I\'m sorry I don\'t have more to give you, poor dear,</i>" Rath says, starting to shuffle away.  "<i>Still, I think I\'ve held onto those long enough.</i>"' );
			EngineCore.outputText( '\n\nYou help ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' into her new clothes, probably the first she\'s been able to wear in years, and leave her to get used to her new body - though not before she draws you into a long kiss, holding you tight once again and whispering her heartfelt thanks.' );
	...
	MilkWaifu.prototype.milkBathTime = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'Tussling ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '\'s hair, you tell her her breasts look a bit full.  She smiles up at you eagerly as you disrobe.  Once nude, you hop down inside and say, "<i>Bath Time.</i>"' );
		EngineCore.outputText( '\n\nWith trembling anticipation, ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' reaches down for her ' );
		if( CoC.getInstance().flags[ kFLAGS.MILK_SIZE ] === 0 ) {
			EngineCore.outputText( 'teat-like' );
		} else {
			EngineCore.outputText( 'prominent' );
		}
		EngineCore.outputText( ' nipples.  Her areola bead with white perspiration in anticipation, and the woman\'s hands eagerly set to work on them, starting to draw out the first hints of lactation, pouring them into the pool around you.  You drop the plug into the drain and look up.  Cooing in delight, the huge-breasted girl finally manages to get her shivering fingertips around each of her aching milk-spouts.  She massages her nipple-flesh for a moment, her eyes lidded and heavy from pleasure, and she releases the first heavy torrent of white into the tub.  As she milks herself, ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '\'s eyes seem to go vacant, overwhelmed by pleasure, and her mouth is too busy making sighs of relief to speak.' );
		EngineCore.outputText( '\n\nPearly fluid quickly fills the first few inches of the tub, pouring as it is in numerous forking streams from its mocha spouts.  Deft hands massage the soft female flesh with smooth, unbroken motions, squeezing each teat from base to tip before retreating back to the bottom.  The steady back-and-forth motions cause the streams to rise and fall to the tempo, but the flow stays thick and steady enough to splatter your [hips] with white.  You relax against one of the benches at the side of the pool and idly trace your hands through the "water," enjoying the feeling of the milk on your ' + CoC.getInstance().player.skinFurScales() + ' as it rises higher and higher.  ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ', your only companion, continues to knead her ' );
		if( CoC.getInstance().flags[ kFLAGS.MILK_SIZE ] === 0 ) {
			EngineCore.outputText( 'gigantic' );
		} else {
			EngineCore.outputText( 'cow-like' );
		}
		EngineCore.outputText( ' breasts as you watch, and you have to admit, you feel a sexual thrill sliding down your spine as you watch her heafty bosoms work to fill your tub.' );
		...
		if( CoC.getInstance().player.gender === 1 ) {
			...
			EngineCore.outputText( Descriptors.multiCockDescriptLight() + ' and [vagina]' );
		} else {
			EngineCore.outputText( '[butt]' );
		}
		EngineCore.outputText( ', you resist the urge to touch yourself in a sexual way and focus on what you wanted to do - bathe.  ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' lets out a satisfied groan, her breasts finally seeming to wind down somewhat as the milk reaches your [chest], though by now she\'s positively quivering with pleasure, mindlessly working her nipples with eyes rolled back in her head.' );
		//{If GIANT boobs:;
		if( CoC.getInstance().flags[ kFLAGS.MILK_SIZE ] === 0 ) {
		...
		EngineCore.outputText( '\n\n' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' pulls back, licking her puffy lips and smelling strongly of female arousal, though obviously satisfied.  She whimpers, "<i>I love bath time,</i>" before ' );
		if( CoC.getInstance().flags[ kFLAGS.MILK_SIZE ] === 0 ) {
			EngineCore.outputText( 'starting to shift her breasts\' bulk toward a comfortable spot for a nap' );
		} else {
			EngineCore.outputText( 'struggling to her feet, legs shaky with the afterglow of a might boobgasm' );
		}
		EngineCore.outputText( '.' );
		if( CoC.getInstance().player.lust >= 33 ) {
			EngineCore.outputText( '  You could probably masturbate in the tub if you wanted to, or maybe pull the dusky milk-maid in for company.' );
		}
	...
	MilkWaifu.prototype.relaxWithMilkWaifu = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'You sit in the tub for a while, letting the fragrant fluids soak into your ' + CoC.getInstance().player.skinFurScales() + '.  You spend the better part of an hour lounging, letting your cares float away in ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '\'s endless white bounty.  Finally though, you pull out the tub\'s plug and climb out, finding a towel nearby.  Thankfully, the milk doesn\'t seem to leave behind any residue, and you feel clean and refreshed, if a bit horny.' );
		//(+Lust, -Fatigue);
	...
	MilkWaifu.prototype.communalBath = function() {
		...
		//[Combine];
		EngineCore.outputText( '\n\nSurrounded by friends and lovers, you relax in the pool, leaning your arms back over the rim and closing your eyes, sighing contentedly.  Your friends splash and play with each other, happy to enjoy a few blissful, normal moments away from the cares of the world, away from the demons, and the monsters, and the horror their world has become.  The waves displace beside you, milk parting as a pair of giant jugs move over to you; you look down to see ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' curling up in your arm.  Her giant teats float atop the waters, boyantly swaying with a strange grace atop the sea of their own making.' );
		EngineCore.outputText( '\n\nWithout prompting, ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' shifts around to rub a little milk into your skin, gently bathing you.  Her dark fingers run through your hair, washing it thoroughly before moving down to your arms, [chest], and legs, cleansing every inch of your ' + CoC.getInstance().player.skinFurScales() + '.  You relax to ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '\'s touch, letting her massage the cares out of your weary muscles.  Around you, your followers begin to do much the same, taking turns bathing each other, scrubbing backs and extremities with the soothing milk of ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '\'s bounty.' );
	...
	MilkWaifu.prototype.pullInZeMilkGirl = function() {
		...
		if( CoC.getInstance().player.gender === 1 ) {
			...
			EngineCore.outputText( 'sexless crotch' );
		}
		EngineCore.outputText( ', still burning hotly with your desires' );
		...
		EngineCore.outputText( '.  Before you can get too far, though, ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' turns on a heel, her huge rack pushing you back through the milky pool and then against the opposite rim.  You\'re dazed for only a brief second before her breasts press firmly into your back, so hard that a new streak of milk pours from her teats, wetting your back much as you did hers.  You relax against the rim as she cups up handfuls of milk, rubbing it into your own hair and shoulders, deft fingers massaging every muscle in your back with the skill of the greatest masseuses, and you can feel the tension bleeding from your muscles.  You yawn powerfully, resting your chin on your arms and letting the milky girl massage you, coating your ' + CoC.getInstance().player.skinFurScales() + ' in her rich, delicious milk.' );
		//If PC has a dick:;
		if( CoC.getInstance().player.hasCock() ) {
			...
			if( CoC.getInstance().player.balls > 0 ) {
				EngineCore.outputText( ', her other hand cupping your [balls], rolling the ' + Utils.num2Text( CoC.getInstance().player.balls ) + ' orbs in her palm with delightful dexterity' );
			}
		...
		if( CoC.getInstance().player.hasVagina() ) {
			...
			if( CoC.getInstance().player.biggestTitSize() >= 1 ) {
				...
				if( CoC.getInstance().player.lactationQ() >= 100 ) {
					EngineCore.outputText( '  A spurt of milk escapes your own full jugs, joining the pool-full of your friend\'s.  She gasps with delight, quickly nuzzling herself into your back and going to work.  She milks you just as she would herself, letting the hefty flow of your motherly fluids pour into the pool, odd trickles smearing down your chest, staining your chest as white as her own.' );
				}
			}
		}
	...
	MilkWaifu.prototype.fuckTheMilkWaifu = function() {
		...
		EngineCore.outputText( '\n\nSubmerged beneath a sea of creamy milk, it\'s so very, very easy to slide into ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ', ' );
		if( CoC.getInstance().player.cockArea( 0 ) < 20 ) {
			EngineCore.outputText( 'pushing your few inches into her until your hips join, her nice and loose cunt easily taking your length' );
		} else if( CoC.getInstance().player.cockArea( 0 ) < 50 ) {
	...
	MilkWaifu.prototype.beARugMunchingMilkDyke = function() {
		...
		if( CoC.getInstance().player.lactationQ() >= 200 ) {
			EngineCore.outputText( ', milking you with skill beyond anything you\'ve ever experienced before; and why not, when her entire existence revolved around that self-same skill?' );
		} else {
			EngineCore.outputText( '.' );
		}
		...
		EngineCore.outputText( '.  You run your fingers through the girl\'s hair, urging her sexual skills on as your chest heaves and quavers, riding out the massive boobgasm as femcum spurts from your cunt and into the milky pool below.' );
	...
	MilkWaifu.prototype.drinkAndFapturbate = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( '"<i>Wait,</i>" you call out to the ebony woman, letting the milk obscure your hands as you begin to masturbate, "I want a drink.</i>"  Sheepishly, ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' obligingly shifts back toward you, presenting her huge teats.' );
		...
		if( CoC.getInstance().flags[ kFLAGS.MILK_SIZE ] === 0 ) {
			EngineCore.outputText( 'a dinner plate' );
		} else {
			EngineCore.outputText( 'an open palm' );
		}
		EngineCore.outputText( ' but far more exciting.  After taking a few swallows of her body\'s milky treat, you reach down to your ' );
		if( CoC.getInstance().player.hasVagina() ) {
		...
		EngineCore.outputText( '.' );
		EngineCore.outputText( '\n\nAn excited moan worms out of ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '\'s puffy lips, a testament to the raw sensitivity of her milk-bloated jugs.  As your tongue swirls over the leaky nozzle\'s pebbly skin, she releases another breathy pant of delight.  The vocal tremors seem to coo all the way down to your loins, joining with your fingers\' caresses to stir you to aching, trembling arousal.  ' );
		//{Fork, no new PG};
		//(DA HERMS);
		...
		//(DA SCHLICKS);
			...
			EngineCore.outputText( '[OneCock] throbs painfully in your hand, so hot and hard that you\'re sure you must have begun to leak precum, but any fluid is swiftly washed away by the ever-present milk.' );
			if( CoC.getInstance().player.cockTotal() > 1 ) {
				EngineCore.outputText( '  You make sure to fondle each of your members equally, caressing, squeezing, and stroking to the tempo of your swelling passion.' );
			}
			if( CoC.getInstance().player.balls > 0 ) {
		...
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( '  [EachCock] release its own gooey load, sputtering and spurting to add to the pearly deluge.' );
		...
		if( CoC.getInstance().flags[ kFLAGS.MILK_SIZE ] === 0 ) {
			EngineCore.outputText( 'drags her gigantic tits over the puddly, milk-slicked floor' );
		} else {
			EngineCore.outputText( 'stumbles away, tenderly cupping her bright-red teats' );
		}
	...
	MilkWaifu.prototype.nyanCatMilkTime = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.MILK_SIZE ] === 1 ) {
			EngineCore.outputText( '"<i>Milk time!</i>" you say, giving ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' a playful swat on her upturned ass, walking around her to the pool.  You jump in and toss your [armor] back out as the milky witch gets situated, well aware of just how much milk this poor girl\'s about to blast you with, even with her much reduced bosom.  Still, she\'s gotta be milked, and you\'re thirsty.  Milk time indeed.' );
			...
			if( !CoC.getInstance().scenes.isabellaFollowerScene.isabellaFollower() || !CoC.getInstance().scenes.sophieFollowerScene.sophieFollower() ) {
				...
				EngineCore.outputText( '\n\nThough you didn\'t intend for this to turn communal, ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' certainly has more than enough to share.  You relax as the last trickles of milk pour into the pool, her breasts looking positively deflated.  You decide to leave the plug in for your friends as you clamber out, figuring they\'ll want to stockpile a little for the day before you drain it.  With her tits lightened for the moment, ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' reaches up and pulls you down to her, just long enough to plant a kiss on your cheek and whisper, "<i>Thank you, [name].  That felt good.</i>"' );
			}
		...
		//Milk Time!  (DD Boobs Ver.);
			...
			EngineCore.outputText( 'You sit yourself down with ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' and ask if she\'d like a good milking.  Her eyes brighten at the suggestion, and she whispers "<i>Yes please,</i>" already pulling off her shirt.  You help her get her top and bra off, leaving her full, perky breasts bare in the cool sun.  You slip behind her, running your hands across her sun-kissed skin, tracing your fingers from hips to chest; ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' shivers at your slightest touch, whining lustily as your hands wrap around her milk-laden mounds.  Her reddened nipples are rock-hard by the time your fingers brush up against them, perspiring clear pearlescent drops as you circle her wide areola.' );
			EngineCore.outputText( '\n\n' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' whimpers as you squeeze a tiny trickle from her teats, the white fluid squirting out in a powerful arc, staining the campground for the briefest moment before the hungry earth swallows it up, leaving no trace but a darkened patch at the girl\'s knees.  Gently, you start to squeeze and knead ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '\'s breasts, holding her back against you as you massage the heavy burden from her seemingly unending reserves.  A steady stream of milk pours from her chest, dribbling down between your fingers and her flat belly.  Soon a pool\'s formed around your [legs], the barren earth not nearly fast enough to consume the bounty flowing from your friend.' );
			EngineCore.outputText( '\n\nWith each caress and squeeze, ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' shivers and shakes, her chest soon heaving with her ragged, pleasure-strained breath.  You can see her lustful secretions stain her paintes as her hidden cunt quivers, desperate for attention as you milk her over-sensitive mammaries.  Her breathing becomes more irregular, her long legs quaking and eyes rolling back as you squeeze out gallon after gallon.  Her whimpers of pleasure become outright cries, piercing the quiet of camp with orgasmic moans and shrill screams as your fingers work their magic on her, sending shockwaves of pleasure from hefty teats to sex-addled mind  that make her entire body convulse and writhe at your touch.' );
			EngineCore.outputText( '\n\n"<i>N-no more.  Can\'t...  take it...  please.  Ahhhhh,</i>" ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' cries, squirming in your embrace, your hands firmly locked on her tits, milking her for every drop.  She\'s in for a mighty boobgasm in mere moments: you could soothe her through it with a gentle suckle or finish her off normally.' );
	...
	MilkWaifu.prototype.finishMilkNormall = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'You whisper a few soothing words into ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '\'s ear, but are unrelenting in your task.  With squeezing and kneading fingers, you continue to coax the milk from her tits until she\'s practically white with flowing cream.  A few minutes later though, you can physically feel her explode over the edge of bliss: the stream of milk from her stiff nipples doubles, blasting an arc several feet long as her voice breaks, a shrill cry to the heavens; ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '\'s entire body shudders, her legs nearly giving out as the boobgasm rocks her slender frame.' );
		EngineCore.outputText( '\n\nAs soon as the explosive boobgasm subsides, she collapses back against you, chest heaving with pleasured exhaustion.  The milky stream trickles down to naught, her breasts momentarily depleted.  ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' looks up at you and smiles ever so slightly, caressing your cheek before cupping her obviously-sore teats and staggering to her feet, searching for a towel.' );
	...
	MilkWaifu.prototype.fuckDatMilkSlat = function() {
		...
		EngineCore.outputText( 'Sitting behind the milky girl, chest pressed to her back tight enough to feel her every breath, you can\'t help but feel ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '\'s full, round ass brushing against your ' + Descriptors.cockDescript( x ) + '.  Responding to her touch, your prick begins to stiffen, filling out through your [armor] and into the valley of ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '\'s ass.  She gasps ever so slightly when she feels your prick pushing against her silky undergarments, but you can feel her heart race, her flesh heating as she starts to move her ass more deliberately, rubbing you from stem to head.' );
		EngineCore.outputText( '\n\nOne of your hands drifts down from ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '\'s teats, fingers tracing along her supple skin and ample curves to the hem of her panties.  You pull them down with one smooth motion, baring the full mounds of her dusky ass cheeks.  She pushes back immediately, humping up and down your rod as you work to free yourself from your [armor].  Finally, your ' + Descriptors.cockDescript( x ) + ' pops free from its binds, only to be instantly buried in ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '\'s rear cleavage; she gives a happy, girlish giggle when your throbbing rod pushes through her valley, practically bouncing on your cock.  You wrap your arms around her waist, one hand working her still-needy teats as the other dives between her legs, easily slipping a few fingers into her sodden gash.  ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' moans lustily as your digits enter her, biting her lower lip when your fingertips caress her inner walls and spasming muscles, soon soaked in her feminine fluids.  Your thumb swirls around the bud of her clit, drawing ragged gasps of pleasure from her lips until you silence her with a kiss, driving your tongue into her mouth as your fingers assault her cunt, fingering her until she\'s nice, wet, and ready.' );
		EngineCore.outputText( '\n\nYou shift forward ever so slightly, dragging your ' + Descriptors.cockDescript( x ) + ' from ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '\'s butt-cleavage and into the welcoming embrace of her womanhood.  You both moan with lust as your ' + CoC.getInstance().player.cockHead( x ) + ' presses into her, parting the folds of her cunny to feel the hot touch of her walls kissing your tip.  You slide into her with measured ease, ' + Descriptors.cockDescript( x ) + ' spreading her walls wide as your hips move to meet, your groin pushing against her bubble butt, flesh yielding as you try and slide as much cockflesh into her eager channel as you can.' );
		EngineCore.outputText( '\n\nBy the time you\'ve hilted ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' she\'s a mess, cunt drooling obscenely and tongue hanging from her mouth.  Her chest heaves in your hands as your fingers wrap back around her cherry-red nipples, her milk coming in erratic spurts as she undulates on your cock, hips and ass starting to bounce atop you.  You move to match her, hammering your [hips] home to meet her, thrusting your ' + Descriptors.cockDescript( x ) + ' into her clinging depths.  ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' cups her hands to her breasts, pooling up handfuls of milk and splashing them onto your prick and her gash, nearly dousing the heat of your lust with the rush of cool cream; but another jack-hammer thrust shows her milk to be a magnificent lubricant, letting you slip and slide into her with ease.  Soon you\'re both moaning your lust, pleasured gasps and sighs echoing through the camp as you fuck ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '\'s dripping cunt.' );
		EngineCore.outputText( '\n\nYou can feel your orgasm mounting, surging on as your ' + Descriptors.cockDescript( x ) + ' hammers relentlessly into ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '.  With an animalistic roar, you push her down onto all fours and grab her ass for leverage.  She squeals when you push her down, but she recovers in an instant, wiggling her ass tantalizingly as your fingers sink into her pliant flesh.  With a vision full of that big, soft ass swaying as your dick pounds ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '\'s, you can\'t help but cum.  You grint your teeth and give ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' a hard spank right on the ass, making her shriek with pleasure and pain, her cunt squeezing your ' + Descriptors.cockDescript( x ) + ' at just the right moment: you shoot a load of white-hot seed straight into her womb, dick buried to the hilt inside her.  Around your spasming cock, ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '\'s quim quivers and contracts, milking you for every drop; her teats let loose a wild stream of milk which pools around you, nearly hiding the excess spooge that pours from her battered cunny to stain the earth.' );
		EngineCore.outputText( '\n\nWhen your dick\'s finally depleted its load, you pull out with a contented sigh, wiping the last drops of spunk off on ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '\'s thigh.  She rolls over, a lust-dazed smile on her face, idly fingering her well-stuffed cunt and caressing her ample bosoms.  A momentary come-hither look from her beckons you over, and ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' pulls you into her arms with a girlish giggle, resting you on your back and nuzzling her cheek on your [chest].  You drift off into pleasant repose, both sexually contented and your lover well-milked.' );
		CoC.getInstance().player.orgasm();
	...
	MilkWaifu.prototype.titFuckDatMilkSlut = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'You sit down beside ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' and give her a little push onto her back.  A tiny gasp escapes her lips, but is silenced as you move over top her and giver her a long, tongue-filled kiss.  Her surprise turns into a lusty moan as you ' );
		if( CoC.getInstance().flags[ kFLAGS.MILK_SIZE ] === 2 ) {
			EngineCore.outputText( 'pull off her shirt and bra' );
		} else {
			EngineCore.outputText( 'sink your fingers into her udder-like teats' );
		}
		EngineCore.outputText( ', caressing her sensitive breasts until a gentle stream of milk runs down her chest.  Her breath catches in anticipation as you back off a moment, freeing your [cock] from your [armor] and letting it flop onto her bare belly.  However, your hips slide forward away from her already-wet cunt and towards the enticing valley of her cleavage.  To your delight, the attention you\'re about to lavish on her breasts seems to excite ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' as much as a good fucking, and her breath picks up excitedly as your cock slips between her teats.  With a little urging, she cups her breasts and squeezes them together around your rock-hard member, enveloping it in the warm embrace of her dusky flesh.' );
		EngineCore.outputText( '\n\nBefore you can start to thrust into her chest, you reach down and pinch her nipples, rolling the two prominent peaks between your fingers, coaxing out a bigger and bigger flood of milk from her deep reserves.  White rivers run down from her mounds, a deluge flooding down into the valley around your [cock].  You shudder as the warm, thick cream pours in around your member.  ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' reaches in and rubs it into your cockflesh with slow, sensuous motions, gently stroking you off as more and more whiteness submerges your wang. Overcome by your lactic lust, you lean in and lap up the run-off from the lake of milk between ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '\'s leaky breasts.  Your senses hum at the of taste sweet cream tinted with lust, and as if on their own accord, your [hips] start to slide forward, your [cock] gliding between her tits like it would a well-lubed cunt.  ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' giggles at the sensation, squeezing her tits tightly together around your wang.' );
		EngineCore.outputText( '\n\nYou grab her shoulders for support and start to really get going.  Your hips move faster and faster, groin pounding into the undersides of ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '\'s tits as you hammer into her milk-drenched cleavage.  She feels just as good as a warm, wet pussy around your cock, but with an added bonus: as the [cockHead] of your [cock] peaks out between the tops of her breasts, ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' leans her head up and wraps her full lips around it, her tongue flicking across the slit of your urethra and sending shivers of pleasure down your spine.  A low, husky moan escapes you as ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' sucks your cock at the crest of every thrust, putting all her suckling skill to work on your brim until your [cock] feels practically afire with pleasure.' );
		EngineCore.outputText( '\n\nCoaxed on by ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '\'s oral ministrations, you abandon her breasts for a moment, ' );
		//If not naga:;
		if( !CoC.getInstance().player.isNaga() ) {
			EngineCore.outputText( 'straddling her shoulders and shoving' );
		} else {
			EngineCore.outputText( 'wrapping yourself around her to shove' );
		}
		EngineCore.outputText( ' your [cock] deep into her mouth.  ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' accepts it eagerly, opening wide as you stuff inch after inch of your manhood down her throat.  Now she can really go to work: her lips wrap tight around your base, tongue lapping at your underside like candy as her throat muscles massage the shaft.  You slide in and out, watching her throat bulge and contract with each thrust' );
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( ', shivering as her tongue darts out around your cock to flick your nads' );
		}
		EngineCore.outputText( '.' );
		EngineCore.outputText( '\n\nJust as soon as you\'re settling into a rhythm, though, ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' surprises you with a sudden shift, rolling over onto all fours and taking your with her.  You flop onto your back, yelping in surprise as the busty maid tops you, her cheek resting in your thigh.  You start to protest, but are quickly silenced as her tongue wraps around the base of your cock, sliding up your fuckpole with languid ease.  You shudder and relax, content to let her put her tremendous oral proficiency to good use; and use it she does, soon getting onto all fours over your [cock], dragging her huge tits across the upright length until the sheer weight of her rack weighs your mighty penis down, pinning it to your belly.  Gently, she rocks her body forward, dragging her well-lubed tits up and down your length, only saving the [cockHead] for her mouth.  She leans down until you can feel every hot breath on your sensitive skin.  ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' takes her time, only slowly rolling her tongue around your crown, flicking her tip across your urethra and lapping up the pre that\'s now spurting liberally from your [cock].  You groan and clench your fists, trying to bear through the combination titfuck and blowjob, but you just can\'t withstand her ministrations.' );
		EngineCore.outputText( '\n\nWith one last surge of sexual energy, you wrap your [legs] around ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '\'s shoulders and roll her over again, putting yourself back on top with cock held firmly between her hefty tits.  You jackhammer your [hips] into her underboobs, digging your fingers into her soft flesh and savaging her nipples until she\'s breathing hard and moaning, milk sloshing down into her cleavage again to perfectly lubricate your last thrusts until, with a feral roar, the first thick globs of your creamy spunk burst forth, mixing momentarily with the sea of milk around your [cock] before spattering onto ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '\'s neck, giving her an instant pearl necklace.  Your cock spasms again and again, shooting jizz into the lake of milk until her breast-valley is a frothy admixture of cum and cream around your prick.  With a mischievous grin, you reach in and scoop up a double-handful of the solution you\'ve made, and hold it up to ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '\'s lips.  She smiles lustily up at you before suckling it all up, drinking every offered drop of sweet cream and seed.' );
		EngineCore.outputText( '\n\nYou sigh contentedly and give ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' an affectionate pat on the boob, watching as her flesh quivers at your touch.  She smiles at you with a lusty warmth, blowing you a kiss as you grab a towel to dry yourself off; you look back to see her happily masterbating, finishing the job you started with one hand knuckle-deep inside her as the other idly plays with the frothy cream you\'ve left between her boobs.' );
		CoC.getInstance().player.orgasm();
		EngineCore.dynStats( 'sen', -1 );
		EngineCore.fatigue( -10 );
		EngineCore.doNext( CoC.getInstance().scenes.camp.returnToCampUseOneHour );
