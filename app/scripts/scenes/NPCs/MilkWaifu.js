...
angular.module( 'cocjs' ).run( function( SceneLib, MainView, EventParser, StatusAffects, Descriptors, CoC, kFLAGS, Utils, EngineCore ) {
	...
	MilkWaifu.prototype.nameZeMilkBath = function() {
		...
		//Having Named the Girl (Didn't name her Bath Slut);
			...
			EngineCore.outputText( 'You crouch down beside the newly-named girl, brushing a few stray strands of her dark hair from her cheeks.  "<i>No more Bath Slut.  You\'re ' + CoC.flags[ kFLAGS.MILK_NAME ] + ' now.  ' + CoC.flags[ kFLAGS.MILK_NAME ] + '\'s your name.</i>"' );
		...
		if( SceneLib.farmCorruption.hasTattoo( 'milky' ) ) {
			...
			if( SceneLib.farmCorruption.milkyFullTribalTats() ) {
				...
				if( CoC.flags[ kFLAGS.MILKY_TATTOO_COLLARBONE ] !== 0 ) {
					EngineCore.outputText( CoC.flags[ kFLAGS.MILKY_TATTOO_COLLARBONE ] + '\n' );
				}
				if( CoC.flags[ kFLAGS.MILKY_TATTOO_SHOULDERS ] !== 0 ) {
					EngineCore.outputText( CoC.flags[ kFLAGS.MILKY_TATTOO_SHOULDERS ] + '\n' );
				}
				if( CoC.flags[ kFLAGS.MILKY_TATTOO_LOWERBACK ] !== 0 ) {
					EngineCore.outputText( CoC.flags[ kFLAGS.MILKY_TATTOO_LOWERBACK ] + '\n' );
				}
				if( CoC.flags[ kFLAGS.MILKY_TATTOO_BUTT ] !== 0 ) {
					EngineCore.outputText( CoC.flags[ kFLAGS.MILKY_TATTOO_BUTT ] + '\n' );
				}
			}
		}
		EngineCore.menu();
	...
	MilkWaifu.prototype.pullInZeMilkGirl = function() {
		...
		if( CoC.player.hasCock() ) {
			...
			if( CoC.player.balls > 0 ) {
				EngineCore.outputText( ', her other hand cupping your [balls], rolling the ' + Utils.num2Text( CoC.player.balls ) + ' orbs in her palm with delightful dexterity' );
			}
	...
	MilkWaifu.prototype.nyanCatMilkTime = function() {
		...
		if( CoC.flags[ kFLAGS.MILK_SIZE ] === 1 ) {
			...
			if( SceneLib.isabellaFollowerScene.isabellaFollower() ) {
				...
				EngineCore.outputText( ' done.</i>"  You find yourself roughly pushed aside as Isabella jumps in with you, tossing her skirt and blouse aside as she grasps ' + CoC.flags[ kFLAGS.MILK_NAME ] + '\'s teats, massaging them much more gently.  The milk girl\'s eyes roll back in her head as Isabella sets to work, though it gets harder and harder as the milk flows up to her thighs and chest.' );
			}
