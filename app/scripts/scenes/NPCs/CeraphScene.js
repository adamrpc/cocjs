...
angular.module( 'cocjs' ).run( function( SceneLib, Ceraph, ConsumableLib, Appearance, ArmorLib, Combat, PregnancyStore, CockTypesEnum, Descriptors, AppearanceDefs, EventParser, CoC, kFLAGS, Utils, StatusAffects, EngineCore ) {
	...
	CeraphScene.prototype.maleFuckCeraphsPussy = function() {
		...
		if( CoC.player.cockArea( 0 ) <= CoC.monster.vaginalCapacity() ) {
			...
			EngineCore.outputText( '.  Her interior is inhumanly hot and covered with thousands of slippery protrusions, each rubbing against your dick\'s flesh.  The orgy of sensation is incredible, and you immediately begin fucking your bound, demonic captive as the need to orgasm rises within you.\n\n', false );
			EngineCore.outputText( 'The defeated omnibus mewls in mock protest, "<i>Oh, don\'t cum in my demonic twat, dear champion!  If you do, you\'ll never want any other pussy ever again!</i>"\n\n', false );
	...
	CeraphScene.prototype.ceraphFirstTimeRepeat = function() {
		EngineCore.outputText( '', true );
		EngineCore.spriteSelect( 7 );
		EngineCore.outputText( 'Ceraph saunters out of the bushes and gives you a disapproving look, "<i>I tried this the easy way.  Now we\'re doing it the hard way.</i>"\n\n', false );
		EngineCore.outputText( 'She uncurls a whip that lights aflame and says, "<i>Time for your spanking!</i>"\n\n', false );
		Combat.startCombat( new Ceraph() );
	};
	...
	CeraphScene.prototype.level3CeraphOfferNo = function() {
		EngineCore.outputText( '', true );
		EngineCore.spriteSelect( 7 );
		EngineCore.outputText( 'You let Ceraph know that you refuse to let her violate your body and mind in such a way.  She shrugs nonchalantly and tugs on the crop, stretching it into a whip that bursts into flames.\n\n', false );
		EngineCore.outputText( '"<i>Mortals always want to do things the hard way,</i>" she sighs.\n\n', false );
		EngineCore.outputText( '<b>You\'ve got a fight on your hands!</b>', false );
		Combat.startCombat( new Ceraph() );
	};
	//[YES OH YES];
	...
	CeraphScene.prototype.tradeCeraphCrotchityBits = function() {
		...
		//Think about it...;
		EngineCore.outputText( 'Ceraph grins lecherously and pulls her tail from her drooling snatch.  "<i>Really?  Well this should be fun.  Let me just figure out what I want...</i>"  Her gaze is quite piercing.\n\n', false );
		//Pick a winner!;
		choice = choices[ Utils.rand( choices.length ) ];
		if( choice === 0 ) {
			EngineCore.outputText( 'FUCK AN ERROR HAPPENED GET FENOXO ON THE HORN!', false );
		}
		//(DICKS);
		else if( choice === 1 ) {
		...
		EngineCore.outputText( 'and declares, "<i>I think I\'ll have ', false );
		if( choice <= 2 ) {
	...
	CeraphScene.prototype.ceraphBodyPartDreams = function() {
		...
		if( choice === 2 ) {
			...
			if( subChoice === 0 ) {
				...
				EngineCore.outputText( '  It must be the sex that Ceraph stole from you!  You feel tight... enclosed somehow, but the ever present trickle of bodily fluids into your imprisoned nethers slowly brings about your own arousal.\n\n', false );
				EngineCore.outputText( 'Choruses of voices moan with building, peaking pleasure, and the ebb and flow of sexual slime into your parted pussy increases.  You moan in your sleep, feeling wet, obscenely wet, as you\'re used as a cum-drain for some massive orgy.  The cum from dozens of Ceraph\'s pets drools into you, filling you with warm, masculine slime and slippery pussy-fluid.  Helpless and imprisoned, you can do nothing but pant with desire until your womb is utterly packed with spooge and your gaping twat is brimming with spunk.\n\n', false );
				EngineCore.outputText( 'You\'re a useless cum-dump, there only to slurp up the leavings from Ceraph\'s more treasured pets, but it makes you oh-so horny.  You pant in your sleep, hopelessly aroused but denied release by the fading dream.', false );
				//(+4 sens, +200 lust);
			...
			//PUSSY FLESHLIGHT;
				...
				//Vagina Wall-Cubby;
				EngineCore.outputText( 'MMMmmm, something is filling you just - ahhhh... right.  You shift position in your blankets, but it doesn\'t seem to move the insistent hardness lodged up your twat.  Gods, it feels so good to be so full.  It\'s like having three or four hard dicks inside you all at once, keeping you stretched and open, the cunt-honey dripping toward the floor.  The only time they move is when a wave of contractions pulses through your female flesh, squeezing about the invaders.  Languidly, you work your muscles faster, wanting to feel greater pleasures.  You secretly wish that whatever it is would fuck you.' );
			...
			//Tit Volleyball;
				...
				EngineCore.outputText( 'You see Ceraph, the conniving omnibus, standing in a hazy pink void, dressed in a tiny purple bikini.  She smiles to herself as she holds out a fleshy orb.  It looks soft, jiggly... and alarmingly familiar.  As you spy a rounded areola, you realize the demon is holding one of your old breasts! She tosses the plump tit ponderously for a moment, before raising it to her mouth and... blowing?' );
				...
				if( EngineCore.silly() ) {
					EngineCore.outputText( 'horsecocks ' );
				} else {
					EngineCore.outputText( 'demonic cocks ' );
				}
				EngineCore.outputText( 'rise from the sand, forming makeshift poles for the twisted volleyball net.' );
	...
	CeraphScene.prototype.buttRapeCeraph = function() {
		...
		EngineCore.outputText( '.  The demon\'s ass is warm enough to make you feel you could melt like an icicle in an oven, the blissful sensations spurring you to play with her supple ass-cheeks while you enjoy the complete penetration of a demon\'s backdoor.  You give those cheeks a hard slap and pull back, enjoying the feel of her gloriously tight ring as it slides over the pussy-slicked length of your ' + Descriptors.cockDescript( x ) + '.\n\n', false );
