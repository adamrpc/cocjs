...
angular.module( 'cocjs' ).run( function( MainView, SceneLib, $rootScope, Combat, Shouldra, Descriptors, Appearance, AppearanceDefs, CoC, kFLAGS, Utils, StatusAffects, EngineCore, ConsumableLib ) {
	...
	ShouldraScene.prototype.shouldraGreeting = function() {
		...
		if( CoC.flags[ kFLAGS.TIMES_MET_SHOULDRA ] < 3 ) {
			MainView.outputText( 'You step into the ruins of the village cautiously, zigzagging through passageways and searching for anything worth pocketing. A sound from the inside of a half-collapsed cottage stops you in your tracks. You furrow your brow, upset at allowing yourself to drop your guard in a place like this. Unluckily, the presence seems aware of you, and you hear it shuffling towards the disfigured doorway. You drop into a crouch, readying your ' + CoC.player.weaponName + ' and preparing for battle. You are caught off-guard when a completely normal female leg steps out of the frame. The young woman takes a few steps closer, apparently ignoring your intimidating stance for a moment as she sizes you up. ', false );
			//([if first time] ;
			if( CoC.flags[ kFLAGS.TIMES_MET_SHOULDRA ] === 0 ) {
				MainView.outputText( '"<i>You\'re the asshole that took my racks, aren\'t you?  I was hoard-0...er, I was keeping an eye on those!</i>" she accuses, eyes narrowing angrily.  ', false );
			}
		...
		//(after three encounters with her);
			...
			if( CoC.flags[ kFLAGS.TIMES_BEATEN_SHOULDRA ] >= 3 ) {
				MainView.outputText( '"<i>Or... maybe not?  Truth be told, I think you\'ve proven your ability to pummel me enough.  Wanna just skip to the good part?</i>"\n\n', false );
				//([if first time] ;
				if( CoC.flags[ kFLAGS.TIMES_BEATEN_SHOULDRA ] === 3 ) {
					MainView.outputText( 'You can\'t say you saw that coming.  It seems she has offered to forego the fight in lieu of sexings.  ', false );
				}
				MainView.outputText( 'Would you like to accept her offer, or do you want to fight her regardless?', false );
	...
	ShouldraScene.prototype.defeatDannyPhantom = function() {
		...
		//defeat by damage);
		if( CoC.monster.HP < 1 ) {
			MainView.outputText( 'With an ear-splitting, ghostly wail that forces you to your knees, the ', false );
		...
		//(defeat by lust);
			...
			MainView.outputText( 'The girl glances down at her leggings, then does a double take, realizing as if for the first time she was completely overwhelmed by her lust. She sinks to her knees, dragging her breeches down and pressing a hand against her pubic mound.  ', false );
			if( CoC.flags[ kFLAGS.TIMES_POSSESSED_BY_SHOULDRA ] < 1 ) {
				MainView.outputText( 'Strangely, the action seems to have little to no effect.', false );
			} else {
				MainView.outputText( 'Of course, the motion has no effect on her libido, due to her ghostly state.', false );
			}
			MainView.outputText( '  Looking up at you with what can only be described as desperation, she pants, "<i>Please, hero... I need release... let me in?</i>"  You take a step forward, stroking your chin softly in contemplation.  ', false );
			//([if first time];
			if( CoC.flags[ kFLAGS.TIMES_POSSESSED_BY_SHOULDRA ] < 1 ) {
				MainView.outputText( 'What does she mean, "<i>let me in?</i>" Do you want to find out?', false );
	...
	ShouldraScene.prototype.noSlimingSlimer = function() {
		...
		if( CoC.flags[ kFLAGS.TIMES_POSSESSED_BY_SHOULDRA ] === 0 ) {
			MainView.outputText( 'not feeling up to heed the strange girl\'s whims.', false );
		} else {
			MainView.outputText( 'mystified at the thought of letting a ghost possess your body.', false );
		}
		MainView.outputText( 'You whirl about and stomp away.  She rises, screaming, "<i>Fine! I\'ll go and take over a fucking goblin, again!</i>" and storms the other direction. Before you move out of earshot, you hear a sniffle from the departing ', false );
		//([if first time];
		if( CoC.flags[ kFLAGS.TIMES_POSSESSED_BY_SHOULDRA ] === 0 ) {
			MainView.outputText( 'girl (what was that about her \'taking over\' a goblin?)', false );
		} else {
	...
	ShouldraScene.prototype.littlePigLittlePigLetMeIn = function() {
		...
		if( CoC.flags[ kFLAGS.TIMES_POSSESSED_BY_SHOULDRA ] === 0 ) {
			MainView.outputText( 'you can afford being a little adventurous with the obviously desperate girl', false );
		} else {
			MainView.outputText( 'having a spiritual tag-along might be a bit of fun', false );
		}
		MainView.outputText( ', and agree to her request to possess you. She staggers to her feet and undresses, panting and shivering with unfulfillable sexual need. You reach out to her, drawing her', false );
		//[if <~7' tall];
		if( CoC.player.tallness < 84 ) {
			MainView.outputText( ' into your arms for a tight hug', false );
		} else {
			MainView.outputText( ' up into your arms for a tight hug', false );
		}
		MainView.outputText( '. The young woman heaves a happy sigh, then begins mumbling an incantation into your shoulder. Soon enough, the girl\'s body turns translucent and she begins to sink into you, planting a little peck on your cheek on her way in.  ', false );
		//([if first time] ;
		if( CoC.flags[ kFLAGS.TIMES_POSSESSED_BY_SHOULDRA ] === 0 ) {
			MainView.outputText( 'Wait... WHAT!?  Did you just let some sort of... g-0... g-0... g-0... G-0... G-0... GHOOOOOST possess you?', false );
		}
	...
	ShouldraScene.prototype.nowOnVickiLake = function() {
		...
		if( ooze !== null && shark !== null ) {
			MainView.outputText( 'You recall the experiences of both the slime and the shark girl. Which encounter would you wish to seek out?', false );
	...
	ShouldraScene.prototype.sharkyEctoginas = function() {
		...
		MainView.outputText( 'Some time later, with another, much softer kiss, you draw the spirit back into yourself, leaving a very tired and surprisingly content shark girl in her wake. She reveals her consciousness throughout the experience, going so far as to apologize ', false );
		//(if repeated: again) ;
		if( CoC.flags[ kFLAGS.TIMED_SHARKGINAS ] > 0 ) {
			MainView.outputText( 'again ', false );
		}

		MainView.outputText( 'to the ghost girl for disrupting the spell—though the spirit would have nothing of that, claiming that though the spell was not what she had meant—or even readily knew how—to cast, she enjoyed the experience as well.  With a friendly wave, the now-naked shark girl dives back into her habitual home.\n\n', false );
	...
	ShouldraScene.prototype.genderlessGhostBuster = function() {
		...
		if( CoC.flags[ kFLAGS.SHOULDRA_GENDERLESS_FUCK_COUNT ] === 0 ) {
			MainView.outputText( 'You raise an eyebrow, genuinely curious as to how she\'ll figure out how to relieve her desire.  ', false );
		}
		MainView.outputText( 'Darting inside, the ghost girl heads unerringly towards a warped and half-rotted dresser. After searching several drawers filled with various knickknacks and trinkets, the spirit squeaks in excitement as she produces a seemingly regular egg carton. She moves to the other side of her room and opens a curiously well-made chest, setting the carton on a nightstand as she goes.  ', false );
		if( CoC.flags[ kFLAGS.SHOULDRA_GENDERLESS_FUCK_COUNT ] === 0 ) {
			MainView.outputText( 'Curious as to its contents, you glance inside—it\'s a chest of dildos, of every shape and size. Before you can ask why the hell a ghost would carry a CHEST of dildos, ', false );
		} else {
			MainView.outputText( 'You recall distinctly the contents of that chest, and are not surprised ', false );
		}
		MainView.outputText( 'when she reaches in and rummages about, producing two eight-inch vibrators with bumpy demon-shafts. She moves back to the center of the room, launching into a short spell as she goes. She sets the dildos on the same nightstand and opens the carton, pulling out four large white eggs. You marvel at how resourceful the ghost-girl is, then pause and consider the ramifications of the eggs. Your suspicions are confirmed as ', false );
		if( CoC.player.biggestTitSize() < 1 ) {
			MainView.outputText( 'two new additions bud on your chest.', false );
		} else if( CoC.player.biggestTitSize() <= 8 ) {
			MainView.outputText( 'your ' + Descriptors.allBreastsDescript() + ' begin to bounce all around.', false );
		} else {
			MainView.outputText( 'your ' + Descriptors.allBreastsDescript() + ' swell slightly to make room for a sudden increase in milk production.', false );
		}
		MainView.outputText( 'As the ghost continues darting around the room, ', false );
		//[if flat to E-cup];
		if( CoC.player.biggestTitSize() <= 8 ) {
			MainView.outputText( 'your new EE-cup breasts settle', false );
		} else {
			MainView.outputText( 'the lactation continues, and the ghost girl moves back to the table and settles down a bit', false );
		}
	...
	ShouldraScene.prototype.penisLossThatIsntALoss = function() {
		...
		if( CoC.flags[ kFLAGS.SHOULDRA_PENIS_DEFEAT_TIMES ] === 0 ) {
			MainView.outputText( 'She abruptly gasps in excitement and begins moving your hands towards your groin, though you swiftly reclaim control. "<i>You have a penis,</i>" she telepathically pulses to you with an anxious and excited edge to her voice. "<i>My hosts... they rarely... they...</i>"', false );
		} else {
			MainView.outputText( 'She chuckles softly as she recalls the times you\'ve shared, and she hungrily goes for your maleness once more, though you throw her off of it.', false );
		}
		MainView.outputText( '  The communication suddenly breaks as the ghost girl\'s will surges against yours, all thoughts of submissiveness vanishing from her mind.  ', false );
		//[if intelligence <50] ;
		if( CoC.player.inte < 50 ) {
			MainView.outputText( 'It\'s not very hard for her to break down your fortifications, and you soon feel yourself being pushed aside by her vast mental capacity.', false );
		} else if( CoC.player.inte < 90 || CoC.player.cockThatFits( 80 ) === -1 ) {
			MainView.outputText( 'You begin to gain an edge, but the ghost girl is craftier than you expected. She begins to pulse mental intrusions at you, scrambling your thoughts just long enough for her to overtake you and push you to the side.  "<i>You are one hundred years too early to challenge me,</i>" she chides. "<i>My intellect has long surpassed most any living creature on this plane.</i>"', false );
		} else {
			x = CoC.player.cockThatFits( 80 );
			MainView.outputText( 'The ghost girl puts up a good fight, but you\'re more than prepared for her mental assault. You feel her panic well as her intrusions are overwhelmed, and though she attempts to disengage the mental link, you easily stop her. She screams mental protests at you, but your ringing ears only strengthen your psychological dominance over the poor spirit. Before long, she\'s reduced to a whimpering wreck. "<i>Out,</i>" you command, tired of the troublesome specter\'s meddlings. Of course, she won\'t even try to rebel now, resignedly accumulating her essence and beginning her exit. Within moments the ghost girl flows out of your body and resolidifies before you.\n\n', false );
			MainView.outputText( 'She moves to leave in a huff, but you stop her with a single telepathic demand. She\'s yours, you remind her, and since she\'s gone through the trouble of trying to get you off, she might as well finish the job. Even as her lips purse in a stubborn pout, a dominating wave of influence sweeps over her, and regardless of herself, she begins unsteadily moving towards you. Resolving to meet halfway, you stride over to her and push her to your knees. As she watches, you shed your ' + CoC.player.armorName + ' and hoist your ' + Descriptors.cockDescript( x ) + ' up to her lips, leaving a fairly clear implication of your expectations. The ghost girl\'s eyes widen at the sight of your manhood, and she cringes as a new wave of arousal sweeps through her. She gives a little whimper at her inability to quell her overwhelming libido, but you simply laugh mockingly and gesture at your maleness.  "<i>Ghost blowjob,</i>" you mutter snidely.\n\n', false );
	...
	ShouldraScene.prototype.ghostBJFinisher = function( inside ) {
		...
		if( inside ) {
			MainView.outputText( 'excitedly convulsing', false );
		} else {
			MainView.outputText( 'practically lust-paralyzed', false );
		}
		MainView.outputText( ' girl, both of you too tired to bother moving away from the raining semen.  Panting heavily, you eventually haul yourself up, dragging the dainty ghost girl up with you.\n\n', false );
		CoC.player.orgasm();
		CoC.flags[ kFLAGS.SHOULDRA_PENIS_DEFEAT_TIMES ]++;
		//if ejaculation allowed;
		if( inside ) {
			MainView.outputText( '"<i>Thank you so much,</i>" she breathes softly, wrapping her arms around you in a soft embrace.  "<i>I wouldn\'t even be able to think straight if that much lust was built up without release.</i>"  You smile and flick her nose, and she retorts with a sly half-grin.  "<i>Oh, and you can have this,</i>" she adds, swiftly tugging her pants down and reaching into her still-drooling cunt, dragging out a handful of ectoplasm.  She tugs out a small bottle from her shirt pocket and sticks the gooey stuff inside before offering it to you with a wink.  "<i>Thanks again!</i>"\n\n', false );
			MainView.outputText( 'You watch her move off into the ruins with a new spring in her step, chuckling as she goes before heading back to camp yourself.', false );
		}
		//if that bitch didn't get nothing;
		else {
			MainView.outputText( 'She wobbles almost exaggeratedly before falling sideways, smacking her head against the ground and sending up a small splash of cum.  If she felt any pain, however, she doesn\'t react to it at all, staring vacantly at a dilapidated building and drooling slightly.  That outta teach her to try to pull a fast one on you twice, you figure.  As you stride past her, you tug her pants down to her knees and scoop out a good amount of ectoplasm from her vagina.  You reach into her pocket and grab a bottle (honking her boob for good measure and eliciting an aroused twitch), dumping the goo in and stuffing it in your pack.  She\'ll probably find a goblin eventually, you figure as you begin your journey back to the camp.', false );
		}
		if( CoC.isInCombat() ) {
			CoC.flags[ kFLAGS.BONUS_ITEM_AFTER_COMBAT_ID ] = ConsumableLib.ECTOPLS.id;
			Combat.cleanupAfterCombat();
	...
	ShouldraScene.prototype.hermaphroditeGhostsCumEctoplasm = function() {
		...
		if( CoC.flags[ kFLAGS.SHOULDRA_HERMSEX_COUNT ] === 0 ) {
			MainView.outputText( 'She abruptly gasps in excitement and begins moving her hands towards your groin, though you swiftly reclaim control of the unruly appendages. "<i>You have a penis,</i>" she telepathically pulses to you with an anxious and excited edge to her voice. "<i>My hosts... they rarely... they... </i>"  ', false );
		}
		//[if repeated] ;
		if( CoC.flags[ kFLAGS.SHOULDRA_HERMSEX_COUNT ] > 0 ) {
			MainView.outputText( 'She chuckles softly as she recalls the times you\'ve shared, and she hungrily goes for your maleness once more, though you mentally throw her off of it.  ', false );
		}
		MainView.outputText( 'The communication suddenly breaks as the ghost-girl\'s will surges against yours, all thoughts of submissiveness vanishing from her mind.  ', false );
		if( CoC.player.inte < 50 ) {
			MainView.outputText( 'It\'s not very hard for her to break down your fortifications, and you soon feel yourself being pushed aside by her vast mental capacity.', false );
		} else if( CoC.player.inte < 90 || CoC.player.cockThatFits( 80 ) === -1 ) {
			MainView.outputText( 'You begin to gain an edge, but the ghost girl is craftier than you expected, and she begins to pulse mental intrusions at you, scrambling your thoughts for just long enough for her to overtake you and push you to the side.  "<i>You are one hundred years too early to challenge me,</i>" she chides. "<i>My intellect has long surpassed most any living creature on this plane.</i>"', false );
		} else {
			x = CoC.player.cockThatFits( 80 );
			MainView.outputText( 'The ghost girl puts up a good fight, but you\'re more than prepared for her mental assault. You feel her panic well as her intrusions are overwhelmed, and though she attempts to disengage the mental link, you easily stop her. She screams mental protests at you, but your ringing ears only strengthen your psychological dominance over the poor spirit. Before long, she\'s reduced to a whimpering wreck. "<i>Out,</i>" you command, tired of the troublesome specter\'s meddlings. Of course, she won\'t even try to rebel now, resignedly accumulating her essence and beginning her exit. Within moments the ghost girl flows out of your body and resolidifies before you.\n\n', false );
	...
	ShouldraScene.prototype.loseToShouldra = function() {
		...
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'Each pulse pushes her a bit further onto your hardening manhood', false );
		} else if( CoC.player.hasVagina() ) {
			MainView.outputText( 'You can\'t deny the heat slowly welling up in your loins, your love-tunnel getting more lubricated by the moment', false );
		} else {
			MainView.outputText( 'You can\'t deny the heat slowly starting to gather in your bare loins' );
		}
		MainView.outputText( '. Finally, the waistband of her poor garment bursts, and several handfuls of butt spill onto your torso.\n\n', false );
		MainView.outputText( 'Wondering what the point of that was, you raise your head and glance up at her face, now at least four inches higher that she just was. She reaches down and gives your ', false );
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'quivering member', false );
		} else if( CoC.player.hasVagina() ) {
			MainView.outputText( 'sopping gash', false );
		} else {
			MainView.outputText( 'sensitive crotch-flesh' );
		}
	...
	ShouldraScene.prototype.loseToShouldraAsHerm = function() {
		...
		if( CoC.flags[ kFLAGS.SHOULDRA_HERMSEX_COUNT ] === 0 ) {
			...
			MainView.outputText( 'She coos softly as she recalls your anatomy, ', false );
		}
	...
	ShouldraScene.prototype.shouldraGiantCockLoss = function() {
		...
		if( CoC.flags[ kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT ] === 0 ) {
			MainView.outputText( 'bemused ', false );
		} else {
			MainView.outputText( 'excited ', false );
		}
		MainView.outputText( 'girl. With a flair, she finishes the incantation, then moves to the other goblin and repeats the process. The first goblin, meanwhile, ', false );
		if( CoC.flags[ kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT ] === 0 ) {
			MainView.outputText( 'searches her body for any signs of change, and when she finds none, begins pouting.', false );
		} else {
			MainView.outputText( 'resists the urge to smash her tits together, waiting with as much patience as she can muster.', false );
		}
		MainView.outputText( '  By this time, the ghost girl has finished with the spell, ', false );
		//[if first time] ;
		if( CoC.flags[ kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT ] === 0 ) {
			MainView.outputText( 'and instead of offering an explanation, she merely ', false );
		} else {
			MainView.outputText( 'and, to the green girl\'s obvious delight, ', false );
		}
		MainView.outputText( 'reaches over and kneads the goblin\'s ', false );
		//([if first time];
		if( CoC.flags[ kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT ] === 0 ) {
			MainView.outputText( 'small, firm B-cups', false );
		} else if( CoC.flags[ kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT ] === 1 ) {
			MainView.outputText( 'perky, sizeable C-cups', false );
		} else if( CoC.flags[ kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT ] === 2 ) {
			MainView.outputText( 'well-formed, jiggly D-cups', false );
		} else if( CoC.flags[ kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT ] === 3 ) {
			MainView.outputText( 'big and squishy E-cups', false );
		} else if( CoC.flags[ kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT ] === 4 ) {
			MainView.outputText( 'oversized, heaving F-cups', false );
		} else {
			MainView.outputText( 'gigantic and openly-bared G-cups', false );
		}
		MainView.outputText( '. The goblin ', false );
		//[if first time] ;
		if( CoC.flags[ kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT ] === 0 ) {
			MainView.outputText( 'instantly stops her protests, suddenly too stimulated by the massage to worry about any magical favoritism, ', false );
		} else {
			MainView.outputText( 'cries out in ecstasy at the still-incredible sensation, ', false );
		}
		//[if first time];
		if( CoC.flags[ kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT ] === 0 ) {
			MainView.outputText( 'although that doesn\'t stop the second goblin to begin voicing her own dissent', false );
		} else {
			MainView.outputText( 'and you can\'t help but notice the excited blush running across the second goblin\'s face', false );
		}
		MainView.outputText( '. The ghost girl pinches a quickly-stiffening nipple and pulls it out, ', false );
		if( CoC.flags[ kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT ] === 0 ) {
			MainView.outputText( 'and... pulls the breast along with it?', false );
		} else {
			MainView.outputText( 'and pulls the breast along with it.', false );
		}
		MainView.outputText( '  You inwardly gasp ', false );
		//[if first time] ;
		if( CoC.flags[ kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT ] === 0 ) {
			MainView.outputText( 'as the effects of the magic are revealed', false );
		} else {
			MainView.outputText( 'despite seeing the magic before, always enthralled by the process', false );
		}
		MainView.outputText( '; it has turned the goblin\'s breasts into an almost putty-like substance', false );
		if( CoC.flags[ kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT ] === 0 ) {
			MainView.outputText( ', evidently!', false );
		} else {
			MainView.outputText( ' once again', false );
		}
		MainView.outputText( '. The ghost girl begins to speed the process up, softly laying the goblin on the ground and kneading out the green girl\'s tits, adding at least half a cup size with every tug. The second goblin, watching intently behind your back, ', false );
		if( CoC.flags[ kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT ] === 0 ) {
			MainView.outputText( 'gasps in amazement, ', false );
		} else {
			MainView.outputText( 'can\'t restrain her urges any longer, ', false );
		}
		MainView.outputText( 'swiftly reaching down to roughly grab at her own ', false );
		//[if first time] ;
		if( CoC.flags[ kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT ] === 0 ) {
			MainView.outputText( 'petite boobs', false );
		} else if( CoC.flags[ kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT ] === 1 ) {
			MainView.outputText( 'modest bosom', false );
		} else if( CoC.flags[ kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT ] === 2 ) {
			MainView.outputText( 'sizable chest', false );
		} else if( CoC.flags[ kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT ] === 3 ) {
			MainView.outputText( 'large endowments', false );
		} else if( CoC.flags[ kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT ] === 4 ) {
			MainView.outputText( 'huge tits', false );
		} else {
			MainView.outputText( 'titanic melons', false );
		}
		MainView.outputText( ', crying in excitement as her breasts also begin to billow out under her own ministrations.\n\n', false );
		MainView.outputText( 'The first goblin\'s breasts, at this time, have reached at least ', false );
		if( CoC.flags[ kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT ] === 0 ) {
			MainView.outputText( 'E-cups', false );
		} else if( CoC.flags[ kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT ] === 1 ) {
			MainView.outputText( 'F-cups', false );
		} else if( CoC.flags[ kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT ] === 2 ) {
			MainView.outputText( 'G-cups', false );
		} else if( CoC.flags[ kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT ] === 3 ) {
			MainView.outputText( 'H-cups', false );
		} else if( CoC.flags[ kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT ] === 4 ) {
			MainView.outputText( 'I-cups', false );
		} else {
			MainView.outputText( 'J-cups', false );
		}
		MainView.outputText( ', and the ghost girl speeds up the massage ever further. Not satisfied with the progress, she raises the goblin\'s head to your lips and blows a steady gust of wind down the little woman\'s throat, ballooning her glistening mammaries out much more efficiently. This continues until the goblin nearly collapses under the weight of her new ', false );
		//[if first time] ;
		if( CoC.flags[ kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT ] === 0 ) {
			MainView.outputText( 'HH-cups', false );
		}
		//[if repeated once] ;
		else if( CoC.flags[ kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT ] === 1 ) {
			MainView.outputText( 'II-cups', false );
		} else if( CoC.flags[ kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT ] === 2 ) {
			MainView.outputText( 'JJ-cups', false );
		} else if( CoC.flags[ kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT ] === 3 ) {
			MainView.outputText( 'KK-cups', false );
		} else if( CoC.flags[ kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT ] === 4 ) {
			MainView.outputText( 'LL-cups', false );
		} else {
			MainView.outputText( 'MM-cups', false );
		}
		MainView.outputText( ', and the ghost girl turns your attention to the second goblin, whose boobs are embarrassingly uneven due to her...unpracticed skills. The ghost girl moves you over to her, mercifully evening the currently ', false );
		//([if first time] ;
		if( CoC.flags[ kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT ] === 0 ) {
			MainView.outputText( 'DD-cup (and C-cup, on the other side)', false );
		} else if( CoC.flags[ kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT ] === 1 ) {
			MainView.outputText( 'EE-cup (and D-cup, on the other side)', false );
		} else if( CoC.flags[ kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT ] === 2 ) {
			MainView.outputText( 'FF-cup (and E-cup, on the other side)', false );
		} else if( CoC.flags[ kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT ] === 3 ) {
			MainView.outputText( 'GG-cup (and F-cup, on the other side)', false );
		} else if( CoC.flags[ kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT ] === 4 ) {
			MainView.outputText( 'HH-cup (and G-cup, on the other side)', false );
		} else {
			MainView.outputText( 'II-cup (and H-cup, on the other side)', false );
		}
		MainView.outputText( 'breasts out, before again blowing into the goblin\'s mouth, inflating the goblin until she matches the first. With an excited giggle, the ghost girl swings you around to stand between the two amazed goblin girls, flinging a large glob of pre-cum into the first goblin\'s face, which she promptly smears over her breasts, enjoying the extra lubrication. She slowly drops to your back, presenting your cock vertically to the two increasingly eager girls, and gives a simple order: "<i>Titfuck.</i>"\n\n', false );
		MainView.outputText( 'Not needing to be asked twice, the two girls fall into place on each side of your prodigious shaft, heaving their engorged bosoms up and wrapping them around your penis. Pillowy boobflesh smooshes delightfully onto your crotch, around your ' + Descriptors.cockDescript( x ) + ', and meeting in the middle, the goblins\' nipples slipping against each other. They look to you for final confirmation, and the ghost girl nods your head; at this point, you find yourself voluntarily nodding along with her, completely entranced by this situation. With aroused grunts and moans, the goblins begin sliding their soft, slippery flesh up and down your shaft, and, on a sudden influence, they step onto your hips, heaving themselves up and using the increased height to stimulate even more of your ' + Descriptors.cockDescript( x ) + '. You and the ghost girl both reel back by the sensations of the goblin double-paizuri, and it\'s not long before you feel yourself reaching the end of your sexual rope, and the ghost girl informs the goblins of this. Faster than you thought possible, given their recent additions, the second goblin zooms over to your shoulders and hoists you up to a sitting position, your gigantic cock falling right into the hands of the first girl, who promptly wraps her pair of tips around each side of your head and plunges her lips around your now-dilating urethra, sucking eagerly and squeezing dollops of breast milk out of her over-stimulated bosom. To help push you over the edge, the second goblin squishes her own pair of boobs against your back and slides up and down, leaving small milky trails across you.\n\n', false );
		MainView.outputText( 'With an anxious moan, you tilt your head back and tense your body as seed begins to trek across the expansive tract of land between your crotch and your cockhead, and the second goblin promptly scrambles over, leaving you to prop yourself up as you wiggle about and clench your jaw in anticipation. The goblin duo position themselves against your manhood and open wide as the first glob of semen slams against them with enough force to bulge both of their cheeks with the stuff. They quickly heft their bosoms under them, catching any errant seed in their vast canyons of cleavage. This continues, the volume of cum increasing with every pulse until both goblins might as well have dumped four gallons of white paint onto themselves. With a final, intense blast, you fall backwards, chest heaving while the aftermath of the climax hits you. While you lay there, occasional drops of semen splattering onto your ' + Descriptors.chestDesc() + ', you manage a glance at the two to find them working together to scoop as much accumulated cum as possible into their needy fuck-holes. You zone out for around five minutes, snapping out of your daze soon after when you realize the lack of ghostly presence. You unsteadily rise to see the goblins still going at it with your semen, bellies bloated with the volume of your impressive orgasm. They stumble to their feet, making sure to move a hand under their cunts to hold in as much seed as they can while they redress—if you can even refer their get-ups as clothing. You notice their oversized breasts rapidly shrinking, deflating until they return to near-normal conditions. You manage a bit of a smile when you realize that ', false );
		if( CoC.flags[ kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT ] <= 4 ) {
			MainView.outputText( 'they have kept at least a cup size of flesh from their experience', false );
		} else {
			MainView.outputText( 'their breasts seem to have reached their crescendo, keeping the same (admittedly incredibly stacked) measurements', false );
		}
		MainView.outputText( ', which they happily fondle as they wander away, side-by-side. Moving to your ' + CoC.player.armorName + ' and redressing, you shake your head, wondering, not for the first time, if such a large appendage is truly beneficial to you, and wondering if this was the last you\'ve seen of your two new goblin friends.\n\n', false );
		MainView.outputText( 'A strange sensation begins in your midsection, and you press a hand against the flesh curiously. You remove it swiftly as a familiar head suddenly pushes out, wiggling about until it faces you. She smiles, then draws her arms out of her torso and uses them as leverage (with a whispered spell to make her hands tangible) to remove herself from you completely. Muttering an incantation and solidifying her again-unremarkable form, she winks at you and gathers her tunic, slipping it over her frame. As you watch, she slips her thumb and forefinger into her satisfied snatch, producing a healthy helping of a spooky gooey substance. Reaching into a pocket inside her tunic, she draws out a small bottle, which she promptly squishes the ectoplasm into. She offers you the bottle, explaining, "<i>The product of a... ghost climax, if you will. Keep it, it\'s good for you.</i>"  With a happy goodbye, she wanders into an alley, grabbing her ruined leggings and discarded shoes as she goes, and disappears into the shadows. You move to leave, but find your ' + CoC.player.legs() + ' growing heavy, EngineCore.fatigue washing over you in the aftermath of the possession. Before you know it, you\'re on your knees, and you have to struggle to keep your eyelids from drooping. You succumb to the exhaustion, slumping to the ground and soon snoring contentedly.', false );
		CoC.player.orgasm();
	...
	ShouldraScene.prototype.genderlessShouldrasLossRapes = function() {
		...
		MainView.outputText( 'when she reaches in and rummages about, producing two eight-inch vibrators with bumpy demon-shafts. She moves back to the center of the room, launching into a short spell as she goes. She sets the dildos on the same nightstand and opens the carton, pulling out four large white eggs.\n\n', false );
		MainView.outputText( 'You marvel at how resourceful the ghost-girl is, then pause and consider the ramifications of the eggs. Your suspicions are confirmed as ', false );
		if( CoC.player.biggestTitSize() < 1 ) {
			MainView.outputText( 'two new additions bud on your chest.', false );
		} else if( CoC.player.biggestTitSize() <= 8 ) {
			MainView.outputText( 'your ' + Descriptors.allBreastsDescript() + ' begin to bounce all around.', false );
		} else {
			MainView.outputText( 'your ' + Descriptors.allBreastsDescript() + ' swells slightly to make room for a sudden increase in milk production.', false );
		}
		MainView.outputText( '  As the ghost continues darting around the room, ', false );
		if( CoC.player.biggestTitSize() <= 8 ) {
			MainView.outputText( 'your new EE-cup breasts settle', false );
		} else {
			MainView.outputText( 'the lactation continues', false );
		}
		MainView.outputText( ', and the ghost girl moves back to the table and settles down a bit. Satisfied with the changes, she wastes no time on tapping the eggs against the table, cracking them cleanly. Your arm freezes up momentarily as she tips the half-shell towards your similarly conflicted mouth, but she wins out quickly, forces your jaw open, and drops the contents inside. She swallows the tasty contents easily, then slides her hands to your chest and lifts your huge tits in anticipation. Your nipples swell up and out, lengthening and stiffening as the first three eggs are consumed. The last one, however, produces a completely different reaction on your body; a comfortable warm tingle begins, deep in the center of your oversized breasts, migrating up steadily to your nipples. With a satisfied groan, the ghost-girl circles the tingling teats with a teasing finger as the change continues. Finally, she sinks a finger into each of the suddenly yielding nipples, shivering in pleasure from the penetration. Wasting no more time, she snatches up her two dildos and shoves them into your ', false );
		//[if new];
		if( !CoC.player.hasFuckableNipples() ) {
			MainView.outputText( 'new ', false );
		} else {
		...
		MainView.outputText( 'nipplecunts, flicking them on as the bumpy demon-dildos penetrate your virgin tits. The ensuing vibration sends little waves up and down your boobs as they jostle about, and the ghost-girl gives a strangled yelp of pleasure. Drunk with the sensation, she staggers about, mewling and gurgling in the pleasure of having your nubs violated. Her foot gets snagged on a bedpost and she flops onto the ground, chest first, the bumpy sextoys pushing slightly further into your tits. She slams your pelvis onto the floor several times in a futile attempt to further her joy, then flops to the side, eyes finally falling upon the still-open chest of dildos. She slowly drags herself to it (mashing your bosom against the ground and inciting a few happy grunts), reaching in and grabbing up a 13-inch floppy horsecock replica. Your irises shift halfway between the yellow pallor of your possessor and your natural palette as your eyes widen in horror. She couldn\'t possibly...\n\n', false );
		MainView.outputText( 'She smacks it ', false );
		if( CoC.player.isNaga() ) {
			MainView.outputText( 'against your naga-crotch', false );
		} else if( CoC.player.isGoo() ) {
			MainView.outputText( 'into your cushiony goo', false );
		} else {
			MainView.outputText( 'between your legs', false );
		}
		MainView.outputText( ', seemingly forgetting about your lack of genitalia, and grudgingly goes for the next penetrable orifice; your ' + Descriptors.assholeDescript() + '. With blatant disregard for safety, the vibrators still buzzing happily in your increasingly warm nipples, she slams the horsecock against the ground, waits for the wiggling to somewhat cease, then lines your butt up with the flared tip. Her crazed laughter mingles with your horrified scream as she sits on it, driving it at least four inches inside in its initial penetration', false );
		//[if anus is not gaping];
		if( CoC.player.ass.analLooseness <= 4 ) {
			MainView.outputText( ' and stretching your butt obscenely', false );
		}
		...
		MainView.outputText( '.  With a happy sigh, you heave yourself upright, slipping back into your ' + CoC.player.armorName + ' and moving out into the ruins once more.\n\n', false );
		MainView.outputText( 'A strange sensation begins in your midsection, and you press a hand against the flesh curiously. You remove it swiftly as a familiar head suddenly pushes out, wiggling about until it faces you. She smiles, then draws her arms out of her torso and uses them as leverage (with a whispered spell to make her hands tangible) to remove herself from you completely. Muttering an incantation and solidifying her again-unremarkable form, she winks at you and gathers her tunic, slipping it over her frame. As you watch, she slips her thumb and forefinger into her satisfied snatch, producing a healthy helping of a spooky gooey substance. Reaching into a pocket inside her tunic, she draws out a small bottle, which she promptly squishes the ectoplasm into. She offers you the bottle, explaining, "<i>The product of a... ghost climax, if you will. Keep it, it\'s good for you.</i>"  With a happy goodbye, she wanders into an alley, grabbing her ruined leggings and discarded shoes as she goes, and disappears into the shadows. You move to leave, but find your ' + CoC.player.legs() + ' growing heavy, EngineCore.fatigue washing over you in the aftermath of the possession. Before you know it, you\'re on your knees, and you have to struggle to keep your eyelids from drooping. You succumb to the exhaustion, slumping to the ground and soon snoring contentedly.', false );
		CoC.flags[ kFLAGS.SHOULDRA_GENDERLESS_FUCK_COUNT ]++;
		CoC.player.orgasm();
		EngineCore.dynStats( 'sen', 1 );
	...
	ShouldraScene.prototype.ghostGartuanLossSceneOrSomeShit = function() {
		...
		MainView.outputText( 'When the proverbial dust settles, you stir groggily. Blinking the sleep out of your eyes, you do a double take at the sheer amount of semen flooding the street. You feel a sensation building in your finally flaccid member, and you glance down. A pale, transparent smoke flows gently from your urethra, reforming into a very tired, moderately sore, but incredibly satisfied ghost girl.  ', false );
		if( CoC.flags[ kFLAGS.SHOULDRA_EXGARTUAN_SPIRIT_SEX_COUNT ] === 0 ) {
			MainView.outputText( '"<i>Spirit sex... I can\'t say I\'ve ever tried that,</i>" she admits. "<i>Damn if it isn\'t good.</i>"', false );
		} else {
			MainView.outputText( '"<i>Damn,</i>" she sighs, "<i>that is still a great ride, I\'d say...</i>"', false );
		}
		MainView.outputText( '\n\n', false );
	...
	ShouldraScene.prototype.superCoolPaladinSexStuffDudeGasmJizzMonkies = function() {
		...
		MainView.outputText( 'You look up to gaze into her moderately attractive face. Is it?', false );
	...
	ShouldraScene.prototype.slimeGhostGalForHerms = function( clearOut ) {
		...
		if( CoC.flags[ kFLAGS.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER ] <= 2 ) {
			MainView.outputText( 'Maybe she really does have a firm grip on the reigns of this beast.' );
		}
		//event occurrence >=3;
		else {
			MainView.outputText( 'Regardless, you\'ve been through this dog and pony show enough times to know that something is bound to go wrong anyway.' );
		}
		...
		if( CoC.player.looseness() === 0 ) {
			...
			MainView.outputText( 'forgiving' );
		} else if( CoC.player.looseness() === 2 ) {
			MainView.outputText( 'loose' );
		} else if( CoC.player.looseness() === 3 ) {
			MainView.outputText( 'gaping' );
		} else {
			MainView.outputText( 'cavernous' );
		}
		MainView.outputText( ', velvety scabbard.  Hands too busy latched onto one another\'s hips, ' );
		if( CoC.player.biggestTitSize() > 0 ) {
			MainView.outputText( 'both you and the spooky slime press your tits together, your swaying thrusts only enhancing the mammary meltdown.' );
		} else {
			MainView.outputText( 'the spooky slime presses her tits hard into your flat chest, your gyrations teasing them to no end.' );
		}
		MainView.outputText( '  The fierce battle increases in tempo; the sly leering the two of you used to try and psyche the other out is giving way to slight head-lashing and gasps through clenched teeth.' );
		...
		MainView.outputText( '"<i>Boo!</i>"  your face shouts at you, triggering a panicked backwards jolt from you which then results in you slipping back down on your [butt].  The spook easily phases out of you during your tumble, standing perfectly still from your previous position as she glares down at you with the same mischievous smile from earlier.' );
		MainView.outputText( '"<i>Yea, I jumped ship from that senseless slime the moment it finished filling you up to christen the S.S.  Ballsac,</i>" she explains, her tone free of any responsibility.  "<i>You\'re lucky I was around to pick up the slack for you.  ' );
		if( CoC.flags[ kFLAGS.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER ] === 0 ) {
			MainView.outputText( 'You couldn\'t spank it even to save your life, as we\'ve learned today.</i>"' );
		} else {
			MainView.outputText( 'Evidently, you haven\'t put in any meaningful practice since last time,</i>" the specter chides you while making an exaggerated jerking-off motion.' );
		}
		if( CoC.flags[ kFLAGS.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER ] === 1 ) {
			MainView.outputText( '  "<i>I was afraid you\'d have... given up the ghost.</i>"' );
		} else if( CoC.flags[ kFLAGS.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER ] > 1 ) {
			MainView.outputText( '  "<i>But I\'m more than willing to do it again next time with a conclusion like that!</i>"' );
		}
