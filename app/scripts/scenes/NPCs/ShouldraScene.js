...
angular.module( 'cocjs' ).run( function( MainView, SceneLib, $rootScope, Combat, Shouldra, Descriptors, Appearance, AppearanceDefs, CoC, kFLAGS, Utils, StatusAffects, EngineCore, ConsumableLib ) {
	...
	ShouldraScene.prototype.timeChange = function() {
		var needNext = false;
		if( CoC.flags[ kFLAGS.SHOULDRA_MAGIC_COOLDOWN ] >= 1 ) {
			CoC.flags[ kFLAGS.SHOULDRA_MAGIC_COOLDOWN ]--;
		}
		if( SceneLib.shouldraFollower.followerShouldra() ) {
			if( CoC.player.statusAffectv1( StatusAffects.Exgartuan ) === 1 && CoC.player.hasCock() && Utils.rand( 10 ) === 0 ) {
				if( CoC.flags[ kFLAGS.SHOULDRA_EXGARTUDRAMA ] === 1 ) {
					SceneLib.shouldraFollower.exgartumonAndShouldraFightPartII();
					needNext = true;
				} else if( CoC.flags[ kFLAGS.SHOULDRA_EXGARTUDRAMA ] === 2 ) {
					SceneLib.shouldraFollower.exgartumonAndShouldraFightPartIII();
					needNext = true;
				}
			}
			CoC.flags[ kFLAGS.SHOULDRA_SLEEP_TIMER ]--;
			if( SceneLib.shouldraFollower.shouldersWarnings() ) {
				needNext = true;
			}
			if( CoC.flags[ kFLAGS.SHOULDRA_SLEEP_TIMER ] === 0 || (CoC.flags[ kFLAGS.SHOULDRA_SLEEP_TIMER ] < 0 && CoC.flags[ kFLAGS.SHOULDRA_SLEEP_TIMER ] % 16 === 0) ) {
				SceneLib.shouldraFollower.shouldraWakesUpOrPokesPCsForShitsAndGigglesIdunnoHowLongCanIMakeThisFunctionNameQuestionMark();
				needNext = true;
			}
			if( CoC.flags[ kFLAGS.SHOULDRA_PLOT_COUNTDOWN ] > 0 && CoC.time.hours === 3 ) {
				CoC.flags[ kFLAGS.SHOULDRA_PLOT_COUNTDOWN ]--;
			}
		}
		return needNext;
	};
	ShouldraScene.prototype.timeChangeLarge = function() {
		if( SceneLib.shouldraFollower.followerShouldra() && CoC.flags[ kFLAGS.SHOULDRA_PLOT_COUNTDOWN ] === 0 && CoC.time.hours === 3 ) {
			CoC.flags[ kFLAGS.SHOULDRA_PLOT_COUNTDOWN ] = -1;
			SceneLib.shouldraFollower.shouldraDream1();
			return true;
		}
		//Ghostgirl recruitment priority;
		if( CoC.flags[ kFLAGS.SHOULDRA_FOLLOWER_STATE ] === 0.5 && CoC.time.hours === 6 ) {
			SceneLib.shouldraFollower.morningShouldraAlert();
			return true;
		}
		//Ghostgirl pissed off dreams;
		if( SceneLib.shouldraFollower.followerShouldra() && CoC.flags[ kFLAGS.SHOULDRA_SLEEP_TIMER ] <= -236 && CoC.time.hours === 3 && CoC.player.gender > 0 ) {
			SceneLib.shouldraFollower.nightTimeShouldraRapesThePC();
			return true;
		}
		//Ghostgirl madness;
		if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00365 ] > 0 ) {
			if( CoC.player.cockTotal() > 1 || CoC.player.faceType !== AppearanceDefs.FACE_HUMAN || CoC.player.lowerBody !== AppearanceDefs.LOWER_BODY_TYPE_HUMAN || CoC.player.tailType > AppearanceDefs.TAIL_TYPE_NONE || CoC.player.horns > 0 || CoC.player.cor > 15 || CoC.player.longestCockLength() > 10 || CoC.player.tallness < 65 || CoC.player.tallness > 78 || CoC.player.hasVagina() ) {
				CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00365 ] = 0;
			} else {
				CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00365 ]--;
				if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00365 ] === 0 ) {
	...
	ShouldraScene.prototype.shouldraGreeting = function() {
		...
		if( CoC.flags[ kFLAGS.TIMES_MET_SHOULDRA ] < 3 ) {
			...
			EngineCore.outputText( 'Finally, she acknowledges your readiness to fight. With a sigh and a shrug, she raises her arms and drops into an orthodox boxing stance. Once again, you heft your ' + CoC.player.weaponName + '.', false );
			//fight;
			Combat.startCombat( new Shouldra() );
		}
		//(after three encounters with her);
		else {
			if( CoC.flags[ kFLAGS.TIMES_BEATEN_SHOULDRA ] >= 3 && CoC.flags[ kFLAGS.TIMES_MET_SHOULDRA ] % 10 === 0 ) {
			...
			if( CoC.flags[ kFLAGS.TIMES_BEATEN_SHOULDRA ] >= 3 ) {
	...
	ShouldraScene.prototype.novemberAlphaHotel = function() {
		EngineCore.spriteSelect( 66 );
		EngineCore.outputText( '', true );
		EngineCore.outputText( 'You shake your head slowly, raising your ' + CoC.player.weaponName + ' as you regard her.  "<i>Ah, so that\'s how you want it?</i>" she titters, a smile gracing her features.  "<i>In that case, enough talk... have at you!</i>"\n\n', false );
		Combat.startCombat( new Shouldra() );
	};
	//Victory Scenarios;
	//Ghost Girl Defeated;
	...
	ShouldraScene.prototype.nowOnVickiLake = function() {
		...
		var ooze = null;
		if( CoC.flags[ kFLAGS.TIMES_MET_OOZE ] > 0 ) {
	...
	ShouldraScene.prototype.sharkyEctoginas = function() {
		...
		//[if repeat];
		if( CoC.flags[ kFLAGS.TIMED_SHARKGINAS ] > 0 ) {
	...
	ShouldraScene.prototype.ghostIzmaPenis = function() {
		EngineCore.spriteSelect( 67 );
		EngineCore.outputText( '', true );
		EngineCore.outputText( 'You swiftly disrobe and lay your ' + CoC.player.armorName + ' to the side, revealing your stiffening ' + Descriptors.multiCockDescriptLight() + ' in all of its glory. The ghost-girl\'s eyes brighten at the sight, and she even allows herself a little swoon (which looks absolutely silly, given her current tough shark-like vessel) as she regards your manhood.  ', false );
		...
		//[if first time];
		if( CoC.flags[ kFLAGS.TIMES_SHARKPENISED ] === 0 ) {
	...
	ShouldraScene.prototype.ghostGooGurlzDuckfaces = function() {
		...
		if( CoC.player.gender === 3 ) {
			if( CoC.flags[ kFLAGS.SLIMEGINAED ] > 0 && CoC.flags[ kFLAGS.SHOULDRA_SLIME_PENOR_TIMES ] > 0 ) {
	...
	ShouldraScene.prototype.slimeGinaFlation = function() {
		...
		} else if( CoC.player.tailType === AppearanceDefs.TAIL_TYPE_NONE ) {
			EngineCore.outputText( 'arms and ' + CoC.player.legs(), false );
		} else {
			EngineCore.outputText( 'arms, ' + CoC.player.legs() + ', and tail', false );
		}
	...
	ShouldraScene.prototype.slimeyPenorsAhoy = function() {
		...
		if( CoC.flags[ kFLAGS.SHOULDRA_SLIME_PENOR_TIMES ] > 0 ) {
		...
		if( CoC.player.balls > 0 ) {
			EngineCore.outputText( 'Your ' + CoC.player.legs() + ' are forced apart by your burgeoning cum-sack', false );
		} else {
	...
	ShouldraScene.prototype.genderlessGhostBuster = function() {
		...
		CoC.player.buttChange( 31, true, true, false );
		...
		EngineCore.outputText( '  With a happy sigh, you heave yourself upright, slipping back into your ' + CoC.player.armorName + ' and moving out into the ruins once more.', false );
	...
	ShouldraScene.prototype.penisLossThatIsntALoss = function() {
		...
		} else if( CoC.player.inte < 90 || CoC.player.cockThatFits( 80 ) === -1 ) {
	...
	ShouldraScene.prototype.penisGartuanGhostSmexVictory = function() {
		...
		EngineCore.outputText( 'A blush (somehow) coming over her ' + CoC.player.skinTone + ' cheeks, the ghost girl nods her head meekly. She obviously doesn\'t know what to expect from the volatile creature. She suddenly gives a little cry, and her head is sucked back into your body. You can vaguely feel her presence shift, and then, as if pulled, moving... into your penis?  You feel full control returning to you, and you place a hand under your prodigious pecker and hoist it slightly. You jump as the thing bucks in your grasp. "<i>ALRIGHT, BITCH, LET\'S GET TO SPIRIT SEX,</i>" Exgartuan exclaims, and to your surprise, your spiritual newcomer groans expectantly.\n\n', false );
		...
		if( CoC.flags[ kFLAGS.SHOULDRA_EXGARTUAN_SPIRIT_SEX_COUNT ] > 0 ) {
	...
	ShouldraScene.prototype.ghostGinaWinSexings = function() {
		if( CoC.player.statusAffectv1( StatusAffects.Exgartuan ) === 2 && Utils.rand( 2 ) === 0 ) {
		...
		EngineCore.outputText( 'You are about to question exactly what she is referring to, but quickly fall silent as she gently but firmly takes control of your body, sliding out of your ' + CoC.player.armorName + ' and idly stroking your cunt. You allow her control of your mouth, and she chants a fairly lengthy spell, her absent-minded masturbation steadily gaining fervor as she progresses. With a final, shouted word of power, both of you cry out as your clitoris suddenly pulses and swells into your hand, the increased sensitivity nearly driving you insane. Your bitch-button does not stop there, however, puffing up with each heartbeat. Your eyes roll back in your head and small primitive grunts and moans escape your lips as your clit engorges in the ghostly magic, not stopping until it reaches at least four times its original length. A breeze blows through the ruined rooftops, and you scream as the wind brushes against your elongated bitch-button.  Too sensitive... the thing is way too sensitive!\n\n', false );
		EngineCore.outputText( 'Against your will, and against all of your better instincts, the ghost-girl, as aroused as you are uncomfortable, moves both hands to your sex, gripping it and giving it a squeeze. Your entire body locks up, causing you to fall backwards onto the pavement as a devastating orgasm rocks through your system. A strangled cry is the only reaction you can give to the explosive sensations. The ghost-girl, however, not satisfied with the suddenness of the climax, begins feverishly stroking your clitoris with one hand, the other reaching into your love-tunnel and stimulating the rosy flesh inside. You arch your back and attempt to dig small holes into the road with your ' + CoC.player.feet() + ' as shock-waves run through your system over and over, never giving you a chance to recuperate. You hear, as if from a great distance, a mix of frenzied laughter and orgasmic cries of pleasure ringing inside your head. Finally, mercifully, you fade into unconsciousness, leaving the ghost girl to her constant stimulation.\n\n', false );
	...
	ShouldraScene.prototype.hermaphroditeGhostsCumEctoplasm = function() {
		...
		if( CoC.player.inte < 50 ) {
			...
			x = CoC.player.cockThatFits( 80 );
			EngineCore.outputText( 'The ghost girl puts up a good fight, but you\'re more than prepared for her mental assault. You feel her panic well as her intrusions are overwhelmed, and though she attempts to disengage the mental link, you easily stop her. She screams mental protests at you, but your ringing ears only strengthen your psychological dominance over the poor spirit. Before long, she\'s reduced to a whimpering wreck. "<i>Out,</i>" you command, tired of the troublesome specter\'s meddlings. Of course, she won\'t even try to rebel now, resignedly accumulating her essence and beginning her exit. Within moments the ghost girl flows out of your body and resolidifies before you.\n\n', false );
			EngineCore.outputText( 'She moves to leave in a huff, but you stop her with a single telepathic demand. She\'s yours, you remind her, and since she\'s gone through the trouble of trying to get you off, she might as well finish the job. Even as her lips purse in a stubborn pout, a dominating wave of influence sweeps over her, and regardless of herself, she begins unsteadily moving towards you. Resolving to meet halfway, you stride over to her and push her to her knees.  As she watches, you shed your ' + CoC.player.armorName + ' and hoist your ' + Descriptors.cockDescript( x ) + ' up to her lips, leaving a fairly clear implication of your expectations. The ghost girl\'s eyes widen at the sight of your manhood, and she cringes as a new wave of arousal sweeps through her. She gives a little whimper at her inability to quell her overwhelming libido, but you simply laugh mockingly and gesture at your maleness.  "<i>Ghost blowjob,</i>" you mutter snidely.\n\n', false );
			EngineCore.outputText( 'You don\'t even give her the chance to act of her own volition, simply reaching out, taking ahold of her head, and slamming her onto your ' + Descriptors.cockDescript( x ) + '.  She takes it surprisingly well, unsurprisingly easily, taking most of the shaft in the first thrust alone.  With not as much as even a sputter, she impatiently double-handedly grabs your ' + Descriptors.buttDescript() + ' and thrusts her head forward.  Before you know it, your entire ' + Descriptors.cockDescript( x ) + ' is wrapped in a fleshy tunnel of love.', false );
			if( CoC.player.cockTotal() > 1 ) {
				EngineCore.outputText( '  Mindful of ', false );
				if( CoC.player.cockTotal() === 2 ) {
		...
		EngineCore.outputText( '\n\n', false );
		EngineCore.outputText( 'Taking complete control of your body as if it was never actually yours, she removes your ' + CoC.player.armorName + ' and throws them carelessly to the ground. Grabbing up your ' + Descriptors.cockDescript( x ) + ' in both hands and fondling it awkwardly, she strikes you as obviously inexperienced with the male anatomy, despite her levels of intelligence. She continues to grope your lengthening member, admitting, "<i>Even with the rare male bodies I have possessed, I rarely got to play with their...</i>"\n\n', false );
	...
	ShouldraScene.prototype.loseToShouldra = function() {
		...
		if( CoC.player.gender === 1 ) {
			...
			} else if( CoC.player.biggestCockArea() >= 200 ) {
	...
	ShouldraScene.prototype.ourDadTaughtUsNotToBeAshamedOfOurDicks = function() {
		...
		EngineCore.outputText( 'and you feel your hands moving unbidden towards your crotch. You snap the sneaky extremities back to your sides, knowing exactly what she wants to do. The ghost girl soon proves herself the stronger in the contest of wills, however, and eventually, your will breaks and she takes complete control once again. She forces your body to stand, removing your ' + CoC.player.armorName + ' and throwing them carelessly to the ground. She takes your ' + Descriptors.cockDescript( x ) + ' in both hands and fondles it awkwardly, obviously inexperienced with the male anatomy.  "<i>Most of my hosts have been women,</i>" she explains as she continues to grope your lengthening member.  "<i>And even then, I rarely got to play with their...</i>"\n\n', false );
	...
	ShouldraScene.prototype.loseToShouldraWithWorms = function() {
		...
		EngineCore.outputText( 'and you feel your hands moving unbidden towards your crotch. The ghost girl soon proves herself the stronger in the contest of wills, however, and eventually, your will breaks and she takes control. She heaves your body off the ground, removing your ' + CoC.player.armorName + ' and throwing them carelessly to the ground. She takes your ' + Descriptors.cockDescript( 0 ) + ' in both hands and fondles it awkwardly, obviously inexperienced with the male anatomy. "<i>Most of my hosts have been- WHAT THE FUCK!?</i>"\n\n', false );
	...
	ShouldraScene.prototype.loseToShouldraAsHerm = function() {
		...
		EngineCore.outputText( 'and you feel your hands moving unbidden towards your crotch. You snap the sneaky extremities back to your sides, knowing exactly what she wants to do. The ghost girl soon proves herself the stronger in the contest of wills, however, and eventually, your will breaks and she takes complete control once again. She forces your body to stand, removing your ' + CoC.player.armorName + ' and throwing them carelessly to the ground. She takes your ' + Descriptors.cockDescript( 0 ) + ' in both hands and fondles it awkwardly, obviously inexperienced with the male anatomy.  "<i>Most of my hosts have been women,</i>" she explains as she continues to grope your lengthening member.  "<i>And even then, I rarely got to play with their...</i>"\n\n', false );
	...
	ShouldraScene.prototype.shouldraGiantCockLoss = function() {
		...
		//[if first time];
		if( CoC.flags[ kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT ] === 0 ) {
		...
		} else if( CoC.flags[ kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT ] === 1 ) {
			EngineCore.outputText( 'perky, sizeable C-cups', false );
		} else if( CoC.flags[ kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT ] === 2 ) {
			EngineCore.outputText( 'well-formed, jiggly D-cups', false );
		} else if( CoC.flags[ kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT ] === 3 ) {
			EngineCore.outputText( 'big and squishy E-cups', false );
		} else if( CoC.flags[ kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT ] === 4 ) {
		...
		if( CoC.flags[ kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT ] <= 4 ) {
		...
		EngineCore.outputText( ', which they happily fondle as they wander away, side-by-side. Moving to your ' + CoC.player.armorName + ' and redressing, you shake your head, wondering, not for the first time, if such a large appendage is truly beneficial to you, and wondering if this was the last you\'ve seen of your two new goblin friends.\n\n', false );
		EngineCore.outputText( 'A strange sensation begins in your midsection, and you press a hand against the flesh curiously. You remove it swiftly as a familiar head suddenly pushes out, wiggling about until it faces you. She smiles, then draws her arms out of her torso and uses them as leverage (with a whispered spell to make her hands tangible) to remove herself from you completely. Muttering an incantation and solidifying her again-unremarkable form, she winks at you and gathers her tunic, slipping it over her frame. As you watch, she slips her thumb and forefinger into her satisfied snatch, producing a healthy helping of a spooky gooey substance. Reaching into a pocket inside her tunic, she draws out a small bottle, which she promptly squishes the ectoplasm into. She offers you the bottle, explaining, "<i>The product of a... ghost climax, if you will. Keep it, it\'s good for you.</i>"  With a happy goodbye, she wanders into an alley, grabbing her ruined leggings and discarded shoes as she goes, and disappears into the shadows. You move to leave, but find your ' + CoC.player.legs() + ' growing heavy, EngineCore.fatigue washing over you in the aftermath of the possession. Before you know it, you\'re on your knees, and you have to struggle to keep your eyelids from drooping. You succumb to the exhaustion, slumping to the ground and soon snoring contentedly.', false );
		CoC.player.orgasm();
	...
	ShouldraScene.prototype.genderlessShouldrasLossRapes = function() {
		...
		EngineCore.outputText( '.', false );
		CoC.player.buttChange( 50, true, true, false );
		EngineCore.outputText( '  Babbling like a crazed addict, her mind clouding increasingly by the triple penetration, she hops up and down, driving the dildo further into your anus with every bounce. Finally, with eight inches of horsey wang shoved into your butt, and vibrating demon-dicks never ceasing their barrage of pleasure on your tits, the ghost-girl climaxes in a big way. She reaches up and roughly pulls the vibrators out of your nipplecunts, stroking them vigorously as they spurt a mixture of transparent spunk and thick white milk, two mini-fountains of orgasmic pleasure. After some time on the ground, taking deep breaths that jostle your boobs against each other, the ghost girl finally reaches down and extricates the equine dong from your pucker, tossing it to the side with a contented sigh. "<i>What did I tell you?</i>" she asks softly. "<i>Any means necessary.</i>"\n\n', false );
		EngineCore.outputText( 'You chuckle to yourself, admittedly impressed by her prowess. A pressure lifts from your chest, and you absently raise a hand to find ', false );
		if( CoC.player.biggestTitSize() < 1 ) {
			EngineCore.outputText( 'your tits, along with the elongated nipples, receding back into a flat chest', false );
		...
		EngineCore.outputText( '.  With a happy sigh, you heave yourself upright, slipping back into your ' + CoC.player.armorName + ' and moving out into the ruins once more.\n\n', false );
		EngineCore.outputText( 'A strange sensation begins in your midsection, and you press a hand against the flesh curiously. You remove it swiftly as a familiar head suddenly pushes out, wiggling about until it faces you. She smiles, then draws her arms out of her torso and uses them as leverage (with a whispered spell to make her hands tangible) to remove herself from you completely. Muttering an incantation and solidifying her again-unremarkable form, she winks at you and gathers her tunic, slipping it over her frame. As you watch, she slips her thumb and forefinger into her satisfied snatch, producing a healthy helping of a spooky gooey substance. Reaching into a pocket inside her tunic, she draws out a small bottle, which she promptly squishes the ectoplasm into. She offers you the bottle, explaining, "<i>The product of a... ghost climax, if you will. Keep it, it\'s good for you.</i>"  With a happy goodbye, she wanders into an alley, grabbing her ruined leggings and discarded shoes as she goes, and disappears into the shadows. You move to leave, but find your ' + CoC.player.legs() + ' growing heavy, EngineCore.fatigue washing over you in the aftermath of the possession. Before you know it, you\'re on your knees, and you have to struggle to keep your eyelids from drooping. You succumb to the exhaustion, slumping to the ground and soon snoring contentedly.', false );
		CoC.flags[ kFLAGS.SHOULDRA_GENDERLESS_FUCK_COUNT ]++;
		CoC.player.orgasm();
		EngineCore.dynStats( 'sen', 1 );
	...
	ShouldraScene.prototype.paladinModeFollowup = function() {
		...
		EngineCore.outputText( 'You begin to ask why she would own such a thing, but she silences you with an outstretched hand. Her open palm turns into a gesture to sit next to her, and you comply, confused. "<i>To please a woman,</i>" she begins in an exaggeratedly articulate way, "<i>you must first let her know you want her-0...</i>" She\'s cut off as you gently lay a hand on her breast, the words dying in her throat. She tosses the manual aside, and happily begins teasing your uncorrupted manhood. You\'ve had enough of the foreplay; your hallowed staff requires attention. Her smile widens as you tug your ' + CoC.player.armorName + ' off and lay the garment at the foot of the bed, and she follows suit. The two of you slide onto the bed, her assuming the position under you. With a last lingering gaze into her eyes, you insert your sacred manhood into her waiting vagina. You then proceed to have consensual sex in the missionary position for the sole purpose of procreation... however, you can\'t help but notice how bored she looks. You give her an extra-hard thrust, and she jumps slightly, as if coming out of a daydream. Her moans, while still sounding oddly lackluster, propel you enough to bring you to orgasm, depositing your seed into her womb in a completely vague but assuredly sexy way. After a few minutes of hugs and cuddles, you both redress, and you announce your need to depart. She nods in understanding, and she follows you to the road as you exit her home. "<i>Oh, by the way,</i>" she calls, turning you around curiously. "<i>You totally just fucked a ghost.</i>"\n\n', false );
		...
		if( CoC.time.hours < 6 || CoC.time.hours >= 23 ) {
	...
	ShouldraScene.prototype.slimeGhostGalForHerms = function( clearOut ) {
		...
		//{event occurrence === 0};
		if( CoC.flags[ kFLAGS.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER ] === 0 ) {
			EngineCore.outputText( 'I\'ve got this thing all figured out.' );
		}
		//event occurrence =1;
		else if( CoC.flags[ kFLAGS.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER ] === 1 ) {
			EngineCore.outputText( 'I\'ve got this thing all figured out.  For real this time.' );
		}
		//event occurrence >=2;
		else if( CoC.flags[ kFLAGS.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER ] === 2 ) {
		...
		//event occurrence === 0;
		if( CoC.flags[ kFLAGS.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER ] <= 2 ) {
		...
		//if cockLength(0) > 24;
		if( CoC.player.cocks[ 0 ].cockLength > 24 ) {
		...
		//vaginalLooseness === 0;
		if( CoC.player.looseness() === 0 ) {
			EngineCore.outputText( 'tight' );
		}//vaginalLooseness === 1;
		else if( CoC.player.looseness() === 1 ) {
			EngineCore.outputText( 'forgiving' );
		} else if( CoC.player.looseness() === 2 ) {
		...
		if( CoC.flags[ kFLAGS.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER ] === 1 ) {
		...
		CoC.flags[ kFLAGS.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER ]++;
	};
