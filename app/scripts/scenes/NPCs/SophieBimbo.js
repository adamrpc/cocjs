...
angular.module( 'cocjs' ).run( function( SceneLib, PregnancyStore, ConsumableLib, Combat, PerkLib, Descriptors, EventParser, CoC, kFLAGS, Utils, StatusAffects, EngineCore ) {
	...
	SophieBimbo.prototype.bimboSophie = function() {
		return (CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00282 ] > 0 && CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00283 ] === 0 && CoC.flags[ kFLAGS.SOPHIE_DEBIMBOED ] === 0);
	};
	SophieBimbo.prototype.sophieIsInSeason = function() {
		return (CoC.flags[ kFLAGS.SOPHIE_HEAT_COUNTER ] > 551 && !SceneLib.sophieScene.pregnancy.isPregnant);
	};
	...
	SophieBimbo.prototype.bimbotizeMeCaptainSophie = function() {
		...
		EngineCore.outputText( 'After a few moments of torrential, slimy output, Sophie\'s quivering cunt spasms and her legs give out.  She slumps over, ass in the air and utterly exposed as she giggles, "<i>Like, wow!  You made me cum so hard, ' + CoC.player.mf( 'mister', 'miss' ) + '!  Can I, like, come home with you?  I don\'t really want to compete with these bitches and you\'re super ' + CoC.player.mf( 'handsome', 'hot' ) + '!  We can have lots of like, sex and stuff!</i>"\n\n', false );
	...
	SophieBimbo.prototype.acceptBimboSophie = function() {
		...
		EngineCore.outputText( 'Sophie looks up and gasps, "<i>Oh no!</i>"  Thankfully, the ditz remembers her own wings and spreads them, slowing the descent to a rapid, controlled glide.  You sigh with relief, supported by her arms and legs thousands of feet in the air.  Sophie titters, "<i>Like, which way should we go, ' + CoC.player.short + '?  Everything is all funny up here, and I dunno where you live.  We need a place to fuck!</i>"\n\n', false );
		...
		} else if( CoC.flags[ kFLAGS.AMILY_FOLLOWER ] > 0 && CoC.flags[ kFLAGS.AMILY_BLOCK_COUNTDOWN_BECAUSE_CORRUPTED_JOJO ] === 0 ) {
	...
	SophieBimbo.prototype.approachBimboSophieInCamp = function( output ) {
		...
		if( CoC.player.hasItem( ConsumableLib.DEBIMBO ) ) {
		...
		if( CoC.flags[ kFLAGS.SOPHIE_CAMP_EGG_COUNTDOWN ] > 0 && output ) {
		...
		if( CoC.flags[ kFLAGS.SOPHIE_BROACHED_SLEEP_WITH ] === 1 && CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_SOPHIE ] === 0 ) {
		...
		if( CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_SOPHIE ] === 0 && !SceneLib.sophieScene.pregnancy.isPregnant && CoC.flags[ kFLAGS.FARM_CORRUPTION_STARTED ] === 1 ) {
		...
		if( CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_SOPHIE ] !== 0 && CoC.flags[ kFLAGS.FOLLOWER_PRODUCTION_SOPHIE ] === 0 ) {
			EngineCore.addButton( 2, 'Harvest Eggs', this.harvestEggs );
		}
		if( CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_SOPHIE ] !== 0 && CoC.flags[ kFLAGS.FOLLOWER_PRODUCTION_SOPHIE ] === 1 ) {
		...
		if( SceneLib.farmCorruption.hasTattoo( 'sophie' ) ) {
			...
			if( SceneLib.farmCorruption.sophieFullTribalTats() ) {
				...
				if( CoC.flags[ kFLAGS.SOPHIE_TATTOO_COLLARBONE ] !== 0 ) {
					EngineCore.outputText( CoC.flags[ kFLAGS.SOPHIE_TATTOO_COLLARBONE ] + '\n' );
				}
				if( CoC.flags[ kFLAGS.SOPHIE_TATTOO_SHOULDERS ] !== 0 ) {
					EngineCore.outputText( CoC.flags[ kFLAGS.SOPHIE_TATTOO_SHOULDERS ] + '\n' );
				}
				if( CoC.flags[ kFLAGS.SOPHIE_TATTOO_LOWERBACK ] !== 0 ) {
					EngineCore.outputText( CoC.flags[ kFLAGS.SOPHIE_TATTOO_LOWERBACK ] + '\n' );
				}
				if( CoC.flags[ kFLAGS.SOPHIE_TATTOO_BUTT ] !== 0 ) {
					EngineCore.outputText( CoC.flags[ kFLAGS.SOPHIE_TATTOO_BUTT ] + '\n' );
				}
			}
		}
		EngineCore.menu();
	...
	SophieBimbo.prototype.postEggSelector = function( selected ) {
		...
		CoC.flags[ kFLAGS.FOLLOWER_PRODUCTION_SOPHIE ] = 1;
		if( CoC.flags[ kFLAGS.FOLLOWER_PRODUCTION_SOPHIE_COLORCHOICE ] !== selected ) {
			CoC.flags[ kFLAGS.FOLLOWER_PRODUCTION_SOPHIE_COLORCHOICE ] = selected;
	...
	SophieBimbo.prototype.bimboSophieSexMenu = function() {
		...
		if( CoC.player.hasCock() ) {
			...
			//VAG FUCKS!;
			if( CoC.player.cockThatFits( this.sophieCapacity() ) >= 0 ) {
			...
			if( CoC.player.biggestCockArea() > this.sophieCapacity() && this.sophieIsInSeason() ) {
		...
		if( CoC.flags[ kFLAGS.TIMES_SOPHIE_HAS_DRUNK_OVI_ELIXIR ] > 0 && (CoC.player.gender > 0) && !SceneLib.sophieScene.pregnancy.isPregnant ) {
			if( CoC.player.hasItem( ConsumableLib.OVIELIX ) || SceneLib.inventory.hasItemInStorage( ConsumableLib.OVIELIX ) ) {
		...
		if( CoC.flags[ kFLAGS.TOLD_SOPHIE_TO_IZMA ] === 0 && CoC.flags[ kFLAGS.TIMES_SOPHIE_AND_IZMA_FUCKED ] > 0 && CoC.flags[ kFLAGS.IZMA_NO_COCK ] === 0 ) {
		...
		if( CoC.flags[ kFLAGS.SOPHIE_ADULT_KID_COUNT ] > 0 && CoC.flags[ kFLAGS.SOPHIE_FAMILY_INCEST ] > 0 && CoC.player.cockThatFits( this.sophieCapacity() ) >= 0 && CoC.player.hasCock() ) {
	...
	SophieBimbo.prototype.tellSophieToVisitIzma = function() {
		...
		CoC.flags[ kFLAGS.TOLD_SOPHIE_TO_IZMA ] = 1;
		EngineCore.doNext( SceneLib.camp.returnToCampUseOneHour );
	};
	...
	SophieBimbo.prototype.bimboSophieLustStickSurprise = function() {
		...
		if( select === 0 ) {
			...
			if( !CoC.player.hasCock() ) {
				...
				EngineCore.outputText( 'Pushing her back, you try not to make contact against her with ' + CoC.player.oMultiCockDesc() + '.  Your lips tingle dangerously, and you know you\'ll be lusting after harpies for at least a few hours.', false );
				EngineCore.dynStats( 'lus', 15 );
			}
			SceneLib.sophieScene.luststickApplication( 5 );
		} else if( select === 2 ) {
			this.sophieSprite();
			if( CoC.player.hasCock() ) {
				EngineCore.outputText( 'While standing there, drinking a sip of water, strong hands pull down your ' + CoC.player.armorName + '.  Plump lips wrap around ' + CoC.player.oMultiCockDesc() + ' and smother it in plush, warm slipperiness.  You look down at your bimbo harpy and sigh while ' + CoC.player.oMultiCockDesc() + ' grows harder in her lips.  She pops off and asks, "<i>Like, wanna fuck?</i>"  You sigh and try to resist the images of yourself mounting the harpy that flood your mind.  It might be time to fuck her soon...', false );
				EngineCore.dynStats( 'lus', 15 );
			} else {
				EngineCore.dynStats( 'lus', 10 );
				EngineCore.outputText( 'While standing there, drinking a sip of water, strong hands pull down your ' + CoC.player.armorName + '.  Plump lips slide over your ' );
				if( CoC.player.hasVagina() ) {
			...
			if( CoC.player.hasCock() ) {
				EngineCore.dynStats( 'lus', 15 );
				EngineCore.outputText( Descriptors.SMultiCockDesc() + ' seems to wiggle inside your ' + CoC.player.armorName + '.  Damn, she feels really good on you!  Maybe you should screw the slutty bimbo soon?  ', false );
			}
	...
	SophieBimbo.prototype.daughterCheckup = function() {
		EngineCore.clearOutput();
		if( CoC.flags[ kFLAGS.SOPHIE_DAUGHTER_MATURITY_COUNTER ] > 200 ) {
		...
		} else if( CoC.flags[ kFLAGS.SOPHIE_DAUGHTER_MATURITY_COUNTER ] > 100 ) {
		...
		} else if( CoC.flags[ kFLAGS.SOPHIE_DAUGHTER_MATURITY_COUNTER ] > 0 ) {
	...
	SophieBimbo.prototype.unlockResistance = function() {
		...
		CoC.player.createPerk( PerkLib.LuststickAdapted, 0, 0, 0, 0 );
	};
	//Get oral as female.  ;
	SophieBimbo.prototype.bimboSophieLicksRugs = function() {
		...
		EngineCore.outputText( 'You sit back on a nearby rock and spread your ' + CoC.player.legs(), false );
		...
		else if( CoC.player.clitLength < 7 ) {
		...
		if( CoC.player.inte * 2 < CoC.player.cor + CoC.player.lib ) {
	...
	SophieBimbo.prototype.bimboSophieSixtyNineAfterCunnilingus = function() {
		...
		EngineCore.outputText( '', true );
		//NON WANG;
		if( !CoC.player.hasCock() || CoC.player.cockThatFits( this.sophieCapacity() ) === -1 ) {
			...
			EngineCore.outputText( 'With rapacious desire, you grab the harpy\'s down-coated ass-cheeks and squeeze her plump rump, pulling her dripping cooch straight onto your face.  You shiver at the potent, womanly smell as it oozes over your ' + CoC.player.face() + ', and dribbles down your chin.  Without a moment\'s hesitation, you take the first long lick of Sophie\'s tasty juices.  They have a tangy flavor that seems to sizzle on your tongue before fading into a soft, sweet, peach-like taste.  You hum in pleasure - the harpy was right; your tongue REALLY wants to taste her.\n\n', false );
			EngineCore.outputText( '"<i>Wow! You\'re so good at pussy-licking, babe,</i>" declares Sophie, pursing her lips slowly and deliberately as she enunciates the rest of her statement, "<i>Your tongue just can\'t get enough harpy pussy!  Ooooh yeah, you\'re totally the best cunny-licker I\'ve like, ever had munch on my box!  Mmm, you love Momma Sophie\'s girl-cum, dontcha?  Yea-ah... lick lick lick!</i>"  She giggles inarticulately after that, swiveling her hips to smear her leaking vagina all over your face.  You cram your ' + CoC.player.face() + ' further up Sophie\'s creamy quim and lick over and over, curling your tongue into every delicious nook and fold.\n\n', false );
			EngineCore.outputText( 'Sophie drops down onto her elbows, her ass-length blond plumage tickling at your ' + CoC.player.legs() + ' as she examines your groin.  The slutty harpy leans over your gold-smeared crotch and gives your ' + Descriptors.clitDescript() + ' a kiss, her plush lips smothering the ', false );
		...
		//SixtyNine Continuation - DUDES;
			...
			var x = CoC.player.cockThatFits( this.sophieCapacity() );
			if( x < 0 ) {
			...
			EngineCore.outputText( 'As her imperatives roll through you, you shudder and shake.  ' + CoC.player.OMultiCockDesc() + ' immediately releases a dollop of pre-cum, and Sophie dives down on it like an addict seeking her fix.  The return of her drugged suction is nearly too much for you, and you feel your ' + Descriptors.cockDescript( x ) + ' swelling in her mouth, the ' + CoC.player.cockHead( x ) + ' thickening against her hungry tongue.  She shakes on top of you and releases another wave of syrupy fluids.  You swallow a mouthful, compulsively drinking her prodigious pussy\'s copious lady-spunk.  Your back arches, your ' + CoC.player.legs() + ' clench, and you erupt inside your bimbo\'s puckered, sucking mouth-hole, giving her tonsils a bath of your creamy semen.  Rivulets of semen run over her juicy lips as you cum too fast for her to keep up with.', false );
	...
	SophieBimbo.prototype.buttFuckBimboSophie = function() {
		...
		EngineCore.outputText( 'Stepping closer, you toss off your ' + CoC.player.armorName + ' and reply, "<i>Yes, just like that.</i>"\n\n', false );
		...
		EngineCore.outputText( ' delic... de... yummy!</i>"  You slap her ass, grab her tail feathers, and pull the harpy\'s winking asshole down onto your ' + Descriptors.cockDescript( x ) + '. Her voices rises, not in anger, but in delight, and your ' + CoC.player.cockHead( x ) + ' pulses pleasantly in the first few inches of the harpy\'s rectum.  The bimbo relaxes and sinks down, giggling and asking, "<i>Is-ooh-is my butt, like, good for you?  It\'s soooo tight, and it\'s making my cunny all wet and stuff!</i>"\n\n', false );
		...
		EngineCore.outputText( 'Pushing her away, you unsheath most of your ' + Utils.num2Text( Math.round( CoC.player.cocks[ x ].cockLength ) ) + ' inches from Sophie\'s stretched asshole, leaving just your ' + CoC.player.cockHead( x ) + ' to hold the rectal portal open.  She whines in disappointment until you lunge forward and stuff her full of dick so suddenly that she squirts dribbles of lube.  You grab her hair with one hand and ride her ass hard and fast, each thrust punctuated by happy squeals of submissive pleasure from your airheaded anal toy.  Sophie cheers, "<i>Fuck me ' + CoC.player.mf( 'stud', 'babe' ) + ', fuck my ass!  Unh, y-yeah like that.  Mmmm, yeah, pull my hair and tell me what a bad girl I\'ve been!</i>"\n\n', false );
		...
		EngineCore.outputText( 'You keep butt-fucking her hard and fast, ignoring the sprinkles of sticky, maternal harpy-juice that splatter on your ' + CoC.player.legs() + ' as you fuck her.  In between throaty moans and screams, she bellows out "<i>YES!</i>" as loudly as possible, then lapses into quiet, exhausted grunts.  Yet, her vagina does not stop its prodigious leaking; if anything it\'s dripping and squirting out even heavier flows of submissive girl-spooge.  The harpie\'s anal ring is convulsing around you, reduced to pointless trembling from the hard fucking you\'re giving her.  Looking back with lidded eyes, she begs, "<i>C-cum... cum in my ass... it\'ll... it\'ll feel soo good, babe.  Please?  Like, gimme your spooge!</i>"\n\n', false );
		if( CoC.player.balls > 0 ) {
	...
	SophieBimbo.prototype.fuckBimboSophiesTits = function() {
		...
		EngineCore.outputText( 'She wobbles forward and her massive mounds easily slide to either side of ' + CoC.player.oMultiCockDesc() + ', completely engulfing it.  Still holding on to her nipples, you pull them together and ', false );
		if( CoC.player.cocks[ x ].cockThickness < 8 ) {
		...
		EngineCore.outputText( 'Suddenly, Sophie\'s puckered lips are on your ' + CoC.player.cockHead( x ) + ', sucking it softly while her tongue tickles the sensitive slit of your urethra.  She pops off for a breath with a lusty look in her eyes and spit drooling from the corner of her puffy lips to lubricate her tits.  A golden lip print sits around the circumference of your ' + Descriptors.cockDescript( x ) + ', marking just how much she\'s managed to smother in oral pleasure.  Pulling her hands up, she presses her palms into the heavy weight of her breasts, nearly vanishing from view as she drags them up your ' + Utils.num2Text( Math.round( CoC.player.cocks[ x ].cockLength ) ) + '-inch length.  Her drugged lipstick makes your ' + CoC.player.cockHead( x ) + ' tingle deliciously, and you sigh, happy to have such glorious udders growing from your bleach-blonde bitch.\n\n', false );
	...
	SophieBimbo.prototype.sophieEggApocalypse = function() {
		...
		EngineCore.outputText( '', true );
		//First time;
		if( CoC.flags[ kFLAGS.TIMES_SOPHIE_HAS_DRUNK_OVI_ELIXIR ] === 0 ) {
		...
		var daddy = null;
		var mommy = null;
		CoC.flags[ kFLAGS.TIMES_SOPHIE_HAS_DRUNK_OVI_ELIXIR ]++;
		if( CoC.player.hasCock() ) {
	...
	SophieBimbo.prototype.beBimboSophiesSugarDaddy = function() {
		...
		//[CoC.player gains random large egg];
	...
	SophieBimbo.prototype.rideDatSophieVag = function( nice ) {
		...
		var y = -1;
		if( CoC.player.cockTotal() > 1 ) {
			y = CoC.player.cockThatFits2( this.sophieCapacity() );
			if( y < 0 ) {
				y = CoC.player.biggestCockIndex();
				if( y === x ) {
					y = CoC.player.smallestCockIndex();
				}
			}
		}
	...
	SophieBimbo.prototype.sophieImpregnationSex = function() {
		var x = CoC.player.cockThatFits( this.sophieCapacity() );
		var y = CoC.player.cockThatFits2( this.sophieCapacity() );
		if( x < 0 ) {
			x = CoC.player.smallestCockIndex();
		}
		EngineCore.clearOutput();
	...
	SophieBimbo.prototype.sophieMoveInAttempt = function() {
		EngineCore.clearOutput();
		this.sophieSprite();
		CoC.flags[ kFLAGS.SOPHIE_BROACHED_SLEEP_WITH ] = 1;
	...
	SophieBimbo.prototype.yesSophieSleepWith = function() {
		...
		EngineCore.outputText( ' midriff and cushiony tits against you.  "<i>You\'re awesome, [name], you know that?  You\'re totally ' + CoC.player.mf( 'hunky', 'cute' ) + ' AND awesome!</i>"  She spins about, dancing happily and shaking her voluptuous backside in your direction as she goes.' );
		CoC.flags[ kFLAGS.SLEEP_WITH ] = 'Sophie';
		EngineCore.doNext( SceneLib.camp.returnToCampUseOneHour );
	};
	...
	SophieBimbo.prototype.sophiePreggoTitJobs = function() {
		...
		EngineCore.outputText( '\n\nSuddenly, the amber nubs are withdrawn, and you stagger dizzily for a moment, realizing you hadn\'t been breathing this whole time.  Sophie drops to her knees in front of you, supporting you by your [butt].  Her hands squeeze you affectionately as she kisses against your ' + CoC.player.cockHead( x ) + '.' );
		if( CoC.player.cockTotal() > 1 ) {
		...
		if( CoC.player.cocks[ x ].cockLength > 30 ) {
			EngineCore.outputText( 'the underside of your immense erection' );
		} else {
			EngineCore.outputText( 'your ' + CoC.player.cockHead( x ) + '' );
		}
	...
	SophieBimbo.prototype.goHogWildOnSophieBewbs = function() {
		...
		if( CoC.player.cocks[ x ].cockLength <= 30 ) {
			EngineCore.outputText( CoC.player.cockHead( x ) + ' pushes into through her startled cock-pillows, muffling her more effectively than any gag.  She immediately sucks on it, a happy look in her eyes.  The bimbo seems to want nothing more than to lie there and polish your rod.' );
		} else {
		...
		EngineCore.outputText( '.  Your [butt] is squished up against her belly, both of you sweating and slipping all over each other.  Bulging in her breasty embrace, your ' + Descriptors.cockDescript( x ) + ' flexes, and your ' + CoC.player.cockHead( x ) + ' thickens.' );
		if( CoC.player.hasKnot( x ) ) {
		...
		EngineCore.outputText( '  You push your ' + CoC.player.cockHead( x ) + ' and get a kiss from her as you finish cumming, oozing one last dollop onto her fat lips.  She whimpers in the most exciting way as you withdraw, wiping your cock off on her swollen belly, marking her as yours once more.' );
	...
	SophieBimbo.prototype.sophiesEggHatches = function() {
		CoC.flags[ kFLAGS.SOPHIE_CAMP_EGG_COUNTDOWN ] = 0;
		this.sophieSprite();
		...
		if( CoC.flags[ kFLAGS.SOPHIE_ADULT_KID_COUNT ] < 4 ) {
			//First Harpy:;
			...
			if( CoC.flags[ kFLAGS.SOPHIE_ADULT_KID_COUNT ] === 0 ) {
				...
				if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00282 ] > 0 ) {
					CoC.flags[ kFLAGS.DAUGHTER_ONE_BIMBO ] = 1;
				}
			}
			//Second daughter;
			//Second Harpy:;
			//-Badonkadonk to 9000.  Blond feathers that grow very long. (violet eyes);
			else if( CoC.flags[ kFLAGS.SOPHIE_ADULT_KID_COUNT ] === 1 ) {
				...
				if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00282 ] > 0 ) {
					...
					CoC.flags[ kFLAGS.DAUGHTER_TWO_BIMBO ] = 1;
				} else {
			...
			//-Not that curvy, but has human feet and purple feathers. (violet eyes);
			else if( CoC.flags[ kFLAGS.SOPHIE_ADULT_KID_COUNT ] === 2 ) {
				...
				if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00282 ] > 0 ) {
					CoC.flags[ kFLAGS.DAUGHTER_THREE_BIMBO ] = 1;
				}
			}
			//Fourth Harpy:;
			//-Curvy like Sophie, but with well-developed wings and extra-soft feathers (WINGJOBS).  Pink plumage.  (blue eyes);
				...
				if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00282 ] > 0 ) {
					CoC.flags[ kFLAGS.DAUGHTER_FOUR_BIMBO ] = 1;
				}
			}
	...
	SophieBimbo.prototype.sophieKidMaturation = function() {
		...
		if( CoC.flags[ kFLAGS.SOPHIE_ADULT_KID_COUNT ] === 0 ) {
			//First Harpy:;
			//-Pink plumage, big tits and (sensitive) lips, normal harpy hips. (blue eyes);
			if( CoC.flags[ kFLAGS.DAUGHTER_ONE_BIMBO ] === 1 ) {
		...
		//Second daughter;
			...
			//Second Harpy:;
			//-Badonkadonk to 9000.  Blond feathers that grow very long. (violet eyes);
			if( CoC.flags[ kFLAGS.DAUGHTER_TWO_BIMBO ] === 1 ) {
		...
		//Third daughter;
			...
			//Third Harpy:;
			//-Not that curvy, but has human feet and purple feathers. (violet eyes);
			if( CoC.flags[ kFLAGS.DAUGHTER_THREE_BIMBO ] === 1 ) {
		...
		//Fourth daughter;
			...
			//Fourth Harpy:;
			//-Curvy like Sophie, but with well-developed wings and extra-soft feathers (WINGJOBS).  Pink plumage.  (blue eyes);
			if( CoC.flags[ kFLAGS.DAUGHTER_FOUR_BIMBO ] === 1 ) {
		...
		if( this.bimboSophie() ) {
			EngineCore.outputText( '\n\n"<i>Sweetie! Just look at you!  You grew up into one hot piece of ass!  Momma\'s so proud of you!  Baby, isn\'t our little girl like, suuuper sexy?!</i>" Sophie says as she pushes her newly matured daughter at you.  Your happy, curvy daughter wraps her arms around you tightly, smooching you on the cheek as she presses her new body against you.  Her kiss leaves a big, golden lipstick mark on your ' + CoC.player.skinFurScales() + '  "<i>What do you think, daddy? Aren\'t I super-duper sexy?</i>" she asks before leaning in, pushing her big, perky tits against you and making a pouty face at you, showing off her plump kissable lips.' );
		} else {
			EngineCore.outputText( '\n\n"<i>Wow, you grew up into a fine looking harpy!  Momma Sophie\'s so proud of you!  [name], isn\'t our little girl a hottie?!</i>" Sophie says as she pushes her newly matured daughter at you.  Your happy, curvy daughter wraps her arms around you tightly, smooching you on the cheek as she presses her new body against you.  Her kiss leaves a big, golden lipstick mark on your ' + CoC.player.skinFurScales() + '  "<i>What do you think, daddy?  Aren\'t I cute?</i>" she asks before leaning in, pushing her big, perky tits against you and making a pouty face at you, showing off her plump kissable lips.' );
		}
		//REPEATS:;
		if( CoC.flags[ kFLAGS.SOPHIE_ADULT_KID_COUNT ] > 0 ) {
			//If Yes Incest;
			if( CoC.flags[ kFLAGS.SOPHIE_FAMILY_INCEST ] === 1 ) {
				...
				EngineCore.outputText( '.  She beams a radiant smile at you and turns around, shaking her rump at you enticingly, "<i>So, when do you want to try me out, ' + CoC.player.mf( 'Daddy', '\'Mom\'' ) + '?</i>"' );
		...
		CoC.flags[ kFLAGS.SOPHIE_ADULT_KID_COUNT ]++;
	};
	//[yes incest plz];
	SophieBimbo.prototype.yesToSophieIncest = function() {
		...
		EngineCore.dynStats( 'lus', 5 );
		CoC.flags[ kFLAGS.SOPHIE_FAMILY_INCEST ] = 1;
		EngineCore.doNext( SceneLib.camp.returnToCampUseOneHour );
	};
	//[nono];
	...
	SophieBimbo.prototype.sophieAndIzmaPlay = function() {
		EngineCore.outputText( '\n' );
		CoC.flags[ kFLAGS.TIMES_SOPHIE_AND_IZMA_FUCKED ]++;
		...
		if( CoC.flags[ kFLAGS.TIMES_SOPHIE_AND_IZMA_FUCKED ] === 1 ) {
