...
angular.module( 'cocjs' ).run( function( ConsumableLib, Isabella, CockTypesEnum, Combat, Descriptors, AppearanceDefs, EventParser, CoC, kFLAGS, Utils, StatusAffects, EngineCore ) {
	...
	IsabellaFollowerScene.prototype.isabellaFollower = function() {
		return CoC.getInstance().flags[ kFLAGS.ISABELLA_FOLLOWER_ACCEPTED ] === 1 && CoC.getInstance().flags[ kFLAGS.ISABELLA_CAMP_DISABLED ] === 0;
	};
	IsabellaFollowerScene.prototype.isabellaAccent = function() {
		if( CoC.getInstance().flags[ kFLAGS.ISABELLA_ACCENT_TRAINING_PERCENT ] < 100 ) {
			return true;
		}
		if( CoC.getInstance().flags[ kFLAGS.ISABELLA_ACCENT_FORCED_ON ] ) {
			return true;
		}
		return false;
	};
	IsabellaFollowerScene.prototype.isabellaAffection = function( mod ) {
		...
		CoC.getInstance().flags[ kFLAGS.ISABELLA_AFFECTION ] += mod;
		if( CoC.getInstance().flags[ kFLAGS.ISABELLA_AFFECTION ] > 100 ) {
			CoC.getInstance().flags[ kFLAGS.ISABELLA_AFFECTION ] = 100;
		} else if( CoC.getInstance().flags[ kFLAGS.ISABELLA_AFFECTION ] < 0 ) {
			CoC.getInstance().flags[ kFLAGS.ISABELLA_AFFECTION ] = 0;
		}
		return CoC.getInstance().flags[ kFLAGS.ISABELLA_AFFECTION ];
	};
		...
		//Repeat Offer;
			...
			if( CoC.getInstance().player.hasCock() ) {
				if( CoC.getInstance().player.cocks[ CoC.getInstance().player.shortestCockIndex() ].cockLength < 9 ) {
					suck = CoC.getInstance().scenes.isabellaScene.izzyGivesSmallWangsFreeOral;
				}
			}
			EngineCore.choices( 'Talk', CoC.getInstance().scenes.isabellaScene.talkWithIsabella, 'Drink', CoC.getInstance().scenes.isabellaScene.nomOnMommaIzzysTits, 'Get Licked', suck,
				'Fight 4 Rape', CoC.getInstance().scenes.isabellaScene.fightIsabella, 'Offer Oral', CoC.getInstance().scenes.isabellaScene.volunteerToSlurpCowCunt, 'Accept Offer', this.moveTheBitchIn,
				'', null, '', null, '', null, 'Leave', CoC.getInstance().scenes.camp.returnToCampUseOneHour );
		}
		CoC.getInstance().flags[ kFLAGS.ISABELLA_TIMES_OFFERED_FOLLOWER ]++;
	};
	...
	IsabellaFollowerScene.prototype.turnDownIsabellaFollower = function() {
		...
		EngineCore.outputText( 'Isabella sighs and slumps down against her metal defender at your words.  Eventually, she answers, "<i>I... understand, ' + CoC.getInstance().player.short + '.  I shall continue mein lonely vigil.  Vas zere somezing else I could do for you, ja?</i>"\n\n', false );
		var suck = null;
		if( CoC.getInstance().player.hasCock() ) {
			if( CoC.getInstance().player.cocks[ CoC.getInstance().player.shortestCockIndex() ].cockLength < 9 ) {
				suck = CoC.getInstance().scenes.isabellaScene.izzyGivesSmallWangsFreeOral;
		...
		EngineCore.choices( 'Talk', CoC.getInstance().scenes.isabellaScene.talkWithIsabella, 'Drink', CoC.getInstance().scenes.isabellaScene.nomOnMommaIzzysTits, 'Get Licked', suck,
			'Fight 4 Rape', CoC.getInstance().scenes.isabellaScene.fightIsabella, 'Offer Oral', CoC.getInstance().scenes.isabellaScene.volunteerToSlurpCowCunt, '', null,
			'', null, '', null, '', null, 'Leave', CoC.getInstance().scenes.camp.returnToCampUseOneHour );
	};
	//Move Ze Bitch In! ;
	IsabellaFollowerScene.prototype.moveTheBitchIn = function() {
		...
		CoC.getInstance().flags[ kFLAGS.ISABELLA_PLAINS_DISABLED ] = 1;
	...
	IsabellaFollowerScene.prototype.callForFollowerIsabella = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.FOLLOWER_AT_FARM_ISABELLA ] === 0 ) {
			...
			if( this.isabellaAccent() ) {
				EngineCore.outputText( 'Hi ' + CoC.getInstance().player.short + '!  Vat do you need from Izabella?', false );
			} else {
				EngineCore.outputText( 'Hiya ' + CoC.getInstance().player.short + '!  Anything I can help you with?', false );
			}
			EngineCore.outputText( '</i>"', false );
			if( CoC.getInstance().player.hasCock() ) {
				if( CoC.getInstance().player.shortestCockLength() > 9 ) {
					EngineCore.outputText( '\n\nSadly, you\'re too big for Isabella to be interested in providing you with morning blowjobs.' );
				} else {
					if( CoC.getInstance().flags[ kFLAGS.ISABELLA_BLOWJOBS_DISABLED ] === 0 ) {
		...
		var milk = null;
		if( CoC.getInstance().flags[ kFLAGS.ISABELLA_MILKED_YET ] < 0 ) {
			milk = this.getMilk;
		}
		var pro = null;
		if( CoC.getInstance().player.hasItem( ConsumableLib.PROBOVA ) && CoC.getInstance().player.gender > 0 ) {
		...
		EngineCore.choices( 'Accent Coach', accent, 'Get Milk', milk, 'GiveProBova', pro, 'Sex', this.campIzzySexMenu, 'Spar', this.isabellaSparMenu,
			'', null, '', null, '', null, '', null, 'Back', CoC.getInstance().scenes.camp.campLoversMenu );
		if( CoC.getInstance().flags[ kFLAGS.FOLLOWER_AT_FARM_ISABELLA ] === 0 && CoC.getInstance().flags[ kFLAGS.FARM_CORRUPTION_STARTED ] === 1 ) {
	...
	IsabellaFollowerScene.prototype.sendToFarm = function() {
		...
		CoC.getInstance().flags[ kFLAGS.FOLLOWER_AT_FARM_ISABELLA ] = 1;
		EngineCore.doNext( CoC.getInstance().scenes.camp.returnToCampUseOneHour );
	};
	IsabellaFollowerScene.prototype.backToCamp = function() {
		...
		CoC.getInstance().flags[ kFLAGS.FOLLOWER_AT_FARM_ISABELLA ] = 0;
	...
	IsabellaFollowerScene.prototype.campIzzySexMenu = function() {
		EngineCore.spriteSelect( 31 );
		var tentacle = null;
		if( this.izzyTentacleRapeBool() && CoC.getInstance().player.lust >= 33 ) {
		...
		if( CoC.getInstance().player.hasCock() ) {
			...
			if( CoC.getInstance().player.shortestCockLength() <= 9 && CoC.getInstance().player.lust >= 33 ) {
				getSucked = CoC.getInstance().scenes.isabellaScene.izzyGivesSmallWangsFreeOral;
			}
		}
		EngineCore.outputText( '\n\n', false );
		...
		var fuckHer = null;
		if( CoC.getInstance().player.cockThatFits( 164 ) >= 0 && CoC.getInstance().player.lust >= 33 ) {
			fuckHer = this.fuckIsabella;
		}
		EngineCore.choices( bjTogText, bjToggle, 'Drink Milk', CoC.getInstance().scenes.isabellaScene.nomOnMommaIzzysTits, 'Hotdog', hotdog, 'Service Her', CoC.getInstance().scenes.isabellaScene.volunteerToSlurpCowCunt, 'TentacleSex', tentacle,
	...
	IsabellaFollowerScene.prototype.isabellasAccentCoaching = function() {
		...
		/*(req's 100% teach score to replace dialogue.  Success
		 based on inte)*/
		//(FIRST TIME) ;
		if( CoC.getInstance().flags[ kFLAGS.ISABELLA_ACCENT_TRAINING_PERCENT ] === 0 ) {
			...
			CoC.getInstance().flags[ kFLAGS.ISABELLA_ACCENT_TRAINING_PERCENT ] = 1;
		}
		//Autosuccess at inte >= 60);
		//Autofail at inte < 20);
		//50/50 shot at inte 40ish;
		var result = CoC.getInstance().player.inte / 5 + Utils.rand( 9 );
		var accentChange = 0;
		//(Success/Fail texts);
		if( result < 12 ) {
		...
		if( CoC.getInstance().flags[ kFLAGS.ISABELLA_ACCENT_TRAINING_PERCENT ] < 100 && CoC.getInstance().flags[ kFLAGS.ISABELLA_ACCENT_TRAINING_PERCENT ] + accentChange >= 100 ) {
			EngineCore.outputText( '<b>\n\nIsabella proudly says, "<i>I think I\'ve learned everything you have to teach, ' + CoC.getInstance().player.short + '.</i>"  You smile at the progress she\'s made and applaud.  There\'s little else you can teach for her.</b>', false );
		} else if( CoC.getInstance().flags[ kFLAGS.ISABELLA_ACCENT_TRAINING_PERCENT ] < 90 && CoC.getInstance().flags[ kFLAGS.ISABELLA_ACCENT_TRAINING_PERCENT ] + accentChange >= 90 ) {
		...
		} else if( CoC.getInstance().flags[ kFLAGS.ISABELLA_ACCENT_TRAINING_PERCENT ] < 75 && CoC.getInstance().flags[ kFLAGS.ISABELLA_ACCENT_TRAINING_PERCENT ] + accentChange >= 75 ) {
		...
		} else if( CoC.getInstance().flags[ kFLAGS.ISABELLA_ACCENT_TRAINING_PERCENT ] < 50 && CoC.getInstance().flags[ kFLAGS.ISABELLA_ACCENT_TRAINING_PERCENT ] + accentChange >= 50 ) {
		...
		} else if( CoC.getInstance().flags[ kFLAGS.ISABELLA_ACCENT_TRAINING_PERCENT ] < 25 && CoC.getInstance().flags[ kFLAGS.ISABELLA_ACCENT_TRAINING_PERCENT ] + accentChange >= 25 ) {
		...
		} else if( CoC.getInstance().flags[ kFLAGS.ISABELLA_ACCENT_TRAINING_PERCENT ] < 10 && CoC.getInstance().flags[ kFLAGS.ISABELLA_ACCENT_TRAINING_PERCENT ] + accentChange >= 10 ) {
		...
		CoC.getInstance().flags[ kFLAGS.ISABELLA_ACCENT_TRAINING_PERCENT ] += accentChange;
		if( CoC.getInstance().flags[ kFLAGS.ISABELLA_ACCENT_TRAINING_PERCENT ] > 100 ) {
			CoC.getInstance().flags[ kFLAGS.ISABELLA_ACCENT_TRAINING_PERCENT ] = 100;
		}
		//4 to 12 hour cooldown;
		CoC.getInstance().flags[ kFLAGS.ISABELLA_ACCENT_TRAINING_COOLDOWN ] = 4 + Utils.rand( 13 );
		EngineCore.doNext( CoC.getInstance().scenes.camp.returnToCampUseOneHour );
	};
	//Morning Wakeup Call ;
	IsabellaFollowerScene.prototype.isabellaMorningWakeupCall = function() {
		EngineCore.spriteSelect( 31 );
		CoC.getInstance().flags[ kFLAGS.ISABELLA_MORNING_FELLATIO_COUNT ]++;
		var x = CoC.getInstance().player.shortestCockIndex();
		EngineCore.outputText( '\n', false );
		...
		EngineCore.outputText( '  Her massive, malleable tongue moulds around your ' + Descriptors.cockDescript( x ) + ' as she works to enhance your pleasure.  A trickle of warm, creamy milk escapes the heavy jugs resting on your ' + CoC.getInstance().player.legs() + ', showing you just how much she enjoys dispensing oral pleasure.  The tangled, crimson mass of hair slowly begins to bob up and down upon your loins, and Isabella\'s big brown eyes look up at you with radiant happiness, the corners of her mouth curling into a cock-filled smile.\n\n', false );
		...
		EngineCore.outputText( 'milk-puddle around your ' + CoC.getInstance().player.legs() + '.  She sighs and says, "<i>', false );
		if( CoC.getInstance().flags[ kFLAGS.ISABELLA_MORNING_FELLATIO_COUNT ] === 1 ) {
			if( this.isabellaAccent() ) {
	...
	IsabellaFollowerScene.prototype.toggleIsabellasMorningWoodChopping = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.ISABELLA_BLOWJOBS_DISABLED ] === 0 ) {
			...
			CoC.getInstance().flags[ kFLAGS.ISABELLA_BLOWJOBS_DISABLED ] = 1;
		} else {
			EngineCore.outputText( 'You ', false );
			if( CoC.getInstance().player.cor < 33 ) {
			...
			EngineCore.outputText( '</i>"', false );
			CoC.getInstance().flags[ kFLAGS.ISABELLA_BLOWJOBS_DISABLED ] = 0;
		}
		//To Izzy SEX menu;
	...
	IsabellaFollowerScene.prototype.repeatGermanBratwurstInCamp = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.ISABELLA_TIME_SINCE_LAST_HOTDOGGING ] < 5 && CoC.getInstance().flags[ kFLAGS.ISABELLA_TIME_SINCE_LAST_HOTDOGGING ] > 0 ) {
			if( this.isabellaAccent() ) {
		...
		} else if( CoC.getInstance().player.cockArea( x ) > 38 ) {
			...
			if( this.isabellaAccent() ) {
				EngineCore.outputText( 'gleefully muses, "<i>Oooh, I see.  Of course I vill help you, ' + CoC.getInstance().player.short + '!</i>"  She traces a finger around ' + CoC.getInstance().player.oMultiCockDesc() + '\' outline and continues, "<i>It vould be mein pleasure to squeeze it all out into mein caboose.  Zat is what you vant, nein?', false );
			} else {
				EngineCore.outputText( 'gleefully muses, "<i>Oooh, I see.  Of course I\'ll help you, ' + CoC.getInstance().player.short + '!</i>"  She traces a finger around ' + CoC.getInstance().player.oMultiCockDesc() + '\' outline and continues, "<i>It would be my pleasure to squeeze it all out into my caboose.  That is what you want, no?', false );
			}
		}
		EngineCore.outputText( '</i>"\n\n', false );
		if( CoC.getInstance().player.cor < 33 ) {
		...
		if( CoC.getInstance().player.cockArea( x ) <= 38 ) {
		...
		if( this.isabellaAccent() ) {
			EngineCore.outputText( '"<i>So hot... Am I making you hot, ' + CoC.getInstance().player.short + '?  Is your cock twitching for me yet?  Aching and leaking and dribbling a mess all over itself, ja?</i>"\n\n', false );
		} else {
			EngineCore.outputText( '"<i>So hot... Am I making you hot, ' + CoC.getInstance().player.short + '?  Is your cock twitching for me yet?  Aching and leaking and dribbling a mess all over itself?</i>"\n\n', false );
		}

		EngineCore.outputText( 'You nod, catching a whiff of her musky pussy\'s aroma without meaning to.  It\'s not like you could avoid it, with her body mere inches in front of your ' + CoC.getInstance().player.face() + ', shaking so perfectly and so closely that an occasional droplet catches on your nose.  Your hands clamp around the chair with an iron grip as you fight not to rise and rape her tender, teasing asshole on the spot.  ' + Descriptors.SMultiCockDesc() + ' is trickling and dribbling, bouncing with your heartbeats as your ' + CoC.getInstance().player.legs() + ' shiver and shake with denied lusts.  You NEED to do something... to fuck, thrust, or just fap... anything - your ' + Descriptors.multiCockDescriptLight() + ' can\'t take it anymore!\n\n', false );
		...
		if( this.isabellaAccent() ) {
			EngineCore.outputText( '"<i>Good ' + CoC.getInstance().player.mf( 'boy', 'girl' ) + '.  Relax, Momma Isabella vill take care of you.</i>"\n\n', false );
		} else {
			EngineCore.outputText( '"<i>Good ' + CoC.getInstance().player.mf( 'boy', 'girl' ) + '.  Relax, Momma Isabella will take care of you.</i>"\n\n', false );
		}
		...
		if( CoC.getInstance().player.cockArea( x ) <= 38 ) {
			...
			//(if libido <=15:  ;
			if( EngineCore.silly() && (CoC.getInstance().player.lust < 50 || CoC.getInstance().player.lib <= 20) ) {
			...
			EngineCore.outputText( 'You whine, trembling beneath her as she denies you penetration.  Then, she changes her angle and presses down.  Her pillowy cheeks slide around your cock, hugging it tightly as the cow-girl\'s wrinkled pucker comes to rest atop your ' + CoC.getInstance().player.cockHead( x ) + '.  You both gasp at the sudden, electric sensation, immobilized by the new feeling for only the briefest of moments.  Then, Isabella is pressing down, her anus dilating to allow your lust-swollen shaft entrance to her back-door.  Her asshole is even tighter than her butt\'s cleavage was moments before, and as the cinching, puckered ring works its way towards your base, you know you won\'t be able to cum until it loosens a little.\n\n', false );
		...
		//(Big dicks) ;
			...
			if( CoC.getInstance().player.cocks[ x ].cockLength > 48 ) {
				...
				EngineCore.outputText( 'to ' + CoC.getInstance().player.cockHead( x ), false );
			}
		...
		CoC.getInstance().flags[ kFLAGS.ISABELLA_TIME_SINCE_LAST_HOTDOGGING ] = 1;
		CoC.getInstance().flags[ kFLAGS.ISABELLA_TIMES_HOTDOGGED ]++;
		CoC.getInstance().player.orgasm();
	...
	IsabellaFollowerScene.prototype.izzyTentacleRapeBool = function() {
		EngineCore.spriteSelect( 31 );
		return _.filter( CoC.getInstance().player.cocks, function( cock ) {
	...
	IsabellaFollowerScene.prototype.tentacleBoneFollowerIzzy = function() {
		...
		EngineCore.outputText( 'You grin at Isabella as you nonchalantly remove your ' + CoC.getInstance().player.armorName + ', exposing your monstrous tentacle-cocks, fully erect, to her frightened face.  Crossing your arms, you stand before her, dicks squirming all over.  "<i>', false );
		if( this.isabellaAccent() ) {
		...
		//[if demon tongue];
		if( CoC.getInstance().player.tongueType === AppearanceDefs.TONUGE_DEMONIC || CoC.getInstance().player.tongueType === AppearanceDefs.TONUGE_SNAKE || CoC.getInstance().player.tongueType === AppearanceDefs.TONUGE_DRACONIC ) {
		...
		if( CoC.getInstance().player.canFly() && CoC.getInstance().player.str >= 90 ) {
			EngineCore.outputText( 'you descend, setting your ' + CoC.getInstance().player.feet() + ' on the soil again but still holding Isabella herself in the air; ', false );
		}
	...
	IsabellaFollowerScene.prototype.izzyMilkingMeinMilkersMya = function() {
		...
		if( this.isabellaAccent() ) {
			EngineCore.outputText( '"<i>Ohh, ' + CoC.getInstance().player.short + ', I had forgotten all about zose milkers vith all ze verk around camp!  Yes, let us go zere immediately!</i>"\n\n', false );
		} else {
			EngineCore.outputText( '"<i>Ohh, ' + CoC.getInstance().player.short + ', I had forgotten all about those milkers with all the work around camp!  Yes, let us go there immediately!</i>"\n\n', false );
		}
	...
	IsabellaFollowerScene.prototype.izzyMilkingMeinMilkersMya2 = function() {
		...
		if( this.isabellaAccent() ) {
			EngineCore.outputText( 'The warrioress nods emphatically.  "<i>Ja, it vould help sooo much vith how busy I am now!  Is zis okay, ' + CoC.getInstance().player.short + '?  For me to visit once a day und relieve the pressure on mein milkers?</i>"\n\n', false );
		} else {
			EngineCore.outputText( 'The warrioress nods emphatically.  "<i>Yes, it would help sooo much with how busy I am now!  Is this okay, ' + CoC.getInstance().player.short + '?  For me to visit once a day and relieve the pressure on my milkers?</i>"\n\n', false );
		}
	...
	IsabellaFollowerScene.prototype.angryMurble = function() {
		...
		CoC.getInstance().flags[ kFLAGS.ISABELLA_MURBLE_BLEH ] = 2;
		EngineCore.doNext( EventParser.playerMenu );
	};
	//Sparring;
	...
	IsabellaFollowerScene.prototype.isabellaSparMenu = function() {
		EngineCore.spriteSelect( 31 );
		EngineCore.outputText( '', true );
		if( CoC.getInstance().flags[ kFLAGS.ISABELLA_SPARRING_INTRO ] === 0 ) {
			...
			CoC.getInstance().flags[ kFLAGS.ISABELLA_SPARRING_INTRO ] = 1;
		} else {
	...
	IsabellaFollowerScene.prototype.sparring = function( type ) {
		...
		EngineCore.spriteSelect( 31 );
		EngineCore.outputText( '', true );
		if( CoC.getInstance().flags[ kFLAGS.ISABELLA_SPARRING_INTRO ] === 1 ) {
			...
			CoC.getInstance().flags[ kFLAGS.ISABELLA_SPARRING_INTRO ] = 2;
		} else {
		...
		EngineCore.outputText( 'Isabella lifts her shield and raps her knuckles against the solid steel, making a loud, gong-like sound that carries for some distance.  You raise your ' + CoC.getInstance().player.weaponName + ' and prepare to fight.  It\'s on!', false );
		if( type === 1 ) {
		...
		CoC.getInstance().monster.createStatusAffect( StatusAffects.Sparring, type, 0, 0, 0 );
		//No gems.;
		CoC.getInstance().monster.gems = 0;
		//Nerf XP if light mode;
		if( type === 2 ) {
			CoC.getInstance().monster.XP = Math.round( CoC.getInstance().monster.XP / 2 );
		}
		EngineCore.spriteSelect( 31 );
	};
	//Isabella Burps;
	...
	IsabellaFollowerScene.prototype.isabellaBurps = function() {
		...
		EngineCore.outputText( '', true );
		//First time;
		if( CoC.getInstance().flags[ kFLAGS.ISABELLA_PROBOVA_BURP_COUNT ] === 0 ) {
			if( this.isabellaAccent() ) {
	...
	IsabellaFollowerScene.prototype.getIzzyBurped = function() {
		EngineCore.outputText( '', true );
		EngineCore.spriteSelect( 31 );
		CoC.getInstance().flags[ kFLAGS.ISABELLA_PROBOVA_BURP_COUNT ]++;
		//First time;
		if( CoC.getInstance().flags[ kFLAGS.ISABELLA_PROBOVA_BURP_COUNT ] === 1 ) {
			...
			if( CoC.getInstance().player.lowerBody !== AppearanceDefs.LOWER_BODY_TYPE_HOOFED || CoC.getInstance().player.horns <= 0 || CoC.getInstance().player.hornType !== AppearanceDefs.HORNS_COW_MINOTAUR ) {
				EngineCore.outputText( '  With that whole affair over with, you go to stand... and find yourself falling once more.', false );
			}
			if( CoC.getInstance().player.lowerBody !== AppearanceDefs.LOWER_BODY_TYPE_HOOFED ) {
				...
				if( CoC.getInstance().player.isTaur() ) {
					...
					EngineCore.outputText( '  Confused, you quickly inspect yourself before your gaze settles on your ' + CoC.getInstance().player.feet() + ' – or should you say, former ' + CoC.getInstance().player.feet() + '.  Triggered by what you surmise are the Pro Bova\'s fumes, your ' + CoC.getInstance().player.feet() + ' have quickly and painlessly refashioned themselves into cloven hooves – not unlike those Isabella is currently sporting.', false );
				}
				changed = true;
			}
			//[if no cow horns present];
			if( CoC.getInstance().player.horns <= 0 || CoC.getInstance().player.hornType !== AppearanceDefs.HORNS_COW_MINOTAUR ) {
				if( CoC.getInstance().player.horns > 0 && CoC.getInstance().player.hornType > AppearanceDefs.HORNS_NONE ) {
					EngineCore.outputText( '  Your existing horns quiver, then reform into those of a ' + CoC.getInstance().player.mf( 'bull', 'cow' ), false );
				} else {
			...
			//[if no cow ears];
			if( CoC.getInstance().player.earType !== AppearanceDefs.EARS_COW ) {
			...
			//[if no cow tail];
			if( CoC.getInstance().player.tailType !== AppearanceDefs.TAIL_TYPE_COW ) {
				if( CoC.getInstance().player.tailType > AppearanceDefs.TAIL_TYPE_NONE ) {
					...
					EngineCore.outputText( '  A tail creeps out of your ' + CoC.getInstance().player.armorName + ', perking up and growing rapidly.  A flare of fur bursts from the tip, the rest of the length changing to match.', false );
				}
				changed = true;
			}
			//([Female/herm: if breasts smaller than G-cup and not lactating a whole lot];
			if( CoC.getInstance().player.hasVagina() && CoC.getInstance().player.biggestTitSize() < 15 && CoC.getInstance().player.biggestLactation() < 2 ) {
				EngineCore.outputText( '  Unbeknownst to you, Isabella\'s potent cow-gas has gone to town on your ' + Descriptors.breastDescript( 0 ) + ', growing them to an obnoxious G-cup... no, not exactly growing, you realize, but filling... you give one of your swollen teats a squeeze, not quite surprised by the gout of milk that stains your ' + CoC.getInstance().player.armorName + '.\n\n', false );
				changed = true;
			}
			//([Female/herm: if breasts >G-cup and lactating];
			else if( CoC.getInstance().player.hasVagina() && CoC.getInstance().player.biggestLactation() >= 2 ) {
				EngineCore.outputText( '  Propped up as you are with your soft ' + Descriptors.breastDescript( 0 ) + ' nestling under your chin, you can easily see the growing puddle of alabaster liquid seeping into the ground.  You heave yourself up, the effort rewarded with a jet of milk shot into the air as your evidently over-laden boobs jostle about inside your ' + CoC.getInstance().player.armorName + '. It seems the effects of Isabella\'s gas has ramped your lactation to overdrive.', false );
				changed = true;
			}
			//both cases, flavor text;
			//[Male/herm: if penis <18 inches & testicles smaller than however long in diameter a baseball is];
			if( CoC.getInstance().player.hasCock() && CoC.getInstance().player.longestCockLength() < 18 ) {
				EngineCore.outputText( '  A pressure builds in your crotch, at first subtle, then incredibly obnoxious to the point where you\'re forced to undo the lower portion of your ' + CoC.getInstance().player.armorName + ' to seek some sort of relief.  Your shock is total as you stare at your former ' + Descriptors.cockDescript( 0 ) + ', now measuring at least 18 inches long – a condition that Isabella has already noticed and is clearly formulating a gripe about.', false );
				if( CoC.getInstance().player.balls < 0 ) {
				...
				} else if( CoC.getInstance().player.ballSize < 4 ) {
			...
			if( changed && CoC.getInstance().player.hasVagina() && CoC.getInstance().player.race() !== 'cow-girl' ) {
			...
			else if( changed && CoC.getInstance().player.hasCock() && CoC.getInstance().player.race() !== 'cow-morph' ) {
				EngineCore.outputText( '  "<i>', false );
				if( this.isabellaAccent() ) {
			...
			if( changed ) {
				CoC.getInstance().player.createStatusAffect( StatusAffects.BurpChanged, 0, 0, 0, 0 );
			}
		} else {
			...
			if( CoC.getInstance().player.lowerBody !== AppearanceDefs.LOWER_BODY_TYPE_HOOFED || CoC.getInstance().player.horns <= 0 || CoC.getInstance().player.hornType !== AppearanceDefs.HORNS_COW_MINOTAUR || CoC.getInstance().player.earType !== AppearanceDefs.EARS_COW || CoC.getInstance().player.tailType !== AppearanceDefs.TAIL_TYPE_COW || (CoC.getInstance().player.hasCock() && CoC.getInstance().player.longestCockLength() < 18) ) {
				EngineCore.outputText( 'You\'re not surprised when you take the now-familiar cow-' + CoC.getInstance().player.mf( 'boy', 'girl' ) + ' form, ', false );
				if( (!CoC.getInstance().player.hasCock() || Utils.rand( 2 ) === 0) && CoC.getInstance().player.hasVagina() ) {
	...
	IsabellaFollowerScene.prototype.declineIzzysCowBurpApology = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.ISABELLA_PROBOVA_BURP_COUNT ] === 1 ) {
			...
			if( CoC.getInstance().player.findStatusAffect( StatusAffects.BurpChanged ) >= 0 ) {
				...
				CoC.getInstance().player.removeStatusAffect( StatusAffects.BurpChanged );
			}
		}
		//no;
		else {
	...
	IsabellaFollowerScene.prototype.fuckIsabella = function() {
		...
		if( CoC.getInstance().player.cocks[ x ].cockLength >= 9 ) {
			x = CoC.getInstance().player.cockThatFits( 164 );
		}
		var y = x + 1;
	...
	IsabellaFollowerScene.prototype.findIzzyMilking = function() {
		...
		EngineCore.menu();
		CoC.getInstance().flags[ kFLAGS.FOUND_ISABELLA_AT_FARM_TODAY ] = 1;
		//[Fuck her] [Leave];
	...
	IsabellaFollowerScene.prototype.fuckIsabellaInTheBarn = function() {
		...
		EngineCore.outputText( '\n\nWith each of your movements, Isabella\'s cheeks clench, tormenting the ' + Descriptors.cockDescript( x ) + ' trapped between your belly and her backside.  Your hips move slowly against her, gently pushing into her soft, forgiving flesh before leaning back until just the ' + CoC.getInstance().player.cockHead( x ) + ' remains caught in her tender embrace.  Between kisses and caresses, you hear your love begging for more, for you to slide your thick shaft into her eager hole, but not yet...  not yet...' );
	...
	IsabellaFollowerScene.prototype.isabellaBarnFuckPartII = function() {
		...
		if( CoC.getInstance().player.cocks[ CoC.getInstance().player.smallestCockIndex() ].cockLength < 9 ) {
		...
		//{If medium cock};
		else if( CoC.getInstance().player.smallestCockArea() < 40 ) {
		...
		if( CoC.getInstance().player.smallestCockArea() <= 164 ) {
	...
	CoC.getInstance().registerScene( 'isabellaFollowerScene', new IsabellaFollowerScene() );
} );