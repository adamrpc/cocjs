...
angular.module( 'cocjs' ).run( function( MainView, SceneLib, ConsumableLib, Isabella, CockTypesEnum, Combat, Descriptors, AppearanceDefs, CoC, kFLAGS, Utils, StatusAffects, EngineCore ) {
	...
	IsabellaFollowerScene.prototype.isabellaMoovesInGreeting = function() {
		...
		if( CoC.flags[ kFLAGS.ISABELLA_TIMES_OFFERED_FOLLOWER ] === 0 ) {
			...
			if( CoC.player.cor < 33 ) {
				MainView.outputText( 'smile, and admit that you do enjoy the time you spend with her.', false );
			} else if( CoC.player.cor < 66 ) {
				MainView.outputText( 'grin and joke that she probably missed you more.', false );
			} else {
				MainView.outputText( 'smirk and wonder if you missed her or her magnificent body.', false );
			}
			MainView.outputText( '  Isabella takes a deep breath before rising to her full, seven-foot-plus height. "<i>' + CoC.player.short + ', I have thought long and hard on zis,</i>" she begins.  "<i>I vould like to join you at your camp and assist you on your quest.  It seems a worthy cause.</i>"  The tall cow-girl brushes her unruly red locks back and admits, "<i>I could use ze companionship...</i>"\n\n', false );
			MainView.outputText( 'Well, you didn\'t expect that.  While you muse over the idea, Isabella sashays up to give you a hug.  Her tits ', false );
			if( CoC.player.tallness < 80 ) {
				MainView.outputText( 'crush around your head, smothering you in heavy, milk-filled breast flesh', false );
			} else {
				MainView.outputText( 'squeeze against your ' + Descriptors.chestDesc() + ', the four hard nipples capping each milk-flooded breast pressing on your own.', false );
			}
			MainView.outputText( '  You blush and squirm a little from the dark-hued warrioress\'s affections, yet you endure them all the same.\n\n', false );
			MainView.outputText( 'Do you invite her to camp?', false );
			EngineCore.doYesNo( this, this.moveTheBitchIn, this, this.turnDownIsabellaFollower );
		}
		//Repeat Offer;
		else {
			MainView.outputText( 'You stumble into Isabella\'s camp and find her hard at work on her shield.  Currently she\'s banging dents out of it with a huge hammer.  She barely looks up when you arrive, but from what you can see, the corners of the cow-girl\'s mouth are curled up in a warm smile.  The dark-skinned beauty pauses long enough look up and ask, "<i>Have you thought about mein offer?  Ve could do more good working together zen apart.</i>"  She finishes her task and jams the edge of the shield into the ground, the hunk of metal vibrating slightly from the sudden impact.\n\n', false );
			MainView.outputText( 'Isabella still seems to think that moving in would be a good idea; do you accept her proposal, or demur for now?  She doesn\'t seem too dead-set on the idea.', false );
	...
	IsabellaFollowerScene.prototype.turnDownIsabellaFollower = function() {
		...
		MainView.outputText( 'Isabella sighs and slumps down against her metal defender at your words.  Eventually, she answers, "<i>I... understand, ' + CoC.player.short + '.  I shall continue mein lonely vigil.  Vas zere somezing else I could do for you, ja?</i>"\n\n', false );
		var suck = null;
		if( CoC.player.hasCock() ) {
			if( CoC.player.cocks[ CoC.player.shortestCockIndex() ].cockLength < 9 ) {
				suck = SceneLib.isabellaScene.izzyGivesSmallWangsFreeOral;
	...
	IsabellaFollowerScene.prototype.moveTheBitchIn = function() {
		...
		if( SceneLib.camp.hasCompanions() ) {
			MainView.outputText( 'newest ', false );
		}
		MainView.outputText( 'camp follower gather and move her possessions to your camp.  It takes the better part of an hour, but the entire decor is coming with you, so it may just be worth it.\n\n', false );
	...
	IsabellaFollowerScene.prototype.callForFollowerIsabella = function() {
		...
		if( CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_ISABELLA ] === 0 ) {
			...
			MainView.outputText( 'Isabella puts down the milk canister she’s carrying and wipes her brow at your approach.' );
			if( this.isabellaAccent() ) {
	...
	IsabellaFollowerScene.prototype.sendToFarm = function() {
		...
		this.isabellaSprite();
		MainView.outputText( '“<i>Do you think you could do me a favor?</i>” you say to the Teutonic tit-monster. “<i>There’s a farm near here, down by the lake. I need anyone who is strong, and vigilant, and... has a lot of milk...</i>” To your relief, Isabella responds well to the idea.' );
		if( this.isabellaAccent() ) {
	...
	IsabellaFollowerScene.prototype.backToCamp = function() {
		MainView.clearOutput();
		this.isabellaSprite();
		MainView.outputText( '“<i>Could you head back to camp? It’s just... it’s getting a little lonely at night up there.</i>” Your gambit pays off and Isabella pinches your cheek in delight.' );
		if( this.isabellaAccent() ) {
	...
	IsabellaFollowerScene.prototype.campIzzySexMenu = function() {
		...
		MainView.outputText( '\n\n', false );
		MainView.outputText( 'You could get a drink of Isabella\'s delicious milk, straight from the tap.', false );
		if( tentacle !== null ) {
			MainView.outputText( '  Or you could take advantage of your numerous tentacles to show her a truly grand time.', false );
		}
		if( getSucked !== null ) {
			MainView.outputText( '  Or you could have her suck you off now, since she seems quite taken with the size of your penis.', false );
		}
		MainView.outputText( '  There\'s always the opportunity to climb under her skirt and service her as well.', false );
		if( CoC.player.lust < 33 ) {
			MainView.outputText( '  <b>You aren\'t aroused enough to initiate sex.</b>', false );
		}
	...
	IsabellaFollowerScene.prototype.isabellasAccentCoaching = function() {
		...
		if( CoC.flags[ kFLAGS.ISABELLA_ACCENT_TRAINING_COOLDOWN ] > 1 ) {
			MainView.outputText( 'Isabella shakes her head and says, "<i>Nein.  I do not vish to spend time on zis now.</b>"', false );
		...
		if( CoC.flags[ kFLAGS.ISABELLA_ACCENT_TRAINING_PERCENT ] === 0 ) {
			MainView.outputText( 'You tell Isabella you\'d like to offer her lessons in order to reduce her accent and make her more understandable.  She folds her arms across her chest and protests, "<i>You have an accent too, nein?</i>"\n\n', false );
			MainView.outputText( 'Chuckling, a little, you nod, but note that yours is far closer to the people of this land than her own.  She agrees, albeit reluctantly.\n\n', false );
		...
		if( result < 12 ) {
			...
			if( choice1 === 0 ) {
				MainView.outputText( 'You spend an hour speaking with Isabella, but ultimately, you wind up making little, if any, progress.  The cow-girl seems a bit dispirited at the failure, but she gives you a big hug anyway.', false );
			}
			//(FAIL2) ;
			else if( choice1 === 1 ) {
				MainView.outputText( 'In spite of your efforts to teach the foreign cow-girl, she doesn\'t seem to make any progress.  The worst part is, she keeps slumping down when you correct her, which only sets her bosom to jiggling, her cleavage looking larger than ever before.  You wind up quite distracted by the time the two of you get finished.', false );
				EngineCore.dynStats( 'lus', 10 );
			}
			//(FAIL3) ;
			else {
				MainView.outputText( 'No matter what you do, you can\'t get the normally-industrious cow-girl to focus right now.  She\'s obviously bored with your attempts to teach her, and she spends the entire lesson trying to tease you with her body.  It\'s more effective than you care to admit', false );
				if( CoC.player.hasCock() ) {
					MainView.outputText( ', and you leave the lesson with certain stiffness in your loins.', false );
				} else if( CoC.player.hasVagina() ) {
					MainView.outputText( ', and you leave the lesson with wet panties.', false );
				} else {
			...
			if( Utils.rand( 2 ) === 0 ) {
				MainView.outputText( 'You spend an hour going over Isabella\'s language and diction with her.  She seems to make a little progress with it, which makes her happy, but you feel that in the grand scheme of things, she could do better.', false );
			} else {
				MainView.outputText( 'You go over Isabella\'s word choice with her and try to correct her whenever she uses the wrong word or slips into her native tongue.  She seems to make a modicum of progress, yet the downward cast of her eyes shows just how much better she feels she could have done.', false );
			}
			accentChange = 4 + Utils.rand( 5 );
		} else if( result <= 20 ) {
			if( Utils.rand( 2 ) === 0 ) {
				MainView.outputText( 'You spend an hour trying to teach Isabella how to speak properly.  While she is often flustered by her mistakes, she makes notable improvements from this session.  By the time you two finish, she seems quite pleased with herself.', false );
			} else {
				MainView.outputText( 'You go over Isabella\'s word choice with her and try to correct her whenever she uses the wrong word or slips into her native tongue.  It takes some time, but you\'re happy to tell Isabella that she seems to be making significant progress.', false );
			}
			accentChange = 8 + Utils.rand( 8 );
		} else {
			if( Utils.rand( 2 ) === 0 ) {
				MainView.outputText( 'You sit down with Isabella to work on her language once again.  She gets quite involved in the lesson, and the cow-girl makes large strides toward lessening her accent.', false );
			} else {
				MainView.outputText( 'You go over Isabella\'s word choice with her in an effort to reduce her accent.  In short order, the cow-girl is looking at you with rapt, attentive eyes, hanging on to your every word as you help her disguise her tongue to match those she\'s surrounded by.', false );
			}
		...
		if( CoC.flags[ kFLAGS.ISABELLA_ACCENT_TRAINING_PERCENT ] < 100 && CoC.flags[ kFLAGS.ISABELLA_ACCENT_TRAINING_PERCENT ] + accentChange >= 100 ) {
			...
			MainView.outputText( '<b>  Isabella can often speak without her accent when she tries to, though she doesn\'t seem to be doing so in everyday conversation yet.  She\'s so close though!</b>', false );
			...
			MainView.outputText( '<b>  Isabella will sometimes manage to say a sentence or two without a single mistake during the lessons.  It\'s good progress!</b>', false );
			...
			MainView.outputText( '<b>  Isabella has gained enough control over her language to speak properly... if she talks about half as fast as normal.</b>', false );
			...
			MainView.outputText( '<b>  Isabella is getting better during your lessons.  Generally she can pick out most of her mistakes if she pauses to think over what she just said.</b>', false );
			...
			MainView.outputText( '<b>  Isabella has made enough progress that if she thinks about it, she will sometimes catch her mistakes on her own.</b>', false );
		}
	...
	IsabellaFollowerScene.prototype.isabellaMorningWakeupCall = function() {
		...
		MainView.outputText( '  Isabella brushes her hair back to watch your reaction; her shining eyes never leave your face as she kicks her efforts into overdrive, pumping and stroking your ' + Descriptors.cockDescript( x ) + ' with nothing more than her tongue\'s moist heat.\n\n', false );
		MainView.outputText( 'Isabella hums softly, more a feeling of slight, slow vibration than any proper sound.  Her sensual purr combines with the fevered pumping of her tongue to push you far beyond your limit.  Starting deep inside you, your body submits to her pleasure and releases the warmth of your orgasm.', false );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( '  Your balls clench tightly as they release your seed', false );
		} else {
			MainView.outputText( '  You clench tightly and release your seed', false );
		}
		MainView.outputText( ' to the cow-girl\'s vacuum-tight lips, ejaculating a nice, thick load of spooge to fill her hungry mouth.', false );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( '  At the same time, you cum from ', false );
			if( CoC.player.cockTotal() > 2 ) {
				MainView.outputText( 'the rest of your ' + Descriptors.multiCockDescriptLight(), false );
			} else {
				MainView.outputText( 'the one penis she\'s left untouched', false );
			}
			MainView.outputText( ', spraying across your belly and ' + Descriptors.chestDesc() + '.  Isabella, ever attentive, grabs the twitching cock', false );
			if( CoC.player.cockTotal() > 2 ) {
				MainView.outputText( 's', false );
			}
			MainView.outputText( ' and pumps to help ease the lusty tension from your body.', false );
		}
		if( CoC.player.cumQ() < 250 ) {
			MainView.outputText( '  She slurps it all down with a smile, giving you a happy wink as her throat swallows the last of your salty load.', false );
		} else if( CoC.player.cumQ() < 500 ) {
			MainView.outputText( '  She swallows hard, but even with her talents, Isabella can\'t quite get it all down.  The cum-dribbling red-head gives you a wink as you finish, gathering the spooge with her fingers to swallow down the excess.', false );
		} else {
			MainView.outputText( '  She gurgles from the volume of jizz suddenly forced into her mouth, spunk spurting out from the corners of her stuffed mouth.  The prodigious cum-shot doesn\'t seem to phase her in the least, as she simply forces herself down harder and relaxes her throat, allowing you to deposit your cum directly into her quickly-rounding belly.', false );
			if( CoC.player.cumQ() >= 2000 ) {
				MainView.outputText( '  Even that fails when Isabella\'s body becomes too semen-stuffed to take any more, and the cow-girl pulls you out of her mouth to let you spray the rest on her face.  Her tongue strokes you like a hand throughout the entire thing, encouraging you to make as big of a mess as possible.', false );
			}
			MainView.outputText( '  Once you finish, she gives you a wink and slurps up some of the cum that escaped.', false );
		}
		MainView.outputText( '\n\n', false );
		MainView.outputText( 'The orally-fixated red-head shudders and quietly moans as she rises, her pleasure evident on her stained fingers as they withdraw from under her rumpled skirt.  Isabella\'s silky white top is utterly soaked with sweet-smelling cream, an aroma echoed by the ', false );
		if( CoC.player.cumQ() >= 2000 ) {
			MainView.outputText( 'spunk-tainted ', false );
		}
		MainView.outputText( 'milk-puddle around your ' + CoC.player.legs() + '.  She sighs and says, "<i>', false );
		if( CoC.flags[ kFLAGS.ISABELLA_MORNING_FELLATIO_COUNT ] === 1 ) {
			if( this.isabellaAccent() ) {
				MainView.outputText( 'Mmm, your penis has such tasty treats for me.  Keep zat cock small enough for mein mouth and I vill be happy to tend to it every morning, ja?', false );
			} else {
				MainView.outputText( 'Mmm, your penis has such tasty treats for me.  Keep that cock small enough for my mouth and I\'ll be happy to tend to it every morning, okay?', false );
			}
		} else if( CoC.player.cumQ() >= 500 ) {
			if( this.isabellaAccent() ) {
				MainView.outputText( 'Yum!  You cum so hard for momma Izabella, don\'t you?</i>"  The sated cow-girl gives your ' + Descriptors.cockDescript( x ) + ' an affectionate squeeze before murmuring, "<i>So yummy...', false );
			} else {
				MainView.outputText( 'Yum!  You cum so hard for momma Isabella, don\'t you?</i>"  The sated cow-girl gives your ' + Descriptors.cockDescript( x ) + ' an affectionate squeeze before murmuring, "<i>So yummy...', false );
			}
		} else {
			if( this.isabellaAccent() ) {
				MainView.outputText( 'I hope you did not mind me getting zat snack...', false );
			} else {
				MainView.outputText( 'I hope you didn\'t mind me getting that snack...', false );
			}
		}
		MainView.outputText( '</i>"\n\n', false );
		MainView.outputText( 'You slump down while the tension oozes from your frame.  By the time you can move again, Isabella has walked off towards her section of camp.  Is it just you, or does her ass have a particularly sensuous sway this morning?', false );
		CoC.player.orgasm();
	...
	IsabellaFollowerScene.prototype.toggleIsabellasMorningWoodChopping = function() {
		...
		if( CoC.flags[ kFLAGS.ISABELLA_BLOWJOBS_DISABLED ] === 0 ) {
			...
			if( this.isabellaAccent() ) {
				MainView.outputText( 'Just let Isabella know if you change your mind, ja?</i>"', false );
			} else {
				MainView.outputText( 'Just let me know if you change your mind, all right?</i>"', false );
			}
			CoC.flags[ kFLAGS.ISABELLA_BLOWJOBS_DISABLED ] = 1;
		} else {
			MainView.outputText( 'You ', false );
			if( CoC.player.cor < 33 ) {
				MainView.outputText( 'blush and ', false );
			} else if( CoC.player.cor >= 66 ) {
				MainView.outputText( 'smirk and ', false );
			}
			MainView.outputText( 'let Isabella know that if she\'s still interested, you\'d like her to suck you off in the mornings again.  Her face colors, but you see her eight nipple-tips poking hard at her silk shirt as she answers, "<i>', false );
			if( CoC.player.cocks[ CoC.player.shortestCockIndex() ].cockLength > 9 ) {
				if( this.isabellaAccent() ) {
					MainView.outputText( 'I vill, but only once you shrink zat monster down.  I cannot handle somezing like zat!', false );
				} else {
					MainView.outputText( 'I will, but only once you shrink that monster down.  I can\'t handle something like that!', false );
				}
			} else {
				if( this.isabellaAccent() ) {
					MainView.outputText( 'Of course; ve vouldn\'t vant you to be out zere battling demons unsated.  Who knows vat vould happen?', false );
				} else {
					MainView.outputText( 'Of course; we wouldn\'t want you to be out there battling demons unsated.  Who knows what would happen?', false );
				}
			}
			MainView.outputText( '</i>"', false );
	...
	IsabellaFollowerScene.prototype.repeatGermanBratwurstInCamp = function() {
		...
		if( CoC.flags[ kFLAGS.ISABELLA_TIME_SINCE_LAST_HOTDOGGING ] < 5 && CoC.flags[ kFLAGS.ISABELLA_TIME_SINCE_LAST_HOTDOGGING ] > 0 ) {
			if( this.isabellaAccent() ) {
				MainView.outputText( 'mumbles, "<i>Already?  Did I not just take care of you?</i>"  The blushing cow-girl teases, "<i>Are you really zat horny, or do you just like mein heiny that much?', false );
			} else {
				MainView.outputText( 'mumbles, "<i>Already?  Didn\'t I just take care of you?</i>"  The blushing cow-girl teases, "<i>Are you really that horny, or do you just like my heiny that much?', false );
			}
		} else if( CoC.player.cockArea( x ) > 38 ) {
			//If too big, use the biggest dick for hotdoggin!;
			x = CoC.player.biggestCockIndex();
			if( this.isabellaAccent() ) {
				MainView.outputText( 'gasps, "<i>Oh yes, I see.  It looks so pent up... a shame it is too big to get ze full treatment in mein caboose.  You vill still cum for mein cheeks though, ja?', false );
			} else {
				MainView.outputText( 'gasps, "<i>Oh yes, I see.  It looks so pent up... a shame it\'s too big for you to get the full treatment in my caboose.  You\'ll still cum for my cheeks though, right?', false );
			}
		} else {
			if( this.isabellaAccent() ) {
				MainView.outputText( 'gleefully muses, "<i>Oooh, I see.  Of course I vill help you, ' + CoC.player.short + '!</i>"  She traces a finger around ' + CoC.player.oMultiCockDesc() + '\' outline and continues, "<i>It vould be mein pleasure to squeeze it all out into mein caboose.  Zat is what you vant, nein?', false );
			} else {
				MainView.outputText( 'gleefully muses, "<i>Oooh, I see.  Of course I\'ll help you, ' + CoC.player.short + '!</i>"  She traces a finger around ' + CoC.player.oMultiCockDesc() + '\' outline and continues, "<i>It would be my pleasure to squeeze it all out into my caboose.  That is what you want, no?', false );
			}
		}
		MainView.outputText( '</i>"\n\n', false );
		if( CoC.player.cor < 33 ) {
			MainView.outputText( 'Coloring red, you sheepishly answer in the affirmative.', false );
		} else if( CoC.player.cor < 66 ) {
			MainView.outputText( 'Not seeing a reason to hide it, you admit as much.', false );
		} else {
			MainView.outputText( 'With a knowing twinkle in your eye, you agree with the busty woman\'s intuition, rocking your hips enticingly.  Her eyes glue to your manhood for a moment, before she remembers herself and meets your gaze.', false );
		}
		MainView.outputText( '  Isabella folds her arms across her belly, putting just enough pressure on the sides of her mountainous melons to make them jiggle and swell up inside her top.  The cleavage seems on the verge of busting from her top by the time she laughs, "<i>Eyes up here, big ' + CoC.player.mf( 'boy', 'girl' ) + '.</i>"  Like a kid caught with his hand in the cookie jar, you ', false );
		if( CoC.player.cor < 33 ) {
			MainView.outputText( 'feel the weight of shame settling over you', false );
		} else if( CoC.player.cor < 66 ) {
			MainView.outputText( 'feel mildly ashamed', false );
		} else {
			MainView.outputText( 'color at getting caught staring', false );
		}
		MainView.outputText( ', but Isabella doesn\'t dwell on it.  She pivots her lush hips around to present her backside, lifts her skirt to reveal her bare bottom, and begins to flex her muscles, causing the creamy dark flesh to shake and jiggle.\n\n', false );
		MainView.outputText( '"<i>Are you going to stare, or undress for me?</i>" the cow-girl asks, never slowing down with her skilled, teasing jiggles.  You undress, but so long as she sways like that, your eyes stay firmly locked on delicious, swollen rump.  Your ' + CoC.player.armorName + ' falls in a pile behind you, finally releasing ' + Descriptors.sMultiCockDesc() + ' to taste the air.  The sumptuous, sweat-slicked cheeks wobble against each other in a way that promises a heaven of sensation for anyone lucky enough to slip into their capacious crack-cleavage.  You can\'t help but feel elated when you realize she\'s going to let you feel that.\n\n', false );
		if( this.isabellaAccent() ) {
			MainView.outputText( 'Isabella crooks a finger at you and commands, "<i>Come, sit down, but lean back so Isabella has plenty of room to vork at milking zis naughty ', false );
		} else {
			MainView.outputText( 'Isabella crooks a finger at you and commands, "<i>Come, sit down, but lean back so I have plenty of room to work at milking this naughty ', false );
		}
		if( CoC.player.cockArea( x ) <= 38 ) {
			MainView.outputText( 'little ', false );
		}
		MainView.outputText( 'snake...</i>"\n\n', false );
		MainView.outputText( 'Approaching slowly, you drink in your cow-girl companion\'s curvy body, devouring every detail of her body.  From her curvy thighs and bubblelicious ass to her pale, creamy spots, her body seems sculpted to titillate any eyes lucky enough to drink in her fertile frame.  Isabella swivels her shaking derriere around, slapping her bronzed ass against you.  The surprising impact pushes you down into the camp chair she previously indicated, teasing you from the brief skin-to-crotch contact as she takes complete charge of the situation.  ' + Descriptors.SMultiCockDesc() + ' stands out from your loins, pulsing hard, just aching for another touch of Isabella\'s bottom.\n\n', false );
		MainView.outputText( 'The confident woman continues her gyrations, knowing all too well the effect she\'s having on you.  "<i>No, not yet,</i>" she commands as her sweat-dripping butt sways dangerously close to ' + CoC.player.oMultiCockDesc() + '.  Isabella raises her marvelous backside higher, bringing it up in front of your face.  She wiggles slowly before spreading her legs wide, allowing you to see her glistening vagina and tightly-clenched pucker peaking past her dusky cheeks.  ', false );
		if( this.isabellaAccent() ) {
			MainView.outputText( '"<i>So wet,</i>" ', false );
		} else {
			MainView.outputText( '"<i>So vet,</i>" ', false );
		}
		MainView.outputText( 'she moans before resuming her shaking, ', false );
		if( this.isabellaAccent() ) {
			MainView.outputText( '"<i>So hot... Am I making you hot, ' + CoC.player.short + '?  Is your cock twitching for me yet?  Aching and leaking and dribbling a mess all over itself, ja?</i>"\n\n', false );
		} else {
			MainView.outputText( '"<i>So hot... Am I making you hot, ' + CoC.player.short + '?  Is your cock twitching for me yet?  Aching and leaking and dribbling a mess all over itself?</i>"\n\n', false );
		}

		MainView.outputText( 'You nod, catching a whiff of her musky pussy\'s aroma without meaning to.  It\'s not like you could avoid it, with her body mere inches in front of your ' + CoC.player.face() + ', shaking so perfectly and so closely that an occasional droplet catches on your nose.  Your hands clamp around the chair with an iron grip as you fight not to rise and rape her tender, teasing asshole on the spot.  ' + Descriptors.SMultiCockDesc() + ' is trickling and dribbling, bouncing with your heartbeats as your ' + CoC.player.legs() + ' shiver and shake with denied lusts.  You NEED to do something... to fuck, thrust, or just fap... anything - your ' + Descriptors.multiCockDescriptLight() + ' can\'t take it anymore!\n\n', false );
		MainView.outputText( 'Drawn up by your dick, you let go of your grip on the cow-crafted chair and start to rise into Isabella.  She expertly drops her bouncy butt into your ' + Descriptors.chestDesc() + '.  It knocks you back down into place while she scolds, ', false );
		if( this.isabellaAccent() ) {
			MainView.outputText( '"<i>I said not yet.  Do you vant me to help you cum or not?</i>"', false );
		} else {
			MainView.outputText( '"<i>I said not yet.  Do you want me to help you cum or not?</i>"', false );
		}
		MainView.outputText( '  You nod to her, feeling a little more in control after having your urges interrupted by the object of your desire.  Isabella looks back at you with lidded eyes and whispers, ', false );
		if( this.isabellaAccent() ) {
			MainView.outputText( '"<i>Good ' + CoC.player.mf( 'boy', 'girl' ) + '.  Relax, Momma Isabella vill take care of you.</i>"\n\n', false );
		} else {
			MainView.outputText( '"<i>Good ' + CoC.player.mf( 'boy', 'girl' ) + '.  Relax, Momma Isabella will take care of you.</i>"\n\n', false );
		}

		MainView.outputText( 'The curvaceous cow-girl\'s stance shifts slightly, her teasing, shaking dance coming to a stop for just a moment as she adjusts to rest her rump against your ' + Descriptors.cockDescript( x ) + '.  ', false );
		if( CoC.player.cockTotal() === 2 ) {
			MainView.outputText( 'The other penis is shoved to the side, forgotten about as Isabella selects the lucky fuck-stick.  ', false );
		} else if( CoC.player.cockTotal() > 2 ) {
			MainView.outputText( 'The other penises are shoved aside, forgotten about as Isabella selects the lucky fuck-stick.  ', false );
		}
		MainView.outputText( 'She arches her body and slides up and down your ' + Descriptors.cockDescript( x ) + ', gradually increasing the weight she\'s pressing on it with every stroke.  Your dripping member slips into Isabella\'s tightly-compressed ass-cleavage with a wet-sounding *POP*.  Her derriere is so hot... so sweat-slicked... so... so... perfectly tight.  You aren\'t even fucking her yet', false );
		if( CoC.player.cockArea( x ) <= 38 ) {
			MainView.outputText( ', though with your dick completely devoured by her big butt-cheeks, you may as well be.', false );
		} else {
			MainView.outputText( ', as the mast rising from the top of her crack indicates.', false );
		}
		MainView.outputText( '  Yet, you know without a doubt that there would be no shame in cumming right now and turning the red-head\'s tight ass-crack into a cum-filled canyon.\n\n', false );
		//(Small enough for buttfukkins);
		if( CoC.player.cockArea( x ) <= 38 ) {
			if( this.isabellaAccent() ) {
				MainView.outputText( '"<i>Oh my, does mein behind feel so good zat your cute, delectable cock vill squirt all over me before you even get to feel ze best part?</i>" teases Isabella as she flexes one side and then the other, forcing your ' + Descriptors.cockDescript( x ) + ' to bend in response to the two dueling masses of her booty cheeks.  She keeps teasing you as she caresses your member with her hefty butt.  "<i>You have such a naughty look on your face, drooling on yourself from just a little bit of play with Momma Isabella.  No, don\'t stop, I want to watch the ecstasy spread through you while I work your nice, hot cock.  My asshole wants to kiss it... Can I kiss your dick with my asshole?</i>"', false );
			} else {
				MainView.outputText( '"<i>Oh my, does my ass-job feel so good that your cute, delectable cock is gonna squirt all over me before you even get to feel the best part?</i>" teases Isabella as she flexes one side and then the other, forcing your ' + Descriptors.cockDescript( x ) + ' to bend in response to the two dueling masses of her booty cheeks.  She keeps teasing you as she caresses your member with her hefty butt.  "<i>You have such a naughty look on your face, drooling on yourself from just a little bit of play with Momma Isabella.  No, don\'t stop, I want to watch the ecstasy spread through you while I work your nice, hot cock.  My asshole wants to kiss it... Can I kiss your dick with my asshole?</i>"', false );
			}
			//(if libido <=15:  ;
			if( EngineCore.silly() && (CoC.player.lust < 50 || CoC.player.lib <= 20) ) {
				MainView.outputText( '  Your cock immediately deflates from her laughable doujin-tier pillow talk and falls out of her buttcheeks.  You\'ll be posting that line to 4chan later for laughs.', false );
				EngineCore.dynStats( 'lus', -99 );
				EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
				return;
			}
			MainView.outputText( '  She doesn\'t even let you answer.  Her back-and-forth flexing abruptly halts as she switches to an up and down motion, hotdogging your ' + Descriptors.cockDescript( x ) + ' in the tight vice of her spotted backside.\n\n', false );
			MainView.outputText( 'You grab hold of the chair again with both hands, struggling not to cum while Isabella\'s ass slides over your member.  It\'s so hot, wet, and sensual that you\'re trembling from the effort of holding still, but you manage, knowing the longer you hold back, the better the finish will be.  She slowly pumps your ' + Descriptors.cockDescript( x ) + ' in her anal valley while you whine and moan, your face taking on a more lurid, sexual expression with every passing moment.  When at last you feel you can hold back no longer, you groan, "<i>G-gonna... gonna-cum-soon!</i>"\n\n', false );
			if( this.isabellaAccent() ) {
				MainView.outputText( 'Isabella roughly lifts up and tuts, "<i>No, zat vill not do.</i>"  ', false );
			} else {
				MainView.outputText( 'Isabella roughly lifts up and tuts, "<i>No, that will not do.</i>"  ', false );
			}
			MainView.outputText( 'You whine, trembling beneath her as she denies you penetration.  Then, she changes her angle and presses down.  Her pillowy cheeks slide around your cock, hugging it tightly as the cow-girl\'s wrinkled pucker comes to rest atop your ' + CoC.player.cockHead( x ) + '.  You both gasp at the sudden, electric sensation, immobilized by the new feeling for only the briefest of moments.  Then, Isabella is pressing down, her anus dilating to allow your lust-swollen shaft entrance to her back-door.  Her asshole is even tighter than her butt\'s cleavage was moments before, and as the cinching, puckered ring works its way towards your base, you know you won\'t be able to cum until it loosens a little.\n\n', false );
			MainView.outputText( 'The owner of the perfectly-tight pucker asks, "<i>Do I have to worry about you cumming yet?  No?  I didn\'t think so.</i>"  She wiggles her rump against your groin, grinding those sweat-soaked buns on your hips and belly for a moment hard enough to rock the chair.  Your ' + Descriptors.cockDescript( x ) + ' tries to dribble, tries to leak, but it\'s clutched so tightly that not even the smallest trickle of release can escape.  Slowly at first, the bovine beauty begins sliding up, dragging that ring around you with agonizing slowness and allowing the bottom-most portions to feel the sensual caress of her cheeks once again.  Then, once she\'s just below the tip, she plunges down, faster this time, and you feel almost like your cock is being crushed from stem to stern.\n\n', false );
			if( this.isabellaAccent() ) {
			...
			MainView.outputText( 'Isabella\'s words motivate you to spend every ounce, every stored-up drop of seed into her hungry back-door.  ', false );
			if( CoC.player.cumQ() >= 1000 ) {
				MainView.outputText( 'Her pleased expression shifts slightly as her belly bloats from the fluid injection', false );
				if( CoC.player.cumQ() >= 2000 ) {
					MainView.outputText( ', eventually growing to a pregnant-looking curve', false );
				}
				MainView.outputText( '.  She whimpers slightly and murmurs, "<i>Such a fertile ' + CoC.player.mf( 'boy', 'girl' ) + '... Momma Izzy can take it.</i>"  ', false );
			}
			MainView.outputText( 'Your ' + Descriptors.cockDescript( x ) + ' writhes in bliss for what seems like forever, but before you know it, you\'re coming back down to Earth.  Isabella whispers, "<i>Good ' + CoC.player.mf( 'boy', 'girl' ) + ',</i>" to you and stands up, ignoring the cum that drips from her dark pucker to stain ' + Descriptors.sMultiCockDesc() + '.  She helps you up, and her hand tussles your hair affectionately.  Then she turns towards her side of camp, walking a little bow-legged as she leaves.', false );
		}
		//(Big dicks) ;
		else {
			if( this.isabellaAccent() ) {
				MainView.outputText( '"<i>Oh my, does mein ass feel zat good on ze huge, pervy thing you call a dick?</i>" teases Isabella as she flexes one side then the other, forcing your ' + Descriptors.cockDescript( x ) + ' to bend in response to the two dueling masses of her booty cheeks.  She keeps talking, "<i>You have such a naughty look on your face, drooling all over yourself like a dog.  Is that what having a big dick does to you?  Does it make you so horny and weak-willed that you\'ll waste your cum all over my ass?</i>"  Isabella stops her back-and-forth flexing in order to begin bouncing up and down.  Her soft, flexing buns caress your ' + Descriptors.cockDescript( x ) + ' from base ', false );
			} else {
				MainView.outputText( '"<i>Oh my, does my ass-job feel that good on that huge, pervy thing you call a dick?</i>" teases Isabella as she flexes one side then the other, forcing your ' + Descriptors.cockDescript( x ) + ' to bend in response to the two dueling masses of her booty cheeks.  She keeps talking, "<i>You have such a naughty look on your face, drooling all over yourself like a dog.  Is that what having a big dick does to you?  Does it make you so horny and weak-willed that you\'ll waste your cum all over my ass?</i>"  Isabella stops her back-and-forth flexing in order to begin bouncing up and down.  Her soft, flexing buns caress your ' + Descriptors.cockDescript( x ) + ' from base ', false );
			}
			if( CoC.player.cocks[ x ].cockLength > 48 ) {
				MainView.outputText( 'to as high as she can reach on the towering shaft\'s length', false );
			} else {
				MainView.outputText( 'to ' + CoC.player.cockHead( x ), false );
			}
			MainView.outputText( ' as she does so, drawing an anguished moan from your shivering, lust-overloaded body.\n\n', false );
			MainView.outputText( 'You grab back onto the chair as pleasure sweeps through your well-endowed body, whimpering, "<i>G-gonna... gonna... gonna-cum-soon...</i>"\n\n', false );
			MainView.outputText( 'Isabella reaches between her legs to ', false );
			if( CoC.player.balls > 0 ) {
				MainView.outputText( 'squeeze your balls', false );
			} else if( CoC.player.hasVagina() ) {
				MainView.outputText( 'rub your soaked vulva and ' + Descriptors.clitDescript(), false );
			} else {
				MainView.outputText( 'caress your taint', false );
			}
			if( this.isabellaAccent() ) {
				MainView.outputText( '.  She teases you with a slight undercurrent of disdain in her melodious voice, "<i>Cumming already?  Very vell, go on zen.  Let your disgusting, obscene cock control you.  I can feel it trickling and dribbling on mein bum.  Do vat your fat, over-sized dick commands and cum all over Momma Isabella\'s butt.  Big, thick pricks always seem to cum vay too fast.</i>"\n\n', false );
			} else {
			...
			MainView.outputText( 'You shudder, trying to hold still and prolong the pleasure, but ' + Descriptors.sMultiCockDesc() + ' is jerking up and down, defiantly grinding itself through the hot, slippery valley of Isabella\'s ass.  The ' + CoC.player.cockHead( x ) + ' bounces above like a flag-pole in a hurricane, and then you can bear it no longer.  You\'re cumming... cumming so very hard.  Your whole body locks tight, back arching slightly from the strain.  Isabella slides down to the bottom of your convulsing pole and resumes rolling your ' + Descriptors.cockDescript( x ) + ' between her bronzed ass-cheeks, causing your eruptions of seed to form lazy \'S\' shapes in the air before splattering down on her heaving bottom.', false );
			if( CoC.player.cumQ() <= 200 ) {
				MainView.outputText( '  Her ass gets a nice glaze from your over-sized cock, though the cow-girl stands up just before you finish, letting the last spurt weakly drip on your belly.', false );
			} else if( CoC.player.cumQ() <= 500 ) {
				MainView.outputText( '  Her ass is completely glazed by your over-sized cock\'s heavy cum-flow in short order, though she stands up off of you to let you spurt the last few drizzles on your own belly.', false );
			} else {
				MainView.outputText( '  Her ass, back, hair, and your own crotch are soon flooded with the complete deluge of spunk that shoots from you.  The cow-girl doesn\'t seem to mind indulging you, but as the stuff is running down her thick thighs, she stands up, letting you baste your belly with the last of your cream.', false );
			}
			MainView.outputText( '\n\n', false );
			MainView.outputText( 'Isabella moans and rubs her hands around the curve of her ass, smearing the jism all over her chocolatey skin.  She even takes a handful of excess and smears it over her dripping cunny, shuddering with her own muted climax as she walks away, slightly bow-legged from pleasure.', false );
		}
	...
	IsabellaFollowerScene.prototype.tentacleBoneFollowerIzzy = function() {
		...
		MainView.outputText( 'You grin at Isabella as you nonchalantly remove your ' + CoC.player.armorName + ', exposing your monstrous tentacle-cocks, fully erect, to her frightened face.  Crossing your arms, you stand before her, dicks squirming all over.  "<i>', false );
		if( this.isabellaAccent() ) {
			MainView.outputText( 'Oh, mein Gott!  Zey... zey are... so hug- vait, vat, vat are you doing?', false );
		} else {
			MainView.outputText( 'My God!  They... they are... so hug- wait, what are you doing?', false );
		}
		MainView.outputText( '</i>"\n\n', false );
		if( this.isabellaAccent() ) {
			MainView.outputText( 'Without a word, you adroitly guide your squirming bunch of tentacle peckers toward her body, slowly encircling her. In no time your ' + Descriptors.cockDescript( tentacles ) + ' is already wrapping her supple waist, quickly followed by your ' + Descriptors.cockDescript( tentacles[ 1 ] ) + '; using the combined strength of your inhumanly powerful pair of dick muscles, you lift her off the ground.  "<i>Oh... you\'re a kinky boy, aren\'t you? Well, if that\'s what you want...</i>"', false );
		} else {
			MainView.outputText( 'Without a word, you adroitly guide your squirming bunch of tentacle peckers toward her body, slowly encircling her. In no time your ' + Descriptors.cockDescript( tentacles[ 0 ] ) + ' is already wrapping her supple waist, quickly followed by your ' + Descriptors.cockDescript( tentacles[ 1 ] ) + '; using the combined strength of your inhumanly powerful pair of dick muscles, you lift her off the ground.  "<i>Oh... you are a kinky boy, ja?  Vell, if zat is vhat you vant...</i>"', false );
		}
		...
		if( tentacles.length > 4 ) {
			...
			MainView.outputText( 'her pussy', false );
			if( CoC.player.cumQ() >= 1500 ) {
				MainView.outputText( ' until she looks 8 months pregnant', false );
			}
		}
		if( tentacles.length > 6 ) {
			MainView.outputText( '; drowning her nipples with your spunk', false );
		}
		//[if 10th cock];
		if( tentacles.length > 9 ) {
			MainView.outputText( ', and cuming on her face as you stare at her', false );
		}
		MainView.outputText( '.  Her luscious eyes are ', false );
		if( tentacles.length < 10 ) {
			MainView.outputText( 'eventually', false );
		} else {
			MainView.outputText( 'immediately', false );
		}
		MainView.outputText( ' masked in fluid as you unload your baby-batter on her.  There isn\'t a single spot of flesh that isn\'t being smeared with your sap; there isn\'t a single hole that isn\'t being loaded to full capacity with thick seed.', false );
		if( tentacles.length > 4 ) {
			MainView.outputText( '  Her own rectal ', false );
			if( tentacles.length > 4 ) {
				MainView.outputText( 'and vaginal ', false );
			}
			MainView.outputText( 'squirts look ridiculous as her body is literally drenched in torrents of semen.', false );
		}
		MainView.outputText( '  You keep soaking her with your lust-fueled fountain of spooge until your ', false );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( 'sack ', false );
		} else {
			MainView.outputText( 'groin ', false );
		}
		MainView.outputText( 'feels empty, devoid of any fluid left to spray on her.  ', false );
		MainView.outputText( 'You keep holding her with ' + Descriptors.sMultiCockDesc() + ' for a moment, both of you enjoying the powerful embrace as you literally dick-hug her.  The entirety of her body is dripping with your jism but she doesn\'t seem to care.  Eventually, you put her down, letting her splat in the puddle of juices that formed below your junk-prison.  At last, you unfold your arms.\n\n', false );
		CoC.player.orgasm();
	...
	IsabellaFollowerScene.prototype.milktasticLacticLactation = function() {
		...
		if( this.isabellaAccent() ) {
			...
			MainView.outputText( '"<i>Ohh, my milkers...</i>"\n\n', false );
		}

		MainView.outputText( 'Isabella\'s moans distract you and you look over.  She\'s rubbing her breasts gingerly, wincing as she tests her strange nipples with a finger.  Do you go and check on her?', false );
		//[Yes][No];
	...
	IsabellaFollowerScene.prototype.izzyMilkingMeinMilkersMya = function() {
		...
		if( CoC.player.cor < 50 ) {
			...
			MainView.outputText( 'Idly', false );
		}
		MainView.outputText( ', you walk over to see what the matter is.  She looks up at you, misery clouding her usually cheerful, smiling face.  "<i>', false );
		if( this.isabellaAccent() ) {
			MainView.outputText( 'Ahh, ' + CoC.player.short + '... it has been sooo long since I have been properly milked, mein poor breasts are as full as zey have ever been.  I have been trying to release ze pressure myself, but zere are just not enuff hours in ze day vis everything else I must do, und mein nipples get quite sore, ja?', false );
		} else {
			MainView.outputText( 'Ahh, ' + CoC.player.short + '... it\'s been sooo long since I\'ve been properly milked, my poor breasts are fuller than they\'ve ever been.  I have been trying to release the pressure myself, but there are just not enough hours in the day with everything else I must do, and my nipples get quite sore.', false );
		}
		MainView.outputText( '</i>"\n\n', false );
		MainView.outputText( 'She\'s right; her tits are bigger than you\'ve ever seen them, and not so much jiggling as sloshing audibly.  You frown in commiseration, and she fixes you with a hopeful, teary stare.  "<i>', false );
		if( this.isabellaAccent() ) {
			MainView.outputText( 'Do you zink you could help me vis mein milkers?  Zere must be more zan a week\'s vorth stored up!</i>"\n\n', false );
		} else {
			MainView.outputText( 'Do you think you could help me with my milkers?  There must be more than a week\'s worth stored up!</i>"\n\n', false );
		}

		MainView.outputText( 'As much as your mouth waters at the idea of drinking her frothy, creamy milk, you completely doubt your ability to handle that much of it without going into sugar shock, and tell her so.  Crestfallen, she begins to sob, and peals, "<i>', false );
		if( this.isabellaAccent() ) {
			MainView.outputText( 'Zen -sniff- vat', false );
		} else {
			MainView.outputText( 'Then -sniff- what', false );
		}
		MainView.outputText( ' am I to do with all of my milk?  Boo hoo hoo!</i>"\n\n', false );
		MainView.outputText( 'The sight of the proud warrioress so emotionally distraught is ', false );
		if( CoC.player.cor < 33 ) {
			MainView.outputText( 'unbearable, but you quickly', false );
		} else {
			MainView.outputText( 'kind of arousing, actually, but at length you', false );
		}
		MainView.outputText( ' decide on a solution to her problem; a trip to Whitney\'s farm and a few hours spent in your milker stall to bleed off all that pressure.  You quickly announce her salvation - resisting the temptation to strike a heroic pose - and Isabella jumps from her seat and tackles you with a hug, flinching when her sore nipples bump your ' + Descriptors.chestDesc() + '.\n\n', false );
		if( this.isabellaAccent() ) {
			MainView.outputText( '"<i>Ohh, ' + CoC.player.short + ', I had forgotten all about zose milkers vith all ze verk around camp!  Yes, let us go zere immediately!</i>"\n\n', false );
		} else {
			MainView.outputText( '"<i>Ohh, ' + CoC.player.short + ', I had forgotten all about those milkers with all the work around camp!  Yes, let us go there immediately!</i>"\n\n', false );
		}
	...
	IsabellaFollowerScene.prototype.izzyMilkingMeinMilkersMya2 = function() {
		...
		if( CoC.player.thickness <= 50 ) {
			MainView.outputText( 'with creaking effort, initially calibrated to your slender form as it was', false );
		} else {
			MainView.outputText( 'startlingly quickly, being already set to your much weightier bulk', false );
		}
		MainView.outputText( '.  The process begins in earnest and Isabella groans as the suction on her finger-chafed nipples begins.  ', false );
		if( this.isabellaAccent() ) {
			MainView.outputText( '"<i>Oooh, mein udders... zey feel as zough zey vill pop already!</i>"\n\n', false );
		} else {
			MainView.outputText( '"<i>Oooh, my udders... they feel as though they will pop already!</i>"\n\n', false );
		}

		MainView.outputText( 'Sure enough, first a trickle and then a gush of Isabella\'s ivory breast-milk wicks down the tube and into the thirsty machine.  The clanging suction continues for nearly an hour, Isabella\'s groans quickly turning to bovine bellows of relief as the pressure in her mammaries begins to abate.  After a while you notice the soft pad of another person\'s footsteps on the barn floor behind you, and look over your shoulder.\n\n', false );
		MainView.outputText( '"<i>Heya ' + CoC.player.short + '!  And heya Isabella; haven\'t seen you in a while!</i>"  Whitney declares.  "<i>I figured it was just you using the machine,</i>" she angles her head at you, "<i>but when I heard a different voice drifting out of the barn door I about dropped the load of stable muck I was tossing!  I was sneaking in the barn when I finally recognized who it was!  I see you get into your milking as much as ever, hon.</i>"  The dog-morph shoots a surreptitious glance at Isabella\'s skirt, where a trickle of girlcum is staining the front.\n\n', false );
		MainView.outputText( 'The cow-girl, all smiles and lolling tongue, still manages to nod briskly at the farmer.  "<i>', false );
		if( this.isabellaAccent() ) {
			MainView.outputText( 'Zat\'s right, I - oooh - had ssso m-much to do that I had no time to empty oh-oh-out mah-mah-my... my... MOOOOOO-ilkers!  Ah, ah, ah!</i>"  Isabella twitches and arches her back as the dark spot on the front of her skirt widens appreciably; she just had an orgasm from having her immense load of milk drained!  Whitney gives a wry smile when she notices you staring, and pats you on the shoulder.  "<i>Used to happen a lot before, too,</i>" she whispers.  "<i>She\'d come in after a week complaining how sore her tits were, cream the machine, and cream herself.  I offered to put her up on the farm so she wouldn\'t have to wait so long, but she wouldn\'t hear of it.  Too independent, I guess; just like you.</i>"  Whitney removes her hand and walks over to the bottling station, turning it on.\n\n', false );
		} else {
			MainView.outputText( 'That\'s right, I - oooh - had ssso m-much to do that I had no time to empty oh-oh-out mah-mah-my... my... MOOOOOO-ilkers!  Ah, ah, ah!</i>"  Isabella twitches and arches her back as the dark spot on the front of her skirt widens appreciably; she just had an orgasm from having her immense load of milk drained!  Whitney gives a wry smile when she notices you staring, and pats you on the shoulder.  "<i>Used to happen a lot before, too,</i>" she whispers.  "<i>She\'d come in after a week complaining how sore her tits were, cream the machine, and cream herself.  I offered to put her up on the farm so she wouldn\'t have to wait so long, but she wouldn\'t hear of it.  Too independent, I guess; just like you.</i>"  Whitney removes her hand and walks over to the bottling station, turning it on.\n\n', false );
		}
		...
		MainView.outputText( 'Whitney returns, handing warm bottles of milk to both of you.  "<i>Here you go from the pump.  Isabella, do you think you\'re going to stop by more from now on?  I could set aside your old stall.</i>"\n\n', false );
		if( this.isabellaAccent() ) {
			MainView.outputText( 'The warrioress nods emphatically.  "<i>Ja, it vould help sooo much vith how busy I am now!  Is zis okay, ' + CoC.player.short + '?  For me to visit once a day und relieve the pressure on mein milkers?</i>"\n\n', false );
		} else {
			MainView.outputText( 'The warrioress nods emphatically.  "<i>Yes, it would help sooo much with how busy I am now!  Is this okay, ' + CoC.player.short + '?  For me to visit once a day and relieve the pressure on my milkers?</i>"\n\n', false );
		}

		MainView.outputText( 'You ', false );
		if( CoC.player.cor > 50 ) {
			MainView.outputText( 'briefly consider forbidding her outright in order to see that exquisite look of pained pleasure on her face again, but you don\'t think she\'d comply.', false );
		} else {
			MainView.outputText( 'don\'t really see a problem with it as long as she has some left to slake your thirst, but that\'s the question, isn\'t it?', false );
		}
		MainView.outputText( '  What do you say?', false );
	...
	IsabellaFollowerScene.prototype.AllowIzzyMilkerUse = function() {
		...
		MainView.outputText( ', as she assures you you\'ll always come first while she\'s with you.\n\n', false );
		if( CoC.player.hasCock() ) {
			MainView.outputText( '(You have a hunch that you might be able to catch her using the milkers at the farm if you \'explore\' there, provided she hasn\'t been recently milked.)\n\n' );
		}
	...
	IsabellaFollowerScene.prototype.noMilkingMilky = function() {
		...
		if( CoC.player.cor > 50 ) {
			MainView.outputText( 'roughly grab', false );
		} else {
			MainView.outputText( 'gently caress', false );
		}
		MainView.outputText( ' the sensitive nipples on her left breast; she moans in ', false );
		if( CoC.player.cor > 50 ) {
			MainView.outputText( 'painful ', false );
		}
		MainView.outputText( 'arousal at your touch, blushing at putting on such a show in front of Whitney.  The farmer shrugs.  "<i>Y\'all please yourselves.  Milker\'ll be here if you need it.</i>"\n\n', false );
	...
	IsabellaFollowerScene.prototype.getMilk = function() {
		...
		if( CoC.flags[ kFLAGS.ISABELLA_MILK_COOLDOWN ] > 0 ) {
			MainView.outputText( 'Isabella shakes her head and tells you she', false );
			if( CoC.flags[ kFLAGS.ISABELLA_MILKED_YET ] === -1 ) {
				MainView.outputText( '\'s out of bottled milk right now, and that you should check back later.', false );
			} else {
				MainView.outputText( '\'s still sore from last time, and that you should wait till later.', false );
			}
			EngineCore.doNext( this, this.callForFollowerIsabella );
			return;
		}
		...
		if( CoC.flags[ kFLAGS.ISABELLA_MILKED_YET ] === -1 ) {
			if( this.isabellaAccent() ) {
				MainView.outputText( 'Isabella nods, and fetches you one of the spares from her latest milking session at the farm.  "<i>Here you go!  Less zan twenty-four hours old, ja?</i>"', false );
			} else {
				MainView.outputText( 'Isabella nods, and fetches you one of the spares from her latest milking session at the farm.  "<i>Here you go!  Less than twenty-four hours old, okay?</i>"\n\n', false );
			}
		} else {
			MainView.outputText( 'Isabella nods, holding an empty bottle out to you and pulling her top down to expose her well-marbled bosom.  "<i>Remember, your promise...</i>"\n\n', false );
			MainView.outputText( 'You smile wryly and begin teasing and pulling the nipples of one breast even as you lean over to the other and pop two of its teats into your mouth.  "<i>', false );
			if( this.isabellaAccent() ) {
				MainView.outputText( 'Ja, zat feels great... hurry and let out mein milk...</i>"', false );
			} else {
				MainView.outputText( 'Ooh, that feels great... hurry and let out my milk...</i>"', false );
			}
			...
			if( this.isabellaAccent() ) {
				MainView.outputText( 'Zat', false );
			} else {
				MainView.outputText( 'That', false );
			}
			MainView.outputText( ' felt gooood... thank you!</i>" Isabella says, breathily; the telltale wet spot is back on the front of her skirt.  You manage not to get caught staring at it, but the aroma of her arousal combined with her quivering, grateful breastflesh makes you consider sticking around for more fun...\n\n', false );
		}
	...
	IsabellaFollowerScene.prototype.angryMurble = function() {
		MainView.outputText( '', true );
		MainView.outputText( 'You come to Isabella\'s part of the camp with Marble in tow, supposing now is as good a time as ever to introduce the two.  Marble greats Isabella warmly but immediately starts bombarding her with questions about her origin.  From her persistence, it seems she is interested in meeting another cow-girl.  Though a little overwhelmed, Isabella recovers quickly, explaining her origins and the impurity of her cow-girl nature.  Marble is visibly disappointed.\n\n', false );
		MainView.outputText( '"<i>The topic of conversation gradually shifts to the reason why Marble has come to the camp.  Marble seems to be happy to meet your friend, and is eager to spend some more time with her in the future.  Isabella, on the other hand, seems a little off-put regarding Marble\'s actions.  Only time will tell how the two take to each other.', false );
	...
	IsabellaFollowerScene.prototype.isabellaSparMenu = function() {
		...
		if( CoC.flags[ kFLAGS.ISABELLA_SPARRING_INTRO ] === 0 ) {
			...
			if( this.isabellaAccent() ) {
				MainView.outputText( 'Zis vill be good for both of us, ya!?</i>"  The beaming cow-girl taps a finger to her chin and suggests, "<i>Ve should set up out on ze edges of the plains, so ve don\'t damage zis camp.</i>"', false );
			} else {
				MainView.outputText( 'This ought to be good for both of us!</i>"  The beaming cow-girl taps a finger to her chin and suggests, "<i>Why don\'t we set up a faux camp on the edge of the plains, so we don\'t damage this one.  Okay?</i>"', false );
			}

			MainView.outputText( '  That seems logical.  This will be like a trip down memory lane.\n\n', false );
			CoC.flags[ kFLAGS.ISABELLA_SPARRING_INTRO ] = 1;
		} else {
			MainView.outputText( 'You suggest a sparring session.  ', false );
		}
		if( this.isabellaAccent() ) {
			MainView.outputText( 'Isabella asks, "<i>Vhat stakes should ve use?  Vould you rather we spar light or fight like ze creatures of zis world?</i>"', false );
		} else {
			MainView.outputText( 'Isabella asks, "<i>How should we fight?  Would you rather we spar light or fight like the creatures of this world?</i>"', false );
		}
	...
	IsabellaFollowerScene.prototype.sparring = function( type ) {
		...
		if( CoC.flags[ kFLAGS.ISABELLA_SPARRING_INTRO ] === 1 ) {
			...
			MainView.outputText( 'You and Isabella hike to the border of the plains, where you set up the make-shift camp for your sparring sessions.\n\n', false );
		}
		MainView.outputText( 'Isabella lifts her shield and raps her knuckles against the solid steel, making a loud, gong-like sound that carries for some distance.  You raise your ' + CoC.player.weaponName + ' and prepare to fight.  It\'s on!', false );
		if( type === 1 ) {
	...
	IsabellaFollowerScene.prototype.isabellaBurps = function() {
		...
		if( CoC.flags[ kFLAGS.ISABELLA_PROBOVA_BURP_COUNT ] === 0 ) {
			if( this.isabellaAccent() ) {
				MainView.outputText( '"<i>Vhat is zat, dear?</i>"', false );
			} else {
				MainView.outputText( '"<i>What do you have there?</i>"', false );
			}
			MainView.outputText( ' Isabella asks innocently, inspecting the bottle of cloudy fluid you\'re offering her.  You reveal the contents to be Pro Bova, basically a supercharged version of what she took to become as she is now.  Her eyes cloud a bit as she absentmindedly presses her oversized bosom together, the most obvious indication of her changes.  "<i>And you ', false );
			if( this.isabellaAccent() ) {
				MainView.outputText( 'v', false );
			} else {
				MainView.outputText( 'w', false );
			}
			MainView.outputText( 'ould like me to drink more?</i>" she asks, only a hint of trepidation in her rich voice.  ', false );
			if( this.isabellaAccent() ) {
				MainView.outputText( '"<i>Vell... nuzzing irreversible can happen, ja?</i>"', false );
			} else {
				MainView.outputText( '"<i>Well... there\'s nothing we can\'t change back, right?</i>"', false );
			}
			MainView.outputText( '\n\n', false );
			MainView.outputText( 'At your assurance that she won\'t change beyond the obvious extra cow-girl aspects, she visibly calms, beaming at you and reaching for the potion.  "<i>', false );
			if( this.isabellaAccent() ) {
				MainView.outputText( 'Bottoms up, in zat case!', false );
			} else {
				MainView.outputText( 'Bottoms up, in that case!', false );
			}
			MainView.outputText( '</i>" she exclaims, uncorking the Pro Bova and gulping the whole thing down.  "<i>Not too bad,</i>" she comments, smacking her lips in satisfaction a couple times. "<i>Now, whe-0...</i>"\n\n', false );
			MainView.outputText( 'Her thought is completely cut off by an angry gurgling in her gut.  She grunts, hitting her stomach with her balled-up hand a few times in an attempt to quiet the rude organ, but her attempt is only rewarded with another, louder belly growl.  You exchange glances, both of you wondering what exactly the potion will do – and did – to her.  Isabella curiously inspects her arm as if to spot any signs of change, finding nothing but chocolate-colored skin and the calloused hands of a working woman.  Suddenly, the cow-girl flinches, wincing and clutching her tummy.  For a moment, you consider asking her what\'s the matter, but you quickly figure it out yourself as your gaze drops south.  Her gut, usually flat and firm, is bloated considerably, rounding out to a little pot belly and slowly growing.\n\n', false );
			MainView.outputText( 'Both of you are at a loss for words, alternating glances from each other to her belly in quick succession.  Isabella straightens unexpectedly, now gripping her stomach with both hands.  "<i>Tight...</i>" she moans through gritted teeth. "<i>Moving...</i>"\n\n', false );
			MainView.outputText( 'Her eyes widen as realization strikes her.  "<i>Back away,</i>" she warns, weakly waving an arm at you. "<i>Allergy...</i>" It seems as if something might be coming. Do you flee from the pained cow-girl, or do you attempt to assist her with her problem?  Though, judging by her reaction, assistance might be a poor plan...', false );
	...
	IsabellaFollowerScene.prototype.runAwayFromIzzyBurps = function() {
		...
		if( CoC.flags[ kFLAGS.ISABELLA_PROBOVA_BURP_COUNT ] === 0 ) {
			...
			MainView.outputText( 'The gaseous assault eventually diminishes, her belly returning to a healthy state and the cloud dispersing.  After waiting a few minutes to confirm the end of the episode, Isabella stomps up to you, a fierce blush painting her cheeks.  "<i>', false );
			if( this.isabellaAccent() ) {
				MainView.outputText( 'Vat did you give me?  Zat was zimply embarrassing!', false );
			} else {
				MainView.outputText( 'What did you give me? That was simply embarrassing!', false );
			}
			MainView.outputText( '</i>" she complains, lip drooped in an upset pout.  You can\'t help but agree, but you do point out that nothing bad happened, after all.\n\n', false );
			MainView.outputText( '"<i>...True,</i>" she concedes, a bit of her bluster lost at the realization.  "<i>', false );
			if( this.isabellaAccent() ) {
				MainView.outputText( 'It seems mein body rejected zis \'Pro Bova\' for one reason or another.</i>"  No harm, no foul, you offer hopefully, and she nods.  "<i>But make sure to seriously zink before giving me a potion again!</i>" she warns, waggling a foreboding finger your way.  You think back to that cloud of gas, and what the probable effect of such a thing would be in close proximity.  Even if you wanted to test that out, however, you\'d need another Pro Bova...', false );
			} else {
				MainView.outputText( 'It seems my body rejected this \'Pro Bova\' for one reason or another.</i>"  No harm, no foul, you offer hopefully, and she nods.  "<i>But make sure to seriously think before giving me a potion again!</i>" she warns, waggling a foreboding finger your way.  You think back to that cloud of gas, and what the probable effect of such a thing would be in close proximity.  Even if you wanted to test that out, however, you\'d need another Pro Bova...', false );
			}
			//Back to follower menu;
			EngineCore.doNext( this, this.callForFollowerIsabella );
		} else {
			...
			MainView.outputText( 'At your nod, she smiles in satisfaction and moves away.  You can\'t be sure, but you almost swear you hear her burp once more.', false );
			EngineCore.doNext( this, this.callForFollowerIsabella );
		}
	};
	//help (ya dumbo);
	IsabellaFollowerScene.prototype.getIzzyBurped = function() {
		...
		if( CoC.flags[ kFLAGS.ISABELLA_PROBOVA_BURP_COUNT ] === 1 ) {
			...
			if( CoC.player.lowerBody !== AppearanceDefs.LOWER_BODY_TYPE_HOOFED || CoC.player.horns <= 0 || CoC.player.hornType !== AppearanceDefs.HORNS_COW_MINOTAUR ) {
				MainView.outputText( '  With that whole affair over with, you go to stand... and find yourself falling once more.', false );
			}
			if( CoC.player.lowerBody !== AppearanceDefs.LOWER_BODY_TYPE_HOOFED ) {
				//[if no hooves present already];
				if( CoC.player.isTaur() ) {
					MainView.outputText( '  Confused, you quickly inspect yourself before your gaze settles on your hooves – or should you say, your SINGLE PAIR of hooves.  Triggered by what you surmise are the Pro Bova\'s fumes, your centaur body has vanished, leaving you with a single pair of cow-like hooves.', false );
				} else {
					MainView.outputText( '  Confused, you quickly inspect yourself before your gaze settles on your ' + CoC.player.feet() + ' – or should you say, former ' + CoC.player.feet() + '.  Triggered by what you surmise are the Pro Bova\'s fumes, your ' + CoC.player.feet() + ' have quickly and painlessly refashioned themselves into cloven hooves – not unlike those Isabella is currently sporting.', false );
				}
				changed = true;
			}
			...
			if( CoC.player.horns <= 0 || CoC.player.hornType !== AppearanceDefs.HORNS_COW_MINOTAUR ) {
				if( CoC.player.horns > 0 && CoC.player.hornType > AppearanceDefs.HORNS_NONE ) {
					MainView.outputText( '  Your existing horns quiver, then reform into those of a ' + CoC.player.mf( 'bull', 'cow' ), false );
				} else {
					MainView.outputText( '  Two nubs sprout from your forehead, swiftly thickening and growing into a large pair of curved bovine horns', false );
				}
				MainView.outputText( ', throwing your center of balance off and aborting yet another attempt to right yourself.', false );
				changed = true;
			}
			//[if no cow ears];
			if( CoC.player.earType !== AppearanceDefs.EARS_COW ) {
				MainView.outputText( '  Your ears wiggle a bit, then simply migrate higher up your head, lengthening and widening as they go.  Before you know it, you have cow ears.', false );
				changed = true;
			}
			//[if no cow tail];
			if( CoC.player.tailType !== AppearanceDefs.TAIL_TYPE_COW ) {
				if( CoC.player.tailType > AppearanceDefs.TAIL_TYPE_NONE ) {
					MainView.outputText( '  The tip of your tail reshapes into a burst of fur, the entire length changing to match.', false );
				} else {
					MainView.outputText( '  A tail creeps out of your ' + CoC.player.armorName + ', perking up and growing rapidly.  A flare of fur bursts from the tip, the rest of the length changing to match.', false );
				}
				changed = true;
			}
			...
			if( CoC.player.hasCock() && CoC.player.longestCockLength() < 18 ) {
				MainView.outputText( '  A pressure builds in your crotch, at first subtle, then incredibly obnoxious to the point where you\'re forced to undo the lower portion of your ' + CoC.player.armorName + ' to seek some sort of relief.  Your shock is total as you stare at your former ' + Descriptors.cockDescript( 0 ) + ', now measuring at least 18 inches long – a condition that Isabella has already noticed and is clearly formulating a gripe about.', false );
				if( CoC.player.balls < 0 ) {
					MainView.outputText( '  Two testicles fall into a scrotum near the base of your enlarged meat, already swelling into baseball-sized balls o\' fun.', false );
				} else if( CoC.player.ballSize < 4 ) {
					MainView.outputText( '  Your testicles engorge in the transformative powers of the cow-girl\'s violent reaction to the Pro Bova, growing to baseball-sized in short order.', false );
				}
				changed = true;
			}
			//[if female/herm and player race not cow-girl];
			if( changed && CoC.player.hasVagina() && CoC.player.race() !== 'cow-girl' ) {
				MainView.outputText( '  "<i>Goodness, look at you!</i>" Isabella exclaims, waving away the remnant wisps of the gassy mist.  "<i>Surely, you\'re giving me a run for ', false );
				if( this.isabellaAccent() ) {
					MainView.outputText( 'mein money now, ja?', false );
				} else {
					MainView.outputText( 'my money now, darling!', false );
				}
				MainView.outputText( '</i>"  Considering your current condition, you can\'t really disagree.  She happily approaches and sweeps you up into a big hug, squishing your tits into hers and wiggling about happily, transferring waves of jiggle-inducing energy into your tits.  "<i>', false );
				if( this.isabellaAccent() ) {
					if( CoC.player.tallness < 87 ) {
						MainView.outputText( 'Now, if only ve could make you a bit taller, ve vould be twins!', false );
					} else if( CoC.player.tallness < 95 ) {
						MainView.outputText( 'Und look at zat, you\'re as grown up as me!', false );
					} else {
						MainView.outputText( 'Zough, of course, you do have me beat in ze height department!', false );
					}
				} else {
					if( CoC.player.tallness < 87 ) {
						MainView.outputText( 'Now, if only we could make you a bit taller, we would be twins!', false );
					} else if( CoC.player.tallness < 95 ) {
						MainView.outputText( 'And look at that, you\'re as grown up as me!', false );
					} else {
						MainView.outputText( 'Though, of course, you do have me beat in the height department!', false );
					}
				}
				MainView.outputText( '</i>"\n\n', false );
			}
			//[if male and player race not cowboy];
				...
				MainView.outputText( '  "<i>', false );
				if( this.isabellaAccent() ) {
					MainView.outputText( 'Nein nein nein nein, zis von\'t do at all!', false );
				} else {
					MainView.outputText( 'No, no! This is awful!', false );
				}
				MainView.outputText( '</i>" she cries as she approaches, eyes locked on your oversized genitalia.', false );
				if( CoC.player.longestCockLength() < 18 ) {
					if( this.isabellaAccent() ) {
						MainView.outputText( '  "<i>Zis did not have to be any bigger!  Vhat a tragedy! Your poor little wiener!</i>"', false );
					} else {
						MainView.outputText( '  "<i>This did not have to be any bigger!  What a tragedy! Your poor little wiener!</i>"', false );
					}
					MainView.outputText( '  You raise an eyebrow at her selective worry, indicating just how cow-like you\'ve become.  ', false );
					if( this.isabellaAccent() ) {
						MainView.outputText( '"<i>...Yah, zat is important too, but mein poor pussy...</i>"', false );
					} else {
						MainView.outputText( '"<i>I suppose that\'s important too, but what about my poor girl-parts?</i>"', false );
					}
				}
				MainView.outputText( '\n\n', false );
			}
			...
			if( CoC.player.lowerBody !== AppearanceDefs.LOWER_BODY_TYPE_HOOFED || CoC.player.horns <= 0 || CoC.player.hornType !== AppearanceDefs.HORNS_COW_MINOTAUR || CoC.player.earType !== AppearanceDefs.EARS_COW || CoC.player.tailType !== AppearanceDefs.TAIL_TYPE_COW || (CoC.player.hasCock() && CoC.player.longestCockLength() < 18) ) {
				MainView.outputText( 'You\'re not surprised when you take the now-familiar cow-' + CoC.player.mf( 'boy', 'girl' ) + ' form, ', false );
				if( (!CoC.player.hasCock() || Utils.rand( 2 ) === 0) && CoC.player.hasVagina() ) {
					MainView.outputText( 'Isabella beaming with eagerness at your sloshing and swelling bosom.\n\n', false );
				} else if( CoC.player.hasCock() ) {
					MainView.outputText( 'Isabella not able to keep a little frustration from her face as your dick grows to enviable size.\n\n', false );
				}
			}
			//if all requirements already filled (already a busty cow-girl/well-endowed cowboy);
			else {
				MainView.outputText( 'Predictably, her sweet-smelling fumes do nothing but further exacerbate your sex-drive, and from the look in her eyes, she\'s looking forward to it.\n\n', false );
			}

			//'sorry for burping in your face bro';
			MainView.outputText( 'Isabella smiles and draws you in for a smooth.  "<i>I suppose I should really apologize again,</i>" she muses as she slowly draws back, tongue licking up the small bridge of drool between your lips.  "<i>Do you accept my apology?</i>" she asks wryly.', false );
		}
	...
	IsabellaFollowerScene.prototype.declineIzzysCowBurpApology = function() {
		...
		//no;
			...
			MainView.outputText( 'Despite your strangely inspired lust, you turn the cow-girl down.  Though crestfallen, she takes the news well, apologizing - sincerely and soberly - once more before moving back to her designated camping spot.  Happily, after about an hour, you get back to normal.', false );
		}
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	//yes;
	...
	IsabellaFollowerScene.prototype.receiveAllTheCowTOngues = function() {
		...
		if( this.isabellaAccent() ) {
			MainView.outputText( '"<i>I hope you are not zinking about cumming yet, mein friend,</i>" she chides, kneading your titflesh a little more gently. "<i>Ve have not even gotten zu ze pussy-licking yet, after all...</i>"' );
		} else {
			MainView.outputText( '"<i>I hope you\'re not thinking about cumming yet, [name],</i>" she chides, kneading your titflesh a little more gently. "<i>We haven\'t even gotten to the good part yet...</i>"' );
		}
		...
		MainView.outputText( ' Isabella says, taking you by the shoulders and, displaying her impressive strength, flips the two of you around, so that you are sitting in her chair and she is atop you.  ' );
		if( this.isabellaAccent() ) {
			MainView.outputText( '"<i>Time fur ze main course,</i>"' );
		} else {
			MainView.outputText( '"<i>Time for the main course,</i>"' );
		}
		MainView.outputText( ' she adds, licking her lips with her big, wide cow-tongue.  She [if (isBiped = true) spreads your [legs]|adjusts your lower body] so that your [pussy] is on display, already drenched with your juices, begging to be filled or pleasured.' );
		...
		if( this.isabellaAccent() ) {
			MainView.outputText( '"<i>Ve vill have to teach you to be mindful vhere you leak!</i>"' );
		} else {
			MainView.outputText( '"<i>Looks like I\'ll have to teach you to watch where you leak!</i>"' );
		}
		...
		if( CoC.player.clitLength < 1 ) {
			MainView.outputText( ', your clit grinding into her nose,' );
		}
		MainView.outputText( ' until you\'ve finally soaked her in enough cum to let her out.' );
		...
		if( this.isabellaAccent() ) {
			MainView.outputText( '"<i>Vell, how vas zhat, [name]?</i>"' );
		} else {
			MainView.outputText( '"<i>Well, what did you think?</i>"' );
		}
		MainView.outputText( ' she asks coyly, licking a bit of your femspray off her cheek.' );
		MainView.outputText( '\n\nYou give her a weak thumbs-up before passing out for a quick nap in her comfy chair.' );
		if( this.isabellaAccent() ) {
			MainView.outputText( '  A bit confused, she nonetheless leaves and returns with a drink, setting it beside you before leaving you to your rest.' );
		}
		CoC.player.orgasm();
		EngineCore.dynStats( 'sen', -1 );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	...
	IsabellaFollowerScene.prototype.fuckIsabella = function() {
		...
		MainView.outputText( 'You ask Isabella if she wouldn\'t mind helping you blow off some steam; it\'s evident for anyone to see that you\'re flushed with arousal, your [cock ' + y + '] straining against the material of your [armor].  Looking at the state of you, Isabella ' );
		//[if cock in her size: ;
		if( CoC.player.cocks[ x ].cockLength < 9 ) {
			MainView.outputText( 'pulls your small [cockFit ' + y + '] out of your armor, preparing to suck you off as normal.  You grab her shoulders to stop her, more interested in... giving back, this time.' );
		} else {
			MainView.outputText( 'cocks an eyebrow at you, crossing her arms over her tremendous bust.  "<i>[name],</i>" she sighs, ' );
			if( this.isabellaAccent() ) {
				MainView.outputText( '"<i>I don\'t know about zhat... I\'m more partial to ze cute little cocks... Zey fit me so vell...</i>"' );
			} else {
				MainView.outputText( '"<i>I don\'t know about that... You know I like \'em cute and tiny. They just fit me so well...</i>"' );
			}
		}
		...
		if( CoC.player.cocks[ x ].cockLength < 9 ) {
			...
			MainView.outputText( ' Isabella says, straightening herself up' );
			if( CoC.player.tallness < 72 ) {
				MainView.outputText( ' to tower over your diminutive frame' );
			}
			MainView.outputText( '.  ' );
			if( this.isabellaAccent() ) {
				MainView.outputText( '"<i>Your tiny little cock ist zo cute, [name], I just couldn\'t zay no...</i>"' );
			} else {
				MainView.outputText( '"<i>You\'ve got just the cutest little cock, [name]... how could I say no?</i>"' );
			}
		}
		//{If cock bigger than she likes};
			...
			if( this.isabellaAccent() ) {
				MainView.outputText( '"<i>I zuppose I could do vith some relief of mein own...  Very vell, [name].  Ve vill have zex, you und I.</i>"' );
			} else {
				MainView.outputText( '"<i>I guess I could do with some relief of my own...  Okay, [name], let\'s do it.</i>"' );
			}
		}
		...
		if( CoC.player.cockArea( x ) < 50 ) {
			MainView.outputText( 'utterly engulfing your length' );
		} else {
			MainView.outputText( 'surrounding a fair amount of your massive shaft' );
		}
		MainView.outputText( '.  Slowly, the cow-girl begins to work her tits along your cock, rubbing her smooth, mottled skin up and down, up and down, occasionally venturing to lick you with her huge, wide tongue, nearly wrapping you in wet muscle between strokes of her breasts.' );
		MainView.outputText( 'You groan with pleasure and stroke the cow-girl\'s dark red hair, urging her on and, with a bit of force, controlling her pace, making sure she goes just as fast as you like.  She obliges you, licking and titfucking you to your pleasure until you feel ready to burst; only then do you push her back, roughly putting her on the ground and spreading her thick, cloven legs.' );
		if( this.isabellaAccent() ) {
			MainView.outputText( '\n\n"<i>Nicht zo rough!</i>"' );
		} else {
			MainView.outputText( '\n\n"<i>Not so rough!</i>"' );
		}
		MainView.outputText( ' she cries, but does not hesitate to open her thighs for you and expose her slick cunt and the tight ring of her anus, nearly hidden between her big butt-cheeks.  You set to work on her as soon as you catch sight of her twat, burying your face in her groin and slurping at her slit with your tongue, lapping at the little trickles of girl-lube clinging to her inner walls.  She moans loudly, her high singing voice warbling into a drawn-out "<i>MOOOOOOOOOOO!</i>" as your tongue teases and caresses her cherry-nub; she grabs your [hair] and pushes you in further as you begin to suckle her clit.  You continue to lap at her womanhood and its bud until she\'s leaking everywhere, her pussy positively soaked from your ministrations.  Contented in your efforts, you shove her hands off you and get onto your knees, your [cock ' + y + '] flopping heavily onto her wet crotch.' );
		...
		if( CoC.player.cockArea( x ) < 80 ) {
			MainView.outputText( 'Ordinarily tight, Isabella is practically soaked with lube, making the effort to cram your cock into her an easy task; soon your hips are pressed against her raised ass, her thick thighs wrapped around your body.  ' );
		}
		MainView.outputText( 'Rearing back, you slam your groin into Isabella\'s, grinning as she sprays femlube around your prick, obviously enjoying the [cockFit ' + y + '] inside her.' );
		...
		if( CoC.player.cumQ() <= 250 ) {
			MainView.outputText( 'until you\'re utterly spent' );
		} else if( CoC.player.cumQ() <= 1000 ) {
			MainView.outputText( 'until cum spurts out of her passage and stains her thighs' );
		} else {
			MainView.outputText( 'until her belly begins to bloat with the sheer amount of cream you\'re stuffing into her' );
		}
		MainView.outputText( '.\n\n' );
		MainView.outputText( 'You give Isabella a final swat on the ass, and laugh as the haughty cow-girl collapses in the pool of spunk and femcum you\'ve made beneath her, utterly exhausted.' );
	...
	IsabellaFollowerScene.prototype.fuckIsabellaInTheBarn = function() {
		...
		MainView.clearOutput();
		MainView.outputText( 'You quietly slip into the barn and make your way over to Isabella\'s stall.  Silently opening the door, you\'re afforded a good look at the nude, bent-over cowgirl, your eyes drinking in her lush curves and thick, soft ass - and the glistening pussy between her meaty thighs, slightly parted by a pair of mottle-skinned fingers rubbing and teasing at her big clit.' );
		...
		MainView.outputText( '\n\n' );
		if( this.isabellaAccent() ) {
			MainView.outputText( '"<i>Ach, [name]?  Vhat do you think you are doing?  Sneaking up on people like zhat...</i>"' );
		} else {
			MainView.outputText( '"<i>Oh, [name]?  What do you think you\'re doing, huh?  Sneaking up on people like that...</i>"' );
		}
		...
		MainView.outputText( ' against her back, your [cock] pushing further and further between her cheeks until you lay hands upon her eight nipples, taking her many buds between your fingers until a hesitant trickle of milk flows forth over your hands, pouring onto the barn floor as you give the cow a more sensual milking.  Your fingers glide across her nipples, going from one large, needy teat to the next, gently caressing her milky breasts until Isabella\'s head rolls back in a throaty moo, her backside clenching powerfully around your prick.  You let out a low groan, reminded for a moment of your own needs.  You start to move your hips, slowly starting to fuck Isabella\'s asscheeks as you bury your chin in her shoulder, pressing kisses all along the nape of her neck.' );
		...
		if( this.isabellaAccent() ) {
			MainView.outputText( '  "<i>I vhant it...  please, [name], do not tease me so...</i>"' );
		} else {
			MainView.outputText( '  "<i>Come on, [name], give it to me already...  enough with the teasing...</i>"' );
		}
	...
	IsabellaFollowerScene.prototype.isabellaBarnFuckPartII = function() {
		...
		if( CoC.player.cocks[ CoC.player.smallestCockIndex() ].cockLength < 9 ) {
			MainView.outputText( '  You slide easily into her, until your [hips] press into Isabella\'s thighs, your groins joined together.  ' );
			if( this.isabellaAccent() ) {
				MainView.outputText( '"<i>Mmm, zuch a nice fit,</i>"' );
			} else {
				MainView.outputText( '"<i>Mmm, that\'s a nice fit,</i>"  Isabella sighs, her vaginal muscles gently caressing your shaft.' );
			}
		}
		//{If medium cock};
		else if( CoC.player.smallestCockArea() < 40 ) {
			MainView.outputText( '  Your cock enters her, stretching Isabella\'s walls apart until your [cock smallest] comes to caress the closed mouth of her cervix, eagerly brushing up against the entrance to her womb.  ' );
			if( this.isabellaAccent() ) {
				MainView.outputText( '"<i>Ach, so big</i>"' );
			} else {
				MainView.outputText( '"<i>It\'s so big,</i>"' );
			}
			MainView.outputText( ' Isabella grunts, her cunt already working hard to milk your prick.' );
		}
		//{If big fucking cock};
		else {
			MainView.outputText( '  Isabella bites her lip as your oversized prick stretches her wide, whimpering that you\'ll tear her apart as your [cock smallest] pushes into her womb, her cervical lips clamping hard around your first inches of thick cockflesh.  "<i>Aaaaahhhhhh, ' );
			if( this.isabellaAccent() ) {
				MainView.outputText( 'It iz sehr too big, zo huge?</i>"' );
			} else {
				MainView.outputText( 'God it\'s so huge, I... it\'s tearing me apart!</i>"' );
			}
		}
		...
		if( CoC.player.smallestCockArea() <= 164 ) {
			MainView.outputText( 'in up to the hilt, smearing her wildly contracting walls with thick smears of leaking pre' );
		} else {
			MainView.outputText( 'in until your [cock smallest] batters into her womb, thick globs of pre smearing her seed-catcher' );
		}
		MainView.outputText( '.  Your wrap your arms firmly around your lover\'s shoulders, hugging her tight as you piston your [cock smallest] into her, starting to move harder and faster with each buck of your [hips].  Isabella cries out again, her open mouth a perfect opportunity to slip your tongue in, running the tip along her own\'s wide, pink surface.  Her lips lock around you, suckling on your tongue as you had on her teats a moment before, tasting herself on you through the kiss.' );
		...
		if( this.isabellaAccent() ) {
			MainView.outputText( '[name], zhat was...  ah, [name]' );
		} else {
			MainView.outputText( 'Oh, wow, [name], that was...  that was incredible!' );
		}
		MainView.outputText( '</i>" Isabella gasps between ragged breaths.  "<i>' );
		if( this.isabellaAccent() ) {
			MainView.outputText( 'You really know how to please a voman, nein?' );
		} else {
			MainView.outputText( 'You really know how to show a girl a good time, don\'t you?' );
		}
		MainView.outputText( '</i>"' );
