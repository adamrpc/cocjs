...
angular.module( 'cocjs' ).run( function( MainView, SceneLib, ImageManager, $log, PregnancyStore, ConsumableLib, PerkLib, Descriptors, AppearanceDefs, Appearance, CockTypesEnum, $rootScope, CoC, kFLAGS, Utils, StatusAffects, EngineCore, Combat ) {
	...
	JojoScene.prototype.jojoMutationOffer = function() {
		this.jojoSprite();
		MainView.outputText( 'A wicked idea comes to mind while thinking of Jojo.  The lethicite you took from the lake goddess – perhaps it could be used to enhance your own budding demonic powers, and twist your mousey fuck-puppet into a truly worthy pet?\n\n<b>Do You?</b> (WARNING only once & unlocks tentacle content)', true );
		EngineCore.doYesNo( this, this.jojoMutationOfferYes, this, this.jojoMutationOfferNo );
	};
	JojoScene.prototype.jojoMutationOfferNo = function() {
		this.jojoSprite();
		MainView.outputText( 'There are some lines even you won\'t cross.  Besides, having a sex-addled mouse with a constantly drooling foot-long cock is all the fun you can handle.\n\nWith that decided you prepare to call on your slut.', true );
		//Normal jojo sex scene here;
		EngineCore.doNext( this, this.corruptJojoSexMenu );
	};
	//CORRUPT CAMP JOJO;
	JojoScene.prototype.corruptCampJojo = function() {
		...
		if( CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_JOJO ] === 0 ) {
			...
			MainView.outputText( 'Before you call for your corrupted pet, how do you want to use him?', true );
		} else {
			MainView.outputText( 'Jojo slinks out of a barn when you call his name, hating and loving every step he takes towards you. He looks dusty and worn out but his dick still strains at the sight of you.' );
			...
			if( this.tentacleJojo() ) {
				MainView.outputText( ' A number of tentacles vaugley resembling cocks have sprouted from his back and groin. They sway restlessly around him, oozing thick, fragrant pre from their tips.' );
			}
			if( SceneLib.farmCorruption.hasTattoo( 'jojo' ) ) {
				MainView.outputText( '\n\n' );
				if( SceneLib.farmCorruption.jojoFullTribalTats() ) {
					MainView.outputText( 'He is covered from head to tail in tribal tattoos, erotic lines snaking all over his naked frame, giving him the look of a barely tamed savage.' );
				} else {
					if( SceneLib.farmCorruption.numTattoos( 'jojo' ) > 1 ) {
						MainView.outputText( 'He has the following tattoos emblazoned across his body:\n' );
					} else {
						MainView.outputText( 'He has ' );
					}

					if( CoC.flags[ kFLAGS.JOJO_TATTOO_COLLARBONE ] !== 0 ) {
						MainView.outputText( CoC.flags[ kFLAGS.JOJO_TATTOO_COLLARBONE ] + '\n' );
					}
					if( CoC.flags[ kFLAGS.JOJO_TATTOO_SHOULDERS ] !== 0 ) {
						MainView.outputText( CoC.flags[ kFLAGS.JOJO_TATTOO_SHOULDERS ] + '\n' );
					}
					if( CoC.flags[ kFLAGS.JOJO_TATTOO_LOWERBACK ] !== 0 ) {
						MainView.outputText( CoC.flags[ kFLAGS.JOJO_TATTOO_LOWERBACK ] + '\n' );
					}
					if( CoC.flags[ kFLAGS.JOJO_TATTOO_BUTT ] !== 0 ) {
						MainView.outputText( CoC.flags[ kFLAGS.JOJO_TATTOO_BUTT ] + '\n' );
					}
					MainView.outputText( '\n' );
				}
			}
		...
		if( CoC.player.hasKeyItem( 'Cock Milker - Installed At Whitney\'s Farm' ) >= 0 ) {
			...
			//First time:;
				...
				if( this.tentacleJojo() ) {
					tentaMilk = EngineCore.createCallBackFunction( this, this.repeatMilkJojo, true );
				}
			} else {
		...
		if( CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_JOJO ] === 1 && CoC.flags[ kFLAGS.FOLLOWER_PRODUCTION_JOJO ] === 0 ) {
			EngineCore.addButton( 7, 'Harvest Draft', this, this.harvestJojoDraft );
		}
		if( CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_JOJO ] === 1 && CoC.flags[ kFLAGS.FOLLOWER_PRODUCTION_JOJO ] === 1 ) {
			EngineCore.addButton( 7, 'Stop Harvest', this, this.stopHarvestJojoDraft );
		}
	...
	JojoScene.prototype.harvestJojoDraft = function() {
		MainView.clearOutput();
		this.jojoSprite();
		MainView.outputText( 'You tell Jojo that you want him hooked up to a cock milker whenever possible; you need his fluids.' );
	...
	JojoScene.prototype.stopHarvestJojoDraft = function() {
		MainView.clearOutput();
		this.jojoSprite();
		MainView.outputText( 'You tell Jojo to stop producing incubus draft; you’re practically drowning in the stuff. Jojo closes his eyes, and you grin as you envisage the image you’ve just planted in his mind.' );
	...
	JojoScene.prototype.sendToFarm = function() {
		MainView.clearOutput();
		this.jojoSprite();
		MainView.outputText( 'You tell your pet mouse' );
		if( this.tentacleJojo() ) {
			MainView.outputText( '-come-tentacle-beast' );
		}
		MainView.outputText( ' that he is to head towards the lake, find a farm, present himself to the lady who works there and do as she says.' );
	...
	JojoScene.prototype.backToCamp = function() {
		MainView.clearOutput();
		this.jojoSprite();
		MainView.outputText( 'You tell him to head back to camp; there are things you need to do to him you can’t do whilst he’s here. Repeatedly. Jojo trembles, manages a bow, and then slowly trails off towards camp.' );
	...
	JojoScene.prototype.jojoMutationOfferYes = function() {
		...
		MainView.outputText( 'There\'s no question about it, this is a great idea.  It might be coming from the corruption in your blood, but why bother to fight it?  You take Marae\'s lethicite and grab one of the larger crystalline protrusions.  With a hard yank, you break it off from the main cluster, sending tiny crystalline shards over the campsite.  They vanish into the ground before you have a chance to gather them.\n\n', true );
		MainView.outputText( 'Whether by luck or some unconscious demonic magic, the smaller piece of lethicite is rather phallic. If it weren\'t for the sharp corners and hard edges, it would look like a large dildo with a pair of balls at the base.  You put away the larger piece, focusing this tool and your plans for your pet.\n\n', false );
		MainView.outputText( 'You lick the sharp crystalline dildo, slathering it with spit.  You aren\'t sure, but you seem unable to stop.  It\'s as if some demonic force guides you.  It\'s difficult to focus on much beyond your own arousal and the tingling feeling that spreads through your body as you taste this piece of a goddess\' essence.  Your mind drifts off into a perverted fantasy, unable to cope with total loss of control and oral debauchery.\n==========================================\n<i>', false );
		//[Male];
		if( CoC.player.gender <= 1 ) {
			MainView.outputText( 'In your fantasy you\'re fucking Jojo again, bottoming out against his cute girlish ass over and over.  His dick spurts mouse-milk with each thrust, feeding a dozen growing puddles underneath him as they slowly merge together.  He writhes and squirms, a pair of crystalline balls against his chin while he deep-throats a massive dildo.  You blast another wave into his overloaded spunk-dumpster, forcing a thick jet out of him and into the cum-puddles underneath you.\n\n', false );
			MainView.outputText( 'Tired of the foreplay, you pull out of him, watching a river of your cum escape from the uncorked opening.  You pull him up, watching the crystal cock slide out of his mouth, the bulge along his throat slowly disappearing as each inch escapes.  He staggers, completely exhausted, but still hard and horny judging by the look in his eye.\n\n', false );
			MainView.outputText( 'You shove him over a log and grab his spit-slick toy, dragging it through the puddled cum until it\'s white and dripping.  Jojo whimpers, his disproportional member stuck between his legs and twitching with anticipation.  The dildo slides into his well-stretched hole with ease, sealing the rest of your tainted seed deep inside his bowels as your magic begins to change him...</i>\n==========================================\n', false );
			MainView.outputText( 'You snap out of the fantasy with a start.  The crystal shard is sitting on the ground in front of you, but it is no longer a shard – it\'s the dildo from your fantasy, complete with a thick covering of your cum.  You manage to pry your hand away from your softening member' );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ' and take the tainted artifact, now ready to be used.</i>\n', false );
		}
		//[Female] ;
		else {
			MainView.outputText( 'In your fantasy, you\'re riding Jojo hard, milking his swollen dick with your pussy', false );
			if( CoC.player.biggestLactation() < 2 ) {
				MainView.outputText( ' while your tits cover his head, nearly suffocating him', false );
			} else {
				MainView.outputText( ' while he suckles one of your breasts hungrily, sucking down your milk', false );
			}
			MainView.outputText( '. A crystalline dildo is buried hilt-deep in his ass, held in place by a harness of your own design that twists it around every time he moves.  You cum again, relishing the desperate and needy look the poor mouse is giving.  Your spasming pussy is immediately filled with even more warmth, and you feel the thick spunk forcing its way into your core until you feel bloated.\n\n', false );
			MainView.outputText( 'You calm your trembling body and stand up off of him, soaking him with his own escaping fluids.  Jojo moans and starts pumping his dick, wriggling his hips while his orgasm is dragged on by the dildo squeezing his prostate.  You curl up alongside his ear and begin whispering into his mind, watching as your words soak in and begin to change him with the aid of the crystalline cock\'s magic.</i>\n==========================================\n', false );
			MainView.outputText( 'You snap out of your fantasy with a start.  The crystal shard is sitting on the ground in front of you, but it\'s no longer a shard – it\'s the dildo from your fantasy, now slick with pussy juices.  You touch your sore cunt and gasp when you realize where it went.  ', false );
			if( CoC.player.vaginas[ 0 ].virgin ) {
				MainView.outputText( '(Yet somehow your virginity remains intact).  ', false );
			}
			MainView.outputText( 'The dildo disappears back into your inventory, and you smile in wicked anticipation of your next meeting with Jojo.\n', false );
		}
		MainView.outputText( '\n<b>You just can\'t wait!  You\'ll call him over now...</b>', false );
		EngineCore.doNext( this, this.mutateJojo );
	};
	JojoScene.prototype.mutateJojo = function() {
		this.jojoSprite();
		MainView.outputText( 'You call out, and Jojo obediently pads into the camp.  At some point he decided to switch to wearing a loin-cloth, for all the good it has done him – it drapes over his member, barely covering half of it as it twitches and throbs from your presence.  You gesture for him to remove that tiny cloth, and he does immediately.  When he gets within a few feet of you, he drops to his knees with his hands behind his back, his head down submissively.  You see little tics and twitches run through his body as he fights to resist touching himself, so you drag it out and see how long he can wait.\n\n', true );
		MainView.outputText( 'It doesn\'t take long.  A plaintive whine escapes him as his hand creeps around his waist.  You grin and push him onto his back, stepping onto his wrist to pin his hand in place.  You drop the crystal dildo onto his chest with a single command, "<i>Use it</i>".\n\n', false );
		MainView.outputText( 'Jojo shivers and takes your offering with his free hand, slowly hefting it and dragging it lower.  He looks up at you questioningly, and you gesture impatiently at him to quit wasting your time.  The dildo\'s crown presses against his sphincter, stopping for a second as the bitch-boy relaxes, and then begins its long journey inward.  Jojo\'s eyes roll up a bit and his prick starts pumping out a clear stream of his pre-cum, wetting most of his belly.  Finally the pinkish-hued balls press against his bum.  He\'s taken the whole thing.\n\n', false );
		MainView.outputText( 'While you still have one of the mouse\'s hands pinned under your ' + CoC.player.foot() + ', his other has already released the tainted sex-toy, and journeyed up to his pre-cum spurting cock.  You waste no time and push his hand away as you jump on top of him, being sure to grind your ' + this.vaginaOrDicksOrCrotch() + ' against his slick boner.  He keeps whining plaintively and wriggling his hips against you, but you deny him any further stimulation, instead focusing on keeping him horny while the tainted dildo settles in.\n\n', false );
		MainView.outputText( 'Jojo\'s moans reach a fevered pitch as a soft buzzing begins to fill the air behind you.  You turn around and see that the lethicite-crafted dildo has begun to vibrate inside of him!  Somehow you can feel that this is the time to change him – that his form is malleable and ready.  Your hand finds itself wrapped around the base of his cock, bottling him up until you\'re ready, while you lean up to his cute mouse-ears and start to whisper, "<i>Aww, does my little rodent-bitch want to cum?</i>"\n\n', false );
		MainView.outputText( 'He shakes his head and whines again, trembling underneath you.\n\n', false );
		MainView.outputText( '"<i>Too bad slut,</i>" you say while you give his balls a squeeze, "<i>you can\'t cum until I\'ve changed you into the perfect fuck-toy.  A toy that can just keep cumming, more than once.  A toy who can fill more than one hole at a time.  A toy I could share with my friends, who could keep up with a succubus and an incubus simultaneously.  You\'ll be that toy for me.  My delicious fuck-puppet that cums on command with tentacle-dicks and pussy-vines aplenty.</i>"\n\n', false );
		MainView.outputText( 'Jojo\'s eyes roll back until all you can see is the whites.  His mouth lolls open from pleasure and lets loose a constant stream of pants and moans.  Already you can feel his cock squirming in your hand, thrashing to get free.  The dildo starts vibrating louder, and his pre-cum manages to start spurting free again, despite your tight grip on his member.  You see his shoulders bulge as something starts wriggling under his skin, thrashing back and forth until they sprout free as a pair of tentacles.\n\n', false );
		MainView.outputText( 'Uncontrolled, the tentacles curl down to Jojo\'s vine-like member, wrapping it in coils until your hand is forced off and wriggling around it.  You note each of his new appendages is developing a flare at their tips, and the puddle of pre seems to be growing faster than it ever did before.\n\n', false );
		MainView.outputText( 'You whisper, "<i>Good bitch, you\'ll be able to fuck every hole around you, won\'t you?  But all I see is an ass and a mouth for cocks.  You need some nice tight cock-sleeves don\'t you?</i>"\n\n', false );
		MainView.outputText( 'On cue, you feel his tail curl up your back, already longer than it ever was before.  To your surprise the fur is vanishing, leaving you feeling smooth rubbery skin.  It splits three ways in an instant, but each grows thicker than his tail ever was before.  One curls over your shoulder as if looking for something, and reveals a strange sucking opening, sticky with some kind of fluid and filled with undulating fleshy ridges.\n\n', false );
		MainView.outputText( 'You smile and give the opening a lick – it\'s sweet – and turn back to his ear, "<i>Now you\'re where I want you bitch.  Feel that dildo squeezing your prostate?  You\'ll cum just from that won\'t you?  Take those new tentacles off your dick and aim them at your face, I want to watch you give yourself a facial and cum all over yourself just from my other toy.</i>"\n\n', false );
		MainView.outputText( 'Despite looking lost to the pleasure, his body obeys, releasing his dick and aiming his shoulder-tentacles at his open mouth.  You can see now that each is a dark green color, and capped with a shiny purplish head, just like the vines in the corrupted glade.  Each of them starts flaring wider, and you feel his cock snake up between your ', false );
		if( CoC.player.biggestTitSize() < 1 ) {
			MainView.outputText( 'bodies', false );
		} else {
			MainView.outputText( Descriptors.allBreastsDescript(), false );
		}
		MainView.outputText( ', until it pops out just under his chin, looking like a fatter version of the twins on his shoulders.  All three start flaring wider in preparation, and you know he is about to erupt.\n\n', false );
		MainView.outputText( '"<i>Good toy,</i>" you whisper, "<i>cum for your ', false );
		if( CoC.player.gender <= 1 ) {
			MainView.outputText( 'master', false );
		} else {
			MainView.outputText( 'mistress', false );
		}
		MainView.outputText( '.</i>"\n\n', false );
		MainView.outputText( 'He submits happily, a smile overtaking his face in spite of his eyes rolled back and his tongue drooling all over his muzzle.  His entire body quakes, and you feel bulges beginning to pass under your ', false );
		if( CoC.player.biggestTitSize() >= 1 ) {
			MainView.outputText( Descriptors.allBreastsDescript(), false );
		} else {
			MainView.outputText( 'chest', false );
		}
		MainView.outputText( '.  You can see them moving up the shoulder tentacles with agonizing slowness, until all three cock-vines erupt simultaneously, splattering Jojo\'s face with his own seed.  The perfect synchronization is broken immediately when his primary tentacle blasts a glob into his hair, and then the entire scene devolves into a mess of quivering tentacles and splattering spooge.  Jojo\'s mouth catches plenty, and he begins swallowing instinctively as his tentacles do their best to drown him.\n\n', false );
		MainView.outputText( 'You pull back and watch, ', false );
		if( CoC.player.cockTotal() > 0 ) {
			MainView.outputText( 'stroking your own member', false );
			if( CoC.player.cockTotal() > 1 ) {
		...
		if( CoC.player.hasVagina() ) {
			MainView.outputText( 'caressing your aching twat ', false );
		}
	...
	JojoScene.prototype.useTentacleJojo = function() {
		...
		if( CoC.player.totalCocks() > 0 ) {
			MainView.outputText( 'Upside down, you\'re forced to watch as his tail-tentacles curl out from between his legs, and extend towards your vulnerable ' + Descriptors.multiCockDescriptLight() + '.  All three of them pause a few inches away for what seems an eternity, revealing circular dripping orifices filled with slimy wriggling cilia.\n\n', false );
			if( CoC.player.cockTotal() === 1 ) {
				//(1dick+balls = ;
				if( CoC.player.balls > 0 ) {
					MainView.outputText( 'One of them lashes forwards, ', false );
					if( CoC.player.cockArea( 0 ) > 60 ) {
						MainView.outputText( 'laboriously stretching around', false );
					} else if( CoC.player.cockArea( 0 ) >= 20 ) {
						MainView.outputText( 'engulfing', false );
					} else {
						MainView.outputText( 'easily swallowing', false );
					}
					MainView.outputText( ' your ' + Descriptors.cockDescript( 0 ) + ' in a tight vise of pleasure.  The other two tentacles slide under, each taking a ' + Descriptors.ballDescript() + ' partially inside their \'mouths\' and massaging them softly.', false );
					MainView.outputText( '\n\n', false );
				}
				//(1 dick no balls = ;
				else {
					...
					MainView.outputText( ' your ' + Descriptors.cockDescript( 0 ) + ' in a tight vise of pleasure.  The other two tentacles slide under, ', false );
					if( !CoC.player.hasVagina() ) {
						MainView.outputText( 'rubbing back and forth along your taint and smearing you with their slime.', false );
					} else {
						MainView.outputText( 'rubbing against your thighs and smearing them with lubricant while one of them easily sucks your ' + Descriptors.clitDescript() + ' inside.', false );
						//(If big clit – ;
						if( CoC.player.clitLength >= 4 ) {
							MainView.outputText( '  You nearly cum on the spot from the clitoral stimulation; it\'s like you have a super-sensitive cock being licked by a thousand tongues.', false );
						}
					}
					MainView.outputText( '\n\n', false );
				}
			...
			if( CoC.player.cockTotal() === 2 ) {
				//balls);
				if( CoC.player.balls > 0 ) {
					MainView.outputText( 'Two of them lash forward, ', false );
					if( CoC.player.cockArea( 0 ) > 60 ) {
						MainView.outputText( 'laboriously stretching around', false );
					} else if( CoC.player.cockArea( 0 ) >= 20 ) {
						MainView.outputText( 'engulfing', false );
					...
					MainView.outputText( ' your ' + Descriptors.multiCockDescriptLight() + ' in tight vises of pleasure.  The remaining tentacle slides underneath, alternatively taking each of your ' + Descriptors.ballsDescript() + ' partway into its \'mouth\' and massaging them with its thousands of wriggling stimulators.\n\n', false );
				}
				//(2 dicks no balls) ;
				else {
					MainView.outputText( 'Two of them lash forward, ', false );
					...
					MainView.outputText( ' your ' + Descriptors.multiCockDescriptLight() + ' in tight vises of pleasure.  The remaining tentacle slides underneath, seeking ', false );
					if( CoC.player.hasVagina() ) {
						MainView.outputText( 'out your ' + Descriptors.clitDescript() + ' and sucking it inside.', false );
						//(If big clit – ;
						if( CoC.player.clitLength >= 4 ) {
							MainView.outputText( '  You nearly cum on the spot from the clitoral stimulation; it\'s like you have a super-sensitive cock being licked by a thousand tongues.', false );
						}
					} else {
						MainView.outputText( 'to rub itself along your taint, massaging the sensitive skin with its slimy lubricants.', false );
					}
					MainView.outputText( '\n\n', false );
				}
			}
			...
			if( CoC.player.cockTotal() >= 3 ) {
				MainView.outputText( 'All three of them lash forward, ', false );
				if( CoC.player.cockArea( 0 ) > 60 ) {
					MainView.outputText( 'laboriously working to swallow', false );
				} else if( CoC.player.cockArea( 0 ) >= 20 ) {
					MainView.outputText( 'engulfing', false );
				} else {
					MainView.outputText( 'easily swallowing', false );
				}
				MainView.outputText( ' your ' + Descriptors.multiCockDescriptLight() + ' in tight vises of pleasure.  The sensation is overwhelmingly pleasurable, like having three cocks being licked and sucked by a thousand tongues.', false );
				//(if extra: ;
				if( CoC.player.cockTotal() > 3 ) {
					MainView.outputText( '  The remaining ' + Appearance.cockNoun( CockTypesEnum.HUMAN ) + '\'s pulse and throb with jealous desire, until one of the tentacles slips off and captures it.  They take turns, slipping off and capturing each of your offerings until you wish they\'d just keep sucking forever.', false );
				}
				MainView.outputText( '\n\n', false );
			}
		}
		...
		if( CoC.player.hasVagina() ) {
			MainView.outputText( 'pair of tentacles ', false );
		} else {
			MainView.outputText( 'tentacle ', false );
		}
		MainView.outputText( 'hanging over your bottom decide', false );
		if( !CoC.player.hasVagina() ) {
			MainView.outputText( 's', false );
		}
		MainView.outputText( ' that now is a good time to penetrate your undefended opening', false );
		if( CoC.player.hasVagina() ) {
			MainView.outputText( 's', false );
		}
		MainView.outputText( ', and press against you, ', false );
		//Virgin taking;
		if( CoC.player.hasVirginVagina() || CoC.player.ass.analLooseness === 0 ) {
			MainView.outputText( 'barely slipping inside your virginal opening', false );
			if( CoC.player.hasVirginVagina() && CoC.player.ass.analLooseness === 0 ) {
				MainView.outputText( 's', false );
			}
		}
		//not virgin taking;
			...
			if( CoC.player.hasVagina() ) {
				if( CoC.player.vaginas[ 0 ].vaginalLooseness >= AppearanceDefs.VAGINA_LOOSENESS_GAPING_WIDE ) {
					MainView.outputText( ' practically falling into your oversized cum-receptacles', false );
				} else {
					if( CoC.player.vaginas[ 0 ].vaginalWetness >= AppearanceDefs.VAGINA_WETNESS_SLICK ) {
						MainView.outputText( ' sliding inside easily thanks to your copious lubrication', false );
					} else {
						MainView.outputText( 'forcing their way inside your ' + Descriptors.vaginaDescript( 0 ) + ' and ' + Descriptors.assholeDescript(), false );
					}
				}
			}
			//Just butt;
				...
				if( CoC.player.ass.analLooseness >= 4 ) {
					MainView.outputText( ' practically falling into your oversized cum-receptacle', false );
				} else {
					if( CoC.player.ass.analWetness >= 3 ) {
						MainView.outputText( ' sliding inside easily thanks to your copious lubrication', false );
					} else {
						MainView.outputText( 'forcing its way inside your ' + Descriptors.assholeDescript(), false );
					}
				}
			}
		}
		//ON WITH THE PENETRATION;
		MainView.outputText( '.  Your ' + this.mouthMuzzle() + ' opens with a wordless cry of surprise ', false );
		if( CoC.player.hasVagina() ) {
			MainView.outputText( 'and pleasure ', false );
		}
		MainView.outputText( 'as Jojo\'s tentacle', false );
		if( CoC.player.hasVagina() ) {
			MainView.outputText( 's begin', false );
		} else {
			MainView.outputText( ' begins', false );
		}
		MainView.outputText( ' thoroughly violating you, pistoning in and out relentlessly.', false );
		if( CoC.player.totalCocks() > 0 ) {
			MainView.outputText( '  You feel a strange pleasure every time it plunges deep into your ass, smacking into your prostate.  ', false );
			if( CoC.player.cockTotal() === 1 ) {
				MainView.outputText( '  The tentacle on your ' + Descriptors.multiCockDescriptLight() + ' seems appreciative, and begins sucking to swallow down all your pre.', false );
			}
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( '  The tentacles on your ' + Descriptors.multiCockDescriptLight() + ' seem appreciative, and begin sucking to swallow down all your pre.', false );
			}
		}
		if( CoC.player.hasVagina() ) {
			MainView.outputText( '  Your ' + Descriptors.vaginaDescript( 0 ) + ' spasms around the invader, clenching uncontrollably as it\'s plowed by the thick vine-prick.', false );
		}
		MainView.outputText( '\n\n', false );
		MainView.outputText( 'You feel the bindings around your waist sliding and twisting to free up some of their length.  The pair of tentacle-cocks rear up before your ' + Descriptors.allBreastsDescript() + ' like hungry snakes eyeing a mouse.\n\n', false );
		...
		if( (CoC.player.biggestTitSize() >= 4 || (CoC.player.biggestTitSize() >= 4 && CoC.player.breastRows.length > 1)) && CoC.player.biggestLactation() < 2 ) {
			titFucking = true;
			MainView.outputText( 'The twin tentacles erupt like kinked up garden-hoses that have suddenly become free, splattering your chest with thick white goop.  They spurt until all of your ' + Descriptors.breastDescript( 0 ) + ' are painted white with warm dripping goop.  One of the purple-headed monsters slides into the mess, while the other ties itself around your ' + Descriptors.breastDescript( 0 ) + ', squeezing its brother tightly between the walls of cum-slickened titflesh.  The swollen purple head bumps your chin, smearing you with its cum as it begins to fuck your tits.  Glob after glob of mouse-cum spatters up from your chest into your chin and mouth, eventually soaking your ' + this.faceMuzzle() + ' and neck.\n\n', false );
		}
		//(Lactating highly);
		if( CoC.player.biggestLactation() >= 2 ) {
			nippleSucking = true;
			MainView.outputText( 'The heads split apart, dividing into quarters as they reveal wet openings with pink, tongue-like appendages.  Milk begins to leak from your nipples as their intent becomes clear, and you inwardly scold yourself for ', false );
			if( CoC.player.cor < 80 ) {
				MainView.outputText( 'being such a wanton slut', false );
			} else {
				MainView.outputText( 'giving it up too easily', false );
			}
			MainView.outputText( '.  Jojo\'s dick-mouths latch on and suck hard, pulling your ' + Descriptors.nippleDescript( 0 ) + 's deep into their internal reservoir and stimulating your breasts to release a torrent of milk.  ', false );
			if( CoC.player.cowScore() >= 5 ) {
				MainView.outputText( 'You find yourself mooing in contentment as your fuck-slave works to milk you.  ', false );
			}
			MainView.outputText( 'You watch with a happy smile, enjoying the sensations as the tentacles bulge and move milk-distended knots down their lengths towards Jojo.  They are so full that the sloshing whiteness actually becomes visible through the \'skin\' of his tendrils.', false );
		...
		//(NEITHER - JUST NIPS) ;
			...
			nippleSucking = true;
			MainView.outputText( 'The heads split apart, dividing into quarters and revealing a wet pink opening with a writhing tongue in the center.  They latch on and suck hard, each pulling your entire ' + Descriptors.nippleDescript( 0 ) + ' into their interior.  Gasping at the sensations, you can only moan as the tentacular tongue twists itself around your nipple.  ', false );
			if( CoC.player.biggestLactation() >= 1 ) {
				MainView.outputText( 'A squirt of milk escapes and is quickly sucked down by the hungry tentacles.  ', false );
			}
			MainView.outputText( 'The sucking and teasing is relentless, keeping your nipples hard and incredibly sensative as Jojo has his way with you.\n\n', false );
		}
		//mouthfuck – if shoulder-tentacles are unoccupied – random;
		else if( CoC.player.biggestTitSize() < 4 && CoC.player.biggestLactation() < 2 ) {
			mouthFucking = true;
			MainView.outputText( 'You feel the bindings around your waist sliding and twisting to free up some of their length.  The pair of tentacle-cocks rear up in front of your face, dripping cum from their tainted purple heads.  One curls around the other, twirling around it until the two tentacles look more like some obscene double-headed dildo.  They smear against your lips, coating them with spunk and begging to be let inside.  Pushing harder and harder, they eventally work your jaw open and cram themselves inside.  An immediate jet of cum paints the back of your throat, nearly gagging you for a moment.  The sensation passes quickly, as if your gag reflex was somehow numbed.\n\n', false );
			MainView.outputText( 'Twinned dicks force themselves further into your throat, until you are sure they must be outlined and clearly visible to Jojo on your neck.  The mouse-dicks push further in, working their way down to the bottom of your esophagus before pulling back, dripping cum all the while.  Every now and then you get a feeling of fullness in your belly, and realize one of them must have shot off a load of spunk directly into you.  The brutal mouthfuck shows no sign of slowing down, forcing you to gasp in a breath through your nose every time the tentacles pull back.\n\n', false );
		}
		//Cumsplosion;
		MainView.outputText( 'Jojo unleashes a howl of bestial pleasure, and you feel the ', false );
		if( CoC.player.hasVagina() ) {
			MainView.outputText( 'tentacles in your ' + this.assholeOrDP() + ' thicken perceptibly.  Bulges of cum work their way down the tentacles, spreading your ' + this.assholeOrDP() + ' wide before stuffing you full of mouse cream.  You can feel your bowels and uterus filling, expanding you until you look ', false );
			if( CoC.player.pregnancyIncubation === 0 ) {
				MainView.outputText( 'mildly pregnant', false );
			} else {
				MainView.outputText( 'even more pregnant', false );
			}
			MainView.outputText( '.  Your body spasms around them, locked in the throes of orgasm', false );
			if( CoC.player.vaginas[ 0 ].vaginalWetness === AppearanceDefs.VAGINA_WETNESS_SLAVERING ) {
				MainView.outputText( ', and soaks him with a wave of puss juice.  ', false );
			} else if( CoC.player.vaginas[ 0 ].vaginalWetness >= AppearanceDefs.VAGINA_WETNESS_SLICK ) {
				MainView.outputText( ', as your drooling cunt forms a slick puddle.  ', false );
			} else {
				MainView.outputText( '.  ', false );
			}
			//(If big clit being sucked – ;
			if( CoC.player.cockTotal() <= 2 && CoC.player.balls === 0 ) {
				MainView.outputText( 'The tentacle locked around your ' + Descriptors.clitDescript() + ' bulges out a bit as your clit throbs from the orgasm, before it sucks harder, making your clitty even larger.  You squeal from the raw sensation until the tentacle pops off, satisfied, leaving your clit looking like an angry red cock.  ', false );
				CoC.player.clitLength += 0.25;
			}
			MainView.outputText( 'Both tentacles pull out, still dripping whiteness that puddles in your holes.', false );
		} else {
			MainView.outputText( 'tentacle in your ' + Descriptors.assholeDescript() + ' thickens perceptibly.  Bulges of cum work their way down the main tentacle, spreading your ' + Descriptors.assholeDescript() + ' wide before stuffing you full of mouse cream.  You can feel your bowels filling, expanding you until you look mildly pregnant.  Your body spasms around them, locked in the throes of orgasm.  The tentacle pulls out, still dripping whiteness that puddles in your well-used hole.', false );
		}
		//Dick milkings ahoy!;
		if( CoC.player.cockTotal() > 0 ) {
			if( CoC.player.cockTotal() === 1 ) {
				MainView.outputText( 'The tentacle working over your ' + Descriptors.multiCockDescript() + ' clamps down and wriggles deliciously around you.  You can\'t help but cum from the assault, feeding the tentacle your hot spoogy load.  It drinks it down eagerly', false );
				if( CoC.player.cumQ() >= 100 ) {
					MainView.outputText( ', but after a few moments it begins to struggle with the sheer output.', false );
					if( CoC.player.cumQ() >= 300 ) {
						MainView.outputText( '  When you finally finish, it\'s bloated and dripping your cum all over your crotch.', false );
						if( CoC.player.cumQ() >= 600 ) {
							MainView.outputText( '  You pump the tentacle so full it turns pinkish-white and bloats obscenely, before suddenly releasing a backflow that soaks you in spunk from the waist down. It drops to the ground limp and spent', false );
							if( CoC.player.cumQ() >= 800 ) {
								MainView.outputText( ' but you don\'t stop cumming until it lies in a deep puddle of jizz', false );
							}
							MainView.outputText( '.', false );
							cumPuddle = true;
						}
			...
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 'The tentacles working over your ' + Descriptors.multiCockDescript() + ' clamp down and wriggle deliciously around you.  You can\'t help but cum from the assault, feeding the tentacles your hot spoogy load.  They drink it down eagerly', false );
				if( CoC.player.cumQ() >= 100 ) {
					MainView.outputText( ', but after a few moments they begin to struggle with the sheer output.', false );
					if( CoC.player.cumQ() >= 300 ) {
						MainView.outputText( '  When you finally finish, they\'re bloated and dripping your cum all over your crotch.', false );
						if( CoC.player.cumQ() >= 600 ) {
							MainView.outputText( '  You pump the tentacles so full they turn pinkish-white and bloat obscenely, before suddenly releasing a backflow that soaks you in spunk from the waist down. They drop to the ground limp and spent', false );
							if( CoC.player.cumQ() >= 800 ) {
								MainView.outputText( ' but you don\'t stop cumming until they lie in a deep puddle of jizz', false );
							}
							MainView.outputText( '.', false );
							cumPuddle = true;
						}
		...
		if( CoC.player.biggestLactation() >= 2 && nippleSucking ) {
			MainView.outputText( 'Your ' + Descriptors.breastDescript( 0 ) + ' squirt milk even harder, pushing the tentacles clear off you with the force of your lactation.  Your ' + Descriptors.nippleDescript( 0 ) + 's look like ', false );
			if( CoC.player.nippleLength >= 2 ) {
				MainView.outputText( 'even larger', false );
			} else {
				MainView.outputText( 'obscene', false );
			}
			MainView.outputText( ' cow-teats from all the suction, fountaining milk everywhere as it puddles on the ground.  The milk flow tapers off after a few seconds, leaving you stretched and sore.', false );
			if( CoC.player.nippleLength < 4 && CoC.player.breastRows[ 0 ].breastRating >= 1 ) {
				MainView.outputText( '  Somehow you know they won\'t shrink the whole way back down to normal.', false );
				CoC.player.nippleLength += 0.25;
			} else {
				MainView.outputText( '  Somehow you know they won\'t get any bigger from his rough treatment.', false );
			}
			MainView.outputText( '  Your ' + Descriptors.allBreastsDescript() + ' finally feel emptied; it\'s a relief.\n\n', false );
			milkPuddle = true;
		}
		//Titfucking breastgasm;
		if( titFucking ) {
			MainView.outputText( 'The titfucking tentacles squeeze tighter against your ' + Descriptors.allBreastsDescript() + ' before they cum again, much harder than before.  Each time it pops free of your flesh, a wave of spunk flows out with it, rolling off your chest to drip into your already soaked face and mouth.  ', false );
			if( CoC.player.cor >= 80 && CoC.player.lib >= 70 ) {
				MainView.outputText( 'You lick your lips and swallow as much of it as possible, wallowing in the mouse-cream.  ', false );
			}
			if( CoC.player.hairLength > 0 ) {
				MainView.outputText( 'He keeps unloading it until you feel it in your ' + Descriptors.hairDescript(), false );
			} else {
				MainView.outputText( 'He keeps unloading it until you feel it on your head', false );
			}
			MainView.outputText( ', then both tentacles abruptly release, going flaccid.\n\n', false );
		}
		if( mouthFucking ) {
			MainView.outputText( 'A wide bulge forces your jaw apart, distends your throat, and finally empties into your gut, filling you with warmth.  The action repeats until you feel bloated and full, far worse than before as you\'re now filled from BOTH ends.  You look more like ', false );
			if( CoC.player.pregnancyIncubation === 0 && CoC.player.buttPregnancyIncubation === 0 ) {
				MainView.outputText( 'an eight month pregnant woman', false );
			} else {
				MainView.outputText( 'a brood-mother immobilized by her growing offspring', false );
			}
			MainView.outputText( ' than your old self at this point, and Jojo wastes no time in pulling out to paint your ' + this.faceMuzzle() + ' with his baby batter.\n\n', false );
			cumPuddle = true;
			EngineCore.dynStats( 'cor', 0.5 );
		}
		MainView.outputText( 'All of Jojo\'s tentacles seem to lose their strength, and he gently sets your violated body down ', false );
		if( cumPuddle ) {
			if( milkPuddle ) {
				MainView.outputText( 'in the puddle of cum and milk', false );
			} else {
				MainView.outputText( 'in the puddle of spunk', false );
			}
		} else {
			if( milkPuddle ) {
				MainView.outputText( 'in the puddle of milk', false );
			} else {
				MainView.outputText( 'on the ground', false );
			}
		}
		MainView.outputText( '.  You moan in frustration, feeling the fluid soak into your skin, secretly wishing he could\'ve fucked you into unconsciousness.  ', false );
		MainView.outputText( 'The mouse kneels over you and begins licking your body, cleaning you with his tongue.  The tentacles join in, noisily slurping up every ounce of fluid from your form until you\'re clean and sated.  Sighing dreamily from the attention, you close your eyes and murmer, "<i>Good boy.</i>" When you open them, he\'s trotting away towards the forest, his tentacles well-hidden again...\n\n', false );
	...
	JojoScene.prototype.jojoMilkPay = function( tentacle ) {
		...
		MainView.outputText( 'A loud \'ding\' chimes and a panel displays ', false );
		...
		MainView.outputText( cumQ / 1000 + ' Ls.  ', false );
		//Calculate payout;
		if( cumQ > 100 ) {
		...
		if( CoC.flags[ kFLAGS.WHITNEY_GEMS_PAID_THIS_WEEK ] + payout >= cap ) {
			...
			MainView.outputText( 'It also shows a warning enough gems for full payment.  GEMS NOW EXHAUSTED.</b>  ', false );
		}
		//If over cap!;
		else if( CoC.flags[ kFLAGS.WHITNEY_GEMS_PAID_THIS_WEEK ] >= cap ) {
			payout = 0;
			MainView.outputText( 'It also shows a warning EXHAUSTED.</b>  ', false );
		}
		if( payout > 0 ) {
			...
			if( payout === 1 ) {
				MainView.outputText( Utils.Num2Text( payout ) + ' gem rolls out into a collection plate.  Whitney really put a lot of work into this!  You pocket the gem and g', false );
			} else {
				MainView.outputText( Utils.Num2Text( payout ) + ' gems roll out into a collection plate.  Whitney really put a lot of work into this!  You pocket the gems and g', false );
			}
		} else {
			MainView.outputText( 'You g', false );
		}
	};
	...
	JojoScene.prototype.milkJojoFirst = function() {
		...
		//(Vagoozle!);
			...
			MainView.outputText( ', and you\'re getting quite turned on from the show.  You shuck your ' + CoC.player.armorName + ' and reveal your ' + Descriptors.vaginaDescript() + ', feeling your ' + Descriptors.clitDescript() + ' grow firm and hard.  For his part, Jojo doesn\'t even notice.  He just moans, squirms, and twitches any time he\'s brought particularly close to orgasm.  Seating yourself down below him, you let your fingers play across your now-wet folds, caressing your labia before you slide a few digits inside.  You brush your thumb against your clitoral hood and shudder from pleasure, enjoying watching your pet get milked as much as he\'s enjoying the milking.  ', false );
			if( CoC.player.clitLength > 3 ) {
				MainView.outputText( 'Your other hand grabs your penis-sized \'button\' and begins to jack it, the action sending tremors of sensation through your ' + Descriptors.hipDescript() + '.  ', false );
			}
			MainView.outputText( 'Feeling naughty, you sit up straight and lick at Jojo\'s twitching balls, observing his shame as he squirms on your nose.\n\n', false );
			CoC.player.orgasm();
		}
		//(NEITHER!);
		else {
			...
			MainView.outputText( ', and while you\'re getting quite turned on from the show, you lack the equipment to take care of your lusts in a satisfactory way.  Meanwhile, Jojo is just moaning, squirming, and twitching each time he\'s brought close to orgasm.  You sit down and watch the machinery take your mouse-toy to task, wishing you had similar endowments you could be taking care of right now.\n\n', false );
			EngineCore.dynStats( 'lus', 50 );
		}
		MainView.outputText( 'At long last, the appointed time has come, and Jojo lets out the most lewd-sounding squeak you\'ve ever heard come from his mouth.  His ass-cheeks and legs clench and flex against their restraints', false );
		if( CoC.player.hasCock() ) {
			MainView.outputText( ', squeezing your ' + Descriptors.cockDescript( x ) + ' with agonizingly pleasurable pressure', false );
		}
		MainView.outputText( '.  His back arches in ecstasy.  Most noticeably, the tube slurping on his cock is suddenly distorting as thick, round globules of mousey seed are sucked towards the wall.  The bulges are quickly smoothed out into one long, hose-dilating river of rodent-spooge.', false );
		if( !CoC.player.hasCock() && CoC.player.hasVagina() ) {
			MainView.outputText( '  His balls tremble and shrink in your mouth, pumping his prodigious semen out, wasting it in exchange for a pittance of gems.', false );
		}
		MainView.outputText( '\n\n', false );
		if( CoC.player.hasCock() ) {
			//(Too big dick: ;
			if( CoC.player.cockThatFits( 40 ) < 0 ) {
				MainView.outputText( 'You release your cum all over Jojo\'s back, matting his white fur with cum.  Each successive splatter of spunk makes the mouse look even more like a semen-drenched whore.  It\'s a good look for him.', false );
			} else {
				MainView.outputText( 'You pull out and release your cum all over Jojo\'s back, matting his white fur with cum.  Each successive splatter of spunk makes the mouse look even more like a semen-drenched whore.  It\'s a good look for him.', false );
			}
		} else if( CoC.player.hasVagina() ) {
			MainView.outputText( 'You climax and clamp down on your fingers, squeezing your hand as your own cries of pleasure mix with your cock-toy\'s milked-out moans.', false );
		} else {
			MainView.outputText( 'You sigh wistfully as Jojo ejaculates his load, wishing you could feel such exquisite sensations.  Maybe you can find something to give you some proper sexual organs soon?', false );
		}
		MainView.outputText( '  Though he cums for minutes on end, the machinery doesn\'t seem to have much problem keeping up.  Eventually the hose pops off and drags back towards the wall, leaking a small trail of mouse-spunk on the floor.\n\n', false );
		MainView.outputText( 'Jojo is lowered to the ground and released.  He immediately nuzzles against you, still trembling from the force of his orgasm and clearly thankful for the treat.  ', false );
		...
		this.jojoMilkPay();
		MainView.outputText( 'o on your way, dragging an exhausted mouse behind you as you head back towards camp.', false );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	JojoScene.prototype.repeatMilkJojo = function( tentacle ) {
		...
		if( !tentacle || !this.tentacleJojo() ) {
			MainView.outputText( 'You call for your corruption-filled mouse-pet, and Jojo immediately bounds out of the woods on all fours, his bloated dick practically dragging the ground as it dribbles behind him. He looks up at you with eyes full of hope, still remembering the pleasure of his last trip to the farm.  You sigh and say, "<i>Yes, I\'m going to take you for another milking.  Are your balls nice and full?  I expect you to earn me a few gems, my little cum-cow.</i>"\n\n', false );
			MainView.outputText( 'Jojo nods his head eagerly and sits up on his haunches, his hand darting down to fondle his dick for you and show you just how much it dribbles now.  He seems to be more than ready for another session.  You start leading him to the farm, listening to him moan and pant behind you.  Drops of pre-cum splatter into the ground with every movement, and you realize he seems to be leaking a little more heavily than the last time the two of you embarked on this journey.  Somehow either the milkers or what you did originally must be slowly increasing his \'productivity\' the more he empties out.  Perfect!\n\n', false );
			MainView.outputText( 'In no time, the two of you are back inside the barn.  Jojo practically leaps into the stall, helping you strap him into the harness in record time.  His dick ', false );
			if( this.jojoCumQ() < 1000 ) {
				MainView.outputText( 'has already dripped everywhere, making a mess of the floor.', false );
			} else if( this.jojoCumQ() < 4000 ) {
				MainView.outputText( 'is drizzling a steady flow of man-slime, puddling on the floor.', false );
			} else {
				MainView.outputText( 'is already pouring out a river of pre-cum, turning the floor into a slippery morass.', false );
			}
			MainView.outputText( '  The mouse is quickly immobilized and left to hang there, waiting on you to finish strapping him into his place.  You tune out his frantic whines and admire your handiwork.  Jojo is utterly restrained and hanging in the air.  His dick is throbbing and dripping in anticipation.  Even his balls are twitching and shuddering, aching to release their heavy cream.  You give his weighty orbs a squeeze and pull the last strap into place, activating the machinery.\n\n', false );
			MainView.outputText( 'Your white-furred slut is hauled up into the air, kept up high enough that he can\'t get any kind of traction.  He can barely move at all!  Well, most of him can barely move - the way his dick is bobbing around makes it clear that no matter how you tie him down, Jojo will find a way to squirm like a bitch.  Once again, the wall pops open and releases a thick tube.  Just looking at the pulsating purple opening, you can tell it will be a near-perfect fit for your fuckpet\'s throbbing erection.  The faint sounds of suction mix with wet squishes as the hose rises up, homing in on Jojo\'s bouncing shaft.\n\n', false );
			MainView.outputText( 'Lunging forward, the tube\'s opening slips around your mouse\'s cock-head, slurping it up as the sound of mechanized suction whines higher.  Jojo\'s own voice rises as well, matching the machine pitch for pitch as he moans in bliss.  Lurid slurps fill the barn as the tube greedily devours the mouse\'s boner, eventually coming to rest snugly against his groin and bouncing, squirming gonads.  Now that his member has been completely encapsulated by the machinery, Jojo sighs blissfully.  His hips are rocking gently against the restraints, as if it would somehow increase the pleasure coursing through his penis.  You know better - he gets to hang there on the edge for most of the next hour.\n\n', false );
			MainView.outputText( 'Giving your furry slut\'s clenching ass a smack, you walk around in front of him, nearly tripping over the bouncing tube as you change position.  Jojo\'s panting with his eyes nearly rolled back into his head.  Every fifteen seconds or so his body will clench, only to have the cock-sucker\'s motions slow torturously.  He squeals and squeaks each time as if it was the first time.  You scratch him behind his large, dish-shaped ears as words bubble up from within you.  "<i>Good pet.  You love being milked.  Your balls get so full and heavy... so full of spunk.  They ache, don\'t they?  Yes... they hurt you so bad.  You get so much cum pent up in there, and you can\'t let it out, not without being sucked like this.  Shhh... shhh... it\'s okay.  Just enjoy how perfectly swollen your balls are getting.  You can let it out eventually.</i>"\n\n', false );
			MainView.outputText( 'Jojo\'s eyes roll the whole way back for a moment, and his mouth begins to froth as he utterly caves in to his animal, pleasure-seeking instincts.  You lean back against a smooth part of the wall and shed your clothes, beginning to masturbate directly in front of your stall.  In time, your own pleasured moans draw your pet from his pleasure-induced stupor, and he watches with wide, bloodshot eyes as you bring yourself off to climax after climax', false );
			if( !CoC.player.hasCock() && !CoC.player.hasVagina() ) {
				MainView.outputText( ', even though to do so you\'re forced to stimulate your ' + Descriptors.assholeDescript(), false );
			}
			MainView.outputText( '.  His twitching rodent-nose sniffs at the air wildly as he pants, inhaling your scent between long, forlorn sounding moans.\n\n', false );
			MainView.outputText( 'Jojo\'s muscles seize, and you can tell his orgasm has arrived.  You smirk and lean closer, tugging an ear close to your mouth for another dose of \'encouragement\'.  "<i>Cum for me, pet.  Let all that spunk clogging up your balls boil out into the milker.  Give it all up for me.  Just let out every... thick... pent-up... liter of your seed.  Empty it all!  Cum and squirt until your balls are as empty as your virtue!</i>"\n\n', false );
			//("<i>Normal</i>" Jojo cum) ;
			if( this.jojoCumQ() < 1000 ) {
				MainView.outputText( 'Your poor semen-slave does not disappoint.  His balls, looking far more swollen than when you arrived, tremble beneath him in time with his bouncing cock, convulsing as they begin to obey your commands.  Big spheres of cum suddenly distend the hose, stretching it with their liquid weight as they\'re pumped away toward the wall.  Jojo happily squeaks with each fresh burst he releases, though he rapidly devolves into just constantly screeching and moaning as his dick begins to continually twitch, unloading the rest of his spermy cargo in one thick deluge.  He hangs over the slightly dilated, whirring hose as it slurps down all of his cum.  Eventually, the mouse is totally drained, and his body goes limp in the harness.\n\n', false );
				MainView.outputText( 'The hose slides off Jojo\'s softening member and heads back to the wall, trailing mouse-slime as it goes.  At the same time, Jojo is lowered down to the floor.  He stands up on wobbling feet and stumbles down to kneel beneath you.  Looking up, he pants, "<i>Did I cum enough?</i>"\n\n', false );
				MainView.outputText( 'You pat his head and answer, "<i>Yes, for now.  I expect your swollen balls can do better next time though, can\'t they?</i>"\n\n', false );
			}
			//("<i>Big</i>" Jojo Cum);
			else if( this.jojoCumQ() < 4000 ) {
				...
				MainView.outputText( 'Finished, the hose slides off Jojo\'s hard, cum-drooling cock and retracts towards the wall, leaving a messy trail of mouse-spunk in its wake.  At the same time, Jojo is lowered down to the floor.  He finds his bearings, waking up to stumble over to kneel before you.  Looking up, he pants, "<i>Was... was that enough?</i>"\n\n', false );
				MainView.outputText( 'You pet him affectionately and answer, "<i>Not too bad, pet.  I still think those balls of yours can do more.  Maybe someday you\'ll put out so much that the machine can\'t take it.  Only then will you be cumming enough for me.</i>"\n\n', false );
			}
			//("<i>Victory</i>" Jojo Cum);
				...
				MainView.outputText( 'Finally finished, you release the tube and let it slither towards its home in the wall.  It burps out a wave of cum that stains the floor as it goes.  Meanwhile, Jojo\'s dick is still rock-hard and dumping his alabaster slime.  Once lowered to the ground, he rouses from his state and drops to his knees, right in the middle of his spunk-puddle.  With heavy-lidded eyes, he asks, "<i>Did I cum enough?</i>"\n\n', false );
				MainView.outputText( 'You pet him affectionately and reply, "<i>Yes you did.  You\'re such a good cum-slave.</i>"\n\n', false );
			}
			this.jojoMilkPay();
			MainView.outputText( 'o on your way, dragging an exhausted mouse behind you as you head back towards camp.', false );
		}
	...
	JojoScene.prototype.jojoPaysForPerms = function() {
		this.jojoSprite();
		MainView.outputText( '', true );
		MainView.outputText( 'Lynnette the goblin answers the door and lets you in, waving you deeper into her shop.  Her shining black dress barely contains her fertile-hips and jiggling chest as she greets you, "<i>Welcome back honey!  Who\'s the cutie?</i>"\n\n', false );
		MainView.outputText( 'You answer that your pet mouse, Jojo will be taking care of payment for you today.  Lynnette looks doubtful, but she quickly drops to her knees in front of the mouse, determined to get her payment one way or another.  Jojo\'s forearm-length erection immediately makes itself apparent.  The goblin matron coos at the sight, humming happily once she sees the rivulets of pre-cum streaming down your slut\'s cock.  In one smooth motion, she devours the heavy mouse-shaft, sliding her lips down all the way to the base.\n\n', false );
		MainView.outputText( 'Jojo moans and reaches for Lynnette\'s hair, but you stop him with a sharp command.  He\'s paying for your services today, not getting a reward.  Big, lusty eyes look up at him from his loins, and he shudders, his heavy balls clenching and relaxing from the goblin\'s perfect throat-fuck.  She reaches up to caress them, ', false );
		if( this.jojoCumQ() >= 4000 ) {
			MainView.outputText( 'and releases a surprised gurgle when she feels just how heavy and full they\'ve gotten.  All that time at the milker has clearly done Jojo\'s body good, and Lynette is likely to leave with a mouse-milk mustache.', false );
		} else if( this.jojoCumQ() >= 1000 ) {
			MainView.outputText( 'and releases a surprised gurgle when she discovers their size.  Clearly all the trips to the milker have given Jojo an impressive set of testes.', false );
		} else {
			MainView.outputText( 'and rolls them back and forth in her fingers, expertly pleasuring Jojo.', false );
		}
		MainView.outputText( '  In no time flat he\'s reached the edge of his endurance.\n\n', false );
		MainView.outputText( 'Lynnette, for her part, has two wet stains on her blouse, but she ignores them as she gives the mouse\'s nuts a gentle squeeze.  He cums easily and hard from that little bit of extra stimulation.  The goblin\'s throat works to devour it all', false );
		if( this.jojoCumQ() < 1000 ) {
			MainView.outputText( ', readily swallowing most of it.  After a short time, she pulls back and lets him finish by filling her mouth.  In turn, she spits the sticky mess into a funnel that leads to who-knows-where.', false );
		} else if( this.jojoCumQ() < 4000 ) {
			MainView.outputText( ', swallowing as much as she can.  Still, Jojo is pumping out enough that Lynette\'s belly quickly grows even more gravid, and the semen froths at the edges of her lips.  Eventually she just gives up and pulls off, aiming him at a funnel.  She keeps stroking his spit-slicked shaft as he\'s drained off that way, the goblin\'s grin growing a touch manic at the sight of that much seed being collected.', false );
		} else {
			MainView.outputText( ', but there\'s no way she can keep up with Jojo\'s ridiculous cum-production.  Two jets of seed leave her belly stretched to its limit, and the goblin matron quickly pulls back with a panicked look in her eyes, barely aiming Jojo\'s spunk-sprayer at the funnel in time for it to be filled by the next blast.  Watching with rapt attention, the green-skinned girl smiles wider and wider when she realizes just how much your pet can produce.  It\'s quite some time before his flow tapers off, but Lynnette doesn\'t seem to mind at all.', false );
		}
		MainView.outputText( '  Perhaps one of her daughters has the other end of that pipe jammed into her pussy?\n\n', false );
		MainView.outputText( 'Lynnete pants', false );
		if( this.jojoCumQ() >= 1000 && this.jojoCumQ() < 4000 ) {
			MainView.outputText( ' with a smile', false );
		} else if( this.jojoCumQ() >= 4000 ) {
			MainView.outputText( ' with a dazed, crazy-looking smile', false );
		}
		MainView.outputText( ', "<i>What\'ll it be today?</i>"\n\n', false );
	...
	JojoScene.prototype.amilyTeachingJojoBJ = function() {
		...
		if( CoC.flags[ kFLAGS.TIMES_AMILY_AND_JOJO_PLAYED_TIMES ] === 0 ) {
			MainView.outputText( 'Your pet mice make an appearance as you\'re plotting your next move, Amily practically dragging Jojo behind her, ignoring the thick ropes of spunk he drizzles on the ground in his path.  He pants and whines, "<i>-don\'t wanna suck cocks!  Fucking is totally better.</i>"' );
		...
		//Universal Outro;
		MainView.outputText( '  She seems visibly distracted by the mouse\'s boner, and will likely wind up fucking him soon.' );
		if( CoC.player.hasCock() ) {
			MainView.outputText( ' Do you surprise them both by allowing Amily to demonstrate on you?' );
		} else {
		...
		EngineCore.addButton( 1, 'Watch', this, this.amilyAndJojoFuck );
	...
	JojoScene.prototype.BJRidesGETYOUONE = function() {
		...
		if( CoC.player.biggestCockArea() > 50 ) {
			MainView.outputText( 'as much of your [cock biggest] as possible' );
		} else {
			MainView.outputText( 'your entire [cock biggest]' );
		}
		MainView.outputText( ' into her mouth and throat.  The tight warmth of your pet\'s puffy, corruption-engorged cock-sucker shoots tingling thrills of delight through your maleness, and as she\'s careful to guide the sensitive underside along her tongue as she swallows, [eachCock] soon stiffens to a full, throbbing erection.  You cannot help but groan at the skillful sucking, and when Amily pops off a moment later, she looks quite pleased with herself.' );
		...
		MainView.outputText( 'almost too great to resist.  Amily seems to sense this, obediently offering their bodies for your release. "<i>[Master], would you like to fill your cum-slut\'s pussy or mouth?  Or maybe you want to give your bitch-boy a taste?</i>"' );
		...
		EngineCore.choices( 'Amily\'s Mouth', this, this.fillAmilysMouth,
			'Amily\'s Pussy', this, puss,
			'Jojo\'s Mouth', this, this.fillJojosMouthWithHotStickyCum,
			'', null, null,
			'', null, null );
	};
	...
	JojoScene.prototype.fillAmilysMouth = function() {
		SceneLib.amilyScene.amilySprite();
		MainView.clearOutput();
		MainView.outputText( 'You tell Amily to open wide, and she gleefully yanks your [cock biggest] away from Jojo.  He whines pathetically but, servile as he is, the mouse-boy' );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( ' leans down to desperately suckle at your [sack], subserviently worshipping your bloated testes and their liquid bliss one after another; tonguing, sucking, and moaning into the spunk - bloated mass of your ambrosial scrotum.' );
		} else {
			MainView.outputText( ' leans down to slurp needily at the bottom of your cock, licking it with languid tongue-strokes each time Amily gives him an opening.' );
		}
		MainView.outputText( ' Amily looks up at you, her cute, succubi - like horns glinting in the light as she purrs, "<i>Please, [master], give slut a throat-thick load to swallow.  Fill your nympho\'s mouth-pussy with sticky baby batter and splatter her whorish face with oozing leftovers.</i>"' );
	...
	JojoScene.prototype.stuffAmilysMouthWithPostBJCUM = function() {
		...
		MainView.outputText( 'on your rigid fuck-tool' );
		MainView.outputText( '.  She squeaks in surprise, her yelp turning into a high-pitched moan as your [cockHead biggest] widens in preparation for your looming orgasm.  Spunk forces your cum-slit open and sprays into Amily\'s clenching uterus, gorging her lascivious cunt with the virile seed it craves.  You hold her, impaled and squirming, as you continue to spray, inseminating her moist twat with squirt after squirt of oozing cock-nectar.[if (cumQuantity > 500)   Her belly rounds out delightfully from the extra stuffing.  You run a palm across it and smile when you feel your sperm churning under her stretched skin.][if (cumQuantity >= 1000) "  Rivulets of spooge run from her overflowing pussy in thick cascades of pearl goo.  Jojo does his best to keep up, but even his growing lust for swallowing your cum can\'t match the volume of the salty surf foaming from the spasming girl\'s flush pussy."][if (cumQuantity >= 2000) "  Amily\'s eyes roll back in complete pleasure as the pressure of your cum begins to cause the ejaculate to spout from her spunk-laden twat.  Jojo gives up on swallowing it all and leans back, opening wide to catch as much as possible as your second-hand seed showers the fallen monk, an ivory rain of hot slime splattering from your freshly stuffed slut\'s cream-sated cunny."]' );
	...
	JojoScene.prototype.amilyAndJojoFuck = function() {
		...
		if( CoC.player.hasVagina() ) {
			MainView.outputText( ' It isn\'t until you notice how wet your crotch is that you come to the realization that you\'ve masturbated yourself through a few orgasms as well.' );
		} else {
			MainView.outputText( ' It isn\'t until you notice how clearheaded you are that you come to the relaization that you\'ve masturbated while watching the pair.' );
		}
		MainView.outputText( ' Maybe you\'ll get to catch them in the act again?' );
		CoC.player.orgasm();
		//{DONT FORGET PREGNANCY CHECK};
		//amilyPreggoChance();;
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	...
	JojoScene.prototype.jojoDefenseToggle = function() {
		...
		if( CoC.player.findStatusAffect( StatusAffects.JojoNightWatch ) ) {
			...
			MainView.outputText( 'You ask the monk if he could guard the camp for you at night.  He smiles politely.  "<i>Certainly, [name].</i>"' );
		}
	...
	JojoScene.prototype.postCombatRape = function() {
		this.jojoSprite();
		MainView.outputText( '  You disrobe and prepare to ' );
		if( this.monk === 5 ) {
			MainView.outputText( 'fuck your violent little slut senseless.  ' );
		} else {
	...
	JojoScene.prototype.jojosSecondRape = function() {
		MainView.clearOutput();
		MainView.outputText( 'The poor mouse is already hard... his cock is throbbing eagerly as it protrudes through the opening in his robe, looking nearly eight inches long.  You\'re pretty sure it wasn\'t that big last time.\n\n' );
		this.monk += 1;
		CoC.player.orgasm();
		EngineCore.dynStats( 'lib', -10, 'cor', 4 );
		if( CoC.player.gender === 1 ) {
			MainView.outputText( 'You force Jojo over a log, running your hands through his fur and slapping his ass.  He grunts, but it\'s impossible to tell if it\'s in frustration, anger, or arousal.  You quickly force yourself back into his ass, finding it noticably stretched from your last incursion.  ' );
			if( CoC.player.averageCockThickness() >= 3 ) {
				MainView.outputText( 'It makes little difference to your ' + Descriptors.cockDescript( 0 ) + ', he still feels tight as a virgin\'s cunt to you.  ' );
			}
			MainView.outputText( 'You grab him by the waist and fuck him powerfully, slamming your ' + Descriptors.cockDescript( 0 ) + ' in brutally hard and fast.  You notice his hands are in his crotch, feverishly masturbating his disproportionately large cock like a slut.  You start leaking pre-cum like a sieve, realizing you\'re doing it, really doing it - making this virtuous mouse into a wanton slut!  You squeeze him tightly as you cum into his bowels, his belly distending slightly as your orgasm goes on and on.  Trails of cum run down his fur as it becomes more than his ass can handle.' );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( '  Your remaining equipment showers him with jizz, more than you ever thought you could produce.' );
			}
			MainView.outputText( '  The mouse moans and cums himself, with loud moans and messy splurts coating the ground every time your hips meet.\n\n' );
			MainView.outputText( 'Eventually it ends, and you drop him into the puddled spooge like a used condom.  He lays there, idly stroking himself in a daze, his prick still swollen with need and dripping fluids.  You can\'t wait to corrupt him some more.' );
		}
		if( CoC.player.gender === 2 || CoC.player.gender === 3 ) {
			MainView.outputText( 'You throw him on the soft soil of the forest and mount him, skillfully guiding his member towards your now dripping wet hole.  As you slide down you marvel at how he feels larger and thicker than before, deliciously so.  Your ' + Descriptors.vaginaDescript( 0 ) + ' throbs in the most pleasant way as you rape his small form.  You play with your clit, watching Jojo\'s face flit between rapture and disgust.  You lick your lips and smile as the disgust vanishes, his hot jets of cum painting your cunt-walls.  You giggle and keep fucking him, hoping that somehow your corruption and lust are influencing him, turning him into your personal fucktoy.  The thought brings you over the edge.  You clamp down, your ' + Descriptors.vaginaDescript( 0 ) + ' milking, squeezing every last drop from him as his prick erupts inside you.  ' );
			...
			if( CoC.player.averageVaginalWetness() === 5 ) {
				MainView.outputText( 'Your ' + Descriptors.vaginaDescript( 0 ) + ' drenches him with your squirting girl-cum, mixed with his own seed.' );
			}
			if( CoC.player.cockTotal() === 1 ) {
				MainView.outputText( 'Jizz sprays onto his chest from your ' + Descriptors.cockDescript( 0 ) + '.  ' );
			}
			if( CoC.player.cockTotal() === 2 ) {
				MainView.outputText( 'A hail of jizz splatters over Jojo from your ' + Descriptors.multiCockDescriptLight() + '.  ' );
			}
			if( CoC.player.cockTotal() === 3 ) {
				MainView.outputText( 'A multitude of thick cum-streams splatter over Jojo from head to waist as your ' + Descriptors.multiCockDescriptLight() + ' hose him down.  ' );
			}
	...
	JojoScene.prototype.jojosThirdRape = function() {
		...
		MainView.outputText( 'It\'s no wonder the monk\'s body has betrayed him so thoroughly, his ' + CoC.monster.cockDescriptShort( 0 ) + ' is nearly ten inches long, pulsing with hot need.\n\n' );
		if( CoC.player.gender === 1 ) {
			MainView.outputText( 'You yank Jojo up from the ground and onto his knees, ' );
			if( CoC.player.cockTotal() === 1 ) {
				MainView.outputText( 'presenting your ' + Descriptors.cockDescript( 0 ) + ' to him.  ' );
			} else {
				MainView.outputText( 'presenting your ' + Descriptors.multiCockDescriptLight() + ' to him.  ' );
			}
			MainView.outputText( 'The monk betrays his violated state of mind, licking his lips demurely and opening wide.  The invitation is all you need ' );
			if( CoC.player.cockTotal() === 1 ) {
				MainView.outputText( 'to ram your ' + Descriptors.cockDescript( 0 ) + ' deep into his maw.  You roughly grab his ears, facefucking him hard, his tongue working hard to please.  ' );
			}
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 'to cram two cocks deep into his maw, making his jaw stretch obscenely.  You roughly grab his ears, facefucking him hard, his tongue working hard to please you.  ' );
			}
			MainView.outputText( 'One of Jojo\'s paws is buried in his groin, stroking his ' + CoC.monster.cockDescriptShort( 0 ) + ' with feverish intensity.  The hornier he gets, the more his throat seems to relax, allowing you to push deeper.  The glazed, lust-addled look on his face is so hot, you can\'t hold back any longer.  ' );
			if( CoC.player.cockTotal() === 1 ) {
				MainView.outputText( 'Your ' + Descriptors.cockDescript( 0 ) + ' clenches tightly, erupting ' );
			}
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 'Your twin dicks clench tightly, erupting ' );
			}
			MainView.outputText( 'hot seed into the now meek and subdued bitch-boy.  His throat squeezes around you as he presses his nose tightly against your crotch, pulling as much of you in as he can.  Mouse-spunk spatters your legs as he cums with you.\n\n' );
			if( CoC.player.lib > 60 && CoC.player.cor > 40 ) {
				MainView.outputText( 'You flip him onto his back, both of you still hard and ready for more.  He gets up on all fours and wiggles his bum tantalizingly.  You press on, ' );
				if( CoC.player.cockTotal() === 1 ) {
					MainView.outputText( 'violating his loosened sphincter, and begin to fuck him hard.  He whimpers with a mixture of pain and pleasure, your spit-lubed ' + Descriptors.cockDescript( 0 ) + ' pounding his prostate mercilessly.  Thick ropes of mousey-cum drool with each anus-stretching thrust of your cock, pooling below you.  You wickedly smile, slapping his ass, imagining him stretched further, his ass gaping, his cock huge and dripping with cum.  The strange thoughts filling your mind seem to travel straight to your balls and distill into pools of cum.  Like a long dormant volcano, you erupt, hot liquid spraying into Jojo, pumping his ass full and leaking down his legs.  He cums again, harder than before, his pulsing prick seeming to grow larger throughout his orgasm.\n\n' );
				} else {
			...
			MainView.outputText( 'You leave the exhausted mousey behind you, wondering how you\'ll take him next time.  ' );
			CoC.player.orgasm();
			if( CoC.player.lib > 60 && CoC.player.cor > 40 ) {
				MainView.outputText( 'You smile as you hear him begin masturbating in the background.  There can be no doubt, you are tainting him more and more...' );
				this.monk += 1;
				EngineCore.dynStats( 'lib', -10, 'cor', 4 );
			} else {
		...
		if( CoC.player.gender === 2 ) {
			if( CoC.player.isBiped() ) {
				MainView.outputText( 'You spread your legs and crook your finger' );
			} else {
				MainView.outputText( 'You crook your finger' );
			}
			MainView.outputText( ', beckoning him towards your ' + Descriptors.vaginaDescript( 0 ) + '.  He looks disheartened, but obediently kneels before you, his whiskers tickling your ' + Descriptors.vaginaDescript( 0 ) + ', his wet nose bumping your clit, his tongue taking long licks between your lips.  ' );
			if( CoC.player.biggestTitSize() >= 2 ) {
				MainView.outputText( 'You sigh and knead your breasts in pleasure.  ' );
			}
		...
		if( CoC.player.gender === 3 ) {
			...
			MainView.outputText( ', beckoning him towards your ' + Descriptors.vaginaDescript( 0 ) + '.  He looks disheartened, but obediently kneels before you, his whiskers tickling, his wet nose bumping your clit, his tongue taking long licks between your lips.  ' );
			if( CoC.player.cockTotal() === 1 ) {
				MainView.outputText( 'You sigh as your ' + Descriptors.cockDescript( 0 ) + ' droops over his head.  ' );
			}
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 'You sigh as your ' + Descriptors.multiCockDescriptLight() + ' pile atop his head.  ' );
			}
			if( CoC.player.biggestTitSize() >= 2 ) {
				MainView.outputText( 'You kneed your breasts, excited and filled with pleasure.  ' );
			}
			if( CoC.player.biggestLactation() >= 1.5 && CoC.player.biggestTitSize() > 2 && CoC.player.mostBreastsPerRow() >= 2 && CoC.player.breastRows.length >= 1 ) {
				MainView.outputText( 'Every sensual peak within you is mirrored with small spurts of milk from your nipples.  It eventually trickles down to Jojo\'s tongue, spurring his efforts on.  ' );
			}
			MainView.outputText( 'The mousey gets more and more into eating your box, making it harder and harder to stave off an orgasm.  You wrap your thighs around his head and quiver with passion, ' );
			if( CoC.player.averageVaginalWetness() <= 1 ) {
				MainView.outputText( 'squeezing him tightly into your ' + Descriptors.vaginaDescript( 0 ) + '.' );
			}
			...
			if( CoC.player.averageVaginalWetness() >= 5 ) {
				MainView.outputText( 'splattering him with you girlcum from your ' + Descriptors.vaginaDescript( 0 ) + '.' );
			}
			if( CoC.player.cockTotal() > 0 ) {
				MainView.outputText( '  Thick runners of your pre dribble down his neck, sticking to his fur.  ' );
			}
			MainView.outputText( '\n\nYou pull the cunt-dazed mouse away reluctantly, and bend over,' );
			if( CoC.player.isBiped() ) {
				MainView.outputText( ' spreading your legs and' );
			...
			CoC.player.cuntChange( 2, true );
			if( CoC.player.cockTotal() === 1 ) {
				MainView.outputText( 'Your ' + Descriptors.cockDescript( 0 ) + ' trembles in orgasm, squirting your load into the thick forest loam.  ' );
			}
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 'Your ' + Descriptors.multiCockDescriptLight() + ' tremble in orgasm, squirting their hot loads all over the thick forest loam.  ' );
			}
			MainView.outputText( '\n\n' );
			MainView.outputText( 'You let the drained mouse pop free, reveling in the sensation of cum pouring from ' );
			if( CoC.player.isBiped() ) {
	...
	JojoScene.prototype.jojosFourthRape = function() {
		MainView.clearOutput();
		MainView.outputText( 'Jojo flops down, eyes filled with anticipation.  His self-control has really slipped away.  The corrupted and horny mouse on display here is anathema to the studious monk you met before.  His cock is close to a foot long and over two inches thick, veiny with arousal.\n\n' );
		//Male Version;
		if( CoC.player.gender === 1 ) {
			MainView.outputText( 'The mousy former-monk kneels as you disrobe, his will nearly broken by desire.  ' );
			if( CoC.player.cockTotal() === 1 ) {
				MainView.outputText( 'You touch his head softly and guide him to your ' + Descriptors.cockDescript( 0 ) + ', smiling as he licks his lips with anticipation.  You gasp at the feeling of his soft lips and wet tongue as he pleasures you, your knees going weak from his efforts.  Jojo cups your balls and slips a fuzzy finger into your ass.  He labors to take as much of you as possible into his mouth, panting warmly and wetly around your ' + Descriptors.cockDescript( 0 ) + '.  You twitch in pleasure as he alternates pressure on your prostate with smooth strokes of his slick tongue.  ' );
			} else {
				MainView.outputText( 'You touch his head softly and guide him to your ' + Descriptors.multiCockDescriptLight() + ', smiling as he licks his lips in anticipation.  You gasp at the feelings of his soft lips and wet tongue on your heads as he pleasures you, your knees going weak from his efforts.  Jojo cups your balls and slips a fuzzy finger into your ass.  He labors to take in two shafts, struggling to swallow them as deep as possible, panting wetly around you.  You twitch with pleasure as he alternates pressure on your prostate with smooth strokes of his slick tongue.  ' );
			}
			//Too thick for him;
			if( CoC.player.averageCockThickness() > 3 || (CoC.player.dogCocks() > 0 && CoC.player.averageCockThickness() > 3) ) {
				MainView.outputText( 'Jojo is forced to back off of your ' + Descriptors.cockDescript( 0 ) + ' from time to time to come up for air, barely opening his jaw wide enough to take your girth back inside his mouth.  ' );
			}
			//Too long for him;
			if( CoC.player.averageCockLength() > 10 || (CoC.player.horseCocks() > 0 && CoC.player.averageCockLength() > 10) ) {
				MainView.outputText( 'He struggles not to gag on your ' + Descriptors.cockDescript( 0 ) + '\'s length, opening his throat as far as he can.  ' );
			}
			MainView.outputText( 'You rock back and forth on his face as he expertly keeps you on the edge of orgasm.  ' );
			if( CoC.player.cumQ() > 25 ) {
				MainView.outputText( 'Your constant stream of heavy pre practically chokes the poor mouse as he edges you mercilessly, his own equipment drizzling in sympathetic lust.  ' );
			}
			MainView.outputText( 'Jojo presses his paw hard into your ass, squeezing your prostate tightly as his hot muzzle dives deeply over your cock.  You feel the building tightness of your orgasm and pull him tightly against you as the pressure builds.  ' );
			if( CoC.player.cumQ() < 25 ) {
				MainView.outputText( 'You buck against him as you orgasm, your small squirts of cum eagerly devoured by the slutty mouse.' );
			}
			if( CoC.player.cumQ() >= 25 && CoC.player.cumQ() < 250 ) {
				MainView.outputText( 'You buck against him as you orgasm, the slutty mouse\'s throat squeezing against you as he works to swallow your seed.' );
			}
			if( CoC.player.cumQ() >= 250 && CoC.player.cumQ() < 500 ) {
				MainView.outputText( 'You spasm against him as you orgasm, the pleasure erupting into the slut-mouse\'s throat as he tries to swallow it all.  Excess cum dribbles from the corners of his mouth as you fully spend yourself.' );
			}
			if( CoC.player.cumQ() >= 500 ) {
				MainView.outputText( 'You buck against him as you orgasm, the slutty mouse\'s cheeks bulging in surprise as your cum explodes into his mouth.  Thick streams escape from the corners of his muzzle, your seed pouring into him faster than he can swallow.  Eventually you finish, and see the mouse dripping with your spunk nearly to the waist, a small bulge in his belly.' );
			}
			MainView.outputText( '  Jojo leans back, panting for breath, a dull smile on his face.  He spits a load of your cum into his paw and begins jerking himself off with it, lewdly putting on a show for you.\n\n' );
			if( CoC.player.lib > 50 && CoC.player.cor > 80 ) {
				MainView.outputText( '<b>You feel a familiar power growing within you and decide to unleash it.</b>  You grab the prayer beads from his outfit and spit on them, making them slick and wet.  Holding them below your flagging cock, you focus on the demonic visions in your mind, slowly but constantly milking larger and larger dollops of cum onto the once holy beads.  Jojo moans as he comes to understand your intent, and turns around, shaking his lithe mouse-bum at you.  You lean over him, whispering into his ear, "<i>Each defiled bead I push into you is going to make you more of a willing slut.  More of a willing receptacle for demon cum.  More of a fountain of desire waiting to be tapped by Succubi.  More my toy.</i>"\n\n' );
				MainView.outputText( 'He whimpers as you slide the first bead in, his eyes growing foggy and his bum wiggling more eagerly.  You push the second bead inside him, and feel his asshole stretch and loosen, welcoming the corruption.  The third bead slips right in, and he moans, "<i>sluuuut</i>," His cock grows longer and thicker throughout the moan, stopping at over a foot long and 3 inches thick, dribbling cum.  You whisper, "<i>Cum, my Toy,</i>" and push the remaining beads inside him.  His eyes roll back as his paws frantically milk his ' + CoC.monster.cockDescriptShort( 0 ) + ', cum spraying from him like a fountain.  Jojo trembles, losing complete control and falling away from you.  You still hold the end of his beads, and smile as they pop out, stained almost as dark as the poor mouse\'s soul.\n\n' );
				MainView.outputText( 'You walk away, leaving your new pet to explore his outlook on life, and to test your awakened powers.  ' );
				this.monk += 1;
				CoC.player.orgasm();
				EngineCore.dynStats( 'lib', -10, 'cor', 10 );
			} else {
				MainView.outputText( 'Jojo eventually cums violently, collapsing into a puddle of spent jizz.  You smile and walk away, hoping to encounter him again.  ' );
				CoC.player.orgasm();
				EngineCore.dynStats( 'lib', 2, 'cor', 1 );
			}
		}
		...
		if( CoC.player.gender === 2 || CoC.player.gender === 3 ) {
			//Oral;
			MainView.outputText( 'The mousy once-monk kneels as you disrobe, his will nearly broken by desire.  ' );
			MainView.outputText( 'You touch his head softly, guiding him to your ' + Descriptors.vaginaDescript( 0 ) + ', lips breaking into a grin as he licks his mouth with desire.  You gasp at the feeling of his wet nose as it bumps against your groin, shooting thunderbolts of pleasure from your ' + Descriptors.clitDescript() + '.  He attacks with his tongue, thrusting strikes practically attacking your ' + Descriptors.vaginaDescript( 0 ) + ' with his long, practically serpentine, tongue.  You shudder, knowing a visible sign of corruption when you see it, moisture slicking the mouse\'s face.\n\n' );
			MainView.outputText( 'Jojo moans into your folds as his ' + CoC.monster.cockDescriptShort( 0 ) );
			if( CoC.player.isBiped() ) {
				MainView.outputText( ' brushes against your calf' );
			} else {
				MainView.outputText( ' brushes against your [leg]' );
			}
			MainView.outputText( '.  You get a devilish idea, ' );
			if( CoC.player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_GOO ) {
				MainView.outputText( 'and push his ' + CoC.monster.cockDescriptShort( 0 ) + ' in between folds of slime, sandwiching it in a mass of slippery, gooey tightness.  Holding his shoulder for balance, you slowly squeeze him, gently milking out small dribbles of pre.  He redoubles his efforts, burying his nose into your ' + Descriptors.vaginaDescript( 0 ) + ', tongue swirling over your folds and around your ' + Descriptors.clitDescript() + '.  For every effort on his part you step up your own, squeezing and stroking him with your goo, doing your best to impart a fetish for being masturbated with someone\'s lower body on his slowly warping mind.  You feel a hot wetness in your slime, and it grows slicker and wetter every second. Jojo\'s cum drips out of you, glazing your jello-like form white.  ' );
			} else {
				MainView.outputText( 'and push his ' + CoC.monster.cockDescriptShort( 0 ) + ' with your ' + CoC.player.foot() + ', sandwiching it under one ' + CoC.player.foot() + ' and on top of the other.  Holding his shoulder for balance, you slowly squeeze him, gently milking out small dribbles of pre.  He redoubles his efforts, burying his nose into your ' + Descriptors.vaginaDescript( 0 ) + ', tongue swirling over your folds and around your ' + Descriptors.clitDescript() + '.  For every effort on his part you step up your own, squeezing and stroking him with your ' + CoC.player.feet() + ', doing your best to impart a ' + CoC.player.foot() + ' fetish on his slowly warping mind.  You feel a hot wetness on your ' + CoC.player.feet() + ', and they grow slicker and wetter every second. Jojo\'s cum drips out from between them, glazing them white.  ' );
			}
			if( CoC.player.averageVaginalWetness() <= 1 ) {
				MainView.outputText( 'You clamp down on his muzzle as you writhe in orgasm.  ' );
			}
			if( CoC.player.averageVaginalWetness() > 1 && CoC.player.averageVaginalWetness() <= 3 ) {
				MainView.outputText( 'Your legs trap his muzzle in your ' + Descriptors.vaginaDescript( 0 ) + ' as orgasm wracks your body.  ' );
			}
			if( CoC.player.averageVaginalWetness() === 4 ) {
				MainView.outputText( 'Your legs trap his muzzle in your ' + Descriptors.vaginaDescript( 0 ) + ', slicking his muzzle with girlcum as you spasm with bliss.  ' );
			}
			if( CoC.player.averageVaginalWetness() === 5 ) {
				MainView.outputText( 'Your legs squeeze him against your ' + Descriptors.vaginaDescript( 0 ) + ', girlcum erupting over his face and soaking him as you bliss out with orgasm  ' );
			}
			if( CoC.player.cockTotal() === 1 ) {
				MainView.outputText( 'Your ' + Descriptors.cockDescript( 0 ) + ' drizzles cum in his hair, some dripping off by his ear.  ' );
			}
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 'Your ' + Descriptors.cockDescript( 0 ) + 's drizzle him with cum, covering his head with thick white streams of your jizz.  ' );
			}
			MainView.outputText( 'Twitching weakly with pleasure, you fall back.\n\n' );
			//Fux!;
			MainView.outputText( 'The pleasure was magnificent... but you want MORE.  You push yourself up, feeling pleased as you spy Jojo stroking his cum-slick ' + CoC.monster.cockDescriptShort( 0 ) + ', thick streams of cum leaking from the tip.  You pounce on him, pushing him down on the ground, and position your ' + Descriptors.vaginaDescript( 0 ) + ' over him, allowing him to scent your sex.  With a triumphant smile, you sink down onto him, impaling yourself on his ' + CoC.monster.cockDescriptShort( 0 ) + '.  ' );
			if( CoC.player.averageVaginalLooseness() === 0 ) {
				MainView.outputText( 'He is painfully large, so much so that you feel more pain than pleasure.  ' );
			}
			if( CoC.player.averageVaginalLooseness() === 1 ) {
				MainView.outputText( 'He stretches you around him like a latex glove, pulling your ' + Descriptors.vaginaDescript( 0 ) + ' taught with his ' + CoC.monster.cockDescriptShort( 0 ) + ', the sensation riding a razor\'s edge between pleasure and pain.  ' );
			}
			if( CoC.player.averageVaginalLooseness() === 2 ) {
				MainView.outputText( 'His ' + CoC.monster.cockDescriptShort( 0 ) + ' stuffs you completely, filling your ' + Descriptors.vaginaDescript( 0 ) + ' to capacity.  ' );
			}
			if( CoC.player.averageVaginalLooseness() === 3 ) {
				MainView.outputText( 'His ' + CoC.monster.cockDescriptShort( 0 ) + ' fits you perfectly, burying deep inside your folds.  ' );
			}
			if( CoC.player.averageVaginalLooseness() === 4 ) {
				MainView.outputText( 'You easily accomadate his member into your ' + Descriptors.vaginaDescript( 0 ) + '.  ' );
			}
			if( CoC.player.averageVaginalLooseness() === 5 ) {
				MainView.outputText( 'His ' + CoC.monster.cockDescriptShort( 0 ) + ' slips inside your ' + Descriptors.vaginaDescript( 0 ) + ' with little resistance, easily sinking in to the hilt.  You muse to yourself, "<i>If only he were thicker...</i>"  ' );
			}
			MainView.outputText( 'You ride him slowly, gyrating your hips in tiny grinding circles while you run your hands through his fur.  His hips bounce you gently with tiny twitching thrusts, cum pooling out of your ' + Descriptors.vaginaDescript( 0 ) + ' as it continues to drip from him.  ' );
			MainView.outputText( 'He gradually ups the tempo, and you are forced to go along for the ride as you begin to bounce on his ' + CoC.monster.cockDescriptShort( 0 ) + '.  You grab fistfuls of his fur and hang on as he begins pounding your ' + Descriptors.vaginaDescript( 0 ) + ', his huge balls slapping against you.  Cum squirts from your pussy with each of his violent thrusts, more pouring deep inside you continually.  Jojo squeals with glee and slams his hips into yours a final time, triggering an eruption of seed in your channel.  You feel it pouring into your womb, slowly distending your belly with every shuddering pump of cum.  You orgasm helplessly, fingering your ' + Descriptors.clitDescript() + ' the whole time.  ' );
			//Futacawk here;
			if( CoC.player.cockTotal() > 0 ) {
				//Single Cock;
				if( CoC.player.cockTotal() === 1 ) {
					...
					if( CoC.player.horseCocks() === 1 ) {
						MainView.outputText( 'Your ' + Descriptors.cockDescript( 0 ) + ' feels a building pressure, the whole thing pulsating wildly with each of your heartbeats, most noticably the tip, which flares out wildly.  Powerful contractions wrack your sheath and ' + Descriptors.cockDescript( 0 ) + ' as pre practically fountains from it.  ' );
					}
					//DogFun!;
					if( CoC.player.dogCocks() === 1 ) {
						MainView.outputText( 'Your ' + Descriptors.cockDescript( 0 ) + ' feels an intense pressure, and begins bulging out obscenely above your sheath.  The knot thickens gratuitiously, filling as it pulses with need.  Cum drips from your pointed tip as it continues to bulge wider, filling you with unbearable pressure.  ' );
					}
					//Else;
					if( CoC.player.normalCocks() === 1 ) {
						MainView.outputText( 'Your ' + Descriptors.cockDescript( 0 ) + ' twitches, muscle contractions slowly working their way up from the base.  ' );
					}
					//CUMSPLOISION;
					if( CoC.player.cumQ() < 25 ) {
						MainView.outputText( 'Your body tenses and cums, spraying spurts of jizz over the mouse.  ' );
					}
					if( CoC.player.cumQ() >= 25 && CoC.player.cumQ() < 250 ) {
						if( CoC.player.horseCocks() === 1 ) {
							MainView.outputText( ' Your ' + Descriptors.cockDescript( 0 ) + ' flares wildly as musky horse-cum erupts from it, splattering over Jojo.  ' );
						}
						if( CoC.player.dogCocks() === 1 ) {
							MainView.outputText( 'The terrible pressure in your ' + Descriptors.cockDescript( 0 ) + ' finally relents, in the form of a fountain of doggie-cum, spraying out from your ' + Descriptors.cockDescript( 0 ) + ' in a steady stream that seems to last and last.  ' );
						}
						if( CoC.player.cocks[ 0 ].cockType === CockTypesEnum.HUMAN || CoC.player.cocks[ 0 ].cockType.Index > 2 ) {
							MainView.outputText( 'Your body tenses and cums a thick eruption far beyond what a normal human could produce.  Jojo is splattered with the stuff.  ' );
						}
					}
					if( CoC.player.cumQ() >= 250 ) {
						if( CoC.player.horseCocks() === 1 ) {
							MainView.outputText( 'Your ' + Descriptors.cockDescript( 0 ) + ' ripples and bulges with animalistic vigor, horse-cum splattering everywhere with each contraction.  The musky animal-jizz never seems to stop pouring from your equine organ, soaking the mouse from the waist up.  ' );
						}
						if( CoC.player.dogCocks() === 1 ) {
							MainView.outputText( 'Your ' + Descriptors.cockDescript( 0 ) + ' suddenly releases the pressure, a constant stream of doggie-cum spouting from your ' + Descriptors.cockDescript( 0 ) + ' like some kind of cum-hose.  It seems to go on endlessly, covering the mouse from the waist up with thick ribbons of doggie-spooge as your knot slowly shrinks to normal.  ' );
						}
						if( CoC.player.cocks[ 0 ].cockType === CockTypesEnum.HUMAN || CoC.player.cocks[ 0 ].cockType.Index > 2 ) {
							MainView.outputText( 'Your body tenses and cums a thick eruption far beyond what a normal human could produce.  Jojo is splattered with the stuff.  ' );
						}
				...
				if( CoC.player.cockTotal() > 1 ) {
					MainView.outputText( 'Your cocks feel a building pressure at their base. It only seems to get stronger and stronger, until at last it explodes out from you, jizz covering the poor mouse from the waist up.  ' );
				}
			}
			//Milk here;
			if( CoC.player.biggestLactation() >= 1 && CoC.player.biggestTitSize() > 3 ) {
				if( CoC.player.biggestTitSize() * CoC.player.biggestLactation() * CoC.player.breastRows.length < 8 ) {
					MainView.outputText( 'Milk sprays from your ' + Descriptors.breastDescript( 0 ) + ' in tiny streams, triggered by your orgasms.' );
				}
				if( CoC.player.biggestTitSize() * CoC.player.biggestLactation() * CoC.player.breastRows.length < 20 &&
					...
					MainView.outputText( 'Milk erupts from your ' + Descriptors.breastDescript( 0 ) + ', spraying out over the mouse, squirting out the contractions of each shuddering orgasm.' );
				}
				if( CoC.player.biggestTitSize() * CoC.player.biggestLactation() * CoC.player.breastRows.length < 35 &&
					...
					MainView.outputText( 'Milk erupts from your ' + Descriptors.breastDescript( 0 ) + ', spraying in pulsing blasts, soaking the mouse.  Each nerve-wracking orgasm seems to wring more and more milk from you, until it puddles around.' );
				}
				if( CoC.player.biggestTitSize() * CoC.player.biggestLactation() * CoC.player.breastRows.length >= 35 ) {
					MainView.outputText( 'Milk fountains from your ' + Descriptors.breastDescript( 0 ) + ', soaking the mouse with a continuous river of cream.  For every blob of cum you feel pushing into your over-filled uterus, another torrent of milk sprays out.  As your mind-bending orgasms drag on, a small lake of milk forms around you.' );
				}
			}
	...
	JojoScene.prototype.jojosFifthRape = function() {
		MainView.clearOutput();
		MainView.outputText( 'Jojo smiles serenely, pleased at the outcome, a foot of tumescent mouse-meat bobbing at attention.\n\n' );
		//Placeholder till I'm less lazy;
		MainView.outputText( 'You fuck your mousey slut for what feels like hours, orgasming until both of you are tired and worn out.  ' );
		CoC.player.orgasm();
		EngineCore.fatigue( -20 );
		if( CoC.player.lib > 40 ) {
			MainView.outputText( 'When you\'re done you feel more clear-headed, but Jojo looks hornier than ever.' );
	...
	JojoScene.prototype.loseToJojo = function() {
		...
		if( this.monk === 2 || this.monk === 3 ) {
			...
			MainView.outputText( 'Jojo grins wickedly as he senses your defeat, ' + CoC.monster.cockDescriptShort( 0 ) + ' throbbing hard.  ' );
			if( CoC.player.lust >= 100 ) {
				if( CoC.player.gender === 1 ) {
					MainView.outputText( 'Too aroused to think, you just bend over, displaying your bum and letting your ' + CoC.player.multiCockDescriptLight() + ' dangle freely.  The mouse doesn\'t hesitate, and he thrusts his ' + CoC.monster.cockDescriptShort( 0 ) + ' with painful force.  You stagger from the size and struggle to stay conscious as he fucks you like a mad beast, hammering your ass with incredible force.  ' );
					if( CoC.player.cockTotal() === 1 ) {
						MainView.outputText( 'Pre and cum drip from your ' + CoC.player.cockDescript( 0 ) + ', forced out of your prostate by the rough beating it\'s taking.  You feel a flash of warm wetness inside you, and realize Jojo is cumming.  A sense of relief washes over you as the last burst of cum squirts out from your cheeks, only to be replaced with a dawning sense of horror as he continues fucking you harder than ever.\n\nYou black out after a few dozen of his orgasms and one or two of your own, your gut painfully distended with semen.' );
					}
					if( CoC.player.cockTotal() > 1 ) {
						MainView.outputText( 'Pre and cum drip from your ' + CoC.player.cockDescript( 0 ) + 's, forced out of your prostate by the rough beating it\'s taking.  You feel a flash of warm wetness inside you, and realize Jojo is cumming.  A sense of relief washes over you as the last burst of cum squirts out from your cheeks, only to be replaced with a dawning sense of horror as he continues fucking you harder than ever.\n\nYou black out after a few dozen of his orgasms and one or two of your own, your gut painfully distended with semen.' );
					}
					CoC.player.buttChange( CoC.monster.cockArea( 0 ), true );
				}
				if( CoC.player.gender >= 2 ) {
					MainView.outputText( 'Too aroused to think, you bend over, displaying your bum and ' + Descriptors.vaginaDescript( 0 ) + ' to Jojo as open targets.  The mouse obliges, plunging himself into you, hard.  He fucks you with abandon, pounding your wanton little pussy with no regard for your pleasure.  Despite yourself, you enjoy the rough treatment.  A spasm of warmth erupts inside you as Jojo cums.  You worry he might stop, but as the mouse\'s orgasm ends he resumes fucking with even greater energy. You cum powerfully, his jizz seeping down your thighs as you begin lose track of yourself.  ' );
					if( CoC.player.cockTotal() > 1 ) {
						MainView.outputText( 'Your ' + CoC.player.cockDescript( 0 ) + ' splatters the ground with cum repeatedly, until both your genders are raw and sore.  ' );
					} else {
						MainView.outputText( 'Your ' + Descriptors.vaginaDescript( 0 ) + ' cums on him many more times it until it is sore and tender, dripping with spunk.  ' );
					}
					MainView.outputText( 'You black out as Jojo cums AGAIN, forcing a river of spunk from your already over-filled uterus.' );
				...
				if( CoC.player.gender === 0 ) {
					MainView.outputText( 'Too aroused to think, you just bend over, displaying your bum and wiggling enticingly.  The mouse doesn\'t hesitate, and he thrusts his ' + CoC.monster.cockDescriptShort( 0 ) + ' with painful force.  You stagger from the size and struggle to stay conscious as he fucks you like a mad beast, hammering your ass with incredible force.  ' );
					MainView.outputText( 'You feel a flash of warm wetness inside you, and realize Jojo is cumming.  A sense of relief washes over you as the last burst of cum squirts out from your cheeks, only to be replaced with a dawning sense of horror as he continues fucking you harder than ever.\n\nYou black out after a few dozen of his orgasms and one or two of your own, your gut painfully distended with semen.' );
				}
				CoC.player.slimeFeed();
			...
			//HP Defeat;
				...
				MainView.outputText( 'You black out from the pain of your injuries.\n\n' );
				MainView.statsView.show();
			}
		}
		Combat.cleanupAfterCombat();
	...
	JojoScene.prototype.corruptJojoSexMenu = function() {
		...
		if( CoC.player.hasVagina() ) {
			...
			EngineCore.addButton( 8, 'Anal Smother', this, this.corruptJojoAnalSmother );
		}
		if( CoC.player.hasCock() ) {
		...
		EngineCore.addButton( 1, 'Give BJ', this, this.corruptJojoCunnilingus );
		if( CoC.player.biggestTitSize() >= 2 ) {
	...
	JojoScene.prototype.corruptJojoBJCruel = function() {
		...
		MainView.outputText( 'You yell out into the jungle, "<i>Slut!</i>" Minutes later Jojo slips into your camp from the jungle\'s shadows, dropping to his knees with a timid look of fear in his eyes. You step forward and grip the fur between his shell-like ears firmly, hissing angrily, "<i>When I call for you, you need to be here. Do I need to teach you your place again?</i>"  ' );
		MainView.outputText( 'He shakes his head as you say this, trying to marshal up the strength to resist you. You draw your teeth back in a snarl of anger at this resistance and punch the mouse in the gut, dropping him to his knees gasping for breath.  ' );
		if( CoC.player.cocks[ x ].cArea() < 10 ) {
			MainView.outputText( 'You grip the fur on his head tightly in one hand and pull his mouth over your ' + CoC.player.cockDescript( x ) + ', thrusting into his muzzle with little concern for letting him catch his breath. You shove your length down his throat and start sawing away, making the mouse\'s eyes roll back from breathlessness. You can feel the muscles of his throat grip and spasm around your cock flesh as he chokes on the length, his thin lips trembling around your.  ' );
		} else if( CoC.player.cocks[ x ].cArea() < 36 ) {
			MainView.outputText( 'You grip the fur on his head tightly in one hand and pull his mouth over your ' + Descriptors.cockDescript( x ) + ', thrusting into his muzzle with little concern for letting him catch his breath. The girth of your ' + CoC.player.cockDescript( x ) + ' nearly dislocates his jaw. You can feel his throat stretching around you, like a hot, wet, tight sleeve, trembling with the pulse of his racing heart as you grind in and out of his mouth.  ' );
		} else {
			MainView.outputText( 'You grip the fur on his head tightly in one hand and pull his mouth over your ' + CoC.player.cockDescript( x ) + ', thrusting against his muzzle with your ' + CoC.player.cockDescript( x ) + '. You can feel his buck teeth scratching against the top and bottom of your ' + CoC.player.cockDescript( x ) + '\'s crown, but it does nothing to prevent what is to come. He lifts his hands to try to push your huge erection away, and since you can\'t fit your girth in his mouth, you decide to use that; grabbing hold of his hands and using them to stroke your length.  ' );
		}
		MainView.outputText( 'His eyes turn to you in fear and his body shudders for lack of breath, but it does nothing more than stoke the fires of your lust. You groan in pleasure as your balls draw up tight, churning with your corrupted seed, and in a rush you feed it to him, your orgasm overtaking you as surge after hot surge of cum flares through your flesh and into his throat.  ' + (CoC.player.hasVagina() ? 'A sympathetic orgasm hits your pussy, causing a surge of feminine juices to splash against his chest and dribble down your thighs lewdly.  ' : '') );
		MainView.outputText( 'Your orgasm seems to last forever, filling his belly with your corrupted essence, causing his stomach to bulge slightly with the sheer volume of it. You pull away at last, letting him gasp for breath and fall to the ground, curling around his bloated belly.  ' );
		MainView.outputText( 'You sneer at him and shake your head, hissing out, "<i>It would be so much better for you if you didn\'t try to resist, my slut.</i>"  ' );
		CoC.player.orgasm();
		EngineCore.dynStats( 'cor', 0.5 );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	JojoScene.prototype.corruptJojoBJGentle = function() {
		...
		MainView.outputText( 'His eyes turn to you in fear and awe, pleading for release, and a slip of your foot to his own straining erection lets you know how in need of an orgasm he is, but this time is yours. You groan in pleasure as your balls draw up tight, churning with your corrupted seed, and in a rush you feed it to him, your orgasm overtaking you as surge after hot surge of cum flares through your flesh and into his throat.  ' + (CoC.player.hasVagina() ? 'A sympathetic orgasm hits your pussy, causing a surge of feminine juices to splash against his chest and dribble down your thighs lewdly.  ' : '') );
		MainView.outputText( 'Your orgasm seems to last forever, filling his belly with your corrupted essence, causing his stomach to bulge slightly with the sheer volume of it. You pull away at last, letting him gasp for breath and fall to the ground, curling around his bloated belly.  ' );
		if( CoC.player.biggestTitSize() >= 2 ) {
			MainView.outputText( 'You draw him to your bosom and kiss his forehead and then stand and go about your duties, leaving him to recover from the intense encounter and then retreat back into the jungle.  ' );
		} else {
			MainView.outputText( 'You give him one last fond caress, running your fingers through his fur in an almost patronizing petting motion, then turn without another word and leave him to retreat back into the jungle.  ' );
		}
		CoC.player.orgasm();
		EngineCore.dynStats( 'cor', 0.5 );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	...
	JojoScene.prototype.corruptJojoCunnilingus = function() {
		this.jojoSprite();
		MainView.clearOutput();
		MainView.outputText( 'You decide to finally reward your slut for all his service to you, summoning him to your camp for pleasure. He meekly appears at your bidding and you direct him to lie down on the ground before you. He does as you ask and you gently spread his legs, settling down between them.  ' );
		MainView.outputText( 'He looks at you in confusion that turns to bliss as you start to lick and caress his sheath and balls, urging the male to a full erection.  ' );
		switch( Utils.rand( 3 ) ) {
			case 0:
				MainView.outputText( 'You take the tip of his member into your mouth, suckling at it as your tongue curls at the crown and teases at the tiny slit at the tip. You take your time with him, letting your hands rub up and down his length, masturbating him slowly and giving his needy balls the occasional caress.  ' );
				break;
			case 1:
				MainView.outputText( 'You take the tip of his member into your mouth and slowly start to bob your head, one hand squeezing at his balls tenderly as your other hand strokes the length of his cock that your lips don\'t reach. You let your pace quicken over time, mimicking a vigorous fucking.  ' );
				break;
			default:
				MainView.outputText( 'You take the tip of his member into your mouth, and then take a deep breath through your nose, before dropping your head down, listening to him gasp as his cock slides all the way into your mouth and down your throat, until your nose presses against his musky sheath.  Your hands tease and squeeze at his balls, urging him to cum as your throat rhythmically swallows at his length in a milking motion.  ' );
		}
		MainView.outputText( 'You work until your slut explodes, and then, keeping all his seed in your mouth, you lift your head and press your lips to his in a firm kiss, feeding him the load of cum that he just released. He blushes as you do so, but obediently takes it all in, swallowing it down as you feed it to him.  ' );
		MainView.outputText( 'Once the vulgar kiss is finished, you stand and smile, dismissing him with a casual wave of your hand.  ' );
		EngineCore.dynStats( 'cor', 0.5 );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	JojoScene.prototype.corruptJojoVaginalGentle = function() {
		this.jojoSprite();
		MainView.clearOutput();
		MainView.outputText( 'Feeling the urge to be filled, you summon your mouse slut to you and smile as he quickly responds, moving to kneel before you reverently. You let your hand caress the side of his head, then order him to lay back.  ' );
		MainView.outputText( 'He swallows and nods, nervously obeying, stretching himself out on his back on the ground. He watches as you crawl slowly up his body and press a firm kiss to his muzzle, which he returns with the impossible lust you have planted within him. You can feel his member stirring between your legs, rising up firm against your ' + (CoC.player.hasCock() ? 'own endowments' : 'crotch') + ' as you grind your dripping slit along it.  ' );
		if( CoC.player.vaginalCapacity() < 10 ) {
			MainView.outputText( 'You lower your hand to take hold of his cock, lining it up with your entrance, and then with a soft grunt, you start to lower your weight atop him.\n' );
			...
			MainView.outputText( 'You lower your hand to take hold of his cock, lining it up with your entrance, and then with a moan of pleasure, you lower your weight atop him. His cock slides into your pussy like a hand into a glove, fitting perfectly, as though he were made for you.\n' );
			CoC.player.cuntChange( 36.4, true );
			...
			MainView.outputText( 'You shift forward, and then tilt your hips and drive back, taking his length into your wide stretched body.\n' );
			CoC.player.cuntChange( 36.4, true );
	...
	JojoScene.prototype.corruptJojoVaginalSmother = function() {
		this.jojoSprite();
		MainView.clearOutput();
		MainView.outputText( 'You feel the need to gain a little sexual relief and a mischievous idea comes to your mind, making you grin wickedly. You slip off into the jungle to seek out your monk mouse fuck toy, and when you find him, you practically pounce atop him, pinning him to his back. He struggles in surprise until he realizes that it is you, at which point he blushes and tries to look away, unable to help the erection that you are sitting against as you straddle him.  ' );
		MainView.outputText( 'You crawl further up his body and grin down at him as you press your already dripping pussy to his mouth and command sharply, "<i>Start licking if you want to breathe.</i>" His eyes go wide, but you can feel his tongue already starting to work at your lusty slit.  ' );
		if( CoC.player.vaginas.wetness > 4 ) {
			MainView.outputText( 'You moan as he works, your juices flowing liberally across his muzzle and into his mouth and nose, making him struggle not to drown in your pleasure as he focuses on giving you even more so.  ' );
		} else if( Utils.rand( 2 ) === 0 ) {
			MainView.outputText( 'You grind your slit against him as he eats you out, moaning with pleasure and writhing above him. You lift off of his face every so often, giving him just enough of a break to catch his breath before cutting it off with your pussy once again.  ' );
		} else {
			MainView.outputText( 'You settle the full of your weight against his face and laugh as you feel him struggling to pleasure you, his nose and mouth trapped tight against your slit so that every attempt to breathe is halted, making him tremble breathlessly beneath you.  ' );
		}
		MainView.outputText( 'His tongue digs deep into your body, finally bringing you to an explosive climax that leaves you shuddering thoughtlessly above him. You actually forget you are sitting on his face for a moment, feeling him go still as he nearly passes out from lack of breath before you stand up.  ' );
		MainView.outputText( 'He gasps for breath and coughs a few times, and once you are sure that he is safe, you laugh softly and walk back to your camp.  ' );
		CoC.player.orgasm();
		EngineCore.dynStats( 'cor', 0.5 );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	...
	JojoScene.prototype.corruptJojoAnalGentle = function() {
		...
		MainView.clearOutput();
		var x = CoC.player.biggestCockIndex();
		MainView.outputText( 'You watch as Jojo slinks into your camp from the dense jungle, moving timidly with his eyes focused on your feet. The sight of such a once pious monk reduced to your submissive fuck toy stirs your loins and brings a smile to your lips.  ' );
		MainView.outputText( 'You pull him against your body in a firm and possessive hug, and press your lips to his in a forceful kiss, laughing as you break the kiss to the sight of his discomfort. You pay it little mind as you gently force him back onto the ground and spread his legs. You can see in his eyes that he knows what is coming, and you can see that he is as eager for it as he is humiliated by that eagerness.  ' );
		if( CoC.player.cocks[ x ].cArea() < 10 ) {
			MainView.outputText( 'You lift the mouse\'s balls out of the way and spit down onto the crinkled star of his anus, then lever your tip to the well used hole. There is little ceremony or foreplay, but his cock is already straining erect, and a blush colors his cheeks as you push into his ass, inch by inch. You set a slow and tender pace at first, but as your orgasm nears, your thrusts become more animal and needy.  ' );
		} else if( CoC.player.biggestCockArea() < 36 ) {
			MainView.outputText( 'You slide your thick and drooling cockhead beneath the mouse\'s balls, working the musky drool of your pre-cum against the well used crinkle of his ass before forcing the thick vein-lined length of your ' + CoC.player.cockDescript( x ) + ' into him. You watch as inch after thick, vulgar inch disappears into his body, grinning as his face contorts in a mix of pain and pleasure from it, and then start to fuck him in earnest, watching as his belly bulges with each thrust of your massive prick.  ' );
		} else {
			MainView.outputText( 'You force your ' + CoC.player.cockDescript( x ) + ' against the mouse\'s ass and watch as he shakes his head, silently begging you not to do it. You smile and grip his hips, then press forward hard, forcing his body to adapt to your girth, stretching his ass and belly dangerously. You can barely get more than a foot of your ' + CoC.player.cockDescript( x ) + ' into him before bottoming out against his diaphragm, so you just fuck him with what you can, churning his insides with each thrust.  ' );
		}
		MainView.outputText( 'You pound away at the mouse\'s tight body for as long as you can, then feel your orgasm hit you hard, your balls drawing up tight as your seed churns and pulses through you and into the mouse\'s ass, filling his belly with your lust and corruption. You watch his belly swell with the seed in a beautifully vulgar display.  ' );
		MainView.outputText( 'His eyes glaze over from the intensity of the act, his teeth tightly grit, and then you can hear a keening groan from him as he falls over the edge into his own orgasm, his untouched mouse cock bouncing and jerking on his belly as his thick seed is sprayed across his chest and face lewdly. He blushes deep at the visible proof that he enjoyed what you did to him and trembles beneath you.  ' );
		MainView.outputText( 'You can\'t help but laugh at the scene, and draw out of his ass with a groan of pleasure. You watch as he crawls back into the jungle in shame, leaving a trail of your cum the whole way.  ' );
		CoC.player.orgasm();
		EngineCore.dynStats( 'cor', 0.5 );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	JojoScene.prototype.corruptJojoAnalSmother = function() {
		...
		MainView.clearOutput();
		MainView.outputText( 'You feel the need to gain a little sexual relief and a mischievous idea comes to your mind, making you grin wickedly. You slip off into the jungle to seek out your monk mouse fuck toy, and when you find him, you practically pounce atop him, pinning him to his back. He struggles in surprise until he realizes that it is you, at which point he blushes and tries to look away, unable to help the erection that you are sitting against as you straddle him.  ' );
		MainView.outputText( 'You crawl further up his body and grin down at him as he stares at your exposed pussy. You suddenly spin, sitting down the other way, so that your ass cheeks envelope his muzzle, trapping his nose and mouth against your tight pucker. "<i>Get that tongue up in there slut.</i>"  ' );
		switch( Utils.rand( 3 ) ) {
			case 0:
				MainView.outputText( 'You grind your ass against him as he eats you out, moaning with pleasure and writhing above him. You lift off of his face every so often, giving him just enough of a break to catch his breath before cutting it off with your ass once again.  ' );
				break;
			case 1:
				MainView.outputText( 'You settle the full of your weight against his face and laugh as you feel him struggling to pleasure you, his nose and mouth trapped tight against your ass so that every attempt to breathe is halted, making him tremble breathlessly beneath you.  ' );
				break;
			default:
				MainView.outputText( 'You moan as he takes you at your word, spearing his tongue deep into your anus and thrusting it in and out as though it were a sleek muscled shaft, making your body tremble in pleasure. It makes you wonder where he learned such a trick in his life as a pious monk.  ' );
		}
		MainView.outputText( 'His tongue continues to work at your ass, finally bringing you to an explosive climax that leaves you shuddering thoughtlessly above him. You actually forget you are sitting on his face for a moment, feeling him go still as he nearly passes out from lack of breath before you stand up.  ' );
		MainView.outputText( 'He gasps for breath and coughs a few times, and once you are sure that he is safe, you laugh softly and walk back to your camp.' );
		CoC.player.orgasm();
		EngineCore.dynStats( 'cor', 0.5 );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	...
	JojoScene.prototype.corruptJojoBreasts = function() { //Should only be available to players with biggestBreastSize > 2
		this.jojoSprite();
		MainView.clearOutput();
		MainView.outputText( 'You lay yourself out for a quiet moment of self pleasure, your hands moving to your breasts and fondling them gently, when the sound of a snapping twig brings your attention to the edge of camp. Jojo stands timidly, half hidden within the shadows just outside your encampment, watching you with a look of submissive desire. You smile and lift your hand, beckoning him towards you with a crook of your finger.  ' );
		MainView.outputText( 'Your mouse slut obediently slips from the darkness and into your camp, approaching you and kneeling at your side. You can see the lust in his eyes as he looks at your breasts, longing and love reflecting wonderfully. You nod your approval and let him worship your bosom.  ' );
		if( CoC.player.biggestLactation() > 1 ) { //For suckling the scene is the same regardless of player's gender
			MainView.outputText( 'He leans in and starts to kiss along your nipples before taking one into his mouth. He gives a firm suckle at the engorged teat, and you can see his eyes open wider in surprise at the sudden surge of milk that fills his muzzle. He shivers and starts to suckle in earnest, drinking from first one breast, then the other, ' );
			if( CoC.player.breastRows.length > 1 ) {
				MainView.outputText( 'and then all the others, ' ); //Extra boob coverage
			}
			MainView.outputText( 'partaking of your blessing until his belly is full.  ' );
			CoC.player.milked();
		} else if( CoC.player.biggestTitSize() <= 5 ) {
			MainView.outputText( 'He leans in to nuzzle and kiss at your breasts, his hands moving to caress the soft and full orbs in gentle worship. His kissing and licking slowly circles in on your nipples, bringing them to firm points that send jolts of warm pleasure through your body when he at last takes them into his mouth. You reach down between your legs, ' + (CoC.player.hasCock() ? 'taking hold of your shaft and masturbating it lazily as he works.  ' : 'slipping your fingers into your slit as you lazily masturbate with the pleasure he brings.  ') );
		} else {
			MainView.outputText( 'He leans in close and presses a kiss to first one nipple, then the other, starting to worship your breasts lovingly. You have other plans, however, and one hand grabs the fur at the back of his neck as the other slips beneath your breasts to pull them together to either side of his face as you press him in tight against the curves of your cleavage, forcing the mouse to fight for every breath.  ' );
		}
		MainView.outputText( 'You can hear Jojo\'s breath quickening, then his body shudders as he climaxes spontaneously, splashing his seed across your hip and belly. You can\'t help the laugh that rises from within you at his submissive gesture, watching as shame washes across his face and his ears lay back.  ' );
		MainView.outputText( 'He slinks back into the woods, chased by your amused laughter.' );
		CoC.player.orgasm();
		EngineCore.dynStats( 'cor', 0.5 );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	...
	JojoScene.prototype.beeEggsInCorruptJojo = function() {
		...
		if( !CoC.player.hasCock() ) {
			if( CoC.flags[ kFLAGS.TIMES_EGGED_JOJO ] === 0 ) {
				MainView.outputText( 'Jojo looks confused at this; he knows you don\'t have a cock!  ' );
			} else {
				MainView.outputText( 'Jojo looks confused at first, and then nods in understanding, remembering the last time you laid in him.  ' );
			}
		}
		MainView.outputText( 'He bends over, obedient slut that he is, lifting his ' );
		if( this.tentacleJojo() ) {
			MainView.outputText( 'tentacle-' );
		}
		MainView.outputText( 'tail for you in an inviting motion.  His butt looks nice and cushy, a soft heart-shape just beckoning to be impaled on your rigid egg-tool.  You give the mouse-tush a hearty swat and smile at his squeak of surprise and winking anus.' );
		MainView.outputText( '\n\nMoving forward, you ' );
		if( CoC.player.isNaga() ) {
			MainView.outputText( 'slither around him' );
		} else if( CoC.player.isGoo() ) {
			MainView.outputText( 'ooze over him' );
		} else if( CoC.player.isTaur() ) {
			MainView.outputText( 'straddle him' );
		} else {
			MainView.outputText( 'climb atop him' );
		}
		MainView.outputText( '.  Jojo looks up over his shoulder trustingly, swaying a bit from trying to hold up your body weight.  He starts to reach for his corruption-fueled boner, but of course, he has to drop his arm to hold himself upright.  The rodent whines plaintively, begging you to take him, stroke him... ANYTHING!  You gently shush him and sink your stinger into his backside, rewarding his obedience with a flash of pain and injection of fresh, unnatural lust.  His arms begin to shake, his butt begins to wiggle, and he starts to drool all over the ground, panting in a way that reminds you more of a canine than a mouse.' );
		...
		if( CoC.player.gender > 0 ) {
			...
			if( CoC.player.gender === 3 ) {
				MainView.outputText( ' and [eachCock] ' );
			}
			if( CoC.player.gender === 1 ) {
	...
	JojoScene.prototype.lowCorruptionJojoEncounter = function() {
		MainView.clearOutput();
		this.jojoSprite();
		MainView.outputText( 'Tired of exploring the forest for the moment, you decide to head back to camp.  Not feeling like taking the scenic route, you move to step through some bushes, but immediately your mind registers a yelp.  The instant you move to look at the source of the noise, a white blur smacks you right on your head.' );
		if( CoC.player.tou >= 50 && CoC.player.isBiped() === true ) {
			MainView.outputText( '  You take a few steps back, momentarily dazed.  Shaking it off, you ready your [weapon] and assume a fighting stance.\n\n' );
		} else if( CoC.player.tou < 50 && CoC.player.isBiped() === false ) {
			MainView.outputText( 'The force of the blow knocks you flat on your [ass].  Shaking it off, you immediately climb to your feet and take on a fighting stance.\n\n' );
		} else if( CoC.player.isTaur() ) {
			MainView.outputText( 'The blow does little more than leave you momentarily dazed but isn’t enough to knock you over.  You shake it off and ready your [weapon] as you assume a fighting stance.\n\n' );
		} else {
			// Was originally isNaga() only, but this will also cover Drider just as well
		}
		{
			MainView.outputText( 'You recoil as you are struck, but the force of the blow does little more than leave you momentarily dazed. You assume a fighting stance, ready to defend yourself.\n\n' );
		}
	...
	JojoScene.prototype.acceptJojosApology = function() {
		MainView.clearOutput();
		this.jojoSprite();
		MainView.outputText( 'You forgive him for hitting you and apologize for spooking him yourself, prompting a relieved sigh.\n\n' );
		MainView.outputText( '“<i>Thanks, it’s a relief to meet a friendly face,</i>” he says, his mouth breaking into a smile. “<i>Oh, where are my manners!</i>”\n\n' );
	...
	JojoScene.prototype.refuseJojosApology = function() {
		MainView.clearOutput();
		this.jojoSprite();
		MainView.outputText( 'With a smile you curl up a fist and knock the unsuspecting mouse morph upside the head, causing him drop his staff and rub the spot where you slugged him.  As he looks up at you you give his angry expression a shrug, telling him that now the two of you are even.\n\n' );
		MainView.outputText( '“<i>O-Kay</i>” The mouse says slowly, suddenly watching your movements very closely with those quick little eyes of his, “<i>But I guess it’s fair, no harm done right?</i>”\n\n' );
		MainView.outputText( 'It’s all water under the bridge to you now; after all you did slug him real good. The two of you agree to start over.\n\n' );
		this.lowCorruptionIntro();
	};
	//Intro;
	JojoScene.prototype.lowCorruptionIntro = function() {
		MainView.outputText( 'He extends a hand, which you gladly shake. “<i>My name is Jojo, pleased to meet you.</i>” You introduce yourself in kind.\n\n' );
		MainView.outputText( 'Now that you have the opportunity to take a good look at him, you notice that he is dressed in simple garbs reminiscent of a monk. A light-blue robe covers his flat chest, tied with a simple sash around his waist. His pants, similar to his robes, fit him snugly as well.\n\n' );
		MainView.outputText( 'His build is lithe, though you detect he isn’t weak by any means. His handshake is firm and transmits confidence; it’s clear that this mouse has trained well, though you can’t see any hint of muscles with his robes covering him. His hair is short and as white as his fur, you’d guess he’s an albino if not for his brown eyes. Surprisingly, he doesn’t seem to be carrying anything on his person, save for a necklace made of beads.\n\n' ); // Can't really presume that they're holy without knowing much more about him, rite?
		MainView.outputText( 'He smiles knowingly, “<i>Yes I am a monk, and yes this is a strange place for one such as I... this world was not always this way.  Long ago this world was home to many villages, including my own.  But then the demons came.  I\'m not sure if they were summoned, created, or simply a perversion of magic or breeding, but they came swarming out of the mountains to destroy everything in their path.</i>”\n\n' );
		MainView.outputText( 'Jojo sighs sadly, “<i>Enough of my woes.  Though I ' );
		if( CoC.player.cor <= 5 ) {
	...
	JojoScene.prototype.meditateInForest = function() {
		...
		if( CoC.player.statusAffectv1( StatusAffects.JojoMeditationCount ) >= 5 ) {
			...
			if( CoC.flags[ kFLAGS.FUCK_FLOWER_LEVEL ] >= 4 && CoC.flags[ kFLAGS.FUCK_FLOWER_KILLED ] === 0 && CoC.player.statusAffectv1( StatusAffects.JojoMeditationCount ) % 5 === 0 ) {
				...
				if( CoC.player.hasKeyItem( 'Jojo\'s Talisman' ) >= 0 ) { //[(if PC has Small Talisman)
					MainView.outputText( 'Jojo smiles at you.  "<i>[name], well done.  Your talent at focusing is undiminished.  Regarding the other issue... you still have the item I gave you?</i>"' );
					MainView.outputText( '\n\nYou hold up the talisman, and he nods.  "<i>Good.  Stay safe and signal me with it if you need help.</i>"' );
				} else {
					{ //(else no Small Talisman)
					}
					MainView.outputText( 'Jojo nods at you respectfully.  "<i>Well done today; your dedication is impressive.  We could meditate together more often.</i>"' );
				...
				MainView.outputText( '"<i>It seems you have quite a talent for this.  We should meditate together more often.</i>"', false );
			}
		}
		if( CoC.player.statusAffectv1( StatusAffects.JojoMeditationCount ) % 5 === 0 ) {
			...
			EngineCore.doYesNo( SceneLib.jojoScene, SceneLib.jojoScene.acceptJojoIntoYourCamp, SceneLib.camp, SceneLib.camp.returnToCampUseTwoHours );
			return;
		} else {
	...
	JojoScene.prototype.acceptOfferOfHelp = function() {
		MainView.clearOutput();
		this.jojoSprite();
		MainView.outputText( '<i>“Thank Marae.  You’re much stronger than I, my friend... to hold so much corruption and still retain your will.  But let us not tempt fate,”</i> he says before the two of you get to it.\n\n' );
		this.doClear = false;
	...
	JojoScene.prototype.jojoCamp = function() {
		...
		if( CoC.player.cor > 10 ) {
			...
			if( CoC.player.cor >= 40 ) {
				MainView.outputText( 'You walk toward the boulder where Jojo usually sits, and as soon as you\'re close Jojo approaches you with urgency.  "<i>By Marae! [name], we must do something! I feel the corruption surrounding you like a dense fog.  We need to meditate or I’m going to lose you!</i>" Jojo pleads.\n\n' );
			} else {
				MainView.outputText( 'You walk up to the boulder where Jojo usually sits, and see him sitting cross legged with his eyes closed.  He seems to be deep in meditation, but when you approach his eyes open suddenly and he gets up appearing slightly distressed, “<i>Uh... [name], I can feel a bit of corruption within you.  It is not much, but I think you should be concerned about it before it gets out of hand and you do something you might regret.  If you want to I\'d be happy to meditate with you as you rid yourself of it.</i>” he offers with a concerned look on his face.\n\n' );
			}
			MainView.outputText( 'Do you accept Jojo\'s help?\n\n' );
			EngineCore.choices( 'Yes', this, this.acceptOfferOfHelp, 'No', this, this.refuseOfferOfHelp, '', null, null, '', null, null, 'Rape', (CoC.player.lust >= 33 && CoC.player.gender > 0 ? this.jojoAtCampRape : null) );
		} else {
			{ //Normal shit
			}
			switch( Utils.rand( 3 ) ) {
				case 0:
					MainView.outputText( 'You walk toward the boulder where Jojo usually sits, and see him cross legged with his eyes closed.  At first he seems to be deep in meditation, but when you approach his mouth curls into a smile; he gets up and opens his eyes regarding you with a welcoming expression.  “<i>Greetings [name], is there anything I can assist you with?</i>”\n\n' );
					break;
				case 1:
					MainView.outputText( 'You walk up to the boulder where Jojo usually sits and find him a few paces behind it. He is standing and practicing his form, gracefully moving from one pose to the next. As you approach him you see his ears visibly perk and he turns his head towards you without breaking his stance, saying, “<i>Greetings [name], is there anything I can assist you with?</i>”\n\n' );
					break;
				default:
					MainView.outputText( 'You find Jojo sitting cross-legged on a flat rock with his staff leaning against his shoulder, thinking.  He looks to you and nods, "<i>Greetings, ' + CoC.player.short + '.  Is there something I could do to assist you?</i>"\n\n' );
			}
			this.jojoCampMenu();
		}
	};
	...
	JojoScene.prototype.jojoAppearance = function() {
		MainView.clearOutput();
		this.jojoSprite();
		MainView.outputText( 'Jojo is a white furred mouse-morph with dish-like ears and a small muzzle below a sometimes twitchy nose. He watches you with striking blue eyes.\n\n' );
	...
	JojoScene.prototype.talkMenu = function() {
		...
		EngineCore.addButton( 3, 'Forest', this, this.jojoTalkForestConvo );
		...
		if( CoC.flags[ kFLAGS.SAND_WITCHES_COWED ] === 1 || CoC.flags[ kFLAGS.SAND_WITCHES_FRIENDLY ] === 1 || CoC.flags[ kFLAGS.SAND_MOTHER_DEFEATED ] === 1 ) {
			EngineCore.addButton( 6, 'SandCave', this, this.jojoTalkSandCave );
		}
		if( CoC.flags[ kFLAGS.UNLOCKED_JOJO_TRAINING ] === 0 && CoC.flags[ kFLAGS.TIMES_TALKED_WITH_JOJO ] >= 4 ) {
			EngineCore.addButton( 7, 'Training', this, this.apparantlyJojoDOESlift );
		}
		EngineCore.addButton( 9, 'Back', this, this.jojoCamp );
	};
	//Talk;
	//Jojo’s Past;
	//Village Convo;
	JojoScene.prototype.jojoTalkVillage = function() {
		...
		MainView.outputText( 'You decide to ask Jojo about his village.\n\n' );
		MainView.outputText( 'He speaks softly with a smile on his face and in his voice, “<i>It was a small village near a large beautiful lake. We were peaceful people who laughed and trusted one another, just good simple folk you know?”\n\n' );
		MainView.outputText( '“Most of the people of Belridge were either fishers or farmers with huge families that stayed near the village. There were a few hunters and a few craftsmen. We made enemies of no one and sought to do no harm to others,</i>” Jojo says, his smile fading.\n\n' );
		MainView.outputText( 'Before you can muster a reaction to his sadness, his fuzzy cheeks spread again as he looks up at you with bright eyes.\n\n' );
		MainView.outputText( '“<i>My father was a fisherman.  He was this calm, strong man with a lot of silver whiskers that always smelled like fish.  I remember I used to go out on the boat with him and a few of my brothers and he’d always make sure to pick me up and put me on his shoulders... that is until I got too big. He always made everything look so easy, like the world was just there to shake his hand and make him smile. No one could cook seafood like he did, no one.</i>”\n\n' );
		MainView.outputText( '“<i>Then there was my mother who was a little high strung, but no one could hug you more fiercely or love you more dearly.  She was a small woman with a big soul who loved her family more than anything.  She was a seamstress before she met my dad and was always the brightest one in the room, which is hard when you have seventeen loud children clamoring for your attention.</i>”\n\n' );
		MainView.outputText( '“<i>Even with 19 people living under one roof my family wasn’t the biggest family in town, but there was always plenty work and plenty food.  It was a nice simple existence and I am thankful for the time I had with everyone in that village,</i>” he finishes with a serene smile.\n\n' );
		if( CoC.player.cor < 40 ) {
			MainView.outputText( 'Looks like Jojo\'s childhood wasn\'t so bad... you thank the mouse morph monk for sharing his treasured memories with you now that the conversation is over.\n\n' );
		} else {
	...
	JojoScene.prototype.jojoTalkForestConvo = function() {
		...
		this.jojoSprite();
		CoC.flags[ kFLAGS.TIMES_TALKED_WITH_JOJO ]++;
		MainView.outputText( 'You think for a while and decide to ask Jojo how he ended up in the forest.\n\n' );
		MainView.outputText( 'He looks at you with suddenly tired eyes as he says, “<i>Well, I was training in the fields with one of the senior brothers when we saw the monastery was under attack.  He sent me to the village to save me since I was a novice.  I decided to rally the people there.  I figured that I had ran like a coward, I wasn’t going to hide like one.  It was the village where I was born and a home to many of my brothers and sisters, both figuratively and literally.  I ran towards the village with everything I had, hoping to redeem my cowardice by returning with a militia of mice to aid the members of my order.</i>”  His voice catches and he looks away, obviously struggling to form words.\n\n' );
		MainView.outputText( 'When you open your mouth to speak he raises his hand, asking for a moment with a single furry finger.\n\n' );
		MainView.outputText( '“<i>I was too late.  The demons had struck there first, then moved on to my monastery once they were finished.  I spent hours searching the streets; every basement, every alley, every attic, every place I could think of where somebody might have hidden.  Nothing but ruined buildings, smears of assorted tainted bodily fluids, and the occasional corpse - some demons, many more mice.</i>”\n\n' );
		if( CoC.player.cor < 35 ) {
			MainView.outputText( 'That\'s terrible... you can only imagine what you\'d feel like if you returned to Ignam and saw it destroyed... your family, your friends... You put a hand on the monk\'s shoulder, intent on comforting him for the moment.\n\n' );
		} else if( CoC.player.cor >= 35 && CoC.player.cor <= 75 ) {
			MainView.outputText( 'Tough luck... thankfully your village still stands and you doubt any demons would dare attack on your watch...  You feel like you should do something for the monk though, so you put a hand on his shoulder, comforting him for the moment.\n\n' );
		} else {
			MainView.outputText( 'Mice... must\'ve been a village of wimps if a few demons could take them out... The monk is obviously distressed... maybe you should comfort him for the moment, if only to make him stop.  You put a hand on his silent shoulder...\n\n' );
		}
		MainView.outputText( '“<i>Thank you [name].  I was born there and seeing that...</i>”  The monk falls silent again.\n\n' );
		if( CoC.player.cor < 35 ) {
			MainView.outputText( 'You slide an arm around Jojo’s shoulders in an attempt to reassure the monk.  He manages a smile in response as he looks up at you.  A single tear manages to slide down his muzzle as he says, “<i>Thank you, my friend.</i>”\n\n' );
		} else if( CoC.player.cor >= 35 && CoC.player.cor <= 75 ) {
			MainView.outputText( 'You try to further console the distressed monk by moving your hand to his back and giving him a few friendly taps.  Jojo visibly pulls himself together. “Thank you, I’m alright now,” he tells you as he looks up and gives you a weak smile.\n\n' );
		} else {
			MainView.outputText( 'Seeing an opportunity, you wrap your arms around the monk as he silently tries to reign in his emotions.  Holding him close you can feel the mouse morph’s lean muscles as you rub his back ‘accidentally’ going too low and feeling the base of his tail and the top of his tight little pert ass.  As you ‘hug’ the mouse you make sure he doesn’t notice your true intentions and when you release him he actually thanks you.\n\n' );
			EngineCore.dynStats( 'lus+', 10 );
		}
		MainView.outputText( 'After you’ve comforted the monk you ask him what he did next.\n\n' );
		MainView.outputText( 'When he answers you his shoulders are squared and his voice has regained some of its former volume, “<i>I did what anyone looking at the shattered remains of their life would.  I buried them.  For the next few hours I took the time to lay each and every villager to rest before praying over them all.  Then I went back to my monastery, praying with all my heart that they had managed to hold out, at least long enough to escape rather than to be captured and twisted into perverse shells of their former selves.  Yet the monastery was another graveyard.  I found many bodies there.  Some were of the order but there were also countless imps and more than a few demons.  The place was defiled with semen and milk reeking of corruption.</i>”\n\n' );
		MainView.outputText( 'You see anger in the monk’s eyes as he clenches his fists, “<i>They had utterly defiled the monastery and there was nothing I could do about it but honor its memory.  I labored for what felt like days; burying the fallen; seeking out survivors; gathering what few items of my faith had escaped demonic desecration.</i>”  He touches the large beads around his neck meaningfully.\n\n' );
	...
	JojoScene.prototype.jojoTalkFactory = function() {
		...
		if( CoC.player.findStatusAffect( StatusAffects.FactoryOverload ) ) {
			...
			MainView.outputText( 'His ears perk at the news as you continue, telling him that you shut down the factory and destroyed the controls, for the sake of the surrounding environment.\n\n' );
	...
	JojoScene.prototype.jojoTalkSandCave = function() {
		...
		// FRIENDLY is the overall victory flag (theres a separate flag used to indicate you dun wanna be attacked by sandbitches in the desert anymore);
			...
			MainView.outputText( 'You describe your battle with the Sand Mother in an animated blow by blow. When you get to the end you tell him about how reasonable the Sand Mother actually was after you beat her.\n\n' );
			MainView.outputText( 'Jojo’s head tils to the side as he says, “<i>Maybe the whole thing didn’t need to come to an altercation in the first place, a little diplomacy on both sides....</i>”  He gives you a pointed look, “<i>Might have gone a long way.</i>”\n\n' );
		}
		//if PC just spoke to the Sand Mother;
			...
			MainView.outputText( 'You tell Jojo about how the Sand Mother spoke with you once you had battled your way to her.  You tell him she was reasonable and how the whole thing was, in the end, a simple misunderstanding.\n\n' );
			MainView.outputText( 'He marvels at the way you handled the situation, “<i>Many would have expected her trying to talk to them to be a trap [name] and hurried to attack her but not you... that is... wow [name], you are truly a great individual.</i>”\n\n' );
		}
		//[if {PC met bath slut} ;
		if( CoC.flags[ kFLAGS.MET_MILK_SLAVE ] === 1 ) {
			MainView.outputText( 'You tell Jojo about the poor mind addled thing you found sitting in a tub of milk acting as a slave to the sand witch coven.\n\n' );
			MainView.outputText( 'He shudders like a child being told a scary story and asks, “<i>What did you do?</i>”\n\n' );
			...
			if( _.isNumber( CoC.flags[ kFLAGS.MILK_NAME ] ) ) {
				MainView.outputText( 'You tell Jojo about how the Sand Mother told you the bath girl was unfit to be free and how they care for her because she can’t care for herself.\n\n' );
				MainView.outputText( 'Jojo reacts by putting his chin in his hands and thinking, “<i>Well... I guess that’s the human thing to do, especially since she doesn’t seem to be corrupted.  Maybe these sand witch covens aren’t all bad, still hard to believe that they’re on our side though....</i>”  He looks up and shrugs, “<i>Any act of charity though is a good thing.  I do hope the poor girl will be alright.</i>”\n\n' );
			}
			// [if {PC has bath slut in camp};
			else if( _.isString( CoC.flags[ kFLAGS.MILK_NAME ] ) ) {
				MainView.outputText( 'As the question leaves his lips you give Jojo a confused look and, with a glance, direct his gaze toward ' + CoC.flags[ kFLAGS.MILK_NAME ] + '.\n\n' );
				MainView.outputText( 'He slaps his own forehead and says, “<i>Oh... yeah... right.</i>” Obviously embarrassed by not putting two and two together. He smiles good naturedly though, “<i>I don’t know I guess I just assumed you found some poor mind addled soul and decided to save her.</i>” Jojo says as he looks over at ' + CoC.flags[ kFLAGS.MILK_NAME ] + '.\n\n' );
				//[if (bathSlutStage1 - unaltered);
				if( CoC.flags[ kFLAGS.MILK_SIZE ] === 0 ) {
					MainView.outputText( '“<i>She’ll fare much better in our care than in the coven’s,</i>” he states with conviction.\n\n' );
				}
				//[if (bathSlutStage2 - HHH) ;
				else if( CoC.flags[ kFLAGS.MILK_SIZE ] === 1 ) {
					MainView.outputText( '“<i>She’s already much better than she was when she got here,</i>” he says with a grin.\n\n' );
				}
				//[if (bathSlutStage3 - DD) ;
				else {
					MainView.outputText( '“<i>The coven wouldn’t have done what you’ve done for her.  You’ve given her a much, much better life and even aided in fixing her condition, you truly are a champion, [name],</i>” he says, giving you a fond smile and a pat on the back.\n\n' );
				}
			}
		}
		...
		if( CoC.flags[ kFLAGS.CUM_WITCH_DEFEATED ] === 1 || CoC.flags[ kFLAGS.BEEN_BLESSED_BY_CUM_WITCH ] === 1 ) {
			MainView.outputText( 'You tell Jojo about the cum witch, the herm witch responsible for inseminating the witches there, acting as a father to the others.  When you do he scratches his ear, “<i>Like I said, I don’t get why sex is so overly important to these creatures but whatever, continue.</i>”\n\n' );
			//[if {PC allowed Cum Witches to increase their numbers};
			if( CoC.flags[ kFLAGS.MORE_CUM_WITCHES ] === 1 ) {
				MainView.outputText( 'You tell him how you tried to remedy the current cum witch’s situation by asking the Sand Mother to make more cum witches.\n\n' );
				MainView.outputText( 'He whistles low, “<i>That’s a bold move [name].  It seems like they would want that though, it’d allow them to, er, you know... more often, and make more sand witches in the long run wouldn’t it?</i>” As the question sound rhetorical you plow on ahead.\n\n' );
			}
			//[if {PC allowed Cum Witches to rome};
			else if( CoC.flags[ kFLAGS.CUM_WITCHES_FIGHTABLE ] === 1 ) {
				MainView.outputText( 'You describe to him how you convinced the Sand Mother to allow her cum witches to rome the desert along with the sand witches and he looks at you with astonishment, “<i>You are a generous spirit [name] and this Sand Mother doesn’t seem entirely unreasonable.</i>”\n\n' );
			}
			//[if {PC did nothing to help Cum Witch} ;
			else {
				MainView.outputText( '“You tell Jojo that you’re actually done. He says, “<i>Oh... well that’s weird.</i>” and after an awkward silence, the two of you burst out laughing.\n\n' );
			}
		}
	...
	JojoScene.prototype.apparantlyJojoDOESlift = function() {
		...
		if( CoC.flags[ kFLAGS.UNLOCKED_JOJO_TRAINING ] === 0 ) {
			...
			MainView.outputText( 'You ask Jojo if he can teach you how to fight like a monk.\n\n' );
			MainView.outputText( 'Jojo considers you for a moment before saying, “<i>Yes I can teach you the forms, skills and techniques I was taught by my order. Plus...</i>” Jojo gazes off into the distance, his attention drifing for a moment before he continues, “<i>since I am all that is left, it is up to me to bestow this knowledge upon a worthy soul.</i>”\n\n' );
		...
		if( !CoC.player.findPerk( PerkLib.ControlledBreath ) && !CoC.player.findPerk( PerkLib.CleansingPalm ) && !CoC.player.findPerk( PerkLib.Enlightened ) ) {
			...
			MainView.outputText( 'By the end of the training session you are covered in sweat, your lungs heaving for breath.\n\n' );
			MainView.outputText( 'As you bow to Jojo he bows back and says, “<i>Go get some rest [name], you’ve earned it.</i>”\n\n' );
			EngineCore.fatigue( 60 );
			if( CoC.flags[ kFLAGS.TIMES_TRAINED_WITH_JOJO ] === 5 ) {
				$log.debug( 'ADDING FIRST PERK' );
				MainView.outputText( '“<i>Breathing is key.</i>”\n\n' );
				MainView.outputText( 'Jojo’s constantly repeated words resonate within you as you realize you’ve learned to control your breathing. It takes you less time to rest than normal and you feel as though you are bursting with energy because of it.  Your [fullChest]' );
				if( CoC.player.biggestTitSize() === 0 ) {
					MainView.outputText( ' rises and falls' );
				} else {
					MainView.outputText( ' rise and fall' );
				}
				MainView.outputText( ' smoothly even in the heat of battle.  From now on you know you’ll recover more quickly.\n\n' );
				MainView.outputText( '<b>(Perk Gained Breath -</b> Increases rate of fatigue regeneration by 10%<b>)</b>' );
		...
		//{after the PC has gained the controlled breath perk};
			...
			if( CoC.flags[ kFLAGS.TIMES_TRAINED_WITH_JOJO ] === 10 ) {
				...
				MainView.outputText( 'The repeated movements are slowly starting to sink in, your muscles becoming accustomed to Jojo’s training.\n\n' );
				MainView.outputText( 'By the end of the training session with the mouse, you think that you may have picked up something that might help against the denizens of this world.\n\n' );
				MainView.outputText( '<b>(Ability Gained Palm -</b> A ranged fighting technique of Jojo’s order, allows you to blast your enemies with waves of pure spiritual energy, weakening them and hurting the corrupt.<b>)</b>' );
		...
		//{after the PC has gained the Cleansing Palm attack};
			...
			MainView.outputText( 'As the two of you give each other decidedly shaky bows, Jojo says, “<i>Great effort [name], you are... wow... I need a rest. I’ve earned it.</i>”  The two of you share a laugh and end you training.\n\n' );
			EngineCore.fatigue( 60 );
	...
	JojoScene.prototype.wormRemoval = function() {
		this.jojoSprite();
		MainView.clearOutput();
		MainView.outputText( '"<i>Excellent, young one,</i>" Jojo continues. "<i>Your dedication to purification is admirable. Relax and know that the parasites will leave you soon.</i>"\n\n' );
		MainView.outputText( 'Jojo gets up and walks over to a backpack hidden in the bushes. He removes a lacquered box. He removes and combines a rather noxious combination of herbs, oils and other concoctions into a mortar and grinds it with a pestle. After a few minutes, he ignites the mixture and uses a feathered fan to blow the fumes over you. The smell of the mix is nauseating and repugnant. Your stomach turns and you fight the urge to vomit. Eventually, you are no longer able to resist and you purge yourself onto the ground. Cramping from your vomiting fits, you wrack with discomfort, which slowly builds to genuine pain. As the pain sets in, you feel a stirring deep in your crotch. The worms inside you are stirring and thus are compelling another unwanted orgasm. Unable to control your body, your cock explodes, launching cum and worms everywhere. Jojo begins fanning faster as he sees the worms leave your body.\n\n' );
		MainView.outputText( '"<i>Further endurance is needed, young one,</i>" Jojo says. "<i>The root of your problem must leave before you may pursue further purification. Healing is always twice as uncomfortable as the illness requiring attention.</i>"\n\n' );
		MainView.outputText( 'Your body cramps up as you feel the fat worm struggle. You feel it pushing up your urethra, fighting to escape your fumigated body. The worm rapidly peeks from the end of your penis. With expedience, Jojo quickly grabs the worm and pulls it out of you, triggering one last orgasm. The monk casts the fat worm to the ground and strikes it dead with his staff.\n\n' );
		MainView.outputText( '"<i>The culprit has been exorcised and will no longer trouble you. Rest here for a while and join me in some meditation to heal your exhausted body and soul.</i>"\n\n' );
		MainView.outputText( 'Being too tired for anything else, you join Jojo in meditation, which does much to relive you of your former woes.' );
