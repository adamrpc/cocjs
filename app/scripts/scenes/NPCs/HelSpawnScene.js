...
angular.module( 'cocjs' ).run( function( SceneLib, MainView, Helspawn, PregnancyStore, Combat, Descriptors, AppearanceDefs, CoC, kFLAGS, Utils, StatusAffects, EngineCore ) {
	...
	HelSpawnScene.prototype.heliasAppearanceScreen = function() {
		...
		EngineCore.outputText( 'Hel the salamander stands seven feet tall, with pale skin and thick, bright-red scales covering her arms and legs, though she has a normal human torso and face.  A fiery tail swishes gaily behind her, blazing with a ' );
		if( CoC.flags[ kFLAGS.HEL_BONUS_POINTS ] < 150 ) {
			EngineCore.outputText( 'bright orange glow' );
		} else {
			EngineCore.outputText( 'bright white glow' );
		}
		EngineCore.outputText( ' that lets off a pleasant heat, though it never seems to burn you.  Hel is wearing her scale bikini and a leather thong, and using her scimitar as a weapon' );
		//if Isabella is cool: ;
		if( CoC.flags[ kFLAGS.HEL_ISABELLA_THREESOME_ENABLED ] >= 1 ) {
			EngineCore.outputText( '; she’s got her blue bandanna wrapped around her head, mostly obscured by her fiery hair' );
		}
		EngineCore.outputText( '.  She has a human face, with bright red eyes, gentle, feminine features and a smattering of pale scales on her cheeks, like freckles.  Hel has long, bright-red hair bound in a pony-tail that hangs down her back.  She has wide-flared hips and a soft, squishy butt.  Her two reptilian legs are visibly adorned with scales and claws, ending in soft, leathery soles.' );
		if( CoC.flags[ kFLAGS.HELSPAWN_NAME ] !== 0 ) {
			EngineCore.outputText( '  A dark trio of scars run down Hel’s thighs, left by ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + '’s youthful claws.' );
		} else {
			switch( SceneLib.helScene.pregnancy.event ) {
				case 1: //She's pregnant, but no special text yet
					break;
				case 2:
					EngineCore.outputText( '  Hel\'s just starting to show a little bulge of pregnancy.' );
					break;
				case 3:
					EngineCore.outputText( '  Hel’s belly is starting to look fairly bloated, swelling with her child.' );
					break;
				case 4:
					EngineCore.outputText( '  Hel’s belly is positively gravid, full of a little salamander child.' );
					break;
				default: //She's not pregnant, check if she's ready for it
					if( CoC.flags[ kFLAGS.HEL_BONUS_POINTS ] >= 150 ) {
						EngineCore.outputText( '  Hel’s long, reptilian tail is currently burning white-hot, signaling her body’s ready for motherhood.' );
					}
			}
		}
	...
	HelSpawnScene.prototype.helSpawnsAppearanceScreen = function() {
		...
		if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] === 0 ) {
			...
			EngineCore.outputText( 'Hel’s ' );
		}
		EngineCore.outputText( 'salamander daughter, ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ', stands about six-foot-six tall, with pale skin and thick, ' );
		if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] === 1 ) {
			EngineCore.outputText( 'chitin-black' );
		} else {
			EngineCore.outputText( 'bright-red' );
		}
		...
		if( CoC.flags[ kFLAGS.HELSPAWN_PERSONALITY ] >= 50 ) {
			EngineCore.outputText( 'a scale bikini, just like her mother’s' );
		} else {
			EngineCore.outputText( 'a short skirt, thigh-high boots, and a sky-blue blouse, in stark contrast to her mother’s sluttier attire' );
		}
		if( CoC.flags[ kFLAGS.HEL_ISABELLA_THREESOME_ENABLED ] >= 1 ) {
			EngineCore.outputText( ', with a brightly-colored scarf wrapped around her neck' );
		}
		EngineCore.outputText( '. She has a human face, with ' );
		if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] === 0 ) {
			EngineCore.outputText( 'golden' );
		} else if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] === 2 ) {
			EngineCore.outputText( 'bright green' );
		} else {
			EngineCore.outputText( 'crimson' );
		}
		EngineCore.outputText( ' eyes, girlish features, and a smattering of pale red scales on her cheeks, like freckles.  Her short, dark-red hair cuts off just below her ' );
		if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] === 2 ) {
			EngineCore.outputText( 'puppy-dog' );
		} else {
			EngineCore.outputText( 'reptilian' );
		}
		EngineCore.outputText( ' ears, and it’s adorned with a pretty pink bow.  She has surprisingly large hips, and a big, squishy butt behind her.  Her reptilian legs are covered with thick, dark scales, ending in short claws atop soft, leathery soles.' );
		...
		EngineCore.menu();
		EngineCore.addButton( 0, 'Next', this.helspawnsMainMenu );
	};
	...
	HelSpawnScene.prototype.heliaBonusPointsAward = function() {
		...
		if( CoC.player.hasVagina() && CoC.player.hasCock() ) {
			EngineCore.outputText( ' and your [chest] and your [vagina]' );
		}
		if( !CoC.player.hasCock() && CoC.player.hasVagina() ) {
			EngineCore.outputText( ' [vagina] and [chest]' );
		}
		EngineCore.outputText( ' and everything else about you.  But... but that doesn\'t mean anything.  It doesn\'t.  I say I love minotaur dicks, and centaurs, and those two fox pricks at the bar filling both my holes, and I love beer and fighting and ramming my tail up peoples\' assholes.  But that\'s not real love, right?  Love is - oh, god, I\'m making a mess of this.  Again. I keep doing this; it always works out so much better in my head.</i>"' );
		...
		EngineCore.menu();
		EngineCore.addButton( 0, 'Yes', this.yesHeliaILoveYourButtHoleReaming );
		EngineCore.addButton( 1, 'No', this.noYouDontLoveHeliaYouMonster );
	};
	//No;
	HelSpawnScene.prototype.noYouDontLoveHeliaYouMonster = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 68 );
		EngineCore.outputText( 'As much as you enjoy the sex, as much as you like Hel, you can\'t say you feel that way towards her.  You grasp her shoulders, telling her as gently as possible how you feel.  She nods slowly as you speak, rubbing at her eyes by the time you\'re finished.' );
	...
	HelSpawnScene.prototype.yesHeliaILoveYourButtHoleReaming = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 68 );
		EngineCore.outputText( 'Smiling, you tell her yes.  The salamander cries out with joy, jumping into your arms and kissing you, squeezing you so tightly that you feel like you\'re about to black out by the time she breaks the kiss, stepping back with chest heaving.' );
	...
	HelSpawnScene.prototype.heliaLoveFollowup = function() {
		...
		if( CoC.flags[ kFLAGS.HEL_LOVE ] === -1 ) {
			EngineCore.outputText( 'Even if you don\'t love me, you\'d love our child, right?  You can fuck me full of kids, or even... even get someone else to, I don\'t care.  I just </i>need<i> it, [name].  More than anything else in the whole wide world.' );
		} else {
			EngineCore.outputText( 'I love you, [name].  I love you so much.  I want to share this with you.  ' );
			if( CoC.player.hasCock() ) {
				EngineCore.outputText( 'You\'ll fuck me full of kids, right?  Please?  Give me your seed, [name].  Give it all to me until I\'m fucking gravid with your salamander babies.' );
			} else {
				EngineCore.outputText( 'Even if you can\'t fuck me full of babies... you could grow a dick!  There\'s plenty of things in this god-forsaken world that do that. O-or if you don\'t want to, I bet I can get one of the fox-girls, or a minotaur, or something.  You know I can find a dick somewhere, just say the word.' );
			}
		}
		...
		EngineCore.addButton( 1, 'Another Dad', this.getAnotherDad );
		EngineCore.addButton( 2, 'No Or Later', this.noKidsHel );
	};
	...
	HelSpawnScene.prototype.haveAKid = function() {
		...
		if( CoC.player.balls > 0 ) {
			...
			EngineCore.outputText( 'the base of your prick' );
		}
		EngineCore.outputText( ' before coiling around the shaft with serpentine dexterity.  Her tail contracts and squeezes, undulating across your ' + Descriptors.cockDescript( 0 ) + ' until it finally massages a thick blob of pre from your ' + CoC.player.cockHead() + ', dribbling out to stain the wasteland ground.' );
	...
	HelSpawnScene.prototype.HaveAHellKidPartII = function() {
		...
		if( CoC.flags[ kFLAGS.HEL_LOVE ] === 1 ) {
			EngineCore.outputText( ', and I\'m so very, very glad you love me back.' );
		} else {
			EngineCore.outputText( ', even if you can\'t say the same.  And that\'s alright; it doesn\'t change my heart, and it never will. I love you' );
		}
	...
	HelSpawnScene.prototype.getAnotherDad = function() {
		...
		if( !CoC.player.hasCock() ) {
			EngineCore.outputText( 'You tell Helia you\'d love to a share a child with her, but you\'re not... properly equipped for the endeavor.  "<i>That\'s fine!  I can... I can wait, a little.  I-if you want to go grow one, I mean.  If not, then we can find someone with a cock.  ' );
			if( CoC.player.statusAffectv1( StatusAffects.TelAdre ) >= 1 ) {
				EngineCore.outputText( 'There\'s Miko and Mai from the bar.  Mai\'s said she wanted a kid, but can\'t take care of one... she\'d probably be willing to fuck one into me!  If that\'s not alright, then... lemme think.  ' );
			}
			EngineCore.outputText( 'Uh, maybe not a minotaur... they always plug more minotaurs, and I don\'t want a bull coming out of my twat.  Uh, maybe I could track down one of the spider boys from the swamp and jump on </i>his<i> dick.  They\'re pretty cute, right?  Dunno how that\'d affect a child, though.  Maybe he\'d end up with like, extra eyes, or chitin?  Still, better than an imp or some shit.  So what do you think?  Wanna grow a dick, or leave the knocking-up to someone else?</i>"' );
			//{Options in next section};
		}
		//[Another Dad] (PC has a dick);
		else {
			EngineCore.outputText( '"<i>W-wha?</i>" Hel says, cocking an eyebrow.  "<i>You sure, [name]?  I-if you don\'t want to be the dad, I guess I understand.  But why?  Am I not... no, nevermind.  I don\'t want to know.  Okay, uh, we can find someone with a cock.  ' );
			if( CoC.player.statusAffectv1( StatusAffects.TelAdre ) >= 1 ) {
				EngineCore.outputText( 'There\'s Miko and Mai from the bar.  Mai\'s said she wanted a kid, but can\'t take care of one... she\'d probably be willing to fuck one into me!  If that\'s not alright, then... lemme think.  ' );
			}
		...
		if( CoC.player.statusAffectv1( StatusAffects.TelAdre ) >= 1 ) {
			EngineCore.addButton( 0, 'Mai', this.maiWouldBeTheBestInseminator );
		}
		//[Spiderboy];
		...
		EngineCore.addButton( 3, 'No Or Later', this.noKidsHel );
	};
	//Mai;
	...
	HelSpawnScene.prototype.growingDicks4Hel = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 68 );
		EngineCore.outputText( 'You tell Hel to give you a little while, that you\'ll go find something to grow a cock for her.' );
	...
	HelSpawnScene.prototype.helGotKnockedUp = function() {
		...
		if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] === 2 ) {
			EngineCore.outputText( 'warm ' );
		} else {
			EngineCore.outputText( 'wan ' );
		}
		EngineCore.outputText( 'smile as you approach, scooting over to allow you to sit beside her.  She almost seems distant, though she\'s quick to wrap her tail around your waist, hugging you close against her.' );
		...
		if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] === 2 ) {
			EngineCore.addButton( 0, 'Sure', this.sureHelGimmeMaiDetails );
			EngineCore.addButton( 1, 'Nope', this.dontTellMeAboutMai );
		}
		//Hel Got Knocked Up by a Spiderboy;
		else if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] === 1 ) {
			EngineCore.addButton( 0, 'Sure', this.sureHelGimmeSpidahBoyDetails );
			EngineCore.addButton( 1, 'Nope', this.dontTellMeAboutSpiderboy );
		}
	};
	//Sure;
	HelSpawnScene.prototype.sureHelGimmeSpidahBoyDetails = function() {
		...
		EngineCore.menu();
		EngineCore.addButton( 0, 'Next', this.dontTellMeAboutSpiderboy );
	};
	//Nah // Combine;
	HelSpawnScene.prototype.dontTellMeAboutSpiderboy = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 68 );
		EngineCore.outputText( '"<i>So, let\'s just say I came away from that encounter carrying enough cute spiderboy cum to fill a bucket.  If I\'m not carrying his child now, then I\'m hopeless.  But... you know, I think I can feel it, [name].  I shouldn\'t be able to, but I can.  Oh, god,</i>" Hel says, looking pointedly away from you, staring wide-eyed into the morning sky.' );
		EngineCore.outputText( '\n\n"<i>What\'s wrong?</i>"' );
	...
	HelSpawnScene.prototype.sureHelGimmeMaiDetails = function() {
		...
		EngineCore.menu();
		EngineCore.addButton( 0, 'Next', this.dontTellMeAboutMai );
	};
	//Nah // Combine;
	HelSpawnScene.prototype.dontTellMeAboutMai = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 68 );
		EngineCore.outputText( '"<i>So, let\'s just say I came away from that encounter carrying enough foxy herm cum to fill a bucket.  If I\'m not carrying her child now, then I\'m hopeless.  But... you know, I think I can feel it, [name].  I shouldn\'t be able to, but I can.  Oh, god,</i>" Hel says, looking pointedly away from you, staring wide-eyed into the morning sky.' );
		EngineCore.outputText( '\n\n"<i>What\'s wrong?</i>"' );
	...
	HelSpawnScene.prototype.heliaSwollenNotice = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 68 );
		EngineCore.outputText( 'You note that Hel\'s wandering aimlessly around camp, one hand over her belly as she mumbles to herself.  You could swear she was cursing her now quite swollen belly, but suddenly she gives a girlishly happy cry and waves you over.\n' );
		...
		EngineCore.menu();
		EngineCore.addButton( 0, 'Boy', this.youWantABoy );
		EngineCore.addButton( 1, 'Girl', this.youWantAGirl );
	};
	//Boy;
	HelSpawnScene.prototype.youWantABoy = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 68 );
		EngineCore.outputText( '"<i>Is that so? Yeah, I can see it.  ' );
		if( CoC.player.hasCock() && !CoC.player.hasVagina() ) {
			EngineCore.outputText( 'If I were a guy, I\'d want a big strong son to hang out with, too.  Take him fishing, teach him how to fight the way you do... you\'ll make a great dad, my love.  I\'m sure you will.' );
		} else {
			EngineCore.outputText( 'Would be nice to have a man around here, you know?  I miss hanging around the boys back home, watching \'em strut like peacocks for every passing girl.' );
		}
	...
	HelSpawnScene.prototype.youWantAGirl = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 68 );
		EngineCore.outputText( '"<i>Yeah, a girl would be pretty great.  ' );
		//if PC is male:;
		if( CoC.player.hasCock() && !CoC.player.hasVagina() ) {
			EngineCore.outputText( 'I dunno if you had any sisters growing up, [name], but let me warn you little girl, especially a little salamandress, is going to be a hell of a handful.  But I can just see you when she\'s all grown up, packing a big old sword and threatening every boy that wants a piece of her her right or you\'ll have the CHAMPION to deal with.  Rawr.\'' );
		} else {
			EngineCore.outputText( 'We\'re going to be a gaggle of tittering girls before you know it, though.  Salamanders grow up so fast, [name]...  I just hope she doesn\'t turn out like me, you know?  I don\'t know if I could stomach seeing my little girl becoming a wanton slut like her mom.  I might get jealous, after all...' );
		}
	...
	HelSpawnScene.prototype.heliaTalkSeven = function() {
		...
		//if PC isn't daddy:;
		if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] !== 0 ) {
			EngineCore.outputText( ', even if he\'s not your get, I can hope, you know?  If he spends enough time around you, maybe he\'ll take more after you than me' );
		}
		EngineCore.outputText( '.</i>"' );
		EngineCore.menu();
		EngineCore.addButton( 0, 'EncourageHer', this.encouragePregalia );
		EngineCore.addButton( 1, 'Wellll...', this.helsLifestyle );
	};
	//[Encourage Her];
	//[Hel's Lifestyle];
	//Encourage Her;
	HelSpawnScene.prototype.encouragePregalia = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 68 );
		EngineCore.outputText( 'You squeeze Hel\'s shoulder and tell her that she\'s going to be a great mother, and that her child would do well to learn from a beautiful, powerful warrior like dear old mom.  ' );
		if( CoC.flags[ kFLAGS.HEL_LOVE ] === 1 ) {
			EngineCore.outputText( 'You love Hel' );
		} else {
			EngineCore.outputText( 'Hel\'s your friend' );
		}
	...
	HelSpawnScene.prototype.helsLifestyle = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 68 );
		EngineCore.outputText( 'Being very serious for a moment, you tell Hel that if she\'s worried about her terrible choices rubbing off on her kid, maybe she ought to reconsider some of the things she\'s done less of a slut, drink less, be less of a bloodthirsty maniac in battle.  Slowly, Hel nods to the rhythm of your words.  "<i>Yeah, maybe... maybe I should.  I don\'t know if I can change though, [name].  I\'m not as young as I was - or at least, I don\'t feel it - and I\'ve been living the way I have been for years.  I don\'t know if I could just give up the shit I do partying and the drinking, the fighting and the fucking.  It\'s part of who I am, ' );
		if( CoC.flags[ kFLAGS.HEL_LOVE ] === 1 ) {
			EngineCore.outputText( 'and I hope that\'s part of why you love me' );
		} else {
			EngineCore.outputText( 'even though I know that\'s probably part of why you can\'t love me back' );
		}
	...
	HelSpawnScene.prototype.heliaBirthEpilogue = function() {
		...
		EngineCore.menu();
		EngineCore.addButton( 0, 'Next', this.nameDatHelspawn );
	};
	//[NEXT];
	HelSpawnScene.prototype.nameDatHelspawn = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 68 );
		EngineCore.outputText( 'You and Helia share a laugh, looking down at ' );
		if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] === 0 ) {
			EngineCore.outputText( 'your ' );
		} else {
			EngineCore.outputText( 'her ' );
		}
		EngineCore.outputText( 'little girl as she suckles.  After a few happy, blissful minutes though, Hel turns to you with a question:' );
		...
		EngineCore.menu();
		EngineCore.addButton( 0, 'Next', this.applyHelspawnName );
	...
	HelSpawnScene.prototype.applyHelspawnName = function() {
		...
		if( MainView.nameBox.value === '' || MainView.nameBox.value === 'Hellgirl' || MainView.nameBox.value === 'Kid' || MainView.nameBox.value === 'Phoenix' || MainView.nameBox.value === 'Savin' || MainView.nameBox.value === 'Helia' || MainView.nameBox.value === 'Mini-doofus' ) {
			...
			if( MainView.nameBox.value === '' ) {
				...
				EngineCore.outputText( '"<i>Hey, that\'s copyright infringement.  Probably.</i>"' );
			} else if( MainView.nameBox.value === 'Kid' ) {
				EngineCore.outputText( '"<i>Wow, what are you, five?</i>"' );
			} else if( MainView.nameBox.value === 'Phoenix' ) {
				EngineCore.outputText( '"<i>Oh hell no.  You\'re not naming my little girl after one of those... things!  Sorry Kiri, but still!</i>"' );
			} else if( MainView.nameBox.value === 'Savin' ) {
				EngineCore.outputText( '"<i>That\'s a boy\'s name, dumbass.</i>"' );
			} else if( MainView.nameBox.value === 'Helia' ) {
				EngineCore.outputText( '"<i>My favorite name!  Except it\'s kinda taken, love.  Don\'t want things to get too confusing around here, do you?</i>"' );
			} else if( MainView.nameBox.value === 'Mini-doofus' ) {
				EngineCore.outputText( '"<i>Oh yeah, Kiha\'ll get a laugh out of that.  You ass.</i>"' );
			}
		...
		if( CoC.flags[ kFLAGS.HELSPAWN_NAME ] === 'Helspawn' ) {
			EngineCore.outputText( '"<i>That\'s what I was gonna call her!  Except not at all.  God dammit, seriously, [name]?</i>"\n\n' );
		}
		if( CoC.flags[ kFLAGS.HELSPAWN_NAME ] === 'Jayne' ) {
			EngineCore.outputText( '"<i>That sounds like a hero\'s name... I like it.  A name that demands respect!</i>"\n\n' );
		}
		if( CoC.flags[ kFLAGS.HELSPAWN_NAME ] === 'Hesper' ) {
			EngineCore.outputText( '"<i>Ah, the Evening Star.  She\'ll be the star of my life, alright...</i>"\n\n' );
		}
		if( CoC.flags[ kFLAGS.HELSPAWN_NAME ] === 'Kiri' ) {
			EngineCore.outputText( '"<i>Aw, that\'s great, lover.  Sis\'ll shit her pants when she hears her niece is named after her!  Gonna get confusing, though...</i>"\n\n' );
		}
		if( CoC.flags[ kFLAGS.HELSPAWN_NAME ] === 'Mai' ) {
			EngineCore.outputText( '"<i>Aw, that\'s great, lover.  Mai\'ll be honored, I know it.  And it is a great name...</i>"\n\n' );
		}
		if( CoC.flags[ kFLAGS.HELSPAWN_NAME ] === 'Tanis' ) {
			EngineCore.outputText( '\<i>I don\'t... I don\'t know if I want her to have that name, lover.  It\'s got a lot of baggage, but... but it does my mother an honor.  More than I could ever give her.</i>"\n\n' );
		}
		//Syn: ;
		if( CoC.flags[ kFLAGS.HELSPAWN_NAME ] === 'Syn' ) {
			EngineCore.outputText( '"Awesome.  I was hoping you\'d choose that one, lover mine.</i>"\n\n' );
		}
		if( CoC.flags[ kFLAGS.HELSPAWN_NAME ] === 'Chara' ) {
			EngineCore.outputText( '"<i>Awesome.  You liked my ideas, after all!</i>"\n\n' );
		}

		EngineCore.outputText( '"<i>So... ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' it is.  That\'s a good name, my love.  A strong name.  ' );
		if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] === 0 ) {
			EngineCore.outputText( 'Our' );
		} else {
			EngineCore.outputText( 'my' );
		}
	...
	HelSpawnScene.prototype.heliaTalkEight = function() {
		...
		EngineCore.spriteSelect( 68 );
		EngineCore.outputText( 'Watching your little daughter crawl around, you and Hel sit down together arm in arm, smiling as the little salamander girl plays with bits and pieces of Hel\'s gear.' );
		...
		if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] === 0 ) {
			EngineCore.outputText( 'parental ' );
		} else {
			EngineCore.outputText( 'care giver\'s ' );
		}
		EngineCore.outputText( 'pride, either.  She really is growing up faster, many times faster than a human girl.  And, by the sound of things, many times faster than a salamander ought to.  You ask Hel if that\'s right, and she nods gravely.' );
	...
	HelSpawnScene.prototype.playWithYourKid = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 68 );
		EngineCore.outputText( 'Picking ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' up off the ground, you spin the giggling little girl around, laughing as your ' );
		if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] !== 0 ) {
			EngineCore.outputText( 'ward ' );
		} else {
			EngineCore.outputText( 'daughter ' );
		}
		EngineCore.outputText( 'cries out in surprise and glee.  Soon, you\'re bouncing her, then down on the ground crawling around with her, playing peek-a-boo and a dozen other games as you and ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' laugh and hug.  Helia herself slips down with you before you know it, holding her daughter close as she tries to communicate, teaching the ever-growing girl a few words at a time.' );
	...
	HelSpawnScene.prototype.helSpawnGraduation = function() {
		...
		EngineCore.outputText( 'You wake up to a scream, high-pitched and terrified.  You bolt up out of bed, tangling up in your bedroll for a minute before stumbling over to Hel and... ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + '?' );
		...
		EngineCore.outputText( '-scaled figure curled up next to Helia; Hel herself is looking wide-eyed down at the drowsy figure on her breast, just barely opening her eyes.  "<i>Y-you\'re... you\'re huge!</i>" Hel manages to gasp out, pushing herself back and away from her daughter.' );
		...
		if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] === 2 ) {
			EngineCore.outputText( 'And the puppy ears, now more wolf-like, are a dead giveaway.  Utterly unique to her.  ' );
		}
		EngineCore.outputText( 'You sit down beside the obviously confused girl and stroke her hair, telling her it\'s all right, her mother\'s just startled, that\'s all.' );
	...
	HelSpawnScene.prototype.helspawnDiscoversBooze = function() {
		...
		if( CoC.player.race() !== 'cat-boy' && CoC.player.race() !== 'cat-girl' ) {
			EngineCore.outputText( 'a cat' );
		} else {
			EngineCore.outputText( 'the cat you are' );
		}
		EngineCore.outputText( ', and poke your head around the iron body of the still.' );
		...
		EngineCore.menu();
		EngineCore.addButton( 1, 'Scold Her', this.scoldHelSpawn );
		EngineCore.addButton( 0, 'Encourage', this.encourageHelspawn );
	};
	//Scold Her;
	HelSpawnScene.prototype.scoldHelSpawn = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 68 );
		EngineCore.outputText( 'You cross your arms over your chest and ask your little salamander exactly what she thinks she\'s doing.' );
	...
	HelSpawnScene.prototype.encourageHelspawn = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 68 );
		EngineCore.outputText( '"<i>Move over, you,</i>" you say, sliding down beside your little salamander as you pop the tap open again.  "<i>Gotta pace yourself, see,</i>" you add, taking a long draught before closing it off again, savoring the potent taste of Hel\'s mighty brew.' );
		...
		EngineCore.outputText( 'scales and the smell of a burning brewery.' );
	...
	HelSpawnScene.prototype.helSpawnChoosesAFightingStyle = function() {
		...
		EngineCore.outputText( 'her sword held at a guard pose between herself and a training dummy. ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' stands a few feet behind her mother, watching intently as Hel paces around the dummy, breathing hard.' );
		...
		EngineCore.menu();
		if( CoC.player.statusAffectv1( StatusAffects.Kelt ) >= 100 ) {
			EngineCore.addButton( 2, 'Bow', this.snipermanders );
		}
		EngineCore.addButton( 0, 'You Train', this.swordAndBoardmander );
		EngineCore.addButton( 1, 'Loan', this.dasBarbarimander );
	};
	...
	HelSpawnScene.prototype.snipermanders = function() {
		...
		EngineCore.outputText( 'You tell Hel to take a break got this handled. Grabbing your bow and quiver from over your shoulder, you hand them over to ' );
		if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] === 0 ) {
			EngineCore.outputText( 'your' );
		} else {
			EngineCore.outputText( 'Hel\'s' );
		}
		EngineCore.outputText( ' eager daughter. ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' takes them up with surprising reverence, holding them as gingerly as glass.  Grinning, you clasp her shoulder and tell her how to put the quiver on, and get her stance right for shooting.  It takes her a moment to get set up scaled, reptilian legs don\'t naturally stand in the shooting pose you\'ve adopted' );
		if( CoC.player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_LIZARD ) {
			EngineCore.outputText( ', even with your own lizard legs' );
		}
		EngineCore.outputText( ', and you find yourself having to correct her grip on the haft several times.  But, finally, you get her ready to shoot.' );
		...
		if( CoC.player.findStatusAffect( StatusAffects.CampRathazul ) >= 0 ) {
			EngineCore.outputText( '  You hear your old alchemist friend suddenly putting up a ruckus as the sounds of breaking glass echo throughout camp.  ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' looks up at you nervously, but you ruffle her fiery hair and tell her to try again. Rath is probably just fine.' );
			...
			EngineCore.outputText( '  You hear a sudden yelp of pain from across camp.  ' + CoC.flags[ kFLAGS.GOO_NAME ] + ' slithers up to you with an irritated look on her gooey face, pointing an accusing finger at an arrow sticking out of her tit.  "<i>Dammit, [name], I\'m a googirl, not a pin cushion!</i>"  You wave her off, and tell ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' to try again.' );
		}
		...
		EngineCore.outputText( 'daughter will be a warrior worthy of her parent' );
		if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] === 0 ) {
	...
	HelSpawnScene.prototype.swordAndBoardmander = function() {
		...
		EngineCore.outputText( 'You tell Hel to take a break got this handled.  You grab some kindling and string from your supplies, and a few minutes later you\'ve got a pair of small, vaguely sword-like practice weapons pieces together.  Tossing one to the young salamander, you tell her to come get some.' );
		...
		EngineCore.outputText( ' daughter on her toes.  She learns quickly, though, and soon she hardly needs reminding to keep her shield up and stance braced against Helia\'s overwhelming assaults.' );
		EngineCore.outputText( '\n\nA few hours like this every night, and ' );
		if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] === 0 ) {
			EngineCore.outputText( 'your ' );
		} else {
	...
	HelSpawnScene.prototype.dasBarbarimander = function() {
		...
		if( CoC.player.gems >= 200 ) {
			EngineCore.outputText( 'You sigh and dig out a handful of gems.  Helia beams at you, planting a quick kiss on your cheek before grabbing ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' and the loot.  "<i>Thanks for the loan, lover mine.  C\'mon, sweetie, we\'re going for a trip to town.</i>"' );
	...
	HelSpawnScene.prototype.helspawnIsASlut = function() {
		...
		if( CoC.player.weaponName !== 'fists' ) {
			EngineCore.outputText( 'clutch your [weapon]' );
		} else {
			EngineCore.outputText( 'ball your fists' );
		}
		...
		EngineCore.outputText( 'is doing.  Well, it doesn\'t take a genius to figure that out, really, but still, you have to be sure.  You slide right up to them, just out of their lines of sight, as ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' turns around, letting the spider-kin unclasp her bra and grab her tits, squeezing the small mounds as ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' suppresses a moan, reaching back to slip a hand into the spider\'s crotch, grabbing her - his dick.' );
		...
		if( CoC.player.cor >= 50 ) {
			EngineCore.outputText( '  And by mistake you clearly mean old enough to get her ass fucked in by a cute little spider trap... you contemplate jumping in on it, but with a sigh figure it\'s ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + '\'s catch.  Let her enjoy it.' );
		}
		EngineCore.menu();
		EngineCore.addButton( 0, 'Stop Them', this.helSpawnStopFucking );
		EngineCore.addButton( 1, 'Do Nothing', this.helspawnDoNothing );
	};
	//Do Nothing;
	...
	HelSpawnScene.prototype.helSpawnStopFucking = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 68 );
		EngineCore.outputText( 'You sigh and step out of the shadows, grabbing the feminine little spider boy by the shoulder and yanking him away from ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + '.  "<i>And just what the hell do you two think you\'re doing?</i>" you yell, scowling ' );
		if( CoC.player.tallness >= 90 ) {
			EngineCore.outputText( 'up ' );
		} else if( CoC.player.tallness <= 72 ) {
			EngineCore.outputText( 'down ' );
		}
		EngineCore.outputText( 'at your ' );
		if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] === 0 ) {
			EngineCore.outputText( 'daughter ' );
		} else {
			EngineCore.outputText( 'ward ' );
		}
		EngineCore.outputText( '. ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' gives a shrill cry of surprise and tumbles away from you, trying to cover herself as best she can; the spider trap squirms in your unrelenting grasp, one set of arms covering his rapidly-wilting erection as the other flails around, trying to get you off of him.  Not happening.' );
		...
		if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] === 0 ) {
			EngineCore.outputText( 'my dad' );
		} else {
			EngineCore.outputText( '[name]' );
		}
		EngineCore.outputText( '. ' + this.championRef() + ', Alex.</i>"' );
		...
		if( CoC.flags[ kFLAGS.HEL_LOVE ] !== 0 ) {
			EngineCore.outputText( '  You don\'t want to be a whore like your mother, do you?  The girl that everyone\'s had a ride with.' );
		}
		//if HelLove:;
		if( CoC.flags[ kFLAGS.HEL_LOVE ] === 1 ) {
			EngineCore.outputText( '  I\'m not going to a raise a whore, ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + '.  Your mother\'s made mistakes, sure, but...</i>" you sigh.  "<i>' );
		}
		EngineCore.outputText( '  You don\'t have to be that person, ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + '.  Don\'t just give yourself out to the first person that asks, you know?  Save it for someone you really care about.  Trust me, it\'ll be worth it.</i>"' );
	...
	HelSpawnScene.prototype.helspawnAllGrownUp = function() {
		...
		EngineCore.outputText( 'like a woman.  The time to raise her, to shape her development, is over.  You can only hope you\'ve done a good enough job to keep her safe in this hellish world; to give her sound judgment and the abilities she\'ll need to protect herself in the years and adventures to come.' );
	...
	HelSpawnScene.prototype.helspawnsMainMenu = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'You call ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' over to you and she comes running, ' );
		if( CoC.player.tallness <= 72 ) {
			EngineCore.outputText( 'grabbing you off your [feet] and clutching you in a tight hug' );
		} else {
			EngineCore.outputText( 'throwing her arms around you and hugging you tightly' );
		}
		EngineCore.outputText( '.  Laughing despite yourself, you ruffle her fiery hair and ask her if she’s got a minute.' );
		EngineCore.outputText( '\n\n"<i>Sure, ' + this.championRef() + '!  What’s up?</i>"' );
		EngineCore.menu();
		//Display Options:;
		//[Hug];
		EngineCore.addButton( 0, 'Hug', this.hugHelspawn );
		//[Talk];
		EngineCore.addButton( 1, 'Talk', this.talkToHelspawn );
		//[Spar];
		EngineCore.addButton( 2, 'Spar', this.sparHelspawn );
		//[Sex] {?};
		//[Appearance];
	...
	HelSpawnScene.prototype.hugHelspawn = function() {
		...
		if( CoC.player.tallness <= 72 ) {
			EngineCore.outputText( 'You reach up and pull ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' down to your level, wrapping your arms tight around her slender shoulders.  Grinning from ear to ear, the towering girl squeezes you right back, her powerful muscles just about crushing your smaller body.  Her tail slips around your [hips], the warm leather binding you to your beloved ' );
			if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] === 0 ) {
				EngineCore.outputText( 'daughter ' );
			} else {
				EngineCore.outputText( 'ward ' );
			}
			EngineCore.outputText( 'as she nuzzles up against you.' );
		...
		//{If PC is 6' or taller:};
			...
			EngineCore.outputText( 'You grab ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' and pull her into a tight hug.  She lets out a girlish giggle as she falls into your arms and squeezes you right back.  Her powerful arms threaten to crush your bigger body, smothering you in affection.  Her tail slips around your [leg], the warm leather binding you to your beloved ' );
			if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] === 0 ) {
				EngineCore.outputText( 'daughter ' );
			} else {
				EngineCore.outputText( 'ward ' );
	...
	HelSpawnScene.prototype.talkToHelspawn = function() {
		...
		if( CoC.flags[ kFLAGS.HAD_FIRST_HELSPAWN_TALK ] === 0 ) {
			...
			//{if Sluttymander:;
			if( CoC.flags[ kFLAGS.HELSPAWN_PERSONALITY ] >= 50 ) {
				EngineCore.outputText( '  Even if she wants to be just like mom, she shouldn\'t be bringing monsters home.  Some of them may be dangerous, and she\'s not a warrior quite like you and Helia yet.' );
			} else {
				EngineCore.outputText( '  She should know better than to bring monsters back.  She\'s not her mother; lust doesn\'t have to dominate her life like that.' );
			}
			...
			if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] === 0 ) {
				EngineCore.outputText( 'Of course, as her father, you could always say it\'s your duty to keep her happy...' );
			} else {
				EngineCore.outputText( 'Of course, as her adoptive father, you could always say that you love her in a different, more intimate way.' );
			}
			EngineCore.outputText( '  She wouldn\'t need monsters with you around.' );
			EngineCore.menu();
			EngineCore.addButton( 0, 'Stop Fucking', this.dontFuckAlex );
			EngineCore.addButton( 1, 'Her Boyfriend', this.helSpawnBoyfriend );
			EngineCore.addButton( 2, 'Incest', this.incestWithHelspawn );
		}
		//Talk 2;
		//{Kiha must be at camp};
			...
			//[Oh God EW] [Um, yum?];
			EngineCore.menu();
			EngineCore.addButton( 0, 'Oh God Ew', this.ohGodEwKihaAndHelspawnSuckAtCooking );
			EngineCore.addButton( 1, 'Um, Yum?', this.umYum );
		}
		//Talk 3;
		//{Needs Rath at camp];
		else if( temp <= 1 && CoC.player.findStatusAffect( StatusAffects.CampRathazul ) >= 0 ) {
			EngineCore.outputText( '"<i>' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + '!</i>" you hear a ragged old voice call, "<i>Get back here this instant!  I\'m not done with you yet!</i>"' );
		...
		//{Bath Slut w/ DD or HHH must be at camp};
			...
			EngineCore.outputText( '"<i>Hey, have you seen ' + CoC.flags[ kFLAGS.MILK_NAME ] + ' around anywhere, ' + this.championRef() + '?</i>" ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' asks, looking around the camp.  "<i>Mom asked me to milk her, but I haven\'t seen her around.  She wouldn\'t have run off, would she?</i>"' );
			...
			EngineCore.menu();
			EngineCore.addButton( 0, 'MilkHerLater', this.helSpawnSureMilkHerLater );
			EngineCore.addButton( 1, 'MilkHerNow', this.helSpawnMilkHerNow );
		}
		//Talk 5;
		//{Isabella must be at camp};
		else if( temp <= 3 && SceneLib.isabellaFollowerScene.isabellaFollower() ) {
			EngineCore.outputText( '"<i>Oh hey, ' + this.championRef() + '!</i>" ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' says, suddenly grinning, "<i>Mom bought me a new guitar in town.  Wanna come listen?</i>"' );
			...
			if( CoC.player.race() !== 'human' ) {
				EngineCore.outputText( ' and you yourself have suffered as well' );
			}
			EngineCore.outputText( '.  You smile as Isabella seats herself beside ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ', joining their voices together for the final chorus that leaves them both shaking as ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' strums the last, desperate notes, nearly clawing through the strings in her passion.' );
			EngineCore.outputText( '"<i>' );
	...
	HelSpawnScene.prototype.dontFuckAlex = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'Stroking ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + '\'s hair, you try to tell her that there\'s a better way, that she doesn\'t have to just fuck everything she comes across.  Her mother\'s that way, sure, but she could do so much better, so much more with herself than giving in to constant lust.' );
		//{If Sluttymander:};
		if( CoC.flags[ kFLAGS.HELSPAWN_PERSONALITY ] >= 50 ) {
	...
	HelSpawnScene.prototype.helSpawnMilkHerNow = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( '"<i>Oh, no.  You\'re not getting out of your chores that easy, kiddo.  In you go,</i>" you say, giving her a little push toward the edge of the pool.' );
		...
		if( CoC.flags[ kFLAGS.MILK_SIZE ] === 2 ) {
			EngineCore.outputText( ', already working to unfasten her shift' );
		}
		EngineCore.outputText( '.  "<i>Suckle?</i>" she asks, turning her full teats toward ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + '.' );
		//If Sluttymander:;
		if( CoC.flags[ kFLAGS.HELSPAWN_PERSONALITY ] >= 50 ) {
	...
	HelSpawnScene.prototype.sparHelspawn = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'You ask ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' if she\'s up for some battle practice, and she answers with an eager nod as she grabs her weapon.' );
		//If Sluttymander: ;
		if( CoC.flags[ kFLAGS.HELSPAWN_PERSONALITY ] >= 50 ) {
	...
	HelSpawnScene.prototype.loseSparringToDaughter = function() {
		//if Sluttymander:;
		if( CoC.flags[ kFLAGS.HELSPAWN_PERSONALITY ] >= 50 ) {
			EngineCore.outputText( 'As you stumble back, ' );
			if( CoC.player.lust > 99 ) {
				EngineCore.outputText( 'succumbing to your own lusts' );
			} else {
				EngineCore.outputText( 'unable to withstand her unending hail of attacks' );
			}
			EngineCore.outputText( ', ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' quickly sweeps your [legs] out from under you, dropping you right on your ass.  You collapse with a grunt, ' );
			if( CoC.player.weaponName !== 'fists' ) {
				EngineCore.outputText( 'weapon tumbling out of hand' );
			} else {
				EngineCore.outputText( 'unable to even clench your fists anymore' );
			}
			EngineCore.outputText( ', and a moment later ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + '\'s on you, straddling you with her powerful legs, pushing you down into the dirt.' );
		...
		//Else If Chastemander:;
			...
			EngineCore.outputText( ', ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' grabs your arm, catching you before you can fall.  "<i>Easy there, ' + this.championRef() + ', I got you,</i>" she says, pulling you into a quick hug.' );
			...
			if( CoC.player.HP < 1 ) {
				EngineCore.outputText( 'recover from that ass kicking' );
			} else {
				EngineCore.outputText( 'dunk your head' );
			}
			EngineCore.outputText( '.' );
		}
	...
	HelSpawnScene.prototype.beatUpYourDaughter = function() {
		...
		if( CoC.flags[ kFLAGS.HELSPAWN_PERSONALITY ] >= 50 && CoC.monster.lust > 99 ) {
			EngineCore.outputText( '"<i>N-no more...</i>" the slutty little salamander moans, slumping down to the ground, arms wrapping around herself.  "<i>Fuck, you\'re sexy... so horny...</i>" she groans, hands slipping down to her soaked bikini bottom.' );
			...
			if( CoC.flags[ kFLAGS.HELSPAWN_INCEST ] === 0 ) {
				EngineCore.outputText( '  You wash your hands of the defeated slut and head back to camp, leaving her to work through her tension herself.' );
			} else {
				EngineCore.outputText( '  You reach down and give your lovely, lusty daughter a pat on her expansive rear, telling her she\'ll always look her best with her ass in the air, begging for ' );
				if( !CoC.player.hasCock() ) {
					EngineCore.outputText( 'sex' );
				} else {
					EngineCore.outputText( 'your cock' );
				}
				EngineCore.outputText( '... and that if she\'s lucky, you might tend to her when you\'ve cooled off.' );
			}
		} else {
			EngineCore.outputText( CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' stumbles back, dropping her weapon and waving her arms, "<i>I give, I give.</i>"' );
			...
			if( CoC.monster.HP < 1 ) {
				EngineCore.outputText( ', rubbing her many bruises' );
			} else {
				EngineCore.outputText( ', blushing brightly after your slutty display' );
			}
			EngineCore.outputText( '.  You chuckle and ruffle her hair, "<i>C\'mon, kiddo, let\'s get some food in you.</i>"' );
	...
	HelSpawnScene.prototype.maiVisitsHerKids = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 68 );
		EngineCore.outputText( 'As you head back into camp, you notice Hel and ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' are sitting around the cook fire in the center of camp, with your foster daughter balanced precariously on the lap of her own father, Mai the fox-girl.' );
	...
	HelSpawnScene.prototype.spiderBrosGift = function() {
		...
		EngineCore.outputText( 'As you\'re getting ready to head out for the day, you notice ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' prancing around camp with a long, brightly-colored scarf wrapped around her neck, standing painfully in contrast with her midnight-black scales and pale flesh.' );
	...
	HelSpawnScene.prototype.hakonAndKiriComeVisit = function() {
		...
		if( CoC.flags[ kFLAGS.HELSPAWN_WEAPON ] === 'scimitar and shield' ) {
			EngineCore.outputText( this.championRef() + '\'s been teaching me how to fight.' );
		} else {
			EngineCore.outputText( 'Mom\'s been teaching me how to fight.' );
		}
		EngineCore.outputText( '  I\'m not as good as mom and ' + this.championRef() + ' yet, but I\'d love to come along.</i>"' );
		EngineCore.outputText( '\n\n"<i>That\'s the spirit.  Now we just have to convince your aunt Kiri to come along.  And ' );
		if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] === 0 ) {
			EngineCore.outputText( 'your old man' );
		} else {
			EngineCore.outputText( '[name]' );
		}
		EngineCore.outputText( ', too.</i>"' );
		...
		EngineCore.menu();
		EngineCore.addButton( 0, 'Sure', this.goHuntingBitches );
		EngineCore.addButton( 1, 'Maybe Not', this.noHuntingBitches );
	};
	//Sure!;
	...
	HelSpawnScene.prototype.goHuntingBitches = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 68 );
		EngineCore.outputText( '"<i>Sure,</i>" you say, quickly earning approving nods from Hakon and Helia.' );
