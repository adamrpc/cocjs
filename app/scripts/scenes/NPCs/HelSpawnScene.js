...
angular.module( 'cocjs' ).run( function( MainView, Helspawn, PregnancyStore, Combat, Descriptors, AppearanceDefs, EventParser, CoC, kFLAGS, Utils, StatusAffects, EngineCore ) {
	...
	HelSpawnScene.prototype.championRef = function() {
		if( CoC.getInstance().flags[ kFLAGS.HELSPAWN_DADDY ] === 0 ) {
			return CoC.getInstance().player.mf( 'Daddy', 'Mom' );
		} else {
	...
	HelSpawnScene.prototype.heliasAppearanceScreen = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.HEL_BONUS_POINTS ] < 150 ) {
		...
		//if Isabella is cool: ;
		if( CoC.getInstance().flags[ kFLAGS.HEL_ISABELLA_THREESOME_ENABLED ] >= 1 ) {
	...
	HelSpawnScene.prototype.helSpawnsAppearanceScreen = function() {
		...
		EngineCore.outputText( 'salamander daughter, ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ', stands about six-foot-six tall, with pale skin and thick, ' );
		if( CoC.getInstance().flags[ kFLAGS.HELSPAWN_DADDY ] === 1 ) {
			EngineCore.outputText( 'chitin-black' );
		} else {
			EngineCore.outputText( 'bright-red' );
		}
		EngineCore.outputText( ' scales covering her arms and legs, though she has a normal human face and torso.  A long leathery tail swishes happily behind her, surrounded by a dim fiery aura that emits a pleasant heat, though thankfully it isn’t hot to the touch, seemingly incapable of harming you.  ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' is wearing ' );
		...
		} else if( CoC.getInstance().flags[ kFLAGS.HELSPAWN_DADDY ] === 2 ) {
		...
		if( CoC.getInstance().flags[ kFLAGS.HELSPAWN_DADDY ] === 2 ) {
		...
		EngineCore.outputText( '\n\n' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' has a pair of perky, palmable C-cup breasts, each with a sensitive half-inch nipple at its tip.' );
	...
	HelSpawnScene.prototype.heliaBonusPointsAward = function() {
		CoC.getInstance().flags[ kFLAGS.HELIA_KIDS_CHAT ] = 1;
		EngineCore.spriteSelect( 68 );
		EngineCore.outputText( '\n<b>That night...</b>\n' );
	...
	HelSpawnScene.prototype.noYouDontLoveHeliaYouMonster = function() {
		...
		//[HelLove to NO];
		CoC.getInstance().flags[ kFLAGS.HEL_LOVE ] = -1;
	...
	HelSpawnScene.prototype.heliaLoveFollowup = function() {
		...
		//If HelLove = No:;
		if( CoC.getInstance().flags[ kFLAGS.HEL_LOVE ] === -1 ) {
	...
	HelSpawnScene.prototype.haveAKid = function() {
		...
		EngineCore.outputText( ' before coiling around the shaft with serpentine dexterity.  Her tail contracts and squeezes, undulating across your ' + Descriptors.cockDescript( 0 ) + ' until it finally massages a thick blob of pre from your ' + CoC.getInstance().player.cockHead() + ', dribbling out to stain the wasteland ground.' );
		EngineCore.outputText( '\n\nSuddenly, Helia grabs your shoulders and shoves you down.  You flop onto your back, tail-encased prick standing straight up like a flagpole as Hel straddles your [hips], vigorously fingering herself as her tail lines your cock up with her drooling womanhood.  Your breath catches as your ' + CoC.getInstance().player.cockHead() + ' brushes her boiling juices, slickening your entry as she slides down your pole.  You grab Hel\'s flared hips, guiding her in as she envelopes your ' + Descriptors.cockDescript( 0 ) + ' in the roiling inferno inside her.  Her tail unravels from around your throbbing shaft as she descends, tantalizingly slowly; she takes the better part of a minute before her groin is cozily joined to yours, her hungry maw kissing the very base of your lusty prick.' );
	...
	HelSpawnScene.prototype.helGotKnockedUp = function() {
		EngineCore.spriteSelect( 68 );
		CoC.getInstance().flags[ kFLAGS.HEL_NTR_TRACKER ] = 2;
		...
		else if( CoC.getInstance().flags[ kFLAGS.HELSPAWN_DADDY ] === 1 ) {
	...
	HelSpawnScene.prototype.sureHelGimmeSpidahBoyDetails = function() {
		...
		if( CoC.getInstance().player.biggestCockArea() >= 20 ) {
	...
	HelSpawnScene.prototype.dontTellMeAboutMai = function() {
		...
		EngineCore.outputText( '\n\nYou put an arm around the mother-to-be\'s shoulder, saying, "<i>I thought this was what you wanted, Hel.  You were-</i>"' );
		EngineCore.outputText( '\n\n"<i>Yeah, I know.  Forget about it, I\'m fine.  Really.  It\'s just that this happened all so suddenly, you know?  This isn\'t exactly where I saw myself a year ago.  Not that I\'m complaining,</i>" she adds, turning back to you with a kiss, squeezing you with her tail.  "<i>I just hope I\'m ready, I guess.  Until the Pale Flame took, I\'d never really thought about being a mother.  Never really wanted it.  But last night, it was like I was possessed, like I couldn\'t control myself.  I just... needed you.  Well, maybe not your sperm - not literally, I mean - but it was like my body screaming \'You\'ve found ' + CoC.getInstance().player.mf( 'him', 'her' ) + ', ' + CoC.getInstance().player.mf( 'he', 'she' ) + '\'s the one, this is a keeper!  Time to propagate!\'  I-I can\'t help but feel like this wasn\'t entirely my choice, you know? Like I was acting on instinct, like an animal, not a person.  But at the same time, if I\'ve made a mistake, then I have you here with me, to help me like you always do.  I love you with all my heart, [name]' );
		//if HelLove is Yes:;
		if( CoC.getInstance().flags[ kFLAGS.HEL_LOVE ] === 1 ) {
			EngineCore.outputText( ', and I\'m so very, very glad you love me back.' );
		} else {
	...
	//if(CoC.getInstance().flags[kFLAGS.HELIA_PREGNANCY_INCUBATION] < 300 && CoC.getInstance().flags[kFLAGS.HEL_PREGNANCY_NOTICES] === 0) {;
	...
	//if(CoC.getInstance().flags[kFLAGS.HELIA_PREGNANCY_INCUBATION] === 200 && CoC.getInstance().flags[kFLAGS.HEL_PREGNANCY_NOTICES] === 1);
	...
	//if(CoC.getInstance().flags[kFLAGS.HELIA_PREGNANCY_INCUBATION] === 100);
	...
	HelSpawnScene.prototype.heliaTalkSeven = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 68 );
		CoC.getInstance().flags[ kFLAGS.HELIA_TALK_SEVEN ] = 1;
	...
	HelSpawnScene.prototype.applyHelspawnName = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] === 'Mai' ) {
			EngineCore.outputText( '"<i>Aw, that\'s great, lover.  Mai\'ll be honored, I know it.  And it is a great name...</i>"\n\n' );
		}
		if( CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] === 'Tanis' ) {
		...
		//Syn: ;
		if( CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] === 'Syn' ) {
			EngineCore.outputText( '"Awesome.  I was hoping you\'d choose that one, lover mine.</i>"\n\n' );
		}
		if( CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] === 'Chara' ) {
			EngineCore.outputText( '"<i>Awesome.  You liked my ideas, after all!</i>"\n\n' );
		}

		EngineCore.outputText( '"<i>So... ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' it is.  That\'s a good name, my love.  A strong name.  ' );
		if( CoC.getInstance().flags[ kFLAGS.HELSPAWN_DADDY ] === 0 ) {
	...
	HelSpawnScene.prototype.heliaTalkEight = function() {
		EngineCore.clearOutput();
		CoC.getInstance().flags[ kFLAGS.HEL_TALK_EIGHT ] = 1;
		EngineCore.spriteSelect( 68 );
	...
	HelSpawnScene.prototype.playWithYourKid = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 68 );
		EngineCore.outputText( 'Picking ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' up off the ground, you spin the giggling little girl around, laughing as your ' );
		if( CoC.getInstance().flags[ kFLAGS.HELSPAWN_DADDY ] !== 0 ) {
			EngineCore.outputText( 'ward ' );
		} else {
			EngineCore.outputText( 'daughter ' );
		}
		EngineCore.outputText( 'cries out in surprise and glee.  Soon, you\'re bouncing her, then down on the ground crawling around with her, playing peek-a-boo and a dozen other games as you and ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' laugh and hug.  Helia herself slips down with you before you know it, holding her daughter close as she tries to communicate, teaching the ever-growing girl a few words at a time.' );
	...
	HelSpawnScene.prototype.helSpawnGraduation = function() {
		...
		EngineCore.spriteSelect( 68 );
		CoC.getInstance().flags[ kFLAGS.HELSPAWN_GROWUP_COUNTER ] = 1;
		CoC.getInstance().flags[ kFLAGS.HELSPAWN_AGE ]++;
		EngineCore.outputText( 'You wake up to a scream, high-pitched and terrified.  You bolt up out of bed, tangling up in your bedroll for a minute before stumbling over to Hel and... ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + '?' );
		...
		EngineCore.outputText( '\n\n"<i>Ugh, what\'s wrong?</i>" ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' groans, looking up at Hel, then to you.  "<i>' + this.championRef() + ', why\'s mom screaming?</i>"' );
		...
		EngineCore.outputText( '\n\nYou have to admit, the change is sudden and drastic... but the girl\'s scales, her eyes, they\'re certainly ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + '\'s.  ' );
		if( CoC.getInstance().flags[ kFLAGS.HELSPAWN_DADDY ] === 2 ) {
		...
		EngineCore.outputText( '\n\n"<i>It\'s just me, mom,</i>" ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' says, sliding over to sit beside her mother, who\'s still wide-eyed with shock.  Your girl tries for a few moments to talk to Hel, but seeing that venture prove fruitless - Hel seems utterly insensate, paralyzed by shock - she instead curls back up beside her mother, head rested on Helia\'s shoulder.  Eventually, warily, Hel reaches out a trembling hand to brush her daughter\'s cheek.' );
	...
	HelSpawnScene.prototype.helspawnDiscoversBooze = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 68 );
		CoC.getInstance().flags[ kFLAGS.HELSPAWN_DISCOVER_BOOZE ] = 1;
		...
		if( CoC.getInstance().player.race() !== 'cat-boy' && CoC.getInstance().player.race() !== 'cat-girl' ) {
		...
		EngineCore.outputText( '\n\nFlopped down on the ground with her mouth right under the spigot is ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ', using her tail to keep the tap open and flowing into her waiting maw as she lazes beneath it.  When you loudly clear your throat, she flails around a moment, letting the tap go as she scrambles to her feet - only to fall drunkenly on her ass.  "<i>Uh...</i>" she groans, wiping the booze off her cheeks as she hiccups drunkenly.  "<i>Hi there, ' + this.championRef() + '.</i>"' );
		EngineCore.menu();
	...
	HelSpawnScene.prototype.encourageHelspawn = function() {
		...
		EngineCore.outputText( '\n\n"<i>Wha...?</i>" ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' starts to say, before catching on as you drink, inviting her to join you.' );
		...
		EngineCore.outputText( '\n\n"<i>Uh...</i>" you groan, slumping over onto your [ass].  ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + '\'s right behind you, collapsing on top of you in a pile of ' );
		if( CoC.getInstance().flags[ kFLAGS.HELSPAWN_DADDY ] !== 1 ) {
			EngineCore.outputText( 'red ' );
		} else {
			EngineCore.outputText( 'black ' );
		...
		EngineCore.outputText( '\n\n"<i>I saved some for ya, mom,</i>" ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' groans, retching.' );
	...
	HelSpawnScene.prototype.snipermanders = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 68 );
		CoC.getInstance().flags[ kFLAGS.HELSPAWN_WEAPON ] = 'bow';
		...
		EngineCore.outputText( '\n\nYou slip behind ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ', putting your arms on hers as you guide her through nocking an arrow, head resting on her fist, a lone finger outstretched toward the dessicated dummy.' );
		...
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.CampRathazul ) >= 0 ) {
			EngineCore.outputText( '  You hear your old alchemist friend suddenly putting up a ruckus as the sounds of breaking glass echo throughout camp.  ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' looks up at you nervously, but you ruffle her fiery hair and tell her to try again. Rath is probably just fine.' );
		...
		EngineCore.outputText( '\n\nYou sigh and tell her to try again.  She can\'t expect to be perfect on the first try, after all.  It takes a little convincing, but soon you\'ve got ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' back into form and nocking an arrow.  You put your hands on hers, helping the young salamander take aim, drawing down on her straw-stuffed target.  Barely audible over ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + '\'s nervous breathing, you whisper, "<i>Take it easy.  Relax.... Good.  Take aim... now breathe out.  Exhale and loose.</i>"' );
		EngineCore.outputText( '\n\nAgain, ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' lets an arrow fly - and this one strikes true.  You watch the missile streak across camp, slamming into the training dummy\'s crotch with deadly force.  It sways, buckles, and falls to pieces.' );
		EngineCore.outputText( '\n\n"<i>Ouch,</i>" Hel laughs, sipping on her flask behind you.  "<i>What\'d that poor dummy ever do to you?</i>"' );
		EngineCore.outputText( '\n\n"<i>I did it!</i>" ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' shouts, pumping her fist in the air. Her mother reaches over to ruffle her fiery hair, and you both shower her with congratulations.' );
		EngineCore.outputText( '\n\nBefore she can get cocky, though, you push the bow back into her hands.  "<i>Lesson 2, kiddo: one shot is never enough.</i>"' );
		EngineCore.outputText( '\n\nBy the time ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' has her shooting stance back, her mother\'s already set the dummy back up, ready for round two.  It\'s several hours long before the three of you quit spent drilling ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' on stance and aim, even going a round with her and a pair of daggers to remind her that enemies will eventually close in if she\'s not careful.  She learns quickly, though, and soon she hardly needs your help to plant an arrow in the target\'s chest.' );
	...
	HelSpawnScene.prototype.swordAndBoardmander = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 68 );
		CoC.getInstance().flags[ kFLAGS.HELSPAWN_WEAPON ] = 'scimitar and shield';
		...
		EngineCore.outputText( '\n\n' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' gives the blade a few practice swings before grabbing the wooden blade in both hands and charging at you.  You put your guard up, catching her first overhand blow and turning it aside, nearly toppling the girl over.  She rolls with it, spinning out of your guard and trying a strike from the side.  You parry it, catch an opening, and give her a nice wallop on the shoulder.  A real blade would have nearly cut her arm off, but ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' merely stumbles back, yelping in pain.' );
		...
		EngineCore.outputText( '\n\nYou sigh.  "<i>Again.</i>"' );
		EngineCore.outputText( '\n\n' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' picks herself up and grips her blade.  This time she\'s a little more cautious, approaching slowly, pausing just out of reach.  You hide a grin and bring your own sword up, the tips of your blades nearly touching.  "<i>Good.  No need to be reckless, kiddo.  Don\'t just charge in at the first opportunity; you\'ll get yourself killed.</i>"' );
		EngineCore.outputText( '\n\n"<i>Get ' + CoC.getInstance().player.mf( 'him', 'her' ) + ', ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + '!</i>" Hel calls from the sidelines, sipping on a mug.  "<i>Show ' + CoC.getInstance().player.mf( 'him', 'her' ) + ' what a salamander can do!</i>"' );
		if( CoC.getInstance().flags[ kFLAGS.HELSPAWN_DADDY ] === 0 ) {
			EngineCore.outputText( '\n\nYour' );
		} else {
			EngineCore.outputText( '\n\nHer' );
		}
		EngineCore.outputText( ' daughter braces herself, drawing a breath just a second before she launches her attack, giving herself away.  You parry three quick blows, stepping back as ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' leaps into another overhand strike.  You dodge, spinning out of the attack and slicing her right across her bare belly.' );
		EngineCore.outputText( '\n\n"<i>Gah!</i>" she cries, stumbling to her knees.  "<i>I-I can\'t...</i>"' );
		EngineCore.outputText( '\n\nYou pull ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' to her feet and ruffle her hair.  "<i>C\'mon kiddo, don\'t give up.  Here,</i>" you say, grabbing a dinner plate and some leather straps from your kit.  In a moment, you\'ve got a rudimentary shield cobbled together.  You fit it over the young salamander\'s free arm and give her a few pointers on stance.  Nervously, she brings her makeshift shield to bear against you, fidgeting under the heft of it as you make a few slow, easy to parry swings for her to get used to it.  Steadily you pick up the pace, swinging your training sword faster and harder, forcing ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' to parry and block, retreating against your onslaught.  You keep it up for a full minute before relenting, letting the kid seize the initiative.  She swings at you, rolling with each parry you make, cleaving into another swing, turning your ripostes aside with her shield and forcing you back.' );
		EngineCore.outputText( '\n\nYou wince in pain as ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' manages to nick you, turning one of your blows aside with her shield and striking through the opening left in your guard.  "<i>Alright, alright,</i>" you laugh, rubbing your shoulder.  "<i>Good job, ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + '. You\'re getting the hang of that shield!</i>"' );
		EngineCore.outputText( '\n\n"<i>Thanks, ' + this.championRef() + '!</i>" the little salamander grins, hugging you.' );
		EngineCore.outputText( '\n\n"<i>Alright, alright,</i>" Hel says, grabbing the dummy sword from your hand.  "<i>Now it\'s time to fight a REAL warrior, ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + '. Come get some!</i>"' );
		EngineCore.outputText( '\n\nIt\'s several hours long before the three of you quit spent drilling ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' on stance and parrying, trading off with Helia every few rounds to keep ' );
		if( CoC.getInstance().flags[ kFLAGS.HELSPAWN_DADDY ] !== 0 ) {
			EngineCore.outputText( 'her' );
		} else {
			EngineCore.outputText( 'your' );
	...
	HelSpawnScene.prototype.dasBarbarimander = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 68 );
		CoC.getInstance().flags[ kFLAGS.HELSPAWN_WEAPON ] = 'scimitar';
		if( CoC.getInstance().player.gems >= 200 ) {
			EngineCore.outputText( 'You sigh and dig out a handful of gems.  Helia beams at you, planting a quick kiss on your cheek before grabbing ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' and the loot.  "<i>Thanks for the loan, lover mine.  C\'mon, sweetie, we\'re going for a trip to town.</i>"' );
	...
	HelSpawnScene.prototype.helspawnIsASlut = function() {
		EngineCore.spriteSelect( 68 );
		CoC.getInstance().flags[ kFLAGS.HELSPAWN_FUCK_INTERRUPTUS ] = 1;
		...
		EngineCore.outputText( 'is doing.  Well, it doesn\'t take a genius to figure that out, really, but still, you have to be sure.  You slide right up to them, just out of their lines of sight, as ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' turns around, letting the spider-kin unclasp her bra and grab her tits, squeezing the small mounds as ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' suppresses a moan, reaching back to slip a hand into the spider\'s crotch, grabbing her - his dick.' );
		EngineCore.outputText( '\n\nWell then. You suppose you ought to go stop ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' from making a whore of herself... but then again, she\'s old enough to make her own mistakes by now.' );
		if( CoC.getInstance().player.cor >= 50 ) {
			EngineCore.outputText( '  And by mistake you clearly mean old enough to get her ass fucked in by a cute little spider trap... you contemplate jumping in on it, but with a sigh figure it\'s ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + '\'s catch.  Let her enjoy it.' );
		}
		EngineCore.menu();
	...
	HelSpawnScene.prototype.helspawnDoNothing = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 68 );
		EngineCore.outputText( 'You turn around and head back to your bed.  As soon as you\'re under your blanket, your ears are assaulted with the quiet moans and grunts of pleasure coming from ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + '\'s bed as she and her first little boyfriend get it on.  You can practically hear the tail-pegging from here.  How cute!' );
		//{HelspawnSlutty +10};
		CoC.getInstance().flags[ kFLAGS.HELSPAWN_PERSONALITY ] += 10;
		EngineCore.doNext( EventParser.playerMenu );
	};
	...
	HelSpawnScene.prototype.helSpawnStopFucking = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 68 );
		EngineCore.outputText( 'You sigh and step out of the shadows, grabbing the feminine little spider boy by the shoulder and yanking him away from ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + '.  "<i>And just what the hell do you two think you\'re doing?</i>" you yell, scowling ' );
		if( CoC.getInstance().player.tallness >= 90 ) {
			EngineCore.outputText( 'up ' );
		} else if( CoC.getInstance().player.tallness <= 72 ) {
			EngineCore.outputText( 'down ' );
		...
		EngineCore.outputText( '. ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' gives a shrill cry of surprise and tumbles away from you, trying to cover herself as best she can; the spider trap squirms in your unrelenting grasp, one set of arms covering his rapidly-wilting erection as the other flails around, trying to get you off of him.  Not happening.' );
		EngineCore.outputText( '\n\n"<i>You!</i>" you say, pointing an accusing finger at ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + '.  "<i>What the hell, kid?  Who the fuck is this?</i>"' );
		...
		EngineCore.outputText( '\n\n"<i>B-but I...</i>"' );
		EngineCore.outputText( '\n\n"<i>' + this.championRef() + '...</i>" ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' groans as you grab the spider boy and toss him out of camp, watching with hawk-like eyes as he slinks back to his jungle.  When he\'s gone, you sigh and walk over to ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ', wiping a tear from her cheek.  Putting an arm around her shoulder, you give her a long, long talk about not being a whore.' );
		EngineCore.outputText( '\n\n"<i>B-but mom...</i>"' );
		EngineCore.outputText( '\n\n"<i>That\'s why you </i>shouldn\'t<i> be bringing men home.  Or women.  Or anything else, kiddo.' );
		//If No Hel Love:;
		if( CoC.getInstance().flags[ kFLAGS.HEL_LOVE ] !== 0 ) {
			EngineCore.outputText( '  You don\'t want to be a whore like your mother, do you?  The girl that everyone\'s had a ride with.' );
		}
		//if HelLove:;
		if( CoC.getInstance().flags[ kFLAGS.HEL_LOVE ] === 1 ) {
			EngineCore.outputText( '  I\'m not going to a raise a whore, ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + '.  Your mother\'s made mistakes, sure, but...</i>" you sigh.  "<i>' );
		}
		EngineCore.outputText( '  You don\'t have to be that person, ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + '.  Don\'t just give yourself out to the first person that asks, you know?  Save it for someone you really care about.  Trust me, it\'ll be worth it.</i>"' );
	...
	HelSpawnScene.prototype.helspawnsMainMenu = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'You call ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' over to you and she comes running, ' );
	...
	HelSpawnScene.prototype.hugHelspawn = function() {
		...
		if( CoC.getInstance().player.tallness <= 72 ) {
			EngineCore.outputText( 'You reach up and pull ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' down to your level, wrapping your arms tight around her slender shoulders.  Grinning from ear to ear, the towering girl squeezes you right back, her powerful muscles just about crushing your smaller body.  Her tail slips around your [hips], the warm leather binding you to your beloved ' );
			if( CoC.getInstance().flags[ kFLAGS.HELSPAWN_DADDY ] === 0 ) {
				EngineCore.outputText( 'daughter ' );
			} else {
				EngineCore.outputText( 'ward ' );
		...
		//{If PC is 6' or taller:};
			...
			EngineCore.outputText( 'You grab ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' and pull her into a tight hug.  She lets out a girlish giggle as she falls into your arms and squeezes you right back.  Her powerful arms threaten to crush your bigger body, smothering you in affection.  Her tail slips around your [leg], the warm leather binding you to your beloved ' );
			if( CoC.getInstance().flags[ kFLAGS.HELSPAWN_DADDY ] === 0 ) {
				EngineCore.outputText( 'daughter ' );
			} else {
				EngineCore.outputText( 'ward ' );
	...
	HelSpawnScene.prototype.talkToHelspawn = function() {
		...
		//Talk 3;
		//{Needs Rath at camp];
		else if( temp <= 1 && CoC.getInstance().player.findStatusAffect( StatusAffects.CampRathazul ) >= 0 ) {
			EngineCore.outputText( '"<i>' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + '!</i>" you hear a ragged old voice call, "<i>Get back here this instant!  I\'m not done with you yet!</i>"' );
		...
		//{Bath Slut w/ DD or HHH must be at camp};
			...
			if( CoC.getInstance().flags[ kFLAGS.MILK_SIZE ] === 1 ) {
				...
				EngineCore.outputText( '\n\nYou doubt she would have; ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' seems to have a fancy for you, but' );
			}
			EngineCore.outputText( ' it wouldn\'t hurt to check around.  She does tend to wander into the traps around camp from time to time.  Offering ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' your hand, you start searching the camp for your missing milk maid.  The two of you make a circuit around camp, checking the traps and behind various rocks and hollows, to no avail.  Finally, as you\'re starting to get worried about the dusky maid, you happen to look into the steel pool in her part of camp.  Sure enough, you spy ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' curled up at the bottom of the pool, cuddled up with a ragged blanket to catch a quick nap.' );
			EngineCore.outputText( '\n\n"<i>Aww,</i>" ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' grins, looking down at the napping ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '.  "<i>Guess I can milk her later, then.</i>"' );
			EngineCore.menu();
	...
	HelSpawnScene.prototype.dontFuckAlex = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'Stroking ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + '\'s hair, you try to tell her that there\'s a better way, that she doesn\'t have to just fuck everything she comes across.  Her mother\'s that way, sure, but she could do so much better, so much more with herself than giving in to constant lust.' );
		//{If Sluttymander:};
		if( CoC.getInstance().flags[ kFLAGS.HELSPAWN_PERSONALITY ] >= 50 ) {
			EngineCore.outputText( '\n\n' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' scoffs, shaking her head.  "<i>But I </i>like<i> mom\'s way.  I love jilling off, the feeling of my cunt getting stretched and my ass being torn apart by the big toys I buy in town.  That\'s who I am, ' + this.championRef() + '.  I\'m like mom, and I don\'t think... no, I know I don\'t want to change.  I love sex, even if you stopped me and Alex from it.  I want to feel a gang of minotaurs raping me, I want to jump on a drider\'s cock and ride him down.  I want to do the things I heard mom doing, and that\'s just what I\'m going to do. I\'m sorry.</i>"' );
			EngineCore.outputText( '\n\nYou try to say something, but ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' steps away, calling back, "<i>I love you, ' + this.championRef() + ', but I can\'t be what you want me to be.  I\'m going to be a slut like mom, and there\'s nothing you can do to stop me!  It\'s who I was born and raised to be.</i>"' );
	...
	HelSpawnScene.prototype.incestWithHelspawn = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'You cup ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + '\'s cheek, turning her to you... only to have her leap you, straddling your [legs] as her powerful arms wrap around your neck.  Her thin lips press hard to yours, slender tongue probing against yours as her svelte body presses against you.  You respond in kind, grabbing her big ass and squeezing, kneading the delicious curves as she starts to grind against you, breath hot and heavy on your ' + CoC.getInstance().player.skinFurScales() + '.  "<i>I said I love you, ' + this.championRef() + ',</i>" she grins, so close you can practically feel the beat of her heart through her perky breasts.  "<i>Glad to know you feel the same way.</i>"' );
	...
	HelSpawnScene.prototype.ohGodEwKihaAndHelspawnSuckAtCooking = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'You spew the stew onto the ground and grab a nearby waterskin, trying to flush the taste from your mouth.  Kiha and ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' look on with horror as you wipe your mouth and begin to try and coherently explain just how god awful whatever that... STUFF... you just put in your mouth was.  Shock turns to anger before you’re halfway through admonishing the pair of scaly redheads.  Kiha scowls at you and snatches the ladle from you.' );
		EngineCore.outputText( '\n\n"<i>Oh, what the fuck do you know anyway, you big ass!  It\'s perfectly fine, isn\'t it ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + '?</i>" she growls, spooning up a mouthful of the stuff before you can stop her.  A heartbeat after she swallows, Kiha goes completely stiff; her eyes growing as wide as saucers as they water.  She collapses backwards, falling onto her back with a muted <i>THUD</i>.' );
		EngineCore.outputText( '\n\n"<i>K-Kiha?</i>" ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' yelps, watching the dragoness collapse into a pile of limp scales and leather.  "<i>Are you okay?  Is she okay, ' + this.championRef() + '?</i>"' );
		...
		EngineCore.outputText( '\n\nYou and ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' are soon working away, stirring up your own special blend of your rations and the spices she and Kiha gathered for the first attempt.  Once you\'ve got the basics in, you step back and hand the ladle over, telling the young \'mander to go wild.  Grinning like a kid, she goes to work, mixing in a wild combination of ingredients.  The cauldron\'s full to bursting by the time she\'s done, and no sooner does she ladle out the first bowel of it than Kiha sits up, groaning.' );
		EngineCore.outputText( '\n\n"<i>The fuck happened?</i>" the dragoness hisses, rubbing her temple.  "<i>Feel like I got hit with a hammer.</i>"' );
		EngineCore.outputText( '\n\n"<i>You, uh, fell off the edge, aunt Kiha.  And spilled all the stew.  Here, we remade it for you!</i>" ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' lies, presenting "aunt Kiha" with a bowl full of an aromatic brown goop, full of carrots and spices floating atop it like boats on a placid lake.  At least it smells better.' );
	...
	HelSpawnScene.prototype.helSpawnMilkHerNow = function() {
		...
		EngineCore.outputText( '\n\n' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' gives a rebellious huff as she clambers down, slipping down to the bottom and giving ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' a gentle nudge.  "<i>C\'mon, cutey, wake up.  Time to relieve some pressure.</i>"' );
		EngineCore.outputText( '\n\nYawning powerfully, ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' rises to her knees and rubs the sleep from her eyes' );
		if( CoC.getInstance().flags[ kFLAGS.MILK_SIZE ] === 2 ) {
			EngineCore.outputText( ', already working to unfasten her shift' );
		}
		EngineCore.outputText( '.  "<i>Suckle?</i>" she asks, turning her full teats toward ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + '.' );
		//If Sluttymander:;
		if( CoC.getInstance().flags[ kFLAGS.HELSPAWN_PERSONALITY ] >= 50 ) {
			EngineCore.outputText( '\n\n"<i>Wouldn\'t have it any other way!</i>" the salamander answers with an eager grin, easing herself into ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + '\'s arms and wrapping her lips around one of her prominent teats. ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' gives a pleasured little shudder as ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' starts to suckle like a babe, drinking down the dusky maid\'s seemingly endless supply of milk.' );
		}

		//If Chastemander:;
		else {
			EngineCore.outputText( '\n\n"<i>Sorry, ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ', you know I\'m not really into that.  Just turn around and sit back, alright?</i>"  With a sigh, ' + CoC.getInstance().flags[ kFLAGS.MILK_NAME ] + ' does as she\'s asked, leaning back into ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + '\'s arms as the young salamander goes to work, clawed fingers gently caressing her prominent nubs until a white flow springs fort, splashing her knees as the dusky maid gives a little gasp of pleasure.' );
		}
	...
	HelSpawnScene.prototype.sparHelspawn = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'You ask ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' if she\'s up for some battle practice, and she answers with an eager nod as she grabs her weapon.' );
		//If Sluttymander: ;
		if( CoC.getInstance().flags[ kFLAGS.HELSPAWN_PERSONALITY ] >= 50 ) {
			EngineCore.outputText( '\n\n"<i>Ready to get your shit kicked in, old ' + CoC.getInstance().player.mf( 'man', 'lady' ) + '?</i>" she grins, drawing her weapon.' );
		} else {
	...
	HelSpawnScene.prototype.loseSparringToDaughter = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.HELSPAWN_PERSONALITY ] >= 50 ) {
			...
			EngineCore.outputText( ', ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' quickly sweeps your [legs] out from under you, dropping you right on your ass.  You collapse with a grunt, ' );
			if( CoC.getInstance().player.weaponName !== 'fists' ) {
				EngineCore.outputText( 'weapon tumbling out of hand' );
			} else {
				EngineCore.outputText( 'unable to even clench your fists anymore' );
			}
			EngineCore.outputText( ', and a moment later ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + '\'s on you, straddling you with her powerful legs, pushing you down into the dirt.' );
		...
		//Else If Chastemander:;
			...
			EngineCore.outputText( ', ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' grabs your arm, catching you before you can fall.  "<i>Easy there, ' + this.championRef() + ', I got you,</i>" she says, pulling you into a quick hug.' );
	...
	HelSpawnScene.prototype.beatUpYourDaughter = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.HELSPAWN_PERSONALITY ] >= 50 && CoC.getInstance().monster.lust > 99 ) {
			...
			//If No Incest: ;
			if( CoC.getInstance().flags[ kFLAGS.HELSPAWN_INCEST ] === 0 ) {
			...
			EngineCore.outputText( CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' stumbles back, dropping her weapon and waving her arms, "<i>I give, I give.</i>"' );
	...
	HelSpawnScene.prototype.maiVisitsHerKids = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 68 );
		EngineCore.outputText( 'As you head back into camp, you notice Hel and ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' are sitting around the cook fire in the center of camp, with your foster daughter balanced precariously on the lap of her own father, Mai the fox-girl.' );
		...
		EngineCore.outputText( '\n\n"<i>Hi, [name],</i>" Mai says with a demure smile, arms wrapped around ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + '\'s belly, trying to hold the daughter that\'s near a foot taller than she is.  Finally, laughing, she lets the young salamander go.  ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' drops down to sit beside her mother and father, and quickly has both a smooth and scaly hand stroking her hair.' );
		//If Chastemander:;
		if( CoC.getInstance().flags[ kFLAGS.HELSPAWN_PERSONALITY ] < 50 ) {
			EngineCore.outputText( '\n\n' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' happily leans her head against Mai\'s leg, accepting the affection with her tail swishing gaily behind her, foxy ears twitching as Mai brushes them.' );
		} else {
			EngineCore.outputText( '\n\n' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' leans back, pulling out a flask and taking a deep swill before passing it to her father, who takes it with a knowing smile and knocks it back like a champ.' );
		}
		...
		EngineCore.outputText( '\n\n"<i>Heeeeeyyyyyy,</i>" ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' whines, crossing her arms with a huff.' );
	...
	HelSpawnScene.prototype.spiderBrosGift = function() {
		EngineCore.clearOutput();
		EngineCore.spriteSelect( 68 );
		CoC.getInstance().flags[ kFLAGS.SPIDER_BRO_GIFT ] = 1;
		EngineCore.outputText( 'As you\'re getting ready to head out for the day, you notice ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' prancing around camp with a long, brightly-colored scarf wrapped around her neck, standing painfully in contrast with her midnight-black scales and pale flesh.' );
	...
	HelSpawnScene.prototype.hakonAndKiriComeVisit = function() {
		...
		EngineCore.outputText( '\n\nIt hardly matters, as a few minutes later, you hear a cry of unabashed glee from the edge of camp, and turn to see ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' running up to meet a pair of oncoming figures.  You wave as Hel\'s father swoops his granddaughter up into a great bear hug, hefting the half-breed \'mander off her feet in his powerful arms.  ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' squeals with delight as she\'s practically carried into camp, until Hakon sets her down and hugs her mother, pulling Hel into a tight hug before she can finish saying hello.' );
		EngineCore.outputText( '\n\n"<i>Hi, [name],</i>" Kiri says, fluttering over to plant a peck on your cheek.  "<i>Long time no see.</i>"' );
		EngineCore.outputText( '\n\n"<i>Too right.  Put \'er there, ' + CoC.getInstance().player.mf( 'son', 'girl' ) + ',</i>" Hakon says, extending a hand to you, grinning wide as he lets his daughter and granddaughter go.  You shake the old salamander\'s hand, nearly wincing as his powerful grip throttles your wrist vigorously, practically walking you back into the heart of the camp.' );
		EngineCore.outputText( '\n\nTurning to ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ', Hakon smiles with pride.  "<i>Gods and demons, girl, you\'ve grown!  What have you two been feeding her, huh?</i>"' );
		EngineCore.outputText( '\n\n"<i>And you should see how strong she is!</i>" Hel grins.  "<i>Go on, ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ', show the old scales what you\'ve got.</i>"' );
		EngineCore.outputText( '\n\n"<i>Old scales!?  I\'ll show you old, you little brat.  Come on, hit me!</i>" Hakon says, just before ' + CoC.getInstance().flags[ kFLAGS.HELSPAWN_NAME ] + ' punches him right in the chest, knocking him right on his ass.  He tumbles back with a grunt, shaking his head with a wry laugh.  "<i>Ha!  Maybe these scales are getting old after all.  This one\'s gonna be a hell of a fighter, though.  Should take her hunting with us one of these days.  What do you say, kid?</i>"' );
		EngineCore.outputText( '\n\n"<i>Sure!  ' );
