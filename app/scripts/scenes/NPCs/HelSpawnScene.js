...
angular.module( 'cocjs' ).run( function( SceneLib, MainView, Helspawn, PregnancyStore, Descriptors, AppearanceDefs, CoC, kFLAGS, Utils, StatusAffects, EngineCore ) {
	...
	//Expanded Variable can now reach 250%. When this occur, At the Edge of Paradise begins. HelAffection is raised as: 10 for sex, 15 for threesomes, 10 for talking/hugging/cuddling.;
	//New Variable: [HelspawnPersonality], rated 0-100. ;
	//New Variable: ' + this.championRef() + ', being whatever the fuck Helspawn calls you.;
	...
	HelSpawnScene.prototype.championRef = function() {
		if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] === 0 ) {
			return CoC.player.mf( 'Daddy', 'Mom' );
		} else {
			return CoC.player.short;
		}
	};
	HelSpawnScene.prototype.helspawnFollower = function() {
		return CoC.flags[ kFLAGS.HELSPAWN_AGE ] === 3;
	};
	...
	HelSpawnScene.prototype.heliasAppearanceScreen = function() {
		...
		MainView.outputText( 'Hel the salamander stands seven feet tall, with pale skin and thick, bright-red scales covering her arms and legs, though she has a normal human torso and face.  A fiery tail swishes gaily behind her, blazing with a ' );
		if( CoC.flags[ kFLAGS.HEL_BONUS_POINTS ] < 150 ) {
			MainView.outputText( 'bright orange glow' );
		} else {
			MainView.outputText( 'bright white glow' );
		}
		MainView.outputText( ' that lets off a pleasant heat, though it never seems to burn you.  Hel is wearing her scale bikini and a leather thong, and using her scimitar as a weapon' );
		//if Isabella is cool: ;
		if( CoC.flags[ kFLAGS.HEL_ISABELLA_THREESOME_ENABLED ] >= 1 ) {
			MainView.outputText( '; she’s got her blue bandanna wrapped around her head, mostly obscured by her fiery hair' );
		}
		MainView.outputText( '.  She has a human face, with bright red eyes, gentle, feminine features and a smattering of pale scales on her cheeks, like freckles.  Hel has long, bright-red hair bound in a pony-tail that hangs down her back.  She has wide-flared hips and a soft, squishy butt.  Her two reptilian legs are visibly adorned with scales and claws, ending in soft, leathery soles.' );
		if( CoC.flags[ kFLAGS.HELSPAWN_NAME ] !== 0 ) {
			MainView.outputText( '  A dark trio of scars run down Hel’s thighs, left by ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + '’s youthful claws.' );
		} else {
			switch( SceneLib.helScene.pregnancy.event ) {
				case 1: //She's pregnant, but no special text yet
					break;
				case 2:
					MainView.outputText( '  Hel\'s just starting to show a little bulge of pregnancy.' );
					break;
				case 3:
					MainView.outputText( '  Hel’s belly is starting to look fairly bloated, swelling with her child.' );
					break;
				case 4:
					MainView.outputText( '  Hel’s belly is positively gravid, full of a little salamander child.' );
					break;
				default: //She's not pregnant, check if she's ready for it
					if( CoC.flags[ kFLAGS.HEL_BONUS_POINTS ] >= 150 ) {
						MainView.outputText( '  Hel’s long, reptilian tail is currently burning white-hot, signaling her body’s ready for motherhood.' );
					}
			}
		}
	...
	HelSpawnScene.prototype.helSpawnsAppearanceScreen = function() {
		...
		if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] === 0 ) {
			...
			MainView.outputText( 'Hel’s ' );
		}
		MainView.outputText( 'salamander daughter, ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ', stands about six-foot-six tall, with pale skin and thick, ' );
		if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] === 1 ) {
			MainView.outputText( 'chitin-black' );
		} else {
			MainView.outputText( 'bright-red' );
		}
		MainView.outputText( ' scales covering her arms and legs, though she has a normal human face and torso.  A long leathery tail swishes happily behind her, surrounded by a dim fiery aura that emits a pleasant heat, though thankfully it isn’t hot to the touch, seemingly incapable of harming you.  ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' is wearing ' );
		if( CoC.flags[ kFLAGS.HELSPAWN_PERSONALITY ] >= 50 ) {
			MainView.outputText( 'a scale bikini, just like her mother’s' );
		} else {
			MainView.outputText( 'a short skirt, thigh-high boots, and a sky-blue blouse, in stark contrast to her mother’s sluttier attire' );
		}
		if( CoC.flags[ kFLAGS.HEL_ISABELLA_THREESOME_ENABLED ] >= 1 ) {
			MainView.outputText( ', with a brightly-colored scarf wrapped around her neck' );
		}
		MainView.outputText( '. She has a human face, with ' );
		if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] === 0 ) {
			MainView.outputText( 'golden' );
		} else if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] === 2 ) {
			MainView.outputText( 'bright green' );
		} else {
			MainView.outputText( 'crimson' );
		}
		MainView.outputText( ' eyes, girlish features, and a smattering of pale red scales on her cheeks, like freckles.  Her short, dark-red hair cuts off just below her ' );
		if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] === 2 ) {
			MainView.outputText( 'puppy-dog' );
		} else {
			MainView.outputText( 'reptilian' );
		}
	...
	HelSpawnScene.prototype.heliaBonusPointsAward = function() {
		...
		if( SceneLib.camp.companionsCount() === 1 ) {
			MainView.outputText( 'You cock an eyebrow, saying that you\'re already alone, but she shakes her head, saying she needs somewhere private, away from prying eyes.  ' );
		}
		...
		if( CoC.player.hasVagina() && CoC.player.hasCock() ) {
			MainView.outputText( ' and your [chest] and your [vagina]' );
		}
		if( !CoC.player.hasCock() && CoC.player.hasVagina() ) {
			MainView.outputText( ' [vagina] and [chest]' );
		}
	...
	//No;
	HelSpawnScene.prototype.noYouDontLoveHeliaYouMonster = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 68 );
		MainView.outputText( 'As much as you enjoy the sex, as much as you like Hel, you can\'t say you feel that way towards her.  You grasp her shoulders, telling her as gently as possible how you feel.  She nods slowly as you speak, rubbing at her eyes by the time you\'re finished.' );
		...
		//[HelLove to NO];
		CoC.flags[ kFLAGS.HEL_LOVE ] = -1;
		this.heliaLoveFollowup();
	};
	//Yes;
	HelSpawnScene.prototype.yesHeliaILoveYourButtHoleReaming = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 68 );
		MainView.outputText( 'Smiling, you tell her yes.  The salamander cries out with joy, jumping into your arms and kissing you, squeezing you so tightly that you feel like you\'re about to black out by the time she breaks the kiss, stepping back with chest heaving.' );
	...
	HelSpawnScene.prototype.heliaLoveFollowup = function() {
		...
		//If HelLove = No:;
		if( CoC.flags[ kFLAGS.HEL_LOVE ] === -1 ) {
			MainView.outputText( 'Even if you don\'t love me, you\'d love our child, right?  You can fuck me full of kids, or even... even get someone else to, I don\'t care.  I just </i>need<i> it, [name].  More than anything else in the whole wide world.' );
		} else {
			MainView.outputText( 'I love you, [name].  I love you so much.  I want to share this with you.  ' );
			if( CoC.player.hasCock() ) {
				MainView.outputText( 'You\'ll fuck me full of kids, right?  Please?  Give me your seed, [name].  Give it all to me until I\'m fucking gravid with your salamander babies.' );
			} else {
				MainView.outputText( 'Even if you can\'t fuck me full of babies... you could grow a dick!  There\'s plenty of things in this god-forsaken world that do that. O-or if you don\'t want to, I bet I can get one of the fox-girls, or a minotaur, or something.  You know I can find a dick somewhere, just say the word.' );
			}
		}
		...
		if( CoC.player.hasCock() && CoC.player.cockThatFits( SceneLib.helFollower.heliaCapacity() ) >= 0 ) {
			...
			MainView.outputText( '  <b>Unfortunately, you\'re too big to squeeze inside Helia to do the business yourself.  You might need to shrink down some.</b>' );
		}
		EngineCore.addButton( 1, 'Another Dad', this, this.getAnotherDad );
		EngineCore.addButton( 2, 'No Or Later', this, this.noKidsHel );
	};
	...
	HelSpawnScene.prototype.haveAKid = function() {
		...
		if( CoC.player.pregnancyIncubation > 0 ) {
			MainView.outputText( 'We\'re going to be mothers!' );
		} else {
			MainView.outputText( 'You\'re going to be a father... and me a mother!' );
		}
	...
	HelSpawnScene.prototype.HaveAHellKidPartII = function() {
		...
		if( CoC.flags[ kFLAGS.HEL_LOVE ] === 1 ) {
			MainView.outputText( ', and I\'m so very, very glad you love me back.' );
		} else {
			MainView.outputText( ', even if you can\'t say the same.  And that\'s alright; it doesn\'t change my heart, and it never will. I love you' );
		}
		...
		MainView.outputText( '\n\nYou tell her to rest easy you\'ll be there for her every step of the way.  She smiles ' );
		if( CoC.player.tallness >= 90 ) {
			MainView.outputText( 'up ' );
		} else if( CoC.player.tallness <= 72 ) {
			MainView.outputText( 'down ' );
		}
		MainView.outputText( 'at you, holding you tight for a long moment before stepping away, still holding your hands.  "<i>This is all new to me, [name].  I never pictured myself as a mother, with a mate and a stable, safe home - or as stable and safe as anything these days - but with you by my side, there\'s nothing I can\'t take on.</i>"' );
	...
	HelSpawnScene.prototype.getAnotherDad = function() {
		...
		if( !CoC.player.hasCock() ) {
			MainView.outputText( 'You tell Helia you\'d love to a share a child with her, but you\'re not... properly equipped for the endeavor.  "<i>That\'s fine!  I can... I can wait, a little.  I-if you want to go grow one, I mean.  If not, then we can find someone with a cock.  ' );
			if( CoC.player.statusAffectv1( StatusAffects.TelAdre ) >= 1 ) {
				MainView.outputText( 'There\'s Miko and Mai from the bar.  Mai\'s said she wanted a kid, but can\'t take care of one... she\'d probably be willing to fuck one into me!  If that\'s not alright, then... lemme think.  ' );
			}
		...
		//[Another Dad] (PC has a dick);
			...
			MainView.outputText( '"<i>W-wha?</i>" Hel says, cocking an eyebrow.  "<i>You sure, [name]?  I-if you don\'t want to be the dad, I guess I understand.  But why?  Am I not... no, nevermind.  I don\'t want to know.  Okay, uh, we can find someone with a cock.  ' );
			if( CoC.player.statusAffectv1( StatusAffects.TelAdre ) >= 1 ) {
				MainView.outputText( 'There\'s Miko and Mai from the bar.  Mai\'s said she wanted a kid, but can\'t take care of one... she\'d probably be willing to fuck one into me!  If that\'s not alright, then... lemme think.  ' );
			}
		...
		//[Spiderboy];
		EngineCore.addButton( 1, 'Spiderboy', this, this.spiderboyWouldBeBestDad );
		//[I will] (If PC has a dick);
		if( CoC.player.hasCock() && CoC.player.cockThatFits( SceneLib.helFollower.heliaCapacity() ) >= 0 ) {
			EngineCore.addButton( 2, 'I Will', this, this.haveAKid );
		} else if( !CoC.player.hasCock() ) {
			EngineCore.addButton( 2, 'I Will', this, this.growingDicks4Hel );
		}
		EngineCore.addButton( 3, 'No Or Later', this, this.noKidsHel );
	};
	//Mai;
	HelSpawnScene.prototype.maiWouldBeTheBestInseminator = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 68 );
		MainView.outputText( 'You tell Hel that you think Mai would make a lovely father.  Helia nods her agreement, saying, "<i>Yeah, I agree.  She\'s a beauty, and I\'m sure our child will be stunning... you wouldn\'t mind if she visited, right?  I mean, you and I will be raising our kid - and he\'ll be ours for sure - but I\'m sure Mai will want to at least visit her kid.</i>"' );
	...
	HelSpawnScene.prototype.spiderboyWouldBeBestDad = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 68 );
		MainView.outputText( 'You tell Helia to go find a spider boy to jump.  She beams at you, and skips off toward the swamp calling, "<i>Thank you, thank you thank you, [name]!</i>" over her shoulder as she goes.  You suppose the next time you see her, Hel\'s probably going to be pregnant with the child you\'ll be helping to raise.' );
	...
	HelSpawnScene.prototype.growingDicks4Hel = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 68 );
		MainView.outputText( 'You tell Hel to give you a little while, that you\'ll go find something to grow a cock for her.' );
	...
	HelSpawnScene.prototype.noKidsHel = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 68 );
		MainView.outputText( 'You shake you head and say no, you don\'t want to have children.  Not right now, anyway.  ' );
		if( CoC.player.hasCock() && CoC.player.cockThatFits( SceneLib.helFollower.heliaCapacity() ) >= 0 ) {
			MainView.outputText( 'You couldn\'t if you wanted to anyway - you\'re too big to fit.  ' );
		}
		MainView.outputText( 'As the words leave your lips, you can see Hel\'s shoulders slump, a crestfallen look spreading across her face.  "<i>A-are you sure?  Please, [name], I really, really want a child.  For </i>us<i> to have one.</i>"' );
	...
	HelSpawnScene.prototype.helGotKnockedUp = function() {
		MainView.spriteSelect( 68 );
		CoC.flags[ kFLAGS.HEL_NTR_TRACKER ] = 2;
		...
		if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] === 2 ) {
			MainView.outputText( 'warm ' );
		} else {
			MainView.outputText( 'wan ' );
		}
	...
	//Nah // Combine;
	HelSpawnScene.prototype.dontTellMeAboutSpiderboy = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 68 );
		MainView.outputText( '"<i>So, let\'s just say I came away from that encounter carrying enough cute spiderboy cum to fill a bucket.  If I\'m not carrying his child now, then I\'m hopeless.  But... you know, I think I can feel it, [name].  I shouldn\'t be able to, but I can.  Oh, god,</i>" Hel says, looking pointedly away from you, staring wide-eyed into the morning sky.' );
		MainView.outputText( '\n\n"<i>What\'s wrong?</i>"' );
	...
	//Nah // Combine;
	HelSpawnScene.prototype.dontTellMeAboutMai = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 68 );
		MainView.outputText( '"<i>So, let\'s just say I came away from that encounter carrying enough foxy herm cum to fill a bucket.  If I\'m not carrying her child now, then I\'m hopeless.  But... you know, I think I can feel it, [name].  I shouldn\'t be able to, but I can.  Oh, god,</i>" Hel says, looking pointedly away from you, staring wide-eyed into the morning sky.' );
		MainView.outputText( '\n\n"<i>What\'s wrong?</i>"' );
		...
		MainView.outputText( '\n\nYou put an arm around the mother-to-be\'s shoulder, saying, "<i>I thought this was what you wanted, Hel.  You were-</i>"' );
		MainView.outputText( '\n\n"<i>Yeah, I know.  Forget about it, I\'m fine.  Really.  It\'s just that this happened all so suddenly, you know?  This isn\'t exactly where I saw myself a year ago.  Not that I\'m complaining,</i>" she adds, turning back to you with a kiss, squeezing you with her tail.  "<i>I just hope I\'m ready, I guess.  Until the Pale Flame took, I\'d never really thought about being a mother.  Never really wanted it.  But last night, it was like I was possessed, like I couldn\'t control myself.  I just... needed you.  Well, maybe not your sperm - not literally, I mean - but it was like my body screaming \'You\'ve found ' + CoC.player.mf( 'him', 'her' ) + ', ' + CoC.player.mf( 'he', 'she' ) + '\'s the one, this is a keeper!  Time to propagate!\'  I-I can\'t help but feel like this wasn\'t entirely my choice, you know? Like I was acting on instinct, like an animal, not a person.  But at the same time, if I\'ve made a mistake, then I have you here with me, to help me like you always do.  I love you with all my heart, [name]' );
		//if HelLove is Yes:;
		if( CoC.flags[ kFLAGS.HEL_LOVE ] === 1 ) {
			MainView.outputText( ', and I\'m so very, very glad you love me back.' );
		} else {
	...
	//if(CoC.flags[kFLAGS.HELIA_PREGNANCY_INCUBATION] < 300 && CoC.flags[kFLAGS.HEL_PREGNANCY_NOTICES] === 0) {;
	HelSpawnScene.prototype.bulgyCampNotice = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 68 );
		MainView.outputText( 'As you\'re walking through camp, your eyes wander over toward Helia, sunning herself on a stone near the edge of camp.  You can just see that her belly\'s starting to bulge out from under her, and Hel\'s hands lie protectively over her full womb, absently rubbing the bulge of her stomach.' );
		EngineCore.doNext( MainView, MainView.playerMenu );
	};
	...
	HelSpawnScene.prototype.heliaSwollenNotice = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 68 );
		MainView.outputText( 'You note that Hel\'s wandering aimlessly around camp, one hand over her belly as she mumbles to herself.  You could swear she was cursing her now quite swollen belly, but suddenly she gives a girlishly happy cry and waves you over.\n' );
	...
	//Boy;
	HelSpawnScene.prototype.youWantABoy = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 68 );
		MainView.outputText( '"<i>Is that so? Yeah, I can see it.  ' );
		if( CoC.player.hasCock() && !CoC.player.hasVagina() ) {
			MainView.outputText( 'If I were a guy, I\'d want a big strong son to hang out with, too.  Take him fishing, teach him how to fight the way you do... you\'ll make a great dad, my love.  I\'m sure you will.' );
		} else {
			MainView.outputText( 'Would be nice to have a man around here, you know?  I miss hanging around the boys back home, watching \'em strut like peacocks for every passing girl.' );
		}
		MainView.outputText( ' And any son of mine is going to be a real lady killer, mark my words.  We\'re going to have to fight off whole hordes of goblin sluts, all looking for a piece of our handsome little boy before you know it.</i>"' );
	...
	HelSpawnScene.prototype.youWantAGirl = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 68 );
		MainView.outputText( '"<i>Yeah, a girl would be pretty great.  ' );
		//if PC is male:;
		if( CoC.player.hasCock() && !CoC.player.hasVagina() ) {
			MainView.outputText( 'I dunno if you had any sisters growing up, [name], but let me warn you little girl, especially a little salamandress, is going to be a hell of a handful.  But I can just see you when she\'s all grown up, packing a big old sword and threatening every boy that wants a piece of her her right or you\'ll have the CHAMPION to deal with.  Rawr.\'' );
		} else {
			MainView.outputText( 'We\'re going to be a gaggle of tittering girls before you know it, though.  Salamanders grow up so fast, [name]...  I just hope she doesn\'t turn out like me, you know?  I don\'t know if I could stomach seeing my little girl becoming a wanton slut like her mom.  I might get jealous, after all...' );
		}
		MainView.outputText( '  Any daughter of ours is going to be a real beauty, mark my words. She\'ll make a succubus look like a toad before she\'s out of swaddling.</i>"' );
		MainView.outputText( '\n\nYou share a quiet laugh with your lover before leaving her with a kiss and a final pat on the belly - and feeling the little kick of your spawn reacting to you.' );
		EngineCore.doNext( MainView, MainView.playerMenu );
	};
	//Hel enters 'gravid' state, play at random from camp menu:;
	//if(CoC.flags[kFLAGS.HELIA_PREGNANCY_INCUBATION] === 100);
	HelSpawnScene.prototype.heliaGravidity = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 68 );
		//Shouldn't be needed, bet this was originally here to stop duplicate notices:	flags[kFLAGS.HEL_PREGNANCY_INCUBATION]--;;
		MainView.outputText( 'You can\'t help but notice that Hel\'s starting to have a hard time getting around, lately - and she\'s been sticking closer and closer to camp, barely leaving at all the last few days.  Now, she\'s fussing around her part of camp, trying to beat some spare logs into a crib one-handed.  You can\'t remember the last time you saw her walking around without a hand on her back to support her gravid belly, the other absently rubbing or poking at it, already playing with the child inside her.' );
	...
	//[Hel's Lifestyle];
	//Encourage Her;
	HelSpawnScene.prototype.encouragePregalia = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 68 );
		MainView.outputText( 'You squeeze Hel\'s shoulder and tell her that she\'s going to be a great mother, and that her child would do well to learn from a beautiful, powerful warrior like dear old mom.  ' );
		if( CoC.flags[ kFLAGS.HEL_LOVE ] === 1 ) {
			MainView.outputText( 'You love Hel' );
		} else {
			MainView.outputText( 'Hel\'s your friend' );
		}
		MainView.outputText( ', and if her child follows in her footsteps, so much the better.  Your lover manages a half-hearted laugh, blushing at your words.  Her tail tip brushes your cheek as its owner cuddles up against you, smiling.  "<i>Thanks, lover mine.  I just hope I - we - can do right by the kid.  That\'s all.</i>"' );
	...
	HelSpawnScene.prototype.helsLifestyle = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 68 );
		MainView.outputText( 'Being very serious for a moment, you tell Hel that if she\'s worried about her terrible choices rubbing off on her kid, maybe she ought to reconsider some of the things she\'s done less of a slut, drink less, be less of a bloodthirsty maniac in battle.  Slowly, Hel nods to the rhythm of your words.  "<i>Yeah, maybe... maybe I should.  I don\'t know if I can change though, [name].  I\'m not as young as I was - or at least, I don\'t feel it - and I\'ve been living the way I have been for years.  I don\'t know if I could just give up the shit I do partying and the drinking, the fighting and the fucking.  It\'s part of who I am, ' );
		if( CoC.flags[ kFLAGS.HEL_LOVE ] === 1 ) {
			MainView.outputText( 'and I hope that\'s part of why you love me' );
		} else {
			MainView.outputText( 'even though I know that\'s probably part of why you can\'t love me back' );
		}
		MainView.outputText( '.  I\'ll try and rein it in, but maybe... it\'d be better if you were the one who raised my child, [name].  God knows I\'m the least qualified person to do it.</i>"' );
	...
	//[NEXT];
	HelSpawnScene.prototype.nameDatHelspawn = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 68 );
		MainView.outputText( 'You and Helia share a laugh, looking down at ' );
		if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] === 0 ) {
			MainView.outputText( 'your ' );
		} else {
			MainView.outputText( 'her ' );
	...
	HelSpawnScene.prototype.applyHelspawnName = function() {
		MainView.spriteSelect( 68 );
		//Easter Egg Names Hel WILL NOT ALLOW:;
		if( MainView.nameBox.value === '' || MainView.nameBox.value === 'Hellgirl' || MainView.nameBox.value === 'Kid' || MainView.nameBox.value === 'Phoenix' || MainView.nameBox.value === 'Savin' || MainView.nameBox.value === 'Helia' || MainView.nameBox.value === 'Mini-doofus' ) {
			MainView.clearOutput();
			if( MainView.nameBox.value === '' ) {
				MainView.outputText( '<b>You must select a name.</b>', false );
			} else if( MainView.nameBox.value === 'Hellgirl' ) {
				MainView.outputText( '"<i>Hey, that\'s copyright infringement.  Probably.</i>"' );
			} else if( MainView.nameBox.value === 'Kid' ) {
				MainView.outputText( '"<i>Wow, what are you, five?</i>"' );
			} else if( MainView.nameBox.value === 'Phoenix' ) {
				MainView.outputText( '"<i>Oh hell no.  You\'re not naming my little girl after one of those... things!  Sorry Kiri, but still!</i>"' );
			} else if( MainView.nameBox.value === 'Savin' ) {
				MainView.outputText( '"<i>That\'s a boy\'s name, dumbass.</i>"' );
			} else if( MainView.nameBox.value === 'Helia' ) {
				MainView.outputText( '"<i>My favorite name!  Except it\'s kinda taken, love.  Don\'t want things to get too confusing around here, do you?</i>"' );
			} else if( MainView.nameBox.value === 'Mini-doofus' ) {
		...
		CoC.flags[ kFLAGS.HELSPAWN_NAME ] = MainView.nameBox.value;
		MainView.nameBox.visible = false;
		//[Name Field];
		//Easter Egg Names:;
		MainView.clearOutput();
		if( CoC.flags[ kFLAGS.HELSPAWN_NAME ] === 'Helspawn' ) {
			MainView.outputText( '"<i>That\'s what I was gonna call her!  Except not at all.  God dammit, seriously, [name]?</i>"\n\n' );
		}
		if( CoC.flags[ kFLAGS.HELSPAWN_NAME ] === 'Jayne' ) {
			MainView.outputText( '"<i>That sounds like a hero\'s name... I like it.  A name that demands respect!</i>"\n\n' );
		}
		if( CoC.flags[ kFLAGS.HELSPAWN_NAME ] === 'Hesper' ) {
			MainView.outputText( '"<i>Ah, the Evening Star.  She\'ll be the star of my life, alright...</i>"\n\n' );
		}
		if( CoC.flags[ kFLAGS.HELSPAWN_NAME ] === 'Kiri' ) {
			MainView.outputText( '"<i>Aw, that\'s great, lover.  Sis\'ll shit her pants when she hears her niece is named after her!  Gonna get confusing, though...</i>"\n\n' );
		}
		if( CoC.flags[ kFLAGS.HELSPAWN_NAME ] === 'Mai' ) {
			MainView.outputText( '"<i>Aw, that\'s great, lover.  Mai\'ll be honored, I know it.  And it is a great name...</i>"\n\n' );
		}
		if( CoC.flags[ kFLAGS.HELSPAWN_NAME ] === 'Tanis' ) {
			MainView.outputText( '\<i>I don\'t... I don\'t know if I want her to have that name, lover.  It\'s got a lot of baggage, but... but it does my mother an honor.  More than I could ever give her.</i>"\n\n' );
		}
		//Syn: ;
		if( CoC.flags[ kFLAGS.HELSPAWN_NAME ] === 'Syn' ) {
			MainView.outputText( '"Awesome.  I was hoping you\'d choose that one, lover mine.</i>"\n\n' );
		}
		if( CoC.flags[ kFLAGS.HELSPAWN_NAME ] === 'Chara' ) {
			MainView.outputText( '"<i>Awesome.  You liked my ideas, after all!</i>"\n\n' );
		}

		MainView.outputText( '"<i>So... ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' it is.  That\'s a good name, my love.  A strong name.  ' );
		if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] === 0 ) {
			MainView.outputText( 'Our' );
		} else {
			MainView.outputText( 'my' );
		}
		MainView.outputText( ' little girl\'s going to grow up big and strong with a name like that... and with you around to guide her,</i>" Hel says, kissing your cheek.  "<i>Why don\'t you give me a minute to catch my breath and get the little one situated, and let\'s talk.  Alright?</i>"' );
		MainView.outputText( '\n\nYou nod and help Helia to her feet, still holding ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' to her breast.  Your lover gives you a wink before walking bow-legged back toward her part of camp, and the little crib she\'s built beside her hammock.' );
		this.helSpawnsSetup();
	...
	HelSpawnScene.prototype.helSpawnsSetup = function() {
		...
		CoC.flags[ kFLAGS.HELSPAWN_AGE ] = 1;
		CoC.flags[ kFLAGS.HELSPAWN_GROWUP_COUNTER ] = 1;
	...
	HelSpawnScene.prototype.heliaTalkEight = function() {
		MainView.clearOutput();
		CoC.flags[ kFLAGS.HEL_TALK_EIGHT ] = 1;
		MainView.spriteSelect( 68 );
		MainView.outputText( 'Watching your little daughter crawl around, you and Hel sit down together arm in arm, smiling as the little salamander girl plays with bits and pieces of Hel\'s gear.' );
		...
		if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] === 0 ) {
			MainView.outputText( 'parental ' );
		} else {
			MainView.outputText( 'care giver\'s ' );
		}
		MainView.outputText( 'pride, either.  She really is growing up faster, many times faster than a human girl.  And, by the sound of things, many times faster than a salamander ought to.  You ask Hel if that\'s right, and she nods gravely.' );
		...
		MainView.outputText( ' little girl\'s going to be a big girl damn soon.  Let\'s enjoy it while we can, eh?</i>"' );
		MainView.outputText( '\n\nYou nod to your lover, and the both of you walk over to spend some quality time playing with ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + '.' );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	...
	HelSpawnScene.prototype.playWithYourKid = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 68 );
		MainView.outputText( 'Picking ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' up off the ground, you spin the giggling little girl around, laughing as your ' );
		if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] !== 0 ) {
			MainView.outputText( 'ward ' );
		} else {
			MainView.outputText( 'daughter ' );
		}
		MainView.outputText( 'cries out in surprise and glee.  Soon, you\'re bouncing her, then down on the ground crawling around with her, playing peek-a-boo and a dozen other games as you and ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' laugh and hug.  Helia herself slips down with you before you know it, holding her daughter close as she tries to communicate, teaching the ever-growing girl a few words at a time.' );
		MainView.outputText( '\n\n"<i>Mommy!</i>" ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' eventually cries, bopping her mother on the nose. Hel laughs, but it turns into a sigh as her daughter reaches down and fiddles with her top, obviously hungry.' );
		MainView.outputText( '\n\n"<i>We\'ve got to wean this one before she bites my tits off,</i>" Hel groans as ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' latches on, starting to suckle.  "<i>Those teeth came in fast...</i>"' );
		MainView.outputText( '\n\nLaughing, you rustle ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + '\'s fiery hair and leave mother and daughter to finish the meal.' );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	//Event: Helspawn Graduates from Baby to Teenager;
	//(Play as the PC wakes up);
	HelSpawnScene.prototype.helSpawnGraduation = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 68 );
		CoC.flags[ kFLAGS.HELSPAWN_GROWUP_COUNTER ] = 1;
		CoC.flags[ kFLAGS.HELSPAWN_AGE ]++;
		MainView.outputText( 'You wake up to a scream, high-pitched and terrified.  You bolt up out of bed, tangling up in your bedroll for a minute before stumbling over to Hel and... ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + '?' );
		...
		MainView.outputText( '-scaled figure curled up next to Helia; Hel herself is looking wide-eyed down at the drowsy figure on her breast, just barely opening her eyes.  "<i>Y-you\'re... you\'re huge!</i>" Hel manages to gasp out, pushing herself back and away from her daughter.' );
		MainView.outputText( '\n\n"<i>Mom?</i>" the girl asks, rubbing her temple.  As she sits up, you can see that she\'s grown exponentially since you put her to bed the night before nearly six feet tall, her hair hanging down in long, silky locks to her bare behind.  Her figure\'s filled out, a pair of burgeoning breasts and flared hips showing themselves on her feminine body.  She\'d easily match any of the teenage girls in your home town in terms of size and figure, maybe fifteen or sixteen in appearance now.' );
		MainView.outputText( '\n\n"<i>Ugh, what\'s wrong?</i>" ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' groans, looking up at Hel, then to you.  "<i>' + this.championRef() + ', why\'s mom screaming?</i>"' );
		...
		MainView.outputText( '\n\nYou have to admit, the change is sudden and drastic... but the girl\'s scales, her eyes, they\'re certainly ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + '\'s.  ' );
		if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] === 2 ) {
			MainView.outputText( 'And the puppy ears, now more wolf-like, are a dead giveaway.  Utterly unique to her.  ' );
		}
		MainView.outputText( 'You sit down beside the obviously confused girl and stroke her hair, telling her it\'s all right, her mother\'s just startled, that\'s all.' );
		MainView.outputText( '\n\n"<i>It\'s just me, mom,</i>" ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' says, sliding over to sit beside her mother, who\'s still wide-eyed with shock.  Your girl tries for a few moments to talk to Hel, but seeing that venture prove fruitless - Hel seems utterly insensate, paralyzed by shock - she instead curls back up beside her mother, head rested on Helia\'s shoulder.  Eventually, warily, Hel reaches out a trembling hand to brush her daughter\'s cheek.' );
	...
	HelSpawnScene.prototype.helspawnDiscoversBooze = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 68 );
		CoC.flags[ kFLAGS.HELSPAWN_DISCOVER_BOOZE ] = 1;
		...
		if( CoC.player.race() !== 'cat-boy' && CoC.player.race() !== 'cat-girl' ) {
			MainView.outputText( 'a cat' );
		} else {
			MainView.outputText( 'the cat you are' );
		}
	...
	//Scold Her;
	HelSpawnScene.prototype.scoldHelSpawn = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 68 );
		MainView.outputText( 'You cross your arms over your chest and ask your little salamander exactly what she thinks she\'s doing.' );
		...
		MainView.outputText( '\n\n"<i>Alright, ' + this.championRef() + ',</i>" ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' says, curling up beside you, head resting on your [chest].' );
		//{HelspawnChaste +10};
		CoC.flags[ kFLAGS.HELSPAWN_PERSONALITY ] -= 10;
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	...
	HelSpawnScene.prototype.encourageHelspawn = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 68 );
		MainView.outputText( '"<i>Move over, you,</i>" you say, sliding down beside your little salamander as you pop the tap open again.  "<i>Gotta pace yourself, see,</i>" you add, taking a long draught before closing it off again, savoring the potent taste of Hel\'s mighty brew.' );
		MainView.outputText( '\n\n"<i>Wha...?</i>" ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' starts to say, before catching on as you drink, inviting her to join you.' );
		...
		MainView.outputText( '\n\n"<i>Uh...</i>" you groan, slumping over onto your [ass].  ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + '\'s right behind you, collapsing on top of you in a pile of ' );
		if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] !== 1 ) {
			MainView.outputText( 'red ' );
		} else {
			MainView.outputText( 'black ' );
		}
		MainView.outputText( 'scales and the smell of a burning brewery.' );
		...
		MainView.outputText( '\n\n"<i>I saved some for ya, mom,</i>" ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' groans, retching.' );
	...
	HelSpawnScene.prototype.helSpawnChoosesAFightingStyle = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 68 );
		MainView.outputText( 'As you make your way back to camp, you begin to hear the sounds of fighting from ahead.  Readying your [weapon], you pick up the pace until your campsite comes into view.  You relax, seeing that it\'s only ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' and her mother.  Helia\'s standing near the center of camp, ' );
		if( SceneLib.camp.companionsCount() > 3 ) {
			MainView.outputText( 'apparently having cleared out most of your other followers for the moment, ' );
		}
		MainView.outputText( 'her sword held at a guard pose between herself and a training dummy. ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' stands a few feet behind her mother, watching intently as Hel paces around the dummy, breathing hard.' );
		...
		EngineCore.addButton( 0, 'You Train', this, this.swordAndBoardmander );
		EngineCore.addButton( 1, 'Loan', this, this.dasBarbarimander );
	};
	//Archery (Dat Snipermander);
	HelSpawnScene.prototype.snipermanders = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 68 );
		CoC.flags[ kFLAGS.HELSPAWN_WEAPON ] = 'bow';
		MainView.outputText( 'You tell Hel to take a break got this handled. Grabbing your bow and quiver from over your shoulder, you hand them over to ' );
		if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] === 0 ) {
			MainView.outputText( 'your' );
		} else {
			MainView.outputText( 'Hel\'s' );
		}
		MainView.outputText( ' eager daughter. ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' takes them up with surprising reverence, holding them as gingerly as glass.  Grinning, you clasp her shoulder and tell her how to put the quiver on, and get her stance right for shooting.  It takes her a moment to get set up scaled, reptilian legs don\'t naturally stand in the shooting pose you\'ve adopted' );
		if( CoC.player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_LIZARD ) {
			MainView.outputText( ', even with your own lizard legs' );
		}
		MainView.outputText( ', and you find yourself having to correct her grip on the haft several times.  But, finally, you get her ready to shoot.' );
		...
		if( CoC.player.findStatusAffect( StatusAffects.CampRathazul ) ) {
			...
			MainView.outputText( '  You hear a sudden yelp of pain from across camp. Valeria slithers up to you with an irritated look on her gooey face, pointing an accusing finger at an arrow sticking out of her tit.  "<i>Dammit, [name], I\'m a googirl, not a pin cushion!</i>"  You wave her off, and tell ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' to try again.' );
		} else if( SceneLib.latexGirl.latexGooFollower() ) {
			MainView.outputText( '  You hear a sudden yelp of pain from across camp.  ' + CoC.flags[ kFLAGS.GOO_NAME ] + ' slithers up to you with an irritated look on her gooey face, pointing an accusing finger at an arrow sticking out of her tit.  "<i>Dammit, [name], I\'m a googirl, not a pin cushion!</i>"  You wave her off, and tell ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' to try again.' );
		}
		MainView.outputText( '\n\n"<i>I-I dunno, ' + this.championRef() + ',</i>" ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' whines, biting her lip as she traces the arrow\'s path with her eyes.  "<i>Maybe mom was right. I should just stick to a sword. That\'s easy....</i>"' );
		MainView.outputText( '\n\nYou sigh and tell her to try again.  She can\'t expect to be perfect on the first try, after all.  It takes a little convincing, but soon you\'ve got ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' back into form and nocking an arrow.  You put your hands on hers, helping the young salamander take aim, drawing down on her straw-stuffed target.  Barely audible over ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + '\'s nervous breathing, you whisper, "<i>Take it easy.  Relax.... Good.  Take aim... now breathe out.  Exhale and loose.</i>"' );
		MainView.outputText( '\n\nAgain, ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' lets an arrow fly - and this one strikes true.  You watch the missile streak across camp, slamming into the training dummy\'s crotch with deadly force.  It sways, buckles, and falls to pieces.' );
		MainView.outputText( '\n\n"<i>Ouch,</i>" Hel laughs, sipping on her flask behind you.  "<i>What\'d that poor dummy ever do to you?</i>"' );
		MainView.outputText( '\n\n"<i>I did it!</i>" ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' shouts, pumping her fist in the air. Her mother reaches over to ruffle her fiery hair, and you both shower her with congratulations.' );
		MainView.outputText( '\n\nBefore she can get cocky, though, you push the bow back into her hands.  "<i>Lesson 2, kiddo: one shot is never enough.</i>"' );
		MainView.outputText( '\n\nBy the time ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' has her shooting stance back, her mother\'s already set the dummy back up, ready for round two.  It\'s several hours long before the three of you quit spent drilling ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' on stance and aim, even going a round with her and a pair of daggers to remind her that enemies will eventually close in if she\'s not careful.  She learns quickly, though, and soon she hardly needs your help to plant an arrow in the target\'s chest.' );
		...
		MainView.outputText( 'daughter will be a warrior worthy of her parent' );
		if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] === 0 ) {
	...
	HelSpawnScene.prototype.swordAndBoardmander = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 68 );
		CoC.flags[ kFLAGS.HELSPAWN_WEAPON ] = 'scimitar and shield';
		MainView.outputText( 'You tell Hel to take a break got this handled.  You grab some kindling and string from your supplies, and a few minutes later you\'ve got a pair of small, vaguely sword-like practice weapons pieces together.  Tossing one to the young salamander, you tell her to come get some.' );
		MainView.outputText( '\n\n' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' gives the blade a few practice swings before grabbing the wooden blade in both hands and charging at you.  You put your guard up, catching her first overhand blow and turning it aside, nearly toppling the girl over.  She rolls with it, spinning out of your guard and trying a strike from the side.  You parry it, catch an opening, and give her a nice wallop on the shoulder.  A real blade would have nearly cut her arm off, but ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' merely stumbles back, yelping in pain.' );
		...
		MainView.outputText( '\n\nYou sigh.  "<i>Again.</i>"' );
		MainView.outputText( '\n\n' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' picks herself up and grips her blade.  This time she\'s a little more cautious, approaching slowly, pausing just out of reach.  You hide a grin and bring your own sword up, the tips of your blades nearly touching.  "<i>Good.  No need to be reckless, kiddo.  Don\'t just charge in at the first opportunity; you\'ll get yourself killed.</i>"' );
		MainView.outputText( '\n\n"<i>Get ' + CoC.player.mf( 'him', 'her' ) + ', ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + '!</i>" Hel calls from the sidelines, sipping on a mug.  "<i>Show ' + CoC.player.mf( 'him', 'her' ) + ' what a salamander can do!</i>"' );
		if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] === 0 ) {
			MainView.outputText( '\n\nYour' );
		} else {
			MainView.outputText( '\n\nHer' );
		}
		MainView.outputText( ' daughter braces herself, drawing a breath just a second before she launches her attack, giving herself away.  You parry three quick blows, stepping back as ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' leaps into another overhand strike.  You dodge, spinning out of the attack and slicing her right across her bare belly.' );
		MainView.outputText( '\n\n"<i>Gah!</i>" she cries, stumbling to her knees.  "<i>I-I can\'t...</i>"' );
		MainView.outputText( '\n\nYou pull ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' to her feet and ruffle her hair.  "<i>C\'mon kiddo, don\'t give up.  Here,</i>" you say, grabbing a dinner plate and some leather straps from your kit.  In a moment, you\'ve got a rudimentary shield cobbled together.  You fit it over the young salamander\'s free arm and give her a few pointers on stance.  Nervously, she brings her makeshift shield to bear against you, fidgeting under the heft of it as you make a few slow, easy to parry swings for her to get used to it.  Steadily you pick up the pace, swinging your training sword faster and harder, forcing ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' to parry and block, retreating against your onslaught.  You keep it up for a full minute before relenting, letting the kid seize the initiative.  She swings at you, rolling with each parry you make, cleaving into another swing, turning your ripostes aside with her shield and forcing you back.' );
		MainView.outputText( '\n\nYou wince in pain as ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' manages to nick you, turning one of your blows aside with her shield and striking through the opening left in your guard.  "<i>Alright, alright,</i>" you laugh, rubbing your shoulder.  "<i>Good job, ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + '. You\'re getting the hang of that shield!</i>"' );
		MainView.outputText( '\n\n"<i>Thanks, ' + this.championRef() + '!</i>" the little salamander grins, hugging you.' );
		MainView.outputText( '\n\n"<i>Alright, alright,</i>" Hel says, grabbing the dummy sword from your hand.  "<i>Now it\'s time to fight a REAL warrior, ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + '. Come get some!</i>"' );
		MainView.outputText( '\n\nIt\'s several hours long before the three of you quit spent drilling ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' on stance and parrying, trading off with Helia every few rounds to keep ' );
		if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] !== 0 ) {
			MainView.outputText( 'her' );
		} else {
			MainView.outputText( 'your' );
		}
		MainView.outputText( ' daughter on her toes.  She learns quickly, though, and soon she hardly needs reminding to keep her shield up and stance braced against Helia\'s overwhelming assaults.' );
		MainView.outputText( '\n\nA few hours like this every night, and ' );
		if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] === 0 ) {
			MainView.outputText( 'your ' );
		} else {
	...
	//{if PC has 200 gems};
	HelSpawnScene.prototype.dasBarbarimander = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 68 );
		CoC.flags[ kFLAGS.HELSPAWN_WEAPON ] = 'scimitar';
		if( CoC.player.gems >= 200 ) {
			MainView.outputText( 'You sigh and dig out a handful of gems.  Helia beams at you, planting a quick kiss on your cheek before grabbing ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' and the loot.  "<i>Thanks for the loan, lover mine.  C\'mon, sweetie, we\'re going for a trip to town.</i>"' );
	...
	HelSpawnScene.prototype.helspawnIsASlut = function() {
		MainView.spriteSelect( 68 );
		CoC.flags[ kFLAGS.HELSPAWN_FUCK_INTERRUPTUS ] = 1;
		...
		if( CoC.player.weaponName !== 'fists' ) {
			MainView.outputText( 'clutch your [weapon]' );
		} else {
			MainView.outputText( 'ball your fists' );
		}
		MainView.outputText( ' as the intruder passes by, ready to defend yourself and your loved ones.  The moment they\'re out of reach, you roll onto your [feet], looking around for signs of a massed demon attack.  Instead, you see the outline of ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' in the dying embers of the fire light, leading what looks like a spider girl by the hand toward her bed.' );
		...
		MainView.outputText( 'is doing.  Well, it doesn\'t take a genius to figure that out, really, but still, you have to be sure.  You slide right up to them, just out of their lines of sight, as ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' turns around, letting the spider-kin unclasp her bra and grab her tits, squeezing the small mounds as ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' suppresses a moan, reaching back to slip a hand into the spider\'s crotch, grabbing her - his dick.' );
		MainView.outputText( '\n\nWell then. You suppose you ought to go stop ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' from making a whore of herself... but then again, she\'s old enough to make her own mistakes by now.' );
	...
	//Do Nothing;
	HelSpawnScene.prototype.helspawnDoNothing = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 68 );
		MainView.outputText( 'You turn around and head back to your bed.  As soon as you\'re under your blanket, your ears are assaulted with the quiet moans and grunts of pleasure coming from ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + '\'s bed as she and her first little boyfriend get it on.  You can practically hear the tail-pegging from here.  How cute!' );
		//{HelspawnSlutty +10};
		CoC.flags[ kFLAGS.HELSPAWN_PERSONALITY ] += 10;
		EngineCore.doNext( MainView, MainView.playerMenu );
	};
	...
	HelSpawnScene.prototype.helSpawnStopFucking = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 68 );
		MainView.outputText( 'You sigh and step out of the shadows, grabbing the feminine little spider boy by the shoulder and yanking him away from ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + '.  "<i>And just what the hell do you two think you\'re doing?</i>" you yell, scowling ' );
		if( CoC.player.tallness >= 90 ) {
			MainView.outputText( 'up ' );
		} else if( CoC.player.tallness <= 72 ) {
			MainView.outputText( 'down ' );
		}
		MainView.outputText( 'at your ' );
		if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] === 0 ) {
			MainView.outputText( 'daughter ' );
		} else {
			MainView.outputText( 'ward ' );
		}
		MainView.outputText( '. ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' gives a shrill cry of surprise and tumbles away from you, trying to cover herself as best she can; the spider trap squirms in your unrelenting grasp, one set of arms covering his rapidly-wilting erection as the other flails around, trying to get you off of him.  Not happening.' );
		MainView.outputText( '\n\n"<i>You!</i>" you say, pointing an accusing finger at ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + '.  "<i>What the hell, kid?  Who the fuck is this?</i>"' );
		MainView.outputText( '\n\n"<i>That\'s, uh.... Alex, meet ' );
		if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] === 0 ) {
			MainView.outputText( 'my dad' );
		} else {
			MainView.outputText( '[name]' );
		}
		MainView.outputText( '. ' + this.championRef() + ', Alex.</i>"' );
		...
		MainView.outputText( '\n\n"<i>B-but I...</i>"' );
		MainView.outputText( '\n\n"<i>' + this.championRef() + '...</i>" ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' groans as you grab the spider boy and toss him out of camp, watching with hawk-like eyes as he slinks back to his jungle.  When he\'s gone, you sigh and walk over to ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ', wiping a tear from her cheek.  Putting an arm around her shoulder, you give her a long, long talk about not being a whore.' );
		MainView.outputText( '\n\n"<i>B-but mom...</i>"' );
		MainView.outputText( '\n\n"<i>That\'s why you </i>shouldn\'t<i> be bringing men home.  Or women.  Or anything else, kiddo.' );
		//If No Hel Love:;
		if( CoC.flags[ kFLAGS.HEL_LOVE ] !== 0 ) {
			MainView.outputText( '  You don\'t want to be a whore like your mother, do you?  The girl that everyone\'s had a ride with.' );
		}
		//if HelLove:;
		if( CoC.flags[ kFLAGS.HEL_LOVE ] === 1 ) {
			MainView.outputText( '  I\'m not going to a raise a whore, ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + '.  Your mother\'s made mistakes, sure, but...</i>" you sigh.  "<i>' );
		}
		MainView.outputText( '  You don\'t have to be that person, ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + '.  Don\'t just give yourself out to the first person that asks, you know?  Save it for someone you really care about.  Trust me, it\'ll be worth it.</i>"' );
	...
	HelSpawnScene.prototype.helspawnAllGrownUp = function() {
		...
		MainView.outputText( 'like a woman.  The time to raise her, to shape her development, is over.  You can only hope you\'ve done a good enough job to keep her safe in this hellish world; to give her sound judgment and the abilities she\'ll need to protect herself in the years and adventures to come.' );
		MainView.outputText( '\n\n<b>' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' has been added to the Followers menu!</b>' );
		CoC.flags[ kFLAGS.HELSPAWN_AGE ] = 3;
		CoC.flags[ kFLAGS.HELSPAWN_GROWUP_COUNTER ] = 0;
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};

	//Helspawn’s Main Menu @ Camp [Followers Tab];
	HelSpawnScene.prototype.helspawnsMainMenu = function() {
		MainView.clearOutput();
		MainView.outputText( 'You call ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' over to you and she comes running, ' );
		if( CoC.player.tallness <= 72 ) {
			MainView.outputText( 'grabbing you off your [feet] and clutching you in a tight hug' );
		} else {
			MainView.outputText( 'throwing her arms around you and hugging you tightly' );
		}
		...
		//[Talk];
		EngineCore.addButton( 1, 'Talk', this, this.talkToHelspawn );
		//[Spar];
		EngineCore.addButton( 2, 'Spar', this, this.sparHelspawn );
		//[Sex] {?};
		//[Appearance];
		EngineCore.addButton( 8, 'Appearance', this, this.helSpawnsAppearanceScreen );
	...
	HelSpawnScene.prototype.hugHelspawn = function() {
		...
		if( CoC.player.tallness <= 72 ) {
			MainView.outputText( 'You reach up and pull ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' down to your level, wrapping your arms tight around her slender shoulders.  Grinning from ear to ear, the towering girl squeezes you right back, her powerful muscles just about crushing your smaller body.  Her tail slips around your [hips], the warm leather binding you to your beloved ' );
			if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] === 0 ) {
				MainView.outputText( 'daughter ' );
			} else {
				MainView.outputText( 'ward ' );
			}
			MainView.outputText( 'as she nuzzles up against you.' );
		...
		//{If PC is 6' or taller:};
			...
			MainView.outputText( 'You grab ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' and pull her into a tight hug.  She lets out a girlish giggle as she falls into your arms and squeezes you right back.  Her powerful arms threaten to crush your bigger body, smothering you in affection.  Her tail slips around your [leg], the warm leather binding you to your beloved ' );
			if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] === 0 ) {
				MainView.outputText( 'daughter ' );
			} else {
				MainView.outputText( 'ward ' );
			}
			MainView.outputText( ' as she nuzzles into your [chest].' );
			MainView.outputText( '\n\n"<i>Love you too, ' + this.championRef() + ',</i>" she laughs, planting a quick kiss on your cheek before letting you go.' );
		}
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	...
	HelSpawnScene.prototype.talkToHelspawn = function() {
		...
		if( CoC.flags[ kFLAGS.HAD_FIRST_HELSPAWN_TALK ] === 0 ) {
			...
			//{if Sluttymander:;
			if( CoC.flags[ kFLAGS.HELSPAWN_PERSONALITY ] >= 50 ) {
				MainView.outputText( '  Even if she wants to be just like mom, she shouldn\'t be bringing monsters home.  Some of them may be dangerous, and she\'s not a warrior quite like you and Helia yet.' );
			} else {
				MainView.outputText( '  She should know better than to bring monsters back.  She\'s not her mother; lust doesn\'t have to dominate her life like that.' );
			}
			...
			if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] === 0 ) {
				MainView.outputText( 'Of course, as her father, you could always say it\'s your duty to keep her happy...' );
			} else {
		...
		//{Kiha must be at camp};
			...
			MainView.outputText( '"<i>Oh hey, ' + this.championRef() + '!  You gotta come see what I was making!</i>"' );
		...
		//{Needs Rath at camp];
			...
			if( (SceneLib.amilyScene.amilyFollower() && !SceneLib.amilyScene.amilyCorrupt()) ) {
				MainView.outputText( 'Little Amily\'s solution was makeshift at best, and the children are beyond my reach to study.  Still.  ' );
			}
			MainView.outputText( 'Imagine whole legions of children reared and raised in the time it takes a goblin to do the same. We might have a fighting chance for once, with numbers to match the demons.</i>"' );
		...
		else if( temp <= 2 && SceneLib.milkWaifu.milkSlave() && CoC.flags[ kFLAGS.MILK_SIZE ] > 0 ) {
			MainView.outputText( '"<i>Hey, have you seen ' + CoC.flags[ kFLAGS.MILK_NAME ] + ' around anywhere, ' + this.championRef() + '?</i>" ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' asks, looking around the camp.  "<i>Mom asked me to milk her, but I haven\'t seen her around.  She wouldn\'t have run off, would she?</i>"' );
			if( CoC.flags[ kFLAGS.MILK_SIZE ] === 1 ) {
				MainView.outputText( '\n\nYou tell her it\'d be more of a wobble, but' );
			} else {
				MainView.outputText( '\n\nYou doubt she would have; ' + CoC.flags[ kFLAGS.MILK_NAME ] + ' seems to have a fancy for you, but' );
			}
		...
		//{Isabella must be at camp};
			...
			if( CoC.player.race() !== 'human' ) {
				MainView.outputText( ' and you yourself have suffered as well' );
			}
			MainView.outputText( '.  You smile as Isabella seats herself beside ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ', joining their voices together for the final chorus that leaves them both shaking as ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' strums the last, desperate notes, nearly clawing through the strings in her passion.' );
			MainView.outputText( '"<i>' );
			if( !SceneLib.isabellaFollowerScene.isabellaAccent() ) {
				MainView.outputText( 'Sehr gut' );
			} else {
				MainView.outputText( 'Very good' );
			}
			MainView.outputText( ' little ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ',</i>" Isabella says, pulling the young \'mander into a great big hug that threatens to smother her betwixt the cowgirl\'s massive bosoms.  Grinning, you congratulate ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' on her performance, showering your girl in praise before you depart, leaving her to Isabella\'s instruction.  As you walk away, you can\'t help but notice Helia standing a short ways off, rubbing her eyes.' );
			MainView.outputText( '\n\nWhen you approach, your lover smiles at you, saying, "<i>I always wanted to be a bard when I was a little girl.  I\'m... I\'m glad ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + '\'s getting the chance, at least.</i>"' );
			EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
		} else {
			MainView.outputText( 'Unfortunately, there doesn\'t seem to be anything in particular to talk about at the moment.  The two of you spend the time in companionable quiet, making smalltalk.' );
			EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
		}
	};
	//Boyfriend;
	HelSpawnScene.prototype.helSpawnBoyfriend = function() {
		MainView.clearOutput();
		MainView.outputText( '"<i>So tell me about this spider boy.</i>"' );
		MainView.outputText( '\n\n"<i>Alex?</i>" ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' asks, brightening up.  "<i>I met him on the way to Tel\'Adre.  Mom was stopping to, uh, take care of a few stray witches, and I ended up wandering off...</i>" she says, launching into the tale of her meeting the effeminate spider boy, and the whirlwind romance that brought them giggling back to camp in the middle of the night.  It\'s typical teen talk, but then, you\'re not much older than she seems, now, and you remember the days at home when you could have done the same.  You grin as she recounts her first kiss, and note the bright blush on her cheek.' );
		MainView.outputText( '\n\nMaybe she ought to keep seeing this boy after all...' );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};

	//StopFucking;
	HelSpawnScene.prototype.dontFuckAlex = function() {
		MainView.clearOutput();
		MainView.outputText( 'Stroking ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + '\'s hair, you try to tell her that there\'s a better way, that she doesn\'t have to just fuck everything she comes across.  Her mother\'s that way, sure, but she could do so much better, so much more with herself than giving in to constant lust.' );
		//{If Sluttymander:};
		if( CoC.flags[ kFLAGS.HELSPAWN_PERSONALITY ] >= 50 ) {
			MainView.outputText( '\n\n' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' scoffs, shaking her head.  "<i>But I </i>like<i> mom\'s way.  I love jilling off, the feeling of my cunt getting stretched and my ass being torn apart by the big toys I buy in town.  That\'s who I am, ' + this.championRef() + '.  I\'m like mom, and I don\'t think... no, I know I don\'t want to change.  I love sex, even if you stopped me and Alex from it.  I want to feel a gang of minotaurs raping me, I want to jump on a drider\'s cock and ride him down.  I want to do the things I heard mom doing, and that\'s just what I\'m going to do. I\'m sorry.</i>"' );
			MainView.outputText( '\n\nYou try to say something, but ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' steps away, calling back, "<i>I love you, ' + this.championRef() + ', but I can\'t be what you want me to be.  I\'m going to be a slut like mom, and there\'s nothing you can do to stop me!  It\'s who I was born and raised to be.</i>"' );
			MainView.outputText( '\n\nYou may have made a horrible mistake.' );
		}
		//{If Chastemander:};
		else {
			MainView.outputText( '\n\n' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' nods slowly, taking in your words.  "<i>I... I know, ' + this.championRef() + '.  I\'m sorry I brought Alex home last night.  It won\'t happen again.  I don\'t... I love mother, but I don\'t want to be like her.  A whore.  She\'s a great woman, but the things she does... they disgust me.</i>"' );
	...
	HelSpawnScene.prototype.incestWithHelspawn = function() {
		MainView.clearOutput();
		MainView.outputText( 'You cup ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + '\'s cheek, turning her to you... only to have her leap you, straddling your [legs] as her powerful arms wrap around your neck.  Her thin lips press hard to yours, slender tongue probing against yours as her svelte body presses against you.  You respond in kind, grabbing her big ass and squeezing, kneading the delicious curves as she starts to grind against you, breath hot and heavy on your ' + CoC.player.skinFurScales() + '.  "<i>I said I love you, ' + this.championRef() + ',</i>" she grins, so close you can practically feel the beat of her heart through her perky breasts.  "<i>Glad to know you feel the same way.</i>"' );
		...
		EngineCore.dynStats( 'lus', CoC.player.sens / 10 + 5, 'resisted', false );
		CoC.flags[ kFLAGS.HELSPAWN_INCEST ] = 1;
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	//[Oh God EW];
	HelSpawnScene.prototype.ohGodEwKihaAndHelspawnSuckAtCooking = function() {
		MainView.clearOutput();
		MainView.outputText( 'You spew the stew onto the ground and grab a nearby waterskin, trying to flush the taste from your mouth.  Kiha and ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' look on with horror as you wipe your mouth and begin to try and coherently explain just how god awful whatever that... STUFF... you just put in your mouth was.  Shock turns to anger before you’re halfway through admonishing the pair of scaly redheads.  Kiha scowls at you and snatches the ladle from you.' );
		MainView.outputText( '\n\n"<i>Oh, what the fuck do you know anyway, you big ass!  It\'s perfectly fine, isn\'t it ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + '?</i>" she growls, spooning up a mouthful of the stuff before you can stop her.  A heartbeat after she swallows, Kiha goes completely stiff; her eyes growing as wide as saucers as they water.  She collapses backwards, falling onto her back with a muted <i>THUD</i>.' );
		MainView.outputText( '\n\n"<i>K-Kiha?</i>" ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' yelps, watching the dragoness collapse into a pile of limp scales and leather.  "<i>Are you okay?  Is she okay, ' + this.championRef() + '?</i>"' );
		...
		MainView.outputText( '\n\nYou and ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' are soon working away, stirring up your own special blend of your rations and the spices she and Kiha gathered for the first attempt.  Once you\'ve got the basics in, you step back and hand the ladle over, telling the young \'mander to go wild.  Grinning like a kid, she goes to work, mixing in a wild combination of ingredients.  The cauldron\'s full to bursting by the time she\'s done, and no sooner does she ladle out the first bowel of it than Kiha sits up, groaning.' );
		MainView.outputText( '\n\n"<i>The fuck happened?</i>" the dragoness hisses, rubbing her temple.  "<i>Feel like I got hit with a hammer.</i>"' );
		MainView.outputText( '\n\n"<i>You, uh, fell off the edge, aunt Kiha.  And spilled all the stew.  Here, we remade it for you!</i>" ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' lies, presenting "aunt Kiha" with a bowl full of an aromatic brown goop, full of carrots and spices floating atop it like boats on a placid lake.  At least it smells better.' );
		...
		MainView.outputText( '\n\nYou just shake your head and grab a bowl, sitting down with the scaly ladies as you enjoy your lunch, trying to ignore the little shit-eating grin ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + '\'s sporting all the while.' );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	HelSpawnScene.prototype.umYum = function() {
		MainView.clearOutput();
		MainView.outputText( 'Um, yum?' );
		MainView.outputText( '\n\nCringing, you set the offered bowl back down, squeaking out that it\'s delicious, thank you very much, but you have something to take care of right now; maybe you can have more later.  Kiha rolls her eyes and shoos you off before she and ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' spoon out their lunch.' );
	...
	HelSpawnScene.prototype.helSpawnSureMilkHerLater = function() {
		MainView.clearOutput();
		MainView.outputText( 'You chuckle, telling her to let the poor girl sleep.  There\'ll be plenty of milk later.' );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	//Now;
	HelSpawnScene.prototype.helSpawnMilkHerNow = function() {
		MainView.clearOutput();
		MainView.outputText( '"<i>Oh, no.  You\'re not getting out of your chores that easy, kiddo.  In you go,</i>" you say, giving her a little push toward the edge of the pool.' );
		MainView.outputText( '\n\n' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' gives a rebellious huff as she clambers down, slipping down to the bottom and giving ' + CoC.flags[ kFLAGS.MILK_NAME ] + ' a gentle nudge.  "<i>C\'mon, cutey, wake up.  Time to relieve some pressure.</i>"' );
		MainView.outputText( '\n\nYawning powerfully, ' + CoC.flags[ kFLAGS.MILK_NAME ] + ' rises to her knees and rubs the sleep from her eyes' );
		if( CoC.flags[ kFLAGS.MILK_SIZE ] === 2 ) {
			MainView.outputText( ', already working to unfasten her shift' );
		}
		MainView.outputText( '.  "<i>Suckle?</i>" she asks, turning her full teats toward ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + '.' );
		//If Sluttymander:;
		if( CoC.flags[ kFLAGS.HELSPAWN_PERSONALITY ] >= 50 ) {
			MainView.outputText( '\n\n"<i>Wouldn\'t have it any other way!</i>" the salamander answers with an eager grin, easing herself into ' + CoC.flags[ kFLAGS.MILK_NAME ] + '\'s arms and wrapping her lips around one of her prominent teats. ' + CoC.flags[ kFLAGS.MILK_NAME ] + ' gives a pleasured little shudder as ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' starts to suckle like a babe, drinking down the dusky maid\'s seemingly endless supply of milk.' );
		}

		//If Chastemander:;
		else {
			MainView.outputText( '\n\n"<i>Sorry, ' + CoC.flags[ kFLAGS.MILK_NAME ] + ', you know I\'m not really into that.  Just turn around and sit back, alright?</i>"  With a sigh, ' + CoC.flags[ kFLAGS.MILK_NAME ] + ' does as she\'s asked, leaning back into ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + '\'s arms as the young salamander goes to work, clawed fingers gently caressing her prominent nubs until a white flow springs fort, splashing her knees as the dusky maid gives a little gasp of pleasure.' );
		}

		MainView.outputText( '\n\n<i>They seem to get on all right</i>, you think as you watch the pair of them.  It\'s nice to have someone else around to help keep ' + CoC.flags[ kFLAGS.MILK_NAME ] + '\'s production under control.  Poor thing just never stops lactating.' );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	//Spar;
	HelSpawnScene.prototype.sparHelspawn = function() {
		MainView.clearOutput();
		MainView.outputText( 'You ask ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' if she\'s up for some battle practice, and she answers with an eager nod as she grabs her weapon.' );
		//If Sluttymander: ;
		if( CoC.flags[ kFLAGS.HELSPAWN_PERSONALITY ] >= 50 ) {
			MainView.outputText( '\n\n"<i>Ready to get your shit kicked in, old ' + CoC.player.mf( 'man', 'lady' ) + '?</i>" she grins, drawing her weapon.' );
		} else {
	...
	HelSpawnScene.prototype.loseSparringToDaughter = function() {
		//if Sluttymander:;
		if( CoC.flags[ kFLAGS.HELSPAWN_PERSONALITY ] >= 50 ) {
			MainView.outputText( 'As you stumble back, ' );
			if( CoC.player.lust > 99 ) {
				MainView.outputText( 'succumbing to your own lusts' );
			} else {
				MainView.outputText( 'unable to withstand her unending hail of attacks' );
			}
			MainView.outputText( ', ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' quickly sweeps your [legs] out from under you, dropping you right on your ass.  You collapse with a grunt, ' );
			if( CoC.player.weaponName !== 'fists' ) {
				MainView.outputText( 'weapon tumbling out of hand' );
			} else {
				MainView.outputText( 'unable to even clench your fists anymore' );
			}
			MainView.outputText( ', and a moment later ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + '\'s on you, straddling you with her powerful legs, pushing you down into the dirt.' );
		...
		//Else If Chastemander:;
			...
			MainView.outputText( ', ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' grabs your arm, catching you before you can fall.  "<i>Easy there, ' + this.championRef() + ', I got you,</i>" she says, pulling you into a quick hug.' );
			...
			if( CoC.player.HP < 1 ) {
				MainView.outputText( 'recover from that ass kicking' );
			} else {
				MainView.outputText( 'dunk your head' );
			}
			MainView.outputText( '.' );
		}
	...
	HelSpawnScene.prototype.beatUpYourDaughter = function() {
		...
		if( CoC.flags[ kFLAGS.HELSPAWN_PERSONALITY ] >= 50 && CoC.monster.lust > 99 ) {
			MainView.outputText( '"<i>N-no more...</i>" the slutty little salamander moans, slumping down to the ground, arms wrapping around herself.  "<i>Fuck, you\'re sexy... so horny...</i>" she groans, hands slipping down to her soaked bikini bottom.' );
			...
			//If No Incest: ;
			if( CoC.flags[ kFLAGS.HELSPAWN_INCEST ] === 0 ) {
				MainView.outputText( '  You wash your hands of the defeated slut and head back to camp, leaving her to work through her tension herself.' );
			} else {
				MainView.outputText( '  You reach down and give your lovely, lusty daughter a pat on her expansive rear, telling her she\'ll always look her best with her ass in the air, begging for ' );
				if( !CoC.player.hasCock() ) {
					MainView.outputText( 'sex' );
				} else {
					MainView.outputText( 'your cock' );
				}
				MainView.outputText( '... and that if she\'s lucky, you might tend to her when you\'ve cooled off.' );
			}
		} else {
			MainView.outputText( CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' stumbles back, dropping her weapon and waving her arms, "<i>I give, I give.</i>"' );
			...
			if( CoC.monster.HP < 1 ) {
				MainView.outputText( ', rubbing her many bruises' );
			} else {
				MainView.outputText( ', blushing brightly after your slutty display' );
			}
	...
	HelSpawnScene.prototype.maiVisitsHerKids = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 68 );
		MainView.outputText( 'As you head back into camp, you notice Hel and ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' are sitting around the cook fire in the center of camp, with your foster daughter balanced precariously on the lap of her own father, Mai the fox-girl.' );
		...
		MainView.outputText( '\n\n"<i>Hi, [name],</i>" Mai says with a demure smile, arms wrapped around ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + '\'s belly, trying to hold the daughter that\'s near a foot taller than she is.  Finally, laughing, she lets the young salamander go.  ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' drops down to sit beside her mother and father, and quickly has both a smooth and scaly hand stroking her hair.' );
		//If Chastemander:;
		if( CoC.flags[ kFLAGS.HELSPAWN_PERSONALITY ] < 50 ) {
			MainView.outputText( '\n\n' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' happily leans her head against Mai\'s leg, accepting the affection with her tail swishing gaily behind her, foxy ears twitching as Mai brushes them.' );
		} else {
			MainView.outputText( '\n\n' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' leans back, pulling out a flask and taking a deep swill before passing it to her father, who takes it with a knowing smile and knocks it back like a champ.' );
		}
		...
		MainView.outputText( '\n\n"<i>Heeeeeyyyyyy,</i>" ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' whines, crossing her arms with a huff.' );
		...
		MainView.outputText( '\n\n' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' lights up as Mai slips the gold chain around her daughter\'s neck.  "<i>Something to remember me by when I\'m not around,</i>" she says with a wink, letting her kid skip off to find a mirror.' );
		MainView.outputText( '\n\n"<i>And you, [name],</i>" she says, turning to you, "<i>Thanks for taking care of ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + '.  I-I mean, I know I was really making her for you and Hel anyway, but still... she\'s a good kid, and I love her anyway.  Keep her safe, alright?</i>"' );
		MainView.outputText( '\n\nYou promise that you will, and with a quick nod, Mai runs after ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' to say goodbye for the day.  As she leaves, Helia grins, holding you tight against her.  "<i>She\'s a good girl, [name].  Couldn\'t have picked a better father for our girl.</i>"' );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	//Spider Bro's Gift;
	//{Requires Helspawn be fathered by a spiderbro. Play at morning.};
	HelSpawnScene.prototype.spiderBrosGift = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 68 );
		CoC.flags[ kFLAGS.SPIDER_BRO_GIFT ] = 1;
		MainView.outputText( 'As you\'re getting ready to head out for the day, you notice ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' prancing around camp with a long, brightly-colored scarf wrapped around her neck, standing painfully in contrast with her midnight-black scales and pale flesh.' );
		...
		if( SceneLib.isabellaFollowerScene.isabellaFollower() ) {
			MainView.outputText( ', or aunt Isabella might have knitted me a new one, I don\'t know' );
		}
		MainView.outputText( '.</i>"' );
	...
	HelSpawnScene.prototype.hakonAndKiriComeVisit = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 68 );
		CoC.flags[ kFLAGS.HAKON_AND_KIRI_VISIT ] = 1;
		MainView.outputText( 'As you\'re returning to camp, you notice Helia running around like a chicken with her head cut off, fussing about damn near everything in her part of the camp' );
		if( SceneLib.camp.companionsCount() > 1 ) {
			MainView.outputText( ' and everyone else\'s too, much to their chagrin' );
		}
		MainView.outputText( '. As she\'s furtively polishing off her giant still, trying to get the dingy old thing to shine, you approach and clear your throat. She gives a sharp yelp and spins around, but seems to relax as she sees it\'s just you.' );
		...
		MainView.outputText( '\n\nIt hardly matters, as a few minutes later, you hear a cry of unabashed glee from the edge of camp, and turn to see ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' running up to meet a pair of oncoming figures.  You wave as Hel\'s father swoops his granddaughter up into a great bear hug, hefting the half-breed \'mander off her feet in his powerful arms.  ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' squeals with delight as she\'s practically carried into camp, until Hakon sets her down and hugs her mother, pulling Hel into a tight hug before she can finish saying hello.' );
		MainView.outputText( '\n\n"<i>Hi, [name],</i>" Kiri says, fluttering over to plant a peck on your cheek.  "<i>Long time no see.</i>"' );
		MainView.outputText( '\n\n"<i>Too right.  Put \'er there, ' + CoC.player.mf( 'son', 'girl' ) + ',</i>" Hakon says, extending a hand to you, grinning wide as he lets his daughter and granddaughter go.  You shake the old salamander\'s hand, nearly wincing as his powerful grip throttles your wrist vigorously, practically walking you back into the heart of the camp.' );
		MainView.outputText( '\n\nTurning to ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ', Hakon smiles with pride.  "<i>Gods and demons, girl, you\'ve grown!  What have you two been feeding her, huh?</i>"' );
		MainView.outputText( '\n\n"<i>And you should see how strong she is!</i>" Hel grins.  "<i>Go on, ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ', show the old scales what you\'ve got.</i>"' );
		MainView.outputText( '\n\n"<i>Old scales!?  I\'ll show you old, you little brat.  Come on, hit me!</i>" Hakon says, just before ' + CoC.flags[ kFLAGS.HELSPAWN_NAME ] + ' punches him right in the chest, knocking him right on his ass.  He tumbles back with a grunt, shaking his head with a wry laugh.  "<i>Ha!  Maybe these scales are getting old after all.  This one\'s gonna be a hell of a fighter, though.  Should take her hunting with us one of these days.  What do you say, kid?</i>"' );
		MainView.outputText( '\n\n"<i>Sure!  ' );
		if( CoC.flags[ kFLAGS.HELSPAWN_WEAPON ] === 'scimitar and shield' ) {
			MainView.outputText( this.championRef() + '\'s been teaching me how to fight.' );
		} else {
			MainView.outputText( 'Mom\'s been teaching me how to fight.' );
		}
		MainView.outputText( '  I\'m not as good as mom and ' + this.championRef() + ' yet, but I\'d love to come along.</i>"' );
		MainView.outputText( '\n\n"<i>That\'s the spirit.  Now we just have to convince your aunt Kiri to come along.  And ' );
		if( CoC.flags[ kFLAGS.HELSPAWN_DADDY ] === 0 ) {
			MainView.outputText( 'your old man' );
		} else {
			MainView.outputText( '[name]' );
		}
	...
	HelSpawnScene.prototype.goHuntingBitches = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 68 );
		MainView.outputText( '"<i>Sure,</i>" you say, quickly earning approving nods from Hakon and Helia.' );
	...
	HelSpawnScene.prototype.noHuntingBitches = function() {
		MainView.clearOutput();
		MainView.spriteSelect( 68 );
		MainView.outputText( '"<i>I\'ll pass,</i>" you say with a laugh, earning a shrug from the salamanders.' );
