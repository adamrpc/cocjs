...
angular.module( 'cocjs' ).run( function( MainView, SceneLib, AppearanceDefs, CockTypesEnum, Sophie, ConsumableLib, Combat, Descriptors, CoC, kFLAGS, Utils, StatusAffects, EngineCore ) {
	...
	SophieFollowerScene.prototype.sophieFollower = function() {
		...
		if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00282 ] > 0 && CoC.flags[ kFLAGS.SOPHIE_DEBIMBOED ] > 0 ) {
			return true;
		}
	...
	SophieFollowerScene.prototype.yesDebimboSophie = function() {
		...
		MainView.outputText( '\n\n"<i>Wha... what did you DO TO ME!?!?!</i>" Sophie screams, a look of sheer feral rage splayed across her still-bimbofied face, full-lips drawn into a snarl beneath gleaming eyes and long golden hair.  Her shiny yellow feathers ruffle as she stomps up to you, putting her face right up to yours.  "<i>You ' + CoC.player.mf( 'bastard', 'bitch' ) + '!  WHAT DID YOU DO TO ME?</i>"' );
		MainView.outputText( '\n\nYou recoil at the sudden outburst of rage, mind racing for an appropriate course of action...' );
		CoC.flags[ kFLAGS.SOPHIE_DEBIMBOED ] = 1;
	...
	SophieFollowerScene.prototype.bimboSophieAgain = function() {
		...
		CoC.flags[ kFLAGS.SOPHIE_DEBIMBOED ] = 0;
	...
	SophieFollowerScene.prototype.beatSophieAroundYouMonster = function() {
		MainView.clearOutput();
	...
	SophieFollowerScene.prototype.beatUpDebimboSophie = function() {
		MainView.clearOutput();
	...
	SophieFollowerScene.prototype.debimboSophieBeatsYouUp = function() {
		MainView.clearOutput();
	...
	SophieFollowerScene.prototype.letDebimboSophieGo = function() {
		MainView.clearOutput();
		MainView.outputText( 'You decide it\'s probably best to let Sophie go.  She remains still for a long moment, before finally looking at you with her big, blue eyes, a strange mixture of loathing and wonder in them.  Without further hesitation, she turns and rushes off, back toward the High Mountains you\'d imagine.' );
		...
		if( CoC.player.lib >= 50 ) {
			...
			if( CoC.player.cor < 40 ) {
				MainView.outputText( '  You blink repeatedly, trying to clear your head of these idle thoughts, but are nonetheless aroused.' );
				//{+10 AROUSAL};
	...
	SophieFollowerScene.prototype.apologizeToDebimboSophie = function() {
		MainView.clearOutput();
		MainView.outputText( 'You decide to apologize to Sophie, saying you didn\'t mean for things to get so out of hand... you didn\'t know she\'d change quite so much!  You only thought the potion would enhance her physique, maybe make her a little more receptive to your advances.' );
	...
	SophieFollowerScene.prototype.sophieDaughterDebimboUpdate = function() {
	...
	SophieFollowerScene.prototype.followerSophieMainScreen = function() {
		...
		if( SceneLib.sophieBimbo.sophieIsInSeason() && CoC.player.hasCock() && CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_SOPHIE ] === 0 ) {
			...
			if( SceneLib.sophieBimbo.bimboSophie() ) {
				MainView.outputText( 'restoring her intellect made her quite skilled at it again' );
			} else {
				MainView.outputText( 'she\'s quite skilled at it' );
			}
			MainView.outputText( '.  "<i>Ohhh, come on, [name]!  Just give it to me like a good ' + CoC.player.mf( 'boy', 'girl' ) + ', would you?  I could even give you a special treat.  <b>Just give. It. To. Me.</b></i>"' );
			...
			if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00282 ] > 0 ) {
				MainView.outputText( 'Sophie leans back in her nest as you approach, her prominent, bare F-cups on display.  She\'s completely nude, as always, and sits with her avian legs shamelessly spread, revealing the slick slit of her womanhood framed by downy-soft platinum blonde feathers.  "<i>Hey there, ' + CoC.player.mf( 'handsome', 'cutey' ) + '.  What can Momma Sophie do for you?  Come for a cuddle in my welcoming wings, maybe; or for something more... exciting?</i>"' );
			} else {
				MainView.outputText( 'Sophie leans back in her nest as you approach, her prominant, bare DD-cups on display.  She\'s completely nude, as always, and sits with her avian legs demurely crossed, concealing the slick slit of her womanhood from you.  "<i>Hey there, ' + CoC.player.mf( 'handsome', 'cutey' ) + '.  What can Momma Sophie do for you?</i>"' );
			}
		} else {
			MainView.outputText( 'When you call her name, Sophie\'s gold-glossed lips curve upward in a smile. She takes her time picking her way out of her nest, and then glides down to you.' );
	...
	SophieFollowerScene.prototype.sendToFarm = function() {
		MainView.clearOutput();
		this.sophieSprite();
		MainView.outputText( 'You tell your live-in harpy that she is to head towards the lake, find a farm, present herself to the lady who works there and do as she says. Sophie isn’t exactly enthused by this idea.' );
	...
	SophieFollowerScene.prototype.backToCamp = function() {
		MainView.clearOutput();
		this.sophieSprite();
		MainView.outputText( 'You tell her she’s welcome to head back to camp now.' );
	...
	SophieFollowerScene.prototype.eggSelector = function() {
		for( var i = 0; i < this.eggColors.length; i++ ) {
			EngineCore.addButton( i, this.eggColors[ i ], this.postEggSelector, this.eggColors[ i ] );
		}
	};
	SophieFollowerScene.prototype.postEggSelector = function( selected ) {
		MainView.clearOutput();
		this.sophieSprite();
		MainView.outputText( '“<i>I’ll make some nice ' + selected.toLowerCase() + 'eggs for you then, hun,</i>” she says with a small grin. “<i>I can only produce one a week, though. I’ll put it with everything else you earn out of this hole in the ground. How’s that?</i>”' );
	...
	SophieFollowerScene.prototype.changeEggs = function() {
		MainView.clearOutput();
		this.sophieSprite();
		MainView.outputText( 'You ask Sophie if she will change the type of eggs that she is producing for you.' );
	...
	//Sleep With:*;
	SophieFollowerScene.prototype.sleepWithSophieToggle = function() {
		MainView.clearOutput();
		if( CoC.flags[ kFLAGS.SLEEP_WITH ] !== 'Sophie' ) {
			MainView.outputText( 'You ask Sophie if she\'d like to start sleeping with you at night.  Sophie cocks an eyebrow up at the suggestion.' );
			//DEBIMBO'ed;
			if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00282 ] > 0 ) {
				...
				MainView.outputText( 'answer that you thought it might be nice to snuggle up with her at night, not even necessarily sexually.  Just having someone to hold during the long, cold nights would be nice, and you admit that Sophie would be a rather lovely body to cuddle up against.' );
				...
				if( CoC.player.smallestCockArea() <= 5 && CoC.player.hasCock() ) {
					MainView.outputText( ', but don\'t be surprised if I go a little rough on that tiny dick of yours in the morning' );
				}
				MainView.outputText( '!</i>" she calls.' );
				if( CoC.player.smallestCockArea() <= 5 && CoC.player.hasCock() ) {
			...
			//NORMAL;
				...
				if( CoC.player.smallestCockArea() <= 5 && CoC.player.hasCock() ) {
					MainView.outputText( '  "<i>You won\'t mind me teasing your cute little dick in the mornings, will you [name]?  It\'s so cute...</i>" she muses.\n\n<b>If you don\'t want her to tease you about your small penis, you had better not sleep with her.</b>' );
				}
			}
			CoC.flags[ kFLAGS.SLEEP_WITH ] = 'Sophie';
		} else {
			//Don't Sleep WIth:*;
			MainView.outputText( 'You tell Sophie that you\'d rather not sleep with her for now.  She sighs heavily at the proclamation and says, "<i>' );
			if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00282 ] === 0 ) {
				MainView.outputText( 'Well, it is your bed.  Let me know if you want me coming back for snuggles, okay?' );
			} else {
				MainView.outputText( 'Couldn\'t handle all these curves, huh, [name]?  Don\'t fret, Momma Sophie will stick around until you can\'t stop yourself from cuddling up on her.' );
			}
	...
	SophieFollowerScene.prototype.fuckFollowerSophie = function() {
		var x = CoC.player.cockThatFits( SceneLib.sophieBimbo.sophieCapacity() );
		if( x < 0 ) {
			x = CoC.player.smallestCockIndex();
		}
		var y = CoC.player.cockThatFits2( SceneLib.sophieBimbo.sophieCapacity() );
		MainView.clearOutput();
		//In season intro;
		if( SceneLib.sophieBimbo.sophieIsInSeason() ) {
			MainView.outputText( 'Sophie pulls you into her feathery embrace, her fingertips sensually exploring every inch of your form.  Her voice coos huskily, "<i>So you want to help make a momma out of Momma Sophie, huh?</i>"  She grabs your hand and stuffs it between her luscious thighs, right into the dripping-wet delta of her mons.  "<i>Don\'t keep me waiting... you got me all... hot and bothered...</i>"' );
		}
		//Regular Intro;
		else {
			MainView.outputText( 'Sophie smirks as she saunters closer, fingertips casually exploring your body.  She coos huskily, "<i>So, you want to have a little fun with Momma Sophie, huh?</i>"  She spreads her luscious thighs apart, just enough to show you the glittering moisture on her mound.  "<i>Well, let\'s see how much of a ' + CoC.player.mf( 'man', '\'man\'' ) + ' you are...</i>"' );
		}
		//Both;
		MainView.outputText( '  You strip out of your [armor] without a second thought and present your ' + Descriptors.cockDescript( x ) + ' to Sophie.' );
		//Small;
		if( CoC.player.cockArea( x ) <= 5 ) {
			MainView.outputText( '\n\nShe snickers, "<i>That little thing is going to fuck me?  ' + CoC.player.mf( 'I thought you were a man, but that looks like a little girl-clit!', 'I know you\'re a herm, but maybe you should just turn that little thing into a proper clit!' ) + '</i>"  Prodding your girlish member, she comments, "<i>It\'s kind of cute in its own way, though I don\'t think I\'ll feel much.</i>"  Her fingers tickle along the underside of it as she giggles, "<i>I could just play with this dainty girl-cock all day long...</i>"' );
		}
		//Medium;
		else if( CoC.player.cockArea( x ) <= SceneLib.sophieBimbo.sophieCapacity() / 2 ) {
		...
		MainView.outputText( '\n\nSophie gives your ' + Descriptors.cockDescript( x ) + ' an appreciative swat before pirouetting around, bent over with her hands on her knees and her butt up in the air.  She wiggles back and forth, her lasciviously luscious backside jiggling, the cheeks clapping audibly at the apex of each sway.  Her voice purrs, "<i>Come and get it, if you think you\'re ' + CoC.player.mf( 'man', 'woman' ) + ' enough!</i>"' );
		...
		MainView.outputText( ', her words still affect you.  Her pussy just feels... so... exquisite.  It\'s so hot and wet that you almost feel like you could cum right now.  Your [legs] wobble, but Sophie just pushes back even harder, holding you up by your ' + Descriptors.cockDescript( x ) + ' and the friction of your [butt] on the boulder.  The harpy watches your expression as her hips continue their slow roll, feeling your fingers twitch and knead her immense backside when the pleasure begins to mount.  You try to stay composed, but it\'s just so gods-damned good!  Her silken slit is so warm, wet, and just plain... erotic.  Slowly, the point where you would normally orgasm approaches, but instead of the spasming clenches that signal release, your jizz stays bottled up inside you tighter than sperm in a goblin\'s womb.' );
		MainView.outputText( '\n\nSophie begins to grunt and move more forcefully.  Her slit batters hard up against you as she cries, "<i>Fuck yes!  Oooh, yeah that feels good.  You\'re a good ' + CoC.player.mf( 'boy', 'girl' ) + ', not cumming just yet.</i>"  The compelling tone returns.  "<i>Massage my butt and ' );
		if( y >= 0 ) {
			MainView.outputText( 'fuck it' );
		} else {
			MainView.outputText( 'finger my asshole' );
		}
		MainView.outputText( ' while I fuck you, [name]!</i>" she commands, fixing you with her eyes authoritatively.  ' );
		if( y >= 0 ) {
			MainView.outputText( 'You shift to bring your ' + Descriptors.cockDescript( y ) + ' up in place, just in time for Sophie\'s wiggling rectum to spread around it.  It slides on in without so much as a whimper.' );
		} else {
			MainView.outputText( 'You thumb at her asshole hesitantly, right up until her wiggling rectum presses on it, easily dilating around your invading finger.' );
		}
		MainView.outputText( '  You squeeze tighter and begin to massage without knowing why, actually able to feel yourself through her thin, internal walls.' );
		...
		if( y >= 0 ) {
			MainView.outputText( ' and anal walls' );
		}
		MainView.outputText( '.  Your fingers dig into her plush butt, no longer kneading, but instead curling possessively, claiming absolute ownership of that supple behind and the rapidly-filling cunt below.' );
		if( y >= 0 ) {
			MainView.outputText( '  You pack her intestines full of cum at the same time that you sperm up her clingy cunny tunnel.' );
		}
		if( SceneLib.sophieBimbo.sophieIsInSeason() ) {
			MainView.outputText( '  Her pussy seems to drink the spunky deposit down, her cervix opening to allow in as much in as possible.  You can feel the suction, and if you weren\'t already riding out a mind-bending orgasm, you\'re sure you\'d be cumming harder than normal right now.' );
		}
		MainView.outputText( '  Your body slacks over her, the only muscles that are actually working are the ones spasming in your loins, pumping out fresh batches of sperm.  You whimper with primal pleasure, spittle frothing at the corners of your mouth.' );
		if( CoC.player.cumQ() >= 500 ) {
			MainView.outputText( '  Sophie\'s belly ' );
			if( CoC.player.cumQ() < 1000 ) {
				MainView.outputText( 'has begun to inflate' );
			} else {
				MainView.outputText( 'has inflated to an absurd degree' );
			}
			MainView.outputText( ' due to all the spooge you\'re squirting' );
			if( CoC.player.cumQ() >= 4000 ) {
				MainView.outputText( ' and it\'s still growing larger' );
			}
			MainView.outputText( '.' );
		}
		if( CoC.player.cumQ() >= 5000 || (CoC.player.cumQ() >= 500 && SceneLib.sophieBimbo.sophieIsInSeason()) ) {
			MainView.outputText( '  A huge wash of white rushes out from her well-sexed fuck-hole, soaking your legs with the mixed cum.' );
		}
		MainView.outputText( '  Then, you black out, with your ' + Descriptors.cockDescript( x ) );
		if( y >= 0 ) {
			MainView.outputText( ' and ' + Descriptors.cockDescript( y ) );
		}
		MainView.outputText( ' still pulsing in ecstasy.  It was such a good cum...' );
		CoC.player.orgasm();
	...
	SophieFollowerScene.prototype.sophieVagFollowerFollowup = function() {
		...
		MainView.outputText( 'You\'re roused a few moments later by a smiling Sophie, the proof of your union dripping between her thighs.  She quips, "<i>' );
		if( SceneLib.sophieBimbo.sophieIsInSeason() ) {
			MainView.outputText( 'If that didn\'t take, I don\'t know what will!' );
		} else if( CoC.player.cockArea( x ) <= 5 ) {
			MainView.outputText( 'Not too bad for a tiny girl-dick!' );
		} else {
			MainView.outputText( 'It\'s always better when you have to wait for it, isn\'t it?' );
		}
		MainView.outputText( '</i>"' );
	...
	SophieFollowerScene.prototype.sophieFollowerGivesBlowjobs = function() {
		...
		MainView.outputText( 'Sophie cocks her head to the side and chuckles, "<i>You wanna feel Momma Sophie\'s lips around that ' );
		if( CoC.player.cockArea( x ) <= 5 ) {
			MainView.outputText( 'tiny clit you call a penis' );
		} else if( CoC.player.cockArea( x ) <= SceneLib.sophieBimbo.sophieCapacity() / 2 ) {
			MainView.outputText( 'needy boy-cock' );
		} else {
			MainView.outputText( 'over-swollen engorgement you call a cock' );
		}
		MainView.outputText( '?</i>"  She saunters up with her hips swaying seductively, her juicy mound squelching noisily, webs of lubricant dangling between her thighs whenever they spread.  The harpy\'s fingertip punches into your chest' );
		if( CoC.player.biggestTitSize() >= 1 ) {
			MainView.outputText( ', right between your [chest]' );
		}
		MainView.outputText( ', and she slowly trails it up to your chin, which she gently grabs in her long-nailed fingers.  "<i>' );
		if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00282 ] > 0 ) {
			MainView.outputText( 'You may have given me a bimbo\'s body and an insatiable need for sex,' );
		} else {
			MainView.outputText( 'You may have seduced me into your camp with hot sex,' );
		}
	...
	SophieFollowerScene.prototype.sophieBlowsSixtyNine = function() {
		...
		MainView.clearOutput();
		MainView.outputText( 'You nod eagerly, something that gets the busty harpy smiling as her whole body flushes.  She rushes at you, leaping and flapping, speeding towards you faster as old feathers kick up in a wake behind her.  You spread your arms and let her slam into you, the impact cushioned by her weighty breasts and large, soft thighs.  Both of you roll end over end through the dirt, eventually coming to rest with her straddling your [chest] and rubbing a finger through her slippery folds.  She adds another finger to spread them wide, the pink interior pulsing wetly and dripping with female moisture.' );
		...
		MainView.outputText( ' mounting higher and higher.  She easily pulls [eachCock] from your constraining [armor], testing the girth and rigidity of the swollen shaft' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 's, searching for her target of the day.  At last, she settles on your ' + Descriptors.cockDescript( x ) + ', giving it an affectionate squeeze' );
		} else {
			MainView.outputText( ', giving it an affectionate squeeze' );
		}
		MainView.outputText( '.  She exhales on it with breath so hot and moist you almost think the blowjob has begun, but when her lips truly meet your ' + CoC.player.cockHead( x ) + ', that notion is shattered by the slippery-sweet embrace.  With your senses so deprived by snatch, smothered in quim, the touch of Sophie\'s golden cock-pillows on you seems multiplied seven-fold.  Your back arches instinctively, trying to lift your [hips] higher, but the matron\'s gentle, firm hand pushes you back down.  Her tongue flicks out to dance along the underside of your urethra while her cushiony mouth slips ever-lower.' );
		...
		if( CoC.player.tongueType > AppearanceDefs.TONUGE_HUMAN && CoC.player.tongueType <= AppearanceDefs.TONUGE_DRACONIC ) {
			MainView.outputText( 'Longer and longer, your tongue continually unspools from your mouth, plunging deeper into the slippery channel with each passing moment.  Her taste saturates your many taste buds, and the velvety folds of her interior feel wonderful as you repeatedly slither your inhuman muscle through that syrupy passage.' );
		} else {
			MainView.outputText( 'Your tongue slides right in, the harpy\'s pussy seeming almost like a second home.  Her taste saturates your taste buds as you plunge in as deeply as possible, gliding over her velvet folds as it goes.' );
		}
		MainView.outputText( '  Sophie\'s immense thighs squeeze tighter in response, shutting out any last vestiges of light and pushing you harder into her sopping-wet mound.  You let your lower lip graze her rigid clitty and savor the lubricating fluid that she leaks into your waiting mouth.' );
		MainView.outputText( '\n\nHer tongue and yours seem to be the only things that matter anymore.  Hers circles the entirety of your ' + Descriptors.cockDescript( x ) + ', accompanied by golden lips that leave behind their drugged, amber residue.  The enjoyable tingling that those chemicals induce is driving you wild with desire.  Your heart is hammering like crazy in your chest, each thrumming beat reinforcing the thickness of your erection in the bird-woman\'s mouth.' );
		if( CoC.player.hasVagina() ) {
			MainView.outputText( '  She tenderly' );
			if( CoC.player.balls > 0 ) {
				MainView.outputText( ' reaches under your [balls] and' );
			}
			MainView.outputText( ' explores your [vagina] with two fingers, her thumb brushing across your [clit] every few seconds.  It seems the harpy is considerate of even your feminine needs - ones that serve to make you even harder, so full it almost hurts.' );
		}
		//else;
		else if( CoC.player.balls > 0 ) {
			MainView.outputText( '  She tenderly caresses your [sack], allowing her fingers to dance over your [balls].  It seems the harpy is considerate of all your needs in a way that makes you even harder, so full it almost hurts.' );
		} else {
			MainView.outputText( 'She tenderly strokes a long nail across the sensitive skin of your taint.  It seems she\'s being very considerate to your pleasure, so considerate she has you so hard and full that it almost hurts.' );
		}
		MainView.outputText( '  Her feathery hair dangles over your [legs], tickling you as she noisily slurps and sucks, while you moan directly into her bare-skinned twat.  The vibrations set her dripping even more fiercely than before.  You do your best to dutifully drink it down.' );
		MainView.outputText( '\n\nSophie hums happily as you begin to drip directly onto her tongue, the pre-cum bubbling out of your spit-polished cum-slit in a steady flow.  Though you know just what is plying your ' + Descriptors.cockDescript( x ) + ' with slices of heavenly sensation, your gloss-drugged mind begins to slip into fantasy anyway.  Just like in the real world, Sophie\'s cunt is slathering your [face] with her fragrant arousal, and your pole is smothered with amber aphrodisiacs.  In the fantasy though, there\'s a pussy squeezing down on you and a second set of thick harpy thighs clenching around your waist.  Those fertile, egg-bearing thighs are bouncing up and down your length, and the twat above you is ripe with arousal, bounded above by a belly swollen with incubating life that will someday be your daughter.  Every morning you\'re sucked off to completion by one of your girls and kissed until your lips and rod are slathered to an amber shine.  Then you lie back and partake in the fruits of your position as ' + CoC.player.mf( 'king', 'breeder-queen' ) + ' of the harpies: creampie after creampie, tangy harpy-cunt to lick, and skilled fellatio every single time your lust begins to flag.  You groan into the slit above, your hands grabbing hold of the juicy haunches and pulling them down into you.' );
		...
		if( CoC.player.cockArea( x ) <= 60 ) {
			MainView.outputText( 'to the [sheath]' );
		} else {
			MainView.outputText( 'as far as she can, gagging slightly' );
		}
		MainView.outputText( '.  Her thighs begin to tremble around you, hard enough that you begin to worry that she might somehow crush your head, yet it\'s more like being smothered with soft pads than anything truly worrisome.  You shudder in excitement of your own and begin to flick her clit, eager to make her share in the pleasure.' );
		...
		if( CoC.player.cumQ() >= 1000 || CoC.player.cockTotal() > 1 ) {
			MainView.outputText( '  Warm goo covers your [hips] and [butt] once your virile deposit becomes too much for Sophie to handle.' );
		}
		if( CoC.player.cumQ() >= 500 ) {
			MainView.outputText( '  She quickly gives up on trying to swallow it after she starts showing some cum-pudge, but her hand pumps you energetically as she milks your ' + Descriptors.cockDescript( x ) + ', squeezing the voluminous ropes of cum all over your [chest].' );
		}
		if( CoC.player.cumQ() >= 3000 ) {
			MainView.outputText( '  You make a big white puddle with all the drippings, sinking into the cummy mud as you eat out the orgasmic harpy matron.' );
		}
		...
		CoC.flags[ kFLAGS.SOPHIE_FOLLOWER_IRRITATION ] = 0;
		//Slimefeed;
	...
	SophieFollowerScene.prototype.forceSophieBlowjob = function() {
		...
		CoC.flags[ kFLAGS.SOPHIE_FOLLOWER_IRRITATION ]++;
		MainView.outputText( 'You grab hold of her and push her down on her knees, a task aided by Sophie\'s utter, uncomprehending surprise.  She hits the ground, pouting, her thick thighs flexing to try and help her rise.  You scold her - if she didn\'t want to suck your dick, she shouldn\'t have moved into your camp.  Instead, she should be glad that you decided to ALLOW her to stay in your camp for a simple blowjob.  Pointing out her oozing snatch, you remind her that she\'s clearly enjoying this as much as you, and that she ought to just get to work and tend to her own needs while she satisfies you.' );
		MainView.outputText( '\n\nSophie grumbles, but one of her hands does nestle itself down between her feathery thighs, her other going to your groin to fish out your ' + Descriptors.cockDescript( x ) + '.  She looks up at you through angry eyebrows softened slightly by concealed lust.  "<i>I\'ll suck you off... this time,</i>" she declares, ' );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( 'fondling your [balls] with almost eager attention,' );
		} else {
			MainView.outputText( 'rubbing your length with almost eager attention,' );
		}
		MainView.outputText( ' "<i>but don\'t make a habit of this, or you\'ll wake up alone.</i>"  Sophie\'s tongue flicks across your ' + CoC.player.cockHead( x ) + ' experimentally.  "<i>At least you taste good.</i>"  The slippery muscle slides across your sensitive dick-skin again, longer this time, lingering on the underside as the golden lips edge closer.' );
		MainView.outputText( '\n\nImpatient, you slide your fingers through her feathery locks and edge her closer.  Her shining, amber mouth bumps into your ' + CoC.player.cockHead( x ) + ', followed by a surprised chirp from the unresisting harpy.  At that first touch, the electric tingle of her glossy gold lipstick hits you, causing your heart to beat faster and your ' + Descriptors.cockDescript( x ) + ' to thicken slightly with arousal in her hand.  You involuntarily push harder without meaning to, spreading Sophie\'s plump cock-kissers around the thickening meat, her spit slicking the slow, steady journey into her mouth.  Like magic, she begins to push forward on her own, almost pulling out of your grip in her eagerness.  Her mouth smoothly slides down ' );
		if( CoC.player.cockArea( x ) <= 60 ) {
			MainView.outputText( 'as far as it can go, even taking you part way into her throat before she runs out of room' );
		} else {
			MainView.outputText( 'the whole way to your base, inch after inch of dick-flesh vanishing into her pliant throat' );
		}
		MainView.outputText( '.' );
		MainView.outputText( '\n\nWith her snug, gold lips clinging around your rod, Sophie looks up with lust-lidded eyes, the noisy squelching that\'s coming from below leaving no doubt in your mind as to her state.  The burning, lust-fueling make-up that girds her cock-pillows seems to be seeping into you in greater and greater quantities.  It makes your whole body feel flush, burning-hot and ready to fuck her sweet, feathery behind.  You prepare to pull out, until you see those plush golden lips locked in place, sliding with slippery friction along every single inch of your ' + Descriptors.cockDescript( x ) + '.  You throb powerfully at that sight and flex inside her oral embrace, causing the harpy to churn out a delighted giggle, interrupted by a pleasured moan as she masturbates beneath your maleness.' );
		if( CoC.player.cockTotal() === 2 ) {
			MainView.outputText( '  Your extra cock flops against her cheek, catching on her ear and slowly leaking pre-cum into her down-soft hair.' );
		} else if( CoC.player.cockTotal() > 2 ) {
			MainView.outputText( '  Your extra cocks flop against her cheeks, catching on her ear and shoulder and slowly leaking pre-cum into her down-soft hair.' );
		}
		MainView.outputText( '\n\nSophie abruptly grabs hold of one of her big, motherly tits and gropes herself passionately.  Her lips open just enough to separate from your ' + Descriptors.cockDescript( x ) + ' and exude a lascivious moan as she masturbates herself.  The quivering tongue tickling at your underside is utterly enjoyable, a fact enhanced by the golden gloss now smeared all over your rod.  There\'s a tingling, pulsating quickening inside you.' );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( '  Your [balls] feel almost like they\'re filling up, taut with liquid weight that begs to pump out' );
		} else {
			MainView.outputText( '  It feels tight, like you\'re filling up with a warm liquid weight that\'s just begging to be let out' );
		}
		MainView.outputText( ' all over the motherly harpy\'s face.  You groan out loud with sheer excitement and fixate your eyes on Sophie\'s bodacious rump and thick thighs, partly obscured by her trembling tailfeathers.' );
		MainView.outputText( '\n\nSlobber and drool drip down' );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( ' your [sack] and over' );
		}
		MainView.outputText( ' your legs.  Sophie\'s eyes roll back slightly - not all the way but enough to proudly display her penultimate pleasure as she cums.  Her syrupy girl-cum splatters off the waste-land earth below, though the sound changes to a wetter, muddy splashing after a few seconds.  Squeezing tightly, the harpy\'s throat convulses around you.  Her lips lock tight as she hums in ecstasy, languidly beginning to suckle your ' + Descriptors.cockDescript( x ) + ' as her own orgasm dies down.  You force her to move faster, pushing her head to pop back and forth, too eager for release to care for her comfort.  Judging by the steady, squelching pumping that begins anew, she doesn\'t care that much either.' );
		MainView.outputText( '\n\nSophie suddenly compresses her gold-lacquered cock-pillows around you and sucks harder.  She flicks her eyes up at you to beg for your cum with a sluttier expression than you thought the harpy matron capable, inaudibly demanding you release inside her.  Your [hips] lurch forward to pound your ' + Descriptors.cockDescript( x ) + ' deep into her face, and your ' + CoC.player.cockHead( x ) + ' tingles, expanding in orgasm.' );
		if( CoC.player.hasKnot( x ) ) {
			MainView.outputText( '  The knot inflates just behind her lips, a golden orb that entraps you within her slick mouth and makes sure all your salty cremè is going to go down her throat.' );
		}
		...
		if( CoC.player.cumQ() >= 500 ) {
			MainView.outputText( '  She gurgles noisily when her midriff begins to bulge from your liquid deposit' );
			if( CoC.player.cumQ() >= 2000 ) {
				MainView.outputText( ', but that\'s just the beginning.  You bloat her tummy to an obscene, almost spherical shape' );
			}
			if( CoC.player.cumQ() >= 4000 ) {
				MainView.outputText( ', and then cum some more, the flooding jizz forced to escape from her nostrils after you finish flooding every inch of her digestive system' );
			}
			MainView.outputText( '.' );
		}
		MainView.outputText( '\n\nAlmost regretfully, you ' );
		if( CoC.player.cocks[ x ].cockType !== CockTypesEnum.DOG ) {
			MainView.outputText( 'slip your spasming boner out of her golden gates, dripping the last few drops onto her pouty lower-lip.' );
		} else {
			MainView.outputText( 'pull against Sophie\'s cock-locked mouth, but your knot won\'t budge.  You sit there waiting for it to go down for what seems like forever, and her lipstick doesn\'t exactly help - it seems like your cum continues to flow and dribble the entire time, your animal-dong intent on inseminating the harpy\'s mouth.  Still, you manage to \'pop\' free eventually, even though it stretches Sophie\'s jaw and echoes around your camp.' );
		}
		...
		if( CoC.flags[ kFLAGS.SOPHIE_FOLLOWER_IRRITATION ] === 1 ) {
			MainView.outputText( 'Ahhh... not... not as much as... I would if you\'d reciprocate!' );
		} else if( CoC.flags[ kFLAGS.SOPHIE_FOLLOWER_IRRITATION ] < 4 ) {
			MainView.outputText( 'Ahhh... I... I don\'t know how many times I\'ll put up with this kind of treatment!' );
		} else if( CoC.flags[ kFLAGS.SOPHIE_FOLLOWER_IRRITATION ] < 8 ) {
			MainView.outputText( 'Ahhh... are... are you ever going to reciprocate?' );
		} else if( CoC.flags[ kFLAGS.SOPHIE_FOLLOWER_IRRITATION ] === 8 ) {
			MainView.outputText( 'Ahhh... I think... I\'ve had enough...' );
		} else {
			MainView.outputText( 'Ahhh... actually... I think... <b>I think if you don\'t treat me nicer, I might leave.</b>' );
		}
		MainView.outputText( '</i>"' );
	...
	SophieFollowerScene.prototype.sophieSmallDongTeases = function() {
		...
		MainView.outputText( ' before lavishing its slickness along the underside.  You get so stiff - so hard - in just seconds, ' + Descriptors.cockDescript( x ) + ' swelling to try and fill the slick prison, but no matter how hard you get, that compressing cavern still seems so much larger and more capable than the small penis that it has to contort itself around.' );
		MainView.outputText( '\n\nAbruptly, your lusty erection is removed from the heated embrace and left cold and wanting.  You shiver in your bed, still not quite woken, at least not until a playful flick slaps the ' + Descriptors.cockDescript( x ) + ' as it pulsates, just hard enough to be noticed without pain.  You start to jolt upright, but are immediately pushed back down by a taloned footclaw pressing down on your shoulder.  You catch a view of feathers and gigantic breasts - it must be the harpy matron, Sophie, come to play with you in the morning.' );
		if( CoC.player.str >= 70 ) {
			MainView.outputText( '  You could force her off you if you wanted, but you didn\'t bring her to bed for nothing.' );
		}
		MainView.outputText( '  Her fluffy behind crashes down on your [legs], just below your waist.  With a familiar coo, she places her other foot on your chest' );
		if( CoC.player.breastRows[ 0 ].breastRating >= 1 ) {
			MainView.outputText( ' and squeezes your ' + CoC.player.breastCup( 0 ) + ' breast roughy.' );
		} else {
			MainView.outputText( ' and lets the talons dig into your ' + CoC.player.skinFurScales() + ' just enough to hurt a little.' );
		}

		MainView.outputText( '\n\nSophie licks her shining lips as she looks down at the little spit-soaked pole that\'s twitching between her legs, just inches away from her pussy.  "<i>Oooh, does Momma Sophie\'s favorite girl' + CoC.player.mf( '-boy', 'y-girl' ) + ' want her little bitch-stick played with?</i>"  She touches it, right below the [cockHead smallest], her warm fingertip sliding in slow, sensuous circles until you begin to twitch at her touches.  "<i>You know what I think?</i>" she purrs, bending over to look you face to face- an amazing feat of flexibility considering her foot is locked on your shoulder.  "<i>I think you like it when I play with this tiny girl-cock.  I think some depraved, sick, nasty part of your brain makes you cum harder when I tease you about how feminine this bitch-cock is.</i>"' );
		...
		if( CoC.flags[ kFLAGS.TIMES_MORNING_SOPHIE_FEMDOMMED ] > 0 ) {
			MainView.outputText( 'In the back of your mind, you know that if her words weren\'t true you wouldn\'t have kept sleeping with her and letting her do this to you.' );
		} else {
			MainView.outputText( 'In the back of your mind, you almost have to wonder if it\'s true, or if her lipstick is just now getting into your system...' );
		}
		...
		//{Low Sensitivity};
		if( CoC.player.sens / 5 + Utils.rand( 20 ) + 1 <= 20 ) {
			...
			if( CoC.player.pregnancyIncubation > 0 && CoC.player.pregnancyIncubation < 150 ) {
				...
				MainView.outputText( 'toned ' );
			}
			MainView.outputText( 'belly, most of it deflecting around and over her moist lips in a spray of syrupy white.' );
			...
			if( CoC.player.cumQ() >= 500 ) {
				MainView.outputText( '  You cum so much that both you and Sophie wind up' );
				if( CoC.player.cumQ() < 1000 ) {
					MainView.outputText( ' with huge wet spots over your bodies' );
				} else {
					MainView.outputText( ' soaked from the waist down' );
				}
				MainView.outputText( '.  It feels so good that neither of you mind.' );
			}
			if( CoC.player.cumQ() >= 2000 ) {
				MainView.outputText( '  Your bed is a reflective, sticky white as well, practically a lake of seed by now.' );
			}
		}
		//{High Sensitivity};
		else {
			...
			if( CoC.player.pregnancyIncubation > 0 && CoC.player.pregnancyIncubation < 150 ) {
				MainView.outputText( ', rolling off the pregnant dome you call your tummy' );
			}
			MainView.outputText( '.' );
			if( CoC.player.cumQ() >= 500 ) {
				MainView.outputText( '  You cum for what seems like hours, emptying your [balls] onto yourself, every drop of wasted seed a testament to your pussy-whipped status.' );
			}
			if( CoC.player.cumQ() >= 1000 ) {
				MainView.outputText( '  A puddle of jizz forms around you before long, mixing with the drippings from her pussy as she rides you into your own soaked bedding.' );
			}
			MainView.outputText( '  When you\'re about empty, Sophie says, "<i>Spurt all that nasty boy-cum out for me, then you can be my lady-pet.</i>"  Your orgasm redoubles, surging to lance out several more long, thick dribbles of pleasure onto your soaked middle.' );
		...
		//{BOTH END HERE};
		MainView.outputText( '\n\nThe older harpy giggles, when it ends.  "<i>I love sensitive ' + CoC.player.mf( 'lady-boys', 'girls' ) + ' like you...  You let me feel so powerful...</i>"' );
		...
		MainView.outputText( 'mess you made, wondering why you enjoyed that so much.' );
	...
	//Similar to bimbo sophie;
	SophieFollowerScene.prototype.sophieFollowerGoesIntoSeas = function() {
	...
	SophieFollowerScene.prototype.sophieFertilityExpired = function() {
	...
	SophieFollowerScene.prototype.sophieFertilityKnockedUpExpired = function() {
	...
	SophieFollowerScene.prototype.sophieSpecial = function() {
		...
		MainView.outputText( '"<i>Ahhh, so you do want a bit of Momma Sophie\'s special treatment... such a brave ' + CoC.player.mf( 'boy', 'girl' ) + ', aren\'t you?</i>" She leans in closer as she starts to get rid of your [armor].  "<i>Very brave... we might have to put you back into place.  Momma Sophie will milk this ' + Descriptors.cockDescript( x ) + ' until the only thing that even allows you to cum anytime in the future will be the fact I drugged you with my lipstick.</i>"' );
		MainView.outputText( '\n\nSlowly, she sinks to her knees, fishing out your ' + Descriptors.cockDescript( x ) + ' from your [armor], licking her lips eagerly.  "<i>First, we cover it with a lot of my special lipstick, so you\'ll stay hard no matter what.  That, and, well, so you can\'t resist when Momma Sophie wants you to squirt.</i>"' );
		MainView.outputText( '\n\nIndeed, her plush golden lips wrap about your ' + Descriptors.cockDescript( x ) + ', slathering it with both her saliva and lipstick as she bobs her head up and down on your member, causing it to become painfully hard as you thrust your [hips] instinctively, seeking more of this delicious mouth.  She grumbles, displeased at you moving and tries to hold you in place by grabbing your [ass] and trying to hold you in place; however, this proves to be rather uneffective as you begin to shake from her oral assault, her tongue licking up and down your ' + Descriptors.cockDescript( x ) + ', covering it completely in a mix of slutty spit and drugged lipstick... as a matter of fact, you feel light in the head' );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( ', and your [balls] are just waiting to unload a virile, sticky package down her throat' );
		}
		MainView.outputText( '.  Sophie seems to sense it and chuckles, pulling her mouth off your ' + Descriptors.cockDescript( x ) + ', causing you to whine as you seek out her mouth again.  "<i>There, there, my little cum pump.  Momma Sophie wouldn\'t be so mean as to leave you and your ' );
		if( CoC.player.cockArea( x ) <= 5 ) {
			MainView.outputText( 'squirting clitty' );
		} else if( CoC.player.cockArea( x ) <= 30 ) {
			MainView.outputText( 'needy boy-cock' );
		} else {
			MainView.outputText( 'giant piece of a meat rod' );
		}
		MainView.outputText( ' without blowing a nice big load. It just can\'t go in my mouth... and you should be nice and compliant, not doing things on your own.</i>"' );
		MainView.outputText( '\n\nYou feel Sophie simultanously push at your [chest] and pull at your [legs], causing you to slowly move down on your back, as you realize your [armor] has apparently disappeared in the meantime.  The confident harpy matron smirks and giggles while stepping over your body and showing you her plump, jiggly ass as her wet pussy lowers towards your ' + Descriptors.cockDescript( x ) + ', eager to devour it.  With a wet squelch, you\'re buried inside her and shuddering beneath her big, wobbly ass.  "<i>My, my... you really can\'t hold it in at all, can you? I\'ve barely even put you inside me and you\'re already so eager to paint my walls white that you\'re simply going to squirt straight away?</i>"' );
		...
		if( CoC.player.balls > 0 ) {
			MainView.outputText( '[balls] and ' );
		}
		MainView.outputText( Descriptors.cockDescript( x ) + ' inside Sophie\'s schlong-milking passage, causing her to moan in satisfaction as the first of your virile cum hits her inner walls.' );
		if( CoC.player.cumQ() < 250 ) {
			MainView.outputText( '  She milks you for all you\'re worth.' );
		} else if( CoC.player.cumQ() < 750 ) {
			MainView.outputText( '  She whimpers at how much you\'ve discharged into her.' );
		} else if( CoC.player.cumQ() <= 1500 ) {
			MainView.outputText( '  Your cum soon gushes out into a small puddle from her pussy and onto your [hips].' );
		} else {
			MainView.outputText( '  Her ass is suddenly pushed out more towards you, cum gushing out of her wet cunt as she starts inflating from the sheer amount of cum in your first load.' );
		}

		MainView.outputText( '\n\n"<i>Good ' + CoC.player.mf( 'boy', 'girl' ) + ' with a nice, obedient ' );
		if( CoC.player.cockArea( x ) <= 5 ) {
			MainView.outputText( 'clitty' );
		} else if( CoC.player.cockArea( x ) <= 30 ) {
			MainView.outputText( 'pretty cock' );
		} else {
			MainView.outputText( 'humongous cunt-stretcher' );
		}
		MainView.outputText( '... all for me... but you can\'t go soft, can you?  You just want to squirt more of your virile cum into Momma Sophie.</i>"  Sophie rolls her hips atop of you, her jizz and girl-cum slickened ass hitting against your pelvis as she ruthlessly uses your overly sensitive ' + Descriptors.cockDescript( x ) + ', going from the tip to the base as she bounces atop of you.  The drugged lipstick and her constant, magic-powered demands don\'t allow you to go soft even for a moment as she rides you to completion, her hand working between her thighs at her own clit all the while when finally her pussy grips hard at you, causing you to thrust your hips and moan from sheer sensitivity, unable to achieve another orgasm so quickly.' );
		MainView.outputText( '\n\nWith a grin, Sophie rises from your ' + Descriptors.cockDescript( x ) + ', ' );
		if( CoC.player.cumQ() < 250 ) {
			MainView.outputText( 'her pussy dripping your cum down onto the ground' );
		} else {
			MainView.outputText( 'your own semen drowning your [hips] and [butt] in a messy cum-puddle as it drips out of her' );
		}
		MainView.outputText( ', and turns towards you, jiggling her tits right above your [face] as she bends over you, asking, "<i>Now, wasn\'t that fun, [name]?  You think we should end now, shouldn\'t we?  Or do you want to give momma Sophie more of your thick cum so she\'s definitely pregnant twice over?</i>"  Drugged and fucked to the edge of your sense of self-worth, you practically beg her to put you inside her again, your golden-tinted member standing proudly in the air, waiting for her hips and pussy to descend.  "<i>Good answer, very good answer.</i>"  She descends upon you again, rubbing her pussy against the tip of your ' + Descriptors.cockDescript( x ) + ', teasing both herself and you before drawing you inside and hilting you down to your [sheath] into her!' );
		...
		if( CoC.player.cockArea( x ) <= 5 ) {
			MainView.outputText( 'dainty little fem-cock' );
		} else if( CoC.player.cockArea( x ) <= 30 ) {
			MainView.outputText( 'nice, sweet cum-squirter' );
		} else {
			MainView.outputText( 'immense, gorgeous beast of a cock' );
		}
		MainView.outputText( ' until it\'s almost broken!  Come on you sweet thing, you know you want to squirt it in me like you\'re a broken faucet!</i>"' );
		MainView.outputText( '\n\nWith a grunt, you manage to withhold from her command, and Sophie coos with delight at your resistance.  "<i>Look at my ' + CoC.player.mf( 'boy', 'girl' ) + ', all grown up now, resisting the pleasures of Momma Sophie\'s fertile, cum-thirsty cunny!  You shouldn\'t do that... though, maybe we could cum together if you do... Okay, baby, once you feel Momma Sophie\'s pussy squeeze you extra hard, you will cum and you won\'t try to hold back, okay?</i>"  You grunt and nod, but already find it hard to hold back when you imagine how Sophie will look knocked up with your kids. Once you cum inside her a second time, there\'s probably no way it won\'t hold.' );
		MainView.outputText( '\n\nYou hear Sophie\'s shlicking vagina swallow your entire ' + Descriptors.cockDescript( x ) + ' and release it repeatedly, her hips and soft, plump ass bouncing off of your [legs] as she continues to fuck herself on your ' + Descriptors.cockDescript( x ) + ' harder and harder.  The way her inner folds lap and massage at your drug-infused cock is exquisite, and it takes every ounce of willpower to indeed stop yourself from cumming inside her repeatedly like you were broken down there.  Finally, Sophie slaps her hips into your hardness, moaning out loud as her cunt squeezes and tugs at your member violently, temporarily stopping the flow of cum out of your ' + Descriptors.cockDescript( x ) + ', but only for a split second.  It churns and boils in your [balls], raising through your ' + Descriptors.cockDescript( x ) + ' to be fired into Sophie\'s wet, ravenous cunt and baby-wanting womb.  Sophie\'s own orgasm and yours blend into one and prolong one another as you release ' );
		if( CoC.player.cumQ() < 250 ) {
			MainView.outputText( 'several ropes' );
		} else if( CoC.player.cumQ() < 750 ) {
			MainView.outputText( 'a large, chaotic stream' );
		} else if( CoC.player.cumQ() < 2000 ) {
			MainView.outputText( 'wave after wave' );
		} else {
			MainView.outputText( 'massive amounts' );
		}
		MainView.outputText( ' of cum into her, causing her to ' );
		if( CoC.player.cumQ() < 500 ) {
			MainView.outputText( 'moan in satisfaction' );
		} else if( CoC.player.cumQ() < 1000 ) {
			MainView.outputText( 'groan as her cunny is unable to hold it all' );
		} else if( CoC.player.cumQ() < 2000 ) {
			MainView.outputText( 'stretch atop of you, a cum-filled tummy added to the jiggling tits and soft butt' );
		} else {
			MainView.outputText( 'erupt your cum violently around your shaft as you stretch her to pregnancy-looking visage with your cum alone' );
		}
		MainView.outputText( '.' );
		MainView.outputText( '\n\n"<i>Ahhh... ahhh... good ' + CoC.player.mf( 'boy', 'girl' ) + '... very good... I\'m definitely a momma now.  But... buuuut... You should squirt out more just to make sure... and as punishment for trying to resist when Momma Sophie ordered you to cum!</i>"' );
		...
		MainView.outputText( ' is probably ready to blow another load very soon, isn\'t it?  You just can\'t keep up with Momma Sophie\'s exquisite pussy, you have to keep cumming and cumming over and over again.</i>"' );
		...
		MainView.outputText( '\n\nShe says that, but every lift and fall of her hips is a torture on your thoroughly milked, overly sensitive ' + Descriptors.cockDescript( x ) + '.  Her pussy is trying to draw out every drop of fluid you have in your body, and soon, it will succeed at doing just that it seems...' );
		...
		if( CoC.player.cumQ() < 250 ) {
			MainView.outputText( '  You only drop a meager few squirts inside of her' );
		} else if( CoC.player.cumQ() < 750 ) {
			MainView.outputText( '  Even your large cum production can\'t keep up with that pace' );
		} else {
			MainView.outputText( '  Inhuman as your cum factories are, you\'re only capable of delivering a few big spurts before it slows to a dribble' );
		}
		MainView.outputText( '.  You thrash and moan and shudder under her, calling her name as she strokes your [hair] and coos, pressing her nipple against your mouth again.  "<i>Shhh... you\'ve done well for me, little cum-pump... give it all to Momma Sophie... It\'ll be over in just a moment...</i>"' );
	...
	//Initial Intro;
	SophieFollowerScene.prototype.sophieFollowerIntro = function() {
	...
	SophieFollowerScene.prototype.kissSophieRecruitment = function() {
		MainView.clearOutput();
		MainView.outputText( 'You shut her up the best way you know how - planting your lips upon her own, knowing full-well of the effects of her lustful lipstick.  Sophie returns the gesture, locking her tongue around yours while she tastes you, grinding her other set of lips against your attentive member.' );
		...
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( ', your second one sliding up between her butt' );
		}
		if( CoC.player.cockTotal() > 3 ) {
			MainView.outputText( ' while the others press against her' );
		}
		MainView.outputText( '.  Sophie cries out in pleasure, almost waking up the entire neighbourhood - which would be a bad thing, considering your neighbours.  You quickly take another taste of her lips, shutting her up while you start to pound upwards into her hungry quim.' );
	...
	SophieFollowerScene.prototype.sophieRecruitmentFinale = function() {
		MainView.clearOutput();
		MainView.outputText( 'Sophie gives you a peck on the cheek, removing herself from your body to prepare for flight.' );
		...
		if( SceneLib.camp.companionsCount() >= 3 ) {
			MainView.outputText( ', sure to introduce her to the other denizens along the way and smooth over any ruffled feathers before things escalate.  Sophie does a good job helping with that.  She seems to have a natural ability to defuse angry situations, something she attributes to raising her many daughters' );
		}
		MainView.outputText( '.' );
	...
	/*Sophie daughter incest.  scenes
	 vaginal, maybe anal.
	 daughter 1 soft, sensitive lips, very large breasts.
	 daughter 2 round ass, very long feathers
	 daughter 3 harpy body, human feet
	 daughter 4 as curvy and voluptuous as sophie, very soft wings.
	 all purpose vaginal and anal sex scene, with modifiers for each daughters body type
	 intro for all scenes*/
	SophieFollowerScene.prototype.sophieIncestInHerCooterOrSomethingIDunno = function() {
		MainView.clearOutput();
		MainView.outputText( 'Approaching the harpy nest you call out for one of your daughters.  ' );
		if( CoC.flags[ kFLAGS.SOPHIE_ADULT_KID_COUNT ] > 1 ) {
			MainView.outputText( 'The prospect of their ' + CoC.player.mf( 'father', 'mother' ) + '\'s attention causes a small scuffle to break out between your feathery daughters. Thankfully, the small fight is soon ended and one of your daughters flies down and stands before you.  ' );
		} else {
			MainView.outputText( 'Your daughter, hearing your call, is more than happy to come to you.  ' );
		}
		MainView.outputText( 'The curvy girl smiles at you, her eyes bright and attentive, positively sparkling with the taboo affection she holds for her beloved daddy.  Smirking to her you give her an appraising look, your eyes wandering up and down her beautiful womanly body.' );
		var daughterCap = CoC.flags[ kFLAGS.SOPHIE_ADULT_KID_COUNT ];
		if( daughterCap > 4 ) {
			daughterCap = 4;
		}
		daughterCap = Utils.rand( daughterCap );
		if( daughterCap === 0 ) {
			//[first daughter: ;
			...
			if( CoC.flags[ kFLAGS.DAUGHTER_ONE_BIMBO ] > 0 && CoC.flags[ kFLAGS.SOPHIES_DAUGHTERS_DEBIMBOED ] === 0 ) {
				MainView.outputText( '"<i>Like, how do I look, ' + CoC.player.mf( 'Daddy', 'Mommy' ) + '?  Aren\'t my breasts so totally soft?  Don\'t you just want to suck and squeeze them?  Ooohhh!  ' + CoC.player.mf( 'Daddy', 'Mommy' ) + ', can you please touch me?  I want you so bad!  My pussy gets sooo wet when I, like, think about you pounding my cunny!</i>"' );
			}
			//normal speech:;
			else {
				MainView.outputText( '"<i>Don\'t you like how big and soft my breasts are, ' + CoC.player.mf( 'Daddy', 'Mommy' ) + '?  Don\'t you just want to squeeze them and suck on them?  Ooohh, ' + CoC.player.mf( 'Daddy', 'Mommy' ) + ', please, I want you so bad.  I get so hot and wet when I think of you!  I just want your [cock] inside me, pounding my hot cunt until I scream!</i>"' );
			}
			...
			if( CoC.flags[ kFLAGS.DAUGHTER_ONE_BIMBO ] > 0 && CoC.flags[ kFLAGS.SOPHIES_DAUGHTERS_DEBIMBOED ] === 0 ) {
				MainView.outputText( '\n\n"<i>Ohhh, oooohhhh!  ' + CoC.player.mf( 'D-Daddy', 'M-Mommy' ) + '!  L-like, I can\'t take so much teasing!  Please, Gimme your h-haaard cock, ' + CoC.player.mf( 'Dad', 'Mom' ) + '!<i>"' );
			}
			//[normal speech:;
			else {
				MainView.outputText( '\n\n"<i>Ohhh, oooohhhh!  ' + CoC.player.mf( 'D-Daddy', 'M-Mommy' ) + '!  Nnnnn... I can\'t take so much teasing!  Please, please give it to me, fuck me as hard as you can, ' + CoC.player.mf( 'Daddy', 'Mommy' ) + '!<i>"' );
			}
		}
		//second daughter: ;
			...
			if( CoC.flags[ kFLAGS.DAUGHTER_TWO_BIMBO ] > 0 ) {
				MainView.outputText( 'bimbo ' );
			}
			MainView.outputText( 'lips and blows you a kiss.  She slides her hands over her hips, pushing her taut butt out even more before spanking herself for your pleasure, her lips gasping at the feeling of her self-spank as she tries to work you into a breeding frenzy.' );
			...
			if( CoC.flags[ kFLAGS.DAUGHTER_TWO_BIMBO ] > 0 && CoC.flags[ kFLAGS.SOPHIES_DAUGHTERS_DEBIMBOED ] === 0 ) {
				MainView.outputText( '"<i>Mmmm... doncha want to just like, pound my sexy ass, ' + CoC.player.mf( 'Daddy', 'Mommy' ) + '?  It\'s so big, it\'s just begging to be spanked and squeezed till it\'s totally red and sensitive</i>"' );
			} else {
				MainView.outputText( '"<i>Come on, ' + CoC.player.mf( 'Daddy', 'Mommy' ) + ', touch me, spank me like the bad girl I am, lusting after my ' + CoC.player.mf( 'Daddy', 'Mommy' ) + ' like this.  Give it to me hard, spank my big round ass and pound my hot tight cunt like I know you want</i>"' );
			}
		...
		//third daughter:;
			...
			MainView.outputText( '\n\nYou clap your hands and applaud your daughter\'s work.  The curvy little harpy is working so hard to please her ' + CoC.player.mf( 'daddy', 'mommy' ) + ' and entice him into her nest...  ' );
			if( CoC.flags[ kFLAGS.DAUGHTER_THREE_BIMBO ] > 0 && CoC.flags[ kFLAGS.SOPHIES_DAUGHTERS_DEBIMBOED ] === 0 ) {
				MainView.outputText( '"<i>L-like, did I do a good job, ' + CoC.player.mf( 'Daddy', 'Mommy' ) + '?  I so totally did my best for you.  I know I\'m not super sexy like Mom and my sisters... but, ' + CoC.player.mf( 'Daddy', 'Mommy' ) + '.. I will like, still please you, like, totally the best that I can!</i>"' );
			} else {
				MainView.outputText( '"<i>W-well, what did you think, ' + CoC.player.mf( 'Dad', 'Mommy' ) + '?  Did I do a good job?  I know I\'m not as hot as Mom, but I promise I\'ll please you as best I can!</i>"' );
			}
			MainView.outputText( ' she says, wanting more than anything to be able to please you.' );
			...
			if( CoC.flags[ kFLAGS.DAUGHTER_FOUR_BIMBO ] > 0 && CoC.flags[ kFLAGS.SOPHIES_DAUGHTERS_DEBIMBOED ] === 0 ) {
				MainView.outputText( '"<i>Come on ' + CoC.player.mf( 'Daddy', 'Mommy' ) + ', can\'t you see how totally hot I am?  Don\'t you just want to let loose and like, just fuck me till I scream.</i>"' );
			} else {
				MainView.outputText( '"<i>Just look at me, ' + CoC.player.mf( 'Daddy', 'Mommy' ) + ', so curvy, so ready, so hot and wet for you.  Just let loose and show me your love, show me how much you love me, ' + CoC.player.mf( 'Daddy', 'Mommy' ) + ', fuck me hard, make me scream and claim me as yours.</i>"' );
			}
			MainView.outputText( ' the horny birdgirl says with a lick of her plump, kissable lips, aching hunger for your incestuous seed.  She wraps her arms around you, pulling her body against yours and sliding a long downy leg around you.  Her position opens her up to you and presses her sodden sex against your crotch.  She\'s such a lusty, lovely woman, and she wants you so badly, her body begging for you to take her and to seed her fertile body with your potent virility.' );
			...
			//bimbo speech;
			if( CoC.flags[ kFLAGS.DAUGHTER_FOUR_BIMBO ] > 0 && CoC.flags[ kFLAGS.SOPHIES_DAUGHTERS_DEBIMBOED ] === 0 ) {
				MainView.outputText( '  "<i>Ahh... ohhh... oh, ' + CoC.player.mf( 'Daddy', 'Mommy' ) + '!  You like my butt, don\'t you; isn\'t it nice?  It\'s so good to grab and squeeze.  I bet it would be fun to, like, spank it too....</i>"' );
			}
			//[normal speech:];
			else {
				MainView.outputText( '  "<i>Mmmm... Oh, ' + CoC.player.mf( 'Daddy', 'Mommy' ) + ', yessss...  That\'s it, touch me, touch me and play with me, do whatever you want with me.</i>"' );
			}
	...
	SophieFollowerScene.prototype.phaseTwoOfIncest = function( daughter ) {
		...
		daughter++;
		MainView.outputText( 'Tightening your hold on your curvaceous daughter, you shove her down onto the ground.  Growling with an almost primal edge, you lean down and inspect your prize, hands running over her ' );
		if( daughter === 1 ) {
			MainView.outputText( 'massive pillowy tits' );
		} else {
			MainView.outputText( 'tender perky breasts' );
		}
		MainView.outputText( ', your fingers sinking into her soft flesh as you caress and squeeze your daughter\'s bountiful body.  You lean down, kissing and sucking her neck, your lips latching on and sucking hard enough to leave your mark on her skin for the world to see.  She\'s unwilling to do anything against your lustful advances, mind and body ready and fully accepting whatever naughty things you would do to her.  With how heavily she is breathing and how lusty her moans and calls to you are, your daughter clearly wants the sweet incestuous union between ' + CoC.player.mf( 'father', 'mother' ) + ' and daughter even more than you do.  What a sexy and lovely daughter you have here...  at your aggressive touch, she groans and croons for you, beckoning to you to continue your familial onslaught.  She wraps her legs around your body, grinding her hips against you.  "<i>Yesss, yessss!  More, ' + CoC.player.mf( 'Daddy', 'Mommy' ) + '!</i>" she squawks in between her breathy moans.' );
		...
		//[cum volume low:;
		if( CoC.player.cumQ() < 250 ) {
			MainView.outputText( '  Squirting over and over into your sweet, lusty girl, you inseminate your fertile daughter, spilling as much of your cream right into her needy, hungry womb as possible.' );
		}//cum volume medium:;
		else if( CoC.player.cumQ() < 500 ) {
			MainView.outputText( '  Gushing over and over into your sweet, lusty daughter, you hold yourself tighter against her, making sure you force as much of your spunk into her waiting, hungry womb as you can.' );
		}//[cum volume high:;
		else if( CoC.player.cumQ() < 2000 ) {
			MainView.outputText( '  Gush after gush of thick rich cum flows from you, basting your daughter\'s womb with your virile spunk.  Wedging yourself as tightly against her as you can, you make sure that every drop packs into her fertile belly.  The potent flow of your seed quickly swells her stomach into a cute cum bloated paunch.' );
		} else {
			MainView.outputText( '  Your seed floods your daughter\'s tight, squeezing cunt like a flood, wave after wave of your thick, frothy spunk forces its way through her convulsing cunt and right into her eager, waiting womb.  Geysers of thick, frothy spunk force their way into her womb, filling it fuller and fuller until her belly swells and swells.  Her once tight, flat tummy rounds out into a healthy round cumbaby, your thick, yogurt-like spunk overflowing from her overstuffed snatch in thick, sticky streams.' );
		}
		MainView.outputText( '  In a moment of lucidity, you look down at your daughter and see the blissed-out look on her face; her creamy skin red; her eyes rolling up into her head.  Her tongue is lolling out from her plump, cocksucking lips.  The sight of your daughter consumed by such mind bending pleasure, the ecstasy of having her ' + CoC.player.mf( 'father', 'mother' ) + ' cumming inside her, only gives you a sense of pride and affection for your sweet daughter.' );
		...
		if( bimbo ) {
			MainView.outputText( '  "<i>Oh, ' + CoC.player.mf( 'Daddy', 'Mommy' ) + ', oh, ' + CoC.player.mf( 'Daddy', 'Mommy' ) + '...  So good!  My ' + CoC.player.mf( 'Daddy', 'Mommy' ) + '\'s such a hot, sexy ' + CoC.player.mf( 'stud', 'herm' ) + '.  I so totally thought I was going to like, lose my mind for a second.  I love you, ' + CoC.player.mf( 'Daddy', 'Mommy' ) + '.</i>"' );
		} else {
			MainView.outputText( '  "<i>Oh, ' + CoC.player.mf( 'Daddy', 'Mommy' ) + ', oh, ' + CoC.player.mf( 'Daddy', 'Mommy' ) + '... so good, you\'re such a fucking ' + CoC.player.mf( 'stud', 'perfect breeder' ) + '!  I could just get addicted to you.  I love you, ' + CoC.player.mf( 'Daddy', 'Mommy' ) + ',</i>"' );
		}
		MainView.outputText( ' she says to you as she kisses you with her soft lips.  Chuckling at your affectionate daughter, you snuggle up with her, spending some sweet, comfortable pillow time with your freshly seeded girl.' );
		MainView.outputText( '\n\nFor nearly an hour you lay there, cuddling, kissing and stroking your daughter\'s body as she hogs you all to herself.  Despite how comfortable it is to cuddle with your daughter like this, you have to pull yourself away from her curvaceous, cock-pleasing body.  She can\'t help but pout as you pull away, clearly wanting to spend more time with her beloved ' + CoC.player.mf( 'father', 'mother' ) + '.  Not wanting to leave her so disappointed, you reach around and give her a spank on the ass and pull her close to you, kissing her once more before promising to spend more time with her later.  With a smile, she giggles and nods, her mind happily thinking about the next time you will spend some quality time with her.' );
		//pass time 1 hour//;
