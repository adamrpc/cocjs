...
angular.module( 'cocjs' ).run( function( MainView, SceneLib, Combat, Clara, StatusAffects, ConsumableLib, Descriptors, CoC, kFLAGS, Utils, EngineCore ) {
	...
	MarblePurification.prototype.dumbshitCowCuntsTryToDrugYouBecauseTheyreCunts = function() {
		...
		//new page;
		if( CoC.player.inte >= 60 ) {
			EngineCore.addButton( 0, 'Next', this, this.highIntelligenceOutcomesVsCowCuntDrugs );
		} else if( CoC.player.inte >= 30 ) {
			EngineCore.addButton( 0, 'Next', this, this.mediumIntelligenceReactsToDrugging );
		} else {
			EngineCore.addButton( 0, 'Next', this, this.lowIntelligenceGetsDruggedByCowSluts );
		}
	};
	...
	MarblePurification.prototype.cowCuntBadEndPartIV = function() {
		...
		if( !CoC.player.hasCock() ) {
			...
			EngineCore.addButton( 0, 'Next', this, this.cockTooBigForCowCuntageBadEndage );
		} else {
			EngineCore.addButton( 0, 'Next', this, this.badEndDickThatFitsClara );
		}
	};
	...
	MarblePurification.prototype.defeatClaraCuntInAFight = function( cheated ) {
		...
		if( (CoC.player.hasCock() && CoC.player.cockThatFits( 20 ) >= 0) || CoC.player.hasVagina() ) {
			EngineCore.addButton( 1, 'Threesome', this, this.pcAndMurbleDoubleTeamCowButt );
		}
		if( (CoC.player.hasCock() && CoC.player.cockThatFits( SceneLib.marbleScene.marbleCuntCapacity() ) >= 0) || CoC.player.hasVagina() ) {
			EngineCore.addButton( 2, 'NTR', this, this.NTRIsClearlyTheWorstFetishWhyWouldYouWriteThisOMG );
		}
		if( CoC.flags[ kFLAGS.MARBLE_DICK_TYPE ] > 0 ) {
			EngineCore.addButton( 3, 'Marble Cock', this, this.futaMarbleIsAHugeCowToCowCuntAndStuffsCowCuntsCuntFullOfCowCock );
		}
		EngineCore.addButton( 4, 'Calm Down', this, this.calmMurblesFatAssDown );
	};
	//Marble spanks Clara;
	...
	MarblePurification.prototype.actuallyGrowPureMarblesTittiesForFunzies = function() {
		...
		EngineCore.doNext( SceneLib.marbleScene, SceneLib.marbleScene.giveItem );
	};
	//Reducto;
	...
	MarblePurification.prototype.nursingFromPureMarble = function() {
		...
		if( CoC.flags[ kFLAGS.MARBLE_TIME_SINCE_NURSED_IN_HOURS ] < 4 ) {
			...
			EngineCore.doNext( SceneLib.marbleScene, SceneLib.marbleScene.interactWithMarbleAtCamp );
		} else {
