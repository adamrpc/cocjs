...
angular.module( 'cocjs' ).run( function( MainView, SceneLib, Combat, Clara, StatusAffects, ConsumableLib, Descriptors, CoC, kFLAGS, Utils, EngineCore ) {
	...
	MarblePurification.prototype.BLUHBLUH = function() {
		...
		MainView.outputText( 'When you return to camp, you find Marble seated at the campfire, staring gloomily into the ashes. Curious, you approach your bovine lover and ask her what the matter is. She looks up at you, her ' );
		if( CoC.flags[ kFLAGS.MARBLE_BOVA_LEVEL ] <= 1 ) {
			MainView.outputText( 'human' );
		} else {
			MainView.outputText( 'bovine' );
		}
		MainView.outputText( ' head still in her hands, and sighs loudly. <i>"I’m sorry, sweetie, I just... I’ve tried to ignore it, but I just don’t feel comfortable here."</i>' );
		...
		if( SceneLib.amilyScene.amilyFollower() && !SceneLib.amilyScene.amilyCorrupt() || SceneLib.kihaFollower.followerKiha() || CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00238 ] === 1 ) {
			MainView.outputText( '  0...Has she been fighting with ' );
		...
		if( CoC.player.hasItem( ConsumableLib.PURHONY ) ) {
			MainView.outputText( '  You then recall that bottle of honey the bee-girl gave you, and ask if maybe it might work?' );
		}
		if( CoC.player.findStatusAffect( StatusAffects.PureCampJojo ) >= 0 ) {
			MainView.outputText( '  You remember how Jojo helped you become pure, and suggest maybe his meditations could assist Marble?' );
		}
	...
	MarblePurification.prototype.giveMarblePureQuestLabova = function() {
		...
		MainView.outputText( 'You tell Marble that you managed to get some LaBova alchemically purified, and you want her to drink some.' );
		//First time;
	...
	MarblePurification.prototype.visitRathazulToPurifyMarbleAfterLaBovaStopsWorkin = function() {
		...
		if( CoC.player.findStatusAffect( StatusAffects.CampRathazul ) < 0 ) {
			MainView.outputText( 'You spot the elderly alchemist\'s camp on the lake once more.\n\n' );
		}
		MainView.outputText( 'As you approach the rodentine alchemist, you remember Marble’s problems and resolve to ask him if he can possibly help you. Once he finishes greeting you, you politely return the greeting and ask if he would be willing to lend his opinion on a problem you have.' );
	...
	MarblePurification.prototype.pageTwoOfVisitingTheRatForMurblesPurification = function() {
		MainView.clearOutput();
		MainView.outputText( 'Fortunately, Marble is at your camp rather than off at Whitney’s, and your bovine lover looks up at you hopefully as you approach. "<i>Did he say he could help?</i>" she asks, excitedly.' );
	...
	MarblePurification.prototype.rathazulsMurbelReport = function() {
		...
		MainView.outputText( 'When you wake and join Marble ' );
		if( SceneLib.camp.companionsCount() > 2 ) {
			MainView.outputText( 'and your other followers ' );
		}
		MainView.outputText( 'for breakfast this morning, you find a bleary-eyed Rathazul waiting at the campfire. Marble clucks her tongue and gives him a mug of something strong, which he accepts gratefully, noisily slurping down a mouthful. He takes a seat, staring into his mug, and then sighs softly.' );
	...
	MarblePurification.prototype.stayHereAndDontGoGetYerSisterShesABitch = function() {
		MainView.clearOutput();
		MainView.outputText( 'You shake your head and tell Marble that you still need her at the camp; you’ll tell her when you can spare her.' );
	...
	MarblePurification.prototype.murbleShouldGoRunAlongAndGetHerCuntySisterSoTheyCanBeCuntsTogether = function() {
		MainView.clearOutput();
		MainView.outputText( 'You tell Marble that if she needs some time off to go and see her family for this matter, it’s okay with you.' );
	...
	MarblePurification.prototype.claraShowsUpInCampBECAUSESHESACUNT = function() {
		...
		if( CoC.flags[ kFLAGS.MARBLE_BOVA_LEVEL ] <= 1 ) {
			MainView.outputText( 'your mate has' );
		} else {
			MainView.outputText( 'what your mate use to have' );
		}
		MainView.outputText( '; cow legs, ears, horns, and large breasts.  However, she doesn’t have Marble’s brown fur, instead having black and white blotched fur across her legs and ears and a mane of white hair atop her head.  She is wearing a one piece thick cloth dress, with a great deal of fancy embroidery decorating it.  You also guess that she’s a bit smaller than Marble, in both the chest and height departments.' );
		...
		if( CoC.player.inte >= 60 ) {
			MainView.outputText( ' with a odd smile on her face' );
		}
		...
		if( CoC.player.inte >= 60 ) {
			MainView.outputText( ' somewhat unconvincingly' );
		}
		MainView.outputText( '.' );
	...
	MarblePurification.prototype.partTwoOfClaraShowingUpAndBeingACunt = function() {
		MainView.clearOutput();
		MainView.outputText( 'You turn back to the camp and continue going about your morning routine.  By the time you’re ready to head out, the girls have come back from the farm, bearing a bottle of Clara’s milk.  The spotted bovine’s foul mood seems to have improved as well.' );
		...
		if( SceneLib.camp.companionsCount() >= 3 ) {
			...
			if( SceneLib.isabellaFollowerScene.isabellaFollower() ) {
				...
				if( SceneLib.isabellaFollowerScene.isabellaAccent() ) {
					MainView.outputText( '  Isabella jumps from her chair, “<i>Tea?  Ooh, ja!  I hear tea und milk iz sehr gut combination, ja?</i>”  Well, she seems rather enthusiastic if nothing else.' );
				} else {
					MainView.outputText( '  Isabella jumps from her chair, “<i>Tea? Ooh, yes!  I hear tea and milk is a really good combination, yes? I\'ve never had the opportunity to try it though...</i>”  Well, she seems rather enthusiastic if nothing else.' );
				}
				teaDrinkers++;
			}
			//if (Izma is in camp);
	...
	MarblePurification.prototype.dumbshitCowCuntsTryToDrugYouBecauseTheyreCunts = function() {
		MainView.clearOutput();
		MainView.outputText( 'Upon returning to the center of activity, you find a rather annoyed looking Marble sitting off to the side while Clara busies herself with a large number of cups from her pack.  Curious over what happened, you sit down next to Marble while watching Clara check the temperature of the pot.  After a moment, you ask your bovine lover why she is just watching.  "<i>After I got the fire started, Clara insisted that I’d get in the way and ruin the tea if I tried to help,</i>" she grumbles in annoyance.' );
	...
	MarblePurification.prototype.lowIntelligenceGetsDruggedByCowSluts = function() {
		MainView.clearOutput();
		MainView.outputText( 'Once everyone has a cup, Clara smiles at all of you and holds her own cup up to her mouth and says, "<i>To Marble’s purification and meeting new friends.</i>"' );
	...
	MarblePurification.prototype.partTwoOfLowIntelligencePCsFallingForTheIncrediblyObviousCowCuntsTrap = function() {
		...
		MainView.outputText( ', you try to figure out what the heck is going on.  You remember meeting Marble’s sister, then getting tea, and then...  What did Clara say to Marble?  You groan slightly, your body feeling an odd old craving.' );
		...
		if( CoC.player.weaponName !== 'fists' ) {
			MainView.outputText( 'your [weapon] is nearby, and ' );
		}
	...
	MarblePurification.prototype.fightCowCuntAfterBeingTotesMcGoatsDrugged = function() {
		MainView.clearOutput();
		MainView.outputText( 'You steel yourself, ' );
		if( CoC.player.weaponName === 'fists' ) {
			MainView.outputText( 'raise your fists' );
		} else {
			MainView.outputText( 'grab your ' + CoC.player.weaponName );
		}
		MainView.outputText( ' and get ready to fight to free yourself from this mad milkmaid’s mammaries.  Realizing your intentions, Clara’s face turns from a crazed grin to an angry glare.  With barely contained fury, she screams at you, "<i>Oh don’t you worry darling, I will <b>make</b> you love drinking my milk!</i>" before standing up, hefting a nasty looking heavy mace, and charging at you.' );
		//go to Clara’s fight;
	...
	MarblePurification.prototype.mediumIntelligencesFightCowCunts = function() {
		MainView.clearOutput();
		MainView.outputText( 'You ready yourself for battle as Clara pulls an angry looking heavy mace out and says, "<i>Fine, we’ll doo this the hard way.</i>"' );
	...
	MarblePurification.prototype.highIntelligenceOutcomesVsCowCuntDrugs = function( teaDrinkers ) {
		...
		if( teaDrinkers === undefined || teaDrinkers <= 1 ) {
			...
			MainView.outputText( 'your followers' );
		}
	...
	MarblePurification.prototype.loseToClara = function( combat ) {
		...
		if( combat === undefined || combat ) {
			if( CoC.player.HP < 1 ) {
				MainView.outputText( 'Your body gives out from the strain, and you fall to the ground.  The last thing you hear before you pass out, is Clara’s triumphant laughter.' );
			}
			//proceed to loss scene;
			//PC loses via lust;
			else {
				MainView.outputText( 'You just can’t fight it anymore, and desperately start to masturbate in front of Clara.  Laughing triumphantly, Clara walks up to you and forces a pill down your throat.  A moment later, you pass out.' );
			}
			MainView.outputText( '\n\n' );
		}
		//proceed to loss scene;
		...
		MainView.outputText( 'You dream of odd things, distorted images that you called out for help to.  Then you were dragged away.  You kept seeing more shapes that felt familiar and important somehow, but you couldn’t focus on them.  You’d call again and again, only to be dragged away by something else again.  Eventually your cries for help become weaker and halfhearted; you know what’s going to happen next, so why bother?  Suddenly you realize that it is a cow-girl that is dragging you away; her face looks like Marble’s at first, then it splits into a huge, wicked smile, and you panic.' );
		...
		if( CoC.player.tallness < 70 ) {
			MainView.outputText( 'much too big for you.' );
		} else if( CoC.player.tallness <= 82 ) {
			MainView.outputText( 'just your size.' );
		} else {
			MainView.outputText( 'way too small for your stature.' );
		}
		MainView.outputText( '  You also find that you aren’t wearing your ' + CoC.player.armorName + ', and your ' + CoC.player.weaponName + ' is nowhere to be seen.  ' );
		...
		if( (CoC.player.hasCock() && CoC.player.smallestCockArea() <= 40) && CoC.player.biggestTitSize() <= 25 && !CoC.player.isTaur() && !CoC.player.isNaga() && !CoC.player.isGoo() && !CoC.player.isDrider() ) {
			MainView.outputText( 'Instead a plain but rather slim form fitting two piece garb adorns your body.  Whoever dressed you in that definitely wanted your bodily assets to be fully visible to anyone looking at you.' );
		} else {
			MainView.outputText( 'You’re completely naked.' );
		}
	...
	MarblePurification.prototype.losingToCowCuntsPageII = function() {
		MainView.clearOutput();
		MainView.outputText( 'It is then that you realize with a shock that a familiar urge is welling up inside you.  No, it is much stronger than you remember it being before.  You want breastmilk.  You <i>need</i> breastmilk, and if you don’t get it, you’ll die.  To make matters worse, you know whose breastmilk it is too.  Clara, Marble’s younger sister, has fatally addicted you to her milk.' );
		...
		if( CoC.player.statusAffectv3( StatusAffects.Marble ) === 1 ) {
			MainView.outputText( '"<i>Oh?  Wondering how I know about that?  Marble told me when she was explaining about you breaking free of the addiction.  Unlike her, I’m going to make sure you behave yourself.</i>"  ' );
		} else {
			MainView.outputText( '"<i>Confused?  That was one of the things Marble found out when she was researching the nature of her addiction.  She said she didn’t want to trouble you with that knowledge, but I don’t see why.</i>"  ' );
		}
		MainView.outputText( 'This woman is nothing like her sister, she loves the power she has over you, and loves to show you just how much she has.' );
		...
		MainView.outputText( '.  Then your eyes drop down to Clara’s chest, and you think of your need.' );
	...
	MarblePurification.prototype.cowCuntBadEndPartIII = function() {
		...
		if( CoC.player.hairLength > 0 ) {
			MainView.outputText( 'through your ' + Descriptors.hairDescript() );
		} else {
			MainView.outputText( 'across your scalp' );
		}
	...
	MarblePurification.prototype.cowCuntBadEndPartIV = function() {
		...
		if( CoC.player.hasCock() && !CoC.player.hasVagina() ) {
			MainView.outputText( 'At this point your ' + Descriptors.multiCockDescriptLight() + ' is rock hard, and Clara gives it a few strokes.  She does seem to lose interest surprisingly quick though.' );
		} else if( CoC.player.hasVagina() && !CoC.player.hasCock() ) {
			MainView.outputText( 'You’ve become quite wet at this point, and Clara gives your ' + Descriptors.vaginaDescript() + ' a few rubs, before tugging on your ' + Descriptors.clitDescript() + ' once.' );
		} else if( CoC.player.hasCock() && CoC.player.hasVagina() ) {
			MainView.outputText( 'Your arousal at this point is quite apparent on the whole of your genitals.  Clara gives you a few touches and caresses, but doesn’t seem all that interested.' );
		} else {
			MainView.outputText( 'Or rather, your lack of thereof.  Clara doesn’t seem to mind too much; Marble must have told her about this part of you too.' );
		}
	...
	MarblePurification.prototype.cockTooBigForCowCuntageBadEndage = function() {
		MainView.clearOutput();
		MainView.outputText( '"<i>Now, we do have one problem.  ' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 'All of your penises are' );
		} else {
			MainView.outputText( 'Your penis is' );
		}
		MainView.outputText( ' too big for a virgin like me to fit!  How the hell am I suppose to get that inside me?</i>"  Clara stands up and starts stomping around the room.  ' );
		...
		if( CoC.player.cockThatFits( SceneLib.marbleScene.marbleCuntCapacity() ) >= 0 ) {
			MainView.outputText( '"<i>‘It’s not a flaw, it’s a feature,’ she said, ‘This is why we train ourselves to take big endowments,’ she said!  Fuck you Marble!  I’m not sticking that stupid big thing inside me.</i>"  ' );
		} else {
	...
	MarblePurification.prototype.finalBadEndWithCowCunt = function() {
		MainView.clearOutput();
		MainView.outputText( 'Years later...' );
	...
	MarblePurification.prototype.defeatClaraCuntInAFight = function( cheated ) {
		if( cheated ) {
			...
			//PC drags Clara back to camp, was originally planning on putting more rapes here, but 7 with Marble are probably enough.;
				...
				MainView.outputText( 'Worried about the others, you head out of the cellar and drag Clara and her things with you.  Thanks to the strange way that travel works in this world, it doesn\'t take you long to find your way back to camp.  You catch your followers there just before they head out to go looking for you and dump your would be kidnaper before them.  Marble steps up next to you and looks down at her sister.' );
		...
		MainView.outputText( '"<i>Clara, what the hell were you trying to pull there?</i>" Marble demands of her sister, her hands shaking in rage.' );
		...
		if( SceneLib.camp.loversCount() > 1 ) {
			MainView.outputText( 'I\'m not even ' + CoC.player.short + '\'s only mate.  ' + CoC.player.mf( 'He', 'She' ) + ' can satisfy their needs, but can\'t satisfy mine?  ' ); //no new paragraph
		}
		MainView.outputText( 'The frustration of not being able to nurse, someone, anyone, but especially my mate is tearing me apart!</i>"  Marble stops screaming and her head drops.  All that\'s coming out of her now is heavy breathing.' );
		...
		if( (CoC.player.hasCock() && CoC.player.cockThatFits( 20 ) >= 0) || CoC.player.hasVagina() ) {
			MainView.outputText( 'use Marble\'s unstable state of mind to get her into a forceful threesome with Clara, ' );
		}
		...
		if( (CoC.player.hasCock() && CoC.player.cockThatFits( SceneLib.marbleScene.marbleCuntCapacity() ) >= 0) || CoC.player.hasVagina() ) {
			MainView.outputText( 'have sex in front of Clara so she can see what your love really is, ' );
		}
		//(if Marble is a futa);
	...
	MarblePurification.prototype.murbleSpanksCowCunt = function() {
		MainView.clearOutput();
		MainView.outputText( 'Marble seems to have a good idea on how to punish Clara already, so you just sit back and watch.' );
	...
	MarblePurification.prototype.pcAndMurbleDoubleTeamCowButt = function() {
		MainView.clearOutput();
		var x = CoC.player.cockThatFits( 20 );
		MainView.outputText( 'You step forward and suggest to Marble that the two of you should work together to punish Clara.  She nods and asks what you have in mind.  Well, you were thinking that a sexual punishment from the two of you would be appropriate.' );
		...
		if( (CoC.player.hasCock() && x >= 0) && CoC.flags[ kFLAGS.MARBLE_DICK_TYPE ] > 0 ) {
			...
			MainView.outputText( 'Eventually you pull her up and sandwich her between your bodies.  Marble expresses an aversion to anal, so you take the backside while she takes the front.  "<i>Hey, you can\'t do this to me!  I don\'t want to lose my virginity to my sister!</i>" Clara cries out indignantly.' );
	...
	MarblePurification.prototype.NTRIsClearlyTheWorstFetishWhyWouldYouWriteThisOMG = function() {
		...
		if( CoC.player.hasCock() && x >= 0 ) {
			MainView.outputText( 'she drops herself onto your ' + Descriptors.cockDescript( x ) + '.  Since your bovine mate isn\'t in the habit of wearing any form of undergarments, her skirt doesn\'t really do much to get in the way of penetration.  A shriek of dismay accompanies this act.' );
		...
		//else if (PC has a vagina and Marble has a cock);
			...
			MainView.outputText( 'she drops herself onto the ground next to you.  Abruptly, she lifts you up and drops your womanhood onto the member you gave her.  Since your bovine mate isn\'t in the habit of wearing any form of undergarments, her skirt doesn\'t really do much to get in the way of penetration.  A shriek of dismay accompanies this act.' );
		...
		//else if (PC has a vagina and Marble does not have a cock);
			...
			MainView.outputText( 'she drops herself onto your legs, and grabs one of them.  Instantly, your two womanhoods are being grinded against one another with great force and urgency.  Since your bovine mate isn\'t in the habit of wearing any form of undergarments, her skirt doesn\'t really do much to get in the way of tribadism.  A shriek of dismay accompanies this act.' );
	...
	MarblePurification.prototype.futaMarbleIsAHugeCowToCowCuntAndStuffsCowCuntsCuntFullOfCowCock = function() {
		MainView.clearOutput();
		MainView.outputText( 'You call out to Marble and suggest that she should use her new "appendage" to punish Clara.  She hesitates for a moment, then nods.  You doubt that you could have ever gotten her to do something like that if she wasn\'t as worked up as she is.' );
	...
	MarblePurification.prototype.calmMurblesFatAssDown = function() {
		...
		if( (CoC.player.tallness >= 80 && CoC.flags[ kFLAGS.MARBLE_BOVA_LEVEL ] < 2) || (CoC.player.tallness >= 86 && CoC.flags[ kFLAGS.MARBLE_BOVA_LEVEL ] >= 2) ) {
			MainView.outputText( 'tightly hugs herself against your body.  Gently stroking her hair, you\'re happy to be a shoulder for her to cry on, given both your affection for the gentle girl and the awareness that you\'re able to help her feel better.' );
		}
		//else if (PC taller than 5 feet);
		else if( CoC.player.tallness >= 60 ) {
			MainView.outputText( 'returns the hug.  Rocking back and forth against one another, you\'re happy to be here for her, given both your affection for the lovely lady and the warm feeling of being so close to one you care so much for.' );
		} else {
			MainView.outputText( 'scoops you up, hugging you tightly into her chest.  Sandwiched between her massive mammaries, you\'re happy to serve as her temporary comfort doll, given both your affection for the big bovine and the pleasant accommodation of her bountiful breasts.' );
		}
	...
	MarblePurification.prototype.rathazulFinishingMarblesPotion = function() {
		...
		if( CoC.flags[ kFLAGS.MARBLE_KIDS ] > 0 ) {
			MainView.outputText( 'Then we\'ll be ready to purify you and your child' );
			if( CoC.flags[ kFLAGS.MARBLE_KIDS ] > 0 ) {
	...
	MarblePurification.prototype.theFormulaIsDone = function() {
		MainView.clearOutput();
		MainView.outputText( 'Rathazul tells you that the formula is ready, so you head over to Marble to let her know.' );
	...
	MarblePurification.prototype.purifyMarbleComplete = function() {
		MainView.clearOutput();
		MainView.outputText( 'The two of you arrive at Rathazul\'s workspace, and find the owner awaiting you, holding a bottle of whitish fluid with gold streaks running throughout.  "<i>So is that the formula?</i>"  Marble asks looking at the bottle.' );
		...
		if( CoC.flags[ kFLAGS.MARBLE_KIDS ] > 0 ) {
			...
			MainView.outputText( ' right away.  Give ' );
			if( CoC.flags[ kFLAGS.MARBLE_KIDS ] === 1 ) {
				MainView.outputText( 'her' );
			} else {
				MainView.outputText( 'each' );
			}
			MainView.outputText( ' a two drop dose,</i>" the alchemist instructs you and hands over the bottle of his formula.' );
			...
			MainView.outputText( ' two drops of the formula.  ' );
			if( CoC.flags[ kFLAGS.MARBLE_KIDS ] > 1 ) {
				MainView.outputText( 'They ' );
			} else {
				MainView.outputText( 'She ' );
			}
			MainView.outputText( 'take' );
			if( CoC.flags[ kFLAGS.MARBLE_KIDS ] === 1 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ' it easily enough, and while there are no outward effects, ' );
			if( CoC.flags[ kFLAGS.MARBLE_KIDS ] === 1 ) {
				MainView.outputText( 'your ' );
			} else {
				MainView.outputText( 'each ' );
			}
			MainView.outputText( 'child does report feeling like something was lifted from ' );
			if( CoC.flags[ kFLAGS.MARBLE_KIDS ] > 1 ) {
				MainView.outputText( 'their' );
			} else {
				MainView.outputText( 'her' );
			}
			MainView.outputText( ' mind.' );
	...
	MarblePurification.prototype.dealingWithCowCuntPostPurification = function() {
		MainView.clearOutput();
		MainView.outputText( 'Now there is only one more loose end in this whole effort to purify Marble sister.' );
	...
	MarblePurification.prototype.purifyMurblesSister = function() {
		MainView.clearOutput();
		MainView.outputText( 'You ask Marble for the bottle of Rathazul\'s formula.  She nods, hands it to you and says, "<i>Yeah, I think that\'s the best choice too.</i>"' );
	...
	MarblePurification.prototype.cageDatCowCunt = function() {
		MainView.clearOutput();
		MainView.outputText( 'You say that you\'re going to be keeping Clara a prisoner in the camp.  She deserves to be imprisoned so she\'ll no longer be a threat to anyone anymore.  Marble looks uncertain at you and asks if you\'re sure she needs to be locked up now that she\'s been purified.  You say you\'re certain, and your mate nods to you and says that she trusts your judgment on this matter.' );
	...
	MarblePurification.prototype.letGoOfMarblesCowCuntSister = function() {
		MainView.clearOutput();
		MainView.outputText( 'You say that now that Clara is no danger to anyone any more, she is free to go, so long as she promises to never bother you or your followers again.  Marble nods at this and says she agrees with your decision.' );
	...
	MarblePurification.prototype.dontPurifyClara = function() {
		MainView.clearOutput();
		MainView.outputText( 'No, you\'ll offer her the mercy of letting her keep her precious corruption.  Marble tells you she will respect your decision on that matter.  Now the question is what to do with her.' );
		MainView.outputText( '\n\nYou guess you could also build a cage in the camp if you really want to keep an eye on her yourself.  That will make her another mouth to feed, and you\'ll have to hear her ranting every day.  Still, the saying does go, "keep your enemies closer."' );
		MainView.outputText( '\n\nIf you\'re feeling especially generous, you could just let her go, with not more than a promise to never come back or try to kidnap you again.' );
		//PC makes their choice: Tel'Adre, SceneLib.camp cage, let go.";
		MainView.menu();
	...
	MarblePurification.prototype.stickUnpurifiedClaraInACage = function() {
		MainView.clearOutput();
		MainView.outputText( 'You say that you\'re going to be keeping Clara a prisoner in the camp.  She deserves to be imprisoned so she\'ll be no threat to anyone anymore.  Marble nods to you and says that she trusts your judgment on this matter.' );
	...
	MarblePurification.prototype.purificationQuestFinal = function() {
		MainView.clearOutput();
		MainView.outputText( '"<i>It has been quite the morning, hasn\'t it sweetie?</i>"  It certainly has been, but at least now, your lover is finally free of her corruption once and for all.' );
	...
	MarblePurification.prototype.actuallyGrowPureMarblesTittiesForFunzies = function() {
		MainView.clearOutput();
		MainView.outputText( 'You offer Marble a bottle of purified succubus milk and tell her that you\'d like her to make her bust bigger.' );
		...
		if( CoC.flags[ kFLAGS.MARBLE_BREAST_SIZE ] < 3 ) {
			...
			MainView.outputText( SceneLib.marbleScene.marbleBreastSize() + ' melons!' );
	...
	MarblePurification.prototype.pureMurblePCChoosesYesToShrinkeyTits = function() {
		MainView.clearOutput();
		MainView.outputText( 'You offer Marble a jar of reducto and tell her you\'d like her to reduce her bust\'s size a bit.' );
		if( CoC.flags[ kFLAGS.MARBLE_BREAST_SIZE ] > 0 ) {
			...
			CoC.flags[ kFLAGS.MARBLE_BREAST_SIZE ]--;
			MainView.outputText( SceneLib.marbleScene.marbleBreastSize() + ' sized breasts.' );
			if( CoC.flags[ kFLAGS.MARBLE_BREAST_SIZE ] === 0 ) {
	...
	MarblePurification.prototype.nursingFromPureMarble = function() {
		...
		if( CoC.flags[ kFLAGS.MARBLE_TIME_SINCE_NURSED_IN_HOURS ] < 4 ) {
			MainView.outputText( 'Feeling a bit thirsty, you ask Marble if there’s any chance she has some milk for you.  She gives you a sad look and informs you that no, she hasn\'t yet built up enough milk for you to have more.  Purifying her has made it take a bit longer for her to produce milk.' );
			EngineCore.doNext( SceneLib.marbleScene, SceneLib.marbleScene.interactWithMarbleAtCamp );
		} else {
			MainView.outputText( 'Feeling a bit thirsty, you ask Marble if theres any chance she has some milk for you.  She smiles at you and tells you she was worried you\'d never ask, then leads you to a familiar, secluded part of the rocks around the camp.' );
			...
			if( CoC.player.tallness < 65 ) {
				MainView.outputText( 'The tall cow-lady takes a seat on the rocks and you jump onto her lap, ' );
			} else if( CoC.player.tallness < 84 ) {
				MainView.outputText( 'You lean forward and ' );
			} else {
				MainView.outputText( 'You lift up the cow-lady to get access, ' );
			}
			MainView.outputText( 'in seconds your lips are wrapped around her teats, and her wonderful milk is flowing forth.' );
	...
	MarblePurification.prototype.marbleWarnsPCAboutCorruption = function() {
		MainView.clearOutput();
		MainView.outputText( 'Marble comes up to you with a worried look on her face.  "<i>Uh, sweetie?  I\'m getting worried about you.  You\'re trying to put so much effort into freeing me of corruption, but you seem to be slipping away yourself...</i>"  She looks at the ground, rubs her hands a few times desperately trying to spit the words out.  "<i>I don\'t think I\'ll be able to stay by your side if you keep falling like that.  Please find a way to clear yourself of the corruption that\'s gripping you!</i>"' );
	...
	MarblePurification.prototype.marbleLeavesThePCOverCorruption = function() {
		MainView.clearOutput();
		MainView.outputText( 'Marble approaches you with a somber expression on her face, her things strapped to a pack on her back' );
		//if (PC has more than one child with Marble);
		if( CoC.flags[ kFLAGS.MARBLE_KIDS ] > 1 ) {
			MainView.outputText( ' , and with her children in tow' );
		}
		//else if (PC has one child with Marble);
		else if( CoC.flags[ kFLAGS.MARBLE_KIDS ] === 1 ) {
			MainView.outputText( ', and holding her child\'s hand' );
		}
		MainView.outputText( '.  "<i>I\'m sorry sweetie.  I tried to warn you, but-</i>" she stops for a moment and takes a deep breath to steady herself.  ' );
		//if (PC has at least one child with Marble);
		if( CoC.flags[ kFLAGS.MARBLE_KIDS ] > 0 ) {
			MainView.outputText( '"<i>Mommy, what\'s wrong?</i>"  The young one holding Marble\'s hand asks nervously."<i>Hush now, little one.  We have to go away for awhile,</i>" the bovine mother responds and turns back to you.  ' );
		}
		MainView.outputText( '"<i>You\'ve gotten too corrupted, and I can\'t stay here with you anymore.  Please find a way to free yourself from the taint within you so that I can come back.  Goodbye.</i>"' );
	...
	MarblePurification.prototype.pureMarbleDecidesToBeLessOfABitch = function() {
		...
		MainView.clearOutput();
		MainView.outputText( 'While checking your traps to ensure that they\'re still working, you hear a voice call out from outside the camp, "<i>Sweetie!</i>"  You look up and see the smiling face of Marble' );
		//if (PC has at least one child with Marble);
		if( CoC.flags[ kFLAGS.MARBLE_KIDS ] > 0 ) {
			MainView.outputText( ' and that of your child' );
		}
		//if (PC has two or more children with Marble) ;
		if( CoC.flags[ kFLAGS.MARBLE_KIDS ] > 1 ) {
			MainView.outputText( 'ren' );
		}
		MainView.outputText( ' just outside the camp.  She must have somehow found out about your purification.' );
	...
	MarblePurification.prototype.claraCampAddition = function() {
		MainView.outputText( '  Nearby the portal sits a large wooden cage for keeping Marble\'s sister Clara prisoner.  It is tall enough for her to stand in and long enough for her to lie down completely.  ' );
		//if (Clara was purified);
		if( CoC.flags[ kFLAGS.CLARA_PURIFIED ] > 0 ) {
			if( Utils.rand( 10 ) === 0 ) {
				MainView.outputText( 'You catch Clara staring at you for a moment, then her gaze once again returns to some distant mountain top.  She has not taken her purification well.' );
			} else {
				MainView.outputText( 'As always, Clara sits within, staring off into the distance.  She has not taken her purification well.' );
			}
		} else {
			//if (time of day <= 7 OR >= 20);
			if( CoC.time.hours <= 7 || CoC.time.hours >= 20 ) {
				MainView.outputText( 'Clara is laying sprawled out on the ground of her cage, snoring loudly.' );
			} else {
