...
angular.module( 'cocjs' ).run( function( SceneLib, $rootScope, $log, Descriptors, PerkLib, UsableLib, ImageManager, CockTypesEnum, Appearance, AppearanceDefs, EventParser, ArmorLib, Amily, CoC, kFLAGS, Utils, StatusAffects, PregnancyStore, EngineCore, Combat, ConsumableLib ) {
		...
		switch( this.pregnancy.event ) {
			...
			default: //Amily is not pregnant
				...
				//[High Affection];
					...
					EngineCore.outputText( 'She grins at you with open delight. "<i>Hey there, ' + CoC.player.short + '! It\'s great to see you again... ', false );
	...
	AmilyScene.prototype.talkToAmily = function() {
		...
		//[Medium Affection];
			...
			EngineCore.outputText( '"<i>Of course, ' + CoC.player.short + ', I always enjoy our talks.  What shall we discuss this time?</i>" she asks happily.\n\n', false );
		}
		//[High Affection];
		else {
	...
	AmilyScene.prototype.thisIsAReallyShittyBadEnd = function() {
		...
		EngineCore.outputText( 'Except for today. Amily walks out to confront you casually. "<i>Ah, ' + CoC.player.short + ', there you are,</i>" she states. "<i>Good. I was hoping to see you one last time; I thought you at least deserved a goodbye.</i>"\n\n', false );
	...
	AmilyScene.prototype.thisFunctionProbablySucksTooOhYeahAmilyFunction = function() {
		...
		EngineCore.outputText( '"<i>' + CoC.player.short + '! Darling! You\'re here!</i>"\n\n', false );
	...
	AmilyScene.prototype.amilySexBusiness = function() {
		EngineCore.outputText( '', true );
		this.amilySprite();
		EngineCore.outputText( 'Allowing Amily to take care of her clothes, you hastily remove your own ' + CoC.player.armorName + '. Once the two of you are naked in front of each other, Amily looks you up and down, and then sniffs - not in disdain, but honestly trying to get a good scent of you. You speculate that this is some kind of check to see that you haven\'t somehow managed to become corrupted since last you met.\n\n', false );
		this.amilySexPtII();
	};
	//[Playtime First];
	...
	AmilyScene.prototype.getDownWithSexTiem = function() {
		...
		this.amilySprite();
		var x = CoC.player.cockThatFits( 61 );
		EngineCore.outputText( 'Too horny to think of anything else than what lies ahead, you hastily remove your ' + CoC.player.armorName + '.  Amily smiles at what she can see, enjoying the sight of your body and your ' + Descriptors.cockDescript( x ) + '.\n\n', false );
		this.continueWithMoreMidLevelAmilySex();
	};
	...
	AmilyScene.prototype.amilyFollowerEncounter = function() {
		...
		//Non corrupt!;
		if( CoC.flags[ kFLAGS.AMILY_FOLLOWER ] === 1 ) {
			EngineCore.outputText( 'As you approach the mouse-woman, her big, hairless ears twitch and she turns to face you with a smile. "<i>Hi, ' + CoC.player.short + '! What\'s up?</i>" She grins at you.\n\n', false );
		}
		//Corrupt!;
		else {
			if( CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_AMILY ] === 0 ) {
				EngineCore.outputText( '"<i>Amily!</i>" you yell. In no time a purplish blur rushes towards you; skidding to a halt, she kneels before you, hands behind her back, fully exposed. "<i>Yes my ' + CoC.player.mf( 'master', 'mistress' ) + '?</i>" Amily says seductively, licking her lips.\n\n', false );
			} else {
		...
		switch( this.pregnancy.event ) {
			...
			case 7:
				...
				break;
			case 8:
				EngineCore.outputText( 'Amily\'s bulge frequently wriggles and squirms, though this doesn\'t seem to bother her. ' + (CoC.flags[ kFLAGS.AMILY_FOLLOWER ] === 1 ? 'T' : 'She smiles with glee, t') + 'he children mustn\'t have too much longer until they are born.\n\n', false );
		}
		this.amilyMenu( true );
	};
	...
	AmilyScene.prototype.amilyCorruptSexMenu = function() {
		...
		if( CoC.player.gender > 0 ) {
			EngineCore.outputText( 'Amily asks, "<i>How would ' + CoC.player.mf( 'master', 'mistress' ) + ' like to use ' + CoC.player.mf( 'his', 'her' ) + ' cum-bucket today?', false );
			if( CoC.player.hasCock() ) {
	...
	AmilyScene.prototype.amilyAppearance = function() {
		...
		if( CoC.flags[ kFLAGS.AMILY_FOLLOWER ] === 1 ) {
			//(Start [horsecock];
			EngineCore.outputText( 'Amily is a 5\' 2" tall ' + what + ', with a lean and wiry build. Her pink eyes normally twinkle merrily, but they can turn hard and cold if the circumstances warrant, just as the normally friendly grin ' + onHerMuzzle + 'can turn cruel and harsh when she is angry. ' + furDesc + ' a long, hairless mouse\'s tail that sways and twitches constantly from her behind. She is currently wearing ' + CoC.flags[ kFLAGS.AMILY_CLOTHING ] + '. She has ' + this.amilyHips() + ' and a ' + this.amilyButt() + '.\n\n', false );
			//(End [horsecock];
			EngineCore.outputText( 'She has a pair of ' + this.amilyTits() + ' on her chest. They have ' + CoC.flags[ kFLAGS.AMILY_NIPPLE_LENGTH ] + '-inch nipples at their tips and must be at least ' + Appearance.breastCup( CoC.flags[ kFLAGS.AMILY_CUP_SIZE ] ) + 's.\n\n', false );
			...
			if( CoC.flags[ kFLAGS.AMILY_WANG_LENGTH ] > 0 ) {
				...
				EngineCore.outputText( 'She has a ' + CoC.flags[ kFLAGS.AMILY_WANG_LENGTH ] + '-inch penis growing from her crotch, replacing the clitoris that would normally jut from her vulva. It is surprisingly human looking, ' + dickDesc + ' - it even has a foreskin, not a sheath.\n\n', false );
			}
			...
			// End [horsecock];
			EngineCore.outputText( 'She has a pair of ' + this.amilyTits() + ' on her chest. They have ' + CoC.flags[ kFLAGS.AMILY_NIPPLE_LENGTH ] + '-inch nipples at their tips and must be at least DD-cups.\n\n', false );
			//(If Amily has a penis:;
			if( CoC.flags[ kFLAGS.AMILY_WANG_LENGTH ] > 0 ) {
				EngineCore.outputText( 'She has a half-erect ' + CoC.flags[ kFLAGS.AMILY_WANG_LENGTH ] + '-inch penis growing from her crotch. It is surprisingly human looking, ' + ((CoC.flags[ kFLAGS.AMILY_NOT_FURRY ] === 0) ? 'naked and hairless ' : '') + 'and pink and throbbing', false );
				if( CoC.flags[ kFLAGS.AMILY_HAS_BALLS_AND_SIZE ] === 0 ) {
					EngineCore.outputText( ' - it even has a foreskin, not a sheath.\n\n', false );
				} else {
					EngineCore.outputText( ', though it does have a' + ((CoC.flags[ kFLAGS.AMILY_NOT_FURRY ] === 1) ? ' little sheath.' : ' fuzzy little sheath.') + '\n\n', false );
				}
			}
			if( CoC.flags[ kFLAGS.AMILY_HAS_BALLS_AND_SIZE ] > 0 ) {
			...
			if( SceneLib.farmCorruption.hasTattoo( 'amily' ) ) {
				...
				if( SceneLib.farmCorruption.amilyFullTribalTats() ) {
					...
					if( CoC.flags[ kFLAGS.AMILY_TATTOO_COLLARBONE ] !== 0 ) {
						EngineCore.outputText( CoC.flags[ kFLAGS.AMILY_TATTOO_COLLARBONE ] + '\n' );
					}
					if( CoC.flags[ kFLAGS.AMILY_TATTOO_SHOULDERS ] !== 0 ) {
						EngineCore.outputText( CoC.flags[ kFLAGS.AMILY_TATTOO_SHOULDERS ] + '\n' );
					}
					if( CoC.flags[ kFLAGS.AMILY_TATTOO_LOWERBACK ] !== 0 ) {
						EngineCore.outputText( CoC.flags[ kFLAGS.AMILY_TATTOO_LOWERBACK ] + '\n' );
					}
					if( CoC.flags[ kFLAGS.AMILY_TATTOO_BUTT ] !== 0 ) {
						EngineCore.outputText( CoC.flags[ kFLAGS.AMILY_TATTOO_BUTT ] + '\n' );
					}
				}
			}
		}
	...
	AmilyScene.prototype.fuckTheMouseBitch = function() {
		...
		EngineCore.outputText( 'She strides over to take you in her arms, "<i>So, what would you like to do, ' + CoC.player.short + '?</i>"\n\n', false );
		//[Take Charge];
		//[Let Amily Lead];
	...
	AmilyScene.prototype.takeChargeAmilyMountHer = function() {
		...
		EngineCore.outputText( '\n\n', false );
		EngineCore.outputText( 'Amily gasps in delight at the sensation of being surrounded by your ' + Descriptors.vaginaDescript() + '. "<i>Oh, yes, ' + CoC.player.short + ', please!</i>" she cries, clearly happy.\n\n', false );
		//(If player is herm with 'self-titfuck' cock:;
		if( CoC.player.longestCockLength() >= 18 ) {
			EngineCore.outputText( 'Amily grabs hold of your ' + Descriptors.cockDescript( CoC.player.longestCock() ) + ', eagerly cramming it between her ' + this.amilyTits() + ', mashing them together and running them up and down against your ' + Descriptors.cockDescript( CoC.player.longestCock() ) + ', surrounding it with warm, soft, velvety flesh.', false );
		...
		//(If player is herm:;
			...
			EngineCore.outputText( 'She reaches out and grabs hold of ' + CoC.player.oMultiCockDesc() + ', stroking, tugging, and teasing it to further excite you into increasing the speed and force with which you thrust.... Something that she succeeds admirably in doing.\n\n', false );
		}
		...
		EngineCore.outputText( ', splattering mixed fluids below your ' + CoC.player.legs() + ' and Amily\'s ', false );
		if( CoC.flags[ kFLAGS.AMILY_NOT_FURRY ] === 0 ) {
	...
	AmilyScene.prototype.letAmilyLead = function() {
		...
		//Let Amily Lead: Amily Mounts You;
			...
			EngineCore.outputText( 'You smile up at her and spread your ' + CoC.player.legs() + ' to let her have a full look at your ' + Descriptors.vaginaDescript() + ', smirking at her as an exhortation to come and get it. From her own smirk, and the fully erect penis she\'s sporting, you know she\'s about to get it as much as she can.\n\n', false );
	...
	AmilyScene.prototype.giveAmilyPureIncubusDraft = function() {
		...
		//CORRUPT;
			...
			if( CoC.flags[ kFLAGS.AMILY_WANG_LENGTH ] === 0 ) {
				...
				EngineCore.outputText( 'You pick up a vial of incubus draft and show it to Amily. "<i>Drink this,</i>" you tell her, passing the bottle to her. "<i>' + CoC.player.mf( 'Master', 'Mistress' ) + ', this will make me grow a cock. Are you sure you wish to do that?</i>"\n\n', false );
				EngineCore.outputText( 'You tell her yes, this is exactly what you want. Amily smiles and replies seductively, "<i>As you wish ' + CoC.player.mf( 'master', 'mistress' ) + '.</i>"\n\n', false );
				...
				EngineCore.outputText( 'You pick up a vial of incubus draft and show it to Amily. "<i>I want you to have a bigger cock; drink this.</i>" You order her, passing the bottle to her. Amily smiles and says, "<i>As you wish, ' + CoC.player.mf( 'master', 'mistress' ) + ', your orders are my pleasure.</i>"\n\n', false );
	...
	AmilyScene.prototype.corruptAmilyGetsDickMaxxedOut = function() {
		...
		if( CoC.player.isBiped() && CoC.player.lowerBody !== AppearanceDefs.LOWER_BODY_TYPE_HOOFED ) {
			EngineCore.outputText( 'You\'re not satisfied; it needs to be bigger. You put your ' + CoC.player.foot() + ' on her cock and begin stroking it, drawing more pleasured moans from the slutty corrupt futa-mouse. You stroke her into an orgasm, milky white fluid flows out of her dick to hit her in the chest and in the face, but you don\'t stop. Her cock throbs and spills pre-cum, making your movements easier. You keep stroking her cock with your ' + CoC.player.foot() + ' vigorously, willing it to grow more and more; each time her cock throbs and orgasms it grows a bit more.\n\n', false );
	...
	AmilyScene.prototype.amilyDrinksSuccubusDelight = function() {
		...
		if( CoC.flags[ kFLAGS.AMILY_HAS_BALLS_AND_SIZE ] === 0 ) {
			...
			EngineCore.outputText( 'You tell her you need her to be able to cum more, so balls would help with that. Amily smiles and says, "<i>Of course, ' + CoC.player.mf( 'master', 'mistress' ) + '. Forgive your stupid mouse slut for questioning you.</i>"\n\n', false );
			...
			EngineCore.outputText( 'You watch, smiling, as a couple of ' + ((CoC.flags[ kFLAGS.AMILY_NOT_FURRY ] === 0) ? 'lumps begin forming' : 'fuzzy lumps begin forming'), false );
			if( CoC.flags[ kFLAGS.AMILY_WANG_LENGTH ] > 0 ) {
				...
				EngineCore.outputText( 'between her legs. Her skin expands with the lumps, forming into a small sack. It continues to expand and is finally complimented by a couple of orbs falling into her ' + ((CoC.flags[ kFLAGS.AMILY_NOT_FURRY ] === 0) ? 'fuzzy ' : '') + ' nutsack, giving it the weight it needs to produce cum, though how it will ever expel it is a mystery to you. ', false );
			}
			EngineCore.outputText( '"<i>Good. Now, I want you to practice walking with these. I can\'t have you hurting yourself as you walk about,</i>" you tell her. "<i>Yes, ' + CoC.player.mf( 'master', 'mistress' ) + '</i>\," she replies, panting slightly; you leave her prone on the ground.', false );
			CoC.player.orgasm();
	...
	AmilyScene.prototype.giveAmilyAPinkEgg = function() {
		...
		//Corrupt;
			...
			EngineCore.outputText( 'You offer her a pink egg, explaining that it will remove her penis.  Amily looks completely devastated as she says, "<i>Did fuck-slut not use her dick well enough?  Cum-bucket is so sorry ' + CoC.player.mf( 'master', 'mistress' ) + ', it won\'t happen again!</i>"  Before you can respond she snatches the egg from your hand and begins to eat it, messily devouring it until she\'s licking her fingers.  Once she has finished, she drops her pants to watch as <b>her penis shrinks and blurs, eventually resolving itself once more into the dainty form of her clitoris</b>.\n\n', false );
			EngineCore.outputText( '"<i>Well, is there something else this worthless cum-collector can do to please her ' + CoC.player.mf( 'master', 'mistress' ) + '?  Slut is so sorry she disappointed you,</i>" she squeaks sadly, still trying to entice you into sex.', false );
		}
	...
	AmilyScene.prototype.giveAmilyAPurpleEgg = function() {
		...
		//Corrupt;
			...
			if( CoC.flags[ kFLAGS.AMILY_HIP_RATING ] < maxSizeCorr ) {
				EngineCore.outputText( 'She looks at it thoughtfully. "<i>' + CoC.player.mf( 'Master', 'Mistress' ) + ' wants my hips even wider?  Are you going to use me as your private breeding stock?  Mmm, that turns me on!</i>" She takes the egg from you, her prominent front-teeth effortlessly biting off the top, whereupon she sucks down the contents in a practiced gulp. Crushing the shell in her hand, she almost loses her balance as her hips suddenly jut wider, the growth happening much faster than she expected. "<i>Gods that made me wet!  Take me... let\'s fuck right now, please!</i>" she begs.\n\n', false );
				if( CoC.player.hasItem( ConsumableLib.PURPLEG ) ) {
					CoC.player.consumeItem( ConsumableLib.PURPLEG );
					CoC.flags[ kFLAGS.AMILY_HIP_RATING ] += 1 + Utils.rand( 2 );
				} else {
			...
			//(If Amily has 'full, womanly hips':;
				...
				EngineCore.outputText( 'She chomps down on the proferred egg, but nothing happens. "<i>Slut\'s body is at its limits ' + CoC.player.mf( 'master', 'mistress' ) + '.  Her huge breeding hips can\'t widen any further.  Would you like to fuck?</i>" she asks, rocking her considerable mass back and forth hypnotically.', false );
				EngineCore.dynStats( 'lus', 4 );
			}
		}
		EngineCore.doNext( this.amilyFollowerEncounter );
	...
	AmilyScene.prototype.giveAmilySomePants = function() {
		EngineCore.outputText( '', true );
		this.amilySprite();
		EngineCore.outputText( 'You offer her a set of comfortable clothes, asking if she\'d like to wear these instead of her ' + CoC.flags[ kFLAGS.AMILY_CLOTHING ] + ' she\'s wearing.\n\n', false );
		...
		//(If Amily is wearing anything other than Tattered Rags:;
			...
			EngineCore.outputText( '"<i>More new clothes? Ooh, you\'re spoiling me, ' + CoC.player.short + '</i>!" she teases you. Unabashed in the slightest at being naked in front of you, she strips down, doing her best to give you a little show as she does so. She then redresses herself in her new offering. "<i>How do I look?</i>" she giggles.\n\n', false );
	...
	AmilyScene.prototype.corruptAmilyOralSuckOff = function() {
		...
		//[(if PC has a pussy);
		if( CoC.player.hasVagina() ) {
			EngineCore.outputText( '; getting some of your juices on her ' + ((CoC.flags[ kFLAGS.AMILY_NOT_FURRY ] === 0) ? 'muzzle' : 'lips'), false );
			if( CoC.player.balls > 0 ) {
		...
		if( CoC.player.balls > 0 ) {
			EngineCore.outputText( 'tickling your balls with her ' + ((CoC.flags[ kFLAGS.AMILY_NOT_FURRY ] === 0) ? 'fur' : 'skin'), false );
		}
	...
	AmilyScene.prototype.corruptAmilyLickPussiesLikeAPro = function() {
		...
		EngineCore.outputText( '"<i>It\'s time to eat,</i>" you declare and Amily instantly perks up after hearing it. You remove the lower half of your ' + CoC.player.armorName + ' and thrust your crotch at Amily; displaying your quickly ', false );
		if( CoC.player.hasCock() ) {
			EngineCore.outputText( 'hardening ' + Appearance.cockNoun( CoC.player.cocks[ 0 ].cockType ) + ' and ', false );
		}
		...
		EngineCore.outputText( '.  The ' + ((CoC.flags[ kFLAGS.AMILY_NOT_FURRY ] === 0) ? 'mousette' : 'succubus') + ' jumps slightly and you can almost imagine her grinning as she eats out her meal.\n\n', false );
		...
		// More [Horsecock];
		EngineCore.outputText( 'You move back and push Amily back into the ground, she hits the ground with a <b>THUD</b> and grins at you.  You move quickly, sitting on top of Amily\'s face and beginning using her mousy ' + ((CoC.flags[ kFLAGS.AMILY_NOT_FURRY ] === 0) ? 'muzzle' : 'nose') + ' like a dildo, rubbing it all over your pussy; her tongue darts out to give little licks and enhance your pleasure.\n\n', false );
		...
		if( CoC.player.vaginalCapacity() >= 45 && CoC.flags[ kFLAGS.AMILY_NOT_FURRY ] === 0 ) {
			...
			EngineCore.outputText( Descriptors.vaginaDescript() + ' , holding her ' + ((CoC.flags[ kFLAGS.AMILY_NOT_FURRY ] === 0) ? 'nose' : 'mouth') + ' closed; Amily\'s eyes bulge with surprise and you voice your pleasure with a long moan, "<i>I\'m not letting you breathe until you make me come slut. </i>" With those words, you begin frigging yourself against Amily\'s ', false );
			EngineCore.outputText( ((CoC.flags[ kFLAGS.AMILY_NOT_FURRY ] === 0) ? 'mousy muzzle' : 'face') + ', intent on driving yourself to orgasm as you feel her cheeks bulge with air and need.\n\n', false );
			// END [Horsecock];
		}
		...
		if( CoC.player.vaginalCapacity() >= 45 ) {
			EngineCore.outputText( 'You pull yourself off Amily\'s ' + ((CoC.flags[ kFLAGS.AMILY_NOT_FURRY ] === 0) ? 'muzzle' : 'face') + ' and moan deeply.', false );
		} else {
			EngineCore.outputText( 'You let go of Amily\'s ' + ((CoC.flags[ kFLAGS.AMILY_NOT_FURRY ] === 0) ? 'muzzle' : 'face') + ' and moan deeply.', false );
		}
	...
	AmilyScene.prototype.corruptAmilyScissorsLikeAPro = function() {
		...
		EngineCore.outputText( '"<i>Oh, Mistress... I love being your plaything,</i>" Amily replies, her spade tipped tail gently coiling around your ' + CoC.player.leg() + ' to tease your ' + Descriptors.vaginaDescript() + '.  "<i>Let me pleasure you, let me worship you, let me serve you, mistress.</i>"  With a diabolic grin, you sink down on top of your rodent sex-slave, giving her ' + this.amilyTits() + ' a good squeeze to emphasise it.  "<i>Well then, let\'s see how well you can do. Lie back and let me enjoy myself, slut.</i>"\n\n', false );
		...
		// [Horsecock];
		EngineCore.outputText( '"<i>Thank you so much mistress! I love to fuck! I love to be fucked! I love to be used! I love being your sextoy! I love the way your pussy feels against mine! I love you, mistress!</i>" Amily screams as an orgasm rocks her. She presses against you with all her strength, legs wrapping themselves around you, ' + ((CoC.flags[ kFLAGS.AMILY_NOT_FURRY ] === 0) ? 'claws' : 'feet') + ' curled in pleasure, eyes rolled back.', false );
		//[(if Amily's a squirter);
		if( CoC.flags[ kFLAGS.AMILY_VAGINAL_WETNESS ] >= 5 ) {
	...
	AmilyScene.prototype.corruptAmilysPussyGetsMotherfuckingFucked = function() {
		this.amilySprite();
		EngineCore.outputText( '', true );
		EngineCore.outputText( 'You tell Amily that you\'re feeling playful and tell her you\'d like to do some role-playing; you tell Amily to play the role of a demon out to seduce you. "<i>But I could never do that ' + CoC.player.mf( 'master', 'mistress' ) + ', the only role I\'m fit for is being your adoring slave!</i>"\n\n', false );
		EngineCore.outputText( 'You grin and tell her she should at least try. Amily smiles and replies, "<i>If that\'s what you want ' + CoC.player.mf( 'master', 'mistress' ) + ', I\'d be glad to try,</i>" Before starting, you tell Amily to try and act like a real demon.\n\n', false );
		EngineCore.outputText( 'You tell Amily to spread her legs and begin undressing, peeling off your ' + CoC.player.armorName + ' piece by piece. Amily doesn\'t even bother to speak to you; she simply grins in delight, moisture already beginning to flow from her gaping pink vagina.\n\n', false );
		//(If pregnant:;
		...
		EngineCore.outputText( '\n\n', false );
		EngineCore.outputText( 'Amily doesn\'t even roll her eyes, she is that much of a helplessly loyal slut for you. "<i>I want it, ' + CoC.player.mf( 'master', 'mistress' ) + '! I want it more than anything else! I\'m a cum-dumpster, a baby factory - I need cum in my hungry pussy, or I\'ll just fade away! Please, ' + CoC.player.mf( 'master', 'mistress' ) + ', pump me full of your hot, salty, gooey cum!', false );
		//(If not pregnant:;
		if( !this.pregnancy.isPregnant ) {
			EngineCore.outputText( '  I want you to fuck me full of cum, bloat me with seed until I\'m a ball of skin over spooge with a head and limbs sticking out! I need to be full and round with the very stuff of life; fucking cum into me, ' + CoC.player.mf( 'master', 'mistress' ) + '!', false );
		}
		//(If pregnant:;
		else {
			EngineCore.outputText( '  The babies need their cum, ' + CoC.player.mf( 'Master', 'Mistress' ) + '. They won\'t grow strong and healthy and slutty if you don\'t flood their womb with your hot baby-making juice! Please, cum and fill me with cum, for their sake?', false );
		}
		EngineCore.outputText( '</i>"\n\n', false );
		...
		EngineCore.outputText( '  Amily pants loudly and sighs in satisfaction. Hesitantly, one hand dares to reach out and stroke your ' + CoC.player.face() + '. "<i>Mmm... You are the light of my world, ' + CoC.player.mf( 'master', 'mistress' ) + '.', false );
		//(if not pregnant:;
		if( !this.pregnancy.isPregnant ) {
	...
	AmilyScene.prototype.corruptAmilyCampBonesPCWithHerCock = function() {
		...
		EngineCore.outputText( '"<i>Stay still, don\'t touch yourself and don\'t move.</i>" You order Amily, she responds with an eager, "<i>Yes ' + CoC.player.mf( 'master', 'mistress' ) + '!</i>" You continue looking at Amily\'s cock, carefully admiring your work.', false );
		//[(if Amily has balls);
		if( CoC.flags[ kFLAGS.AMILY_HAS_BALLS_AND_SIZE ] > 0 ) {
		...
		if( CoC.player.isBiped() ) {
			EngineCore.outputText( '  You begin stroking her limp dick with your ' + CoC.player.foot() + ', ', false );
		}//(else);
		else {
	...
	AmilyScene.prototype.corruptAmilyBuckFutter = function() {
		...
		EngineCore.outputText( '', true );
		//[Anal - non-mutant];
		EngineCore.outputText( 'You order Amily to get on her hands and knees; you want to pound her ass. Amily\'s eyes light up in happiness and she replies excitedly, "<i>Yes, ' + CoC.player.mf( 'master', 'mistress' ) + '</i>!" She quickly turns around and gets in position; her full ass swaying invitingly, while her tail stands erect to give you access. Her excitement at the prospect of having her ass fucked by you is made evident by the trickles of moisture that run down her legs', false );
		if( CoC.flags[ kFLAGS.AMILY_WANG_LENGTH ] > 0 ) {
	...
	AmilyScene.prototype.amilyPostConfessionGirlRemeeting = function() {
		this.amilySprite();
		EngineCore.outputText( '', true );
		EngineCore.outputText( 'Amily looks happy to see you, as usual, but shy as well. "<i>Ah... ' + CoC.player.short + '... it\'s good to see you again.</i>"\n\n', false );
	...
	AmilyScene.prototype.hermilyOnFemalePC = function() {
		...
		if( CoC.player.isBiped() ) {
			EngineCore.outputText( CoC.player.legs() + ' in readiness for her.' );
		} else {
	...
	AmilyScene.prototype.pcBirthsAmilysKidsQuestVersion = function() {
		...
		if( CoC.flags[ kFLAGS.PC_TIMES_BIRTHED_AMILYKIDS ] + CoC.flags[ kFLAGS.AMILY_BIRTH_TOTAL ] >= 5 ) {
			...
			EngineCore.outputText( ((CoC.flags[ kFLAGS.AMILY_NOT_FURRY ] === 0) ? ', naked, pink, and totally hairless' : '') + '. Amily helps hold it to your ' + Descriptors.breastDescript( 0 ) + ', where it eagerly takes hold of your ' + Descriptors.nippleDescript( 0 ) + ' and starts to suckle. As it drinks, it starts to grow larger, and ' + ((CoC.flags[ kFLAGS.AMILY_NOT_FURRY ] === 0) ? 'fur the same color as your own hair starts to cover its body' : '') + '. It quickly drinks its fill and then detaches, its \'father\' putting it aside, which is good, because by this time there\'s another baby waiting for its turn... and another... and another...\n\n', false );
		...
		EngineCore.outputText( 'Any reply you may have been inclined to make to that is swallowed by another cry of pain as yet another contraction wrings its way through you. Amily takes your hand in hers and you cling to the lifeline of comfort it offers, thankful to not be alone for this. You can feel the first child moving out of your womb, through your cervix, down and into your ' + Descriptors.vaginaDescript() + '. Your lips part and, with a grunt, you expel the first child into Amily\'s waiting hand. She holds it up to you so that you can see your firstborn; it\'s a little mouselet', false );
		//(if player is female: 1 in 3 chance of it being boy, girl or herm, if player is herm, 100% chance of it being a herm);
		EngineCore.outputText( ((CoC.flags[ kFLAGS.AMILY_NOT_FURRY ] === 0) ? ', naked, pink, and totally hairless' : '') + '. Amily helps hold it to your ' + Descriptors.chestDesc() + ', where it eagerly takes hold of your ' + Descriptors.nippleDescript( 0 ) + ' and starts to suckle. As it drinks, it starts to grow larger, and ' + ((CoC.flags[ kFLAGS.AMILY_NOT_FURRY ] === 0) ? 'fur the same color as your own hair starts to cover its body' : '') + '. It quickly drinks its fill and then detaches, its \'father\' putting it aside, which is good, because by this time there\'s another baby waiting for its turn... and another... and another...\n\n', false );
		EngineCore.outputText( 'Soon, you are back to your old self again, lying down in exhaustion with Amily sitting nearby, your many rambunctious offspring already starting to walk and play around you.\n\n', false );
			...
			//[Female to Herm];
				...
				if( CoC.flags[ kFLAGS.AMILY_WANG_LENGTH ] === 0 ) {
					...
					//Medium Affection:;
						...
						EngineCore.outputText( '"<i>I... don\'t take this the wrong way, ' + CoC.player.short + ', but... I\'m not so sure we can be together any more while you have that. Be one thing or the other, not both.</i>" Amily states. "<i>But we can still talk, this time.</i>"', false );
	...
	AmilyScene.prototype.amilyPopsOutKidsInCamp = function() {
		...
		if( CoC.flags[ kFLAGS.AMILY_FOLLOWER ] === 1 ) {
			...
			EngineCore.outputText( 'Worry fills you; how on earth are you going to take care of such small, defenseless creatures in a place like this? Before you can voice such concerns to Amily, though, rustling in the bushes catches your attention. As you watch, a pair of young ' + ((CoC.flags[ kFLAGS.AMILY_NOT_FURRY ] === 0) ? 'mouse-morphs' : 'mouse fellows') + ' appear and walk confidently into your camp. They\'re two of your children, the ones who Amily told you had left the area.\n\n', false );
			EngineCore.outputText( '"<i>I hope you don\'t mind, ' + CoC.player.short + ', but I contacted them to come and take their new siblings back with them shortly after we confirmed I was pregnant.</i>" Amily says, sheepishly. "<i>We just can\'t really care for them here, and they\'ll be safer and happier with all their siblings.</i>"\n\n', false );
			EngineCore.outputText( 'You nod your head and admit that you agree. Once you have regained your strength, you spend some time talking with your fully adult children and playing with your overdeveloped newborns. Then, with a final wave goodbye, the ' + ((CoC.flags[ kFLAGS.AMILY_NOT_FURRY ] === 0) ? 'mouse-morphs' : 'family of mice') + ' disappear into the wilderness once more.', false );
		}
		//CORRUPT!;
		else {
			...
			EngineCore.outputText( 'When the last ' + ((CoC.flags[ kFLAGS.AMILY_NOT_FURRY ] === 0) ? 'mouse' : 'mouse-girl') + ' is finally free of the confines of its slutty mother\'s cunt, they all crawl up towards their mother\'s breasts and each takes their turn drinking from their mother\'s corrupt milk. You just smile and observe; Amily\'s babies come in various flavors, some are girls, some are boys, and some are even both; they all share the same lewd purple ' + ((CoC.flags[ kFLAGS.AMILY_NOT_FURRY ] === 0) ? 'fur' : 'skin tone') + ' of their mother; you can\'t help but wonder if they\'ll be wonderful cum-sluts like their mother. As they drink, you see the little babies grow bigger; and that\'s not all, you see little cocks getting bigger, balls developing, breasts expanding, pussies juicing up.  The sight fills you with joy.\n\n', false );
			...
			if( CoC.player.hasCock() && (CoC.player.gender === 1 || Utils.rand( 2 ) === 0) ) {
				...
				//[(if PC has large cum amount);
				if( CoC.player.cumQ() >= 500 ) {
					EngineCore.outputText( '  Her belly distends, but she continues drinking and suckling as efficiently as ever; making sure not to spill even a single drop of her ' + CoC.player.mf( 'master', 'mistress' ) + '\' precious seed; the sight is enough to make you swell with pride at how seriously Amily takes her job; you decide to reward her with even more spunk.', false );
				}
				EngineCore.outputText( '\n\n', false );
			}
			//[=Pussy=];
				...
				EngineCore.outputText( Descriptors.vaginaDescript(), false );
				EngineCore.outputText( '; Amily licks her lips in anticipation. You squat above Amily, hovering over her ' + ((CoC.flags[ kFLAGS.AMILY_NOT_FURRY ] === 0) ? 'muzzle' : 'face') + '.', false );
				//[(if PC has balls);
				if( CoC.player.balls > 0 ) {
	...
	AmilyScene.prototype.talkWithCORRUPTCUNT = function( sexAfter ) {
		...
		if( convo === 0 ) {
			...
			EngineCore.outputText( '"<i>' + CoC.player.mf( 'Master', 'Mistress' ) + ', I have some experience in alchemy, so if you wish I can share some of what I know with you,</i>" she says.\n\n', false );
			//(If player has no main item:);
			if( CoC.player.itemSlot1.quantity === 0 ) {
			...
			//Succubus Milk/Incubus Draft:;
				...
				EngineCore.outputText( 'She hums at the scent. "<i>That\'s demon fluid, ' + CoC.player.mf( 'master', 'mistress' ) + '. It\'d be like drinking liquid corruption. It could turn you into a demon, or supercharge your feminity or masculinity. I\'ve heard it can even mess with your gender if you drink too much of the opposite stuff. But none of that is important, I\'d be happy to drink from you no matter what gender you are ' + CoC.player.mf( 'master', 'mistress' ) + ',</i>" she says with a grin.\n\n', false );
			}
			//Succubi's Delight:;
			else if( CoC.player.itemSlot1.itype === ConsumableLib.SDELITE ) {
		...
		//Conversation: Sand Witches;
			...
			EngineCore.outputText( '"<i>I don\'t know... Maybe make me into some kind of milk-producing slave? That wouldn\'t do... if I became their slave I would never have been able to become your slave, ' + CoC.player.mf( 'master', 'mistress' ) + ',</i>" she responds, grinning at you.\n\n', false );
			...
			EngineCore.outputText( 'You tell her no, when you want something inside her pussy you can shove it in yourself.  "<i>' + CoC.player.mf( 'Master', 'Mistress' ) + ', you\'re going to make me horny saying things like that,</i>" she says in mock embarassment. You laugh and dismiss her with a wave.\n\n', false );
		}
		//Conversation: Giant Bees;
		else if( convo === 3 ) {
			...
			EngineCore.outputText( '"<i>Well... the Giant Bees you\'ve probably seen ' + CoC.player.mf( 'master', 'mistress' ) + '... the official name for them is the fertile caste, but folks tend to just call them handmaidens,</i>" Amily explains.\n\n', false );
			EngineCore.outputText( 'At your confused look, she continues.\n\n', false );
			...
			EngineCore.outputText( 'You smile and tell her you\'ll stuff her ass later. "<i>Thank you, ' + CoC.player.mf( 'master', 'mistress' ) + ',</i>" she says, smiling. You dismiss her with a wave.\n\n', false );
		}
		//Conversation: Fetish Cultists & Zealouts;
		else if( convo === 4 ) {
			...
			EngineCore.outputText( 'Amily looks down to the floor and says, "<i>Forgive your ignorant juice slave, ' + CoC.player.mf( 'master', 'mistress' ) + '. I haven\'t had much contact with them so I can\'t tell you much. It looks like they have some weird magic that lets them change their clothes at random.</i>"\n\n', false );
			EngineCore.outputText( '"<i>If you want ' + CoC.player.mf( 'master', 'mistress' ) + ', I could dress up for you, I\'d love to wear any kind of kinky wear you would give me. Even if it was just ropes for tying me down.</i>" Amily adds, smiling.\n\n', false );
		...
		//Conversation: Imps;
			...
			if( CoC.player.cockArea( 0 ) < 24 && CoC.player.hasCock() ) {
				EngineCore.outputText( '"<i>' + CoC.player.mf( 'Master', 'Mistress' ) + ', can you grow a bigger cock to fuck your loyal cumdumpster?</i>"', false );
			}//(else if PC has a dick >= 24 area);
			else if( CoC.player.hasCock() ) {
				EngineCore.outputText( '"<i>' + CoC.player.mf( 'Master', 'Mistress' ) + ' I\'m so horny, can you take your huge cock and jam it into your slutty mouse slave?</i>"', false );
			}//(else);
			else {
				EngineCore.outputText( '"<i>' + CoC.player.mf( 'Master', 'Mistress' ) + ', can you grow a huge dick to fuck your needy cumslut?</i>"', false );
			}
		...
		//Conversation: Shark Girls;
			...
			EngineCore.outputText( '"<i>Those are Shark Girls, ' + CoC.player.mf( 'master', 'mistress' ) + '. I think they used to be a village of humans who lived on the lake... then the lake was contaminated, and they all turned into the lovely sharks they are now,</i>" Amily says with a smile.\n\n', false );
			if( !SceneLib.izmaScene.izmaFollower() ) {
			...
			EngineCore.outputText( 'She looks at you with lust-filled eyes. "<i>' + CoC.player.mf( 'Master', 'Mistress' ) + ', let\'s fuck like them; spank me, hit me and then fuck me. Cum all over me, make me feel like the cumslut that I am,</i>" she begs, openly panting.\n\n', false );
		...
		//Conversation: Goblins;
			...
			EngineCore.outputText( 'She looks to you pleadingly and says, "<i>I love getting knocked up too, ' + CoC.player.mf( 'master', 'mistress' ) + '. I could birth you many mice to worship you like you deserve; and then we could all fuck. Oh, ' + CoC.player.mf( 'master', 'mistress' ) + '!', false );
			//[(if Amily's pregnant);
			if( this.pregnancy.isPregnant ) {
		...
		//Conversation: What was life in her village like?;
			...
			EngineCore.outputText( 'You decide to amuse yourself and ask Amily about her past. She rolls her eyes as if just the idea of breaching the subject was boring, but a quick glare puts her back in her place. "<i>Umm... As you wish, ' + CoC.player.mf( 'master', 'mistress' ) + ',</i>" she replies meekly.\n\n', false );
			...
			EngineCore.outputText( 'You approach her and pat her head, allowing her to nuzzle your crotch lovingly. You tell her not to worry; you will fuck her much more in the future. "<i>Cum-pet can\'t wait for that, ' + CoC.player.mf( 'master', 'mistress' ) + ',</i>" she replies. You dismiss her with a wave.\n\n', false );
		}
		//Conversation: Who was she before it was all lost?;
		else if( convo === 9 ) {
			EngineCore.outputText( 'You decide to amuse yourself and ask Amily about her past. She rolls her eyes as if just the idea of breaching the subject was boring, but a quick glare puts her back in her place. "<i>Umm... As you wish, ' + CoC.player.mf( 'master', 'mistress' ) + ',</i>" She replies meekly.\n\n', false );
			...
			EngineCore.outputText( 'You pat her head, and then push her away. She settles herself back on her knees. "<i>Sorry about that, ' + CoC.player.mf( 'master', 'mistress' ) + ',</i>" she says, a bit ashamed of her little outburst.  You ask her what the fate of her parents was.\n\n', false );
			EngineCore.outputText( '"<i>Who knows? Probably got caught by some inferior demon and made to be their slaves,</i>" she says shrugging. "<i>I\'m just happy I got caught by a good ' + CoC.player.mf( 'master', 'mistress' ) + '.</i>" She smiles. You smile back and dismiss her with a wave.\n\n', false );
		}
		//Conversation: How did it fall?;
		else if( convo === 10 ) {
			EngineCore.outputText( 'You decide to amuse yourself and ask Amily about her past. She rolls her eyes as if just the idea of breaching the subject was boring, but a quick glare puts her back in her place. "<i>Umm... As you wish, ' + CoC.player.mf( 'master', 'mistress' ) + ',</i>" she replies meekly.\n\n', false );
	...
	AmilyScene.prototype.rapeCorruptAmily2Female = function() {
		...
		//[(if PC is a squirter);
		if( CoC.player.wetness() >= 5 ) {
			EngineCore.outputText( '  Your juices rocket into her face, painting her ' + ((CoC.flags[ kFLAGS.AMILY_NOT_FURRY ] === 0) ? 'muzzle' : 'face') + ' in your femcum as she giggles with happiness and pleasure.', false );
		}
	...
	AmilyScene.prototype.rapeCorruptAmily3Epilogue = function() {
		...
		EngineCore.outputText( '"<i>Please! Wait, ' + CoC.player.mf( 'master', 'mistress' ) + ', please!</i>" Amily yells, you turn around and look back at her. "<i>I want more. Please,</i>" she begs.\n\n', false );
	...
	AmilyScene.prototype.rapeCorruptAmily4Meeting = function() {
		...
		EngineCore.outputText( 'As soon as she realizes you\'re there, she drops the dildo and rushes towards you; kneeling submissively in front of you, she nuzzles your crotch and asks, "<i>' + CoC.player.mf( 'Master', 'Mistress' ) + ', did you come to feed me?</i>"\n\n', false );
	...
	AmilyScene.prototype.rapeCorruptAmily4Male = function() {
		this.amilySprite();
		EngineCore.outputText( '', true );
		EngineCore.outputText( 'You slowly strip off your ' + CoC.player.armorName + ', while Amily pants in anticipation. When you\'re done you present to her your erect ' + Descriptors.cockDescript( 0 ) + '; she quickly nuzzles and kisses along your length, rubbing her breasts along your length', false );
		//[(if pc has balls);
		if( CoC.player.balls > 0 ) {
		...
		EngineCore.outputText( 'Moaning, she drops on fours and crawls toward you to grovel at your ' + CoC.player.feet() + ', rubbing her face on the very ground you stand on, and begging all the while.\n\n', false );
		EngineCore.outputText( '"<i>Please ' + CoC.player.mf( 'master', 'mistress' ) + ', make me your cumbucket. There is nothing I would love more than to become a receptacle for your lust. This would be like a dream come true! Please, oh please ' + CoC.player.mf( 'master', 'mistress' ) + '! Knock me up and allow me to birth more mice to worship you like you deserve.</i>"\n\n', false );
		...
		if( CoC.player.isNaga() ) {
			...
			EngineCore.outputText( 'put a ' + CoC.player.foot() + ' on her head.', false );
		}
		EngineCore.outputText( ' She falls silent and you say. "<i>Very well. From now on I am your one and only ' + CoC.player.mf( 'master', 'mistress' ) + '.  You will address me as such, always. My word is law, and you shall do whatever I tell you to, whenever I tell you to. Rise, it\'s time to break you into your new role as my cum-dumpster.</i>"\n\n', false );
	...
	AmilyScene.prototype.rapeCorruptAmily4Female = function() {
		this.amilySprite();
		EngineCore.outputText( '', true );
		EngineCore.outputText( 'You slowly strip off your ' + CoC.player.armorName + ', while Amily pants in anticipation. When you\'re done you present to her your dripping ' + Descriptors.vaginaDescript() + '; she quickly nuzzles and kisses your clit.', false );
		//[(if PC has balls);
		if( CoC.player.balls > 0 ) {
		...
		EngineCore.outputText( 'Moaning, she drops on all fours and crawls toward you to grovel at your ' + CoC.player.feet() + ', rubbing her face on the very ground you stand on, and begging all the while.\n\n', false );
		EngineCore.outputText( '"<i>Please ' + CoC.player.mf( 'master', 'mistress' ) + ', make me your cum-bucket. There is nothing I would love more than to become a receptacle for your lust. This would be like a dream come true! Please, oh please ' + CoC.player.mf( 'master', 'mistress' ) + '!  Allow me to knock you up so you can birth more mice to worship you like you deserve.</i>"\n\n', false );
		EngineCore.outputText( 'Her words please you, and you ', false );
		if( CoC.player.isNaga() ) {
			EngineCore.outputText( 'coil your serpentine tail around her head.', false );
		} else if( CoC.player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_GOO ) {
	...
	AmilyScene.prototype.rapeCorruptAmily4Epilogue = function() {
		...
		EngineCore.outputText( 'She looks herself over, examining and enjoying her new sensitive body; every movement is accompanied with a pleasured sigh or a moan. She looks at you and kneels in front of you, a rush of fluids escapes her pussy as she does so. "<i>' + CoC.player.mf( 'Master', 'Mistress' ) + '... What are your orders?</i>" Amily asks with a seductive tone.\n\n', false );
		...
		EngineCore.outputText( 'Amily grins and replies seductively, "<i>Yes, ' + CoC.player.mf( 'master', 'mistress' ) + '... Your orders are my pleasure.</i>"\n\n', false );
	...
	AmilyScene.prototype.stalkingZeAmiliez = function() {
		...
		//(if PC's intelligence >= 50);
			...
			EngineCore.outputText( 'You cautiously survey the surroundings, and finally spot some recent tracks made by Amily. It\'s clear she tried to hide them, but using your superior wits, you not only find the routes leading to all of her hideouts, but deduce which hideout she\'ll use next and decide to hide inside; now, all that\'s left to do is wait...\n\n', false );
			EngineCore.outputText( 'You wait for quite some time, but eventually you hear Amily approaching; she checks the hideout for any signs of intrusions, but you have concealed your presence well and she finds nothing. Satisfied, she sets down a bag with some fruits she gathered and sits down on a stool, letting out a sigh of relief.\n\n', false );
			EngineCore.outputText( 'This is your cue to act; you quickly burst out of your hideout and swipe her blowpipe away. Amily jumps away in surprise and reaches for her knife, assuming a fighting stance. You ready your ' + CoC.player.weaponName + ' and prepare to teach the foolish mouse a lesson.\n\n', false );
			//[Proceed to battle.];
		...
		//(if PC's speed >= 65);
			...
			EngineCore.outputText( 'Panicked, she takes her knife and prepares to fight you. You ready your ' + CoC.player.weaponName + ' and prepare to teach the foolish mouse a lesson.\n\n', false );
			//[Proceed to battle.];
			Combat.startCombat( new Amily(), true );
		} else {
	...
	AmilyScene.prototype.stalkingZeAmiliez3 = function() {
		...
		EngineCore.outputText( 'Amily plants her head on the ground and starts. "<i>' + CoC.player.mf( 'Master', 'Mistress' ) + ', please let me drink more of your cum. Please! I need it so bad that I can\'t help but drool. I need it so bad that not being able to drink from you is like torture. Please let me drink!</i>"\n\n', false );
	...
	AmilyScene.prototype.amilyReturns = function() {
		...
		EngineCore.outputText( '"<i>Thank you, ' + CoC.player.short + '. I don\'t know how you did it, or if it had anything to do with me, but you managed to do something that I\'ve never heard of anyone else having the will to do. You turned back from demonhood and repurified yourself again.</i>"\n\n', false );
	...
	AmilyScene.prototype.dateNightFirstTime = function() {
		...
		EngineCore.outputText( 'Amily smiles, as friendly as she can be when placed in such an awkward situation, "<i>Hi! My name\'s Amily - ' + CoC.player.short + '\'s mate.</i>"  You give Amily\'s hand a little squeeze, proud of her for not overreacting to Urta\'s casual drop of the word "<i>lover.</i>"\n\n', false );
		EngineCore.outputText( '"<i>Oh, ' + CoC.player.mf( 'his', 'her' ) + '... mate...</i>"  Urta mumbles, looking a little crestfallen at the admission.  Her emerald eyes light up with an idea while a mischievous grin spreads over her vulpine muzzle.  Before she can act on it, you request her to allow the two of you through the gates, interrupting whatever tricks the clever fox has planned.  She retrieves the corruption-detecting gem she once used on you and gives Amily a quick sweep.  When the gem emits a white glow, Urta gives an authoritative nod and calls for the gates to be opened.\n\n', false );
	...
	AmilyScene.prototype.liqueurUpTheWaifus = function() {
		...
		EngineCore.outputText( 'With a drunken grin, Urta laughs, "<i>Oh, shit. Sorry, sweetie, I figured ' + CoC.player.short + ' woulda\' blabbed about my third leg by now...</i>" She leans over the table, nearly brushing noses with Amily as she adds, "<i>Hey... Wanna give it a try?</i>"\n\n', false );
	...
	AmilyScene.prototype.fuckPureAmilysHeiny = function() {
		...
		if( CoC.flags[ kFLAGS.TIMES_FUCKED_AMILYBUTT ] === 0 ) {
			...
			EngineCore.outputText( 'You smile broadly, nodding, and assure her you\'ll be gentle as you begin to strip out of your ' + CoC.player.armorName + '.\n\n', false );
		...
		EngineCore.outputText( 'She wipes her hand off on a discreet corner of her ' + CoC.flags[ kFLAGS.AMILY_CLOTHING ] + ' and begins to shimmy out of them.', false );
		if( CoC.flags[ kFLAGS.TIMES_FUCKED_AMILYBUTT ] === 0 ) {
	...
	AmilyScene.prototype.fuckIncestCunts = function( all ) {
		...
		EngineCore.outputText( 'at you and says, "<i>Hey, ' + CoC.player.mf( 'Dad', 'other Mom' ) + ', go ahead and put it in.  I\'ve been wanting this since the first time I saw you.</i>"' );
