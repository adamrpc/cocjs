...
angular.module( 'cocjs' ).run( function( MainView, SceneLib, OnLoadVariables, Hel, PerkLib, Descriptors, AppearanceDefs, CoC, kFLAGS, Utils, StatusAffects, EngineCore, ConsumableLib ) {
	...
	HelFollower.prototype.helCapacity = function() {
		return 85;
	};
	HelFollower.prototype.helAnalCapacity = function() {
		var anal = 85;
		if( CoC.flags[ kFLAGS.HELIA_ANAL_TRAINING ] >= 1 ) {
			anal += 100;
		}
		if( CoC.flags[ kFLAGS.HELIA_ANAL_TRAINING ] >= 2 ) {
			anal += 300;
		}
		return anal;
	};
	HelFollower.prototype.heliaCapacity = HelFollower.prototype.helCapacity;
	HelFollower.prototype.heliaAnalCapacity = HelFollower.prototype.helAnalCapacity;
	HelFollower.prototype.helAffection = function( diff ) {
		if( diff === undefined ) {
			diff = 0;
		}
		if( CoC.flags[ kFLAGS.HEL_AFFECTION_FOLLOWER ] > 70 && CoC.flags[ kFLAGS.HEL_HARPY_QUEEN_DEFEATED ] === 0 ) {
			CoC.flags[ kFLAGS.HEL_AFFECTION_FOLLOWER ] = 70;
		}
		if( CoC.flags[ kFLAGS.HEL_AFFECTION_FOLLOWER ] < 100 || (CoC.flags[ kFLAGS.HEL_BONUS_POINTS ] === 0 && diff < 0) ) {
			CoC.flags[ kFLAGS.HEL_AFFECTION_FOLLOWER ] += diff;
			if( CoC.flags[ kFLAGS.HEL_AFFECTION_FOLLOWER ] >= 100 ) {
				CoC.flags[ kFLAGS.HEL_AFFECTION_FOLLOWER ] = 100;
			}
			if( CoC.flags[ kFLAGS.HEL_AFFECTION_FOLLOWER ] < 0 ) {
				CoC.flags[ kFLAGS.HEL_AFFECTION_FOLLOWER ] = 0;
			}
		} else if( SceneLib.helScene.followerHel() ) {
			CoC.flags[ kFLAGS.HEL_AFFECTION_FOLLOWER ] = 100;
			CoC.flags[ kFLAGS.HEL_BONUS_POINTS ] += diff * 3;
			if( diff > 0 ) {
				if( CoC.flags[ kFLAGS.HEL_BONUS_POINTS ] > 150 ) {
					CoC.flags[ kFLAGS.HEL_BONUS_POINTS ] = 150;
				}
			} else if( diff < 0 ) {
				if( CoC.flags[ kFLAGS.HEL_BONUS_POINTS ] < 0 ) {
					CoC.flags[ kFLAGS.HEL_BONUS_POINTS ] = 0;
				}
			}
		}
		return CoC.flags[ kFLAGS.HEL_AFFECTION_FOLLOWER ];
	};
	HelFollower.prototype.isHeliaBirthday = function() {
		return OnLoadVariables.date.month === 7;
	};
	...
	HelFollower.prototype.heliaFollowerIntro = function() {
		...
		if( CoC.flags[ kFLAGS.HARPY_QUEEN_EXECUTED ] > 0 ) {
			MainView.outputText( 'kill' );
		} else {
			MainView.outputText( 'bring down' );
		}
		MainView.outputText( ' the Harpy Queen... and getting my sister out of that shithole.  For giving me a chance to meet my dad.  For everything.</i>"  You start to tell her that you\'re happy to help, but Hel cuts you off, speaking quickly, Dad, and I have been trying to make it work, living out on the plains - we really have - but Dad\'s not in any shape to fight and Kiri\'s no good at it; the plains are just too dangerous for them and Dad\'s been having nightmares about mom, and..."</i> she pauses to take a gasping breath, unaccustomed to talking so quickly.' );
	...
	HelFollower.prototype.iCantLetFireButtsRapeMyCampsButt = function() {
		...
		if( CoC.flags[ kFLAGS.HEL_FOXY_FOURSOME_WARNED ] > 0 || CoC.flags[ kFLAGS.HEL_EDRYN_OFFER ] > 0 ) {
			MainView.outputText( 'like usual' );
		}
		MainView.outputText( ', and maybe you and I can spend some quality time together.  Alright?"</i>' );
	...
	HelFollower.prototype.afterMoveInBoningAnalFireTail = function() {
		MainView.clearOutput();
		MainView.outputText( '<b>An hour later...</b>' );
	...
	HelFollower.prototype.justFriendsWithAnalTailWaifu = function() {
		MainView.clearOutput();
		MainView.outputText( 'Awkwardly, you spend the next few minutes saying that, while you appreciate the offer and her affections, you\'re more interested in just being friends with Hel, as you have been for some time.' );
		...
		MainView.outputText( ' to give you a little peck on the cheek.  <i>"Well, that\'s that, then.  I guess.  Well, see you around, [name]."</i>' );
		MainView.outputText( '\n\n<i>"Sure will."</i>' );
		MainView.outputText( '\n\nYou wave as Hel retreats back toward her own home.' );
		CoC.flags[ kFLAGS.HELIA_FOLLOWER_DISABLED ] = 0.5;
	...
	HelFollower.prototype.helFollowersIntro = function() {
		...
		if( SceneLib.kihaFollower.followerKiha() && CoC.flags[ kFLAGS.HEL_INTROS_LEVEL ] < 1 ) {
			CoC.flags[ kFLAGS.HEL_INTROS_LEVEL ] = 1;
			MainView.outputText( 'You pace around camp, awaiting the return of your new companion.  After several minutes, you notice Kiha sitting behind you, eyeing you with an eyebrow cocked.  Suddenly self-conscious, you stop and face the dragon-girl.' );
		...
		//If Isabella ;
		else if( SceneLib.isabellaFollowerScene.isabellaFollower() && CoC.flags[ kFLAGS.HEL_INTROS_LEVEL ] < 2 ) {
			CoC.flags[ kFLAGS.HEL_INTROS_LEVEL ] = 2;
			//is at Camp (She and Hel are cool);
			if( CoC.flags[ kFLAGS.HEL_ISABELLA_THREESOME_ENABLED ] > 0 ) {
				MainView.outputText( 'Showing Hel around, you eventually come to the part of your camp inhabited by the towering cowgirl Isabella.  When you arrive, you find Isabella reclining in her armchair, humming a sweet melody as she cleans her tower shield.' );
			...
			//If Isabella is at Camp (And she and Hel are NOT cool);
				...
				if( CoC.player.cor < 50 ) {
					MainView.outputText( 'fall silent under their considering gazes.' );
				}
				//(corruption = high enough to call bitches out when they're hypocrites);
				else {
					MainView.outputText( 'glower balefully at them, as if to challenge either to invite your opinion of their own sexual \'résumé\' with an ill-considered comment.' );
				}
				...
				if( SceneLib.isabellaFollowerScene.isabellaAccent() ) {
					MainView.outputText( '<i>"You are... velcome,"</i> Isabella says before collecting some of the scattered belongings from the ground.  You continue your tour, now that the girls are...  not going to murder each other in the middle of the night, at least.' );
				} else {
					MainView.outputText( '<i>"You\'re... welcome,"</i> Isabella says before collecting some of the scattered belongings from the ground.  You continue your tour, now that the girls are...  not going to murder each other in the middle of the night, at least.' );
				}
		...
		//If Bimbo Sophie is at Camp:;
		else if( CoC.flags[ kFLAGS.HEL_INTROS_LEVEL ] < 4 && SceneLib.sophieBimbo.bimboSophie() && CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_SOPHIE ] === 0 ) {
			CoC.flags[ kFLAGS.HEL_INTROS_LEVEL ] = 4;
			...
			MainView.outputText( '.' );
			MainView.outputText( '\n\n<i>"Yay!  My special little cutie is back!</i>"  Sophie exclaims, giggling as she wraps her feathered arms around you.  "<i>Is my ' + CoC.player.mf( 'hunk', 'babe' ) + ' interested in like, some \'fun\'?"</i>' );
	...
	//[Boot Sophie];
	HelFollower.prototype.bimboSophieGetsBooted4Firebutt = function() {
		MainView.clearOutput();
		MainView.outputText( 'You sigh and pick Sophie up, slinging the harpy over your shoulders.  <i>"Yaaaay~"</i> she cheers.  </i>"I\'m going for a ride!"</i>' );
		...
		if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00326 ] >= 3 ) {
			...
			CoC.flags[ kFLAGS.MINO_SONS_HAVE_SOPHIE ] = 1;
		}
	...
	HelFollower.prototype.bootHelOutForBimboSophie = function() {
		MainView.clearOutput();
		MainView.outputText( '<i>"Nope, you get out,"</i> you answer, scowling at the salamander.' );
	...
	HelFollower.prototype.workItOutWithSophieAndFireTits = function() {
		MainView.clearOutput();
		MainView.outputText( 'You cross your arms and tell Hel to deal with it, remarking that you won\'t just dump Sophie out in the wilderness - she can\'t take care of herself - but you still want Hel around.' );
		...
		CoC.flags[ kFLAGS.KEEP_HELIA_AND_SOPHIE ] = 1;
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	//If Hel is at Camp and Isabella Arrives, neither are cool;
	HelFollower.prototype.angryHelAndIzzyCampHelHereFirst = function() {
		MainView.clearOutput();
		MainView.outputText( 'Showing Isabella around, you eventually come to the chaotic, cluttered part of camp inhabited by Hel the salamander, who\'s currently sitting on her hammock sharpening her scimitar.' );
		MainView.outputText( '\n\nYou only have a moment to remember Hel\'s disdain for the cowgirl before...' );
		...
		if( SceneLib.isabellaFollowerScene.isabellaAccent() ) {
			MainView.outputText( 'V' );
		} else {
			MainView.outputText( 'W' );
		}
		MainView.outputText( 'hat are YOU doing here?!"</i>' );
		MainView.outputText( '\n\nBefore the two of them can come to further violence, you leap between them and try to hold them apart.  You suffer the brunt of their aggression for a few moments, but manage to hold them off for the moment until they come down off their combat high.' );
		//(Continued Below);
	...
	HelFollower.prototype.heliaFollowerMenu = function( display ) {
		...
		if( CoC.flags[ kFLAGS.HEL_FOLLOWER_LEVEL ] === 2 ) {
			...
			//Take a Bath;
			//Appearance;
			EngineCore.addButton( 0, 'Appearance', SceneLib.helSpawnScene, SceneLib.helSpawnScene.heliasAppearanceScreen );
			EngineCore.addButton( 1, 'Sex', this, this.heliaRoughSex );
			EngineCore.addButton( 2, 'Threesomes', this, this.heliaThreesomes );
			EngineCore.addButton( 4, 'Talk', this, this.heliaOptions );
			if( !SceneLib.helScene.pregnancy.isPregnant ) {
				EngineCore.addButton( 5, 'Spar', this, this.sparWithHeliaFirebuttsAreHot );
			} else {
				MainView.outputText( '\n\n<b>Helia will not spar or box while pregnant.</b>' );
			}
			if( !SceneLib.helScene.pregnancy.isPregnant ) {
				EngineCore.addButton( 6, 'Box', this, this.boxWithInCampHel );
			}
			if( CoC.flags[ kFLAGS.HEL_LOVE ] === 1 || CoC.flags[ kFLAGS.HEL_LOVE ] === -1 ) {
				if( CoC.player.hasCock() && CoC.player.cockThatFits( this.heliaCapacity() ) >= 0 && CoC.player.lust >= 33 && !SceneLib.helSpawnScene.helPregnant() && CoC.flags[ kFLAGS.HELSPAWN_AGE ] === 0 ) {
					EngineCore.addButton( 7, 'Have A Kid', SceneLib.helSpawnScene, SceneLib.helSpawnScene.haveAKid );
				}
			}
			...
			if( CoC.flags[ kFLAGS.HEL_HARPY_QUEEN_DEFEATED ] === 1 ) {
				if( display ) {
					MainView.outputText( '(You\'ve reached the culmination of Helia\'s current storyline.  Stay tuned for more!)' );
				}
			} else {
				if( display ) {
					MainView.outputText( 'You approach Hel as she\'s pacing around camp.  She\'s clad in her normal field attire simple scale bikini top and leather thong which supports her scimitar\'s scabbard.  Her cloak is loosely thrown over her shoulders, giving her a slight measure of protection from the mountain\'s harsh environs.' );
				}
				if( display ) {
					MainView.outputText( '\n\n"<i>Heya, [name]! Ready to hit the road?</i>"' );
				}
				//(Display Options: [Dungeon] [Not Yet]);
				EngineCore.choices( 'Dungeon', SceneLib.dungeonHelSupplimental, SceneLib.dungeonHelSupplimental.goToHeliaDungeon, '', null, null, '', null, null, '', null, null, 'Not Yet', SceneLib.dungeonHelSupplimental, SceneLib.dungeonHelSupplimental.notYet );
			}
		}
	};
	HelFollower.prototype.heliaOptions = function() {
		if( SceneLib.helScene.pregnancy.event >= 3 && CoC.flags[ kFLAGS.HELIA_TALK_SEVEN ] === 0 ) {
			SceneLib.helSpawnScene.heliaTalkSeven();
			return;
		}
		if( CoC.flags[ kFLAGS.HELSPAWN_AGE ] === 1 && CoC.flags[ kFLAGS.HEL_TALK_EIGHT ] === 0 ) {
		...
		if( CoC.time.hours >= 21 ) {
			...
			EngineCore.addButton( 2, 'Hug', this, this.hugASmokeyTail );
		}
		if( CoC.flags[ kFLAGS.HELIA_ANAL_TRAINING_OFFERED ] > 0 && CoC.flags[ kFLAGS.HELIA_ANAL_TRAINING ] < 2 && CoC.player.biggestCockArea() > this.heliaAnalCapacity() && CoC.player.hasItem( ConsumableLib.GOB_ALE, 1 ) ) {
			EngineCore.addButton( 3, 'Anal Train', this, this.heliaGapeSceneChoices );
		}
		EngineCore.addButton( 5, 'Bathe', this, this.takeABath );
		if( CoC.flags[ kFLAGS.HELSPAWN_AGE ] === 1 ) {
			EngineCore.addButton( 7, CoC.flags[ kFLAGS.HELSPAWN_NAME ], SceneLib.helSpawnScene, SceneLib.helSpawnScene.playWithYourKid );
		}
		if( CoC.flags[ kFLAGS.HEL_GUARDING ] === 0 ) {
			EngineCore.addButton( 8, 'GuardCamp', this, this.helGuardToggle );
		} else {
			EngineCore.addButton( 8, 'NoGuarding', this, this.helGuardToggle );
		}
		EngineCore.addButton( 9, 'Back', this, this.heliaFollowerMenu );
	};

	//Hel: Spar Intro;
	HelFollower.prototype.sparWithHeliaFirebuttsAreHot = function() {
		MainView.clearOutput();
	...
	HelFollower.prototype.loseToSparringHeliaLikeAButtRapedChump = function() {
		MainView.clearOutput();
		//If HP loss) ;
		if( CoC.player.HP < 1 ) {
			MainView.outputText( 'You collapse on the ground, overwhelmed by pain and exhaustion caused by the berserker\'s onslaught.' );
		} else {
			MainView.outputText( 'Your arousal is too great, and your mind can no longer focus on anything but a desperate need for release.  Lust robs you of your will, and with buckling knees you collapse.' );
		}
	...
	//PC Whips Hel's Ass;
	HelFollower.prototype.PCBeatsUpSalamanderSparring = function() {
		MainView.clearOutput();
		//(If HP loss) ;
		if( CoC.monster.HP < 1 ) {
			MainView.outputText( 'Unable to withstand your onslaught, the salamander collapses to a knee, barely supporting her weight on her sword.' );
		} else {
			MainView.outputText( 'Panting heavily, knees snaking, she collapses, heavily leaning upon her sword.' );
		}

		MainView.outputText( '\n\n<i>"Come on, [name],"</i> Hel groans, swaying unsteadily.  ' );
		if( CoC.monster.HP < 1 ) {
			MainView.outputText( '<i>"Kick my ass a little harder, why don\'t you?"</i>' );
		} else {
			MainView.outputText( '<i>"Turn me on till I can\'t think straight, why don\'t you?"</i>' );
		}
	...
	HelFollower.prototype.talkToHel = function() {
		MainView.clearOutput();
		//Hel Talk 1 (Play at First Time);
		if( CoC.flags[ kFLAGS.FOLLOWER_HEL_TALKS ] === 0 ) {
			MainView.outputText( 'You run a hand through Hel\'s hair and ask the recent addition to your camp if she\'d like to talk.' );
			...
			if( CoC.player.tou < 50 ) {
				MainView.outputText( 'You cough and gag, eyes misting as the pure-grain booze burns your throat worse than hot coals.  Hel laughs riotously, slapping you on the back hard enough to cause booze to snort out through your nose.' );
			} else {
				MainView.outputText( 'You knock back the stein easily, your super-human endurance keeping you from choking on the powerful brew.  Impressed, Hel gives you a high-five and throws her head back in a laugh.' );
			}
		...
		//Hel Talk 2;
		else if( CoC.flags[ kFLAGS.FOLLOWER_HEL_TALKS ] === 1 ) {
			MainView.outputText( 'Sitting Hel down, you ask if she\'s got any stories she\'d like to share.  After all, she\'s been adventuring for years - surely she\'s got some fun tales to tell.' );
			MainView.outputText( '\n\n"<i>Me? Good stories?  Psssh, naw,</i>" she laughs, grabbing a glass from her home-made still and filling it up.  "<i>Well, there\'s always that one time...  Okay, context: way back when, three, maybe four years ago.  I was still with some of the old tribe - we hadn\'t quite gotten ourselves wiped out yet, but we were already hurting pretty bad, you follow?  So we\'re sending out little scouting parties, really, going to steal what we can\'t forage ourselves from the gnolls.  Fuck gnolls, by the way.  Just throwin\' that out there.  Anyway, so I get slotted for a party, Anika, who made me look like a busty giantess - and I\'m on the short side for a salamander bitch - though she was amazing with a bow; and this big fucking bruiser, Dane.  Oh, shit, Dane was awesome.  He\'d literally rip people in half.  Saw him do it two, three times.  Big scary motherfucker, but the nicest guy otherwise.</i>"' );
		...
		//Hel Talk 5(C);
		else if( CoC.flags[ kFLAGS.FOLLOWER_HEL_TALKS ] === 4 ) {
			MainView.outputText( '<i>"Hey, [name],"</i> Hel says, slipping out of your grasp with a come-hither wag of her finger, <i>"wanna have a drink with me?"</i>' );
			MainView.outputText( '\n\n<i>"Sure,"</i> you say, following Hel over to her still.' );
			MainView.outputText( '\n\nShe pours out two tankards of ' );
			if( SceneLib.helSpawnScene.helPregnant() ) {
				MainView.outputText( 'beer ' );
			} else {
				MainView.outputText( 'fizzy, non-alcoholic beer ' );
			}
			MainView.outputText( 'and, swinging onto your lap, clinks her glass against yours.  <i>"To loving friends and an awesome new home,"</i> she says, raising her tankard high before knocking it back.  Before you can finish yours, Hel belches loudly and violently shakes her head' );
			if( !SceneLib.helSpawnScene.helPregnant() ) {
				MainView.outputText( ', her eyes crossing a little.  Looks like she was dipping in before you got here...' );
			} else {
				MainView.outputText( '.' );
			}
			...
			if( !SceneLib.helSpawnScene.helPregnant() ) {
				...
				if( SceneLib.kihaFollower.followerKiha() ) {
					MainView.outputText( 'further pissing off Kiha, who throws a rock at Hel, and ' );
				}
				MainView.outputText( 'shooting up like a beacon over the wasteland.' );
			} else {
				MainView.outputText( ' and launches into a series of debauched jokes. You answer her, lewd jest for jest.' );
			}
			...
			MainView.outputText( 'into a hug, mashing your face into her warm, soft bosom.  You struggle for air, finally bursting out of her tight cleavage.  Hel chuckles, running her hand through your [hair] as her fiery tail wags gaily behind her.' );
		...
		//Hel Talk 6 (Needs Isabella and Kiha at camp; at least 1 gem)(C);
		else if( CoC.flags[ kFLAGS.FOLLOWER_HEL_TALKS ] === 5 && CoC.player.gems >= 1 && SceneLib.isabellaFollowerScene.isabellaFollower() && SceneLib.kihaFollower.followerKiha() ) {
			var gems = 0;
			MainView.outputText( '<i>"Hey, [name],"</i> Hel says with a sly grin.  <i>"Me, Izzy, and spitfire were just playing a little game.  Wanna deal in?"</i>' );
			...
			if( !SceneLib.isabellaFollowerScene.isabellaAccent() ) {
				MainView.outputText( ', obviously only somewhat interested' );
			}
			MainView.outputText( '.' );
			if( SceneLib.isabellaFollowerScene.isabellaAccent() ) {
			...
			//{Make an INT check};
			if( Utils.rand( 20 ) + 1 + CoC.player.inte / 5 >= 15 ) {
				//[Successful];
			...
			if( CoC.player.gems + gems < 0 ) {
				gems = CoC.player.gems;
			}
			if( gems < 0 ) {
			...
			CoC.flags[ kFLAGS.FOLLOWER_HEL_TALKS ] = 1;
			this.talkToHel();
			return;
		}
		CoC.flags[ kFLAGS.FOLLOWER_HEL_TALKS ]++;
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	//Shut up, slut;
	HelFollower.prototype.shutUpHelTalks = function() {
		MainView.clearOutput();
		MainView.outputText( 'Quickly you interject before Helia can continue with the story; you\'ve heard quite enough.  <i>"Whaaaat?  We were just getting to the good part..."</i>  Hel whines, rolling her eyes.  <i>"Hey, you\'re the one who asked!"</i>' );
	...
	HelFollower.prototype.listenToHelTalkAboutGnolls = function() {
		...
		MainView.outputText( '\n\n"<i>\'Yes,\' I sobbed. My mind was shattered. I couldn\'t think straight; I just wanted pleasure and more and more.</i>"' );
	...
	//Hug(C);
	HelFollower.prototype.hugASmokeyTail = function() {
		MainView.clearOutput();
		...
		if( CoC.time.hours >= 21 ) {
			...
			MainView.outputText( '\n\nWith a bit of effort, Hel turns around in your embrace and starts to fiddle with your [armor], slowly pulling it off, leaving your bare flesh pressed against her own.  You breathe in the woodsmoke scent of her hair; rub your ' + CoC.player.skinFurScales() + ' along her smooth flesh; gasp lightly as her long tail wraps lovingly around your [legs], drawing you even closer, letting your face rest against her yielding chest.' );
			...
			MainView.outputText( ' intertwined.  <i>"Oh, this is nice,"</i> Hel laughs, running her long, scaled fingers through your hair.  <i>"So, you wanna stay with me tonight, lover mine?  I\'d appreciate the company...."</i>' );
	...
	HelFollower.prototype.sleepyNightMareHel = function() {
	...
	//Rough Sex;
	//Into Text;
	HelFollower.prototype.heliaRoughSex = function( output ) {
		if( output === undefined || output ) {
			output = true;
			MainView.clearOutput();
			MainView.outputText( '<i>"Mmm, need to blow off some steam, lover?"</i>  Hel grins, reaching around to undo the straps of her scale bikini.  <i>"Well, I\'m all for that!"</i>' );
		}
		var buttons = 0;
		//(Display Options: ;
		...
		if( CoC.player.hasCock() && CoC.player.lust >= 33 ) {
			...
			if( CoC.player.cockThatFits( this.heliaCapacity() ) >= 0 && buttons < 9 ) {
				EngineCore.addButton( buttons, 'FuckVag', SceneLib.helScene, SceneLib.helScene.beatUpHelAndStealHerWalletFromHerVagina );
				buttons++;
			}
			//85 ass capacity;
			if( CoC.player.cockThatFits( this.heliaAnalCapacity() ) >= 0 && buttons < 9 ) {
				EngineCore.addButton( buttons, 'Anal', SceneLib.helScene, SceneLib.helScene.fuckHelsAss );
				buttons++;
			}
			if( buttons < 9 ) {
				EngineCore.addButton( buttons, 'Get Blown', SceneLib.helScene, SceneLib.helScene.helBlowsYou );
				buttons++;
			}
			if( CoC.player.cockThatFits( this.heliaCapacity() ) >= 0 && CoC.player.cockThatFits2( this.heliaCapacity() ) >= 0 && buttons < 9 ) {
				EngineCore.addButton( buttons, 'DoublePen', SceneLib.helScene, SceneLib.helScene.dpHel );
				buttons++;
			}
			if( buttons < 9 ) {
				EngineCore.addButton( buttons, 'Tail Wank', SceneLib.helScene, SceneLib.helScene.helTailWanksYourDickBecauseSheLovesYouDesuDesuHoraHora );
				buttons++;
			}
		}
		if( CoC.player.hasVagina() && CoC.player.lust >= 33 && buttons < 9 ) {
			EngineCore.addButton( buttons, 'GetLicked', SceneLib.helScene, SceneLib.helScene.getLickedByHel );
			buttons++;
		}
		if( CoC.player.lust >= 33 && buttons < 9 ) {
			EngineCore.addButton( buttons, 'TailPeg', SceneLib.helScene, SceneLib.helScene.helTailPegging );
			buttons++;
		}
		//Morph-based: [Possession] [Mount Her] [Hanging 69] [Coil Her Up] [Tentafuck]);
		if( CoC.player.lust >= 33 && CoC.player.isTaur() ) {
			if( CoC.player.hasCock() ) {
				if( CoC.player.cockThatFits( this.heliaCapacity() ) >= 0 && buttons < 9 ) {
					EngineCore.addButton( buttons, 'Mount Her', this, this.centaurMountsCampHel );
					buttons++;
				}
			}
			if( CoC.player.hasKeyItem( 'Centaur Pole' ) >= 0 && CoC.player.hasVagina() && buttons < 9 ) {
				EngineCore.addButton( buttons, 'CentaurToy', this, this.femtaurPlusCampHel );
				buttons++;
			}
		}
		...
		if( CoC.player.lust >= 33 && CoC.player.tentacleCocks() > 6 ) {
			EngineCore.addButton( buttons, 'Tentacles', this, this.heliaFollowerTentafuck );
			buttons++;
		}
		if( CoC.player.lust >= 33 && CoC.player.isNaga() ) {
			//'Rough' Sex -- Naga Coil (Female w/ Naga Lower Body);
			if( CoC.player.hasVagina() && buttons < 9 ) {
				EngineCore.addButton( buttons, 'NagaCoilF', this, this.nagaCoilForHelCampWithGirls );
				buttons++;
			}
			//'Rough' Sex (Well, it's in the menu, if not in spirit) -- Naga Coil (Male w/ Naga Lower Body);
			//Male naga shit;
			if( CoC.player.hasCock() && buttons < 9 ) {
				if( CoC.player.cockThatFits( this.heliaCapacity() ) >= 0 ) {
					EngineCore.addButton( buttons, 'NagaCoilM', this, this.inCampHelNagaLuv );
					buttons++;
				}
			}
		}
	...
	HelFollower.prototype.heliaCampPossession = function() {
		MainView.clearOutput();
		MainView.outputText( 'With a coy grin, you let your ghostly attributes go transparent, and ask Hel if she\'d mind taking an extra passenger for a little bit.' );
		MainView.outputText( '\n\n"<i>Oh, that\'s too spooky for me, lover mi - HEY!</i>" she yelps as you dive into her.  Your lover flails around for a minute before settling down, grudgingly allowing you to take the wheels.  Internally, you hear her say, alright, I trust you. Have fun, [name].</i>"  Now having assumed direct control of the salamander, you spend a few quick minutes getting adjusted to your slutty new body.  You run your scaled, clawed hands across Hel\'s smooth, taut body, giving your gropable ass and big, soft tits each a squeeze.  To your delight, even that slight stimulation is enough to cause a little trickle to escape the band of Hel\'s bikini bottom, staining your thighs with her lubricant.  Slutty body indeed.' );
	...
	HelFollower.prototype.possessIzma = function() {
		...
		MainView.outputText( '\n\n"<i>Er, uh, I, um,</i>" Izma stammers, squirming in your firm grasp, "<i>I don\'t think [name] would be okay with thaaaaaa!</i>" she yelps as you reach down and grasp her erect mast, gripping it tightly in your warm, leathery hand.  Izma gulps heavily as you give her a single, experimental pump.  With a few encouraging words, you tell her to relax obviously wants it... and her lovely [name] surely wouldn\'t mind Izma taking care of her... urges.' );
	...
	//'Rough' Sex (Well, it's in the menu, if not in spirit) -- Naga Coil (Male w/ Naga Lower Body);
	//Male naga shit;
	HelFollower.prototype.inCampHelNagaLuv = function() {
		MainView.clearOutput();
		var x = CoC.player.cockThatFits( this.heliaCapacity() );
		if( x < 0 ) {
			x = CoC.player.smallestCockIndex();
		}
		MainView.outputText( 'You slither over to the salamander and circle around her, your serpentine lower body trailing around her legs as you sweep Helia into a tight embrace.  She grins wolfishly, pressing her lips to yours; you let her tongue enter your mouth, entwining with your tongue, letting her distract herself as you coil more and more of your snake-body around the salamander.  By the time Hel realizes what you\'re up to, there isn\'t much she can do about it - you grab her arms and press them to her sides as you bind her up past the waist, wrapping your lover in your lengthy, serpentine form.' );
		...
		MainView.outputText( '\n\nUnable to move much in your serpentine embrace, Hel begins to rapidly contract her vaginal and anal muscles, squeezing and grinding against your ' + Descriptors.cockDescript( x ) + ' and tip.  Content to close your eyes and bask in the pleasures of your lover, ready to lose yourself in pleasure.  You relax, leaning back on yourself as you buck your hips into your lover\'s sweet hot box, flick your tail inside her ass, and play your tongue around her mouth, tasting her warm saliva and the aftertaste of beer.' );
		MainView.outputText( '\n\nYou cum easily, only a small grunt escaping your lips as Hel\'s delicious cunt brings you over, pulling your ' + Descriptors.cockDescript( x ) + ' deep into her and tightening your hold around her.  The sudden squeeze is the last hurrah as you empty your ' );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( '[balls]' );
		} else {
			MainView.outputText( 'load' );
		}
		MainView.outputText( ' well into her womb.  Your tail spasms as you cum, running wild inside Hel\'s rectum until she cums too, yelping into your open mouth as she climaxes a moment after you, girlcum squirting out of her twat until your [hips] are thoroughly soaked.' );
		...
		MainView.outputText( '\n\nBefore you know it, you\'re drifting off to sleep in your lover\'s embrace...  The last thing you hear is her quiet whisper really do give the best hugs...</i>"' );
		CoC.player.orgasm();
		EngineCore.dynStats( 'sen', -1 );
		if( CoC.isInCombat() ) {
			SceneLib.combatScene.cleanupAfterCombat();
	...
	//'Rough' Sex -- Naga Coil (Female w/ Naga Lower Body);
	HelFollower.prototype.nagaCoilForHelCampWithGirls = function() {
		MainView.clearOutput();
	...
	HelFollower.prototype.centaurMountsCampHel = function() {
		var x = CoC.player.cockThatFits( this.heliaCapacity() );
		if( x < 0 ) {
			x = CoC.player.smallestCockIndex();
		}
		MainView.clearOutput();
		MainView.outputText( 'You trot around your lover, your powerful centaur frame brushing lightly against her.  Hel grins, reaching out to pet your flanks and running her claws across your ' + CoC.player.skinFurScales() + '.  "<i>You\'ve got a helluva body,</i>" she smiles, and with an invitation from you, swings up onto your back, wrapping her arms tightly around your waist as you break out into a canter around camp.' );
		MainView.outputText( '\n\nShe nudges you toward her part of camp.  When you arrive, Hel leaps off and grabs some ropes from her pack.  You raise an eyebrow, but allow her to tie them around your equine half, creating a makeshift harness underneath you.  Once done, Hel lowers herself to her knees and, pushing aside your [armor], takes hold of your ' + Descriptors.cockDescript( x ) + '.  She gives it an experimental stroke, running her warm, leathery palm and scaled fingers across your length until you let out a little whinny and stamp your hooves.' );
		MainView.outputText( '\n\nHel pats your flank with her free hand, but doesn\'t let up, instead adding her long, slender tongue to the mix.  She flicks it along the ' + CoC.player.cockHead( x ) + ' of your ' + Descriptors.cockDescript( x ) + ', sending shivers of pleasure up your rigid shaft.  You ache to do something here, to add to your own pleasure or return it to your partner, ' );
		//if no boobs: ;
		if( CoC.player.biggestTitSize() < 3 ) {
			MainView.outputText( ' but your body is too big, and you must content yourself to endure Hel\'s ministrations' );
		} else {
			MainView.outputText( ' but all you can do is take hold of your [chest] and tease and tweak your [nipples], getting what pleasure you can from your sensitive teats' );
		}
		MainView.outputText( '.' );
		MainView.outputText( '\n\nAfter a few mind-hazing minutes of salamander hand-job, Hel takes the ' + CoC.player.cockHead( x ) + ' of your cock into her mouth, slurping up the tip of your ' + Descriptors.cockDescript( x ) + '.  Your breath catches in your throat as the first inches of your cockmeat are engulfed in the hot, wet embrace of Helia\'s mouth.  Her lizard-tongue wastes no time in flicking across your ' + CoC.player.cockHead( x ) + ', the forked tip playing across your urethra, drawing forth a thick dollop of pre that soon smears her cheeks with white.  She pumps your cock into her mouth, licking and sucking on your crown as her pistoning hands force thicker globs of sticky precum out of you, eagerly swallowing every drop - but only after savoring it, rolling each drop around in her mouth until her entire gob is a sticky mess, thick webs of seed coating her lips each time she swallows another appetizing taste.' );
		MainView.outputText( '\n\nThen as quickly as it began, Hel\'s blowjob comes to a halt.  The salamander pops your prick out of her maw and, giving it one last kiss on the ' + CoC.player.cockHead( x ) + ', lets it fall aside as she moves.  You lean around yourself awkwardly as your lover ducks beneath your horse-half, clambering into her harness.  Hel disappears from view, completely obscured by your own body; all you can feel of her is her weight pulling down on your back and her powerful scaled legs hooking around your thighs, spreading wide as she grabs your cock again.  You groan pleasurably as the tip brushes her lower lips, the heat of her inhuman pussy radiating out to set your nerves ablaze.' );
		...
		if( CoC.player.cockArea( x ) > this.heliaCapacity() ) {
			MainView.outputText( 'she can cram in, the rest of your length twitching between her legs' );
		} else {
			MainView.outputText( 'you have to offer her' );
		}
		MainView.outputText( '.  Finally, though, you manage to settle down, your breath coming ragged as Hel\'s vaginal muscles clamp and release, gently massaging your shaft with expert movements, always keeping you on edge until...' );
		MainView.outputText( '\n\nTHWACK!' );
		MainView.outputText( '\n\nYou start forward as a rough tail-slap lands on your flank.  You reflexively step forward, your equine muscles acting on instinct...  which only serves to rock Hel off and down again on your ' + Descriptors.cockDescript( x ) + ', her impromptu harness moving her like a swing. You arch your back as the movement hammers Hel down on your cock like a free-swinging onahole, but the moment of pleasure lurches out of control when she gives you another hard tail-swat, and another; your insticts carry you forward until you reach a full canter. Shudders and twitches wrack your body as your own motions rock Hel on and off your [cock], slamming her back down on your shaft again and again.' );
	...
	HelFollower.prototype.femtaurPlusCampHel = function() {
		MainView.clearOutput();
		MainView.outputText( 'You trot around your lover, your powerful centaur frame brushing lightly against her. Hel grins, reaching out to pet your flanks and running her claws across your ' + CoC.player.skinFurScales() + '.  "<i>You\'ve got a helluva body,</i>" she smiles, and with an invitation from you, swings up onto your back, wrapping her arms tightly around your waist as you break out into a canter around camp.  She gives you a gentle nudge toward a certain pile of your gear, and it doesn\'t take but a moment for you to notice she\'s urging you toward the special toys Whitney gave you for your equine-like body\'s needs.' );
	...
	HelFollower.prototype.heliaFollowerTentafuck = function() {
		MainView.clearOutput();
		MainView.outputText( 'With a flourish you pull your [armor] off, exposing your writhing mass of tentacle-cocks for Hel`s enjoyment.  "<i>Wow, think you got enough of th--urf!</i>" the salamander starts to say before you shut her up with one of your tentacle pricks, lashing forward and plugging her mouth.  Hel staggers back, flailing around before grabbing the cock you\'ve shoved down her throat and giving it a rough tug.  Just as she nearly succeeds in pulling it out, your other wriggling members wrap around her thick thighs; you pull her in toward you, grabbing her shoulders as your pricks peel off her bikini bottom to reveal the smooth face of Helia\'s crotch.' );
		...
		if( CoC.player.balls > 0 ) {
			...
			MainView.outputText( 'wriggling tentacles' );
		}
		MainView.outputText( ' feel painfully empty and white slush pours from all her holes, painting the ground white beneath her dangling feet.' );
	...
	HelFollower.prototype.boxWithInCampHel = function() {
		...
		MainView.outputText( '\n\nYou toss your [armor] aside and assume your own battle stance, your powerful muscles tightening beneath your ' + CoC.player.skinFurScales() + ', ready to spring into action.  You move slowly, cautiously; you and Hel begin to circle each other like predators observing their prey, each watching the other for any momentary lapse, any split-second vulnerabilities.  Suddenly, Hel lashes out, a quick one-two jab at you, narrowly blocked in your guard.  You grind your feet into the ground, deflecting her inhumanly-strong blows as best you can.  She launches a third strike, but you duck aside, spinning into her guard and delivering a hard jab to her gut.  The salamander recoils, swinging her fiery tail at you as she gets some distance, staggering back from the hit.  You exchange another quick flurry of blows, punches connecting, blocked, or dodged in rapid succession.  The fight drags on, neither of you able to land a telling blow, your strikes deflected or dodged, your prowess in battle nearly matched.' );
		EngineCore.fatigue( 5 + Utils.rand( 20 ) );
		//If INT check succeeds: ;
		if( CoC.player.inte / 20 + 1 + Utils.rand( 20 ) >= 13 ) {
			MainView.outputText( '\n\nUntil you see an opening for a second, Hel leaves her side exposed between attacks.  You take a step back, inviting her to attack; the salamander lunges just as planned, throwing a mighty punch your way, which you nimbly dodge, putting you just inside her guard.  With all your force, you slam your elbow into her side, throwing your fiery foe to the ground with an audible "<i>OOF!</i>"  She\'s barely landed on her big ol\' ass before you jump on her, tackling Hel while she\'s down and pinning her.  The salamander flails, trying to throw you off, but you hold fast and count down:' );
		...
		else if( CoC.player.str / 20 + 1 + Utils.rand( 20 ) >= 13 ) {
		...
		//If PC fails the Check:  ;
			...
			if( CoC.player.lust >= 33 && CoC.player.hasCock() ) {
				if( CoC.player.cockThatFits( this.helCapacity() ) >= 0 ) {
	...
	HelFollower.prototype.muddyLizardFeet = function() {
		...
		if( CoC.player.balls > 0 ) {
			MainView.outputText( 'her heel coming to caress your taut sack' );
		} else if( CoC.player.hasVagina() ) {
			MainView.outputText( 'her heel brushing against the nub of your [clit]' );
		} else {
			MainView.outputText( 'her heel going so low as to brush your taint, making your [asshole] clench in surprise before she moves back up your length' );
		}
		MainView.outputText( '.  You groan lustily as her toes and soft sole jerk you off, gently urging the tight knot of pleasure in your gut to fruition.  Cupping one of her hefty tits, Hel licks her lips as the first thick smears of pre leak from your cock\'s [cockHead biggest], smearing onto her long-clawed toes.' );
		...
		if( CoC.player.biggestCockArea() > this.heliaCapacity() ) {
			MainView.outputText( 'you feel your [cockHead biggest] against the lips of her cervix, as fully into her as you can go' );
		} else {
			MainView.outputText( 'your groin meets Helia\'s, your [cock] wholly enveloped within her' );
		}
		MainView.outputText( '.  Before you can think to withdraw, Hel tightens her hold on you, whispering huskily, "<i>Just leave it... just for a second, [name].  Oh, God,</i>" she moans, her eyes closing as she savors the feeling of you buried so deep inside her, your slick prick twitching as her walls massage it, milking it of the precum that freely flows from your tip.' );
	...
	HelFollower.prototype.takeABath = function() {
		...
		//{Repeat Intro (If you taught Hel to swin; else, play 1st Time)};
			...
			MainView.clearOutput();
			MainView.outputText( 'You ruffle Hel\'s hair and ask if the fiery salamander\'s up for a dip.' );
			...
			if( CoC.player.armorName === 'goo armor' && CoC.player.lust >= 33 && 9999 === 0 ) {
	...
	//Push It;
	HelFollower.prototype.pushHelToSwim = function() {
		MainView.clearOutput();
	...
	//Teach Her;
	HelFollower.prototype.teachHelToSwim = function() {
		MainView.clearOutput();
		MainView.outputText( 'Putting a hand on Hel\'s shoulder, you offer to teach her how to swim.' );
		...
		CoC.flags[ kFLAGS.HEL_CAN_SWIM ] = 1;
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};

	//Threesomes;
	HelFollower.prototype.heliaThreesomes = function() {
		MainView.clearOutput();
		MainView.outputText( 'You slip an arm around Hel\'s waist and ask if she\'s up for some... group activities?' );
		...
		if( SceneLib.kihaFollower.followerKiha() && CoC.player.lust >= 33 && CoC.player.gender > 0 ) {
			EngineCore.addButton( 0, 'Kiha', this, this.campHelAndKihaThreeSome );
		}
		if( (CoC.player.armorName === 'goo armor' || CoC.flags[ kFLAGS.VALARIA_AT_CAMP ] === 1) && CoC.player.lust >= 33 && CoC.player.hasVagina() ) {
			EngineCore.addButton( 2, 'Valeria', this, this.helAndValeriaCampThreesomes );
		}
		if( CoC.player.lust >= 33 && SceneLib.sophieBimbo.bimboSophie() ) {
			EngineCore.addButton( 3, 'Sophie', this, this.helAndSluttyHarpy );
		}
		if( CoC.player.lust < 33 ) {
	...
	//Helia at Camp, Intro;
	HelFollower.prototype.heliaAndVapula = function() {
		MainView.clearOutput();
		var x = CoC.player.cockThatFits( this.heliaCapacity() );
		var y = CoC.player.cockThatFits2( this.heliaCapacity() );
		MainView.outputText( 'You gab Hel and start for the edge of camp, where a certain succubus dwells....' );
		MainView.outputText( '\n\nYou are greeted by a bunch of imps that scatter as soon as they see you appeared to be have been busy on the purple woman.  You smile as you walk up to Vapula, the salamander girl still clinging to your shoulders.  The trembling succubus has trouble keeping an air of composure when she sees you.' );
		...
		MainView.outputText( '\n\n"<i>You talk too much, slut.  Wipe that precum out of your body, I want you to be clean for my girl.</i>"  For a moment, she thinks you\'ve decided to replace her and asks with horrified eyes is she? Where did you-</i>"' );
		...
		MainView.outputText( '\n\nYou feel hands wandering across your body, Hel slowly, sensuously, pulling off your [armor].  Soon, the two of you are bare, clothes and weapons discarded all around the succubus.  You push Hel over, grinding your hard ' + Descriptors.cockDescript( x ) + ' between her legs.  As you grope the salamander girl\'s body, you gesture at Vapula and tell her to come closer so that you can stroke her, too.  You begin caressing each other, spending a few moments in languid pleasure until your ' + Descriptors.cockDescript( x ) + ' looks ready to explode.' );
		//Combine, Body;
		MainView.outputText( '\n\nYou whisper to your scaly girl a nice hard tail you\'ve got there.  Do you know how to use it?</i>" Hel nods eagerly, too turned on to form coherent words.  "<i>Is the concept of tail-fucking foreign to you?  No, I guess you know it very well, you little slut.  Well, I want you to fuck my succubus pet senseless with your tail.  I want you to ream her ass over and over until she begs for mercy.  Don\'t be afraid to be rough, that\'s what she\'s here for.  I\'ll take care of your own fuck-holes myself.  Do you understand?</i>"' );
		...
		MainView.outputText( '\n\nThis libertine orgy cannot last indefinitely, and even your girls are trembling convulsively, ready to reach their climax. You manage to slide your ' + Descriptors.cockDescript( x ) + ' up the reptile girl\'s twat a few more times before blasting her with your spooge.  You fill Hel\'s interior, pouring cum like a torrent into her womb' );
		//[IF cum production massive: ;
		if( CoC.player.cumQ() >= 750 ) {
			MainView.outputText( ' until her belly swells and starts spurting ropes of jism back out of her love-tunnel' );
		}
		MainView.outputText( '.  Your junk palpitates and bulges as globs push their way out of your shaft before being absorbed by her needy pussy, and her skin and scales seem to literally boil with consumed lust.  At the peak of your orgasm, you stare at your girls\' faces: Hel\'s is nothing but complete, insensate bliss, while Vapula is literally crying in ecstasy, tears of joy streaming down her cheeks.' );
		MainView.outputText( '\n\nSeeing how much pleasure you\'re giving them is quite touching, and you give each of them a long, ardent kiss, letting each girl know how the other tastes like.  At last, when your ' + Descriptors.cockDescript( x ) + ' has unloaded its last rope of jism, you pull out and break the kiss.  Hel pants and sighs, utterly drained; you see her tail popping out of Vapula\'s ass, and the released succubus just falls on her back, cunt exposed and mouth open in blissful exhaustion.' );
		...
		if( !SceneLib.helScene.followerHel() ) {
			MainView.outputText( '  You kiss her and ask if she\'ll be able to find her way back to her home.' );
	...
	HelFollower.prototype.dudeHeliaAndKihaThreeSome = function() {
		...
		MainView.outputText( '\n\nTenderly, Kiha plants a kiss in the nape of your neck, her long forked tongue tickling your ' + CoC.player.skinFurScales() + '.  You turn and return her kisses, locking lips with the dragoness, your tongues entwining to the rhythm of the feet servicing your [cock].' );
		...
		MainView.outputText( '\n\nKiha gives the salamander a grave look, but soon softens, nuzzling her chin in your ' + Descriptors.hairDescript() + '.  Hel sighs, but leans down to give you each a quick kiss on the head; her foot never misses a beat, though, and combined with Kiha\'s sensual massaging, you realize you\'re dangerously close to the edge again.  You grit your teeth and let loose an animalistic growl, the only forewarning the distracted lizards get before the first burst issues forth from your [cockHead], smearing the sole of Hel\'s foot with creamy seed.  The salamander makes a happy gasp as another squirt of cum smears her clawed toes, a particularly strong arc fountaining so high that it smears her thighs.' );
	...
	HelFollower.prototype.helXValeriaFemalePC = function() {
		MainView.clearOutput();
		MainView.outputText( 'In the blink of an eye, Valeria slithers around Hel\'s body, almost completely encasing her in thick blue goo, leaving only her head and her resplendent reptilian rack bare.  Hel grins at you, putting a hand to her crotch and taking a handful of goo between her fingers.  With a bit of effort, she shapes a pair of thick blue pricks out of her groin, each shimmering in the sunlight as Hel strokes Valeria\'s new appendages, coaxing out a drop of slimey pre from both tips.  Valeria\'s face reappears on Hel\'s flat belly a moment after she\'s done, looking down to her groin with glee as she further shapes the erect pillars of slime, morphing them into a thick knotty doggy-dick on top, and a huge-flared horsecock on bottom.  You lick your lips as the dog-dick\'s knot inflates enticingly.' );
	...
	HelFollower.prototype.helGuardToggle = function() {
		...
		if( CoC.flags[ kFLAGS.HEL_GUARDING ] === 0 ) {
			MainView.outputText( 'You ask Hel if she wouldn\'t mind watching the perimeter for a few hours during the night.' );
			MainView.outputText( '\n\n"<i>Sure thing, [name]!  I\'ve got your back!</i>" she says, her tail reaching around to give you a pop on the [butt].' );
			CoC.flags[ kFLAGS.HEL_GUARDING ] = 1;
		}
		//[If Hel IS guarding Camp];
		else {
			MainView.outputText( 'You tell Hel she doesn\'t need to keep watch at night anymore.' );
	...
	HelFollower.prototype.heliaAnalTrainingPrompt = function() {
		MainView.clearOutput();
		CoC.flags[ kFLAGS.HELIA_ANAL_TRAINING_OFFERED ] = 1;
		MainView.outputText( '“<i>Hey there, lover mine. Got a minute?</i>” Helia asks with dangerous glint in her eyes.' );
		...
		if( CoC.player.tallness < 60 ) {
			MainView.outputText( 'her bosom towering over you' );
		} else if( CoC.player.tallness < 72 ) {
			MainView.outputText( 'towering over you' );
		} else if( CoC.player.tallness < 90 ) {
			MainView.outputText( 'meeting you eye to eye' );
		} else {
			MainView.outputText( 'looking up at you confidently' );
		}
		MainView.outputText( ' as she boldly reaches into your [armor] to squeeze your ' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 'biggest ' );
		}
	...
	//Give Her Ale;
	HelFollower.prototype.giveHeliaAnalTraining = function() {
		MainView.clearOutput();
		MainView.outputText( 'You hand Helia a bottle of goblin ale and question if she’s sure this won’t make her loose.' );
		...
		if( CoC.player.balls > 0 ) {
			MainView.outputText( 'She tilts forward to press her chin into your [sack] and begins to lather your [balls] with frothy, warm, salamander spit. One hand hefts a weighty nut as she works, caressing it as if it would somehow coax the cum out of it. ' );
		}
		MainView.outputText( 'Helia slides her palms along either side of your urethra, letting her fingers handle the central, sensitive bit on the underside, following in the path of slick saliva she left for them to follow. She rubs her nose into the sensitive area just below your [cockHead biggest]. You grunt as pre-cum begins to pour in response, your bloated boner flexing atop Helia’s head as you feel a dangerously powerful orgasm welling up within you.' );
		...
		if( CoC.player.balls > 0 ) {
			MainView.outputText( ', hard enough to lift your nuts in their [sack]' );
		}
		MainView.outputText( ', and you erupt, cumming a steamy hot load of spunk straight onto Helia’s face, head, and back.' );
		if( CoC.player.cumQ() < 250 ) {
			MainView.outputText( ' You run out before long, but it’s still enough to make her a white painted mess. She licks the excess from her upper lip and shudders happily.' );
		} else if( CoC.player.cumQ() < 2000 ) {
			MainView.outputText( ' You spurt rope after rope onto her, actually drenching her hair with alabaster moisture. Her back and face look equally smothered with spooge when you finish. She’s happy to wipe the goop out of her eyes and shovel it into her mouth, swallowing it down with a satisfied smile.' );
		} else {
			MainView.outputText( ' You pour ivory waves of spooge across her body, soaking her from head to toe with the fruit of your loins, liberally basting her with baby-batter. She groans as the excess washes out around her knees' );
			if( CoC.player.cumQ() >= 3000 ) {
				MainView.outputText( ', but you keep cumming, cascading fresh waves onto her until there’s not a single part of her that isn’t stained and dripping with your salty outflow' );
			}
			MainView.outputText( '. Helia licks her hands clean, wipes her eyes and devours that as well, and then gives you a dripping-wet, sensuous smile.' );
		}
		...
		CoC.flags[ kFLAGS.HELIA_ANAL_TRAINING ] = 1;
		if( CoC.player.biggestCockArea() < this.helAnalCapacity() ) {
			MainView.outputText( 'then maybe I can handle you, my hung ' + CoC.player.mf( 'stud', 'breeder' ) + '.' );
		} else {
			MainView.outputText( 'then maybe we can see about trying something bigger.' );
		}
		MainView.outputText( '</i>”' );
		CoC.player.consumeItem( ConsumableLib.GOB_ALE, 1 );
		CoC.player.orgasm();
		EngineCore.dynStats( 'sen', -1 );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	HelFollower.prototype.heliaGapeSceneChoices = function() {
		if( CoC.flags[ kFLAGS.HELIA_ANAL_TRAINING ] === 0 ) {
			this.giveHeliaAnalTraining();
		} else if( CoC.flags[ kFLAGS.HELIA_ANAL_TRAINING ] === 1 ) {
			this.heliaAnalTrainingPartTwo();
		}
	};
	//Anal Training #2;
	HelFollower.prototype.heliaAnalTrainingPartTwo = function() {
		CoC.player.consumeItem( ConsumableLib.GOB_ALE, 1 );
		MainView.clearOutput();
		MainView.outputText( 'Pulling out another bottle of ale you dangle it before the slutty salamander, saying, “<i>A present for my favorite buttslut.</i>”' );
	...
	HelFollower.prototype.noGapeSlutForHelia = function() {
		MainView.clearOutput();
		MainView.outputText( 'You step up and kiss Helia, telling her that she doesn’t have to this for you, at least not right now.' );
	...
	HelFollower.prototype.yesMakeHeliaAGapeSlut = function() {
		...
		if( CoC.player.cumQ() < 1000 ) {
			MainView.outputText( ', ignoring the way your sheer volume of spunk leaks from the corners of her mouth and nose, her belly gradually rounding with cum in addition to thick dildo' );
		}
		MainView.outputText( '. The slutty salamander eventually pulls it off and ' );
		if( CoC.player.cumQ() < 2000 ) {
			MainView.outputText( 'squeezes a last few dollops onto her face, letting them stain her with your scent.' );
		} else {
			MainView.outputText( 'allows you to hose her down with your ongoing waves of salty jism, soaking her in your sexual scent.' );
		}
		MainView.outputText( ' Helia burps. “<i>Fuck me that was awesome! Whoah, don’t mind me, but I, uh... I think I’m gonna stay here for a little while, maybe just rock up and down till I come down off this buzz. She blushes and begins to finger her snatch again. Just let me... just let me get used to.</i>” She shoos you away and goes back to toying with herself, getting used to her new “flexibility”.' );
	...
	HelFollower.prototype.stayHomeFromHeliaParty = function() {
		MainView.clearOutput();
		MainView.outputText( '"<i>Aww, lame,</i>" Hel groans, sighing.  "<i>Fiiiiiine.  I guess I\'ll just go have awesome fun and party hard without you. Don\'t stick too hard into that mud, lover,</i>" she chuckles, giving you a reassuringly affectionate kiss on the cheek before trucking off.' );
		EngineCore.doNext( MainView, MainView.playerMenu );
	};
	HelFollower.prototype.helPartyMenu = function() {
		...
		//this.heliaHakonAndKiri;
		if( CoC.flags[ kFLAGS.HELIA_BDAY_HAKON_AND_KIRI ] === 0 ) {
			EngineCore.addButton( 1, 'Hakon \'n Kiri', this, this.heliaHakonAndKiri );
		}
		//this.heliaPhoenixes;
		if( CoC.flags[ kFLAGS.HELIA_BDAY_PHOENIXES ] === 0 ) {
			EngineCore.addButton( 2, 'Phoenixes', this, this.heliaPhoenixes );
		}
		//this.heliaAndFoxTwins;
		if( CoC.flags[ kFLAGS.HELIA_BDAY_FOX_TWINS ] === 0 ) {
			EngineCore.addButton( 3, 'Fox Twins', this, this.heliaAndFoxTwins );
		}
		//this.leaveWithoutSex;
		EngineCore.addButton( 4, 'Leave', this, this.leaveWithoutSex );
		//this.leaveWithGirls;
		if( CoC.flags[ kFLAGS.HELIA_BDAY_FOX_TWINS ] > 0 ) {
			EngineCore.addButton( 5, 'Leave w/Girls', this, this.leaveWithGirls );
		}
	};

	//Go with Her;
	HelFollower.prototype.goWithHelia = function() {
		MainView.clearOutput();
		MainView.outputText( '"<i>Sounds like fun.  Let\'s go,</i>" you say, offering your arm to Hel.  She grins and takes it, holding herself close as she takes the lead, steady paces quickly drawing you out into the vast deserts of Mareth beside your lover.' );
		//{If Kiha, Isabella, and/or Valeria are at camp:};
		if( SceneLib.isabellaFollowerScene.isabellaFollower() || SceneLib.kihaFollower.followerKiha() || CoC.flags[ kFLAGS.VALARIA_AT_CAMP ] === 1 ) {
			MainView.outputText( '  Looking over your shoulder as you depart, though, you notice that camp looks particularly... empty. Perhaps your other camp-mate' );
			if( SceneLib.camp.companionsCount() > 1 ) {
				MainView.outputText( 's are' );
			} else {
				MainView.outputText( ' is' );
			}
			MainView.outputText( ' out, too? You shrug. You and Hel aren\'t the only ones with lives, you suppose.' );
		}
		...
		//if less than 7' PC: ;
		if( CoC.player.tallness < 84 ) {
			MainView.outputText( 'sweeping you off your feet' );
		} else {
			MainView.outputText( 'hanging off you' );
		}
		...
		if( CoC.player.tallness <= 72 ) {
			MainView.outputText( ', her head resting on your shoulder' );
		}
		MainView.outputText( '.  "<i>Hey, thanks for coming, [name],</i>" she says quietly, squeezing your arm, "<i>Kiri\'s done nothing but talk about you since the tower.  I think someone has a crush...</i>"' );
		...
		MainView.outputText( '\n\nHel jumps back, practically falling into your arms as the bar lights up, filled with people and Kiri at the forefront, her family smiling wide.  Behind them, surrounding a great big cake practically as tall as Hel is, stands ' );
		var list = [ 'Edryn' ];
		...
		MainView.outputText( Utils.formatStringArray( list ) + '. You turn to Helia and cock a grin as you see the surprise etched on her face, followed by the growing smile.  "<i>Happy birthday, Helia!</i>" they all shout out at once, raising mugs frothing over with beer.' );
		...
		if( CoC.flags[ kFLAGS.HARPY_QUEEN_EXECUTED ] > 0 ) {
			MainView.outputText( 'who forgave us for the queen\'s death' );
		} else {
			MainView.outputText( 'who wanted to be friends still, even after the Tower' );
		}
		MainView.outputText( ' and... it sort of became a thing.  Hope you don\'t mind!</i>"' );
		...
		MainView.outputText( '\n\nKiri slips out of the crowd to plant a quick kiss on your cheek, followed by Hakon vigorously shaking your hand, great claws scoring your ' + CoC.player.skinFurScales() + ' as he thanks you again for the rescue, and for ' );
		if( CoC.flags[ kFLAGS.HARPY_QUEEN_EXECUTED ] > 0 ) {
			MainView.outputText( 'finally killing that bitch of a queen' );
		} else {
			MainView.outputText( 'setting the queen right, but leaving her alive for when it\'s finally time to bring the fight to Lethice' );
		}
		MainView.outputText( '.  You nod at his thanks, and his hand is soon replaced by Helia\'s as she grabs you on her way toward the gigantic cake.  The towering white confectionary seems more than big enough to feed everyone in the group, a huge extravagance on the part of her family.  You hear a quiet whisper about ' );
		if( CoC.flags[ kFLAGS.FREED_VALA ] === 1 ) {
			MainView.outputText( 'a certain waitress fairy having... supplied the icing herself' );
		} else {
			MainView.outputText( 'a certain minotaur chef having supplied the icing himself' );
		}
		MainView.outputText( ' as Hel is handed an oversized knife and urged to blow out the candles atop it, arranged into a perfect 23.' );
		...
		if( CoC.flags[ kFLAGS.HEL_FOXY_FOURSOME_WARNED ] === 1 ) {
			MainView.outputText( 'a pair of lovely fox-eared herms' );
		} else {
			MainView.outputText( 'Miko and Mai' );
		}
		MainView.outputText( ' burst out, their big breasts covered in nothing but tiny pink pasties and an all-too-small thong sheathing their red pricks.  Arm in arm with tail wagging furiously, they cry out "<i>Happy birthday, Hel!</i>" before hopping out and slipping into the salamander\'s arms.  "<i>Ladies,</i>" Hel says, grinning like an idiot, a moment before she\'s pushed into a waiting chair behind her.  One of the girl\'s busts is quickly shoved into her face as the rest of the group laughs and starts passing out another round of drinks.' );
		//{PC can play each short scene once};
		this.helPartyMenu();
	};
	HelFollower.prototype.heliaDrinks = function() {
		MainView.clearOutput();
		//Drinks;
		MainView.outputText( 'You saunter up to the bar, where the staff have laid out more than enough beers for the entire party.  Edryn and a handful of phoenixes are milling around, exchanging tips for swordsmanship advocating charging in with a greatweapon swinging, as the phoenixes try and explain their shield-wall tactics.  You knock back a drink and listen in, watching with interest as the phoenix girls get progressively more and more flustered' );
		//if PC is male:;
		if( CoC.player.hasCock() && CoC.player.hasVagina() ) {
			MainView.outputText( ' just as you do' );
		}
		MainView.outputText( ' in the presence of the steadily mounting pheromone cloud around the lusty centauress.  Before you\'ve finished your drink, all the half-harpies are sporting sizable tents in their britches and are subtly sniffing around, trying to figure out what\'s happening to them... just before Edryn grabs the lead phoenix by the arm and leads her off into one of the back rooms, shooting the others a wink and a flicking tail over her drooling horse-sex.  The others are quick to follow.' );
		CoC.flags[ kFLAGS.HELIA_BDAY_DRINKS ] = 1;
		this.helPartyMenu();
	};
	//Hakon & Kiri;
	HelFollower.prototype.heliaHakonAndKiri = function() {
		MainView.clearOutput();
		MainView.outputText( 'You meander over to the veteran salamander and his half-harpy daughter, and quickly find yourself surrounded by fluttering red feathers as Kiri practically jumps on you, hugging you.  "<i>Hey, [name], long time no see,</i>" she says, smiling demurely as she leads you back to where she and her father were standing, removed from the real bustle of the party.' );
		MainView.outputText( '\n\n"<i>How\'re you holding up, ' + CoC.player.mf( 'son', 'kid' ) + '?</i>" Hakon says, clapping you on the shoulder and passing you another beer.  "<i>Hel tells me you\'re quite a handful.  Treating my girl right, I hope?</i>"' );
		MainView.outputText( '\n\nYou falter at that, not quite sure how to address your lover\'s father in that regard, but the old salamander just laughs and gives you another rough shoulder-clap.  "<i>Just teasing, ' + CoC.player.mf( 'boy', 'girl' ) );
		MainView.outputText( '.  We salamanders are a bit more open about that sort of thing, though, if you haven\'t noticed.  Helia\'s regaled us with more than a few of her \'stories,\' let me tell you. Isn\'t that right, sweetheart?</i>"' );
		...
		CoC.flags[ kFLAGS.HELIA_BDAY_HAKON_AND_KIRI ] = 1;
		this.helPartyMenu();
	};
	//Phoenixes;
	HelFollower.prototype.heliaPhoenixes = function() {
		MainView.clearOutput();
		MainView.outputText( 'You approach the small squad of phoenixes milling around the cake, mostly talking amongst themselves.  They nod as you approach, a bit wary of the person who recently kicked their asses inside out.  Still, they nod respectfully as you approach.' );
		...
		if( CoC.flags[ kFLAGS.HARPY_QUEEN_EXECUTED ] > 0 ) {
			MainView.outputText( 'dead' );
		} else {
			MainView.outputText( 'deposed' );
		}
		MainView.outputText( ', he thought it would be good to try and at least... know each other.  I\'m not complaining.  Things like this party... never would have seen anything like this under Queen Calais.  It\'s nice to see new things, meet new people.</i>"' );
		...
		if( SceneLib.kihaFollower.followerKiha() && SceneLib.isabellaFollowerScene.isabellaFollower() ) {
			MainView.outputText( ', then to the fiery dragoness and the ultra-busty cowgirl talking in the corner' );
		}
		MainView.outputText( ', and you figure you know what they mean.' );
		CoC.flags[ kFLAGS.HELIA_BDAY_PHOENIXES ] = 1;
		this.helPartyMenu();
	};
	//Hel & Fox Twins;
	HelFollower.prototype.heliaAndFoxTwins = function() {
		MainView.clearOutput();
		MainView.outputText( 'You approach the foxy strippers, still well into giving Helia a dual lap-dance, their red puppy peckers poking well out of their tiny thongs.  They grin as you approach, and Hel pulls you into her lap, and you quickly find yourself with a pair of big, soft breasts to rest your head on, courtesy of ' );
		if( CoC.flags[ kFLAGS.HEL_FOXY_FOURSOME_WARNED ] === 1 ) {
			MainView.outputText( 'Mai' );
		} else {
			MainView.outputText( 'one of the foxes' );
		}
		MainView.outputText( '.  Hel plants a deep, serpent-tongue filled kiss on your lips, nuzzling her cheek on your [chest].' );
		...
		CoC.flags[ kFLAGS.HELIA_BDAY_FOX_TWINS ] = 1;
		this.helPartyMenu();
	};
	//Leave {Or: Joke's on you, no (special) birthday sex!};
	HelFollower.prototype.leaveWithoutSex = function() {
		MainView.clearOutput();
		MainView.outputText( 'You decide you\'ve had about enough of the party for now. You slip over to Hel and tell her you\'re ready to head out when she is.' );
	...
	HelFollower.prototype.leaveWithGirls = function() {
		...
		if( CoC.player.gender === 3 ) {
			...
			EngineCore.choices( 'As Male', SceneLib.helScene, SceneLib.helScene.foxyFluffsFoursomeAsMale, 'As Female', SceneLib.helScene, SceneLib.helScene.foxyFluffGirlsFuckSex, '', null, null, '', null, null, '', null, null );
		} else if( CoC.player.gender === 2 ) {
			EngineCore.doNext( SceneLib.helScene, SceneLib.helScene.foxyFluffGirlsFuckSex );
		} else {
			EngineCore.doNext( SceneLib.helScene, SceneLib.helScene.foxyFluffsFoursomeAsMale );
		}
	};
	...
	HelFollower.prototype.helAndSluttyHarpyMale = function() {
		MainView.clearOutput();
		MainView.outputText( 'You take Helia’s hand and lead her through the camp in an attempt to find someone to include in your threesome. After some searching you see Sophie sitting on a rock and get a wonderful idea. You ask Hel if she would be ok with a threesome with the busty harpy. Her smile turns to a serious frown for a moment.' );
		...
		if( CoC.player.biggestCockLength() >= 8 ) {
			MainView.outputText( ' no longer be hilted inside' );
		} else {
			MainView.outputText( ' slip out of' );
		}
		MainView.outputText( ' her sodden cunt, and hear Hel gasp for air. Before the birdslut can comment on the matter, you spin her around and push her down onto Helia, leaving them face to face.' );
		...
		if( CoC.player.cocks.length === 1 ) {
			...
			MainView.outputText( ' to boil over. White jissom blasts from your [cock], coating Helia and Sophie’s stomachs, as well as the underside of the pair’s tits.' );
			if( CoC.player.vaginas.length > 0 ) {
				MainView.outputText( ' Your unused [vagina] cums with your cock in a show of sympathetic orgasm, coating your thighs with femcum.' );
			}
		}
		// 2-3 cocks;
			...
			if( CoC.player.cumQ() < 500 ) {
				MainView.outputText( ' with your cum' );
			} else {
				MainView.outputText( ' full of cum' );
				if( CoC.player.cumQ() <= 1000 ) {
					MainView.outputText( ' until it overflows' );
					if( CoC.player.cumQ() >= 5000 ) {
						MainView.outputText( ', pooling around your knees' );
					}
				}
			}
			MainView.outputText( '.' );
			if( CoC.player.cocks.length === 3 ) {
				MainView.outputText( ' Your unused cock splatters the two with hot, white jizz.' );
			}
			if( CoC.player.vaginas.length > 0 ) {
				MainView.outputText( ' Your unused [vagina] cums with your cocks in a show of sympathetic orgasm, coating your thighs with femcum.' );
			}
		}
		// > 4 cocks;
		else {
			...
			if( CoC.flags[ kFLAGS.HELIA_ANAL_TRAINING ] === 0 ) {
				...
				MainView.outputText( ' well-trained' );
			} else if( CoC.flags[ kFLAGS.HELIA_ANAL_TRAINING ] >= 2 ) {
				MainView.outputText( ' gaped' );
			}
			MainView.outputText( ' asshole. She gasps at the sudden double penetration, and you reciprocate because of the furnace-like heat of both her holes. Your [cock biggest3] lines up with Sophie’s slutty pussy and drives home while your [cock smallest] does the same to her ass.' ); // Haha look at me being a lazy shit.
			...
			MainView.outputText( '. The two girls seem to share your problem, their teeth clenched and clinging tightly to each other trying to resist. Seeing their clearly shared pleasure in the moment puts you over the edge, and you let loose the torrent of liquid lust from [eachCock]. Sophie cums as the tsunami of jizz floods her levies, the copious amount spilling back out of her holes and pooling around your knees.' );
			if( CoC.player.cocks.length > 4 ) {
				MainView.outputText( ' Your extra cocks spray pearly cum all over the nearby ground.' );
			}
			if( CoC.player.vaginas.length > 0 ) {
				MainView.outputText( ' Your unused [vagina] cums with your cocks in sympathetic orgasm, coating your thighs with femcum.' );
			}
		}
		// Merge;
	...
	HelFollower.prototype.helAndSluttyHarpyFemale = function() {
		...
		if( CoC.player.cocks.length >= 1 ) {
			MainView.outputText( 'The blasts spatter against the underside of Sophie’s gigantic melons and slide off Hel’s sides like an overfull cream pastry.' );
		}
		// Continue;
	...
	HelFollower.prototype.helAndSluttyHarpyGenderless = function() {
		MainView.clearOutput();
		MainView.outputText( 'You walk up to Helia and ask if she would be up for fucking you with that wondrous tail of hers. She smiles slyly and wraps her arms around you. You take her response as an unspoken yes, hugging her back and pulling her off to find a better place for your loving.' );
