...
angular.module( 'cocjs' ).run( function( CoC, kFLAGS, EngineCore, ConsumableLib, PerkLib, StatusAffects, ArmorLib, WeaponLib, Utils, EventParser ) {
	...
	FollowerInteractions.prototype.amilyMeetsPureJojo = function() {
		...
		CoC.getInstance().flags[ kFLAGS.AMILY_MET_PURE_JOJO ]++;
		...
		EngineCore.outputText( 'Amily sighs and hugs him again, "<i>You\'re a pansy. You know that Jojo?  But I\'m glad ' + CoC.getInstance().player.short + ' reunited us so that we could meet again.</i>"\n\n', false );
	...
	FollowerInteractions.prototype.amilyIsPissedAtYouForRuiningJojo = function() {
		...
		CoC.getInstance().flags[ kFLAGS.AMILY_PISSED_PC_CORRUPED_JOJO ]++;
	...
	FollowerInteractions.prototype.tellAmilyToFuckOffRuinedJojo = function() {
		...
		if( CoC.getInstance().player.inte >= 45 ) {
	...
	FollowerInteractions.prototype.agreeToHelpAmilyFixJojo = function() {
		...
		CoC.getInstance().flags[ kFLAGS.AMILY_WAIT_FOR_PC_FIX_JOJO ] = 1;
		EngineCore.doNext( EventParser.playerMenu );
	};
	...
	FollowerInteractions.prototype.fixJojoOOOOHYEEEEAHSNAPINTOASLIMJIM = function() {
		...
		CoC.getInstance().flags[ kFLAGS.AMILY_BLOCK_COUNTDOWN_BECAUSE_CORRUPTED_JOJO ] = 4;
	...
	FollowerInteractions.prototype.amilyDiscoversJojoWithTentaclesAndShitOhBoy = function() {
		EngineCore.clearOutput();
		CoC.getInstance().flags[ kFLAGS.AMILY_DISCOVERED_TENTATLE_JOJO ]++;
	...
	FollowerInteractions.prototype.jojoOffersRathazulMeditation = function() {
		EngineCore.outputText( '', true );
		CoC.getInstance().flags[ kFLAGS.JOJO_RATHAZUL_INTERACTION_COUNTER ]++;
	...
	FollowerInteractions.prototype.beAPimpMarbleLovesIt = function() {
		...
		if( CoC.getInstance().player.findPerk( PerkLib.MarblesMilk ) >= 0 ) {
			EngineCore.outputText( 'Marble blinks before saying "<i>' + CoC.getInstance().player.short + ', try again, I know you\'re better than that.</i>"  You blink in response and wonder yourself what possessed you to say that.\n\n', false );
	...
	FollowerInteractions.prototype.srslyPimpinGuyz = function() {
		...
		CoC.getInstance().player.itemSlot3.quantity = 0;
		CoC.getInstance().player.itemSlot4.quantity = 0;
		CoC.getInstance().player.itemSlot5.quantity = 0;
		CoC.getInstance().player.setArmor( ArmorLib.C_CLOTH ); //Old armor disappears unless it's Valeria
	...
	FollowerInteractions.prototype.askAboutAmilyPt2 = function() {
		...
		CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00350 ] = 1;
	};
	FollowerInteractions.prototype.cheatingSloots = function() {
		EngineCore.outputText( '', true );
		EngineCore.outputText( 'You kick open the door and roar, "<i>What the fuck is this!?</i>"  Both Amily and Urta turn to you and look at you with puzzled expressions before the severity of the situation seeps into their alcohol-fuzzed consciousnesses.  Almost simultaneously, they yell, "<i>' + CoC.getInstance().player.short + '!</i>" and stumble apart drunkenly, leaking each other\'s slime all over the floor.  Urta slurs, "<i>We didn\'t mean to... it jusht... happened...</i>"\n\n', false );
	...
	FollowerInteractions.prototype.beADickToAmily = function() {
		...
		EngineCore.outputText( '"<i>F-fuck you, ' + CoC.getInstance().player.short + ',</i>" she finally rejoins.  "<i>You did the same damn thing.</i>"\n\n', false );
	...
	FollowerInteractions.prototype.amilyUrtaSex = function() {
		...
		if( CoC.getInstance().player.cor > 50 && CoC.getInstance().flags[ kFLAGS.AMILY_WANG_LENGTH ] > 0 ) {
