...
angular.module( 'cocjs' ).run( function( EventParser, Appearance, ConsumableLib, CockTypesEnum, ImageManager, $rootScope, PerkLib, Descriptors, AppearanceDefs, CoC, kFLAGS, Utils, StatusAffects, EngineCore ) {
	function Marae() {
	...
	Marae.prototype.timeChange = function() {
		if( CoC.getInstance().time.hours > 23 ) {
			if( CoC.getInstance().flags[ kFLAGS.CORRUPT_MARAE_FOLLOWUP_ENCOUNTER_STATE ] > 0 ) {
				{ //Marae met 2nd time?
				}
				if( CoC.getInstance().flags[ kFLAGS.FUCK_FLOWER_KILLED ] === 0 ) {
					{ //If flower hasn't been burned down yet
					}
					if( CoC.getInstance().flags[ kFLAGS.FUCK_FLOWER_LEVEL ] < 4 && CoC.getInstance().flags[ kFLAGS.FUCK_FLOWER_GROWTH_COUNTER ] < 1000 ) {
						{ //Grow flower if it isn't fully grown.
						}
						CoC.getInstance().flags[ kFLAGS.FUCK_FLOWER_GROWTH_COUNTER ]++;
					}
				}
			}
			if( CoC.getInstance().flags[ kFLAGS.HOLLI_FUCKED_TODAY ] === 1 ) {
				CoC.getInstance().flags[ kFLAGS.HOLLI_FUCKED_TODAY ] = 0; //Holli Fuck Tracking
			}
		}
		return false;
	};
	...
	Marae.prototype.encounterMarae = function() {
		...
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.DungeonShutDown ) < 0 ) {
			...
			if( CoC.getInstance().player.findStatusAffect( StatusAffects.MetMarae ) < 0 ) {
				CoC.getInstance().player.createStatusAffect( StatusAffects.MetMarae, 0, 0, 0, 0 );
				...
				//Explain the dungeon scenario;
					...
					CoC.getInstance().player.createStatusAffect( StatusAffects.MaraesQuestStart, 0, 0, 0, 0 );
					...
					if( CoC.getInstance().player.lib + CoC.getInstance().player.cor > 80 ) {
					...
					if( CoC.getInstance().player.lib + CoC.getInstance().player.cor > 80 ) {
						...
						EngineCore.choices( 'Boob', this.grabHerBoob, '', null, '', null, '', null, 'Leave', CoC.getInstance().scenes.camp.returnToCampUseOneHour );
					} else {
			...
			//Second meeting;
				...
				if( CoC.getInstance().player.cor > 66 ) {
					...
					if( CoC.getInstance().player.findStatusAffect( StatusAffects.MaraesQuestStart ) >= 0 ) {
		...
		//Dungeon inoperable;
			...
			if( CoC.getInstance().player.findStatusAffect( StatusAffects.FactoryOverload ) < 0 ) {
				...
				CoC.getInstance().scenes.inventory.takeItem( ConsumableLib.P_PEARL, CoC.getInstance().scenes.camp.returnToCampUseOneHour );
				CoC.getInstance().player.createStatusAffect( StatusAffects.MaraeComplete, 0, 0, 0, 0 );
			}
			//Corrupt!;
			else {
				...
				if( CoC.getInstance().player.findStatusAffect( StatusAffects.MetCorruptMarae ) < 0 ) {
					CoC.getInstance().player.createStatusAffect( StatusAffects.MetCorruptMarae, 0, 0, 0, 0 );
	...
	Marae.prototype.grabHerBoob = function() {
		...
		CoC.getInstance().player.takeDamage( CoC.getInstance().player.HP - 1 );
		EngineCore.doNext( CoC.getInstance().scenes.camp.returnToCampUseOneHour );
	};
	...
	Marae.prototype.maraeBadEnd = function() {
		...
		if( CoC.getInstance().player.cocks.length === 0 ) {
			...
			if( CoC.getInstance().player.tentacleCocks() < CoC.getInstance().player.cockTotal() ) {
				//Single cawks;
	...
	Marae.prototype.maraeStealLethicite = function() {
		...
		EngineCore.outputText( '', true );
		//(SUCCESS);
		if( (CoC.getInstance().player.spe > 35 && (Utils.rand( CoC.getInstance().player.spe / 3 + 30 ) > 20)) || (CoC.getInstance().player.spe > 35 && CoC.getInstance().player.findPerk( PerkLib.Evade ) >= 0 && Utils.rand( 3 ) < 2) ) {
			...
			CoC.getInstance().player.createKeyItem( 'Marae\'s Lethicite', 0, 0, 0, 0 );
			EngineCore.doNext( CoC.getInstance().scenes.camp.returnToCampUseOneHour );
		}
		//(FAIL);
		else {
			...
			//DICK;
			if( CoC.getInstance().player.gender === 1 || (CoC.getInstance().player.gender === 3 && Utils.rand( 2 ) === 1) ) {
				...
				CoC.getInstance().player.createPerk( PerkLib.MaraesGiftStud, 0, 0, 0, 0 );
				EngineCore.doNext( CoC.getInstance().scenes.camp.returnToCampUseTwoHours );
			}
			//FEM);
			else {
				EngineCore.outputText( 'She extends a hand expectantly, watching with detached concentration while a tentacle lowers from the tree into her palm.  A swift slash of her free hand cuts your ' + CoC.getInstance().player.armorName + ' free, exposing your ', false );
				if( CoC.getInstance().player.gender === 0 ) {
				...
				CoC.getInstance().player.createPerk( PerkLib.MaraesGiftFertility, 0, 0, 0, 0 );
				EngineCore.doNext( CoC.getInstance().scenes.camp.returnToCampUseOneHour );
			}
		}
	};
	Marae.prototype.level2MaraeEncounter = function() {
		EngineCore.spriteSelect( 40 );
		CoC.getInstance().flags[ kFLAGS.CORRUPT_MARAE_FOLLOWUP_ENCOUNTER_STATE ] = 1;
		EngineCore.outputText( '', true );
		...
		//(STOLE);
		if( CoC.getInstance().player.hasKeyItem( 'Marae\'s Lethicite' ) >= 0 ) {
			EngineCore.outputText( 'Welcome back, sneak-thief.  What kind of ' + CoC.getInstance().player.mf( 'gentleman', 'lady' ) + ' is offered sex and then ransacks a god\'s soul?  Honestly, that right-right theeeeeree-oh yeah right there-is true depravity.</i>', false );
		}
		//(FAIL-STOLE);
		else if( CoC.getInstance().player.findPerk( PerkLib.MaraesGiftStud ) >= 0 || CoC.getInstance().player.findPerk( PerkLib.MaraesGiftFertility ) >= 0 ) {
			EngineCore.outputText( 'Hey there ' + CoC.getInstance().player.short + '.  I didn\'t think I\'d manage to snag you again so soon.  Are you enjoying my gifts?  I\'ve been feeling kind of lonely without anyone around here to play with.</i>', false );
		}
		//(Left Like a Bitch);
		else {
		...
		if( CoC.getInstance().player.findPerk( PerkLib.MaraesGiftFertility ) >= 0 || CoC.getInstance().player.findPerk( PerkLib.MaraesGiftStud ) >= 0 ) {
	...
	Marae.prototype.MaraeIIStageII = function() {
		...
		CoC.getInstance().flags[ kFLAGS.CORRUPT_MARAE_FOLLOWUP_ENCOUNTER_STATE ] = 2;
		//[Girls];
		...
		//Marae uses a tree-tentacle to 'milk' male PC's.  Oral or Vajayjay? Not sure.;
			...
			EngineCore.outputText( 'Vines whip out, sliding under your ' + CoC.getInstance().player.armorName + ', undoing clasps, and removing it until you stand naked and exposed to Marae.  She smirks and crooks her finger at you in a \'come hither\' gesture.  You lurch forward, as if pulled on an invisible string until you\'re standing inches away from her, your ' + Descriptors.cockDescript( 0 ) + ' rubbing her belly', false );
			...
			EngineCore.outputText( 'The tentacle on her right arm convulses, then splits open along four joints.  The tip folds open to reveal a pink, wriggling interior that promises pleasures mortal minds weren\'t meant to comprehend.  Meanwhile, while you\'re distracted by the eager plant-hole, the other tentacle slips behind you and climbs up your ' + CoC.getInstance().player.leg() + ', leaving a trail of slime in its wake.   It slides between your cheeks and prods at your ' + Descriptors.assholeDescript() + '.  You jerk forwards in surprise, but Marae pushes your ' + Descriptors.hipDescript() + ' back, allowing it to work its way inside.', false );
			...
			EngineCore.outputText( 'It doesn\'t matter, you\'re giving her what she wants.  You cum until your ' + CoC.getInstance().player.legs() + ' give out and you\'re sprawled on your back, ' + Descriptors.sMultiCockDesc() + ' being milked of its seed by the slurping, cunt-tentacles.  Every time you start to come down, the one in your backdoor rubs you just right and forces out another load.  Marae steps over your ' + CoC.getInstance().player.face() + ' and drops down, allowing you to lick the nectar that drips from her sensitive, flower-like folds while you cum.  It\'s sweet, potent, and refreshing.  It makes it easy to keep cumming but hard to stay awake, and your eyes roll back as you pass out from an overload of pleasure.', false );
		}
		//[Hermz]  Marae grows vinecawks for DP under her flowercunt and sexes.;
			...
			if( CoC.getInstance().player.cocks[ 0 ].cockThickness > 5 || CoC.getInstance().player.cockArea( 0 ) > 100 ) {
			...
			EngineCore.outputText( ' and erupts into Marae\'s womb.  Your hips rock forward, grazing her cervix with your ' + CoC.getInstance().player.cockHead() + ' to better fill her uterus.  ', false );
			if( CoC.getInstance().player.cockTotal() > 1 ) {
	...
	Marae.prototype.MaraePt2RoundIIIPrizes = function() {
		...
		//[HERMS];
			...
			//(HAZ BREEDER);
			else if( CoC.getInstance().player.findPerk( PerkLib.MaraesGiftFertility ) >= 0 ) {
	...
	CoC.getInstance().registerScene( 'marae', new Marae() );
} );