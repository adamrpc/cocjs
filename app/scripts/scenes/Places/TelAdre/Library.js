...
angular.module( 'cocjs' ).run( function( WeaponLib, PerkLib, Utils, StatusAffects, kFLAGS, CoC, EngineCore ) {
	function Library() {
	}
	...
	Library.prototype.visitZeMagesTower = function() {
		...
		EngineCore.addButton( 4, 'Back', CoC.getInstance().scenes.telAdre.telAdreMenu );
	};
	...
	Library.prototype.towerFollowUpVisits = function() {
		...
		//(follow-up visits, 18:00-20:00);
			...
			EngineCore.outputText( 'You return to the mage\'s tower.  Entering the main room, Quinn is nowhere to be seen.  The room looks slightly cleaner from when you saw it last, but at the rate he\'s going it seems like it will be some time before it\'s finished.' );
	...
	Library.prototype.commonQuinnTroduction = function() {
		EngineCore.outputText( ', though he does not seem to be reading it.  Stacks of books sit next to him.  As you close the door, he glances up at you.' );
	...
	Library.prototype.studyInTA = function() {
		...
		//[Study, 18:00-20:00];
			...
			EngineCore.outputText( ' through some of the literature collected by the Covenant.' );
			if( Utils.rand( 3 ) === 0 ) {
				//magic);
	...
	CoC.getInstance().registerScene( 'library', new Library() );
} );