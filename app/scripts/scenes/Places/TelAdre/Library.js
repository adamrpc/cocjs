...
angular.module( 'cocjs' ).run( function( SceneLib, WeaponLib, MainView, PerkLib, Utils, StatusAffects, kFLAGS, CoC, EngineCore ) {
	...
	Library.prototype.visitZeMagesTower = function() {
		...
		if( CoC.flags[ kFLAGS.TIMES_BEEN_TO_LIBRARY ] === 0 || CoC.time.hours <= 17 ) {
			...
			if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00175 ] > 0 ) {
				EngineCore.addButton( 2, 'Mali', this, this.talkToMali );
			}
		}
		if( CoC.flags[ kFLAGS.TIMES_VISITED_MALI ] > 0 ) {
			EngineCore.addButton( 2, 'Mali', this, this.talkToMali );
		}
		EngineCore.addButton( 0, 'Study', this, this.studyInTA );
	...
	Library.prototype.firstTowerVisit = function() {
		...
		MainView.outputText( 'You make your way to the largest fixture of the city, the impressive tower in the center.  The large spire could easily hold everyone you know ten times over and still have room to spare.  It is far too large for a city with Tel\'adre\'s population – but then, you reflect, so is Tel\'adre itself.' );
		...
		if( CoC.time.hours <= 17 ) {
			...
			MainView.outputText( '  A single man carefully turns through the pages of one book' );
	...
	Library.prototype.towerFollowUpVisits = function() {
		...
		if( CoC.flags[ kFLAGS.TIMES_BEEN_TO_LIBRARY ] === -1 ) {
			...
			if( CoC.time.hours <= 17 ) {
				MainView.outputText( 'You return to the mage\'s tower.  Entering the main room, you\'re surprised to see a man carefully turning the pages of one of the tomes' );
				this.commonQuinnTroduction();
			} else {
				MainView.outputText( 'As before, there\'s no one here.  At least there\'s no lack of reading material.  Looking up and down the staircases reveals two locked doors, so it\'s unlikely you could do anything but study here.' );
				CoC.flags[ kFLAGS.TIMES_BEEN_TO_LIBRARY ] = -2; //This will be incremented to -1 by the visitZeMagesTower function after we return
			}
			return;
		}
		...
		if( CoC.time.hours <= 17 ) {
			MainView.outputText( 'You return to the mage\'s tower.  Entering the main room, Quinn is carefully inspecting the pages of a book.  The room looks slightly more organized from when you last saw it, but it looks as though Quinn will be working on it for some time.' );
		...
		//(follow-up visits, 18:00-20:00);
			...
			MainView.outputText( 'You return to the mage\'s tower.  Entering the main room, Quinn is nowhere to be seen.  The room looks slightly cleaner from when you saw it last, but at the rate he\'s going it seems like it will be some time before it\'s finished.' );
	...
	Library.prototype.commonQuinnTroduction = function() {
		MainView.outputText( ', though he does not seem to be reading it.  Stacks of books sit next to him.  As you close the door, he glances up at you.' );
	...
	Library.prototype.studyInTA = function() {
		...
		//[Study, 18:00-20:00];
			...
			if( CoC.flags[ kFLAGS.TIMES_BEEN_TO_LIBRARY ] === -1 ) {
				MainView.outputText( 'Looking around you decide to spend some time reading' );
			} else {
				MainView.outputText( 'Without Quinn to hassle you and request your absence from the presences, you have some time to read' );
			}
			MainView.outputText( ' through some of the literature collected by the Covenant.' );
			if( Utils.rand( 3 ) === 0 ) {
				//magic);
	...
	Library.prototype.youOkayBuddy = function() {
		MainView.clearOutput();
		MainView.outputText( 'A bit perturbed by Quinn\'s countenance and apparent exhaustion you can\'t help but inquire as to his well-being.' );
	...
	Library.prototype.talkToMali = function() {
		...
		//[[Mali], CoC.player has spellblade];
			...
			MainView.outputText( 'You tell Quinn you\'re here to see Mali.  He seems intrigued by the wrapped blade you\'re carrying, but doesn\'t ask any questions.  Unlocking the second floor as usual, he escorts you to Mali\'s quarters.' );
		...
		//[[Mali], CoC.player does not have spellblade];
			...
			MainView.outputText( 'You tell Quinn you\'re here to see Mali.  He rolls his eyes but doesn\'t say anything, unlocking the second floor and leading you up once more.' );
