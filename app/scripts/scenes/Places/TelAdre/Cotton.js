...
angular.module( 'cocjs' ).run( function( MainView, SceneLib, $rootScope, $log, Appearance, PregnancyStore, Utils, ImageManager, StatusAffects, kFLAGS, Descriptors, CoC, EngineCore ) {
	...
	Cotton.prototype.cottonGreeting = function() {
		...
		if( CoC.flags[ kFLAGS.LIFETIME_GYM_MEMBER ] === 0 ) {
			MainView.outputText( 'The centauress sees you starting for the horse-girl and says, "<i>Go ahead and talk, but if you want to work out with her I\'ll have to charge you.</i>"\n\n', false );
		}
		//Greeting (first time):;
		if( CoC.flags[ kFLAGS.COTTON_MET_FUCKED ] === 0 ) {
			CoC.flags[ kFLAGS.COTTON_MET_FUCKED ] = 1;
			MainView.outputText( 'You wander over to the equine on the mat, curious as to what she\'s doing.  She stretches out on her mat, spine flexed.  Her head cranes backwards to see you approach.  "<i>Oh, hi there!</i>" She grins an upside-down grin at you and finishes her stretch before standing up.  ', false );
			//(If PC height is greater than 6'6</i>";
			if( CoC.player.tallness > 78 ) {
				MainView.outputText( 'Despite her impressive stature, you still look down on her.', false );
			}
			//(If PC height is in between:;
			else if( CoC.player.tallness >= 76 ) {
				MainView.outputText( 'She\'s roughly your height, which is rather impressive in itself.', false );
			}
			//(If PC height is under 6'4</i>":;
			else {
				MainView.outputText( 'She is truly tall, and you have to look up to her.', false );
			}
			MainView.outputText( '  She\'s garbed in a white and pink form-fitting tank top along with a pair of black skintight pants that come down to about her mid-shin.  The pants do absolutely nothing to hide the enormous bulge in her crotch, if anything they only enhance it.  Her dark brown skin seems smooth and hairless, unlike most equines you\'ve met, and her red mane of hair falls just past her shoulders, though it\'s currently pulled back into an efficient ponytail.\n\n', false );
			...
			MainView.outputText( '"<i>Hey, I\'m Cotton, what can I do you for?</i>" she says in a friendly manner.  You also give your name, and explain you were curious as to what she was doing.  "<i>Oh, this? Just doing some yoga.</i>"  Judging by your quizzical look, she continues, "<i>Yoga is like an exercise routine for your body and soul.  When the body is happy and healthy, the mind and soul follow.  It\'s a very relaxing and... sensual exercise.  Would you like to try it?</i>"', false );
			//[Yes] [No];
	...
	Cotton.prototype.cottonGreetingCommonEnd = function() {
		if( CoC.player.pregnancyIncubation <= 225 && CoC.player.pregnancyType === PregnancyStore.PREGNANCY_COTTON ) {
			...
			MainView.outputText( 'As you approach Cotton, she smiles and looks at your round belly.  "<i>Hey there my pet, I\'m afraid in your condition, yoga is out of the question... but we can do some special stretches and lamaze, just get dressed as usual.</i>"' );
			MainView.outputText( 'Do you want to engage in yoga lamaze with her?' );
		} else {
			switch( this.pregnancy.event ) {
				case 1:
					MainView.outputText( 'You approach Cotton, who, to your surprise, is just polishing off what must have been the largest glazed pastry you\'ve ever seen.  You notice muffin wrappers and other evidence of baked goods near her workout mat as well.  You ask if Cotton\'s been feeling a bit peckish recently... you correct yourself and ask if Cotton\'s been starving recently.' );
				...
				case 4:
					...
					break;
				default:
					MainView.outputText( 'You approach Cotton, who smiles and says, "<i>Hey there my little pet, if you\'d like a work out, just get changed and come on back.</i>"\n\nDo you want to engage in yoga with her?' );
			}
		}
		this.cottonMenu();
	};
	...
	Cotton.prototype.centaurNagaBodyBookStuff = function() {
		...
		//First time with book;
			...
			MainView.outputText( 'You approach Cotton, who gives you an exuberant grin. "<i>I\'ve read through the book and I\'m pretty confident I can coach you now. What do you say we give it a go? I\'ve arranged for some proper yoga clothes for you. Well, really just the top, but that\'s what matters for you, isn\'t it?</i>"\n\n', false );
			MainView.outputText( 'Do you want to engage in Yoga with her?', false );
			CoC.flags[ kFLAGS.COTTON_UNUSUAL_YOGA_BOOK_TRACKER ]++;
			this.cottonMenu();
		} else {
			this.cottonGreetingCommonEnd();
	...
	Cotton.prototype.acceptYoga = function() {
		...
		if( CoC.player.fatigue > 80 ) {
			MainView.outputText( 'You\'re way too tired to do any yoga right now.', false );
		...
		if( CoC.flags[ kFLAGS.LIFETIME_GYM_MEMBER ] === 0 && CoC.player.gems < 10 ) {
			MainView.outputText( 'Before you can start the yogo the centauress steps in and says, "<i>Ten gems for gym fees.</i>"\n\nYou fish around in your pouches, but you just don\'t have enough.  Maybe some other time!', false );
			EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
			return;
		}
		if( CoC.flags[ kFLAGS.LIFETIME_GYM_MEMBER ] === 0 ) {
			...
			MainView.outputText( 'The centauress collects ten gems for gym fees before the two of you can get into it.\n\n', false );
		}
		//(Yes) LAMAZE;
		if( CoC.player.pregnancyIncubation <= 225 && CoC.player.pregnancyType === PregnancyStore.PREGNANCY_COTTON ) {
			MainView.outputText( 'You change into your yoga clothes and approach Cotton, saying you\'d love a lamaze class. Cotton smiles and sets up a mat for you, then sits down, urging you to sit in front of her.  You do so, feeling the bulge in her pants pressing against your rump, and her breasts at your back.  You spend the next fifteen minutes doing breathing exercises like this, and another fifteen minutes doing stretches on an exercise ball.  As you\'re working out, Cotton presses her body against yours, running her hands around your swollen belly at every opportunity.\n\n' );
			//CHAT STUFF!;
			this.cottonChat();
			MainView.outputText( 'Once you\'re done and about to hit the showers, Cotton pulls you aside and says with a grin, "<i>Up for some post-workout exercises?</i>"', false );
		...
		//First time;
			...
			MainView.outputText( '"<i>Good, good, you won\'t regret it. First things first, pet, let\'s get you out of that dreadful clothing.</i>"  She leads you to the lockers and helps you strip out of your ' + CoC.player.armorName + '.', false );
			if( CoC.player.gender === 3 ) {
				MainView.outputText( '  She spies your crotch and smiles, "<i>Oh, best of both worlds, are we? Well you\'re in good company then.</i>"', false );
			}
			//(If PC is male:;
			else if( CoC.player.hasCock() ) {
				MainView.outputText( '  She cradles your ' + Descriptors.cockDescript( 0 ) + ' and smiles at you, "<i>Well, we might find a use for that later.</i>"', false );
			}//(If PC is female:;
			else if( CoC.player.hasVagina() ) {
				MainView.outputText( '  She pats your groin and says, "<i>Y\'know, I know some good exercises that really work the vaginal muscles... perhaps we\'ll talk about that later.</i>"', false );
			}
			MainView.outputText( '\n\n', false );
			MainView.outputText( 'She then helps you into an outfit identical to her own and you both return to the gym proper.\n\n', false );
			MainView.outputText( 'She instructs you to lay on the mat, and you do. She proceeds to talk you through several poses, and hovers over you to correct your positioning here or there with a soft touch. Several times she bends over you, and her enormous bosom gets right in your face. At one point she stands over your head and you accidentally get a face full of her musky crotch. After about a half hour of this, the arousal and fatigue is too much and you explain you have to quit for now.\n\n', false );
			EngineCore.dynStats( 'lus', (10 + CoC.player.lib / 10 + CoC.player.sens / 20) );
			MainView.outputText( '"<i>Oh, that\'s too bad. But you\'ve done pretty good for a beginner,</i>" she helps you up off the mat and pats you gently on the back. "<i>Want to hit the showers then?</i>" Despite having done little more than stretching, you find you are sweating quite a bit... but something makes you wonder if her idea of hitting the shower is the same as yours.', false );
			//[Shower] or [Leave];
		...
		//Done yoga > 0 && met type = 1;
			...
			MainView.outputText( 'You change into your yoga clothes and approach Cotton, saying you\'d love another yoga workout. Cotton smiles and sets up a mat for you. You spend the next half an hour trying different poses and stretches while she corrects you, sometimes pressing her body against yours to show you how it\'s done.\n\n', false );
			MainView.outputText( 'Once you\'re done and about to hit the showers, Cotton pulls you aside and says, "<i>I know you weren\'t comfortable with our shower before, so I won\'t join you this time. But if you ever change your mind, just say the word.</i>"', false );
	...
	Cotton.prototype.cottonChat = function() {
		...
		//(Scylla chat);
			...
			if( CoC.flags[ kFLAGS.NUMBER_OF_TIMES_MET_SCYLLA ] < 5 && CoC.flags[ kFLAGS.NUMBER_OF_TIMES_MET_SCYLLA ] > 0 ) {
				MainView.outputText( 'While you\'re doing your stretches, you find yourself chatting about the folks of Tel\'Adre. "<i>Scylla?</i>" Cotton says, "<i>She\'s the oddly dressed woman at the Wet Bitch, right? Can\'t say I know much about her. She\'s so secretive.</i>"\n\n', false );
			}
			//(if Scylla has formed support group);
			else if( CoC.flags[ kFLAGS.NUMBER_OF_TIMES_MET_SCYLLA ] >= 5 ) {
				MainView.outputText( 'While you\'re doing your stretches, you find yourself chatting about the folks of Tel\'Adre. "<i>Scylla?</i>" Cotton says, "<i>I hear she formed an addiction support group. Good on her, there\'s a lot of people in and around town who need help. I\'m glad she\'s stepping up.</i>"\n\n', false );
			}
		}
		//(Vala chat);
		else if( choice === 4 ) {
			...
			if( CoC.flags[ kFLAGS.FREED_VALA ] !== 0 ) {
				MainView.outputText( 'While you\'re doing your stretches, you find yourself chatting about the folks of Tel\'Adre. "<i>Vala?</i>" Cotton says, "<i>That\'s the new waitress at the Wet Bitch, right? She\'s cute. A shame what she\'s gone through. Sometimes I wish we could wipe out every last imp.</i>"\n\n', false );
			}
		}
		//(Jojo chat);
		else if( choice === 5 ) {
	...
	Cotton.prototype.cottonShowerFunTimes = function() {
		...
		var option2 = null;
		MainView.outputText( '', true );
		MainView.outputText( '"<i>Perfect! Let\'s go,</i>" she takes you by the arm and leads you back into the lockers, and then into the nearby showers, which are apparently unisex. You both strip down and turn on a shower head. After a few moments of scrubbing up, you give your shower buddy a surreptitious glance.\n\n', false );
		MainView.outputText( 'Without the clothes on, you can see her body is quite well toned. Not overly muscular, but you can easily make out the muscles on her belly. She sports a belly ring, and each nipple has a barbell piercing. Looking a bit lower, you finally take in her equine cock. It\'s quite large, at least a foot long flaccid, as it is now. You wonder how big it gets... She catches you looking and gives you a devilish grin.\n\n', false );
		MainView.outputText( '"<i>Well? Care for a little... post-workout stretching?</i>"', false );
	...
	Cotton.prototype.cottonFucksYou = function() {
		...
		//(Get fucked, as Female);
			...
			MainView.outputText( '\n\n', false );
			MainView.outputText( 'Before long you find yourself moaning beneath her, your cunt clenching and unclenching uncontrollably. "<i>Ooh, my little pet likes it now, hmmm?</i>" She whispers into your ear and nibbles on it ever-so-slightly. You can\'t help but give a breathless "<i>Yes</i>" in response. Cotton giggles and speeds up her thrusts. You find yourself pushing back into her, urging her to go deeper and deeper. Your clit twinges with pleasure after every thrust.\n\n', false );
			...
			MainView.outputText( 'The two of you slip down onto the floor, letting the water rinse you off for the time being. After a minute, Cotton pulls her now flaccid (but still impressive) cock from your folds, which gapes open for several minutes afterward, and proceeds to clean the both of you up.\n\n', false );
			MainView.outputText( 'Your \'post-workout stretch\' and shower done, the two of you dry off, redress and leave the gym. Cotton takes you by the arm and says, "<i>That was great, little pet. Come by the gym anytime. I\'ll be waiting.</i>"  Then, she heads back home.  With a little grin on your face, you do the same.', false );
			CoC.player.orgasm();
			EngineCore.dynStats( 'sen', -1 );
	...
	Cotton.prototype.fuckCottonInShowerRepeat = function() {
		...
		/*OLD ORIGINAL REPEAT SHOWER SEX
		 ...
		 MainView.outputText('Finally breaking the kiss, you reach down and hook your arm under one of her legs, lifting her leg up while you position your rock hard member at her pussy. She leans forward to kiss again as you thrust forwards, causing her to moan into your mouth. You thrust in and out while her horse-cock bounces between the two of you, only adding to the excitement.\n\n', false);
		 MainView.outputText('After several minutes, the hot shower, hot woman and hot sex are just too much for you, and give one last thrust, pushing your bodies forward to slam against the wall. It\'s clearly too much for Cotton as well, as she groans out in orgasmic ecstasy. Her cock, sandwiched between you, tenses and explodes just as yours does, covering Cotton in cum inside and out.\n\n', false);
		 MainView.outputText('After a moment you pull out, share a deep kiss, and wash each other up before redressing and leaving the gym.', false);
		 */
		//(Repeat Fuck Her, for centaurs);
		if( CoC.player.isTaur() ) {
			MainView.outputText( 'You decide to take her up on her offer and lead her into the showers, quickly disrobing and turning on an available shower-head. Cotton strips as well and you pull her under the stream of water, letting your horse body remain out of the water for now, sharing a kiss as steam begins to form around you. She runs a hand through your ' + Descriptors.hairDescript() + ' and grips the back of your neck, ' );
			if( this.pregnancy.event > 1 ) {
				MainView.outputText( 'pulling you gently towards her bulging belly, which you can\'t help but put your hands against as you kiss her. The bulge is firm and solid, almost a drum of solid muscle, and you caress it as you and the mare-morph make out, eliciting soft murmurs of appreciation for your efforts.\n\n' );
			} else {
				MainView.outputText( 'pulling you closer.\n\n', false );
			}

			//(If PC has one cock);
			if( CoC.player.cockArea( x ) >= 100 ) {
				if( CoC.player.cockTotal() === 1 ) {
					MainView.outputText( 'You feel your ' + Descriptors.cockDescript( x ) + ' stirring beneath your large body, while Cotton\'s remains curiously limp.  While her cock dangles, yours strains for attention.', false );
				}//(If PC has multiple cocks);
				else if( CoC.player.cockTotal() > 1 ) {
					MainView.outputText( 'You feel your ' + Descriptors.multiCockDescriptLight() + ' stirring beneath your large body, while Cotton\'s remains curiously limp.  While her cock dangles, your group of cocks strains for attention.', false );
				}
			} else {
				if( CoC.player.cockTotal() === 1 ) {
					MainView.outputText( 'You feel your ' + Descriptors.cockDescript( x ) + ' stirring beneath your large body, while you watch Cotton\'s do the same. Her cock rubs against your stomach while yours strains for attention.', false );
				}//(If PC has multiple cocks);
				else if( CoC.player.cockTotal() > 1 ) {
					MainView.outputText( 'You feel your ' + Descriptors.multiCockDescriptLight() + ' stirring beneath your large body, while you watch Cotton\'s do the same. Her cock rubs against your stomach while your group of cocks strains for attention.', false );
				}
			}
			//(If PC has a pussy, add the following);
			if( CoC.player.hasVagina() ) {
				MainView.outputText( '  Meanwhile, your ' + Descriptors.vaginaDescript( 0 ) + ' behind you moistens both from the steam and from arousal, and your ' + Descriptors.clitDescript() + ' aches, craving attention.', false );
			}
			MainView.outputText( '\n\n', false );
			//(If PC has breasts);
			if( CoC.player.biggestTitSize() >= 2 ) {
				MainView.outputText( 'Cotton leans down, groping your ' + Appearance.biggestBreastSizeDescript() + ', taking one ' + Descriptors.nippleDescript( 0 ) + ' into her mouth and sucking it greedily.', false );
				...
				//(else is PC is not lactating);
					...
					MainView.outputText( '  Cotton soon switches to the other breast, teasing your ' + Descriptors.nippleDescript( 0 ) + ' with her talented tongue.', false );
				}
				MainView.outputText( '\n\n', false );
			}
			MainView.outputText( 'Suitably turned on now, you look around and drag one of the nearby benches from the changing rooms into the showers and pull it under the water. Cotton grins, understanding what you plan to do with it, and gets onto all fours on top of it. You walk over the bench, getting your body under the spray of water, and get your cock into position. Your fellow equine lover helps with the fine movement, placing your cock at her entrance while giving your underside a reassuring pat. You slowly push forward, and Cotton bites her lip, again giving a reassuring pat.', false );
			//(If player has two cocks, add);
			if( CoC.player.cockTotal() === 2 ) {
				MainView.outputText( '  With a slight grin, you begin poking your extra dick at Cotton\'s other hole. You hear an indignant grunt, followed by a sigh, and feel her hands on your member, guiding it all the way into her hole.', false );
			}//(If the player has more cocks, add);
			else if( CoC.player.cockTotal() > 2 ) {
				MainView.outputText( '  The rest of your cocks jiggle and bob, rubbing up against her thighs and butt wildly.', false );
			}
			//(If player is too big, add);
			if( CoC.player.cockArea( x ) > 70 ) {
				MainView.outputText( '  Though you are far too big for her, you make sure to stuff her as much as you can. Her cunt, big as it is, squeezes tightly on your ' + Descriptors.cockDescript( x ) + '. She shivers and quakes, and leans against the bench, gripping your forelegs for support. You thrust your overly large tool inside her over and over, stretching her already not-insignificant cunt even wider. Her tongue actually rolls out of her snout, lolling to the side while her eyes roll up into her head, lost in the pleasure. Her body rocks with an orgasm while you piston away under the spray of water.', false );
			}//(if player is too small (under 4</i>");
			else if( CoC.player.cocks[ x ].cockLength < 4 ) {
				MainView.outputText( '  Her approval wavers and you hear from below you, "<i>Is it in yet?</i>" Your face flushes red and you confirm it is, looking down with embarrassment. Cotton rubs one of your forelegs reassuringly and says, "<i>' );
				if( this.pregnancy.event > 1 ) {
					MainView.outputText( 'Don\'t worry pet, it\'s plenty adequate for me.  And you\'ve got to admit,</i>" she smirks, rubbing her round belly, "<i>It gets the job done, doesn\'t it?' );
				} else {
					MainView.outputText( 'Don\'t worry pet, it\'s plenty adequate for me.' );
				}
				MainView.outputText( '</i>" Letting a little smile cross your lips, you push your ' + Descriptors.cockDescript( x ) + ' fully into her, not a big accomplishment for one of your size, but still enough to make you feel good.', false );
			}
			//(if none of the above, add);
			else {
				MainView.outputText( '  You piston back and forth, and though Cotton\'s cunt is sizable, it\'s able to grip you tightly. Your equine lover moans and grasps at your chest, eventually seizing your leg for support, though she leans backwards.', false );
			}
			MainView.outputText( '\n\n', false );
			MainView.outputText( 'You keep this up for a while, EngineCore.fatigue not really an issue with a body of your size and shape. Cotton, meanwhile, gasps for breath audibly and has taken to laying her front half down on the bench, her ass right up in the air. She strokes your forelegs lovingly, showing she\'s still ready for more.  ' );
			if( CoC.player.cockArea( x ) >= 100 ) {
				MainView.outputText( 'Oblivious to you, her own limp cock leaks cum down onto the bench as an orgasm rocks her body.' );
			} else if( this.pregnancy.event > 1 ) {
				MainView.outputText( 'Oblivious to you, her own cock twitches and spasms, letting loose a spray of cum across her swollen, pregnant belly, breasts and the bench as an orgasm rocks her body.' );
			} else {
				MainView.outputText( 'Oblivious to you, her own cock twitches and spasms, letting loose a spray of cum across her belly, breasts and the bench as an orgasm rocks her body.', false );
			}
			//(If player is too big, add);
			if( CoC.player.cockArea( x ) > 70 ) {
				MainView.outputText( '  Even with all the pounding you\'ve been doing, you can\'t fit your entire girth into her, but she doesn\'t care. Her eyes seem to roll back and forth like it\'s hard to focus, and her tongue drips water, sweat and saliva onto her body. She\'s panting for breath and you are able to make out faint, "<i>Oh gods, oh gods</i>" over the running water. Were you able to see her toned stomach, you\'re sure you could see the outline of your ' + Descriptors.cockDescript( x ) + ' in it. Her back arches under the shower, her red hair messily draped around her face and bench, and she bites her lip as another orgasm rocks her body.', false );
			}
			MainView.outputText( '\n\n', false );
			MainView.outputText( 'Finally you just can\'t take any more. You give one last thrust into your partner and audibly gasp in relief as an orgasm rolls across your body, hitting your nerves with bolts of ecstatic lightning.', false );
			//(If PC has a pussy);
			if( CoC.player.hasVagina() ) {
				MainView.outputText( '  Your ' + Descriptors.cockDescript( x ) + ' erupts inside Cotton\'s warm cunt which clenches you tightly, while your own femme sex twinges and clenches on air, drooling juices down your legs.', false );
			}//(else);
			else {
				MainView.outputText( '   Your ' + Descriptors.cockDescript( x ) + ' erupts inside Cotton\'s warm cunt which clenches you tightly.', false );
			}
			//(regardless or above, add);
			MainView.outputText( '  You give a couple more token thrusts and pull out. As you do, ', false );
			if( CoC.player.cumQ() < 200 ) {
				MainView.outputText( 'cum begins to seep from her folds', false );
			} else if( CoC.player.cumQ() < 700 ) {
				MainView.outputText( 'a gush of cum escapes from Cotton\'s folds', false );
			} else if( CoC.player.cumQ() < 1200 ) {
				MainView.outputText( 'a small torrent of cum escapes from Cotton\'s folds', false );
			} else if( CoC.player.cumQ() < 2000 ) {
				MainView.outputText( 'cum surges from her abused hole', false );
			} else {
				MainView.outputText( 'a massive flood of cum escapes from her cunt', false );
			}
			MainView.outputText( ', immediately getting washed towards the drain.\n\n', false );
			MainView.outputText( 'You both bask in the afterglow for a few moments before you stand up and help Cotton up. You return to the task of cleaning yourselves, sensually washing each other\'s private areas. Yoga, sex and cleanup done, you get dressed and leave the gym, giving Cotton\'s hand a final squeeze before departing.', false );
		}
		//(Repeat fuck her, for nagas);
			...
			if( CoC.player.cockArea( x ) >= 100 ) {
				//(If PC has one cock);
				if( CoC.player.cockTotal() === 1 ) {
					MainView.outputText( '  You feel your ' + Descriptors.cockDescript( x ) + ' stirring beneath you, slowly coming to attention, though Cotton\'s remains curiously limp.  Your ' + CoC.player.cockHead( x ) + ' rubs against her belly, sending ripples of pleasure up your spine.', false );
				}//(If PC has multiple cocks);
				else if( CoC.player.cockTotal() > 1 ) {
					MainView.outputText( '  You feel your ' + Descriptors.multiCockDescriptLight() + ' stirring beneath you, slowly coming to attention, though Cotton\'s remains curiously limp.  The group of cocks rubs together as you make out, sending ripples of pleasure up your spine.', false );
				}
			} else {
				//(If PC has one cock);
				if( CoC.player.cockTotal() === 1 ) {
					MainView.outputText( 'You feel your ' + Descriptors.cockDescript( x ) + ' stirring beneath you, slowly coming to attention alongside Cotton\'s equine member.  The two rub together as you make out, sending ripples of pleasure up your spine.', false );
				}//(If PC has multiple cocks);
				else if( CoC.player.cockTotal() > 1 ) {
					MainView.outputText( 'You feel your ' + Descriptors.multiCockDescriptLight() + ' stirring beneath you, slowly coming to attention alongside Cotton\'s equine member. The group of cocks rubs together as you make out, sending ripples of pleasure up your spine.', false );
				}
			}
			//(If PC has a pussy, add the following);
			if( CoC.player.hasVagina() ) {
				MainView.outputText( 'Your ' + Descriptors.vaginaDescript( 0 ) + ' meanwhile moistens both from the water and from arousal, and your ' + Descriptors.clitDescript() + ' aches, craving attention.', false );
			}
			MainView.outputText( '\n\n', false );
			//(If PC has breasts);
			if( CoC.player.biggestTitSize() >= 2 ) {
			...
			MainView.outputText( 'Suitably turned on now, you coil your body around Cotton\'s torso, with your tail spreading her legs apart while you place your ' + Descriptors.cockDescript( x ) + ' at her pussy. You slowly push in, and Cotton bites her lip, looking at you in approval.', false );
			//(If player has two cocks, add);
			if( CoC.player.cockTotal() === 2 ) {
				MainView.outputText( '  With a slight grin, you position your extra dick at Cotton\'s other hole. She quirks her eyebrow and a look of panic momentarily crosses her face, but you push forward anyway. Cotton\'s eyes roll up briefly and her lip quivers.', false );
			}//(If the player has more cocks, add);
			else if( CoC.player.cockTotal() > 2 ) {
				MainView.outputText( '  The rest of your cocks strain, aching for holes to fill. With none available, they throb as they rub against Cotton\'s smooth skin.', false );
			}
			//(If player is too big, add);
			if( CoC.player.cockArea( x ) > 70 ) {
				...
				MainView.outputText( '  Her approval wavers and she asks, "<i>Is it in yet?</i>" Your face flushes red and you confirm it is, looking down with embarrassment. Cotton lifts your chin and gives you a kiss, "<i>' );
				if( this.pregnancy.event > 1 ) {
					MainView.outputText( 'Don\'t worry pet, it\'s plenty adequate for me.  And you\'ve got to admit,</i>" she smirks, rubbing her round belly, "<i>It gets the job done, doesn\'t it?' );
				} else {
					MainView.outputText( 'Don\'t worry pet, it\'s plenty adequate for me.' );
				}
				MainView.outputText( '</i>" Giving her a little smile, you push your ' + Descriptors.cockDescript( x ) + ' fully into her, not a big accomplishment for one of your size, but still enough to give you a little smile.', false );
			}
			//(otherwise add);
			else {
				MainView.outputText( '  You piston back and forth, and though Cotton\'s cunt is sizeable, it\'s able to grip you tightly. Your equine lover moans and grasps at your chest, eventually seizing your shoulder for support, though she leans backwards.', false );
			}
			MainView.outputText( '\n\n', false );
			MainView.outputText( 'You keep this up for a while, before fatigue takes over. You carefully lay Cotton down in the spray of water, coiling your body under her so her rear end is up in the air, hooves resting on the ground, and continue your work unhindered. She kisses your lips, neck, and whatever she can get her mouth on.  ' );
			if( CoC.player.cockArea( x ) >= 100 ) {
				MainView.outputText( 'Her own cock, still limp, visibly twitches and begins leaking cum onto her belly as an orgasm rocks her body.' );
			} else if( this.pregnancy.event > 1 ) {
				MainView.outputText( 'Her own cock twitches and spasms, letting loose a spray of cum across her swollen, pregnant belly, breasts and face as an orgasm rocks her body.' );
			} else {
				MainView.outputText( 'Her own cock twitches and spasms, letting loose a spray of cum across her belly, breasts and face as an orgasm rocks her body.', false );
			}
			//(If player is too big, add);
			if( CoC.player.cockArea( x ) > 70 ) {
				MainView.outputText( '  Even in this pseudo-missionary position you can\'t stuff your entire enormous girth into her, but she doesn\'t care. Her eyes seem to roll back and forth like it\'s hard to focus, and her tongue drips water, sweat and saliva onto her body. She\'s panting for breath and you are able to make out faint, "<i>Oh gods, oh gods</i>" over the running water. You can even make out the shape of your cock in Cotton\'s toned stomach, pumping back and forth, which brings a smile to your face. Her back arches under the shower, her hair messily spread out beneath her and she bites her lip as another orgasm rocks her body.', false );
			}
			MainView.outputText( '\n\n', false );
			MainView.outputText( 'Finally you just can\'t take anymore. You give one last thrust into your partner and audibly gasp in relief as an orgasm rolls across your body, hitting your nerves with bolts of ecstatic lightning.', false );
			//(If PC has a pussy);
			if( CoC.player.hasVagina() ) {
				MainView.outputText( '  Your ' + Descriptors.cockDescript( x ) + ' erupts inside Cotton\'s warm cunt, which clenches you tightly, while your own femme sex twinges and clenches on air, drooling juices down your legs.', false );
			}//(else);
		...
		//(Repeat Fuck Her, for humanoid bodies);
			...
			if( this.pregnancy.event > 1 ) {
				...
				MainView.outputText( 'pulling you closer.', false );
			}
			...
			if( CoC.player.hasVagina() ) {
				MainView.outputText( '  Your ' + Descriptors.vaginaDescript( 0 ) + ' moistens both from the water and from arousal, and your ' + Descriptors.clitDescript() + ' aches, craving attention.', false );
			}
			MainView.outputText( '\n\n', false );
			//(If PC has breasts);
			if( CoC.player.biggestTitSize() >= 2 ) {
			...
			MainView.outputText( 'Suitably turned on now, you reach down and hook your arm under one of Cotton\'s legs, lifting it up while you position your ' + Descriptors.cockDescript( x ) + ' at her pussy. You slowly push in, and Cotton bites her lip, looking at you in approval.', false );
			//(If player has two cocks, add);
			if( CoC.player.cockTotal() === 2 ) {
				MainView.outputText( '  With a slight grin, you position your other dick at Cotton\'s other hole. She quirks her eyebrow and a look of panic momentarily crosses her face, but you push forward anyway. Cotton\'s eyes roll up briefly and her lip quivers.', false );
			}
			//(If the player has more cocks, add);
			if( CoC.player.cockTotal() > 2 ) {
				MainView.outputText( '  The rest of your cocks strain, aching for holes to fill. With none available, they throb as they rub against Cotton\'s smooth skin.', false );
			...
			if( CoC.player.cockArea( x ) > 70 ) {
				...
				MainView.outputText( '  Her approval wavers and she asks, "<i>Is it in yet?</i>" Your face flushes red and you confirm it is, looking down in embarrassment. Cotton lifts your chin and gives you a kiss, "<i>' );
				if( this.pregnancy.event > 1 ) {
					MainView.outputText( 'Don\'t worry pet, it\'s plenty adequate for me.  And you\'ve got to admit,</i>" she smirks, rubbing her round belly, "<i>It gets the job done, doesn\'t it?' );
				} else {
					MainView.outputText( 'Don\'t worry pet, it\'s plenty adequate for me.' );
			...
			MainView.outputText( '\n\n', false );
			MainView.outputText( 'You keep this up for a while before fatigue takes over. You carefully lay Cotton down in the spray of water and continue your work unhindered. She kisses your lips, neck, and whatever she can get her mouth on.  ' );
			if( CoC.player.cockArea( x ) >= 100 ) {
				MainView.outputText( 'Her own cock, still limp, visibly twitches and begins leaking cum onto her belly as an orgasm rocks her body.' );
			} else if( this.pregnancy.event > 1 ) {
				MainView.outputText( 'Her own cock twitches and spasms, loosing a spray of cum across her swollen, pregnant belly, breasts and face as an orgasm rocks her body.' );
			} else {
				MainView.outputText( 'Her own cock twitches and spasms, letting loose a spray of cum across her belly, breasts and face as an orgasm rocks her body.', false );
			}
			//(If player is too big, add);
			if( CoC.player.cockArea( x ) > 70 ) {
				MainView.outputText( '  Even in the missionary position you can\'t stuff your entire enormous girth into her, but she doesn\'t care. Her eyes seem to roll back and forth like it\'s hard to focus, and her tongue drips water, sweat and saliva onto her body. She\'s panting for breath and you are able to make out faint, "<i>Oh gods, oh gods</i>" over the running water. You can even make out the shape of your cock in Cotton\'s toned stomach, pumping back and forth, which brings a smile to your face. Her back arches under the shower, her hair messily spread out beneath her and she bites her lip as another orgasm rocks her body.', false );
			}
			MainView.outputText( '\n\n', false );
			MainView.outputText( 'Finally you just can\'t take any more. You give one last thrust into your partner and audibly gasp in relief as an orgasm rolls across your body, hitting your nerves with bolts of ecstatic lightning.', false );
			//(If PC has a pussy);
	...
	Cotton.prototype.cottonFucksYouInShowerRepeat = function() {
		...
		CoC.flags[ kFLAGS.COTTON_MET_FUCKED ] = 2;
		MainView.outputText( '', true );
		MainView.outputText( ImageManager.showImage( 'cotton-shower-fucks-you-repeat' ) );
		/*OLD SEX SCENES HERE
		 ...
		 MainView.outputText('Finally breaking the kiss, Cotton reaches down and hooks her arms under both your legs. You quickly wrap your arms around her neck as she lifts you off the ground. You carefully grip her with your legs as she uses one arm to position her dick at your waiting entrance. You give her a kiss just as she presses into you, and moan into her mouth. She gives a couple careful thrusts before her free hand returns to holding you.\n\n', false);
		 MainView.outputText('At this angle she can\'t quite get her entire girth into you, but that doesn\'t matter, as it feels absolutely exquisite.', false);
		 CoC.player.buttChange(72,true,true,false);
		 MainView.outputText('  You take turns kissing each other\'s necks and nibbling each other\'s ears while she thrusts in and out. And after several minutes neither of you can take much more. She gives one last thrust, pulling you down further onto her cock as it explodes within you. Your whole body shudders with orgasmic energy and you bury your head into her neck, stifling a scream.\n\n', false);
		 MainView.outputText('After a moment, Cotton pulls you up, letting her shrinking member flop to the floor, and sets you down. Only now you notice your flaccid cock also came at some point, covering both your bodies in seed. You share a deep kiss again and wash each other up before redressing and leaving the gym.\n\n', false);
		 CoC.player.orgasm();
		 EngineCore.dynStats('sen', 1);
		 }
		 ...
		 CoC.player.cuntChange(72,true,true,false);
		 MainView.outputText('\n\n', false);
		 MainView.outputText('At this angle she can\'t quite get her entire girth into you, but that doesn\'t matter, as it feels absolutely exquisite. You take turns kissing each other\'s necks and nibbling each other\'s ears while she thrusts in and out. And after several minutes neither of you can take much more. She gives one last thrust, pulling you down further onto her cock as it explodes within you. Your whole body shudders with orgasmic energy and you bury your head into her neck, stifling a scream.\n\n', false);
		 MainView.outputText('After a moment, Cotton pulls you up, letting her shrinking member flop to the floor, and sets you down. You share a deep kiss again and wash each other up before redressing and leaving the gym.', false);
		 CoC.player.orgasm();
		 EngineCore.dynStats('sen', -1);
		 }*/
		...
		if( CoC.player.isTaur() ) {
			...
			if( CoC.player.gender === 1 ) {
				MainView.outputText( '  Your ' + Descriptors.cockDescript( 0 ) + ' tingles betwixt your legs, but remains limp in the presence of Cotton\'s impressive member.', false );
			}
			//(If PC has a penis and vagina);
			else if( CoC.player.gender === 3 ) {
				MainView.outputText( '  Your ' + Descriptors.cockDescript( 0 ) + ' tingles betwixt your legs, but remains limp in the presence of Cotton\'s impressive member, while your ' + Descriptors.vaginaDescript( 0 ) + ' moistens almost immediately from the steam and arousal.', false );
			}
			//(If PC has a vagina and no penis);
			else if( CoC.player.gender === 2 ) {
				MainView.outputText( '  Your ' + Descriptors.vaginaDescript( 0 ) + ' moistens almost immediately as you make out, both from the steam and your increasing arousal.', false );
			}
			//(If PC is genderless);
			else {
				MainView.outputText( '  A deep aching burns within you, a need your body is ill-equipped to process, but still your nipples harden and you find your ' + Descriptors.assholeDescript() + ' puckering in anticipation.', false );
			}
			MainView.outputText( '\n\n', false );
			//(If PC has breasts);
			if( CoC.player.biggestTitSize() >= 2 ) {
				MainView.outputText( 'Cotton\'s kisses lead down to your ' + Appearance.biggestBreastSizeDescript() + ', where she takes one ' + Descriptors.nippleDescript( 0 ) + ' into her mouth, sucking it greedily and teasing it masterfully.', false );
				//(and if PC is lactating);
				if( CoC.player.biggestLactation() >= 1 ) {
			...
			MainView.outputText( 'She continues kissing down your belly, reaching your centaur body. She makes you turn around so your rear end is in the spray of water with her.', false );
			//(If PC has a large penis, bigger than Cotton's capacity, add);
			if( CoC.player.hasCock() ) {
				if( CoC.player.cockArea( 0 ) > 70 ) {
					MainView.outputText( '  "<i>Oh my, what\'s this?</i>" She puts a hand under your enormous, yet embarrassingly limp cock and lifts it slightly. "<i>My little pet has such a big dick... Just how I like it. Perhaps next time I\'ll get to try it out... but not today, hm? This is all about you right now.</i>"', false );
				}//(If PC has a penis under 4</i>", add);
				else if( CoC.player.longestCockLength() < 4 ) {
					MainView.outputText( '  "<i>Awww, what\'s this?</i>" She puts a hand under your embarrassingly small and limp cock and lifts it slightly. "<i>It\'s so cute and tiny. And it certainly knows its place. Only room for one cock right now, not that this is much of a cock.</i>" She giggles and plants a kiss on the tip, "<i>It is cute though. I love it.</i>"', false );
				}//(If PC has a penis neither large or small, add);
				else {
					MainView.outputText( 'Cotton puts a hand under your embarrassingly limp cock and smiles, "<i>You know how to show a girl you like her... There\'s only room for one cock right now.</i>"', false );
				}
			}
			//(If PC has a vagina);
			if( CoC.player.hasVagina() ) {
				MainView.outputText( '  Spying your ' + Descriptors.vaginaDescript( 0 ) + ', Cotton smiles and flicks your ' + Descriptors.clitDescript() + ' teasingly before slipping two fingers inside your folds and bringing them to her mouth, licking them clean. "<i>Mmm... I love the taste of your juices, pet...</i>"', false );
			}
			//(if PC is genderless);
			if( CoC.player.gender === 0 ) {
				MainView.outputText( '  She places a hand on your bare crotch and quirks an eyebrow. "<i>Well this is new... Certainly not bad though.</i>" She runs a hand along your bare mound, which somehow manages to send ripples of pleasure up your spine.', false );
			}
			MainView.outputText( '\n\n', false );
			MainView.outputText( 'Cotton continues the kisses down to your ' + CoC.player.legs() + ' and stands, dragging over a bench from the locker room before standing on it and giving your ' + Descriptors.buttDescript() + ' a good smack. You turn back and give her a coy look, which she returns and gives your flank another smack.', false );
			MainView.outputText( '  She takes a moment to get some water from the shower over your rear end before pressing her cock against your ' + CoC.player.assholeOrPussy() + ', slipping it in gently, careful not to go too quick. You moan slightly and blush, whispering back at her, urging her to continue.', false );
		...
		//(Repeat get fucked for humanoids and nagas);
			...
			//(If PC has a penis, no vagina);
			if( CoC.player.gender === 1 ) {
				MainView.outputText( '  Your ' + Descriptors.cockDescript( 0 ) + ' tingles beneath you, but remains limp in the presence of Cotton\'s impressive member.', false );
			}
			//(If PC has a penis and vagina);
			else if( CoC.player.gender === 3 ) {
				MainView.outputText( '  Your ' + Descriptors.cockDescript( 0 ) + ' tingles beneath you, but remains limp in the presence of Cotton\'s impressive member while your ' + Descriptors.vaginaDescript( 0 ) + ' moistens almost immediately from the steam and arousal.', false );
			}
			//(If PC has a vagina and no penis);
			else if( CoC.player.gender === 2 ) {
				MainView.outputText( '  Your ' + Descriptors.vaginaDescript( 0 ) + ' moistens almost immediately as you make out, both from the steam and your increasing arousal.', false );
			...
			MainView.outputText( 'She continues kissing a trail down your belly and to your crotch.', false );
			if( CoC.player.hasCock() ) {
				//(If PC has a large penis, bigger than Cotton's capacity, add);
			...
			if( CoC.player.isNaga() ) {
				MainView.outputText( 'Cotton continues the kisses down to your ' + CoC.player.legs() + ', and stands, grabbing you by the waist and lifting you up off the ground.  ' );
				if( this.pregnancy.event > 1 ) {
					MainView.outputText( 'Moving quickly, you wrap your arms around Cotton\'s neck and coil your body around her waist, just below her round, pregnant belly, and down one leg.  You can feel her swollen midriff rubbing against the scales on your serpentine half with each motion you make.' );
				} else {
					MainView.outputText( 'Moving quickly, you wrap your arms around Cotton\'s neck and coil your body around her waist and down one leg.', false );
				}
			}
			//(If Humanoid body);
			else {
				MainView.outputText( 'Cotton continues the kisses down to your ' + CoC.player.legs() + ' and stands, hooking an arm underneath each of your legs and lifting you up off the ground.  ' );
				if( this.pregnancy.event > 1 ) {
					MainView.outputText( 'Moving quickly, you wrap your arms and legs around Cotton so you don\'t fall over, carefully avoiding her pregnant belly, which is no mean feat. Her belly is pressed into yours, letting you feel the solid weight of it, gentle kicks from your unborn child within.' );
				} else {
					MainView.outputText( 'Moving quickly, you wrap your arms and legs around Cotton so you don\'t fall over.', false );
				}
			}
			MainView.outputText( '  She gives you a quick kiss and moves her cock to your ' + CoC.player.assholeOrPussy() + ', slipping it in gently, careful not to go too quick. You give her a submissive, but encouraging kiss, and whisper in her ear, urging her to continue.', false );
			//(Stretch and appropriate virginity check);
			if( CoC.player.hasVagina() ) {
				CoC.player.cuntChange( 72, true, true, false );
			} else {
			...
			MainView.outputText( '\n\n', false );
			MainView.outputText( 'She can\'t quite fit her entire girth into you at this angle, but that doesn\'t matter as it feels absolutely exquisite. You take turns kissing each other\'s necks and nibbling on each other\'s ears while she thrusts in and out.  ' );
			if( this.pregnancy.event > 1 ) {
				MainView.outputText( 'Cotton leans her gravid bulge heavily into your belly, bending over as best she can to whisper into her ear.  "<i>Mmm... how do you like this? Knowing you\'re being fucked like a woman by a woman?  A woman carrying your child, no less?  Because I like this a lot - this is so kinky, to me... oh, I\'m gonna fuck the shit out of you, just like you fucked this baby into me.</i>" She croons, even as she continues to thrust.  ' );
			}
			MainView.outputText( 'Your body shivers and quakes as a miniature orgasm rockets through you, somehow making your whole body more sensitive.  ' );
			MainView.outputText( 'Your powerful partner holds you close, her jet black cock pistoning in and out of your hole. You feel safe in her arms, secure in a way you rarely feel elsewhere. You wrap your arms more tightly around her, hugging Cotton close and biting your lip, stifling your moans and groans of passion.\n\n', false );
			MainView.outputText( 'Unable to take any more, Cotton gives one last thrust, pulling your body down onto her cock and giving a whinny of a moan. A full orgasm rocks through your body, like lightning striking every nerve. Your back arches, pressing your chest into Cotton\'s prodigious bosom and even biting your lip doesn\'t stifle the long, deep moan from escaping your lips. Your equine lover\'s cock erupts in you, coating your insides with her hot, sticky seed, which even now begins to leak from your hole around the invading cock.\n\n', false );
	...
	Cotton.prototype.cottonTantricSex = function() {
		...
		CoC.flags[ kFLAGS.COTTON_MET_FUCKED ] = 2;
		//OLD TANTRIC SMEX;
		/*MainView.outputText('You decide to ask instead about using yoga to blow off some steam. She grins, "<i>I know just what you need. It\'s called tantric sex, and I think you\'re just limber and quick enough to try it.</i>"\n\n', false);
		 MainView.outputText('She leads you to a private room in the gym and sets up a larger mat. She disrobes and you do the same. She sits on the mat and you sit opposite her. You spend a couple minutes just breathing, exploring your partner\'s body with your eyes only. Then Cotton slips her legs outwards, and leans backwards, letting her enormous member to waggle freely in the air. She curls a finger at you, and gives you your instructions.\n\n', false);
		 MainView.outputText('You stand and straddle her hips, facing her, carefully lowering yourself down onto her length. You take your time, matching your breathing with hers, until you\'re completely impaled. Then you stretch your legs out behind your partner and also lean back. You both gyrate your hips as your bodies glisten with a light sheen of sweat.', false);
		 CoC.player.buttChange(72,true,true,false);
		 MainView.outputText('\n\n', false);
		 MainView.outputText('After several minutes of this you switch positions, with Cotton leaning forward and grabbing you under the waist while you curl your legs under you. Then Cotton pulls herself up into a kneeling position while still inside you, pushing your head and shoulders down onto the mat and your legs up into the ceiling. Several more positions later, your breathing is a little ragged but still in sync, and it\'s clear neither of you can take much more of this.\n\n', false);
		 MainView.outputText('With one last pose, you both stand, Cotton grabs your hips and you lean down to grab your ankles. Your whole body shivers with sexual energy, feeling like it\'s on fire with orgasmic light as your equine lover gives one final thrust into you and explodes, her own body shivering and shuddering. Unable to remain standing any longer, you flop down onto the mat, letting Cotton\'s seed slowly leak out of you, while Cotton lays down next to you, equally exhausted.\n\n', false);
		 MainView.outputText('"<i>I must say, my little pet, you are the best yoga partner I\'ve had in a long time.</i>" You share a kiss and take a moment to rest before heading to the showers and then heading home.', false);*/
		//NU HOTNESSSSS;
		//(Tantric Sex ONLY available for humanoids and naga, still requires 50+ Speed);
		MainView.outputText( 'Knowing full well what she plans for the showers, you instead ask about more "<i>advanced</i>" forms of yoga. With a certain coy emphasis on advanced. She grins, "<i>I know just what you mean. It\'s called tantric sex, and I think you\'re just limber and quick enough to try it.</i>"\n\n', false );
		MainView.outputText( 'She leads you to a private area in the gym, a large room where one side of the wall is covered in mirrors with a long bar across it. You\'d guess this room is used for dance lessons, or other activities that require large amounts of space. She sets up a large mat on the floor, big enough for the both of you to sit on and then some, and proceeds to strip. "<i>It\'s best to do this naked... we don\'t want our clothing to block our spiritual energies. Or, y\'know, other things,</i>" she says with a light laugh. You nod and strip down as well.\n\n', false );
		MainView.outputText( 'Cotton sits on one end of the mat, crossing her hooved feet under her in the traditional meditative position. You sit on the other side and do the same. "<i>Now we match our breathing, little pet. In, and out. Each breath brings our spirits closer to each other.</i>" You inhale deeply, and she does the same. Then out. This continues for minutes, just breathing, until in one exhalation you feel a shiver run down your spine.\n\n', false );
		MainView.outputText( '"<i>And our spirits are joined... for now,</i>" she laughs and runs her hands along her upper torso, and in a weird, sort of detached way, you feel it. You touch your chest, rubbing a ' + Descriptors.nippleDescript( 0 ) + ' experimentally, and you see Cotton visibly shiver and give you an approving nod. Whatever she did... you can feel what she feels, however faintly, and vice-versa!\n\n', false );
		MainView.outputText( '"<i>Now comes the yoga portion of this little workout,</i>" she says as she uncurls her legs, spreading them wide and long on the mat. She bends her knees and leans back, propping herself up on her arms. Her jet-black equine cock stands at full attention, pulsing steadily. "<i>Come sit down on my cock, pet, and we\'ll get this party started right.</i>"\n\n', false );
		MainView.outputText( 'Gulping slightly, you stand and approach her. Cotton runs a hand along her member, causing you to shiver with the', false );
		if( CoC.player.hasCock() ) {
			MainView.outputText( ' familiar', false );
		} else {
			MainView.outputText( ' alien', false );
		}
		MainView.outputText( ' sensations on an unfamiliar part of your body... well, her body. You stand over her hips and slowly lower yourself down, impaling your ' + CoC.player.assholeOrPussy() + ' on her meaty tool. In addition to the beautiful sensation of being filled, you also get the sensation of warm, tight constriction on your phantom cock.', false );
		//(for Naga Body, add);
		if( CoC.player.isNaga() ) {
			MainView.outputText( '  You impale yourself fully, settling down and sitting on her lap, stretching your snake-like body out behind and to one side of her, with your arms propping you up as you lean back.', false );
		}//(for Humanoid Body, add);
		else {
			MainView.outputText( '  You impale yourself fully, settling down and sitting on her lap, mimicking her pose with your legs stretched out behind her and your arms propping you up as you lean back.', false );
		}
		MainView.outputText( '\n\n', false );
		MainView.outputText( 'Though there\'s no real thrusting or vulgar movements of any kind, the feelings are intense, likely heightened by your tantric link. You can simultaneously feel Cotton\'s long equine cock pulse within you, as well as feel your own ' + CoC.player.assholeOrPussy() + ' clamping down on a cock that is not your own.', false );
		if( CoC.player.hasVagina() ) {
			CoC.player.cuntChange( 72, true, true, false );
		} else {
			CoC.player.buttChange( 72, true, true, false );
		...
		MainView.outputText( '\n\n', false );
		MainView.outputText( 'Now comes the thrusting. Or rather, gyrating. Cotton begins making small circular motions with her hips, letting her cock thrust ever-so-slightly in and out of you. Despite the subtle motions, combined with the tantric link you\'re in complete and utter bliss. Soon, you too are gyrating your hips the best you can.\n\n', false );
		MainView.outputText( 'It doesn\'t take much longer for the sensations to consume you, feeling yourself both getting fucked and fucking at the same time. Lightning fires through every nerve in your body as an orgasm begins to overtake you. Cotton doesn\'t look far behind. As your body begins to tremble and arch, so too does hers.', false );
		//(If PC has a penis);
		if( CoC.player.hasCock() ) {
			MainView.outputText( '  Your ' + Descriptors.cockDescript( 0 ) + ' twitches and tingles, releasing its pent up seed in a dribbling torrent down your belly and chest, right into your face and pooling around your head.', false );
		}
		//(If PC has a vagina);
		if( CoC.player.hasVagina() ) {
			MainView.outputText( '  Your ' + Descriptors.vaginaDescript( 0 ) + ' shivers and clamps down hard on the invading cock, pulsing and milking it as it unloads directly into your womb.', false );
		}//(If PC doesn't have a vagina);
		else {
			MainView.outputText( '  Your ' + Descriptors.assholeDescript() + ' shivers and clamps down hard on the invading cock, pulsing and milking it as it unloads directly into your intestines.', false );
		}
		//(regardless of above, add);
		MainView.outputText( '  Lightning arcs from nerve to nerve, in both of your bodies. You can feel every sensation Cotton feels. In that moment, you feel perfectly as one. You aren\'t simply getting fucked by Cotton. You are simultaneously Cotton fucking yourself, as is she.', false );
		MainView.outputText( '\n\n', false );
		MainView.outputText( 'Then in a flash, it\'s gone. The lightning subsided, you both collapse sideways onto the floor, panting. The tantric link is apparently gone, and you suddenly feel very alone. Cotton, seemingly sensing this, pulls out and curls up next to you, cuddling you until the sudden feeling of loss dissipates.\n\n', false );
		MainView.outputText( 'After a few minutes, you feel your strength ebbing back into you and you get up. Cotton does the same, and you both spend the time to clean the mat of the results of your escapades, before retreating to the showers to clean each other up. Once that\'s over, you both leave the gym hand-in-hand, with Cotton giving your hand a final squeeze before you depart to your camp.', false );
		CoC.player.orgasm();
	...
	Cotton.prototype.nomSomeTitMilkCereal = function() {
		...
		//(Event proper);
		MainView.outputText( 'You quickly arise and ready your weapon, prepared for anything to come through the brush. After a moment, a large dark shape bursts through the bushes. You\'re about to strike when you recognize the figure! It\'s Cotton from the gym in Tel\'Adre! Rather than her normal yoga outfit, she\'s wearing a tight brown shirt and loose brown pants, along with a backpack slung across one shoulder.\n\n', false );
		MainView.outputText( 'The horse girl holds up her hands in a nonthreatening manner. "<i>Whoa there, pet. I just came by to join you for breakfast.</i>" You sigh and put your weapon away. "<i>Jumpy huh?</i>" she says, looking around your camp. "<i>You\'ve got a pretty good setup here. I don\'t know why you don\'t just move into Tel\'Adre. I\'ve got a nice little place we could share... But no matter.</i>"\n\n', false );
		MainView.outputText( 'You invite her over to your bedroll and sit down. She follows suit and removes her backpack, digging through it for a minute, retrieving a clay bowl and a large brown bag. She opens the bag and pours out a bowlful of grains and oats. "<i>Normally I just eat my breakfast raw... but today I want to try something a little different.</i>"', false );
		MainView.outputText( 'Cotton sets the bowl down in your lap and pulls down the front of your ' + CoC.player.armorName + ', exposing your ' + Descriptors.chestDesc() + ' to the cool morning breeze. A shiver runs down your spine and your nipples immediately begin to harden, as if knowing what this equine girl has in mind. Cotton leans down and wraps her lips around one ' + Descriptors.nippleDescript( 0 ) + ', sucking sensually and flicking the tip with her tongue.\n\n', false );
		MainView.outputText( 'You feel the familiar sensation of fluids being drawn from you, and soon Cotton is suckling from your milky tit. She relinquishes her hold on the teat, smacks her lips, and brings the bowl up to your breast. With the other hand, she grabs your ' + Descriptors.nippleDescript( 0 ) + ' and breast firmly, pulling and squeezing it. Delicious white milk begins squirting from the ducts of your breast, dribbling and spraying into the bowl of grains. You can\'t help but moan, squirming in your seat as your tit is ruthlessly milked.\n\n', false );
		MainView.outputText( 'Cotton gives the breast one last lick and moves on to the next. In no time, that one too is spraying a copious amount of milk into the bowl. The sensation is amazing, being milked by hand. So intimate and personal. Your body quakes and trembles as little earthquakes shake across your breasts and down your body. Your mouth forms an O and you let out little moans of pleasure as the boobgasm rolls throughout your body.\n\n', false );
		MainView.outputText( 'Just like that, Cotton\'s hand leaves your breast, though the hot and aroused feeling doesn\'t leave. You open your eyes and look at her. She gives you a smile and takes a bite of grains.\n\n', false );
		MainView.outputText( '"<i>Mmm, you are so tasty, pet,</i>" she trills, "<i>Here, try a bite.</i>" She lifts a spoon full of grain, oats and milk to your mouth. You hesitantly take a bite and can\'t help but smile. The sweet, smooth taste of your milk perfectly complements the grains. You munch on it and swallow, and have another bite.\n\n', false );
		MainView.outputText( 'You chat about inconsequential things, mostly the Tel\'Adre rumor mill, while you both eat from the bowl. When it\'s completely gone, Cotton licks out the bowl and packs everything up.\n\n', false );
		MainView.outputText( '"<i>Thanks, pet, that was delicious. I\'ll be sure to stop by another time so we can share a meal. Feel free to come by the gym too. We can get a little more... personal.</i>" She kisses you on the cheek, and then departs back towards Tel\'Adre.', false );
	...
	Cotton.prototype.cottonPregnantAlertII = function() {
		MainView.clearOutput();
		MainView.outputText( ImageManager.showImage( 'cotton-you-got-her-preggo' ) );
		MainView.outputText( '"<i>Hello, pet,</i>" she says, and you notice worry in her voice.  There are slight bags under her eyes, and her crimson ponytail is a little unkempt, with hair jutting out at odd angles.  Something clearly has her frazzled.' );
		...
		if( CoC.flags[ kFLAGS.EDRYN_NUMBER_OF_KIDS ] > 0 || (SceneLib.edryn.pregnancy.isPregnant && CoC.flags[ kFLAGS.EDRYN_PREGNANT_AND_NOT_TOLD_PC_YET ] !== 0) ) {
			MainView.outputText( '  Should have figured as much after you got Edryn pregnant.' );
		}
		MainView.outputText( '</i>"  She wrenches her hands nervously and looks you in the eye. The cool, confident yoga instructor has clearly melted away, revealing the real her.  "<i>I know we\'re not serious, but ' );
		if( CoC.flags[ kFLAGS.EDRYN_NEVER_SEE_AGAIN ] === 0 ) {
			MainView.outputText( 'you\'ll stick with me through this, right?' );
		} else {
			MainView.outputText( 'you won\'t abandon me like you did Edryn, will you?' );
		}
	...
	Cotton.prototype.beABadCottonDad = function() {
		MainView.clearOutput();
		MainView.outputText( 'You shake your head.  You certainly can\'t deal with a kid.  You tell her point blank that you want nothing to do with the child.  Tears well up in her eyes, and her mouth opens and closes several times, without a single sound coming out.' );
		...
		if( CoC.flags[ kFLAGS.EDRYN_NEVER_SEE_AGAIN ] === 0 ) {
			MainView.outputText( 'and slaps you as hard as she can' );
		} else {
			MainView.outputText( 'and punches you as hard as she can' );
		}
		MainView.outputText( '. "<i>Fuck you \'pet\'.' );
		if( CoC.flags[ kFLAGS.EDRYN_NEVER_SEE_AGAIN ] > 0 ) {
			MainView.outputText( '  That was for Edryn too.' );
		}
		MainView.outputText( '</i>"  With that, she turns on her hoof and leaves.' );
		//bold;
		MainView.outputText( '<b>(Cotton will no longer speak with you.)</b>' );
	...
	Cotton.prototype.beAGoodCottonDad = function() {
		MainView.clearOutput();
		MainView.outputText( 'You take her by the shoulders and nod.  You confirm that of course you\'ll be there for her, whatever she needs.' );
	...
	Cotton.prototype.cottonPopsOutAKid = function() {
		...
		if( CoC.flags[ kFLAGS.COTTON_KID_COUNT ] === 0 ) {
			...
			MainView.outputText( ' for the first time.  Though you don\'t want to let ' );
			if( kid !== 1 ) {
				MainView.outputText( 'her' );
			} else {
				MainView.outputText( 'him' );
			}
			MainView.outputText( ' go, you relinquish your grasp as Cotton\'s arms sweep ' );
			if( kid !== 1 ) {
				MainView.outputText( 'her' );
			} else {
				MainView.outputText( 'him' );
			}
			MainView.outputText( ' up.  With one hand she lifts up her top, exposing her large brown nipples, and pulls the child to one, where it latches on and suckles like a pro.  You just watch, ready to look away if Cotton asks, but she seems too preoccupied with nursing her new baby to pay much attention to you.' );
			...
			if( kid === 3 ) {
				MainView.outputText( '  I guess it runs in the family.' );
			}
			MainView.outputText( '</i>"' );
			...
			if( kid < 2 ) {
				MainView.outputText( 'guy' );
			} else {
				MainView.outputText( 'girl' );
			}
			MainView.outputText( ' situated.</i>"  She takes the bundle from you, retrieves a fresh clean blanket, and within seconds has ' );
			if( kid !== 1 ) {
				MainView.outputText( 'her' );
			} else {
				MainView.outputText( 'him' );
			}
			MainView.outputText( ' expertly swaddled.  Cotton sets the child in the crib and stands back to admire her handiwork. You step up beside her and wrap an appreciative arm around her waist, telling her that she\'s done a wonderful job.' );
		...
		//(Replaces the Approach scenes);
			...
			MainView.outputText( 'As you head towards Cotton, you realize something about her is different; the swollen bulge of her late pregnancy is gone!  You ask her if everything is all right.' );
			...
			MainView.outputText( '!</i>" she says happily, pulling you into a hug.  "<i>We could go see ' );
			if( kid !== 1 ) {
				MainView.outputText( 'her' );
			} else {
				MainView.outputText( 'him' );
			}
			MainView.outputText( ' if you like, ' );
			if( kid !== 1 ) {
				MainView.outputText( 'she' );
			} else {
				MainView.outputText( 'he' );
			}
			MainView.outputText( '\'s at the daycare here, I wanted to keep a close eye on ' );
			if( kid !== 1 ) {
				MainView.outputText( 'her' );
			} else {
				MainView.outputText( 'him' );
			}
			MainView.outputText( ' today.</i>"' );
			...
			MainView.outputText( ', if Cotton thinks that won\'t upset ' );
			if( kid !== 1 ) {
				MainView.outputText( 'her' );
			} else {
				MainView.outputText( 'him' );
			...
			MainView.outputText( ' good to see ' );
			if( kid !== 1 ) {
				MainView.outputText( 'her' );
			} else {
				MainView.outputText( 'him' );
			}
			MainView.outputText( ' daddy,</i>" she says, and leads you down a corridor and into a room marked "<i>DAYCARE</i>".  As soon as the door opens, you\'re assaulted by the laughs and giggles of kids playing.' );
			...
			MainView.outputText( '\'s just down for a nap right now.  Would you like to see ' );
			if( kid !== 1 ) {
				MainView.outputText( 'her' );
			} else {
				MainView.outputText( 'him' );
			}
			MainView.outputText( '?</i>"  You both nod your heads in the affirmative, and are taken to an off-room.  It\'s dark, but there\'s a crib just by the door, and you can make out a slumbering baby with its mother\'s horse ears twitching as it dreams whatever babies dream.' );
			MainView.outputText( '\n\nNot wanting to wake ' );
			if( kid !== 1 ) {
				MainView.outputText( 'her' );
			} else {
			...
			MainView.outputText( ', you look ' );
			if( kid !== 1 ) {
				MainView.outputText( 'her' );
			} else {
				MainView.outputText( 'him' );
			}
			MainView.outputText( ' over and then back out of the room, content with the knowledge that you\'ll get to spend more time with ' );
			if( kid !== 1 ) {
				MainView.outputText( 'her' );
			} else {
				MainView.outputText( 'him' );
			}
			MainView.outputText( ' later.  The two of you head back out to the main section of the gym, while Cotton slaps you on the back, "<i>We sure do make \'em good, don\'t we?  Anyway, if you want to work out, just get changed and come on back.</i>"' );
			this.pregnancy.knockUpForce(); //Clear Pregnancy
			CoC.flags[ kFLAGS.COTTON_KID_COUNT ]++;
			if( CoC.flags[ kFLAGS.COTTON_KID_COUNT ] === 1 ) {
				CoC.flags[ kFLAGS.COTTON_OLDEST_KID_AGE ] = 1;
	...
	Cotton.prototype.cottonContraceptionToggle = function() {
		...
		MainView.outputText( 'You ask the ' );
		if( this.pregnancy.isPregnant ) {
			MainView.outputText( 'pregnant ' );
		}
		MainView.outputText( 'she-stallion if she\'s still taking her contraceptive herbs.' );
		...
		if( CoC.player.gender !== 3 || this.pregnancy.isPregnant ) {
			MainView.outputText( 'virile' );
		} else {
			MainView.outputText( 'fertile' );
		}
		MainView.outputText( '; you actually broke right through the protection they were supposed to give me from ending up a mommy or a daddy.  I may as well not be taking them at all when I\'m with you, don\'t you think?</i>" she concludes.' );
		...
		if( CoC.flags[ kFLAGS.COTTON_KID_COUNT ] < 3 ) {
			MainView.outputText( 'Well that\'s true,</i>" she admits.  "<i>But I leave it up to you, pet.' );
		} else {
			MainView.outputText( 'Well, considering the brood we have, they\'ve already proven pretty useless, haven\'t they?   So, while I\'ll keep taking them if you want me to, I don\'t see much point in it.' );
		}
		MainView.outputText( '</i>"' );
	...
	Cotton.prototype.tellCottonStopEatingHorsePills = function() {
		MainView.clearOutput();
		MainView.outputText( 'You tell Cotton that, if they\'re going to be that useless to her, and she doesn\'t mind the idea of potentially having more kids, she has your permission to stop taking them.' );
	...
	Cotton.prototype.tellCottonToKeepFiringBlanksAsshole = function() {
		MainView.clearOutput();
		MainView.outputText( 'You tell Cotton you\'d rather not force her to have any more kids, so she should keep taking the herbs; in your opinion, imperfect protection is better than no protection.' );
		MainView.outputText( '\n\nCotton nods, "<i>All right then.  As they say, better safe than ' );
		if( !EngineCore.silly() ) {
			MainView.outputText( 'sorry' );
		} else {
			MainView.outputText( 'overrun by a herd of little horsefolk' );
		}
	...
	Cotton.prototype.repeatContraceptionToggleCotton = function() {
		...
		if( CoC.flags[ kFLAGS.COTTON_HERBS_OFF ] === 1 ) {
			MainView.outputText( 'You tell Cotton that, while you know it\'s not a perfect solution, you want her to start taking her herbs again.' );
			CoC.flags[ kFLAGS.COTTON_HERBS_OFF ] = 0;
			MainView.outputText( '\n\nCotton nods, "<i>All right then.  As they say, better safe than ' );
			if( !EngineCore.silly() ) {
				MainView.outputText( 'sorry' );
		...
		//Play this scene on subsequent deactivations of Cotton Contraception;
			...
			MainView.outputText( 'You tell Cotton that you\'re ready for her to go off her herbs again if she wants to.' );
			MainView.outputText( '\n\nShe nods and says, "<i>Starting tomorrow I\'ll go ahead and stop, then.</i>"' );
			MainView.outputText( '\n\nYou thank her for understanding you and change the subject.' );
	...
	Cotton.prototype.goTellCottonShesAMomDad = function() {
		MainView.clearOutput();
		MainView.outputText( 'Convinced that Cotton is the father of the child in your womb, at daybreak, you pack your usual things and head off to Tel\'Adre.  Stopping in at the gym, you ask for directions to Cotton\'s place, and then make your way there.' );
	...
	Cotton.prototype.birthingCottonsKids = function() {
		...
		MainView.outputText( 'You can feel movement underneath the skin, and watch as it bulges and shifts as another living being moves independently inside you.' );
		...
		MainView.outputText( ' \'father\', ' );
		if( kid === 1 ) {
			MainView.outputText( 'he' );
		} else {
			MainView.outputText( 'she' );
		}
		MainView.outputText( '\'s hairless except on ' );
		if( kid === 1 ) {
			MainView.outputText( 'his' );
		} else {
			MainView.outputText( 'her' );
		}
		MainView.outputText( ' head, where there\'s a light sprinkling of dark hair.  ' );
		if( kid === 1 ) {
			MainView.outputText( 'His' );
		} else {
			MainView.outputText( 'Her' );
		}
		MainView.outputText( ' feet end in tiny pristine hooves, and a little brush of a tail extends from ' );
		if( kid === 1 ) {
			MainView.outputText( 'his' );
		} else {
			MainView.outputText( 'her' );
		...
		MainView.outputText( ', bundle ' );
		if( kid === 1 ) {
			MainView.outputText( 'him' );
		} else {
			MainView.outputText( 'her' );
		}
		MainView.outputText( ' up good and tight, get dressed, and head off towards Tel\'Adre.' );
		...
		if( CoC.flags[ kFLAGS.PC_IS_A_DEADBEAT_COTTON_DAD ] > 0 ) {
			...
			MainView.outputText( '.  She takes the child from you, holding ' );
			if( kid === 1 ) {
				MainView.outputText( 'him' );
			} else {
				MainView.outputText( 'her' );
			}
			MainView.outputText( ' close with practiced grace.' );
		...
		//[First Child with Cotton?];
			...
			if( kid === 1 ) {
				MainView.outputText( 'He\'ll' );
			} else {
				MainView.outputText( 'She\'ll' );
			}
			MainView.outputText( ' be so happy here, pet, I promise you.  I\'m an excellent momma, or daddy, as the case may be.  I don\'t know if I\'ve ever told you, but I had a couple of kids a few years back.  Twins.  Boy they were rambunctious little devils.  Couldn\'t sit still,</i>" she smiles wistfully.' );
			MainView.outputText( '\n\nSo, Cotton\'s been a parent before? You ask if she was the mother or the father.' );
			MainView.outputText( '\n\n"<i>Oh, I was the \'daddy\', I guess,</i>" she laughs a little at the word.  "<i>Their mother was from my home village.  We moved into Tel\'Adre together after the demons came.  One thing led to another, and we had two bundles of joy not long after.</i>"' );
		...
		//[Additional Kids];
			...
			MainView.outputText( '\'s so cute, pet.  We sure do make \'em good.  ' );
			if( kid === 1 ) {
				MainView.outputText( 'He' );
			} else {
				MainView.outputText( 'She' );
			}
			MainView.outputText( '\'ll just love it here with ' );
			if( kid === 1 ) {
				MainView.outputText( 'his' );
			} else {
				MainView.outputText( 'her' );
			}
			MainView.outputText( ' family,</i>" Cotton declares.  You nod your head, then ask if Cotton will be okay looking after one more child?' );
	...
	Cotton.prototype.visitCottonKids = function() {
		MainView.clearOutput();
		MainView.outputText( 'You tell Cotton that, if it\'s okay with her, you\'d like to skip exercising today; you were hoping that you could visit your ' );
		if( CoC.flags[ kFLAGS.COTTON_KID_COUNT ] === 1 ) {
			MainView.outputText( 'kid' );
		} else {
			MainView.outputText( 'kids' );
		}
		MainView.outputText( ', instead? Cotton\'s eyes widen in surprise before a happy grin splits her face.  "<i>Sure, pet, all you had to do was ask!  Give me a minute to get my stuff, and then we\'ll go and see ' );
		if( CoC.flags[ kFLAGS.COTTON_KID_COUNT ] === 1 ) {
		...
		MainView.outputText( ', all right?</i>"  You nod your head and watch as the equine yoga instructor busies herself gathering her few daily belongings, then follow her as she heads back home, eager to see how your ' );
		if( CoC.flags[ kFLAGS.COTTON_KID_COUNT ] === 1 ) {
			MainView.outputText( 'child is' );
		} else {
			MainView.outputText( 'children are' );
		}
		...
		if( scene === 0 ) {
			...
			if( CoC.flags[ kFLAGS.COTTON_OLDEST_KID_GENDER ] >= 2 ) {
				MainView.outputText( 'baby girl is' );
			} else {
				MainView.outputText( 'baby boy is' );
			}
			MainView.outputText( ' sitting up in ' );
			if( CoC.flags[ kFLAGS.COTTON_OLDEST_KID_GENDER ] >= 2 ) {
				MainView.outputText( 'her' );
			} else {
				MainView.outputText( 'his' );
			}
			MainView.outputText( ' crib when you both enter, cooing happily and babbling like babies do, clearly happy in ' );
			if( CoC.flags[ kFLAGS.COTTON_OLDEST_KID_GENDER ] >= 2 ) {
				MainView.outputText( 'her' );
			} else {
				MainView.outputText( 'his' );
			...
			MainView.outputText( ' own little way to see you.  Cotton smiles at you, "<i>Watch this; ' );
			if( CoC.flags[ kFLAGS.COTTON_OLDEST_KID_GENDER ] >= 2 ) {
				MainView.outputText( 'she' );
			} else {
				MainView.outputText( 'he' );
			}
			MainView.outputText( ' loves this game,</i>" she tells you, and then slowly kneels before the crib.  "<i>Hello, my little sweet, hellooo.</i>"  Cotton coos, patting the ' );
			if( CoC.flags[ kFLAGS.COTTON_KID_COUNT ] === 1 ) {
				MainView.outputText( 'baby' );
			} else {
				MainView.outputText( 'babies' );
			}
			MainView.outputText( ' on the nose and babbling to ' );
			if( CoC.flags[ kFLAGS.COTTON_OLDEST_KID_GENDER ] === 1 ) {
				MainView.outputText( 'him' );
			} else {
				MainView.outputText( 'her' );
			...
			MainView.outputText( ' in that way that parents of small children tend to do.  When the little ' );
			if( CoC.flags[ kFLAGS.COTTON_OLDEST_KID_GENDER ] === 1 ) {
				MainView.outputText( 'colt' );
			} else {
				MainView.outputText( 'filly' );
			}
			MainView.outputText( ' foal is giggling merrily, Cotton slowly cover her eyes with her hands.  "<i>Where\'s the baby? Where\'s the little baby?</i>" she asks, then quickly uncovers her eyes, adopting an exaggerated expression of surprise.  "<i>Why, there ' );
			if( CoC.flags[ kFLAGS.COTTON_OLDEST_KID_GENDER ] === 1 ) {
				MainView.outputText( 'he' );
			} else {
				MainView.outputText( 'she' );
			}
			MainView.outputText( ' is!</i>"  She gasps, making the baby laugh in delight.  She repeats this several times, the baby enjoying it thoroughly.  You ask Cotton if you can try, and she agrees; ' );
			MainView.outputText( 'your baby seems' );
			MainView.outputText( ' a little surprised to see you, but that doesn\'t stop ' );
			if( CoC.flags[ kFLAGS.COTTON_OLDEST_KID_GENDER ] >= 2 ) {
				MainView.outputText( 'her' );
			} else {
				MainView.outputText( 'him' );
			}
			MainView.outputText( ' from laughing just as hard at you as ' );
			if( CoC.flags[ kFLAGS.COTTON_OLDEST_KID_GENDER ] === 1 ) {
				MainView.outputText( 'he' );
			} else {
				MainView.outputText( 'she' );
			}
			MainView.outputText( ' did at Cotton.  Feeling a little left out, Cotton rejoins you and you start playing peek-a-boo together, much to the baby\'s delight.' );
			//[Additional Kids:;
			if( CoC.flags[ kFLAGS.COTTON_KID_COUNT ] > 1 ) {
				MainView.outputText( '\n\nYour other ' );
				if( CoC.flags[ kFLAGS.COTTON_KID_COUNT ] === 2 ) {
					MainView.outputText( 'baby is' );
				} else {
					MainView.outputText( 'babies are' );
				}
				MainView.outputText( ' clearly enjoying it just as much as your first-born, giggling and clapping at yours and Cotton\'s antics, even clumsily trying to mimic the game themselves.' );
			}
			MainView.outputText( '\n\nEventually, though, ' );
			if( CoC.flags[ kFLAGS.COTTON_KID_COUNT ] === 1 ) {
				if( CoC.flags[ kFLAGS.COTTON_OLDEST_KID_GENDER ] >= 2 ) {
					MainView.outputText( 'she grows tired' );
				} else {
					MainView.outputText( 'he grows tired' );
				}
			} else {
				MainView.outputText( 'they grow tired' );
			}
			MainView.outputText( ' and, still giggling, settle' );
			if( CoC.flags[ kFLAGS.COTTON_KID_COUNT ] === 1 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ' down to sleep.  You and Cotton fuss over ' );
			if( CoC.flags[ kFLAGS.COTTON_KID_COUNT ] === 1 ) {
				if( CoC.flags[ kFLAGS.COTTON_OLDEST_KID_GENDER ] >= 2 ) {
					MainView.outputText( 'her' );
				} else {
			...
			MainView.outputText( ' in and mussing ' );
			if( CoC.flags[ kFLAGS.COTTON_KID_COUNT ] === 1 ) {
				if( CoC.flags[ kFLAGS.COTTON_OLDEST_KID_GENDER ] >= 2 ) {
					MainView.outputText( 'her' );
				} else {
			...
			MainView.outputText( ' hair gently, but watch until ' );
			if( CoC.flags[ kFLAGS.COTTON_KID_COUNT ] === 1 ) {
				if( CoC.flags[ kFLAGS.COTTON_OLDEST_KID_GENDER ] >= 2 ) {
					MainView.outputText( 'she goes' );
				} else {
					MainView.outputText( 'he goes' );
				}
			} else {
				MainView.outputText( 'they go' );
			}
			MainView.outputText( ' to sleep.  You share a smile with your equine lover, kiss your ' );
			if( CoC.flags[ kFLAGS.COTTON_KID_COUNT ] === 1 ) {
				MainView.outputText( 'kid on the cheek' );
			} else {
				MainView.outputText( 'kids on the cheeks' );
			}
			MainView.outputText( ', kiss her on the lips, and then quietly leave Cotton\'s house to head back to camp.' );
		}
		//Little Angel;
		else if( scene === 1 ) {
			MainView.outputText( '\n\nHowever, it turns out that ' );
			if( CoC.flags[ kFLAGS.COTTON_KID_COUNT ] === 1 ) {
				MainView.outputText( 'your baby is' );
			} else {
				MainView.outputText( 'all your children are' );
			}
			MainView.outputText( ' fast asleep.  Rather than disturb ' );
			if( CoC.flags[ kFLAGS.COTTON_OLDEST_KID_GENDER ] >= 2 ) {
				MainView.outputText( 'her' );
			} else {
				MainView.outputText( 'him' );
			}
			MainView.outputText( ', Cotton simply leans over the crib, gently stroking a cheek with a soft smile on her lips.  "<i>I\'d forgotten what it was like to be a parent... it\'s hard work, but it\'s definitely rewarding.</i>"  She gives you a sidelong look.  "<i>I can\'t say that this is exactly what I expected of our little relationship... but I\'m not complaining.</i>"' );
			...
			MainView.outputText( ' to get some rest together.' );
		}
		//Breastfeeding;
		else if( scene === 2 ) {
			...
			MainView.outputText( ' starts to wail and cry, the mare looking tired.  "<i>Oh, you\'re hungry again my little one?  All right, all right, mommy will get your milk, just give me a moment...</i>"' );
			if( CoC.flags[ kFLAGS.COTTON_KID_COUNT ] > 1 ) {
				MainView.outputText( '  Fortunately, your other kids don\'t seem to be hungry, so it\'s just the one who needs feeding.' );
			}

			//(If PC lactates:;
			if( CoC.player.lactationQ() >= 50 ) {
				MainView.outputText( '  You could probably offer to nurse your ' );
				if( CoC.flags[ kFLAGS.COTTON_OLDEST_KID_GENDER ] !== 1 ) {
					MainView.outputText( 'daughter' );
				} else {
					MainView.outputText( 'son' );
		...
		//Cuddlebug;
			...
			MainView.outputText( ' stares up at you with eyes wide and wet, sniffling audibly as ' );
			if( CoC.flags[ kFLAGS.COTTON_OLDEST_KID_GENDER ] >= 2 ) {
				MainView.outputText( 'she' );
			} else {
				MainView.outputText( 'he' );
			}
			MainView.outputText( ' makes tiny little sobs.  "<i>Aw, looks like somebody was lonely,</i>" Cotton coos, then gives you a meaningful nudge.  Realising she wants you to hold your child, you step forward and gently lift ' );
			if( CoC.flags[ kFLAGS.COTTON_OLDEST_KID_GENDER ] >= 2 ) {
				MainView.outputText( 'her' );
			} else {
				MainView.outputText( 'he' );
			}
			MainView.outputText( ' up into your arms, commenting on what a big, heavy ' );
			if( CoC.flags[ kFLAGS.COTTON_OLDEST_KID_GENDER ] >= 2 ) {
				MainView.outputText( 'girl' );
			} else {
				MainView.outputText( 'he' );
			}
			MainView.outputText( '\'s getting to be.  The baby is scared and hesitant, and bursts into tears at first, but you croon and shush ' );
			if( CoC.flags[ kFLAGS.COTTON_OLDEST_KID_GENDER ] >= 2 ) {
				MainView.outputText( 'her' );
			} else {
				MainView.outputText( 'he' );
			}
			MainView.outputText( ' gently, holding the foal close to your ' );
			MainView.outputText( '[chest] and rocking ' );
			if( CoC.flags[ kFLAGS.COTTON_OLDEST_KID_GENDER ] >= 2 ) {
				MainView.outputText( 'her' );
			} else {
				MainView.outputText( 'him' );
			}
			MainView.outputText( ' back and forwards; soon enough, the foal\'s cooing and snuggling eagerly against you as you cuddle ' );
			if( CoC.flags[ kFLAGS.COTTON_OLDEST_KID_GENDER ] >= 2 ) {
				MainView.outputText( 'her' );
			} else {
				MainView.outputText( 'him' );
			...
			MainView.outputText( ' little ears flick in the most adorable manner.  So caught up are you in cuddling your child that you don\'t notice Cotton until she has her arms wrapped around you, leaning her head against you as she embraces the pair of you.' );
	...
	Cotton.prototype.feedYourCottonKids = function() {
		MainView.clearOutput();
		MainView.outputText( 'You interrupt Cotton by pointing out that you have breasts full of milk; you\'d be happy to nurse the little ' );
		if( CoC.flags[ kFLAGS.COTTON_OLDEST_KID_GENDER ] >= 2 ) {
			MainView.outputText( 'filly ' );
		} else {
			MainView.outputText( 'cotton ' );
		}
		MainView.outputText( ' for her, if she wants.' );
		...
		MainView.outputText( ' to you, by which time you have already exposed your [chest].' );
		...
		MainView.outputText( ' weight in your arms, before bringing ' );
		if( CoC.flags[ kFLAGS.COTTON_OLDEST_KID_GENDER ] >= 2 ) {
			MainView.outputText( 'her' );
		} else {
			MainView.outputText( 'him' );
		...
		MainView.outputText( ' in close' );
		MainView.outputText( '.  At first, the baby is confused and whimpers quietly in fear, but ' );
		if( CoC.flags[ kFLAGS.COTTON_OLDEST_KID_GENDER ] >= 2 ) {
			MainView.outputText( 'her' );
		} else {
			MainView.outputText( 'his' );
		...
		MainView.outputText( ' presence and weight make your boobs start releasing their sweet cargo.  As the scent of milk fills the air, the baby\'s instincts take over and it brings its little face in to first nuzzle your [nipple], and then finally close ' );
		if( CoC.flags[ kFLAGS.COTTON_OLDEST_KID_GENDER ] >= 2 ) {
			MainView.outputText( 'her' );
		} else {
			MainView.outputText( 'his' );
		}
		MainView.outputText( ' lips around it.' );
		...
		MainView.outputText( ' back and forth, quietly starting to sing whatever dredges of lullabies you can remember overhearing from the nursing mothers back in your own village in Ingnam.  You can feel ' );
		if( CoC.flags[ kFLAGS.COTTON_OLDEST_KID_GENDER ] >= 2 ) {
			MainView.outputText( 'her' );
		} else {
			MainView.outputText( 'his' );
		}
		MainView.outputText( ' little heart beating inside ' );
		if( CoC.flags[ kFLAGS.COTTON_OLDEST_KID_GENDER ] >= 2 ) {
			MainView.outputText( 'her' );
		} else {
			MainView.outputText( 'his' );
		}
		MainView.outputText( ' breast, and you wonder if ' );
		if( CoC.flags[ kFLAGS.COTTON_OLDEST_KID_GENDER ] >= 2 ) {
			MainView.outputText( 'she' );
		} else {
			MainView.outputText( 'he' );
		}
		MainView.outputText( ' can hear your heart inside of yours.' );
		...
		if( CoC.flags[ kFLAGS.COTTON_KID_COUNT ] === 1 ) {
			MainView.outputText( ' needs' );
		} else {
			MainView.outputText( 'ren need' );
		}
		MainView.outputText( ' all the energy they can get.</i>" Her hands begin to massage your shoulders and back, kneading the flesh, helping you to relax.' );
		...
		MainView.outputText( ' is just a sleepy little thing who needs to go to bed, you tell her, and you gently put the baby back in ' );
		if( CoC.flags[ kFLAGS.COTTON_OLDEST_KID_GENDER ] >= 2 ) {
			MainView.outputText( 'her' );
		} else {
			MainView.outputText( 'his' );
		}
		MainView.outputText( ' crib to get some sleep.  You recover your breasts and ask if Cotton will be okay looking after them now, or if you should stick around and help out a little longer?' );
	...
	Cotton.prototype.letCottonFeedKids = function() {
		...
		if( CoC.flags[ kFLAGS.COTTON_OLDEST_KID_GENDER ] >= 2 ) {
			MainView.outputText( 'her down for her nap, hm pet?</i>"' );
		} else {
			MainView.outputText( 'him down for his nap, hm pet?</i>"' );
		}
		...
		MainView.outputText( ' crib.  You ruffle ' );
		if( CoC.flags[ kFLAGS.COTTON_OLDEST_KID_GENDER ] >= 2 ) {
			MainView.outputText( 'her' );
		} else {
			MainView.outputText( 'his' );
		}
		MainView.outputText( ' hair and tell Cotton that she makes some gorgeous little foals.  She just nods and gives your shoulder a little squeeze, content to watch in silence for a moment.' );
