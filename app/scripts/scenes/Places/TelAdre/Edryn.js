...
angular.module( 'cocjs' ).run( function( SceneLib, $rootScope, $log, CockTypesEnum, ConsumableLib, AppearanceDefs, Appearance, EventParser, PregnancyStore, Utils, ImageManager, StatusAffects, kFLAGS, Descriptors, CoC, EngineCore ) {
	...
	Edryn.prototype.edrynBarTalk = function() {
		...
		if( CoC.flags[ kFLAGS.EDRYN_NEEDS_TO_TALK_ABOUT_KID ] === 1 ) {
			...
			if( CoC.flags[ kFLAGS.EDRYN_NUMBER_OF_KIDS ] === 1 ) {
				...
				EngineCore.outputText( 'I can\'t believe we\'ve had ' + Utils.num2Text( CoC.flags[ kFLAGS.EDRYN_NUMBER_OF_KIDS ] ) + ' kids together!  You better come see me later on - ', false );
		...
		if( CoC.player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_CENTAUR && CoC.player.totalCocks() > 0 ) {
			...
			if( CoC.player.horseCocks() > 0 ) {
				...
				if( CoC.player.cocks[ 0 ].cockLength > 16 ) {
					...
					EngineCore.outputText( 'You squirm uncomfortably, feeling constrained by your ' + CoC.player.armorName + ' as you surge to erectness.\n\n', false );
				}
			...
			//Non horsedick;
				...
				if( CoC.player.cor > 50 || CoC.player.statusAffectv1( StatusAffects.Edryn ) > 0 || CoC.flags[ kFLAGS.LOW_STANDARDS_FOR_ALL ] ) {
					EngineCore.outputText( '  Unbidden, your mind wonders what her juicy horse-snatch would feel like, and your ' + Descriptors.cockDescript( 0 ) + ' responds immediately, thickening with readiness.  You squirm uncomfortably from how constricting your ' + CoC.player.armorName + ' feels.\n\n', false );
		...
		if( CoC.player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_CENTAUR && CoC.player.totalCocks() > 0 ) {
			...
			//Normal Wingdangdoodle;
				...
				if( CoC.player.cockArea( x ) >= 24 ) {
					...
					EngineCore.outputText( 'gets you the ride of a lifetime.  So what do you say ' + CoC.player.short + '?  Do you want to be my sex-toy?</i>"\n\n', false );
	...
	Edryn.prototype.fuckEdrynTaur = function() {
		...
		EngineCore.outputText( ' while she caresses the side of your ' + CoC.player.face() + ' with the other.  Your body pumps away while your tongues entwine, tracing saliva over each other\'s lips.  She pulls back and moans out again, trailing off into a whinny as she orgasms underneath you.\n\n', false );
		EngineCore.outputText( 'Her cunt locks tightly around you once again, clamping down in a series of rings from the base all the way to your ' + CoC.player.cockHead( x ) + '.  At once, they start sliding from the base towards the tip, new ones forming along the bottom of the shaft as her pussy milks your ' + Descriptors.cockDescript( x ) + ' hard.  Trapped inside her, all you can do is groan and cum in response to her climax.  You throw back your head as you feel the warmth of your release building in your ', false );
		if( CoC.player.balls > 0 ) {
	...
	Edryn.prototype.fuckEdrynNonTaur = function() {
		...
		EngineCore.outputText( ', pressing your ' + CoC.player.face() + ' into her slick folds and tasting her flavor.  Your nose easily slips into her gash as your tongue explores it, testing and tasting thick, liquid centaur lust.  It\'s tangy and sweet, surprisingly unlike the scent it gives off.  You easily find her large clit and suckle on it, listening to the wet squelches and whinnies of pleasure.\n\n', false );
		...
		if( CoC.player.statusAffectv1( StatusAffects.Edryn ) <= 1 ) {
			...
			EngineCore.outputText( 'She sighs happily and looks down at you, "<i>Ooh ' + CoC.player.short + ', you\'re the best.  We should do this again soon.  Oh goddess, I can feel my juices starting just thinking about it.</i>"\n\n', false );
		} else if( CoC.player.statusAffectv1( StatusAffects.Edryn ) === 4 ) {
	...
	Edryn.prototype.helEdrynThreeSomeStartYerEngines = function() {
		...
		if( CoC.flags[ kFLAGS.EDRYN_TIMES_HEL_THREESOMED ] === 0 ) {
			...
			EngineCore.outputText( '"<i>Hey, ' + CoC.player.short + ',</i>" Edryn says with a half-smile. "<i>Who\'s your friend?</i>"\n\n', false );
	...
	Edryn.prototype.threesomeEdrynAndHel = function() {
		...
		EngineCore.outputText( '"<i>Oh yeah. We... We gotta go again sometime,</i>" Hel agrees, nearly falling off Edryn\'s back.  You give each of the girls a long kiss before collecting your ' + CoC.player.armorName + ' and walking off back to camp.\n\n', false );
		CoC.player.orgasm();
	...
	Edryn.prototype.pregdrynOffer = function( cs ) {
		...
		if( this.pregnancy.incubation < 250 ) {
			...
			if( CoC.player.cor + CoC.player.lib < 100 || CoC.player.cockTotal() < 1 ) {
				EngineCore.outputText( 'you struggle not to reach into your ' + CoC.player.armorName + ' to touch yourself.', false );
			} else {
	...
	Edryn.prototype.fuckPregEdryn = function() {
		...
		if( CoC.player.lowerBody !== AppearanceDefs.LOWER_BODY_TYPE_CENTAUR ) {
			...
			EngineCore.outputText( 'You tear off your ' + CoC.player.armorName + ' in a flash, fully exposing your ', false );
			if( CoC.player.cockTotal() > 1 ) {
		...
		//TAURZILLA;
			...
			EngineCore.outputText( 'The centauress pulls back and moans, "<i>Oh gods yes, it feels so much better pregnant!  My pu-pu-ahhhh-ussy is so much WETTER.  It\'s like I\'ve got a faucet back there!  And I\'m soo-ohhhh sensitive!  Ung fuck ' + CoC.player.mf( 'stud', 'dear' ) + ' don\'t stop.  Please don\'t stop!</i>"\n\n', false );
	...
	Edryn.prototype.edrynFucktroduction = function() {
		...
		if( !CoC.player.isTaur() ) {
			if( cost > 0 ) {
				EngineCore.outputText( 'Coins exchange hands and her arm finds its way back inside your ' + CoC.player.armorName + ', pulling you towards a back room.  A round of chuckles chases you through the bar room until they\'re silenced by the heavy thud of a door closing behind you.  You\'re pushed against the wall hard enough to make your teeth click.  The stars gradually clear from your view, revealing a massive gash, drooling with clear feminine slime.  The scent assails you, ', false );
			} else if( CoC.flags[ kFLAGS.EDRYN_NUMBER_OF_KIDS ] === 0 ) {
				EngineCore.outputText( 'Her arm finds its way back inside your ' + CoC.player.armorName + ', pulling you towards a back room.  A round of chuckles chases you through the bar room until they\'re silenced by the heavy thud of a door closing behind you.  You\'re pushed against the wall hard enough to make your teeth click.  The stars gradually clear from your view, revealing a massive gash, drooling with clear feminine slime.  The scent assails you, ', false );
			} else {
