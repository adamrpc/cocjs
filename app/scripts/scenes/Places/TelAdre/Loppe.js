...
angular.module( 'cocjs' ).run( function( SceneLib, MainView, ImageManager, Descriptors, AppearanceDefs, Utils, kFLAGS, CoC, EngineCore ) {
	...
	Loppe.prototype.appearanceOfLoppe = function() {
		...
		if( CoC.flags[ kFLAGS.LOPPE_FURRY ] === 0 ) {
			MainView.outputText( 'Loppe is a 6\'2" bunny-girl with deep brown eyes set in her pretty face.  Shoulder-length black hair and a pair of snow-white rabbit ears adorn the top of her head, tilted and lying backward to conceal the insides.  Curiously, her face is that of a normal human woman.' );
	...
	Loppe.prototype.loppeFirstMeeting = function() {
		MainView.clearOutput();
		MainView.outputText( 'Wandering towards the bar, you notice an unusual commotion; there are a lot more people hanging around here than is usual for the time of day.  Feeling curious, you go over to investigate, and ask a bystander what all of this is about.' );
	...
	Loppe.prototype.yesToLoppeMeetingTwo = function() {
		MainView.clearOutput();
		MainView.outputText( ImageManager.showImage( 'loppe-show-at-bar' ) );
		MainView.outputText( 'Suddenly the bar goes silent and the lights dim; a figure clad in a white mantle steps onto the stage; discarding the covering with a swift movement, it reveals the comely visage of a bunny-girl - a literal bunny-girl, in that apart from the obvious bunny ears and the hints of rabbit-like legs, she actually looks otherwise human despite the prevalence of anthropomorphic animals in this city.' );
		...
		if( CoC.player.cor < 40 ) {
			MainView.outputText( ' as politely as possible. It felt nice to see a display like that here; makes it feel a little less like a wasteland of sex-mad monsters and a little more like home' );
		}
		MainView.outputText( '.' );
	...
	Loppe.prototype.sureBakeryWithLoppe = function() {
		MainView.clearOutput();
		MainView.outputText( 'You consider the time of day, and the girl offering, and decide that it can\'t hurt.  Loppe smiles and takes your hand, leading you towards a nearby bakery.' );
		...
		if( CoC.player.gems >= 30 ) {
			...
			//[(corr > 40);
			if( CoC.player.cor > 40 ) {
				MainView.outputText( '  A small voice inside wonders if she\'d eat so extravagantly were she footing her own bill.' );
			}
			//remove 30 gems;
			CoC.player.gems -= 30;
	...
	Loppe.prototype.NoWayLoppe = function() {
		MainView.clearOutput();
		MainView.outputText( 'Loppe lets out a breath she had obviously been holding in a bitter sigh, the ghost of the words, "<i>I knew it</i>" echoing faintly as she does so.  "<i>Well, I understand.  I just had to give it a try all the same.  It was nice talking to you, [name].  Might see you again, someday.  Sorry for wasting your time...</i>"' );
	...
	Loppe.prototype.noLoppesHouse = function() {
		MainView.clearOutput();
		MainView.outputText( 'Loppe gives a disappointed sigh, but smiles at you all the same.  "<i>I understand... if you ever feel like talking some more, you can find me in the gym.  I need to keep myself in tip-top shape for my dancing shows, after all.</i>"' );
	...
	Loppe.prototype.yesLoppesHouse = function() {
		MainView.clearOutput();
		//Loppe can now be found in the Gym;
		MainView.outputText( 'The dancer smiles mischievously at you.  "<i>Wonderful.  I\'m going to show you just how great my body looks without this dress.  You wouldn\'t believe how much time I spend in the gym, working out.</i>"  Loppe grabs her last cookie in one hand and your arm in the other, leading you away - presumably to her house.  Enroute, she giggles constantly, provoking a question from you.' );
	...
	Loppe.prototype.loppeGenericMeetings = function() {
		MainView.clearOutput();
		MainView.outputText( 'You decide to approach the bunny-girl.  Loppe smiles and wipes the sweat off her brow with the towel.  "<i>Hey there, [name], nice seeing you around here.  So... do you want to do something?  Talk, maybe?  Or go to my place for a \'workout\'?</i>" she asks with a smirk.' );
		//Appearance;
		//Talk;
		//Sex;
	...
	Loppe.prototype.talkWithLoppeAboutLoppe = function() {
		MainView.clearOutput();
		MainView.outputText( 'You tell the laquine that you\'re curious about her, that you\'d like to know more about her: who she is, where she comes from, et cetera.' );
	...
	Loppe.prototype.askLoppeAboutChildren = function() {
		MainView.clearOutput();
		MainView.outputText( 'You decide to ask Loppe for her thoughts on children.  Has she considered having any?' );
	...
	Loppe.prototype.noOpinionOnLoppeArt = function() {
		MainView.clearOutput();
		MainView.outputText( 'You decline to answer, waving the question off.  Loppe raises an eyebrow at that.  "<i>Haven\'t thought about it either, huh?</i>"' );
	...
	Loppe.prototype.loppeIDontReallyWantKidsYouStupidTwat = function() {
		MainView.clearOutput();
		MainView.outputText( 'You confess you aren\'t really a \'kids\' person; you don\'t think you could ever envision yourself as a parent, especially in this crazy world.' );
	...
	Loppe.prototype.loppeIWantKidsSoonOkayCanWeFuck = function() {
		MainView.clearOutput();
		MainView.outputText( 'With a faint smile, you declare that having children is definitely in your future.  You want to be a parent; all you need is to find somebody who feels similar to you.' );
	...
	Loppe.prototype.gossipWithLoppeAboutUrta = function() {
		...
		if( CoC.flags[ kFLAGS.TIMES_FUCKED_URTA ] <= 0 || CoC.flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] === -1 ) {
			MainView.outputText( '"<i>Ah, so you\'ve met Watch Captain Urta?  She\'s famous around here, you know; people say she\'s a legendary bare-knuckle brawler and one of the toughest guards on the force.  The only problem is that she\'s not really that easy to approach... I guess she prefers to keep to herself.  Although she acts very friendly with that pretty centauress, Edryn.</i>"' );
	...
	Loppe.prototype.noLoppeWhosFuckingUrta = function() {
		MainView.clearOutput();
		MainView.outputText( 'You shake your head, claiming that you have no idea what she\'s talking about.  Loppe looks at you, then shrugs, clearly not caring if you don\'t have any gossip on the subject to share.' );
		//end scene;
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	//[=It's Me=];
	...
	Loppe.prototype.playAlongWivLoppesesUrtaGossip = function() {
		MainView.clearOutput();
		MainView.outputText( 'And... what would she do if, hypothetically, you <i>did</i> have a fetish for dickgirls ready to give a stallion an inferiority complex?' );
	...
	Loppe.prototype.admitToLoppeThatYouLoveZeHorsecock = function() {
		MainView.clearOutput();
		MainView.outputText( 'You look her in the eyes and tell her she\'s hit the nail on the head; you do have a fetish for chicks with dicks, and having a horsecock makes a dickgirl even sexier to you.' );
	...
	Loppe.prototype.dontLoppesHouse4Fucks = function() {
		MainView.clearOutput();
		MainView.outputText( 'Loppe raspberries you.  "<i>You tell me you love herms with big horse cocks and now you won\'t have sex with me?  You\'re one big tease, [name].</i>"' );
	...
	Loppe.prototype.denyToLoppeThatYouLoveZeHorsecock = function() {
		MainView.clearOutput();
		MainView.outputText( 'Not in the mood to play along with Loppe\'s little game, you simply state that your reasons are your own and will stay that way for now.' );
	...
	Loppe.prototype.gossipWithLoppeAboutScylla = function() {
		MainView.clearOutput();
		MainView.outputText( '"<i>The nun who likes sucking on dicks?</i>" Loppe asks you.  "<i>Yes, I\'ve run into her at the Wet Bitch.  She offered to blow me too.  I was tempted, y\'know?  She has those wonderful looking lips... nice big breasts too... but ultimately I wound up refusing.  It\'s just... not right.</i>"' );
	...
	Loppe.prototype.gossipWithLoppeAboutHeckel = function() {
		MainView.clearOutput();
		MainView.outputText( 'You ask if Loppe\'s ever had a run-in with a herm hyena who\'s usually running in the track at the gym.' );
	...
	Loppe.prototype.gossipWithLoppeAboutEdryn = function() {
		MainView.clearOutput();
		MainView.outputText( '"<i>That pretty centauress watchwoman that hangs in the Wet Bitch when she\'s off duty?  Yes, it\'s no secret that she specializes in particular sorts of wetwork for those with... compatible endowments,</i>" Loppe says, giving you a lecherous wink.' );
	...
	Loppe.prototype.gossipWithLoppeAboutLottie = function() {
		MainView.clearOutput();
		MainView.outputText( '"<i>That pig-girl that started hanging around the gym in the evenings?  I heard about her... I believe she wants to get fit,</i>" Loppe comments conversationally.' );
	...
	Loppe.prototype.talkWithLoppeAboutWorking = function() {
		MainView.clearOutput();
		MainView.outputText( 'Her curse naturally impacts her social life, but what about work?  How do she and her mother put food on the table?' );
	...
	Loppe.prototype.chatWithLoppeAboutHerMom = function() {
		MainView.clearOutput();
		//req LoppeChat > 0;
		MainView.outputText( 'Loppe rubs her chin, deep in thought.  "<i>Where do I start?  I guess by saying that my mom is a super mom.  She raised me all on her own, and she\'s always had time for me, despite having to keep up with her job.  I love her very much.</i>"' );
		...
		if( CoC.player.gender === 1 ) {
			MainView.outputText( '  Well, from what Loppe\'s told you, Uma wouldn\'t be attracted to you anyway.' );
		}
	...
	Loppe.prototype.chatWithLoppeAboutLoppesVillage = function() {
		MainView.clearOutput();
		MainView.outputText( 'You ask if Loppe would be willing to tell you about her village.' );
	...
	Loppe.prototype.loppeSexChoice = function( bakery ) {
		...
		if( CoC.flags[ kFLAGS.LOPPE_TIMES_SEXED ] === 0 ) {
			...
			if( !bakery ) {
				MainView.outputText( 'That \'workout\' seems like a nice option right now.' );
				...
				//(else);
					...
					if( CoC.player.lib >= 40 ) {
						MainView.outputText( '.. it almost sounds exciting.' );
					}
				}
			...
			MainView.outputText( 'It\'s not very difficult to reach Loppe\'s place; it happens to be located only a few blocks away from the gym, behind a small shop with the sign "<i>Kemono\'s Oriental Clinic</i>".' );
			...
			//[(corr < 40)];
			if( CoC.player.cor < 40 ) {
				MainView.outputText( '  You tell her they\'re quite nice... though privately you\'re curious what the interior looks like.' );
			} else {
				MainView.outputText( '  They don\'t seem to work very well - after all, <i>you\'re</i> here.' );
			}
			...
			if( CoC.player.hasCock() ) {
				if( CoC.player.smallestCockArea() <= this.loppeCapacity() ) {
					MainView.outputText( 'Pulling her onto your dick would result in some cowgirl fun, though you\'d have to deal with her cock pointed right at you when she came.  ' );
				} else {
					MainView.outputText( 'As good as it would feel to shove your [cock smallest] into her cunt, the nervous looks she\'s giving it tell you what her response would probably be.  ' );
				}
			}
			//[(any cock);
			if( CoC.player.hasCock() ) {
				MainView.outputText( 'You could just whip it out and ask what she thinks of it, compared to hers - it might lead to some cross words and crossed swords, though.  ' );
			}
			if( CoC.player.hasVagina() ) {
				MainView.outputText( 'You could take her monster length vaginally, but if her libido is what she claims, you\'ll probably wind up quite stretched.  ' );
			}
			//[(boobs >= boobjob req);
			if( CoC.player.biggestTitSize() >= 5 ) {
				MainView.outputText( 'Her hardening horsecock looks like it would fit between your [chest], an act likely to net you a messy demonstration of her enthusiasm.  ' );
			}
			MainView.outputText( 'There\'s always the option to receive her anally, though with her vaunted libido, you\'d probably end up so flooded with her cum that it would wash from your mouth.  Or you could be a prick-tease and leave.' );
			//Display sex options;
			//[Cowgirl][Frot][TakeVaginal][Boobjob][TakeAnal][Bail];
		}
		//Repeat Intro (edited);
		else {
			MainView.outputText( 'Loppe\'s other \'workout\' sounds pretty good right now.' );
			...
			MainView.outputText( 'There\'s always the option to receive her anally, though with her vaunted libido, you\'d probably end up so flooded with her cum that it would wash from your mouth.  Or you could be a prick-tease and leave.' );
			//[(loppesexed);
			MainView.outputText( '  Just jerking that cock and squeezing it so hard her balls swell up when she comes, though, that might be fun.  It\'d be interesting to see how big they can get.' );
			//Display sex options;
	...
	Loppe.prototype.loppeRidesCocks = function() {
		MainView.clearOutput();
		MainView.outputText( ImageManager.showImage( 'loppe-rides-your-cocks' ) );
		MainView.outputText( 'Looking over the hermaphroditic, horse-cocked bunny-girl, you contemplate your options.  You settle yourself ' );
		//[(not centaur)];
		if( !CoC.player.isTaur() ) {
			MainView.outputText( 'on her bed, making yourself comfortable, and start suggestively stroking yourself' );
		}//(centaur);
		else {
			MainView.outputText( 'cumbersomely on the floor, rolling over and spreading your hindlegs to expose yourself' );
		}
		MainView.outputText( '.  Quietly, you ask Loppe how she would feel about indulging her feminine half.' );
		...
		if( y >= 0 && CoC.player.cockTotal() === 2 ) {
			MainView.outputText( '  "<i>In fact... I could just eat both of those up.</i>"' );
		}//(3+ fit cocks);
		else if( y >= 0 && CoC.player.cockTotal() === 3 ) {
			MainView.outputText( '  "<i>But I\'m afraid I\'ll only be able to handle two of them.</i>"' );
		}
		...
		MainView.outputText( ', rubbing gently.  "<i>Just sit back and leave everything in my capable hands, sugar.</i>"' );
		...
		MainView.outputText( ' nice and slick.  You gasp in pleasure and surprise as you feel Loppe\'s erect cock sidle up to yours, helping lube you with its leaking pre. Raising up your hips, you clumsily try to ' );
		if( y < 0 ) {
			MainView.outputText( 'slide your shaft against her' );
		} else {
			MainView.outputText( 'pinion her shaft between your' );
		}
		MainView.outputText( ' own, shivering from the sensation of your sensitive ' + CoC.player.skin() + ' against her proud horseflesh, already drooling even though you can plainly feel that it\'s only half-erect.' );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( '  Your [balls] gently brush and rub against her own swollen cum-factories, and you can\'t wait to empty your overfilled sac into her waiting womb.' );
		}
		...
		if( y >= 0 ) {
			MainView.outputText( '; its neighbor is aimed for her tight rosebud' );
		}
		MainView.outputText( '.  "<i>Itadakimasu!</i>" Loppe says, licking her lips as she finally slides you home.' );
		...
		MainView.outputText( ', yet squeezing you as tightly as a velvety vise.  In fact, she\'s gripping you so hard that you can\'t seem to push yourself any further into her depths; she\'s squeezing you much too tightly to move in or out, an outright amazing feat, if Loppe gets half as much action as she claims to.' );
		...
		MainView.outputText( ' with happy squeals of pleasure and happiness, softly accompanied by the wet sounds of your coupling.  You gasp and cry, savoring every motion as she squeezes and wriggles, her inner walls rippling so deliciously around your cock' );
		if( y >= 0 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ', stroking them in the most delicious way.  You thrust your hips, trying to slip deeper inside her, but still she\'s holding you at bay.' );
		...
		if( y >= 0 ) {
			MainView.outputText( ' and watertight ass' );
		}
		MainView.outputText( '.  The sensation borders on the thin line between pleasure and pain, and you can\'t help but moan into Loppe\'s kiss; you wrap your ' );
		if( CoC.player.isDrider() ) {
			MainView.outputText( 'spindly legs' );
		} else if( CoC.player.isTaur() ) {
			MainView.outputText( 'forelegs' );
		} else {
			MainView.outputText( 'arms' );
		}
		MainView.outputText( ' around the bunny-girl\'s ' );
		if( CoC.flags[ kFLAGS.LOPPE_FURRY ] === 0 ) {
			MainView.outputText( 'silky-smooth' );
		} else {
			MainView.outputText( 'softly furred' );
		}
		MainView.outputText( ' back and hold her tight against you, her long horse-cock sandwiched between your bellies, smearing both of you with drooling precum, rivers of the stuff flowing out steadily with every thrust and grind the two of you make.' );
	...
	Loppe.prototype.loppeRidesPCCockFinal = function() {
		var x = CoC.player.cockThatFits( this.loppeCapacity() );
		var y = CoC.player.cockThatFits2( this.loppeCapacity() );
		MainView.outputText( ImageManager.showImage( 'loppe-rides-your-cock' ) );
		MainView.outputText( '\n\nYour ' + Descriptors.cockDescript( x ) + ' gushes fluids into her hungry womb' );
		if( y >= 0 || CoC.player.hasVagina() ) {
			MainView.outputText( ', while ' );
			if( y >= 0 ) {
				MainView.outputText( 'your second ' + Descriptors.cockDescript( y ) + ' just as easily pumps her perverted ass full of baby juice' );
			}
			if( y >= 0 && CoC.player.hasVagina() ) {
			...
			if( CoC.player.hasVagina() ) {
				...
				if( CoC.player.wetness() < 3 ) {
					MainView.outputText( 'drizzles' );
				} else if( CoC.player.wetness() < 5 ) {
					MainView.outputText( 'spatters' );
				} else {
					MainView.outputText( 'gushes' );
				}
				MainView.outputText( ' femcum onto the bed below you, forgotten during the act but not unsympathetic to her ministrations' );
			}
		}
		MainView.outputText( '.  You cry out as you cum long and hard, arching your back off of the bed in your pleasure before finishing and collapsing, gasping for breath as, above you, the laquine comes to her own climactic end.' );
		...
		MainView.outputText( Descriptors.multiCockDescriptLight() + ' and slides down your body, pressing her soft, cum-slickened breasts against you.  You moan as the smooth curves of her chest press against [eachCock]; the cum-slick ' );
		if( CoC.flags[ kFLAGS.LOPPE_FURRY ] === 0 ) {
			MainView.outputText( 'skin' );
		} else {
			MainView.outputText( 'fur' );
		}
		MainView.outputText( ' give you the impression of being wrapped in the moist folds of a woman\'s sex.  As Loppe slides further down you feel her teasingly licking at the tip' );
		if( y >= 0 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ' of your ' + Descriptors.multiCockDescriptLight() + '.' );
		...
		MainView.outputText( ' an involuntary shudder from you; the erotic stimulation sends blood pulsing back into your veins, and your shaft' );
		if( y >= 0 ) {
			MainView.outputText( 's rise' );
		} else {
			MainView.outputText( ' rises' );
		}
		MainView.outputText( ' to full mast again, so swiftly ' );
		if( y < 0 ) {
			MainView.outputText( 'it almost smacks' );
		} else {
			MainView.outputText( 'they almost smack' );
		}
		MainView.outputText( ' the laquine on her nose, leaving you ready to be mounted by her once again.' );
		...
		if( CoC.player.minLust() <= 30 && CoC.player.lib <= 50 ) {
			MainView.outputText( '; if I have to stop to tend to you after every coupling, I\'ll never get these empty' );
		}
		MainView.outputText( '.</i>"  She points with a smirk to her swollen balls.  Er... didn\'t she just blow her load? Her balls look at least as full as, if not fuller than, when you started, and they nearly slosh with eagerness to be rid of their cargo!' );
		...
		MainView.outputText( ' once more.  You moan as once again you follow the lusty dickgirl into the throes of pleasure...' );
		...
		if( CoC.player.cumQ() >= 500 ) {
			MainView.outputText( ' despite your usually messy orgasms' );
		}
		MainView.outputText( '.' );
		...
		if( CoC.flags[ kFLAGS.PC_FETISH ] > 0 ) {
			MainView.outputText( '; it gives you something of a thrill, though nowhere near as much as taking them off for her in the first place did' );
		}
		MainView.outputText( '.  Finished, you depart.' );
		CoC.player.orgasm();
	...
	Loppe.prototype.loppeWorshipsDicks = function() {
		MainView.clearOutput();
		MainView.outputText( ImageManager.showImage( 'loppe-worships-your-cock' ) );
		MainView.outputText( 'As your eyes sweep over the naked half-breed\'s form, they focus on her equine masculinity, already standing proud and eager at the prospect of sex.  Distinctly aware of your own male appendage' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ', you conversationally ask Loppe what she thinks of your [cock biggest], an idea starting to form in your mind.' );
		//(if cock area <= 17.5);
		if( CoC.player.biggestCockArea() <= 17.5 ) {
			...
			if( CoC.player.biggestCockArea() > this.loppeCapacity() ) {
				MainView.outputText( 'even if it does look too big to fit.' );
			} else {
				MainView.outputText( 'or maybe I should get a taste first?' );
			}
			MainView.outputText( '</i>"  She licks her lips.' );
		}
		...
		if( CoC.player.balls > 0 ) {
			MainView.outputText( ' and balls to balls' );
		}
		MainView.outputText( ', the delicious friction pushing you over the edge.' );
	...
	Loppe.prototype.getFuckedInYerTwatYaCunt = function() {
		MainView.clearOutput();
		MainView.outputText( ImageManager.showImage( 'loppe-sticks-it-in-your-vagoo' ) );
		MainView.outputText( 'As you contemplate the delicious possibilities that the herm presents, your eyes are drawn to her admirable piece of horse-meat.  With a half-grin, you settle yourself on her bed, ' );
		//[(mans);
		if( !CoC.player.isTaur() ) {
			MainView.outputText( 'spreading your labia in an invitation that needs no words.' );
		} else {
			MainView.outputText( 'exposing your rump and spreading your hind legs in your best effort to make her feel welcome.' );
		}

		MainView.outputText( '\n\nLoppe smirks at you and winks.  "<i>Don\'t worry; I\'ll treat you like a ' );
		//[(femininity <= 40);
		if( CoC.player.femininity > 50 ) {
			MainView.outputText( 'princess.  A sexy, smoking hot princess' );
		} else {
			MainView.outputText( 'prince.  A dashing, dandy prince' );
		}
		MainView.outputText( ' that I intend to fill up with my laquine love.</i>"  She wastes no time and pounces on the bed, ready to bury her face between your [legs], licking her lips while taking deep breaths to inhale as much of your pheromones as she can.  You blink at the speed with which she moves, but smile, repositioning yourself to give her better access' );
		//[(herm non-horse);
		if( CoC.player.gender === 3 && !CoC.player.isTaur() ) {
			MainView.outputText( ', though she needs to move aside ' + Descriptors.sMultiCockDesc() + ' ' );
			if( CoC.player.balls > 0 ) {
				MainView.outputText( 'and [balls] ' );
			}
			MainView.outputText( 'to really get at your cunt' );
		}
		MainView.outputText( '.' );
		...
		if( !CoC.player.isTaur() && CoC.player.gender === 3 ) {
			MainView.outputText( ', under your ' );
			if( CoC.player.balls > 0 ) {
		...
		MainView.outputText( ' to lick and kiss your sensitive labia.  You arch your back and thrust your crotch at her, letting the laquine have your precious pussy to taste and suckle and ravage... she\'s pretty good with that tongue of hers.  Loppe lifts her head and wipes her face with her arm.  "<i>I\'m even better with this,</i>" she says, mirroring your thoughts and holding her painfully erect equine-shaft.  You watch as it throbs, every vein bulging visibly, balls engorged as they churn up a load of cum to deposit in your waiting womb.  She ducks to take another lick, tasting you like one tastes a fine wine.' );
		if( CoC.player.vaginas[ 0 ].virgin ) {
			...
			if( CoC.player.cor < 50 ) {
				MainView.outputText( '  You look coy, for her benefit, and cast your eyes down shyly.' );
			} else {
				MainView.outputText( '  You do your best to warn her away from the topic with your eyes, eager to get on with the fuck.' );
			}
			//(not for horsez);
			if( !CoC.player.isTaur() ) {
			...
			MainView.outputText( '\n\nYou reach out ' );
			if( CoC.player.cor < 50 ) {
				MainView.outputText( 'and tap her playfully on the nose' );
			} else {
				MainView.outputText( 'in your irritation and squeeze her face imploringly' );
			}
			MainView.outputText( '.  Would you be here, with your clothes off in Loppe\'s bed, if you weren\'t okay with it?' );
		...
		if( CoC.player.vaginalCapacity() >= 35 ) {
			MainView.outputText( ' bottoms out, completely hilted inside you' );
		}//[else:;
		else {
			MainView.outputText( ' bumps against your cervix' );
		}
		MainView.outputText( '. You moan in delight as she sinks into you' );
		if( CoC.player.vaginas[ 0 ].virgin ) {
			MainView.outputText( ', repressing a shiver of pain as she tears through your hymen, taking your virginity' );
		} else if( CoC.player.vaginalCapacity() < 35 ) {
			MainView.outputText( ', forcing your entrance wide to accommodate herself' );
		}
		MainView.outputText( '.' );
		...
		//[(not horse);
		if( !CoC.player.isTaur() ) {
			MainView.outputText( '  You wrap your [legs] around her hips as best you can, thrusting in your efforts to meet her strong rhythm, feeling your cunt rippling around her wonderfully long, thick horsecock.' );
		}
		...
		if( CoC.player.hasCock() ) {
			MainView.outputText( ', precum begins to drool from your ' + Descriptors.multiCockDescriptLight() );
		}
		MainView.outputText( '.' );
		//Not Centaur:;
		if( !CoC.player.isTaur() ) {
			...
			if( CoC.player.gender >= 3 ) {
				MainView.outputText( ' and smearing both your bellies with herm-pre' );
			}
			MainView.outputText( '.' );
		}
		//Centaur:;
		...
		if( CoC.player.hasCock() ) {
			MainView.outputText( ', neglected cock fountaining cum ' );
			if( CoC.player.cumQ() < 500 ) {
				MainView.outputText( 'between you and painting you both' );
			} else {
				MainView.outputText( 'gushing onto the bedsheets and smearing your belly' );
			}
			MainView.outputText( ' with this aspect of your dual-sexed orgasm' );
		}
		MainView.outputText( '.' );
		...
		if( !CoC.player.isTaur() && CoC.player.hasCock() ) {
			MainView.outputText( ', giving you a quick peck on the cheek and licking a small gob of cum that seems to have stuck to your chin' );
		}
		MainView.outputText( '.' );
		...
		//[(low corr);
		if( CoC.player.cor < 50 ) {
			MainView.outputText( 'a pleading look... and if there\'s maybe a trace of horror dancing in the depths of your eyes, well, that\'s understandable, isn\'t it?' );
		} else {
			MainView.outputText( 'an incendiary glare, letting her know just what you think of her posturing.' );
		}
		MainView.outputText( '  Loppe bursts out laughing.  "<i>Just kidding, sugar.  If we go another round I think my dick might fall off.</i>"' );
		...
		if( EngineCore.silly() ) {
			MainView.outputText( '  "<i>Da! Sandvich is kredit to team!</i>"' );
		}
		//(First Time);
		if( CoC.flags[ kFLAGS.LOPPE_TIMES_SEXED ] === 0 ) {
		...
		if( CoC.player.lib >= 50 || CoC.player.minLust() >= 30 ) {
			MainView.outputText( '  Not that you didn\'t just titillate yourself thinking of it...' );
		}
		MainView.outputText( '  Your lover laughs, "<i>Okay, you got me there... but, anyway, I have some chores to run for my mom, so I guess I\'ll see you later?</i>"' );
	...
	Loppe.prototype.getButtFuckedNonHoarseByLoppe = function() {
		MainView.clearOutput();
		MainView.outputText( ImageManager.showImage( 'loppe-sticks-it-in-your-butt' ) );
		MainView.outputText( 'You swallow as your eyes are drawn to the equine sausage jutting out between the bunny-girl\'s legs, and know you just have to have it.  Slowly, you spread yourself upon Loppe\'s bed, scooting up toward the head and pillows and kneeling, [butt] in the air, inviting the laquine to do with you as she will.' );
		...
		if( CoC.player.hasCock() ) {
			...
			MainView.outputText( ' dripping pre' );
			if( CoC.player.hasVagina() ) {
				MainView.outputText( ' and' );
			}
		...
		if( CoC.player.hasVagina() ) {
			MainView.outputText( ' pussy oozing femlube' );
		}
		MainView.outputText( '... you beg her to go faster, to do it harder!' );
		MainView.outputText( '\n\nLoppe happily complies, thrusting herself deeper and stronger.  ' );
		//[(cock);
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'One of her hands reaches around to grasp your pre-slickned [cock biggest], stroking you in time with her thrusts.  ' );
		}
		if( CoC.player.gender === 2 ) {
			MainView.outputText( 'One of her hands reaches around to tease your dripping [vagina] and erect [clit], dipping a finger in every time she bottoms out.  ' );
		}
		MainView.outputText( 'Wet slapping noises fill the room, and you gasp in pleasure each time her cum-churning balls slap against ' );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( 'yours' );
		} else {
			MainView.outputText( 'your [butt]' );
		}
		MainView.outputText( '.  Loppe lays down on top of you, panting hotly on your neck, moaning in pleasure each time your sphincter contracts.' );
		...
		if( CoC.player.hasVagina() ) {
			MainView.outputText( '  A gush of sexual fluids from your crotch ' );
			if( CoC.player.wetness() < 3 ) {
				MainView.outputText( 'spatters' );
				...
				MainView.outputText( 'completely soaks' );
			}
			MainView.outputText( ' the bed-linen under you.' );
		}
		...
		if( CoC.flags[ kFLAGS.SATYR_KIDS ] > 0 ) {
			MainView.outputText( 'You laugh softly; hopefully she\'ll never have to meet the creature that dispelled that same doubt in you.  ' );
		}
		MainView.outputText( '"<i>I think this is time for sleeping, not joking.</i>"  With a trace of wounded dignity, she lays herself down and curls into your back, one arm wrapped around your gurgling belly.' );
		...
		//(First Time);
		if( CoC.flags[ kFLAGS.LOPPE_TIMES_SEXED ] === 0 ) {
			MainView.outputText( 'Loppe laughs nervously.  "<i>Sorry about that.  It\'s just that I usually can\'t bag a real cutie like you, and I think I might have gone a bit overboard... is there anyway I can make it up to you?</i>"' );
	...
	Loppe.prototype.getAssFuckedByLoppeAsACentaur = function() {
		MainView.clearOutput();
		MainView.outputText( ImageManager.showImage( 'loppe-sticks-it-in-your-butt-you-centaur' ) );
		MainView.outputText( 'You swallow as your eyes are drawn to the equine sausage jutting out between the bunny-girl\'s legs, and know you just have to have it.  But is she confident in her ability, even with your shape?' );
		...
		//[ass wet);
		if( CoC.player.ass.analWetness > 0 ) {
			MainView.outputText( '  She doesn\'t even have to bother with lube; your ass is wet enough to accommodate her on its own.' );
		}
		...
		if( CoC.player.balls > 0 ) {
			...
			MainView.outputText( 'r pussy' );
		}
		MainView.outputText( '.' );
		MainView.outputText( '\n\nYou moan as she gets into her task' );
		if( CoC.player.hasCock() ) {
			MainView.outputText( ', feeling your erect [cock] press against your underbelly, mashed between your centaur half and the soft carpet of Loppe\'s room.  You hope she doesn\'t mind a few stains...  ' );
		}//(else vag);
		else if( CoC.player.hasVagina() ) {
			MainView.outputText( '; your cunt is sopping, making a small pool of juices one the floor.  ' );
		} else {
			MainView.outputText( '.  ' );
		}
		MainView.outputText( 'She seems more worked up than usual, perhaps because you have a horse-like lower body?  "<i>Are your instincts acting up?</i>" you tease her.  "<i>Or do you just love ass that much?</i>"' );
		...
		if( CoC.player.gender <= 1 ) {
			MainView.outputText( '  Even if you are the wrong sex entirely for that.' );
		}
		...
		//[PC has cock:;
		if( CoC.player.hasCock() ) {
			MainView.outputText( '  The bumps against your prostate send waves of pleasure running through [eachCock] and spreading along your body, until they reach your throat and come out as a ragged groan.  ' );
		}
		MainView.outputText( 'If she keeps this up for too much longer, you will pop any moment now...' );
		...
		if( CoC.player.hasCock() ) {
			MainView.outputText( '  [EachCock] spasms and unloads beneath you, smearing you and the carpet below and pooling thickly with fluid forced out by the prostate stimulation.' );
		}
		if( CoC.player.hasVagina() ) {
			MainView.outputText( '  Your vagina contracts, trying to grip at a phantom member as you spill your fluids on the carpet beneath you.' );
		}
		...
		if( CoC.player.skinType !== AppearanceDefs.SKIN_TYPE_SCALES ) {
			MainView.outputText( ' have goosebumps all over your body' );
		} else {
			MainView.outputText( 'r scales are practically standing on end' );
		}
		MainView.outputText( '.' );
		MainView.outputText( '\n\n"<i>Ohhh... you are incredible, [name].  ' );
		//[PC has dick that fits);
		if( CoC.player.cockThatFits( this.loppeCapacity() ) >= 0 ) {
			MainView.outputText( 'Though I hope you won\'t mind riding me, sometime,</i>" she laughs.  "<i>' );
		}
		MainView.outputText( 'How about another round, hmm?  Big strong \'taur like you should certainly be able to hold a little more loving inside of you, right?</i>"  She croons happily, running her hands up and down your ass as if trying to entice you.' );
		//(Low Libido);
		if( CoC.player.lib < 33 ) {
		...
		MainView.outputText( '  Anyway, your clothes are back in my room; sorry to fuck and run, but I\'ve got some errands to run before mom gets home - you\'ll have to see yourself out, okay?</i>"' );
	...
	Loppe.prototype.loppeSqueezedickWhateverThatIs = function() {
		MainView.clearOutput();
		MainView.outputText( ImageManager.showImage( 'loppe-orgazm-denial' ) );
		//(if LoppeDenial === 0);
		if( CoC.flags[ kFLAGS.LOPPE_DENIAL_COUNTER ] === 0 ) {
			MainView.outputText( 'After seeing Loppe\'s prodigious output, you wonder how she would handle not being able to orgasm.' );
	...
	Loppe.prototype.letsLoppeGoCum = function() {
		MainView.clearOutput();
		MainView.outputText( ImageManager.showImage( 'loppe-comes-after-orgasm-denial' ) );
	...
	Loppe.prototype.superLoppeOrgasmDenialGo = function() {
		MainView.clearOutput();
		MainView.outputText( ImageManager.showImage( 'loppe-comes-after-moar-orgasm-denial' ) );
		MainView.outputText( 'For fear of not being able to hold her back anymore, you return both hands to tightly gripping her shaft.  Tears escape Loppe\'s eyes as she\'s denied yet again.  "<i>Stop...</i>" she says meekly.' );
		...
		MainView.outputText( '\n\n"<i>Bye, sugar!' );
		if( CoC.player.hasCock() ) {
			MainView.outputText( '  Maybe next time I\'ll be the one to give you a bit of a holdout!' );
		}
	...
	Loppe.prototype.boobjobLoppe = function() {
		MainView.clearOutput();
		MainView.outputText( ImageManager.showImage( 'loppe-give-her-a-titty-fuck' ) );
		MainView.outputText( 'You contemplate your options, unthinkingly rubbing your breasts.  As you do so, you notice your lover is observing your hands very keenly.  With a grin, you grope your tits and ask if Loppe likes your display.' );
		...
		if( (CoC.player.str < 50 && !CoC.player.isTaur()) || (CoC.player.str < 30 && CoC.player.isTaur()) ) {
			MainView.outputText( '  Struggle as you may, you\'re powerless to stop the drooling laquine as she sends you crashing down on her bed.  The whole piece of furniture shakes ominously with the force of your impact, and for a moment you think it will break apart.' );
			...
			if( CoC.player.tongueType > AppearanceDefs.TONUGE_HUMAN ) {
				MainView.outputText( 'looping your inhuman tongue around it and stroking for extra stimulus even while ' );
			}
			MainView.outputText( 'grabbing your breasts and squeezing the horsemeat between them.  The sooner you help her get over this frenzy, the sooner she\'ll get off of you... hopefully.  "<i>Ah, sugar!  I knew you liked it rough!</i>" your suddenly forceful lover shouts, ominously.' );
			...
			if( EngineCore.silly() ) {
				MainView.outputText( '.  You can\'t help but think of Family Matters' );
			}
			MainView.outputText( '.  You simply nod and groan, hands cradling your distended gut.  Loppe snuggles up against you, rubbing your bloated midriff.  "<i>Sorry about that... hmm, how about we just cuddle and rest?</i>"' );
			...
			MainView.outputText( '  You hold the laquine\'s arms and force her onto the floor; she lands with an audible "<i>oomph</i>" and you grin as you hold the eager herm down.' );
			...
			if( CoC.player.cor >= 60 ) {
				MainView.outputText( 'You don\'t really feel like it, but you figure you had better humor the greedy herm if you want to keep fucking her whenever you like.  ' );
			}
			MainView.outputText( 'Oh, well... you reach down and heft the surprised hybrid off of the floor and onto her spattered bed, then slide yourself into it as best you can, wrapping your arms around her.  She hugs you back, then yawning widely.  "<i>Naptime...</i>"' );
		...
		if( (CoC.player.str < 50 && !CoC.player.isTaur()) || (CoC.player.str < 30 && CoC.player.isTaur()) ) {
			MainView.outputText( '- while your stomach is still heavily swollen and gurgles in protest, it\'s not bad enough to impair you - ' );
		}
		MainView.outputText( 'before going over and dressing yourself.  Loppe waves goodbye to you as you head out once again.' );
		CoC.flags[ kFLAGS.LOPPE_TIMES_SEXED ]++;
		CoC.player.slimeFeed();
		CoC.player.orgasm();
		EngineCore.dynStats( 'tou', 0.5, 'lib', 0.5, 'sen', -4 );
	...
	Loppe.prototype.beATeaseAndLeaveLoppeAfterSexInvite = function() {
		MainView.clearOutput();
		MainView.outputText( ImageManager.showImage( 'loppe-teaze-and-leave' ) );
		MainView.outputText( 'Despite the hybrid beauty standing stark naked before you, you decide that you aren\'t in the mood after all.' );
	...
	Loppe.prototype.fondleAndTease = function( output ) {
		if( output === undefined || output ) {
			MainView.clearOutput();
			MainView.outputText( 'You lean in over the table and tell Loppe you\'d like to talk about her.  She looks vaguely confused.  "<i>Well, I already told you about everything I can think of.  Was there something you wanted to explore in more detail?</i>"' );
	...
	Loppe.prototype.teaseLoppeNSuck = function() {
		MainView.clearOutput();
		MainView.outputText( 'You lean over the table and give the laquine a kiss.  Confused, but not displeased, Loppe eagerly returns it, then watches as you break the lip lock and start to slide yourself under the table.  "<i>Umm... sugar?  What are you doing?</i>"' );
		...
		if( CoC.player.tongueType > AppearanceDefs.TONUGE_HUMAN ) {
			MainView.outputText( ', inhumanly long tongue looping out to coil wetly around the length' );
		}
		MainView.outputText( '.  You suckle gently, letting the salty tang of hot spunk wash over your tastebuds, and then start to gulp and swallow your way down her prick\'s sizable length.' );
