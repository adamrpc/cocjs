...
angular.module( 'cocjs' ).run( function( SceneLib, Appearance, CockTypesEnum, kFLAGS, Descriptors, CoC, EngineCore ) {
	...
	Heckel.prototype.greetHeckel = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.MET_HECKEL ] === 0 ) {
			...
			EngineCore.outputText( '"<i>So, ' + CoC.getInstance().player.short + ', what can I do for you?  The person working the desk can give you a tour if that\'s what you need... or maybe you were looking for a training partner?  If you can keep up with me, of course.</i>"\n\n', false );
			CoC.getInstance().flags[ kFLAGS.MET_HECKEL ]++;
		}
		//Brooke + Heckel 3some;
	...
	Heckel.prototype.heckelTraining = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.TIMES_FUCKED_HECKEL_BLOWJOB ] + CoC.getInstance().flags[ kFLAGS.TIMES_FUCKED_HECKEL_ANAL ] === 0 ) {
			...
			if( CoC.getInstance().player.str > 50 && CoC.getInstance().player.hasCock() ) {
	...
	Heckel.prototype.heckelOrallyFixated = function() {
		EngineCore.outputText( '', true );
		if( CoC.getInstance().flags[ kFLAGS.TIMES_FUCKED_HECKEL_BLOWJOB ] === 0 ) {
	...
	Heckel.prototype.heckelLovesDaAnal = function( loss ) {
		...
		EngineCore.outputText( 'Heckel\'s eyes flash as her grin splits her face, and for just a moment you wonder if this might have been a bad idea. Before you can object, however, she\'s pushing you down onto your stomach, your arms and ' + CoC.getInstance().player.legs() + ' draped over either side of the wooden bench.\n\n', false );
		...
		if( CoC.getInstance().player.tone >= 80 ) {
			EngineCore.outputText( '  "<i>Damn ' + CoC.getInstance().player.short + ', you\'ve got one hell of an ass. I bet it\'s gonna feel even better than it looks.</i>"', false );
		}
		//[<IF TONE BETWEEN 60-79>;
		else if( CoC.getInstance().player.tone >= 60 ) {
			EngineCore.outputText( '  "<i>Mmm, not bad ' + CoC.getInstance().player.short + ', not perfect, but not bad.</i>"', false );
		}
		EngineCore.outputText( '\n\n', false );
		...
		CoC.getInstance().flags[ kFLAGS.TIMES_FUCKED_HECKEL_ANAL ]++;
	};
	Heckel.prototype.heckelLovesAnal2 = function() {
		...
		CoC.getInstance().player.buttChange( 28, true, true, false );
		EngineCore.outputText( '\n\n', false );
		...
		if( CoC.getInstance().player.gender === 3 ) {
			EngineCore.outputText( 'Heckel wastes no time before going back to her brutal fucking, though her thrusts are limited to short bursts now. She makes up for this in speed as you lie helplessly below her, the pain of taking her knot slowly turning into intense pleasure. The heat and pleasure spreads from your ass to your entire groin, and the knot pushes against the walls of your vagina hard enough to make them rub against each other. Heckel cries out above you and slams her hips into you as much as she can, and you immediately feel a burning heat shooting up your gut. She pumps again as she seeds your insides with another spurt, and this time you orgasm along with her. Your ' + Descriptors.vaginaDescript( 0 ) + ' spasms and gushes over your soaked ' + CoC.getInstance().player.legs() + ', your asshole spasming around her cock just as much. Your ' + Descriptors.multiCockDescriptLight() + ' soon follows, shooting your own seed up between your ' + Descriptors.chestDesc() + ' and coating your stomach. Even as your orgasm begins to taper off, you can do little more than lie panting in a pool of your own spunk. Your legs quiver beneath hers as the warmth spreads further up your gut, leading you into a warm and hazy afterglow.\n\n', false );
		}
		//[<MALE PC>;
		else if( CoC.getInstance().player.hasCock() ) {
	...
	Heckel.prototype.dominateHeckel = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.TIMES_DOMMED_HECKEL ] === 0 ) {
		...
		if( CoC.getInstance().flags[ kFLAGS.TIMES_LOST_HECKEL_DOM_CHALLENGE ] > 0 ) {
		...
		if( CoC.getInstance().player.str < 75 ) {
			...
			CoC.getInstance().flags[ kFLAGS.TIMES_LOST_HECKEL_DOM_CHALLENGE ]++;
		}
		//{Win};
		else {
			...
			CoC.getInstance().flags[ kFLAGS.TIMES_DOMMED_HECKEL ]++;
		}
	};
	//[Fuck Heckel];
	Heckel.prototype.dominateHeckelConclusion = function() {
		...
		if( CoC.getInstance().player.balls > 0 ) {
			...
			EngineCore.outputText( ', basting your ' + CoC.getInstance().player.skin() + ' with her steaming warmth' );
		}
		EngineCore.outputText( '.  You give her a knowing smile, ' + CoC.getInstance().player.mf( 'man', '\'woman\'' ) + ' to woman, and rock forward, sliding through her puffy labia and into her silky canal.  Those wet hyena-lips cling to every vein and rigid nook of your ' + Descriptors.cockDescript( x ) + ', slurping your penis down the slick tunnel with hungry undulations.' );
		...
		if( CoC.getInstance().player.cumQ() > 500 ) {
			...
			if( CoC.getInstance().player.cumQ() >= 3000 ) {
				...
				if( CoC.getInstance().player.cumQ() >= 4000 ) {
					...
					} else if( CoC.getInstance().player.cumQ() < 15000 ) {
						EngineCore.outputText( 'knee high by most measurements' );
					} else if( CoC.getInstance().player.cumQ() < 20000 ) {
