...
angular.module( 'cocjs' ).run( function( SceneLib, Utils, AppearanceDefs, kFLAGS, Descriptors, CoC, EngineCore ) {
	...
	Ifris.prototype.approachIfris = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.MET_IFRIS ] === 0 ) {
			CoC.getInstance().flags[ kFLAGS.MET_IFRIS ] = 1;
		...
		//2a-Repeat approach, no sex-;
		else if( CoC.getInstance().flags[ kFLAGS.TIMES_FUCKED_IFRIS_BLOWJOB ] + CoC.getInstance().flags[ kFLAGS.TIMES_FUCKED_IFRIS_LICKED ] === 0 ) {
			...
			if( CoC.getInstance().flags[ kFLAGS.IFRIS_SHOWED_OFF ] === 0 ) {
	...
	Ifris.prototype.workOutForIfris = function() {
		...
		EngineCore.choices( 'Work Out', this.liftWhileIfrisWatches, 'Show Off', this.showOffForIfris, '', null, '', null, '', null );
	};
	...
	Ifris.prototype.liftWhileIfrisWatches = function() {
		EngineCore.spriteSelect( 28 );
		CoC.getInstance().flags[ kFLAGS.IFRIS_SHOWED_OFF ]++;
		EngineCore.outputText( '', true );
	...
	Ifris.prototype.showOffForIfris = function() {
		...
		if( CoC.getInstance().player.gender === 0 || (CoC.getInstance().player.femininity < 40 && CoC.getInstance().player.cor > 75 && CoC.getInstance().player.faceType === AppearanceDefs.FACE_COW_MINOTAUR) ) {
		...
		if( CoC.getInstance().player.hasCock() ) {
			//Double dickings!;
			if( CoC.getInstance().player.cockTotal() > 1 && Utils.rand( 2 ) === 0 && CoC.getInstance().player.cockThatFits( 70 ) !== -1 && CoC.getInstance().player.cockThatFits2( 70 ) !== -1 ) {
				this.ifrisDP();
				return;
			}
			//First time;
			if( CoC.getInstance().flags[ kFLAGS.TIMES_FUCKED_IFRIS_BLOWJOB ] === 0 ) {
				...
				EngineCore.outputText( 'Spurred on by your ego, you lift the huge weight with seeming effortlessness.  The devil-girl seems to move closer as your muscles burn and strain. Suddenly you feel one of her slender hands sliding up your ' + CoC.getInstance().player.leg() + ', the smooth palm stroking your thigh, and then cupping your crotch. A surge of arousal ripples through your cock, making the length twitch and immediately begin to swell as she squeezes, making you pause in your efforts.\n\n', false );
			...
			//5a-Repeat oral scene, male/herm-;
				...
				EngineCore.outputText( 'With that slow, almost painfully-hard suckle, she pops from the ' + CoC.getInstance().player.cockHead() + ' of your ' + Descriptors.cockDescript( 0 ) + ' out and sucks in a deep breath, giggling and leaning down to kiss you.  She murmurs, "<i>You do better each time, darling. I\'ll see you again I hope.</i>"  With that, she\'s leaving you again, all but skipping out the door, that wetness trickling down her long, red legs...', false );
			}
			CoC.getInstance().flags[ kFLAGS.TIMES_FUCKED_IFRIS_BLOWJOB ]++;
		}
		//4b4-PC succeeds! Is female!-;
		else if( CoC.getInstance().player.hasVagina() ) {
			if( CoC.getInstance().flags[ kFLAGS.TIMES_FUCKED_IFRIS_LICKED ] === 0 ) {
				EngineCore.outputText( 'Gripping the bars tightly, you grit your teeth and give a mighty push upwards! Your muscles strain and you grunt with effort, a little amazed gasp leaving the red beauty\'s lips as the weights lift slowly but surely. You can\'t help but grin proudly as your elbows lock, the weight completely lifted, and you bring it back down, only to push again.\n\n', false );
				EngineCore.outputText( 'Ifris moves closer, her oiled skin glistening as she watches your muscles straining, bending in to get a closer look. As you look down you notice her bend forward, the weight momentarily forgotten as those luscious, perfect mounds of tit-flesh almost spill out of her bikini top. She grins slowly at you, following your gaze, and even giving a teasing little shake, making the orbs jiggle.\n\n', false );
				EngineCore.outputText( '"<i>Mmm, keep going, darling... I like what I see,</i>" she all but purrs out.\n\n', false );
			...
			CoC.getInstance().flags[ kFLAGS.TIMES_FUCKED_IFRIS_LICKED ]++;
		}
		//Stat changes HERE!;
		if( CoC.getInstance().player.str < 90 ) {
			EngineCore.dynStats( 'str', 0.5 );
	...
	Ifris.prototype.ifrisDP = function() {
		...
		EngineCore.outputText( 'With the first set finished, you lower the bar until the weight is released from your well-developed musculature.  The demonic beauty is leaning down, over your ' + Descriptors.chestDesc() + ', drinking in every sweat-slicked part of you.  With her breasts wobbling dangerously, her glossy, dark lips slowly part to blow you a kiss.  Then, the onyx pillows open to purr, "<i>Mmmm... marvelous work darling.</i>"  She leans a little lower, letting her slick breasts rub over the crotch of your ' + CoC.getInstance().player.armorName + ' as she asks, "<i>Do you think... you could lift more?  Enough to truly handle anything the world might throw at you?</i>"  Ifris trails her fingers over your ' + CoC.getInstance().player.armorName + ', slowly undoing the equipment with each enunciated syllable.\n\n', false );
		...
		else if( CoC.getInstance().player.demonCocks() >= 2 ) {
		...
		else if( CoC.getInstance().player.horseCocks() >= 2 ) {
		...
		//(Tentacle pistoning + head-mushrooming);
			...
			EngineCore.outputText( 'At the same time, both tentacle-shafts curl and piston, winding around in other in the space between your bodies, curling and thrusting to rub every inch of your sensitive, prehensile shafts against Ifris\' pussy-walls.  You shake and shudder, feeling more like a tentacle beast than a ' + CoC.getInstance().player.mf( 'man', 'woman' ) + '.  ', false );
		}
		//(Anemone - stinging funtimes);
		else if( CoC.getInstance().player.anemoneCocks() >= 2 ) {
