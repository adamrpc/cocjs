...
angular.module( 'cocjs' ).run( function( SceneLib, Utils, AppearanceDefs, kFLAGS, Descriptors, CoC, EngineCore ) {
	...
	Ifris.prototype.approachIfris = function() {
		...
		if( CoC.flags[ kFLAGS.MET_IFRIS ] === 0 ) {
			...
			EngineCore.outputText( 'The curious, appraising gaze on her pretty face turns into a pleased smile as you walk over, and her clawed toes click gently on the floor as she takes the last few steps to meet you. As you open your mouth to speak she preempts you, those pretty, black-painted lips parting, a voice too husky and smooth to be up to any good greeting you.\n\n', false );
			EngineCore.outputText( '"<i>Hello, darling. I\'m so glad you decided to join me... You can call me Ifris.</i>" She tilts her head to the side cutely, gazing up at you through her lashes.\n\n', false );
			EngineCore.outputText( 'You give her your name, mouth suddenly feeling a little too dry for some reason as she gives you that sultry smile and lingering gaze.\n\n', false );
			EngineCore.outputText( '"<i>It\'s nice to meet you. Are you here to use the machines, or were you just hoping to watch the pretty bodies working up a sweat?</i>" she giggles cutely, two of her clawed fingers covering her mouth demurely.\n\n', false );
		}
		//2a-Repeat approach, no sex-;
			...
			EngineCore.outputText( 'Ifris grins as you walk over again, the devil girl\'s dark tongue dabbing at her even darker lips as she all but coos up at you.\n\n', false );
			if( CoC.flags[ kFLAGS.IFRIS_SHOWED_OFF ] === 0 ) {
				EngineCore.outputText( '"<i>Hello darling... here to actually give me a show?</i>"\n\n', false );
			} else {
				EngineCore.outputText( '"<i>Hello darling... here to give another show? I did so enjoy your last performance.</i>"\n\n', false );
			}
		}
		//2b-Repeat approach, had oral-;
		else {
			EngineCore.outputText( 'Ifris grins as you walk over again, the devil girl\'s dark tongue dabbing at her even darker lips. A small shudder runs through you as you imagine her remembering your flavor...\n\n', false );
			EngineCore.outputText( '"<i>Hello darling... so lovely to see you. Are you here to give me a show this time? Or do you think you want to try to beat your last record?</i>"\n\n', false );
			EngineCore.outputText( 'A bit confused, you ask her what record she\'s talking about, only to evoke a small, cute laugh from her.\n\n', false );
			EngineCore.outputText( '"<i>How long you can lift that much before my attentions are too distracting, of course.</i>"\n\n', false );
		}
	...
	Ifris.prototype.workOutForIfris = function() {
		...
		EngineCore.outputText( '"<i>Oh, don\'t mind me, then. Please, by all means.</i>" She gestures to the bench-press, stepping back and watching you with a smile.\n\n', false );
		EngineCore.outputText( '"<i>I hope you don\'t mind if I keep you company. I\'d love to... see you in action.</i>"\n\n', false );
	...
	Ifris.prototype.askIfrisToJoinYou = function() {
		...
		EngineCore.outputText( '"<i>That\'s sweet of you, darling... but I don\'t want to get in the way. I\'ll just wait my turn. Don\'t worry about me at all.</i>"\n\n', false );
		EngineCore.outputText( 'You shrug at her response, but there\'s something about the way she eyes you now...', false );
		//WORK OUT or SHOW OFF?;
		EngineCore.choices( 'Work Out', this, this.liftWhileIfrisWatches, 'Show Off', this, this.showOffForIfris, '', null, null, '', null, null, '', null, null );
	};
	...
	Ifris.prototype.showOffForIfris = function() {
		...
		if( CoC.player.str < 40 ) {
			...
			EngineCore.outputText( 'Ifris only rolls her eyes, flipping a hand at you dismissively as she walks away, murmuring.\n\n', false );
			EngineCore.outputText( '"<i>I\'ve no time for show-offs who can\'t back it up, darling.</i>"\n\n', false );
			EngineCore.outputText( 'Humiliated, you stand and leave, nursing your sore body and ego.', false );
		...
		if( CoC.player.str < 75 ) {
			...
			EngineCore.outputText( '"<i>Oh well...</i>" She sighs, clearly crestfallen. Cocking one fist on her hip, she gives your ' + CoC.player.leg() + ' a light pat as she leaves your presence, vanishing out the door within moments.\n\n', false );
	...
	Ifris.prototype.ifrisDP = function() {
		...
		if( CoC.player.dogCocks() >= 2 ) {
			EngineCore.outputText( 'Both your canine cocks bloat with seed, the knot\'s distending and stretching so wide that Ifris couldn\'t pull off if she wanted.  A moment later, the twin dog-dicks explode pillars of alabaster cream, painting the truly knotted woman\'s innards white.  ', false );
			if( CoC.player.cumQ() >= 1000 ) {
				EngineCore.outputText( 'She groans and moans as she\'s filled, her body distending to deal with the unholy amount of seed your cocks can spew.  ', false );
			}
			EngineCore.outputText( 'Throughout the massive release, you\'re softly grunting, held immobile by your animalistic body\'s instincts as your peckers try their damnedest to impregnate your overly fit mate.  You\'re still cumming, but your knots are starting to deflate at last.  ', false );
		}
		//(Nodule growing, unholy demonocity);
		else if( CoC.player.demonCocks() >= 2 ) {
			EngineCore.outputText( 'Both your demonic cocks explode inside Ifris, the nodules along each shaft swelling out, thickening to stroke and caress your oily lover\'s inner walls.  A moment later, the tainted tips explode out, spurting demon-spunk into both silky tunnels.  ', false );
			if( CoC.player.cumQ() >= 1000 ) {
				EngineCore.outputText( 'She groans and moans as she\'s filled, her body distending to deal with the unholy amount of seed your cocks can spew.  ', false );
			}
			EngineCore.outputText( 'At the same time, your hands squeeze hard on her butt-cheeks and your hips start to jack-hammer against her, slamming into her as if she was a worthless piece of meat for you to impregnate.  You shiver, feeling not entirely in control of yourself, the demonic influence of your tainted members robbing you of control.  ', false );
		}
		//Flaring - req's 2 horsecocks!);
		else if( CoC.player.horseCocks() >= 2 ) {
			EngineCore.outputText( 'Both your equine cocks explode inside Ifris, their tips swelling outward into thick flares that plug her stuffed channels.  A moment later, the swollen tips unleash pillars of alabaster cream, painting everything above the flares white.  ', false );
			if( CoC.player.cumQ() >= 1000 ) {
				EngineCore.outputText( 'She groans and moans as she\'s filled, her body distending to deal with the unholy amount of seed your cocks can spew.  ', false );
			}
			EngineCore.outputText( 'Your hips begin to rock and thrust, and no matter what you try to do, you cannot seem to still your body, your animal instincts owning your waist as you inseminate your seductive, teasing lover\'s body.  ', false );
		}
		//(Tentacle pistoning + head-mushrooming);
		else if( CoC.player.tentacleCocks() >= 2 ) {
			EngineCore.outputText( 'Both your plant-like pricks explode inside Ifris, the purplish tips swelling up inside her to disproportionate sizes.  A moment later, the vine-dicks explode out twin pillars of ropey seed, painting her inner walls with spunk.  ', false );
			if( CoC.player.cumQ() >= 1000 ) {
				EngineCore.outputText( 'She groans and moans as she\'s filled, her body distending to deal with the unholy amount of seed your cocks can spew.  ', false );
			}
			EngineCore.outputText( 'At the same time, both tentacle-shafts curl and piston, winding around in other in the space between your bodies, curling and thrusting to rub every inch of your sensitive, prehensile shafts against Ifris\' pussy-walls.  You shake and shudder, feeling more like a tentacle beast than a ' + CoC.player.mf( 'man', 'woman' ) + '.  ', false );
		}
		//(Anemone - stinging funtimes);
		else if( CoC.player.anemoneCocks() >= 2 ) {
			EngineCore.outputText( 'Both your unusual, aquatic cocks explode inside Ifris, their shafts thickening as the prepare for release.  A moment later, they explode out twin pillars of alabaster cum, painting her inner walls with gooey spunk.  ', false );
			if( CoC.player.cumQ() >= 1000 ) {
				EngineCore.outputText( 'She groans and moans as she\'s filled, her body distending to deal with the unholy amount of seed your cocks can spew.  ', false );
			}
			EngineCore.outputText( 'At the same time, your stinging cilia go wild, pricking her inside and out, uncontrollably forcing aphrodisiacs into your partner\'s shuddering, orgasmic body.  The drugs force her to climb to whole new plateaus of pleasure, making her squeal with unrepentant pleasure.  You convulse and get off even more forcefully from using your strange members in such a way.  ', false );
		}
		//(Standard);
		else {
			EngineCore.outputText( 'Both your cocks explode inside Ifris, submitting alabaster pillars to her request for your release.  The red-hued sexpot exhales a happy hum and rocks her hips back and forth, her oily, lubricated holes squeezing your dicks for every drop of delectable seed.  ', false );
			if( CoC.player.cockTotal() > 2 ) {
				EngineCore.outputText( 'The ', false );
				if( CoC.player.cockTotal() === 1 ) {
					EngineCore.outputText( 'one unbound prick remaining paints the muscular woman\'s lower back with goo, but she doesn\'t seem to mind.', false );
				} else {
					EngineCore.outputText( 'other, unbound pricks paint the muscular woman\'s lower back in goo, but she doesn\'t seem to mind.', false );
				}
				EngineCore.outputText( '  ', false );
			}
		}
		//(ALL GO HERE - NOT NEW PG) ;
		EngineCore.outputText( 'She purrs, "<i>Delightful,</i>" into your ear and plants a wet kiss on you, surely leaving a dusky, black lip-print on your neck.\n\n', false );
		EngineCore.outputText( 'The last few contractions work through you, emptying the last of your load inside Ifris and allowing you to feel utterly, completely at peace.  Sliding up, she lets your slowly-deflating cocks ', false );
		if( CoC.player.dogCocks() >= 2 ) {
			EngineCore.outputText( 'noisily pop free', false );
		} else {
			EngineCore.outputText( 'slide', false );
		}
		EngineCore.outputText( ' from her sperm-glazed snatch and anus.  Her body\'s warmth vanishes as she stands, and with a wink, she reattaches her bikini bottom, the tight garment moulding to her soaked camel-toe and holding in all of your leavings.  She walks away without another word, ', false );
		if( CoC.player.cumQ() >= 1000 ) {
			EngineCore.outputText( 'her body bloated with cum and sloshing audibly with every step', false );
		} else if( CoC.player.cumQ() >= 500 ) {
			EngineCore.outputText( 'her belly bloated and gurgling with every step', false );
		} else {
			EngineCore.outputText( 'her body double-stuffed with seed', false );
		}
		EngineCore.outputText( '.  You slump back and breathe heavy, feeling like you\'ve just run a marathon.\n\n', false );
		EngineCore.outputText( 'It is some time until you gain the energy to rise.', false );
