...
angular.module( 'cocjs' ).run( function( SceneLib, MainView, OnLoadVariables, PerkLib, ConsumableLib, kFLAGS, Utils, Descriptors, CoC, EngineCore ) {
	...
	BakeryScene.prototype.checkBakeryMenu = function() {
		...
		MainView.outputText( 'Fox Berry - 5 gems.\n' );
		MainView.outputText( 'Ringtail Fig - 5 gems.\n' );
	...
	BakeryScene.prototype.ingredientsMenu = function() {
		MainView.clearOutput();
		MainView.outputText( 'Also try our special ingredients in your own baking!\n' );
		MainView.outputText( 'Fox Berry - 5 gems.\n' );
		MainView.outputText( 'Ringtail Fig - 5 gems.\n' );
		MainView.outputText( 'Mouse Cocoa - 10 gems.\n' );
		MainView.outputText( 'Ferret Fruit - 20 gems.\n' );
		MainView.menu();
	...
	BakeryScene.prototype.talkToBaker = function() {
		MainView.clearOutput();
		MainView.outputText( 'The minotaur snorts as you approach him, but waves you into the kitchen.  "<i>What?</i>" he asks, patiently watching you.  "<i>Want to hear about baking?' );
		//(Maddie 1 completed);
		if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00242 ] >= 4 ) {
			MainView.outputText( '  Or you want special order?' );
		}
	...
	BakeryScene.prototype.talkAboutBrownies = function() {
		MainView.clearOutput();
		MainView.outputText( '"<i>Like our brownies?</i>" the baker asks.  "<i>Recipe been handed down from chef to chef for years.  Original maker invented it at an inn, for guests to carry in their lunchboxes.</i>"' );
	...
	BakeryScene.prototype.talkAboutCupcakes = function() {
		MainView.clearOutput();
		MainView.outputText( '"<i>Cupcakes take work,</i>" the baker intones, tilting his long face downward.  "<i>Need butter, sugar, flour, and eggs for batter; gotta mix long time and add stuff slowly.  Candied berries get cut up, put inside batter in little pieces.  Bake batter in a special pan.</i>"' );
	...
	BakeryScene.prototype.talkAboutMouseCocoa = function() {
		MainView.clearOutput();
		MainView.outputText( '"<i>Mouse cocoa comes from warm side of the lake, by forest border.  Like the name says, mouse people used to grow and eat a lot of it.  No mice left, though... hard to get now and expensive.  Have to buy it from the farmer at the lake; she sends out gathering parties.  Same one we get milk from.  Less and less every year... going to have to raise prices soon.  Ten gems for one handful, now.</i>"' );
		//[Yes][No];
		MainView.menu();
	...
	BakeryScene.prototype.buyCocoa = function() {
		...
		if( CoC.player.gems < 10 ) {
			MainView.outputText( 'You can\'t afford one of those!' );
			MainView.menu();
			EngineCore.addButton( 0, 'Next', this, this.ingredientsMenu );
			return;
		}
		MainView.outputText( 'You pay ten gems for some cocoa.  ' );
		CoC.player.gems -= 10;
	...
	BakeryScene.prototype.buyFerretFruit = function() {
		...
		MainView.outputText( 'You pay twenty gems for a single ferret fruit.  ' );
		CoC.player.gems -= 20;
		MainView.statsView.show();
	...
	BakeryScene.prototype.buyFig = function() {
		...
		MainView.outputText( 'You pay five gems for a fig.  ' );
		CoC.player.gems -= 5;
		MainView.statsView.show();
	...
	BakeryScene.prototype.talkBakeryMenu = function() {
		MainView.clearOutput();
		MainView.outputText( 'Who will you talk to?\n' );
		var rubiT = 'Waitress';
	...
	BakeryScene.prototype.nomnomnom = function( name, price ) {
		...
		if( CoC.player.gems < CoC.flags[ kFLAGS.TEMP_STORAGE_PASTRY_PRICE ] ) {
			MainView.outputText( 'You don\'t have enough gems to order that!', false );
		...
		if( CoC.flags[ kFLAGS.TEMP_STORAGE_PASTRY_NAME ] === 'eclair' ) {
			MainView.outputText( 'You hand over 10 gems and ask for the \'special eclair\'.  The centaur working the counter smirks ', false );
			if( CoC.player.tallness <= 52 ) {
				MainView.outputText( 'down ', false );
			} else if( CoC.player.tallness >= 84 ) {
				MainView.outputText( 'up ', false );
			}
			MainView.outputText( 'at you gives pulls a cream-filled pastry from a box concealed behind the counter.  It\'s warm... so very warm, and you try to steady your hands as you walk off to towards a table, sniffing in deep lungfuls of its \'special\' scent.  The first bite is heaven, sating a craving you didn\'t even know you had.  You can\'t stop yourself from moaning with delight as you drain every drop and finish off the sweet doughnut shell.  The minotaur goo is all over your fingers, but you don\'t mind licking them all clean.  With the lust now you now feel burning inside you, you even try to make a show of it.  Though you make a few ', false );
			if( CoC.player.femininity >= 75 ) {
				MainView.outputText( 'males fill their pants', false );
			} else if( CoC.player.femininity <= 25 ) {
				MainView.outputText( 'females squirm', false );
			} else {
				MainView.outputText( 'other patrons squirm and fill out their pants', false );
			}
			MainView.outputText( ', none of them tries to make a move.  Pity.', false );
			EngineCore.dynStats( 'lus', (20 + CoC.player.lib / 10) );
			CoC.player.minoCumAddiction( 10 );
		} else {
			MainView.outputText( 'You hand over ' + Utils.num2Text( CoC.flags[ kFLAGS.TEMP_STORAGE_PASTRY_PRICE ] ) + ' gems and get your ' + CoC.flags[ kFLAGS.TEMP_STORAGE_PASTRY_NAME ] + '.  A moment later you\'re at a table, licking the sugary residue from your fingertips and wondering just how they make the food so damned good.', false );
			if( CoC.flags[ kFLAGS.TEMP_STORAGE_PASTRY_NAME ] === 'doughnuts' ) {
				MainView.outputText( CoC.player.modTone( 0, 2 ), false );
				MainView.outputText( CoC.player.modThickness( 100, 1 ), false );
				...
				MainView.outputText( CoC.player.modTone( 0, 1 ), false );
				MainView.outputText( CoC.player.modThickness( 100, 2 ), false );
				if( Utils.rand( 3 ) === 0 && CoC.player.hipRating < 20 ) {
					MainView.outputText( '\n\nAfter finishing, you find your gait has changed.  Did your hips widen?', false );
					CoC.player.hipRating++;
				}
			} else if( CoC.flags[ kFLAGS.TEMP_STORAGE_PASTRY_NAME ] === 'brownies' ) {
				MainView.outputText( CoC.player.modThickness( 100, 4 ), false );
	...
	BakeryScene.prototype.buySlutCake = function() {
		...
		if( CoC.player.gems < 500 ) {
			MainView.outputText( 'You don\'t have enough gems for one of those!', false );
			//EngineCore.doNext( this, this.bakeryuuuuuu);;
			MainView.menu();
			EngineCore.addButton( 0, 'Next', this, this.checkBakeryMenu );
			return;
		}
		MainView.outputText( 'The minotaur chef emerges from the backroom bearing a box that contains your cupcake.  It\'s too big to scarf down immediately.\n\n', false );
		CoC.player.gems -= 500;
		MainView.statsView.show();
	...
	BakeryScene.prototype.buyFoxBerry = function() {
		...
		MainView.outputText( 'You pay five gems for a fox berry.  ' );
		CoC.player.gems -= 5;
		MainView.statsView.show();
	...
	BakeryScene.prototype.easterBakeSaleHelp = function() {
		...
		if( CoC.player.biggestTitSize() >= 1 ) {
			MainView.outputText( 'Your [fullChest] bounce at the vigor of your movement.  ' );
		}
		MainView.outputText( 'Going over the egg like an elaborate puzzle with its secrets only limited by your ability to unlock them, you are delighted to feel a stir of movement from within.  The heat is leaving not only your hands, but the entire room now, bringing the bristling heat down until you\'re sure it\'s cooler in here than outside with the swarm of customers.' );
		...
		if( CoC.player.gender === 1 ) {
			MainView.outputText( '[eachCock] jumps to full hardness.' );
		}
		//(if the pc is female);
		else if( CoC.player.hasVagina() ) {
			MainView.outputText( 'your nipples stiffening noticeably, while your [vagina] prepares for what\'s to come.' );
		}//(if the pc is a herm);
		else {
			MainView.outputText( '[eachCock jumping to full hardness, your nipples and [vagina] not far behind in getting ready for your encounter.' );
		}
	...
	BakeryScene.prototype.malesHelpOutWithEaster = function() {
		...
		if( CoC.player.cumQ() < 250 ) {
			...
			MainView.outputText( 'Your cock spews out a significant amount of seed, filling your partners deepness quickly while a small volume shoots out with some force.  You are happy to see that she seems to have gained a little weight from your baby-batter.' );
		}//(very large cum vol);
		else if( CoC.player.cumQ() < 5000 ) {
			MainView.outputText( 'Your cock spews into your partner\'s deepness, filling it almost instantly while a significant volume splatters out.  You are happy to see she seems to have gained a little weight from your baby-batter.' );
		} else {
			MainView.outputText( 'Your cock opens like a river, streaming into your partner with such force that her belly distends.  A spew begins to erupt from her vagina, empting the significant amount she could not take on to the floor.  You are happy to see she has gained some weight from your baby-batter.' );
		}
		MainView.outputText( '  It\'s about all you can do to get to the floor before passing out.  So much for helping.  In the back of your mind you picture the minotaur with a smug grin as your consciousness fades.' );
		MainView.outputText( '\n\n<b>Later...</b>' );
