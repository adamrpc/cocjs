...
angular.module( 'cocjs' ).run( function( SceneLib, MainView, PregnancyStore, AppearanceDefs, Utils, StatusAffects, kFLAGS, CoC, EngineCore ) {
	...
	KatherineThreesome.prototype.circlejerk = function() { //Not available if Kath has no cock
		...
		EngineCore.outputText( 'As soon as Urta’s top is off her, you move your hands to grope at her soft orbs, pinching and twisting her nipples, then pulling her head against your ' + (CoC.player.hasBreasts() ? CoC.player.breastDescript( 0 ) : 'chest') + ', and finally giving her perky fox-ear a gentle bite' );
		...
		EngineCore.outputText( '“<i>Is that a challenge, ' + CoC.player.short + '?  That you can get vixxy here to cum before she gets me to cum, hmm?</i>” Kath asks, grinning and showing needle-like teeth.  “<i>Well, since you went and got so obligingly naked for us, I’m gonna make this a little more challenging...</i>”  She reaches out, her fingers ' );
		if( CoC.player.hasCock() ) {
			EngineCore.outputText( 'wrapping around your cock' + (CoC.player.hasVagina() ? ' and ' : '') );
		}
		...
		if( CoC.player.hasVagina() ) {
			EngineCore.outputText( '   Her ' + (CoC.player.hasCock() ? 'other ' : '') + 'hand teases your labia by gently circling it, the fur tickling your entrance so deliciously, you can’t help but buck against her wandering finger, trying to capture it within your moist walls.' );
		}
		...
		EngineCore.outputText( '“<i>You cheated, ' + CoC.player.short + '!</i>” Urta complains.\n\n' );
		...
		if( CoC.player.hasCock() && CoC.player.hasVagina() ) { //Herm
			EngineCore.outputText( 'you\'re overcome.  Your pussy and hips lock up and you dribble femcum as Urta and Kath\'s efforts pay off.  You feel hot liquid rushing up from your ' + (CoC.player.balls > 0 ? 'balls' : 'prostate') + ', then an explosion...\n\n' );
		...
		EngineCore.outputText( 'The two morphic herms moan and growl lustily, eagerly rubbing their stiff, mismatched dicks across your stomach, your ' + (CoC.player.hasBreasts() ? CoC.player.breastDescript( 0 ) : 'chest') + ', your [hips]' );
		if( CoC.player.hasCock() ) {
			EngineCore.outputText( ' and especially against your ' + CoC.player.cockDescript() );
		}
	...
	KatherineThreesome.prototype.threeSixtyNine = function() {
		...
		if( CoC.player.cumQ() < 1500 ) {
			...
			EngineCore.outputText( 'Your overpowered ' + (CoC.player.balls > 0 ? 'balls produce' : 'prostate produces') + ' jet after boiling jet of cum.  ' );
		}
		if( doKath || SceneLib.katherine.hasCock() ) {
	...
	KatherineThreesome.prototype.roastYou = function() { //Not available if Kath has no cock
		...
		if( SceneLib.katherine.cockNumber > 1 ) {
			EngineCore.outputText( 's of her cocks press firmly against ' + (CoC.player.hasVagina() ? 'both your holes' : 'your ass') );
		} else {
			EngineCore.outputText( ' of her cock presses firmly against your ' + (CoC.player.hasVagina() ? 'cunt' : 'ass') );
		}
		EngineCore.outputText( '.\n\nKath pushes your legs apart with her knees and starts to force ' );
		if( SceneLib.katherine.cockNumber === 1 ) {
			EngineCore.outputText( 'her ' + SceneLib.katherine.cockLength + ' inch ' + SceneLib.katherine.cockType( 'doggy dick', 'cat cock' ) + ' into your ' + (CoC.player.hasVagina() ? 'pussy' : 'rectum') );
		} else if( CoC.player.hasVagina() ) {
		...
		EngineCore.outputText( ' for her coming deposit' + (CoC.player.hasVagina() && !CoC.player.isPregnant() ? ' and ensuring your uterus will be awash with her seed' : '') + '.\n\n' );
		if( CoC.player.hasVagina() ) {
	...
	KatherineThreesome.prototype.spitroastKath = function() { //Works for all except genderless characters
		...
		if( CoC.player.isTaur() ) {
			...
			EngineCore.outputText( 'her mouth is lined up with your ' + (CoC.player.hasCock() ? 'cock' : 'pussy') + '.  Your submissive pussy cat doesn’t need any instruction to ' + (CoC.player.hasCock() ? 'swallow the tip of your cock' : 'slam her tongue into the depths of your pussy') );
		}
		EngineCore.outputText( '.\n\nUrta thrusts and drives Katherine’s ' );
		if( CoC.player.hasCock() ) {
			EngineCore.outputText( 'lips down your shaft. ' + (CoC.player.longestCock() >= 12 ? ' Despite your length' : '') );
		} else {
			EngineCore.outputText( 'face into your groin. ' );
		}
		EngineCore.outputText( ' Kath takes it without complaint and her rough tongue does wonders for your mood and ' + (CoC.player.hasCock() ? 'member' : 'your moist cunt') + '.\n\n' );
		EngineCore.outputText( 'Urta really gets into it - she places your hands on her breasts and starts to fuck Kath as though it’s life or death.  ' + (CoC.player.isTaur() ? 'You’re not used to having a lover in front of you anymore and the added stimulation' : 'The rocking motion along with Katherine’s eager tongue') + ' brings your climax more quickly than you thought possible.\n\n' );
		if( !CoC.player.hasCock() ) {
		...
		if( SceneLib.katherine.hasCock() ) {
			EngineCore.outputText( 'A hot spray of cum lands on and around your ' + (CoC.player.isTaur() ? 'rear ' : '') + (CoC.player.isNaga() ? 'tail' : 'feet') + '.  ' );
		}
		...
		if( !CoC.player.hasCock() || CoC.player.cumQ() < 3000 ) {
			EngineCore.outputText( 'Sure enough you ' + (CoC.player.isTaur() ? 'feel Kath’s ass and back press against your belly and you realize' : 'watch her belly stretch and stretch until') + ' she’s so full her belly is resting against the floorboards' );
		} else {
	...
	KatherineThreesome.prototype.pinAndFuck = function() { //Kath is sober, Urta is drunk //Not available if Kath has no cock
		...
		if( CoC.player.isTaur() ) {
			...
			EngineCore.outputText( (CoC.player.isNaga() ? 'quickly coil your tail over' : 'kneel atop') );
		}
		...
		if( SceneLib.urta.pregnancy.isPregnant && SceneLib.urta.pregnancy.type !== PregnancyStore.PREGNANCY_BEE_EGGS && SceneLib.urta.pregnancy.type !== PregnancyStore.PREGNANCY_DRIDER_EGGS ) {
			...
			EngineCore.outputText( ' - she needs a womb full of seed.  Urta laughs and says “<i>' + (CoC.flags[ kFLAGS.URTA_FERTILE ] > 0 ? 'Yeah, fill me up, maybe you’ll get to be a daddy' : 'Oh please fill me up.  It’s so good') );
		}
	...
	KatherineThreesome.prototype.pinAndFuckMountUrta = function() { //Plays for anyone without a cock and for herms who select this option
		...
		EngineCore.outputText( 'Kath laughs and continues to gently fuck Urta.  You pull off Urta’s cock and wait for her massive load to finish gushing from your ' + (CoC.player.hasVagina() ? 'pussy' : 'ass') + ' before putting on your clothes.  Before you leave you give Urta a kiss and squeeze her belly a little.  Her only reply is to moan.  You tell Kath to take good care of Urta and then leave.  You know that if you stay much longer their display will get you excited enough that you’d stay around all afternoon and unfortunately you’ve got things you need to do.\n\n' );
		CoC.player.orgasm();
		CoC.flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ]++;
	...
	KatherineThreesome.prototype.pinAndFuckStuffKath = function() { //Plays for any males and for herms who select this option
		...
		if( CoC.player.cor < 25 ) {
			EngineCore.outputText( 'hold your cock' + (CoC.player.cocks.length > 1 ? 's' : '') + ' still for a moment, letting Kath adjust her hips so she’s in a more comfortable position.  You massage the base of her tail which draws even more moans of pleasure from Kath.  When you finally start to thrust again Kath weakly tries to push her hips back toward you and her pussy ' + (CoC.player.cocks.length > 1 ? 'and ass do their' : 'does its') + ' best to pull you in deeper.' );
		} else if( CoC.player.cor < 75 ) {
			EngineCore.outputText( 'start to fuck her slowly and gently but quickly build to thrusting hard and fast as your base instincts take over.  It doesn’t seem to matter to Kath, she moans and purrs happily as long as your cock' + (CoC.player.cocks.length > 1 ? 's are filling her holes.' : ' is filling her pussy.') );
		} else {
		...
		EngineCore.outputText( 'Your cock' + (CoC.player.cocks.length > 1 ? 's erupt inside Kath' : ' erupts inside Kath') );
		if( CoC.player.cumQ() < 500 ) {
			EngineCore.outputText( ', your load soaking into her ' + (CoC.player.cocks.length > 1 ? 'pussy and ass' : 'waiting pussy') + '.  Katherine just purrs and rolls slightly so she can kiss you.' );
		} else if( CoC.player.cumQ() < 1500 ) {
			EngineCore.outputText( 'and you feel ' + (CoC.player.cocks.length > 1 ? 'little lakes of cum forming around the tips of your cocks' : 'a little lake of cum forming around the tip of your cock') + '.  Her belly swells just a bit and Kath puts her hand on it, a smile on her lips.' );
		} else if( CoC.player.cumQ() < 3000 ) {
			...
			if( SceneLib.katherine.pregSize() > 0 ) {
				...
				EngineCore.outputText( 'She starts to look a little pregnant as her womb ' + (CoC.player.cocks.length > 1 ? ' and colon' : '') + ' fill with your seed.' );
			}
		} else {
			...
			if( SceneLib.katherine.pregSize() > 0 ) {
				...
				if( CoC.player.cocks.length > 1 ) {
					EngineCore.outputText( 'your lower shaft Kath’s vagina quickly reaches its limit and you feel all your cum running down your other cock' + (CoC.player.cocks.length > 2 ? 's, blasting all over the room.' : ' stuffing her colon with cum.') );
				} else {
					EngineCore.outputText( 'your supercharged ' + (CoC.player.balls > 0 ? 'balls' : 'prostate') + ' you quickly fill her cunt to its limit, the rest of your seed blasting back against your groin.' );
				}
			}
		...
		EngineCore.outputText( 'You lock the door on the way out, ' + (CoC.flags[ kFLAGS.KATHERINE_URTA_AFFECTION ] >= 31 ? 'knowing that with those two the sex option will eventually win out.' : 'hoping your sated girlfriends will bond over this.') );
		CoC.flags[ kFLAGS.URTA_TIME_SINCE_LAST_CAME ] = 2 + Utils.rand( 2 );
		if( CoC.time.hours >= 13 ) {
			CoC.flags[ kFLAGS.KATHERINE_LOCATION ] = SceneLib.katherine.KLOC_URTAS_APT; //Katherine.timeChange will sort out whether Kath actually stays with Urta
		}
	...
	KatherineThreesome.prototype.knothole = function() { //Kath is drunk, Urta is sober //Not available if Kath has no cock
		...
		if( CoC.player.isTaur() ) {
			...
			EngineCore.outputText( CoC.player.isNaga() ? 'slither in front of' : 'kneel in front of' );
		}
		EngineCore.outputText( ' Kath and present ' );
		if( CoC.player.cocks.length > 0 ) {
			EngineCore.outputText( (CoC.player.cocks.length > 1 ? 'one of your cocks' : 'your cock') + '.  Kath sucks on the tip eagerly,' );
		} else {
		...
		EngineCore.outputText( 'You start to scratch Kath’s sensitive ears and she goes wild.  She grabs your hips and pulls herself closer, ' + (CoC.player.cocks.length > 0 ? 'her teeth locked around the base of your cock' : 'like her tongue is trying to reach your cervix') + '.\n\n' );
	...
	KatherineThreesome.prototype.sandwichGetFucked = function() {
		...
		if( CoC.player.isTaur() ) {
			...
			EngineCore.outputText( CoC.player.isNaga() ? 'slide across the floor, tensing your tail so as to bring your [vagOrAss] level with her flared shaft' : 'turn around and bend over, presenting your ass' );
		}
		EngineCore.outputText( '.  Urta, already horny thanks to Kath’s ministrations, wastes no time.\n\n' );
		EngineCore.outputText( 'You feel her hands latch onto your ' + (CoC.player.isTaur() ? 'flanks' : 'hips') + ' and her meaty cock presses against your [vagOrAss].  You moan and try to open yourself for this invader, but Urta is in a hurry.  Her fingers dig into your sides and she reels you in, relentlessly feeding you inch after inch of her prick.  When she bottoms out you hear a deep sigh.  It must feel amazing to be both completely filled and completely filling someone at the same time.\n\n' );
		...
		if( SceneLib.katherine.cumQ() <= 1500 ) {
			EngineCore.outputText( 'grow warm as it fills with Katherine\'s seed.  Urta stops for just a moment, sighing contentedly, before she once again grips your ' + (CoC.player.isTaur() ? 'flanks' : 'hips') );
		} else {
			EngineCore.outputText( 'expanding against ' + (CoC.player.isTaur() ? 'your rump' : 'the small of your back') + '.  The air is forced from Urta’s lungs and you get pushed to the ground as both your lovers collapse against you.\n\nYou smile to yourself and flex ' + (CoC.player.hasVagina() ? 'the muscles in your pussy' : 'your sphincter') + ', milking Urta’s cock.  She may be packed with Kath’s sperm but Urta is still Urta.  She responds by gripping your ' + (CoC.player.isTaur() ? 'flanks' : 'hips') + ' once more' );
		}
	...
	KatherineThreesome.prototype.sandwichGetLicked = function() {
		...
		EngineCore.outputText( '.  Every time she thrusts her rack jiggles delightfully.  At the same time Urta’s expert tongue is ' + (CoC.player.hasCock() ? 'sliding around your cockhead' : 'busy teasing your folds') + ', nudging you towards an orgasm.\n\n' );
		...
		EngineCore.outputText( 'It’s the pleading look Urta gives you that sends you over the edge.  That and ' + (CoC.player.hasCock() ? 'the feeling of her whole throat clamping down on your member' : 'her nose tickling your clit while her tongue probes your depths.  You cum hard, soaking her face with fem-cum') + '.\n\n' );
		if( CoC.player.hasCock() ) {
			if( CoC.player.cumQ() < 500 ) {
				...
				EngineCore.outputText( 'You pump shot after shot down Urta’s throat.  With each blast you fire you feel her innards stretch more and more.  By the time your ' + (CoC.player.balls > 0 ? 'balls are' : 'prostate is') + ' empty there’s a distinct warmth and wetness in her throat.  Thank goodness she has no gag reflex.\n\n' );
				EngineCore.outputText( 'Urta doesn’t have a chance to recover' );
			} else if( CoC.player.cumQ() < 3000 ) {
	...
	KatherineThreesome.prototype.orgy = function() { //Both Kath and Urta are drunk
		...
		if( CoC.player.hasCock() ) {
			...
			EngineCore.outputText( CoC.player.hasBreasts() ? 'breasts' : 'ass' );
		}
		...
		if( valaHere ) {
			...
			if( CoC.flags[ kFLAGS.KATHERINE_VALA_TIMES_SEX ] > 0 ) {
				...
				EngineCore.outputText( '“<i>Hi ' + CoC.player.short + ',</i>” she says cheerfully, “<i>I see you brought a friend.  Did you save her too?</i>”\n\n' );
		...
		if( CoC.player.hasCock() ) {
			...
			EngineCore.outputText( CoC.player.hasVagina() ? 'licked you out' : 'fisted your ass' );
		}
		...
		EngineCore.outputText( 'She tosses a spermy bit of your clothing to you and says, “<i>You should get out of here ' + CoC.player.short + '.  Don’t you worry, I’ll take care of this.</i>”  Poking Katherine with her foot, she adds, “<i>Besides, Kath needs a bit more education on the subject of Watch paperwork.</i>”\n\n' );
	...
	KatherineThreesome.prototype.doubleStuffKath = function() { //Both Kath and Urta are drunk
		...
		EngineCore.outputText( 'Kath starts to laugh.  “<i>' + (SceneLib.katherine.hasCock() ? 'No room left for you' : 'Too slow' + CoC.player.short) + ',</i>” she says, “<i>but I bet we can still make you feel good.</i>”\n\n' );
		...
		EngineCore.outputText( 'Your two lovers trade you' + (CoC.player.hasCock() ? 'r cock' : '') + ' back and forth a few times until you feel that you’re about to cum.  ' );
		if( !CoC.player.hasCock() ) {
			EngineCore.outputText( 'There\'s no reason to fight it, you let go and femcum dribbles down ' + (CoC.player.isNaga() ? 'the scales of your tail' : 'your legs') + ' and washes over both Kath and Urta.  Your muscles give out and you collapse on the floor next to your panting, sweating lovers.' );
			...
			if( CoC.player.cumQ() < 500 ) {
				...
				if( CoC.player.cockTotal() > 2 ) {
					EngineCore.outputText( '  Your other cock' + (CoC.player.cockTotal() > 3 ? 's' : '') + ' produce a few squirts which coat both Kath and Urta’s bodies.' );
				}
				...
				if( CoC.player.cockTotal() > 2 ) {
					EngineCore.outputText( '  You realize that your free cock' + (CoC.player.cockTotal() > 3 ? 's' : '') + ' fired their streams all over the room, adding to the mess Urta will have to clean up later.' );
				}
				...
				if( CoC.player.cockTotal() > 2 ) {
					EngineCore.outputText( '  Your other cock' + (CoC.player.cockTotal() > 3 ? 's' : '') + ' release a torrent of cum, coating Kath, Urta, the floor and a few of the walls.  Though it’s messy you’re sure both your lovers are happy that at least some of your sperm didn’t wind up inside them.' );
				}
			...
			if( Utils.rand( 2 ) === 0 ) { //Urta takes it
				EngineCore.outputText( 'Urta had just swallowed you again, the ride down her throat setting you off.  You grip the back of her head and your ' + (CoC.player.balls > 0 ? 'balls start' : 'prostate starts') + ' pumping.  Her eyes go wide but she accepts her fate, trying to swallow your load as fast as you can produce it.\n\n' );
				EngineCore.outputText( 'Kath realizes what’s happened' + (CoC.player.balls > 0 ? ' and rubs your balls' : ', quicky inserts her finger into your ass and starts milking your prostate') + ', trying to coax an even bigger load out of ' + (CoC.player.balls > 0 ? 'them' : 'it') + '.  ' );
				if( CoC.player.cumQ() < 500 ) {
				...
				EngineCore.outputText( 'Kath’s licking and humming set you off, so you grip the back of her head.  Your ' + (CoC.player.balls > 0 ? 'balls start' : 'prostate starts') + ' pumping.  Her eyes go wide but she accepts her fate, trying to swallow your load as fast as you can produce it.\n\n' );
				EngineCore.outputText( 'Urta realizes what’s happened' + (CoC.player.balls > 0 ? ' and rubs your balls' : ', quicky inserts her finger into your ass and starts milking your prostate') + ', trying to coax an even bigger load out of ' + (CoC.player.balls > 0 ? 'them' : 'it') + '.  ' );
				if( CoC.player.cumQ() < 500 ) {
		...
		if( SceneLib.katherine.hasCock() ) {
			...
			if( CoC.player.cor < 33 ) {
				...
				EngineCore.outputText( CoC.player.cor < 66 ? '.  Despite having just cum you feel a little twitch from your cock.' : '.  It\'s sickly sweet, but who cares as long as they keep fucking like that.' );
			}
		}
		CoC.player.orgasm();
		SceneLib.katherine.orgasm();
	...
	KatherineThreesome.prototype.doublePenetrateKath = function() { //Both Kath and Urta are drunk
		...
		EngineCore.outputText( 'You watch the show, your cock beginning to strain against the confines of your clothes' + (CoC.player.hasVagina() ? ' as your pussy begins to wet them' : '') + '.  Katherine starts to tug at Urta’s clothes and despite her drunkenness she does a good job of removing them.\n\n' );
		EngineCore.outputText( 'Urta rips off the last few bits of her own clothing and then starts on Kath’s garments.  She looks over at you and says, “<i>Better hurry up ' + CoC.player.short + '.  I don’t think you want to get left out of this.</i>”  Just as she says the final word she pulls off the last of Kath’s clothes and cups both Kath’s ' + SceneLib.katherine.breasts.adj() + ' breasts in her hands.\n\n' );
		...
		EngineCore.outputText( 'Now it’s your turn.  You tighten your grip on Kath’s hips and jam your prick all the way in.  You hear a whimpered “<i>Yes</i>” from Kath before your ' + (CoC.player.balls > 0 ? 'balls start to empty their' : 'prostate starts to empty its') + ' contents.  ' );
		if( CoC.player.cumQ() < 500 ) {
			...
			EngineCore.outputText( 'You pump several streams of cum into Kath’s rectum.  With each shot you fire you feel her innards stretch more and more.  By the time your ' + (CoC.player.balls > 0 ? 'balls are' : 'prostate is') + ' empty you can’t feel the insides of her colon anymore, just an inner sea of your cum.' );
		...
		if( CoC.flags[ kFLAGS.KATHERINE_URTA_AFFECTION ] === 1 ) {
			...
			EngineCore.outputText( 'Urta gives you a big wet kiss and says, “<i>You must really like seeing me fill your friend.' + (CoC.flags[ kFLAGS.URTA_FERTILE ] === 1 ? '  Better be careful, I might just knock her up.' : '') + '</i>”\n\n' );
			...
			EngineCore.outputText( '“<i>Mmmm yes.  I’m always ready to cum inside a hot kitty like you,</i>” says Urta.  “<i>You know ' + CoC.player.short + ', you don’t <b>have</b> to get us wasted.  Kath and I are always willing to fuck.</i>”\n\n' );
	...
	KatherineThreesome.prototype.doubleStuffVala = function() { //Not available if Kath has no cock
		...
		if( CoC.player.cumQ() < 1500 ) {
			EngineCore.outputText( 'Your ' + (CoC.player.balls > 0 ? 'balls have' : 'prostate has') + ' to work much harder than usual to force your cum past Katherine’s engorged knot' + SceneLib.katherine.cockMultiple( '', 's' ) + '.  It’s somewhat painful, but after a while you can feel a small pool of your seed forming in Vala’s colon.\n\n' );
			EngineCore.outputText( 'Her belly stretches out' + (SceneLib.katherine.cumQ() > CoC.player.cumQ() ? ' - more from Kath’s contribution than yours.  It\'s' : ' - it\'s') + ' incredible that Vala can hold so much.  Even on a normal woman that belly would suggest a baby on the way but on Vala it’s massive.\n\n' );
		} else {
	...
	KatherineThreesome.prototype.fistKathAndVala = function() {
		...
		EngineCore.outputText( '“<i>' + CoC.player.short + '?</i>” Vala asks, “<i>aren\'t you going to... to... oh!</i>”  You never learn what she was trying to say.  Your talented fingers keep her busy until Kath decides she\'s tired of waiting.  She grabs Vala\'s head and pulls her in for another kiss.\n\n' );
