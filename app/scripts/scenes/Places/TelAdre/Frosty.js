...
angular.module( 'cocjs' ).run( function( SceneLib, PerkLib, MainView, kFLAGS, CoC, EngineCore ) {
	...
	Frosty.prototype.approachFrosty = function() {
		...
		if( CoC.flags[ kFLAGS.MET_FROSTY ] === 0 ) {
			...
			MainView.outputText( '\n\n“<i>Well, ' + CoC.player.mf( 'sir', 'madam' ) + ', I was thinking of a new business plan last night. You could... be a... beta tester for it. All you have to do is sign these agreement papers.</i>”' );
			...
			MainView.outputText( 'You go out to Frosty’s stand; she’s there manning it with a huge smile on her face. “<i>Hello there customer, how may I help you?</i>” You greet her with your own smile and hello and take a look at her menu:\n' );
			MainView.outputText( '1. Cupcake x 1 = 15gems\n' );
			MainView.outputText( '2. Cupcake x 5 = 60gems\n' );
			MainView.outputText( '3. Cupcakes x 10 = 110gems\n' );
			MainView.outputText( '4. Cupcakes x 20 = 200gems\n' );
	...
	Frosty.prototype.cupcakeBuy = function( arg ) {
		...
		MainView.clearOutput();
		MainView.outputText( 'You make your purchase and swiftly devour the results. They\'re surprisingly delicious!' );
		//15;
		if( arg === 1 ) {
			CoC.player.gems -= 15;
		}
	...
	Frosty.prototype.yesIWantYourFreeSampleYouFuckingDiseasedCupcakeSlut = function() {
		MainView.clearOutput();
		MainView.outputText( 'You follow Frosty to her booth. She goes behind the counter and spins the menu with her fingertip and stops it with the palm of her hand. She goes back to her business while you look over the menu.  The lettering and style of it has changed into something more appealing and seductive.' );
	...
	Frosty.prototype.getAFuckingFuckContractFromFucks = function() {
		MainView.clearOutput();
		MainView.outputText( 'Frosty zooms around the counter, appearing next to you like a pink blur “<i>OH! You changed your mind!?</i>” she asks excitedly with her hands held up to her face like she’s praying.' );
		//[Yes [No];
		//[Yes] ;
	...
	Frosty.prototype.noIDontWantToSignYourFuckingContractGoddamned = function() {
		MainView.clearOutput();
		MainView.outputText( 'Frosty lets out a discouraged “<i>Oh</i>” and slowly walks back to her side of the counter.' );
	...
	Frosty.prototype.frostysLimitedSpecialMenu = function() {
		MainView.clearOutput();
		MainView.outputText( 'Frosty spins the menu with a fingertip and stops it with the palm of her hand. She goes back to her business while you look over the menu.  The lettering and style of it has changed into something more appealing and seductive Hand – 5 points\n2. Mouth - 15 points\nSPECIAL: Eat Me Out - 1 point\n\n<b>Current points: ' + this.frostyPoints() + '</b>' );
		this.frostySpecialsMenu();
	};
	Frosty.prototype.frostyPoints = function( arg ) {
		if( arg === undefined || arg === 0 ) {
			return CoC.flags[ kFLAGS.FROSTY_POINTS ];
		}
		//If any change, add it in.;
		CoC.flags[ kFLAGS.FROSTY_POINTS ] += arg;
		//Make sure it aint negative;
		if( CoC.flags[ kFLAGS.FROSTY_POINTS ] < 0 ) {
			CoC.flags[ kFLAGS.FROSTY_POINTS ] = 0;
		}
		return CoC.flags[ kFLAGS.FROSTY_POINTS ];
	};
	//[Hands];
	Frosty.prototype.frostysHandsAreColdHolyShitWhyDontYouWarmTHoseMitsUp = function() {
		...
		if( CoC.player.hasCock() && !CoC.player.hasVagina() ) {
			...
			MainView.outputText( '\n\n“<i>Ooooh, your ' );
			if( CoC.player.biggestCockLength() < 7 ) {
				MainView.outputText( 'little' );
			} else {
				MainView.outputText( 'big' );
			}
			MainView.outputText( ' friend is just waiting for me to milk his cream out. Tee-hee.</i>”' );
		...
		//[If player is Herm];
			...
			MainView.outputText( '“<i>Okay, one stroke-and-poke comin’ right up</i>” she snickers at her own name for using her hand to pleasure a herm. She ducks under the table and pulls up a big, fancy, cushioned chair that looks like it belongs to a noble and also shouldn’t be able to fit in the tiny space under her booth.' );
	...
	Frosty.prototype.useFrostysMouth = function() {
		...
		//[If player is Herm];
			...
			MainView.outputText( '“<i>One banana split sundae special coming right up.</i>” You roll your eyes at the comment as Frosty blurs away her display then vaults over the counter with ease and is suddenly behind you. You quickly turn around just as she pushes your backside against her stand and falls to her knees. She’s about to pull the lower half of your [armor] off but decides not to. You wonder what she’s going to do as she stretches her jaw and sticks out her tongue for a few minutes. After she’s done with that, she cracks her neck and looks at your pants, like it’s a challenge. Then, in an instant, she’s able to undo your [armor] with just her tongue and teeth and has you standing waist-down naked. Now you have second thoughts of having any of your naughty bits near that mouth.' );
	...
	Frosty.prototype.genderlessCatchAllForFrosty = function() {
		...
		//[If player selects any option besides sex and is Gender-less repeatable];
			...
			MainView.outputText( '“<i>Sorry customer, you need a cock or a pussy or BOTH if I’m to do that!</i>” she sings in a cheesy jingle tone that makes you go, “<i>ughhhhhh.</i>”' );
		}
		//[Back to Frosty’s Special Menu.];
		this.frostySpecialsMenu();
	};
	...
	/*Anal (One service genderless can have) -WIP-
	 ...
	 MainView.outputText('“<i>So you wanna be the pitcher or the catcher?</i>” she ask with devilish smile. What the hell is that suppose to mean?');
	 //[pitcher?]  [catcher?];
	 } else {
	 ...
	 if(CoC.player.cockTotal() === 1) output('its');
	 else {
	 output('their');
	 }
	 MainView.outputText(' prision, the hot mass getting tangled in the curly hair draping over her back. It’ll take some maneuvering from the both of you but eventually your cock-head makes contact with her tight pink hole. Frosty take a deep breath and starts easing her-self down your ' + Descriptors.cockDescript(x) + ". It takes her only a few moment for your cock to  be snugly secured in her insides. She stands up straight and the grip around your cock becomes tighter. She breath out “This is like the only service where I can man the shop and pleasure a customer, a bit exhausting but business is business.”
	 ...
	 You bend over the counter and present your [buttDescript] to the fake herm nymph. She snaps her fingers once more and the strap-on spring backs to attention. You feel her spread your cheeks aparts, getting a better view of your (Descriptors.assholeDescript) She manuervers her hips to line the flared head up to your pucker.You feel it’s cold material lightly pushing brushing against your back-hole.  Knowing of what behind that small cold feeling causes your
	 [If player has just cock ]
	 ...
	 It moves only few inches on your onsides. Your mind starts racing with pleasure as Frosty’s other hand wanders over your [allBreastsDescript] and pinches on your [Descriptors.nippleDescript(0)], pulling and twisting the nip and taking nice hard grabs at your [ Descriptors.breastDescript(0)] A familiar build-up forms down in your crotch, you start pushing back against Frosty’s “cock” to hurry up the climax and start pinching your other [Descriptors.nippleDescript(0)] minicing the same action as the torturous nymph does, abusing the sensitive bud all in the name of sexaul pleasure.
	 ...
	 [else]
	 Frosty makes a satisfied sigh and thrusted her hip to make sure you satisfied. She releases your hair, your head thudding against the countertop. She snaps her fingers again and the toy shrivels out of your (Descriptors.assholeDescript) and dangels back to between her ankles, leaving your ass-hole gaping wide open.
	 ...
	 *WHACK* *WHACK *WHACK* *WHACK*
	 ...
	 Random Event
	 (Met Maddie and Frosty, be at the bakery at 20)
