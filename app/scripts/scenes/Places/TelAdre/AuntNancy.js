...
angular.module( 'cocjs' ).run( function( kFLAGS, AppearanceDefs, Utils, Descriptors, CoC, EngineCore ) {
	...
	AuntNancy.prototype.auntNancy = function( display ) {
		if( CoC.getInstance().time.hours >= 6 && CoC.getInstance().time.hours <= 14 ) {
	...
	AuntNancy.prototype.interactWithAuntNancy = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00264 ] >= 30 && CoC.getInstance().time.hours === 14 && CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00266 ] === 0 ) {
		...
		else if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00264 ] >= 30 && CoC.getInstance().time.hours >= (14 - (CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00264 ] / 30)) && CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00266 ] > 0 ) {
		...
		//If Relationship with Nancy < 30;
			...
			if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00263 ] < 1 ) {
	...
	AuntNancy.prototype.strongStuff = function() {
		...
		if( CoC.getInstance().player.tou >= 30 ) {
			EngineCore.outputText( 'It was good, but it seems to have gone straight to your head with surprising speed, giving you a pleasant buzz after only one glass. It might be a prudent idea to walk it off before you get another.  You thank Aunt Nancy for the drink, before climbing to your ' + CoC.getInstance().player.feet() + ' and leaving.  She eyes you respectfully as you go.', false );
			//[+10 Relationship with Aunt Nancy];
			CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00264 ] += 10;
		}
		//[Else];
		else {
	...
	AuntNancy.prototype.lightStuff = function() {
		...
		//[If FirstTalkNancy = 0];
		if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00265 ] === 0 ) {
			CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00265 ]++;
	...
	AuntNancy.prototype.timeForAuntNancySpiderCooch = function() {
		...
		if( CoC.getInstance().player.biggestTitSize() >= 1 ) {
			EngineCore.outputText( ' and taking the opportunity to grope your ' + CoC.getInstance().player.allBreastsDescript() + ' right through your ' + CoC.getInstance().player.armorName, false );
		}
		EngineCore.outputText( '.\n\n', false );
		...
		CoC.getInstance().player.lust += 29;
	...
	AuntNancy.prototype.auntNancyPoonPartII = function() {
		...
		EngineCore.outputText( 'You try to climb back up, only to find that your flesh and ' + CoC.getInstance().player.armorName + ' alike are caught fast against the adhesive matter of the net, which, you suddenly realize, is actually a thick web, spread across the house\'s floor. You feel a surge of panic and begin to pull and struggle, when suddenly you feel a smooth, cool hand on your face. Your night vision is beginning to recover, and you make out Aunt Nancy\'s face, her pale skin seeming to glow in the gloom, as she leans over you, staring into your eyes with hers, one finger trailing across your face before covering your lips in a gesture for quiet.\n\n', false );
		...
		//[If PillowTalk = 0];
		if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00266 ] === 0 ) {
		...
		EngineCore.outputText( '.\n\n', false );
		EngineCore.outputText( 'As you unwind at her touch, her many arms are working to strip away your ' + CoC.getInstance().player.armorName + ', gently peeling them away with care until you are naked and needy in the spider-lady\'s web.\n\n', false );
		...
		if( CoC.getInstance().player.balls > 0 ) {
			...
			if( CoC.getInstance().player.ballSize < 15 ) {
				if( CoC.getInstance().player.ballSize > 5 && CoC.getInstance().player.ballSize < 15 ) {
		...
		if( CoC.getInstance().player.hasCock() ) {
			...
			if( CoC.getInstance().player.cockTotal() >= 2 ) {
				if( CoC.getInstance().player.cocks[ y ].cockLength >= 22 ) {
					...
					if( CoC.getInstance().player.cocks[ y ].cockLength >= 30 ) {
			...
			if( !titFucked && CoC.getInstance().player.biggestLactation() >= 1 ) {
				EngineCore.outputText( 'As milk leaks slowly from your ' + Descriptors.nippleDescript( 0 ) + 's, the spider-lady notices.  She lowers her head and wraps her soft, lovely lips around the tip of one of your ' + CoC.getInstance().player.allBreastsDescript() + ', sucking gently and frantically flicking her tongue against your ' + Descriptors.nippleDescript( 0 ) + '.  You moan and lie back into the web, the multitude of sexual pleasures assaulting your senses almost more than you can bear as Aunt Nancy begins swallowing up your milk.\n\n', false );
			}
			...
			if( !titFucked || (titFucked && CoC.getInstance().player.cockTotal() <= 2) ) {
		...
		//[If player has vagina but no dicks: (Nancy prefers dicks and will always choose them over lesbian sex)];
			...
			EngineCore.outputText( 'Suddenly, you realize that your left hand is now free, courtesy of one of Aunt Nancy\'s smooth, gentle arms.  She whispers, quietly, "<i>A little for me too, please.</i>"  Realizing exactly what she means, you press the ' + CoC.getInstance().player.skinFurScales() + ' of your hand against her pussy.  Its entrance, two parted shell-plates dripping wet fluid, would almost be scary if you weren\'t so very relaxed.  Gently, your press your fingers into her body, and hear her moan, loudly, feel her own hands scrabbling inside of your ' + Descriptors.vaginaDescript( 0 ) + ' and ' + Descriptors.assholeDescript() + ', pawing your chest and ' + Descriptors.buttDescript() + ', trailing through your ' + Descriptors.hairDescript() + '.\n\n', false );
		...
		//[If the PC has no genitals at all:];
			...
			EngineCore.outputText( 'Suddenly, you realize that your left hand is now free, courtesy of one of Aunt Nancy\'s smooth, gentle arm.  She whispers, quietly, "<i>A little for me too, please.</i>"  Realizing exactly what she means, you press the ' + CoC.getInstance().player.skinFurScales() + ' of your hand against her pussy.  Its entrance, two parted shell-plates dripping wet fluid, would almost be scary if you weren\'t so very relaxed.  Gently, your press your ingers into her body, and hear her moan, loudly, feel her own hands scrabbling inside of your ' + Descriptors.assholeDescript() + ', pawing your chest and ' + Descriptors.buttDescript() + ', trailing through your ' + Descriptors.hairDescript() + '.\n\n', false );
			EngineCore.outputText( 'It feels wonderful.  You lie back and moan, enjoying the smooth, silky insides of her pussy, feeling tiny feelers occasionally caress and pull at your fingers inside of her.\n\n', false );
			//[If PC is lactating:];
			if( CoC.getInstance().player.biggestLactation() >= 1 ) {
				EngineCore.outputText( 'As milk leaks slowly from your ' + Descriptors.nippleDescript( 0 ) + 's, the spider-lady notices. She lowers her head and wraps her soft, lovely lips around the tip of one of your ' + CoC.getInstance().player.allBreastsDescript() + ', sucking gently and frantically flicking her tongue against your ' + Descriptors.nippleDescript( 0 ) + '.  You moan and lie back into the web, the multitude of sexual pleasures assaulting your senses almost more than you can bear as Aunt Nancy begins swallowing up your milk.\n\n', false );
	...
	AuntNancy.prototype.auntNancyPoonPartIII = function() {
		...
		if( CoC.getInstance().player.hasCock() ) {
			...
			if( CoC.getInstance().player.cocks[ y ].cockLength >= 22 && CoC.getInstance().player.cockTotal() > 1 ) {
			...
			if( CoC.getInstance().player.cockTotal() > 2 ) {
				...
				if( CoC.getInstance().player.cockTotal() > 1 ) {
					if( (CoC.getInstance().player.cocks[ y ].cockLength < 22 && CoC.getInstance().player.cockTotal() === 2) || (CoC.getInstance().player.cocks[ y ].cockLength >= 22 && CoC.getInstance().player.cockTotal() === 2) ) {
					...
					else if( (CoC.getInstance().player.cocks[ y ].cockLength < 22 && CoC.getInstance().player.cockTotal() > 2) || (CoC.getInstance().player.cocks[ y ].cockLength >= 22 && CoC.getInstance().player.cockTotal() > 2) ) {
			...
			if( CoC.getInstance().player.hasVagina() ) {
				...
				if( CoC.getInstance().player.balls === 0 && (CoC.getInstance().player.cocks[ y ].cockLength < 22 || (CoC.getInstance().player.cocks[ y ].cockLength >= 22 && CoC.getInstance().player.cockTotal() <= 2)) ) {
			...
			if( CoC.getInstance().player.cockTotal() >= 2 && CoC.getInstance().player.cumQ() >= 700 ) {
		...
		//[Else:];
			...
			if( CoC.getInstance().player.wetness() >= 5 || CoC.getInstance().player.biggestLactation() >= 1 ) {
	...
	AuntNancy.prototype.auntNancyPoonPartIV = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00266 ] === 0 ) {
			...
			EngineCore.outputText( 'The spider-lady pulls her head back, releasing your body with her four arms, and says, rather loudly, "<i>Now, friend, get on out of here!  Go out there and get to the adventuring and world-saving!</i>"  Suddenly, mischeviously, she pinches you rather hard on the ' + Descriptors.buttDescript() + '.  You jump, yelping in pain, and turn indignantly to face her as she doubles over, chuckling, tears still streaming down her face.  "<i>Let these old exoskeletal chitins rest awhile, friend,</i>" Aunt Nancy laughs, lying back onto the soft mound of webbing.  She smiles lazily up at you.  "<i>There\'s water next to the foyer if you want to clean off, and your ' + CoC.getInstance().player.armorName + ' in the parlor.  And... please do keep in touch.</i>"\n\n', false );
