...
angular.module( 'cocjs' ).run( function( ImageManager, StatusAffects, kFLAGS, Descriptors, CoC, EngineCore ) {
	function Brooke() {
	}
	...
	Brooke.prototype.brookeAffection = function( arg ) {
		CoC.getInstance().flags[ kFLAGS.BROOKE_AFFECTION ] += arg === undefined ? 0 : arg;
		if( CoC.getInstance().flags[ kFLAGS.BROOKE_AFFECTION ] > 100 ) {
			CoC.getInstance().flags[ kFLAGS.BROOKE_AFFECTION ] = 100;
		} else if( CoC.getInstance().flags[ kFLAGS.BROOKE_AFFECTION ] < 0 ) {
			CoC.getInstance().flags[ kFLAGS.BROOKE_AFFECTION ] = 0;
		}
		if( (CoC.getInstance().player.isTaur() || CoC.getInstance().flags[ kFLAGS.BROOKE_SHOWERED_WITH ] === 0) && CoC.getInstance().flags[ kFLAGS.BROOKE_AFFECTION ] > 20 ) {
			CoC.getInstance().flags[ kFLAGS.BROOKE_AFFECTION ] = 20;
		}
		return CoC.getInstance().flags[ kFLAGS.BROOKE_AFFECTION ];
	};
	...
	Brooke.prototype.meetBrookeFirstTime = function() {
		...
		if( CoC.getInstance().player.isNaga() ) {
			EngineCore.outputText( 'coil your tail nervously under yourself' );
		} else {
			EngineCore.outputText( 'shuffle your [feet]' );
		}
		EngineCore.outputText( ', and say that you\'re just about to shower yourself.  She steps aside, dropping the towel off her head and wrapping it around her body (almost teasingly), allowing you to pass.  "<i>Plenty of stalls to pick from,</i>" she laughs, and you thank her, walking on and avoiding eye contact.' );
		...
		CoC.getInstance().flags[ kFLAGS.BROOKE_MET_TODAY ] = 1;
		CoC.getInstance().flags[ kFLAGS.BROOKE_MET ] = 1;
		EngineCore.doNext( CoC.getInstance().scenes.camp.returnToCampUseOneHour );
	};
	Brooke.prototype.repeatChooseShower = function() {
		EngineCore.clearOutput();
		if( CoC.getInstance().time.hours < 16 || CoC.getInstance().time.hours > 18 || CoC.getInstance().player.tone < 30 ) {
			...
			else if( CoC.getInstance().flags[ kFLAGS.BROOKE_MEDIUM_SCENE ] === 0 ) {
		...
		else if( CoC.getInstance().flags[ kFLAGS.BROOKE_GRUMPS_ABOUT_TA ] === 0 && CoC.getInstance().player.tone >= 30 ) {
			this.brookeThirdEncounter();
		}
		//Affection <= 19;
		else if( this.brookeAffection() <= 19 ) {
			this.lowAffectionBrookeMeeting();
		} else if( this.brookeAffection() <= 39 && CoC.getInstance().flags[ kFLAGS.BROOKE_SHOWERED_WITH ] === 0 ) {
			this.mediumLowBrookeAffectionOneTime();
		}
		//Affection >=40, after first-time sex;
		else if( this.brookeAffection() >= 40 && CoC.getInstance().flags[ kFLAGS.BROOKE_MEDIUM_SCENE ] > 0 ) {
			this.mediumBrookeAffectionMeetingAfterSex();
		} else if( this.brookeAffection() >= 40 && CoC.getInstance().flags[ kFLAGS.BROOKE_MEDIUM_SCENE ] === 0 && CoC.getInstance().player.tone >= 50 && (CoC.getInstance().player.hasVagina() || CoC.getInstance().player.cockThatFits( this.brookeCapacity() ) >= 0) ) {
	...
	Brooke.prototype.brookeThirdEncounter = function() {
		...
		this.brookeAffection( 5 );
		CoC.getInstance().flags[ kFLAGS.BROOKE_GRUMPS_ABOUT_TA ] = 1;
		EngineCore.doNext( CoC.getInstance().scenes.camp.returnToCampUseOneHour );
	};
	...
	Brooke.prototype.helpBrookeOut = function() {
		...
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( ' and being so close to such a woman, you find it difficult to keep [eachCock] from getting too excited' );
		}
		EngineCore.outputText( '.' );
		...
		if( CoC.getInstance().player.gender === 1 ) {
			EngineCore.outputText( 'not caring about the stirring in [eachCock] in the privacy of the stall' );
		}
		//[if {PC is female}];
		if( CoC.getInstance().player.gender === 2 ) {
			EngineCore.outputText( 'feeling a fire stoke in your [vagina] even under the constant water' );
		}
		if( CoC.getInstance().player.gender === 3 ) {
			EngineCore.outputText( 'your loins beginning to stir, each and every part of you enticed' );
		}
		EngineCore.outputText( ' as you replay the memory of the Shepherd girl\'s taut, firm ass swaying, just for you.' );
		...
		EngineCore.dynStats( 'lus', 15 + CoC.getInstance().player.lib / 20, 'resisted', false );
		CoC.getInstance().flags[ kFLAGS.BROOKE_SHOWERED_WITH ] = 1;
	...
	Brooke.prototype.mediumLowBrookeAffection = function() {
		...
		if( this.brookeAffection() >= 40 && this.brookeAffection() <= 70 && CoC.getInstance().flags[ kFLAGS.BROOKE_MEDIUM_SCENE ] > 0 ) {
	...
	Brooke.prototype.mediumBrookeAffectionMeetingAfterSex = function() {
		...
		EngineCore.addButton( 2, 'Her Bod', this.brookesRockinBod );
		if( this.brookeAffection() >= 40 && CoC.getInstance().flags[ kFLAGS.BROOKE_MEDIUM_SCENE ] > 0 ) {
			EngineCore.addButton( 3, 'Grope', this.gropeDatBrooke );
		}
	};
	//[=Working Out=];
	...
	Brooke.prototype.talkToBrookeAboutGymFolks = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.BROOKE_AND_HELIA_3SOME ] > 0 && CoC.getInstance().flags[ kFLAGS.BROOKE_AND_HELIA_3SOME ] < 4 ) {
		...
		else if( CoC.getInstance().flags[ kFLAGS.BROOKE_AND_HELIA_3SOME ] > 3 && CoC.getInstance().flags[ kFLAGS.BROOKE_AND_HELIA_3SOME ] < 7 ) {
		...
		else if( CoC.getInstance().flags[ kFLAGS.BROOKE_AND_HELIA_3SOME ] > 6 ) {
		...
		//[if {met Loppe}];
		if( CoC.getInstance().flags[ kFLAGS.LOPPE_MET ] > 0 ) {
	...
	Brooke.prototype.brookesRockinBod = function() {
		...
		EngineCore.outputText( '\n\n<i>"Ooh, keep talking sweet things, sweet thing,"</i> she moans, her eyes drifting dreamily closed.  <i>"I love it when a ' + CoC.getInstance().player.mf( 'guy', 'girl' ) + ' notices all my hard work."</i>  She stops, expecting you to keep admiring her body, when you remind her that you asked her a question first.  <i>"You did?  Oh, right!  Why I... yeah, why I work out.' );
	...
	Brooke.prototype.gropeDatBrooke = function() {
		...
		//[if (hasCock = true)];
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( '  You can only imagine what it could feel like to drive your [cock] between those cheeks, just inches away from you.  The thought makes [eachCock] rouse.' );
		}
		EngineCore.outputText( '  The tension is beginning to get a little unbearable and your resolve wavers.' );
		...
		if( CoC.getInstance().player.hasCock() ) {
			...
			EngineCore.outputText( 'your pelvis' );
			if( CoC.getInstance().player.hasVagina() ) {
				EngineCore.outputText( ' and dragging her wet tail enticingly through the vulva of your [vagina]' );
			}
		}
		EngineCore.outputText( '.  <i>"I\'m game.  Hope you\'re ready, though; you\'re gonna need more than a few laps around the track to keep up with me."</i>' );
		...
		EngineCore.menu();
		if( CoC.getInstance().player.hasCock() && CoC.getInstance().player.cockThatFits( this.brookeCapacity() ) >= 0 ) {
	...
	Brooke.prototype.penetrateBrooke = function() {
		...
		//[if (cocks >= 2)];
		if( y >= 2 ) {
			EngineCore.outputText( '.  Your ' + Descriptors.cockDescript( y ) + ' rubs up and down the crack of her ass lamely, though the way she clenches against you with every inward thrust makes every sensation worth the effort' );
		}
		//[if (isHerm = true);
		if( CoC.getInstance().player.gender === 3 ) {
			EngineCore.outputText( '.  Your [vagina] pulses longingly, jealous of the attention you\'re giving to Brooke\'s own, and hungers for something to suffocate the fire built inside it' );
		}
		EngineCore.outputText( '.  She moans appreciatively with you some more, loving the feel of your ' + Descriptors.cockDescript( x ) + ' in her.' );
		...
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( ' across your [sack] and' );
		}
		EngineCore.outputText( ' down your thighs that she\'s getting wet enough for the both of you all the while.' );
		...
		EngineCore.outputText( '.  You\'re not too far behind her, and with just a few more pumps, you\'re shooting your load deep into her, pumping and fucking the whole time, making sure to get it in deep.' );
		if( y >= 0 ) {
			EngineCore.outputText( '  Your ' + Descriptors.cockDescript( y ) + ' shoots its own load up and onto her lower back and the base of her tail, snug between the cheeks of her ass' );
		}
		//[if (cocks >= 3);
		if( CoC.getInstance().player.cockTotal() > 1 && y >= 0 ) {
			EngineCore.outputText( ' while [eachCock] sprays your jizz all over the place: the floor and the walls of the small stall in particular, but also on her ass, onto her tail, and down her legs.  Most of it washes into the drain beneath you, but it\'ll definitely take a bit of scrubbing for her to get really clean after this.' );
		}
		//[if (cocks = 2);
		if( CoC.getInstance().player.cockTotal() === 2 && y >= 0 ) {
			EngineCore.outputText( '.' );
		}
		if( CoC.getInstance().player.gender === 3 ) {
			EngineCore.outputText( '  Your [vagina] cums along with the rest of you, spurting your femcum down your legs, washing the inside of your thighs in your juices.  The combined pleasure between your genitals makes your knees buckle – if only you had something, or someone, fucking you too!' );
		}
	...
	Brooke.prototype.brookeAnal = function() {
		...
		//[if {cockFit >= 61};
		if( CoC.getInstance().player.cockArea( x ) >= 61 ) {
		...
		//[if {first time}];
		if( CoC.getInstance().flags[ kFLAGS.TIMES_IN_BROOKES_BUTT ] === 0 ) {
		...
		EngineCore.outputText( ', leaning her ass back and trapping your ' + Descriptors.cockDescript( x ) + ' between her asscheeks expertly.  You moan with pleasure while she glides her globes up and down, feeling your manhood rise and swell with each bob.  Your hands are already on her butt, clenching the skin hard together while you thrust in the space between her buttocks.  <i>"Well... all right.  But only because I like you so much."</i>' );
		...
		//[if {first time}];
		if( CoC.getInstance().flags[ kFLAGS.TIMES_IN_BROOKES_BUTT ] === 0 ) {
			EngineCore.outputText( '  You wonder – is this the first time she\'s ever had anal before?' );
		} else {
			EngineCore.outputText( '  She\'s tight as a vice, but you know she can take you.  She\'s just getting herself ready.' );
		}

		EngineCore.outputText( '\n\nThe ' + CoC.getInstance().player.cockHead( x ) + ' of your ' + Descriptors.cockDescript( x ) + ' rubs against her rosebud in anticipation' );
		if( CoC.getInstance().player.cockTotal() > 1 ) {
			EngineCore.outputText( ' while your other member entertains itself between her thighs, bumping and sliding pleasurably across her vulva' );
		}
		EngineCore.outputText( '.  She instinctively clenches her ass, and you give her a moment to relax before you stroke her there again.  She loosens, and, almost teasingly now, you prod gently at her hole, implying but not acting just yet.  She takes deep breaths, working to steady herself.  <i>"Okay,"</i> she says, looking over her shoulder to you with a smile.' );
		EngineCore.outputText( ImageManager.showImage( 'brooke-gym-male-analbrooke' ) );
		EngineCore.outputText( '\n\nYou plan to take it easy on her.  You realign yourself, pointing your ' + CoC.getInstance().player.cockHead( x ) + ' right at her hole, and slowly begin to push yourself in.  On reflex, she inhales sharply, pressing her upper body into the wall some more.  You reassure her and push in a little deeper, sliding a quarter of your ' + Descriptors.cockDescript( x ) + ' into her anus.  She gasps, but doesn\'t object.  Another push, sliding in to halfway.  A grunt, and she shuffles her feet slightly, getting more stability.  Another push, nearly to the base.  She gives out a whorish moan, and you wonder if she\'s at her comfort limit.  You push forward anyway, to your [sheath], and gasp in pleasure and sigh with relief when she doesn\'t object.' );
		...
		EngineCore.outputText( ', burying yourself into her voracious ass.  With her body climbs her voice: a few pumps more and her moans become wails, loud enough that you\'re sure someone else must have heard.' );
		if( CoC.getInstance().player.cor >= 45 ) {
			EngineCore.outputText( '  With any luck, that someone will come to investigate – you\'re sure it wouldn\'t take much convincing to start a party.' );
		}

		EngineCore.outputText( '\n\n<i>"Harder!"</i> she demands, thrusting her ass back onto you.  You\'re not sure how much harder you can go' );
		if( CoC.getInstance().player.cockTotal() >= 2 ) {
			EngineCore.outputText( '.  Any one of your dicks that\'s not inside her is battering painfully around her ass as she continues to hit you with it, making it harder to focus on fucking her this way' );
		}
		EngineCore.outputText( '.  <i>"<b>Harder!</b>"</i> she yells, almost viciously.  You just keep going, hoping that she won\'t notice you can\'t fuck her ass any harder; your orgasm isn\'t much farther off as it is, and you hope that Brooke\'s isn\'t either.  Her voice continues to rise even as she continues to make unreasonable demands, until finally, her words taper off into simple screams, and she orgasms, the wall, her inner thighs, and her lower stomach splattering with her cum.  Her anus squeezes down on your member, making it all the more difficult to keep up brutalizing her ass.' );
		...
		EngineCore.outputText( '; your cock stiffens' );
		if( CoC.getInstance().player.gender === 3 ) {
			EngineCore.outputText( '; your lonely pussy clenches at nothing' );
		}
		EngineCore.outputText( '; and you cum into her, blasting rope after rope of your jism inside her.' );
		if( CoC.getInstance().player.cockTotal() > 1 ) {
			EngineCore.outputText( '  Your second dick flails and erupts with your ' + Descriptors.cockDescript( x ) + ', spraying onto the wall in front of Brooke and across the lips of her sex.' );
		}
		//[if (cocks >= 3)];
		if( CoC.getInstance().player.cockTotal() > 2 ) {
			EngineCore.outputText( '  [EachCock] blasts in time with your ' + Descriptors.cockDescript( x ) + ', each of your dicks pointing in a different direction.  Your cum gets on everything: Brooke, yourself, the stall – some even reaches as high as the shower nozzle.' );
		}
		...
		//[if {first time}];
		if( CoC.getInstance().flags[ kFLAGS.TIMES_IN_BROOKES_BUTT ] === 0 ) {
			EngineCore.outputText( '<i>"I never knew anal could be such a thrill!  We\'ll have to do that again sometime!"</i>  You chuckle politely but deep down you\'re almost dreading your next anal session with her.  You\'ll have to build up your stamina some more to keep up with her.' );
		} else {
			EngineCore.outputText( '<i>"Thanks for putting up with me being such a bossy bitch, [name].  But it\'s your fault, you know.  You\'re the one who awakened the anal-hungry monster within me."</i>  You\'re aware of the beast you\'ve created, you tell her, but truth be told, since your first session, it\'s been a bit easier to keep up with her.  She\'s just that good of a workout partner.' );
		}
		...
		CoC.getInstance().flags[ kFLAGS.TIMES_IN_BROOKES_BUTT ]++;
		this.brookeAffection( 10 );
		EngineCore.doNext( CoC.getInstance().scenes.camp.returnToCampUseOneHour );
	};
	//[=Tribadism=];
	...
	Brooke.prototype.tribadism = function() {
		...
		//[if (isHerm = true);
		if( CoC.getInstance().player.gender === 3 ) {
			EngineCore.outputText( ' your [cock], impishly wishing it were put to use.  You have some other plans this time, though.' );
		} else {
			EngineCore.outputText( ' your ' + CoC.getInstance().player.skinFurScales() + '.  The tone of her ass almost makes you wish you had a dick to fuck her with, but you\'re sure she\'ll give you a hell of a ride all the same.' );
		}
		...
		if( CoC.getInstance().player.gender === 3 ) {
			EngineCore.outputText( ' and all the way to the base of your now-straining, pre-leaking cock' );
		}
		EngineCore.outputText( '.' );
		...
		if( CoC.getInstance().player.tone >= 50 ) {
			EngineCore.outputText( ', and she coos sexily, gripping and pulling at each of your toned, chiseled asscheeks, even giving one as hard a slap as her hand can manage' );
		}
		EngineCore.outputText( '.  You start to fuck against her in earnest, pressing and pumping, sliding and bucking against her, loving and living for the sounds and expressions she makes as you love her the way only another ' + CoC.getInstance().player.mf( 'woman', '\'woman\'' ) + ' can.' );
		...
		if( !CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( '.  Brooke moves her hands up to your tits, her hands groping onto your [chest], her palms pressing onto your [nipples] mindlessly, and her tongue lolls and flags with each buck up against her you make' );
		}
		//[if {is not a herm and is lactating};
		if( !CoC.getInstance().player.hasCock() && CoC.getInstance().player.lactationQ() >= 200 ) {
			EngineCore.outputText( '.  Your milk streams from your [chest] as Brooke puts more of her weak pressure on them: it sprays from between her fingers and drips down the back of her hands on across her wrists' );
		}
		if( !CoC.getInstance().player.hasCock() && CoC.getInstance().player.hasFuckableNipples() ) {
			EngineCore.outputText( '.  Feeling the texture of your [nipples] under her hands snaps her out of her reverie enough to think to stimulate you further by pressing into each with a thumb, fucking into and against you with each of your excited thrusts' );
		}
		EngineCore.outputText( '.' );
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( '  Brooke moves one hand to your [chest] and wraps her other around your stiff [cock], pumping it in time with your thrusts against her.' );
		}
		...
		if( CoC.getInstance().player.gender === 3 ) {
			EngineCore.outputText( '.  Your [cock] explodes along with its sister, spurting your cum high between your pressed bodies, coating the underside of your breasts and your stomachs, but mostly onto Brooke, who keeps your flow pointed towards her as she pumps you' );
		} else {
			EngineCore.outputText( '.  Brooke continues to drive against you as she mauls your breasts, using whatever strength is in her fingers to grope what she can, eager to reach her own peak not far off' );
		}
		EngineCore.outputText( '.  Brooke\'s moans rise and climb in her own ecstasy, until she climaxes along with you, returning the favor and spraying your crotch with her fluids, coating your [vagina] with her own gift.' );
		EngineCore.outputText( '\n\nYou continue to thrust against each other for a bit, but eventually you wind down' );
		//[if (isHerm = true)];
		if( CoC.getInstance().player.gender === 3 ) {
			EngineCore.outputText( ' and [eachCock] goes limp' );
		}
		EngineCore.outputText( ' from all the excitement.  Brooke\'s body shudders several times, feeling several miniature orgasms rip through her, until finally, she opens her eyes and meets yours.  Without a sound, she pulls forward, making out with you once more, shamelessly gliding her tongue over yours.  <i>"Leave it to a girl to know how to fuck another so well,"</i> she smiles, rubbing against you some more for emphasis' );
		if( CoC.getInstance().player.gender === 3 ) {
			EngineCore.outputText( '.  <i>"Lucky for me, we won\'t always have to choose."</i>  She gently strokes your waning erection some more, releasing you when the [cockHead] touches down on her skin limply' );
		}
		EngineCore.outputText( '.' );
	...
	Brooke.prototype.goDownOnBrooke = function() {
		...
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( 'and [eachCock] pulls forward, pointing towards her like a compass to magnetic north, leaking your pre at her scent and ready to fuck her, or something, anything.' );
		} else {
			EngineCore.outputText( 'and you find yourself salivating – such a viable, powerful specimen like herself has you wishing you had a dick to fuck her with.' );
		}
		...
		//[if (hasCock = true)];
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( '  [EachCock] has hardly gone unnoticed this whole while, still standing at attention and begging, absolutely pleading for some action.' );
		} else {
			EngineCore.outputText( '  Your own [vagina] stands out, puffy and needy against the cold air, and it\'s certainly not gone unnoticed.' );
		}
		EngineCore.outputText( '  Brooke offers to return the favor, more than once, but you decline each time, knowing that if she did, it\'d be a cycle between you two that would last for hours.  She understands, but promises you that she owes you one.' );
	...
	Brooke.prototype.getLaidByBrooke = function() {
		...
		EngineCore.outputText( 'You step closer into her, wrapping your left arm around her torso diagonally, so that your hand rests on her right hip, and you reach over her chest with your right hand, grasping her left boob.  You pull her into you, and she coos at the affection, enjoying your loving embrace, but doesn\'t say anything, allowing you to have your way with her for now.' );
		EngineCore.outputText( '\n\nYou roll your hips into her tight, toned ass, ' );
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( 'spearing your ' + Descriptors.cockDescript( x ) + ' between her cheeks wickedly, feeling it rise effortlessly against her soft fur and the smooth, strong muscles of her glutes' );
		}//[if (hasCock = false)];
		else {
			EngineCore.outputText( 'and with some clever maneuvering, you manage to wiggle her tail between your legs, and you feel the soft fur and the smooth, powerful muscles of her tail brush against your [vagina], stimulating you easily' );
		}
		EngineCore.outputText( '.  You kiss and nip at her neck, knowing just what she likes, and she moans, knowing that you like hearing her.' );
		...
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( 'your ' + Descriptors.cockDescript( x ) + ' continues to thrust and spear between her luscious ass, demanding attention be paid to it.  Brooke notices, and rocks her hips against you, squeezing your meat teasingly with each downstroke her ass makes on your ' + Descriptors.cockDescript( x ) );
		} else {
		...
		EngineCore.outputText( '.  <i>"Excited?"</i> she asks, grinning back at you.  You let your actions speak for themselves, and you hump against her butt some more, dragging out your pleasure.' );
		...
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( 'horndog' );
		} else {
			EngineCore.outputText( 'whore' );
		}
		EngineCore.outputText( ' sometimes, [name],"</i> she says, turning herself around in your arms.  She reaches down between you, ' );
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( 'stroking along [eachCock], hissing inwardly at what she finds, loving the way her fingers, hurt as they are, just manage to glide across your skin' );
		} else {
			EngineCore.outputText( 'stroking along your [vagina], grinning at how your lower lips just manage to wrap around her fingers like they were made for each other.  Her thumb drags across your [clit] intentionally, several times, and she smiles wider at every feminine grunt you make' );
		}
		EngineCore.outputText( '.  <i>"Feeling a little pent up, sweet thing?"</i> she asks innocently, her fingers working harder on your genitals.  <i>"It\'s okay.  Brooke\'s in a giving mood."</i>' );
		...
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( ' [eachCock], now fully erect and at attention, just for her' );
		} else {
			EngineCore.outputText( ' your [vagina], loving how you can hardly resist fucking your hips at nothing in anticipation' );
		}
		EngineCore.outputText( ', and, licking her drooling lips, leans in.' );
		//{male/herm scene}[if (cocks = 1);
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( ImageManager.showImage( 'brooke-gym-male-getLaid' ) );
			...
			if( CoC.getInstance().player.cockTotal() >= 3 ) {
				EngineCore.outputText( ' and all over the rest of her' );
			}
			EngineCore.outputText( '.' );
			...
			if( CoC.getInstance().player.cockTotal() <= 2 ) {
				EngineCore.outputText( 'enjoying the lips of your canine lover as they slide over [eachCock] and how she seems to enjoy herself so thoroughly' );
			} else if( CoC.getInstance().player.cockTotal() >= 3 ) {
				EngineCore.outputText( 'enjoying how she tries her damndest to make sure every little bit of you has enough attention and is pleased to the best of her ability, no matter how much effort she requires' );
			}
			EngineCore.outputText( '.' );
			if( CoC.getInstance().player.gender !== 3 ) {
				EngineCore.outputText( '\n\n' );
			...
			//[if (cocks >= 3)];
			if( CoC.getInstance().player.cockTotal() >= 3 ) {
				EngineCore.outputText( '.  Her hands are practically a blur, up and down on [eachCock] – she\'s pulled them each down and towards her, aiming them towards their proper target' );
			}
			//[if (isHerm = true)];
			if( CoC.getInstance().player.hasVagina() ) {
				EngineCore.outputText( '.  Her fingers are straight as can be, pistoning into you roughly while her thumb frigs against your [clit], and the feel of her enthusiastic fingers inside your [vagina], massaging and caressing your walls, has you seeing stars' );
			}
			EngineCore.outputText( '.  You\'re not entirely positive if she\'s giving you pleasure because she wants to, or if you\'re just some perverse toy for her to suck on.  Whatever the case, you\'re not complaining, that\'s for sure.  Your hands grip hard onto her head, harder than you intend to, but that seems only to spur her on.  It\'s not much longer until you cum from her expertise.' );
			//[if (cumQuantity <= 349)];
			if( CoC.getInstance().player.cumQ() <= 349 ) {
			...
			else if( CoC.getInstance().player.cumQ() <= 999 ) {
		...
		//{female scene};
			...
			EngineCore.outputText( ImageManager.showImage( 'brooke-gym-female-getLaid' ) );
			...
			if( CoC.getInstance().player.cor <= 49 ) {
				EngineCore.outputText( '  That would almost be embarrassing enough to turn you off.' );
			} else {
				EngineCore.outputText( '  You want this sexy bitch all for yourself, and you\'ll be damned if you\'re sharing.  At least not until you\'re done.' );
			}
			EngineCore.outputText( '  Brooke, though, giggles through her mouthful of quim and thrusts again, enjoying the strained efforts you\'re making to not cry out.  Her hand jills hard and fast against her own neglected pussy, and from her actions, she\'s maybe as close as you are.' );
		...
		if( CoC.getInstance().player.cockTotal() >= 3 ) {
			EngineCore.outputText( ', and letting the excess wash from her fur half-heartedly' );
		}
		EngineCore.outputText( '.  Moments pass in a completely undecipherable blur to you, and when you can finally make sense of your surroundings, Brooke\'s standing up and has her tongue in your mouth.' );
		...
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( ', snowballing what she hasn\'t swallowed quite yet' );
		}
		EngineCore.outputText( '.  She cradles you in her strong arms until she\'s done, and you pull away from the wall, still a bit wobbly.  <i>"Was it good for you too, sweet thing?"</i> she asks.  You barely nod.' );
	...
	Brooke.prototype.doubleDicked = function() {
		...
		var y = CoC.getInstance().player.cockThatFits2( this.brookeCapacity() );
		//if {all cockFits  >= 61};
		if( y < 0 || CoC.getInstance().player.cockThatFits( x ) >= this.brookeCapacity() ) {
		...
		EngineCore.outputText( '"</i> she mentions, almost casually, while you continue to masturbate yourself on her.  <i>"' );
		if( CoC.getInstance().player.isNaga() ) {
			EngineCore.outputText( 'Twice the dick means twice the fun... but twice the work, too.  I\'m up to the challenge, though, if you are.' );
		} else {
			EngineCore.outputText( 'Haven\'t actually seen anyone that\'s not half-snake sporting more than one.  Mind if I put \'em to the test?' );
		}
		EngineCore.outputText( '"</i>  She rocks her hips a little higher, brushing the ring of her anus across your ' + Descriptors.cockDescript( y ) + ' while driving your ' + Descriptors.cockDescript( x ) + ' further across her labia.' );
		//[if {not first time anal};
		if( CoC.getInstance().flags[ kFLAGS.TIMES_IN_BROOKES_BUTT ] > 0 ) {
			EngineCore.outputText( '  <i>"Let\'s just hope this doesn\'t turn me any more into a buttfucking freak, huh?"</i>' );
		} else {
			EngineCore.outputText( '  Brooke hesitates just a bit as she feels the heat of your dick against her asshole.  <i>"Well, first time for everything,"</i> she says to herself.' );
		}
		...
		if( CoC.getInstance().player.cockTotal() >= 3 ) {
			EngineCore.outputText( '  Your other dicks rise as well, though not as quickly as the first two, jealous that they were chosen instead.' );
		}
		...
		//[if {first time anal};
		if( CoC.getInstance().flags[ kFLAGS.TIMES_IN_BROOKES_BUTT ] === 0 ) {
			EngineCore.outputText( 'it\'s certainly not a workout she\'s accustomed to,' );
		} else {
			EngineCore.outputText( 'no matter how much she loves getting fucked in the butt, it\'s still not a usual thing for her,' );
		}
		EngineCore.outputText( ' and for every inch her ass takes, another simultaneously slides into her pussy, stuffing her fuller than she\'s ever felt.' );
		...
		if( CoC.getInstance().player.cockTotal() >= 3 ) {
			EngineCore.outputText( 'The rest of you lamentably stands between you, leaking their pre uselessly between your bodies, pleading for some attention themselves.  ' );
		}
		if( CoC.getInstance().player.gender === 3 ) {
			EngineCore.outputText( 'Your [vagina] clenches in time with its neighbor – you can easily imagine what it might feel like to have two dicks in you at once.  You almost wish for it while you\'re waiting for Brooke\'s go-ahead.  ' );
		}
		EngineCore.outputText( 'Brooke\'s face contorts, not in pain but in concentration, while she fidgets with her ass and lower body, adjusting to both of her holes being so totally full.  After minutes of her positioning and repositioning herself, her eyes finally meet yours.  <i>"Okay,"</i> she whispers, <i>"but... a bit gentler than what we\'re used to, okay?"</i>' );
		...
		if( CoC.getInstance().flags[ kFLAGS.TIMES_IN_BROOKES_BUTT ] === 0 ) {
			EngineCore.outputText( 'Brooke\'s breathing slowly starts to even out, and she starts to moan enjoyably.  Maybe she\'s starting to like it?' );
		} else {
			EngineCore.outputText( 'Brooke rocks her hips onto both of your cocks, alternating from taking one deeper than the other.  As much as she just loves sex in general, it\'s the alien feeling of being so full that keeps her from going any harder.' );
		}
		EngineCore.outputText( '  She gasps and jerks with your every thrust, and you grunt, shoving yourself' );
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( ' down to your [balls]' );
		}
		EngineCore.outputText( ' deep into her.  You can practically feel your two dicks rubbing against each other inside her with each thrust.' );
		...
		if( CoC.getInstance().player.cockTotal() >= 3 ) {
			EngineCore.outputText( '  Your extra equipment, painfully erect, squashes between your bodies, pointing right at the underside of her boobs and your own [chest].' );
		}
		EngineCore.outputText( '  You fuck her a bit more forcefully, but not any faster, for both your sakes.  She moans into you: her body\'s adjusted to the feel, and she\'s more receptive of your actions.' );
		...
		if( CoC.getInstance().player.cumQ() >= 10000 ) {
			EngineCore.outputText( ' – she\'s probably going to have trouble doing much of anything for a day or two until she \'digests\' her load' );
		}
		EngineCore.outputText( '.  Wearily, she pulls herself away, just enough to look you in the eye, and, without a moment, leans in to kiss you, tongue and all.' );
	...
	//CoC.getInstance().flags[kFLAGS.BROOKE_MEDIUM_SCENE] === 1;
	Brooke.prototype.mediumAffectionOneTimeEvent = function() {
		...
		//[if (hasCock = true)];
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( '  You feel [eachCock] beginning to wake from her proximity, and in just a few seconds, she\'ll know it.' );
		}
	...
	Brooke.prototype.brookeSpecialMediumSceneContinued = function() {
		...
		if( x >= 0 && !CoC.getInstance().player.hasVagina() ) {
			...
			var y = CoC.getInstance().player.cockThatFits2( this.brookeCapacity() );
			if( CoC.getInstance().player.cockTotal() === 2 ) {
				EngineCore.outputText( ' while your second phallus brushes higher, rubbing itself through the wet fur on her stomach between the gorge of her abs' );
			}
			//[if (cocks >= 3);
			if( y >= 0 && CoC.getInstance().player.cockTotal() >= 3 ) {
				EngineCore.outputText( ' while [eachCock] finds themselves either driving themselves higher, through the wet fur on her stomach and across each of her tensed abs, or joining the first in between her legs, pressing and pulling with her needy humping' );
			}
			EngineCore.outputText( ', and you can feel the heat, the need, pour from Brooke\'s pussy as she continues to grind on you.  You can hear her pant as she continues to kiss your neck, whimpering at your denial to penetrate her and moaning with each brush across her vulva.' );
			EngineCore.outputText( '\n\nShe needs it as badly as you do, and you\'re not about to deny it.  With your grip on her thighs, you bend your [legs] just slightly, lining your ' + CoC.getInstance().player.cockHead( x ) + ' up with her target.  She stops breathing, tensing slightly, waiting for your thrust, and you oblige.  Straightening your legs and pushing forward, you enter her, pushing in only slightly so you can adjust.' );
			...
			if( CoC.getInstance().player.balls > 0 ) {
				EngineCore.outputText( '[sack] resting gently against her crotch' );
			} else {
				EngineCore.outputText( 'crotch resting gently against hers' );
			}
			EngineCore.outputText( ', her fur lightly tingling the skin.' );
			...
			if( CoC.getInstance().player.balls > 0 ) {
				EngineCore.outputText( '  Your [balls] swing away with you, slapping onto her pussy with every inward thrust.  You feel no pain, focused only on fucking this girl raw, knowing she\'s strong enough to take you.' );
			}
			if( CoC.getInstance().player.cockTotal() === 2 ) {
				EngineCore.outputText( '  Your other dick swings freely, cold and alone in the shower between you, occasionally spurting pre into the air and onto Brooke\'s fur.  It\'s quickly washed away with the water.' );
			}
			if( CoC.getInstance().player.cockTotal() >= 3 ) {
				EngineCore.outputText( '  Your other dicks swing freely, cold and alone in the shower between you, occasionally spurting pre into the air and onto Brooke\'s fur.  They\'re quickly washed away with the water.' );
			}
			...
			if( CoC.getInstance().player.hairLength >= 1 ) {
				EngineCore.outputText( ', running her fingers through your ' + Descriptors.hairDescript() );
			}
			EngineCore.outputText( '.  <i>"Fuck me, [name]!  Just like that!  Give me that hard dick!  Fuck me!"</i>' );
			...
			if( CoC.getInstance().player.balls > 0 ) {
				EngineCore.outputText( '  Your [balls] begin to churn.' );
			}
			EngineCore.outputText( '  Your heart races, and your ' + Descriptors.cockDescript( x ) + ' pulls and tightens in her pussy.  With some more encouraging, demanding words, and with a little more activity on her part, you finally cum, giving Brooke everything she asked for.' );
			//if (cumNormal = true);
			...
			if( CoC.getInstance().player.cockTotal() === 2 ) {
				EngineCore.outputText( '  Your other prick tenses along with the first, pointing up and along the tight abs of the Shepherd girl, jizz erupting from the tip and blasting her on the underside of her perky tits.  If she even notices, she certainly doesn\'t seem to care.' );
			}
			if( CoC.getInstance().player.cockTotal() >= 3 ) {
				EngineCore.outputText( '  [EachCock] spurts and flails along with the first, painting the stall white with your warm sperm.  It lands on the walls and the floor mostly, but more than a good deal lands on Brooke and into her fur.  She\'s so out of it from her own orgasm that she barely even notices.' );
			}
		...
		//{Female only};
		else if( CoC.getInstance().player.hasVagina() && x < 0 ) {
			EngineCore.outputText( ImageManager.showImage( 'brooke-gym-female-specialscene' ) );
			EngineCore.outputText( 'You can barely keep up with her sheer, wanton desire as her hands grope your body and her tongue conquers your mouth.  You can tell from her squeezing that she\'s trying to lift your left leg up and across her hips, but, unable to grip or lift with her hands, all she can do is grunt in frustration as she instead takes to humping into you.  You do her a favor and wrap your leg around her anyway.' );
		...
		//{Herms only};
			...
			EngineCore.outputText( 'Your hands waste no time in feeling and groping all over your new canine lover\'s body, appreciating every nuance of the tight muscles you feel under her short fur.  Her strong shoulders; her thick upper-back; her taut lower-back; and her tight ass.  [EachCock] rises in the excitement, reaching up between your bodies, scratching itself across her fine belly fur.  Your [vagina] likewise quivers in excitement, readying itself, though in Brooke\'s ravaging passion, you wonder how you\'ll be able to satisfy all of yourself, if you even can.' );
			...
			EngineCore.outputText( '  She finally drops her hands down, one wrapping and caressing carefully, diligently, around your ' + Descriptors.cockDescript( x ) + ' while the other trails the cleft of your [butt], a single claw running between them and towards your taint.  She doesn\'t waste a moment when she finds your [vagina] instead of plain skin, apparently not unfamiliar with multitasking.' );
			...
			//[if (hasBalls = true);
			if( CoC.getInstance().player.balls > 0 ) {
				if( CoC.getInstance().player.findStatusAffect( StatusAffects.Uniball ) < 0 ) {
					EngineCore.outputText( '  Your [sack] swings with every pump, slapping onto the firm skin of Brooke\'s vulva every time you bottom out.  You feel no pain, and in fact the smoothness of her fur around her genitals tickles the cum out of your [balls] all the faster.' );
				} else {
					EngineCore.outputText( '  Your [sack] is to tight and smooth to swing, but it still slaps against the firm skin of Brooke\'s vulva every time you bottom out.  The smoothness of her fur around her genitals tickles the cum out of your [balls] all the faster.' );
				}
			}
			if( CoC.getInstance().player.cockTotal() === 1 ) {
				EngineCore.outputText( '  You drill into that canine pussy, giving Brooke everything you have.  Your body burns and your cum churns the longer you go.  Her voice ascends from pitiful puppy whining to the jubilant cries of a woman well fucked, and the rise of her yells bring with them the rise of your cum.' );
			}
			if( CoC.getInstance().player.cockTotal() === 2 ) {
				EngineCore.outputText( '  Your second penis swings, cold and alone, between your body and Brooke\'s as you continue to saw into her with your ' + Descriptors.cockDescript( x ) + '.  The tip points up to the cleft of her swinging, pounding breasts, ready to erupt and coat the underside of her tits with your offering.  She knows of it, and she does what she can to pleasure it, but even her well-defined six-pack can only do so much.' );
			}
			if( CoC.getInstance().player.cockTotal() >= 3 ) {
				EngineCore.outputText( '  Unfortunately, as much as both you and Brooke want to pleasure as much of you as possible, she can only do so much.  [EachCock] thrusts into nothing between you, each of them stiff and pointed right up at her face – if they can\'t cum inside her, then they\'ll jealously paint her white on the outside.' );
			}
			...
			//[if (cumNormal = true)];
			if( CoC.getInstance().player.cumQ() <= 250 ) {
				EngineCore.outputText( '  Your cum erupts into the Shepherd girl, pumping into her hard, and she continues to coax as much as she can with each ripple her orgasm gives her.  It\'s the grandest orgasm you\'ve had in some time, and it takes a good few moments before you finally calm down.' );
			}
			//[if (cumMedium = true)];
			if( CoC.getInstance().player.cumQ() <= 500 ) {
				EngineCore.outputText( '  Your fluid pumps into her hungry cooch with abandon: each of your heartbeats begets a pulse through your ' + Descriptors.cockDescript( x ) + ' and into her womb.  She continues to shout with each of her convulsions, her voice slowly and steadily descending as her own orgasm subsides.  It takes you a moment to calm down, yourself.' );
			}//[if (cumHigh = true)];
			else if( CoC.getInstance().player.cumQ() <= 1000 ) {
				EngineCore.outputText( '  Urgently, your cum floods into her, pumping into her with each heartbeat.  She feels it all and squirms with each new squirt she manages to pull out of you – she continues to rock her hips, her howling from her orgasm keeping up with your own.  She holds you tight with her powerful legs, intent on not letting a drop go.' );
			}//[if (cumVeryHigh = true)];
			else if( CoC.getInstance().player.cumQ() <= 3000 ) {
				EngineCore.outputText( '  You orgasm powerfully, the cum ripping through your ' + Descriptors.cockDescript( x ) + ' and into her with a force she\'s never experienced before.  Her howling is cut short as she doubles over, pressing her tits into your face, her hips desperately rocking into yours every time you deposit another spurt.  Her stomach begins to lose some definition from the sheer amount of cum of pumping into her, but from how she continues to convulse even long after her initial orgasm, she seems to like it.' );
			}//[if (cumExtreme = true)];
			else {
				EngineCore.outputText( '  Your orgasm is undeniable, the initial blast into her womb barely even a hint of the torrent following behind it.  Her howl is cut short as she feels her womb fill so full, so immediately, and then she howls again, even louder, as you continue to relentlessly pump your semen into her.  Her stomach not only loses its six-pack definition, but it actually begins to balloon slightly, nearly suggesting that she\'s not all muscle.  Her orgasm either never stops, or she has more than a few – you\'re not positive.' );
			}
			//[if (cocks = 2)];
			if( CoC.getInstance().player.cockTotal() === 2 ) {
				EngineCore.outputText( '  Your second phallus erupts in time with its brother, spraying into the air uselessly, but coating the underside of Brooke\'s breasts just as it promised it would.  Your cum erupts in equal parts from both dicks, soaking into her fine fur, blending in with the white strip of fur running down her torso.  She\'ll definitely need a second rinsing after this.' );
			}
			//[if (cocks >= 3)];
			if( CoC.getInstance().player.cockTotal() >= 3 ) {
				EngineCore.outputText( '  [EachCock] not inside her sprays out in time with their lucky brother, aimed right at her hunched, humping body and coating it all with your cum.  Her tight thighs, her taught belly, her ribs, her round tits, and even as high as her collarbone and her face each get a hefty blast of your cum, fulfilling their unspoken promise.' );
			}
			//if (isSquirter = false);
			if( CoC.getInstance().player.wetness() >= 4 ) {
	...
	Brooke.prototype.submitHeckelXBrookeThreesome = function() {
		...
		if( CoC.getInstance().player.gender === 2 ) {
			EngineCore.outputText( 'wet, waiting vagina' );
		}
		EngineCore.outputText( ', too out of it from the sudden penetration to do as Heckel said.  In response, Heckel thrusts forward much harder, roughly shoving a good third of her dick into Brooke\'s cunt.  <i>"<b>Now!</b>"</i> she demands, pushing her skull forward into your crotch, and finally, Brooke gets the sense to obey.' );
		EngineCore.outputText( '\n\nResting her forearms on the bench for support, Brooke lunges forward, ' );
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( 'engulfing your ' + Descriptors.cockDescript( 0 ) + ' into her warm, busy mouth' );
		}
		if( CoC.getInstance().player.cockTotal() === 2 ) {
			EngineCore.outputText( ' while she strokes your second dick with a free hand, wobbly balancing herself on her other, tired arm' );
		}
		if( CoC.getInstance().player.cockTotal() >= 3 ) {
			EngineCore.outputText( ' while she does her best to please your [eachCock], massaging one with her fingers and shoving the others in her mouth, sometimes at once' );
		}
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( '.  Brooke and Heckel move at the same pace: the Shepherd girl, eager to please, does all she can with her mouth, sucking, bathing, and worshiping your tool, ' );
		}
		if( !CoC.getInstance().player.hasCock() && CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( 'burying her muzzle into your [vagina] and immediately gets to work digging and searching your tunnel with her warm, moist, energetic tongue.  She licks, kisses, and worships everything from your vulva to your [clit] and when she gets bored of that, she thrusts her whole face into you once more, finding some new depth neither of you thought she could reach.  All the ' );
		}
		EngineCore.outputText( 'while the hyena behind her pounds into her cunt with an unusual ferocity, atypical of how you know her.  Whether Heckel is just unusually aroused or she\'s taking the whole \'dominant\' role more personally with Brooke, you\'re unsure and uncaring, if it means getting this sort of action.' );
		if( CoC.getInstance().player.gender === 3 ) {
			EngineCore.outputText( '  Brooke, knowing of your duality, releases your cock from her mouth and dips lower, snaking her tongue below you and running it deliciously over your feminine sex.  Despite her weak hands, she does absolutely everything she can with what reach she\'s given, and you lean back, sliding your ass forward a bit to give her better reach to everything you have.' );
		}
		...
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( '  You might have taken offense to the idea you\'re not as good a fuck if you weren\'t too focused on the amazing blowjob Brooke\'s giving you.' );
		}
		...
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( 'you erupt, spraying your cum directly into Brooke\'s thirsty throat.  Despite her own ecstasy, she knows to swallow to avoid choking, taking hurried breaths through her nostrils when she can while you coat her mouth white several times over.  Her mouth and her tongue are mostly still, barely conscious enough to move much less pleasure you farther.  You don\'t mind, and you cum and cum into her mouth regardless.' );
		}
		if( CoC.getInstance().player.gender === 2 ) {
			EngineCore.outputText( 'you cum into her mouth, spraying your feminine juices onto her tongue and wherever else Brooke didn\'t have the sense to cover.  It gets into her mouth; onto her muzzle; and down her face.  You coat her from your end as your pleased cunny quivers and cums again and again.  Brooke, for her part, swallows whatever happens to land into her maw, but she barely even seems to notice whenever your happen to get her anywhere else.' );
		}
	...
	Brooke.prototype.brookeVHeckelBrookeWins2 = function() {
		...
		if( x >= 0 ) {
			...
			if( CoC.getInstance().player.gender === 3 ) {
				EngineCore.outputText( '  With every thrust into Heckel, you\'re reminded of your own empty pussy and you idly wish there was a spare cock around to fill you up too.' );
			}
			...
			if( CoC.getInstance().flags[ kFLAGS.TIMES_FUCKED_HECKEL_BLOWJOB ] > 0 || CoC.getInstance().flags[ kFLAGS.TIMES_FUCKED_HECKEL_ANAL ] > 0 ) {
				var bj = CoC.getInstance().flags[ kFLAGS.TIMES_FUCKED_HECKEL_BLOWJOB ] > 0;
				var anal = CoC.getInstance().flags[ kFLAGS.TIMES_FUCKED_HECKEL_ANAL ] > 0;
				if( bj && anal ) {
					EngineCore.outputText( '  Every time you met Heckel, you either went down on her, or had to take her dick up your ass – now, with the rare opportunity to return the favor, y' );
				} else if( bj && !anal ) {
					EngineCore.outputText( '  Every time you met Heckel, you went down on her - now, with the rare opportunity to return the favour, y' );
				} else {
					EngineCore.outputText( '  Every time you met Heckel, you had to take her dick up your ass - now, with the rare opportunity to return the favour, y' );
				}
			} else {
				EngineCore.outputText( 'Y' );
			}
			EngineCore.outputText( 'ou savor every thrust into her tight velvet tunnel.  She\'s tight, far tighter than you had expected, but she\'s definitely no virgin either.  You draw your hands down to her hips, gripping and pulling delightedly at her tight muscles as you work her cunt, whetting your ' + Descriptors.cockDescript( x ) + ' with the fruit you\'ve been forbidden all this time.' );
			//[if (cocks >= 2);
			if( CoC.getInstance().player.cockTotal() >= 2 ) {
				EngineCore.outputText( '  Your extra dick flops lamely up the crack of her ass, hotdogging between her cheeks with every thrust.  You consider the possibility of maybe double-dicking the cocky hyena, but you opt against it, not willing to break the good flow you\'ve got between the three of you.' );
			}
			...
			if( CoC.getInstance().player.hasKnot( x ) ) {
				EngineCore.outputText( '  Likewise with Brooke, you pound your knot against Heckel\'s quim again and again, refusing to be denied entry, and with your orgasm, her pussy relents, taking in every inch of your ' + Descriptors.cockDescript( x ) + ' with you.' );
			}
			EngineCore.outputText( '  You cum and you cum, thrusting hard, making sure to drench every possible nook and cranny within Heckel\'s needy tunnel, seeding her deeply with your every thrust, and from how she coos and rocks in time with each of your pulses rather than her own as she does the same to Brooke, it\'s safe to say she enjoys it' );
			//[if (isHerm = true)];
			if( CoC.getInstance().player.gender === 3 ) {
				EngineCore.outputText( '.  Your [vagina] pulses in time with your ' + Descriptors.cockDescript( x ) + ', spraying your feminine juices down the inside of your tired legs uselessly, lonely and longing for a cock of its own to take' );
			}
			if( CoC.getInstance().player.cockTotal() === 2 ) {
				EngineCore.outputText( '.  Your second dick flails and pulses, ejaculating in time with its lucky brother, shooting up and coating Heckel\'s back with your jism' );
			}
			if( CoC.getInstance().player.cockTotal() >= 3 ) {
				EngineCore.outputText( ' while [eachCock] pumps yet more of your virile jism every which way, staining the tile floor and tagging the lockers surrounding you with your cum' );
			}
			EngineCore.outputText( '.' );
		}
		//{Females only};
	...
	CoC.getInstance().registerScene( 'brooke', new Brooke() );
} );