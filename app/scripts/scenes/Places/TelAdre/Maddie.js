...
angular.module( 'cocjs' ).run( function( SceneLib, MainView, PerkLib, ConsumableLib, Descriptors, Utils, kFLAGS, CoC, EngineCore ) {
	function Maddie() {
	}
	...
	Maddie.prototype.procMaddieOneIntro = function() {
		...
		if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00240 ] === 0 ) {
			MainView.outputText( 'You enter the bakery, savoring the sweet smells of sugar and baked goods.  A burly, hairy figure steps up beside you and places a strong hand on your shoulder.   The gravelly voice of the stranger says, "<i>You ain\'t from around here.  Come.  I need your help.  Show you something.</i>"  You turn to look, and are quite surprised when you see the horned visage of a minotaur ', false );
			if( CoC.player.tallness < 72 ) {
				MainView.outputText( 'looking down at', false );
			} else if( CoC.player.tallness < 100 ) {
				MainView.outputText( 'staring levelly at', false );
			} else {
				MainView.outputText( 'glaring up at', false );
			}
			MainView.outputText( ' you. It releases your shoulder and starts walking towards an \'employees only\' door.  Do you follow?\n\n', false );
			CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00240 ] = 1;
		}
		//(REPEAT) ;
		else {
			MainView.outputText( 'You walk into the bakery and a burly, hair-covered arm grabs your shoulder.  The familiar voice of a minotaur barks, "<i>You.  You can help.  Come.</i>"  You turn, but he\'s already walking towards an \'employees only\' door.  Do you follow?', false );
		}
		EngineCore.doYesNo( this, this.followMinotaurIntoBackroom, SceneLib.bakeryScene, SceneLib.bakeryScene.bakeryuuuuuu );
	};
	//[Follow] ;
	Maddie.prototype.followMinotaurIntoBackroom = function() {
		...
		if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00241 ] === 0 ) {
			...
			if( CoC.player.hasItem( ConsumableLib.BEEHONY ) && CoC.player.hasItem( ConsumableLib.L_DRAFT ) ) {
				EngineCore.choices( 'Give Them', this, this.handOverIngredientsItBeBakingTimeYo, '', null, null, '', null, null, '', null, null, 'Leave', this, this.nopeAintGotNoneODemSpeculIngredimathings );
			} else {
		...
		//(Explained) ;
			...
			MainView.outputText( 'You follow the burly chef through the door, winding through the familiar ovens.  By the time you reach his work area, you\'re both covered in a fine sheen of sweat and you find yourself responding to the minotaur musk unconsciously.  The strange chef turns to ask, "<i>You have special ingredients now, yes?</i>"', false );
			//[Yes] [Lie – No/Not Yet];
			if( CoC.player.hasItem( ConsumableLib.BEEHONY ) && CoC.player.hasItem( ConsumableLib.L_DRAFT ) ) {
				EngineCore.choices( 'Yes', this, this.handOverIngredientsItBeBakingTimeYo, 'Lie - No', this, this.nopeAintGotNoneODemSpeculIngredimathings, '', null, null, '', null, null, '', null, null );
			} else {
				EngineCore.choices( 'No', this, this.nopeAintGotNoneODemSpeculIngredimathings, '', null, null, '', null, null, '', null, null, '', null, null );
			}
		}
	};
	//[Not Yet/No];
	Maddie.prototype.nopeAintGotNoneODemSpeculIngredimathings = function() {
		MainView.outputText( '', true );
		MainView.outputText( 'The chef sighs and slams a fist into the counter hard enough to dent the metal and throw the bowls full of dough inches into the air.  A number of empty éclairs bounce and roll everywhere.  The minotaur looks back at you and snorts, "<i>Best you go.  Don\'t come without ingredients.</i>"\n\n', false );
		MainView.outputText( 'Well, no point in ', false );
		if( CoC.player.cor > 50 ) {
			MainView.outputText( 'starting a fight inside Tel\'Adre', false );
		} else {
			MainView.outputText( 'overstaying your welcome', false );
		}
		MainView.outputText( ' – you depart.', false );
	...
	Maddie.prototype.handOverIngredientsItBeBakingTimeYo = function() {
		...
		if( CoC.player.findPerk( PerkLib.MinotaurCumAddict ) ) {
			...
			EngineCore.choices( 'Wait', this, this.waitForSlutCake, 'Sneak Out', this, this.sneakAwayFromMaddie, '', null, null, '', null, null, '', null, null );
		}
	};
	//[Sneak Out];
	Maddie.prototype.sneakAwayFromMaddie = function() {
		MainView.outputText( '', true );
		MainView.outputText( 'You get out before he can find you again.  Whatever he\'s making is nothing you ever want to taste.', false );
		//(No more mino chef);
	...
	Maddie.prototype.waitForSlutCake = function() {
		...
		if( CoC.player.cor < 33 ) {
			MainView.outputText( 'antsy', false );
		} else if( CoC.player.cor < 66 ) {
			MainView.outputText( 'nervous about this whole thing', false );
		} else {
			MainView.outputText( 'intrigued by this whole thing', false );
		}
		MainView.outputText( '.  One of the waitresses brings you a glass of milk, and ', false );
		if( CoC.player.cor < 50 ) {
			MainView.outputText( 'it smells normal enough, so you go ahead and sip on it', false );
		} else {
			MainView.outputText( 'you sip on it while you wait', false );
		}
		MainView.outputText( '.  After what feels like an eternity, you get sick of waiting and push through the door into the bakery\'s backrooms to see what the hold-up is.  The minotaur isn\'t at his usual workstation, and doesn\'t look to have been there in quite some time.\n\n', false );
		MainView.outputText( 'Where could he have gone?  You backtrack through the ovens, looking down side-paths and searching through the labyrinthine storage rooms.  Just when you\'re about to give up, you hear an airy, light-headed giggle from the next room.  You peek around the corner and gasp in absolute shock.  The minotaur is pinned to the wall, his wrists stuck in place by what looks like hardened, white icing.   On top of him is the strangest - no, ONLY, cupcake-woman you\'ve ever seen.\n\n', false );
		MainView.outputText( 'She\'s taller than the imprisoned minotaur, and wider too.  The pastry-girl\'s skin is slightly porous, colored light chocolate and gleaming in the dim light where it isn\'t covered by shining, blue-iced \'clothes\'.  Her hair is white as whipped cream, and tied back with a cinnamon bun.  Her curvaceous form turns, jiggling ever so slightly as she takes you in with her green, gum-drop eyes and revealing her whipped-cream bra.  The novelty cup-cake mold is balanced atop her head, worn like a comparatively tiny fez.\n\n', false );
		MainView.outputText( 'The minotaur chef is still wearing his poofy hat, but he\'s pinned completely and irrevocably under this baked behemoth as she bounces and grinds on his convulsing member.  While you watch, his balls shrink smaller and smaller, emptying their pent up, steamy cargo directly into the cupcake\'s soft, cushiony center.  She grows larger from the sudden intake of fresh jism, giggling as she drains every drop from her creator.  "<i>Tee-hee!  Mmm, you\'re like, delicious and stuff, creat- cr... dad!  So sticky and yummy, just like me!</i>" exclaims the fluffy slut-cake.\n\n', false );
		MainView.outputText( 'Utterly shocked and drained, the chef-o-taur\'s eyes roll back in his sockets.  He slumps weakly under his creation as she bounces a few last times, futilely trying to squeeze more cum from the slumping minotaur-dick.  The cupcake-girl rises at last, not with yeast, but with a new-found purpose.  The reflective, alien surface of her eyes locks against your groin as she takes one lumbering step after another in your direction.  Her massive, spongy tits wobble dangerously close to you, nearly entrancing you with their beautiful, unnatural curves.\n\n', false );
		MainView.outputText( 'Running seems like a very good idea.  Who knows what she has planned for you?', false );
		//[RUN] [TRY TO TALK];
		EngineCore.choices( 'Run Away', this, this.runAwayFromMaddiiiieee, 'TryToTalk', this, this.talkToMaddie, '', null, null, '', null, null, '', null, null );
	};
	//[RUN DAFUQ AWAY];
	...
	Maddie.prototype.talkToMaddie = function() {
		...
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'is like, all out of icing mix!  So I\'m going to borrow a few cups from you, \'kay?', false );
		} else {
			MainView.outputText( 'gave me so much icing mix, and you like, would look soooo much better with some vanil- van... yummy frosting!', false );
		}
		MainView.outputText( '</i>"  She\'s... what!?\n\n', false );
		...
		if( CoC.player.hasCock() ) {
			...
			MainView.outputText( '"<i>Dad said my name is Madeleine, but that\'s no fun.  Just call me Maddie.  You\'ve got lots of icing like Dad, right?  I-I... need more icing.  It\'s in my recipe,</i>" says Maddie.  The baked broad strips your ' + CoC.player.armorName + ' to expose your ' + Descriptors.multiCockDescriptLight() + '.  Cooing with excitement, she examines your ', false );
			if( CoC.player.lust >= 75 ) {
				MainView.outputText( 'rock-hard', false );
			} else {
			...
			MainView.outputText( ', running a sponge-soft hand over the love-muscle.  You rock your hips, trying to squirm away.  Maddie laughs, breathily chortling while her well-rounded breasts slide to either side of you and pin you to the wall.\n\n', false );
			MainView.outputText( '"<i>So is this like, where the icing spouts out right?</i>" asks the confectionery cutey, squeezing you softly.  "<i>Awww, how sad – yours is stuck, just like Daddy\'s!  I\'ll have to squeeze and rub it until it\'ll let out the icing.</i>"\n\n', false );
			if( CoC.player.cor < 33 ) {
				MainView.outputText( 'You muster as much authority as you can in such a compromising position and explain to Maddie that what comes out of there is NOT icing.', false );
			} else if( CoC.player.cor < 66 ) {
				MainView.outputText( 'You offhandedly mention that you don\'t actually make icing.', false );
			} else {
				MainView.outputText( 'You smirk and mention that what you squirt isn\'t quite icing.', false );
			}
			MainView.outputText( '  "<i>Liar!  If that wasn\'t icing, then why would Daddy have put his in all those eclairs and me?</i>" retorts the busty cupcake, continuing on to say, "<i>I know, I can suck it out!</i>"  She purses her jelly-like lips and plunges forward, slurping all ' + Utils.num2Text( Math.floor( CoC.player.cocks[ x ].cockLength ) ) + ' inches into her oven-warmed esophagus.  Your protests are cut off by the tightness squeezing around your ' + Descriptors.cockDescript( x ) + '.  It milks you in rippling motions, buttery-slick and pulsing hungrily.\n\n', false );
			MainView.outputText( 'A half-melted tongue ', false );
			if( !CoC.player.hasSheath() ) {
				MainView.outputText( 'encircles the base', false );
			} else {
				MainView.outputText( 'pokes and prods into your sheath', false );
			}
	...
	Maddie.prototype.bakeryEpilogue = function() {
		...
		MainView.outputText( '"<i>Thanks.  Figured out what went wrong with Maddie\'s help.  Made masterpiece.  Buy giant cupcake sometime.  Delicious!  Promise it\'s safe and non-addictive.  Expensive though.  Ingredients rare.\n\n', false );
		MainView.outputText( '-X</i>"', false );
