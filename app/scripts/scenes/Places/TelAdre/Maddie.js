...
angular.module( 'cocjs' ).run( function( SceneLib, PerkLib, ConsumableLib, Descriptors, Utils, kFLAGS, CoC, EngineCore ) {
	...
	Maddie.prototype.procMaddieOneIntro = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00240 ] === 0 ) {
			...
			} else if( CoC.getInstance().player.tallness < 100 ) {
			...
			CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00240 ] = 1;
		}
		//(REPEAT) ;
		else {
	...
	Maddie.prototype.followMinotaurIntoBackroom = function() {
		EngineCore.outputText( '', true );
		//	(Not yet explained) ;
		if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00241 ] === 0 ) {
	...
	Maddie.prototype.talkToMaddie = function() {
		...
		if( CoC.getInstance().player.hasCock() ) {
			...
			EngineCore.outputText( '"<i>Dad said my name is Madeleine, but that\'s no fun.  Just call me Maddie.  You\'ve got lots of icing like Dad, right?  I-I... need more icing.  It\'s in my recipe,</i>" says Maddie.  The baked broad strips your ' + CoC.getInstance().player.armorName + ' to expose your ' + Descriptors.multiCockDescriptLight() + '.  Cooing with excitement, she examines your ', false );
			...
			EngineCore.outputText( '  "<i>Liar!  If that wasn\'t icing, then why would Daddy have put his in all those eclairs and me?</i>" retorts the busty cupcake, continuing on to say, "<i>I know, I can suck it out!</i>"  She purses her jelly-like lips and plunges forward, slurping all ' + Utils.num2Text( Math.floor( CoC.getInstance().player.cocks[ x ].cockLength ) ) + ' inches into her oven-warmed esophagus.  Your protests are cut off by the tightness squeezing around your ' + Descriptors.cockDescript( x ) + '.  It milks you in rippling motions, buttery-slick and pulsing hungrily.\n\n', false );
			...
			EngineCore.outputText( 'The cream-filled creation leans back and squirts some more icing onto the straps holding you, but instead of reinforcing the bonds, it eats through the hardened confection to release you into her waiting bosom.  She catches you in the pillowy chest-embrace, stroking your hair while she says in a sing-song voice, "<i>Thanks for all the icing ' + CoC.getInstance().player.mf( 'mister', 'miss' ) + '!  I think I have enough for now.  I think I\'ll go like, check on my Dad and stuff.  Maybe he wants to add some icing to the recipe?</i>"\n\n', false );
		...
		//(FEMALE/Genderpoots) ;
			...
			EngineCore.outputText( '"<i>Shhh, shhh... that\'s a good ' + CoC.getInstance().player.mf( 'boy', 'girl' ) + '.  Isn\'t my icing the absolute best?</i>" she verbally gushes, just like the nipple between your teeth.  "<i>Drink up', false );
			if( CoC.getInstance().player.thickness < 60 ) {
			...
			if( CoC.getInstance().player.thickness < 60 || CoC.getInstance().player.tone >= 50 ) {
			...
			EngineCore.outputText( 'Once the beast-man has finished talking you realize the cupcake-girl, Madeleine, is standing behind him.  Her blue-iced \'clothes\' have been remade, shaped into a form-fitting apron that accentuates her massive, otherworldly curves.  The minotaur chef utters, "<i>We go now.  Get dressed.  Maybe sometime can visit Maddie.</i>"  Maddie claps her hands, bouncing and jiggling with excitement as the two of them leave you there to get dressed.', false );
			EngineCore.outputText( CoC.getInstance().player.modThickness( 100, 10 ), false );
