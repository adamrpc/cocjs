...
angular.module( 'cocjs' ).run( function( SceneLib, MainView, $log, CockTypesEnum, ImageManager, BreastStore, Appearance, ArmorLib, PerkLib, ConsumableLib, Descriptors, Utils, kFLAGS, CoC, EngineCore ) {
	...
	Rubi.prototype.specialRelationship20scene = function() {
		...
		this.rubiSprite();
		MainView.outputText( 'As you\'re leaving the bakery, you hear the clicking of heels behind you.  Turning back you see Rubi rushing towards you shouting, "<i>Hey, wait a second!</i>"  You do, and wait for her to catch up.  Stopping in front of you, she puts her hands on her dainty knees and catches her breath.' );
	...
	Rubi.prototype.acceptRubi = function() {
		MainView.clearOutput();
		this.rubiSprite();
		MainView.outputText( 'You wrap your arms around and pull him into a hug.  You declare that you like him no matter what sort of genitals he happened to be born with.' );
	...
	Rubi.prototype.rejectRubi = function() {
		MainView.clearOutput();
		this.rubiSprite();
		MainView.outputText( 'With your hands on his shoulders you carefully keep him at arm\'s length.  Putting it as delicately as you can, you tell him that you\'re just not interested in him <i>that</i> way, not anymore at least.' );
	...
	Rubi.prototype.rubiIntros = function() {
		...
		if( CoC.flags[ kFLAGS.RUBI_INTRODUCED ] === 0 ) {
			MainView.outputText( 'A cute human girl moves from table to table, bussing the empty ones and serving drinks to the occupied.  She\'s pretty flat-chested, and wears a pink and white frilly waitress uniform with an apron.  When she turns her back to you, you can tell she\'s not <i>entirely</i> human, what with the long red spaded tail coming out from under her dress.' );
		}
		//(Have met, relationship 0-19);
		else if( this.rubiAffection() < 30 ) {
			MainView.outputText( 'Rubi is here, moving from table to table efficiently clearing the empty ones and serving new customers.  Her long red tail swishes back and forth, sometimes lifting her dress enough to get a peek at her panties.' );
			...
			MainView.outputText( 'Strangely, even though you know it\'s time for Rubi\'s shift, she\'s not here.\n' );
			...
			MainView.outputText( 'Rubi is here, moving from table to table, not quite as efficiently as [rubi ey] used to, but with a lot more exuberance.  [rubi Eir] long red tail swishes around, seemingly drawn towards the crotch of patrons and co-workers alike.' );
		} else {
			MainView.outputText( 'Rubi is here, moving from table to table efficiently clearing the empty ones and serving new customers.  [rubi Eir] long red tail swishes back and forth excitedly.' );
		}
		MainView.outputText( '\n' );
	...
	Rubi.prototype.approachRubiScenes = function() {
		...
		//(Approach repeat, relationship 0-19);
			...
			MainView.outputText( 'You take a seat at a table, and within seconds Rubi pulls up a chair next to you.  ' );
		}
		//(Approach repeat, relationship 20+.  At this stage, randomly select an outfit from what the PC has given him, plus his waitress outfit.  Variants included for Bimbo /Incubus Rubi.);
			...
			if( select === 1 ) {
				...
				MainView.outputText( this.rubiMF( 'He', 'She' ) + '\'s dressed in [rubi eir] usual waitress uniform: a pink blouse and skirt covered by a frilly white apron.' );
				if( CoC.flags[ kFLAGS.RUBI_BREAST_SIZE ] >= 4 ) {
					MainView.outputText( '  Rubi\'s breasts strain against the tight blouse, threatening to pop a button at any moment.' );
				}
				if( CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] >= 9 ) {
					MainView.outputText( '  You notice Rubi holds [rubi emself] a little awkwardly, and it\'s only when [rubi ey] sits down that you spy the slight bulge in [rubi eir] skirt.' );
				}
			}
			//(Suit Clothes (Normal));
			else if( select === 2 ) {
				//(Suitclothes);
				MainView.outputText( this.rubiMF( 'He', 'She' ) + '\'s dressed differently today, having foregone the normal waitress uniform for a professional black and white suit with a loose tie, though [rubi ey] still wears the waitress headband' );
				if( CoC.flags[ kFLAGS.RUBI_HORNTYPE ] > 0 ) {
					MainView.outputText( 'just behind the horns' );
				}
				MainView.outputText( '.  In a sexy sort of way, [rubi ey] looks like a girl dressed in her boyfriend\'s clothes' );
				if( CoC.flags[ kFLAGS.RUBI_BREAST_SIZE ] >= 4 ) {
					MainView.outputText( ', though with [rubi eir] large bosom, [rubi ey] can\'t quite button up all the way, giving the appearance of a sexy secretary' );
				}
				MainView.outputText( '.' );
			} else if( select === 3 ) {
				//(Rubber Fetish Outfit (Bimbo));
				//(Rubber Fetish Clothes);
				MainView.outputText( '[rubi Ey]\'s dressed differently today, having foregone the normal waitress uniform for a tight, black, rubber shirt, along with a skirt, and stockings.  The holes cut into the skirt, proudly displaying Rubi\'s ' );
				if( CoC.flags[ kFLAGS.RUBI_NIPPLE_TYPE ] > 0 ) {
					MainView.outputText( 'black ' );
				}
				MainView.outputText( 'nipples, which stand at perky attention in the cool air.  Rubber straps crisscross the outfit in odd places, and the whole ensemble looks like something out of one of Mareth\'s many fetish dungeons.  The outfit draws attention to [rubi eir]' );
				if( CoC.flags[ kFLAGS.RUBI_BREAST_SIZE ] >= 4 ) {
					MainView.outputText( ' impressive bosom, and sort of makes [rubi em] look like the kind of rubber doll one might buy for those lonely nights.' );
				} else {
					MainView.outputText( ' relatively flat chest, though with the rest of [rubi eir] figure, it\'s hard to imagine [rubi em] as anything but a woman.' );
				}
			} else if( select === 5 ) {
				//(Tube Top);
				MainView.outputText( '[rubi Ey]\'s dressed differently today, having foregone the normal waitress uniform for a neon pink tube top and a pair of tiny denim shorts.' );
				//(D breasts or higher);
				if( CoC.flags[ kFLAGS.RUBI_BREAST_SIZE ] >= 4 ) {
					MainView.outputText( '  [rubi Eir] breasts strain the fabric of the tube top, any wrong move threatening to have them burst forth.' );
				}
				//(dick 9" or higher);
				if( CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] >= 4 ) {
					MainView.outputText( '  [rubi Eir] rather large bulge can\'t be concealed, however, and you sometimes catch Rubi holding [rubi eir] notepad down across [rubi eir] crotch when [rubi ey] moves from table to table.' );
				} else {
					MainView.outputText( '[rubi Eir] bulge is well concealed at least.  You\'re sure it would take a skilled eye, or someone who knew what to look for, to notice Rubi\'s little package.' );
				}
			} else if( select === 6 ) {
				//(Bodysuit);
				MainView.outputText( '[rubi Ey]\'s dressed differently today, having foregone the normal waitress uniform for an almost sheer bodysuit covering [rubi em] from the groin up to the neck.  The lacy outfit is covered with flowers and swirling, colorful decorations in strategically placed locations.' );
				if( CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] >= 9 ) {
					MainView.outputText( '  However, with an outfit this tight, it\'s almost impossible for [rubi em] to hide [rubi eir] impressive package.  Rubi\'s cock pokes straight upwards, creating a bulge from [rubi eir] crotch up towards [rubi eir] belly button.' );
				} else if( CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] > 0 ) {
					MainView.outputText( '  However, with an outfit this tight you can see a slight outline where [rubi eir] small package is tucked away.  You wonder how many other patrons have noticed the same thing.' );
				}
			} else if( select === 7 ) {
				//(Long Dress (Bimbo));
				MainView.outputText( '[rubi Ey]\'s dressed differently today, having foregone the normal waitress uniform for an elegant, sequined ballroom gown.' );
				if( this.rubiBimbo() ) {
					MainView.outputText( '  [rubi Ey]\'s even modified the dress with false gems and a slit up the side to show off [rubi eir] leg.' );
				}
				MainView.outputText( '  You are stunned by the breathtaking combination of classy and carnal.  You\'re sure [rubi ey] would look great at a fancy ball, but here at the bakery [rubi ey] looks out of place, especially as a waitress.' );
				//(Breasts flat or A cup);
				if( CoC.flags[ kFLAGS.RUBI_BREAST_SIZE ] <= 1 ) {
					MainView.outputText( '  The dress must have come with a padded bra, because you\'re surprised to see Rubi with  breasts.  Though small, they help to accentuate [rubi eir] feminine figure.' );
				}
			} else if( select === 8 ) {
				//(Tight Leather Pants (Bimbo));
				MainView.outputText( '[rubi Ey]\'s dressed differently today, having foregone the normal waitress uniform for a loose silk white shirt with tight black leather pants.' );
				if( CoC.flags[ kFLAGS.RUBI_BREAST_SIZE ] >= 4 ) {
					MainView.outputText( '  With [rubi eir] large rack, [rubi ey] looks much like a busty piratess.  From the way [rubi eir] breasts strain at the top, you\'re certain that one wrong breath could send a button flying across the room.' );
				} else {
					MainView.outputText( '  With [rubi eir] relatively flat chest, however, you wonder if perhaps it makes [rubi em] look a bit too much like a man.  An effeminate man, but a man nonetheless.' );
				}
				//Cock variants:;
				//(9" or higher);
				if( CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] >= 9 ) {
					MainView.outputText( '  [rubi Eir] generous cock is on display, much to [rubi eir] chagrin, as a bulge running down the length of one pant leg.' );
				} else {
					MainView.outputText( 'Luckily, given [rubi eir] relatively small package, [rubi eir] bulge is kept nice and secret even with the tightness of the clothing.' );
				}
			} else if( select === 9 ) {
				//(Nurse Clothes (Bimbo));
				MainView.outputText( '[rubi Ey]\'s dressed differently today, having foregone the normal waitress uniform for a tight, one-piece white dress with a red cross over the breast pocket, and atop her head is perched a little white hat with the same red cross.  [rubi Ey] looks like the kind of nurse you\'d expect to see in your dirty dreams, and you wonder how many bakery customers were thrown by Rubi\'s odd, albeit sexy, appearance today.' );
				//(D breasts or higher);
				if( CoC.flags[ kFLAGS.RUBI_BREAST_SIZE ] >= 4 ) {
					MainView.outputText( '  Rubi\'s large breasts strain the fabric, drawing the dress further up [rubi eir] body and showing off more of [rubi eir] legs and ass.' );
				}
			}
			//(Slutty Swimwear (Normal));
			else if( select === 10 ) {
				//(Slutty Swimwear);
				MainView.outputText( '[rubi Ey]\'s dressed differently today, having foregone the normal waitress uniform for an impossibly skimpy black bikini.  It\'s a cute swimsuit, and Rubi looks damn sexy in it, but you can\'t help but think it\'s more suited for a day at the beach and not a day working in a bakery.' );
				//(D breasts or higher);
				if( CoC.flags[ kFLAGS.RUBI_BREAST_SIZE ] >= 4 ) {
					MainView.outputText( '  Rubi\'s tremendous cleavage threatens to spill out of the skimpy bikini top with every movement, and from the red faces around the bakery, you wonder if that\'s already happened.' );
				} else if( CoC.flags[ kFLAGS.RUBI_BREAST_SIZE ] <= 1 ) {
					MainView.outputText( '  The top only emphasizes Rubi\'s relatively flat chest, though with the rest of [rubi eir] figure, [rubi ey] still looks extremely feminine.' );
				}
			}
			//(Minidress Outfit (Bimbo));
			else if( select === 11 ) {
				MainView.outputText( '[rubi Ey]\'s dressed differently today, having foregone the normal waitress uniform for an impossibly short strapless pink mini-dress.  The top barely comes up to cover [rubi eir] nipples, and you\'re sure [rubi ey] has to adjust it constantly to avoid any wardrobe malfunctions.  The bottom is so short [rubi ey]\'s certainly giving a show to anyone behind [rubi em] whenever [rubi ey] leans forward.  All in all, Rubi looks like [rubi ey]\'s ready for a night on the town and not a day waiting tables.' );
				if( CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] > 0 ) {
					MainView.outputText( '  [rubi Eir] skirt rides so high you catch glimpses of [rubi eir] unsurprisingly unpantied crotch, and the enticing package held within.  You wonder how many people have gotten a surprise with their drinks today!' );
				}
			}
			//(Bondage Straps);
			else if( select === 12 ) {
				MainView.outputText( '[rubi Ey]\'s dressed differently today, having foregone the normal waitress uniform for a series of leather straps and hooks.  A series of leather belts cross [rubi eir] body, covering strategic areas with thicker, more pliable straps.  You wonder how comfortable such an outfit could be, and as you do so you notice Rubi dig out a wedgie when [rubi ey] thinks no one\'s looking.  How Rubi can continue to work at the bakery wearing this, you\'ll never know.' );
			}
			//(Inquisitor's Corset);
			else if( select === 13 ) {
				MainView.outputText( '[rubi Ey]\'s dressed differently today, having foregone the normal waitress uniform for a tight red and gold corset, connected with garter straps to a pair of sheer black stockings.  The corset really enhances Rubi\'s naturally feminine figure, though you can\'t imagine it being too comfortable.  The black, opaque stockings come up to [rubi eir] thighs have an inlaid gold corset pattern on the back, which lead up to a pair of bows that bounce perkily as Rubi walks.' );
			}
			//(Risque Waitress Uniform);
			else if( select === 14 ) {
				MainView.outputText( '[rubi Ey]\'s dressed differently today, having foregone the normal waitress uniform for an extremely modified version.  The skirt wrapped around [rubi eir] waist could be confused for a belt, and the apron (the only other thing covering [rubi eir] torso) doesn\'t extend much further.  Any time Rubi takes a step or bends in any direction, anyone within sight is sure to get a good look at [rubi eir] ' );
				if( CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] > 0 ) {
					MainView.outputText( this.rubiCock( 33 ) + ', clad in a cute little frilly pink cocksock' );
				}
				...
				if( CoC.flags[ kFLAGS.RUBI_NO_CUNT ] === 0 ) {
					MainView.outputText( 'feminine mound trapped beneath a pair of pink panties' );
					//(if Bimbo Brain);
					if( this.rubiBimbo() && CoC.flags[ kFLAGS.RUBI_DEBIMBO ] === 0 ) {
						MainView.outputText( ' trimmed feminine mons, free and open to the air' );
					}
				}
				MainView.outputText( '.' );
				//(D breasts or higher);
				if( CoC.flags[ kFLAGS.RUBI_BREAST_SIZE ] >= 4 ) {
					MainView.outputText( '  Rubi\'s breasts, large as they are, show off a decent amount of sideboob and threaten to spill out at any moment.  Judging by the flushed faces of some customers, this may have already happened.' );
				}
				MainView.outputText( '  This outfit, as risque as it is, is clearly something Rubi never would have worn before [rubi ey] met you.  [rubi Ey] must be really comfortable in [rubi eir] body now!' );
			}
			MainView.outputText( '\n\n[rubi Ey] hands you the usual menu and says, "<i>' );
			if( this.rubiBimbo() && CoC.flags[ kFLAGS.RUBI_DEBIMBO ] === 0 ) {
				MainView.outputText( 'Like, h' );
			} else {
				MainView.outputText( 'H' );
			}
			MainView.outputText( 'ere\'s the menu, babe.  Think about what you want and let me know when you\'re ready.</i>"  [rubi Ey] stands and moves to clean off a nearby table, [rubi eir] red spaded tail swishing behind [rubi em] sensually.' );
			//(Adventurer's Outfit);
			if( select === 4 ) {
				MainView.outputText( '[rubi Ey]\'s dressed differently today, having foregone the normal waitress uniform for a large green tunic which looks more like a dress on [rubi em], along with brown tights and a pointed green hat.  At [rubi eir] waist is a wooden sword alongside [rubi eir] normal notepad.  Were Rubi more masculine, you might think [rubi ey] looks like an adventurer of some kind, but in reality [rubi ey] looks like a normal girl in a strange hat.' );
				if( CoC.flags[ kFLAGS.RUBI_BREAST_SIZE ] >= 4 ) {
					MainView.outputText( '  In an odd sort of way, [rubi ey] looks like an adventurer who met the wrong end of a witch\'s curse.' );
				}
			}
			...
			if( select === 1 ) {
				...
				if( this.rubiBimbo() && CoC.flags[ kFLAGS.RUBI_DEBIMBO ] === 0 ) {
					MainView.outputText( 'surprised to see [rubi ey]\'s wearing nothing at all underneath, showing off [rubi eir] asshole' );
					if( CoC.flags[ kFLAGS.RUBI_NO_CUNT ] === 0 && CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] === 0 ) {
						MainView.outputText( ' and wet cunt' );
					} else if( CoC.flags[ kFLAGS.RUBI_NO_CUNT ] > 0 && CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] > 0 ) {
						MainView.outputText( ' and limp cock' );
					} else {
						MainView.outputText( ', wet cunt, and limp cock' );
					}
					MainView.outputText( ' to anyone behind [rubi em]' );
				} else {
					MainView.outputText( 'excited to see Rubi\'s choice of frilly pink panties clinging to [rubi eir] supple ass' );
				}
				MainView.outputText( '.' );
			}
			//(Dashing Outfit);
			...
			//(Adventurer's Outfit);
				...
				//If Bimbo Body:;
				if( this.rubiBimbo() ) {
					MainView.outputText( '  You even spy a wet patch in the crotch, with rivulets of moisture running down the legs, which [rubi ey] must have trouble hiding... if [rubi ey] even bothers to try.' );
				}
			}
			//(Long Dress);
			else if( select === 7 ) {
				...
				if( this.rubiBimbo() && CoC.flags[ kFLAGS.RUBI_DEBIMBO ] === 0 ) {
					MainView.outputText( 'And then [rubi ey] begins to shake [rubi eir] ass, making spanking motions and effectively ruining the air of sophistication.' );
				}
			}
			//(Nurse Uniform);
			else if( select === 9 ) {
				...
				if( this.rubiBimbo() && CoC.flags[ kFLAGS.RUBI_DEBIMBO ] === 0 ) {
					...
					MainView.outputText( 'excited to see Rubi\'s choice of frilly white panties clinging to [rubi eir] supple ass' );
				}
				MainView.outputText( '.' );
			}
			//(Rubber Fetish Outfit);
				...
				if( this.rubiBimbo() && CoC.flags[ kFLAGS.RUBI_DEBIMBO ] === 0 ) {
					...
					MainView.outputText( 'excited to see a pair of translucent rubber panties clinging to [rubi eir] supple ass' );
				}
				MainView.outputText( '.' );
			}
			//(Suitclothes);
			...
			//(Bimbo Minidress);
				...
				if( this.rubiBimbo() && CoC.flags[ kFLAGS.RUBI_DEBIMBO ] === 0 ) {
					...
					MainView.outputText( 'excited to see a cute, little thong wedged into [rubi eir] supple ass' );
				}
				MainView.outputText( '.' );
			}
			//(Bodysuit);
				...
				//If Bimbo Body:;
				if( this.rubiBimbo() ) {
					MainView.outputText( '  You even spy a moist patch in the crotch, which must be impossible to hide... if [rubi ey] even bothers to try.' );
				}
			}
			//(Bondage Straps);
			else if( select === 12 ) {
			...
			//(Risque Waitress Uniform);
				...
				if( this.rubiBimbo() && CoC.flags[ kFLAGS.RUBI_DEBIMBO ] === 0 ) {
					...
					MainView.outputText( 'excited to see a pair of pink frilly panties clinging to [rubi eir] supple ass' );
				}
				MainView.outputText( '.' );
			}
		}
	...
	Rubi.prototype.getTeaFromRubi = function() {
		...
		//(Relationship 0-19);
		if( this.rubiAffection() < 30 ) {
			MainView.outputText( 'You flag Rubi down and tell her you\'d like to order a cup of tea.  She nods and soon you\'ve got a piping hot cup of fragrant tea in front of you.  "<i>It\'s our relaxing blend, soothes the body and mind,</i>" she says.  You take a sip and sigh.  It is indeed relaxing.' );
		...
		//(Relationship 20+, normal Rubi);
			...
			MainView.outputText( 'You flag Rubi down and tell [rubi em] you\'d like to order a cup of tea.  [rubi Ey] nods and soon you\'ve got a piping hot cup of fragrant tea in front of you.  "<i>It\'s our relaxing blend, soothes the body and mind,</i>" [rubi ey] says.  You take a sip and sigh.  It is indeed relaxing.' );
	...
	Rubi.prototype.chatWithRubi = function() {
		...
		if( this.rubiAffection() < 15 ) {
			MainView.outputText( 'You decide not to order anything to drink, instead taking the chance to talk with your waitress, Rubi, whenever [rubi ey]\'s available.  You talk of pleasant but inconsequential things.  Rubi talks of growing up in a town far away, an actual human village, which was later raided by demons.  [rubi Ey] lived off the land for a while, drinking down one too many bottles of mysterious milk which gave [rubi em] the tail she now possesses.  [rubi Ey] eventually found Tel\'Adre and was taken in by a kindly trio of ferret-morphs.  [rubi Ey] talks wistfully of home, and [rubi eir] green eyes shimmer with a faraway look for a moment before [rubi ey] snaps back to reality.' );
		...
		//(Relationship 40, Bimbo Rubi);
			...
			MainView.outputText( 'You decide not to order anything to drink, instead taking the opportunity to hang out with Rubi whenever [rubi ey] has a free moment.  You talk of pleasant but inconsequential things.  Rubi spends some time discussing the fashion movements of Tel\'Adre and how they\'re nothing like the fashion in [rubi eir] home village, an honest-to-goodness human village.  [rubi Ey] mentions how everyone back \'home\' dressed so conservatively, not like the sexy body-baring fashions here.  Rubi even says how glad [rubi ey] is that demons attacked those years ago, so that [rubi ey] could come here and start her new life with hot new clothes and a new body.' );
	...
	Rubi.prototype.rubisFuckingHouseYouPervert = function() {
		MainView.outputText( ImageManager.showImage( 'rubi-at-house' ), false );
		MainView.clearOutput();
		this.rubiSprite();
		//(Not available to those with a centaur, naga or drider body.);
		//(First Time?);
		if( CoC.flags[ kFLAGS.RUBIS_HOUSE_FIRST_TIME ] === 0 ) {
			MainView.outputText( 'Rather than order anything, you stop Rubi as he\'s going by your table and pull [rubi em] close.  "<i>How about we get out of here?</i>" you whisper into [rubi eir] ear.' );
		...
		//Repeat unirubi variant;
			...
			MainView.outputText( 'Rather than order anything, you stop Rubi as [rubi ey]\'s going by your table and pull [rubi em] close.  "<i>How about we get out of here?</i>" you whisper into [rubi eir] ear.' );
	...
	Rubi.prototype.rubiIdentity = function() {
		...
		if( CoC.flags[ kFLAGS.TIMES_DISCUSSED_RUBIS_IDENTITY ] === 0 ) {
			MainView.outputText( 'You\'ve noticed Rubi always refers to himself as a "he", despite ' );
			if( CoC.flags[ kFLAGS.RUBI_NO_CUNT ] === 1 ) {
				MainView.outputText( 'his attempts to appear otherwise' );
			} else {
				MainView.outputText( 'his relatively recent transformation to the contrary' );
			}
			MainView.outputText( '.  Questioning him about it, he simply blushes.' );
			...
			if( CoC.flags[ kFLAGS.RUBI_NO_CUNT ] === 0 ) {
				MainView.outputText( '  Though now that I am, I absolutely love it!' );
			}
			MainView.outputText( '</i>" he confides, fidgeting slightly.  "<i>If you wanted to call me a "she", I\'d be OK with that, just so you know.</i>"' );
			MainView.outputText( '\n\nWhich type of pronoun would you prefer Rubi to use?' );
		}
		//(Repeating?);
		else {
			MainView.outputText( 'You talk with Rubi at length about [rubi eir] gender identity again.  All in all, [rubi ey] seems pretty open to being either gender, with no real preference one way or another as long as [rubi ey] gets [rubi eir] pretty dresses.' );
			MainView.outputText( '\n\nWhich type of pronoun would you prefer Rubi to use?' );
		}
	...
	Rubi.prototype.fuckRubi = function() {
		MainView.outputText( ImageManager.showImage( 'rubi-fuck' ), false );
		var x;
		...
		this.rubiSprite();
		//(-100 Arousal);
		MainView.outputText( 'You take Rubi by the hand and head into the bedroom, clutching [rubi em] close.  You sit on the edge of the soft fluffy bed and pull the little demon ' + this.rubiMF( 'boy', 'girl' ) + ' onto your lap.  The two of you kiss, letting your ' + Descriptors.tongueDescript() + ' explore the depths of your partner\'s mouth.' );
		...
		if( size === 0 ) {
			...
			if( CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] <= 5 ) {
				MainView.outputText( 'I guess we\'re in the same boat.</i>"' );
			} else if( this.rubiBimbo() && CoC.flags[ kFLAGS.RUBI_DEBIMBO ] === 0 ) {
				MainView.outputText( 'That\'s too bad, I was soooo looking forward to a real fucking.</i>"' );
			} else {
				MainView.outputText( 'That\'s pretty sad, getting shown up by a dainty little thing like me...</i>"' );
			}
		} else if( size === 1 ) {
		...
		MainView.outputText( ', Rubi slips off your lap and kneels on the floor.  [rubi Ey] pauses a moment, taking in your ' );
		if( CoC.player.hasCock() ) {
			MainView.outputText( Descriptors.cockDescript( x ) );
		} else {
			MainView.outputText( '[clit]' );
		}
		MainView.outputText( ' with [rubi eir] eyes before leaning in and giving it a tentative lick, which sends a shiver up your spine.  Seeing that, [rubi ey] gives a little smile and licks again, running [rubi eir] smooth tongue all along the length of your ' );
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'swollen member' );
		} else {
			MainView.outputText( 'impressive love button' );
		}
		MainView.outputText( '.  Rubi\'s mouth dances along its length, at times flicking delicately and at others slurping loudly.  [rubi Eir] tongue ripples along the underside of ' );
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'your cockhead' );
		} else {
			MainView.outputText( 'the tip of your clit' );
		}
		MainView.outputText( ', sending shivers of pleasure up your spine.' );
		//(If Cock);
		if( CoC.player.hasCock() ) {
			MainView.outputText( '  Amazingly, with the expert ministrations of your lover\'s mouth, you can\'t hold yourself in much longer, and begin to tense up, but at the last minute you pull yourself away from Rubi\'s grip and lips, halting yourself before you can cum.' );
		}
		...
		if( size < 2 ) {
			...
			MainView.outputText( ' at [rubi eir] tight ' );
			if( CoC.flags[ kFLAGS.RUBI_NO_CUNT ] > 0 ) {
				MainView.outputText( 'tailhole' );
			} else {
				MainView.outputText( 'pussy' );
			}
			MainView.outputText( ', rubbing the mixture of [rubi eir] saliva and your juices into [rubi eir] cleft to get [rubi em] suitably lubricated before the real fun begins' );
			if( this.rubiBimbo() ) {
				MainView.outputText( ', not that [rubi ey] needs the extra lubrication, as [rubi ey]\'s practically dripping like a faucet' );
			}
			MainView.outputText( '.' );
			MainView.outputText( '  You press forward, sinking your entire length into Rubi\'s depths, which clench and unclench sensually.  Aided by the additional lubrication, you slip easily in and out, pumping slowly at first.' );
		}
		//(These two paragraphs for cocks over 24 area);
			...
			MainView.outputText( ' at the space just below [rubi eir] ' );
			if( CoC.flags[ kFLAGS.RUBI_NO_CUNT ] > 0 ) {
				MainView.outputText( 'balls' );
			} else {
				MainView.outputText( 'cunt' );
			}
			MainView.outputText( ', rubbing the mixture of [rubi eir] saliva and your juices into [rubi eir] legs to get [rubi em] suitably lubricated before the real fun begins' );
			//(if bimbo);
			if( this.rubiBimbo() ) {
				MainView.outputText( ', not that [rubi ey] needs the extra lubrication, as she\'s practically dripping like a faucet, which dribbles down [rubi eir] legs suitably enough' );
			}
			MainView.outputText( '.' );
			...
			MainView.outputText( ' slipping between Rubi\'s thighs, which grip you tightly.  You feel [rubi eir] ' );
			if( CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] > 0 ) {
				MainView.outputText( 'hard cock' );
			} else {
				MainView.outputText( 'stiff clit' );
			}
			MainView.outputText( ' pressing against the top of ' );
			if( CoC.player.hasCock() ) {
				MainView.outputText( 'your dick' );
			} else {
				MainView.outputText( 'your clit' );
			}
			MainView.outputText( ', and know that with every thrust you give, [rubi ey]\'ll get a good thrill too.' );
		}
		...
		if( CoC.flags[ kFLAGS.RUBI_ORGASM_DENIAL ] > 0 && !this.rubiBimbo() ) {
			MainView.outputText( '  You give Rubi\'s ass a hard smack and remind [rubi em] that no matter what happens, [rubi ey] still can\'t cum yet.  [rubi Eir] eyes flutter open and [rubi ey] actually whimpers a little, but can\'t form the words to object.' );
		}//(If Orgasm Denial is NOT activated; male/herm Rubi);
		else if( CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] > 0 ) {
			MainView.outputText( '  Rubi’s body shivers and trembles, and almost collapses.  [rubi Ey] grasps at the blankets with wild abandon as ropes of pearly white cum erupt from [rubi eir] cock, splattering against [rubi eir] chest and the blankets.' );
		}
		//(Female Rubi);
		else {
			MainView.outputText( '  Rubi\'s body shivers and trembles, and [rubi ey] almost collapses.  [rubi Ey] grasps at the blankets with wild abandon and gives and orgasmic scream as [rubi eir] cunt clenches and unclenches fiercely.  A spray of femjuices erupts outwards from [rubi em], coating your crotch and dripping down both yours and [rubi eir] legs.' );
		}
		...
		if( CoC.player.hasCock() && CoC.player.cockArea( x ) <= this.rubiCapacity() ) {
			MainView.outputText( 'Your ' + Descriptors.cockDescript( x ) + ' engorges for a second before erupting deep inside your demonic little fucktoy.  ' );
			if( CoC.player.cumQ() < 250 ) {
				MainView.outputText( 'Cum dribbles out from [rubi eir] hole as you give a couple extra thrusts.' );
			} else if( CoC.player.cumQ() < 1000 ) {
				MainView.outputText( 'A generous amount of cum trickles from [rubi eir] depths as you give a couple extra thrusts.' );
			} else if( CoC.player.cumQ() < 2000 ) {
				MainView.outputText( 'Cum practically floods from  [rubi eir] abused ' );
				if( CoC.flags[ kFLAGS.RUBI_NO_CUNT ] > 0 ) {
					MainView.outputText( 'ass' );
				} else {
					MainView.outputText( 'cunt' );
				}
				MainView.outputText( ' as you give a couple extra thrusts.' );
			}
		} else if( CoC.player.hasCock() ) {
			MainView.outputText( 'Your ' + Descriptors.cockDescript( x ) + ' engorges for a second before erupting beneath Rubi, splattering [rubi em] and the blankets ' );
			if( CoC.player.cumQ() < 250 ) {
				MainView.outputText( 'with a fair amount of your seed.' );
			} else if( CoC.player.cumQ() < 1000 ) {
				MainView.outputText( 'with a generous amount of your spunk.' );
			} else {
				MainView.outputText( 'with a veritable torrent of potent pearlescent spooge.' );
			}
		}
		//(Player has clit?);
		else {
			MainView.outputText( '  Your ' + Descriptors.clitDescript() + ' throbs and engorges briefly as your ' + Descriptors.vaginaDescript( 0 ) + ' reflexively contracts and a warm feeling rocks through your entire body.  Femjuices run down your [legs] and relief overtakes you like a powerful wave upon a beach.' );
		}
		...
		if( this.rubiBimbo() && CoC.flags[ kFLAGS.RUBI_DEBIMBO ] === 0 ) {
			MainView.outputText( 'like, totally ' );
		} else {
			MainView.outputText( 'absolutely ' );
		}
		MainView.outputText( 'fantastic babe,</i>" [rubi ey] murmurs before drifting off into a sex-filled dreamland.' );
	...
	Rubi.prototype.dildoFuckRubi = function() {
		MainView.outputText( ImageManager.showImage( 'rubi-dildo-fuck' ), false );
		MainView.clearOutput();
		this.rubiSprite();
		//(If PC has no cock);
		if( !CoC.player.hasCock() ) {
			MainView.outputText( 'You don\'t have the equipment to give Rubi a good fucking, but there is one thing you do have.' );
		}//(If PC has cock);
		else {
			MainView.outputText( 'While you do have the requisite equipment to give Rubi a good fucking, you\'d much rather use something else.' );
		}
		//(Add);
		MainView.outputText( '  You produce the goblin-made dildo from your pack, and Rubi\'s eyes immediately go wide.  "<i>Wow,</i>" [rubi ey] says, reaching out to touch it.  [rubi Ey] runs [rubi eir] hands along its length, marveling at its color, shape and size.' );
		...
		if( CoC.player.hasVagina() ) {
			MainView.outputText( 'You press the tip into your cunt quickly, giving it a taste of what is to come.  The brief contact is enough to make you shudder slightly with pleasure.  ' );
		}
		//(If PC has fuckable nipples);
		if( CoC.player.hasFuckableNipples() ) {
			MainView.outputText( 'You let the dildo meander up your body until it reaches your breasts, and with a bit of a grin you insert the tip right into one nipple, enjoying the feeling of your breasts swell up with arousal.  You do the same with the other nipple, and take a minute going back and forth.  ' );
		}
		MainView.outputText( 'Finally you bring the dildo up to your mouth, licking the juices off of it, eagerly downing the goblin sex-drug.' );
		...
		//(Normal/Incubus Rubi);
		if( !this.rubiBimbo() ) {
			MainView.outputText( 'They part shakily and [rubi eir] tongue reaches out to taste the dildo.  ' );
		}//(Bimbo Rubi);
		else {
			MainView.outputText( 'They easily part, and [rubi eir] tongue takes a long slurp, just to taste the dildo.  ' );
		}

		MainView.outputText( '[rubi Ey] apparently finds it pleasing, because [rubi ey] immediately wraps [rubi eir] lips around the tip, sucking and bobbing on it like it was a real cock.' );
		...
		if( CoC.flags[ kFLAGS.RUBI_NO_CUNT ] === 1 ) {
			MainView.outputText( 'against [rubi eir] tight ass' );
		} else {
			MainView.outputText( 'against [rubi eir] tight pussy' );
		}
		MainView.outputText( ', which resists a little at first, but once the drug has a moment to work, it slips right in.  Rubi actually coos a little as it slips inside.' );
		...
		//(If player has pussy);
		if( CoC.player.hasVagina() ) {
			MainView.outputText( '  Your ' + Descriptors.clitDescript() + ' stiffens, aching for attention.  It peeks from your folds like a shy little lewd faerie.' );
		}
		//(If player has a cock);
		if( CoC.player.hasCock() ) {
			MainView.outputText( '  Meanwhile, [eachCock] stiffens, throbbing as the drug overtakes it.' );
		}
		if( CoC.player.lactationQ() > 0 ) {
			MainView.outputText( '  As the warmth spreads up your body, not even your breasts are left alone.  Your nipples harden and begin leaking milk, which pours down the sides of your body and is soaked up into the cushions beneath you.' );
		}
		...
		//(Normal/Incubus Rubi, Orgasm Denial off);
			...
			if( !this.rubiIncubus() ) {
				MainView.outputText( 'little cock' );
			} else {
				MainView.outputText( 'demonic cock' );
			}
			MainView.outputText( ' twitches, launching jets of cum onto [rubi eir] stomach, chest and face.  In [rubi eir] sex-filled stupor, [rubi ey] eagerly swallows down whatever lands in [rubi eir] mouth, and actually uses [rubi eir] fingers to collect the rest , gulping that down as well.' );
		}
		//(Bimbo/Female Rubi);
		else {
		...
		//(PC has cock?);
		if( CoC.player.hasCock() ) {
			MainView.outputText( '[eachCock], stroking it fervently, in desperate need of release.' );
		}//(PC doesn't have cock, but has pussy?);
		else if( CoC.player.hasVagina() ) {
			MainView.outputText( 'your ' + Descriptors.clitDescript() + ', rubbing it frantically, in desperate need of release.' );
		}//(PC doesn't have either, but breasts above A-cup?);
		else {
			MainView.outputText( 'your [chest], rubbing your nipples vigorously, in desperate need of release.' );
		}
		//(Bonus, fuckable nipples?);
		if( CoC.player.hasFuckableNipples() ) {
			MainView.outputText( '  Your fingers easily slip inside each nipple, pumping in and out like little cocks.' );
		}
		//(No cock, no pussy, no breasts?);
		if( CoC.player.gender === 0 && CoC.player.biggestTitSize() === 0 ) {
			MainView.outputText( '  Your hands roam all along your body, from your bare crotch to your nipples, even through your hair, all in search of something to give you the release you so desperately crave.' );
		}
		...
		//(PC has cock?);
		if( CoC.player.hasCock() ) {
			MainView.outputText( '  [EachCock] twitches and shudders, loosing several jets of cum onto your body, ' );
			if( CoC.player.cumQ() < 250 ) {
				MainView.outputText( 'splattering messily' );
			} else if( CoC.player.cumQ() < 500 ) {
				MainView.outputText( 'coating you lightly' );
			} else if( CoC.player.cumQ() < 1000 ) {
				MainView.outputText( 'covering you in a copious layer of cum' );
			} else {
				MainView.outputText( 'covering you in a thick layer of your potent seed' );
			}
			MainView.outputText( '.' );
		}
		//(PC has pussy?);
		if( CoC.player.hasVagina() ) {
			MainView.outputText( '  Your ' + Descriptors.vaginaDescript( 0 ) + ' clenches down on the enormous invader, which at this point feels like an enormous steel rod lodged firmly inside you, and a spray of fem-juices erupts from your nethers, coating the already slick dildo and Rubi.' );
		}
		//(PC lactating?);
		if( CoC.player.lactationQ() > 0 ) {
			MainView.outputText( '  Your breasts shudder and quake as the orgasmic tsunami washes over them, spraying your sweet smelling milk all over yourself, Rubi and the couch.' );
		}
		...
		MainView.outputText( ' with a slight smile on [rubi eir] face, and says, "<i>Well, that was interesting.</i>"  You laugh at the understatement of the century, and stand, helping Rubi up off the couch as well.' );
	...
	Rubi.prototype.teaseRubi = function() {
		MainView.outputText( ImageManager.showImage( 'rubi-tease' ), false );
		MainView.clearOutput();
		this.rubiSprite();
		//(Normal and Incubus Rubi.  Activates Orgasm Denial and adds 1 to Blue Balls.);
		CoC.flags[ kFLAGS.RUBI_ORGASM_DENIAL ] = 1;
		if( CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] > 0 ) {
			MainView.outputText( 'Having something quite different in mind, you order Rubi to strip down.  With a curious look on [rubi eir] face, [rubi ey] does so, slowly.  Each garment [rubi ey] peels from [rubi eir] soft ' );
			if( CoC.flags[ kFLAGS.RUBI_SKIN ] === 0 ) {
				MainView.outputText( 'pale' );
			} else if( CoC.flags[ kFLAGS.RUBI_SKIN ] === 2 ) {
				MainView.outputText( 'fur-covered' );
			} else {
				MainView.outputText( 'red' );
			}
			MainView.outputText( ' skin seems to take an eternity, until finally [rubi ey]\'s standing in front of you in naught but [rubi eir] panties.' );
			MainView.outputText( '  [rubi Eir] [rubi cock] seems to strain at the panties, aching to be let free.  Standing, you move behind Rubi and sensually whisper into [rubi eir] ear, nibbling and kissing [rubi eir] neck between words.  You tell [rubi em] that, until you specify otherwise, [rubi ey] is not to cum under any circumstances.  If [rubi ey] does, [rubi ey]\'ll be severely punished.  Rubi visibly gulps and nods [rubi eir] assent.' );
			//(Taller than 5');
			...
			//(Normal Rubi);
			MainView.outputText( '  [rubi Ey] blushes, looking back at you with a curious, yet lustful expression.  [rubi Ey] gulps and lifts [rubi eir] tail, giving you free access to [rubi eir] pantied rump.' );
		...
		//Girl Rubi!;
			...
			MainView.outputText( 'Having something quite different in mind, you order Rubi to strip down.  With a curious look on [rubi eir] face, [rubi ey] does so, eagerly.  Each garment [rubi ey] peels from [rubi eir] soft ' );
			if( CoC.flags[ kFLAGS.RUBI_SKIN ] === 0 ) {
				MainView.outputText( 'pale' );
			} else if( CoC.flags[ kFLAGS.RUBI_SKIN ] === 1 ) {
				MainView.outputText( 'crimson' );
			} else if( CoC.flags[ kFLAGS.RUBI_SKIN ] === 2 ) {
				MainView.outputText( 'furred' );
			} else {
				MainView.outputText( 'striped' );
			}
			MainView.outputText( ' skin seems to take an eternity.  In fact, [rubi ey] seems to take great delight in stripping, giving little twirls and tossing [rubi eir] clothes towards you, until [rubi ey]\'s standing in front of you in naught but [rubi eir] birthday suit.' );
			...
			MainView.outputText( '  "<i>Ooh, YES, right there,</i>" Rubi gasps.  Your hand pinches [rubi eir] clit tight, eliciting a yelp and a coo of pleasure at the same time.' );
			...
			if( this.rubiBimbo() && CoC.flags[ kFLAGS.RUBI_DEBIMBO ] === 0 ) {
				MainView.outputText( 'like, ' );
			}
			MainView.outputText( 'cum!</i>"' );
	...
	Rubi.prototype.releaseRubi = function() {
		MainView.outputText( ImageManager.showImage( 'rubi-orgasm-denial-release' ), false );
		MainView.clearOutput();
		this.rubiSprite();
		//Will deactivate Orgasm Denial.  To start Denying again, the PC will have to Tease.;
		CoC.flags[ kFLAGS.RUBI_ORGASM_DENIAL ] = 0;
		MainView.outputText( 'You order Rubi to strip down.  Knowing the drill, [rubi ey] does so slowly.  There\'s no striptease, no show, [rubi ey] just disrobes as quickly as possible until [rubi ey]\'s standing in naught but [rubi eir] panties, ' );
		if( CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] > 0 ) {
			MainView.outputText( '[rubi eir] [rubi cock] straining at the seams, trying to break free.' );
		} else {
			MainView.outputText( 'clinging tight to [rubi eir] flat little mound.' );
		}
		...
		if( CoC.flags[ kFLAGS.RUBI_BLU_BALLS ] <= 3 ) {
			MainView.outputText( '  Not to mention [rubi eir] tiny balls' );
			if( CoC.flags[ kFLAGS.RUBI_BALLS_TYPE ] === 1 ) {
				MainView.outputText( ', trapped in their little sissy sack' );
			}
			MainView.outputText( ', which you think have swelled somewhat, although you haven\'t been teasing [rubi em] long.' );
		}
		//(if Blueballs 4-9);
		else if( CoC.flags[ kFLAGS.RUBI_BLU_BALLS ] <= 9 ) {
			MainView.outputText( '  Not to mention [rubi eir] sizeable balls, which seem to have swelled up to twice their normal size, thanks to your relentless teasing.  They even have a slight bluish tinge to them, although it\'s not immediately visible.' );
		} else {
			MainView.outputText( '  Not to mention [rubi eir] enormous balls, which, thanks to your endless teasing, are seemingly the size of baseballs.  They even have a noticeable blue tinge to them, obviously aching for relief.' );
		}
		...
		if( CoC.flags[ kFLAGS.RUBI_BLU_BALLS ] <= 3 ) {
			...
			MainView.outputText( 'slightly blue' );
		} else {
			MainView.outputText( 'incredibly swollen blue' );
		}
		MainView.outputText( ' balls.  Once it\'s properly lubed up, you press two fingers inside.' );
		...
		MainView.outputText( '.  With all three fingers you press and massage [rubi eir] prostate, increasing the amount of pre-cum flowing from [rubi eir] tip.' );
		...
		//(Blueballs 1-3);
		if( CoC.flags[ kFLAGS.RUBI_BLU_BALLS ] <= 3 ) {
			MainView.outputText( '  It\'s quickly followed by another, less forceful jet of pearly white cum, which lands on [rubi eir] face, and several even smaller jets which splatter the area around [rubi em].  [rubi Eir] cock twitches and spasms, then slowly softens.' );
		}
		//(Blueballs 4-5);
		else if( CoC.flags[ kFLAGS.RUBI_BLU_BALLS ] <= 5 ) {
			MainView.outputText( '  It\'s quickly followed by another, less forceful jet of pearly white cum, then another, and another, each landing closer to [rubi eir] face.  Finally, it gives a few more token jerks, spurting out tiny globules of cum onto the surrounding area.  [rubi Eir] cock twitches and spasms, squirting nothing but air, as if [rubi eir] body is still craving release.' );
		}
		//(Blueballs 6-7);
		else if( CoC.flags[ kFLAGS.RUBI_BLU_BALLS ] <= 7 ) {
			MainView.outputText( '  It\'s quickly followed by another, more forceful jet of pearly white cum which splatters against the ceiling, dripping down onto the both of you.  Another spurt of cum bursts forth, then another, and another, each one liberally covering Rubi\'s stomach, chest and face.  [rubi Eir] cock finally gives a few last twitches and spasms, squirting nothing but air, as if [rubi eir] body is still craving release.' );
		}
		//(Blueballs 8-9);
		else if( CoC.flags[ kFLAGS.RUBI_BLU_BALLS ] <= 9 ) {
			MainView.outputText( '  It\'s quickly followed by another, more forceful jet of pearly white cum, which splatters against the ceiling, dripping down onto the both of you, and then another, and another.  It isn\'t long before the ceiling has a huge white cum-covered blotch right above you, drizzling down on you like dew dripping from trees.  Rubi\'s cock doesn\'t stop there, however, it continues twitching and spurting [rubi eir] seed until [rubi ey] is completely covered in the stuff.  Finally, mercifully, it ends, though [rubi eir] [rubi cock] continues to spasm, squirting nothing but air, as if [rubi eir] body is still craving release.' );
		}
		//(Blueballs 10+);
		else {
			MainView.outputText( '  It\'s quickly followed by another, more powerful jet of pearly white cum, which splatters against the ceiling, dripping down onto the both of you, and then another, and another.  It isn\'t long before the ceiling has a huge white cum-covered blotch right above you, drizzling down on you like dew dripping from the trees.  Rubi\'s cock doesn\'t stop there, however, it continues twitching and spurting [rubi eir] seed, liberally covering [rubi em]self, you and the surrounding area until everything appears to be in a white haze.' );
		}
		...
		if( CoC.flags[ kFLAGS.RUBI_BLU_BALLS ] <= 5 ) {
			MainView.outputText( 'You clean yourself up, leave Rubi a little "<i>See you next time</i>" note, and head out the door, back to camp.' );
		}
		//(Blueballs 6-9);
		else if( CoC.flags[ kFLAGS.RUBI_BLU_BALLS ] <= 9 ) {
			MainView.outputText( 'You head into the bathroom and wash the cum off you, taking a short shower.  Afterwards, you dry up, leave Rubi a little "<i>See you next time</i>" note, and head out the door, back to camp.' );
		}
		//(Blueballs 10+);
		else {
			MainView.outputText( 'You head into the bathroom and wash the cum off you, no easy feat to be sure.  Afterwards, you dry up, leave Rubi a little "<i>See you next time</i>" note, and head out the door, back to camp.  You certainly don\'t want to be there when [rubi ey] wakes up, lest you have to clean the entire house with [rubi em].' );
		}
	...
	Rubi.prototype.cheatingRubi = function() {
		...
		MainView.outputText( 'You take a seat at a table, expectantly waiting for Rubi to slide into your lap...  but she never does.  You look around the place for her, but again, there\'s no sight of her.  You flag down Dia, the caninemorph waitress, who looks haggard, and inquire as to the location of her co-worker.' );
	...
	Rubi.prototype.dontCareAboutNoCheatingRubis = function() {
		MainView.clearOutput();
		this.rubiSprite();
		MainView.outputText( 'You shrug your shoulders.  It\'s not really your problem, is it?  You\'re free to fuck who you choose, and so is she.' );
	...
	Rubi.prototype.findBimboCheatster = function() {
		MainView.clearOutput();
		this.rubiSprite();
		MainView.outputText( 'You let out a disgruntled sigh.  Rubi\'s seeing someone else?!  Not on your watch!  You leave the bakery and make your way towards her neighborhood.  It doesn\'t take long, you\'ve gone with her there enough times to memorize the path, even without her leading.' );
	...
	Rubi.prototype.interruptTheNTRsYouCrazyFool = function() {
		MainView.clearOutput();
		this.rubiSprite();
		MainView.outputText( 'No one\'s sticking their dick in your ' );
		if( EngineCore.silly() ) {
			MainView.outputText( 'waifu' );
		} else {
			MainView.outputText( 'slut' );
		}
		MainView.outputText( ' if you\'ve got a say in it!  You get out of your hiding place, shout a word of warning and burst inside the vibrant pink house.' );
	...
	Rubi.prototype.waitAndGetNTRedLikeTheBoyBitchYouAre = function() {
		...
		MainView.outputText( 'The two go on like this for five minutes, then ten, then fifteen.  Each position, you find, is more intriguing than the last.  ' );
		if( EngineCore.silly() ) {
			MainView.outputText( 'They\'re even doing positions that aren\'t available in the game!  ' );
		}
		MainView.outputText( 'Half an hour later, Rubi and the wolfman sit exhausted on the couch, his seed pouring from each of her holes.' );
	...
	Rubi.prototype.NTRbimboBitchResolution = function() {
		MainView.clearOutput();
		this.rubiSprite();
		MainView.outputText( 'You look at Rubi and say that you\'re definitely going to have to talk about what just happened.' );
	...
	Rubi.prototype.noBimboNTR = function() {
		MainView.clearOutput();
		this.rubiSprite();
		MainView.outputText( 'Folding your arms, you shake your head.  You tell her that if this is going to work out, she has to remain faithful to you.  Sex toys and masturbation will have to do to keep her "<i>urges</i>" under control.  She looks a little sad, but nods as you speak.' );
	...
	Rubi.prototype.yesBimboNTR = function() {
		MainView.clearOutput();
		this.rubiSprite();
		MainView.outputText( 'You contemplate it for a moment, and then slowly nod your head.  What\'s the harm in letting her blow off steam?  You know she\'ll always come back to you.  You assent to her wishes, to which she squeals and hugs you tightly, tossing the pink cushion aside.' );
	...
	Rubi.prototype.breakUpWithRubi = function() {
		MainView.clearOutput();
		this.rubiSprite();
		MainView.outputText( 'You think about it for a long moment and then sigh sadly.  You look Rubi dead in the eyes and explain you can\'t be with a woman who wants to cheat on her partner.  Tears form in her eyes as she sniffles, but she nods with you.' );
	...
	Rubi.prototype.pimpOutRubi = function() {
		MainView.clearOutput();
		this.rubiSprite();
		MainView.outputText( 'You think about it for a long while, mulling over all the possibilities.  Then something else occurs to you. She wants help controlling her urges, why not make a little money in the meantime?  You explain that she\'s free to sleep with whoever she wants, but that she could charge them for her time.  After all, if you\'re going to do what you love, you may as well get paid for it.' );
	...
	Rubi.prototype.chocoRubiMassage = function() {
		...
		if( CoC.player.cor >= 66 ) {
			MainView.outputText( 'start sensually undressing, turning the simple act into a seductive show that causes Rubi\'s petite boner to jerk unpredictably beneath [rubi eir] clothes.  A small, damp stain appears at the tip by the time you finish your salacious tease.' );
		} else if( CoC.player.cor >= 33 ) {
			MainView.outputText( 'get undressed without much pause or fanfare.  Rubi makes a show of turning [rubi eir] head, but you can see [rubi em] peeping glimpses out of the corners of [rubi eir] eyes, and [rubi eir] boner seems to be bigger and harder than it was moments before.' );
		} else {
			MainView.outputText( 'make [rubi em] turn around while you undress.  You quickly shuck your clothes and ask [rubi em] what to do next, holding a hand across your [chest] and groin' );
			if( CoC.player.biggestCockArea() >= 40 || CoC.player.biggestTitSize() >= 8 ) {
				MainView.outputText( ', not that it does much good' );
			}
			MainView.outputText( '.' );
		}
	...
	Rubi.prototype.releaseRubiMassage = function() {
		MainView.clearOutput();
		this.rubiSprite();
		MainView.outputText( 'You figure after all the pampering [rubi ey] did, [rubi ey] deserves a treat.' );
	...
	Rubi.prototype.goInRubisClosetSoThatYouCanComeOutOfTheCloset = function() {
		MainView.clearOutput();
		this.rubiSprite();
		MainView.outputText( 'You ask to see Rubi\'s wardrobe.  [rubi Eir] eyes light up and [rubi ey] claps [rubi eir] hands together excitedly.  "<i>' );
		if( this.rubiBimbo() && CoC.flags[ kFLAGS.RUBI_DEBIMBO ] === 0 ) {
			MainView.outputText( 'Oh my god!  ' );
		}
		MainView.outputText( 'Really?  It\'s right over here!</i>"  Rubi clasps your hand and drags you into the bedroom, then through another doorway.' );
	...
	Rubi.prototype.rubiAppearance = function() {
		MainView.outputText( ImageManager.showImage( 'rubi-inspect-appearance' ), false );
		MainView.clearOutput();
		this.rubiSprite();
		MainView.outputText( 'You look over your naked, demon-morph lover.' );
		MainView.outputText( '\n\nRubi is about five feet tall, with a very lithe, feminine body.  [rubi Eir] skin is ' );
		if( CoC.flags[ kFLAGS.RUBI_SKIN ] === 0 ) {
			MainView.outputText( 'pale, almost like porcelain' );
		} else if( CoC.flags[ kFLAGS.RUBI_SKIN ] === 1 ) {
			MainView.outputText( 'an exotic cherry red' );
		} else if( CoC.flags[ kFLAGS.RUBI_SKIN ] === 2 ) {
			MainView.outputText( 'covered with a fine layer of silky-smooth pink fur' );
		} else {
			MainView.outputText( 'an exotic cherry red which contrasts well with a grey belly and stripes that run up and down [rubi eir] legs and arms' );
		}

		if( CoC.flags[ kFLAGS.RUBI_SKIN ] !== 2 ) {
			MainView.outputText( ' and completely hairless, except for [rubi eir]' );
		} else {
			MainView.outputText( ', and [rubi ey] sports' );
		}
		...
		MainView.outputText( ' hair, which [rubi ey] keeps cut in a cute, feminine style.  Rubi\'s face is soft and rounded' );
		if( CoC.flags[ kFLAGS.RUBI_WHISKERS ] > 0 ) {
			MainView.outputText( ', with a cute set of whiskers sprouting from [rubi eir] cheeks' );
		}
		MainView.outputText( ', [rubi eir] lips painted a ' );
		//(normal/furry skin);
		if( CoC.flags[ kFLAGS.RUBI_SKIN ] === 0 || CoC.flags[ kFLAGS.RUBI_SKIN ] === 2 ) {
			MainView.outputText( 'crimson red' );
		}
		//(red/siren skin);
		else {
			MainView.outputText( 'dusky black' );
		}
		MainView.outputText( ', [rubi eir] ' );
		//if(CoC.flags[kFLAGS.RUBI_EYE_TYPE] === 1) MainView.outputText('slitted ');;
		if( CoC.flags[ kFLAGS.RUBI_EYE_TYPE ] === 0 || this.rubiBimbo() ) {
			MainView.outputText( 'green' );
		} else if( CoC.flags[ kFLAGS.RUBI_EYE_TYPE ] === 1 ) {
			MainView.outputText( 'solid black' );
		}
		//else outputText('black');;
		MainView.outputText( ' eyes framed with eyeliner, belying [rubi eir] masculine origins.  ' );
		if( CoC.flags[ kFLAGS.RUBI_HORNTYPE ] > 0 ) {
			MainView.outputText( 'Two tiny horns extend upwards from [rubi eir] hairline, solid black in color.  ' );
		}
		MainView.outputText( 'A pair of ' );
		if( CoC.flags[ kFLAGS.RUBI_EAR_TYPE ] === 0 ) {
			MainView.outputText( 'cute, normal ears protrude from the sides of [rubi eir] head, each one pierced with a small dangling earring' );
		} else if( CoC.flags[ kFLAGS.RUBI_EAR_TYPE ] === 1 ) {
			MainView.outputText( 'cute, cat-like ears pop up from the top of [rubi eir] head, each one seemingly able to move independently' );
		} else if( CoC.flags[ kFLAGS.RUBI_EAR_TYPE ] === 2 ) {
			MainView.outputText( 'feathered ears protrude from the sides of [rubi eir] head, each one fluttering cutely from time to time, and each pierced with a small dangling earring' );
		}
		MainView.outputText( '.' );
		MainView.outputText( '\n\n[rubi Ey] has a ' );
		if( CoC.flags[ kFLAGS.RUBI_BREAST_SIZE ] < 1 ) {
			MainView.outputText( 'flat-chest ' );
		} else {
			MainView.outputText( '[rubi breasts]' );
		}
		MainView.outputText( ' supporting two perky ' );
		...
		if( CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] > 0 ) {
			MainView.outputText( this.rubiCock() + ', which starts to perk up under your scrutiny. From experience, you know it can reach a impressive ' + CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] + ' inches when erect.  ' );
			MainView.outputText( 'Under [rubi eir] cock swings a set of ' );
			if( CoC.flags[ kFLAGS.RUBI_BLU_BALLS ] >= 4 && CoC.flags[ kFLAGS.RUBI_BLU_BALLS ] < 9 ) {
				MainView.outputText( 'cum-swollen ' );
			} else if( CoC.flags[ kFLAGS.RUBI_BLU_BALLS ] >= 9 ) {
				MainView.outputText( 'engorged, blue-tinged ' );
			}
			MainView.outputText( 'testicles' );
			if( CoC.flags[ kFLAGS.RUBI_BALLS_TYPE ] === 1 ) {
				MainView.outputText( ', which cling tight to [rubi eir] body in a cute spherical pouch' );
			}
		}
		...
		if( CoC.flags[ kFLAGS.RUBI_NO_CUNT ] === 0 ) {
			MainView.outputText( 'cute ' );
			if( CoC.flags[ kFLAGS.RUBI_CUNTTYPE ] > 0 ) {
				MainView.outputText( 'black ' );
			}
			MainView.outputText( 'pussy topped off by a little love button' );
			MainView.outputText( '.  Though you can\'t see it, you know Rubi\'s got a perfect little asshole, between [rubi eir] buttcheeks, right where it belongs.' );
		} else {
			MainView.outputText( '.' );
		}
		MainView.outputText( '\n\nSwaying behind [rubi em] is a red spaded tail, which coils around [rubi eir] leg occasionally.  ' );
		if( CoC.flags[ kFLAGS.RUBI_FEET ] === 1 ) {
			MainView.outputText( 'Finally, Rubi stands on a long black spike that makes [rubi em] walk as though [rubi ey] were wearing high heels.' );
		}
	...
	Rubi.prototype.playDressUp = function() {
		...
		if( closet.length > 0 ) {
			MainView.outputText( '<b>Rubi\'s Closet Contains:</b>\n' );
			_.forEach( closet, function( item ) {
				MainView.outputText( item + '\n' );
			} );
			MainView.outputText( '\n' );
		}
		var button = 0;
	...
	Rubi.prototype.putRubiInAWaitressUniform = function() {
		MainView.clearOutput();
		this.rubiSprite();
		MainView.outputText( 'You tell Rubi you\'d like to see [rubi em] in the waitress uniform.  "<i>Hm, well all right, but it\'s just my work clothes,</i>" [rubi ey] replies.' );
		MainView.outputText( '\n\nRubi starts off with a white bra trimmed with lace' );
		if( CoC.flags[ kFLAGS.RUBI_BREAST_SIZE ] < 1 ) {
			MainView.outputText( ', though it\'s obvious [rubi ey] doesn\'t need it with [rubi eir] flat-chest' );
		}
		MainView.outputText( '.  [rubi Ey] slips [rubi eir] arms through the holes and expertly latches the back.  Next [rubi ey] takes a black garter belt and slips it up [rubi eir] slender ' );
		if( CoC.flags[ kFLAGS.RUBI_SKIN ] !== 2 ) {
			MainView.outputText( 'hairless' );
		} else {
			MainView.outputText( 'furred' );
		}
		MainView.outputText( ' legs, until it rests snugly around [rubi eir] waist.  A matching pair of sheer stockings quickly follow, held up by the garter.' );
		if( !this.rubiBimbo() || CoC.flags[ kFLAGS.RUBI_DEBIMBO ] > 0 ) {
		...
		MainView.outputText( '  Rubi approaches a rack of clothes, searching quietly for a moment before picking two hangars off it: a pink blouse and a matching pink skirt.' );
		MainView.outputText( '\n\nThe blouse goes on first, followed by the skirt.  Rubi only buttons the blouse about half way up' );
		if( CoC.flags[ kFLAGS.RUBI_BREAST_SIZE ] < 1 ) {
			MainView.outputText( ', showing off some of [rubi eir] flat chest' );
		} else {
			MainView.outputText( ', showing off a generous portion of cleavage' );
		}
		MainView.outputText( ', and tucks the hem of the blouse into the skirt.  Finally, Rubi selects a pair of ' );
		if( CoC.flags[ kFLAGS.RUBI_FEET ] === 1 ) {
			MainView.outputText( 'what appears to be flats, with a hole in the heel for [rubi eir] demonic heels' );
		} else {
			MainView.outputText( 'short heels' );
		}
		MainView.outputText( ' and slips them on.  [rubi Ey] gives a little twirl in the waitress uniform, and bends over several times to show off [rubi eir] ' );
		if( !this.rubiBimbo() || CoC.flags[ kFLAGS.RUBI_DEBIMBO ] > 0 ) {
			MainView.outputText( 'girly panties' );
		} else {
			MainView.outputText( 'naked nethers' );
		}
		MainView.outputText( '.' );
	...
	Rubi.prototype.putOnADashingOutfitYouWhore = function() {
		MainView.clearOutput();
		this.rubiSprite();
		MainView.outputText( 'You tell Rubi you\'d like to see [rubi em] in the dashing outfit.  "<i>Ohh, feeling like plundering a little booty today?</i>" [rubi ey] asks with a coy smile.' );
		...
		if( CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] > 0 ) {
			MainView.outputText( '  Combined with the sizeable bulge in the crotch, Rubi looks like quite the dashing, if slutty, piratess.' );
		}
	...
	Rubi.prototype.goOnAnAnalAdventureRubiNotReallyJustAnAdventurersOutfit = function() {
		MainView.clearOutput();
		this.rubiSprite();
		MainView.outputText( 'You tell Rubi you\'d like to see [rubi em] in the adventurer\'s outfit.  "<i>Ah yes, am I going to be the Champion now?</i>" Rubi asks, balling up [rubi eir] dainty hands and putting them on [rubi eir] hips.  "<i>Rubi, Champion of Mareth!  Slayer of demons and ' );
		if( EngineCore.silly() ) {
			MainView.outputText( 'collector of waifus!' );
		} else {
			MainView.outputText( 'layer of ladies' );
		}
		MainView.outputText( '!  Has a bit of a ring to it doesn\'t it?</i>"  [rubi Ey] giggles for a moment and then gathers up the clothes.' );
		MainView.outputText( '\n\n[rubi Ey] starts with a simple, only slightly lacy white bra' );
		if( CoC.flags[ kFLAGS.RUBI_BREAST_SIZE ] < 1 ) {
			MainView.outputText( ', though it\'s obvious [rubi ey] doesn\'t need it with [rubi eir] flat-chest' );
		}
		MainView.outputText( '.  [rubi Ey] slips [rubi eir] arms through the holes and expertly latches the back.  Following that is a matching pair of panties that [rubi ey] slips up [rubi eir] long, smooth legs.  [rubi Ey] pulls them up so that they hug [rubi eir] round, cute ass tightly.' );
		...
		if( CoC.flags[ kFLAGS.RUBI_EAR_TYPE ] === 1 ) {
			MainView.outputText( ', covering [rubi eir] feline ears' );
		}
		MainView.outputText( '.  The boots that go with the outfit are mid-calf length and made of supple, fine leather' );
		if( CoC.flags[ kFLAGS.RUBI_FEET ] === 1 ) {
			MainView.outputText( ' with a hole cut out in the heel, apparently for Rubi\'s demonic heels to fit through' );
		}
	...
	Rubi.prototype.giveRubiALongDressToHideHerHideousBody = function() {
		MainView.clearOutput();
		this.rubiSprite();
		MainView.outputText( 'You tell Rubi you\'d like to see [rubi em] in the long dress.  "<i>Oohh, I just </i>love<i> that one! One second!</i>" [rubi ey] says as [rubi ey] pulls out the dress and several accessories.' );
		MainView.outputText( '\n\nRubi starts with an off-pink strap-less bra' );
		if( CoC.flags[ kFLAGS.RUBI_BREAST_SIZE ] < 1 ) {
			MainView.outputText( ', despite the fact that [rubi ey] clearly doesn\'t need one' );
		}
		MainView.outputText( ', which [rubi ey] expertly fastens behind [rubi em]' );
		MainView.outputText( '.  Next, [rubi ey] takes a white garter belt and slips it up [rubi eir] slender ' );
		if( CoC.flags[ kFLAGS.RUBI_SKIN ] !== 2 ) {
			MainView.outputText( 'hairless' );
		} else {
			MainView.outputText( 'furred' );
		}
		MainView.outputText( ' legs, until it rests snugly around [rubi eir] waist.  A matching pair of white stockings quickly follow, held up by the garter.  The next thing is an off-pink pair of panties that [rubi ey] slides up [rubi eir] legs and pulls up so that they cling tightly to [rubi eir] ample rump.' );
		...
		if( CoC.flags[ kFLAGS.RUBI_SKIN ] === 1 || CoC.flags[ kFLAGS.RUBI_SKIN ] === 3 ) {
			MainView.outputText( ' (or darkens at least)' );
		}
		MainView.outputText( ', and [rubi ey] returns to the task at hand.  The dress goes on neatly, and once it\'s in place you help to pull the strings on the built-in corset, tying them off with a neat little bow in back. To top it all off is a pair of pink pumps' );
		if( CoC.flags[ kFLAGS.RUBI_FEET ] === 1 ) {
			MainView.outputText( ' with the heels cut off and a hole for Rubi\'s demonic heel to fit through' );
		}
		MainView.outputText( ', which [rubi ey] slips on quickly.' );
	...
	Rubi.prototype.rubiPutsOnNursesClothesSoSheCanCheckYourTemperature = function() {
		MainView.clearOutput();
		this.rubiSprite();
		MainView.outputText( 'You tell Rubi you\'d like to see [rubi em] in the nurse\'s uniform.  "<i>Ohh, coming down with a bit of a fever?  Do you need a sponge bath?</i>" [rubi ey] says coyly, before gathering up the clothing.' );
		MainView.outputText( '\n\n[rubi Ey] starts with a simple, only slightly lacy white bra' );
		if( CoC.flags[ kFLAGS.RUBI_BREAST_SIZE ] < 1 ) {
			MainView.outputText( ', though it\'s obvious [rubi ey] doesn\'t need it with [rubi eir] flat-chest' );
		}
		MainView.outputText( '.  [rubi Ey] slips [rubi eir] arms through the holes and expertly latches the back.  Next [rubi ey] takes a white garter belt and slips it up [rubi eir] slender ' );
		if( CoC.flags[ kFLAGS.RUBI_SKIN ] === 2 ) {
			MainView.outputText( 'furred ' );
		} else {
			MainView.outputText( 'hairless ' );
		}
		MainView.outputText( 'legs, until it rests snugly around [rubi eir] waist.  A matching pair of white fishnet stockings quickly follow, held up by the garter.  The next thing is a pair of white cotton panties that [rubi ey] slides up [rubi eir] legs and pulls up so that they cling tightly to [rubi eir] ample rump.' );
		...
		if( CoC.flags[ kFLAGS.RUBI_BREAST_SIZE ] < 1 ) {
			MainView.outputText( 'heart' );
		} else {
			MainView.outputText( 'left breast' );
		}
		MainView.outputText( '.  [rubi Ey] places the matching hat on [rubi eir] head and adjusts it before putting on a pair of white pumps' );
		if( CoC.flags[ kFLAGS.RUBI_FEET ] > 0 ) {
			MainView.outputText( ' with the heels removed and a hole for [rubi eir] demonic heels' );
		}
		MainView.outputText( '.' );
		MainView.outputText( '\n\nNow that Rubi is dressed up, what do you want to do?' );
		//[Date (go to ice cream date)] [Back];
	...
	Rubi.prototype.rubiHasARubberFetish = function() {
		MainView.clearOutput();
		this.rubiSprite();
		MainView.outputText( 'You tell Rubi you\'d like to see [rubi em] in the rubber fetish outfit.  "<i>Ohoh,</i>" [rubi ey] giggles and quirks an eyebrow at you, "</i>feeling a bit kinky are we?</i>"' );
		...
		if( CoC.flags[ kFLAGS.RUBI_SKIN ] !== 2 ) {
			...
			MainView.outputText( 'furred body' );
		}
		MainView.outputText( ', starting with [rubi eir] legs and moving up to the torso and arms.  Once Rubi\'s ' );
		if( CoC.flags[ kFLAGS.RUBI_SKIN ] !== 2 ) {
			MainView.outputText( 'skin ' );
		} else {
			MainView.outputText( 'fur ' );
		}
		MainView.outputText( 'glistens with the oils it\'s time to put on the actual clothes.  ' );
		//(If Not Bimbo);
		if( !this.rubiBimbo() || CoC.flags[ kFLAGS.RUBI_DEBIMBO ] > 0 ) {
			MainView.outputText( '[rubi Ey] first takes a pair of transparent rubber panties and slides them up [rubi eir] legs until they lovingly cling around [rubi eir] ass.  ' );
		}
		MainView.outputText( 'Rubi next slides into a transparent latex shirt, long sleeved, with holes cut out to accommodate [rubi eir] nipples.  It takes some work, and you have to help Rubi to put on the difficult garment, but once it\'s on, it looks fantastic, clinging to [rubi eir] body like a second skin.' );
		...
		MainView.outputText( 'legs.  You wonder how [rubi ey] does this when you\'re not around!  Finally, Rubi slips on a pair of heels' );
		if( CoC.flags[ kFLAGS.RUBI_FEET ] === 1 ) {
			MainView.outputText( ', the actual heel cut off to make room for [rubi eir] demonic heel,' );
		}
	...
	Rubi.prototype.putOnASuitYouSlut = function() {
		MainView.clearOutput();
		this.rubiSprite();
		MainView.outputText( 'You tell Rubi you\'d like to see [rubi em] in the suit.  "<i>Oh, really?  It looks nice, but you don\'t think it\'s too manly?</i>"' );
		MainView.outputText( '\n\nRubi starts with a black, lacy bra' );
		if( CoC.flags[ kFLAGS.RUBI_BREAST_SIZE ] < 1 ) {
			MainView.outputText( ', despite the fact that [rubi ey] clearly doesn\'t need one' );
		}
		MainView.outputText( ', which [rubi ey] expertly fastens behind [rubi em].  Next, [rubi ey] takes a black garter belt and slips it up [rubi eir] slender ' );
		if( CoC.flags[ kFLAGS.RUBI_SKIN ] !== 2 ) {
			MainView.outputText( 'hairless' );
		} else {
			MainView.outputText( 'furred' );
		...
		if( CoC.flags[ kFLAGS.RUBI_FEET ] === 1 ) {
			MainView.outputText( ', with the actual heel cut off to make room for [rubi eir] demonic heel,' );
		}
	...
	Rubi.prototype.putOnATubeTopYouWhore = function() {
		MainView.clearOutput();
		this.rubiSprite();
		MainView.outputText( 'You tell Rubi you\'d like to see [rubi em] in the tube top and shorts.  "<i>Ooh, I like that one.  It\'s a little risque, but so cute!</i>"' );
		MainView.outputText( '\n\nRubi starts off with what seems to be the skimpiest g-string you\'ve ever seen.  [rubi Ey] steps into the tiny underwear and pulls them up, ' );
		if( CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] > 0 ) {
			MainView.outputText( 'tucking [rubi eir] cock firmly into the triangular strip of cloth covering [rubi eir] crotch, while ' );
		}
		MainView.outputText( 'the string gets firmly lodged between Rubi\'s butt cheeks.  This is followed up by an impossibly small pair of denim shorts which slide up [rubi eir] slender legs and barely cover the g-string already worn.  Rubi pulls at the side strings of the skimpy underwear, making sure they\'re plainly visible over the denim shorts.' );
		...
		if( CoC.flags[ kFLAGS.RUBI_BREAST_SIZE ] > 2 ) {
			MainView.outputText( ', which strain the flimsy piece of material' );
		}
		MainView.outputText( '.  Completing the look is a simple pair of sandals' );
		if( CoC.flags[ kFLAGS.RUBI_FEET ] === 1 ) {
			MainView.outputText( ', with a hole cut in the sole to make room for Rubi\'s demonic heels' );
		}
		MainView.outputText( '.' );
		MainView.outputText( '\n\nNow that Rubi is dressed up, what do you want to do?' );
	...
	Rubi.prototype.putOnSluttySwimwearYouSkank = function() {
		MainView.clearOutput();
		this.rubiSprite();
		MainView.outputText( 'You tell Rubi you\'d like to see [rubi em] in the swimsuit.  "<i>Oh, but there\'s no pool or lake nearby.  Or do you just want to see me in a bikini?</i>"  Rubi giggles a moment.' );
		...
		if( CoC.flags[ kFLAGS.RUBI_BREAST_SIZE ] < 3 ) {
			MainView.outputText( ' sit loosely over Rubi\'s small chest' );
		} else {
			MainView.outputText( 'strain thanks to Rubi\'s ample busom' );
		}
		MainView.outputText( '.' );
		MainView.outputText( '\n\nTopping it off, your demonic lover slips on a pair of sandals' );
		if( CoC.flags[ kFLAGS.RUBI_FEET ] > 0 ) {
			MainView.outputText( ', with a hole in the sole to make room for [rubi eir] demonic heels' );
		}
		MainView.outputText( '.' );
		MainView.outputText( '\n\nNow that Rubi is dressed up, what do you want to do?' );
		//[Date (go to exhibitionist date)] [Back];
	...
	Rubi.prototype.putOnADressYouBimbo = function() {
		MainView.clearOutput();
		this.rubiSprite();
		MainView.outputText( 'You tell Rubi you\'d like to see [rubi em] in the minidress.  "<i>Oh that cute little thing we picked up together?  For sure!  It\'s so cute!</i>"' );
		...
		if( CoC.flags[ kFLAGS.RUBI_FEET ] > 0 ) {
			MainView.outputText( ', with the heels removed to make room for [rubi eir] own demonic heels' );
		}
		MainView.outputText( '.' );
	...
	Rubi.prototype.putOnMyBodysuitYouWhore = function() {
		MainView.clearOutput();
		this.rubiSprite();
		MainView.outputText( 'You tell Rubi you\'d like to see [rubi em] in the bodysuit.  "<i>Oh, that thing is pretty cute,</i>" [rubi ey] says, "<i>Though there\'s not much to it.  But I suppose that\'s the point, huh?</i>"' );
		...
		MainView.outputText( ' legs, then up the rest of [rubi eir] body until [rubi ey] slips [rubi eir] arms into the long, sheer sleeves.  After a few adjustments, making sure ' );
		if( CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] > 0 ) {
			MainView.outputText( '[rubi eir] package is neatly tucked away' );
		} else {
			MainView.outputText( 'a red rose is neatly covering up [rubi eir] pubic mound' );
		}
	...
	Rubi.prototype.putOnBondageStrapsYouBondageTrap = function() {
		MainView.clearOutput();
		this.rubiSprite();
		MainView.outputText( 'You tell Rubi you\'d like to see [rubi em] in the bondage straps.  "<i>Oh!  That outfit, huh?</i>"  Rubi blushes brightly' );
		if( CoC.flags[ kFLAGS.RUBI_SKIN ] > 0 ) {
			MainView.outputText( ', or so you imagine' );
		}
		MainView.outputText( '.  "<i>That one\'s so embarrassing... but for you, I\'ll wear it.</i>"' );
	...
	Rubi.prototype.whyTheFuckIsRubiWearingAnInquisitorsCorset = function() {
		MainView.clearOutput();
		this.rubiSprite();
		MainView.outputText( 'You tell Rubi you\'d like to see [rubi em] in the inquisitor\'s corset.  "<i>Oh my, I really love that one.  It\'s so pretty and lacy!</i>"' );
	...
	Rubi.prototype.putOnAWaitressUniformYouWhore = function() {
		MainView.clearOutput();
		this.rubiSprite();
		MainView.outputText( 'You tell Rubi you\'d like to see [rubi em] in the risque version of [rubi eir] normal waitress uniform.  "<i>Oh, you like that outfit, huh?  I thought you might,</i>" Rubi says with a giggle.' );
		MainView.outputText( '\n\nOf course, there\'s not much to this outfit, which starts off with a skirt so short it could be confused for a belt.  ' );
		//(not bimbo);
		if( !this.rubiBimbo() || CoC.flags[ kFLAGS.RUBI_DEBIMBO ] > 0 ) {
			MainView.outputText( 'Next [rubi ey] slips on a pair of pink frilly panties, sliding them up [rubi eir] slender legs until they come up to cling to [rubi eir] ample ass.  ' );
			if( CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] > 0 ) {
				MainView.outputText( 'Oddly enough, these panties only seem to cover Rubi\'s balls, leaving [rubi eir] cock right out in the open.  ' );
			}
		}
		MainView.outputText( 'Taking a simple white apron, Rubi throws that on next, looping the halter-like top over [rubi eir] neck and tying off the straps around [rubi eir] waist.' );
		if( CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] > 0 ) {
			MainView.outputText( '  To top it all off, Rubi takes a pink lacy cocksock and slips it down [rubi eir] [rubi cock], fastening it into place with a pink ribbon that [rubi ey] ties into a bow around the base of [rubi eir] cock.' );
		}
	...
	Rubi.prototype.dateIntro = function( date ) {
		...
		if( CoC.flags[ kFLAGS.TIMES_RUBI_DATED ] === 0 ) {
			MainView.outputText( 'An idea strikes you.  While you have talked with Rubi and gotten to know [rubi em] at work, you haven\'t really been out with [rubi em] anywhere else.  You wonder, and ask if Rubi would like to go out somewhere, maybe to pick up some food and talk.' );
		...
		//(Repeats);
			...
			MainView.outputText( 'Seeing Rubi all dressed up like this, you figure it might be time to go on another date.  You ask, and Rubi predictably makes a gleeful sound and assents.' );
		}
	...
	Rubi.prototype.fancyDate = function() {
		...
		if( CoC.player.gems >= 10 ) {
			...
			if( CoC.flags[ kFLAGS.MET_OTTERGIRL ] === 1 ) {
				MainView.outputText( ' though not as good as Callu\'s, the ottergirl fisherman you met at the lake.' );
			} else {
				MainView.outputText( ', which makes you wonder who Rubi is thinking about.  Perhaps one of [rubi eir] parents?' );
			}
	...
	Rubi.prototype.exhibitionistDate = function() {
		...
		if( CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] > 0 ) {
			MainView.outputText( this.rubiCock() );
		} else {
			MainView.outputText( 'glistening, wet pussy' );
		}
		MainView.outputText( '.' );
	...
	Rubi.prototype.anallyTrainYourBitchBoySlutHowToBeAnalForDCL = function() {
		...
		if( CoC.flags[ kFLAGS.RUBI_TIMES_ANALLY_TRAINED ] === 1 ) {
			MainView.outputText( 'You sit down on the couch next to Rubi, making small talk.  After a few minutes the two of you are laughing comfortably as you lounge on the cushions, discussing everything from life in Tel\'Adre to your adventuring exploits.  Eventually the topic of conversation circles back to sex, and how you\'re a bit too much for [rubi em] to handle at the moment.' );
			MainView.outputText( '\n\nRubi gulps visibly and nods, "<i>I can\'t help it, babe.  That cock of yours looks so ' );
			if( this.rubiBimbo() || CoC.flags[ kFLAGS.RUBI_DEBIMBO ] === 0 ) {
				MainView.outputText( 'totally ' );
			}
			MainView.outputText( 'delicious, but I\'m afraid it\'s too big!  I couldn\'t possibly fit that entire thing inside me... not comfortably at least.</i>"' );
			...
			if( CoC.flags[ kFLAGS.RUBI_SKIN ] === 1 || CoC.flags[ kFLAGS.RUBI_SKIN ] === 3 ) {
				MainView.outputText( ', or redder at least' );
			} else {
				MainView.outputText( ', what little of it you can see' );
			}
			MainView.outputText( ', and [rubi ey] sits knock-kneed behind the chest.' );
			...
			if( CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] > 0 ) {
				MainView.outputText( ', [rubi eir] dick wobbling enticingly' );
			}
			MainView.outputText( '.' );
			...
			MainView.outputText( 'nipples, which jut straight up, straining against the sheer fabric of the negligee.  A soft gasp escapes Rubi\'s soft lips as [rubi ey] pinches one ' );
			if( CoC.flags[ kFLAGS.RUBI_NIPPLE_TYPE ] === 1 ) {
				MainView.outputText( 'ebon ' );
			}
			MainView.outputText( 'nipple, while [rubi eir] other hand flutters down between [rubi eir] thighs to stroke [rubi eir] ' );
			if( CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] > 0 ) {
				MainView.outputText( this.rubiCock() );
			} else {
				MainView.outputText( 'quickly dampening pussy' );
			}
			MainView.outputText( '.' );
			...
			MainView.outputText( ' lips, licking the ' );
			if( CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] > 0 ) {
				MainView.outputText( 'pre-cum' );
			} else {
				MainView.outputText( 'juices' );
			}
			MainView.outputText( ' off of them with a smile.  [rubi Eir] newly cleaned fingers move to the bottle of lube sitting nearby.  After pumping a handful into [rubi eir] palm, the rich scent of raspberries seems to fill the air, making your nostrils twitch.  Scented lube?' );
			...
			if( CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] === 0 ) {
				MainView.outputText( 'slick vagina' );
			} else {
				MainView.outputText( 'hungry asshole' );
			}
			MainView.outputText( '.' );
			...
			MainView.outputText( ' clinging so tightly to the toy you imagine it must be like a vice.' );
			...
			if( CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] > 0 ) {
				MainView.outputText( this.rubiCock() + ' is standing at full attention, bobbing up and down as Rubi fucks [rubi emself] on the dildo' );
			}
			if( CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] > 0 && CoC.flags[ kFLAGS.RUBI_NO_CUNT ] === 0 ) {
				MainView.outputText( ', and [rubi eir] ' );
			}
			if( CoC.flags[ kFLAGS.RUBI_NO_CUNT ] === 0 ) {
				MainView.outputText( 'cunt lips are glistening red with arousal, just begging for attention' );
			}
			MainView.outputText( '.  Rubi\'s cries grow louder, [rubi eir] lips parting and [rubi eir] tongue lolling out as [rubi ey] pants for breath.  You can tell [rubi ey]\'s building up to something, something powerful at that.' );
			MainView.outputText( '\n\n[rubi Eir] hips spin and oscillate, working vigorously to keep [rubi eir] ' );
			if( CoC.flags[ kFLAGS.RUBI_NO_CUNT ] === 1 ) {
				MainView.outputText( 'backside' );
			} else {
				MainView.outputText( 'fuck hole' );
			}
			MainView.outputText( ' filled with dick, however fake it may be.  The slender, feminine demon\'s voice is so suffused with lustful fervor that it\'s virtually palpable in the warm air permeating the area.  Each near-orgasmic tone that worms its ways from [rubi eir] throat just drips with euphoria and writhes its way into your ears, filling your body with a portion of [rubi eir] passion.' );
			...
			if( CoC.flags[ kFLAGS.RUBI_SKIN ] !== 2 ) {
				MainView.outputText( 'bare' );
			} else {
				MainView.outputText( 'furred' );
			}
			MainView.outputText( ' legs buckle, one jerking like mad, causing [rubi em] to impale [rubi emself] fully onto the imitation shaft of man meat.  Amid the chaos of spasming limbs and newfound modesty, Rubi loses [rubi eir] breath, making small gasping noises when [rubi ey] isn\'t moaning for more.' );
			if( CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] > 0 ) {
			...
			MainView.outputText( ', seeking freedom from its sweet, fleshy prison.  A long supine sigh escapes your slender lover\'s mouth.  [rubi Ey]\'s clearly exhausted and happy for this rapturous release.' );
			...
			if( CoC.flags[ kFLAGS.RUBI_EAR_TYPE ] === 0 ) {
				MainView.outputText( 'ear' );
			} else if( CoC.flags[ kFLAGS.RUBI_EAR_TYPE ] === 1 ) {
				MainView.outputText( 'feline ear' );
			} else {
				MainView.outputText( 'feathered ear' );
			}
			MainView.outputText( ' that [rubi ey] should keep that in for a few hours, just to make sure this training takes hold.  Rubi gives you a small smile in response along with a small peck on the cheek before slipping under the sheets for a quick nap.' );
		...
		//Rubi's capacity won't go larger than 100.;
			...
			if( CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] > 0 ) {
				...
				MainView.outputText( 'sweet pussy' );
			}
			MainView.outputText( ' is readily on display, and it\'s all for you.  Rubi\'s makeup is less than subtle, with bright red lips and a light dusting of silver eyeshadow, enough to make [rubi eir] eyes pop and not clash with the outfit.  Completing the look is a pair of white fishnet stockings that go up [rubi eir] legs, ending about mid-thigh.' );
			...
			MainView.outputText( '\'s mouth, exploring every inch and entwining with Rubi\'s own.' );
			...
			if( CoC.flags[ kFLAGS.RUBI_SKIN ] === 0 ) {
				MainView.outputText( 'go bright red' );
			} else if( CoC.flags[ kFLAGS.RUBI_SKIN ] === 1 || CoC.flags[ kFLAGS.RUBI_SKIN ] === 3 ) {
				MainView.outputText( 'go a vivid purple' );
			} else {
				MainView.outputText( 'would be blushing bright red now, were it not for the fine layer of fur' );
			}
			MainView.outputText( ', and [rubi ey] snaps back to the present.  "<i>Eheh... I guess I deserved that.</i>"' );
			...
			MainView.outputText( ', making sure to cover every inch.  You\'re certain you even see a few fingers press inside [rubi eir] hole.' );
			...
			MainView.outputText( ' is stretched nearly to its limits.  Finally [rubi ey] lets out a sigh of relief as the knot pops completely into place.  "<i>Oh... fuck me it\'s so big,</i>" [rubi ey] squeals, [rubi eir] body visibly shaking.  Rubi\'s tongue lolls out, dripping a bit of spittle down [rubi eir] chin.' );
			...
			MainView.outputText( ' just widened slightly.  Rubi\'s eye go wide as [rubi ey] looks up at you with a mouth full of cock, moaning something you can\'t understand.' );
			...
			if( CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] > 0 ) {
				MainView.outputText( 'You can feel your lover\'s cock straining against your leg, aching for some release.  You idly press your foot against it, feeling a slick layer of precum coating it from top to bottom already.  ' );
			}
			MainView.outputText( 'A pleading look enters Rubi\'s eyes.  You hold up the pump and ask [rubi em] if [rubi ey] wants you to use it again.  [rubi Ey] gives [rubi eir] head a quick nod of assent, and reaffirms this by taking your [cock biggest] even deeper into [rubi eir] mouth.' );
			...
			if( CoC.flags[ kFLAGS.RUBI_EYE_TYPE ] === 0 || this.rubiBimbo() ) {
				...
				MainView.outputText( 'shiny black' );
			}
			MainView.outputText( ' eyes close tight as [rubi ey] succumbs to a brain-melting orgasm.  [rubi Eir] entire body trembles, from [rubi eir] curling toes right up [rubi eir] twitching, painted fingers.' );
			if( CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] > 0 ) {
				MainView.outputText( '  Even, no, <i>especially</i> [rubi eir] cock twitches and spasms jerking upwards and splattering globules of sticky white goo against the inside of Rubi\'s babydoll before dripping down onto the floor between [rubi eir] legs, creating a rapidly growing puddle.' );
			}
			...
			MainView.outputText( ', to make [rubi em] squeal from your actions, and not that of a lifeless dildo.  You imagine the ' + this.rubiMF( 'demon', 'demoness' ) + '\'s hot, wet walls clamped around you, milking you for everything you\'ve got.' );
	...
	Rubi.prototype.getFuckedByRubi = function() {
		MainView.outputText( ImageManager.showImage( 'rubi-fucks-you' ), false );
		MainView.clearOutput();
		this.rubiSprite();
		MainView.outputText( 'Rather than wait for you to initiate something, Rubi takes you by the hands and leads you into the bedroom.  [rubi Ey] puts one arm on your shoulder and runs [rubi eir] fingers up through the hair on the back of your head before pulling you into a deep kiss.  Rubi\'s ' );
		//(normal/furry skin);
		if( CoC.flags[ kFLAGS.RUBI_SKIN ] === 0 || CoC.flags[ kFLAGS.RUBI_SKIN ] === 2 ) {
			MainView.outputText( 'red' );
		} else {
		...
		MainView.outputText( ' lips feel so delicate against yours, but they quickly part as [rubi eir] tongue slithers between them, exploring the depths of your warm, wet mouth.' );
		...
		if( CoC.flags[ kFLAGS.RUBI_EYE_TYPE ] > 0 ) {
			MainView.outputText( ' slitted' );
		}
		MainView.outputText( ' eyes constantly meet yours, looking for approval, before and after every act.  You\'re certain you\'re still the one in charge here... but you\'re simply letting Rubi take the lead.' );
		...
		MainView.outputText( ' of yours all ready for me.</i>"  Rubi\'s head descends between your legs' );
		if( CoC.player.hasCock() ) {
			MainView.outputText( ', ignoring your ' + Descriptors.multiCockDescriptLight() + ' completely,' );
		}
		MainView.outputText( ' and presses [rubi eir] face into your [vagOrAss], inhaling deeply.  [rubi Eir] lips pucker as [rubi ey] plants a kiss directly onto your ' );
		if( !CoC.player.hasVagina() ) {
			MainView.outputText( 'ass' );
		} else {
			MainView.outputText( 'moistening pussy' );
		}
		MainView.outputText( '.  You let out a small moan and lean backwards, taking your eyes off the ' );
		if( CoC.flags[ kFLAGS.RUBI_HAIR ] === 0 ) {
			MainView.outputText( 'black-haired' );
		} else {
			MainView.outputText( 'blonde' );
		}
		MainView.outputText( ' head betwixt your thighs and taking in the sensations.' );
		...
		if( this.rubiGetCockType() === CockTypesEnum.HUMAN ) {
			MainView.outputText( 'pink, mushroom-like tip' );
		} else if( this.rubiGetCockType() === CockTypesEnum.DEMON ) {
			MainView.outputText( 'purple, nodule-laden tip' );
		} else if( this.rubiGetCockType() === CockTypesEnum.HORSE ) {
			MainView.outputText( 'blunted, musky tip' );
		} else if( this.rubiGetCockType() === CockTypesEnum.TENTACLE ) {
			MainView.outputText( 'tentacle-ringed crown' );
		} else {
			MainView.outputText( '<b>ERROR: Rubi Cock Type set invalid. Currently: ' + this.rubiGetCockType() + '</b>  ' );
		}
		MainView.outputText( ' emerges from the foreskin surrounding it as [rubi eir] cock strains, aching for something, anything to fill.' );
		...
		MainView.outputText( ' briefly, but enough to send a hot shiver up your spine.  Seeing this reaction, Rubi\'s smile widens into a grin and begins to thrust in and out at a slow pace.  Every thrust brushes past your sensitive ' );
		if( !CoC.player.hasVagina() ) {
			MainView.outputText( 'prostate' );
		} else {
			MainView.outputText( 'g-spot' );
		}
		MainView.outputText( ', and one by one your limbs start to tingle.  In and out, [rubi ey] goes, and you can feel something building inside you.' );
		...
		MainView.outputText( ' eyes, "<i>Can I come inside you?</i>"' );
		...
		if( CoC.flags[ kFLAGS.RUBI_BLU_BALLS ] < 4 ) {
			...
			MainView.outputText( 'There\'s so much of it that it oozes out from your abused ' );
			if( !CoC.player.hasVagina() ) {
				MainView.outputText( 'ass' );
			} else {
				MainView.outputText( 'cunt' );
			}
			MainView.outputText( ', dribbling down your buttcheeks and pooling onto the bed and floor below.  ' );
		}
		//(Blueballs 6-9);
		else if( CoC.flags[ kFLAGS.RUBI_BLU_BALLS ] <= 9 ) {
			MainView.outputText( 'Your insides rumble suddenly as Rubi\'s impressive load begins to fill you up, [rubi eir] seed slipping into every available nook and cranny.  What doesn\'t fit slips out of your abused ' );
			if( !CoC.player.hasVagina() ) {
				MainView.outputText( 'ass' );
			} else {
				MainView.outputText( 'cunt' );
			}
			MainView.outputText( ' in a small torrent, pooling onto the bed and floor below.  ' );
		}
		//(Blueballs 10);
		else {
			MainView.outputText( 'Your insides rumble dangerously and your stomach actually swells up, gaining a noticeable paunch as Rubi\'s lavish load coats your insides, filling every available nook and cranny and even creating new ones.  What doesn\'t fit slips out of your abused hole in a flood, coating the bed and floor beneath you in a slick, white glaze.  ' );
		}
		MainView.outputText( 'Rubi doesn\'t stop there, however; [rubi ey] continues to assail your [vagOrAss], bringing you to several more miniature, body-shaking orgasms, until [rubi ey] just can\'t handle any more, and [rubi ey] pulls out, [rubi eir] cock still rather erect and looking cozy in its jacket-like foreskin.' );
	...
	Rubi.prototype.rubiHotdogging = function() {
		...
		if( CoC.flags[ kFLAGS.RUBI_SKIN ] === 0 ) {
			MainView.outputText( 'pale skinned' );
		} else if( CoC.flags[ kFLAGS.RUBI_SKIN ] === 1 || CoC.flags[ kFLAGS.RUBI_SKIN ] === 3 ) {
			MainView.outputText( 'red skinned' );
		} else {
			MainView.outputText( 'fur-covered' );
		}
		MainView.outputText( ' ' + this.rubiMF( 'boy', 'girl' ) + ' lays naked on your lap.  Rubi giggles and dips a hand between [rubi eir] thighs ' );
		if( CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] > 0 ) {
			MainView.outputText( 'taking hold of [rubi eir] cock' );
		} else {
			MainView.outputText( 'rubbing a finger along [rubi eir] clit' );
		}
		MainView.outputText( ' while [rubi ey] says, "<i>So babe, what do you intend to do?  Are you going to hold me down and ravish me?  What will it be?</i>"' );
		...
		if( CoC.flags[ kFLAGS.RUBI_SKIN ] !== 2 ) {
			MainView.outputText( 'tender skin' );
		} else {
			MainView.outputText( 'silky fur' );
		}
		MainView.outputText( '.  Rubi\'s back arches and a soft gasp escapes [rubi eir] lips.  [rubi Eir] body reflexively jerks towards your hand, hoping for more than a simple teasing touch, but you pull your hand away and give [rubi eir] round ass a playful smack, chastising [rubi em].' );
		...
		if( CoC.flags[ kFLAGS.RUBI_NO_CUNT ] === 0 ) {
			MainView.outputText( 'cunt moistening' );
		}
		if( CoC.flags[ kFLAGS.RUBI_NO_CUNT ] === 0 && CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] > 0 ) {
			MainView.outputText( ' and ' );
		}
		if( CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] > 0 ) {
			MainView.outputText( 'cock hardening' );
		}
		MainView.outputText( '.  [rubi Ey] obviously enjoys this light, playful teasing.  You give Rubi\'s ass another playful smack, watching [rubi eir] ' );
		if( CoC.flags[ kFLAGS.RUBI_SKIN ] === 0 ) {
			MainView.outputText( 'pale skin' );
		} else if( CoC.flags[ kFLAGS.RUBI_SKIN ] === 1 || CoC.flags[ kFLAGS.RUBI_SKIN ] === 3 ) {
			MainView.outputText( 'crimson skin' );
		} else {
			MainView.outputText( 'fur-covered flesh' );
		}
		MainView.outputText( ' jiggle with a smile.' );
		...
		if( CoC.flags[ kFLAGS.RUBI_SKIN ] !== 2 ) {
			...
			MainView.outputText( 'coat' );
		}
		MainView.outputText( ' against your manhood as you slowly begin to pump in and out of Rubi\'s ass cheeks.' );
		...
		MainView.outputText( ', while your index and middle fingers rub slowly around [rubi eir] ' );
		if( CoC.flags[ kFLAGS.RUBI_NO_CUNT ] === 0 ) {
			MainView.outputText( 'clit' );
		} else {
			MainView.outputText( 'perineum' );
		}
		MainView.outputText( '.' );
	...
	Rubi.prototype.teaseButtjobs = function() {
		MainView.outputText( ImageManager.showImage( 'rubi-tease-buttjob' ), false );
		MainView.clearOutput();
		this.rubiSprite();
		MainView.outputText( 'Grinning to yourself, you decide to tease the little demon ' + this.rubiMF( 'boy', 'girl' ) + ' a little further.  You plunge your thumb further into [rubi eir] ' );
		if( CoC.flags[ kFLAGS.RUBI_NO_CUNT ] === 1 ) {
			MainView.outputText( 'ass' );
		} else {
			MainView.outputText( 'pussy' );
		}
		MainView.outputText( ', finding [rubi eir] ' );
		if( CoC.flags[ kFLAGS.RUBI_NO_CUNT ] === 1 ) {
			MainView.outputText( 'prostate' );
		} else {
			MainView.outputText( 'g-spot' );
		}
		MainView.outputText( ' and rubbing circles around it, drawing out a long moan from Rubi\'s smeared lips.  With every thrust into Rubi\'s soft ass-pillows you draw another loop around [rubi eir] most sensitive spot with your digit, until you can feel [rubi eir] body start to tense up.' );
		...
		if( CoC.player.cumQ() <= 50 ) {
			MainView.outputText( 'in one go, with a small, proud look on [rubi eir] face' );
		} else if( CoC.player.cumQ() <= 150 ) {
			MainView.outputText( 'with one, throat-straining swallow, a small, proud smile taking [rubi eir] face when [rubi ey] stops panting for breath' );
		} else {
			MainView.outputText( 'as it comes, mouthful after mouthful of cum practically injected straight into [rubi eir] stomach. There\'s so much of it, [rubi ey] can\'t help but smile mid-way through, proud to take such a huge load' );
		}
		if( CoC.player.cumQ() >= 1000 ) {
			MainView.outputText( '.  After guzzling down so much, you\'re certain Rubi\'s actually sporting a bit of a belly now, almost as if [rubi ey] were several months pregnant' );
		}
		MainView.outputText( '.' );
	...
	Rubi.prototype.popButtjobs = function() {
		MainView.outputText( ImageManager.showImage( 'rubi-pop-buttjob' ), false );
		MainView.clearOutput();
		this.rubiSprite();
		MainView.outputText( 'No, you figure Rubi has been so great lately [rubi ey] definitely deserves to cum.  You plunge your thumb further into [rubi eir] ' );
		if( CoC.flags[ kFLAGS.RUBI_NO_CUNT ] === 1 ) {
			MainView.outputText( 'ass' );
		} else {
			MainView.outputText( 'pussy' );
		...
		MainView.outputText( '\n\nRubi\'s ' );
		if( CoC.flags[ kFLAGS.RUBI_NO_CUNT ] === 0 ) {
			MainView.outputText( 'cunt clamps down on your thumb and gushes, coating your hand in [rubi eir] slick juices' );
		} else {
			MainView.outputText( 'asshole quivers and attempts to suck in more of your thumb before clamping down, leaking slick fluids all over your hand' );
		}
		MainView.outputText( '.  [rubi Eir] entire body spasms as the climax wracks it, one sexy leg suddenly jerking back and forth uncontrollably.  Rubi grips at a pillow, biting it and stifling another orgasmic moan.' );
		//(If Rubi has Penis);
		if( CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] > 0 ) {
			MainView.outputText( '  Beneath [rubi em], [rubi eir] cock jerks and twitches, loosing a spray of pearly white cum which splatters against the couch below' );
		}
		if( CoC.flags[ kFLAGS.RUBI_BLU_BALLS ] < 4 ) {
			{
			}
		...
		//((blueballs 4-5);
			...
			MainView.outputText( ' which is quickly followed by another, more powerful stream of cum, which splashes against [rubi eir] stomach, coating [rubi eir] [rubi breasts] in semen' );
		} else if( CoC.flags[ kFLAGS.RUBI_BLU_BALLS ] <= 7 ) {
			MainView.outputText( ' which is quickly followed by another, more powerful stream of cum, and a third, and a fourth.  Before you know it, Rubi\'s entire torso, including [rubi eir] [rubi breasts] are practically glazed with cum' );
		} else if( CoC.flags[ kFLAGS.RUBI_BLU_BALLS ] <= 9 ) {
			MainView.outputText( ' which is followed up by several more, each seemingly more poweful than the one previous.  Rubi\'s body twitches and shudders with every jet, until [rubi eir] entire torso, and the couch, is covered in the musky jizz' );
		} else {
			MainView.outputText( ' which is followed by a dozen more, each one thicker and more powerful than the last.  Rubi\'s cock twitches uncontrollably, just like [rubi eir] body, as it coats the entire couch, [rubi eir] entire upper body, and even some of the wall five feet away in the musky stuff' );
		}
		MainView.outputText( '.' );
		...
		if( CoC.player.balls > 1 ) {
			MainView.outputText( 'churn' );
		} else {
			MainView.outputText( 'churns' );
		}
		MainView.outputText( ' pleasurably, your [cock biggest] stiffens suddenly, and you let loose a long, drawn-out sigh as your dick erupts, jetting cum upwards, which immediately splatters against Rubi\'s tail and back before dripping down [rubi eir] bountiful asscheeks.' );
		...
		MainView.outputText( '.' );
		MainView.outputText( '\n\nFinally, with one final spurt of cum that lands on your lover\'s face, you slip sideways onto the comfortable couch, completely spent.  Rubi, however, seems to have plenty of energy left, and spends the next few minutes dutifully licking over your [cock biggest], cleaning it of your juices until it sits, shiny and pristine, in your lap.' );
		MainView.outputText( 'You laugh and give the demon-' + this.rubiMF( 'boy', 'girl' ) + ' a kiss, remarking how lucky you are to be with [rubi em].' );
		MainView.outputText( '\n\n"<i>No, babe, I\'m lucky to have found you,</i>" [rubi ey] says, snuggling [rubi eir] naked body up against yours as the two of you drift off into a lazy, sex-induced nap.' );
		CoC.player.orgasm();
	...
	Rubi.prototype.giveRubiATFItem = function( itype ) {
		...
		if( CoC.flags[ kFLAGS.RUBI_TIMES_GIVEN_AN_ITEM ] === 0 ) {
			MainView.outputText( 'Rubi looks at the offered item, curiously.  [rubi Ey] turns it over in [rubi eir] hands, carefully examining the liquid within.  Finally [rubi ey] asks apprehensively, "<i>Is this going to... change my body?</i>"' );
		...
		if( itype === ConsumableLib.SUCMILK || itype === ConsumableLib.P_S_MLK ) {
			if( itype === ConsumableLib.SUCMILK ) {
				...
				MainView.outputText( 'You hand over the bottle of milky white fluids to Rubi.  [rubi Ey] takes a moment to read over the pink label before pulling out the cork and downing the whole bottle in one gulp.' );
			} else {
				...
				MainView.outputText( 'You hand over the bottle of purified milky white fluids to Rubi.  [rubi Ey] takes a moment to read over the pink label before pulling out the cork and downing the whole bottle in one gulp.' );
			}
			...
			if( (CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] > 4) && (!CoC.flags[ kFLAGS.HYPER_HAPPY ]) ) {
				...
				MainView.outputText( Math.round( CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] * 10 ) / 10 + '-inch penis.' );
			}
			//(If Penis is removed);
			...
			if( Utils.rand( 3 ) === 0 && CoC.flags[ kFLAGS.RUBI_SKIN ] !== 1 && itype === ConsumableLib.SUCMILK ) {
				...
				if( CoC.flags[ kFLAGS.RUBI_SKIN ] !== 2 ) {
					...
					MainView.outputText( 'coat of fur' );
				}
				MainView.outputText( '.  The blotches soon spread, rapidly overtaking [rubi eir] body.  ' );
				if( CoC.flags[ kFLAGS.RUBI_SKIN ] === 2 ) {
					MainView.outputText( 'Meanwhile the silky soft fur that covered [rubi eir] body seems to be falling out as this red menace takes over.  ' );
				}
				MainView.outputText( 'In no time at all, [rubi eir] smooth, hairless skin now appears to be a deep crimson, matching the color of [rubi eir] spade tail that [rubi ey] received long ago.' );
			...
			if( itype === ConsumableLib.INCUBID ) {
				MainView.outputText( 'You hand over the bottle of milky white fluids to Rubi.  [rubi Ey] takes a moment to read over the label before pulling out the cork and downing the whole bottle in one gulp.' );
			}
			...
			//(If Rubi has no penis; instead);
				...
				MainView.outputText( ' protrusion... a brand new cock!  Rubi shudders, [rubi eir] delicate little fingers wrapped around the new, five-inch cock.' );
				CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] = 5;
			}
			//(If Vagina removed);
		...
		//Bimbo Liqueur (Modified for variant genders);
			...
			MainView.outputText( 'You hold out the bottle of Bimbo Liqueur.  Rubi takes it, and looks it over tentatively.  "<i>Bimbo... liqueur?</i>" [rubi ey] asks, quirking an eyebrow at you.  "<i>But what does this do?  There\'s a huge warning label right here.  Effects are permanent, strong...</i>" [rubi eir] voice drifts off as [rubi ey] reads.' );
			...
			MainView.outputText( ' hair suddenly bursts outwards, long platinum blonde locks flow past [rubi eir] shoulders like a waterfall.  [rubi Eir] already dainty hands grow more feminine, as do [rubi eir] feet.' );
			...
			if( CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] > 0 ) {
				...
				if( CoC.flags[ kFLAGS.RUBI_NO_CUNT ] === 0 ) {
					MainView.outputText( '[rubi eir] vagina' );
				} else {
					MainView.outputText( 'a pristine new vagina' );
				}
				MainView.outputText( '.  It\'s pretty clear, this former male is now completely female.  Rubi collapses backwards onto the couch, the transformations seemingly done.' );
				CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] = 0;
				CoC.flags[ kFLAGS.RUBI_NO_CUNT ] = 0;
				CoC.flags[ kFLAGS.RUBI_SHE ] = 1;
			}
			...
			if( CoC.flags[ kFLAGS.RUBI_BREAST_SIZE ] < 7 ) {
				...
				MainView.outputText( 'nipples begins to tremble and then expand.  You swear you hear a whooshing sound, like someone is blowing up a balloon.' );
				//(if flat-chested);
				if( CoC.flags[ kFLAGS.RUBI_BREAST_SIZE ] === 0 ) {
					MainView.outputText( '  Within seconds, Rubi has the chest of a pubescent girl, within a minute they\'re at least a B-cup.' );
				}
				MainView.outputText( '  You both sit with morbid curiosity, watching the flesh grow outwards, bit by bit and cup by cup.  After five minutes the growth stops, and Rubi carefully cradles [rubi eir] new enormous E-cup breasts.' );
				CoC.flags[ kFLAGS.RUBI_BREAST_SIZE ] = 7;
			}
			...
			MainView.outputText( '.  "<i>Like, thank you so much baby!  You\'re the freaking best!  Now... you want to break this body in?</i>"' );
		...
		//Grows cock. (As Incubus Draft above);
			...
			if( this.rubiGetCockType() !== CockTypesEnum.HORSE ) {
				...
				if( CoC.flags[ kFLAGS.RUBI_BALLS_TYPE ] === 1 ) {
					MainView.outputText( 'tight, trappy ' );
				}
				MainView.outputText( 'ballsack.  The colors move forward, seemingly stopping at the thick ring that marks the edge of [rubi eir] sheathe... except patches of  dusky gray appear along the next few inches of [rubi eir] length, creating a nice contrast and pattern with the bright pink of [rubi eir] cockflesh.' );
		...
		//Trap Oil;
			...
			if( CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] > 0 ) {
				MainView.outputText( '  Rubi\'s [rubi cock] twitches, already leaking precum from its tip as it attempts to reach the skies above' );
			} else {
				MainView.outputText( '  Rubi\'s cunt glistens, soaked through entirely thanks to your ministrations' );
			}
			if( CoC.flags[ kFLAGS.RUBI_BREAST_SIZE ] > 0 ) {
				MainView.outputText( ', while [rubi eir] [rubi breasts] gently rock up and down with the motions of [rubi eir] breathing' );
			}
			MainView.outputText( '.' );
		...
		//Progression of furriness: cat ears, then whiskers and paws, then cat penis, then furry body.;
			...
			MainView.outputText( 'You hand over the tiny peach-like fruit to Rubi.  [rubi Ey] turns it over in [rubi eir] hands and remarks, "<i>How can you like these things?  They\'re so... furry!  Well, to each their own I suppose.</i>"  Rubi gulps visibly, preparing [rubi em]self for the whiskery fruit before biting down.  Apparently, it\'s not as bad as [rubi ey] thought it was, as [rubi ey] makes an appreciative grunt and consumes the fruit down to the pit.' );
			//(Gaining Cat Ears);
			if( CoC.flags[ kFLAGS.RUBI_EAR_TYPE ] !== 1 ) {
		...
		if( itype === ConsumableLib.PURPEAC ) {
			...
			if( CoC.flags[ kFLAGS.RUBI_SKIN ] === 0 ) {
				...
				MainView.outputText( 'red skin' );
			} else if( CoC.flags[ kFLAGS.RUBI_SKIN ] === 2 ) {
				MainView.outputText( 'furred skin' );
			} else {
				MainView.outputText( 'striped skin' );
			}
			MainView.outputText( ' underneath.  The demon-marked ' + this.rubiMF( 'boy', 'girl' ) + ' rubs [rubi eir] stomach appreciatively.  "<i>Those were some great peaches, babe.</i>"' );
			//(Any changes? Yes);
			if( CoC.flags[ kFLAGS.RUBI_EAR_TYPE ] !== 2 || CoC.flags[ kFLAGS.RUBI_SKIN ] !== 3 || (CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] > 0 && this.rubiGetCockType() !== CockTypesEnum.TENTACLE) ) {
				...
				if( CoC.flags[ kFLAGS.RUBI_EAR_TYPE ] !== 2 ) {
					...
					if( CoC.flags[ kFLAGS.RUBI_EAR_TYPE ] === 0 ) {
						MainView.outputText( 'normal' );
					} else {
						MainView.outputText( 'feline' );
					}
					MainView.outputText( ' ears seem to pull into [rubi eir] skull before disappearing, leaving nothing but skin and hair behind.  You wonder if Rubi\'s lost [rubi eir] ears permanently, but as the thought finishes crossing your mind you notice two small nubs growing where normal ears would be.' );
				...
				//(Shark Skin);
					...
					if( CoC.flags[ kFLAGS.RUBI_SKIN ] === 0 ) {
						MainView.outputText( 'its normal paleness replaced with a vibrant cherry red' );
					} else if( CoC.flags[ kFLAGS.RUBI_SKIN ] === 1 ) {
						MainView.outputText( 'which is barely noticeable on [rubi eir] already cherry red flesh' );
					} else if( CoC.flags[ kFLAGS.RUBI_SKIN ] === 2 ) {
						MainView.outputText( 'the fur rapidly retreating and replaced with bright red flesh underneath' );
					}
					MainView.outputText( ', but it doesn\'t seem to be even.  The majority of Rubi\'s body is cherry red, while [rubi eir] belly actually lightens, becoming a sweet, silvery gray.  A number of similarly silver stripes appear, seemingly at random, up and down Rubi\'s arms and legs, and even down [rubi eir] demonic tail, amazingly enough.' );
				...
				//(Anemone Penis, must have a penis);
					...
					if( this.rubiGetCockType() !== CockTypesEnum.HUMAN ) {
						MainView.outputText( ', first shifting towards a more human-like appearance' );
					}
					MainView.outputText( '.  Small nodules appear, around the crown of the penis, and around its base.  Rubi\'s delicate hands run across two of the nodules, and [rubi eir] cock suddenly stiffens, harder than you\'ve ever seen it!  Whatever those things are, they\'re quite sensitive.' );
	...
	Rubi.prototype.rubiGrowPlusBreasts = function() {
		MainView.outputText( ImageManager.showImage( 'rubi-gro-plus-tits' ), false );
		MainView.clearOutput();
		this.rubiSprite();
		MainView.outputText( 'You hand over the vial of blueish liquid.  Rubi takes a moment to look it over, before taking a deep breath and injecting half of the vial into one breast and the rest into [rubi eir] other breast.' );
		//(If breast increase);
		if( CoC.flags[ kFLAGS.RUBI_BREAST_SIZE ] < 7 || (CoC.flags[ kFLAGS.RUBI_BREAST_SIZE ] < 30 && CoC.flags[ kFLAGS.HYPER_HAPPY ]) ) {
	...
	Rubi.prototype.rubiPenisGroPlus = function() {
		MainView.outputText( ImageManager.showImage( 'rubi-gro-plus-penis' ), false );
		MainView.clearOutput();
		this.rubiSprite();
		MainView.outputText( 'You hand over the vial of greenish liquid.  Rubi takes a moment to look it over, before taking a deep breath and injecting the entire thing into the base of [rubi eir] [rubi cock].' );
		//(If Penis increase);
		if( CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] < 12 || (this.rubiGetCockType() === CockTypesEnum.HORSE && CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] < 20) || (CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] < 30 && CoC.flags[ kFLAGS.HYPER_HAPPY ]) ) {
			...
			if( this.rubiGetCockType() === CockTypesEnum.HORSE ) {
				MainView.outputText( 'sheath-stuffing horse-dick' );
			} else {
				MainView.outputText( 'giant dick.' );
			}
		}
		CoC.player.consumeItem( ConsumableLib.GROPLUS );
		MainView.menu();
	...
	Rubi.prototype.rubiBoobsReducto = function() {
		MainView.clearOutput();
		this.rubiSprite();
		MainView.outputText( 'You hand over the jar of paste.  Rubi looks it over and, after giving it a sniff, uses two fingers to scoop up some of the pale substance, smearing it over [rubi eir] breasts.  Before long, the jar is empty and [rubi eir] [rubi breasts] are covered in the stuff.' );
		if( CoC.flags[ kFLAGS.RUBI_BREAST_SIZE ] > 0 ) {
	...
	Rubi.prototype.rubiPenisReducto = function() {
		MainView.clearOutput();
		this.rubiSprite();
		MainView.outputText( 'You hand over the jar of paste.  Rubi looks it over and, after giving it a sniff, uses two fingers to scoop up some of the pale substance, smearing it over [rubi eir] penis.  Before long, the jar is empty and [rubi eir] [rubi cock] is covered in the stuff.' );
		if( CoC.flags[ kFLAGS.RUBI_COCK_SIZE ] >= 3 ) {
	...
	Rubi.prototype.pickAnItemToFeedRubi = function() {
		...
		if( closet.length > 0 ) {
			MainView.outputText( '<b>Rubi\'s Closet:</b>' );
			_.forEach(closet, function(item) {
				MainView.outputText( '\n' + item );
			});
			MainView.outputText( '\n\n' );
		...
		if( gifts.length > 0 ) {
			MainView.outputText( 'After a quick glance around Rubi\'s things, you bet [rubi ey] could really use... ' + gifts[ Utils.rand( gifts.length ) ] + '.\n\n' );
		} else {
			MainView.outputText( 'You\'ve given Rubi all the clothes [rubi ey] would want to make use of.' );
			if( EngineCore.silly() ) {
				MainView.outputText( '  (Achievement Unlocked: Dress-tacular)' );
			}
			MainView.outputText( '\n\n' );
		}
		if( !this.rubiBimbo() && !this.rubiIncubus() && this.rubiAffection() >= 100 ) {
			MainView.outputText( 'You could give Rubi some incubi draft to make [rubi em] a little more manly but corrupt, or you could give Rubi a Bimbo Draft to turn [rubi em] into a smoking hot, female sex-bomb.\n\n' );
		}
		MainView.outputText( 'What will you give [rubi em]?' );
	...
	Rubi.prototype.giveRubiClothes = function( itype ) {
		MainView.clearOutput();
		this.rubiSprite();
		MainView.outputText( 'You hand over the spare set of clothes, and Rubi\'s eyes light up.  "<i>For me?</i>"  the little demon practically screams, ecstatic.  "<i>ThankyouthankyouthankyouTHANKYOU!</i>"' );
	...
	Rubi.prototype.hypnoBimboficationForRubiSloots = function() {
		MainView.clearOutput();
		MainView.outputText( 'You slither out of your [armor], flicking your tongue hungrily at the air, tasting the petite boy\'s desire. He is so attracted to you, and yet, he\'s trying so hard to be classy about it. It\'s cute in its own way, but you\'d rather see him be more... honest about his drives. You want the cute, ' );
		if( CoC.flags[ kFLAGS.RUBI_BREAST_SIZE ] < 1 ) {
			MainView.outputText( 'femmy boy' );
		} else if( CoC.flags[ kFLAGS.RUBI_NO_CUNT ] > 0 ) {
			MainView.outputText( 'dick-girl' );
		} else {
			MainView.outputText( 'little slut' );
		}
		MainView.outputText( ' ready to lust after your cock openly, to cling to you whenever you arrive and grope your bulge in public. He needs to be less worried about propriety and more focused on sexual satiation and appeal... yes.' );
		...
		MainView.outputText( ' whipped out and swaying with your sinuous movements, and all he has is his comparatively undersized little pecker. You rock your whole body with the snake-like grace given to you by your naga body, swaying rhythmically as you meet his eyes. Knowing full well just what kinds of depravity you\'d like to force him into, you feel a ' );
		if( SceneLib.jojoScene.monk >= 5 ) {
			MainView.outputText( 'familiar ' );
		} else {
			MainView.outputText( 'strange ' );
		}
		MainView.outputText( 'dark power welling up within you.' );
		...
		if( CoC.flags[ kFLAGS.RUBI_NO_CUNT ] > 0 ) {
			MainView.outputText( 'femboy' );
		} else {
			MainView.outputText( 'hermaphrodite' );
		}
		MainView.outputText( '. The longer he stares into your power, the slacker his jaw becomes.' );
		...
		if( CoC.flags[ kFLAGS.RUBI_NO_CUNT ] > 0 ) {
			...
			MainView.outputText( 'dickgirl' );
		}
		MainView.outputText( ' must have quite the mind on him if it\'s making him drip THAT fast.' );
		...
		if( CoC.flags[ kFLAGS.RUBI_NO_CUNT ] > 0 ) {
			MainView.outputText( 'y-boy' );
		}
		MainView.outputText( ' simply gives the slightest of nods as a reply.' );
		...
		MainView.outputText( '\n\n<b>Rubi is now a ' );
		if( CoC.flags[ kFLAGS.RUBI_NO_CUNT ] > 0 ) {
			MainView.outputText( 'shemale ' );
		}
		MainView.outputText( 'bimbo! She\'s going to be a lot more lusty and airheaded, but isn\'t that what you wanted?</b>' );
		CoC.flags[ kFLAGS.RUBI_BIMBO ] = 1;
		CoC.flags[ kFLAGS.RUBI_SHE ] = 1;
