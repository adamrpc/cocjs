...
angular.module( 'cocjs' ).run( function( MainView, SceneLib, Descriptors, ImageManager, PerkLib, $rootScope, CoC, kFLAGS, Utils, StatusAffects, PregnancyStore, EngineCore, ConsumableLib, OnLoadVariables ) {
	...
	Niamh.prototype.timeChangeLarge = function() {
		return false;
	};
	...
	Niamh.prototype.telAdreNiamh = function() {
		if( CoC.flags[ kFLAGS.MET_NIAMH ] === 0 ) {
			...
			if( CoC.time.hours <= 8 ) {
				MainView.outputText( 'massive N-cup breasts, spilling over the table' );
			} else if( CoC.time.hours <= 9 ) {
				MainView.outputText( 'huge M-cup breasts, taking up much of the table' );
			} else if( CoC.time.hours <= 10 ) {
				MainView.outputText( 'great L-cup breasts, advancing far onto the table' );
			} else if( CoC.time.hours <= 11 ) {
				MainView.outputText( 'heavy K-cup breasts, lying on the table for support' );
			} else if( CoC.time.hours <= 12 ) {
				MainView.outputText( 'hefty J-cup breasts, resting comfortably on the table' );
			} else if( CoC.time.hours <= 13 ) {
				MainView.outputText( 'weighty I-cup breasts, dangling to lie on the table' );
			} else if( CoC.time.hours <= 14 ) {
				MainView.outputText( 'squeezable H-cup breasts, settling burdensomely on her chest' );
			} else {
				MainView.outputText( 'moderately manageable G-cups, trembling with every breath she takes' );
			}
			MainView.outputText( '.  Smooth, touchable skin covers her chest, face, hands and presumably the rest of her; it seems her animalistic accoutrements stop at kitty ears and tail. She takes a deep swig of her drink, following it up with a cute little burp.' );
		}
		//This scene only plays if the PC has either Talked to Niamh or has gotten at least 1 mug of Black Cat Beer from her.;
			...
			if( CoC.time.hours <= 8 ) {
				MainView.outputText( 'her titanic N-cup \'kegs\' resting against the thankfully sturdy table in front of her.  Her teats are engorged with the beer she\'s cursed to carry, and the stuff dribbles out whenever she shifts.' );
			} else if( CoC.time.hours <= 14 ) {
				MainView.outputText( 'her ' );
				if( CoC.time.hours <= 9 ) {
					MainView.outputText( 'M-cup' );
				} else if( CoC.time.hours <= 10 ) {
					MainView.outputText( 'L-cup' );
				} else if( CoC.time.hours <= 11 ) {
					MainView.outputText( 'K-cup' );
				} else if( CoC.time.hours <= 12 ) {
					MainView.outputText( 'J-cup' );
				} else if( CoC.time.hours <= 13 ) {
					MainView.outputText( 'I-cup' );
				} else {
					MainView.outputText( 'H-cup' );
				}
				MainView.outputText( ' brew-burdened breasts have clearly gotten smaller as the day dragged on, although they\'re still uncomfortably sizeable and swollen.' );
			} else {
				MainView.outputText( 'her G-cup beer-filled boobs far smaller than their morning volume, though still impressive in stature.  You can follow the progress she\'s made throughout the day by the beer stains running along the wooden surface, trailing from all the way off the table and ending where her boobs now lie.' );
			}
		}
	};
	//Approach:;
	Niamh.prototype.approachNiamh = function() {
		...
		MainView.outputText( ImageManager.showImage( 'niamh-approach-in-bar' ) );
	...
	Niamh.prototype.getANiamhBeer = function() {
		MainView.clearOutput();
		MainView.outputText( ImageManager.showImage( 'niamh-get-beer' ) );
		MainView.outputText( '"<i>That\'ll be two gems,</i>" she replies.  You hand over the payment, which she holds up to the light and scrutinizes thoroughly.  Satisfied, she drops the things into a sack at her waist.  "<i>Sorry,</i>" she mutters. "<i>I do want to get this damned beer out o\' me, but this is still my primary livelihood; I canno\' afford to be cheated.  Anyway, here\'s your mug - you can keep that f\'r another two gems; I get \'em cheap.  Go ahead and fill it up, but keep in mind what you\'re tweakin\', right?</i>"' );
		//Feeder;
		if( CoC.player.findPerk( PerkLib.Feeder ) >= 0 ) {
			MainView.outputText( '  You understand exactly what she\'s going through; the weight of a huge pair of breasts, the sensitivity as they engorge with fluids, the near-maddening sensation of the contents sloshing around underneath your straining skin...' );
		}//(Player has G-cups or bigger;;
		else if( CoC.player.biggestTitSize() >= 15 ) {
			if( CoC.player.lactationQ() === 0 ) {
				MainView.outputText( '  Not terribly surprising; you know your tits are sensitive, and you don\'t even make milk in them.' );
			} else {
				MainView.outputText( '  You can easily imagine just how much trouble she must have, especially given the bulk of her tits are fluid.' );
			}
		} else if( CoC.player.lactationQ() > 0 ) {
			MainView.outputText( '  You know just how distracting it is to have your tits full of fluid waiting to get out.' );
		}
		//([if first time];
		...
		if( CoC.flags[ kFLAGS.MET_NIAMH ] > 0 ) {
			MainView.outputText( 'Niamh ' );
		} else {
			MainView.outputText( 'the cat-girl ' );
		}
		MainView.outputText( 'sighs in relief, biting her lower lip slightly.' );
		//General Ending;
	...
	Niamh.prototype.drinkNiamhsBeerInTelAdre = function() {
		...
		if( CoC.player.lactationQ() >= 300 && CoC.player.biggestTitSize() >= 5 && Utils.rand( 2 ) === 0 && CoC.flags[ kFLAGS.MET_NIAMH ] > 0 ) {
			...
			if( SceneLib.xmasBitch.isHolidays() && OnLoadVariables.date.date >= 31 ) {
				...
				if( CoC.player.hasVagina() ) {
					MainView.outputText( 'pussy to wetten' );
				}
				if( CoC.player.gender === 3 ) {
					MainView.outputText( ' and ' );
				}
				if( CoC.player.hasCock() ) {
					MainView.outputText( '[eachCock] to stiffen' );
				}
				MainView.outputText( '.  "<i>Ye know, I got\'s an idea.  How\'s about ye stick by me for the next hour.  I\'m sure ye must be diein\' to drain those big\'uns of yours.  We might as well sell your champagne ta the patrons.  Whaddaya say Lassie?</i>"' );
			...
			// Christmas;
			else if( SceneLib.xmasBitch.isHolidays() ) {
			...
			// Valentine's;
			else if( SceneLib.valentines.isValentine() ) {
		...
		MainView.outputText( 'You yell "<i>Skoal!</i>" and upend the mug into your mouth, drinking greedily.  The warmth of the booze quickly follows the liquid itself - potent stuff, this is.' );
	...
	Niamh.prototype.blackCatBeerToGo = function() {
		MainView.clearOutput();
		MainView.outputText( '"<i>That\'ll be two more gems, then,</i>" the girl says.  "<i>Though, if your hands get any more skillful I might have to pay you to take it,</i>" she flirts.  You pass over two more gems, and she gratefully bags them and beams you a smile.' );
		CoC.player.gems -= 2;
		MainView.statsView.show();
		//both output;
		if( CoC.time.hours <= 15 ) {
	...
	Niamh.prototype.talkToNiamh = function() {
		...
		if( CoC.flags[ kFLAGS.TALKED_NIAMH ] === 0 ) {
			MainView.outputText( 'You ask the cat-girl to tell you about her past.' );
	...
	Niamh.prototype.leaveNiamh = function() {
		MainView.clearOutput();
		MainView.outputText( 'You decide that you don\'t really want to talk to this strange cat-girl and, as politely as possible, excuse yourself.' );
	...
	Niamh.prototype.blackCatBeerEffects = function( character, clearScreen, newLine ) {
		...
		MainView.outputText( 'Uncapping the mug, you swill the stuff down in a single swig, gasping as it burns a fiery trail into your belly.  It\'s rich and sweet, and damn, but it\'s strong stuff!' );
		...
		if( character.findStatusAffect( StatusAffects.BlackCatBeer ) >= 0 ) {
			if( 100 - character.lib >= 10 ) {
				...
				lib = 100 - character.lib;
			}
			character.addStatusValue( StatusAffects.BlackCatBeer, 1, 4 );
			character.addStatusValue( StatusAffects.BlackCatBeer, 2, lib );
			MainView.outputText( '\n\nDamn, it\'s even better with every extra drink!' );
			EngineCore.dynStats( 'spe', -1, 'int', -1, 'lib', lib, 'lus', 30 + Utils.rand( character.lib / 4 ) );
		} else {
			if( 100 - character.lib >= 10 ) {
				lib = 10;
			} else {
			...
			character.createStatusAffect( StatusAffects.BlackCatBeer, 8, lib, 0, 0 );
			EngineCore.dynStats( 'spe', -5, 'int', -5, 'lib', lib, 'lus', 20 + Utils.rand( character.lib / 4 ) );
		}
		character.slimeFeed();
	};
	...
	Niamh.prototype.giveNiamphSuccubiMilk = function() {
	...
	Niamh.prototype.corruptOrBimboNiamhIntro = function() {
		MainView.clearOutput();
		MainView.outputText( '"<i>Heya, favored customer,</i>" the busty kitty greets as you approach.  "<i>I have a favor to ask!  My drink\'s empty, and I get bored of me own brew,</i>" Niamh explains.  "<i>Would ya mind fetchin\' me somethin\', or are ye gonna force me to sample me own goods once more?</i>"' );
		...
		if( CoC.player.hasItem( ConsumableLib.BIMBOLQ ) ) {
			MainView.outputText( '"bubbly" for her, in the form of the Bimbo Liqueur you got from the imp lord, or ' );
		}
	...
	Niamh.prototype.taintNiamh = function( bimbo ) {
		MainView.clearOutput();
		MainView.outputText( 'You grab Niamh\'s glass and one more and head off, making sure to turn a corner before commandeering an empty table.  Plunking the mugs down in front of you, you mutter to yourself as she moves away.' );
		//[if sucmilk];
		if( !bimbo ) {
	...
	Niamh.prototype.niamhPostTelAdreMoveOut = function() {
		...
		MainView.clearOutput();
		MainView.outputText( 'Upon one of your routine searches, you\'re given pause as a half-coherent mess of a song comes tumbling across the grasses.  You move toward the cacophony of sound, fairly confident in what you will find.  Indeed, stumbling and shambling towards you comes a ' );
		if( CoC.flags[ kFLAGS.NIAMH_STATUS ] === 1 ) {
			MainView.outputText( 'demonic-looking' );
		} else {
			MainView.outputText( 'hourglass-shaped' );
		}
		MainView.outputText( ' Niamh, and it\'s obvious she\'s completely hammered.  ' );
		if( CoC.flags[ kFLAGS.NIAMH_STATUS ] === 1 ) {
			MainView.outputText( '"<i>W\'hey there ' + CoC.player.mf( 'boyo', 'big girl' ) + ', wanna...g\'forrra goooo-gfh...time?</i>"' );
		} else {
			MainView.outputText( '"<i>W\'heeeey, honey!  I never thought I\'d see yaaaagain!</i>"' );
		}
		MainView.outputText( ' she slurs, struggling to stay upright between her impaired motor skills and her absolutely overfilled chest.  "<i>Aaah\'ve been drinkin\' me \'shtash\',</i>" she explains, noticing your gaze.  "<i>Canno\' leddit go\'tuh waste!</i>" You nod your agreement, sneaking a glance behind her to observe the twin trails of ' );
		if( CoC.flags[ kFLAGS.NIAMH_STATUS ] === 1 ) {
			MainView.outputText( 'dark' );
		} else {
			MainView.outputText( 'light' );
		}
		MainView.outputText( ' fluid she\'s leaving behind.' );
	...
	Niamh.prototype.bimboChampagne = function( character, clearScreen, intro ) {
		...
		if( intro ) {
			if( (character.findPerk( PerkLib.FutaFaculties ) >= 0 && character.findPerk( PerkLib.FutaForm ) >= 0) || (character.findPerk( PerkLib.BimboBody ) >= 0 && character.findPerk( PerkLib.BimboBrains ) >= 0) ) {
		...
		if( character.findStatusAffect( StatusAffects.BimboChampagne ) >= 0 ) {
			...
			character.createStatusAffect( StatusAffects.BimboChampagne, 8, 0, 0, 0 );
			//(Player has breasts smaller than DD-cup:;
			if( character.breastRows[ 0 ].breastRating < 5 ) {
				...
				character.changeStatusValue( StatusAffects.BimboChampagne, 2, 5 - character.biggestTitSize() );
				character.breastRows[ 0 ].breastRating = 5;
			}
			//(Player does not have vagina:;
			if( !character.hasVagina() ) {
				character.createVagina();
				character.genderCheck();
				MainView.outputText( '\n\nYou can feel ' );
				if( character.hasCock() ) {
					MainView.outputText( 'the flesh under your cock[if (hasBalls = true)  and behind your [balls]]' );
				} else {
					MainView.outputText( 'the blank expanse of flesh that is your crotch' );
				}
				MainView.outputText( ' start to tingle and squirm... mmm... that feels nice!  There\'s a sensation you, like, can\'t describe, and then your crotch feels all wet... but in a good, sticky sorta way.  Oh, wow!  <b>You\'ve, like, just grown a new virgin pussy!</b>  Awesome!' );
				character.changeStatusValue( StatusAffects.BimboChampagne, 3, 1 );
			}
			// character ass smaller than bimbo:;
			if( character.buttRating < 12 ) {
				...
				character.changeStatusValue( StatusAffects.BimboChampagne, 4, 12 - character.buttRating );
				character.buttRating = 12;
				if( character.hipRating < 10 ) {
					character.hipRating = 10;
				}
			}
			character.genderCheck();
			EngineCore.dynStats( 'spe', -10, 'lib', 1, 'lus', 25 );
		}
	};
	Niamh.prototype.removeBimboChampagne = function() {
		...
		if( CoC.player.statusAffectv2( StatusAffects.BimboChampagne ) > 0 ) {
			...
			MainView.outputText( '  As the trecherous brew fades, your [chest] loses some of its... bimboliciousness.  Your back feels so much lighter without the extra weight dragging down on it.' );
		}
		if( CoC.player.statusAffectv3( StatusAffects.BimboChampagne ) > 0 ) {
			MainView.outputText( '  At the same time, your [vagina] slowly seals itself up, disappearing as quickly as it came.  Goodbye womanhood.' );
		...
		if( CoC.player.statusAffectv4( StatusAffects.BimboChampagne ) > 0 ) {
			...
			MainView.outputText( '  Of course, the added junk in your trunk fades too, leaving you back to having a [butt].' );
		}
	...
	Niamh.prototype.bazaarNiamh = function() {
		...
		if( CoC.flags[ kFLAGS.NIAMH_STATUS ] === 2 ) {
			...
			//[Bizarre Bazaar, repeat encounter];
				...
				MainView.outputText( 'Surrounded by blonde-haired, horny women, herms and traps, many of them sporting stomachs swollen with Niamh\'s copious champagne lactation, the bimbo catgirl enthusiastically waves you over.  "<i>[name]!  Like, what can I do you for?</i>" she giggles.' );
			}
		}
		//Corrupt:;
		else {
			//(General notification:;
			MainView.outputText( 'Given the pile of snoozing bodies, you can probably hazard a guess as to where Niamh decided to set up shop.' );
			//(Approach:;
	...
	Niamh.prototype.bazaardNiamhDrink = function() {
		MainView.clearOutput();
		CoC.player.gems -= 2;
		MainView.outputText( 'Approaching the fallen-from-grace (yet completely content) catgirl, you toss her a couple gems and request a glass of the house drink.' );
		//Bimbo:;
		if( CoC.flags[ kFLAGS.NIAMH_STATUS ] === 2 ) {
		...
		//Corrupt;
			...
			MainView.outputText( 'Succubus milk Niamh Not implemented yet' );
			EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
		}
	};
	//{If player drinks from the tap:};
	Niamh.prototype.drinkFromZeTap = function() {
		MainView.clearOutput();
		MainView.outputText( 'Your eyes lock onto her large, ever-dribbling nipples, the froth of spilt champagne caked on the table supporting her mammoth breasts, and you know you just have to have it from the source.  You push your way forward and seat yourself on a plush cushion that, from its many tears and suspicious-smelling stains, has seen better days, then grasp the huge boob in front of you, making the ditzy catgirl yowl in anticipation of pleasure.' );
	...
	Niamh.prototype.getBimboChampFromNiamh = function() {
		MainView.clearOutput();
		MainView.outputText( 'You explain your need for a bottle of her champagne; the ditzy kitty takes several moments to register your words, then nods idly.  "<i>Works fer me!</i>" she responds, reaching behind her and rummaging in a large burlap bag.  She produces a flask from the sack\'s depths, unscrewing the cap and attempting to line it up with a nipple.  Try as she might, she just can\'t quite reach all the way, and with an airy giggle, she passes it to you instead.' );
		...
		SceneLib.inventory.takeItem( ConsumableLib.BIMBOCH, SceneLib.camp.returnToCampUseOneHour );
	};
	//[Bazaar sex];
	...
	Niamh.prototype.seanBimboBrewing = function() {
		MainView.clearOutput();
		MainView.outputText( 'The uncharacteristically gentlemanly incubus raises his eyebrow as you produce an armful of filled flasks and drop them onto his mostly-empty table.  "<i>I assume you\'d like me to do something special with these,</i>" he quips, somewhat dryly.' );
	...
	//24 hours later;
	Niamh.prototype.getBimboozeFromSean = function() {
		MainView.clearOutput();
		MainView.outputText( 'Sean turns as you enter, managing a thin smile as he regards you.  "<i>Excellent timing, I have just finished this!</i>"' );
		MainView.outputText( 'He scoops a familiar-looking bottle from a tabletop, passing it off to you.  "<i>Be somewhat careful with this,</i>" he warns as you take it.  "<i>The original\'s deleterious effect on the mind has been duplicated, I assure you.</i>"\n\n' );
		MainView.outputText( 'You drop the vial into a pouch on your person.\n\n' );
	...
	Niamh.prototype.sellYourBooze = function() {
		...
		if( CoC.player.bRows() === 1 ) {
			MainView.outputText( 'With a chipper giggle Niamh pulls you closer to the bar.  With both hands she helps you heft your tits up and on to the counter.' );
		}//If player has four or six breasts.;
		else {
			MainView.outputText( 'Niamh giggles as you try to move your breasts closer to the bar to be milked by the patrons, but it\'s obvious only the top pair of your tits can easily rest on the counter top.  "<i>Lassie, it might be unconventional, but givin\' the circumstance mayhaps it\'d be best if ye laid down on the bar.</i>"' );
		}
		...
		if( CoC.player.hasVagina() ) {
			MainView.outputText( '  You feel your excitement dripping down the inside of your thighs as your pussy wettens.' );
		}
		EngineCore.dynStats( 'lus', 10 + CoC.player.sens / 5, 'resisted', false );
		//[If player has cocks];
		if( CoC.player.hasCock() ) {
			MainView.outputText( '  You start to feel sympathetic sensations in [eachCock] as the hands and paws of the bar goers milk your nipples.  With their fingers sliding over your sensitive nubs you can easily imagine them sliding up and down your cock' );
		}
		...
		//If lust is high;
			...
			if( CoC.player.hasVagina() ) {
				MainView.outputText( 'You can feel your labia dripping with need.  ' );
			}
			if( CoC.player.hasCock() ) {
				MainView.outputText( 'pre-cum drizzles down your ' + Descriptors.multiCockDescriptLight() + ' as the need to fuck a pussy grows stronger.  A few wandering hands reveals some apparent interest amongst the folks in the bar in seeing your genitals as active as your nipples.' );
			}
			//[FUCK THE BAR] [LEAVE];
	...
	Niamh.prototype.barBeerOrgyTits = function() {
		...
		if( CoC.player.hasVagina() ) {
			MainView.outputText( 'The drooling of your [vagina] gets worse as the constant "tapping of your kegs" drives your sense of self-restraint to its limits.  The hands of various drunks start moving more aggressively over your breasts, eventually winding their way down between your thighs.' );
		}
		//IF [CoC.player has cock/s];
		if( CoC.player.hasCock() ) {
			MainView.outputText( '[EachCock] grows to its full length.  You try to keep ' );
			if( CoC.player.cockTotal() === 1 ) {
				MainView.outputText( 'it' );
			} else {
				MainView.outputText( 'them' );
			}
			MainView.outputText( ' discreet but it\'s impossible.  You try to push ' );
			if( CoC.player.cockTotal() === 1 ) {
				MainView.outputText( 'it' );
			} else {
				MainView.outputText( 'them' );
			}
			MainView.outputText( ' down or aside, but other pairs of hands begin stroking you.' );
		}
		...
		if( CoC.player.buttRating >= 20 ) {
			MainView.outputText( '  She slaps your ass to emphasize her meaning, and the action sends jiggling waves through each immense cheek.' );
		}
		...
		if( CoC.player.biggestTitSize() >= 20 ) {
			MainView.outputText( 'Your giant boobs are large enough to temporarily hang off the sides of your body as you\'re carried.' );
		}
		MainView.outputText( '  Your [butt] hits the table first as the crowd puts you down.  Some patrons begin sucking on your nipples directly.' );
		...
		if( (CoC.player.pregnancyIncubation === 0 || CoC.player.pregnancyIncubation > 100) && CoC.player.bRows() > 1 ) {
			MainView.outputText( '  You are essentially a pile of ' + this.boozeBoobsType() + ' filled tits.' );
		}// [IF player is hugely pregnant with four or more tits];
		else if( CoC.player.bRows() > 1 ) {
			MainView.outputText( '  You are essentially a giant pregnant belly surrounded by your [chest] on the table, helpless to the sexual advances of the bar goers and you can\'t get enough of it.' );
		}
		//===============;
		//Third Paragraph;
		//===============;
		...
		if( CoC.time.hours < 15 && CoC.flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] > 0 && CoC.player.hasVagina() ) {
			...
			if( CoC.player.pregnancyIncubation === 0 ) {
				MainView.outputText( '  Some folks give yelps and cheers when they see the bulge that repeatedly forms in your abdomen each time Urta thrusts herself inside you.' );
			}
		...
		//ELSE IF player has pussy and Urta is not in the bar.;
			...
			if( CoC.player.vaginalCapacity() < 60 ) {
				MainView.outputText( '  One is sporting a cock that is obviously too massive for your pussy, and eventually backs down so that the other one, despite still being on the large size, can take the honor of fucking your cunt silly.' );
			}//ELSE IF [CoC.player has a gaping pussy or bigger];
			else {
				MainView.outputText( '  They\'re both sporting large canine cocks, but one is particularly massive while the other would likely be a loose fit for a pussy as naturally stretched out as yours.  The largest one wins out and positions himself to give your overly wide fuck hole a much needed stretching.' );
			}
		...
		if( CoC.player.pregnancyIncubation > 0 && CoC.player.pregnancyIncubation < 100 ) {
			MainView.outputText( '  "<i>Oi!  Careful folks, this babe could blow at any time!</i>"  Niamh pats your belly with chiding affection.' );
		}
		...
		if( CoC.player.hasFuckableNipples() ) {
			MainView.outputText( '  Eventually some of the randier drunks start whipping out their cocks, and begin pressing the heads against the openings of your nipples.  Within moments you feel your booze bloated breasts being fucked silly by ' + Utils.num2Text( CoC.player.totalNipples() ) + ' hard cocks.  Each thrust causes ' + this.boozeBoobsType() + ' to spill out around their shafts.' );
		}// ELSE[Character doesn't have fuckable nipples];
		else {
			MainView.outputText( '  Some of the hornier customers start shoving cocks of all shapes and sizes into the moist wet cleavages formed by yours and Niamh\'s breasts.' );
		}
		...
		if( CoC.player.hasFuckableNipples() ) {
			MainView.outputText( '  The cocks filling your nipples start pumping hot sperm into your breasts, mixing with your ' + this.boozeBoobsType() + ' and causing every nerve ending in your nipples to tingle.  Some of them ejaculate so much that it feels as if your breasts are swelling even larger, almost to the point of bursting before the cum spills out.' );
		}
		//IF [Player has cocks];
		if( CoC.player.cockTotal() > 0 ) {
			MainView.outputText( '  You feel as if your body is exploding everywhere.  Niamh gags momentarily as her mouth fills with your seed, and you realize your cock' );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 's have' );
			} else {
				MainView.outputText( ' has' );
			}
			MainView.outputText( ' begun spurting ' );
			if( CoC.player.cockTotal() === 1 ) {
				MainView.outputText( 'its ' );
			} else {
				MainView.outputText( 'their ' );
			}
			MainView.outputText( 'jizz.  Niamh swallows it down her throat but not without some of it getting plastered across her face.' );
			//IF [character is pregnant];
			if( CoC.player.pregnancyIncubation > 0 && CoC.player.pregnancyIncubation < 200 ) {
				MainView.outputText( '  Globs of cum splatter across your gravid belly.' );
			}
		}
		//================;
		//Ending Paragraph;
		...
		if( CoC.player.statusAffectv1( StatusAffects.Edryn ) >= 5 && CoC.player.gender === 3 && CoC.time.hours === 14 && CoC.flags[ kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY ] > 0 ) {
			...
			if( CoC.player.cockTotal() === 1 ) {
				MainView.outputText( '  Edryn moans out loud in ecstasy as your [cock] fills her up.' );
			}// If [CoC.player has multiple cocks];
			else if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( '  The heads of your cocks press in between Edryn\'s labia causing her to cry out, "<i>Oh gawd!  How many dicks are y-you f-f-fucking meeee with???</i>"  Edryn\'s orgasm cuts her voice off and every muscle in her cunt contracts around your ' + Descriptors.multiCockDescriptLight() + '.' );
			}
			...
			if( CoC.player.pregnancyIncubation > 0 && CoC.player.pregnancyIncubation < 200 ) {
				MainView.outputText( '  Your pregnant belly presses up against Edryn\'s ass as both Urta\'s cock and Edryn\'s pussy keeps you pinned in place between them.' );
			}
			// IF[CoC.player is not pregnant];
			else {
				MainView.outputText( '  Urta\'s cock and Edryn\'s pussy pin you in place between them, and as Urta fucks you deeper her cock head forces your belly to bulge outwards until it presses against Edryn\'s ass.' );
			}
			...
			if( CoC.player.bRows() === 2 ) {
				MainView.outputText( '  Your second row of tits get thrust out to either side every time Urta\'s torso presses your own against Edryn.' );
			}//IF [CoC.player has six giant breasts];
			else if( CoC.player.bRows() === 3 ) {
				MainView.outputText( '  Your lowest row of tits slap against all three of your thighs as the love making becomes more brutal.' );
			}
			//IF [CoC.player has large balls];
			if( CoC.player.balls > 0 && CoC.player.ballSize > 14 ) {
				MainView.outputText( '  Your hanging testicles bounce around between everyone\'s legs.' );
			}
			//IF [CoC.player has giant ass];
			if( CoC.player.buttRating >= 22 ) {
				MainView.outputText( '  The fat of your ass cheeks jiggles and quakes even as Urta practically stuffs her hips into your crack.  "<i>Damn, you\'ve got a bigger ass than Edryn,</i>" she chides as she spanks you.  "<i>Hey!</i>"  Edryn calls out with a false tone of hurt pride.' );
			}
		...
		// IF [Urta but not Edryn is present in the bar and sex with her is unlocked and character has pussy];
			...
			if( CoC.player.bRows() > 1 ) {
				MainView.outputText( '  Your rows of breasts shift around as your posture changes, making squelching sounds as they slide around.' );
			}
			//If [CoC.player not pregnant];
			if( CoC.player.pregnancyIncubation === 0 ) {
				MainView.outputText( '  Urta\'s small ocean of sperm streams out from your pussy like a river down your legs as you try to stand.' );
			}
		...
		//Generic ending if the first two don't trigger;
			...
			// IF [CoC.player has pussy];
			if( CoC.player.hasVagina() ) {
				MainView.outputText( '  The giant knot of the dog morph finally breaks its way inside causing your pussy to become overstretched.  The pleasurable torture only gets worse as he begins unloading a river of his cum inside you.  It begins squirting out around his cock and soaks both of your thighs in the process.' );
			}
			CoC.player.cuntChange( 60, true, true, false );
			MainView.outputText( '  Niamh finally has her fill and dismounts you, but no sooner has her pussy left your face than a determined cock belonging to some sort of cat morph fills your mouth.  Your eyes bug out as it thrusts down your throat.' );
			// If [CoC.player has pussy];
			if( CoC.player.hasVagina() ) {
				MainView.outputText( '  You gag on it at the same time a new cock fills your cunt, ready to renew the thrusting that\'s been causing your tits to bounce all over the place.' );
			}
			...
			if( CoC.player.cockTotal() === 1 ) {
				MainView.outputText( '  The six titted girl smiles at you, patting her belly while your cum runs down the inside of her thighs.  You only now wonder about how fertile she might be.' );
			}// IF [CoC.player has multiple cocks];
			else if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( '  Various girls around the room are patting their bellies and giving you sultry looks while your cum continues to drip from their pussies.  You passively wonder if any of them might indeed be hauling around some larger pregnant bellies in a few days, but you try to push those thoughts from your head.  At the moment they seem happy.' );
			}
			// IF [CoC.player has pussy];
			if( CoC.player.hasVagina() ) {
				MainView.outputText( '  You walk slightly bow legged out of the bar.  Cum is still dripping from your snatch and will likely continue to do so for a while.' );
				//IF [CoC.player is not pregnant];
				if( CoC.player.pregnancyType === 0 ) {
					MainView.outputText( '  You can\'t help but wonder how virile those dog morphs might have been as their cum and the cum of other customers sloshes around inside your uterus.' );
				}
	...
	SceneLib.registerScene( 'niamh', new Niamh() );
} );