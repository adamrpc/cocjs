...
angular.module( 'cocjs' ).run( function( EventParser, Descriptors, ImageManager, PerkLib, XmasBitch, $rootScope, CoC, kFLAGS, Utils, StatusAffects, PregnancyStore, EngineCore, ConsumableLib, OnLoadVariables ) {
	...
	Niamh.prototype.telAdreNiamh = function() {
		if( CoC.getInstance().flags[ kFLAGS.MET_NIAMH ] === 0 ) {
			...
			EngineCore.outputText( '.  Smooth, touchable skin covers her chest, face, hands and presumably the rest of her; it seems her animalistic accoutrements stop at kitty ears and tail. She takes a deep swig of her drink, following it up with a cute little burp.' );
		}
	...
	Niamh.prototype.talkToNiamh = function() {
		...
		EngineCore.choices( 'Beer', beer, '', null, '', null, '', null, 'Leave', CoC.getInstance().scenes.camp.returnToCampUseOneHour );
	};
	//Leave;
	...
	Niamh.prototype.blackCatBeerEffects = function( character, clearScreen, newLine ) {
		...
		EngineCore.outputText( 'Uncapping the mug, you swill the stuff down in a single swig, gasping as it burns a fiery trail into your belly.  It\'s rich and sweet, and damn, but it\'s strong stuff!' );
	...
	Niamh.prototype.bimboChampagne = function( character, clearScreen, intro ) {
		...
		if( intro ) {
			if( (character.findPerk( PerkLib.FutaFaculties ) >= 0 && character.findPerk( PerkLib.FutaForm ) >= 0) || (character.findPerk( PerkLib.BimboBody ) >= 0 && character.findPerk( PerkLib.BimboBrains ) >= 0) ) {
				EngineCore.outputText( 'You could\'ve swore the stuff worked when you saw Niamh do it to others, but for some reason, it had, like, no effect on you. How weird!' );
			} else if( character.findStatusAffect( StatusAffects.BimboChampagne ) < 0 ) {
				EngineCore.outputText( 'You uncork the bottle and breathe in the fizzy, spicy aroma of the sparkling liquor.  Breathing deeply, you open your mouth and begin pouring the ever-effervescent fluid inside.  It\'s sweet and slightly gooey, and the feel of it sliding down your throat is intensely... awesome?  Like, totally!' );
			} else {
				EngineCore.outputText( 'You find yourself falling even further into the dense bimbo mindset.  You do feel, like, super-good and all, though!\n\nMoaning lewdly, you begin to sway your hips from side to side, putting on a show for anyone who might manage to see you.   You just feel so... sexy.  Too sexy to hide it.  Your body aches to show itself and feel the gaze of someone, anyone upon it.  Mmmm, it makes you so wet!  You sink your fingers into your sloppy cunt with a groan of satisfaction.  Somehow, you feel like you could fuck anyone right now!' );
			}
		}
		if( character.findStatusAffect( StatusAffects.BimboChampagne ) >= 0 ) {
			...
			if( !character.hasVagina() ) {
				...
				if( character.hasCock() ) {
					EngineCore.outputText( 'the flesh under your cock[if (hasBalls = true)  and behind your [balls]]' );
				} else {
					EngineCore.outputText( 'the blank expanse of flesh that is your crotch' );
				}
				EngineCore.outputText( ' start to tingle and squirm... mmm... that feels nice!  There\'s a sensation you, like, can\'t describe, and then your crotch feels all wet... but in a good, sticky sorta way.  Oh, wow!  <b>You\'ve, like, just grown a new virgin pussy!</b>  Awesome!' );
	...
	Niamh.prototype.bazaarNiamh = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.NIAMH_STATUS ] === 2 ) {
			...
			//[Bizarre Bazaar, repeat encounter];
				...
				EngineCore.outputText( 'Surrounded by blonde-haired, horny women, herms and traps, many of them sporting stomachs swollen with Niamh\'s copious champagne lactation, the bimbo catgirl enthusiastically waves you over.  "<i>[name]!  Like, what can I do you for?</i>" she giggles.' );
			}
		}
		//Corrupt:;
		else {
			//(General notification:;
			EngineCore.outputText( 'Given the pile of snoozing bodies, you can probably hazard a guess as to where Niamh decided to set up shop.' );
			//(Approach:;
		...
		EngineCore.choices( 'Get A Drink', drink, 'Tit-Fuck', fuck, '', null, '', null, 'Back', CoC.getInstance().scenes.bazaar.enterTheBazaar );
	};
	...
	Niamh.prototype.drinkFromZeTap = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'Your eyes lock onto her large, ever-dribbling nipples, the froth of spilt champagne caked on the table supporting her mammoth breasts, and you know you just have to have it from the source.  You push your way forward and seat yourself on a plush cushion that, from its many tears and suspicious-smelling stains, has seen better days, then grasp the huge boob in front of you, making the ditzy catgirl yowl in anticipation of pleasure.' );
	...
	Niamh.prototype.getBimboChampFromNiamh = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'You explain your need for a bottle of her champagne; the ditzy kitty takes several moments to register your words, then nods idly.  "<i>Works fer me!</i>" she responds, reaching behind her and rummaging in a large burlap bag.  She produces a flask from the sack\'s depths, unscrewing the cap and attempting to line it up with a nipple.  Try as she might, she just can\'t quite reach all the way, and with an airy giggle, she passes it to you instead.' );
	...
	Niamh.prototype.seanBimboBrewing = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'The uncharacteristically gentlemanly incubus raises his eyebrow as you produce an armful of filled flasks and drop them onto his mostly-empty table.  "<i>I assume you\'d like me to do something special with these,</i>" he quips, somewhat dryly.' );
	...
	Niamh.prototype.getBimboozeFromSean = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'Sean turns as you enter, managing a thin smile as he regards you.  "<i>Excellent timing, I have just finished this!</i>"' );
	...
	CoC.getInstance().registerScene( 'niamh', new Niamh() );
} );