...
angular.module( 'cocjs' ).run( function( SceneLib, MainView, Appearance, ConsumableLib, CockTypesEnum, StatusAffects, ImageManager, Descriptors, Utils, kFLAGS, CoC, EngineCore ) {
	...
	Lottie.prototype.lottieAppearance = function( display ) {
		...
		if( CoC.time.hours >= 15 && CoC.time.hours <= 23 ) {
			if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00281 ] === 0 ) {
				...
				if( display ) {
					//[RANDOM INTROS];
					//[Encouragement 0 – 30];
					if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00280 ] < 30 && Utils.rand( 6 ) === 0 ) {
						...
						if( EngineCore.silly() ) {
							MainView.outputText( '  What can you say? You love making bacon.', false );
						}
					}
					//[Encouragement 0 – 30];
					else if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00280 ] < 30 && Utils.rand( 5 ) === 0 ) {
						...
						if( EngineCore.silly() ) {
							MainView.outputText( '  What can you say? Her pork buns are the best.', false );
						}
					}
					//[REPEAT INTROS];
						...
						if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00279 ] <= 33 ) {
							MainView.outputText( 'voluptuous', false );
						} else if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00279 ] <= 66 ) {
							MainView.outputText( 'plump', false );
						} else {
							MainView.outputText( 'curvy', false );
						}
						MainView.outputText( ' pig girl jogs on the spot to wake herself up - unintentionally jiggling her ' + this.lottieCup() + '-cup breasts and soft ass in the process. She walks over to the nearby cafe and sits down on her ', false );
						if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00279 ] <= 33 ) {
							MainView.outputText( 'bountiful', false );
						} else if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00279 ] <= 66 ) {
							MainView.outputText( 'chubby', false );
						} else {
							MainView.outputText( 'cute', false );
						}
						MainView.outputText( ' rump.', false );
					} else {
						...
						if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00279 ] <= 33 ) {
							MainView.outputText( 'voluminous', false );
						} else if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00279 ] <= 66 ) {
							MainView.outputText( 'chubby', false );
						} else {
							MainView.outputText( 'cute', false );
	...
	Lottie.prototype.lottieCup = function() {
		//(d/e/f/g);
		if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00279 ] >= 75 ) {
			return 'D';
		} else if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00279 ] >= 50 ) {
			return 'E';
		} else if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00279 ] >= 25 ) {
			return 'F';
		} else {
			return 'G';
		}
	...
	Lottie.prototype.encounterLottie = function( charge ) {
		...
		if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00310 ] + CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00311 ] + CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00312 ] + CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00313 ] + CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00314 ] + CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00315 ] + CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00316 ] + CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00317 ] + CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00318 ] + CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00319 ] + CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00320 ] + CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00298 ] > 0 && CoC.player.lust >= 33 ) {
			sex = this.lottieFuck;
		}

		MainView.clearOutput();
		MainView.outputText( ImageManager.showImage( 'lottie-encounter' ) );
		if( charge ) {
			if( CoC.flags[ kFLAGS.LIFETIME_GYM_MEMBER ] === 0 ) {
				MainView.outputText( 'The centauress working the door walks up to collect her fee, and you drop 10 gems for an hour at the gym into her hand.\n\n', false );
				CoC.player.gems -= 10;
		...
		if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00281 ] === 1 ) {
			...
			//[If Don't Meet Full Requirements];
				...
				MainView.outputText( '"<i>Oh, uh... thanks! Hey, listen – I gotta go start my yoga class now, but it was pretty great letting it all out with you. We should run into each other again sometime!</i>"  The girl flashes a grin and jogs towards the showers.  "<i>My name\'s Lottie, by the way!</i>"\n\n', false );
				MainView.outputText( 'You aren\'t really doing much else here, so you decide to return home.', false );
		...
		//[REPEAT ENCOUNTER];
			...
			MainView.outputText( 'You approach the pig girl ', false );
			if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00280 ] >= 50 ) {
				MainView.outputText( 'relaxing', false );
			} else {
				MainView.outputText( 'sitting timidly', false );
			}
			MainView.outputText( ' at a table, wearing her low-cut top and booty shorts that accentuate her ', false );
			if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00279 ] <= 50 ) {
				MainView.outputText( 'plump', false );
			} else {
				MainView.outputText( 'curvy', false );
			}
			MainView.outputText( ' figure.  ', false );
			if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00297 ] === 0 ) {
				MainView.outputText( 'Lightly sipping', false );
			} else {
				MainView.outputText( 'Slurping down', false );
			}
			MainView.outputText( ' what appears to be ', false );
			if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00297 ] === 0 ) {
				MainView.outputText( 'some juice', false );
			} else {
				MainView.outputText( 'a milkshake', false );
			}
			MainView.outputText( ', her eyes wander over the room when she spots you and waves', false );
			if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00280 ] >= 70 ) {
				MainView.outputText( ' excitedly', false );
			}
			MainView.outputText( '.  You walk over to her and sit down, listening to Lottie update you on her progress.\n\n', false );
			...
			else if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00281 ] === 2 && CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00298 ] > 0 ) {
				MainView.outputText( '"<i>Hey, ' + CoC.player.short + '! I...I really enjoyed what we did. God, if that\'s what it takes to get healthy I wouldn\'t mind! I think I\'m falling in love with you already, haha!</i>" She forces a giggle at her last sentence, albeit rather nervously. You raise an eyebrow, but decide not to push the issue further.', false );
			}
			//[Second Encounter, If didn't fuck Lottie];
			else if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00281 ] === 2 ) {
				MainView.outputText( '"<i>Hey, ' + CoC.player.short + '! I...I\'m really glad we\'re doing this, y\'know? I really think I can achieve something this time, if I work hard – and if you work hard with me!</i>" You smile and tell her how glad you are that she\'s doing this. Lottie responds with a delighted expression as you both return to normal conversation.', false );
			...
			//[Encouragement moved from 79 to 80, first time];
				...
				CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00300 ] = CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00280 ];
				//, first time];
				if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00304 ] === 0 ) {
					CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00304 ] = 1;
					MainView.outputText( 'Lottie seems a lot more nervous than usual, curling her hair around her finger and fidgeting with the cup of tea in her hand. "<i>H-hi... hi ' + CoC.player.short + '.</i>" she squeaks, unable to form sentences under the immense amount of pressure she\'s put herself under. You place your hand on hers to relieve her own stress, but this seems to make her more nervous. "<i>Oh! I... gosh, ' + CoC.player.short + ', I don\'t know what to say!</i>" Lottie shakes her head several times, muttering a \'pull yourself together, Charlotte!\' before starting again. "<i>I like you! I really, really like you, ' + CoC.player.short + '!</i>" She squeals at the top of her lungs, jumping up in excitement before immediately clapping her hands over her mouth at the realisation that she\'s caught the attention of the entire cafe. You silently walk over to her, placing her hands back down before pulling her into a warm embrace. "<i>I... I...</i>" Looking down, you brush the hair out of her eyes and place a small kiss on her forehead, calming her down considerably. "<i>Do you like me too?</i>" She asks. You smile, and hug her tighter than before, the scent of black cherries and roses lingering in the air. Her voice rings through your head while you embrace the girl.  Do you like me too?\n\n', false );
					MainView.outputText( 'You don\'t know the answer.', false );
				}
				//[Encouragement moved from 79 to 80, regular];
				else {
					MainView.outputText( '"<i>Hey, ' + CoC.player.short + '.Thanks for being... nicer, again. Sometimes I really worry about you, but... well; I\'m fine with putting that behind us. I\'m glad, y\'know?</i>" She throws you a smile, confident that you won\'t mistreat her again. You smile back, silently assuring her that you won\'t, today.', false );
				}
			}
			//[Normal Encounter, Encouragement 60 - 100];
			else if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00280 ] >= 60 ) {
				//[Figure 0 – 20];
				if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00279 ] <= 20 && CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00297 ] === 0 ) {
					MainView.outputText( '"<i>Oh, ' + CoC.player.short + '! Let\'s do our best today, I really wanna lose those pounds! I know I can do my best when I\'m with you!</i>" Lottie beams, her massive tits pushed against the table. She seems to be excited for what you\'ve got in store today.', false );
				}
				//[Figure 0 - 20, convinced her to accept body];
				else if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00279 ] <= 20 ) {
					MainView.outputText( '"<i>Hey, ' + CoC.player.mf( 'stud', 'cutie' ) + '! You shouldn\'t keep a big girl like me waiting; unless you wanna get crushed by these!</i>" Lottie grins and grabs her luscious breasts firmly, giggling when she notices you\'re at full attention now. She seems to be excited for what you\'ve got in store today.', false );
				}
				//Figure 20 - 40, haven't convinced her to accept body];
				else if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00279 ] <= 40 && CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00297 ] === 0 ) {
					MainView.outputText( '"<i>Ah, ' + CoC.player.short + '! We should probably get started soon; I\'m gonna push myself to the limit today!</i>" Lottie\'s arms are folded over her chest, subconsciously trying to hide her nervousness, though squishing her pillowy tits in the process. She seems to be excited for what you\'ve got in store today.', false );
				}
				//[Figure 20 - 40, convinced her to accept body];
				else if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00279 ] <= 40 ) {
					MainView.outputText( '"<i>Hey, ' + CoC.player.mf( 'stud', 'cutie' ) + '! You shouldn\'t keep me waiting; the only thing I\'m losing right now is my patience!</i>" Lottie grins and punches your arm playfully, squealing when you try to get her back. She seems to be excited for what you\'ve got in store today.', false );
				}
				//[Figure 40 - 60];
				else if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00279 ] <= 60 ) {
					MainView.outputText( '"<i>Hey! I was looking for you earlier, y\'know? I can\'t wait to get started.</i>" She looks happy that you\'ve arrived. Lottie leans over to grab her beverage, giving you an excellent view of her supple mounds. She seems to be excited for what you\'ve got in store today.', false );
				}
				//[Figure 60 - 80];
				else if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00279 ] <= 80 ) {
					MainView.outputText( '"<i>Yo, ' + CoC.player.mf( 'big guy', 'babe' ) + '! I\'ve been waiting for you, so you better be pumped! Ready to get started?</i>" Lottie leans over, her creamy tits pushed against the table. She seems to be excited for what you\'ve got in store today.', false );
				}
				//[Figure 80 - 100];
				else {
					MainView.outputText( '"<i>Hey, ' + CoC.player.mf( 'stud', 'cutie' ) + '! You shouldn\'t keep me waiting, or I\'ll put you through the burn twice as hard! You ready?</i>" Lottie winks at you and punches your arm playfully. She seems to be excited for what you\'ve got in store today.', false );
				}
			}
			//[Normal Encounter, Encouragement 30 - 60];
			else if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00280 ] >= 30 ) {
				//[Figure 0 - 20];
				if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00279 ] <= 20 ) {
					MainView.outputText( '"<i>Oh, ' + CoC.player.short + ', hey. I think we should try and get into it today. I don\'t think I tried hard enough last time, y\'know?</i>" Lottie leans over to grab her beverage, her massive tits pushed against the table. She seems to be intent on trying her best today.', false );
				}
				//[Figure 20 - 40];
				else if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00279 ] <= 40 ) {
					MainView.outputText( '"<i>A-ah, ' + CoC.player.short + '! We should probably get started soon; I... I want to push myself to the limit today!</i>" Lottie\'s arms are folded over her chest, subconsciously trying to hide her nervousness, though squishing her pillowy tits in the process. She seems curious as to what you both plan to do today.', false );
				}
				//[Figure 40 - 60];
				else if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00279 ] <= 60 ) {
					MainView.outputText( '"<i>H-hey! I was looking for you earlier, y\'know? I can\'t wait to get started.</i>" She looks at ease now that you\'ve arrived. Lottie leans over to grab her beverage, giving you an excellent view of her supple mounds. She seems curious as to what you both plan to do today.', false );
				}
				//[Figure 60 - 80];
				else {
					MainView.outputText( '"<i>Hello ' + CoC.player.short + '! I\'ve... I\'ve been waiting for you, so you better be ready, hey! Don\'t hold back!</i>" Lottie leans over, her creamy tits pushed against the table. She seems to be excited for what you\'ve got in store today.', false );
				}
			}
			//[Normal Encounter, Encouragement 0 - 30];
			else {
				//[Normal Encounter, Figure 0 – 70];
				if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00279 ] <= 70 ) {
					MainView.outputText( '"<i>Oh, ' + CoC.player.short + '... I know I haven\'t been trying the way you want me to. I – I punished myself extra hard last night! Please, let me try again today!</i>" Lottie leans over and looks at you desperately, her creamy tits pushed against the table. You can see they\'re redder than before. Has she been trying to slap her fat away? You turn away, not giving her the pleasure of looking at your face. You agree to help her – but don\'t expect to be looked at by the only person who can stand you if this goes on. Lottie squeals and manages to blurt out a \'th-thank you!\' in response.', false );
				}
				//[Normal Encounter, Figure 70 - 100];
				else {
					MainView.outputText( '"<i>' + CoC.player.short + '! I know you don\'t like looking at me, but I... I lost some more weight for you today!</i>" Lottie removes her top and exposes her curvaceous body, plump tits in full view. You stare her down, and she smiles weakly, grateful for your patience. She waits for your response, but you turn away and tell her to cover up her disgusting body. "<i>But I – oh god, I\'m sorry, I\'m so sorry, I\'ll try extra hard today, I\'ll try so so hard...</i>" She babbles on while you go to the counter and order some food for yourself.', false );
				}
			}
		}
		//[Appearance][Talk][Exercise][Give Item][Sex][Hug];
	...
	Lottie.prototype.fourthMeetingChooseSexWithLottie = function() {
		...
		MainView.outputText( 'You tell her that you\'re fine with the idea, as long as she\'s willing to follow your lead. "<i>W-wow, you really wanna... and with me? Okay, sure, lemme just uh, hang on...</i>" She fumbles with her clothing, clearly trying to get undressed in public before you decide to stop her – there are far better places for this. Taking her by the hand, you lead her into one of the empty saunas, your erection stirring in your pants as you grow more and more eager to take her. "<i>' + CoC.player.short + '... I\'m – I really don\'t know where to start, h-hey.</i>" She throws an uneasy glance at you, her intent for you to take the lead apparent in her heavy breathing and wanting eyes. Not one to disappoint, you grab her by the hips and lean her against the wall, ripping off her clothes and revealing her velvety skin underneath. Reaching down, you grab a handful of her luxurious ass, squeezing and slapping it lightly. Lottie moans into your shoulder and reaches towards your groin, removing your ' + CoC.player.armorName + '. She begins stroking your ' + Descriptors.cockDescript( x ) + ' lightly, picking up the pace as you continue to work on her pert rump. You can\'t help but lust over her full figure, using one of your hands to firmly grope her pillowy jugs and reaching your other towards her glistening pussy. You move your hand over her cushiony rack before burying your head into her bosom and eagerly sucking on one of her nipples, fondling the other between your thumb and forefinger. Your other hand is already fast at work, fingering her hot cunt. You can feel her legs about to buckle beneath their own weight. Lottie moans in between short, hot breaths, "<i>Y-you\'re... really amazing, ' + CoC.player.mf( 'guy', 'lady' ) + '.</i>" You give her chubby ass a hard slap in agreement.  Lottie squeals with  glee, jerking your ' + Descriptors.cockDescript( x ) + ' as it pokes into her soft stomach. The sensation of your tip rubbing against her velvety skin drives you wild.\n\n', false );
		MainView.outputText( 'You tell Lottie you\'ve decided you\'re going to take her for a vigorous work out. She can only pant with anticipation.\n\n', false );
	...
	Lottie.prototype.declineBeingLottiesGymHamsandwich = function() {
		MainView.spriteSelect( 36 );
		MainView.outputText( '', true );
		MainView.outputText( '"<i>O-oh, um... okay. I understand! You\'re probably busy what with all that lifting you\'re getting up to, haha!</i>" The pig girl chuckles nervously, shifting her towel to cover more of her soft cleavage.  "<i>I\'ll uh – I\'ll go get outta your hair then. I\'ve got some stuff to do at home.</i>"  You watch as she jogs off through the door and out into Tel\'Adre, towel and all.  You decide you better return home too, and head back towards the camp.', false );
	...
	Lottie.prototype.gotInShapeButTurnedDownLottie = function() {
		MainView.spriteSelect( 36 );
		MainView.outputText( '"<i>O-oh, um... okay. I understand! You\'re probably busy what with all that lifting you\'re getting up to, haha!</i>" The pig girl chuckles nervously, shifting her towel to cover more of her soft cleavage. "<i>I\'ll uh – I\'ll go get outta your hair then. I\'ve got some stuff to do at home.</i>" You watch as she jogs off through the door and out into Tel\'Adre, towel and all. You decide you\'d better return home too, and head back towards the camp.', false );
		//[PC no longer encounters Lottie.];
		CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00278 ] = 1;
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	...
	Lottie.prototype.gotInShapeComedyDeclineLottie = function() {
		MainView.spriteSelect( 36 );
		MainView.outputText( '', true );
		MainView.outputText( 'Haha, of course you fucking aren\'t, why would you be? Lottie stares at you, tears welling in her eyes. Oh, whoops – you must\'ve said that out loud. Proceeding to not give two shits about the bawling pig-slut in front of you, you take a good squeeze of her cushiony tits before whipping your ' + Descriptors.cockDescript( CoC.player.biggestCockIndex() ) + ' out and slapping her in the face with it.  "<i>W-w-why... why are you... *sniff* p-please...!</i>" The pig girl chokes in between sobs. It almost gives you a hard on - almost. You take one final step backwards, before announcing quite loudly to the other cafe-goers that you\'re going for a home run. Lottie squeaks before you take your ' + Descriptors.cockDescript( CoC.player.biggestCockIndex() ) + ' and swing at her face, hitting dead on and managing to leave a bit of pre-cum on her newly-christened cheek. You stand triumphantly at your latest work of art, though Lottie seems to have other ideas, running out the door in complete misery, pre-cum and all. You decide you\'d better return home too, and head back towards the camp – but not before spending the next hour or so chasing after Lottie and taking grabs at her plush ass.', false );
		//[PC no longer encounters Lottie.];
		CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00278 ] = 1;
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	...
	Lottie.prototype.gotInShapeAcceptTrainingOffer = function() {
		...
		CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00299 ] = 1;
		MainView.outputText( '', true );
		MainView.outputText( '"<i>A-ah, really? Y\'mean it? This isn\'t some trick or something, right? You ain\'t about to go ahead and flake out on me, are you?</i>" You assure her you\'re serious if she is, though you make a mental note that she\'s more likely to flake out than you are, given her history. "<i>Eeee! Thanks, ' + CoC.player.mf( 'big guy', 'babe' ) + ' – I can\'t wait to get started!</i>" You chuckle, remarking on her enthusiasm. "<i>Well ' + CoC.player.short + ', I\'m excited because I think the two of us are about to become super close! Do you feel it? Do ya?</i>" Lottie wiggles in excitement, loosening the already skimpy towel and revealing one of her perky nipples. You definitely feel something, that\'s for sure. She follows your eyes down to her breasts and reddens as she quickly covers herself up as best as she can - only succeeding in making her pillowy breasts push up against the towel, increasing her cleavage.\n\n', false );
		MainView.outputText( 'You take this opportunity to get a good look at Lottie. She stands at around 4\'8</i>", considerably short on average, ', false );
		if( CoC.player.tallness > 60 ) {
			MainView.outputText( 'and shorter than you', false );
		} else if( CoC.player.tallness > 53 ) {
			MainView.outputText( 'but you can\'t judge, being roughly the same height', false );
	...
	Lottie.prototype.drinkLustWithYourHamhock = function() {
		MainView.spriteSelect( 36 );
		MainView.outputText( '', true );
		MainView.outputText( 'You decide to down the cup of pink liquid, savoring the sweet taste as it pours down your throat. Your thirst quenched, you start to walk back towards the track when you\'re suddenly overcome with a warm feeling that stretches down to your groin. ' + Descriptors.SMultiCockDesc() + ' stirs from beneath your ' + CoC.player.armorName + ' – you need to fuck something, and fast.  "<i>Uh, is something wrong?</i>" Lottie glances over to you, noticing the bulge in your pants as it strains against your ' + CoC.player.armorName + '. She looks up at your face, concerned about your well being. You grin and tell her you\'ve had enough running.  Confused, Lottie asks what you mean.  You explain that you know a far more effective way to lose weight and stay in shape – in fact, it\'s how you got to be the way you are now.  You can see Lottie\'s eyes widen at the idea of such a technique, especially considering your own well-muscled physique.  "<i>I\'m all ears! What is it?</i>"  She asks.  Clearly not getting the picture, you grab her hand and put it on your crotch.  Lottie quietly squeals and looks around to see if anyone noticed before looking back at you.  "<i>Oh, um, I don\'t- I don\'t know about that, ' + CoC.player.short + '...</i>" She mumbles, her hand still remaining on your growing hard-on.  It\'s perfectly fine - you\'ve trained like this with hundreds of people, you lie.  Lottie swallows and looks down at the sound of \'hundreds\'.  Not being one to question your methods, she begins to stroke your hardening length through your ' + CoC.player.armorName + ', using her palm to massage your shaft', false );
		//[if balls];
		if( CoC.player.balls > 0 ) {
			MainView.outputText( ' while her fingers nervously trace around your balls', false );
		}
		MainView.outputText( '. You stop her for now, though not before softly squeezing her hand as it caresses your girth. You know where you two can go \'work out\', you tell her. She nods, her eyes glued to your erection.\n\n', false );
		MainView.outputText( 'You take her by the hand and lead her into one of the empty saunas. "<i>' + CoC.player.short + '...</i>" You grab her by the hips and lean her against the wall, throwing off her towel and revealing her velvety skin underneath. Reaching down, you grab a handful of her luxurious ass, squeezing and slapping it lightly. Lottie moans into your shoulder and reaches towards your groin, removing your ' + CoC.player.armorName + '. She begins stroking ' + CoC.player.oMultiCockDesc() + ' lightly, picking up the pace as you continue to work on her pert rump. You can\'t help but lust over her full figure, using one of your hands to firmly grope her pillowy jugs and reaching your other towards her glistening pussy. You move your hand over her cushiony rack, before burying your head into her bosom and eagerly sucking on one of her nipples, fondling the other between your thumb and forefinger. Your other hand is already fast at work, fingering her hot cunt. You can feel her legs about to buckle beneath their own weight. Lottie moans in between short, hot breaths, "<i>Y-you\'re... really amazing, ' + CoC.player.mf( 'guy', 'lady' ) + '.</i>"  You give her chubby ass a hard slap in agreement.  Lottie squeals in glee, jerking your ' + Descriptors.cockDescript( 0 ) + ' as it pokes into her soft stomach. The sensation of your tip rubbing against her velvety skin drives you wild.\n\n', false );
		MainView.outputText( 'You tell Lottie you\'ve decided you\'re going to take her for a vigorous work out. She can only pant with anticipation.', false );
	...
	Lottie.prototype.analLottieMilkshake = function() {
		MainView.spriteSelect( 36 );
		MainView.outputText( '', true );
		MainView.outputText( ImageManager.showImage( 'lottie-fuck-her-in-teh-butzor' ) );
		var x = CoC.player.cockThatFits( 40 );
		if( x < 0 ) {
			x = 0;
		}
		MainView.outputText( 'You brush her grip off of ' + CoC.player.oMultiCockDesc() + ', only to turn her around and fit your girth between her large cheeks', false );
		if( CoC.player.totalCocks() === 2 ) {
			MainView.outputText( ', managing to squeeze in your other cock along with it', false );
		} else if( CoC.player.totalCocks() > 2 ) {
			MainView.outputText( ', the rest of your dicks poking and prodding against her soft flesh', false );
		}
		MainView.outputText( '.  "<i>You sure do like my butt, huh?</i>"  She giggles softly, satisfied that at least some of her fat is being put to use.  You groan as she starts to move her ample hips, massaging your ' + Descriptors.cockDescript( x ) + ' within her jiggling buttocks.  Your tip brushes against her asshole, smearing pre-cum all over the hot entrance.  Lottie starts to push back towards your cock, clearly overcome with lust.  You wait a few seconds as she aimlessly grinds against you before you spread her cheeks and plunge your ' + Descriptors.cockDescript( x ) + ' into her tight backdoor. Lottie squeals against the wall in delight as you fill her stomach and begin pounding away at her relentlessly, her large tits bouncing rhythmically. She continues to drive herself into you, her tight, fiery insides squeezing your length whilst you watch her heavy butt ripple with every thrust.\n\n', false );
		MainView.outputText( 'Lottie is a complete mess, gasping and convulsing underneath you as you plunge into her depths. Her mouth hangs wide open as she holds on to one of her plush tits, using her free hand to pleasure herself. You take hold of her lavish buttcheeks and force your length in deep, bottoming out inside her hot ass. You remain inside her and begin gyrating your hips, rubbing against the tight walls of her interior. Her legs collapse underneath you and you wrap one hand around her pot belly to hold her in place, resuming your thrusting.\n\n', false );
		MainView.outputText( '"<i>A-aa-ah, I-I... AHH!</i>" Lottie oinks noisily, her eyes rolling to the back of her head and her large thighs slapping against your own, pulling your ' + Descriptors.cockDescript( x ) + ' further in. She shudders in orgasm', false );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( ', her soaked pussy rubbing against your balls', false );
		}
		MainView.outputText( '.\n\n', false );
		//[If Multi];
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 'Deciding you aren\'t done with her bouncy rear, you take ', false );
			if( CoC.player.cockTotal() === 2 ) {
				MainView.outputText( 'your other cock', false );
			} else {
				MainView.outputText( 'one of your cocks', false );
			}
			MainView.outputText( ' and push it against her puckered hole. "<i>H-hey, wait a mome- OHH!</i>" You ease yourself back into her, with ', false );
			if( CoC.player.cockTotal() === 2 ) {
				MainView.outputText( 'your two ', false );
			} else {
				MainView.outputText( 'two of your ', false );
			}
			MainView.outputText( ' throbbing cocks buried deep in her rectum. The pressure against your hot rods is immense, though you continue to slowly push and pull her sweaty ass against you, enjoying the feeling of her meaty fuckpillows as they massage your cock', false );
			if( CoC.player.cockTotal() > 1 ) {
		...
		if( CoC.player.balls > 0 ) {
			MainView.outputText( 'Your ' + Descriptors.sackDescript() + ' begins to swell', false );
		} else {
			MainView.outputText( 'You feel yourself about to explode', false );
		}
		MainView.outputText( ' with the large amount of cum you\'ve prepared to paint her inner walls with.  Your unrelenting fucking picks up the pace as you feel yourself coming to a climax.  Lottie\'s soft flesh starts to quiver, signaling her second orgasm.  You lift Lottie\'s plump body up straight and against yours as you groan loudly and thrust hard, the noisy broad ', false );
		if( CoC.player.cumQ() >= 1000 ) {
			MainView.outputText( 'overflowing', false );
		} else if( CoC.player.cumQ() >= 500 ) {
			MainView.outputText( 'dripping', false );
		} else {
			MainView.outputText( 'filled', false );
		}
		MainView.outputText( ' with your hot, thick spunk. She squeals again as you bring her to orgasm, ramming herself into you to collect as much of your cum as possible. You continue to pound her bouncy buttocks as you release the rest of your seed inside of her', false );
		//[If large cum production];
		if( CoC.player.cumQ() >= 500 ) {
			MainView.outputText( ', a heavy amount leaking from her ass', false );
		}
		MainView.outputText( '.  Pulling yourself free, you wipe yourself against her plush butt and give it one final slap before walking out of the sauna.  "<i>I-I\'ll see you again, hey?</i>" Lottie asks, collapsed onto the floor in a mess of sweat and drool. You give her a thumbs up before turning around and heading out of the gym, making sure to grab a ', false );
		if( EngineCore.silly() ) {
			MainView.outputText( 'bacon ', false );
		}
		MainView.outputText( 'shake on the way out.', false );
		//LOTTIE FIGURE +1, MUSCLE TONE +1, DRAINS PC'S LUST.;
		this.lottieTone( 1 );
		if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00307 ] === 0 ) {
			CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00307 ] = 1;
		}
		MainView.outputText( CoC.player.modTone( 100, 1 ), false );
		CoC.player.orgasm();
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	//[If Oral];
	Lottie.prototype.getSuckedOffByAHamSandwich = function() {
		MainView.spriteSelect( 36 );
		MainView.outputText( '', true );
		MainView.outputText( ImageManager.showImage( 'lottie-gives-you-a-blowjob' ) );
		var x = CoC.player.biggestCockIndex();
		MainView.outputText( 'You grab Lottie\'s shoulders and firmly move her down towards your crotch. She kneels down, face to face with your ' + Descriptors.cockDescript( x ) + '.  Feeling her hot breath against your ' + CoC.player.skinFurScales() + ', you tell her to suck it.\n\n', false );
		//[If player cock is at or below 4'];
		if( CoC.player.cocks[ x ].cockLength <= 4 ) {
			MainView.outputText( 'She stares at your ' + Descriptors.cockDescript( x ) + ' and then up at you. "<i>I... oh, well I guess this is okay.</i>" She lazily slurps your member into her mouth, and moans at the taste of your ' + Descriptors.cockDescript( x ) + '. Her agile tongue moves over your entire length, licking gently over your sensitive head and along your shaft. Your ' + Descriptors.cockDescript( x ) + ' sits comfortably inside her warm hole, covered in her saliva', false );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( ' while your other cock', false );
				if( CoC.player.cockTotal() === 2 ) {
					MainView.outputText( ' pokes', false );
				} else {
					MainView.outputText( 's poke', false );
				}
				MainView.outputText( ' against her chin', false );
			}
			MainView.outputText( '.\n\n', false );
		}
		//[If the player has a cock between 5' – 12'];
		else if( CoC.player.cocks[ x ].cockLength <= 12 ) {
			MainView.outputText( 'She stares intently at your ' + Descriptors.cockDescript( x ) + ' with hunger in her eyes. A small amount of drool escapes her luscious lips as she moves forward, kissing your ', false );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( ' biggest head lightly', false );
			} else {
				MainView.outputText( ' head lightly', false );
			}
			MainView.outputText( '.  Her saliva leaks onto your ' + Descriptors.cockDescript( x ) + ', though she is quick to lick it up while she works her tongue around the rest of your shaft', false );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( ', jerking off your ' + Descriptors.cockDescript( CoC.player.biggestCockIndex2() ) + ' with both hands', false );
			}
			MainView.outputText( '.  She places small licks on the tip of your head, and then hungrily devours the rest of your length in one fell swoop.\n\n', false );
		}
		//[If the player has a cock equal to or over 13'];
		else {
			MainView.outputText( 'She gasps as you lift your ' + Descriptors.cockDescript( x ) + ' and slap her face. "<i>It\'s... wow, ' + CoC.player.short + '!</i>" You grin before poking her pudgy cheek with the tip of your ', false );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 'biggest erection', false );
			} else {
				MainView.outputText( 'erection', false );
			}
			MainView.outputText( '.  A small amount of drool escapes her luscious lips as she moves forward, kissing your head lightly. Her saliva leaks onto your ' + Descriptors.cockDescript( x ) + ', though she is quick to lick it up while she works her tongue around the rest of your lengthy shaft', false );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( ' - using her hands to massage your leftover ' + Descriptors.cockDescript( CoC.player.biggestCockIndex2() ), false );
			}
			MainView.outputText( '. She places small licks on the tip of your head, and without warning stuffs as much of your thick girth as she can into her mouth, coughing around your ' + Descriptors.cockDescript( x ) + '.\n\n', false );
		}
		MainView.outputText( 'She begins fellating your cock at an incredible rate, throatfucking herself on your ' + Descriptors.cockDescript( x ), false );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( ', your sack slapping against her chin ', false );
		} else if( CoC.player.biggestTitSize() > 2 ) {
			MainView.outputText( ' while you play with your ' + Appearance.biggestBreastSizeDescript(), false );
		}
		MainView.outputText( '. You remark on what a hungry little piggy she must be, and the chubby girl moans in response, the vibrations massaging your member. Relishing the feeling of your ' + Descriptors.cockDescript( x ) + ' in between her hungry lips, you start to thrust into her warm mouth hole, grabbing onto her head as you pound her face against your groin.', false );
		if( CoC.player.cockTotal() > 1 ) {
			if( CoC.player.cockTotal() === 2 ) {
				MainView.outputText( '  Your extra cock flails wildly, slapping against her cheeks.', false );
			} else if( CoC.player.cockTotal() > 2 ) {
				MainView.outputText( '  Your extra cocks flail wildly, slapping against her cheeks.', false );
			}
		}
		MainView.outputText( '  Lottie\'s slick tongue desperately wraps itself around your length as you drive in and out of her fuckhole relentlessly, drool beginning to leak from her mouth. She', false );
		//[if Multi];
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( ' stops rubbing your ' + Descriptors.cockDescript( x ) + ' to ', false );
		}//(Otherwise);
		else {
			MainView.outputText( ' reaches down towards her dripping pussy; plunging two fingers into her wetness while using the other hand to ', false );
		}
		//If Balls];
		if( CoC.player.balls > 0 ) {
			MainView.outputText( 'knead your ' + Descriptors.ballsDescriptLight(), false );
		} else {
			MainView.outputText( 'reach around and squeeze your ' + Descriptors.buttDescript(), false );
		}
		MainView.outputText( '.  You grab the back of her head and pull her in further, bobbing her head while she slurps and sucks at your meatstick. The stimulation almost causes you to explode right then and there, but her bouncing tits give you an idea.\n\n', false );
		MainView.outputText( 'You slowly take your ' + Descriptors.cockDescript( x ) + ' out of her hot mouth, a small *pop* breaking the silence as it escapes her plump, puckered lips. Grabbing one of her hefty tits in your hands, you rub your cockhead against her erect nipple, eliciting a small moan from the sweaty slut. You slap your ' + Descriptors.cockDescript( x ) + ' against her sweaty chest a few times before burying it in between her cleavage, ', false );
		//[If the player has a cock equal to or over 13'];
		if( CoC.player.cocks[ x ].cockLength > 12 ) {
			MainView.outputText( 'your sensitive tip brushing against her eager mouth', false );
		}
		//Otherwise];
		else {
			MainView.outputText( 'resting comfortably in between her cushiony fuckpillows', false );
		}
		MainView.outputText( '. Lottie wraps her tits around your length and begins working at pleasuring your every inch.\n\n', false );
		//[If the player has a cock equal to or over 13'];
		if( CoC.player.cocks[ x ].cockLength > 12 ) {
			MainView.outputText( 'Licking at your cockhead, Lottie squishes her breasts against the rest of your member, making sure to lap up the stray beads of pre-cum squirting from your tip. Relishing the taste of your ' + Descriptors.cockDescript( x ) + ' in her mouth she begins to suck at your head, flicking her tongue around your urethra and pressing her full lips against it, creating a vacuum of pleasure around your manhood.\n\n', false );
		}

		//[If Multi];
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 'You decide to make use of ', false );
			if( CoC.player.cockTotal() === 2 ) {
				MainView.outputText( 'your', false );
			} else {
				MainView.outputText( 'an', false );
			}
			MainView.outputText( ' extra cock as you push it between her pouty lips, her face brushing against it while she tries to titfuck your main member. Looking up at you, Lottie descends on you and begins fellating your ' + Descriptors.cockDescript( CoC.player.biggestCockIndex2() ) + ', not forgetting to knead her fat tits into your ' + Descriptors.cockDescript( x ) + '.\n\n', false );
		}
		MainView.outputText( 'Raising her body up and down, you watch as the pig-slut massages her rack tightly around your rod whilst you plunge between her cleavage, thrusting slightly to match her movements. "<i>' + CoC.player.short + ', p-please cum on me, god- I need this!</i>" she moans, her saliva lubing her chest while she pumps your dick in between her melons', false );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( ' and chokes on your meat', false );
		}
		MainView.outputText( '.  You throb against her hot flesh, feeling your spunk bubbling up inside you.  Throwing your head back, you give one final thrust before pulling back and exploding all over your squishy slut, ', false );
		if( CoC.player.cumQ() < 500 ) {
			MainView.outputText( 'thick ropes of cum splattering all over her face and pooling in between her tits', false );
		} else {
			MainView.outputText( 'massive globules of cum splattering all over her face and painting her tits', false );
		}
		MainView.outputText( '.  She continues to move up and down against the thick shaft between her breasts, determined to drain you of your seed. She pulls you out of her bosom and plunges your member back into her mouth, suckling on it for any stray milk left. After a while, you groan as she pops you back out, collecting the ', false );
		if( CoC.player.cumQ() > 500 ) {
			MainView.outputText( 'incredible amounts of ', false );
		}
		MainView.outputText( 'cum you\'ve painted on her flesh and gulping it all down. "<i>Thanks for the load, ' + CoC.player.mf( 'big guy', 'babe' ) + ',</i>" she remarks, sucking gently on her fingers. "<i>I\'ll... I\'ll see you again, hey!</i>" You pat her on the head and begin to make your way out of the gym, making sure to grab a ', false );
		if( EngineCore.silly() ) {
			MainView.outputText( 'bacon ', false );
		}
		MainView.outputText( 'shake on the way out.', false );
		//LOTTIE FIGURE -1, MUSCLE TONE +1, DRAINS PC'S LUST.;
		this.lottieTone( -1 );
		if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00306 ] === 0 ) {
			CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00306 ] = 1;
		}
		MainView.outputText( CoC.player.modTone( 100, 1 ), false );
		CoC.player.orgasm();
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	...
	Lottie.prototype.lotteryAppearance = function() {
		MainView.spriteSelect( 36 );
		MainView.outputText( '', true );
		MainView.outputText( ImageManager.showImage( 'lottie-examine-appearance' ) );
		//[If Figure between 0 – 20];
		if( this.lottieTone() <= 20 ) {
			MainView.outputText( 'She stands at around 4\'8</i>", considerably short on average, ', false );
			if( CoC.player.tallness > 60 ) {
				MainView.outputText( 'and shorter than you', false );
			} else if( CoC.player.tallness > 53 ) {
				MainView.outputText( 'but you can\'t judge, being roughly the same height', false );
	...
	Lottie.prototype.lottieTone = function( dif ) {
		if( dif === undefined || dif === 0 ) {
			return CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00279 ];
		}
		CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00279 ] += dif;
		if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00279 ] > 100 ) {
			CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00279 ] = 100;
		} else if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00279 ] < 0 ) {
			CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00279 ] = 0;
		}
		//Clear blowjob/anal change talk flags in case tone is getting out of whack.;
		if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00306 ] === 1 ) {
			CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00306 ] = 0;
		}
		if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00307 ] === 1 ) {
			CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00307 ] = 0;
		}
		return CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00279 ];
	};
	Lottie.prototype.lottieMorale = function( dif ) {
		if( dif === undefined || dif === 0 ) {
			return CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00280 ];
		}
		CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00280 ] += dif;
		if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00280 ] > 100 ) {
			CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00280 ] = 100;
		} else if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00280 ] < 0 ) {
			CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00280 ] = 0;
		}
		return CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00280 ];
	};
	...
	Lottie.prototype.lottieLowMoraleWeightGainEncourage = function() {
		MainView.spriteSelect( 36 );
		MainView.outputText( '', true );
		MainView.outputText( 'You tell her that she shouldn\'t cry, or worry – sometimes, these things happen; results don\'t magically appear in the first few weeks, right? Her weight is bound to fluctuate now and then, it\'s completely natural. She looks up at you, confused at your tone of voice, but relieved at the same time. "<i>I... okay. Th-thank you, ' + CoC.player.short + '.</i>" You nod and smile, waving it off as nothing. She sniffs, wiping away her tears and taking a deep breath. You relinquish your grip on her chin, and she relaxes considerably. Lottie continues to stay at ease for a few more minutes before you both decide to hit the track.\n\n', false );
		MainView.outputText( '<b>...</b>After a good workout, you decide to return back to camp, exhausted.', false );
		//ENCOURAGEMENT +2;
		this.lottieMorale( 3 );
		EngineCore.fatigue( 10 );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	...
	Lottie.prototype.lottieMediumMoraleWeightGainEncourage = function() {
		MainView.spriteSelect( 36 );
		MainView.outputText( '', true );
		MainView.outputText( 'You tell her it isn\'t her fault; these things happen all the time for a variety of reasons. Her weight is bound to fluctuate now and then, it\'s completely natural. She looks up at you, her eyes welling up slightly. "<i>Is that true? It\'s completely natural?</i>" You nod your head. "<i>I\'m glad. I\'m so glad.</i>" She sniffs, wiping away her tears and taking a deep breath. You rub her shoulder as she places her hand upon yours. Lottie continues to talk about her day for a few more minutes before you both decide to hit the track.\n\n', false );
		MainView.outputText( '<b>...</b>\nAfter a good workout, you decide to return back to camp, exhausted.', false );
		//ENCOURAGEMENT +2;
		this.lottieMorale( 3 );
		EngineCore.fatigue( 10 );
	...
	Lottie.prototype.lottieHighMoraleWeightGainEncourage = function() {
		MainView.spriteSelect( 36 );
		MainView.outputText( '', true );
		MainView.outputText( 'You tell her it\'s never her fault for something like that; and it\'s perfectly fine to be frustrated – as long as it doesn\'t detract from her main goal. Her weight is bound to fluctuate now and then, it\'s completely natural. She looks up at you, beaming at your kind words. "<i>' + CoC.player.short + ', you really do know how to make people feel better, y\'know?</i>" You nod your head in agreement – you know. "<i>I\'m glad. I\'m so glad.</i>" She sighs, taking a deep breath. You rub her shoulder as she places her hand upon yours. Lottie continues to talk about her day for a few more minutes before you both decide to hit the track.\n\n', false );
		MainView.outputText( '<b>...</b>\nAfter a good workout, you decide to return back to camp, exhausted.', false );
		//ENCOURAGEMENT +2;
		this.lottieMorale( 3 );
		EngineCore.fatigue( 10 );
	...
	Lottie.prototype.lottieLowMoraleWeightLossEncourage = function() {
		MainView.spriteSelect( 36 );
		MainView.outputText( '', true );
		MainView.outputText( 'You congratulate her on her success, in fact – you could almost say you are quite happy with her results. Lottie is visibly confused, not expecting you to react this way. Regardless, she takes the compliment for what it\'s worth and squeals happily, popping your finger into her warm mouth and suckling gently. Pulling your hand away, you pat her softly on the head. "<i>Th-thank you... thanks, ' + CoC.player.short + '.</i>"  You both share a smile, Lottie at ease with your unnaturally welcoming nature. You stand up, telling her you\'re going to go hit the track, if she\'s willing to join you – and of course, Lottie eagerly follows.\n\n', false );
		MainView.outputText( '<b>...</b>\nAfter a good workout, you decide to return back to camp, exhausted.', false );
		//ENCOURAGEMENT +2;
		this.lottieMorale( 3 );
		EngineCore.fatigue( 10 );
	...
	Lottie.prototype.lottieHighMoraleFatYoullGetThereOneTimeEvent = function() {
		...
		MainView.outputText( 'You tell Lottie that the way she looks right now shouldn\'t affect how she feels about herself as a person – her value isn\'t determined by her weight, and never will be. You pat her hand before giving her a thumbs up. You believe in her and you believe in her goal, whether it leads to a better appreciation for herself or her body. She\'s reassured by your words, stroking your hand gently. You make clear to the girl that you aren\'t going to leave her because of how she looks, because she\'s beautiful where it counts. Lottie melts, blushing hard at your compliments. "<i>Oh, ' + CoC.player.short + ', sometimes I wish I could be as brave as you, y\'know?</i>" she exclaims, sighing politely. Continuing your rant, you explain how all she needs is a little self confidence to get her going – she can\'t be afraid to take leaps into the deep end to get what she wants, right? She giggles at your enthusiasm, though takes your words into consideration. "<i>Self confidence, huh?</i>" Lottie mutters to herself. "<i>' + CoC.player.short + '... could you come with me for a moment?</i>"\n\n', false );
		MainView.outputText( 'You follow Lottie out into the pool area, waiting as she walks into the dressing room to get changed. When she comes out however, you\'re surprised to find that she hasn\'t changed into anything – in fact, she\'s removed all her clothes entirely!\n\n', false );
		MainView.outputText( 'The nude pig-girl stands in front of you, turning red at your hungry gaze. "<i>I\'m not comfortable with my body, but... if self confidence is the key to getting what I want, then... will you fuck me, ' + CoC.player.short + '?</i>"\n\n', false );
		MainView.outputText( 'In only seconds flat you remove your clothes to jump the piggy, whipping out your ' + Descriptors.cockDescript( x ) + ' and pushing it against Lottie\'s wet slit, the pig-girl lying on her back while her legs rest upon your shoulders. With a hearty thrust you drive yourself into her, eliciting a loud moan while you pork her insides with your meat. Her thick thighs slap against you as you bend over to take a lustful grab at her melons, sucking at her nipple and pinching the other, her sweet taste enveloping your mouth. Lottie buries you in her cleavage, yelping as you piledrive her tight pussy into submission – but you\'ve got more in store. Flipping her over, Lottie lies face down with her soft butt jutting up in the air, your persistent fucking driving her wild. You continue gripping onto her tits as you pound her flesh, every thrust accompanied by her pleasured grunts. Back and forth you mount her until the familiar sensation in your loins signal your release. You increase your pace, pounding her pussy until you pull your head back and ejaculate inside her inner walls, slamming in harder every few seconds with each burst.', false );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 'Your other cock', false );
			if( CoC.player.cockTotal() === 2 ) {
				MainView.outputText( ' twitches ', false );
			} else {
				MainView.outputText( 's twitch ', false );
			}
			MainView.outputText( 'in ecstasy, releasing all over her plump butt.', false );
		}
		MainView.outputText( '\n\n', false );
		MainView.outputText( 'Sweaty and panting, Lottie lies on the floor, exhausted and full of your spunk. You slap her jiggling ass before walking out of the pool area, making sure to grab a ', false );
		if( EngineCore.silly() ) {
			MainView.outputText( 'bacon ', false );
		}
		MainView.outputText( 'shake on the way out.', false );
		if( EngineCore.silly() ) {
			MainView.outputText( '  They\'re fucking delicious.', false );
		}
		MainView.outputText( '\n\n', false );
		//[Unlocks Quickie];
		MainView.outputText( '(<i>\'Quickie\' has been unlocked from Lottie\'s sex menu.</i>)', false );
		CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00314 ] = 1;
		CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00298 ]++;
		CoC.player.orgasm();
		EngineCore.dynStats( 'sen', -3, 'cor', -1 );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	...
	Lottie.prototype.ewLottieNo = function() {
		MainView.spriteSelect( 36 );
		MainView.outputText( '', true );
		MainView.outputText( 'You begin to talk about your story, about how you came to be in Mareth and how you got to be where you are now. You\'ve encountered nasty things, and you\'ve seen people become nasty things themselves. You also know that you\'ve changed since then. You\'ve become someone you didn\'t expect yourself to become in a million years – and whether it was for better or for worse was up to you to decide. You know you\'ve been through hard times, but you\'ve never once regretted the decisions you\'ve made – because they led to her. To Lottie. The girl begins to tear up at your confession. "<i>Oh, ' + CoC.player.short + ', I...</i>" But then you laugh. You laugh harder than you ever have since you came into this hellhole. In fact, you\'d be willing to bet it was the first time you\'ve ever laughed in Mareth. Lottie\'s face immediately scrunches up as she begins to bawl, but you could give less of a fuck as you stand up and point at the pig-slut as cafe-goers look at you both in curiosity. "<i>ATTENTION FURRY ASSHOLES</i>" you yell, much to the disgust of the anthropomorphic animal-people staring in your direction. "<i>THIS SLUT – "<i>you point to Lottie. "<i>- WON\'T EAT MY HAM SANDWICHES.</i>" An audible gasp is heard from the growing crowd. Lottie\'s buried her head in her hands but your tirade continues. Walking to the counter, you take one look at the fuzzy clerk before he nods and hands you the aforementioned sandwich. Weird, you don\'t remember him being fuzzy – you really need to find a pair of fucking glasses in this sand pit. Regardless, you stomp over to the pig-girl, pulling her out of her seat and forcefully shoving the meaty meal down her throat. She gags for a second before unwillingly swallowing her own species. You grab her cheeks and pull her face several inches towards yours. "<i>You disgust me.</i>" You tell her, before whipping your ' + Descriptors.cockDescript( 0 ) + ' out and fucking her ham and bread-filled mouth. </i>"Here comes the mayonnaise train!</i>" You yell, ejaculating straight down her gullet and into her flabby abyss.\n\n', false );
		MainView.outputText( 'For hours, people take turns pumping their seed into Lottie while you dance naked around her, chanting obscenities at the blurry people around you.\n\n', false );
		MainView.outputText( 'You really need glasses.', false );
	...
	Lottie.prototype.lottieSuccessOneShot = function() {
		...
		//[Otherwise];
			...
			//[If Cum Production High];
				...
				MainView.outputText( 'Unfortunately for her, you have far too much built up to stop now. With another thrust, your ' + Descriptors.cockDescript( x ) + ' pulses in delight, exploding in orgasm as ropes of spunk splatter upon Lottie\'s face and tits. The girl gasps, the sheer amount of your seed painting her white. "<i>N-no, stop! Oh god!</i>" she yelps, before her mouth is filled with your hot essence. She forces herself to swallow, coughing as you grin at the defeated slut. "<i>O-okay, y-you win, ' + CoC.player.short + '.</i>" she looks up at you and smiles, before untying your binds and setting you free. "<i>But you have to admit, this was pretty fun, right?</i>" You give her a thumbs up before walking out the door, leaving her to clean your thick mess. On the way out, a milkshake vendor waves over to you – it\'s the guy from the cafe. You buy a ', false );
			}
			if( EngineCore.silly() ) {
				MainView.outputText( 'bacon ', false );
			}
			MainView.outputText( 'shake before deciding to head home.', false );
			if( EngineCore.silly() ) {
				MainView.outputText( '  You fucking love bacon shakes.', false );
			}
			//[Unlocks FemDom];
			CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00319 ] = 1;
			CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00298 ]++;
			MainView.outputText( '(<i>\'FemDom\'</i> unlocked in Lottie\'s sex menu</i>)', false );
		}
		CoC.player.orgasm();
		EngineCore.dynStats( 'sen', -2 );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	...
	Lottie.prototype.leaveLottieInsteadOfYogaSex = function() {
		MainView.spriteSelect( 36 );
		MainView.outputText( '', true );
		MainView.outputText( 'As excited as you are, your mind is telling you to leave. In blissful ignorance, you walk out the door of the cafe, through the pool area, into the main gym and out the main entrance. Once outside, you figure it\'s as good a time as any to take a friendly stroll back to the camp.\n\n', false );
		MainView.outputText( 'As you head home, you have the strangest feeling you\'ve missed out on incredibly sexy sex.', false );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	//[Exercise];
	...
	Lottie.prototype.boastAtLottie = function() {
		MainView.spriteSelect( 36 );
		MainView.outputText( '', true );
		MainView.outputText( 'You laugh as you pick up one of the larger ones and pump mercilessly, looking at her expectantly as if she had been failing out of laziness. She tries harder to pull the heavy object over her head, but eventually she begins to shake and drops it, landing in the sand with a ', false );
		if( EngineCore.silly() ) {
			MainView.outputText( 'large *pomf =3*', false );
		} else {
			MainView.outputText( 'loud clonk', false );
		}
		MainView.outputText( '. She looks up at you and sniffles, but you just grunt in response, disappointed with her efforts. She sighs and tries her best to do it again.\n\n', false );
		MainView.outputText( 'After a few more attempts at the weights, she eventually buckles under the pressure and gives up, walking home by herself. You snort and continue a few more well-earned reps before dropping down and recharging for the night.', false );
		EngineCore.fatigue( 10 );
	...
	Lottie.prototype.trainWithLottie = function() {
		...
		MainView.outputText( 'After a few more attempts at the weights, you both decide you\'ve had enough, going your separate ways and waving as you leave the gym.', false );
	...
	Lottie.prototype.sprintAndLeaveLottieBehind = function() {
		...
		MainView.outputText( 'A few rounds later, you\'ve exhausted yourself so you decide to return home. You notice that Lottie\'s nowhere in sight, but it\'s her loss - not yours.', false );
	...
	Lottie.prototype.jogWithLottie = function() {
		MainView.spriteSelect( 36 );
		MainView.outputText( '', true );
		MainView.outputText( 'You decide that sprinting wouldn\'t really get either of you anywhere, though taking things a little faster wouldn\'t be any harm. You take the power walking to the next level with a quick jog, before reminding Lottie to take this as leisurely as possible - it isn\'t a race. Lottie quickly picks up the idea and starts to push herself further, trying her best to keep up with you, whilst maintaining a steady rhythm on the track. You both spend the good part of the evening jogging a fair distance, before deciding to retire for the night. You\'re both tired, but satisfied with today\'s efforts.\n\n', false );
	...
	Lottie.prototype.sprintAndLeaveFitLottieBehind = function() {
		...
		MainView.outputText( 'A few rounds later and you\'ve exhausted yourself, so you decide to return home. You notice that Lottie\'s nowhere in sight, but it\'s her loss - not yours.', false );
		EngineCore.dynStats( 'tou', 0.2, 'spe', 0.25 );
		this.lottieMorale( -3 );
		EngineCore.fatigue( 10 );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	...
	Lottie.prototype.lottieYoga = function() {
		...
		MainView.outputText( '', true );
		//[If Figure 0 – 100];
		MainView.outputText( 'You know that Lottie has an affinity for yoga, so you decide to enter the dimly lit room at the end of the pool. She gladly follows, bouncing along with you as you take two of the mats out. A class has already started, and you can see many people gathering around a tanned, four-armed woman performing warm-up exercises. Knowing you both have a little time before they break out into the more complicated stretches; you scan around the room for optimal places to sit down without blocking your view of the tetra-armed instructor. By the time you\'ve searched the room, a few more people had already blocked off most of the available spots. You look over to see Lottie already spread out and lying on her stomach – she looks over to you and giggles before patting the vacant area next to her. Throwing a quick smile, you also notice a small unused area right next to the teacher – you\'d be able to take it if you went by yourself. Should you go at it alone, or stretch with Lottie?', false );
		//[Alone] [Together];
	...
	Lottie.prototype.lottieYogaAlone = function() {
		MainView.spriteSelect( 36 );
		MainView.outputText( '', true );
		MainView.outputText( 'Taking your mat, you walk over towards the four-armed yoga trainer, sitting down only a meter or two away from her tanned, muscular body. In the corner of your eye, you can see Lottie visibly disheartened as she watches you gazing at the fit, caramel-colored woman in front of you. As you take your position, the intense smell of coffee and desert flowers hits you as the instructor lays on her back and pulls both of her legs up, giving you a clear view of her large thighs and puffy mound. You swear you can see her smiling as the other surrounding members begin to imitate her position. Not wanting to disappoint, you lie on your rear and tilt your head backwards, confronted by another pair of legs lifted into the air. You can see Lottie attempting the same position, only to lean to her right and fall over, earning an annoyed stare from the person next to her.\n\n', false );
		MainView.outputText( 'You continue to take on the vaguely sexual positions of the athletic yoga teacher, the class ending before you know it. Your head strangely dizzy from the smell of sweat and pheromones, you walk home by yourself, completely forgetting about Lottie.', false );
		MainView.outputText( CoC.player.modThickness( 30, 1 ), false );
		MainView.outputText( CoC.player.modTone( 60, 1 ), false );
		this.lottieMorale( -2 );
	...
	Lottie.prototype.jumpInPoolBeforeLottieSeesYourWingWang = function() {
		MainView.spriteSelect( 36 );
		MainView.outputText( '', true );
		MainView.outputText( 'You can\'t risk it, so you decide to take a plunge into the water – it\'s freezing at first, but you quickly warm up. Lottie takes your avoidance to heart and looks visibly discouraged, though you wonder what her reaction would\'ve been if your cockrocket', false );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 's', false );
		}
		MainView.outputText( ' decided to say hello. Needless to say, you decide it\'d be better if you swam alone – nothing\'s wrong with the occasional chubby-assed slut wandering now and then, but it\'s a pain to not be distracted. Lottie wades at the other end of the pool, doing the occasional stretch before breaking into laps. You take a page from her book and begin to work back and forth from either end, making sure not to stare at Lottie\'s massive chest as it bounces into the air every so often after a stroke. After a considerable amount of time, you both finish considerably more exhausted than before. She seems to have lightened up since your initial reaction to her body, so you shrug it off and head home.', false );
		//ENCOURAGEMENT -1, FIGURE +2, MUSCLE TONE +2, LUST +9001;
		MainView.outputText( CoC.player.modTone( 100, 2 ), false );
	...
	Lottie.prototype.waitAndShowLottieYourPoolBoner = function() {
		MainView.spriteSelect( 36 );
		MainView.outputText( '', true );
		MainView.outputText( 'You figure it can\'t do much harm to let ' + Descriptors.sMultiCockDesc() + ' fly, so you decide to leave it be, even leaning back so as to accentuate the hardening length. It doesn\'t take long for Lottie to notice, letting out a little gasp as she observes your ', false );
		if( CoC.player.biggestCockArea() >= 15 ) {
			MainView.outputText( 'pride and joy', false );
		} else if( CoC.player.biggestCockArea() < 5 ) {
			MainView.outputText( 'secret shame', false );
		} else {
			MainView.outputText( 'painfully average prick', false );
		}
		MainView.outputText( '. You almost experience a tinge of regret from the now incredibly awkward silence, but it\'s thrown out the window as soon as you feel a soft hand grab your throbbing member. Looking up, Lottie\'s face has reddened considerably, but her curiosity overrides her shame as she begins to slowly massage you to full mast. "<i>I... I did this?</i>" she asks, facing you with large, wondering eyes. You grin and nod, taking her hand and helping her stroke it further, only to be met with a slap on your wrist as she removes her grip. You look up, frowning as Lottie laughs at the situation. "<i>I\'m – it\'s nice to know that someone likes me.</i>" She stutters, smiling timidly. "<i>But, um, we\'re here to exercise, ' + CoC.player.mf( 'big guy', 'babe' ) + '. We should do that.</i>" You groan at the sudden cockblock, but you can\'t really argue. She takes your hand and leads you into the water, but not before you playfully grab a handful of her plush ass.\n\n', false );
		MainView.outputText( 'A while later and you\'ve both finished off several laps of the pool, exhausted from your progress. Lottie flashes you a large smile before you part ways, leaving you still somewhat aroused from the experience.', false );
		MainView.outputText( CoC.player.modTone( 100, 2 ), false );
		EngineCore.dynStats( 'lus', 80 );
		this.lottieTone( 2 );
		this.lottieMorale( 2 );
	...
	Lottie.prototype.hamSammitchTimeBIATCH = function() {
		MainView.spriteSelect( 36 );
		MainView.outputText( '', true );
		if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00324 ] === 0 ) {
			CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00324 ]++;
			MainView.outputText( 'You walk over to the cafe, Lottie watching as you order a sandwich from their menu. In a matter of seconds the collie-like clerk pulls one up from the glass display and hands it over to you', false );
			if( EngineCore.silly() ) {
				MainView.outputText( ', mumbling something about lifetime members receiving free pig-related food items', false );
			}
			MainView.outputText( '. You exchange a smile before walking back to the table and handing Lottie the moist meal, explaining that you saw the ham sandwich the other day, figuring she might enjoy it. Lottie throws it on the floor. "<i>What the heck is wrong with you?</i>"\n\n', false );
			MainView.outputText( 'You reach for a high five that never comes.', false );
		}
		//[If Ham Sandwich, Repeat Encounter, High Corruption];
		else if( CoC.player.cor >= 75 ) {
			MainView.outputText( 'You walk over to the cafe, Lottie watching as you order a sandwich from their menu. In a matter of seconds the collie-like clerk pulls one up from the glass display and hands it over to you, mumbling something about giving a pig-girl a ham sandwich being a stupid fucking idea. You exchange a smile before quickly walking out of the cafe and into the currently empty pool area. You hastily remove your ' + CoC.player.armorName + ' and take out your ' + Descriptors.cockDescript( 0 ) + ', jamming it straight into the sandwich and fucking it relentlessly. In moments you cum between the velvety slices of ham, leaving a large puddle inside and leaving the rest of your spunk leaking out the back and pooling onto the floor. You dress yourself before walking back to the table and handing Lottie the incredibly moist meal, explaining that you saw the ham sandwich the other day, figuring she might enjoy it – and that you\'ve taken care into adding your own \'special mayonnaise\'. Lottie throws it on the floor. "<i>Don\'t cum inside ham sandwiches, asshole!</i>"\n\n', false );
			MainView.outputText( 'You reach for a high five that never comes.', false );
			CoC.player.orgasm();
		}
		//[If Ham Sandwich, Repeat Encounter];
		else {
			MainView.outputText( 'You walk over to the cafe, Lottie watching as you order a sandwich from their menu. In a matter of seconds the collie-like clerk pulls one up from the glass display and hands it over to you, mumbling something about giving a pig-girl a ham sandwich being a stupid fucking idea. You exchange a smile before walking back to the table and handing Lottie the moist meal, explaining that you saw the ham sandwich the other day, figuring she might enjoy it. Lottie throws it on the floor. "<i>Stop giving me ham sandwiches, asshole!</i>"\n\n', false );
			MainView.outputText( 'You reach for a high five that never comes.', false );
		}
		EngineCore.dynStats( 'cor', 1 );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	...
	Lottie.prototype.lottieFuck = function() {
		...
		if( EngineCore.silly() && CoC.player.hasCock() ) {
			thirtyMinute = this.lottiesThirtyMinutePigGasm;
		}
		if( CoC.player.tentacleCocks() > 0 && CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00306 ] === 2 ) {
	...
	Lottie.prototype.lottieRepeatWalkies = function() {
		MainView.spriteSelect( 36 );
		MainView.outputText( '', true );
		MainView.outputText( ImageManager.showImage( 'lottie-fuck-her-in-public-repeat' ) );
		var x = CoC.player.cockThatFits( 40 );
		if( x < 0 ) {
			x = 0;
		}
		MainView.outputText( 'A few minutes later and you\'ve taken your whore outside of the gym and around to the back, where an equipment shed lies waiting. You make quick work of the lock, busting it open with a nearby metal pole. The two of you walk slowly inside, Lottie pausing behind you to gaze at the tools and exercise apparatuses littering the walls. Soon, you find what you\'re looking for. You\'ve never been one to judge a person for their fetishes, but you make a mental note to thank the person who couldn\'t control theirs. A single leather collar attached to a leash dangles silently upon a hook attached to the walls of the shed. Grinning, you take it down, adjusting the length of the rope before handing it to Lottie. "<i>W-what\'s this for, ' + CoC.player.mf( 'sir', 'ma\'am' ) + '?</i>" she mumbles, but you pay no attention to her questioning as you loosen the collar and push it down on the girl\'s neck, tightening it before she can protest. "<i>N-no! What are yo-!</i>" Slap! You pelt your hand against her soft cheek, causing her to jerk on the rope and fall to the floor, miserable and in pain. Your slut slowly looks up at you with watering eyes, a hand to her face to quell the stinging sensation. "<i>I... I understand.</i>" She mutters quietly, picking up the leash and placing it in your hands, before standing on her knees and adopting a begging position in front of you. You grunt as the girl pulls out her small tongue, reluctantly licking at your groin to satisfy the beast behind your clothes. Just like a whore to assume that\'s what you\'re here for. You walk away from her attempts to fellate you, pulling on the leash and yanking your girl forward. She gets up and starts walking behind you and out into the sunlight - you want the world to know how well you\'ve trained your fat little piggy.\n\n', false );
		MainView.outputText( 'You look back to the shed to see if there\'s anything you might be missing. Weights, large poles, a whip, some rope, and – ah yes, the steel ring gag. \'Where there\'s a whip, there\'s a way\', you chuckle to yourself as you take the strap and bring it over to Lottie. Your pig slut is naked and ready, waiting on your command as you pull out the ring-shaped device and stretch the strap over her head, placing the metal ring inside her mouth and forcing the pale slut to make a permanent \'O\'-shape with her lips. You tighten the gag, causing Lottie to cough uncomfortably as drool forms at the base of her mouth. Taking the leash, you drag Lottie away from the shed, her large ass jiggling as she tries to catch up to you. You figure you\'ll take her to the backstreets of Tel\'Adre – you want people to see your work of art, but you know only the scum of the city will truly appreciate her for what she is. You pull the leash, leading Lottie through an alleyway and into a smaller, dirtier part of the desert city, an enclosed area with several food stalls littered around.  A majority of the lower-class citizens wander these parts, shuffling about as they haggle, argue and steal – as is usual in the slums. Most people notice your pig-girl trotting behind you, though few offer a second glance. The majority stand and stare as you present your trophy – a few of the more courageous ones openly masturbate to the site, whereas the braver still walk up and slap your pig on the ass, eliciting a squeal or a moan. You decide to up the ante while you walk through the commons, placing a hand between Lottie\'s pale thighs and thrusting two fingers into her pussy, the heat enveloping your digits as you shamelessly fingerfuck the poor girl.\n\n', false );
		MainView.outputText( 'On your walk, you find yourself in a familiar location – the back alleyway of the Wet Bitch. Figuring it wouldn\'t hurt to take your slut inside; you jerk your hand forward, forcing Lottie to walk towards the door – when you recognize the face of a furiously masturbating fox herm.  Urta looks at you while she pumps her thick horsecock relentlessly, winking at the pig-slut you\'ve currently got on a leash. She doesn\'t seem to recognize you again, which could either mean she\'s somehow contracted serious amnesia, or she\'s a lot drunker than usual. "<i>', false );
		if( !EngineCore.silly() ) {
			MainView.outputText( 'Heyyy, is she like *hic* your shex slave er shumthin?', false );
		} else {
			MainView.outputText( 'Heyyy, do I shmell bacon?  I fucking love bacon!  BACON BACON BACON!', false );
		}
		MainView.outputText( '</i>" You decide to give the drunk vixen a treat. Pulling your slick fingers out of her cunt, you push Lottie towards the gray fox-woman, tugging on the leash to jerk her backwards onto the floor. As she tries to get up, the girl squeals as Urta\'s large equine member slaps Lottie on the nose. You take out your own ' + Descriptors.cockDescript( x ) + ', barking at her to keep on all fours – as long as you own the backside, you couldn\'t care less what the fox-herm does to her face. Urta starts by rubbing the flat head of her length on the full lips of your slut, the scent of her intoxicating pre-cum enticing Lottie to place small licks along the wet tip. Not wanting to be left out, you forcefully grab her hips, gliding your ' + Descriptors.cockDescript( x ) + ' straight into the slut\'s warm hole, groaning as you feel her walls tighten around you. You jerk as her hips suddenly thrust backwards, and you glance over to find Urta burying as much of her thick cock into the girl\'s mouth as she can. You both glance at each other, sharing a nod before you both plunge your lengths into the sobbing pig-whore, rocking back and forth. Urta moans while she holds onto the back of Lottie\'s head with one hand, fondling her own tits with the other. On the other end of your slave, you fuck relentlessly as you hold onto her fat asscheeks, panting heavily from the pleasure of them slapping against your groin. The fox-herm lets out a loud moan before pushing as much of Lottie\'s face into her groin as she can, her balls growing as she prepares to fill the slut\'s throat. You feel yourself about to explode as well, taking longer, and deeper thrusts into the girl before finally pushing forwards and filling her with your spunk, the fox-woman on the opposite end forcing her twitching, animalistic member into Lottie\'s throat and doing the same. The both of you slowly remove yourselves from your trophy, the pale, sweating girl now on the floor, thick seed leaking from both ends. With one more grunt you unload the rest on her ample butt, Urta painting her face.\n\n', false );
		MainView.outputText( 'You decide to leave Lottie there while Urta jerks off for another round at her mouth, heading back to the gym to grab a ', false );
		if( EngineCore.silly() ) {
			MainView.outputText( 'bacon ', false );
		}
		MainView.outputText( 'shake.', false );
		if( EngineCore.silly() ) {
			MainView.outputText( '  You sure do love spit roasting a pig, but nothing can beat a bacon shake.', false );
		}
		CoC.player.orgasm();
	...
	Lottie.prototype.lottieRepeatCowGirl = function() {
		MainView.spriteSelect( 36 );
		MainView.outputText( '', true );
		MainView.outputText( ImageManager.showImage( 'lottie-fuck-her-cowgirl-repeat' ) );
		var x = CoC.player.cockThatFits( 40 );
		if( x < 0 ) {
			x = 0;
		}
		MainView.outputText( 'A few minutes later and you\'ve both gone to the yoga rooms – you\'ve already placed a mat down while you wait for Lottie to change. Your rosy lover bounces into the empty, dim lit room, her ample thighs squishing out of a pair of super tight shorts, while her breasts fill over a sports bra. You\'re immediately turned on by the incredibly erotic display, Lottie catching scent of your increasingly hardening member and giggling while she crushes her boobs together in front of you. "<i>I\'m guessing you like what you see?</i>" she purrs, walking over to you and pushing you down upon the gym mat. Lottie makes quick work of your lower clothes, your ' + Descriptors.cockDescript( x ) + ' bouncing out and at attention, which she grabs hastily as she begins to jerk you off. The pig-girl presses your member against her moist slit, grinding it back and forth while her moans fill the empty room. You grunt in pleasure, jerking your hips slightly to help the process along, to which she replies by bending down to press her overflowing tits against your chest. You take your hands and slap them upon her ass, groping and squeezing while she moans into you. The girl shudders in bliss as the grinding sends her into an early orgasm. "<i>I... I\'ve got something better for you, babe...</i>" she pants as she pulls apart the opening in her shorts to reveal her blushing pussy. Keeping a firm grip on your ' + Descriptors.cockDescript( x ) + ', Lottie lifts herself up and slowly lowers her body upon it, your tip parting her folds and entering her hot insides. You grunt at the sensation of her inner walls squeezing tightly against your member, ', false );
		if( CoC.player.cockArea( x ) <= 40 ) {
			MainView.outputText( 'her ample cheeks resting on your groin as you bottom out in her', false );
		} else {
			MainView.outputText( 'though you only reach so far before you\'ve filled her up entirely with your length', false );
		...
		MainView.outputText( '  You both lay on the mat, panting in ecstasy while your ' + Descriptors.cockDescript( x ) + ' continues to ejaculate inside her.\n\n', false );
		MainView.outputText( 'A while later, you\'ve both said your goodbyes for the night. Before you leave, you make sure to head out to the cafe to buy yourself a ', false );
		if( EngineCore.silly() ) {
			MainView.outputText( 'bacon ', false );
		}
		MainView.outputText( 'shake.', false );
		if( EngineCore.silly() ) {
			MainView.outputText( '  You love bacon shakes.', false );
		}
		CoC.player.orgasm();
		EngineCore.dynStats( 'sen', -2 );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	...
	Lottie.prototype.lottieRepeatQuickie = function() {
		...
		MainView.outputText( 'You follow Lottie out into the pool area, waiting as she walks into the dressing room to get changed. When she comes out however, you\'re surprised to find that she hasn\'t changed into anything – in fact, she\'s removed all her clothes entirely!\n\n', false );
		MainView.outputText( 'The nude pig-girl stands in front of you, turning red at your hungry gaze. "<i>Will you f-fuck my pussy, ' + CoC.player.short + '?</i>"\n\n', false );
		MainView.outputText( 'In moments flat you remove your clothes to jump the piggy, whipping out your ' + Descriptors.cockDescript( x ) + ' and pushing it against Lottie\'s wet slit, the pig-girl lying on her back while her legs rest upon your shoulders. With a hearty thrust you drive yourself into her, eliciting a loud moan while you pork her insides with your meat. Her thick thighs slap against you as you bend over to take a lustful grab at her melons, sucking at her nipple and pinching the other, her sweet taste enveloping your mouth. Lottie buries you in her cleavage, yelping as you piledrive her tight pussy into submission – but you\'ve got more in store. Flipping her over, Lottie lies face down with her soft butt jutting up in the air, your persistent fucking driving her wild. You continue gripping onto her tits as you pound her flesh, every thrust accompanied by her pleasured grunts. Back and forth you mount her until the familiar sensation in your loins signal your release. You increase your pace, pounding her pussy until you pull your head back and ejaculate inside her inner walls, slamming in harder every few seconds with each burst.', false );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( '  Your other cock', false );
			if( CoC.player.cockTotal() === 2 ) {
				MainView.outputText( ' twitches ', false );
	...
	Lottie.prototype.lottieRepeatSUPERNEATORARES = function() {
		...
		MainView.outputText( 'You both walk to the yoga rooms, to find Elle stretching in the empty area. She spots the both of you, waving hello -  but you all have other, dirtier intentions in mind. Elle strips down, her perky tits letting loose. You chuckle devilishly as you undress in front of your naked and wanting harem – but you only have eyes for one girl. You point at the spider-girl, informing the both of them that you only plan to fuck her. Lottie may have thought she could win you over with another slut, but you feel as if you should teach her a lesson about being too presumptuous about what you want. Elle walks towards you, her child-bearing hips swaying hypnotically as she ', false );
		if( CoC.player.cockTotal() === 1 ) {
			MainView.outputText( 'begins to caress your ' + Descriptors.cockDescript( x ) + ' with two of her hands, while the other sets to work on massaging your ', false );
			if( CoC.player.balls > 0 ) {
				MainView.outputText( 'balls', false );
			} else if( CoC.player.hasVagina() ) {
				MainView.outputText( 'pussy', false );
			} else {
				MainView.outputText( 'ass', false );
			}
		} else {
			MainView.outputText( 'uses all four of her hands to handle ' + Descriptors.sMultiCockDesc() + ', jerking each one with ease', false );
		}
	...
	Lottie.prototype.lottieRepeatBackwardsAssCowgirl = function() {
		...
		MainView.outputText( 'You both walk into the yoga area, met with the smell of sweat and flowers pervading the room. Lottie gently asks for you to lie down on your back, to which you comply quite easily. You\'re comfortable, folding your arms behind your head to give you leverage as you watch your lover walk slowly to you. Lottie turns around, her thick thighs straddling you as she sits down on your groin with her back facing your direction, giving you  a perfect view of her curvy body and plump ass. Your ' + Descriptors.cockDescript( x ) + ' nestles gently between her rosy cheeks, pulsing with lustful intent as the girl grabs her hips and begins to pleasure you through her ass. The silence of the room breaks at your fevered pants and grunts as Lottie\'s skillful assjob sends you through fits of ecstasy, every curve of her bottom jiggling against the heat of your member. Her pace picks up while she bounces upon you, squeezing and squashing her perfect ass until one final lift in the air is met with her plunging your length up into her hot interior. The feeling of your molten cock inside her immediately sends Lottie into fits of orgasmic bliss, the intense heat filling the pig-girl with pleasure. Her attempts to continue are thwarted by your ' + Descriptors.cockDescript( x ) + ' scraping against her inner walls, her thick thighs clenched together while she slowly moves up and down upon you. Deciding to take things into your own hands, you thrust upwards, almost pushing the girl off with your strength and causing Lottie to squeal as you begin pounding her irresistible hole. Her lush buttcheeks jiggle with every movement you make, the rising and falling of her body moving in accordance to your own will. You pump in and out, keeping a steady rhythm inside the hot and bothered woman.\n\n', false );
		MainView.outputText( 'Inevitably, you can feel your release approaching, and with one final push you grab a hold of the pig-girl\'s tight ass, slamming against her and pumping your spunk directly into her clenched pussy, your ' + Descriptors.cockDescript( x ) + ' twitching as ropes of cum coat her insides. Lottie yelps, collapsing on the floor and full of your hot seed. You wipe your cock along her ass cleavage, leaving her as a sweaty, cum-filled mess on the floor. Walking out, you make sure to grab a ', false );
		if( EngineCore.silly() ) {
			MainView.outputText( 'bacon ', false );
		}
		MainView.outputText( 'shake from the cafe. They\'re fantastic.', false );
		CoC.player.orgasm();
		EngineCore.dynStats( 'sen', -2 );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	...
	Lottie.prototype.lottieRepeatFEMDOMFEMFDOM = function() {
		...
		MainView.outputText( 'You both walk into the yoga area and are met with the smell of sweat and flowers pervading the room. You look around, unable to find your lover until –THUNK! A heavy weight is brought down upon your head, pulling you down to the ground and away from consciousness as everything fades to black.\n\n', false );
	...
	Lottie.prototype.tentaDickLottieButtFuckbuttFuckButtFUCKBUTTFUCKBUTTFUCK = function() {
		MainView.outputText( '', true );
		MainView.outputText( ImageManager.showImage( 'lottie-fuck-her-with-tentacle-dick' ) );
		var x = -1;
		var y = -1;
		var z = -1;
		var zz = -1;
		...
		var zzzzzzzz = -1;
		var counter = 0;
		while( counter < CoC.player.cockTotal() ) {
			if( CoC.player.cocks[ counter ].cockType === CockTypesEnum.TENTACLE ) {
				if( x === -1 ) {
					x = counter;
				} else if( y === -1 ) {
					y = counter;
					...
					if( zzz >= 0 ) {
						...
						if( zzzz >= 0 ) {
							...
							if( zzzzz >= 0 ) {
								...
								if( zzzzzz >= 0 ) {
									MainView.outputText( '  Seeing the effect your junk had on her, you move your ' + Descriptors.cockDescript( zzzzzz ) + ' to her remaining hand and she grips it likewise.', false );
								}
								MainView.outputText( '  She keeps stroking you in a compulsive way, her hands clearly acting under the effect of her tremendous lust.', false );
								MainView.outputText( '\n\n', false );
								//[if another extra cock]//to be added to standard text;
								if( zzzzzzz >= 0 ) {
									MainView.outputText( 'With another cock left unsatisfied, you place your thick, tentacled member against the soft flesh of Lottie\'s ass, driving it to and fro as the sensation of her chubby flesh drives you wild.\n\n', false );
	...
	Lottie.prototype.lottieAndIfrisNTREpilogue = function() {
		MainView.outputText( '', true );
		MainView.outputText( 'A dry wind across your face rouses you from your peaceful sleep.  It seems you\'re outside the walls of Tel\'Adre.  All your equipment is sitting to the right of you, and when you gather the pile of your gear you find a note underneath.\n\n', false );
		MainView.outputText( '"<i>Dear ' + CoC.player.short + ',\n\nSleep well?  I hope so, considering I found you snoring on top of two luscious babes.  Don\'t worry about getting in trouble at the gym.  Most of the staff and customers were too busy getting themselves off to be angry or offended, though I have to admit I\'m a bit jealous.  The little piggy and that tight piece of red ass are both safe in their homes.  ', false );
		if( !SceneLib.urta.urtaLove() ) {
			MainView.outputText( 'Of course, I had to fine you for public indecency - you really ought to watch how you behave, lest someone take you for a demon agent.  Next time, try and keep it in your pants.\n\n-Urta, Captain of the Guard', false );
			...
			MainView.outputText( 'I had to make a show of fining you for public indecency, but you\'ll note your money is all still there.  Pay me back sometime, okay, hun?\n\n-Always up for you, Urta', false );
		}
		MainView.outputText( '</i>"\n\n', false );
		MainView.outputText( 'Satisfied, you get dressed and strut off towards camp.', false );
		if( EngineCore.silly() ) {
			MainView.outputText( '  You\'ll make damn sure to buy Urta a bacon shake tomorrow.', false );
		}
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
