	...
	Jasun.prototype.meetJasun = function() {
		...
		if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00179 ] === 0 ) {
			...
			//IF FEMALE;
				...
				MainView.outputText( '"<i>Look at that, it\'s like you were ready for me the whole time!</i>" He motions with one of his clawed fingers at your attire. You shamelessly pose in a proud manner and it causes him to laugh heartily. He seems unperturbed or even turned on by the fact that your swimsuit barely covers anything at all, almost like he\'s totally used to seeing people wear this kind of thing. You wonder what world he could possibly be from to end up the way he is. "<i>Come, come with me. I have something to show you.</i>" He walks up to you and pushes by you with his humongous frame. As he does, he grabs your left wrist with his right arm and all but drags you from the changing room to show you whatever he\'s talking about.\n\n', false );
				//[Continue];
				EngineCore.doNext( this, this.meetJasun2 );
				return;
			}
		}
		//Met Jasun but not boned!;
		else if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00179 ] > 0 && CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00180 ] === 0 ) {
			if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00181 ] === 0 ) {
				MainView.outputText( 'While you look around the changing room, one of the gym\'s other patron asks you if you\'re looking for Jasun.  Seeing your confusion, they describe the hunky shark guy you met before.  You nod and smile, happy to have learned his name.\n\n', false );
				CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00181 ]++;
			}
			//If you're a dude.;
			if( CoC.player.gender <= 1 ) {
				MainView.outputText( 'Jasun walks out of a stall, his thick and muscular figure still as glorious as the last time you set eyes upon it.  He glances your way and smirks.\n\n', false );
				MainView.outputText( '"<i>Eyes to yourself, boy. Try to play any games and I\'ll tear you apart limb from limb. Now get out of here before I do it anyway.</i>" The shark-morph snarls threateningly before turning to walk away.\n\n', false );
				MainView.outputText( 'You doubt he\'d have the nerve to murder someone here, but you feel like you\'ve wasted your time.', false );
				EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
				return;
			}
			//IF YOU FAIL TO MEET ANY OF HIS CRITERIA NOW;
			if( CoC.player.tone < 75 || CoC.player.armorName !== 'slutty swimwear' ) {
				MainView.outputText( 'Jasun walks out of a stall, his thick and muscular figure still as glorious as the last time you set eyes upon it. As he turns to greet you, though, you notice that his smirk suddenly turns around to a sense of shock. He\'s noticed that you\'re not quite the same as you were before, and you know it. He pauses a moment before turning back toward the mirror and addressing you without eye contact.\n\n', false );
				MainView.outputText( '"<i>' + CoC.player.short + ', you know, I still don\'t think you\'re quite ready to swim with me. That\'s fine, and it\'s entirely up to you, but I think today I should probably swim alone. You can come and visit me anytime, of course, maybe when you\'re ', false );
				if( CoC.player.tone < 75 ) {
					MainView.outputText( 'fit enough', false );
				} else if( CoC.player.armorName !== 'slutty swimwear' ) {
					MainView.outputText( 'properly attired', false );
				} else {
					MainView.outputText( 'a little more compatible with me', false );
				}
				MainView.outputText( ', we can go swim.</i>" He hesitates for a moment at the mirror, and then walks past you quickly without making eye contact. Feeling dejected and suddenly very alone, you ignore the sensation caused by his skin brushing against yours and walk back to the gym\'s entrance. Maybe you can work yourself back into Jasun\'s graces with time.\n\n', false );
				//(-15 Lust, back to gym entrance);
				EngineCore.dynStats( 'lus', -15 );
				EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
				return;
			}
			MainView.outputText( 'Jasun comes out of his stall and smiles when he sees you, already beginning to strain against the skimpy fabric of his bottom.  He asks, "<i>Would you like to come swimming with me?</i>" though his tone indicates that swimming may involve more than a few laps.', false );
			EngineCore.doYesNo( this, this.meetJasun2, this, this.turnAwayFromAdj );
			return;
		}
		//Repeat;
		else {
			...
			if( CoC.player.gender <= 1 ) {
				MainView.outputText( 'Jasun walks out of a stall, his thick and muscular figure still as glorious as the last time you set eyes upon it.  He glances your way and smirks.\n\n', false );
				MainView.outputText( '"<i>Eyes to yourself, boy. Try to play any games and I\'ll tear you apart limb from limb. Now get out of here before I do it anyway.</i>" The shark-morph snarls threateningly before turning to walk away.\n\n', false );
				MainView.outputText( 'You doubt he\'d have the nerve to murder someone here, but it\'s clear that so long as you lack a vagina Jasun will have no interest in you.  What a waste.', false );
				EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
				return;
			}
			//IF YOU FAIL TO MEET ANY OF HIS CRITERIA NOW;
