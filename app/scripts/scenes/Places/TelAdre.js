...
angular.module( 'cocjs' ).run( function( $log, ArmorLib, Combat, OnLoadVariables, WeaponLib, PregnancyStore, Descriptors, ConsumableLib, PerkLib, Appearance, StatusAffects, Utils, CoC, kFLAGS, EngineCore ) {
	...
	TelAdre.prototype.encounterTelAdre = function() {
		...
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.TelAdre ) < 0 ) {
			...
			EngineCore.outputText( 'Once again you find the gray fox, Urta, guarding the gates.  She nods at you and whistles for her companion, Edryn once again.  The centauress advances cautiously, and you submit herself to her inspection as she once again produces her magical amulet.  ', false );
			this.telAdreCrystal();
		}
	};
	...
	TelAdre.prototype.telAdreCrystal = function() {
		...
		//-Low corruption/pure characters;
			...
			EngineCore.outputText( 'The crystal shines a pale white light.  Edryn steps back and smiles broadly at you and says, "<i>You\'ve yet to be ruined by the demonic taint that suffuses the land of Mareth.  Come, you may enter our city walls and find safety here, though only so long as the covenant\'s white magic protects us from the demons\' lapdogs.</i>"\n\n', false );
		}
	...
	TelAdre.prototype.telAdreTour = function() {
		...
		EngineCore.outputText( ' at the rather suggestive name as Urta turns around and says, "<i>With how things are, we\'ve all gotten a lot more comfortable with our sexuality.  I hope it doesn\'t bother you.</i>"\n\n', false );
	...
	TelAdre.prototype.chooseAdvancedMaterials = function() {
		EngineCore.spriteSelect( 63 );
		EngineCore.outputText( 'Yara goes back into the back and comes out with a gilded tray full of exotic materials.  She hands you a brochure and asks, "<i>Ok, now what am I going to be working with?</i>"', true );
	...
	TelAdre.prototype.normalPierceAssemble = function() {
		EngineCore.spriteSelect( 63 );
		EngineCore.outputText( 'Yara makes you comfortable and has you look away while she uses her piercing tools.  It hurts, but she\'s skilled and before you know it, your piercing is done!', true );
		//1. Amethyst (+1 int, +1 lib);
		//2. Diamond (+2 int, -1 cor);
		//3. Gold (+1 int, +1 sens);
		//4. Emerald (+1 spe);
	...
	TelAdre.prototype.gymDesc = function() {
		...
		EngineCore.outputText( 'Though the gym sees heavy use by the city guard and various citizens, it\'s not too busy at present.', false );
	...
	TelAdre.prototype.letsDoYaraSex = function( girl ) {
		...
		} else if( (x === -1) && !girl )  // No cock that fits
		{
			if( CoC.getInstance().player.hasVagina() ) // But the PC has a vagoo! Swap over to female mode"
			{
				...
				EngineCore.outputText( '"<i>I\'m sorry, cutie. There is no way I could take that huge cock of yours!</i>" she says, looking rather crestfallen at your enormous member. Maybe come back after you\'ve shrunk it down to a reasonable size?' );
				return;
			}
		}
