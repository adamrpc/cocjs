...
angular.module( 'cocjs' ).run( function( SceneLib, MainView, $log, Descriptors, ConsumableLib, PerkLib, AppearanceDefs, Appearance, CockTypesEnum, StatusAffects, Utils, CoC, kFLAGS, EngineCore ) {
	...
	Farm.prototype.farmExploreEncounter = function() {
		...
		if( CoC.player.statusAffectv1( StatusAffects.MetWhitney ) < 2 ) {
			...
			if( CoC.player.findStatusAffect( StatusAffects.MetWhitney ) < 0 ) {
				...
				MainView.outputText( 'You stumble across Whitney\'s farm again.  The anthropomorphic canine woman gives you a friendly wave and tosses you another Canine pepper.\n\n', true );
				CoC.player.addStatusValue( StatusAffects.MetWhitney, 1, 1 );
				if( CoC.player.statusAffectv1( StatusAffects.MetWhitney ) === 2 ) {
					MainView.outputText( '<b>You\'ve been to the farm enough to easily find it.  You can return by selecting it from the places menu (and will no longer encounter it during random lake exploration)</b>.\n\n', false );
				}
			}
		...
		//Repeat Offender;
			...
			if( CoC.flags[ kFLAGS.KELT_BREAK_LEVEL ] >= 4 && CoC.flags[ kFLAGS.WHITNEY_FLIPPED_OUT_OVER_KELLY ] === 0 ) {
				...
				MainView.outputText( 'As soon as you head to the farm, Whitney comes storming up to meet you.  "<i>What in tarnation do you think you\'re pulling?!</i>"' );
				...
				MainView.outputText( 'You aren\'t welcome on the farm proper, but you can see Kelly cantering about the fields, looking for you.', true );
			} else {
	...
	Farm.prototype.talkWhitney = function() {
		...
		if( CoC.flags[ kFLAGS.DISCOVERED_HIGH_MOUNTAIN ] > 0 && CoC.flags[ kFLAGS.WHITNEY_TALK_HIGH_MOUNTAIN ] === 0 && CoC.flags[ kFLAGS.WHITNEY_TALK_TELADRE ] === 1 ) {
			...
			if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00331 ] > 0 && CoC.flags[ kFLAGS.ISABELLA_MILKED_YET ] !== -1 ) {
				// Jojo only;
				MainView.outputText( ' At the far end there is a small alabaster cluster labelled ‘Jojo’.' );
			} else if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00331 ] === 0 && CoC.flags[ kFLAGS.ISABELLA_MILKED_YET ] === -1 ) {
				// Isabella Only;
				MainView.outputText( ' At the far end there is a small alabaster cluster labelled ‘Isabella’.' );
			} else if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00331 ] > 0 && CoC.flags[ kFLAGS.ISABELLA_MILKED_YET ] === -1 ) {
				// Both;
				MainView.outputText( ' At the far end there is one alabaster cluster labelled ‘Jojo’, another labelled ‘Isabella’.' );
			}
		...
		if( CoC.flags[ kFLAGS.WHITNEY_TALK_DUNGEON_FOLLOWUP ] === 0 && CoC.flags[ kFLAGS.WHITNEY_TALK_DUNGEON ] === 1 ) {
			...
			MainView.outputText( 'You find Whitney hard at work in the pepper patch. You approach her cautiously, but when she sees you she hails you brightly.' );
		...
		if( Utils.rand( 4 ) === 0 && CoC.player.hasKeyItem( 'Breast Milker - Installed At Whitney\'s Farm' ) < 0 ) {
			if( CoC.player.hasKeyItem( 'Breast Milker' ) >= 0 ) {
				MainView.outputText( 'Before you can say much of anything, Whitney exclaims, "<i>My stars! Is that one of them demon\'s milking machines?</i>"\n\n', false );
				MainView.outputText( 'You nod and tell her how you liberated it from the demonic factory and explain that even though it should be fully functional, it\'ll need to connect to some other machinery to work, and it\'s way more than any one person could handle.\n\n', false );
				...
				MainView.outputText( 'Do you give the breast milker to Whitney for her to hook up?', false );
				EngineCore.doYesNo( this, this.whitneyMilkerHookup, this, this.whitneyMilkerRefusal );
				return;
			} else if( CoC.player.biggestLactation() >= 2 ) {
				MainView.outputText( 'Whitney gives you a bemused look when you settle down for a chat.\n\n', false );
				MainView.outputText( '"<i>Ya might wanna get that looked at darlin\',</i>" she says, gesturing at milky wetness dripping from the front of your ' + CoC.player.armorName + '.\n\n', false );
				if( CoC.player.cor < 33 ) {
					MainView.outputText( 'You blush with shame', false );
				} else if( CoC.player.cor <= 66 ) {
					MainView.outputText( 'You flush with a touch of exhibitionism', false );
				} else {
					MainView.outputText( 'You flush hotly and arch your back, openly displaying your milk to the dog-girl', false );
				}
				MainView.outputText( ' as her words sink in.   Runners of milk leak down your ' + Descriptors.allBreastsDescript() + ', released by the stress of being so exposed.   In no time flat you\'re soaked in milk.\n\n', false );
				MainView.outputText( 'Whitney starts to giggle, but immediately stifles it, apologizing, "<i>Ah\'m sorry, I didn\'t mean nothing by it.  I know a few folks who LOVE being a walking milk-fountain.  If ya like, I could probably get you set up with your own milking equipment.   You\'d be able to keep up with the... fluid accumulations that seem to be troubling you.   I\'d even be able to toss you a few gems if you produce enough.</i>"\n\n', false );
				MainView.outputText( 'It almost sounds too good to be true.   The farmer-girl nods, reading your expression quite clearly, "<i>Yes, there is a bit of a catch.  I\'ll need 250 gems for the parts to get this all set up.   Equipment like this isn\'t cheap.   Whaddya say, hun?   I understand if you don\'t want to – you can always just wait for the milk to stop.</i>"\n\n', false );
				if( CoC.player.gems >= 250 ) {
					MainView.outputText( 'Do you purchase a breast-milker from Whitney for 250 gems?', false );
					EngineCore.doYesNo( this, this.breastMilkerPurchase, this, this.breastMilkerNoPurchase );
				} else {
					MainView.outputText( 'You don\'t have enough money for the milker.  You apologize and head back to camp, maybe you can get one later.', false );
					EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
				}
				return;
			}
		...
		if( Utils.rand( 4 ) === 0 && CoC.player.hasKeyItem( 'Cock Milker - Installed At Whitney\'s Farm' ) < 0 && CoC.player.hasKeyItem( 'Cock Milker' ) >= 0 ) {
			MainView.outputText( 'Before you can say much of anything, Whitney exclaims, "<i>My stars! Is that one of them demon\'s milking machines?</i>"\n\n', false );
			MainView.outputText( 'You nod and tell her how you got it and explain that even though it should be fully functional, it\'ll need to connect to some other machinery to work, and it\'s way more than any one person could handle.\n\n', false );
			...
			MainView.outputText( 'Do you give the cock milker to Whitney for her to hook up?', false );
		...
		//[GENERIC TALK];
		MainView.outputText( 'You tell her of your recent trials and tribulations ', false );
		...
		MainView.outputText( '.  It feels good to get a chance to talk with another sane individual, but before long Whitney has to return to work, and you should check back on your camp.', false );
	...
	Farm.prototype.breastMilkerPurchase = function() {
		MainView.outputText( 'Whitney takes the gems and leaves with the promise of having your gear set up within the hour.  She calls back over her shoulder with a cryptic warning, "<i>Watch how much time you spend getting milked like an animal, lest you wind up like one.</i>"', true );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
		CoC.player.createKeyItem( 'Breast Milker - Installed At Whitney\'s Farm', 0, 0, 0, 0 );
		CoC.player.gems -= 250;
	...
	Farm.prototype.breastMilkerNoPurchase = function() {
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
		MainView.outputText( 'Whitney shrugs and the two of you chat about other things, just passing the time and enjoying a relatively normal chat.', true );
		//+3 int if less than 15, +2 int if less 20, +1 int if less than 30, +.5 int if less than 40.;
		if( CoC.player.inte < 15 ) {
			EngineCore.dynStats( 'int', 1 );
		}
	...
	Farm.prototype.workFarm = function() {
		...
		if( Utils.rand( 4 ) === 0 ) {
			...
			MainView.outputText( 'You steel yourself, ignore your ', false );
			if( CoC.player.faceType === AppearanceDefs.FACE_DOG ) {
				MainView.outputText( 'sensitive ', false );
			}
			MainView.outputText( 'nose, and set to work.', false );
	...
	Farm.prototype.exploreFarm = function() {
		...
		if( explore === 0 ) {
			...
			if( CoC.player.spe < 30 ) {
				...
				MainView.outputText( 'Whitney easily outpaces you, leaving you so far behind that she laps around the farm twice for each pass you make.', false );
			}
			//Less than 50 speed (+1 speed);
			else if( CoC.player.spe < 50 ) {
				EngineCore.dynStats( 'spe', 1 );
				MainView.outputText( 'Whitney is still faster than you, and manages to get far enough ahead of you to disappear from time to time.', false );
			}
			//Less than 70 speed (+.75 speed);
			else if( CoC.player.spe < 70 ) {
				EngineCore.dynStats( 'spe', 0.75 );
				MainView.outputText( 'Whitney and you are evenly matched, and the two of you run together for a while, each pushing yourself harder in an effort to best the other.', false );
			}
			//Else (+.5 speed);
			else {
				EngineCore.dynStats( 'spe', 0.5 );
				MainView.outputText( 'Whitney falls behind, unable to cope with your speed as you tear around the farm.', false );
			}
		...
		//Other stuff;
		if( explore === 1 ) {
			MainView.outputText( 'After wandering around for a while, you find yourself atop a slight rise looking out over the farm and the distant lake. Despite the corruption you know is slowly consuming this land, being here now makes you feel so at peace you wish it could go on forever.', true );
		...
		//Cows;
		if( explore === 2 ) {
			MainView.outputText( 'Your explorations take you to the cow pasture. There\'s no bull here, so the cows are all placidly grazing, building up milk for Whitney to gather. One turns to face you, and you get the odd feeling that it\'s trying to tell you something.\n\n', true );
			//[if have a horse dick and rape-high Lust] ;
			if( CoC.player.horseCocks() > 0 ) {
				MainView.outputText( 'Unbidden, the notion that a cow-slit would probably feel pretty good wrapped around your horse-prick flickers through your mind.  ', false );
				//[if high corruption];
				if( CoC.player.cor > 60 ) {
					MainView.outputText( 'It makes you smile.', false );
				}
				if( CoC.player.cor < 30 ) {
					MainView.outputText( 'It disgusts you.', false );
				}
				//[else/mid-corruption] ;
				else if( CoC.player.cor <= 60 ) {
					MainView.outputText( 'You aren\'t sure how you feel about that.', false );
				}
				//[continue paragraph condition] ;
				MainView.outputText( '  It for certain would get Whitney chasing you off with a pitchfork.\n\n', false );
				EngineCore.dynStats( 'lus', 10 );
			}
			//[if no horse dick, a deep cow or horse vag, and in heat];
			else if( CoC.player.inHeat ) {
				MainView.outputText( 'Maybe it wants a bull? You do, one with long, thick dick-meat to satisfy your starving fuck-hole...\n\n', false );
				EngineCore.dynStats( 'lus', 15 );
			}
			//- [if no dick, not in heat, but overfull with milk];
			else if( CoC.player.biggestLactation() >= 2 ) {
				MainView.outputText( '"<i>Maybe she wants to be milked?</i>" you think. You certainly do.\n\n', false );
				EngineCore.dynStats( 'lus', 3 );
			}
			MainView.outputText( 'Shaking your head, you clear your thoughts and turn away from the pasture. Cows don\'t have your problems.', false );
			EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
			return;
		}
		if( explore === 3 ) {
		...
		//[NOTHING];
			...
			MainView.outputText( 'You wander around, unable to find anything entertaining on this patch of rural bliss.', true );
			EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
		}
	};
	Farm.prototype.getMilked = function() {
		...
		if( CoC.player.findStatusAffect( StatusAffects.BreastsMilked ) < 0 ) {
			if( CoC.player.cor < 50 ) {
				...
				MainView.outputText( 'You smile eagerly as you walk inside.', false );
			}
			MainView.outputText( '  The barn is filled with the earthy smells of earth, wood, and grease.  It\'s clean for the most part, though the floor is just packed dirt and the stalls look old and well-used.  A bank of machinery along the wall thrums and pulses as if it\'s a living creature.  Hoses and cables run from it in a dozen places, disappearing into the walls.   There is even a set of stout wooden doorways along the west wall.  That must be where the farm\'s intelligent denizens stay.  You notice each of the stalls have name-plates on them, and there is even one that says ' + CoC.player.short + '.  It must be for you.\n\n', false );
		}
		//Repeat;
		else {
			...
			if( CoC.player.findStatusAffect( StatusAffects.LactationReduction ) >= 0 && CoC.player.totalCocks() === 0 ) {
				...
				MainView.outputText( 'Your ' + Descriptors.multiCockDescriptLight() + ' erect', false );
				if( CoC.player.totalCocks() > 1 ) {
					MainView.outputText( 's', false );
				}
				MainView.outputText( ' and throb', false );
				if( CoC.player.totalCocks() === 1 ) {
					MainView.outputText( 's', false );
				}
			...
			if( CoC.player.findStatusAffect( StatusAffects.LactationReduction ) >= 0 && CoC.player.cockTotal() > 0 ) {
				...
				MainView.outputText( ' own volition.', false );
			}
		...
		//REPEAT;
			...
			MainView.outputText( 'You easily attach the harnesses and lift up into position, hearing the machinery activate automatically. ', false );
		}
		...
		if( CoC.player.findStatusAffect( StatusAffects.BreastsMilked ) < 0 || application === 0 ) {
			...
			if( CoC.player.nippleLength <= 1.5 ) {
				MainView.outputText( 'pulling each of your nipples entirely into the suction-tubes.  ', false );
			} else {
				MainView.outputText( 'struggling to fit around each of your nipples as they slide into the suction-tubes.  ', false );
			}
			MainView.outputText( 'There is a mechanical lurching noise as the suction builds rapidly.  Your nipple swells out to ' + Math.ceil( CoC.player.nippleLength * 1.5 * 10 ) / 10 + ' inches of length, turning purplish from the strain.   You can feel something welling up inside your ' + Descriptors.allBreastsDescript() + ', building as it moves towards your ' + Descriptors.nippleDescript( 0 ) + 's.\n\n', false );
		}
		//Apply repeat alternate;
		else if( application === 1 ) {
			MainView.outputText( 'You stretch down and grab onto the suction cups, pulling them up to your eager nipples.  They latch on, slapping tight against you as the vacuum pressure seals them tightly against your body.  You can feel your ' + Descriptors.nippleDescript( 0 ) + 's pulling tight, nearly doubling in size from the intense pressure.  ', false );
			if( CoC.player.nippleLength >= 3 ) {
				MainView.outputText( 'They nearly burst the tubes designed to milk them by virtue of their sheer size.  ', false );
			}
			MainView.outputText( 'The sensitive flesh of your ' + Descriptors.allBreastsDescript() + ' fill with a burgeoning pressure that centers around the tubes connected to your nips.\n\n', false );
		}
		//Version 3;
		else if( application === 2 ) {
			MainView.outputText( 'Despite the tightness of your harness, you manage to reach down to grab the clear cups of the breast milker.  The cups twitch and move in your hands as you bring them up, ready to milk you.  You begin holding them against your ' + Descriptors.nippleDescript( 0 ) + 's and with a sudden lurch the suction pulls against you, pressing the breast-milker\'s cups tightly against your chest, stretching your ' + Descriptors.nippleDescript( 0 ) + 's to nearly twice their normal length.  You feel a building pressure as the machine sucks you relentlessly, drawing your milk to the surface.\n\n', false );
		}
		//Version 4 huge nips;
		if( application === 3 ) {
			MainView.outputText( 'In spite of the tightness of your harness, you collect the suction cups and bring them up to your huge nipples, letting the machine pull them into the tight cups with agonizing slowness.  In spite of the large size of your aureola, the machine slowly sucks you inside, the tightness serving only to arouse you further.  The suction pulls the walls of the nipple-tubes tight against your nipples, turning them purple as they swell up like dicks.  Drops of milk leak from the tips as your body lets your milk down, letting it flow through your imprisoned nipples towards its release.\n\n', false );
			EngineCore.dynStats( 'lus', 10 );
		}
		//Milksplosion Texts;
		//Lactation * breastSize x 10 (milkPerBreast) determines scene;
		...
		//Medium (+30 lust);
			...
			if( milksplosion === 2 ) {
				...
				if( CoC.player.totalCocks() === 0 ) {
					...
					if( CoC.player.hasVagina() ) {
						MainView.outputText( 'wet and ', false );
					}
					MainView.outputText( 'hard', false );
				}
				MainView.outputText( ' from the sensation.  Over the next hour you\'re drained totally dry, until the only answer to the machine\'s effort is a tiny trickle of whiteness.  The harness gently lowers you to the ground and releases you, leaving you feeling sore.\n\n', false );
			}
		}
		//High Output (+ 40 lust);
			...
			if( milksplosion === 0 ) {
				MainView.outputText( 'An eruption of milk floods the suction-tubes with a vortex of cream.  The machinery chugs loudly, struggling to keep up with the waves of fluid as your nipples continue to fountain into the receptacles.  You squeal in delight as your nipples get red and sensitive, but never slow in their production.  Writhing in the harness, you become more and more aroused by this milk-draining device until you feel as if you can bear it no longer.  When you get out, you\'ll NEED to get off.  After an hour of sexual torture, the suction cuts off and the harness releases.  The nipple-suckers drop off and spill your milk over the floor as droplets continue to leak from your over-productive chest.\n\n', false );
			}
			//High Output2;
			if( milksplosion === 1 ) {
				MainView.outputText( 'Your ' + Descriptors.nippleDescript( 0 ) + ' swell up like tiny balloons for a moment before they unleash a torrent of your milk.  The nipple-cylinders instantly flood to capacity, and the milking machinery chugs loudly as it tries to suck it all down the tubes, barely keeping up with you.  You pant and writhe in the harness, each pulse of milk sending a growing sensation of your warmth to your groin that makes you ', false );
				if( CoC.player.totalCocks() === 0 ) {
					if( CoC.player.hasVagina() ) {
						MainView.outputText( 'wet', false );
					} else {
				...
				MainView.outputText( ' with excitement.  The milking drags on for an hour, but your output only slows slightly, forcing the machinery to work at maximum capacity the entire time.  At last it ends, and the harnesses lower you to the ground.  The milk cups pop off, leaving your leaky tits to make a puddle on the floor.\n\n', false );
			}
			//High Output3;
			if( milksplosion === 2 ) {
				MainView.outputText( 'Milk floods the milker\'s cups as your breasts respond to the mechanized suckling.   The machinery groans as it kicks into high gear, working hard to keep up with your prodigious production rate.  Your nipples tingle with happy little bursts of pleasure as they continue to pour out ever greater quantities of milk.  Arousal wells up, flushing your body with a reddish tint that\'s difficult to hide.  You wriggle in the harness, sweating profusely and trying to grind against something, anything, whatever it takes to get off.  The milking drags on for an hour, but your breasts keep pouring out milk the entire time.  When it ends, you\'re lowered to the floor and released.  The milk-tubes pop off, leaving you lying in a milk-puddle as your leaky teats continue to drip.\n\n', false );
			}
		}
		...
		MainView.outputText( 'The machinery displays ' + liters + ' liters of milk', false );
		//If already at cap;
		if( CoC.flags[ kFLAGS.WHITNEY_GEMS_PAID_THIS_WEEK ] >= cap ) {
			MainView.outputText( ' and displays a warning that <b>you\'re producing more than Whitney can pay for</b>', false );
			payout = 0;
		}
		if( payout > 0 ) {
			...
			MainView.outputText( ' and automatically dispenses ' + Utils.num2Text( payout ) + ' gem' + (payout === 1 ? '' : 's') + '.  Whitney really went all out with this setup!', false );
			//Display a warning that you've capped out.;
			if( CoC.flags[ kFLAGS.WHITNEY_GEMS_PAID_THIS_WEEK ] >= cap ) {
				MainView.outputText( '  <b>The machinery warns you that Whitney can\'t afford any more this week!</b>', false );
			}
		...
		if( liters > 2 ) {
			...
			if( CoC.player.inte < 10 ) {
				...
				MainView.outputText( '  You stretch and let out a contented moo, long and loud.  How silly!', false );
			} else if( CoC.player.inte < 25 ) {
				MainView.outputText( '  You quietly moo, then giggle to yourself at how strange you\'re acting.', false );
			}
		}
		MainView.outputText( '\n\n', false );
		//Not very horny yet;
		if( CoC.player.lust < 75 ) {
			MainView.outputText( 'Feeling sore and VERY hungry, you make yourself decent and stagger back towards camp, ignoring the knowing smile Whitney gives you when you pass by her.', false );
		}
		//Horny;
		else {
			MainView.outputText( 'Overwhelmed with your desire, you don\'t even bother to cover up and make yourself decent, you just run out of the barn, ' + Descriptors.allBreastsDescript() + ' jiggling and wet, heading straight for camp.' );
			if( SceneLib.farmCorruption.whitneyCorruption() < 90 ) {
				MainView.outputText( ' It isn\'t until you get back that you remember the disapproving look Whitney gave you, but if anything, it only makes you hornier.', false );
			}
	...
	Farm.prototype.cockPumping = function() {
		...
		//Repeat;
			...
			MainView.outputText( 'You walk over to the barn, eagerly anticipating your session with the \'milker\'.  ', false );
			MainView.outputText( 'Your ' + Descriptors.multiCockDescriptLight() + ' erect', false );
			...
			MainView.outputText( ' with building desire.  It\'s as if ', false );
			if( CoC.player.cockTotal() === 1 ) {
				MainView.outputText( 'it', false );
			} else {
				MainView.outputText( 'they', false );
			}
			MainView.outputText( ' know', false );
			if( CoC.player.cockTotal() === 1 ) {
				MainView.outputText( 's', false );
			}
		...
		if( CoC.player.findStatusAffect( StatusAffects.CockPumped ) < 0 ) {
			...
			if( CoC.player.cockTotal() === 1 ) {
				MainView.outputText( 'single hose wriggles out from the hole, squirming across the floor.  It stops and lifts up, slowly homing in your horny shaft.\n\n', false );
			} else if( CoC.player.cockTotal() === 2 ) {
				MainView.outputText( 'pair of hoses wriggle out from the hole, squirming across the floor.  They stop and lift up, slowly homing in your horny shafts.\n\n', false );
			} else {
				MainView.outputText( 'group of hoses wriggle out from the hole, squirming across the floor.  They stop and lift up, slowly homing in your horny shafts.\n\n', false );
			}
			CoC.player.createStatusAffect( StatusAffects.CockPumped, 1, 0, 0, 0 );
		}
		//REPEAT;
		else {
			MainView.outputText( 'You easily attach the harnesses and lift up into position, hearing the machinery activate automatically.  The hungry ', false );
			if( CoC.player.cockTotal() === 1 ) {
				MainView.outputText( 'hose wriggles out from the wall, curving up towards your crotch. ', false );
			} else {
				MainView.outputText( 'hoses wriggle out from the wall, curving up towards your crotch. ', false );
			}
		...
		if( CoC.player.cumQ() >= 50 && CoC.player.fatigue >= 100 && CoC.flags[ kFLAGS.USED_MILKER_TODAY ] > 0 ) {
			...
			if( cumQ < 1000 ) {
				...
				if( CoC.player.biggestCockArea() > 70 ) {
					MainView.outputText( 'the end', false );
					if( CoC.player.cockTotal() > 1 ) {
						MainView.outputText( 's', false );
					}
					MainView.outputText( ' wrapped so snugly around your huge member', false );
					MainView.outputText( 's', false );
					MainView.outputText( ' and ', false );
				}
				MainView.outputText( 'how tired you are, you can\'t break the suction.  Every time you get a good grip and start to pull, the suction shoots a shiver of pleasure down ' + Descriptors.sMultiCockDesc() + ' and robs you of your focus and muscle control.  Practically helpless against the tubes, you feel your ' + Descriptors.multiCockDescriptLight() + ' get harder and harder, leaking out more pre-cum.\n\n', false );
			}
			//(big skeet, ordinarily would be helper-assisted);
			else {
				MainView.outputText( 'An eruption of jism boils out, splattering with loud wet noises into the tube.  You can feel the pressure build up, pushing back against your ' + Descriptors.multiCockDescriptLight() + ' as the machinery fails to keep up with the gushes of white fluid erupting inside it.  Your eyes cross from the pleasure and you start drooling all over yourself, not even noticing that you\'re about to blast the milker off your member.  The machinery grinds as it struggles to keep up, then pops off as it overflows with semen.  You don\'t even notice, instead focusing on the feeling of ejecting sticky globs from your ' + Descriptors.multiCockDescriptLight() + '.\n\n', false );
				MainView.outputText( 'Jism splatters on the floor below you as you discharge rivers of spunk.  Liters upon liters of the stuff spill out of your throbbing ' + Descriptors.multiCockDescriptLight() + ', coating the outside of the ejected suction tubes to match the inside.  Eventually you spend your reserve and the cum flow reduces to a trickle.  Stupid with pleasure, you don\'t notice the suction tube', false );
				if( CoC.player.cockTotal() > 1 ) {
					MainView.outputText( 's', false );
				}
				MainView.outputText( ' wriggling back into place on your ' + Descriptors.multiCockDescriptLight() + ' as the harness lets you down into the puddle you made.  Only when the velvet walls of the demon machine resume their caresses do you take something as amiss with the situation.\n\n', false );
				MainView.outputText( 'A loud \'ding\' sounds and a clatter of gems drop into the plate as the machine measures your contribution, but it barely registers in the back of your mind as ' + Descriptors.sMultiCockDesc() + ' hardens again from the unending, expertly-given stimulation of the animate tube', false );
				if( CoC.player.cockTotal() > 1 ) {
					MainView.outputText( 's', false );
				}
				MainView.outputText( '.  You try to pull the milker off your ' + Descriptors.multiCockDescriptLight() + ', but the outside of the tubing is slick with your semen', false );
				//[(big dix);
				if( CoC.player.biggestCockArea() > 70 ) {
					MainView.outputText( ', and the end', false );
					if( CoC.player.cockTotal() > 1 ) {
						MainView.outputText( 's', false );
					}
					MainView.outputText( ' wrap too snugly around your swollen member', false );
					if( CoC.player.cockTotal() > 1 ) {
						MainView.outputText( 's', false );
					}
				}
				MainView.outputText( '.  Every time you get a good grip and start to pull, a shiver of pleasure shoots down ' + Descriptors.sMultiCockDesc() + ' and robs you of your focus and muscle control.  Practically helpless against the tubes, you feel ' + Descriptors.sMultiCockDesc() + ' get harder and harder as the machine sucks out more pre-cum along with the milky aftershocks of your last massive orgasm.\n\n', false );
			}
			//TO BAD ENDAGE;
		...
		//Massive Scene – else;
			...
			if( CoC.player.cockTotal() === 1 ) {
				...
				MainView.outputText( 'Hurried footsteps enter the barn, and a pair of firm hands grab onto your bouncing ' + Descriptors.multiCockDescriptLight() + ', pulling the milkers back against your groin and splattering spunk around the seal from the added pressure.  The machine-suction totally cuts off, but you don\'t even care.  You just keep cumming, pumping the thick white cream down the tubes by the force of your orgasm alone.   Eventually your incredible orgasm winds down, and the hands release you, allowing the tubes to pop off.  They slowly retracts to the wall as your harness is lowered, dropping your exhausted body in a wet heap on the ground.  When you regain your senses, your mysterious helper is gone.\n\n', false );
			}
			//(+40 fatigue);
			EngineCore.fatigue( 40 );
		}
		//EPILOGUE;
		MainView.outputText( 'There\'s a ', false );
		if( CoC.player.cumQ() < 20 ) {
			MainView.outputText( 'barely noticeable ', false );
		} else {
			MainView.outputText( 'thick ', false );
		}
		MainView.outputText( 'trail of your cum along the floor where the hose dragged itself back into the wall, though the machinery has closed back up and is now chugging noisily, clearly doing something.  A loud \'ding\' chimes and a panel displays ', false );
		//Set temp to liter amount produced.;
		var payout = 0;
		var cap = 500;
		//Ez mode cap doubles;
		...
		if( cumQ > 100 ) {
			...
			MainView.outputText( cumQ + ' mLs.  ', false );
		}
		...
		if( CoC.flags[ kFLAGS.WHITNEY_GEMS_PAID_THIS_WEEK ] >= cap ) {
			payout = 0;
			MainView.outputText( 'It also shows a warning: <b>FUNDS EXHAUSTED.</b>  ', false );
		}
		//Reduce payout if it would push past;
		else if( CoC.flags[ kFLAGS.WHITNEY_GEMS_PAID_THIS_WEEK ] + payout >= cap ) {
			payout = cap - CoC.flags[ kFLAGS.WHITNEY_GEMS_PAID_THIS_WEEK ];
			MainView.outputText( 'It also shows a warning: <b>Not enough gems for full payment.  GEMS NOW EXHAUSTED.</b>  ', false );
		}
		if( payout > 0 ) {
			...
			if( payout === 1 ) {
				MainView.outputText( Utils.Num2Text( payout ) + ' gem rolls ', false );
			} else {
				MainView.outputText( Utils.Num2Text( payout ) + ' gems roll', false );
			}

			MainView.outputText( 'out into a collection plate.  Whitney really put a lot of work into this! ', false );
			CoC.player.gems += payout;
			CoC.flags[ kFLAGS.WHITNEY_GEMS_PAID_THIS_WEEK ] += payout;
			MainView.statsView.show();
		}
	...
	Farm.prototype.milkerBadEnd1 = function() {
		...
		MainView.outputText( '.  As before, the machine works you on and off for roughly an hour before the wall light once again turns green and the suction on your ' + Descriptors.multiCockDescriptLight() + ' intensifies.  Your hips take over as you buck against the machine and push out another ', false );
		if( cumQ < 100 ) {
		...
		//[(big skeet);
		if( cumQ > 1000 ) {
			MainView.outputText( '  Unlike before, a fleshy valve flap pops open in the rear of the suction cup', false );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 's', false );
			}
			MainView.outputText( ', and the pressure of your ejaculation forces squirts of cum out of the valve with every spasm, dribbling sticky whiteness back onto your crotch.', false );
		}
		MainView.outputText( '  The orgasm winds down but still the suction and the massage do not.\n\n', false );
		MainView.outputText( 'Another \'ding\'', false );
		//[(payment-worthy skeet);
		if( cumQ > 100 ) {
			MainView.outputText( ' and another clatter of gems', false );
		}
		MainView.outputText( ' reaches your ears as your second deposit is massed, but your attention is drawn to the new tube', false );
		...
		MainView.outputText( ' snaking out of the wall toward you.  Spent as you are, you can only manage a token resistance as ', false );
		if( !tubes ) {
			MainView.outputText( 'it', false );
		} else {
			MainView.outputText( 'one of them', false );
		}
		MainView.outputText( ' inserts a rod-like end into your mouth and begins trickling salty fluids down your throat.', false );
		//[(if vag)  ;
		if( CoC.player.hasVagina() ) {
			MainView.outputText( '  Another tube, likewise capped with a thick, firm end instead of a suction cup, reaches your ' + Descriptors.vaginaDescript( 0 ) + ' and pushes inside, secreting more of the fluid.  ', false );
			CoC.player.cuntChange( 15, true );
			//[(bigclit);
			if( CoC.player.clitLength > 3 ) {
				MainView.outputText( 'A smaller suction tube fastens over your ' + Descriptors.clitDescript() + ' as it rouses from the vaginal stimulation.', false );
			}
		}
		//[(lactation, no nipplecunts);
		if( CoC.player.biggestLactation() >= 1 && !CoC.player.hasFuckableNipples() ) {
			MainView.outputText( '  Some tubes of the cup-type latch onto your ' + Descriptors.nippleDescript( 0 ) + 's, then begin suction of their own. You feel your breastmilk shift toward the nipples, then begin to dribble into the tubes.', false );
		}
		//[(nipplecunts);
		if( CoC.player.hasFuckableNipples() ) {
			MainView.outputText( '  More tubes snake up to your ' + Appearance.biggestBreastSizeDescript() + ' and work their way into your ' + Descriptors.nippleDescript( 0 ) + 's.', false );
		}
		MainView.outputText( '  ' + Descriptors.SMultiCockDesc() + ' becomes enormously hard again in a matter of seconds as you absorb the secretions.  You quickly deduce what it is you\'re tasting; the demon-designed tubing is feeding your own semen back into you, laced with some kind of chemical!\n\n', false );
		MainView.outputText( 'Under the effects of this new drug, ' + Descriptors.sMultiCockDesc() + ' inflates, engorging with blood until it\'s much larger than usual', false );
		//[(balls);
		if( CoC.player.balls > 0 ) {
			MainView.outputText( ' and your ' + Descriptors.sackDescript() + ' stretches as your ' + Descriptors.ballsDescriptLight() + ' nearly double in size', false );
		}
		MainView.outputText( '.  Another orgasm passes, this time unhindered as the machine does not slow its efforts in the slightest.  More than twice the previous quantity of semen shoots out of your cock', false );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 's', false );
		}
		MainView.outputText( ', thanks assuredly to the drug', false );
		//[(balls);
		if( CoC.player.balls > 0 ) {
			MainView.outputText( ' and the new volume of your monstrous testicles', false );
		}
		//[(big skeet);
		if( cumQ > 1000 ) {
			MainView.outputText( ', spraying out of the overflow valve and coating your entire lower body', false );
		}
		MainView.outputText( '.', false );
		//[(if vag);
		if( CoC.player.hasVagina() ) {
			MainView.outputText( '  Your cunt spasms around its invader, milking the pseudo-cock and drawing the tainted semen it emits even deeper into your womb', false );
			//[(big clit);
			if( CoC.player.clitLength > 3 ) {
				MainView.outputText( ', and your clitoris twitches under the cup, threatening to black you out from the pleasure', false );
			}
			MainView.outputText( '.', false );
		}
		//[(lactation) ;
		if( CoC.player.biggestTitSize() > 1 ) {
			MainView.outputText( '  Your milk sprays out of your ' + Descriptors.nippleDescript( 0 ) + 's', false );
			//[(nipplecunts);
			if( CoC.player.hasFuckableNipples() ) {
				MainView.outputText( ' and rings raise from the bases of the shafts inside your nipples, forming a seal,', false );
			}
			MainView.outputText( ' as suction begins to whisk it away.', false );
		}
		MainView.outputText( '  A minute after your release, the flow in the other direction intensifies, pouring more fluid into you than before', false );
		//[(lactation);
		if( CoC.player.biggestLactation() > 1 ) {
			MainView.outputText( ', the drug now tasting of the milk mixed in', false );
		}
		MainView.outputText( '. Another chorus sounds from the counter and you hear some gems tinkling as they roll off the plate and onto the ground.\n\n', false );
		MainView.outputText( 'For several hours this continues, ' + Descriptors.sMultiCockDesc() + ' swelling to nearly twice its previous size and your orgasms coming one after another without a break as you pump your hips, knowing or caring about nothing except getting off on your machine lover and being fed in turn.  Your load each time is increased rather than decreased, with the cups growing multiple fleshy overflow valves and working overtime to vent the excess pressure every time you ejaculate.  ', false );
		//[(corruption > 90, high demonscore);
		if( CoC.player.cor > 90 ) {
			MainView.outputText( 'After literally hundreds of orgasms, the machine once again begins carefully regulating its suction, denying your release.  Repeatedly it teases your ' + Descriptors.multiCockDescriptLight(), false );
			//[(vag);
			if( CoC.player.hasVagina() ) {
				MainView.outputText( ' and ' + Descriptors.vaginaDescript( 0 ), false );
			}
			MainView.outputText( ' to the edge of orgasm and then backs off.  All the pumping in the world can\'t seem to get you over it, and your frustration mounts as the minutes roll by.  A sharper pressure than usual forms at the base of your ' + Descriptors.multiCockDescriptLight() + ' as you fruitlessly grind into the tubes, wishing more than anything to climax; the green light on the wall steadfastedly refuses to light up, mocking your attempts.  From the new pressure an odd strength flows through the rest of your body, increasing your sexual fervor and giving you a second wind, and you finally manage to get a good grip on the tubes, nearly slamming them back and forth on your shaft', false );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 's', false );
			}
			MainView.outputText( '.  This damn demon machine can\'t stop you from getting off anymore; with a violent shove you bury ' + Descriptors.sMultiCockDesc() + ' into the mouth', false );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 's', false );
			}
		...
		MainView.outputText( 'It is at this point Whitney walks in.\n\n', false );
		MainView.outputText( '"<i>What the FUCK!</i>" the rancher says, her eyes bugging out at the spectacle of you tied to the machine with demonic tubing and the sheer volume of semen you\'ve left in thick ropes all over her barn.', false );
		//[(corruption < 90)  ;
		if( CoC.player.cor < 90 ) {
			MainView.outputText( '  You barely take notice of her as another explosive orgasm goes off, your enormously swollen penis', false );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 'es', false );
			}
			MainView.outputText( ' pushing massive loads of semen down the tubes and out of the release valves in a firework pattern. Whitney shrinks back as some of the spooge squirts across the barn and comes to a stop less than a foot from her. The canine woman screams volubly enough to wake the entire farm. "<i>Dammit, ' + CoC.player.short + '; just... DAMMIT!</i>" You spare her not even a thought as you anticipate your next injection of the tainted chemicals and work up another batch of jizz.', false );
			EngineCore.dynStats( 'lib', 20, 'sen', 10, 'lus', 80, 'cor', 20 );
		}
		//[(corruption > 90, high demonscore);
		else {
			MainView.outputText( '  Still filled with the residual effects of your sudden burst of energy, you pull off the tubes one by one and, taking a moment to snatch up the pink crystal, saunter over to her. Her eyes widen at the simultaneously horrifying and comical sight of you approaching with lust in your eyes and ', false );
			//[(onecox);
			if( CoC.player.cockTotal() === 1 ) {
				MainView.outputText( 'a gigantic, erect penis bobbing in front of you and throwing off your balance', false );
			}//(multi);
			else {
				MainView.outputText( 'multiple gigantic, erect penises bobbing in front of you and dragging between your legs, head pinned against the floor', false );
			}
			MainView.outputText( '.  Just in time she backs up and pulls a pitchfork down from the wall. "<i>I expect I was wrong about you when we met, ' + CoC.player.short + '.  Get on out of here now and never come back or I\'ll make sure you never go anywhere again.</i>"  Scowling, you break off your advance and head toward the door.  Though you definitely want to fuck her, giving yourself a chance to adjust to your magnificent new body might not be a bad idea either.  After all, no matter what she says you can always come back... meanwhile you vow to find something or someone to rape or turn into your personal cock-milker.  With an amazing ', false );
			if( CoC.player.totalCocks() === 1 ) {
				MainView.outputText( 'dick', false );
			} else {
				MainView.outputText( 'set of dicks', false );
			}
			MainView.outputText( ' like yours, why worry about anything else for the moment?', false );
	...
	Farm.prototype.centaurToysHoooooo = function() {
		...
		MainView.outputText( '"<i>Oh, hey there, ' + CoC.player.short + ',</i>" Whitney says, leaning heavily on her pitchfork.  "<i>Not much, just trying to figure out... Hey, now!</i>" she says, eying up your powerful centaur frame.  ', false );
		if( CoC.player.cor < 50 ) {
			MainView.outputText( 'You shift awkwardly and ask her what\'s wrong.', false );
		} else {
			MainView.outputText( 'You strut a bit, showing yourself off in a subtly lewd manner.  When you\'re finished, you ask the dog-girl if she likes what she saw.', false );
		}
		MainView.outputText( '\n\n', false );
		MainView.outputText( 'She clears her throat awkwardly.  "<i>Uh, well... A pair of centaurs that lived here the last couple of years ran off, looks like.  Gone to get married or something, by the sound of things.  So, hey, wanna help me solve a little problem I\'m having?</i>"\n\n', false );
		MainView.outputText( 'You shrug. Sure.\n\n', false );
		MainView.outputText( '"<i>See, the two of them left some rather personal belongings behind,</i>" Whitney says, swinging the barn door open.  Inside, lying in the middle of a stall are what looks like a tall, slender totem with a giant rubber horsecock sticking out of it and a fake mare standing on reinforced wooden legs with a glistening, horsey onahole between its thighs.  Oh, my.  "<i>So, ' + CoC.player.short + ', since you look to have the right build for \'em... they\'re yours.</i>"\n\n', false );
		MainView.outputText( 'You tell her sure, and spend the next few minutes loading them onto the back of your horse-body.  Even if you don\'t end up using them yourself, you\'ve got plenty of room in camp for them, unlike Whitney.  Loaded up with centaur-friendly sex toys, you make your way back to camp.\n\n', false );
		MainView.outputText( '(<b>Key Items Gained: Fake Mare and Centaur Pole</b>)', false );
