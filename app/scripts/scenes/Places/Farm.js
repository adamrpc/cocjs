...
angular.module( 'cocjs' ).run( function( $log, Descriptors, EventParser, ConsumableLib, PerkLib, AppearanceDefs, Appearance, CockTypesEnum, StatusAffects, Utils, CoC, kFLAGS, EngineCore ) {
	...
	Farm.prototype.farmExploreEncounter = function() {
		...
		if( CoC.getInstance().player.statusAffectv1( StatusAffects.MetWhitney ) < 2 ) {
			EngineCore.spriteSelect( 62 );
			if( CoC.getInstance().player.findStatusAffect( StatusAffects.MetWhitney ) < 0 ) {
				CoC.getInstance().player.createStatusAffect( StatusAffects.MetWhitney, 0, 0, 0, 0 );
				...
				CoC.getInstance().player.addStatusValue( StatusAffects.MetWhitney, 1, 1 );
				if( CoC.getInstance().player.statusAffectv1( StatusAffects.MetWhitney ) === 2 ) {
		...
		//Repeat Offender;
			...
			if( CoC.getInstance().flags[ kFLAGS.KELT_BREAK_LEVEL ] >= 4 && CoC.getInstance().flags[ kFLAGS.WHITNEY_FLIPPED_OUT_OVER_KELLY ] === 0 ) {
				...
				CoC.getInstance().flags[ kFLAGS.WHITNEY_FLIPPED_OUT_OVER_KELLY ] = 1;
			} else if( CoC.getInstance().flags[ kFLAGS.WHITNEY_FLIPPED_OUT_OVER_KELLY ] === 1 ) {
			...
			if( CoC.getInstance().player.hasKeyItem( 'Breast Milker - Installed At Whitney\'s Farm' ) >= 0 ) {
				...
				} else if( CoC.getInstance().flags[ kFLAGS.WHITNEY_FLIPPED_OUT_OVER_KELLY ] === 0 ) {
	...
	Farm.prototype.whitneyMilkerHookup = function( breast ) {
		...
		if( breast === undefined || breast ) {
			...
			CoC.getInstance().player.removeKeyItem( 'Breast Milker' );
		} else {
	...
	Farm.prototype.talkWhitney = function() {
		...
		//Centaur Hookups!;
		if( CoC.getInstance().player.hasKeyItem( 'Fake Mare' ) < 0 && CoC.getInstance().player.isTaur() ) {
			this.centaurToysHoooooo();
			return;
		}
		// Requires: PC has met both Marble and Kelt;
		if( CoC.getInstance().flags[ kFLAGS.MURBLE_FARM_TALK_LEVELS ] > 0 && CoC.getInstance().player.findStatusAffect( StatusAffects.Kelt ) >= 0 && CoC.getInstance().flags[ kFLAGS.WHITNEY_TALK_MURBLE_AND_KELT ] === 0 ) {
			CoC.getInstance().flags[ kFLAGS.WHITNEY_TALK_MURBLE_AND_KELT ] = 1;
		...
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.TelAdre ) >= 0 && CoC.getInstance().player.statusAffectv1( StatusAffects.TelAdre ) >= 1 && CoC.getInstance().flags[ kFLAGS.WHITNEY_TALK_TELADRE ] === 0 && CoC.getInstance().flags[ kFLAGS.WHITNEY_TALK_MURBLE_AND_KELT ] === 1 ) {
			CoC.getInstance().flags[ kFLAGS.WHITNEY_TALK_TELADRE ] = 1;
		...
		if( CoC.getInstance().flags[ kFLAGS.DISCOVERED_HIGH_MOUNTAIN ] > 0 && CoC.getInstance().flags[ kFLAGS.WHITNEY_TALK_HIGH_MOUNTAIN ] === 0 && CoC.getInstance().flags[ kFLAGS.WHITNEY_TALK_TELADRE ] === 1 ) {
			CoC.getInstance().flags[ kFLAGS.WHITNEY_TALK_HIGH_MOUNTAIN ] = 1;
			...
			if( CoC.getInstance().player.findStatusAffect( StatusAffects.BreastsMilked ) >= 0 ) {
			...
			if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00331 ] > 0 && CoC.getInstance().flags[ kFLAGS.ISABELLA_MILKED_YET ] !== -1 ) {
				// Jojo only;
				EngineCore.outputText( ' At the far end there is a small alabaster cluster labelled ‘Jojo’.' );
			} else if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00331 ] === 0 && CoC.getInstance().flags[ kFLAGS.ISABELLA_MILKED_YET ] === -1 ) {
				// Isabella Only;
			...
			if( CoC.getInstance().player.findStatusAffect( StatusAffects.CockPumped ) >= 0 ) {
				if( CoC.getInstance().player.findStatusAffect( StatusAffects.BreastsMilked ) < 0 ) {
					EngineCore.outputText( '\n\n' );
				}
		...
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.DungeonShutDown ) >= 0 && CoC.getInstance().flags[ kFLAGS.WHITNEY_TALK_DUNGEON ] === 0 && CoC.getInstance().flags[ kFLAGS.WHITNEY_TALK_HIGH_MOUNTAIN ] === 1 ) {
			CoC.getInstance().flags[ kFLAGS.WHITNEY_TALK_DUNGEON ] = 1;
		...
		// Subsequent visit;
		if( CoC.getInstance().flags[ kFLAGS.WHITNEY_TALK_DUNGEON_FOLLOWUP ] === 0 && CoC.getInstance().flags[ kFLAGS.WHITNEY_TALK_DUNGEON ] === 1 ) {
			CoC.getInstance().flags[ kFLAGS.WHITNEY_TALK_DUNGEON_FOLLOWUP ] = 1;
		...
		if( Utils.rand( 4 ) === 0 && CoC.getInstance().player.hasKeyItem( 'Breast Milker - Installed At Whitney\'s Farm' ) < 0 ) {
			if( CoC.getInstance().player.hasKeyItem( 'Breast Milker' ) >= 0 ) {
				...
				EngineCore.outputText( '"<i>Ya might wanna get that looked at darlin\',</i>" she says, gesturing at milky wetness dripping from the front of your ' + CoC.getInstance().player.armorName + '.\n\n', false );
				...
				if( CoC.getInstance().player.gems >= 250 ) {
		...
		if( Utils.rand( 4 ) === 0 && CoC.getInstance().player.hasKeyItem( 'Cock Milker - Installed At Whitney\'s Farm' ) < 0 && CoC.getInstance().player.hasKeyItem( 'Cock Milker' ) >= 0 ) {
			EngineCore.outputText( 'Before you can say much of anything, Whitney exclaims, "<i>My stars! Is that one of them demon\'s milking machines?</i>"\n\n', false );
	...
	Farm.prototype.workFarm = function() {
		...
		if( Utils.rand( 4 ) === 0 ) {
			...
			EngineCore.dynStats( 'lus', CoC.getInstance().player.cowScore() + CoC.getInstance().player.minoScore() );
	...
	Farm.prototype.exploreFarm = function() {
		...
		if( explore === 0 ) {
			...
			if( CoC.getInstance().player.spe < 30 ) {
			...
			else if( CoC.getInstance().player.spe < 50 ) {
			...
			else if( CoC.getInstance().player.spe < 70 ) {
		...
		if( explore === 2 ) {
			...
			else if( CoC.getInstance().player.inHeat ) {
	...
	Farm.prototype.getMilked = function() {
		...
		//Repeat;
			...
			if( CoC.getInstance().player.findStatusAffect( StatusAffects.LactationReduction ) >= 0 && CoC.getInstance().player.totalCocks() === 0 ) {
			...
			//If both;
			if( CoC.getInstance().player.findStatusAffect( StatusAffects.LactationReduction ) >= 0 && CoC.getInstance().player.cockTotal() > 0 ) {
		...
		if( CoC.getInstance().player.nippleLength === 3 && Utils.rand( 2 ) === 0 ) {
			application = 3;
		}
		//Apply;
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.BreastsMilked ) < 0 || application === 0 ) {
			if( CoC.getInstance().player.findStatusAffect( StatusAffects.BreastsMilked ) < 0 ) {
				CoC.getInstance().player.createStatusAffect( StatusAffects.BreastsMilked, 0, 0, 0, 0 );
			}
			...
			if( CoC.getInstance().player.nippleLength <= 1.5 ) {
			...
			EngineCore.outputText( 'There is a mechanical lurching noise as the suction builds rapidly.  Your nipple swells out to ' + Math.ceil( CoC.getInstance().player.nippleLength * 1.5 * 10 ) / 10 + ' inches of length, turning purplish from the strain.   You can feel something welling up inside your ' + Descriptors.allBreastsDescript() + ', building as it moves towards your ' + Descriptors.nippleDescript( 0 ) + 's.\n\n', false );
		}
		...
		if( CoC.getInstance().player.lactationQ() < 50 ) {
			...
			if( milksplosion === 0 ) {
				...
				} else if( CoC.getInstance().player.totalBreasts() === 4 ) {
		...
		//Medium (+30 lust);
		else if( CoC.getInstance().player.lactationQ() < 250 ) {
		...
		//High Output (+ 40 lust);
		else if( CoC.getInstance().player.lactationQ() < 750 ) {
		...
		//CRAZY OUTPUT1 (+60 lust);
			...
			if( milksplosion === 0 ) {
				...
				} else if( CoC.getInstance().player.cockTotal() ) {
		...
		liters = Math.ceil( CoC.getInstance().player.lactationQ() * (Utils.rand( 10 ) + 90) / 100 ) / 1000;
		if( liters < 0 ) {
			liters = 1337;
		}
		...
		//If already at cap;
		if( CoC.getInstance().flags[ kFLAGS.WHITNEY_GEMS_PAID_THIS_WEEK ] >= cap ) {
		...
		if( payout > 0 ) {
			...
			if( payout + CoC.getInstance().flags[ kFLAGS.WHITNEY_GEMS_PAID_THIS_WEEK ] > cap ) {
		...
		if( CoC.getInstance().player.statusAffectv1( StatusAffects.LactationEndurance ) < 1.5 ) {
	...
	Farm.prototype.cockPumping = function() {
		...
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.CockPumped ) < 0 ) {
			...
			CoC.getInstance().player.createStatusAffect( StatusAffects.CockPumped, 1, 0, 0, 0 );
		}
		//REPEAT;
		else {
			...
			CoC.getInstance().player.addStatusValue( StatusAffects.CockPumped, 1, 1 );
		}
		...
		//BAD END!?;
		if( CoC.getInstance().player.cumQ() >= 50 && CoC.getInstance().player.fatigue >= 100 && CoC.getInstance().flags[ kFLAGS.USED_MILKER_TODAY ] > 0 ) {
		...
		//Light Scene – output less than 100 mLs;
			...
			if( CoC.getInstance().player.cockTotal() === 1 ) {
				EngineCore.outputText( 'The orgasm rolls over you, shutting down your thoughts as your body spasms in its straps, boiling out thick spurts of spunk.  Wriggling cilia convulse around you, licking your ' + CoC.getInstance().player.cockHead() + ' as it flares wide, filling with blood and pouring out cum.  You moan and scream with delight, babbling happily as you watch your cum wick up the clear tube and into the machinery in the wall.  All too soon the pleasure comes to an end, and your cock starts to soften inside its squishy prison.  The harness slowly loosens, lowering you to the ground and releasing you.\n\n', false );
			} else {
				EngineCore.outputText( 'The orgasm rolls over you, shutting down your thoughts as your body spasms in its straps, boiling out thick spurts of spunk.  Wriggling cilia convulse around you, licking your ' + CoC.getInstance().player.cockHead() + 's as they flare wide, filling with blood and pouring out cum.  You moan and scream with delight, babbling happily as you watch your cum wick up the clear tube and into the machinery in the wall.  All too soon the pleasure comes to an end, and your cocks start to soften inside their squishy prisons.  The harness slowly loosens, lowering you to the ground and releasing you.\n\n', false );
			}
		...
		if( CoC.getInstance().player.cumQ() < 20 ) {
		...
		else if( CoC.getInstance().flags[ kFLAGS.WHITNEY_GEMS_PAID_THIS_WEEK ] + payout >= cap ) {
			payout = cap - CoC.getInstance().flags[ kFLAGS.WHITNEY_GEMS_PAID_THIS_WEEK ];
		...
		if( payout > 0 ) {
			if( CoC.getInstance().player.cumQ() < 1000 ) {
				CoC.getInstance().player.modCumMultiplier( 1 );
			}
			if( payout === 1 ) {
		...
		if( CoC.getInstance().player.countCockSocks( 'gilded' ) > 0 ) {
			var gems = Utils.rand( 2 ) + CoC.getInstance().player.countCockSocks( 'gilded' );
			if( CoC.getInstance().player.cumQ() > 1000 ) {
				gems *= 1.5;
			}
			if( payout > 0 ) {
	...
	Farm.prototype.milkerBadEnd1 = function() {
		...
		var tubes = false;
		if( CoC.getInstance().player.hasVagina() || CoC.getInstance().player.hasFuckableNipples() || CoC.getInstance().player.biggestLactation() > 1 ) {
			EngineCore.outputText( 's', false );
			tubes = true;
		}
		...
		if( CoC.getInstance().player.cor < 90 ) {
			...
			EngineCore.outputText( ' pushing massive loads of semen down the tubes and out of the release valves in a firework pattern. Whitney shrinks back as some of the spooge squirts across the barn and comes to a stop less than a foot from her. The canine woman screams volubly enough to wake the entire farm. "<i>Dammit, ' + CoC.getInstance().player.short + '; just... DAMMIT!</i>" You spare her not even a thought as you anticipate your next injection of the tainted chemicals and work up another batch of jizz.', false );
	...
	Farm.prototype.centaurToysHoooooo = function() {
		...
		EngineCore.outputText( '"<i>Oh, hey there, ' + CoC.getInstance().player.short + ',</i>" Whitney says, leaning heavily on her pitchfork.  "<i>Not much, just trying to figure out... Hey, now!</i>" she says, eying up your powerful centaur frame.  ', false );
		if( CoC.getInstance().player.cor < 50 ) {
		...
		EngineCore.outputText( '"<i>See, the two of them left some rather personal belongings behind,</i>" Whitney says, swinging the barn door open.  Inside, lying in the middle of a stall are what looks like a tall, slender totem with a giant rubber horsecock sticking out of it and a fake mare standing on reinforced wooden legs with a glistening, horsey onahole between its thighs.  Oh, my.  "<i>So, ' + CoC.getInstance().player.short + ', since you look to have the right build for \'em... they\'re yours.</i>"\n\n', false );
