...
angular.module( 'cocjs' ).run( function( Combat, LustyDemons, WeaponLib, PregnancyStore, Farmers, Descriptors, EventParser, ConsumableLib, PerkLib, AppearanceDefs, Appearance, StatusAffects, Utils, CoC, kFLAGS, EngineCore ) {
	...
	Owca.prototype.gangbangVillageFirstGoRound = function() {
		EngineCore.clearOutput();
		if( CoC.getInstance().flags[ kFLAGS.DECLINED_TO_VISIT_REBECCS_VILLAGE ] === 0 && CoC.getInstance().flags[ kFLAGS.TIMES_REFUSED_REBECCS_OFFER ] === 0 && CoC.getInstance().flags[ kFLAGS.TIMES_IN_DEMON_PIT ] === 0 ) {
			//[Attitude is set to 50];
			CoC.getInstance().flags[ kFLAGS.OWCAS_ATTITUDE ] = 50;
			CoC.getInstance().flags[ kFLAGS.VAPULA_SUBMISSIVENESS ] = 50;
			EngineCore.outputText( 'You wander for quite a while in an endless field of grass before spotting a commotion in the distance.  A myriad of creatures have gathered in the path ahead, waving to you enthusiastically, some still holding tightly to their farm tools.  It\'s quite an odd sight, all these happy farmhands extending a warm welcome.  You simply accept it as a gesture of kindness and make your way toward the crowd of smiling faces.' );
		...
		else if( CoC.getInstance().flags[ kFLAGS.TIMES_IN_DEMON_PIT ] === 0 ) {
	...
	Owca.prototype.agreeToFollowRebecFirstTime = function() {
		EngineCore.clearOutput();
		if( CoC.getInstance().flags[ kFLAGS.TIMES_REFUSED_REBECCS_OFFER ] === 0 ) {
			...
			if( CoC.getInstance().player.gender === 0 ) {
				EngineCore.outputText( 'regretfully bare ' );
			}
			EngineCore.outputText( 'groin as you feel the contact of her tender curves against your body.  Taking advantage of the effect you seem to have on her, you start questioning her about her village, her people and how she was affected by demons.' );
			...
			//[if silly mode on];
			if( EngineCore.silly() ) {
				EngineCore.outputText( 'I-I don\'t think there\'s a single grown man or woman in our village that still has their anal virginity.' );
			}//[if silly mode off];
			else {
				EngineCore.outputText( 'Th-they abused us.  They mistreated us.  They would gather in circles and... use us in every possible way.' );
			}
			...
			if( CoC.getInstance().player.cor < 20 ) {
				...
				//[[if silly mode on];
				if( EngineCore.silly() ) {
					EngineCore.outputText( 'she\'ll never have to worry about her asshole again.' );
				}//[if silly mode off] ;
				else {
					EngineCore.outputText( 'she will never be abused again.' );
				}
				EngineCore.outputText( '  If there\'s anything that can be done to hold off these evil creatures, you will do it.' );
			}
			//[else if corr and libido both > 60];
			else if( CoC.getInstance().player.cor > 60 && CoC.getInstance().player.lib > 60 && CoC.getInstance().player.hasCock() ) {
				...
				EngineCore.outputText( 'You are uncontrollably turned on by this revelation; your genitals actually hurt at your absurd level of arousal.  Just imagining groups of demons gathered around the helpless villagers, sometimes taking turns with each one and sometimes taking someone all at once, turns you on way more than it should.  You stupidly grin for a few seconds as your mind pictures vivid and lecherous scenes of collective ass-rape, but then you remember this is supposed to be bad news; you quickly recompose yourself and put on a false expression of worry.  Luckily she didn\'t notice the lust that temporarily filled your eyes; neither does she spot the massive tent growing in your ' + CoC.getInstance().player.armorName + '.  You ask her if there\'s anything you can do to meet these demons and \'handle\' them on your own.' );
			...
			if( CoC.getInstance().player.cor < 50 && CoC.getInstance().player.inte >= 50 ) {
				...
				if( CoC.getInstance().player.lib > 50 ) {
					EngineCore.outputText( '... and considering what Rebecc meant by \'gratitude\'' );
				}
				EngineCore.outputText( '.  Maybe it wasn\'t all an act?' );
			}
			//[if corr < 50 and int < 50];
		...
		//Subsequent pleas upon first refusal (Z);
			...
			EngineCore.outputText( '"<i>So, have you changed your mind yet?  Will you help us?  The demons will want another sacrifice soon, and we are afraid of what might happen if we don\'t comply!  Please, do accept this time!</i>"' );
			EngineCore.outputText( '\n\nYou know very well what these people want: some meat to appease the demons.  Will you help them?' );
		}
		//Yes/No;
		if( CoC.getInstance().flags[ kFLAGS.TIMES_IN_DEMON_PIT ] === 0 ) {
	...
	Owca.prototype.declineRebeccsPlea = function() {
		...
		//[Attitude is set to 50];
		if( CoC.getInstance().flags[ kFLAGS.OWCAS_ATTITUDE ] > 5 ) {
	...
	Owca.prototype.acceptRebeccsPlea = function() {
		EngineCore.clearOutput();
		CoC.getInstance().flags[ kFLAGS.OWCAS_ATTITUDE ] += 10;
		if( CoC.getInstance().flags[ kFLAGS.OWCAS_ATTITUDE ] > 100 ) {
			CoC.getInstance().flags[ kFLAGS.OWCAS_ATTITUDE ] = 100;
		}
		EngineCore.outputText( 'You tell the desperate girl that you\'re going to do as she asks.  As soon as you finish speaking, everyone cheers and applauds you, praising you as their savior.  Rebecc grabs hold of your shoulders and gives you a soft kiss on your lips; she whispers, "<i>Thank you, champion.  We all owe you.  I promise that you will be rewarded for the heroic deed you\'re about to undertake.</i>"' );
		...
		if( CoC.getInstance().player.cor > 70 && CoC.getInstance().player.gender > 0 ) {
			...
			EngineCore.outputText( ' already slickening with sexual juices; the sight of these restraints brings all kinds of perverted thoughts to your mind: dozens of ways to be used and deliciously abused like a sex-toy by wretched and well-endowed beings.' );
		}
		//[else];
		else {
	...
	Owca.prototype.intoTheDemonPit = function( sacrifice ) {
		...
		if( sacrifice === undefined || sacrifice ) {
			sacrifice = true;
			EngineCore.outputText( ', tangled in your restraints' );
		}
		EngineCore.outputText( '.  She leans down and coos, ' );
		//[if first time];
		if( CoC.getInstance().flags[ kFLAGS.TIMES_IN_DEMON_PIT ] === 0 ) {
			EngineCore.outputText( '"<i>Awww, look at the poor little thing!  So this is what they gave us this time... Look at [him], everyone!  Do you know you\'d be a cute pet?  People here call me Vapula, and I don\'t think you\'ll forget that name any time soon.</i>"' );
		}
		//[else if Vapula Submissiveness >50];
		else if( CoC.getInstance().flags[ kFLAGS.VAPULA_SUBMISSIVENESS ] > 50 ) {
			EngineCore.outputText( '"<i>You again?  By Lethice, you must really enjoy it!  You love being a bitch for Vapula, don\'t you?  Yes you do!</i>"' );
		}
		//[else];
		else {
			EngineCore.outputText( '"<i>Fuck, you again?  Tell me it\'s so!  I\'ve been looking forward to putting you in your place as my newest slut!</i>"' );
		}
		//[(if sacrificed);
		if( sacrifice ) {
			//[if str >= 80];
			...
			} else if( CoC.getInstance().player.spe >= 80 ) {
			...
			EngineCore.outputText( '  You\'re once again feeling regret over letting the villagers hold your equipment, but there\'s nothing for it now.' );
		}
		CoC.getInstance().flags[ kFLAGS.TIMES_IN_DEMON_PIT ]++;
		CoC.getInstance().flags[ kFLAGS.DAYS_SINCE_LAST_DEMON_DEALINGS ] = 0;
		if( sacrifice ) {
	...
	Owca.prototype.fightZeDemons = function( sacrifice ) {
		...
		if( sacrifice === undefined || sacrifice ) {
			...
			CoC.getInstance().monster.createStatusAffect( StatusAffects.BowDisabled, 0, 0, 0, 0 );
			if( CoC.getInstance().player.str < 80 && CoC.getInstance().player.spe < 80 ) {
				CoC.getInstance().monster.createStatusAffect( StatusAffects.AttackDisabled, 0, 0, 0, 0 );
				CoC.getInstance().monster.createStatusAffect( StatusAffects.RunDisabled, 0, 0, 0, 0 );
				CoC.getInstance().monster.createStatusAffect( StatusAffects.PhysicalDisabled, 0, 0, 0, 0 );
			} else {
	...
	Owca.prototype.loseOrSubmitToVapula = function() {
		...
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( ', before finally wrapping around your ' + Descriptors.cockDescript( 0 ) + ' and stroking it; the friction uncontrollably arouses you, and you find yourself reaching full erectness' );
		}
		...
		EngineCore.outputText( '  The double penetration is brutal, unexpected and painful.  Your insides are protesting vigorously against this rough treatment, even though you feel a tingle of pleasure gently tickling your colon at every thrust.  ' );
		//[(no Buttslut);
		if( CoC.getInstance().player.findPerk( PerkLib.Buttslut ) < 0 ) {
			EngineCore.outputText( 'No!  You aren\'t supposed to enjoy it...  ' );
		}
		EngineCore.outputText( 'You try to cry out but as soon as your mouth opens it is filled with another dick, then a second one.  A third tries to push its way between the first two, stretching your cheeks and making you drool.  Seeing that the monstrous dong won\'t fit in your already double-stuffed mouth, its owner groans in frustration and proceeds to slap your cheek with it.  He is soon joined by other demons who find the idea very entertaining.' );
		...
		if( CoC.getInstance().player.hasVagina() ) {
			...
			if( CoC.getInstance().player.statusAffectv1( StatusAffects.BonusVCapacity ) < 200 ) {
				CoC.getInstance().player.addStatusValue( StatusAffects.BonusVCapacity, 1, 15 );
			}
		...
		if( CoC.getInstance().player.hasCock() ) {
			...
			if( CoC.getInstance().player.cockTotal() > 1 ) {
				EngineCore.outputText( '  She is soon joined by other horny girls, too eager to wait for a piece of your crotch to play with.  They all comfortably position themselves, positioning their pussy or anus atop each of your cocks before riding you like there\'s no tomorrow.' );
			}
			//[if 4 to 7 cocks];
			if( CoC.getInstance().player.cockTotal() >= 4 ) {
				EngineCore.outputText( '  Nevertheless, they aren\'t satisfied with a single insertion and most grab hold of one of your extra cocks before forcefully stuffing it in their other hole, howling and thrashing all the more as they are penetrated a second time.' );
			}
			//[if 7 cocks or more];
			if( CoC.getInstance().player.cockTotal() >= 7 ) {
				EngineCore.outputText( '  Sadly, surrounded as you are, no one will be able to find extra room to ride your remaining cocks; fortunately, you feel warm, long demonic tongues being pressed against your ' + Descriptors.cockDescript( 6 ) );
				if( CoC.getInstance().player.cockTotal() > 8 ) {
					EngineCore.outputText( ' as well as your remaining pricks' );
				}
				EngineCore.outputText( '.  The tongues expertly lick your meat and wrap themselves around it, and questing lips give it multiple kisses before one pair suddenly loses patience and deepthroats you with voracity; the tightness of these lips makes them feel like a very efficient cock-ring.  However, it is hard to focus on the sweet warmth provided by these avid mouths when all your other cocks are being clamped by powerful, inhuman vaginal muscles.' );
			}
			EngineCore.outputText( '\n\nBetween every thrust, you feel hands working and polishing your shaft' );
			if( CoC.getInstance().player.cockTotal() > 1 ) {
				EngineCore.outputText( 's' );
			}
			EngineCore.outputText( ', as if every inch had to be taken care of at every instant.  As you slide back into the pussy, the hands return to crawling over your body, caressing your groin and staining your belly with pre-cum - yours and others.' );
		...
		//[if cocks];
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( '  ' + Descriptors.SMultiCockDesc() + ' pulsates and throbs' );
			//[[if balls];
			if( CoC.getInstance().player.balls > 0 ) {
				EngineCore.outputText( ', and your balls swell and boil' );
			}
			EngineCore.outputText( '; a gigantic spooge-flow pushes its way up your urethra and spurts outside in the waiting love-tunnels' );
			if( CoC.getInstance().player.cockTotal() > 7 ) {
				EngineCore.outputText( ' and mouths' );
			}
			EngineCore.outputText( '.  The violent cumshot almost instantly produces a shiver of orgasm once again and vaginal walls contract furiously, determined to absorb as much of your baby-batter as possible.' );
			//[if cum production is massive];
			if( CoC.getInstance().player.cumQ() > 1500 ) {
				EngineCore.outputText( '  The effort appears to be vain, as the absurdly high volume you\'ve ejected is enough to completely pack every hole; the remaining jism spills on the tainted ground, soaking it further.' );
			}
		}
		...
		if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( ' and filling your womb to its maximum capacity' );
		}
		EngineCore.outputText( '.  The remaining cum spurts outside and dribbles onto your [legs].  Your asshole clenches, squeezing the dongs inside you and milking their very last drops of spunk.' );
		//[if vagina];
		if( CoC.getInstance().player.hasVagina() ) {
		...
		CoC.getInstance().flags[ kFLAGS.VAPULA_SUBMISSIVENESS ] += 10;
		if( CoC.getInstance().flags[ kFLAGS.VAPULA_SUBMISSIVENESS ] >= 90 && CoC.getInstance().flags[ kFLAGS.VAPULA_SUBMISSIVENESS ] < 100 ) {
		...
		if( CoC.getInstance().flags[ kFLAGS.VAPULA_SUBMISSIVENESS ] >= 100 ) {
			EngineCore.doNext( this.slaveToVapulaBadEnd );
		} else if( CoC.getInstance().player.findStatusAffect( StatusAffects.LostVillagerSpecial ) >= 0 ) {
		...
		CoC.getInstance().flags[ kFLAGS.REBECCS_LAST_PLEA ] = 0;
	};
	...
	Owca.prototype.defeetVapulasHorde = function() {
		EngineCore.clearOutput();
		CoC.getInstance().flags[ kFLAGS.REBECCS_LAST_PLEA ] = 0;
		if( CoC.getInstance().flags[ kFLAGS.VAPULA_SUBMISSIVENESS ] <= 0 ) {
			this.subdueVapula();
			return;
		}
		...
		if( CoC.getInstance().monster.findStatusAffect( StatusAffects.AttackDisabled ) >= 0 ) {
			if( CoC.getInstance().monster.HP < 1 ) {
				EngineCore.outputText( 'You grin wickedly as the last demons fall, defeated.  Some of the errant blows have broken and mangled the links of your chains, and you find you can get free with a bit of additional effort.' );
			}//[if won by Lust];
			else {
				EngineCore.outputText( 'You grin wickedly as the demons give up the fight, too turned on to care about you.  One even begins unfastening your bindings, hopeful desperation glinting in her eyes as she attempts to entice you with her long, thick nipples and enormous, dripping gash.' );
			}
		}
		//Not;
		else {
			if( CoC.getInstance().monster.HP < 1 ) {
				EngineCore.outputText( 'You grin wickedly as the last demons fall, defeated.' );
			}
			//[if won by Lust];
			else {
				EngineCore.outputText( 'You grin wickedly as the demons give up the fight, too turned on to care about you.  One even has hopeful desperation glinting in her eyes as she attempts to entice you with her long, thick nipples and enormous, dripping gash.' );
			}
		}
		//[(requires genitals and and corr >60);
		if( CoC.getInstance().player.cor > 60 && CoC.getInstance().player.gender > 0 ) {
			...
			CoC.getInstance().flags[ kFLAGS.VAPULA_SUBMISSIVENESS ] -= 5;
			Combat.cleanupAfterCombat();
		}
	};
	...
	Owca.prototype.rapeZeVapula = function() {
		...
		if( CoC.getInstance().player.hasCock() ) {
			...
			//[if vagina];
			if( CoC.getInstance().player.hasVagina() ) {
				EngineCore.outputText( 'You stick a finger in your own ' + Descriptors.vaginaDescript() + ' in order to lubricate it.  ' );
			}
			EngineCore.outputText( 'The defeated horde watches you touch yourself with avid, almost desperate eyes.  You keep teasing yourself, saving your lust for the torrid storm of rape you\'re about to unleash on the poor creatures who dared attack you.  You walk among the battered bodies, looking for the prey that will best suit you; then you find her.  Vapula is lying here, her purple skin masking ' );
			if( CoC.getInstance().monster.HP < 1 ) {
				EngineCore.outputText( 'the many bruises she\'s received' );
			} else {
				EngineCore.outputText( 'the constant flushing due to intense arousal' );
			}
			EngineCore.outputText( '.  You harshly grab her hand and grunt, "<i>On your feet, cunt.</i>"  She is too stunned to protest and weakly stands up, her leather straps torn to completely reveal her voluptuous body.  You feel the sudden urge to take her right now and fuck her like a ' );
			if( CoC.getInstance().player.hasVagina() ) {
				EngineCore.outputText( 'bitch in heat' );
			} else {
				EngineCore.outputText( 'centaur in rut, abusing and filling her teasing cunt and ass with every protuberance you have' );
			}
			EngineCore.outputText( '.  However, you have better plans for her.' );
			...
			EngineCore.outputText( '\n\n"<i>What are you waiting for?</i>" she moans.  "<i>Fuck me already, you dirty bastard.  Go ahead, stuff me.  Why don\'t you get down to business?  Are you afraid that you might last only a minute?  What are you, a poor little sissy with no stamina?  You innocent little ' + CoC.getInstance().player.mf( 'manlet', 'maiden' ) + ', I will–</i>"' );
			...
			if( CoC.getInstance().player.cockTotal() > 1 ) {
				EngineCore.outputText( '  Seeing that she has extra room left, you grab another of your dongs and put it at the entrance of her dripping cunt before forcing it in.' );
			}
			...
			EngineCore.outputText( ' in front of the once-powerful dominatrix, grunting and panting as you accelerate the pace, until you finally reach your climax and release rope after rope of jism in your fuck-toy, completely packing her backdoor' );
			if( CoC.getInstance().player.cockTotal() > 1 ) {
				EngineCore.outputText( ' and her love-tunnel' );
			}
			EngineCore.outputText( '.' );
			//[if massive cum production];
			if( CoC.getInstance().player.cumQ() > 1000 ) {
				EngineCore.outputText( '  Some of it spurts by little jets outside of her, even as she instinctively contracts her muscles to keep as much spooge as possible and her belly accomodates by bloating out absurdly.' );
			}
			EngineCore.outputText( '  Needless to say, watching this rough ' );
			if( CoC.getInstance().player.cockTotal() === 1 ) {
				EngineCore.outputText( 'anal' );
			} else {
				EngineCore.outputText( 'double-penetration' );
			}
			EngineCore.outputText( ' session has brought Vapula to a new level of arousal; she is struggling to free her arms and finger herself, but her tight restraints only allow her to wriggle uncomfortably.  Her pussy is gushing of its own and she whimpers from time to time, unable to control her lust.' );
			EngineCore.outputText( '\n\nYou throw away your expendable' );
			if( CoC.getInstance().player.cumQ() > 1000 ) {
				EngineCore.outputText( ', expandable' );
			}
			EngineCore.outputText( ' cumdumpster and grab another slut.  This time you feel like you could use a blowjob to clean off the demonic sweat and slime from your cock; you forcefully open her jaw and shove your ' + Descriptors.cockDescript( 0 ) + ' down her throat without any care.  The sudden influx of dickflesh blocking her air-flow makes her choke and cough, but a withdrawal and a good cock-slap reminds her of her place.  "<i>You fucking bitch!  You goddamn worthless slut!  What good are you if you can\'t even give proper head?  Lick, you whore!  I want your tongue all over my crotch!  You can breathe when you\'ve properly moistened my ' );
			if( CoC.getInstance().player.balls > 0 ) {
				EngineCore.outputText( 'balls' );
			} else {
				EngineCore.outputText( CoC.getInstance().player.skin() );
			}
			EngineCore.outputText( '!</i>"  The poor demoness, cheek stained with semen, mutters an apology and proceeds to lick your ' + Descriptors.cockDescript( 0 ) + ', suckling the tip, ' );
			if( CoC.getInstance().player.balls > 0 ) {
				EngineCore.outputText( 'fondling your ' + Descriptors.ballsDescriptLight() + ', ' );
			}
			//[if multicocks] ;
			if( CoC.getInstance().player.cockTotal() > 1 ) {
				EngineCore.outputText( 'alternatively deepthroating or jerking every cock of yours, ' );
			}
			EngineCore.outputText( 'warming up your dickflesh with her demonic tongue.  You enjoy her ministrations while staring Vapula in the eyes; she has trouble looking back at you as her body trembles and her eyes widen at your sheer display of depravity.  Your semen-dribbling shaft' );
			if( CoC.getInstance().player.cockTotal() > 1 ) {
				EngineCore.outputText( 's harden and thicken ' );
			} else {
				EngineCore.outputText( ' hardens and thickens ' );
			}
			EngineCore.outputText( 'again as the demoness sucks you off; Vapula can\'t help but take a quick glance at it, but you pretend not to notice.  After all, she\'s only teasing herself for you.' );
			...
			EngineCore.outputText( ' and aim at Vapula, splattering her body with your spunk.  Words cannot describe the sheer triumph you feel.  You burst into laughter as you drench that unworthy bitch with your seed.' );
			...
			if( CoC.getInstance().player.cockTotal() > 1 ) {
				EngineCore.outputText( 'In an unholy sex-frenzy, you want more; you start grabbing creatures by groups and alternatively pump in and out of their asses with your ' + Descriptors.cockDescript( 0 ) + ', always hungry for more anal.' );
			}
			...
			if( EngineCore.silly() ) {
				EngineCore.outputText( '  As you look back, you snicker, "<i>Ass for ass, assholes.</i>"' );
			}
		...
		//Victory rape using vagina/anus [Take Dicks in All Holes](Z);
			...
			if( CoC.getInstance().player.vaginalCapacity() > 80 ) {
				EngineCore.outputText( 'His cock is lost in your vast cunt and you work him as hard as you can to feel him against your walls, his body convulsing and dazed eyes rolling against the wet, savage rape.  ' );
			}
			EngineCore.outputText( 'You keep thrusting back and forth, treating the worthless horned boy like a disposable dildo.  As you pump him, you look at Vapula, warning her, "<i>Don\'t turn your head.  If you ever look away or close your eyes you will regret it.</i>" Overwhelmed by your dominance and subdued by the defeat, Vapula can only nod.' );
			...
			//[if squirter] ;
			if( CoC.getInstance().player.wetness() >= 5 ) {
				EngineCore.outputText( 'The sound of spattering fluid fills the air as your cunt spurts its juices around the incubus\'s cock, coating him in your fragrant slime.  ' );
			}
			EngineCore.outputText( 'Against your relentless milking pressure he stands no chance and he moans raggedly as he cums in tandem, still incapable of moving his body as your mixed juices dribble down him.' );
			...
			if( CoC.getInstance().player.biggestTitSize() >= 2 ) {
				EngineCore.outputText( ' and your ' + Descriptors.breastDescript( 0 ) + ' bounce gently' );
			}
			EngineCore.outputText( ' as you grind your snatch into the succubus\'s face; Vapula can\'t help but take a quick glance at your oozing, slippery cunt, but you pretend not to notice.  After all, she\'s only teasing herself for you.' );
			...
			if( CoC.getInstance().player.hasFuckableNipples() ) {
				EngineCore.outputText( '  Your freakish nipples moisten with excitement to the smell of man juice, and you feel your libido ratcheting up even higher.  Bending forward, you grab two other imps with smaller dicks, quickly guiding their lengths into your nipplecunts; they are small enough that you can mash their bodies against your soft flesh, doing all the work for them as your sensitive nubs are penetrated.' );
			}
			...
			EngineCore.outputText( '\n\nWhen you walk up to Vapula, she is barely recognizable.  Her will is utterly broken and she seems considerably mollified.  Your repeated denials made her mind collapse; she\'s but an empty shell driven crazy by her burning unsatisfied lust.  With a content smirk, you lean down and give her a tender kiss, enjoying the taste of secondhand sexual juices coating her lips.  You whisper to her, "<i>Good night, beauty.</i>"  She doesn\'t seem to hear your taunt.' );
			EngineCore.outputText( '\n\nA few meters away, outside the pit, you notice your items and your ' + CoC.getInstance().player.armorName + '. The village appears to be deserted. Considering these demons will have a hard time threatening anyone with white goo oozing out of their absurdly milked urethras, you let them rest, wallowing in their pool of mixed spunk.  You pick up your belongings and head back to camp.' );
	...
	Owca.prototype.owcaMainScreenOn = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.OWCAS_ATTITUDE ] <= 5 && CoC.getInstance().flags[ kFLAGS.OWCA_ANGER_DISABLED ] === 0 && CoC.getInstance().flags[ kFLAGS.OWCA_SACRIFICE_DISABLED ] === 0 ) {
			this.desperateVillages();
			return;
		}
		//Desperate plea;
		if( Utils.rand( 10 ) <= 2 && CoC.getInstance().flags[ kFLAGS.DAYS_SINCE_LAST_DEMON_DEALINGS ] >= 7 && CoC.getInstance().flags[ kFLAGS.OWCA_SACRIFICE_DISABLED ] === 0 ) {
		...
		//[if attitude > 80];
		if( CoC.getInstance().flags[ kFLAGS.OWCAS_ATTITUDE ] > 80 ) {
			EngineCore.outputText( 'Villagers greet you as you arrive, praising your courage and your heroism.  People shake your hands or ask you to kiss them, as if they hoped your nobility were somehow contagious.  Once they\'re done complimenting and flattering you in every way possible, they leave you to your business.' );
		}
		//[if 60<=attitude<80];
		else if( CoC.getInstance().flags[ kFLAGS.OWCAS_ATTITUDE ] > 60 ) {
			EngineCore.outputText( 'Villagers greet you with a friendly smile as you arrive.  Some approach you and thank you for what you\'ve done, before leaving awkwardly.  They\'re obviously not used to strangers.' );
		}
		//[if 40<=attitude<60];
		else if( CoC.getInstance().flags[ kFLAGS.OWCAS_ATTITUDE ] > 40 ) {
			EngineCore.outputText( 'Villagers seem to accept your presence without comment now; though none go out of their way to greet you, neither do they avoid doing so when you pass by.' );
		}
		//[if 20<attitude <= 40];
		else if( CoC.getInstance().flags[ kFLAGS.OWCAS_ATTITUDE ] > 20 ) {
			EngineCore.outputText( 'Villagers keep looking at you uncomfortably; no one seems to be willing to talk to you apart from Rebecc, your friend.' );
		}
		//[if attitude <= 20];
		else {
			EngineCore.outputText( 'Villagers here appear to be openly hostile.  You keep hearing muffled whispers as you pass by, and people look at you rancorously.  They don\'t bother you, though.' );
		}
		//end variable display;
		EngineCore.outputText( '  The village is rather small.  A dozen humble thatched cottages are standing here and there, linked by dirt tracks and enclosed with small palisades.  There seems to be very little activity.' );
		...
		if( CoC.getInstance().flags[ kFLAGS.DAYS_SINCE_LAST_DEMON_DEALINGS ] === 7 && CoC.getInstance().flags[ kFLAGS.OWCA_SACRIFICE_DISABLED ] === 0 ) {
			EngineCore.outputText( '  A week' );
		} else if( CoC.getInstance().flags[ kFLAGS.DAYS_SINCE_LAST_DEMON_DEALINGS ] > 7 && CoC.getInstance().flags[ kFLAGS.OWCA_SACRIFICE_DISABLED ] === 0 ) {
			EngineCore.outputText( '  More than a week' );
		}
		if( CoC.getInstance().flags[ kFLAGS.DAYS_SINCE_LAST_DEMON_DEALINGS ] >= 7 && CoC.getInstance().flags[ kFLAGS.OWCA_SACRIFICE_DISABLED ] === 0 ) {
			EngineCore.outputText( ' has passed since the last offering to the demons; guarding the pit would certainly help improve your relations with the little town\'s denizens.' );
		}
		//Option: ;
		var pit = null;
		var herd = null;
		var tavern = null;
		if( CoC.getInstance().time.hours >= 16 && CoC.getInstance().flags[ kFLAGS.OWCA_SACRIFICE_DISABLED ] === 0 ) {
		...
		if( CoC.getInstance().flags[ kFLAGS.OWCAS_ATTITUDE ] >= 50 ) {
		...
		if( CoC.getInstance().flags[ kFLAGS.OWCAS_ATTITUDE ] >= 10 ) {
	...
	Owca.prototype.owcaTavern = function() {
		...
		EngineCore.outputText( '\nSheep Milk Bottle: ' + (180 - CoC.getInstance().flags[ kFLAGS.OWCAS_ATTITUDE ]) + ' gems' );
		if( (180 - CoC.getInstance().flags[ kFLAGS.OWCAS_ATTITUDE ]) > CoC.getInstance().player.gems ) {
			milk = null;
		}
		var goblin = EngineCore.createCallBackFunction( this.owcaBuySetup, ConsumableLib.GOB_ALE );
		EngineCore.outputText( '\nGoblin Ale: ' + (60 - Math.round( CoC.getInstance().flags[ kFLAGS.OWCAS_ATTITUDE ] / 2 )) + ' gems' );
		if( (60 - Math.round( CoC.getInstance().flags[ kFLAGS.OWCAS_ATTITUDE ] / 2 )) > CoC.getInstance().player.gems ) {
			goblin = null;
		}
		var brew = EngineCore.createCallBackFunction( this.owcaBuySetup, ConsumableLib.BROBREW );
		if( Utils.rand( 10 ) > CoC.getInstance().flags[ kFLAGS.OWCAS_ATTITUDE ] / 10 ) {
			EngineCore.outputText( '\nBro Brew: 2000 gems' );
			if( (2000) > CoC.getInstance().player.gems ) {
				brew = null;
			}
		} else {
			brew = null;
		...
		EngineCore.outputText( '\nMinotaur Cum: ' + (300 - CoC.getInstance().flags[ kFLAGS.OWCAS_ATTITUDE ]) + ' gems' );
		if( (300 - CoC.getInstance().flags[ kFLAGS.OWCAS_ATTITUDE ]) > CoC.getInstance().player.gems ) {
			cum = null;
		}
		EngineCore.outputText( '</i>' );
	...
	Owca.prototype.owcaBuySetup = function( item ) {
		if( item === ConsumableLib.SHEEPMK ) {
			this.buyOwcaShit( item, (180 - CoC.getInstance().flags[ kFLAGS.OWCAS_ATTITUDE ]) );
		} else if( item === ConsumableLib.GOB_ALE ) {
			this.buyOwcaShit( item, (60 - Math.round( CoC.getInstance().flags[ kFLAGS.OWCAS_ATTITUDE ] / 2 )) );
		} else if( item === ConsumableLib.BROBREW ) {
			this.buyOwcaShit( item, 2000 );
		} else {
			this.buyOwcaShit( item, (300 - CoC.getInstance().flags[ kFLAGS.OWCAS_ATTITUDE ]) );
		}
	};
	...
	Owca.prototype.rebeccMenu = function() {
		...
		EngineCore.outputText( '\n\n"<i>' + CoC.getInstance().player.short + '!  You came!  I was just preparing a hot bath.  My poor darling, you must have gone through a really perilous ordeal.  Here, do you want to get cleaned up?</i>"' );
	...
	Owca.prototype.rebeccBathScene = function() {
		...
		if( CoC.getInstance().player.gender === 0 ) {
			...
			//[if cock];
			if( CoC.getInstance().player.hasCock() ) {
				EngineCore.outputText( '; your ' + Descriptors.cockDescript( 0 ) + ' grows to full erect size' );
			}
			if( CoC.getInstance().player.hasVagina() && CoC.getInstance().player.hasCock() ) {
				EngineCore.outputText( ' and ' );
			...
			if( CoC.getInstance().player.hasVagina() ) {
				EngineCore.outputText( 'your ' + Descriptors.vaginaDescript() + ' leaks a little trickle of girl-juice' );
			}
			EngineCore.outputText( '.  She doesn\'t seem to notice the effect she has on you - or so you think.  Suddenly, you feel a hand darting between your legs.' );
		}
	...
	Owca.prototype.rapeRebecc = function( outside ) {
		...
		if( CoC.getInstance().player.hasCock() ) {
			...
			if( CoC.getInstance().player.cocks[ 0 ].cockLength > 12 ) {
				EngineCore.outputText( '  You keep burying more of your ' + Descriptors.cockDescript( 0 ) + ' deep inside her rectum until you reach her limit and can\'t fit more cock in.' );
			} else {
				EngineCore.outputText( '  Eventually the entirety of your prick is lodged inside her lush asshole.' );
			}
			EngineCore.outputText( '  She trills and bleats in pain and flounders about in your arms, futilely trying to avoid the incoming butt-rape.  You lock her arms with yours, implacably kneading her tits, and her shivering flesh brings the most exquisite contact to your fingers; you toy with her nipples as well, making her scream savagely in forced pleasure.' );
			...
			//[if balls];
			if( CoC.getInstance().player.balls > 0 ) {
				EngineCore.outputText( ', and your ' + Descriptors.ballsDescriptLight() + ' almost feel sore from colliding with her dribbling cunt' );
			}
			EngineCore.outputText( '.  Finally, when you think you\'re about to pass out from exhaustion, you cum in a powerful jet, literally blasting Rebecc\'s ass with your spooge, globs of  goo being propelled like projectiles into her colon and splattering her backside with huge white smears.' );
			if( CoC.getInstance().player.cumQ() > 1500 ) {
				EngineCore.outputText( '  After completely filling up her rectum, your baby-batter erupts in thick spurts from her anus despite her instinctive ass-clenching efforts to hold as much semen as possible inside.  The sheer might of your cumshot is enough to send Rebecc over the edge, and she releases an absolutely torrential stream of girl-cum, squirting against her will all over your groins and mixing her own fluids with yours in a pathetic show of sluttiness.' );
			}
			EngineCore.outputText( '  She unconsciously milks you of all your spunk, her anal muscles working on their own to massage your meat and extract every drop from your urethra.' );
			...
			if( outside ) {
				EngineCore.outputText( 'Some of the villagers are apathetically watching you, too stunned and bruised to intervene.  ' );
			}
			EngineCore.outputText( 'You silently remove your prick from your girl\'s ass, watch her contentedly as her exposed rectum is literally oozing with spooge and walk back to your camp.' );
			EngineCore.outputText( '\n\nThis is a more-than-appropriate goodbye gift' );
			if( outside ) {
				EngineCore.outputText( ' to these miserable people, and even moreso' );
			}
			EngineCore.outputText( ' to the manipulative slut who kept teasing you.  Serves her right, to use her body in the most provocative ways and never let you actually have your way with her.  You\'re unlikely to ever see her again.' );
		...
		//Rebecc Female Rape Scene (Z);
			...
			EngineCore.outputText( 'You silently pick yourself up and dress, admiring the quivering, crying handiwork at your feet, before walking back to your camp.  This is a more-than-appropriate goodbye gift' );
			if( outside ) {
				EngineCore.outputText( ' to these miserable people, and even moreso' );
			}
			EngineCore.outputText( ' to the manipulative slut who kept teasing you.  Serves her right, to use her body in the most provocative ways and never let you actually have your way with her.  You\'re unlikely to ever see her again.' );
	...
	Owca.prototype.beatUpOwca = function() {
		...
		var torch = null;
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.KnowsWhitefire ) >= 0 || CoC.getInstance().player.findPerk( PerkLib.FireLord ) >= 0 || CoC.getInstance().player.findPerk( PerkLib.Hellfire ) >= 0 ) {
			torch = this.torchOwcaMotherFuckers;
		}
		var rape = null;
		if( CoC.getInstance().player.cor >= 60 && CoC.getInstance().player.gender > 0 && CoC.getInstance().player.lust >= 33 ) {
	...
	Owca.prototype.leaveOwcaAfterWhupping = function() {
		...
		CoC.getInstance().flags[ kFLAGS.REBECCS_LAST_PLEA ] = 1;
		Combat.cleanupAfterCombat();
	};
	...
	Owca.prototype.torchUpVillagersAndLeave = function() {
		...
		CoC.getInstance().player.gems += 900 + Utils.rand( 150 );
	...
	Owca.prototype.loseToOwca = function() {
		...
		CoC.getInstance().player.createStatusAffect( StatusAffects.LostVillagerSpecial, 0, 0, 0, 0 );
	};
	//Post-Mob Encounter (Z);
	Owca.prototype.morningAfterRape = function() {
		CoC.getInstance().player.removeStatusAffect( StatusAffects.LostVillagerSpecial );
		EngineCore.clearOutput();
	...
	Owca.prototype.forgiveOwca = function() {
		...
		EngineCore.outputText( ' that you don\'t feel any resentment at all; she tried her best to help you, and you\'re grateful to her.' );
		...
		CoC.getInstance().flags[ kFLAGS.OWCAS_ATTITUDE ] = 60;
		CoC.getInstance().flags[ kFLAGS.OWCA_ANGER_DISABLED ] = 1;
		//To main owca menu;
		if( CoC.getInstance().isInCombat() ) {
	...
	Owca.prototype.faceDownHordes = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'You tell Rebecc to go hide on the plains; you\'re going to handle the villagers as well as the demon horde on your own.  She cries, "<i>But ' + CoC.getInstance().player.short + '!  You stand no chance against this ruthless horde!  Besides, the villagers won\'t trust you.  They\'ll think you want to cheat them... they\'ll tie you up to prevent you from getting away!</i>"' );
	...
	Owca.prototype.slaveToVapulaBadEnd = function() {
		...
		EngineCore.outputText( '\n\n"<i>Enough.  Remove that dick from my pet\'s mouth, I want to talk to ' + CoC.getInstance().player.mf( 'him', 'her' ) + ' for a second.</i>"' );
	...
	Owca.prototype.subdueVapula = function() {
		...
		if( CoC.getInstance().monster.HP < 1 ) {
			...
			EngineCore.outputText( 'rendered insensate by his own lust' );
		}
		EngineCore.outputText( '.  As usual, you walk among the forlorn bodies of your enemies.  Vapula is groveling on the floor again, looking up to you with imploring eyes.  Even beaten and broken, the luscious succubus remains the most preeminent figure of the motley rabble.' );
		...
		var enslave = null;
		if( CoC.getInstance().player.gender > 0 && CoC.getInstance().player.cor >= 66 ) {
	...
	Owca.prototype.disbandHorde = function() {
		...
		//Attitude set to 100;
		CoC.getInstance().flags[ kFLAGS.OWCAS_ATTITUDE ] = 100;
		CoC.getInstance().flags[ kFLAGS.OWCA_SACRIFICE_DISABLED ] = 1;
		Combat.cleanupAfterCombat();
	};
	...
	Owca.prototype.enslaveVapulaWithYourWang = function() {
		...
		if( CoC.getInstance().player.lust < 80 ) {
			CoC.getInstance().player.lust = 80;
		}
	...
	Owca.prototype.enslaveVapulaAsACuntWielder = function() {
		...
		CoC.getInstance().flags[ kFLAGS.VAPULA_FOLLOWER ] = 0.5;
