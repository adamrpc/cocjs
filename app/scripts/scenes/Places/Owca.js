...
angular.module( 'cocjs' ).run( function( MainView, SceneLib, LustyDemons, WeaponLib, PregnancyStore, Farmers, Descriptors, ConsumableLib, PerkLib, AppearanceDefs, StatusAffects, Utils, CoC, kFLAGS, EngineCore ) {
	...
	Owca.prototype.gangbangVillageFirstGoRound = function() {
		...
		//As long as you don't follow her once, display this on subsequent encounters (Z);
			...
			MainView.outputText( 'After wandering across the plains for a while, you spot Rebecc, the girl you met before, amongst a group of fellow farmers; she calls out to you.  "<i>Hello again, traveler!  You seem lost... would you mind spending some time with us at our village?</i>"' );
	...
	Owca.prototype.agreeToFollowRebecFirstTime = function() {
		...
		if( CoC.flags[ kFLAGS.TIMES_REFUSED_REBECCS_OFFER ] === 0 ) {
			...
			//[if silly mode on];
			if( EngineCore.silly() ) {
				MainView.outputText( 'I-I don\'t think there\'s a single grown man or woman in our village that still has their anal virginity.' );
			}//[if silly mode off];
			else {
				MainView.outputText( 'Th-they abused us.  They mistreated us.  They would gather in circles and... use us in every possible way.' );
			}
			...
			if( CoC.player.cor < 20 ) {
				...
				//[[if silly mode on];
				if( EngineCore.silly() ) {
					MainView.outputText( 'she\'ll never have to worry about her asshole again.' );
				}//[if silly mode off] ;
				else {
					MainView.outputText( 'she will never be abused again.' );
				}
				MainView.outputText( '  If there\'s anything that can be done to hold off these evil creatures, you will do it.' );
			}
			//[else if corr and libido both > 60];
			else if( CoC.player.cor > 60 && CoC.player.lib > 60 && CoC.player.hasCock() ) {
				...
				MainView.outputText( 'You are uncontrollably turned on by this revelation; your genitals actually hurt at your absurd level of arousal.  Just imagining groups of demons gathered around the helpless villagers, sometimes taking turns with each one and sometimes taking someone all at once, turns you on way more than it should.  You stupidly grin for a few seconds as your mind pictures vivid and lecherous scenes of collective ass-rape, but then you remember this is supposed to be bad news; you quickly recompose yourself and put on a false expression of worry.  Luckily she didn\'t notice the lust that temporarily filled your eyes; neither does she spot the massive tent growing in your ' + CoC.player.armorName + '.  You ask her if there\'s anything you can do to meet these demons and \'handle\' them on your own.' );
			...
			if( CoC.player.cor < 50 && CoC.player.inte >= 50 ) {
				...
				if( CoC.player.lib > 50 ) {
					MainView.outputText( '... and considering what Rebecc meant by \'gratitude\'' );
				}
				MainView.outputText( '.  Maybe it wasn\'t all an act?' );
			}
			//[if corr < 50 and int < 50];
		...
		//Subsequent pleas upon first refusal (Z);
			...
			MainView.outputText( '"<i>So, have you changed your mind yet?  Will you help us?  The demons will want another sacrifice soon, and we are afraid of what might happen if we don\'t comply!  Please, do accept this time!</i>"' );
	...
	Owca.prototype.acceptRebeccsPlea = function() {
		...
		MainView.outputText( 'You tell the desperate girl that you\'re going to do as she asks.  As soon as you finish speaking, everyone cheers and applauds you, praising you as their savior.  Rebecc grabs hold of your shoulders and gives you a soft kiss on your lips; she whispers, "<i>Thank you, champion.  We all owe you.  I promise that you will be rewarded for the heroic deed you\'re about to undertake.</i>"' );
		...
		if( CoC.player.cor > 70 && CoC.player.gender > 0 ) {
			...
			MainView.outputText( ' already slickening with sexual juices; the sight of these restraints brings all kinds of perverted thoughts to your mind: dozens of ways to be used and deliciously abused like a sex-toy by wretched and well-endowed beings.' );
		}
		//[else];
		else {
	...
	Owca.prototype.intoTheDemonPit = function( sacrifice ) {
		...
		if( CoC.time.hours < 21 ) {
			...
			MainView.outputText( '<b>' + Utils.Num2Text( passed ) + ' ' + (passed > 1 ? 'hours pass' : 'hour passes') + '...</b>\n' );
			CoC.time.hours = 21;
			MainView.statsView.show();
		}
		MainView.outputText( 'You are awakened by a sudden cackling, and open your eyes; you are almost entirely surrounded by darkness, and the dim light provided by the menacing red moon only makes the landscape bleaker.' );
		...
		if( sacrifice === undefined || sacrifice ) {
			sacrifice = true;
			MainView.outputText( ', tangled in your restraints' );
		}
		MainView.outputText( '.  She leans down and coos, ' );
		//[if first time];
		if( CoC.flags[ kFLAGS.TIMES_IN_DEMON_PIT ] === 0 ) {
			MainView.outputText( '"<i>Awww, look at the poor little thing!  So this is what they gave us this time... Look at [him], everyone!  Do you know you\'d be a cute pet?  People here call me Vapula, and I don\'t think you\'ll forget that name any time soon.</i>"' );
		}
		//[else if Vapula Submissiveness >50];
		else if( CoC.flags[ kFLAGS.VAPULA_SUBMISSIVENESS ] > 50 ) {
			MainView.outputText( '"<i>You again?  By Lethice, you must really enjoy it!  You love being a bitch for Vapula, don\'t you?  Yes you do!</i>"' );
		}
		//[else];
		else {
			MainView.outputText( '"<i>Fuck, you again?  Tell me it\'s so!  I\'ve been looking forward to putting you in your place as my newest slut!</i>"' );
		}
		//[(if sacrificed);
		if( sacrifice ) {
			//[if str >= 80];
			...
			MainView.outputText( '  You\'re once again feeling regret over letting the villagers hold your equipment, but there\'s nothing for it now.' );
		}
	...
	Owca.prototype.fightZeDemons = function( sacrifice ) {
		...
		if( sacrifice === undefined || sacrifice ) {
			...
			CoC.monster.createStatusAffect( StatusAffects.BowDisabled, 0, 0, 0, 0 );
			if( CoC.player.str < 80 && CoC.player.spe < 80 ) {
				CoC.monster.createStatusAffect( StatusAffects.AttackDisabled, 0, 0, 0, 0 );
				CoC.monster.createStatusAffect( StatusAffects.RunDisabled, 0, 0, 0, 0 );
				CoC.monster.createStatusAffect( StatusAffects.PhysicalDisabled, 0, 0, 0, 0 );
			} else {
	...
	Owca.prototype.loseOrSubmitToVapula = function() {
		...
		MainView.outputText( '\n\nSoon you find yourself completely overwhelmed by hot demon hands and dicks touching every part of your body.  In no time at all two imps bend you over and forcefully ram your ' + Descriptors.assholeDescript() + ' with their mammoth peckers.' );
		...
		if( CoC.player.hasCock() ) {
			MainView.outputText( '  It wraps around [eachCock] for a while, jerking it for a bit and rubbing its tip against yours.' );
		}
		//[if antennae];
		if( CoC.player.antennae > AppearanceDefs.ANTENNAE_NONE ) {
			MainView.outputText( '  Your antennae are being harshly pulled and twisted; some imps, in a crazy show of libertinism, start inserting your sensitive peduncles down their bloated urethrae. The intimate friction and the sudden jolts when the internal walls slather your appendages in slick, hot pre-cum are driving you mad with irregular shots of unbearable pleasure.' );
		}
		//[if horns];
		if( CoC.player.horns > 0 ) {
			MainView.outputText( '  You feel your horns being used as leverage to slap your head with even more rock-hard dick.  The impacts stun you until your whole forehead feels numb and coated with a mixture of ballsweat and pre-cum.' );
		}
		//[if vagina;
		if( CoC.player.hasVagina() ) {
		...
		//[if cocks];
		if( CoC.player.hasCock() ) {
			MainView.outputText( '\n\nA soft brush against your ' + Descriptors.cockDescript( 0 ) + ' warns you that your crotch is going to get some more love.  At least four hands are caressing it, helping you rise and thicken until it can\'t grow anymore.  After a while of playful stroking, the hands are suddenly gone, only to be replaced by a fluid-gushing cunt.  It impales itself on your ' + Descriptors.cockDescript( 0 ) + ' at full force, driving a very whorish and high-pitched cry from its owner.' );
			//[if more than 1 and less than 4 cocks];
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( '  She is soon joined by other horny girls, too eager to wait for a piece of your crotch to play with.  They all comfortably position themselves, positioning their pussy or anus atop each of your cocks before riding you like there\'s no tomorrow.' );
			}
			//[if 4 to 7 cocks];
			if( CoC.player.cockTotal() >= 4 ) {
				MainView.outputText( '  Nevertheless, they aren\'t satisfied with a single insertion and most grab hold of one of your extra cocks before forcefully stuffing it in their other hole, howling and thrashing all the more as they are penetrated a second time.' );
			}
			//[if 7 cocks or more];
			if( CoC.player.cockTotal() >= 7 ) {
				MainView.outputText( '  Sadly, surrounded as you are, no one will be able to find extra room to ride your remaining cocks; fortunately, you feel warm, long demonic tongues being pressed against your ' + Descriptors.cockDescript( 6 ) );
				if( CoC.player.cockTotal() > 8 ) {
					MainView.outputText( ' as well as your remaining pricks' );
				}
				MainView.outputText( '.  The tongues expertly lick your meat and wrap themselves around it, and questing lips give it multiple kisses before one pair suddenly loses patience and deepthroats you with voracity; the tightness of these lips makes them feel like a very efficient cock-ring.  However, it is hard to focus on the sweet warmth provided by these avid mouths when all your other cocks are being clamped by powerful, inhuman vaginal muscles.' );
			}
			MainView.outputText( '\n\nBetween every thrust, you feel hands working and polishing your shaft' );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ', as if every inch had to be taken care of at every instant.  As you slide back into the pussy, the hands return to crawling over your body, caressing your groin and staining your belly with pre-cum - yours and others.' );
		...
		//[if cocks];
		if( CoC.player.hasCock() ) {
			MainView.outputText( '  ' + Descriptors.SMultiCockDesc() + ' pulsates and throbs' );
			//[[if balls];
			if( CoC.player.balls > 0 ) {
				MainView.outputText( ', and your balls swell and boil' );
			}
			MainView.outputText( '; a gigantic spooge-flow pushes its way up your urethra and spurts outside in the waiting love-tunnels' );
			if( CoC.player.cockTotal() > 7 ) {
				MainView.outputText( ' and mouths' );
			}
			MainView.outputText( '.  The violent cumshot almost instantly produces a shiver of orgasm once again and vaginal walls contract furiously, determined to absorb as much of your baby-batter as possible.' );
			//[if cum production is massive];
			if( CoC.player.cumQ() > 1500 ) {
				MainView.outputText( '  The effort appears to be vain, as the absurdly high volume you\'ve ejected is enough to completely pack every hole; the remaining jism spills on the tainted ground, soaking it further.' );
			}
		}
		MainView.outputText( '\n\nAfter what seems like hours of forceful penetration, the gigantic members inside you release a milky torrent of corrupted seed, completely packing your colon through your ' + Descriptors.assholeDescript() );
		if( CoC.player.hasVagina() ) {
			MainView.outputText( ' and filling your womb to its maximum capacity' );
		}
		MainView.outputText( '.  The remaining cum spurts outside and dribbles onto your [legs].  Your asshole clenches, squeezing the dongs inside you and milking their very last drops of spunk.' );
		//[if vagina];
		if( CoC.player.hasVagina() ) {
			MainView.outputText( '\n\nYour own ' + Descriptors.vaginaDescript( 0 ) + ' clenches too, doing its best to waste as little semen as possible.  Your muscles quiver as they feel the incoming last release.  Then, in climax, you squirt your own girl-cum, splattering the demon dicks with your juice.' );
		}
		//[if lactating breasts];
		if( CoC.player.lactationQ() > 0 ) {
			MainView.outputText( '\n\nThe combined efforts of a dozen demons proves too much for your poor ' + Descriptors.chestDesc() + ' and your nipples let out a white creamy flood, which is immediately gobbled by thirsty imps.  ' );
		} else {
			MainView.outputText( '\n\n' );
		}
		MainView.outputText( 'Everywhere around you, people keep cumming and shooting ropes of jism.  In no time your entire body is drenched with spooge.  Your eyes and ears are completely covered with sticky goo, preventing you from hearing and seeing.  Still, you can feel penises being stuffed into your mouth again as they unload their unholy swill.  The taste is strong, hot, and salty, but not unpleasant.  You swallow until your belly swells, but you don\'t care; temporarily reduced to the instincts of breeding stock, you want more spunk and you suck it up as if your life depended on it.' );
		...
		if( CoC.flags[ kFLAGS.VAPULA_SUBMISSIVENESS ] >= 90 && CoC.flags[ kFLAGS.VAPULA_SUBMISSIVENESS ] < 100 ) {
	...
	Owca.prototype.wakeUpAfterDemonGangBangs = function() {
		...
		CoC.time.hours = 7;
		CoC.time.days++;
		MainView.outputText( 'When you wake up, you are alone, and your restraints are broken.  You are sloshing in a pool of stinky juices; your mouth and ears are still full of it.  Your whole body is covered with a thin white layer that must certainly be dried spooge.  Underneath, you\'re nothing but bruises and every movement seems to hurt.  A few meters away, outside the pit, you notice your items and your gear.  The village itself appears to be empty... your best assumption is that the residents are hiding, either from shame at having sacrificed you or from awkwardness at the prospect of talking to a sloshing, crusty cumdumpster.  Wearily, you head back to your camp.' );
	...
	Owca.prototype.defeetVapulasHorde = function() {
		...
		if( CoC.monster.findStatusAffect( StatusAffects.AttackDisabled ) ) {
			if( CoC.monster.HP < 1 ) {
				...
				MainView.outputText( 'You grin wickedly as the demons give up the fight, too turned on to care about you.  One even begins unfastening your bindings, hopeful desperation glinting in her eyes as she attempts to entice you with her long, thick nipples and enormous, dripping gash.' );
			}
		}
		//Not;
		else {
			if( CoC.monster.HP < 1 ) {
				MainView.outputText( 'You grin wickedly as the last demons fall, defeated.' );
			}
			//[if won by Lust];
			else {
				MainView.outputText( 'You grin wickedly as the demons give up the fight, too turned on to care about you.  One even has hopeful desperation glinting in her eyes as she attempts to entice you with her long, thick nipples and enormous, dripping gash.' );
			}
		}
	...
	Owca.prototype.rapeZeVapula = function() {
		CoC.flags[ kFLAGS.VAPULA_SUBMISSIVENESS ] -= 10;
		//Victory rape with penis [Anal Orgy and Bukkake] (NTR Vapula) (Z);
		if( CoC.player.hasCock() ) {
			MainView.clearOutput();
			MainView.outputText( 'The demon horde struggles before you.  With a disdainful smile, you fully expose your genitals for everyone to see.  ' );
			//[if cocks];
			if( CoC.player.hasCock() ) {
				MainView.outputText( '\n\nYou gently stroke your ' + Descriptors.cockDescript( 0 ) + ', bringing it to full erectness.  ' );
			}
			//[if vagina];
			if( CoC.player.hasVagina() ) {
				MainView.outputText( 'You stick a finger in your own ' + Descriptors.vaginaDescript() + ' in order to lubricate it.  ' );
			}
			MainView.outputText( 'The defeated horde watches you touch yourself with avid, almost desperate eyes.  You keep teasing yourself, saving your lust for the torrid storm of rape you\'re about to unleash on the poor creatures who dared attack you.  You walk among the battered bodies, looking for the prey that will best suit you; then you find her.  Vapula is lying here, her purple skin masking ' );
			if( CoC.monster.HP < 1 ) {
				MainView.outputText( 'the many bruises she\'s received' );
			} else {
				MainView.outputText( 'the constant flushing due to intense arousal' );
			}
			MainView.outputText( '.  You harshly grab her hand and grunt, "<i>On your feet, cunt.</i>"  She is too stunned to protest and weakly stands up, her leather straps torn to completely reveal her voluptuous body.  You feel the sudden urge to take her right now and fuck her like a ' );
			if( CoC.player.hasVagina() ) {
				MainView.outputText( 'bitch in heat' );
			} else {
				MainView.outputText( 'centaur in rut, abusing and filling her teasing cunt and ass with every protuberance you have' );
			}
			MainView.outputText( '.  However, you have better plans for her.' );
			...
			MainView.outputText( '\n\n"<i>What are you waiting for?</i>" she moans.  "<i>Fuck me already, you dirty bastard.  Go ahead, stuff me.  Why don\'t you get down to business?  Are you afraid that you might last only a minute?  What are you, a poor little sissy with no stamina?  You innocent little ' + CoC.player.mf( 'manlet', 'maiden' ) + ', I will–</i>"' );
			...
			MainView.outputText( '\n\nWith lazy nonchalance you grab hold of another demon crawling at your feet, another succubus; although not so tall or big-breasted as Vapula, her provocative curves are a delight to your eyes.  You bend her over and, without ceremony, start working her tight pucker, filling her insides with your ' + Descriptors.cockDescript( 0 ) + '.' );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( '  Seeing that she has extra room left, you grab another of your dongs and put it at the entrance of her dripping cunt before forcing it in.' );
			}
			...
			MainView.outputText( ' in front of the once-powerful dominatrix, grunting and panting as you accelerate the pace, until you finally reach your climax and release rope after rope of jism in your fuck-toy, completely packing her backdoor' );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( ' and her love-tunnel' );
			}
			MainView.outputText( '.' );
			//[if massive cum production];
			if( CoC.player.cumQ() > 1000 ) {
				MainView.outputText( '  Some of it spurts by little jets outside of her, even as she instinctively contracts her muscles to keep as much spooge as possible and her belly accomodates by bloating out absurdly.' );
			}
			MainView.outputText( '  Needless to say, watching this rough ' );
			if( CoC.player.cockTotal() === 1 ) {
				MainView.outputText( 'anal' );
			} else {
				MainView.outputText( 'double-penetration' );
			}
			MainView.outputText( ' session has brought Vapula to a new level of arousal; she is struggling to free her arms and finger herself, but her tight restraints only allow her to wriggle uncomfortably.  Her pussy is gushing of its own and she whimpers from time to time, unable to control her lust.' );
			MainView.outputText( '\n\nYou throw away your expendable' );
			if( CoC.player.cumQ() > 1000 ) {
				MainView.outputText( ', expandable' );
			}
			MainView.outputText( ' cumdumpster and grab another slut.  This time you feel like you could use a blowjob to clean off the demonic sweat and slime from your cock; you forcefully open her jaw and shove your ' + Descriptors.cockDescript( 0 ) + ' down her throat without any care.  The sudden influx of dickflesh blocking her air-flow makes her choke and cough, but a withdrawal and a good cock-slap reminds her of her place.  "<i>You fucking bitch!  You goddamn worthless slut!  What good are you if you can\'t even give proper head?  Lick, you whore!  I want your tongue all over my crotch!  You can breathe when you\'ve properly moistened my ' );
			if( CoC.player.balls > 0 ) {
				MainView.outputText( 'balls' );
			} else {
				MainView.outputText( CoC.player.skin() );
			}
			MainView.outputText( '!</i>"  The poor demoness, cheek stained with semen, mutters an apology and proceeds to lick your ' + Descriptors.cockDescript( 0 ) + ', suckling the tip, ' );
			if( CoC.player.balls > 0 ) {
				MainView.outputText( 'fondling your ' + Descriptors.ballsDescriptLight() + ', ' );
			}
			//[if multicocks] ;
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 'alternatively deepthroating or jerking every cock of yours, ' );
			}
			MainView.outputText( 'warming up your dickflesh with her demonic tongue.  You enjoy her ministrations while staring Vapula in the eyes; she has trouble looking back at you as her body trembles and her eyes widen at your sheer display of depravity.  Your semen-dribbling shaft' );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 's harden and thicken ' );
			} else {
				MainView.outputText( ' hardens and thickens ' );
			}
			MainView.outputText( 'again as the demoness sucks you off; Vapula can\'t help but take a quick glance at it, but you pretend not to notice.  After all, she\'s only teasing herself for you.' );
			...
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 'In an unholy sex-frenzy, you want more; you start grabbing creatures by groups and alternatively pump in and out of their asses with your ' + Descriptors.cockDescript( 0 ) + ', always hungry for more anal.' );
			}
			...
			if( EngineCore.silly() ) {
				MainView.outputText( '  As you look back, you snicker, "<i>Ass for ass, assholes.</i>"' );
			}
			//+med-high corruption, + strength/cons, -10 libido;
			CoC.player.orgasm();
			EngineCore.dynStats( 'str', 1, 'tou', 1, 'lib', -4, 'cor', 4 );
		}
		//Victory rape using vagina/anus [Take Dicks in All Holes](Z);
			...
			MainView.outputText( '\n\nWith lazy nonchalance you grab hold of another demon crawling at your feet, an incubus; shorter and younger than Vapula, his tight, pert body is a delight to your eyes.  Without ceremony you shove him to the ground and grind your ' + Descriptors.vaginaDescript() + ' into his crotch.  He looks at you, semi-comatose and bewildered, as you get a rise out of his dick, which hardens against your damp strokes.  You pin him down as you sink onto his red length, sighing as you rapidly engulf him, then begin to ride him ruthlessly.  ' );
			CoC.player.cuntChange( 40, true, true, false );
			if( CoC.player.vaginalCapacity() > 80 ) {
				MainView.outputText( 'His cock is lost in your vast cunt and you work him as hard as you can to feel him against your walls, his body convulsing and dazed eyes rolling against the wet, savage rape.  ' );
			}
			MainView.outputText( 'You keep thrusting back and forth, treating the worthless horned boy like a disposable dildo.  As you pump him, you look at Vapula, warning her, "<i>Don\'t turn your head.  If you ever look away or close your eyes you will regret it.</i>" Overwhelmed by your dominance and subdued by the defeat, Vapula can only nod.' );
			MainView.outputText( '\n\nYou keep working the incubus\'s hot cock in front of the once-powerful dominatrix, panting as you accelerate the pace until you finally reach your climax, clenching your thighs tightly to his fit body as your ' + Descriptors.vaginaDescript() + ' clenches and spasms.  ' );
			//[if squirter] ;
			if( CoC.player.wetness() >= 5 ) {
				MainView.outputText( 'The sound of spattering fluid fills the air as your cunt spurts its juices around the incubus\'s cock, coating him in your fragrant slime.  ' );
			}
			MainView.outputText( 'Against your relentless milking pressure he stands no chance and he moans raggedly as he cums in tandem, still incapable of moving his body as your mixed juices dribble down him.' );
			...
			MainView.outputText( '\n\nYou throw your expendable cock toy to one side and grab another slut, this time a succubus.  Your semen-dribbling cunt is now in need of a cleaning, and without any ado you sit yourself straight down onto her face, banging the back of her head against the ground.  She whines into your ' + Descriptors.vaginaDescript() + ', to which you respond by pulling her hair.  "<i>You fucking bitch!  You worthless slut!  What good are you if you can\'t even munch rug properly?  Lick, you whore!  I want your tongue to clean up what your useless fuckhead of a friend couldn\'t keep to himself.</i>"  The poor demoness mutters an apology and proceeds to lap up the juices drooling out of your hole, licking your labia and mons clean of what the incubus left, darting her wet muscle across your ' + Descriptors.clitDescript() + ' before reaching into your vagina, carefully teasing out and drinking every drop of jizz she can find, making your own juices run afresh.  You enjoy her ministrations while staring Vapula in the eyes; she has trouble looking back at you as her body trembles and her eyes widen at your sheer display of depravity.  Your ' + Descriptors.nippleDescript( 0 ) + 's harden' );
			if( CoC.player.biggestTitSize() >= 2 ) {
				MainView.outputText( ' and your ' + Descriptors.breastDescript( 0 ) + ' bounce gently' );
			}
			MainView.outputText( ' as you grind your snatch into the succubus\'s face; Vapula can\'t help but take a quick glance at your oozing, slippery cunt, but you pretend not to notice.  After all, she\'s only teasing herself for you.' );
	...
	Owca.prototype.owcaMainScreenOn = function() {
		...
		if( CoC.flags[ kFLAGS.OWCAS_ATTITUDE ] <= 5 && CoC.flags[ kFLAGS.OWCA_ANGER_DISABLED ] === 0 && CoC.flags[ kFLAGS.OWCA_SACRIFICE_DISABLED ] === 0 ) {
			this.desperateVillages();
			return;
		}
		//Desperate plea;
		if( Utils.rand( 10 ) <= 2 && CoC.flags[ kFLAGS.DAYS_SINCE_LAST_DEMON_DEALINGS ] >= 7 && CoC.flags[ kFLAGS.OWCA_SACRIFICE_DISABLED ] === 0 ) {
		...
		//[if attitude > 80];
		if( CoC.flags[ kFLAGS.OWCAS_ATTITUDE ] > 80 ) {
			MainView.outputText( 'Villagers greet you as you arrive, praising your courage and your heroism.  People shake your hands or ask you to kiss them, as if they hoped your nobility were somehow contagious.  Once they\'re done complimenting and flattering you in every way possible, they leave you to your business.' );
		}
		//[if 60<=attitude<80];
		else if( CoC.flags[ kFLAGS.OWCAS_ATTITUDE ] > 60 ) {
			MainView.outputText( 'Villagers greet you with a friendly smile as you arrive.  Some approach you and thank you for what you\'ve done, before leaving awkwardly.  They\'re obviously not used to strangers.' );
		}
		//[if 40<=attitude<60];
		else if( CoC.flags[ kFLAGS.OWCAS_ATTITUDE ] > 40 ) {
			MainView.outputText( 'Villagers seem to accept your presence without comment now; though none go out of their way to greet you, neither do they avoid doing so when you pass by.' );
		}
		//[if 20<attitude <= 40];
		else if( CoC.flags[ kFLAGS.OWCAS_ATTITUDE ] > 20 ) {
			MainView.outputText( 'Villagers keep looking at you uncomfortably; no one seems to be willing to talk to you apart from Rebecc, your friend.' );
		}
		//[if attitude <= 20];
		else {
			MainView.outputText( 'Villagers here appear to be openly hostile.  You keep hearing muffled whispers as you pass by, and people look at you rancorously.  They don\'t bother you, though.' );
		}
		//end variable display;
		MainView.outputText( '  The village is rather small.  A dozen humble thatched cottages are standing here and there, linked by dirt tracks and enclosed with small palisades.  There seems to be very little activity.' );
		...
		if( CoC.flags[ kFLAGS.DAYS_SINCE_LAST_DEMON_DEALINGS ] === 7 && CoC.flags[ kFLAGS.OWCA_SACRIFICE_DISABLED ] === 0 ) {
			MainView.outputText( '  A week' );
		} else if( CoC.flags[ kFLAGS.DAYS_SINCE_LAST_DEMON_DEALINGS ] > 7 && CoC.flags[ kFLAGS.OWCA_SACRIFICE_DISABLED ] === 0 ) {
			MainView.outputText( '  More than a week' );
		}
		if( CoC.flags[ kFLAGS.DAYS_SINCE_LAST_DEMON_DEALINGS ] >= 7 && CoC.flags[ kFLAGS.OWCA_SACRIFICE_DISABLED ] === 0 ) {
			MainView.outputText( ' has passed since the last offering to the demons; guarding the pit would certainly help improve your relations with the little town\'s denizens.' );
		}
		//Option: ;
		var pit = null;
		var herd = null;
		var tavern = null;
		if( CoC.time.hours >= 16 && CoC.flags[ kFLAGS.OWCA_SACRIFICE_DISABLED ] === 0 ) {
		...
		if( CoC.flags[ kFLAGS.OWCAS_ATTITUDE ] >= 50 ) {
		...
		if( CoC.flags[ kFLAGS.OWCAS_ATTITUDE ] >= 10 ) {
	...
	Owca.prototype.owcaTavern = function() {
		MainView.clearOutput();
		MainView.outputText( 'The tavern is nice and cozy; there are a few tables and chairs scattered around in no ordered pattern, and most clients here appear to belong to the same species.  By the crude wooden bar, you see a list of all the current drinks on sale:\n<i>' );
		//SheepMk;
	...
	Owca.prototype.owcaBuySetup = function( item ) {
		if( item === ConsumableLib.SHEEPMK ) {
			this.buyOwcaShit( item, (180 - CoC.flags[ kFLAGS.OWCAS_ATTITUDE ]) );
		} else if( item === ConsumableLib.GOB_ALE ) {
			this.buyOwcaShit( item, (60 - Math.round( CoC.flags[ kFLAGS.OWCAS_ATTITUDE ] / 2 )) );
		} else if( item === ConsumableLib.BROBREW ) {
			this.buyOwcaShit( item, 2000 );
		} else {
			this.buyOwcaShit( item, (300 - CoC.flags[ kFLAGS.OWCAS_ATTITUDE ]) );
		}
	};
	Owca.prototype.buyOwcaShit = function( bleh, price ) {
		...
		MainView.statsView.show();
		MainView.outputText( 'The bartender hands you a bottle and grabs your gems before attending other clients, leaving you to your own business.\n\n' );
		SceneLib.inventory.takeItem( bleh, this.owcaTavern );
	};
	//Herds (Z);
	Owca.prototype.herds = function() {
		MainView.clearOutput();
		MainView.outputText( 'The herders greet you warmly.  "<i>Welcome, champion!  It is indeed a pleasure to have the honor to talk to such a noble and revered hero.</i>"  You reply with equal grace and ask in a more formal tone if there\'s anything you can do to help them out.' );
		...
		//[if attitude > 70];
		if( CoC.flags[ kFLAGS.OWCAS_ATTITUDE ] > 70 ) {
	...
	Owca.prototype.zePit = function() {
		MainView.clearOutput();
		MainView.outputText( 'You announce to all the villagers present that you\'re going to guard the pit as well as the village\'s entrance against the demons, trusting them to spread the news; they seem glad that you took this initiative on your own.  Checking your equipment, you head toward the pit, ready to stand your ground against the lewd horde.' );
	...
	Owca.prototype.rebeccMenu = function() {
		MainView.clearOutput();
		MainView.outputText( 'You knock and step into the old shack, accustoming yourself to the meager light.  As soon as you close the door behind you, you spot Rebecc stepping out of a little backroom.' );
	...
	Owca.prototype.rebeccBathScene = function() {
		MainView.clearOutput();
		MainView.outputText( 'She grabs your hand and leads you to her bathroom, a tiny place almost entirely occupied by an enormous circular container acting as a bathtub, full of hot steaming milk.  You watch her, puzzled.' );
		...
		if( CoC.player.gender === 0 ) {
			...
			//[if cock];
			if( CoC.player.hasCock() ) {
				MainView.outputText( '; your ' + Descriptors.cockDescript( 0 ) + ' grows to full erect size' );
			}
			if( CoC.player.hasVagina() && CoC.player.hasCock() ) {
				MainView.outputText( ' and ' );
			...
			if( CoC.player.hasVagina() ) {
				MainView.outputText( 'your ' + Descriptors.vaginaDescript() + ' leaks a little trickle of girl-juice' );
			}
			MainView.outputText( '.  She doesn\'t seem to notice the effect she has on you - or so you think.  Suddenly, you feel a hand darting between your legs.' );
		}
	...
	Owca.prototype.rapeRebecc = function( outside ) {
		...
		if( CoC.player.hasCock() ) {
			...
			MainView.outputText( '\n\nThe sight of her glorious rear is too tempting a target to resist, and you quickly shove the tip of your ' + Descriptors.cockDescript( 0 ) + ' between her plush cheeks, bracing yourself for torrid anal penetration.  She keeps whining, unable to break free of your iron grasp.  In her constant struggles you notice an opening between her buttocks - you thrust inside with all your might, devastating her backdoor entrance, stretching her holes, tearing her insides and grinding against her rugged anal walls in a extremely rough way.' );
			//[if cock is too long, >12 inches];
			if( CoC.player.cocks[ 0 ].cockLength > 12 ) {
				MainView.outputText( '  You keep burying more of your ' + Descriptors.cockDescript( 0 ) + ' deep inside her rectum until you reach her limit and can\'t fit more cock in.' );
			} else {
				MainView.outputText( '  Eventually the entirety of your prick is lodged inside her lush asshole.' );
			}
			MainView.outputText( '  She trills and bleats in pain and flounders about in your arms, futilely trying to avoid the incoming butt-rape.  You lock her arms with yours, implacably kneading her tits, and her shivering flesh brings the most exquisite contact to your fingers; you toy with her nipples as well, making her scream savagely in forced pleasure.' );
			MainView.outputText( '\n\nAfter a few minutes of resistance, her body tenses up and you ease yourself into effectively butt-fucking her.  You pull her white fuzzy hair, seize her squeezable hips for more leverage and start raping her ass; the thrusts come hard and fast, almost bringing your ' + Descriptors.cockDescript( 0 ) + ' out before jamming it all the way inside again.  This forceful and tigerish motion makes her shriek in excruciating ecstasy; she groans and moans rhythmically at the pace of your regular ass-pounding.  The torment you\'re inflicting on her poor interior walls is too much to bear and she twists violently to escape the dick-beast dominating her pucker; however, all she can do is wave her appreciable butt, bringing extra sensations to your ' + Descriptors.cockDescript( 0 ) + ' and adding extra spice to your already ardent anal-rape.' );
			MainView.outputText( '\n\nThe cadenced wobbling of her rump as well as your constant pumping is bringing you to a higher state of pleasure; the heavenly tingle your ' + Descriptors.cockDescript( 0 ) + ' is receiving is making you heedless of anything else but the burning heat of her insides cooking your meat.  You feel your lust rise as you keep riding the buttslut below you, ignoring her muffled complaints and whining, her tight confines gradually taking over any control you have over your rod.  You pound Rebecc\'s ass as if your life depended on it, clutching her curvy hips instinctively like an imp in rut.  Your groin impacts her buttocks with a loud SLAP at every thrust' );
			//[if balls];
			if( CoC.player.balls > 0 ) {
				MainView.outputText( ', and your ' + Descriptors.ballsDescriptLight() + ' almost feel sore from colliding with her dribbling cunt' );
			}
			MainView.outputText( '.  Finally, when you think you\'re about to pass out from exhaustion, you cum in a powerful jet, literally blasting Rebecc\'s ass with your spooge, globs of  goo being propelled like projectiles into her colon and splattering her backside with huge white smears.' );
			if( CoC.player.cumQ() > 1500 ) {
				MainView.outputText( '  After completely filling up her rectum, your baby-batter erupts in thick spurts from her anus despite her instinctive ass-clenching efforts to hold as much semen as possible inside.  The sheer might of your cumshot is enough to send Rebecc over the edge, and she releases an absolutely torrential stream of girl-cum, squirting against her will all over your groins and mixing her own fluids with yours in a pathetic show of sluttiness.' );
			}
			MainView.outputText( '  She unconsciously milks you of all your spunk, her anal muscles working on their own to massage your meat and extract every drop from your urethra.' );
			MainView.outputText( '\n\nWith a last, satisfied sigh, you fall over, ' + Descriptors.cockDescript( 0 ) + ' deeply buried inside Rebecc\'s butt as you feel her warm, curvy body slowly waggling below you.  ' );
			if( outside ) {
				MainView.outputText( 'Some of the villagers are apathetically watching you, too stunned and bruised to intervene.  ' );
			}
			MainView.outputText( 'You silently remove your prick from your girl\'s ass, watch her contentedly as her exposed rectum is literally oozing with spooge and walk back to your camp.' );
			MainView.outputText( '\n\nThis is a more-than-appropriate goodbye gift' );
			if( outside ) {
				MainView.outputText( ' to these miserable people, and even moreso' );
			}
			MainView.outputText( ' to the manipulative slut who kept teasing you.  Serves her right, to use her body in the most provocative ways and never let you actually have your way with her.  You\'re unlikely to ever see her again.' );
		...
		//Rebecc Female Rape Scene (Z);
			...
			MainView.outputText( 'You silently pick yourself up and dress, admiring the quivering, crying handiwork at your feet, before walking back to your camp.  This is a more-than-appropriate goodbye gift' );
			if( outside ) {
				MainView.outputText( ' to these miserable people, and even moreso' );
			}
			MainView.outputText( ' to the manipulative slut who kept teasing you.  Serves her right, to use her body in the most provocative ways and never let you actually have your way with her.  You\'re unlikely to ever see her again.' );
	...
	Owca.prototype.torchUpVillagersAndLeave = function() {
		MainView.clearOutput();
		MainView.outputText( 'You don\'t think you\'re going to see these villagers again, and you don\'t really want to.' );
	...
	Owca.prototype.loseToOwca = function() {
		MainView.clearOutput();
		MainView.outputText( 'You kneel down and then fall, overwhelmed by the villagers\' fury and numbers.  Without hesitation, the angry mob jumps on you; you recoil in terror, persuaded that you\'re going to be beaten to death.  Instead, you are carelessly lifted and completely stripped off; ' );
		if( CoC.player.weaponName !== 'fists' ) {
			MainView.outputText( 'your ' + CoC.player.weaponName + ' is taken away and ' );
		}
		MainView.outputText( 'you are being uncomfortably transported to a destination you can guess easily.  Too dazed to resist or even worry about it; you are promptly brought to the dreaded pit, where the villagers tie you up and rudely shackle you.  Then, before you even realize how desperate your situation is, they\'re all gone.  Your numerous bruises and fatigue get the better of you and you quickly fall asleep.' );
	...
	Owca.prototype.forgiveOwca = function() {
		MainView.clearOutput();
		MainView.outputText( 'Swallowing your pride, you tell her that it\'s all right and that you hold no grudge against her; the villagers were obviously desperate, to have resorted to such extreme measures.  You assure her' );
		if( CoC.player.cor >= 50 ) {
			MainView.outputText( ', lying through your teeth,' );
		}
		MainView.outputText( ' that you don\'t feel any resentment at all; she tried her best to help you, and you\'re grateful to her.' );
		...
		EngineCore.dynStats( 'int', 2, 'cor', -8 );
		CoC.flags[ kFLAGS.OWCAS_ATTITUDE ] = 60;
	...
	Owca.prototype.faceDownHordes = function() {
		MainView.clearOutput();
		MainView.outputText( 'You tell Rebecc to go hide on the plains; you\'re going to handle the villagers as well as the demon horde on your own.  She cries, "<i>But ' + CoC.player.short + '!  You stand no chance against this ruthless horde!  Besides, the villagers won\'t trust you.  They\'ll think you want to cheat them... they\'ll tie you up to prevent you from getting away!</i>"' );
	...
	Owca.prototype.slaveToVapulaBadEnd = function() {
		...
		MainView.outputText( '\n\n"<i>Enough.  Remove that dick from my pet\'s mouth, I want to talk to ' + CoC.player.mf( 'him', 'her' ) + ' for a second.</i>"' );
	...
	Owca.prototype.subdueVapula = function() {
		MainView.clearOutput();
		MainView.outputText( 'At last, the final demon falls, ' );
		if( CoC.monster.HP < 1 ) {
			MainView.outputText( 'overwhelmed by your might' );
		} else {
			MainView.outputText( 'rendered insensate by his own lust' );
		}
		MainView.outputText( '.  As usual, you walk among the forlorn bodies of your enemies.  Vapula is groveling on the floor again, looking up to you with imploring eyes.  Even beaten and broken, the luscious succubus remains the most preeminent figure of the motley rabble.' );
	...
	Owca.prototype.enslaveVapulaWithYourWang = function() {
		...
		MainView.outputText( 'You speak in a feverish voice.  "<i>You\'re such a bitch.  Damn, why did you ever think you were going to make me your slut?  How many times do you need to be shown that I\'m not the bottom in our relationship?  You deserve to be turned in to Lethice so she can make you her personal fuck-toy; at least you\'ll remember your place.</i>"' );
	...
	Owca.prototype.enslaveVapulaAsACuntWielder = function() {
		...
		CoC.flags[ kFLAGS.VAPULA_FOLLOWER ] = 0.5;
