...
angular.module( 'cocjs' ).run( function( $rootScope, $log, Kelt, CockTypesEnum, PregnancyStore, Combat, Descriptors, CoC, kFLAGS, Utils, StatusAffects, EngineCore, ConsumableLib ) {
	//Items;
	...
	Kelly.prototype.breakingKeltOptions = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.KELT_BREAK_LEVEL ] > 0 ) {
			if( !CoC.getInstance().player.hasCock() ) {
				...
				EngineCore.menu();
				EngineCore.addButton( 0, 'Next', CoC.getInstance().scenes.farmExploreEncounter );
				return;
			}
		...
		EngineCore.menu();
		EngineCore.addButton( 0, 'Endure', CoC.getInstance().scenes.keltScene.keltEncounter );
	...
	Kelly.prototype.resistKeltsBSBreakHimIntro = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.KELT_BREAK_LEVEL ] === 0 ) {
			...
			if( !(CoC.getInstance().player.hasItem( ConsumableLib.SUCMILK, 15 ) || (CoC.getInstance().player.hasItem( ConsumableLib.SUCMILK, 10 ) && this.hasPinkEgg()) || (CoC.getInstance().player.hasItem( ConsumableLib.P_S_MLK, 10 ) && this.hasPinkEgg()) || CoC.getInstance().player.hasItem( ConsumableLib.P_S_MLK, 15 )) ) {
				...
				EngineCore.addButton( 1, 'Not Yet', CoC.getInstance().scenes.farmExploreEncounter );
				EngineCore.addButton( 2, 'Never', this.neverBreakKeltIntoKelly );
			}
	...
	Kelly.prototype.neverBreakKeltIntoKelly = function() {
		...
		EngineCore.menu();
		EngineCore.addButton( 0, 'Go To Kelt', CoC.getInstance().scenes.keltScene.keltEncounter );
		EngineCore.addButton( 1, 'Go Home', CoC.getInstance().scenes.camp.returnToCampUseOneHour );
	};
	...
	Kelly.prototype.breakKeltGo = function() {
		...
		EngineCore.outputText( 'our [weapon] hits Kelt\'s forehead, knocking him back.  He didn\'t even dodge or defend himself; the hit brought him down immediately.' );
		...
		if( this.hasPinkEgg() ) {
			EngineCore.outputText( ', as well as the egg' );
		}
		EngineCore.outputText( '.  You grab a nearby keg and pour down vial after vial until it\'s half-full.  The creamy substance exudes strong female scents that arouse both of you.' );
		if( this.hasPinkEgg() ) {
			EngineCore.outputText( '  You then throw the egg into the keg before stirring the whole mixture, grinning as you prepare Kelt a whole new meal.' );
		}
		...
		EngineCore.outputText( ' to use later.  You lift the keg and walk up to Kelt.' );
		...
		EngineCore.outputText( '  She occasionally gives your tip a wet kiss, trying to make up for her mistakes.  You ignore her and thrust harder, her round melons squeezing your shaft rhythmically.' );
	...
	Kelly.prototype.defeatKellyNDBREAKHIM = function() {
		...
		EngineCore.outputText( '.  You close in, saying, "<i>Easy now, okay?  You know what I can do with my [weapon].  Now if you just calm down, I promise I\'ll be much nicer this time.</i>"' );
		...
		//(Silly:);
		if( EngineCore.silly() && CoC.getInstance().scenes.amilyScene.amilyFollower() && CoC.getInstance().scenes.amilyScene.amilyCorrupt() ) {
	...
	Kelly.prototype.breakingKeltNumeroThree = function() {
		...
		EngineCore.outputText( 'he doesn\'t attempt to escape or attack you.  Before he can do anything, you\'re already on him. His reactions appear to be a lot slower since he began discovering his new craving for your dick.  He just stares at you nervously, waiting for your next move.' );
	...
	Kelly.prototype.finalKeltBreaking = function() {
		...
		if( EngineCore.silly() ) {
			EngineCore.outputText( '  CUM CANNON!' );
		}
		EngineCore.outputText( '</i>"  A familiar power gathers inside you, and you decide to tap into it.  You pull out of her tits and cum really, really hard.' );
	...
	Kelly.prototype.kellyAppearance = function() {
		...
		if( this.pregnancy.isPregnant ) {
			EngineCore.outputText( '  Drops of milk often dribble from her tits, betraying her potent lactation.' );
		}
		//[enter vagina text] ;
		EngineCore.outputText( '\n\nKelly has a ' );
		//[virgin];
		...
		if( CoC.getInstance().scenes.farmCorruption.hasTattoo( 'kelly' ) ) {
			EngineCore.outputText( '\n\n' );
			if( CoC.getInstance().scenes.farmCorruption.kellyFullTribalTats() ) {
				...
				if( CoC.getInstance().scenes.farmCorruption.numTattoos( 'kelly' ) > 1 ) {
	...
	Kelly.prototype.fuckKellysCunt = function() {
		...
		if( CoC.getInstance().player.cockTotal() > 1 ) {
			...
			EngineCore.outputText( ' down below, collecting her drippings and smearing across her belly.' );
		}
		...
		EngineCore.outputText( 'You lay immobile on her back, panting while her wonderful cunt drains you of every drop.' );
		...
		if( !this.pregnancy.isPregnant ) {
			EngineCore.outputText( ' so the cum can turn you into a proper broodmare' );
		} else {
			EngineCore.outputText( ', maybe knit some for the kids you\'re going to be pumping out for me non-stop' );
		}
	...
	Kelly.prototype.rideKellyForPunishment = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.TIMES_RIDDEN_KELLY_FOR_PUNISHMENT ] === 1 ) {
			...
			//[PC is 6'6</i>" or more/centaur:;
				...
				EngineCore.outputText( ' and after a pause she follows; deliberately led towards your destination.' );
	...
	Kelly.prototype.kellyPopsOutARunt = function() {
		...
		EngineCore.outputText( ' mother\'s milk.' );
		//[if female];
		if( gender === 2 ) {
	...
	Kelly.prototype.talkNHandToKelly = function() {
		...
		if( temp === 0 ) {
			...
			//Futa:;
				...
				EngineCore.outputText( '  Now I am a woman, I work for another woman, and I spend most days polishing her [cock biggest]!' );
			}
		...
		//4.;
			...
			EngineCore.outputText( '.  It\'s a beautiful tactic - the sound and sensation is wonderful but it also cools your cock down just a bit.' );
		...
		//5.;
			...
			if( !CoC.getInstance().scenes.farmCorruption.whitneyCorrupt() ) {
	...
	Kelly.prototype.giveKellyAppleSauce = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.KELLY_TIMES_APPLESAUCED ] === 0 ) {
			...
			if( !CoC.getInstance().scenes.farmCorruption.whitneyCorrupt() ) {
				EngineCore.outputText( ' "<i>I\'d like to go and check on it, but... I don\'t think Whitney likes me much anymore.  I\'d hate to cause more trouble than I already have.</i>"' );
			} else {
				EngineCore.outputText( '“<i>I\'d like to go and check on it, but... I’d hate to get into Mistress Whitney’s bad books.  I’ve caused enough trouble already.</i>”' );
			}
			EngineCore.outputText( ' You \'pfft\' and wave your hand airily to show what YOU think of Whitney.  You tell her to stay put - as her treat, you\'ll go check on the orchard and bring back some apples for her, if there are any.' );
		...
		//Repeat:;
			...
			if( EngineCore.silly() ) {
				EngineCore.outputText( '  Parsley, sage, rosemary and c...' );
			}
		...
		if( !CoC.getInstance().player.isTaur() ) {
			...
			EngineCore.outputText( '.  Did she cum already?  It\'s fortunate you\'ve already taken the edge off, because you\'re not sure how long you would otherwise be able to last against her impossibly turned on, sucking, clinching pussy.  As it is... you grip her huge, muscled backside and pull yourself almost all the way out of her fleshy pocket then spear your way in again, steadily in and then out, working your way into a slow, sensual rhythm.  Ahead of you Kelly\'s head sinks and bobs, and your ears are full of her wet, muffled chews, gulps and moans as she gorges herself on your sweet, sticky sex whilst you gently fuck her.' );
			//1 < cock:;
			if( y >= 0 ) {
			...
			if( y >= 0 ) {
				EngineCore.outputText( ' as her anus clenches around your ' + Descriptors.cockDescript( x ) );
			}
			EngineCore.outputText( '.  With a wordless shout, you tense up and orgasm in tandem, surging line after line of glorious into her wet, churning depths, lost in the ageless sensation of filling a breeding slut\'s holes with thick seed for what seems like oceans of blissful time.' );
		}
		//Centaur: ;
		else {
			...
			if( y >= 0 ) {
				EngineCore.outputText( ' as her anus clenches around your ' + Descriptors.cockDescript( y ) );
			}
			EngineCore.outputText( '.  With a wordless shout, you tense up and orgasm in tandem, surging line after line of glorious into her wet, churning depths, lost to the ageless sensation of filling a breeding mare\'s holes with thick seed for what seems like blissful oceans of time.' );
		}
		//[merge];
	...
	Kelly.prototype.kellyBJsAhoy = function() {
		...
		if( !CoC.getInstance().player.isTaur() ) {
			...
			if( CoC.getInstance().player.biggestCockLength() > 12 ) {
				...
				EngineCore.outputText( '  Her mouth is a good fit for your dick and you soon find your limit, touching the back of her throat with each gentle thrust.  You manage to hold back on your urge to simply face fuck her as hard as you can, instead taking hold of the base of her braid and moving her back and forth on your length as sensually as you can, drawing your dick back to the entrance of her mouth so she can worship your [cockHead biggest] with her tongue and lips, eliciting a moaned \'mmm\' or from her each time before thrusting deep into her maw again.' );
			}
		...
		//Centaur:;
			...
			EngineCore.outputText( '"<i>Please [Master], let me swallow you, let me taste your strength, let me feel like a slut should,</i>" she begs. "<i>I\'ll make it so good for you you\'ll never want anyone else, just like I will never want anyone else.  Just... let me drink your cum!</i>"' );
			...
			if( CoC.getInstance().player.hasVagina() && CoC.getInstance().player.balls > 0 ) {
				...
				if( CoC.getInstance().player.ballSize <= 8 ) {
					...
					EngineCore.outputText( ' breasts whilst slavishly polishing the [balls] of the centaur who made her into, well, a ball-licking bitch.  Whatever the cause the result is the same.  Pleasure thrums through your groin and up your [cock biggest] and you close your eyes, lost to the sensations of her skillful tongue.' );
				}
				//[Huge balls:;
				else {
					EngineCore.outputText( '\n\nAfter curling her tongue here and there over each sensitive orb, bathing them in saliva, she attempts to envelop one in her mouth.  She can\'t though - your testicles, bulging and tight with arousal, are simply too big.  After a short pause she goes back to licking them, lapping at their surface tenderly, licking all around each one for every trace of salt and musk, wetly caressing first one, then the second' );
			...
			//Balls: ;
				...
				if( CoC.getInstance().player.ballSize <= 8 ) {
					...
					EngineCore.outputText( ' breasts whilst slavishly polishing the [balls] of the centaur who made her into, well, a ball-licking bitch, fills her with deep, shameful lust.  Whatever the cause the result is the same; pleasure thrums through your groin and up your [cock biggest] and you close your eyes, lost to the sensations of her skillful tongue.' );
				}
				//[Huge balls:;
				else {
					EngineCore.outputText( '  After curling her tongue here and there over each sensitive orb, bathing them in saliva, she attempts to envelop one in her mouth.  She can\'t though - your testicles, bulging and tight with arousal, are simply too big.  After a short pause, she goes back to licking them, lapping at their surface tenderly, licking all around each one for every trace of salt and musk, wetly caressing first one, then the second' );
			...
			//[Cock <12: ;
				...
				EngineCore.outputText( '  Her mouth is a good fit for your dick and you soon find your limit, touching the back of her throat with each gentle thrust.  You manage to hold back on your urge to simply face fuck her as hard as you can, instead moving your haunches back and forth as sensually as you can, drawing your dick back to the entrance of her mouth so she can worship your [cockHead biggest] with her tongue and lips, eliciting a moaned “mmm” from her each time before thrusting deep into her maw again.' );
			}
	...
	CoC.getInstance().registerScene( 'kelly', new Kelly() );
} );