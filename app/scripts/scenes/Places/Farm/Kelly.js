...
angular.module( 'cocjs' ).run( function( SceneLib, $rootScope, MainView, $log, Kelt, CockTypesEnum, PregnancyStore, Descriptors, CoC, kFLAGS, Utils, StatusAffects, EngineCore, ConsumableLib ) {
	...
	Kelly.prototype.timeChangeLarge = function() {
		return false;
	};
	...
	Kelly.prototype.breakingKeltOptions = function() {
		...
		if( (!CoC.player.hasCock() && CoC.flags[ kFLAGS.KELT_BREAK_LEVEL ] === 0) || CoC.flags[ kFLAGS.NEVER_RESIST_KELT ] === 1 || CoC.player.statusAffectv2( StatusAffects.Kelt ) >= 40 || !CoC.player.findStatusAffect( StatusAffects.Kelt ) ) {
	...
	Kelly.prototype.resistKeltsBSBreakHimIntro = function() {
		...
		if( CoC.flags[ kFLAGS.KELT_BREAK_LEVEL ] === 0 ) {
			...
			MainView.outputText( 'You set out to go get Kelt, eager to teach this slut another lesson of your own.  You explore the farm for a bit before spotting the centaur behind the barn.  However, Kelt seems to have changed since last time: he somehow changed back his gender.  That rebellious little bitch!  Although he doesn\'t look as aggressively masculine as before, and his chest still bears some man-tits, he has gotten back his stern, rude face and you can clearly see a fat prick hanging from his backside.  It doesn\'t seem to be as big as it was before, though.  He must have grown it in a hurry.' );
			//back to farm];
	...
	Kelly.prototype.breakKeltGo = function() {
		...
		if( CoC.player.tallness < 70 ) {
			MainView.outputText( 'You jump up, and y' );
		} else {
			MainView.outputText( 'Y' );
		}
		MainView.outputText( 'our [weapon] hits Kelt\'s forehead, knocking him back.  He didn\'t even dodge or defend himself; the hit brought him down immediately.' );
		...
		if( this.hasPinkEgg() ) {
			MainView.outputText( ', as well as the egg' );
		}
		MainView.outputText( '.  You grab a nearby keg and pour down vial after vial until it\'s half-full.  The creamy substance exudes strong female scents that arouse both of you.' );
		if( this.hasPinkEgg() ) {
			MainView.outputText( '  You then throw the egg into the keg before stirring the whole mixture, grinning as you prepare Kelt a whole new meal.' );
		}
		...
		if( !CoC.player.isTaur() ) {
			MainView.outputText( 'pulling [eachCock] out of your [armor] and stroking yourself with your free hand' );
		} else {
			MainView.outputText( 'slapping your hardening dick' );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ' against your belly in anticipation' );
		}
		MainView.outputText( '.  You might as well add some flavor to the meal.' );
		...
		MainView.outputText( ' small droplets of pre-cum begin dripping out of your ' + Descriptors.multiCockDescriptLight() + ' before being swallowed by the potent, feminine mixture down below.  The white spots are drowning in that milky goo, but you\'re confident that Kelt will be able to recognize the taste.' );
		...
		MainView.outputText( ' to use later.  You lift the keg and walk up to Kelt.' );
		...
		if( !CoC.player.isTaur() ) {
			MainView.outputText( 'fully whip out your [cock biggest] and shove it between her busty tits and slowly start grinding' );
		} else {
			MainView.outputText( 'step over her, shoving your [cock biggest] between her busty tits, grinding it through that smooth valley' );
		}
		MainView.outputText( '.' );
		...
		if( CoC.player.isTaur() ) {
			MainView.outputText( 'dance your hindlegs about in order to ' );
		}
		MainView.outputText( 'cockslap her.' );
		...
		if( CoC.player.isTaur() ) {
			MainView.outputText( 'step over her, moving your hips back and forth to thrust through her more-than-ample cleavage.' );
		} else {
			MainView.outputText( 'grab her nipples and begin to thrust your pecker against her boob valley.' );
		}
		MainView.outputText( '  She occasionally gives your tip a wet kiss, trying to make up for her mistakes.  You ignore her and thrust harder, her round melons squeezing your shaft rhythmically.' );
	...
	Kelly.prototype.defeatKellyNDBREAKHIM = function() {
		...
		if( CoC.monster.lust > 99 ) {
			MainView.outputText( 'Kelt moans, mauling at his mantits in his lust before he realizes what\'s going on' );
		} else {
			MainView.outputText( 'Kelt groans, slumping slightly from all the damage you\'ve done to him' );
		}
		MainView.outputText( '.  You close in, saying, "<i>Easy now, okay?  You know what I can do with my [weapon].  Now if you just calm down, I promise I\'ll be much nicer this time.</i>"' );
		...
		//(Silly:);
		if( EngineCore.silly() && SceneLib.amilyScene.amilyFollower() && SceneLib.amilyScene.amilyCorrupt() ) {
		...
		if( CoC.player.isTaur() ) {
			MainView.outputText( 'push her underneath you towards' );
		} else {
			MainView.outputText( 'pull her close to' );
		}
		MainView.outputText( ' your erect ' + Descriptors.multiCockDescriptLight() + '.' );
		...
		if( CoC.player.balls > 0 ) {
			MainView.outputText( ' and fondling your [balls] with great care' );
		}
		MainView.outputText( '.  Her lips regularly wrap around your [cock biggest] in near-perfect sucking motions.  She\'s getting better at this.' );
	...
	Kelly.prototype.breakingKeltNumeroThree = function() {
		MainView.clearOutput();
		if( CoC.monster.HP < 1 ) {
			MainView.outputText( 'Slumping down, ' );
		} else {
			MainView.outputText( 'Moaning, ' );
		}
		MainView.outputText( 'he doesn\'t attempt to escape or attack you.  Before he can do anything, you\'re already on him. His reactions appear to be a lot slower since he began discovering his new craving for your dick.  He just stares at you nervously, waiting for your next move.' );
		...
		if( !CoC.player.isTaur() ) {
			MainView.outputText( ', sometimes with your [cock] -' );
		}
		MainView.outputText( ' and yell: "<i>What\'s your name?</i>"' );
		...
		if( CoC.player.isTaur() ) {
			MainView.outputText( 'push from behind' );
		} else {
			MainView.outputText( 'pull' );
		}
		MainView.outputText( ', you pin her head against your groin and shove your wang down her throat.  This time, the centauress doesn\'t seem to be hindered or about to choke.  She takes the entirety of your length and sucks you up like the good little whore she is.  Your entrapped cock is squeezed so tightly that you feel like it\'s about to explode; not willing to reach your climax so soon, you swiftly pull out of her mouth.' );
		...
		MainView.outputText( '\n\nYou briskly reach ' );
		if( CoC.player.isTaur() ) {
			MainView.outputText( 'down, pushing her' );
		} else {
			MainView.outputText( 'forward to grab one of her tits and grope it' );
		}
		MainView.outputText( ' convulsively as you feel your lust rise.  ' );
		if( !CoC.player.isTaur() ) {
			MainView.outputText( 'Soon one breast isn\'t enough to contain your lust-driven energy and you reach for the other one with your free hand.  Your constant nipple-teasing has an effect on' );
		} else {
			MainView.outputText( 'Soon, you cannot contain your lust, and you begin to buck against her hands, whinnying with delight.  The rigid, pulsing dick-flesh in her mouth has an effect on' );
		}
		MainView.outputText( ' Kelly at last: you can hear her moan and sigh, and her hands move erratically on your ' + Descriptors.multiCockDescriptLight() + ', sometimes violently yanking it as she loses control.  This brings all kinds of painful yet pleasurable tingles to your body, and only entices you to feel her up even more.' );
		MainView.outputText( '\n\nA familiar warmth builds in your loins ' );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( 'and your [balls] churn ' );
		}
		MainView.outputText( 'as you feel your climax coming.  ' );
		if( !CoC.player.isTaur() ) {
			MainView.outputText( 'You squeeze Kelly\'s boobs so harshly she screams in pain, and y' );
		} else {
			MainView.outputText( 'Y' );
		}
		MainView.outputText( 'ou release a loud groan yourself.  You cum hard, your quivering [cock biggest] spraying spooge in steady jets directly into the keg.  You maintain your grip on the centauress' );
		if( !CoC.player.isTaur() ) {
			MainView.outputText( '\' breasts' );
		}
		MainView.outputText( ', heedless of her pain and only caring about your own orgasm.  You feel her soft hands squeezing your junk there and there in order to milk the last drop of cum and making sure none of it remains stuck in your urethra.  When you\'re done riding out your orgasm, you walk up to the keg and pour down the two bottles of Succubi Milk you have left.  The copious amount of baby-batter mixes with the alright-alabaster fluid to form a bubbling ivory mixture that you bring close to Kelly\'s lips.' );
		MainView.outputText( '\n\n"<i>Drink.</i>"' );
	...
	Kelly.prototype.finalKeltBreaking = function() {
		MainView.clearOutput();
		MainView.outputText( 'You resolutely walk up to the centaur slut for her final lesson.  This time you don\'t even need to find her: she rushes to you on her own, her eyes glinting with need.' );
		...
		if( CoC.player.isTaur() ) {
			MainView.outputText( 'kneels down' );
		} else {
			MainView.outputText( 'bends over' );
		}
		MainView.outputText( ' to reach for your [cock biggest].  She sucks with eagerness and passion; you don\'t even need to pull her head as it moves on its own around your length.  ' );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( 'Her free hands moves down on your groin and caresses your [balls] with expert care and precision; obviously her previous experience in groping has had an effect on her current skills.  ' );
		}
		MainView.outputText( 'Her hunger for your meaty junk seems to be insatiable, for after moments of dick kissing and licking, she decides to take the entirety of your length.  Her eyes are voracious as her fat lips swallow inch after inch of dick-flesh.' );
		...
		if( CoC.player.isTaur() ) {
			MainView.outputText( 'hindleg haunches' );
		} else {
			MainView.outputText( '[butt]' );
		}
		MainView.outputText( ', always pulling your groin closer to her avid mouth as she relentlessly throatfucks herself on you.  Your [cock biggest] is being squeezed and massaged in the most divine way as it knocks against her warm esophagus, and you\'re tempted to cum in her right now, right here. But you need to finish the \'lesson\'; with great regret, you softly ' );
		if( CoC.player.isTaur() ) {
			MainView.outputText( 'back away' );
		} else {
			MainView.outputText( 'push the centaur girl away' );
		}
		MainView.outputText( ' and pull out of her mouth. Wailing with displeasure, the cock-slut decides to give your meat a couple of sad licks, disappointed to see her phallic prize withdrawing from her.  She quickly regains her enthusiasm, however.' );
		...
		if( EngineCore.silly() ) {
			MainView.outputText( '  CUM CANNON!' );
		}
		MainView.outputText( '</i>"  A familiar power gathers inside you, and you decide to tap into it.  You pull out of her tits and cum really, really hard.' );
		...
		MainView.outputText( '\n\nThe cum-flow eventually starts to ebb; less and less goo pours from your ' + Descriptors.multiCockDescriptLight() + ' and your orgasm gradually fades out.  When your euphoric pleasure wears off, you finally get a good look at the creature standing before you.' );
		MainView.outputText( '\n\nShe doesn\'t seem to have changed much in appearance still sports the same nice pair of bouncy tits that seem to ask you to cup them with your hands, and her face is even more cutely feminine than ever.  Nevertheless, you can tell by subtle modifications of her body language that corruption has taken a good hold in her: the way her eyes glow with craving need, her moist pussy that drips seemingly unnatural amounts of juices, and her tail adopts shapes that remind you that of a demon.  Even the way she smiles shows that she feels more than meek pleasure in serving you.' );
	...
	Kelly.prototype.kellyAppearance = function() {
		...
		if( CoC.flags[ kFLAGS.KELLY_BONUS_BOOB_ROWS ] > 0 ) {
			MainView.outputText( '  Beneath those, she has a second row of jiggly tits, just aching to be squeezed.' );
		}
		if( this.pregnancy.isPregnant ) {
			MainView.outputText( '  Drops of milk often dribble from her tits, betraying her potent lactation.' );
		}
		//[enter vagina text] ;
		MainView.outputText( '\n\nKelly has a ' );
		//[virgin];
		if( CoC.flags[ kFLAGS.KELLY_VAGINALLY_FUCKED_COUNT ] === 0 ) {
			MainView.outputText( 'virgin, ' );
		}
		if( CoC.flags[ kFLAGS.KELLY_CUNT_TYPE ] === 0 ) {
			MainView.outputText( 'pink pussy' );
		} else {
			MainView.outputText( 'puffy, black mare-cunt' );
		}
		MainView.outputText( ' placed below her hindquarters.  It is almost constantly dripping rivulets of moisture when you\'re around.  Its shining openness seems to welcome you in her warmth.' );
		if( CoC.flags[ kFLAGS.KELLY_HEAT_TIME ] > 0 && !this.pregnancy.isPregnant ) {
			MainView.outputText( '<b>  There is so much leaking from her that you think she might be in heat, rendering her more receptive to impregnation.</b>' );
		}
		...
		if( SceneLib.farmCorruption.hasTattoo( 'kelly' ) ) {
			MainView.outputText( '\n\n' );
			if( SceneLib.farmCorruption.kellyFullTribalTats() ) {
				...
				if( SceneLib.farmCorruption.numTattoos( 'kelly' ) > 1 ) {
				...
				if( CoC.flags[ kFLAGS.KELLY_TATTOO_COLLARBONE ] !== 0 ) {
					MainView.outputText( CoC.flags[ kFLAGS.KELLY_TATTOO_COLLARBONE ] + '\n' );
				}
				if( CoC.flags[ kFLAGS.KELLY_TATTOO_SHOULDERS ] !== 0 ) {
					MainView.outputText( CoC.flags[ kFLAGS.KELLY_TATTOO_SHOULDERS ] + '\n' );
				}
				if( CoC.flags[ kFLAGS.KELLY_TATTOO_LOWERBACK ] !== 0 ) {
					MainView.outputText( CoC.flags[ kFLAGS.KELLY_TATTOO_LOWERBACK ] + '\n' );
				}
				if( CoC.flags[ kFLAGS.KELLY_TATTOO_BUTT ] !== 0 ) {
					MainView.outputText( CoC.flags[ kFLAGS.KELLY_TATTOO_BUTT ] + '\n' );
				}
				MainView.outputText( '\n' );
			}
		}
		...
		if( CoC.flags[ kFLAGS.KELLY_KIDS ] > 0 ) {
			if( CoC.flags[ kFLAGS.KELLY_KIDS ] === 1 ) {
				...
				MainView.outputText( ' is off in a separate field; still very young, you can see ' );
				if( CoC.flags[ kFLAGS.KELLY_FIRST_KID_GENDER ] === 1 ) {
					MainView.outputText( 'him playing in the distance, lost in his own little world.' );
				} else {
					MainView.outputText( 'her playing in the distance, lost in her own little world.' );
				}
	...
	Kelly.prototype.approachKelly = function() {
		...
		if( CoC.flags[ kFLAGS.FARM_CORRUPTION_STARTED ] === 0 ) {
			MainView.outputText( 'You aren\'t welcome on the farm proper, but you can go visit Kelly\'s field.' );
		}
		...
		if( CoC.time.hours >= 9 && CoC.time.hours <= 11 && CoC.flags[ kFLAGS.KELLY_KIDS ] >= 2 ) {
			...
			if( CoC.flags[ kFLAGS.KELLY_KIDS ] > 2 ) {
				MainView.outputText( 'two youngest ' );
			}
			MainView.outputText( 'children with ' );
			if( CoC.flags[ kFLAGS.KELLY_BONUS_BOOB_ROWS ] > 0 ) {
				MainView.outputText( 'the top row of ' );
			}
			MainView.outputText( 'her milk-laden tits.  When she dreamily opens her eyes and sees you standing there, she nudges them off gently.' );
		...
		if( CoC.player.hasItem( ConsumableLib.CANINEP ) ) {
			...
			if( CoC.flags[ kFLAGS.KELLY_BONUS_BOOB_ROWS ] === 0 ) {
				MainView.outputText( ', but who knows how it will change her' );
			}
			MainView.outputText( '.' );
	...
	Kelly.prototype.fuckKellysCunt = function() {
		...
		MainView.outputText( 'You strip out of your [armor] in a hurry, intent on claiming your equine slut from behind.  As soon as [eachCock] flops free, Kelly takes notice, her hand coming in front of her mouth in surprise.' );
		...
		if( CoC.player.tallness < 72 ) {
			MainView.outputText( 'You turn around to get a stool, since your ' );
			if( CoC.player.tallness < 52 ) {
				MainView.outputText( 'diminutive ' );
			}
			MainView.outputText( 'height makes reaching her pussy a difficult task.  ' );
		}
		MainView.outputText( 'Holding your ' + Descriptors.cockDescript( x ) + ' in one hand, you press your ' + CoC.player.cockHead( x ) + ' against her entrance.   The steamy cunt pulses wetly around your girth, slowly spreading its thick lips to accept your rigid endowment.' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( '  Your other erection' );
			if( CoC.player.cockTotal() > 2 ) {
				MainView.outputText( 's hang' );
			} else {
				MainView.outputText( ' hangs' );
			}
			MainView.outputText( ' down below, collecting her drippings and smearing across her belly.' );
		}
		MainView.outputText( '  You slowly slide inside the tight tunnel, feeling Kelly\'s powerful muscles squeeze delightfully around you, massaging your ' + Descriptors.cockDescript( x ) + ' with slow undulations.' );
		...
		if( CoC.player.cockArea( x ) >= 300 ) {
			MainView.outputText( 'bottom out inside her' );
		} else {
			MainView.outputText( 'slide as far into her as she can handle' );
		}
		MainView.outputText( ' and smack her ass for good measure.  She\'s so fucking warm!  You sit there for a moment, just to revel in the hot, slippery heat.  Then, you draw back, exposing yourself to the cool air - but only for a moment.  You slam your hips back into the tight centaur-twat without hesitation, the force of the impact jolting the larger woman\'s body forward.' );
		...
		if( CoC.player.balls > 0 ) {
			MainView.outputText( ' and roiling balls' );
		}
		MainView.outputText( ' leaves no doubt that climax is fast approaching.  Kelly cums first, splattering your [hips] with her ' );
		if( CoC.flags[ kFLAGS.KELLY_CUNT_TYPE ] > 0 ) {
			MainView.outputText( 'fragrant ' );
		}
		MainView.outputText( 'centaur-cum, soaking your [legs] immediately.  Her pussy goes wild, the once-ordered spasms going wild, wringing your ' + Descriptors.cockDescript( x ) + ' with uncoordinated motions.  A moment later, all those wild contractions sync up into intense, dick-massaging waves.  The power of her equine lower half becomes obvious when her twat seems to suck you inside, squeezing you off to orgasm.' );
		...
		if( CoC.player.cumQ() >= 500 ) {
			MainView.outputText( '  Her pussy seems like a soggy, cum-soaked sex-toy before you\'re even halfway done.' );
		}
		if( CoC.player.cumQ() >= 1200 ) {
			MainView.outputText( '  The sheer amount of spunk exploding into her womb actually rounds her belly a little' );
			if( CoC.player.cumQ() >= 4000 ) {
				MainView.outputText( ', until she\'s so pregnant with spooge that she can barely stand' );
			}
			MainView.outputText( '.  ' );
		}
		MainView.outputText( 'You lay immobile on her back, panting while her wonderful cunt drains you of every drop.' );
		...
		if( !this.pregnancy.isPregnant ) {
			MainView.outputText( ' so the cum can turn you into a proper broodmare' );
		} else {
			MainView.outputText( ', maybe knit some for the kids you\'re going to be pumping out for me non-stop' );
		}
	...
	Kelly.prototype.taurOnTaurSexKelly = function() {
		...
		MainView.outputText( ', pressing it into her face, letting your sweat and the scent of your groin smother as effectively as your presence.  Her tongue extends, knowing what you want even before you have to ask.  She licks you, tentatively and then with increasing levels of confidence, smearing saliva all over your flesh as she licks up every drop of sweat, orally lubricating you with hot, wet tongue.' );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( '  Your [balls] tingle pleasantly from the attention, and you groan when she sucks one into her mouth, worshipping it with her lips.' );
		}
		...
		if( CoC.player.balls > 0 ) {
			...
			MainView.outputText( 'hindlegs' );
		}
		MainView.outputText( ', splattering out in fresh waves from Kelly\'s ' );
		if( CoC.flags[ kFLAGS.KELLY_CUNT_TYPE ] > 0 ) {
			MainView.outputText( 'musky ' );
		}
		MainView.outputText( 'womanhood.  With her body firmly under your own, where it belongs, you firmly assert your control, grabbing her hair in one hand and yanking.  You tug her head back as you bottom out, your body filled with the pleasure of mating like the beasts that you are.' );
		...
		if( CoC.player.cumQ() >= 1000 ) {
			MainView.outputText( '  Her belly grows round with the liquid weight of your spunk' );
			if( CoC.player.cumQ() >= 4000 ) {
				MainView.outputText( ', and when she grows too full to hold any more, waves of alabaster spooge roll out of her abused nether-lips, adding to the messy sex-puddle below' );
			}
			MainView.outputText( '.  ' );
		}
		MainView.outputText( 'Thoroughly creamed, the centaur cum-dump sighs in satisfaction while you dismount her.' );
		MainView.outputText( '\n\nKelly starts to move, then immediately slips in the lubed mud below, legs flailing wildly.' );
		if( CoC.player.cor < 50 ) {
			MainView.outputText( '  You feel a little bad for her and help her up - she did just milk your dick in the way that only a centaur can.' );
		} else {
			MainView.outputText( '  You snicker and walk away - she\'s already served her purpose.' );
		}
		this.kellyPreggers();
		CoC.player.orgasm();
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	...
	Kelly.prototype.tentaFuckKelly = function() {
		MainView.clearOutput();
		MainView.outputText( 'With a mischievous grin, you remove your [armor], fully revealing your ' + Descriptors.multiCockDescriptLight() + '.  The plant-like appendages wriggle around the horny centauress, inspecting her body from every angle.  You feel your own lust rising as the tentacle peckers grow harder, their green heads turning pink in arousal.  Kelly stammers, confused: "<i>W-what are you-</i>"' );
		var one = -1;
		var two = -2;
		var three = -3;
		var four = -4;
		...
		MainView.outputText( '\n\nSLAM!  You profit from the slut\'s babbling to shove your ' + Descriptors.cockDescript( one ) + ' right into her gaping mouth, forcefully filling her throat with dick-flesh.  Her lips instinctively cling to your shaft, sucking it like a popsicle.  Drops of pre-cum are quickly swallowed and ingested by the hungry whore as they escape your urethra.  You begin thrusting in and out of her warm mouth, but her lips squeeze your rod so tight that you have to fight for every inch.  As you stuff her full of your ' + Descriptors.cockDescript( one ) + ', you can hear her endless muffled moans; the slut is obviously getting off to the taste of your meaty junk.  Completely driven with lust, the green tentacle prick is vibrating on its own as it fucks Kelly\'s throat, sending you soft, prickling sensations.  You can\'t help but release a grunt as the centaur slut fellates you so expertly.' );
		MainView.outputText( '\n\nKelly is doing an incredible job with her mouth, teasing you into oblivion.  Your ' + Descriptors.cockDescript( two ) + ' is convulsing recklessly in frustration, begging to be stuffed into some warm hole.  Therefore, you naturally direct it toward the centauress\' wet snatch; it is easily spottable due to the regular streams of juices that ooze from it.  Moreover, the slut is incredibly turned on from sucking you off, and the fem-spunk trails are even thicker than usual.  It doesn\'t take long for your secondary tentacle dong to reach the entrance of her moist love-tunnel.  You grind the tip of your ' + Descriptors.cockDescript( two ) + ' against it, using her genitals as a masturbation aid.  When it is hard as bark and ready to go, you ram it through Kelly\'s fuck-hole, utterly piercing her from behind.' );
		MainView.outputText( '\n\nYou ream the centaur slut from both sides, making her whinny and thrash violently.  She tramples the floor with her hooves, arches her back and screams in ecstasy through the inches of dick-flesh blocking her throat; she is completely heedless of the world beside the two hard vegetal poles fully impaling her on either end.  You pump away in rhythm, synchronizing each thrust in both holes.  Her twat is almost inhumanly hot, and gloriously tight.  However, the slut tries her best to wrap her lips even tighter around your main flexible pecker, and her tongue is teasing your ' + Descriptors.cockDescript( one ) + ' in the most excruciating way.  You stuff the mare slut full of your green appendages, treating her body like a bipolar cocksleeve.' );
		//[if (cocks > 2);
		if( three >= 0 ) {
			MainView.outputText( '\n\nThe display of Kelly trembling in pleasure as you spit her like a pig on your duo of plant-like pricks is enticing, and your ' + Descriptors.cockDescript( three ) + ' is aching with need.  It almost moves on its own volition toward the woman\'s perky tits, ready for a good boobjob.  The supple green wang lodges itself between the busty melons; the slut instantly begins to squeeze her heavy globes around the rod of dick-flesh, fully entrapping it in her bosom.  Her tits are so tender and soft you feel like your tentacle pecker is about to melt.  Groaning in pleasure, you thrust your ' + Descriptors.cockDescript( three ) + ' up and down as the titty-fucked whore accompanies your motion.  You grind in rhythm so the slut doesn\'t forget your other tentacle dongs.  Fortunately, Kelly quickly picks up your intention and soon you can\'t tell which member is providing you with the most pleasure; her mouth, vagina and soft tits are all working in conjunction, and you are fully lost in a never-ending spiral of ecstasy.' );
		}
		//if (cocks > 3);
		if( four >= 0 ) {
			MainView.outputText( '\n\nYour ' + Descriptors.cockDescript( four ) + ' trembles furiously, unable to find some body part that isn\'t taken by its colleagues.  You grin wickedly as an idea comes to your mind.  The protruding appendage writhes as it threads its way toward Kelly\'s right hand; the slut is still focusing on your ' + Descriptors.cockDescript( three ) + ', squeezing her tits as tightly as she can.  Needless to say, she is surprised when another tentacle shoves itself in the way, right under her palm.  She doesn\'t lose her composure, though; using her forearm to press on her chest, her hand quickly grabs hold of your tree-like pecker and begins stroking your length.  Although her handjob doesn\'t feel as warm and tight as the wet depths of her love-tunnel or her throat, or even the squelching contact of her tender boob-flesh, she teases you efficiently with her fingers.  Her hand massages your ' + Descriptors.cockDescript( four ) + ' and clutches it like a vice, milking some drops of pre-cum out of your shaft.  She takes care of the most sensitive points and often makes you moan when she squeezes the towering rod.' );
		}
		//[if (cocks > 4);
		if( five >= 0 ) {
		...
		if( CoC.player.cor < 33 ) {
			MainView.outputText( 'but also a bit disgusted to watch such shameless, wanton behavior, ' );
		}
		MainView.outputText( 'you walk away, letting the slut enjoy herself.' );
		this.kellyPreggers();
		CoC.player.orgasm();
	...
	Kelly.prototype.giveKellyEquinum = function() {
		MainView.clearOutput();
		MainView.outputText( 'Taking the long, flared vial in your hand, you get a wicked idea - what if you gave your personal mare-slave a cunt that was better suited to her depraved, animalistic shape?  What if you turned that pretty, pink pussy into a slobbering, black mare-cunt?' );
	...
	Kelly.prototype.giveKellySuccubiMilk = function() {
		MainView.clearOutput();
		MainView.outputText( 'You produce a vial of succubi milk and dangle it before Kelly, the ivory demon-juice sloshing audibly inside.  Kelly licks her lips immediately, running her hands across her perky nipples without meaning to.  She asks, "<i>[Master], is that what I think it is?</i>"' );
	...
	Kelly.prototype.getARimjobFromKelly = function() {
		...
		MainView.outputText( '"<i>You aren\'t worthy of my dick,</i>" you say, as you slowly peel off your [armor]; you do it facing away from Kelly so that she can take in your [butt] as you slowly wriggle free of your underclothing.  "<i>But I still need servicing.  So...</i>"' );
		//[Naga: ;
		if( CoC.player.isNaga() ) {
			...
			MainView.outputText( '\n\nYou gesture at the ground.  "<i>Lie down,</i>" you command.  Carefully Kelly folds her body up to do so, until her horse half is prostrate and only her human half is propped up.  You step forwards and for a few moments slowly brush your semi-erect ' + Descriptors.cockDescript( x ) + ' against her soft face, teasing her with the idea that you\'ve changed your mind; she struggles with the urge to wrap her lips around it, emitting small bitten off grunts and moans to your ministrations.  Finally, with a grin you pull away and turn around, so that her mouth is now facing your [butt].' );
		}
		...
		if( CoC.player.isNaga() ) {
			MainView.outputText( 'With reptilian litheness, you slowly lean backwards and press your hands onto the back of her head, pushing her face further between your cheeks.  ' );
		} else if( !CoC.player.isTaur() ) {
			MainView.outputText( 'You lean backwards and press your hands onto the back of her head, pushing her face further between your cheeks.  ' );
		}
		MainView.outputText( 'You laugh at the sensation of her tongue as it pushes into your anal passage, slathering it with worshipful attention, perhaps hoping the sooner she gets you off the sooner this can end.' );
		//[Not Centaur: ;
		if( !CoC.player.isTaur() ) {
			MainView.outputText( '  You smirk and grip her hair, moving her head in rhythm with your own movements.' );
		} else {
			MainView.outputText( '  You gyrate your [butt] this way and that, moving her head in rhythm with your own movements.' );
		}
		...
		MainView.outputText( '?  I can\'t say it\'s something you don\'t deserve for being such a shameless slut, and I don\'t doubt you\'d enjoy every minute of it.</i>"' );
		...
		if( CoC.player.hasVagina() ) {
			MainView.outputText( '; she slides as many fingers as she can into your [vagina] and frigs you as best she can' );
		}
		MainView.outputText( '.  Grunting, you reach your peak.  Your ' + Descriptors.cockDescript( x ) + ' opens wide and you ejaculate streams of cum, splattering into the dust in front of you, your whole body clenching and seizing around the warm, wet muscle wedged in your [butt].' );
		//[Pussy:];
		if( CoC.player.hasVagina() ) {
			MainView.outputText( '  Your female sex orgasms in tandem, your wet flesh seizing up around Kelly\'s fingers and spattering her arm and chest with girl juices.' );
		}
	...
	Kelly.prototype.rideKellyForPunishment = function() {
		...
		if( CoC.flags[ kFLAGS.TIMES_RIDDEN_KELLY_FOR_PUNISHMENT ] === 1 ) {
			MainView.outputText( 'You stroke Kelly\'s side thoughtfully.  What would be a good punishment for a centaur? Several ideas immediately occur, but all require... equipment.  You tap your chin as a plan of action falls into place.' );
			...
			if( CoC.player.tallness < 78 && !CoC.player.isTaur() ) {
				...
				//[Naga:];
				if( CoC.player.isNaga() ) {
					MainView.outputText( '  Your reptilian half is hardly ideal for riding a horse but eventually you reach a happy compromise; you perch upon her with your long tail wrapped around her barrel-like lower body.' );
				}//[Drider:];
				else if( CoC.player.isDrider() ) {
					MainView.outputText( '  Your spider half is hardly ideal for riding a horse but eventually you reach a happy compromise; you perch upon her with your long, chitinous legs wrapped carefully around her barrel-like lower body.  You must look utterly bizarre but feel reasonably comfortable.' );
				}
				MainView.outputText( '  You lean in close to her human half and wrap your arms around her waist, drinking in the sweet scent of her pale skin; you know your own smell and power is pressing upon her senses and she whimpers softly as you slide your hands up her stomach until they are clutching her ' );
				if( CoC.flags[ kFLAGS.KELLY_BONUS_TIT_ROWS ] > 0 ) {
					MainView.outputText( 'top row of ' );
				}
				MainView.outputText( 'breasts.' );
				...
				if( CoC.flags[ kFLAGS.KELLY_LACTATING ] > 0 ) {
					MainView.outputText( ' hard enough to produce thin arcs of milk' );
				}
				MainView.outputText( '.  The centauress gasps, her breath already ragged. "<i>...I\'ve got to hold onto something.  Now, giddy up!</i>" Kelly jolts into action, stumbles slightly at the extra weight, before righting and trotting in the direction of the farm; you keep yourself steady by maintaining a firm grip upon her breasts.' );
				//[PC is riding: ;
			...
			//[PC is 6'6</i>" or more/centaur:;
				...
				if( CoC.flags[ kFLAGS.KELLY_LACTATING ] > 0 ) {
					if( CoC.flags[ kFLAGS.KELLY_BONUS_TIT_ROWS ] > 0 ) {
						MainView.outputText( 'All four of h' );
					} else {
						MainView.outputText( 'H' );
					}
					MainView.outputText( 'er swollen teats immediately begin to dribble milk, and she moans.  ' );
				}
				MainView.outputText( '"<i>So we will have to find some other way you can serve me.  We\'re going to go across to the farm.  You remember where that is, don\'t you?  It\'s where I taught you who you really are.  We\'re going to learn some more lessons there.  Won\'t that be fun?</i>" You let go and set off at a ' );
				if( !CoC.player.isTaur() ) {
					MainView.outputText( 'march' );
				} else {
					MainView.outputText( 'canter' );
				}
				MainView.outputText( ' and after a pause she follows; deliberately led towards your destination.' );
		...
		//First repeat: ;
			...
			MainView.outputText( 'Without another word, you stride over to the stones underneath which you secreted the riding gear.  Kelly watches you with deep apprehension, and then bursts into protests as you return, carrying the heap of clinking leather and metal.' );
			...
			if( CoC.flags[ kFLAGS.KELLY_BONUS_TIT_ROWS ] > 0 ) {
				MainView.outputText( '  Just as well there were four of them, eh?' );
			}
			MainView.outputText( '</i>" You step back to admire the effect.  Kelly stares back at you behind her leather straps and ball gag, her tasselled boobs forced outwards by her arms bound behind her back.  You laugh and stroke her face; she closes her eyes and bends into it, enjoying the simple motion of her [Master]\'s hand.' );
			...
			//PC not rider: ;
				...
				MainView.outputText( 'You grab hold of the reigns and stride off; after a short jerk Kelly follows behind you.  "<i>We\'re heading to the desert,</i>" you announce.  "<i>I think you can maybe guess where.  Remember; flick those legs high, prance a little, make those tassels whirl.  You represent your [Master], and you don\'t want ' + CoC.player.mf( 'him', 'her' ) + ' to look bad, do you?</i>"  There\'s a muffled whimper in response.' );
	...
	Kelly.prototype.takeKellysVirginity = function() {
		...
		MainView.outputText( 'You tell Kelly that you intend to come claim the prize you broke her for - her virgin ' );
		...
		if( CoC.flags[ kFLAGS.KELLY_CUNT_TYPE ] === 1 ) {
			MainView.outputText( 'lust-swollen, black-lipped fuck-hole' );
		} else {
			MainView.outputText( 'blooming, pink-tinged pussy' );
		}
		MainView.outputText( ' to your roving eyes.  She asks, "<i>Is th-th-this good, [Master]?</i>"' );
		...
		MainView.outputText( ' cunt until her hymen is easily visible.  You rub every sensitive fold that you can get to' );
		if( CoC.flags[ kFLAGS.KELLY_CUNT_TYPE ] === 1 ) {
			MainView.outputText( ', taking time to squeeze her plush, onyx vulva teasingly.  Kelly whinnies in response, overtaken by her potent equine instincts.' );
		} else {
			MainView.outputText( ', taking time to fondle her slick labia.  Kelly moans in response, overtaken by the sensations you\'re forcing through her.' );
		}
		...
		if( !CoC.player.isTaur() ) {
			MainView.outputText( 'get down behind her and line your ' + Descriptors.cockDescript( x ) + ' with her dribbling lips.' );
		} else {
			MainView.outputText( 'lie down behind her, sliding your hindlegs behind hers in order to properly align your ' + Descriptors.cockDescript( x ) + ' with her dribbling lips.' );
		}
		MainView.outputText( '  The first contact of ' + CoC.player.cockHead( x ) + ' on her feminine mound sends an electric current of pleasure through your member, one that has you immediately and heedlessly pushing forward, sliding your full, sensitive phallus as deeply into that slick opening as you can.  That turns out not to be very far, as you almost immediately hit her virginal hymen.  It blocks off the rest of that hot, dripping hole almost spitefully.' );
		MainView.outputText( '\n\nYou squeeze Kelly\'s ' );
		if( CoC.player.isTaur() ) {
			MainView.outputText( 'breast as you snuggle up behind her, centaur to centaur,' );
		} else {
			MainView.outputText( 'ass' );
		}
		MainView.outputText( ' and laugh, "<i>Here it comes, you eager whore!</i>"  Then, you spear through her entrance with one powerful thrust.  In that second, her pussy somehow gets even hotter and warmer around your girth, and you sink yourself as deeply as you can' );
		if( CoC.player.cockArea( x ) <= 300 ) {
			MainView.outputText( ', nestling your [sheath] up against her ' );
			if( CoC.flags[ kFLAGS.KELLY_CUNT_TYPE ] === 1 ) {
				MainView.outputText( 'plump, equine cunt-lips' );
			} else {
				MainView.outputText( 'slippery, pink cunt-lips' );
			}
		} else {
			MainView.outputText( ', pressing your ' + CoC.player.cockHead( x ) + ' up against the barrier her cervix makes' );
		}
		MainView.outputText( '.  A pinkish froth oozes out around you, tinted by the few drops of blood that claiming her generated.  Kelly made a pained face as you took her, but now, she\'s back to panting and moaning like a whore.' );
		...
		if( CoC.player.cumQ() >= 500 ) {
			MainView.outputText( '  You even have enough jism to make sure that some is squirting into her womb and drizzling out of her entrance, mixing with her clear fluids to make a runny, off-wet mess down her rump.' );
			if( CoC.player.cumQ() >= 1500 ) {
				MainView.outputText( '  In no time flat, her belly has begun to round slightly, puffed up with spunk.' );
			}
			if( CoC.player.cumQ() >= 3500 ) {
				MainView.outputText( '  That doesn\'t stop you from seeding it even further, pumping her poor horse-gut until it jiggles and jostles with your spermy cargo.  There\'s no way she\'ll forget this.' );
			}
		}
		MainView.outputText( '\n\nYou pull out with a self-satisfied sigh, admiring ' );
		if( CoC.flags[ kFLAGS.KELLY_CUNT_TYPE ] === 0 ) {
			MainView.outputText( 'the redness of her well-used cunt.' );
		} else {
			MainView.outputText( 'the shining juices that drip from her oh-so-equine entrance, gilding the smooth onyx lips with a slimey sheen.' );
		}
		MainView.outputText( '  Kelly has passed out in a puddle of her own making, and you leave her like that, getting dressed without a backwards glance.' );
		this.kellyPreggers();
		CoC.player.orgasm();
	...
	Kelly.prototype.giveKellyAPepper = function() {
		...
		//Repeat:;
			...
			MainView.outputText( 'You grin and dangle the pepper in front of Kelly\'s face wordlessly.  The centauress knows what\'s coming and color spreads across her cheeks; however, she still opens wide and accepts the "<i>gift</i>", chewing slowly and swallowing.' );
			//If no second row:;
			if( CoC.flags[ kFLAGS.KELLY_BONUS_BOOB_ROWS ] === 0 ) {
	...
	Kelly.prototype.kellyTitJob = function() {
		...
		if( CoC.player.cocks[ x ].cockLength <= 18 ) {
			...
			MainView.outputText( '  Her expression is one of deep concentration and as you quicken the pace and thrust further upwards she bends her chin down to lick at your head as it presents itself between her pressed together tits.' );
		...
		//Dick >17 inches, Kelly has four breasts:;
			...
			MainView.outputText( '  Her expression is one of deep concentration and as you quicken the pace and thrust further upwards she bends her chin to lick at your head as it presents itself between her pressed together tits.' );
	...
	Kelly.prototype.kellyPreggers = function() {
		if( this.pregnancy.isPregnant ) {
			return;
		}
	...
	Kelly.prototype.kellyPopsOutARunt = function() {
		...
		MainView.outputText( ' mother\'s milk.' );
		//[if female];
		if( gender === 2 ) {
	...
	Kelly.prototype.talkNHandToKelly = function() {
		...
		if( CoC.flags[ kFLAGS.KELLY_TALK_N_HAND_TIMES ] === 0 ) {
			MainView.outputText( '"<i>Talk?  Oh, that\'s the least interesting thing I can do with my mouth, [Master],</i>" Kelly purrs.  That may be, you say, but she can amuse you with her hands whilst she amuses you with her voice.  The centaur gets the picture and grins.  ' );
			//Not Centaur: ;
			if( !CoC.player.isTaur() ) {
				MainView.outputText( 'She helps you take off the lower half of your [armor] as you set yourself down and then wraps her warm hand around ' );
				if( CoC.player.cockTotal() === 1 ) {
					MainView.outputText( '[oneCock], gently tugging it.  It quickly hardens with the pleasing friction she coils around it.' );
				} else {
					MainView.outputText( 'first your [cock], then your [cock 2], gently tugging until they are both hard from the pleasing friction she coils around them.' );
				}
			}
			//Centaur:;
			else {
				MainView.outputText( 'You leisurely clop yourself around so your back end is facing her and sigh as ' );
				if( CoC.player.cockTotal() === 1 ) {
					MainView.outputText( 'a warm hands wraps itself around your [cock] and then begins to gently tug, quickly hardening it with pleasing coils of friction.' );
				} else {
					MainView.outputText( 'a warm hand wraps around first your [cock] and then your [cock 2] before beginning to gently tug, both quickly hardening from the pleasing coils of friction.' );
				}
			}
		}
		//Repeat: ;
		else {
			MainView.outputText( 'You tell her you\'d like to... talk.' );
			MainView.outputText( '\n\nGrinning, your centaur slave ' );
			if( !CoC.player.isTaur() ) {
				MainView.outputText( 'slides off the lower half of your [armor] as you set yourself down and then grasps your cock' );
				if( CoC.player.cockTotal() > 1 ) {
					MainView.outputText( 's' );
				}
				MainView.outputText( ' with her soft hands.  You sigh as she begins to gently pump, making ' );
				if( CoC.player.cockTotal() > 1 ) {
					MainView.outputText( 'them' );
				} else {
					MainView.outputText( 'it' );
				}
				MainView.outputText( ' harden quickly.' );
			} else {
				MainView.outputText( 'trots around behind you.  You sigh as ' );
				if( CoC.player.cockTotal() === 1 ) {
					MainView.outputText( 'a warm hands wraps itself around your [cock] and then begins to gently tug, quickly hardening it with pleasing coils of friction.' );
				} else {
					MainView.outputText( 'a warm hand wraps around first your [cock] and then your [cock 2] before beginning to gently tug, both quickly hardening from the pleasing coils of friction.' );
		...
		if( temp === 0 ) {
			...
			if( CoC.player.cor < 70 ) {
				MainView.outputText( '  Plus, the thought of making Kelt\'s victims pay the price for his own crimes makes you feel uncomfortable.' );
			}
			MainView.outputText( '  You tell Kelly you appreciate the thought as you pick yourself up and leave, but you think you need to save your energy and cum for the really special case that she is.  The centaur shivers with glee.' );
		...
		//2. ;
			...
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( ' and [cock 2]' );
			}
			MainView.outputText( '.  "<i>Of course I can never repay you for what you\'ve done for me.  I wasn\'t thinking.</i>"' );
		...
		//3.;
			...
			if( !CoC.player.isTaur() ) {
				MainView.outputText( 'into the air' );
			} else {
				MainView.outputText( 'onto the ground' );
			}
			MainView.outputText( '.  Kelly nuzzles your [cocks] when you\'re done, slurping obscenely as she polishes your nobhead' );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ' to a shine.  You thank her with a slap on the ass and leave.' );
		}
		//4.;
		else if( temp === 3 ) {
			...
			if( CoC.player.balls === 0 ) {
				MainView.outputText( 'suckles and laps at the very base of your [cock] for a time' );
			} else {
				MainView.outputText( 'suckles and laps at your [balls] for a time' );
			}
			MainView.outputText( '.  It\'s a beautiful tactic - the sound and sensation is wonderful but it also cools your cock down just a bit.' );
			...
			if( CoC.flags[ kFLAGS.KELLY_KIDS ] > 0 ) {
				MainView.outputText( ' and bearing and raising your children' );
			}
			MainView.outputText( ', I\'m a hundred times happier.  I just used a demon\'s curse to get my way - you did it all on your own.  You showed me what true strength and dominance is, [Master]!</i>"' );
		...
		//5.;
			...
			if( !SceneLib.farmCorruption.whitneyCorrupt() ) {
	...
	Kelly.prototype.rewardKelly = function() {
		...
		if( CoC.flags[ kFLAGS.KELLY_TIMES_REWARDED ] === 0 ) {
			MainView.outputText( 'There seems to be an added bounce to Kelly\'s canter today.  She prances a bit playfully in front of you, grinning, whipping her demonic tail so that the high, sweet smell of her gushing pussy is in the air.  You realize she is calling your attention to her backside, which is unmarked.' );
		...
		//Repeat:;
			...
			MainView.outputText( 'Kelly holds her head up proudly as you inspect her, running your hand slowly over her backside.   It\'s nice and smooth.  You smile at her benevolently and tell her that since she\'s been such a good cumslut, you\'re going to reward her.  She claps excitedly.' );
			MainView.outputText( '  "<i>Hurray!</i>"' );
		}
	...
	Kelly.prototype.dyeKellysBitchAssHair = function( color ) {
		MainView.clearOutput();
		MainView.outputText( 'You tell her you\'ve brought her a gift as you rummage around in your pockets.  Kelly looks apprehensive but pleasant surprise forms on her face when she catches the small vial of dye you throw at her.' );
		...
		MainView.outputText( 'This\'ll be so much fun!</i>"  You walk over to the barn with her and pour some water into a wide bucket.  She lets down her braid and you spend a pleasant quarter of an hour or so helping her work the sharp-smelling substance into her long, luscious hair.  When you\'re done, you step back to admire the effect.  Amazingly the color is not only sinking into her hair but also into her tail, her demon spade slowly bleaching with the hue you chose.' );
	...
	Kelly.prototype.giveKellyAppleSauce = function() {
		...
		if( CoC.flags[ kFLAGS.KELLY_TIMES_APPLESAUCED ] === 0 ) {
			MainView.outputText( 'You tap your chin idly and ask Kelly what she likes eating.  Aside from dick, you add, rolling your eyes as she opens her mouth eagerly.' );
			...
			if( !SceneLib.farmCorruption.whitneyCorrupt() ) {
				MainView.outputText( ' "<i>I\'d like to go and check on it, but... I don\'t think Whitney likes me much anymore.  I\'d hate to cause more trouble than I already have.</i>"' );
			} else {
				MainView.outputText( '“<i>I\'d like to go and check on it, but... I’d hate to get into Mistress Whitney’s bad books.  I’ve caused enough trouble already.</i>”' );
			}
			MainView.outputText( ' You \'pfft\' and wave your hand airily to show what YOU think of Whitney.  You tell her to stay put - as her treat, you\'ll go check on the orchard and bring back some apples for her, if there are any.' );
			...
			if( !CoC.player.isTaur() ) {
				MainView.outputText( 'You remove the bottom half of your [armor] and grip your [cock].  You close your eyes, begin to stroke yourself and sigh as you think of Kelly- her fine, bountiful curves, her cute, dimpling face, how she unconsciously licks her cock-sucking lips when she sees you coming, how she begs for it, how sweetly she moans when you thrust deep into her tight, wet holes... your body clenches as you orgasm, spurting streamer after streamer of spunk into the apple mash.' );
				if( CoC.player.hasVagina() ) {
				...
				if( CoC.player.cumQ() >= 750 ) {
					MainView.outputText( '  The bowl quickly begins to brim as your [cock] torrents out cum, and eventually you have to turn away, grunting out the last jets of filth into the undergrowth.' );
				}
			}
			//[Centaur:];
			else {
				MainView.outputText( 'You position your lower half over the bowl.  Your [cock] is already semi-turgid with the idea of what you\'re going to do, and it begins to bulge as you close your eyes, sigh and think of Kelly- her fine, bountiful curves, her cute, dimpling face, how she unconsciously licks her cock-sucking lips when she sees you coming, how she begs for it, how sweetly she moans when you thrust deep into her tight, wet holes... your body clenches as you orgasm, spurting streamer after streamer of spunk into the apple mash.' );
				if( CoC.player.hasVagina() ) {
					MainView.outputText( '\n\nYour [vagina] quivers and clenches, and gasping with the dual orgasm which is clutching your body you stand over the bowl and let some of your femcum drip into it.  Taste my summer flavor Kelly, you think wildly, and laugh with a slightly manic edge.' );
				}
				//[High cum:;
				if( CoC.player.cumQ() >= 750 ) {
					MainView.outputText( '  Your [cock] torrents out cum, and eventually you have to clop away, grunting out the last jets of filth onto the ground to avoid overflowing the bowl.' );
				}
			}
		...
		//Repeat:;
			...
			if( CoC.flags[ kFLAGS.KELLY_KIDS ] > 1 ) {
				MainView.outputText( '  Somebody has even cleared an oblong patch of earth alongside the trees- a piece of card stuck into it is scrawled with a childish hand which declares it to be “CAROTS”.  You marvel at it all.  Who would have imagined your lust for horse pussy would have turned a pointless asshole into a loving mother who gardens with her children?  Really, you deserve a medal for your efforts.' );
			}
			...
			if( !CoC.player.isTaur() ) {
				MainView.outputText( 'grasp your semi-turgid cock and drift into a fond reverie of Kelly- the way her boobs bounce when she canters towards you, the way her green eyes burn when they fall upon your [eachCock], the way she gasps when you slap her ass, how sweetly she moans when you thrust deep into her tight, wet holes, the galaxy of filthy things you still have left to do to her....  Your body clenches as you orgasm, spurting streamer after streamer of spunk into the apple mash.' );
				if( CoC.player.hasVagina() ) {
					MainView.outputText( '  Your [vagina] quivers and clenches, and once again, gasping with the dual orgasm which is clutching your body, you stand over the bowl and let some of your femcum drip into it.' );
				}
				//[High cum:;
				if( CoC.player.cumQ() >= 750 ) {
					MainView.outputText( '  The bowl quickly begins to brim as your [cock] torrents out cum, and eventually you have to turn away, grunting out the last jets of filth into the undergrowth.' );
			...
			if( EngineCore.silly() ) {
				MainView.outputText( '  Parsley, sage, rosemary and c...' );
			}
			MainView.outputText( '  Ooh!</i>"  You slap her ass and grab her around her supple, human waist as you pass her.  Laughing, you walk into the barn together.' );
		}
		//[merge];
		var x = CoC.player.cockThatFits( 300 );
		...
		if( !CoC.player.isTaur() ) {
			...
			if( CoC.player.cocks[ x ].cockThickness >= 4 ) {
				MainView.outputText( '  Your girth spreads her fuck tunnel wide and you grunt at the delicious tightness of it as you pack her full of cock.' );
			}
			...
			MainView.outputText( '.  Did she cum already?  It\'s fortunate you\'ve already taken the edge off, because you\'re not sure how long you would otherwise be able to last against her impossibly turned on, sucking, clinching pussy.  As it is... you grip her huge, muscled backside and pull yourself almost all the way out of her fleshy pocket then spear your way in again, steadily in and then out, working your way into a slow, sensual rhythm.  Ahead of you Kelly\'s head sinks and bobs, and your ears are full of her wet, muffled chews, gulps and moans as she gorges herself on your sweet, sticky sex whilst you gently fuck her.' );
			//1 < cock:;
			if( y >= 0 ) {
				MainView.outputText( '\n\nYour eyes fasten upon her tight, puckered asshole as lust grips you closer, a tiny puddle of human pink buried in her heaving brown horse flesh.  Your breath coming heavily now you withdraw your ' + Descriptors.cockDescript( x ) + ' from her wet softness, dripping with her lubrication.  Quickly you push it against her puckered anus whilst lining up your ' + Descriptors.cockDescript( y ) + ' with her gaping vagina.' );
			...
			if( CoC.player.balls > 0 ) {
				MainView.outputText( ', your [balls] slapping into her thighs demandingly ' );
			}
			...
			if( y >= 0 ) {
				MainView.outputText( ' as her anus clenches around your ' + Descriptors.cockDescript( x ) );
			}
			MainView.outputText( '.  With a wordless shout, you tense up and orgasm in tandem, surging line after line of glorious into her wet, churning depths, lost in the ageless sensation of filling a breeding slut\'s holes with thick seed for what seems like oceans of blissful time.' );
		}
		//Centaur: ;
		else {
			...
			MainView.outputText( ' as you push towards your reward.' );
			MainView.outputText( '\n\nPanting, Kelly thrusts back into you as best she can, resisting your frenetic tugs to her hair so that the big, final slather of apple sauce can slide out of the bowl and into her mouth.  She screams around the wet, gooey mouthful and her vagina quivers and seizes your bulging ' + Descriptors.cockDescript( x ) + ' in milking ripples' );
			if( y >= 0 ) {
				MainView.outputText( ' as her anus clenches around your ' + Descriptors.cockDescript( y ) );
			}
			MainView.outputText( '.  With a wordless shout, you tense up and orgasm in tandem, surging line after line of glorious into her wet, churning depths, lost to the ageless sensation of filling a breeding mare\'s holes with thick seed for what seems like blissful oceans of time.' );
		}
		//[merge];
		MainView.outputText( '\n\nYou pool onto the floor when you are finished, ' );
		if( CoC.player.isBiped() ) {
			MainView.outputText( 'giving at the knees, ' );
		}
		MainView.outputText( 'your aching cock still weakly clenching.  In a beatific daze you listen to the sound of a tongue eagerly exploring every inch of a metal surface, before a bowl hits the ground.  A hand is offered to you and woozily you take it.  As soon as you are up Kelly pulls you into a fervent, passionate kiss, her soft lips pushing into yours.  The smell of apples envelops you.  You tense for a moment but decide to allow it.  It\'s her treat, after all.' );
		MainView.outputText( '\n\nYou bend your head, sink your fingers into her hair and gently tangle your tongue with hers.  ' );
		if( CoC.player.cor < 60 ) {
			MainView.outputText( 'The taste of cum glazing her apple breath is off-putting but what did you really expect?' );
		} else {
		...
		//Repeat:;
			...
			MainView.outputText( '"<i>That tastes better every time you make it [Master],</i>" she purrs when you break away, gazing into your eyes lovingly.  "<i>You should go into business with it.</i>"  You grin at the idea and say who knows?  Maybe Mareth will eventually behave well enough to appreciate it as much as she does.  "<i>I long for that day every moment of every day,</i>" your slave replies fervently, stroking the hand you\'re holding her face with.' );
	...
	Kelly.prototype.kellyBJsAhoy = function() {
		...
		if( !CoC.player.isTaur() ) {
			...
			if( CoC.player.hasVagina() ) {
				MainView.outputText( '  You\'re in no hurry, and with a slight roll of your [hips] you direct her to start at the back and work her way up.' );
			}
			//Vagina and balls: ;
			if( CoC.player.balls > 0 && CoC.player.hasVagina() ) {
				...
				if( CoC.player.ballSize <= 8 ) {
					...
					if( CoC.player.balls > 2 ) {
						MainView.outputText( ', then the third, then the fourth' );
					}
					MainView.outputText( ', then back to the first one, a slow and sensual repetition.  She moans intermittently as she does it, and you\'re not sure if it\'s intentionally to send delightful shivers of sensation through your scrotum and up the spine of [eachCock], or it\'s simply because the sheer degradation of the act fills her with deep, shameful lust.  Supporting her soft, ' );
					if( this.pregnancy.isPregnant ) {
						MainView.outputText( 'milk-laden ' );
					}
					MainView.outputText( 'breasts whilst slavishly polishing the [balls] of the ' + CoC.player.mf( 'man', 'woman' ) + ' who made her into, well, a ball-licking bitch.  Whatever the cause the result is the same: pleasure thrums through your groin and up your [cock biggest], and you close your eyes, lost to the sensations of her skillful tongue.' );
				}
				//[Huge balls:;
				else {
					...
					MainView.outputText( ', then back to the first one, a slow and sensual repetition.  The warm air is punctuated with desperate \'ahh, ahn, ahh\'s as she surrenders herself to the deep, shameful lust of the act, supporting her soft' );
					if( this.pregnancy.isPregnant ) {
						MainView.outputText( ', milk laden' );
					}
					MainView.outputText( ' breasts whilst slavishly polishing the [balls] of the ' + CoC.player.mf( 'man', 'woman' ) + ' who made her into, well, a ball-licking bitch.' );
					//[Dominika dreams:;
					if( CoC.flags[ kFLAGS.NUMBER_OF_TIMES_MET_SCYLLA ] > 0 && CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00150 ] > 0 ) {
						MainView.outputText( '  You are gripped by a sudden but thrilling velvet-edged sense of déjà vu.  Has this scene not repeated somewhere else?' );
					}
				}
			}
			//Vagina:;
			...
			//Balls:;
				...
				//[Normal balls:];
				if( CoC.player.ballSize <= 8 ) {
					MainView.outputText( 'After curling her tongue here and there over each sensitive orb, bathing them in saliva, she envelops each one in her mouth, sucking gently first one, then the second' );
					if( CoC.player.balls >= 4 ) {
						MainView.outputText( ', then the third, then the fourth' );
					}
					MainView.outputText( ', then back to the first one, a slow and sensual repetition.  She moans intermittently as she does it, and you\'re not sure if it\'s intentionally to send delightful shivers of sensation through your scrotum and up the spine of [eachCock], or it\'s simply because the sheer degradation of the act, supporting her soft, ' );
					if( this.pregnancy.isPregnant ) {
						MainView.outputText( 'milk-laden ' );
					}
					MainView.outputText( 'breasts whilst slavishly polishing the [balls] of the ' + CoC.player.mf( 'man', 'woman' ) + ' who made her into, well, a ball-licking bitch, fills her with deep, shameful lust.  Whatever the cause, the result is the same: pleasure thrums through your groin and up your [cock biggest], and you close your eyes, lost to the sensations of her skillful tongue.' );
				}
				//Huge balls:;
				else {
					MainView.outputText( 'After curling her tongue here and there over each sensitive orb, bathing them in saliva, she attempts to envelop one in her mouth.  She can\'t though; your testicles, bulging and tight with arousal, are simply too big.  After a short pause she goes back to licking them, lapping at their surface tenderly, licking all around each one for every trace of salt and musk, wetly caressing first one, then the second' );
					if( CoC.player.balls >= 4 ) {
						MainView.outputText( ', then the third, then the fourth' );
					}
					MainView.outputText( ', then back to the first one, a slow and sensual repetition.  The warm air is punctuated with desperate \'ahh, ahn, ahh\'s as she surrenders herself to the deep, shameful lust of the act, supporting her soft ' );
			...
			if( CoC.player.biggestCockLength() > 15 ) {
				MainView.outputText( 'It\'s a long way up and the effect is like a lift slowly rising, turning on a glow of lights on each level as it goes.  ' );
			}
			MainView.outputText( 'Finally she reaches your [cockHead biggest], and after resting her lips on your crown for a short time, moving them ever-so-gently to tantalize the most sensitive part of your bulging erection, she opens wide and envelops you, gripping the end of your dick in a soft, wet cave of shifting pleasure.  She slowly impales herself on it, her plump lips encapsulating more and more of your dick in sucking mouth flesh, and then it recedes, eventually even retreating from the crown as she goes back to licking and kissing the length.' );
			...
			MainView.outputText( '  Finally you are spent.' );
		}
		//Centaur:;
		else {
			...
			MainView.outputText( '  Finally, after about a minute of this tender torture, you hear her groan from deep within her throat.' );
			MainView.outputText( '"<i>Please [Master], let me swallow you, let me taste your strength, let me feel like a slut should,</i>" she begs. "<i>I\'ll make it so good for you you\'ll never want anyone else, just like I will never want anyone else.  Just... let me drink your cum!</i>"' );
			...
			if( CoC.player.cockTotal() > 1 || CoC.player.hasVagina() ) {
				MainView.outputText( '  You\'re in no hurry, and with a slight roll of your [hips] you direct her to start at the front and work her way down.' );
			}
			//Vagina and balls:;
			if( CoC.player.hasVagina() && CoC.player.balls > 0 ) {
				...
				if( CoC.player.ballSize <= 8 ) {
					...
					MainView.outputText( ' breasts whilst slavishly polishing the [balls] of the centaur who made her into, well, a ball-licking bitch.  Whatever the cause the result is the same.  Pleasure thrums through your groin and up your [cock biggest] and you close your eyes, lost to the sensations of her skillful tongue.' );
				}
				//[Huge balls:;
				else {
					MainView.outputText( '\n\nAfter curling her tongue here and there over each sensitive orb, bathing them in saliva, she attempts to envelop one in her mouth.  She can\'t though - your testicles, bulging and tight with arousal, are simply too big.  After a short pause she goes back to licking them, lapping at their surface tenderly, licking all around each one for every trace of salt and musk, wetly caressing first one, then the second' );
					...
					MainView.outputText( ' breasts whilst slavishly polishing the [balls] of the centaur who made her into, well, a ball-licking bitch.' );
					if( CoC.flags[ kFLAGS.NUMBER_OF_TIMES_MET_SCYLLA ] > 0 && CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00150 ] > 0 ) {
						MainView.outputText( '  You are gripped by a sudden but thrilling, velvet-edged sense of déjà vu.  Has this scene not repeated somewhere else?' );
					}
				}
			...
			//Balls: ;
				...
				if( CoC.player.ballSize <= 8 ) {
					...
					MainView.outputText( ' breasts whilst slavishly polishing the [balls] of the centaur who made her into, well, a ball-licking bitch, fills her with deep, shameful lust.  Whatever the cause the result is the same; pleasure thrums through your groin and up your [cock biggest] and you close your eyes, lost to the sensations of her skillful tongue.' );
				}
				//[Huge balls:;
				else {
					MainView.outputText( '  After curling her tongue here and there over each sensitive orb, bathing them in saliva, she attempts to envelop one in her mouth.  She can\'t though - your testicles, bulging and tight with arousal, are simply too big.  After a short pause, she goes back to licking them, lapping at their surface tenderly, licking all around each one for every trace of salt and musk, wetly caressing first one, then the second' );
			...
			//[Cock >15 inches:;
			if( CoC.player.biggestCockLength() > 15 ) {
				MainView.outputText( '  It\'s a long way down and the effect is like a lift slowly descending, turning on a glow of lights on each level as it goes.' );
			}
			MainView.outputText( '  Finally she reaches your [cockHead biggest], and after resting her lips on your crown for a short time, moving them ever-so-gently to tantalize the most sensitive part of your bulging erection, she opens wide and envelops you, gripping the end of your dick in a soft, wet cave of shifting pleasure.  She slowly impales herself on it, her plump lips encapsulating more and more of your dick in sucking mouth flesh... and then it recedes, eventually even retreating from the crown as she goes back to licking and kissing the length.' );
			...
			//[Cock >12:;
			if( CoC.player.biggestCockLength() > 12 ) {
				MainView.outputText( '  She can\'t take all of your dick but she tries her best, thrusting her head down as she crams her mouth with your thick hardness until you are touching her tonsils.  Lust crowding you now, you thrust your back haunches forward, forcing her further down your [cock biggest], gently but purposefully pushing further into her tight throat with each thrust.  You manage to hold back on your urge to simply face fuck her as hard as you can, instead moving her back and forth on your length as sensually as you can, drawing your dick back from the delicious tightness of her throat so she can breathe deep and spend a moment worshipping your [cockHead biggest] with her tongue and lips, eliciting a moaned “mmm” from her each time before penetrating her gullet again.' );
			}
			//[Cock <12: ;
			else {
				MainView.outputText( '  Her mouth is a good fit for your dick and you soon find your limit, touching the back of her throat with each gentle thrust.  You manage to hold back on your urge to simply face fuck her as hard as you can, instead moving your haunches back and forth as sensually as you can, drawing your dick back to the entrance of her mouth so she can worship your [cockHead biggest] with her tongue and lips, eliciting a moaned “mmm” from her each time before thrusting deep into her maw again.' );
			}
			...
			MainView.outputText( ', you pull out and point your [cock biggest] downwards.  She \'aww\'s in deep disappointment as the dick is torn away from her, but you shush her.' );
			...
			if( CoC.player.hasVagina() ) {
				MainView.outputText( 'pussy-licking, ' );
			}
			if( CoC.player.balls > 0 ) {
				MainView.outputText( 'ball-sucking ' );
			}
			MainView.outputText( 'cock addict, you will have to do a good job of catching it on those whore pillows of yours.</i>"  With that, you thrust your hindquarters forward.  Her breathy groan is what you hear as orgasm clenches you.' );
			MainView.outputText( '\n\nYour cock slit dilates and you spurt out rope after rope of jizz' );
			if( CoC.flags[ kFLAGS.KELLY_BONUS_BOOB_ROWS ] > 0 ) {
				MainView.outputText( ' onto all four of her tits' );
			}
			MainView.outputText( '.  The sensation of deep release is ecstatic, the sound of your cock cream spattering onto your slave\'s softness delicious.' );
			//[Vagina:;
			if( CoC.player.hasVagina() ) {
				MainView.outputText( '  Your female sex clenches and orgasms in sympathy, and you feel fluid dribble down your [hips] as you continue to paint Kelly.  Your only dim regret is that there\'s no way of soaking her with that, too.' );
			}
			MainView.outputText( '\n\nFinally you are spent.  You clop around to admire your handiwork.' );
			//Low cum:;
			if( CoC.player.cumQ() < 750 ) {
				MainView.outputText( '  Kelly has done a sterling job of catching your load; you\'ve frosted her boobs with long strokes quite convincingly.' );
			} else {
				MainView.outputText( '  As ever, your virility has gotten the better of you.  Kelly looks like she\'s in an ecstatic trance.  You\'ve completely pasted not just ' );
				if( CoC.flags[ kFLAGS.KELLY_BONUS_BOOB_ROWS ] > 0 ) {
					MainView.outputText( 'all four of ' );
				}
				MainView.outputText( 'her boobs with alabaster ooze, but her stomach, face and hair.  She\'s holding a reservoir of the stuff in her cleavage.' );
			}
		}
		//[merge];
		...
		if( !CoC.player.isTaur() ) {
			MainView.outputText( ', still clutching your pleasantly aching prick' );
		}
		MainView.outputText( ', saying you know she is, but it\'s a lot more pleasing to you to see her get her fix this way.  Quickly cottoning on, the centaur stares at you with her lustful green eyes as she lifts a cum-spattered boob to her mouth.' );
		...
		MainView.outputText( '  She leaves a trail of cum leaking down a nipple until at last, closing her eyes, she tenses and orgasms with a muffled sigh as her wet tongue pushes over her erect nub and guides the last of your fix into her mouth.' );
		//[Libido <50: ;
