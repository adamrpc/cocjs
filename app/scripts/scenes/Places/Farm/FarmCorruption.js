...
angular.module( 'cocjs' ).run( function( SceneLib, MainView, $log, AppearanceDefs, ConsumableLib, StatusAffects, Appearance, Utils, Descriptors, CoC, kFLAGS, EngineCore ) {
	...
	FarmCorruption.prototype.collectTheGoodies = function() {
		...
		if( CoC.flags[ kFLAGS.FARM_SUCCUMILK_STORED ] > 0 ) {
			EngineCore.outputText( '<b>' + CoC.flags[ kFLAGS.FARM_SUCCUMILK_STORED ] + 'x ' );
		...
		if( CoC.flags[ kFLAGS.FARM_INCUDRAFT_STORED ] > 0 ) {
			EngineCore.outputText( '<b>' + CoC.flags[ kFLAGS.FARM_INCUDRAFT_STORED ] + 'x ' );
		...
		if( CoC.flags[ kFLAGS.FARM_EGG_STORED ] > 0 ) {
			EngineCore.outputText( '<b>' + CoC.flags[ kFLAGS.FARM_EGG_STORED ] + 'x ' );
			EngineCore.outputText( 'large ' + CoC.flags[ kFLAGS.FOLLOWER_PRODUCTION_SOPHIE_COLORCHOICE ] + ' egg' );
		...
		if( CoC.flags[ kFLAGS.FARM_CONTRACEPTIVE_STORED ] > 0 ) {
			EngineCore.outputText( '<b>' + CoC.flags[ kFLAGS.FARM_CONTRACEPTIVE_STORED ] + 'x ' );
	...
	FarmCorruption.prototype.rootScene = function() {
		...
		if( !this.whitneyCorrupt() ) {
			...
			if( EngineCore.silly() ) {
				EngineCore.outputText( CoC.player.mf( 'You feel an irresistible hankering for a cigar.', 'You feel an irresistible hankering for a cigarette holder.' ) );
			}
			if( CoC.flags[ kFLAGS.FARM_UPGRADES_REFINERY ] === 1 ) {
				EngineCore.outputText( ' A large machine, bulky and rotund with a conical top, has been built into the milking barn. Fat pipes crawl up onto the roof from it like metal ivy, and white smoke billows busily out of its whistle chimney.' );
			}
	...
	FarmCorruption.prototype.whitneyAppearanceCorrupt = function() {
		...
		if( this.whitneyHasTattoo() ) {
			...
			if( CoC.flags[ kFLAGS.WHITNEY_TATTOO_COLLARBONE ] !== 0 ) {
				EngineCore.outputText( CoC.flags[ kFLAGS.WHITNEY_TATTOO_COLLARBONE ] + '\n' );
			}
			if( CoC.flags[ kFLAGS.WHITNEY_TATTOO_SHOULDERS ] !== 0 ) {
				EngineCore.outputText( CoC.flags[ kFLAGS.WHITNEY_TATTOO_SHOULDERS ] + '\n' );
			}
			if( CoC.flags[ kFLAGS.WHITNEY_TATTOO_LOWERBACK ] !== 0 ) {
				EngineCore.outputText( CoC.flags[ kFLAGS.WHITNEY_TATTOO_LOWERBACK ] + '\n' );
			}
			if( CoC.flags[ kFLAGS.WHITNEY_TATTOO_BUTT ] !== 0 ) {
				EngineCore.outputText( CoC.flags[ kFLAGS.WHITNEY_TATTOO_BUTT ] + '\n' );
			}
		}
		EngineCore.outputText( '\n' );
		if( !this.whitneyDefurred() ) {
