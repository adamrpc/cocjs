...
angular.module( 'cocjs' ).run( function( SceneLib, MainView, $log, AppearanceDefs, ConsumableLib, StatusAffects, Utils, Descriptors, CoC, kFLAGS, EngineCore ) {
	...
	FarmCorruption.prototype.corruptFollowers = function() {
		...
		if( SceneLib.ceraphScene.ceraphIsFollower() ) {
			count++;
		}
	...
	FarmCorruption.prototype.whitneyCorruption = function( mod ) {
		...
		if( mod !== 0 ) {
			CoC.flags[ kFLAGS.WHITNEY_CORRUPTION ] += mod;
			// Track highest corruption value;
			if( CoC.flags[ kFLAGS.WHITNEY_CORRUPTION ] > CoC.flags[ kFLAGS.WHITNEY_CORRUPTION_HIGHEST ] ) {
				CoC.flags[ kFLAGS.WHITNEY_CORRUPTION_HIGHEST ] = CoC.flags[ kFLAGS.WHITNEY_CORRUPTION ];
			}
		}
		// Cap the values into the appropriate range;
		if( CoC.flags[ kFLAGS.FARM_CORRUPTION_APPROACHED_WHITNEY ] === 0 && CoC.flags[ kFLAGS.WHITNEY_CORRUPTION ] > 30 ) {
			CoC.flags[ kFLAGS.WHITNEY_CORRUPTION ] = 30;
			$log.debug( 'Whitney at 30 corruption clamp.' );
		} else if( CoC.flags[ kFLAGS.WHITNEY_LEAVE_0_60 ] === 0 && CoC.flags[ kFLAGS.WHITNEY_CORRUPTION ] > 60 ) {
			CoC.flags[ kFLAGS.WHITNEY_CORRUPTION ] = 60;
			$log.debug( 'Whitney at 60 corruption clamp.' );
		} else if( CoC.flags[ kFLAGS.WHITNEY_LEAVE_61_90 ] === 0 && CoC.flags[ kFLAGS.WHITNEY_CORRUPTION ] > 90 ) {
			CoC.flags[ kFLAGS.WHITNEY_CORRUPTION ] = 90;
			$log.debug( 'Whitney at 90 corruption clamp.' );
		} else if( CoC.flags[ kFLAGS.WHITNEY_MENU_91_119 ] === 0 && CoC.flags[ kFLAGS.WHITNEY_CORRUPTION ] > 119 ) {
			CoC.flags[ kFLAGS.WHITNEY_CORRUPTION ] = 119;
			$log.debug( 'Whitney at 119 corruption clamp' );
		}
		// Clamp values to valid min/max;
		if( CoC.flags[ kFLAGS.WHITNEY_CORRUPTION ] < 0 ) {
			CoC.flags[ kFLAGS.WHITNEY_CORRUPTION ] = 0;
		}
		if( CoC.flags[ kFLAGS.WHITNEY_CORRUPTION ] >= 120 ) {
			CoC.flags[ kFLAGS.WHITNEY_CORRUPTION ] = 120;
		}
		$log.debug( 'Whitney corruption changed by ' + String( mod ) );
		$log.info( 'Whitney corruption now at ' + String( CoC.flags[ kFLAGS.WHITNEY_CORRUPTION ] ) );
		return CoC.flags[ kFLAGS.WHITNEY_CORRUPTION ];
	};
	FarmCorruption.prototype.whitneyCorrupt = function() {
		if( CoC.flags[ kFLAGS.WHITNEY_CORRUPTION_COMPLETE ] > 0 ) {
			return true;
		}
		return false;
	};
	FarmCorruption.prototype.whitneyDom = function() {
		if( CoC.flags[ kFLAGS.WHITNEY_DOM ] === 1 ) {
			return true;
		}
		return false;
	};
	FarmCorruption.prototype.whitneyDefurred = function() {
		if( CoC.flags[ kFLAGS.WHITNEY_DEFURRED ] === 1 ) {
			return true;
		}
		return false;
	};
	FarmCorruption.prototype.whitneyHasTattoo = function() {
		...
		if( CoC.flags[ kFLAGS.WHITNEY_TATTOO_SHOULDERS ] !== 0 ) {
			return true;
		}
		if( CoC.flags[ kFLAGS.WHITNEY_TATTOO_LOWERBACK ] !== 0 ) {
			return true;
		}
		if( CoC.flags[ kFLAGS.WHITNEY_TATTOO_BUTT ] !== 0 ) {
			return true;
		}
		return false;
	};
	...
	FarmCorruption.prototype.whitneyMaxedOralTraining = function() {
		if( CoC.flags[ kFLAGS.WHITNEY_ORAL_TRAINING ] === 100 ) {
			return true;
		}
		return false;
	};
	...
	FarmCorruption.prototype.updateFarmCorruption = function() {
		...
		if( CoC.flags[ kFLAGS.WHITNEY_DISABLED_FOR_DAY ] === 1 ) {
			CoC.flags[ kFLAGS.WHITNEY_DISABLED_FOR_DAY ] = 2;
		}
		// Process queued farm upgrades;
		// Breastmilkers;
		if( CoC.flags[ kFLAGS.QUEUE_BREASTMILKER_UPGRADE ] !== 0 && CoC.player.hasKeyItem( 'Breast Milker - Installed At Whitney\'s Farm' ) < 0 ) {
			CoC.flags[ kFLAGS.QUEUE_BREASTMILKER_UPGRADE ] = 0;
			CoC.player.createKeyItem( 'Breast Milker - Installed At Whitney\'s Farm', 0, 0, 0, 0 );
		}
		// Cockmilkers;
		if( CoC.flags[ kFLAGS.QUEUE_COCKMILKER_UPGRADE ] !== 0 && CoC.player.hasKeyItem( 'Cock Milker - Installed At Whitney\'s Farm' ) < 0 ) {
			CoC.flags[ kFLAGS.QUEUE_COCKMILKER_UPGRADE ] = 0;
			CoC.player.createKeyItem( 'Cock Milker - Installed At Whitney\'s Farm', 0, 0, 0, 0 );
		}
		// Contraceptives;
		if( CoC.flags[ kFLAGS.QUEUE_CONTRACEPTIVE_UPGRADE ] !== 0 ) {
			CoC.flags[ kFLAGS.QUEUE_CONTRACEPTIVE_UPGRADE ]++;
			if( CoC.flags[ kFLAGS.QUEUE_CONTRACEPTIVE_UPGRADE ] > 7 ) {
				CoC.flags[ kFLAGS.FARM_UPGRADES_CONTRACEPTIVE ] = 1;
				CoC.flags[ kFLAGS.QUEUE_CONTRACEPTIVE_UPGRADE ] = 0;
			}
		}
		// Refinery;
		if( CoC.flags[ kFLAGS.QUEUE_REFINERY_UPGRADE ] !== 0 ) {
			CoC.flags[ kFLAGS.QUEUE_REFINERY_UPGRADE ]++;
			if( CoC.flags[ kFLAGS.QUEUE_REFINERY_UPGRADE ] > 3 ) {
				CoC.flags[ kFLAGS.FARM_UPGRADES_REFINERY ] = 1;
				CoC.flags[ kFLAGS.QUEUE_REFINERY_UPGRADE ] = 0;
			}
		}
		// Milktank;
		if( CoC.flags[ kFLAGS.QUEUE_MILKTANK_UPGRADE ] !== 0 ) {
			CoC.flags[ kFLAGS.QUEUE_MILKTANK_UPGRADE ]++;
			if( CoC.flags[ kFLAGS.QUEUE_MILKTANK_UPGRADE ] > 3 ) {
				CoC.flags[ kFLAGS.QUEUE_MILKTANK_UPGRADE ] = 0;
				CoC.flags[ kFLAGS.FARM_UPGRADES_MILKTANK ] = 1;
				CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_BATH_GIRL ] = 1;
				CoC.flags[ kFLAGS.MILK_SIZE ] = 0;
			}
		}
		// Branding;
		if( CoC.flags[ kFLAGS.QUEUE_BRANDING_UPGRADE ] !== 0 ) {
			CoC.flags[ kFLAGS.QUEUE_BRANDING_UPGRADE ]++;
			if( CoC.flags[ kFLAGS.QUEUE_BRANDING_UPGRADE ] > 2 ) {
				CoC.flags[ kFLAGS.QUEUE_BRANDING_UPGRADE ] = 0;
				CoC.flags[ kFLAGS.FARM_CORRUPTION_BRANDING_MENU_UNLOCKED ] = 1;
				CoC.flags[ kFLAGS.QUEUE_BRANDING_AVAILABLE_TALK ] = 1;
			}
		}
		// Orgyroom;
		if( CoC.flags[ kFLAGS.QUEUE_ORGYROOM_UPGRADE ] !== 0 ) {
			CoC.flags[ kFLAGS.QUEUE_ORGYROOM_UPGRADE ]++;
			if( CoC.flags[ kFLAGS.QUEUE_ORGYROOM_UPGRADE ] > 7 ) {
				CoC.flags[ kFLAGS.QUEUE_ORGYROOM_UPGRADE ] = 0;
				CoC.flags[ kFLAGS.FARM_UPGRADES_ORGYROOM ] = 2;
			}
		}
		...
		// Izmael if (CoC.flags[kFLAGS.FOLLOWER_AT_FARM_IZMA] === 1) modValue += 1;;
		// Bimbo if (CoC.flags[kFLAGS.FOLLOWER_AT_FARM_ISABELLA] === 1) modValue += 2;;
		...
		if( CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_HOLLI ] > 0 ) {
			modValue += 4;
		}
		if( CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_KELLY ] === 1 ) {
			modValue += 2;
		}

		this.whitneyCorruption( modValue );
		// Now figure out how many gems we're gonna get for the day;
		CoC.flags[ kFLAGS.FARM_CORRUPTION_GEMS_WAITING ] += this.farmValue();
		// If Amily is producing, stack up some milks to gib;
		if( CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_AMILY ] === 1 && CoC.flags[ kFLAGS.FOLLOWER_PRODUCTION_AMILY ] === 1 && CoC.flags[ kFLAGS.FARM_UPGRADES_REFINERY ] === 1 ) {
			if( CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_AMILY_GIBS_MILK ] === 0 ) {
				CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_AMILY_GIBS_MILK ] = 1;
			}
			CoC.flags[ kFLAGS.FARM_SUCCUMILK_STORED ]++;
		}
		// If jojo is producing, stack up some drafts to gib;
		if( CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_JOJO ] === 1 && CoC.flags[ kFLAGS.FOLLOWER_PRODUCTION_JOJO ] === 1 && CoC.flags[ kFLAGS.FARM_UPGRADES_REFINERY ] === 1 ) {
			if( CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_JOJO_GIBS_DRAFT ] === 0 ) {
				CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_JOJO_GIBS_DRAFT ] = 1;
			}
			CoC.flags[ kFLAGS.FARM_INCUDRAFT_STORED ]++;
		...
		if( CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_SOPHIE ] > 0 && CoC.flags[ kFLAGS.FOLLOWER_PRODUCTION_SOPHIE ] === 1 ) {
			if( CoC.flags[ kFLAGS.FARM_EGG_STORED ] === 0 ) {
				CoC.flags[ kFLAGS.FARM_EGG_COUNTDOWN ]--;
				if( CoC.flags[ kFLAGS.FARM_EGG_COUNTDOWN ] === 0 ) {
					CoC.flags[ kFLAGS.FARM_EGG_STORED ] = 1;
		...
		if( CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_VAPULA ] === 1 && CoC.flags[ kFLAGS.FOLLOWER_PRODUCTION_VAPULA ] === 1 ) {
			...
			if( CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_VAPULA_GIBS_MILK ] === 0 ) {
				CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_VAPULA_GIBS_MILK ] = 1;
			}
		}
		// Item caps;
		if( CoC.flags[ kFLAGS.FARM_SUCCUMILK_STORED ] > 5 ) {
			CoC.flags[ kFLAGS.FARM_CORRUPTION_GEMS_WAITING ] += (CoC.flags[ kFLAGS.FARM_SUCCUMILK_STORED ] - 5) * 3; // 3 extra gems per succumilk
			CoC.flags[ kFLAGS.FARM_SUCCUMILK_STORED ] = 5;
		}
		if( CoC.flags[ kFLAGS.FARM_INCUDRAFT_STORED ] > 5 ) {
			CoC.flags[ kFLAGS.FARM_CORRUPTION_GEMS_WAITING ] += (CoC.flags[ kFLAGS.FARM_INCUDRAFT_STORED ] - 5) * 5; // 5 extra gems per incudraft (less of them produced etc)
			CoC.flags[ kFLAGS.FARM_INCUDRAFT_STORED ] = 5;
		}
		...
		if( CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_CERAPH ] > 0 ) {
			CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_CERAPH ]++;
			if( CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_CERAPH ] > 7 ) {
				CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_CERAPH ] = -1;
			}
		}
		// If Holli is doing her thing;
		if( CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_HOLLI ] > 0 ) {
			CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_HOLLI ]++;
			if( CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_HOLLI ] > 20 ) {
				CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_HOLLI ] = -1;
			}
		}
		// Contraceptives;
		if( CoC.flags[ kFLAGS.FARM_UPGRADES_CONTRACEPTIVE ] === 1 ) {
			if( CoC.flags[ kFLAGS.FARM_CONTRACEPTIVE_STORED ] === 0 ) {
				CoC.flags[ kFLAGS.FARM_CONTRACEPTIVE_STORED ] = 1;
			}
		}
		// Increment days since last paid out;
		CoC.flags[ kFLAGS.FARM_CORRUPTION_DAYS_SINCE_LAST_PAYOUT ] += 1;
		return modValue;
	};
	FarmCorruption.prototype.collectTheGoodies = function() {
		...
		if( CoC.flags[ kFLAGS.FARM_CORRUPTION_DAYS_SINCE_LAST_PAYOUT ] >= 7 ) {
			MainView.outputText( 'You stroll over to the big rock at the edge of the pepper patch, smiling as you see a small burlap sack shoved underneath a fold in the stone. You scoop it up, open it and count out ' + CoC.flags[ kFLAGS.FARM_CORRUPTION_GEMS_WAITING ] + ' gems.\n\n' );
			if( this.farmValue() < 25 ) {
				MainView.outputText( ' You frown; it seems like a feeble amount for such a large operation. Perhaps you could talk to Whitney about that.\n\n' );
			}
		...
		if( CoC.flags[ kFLAGS.FARM_SUCCUMILK_STORED ] > 0 ) {
			...
			if( CoC.flags[ kFLAGS.FARM_SUCCUMILK_STORED ] === 1 ) {
				MainView.outputText( 'vial of Succubi milk.' );
			} else {
				MainView.outputText( 'vials of Succubi milk.' );
			}
			MainView.outputText( '</b>\n' );
			EngineCore.addButton( 0, this.getItemObj( kFLAGS.FARM_SUCCUMILK_STORED ).shortName, this, this.takeItems, kFLAGS.FARM_SUCCUMILK_STORED );
		}
		if( CoC.flags[ kFLAGS.FARM_INCUDRAFT_STORED ] > 0 ) {
			MainView.outputText( '<b>' + CoC.flags[ kFLAGS.FARM_INCUDRAFT_STORED ] + 'x ' );
			if( CoC.flags[ kFLAGS.FARM_INCUDRAFT_STORED ] === 1 ) {
				MainView.outputText( 'vial of Incubi draft.' );
			} else {
				MainView.outputText( 'vials of Incubi draft.' );
			}
			MainView.outputText( '</b>\n' );
			EngineCore.addButton( 1, this.getItemObj( kFLAGS.FARM_INCUDRAFT_STORED ).shortName, this, this.takeItems, kFLAGS.FARM_INCUDRAFT_STORED );
		}
		if( CoC.flags[ kFLAGS.FARM_EGG_STORED ] > 0 ) {
			MainView.outputText( '<b>' + CoC.flags[ kFLAGS.FARM_EGG_STORED ] + 'x ' );
			MainView.outputText( 'large ' + CoC.flags[ kFLAGS.FOLLOWER_PRODUCTION_SOPHIE_COLORCHOICE ] + ' egg' );
			if( CoC.flags[ kFLAGS.FARM_EGG_STORED ] > 1 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( '.' );
			MainView.outputText( '</b>\n' );
			EngineCore.addButton( 2, this.getItemObj( kFLAGS.FARM_EGG_STORED ).shortName, this, this.takeItems, kFLAGS.FARM_EGG_STORED );
		}
		if( CoC.flags[ kFLAGS.FARM_CONTRACEPTIVE_STORED ] > 0 ) {
			MainView.outputText( '<b>' + CoC.flags[ kFLAGS.FARM_CONTRACEPTIVE_STORED ] + 'x ' );
			MainView.outputText( 'bundle of Herbal Contraceptive.' );
			MainView.outputText( '</b>\n' );
			EngineCore.addButton( 3, this.getItemObj( kFLAGS.FARM_CONTRACEPTIVE_STORED ).shortName, this, this.takeItems, kFLAGS.FARM_CONTRACEPTIVE_STORED );
		}
	...
	FarmCorruption.prototype.getItemObj = function( flag ) {
		...
		if( flag === kFLAGS.FARM_EGG_STORED ) {
			return SceneLib.sophieBimbo.eggTypes[ SceneLib.sophieBimbo.eggColors.indexOf( CoC.flags[ kFLAGS.FOLLOWER_PRODUCTION_SOPHIE_COLORCHOICE ] ) ];
		}
	...
	FarmCorruption.prototype.takeItems = function( flag ) {
		...
		CoC.flags[ flag ]--;
		SceneLib.inventory.takeItem( item, this.afterTakeItems );
	};
	...
	FarmCorruption.prototype.farmValue = function() {
		...
		if( CoC.flags[ kFLAGS.WHITNEY_CORRUPTION_COMPLETE ] === 1 ) {
			fValue *= 1.25;
		}
		return fValue;
	};
	FarmCorruption.prototype.takeoverPrompt = function() {
		if( CoC.flags[ kFLAGS.FARM_CORRUPTION_DISABLED ] === 1 ) {
			return false;
		}
		if( CoC.flags[ kFLAGS.FARM_CORRUPTION_STARTED ] === 1 ) {
			this.farmMenu();
		...
		if( CoC.player.cor >= 70 && this.corruptFollowers() >= 2 && CoC.player.level >= 12 ) {
			if( CoC.flags[ kFLAGS.FARM_CORRUPT_PROMPT_DISPLAY ] === 0 ) {
				...
				CoC.flags[ kFLAGS.FARM_CORRUPT_PROMPT_DISPLAY ]++;
				return false;
			}
		}
		return false;
	...
	FarmCorruption.prototype.takeoverPromptKelly = function() {
		...
		if( SceneLib.amilyScene.amilyCorrupt() || SceneLib.jojoScene.campCorruptJojo() ) {
			MainView.outputText( ' and the rodent slut' );
			if( SceneLib.amilyScene.amilyCorrupt() && SceneLib.jojoScene.campCorruptJojo() ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ' before him' );
		}
		MainView.outputText( '?' );
	};
	FarmCorruption.prototype.takeoverPromptMarbleRape = function() {
		...
		if( SceneLib.amilyScene.amilyCorrupt() || SceneLib.jojoScene.campCorruptJojo() ) {
			...
			MainView.outputText( ' before her' );
		}
		MainView.outputText( '?' );
	};
	...
	FarmCorruption.prototype.takeoverPromptMerge = function( firstTime ) {
		CoC.flags[ kFLAGS.FARM_CORRUPT_PROMPT_DISPLAY ] = 1;
		if( firstTime ) {
	...
	FarmCorruption.prototype.takeoverPromptNow = function() {
		MainView.clearOutput();
		MainView.outputText( 'You stride down to the farm and leap over a gate. You move casually, swaggering towards the pepper field with no obvious intent. When Whitney spots you and slowly stands up from her weeding, you raise your hand in friendly greeting' );
		if( CoC.player.weaponName !== 'fists' ) {
			MainView.outputText( ', and to demonstrate your peacefulness, you theatrically hold up your [weapon] and then discard it with a careless swing of your arm.' );
		} else {
			MainView.outputText( '.' );
		}
		...
		if( CoC.flags[ kFLAGS.KELT_BREAK_LEVEL ] >= 4 || CoC.player.findStatusAffect( StatusAffects.MarbleRapeAttempted ) >= 0 ) {
			...
			if( CoC.flags[ kFLAGS.KELT_BREAK_LEVEL ] >= 4 ) {
				MainView.outputText( ' Kelt' );
			} else {
				MainView.outputText( ' Marble' );
			}
	...
	FarmCorruption.prototype.rootScene = function() {
		...
		if( CoC.flags[ kFLAGS.WHITNEY_DISABLED_FOR_DAY ] === 2 ) {
			...
			EngineCore.doNext( this, this.dontDeFurDoge );
			return;
		}
		// Whitney not corrupt;
		if( !this.whitneyCorrupt() ) {
			MainView.outputText( 'You stand on the rise you’ve taken to using to look down on the farm which you are invested in.' );
			if( CoC.flags[ kFLAGS.FARM_UPGRADES_REFINERY ] === 1 ) {
				MainView.outputText( ' A large machine, bulky and rotund with a conical top, has been built into the milking barn. Fat pipes crawl up onto the roof from it like metal ivy, and white smoke billows busily out of its whistle chimney.' );
			}

			if( CoC.flags[ kFLAGS.FARM_UPGRADES_CONTRACEPTIVE ] === 1 ) {
				MainView.outputText( ' Next to the pepper patch another crop has been planted, a field of verdant green shrubs. Their thin stems bob idly in the breeze.' );
			}

			if( CoC.flags[ kFLAGS.QUEUE_ORGYROOM_UPGRADE ] > 1 ) {
			...
			MainView.outputText( 'You stand on your rise and take in your slave farm. ' );
			if( EngineCore.silly() ) {
				MainView.outputText( CoC.player.mf( 'You feel an irresistible hankering for a cigar.', 'You feel an irresistible hankering for a cigarette holder.' ) );
			}
			if( CoC.flags[ kFLAGS.FARM_UPGRADES_REFINERY ] === 1 ) {
				MainView.outputText( ' A large machine, bulky and rotund with a conical top, has been built into the milking barn. Fat pipes crawl up onto the roof from it like metal ivy, and white smoke billows busily out of its whistle chimney.' );
			}
	...
	FarmCorruption.prototype.farmMenu = function() {
		...
		if( CoC.player.findStatusAffect( StatusAffects.Kelt ) >= 0 && CoC.player.findStatusAffect( StatusAffects.KeltOff ) < 0 ) {
			if( CoC.flags[ kFLAGS.KELT_BREAK_LEVEL ] >= 4 ) {
				...
				EngineCore.addButton( 2, 'Kelt', this, this.keltAChangeInManagement );
			} else {
				EngineCore.addButton( 2, 'Kelt', SceneLib.kelly, SceneLib.kelly.breakingKeltOptions );
			}
		}
		...
		if( CoC.player.hasKeyItem( 'Cock Milker - Installed At Whitney\'s Farm' ) >= 0 && CoC.player.cockTotal() > 0 ) {
			EngineCore.addButton( 4, 'Milk Cock', SceneLib.farm, SceneLib.farm.cockPumping );
		}
		EngineCore.addButton( 5, 'Farm', this, this.corruptingTheFarmExplore );
		if( this.slavesAtFarm() ) {
			EngineCore.addButton( 6, 'Slaves', this, this.slavesAtFarmMenu );
		}
		if( this.loversAtFarm() ) {
			EngineCore.addButton( 7, 'Lovers', this, this.loversAtFarmMenu );
		}
		if( this.followersAtFarm() ) {
			EngineCore.addButton( 8, 'Followers', this, this.followersAtFarmMenu );
		}
	...
	FarmCorruption.prototype.corruptingTheFarmExplore = function() {
		...
		if( CoC.flags[ kFLAGS.FARM_CORRUPTION_DAYS_SINCE_LAST_PAYOUT ] >= 7 || CoC.flags[ kFLAGS.FARM_SUCCUMILK_STORED ] > 0 || CoC.flags[ kFLAGS.FARM_INCUDRAFT_STORED ] > 0 || CoC.flags[ kFLAGS.FARM_EGG_STORED ] > 0 || CoC.flags[ kFLAGS.FARM_CONTRACEPTIVE_STORED ] > 0 ) {
			EngineCore.addButton( 2, 'Collect', this, this.collectTheGoodies );
		}
	...
	FarmCorruption.prototype.keltAChangeInManagement = function() {
		MainView.clearOutput();
		MainView.outputText( '“<i>Hear there’s been a change in management,</i>” says Kelt, clopping to a halt in front of you. You confirm that that is the case. The big centaur looks at you thoughtfully. There’s something different in his dark eyes and rugged scowl than his usual wearied contempt. Grudging admiration?' );
		MainView.outputText( '“<i>Find it difficult to believe someone like you could put a bitch in her place,</i>” he says eventually. “<i>Perhaps you’ve got bigger balls than I thought you had.' );
		if( CoC.player.balls === 0 ) {
			MainView.outputText( ' So to speak, anyway.' );
		}
		MainView.outputText( '</i>” He snorts, and trots towards the butts. “<i>Just don’t expect me to treat you any different. As long as you’re getting free instruction from me, I’m the master and you’re the whelp. Got that?</i>”' );
		...
		MainView.outputText( '“<i>Do I look like I want or need your sloppy seconds? Godsdamn, I can only imagine what kind of pathetic creatures would have </i>you<i> lording it over them. What a joke! Now, are we going to go watch you fail miserably to hit a target from five yards or what?</i>”' );
		MainView.outputText( 'It’s better than you were expecting from him in all honesty. You take your bow out and silently follow him to the archery range.' );
		CoC.flags[ kFLAGS.KELT_TALKED_FARM_MANAGEMENT ] = 1;
		CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_KELLY ] = 1;
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	...
	FarmCorruption.prototype.slavesAtFarmMenu = function() {
		...
		if( CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_SOPHIE ] === 1 && SceneLib.sophieBimbo.bimboSophie() ) {
			EngineCore.addButton( 2, 'Sophie', SceneLib.sophieBimbo, SceneLib.sophieBimbo.approachBimboSophieInCamp );
		}
		if( CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_VAPULA ] === 1 ) {
			EngineCore.addButton( 3, 'Vapula', SceneLib.vapula, SceneLib.vapula.callSlaveVapula );
		}

		if( CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_LATEXY ] === 1 ) {
			EngineCore.addButton( 4, CoC.flags[ kFLAGS.GOO_NAME ], SceneLib.latexGirl, SceneLib.latexGirl.approachLatexy );
		}

		if( CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_BATH_GIRL ] === 1 ) {
			EngineCore.addButton( 5, CoC.flags[ kFLAGS.MILK_NAME ], SceneLib.milkWaifu, SceneLib.milkWaifu.milkyMenu );
		}
	...
	FarmCorruption.prototype.loversAtFarmMenu = function() {
		...
		if( CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_IZMA ] === 2 ) {
			EngineCore.addButton( 0, 'Izmael', null, EngineCore.gameOver );
		}

		if( CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_ISABELLA ] === 1 ) {
			EngineCore.addButton( 1, 'Isabella', SceneLib.isabellaFollowerScene, SceneLib.isabellaFollowerScene.callForFollowerIsabella );
		}
	...
	FarmCorruption.prototype.dogeNotCorruptYet = function() {
		...
		if( this.whitneyCorruption() < 30 && CoC.flags[ kFLAGS.WHITNEY_CORRUPTION_HIGHEST ] > 30 && CoC.flags[ kFLAGS.WHITNEY_CORRUPTION_0_30_DROP_MESSAGE ] === 0 ) {
			MainView.outputText( 'You hail Whitney as she hauls a bag of grain towards the storage barn. She entirely ignores you; there’s an expression of cold triumph on her face as she breezes past you, color high in her cheeks. You notice that she’s rediscovered her crossbow and has it strapped to her back. It hurts your eyes slightly to look at her, as if she’s standing directly in front of the sun. You scowl at her retreating back. You’ll have to do something about this.' );
			// [Plays once, reverts to standard message];
			CoC.flags[ kFLAGS.WHITNEY_CORRUPTION_0_30_DROP_MESSAGE ] = 1;
			if( CoC.flags[ kFLAGS.WHITNEY_LEAVE_0_60 ] === 0 ) {
				EngineCore.doNext( this, this.dogeNotCorruptLeaveFirstTime );
			} else {
				EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
			}
		...
		if( this.whitneyCorruption() <= 30 ) {
			CoC.flags[ kFLAGS.FARM_CORRUPTION_APPROACHED_WHITNEY ] = 1;
			MainView.outputText( 'You find Whitney sat in the cow field, hand milking one of her herd; her crossbow is propped up against a bucket. As you approach, she looks up sharply before slowly reaching for the weapon. You stop in your tracks, and she stops in kind, her hand frozen on the grip. You take a step forward and she picks it up, her finger finding the trigger, her expression entirely emotionless. You take several hasty steps back, she puts it down and goes back to milking peacefully.' );
		...
		if( this.whitneyCorruption() >= 31 && this.whitneyCorruption() <= 60 ) {
			if( CoC.flags[ kFLAGS.WHITNEY_MENU_31_60 ] === 0 ) {
				CoC.flags[ kFLAGS.WHITNEY_MENU_31_60 ] = 1;
				MainView.outputText( 'Once again you approach Whitney, this time as she is carrying full buckets of milk towards the bottling station. She sees you coming and quickens her step. You notice this time however she doesn’t have her crossbow, and this emboldens you.' );
				...
				// Subsequent 31-60: ;
				MainView.outputText( 'Whitney puts down her work as you stride towards her, resigned unhappiness in her expression.' );
				MainView.outputText( '\n\n“<i>What is it?</i>”' );
			}
		...
		if( this.whitneyCorruption() >= 61 && this.whitneyCorruption() <= 90 ) {
			if( CoC.flags[ kFLAGS.WHITNEY_MENU_61_90 ] === 0 ) {
				CoC.flags[ kFLAGS.WHITNEY_MENU_61_90 ] = 1;
				// 61-90 Farm Corruption;
				MainView.outputText( 'You find Whitney on the steps of the porch to her small house, adjoined to the storage barn. There’s a book next to her but she is simply sat drinking in the sun, her eyes closed. When she opens them and sees you standing there she smiles at you wanly; it’s the first time you’ve seen her do that for some time. There is something indefinable in her gaze and the long lines of her face: trepidation perhaps, coated patchily with coyness.' );
				MainView.outputText( '\n\n“<i>Hello [name],</i>” she says softly. “<i>Something on your mind?</i>”' );
			} else {
				// 61-90 Subsequent: ;
				MainView.outputText( 'Whitney puts down her work and smiles at you vaguely as you approach her. You again sense that queasy mixture of anxiety, unhappiness and excitement in her jerky, cramped movements and expression.' );
		...
		if( this.whitneyCorruption() >= 91 && this.whitneyCorruption() <= 119 ) {
			...
			if( CoC.flags[ kFLAGS.WHITNEY_MENU_91_119 ] === 0 ) {
				...
				// Subsequent 91-119: ;
				MainView.outputText( 'Whitney lays down her work eagerly as you approach. ' );
		...
		if( this.whitneyCorruption() === 120 ) {
			if( CoC.flags[ kFLAGS.WHITNEY_CORRUPTION_COMPLETE ] === 0 ) {
				CoC.flags[ kFLAGS.WHITNEY_CORRUPTION_COMPLETE ] = 1;
				// 120 Farm Corruption ;
				MainView.outputText( 'You don’t need to seek out Whitney this time; almost the moment you arrive on the farm a sandy furred figure is hurrying over to you. There’s deep excitement in her face and her breast is heaving with more than just exertion.' );
	...
	FarmCorruption.prototype.dogeNotCorruptLeaveFirstTime = function() {
		...
		MainView.outputText( 'As you turn to leave, you feel something tug on your [armor]. You turn back to look into Whitney’s searching eyes.' );
	...
	FarmCorruption.prototype.dogeNotCorruptYetMenu = function() {
		...
		if( this.availableInvestments() ) {
			EngineCore.addButton( 2, 'Investment', this, this.investmentMenu );
		}
		EngineCore.addButton( 9, 'Back', this, this.rootScene );
		if( this.whitneyCorruption() <= 60 && CoC.flags[ kFLAGS.WHITNEY_LEAVE_0_60 ] === 0 ) {
			EngineCore.addButton( 9, 'Back', this, this.dogeNotCorruptLeaveFirstTime );
		} else if( this.whitneyCorruption() <= 90 && CoC.flags[ kFLAGS.WHITNEY_LEAVE_61_90 ] === 0 ) {
			EngineCore.addButton( 9, 'Back', this, this.dogeNotCorruptLeave6190 );
		}
	};
	FarmCorruption.prototype.availableInvestments = function() {
		...
		if( CoC.player.hasKeyItem( 'Cock Milker - Installed At Whitney\'s Farm' ) < 0 && CoC.flags[ kFLAGS.QUEUE_COCKMILKER_UPGRADE ] === 0 ) {
			return true;
		}
		if( CoC.flags[ kFLAGS.FARM_UPGRADES_REFINERY ] === 0 && CoC.flags[ kFLAGS.QUEUE_REFINERY_UPGRADE ] === 0 ) {
			return true;
		}
		if( CoC.flags[ kFLAGS.FARM_UPGRADES_CONTRACEPTIVE ] === 0 && CoC.flags[ kFLAGS.QUEUE_CONTRACEPTIVE_UPGRADE ] === 0 ) {
			return true;
		}
		if( CoC.flags[ kFLAGS.FARM_UPGRADES_MILKTANK ] === 0 && SceneLib.milkWaifu.milkSlave() && CoC.flags[ kFLAGS.QUEUE_MILKTANK_UPGRADE ] === 0 ) {
			return true;
		}
		return false;
	};
	...
	FarmCorruption.prototype.whitneyAppearanceCorrupt = function() {
		MainView.clearOutput();
		this.whitneySprite();
		MainView.outputText( 'Whitney is a 5’8” dog' );
		if( this.whitneyDefurred() ) {
			MainView.outputText( '-girl. Her human transformation has rendered her pretty in a delicate, diffident kind of way.' );
		} else {
			MainView.outputText( '-morph. Her muzzle is suggestive of a golden retriever but really she could be any breed.' );
		}
		if( this.whitneyMaxedOralTraining() ) {
			MainView.outputText( ' In sharp contrast to her otherwise fairly thin features, her lips are plump and a depthless black, shining like wet tar. She constantly moves her tongue over them unconsciously.' );
		}
		MainView.outputText( ' Although she continues to dress in the same modest cotton blouses and long skirts she always has, complete with holes cut in to allow her short, perky tail to poke through, the cut of it and the way she moves and swings herself about, livid with arousal and near-constant sexual frustration, makes the demure outfit more perverse than anything a demon could dream up.\n' );
		if( this.whitneyHasTattoo() ) {
			...
			if( CoC.flags[ kFLAGS.WHITNEY_TATTOO_COLLARBONE ] !== 0 ) {
				MainView.outputText( CoC.flags[ kFLAGS.WHITNEY_TATTOO_COLLARBONE ] + '\n' );
			}
			if( CoC.flags[ kFLAGS.WHITNEY_TATTOO_SHOULDERS ] !== 0 ) {
				MainView.outputText( CoC.flags[ kFLAGS.WHITNEY_TATTOO_SHOULDERS ] + '\n' );
			}
			if( CoC.flags[ kFLAGS.WHITNEY_TATTOO_LOWERBACK ] !== 0 ) {
				MainView.outputText( CoC.flags[ kFLAGS.WHITNEY_TATTOO_LOWERBACK ] + '\n' );
			}
			if( CoC.flags[ kFLAGS.WHITNEY_TATTOO_BUTT ] !== 0 ) {
				MainView.outputText( CoC.flags[ kFLAGS.WHITNEY_TATTOO_BUTT ] + '\n' );
			}
		}
		MainView.outputText( '\n' );
		if( !this.whitneyDefurred() ) {
			MainView.outputText( 'Her fur is sandy, dusking to black at her extremities' );
		} else {
	...
	FarmCorruption.prototype.prosperityGoNotCorrupt = function() {
		MainView.clearOutput();
		this.whitneySprite();
		MainView.outputText( 'You ask her how the help is coming on.' );
		var lowProtection;
		var lowValue;
		...
		if( lowValue && lowProtection ) {
			...
			if( this.numFollowersAtFarm() + this.numSlavesAtFarm() + this.numLoversAtFarm() === 0 ) {
				MainView.outputText( ' Send me any kind of decent body - someone, anyone - and I can start trying to make more-a this place.' );
			} else {
				MainView.outputText( ' I need more hands to make the best of the place. More suitable hands at least.' );
			}
	...
	FarmCorruption.prototype.investmentMenu = function() {
		...
		if( CoC.player.hasKeyItem( 'Cock Milker - Installed At Whitney\'s Farm' ) < 0 && CoC.flags[ kFLAGS.QUEUE_COCKMILKER_UPGRADE ] === 0 ) {
			EngineCore.addButton( 1, 'Cock Milker', this, this.investmentCockMilker );
		}
		if( CoC.flags[ kFLAGS.FARM_UPGRADES_REFINERY ] === 0 && CoC.flags[ kFLAGS.QUEUE_REFINERY_UPGRADE ] === 0 ) {
			EngineCore.addButton( 2, 'Refinery', this, this.investmentRefinery );
		}
		if( CoC.flags[ kFLAGS.FARM_UPGRADES_CONTRACEPTIVE ] === 0 && CoC.flags[ kFLAGS.QUEUE_CONTRACEPTIVE_UPGRADE ] === 0 ) {
			EngineCore.addButton( 3, 'Contraceptive', this, this.investmentContraceptive );
		}
		if( CoC.flags[ kFLAGS.FARM_UPGRADES_MILKTANK ] === 0 && SceneLib.milkWaifu.milkSlave() && CoC.flags[ kFLAGS.QUEUE_MILKTANK_UPGRADE ] === 0 ) {
			EngineCore.addButton( 4, 'MilkTank', this, this.investmentMilktank );
		}
		if( !this.whitneyCorrupt() ) {
			EngineCore.addButton( 9, 'Back', this, this.dogeNotCorruptYetMenu );
		} else {
			EngineCore.addButton( 9, 'Back', this, this.dogeCorruptedMissionComplete );
		}
	};
	FarmCorruption.prototype.investmentBreastMilker = function() {
		...
		this.whitneySprite();
		if( !this.whitneyCorrupt() ) {
			MainView.outputText( 'You say you’d like a breast milker installed, one that you could make use of. Whitney frowns, but doesn’t seem as thrown by this idea as you expected.' );
			...
			MainView.outputText( 'You say you’d like a breast milker installed, one that you could make use of. Whitney lets her gaze sink down to your [chest].' );
		...
		if( CoC.player.gems >= 1000 ) {
			...
			EngineCore.addButton( 0, 'Do it', this, this.turnDownInvestment, true );
		}
	...
	FarmCorruption.prototype.doBreastMilkerInvestment = function() {
		...
		MainView.statsView.show();
		MainView.outputText( 'You silently hand over a hefty bag of gems. Whitney stows it away.' );
	...
	FarmCorruption.prototype.investmentCockMilker = function() {
		...
		this.whitneySprite();
		if( !this.whitneyCorrupt() ) {
			MainView.outputText( 'You say you’d like a semen extractor built, one that you could make use of. Whitney frowns, but doesn’t seem as thrown by this idea as you expected.' );
	...
	FarmCorruption.prototype.doCockMilkerInvestment = function() {
		...
		CoC.flags[ kFLAGS.QUEUE_COCKMILKER_UPGRADE ] = 1;
		EngineCore.doNext( this, this.rootScene );
	};
	FarmCorruption.prototype.investmentRefinery = function() {
		...
		this.whitneySprite();
		if( !this.whitneyCorrupt() ) {
			MainView.outputText( 'You say you want a machine built that could concentrate the fluids your followers produce into actual transformatives. Whitney shudders, but is in no position to argue.' );
	...
	FarmCorruption.prototype.doContraceptiveInvestment = function() {
		...
		MainView.statsView.show();
		if( !this.whitneyCorrupt() ) {
			MainView.outputText( 'You hand over the dough. Whitney stows it away.' );
		...
		CoC.flags[ kFLAGS.QUEUE_CONTRACEPTIVE_UPGRADE ] = 1;
		EngineCore.doNext( this, this.rootScene );
	};
	FarmCorruption.prototype.investmentMilktank = function() {
		...
		if( !this.whitneyCorrupt() ) {
			// If Bathgirl normal:;
			if( CoC.flags[ kFLAGS.MILK_SIZE ] > 0 && CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_BATH_GIRL ] === 1 ) {
				MainView.outputText( 'You tell Whitney you want her to give [bathgirlName] an intensive course of Gro-plus and Lactaid, and to build a small swimming tank to house her.' );
			...
			// If Bathgirl boobed and at camp: ;
				...
				MainView.outputText( 'You tell Whitney you want her to build a swimming tank at the farm, then come to your camp, take away your expensively acquired milk slave and install her in it. The dog woman slowly absorbs this.' );
	...
	FarmCorruption.prototype.doMilktankInvestment = function() {
		...
		this.whitneySprite();
		if( !this.whitneyCorrupt() ) {
			MainView.outputText( 'You press the gems into your taskmaster’s hand, turn and leave without another word. Your thoughts turn to huge, delightfully plush brown boobs and luxurious milky baths; it will be well worth it. ' );
		} else {
			MainView.outputText( 'You press the gems into your taskmaster’s hand, turn and leave without another word. Your thoughts turn to huge, delightfully plush, brown boobs, of luxurious milky baths; it will be well worth the cost. ' );
		}

		// In each case Bath girl reverts to her boobed state and is at farm;
		CoC.flags[ kFLAGS.QUEUE_MILKTANK_UPGRADE ] = 1;
		CoC.player.gems -= 400;
		EngineCore.doNext( this, this.rootScene );
	};
	FarmCorruption.prototype.turnDownInvestment = function( money ) {
		...
		if( !this.whitneyCorrupt() ) {
			// Any “No” option:;
			MainView.outputText( 'Whitney shrugs, nonplussed.' );
			if( money ) {
			...
			MainView.outputText( 'Whitney shrugs with a simper. ' );
	...
	FarmCorruption.prototype.deFurDoge = function() {
		MainView.clearOutput();
		this.whitneySprite();
		MainView.outputText( 'You put away the gem and move into Whitney, holding her gaze as you gently place your finger and thumb along her jaw line. She doesn’t pull away and lets out a shuddering sigh as you touch her.' );
	...
	FarmCorruption.prototype.dontDeFurDoge = function() {
		...
		if( CoC.player.hasCock() ) {
			MainView.outputText( ' crotch into' );
		} else {
			MainView.outputText( ' hips against' );
		}
		MainView.outputText( ' her firm ass and she moans, her breath starting to come in shallow gulps, her heart beating furiously against your skin. Although you keep hold of her you don’t have to do anything to make the hatchet rise again.' );
	...
	FarmCorruption.prototype.makeDogeSubby = function() {
		MainView.clearOutput();
		this.whitneySprite();
		MainView.outputText( 'You again retrieve your gem, and once more begin to rhythmically turn it; this time, however, you walk steadily towards Whitney, your voice getting louder in her ears and the shining light in her unfocused eyes getting brighter as you go on.' );
	...
	FarmCorruption.prototype.makeDogeDommy = function() {
		MainView.clearOutput();
		this.whitneySprite();
		MainView.outputText( 'You again retrieve your gem, and once more begin to rhythmically turn it. This time, however, you walk steadily towards Whitney, your voice getting louder in her ears, the shining light in her unfocused eyes getting brighter as you go on.' );
		...
		CoC.flags[ kFLAGS.WHITNEY_DOM ] = 1;
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	FarmCorruption.prototype.dogeCorruptedMissionComplete = function( output ) {
		...
		if( CoC.flags[ kFLAGS.QUEUE_BRANDING_AVAILABLE_TALK ] === 1 && output ) {
			this.brandingAvailableTalk();
			return;
		}
		if( CoC.flags[ kFLAGS.FARM_UPGRADES_ORGYROOM ] === 2 && output ) {
			this.orgyRoomTalk();
			CoC.flags[ kFLAGS.FARM_UPGRADES_ORGYROOM ] = 1;
			return;
		}
		if( output ) {
			MainView.clearOutput();
		...
		if( this.whitneyDom() && output ) {
			MainView.outputText( 'Whitney is in front of you before you’ve finished calling her name.' );
		...
		if( CoC.flags[ kFLAGS.FARM_CORRUPTION_BRANDING_MENU_UNLOCKED ] === 1 || CoC.flags[ kFLAGS.QUEUE_BRANDING_UPGRADE ] < 1 ) {
			EngineCore.addButton( 2, 'Branding', this, this.brandingMenu );
		}
		if( this.whitneyDom() ) {
			EngineCore.addButton( 3, 'Pleasure', this, this.whitneyDomPleasure );
		} else {
			EngineCore.addButton( 3, 'Pleasure', this, this.whitneySubPleasure );
		}
		this.orgyRoomRouter();
		EngineCore.addButton( 9, 'Back', this, this.rootScene );
	};
	FarmCorruption.prototype.whitneySubPleasure = function() {
		...
		if( CoC.flags[ kFLAGS.WHITNEY_ORAL_TRAINING ] === 0 ) {
			functorOnNext = true;
		...
		CoC.flags[ kFLAGS.WHITNEY_ORAL_TRAINING ]++;
		if( functorOnNext ) {
			EngineCore.doNext( this, doFunctor );
		} else {
			doFunctor();
		}
	};
	FarmCorruption.prototype.cockOralTraining = function() {
		...
		} else if( CoC.flags[ kFLAGS.WHITNEY_ORAL_TRAINING ] <= 3 ) {
			return this.cockOralTrainingStageOne;
		} else if( CoC.flags[ kFLAGS.WHITNEY_ORAL_TRAINING ] === 4 ) {
			return this.firstCockOralTrainingStageTwo;
		} else if( CoC.flags[ kFLAGS.WHITNEY_ORAL_TRAINING ] <= 9 ) {
			return this.cockOralTrainingStageTwo;
		} else if( CoC.flags[ kFLAGS.WHITNEY_ORAL_TRAINING ] === 10 ) {
			return this.firstCockOralTrainingStageThree;
		} else if( CoC.flags[ kFLAGS.WHITNEY_ORAL_TRAINING ] === 11 ) {
	...
	FarmCorruption.prototype.firstOralTraining = function() {
		...
		if( CoC.player.hasCock() ) {
			MainView.outputText( ' cocksucker' );
		} else {
			MainView.outputText( ' pussy eater' );
		}

		MainView.outputText(' there has ever been. Doubt wages war with aroused apprehension on Whitney’s face.');
		MainView.outputText( '\n\n“<i>I, I will try, [master],</i>” she says. You reach down and hold her around the jaw-line.' );
		MainView.outputText( '\n\n“<i>No,</i>” you say firmly.' );
		if( EngineCore.silly() ) {
			MainView.outputText( ' “<i>Do. Or do not. There is no try.</i>”' );
		} else {
			MainView.outputText( ' “<i>You will.</i>”' );
		}
		MainView.outputText( ' Determination lines the dog woman’s mouth.' );
		MainView.outputText( '\n\n“<i>Yes, [master]. I will tr- I will.</i>” She takes a long breath and transfers her gaze to ' + ((CoC.player.hasCock()) ? '[eachCock]' : '[vagina]') + ', looking for all the world like she’s about to embark on a marathon.' );
	...
	FarmCorruption.prototype.cockOralTrainingStageOne = function() {
		...
		if( CoC.player.biggestCockLength() > 12 ) {
			MainView.outputText( ' She still struggles a bit with her gag reflex but it only takes her a few tries to swallow your thick prick, allowing you to pump into the delightful tightness of her throat without issue.' );
		}
	...
	FarmCorruption.prototype.firstCockOralTrainingStageTwo = function() {
		...
		if( CoC.player.hasVagina() && CoC.player.balls > 0 ) {
			...
			MainView.outputText( '\n\nEventually you cannot hold your lust back. You grab her by the head and begin to thrust deep into her ' + ((CoC.player.biggestCockLength() < 8) ? 'mouth' : 'throat') + '. Her breath hisses through her nose as she lets you use her mouth, struggling to keep her fingers schlicking away; you coo as your own hard movements force her to move her hands fast and erratically, skittering across your urgently bulging button and making you pump into her wet, milking warmth all the more frenetically.' );
			MainView.outputText( '\n\n“<i>Cum, bitch.</i>” you grit as you surge to your own high. Whitney’s muffled moans are what you hear as your cock-slit dilates and pumps jizz down her throat; you crow with delight as your [vagina] quivers and then clenches in orgasm in ecstatic sympathy with your male sex, ' + ((CoC.player.wetness() <= 2) ? 'wetting your slave’s hand' : 'absolutely soaking your slave’s arm') + ' with femjizz as it does. She shudders around you and her eyes roll as she finds her own high rooting in her pussy; you are extremely gratified to note that, despite being on a plane of submissive pleasure far away, she still manages to swallow every drop of cum you eject' );
			if( CoC.player.cumQ() >= 1500 ) {
				MainView.outputText( ' which is no mean feat, given it feels like you’ve fucked what feels like a gallon of it down her gullet. Once you’re almost done, as ever you spurt the last few creamy ropes across her face and clothes' );
			}
			MainView.outputText( '.' );
		...
		//Herm without balls;
		else if( CoC.player.hasVagina() && CoC.player.balls === 0 ) {
			...
			MainView.outputText( '\n\nOnce again, there are difficulties. You have got your dog girl doing three intense things at once, and she performs the balancing act shakily, usually forgetting either to continue frigging you or herself with her mouth full of cock. You keep an eye on her and whisper instruction and encouragement when she flags, although it becomes increasingly difficult as the heat inundating both your male and female genitals grows. ' );
			MainView.outputText( '\n\nEventually you cannot hold your lust back. You grab her by the head and begin to thrust deep into her ' + ((CoC.player.biggestCockLength() < 8) ? 'mouth' : 'throat') + '. Her breath hisses through her nose as she lets you use her mouth, struggling to keep her fingers schlicking away; you coo as your own hard movements force her to move her hands faster and more erratically, skittering across your urgently bulging button and making you pump into her wet, milking warmth all the more frenetically.' );
			MainView.outputText( '\n\n“<i>Cum, bitch,</i>” you grit as you surge to your own high. Whitney’s muffled moans are what you hear as your cock-slit dilates and pumps jizz down her throat; you crow with delight as your [vagina] quivers and then clenches in orgasm in ecstatic sympathy with your male sex, ' + ((CoC.player.wetness() <= 2) ? 'wetting your slave’s hand' : 'absolutely soaking your slave’s arm') + ' with femjizz as it does. She shudders around you and her eyes roll as she finds her own high rooting in her pussy; you are extremely gratified to note that, despite being on a plane of submissive pleasure far away, she still manages to swallow every drop of cum you eject' );
			if( CoC.player.cumQ() >= 1500 ) {
				MainView.outputText( ' which is no mean feat, given it feels like you’ve fucked what feels like a gallon of it down her gullet. Once you’re almost done, as ever you spurt the last few creamy ropes across her face and clothes' );
			}
		...
		//Male w/ balls:;
			...
			if( CoC.player.cocks.length === 1 ) {
				...
				MainView.outputText( '\n\nEventually you cannot hold your lust back. You grab her by the head and begin to thrust deep into her ' + ((CoC.player.biggestCockLength() < 8) ? 'mouth' : 'throat') + '. Her breath hisses through her nose as she lets you use her mouth, struggling to keep her fingers schlicking away; you coo as your own hard movements force her to push slightly into your ass, making you pump into her wet, milking warmth all the more frenetically.' );
			...
			MainView.outputText( '.' );
			if( CoC.player.cocks.length > 1 ) {
				MainView.outputText( ' The second cock trapped in her grasp flexes out cum just as eagerly as the first, sweat standing out on your forehead as you are clenched by your twin high; it paints her skirt with its generous discharge.' );
			}
		}
		//Male without balls:;
		else if( !CoC.player.hasVagina() && CoC.player.balls === 0 ) {
		...
		if( !CoC.player.hasVagina() ) {
			...
			if( CoC.player.cocks.length > 1 ) {
				MainView.outputText( ' and [cock biggest2]' );
			}
			MainView.outputText( '. There’s an eagerness on display here which you don’t think was there before as she laps at your cock [headplural], and you think she even stifles a disappointed whine when she’s finished. Smiling softly, you get up, dress and send her on her way.' );
		}
		CoC.player.orgasm();
		EngineCore.dynStats( 'sen-', 1 );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	...
	FarmCorruption.prototype.cockOralTrainingStageTwo = function() {
		...
		if( !CoC.player.hasVagina() ) {
			...
			if( CoC.player.cocks.length > 1 ) {
				MainView.outputText( 'She wraps her hand around your [cock biggest2] when she’s ready, giving it an experimental pump before beginning to softly and slowly work it, remembering exactly how to grip and bend its girth as she prepares for the main course.' );
			}
			...
			if( CoC.player.cocks.length > 1 ) {
				MainView.outputText( 'she occasionally forgets the length bulging in her hand so eager is she to push her lips further down your [cock biggest] and work at her own clit, and you need to murmur to get her to concentrate.' );
			} else {
				MainView.outputText( 'she still occasionally forgets the length buried in her mouth, so eager is she to work keenly at her own clit, and you need to murmur to get her to concentrate.' );
			}
			MainView.outputText( ' Nonetheless, the worshipful pleasure inundating your groin is close to masterful and you find yourself drifting away on it, closing your eyes and forgetting about instruction, lost entirely in the waves of your bitch’s soft, exact, ecstatic movements, her sighs and slurps filling your ears.' );
			...
			if( CoC.player.cocks.length === 1 ) {
				MainView.outputText( 'Whitney cleans your [cock biggest] with her flat tongue, humming as she laps your oozing head clean, deliciously soothing your aching cock.' );
			} else {
				MainView.outputText( 'Whitney cleans first your [cock biggest2] and then your [cock biggest] with her flat tongue, humming as she laps each of your oozing heads clean, deliciously soothing your aching cock.' );
			}
	...
	FarmCorruption.prototype.firstCockOralTrainingStageThree = function() {
		...
		if( !CoC.player.hasVagina() ) {
			...
			if( CoC.player.cocks.length > 2 ) {
				...
				if( CoC.player.cocks.length > 1 ) {
					MainView.outputText( ' jerking both your cocks with her smooth, exacting grasp in tandem' );
				}
				MainView.outputText( '.' );
			}
			...
			if( CoC.player.cocks.length > 1 ) {
				MainView.outputText( ' all the while your [cock biggest2]' );
				if( CoC.player.cocks.length > 2 ) {
					MainView.outputText( ' and [cock biggest3]' );
				}
				MainView.outputText( ' bulge and vibrate with pleasure in her pumping hand' );
				if( CoC.player.cocks.length > 2 ) {
					MainView.outputText( 's' );
				}
			}
			MainView.outputText( '. Though it would be so easy to lose yourself entirely to her expert movements you keep abreast it for now, holding a close eye on the dog woman kneeling in front of you, now entirely devoted to pleasuring you.' );
			...
			MainView.outputText( '\n\nThe sight and sound of her inundates your senses with the same drowning intensity of the pleasure enveloping your maleness and you pump upwards to an irresistible high, grasping her head and beginning to thrust deep into the tight suction of her' + ((CoC.player.biggestCockLength() < 8) ? ' mouth' : ' throat') + '. She moans deeply as you begin to use her like this, and even when you’re face fucking her she does exactly the right things, hollowing her cheeks as you pull outwards to suck deliciously at your receding shaft then softening her mouth completely to give you the best access possible to the beautiful, tight warmth of ' + ((CoC.player.biggestCockLength() < 8) ? 'the back of her mouth' : 'her gullet') + ' as you spear decisively inwards. ' );
			...
			if( CoC.player.cocks.length > 1 || CoC.player.cumQ() >= 1500 ) {
				MainView.outputText( '\n\n' );
			}
			if( CoC.player.cocks.length > 1 ) {
				MainView.outputText( 'She doesn’t forget the cock' );
				if( CoC.player.cocks.length > 2 ) {
					MainView.outputText( 's' );
				}
				MainView.outputText( ' trapped in her grasp for a moment; she milks' );
				if( CoC.player.cocks.length > 2 ) {
					MainView.outputText( ' them' );
				} else {
					MainView.outputText( ' it' );
				}
				MainView.outputText( ' as hard as you’re fucking her mouth, and' );
				if( CoC.player.cocks.length > 2 ) {
					MainView.outputText( ' they flex' );
				} else {
					MainView.outputText( ' it flexes' );
				}
				MainView.outputText( ' out cum just as eagerly as the first, delirium seizing you as you are clenched by your' );
				if( CoC.player.cocks.length === 2 ) {
					MainView.outputText( ' twin' );
				} else if( CoC.player.cocks.length === 3 ) {
					MainView.outputText( ' triple' );
				} else {
					MainView.outputText( ' transcendent' );
				}
				MainView.outputText( ' high;' );
				if( CoC.player.cocks.length > 2 ) {
					MainView.outputText( ' they paint' );
				} else {
					MainView.outputText( ' it paints' );
				}
				MainView.outputText( ' her skirt with' );
				if( CoC.player.cocks.length > 2 ) {
					MainView.outputText( ' their' );
				} else {
					MainView.outputText( ' its' );
				}
				MainView.outputText( ' generous discharge. ' );
			}
			if( CoC.player.cumQ() >= 1500 ) {
				MainView.outputText( 'Once you’re almost done, as ever you pull out and spurt the last few creamy ropes across her face and clothes. Red with her own huge high, the dog woman does her level best to catch them in her mouth.' );
			}
			MainView.outputText( '\n\nOnce you have tensed your last you sigh beatifically and float high and formless on your post-blowjob haze. You close your eyes as, without any bidding, ' + ((CoC.player.cocks.length === 1) ? 'Whitney cleans your [cock biggest] with her flat tongue, lapping your oozing head clean, deliciously soothing your aching cock' : 'Whitney cleans all of your pricks with her flat tongue, lapping each of your oozing heads clean, deliciously soothing your aching cock') + '. She moans the low, blissed-out moan of an addict as she cleans you up, actually gently pumping your spent [prickplural] to make the last of it drool out. Gods... just these sounds and sensations, the gentle licking at your [cock biggest]’s tip and the little gasping exhale as she manages to coax a final drop onto her thirsty tongue, make you want to do it all over again... if only she hadn’t just drained you utterly. You pull her up before she makes your cock ache even more, pressing her firm, petite body into your [chest] and gaze proudly and complacently at your champion cocksucker.' );
		...
		//Herm;
			...
			MainView.outputText( '\n\n“<i>Now get back to your main task.</i>” You hum with deep approval as once again your [cock biggest] is swaddled in mouth flesh, her tongue sliding up and down your shaft in the same masterful, alternating oscillation all the while your [vagina] ' + ((CoC.player.cocks.length >= 2) ? 'and [cock biggest2] thrum and vibrate' : 'thrums and vibrates') + ' with pleasure from her teasing hands. Though it would be so easy to lose yourself entirely to her expert movements you keep abreast it for now, keeping a close eye on the dog woman kneeling in front of you, entirely devoted now to pleasuring you.' );
			...
			MainView.outputText( '\n\nThe sight and sound of her inundates your senses with the same drowning intensity of the pleasure enveloping your extensive sex and you pump upwards to an irresistible high, grasping her head and beginning to thrust deep into the tight suction of her ' + ((CoC.player.biggestCockLength() < 8) ? 'mouth' : 'throat') + '. She moans deeply as you begin to use her like this, and even when you’re face fucking her she does exactly the right things, hollowing her cheeks as you pull outwards to suck deliciously at your receding shaft then softening her mouth completely to give you the best access possible to the beautiful, tight warmth of ' );
			if( CoC.player.biggestCockLength() < 8 ) {
				MainView.outputText( 'the back of her mouth' );
			} else {
				MainView.outputText( 'her gullet' );
			}
			MainView.outputText( ' as you spear decisively inwards.' );
			MainView.outputText( '\n\nYou can’t find the words as you fly into your golden, ecstatic high; you throw your head back as you bury as much of your cock into her as you can, sweat standing out on your brow as you bark wordlessly with each bucket of jizz you fountain down her throat. There’s a series of shrill, strangled squeals below you, her body shakes around you and you know without needing to see Whitney is cumming too, purely from the slavering joy of swallowing every last drop of the thick seed thrust into her by her master. ' );
			MainView.outputText( '\n\nHer fingers slip and slide around your [vagina] and it quivers and contracts in shared orgasm, ' );
			if( CoC.player.wetness() > 2 ) {
				MainView.outputText( 'soaking her arm as it gutters your juices, ' );
			}
			MainView.outputText( 'heightening the ecstasy you feel to mind-blowing heights. ' );
			if( CoC.player.cocks.length > 1 ) {
				MainView.outputText( ' She doesn’t forget the cock trapped in her grasp for a moment either; she milks it as hard as you’re fucking her mouth, and it flexes out cum just as eagerly as the first, delirium seizing you as you are clenched by your triple high as it paints her skirt with its generous discharge.' );
			}
			if( CoC.player.cumQ() >= 1500 ) {
				MainView.outputText( ' Once you’re almost done, as ever you pull out and spurt the last few creamy ropes across her face and clothes. Red with her own huge high, the dog woman does her level best to catch them in her mouth.' );
			}
			MainView.outputText( '\n\nOnce you have tensed your last you sigh beatifically and float high and formless on your post-blowjob haze. You close your eyes as, without any bidding, ' );
			if( CoC.player.cocks.length === 1 ) {
				MainView.outputText( 'Whitney first cleans your [vagina] and then your [cock biggest] with her flat tongue, polishing your dripping pussy and thighs to a shine before lapping your oozing head clean, deliciously soothing your aching cock.' );
			} else {
				MainView.outputText( 'Whitney uses her flat tongue to clean first your [vagina], polishing your dripping pussy and thighs to a shine, and then each of your pricks, lapping the oozing heads clean, deliciously soothing your aching cocks.' );
			}
			MainView.outputText( ' She moans the low, blissed-out moan of an addict as she cleans you up, actually gently pumping your spent [prickplural] to make the last of it drool out. Gods... just these sounds and sensations, the gentle licking at your [cock biggest]’s tip and the little gasping exhale as she manages to coax a final drop onto her thirsty tongue, make you want to do it all over again... if only she hadn’t so adroitly drained you utterly. You pull her up before she makes your cock ache even more, pressing her firm, petite body into your [chest] and gaze proudly and complacently at your champion cocksucker.' );
			MainView.outputText( '\n\n“<i>Think I’ve got it,</i>” she whispers, grinning back at you woozily, red throbbing deep in the brown pools of her eyes. You rub her behind the ear and tell her she almost has; there’s one last lesson she has to learn. You grip her tight, hot body for a few moments more and then send her on her unsteady way.' );
		}
		CoC.player.orgasm();
		EngineCore.dynStats( 'sen-', 1 );
	...
	FarmCorruption.prototype.cockOralTrainingStageThree = function() {
		...
		if( CoC.player.cocks.length >= 2 ) {
			MainView.outputText( ', allowing your other cock' );
		}
		if( CoC.player.cocks.length > 2 ) {
			MainView.outputText( 's' );
		}
		if( CoC.player.cocks.length >= 2 ) {
			MainView.outputText( ' to unfurl and bob behind it' );
		}
		if( CoC.player.hasVagina() && CoC.player.cocks.length >= 2 ) {
			MainView.outputText( ' and' );
		}
		if( CoC.player.hasVagina() ) {
			MainView.outputText( ', leisurely exposing your moist [vagina]' );
		}
		if( CoC.player.cocks.length >= 2 && CoC.player.hasVagina() ) {
			MainView.outputText( ' at the same time' );
		}
		MainView.outputText( ', perfuming the air around her with your heavy musk.' );
		...
		if( !CoC.player.hasVagina() ) {
			...
			if( CoC.player.cocks.length > 2 ) {
				MainView.outputText( 'Finally, she snakes her other hand around your [cock biggest3], rolling her wrist as she gently wrings it with the same sliding thoroughness.} You open your mouth to the shifting delectation inundating your overgrowth of cocks.' );
			}
			...
			MainView.outputText( '\n\nYou grab hold of those thoughts, filling them with a black power as the drowning intensity of the pleasure inundating your maleness increases and you begin to pump upwards to an irresistible high, grasping her head and beginning to thrust deep into the tight suck of her ' + ((CoC.player.biggestCockLength() < 8) ? 'mouth' : 'throat') + '. With some effort you stay afloat and consider the dog woman as she whimpers in delight to you hammering into her juicy mouth' );
			if( CoC.player.balls > 0 ) {
				MainView.outputText( ', your saliva-glossed [balls] beating against her chin' );
			}
			MainView.outputText( '. If there is one deficiency to her right now it is her fairly thin, pale lips. No cocksucking champion has lips like that - they need to be big, bee-stung, constantly wet with the movements of a needy tongue. Luscious whore pillows worthy of having your cock slide between them. You push these succulent, evil thoughts deep into your groin, deep into the heat building inexorably there. ' );
			MainView.outputText( '\n\nEven when you’re face fucking her Whitney does exactly the right things, hollowing her cheeks as you pull outwards to suck deliciously at your receding shaft then softening her mouth completely to give you the best access possible to the beautiful, tight warmth of ' + ((CoC.player.biggestCockLength() < 8) ? 'the back of her mouth' : 'her gullet') + ' as you spear decisively inwards. You can’t find the words as you fly into your golden, ecstatic high; you throw your head back as you bury as much of your cock into her as you can, sweat standing out on your brow as you bark wordlessly with each bucket of jizz you fountain down her throat. ' );
			...
			MainView.outputText( 'She suddenly gasps in shock, and you quickly look down. Her lips look swollen, and they seem to be darkening.' );
			...
			if( CoC.player.cocks.length > 1 ) {
				MainView.outputText( ' all of' );
			}
			MainView.outputText( ' your cock' );
			if( CoC.player.cocks.length > 1 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ', first kissing your head deep and then lapping you close; she moans like a whore. She thrusts her ass in the air, sending her tongue pushing all around her new cock pillows with the taste of your cum, gently pumping your spent prick' );
			if( CoC.player.cocks.length > 1 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ' to make the last of it drool out.' );
		...
		//Herm ;
			...
			if( CoC.player.cocks.length > 1 ) {
				MainView.outputText( 'As she bathes your [cock biggest] in delicious sensation she wraps her hand around your [cock biggest2], softening and then tightening her grip, sliding up and down its stem until it is every bit as rock-hard as the one she is lapping at. ' );
			}
			MainView.outputText( ' You close your eyes as the pleasure inundating both your male and female sex twist around each other, a slow but brilliant chemical reaction consuming you from within.' );
			MainView.outputText( '\n\nWhen she finally envelopes your jutting head with her mouth, you cannot help but groan with delight. She does it ever so slowly, letting her thin lips slide gradually down your shaft whilst hollowing her cheeks with syncopated suction, blanketing more and more of your [cock biggest] in sucking, liquid warmth. Her head bobs as she immediately finds a sensual rhythm, sending her tongue sliding downwards as her mouth withdraws to your bulbous tip and then back up as she buries your length deep into her mouth, using the alternating movement to send pulses of sheer ecstasy through your groin. ' );
			...
			MainView.outputText( '. With some effort you stay afloat and consider the dog woman as she whimpers in delight to you hammering into her juicy mouth' );
			if( CoC.player.balls > 0 ) {
				MainView.outputText( ', your saliva-glossed [balls] beating against her chin' );
			}
			MainView.outputText( '. If there is one deficiency to her right now it is her fairly thin, pale lips. No cocksucking champion has lips like that - they need to be big, bee-stung, constantly wet with the movements of a needy tongue. Luscious whore pillows worthy of having your cock slide between them. You push these succulent, evil thoughts deep into your groin, deep into the heat building inexorably there. ' );
			...
			MainView.outputText( ', heightening the ecstasy you feel to mind-blowing heights.' );
			if( CoC.player.cocks.length > 1 ) {
				MainView.outputText( ' She doesn’t forget the cock trapped in her grasp for a moment; she milks it as hard as you’re fucking her mouth, and it flexes out cum just as eagerly as the first, delirium seizing you as you are clenched by your ' );
				if( CoC.player.cocks.length === 2 ) {
					MainView.outputText( 'triple' );
				} else {
					MainView.outputText( 'transcendent' );
				}
				MainView.outputText( ' high; it paints her skirt with its generous discharge.' );
			}
	...
	FarmCorruption.prototype.cockOralTrainingMaxed = function() {
		...
		if( !CoC.player.hasVagina() ) {
			...
			if( CoC.player.cocks.length > 2 ) {
				MainView.outputText( ' Finally, she snakes her other hand around your [cock biggest3], rolling her wrist as she gently wrings it with the same sliding thoroughness.' );
			}
			MainView.outputText( ' You open your mouth to the shifting delectation inundating [eachCock].' );
			...
			if( CoC.player.cocks.length > 1 ) {
				MainView.outputText( 'In the long, delicious daze that follows you move her around' );
			}
			if( CoC.player.cocks.length === 2 ) {
				MainView.outputText( ', switching her attention to your [cock biggest2] for a while' );
			} else if( CoC.player.cocks.length >= 3 ) {
				MainView.outputText( ' switching her attention first to one cock and then the other' );
			}
			if( CoC.player.cocks.length > 1 ) {
				MainView.outputText( ' so that each of your fiercely erect pricks has time to enjoy the tight, sucking embrace of her mouth.' );
			}

			MainView.outputText( '\n\nPushing her all the way down to the base of your [cock biggest] so that the heavy breath through her nose warms your crotch and holding her there for a time, you enjoy the glorious closeness of the back of her ' + ((CoC.player.biggestCockLength() < 8) ? 'mouth' : 'throat') );
			if( CoC.player.balls > 0 ) {
				MainView.outputText( ' and her tongue once again sliding across your [balls], ' );
			}
			MainView.outputText( 'before wrapping your hand through her silky hair and lifting her carefully but firmly all the way up, above your gleaming cock, at a height where she is just able to reach it with her tongue, telling her to cup her peachy breasts as you do. You enjoy the soft lapping at your beading head and the frustrated moans filling your ears at leisure, reveling in the sight of her, attempting to reconcile the memory of the calm, independent dog woman you once knew with the whimpering bitch dangling from your fist and creaming herself from worshiping your cock now.' );
			MainView.outputText( '\n\nYou let her go and she immediately impales herself back onto your length' );
			if( CoC.player.cocks.length > 1 ) {
				MainView.outputText( ', her hands wrapping back around your other cock' );
			}
			if( CoC.player.cocks.length > 2 ) {
				MainView.outputText( 's' );
			}
			if( CoC.player.cocks.length > 1 ) {
				MainView.outputText( ' as if they never left them' );
			}
			MainView.outputText( ' pulling and kneading you now with real need. The drowning intensity of the pleasure inundating your maleness increases and you begin to pump upwards to an irresistible high, grasping her head and beginning to thrust deep into the tight suction of her ' + ((CoC.player.biggestCockLength() < 8) ? 'mouth' : 'throat') + '.' );
			MainView.outputText( '\n\nAs always when you’re face fucking her Whitney does exactly the right things, hollowing her cheeks as you pull outwards to suck deliciously at your receding shaft then softening her mouth completely to give you the best access possible to the beautiful, tight warmth of ' + ((CoC.player.biggestCockLength() < 8) ? 'the back of her mouth' : 'her gullet') + ' as you spear decisively inwards, her lush cock pillows providing... not resistance but a wonderful squeezing lushness moving up and down your urgently bulging length.' );
			...
			MainView.outputText( ' Eventually you can give her no more and withdrawing you stumble back, stars swimming in your eyes from the force of it.' );
		...
		//Herm ;
			...
			MainView.outputText( ' You open your mouth to the shifting delectation inundating both your male and female sex, her teasing digits moving with innate expertise to send twin pleasure twisting through you, a slow but brilliant chemical reaction consuming you from within.' );
			MainView.outputText( '\n\nWhen she finally envelopes your jutting head with her mouth, you cannot help but groan with delight. She does it ever so slowly, letting her full, frictionless lips glissade over your sensitive cock slit before opening wide and sliding gradually down your shaft, hollowing her cheeks with syncopated suction, blanketing more and more of your [cock biggest] in sucking, liquid warmth. Her head bobs as she immediately finds a sensual rhythm, sending her tongue sliding downwards as her mouth withdraws to your bulbous tip and then back up as she buries your length deep into her mouth, using the alternating movement to send sheer ecstasy pulsing through your groin. There is no need for instruction anymore, no need to do anything but to enjoy that plump, wet seal sliding up and down your prick, a tide of warm, shifting pleasure which carries you off into an extended sexual euphoria.' );
			...
			if( CoC.player.cocks.length >= 3 ) {
				MainView.outputText( ', switching her first to one cock then another' );
			}
			if( CoC.player.cocks.length > 1 ) {
				MainView.outputText( ' so that each of your fiercely erect pricks has time to enjoy the tight, sucking embrace of her mouth.' );
			}
			...
			MainView.outputText( ' before wrapping your hand through her silky hair and lifting her carefully but firmly all the way up, above your [cock biggest] at a height where she is just able to reach it with her tongue, telling her to cup her peachy breasts as you do. You enjoy the soft lapping at your beading head and the frustrated moans filling your ears at leisure, reveling in the sight of her, attempting to reconcile the memory of the calm, independent dog woman you once knew with the whimpering bitch dangling from your fist and creaming herself from worshiping your cock now.' );
			...
			if( CoC.player.cocks.length > 1 ) {
				MainView.outputText( ' and her other hand wrapping back around your [cock biggest2]' );
			}
			MainView.outputText( ' as if they never left them, pulling, kneading and curling into you now with real need. The drowning intensity of the pleasure inundating your sex increases and you begin to pump upwards to an irresistible high, grasping her head and beginning to thrust deep into the tight suction of her ' + ((CoC.player.biggestCockLength() < 8) ? 'mouth' : 'throat') + '.' );
			MainView.outputText( '\n\nEven when you’re face fucking her Whitney does exactly the right things, hollowing her cheeks as you pull outwards to suck deliciously at your receding shaft then softening her mouth completely to give you the best access possible to the beautiful, tight warmth of ' + ((CoC.player.biggestCockLength() < 8) ? 'the back of her mouth' : 'her gullet') + ' as you spear decisively inwards. Her lush cock pillows provide no resistance but a wonderful squeezing lushness running up and down your urgently bulging length. You can’t find the words as you fly into your golden, ecstatic high; you throw your head back as you bury as much of your cock into her as you can, sweat standing out on your brow as you bark wordlessly with each bucket of jizz you fountain down her throat. ' );
			...
			MainView.outputText( 'heightening the ecstasy you feel to mind-blowing heights.' );
			if( CoC.player.cocks.length > 1 ) {
				MainView.outputText( ' She doesn’t forget the [cock biggest2] trapped in her grasp for a moment; she milks it as hard as you’re fucking her mouth, and it flexes out cum just as eagerly as the first, delirium seizing you as you are clenched by your triple high; it paints her skirt with its generous discharge.' );
			}
			MainView.outputText( ' Eventually you can give her no more and withdraw, stars swimming in your eyes from the force of your orgasm.' );
	...
	FarmCorruption.prototype.vaginaOralTrainingStageOne = function() {
		...
		if( CoC.player.wetness() >= 5 ) {
			...
			MainView.outputText( ' The dog woman coughs and splutters around the lubricant you dribble into her, some of it dripping out of her nose even as you continue to push yourself into her. This doesn’t stop her from furiously moving her hand in her knickers, flicking her clit for all she is worth as she attempts to cope with the pressing demand of your snatch. On the edge already it doesn’t take her long, and her eyes roll as she tenses and shudders.' );
		}
	...
	FarmCorruption.prototype.firstVaginaOralTrainingStageTwo = function() {
		...
		if( CoC.player.lactationQ() > 0 ) {
			MainView.outputText( ' dripping slightly' );
		} else {
			MainView.outputText( ' semi-erect' );
		}
		MainView.outputText( ' from the arousal you already feel. “<i>A good slut knows her mistress’s breasts need to be attended to, her nipples and skin shining with a slave’s worship for all the world to see, before she’s deserving of drinking from her pussy. Let’s see how you do.</i>” ' );
		...
		if( CoC.player.hasFuckableNipples() ) {
			MainView.outputText( ' obscene folds' );
		} else {
			MainView.outputText( ' sensitive points' );
		}
		MainView.outputText( ', kneading them gently and bathing them in saliva. Given her initial awkwardness at cunnilingus it’s remarkable how good she is at this. “<i>Lick every inch... that’s good. Suckle on them a bit... very good!</i>” You gasp a bit as she envelopes one of your nipples and' );
		if( CoC.player.hasFuckableNipples() ) {
			MainView.outputText( ' sends her tongue questing into its sensitive inside.' );
		} else {
			MainView.outputText( 'bites it ever so gently.' );
		}
		if( CoC.player.lactationQ() > 0 ) {
			MainView.outputText( ' The gentle kneading is easily enough to make your fecund teats dribble milk into her mouth, and after being slightly startled by it Whitney evidently enjoys the taste of it, because she begins to pull at your plump breast for more. You allow her another mouthful before gently pulling her away.' );
		...
		if( CoC.player.wetness() <= 2 ) {
			MainView.outputText( ' wetting both your slave’s hand and mouth' );
		} else {
			MainView.outputText( ' absolutely soaking your slave’s face and arm' );
		}
		MainView.outputText( ' with femjizz as it does. She shudders around you and her eyes roll as she finds her own high rooting in her pussy; you are extremely gratified to note that, despite being on a plane of submissive pleasure far above you, she still manages to swallow every drop of cum that showers into her mouth' );
		if( CoC.player.wetness() >= 5 ) {
			MainView.outputText( ' which is no mean feat, given it feels like you gutter what feels like a gallon down her gullet. Once you’re almost done, as ever you get up and spurt the last of it on her hair and clothes' );
		}
		MainView.outputText( '.' );
		MainView.outputText( '\n\nYou exhale long and low when you’re finally done, feeling a glow of intense satisfaction. You gaze dozily down at your slave, who is panting hoarsely, her eyes still closed. You can’t help but notice she occasionally and impulsively licks her lips, as if she is searching for the flavor of something delicious.' );
	...
	FarmCorruption.prototype.vaginaOralTrainingStageTwo = function() {
		MainView.clearOutput();
		this.whitneySprite();
		MainView.outputText( 'Not saying a word, you undo your [armor], exposing your whole body with a soft smile. Without any need for instruction Whitney gets to work, her hand delving into her underwear with a contented sigh, her eyes focused on the task in front of her as she steadily stokes the heat in her crotch with tight, urgent jerks of her slim body.' );
		...
		if( CoC.player.lactationQ() > 0 ) {
			MainView.outputText( ' and it quickly makes the liquid warmth in your breasts rise to a point, beading milk freely from your teats. Whitney laps and sucks it down eagerly, but gratifyingly she remembers your order and stops reluctantly after only taking a single pull' );
		}
		MainView.outputText( ', leaving your nipples' );
		if( !CoC.player.hasFuckableNipples() ) {
			MainView.outputText( ' rock-hard and' );
		}
		MainView.outputText( ' shining and your [vagina] beading eagerly.' );
		...
		if( CoC.player.wetness() >= 5 ) {
			MainView.outputText( ', fountaining fem juice down her throat' );
		}
		...
		if( CoC.player.wetness() >= 5 ) {
			MainView.outputText( ', soaking her again and again with warm, female approval' );
		}
		MainView.outputText( '.' );
	...
	FarmCorruption.prototype.firstVaginaOralTrainingStageThree = function() {
		...
		if( !CoC.player.hasFuckableNipples() ) {
			MainView.outputText( ' she takes each of your [nipples] into her mouth to bathe them in close, sucking attention' );
		} else {
			MainView.outputText( ' she sends her tongue lapping into each of your cunt nipples, curling at their sensitive pink until they are wet with need' );
		}
		MainView.outputText( '. It’s a process which is beautifully sensuous, made all the more gratifying by the soft, lustful moans she makes as she polishes your softness.' );
		if( CoC.player.lactationQ() > 0 ) {
			MainView.outputText( ' It quickly makes the liquid warmth in your breasts rise to a point, beading milk freely from your teats. Whitney laps and sucks it down eagerly, but gratifyingly she remembers your order and stops reluctantly after only taking a single pull.' );
		}
		MainView.outputText( ' She sinks downwards leaving your nipples' );
		if( !CoC.player.hasFuckableNipples() ) {
			MainView.outputText( ' rock-hard and' );
		}
		MainView.outputText( ' shining, your [vagina] beading eagerly.' );
		...
		if( CoC.player.clitLength >= 4 ) {
			MainView.outputText( ' She uses her new anchorage to take your bulging fem-dick entirely into her mouth, sucking on it avidly; she hums as she does it, sending ecstatic pleasure thrumming through you.' );
		}
		...
		if( CoC.player.wetness() >= 5 ) {
			MainView.outputText( ', soaking her mouth and face again and again with warm, female approval' );
		}
		MainView.outputText( '. There’s a series of shrill, strangled squeals below you, her body shakes around you and you know without needing to see Whitney is cumming too, purely from the slavering joy of swallowing every last drop of the musky fruit guttered into her by her mistress.' );
		if( CoC.player.wetness() >= 5 ) {
			MainView.outputText( ' It is in a state of ecstatic momentum that you rise away from her mouth and spurt the last of it onto her hair and clothes. Red with her own huge high, the dog woman does her level best to catch your spotting fluids in her mouth.' );
		}
	...
	FarmCorruption.prototype.vaginaOralTrainingStageThree = function() {
		...
		MainView.outputText( '. It’s a process which is beautifully sensuous.' );
		if( CoC.player.lactationQ() > 0 ) {
			MainView.outputText( ' It quickly makes the liquid warmth in your breasts rise to a point, beading milk freely from your teats. Whitney laps and sucks it down eagerly, whimpering as she withdraws after taking a single pull. You smile softly, knowing that the small, teasing taste of your sweetness only serves to make her thirstier.' );
		}
		MainView.outputText( ' She sinks downwards leaving your nipples' );
		if( CoC.player.hasFuckableNipples() ) {
			MainView.outputText( ' rock-hard and' );
	...
	FarmCorruption.prototype.vaginaOralTrainingMaxed = function() {
		...
		if( !CoC.player.hasFuckableNipples() ) {
			MainView.outputText( ' she envelopes each of your [nipples] with her lips, kissing them with their sumptuous sponginess before bathing them in close, sucking attention' );
		} else {
			MainView.outputText( ' she pushes her lips against your cunt nipples before sending her tongue gently into each of them, curling at their sensitive pink until they are oozing wetness in delight' );
		}
		MainView.outputText( '. It’s a process which is beautifully sensuous, made all the better by the soft, lustful moans she makes as she polishes your bust.' );
		if( CoC.player.lactationQ() > 0 ) {
		...
		MainView.outputText( ' Having left your [chest] gleaming with warm, oily saliva and your nipples' );
		if( !CoC.player.hasFuckableNipples() ) {
			MainView.outputText( ' rock-hard and' );
		}
		MainView.outputText( ' shining, she sinks down to the next stage.' );
		...
		if( CoC.player.buttRating < 4 ) {
			MainView.outputText( 'firm butt' );
		}
		if( CoC.player.buttRating < 8 ) {
			MainView.outputText( 'supple rump' );
		} else {
			MainView.outputText( 'soft ass' );
		}
		MainView.outputText( ', oiling it to a sheen. She makes little gasping, stifled breaths as she goes about it, and a grin comes to your lips.' );
		...
		CoC.player.orgasm();
		EngineCore.dynStats( 'sen-', 1 );
		if( CoC.player.wetness() < 5 && Utils.rand( 4 ) === 0 ) {
	...
	FarmCorruption.prototype.whitneyDomPleasure = function() {
		...
		if( CoC.flags[ kFLAGS.WHITNEY_DOM_FIRST_PLEASURE ] === 0 ) {
			...
			CoC.flags[ kFLAGS.WHITNEY_DOM_FIRST_PLEASURE ] = 1;
		} else {
		...
		EngineCore.doNext( this, scenes[ Utils.rand( scenes.length ) ] );
	};
	FarmCorruption.prototype.firstWhitneyDomPleasure = function() {
		MainView.outputText( 'You are slightly apprehensive about this, but you’ve gone to the trouble of crafting Whitney into a fairly unique slave, and it seems churlish not to test just how deep the dominating streak you’ve brought out in her goes. You tell her you could do with a bit of relaxation.' );
	...
	FarmCorruption.prototype.repeatWhitneyDomPleasure = function() {
		MainView.outputText( 'You say you could do with some relaxation.' );
	...
	FarmCorruption.prototype.whitneyDomBondageOral = function() {
		MainView.clearOutput();
		this.whitneySprite();
		MainView.outputText( 'Whitney climbs on top of your supine form, her hands pressing into your [chest], firm hips clenching around your abdomen as she considers you.' );
		...
		if( CoC.player.hasCock() && !CoC.player.hasVagina() ) {
			...
			if( CoC.player.totalCocks() > 1 ) {
				MainView.outputText( ' “<i>So many things to play with tucked away in here!</i>” comes her merry voice again, as you feel her grasp your [cock smallest] at the same time; you groan into the confines of her flesh as she gives it a soft, teasing pump. It’s obvious your main member has really caught her attention though, and both her hands soon return to it.' );
			}
		...
		MainView.outputText( ', and only after relaxing for a long, heavy moment on your face does she finally slide off you. You heave and pant for breath, still cuffed to the bed, rivulets of femcum trickling down your face, your tongue and ' );
		if( CoC.player.hasCock() ) {
		...
		MainView.outputText( ' aching, oozing. In a deep, post-coital haze you feel someone take a towel to your body before curling into you, throwing a dense thigh over you, holding and running her hands over you possessively as you recover. ' );
	...
	FarmCorruption.prototype.whitneyDomStraponDoggy = function() {
		...
		if( CoC.player.isBiped() || CoC.player.isTaur() ) {
			MainView.outputText( ' Next, she pushes your ankles apart, threads a leather strap around each and then pulls them tight with a sharp yank; you carefully test them and find with a cold thrill that there is a metal bar attached between them, forcing you to stay in this position.' );
		} else if( CoC.player.isDrider() ) {
			MainView.outputText( ' You don’t struggle as she then forces your spindly legs beneath the bed, looping a leather strap attached to the frame around them all, trapping them there.' );
		} else if( CoC.player.isNaga() ) {
			MainView.outputText( ' She then ties your reptilian coils around the bedposts, securing them with leather straps fitted for the purpose.' );
		} else {
		...
		if( CoC.player.hasVagina() && !CoC.player.hasCock() && SceneLib.vapula.vapulaSlave() ) {
		...
		if( capacity >= 36 ) {
			...
			if( CoC.player.hasVagina() ) {
				MainView.outputText( 'sit and spread ‘em for anything going, don’t you?' );
			} else {
				MainView.outputText( 'I bet you get on your knees for anything with a nice, juicy cock, right?' );
			}
			MainView.outputText( ' What would your other slaves think of that? My secret little slut.</i>” ' );
			...
			if( CoC.player.hasVagina() ) {
				MainView.outputText( 'take of your tight cunt' );
			} else {
				MainView.outputText( 'give of your tight ass' );
			}
			MainView.outputText( ', pushing the other end of the dildo into her. An even louder moan is forced out of your throat by the intensity of your tunnel being packed full of cock, albeit an imitation.' );
			if( CoC.player.hasVagina() ) {
				CoC.player.cuntChange( 36, true, true, false );
			} else {
				CoC.player.buttChange( 36, true, true, false );
			...
			if( CoC.player.hasVagina() ) {
				MainView.outputText( 'servants, you tight, bossy little bitch' );
			} else {
				MainView.outputText( 'cattle, you tight, bossy little boy-bitch' );
			}
			MainView.outputText( '.</i>” ' );
		...
		MainView.outputText( '\n\nShe withdraws almost all the way out, the knobbed head pulling at your ' + ((CoC.player.hasVagina()) ? 'entrance before thrusting back into your [vagina]' : 'sphincter before thrusting back into your [ass]') + ', bringing the paddle down on your reddened behind as she does. In, slap, out, in, slap, out, first one cheek, then the other...' );
		MainView.outputText( '\n\nThe panties in your mouth are dense with the taste and smell of the dog woman’s excitement; stuck high in your nose and throat, the rich musk makes you feel impossibly turned on, and there’s no getting away from it. Although your intention was to take your ‘relaxation’ in a stoical silence befitting of your position, you can’t stop yourself whining and moaning around the soiled cotton, welcoming each hard slap to your [butt] as it heralds another firm stroke from the delightfully thick and bumpy cock' + ((CoC.player.hasCock()) ? ' bumping delightfully against your prostate, making [eachCock] thrum and bulge urgently in pleasure' : '') + '.' );
		MainView.outputText( '\n\nYou convulse fruitlessly against your bonds as you are pushed relentlessly up to your high, ' + ((CoC.player.hasVagina()) ? 'your pussy quivering and then clenching ardently around the dildo' : 'your [cockplural] pulsing, opening wide and then forced to unload, your back passage tightening eagerly around the thick dildo as you spurt cum deliriously') + ', blood rushing to the surface of your behind - only to meet the paddle’s hard blow. Whitney continues to thrust into you indifferent to your orgasm and you groan into your musky gag as the warm, prosthetic dick bends ' + ((CoC.player.hasVagina()) ? 'mercilessly' : '') + ' into your sensitive walls ' + ((CoC.player.hasCock()) ? 'continuing to milk you mercilessly, keeping your cock achingly hard' : '') + '.' );
		...
		MainView.outputText( '\n\nA ' + ((this.whitneyDefurred()) ? 'hand' : 'paw') + ' reaches around and clutches your [chest] for anchorage as she finally begins to pound into your tenderized hole with all she’s got. She catches a [nipple] between her fingers and you find yourself being forced towards orgasm again, your ears full of her harsh breathing, your nose full of her heavy scent. She’s no longer thrusting into you for your pleasure though, if she ever was; the hard, feral thrusting of the strap-on into your ' + ((CoC.player.hasVagina()) ? 'oozing cunt' : 'ass') + ' is all about her, all about working the smaller, teasing end in her own pussy as hard as she can. You are simply a vehicle for her own lusts, and when you come again with a juddering cry into her panties, ' + ((CoC.player.hasVagina()) ? 'wetting the bed this time with the jackhammer force of it' : '') + ' she keeps going, hammering into your hapless ' + ((CoC.player.hasVagina()) ? '[vagina]' : 'butt') + '.' );
		...
		if( CoC.player.hasVagina() && !CoC.player.hasCock() && SceneLib.vapula.vapulaSlave() ) {
			MainView.outputText( ' You feel the end of the dildo tense deep within you before releasing, filling you full of warm demon spunk until it pools out onto the bed. The feeling is horribly, disgustingly satisfying, and for a moment you can see exactly how Vapula views the world.' );
		}
	...
	FarmCorruption.prototype.whitneyDomRide = function() {
		...
		if( CoC.player.isBiped() ) {
			MainView.outputText( ' She does the same with your ankles, leaving you helplessly spread-eagled across the bed.' );
		} else if( CoC.player.isDrider() ) {
			MainView.outputText( ' She has long, leather straps attached to the underside of the bed; these are wrapped around the joining of your spindly legs to your heavy abdomen, leaving your chitinous feet pointing helplessly at the ceiling.' );
		} else if( CoC.player.isGoo() ) {
			MainView.outputText( ' “<i>I can’t secure you properly,</i>” she breathes, squeezing your gooey form, sinking her fingers into it. “<i>Owing to this nonsense you call a body. So I will jus’ say it again. Lie. Still.</i>”' );
		} else if( CoC.player.isNaga() ) {
			MainView.outputText( ' Your long, heavy tail is wrapped around the bedsteads at the foot of the bed and secured with more leather straps, leaving you helplessly spread-eagled across the bed.' );
		}
		...
		MainView.outputText( '\n\n“<i>Stop fussing now, sweetheart,</i>” Whitney croons, adjusting the black, elastic blindfold so it is firmly secured over your eyes.' + ((CoC.player.eyeType === AppearanceDefs.EYES_FOUR_SPIDER_EYES) ? ' Once she’s finished with that she places another blindfold over your second pair of eyes. You can only groan with laughter at the sheer level of preparation the dog woman is capable of.' : '') + ' “<i>Wouldn’t want you to see what’s coming, would we? That’d ruin half the fun.</i>” ' );
		MainView.outputText( '\n\nShe crawls down to your lower body, taking her time now as she slides her hands across your abdomen and [hips] and sits herself down in front of [eachCock]' + ((CoC.player.balls > 0) ? ', making you twitch as she momentarily cups your [balls]' : '') + '. She teases you with slow, deliberate movements of her smooth digits, fully aware that all you can do now in your black space is lie there, feeling and listening to what she’s doing to you.' );
		...
		var cockI = CoC.player.cockThatFits( this.whitneyVagCapacity() * 1.33 ) + 1;
		var tooBig = false;
		// Catch a no-cock-fits scenario so we can do things.;
		if( cockI === 0 ) {
			tooBig = true;
			cockI = CoC.player.smallestCock() + 1;
		}
		if( tooBig ) {
		...
		// Medium - cock area is within 33% of Whitneys cap without going over.;
		else if( CoC.player.cockArea( cockI - 1 ) > (this.whitneyVagCapacity() * 0.66) ) {
			...
			MainView.outputText( '\n\n“<i>Massive centaur cock, lil’ sissy dick, that’d be about you, that’d be a statement about the kinda [guy] you are,</i>” she goes on in a low growl, warming to her theme as she screws you, slowly, rhythmically and surely, the candle she’s clutching somewhere above you. “<i>But you’re weak, ain’t ya? You got no self-control when it comes to pussy. You jus want to be used by anyone and anything. So - get - used!</i>” More liquid wax splashes down, this time on a ' + ((CoC.player.isLactating()) ? 'moistened' : 'taut') + ' [nipple] and you can’t help but cringe again, your involuntary movements again making Whitney cry out in pleasure, her tight, succulent sex wringing your [cock ' + cockI + '] gleefully.' );
	...
	FarmCorruption.prototype.brandingMenu = function() {
		...
		if( CoC.flags[ kFLAGS.FARM_CORRUPTION_BRANDING_MENU_UNLOCKED ] === 0 ) {
			...
			if( CoC.player.gems >= 500 ) {
				...
				EngineCore.addButton( 0, 'Do it', this, this.dontGetBrandingStuff );
			}
			EngineCore.addButton( 1, 'No', this, this.dontGetBrandingStuff );
		} else {
			MainView.clearOutput();
			MainView.outputText( 'Your thoughts turn to the ingenious magic of your tattooing gear, stashed away in the barn. Grinning, you consider who you will summon to brand.' );
			...
			if( CoC.player.gems >= 50 ) {
				...
				if( CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_AMILY ] === 1 && this.hasFreeTattooSlot( 'amily' ) ) {
					EngineCore.addButton( 1, 'Amily', this, this.brandAmily );
				}
				if( CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_JOJO ] === 1 && this.hasFreeTattooSlot( 'jojo' ) ) {
					EngineCore.addButton( 2, 'Jojo', this, this.brandJojo );
				}
				if( CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_SOPHIE ] === 1 && SceneLib.sophieBimbo.bimboSophie() && this.hasFreeTattooSlot( 'sophie' ) ) {
					EngineCore.addButton( 3, 'Sophie', this, this.brandBimboSophie );
				}
				if( CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_VAPULA ] === 1 && this.hasFreeTattooSlot( 'vapula' ) ) {
					EngineCore.addButton( 4, 'Vapula', this, this.brandVapula );
				}
				if( CoC.flags[ kFLAGS.KELT_BREAK_LEVEL ] >= 4 && this.hasFreeTattooSlot( 'kelly' ) ) {
					EngineCore.addButton( 5, 'Kelly', this, this.brandKelly );
				}
				if( CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_BATH_GIRL ] === 1 && CoC.flags[ kFLAGS.MILK_SIZE ] > 0 && this.hasFreeTattooSlot( 'milky' ) ) {
					EngineCore.addButton( 6, CoC.flags[ kFLAGS.MILK_NAME ], this, this.brandSmallMilky );
				}
				if( CoC.flags[ kFLAGS.FOLLOWER_AT_FARM_BATH_GIRL ] === 1 && CoC.flags[ kFLAGS.MILK_SIZE ] === 0 && this.hasFreeTattooSlot( 'milky' ) ) {
					EngineCore.addButton( 6, CoC.flags[ kFLAGS.MILK_NAME ], this, this.brandBigMilky );
				}
			}
			EngineCore.addButton( 9, 'Back', this, this.dogeCorruptedMissionComplete );
		}
	...
	FarmCorruption.prototype.hasFreeTattooSlot = function( name ) {
		if( name === 'whitney' ) {
			...
			if( CoC.flags[ kFLAGS.VAPULA_TATTOO_LOWERBACK ] === 0 ) {
				return true;
			}
			if( CoC.flags[ kFLAGS.VAPULA_TATTOO_SHOULDERS ] === 0 ) {
				return true;
			}
			if( CoC.flags[ kFLAGS.VAPULA_TATTOO_BUTT ] === 0 ) {
				return true;
			}
			return false;
		} else if( name === 'kelly' ) {
			...
			if( CoC.flags[ kFLAGS.MILKY_TATTOO_LOWERBACK ] === 0 ) {
				return true;
			}
			if( CoC.flags[ kFLAGS.MILKY_TATTOO_SHOULDERS ] === 0 ) {
				return true;
			}
			if( CoC.flags[ kFLAGS.MILKY_TATTOO_BUTT ] === 0 ) {
				return true;
			}
			return false;
		} else {
	...
	FarmCorruption.prototype.brandWhitney = function() {
		MainView.clearOutput();
		this.whitneySprite();
		MainView.outputText( 'Whitney knows exactly what it means when you head off to the barn where she stashed your “branding” equipment; the dog girl is already eagerly stripped down by the time you’ve returned. You hold the pots of ink and consider where, and what, to put on her.' );
		CoC.player.gems -= 50;
		MainView.statsView.show();
	...
	FarmCorruption.prototype.brandAmily = function() {
		MainView.clearOutput();
		MainView.outputText( 'You retrieve the pots of ink and paper from the barn and, smiling, tell your pet mouse you’re going to give her a treat.' );
		...
		MainView.outputText( ', [master]? That’s my favorite type of treat.</i>” You consider where, and what, to put on her.' );
		CoC.player.gems -= 50;
		MainView.statsView.show();
	...
	FarmCorruption.prototype.brandJojo = function() {
		MainView.clearOutput();
		MainView.outputText( 'You retrieve the pots of ink and paper from the barn and, smiling brightly, tell your pet mouse you’re going to give him a special treat. Jojo knows all too well the nature of your treats. He closes his eyes and waits for the worst as you consider where and what to put on him.' );
		CoC.player.gems -= 50;
		MainView.statsView.show();
	...
	FarmCorruption.prototype.brandBimboSophie = function() {
		MainView.clearOutput();
		MainView.outputText( 'You retrieve the pots of ink and paper from the barn and, smiling, tell your pet harpy you’re going to give her a treat.' );
	...
	FarmCorruption.prototype.brandVapula = function() {
		MainView.outputText( 'You retrieve the pots of ink and paper from the barn and, smiling, tell your succubus you’re going to give her a treat.' );
	...
	FarmCorruption.prototype.brandKelly = function() {
		MainView.outputText( 'You retrieve the pots of ink and paper from the barn and smilingly tell your centaur cumslut you’re going to give her a treat.' );
	...
	FarmCorruption.prototype.brandSmallMilky = function() {
		MainView.outputText( 'You retrieve the pots of ink and paper from the barn and, smiling kindly, tell your ex Sand Witch slave you’re going to give her a treat. ' );
	...
	FarmCorruption.prototype.dontGetBrandingStuff = function() {
		MainView.clearOutput();
		this.whitneySprite();
		MainView.outputText( 'You stroke at her tiny bulging button until you adjudge she’s on the edge, before withdrawing your hand completely. Maybe some other time, you say airily.' );
		MainView.outputText( '\n\n“<i>Oh [master],</i>” groans Whitney, in a tone of deepest exasperation, her hands between her thighs.' );
		if( this.whitneyDom() ) {
			MainView.outputText( ' “<i>You’ll pay for that later, I swear!</i>”' );
		}
		MainView.outputText( ' Grinning, you turn and leave.' );
		this.dogeCorruptedMissionComplete();
	};
	FarmCorruption.prototype.brandingAvailableTalk = function() {
		MainView.clearOutput();
		this.whitneySprite();
		MainView.outputText( '“<i>[Master]! [Master], I’ve done as you asked,</i>” says Whitney, her tail wagging frantically as you approach. Her eyes are lit with glee and she’s got a cow with her on tether. You settle down and watch as she produces unassuming pots of black and red ink, and a sheaf of what looks like blotting paper.' );
		...
		if( this.whitneyDefurred() ) {
			MainView.outputText( ' fingernail' );
		} else {
			MainView.outputText( ' claw' );
		}
		MainView.outputText( ' into the pot, withdraws it and then, dripping black fluid, presses it against the cow’s ample backside. The animal moos and swishes its tail in mild protest, but it doesn’t seem in any grievous amount of pain as Whitney’s digit moves, artfully tracing a large number 12 with the ink.' );
	...
	FarmCorruption.prototype.testBranding = function() {
		MainView.clearOutput();
		this.whitneySprite();
		MainView.outputText( 'You return her devilish grin with interest.' );
	...
	FarmCorruption.prototype.brandSlotSelect = function() {
		...
		if( CoC.flags[ kFLAGS.WHITNEY_TATTOO_SHOULDERS ] === 0 ) {
			EngineCore.addButton( 1, 'Shoulders', this, this.brandSelect, 1 );
		}
		if( CoC.flags[ kFLAGS.WHITNEY_TATTOO_LOWERBACK ] === 0 ) {
			EngineCore.addButton( 2, 'Lower Back', this, this.brandSelect, 2 );
		}
		if( CoC.flags[ kFLAGS.WHITNEY_TATTOO_BUTT ] === 0 ) {
			EngineCore.addButton( 3, 'Butt', this, this.brandSelect, 3 );
		}
	};
	FarmCorruption.prototype.amilyBrandSlotSelect = function() {
		...
		if( CoC.flags[ kFLAGS.AMILY_TATTOO_SHOULDERS ] === 0 ) {
			EngineCore.addButton( 1, 'Shoulders', this, this.amilyBrandSelect, 1 );
		}
		if( CoC.flags[ kFLAGS.AMILY_TATTOO_LOWERBACK ] === 0 ) {
			EngineCore.addButton( 2, 'Lower Back', this, this.amilyBrandSelect, 2 );
		}
		if( CoC.flags[ kFLAGS.AMILY_TATTOO_BUTT ] === 0 ) {
			EngineCore.addButton( 3, 'Butt', this, this.amilyBrandSelect, 3 );
		}
	};
	FarmCorruption.prototype.jojoBrandSlotSelect = function() {
		...
		if( CoC.flags[ kFLAGS.JOJO_TATTOO_SHOULDERS ] === 0 ) {
			EngineCore.addButton( 1, 'Shoulders', this, this.jojoBrandSelect, 1 );
		}
		if( CoC.flags[ kFLAGS.JOJO_TATTOO_LOWERBACK ] === 0 ) {
			EngineCore.addButton( 2, 'Lower Back', this, this.jojoBrandSelect, 2 );
		}
		if( CoC.flags[ kFLAGS.JOJO_TATTOO_BUTT ] === 0 ) {
			EngineCore.addButton( 3, 'Butt', this, this.jojoBrandSelect, 3 );
		}
	};
	FarmCorruption.prototype.bimboSophieSlotSelect = function() {
		...
		if( CoC.flags[ kFLAGS.SOPHIE_TATTOO_SHOULDERS ] === 0 ) {
			EngineCore.addButton( 1, 'Shoulders', this, this.bimboSophieBrandSelect, 1 );
		}
		if( CoC.flags[ kFLAGS.SOPHIE_TATTOO_LOWERBACK ] === 0 ) {
			EngineCore.addButton( 2, 'Lower Back', this, this.bimboSophieBrandSelect, 2 );
		}
		if( CoC.flags[ kFLAGS.SOPHIE_TATTOO_BUTT ] === 0 ) {
			EngineCore.addButton( 3, 'Butt', this, this.bimboSophieBrandSelect, 3 );
		}
	};
	FarmCorruption.prototype.vapulaSlotSelect = function() {
		...
		if( CoC.flags[ kFLAGS.VAPULA_TATTOO_SHOULDERS ] === 0 ) {
			EngineCore.addButton( 1, 'Shoulders', this, this.vapulaBrandSelect, 1 );
		}
		if( CoC.flags[ kFLAGS.VAPULA_TATTOO_LOWERBACK ] === 0 ) {
			EngineCore.addButton( 2, 'Lower Back', this, this.vapulaBrandSelect, 2 );
		}
		if( CoC.flags[ kFLAGS.VAPULA_TATTOO_BUTT ] === 0 ) {
			EngineCore.addButton( 3, 'Butt', this, this.vapulaBrandSelect, 3 );
		}
	};
	FarmCorruption.prototype.kellySlotSelect = function() {
		...
		if( CoC.flags[ kFLAGS.KELLY_TATTOO_SHOULDERS ] === 0 ) {
			EngineCore.addButton( 1, 'Shoulders', this, this.kellyBrandSelect, 1 );
		}
		if( CoC.flags[ kFLAGS.KELLY_TATTOO_LOWERBACK ] === 0 ) {
			EngineCore.addButton( 2, 'Lower Back', this, this.kellyBrandSelect, 2 );
		}
		if( CoC.flags[ kFLAGS.KELLY_TATTOO_BUTT ] === 0 ) {
			EngineCore.addButton( 3, 'Butt', this, this.kellyBrandSelect, 3 );
		}
	};
	FarmCorruption.prototype.smallMilkySlotSelect = function() {
		...
		if( CoC.flags[ kFLAGS.MILKY_TATTOO_SHOULDERS ] === 0 ) {
			EngineCore.addButton( 1, 'Shoulders', this, this.smallMilkyBrandSelect, 1 );
		}
		if( CoC.flags[ kFLAGS.MILKY_TATTOO_LOWERBACK ] === 0 ) {
			EngineCore.addButton( 2, 'Lower Back', this, this.smallMilkyBrandSelect, 2 );
		}
		if( CoC.flags[ kFLAGS.MILKY_TATTOO_BUTT ] === 0 ) {
			EngineCore.addButton( 3, 'Butt', this, this.smallMilkyBrandSelect, 3 );
		}
	};
	FarmCorruption.prototype.bigMilkySlotSelect = function() {
		...
		if( CoC.flags[ kFLAGS.MILKY_TATTOO_SHOULDERS ] === 0 ) {
			EngineCore.addButton( 1, 'Shoulders', this, this.bigMilkyBrandSelect, 1 );
		}
		if( CoC.flags[ kFLAGS.MILKY_TATTOO_LOWERBACK ] === 0 ) {
			EngineCore.addButton( 2, 'Lower Back', this, this.bigMilkyBrandSelect, 2 );
		}
		if( CoC.flags[ kFLAGS.MILKY_TATTOO_BUTT ] === 0 ) {
			EngineCore.addButton( 3, 'Butt', this, this.bigMilkyBrandSelect, 3 );
		}
	};
	FarmCorruption.prototype.brandSelect = function( slot ) {
		...
		EngineCore.addButton( 3, 'No.1 Bitch', this, this.no1Tattoo, slot );
		if( CoC.player.hasCock() && this.whitneyMaxedOralTraining() ) {
			EngineCore.addButton( 4, 'Cocksucker', this, this.champCocksuckerTattoo, slot );
		}
		if( CoC.player.hasVagina() && this.whitneyMaxedOralTraining() ) {
			EngineCore.addButton( 5, 'Pussylicker', this, this.champPussylickerTattoo, slot );
		}
	...
	FarmCorruption.prototype.amilyBrandSelect = function( slot ) {
		...
		EngineCore.addButton( 3, 'Breeding Bitch', this, this.amilyBreedingBitchTattoo, slot );
		if( CoC.player.hasCock() && slot === 2 ) {
			EngineCore.addButton( 4, 'Cock Here', this, this.amilyCockGoesHereTattoo, slot );
		}
		if( CoC.player.hasVagina() ) {
	...
	FarmCorruption.prototype.jojoBrandSelect = function( slot ) {
		...
		EngineCore.addButton( 3, 'Sissy Slut', this, this.jojoSissySlutTattoo, slot );
		if( CoC.player.hasCock() && slot === 2 ) {
			EngineCore.addButton( 4, 'Cock Here', this, this.jojoCockGoesHereTattoo, slot );
		}
		if( CoC.player.hasVagina() ) {
	...
	FarmCorruption.prototype.bimboSophieBrandSelect = function( slot ) {
		...
		EngineCore.addButton( 3, 'Property Of', this, this.bimboSophiePropertyOfTattoo, slot );
		EngineCore.addButton( 4, 'Breeding Bitch', this, this.bimboSophieBreedingBitchTattoo, slot );
		if( slot === 3 ) {
			EngineCore.addButton( 5, 'Wide Load', this, this.bimboSophieWideLoadTattoo, slot );
		}
		if( CoC.player.hasCock() && slot === 2 ) {
			EngineCore.addButton( 6, 'Cock Goes Here', this, this.bimboSophieCockGoesHereTattoo, slot );
		}
	...
	FarmCorruption.prototype.vapulaBrandSelect = function( slot ) {
		...
		EngineCore.addButton( 3, 'Cum Addict', this, this.vapulaCumAddictTattoo, slot );
		if( slot === 2 || slot === 3 ) {
			EngineCore.addButton( 4, 'Buttslut', this, this.vapulaButtslutTattoo, slot );
		}
		if( !CoC.player.hasCock() ) {
			EngineCore.addButton( 5, 'Dildo Polisher', this, this.vapulaDildoPolisherTattoo, slot );
		}
	...
	FarmCorruption.prototype.kellyBrandSelect = function( slot ) {
		...
		EngineCore.addButton( 3, 'No.1 Filly', this, this.kellyNo1FillyTattoo, slot );
		if( EngineCore.silly() ) {
			EngineCore.addButton( 4, 'Dick Won', this, this.kellyDickWonTattoo, slot );
		}
		if( slot === 1 ) {
			EngineCore.addButton( 5, 'Horseshoe', this, this.kellyHorseshoeTattoo, slot );
		}
	...
	FarmCorruption.prototype.smallMilkyBrandSelect = function( slot ) {
		...
		EngineCore.addButton( 3, 'Property Of', this, this.smallMilkyPropertyOfTattoo, slot );
		EngineCore.addButton( 4, 'Bath Toy', this, this.smallMilkyBathToyTattoo, slot );
		if( slot === 0 ) {
			EngineCore.addButton( 5, 'Mega Milk', this, this.smallMilkyMegaMilkTattoo, slot );
		}
		if( slot === 0 && CoC.player.hasCock() ) {
			EngineCore.addButton( 6, 'Cock Cozy', this, this.smallMilkyCockCozyTattoo, slot );
		}
	...
	FarmCorruption.prototype.bigMilkyBrandSelect = function( slot ) {
		...
		EngineCore.addButton( 3, 'Bath Toy', this, this.bigMilkyBathToyTattoo, slot );
		if( slot === 0 ) {
			EngineCore.addButton( 4, 'Mega Milk', this, this.bigMilkyMegaMilkTattoo, slot );
		}
		if( slot === 0 && CoC.player.hasCock() ) {
			EngineCore.addButton( 5, 'Cock Cozy', this, this.bigMilkyCockCozyTattoo, slot );
		}
		EngineCore.addButton( 9, 'Back', this, this.bigMilkySlotSelect );
	};
	FarmCorruption.prototype.collarboneIntro = function() {
		MainView.outputText( 'You command her to kneel in front of you and be still. Your lithe, naked dog girl does so, staying perfectly still as you dip your finger into the ink, carefully draw your design on her, and then seal it on with the paper.' );
	};
	FarmCorruption.prototype.amilyCollarboneIntro = function() {
		MainView.outputText( 'You command her to kneel in front of you and be still. The succubus mouse does so, staying perfectly still as you dip your finger into the ink, carefully draw your design on her, and then seal it on with the paper. ' );
	};
	FarmCorruption.prototype.jojoCollarboneIntro = function() {
		MainView.outputText( 'You command him to kneel in front of you and be still. The mouse-demon does so, staying perfectly still as you dip your finger into the ink, carefully draw your design on him, and then seal it on with the paper. ' );
	...
	FarmCorruption.prototype.bimboSophieCollarboneIntro = function() {
		MainView.outputText( 'You command her to kneel in front of you and be still. It’s difficult to get her to stay still, but she goes into a trance-like state when you finally lay your ink-soaked finger on her, mesmerized by its movement until you lay the paper over the design you’ve drawn and seal the ink.' );
	};
	FarmCorruption.prototype.vapulaCollarboneIntro = function() {
		MainView.outputText( 'You command her to kneel in front of you and be still. The succubus does so, staying perfectly still as you dip your finger into the ink, carefully draw your design on her, and then seal it on with the paper. ' );
	};
	FarmCorruption.prototype.kellyCollarboneIntro = function() {
		MainView.outputText( 'You command her to kneel in front of you and be still. The centaur does so, staying perfectly still as you dip your finger into the ink, carefully draw your design on her, and then seal it on with the paper. ' );
	};
	FarmCorruption.prototype.smallMilkyCollarboneIntro = function() {
		MainView.outputText( 'You command her to take her clothes off, kneel in front of you and be still. The dusky girl does so, staying perfectly still as you dip your finger into the ink, carefully draw your design on her and then seal it on with the paper.' );
	};
	...
	FarmCorruption.prototype.shouldersIntro = function() {
		MainView.outputText( 'You command her to kneel facing away from you and be still. Your lithe, naked dog girl does so, staying perfectly still as you dip your finger into the ink, carefully draw your design on her, and then seal it on with the paper. “<i>What did you draw, [master]?</i>” she says eagerly.' );
	...
	FarmCorruption.prototype.jojoShouldersIntro = function() {
		MainView.outputText( 'You command him to kneel facing away from you and be still. The mouse-demon does so, staying perfectly still as you dip your finger into the ink, carefully draw your design on him, and then seal it on with the paper. ' );
	...
	FarmCorruption.prototype.amilyShouldersIntro = function() {
		MainView.outputText( 'You command her to kneel facing away from you and be still. Your succubus mouse does so, staying perfectly still as you dip your finger into the ink, carefully draw your design on her and then seal it on with the paper. ' );
	...
	FarmCorruption.prototype.bimboSophieShouldersIntro = function() {
		MainView.outputText( 'You command her to kneel facing away from you and be still. It’s difficult to get her to stay still, but she goes into a trance-like state when you finally lay your ink-soaked finger between her blonde wings, mesmerized by its movement until you lay the paper over the design you’ve drawn and seal the ink.' );
	...
	FarmCorruption.prototype.vapulaShouldersIntro = function() {
		MainView.outputText( 'You command her to kneel facing away from you and be still. Your succubus does so, staying perfectly still as you dip your finger into the ink, carefully draw your design on her, and then seal it on with the paper. ' );
	...
	FarmCorruption.prototype.kellyShouldersIntro = function() {
		MainView.outputText( 'You command her to kneel facing away from you and be still. The centaur does so, staying perfectly still as you dip your finger into the ink, carefully draw your design on her shoulder blades, and then seal it on with the paper. ' );
	...
	FarmCorruption.prototype.smallMilkyShouldersIntro = function() {
		MainView.outputText( 'You command her to kneel facing away from you and be still. The dusky girl does so, keeping her back straight and still as you dip your finger into the ink, carefully draw your design on her, and then seal it on with the paper. ' );
	};
	FarmCorruption.prototype.bigMilkyShouldersIntro = function() {
		MainView.outputText( 'You set yourself down behind [bathgirlName] before dipping your finger into the ink, carefully draw your design on her, and then seal it on with the paper. She barely seems to notice.' );
		MainView.outputText( '\n\n“<i>Bath time?</i>”' );
		MainView.outputText( '\n\n“<i>Soon,</i>” you reply soothingly, as you turn to go put your branding equipment back.' );
	};
	FarmCorruption.prototype.lowerbackIntro = function() {
		MainView.outputText( 'You command her to set herself down on your [legs], as if she were about to receive a spanking. Your lithe, naked dog girl does so, her laughter at her own compromising permission turning to a sharp coo as you dip your finger into the ink and carefully draw your design on her, before sealing it on with the paper. “<i>What did you draw, [master]?</i>” she says eagerly.' );
		MainView.outputText( '\n\nLaughing, you admire it for yourself and then say she’ll have to ask one of your other slaves... and hope they tell the truth. “<i>Aw no, come on, tell me what it is! It’s a rude word, isn’t it? [Master], it better not be somethin’ the slaves are gonna laugh at...</i>”' );
	};
	FarmCorruption.prototype.jojoLowerbackIntro = function() {
		MainView.outputText( 'You command him to set himself face down across your [legs], as if he were about to receive a spanking. The mouse-demon does so, staying perfectly still as you dip your finger into the ink, carefully draw your design on him and then seal it on with the paper. ' );
		MainView.outputText( '\n\n“<i>Th-thanks, [master],</i>” he says. He pauses. “<i>I don’t suppose I could know what it-?</i>” You tussle his adorable ears and tell him of course not.' );
	};
	FarmCorruption.prototype.amilyLowerBackIntro = function() {
		MainView.outputText( 'You command her to set herself face down across your [legs], as if she were about to receive a spanking. Your succubus mouse does so, her giggles at her own compromising position turning to a sharp coo as you dip your finger into the ink and carefully draw your design on her, before sealing it on with the paper. ' );
		MainView.outputText( '\n\n“<i>What did you draw, [master]?</i>” she says eagerly. Laughing, you admire it for yourself and then say she’ll have to ask one of your other slaves... and hope they tell the truth. “<i>Aw [master], you know they’ll just lie and say it’s something like ‘breeding bitch’. C’mon, please tell me!</i>”' );
	};
	FarmCorruption.prototype.bimboSophieLowerBackIntro = function() {
		MainView.outputText( 'You command her to set herself face down across your [legs], as if she were about to receive a spanking. It’s difficult to get her to stay still, cooing and shaking with giggles at her compromising position. She finally falls into a trance-like state when you lay your ink-soaked finger on her, mesmerized by its movement until you lay the paper over the design you’ve drawn and seal the ink.' );
	...
	FarmCorruption.prototype.vapulaLowerBackIntro = function() {
		MainView.outputText( 'You command her to set herself face down across your [legs], as if she were about to receive a spanking. Your succubus does so, her put-upon sigh turning to a sharp coo as you dip your finger into the ink and carefully draw your design on her, before sealing it on with the paper. ' );
	...
	FarmCorruption.prototype.kellyLowerBackIntro = function() {
		MainView.outputText( 'You command her to stand perfectly still.  She can’t quite stop clopping her hooves fretfully as you dip your finger into the ink and carefully draw your design on her human lower back before sealing it on with the paper. ' );
		MainView.outputText( '\n\n“<i>What did you draw, [master]?</i>” she asks tentatively when you’re done. Trying to keep a straight face, you say a giant spunky horse cock. “<i>That’s funny, [master]. Ha ha. Y-you didn’t really do that, did you?</i>”' );
	};
	FarmCorruption.prototype.smallMilkyLowerBackIntro = function() {
		MainView.outputText( 'You command her to set herself down across your [legs]. The dusky girl does so, staying perfectly still as you dip your finger into the ink, carefully draw your design on her, and then seal it on with the paper.' );
	};
	...
	FarmCorruption.prototype.buttIntro = function() {
		MainView.outputText( 'You command her to set herself down on your [legs], as if she were about to receive a spanking. Your lithe, naked dog girl does so, her laughter at her own compromising permission turning to a sharp coo as you dip your finger into the ink and carefully draw your design on the softest part of her anatomy, before sealing it on with the paper.' );
	...
	FarmCorruption.prototype.amilyButtIntro = function() {
		MainView.outputText( 'You command her to set herself face down across your [legs], as if she were about to receive a spanking. Your succubus mouse does so, her laughter at her own compromising position turning to a sharp coo as you dip your finger into the ink and carefully draw your design on the softest part of her anatomy, before sealing it on with the paper. ' );
	...
	FarmCorruption.prototype.jojoButtIntro = function() {
		MainView.outputText( 'You command him to set himself face down across your [legs], as if he were about to receive a spanking. The mouse-demon does so, staying perfectly still as you dip your finger into the ink, carefully draw your design onto the softest part of his body and then seal it on with the paper. ' );
	...
	FarmCorruption.prototype.vapulaButtIntro = function() {
		MainView.outputText( 'You command her to set herself down across your [legs], as if she were about to receive a spanking. Your succubus does so, her put-upon sigh turning to a sharp coo as you dip your finger into the ink and carefully draw your design on the softest part of her anatomy, before sealing it on with the paper. ' );
	...
	FarmCorruption.prototype.kellyButtIntro = function() {
		MainView.outputText( 'You command her to stand perfectly still.  She can’t quite stop clopping her hooves fretfully as you dip your finger into the ink and carefully draw your design on her smooth, brawny horse ass before sealing it on with the paper. ' );
	...
	FarmCorruption.prototype.smallMilkyButtIntro = function() {
		MainView.outputText( 'You command her to set herself down across your [legs]. The dusky girl does so, staying perfectly still as you dip your finger into the ink, carefully draw your design on her fine, round ass, and then seal it on with the paper.' );
	};
	FarmCorruption.prototype.bigMilkyButtIntro = function() {
		MainView.outputText( 'You set yourself down behind [bathgirlName] before dipping your finger into the ink, carefully draw your design on her fine, round ass, and then seal it on with the paper. She barely seems to notice.' );
		MainView.outputText( '\n\n“<i>Bath time?</i>”' );
		MainView.outputText( '\n\n“<i>Soon,</i>” you reply soothingly, as you turn to go put your branding equipment back.' );
	};
	...
	FarmCorruption.prototype.tribalTattoo = function( slot ) {
		...
		if( slot === 0 ) {
			...
			tText += 'collarbone.';
			CoC.flags[ kFLAGS.WHITNEY_TATTOO_COLLARBONE ] = tText;
		} else if( slot === 1 ) {
			this.shouldersIntro();
			tText += 'shoulders.';
			CoC.flags[ kFLAGS.WHITNEY_TATTOO_SHOULDERS ] = tText;
		} else if( slot === 2 ) {
			this.lowerbackIntro();
			tText += 'lower back.';
			CoC.flags[ kFLAGS.WHITNEY_TATTOO_LOWERBACK ] = tText;
		} else if( slot === 3 ) {
			this.buttIntro();
			tText += 'butt.';
			CoC.flags[ kFLAGS.WHITNEY_TATTOO_BUTT ] = tText;
		}
		this.tattooMerge();
	};
	FarmCorruption.prototype.amilyTribalTattoo = function( slot ) {
		...
		if( slot === 0 ) {
			...
			tText += 'collarbone.';
			CoC.flags[ kFLAGS.AMILY_TATTOO_COLLARBONE ] = tText;
		} else if( slot === 1 ) {
			this.amilyShouldersIntro();
			tText += 'shoulders.';
			CoC.flags[ kFLAGS.AMILY_TATTOO_SHOULDERS ] = tText;
		} else if( slot === 2 ) {
			this.amilyLowerBackIntro();
			tText += 'lower back.';
			CoC.flags[ kFLAGS.AMILY_TATTOO_LOWERBACK ] = tText;
		} else if( slot === 3 ) {
			this.amilyButtIntro();
			tText += 'butt.';
			CoC.flags[ kFLAGS.AMILY_TATTOO_BUTT ] = tText;
		}
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	FarmCorruption.prototype.jojoTribalTattoo = function( slot ) {
		...
		if( slot === 0 ) {
			...
			tText += 'collarbone.';
			CoC.flags[ kFLAGS.JOJO_TATTOO_COLLARBONE ] = tText;
		} else if( slot === 1 ) {
			this.jojoShouldersIntro();
			tText += 'shoulders.';
			CoC.flags[ kFLAGS.JOJO_TATTOO_SHOULDERS ] = tText;
		} else if( slot === 2 ) {
			this.jojoLowerbackIntro();
			tText += 'lower back.';
			CoC.flags[ kFLAGS.JOJO_TATTOO_LOWERBACK ] = tText;
		} else if( slot === 3 ) {
			this.jojoButtIntro();
			tText += 'butt.';
			CoC.flags[ kFLAGS.JOJO_TATTOO_BUTT ] = tText;
		}
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	FarmCorruption.prototype.bimboSophieTribalTattoo = function( slot ) {
		...
		if( slot === 0 ) {
			...
			tText += 'collarbone.';
			CoC.flags[ kFLAGS.SOPHIE_TATTOO_COLLARBONE ] = tText;
		} else if( slot === 1 ) {
			this.bimboSophieShouldersIntro();
			tText += 'shoulders.';
			CoC.flags[ kFLAGS.SOPHIE_TATTOO_SHOULDERS ] = tText;
		} else if( slot === 2 ) {
	...
	FarmCorruption.prototype.vapulaTribalTattoo = function( slot ) {
		...
		if( slot === 0 ) {
			...
			tText += 'collarbone.';
			CoC.flags[ kFLAGS.VAPULA_TATTOO_COLLARBONE ] = tText;
		} else if( slot === 1 ) {
			this.vapulaShouldersIntro();
			tText += 'shoulders.';
			CoC.flags[ kFLAGS.VAPULA_TATTOO_SHOULDERS ] = tText;
		} else if( slot === 2 ) {
			...
			tText += 'butt.';
			CoC.flags[ kFLAGS.VAPULA_TATTOO_BUTT ] = tText;
		}
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	FarmCorruption.prototype.kellyTribalTattoo = function( slot ) {
		...
		if( slot === 0 ) {
			...
			MainView.outputText( '“<i>That’s- that’s actually really pretty, [master]!</i>” Kelly says when you’re done, looking down with delight. “<i>Thank you!</i>”' );
			tText += 'collarbone.';
			CoC.flags[ kFLAGS.KELLY_TATTOO_COLLARBONE ] = tText;
		} else if( slot === 1 ) {
			...
			tText += 'lower back.';
			CoC.flags[ kFLAGS.KELLY_TATTOO_LOWERBACK ] = tText;
		} else if( slot === 3 ) {
			this.kellyButtIntro();
			tText += 'butt.';
			CoC.flags[ kFLAGS.KELLY_TATTOO_BUTT ] = tText;
		}
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	FarmCorruption.prototype.smallMilkyTribalTattoo = function( slot ) {
		...
		if( slot === 0 ) {
			...
			MainView.outputText( '“<i>Oh hey, that’s quite pretty.</i>” [bathgirlName] smiles down at what you’ve drawn as you withdraw the pen. “<i>Thanks, [name]!</i>” You kiss her fondly on the forehead and send her on her way.' );
			tText += 'collarbone.';
			...
			tText += 'shoulders.';
			CoC.flags[ kFLAGS.MILKY_TATTOO_SHOULDERS ] = tText;
		} else if( slot === 2 ) {
			...
			MainView.outputText( '\n\n“<i>Yes, but what is it? [name]? Hello?</i>”' );
			tText += 'lower back.';
			CoC.flags[ kFLAGS.MILKY_TATTOO_LOWERBACK ] = tText;
		} else if( slot === 3 ) {
			this.smallMilkyButtIntro();
			MainView.outputText( '\n\n“<i>So, uh...</i>” she says when you’re done. “<i>What did you do?</i>” Something much better than a butterfly you say, grinning as you touch the permanent marking before putting her down to go stow the branding gear away.' );
			MainView.outputText( '\n\n“<i>Yes, but what is it? [name]? Hello?</i>”' );
			tText += 'butt.';
			CoC.flags[ kFLAGS.MILKY_TATTOO_BUTT ] = tText;
		}
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	...
	FarmCorruption.prototype.numMilkyButterflyTats = function() {
		var count = 0;
		if( _.isString( CoC.flags[ kFLAGS.MILKY_TATTOO_COLLARBONE ] ) ) {
			if( CoC.flags[ kFLAGS.MILKY_TATTOO_COLLARBONE ].indexOf( 'A butterfly' ) >= 0 ) {
				count++;
			}
		}
		if( _.isString( CoC.flags[ kFLAGS.MILKY_TATTOO_SHOULDERS ] ) ) {
			if( CoC.flags[ kFLAGS.MILKY_TATTOO_SHOULDERS ].indexOf( 'A butterfly' ) >= 0 ) {
				count++;
			}
		}
		if( _.isString( CoC.flags[ kFLAGS.MILKY_TATTOO_LOWERBACK ] ) ) {
			if( CoC.flags[ kFLAGS.MILKY_TATTOO_LOWERBACK ].indexOf( 'A butterfly' ) >= 0 ) {
				count++;
			}
		}
		if( _.isString( CoC.flags[ kFLAGS.MILKY_TATTOO_BUTT ] ) ) {
			if( CoC.flags[ kFLAGS.MILKY_TATTOO_BUTT ].indexOf( 'A butterfly' ) >= 0 ) {
				count++;
			}
		}
		return count;
	...
	FarmCorruption.prototype.dontTestBranding = function() {
		MainView.clearOutput();
		this.whitneySprite();
		MainView.outputText( 'Maybe later, you say. Whitney looks disappointed.' );
	...
	FarmCorruption.prototype.numAmilyTribalTats = function() {
		var count = 0;
		if( _.isString( CoC.flags[ kFLAGS.AMILY_TATTOO_COLLARBONE ] ) ) {
			if( CoC.flags[ kFLAGS.AMILY_TATTOO_COLLARBONE ].indexOf( 'A tribal tattoo' ) >= 0 ) {
				count++;
			}
		}
		if( _.isString( CoC.flags[ kFLAGS.AMILY_TATTOO_SHOULDERS ] ) ) {
			if( CoC.flags[ kFLAGS.AMILY_TATTOO_SHOULDERS ].indexOf( 'A tribal tattoo' ) >= 0 ) {
				count++;
			}
		}
		if( _.isString( CoC.flags[ kFLAGS.AMILY_TATTOO_LOWERBACK ] ) ) {
			if( CoC.flags[ kFLAGS.AMILY_TATTOO_LOWERBACK ].indexOf( 'A tribal tattoo' ) >= 0 ) {
				count++;
			}
		}
		if( _.isString( CoC.flags[ kFLAGS.AMILY_TATTOO_BUTT ] ) ) {
			if( CoC.flags[ kFLAGS.AMILY_TATTOO_BUTT ].indexOf( 'A tribal tattoo' ) >= 0 ) {
				count++;
			}
		}
		return count;
	...
	FarmCorruption.prototype.numWhitneyTribalTats = function() {
		var count = 0;
		if( _.isString( CoC.flags[ kFLAGS.WHITNEY_TATTOO_COLLARBONE ] ) ) {
			if( CoC.flags[ kFLAGS.WHITNEY_TATTOO_COLLARBONE ].indexOf( 'A tribal tattoo' ) >= 0 ) {
				count++;
			}
		}
		if( _.isString( CoC.flags[ kFLAGS.WHITNEY_TATTOO_SHOULDERS ] ) ) {
			if( CoC.flags[ kFLAGS.WHITNEY_TATTOO_SHOULDERS ].indexOf( 'A tribal tattoo' ) >= 0 ) {
				count++;
			}
		}
		if( _.isString( CoC.flags[ kFLAGS.WHITNEY_TATTOO_LOWERBACK ] ) ) {
			if( CoC.flags[ kFLAGS.WHITNEY_TATTOO_LOWERBACK ].indexOf( 'A tribal tattoo' ) >= 0 ) {
				count++;
			}
		}
		if( _.isString( CoC.flags[ kFLAGS.WHITNEY_TATTOO_BUTT ] ) ) {
			if( CoC.flags[ kFLAGS.WHITNEY_TATTOO_BUTT ].indexOf( 'A tribal tattoo' ) >= 0 ) {
				count++;
			}
		}
		return count;
	...
	FarmCorruption.prototype.numJojoTribalTats = function() {
		var count = 0;
		if( _.isString( CoC.flags[ kFLAGS.JOJO_TATTOO_COLLARBONE ] ) ) {
			if( CoC.flags[ kFLAGS.JOJO_TATTOO_COLLARBONE ].indexOf( 'A tribal tattoo' ) >= 0 ) {
				count++;
			}
		}
		if( _.isString( CoC.flags[ kFLAGS.JOJO_TATTOO_SHOULDERS ] ) ) {
			if( CoC.flags[ kFLAGS.JOJO_TATTOO_SHOULDERS ].indexOf( 'A tribal tattoo' ) >= 0 ) {
				count++;
			}
		}
		if( _.isString( CoC.flags[ kFLAGS.JOJO_TATTOO_LOWERBACK ] ) ) {
			if( CoC.flags[ kFLAGS.JOJO_TATTOO_LOWERBACK ].indexOf( 'A tribal tattoo' ) >= 0 ) {
				count++;
			}
		}
		if( _.isString( CoC.flags[ kFLAGS.JOJO_TATTOO_BUTT ] ) ) {
			if( CoC.flags[ kFLAGS.JOJO_TATTOO_BUTT ].indexOf( 'A tribal tattoo' ) >= 0 ) {
				count++;
			}
		}
		return count;
	...
	FarmCorruption.prototype.numSophieTribalTats = function() {
		var count = 0;
		if( _.isString( CoC.flags[ kFLAGS.SOPHIE_TATTOO_COLLARBONE ] ) ) {
			if( CoC.flags[ kFLAGS.SOPHIE_TATTOO_COLLARBONE ].indexOf( 'A tribal tattoo' ) >= 0 ) {
				count++;
			}
		}
		if( _.isString( CoC.flags[ kFLAGS.SOPHIE_TATTOO_SHOULDERS ] ) ) {
			if( CoC.flags[ kFLAGS.SOPHIE_TATTOO_SHOULDERS ].indexOf( 'A tribal tattoo' ) >= 0 ) {
				count++;
			}
		}
		if( _.isString( CoC.flags[ kFLAGS.SOPHIE_TATTOO_LOWERBACK ] ) ) {
			if( CoC.flags[ kFLAGS.SOPHIE_TATTOO_LOWERBACK ].indexOf( 'A tribal tattoo' ) >= 0 ) {
				count++;
			}
		}
		if( _.isString( CoC.flags[ kFLAGS.SOPHIE_TATTOO_BUTT ] ) ) {
			if( CoC.flags[ kFLAGS.SOPHIE_TATTOO_BUTT ].indexOf( 'A tribal tattoo' ) >= 0 ) {
				count++;
			}
		}
		return count;
	...
	FarmCorruption.prototype.numVapulaTribalTats = function() {
		var count = 0;
		if( _.isString( CoC.flags[ kFLAGS.VAPULA_TATTOO_COLLARBONE ] ) ) {
			if( CoC.flags[ kFLAGS.VAPULA_TATTOO_COLLARBONE ].indexOf( 'A tribal tattoo' ) >= 0 ) {
				count++;
			}
		}
		if( _.isString( CoC.flags[ kFLAGS.VAPULA_TATTOO_SHOULDERS ] ) ) {
			if( CoC.flags[ kFLAGS.VAPULA_TATTOO_SHOULDERS ].indexOf( 'A tribal tattoo' ) >= 0 ) {
				count++;
			}
		}
		if( _.isString( CoC.flags[ kFLAGS.VAPULA_TATTOO_LOWERBACK ] ) ) {
			if( CoC.flags[ kFLAGS.VAPULA_TATTOO_LOWERBACK ].indexOf( 'A tribal tattoo' ) >= 0 ) {
				count++;
			}
		}
		if( _.isString( CoC.flags[ kFLAGS.VAPULA_TATTOO_BUTT ] ) ) {
			if( CoC.flags[ kFLAGS.VAPULA_TATTOO_BUTT ].indexOf( 'A tribal tattoo' ) >= 0 ) {
				count++;
			}
		}
		return count;
	...
	FarmCorruption.prototype.numKellyTribalTats = function() {
		var count = 0;
		if( _.isString( CoC.flags[ kFLAGS.KELLY_TATTOO_COLLARBONE ] ) ) {
			if( CoC.flags[ kFLAGS.KELLY_TATTOO_COLLARBONE ].indexOf( 'A tribal tattoo' ) >= 0 ) {
				count++;
			}
		}
		if( _.isString( CoC.flags[ kFLAGS.KELLY_TATTOO_SHOULDERS ] ) ) {
			if( CoC.flags[ kFLAGS.KELLY_TATTOO_SHOULDERS ].indexOf( 'A tribal tattoo' ) >= 0 ) {
				count++;
			}
		}
		if( _.isString( CoC.flags[ kFLAGS.KELLY_TATTOO_LOWERBACK ] ) ) {
			if( CoC.flags[ kFLAGS.KELLY_TATTOO_LOWERBACK ].indexOf( 'A tribal tattoo' ) >= 0 ) {
				count++;
			}
		}
		if( _.isString( CoC.flags[ kFLAGS.KELLY_TATTOO_BUTT ] ) ) {
			if( CoC.flags[ kFLAGS.KELLY_TATTOO_BUTT ].indexOf( 'A tribal tattoo' ) >= 0 ) {
				count++;
			}
		}
		return count;
	...
	FarmCorruption.prototype.numMilkyTribalTats = function() {
		var count = 0;
		if( _.isString( CoC.flags[ kFLAGS.MILKY_TATTOO_COLLARBONE ] ) ) {
			if( CoC.flags[ kFLAGS.MILKY_TATTOO_COLLARBONE ].indexOf( 'A tribal tattoo' ) >= 0 ) {
				count++;
			}
		}
		if( _.isString( CoC.flags[ kFLAGS.MILKY_TATTOO_SHOULDERS ] ) ) {
			if( CoC.flags[ kFLAGS.MILKY_TATTOO_SHOULDERS ].indexOf( 'A tribal tattoo' ) >= 0 ) {
				count++;
			}
		}
		if( _.isString( CoC.flags[ kFLAGS.MILKY_TATTOO_LOWERBACK ] ) ) {
			if( CoC.flags[ kFLAGS.MILKY_TATTOO_LOWERBACK ].indexOf( 'A tribal tattoo' ) >= 0 ) {
				count++;
			}
		}
		if( _.isString( CoC.flags[ kFLAGS.MILKY_TATTOO_BUTT ] ) ) {
			if( CoC.flags[ kFLAGS.MILKY_TATTOO_BUTT ].indexOf( 'A tribal tattoo' ) >= 0 ) {
				count++;
			}
		}
		return count;
	...
	FarmCorruption.prototype.orgyRoomRouter = function() {
		var doFunctor = null;
		if( CoC.flags[ kFLAGS.FARM_UPGRADES_ORGYROOM ] === 0 && CoC.flags[ kFLAGS.QUEUE_ORGYROOM_UPGRADE ] === 0 && !this.whitneyDom() ) {
			doFunctor = this.wantOrgyRoom;
		}
		if( CoC.flags[ kFLAGS.FARM_UPGRADES_ORGYROOM ] === 1 ) {
			if( this.whitneyDom() ) {
				// Nothing... yet. Hopefully.;
			} else {
				if( !CoC.player.isGoo() && !CoC.player.isTaur() && (CoC.player.hasCock() || CoC.player.hasVagina()) ) {
					doFunctor = this.orgyRoomSubMassage;
				}
			}
		}
		if( doFunctor !== null ) {
			EngineCore.addButton( 4, 'Massage', this, doFunctor );
		}
	};
	FarmCorruption.prototype.wantOrgyRoom = function() {
		MainView.clearOutput();
		this.whitneySprite();
		MainView.outputText( '“<i>[Master],</i>” says Whitney haltingly, after she’s knelt in front of you. “<i>May I make a suggestion?</i>” You shrug nonchalantly. “<i>Well... it’s just you often roll in here looking quite worn out, like. From fightin’ demons and dragons and such, whatever it is you do out there in the wastelands. It must be real important, cause you’re often banged up. To mention nothing of you needing to take care of... your chattel...</i>” she closes her eyes and sighs deeply. ' );
	...
	FarmCorruption.prototype.getOrgyRoom = function() {
		MainView.clearOutput();
		this.whitneySprite();
		MainView.outputText( 'It’s a heavy investment, but her description of what she intends has definitely piqued your interest. Slowly you turn and, with a flick of the head, indicate the rock where she leaves the farm’s profit each week. Whitney breaks out onto a wide grin, her hands scrunching up on her skirt.' );
	...
	FarmCorruption.prototype.noOrgyRoomPlz = function() {
		MainView.clearOutput();
		this.whitneySprite();
		MainView.outputText( 'You tell her it’s a fine idea, but it will have to wait for now.' );
		...
		EngineCore.doNext( this, this.dogeCorruptedMissionComplete );
		// Orgy Room added to Investments menu;
	};
	FarmCorruption.prototype.orgyRoomTalk = function() {
		MainView.clearOutput();
		this.whitneySprite();
		MainView.outputText( 'You can hear Whitney humming to herself happily as you approach her outside the milking shed, and when she sees you she rushes over to clutch at your arm excitedly.' );
		...
		if( this.whitneyDom() ) {
			MainView.outputText( 'The sight sends a shuddering thrill down your spine. “<i>You didn’t think I’d forget those, did you [master]? We both know how important they are for properly relaxing you,</i>” Whitney murmurs, pushing her hot suppleness into your side. ' );
		} else {
			MainView.outputText( 'You raise an eyebrow at these and Whitney giggles and blushes. “<i>Just in case you’re feeling adventurous, [master]. Or, um, want help.</i>”' );
		}
		MainView.outputText( 'You might be able to recognize those but you have no idea what the final object you can make out in the room’s dusky gloom is, stood against the far wall. It’s at least four foot tall and looks like an ornate stone urn, with a round bottom, a thin body, some sort of square contraption at the top, and a series of long tubes emerging from its stem. There’s a very small brazier next to it, with a few pebbles of charcoal huffing feebly at the bottom.' );
		...
		if( !this.whitneyDom() ) {
			MainView.outputText( '“<i>It’s exactly what a harem owner needs, [master] - with this you could take care of me and your other sluts all night, every night!</i>”' );
		} else {
			MainView.outputText( '“<i>I could do so many things to you [master], if you were on this stuff. And for so long. Just imagine it!</i>”' );
		}
		MainView.outputText( ' You don’t feel woozy or spaced out at all - if anything, you feel slightly more aware and in control of your surroundings, it’s just that your skin feels like it’s coated with something vaguely fuzzy. ' );
	...
	FarmCorruption.prototype.orgyRoomSubMassage = function() {
		MainView.clearOutput();
		this.whitneySprite();
		MainView.outputText( 'You say in a grand, wearied tone that you’ve arrived from another hard day of bringing Mareth to its knees, and that you require the services of your pet masseuse. ' );
		...
		if( CoC.player.isBiped() ) {
			MainView.outputText( ' you clamber onto the heated slab, placing your chin on your hands on the cushion at the end' );
		} else if( CoC.player.isDrider() ) {
			MainView.outputText( ' you tick-tack over and sink your spider underbelly onto the heated marble, before resting your human half down as well. Whitney was foresighted enough to make the slab quite long, and you complete the action in comfort' );
		} else if( CoC.player.isNaga() ) {
			MainView.outputText( ' you slither over and slide onto the heated slab, placing your chin on your hands on the cushion at the end. You allow your long, heavy tail to pool onto the floor below' );
		}
		MainView.outputText( '.' );
		...
		if( CoC.player.buttRating >= 6 ) {
			...
			MainView.outputText( 'brawny' );
		}
		MainView.outputText( ' rear. After she’s finished making your butt feel red and smooth she dips a heated, greasy hand between your [hips] and' );
		if( CoC.player.hasCock() ) {
			MainView.outputText( ' grips your [cock]' );
		} else if( CoC.player.hasVagina() ) {
			MainView.outputText( ' traces the entrance of your [vagina]' );
		}
		MainView.outputText( ' for a moment... before sliding out with a throaty giggle, moving on down to your hips.' );
		...
		if( CoC.player.isBiped() ) {
			...
			if( !(CoC.player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_HOOFED || CoC.player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_NAGA || CoC.player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_GOO || CoC.player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_PONY) ) {
				MainView.outputText( ' Your mouth opens as a twinge of pure pleasure veins up your [leg] as she massages the soft arch of the sole, making your' );
				if( CoC.player.hasCock() ) {
					MainView.outputText( ' cock thicken' );
				}
				if( CoC.player.hasCock() && CoC.player.hasVagina() ) {
					MainView.outputText( ' and your' );
				}
				if( CoC.player.hasVagina() ) {
					MainView.outputText( ' pussy moisten' );
				}
				MainView.outputText( '.' );
				MainView.outputText( ' There must be a direct nerve link leading right from the bottom of your body up to your groin because my word, that really shouldn’t feel as good as it does. She swirls her thumbs across the rougher pads of your feet, dipping her warm fingers in and around the valleys of your' );
				if( CoC.player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_DEMONIC_CLAWS || CoC.player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_LIZARD || CoC.player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_HARPY || CoC.player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_DRAGON ) {
					MainView.outputText( ' claws' );
				} else {
					MainView.outputText( ' toes' );
				}
				MainView.outputText( ', before returning deliberately to stroke at your arch, indulging that nervous link until you are deep in the unexpected bliss of it... before slowly releasing, leaving you to wallow delightfully in the knowledge that that same slow, delicious attention is about to be lavished on your other [foot].' );
			} else {
		...
		// Naga: ;
			...
			if( CoC.player.tongueType === AppearanceDefs.TONUGE_SNAKE ) {
				MainView.outputText( ' You grin, flicking your forked tongue out to smell her arousal.' );
			}
			MainView.outputText( ' “<i>I imagine watching your [master] move around is almost a masochistic experience for you. Something so alien and unnerving now linked forever with pleasure and obedience. Is that how it is? You can’t stop staring at my beautiful, deadly form because it’s almost like a slutty punishment for you?</i>”' );
		...
		if( CoC.player.lust >= 33 ) {
			MainView.outputText( ' The hot tub bubbles quietly behind her and it is not just oil which is making your skin glow. This would be the moment to pursue a happy ending if you so wished.' );
		} else {
	...
	FarmCorruption.prototype.orgyRoomSubMassageHappyEnding = function() {
		MainView.clearOutput();
		this.whitneySprite();
		MainView.outputText( 'You silently rise from the slab and put your arm around your masseuse’s tight body. It radiates heat into you as you send a hand sliding down the curve of her back to squeeze at her tight, supple butt.' );
		...
		if( CoC.player.tallness >= 67 ) {
			MainView.outputText( ' With a single movement you swing her hot, compact weight over your shoulder. You growl into her ear that she’s still got plenty to do if she’s to finish her job of thoroughly relaxing every part of you.' );
			...
			MainView.outputText( ' You tighten your grip on her pert behind and growl that she’s still got plenty to do if she’s to finish her job of thoroughly relaxing every part of you.' );
		...
		if( CoC.player.hasCock() && CoC.player.cockThatFits( this.whitneyVagCapacity() * 1.33 ) !== -1 && CoC.player.hasVagina() ) {
		...
		if( CoC.player.hasVagina() ) {
			EngineCore.addButton( 1, 'Female', this, this.subHappyEndingFemale );
		}
	};
	FarmCorruption.prototype.subHappyEndingMale = function() {
		...
		if( hasBiggerCock ) {
			...
			MainView.outputText( ' against your [hips]' );
		} else {
			MainView.outputText( ' around your back' );
		}
		MainView.outputText( ' momentarily. You grin, and begin to pump into her a little harder, using the full breadth of movement afforded to you by your oiled, [hips] to do so. It has the effect you anticipated. After a few more moments gamely keeping time with your own heavy exhalations the dog girl you’re buried into gasps, unable to contain herself. You winch her legs up by the knees and go at her faster again, still using the full movement of your pelvis to withdraw and sink every inch of your [cock ' + cockThatFits + '], pushing into her pink tightness vigorously enough for the water around to splash briskly, buried into her even deeper. ' );
	...
	FarmCorruption.prototype.subHappyEndingFemale = function() {
		...
		if( CoC.player.wetness() >= 5 ) {
			MainView.outputText( ' Your pussy spurts excitement gleefully with each thrust, utterly drowning her own in your juices, polluting the water around you with your musk.' );
		}
		if( CoC.player.hasCock() ) {
			MainView.outputText( ' Whitney has stroked and tantalized your [cock biggest] with her hot hands and smooth belly intermittently this whole time, and as your orgasm builds and overwhelms you she gets her just reward for it; you jet cum from [eachCock], pasting first her face and then her tight breasts with your thick cream as you ride her hard.' );
		}
	...
	FarmCorruption.prototype.happyEndingMerge = function() {
		...
		CoC.flags[ kFLAGS.MASSAGE_HAPPY_ENDINGS ]++;
		CoC.player.orgasm();
		EngineCore.dynStats( 'sen-', 1 );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	SceneLib.registerScene( 'farmCorruption', new FarmCorruption() );
} );