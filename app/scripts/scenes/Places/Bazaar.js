...
angular.module( 'cocjs' ).run( function( Utils, PerkLib, ArmorLib, ConsumableLib, AppearanceDefs, Descriptors, CoC, kFLAGS, EngineCore ) {
	...
	Bazaar.prototype.enterTheBazaar = function() {
		if( CoC.getInstance().time.hours === 19 || CoC.getInstance().time.hours === 20 ) {
			CoC.getInstance().flags[ kFLAGS.COUNTDOWN_TO_NIGHT_RAPE ]++;
			if( CoC.getInstance().flags[ kFLAGS.COUNTDOWN_TO_NIGHT_RAPE ] % 4 === 0 && CoC.getInstance().player.gender === 1 ) {
	...
	//[Enter];
	Bazaar.prototype.enterTheBazaarAndMenu = function( demons ) {
		if( demons === undefined ) {
			demons = true;
		}
		EngineCore.outputText( '', true );
		var rat = 'Rat';
		var lilium2 = 'Demon';
		if( CoC.getInstance().time.hours >= 15 && CoC.getInstance().time.hours <= 20 && CoC.getInstance().flags[ kFLAGS.CINNABAR_NUMBER_ENCOUNTERS ] > 0 ) {
			rat = 'Cinnabar';
		}
		...
		if( CoC.getInstance().time.hours >= 9 && CoC.getInstance().time.hours <= 17 ) {
			if( (CoC.getInstance().flags[ kFLAGS.FEMOIT_NEXTDAY_EVENT_DONE ] === 1 && CoC.getInstance().time.days >= CoC.getInstance().flags[ kFLAGS.FEMOIT_NEXTDAY_EVENT ]) || CoC.getInstance().flags[ kFLAGS.FEMOIT_NEXTDAY_EVENT_DONE ] !== 1 ) {
		...
		if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00267 ] > 0 ) {
			lilium2 = 'Lilium';
		}
		if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00292 ] === 0 && Utils.rand( 4 ) === 0 && demons ) {
			this.overHearDemonsAboutSyrena();
			return;
		}
		if( (CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00292 ] === 1 || CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00292 ] === 2) && demons && Utils.rand( 10 ) === 0 ) {
			//[Repeat Variant];
		...
		var niamh = null;
		if( CoC.getInstance().flags[ kFLAGS.NIAMH_STATUS ] > 0 && CoC.getInstance().flags[ kFLAGS.NIAMH_MOVED_OUT_COUNTER ] === -1 ) {
	...
	Bazaar.prototype.theSlipperySqueeze = function() {
		...
		//Big balls variant;
		else if( Utils.rand( 10 ) === 0 ) {
			this.joeyBigBalls();
			return;
		}
		//Cock Milker;
		else if( CoC.getInstance().player.hasKeyItem( 'Cock Milker' ) < 0 && CoC.getInstance().flags[ kFLAGS.JOEY_OFFERED_MILKER ] === 0 && CoC.getInstance().player.hasKeyItem( 'Cock Milker - Installed At Whitney\'s Farm' ) < 0 ) {
			EngineCore.outputText( 'You notice Joey leaning on the counter, lost in thought.  The bunny boy frowns, troubled by something.  He jolts upright when he notices you approaching, his expression lifting into a polite grin.  "<i>Hey, welcome!  Always nice to see a familiar face.  So, do you have some \'tension\' that needs relieving?</i>"  His eyes travel down to your crotch.  Shameless as ever but with a hesitant offer buried in his eyes...\n\n' );
			CoC.getInstance().flags[ kFLAGS.JOEY_OFFERED_MILKER ] = 1;
		}
		//(Repeat visit: ;
		else {
			EngineCore.outputText( 'Joey purses his glossed lips when you enter and coyly cocks one of his ears as he says, "<i>Welcome back to \'The Slippery Squeeze\', ' + CoC.getInstance().player.short + '.  Would you like me to give you a nice, salty rub-down?  Or would you prefer Sara do it?  Just remember, she can\'t make her own \'oil\' like I can.</i>"\n\n', false );
			//No scenes for Sara yet!;
			EngineCore.outputText( '"<i>I told you I\'m not working until you give me a raise!  Do the damned massages yourself!</i>"\n\nJoey blushes and apologies.  "<i>I guess I\'m the only one available for now.</i>"\n\n', false );
			EngineCore.outputText( 'The price list indicate it\'s 10 gems for a massage, though the gleefully illustrated \'oil\' reminds you just what they plan to use on you.\n\n', false );
			if( CoC.getInstance().player.findPerk( PerkLib.Androgyny ) < 0 ) {
				EngineCore.outputText( 'There also appears to be an option for a special \'Androgyny Treatment\' that costs 500 gems.  Joey catches you looking at it and mutters, "<i>That treatment isn\'t fun like our massages.  It would unlock the full potential of your visage, allowing it to be as masculine or feminine as possible.</i>"\n\n', false );
		...
		if( CoC.getInstance().flags[ kFLAGS.JOEY_OFFERED_MILKER ] > 0 && CoC.getInstance().player.hasKeyItem( 'Cock Milker' ) < 0 && CoC.getInstance().player.hasKeyItem( 'Cock Milker - Installed At Whitney\'s Farm' ) < 0 ) {
	...
	Bazaar.prototype.joeyAndrogyny = function() {
		...
		EngineCore.statScreenRefresh();
		EngineCore.outputText( 'Joey takes your hand in his and insistently pulls towards the back rooms.  "<i>Once we have this all done ', false );
		if( CoC.getInstance().player.gender === 1 ) {
			EngineCore.outputText( 'you\'ll be able to be as cute as me!', false );
		} else if( CoC.getInstance().player.gender === 2 ) {
			EngineCore.outputText( 'you\'ll be able to be as masculine as you want!', false );
		} else {
			EngineCore.outputText( 'you\'ll be able to be as cute as me or as masculine as you want!', false );
		}
		EngineCore.outputText( '</i>" explains the bunny-eared fem-boy.   You allow yourself to be lead into a small back room, where you\'re guided into a soft, padded chair.  Joey turns about in the cramped interior, his shapely ass mere inches from your face, tail tickling your nose.  You struggle not to sneeze or stare too deeply at the curvy fem-boy\'s butt-cheeks, but it\'s hard to focus with that delicious target waggling in front of you.\n\n', false );
		EngineCore.outputText( 'At last he turns back around, holding a cork-stoppered vial of pink-hued liquid in his manicured finger-tips.  "<i>This is the stuff!  Now just close your eyes, we gotta get this worked into your skin and it stings worse than an angry wasp-girl if it gets in your eyes,</i>" says Joey.  You blink your eyelids closed, as instructed, and you feel the soft skin of the feminine bunny rubbing over your ' + CoC.getInstance().player.face() + ', working something wet and tingly into your ' + CoC.getInstance().player.skinDesc + '.  The delicate facial massage takes roughly an hour, but you feel fresh and relaxed once it\'s finished.\n\n', false );
		EngineCore.outputText( 'You ask for a mirror, but Joey just titters with a knowing smile on his succulent lips as he replies, "<i>Oh, you haven\'t changed at all ' + CoC.getInstance().player.mf( 'handsome', 'dear' ) + '.  This will let you change it to whatever extreme you like, but those kinds of facials aren\'t a service we currently offer.  I do hear that there\'s a goblin salon in the mountains that might be able to help you finish up your look though!</i>"\n\n', false );
	...
	Bazaar.prototype.joeyMassage = function() {
		...
		if( CoC.getInstance().player.gems < 10 ) {
			EngineCore.outputText( 'Joey frowns when you realize you don\'t have the 10 gems.  He apologizes, "<i>I\'m sorry, ' + CoC.getInstance().player.short + ' but I can\'t give freebies - our special potions cost us plenty.', false );
			EngineCore.doNext( this.enterTheBazaar );
			return;
		}
	...
	Bazaar.prototype.joeysMassageWifNoExtraJizz = function() {
		EngineCore.outputText( '', true );
		//(Continue as NoWang);
		EngineCore.outputText( 'The rabbit-eared fem-boy climbs back onto the table and strokes himself a few times over your ' + Descriptors.assDescript() + '; the first drops of his \'special oil\' feel hot as they land on the curves of your butt cheeks.  He climbs over you, touching himself just enough to stay hard while his cum-drooling cock stops dripping and starts genuinely leaking.  A long trail of bunny-spunk is dripped onto your ' + Descriptors.assDescript() + ' until you\'re glazed with thick ropes of it.  You spot his discarded thong on the floor and giggle as you feel him flip around to put his cute bunny-butt on your shoulders.  His spunk immediately runs down your spine, even as his hands smear it all over your ' + CoC.getInstance().player.skinDesc + '.\n\n', false );
		EngineCore.outputText( 'The massage heads back towards your ' + Descriptors.buttDescript() + '; Joey\'s hands fill with your flesh as he fondles and strokes, spreading the jism into every nook and cranny, even your ' + Descriptors.assholeDescript(), false );
		if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( ' and ' + Descriptors.vaginaDescript( 0 ), false );
		}
		EngineCore.outputText( '.  The strange, slippery feeling would\'ve made you jump if you weren\'t so thoroughly relaxed, but the warmth of the room and sureness of your masseuse\'s touch only serve to stoke your lust as effectively as he had the oven\'s fire.  You mewl happily when he slides his cum-soaked fingers up your back, spreading the sloppy mess over you like icing on a cake.\n\n', false );
		EngineCore.outputText( 'Joey stands and turns, his cum drooling into your ' + Descriptors.hairDescript() + ' as he asks, "<i>Would you roll over for me?</i>"  Eager for more of his skilled massage and impelled by your own growing arousal, you roll over, letting his jism drip onto your face.  He sits down gently, resting his barely-felt weight atop your bellybutton, and in no time, his fingers are smearing more of his fragrant, constantly-leaking goo over your ' + Descriptors.chestDesc() + '.  The long-eared fem-boy rubs around your ' + Descriptors.nippleDescript( 0 ) + 's, trailing delicate circles that make the perky, pink flesh harden and glisten.\n\n', false );
		EngineCore.outputText( 'A soft, slippery hand takes your own and guides it to one of your ' + Descriptors.nippleDescript( 0 ) + 's, bidding you to play with it.  You do enthusiastically, even while Joey\'s same hand gathers more of the salty white lotion to smear over the front of your shoulders and arms, working it into your pores as effectively as he works the tension from you.  Unable to deny your growing need, you lift your head to watch with perverse fascination.  Joey is blushing hard, but he holds himself immobile while the whitish goop continually pours from him.  While his dick would\'ve been considered average back home, here, in this place, he\'s tiny.  He really does remind you of a lotion dispenser, pumping out globs of sticky whiteness to be rubbed into one\'s skin.\n\n', false );
		EngineCore.outputText( 'You catch a view of your hand as it ', false );
		if( !CoC.getInstance().player.hasFuckableNipples() ) {
			EngineCore.outputText( 'tweaks and teases', false );
		} else {
			EngineCore.outputText( 'sinks a finger deep inside', false );
		}
		EngineCore.outputText( ' your ' + Descriptors.nippleDescript( 0 ) + ', and you raise your other to match it.  Joey smiles and pants, "<i>Yes, we need to get out allll your tension, even that... uh... naughty... ah... sexual tension.  Mmmmm, now just lie your head back.  I\'m almost done, and we always give our clients a facial treatment while we ease their worries.</i>"\n\n', false );
		EngineCore.outputText( 'The bed cradles you as you close your eyes and lie back, noting the slight change in darkness beyond your eyelids from Joey\'s new position.  Spunk begins to rain over your ' + CoC.getInstance().player.face() + ', puddling seed around your eyes and forehead before it drips down your cheeks and bubbles on your lips.  You\'re quickly distracted from the salty, cummy facial when your personal leporid lotion-dispenser ', false );
		//(fork to male or genderless, no new PG);
		//(MALE);
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( 'slides his warm, ruby lips over your ' + Descriptors.cockDescript( 0 ) + ', licking and slurping the ' + CoC.getInstance().player.cockHead() + ' like a treasured candy.  That hot, breathy embrace hugs tight to your urethral bulge, slobbering up the cock-tip.  The flexible, thin rabbit tongue swirls over your cum-slit to lap at the bubbling pre-cum, even as his quaking balls continue to bury your face in bunny-semen.', false );
			if( CoC.getInstance().player.totalCocks() > 1 ) {
				EngineCore.outputText( '  He takes your ' + Descriptors.cockDescript( 1 ) + ' with his free hand and pulls it over, giving it an affectionate, loving smooch.', false );
				if( CoC.getInstance().player.totalCocks() > 2 ) {
					EngineCore.outputText( '  The process is repeated', false );
					if( CoC.getInstance().player.totalCocks() > 3 ) {
						EngineCore.outputText( ' as necessary until ' + Descriptors.sMultiCockDesc() + ' is coated with dick-drenching bunny spit.', false );
					} else {
						EngineCore.outputText( ', coating your ' + Descriptors.cockDescript( 2 ) + ' with dick-drenching bunny spit.', false );
					}
				}
			}
			EngineCore.outputText( '  A moment later, ', false );
			if( CoC.getInstance().player.balls > 0 ) {
				EngineCore.outputText( 'his hand firmly rubs your ' + Descriptors.sackDescript() + ', and ', false );
			}
			EngineCore.outputText( 'gentle fingertips are probing between your cheeks, rubbing his dripping seed against the pucker of your ' + Descriptors.assholeDescript() + '.\n\n', false );
		}
		//(FEM/GENDERLESS) ;
		else {
			if( CoC.getInstance().player.hasVagina() ) {
				EngineCore.outputText( 'presses his ruby lips into the glistening delta of your mons', false );
			} else {
				EngineCore.outputText( 'presses a finger against the semen-soaked ring of your ' + Descriptors.assholeDescript(), false );
			}
			EngineCore.outputText( '.  He skillfully works a free hand over your slippery butt-cheek, squeezing the supple flesh while he expertly rubs your interior, stroking it with semen-lubed touches.\n\n', false );
		}
		//Fems/Genderless cum+epilogue;
		if( !CoC.getInstance().player.hasCock() ) {
			//(Genderless orgasm);
			if( CoC.getInstance().player.gender === 0 ) {
				EngineCore.outputText( 'Though his single finger makes you burn with passion, Joey\'s second slides effortlessly after it, filling your ' + Descriptors.assholeDescript() + ' with another of his cum-soaked digits.  His warm jism slides down the crack between the fingers, slowly pooling in your backdoor.  Once you\'ve adjusted, he continues to the massage, stroking and bumping your interior with the confident, practiced strokes of a professional. You pull hard on your tortured ' + Descriptors.nippleDescript( 0 ) + 's, egging up the gradual upwelling of pleasure while you lick the bunny-cream from your lips.  A moment later, the no-longer-offending digits press hard on a sensitive spot, and you\'re arching your back, screaming with pleasure.\n\n', false );
			}
			//(Female orgasm);
			else {
				EngineCore.outputText( 'Though his spit and cum-lubed tongue is quite skilled, deftly tasting your labia and channel, the bunny adds a pair of fingers to the mix, pulling the musky tunnel wide and letting more of his slippery seed inside you.  He uses it like lube, sliding his digits around while he sucks your ' + Descriptors.clitDescript() + ' ', false );
				if( CoC.getInstance().player.clitLength >= 4 ) {
					EngineCore.outputText( 'like a practiced whore fellating a john.', false );
				} else {
					EngineCore.outputText( 'like a professional pussy-licker.', false );
				}
				EngineCore.outputText( '  You give your ' + Descriptors.nippleDescript( 0 ) + 's a hard tweak, torturing them as the building pleasure grows.  Aware of this, Joey curls a finger to press directly on a sensitive spot, deep inside you, and then you\'re arching your back and howling with unleashed release.\n\n', false );
			}
			//(continuing);
			EngineCore.outputText( 'You tremble and moan for some time, until you realize there\'s no more cum dripping onto your face and your pleasantly tingly hole is empty.   Joey pads around you, rubbing your neck sensually as you blink the spunk from your eyes and lick his flavorful spooge from your lips.  A towel is placed into your hand by the breathy masseuse.  He says, "<i>There\'s a shower and more towels in the back if you want to clean up.</i>"  You note that he\'s already got his thong back on, but the front is dark and nearly spherical from what he\'s leaking into it.  He turns to leave you, and you\'re given a better view – his thong is bulging from between his legs all the way around to his ass.  The feminine bunny-boy is still cumming, and his special thong seems to be directing it all into his back door.  Kinky.', false );
		}
		//(Dickgasms;
		else {
			EngineCore.outputText( 'Though Joey\'s mouth is making you burn with passion, it\'s the feeling of a single, intruding fingertip against your ' + Descriptors.assholeDescript() + ' that puts you on edge.  His warm jism slides down his finger, dripping into your violated backside with each slow pump.  Once you\'ve adjusted, he continues the rectal massage, gently caressing the sides of your prostate while his mouth stays busy with your ' + Descriptors.cockDescript( 0 ) + '.  The bunny\'s tongue thrashes, twisting like a slippery eel over your length, his cheeks hollowing from the suction he\'s applying.  A moment later, Joey curls the digit inside you to press firmly against your prostate, squeezing the sensitive organ tightly enough to make your hips pump into the air.\n\n', false );
			EngineCore.outputText( 'Globules of spooge squirt in time to your motions, feeding those ruby lips the treat them seem to ache for.  The rain of slick, syrupy cum on your face intensifies, spurting heavier ropes of bunny-cream each time you make the poof-tailed fem-boy swallow.  He gasps hard in between each semen-sucking gulp, all while giving you an impressive facial.  If you could see yourself, you would probably look like you\'re wearing enough boy-sludge to knock up an entire village.  The perverse mental image sets off a whole new set of contractions, releasing a few more waves of bliss and \'tension\'.\n\n', false );
			EngineCore.outputText( 'Joey climbs off once you\'ve wound down and laps at the cum over your eyes, nose, and mouth, cleaning you enough to see properly.  The first thing you see is a girlish, cum-stained face.  He makes a show of cleaning the spunk from himself, but throws you a towel as he does so. You lick the last of his flavorful cum from your mouth, noting that while he\'s already put his thong back on, it\'s bulging obscenely and he\'s still squirting inside!', false );
			if( CoC.getInstance().player.cumQ() >= 700 ) {
				EngineCore.outputText( '  A smile widens your ' + CoC.getInstance().player.face() + ' when you see the ', false );
				if( CoC.getInstance().player.cumQ() >= 2000 ) {
					EngineCore.outputText( 'massive, jiggling belly you\'ve given him.', false );
				} else {
					EngineCore.outputText( 'little paunch on his belly jiggle.', false );
				}
				EngineCore.outputText( '  He won\'t be hungry for some time.', false );
			}
			EngineCore.outputText( '  Joey turns and prances away, saying, "<i>There\'s a shower if you need to clean up, and be sure and visit me the next time you need help to squeeze out all that tension!</i>"  You barely hear his words, so focused are you on his cum-darkened, distended thong.  You can see it bulging between his legs, and while his pert butt sways out the door, you can see that his cum-filled thong is designed to redirect all that fluid over his taint and into his backdoor. Kinky.', false );
		}
		...
		if( CoC.getInstance().player.lib > 80 ) {
	...
	Bazaar.prototype.joeysMassageWithEXTRASpooge = function() {
		EngineCore.outputText( '', true );
		EngineCore.outputText( 'The rabbit-eared fem-boy pulls the cork on another bottle and helps you to roll to your side to drink it.  He holds the lip of the bottle to your lips and raises the bottom slowly, giving you just enough time to guzzle it without drowning.  It\'s sweet and syrupy, though there\'s an undertone of spicy strangeness that you can\'t quite place.  Whatever the secret ingredients are, you\'ll never figure them out from taste alone.  You feel warmth once you\'ve finished, and a tightness settles ', false );
		if( CoC.getInstance().player.balls === 0 ) {
			EngineCore.outputText( 'inside you', false );
		} else {
			EngineCore.outputText( 'in your ' + Descriptors.ballsDescriptLight(), false );
		}
		EngineCore.outputText( ' that reminds you of the sensation just before orgasm.', false );
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( '  They even feel a little bigger.', false );
		}
		EngineCore.outputText( '\n\n', false );
		EngineCore.outputText( 'Joey\'s fingertips brush along your shaft, squeezing it with tender touches that make it stiffen and thicken.  He starts slowly jacking you off while his other hand traces one of your nipples.  "<i>We\'ve got to get you nice and hard now so that you can let out all that nice, creamy lotion,</i>" explains the fem-boy.  You nod in understanding, blushing hard while he fondles ' + Descriptors.sMultiCockDesc() + ' with soft caresses.  He plays your manhood', false );
		if( CoC.getInstance().player.cockTotal() > 1 ) {
			EngineCore.outputText( 's like fiddles', false );
		} else {
			EngineCore.outputText( ' like a fiddle', false );
		}
		EngineCore.outputText( ', expertly running his fingertip around the sensitive ' + CoC.getInstance().player.cockHead() + ' before tracing down along your rapidly filling urethra.  It feels good – better than it should, and the warmth inside you begins to leak into the bunny-boy\'s waiting hand in moments.\n\n', false );
		EngineCore.outputText( 'You\'re rolled back on to your front, crushing ' + Descriptors.sMultiCockDesc() + ' between you and the sheets.  Joey leaps back atop you, straddling your back and facing your ' + Descriptors.assDescript() + ', his hands locking onto the steam-moistened cheeks.   He slides forward slightly, placing his hardness between them, and it\'s then that you notice his discarded thong on the floor.  A moment later the first drops of Joey\'s own hot seed are dripping over your exposed derriere.  He slides himself through your buns, hotdogging the rapidly-slickening surface of your ass while his hands massage the tense flesh a little more enthusiastically than they ought to.\n\n', false );
		EngineCore.outputText( 'Of course you don\'t complain, not with the growing puddle forming between you and the cum-soaked sheets.  Every time your masseuse shifts, the movement makes you slide in your sticky mess, the cum-lubed friction of the sheets helping the constant jizz-flow to thicken into a steady river of seed.  ', false );
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( 'Your balls pull and relax and pull tight, bouncing below you over and over, all while working hard to produce more juice for you to gush.  ', false );
		}
		EngineCore.outputText( 'Even though you\'re draining spunk at an alarming rate, it doesn\'t feel like a normal orgasm.  There\'s no wave of pressure and subsequent release, just a constant, pulsing contraction that makes you melt with ever-increasing feelings of satisfaction.\n\n', false );
		EngineCore.outputText( 'Joey giggles, turning about to put his lotion-dispenser on the small of your back, and you feel the hot bunny-spoo pour onto you in a wave.   It\'s gathered up and pushed up your back.  Globs of it roll down your side, sliding over the semi-waterproofed sheets to mix with your growing spunk-lake.  With the slippery-seed aiding Joey\'s massage, he somehow manages to work out even more of your tension.  By this point your muscles feel like jello melting in the afternoon sun.  You feel like you could melt into your cum, and you sigh in bliss', false );
		if( CoC.getInstance().player.cumQ() >= 1500 ) {
			EngineCore.outputText( '.  The sound of your rivers of spooge splattering against the floor barely registers, but a part of you is proud to be so incredibly fertile', false );
		}
		EngineCore.outputText( '.\n\n', false );
		EngineCore.outputText( 'You\'re prodded insistently, the masseuse saying, "<i>Come on, we\'ve got so much more of your body to work on before you\'re completely emptied of all that nasty stress.  Now roll over, don\'t be shy about the mess.  I\'ll be making it messier.</i>"  True to his word, Joey is still drooling his \'special lotion\' over your ' + CoC.getInstance().player.legs() + '.  You oblige and gingerly roll over, the cummy sheets squishing lewdly under you as your slime-slicked belly and cum-drooling crotch are revealed.', false );
		if( CoC.getInstance().player.cumQ() >= 1500 ) {
			EngineCore.outputText( '  Joey\'s jaw drops from the unexpectedly large volume you\'re producing, and he watches in awe as each huge globule of cum rolls off you towards the floor.', false );
		}
		EngineCore.outputText( '\n\n', false );
		EngineCore.outputText( 'The bunny-boy springs back atop you, landing hard just below your hips.  His dripping seed washes over ' + Descriptors.sMultiCockDesc() + ', and the sudden onslaught of fresh, liquid warmth on your groin sets off a small explosion of jism that splatters into your chin, leaving a long trail of slime behind like a snail.  The long-eared girly-boy smiles and shifts to rub his small cock against your ' + Descriptors.cockDescript( 0 ) + ', frotting you aggressively while you both spray cum like faucets with the knobs torn off.  You don\'t mind that he seems to have forgotten the massage, and you run your hands up and down your ' + Descriptors.chestDesc() + ' to smear the heavy loads ', false );
		if( CoC.getInstance().player.skinType !== AppearanceDefs.SKIN_TYPE_FUR ) {
			EngineCore.outputText( 'over your ' + CoC.getInstance().player.skinDesc, false );
		} else {
			EngineCore.outputText( 'through your fur', false );
		}
		EngineCore.outputText( '.\n\n', false );
		EngineCore.outputText( 'The special potion makes it so your orgasm is long and languid, oozing out for minutes instead of seconds, and the small chamber is filled with breathy, exultant cries of passion.  Joey leans forward and grabs hold of your ' + Descriptors.nippleDescript( 0 ) + 's, ', false );
		if( !CoC.getInstance().player.hasFuckableNipples() ) {
			EngineCore.outputText( 'squeezing and tugging on them', false );
		} else {
			EngineCore.outputText( 'sliding cum-coated fingers into their slippery depths', false );
		}
		EngineCore.outputText( ' while he grinds and cums on your ' + Descriptors.cockDescript( 0 ) + '.  "<i>Ungg... yes... gotta... squeeze... allofitooooouuuuut!</i>" he pants.  He lets go of one tender nipple to reach underneath you, and before you can react, his cum-soaked finger is inside your ' + Descriptors.assholeDescript() + ', pressing on your prostate.\n\n', false );
		EngineCore.outputText( 'Your eyes cross and you black out, twitching weakly as a regular orgasm bursts on top of your already-protracted potion-gasm.  A pleasant, heavily sexualized dream is interrupted by a finger poking your lotion-lubricated cheek and the best you can manage is an utterly contented "<i>mmmm.</i>"  A warm, moist towel is rubbed over your ' + CoC.getInstance().player.face() + ', wiping away the spooge you blasted over it, and now that you can see him, Joey says, "<i>You had me worried for a minute there!  Keep the towel, it\'s on the house, and if you need to clean up, there\'s a shower in the back!', false );
		if( CoC.getInstance().player.cumQ() >= 1500 ) {
			EngineCore.outputText( '  You really flooded the room though, so you\'d best walk on your tip-toes until we can get a succubi or a goblin in here to clean up.', false );
		}
		EngineCore.outputText( '  Come back ANY time, ' + CoC.getInstance().player.short + '.  I\'d love to help you release again.</i>"\n\n', false );
		EngineCore.outputText( 'Joey leaves, his poofy tail bobbing back and forth.  You can see his thong is distended, virtually packed with his own still-pumping spooge, and you marvel at his perverse ingenuity when you realize his thong is waterproofed and shaped to guide all the jizz between his thighs and into his back-door.  Kinky.', false );
	...
	Bazaar.prototype.joeyBigBalls = function() {
		EngineCore.outputText( '', true );
		//(FIRST TIME) ;
		if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00348 ] === 0 ) {
			EngineCore.outputText( 'Before you can even clear the door-frame, Joey the bunny-boy masseuse launches himself into you, his hands clutching wildly at your ' + CoC.getInstance().player.armorName + '.  You look down at him, and his wide, open eyes stare back with panic; namely, the look of someone in over their head with no idea how to save themselves.  Worse still, his trademark thong is bulging out obscenely, cum spilling down the sides while his immensely swollen gonads threaten to burst free of the garment\'s fraying threads.  Joey babbles, "<i>Help!  I was testing the potions, and-and-and... I dunno what went wrong, b-b-but my balls are backing up faster than it dribbles out.  They feel like they\'re going to burst!!  Help meeeeee!</i>"\n\n', false );
			EngineCore.outputText( 'You push the panicked lagomorph back a pace so that you can breathe and appraise the situation.  Joey\'s legs are drenched, soaked with sloppy spooge.  His thong is on the verge of bursting.  Most notable, his bloated balls look more like cantaloupes than testicles, and these melons are ripening to an unseen Demeter\'s power, swelling ever-so-slightly larger with each passing second.  You estimate that there\'s precious little time.\n\n', false );
			EngineCore.outputText( 'Why is it that no one ever desperately requires your assistance with a mundane task?  Sighing, you judge', false );
			if( CoC.getInstance().player.cor > 60 ) {
				EngineCore.outputText( ', with no more than a bit of wishful thinking,', false );
			}
			EngineCore.outputText( ' that if someone were to apply suction to his member, it might help to vent the thick-flowing jism fast enough to help out Joey.  There doesn\'t seem to be anyone else around, so it would have to be you, but with how the parlor\'s potions work, you might be sucking for a long time...  Then again, if you made have him masturbate, it would probably stimulate his muscles to force all that fluid out.  Of course, if you tell him that, he\'ll probably run into a back room to try it and leave you be', false );
			if( CoC.getInstance().player.cor > 70 ) {
				EngineCore.outputText( '; you won\'t get to watch him fountaining all that pearly spunk like a perverted statue', false );
			}
		...
		CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00348 ]++;
	};
	//Masturbate It Out (work it out on the floor);
	Bazaar.prototype.joeyWanksItOut = function() {
		EngineCore.outputText( '', true );
	...
	Bazaar.prototype.suckOffJoeysGardenHose = function() {
		EngineCore.outputText( '', true );
		EngineCore.outputText( 'Smiling impishly, you say "<i>I\'ll just have to suck all that cum out then, won\'t I?</i>"  Joey blushes, cheeks reddening to match his plump lips as you yank his rapidly disintegrating thong down to the ground.  Popping free, his half-hard member bobs before your eyes, trailing a thick trail of man-slime the whole way to the ground.  The semi-turgid mass starts at only about four and a half inches long, but as your hot breath washes over it, the twitching, slimy cock grows to its full six-inch size.  Joey moans as the flow from his cock thickens.  His balls keep right on growing, and you realize that they\'re at least as big as basketballs now.\n\n', false );
		EngineCore.outputText( 'You grab the bunny-boy\'s pert ass-cheeks and pull your mouth onto his cock, his member easily sliding along your tongue, lubricated by the unholy flow of dick-juice it drips.   Sealing your lips down on his base into a vacuum-tight O-ring, you start to suck, ever-so-slightly ratcheting up the pressure on Joey\'s poor, backed up penis.  Thanks to your oral cock-pump, he quickly swells beyond his normal max.   Seven inches of dick push towards your throat, and like a valve suddenly becoming unstuck, that cock\'s cumslit suddenly dilates wide, stretched out to handle the heavy flow.\n\n', false );
		EngineCore.outputText( 'Gurgling in surprise, you nearly choke from the deluge of spooge flooding your throat.  Your tongue tingles from the salty aftertaste while you gulp down the rest of the bunny-cream.  Joey pants and pleads, "<i>Oooh... it\'s... it\'s... so good.  Feels so amazing... don\'t stop!  Please don\'t stop!</i>"\n\n', false );
		EngineCore.outputText( 'You cup the fem-boy\'s heavy, swollen sack in your hands and keep up the pressure, and he rewards you with another languid eruption of goo.  His balls shrink ever-so-slightly in your hands with every pulsating pump.  You look up while your throat works, meeting Joey\'s dazed, relieved expression with your own sultry gaze.  He shivers, delivering the next creamy deluge to you on the spot, nearly making you gag.  It\'s even more voluminous than the pulse before, and if this keeps up, you won\'t be able to swallow it down.\n\n', false );
		EngineCore.outputText( 'Joey\'s rouged lips silently mouth, "<i>Oh gods,</i>" as he delivers the next, even bigger rope to your eager maw, stuffing your mouth so quickly that his jism squirts from the corners before you can react.  You shiver from the semen overload', false );
		if( CoC.getInstance().player.minotaurAddicted() ) {
			EngineCore.outputText( ' and find yourself wishing you had one of these potions to use on a minotaur.  It would be divine!', false );
		} else {
			EngineCore.outputText( ' and keep at it.  The knowledge that you\'re helping a friend is almost as filling as his delicious cum.', false );
		}
		EngineCore.outputText( '  White goo drips down your chin towards the floor, but you note that his balls are now back down to the size of grapefruits.  You\'re making progress!\n\n', false );
		EngineCore.outputText( 'Giving the girly-boy a semen-leaking smile, you milk his cock with determination and enthusiasm, only letting dribbles escape from your mouth when you do too good a job.  Your belly gurgles unhappily as if you\'ve just eaten a huge meal, but you\'ve got to press on!  Lick and swallow, suck and slurp - you fall into a practiced rhythm as you accommodate the bunny\'s chemically-induced virility.  Soon, your bulging midsection forces you to abandon swallowing and alternate between sucking at Joey\'s shaft and spitting out his ungodly eruptions of cum.  It runs down your ' + Descriptors.allChestDesc() + ', makes a mess of your belly, glazes your ' + CoC.getInstance().player.legs() + ', and eventually comes to rest in the growing, three foot wide spunk-puddle on the floor.\n\n', false );
		EngineCore.outputText( 'Joey moans, "<i>You\'re so good at this!  B-but I still feel so pent up... so full.  I don\'t think it\'s slowing down yet!</i>"\n\n', false );
		EngineCore.outputText( 'You pull back and gasp for air, ignoring the ropes of spooge splattering across your face and into your ' + Descriptors.hairDescript() + '.  Determined not to lose your progress, you take his none-too-impressive cock in your hand and jack it, spurring his body to take over and propel even more eruptions of slick seed onto your ' + CoC.getInstance().player.face() + '.  It\'s so messy, so decadent, but it\'s ', false );
		if( CoC.getInstance().player.cor < 33 ) {
			EngineCore.outputText( 'all for a good cause, right?', false );
		} else if( CoC.getInstance().player.cor < 66 ) {
			EngineCore.outputText( 'also kind of fun, in a naughty way.', false );
		} else {
			EngineCore.outputText( 'exactly the kind of nasty stuff that gets your blood pumping.', false );
		}
		EngineCore.outputText( ' Still, his balls feel even heavier in your hands by the time you return to your post on his post.  You work him dutifully while your mind races, trying to think up a solution.\n\n', false );
		EngineCore.outputText( 'An idea bubbles up to your semen-sludged mind, and you act on it without thinking in your desperation.  You slowly squeeze Joey\'s balls in your hands and smile when his white goo pours from his cum-slit in a high-pressure wave, eventually leaking out your nose.  It takes a moment for you to cough out the salty seed.  You don\'t deviate from your task in the slightest while you see to your breathing - Joey\'s balls are kept compressed in your hands.  He wriggles and gasps, but his now-freed member flails around, squirting trails of white across you, the floor, the walls, and the ceiling.\n\n', false );
		EngineCore.outputText( 'Miraculously, his nuts shrink smaller, just a hair bigger than his original size as his seemingly-endless spray winds down.  You wipe his cum from your eyes and lap the stuff from your lips.  Joey sighs and futilely pumps his hips, ejaculating large ropes of seed all over your body, but his condition seems to have stabilized.  If you remember correctly, the potion probably has a few more minutes before it stops.  Time for some fun.\n\n', false );
		EngineCore.outputText( 'You pull Joey down into the puddle he made, instantly soaking his pink top into a salmon-tinged jizz-mess.  "<i>What\'s going on?</i>" he cries, flailing his arms through his rippling spunk-mess before he calms.  You aim his cock up towards his face and smile lewdly as his jizz starts to rain down across his body.  It doesn\'t have quite enough force to hit his ruby-painted lips, however.\n\n', false );
		EngineCore.outputText( '"<i>I\'m just having a little fun with the situation,</i>" you answer while gently pumping the bunny-boy\'s dick.  He moans beneath your touches, electric shivers of pleasure racing through him as his spurts shoot further and further, finally reaching his face.  In no time, his lips are as pink as his top, with a nice, salty shine.  Joey moans and lands a glob on his forehead.  The next creams his hair.  You gently swat his balls and watch the next shot go past his head to splatter on the side of the counter.  Purring, you muse, "<i>Oooh, you\'re getting better at this.</i>"\n\n', false );
		EngineCore.outputText( '"<i>I\'m so sorry!</i>" cries Joey as his hips grind against your hands, "<i>Something was - ah - wrong wi... with this batch.  I didn\'t mean for any of this to happen!  I t-think the demons are trying to sabotage my business!</i>"\n\n', false );
		EngineCore.outputText( 'You sigh at that, but it would make sense.  Even here, under a watchful eye, the demons can\'t seem to stop their scheming.  You rub Joey\'s balls as the last of his cum drains from his twitching, unused fuck-stick and stand up, the added weight of his jizz in your belly making it a little difficult.  The bunny-boy keeps moaning and wriggling on the floor, still coming down from his orgasmic high.\n\n', false );
		EngineCore.outputText( 'Pulling him up, you drag the both of you deeper into the trailer to find a shower, where you both clean up with a touch of good-natured teasing.  Once the cute \'trap\' and you are free of his goo, you head back to the entrance.\n\n', false );
	...
	Bazaar.prototype.overHearDemonsAboutSyrena = function() {
		EngineCore.outputText( '', true );
		if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00292 ] === 0 ) {
			EngineCore.outputText( 'A whisper of conversation catches your ear while you\'re wandering the bazaar, and overcome by curiosity, you veer towards it.\n\n', false );
			EngineCore.outputText( 'As you\'re closing in on the voices, the dialogue grows clear enough to understand.\n\n', false );
			EngineCore.outputText( '"<i>-old him if he doesn\'t finish this week\'s experiments, she\'s going to drop him into a submission tank instead of the champion!</i>" exclaims the first voice, sounding quite feminine.\n\n', false );
			EngineCore.outputText( 'A baritone response rings out, "<i>You\'re shitting me!  Syrena\'s off her fucking rocker if she thinks Lethice will let her turn another of our researchers into a gibbering slut.  We\'ve got enough of those.</i>"\n\n', false );
			...
			EngineCore.outputText( 'The male laughs and says, "<i>You belong underneath a fat dick, bitch.  Look at you, you\'ve got strings of lube trailing on the ground behind you.  You\'re too busy slutting it up to accomplish anything.  I\'m thinking I had better head back to the lab, though.  I don\'t want to wind up looking like one of the escaped factory rejects - gotta keep this perfect bod, ya know?</i>"\n\n', false );
			EngineCore.outputText( 'You get a good look at the two when the crowd parts, and wow, that incubus is right.  He\'s a curly-haired, chiseled Adonis of a man, with a bulge as over-the-top of the rest of him straining at his fashionable pants.  The demon fiddles with his collar, popping it up, and he turns to leave.  The other commiserating devil is a busty succubus in a lab coat... nothing else.  She\'s even wetter than her companion\'s words would indicate, painting a thick trail of slippery girl-slime in her wake as she moves.  The succubus\' nipples are like two hard bullets, soaking two glorious wet patches into her coat as she argues back.\n\n', false );
			EngineCore.outputText( '"<i>S-shut up.  That cunt tested some of her prototype compounds on me, remember?  That stuff turned my pussy into an insatiable furnace.  If I can get some free time... I\'ll... I\'ll... oh screw it, let\'s go rent a room and fuck a few times before we go back.  I\'ll want to be thinking clearly when I face her.  Syrena won\'t get to gloat about me jumping on her fat... juicy... mmm... cock... this time!</i>"  The succubus takes the incubus\' hand and drags him away in a hurry.\n\n', false );
			EngineCore.outputText( 'Well, that explains a lot.  The demons seem to have an active research department, though the one called Syrena does not seem to please her underlings very much.  Then again, you doubt any demonic servants are pleased with their bosses.  You can\'t wait to put a stop to their labors, but for now, there\'s nothing to do but use the bazaar or go home.\n\n', false );
		}
		//[Listen in repeat];
		else if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00292 ] === 1 ) {
			EngineCore.outputText( 'Just as before, you spot the collar-popping incubus and his lab coat-clad lover complaining about their boss.\n\n', false );
			EngineCore.outputText( '"<i>-still sore!  I can\'t believe she did that to me!</i>" groans the male.\n\n', false );
			EngineCore.outputText( 'Smiling cruelly, the slick-pussied succubus says, "<i>You deserved it.  Honestly, you turned in cum-stained reports to your boss, and you\'re surprised that she took your ass for a ride as punishment?  If you ask me, you planned all this.  Who do you think had to clean up the huge mess you left on the floor?</i>"\n\n', false );
			EngineCore.outputText( 'For once, the incubus seems speechless.  He mutters, "<i>Well, I didn\'t plan it.  I-I was just having fun with one of the new slaves - for inspiration - while I finished up the paperwork.</i>"  He looks over and punches the leering female\'s arm.  "<i>Besides, I thought you liked cleaning up my cum?  I\'ve never seen you complain before.</i>"\n\n', false );
			EngineCore.outputText( 'The succubus laughs and slaps his ass as she answers, "<i>Don\'t worry, you taste great... just not as good as the boss.  I dunno what concoction she made that did it, but her spunk is heavenly.  I could just... mmm... swallow that delicious cream all day long.  Do ya think she\'s part minotaur?</i>"\n\n', false );
			EngineCore.outputText( '"<i>Babe, you\'re sliming everywhere again.  Why don\'t we go blow off some steam?</i>"  The two horny demons run off and disappear.\n\n', false );
		}
		//[Listen in Repeat 2]  ;
		else if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00292 ] >= 2 ) {
			EngineCore.outputText( 'This time, the two chatty demons are seated near the fire, and the reason for their altered location seems clear.  The succubus\' belly is gravid to an unusual degree, utterly packed with some kind of corrupted offspring.  She\'s rubbing both her hands over the stretched skin-dome and moaning in discomfort, the packed womb squirming beneath her touches.\n\n', false );
			EngineCore.outputText( 'Meanwhile, the male incubus is knocking back a beer, grumbling, "<i>Could you take it down a notch?  It isn\'t like this is the first time you\'ve had to lug around a load of imps - don\'t be so melodramatic.</i>"\n\n', false );
			EngineCore.outputText( 'His pregnant companion growls and tugs at her undersized labcoat, failing to conceal the blueberry-colored bulge of her belly from him as she retorts, "<i>You didn\'t have to get all these fucked into you, now did you?  Hell, she even shot me up with fertility-plus first!  It feels like there\'s two dozen of the little bastards packed in there!  I figure in another day or two I won\'t even be able to walk.</i>"\n\n', false );
			EngineCore.outputText( '"<i>So does that mean you won\'t mind if I stop by whilst you\'re laid up?  I mean you can\'t get any more pregnant with imps, and it might pass the time,</i>" suggests the well-muscled male.\n\n', false );
			EngineCore.outputText( 'Smirking, the demoness retorts, "<i>Sure, but next time you fuck up, I\'m loaning Syrena some of my girly stuff to make you suffer just like this.</i>"  Her hands both point at the imp-engorged skin of her midsection for emphasis.\n\n', false );
			EngineCore.outputText( 'The incubus rises and taunts, "<i>If I fucked up as often as you did, I\'d be feeding the furnace in a factory somewhere or fed to the hellhounds.  Come on, let\'s get you home before I have to roll you back.</i>"\n\n', false );
		...
		EngineCore.doNext( this.enterTheBazaar );
		CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00292 ]++;
	};
	//'Greta\'s Garments' - Interior;
	Bazaar.prototype.gretasGarments = function() {
		EngineCore.clearOutput();
		if( CoC.getInstance().flags[ kFLAGS.FOUND_SOCKS ] === 1 && CoC.getInstance().flags[ kFLAGS.SOCK_COUNTER ] === 0 ) {
		...
		if( CoC.getInstance().flags[ kFLAGS.FOUND_SOCKS ] < 2 ) {
			...
			if( CoC.getInstance().flags[ kFLAGS.OWN_MAIDEN_BIKINI ] === 0 ) {
				EngineCore.outputText( ', except maybe a super-skimpy chain bikini that\'s chased with white and gold highlights' );
			}
		} else {
			EngineCore.outputText( 'The interior of Greta\'s Garments is surprisingly colorful.  Though it started off as an empty wagon filled with loose bolts of cloth and sewing tools, vivid fabrics now cover all the shelves.  Curtains hang from the walls in every color of the rainbow, and a single wooden mannequin stands near the counter, its surprisingly anatomically correct cock covered in a thin, lacey cock-sock.  Sitting nearby, behind the counter, is a pink-skinned succubus, busy knitting what looks like another such sock. Even with her slouching posture, you can see that her breasts are truly tremendous - mountainous mammaries that she actually rests her arms on while she knits.  She\'s completely nude, save for two thin black squares that stretch over her taut nipples (concealing absolutely nothing) and a soaked triangle that\'s even now threatening to disappear into her gushing crevice forever.' );
			EngineCore.outputText( '\n\nNoticing your gaze, she sits up a little straighter and swivels on some kind of rotating chair to face you more directly.  Her jiggling breasts slowly bounce to a stop on the counter before her as she asks, "<i>Can I interest you in something, honey?</i>"' );
		...
		EngineCore.menu();
		if( CoC.getInstance().flags[ kFLAGS.FOUND_SOCKS ] === 0 ) {
			EngineCore.addButton( 4, 'Low Stock', this.askGretaAboutInventory );
		} else {
			if( CoC.getInstance().flags[ kFLAGS.FOUND_SOCKS ] === 2 && CoC.getInstance().player.cocks.length > 0 && CoC.getInstance().player.hasSockRoom() ) {
				EngineCore.addButton( 1, 'Browse Socks', this.browseDemSocksSon );
			}
			if( CoC.getInstance().player.hasSock() ) {
	...
	Bazaar.prototype.askGretaAboutInventory = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'Curious about the lack of selection, you broach the topic with the slutty shopkeeper, asking just where the items she has for sale are at.' );
		...
		EngineCore.outputText( '\n\nGreta laughs so hard that her tits quake, scattering her knitting needles away.  Her tail deftly catches one that rolls off the crude counter\'s edge, twirling it about before stuffing it into the tight bob she has atop her head.  "<i>You think this is a sock!?  Ahahahahah!  No, dear ' + CoC.getInstance().player.mf( 'boy', 'girl' ) + ', this isn\'t an ordinary sock.  It\'s a cock-sock.  See the pattern here?  And the specially sewn gaps?  They let the wearer accentuate every sweet curve and throbbing vein on their erection, all while exposing the most sensitive bits for busy hands and hungry tongues, like mine.</i>"  She lets her tongue slowly extend out from her mouth, inch after inch of the wiggling, slick organ slowly disappearing into the valley between her tits.  She slowly retracts it with a giggle.' );
		...
		//[Back];
		if( CoC.getInstance().flags[ kFLAGS.FOUND_SOCKS ] === 0 ) {
			CoC.getInstance().flags[ kFLAGS.FOUND_SOCKS ] = 1;
		}
	...
	Bazaar.prototype.buyGretasBikini = function() {
		EngineCore.clearOutput();
		CoC.getInstance().flags[ kFLAGS.OWN_MAIDEN_BIKINI ] = 1;
		CoC.getInstance().player.gems -= 500;
		EngineCore.statScreenRefresh();
		EngineCore.outputText( 'Greta\'s eyes widen in surprise.  "<i>Really?</i>"' );
	...
	Bazaar.prototype.scarletCockSocK = function() {
		...
		CoC.getInstance().flags[ kFLAGS.SOCK_HOLDING ] = 'scarlet';
		this.cockSelectionMenu();
	};
	//Cobalt Cock-sock;
	Bazaar.prototype.cobaltCockSock = function() {
		EngineCore.clearOutput();
		...
		CoC.getInstance().flags[ kFLAGS.SOCK_HOLDING ] = 'cobalt';
		this.cockSelectionMenu();
	};
	//Gilded Cock-sock;
	Bazaar.prototype.gildedCockSock = function() {
		EngineCore.clearOutput();
		...
		CoC.getInstance().flags[ kFLAGS.SOCK_HOLDING ] = 'gilded';
		this.cockSelectionMenu();
	};
	//Amaranthine;
	Bazaar.prototype.amaranthineCockSock = function() {
		EngineCore.clearOutput();
		...
		CoC.getInstance().flags[ kFLAGS.SOCK_HOLDING ] = 'amaranthine';
		this.cockSelectionMenu();
	};

	Bazaar.prototype.cockSelectionMenu = function() {
		EngineCore.menu();
		if( (CoC.getInstance().flags[ kFLAGS.SOCK_HOLDING ] === 'amaranthine' && CoC.getInstance().player.gems >= 1000) || (CoC.getInstance().flags[ kFLAGS.SOCK_HOLDING ] === 'gilded' && CoC.getInstance().player.gems >= 3000) || (CoC.getInstance().flags[ kFLAGS.SOCK_HOLDING ] === 'cobalt' && CoC.getInstance().player.gems >= 250) || (CoC.getInstance().flags[ kFLAGS.SOCK_HOLDING ] === 'scarlet' && CoC.getInstance().player.gems >= 250) || (CoC.getInstance().flags[ kFLAGS.SOCK_HOLDING ] === 'viridian' && CoC.getInstance().player.gems >= 1000) || (CoC.getInstance().flags[ kFLAGS.SOCK_HOLDING ] === 'cockring' && CoC.getInstance().player.gems >= 100) || (CoC.getInstance().flags[ kFLAGS.SOCK_HOLDING ] === 'alabaster' && CoC.getInstance().player.gems >= 25) || (CoC.getInstance().flags[ kFLAGS.SOCK_HOLDING ] === 'wool' && CoC.getInstance().player.gems >= 10) ) {
	...
	Bazaar.prototype.pickACockForSock = function() {
		//Buy Cock-sock;
		EngineCore.clearOutput();
		EngineCore.outputText( 'You take the cock-sock over to the counter where Greta sits, knitting even more garments and place down the gems required.  "<i>Aha, good choice, honey!</i>" the succubus says, snatching up the money and stashing it away.  "<i>Now let\'s get that bad boy fitted on you.</i>"' );
		...
		if( CoC.getInstance().player.cockTotal() === 1 ) {
			...
			var button = 0;
			EngineCore.menu();
			while( button < CoC.getInstance().player.cockTotal() ) {
				if( CoC.getInstance().player.cocks[ button ].sock === '' ) {
	...
	Bazaar.prototype.cockSockTarget = function( target ) {
		EngineCore.clearOutput();
		CoC.getInstance().flags[ kFLAGS.SOCKS_BOUGHT ]++;
		//Putting it On - First Time;
		if( CoC.getInstance().flags[ kFLAGS.SOCKS_BOUGHT ] === 1 ) {
			EngineCore.outputText( 'The gravity-defying succubus gestures towards your crotch.  "<i>Well, come on then, let\'s see the tasty cock getting all dressed up,</i>" she says, her voice becoming a deep purr.  You raise your eyebrow, questioning why she needs to see that.' );
		...
		//Putting It On - Additional Times;
			...
			EngineCore.outputText( 'Greta motions with her hand, a movement that causes her mountainous cleavage to jiggle hypnotically.  "<i>Well, come on then, let\'s see the tasty cock getting all dressed up,</i>" she says, her voice becoming a deep purr.' );
	...
	Bazaar.prototype.yesPutDatSockOnMe = function( target ) {
		EngineCore.clearOutput();
		var conflict = false;
		if( CoC.getInstance().flags[ kFLAGS.SOCK_HOLDING ] === 'amaranthine' ) {
			CoC.getInstance().player.gems -= 1000;
		}

		if( CoC.getInstance().flags[ kFLAGS.SOCK_HOLDING ] === 'gilded' ) {
			CoC.getInstance().player.gems -= 3000;
		}

		if( CoC.getInstance().flags[ kFLAGS.SOCK_HOLDING ] === 'cobalt' ) {
			CoC.getInstance().player.gems -= 250;
			// if(CoC.getInstance().player.findPerk(PerkLib.PhallicRestraint) < 0) CoC.getInstance().player.createPerk(PerkLib.PhallicRestraint,0,0,0,0);;
		}
		if( CoC.getInstance().flags[ kFLAGS.SOCK_HOLDING ] === 'scarlet' ) {
			//if(CoC.getInstance().player.findPerk(PerkLib.PhallicPotential) < 0) CoC.getInstance().player.createPerk(PerkLib.PhallicPotential,0,0,0,0);;
			CoC.getInstance().player.gems -= 250;
		}
		if( CoC.getInstance().flags[ kFLAGS.SOCK_HOLDING ] === 'viridian' ) {
			if( CoC.getInstance().player.findPerk( PerkLib.LustyRegeneration ) < 0 ) {
				CoC.getInstance().player.createPerk( PerkLib.LustyRegeneration, 0, 0, 0, 0 );
				CoC.getInstance().player.gems -= 1000;
			} else {
				conflict = true;
			}
		}
		if( CoC.getInstance().flags[ kFLAGS.SOCK_HOLDING ] === 'cockring' ) {
			CoC.getInstance().player.gems -= 100;
			if( CoC.getInstance().player.findPerk( PerkLib.PentUp ) < 0 ) {
				CoC.getInstance().player.createPerk( PerkLib.PentUp, 10, 0, 0, 0 );
			} else {
				CoC.getInstance().player.addPerkValue( PerkLib.PentUp, 1, 5 );
			}
		}
		if( CoC.getInstance().flags[ kFLAGS.SOCK_HOLDING ] === 'alabaster' ) {
			CoC.getInstance().player.gems -= 25;
		}

		if( CoC.getInstance().flags[ kFLAGS.SOCK_HOLDING ] === 'wool' ) {
			CoC.getInstance().player.gems -= 10;
		}

		EngineCore.outputText( 'You nod to the busty succubus and strip off your [armor], revealing your naked body.  Greta\'s eyes light up as she looks over your body with barely-contained lust.  Finally her eyes settle onto your ' + Descriptors.cockDescript( target ) + ', and she licks her lips.  ' );
		if( !conflict ) {
			{ // There's no conflict. DO IT!!!
			}
			CoC.getInstance().player.cocks[ target ].sock = CoC.getInstance().flags[ kFLAGS.SOCK_HOLDING ];
			EngineCore.statScreenRefresh();
			EngineCore.outputText( 'With one hand she lifts your limp cock up, giving it a pleasant little stroke.' );
			EngineCore.outputText( '\n\nHer other hand approaches, her thumb, fore- and middle-fingers holding the sock open as she slips it over your ' + CoC.getInstance().player.cockHead( target ) + '.  She pulls it snugly into place and then gives your penis a little kiss.  The second her lips make contact with your flesh, a chill runs across your body, followed by a flood of warmth.' );
	...
	Bazaar.prototype.noCockSock = function() {
		EngineCore.clearOutput();
		CoC.getInstance().flags[ kFLAGS.SOCK_HOLDING ] = 0;
	...
	Bazaar.prototype.takeOffDatSock = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'Which cock-sock would you like to get removed?' );
	...
	Bazaar.prototype.removeTargettedSock = function( index ) {
		EngineCore.clearOutput();
		//Select-A-Cock!;
		EngineCore.outputText( 'You walk up to the counter top.  Greta the succubus looks up at you over her latest creation, and you explain you\'d like to remove a cocksock.' );
		EngineCore.outputText( '\n\n"<i>Ah, all right then,</i>" she says smoothly, setting aside her knitting needles.  "<i>Making room for a new sock, or just looking to get rid of this one?  No matter, it\'s a simple counterspell.</i>"  Greta stands up from her chair, though she\'s only on her feet for a moment before she kneels down in front of you, placing one hand under your ' + Descriptors.cockDescript( index ) + '.  With her free hand, she runs a little circle around your ' + CoC.getInstance().player.cockHead( index ) + ', muttering something under her breath.' );
		EngineCore.outputText( '\n\nSuddenly your cock feels white-hot, burning with passionate arousal.  It jumps to attention immediately' );
		if( CoC.getInstance().player.cockArea( index ) >= 100 ) {
			EngineCore.outputText( ', almost knocking Greta over in the process' );
		}
		EngineCore.outputText( ', the cock-sock suddenly feeling unforgivingly tight.  With a light giggle, Greta gives your dick a soft kiss, and the burning arousal seems to dissipate, replaced with a cool, relaxing sensation that spreads throughout your body.' );
		...
		var storage = CoC.getInstance().player.cocks[ index ].sock;
		CoC.getInstance().player.cocks[ index ].sock = '';
		var extra = _.find( CoC.getInstance().player.cocks, function( cock ) {
			return cock.sock === storage;
		} );
		if( extra && storage === 'cockring' ) {
			if( CoC.getInstance().player.perkv1( PerkLib.PentUp ) >= 10 ) {
				CoC.getInstance().player.addPerkValue( PerkLib.PentUp, 1, -5 );
			} else {
				CoC.getInstance().player.setPerkValue( PerkLib.PentUp, 1, 10 );
			}
		} else {
			if( storage === 'gilded' ) {
				CoC.getInstance().player.removePerk( PerkLib.MidasCock );
			}
			if( storage === 'cobalt' ) {
				CoC.getInstance().player.removePerk( PerkLib.PhallicRestraint );
			}
			if( storage === 'scarlet' ) {
				CoC.getInstance().player.removePerk( PerkLib.PhallicPotential );
			}
			if( storage === 'viridian' ) {
				CoC.getInstance().player.removePerk( PerkLib.LustyRegeneration );
			}
			if( storage === 'cockring' ) {
				CoC.getInstance().player.removePerk( PerkLib.PentUp );
			}
		}
	 ...
	 */
	//'Sweet Massage';
	Bazaar.prototype.joeySweetMassage = function() {
		EngineCore.clearOutput();
		CoC.getInstance().player.gems -= 20;
		EngineCore.statScreenRefresh();
		EngineCore.outputText( 'Joey claps excitedly, his little puffy tail bouncing happily.  "<i>Great!  It\'s a customer favorite, so I\'m sure you\'ll just love, love, love it!</i>"  He takes your hand and leads you towards the back rooms, letting the door shut behind the two of you with a soft click.  As usual, in the center of the room is a padded bed meant for you, and at Joey\'s insistence you disrobe to lie down on it.  You put your face through the rounded oval and relax, taking the moment to watch Joey\'s bare feet with their cute painted nails prance in and out of sight as he bustles around the room.  He pauses briefly by the coal-fired steam-oven, stoking it for a minute before turning back towards you.' );
		EngineCore.outputText( '\n\nThe warm, steamy air caresses your ' + CoC.getInstance().player.skinFurScales() + ' even before Joey does, running down your back, butt and [legs].  In no time at all, the air in the room is thick and warm, making you feel delightfully lazy with every breath, and a bead of sweat forms on your brow.  The bed shifts a bit as Joey leaps up onto it, straddling your waist, while he starts working his fingers over your shoulders.  His petite hands are surprisingly talented, finding every little knot you\'ve accumulated in your journeys and unravelling them with one blissful touch.' );
	...
	Bazaar.prototype.eggsInButt = function( eggButt ) {
		...
		EngineCore.outputText( ', spreading his sweet bounty all over.  His fingers to go work, rubbing up and down' );
		if( !CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( ', in and out' );
		}
		EngineCore.outputText( ', removing all that pent up tension from inside you.  ' );
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( '[EachCock] begins leaking pre-cum, mixing with Joey\'s on your belly to create what looks like a chocolate-white chocolate marbled confection.' );
		} else if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( 'Your cunt grows ever wetter, and not just because of the new chocolate coating.  Despite Joey\'s femboy appearance, you\'ve got to hand it to him, he does know his way around a pussy.' );
		} else {
			EngineCore.outputText( 'Your asshole winks and clenches, aching for release that Joey is kind, and skilled, enough to give.' );
		}
		...
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( 'takes hold of [oneCock] and strokes gently, rubbing his thumb against the head' );
		} else if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( 'reaches between his legs, slipping two fingers inside you while his thumb gently rubs your clit' );
		} else {
			EngineCore.outputText( 'reaches between his legs, slipping two fingers inside your ass while his thumb rubs against your perineum' );
		}
		EngineCore.outputText( ', and with his free hand he leans forward, tweaking a [nipple] softly.' );
		...
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( '[eachCock] pulses and spasms, loosing rope after rope of white jizz through the air, splattering against the wall and floor messily' );
		} else if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( 'your pussy tightens around his fingers, suddenly releasing a flood of femcum that splatters messily against the table' );
		} else {
			EngineCore.outputText( 'your asshole twitches and spasms, unable to do much more than grant you a vague half-orgasm' );
		}
		EngineCore.outputText( '.  Your entire body tingles as you slowly descend back to the table, practically melting into it as if you were nothing more than the bunny-spunk covering your skin.' );
		...
		if( CoC.getInstance().player.hasCock() ) {
			if( CoC.getInstance().player.biggestCockArea() >= 30 ) {
				EngineCore.outputText( ', especially compared to you.' );
			} else if( CoC.getInstance().player.smallestCockArea() < 6 ) {
				EngineCore.outputText( ', of course, if he\'s considered tiny, what does that make you?' );
			}
		} else {
			EngineCore.outputText( '.' );
		}
		EngineCore.outputText( '  You suck and slurp, slipping your tongue up and down his length until not a trace of chocolate remains.  His penis continues to leak the chocolatey pre-cum, but the deeper you take him, the less it matters, until he\'s practically pumping the chocolate directly into your stomach.' );
	...
	//<Option 1 Leave>;
	Bazaar.prototype.assaultWinAndLeave = function() {
		EngineCore.clearOutput();
	...
	Bazaar.prototype.abuseHisAss = function() {
		...
		EngineCore.outputText( '\n\nYou decide to have a little fun with the muscular ass in front of you.  You hold the struggling tiger\'s wrists with one hand and fish out your ' + Descriptors.multiCockDescriptLight() + ' free from your [armor].  Your eyes drink in how the burly tiger man writhes under you.  He\'s completely at your mercy.' );
		//[(If multicock);
		if( CoC.getInstance().player.cockTotal() > 1 ) {
			EngineCore.outputText( '  Your hand strokes along your ' + Descriptors.multiCockDescriptLight() + ' and press them to the warm cheeks exposed before you.' );
		} else {
			EngineCore.outputText( '  You pump your ' + Descriptors.multiCockDescriptLight() + ' and milk a pearl of pre-cum out to drip onto the enticing ass before you.' );
		}
		EngineCore.outputText( '  "<i>O-OI!</i>"  The tiger man shouts out, his head turning to try to see behind him.  With a grin on your lips you grip the hilt of [oneCock] and slap its length over the tiger\'s rump.  You can see the tint to the tiger man\'s cheek as he hears and feels your dick smacking his exposed ass.  Pre-cum drips down [oneCock] as you line yourself up, jamming it between the tiger\'s lovely mounds. He gives a surprised shout as your cockhead presses to his tiny, puckered hole.' );
		//Cocksize check;
		//[[If smallest cock is over 16 inches);
		if( CoC.getInstance().player.smallestCockLength() >= 16 ) {
			...
			if( CoC.getInstance().player.totalCocks() === 1 ) {
				EngineCore.outputText( '  You squeeze your ' + Descriptors.multiCockDescriptLight() + ' between his warm ass cheeks, stroking yourself with his exposed bottom.  The tiger whimpers meekly under you.' );
			} else {
				EngineCore.outputText( '  Your fattest cock grinds between the tiger man\'s rear, dripping spunk over his back and clothing while your other cocks make a sloppy mess of his ass.' );
			}
			EngineCore.outputText( '  The tiger man\'s muscled butt is soon glazed with your pre-cum.  Wet, slapping noises echo out into the alley as you grind your hips into his rear.  You can\'t help but think in the back of your head that the noise is stirring people inside the tent he came out of.' );
		}
		//[[(If largest cock is smaller than 16);
			...
			//[(If multicock);
			if( CoC.getInstance().player.cockTotal() > 1 ) {
				EngineCore.outputText( '  One of your cocks slips into his hot, constricting confines, the remaining lengths pressed fast to the tiger man\'s rear and leaving glistening trails of eager fluids like markings on a bitch\'s ass.' );
			} else {
				EngineCore.outputText( '  Your ' + Descriptors.multiCockDescriptLight() + ' pushes in, your pre-cum-slicked length sliding in with a wet noise.' );
			}
			EngineCore.outputText( '  The burly man under you whimpers in protest as you begin to rock against his fine ass.  His firm rear dimples and twitches under your hips, each slam' );
			if( CoC.getInstance().player.balls > 0 ) {
				EngineCore.outputText( ' swatting your ' + Descriptors.ballsDescriptLight() + ' against him,' );
			}
			EngineCore.outputText( ' making him shout out in discomfort.  Your ' + Descriptors.multiCockDescriptLight() + ' stretching him out' );
			if( CoC.getInstance().player.cockTotal() > 1 ) {
				EngineCore.outputText( ' while beating his rear' );
			}
			EngineCore.outputText( '.  All his shouting stirs whoever resides in the tent he came out of.' );
		}
	...
	Bazaar.prototype.collectSomeButtSmex = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'Your body crashes into a wooden table and you choke, feeling the wind knocked out of you.  You struggle to lift your head and catch your breath, only to find yourself laying in... poker chips?' );
		...
		if( CoC.getInstance().player.hasCock() ) {
			...
			EngineCore.outputText( '\n\nHis fingers slip past your puckered flesh and dip into your ' + Descriptors.assholeDescript() + ' and spread apart, loosening you for what feels like ages.' );
		}
		EngineCore.outputText( '  Without warning the hand is gone' );
		if( CoC.getInstance().player.cockTotal() > 1 ) {
			EngineCore.outputText( ' and your ' + Descriptors.multiCockDescriptLight() + ' wetly slaps your stomach with need' );
		}
		EngineCore.outputText( '.  A whimper escapes your lips.  "<i>This should be fine...</i>"  The cool, collected voice of the satyr murmurs.' );
		...
		if( CoC.getInstance().player.lib < 50 ) {
			EngineCore.outputText( 'fear' );
		} else {
			EngineCore.outputText( 'excitement' );
		}
		EngineCore.outputText( '.' );
		...
		if( CoC.getInstance().player.isBiped() ) {
			EngineCore.outputText( '  The bear shifts where he stands, grabbing your ankles and spreading your limbs almost painfully wide.  You wince from the sting of the strain on your leg muscles.' );
		}
		EngineCore.outputText( '  You hear the clopping of hooves on the floor as the satyr moves over to your ' + Descriptors.buttDescript() + '.  His hands grip over your ' + Descriptors.hipDescript() + ' and drags you closer to the edge of the table so that your ' + Descriptors.buttDescript() + ' hangs over the side of the table.  You raise your head to watch with a ' );
		if( CoC.getInstance().player.lib < 50 ) {
			EngineCore.outputText( 'horrified' );
		} else {
			EngineCore.outputText( 'excited' );
		}
		EngineCore.outputText( ' expression as the satyr\'s hand strokes over his hard length.  It looks at least eight inches long, smooth like a human\'s cock and protruding from a forest of curly, brown fur.  The satyr lines himself up and rubs his stiff cockhead over your ' + Descriptors.assholeDescript() + '.' );
		EngineCore.outputText( '\n\nThe effects of the draft still fresh in your system, you moan out as your sensitive ' + Descriptors.assholeDescript() + ' is poked and prodded.  The slick, hot satyr spunk clings to your pucker and eases your entrance open with his bulbous cockhead.  The satyr calmly takes his time until your hole is dripping and ready for him.  The seemingly endless torture has erstwhile driven you mad, seeming like the satyr has waited hours before finally deciding to plunge into you.  Once satisfied, the satyr shoves his hips forward and stuffs his cock into you roughly.  You wince at the sting and bite your lower lip, trying to keep from giving them the satisfaction of hearing your whimpers.  You shut your eyes and utter out a groan regardless, the feeling of the member pulsing against your silken walls making you utter a low moan' );
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( ' while [eachCock] twitches and leaks generously over your ' + CoC.getInstance().player.skinFurScales() );
		}
		EngineCore.outputText( '.' );
	...
	Bazaar.prototype.finalGayFinallee = function( road ) {
		...
		if( road === 0 ) {
			...
			if( CoC.getInstance().player.hasCock() ) {
				EngineCore.outputText( 'His hand dips down to play with your ' + Descriptors.multiCockDescriptLight() + ', stroking you off as he gives a few more brutal pounds of his hips.  ' );
			} else if( CoC.getInstance().player.hasVagina() ) {
				EngineCore.outputText( 'Fingers ram into your ' + Descriptors.vaginaDescript( 0 ) + ', thrusting into you as he fucks your other hole.  ' );
			}
			EngineCore.outputText( 'His face near yours, he licks your cheek and plants a brief kiss on your lips. You give him a confused look at the affection and he meets it with a wide grin.  "<i>You\'re cute for a cocksucker.  Knew I had it made when I found you.</i>" he snickers, causing you to blush.' );
			if( CoC.getInstance().player.gender > 0 ) {
				EngineCore.outputText( '  Your lips part as you moan, feeling your climax building before coating the tiger\'s hand and your lower body in your sex.  The tiger man just brings his hand up to his lips to taste your juices before hilting into you one last time.' );
			}
			EngineCore.outputText( '  A third wave  of warmth floods you, the pressure growing even higher as you resemble an overstuffed, breeding bitch.  Exhaustion catches up with you and you begin to feel your eyes grow heavy.  The last thing you see is the feline\'s face contorted in pleasurable orgasm before you nod off.' );
			...
			if( CoC.getInstance().player.gems > 1 ) {
				EngineCore.outputText( ' with only a few gems missing' );
				CoC.getInstance().player.gems -= 3 + Utils.rand( 4 );
				if( CoC.getInstance().player.gems < 0 ) {
					CoC.getInstance().player.gems = 0;
				}
			}
			EngineCore.outputText( '.  You redress yourself before realizing something.  You have acquired something extra lust draft potion with an orange ribbon tied to it.  A note at the end reads: "<i>Thanks for bailing me out of buying booze.  Your ass was amazing even after that orc had it.</i>"  There\'s no name on the note but you have a good idea who left it for you, judging by the drawn tiger paw print in the corner.  You pocket the lust draft and leave the tent to head back to the bazaar in the morning light.\n\n' );
			CoC.getInstance().player.orgasm();
		...
		//((Ending 2 , suck bear, //crazy steel jaw));
			...
			EngineCore.outputText( '  His face near yours, he licks your cheek and plants a brief kiss on your lips.  You give him a confused look at the affection and he meets it with a wide grin.  "<i>You\'re cute for a cocksucker.  Loved the way you went right for my dick.</i>"  He snickers, causing you to blush.' );
			if( CoC.getInstance().player.gender > 0 ) {
				EngineCore.outputText( '  Your lips part as you moan, feeling your climax building before coating the bear\'s hand and your lower body in your sex.  The bear just brings his hand up to his lips to taste your honey before hilting into you one last time.' );
			}
			EngineCore.outputText( '  A third wave of warmth floods you, the pressure growing even higher as you resemble an overstuffed, breeding bitch.  Exhaustion catches up with you and you begin to feel your eyes grow heavy.  The last thing you see is the bear\'s face contorted in pleasurable orgasm before you nod off.' );
			EngineCore.outputText( '\n\nYou wake up with a start, the sound of morning birds echoing in the abandoned tent is all that greets you.  You try to get up quickly but immediately regret it.  Your body is sore from all the abuse you took last night.  You slide off the table and look around for your gear.  Shockingly enough, you find it folded up in a neat bundle' );
			if( CoC.getInstance().player.gems > 1 ) {
				EngineCore.outputText( ' with only a few gems missing' );
				CoC.getInstance().player.gems -= 3 + Utils.rand( 4 );
		...
		//((Ending 3 , suck none, //No furry plx));
			...
			if( CoC.getInstance().player.hasCock() ) {
				EngineCore.outputText( '  His hand drips down to clumsily play with your ' + Descriptors.multiCockDescriptLight() + ', stroking you off as he gives you a few more brutal pounds of his hips.' );
			} else if( CoC.getInstance().player.hasVagina() ) {
				EngineCore.outputText( '  Fingers dip into your ' + Descriptors.vaginaDescript( 0 ) + ', ramming into you as he fucks your other hole.' );
			}
			if( CoC.getInstance().player.gender > 0 ) {
				EngineCore.outputText( '  Your lips part as you feel your climax building, and soon you coat the orc\'s hand and your lower body in your lust.  The orc just brings his hand up to taste your juices before hilting into you one last time.' );
			}
			EngineCore.outputText( '  A third wave of warmth floods you and the pressure grows even higher as you resemble an overstuffed breeding bitch.  Exhaustion catches up with you and your eyes begin to grow heavy.  The last thing you see is the orc\'s face twisted in orgasmic pleasure before you pass out.' );
			...
			if( CoC.getInstance().player.gems > 1 ) {
				EngineCore.outputText( 'The only thing missing is a few gems.  ' );
				CoC.getInstance().player.gems -= 3 + Utils.rand( 4 );
				if( CoC.getInstance().player.gems < 0 ) {
					CoC.getInstance().player.gems = 0;
				}
			}
			EngineCore.outputText( 'You re-dress yourself quickly before realizing that you have something extra bimbo potion with a green ribbon tied to it.  A note at the end of the ribbon reads: "<i>Zug think you make good orc bitch. Drink this and Zug fuck all your holes!</i>"  The writing is a childish scrawl, and you can barely make out the rest of the words.  By the stains on the paper, it\'s obvious the orc got too excited to properly finish his note, broken English or otherwise.  You pocket the bimbo potion and leave the tent to head back to the bazaar in the morning light.\n\n' );
			//Lust sated;
