...
angular.module( 'cocjs' ).run( function( ItemType, ArmorLib, UsableLib, AppearanceDefs, PregnancyStore, ConsumableLib, PerkLib, Descriptors, CoC, kFLAGS, Utils, StatusAffects, EngineCore ) {
	function Benoit() {
	}
	...
	Benoit.prototype.benoitMF = function( stringM, stringF ) {
		if( CoC.getInstance().flags[ kFLAGS.BENOIT_STATUS ] === 1 || CoC.getInstance().flags[ kFLAGS.BENOIT_STATUS ] === 2 ) {
			return stringF;
		}
		return stringM;
	};
	Benoit.prototype.benoitLover = function() {
		return CoC.getInstance().flags[ kFLAGS.BENOIT_TIMES_SEXED_FEMPCS ] >= 2;
	};
	Benoit.prototype.benoitAffection = function( changes ) {
		if( changes !== undefined && changes !== 0 ) {
			CoC.getInstance().flags[ kFLAGS.BENOIT_AFFECTION ] += changes;
			if( CoC.getInstance().flags[ kFLAGS.BENOIT_AFFECTION ] > 100 ) {
				CoC.getInstance().flags[ kFLAGS.BENOIT_AFFECTION ] = 100;
			} else if( CoC.getInstance().flags[ kFLAGS.BENOIT_AFFECTION ] < 0 ) {
				CoC.getInstance().flags[ kFLAGS.BENOIT_AFFECTION ] = 0;
			}
		}
		return CoC.getInstance().flags[ kFLAGS.BENOIT_AFFECTION ];
	};
	Benoit.prototype.benoitKnocksUpPCCheck = function() {
		//Convert old basi's to real basi's!;
		if( CoC.getInstance().player.pregnancyType === PregnancyStore.PREGNANCY_BASILISK && CoC.getInstance().player.findPerk( PerkLib.BasiliskWomb ) >= 0 ) {
			CoC.getInstance().player.knockUpForce( PregnancyStore.PREGNANCY_BENOIT, CoC.getInstance().player.pregnancyIncubation );
		}
		//Knock up chances:;
		if( (CoC.getInstance().player.pregnancyType === PregnancyStore.PREGNANCY_OVIELIXIR_EGGS || CoC.getInstance().player.findPerk( PerkLib.HarpyWomb ) >= 0 || CoC.getInstance().player.findPerk( PerkLib.Oviposition ) >= 0 || CoC.getInstance().player.findPerk( PerkLib.BasiliskWomb ) >= 0) && (CoC.getInstance().player.pregnancyIncubation === 0 || CoC.getInstance().player.pregnancyType === PregnancyStore.PREGNANCY_OVIELIXIR_EGGS) ) {
			if( CoC.getInstance().player.findPerk( PerkLib.BasiliskWomb ) >= 0 && CoC.getInstance().flags[ kFLAGS.BENOIT_TESTED_BASILISK_WOMB ] === 1 ) {
				if( CoC.getInstance().player.pregnancyType !== PregnancyStore.PREGNANCY_OVIELIXIR_EGGS || CoC.getInstance().player.pregnancyIncubation === 0 ) {
					CoC.getInstance().player.knockUp( PregnancyStore.PREGNANCY_BENOIT, PregnancyStore.INCUBATION_BASILISK );
				}
	...
	Benoit.prototype.benoitPreggers = function() {
		if( CoC.getInstance().flags[ kFLAGS.BENOIT_STATUS ] === 0 ) {
			return false;
		}
	...
	Benoit.prototype.benoitRegularPreggers = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.FEMOIT_EGGS ] >= 1 && CoC.getInstance().flags[ kFLAGS.FEMOIT_EGGS ] <= 4 ) {
			return true;
		}
		return false;
	};
	Benoit.prototype.benoitHeavyPreggers = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.FEMOIT_EGGS ] >= 5 && CoC.getInstance().flags[ kFLAGS.FEMOIT_EGGS ] <= 8 ) {
			return true;
		}
		return false;
	};
	Benoit.prototype.benoitVeryHeavyPreggers = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.FEMOIT_EGGS ] >= 9 && CoC.getInstance().flags[ kFLAGS.FEMOIT_EGGS ] <= 12 ) {
			return true;
		}
		return false;
	};
	Benoit.prototype.benoitExtremePreggers = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.FEMOIT_EGGS ] >= 13 ) {
			return true;
		}
		return false;
	};
	Benoit.prototype.benoitInClutch = function() {
		...
		var startDay = CoC.getInstance().flags[ kFLAGS.FEMOIT_NEXTDAY_EVENT ];
	...
	Benoit.prototype.benoitKnockUp = function() {
		...
		var cumQ = CoC.getInstance().player.cumQ();
		var bounty = (CoC.getInstance().player.findPerk( PerkLib.ElvenBounty ) >= 0) ? true : false;
		var stud = (CoC.getInstance().player.findPerk( PerkLib.MaraesGiftStud ) >= 0) ? true : false;
		var alpha = (CoC.getInstance().player.findPerk( PerkLib.FerasBoonAlpha ) >= 0) ? true : false;
		var eggMod = 0;
		if( bounty ) {
			eggMod += 1;
		}
		...
		numEggs += eggMod;
		CoC.getInstance().flags[ kFLAGS.FEMOIT_EGGS ] = numEggs;
		CoC.getInstance().flags[ kFLAGS.FEMOIT_INCUBATION ] = 168;
		return true;
	};
	Benoit.prototype.clearBenoitPreggers = function() {
		if( CoC.getInstance().flags[ kFLAGS.FEMOIT_EGGS ] !== 0 ) {
			CoC.getInstance().flags[ kFLAGS.FEMOIT_EGGS_LAID ] += CoC.getInstance().flags[ kFLAGS.FEMOIT_EGGS ];
			CoC.getInstance().flags[ kFLAGS.FEMOIT_EGGS ] = 0;
			CoC.getInstance().flags[ kFLAGS.FEMOIT_INCUBATION ] = 0;
			CoC.getInstance().flags[ kFLAGS.FEMOIT_NEXTDAY_EVENT ] = this.getGame().model.time.days; // Cycle 'resets' based off birth day.
		}
	};
	//Introduction Scenes;
	Benoit.prototype.benoitIntro = function() {
		...
		} else if( CoC.getInstance().flags[ kFLAGS.BENOIT_SUGGEST_UNLOCKED ] === 0 && CoC.getInstance().player.hasVagina() && (CoC.getInstance().player.inHeat || CoC.getInstance().player.pregnancyType === PregnancyStore.PREGNANCY_OVIELIXIR_EGGS || CoC.getInstance().player.findPerk( PerkLib.HarpyWomb ) >= 0 || CoC.getInstance().player.findPerk( PerkLib.Oviposition ) >= 0) && (CoC.getInstance().player.pregnancyType === PregnancyStore.PREGNANCY_OVIELIXIR_EGGS || CoC.getInstance().player.pregnancyIncubation === 0) && CoC.getInstance().flags[ kFLAGS.BENOIT_STATUS ] === 0 ) {
			if( CoC.getInstance().flags[ kFLAGS.BENOIT_SUGGEST_UNLOCKED ] === 0 ) {
				this.benoitAndFemPCTalkAboutEggings();
			}
			suggest = this.eggySuggest;
			suggestText = 'Suggest';
		} else if( CoC.getInstance().flags[ kFLAGS.FEMOIT_NEXTDAY_EVENT_DONE ] === 1 && CoC.getInstance().flags[ kFLAGS.FEMOIT_NEXTDAY_EVENT ] <= this.getGame().model.time.days && CoC.getInstance().flags[ kFLAGS.BENOIT_STATUS ] === 0 ) {
			this.femoitNextDayEvent();
		} else if( this.benoitInClutch() && CoC.getInstance().flags[ kFLAGS.FEMOIT_READY_FOR_EGGS ] === 0 && (CoC.getInstance().flags[ kFLAGS.BENOIT_STATUS ] === 1 || CoC.getInstance().flags[ kFLAGS.BENOIT_STATUS ] === 2) ) {
			CoC.getInstance().flags[ kFLAGS.FEMOIT_READY_FOR_EGGS ]++;
			EngineCore.outputText( 'As you enter the stall Benoite looks up; though her blind, milky eyes make her harder to read, she looks excited. "<i>Ah! [name]; it iz good you are here... I had hope zat you would come for a vizit today.</i>"' );
		...
		} else if( this.benoitPreggers() && CoC.getInstance().flags[ kFLAGS.FEMOIT_INCUBATION ] < 0 ) {
			// Missed it by a week;
			if( CoC.getInstance().flags[ kFLAGS.FEMOIT_INCUBATION ] < -168 ) {
				EngineCore.outputText( 'When you enter the stall, you notice that Benoite\'s stomach is flat again. She gives you a toothy grin when you enter her stall. "<i>I have good newz [name]! You are a fazer! ' + String( CoC.getInstance().flags[ kFLAGS.FEMOIT_EGGS ] ) + ' timez over in fact! Oh I wish zat you could have helped; but I was ztrong, ztrong enough get through it on my own.</i>" she gushes, speaking at a mile-a-minute. She\'s clearly pleased with herself; and your virility.' );
				...
				CoC.getInstance().flags[ kFLAGS.FEMOIT_NEXTDAY_EVENT ] -= 7;
			} else {
				this.femoitBirths();
				return;
			}
		} else if( !this.benoitInClutch() && !this.benoitPreggers() && (this.getGame().model.time.days - CoC.getInstance().flags[ kFLAGS.FEMOIT_NEXTDAY_EVENT ] >= 30) && (CoC.getInstance().flags[ kFLAGS.BENOIT_STATUS ] > 0) ) {
			if( CoC.getInstance().flags[ kFLAGS.FEMOIT_FIRST_CLUTCH_MISSED ] === 0 ) {
				CoC.getInstance().flags[ kFLAGS.FEMOIT_FIRST_CLUTCH_MISSED ]++;
				...
				EngineCore.outputText( 'The flat-bellied basilisk is tucking away into a plate laden with a heavy omlette when you arrive.' );
				EngineCore.outputText( '\n\n"<i>Ah, [name]! What can I azzist you wiz?</i>"' );
			}
		} else {
			//Subsequent Visit, Affection 0-10: ;
			if( this.benoitAffection() <= 10 ) {
				EngineCore.outputText( 'Once again, you carefully enter the gloom of Benoit\'s salvage shop.  The proprietor sniffs the air as you enter, and then looks at you sourly.  "<i>Well?</i>" he rasps.' );
			}//Benoit reacts after fucking the PC.;
			else if( CoC.getInstance().flags[ kFLAGS.BENOIT_TIMES_SEXED_FEMPCS ] === 1 && CoC.getInstance().flags[ kFLAGS.BENOIT_POST_FIRSTFUCK_TALK ] === 0 ) {
				this.firstTimeAfterBoningEncounterBenoit();
			} else if( this.benoitAffection() < 35 ) {
				EngineCore.outputText( 'Once again, you carefully enter the gloom of Benoit\'s salvage shop.  The proprietor sniffs the air as you enter, and then relaxes in his seat.  "<i>Allo again, ' + CoC.getInstance().player.short + '.  What brings you \'ere?</i>"' );
			}//Subsequent Visit, Affection 35+ but pre-lover/pre-fem: ;
			else {
				EngineCore.outputText( 'Once again, you carefully enter the gloom of ' + this.benoitMF( 'Benoit', 'Benoite' ) + '\'s salvage shop.  The proprietor sniffs the air as you enter, and then smiles widely.  "<i>If it isn\'t my favorite customer!  Do not \'ang around out zere, [name]; please, come in and let us, \'ow you say, chew ze fat.</i>"' );
				// Preggers stuff;
		...
		if( CoC.getInstance().flags[ kFLAGS.BENOIT_SUGGEST_UNLOCKED ] > 0 && CoC.getInstance().player.hasVagina() && CoC.getInstance().flags[ kFLAGS.BENOIT_STATUS ] === 0 ) {
			suggest = this.eggySuggest;
			suggestText = 'Suggest';
		}
		if( CoC.getInstance().player.hasCock() && CoC.getInstance().flags[ kFLAGS.BENOIT_STATUS ] > 0 && CoC.getInstance().player.lust >= 33 ) {
			suggest = this.femoitSexIntro;
			suggestText = 'Sex';
		}
		CoC.getInstance().flags[ kFLAGS.TIMES_IN_BENOITS ]++;
		if( CoC.getInstance().flags[ kFLAGS.BENOIT_WOMB_TALK_UNLOCKED ] === 1 && CoC.getInstance().player.findPerk( PerkLib.BasiliskWomb ) < 0 && CoC.getInstance().flags[ kFLAGS.BENOIT_TESTED_BASILISK_WOMB ] === 0 && CoC.getInstance().flags[ kFLAGS.BENOIT_STATUS ] === 0 ) {
			womb = this.tryToConvertToBassyWomb;
		}
		var fem;
		if( CoC.getInstance().flags[ kFLAGS.FEMOIT_UNLOCKED ] === 1 && CoC.getInstance().flags[ kFLAGS.BENOIT_STATUS ] === 0 ) {
			// Only available if the player hasn't since fucked Benoit after unlocking the option.;
			if( CoC.getInstance().flags[ kFLAGS.BENOIT_TIMES_SEXED_FEMPCS ] === 0 ) {
		...
		EngineCore.choices( 'Buy', this.benoitsBuyMenu, 'Sell', this.benoitSellMenu, 'Talk', this.talkToBenoit, suggestText, suggest, 'Basil. Womb', womb,
			'Feminize', fem, '', null, '', null, '', null, 'Leave', CoC.getInstance().scenes.bazaar.enterTheBazaar );
	};
	...
	Benoit.prototype.buyOrSellExplanationFirstTime = function() {
		CoC.getInstance().flags[ kFLAGS.BENOIT_EXPLAINED_SHOP ]++;
		EngineCore.outputText( '"<i>If you can see something you want in Benoit\'s Spectacular Salvage Shop, and you can walk away with it, it\'s yours,</i>" replies Benoit, theatrically sweeping his claw to take in the entirety of his stall but almost knocking over a birdcage.  "<i>Assuming you can pay whatever I\'ve decided it\'s worth, of course.  If you want to unload your garbage \'ere?  Zis is also fine.  I cannot pay what ze fat cats in Tel\'Adre can, though.  Check back every day; ze Spectacular Salvage Shop always \'as new zings to sell.</i>"' );
	};
	Benoit.prototype.benoitsBuyMenu = function() {
		EngineCore.clearOutput();
		if( CoC.getInstance().flags[ kFLAGS.BENOIT_1 ] === 0 ) {
			this.updateBenoitInventory();
		}
		if( CoC.getInstance().flags[ kFLAGS.BENOIT_EXPLAINED_SHOP ] === 0 ) {
			this.buyOrSellExplanationFirstTime();
		}
		var buyMod = 2;
		if( CoC.getInstance().flags[ kFLAGS.BENOIT_STATUS ] === 1 ) {
			buyMod = 1.66;
			EngineCore.outputText( '"<i>Some may call zis junk,</i>" says Benoite, indicating her latest wares.  "<i>Me... I call it garbage.</i>"' );
		} else {
			EngineCore.outputText( '"<i>Some may call zis junk,</i>" says Benoit, indicating his latest wares.  "<i>Me... I call it garbage.</i>"' );
		}
		EngineCore.outputText( '\n\n<b><u>' + this.benoitMF( 'Benoit', 'Benoite' ) + '\'s Prices</u></b>', false );
		EngineCore.outputText( '\n' + ItemType.lookupItem( CoC.getInstance().flags[ kFLAGS.BENOIT_1 ] ).longName + ': ' + Math.round( buyMod * ItemType.lookupItem( CoC.getInstance().flags[ kFLAGS.BENOIT_1 ] ).value ) );
		EngineCore.outputText( '\n' + ItemType.lookupItem( CoC.getInstance().flags[ kFLAGS.BENOIT_2 ] ).longName + ': ' + Math.round( buyMod * ItemType.lookupItem( CoC.getInstance().flags[ kFLAGS.BENOIT_2 ] ).value ) );
		EngineCore.outputText( '\n' + ItemType.lookupItem( CoC.getInstance().flags[ kFLAGS.BENOIT_3 ] ).longName + ': ' + Math.round( buyMod * ItemType.lookupItem( CoC.getInstance().flags[ kFLAGS.BENOIT_3 ] ).value ) );
		EngineCore.choices( CoC.getInstance().flags[ kFLAGS.BENOIT_1 ], EngineCore.createCallBackFunction( this.benoitTransactBuy, 1 ),
			CoC.getInstance().flags[ kFLAGS.BENOIT_2 ], EngineCore.createCallBackFunction( this.benoitTransactBuy, 2 ),
			CoC.getInstance().flags[ kFLAGS.BENOIT_3 ], EngineCore.createCallBackFunction( this.benoitTransactBuy, 3 ),
	...
	Benoit.prototype.benoitSellMenu = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.BENOIT_EXPLAINED_SHOP ] === 0 ) {
			...
			EngineCore.outputText( '"<i>Let us feel what you are trying to palm off upon me zis time, zen,</i>" sighs Benoit' + this.benoitMF( '', 'e' ) + ', sitting down and opening ' + this.benoitMF( 'his', 'her' ) + ' hand to you.' );
		}
		var sellMod = 3;
		if( CoC.getInstance().flags[ kFLAGS.BENOIT_EGGS ] > 0 || CoC.getInstance().flags[ kFLAGS.BENOIT_STATUS ] !== 0 ) {
			sellMod = 2;
		}
		...
		for( var slot = 0; slot < 5; slot++ ) {
			if( CoC.getInstance().player.itemSlots[ slot ].quantity > 0 && Math.ceil( CoC.getInstance().player.itemSlots[ slot ].itype.value / sellMod ) >= 1 ) {
				EngineCore.outputText( '\n' + Math.ceil( CoC.getInstance().player.itemSlots[ slot ].itype.value / sellMod ) + ' gems for ' + CoC.getInstance().player.itemSlots[ slot ].itype.longName + '.' );
				EngineCore.addButton( slot, (CoC.getInstance().player.itemSlots[ slot ].itype.shortName + ' x' + CoC.getInstance().player.itemSlots[ slot ].quantity), EngineCore.createCallBackFunction2( this.benoitSellTransact, slot, sellMod ) );
	...
	Benoit.prototype.benoitTransactBuy = function( slot ) {
		...
		if( slot === 1 ) {
			itype = ItemType.lookupItem( CoC.getInstance().flags[ kFLAGS.BENOIT_1 ] );
		} else if( slot === 2 ) {
			itype = ItemType.lookupItem( CoC.getInstance().flags[ kFLAGS.BENOIT_2 ] );
		} else {
			itype = ItemType.lookupItem( CoC.getInstance().flags[ kFLAGS.BENOIT_3 ] );
		}
		if( CoC.getInstance().player.gems < Math.ceil( buyMod * itype.value ) ) {
		...
		if( this.benoitLover() ) {
			EngineCore.outputText( 'After examining what you\'ve picked out with ' + this.benoitMF( 'his', 'her' ) + ' fingers, ' + this.benoitMF( 'Benoit', 'Benoite' ) + ' hands it over and accepts your gems with a grin.' );
		} else {
			EngineCore.outputText( 'After examining what you\'ve picked out with ' + this.benoitMF( 'his', 'her' ) + ' fingers, ' + this.benoitMF( 'Benoit', 'Benoite' ) + ' hands it over, names the price and accepts your gems with a curt nod.\n\n' );
		}
		//(+3 Affection);
	...
	Benoit.prototype.benoitSellTransact = function( slot, sellMod ) {
		EngineCore.clearOutput();
		if( this.benoitLover() ) {
			EngineCore.outputText( 'Benoit' + this.benoitMF( '', 'e' ) + ' gives your object the briefest of goings-over with ' + this.benoitMF( 'his', 'her' ) + ' fingers before stowing it away and handing over your gem reward with a trusting smile.' );
		} else {
			EngineCore.outputText( 'Following a painstaking examination of what you\'ve given him with his hands and nose, Benoit grudgingly accepts it and carefully counts out your reward.' );
		}
		CoC.getInstance().player.gems += Math.ceil( CoC.getInstance().player.itemSlots[ slot ].itype.value / sellMod );
	...
	Benoit.prototype.benoitSellAllTransact = function( totalItems, sellMod ) {
		...
		for( var slot = 0; slot < 5; slot++ ) {
			if( CoC.getInstance().player.itemSlots[ slot ].quantity > 0 && Math.ceil( CoC.getInstance().player.itemSlots[ slot ].itype.value / sellMod ) >= 1 ) {
				itemValue += CoC.getInstance().player.itemSlots[ slot ].quantity * Math.ceil( CoC.getInstance().player.itemSlots[ slot ].itype.value / sellMod );
				CoC.getInstance().player.itemSlots[ slot ].quantity = 0;
			}
		}
		if( this.benoitLover() ) {
			EngineCore.outputText( 'Benoit' + this.benoitMF( '', 'e' ) + ' gives your objects the briefest of goings-over with ' + this.benoitMF( 'his', 'her' ) + ' fingers before stowing them away and handing over your ' + Utils.num2Text( itemValue ) + ' gem reward with a trusting smile.' );
		} else {
			EngineCore.outputText( 'Following a painstaking examination of the items you\'ve given him with his hands and nose, Benoit grudgingly accepts them and carefully counts out your ' + Utils.num2Text( itemValue ) + ' gem reward.' );
		}
	...
	Benoit.prototype.updateBenoitInventory = function() {
		...
		CoC.getInstance().flags[ kFLAGS.BENOIT_1 ] = Utils.randomChoce(
		...
		if( Utils.rand( 10 ) === 0 ) {
			CoC.getInstance().flags[ kFLAGS.BENOIT_1 ] = ConsumableLib.GODMEAD.id;
		}
		...
		CoC.getInstance().flags[ kFLAGS.BENOIT_2 ] = Utils.randomChoce(
		...
		CoC.getInstance().flags[ kFLAGS.BENOIT_3 ] = Utils.randomChoce(
			ArmorLib.W_ROBES.id,
	...
	Benoit.prototype.talkToBenoit = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.BENOIT_TALKED_TODAY ] === 0 ) {
			CoC.getInstance().flags[ kFLAGS.BENOIT_TALKED_TODAY ] = 1;
			this.benoitAffection( 5 );
		}
		//Basilisk Womb;
		//Requires: Had sex with Benoit at least twice, have vagina, select talk;
		if( ((CoC.getInstance().flags[ kFLAGS.BENOIT_TIMES_SEXED_FEMPCS ] > 2 && CoC.getInstance().player.inte >= 60 && CoC.getInstance().flags[ kFLAGS.BENOIT_WOMB_TALK_UNLOCKED ] === 0) || CoC.getInstance().flags[ kFLAGS.BENOIT_TIMES_SEXED_FEMPCS ] === 2) && CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( 'You ask ' + this.benoitMF( 'Benoit', 'Benoite' ) + ' if ' + this.benoitMF( 'he', 'she' ) + ' has ever thought about trying to do something to help his people\'s plight.' );
			...
			if( CoC.getInstance().player.inte >= 60 ) {
				...
				CoC.getInstance().flags[ kFLAGS.BENOIT_WOMB_TALK_UNLOCKED ] = 1;
		...
		// First time Femoit talk;
		else if( CoC.getInstance().flags[ kFLAGS.FEMOIT_TALKED_TO ] === 0 && CoC.getInstance().flags[ kFLAGS.BENOIT_STATUS ] !== 0 ) {
			CoC.getInstance().flags[ kFLAGS.FEMOIT_TALKED_TO ]++;
			EngineCore.outputText( 'You ask Benoite if she intends to go back to the mountains now.  She laughs long and hard at this.  One thing the transformation has certainly gifted her is an extraordinarily filthy laugh.' );
			...
			return;
		} else if( CoC.getInstance().flags[ kFLAGS.BENOIT_TALKED_TO_PROPERLY ] !== 0 && this.benoitAffection() >= 40 && CoC.getInstance().flags[ kFLAGS.BENOIT_TIMES_SEXED_FEMPCS ] === 0 && CoC.getInstance().flags[ kFLAGS.FEMOIT_UNLOCKED ] === 0 ) {
		...
		//Subsequent Talk;
			...
			if( this.benoitLover() && this.benoitMF( 'he', 'she' ) === 'he' && CoC.getInstance().player.hasVagina() ) {
			...
			//(Randomly generated);
			if( choice === 0 ) {
				EngineCore.outputText( 'You ask if all basilisks talk as ' + this.benoitMF( 'he', 'she' ) + ' does.' );
				...
				EngineCore.outputText( 'You ask ' + this.benoitMF( 'Benoit', 'Benoite' ) + ' about the dog.' );
				...
				EngineCore.outputText( 'You ask ' + this.benoitMF( 'him', 'her' ) + ' about the sign above the shop.' );
				...
				EngineCore.outputText( 'You ask if ' + this.benoitMF( 'he', 'she' ) + ' has always been blind.' );
				...
				EngineCore.outputText( 'You ask ' + this.benoitMF( 'Benoit', 'Benoite' ) + ' how ' + this.benoitMF( 'he', 'she' ) + ' got into this line of work.' );
				...
				EngineCore.outputText( 'You ask Benoit if ' + this.benoitMF( 'he', 'she' ) + ' can tell you anything about the Bazaar.' );
				...
				EngineCore.outputText( 'You ask ' + this.benoitMF( 'Benoit', 'Benoite' ) + ' for any rumors going around.' );
				//[Deep cave cleared: ;
				...
				EngineCore.outputText( 'You ask if ' + this.benoitMF( 'he', 'she' ) + '\'s ever had any trouble with the demons who frequent the Bazaar.' );
				...
				EngineCore.outputText( 'You ask ' + this.benoitMF( 'Benoit', 'Benoite' ) + ' what results when basilisks mate with harpies.' );
				...
				EngineCore.outputText( 'You ask if ' + this.benoitMF( 'Benoit', 'Benoite' ) + ' really can tell who you are just by smell.' );
				...
				if( this.benoitMF( 'he', 'she' ) === 'he' ) {
					EngineCore.outputText( '  There\'s a hint of longing jealousy in the basilisk\'s voice and when he lapses into silence you decide not to push the subject.' );
				}
			}
			...
			//(Randomly generated.  Added to normal talk options after PC has had sex with Benoit two times or more regardless of womb quest.);
				...
				EngineCore.outputText( 'You ask Benoit there is anything useful he can tell you about the demon strongholds.' );
				...
				EngineCore.outputText( 'You ask Benoit if he can suggest anything to help you fight his brethren in the high mountains.' );
				...
				EngineCore.outputText( '"<i>Gnoll.</i>"' );
				...
				EngineCore.outputText( 'You ask Benoit if he really, <b>really</b> can tell who you are just by smell.' );
				...
				EngineCore.outputText( 'You ask Benoite how she’s getting on with being the opposite sex.  Benoite stops cleaning the tarnished silver plate in her hands to think.' );
				...
				if( this.benoitLover() && CoC.getInstance().player.hasCock() && CoC.getInstance().player.hasVagina() ) {
				...
				EngineCore.outputText( 'You ask Benoite if she really can tell who you are just by smell.' );
	...
	Benoit.prototype.benoitAndFemPCTalkAboutEggings = function() {
		EngineCore.clearOutput();
		CoC.getInstance().flags[ kFLAGS.BENOIT_SUGGEST_UNLOCKED ] = 1;
		EngineCore.outputText( 'Benoit seems very on edge today.  He paces up and down, returns your greeting with a stiff nod, and fiddles endlessly with the stock in reach of his counter as you search for a topic.  You ask if there\'s something wrong.' );
	...
	Benoit.prototype.eggySuggest = function() {
		EngineCore.clearOutput();
		if( CoC.getInstance().flags[ kFLAGS.BENOIT_TESTED_BASILISK_WOMB ] === 0.5 ) {
			this.suggestSexAfterBasiWombed( true );
			return;
		}
		if( CoC.getInstance().flags[ kFLAGS.BENOIT_TIMES_SEXED_FEMPCS ] > 0 ) {
			if( CoC.getInstance().player.isTaur() ) {
				EngineCore.outputText( 'You silently reach across the counter to squeeze his hands again.  Benoit grins with deep affection at you and, hand in hand, the two of you quietly debunk to the store room again.' );
		...
		CoC.getInstance().flags[ kFLAGS.BENOIT_TIMES_SEXED_FEMPCS ]++;
		if( CoC.getInstance().player.isTaur() ) {
			//Suggest: ;
			EngineCore.outputText( 'You reach your fingers across the counter and lightly touch Benoit\'s hands, saying you don\'t mind working out a few natural urges, if he\'s in the mood.' );
			...
			if( CoC.getInstance().player.gender === 3 ) {
				...
				if( CoC.getInstance().player.longestCockLength() <= 10 ) {
					EngineCore.outputText( '  Although this is evidently an uncanny experience for him, he does manage a cocky smile as his hand moves around your male sex. “Mine is bigger,” he teases.  Unable to reward his cheek from where he is, you simply snort and shuffle your hooves, impatient now with need.' );
				}//[cock  >10 inches: ;
				else {
					EngineCore.outputText( '  This is evidently an uncanny experience for him, the alien nature of it deepening as his hands moves around your male sex. “Oly Gods [name], you are a monster,” he says thickly.  You smile and stamp your hooves, impatient now with need.' );
				}
			}
			...
			if( CoC.getInstance().player.lactationQ() >= 50 ) {
				EngineCore.outputText( '  The slow, sensual sex is enough for you to begin to bead milk from your sensitive [nipples]; you moan as the flow intensifies until you are instinctively kneading your [chest], spurting your sweet, warm fluids onto the floor.  Benoit starts in genuine amazement at the sound.  “Mammals are so damn weird,” he mutters, making you giggle.' );
			}
			...
			if( CoC.getInstance().player.gender === 3 ) {
				EngineCore.outputText( '  [EachCock] thickens and spurts in sympathy to your female high, spattering the floor with white paint.' );
			}
		} else {
			EngineCore.outputText( 'You reach your fingers across the counter and lightly touch Benoit\'s hands, saying you don\'t mind working out a few natural urges, if he\'s in the mood.' );
			...
			//[Demon: ;
			if( CoC.getInstance().player.horns > 0 && CoC.getInstance().player.hornType === AppearanceDefs.HORNS_DEMON && CoC.getInstance().player.tailType === AppearanceDefs.TAIL_TYPE_DEMONIC && CoC.getInstance().player.demonScore() >= 3 ) {
			...
			else if( CoC.getInstance().player.earType === AppearanceDefs.EARS_DOG && CoC.getInstance().player.tailType === AppearanceDefs.TAIL_TYPE_DOG && CoC.getInstance().player.dogScore() >= 3 ) {
			...
			else if( CoC.getInstance().player.catScore() >= 3 && CoC.getInstance().player.tailType === AppearanceDefs.TAIL_TYPE_CAT && CoC.getInstance().player.earType === AppearanceDefs.EARS_CAT ) {
			...
			else if( CoC.getInstance().player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_HARPY && CoC.getInstance().player.wingType === AppearanceDefs.WING_TYPE_FEATHERED_LARGE ) {
			...
			//[Reptile/Naga: ;
			else if( CoC.getInstance().player.skinType === AppearanceDefs.SKIN_TYPE_SCALES && (CoC.getInstance().player.lizardScore() >= 3 || CoC.getInstance().player.nagaScore() >= 3) || CoC.getInstance().player.dragonScore() >= 3 ) {
			...
			else if( (CoC.getInstance().player.wingType === AppearanceDefs.WING_TYPE_BEE_LIKE_SMALL || CoC.getInstance().player.wingType === AppearanceDefs.WING_TYPE_BEE_LIKE_LARGE) && CoC.getInstance().player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_BEE ) {
			...
			if( CoC.getInstance().player.inHeat ) {
				EngineCore.outputText( ' By now your vagina is practically gushing, your body\'s own deep-seated pheromone need stoked to blazing heights by the basilisk\'s gentle, painstaking exploration.  You cannot stop yourself thrusting against his soaked hand, announcing how badly you want this with heavy moans.' );
			}
			...
			CoC.getInstance().player.cuntChange( 14, true, true, false );
			...
			if( CoC.getInstance().player.lactationQ() > 100 ) {
				EngineCore.outputText( '  You moan as your breasts reward his attention with spurts of milk; he starts in genuine amazement when they ' );
				if( !CoC.getInstance().player.isTaur() ) {
					EngineCore.outputText( 'hit him in the face' );
				} else {
					EngineCore.outputText( 'trickle over his fingers' );
				}
				EngineCore.outputText( '.  "<i>Mammals are so damn weird,</i>" he mutters, making you giggle.' );
			}
			...
			if( CoC.getInstance().player.hasCock() ) {
				if( !CoC.getInstance().player.isTaur() ) {
					EngineCore.outputText( '  Stuck between your two burning bodies, y' );
				} else {
					EngineCore.outputText( 'Y' );
				}
				EngineCore.outputText( 'our ' + Descriptors.cockDescript( 0 ) + ' spurts in sympathy to your female high, soaking ' );
				if( !CoC.getInstance().player.isTaur() ) {
					EngineCore.outputText( 'both of you' );
				} else {
					EngineCore.outputText( 'the dry dirt' );
				}
				EngineCore.outputText( ' with white.' );
			}
		}
		...
		if( (CoC.getInstance().player.pregnancyType === PregnancyStore.PREGNANCY_OVIELIXIR_EGGS || CoC.getInstance().player.findPerk( PerkLib.HarpyWomb ) >= 0 || CoC.getInstance().player.findPerk( PerkLib.Oviposition ) >= 0) && (CoC.getInstance().player.pregnancyIncubation === 0 || CoC.getInstance().player.pregnancyType === PregnancyStore.PREGNANCY_OVIELIXIR_EGGS) ) {
	...
	Benoit.prototype.dontTakeEggtraceptives = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'You smile and say you don\'t mind carrying and laying a few basilisk eggs. "<i>You... you don\'t?</i>" he says hesitantly.  He faces you and for a moment looks like he\'s going to say something else; but then he shakes his head and puts the bag back into the drawer.' );
	...
	Benoit.prototype.repeatSexWithBenoitLetHim = function() {
		...
		if( CoC.getInstance().player.isTaur() ) {
			...
			if( CoC.getInstance().player.isBiped() ) {
				EngineCore.outputText( 'knees' );
			} else {
				EngineCore.outputText( 'lower body' );
			}
			EngineCore.outputText( '.  His hands descend upon your [butt]; grinning, you poke it outwards invitingly, sighing with affected frustration as the blind basilisk painstakingly lines up his next shot with your drooling, sticky sex.' );
			EngineCore.outputText( '\n\nWhen he slides into you he does so with virtually no resistance whatsoever; the two of you have turned your pussy into an inviting, gooey sleeve.  ' );
			if( CoC.getInstance().player.vaginalCapacity() < 30 ) {
				EngineCore.outputText( 'You don\'t mind; your tight sex is a perfect fit for his smooth, thin dick, and you work with him as he thrusts, pulling and pushing your walls in time with his length, your lubrication allowing him to quickly increase the tempo until the two of you are once again bucking against each other gleefully, your fluids spattering against each other.' );
			} else {
	...
	Benoit.prototype.repeatBenoitFuckTakeCharge = function() {
		...
		if( CoC.getInstance().player.isTaur() ) {
			...
			//[Small capacity:];
			if( CoC.getInstance().player.vaginalCapacity() < 30 ) {
				EngineCore.outputText( 'You slide back down onto him, cooing this time as you feel him bottom out with several inches of him still outside of you.  His thin, long length could not be more perfect for your tight sex; he rubs your walls up and down as you dreamily thrust in and out of his body, sending waves of pleasure flowing through you.' );
			}//Large capacity: ;
			else {
				EngineCore.outputText( 'You slide back down onto him, cooing this time as your groin meets his muscular thighs with a wet kiss, your sex swallowing his cock whole.  You begin to ride him hard and slow, bending his cock upwards to push at your sensitive walls, waves of pleasure starting to flow through you.' );
			}
			...
			//begin copypasta;
			EngineCore.outputText( 'His hands trail further down, moving into your inner thighs, exploring your soft flesh until he touches your lips.  You close your eyes and sigh as he slowly parts them with his smooth fingers and slides into your [vagina].  Although his breath is becoming increasingly heavy he also seems genuinely curious about you; with surprising gentleness his fingers travel over and around your moistening sex, exploring your every fold, working deeper and deeper as he does.  You let him know what pleases you by sighing when he touches a sweet spot, moving deliberately with his finger\'s motions so he may give them better attention.  He soon finds your [clit], beginning to bulge with need; slowly he circles it and then flicks at it, gently frigging you.' );
			if( CoC.getInstance().player.inHeat ) {
				EngineCore.outputText( '  By now your vagina is practically gushing, your body\'s own deep-seated pheromone need stoked to blazing heights by the basilisk\'s gentle, painstaking exploration.  You cannot stop yourself thrusting against his soaked hand, announcing how badly you want this with heavy moans.' );
			}
	...
	Benoit.prototype.tryToConvertToBassyWomb = function() {
		...
		if( !(CoC.getInstance().player.hasItem( ConsumableLib.OVIELIX, 2 ) && CoC.getInstance().player.hasItem( ConsumableLib.REPTLUM ) && CoC.getInstance().player.hasItem( ConsumableLib.GOB_ALE )) ) {
		...
		//Ingredients in inventory: ;
			...
			CoC.getInstance().player.consumeItem( ConsumableLib.OVIELIX, 2 );
			CoC.getInstance().player.consumeItem( ConsumableLib.REPTLUM );
			CoC.getInstance().player.consumeItem( ConsumableLib.GOB_ALE );
			EngineCore.outputText( 'You ferret out the ingredients you have collected and begin to bang them onto the counter in front of Benoit, telling him that you\'ve got what he needs.  Pierre barks excitedly at the noise.' );
			...
			//[No oviposition:;
			if( CoC.getInstance().player.findPerk( PerkLib.Oviposition ) < 0 ) {
				EngineCore.outputText( '  You feel slightly bloated, but otherwise fine; you sense that you can lay eggs of your own volition now.' );
			} else {
				EngineCore.outputText( 'You feel slightly bloated, but otherwise fine; you sense that were you to get impregnated by basilisk seed, the eggs you produce would be pure basilisk.' );
			}
			...
			CoC.getInstance().flags[ kFLAGS.BENOIT_TESTED_BASILISK_WOMB ] = 0.5;
			//[Lust 30 or more: ;
			if( CoC.getInstance().player.lust >= 33 ) {
				EngineCore.outputText( '  You grin and say you\'re not sure it worked, but you suppose there\'s only one real way of finding out...' );
				if( CoC.getInstance().player.tallness <= 78 && !CoC.getInstance().player.isTaur() ) {
					EngineCore.outputText( '  The basilisk is still for a moment, and then with a sudden surge of movement, grabs you by the waist and hoists you over his shoulder.  You squeal in mock terror as he hauls you as fast as he can into the back room, knocking over half his stock as he does.' );
				} else {
	...
	Benoit.prototype.suggestSexAfterBasiWombed = function( later ) {
		...
		if( later === undefined || later ) {
			EngineCore.outputText( 'You smile coquettishly and ask if he\'s interested in taking your new body for a... test drive.  ' );
			//(6'6 or less: ;
			if( CoC.getInstance().player.tallness <= 78 && !CoC.getInstance().player.isTaur() ) {
				EngineCore.outputText( 'The basilisk is still for a moment, and then with a sudden surge of movement, grabs you by the waist and hoists you over his shoulder.  You squeal in mock terror as he hauls you as fast as he can into the back room, knocking over half his stock as he does.' );
			} else {
		...
		CoC.getInstance().flags[ kFLAGS.BENOIT_TESTED_BASILISK_WOMB ] = 1;
		this.benoitKnocksUpPCCheck();
		//(Oviposition perk added);
		CoC.getInstance().player.createPerk( PerkLib.BasiliskWomb, 0, 0, 0, 0 );
		EngineCore.outputText( '\n\n(<b>Perk Unlocked: Basilisk Womb - You can now give birth to female basilisks.</b>)' );
		if( CoC.getInstance().player.findPerk( PerkLib.Oviposition ) < 0 ) {
			CoC.getInstance().player.createPerk( PerkLib.Oviposition, 0, 0, 0, 0 );
			EngineCore.outputText( '\n(<b>Perk Unlocked: Oviposition - You will now regularly lay unfertilized eggs.</b>)' );
		}
		if( CoC.getInstance().player.pregnancyType === PregnancyStore.PREGNANCY_BASILISK ) {
			CoC.getInstance().player.knockUpForce( PregnancyStore.PREGNANCY_BENOIT, CoC.getInstance().player.pregnancyIncubation );
		}
		EngineCore.doNext( CoC.getInstance().scenes.camp.returnToCampUseOneHour );
		CoC.getInstance().player.orgasm();
	...
	Benoit.prototype.popOutBenoitEggs = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.BENOIT_EGGS ] > 0 ) {
			EngineCore.outputText( ' once more' );
		}
		EngineCore.outputText( '.  Well, he will be when this ' );
		if( CoC.getInstance().flags[ kFLAGS.BENOIT_EGGS ] > 0 ) {
			EngineCore.outputText( 'latest ' );
		}
		EngineCore.outputText( 'clutch hatches, anyway.' );
		//First Time: ;
		if( CoC.getInstance().flags[ kFLAGS.BENOIT_EGGS ] === 0 ) {
		...
		CoC.getInstance().flags[ kFLAGS.BENOIT_EGGS ] += Math.floor( CoC.getInstance().player.totalFertility() / 10 );
		//EngineCore.doNext(1);;
	};
	//Feminising;
	//Opening Talk;
	...
	Benoit.prototype.femoitInitialTalk = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'You ask Benoit if he has ever thought about trying to do something to help his people\'s plight.' );
		...
		/*
		 ...
		 CoC.getInstance().flags[kFLAGS.BIMBO_FEMOIT_UNLOCKED] = 1;
		 }
		 */
		if( CoC.getInstance().player.inte >= 60 ) {
			...
			CoC.getInstance().flags[ kFLAGS.FEMOIT_UNLOCKED ] = 1;
		} else {
	...
	Benoit.prototype.benoitFeminise = function() {
		EngineCore.clearOutput();
		// Ingredients not in inventory;
		if( !CoC.getInstance().player.hasItem( ConsumableLib.P_S_MLK, 2 ) || !CoC.getInstance().player.hasItem( ConsumableLib.L_PNKEG ) || !CoC.getInstance().player.hasItem( ConsumableLib.OVIELIX ) || !CoC.getInstance().player.hasItem( ConsumableLib.REPTLUM ) ) {
			...
			CoC.getInstance().player.destroyItems( ConsumableLib.P_S_MLK, 2 );
			CoC.getInstance().player.destroyItems( ConsumableLib.L_PNKEG, 1 );
			CoC.getInstance().player.destroyItems( ConsumableLib.OVIELIX, 1 );
			CoC.getInstance().player.destroyItems( ConsumableLib.REPTLUM, 1 );
			EngineCore.outputText( 'You ferret out the ingredients you have collected and begin to bang them onto the counter in front of Benoit, telling him that you\'ve got what he needs.  Pierre barks excitedly at the noise.' );
			...
			CoC.getInstance().flags[ kFLAGS.FEMOIT_NEXTDAY_EVENT ] = this.getGame().model.time.days + 1;
			CoC.getInstance().flags[ kFLAGS.FEMOIT_NEXTDAY_EVENT_DONE ] = 1;
			EngineCore.menu();
			EngineCore.doNext( CoC.getInstance().scenes.camp.returnToCampUseOneHour );
		}
	};
	Benoit.prototype.femoitNextDayEvent = function() {
		EngineCore.clearOutput();
		CoC.getInstance().flags[ kFLAGS.BENOIT_STATUS ] = 1;
		CoC.getInstance().flags[ kFLAGS.FEMOIT_NEXTDAY_EVENT_DONE ] = 2;
	...
	Benoit.prototype.femoitFirstTimeNo = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'You let her down as kindly as you can.' );
	...
	Benoit.prototype.femoitFirstTimeYes = function() {
		CoC.getInstance().flags[ kFLAGS.TIMES_FUCKED_FEMOIT ]++;
		EngineCore.clearOutput();
		EngineCore.outputText( 'Smiling, you reach across the counter and squeeze Benoite\'s hands until her nervous babble dies out and she smiles back.  Still holding her hand, you move behind the crates and then gently lead her behind the stall\'s canopy.' );
		...
		} else if( CoC.getInstance().player.demonScore() > 4 ) {
		...
		} else if( CoC.getInstance().player.dogScore() >= 4 && CoC.getInstance().player.earType === 2 && CoC.getInstance().player.tailType === 2 ) {
		...
		} else if( (CoC.getInstance().player.bunnyScore() >= 4 && CoC.getInstance().player.earType === 7 && CoC.getInstance().player.tailType === 10) || (CoC.getInstance().player.catScore() >= 4 && CoC.getInstance().player.earType === 5 && CoC.getInstance().player.tailType === 8) ) {
		...
		} else if( CoC.getInstance().player.harpyScore() >= 4 && CoC.getInstance().player.wingType !== 0 && CoC.getInstance().player.armType === 1 ) {
		...
		} else if( CoC.getInstance().player.beeScore() >= 4 && CoC.getInstance().player.wingType !== 0 && CoC.getInstance().player.lowerBody === 7 ) {
		...
		} else if( CoC.getInstance().player.gooScore() >= 4 && CoC.getInstance().player.skinType === 3 ) {
		...
		} else if( CoC.getInstance().player.skinType === 2 ) {
		...
		if( CoC.getInstance().player.biggestCockLength() <= 10 ) {
			EngineCore.outputText( '  Although this is evidently an uncanny experience for her, she does manage a cocky smile as her hand moves around your sex.  "<i>Mine was bigger,</i>" she teases.  You reward her cheek by doing some feeling yourself, grasping her large, supple behind, making her squeak as you move into her.' );
		} else {
		...
		if( CoC.getInstance().player.isLactating() ) {
			EngineCore.outputText( ' The pressure and arousal makes your [nipples] dribble milk, spattering fluids across both of your chests.' );
		}
		EngineCore.outputText( '  Soon the two of you are thrashing into each other, Benoite moaning huskily as you batter her against the wagon wall, both seeking your peak together, each pushing the other a bit further upwards.  Your world is lost to everything but the warm tightness around your [cock] and the pressure against your chest.' );
	...
	Benoit.prototype.femoitSexIntro = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'Once again, you take Benoite\'s hand and lead her into the back room.  Your free hand roams underneath her apron as you carefully pick out a path through the junk and Benoite is swiftly in on the game too; her fingers slide downwards to pinch and fondle at her favorite parts of you.  By the time you make it into the storage space you are practically falling over each other, laughing as you fumble off each other\'s clothes.' );
		if( !this.benoitPreggers() ) {
			...
			if( this.benoitInClutch() ) {
				EngineCore.outputText( '  Her stomach bulges with clutch and the air is thick with female musk; the sight and smell of overwhelming ripeness speaks to your body in a way which bypasses your higher cognition entirely, sending you growling on top her, barely even hearing her giggle.' );
			}
			...
			if( CoC.getInstance().player.biggestCockLength() < 15 ) {
				EngineCore.outputText( 'Soon you are hilting yourself in her depths, making her gasp as you slap into her.' );
			} else {
				EngineCore.outputText( 'Soon you are bottoming out in her, making her gasp as your hulking length spreads her wide.' );
			}
			EngineCore.outputText( '  You quickly pick up the pace as you enter rut, thrusting into your basilisk lover with red-flecked abandon, her powerful thighs working with yours to make each return plunge into her warm depths more gratifying than the last.' );
			EngineCore.outputText( '\n\n' );
			if( !CoC.getInstance().player.isTaur() ) {
				EngineCore.outputText( 'Eventually, sweat dripping off you, you grab her thighs and heave them upwards so that you can really go to town, drawing yourself almost all the way out of her before smacking back into her,' );
				if( !this.benoitInClutch() ) {
					EngineCore.outputText( ' your stomach beating out a slapping rhythm against her own flat abdomen.  ' );
				} else {
					EngineCore.outputText( ' your stomach beating out a slapping rhythm against her bulging, gravid abdomen.  ' );
				}
			}
			EngineCore.outputText( 'Benoite moans, squeals and eventually screams to your exertions, her fluids spurting and spattering against your groin' );
			if( CoC.getInstance().player.balls > 0 ) {
				EngineCore.outputText( ' and [balls].  You tumble over your peak as her cunt suddenly tightens around yours, sending surge after surge of cum into her fertile depths, your body seized in a rictus of pleasure.' );
			}
			if( CoC.getInstance().player.cumQ() >= 2500 ) {
				EngineCore.outputText( '  The quantity of it is such that it quickly dribbles back out around your cock and pools on the floor.' );
			}
			...
			this.benoitKnockUp();
			CoC.getInstance().player.orgasm();
		} else if( this.benoitRegularPreggers() && (!CoC.getInstance().player.isTaur() || (CoC.getInstance().player.isTaur() && (CoC.getInstance().player.tallness * (5 / 6) < CoC.getInstance().player.cocks[ CoC.getInstance().player.longestCock() ].cockLength))) ) {
			EngineCore.clearOutput();
			EngineCore.outputText( 'Once you are both in the usual spot, neither of you waste any time undressing.  The pregnant basilisk stands there, staring blindly at you, and waiting for you to make the first move, tongue occasionally flicking past her lips to nervously wet them.' );
			...
			if( CoC.getInstance().player.cumQ() >= 2500 ) {
				EngineCore.outputText( '  Because her womb is already so jam-packed with eggs, the bulk of your deposit simply oozes messily back out of her, leaving her well and truly creampied.' );
			}
			...
			EngineCore.clearOutput();
			if( CoC.getInstance().player.isTaur() ) {
				EngineCore.outputText( '"<i>No.  Non!  Absolutely non!</i>" Benoite insists.  "<i>You are my amazing stallion, [name], but... it is simply too much.  Wis all zis weight on me, zere is simply no way we can do it.</i>"' );
				...
				if( CoC.getInstance().flags[ kFLAGS.FEMOIT_SPOONED ] === 0 ) {
					...
					EngineCore.outputText( 'Benoite smirks at you.  "<i>Well, I guess ze last time was enjoyable enough.  Come, then; I am horny and you arrived in time to scratch my itch.</i>"' );
				}
				...
				if( CoC.getInstance().player.balls === 0 ) {
					EngineCore.outputText( ' with a smile, knowing damn well you don\'t, literally' );
				}
				EngineCore.outputText( '.' );
	...
	Benoit.prototype.femoitBirths = function() {
		...
		// (First time: ;
		if( CoC.getInstance().flags[ kFLAGS.FEMOIT_HELPED_LAY ] === 0 ) {
			...
			if( this.benoitVeryHeavyPreggers() || this.benoitExtremePreggers() ) {
				EngineCore.outputText( '... which is good, because you can tell there\'re a lot of them coming' );
			}
			EngineCore.outputText( '.  That done, you squat down behind Benoite, keeping your head down to avoid having it slapped by her swishing tail, and reach up between her legs. She promptly lets out a shriek.' );
			if( CoC.getInstance().flags[ kFLAGS.FEMOIT_HELPED_LAY ] === 1 ) {
				EngineCore.outputText( '"<i>What\'s the matter?</i>" you blurt, alarmed.' );
				EngineCore.outputText( '"<i>You... you \'ave cold \'ands!</i>" she squeals, shuddering in displeasure.' );
			} else {
				EngineCore.outputText( '"<i>What have I told you about warming zose hands?</i>" she snaps angrily.' );
			}
		}
		CoC.getInstance().flags[ kFLAGS.FEMOIT_HELPED_LAY ]++;
		...
		if( EngineCore.silly() ) {
			EngineCore.outputText( '.  It\'s shaped like a complex rhomboidal polygon with 15 sides.' );
		} else {
			EngineCore.outputText( ', the smoothly curved peak of a jade egg beginning to crest.' );
		}
		EngineCore.outputText( '  With a strangled cry of orgasm tinged with pained relief, Benoite pushes it from her passage into your hands. Slick with juices, the egg makes for quite a handful and you find yourself struggling to safely cradle it within your grasp. You quickly place it into the makeshift next at her feet.' );
		if( CoC.getInstance().flags[ kFLAGS.FEMOIT_EGGS ] >= 2 ) {
		...
		if( CoC.getInstance().flags[ kFLAGS.FEMOIT_EGGS ] > 1 ) {
			EngineCore.outputText( 's' );
		}
		EngineCore.outputText( ' she just laid.  You nod from behind her, and cautiously move around to give her a hug.  She is a very, very brave woman, and an even braver man for making the decision to do this in the first place.' );
		...
		if( this.benoitRegularPreggers() ) {
			EngineCore.outputText( ' She sighs softly.  "<i>A small clutch, this is... normal for my people, yes, but not very good for my mission, is it?  Still, a small step to freedom is still a step.</i>"  She manages to smile.  "<i>My children will be free, and that is something to celebrate.</i>"' );
		} else if( this.benoitHeavyPreggers() ) {
			EngineCore.outputText( ' "<i>My, a nice big clutch we had together, didn\'t we?</i>" she smiles, proudly.  "<i>Yes, these will be strong children, I zink.</i>"' );
		} else if( this.benoitVeryHeavyPreggers() ) {
			EngineCore.outputText( ' "<i>...Wow, you\'re quite ze stud, aren\'t you?</i>" Benoite says, giving a throaty growl of lust.  "<i>I chose well when I decided to let you fertilize my eggs... so many eggs, too.</i>"' );
		} else if( this.benoitExtremePreggers() ) {
			EngineCore.outputText( ' The basilisk stops after her counting, visibly stunned.  She recounts her eggs again, and then again, shaking her head in amazement.  "<i>Incredible... simply, incredible.  I didn\'t think it was possible for one woman to lay zo many eggs!  Zut alors!</i>"  A sudden look of horrified realization washes over her features.  "<i>I\'m going to be run off my feet with all zese little monsters, aren\'t I?</i>" she murmurs to herself.' );
		}
	...
	CoC.getInstance().registerScene( 'benoit', new Benoit() );
} );