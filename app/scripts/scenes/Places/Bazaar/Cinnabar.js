...
angular.module( 'cocjs' ).run( function( SceneLib, Descriptors, CoC, kFLAGS, Utils, EngineCore ) {
	...
	Cinnabar.prototype.cinnabarAppearance = function( output ) {
		...
		if( CoC.getInstance().time.hours < 15 || CoC.getInstance().time.hours > 20 ) {
			return null;
		}
		if( output === undefined || output ) {
			//Not yet introduced;
			if( CoC.getInstance().flags[ kFLAGS.CINNABAR_NUMBER_ENCOUNTERS ] === 0 ) {
	...
	Cinnabar.prototype.cinnabarGreeting = function() {
		...
		//1st Time:;
		if( CoC.getInstance().flags[ kFLAGS.CINNABAR_NUMBER_ENCOUNTERS ] === 0 ) {
			CoC.getInstance().flags[ kFLAGS.CINNABAR_NUMBER_ENCOUNTERS ]++;
			//(No cock) ;
	...
	Cinnabar.prototype.cinnabarNonHugeDickings = function() {
		CoC.getInstance().flags[ kFLAGS.CINNABAR_HOUSE_VISITED ]++;
		CoC.getInstance().flags[ kFLAGS.CINNABAR_NUMBER_TIMES_FUCKED ]++;
		CoC.getInstance().flags[ kFLAGS.CINNABAR_FUCKED_CORRUPT_PREVIOUSLY ] = 0;
		EngineCore.outputText( '', true );
		EngineCore.spriteSelect( 91 );
		//(Too poor);
		...
		EngineCore.outputText( 'Once you give your assent, Cinnabar\'s paws dive into your ' + CoC.getInstance().player.armorName + ', gliding over your ' + CoC.getInstance().player.skinDesc + ' before re-emerging above your gem-pouch.  She deftly grabs her payment and snakes back through, tickling your belly with her soft, well-groomed fur.  You look at her, then yourself, then her again.  How the hell was she flexible enough to do that?  Cinnabar giggles and curls her tail around your ' + CoC.getInstance().player.legs() + ', looping it like a long, flexible rope.  She hugs you and murmurs, "<i>Where will my ' + CoC.getInstance().player.mf( 'master', 'mistress' ) + ' take ' + CoC.getInstance().player.mf( 'his', 'her' ) + ' slutty, fuck-hungry companion?</i>"  Her tongue flicks your ear like a snake tasting its prey, and though her attitude sounds submissive and weak, your horny \'companion\' is clearly hiding a fiery, aggressive streak.\n\n', false );
		EngineCore.outputText( 'Before you can make an answer, Cinnabar\'s hands are back inside your ' + CoC.getInstance().player.armorName + ', tugging your ' + Descriptors.nippleDescript( 0 ) + ' towards an unmarked wagon that\'s clearly her home.  You smirk and rub her between her legs, feeling the moistness of her sloppy cunt through her now-dampened dress.  Before she can tease you any more, you hook a finger up, pushing her dress into her hungry snatch.  It\'s easy to lead the giggling rat-whore like this, and when you reach the door she slides her tail into the lock, clicking it open with ease.\n\n', false );
		...
		EngineCore.outputText( 'leave you panting like a bitch in heat.  She urges you on, practically begging, "<i>Yes!  Stuff my hole!  Fist me, ' + CoC.getInstance().player.mf( 'master', 'mistress' ) + '; stretch me with your hand until I\'m too loose for a minotaur!  Please... please... fist my ass too.  Ruin my holes, ' + CoC.getInstance().player.mf( 'master', 'mistress' ) + '!  Break fuck-toy\'s body!</i>"\n\n', false );
		...
		if( CoC.getInstance().player.hasVagina() ) {
			if( CoC.getInstance().player.vaginalCapacity() < 30 && CoC.getInstance().player.analCapacity() < 30 ) {
				EngineCore.outputText( 'fingers', false );
			} else if( CoC.getInstance().player.vaginalCapacity() >= 30 && CoC.getInstance().player.analCapacity() >= 30 ) {
	...
	Cinnabar.prototype.cinnabarGetsFUKKKKED = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.CINNABAR_FUCKED_CORRUPT_PREVIOUSLY ] > 0 ) {
			EngineCore.outputText( '  Just be gentler than last time, okay ' + CoC.getInstance().player.mf( 'stud', 'baby' ) + '?', false );
		}
		...
		if( CoC.getInstance().player.str >= 60 || CoC.getInstance().player.tallness >= 72 ) {
		...
		EngineCore.outputText( 'Flexible, determined fingertips are buried deep into your crotch before the entry has closed behind.  Cinnabar\'s eyes fixate on the massive, tenting bulge between your ' + CoC.getInstance().player.legs() + ', her soft finger-pads shaking while they try to set you free.  You appreciate her efforts, but she nearly got you into some trouble out there.  She needs to settle down, or at least be a little more mindful of your wishes.  You yank her arms away from your bouncing, newly-freed ' + Descriptors.cockDescript( x ) + ', groaning from sensation while you wrestle to deny Cinnabar her desire.\n\n', false );
		EngineCore.outputText( 'The rat squirms and kicks, but you twist away from each vicious blow.  She growls, "<i>You\'ve got that fucking huge dick and you won\'t let me give it a hug?  Seriously, what are you, some cunt-shy freak?  I thought you\'d be a good fuck, but if you\'re this scared of a woman\'s pussy, maybe I should go find a ' + CoC.getInstance().player.mf( 'real man', 'horse-herm' ) + ' to shack up with!</i>"  She\'s got a mouth on her, but she needs to be taken down a peg.  You sweep her legs out from under her and push, slamming her into the bed hard enough to make her tits quake from the impact.\n\n', false );
		...
		EngineCore.outputText( 'Sure enough, when you look at the mirrors you get a surprise.  There, in all its glory, is Cinnabar\'s dripping, wet snatch.  Her lips are a ruddy-red near her thighs, but the closer they get to her slit, the blacker they become.  You note her vulva and labia are slightly parted, perhaps by your previous attempt at penetration, and her shining, black love-tunnel seems to almost beckon for your attentions.  "<i>Mmmm, look how wet I am for you, ' + CoC.getInstance().player.mf( 'stud', 'slut' ) + '... I\'ve been such a bad girl – look at my puny pussy.  Why doesn\'t ' + CoC.getInstance().player.mf( 'master', 'mistress' ) + ' punish it?</i>" she asks, pressing back hard enough to make her genitals bulge around the mirror\'s rim.\n\n', false );
		...
		EngineCore.outputText( 'Your ' + CoC.getInstance().player.cockHead( x ) + ' hits those plush, juice-gushing lips and forces them wide.  She\'s tight, so goddamned tight, but you\'ve only got the barest portion of your tip inside.  Cinnabar squeaks in pleasure and pain, rocking her ass up and down to try and loosen up her poor, abused entrance.\n\n', false );
		...
		if( CoC.getInstance().player.cockArea( x ) >= 200 ) {
			if( CoC.getInstance().player.cor >= 75 ) {
				EngineCore.doNext( this.fuckCinnabarHugerAndCorruptedEr );
			} else {
				EngineCore.doNext( this.cinnabarHuger );
			}
		} else {
			EngineCore.doNext( this.cinnabarHuge );
		}
	};
	//(Huger and Corrupteder) (75+ corr);
	Cinnabar.prototype.fuckCinnabarHugerAndCorruptedEr = function() {
		CoC.getInstance().flags[ kFLAGS.CINNABAR_FUCKED_CORRUPT_PREVIOUSLY ] = 1;
		var x = CoC.getInstance().player.biggestCockIndex();
		EngineCore.outputText( '', true );
		EngineCore.spriteSelect( 91 );
		...
		EngineCore.outputText( ', squirting juices from the callous, cunt-piercing thrust.  The ' + CoC.getInstance().player.cockHead( x ) + ' of your ' + Descriptors.cockDescript( x ) + ' can be seen just below her neck', false );
		if( CoC.getInstance().player.cocks[ x ].cockLength >= 60 ) {
		...
		EngineCore.outputText( 'Cinnabar pulls her dirty paw out and grins like a mouse with a chunk of cheese, massaging her distorting neck while you empty every ounce of cum from your ' + Descriptors.ballsDescriptLight() + ' into her over-stretched womb-filled body.  She squeaks, "<i>No-ohhhh-ormally I LIKE being a mewling slut for ' + CoC.getInstance().player.mf( 'studs', 'bitches' ) + ' like you, but if you won\'t treat a cum-sleeve like me with respect, then I\'m going to teach it to you.</i>"  You nod dumbly, still locked in an climax that ', false );
		if( CoC.getInstance().player.cumQ() < 250 ) {
		...
		EngineCore.outputText( 'You sit up, completely and totally awake thanks to your abusive lover, who\'s just smirking at you.  "<i>Goddamn you, you huge-cocked ' + CoC.getInstance().player.mf( 'stud', 'fucker' ) + '.  I\'m STILL sore, but Marae\'s tits, did you ever get me off!  Don\'t get the wrong impression, it hurt like a BITCH.  I pity the girls you meet who can\'t stretch themselves like I can.  I truly do.  If you try that shit with me again, I\'ll give you another lesson in empathy – I hope you don\'t forget this one.</i>"\n\n', false );
		EngineCore.outputText( 'Cinnabar tosses your ' + CoC.getInstance().player.armorName + ' and you realize the nude rat doesn\'t show any signs of your tryst', false );
	...
	Cinnabar.prototype.cinnabarHuger = function() {
		...
		if( CoC.getInstance().player.cocks[ x ].cockLength < 24 ) {
			EngineCore.outputText( Utils.num2Text( Math.round( CoC.getInstance().player.cocks[ x ].cockLength ) ) + ' inches', false );
		} else {
			EngineCore.outputText( Utils.num2Text( Math.round( CoC.getInstance().player.cocks[ x ].cockLength / 12 ) ) + ' feet', false );
		}
		...
		EngineCore.outputText( 'At last, your ' + CoC.getInstance().player.cockHead( x ) + ' pops from her pussy, showing just how completely you\'ve worked over her body.  Her hips are stretched damn near a foot apart, and you can see deep enough inside her ', false );
		if( CoC.getInstance().player.cocks[ x ].cockLength >= 60 ) {
	...
	Cinnabar.prototype.cinnabarHuge = function() {
		...
		EngineCore.outputText( '', true );
		EngineCore.spriteSelect( 91 );
		EngineCore.outputText( 'You\'re almost there, and though you can feel the burgeoning warmth growing underneath your ' + Descriptors.cockDescript( x ) + ', you\'re determined to push the whole way inside her before you blow.  Cinnabar crams her muzzle through a particularly large mirror and locks onto your lips, ramming her tongue deeper into your throat, pushing harder with each inch of tightly-packed cock you shove inside her.  At last you bottom out, your ' + CoC.getInstance().player.cockHead( x ) + ' bumps the tight ring of her cervix while her tail pulls tight about your ', false );
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( Descriptors.ballsDescriptLight(), false );
		...
		EngineCore.outputText( '.\n\n', false );
		EngineCore.outputText( 'The kiss is broken abruptly, the suddenly invigorated rat-girl\'s lips pressing close to your ear as she murmurs, "<i>Oh yes... feel how tight my poor, abused cunt is... so very tight, clutching you like a swimsuit two sizes too small... Mmm, I can feel you twitching; are you going to cum in this slut\'s womb?  You\'ve fucked me so hard, gaped me and ruined me... will you finish me off?  Do it ' + CoC.getInstance().player.mf( 'stud', 'bitch' ) + ', fuck this horny slut\'s womb full, mark her as your bitch - no, your fucking property!</i>"\n\n', false );
		...
		EngineCore.outputText( 'your ' + CoC.getInstance().player.legs() + ', and ', false );
		if( !CoC.getInstance().player.hasSheath() ) {
			EngineCore.outputText( 'the base of your twitching cock', false );
		} else {
			EngineCore.outputText( 'your twitching cock-sheath', false );
		}
		EngineCore.outputText( '.  You rub your hand over her belly, feeling your ' + CoC.getInstance().player.cockHead( x ) + ' through her distended, dick-dominated fur.  Each fiery, pumping load of cum boils underneath the rat\'s fur, bubbling between your fingers as it sloshes into her womb.  Cinnabar is thrashing, squeaking and crying, though it\'s quite clear her tears are tears of joy.  She gasps, "<i>So big', false );
		if( CoC.getInstance().player.cumQ() >= 1000 ) {
	...
	Cinnabar.prototype.cinnabarMultiCockPortalFuckFest = function() {
		...
		var z = CoC.getInstance().player.biggestCockIndex3();
		EngineCore.outputText( 'Cinnabar\'s hands fly through your ' + CoC.getInstance().player.armorName + ', diving through your undergarments to squeeze ' + Descriptors.sMultiCockDesc() + '.  ', false );
		...
		EngineCore.outputText( '  Your ' + CoC.getInstance().player.legs() + ' go wobbly from the delicate touches dancing through your ' + Descriptors.multiCockDescriptLight() + ', making it hard to stand.  Cinnabar slides ', false );
		...
		EngineCore.outputText( ', supporting you while she asks, "<i>Do we need to get inside and get these taken care of, big ' + CoC.getInstance().player.mf( 'boy', 'girl' ) + '?  Ohhh, that\'s just too bad.  I think I\'ll just play with you in the street until you\'re ready to pop, and then leave you like that until you go soft.  Then we can start over.</i>"\n\n', false );
		...
		if( CoC.getInstance().flags[ kFLAGS.CINNABAR_HOUSE_VISITED ] === 0 ) {
		...
		EngineCore.outputText( 'You slap her face with your ' + Descriptors.cockDescript( x ) + ', smearing her pussy-juice across her cheek and mixing a bit of your pre-cum into it.  She groans and jumps onto her bed, but before you can close the distance she asks, "<i>Can slut show ' + CoC.getInstance().player.mf( 'master', 'mistress' ) + ' her tricks?  She has so many sexy tricks to help ' + CoC.getInstance().player.mf( 'master', 'mistress' ) + ' fuck her needy, achy fuck-holes.</i>"\n\n', false );
		...
		EngineCore.outputText( '"<i>D-does ' + CoC.getInstance().player.mf( 'master', 'mistress' ) + ' like?  Ohhhh, fuck this slut ' + CoC.getInstance().player.mf( 'master', 'mistress' ) + '... shit, come fuck me!  That fucking breast enlargement spell makes my cunt ache and drool for a big, hard cock.  Plug me ' + CoC.getInstance().player.mf( 'stud', 'baby' ) + ', you can fill me so good!  Do it!  FUCK ME NOW!</i>" she screams, rocking and grinding into the portals, her tongue thrashing wildly while her lips arch into a needy \'o\'.\n\n', false );
		...
		EngineCore.outputText( 'The slut-rat gargles in irritation around your sensitive, cummed-out cock, clearly displeased that you\'re still inside her.  You step back from the wall, your ' + CoC.getInstance().player.legs() + ' giving out from the sensory overload of unsheathing your ' + Descriptors.multiCockDescriptLight() + ' simultaneously to drop you flat on your ' + Descriptors.assDescript() + '.  At the same time, Cinnabar starts coughing and groaning, clearly exhausted from the one-' + CoC.getInstance().player.mf( 'man', 'woman' ) + ' gangbang.  You understand how that feels; just touching ' + Descriptors.sMultiCockDesc() + ' makes your head swoon.\n\n', false );
