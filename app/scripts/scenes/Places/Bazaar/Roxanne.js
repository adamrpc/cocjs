...
angular.module( 'cocjs' ).run( function( MainView, $rootScope, PerkLib, Descriptors, AppearanceDefs, CoC, kFLAGS, Utils, StatusAffects, EngineCore ) {
	...
	Roxanne.prototype.timeChange = function() {
		//Increase Roxanne's growing dick size...;
		CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00225 ]++;
		//Reset if she finds someone to take it (random at high values);
		if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00225 ] >= 300 && CoC.getInstance().time.hours === 1 && Utils.rand( 5 ) === 0 ) {
			CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00225 ] = 1;
		}
		//hangover status stuff;
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.Hangover ) >= 0 ) {
			//Countdown;
			if( CoC.getInstance().player.statusAffectv1( StatusAffects.Hangover ) > 0 ) {
				CoC.getInstance().player.addStatusValue( StatusAffects.Hangover, 1, -1 );
			} else {
				EngineCore.outputText( '\n<b>Your head finally clears as your hangover wears off.  Drinking with the shemale lizard was definitely a bad idea.</b>\n', false );
				//Restore stats;
				CoC.getInstance().player.str += CoC.getInstance().player.statusAffectv2( StatusAffects.Hangover );
				CoC.getInstance().player.spe += CoC.getInstance().player.statusAffectv3( StatusAffects.Hangover );
				CoC.getInstance().player.inte += CoC.getInstance().player.statusAffectv4( StatusAffects.Hangover );
				EngineCore.dynStats( 'cor', 0 );
				//Clear status;
				CoC.getInstance().player.removeStatusAffect( StatusAffects.Hangover );
				return true;
			}
		}
		if( CoC.getInstance().time.hours > 23 && CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00227 ] > 0 ) {
			CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00227 ]--; //Reduce drinking contest bonus
		}
		return false;
	};
	Roxanne.prototype.timeChangeLarge = function() {
		return false;
	};
	//End of Interface Implementation;
	//[Drinking Table Appearance];
	Roxanne.prototype.RoxanneAppearance = function() {
		//When she there?;
		if( CoC.getInstance().time.hours > 12 && CoC.getInstance().time.hours < 19 ) {
			//(Not Met) ;
	...
	Roxanne.prototype.Roxanne1stApproach = function() {
		...
		EngineCore.outputText( 'Suddenly, the alpha-lizan glances up and meets your eye, her expression turning into a leering sneer as she asks, "<i>See something you like ' + CoC.getInstance().player.mf( 'buddy', 'girly' ) + '?  Come on over, tell us your story!</i>"\n\n', false );
		EngineCore.outputText( 'Do you approach?', false );
		EngineCore.doYesNo( this.RoxanneChooseApproachOrRepeat, CoC.getInstance().scenes.bazaar.enterTheBazaar );
	};
	...
	Roxanne.prototype.RoxanneChooseApproachOrRepeat = function() {
		...
		EngineCore.outputText( '', true );
		if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00221 ] === 0 ) {
			CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00221 ]++;
		...
		else if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00224 ] === 2 && CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00225 ] >= 200 ) {
			EngineCore.outputText( '"<i>Oooh, praise the sea god Ulrun, you\'ve returned.  ' + CoC.getInstance().player.short + ', it\'s been a while, and I\'ve gotten sooooo big,</i>" cheers an ecstatic Roxanne.  To emphasize her point she shows the seam-splitting bulge in her pants, rubbing it from the base all the way down to the tip, which rests next to her knee.  She\'s so big, and a damp spot appears at her pant-leg\'s knee while the huge cock-sausage visibly inflates.  Captain Poisontail asks, "<i>So, can I count on you to get sauced and help me take care of this beast again, or do you actually think you have a chance of winning now?</i>"\n\n', false );
		...
		CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00226 ] = 0;
	...
	Roxanne.prototype.roxanneDrinkingContestNo = function() {
		if( CoC.getInstance().time.hours === 19 || CoC.getInstance().time.hours === 20 ) {
			...
			if( CoC.getInstance().flags[ kFLAGS.COUNTDOWN_TO_NIGHT_RAPE ] % 4 === 0 && CoC.getInstance().player.gender === 1 ) {
				CoC.getInstance().scenes.bazaar.nightBazaarButtfuck();
				return;
			}
		}
		CoC.getInstance().scenes.bazaar.enterTheBazaarAndMenu();
	};
	Roxanne.prototype.roxanneDrinkingContestLoseDeliberately = function() {
		CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00226 ] = 1;
	...
	Roxanne.prototype.roxanneDrinkingContest = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00225 ] >= 200 ) {
		...
		if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00222 ] + CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00223 ] <= 0 || CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00224 ] === 2 ) {
			...
			if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00224 ] === 2 ) {
		...
		if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00222 ] + CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00223 ] < 3 ) {
		...
		else if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00222 ] > 0 && CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00223 ] <= 0 ) {
		...
		else if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00223 ] > 0 && CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00224 ] === 1 ) {
		...
		if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00226 ] === 0 ) {
		...
		//[DRINKING CONTEST CONTINUES – losing intentionally] ;
			...
			EngineCore.outputText( 'The \'taur at the tap quickly grows bored with the constant bantering from Roxanne and the flirting you shower the lizan in.  You down your drinks quickly, even sneaking refills while the lizan is distracted in order to speed your inevitable loss.  She looks at you, clearly checking you out while you unabashedly fixate on the pulsing mass of cock-flesh that strains her oh-so-tight pants.  Roxanne stops drinking and walks over to you, a little unsteady but still in control of herself, and pulls your head against her groin, letting you nuzzle it while she puts filled mugs in your hands.  "<i>Go on and drink... good ' + CoC.getInstance().player.mf( 'boy', 'girl' ) + ',</i>" she coos when you turn to the side and swallow more of the delicious brew.\n\n', false );
		}
		var score = 0;
		//Calculate score if not ;
		if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00226 ] === 0 ) {
			score = (CoC.getInstance().player.tallness * ((CoC.getInstance().player.thickness + 100) / 200) * (CoC.getInstance().player.tou / 100)) + CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00227 ];
		}
		//If score is less than 30-50 (Strahza is inconsistant!);
		//[Lose!] ;
		if( score < (45 + Utils.rand( 20 )) ) {
			//Increment loss count!;
			CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00223 ]++;
			//Set who won contast last;
			CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00224 ] = 2;
			//Gain big bonus;
			CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00227 ] += 10;
			EngineCore.outputText( 'Giggling and nearly tripping up on your own ' + CoC.getInstance().player.feet() + ', you stumble up to the corrupted deer-taur.  He looks at your wobbling stance, nearly-vacant eyes, and dopey grin before he shakes his head from side to side and says, "<i>No.</i>"  ', false );
			if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00226 ] === 0 ) {
			...
			if( CoC.getInstance().player.buttRating > 12 && CoC.getInstance().player.tone <= 50 && CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00223 ] > 1 && Utils.rand( 2 ) === 0 ) {
				EngineCore.doNext( this.bigBootyRoxanneContestLoss );
			}//TO huge or regular anal;
			else if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00225 ] >= 200 ) {
		...
		//[WIN];
			...
			//Increment win count!;
			CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00222 ]++;
			//Set who won contest last;
			CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00224 ] = 1;
			//Gain small bonus;
			CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00227 ] += 4;
	...
	Roxanne.prototype.roxanneGivesABlowjob = function() {
		...
		EngineCore.outputText( 'You open the lower portion of your ' + CoC.getInstance().player.armorName + ' and, pulling it back, hang out your ' + Descriptors.multiCockDescriptLight() + '; ', false );
		...
		EngineCore.outputText( ', squeezing you softly while she steadies herself on your ' + Descriptors.cockDescript( x ) + '.  She titters, letting a few inches of tongue slip through her lips to envelop your sensitive ' + CoC.getInstance().player.cockHead( x ) + ', circling the engorged cock-flesh with drunken slobbers.  ', false );
		...
		EngineCore.outputText( 'SMACK!  Roxanne\'s tail slaps into your ' + Descriptors.buttDescript() + ', stinging the exposed ' + CoC.getInstance().player.skinDesc + '.  Her eyebrows narrow in irritation while she wraps her arms around your waistline, steadying her off-balance body while she spanks and pumps you in a drunken frenzy.  You hump her face, pressing your ' + CoC.getInstance().player.cockHead( x ) + ' against her lips to smear them with a glaze of leaky pre-cum.  Grabbing her horns, you cry out and pleasure and try to pull her down, but she spanks you, HARD.  You yelp in pain, dropping her horns and submitting completely to her tongue and the pleasure it brings.\n\n', false );
		...
		EngineCore.outputText( '.  "<i>Jeeze, pent up much?</i>" she laughs, struggling to stand while giggling at her own supposed cleverness.  Happy with how the contest worked out, you slip ' + Descriptors.sMultiCockDesc() + ' into your ' + CoC.getInstance().player.armorName + ' and wobble off back to camp.', false );
		//(-100 lust, -1 int);
	...
	Roxanne.prototype.roxanneCunnilingus = function() {
		EngineCore.spriteSelect( 78 );
		EngineCore.outputText( '', true );
		EngineCore.outputText( 'You shimmy out of your ' + CoC.getInstance().player.armorName + ' and lean back, exposing your ' + Descriptors.vaginaDescript() + ' to the drunken lizan\'s maw.  She wobbles back and forth for a moment, clutching at your ' + CoC.getInstance().player.legs() + ' to steady herself before she leans in, brushing her scaled nose against your vulva.  You shudder from the sudden contact with her scales, but she doesn\'t rush it, instead huffing and sniffing at the moist entrance.  Her alcohol-lidded eyes gaze up at you, the bleary orbs slightly confused as her jaw slowly opens to release her tongue.\n\n', false );
		...
		EngineCore.outputText( 'entrance of your loins.  You swoon, leaning against the chair\'s back and scooting your crotch forward in a fit of wanton need.  The tip of Roxanne\'s nose disappears into the moist lips, her slick, oral organ engulfed by your lusty tunnel.  It squirms and wriggles inside you, the thick pink protrusion lashing about, swirling in a circular motion to lap the juice from your walls.  Spit begins to drip down your ' + CoC.getInstance().player.legs() + ' and you see Roxanne\'s saliva frothing at the corners of her mouth while she works you over.\n\n', false );
		...
		if( CoC.getInstance().player.clitLength < 16 ) {
	...
	Roxanne.prototype.roxanneRimjob = function() {
		EngineCore.spriteSelect( 78 );
		EngineCore.outputText( '', true );
		EngineCore.outputText( 'You shimmy out of your ' + CoC.getInstance().player.armorName + ' and smirk, turning around to lean onto the table', false );
	...
	Roxanne.prototype.roxanneReamsYouNormal = function() {
		EngineCore.spriteSelect( 78 );
		EngineCore.outputText( '', true );
		EngineCore.outputText( 'A foot interposes itself between you and the table, and a shove sends you flopping onto the booze-soaked planks face-down, the last few inches of descent slowed by Roxanne\'s firm hands.  A scaled digit digs under your ' + CoC.getInstance().player.armorName + ', pulling and tugging.  As each piece is forcibly removed, you\'re left more and more naked, totally exposed to the heat of the fire and the smooth scales of the lizan\'s body.  The room lurches when you to try to move and stop her, and you flop back down, ignoring the drunken spin of your surroundings.  Laughing heartily, the pirate leans over you to pin your hands to the table.  While her position has the intended effect of restraining you, it also gives you the enviable ability to feel Roxanne\'s iron-hard nipples pressing into your back.\n\n', false );
		...
		CoC.getInstance().player.buttChange( Math.floor( 30 + (CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00225 ] / 4) ), true, true, false );
		EngineCore.outputText( '\n\n', false );
		...
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( 'against your own, the ' + Utils.num2Text( CoC.getInstance().player.balls + 2 ) + ' orbs rolling around each other in their sweaty sacks', false );
		} else if( CoC.getInstance().player.hasVagina() ) {
		...
		EngineCore.outputText( '  She doubles her pace suddenly, balls slapping loudly into you while she moans and howls lustily.  You feel the telltale bulging of an impending orgasm in your gut, and her balls drag up your ' + CoC.getInstance().player.skinDesc + ', preparing to disgorge their steamy cargo.  The lizan grabs your hair and twists her fingers through it while she hilts you, screaming out her pleasure for all to hear.\n\n', false );
		...
		EngineCore.outputText( 'You wake in the lizan\'s bed<b> with a nasty hangover</b>, her arm curled around your gurgling belly.  From how sore your rear feels, she kept \'winning\' at least two or three more times.  Your head is pounding, your ' + CoC.getInstance().player.legs() + ' are weak, and you dribble cum with every movement.  It takes some doing to extricate yourself from Roxanne\'s slumbering form, but you find your equipment and leave, hanging your head in shame under the leering eyes of the caravan-goers.', false );
	...
	Roxanne.prototype.roxanneFucksYourAssOHGODITSHUGE = function() {
		...
		} else if( CoC.getInstance().player.tallness < 50 ) {
		...
		EngineCore.outputText( '"<i>Ohhh, I\'ve waited for this for so long ' + CoC.getInstance().player.short + '.  You have no idea of the need... it just builds and builds, getting bigger and bigger until every step is a cacophony of pleasure that never ends.  I\'m never sated... never rested... I can\'t even please myself.  I just have to let it grow... and grow until I can find a drunk ' + CoC.getInstance().player.mf( 'boy', 'slut' ) + ' like you to slake my need and give me reprieve from the curse,</i>" explains Roxanne as she shreds her own clothes in a frenzy.  Her lithe, scaly body glitters in the candle-light, the pendulous swell of her breasts nearly threatening to pull your eyes from the blood-engorged beast between her thighs.  Lashing from side to side while she advances, the lizan\'s tail betrays the rampant emotions surging through her.\n\n', false );
		EngineCore.outputText( 'The nude newhalf pounces, her thigh-sized monster-cock gushing pre-cum over your belly and twitching in anticipation of the violation to come.  Her need is so cute... endearing even, but it isn\'t until your gear is being removed that you remember Roxanne intends to hilt that thing inside you.  Your eyes go wide, your mouth stammering and blubbering about how it will never fit.  She places a scaled finger on your boozy lips and whispers, "<i>Shhhh, don\'t struggle.  You knew what you were getting into, ' + CoC.getInstance().player.mf( 'my dear', 'sexy girl' ) + '.  You wouldn\'t welch out on a wager, would you?  Besides, it\'s much too late for you to do anything... my nude, drunken cock-holster.</i>"\n\n', false );
		EngineCore.outputText( 'Oh gods, Roxanne\'s right.  She finished getting you naked while she was talking, and you\'re completely exposed.  Her hands grab your arm and pull, rolling you onto your front and letting your ' + CoC.getInstance().player.legs() + ' hang off the side of her bed toward the floor.  It\'s such a silly pose that you nearly forget how perfect a target it makes your ' + Descriptors.buttDescript() + '!  The now-confident lizan gives your butt-cheek a slap, giggling when you start in surprise.  ', false );
		if( CoC.getInstance().player.tailType > AppearanceDefs.TAIL_TYPE_NONE ) {
		...
		if( CoC.getInstance().player.analCapacity() < 150 ) {
			EngineCore.outputText( 'Oh, poor ' + CoC.getInstance().player.mf( 'boy', 'girl' ) + '!  I don\'t think you can handle me like this.  Let me get the ointment... after all, I want you to come back for more.', false );
		} else {
		...
		} else if( CoC.getInstance().player.hasVagina() && CoC.getInstance().player.wetness() >= 4 ) {
	...
	Roxanne.prototype.applyHangover = function() {
		...
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.Hangover ) >= 0 ) {
			CoC.getInstance().player.changeStatusValue( StatusAffects.Hangover, 1, 8 );
		}//No hangover yet?  Create and yoink stats;
		else {
			CoC.getInstance().player.createStatusAffect( StatusAffects.Hangover, 8, 0, 0, 0 );
			//Strength minus 5;
			var temp = 5;
			while( temp > 0 ) {
				temp--;
				...
				if( CoC.getInstance().player.str >= 2 ) {
					...
					CoC.getInstance().player.addStatusValue( StatusAffects.Hangover, 2, 1 );
				}
			}
			//speed minus 10;
			temp = 10;
			while( temp > 0 ) {
				...
				if( CoC.getInstance().player.spe >= 2 ) {
					...
					CoC.getInstance().player.addStatusValue( StatusAffects.Hangover, 3, 1 );
				}
			}
			//int minus 15;
			temp = 15;
			while( temp > 0 ) {
				...
				if( CoC.getInstance().player.inte >= 2 ) {
					...
					CoC.getInstance().player.addStatusValue( StatusAffects.Hangover, 4, 1 );
				}
			}
		}
	...
	Roxanne.prototype.bigBootyRoxanneContestLoss = function() {
		...
		EngineCore.outputText( 'Gods, your head is swimming!  The room is pitching from side to side, and you ' + CoC.getInstance().player.mf( 'chuckle', 'giggle' ) + ' as you idly wonder if this is what it would be like aboard Roxanne\'s ship.  Still, the well-endowed shemale doesn\'t seem to mind your tipsy bumbling in the slightest.  She hooks her hands under your [butt] and squeezes the spacious ass-flesh appreciatively, then growls, "<i>Such a nice, round ass; perfect for stuffing with cock!  You\'re wasted as a champion.  You\'d make a better cabin-' + CoC.getInstance().player.mf( 'boy', 'girl' ) + ',</i>" into your ear as she drags you towards her wagon.' );
		...
		EngineCore.outputText( '\n\nThe drunken, scaled dick-girl pulls most of the way out a few seconds later, teasing, "<i>Feeling empty, dear?  I\'ll fill you up, my cabin-' + CoC.getInstance().player.mf( 'boy', 'girl' ) + ',</i>" before plunging back in.  Roxanne\'s hips begin to pump your gaping asshole with practiced efficiency, every stroke smearing her rounded cock-tip [if (hasCock = true) "across your pre-greased prostate.  You cannot help but moan in forced bliss." else "through every sensitive part of your body, hard enough to make you shiver from the onslaught of sensation."] Roxanne cries out ecstatically and shoots large globules of fluid into your rectum.  Muffled, wet squishes and audible liquid churning can be heard from your guts, but you\'re pretty certain it\'s still only pre-cum.' );
		...
		EngineCore.outputText( '\n\n<b>LATER...</b>\n' );
		EngineCore.outputText( 'You wake in the lizan\'s bed, her arm curled around your gurgling belly.  From how sore your rear feels, she probably kept \'winning\' at least two or three more times.  Your head is pounding, your ' + CoC.getInstance().player.legs() + ' are weak, and you dribble cum from your ass with every movement.  It takes some doing to extricate yourself from Roxanne\'s slumbering form, but you find your equipment and leave, hanging your head in shame under the leering eyes of the caravan-goers.' );
		//(-100 lust, -1 int, hangover effect);
		CoC.getInstance().player.orgasm();
		EngineCore.dynStats( 'int', -1 );
		this.applyHangover();
	...
	CoC.getInstance().registerScene( 'roxanne', new Roxanne() );
} );