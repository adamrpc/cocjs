...
angular.module( 'cocjs' ).run( function( SceneLib, MainView, $rootScope, PerkLib, Descriptors, AppearanceDefs, CoC, kFLAGS, Utils, StatusAffects, EngineCore ) {
	...
	Roxanne.prototype.timeChange = function() {
		...
		CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00225 ]++;
		//Reset if she finds someone to take it (random at high values);
		if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00225 ] >= 300 && CoC.time.hours === 1 && Utils.rand( 5 ) === 0 ) {
		...
		if( CoC.player.findStatusAffect( StatusAffects.Hangover ) >= 0 ) {
			...
			if( CoC.player.statusAffectv1( StatusAffects.Hangover ) > 0 ) {
				...
				CoC.player.inte += CoC.player.statusAffectv4( StatusAffects.Hangover );
				EngineCore.dynStats( 'cor', 0 );
				//Clear status;
				CoC.player.removeStatusAffect( StatusAffects.Hangover );
				return true;
			}
		}
		if( CoC.time.hours > 23 && CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00227 ] > 0 ) {
			CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00227 ]--; //Reduce drinking contest bonus
		}
		return false;
	};
	...
	Roxanne.prototype.roxanneDrinkingContestLoseDeliberately = function() {
		CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00226 ] = 1;
	...
	Roxanne.prototype.roxanneDrinkingContest = function() {
		...
		if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00226 ] === 0 ) {
			score = (CoC.player.tallness * ((CoC.player.thickness + 100) / 200) * (CoC.player.tou / 100)) + CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00227 ];
		}
		//If score is less than 30-50 (Strahza is inconsistant!);
		//[Lose!] ;
		if( score < (45 + Utils.rand( 20 )) ) {
			//Increment loss count!;
			CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00223 ]++;
		...
		//[WIN];
			...
			//Increment win count!;
			CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00222 ]++;
	...
	Roxanne.prototype.applyHangover = function() {
		...
		if( CoC.player.findStatusAffect( StatusAffects.Hangover ) >= 0 ) {
			...
			CoC.player.createStatusAffect( StatusAffects.Hangover, 8, 0, 0, 0 );
			//Strength minus 5;
			var temp = 5;
			while( temp > 0 ) {
				temp--;
				...
				if( CoC.player.str >= 2 ) {
					...
					CoC.player.str--;
					CoC.player.addStatusValue( StatusAffects.Hangover, 2, 1 );
				}
			}
			//speed minus 10;
			temp = 10;
			while( temp > 0 ) {
				...
				if( CoC.player.spe >= 2 ) {
					...
					CoC.player.spe--;
					CoC.player.addStatusValue( StatusAffects.Hangover, 3, 1 );
				}
			}
			//int minus 15;
			temp = 15;
			while( temp > 0 ) {
				...
				if( CoC.player.inte >= 2 ) {
					...
					CoC.player.inte--;
					CoC.player.addStatusValue( StatusAffects.Hangover, 4, 1 );
				}
			}
		}
