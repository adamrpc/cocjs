...
angular.module( 'cocjs' ).run( function( MainView, SceneLib, Utils, PerkLib, StatusAffects, Descriptors, CockTypesEnum, OnLoadVariables, AppearanceDefs, kFLAGS, CoC, EngineCore ) {
	function Dreams() {
	}

	Dreams.prototype.dreamSelect = function() {
		var daydream = 0;
		var choices = [].concat(
			_.times( CoC.player.humanScore(), _.constant( 0 ) ),
			_.times( CoC.player.dogScore(), _.constant( 2 ) ),
			_.times( CoC.player.horseScore(), _.constant( 1 ) ),
			_.times( CoC.player.cowScore(), _.constant( 3 ) ),
			_.times( CoC.player.catScore(), _.constant( 4 ) ),
			_.times( CoC.player.demonScore(), _.constant( 5 ) )
		);
		//dream quantity;
		var dreamtemp = Utils.rand( CoC.player.lib / 10 + CoC.player.cor / 20 ) + CoC.player.cor / 20;
		...
		if( CoC.flags[ kFLAGS.AKBAL_SUBMISSION_COUNTER ] > 4 ) {
			if( CoC.flags[ kFLAGS.AKBAL_SUBMISSION_COUNTER ] > 10 ) {
				choices.push( 7 );
			}
			if( CoC.flags[ kFLAGS.AKBAL_SUBMISSION_COUNTER ] > 13 ) {
				choices.push( 7 );
			}
			if( CoC.flags[ kFLAGS.AKBAL_SUBMISSION_COUNTER ] > 15 ) {
				choices.push( 7 );
			}
			if( CoC.flags[ kFLAGS.AKBAL_SUBMISSION_COUNTER ] > 18 ) {
				choices.push( 7 );
			}
			choices.push( 7 );
			choices.push( 7 );
		...
		//Exgartucock;
		if( CoC.player.statusAffectv1( StatusAffects.Exgartuan ) === 1 && CoC.player.hasCock() ) {
			choices.push( 9 );
			choices.push( 9 );
			choices.push( 9 );
		}
		//Latexy Skinz;
		if( CoC.player.skinAdj === 'latex' || CoC.player.skinAdj === 'rubber' ) {
			choices.push( 10 );
			choices.push( 10 );
			choices.push( 11 );
			choices.push( 11 );
		...
		if( CoC.flags[ kFLAGS.NUMBER_OF_TIMES_MET_SCYLLA ] > 0 && CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00150 ] > 0 ) {
			...
			choices.push( 12 );
			choices.push( 12 );
			if( CoC.player.hasCock() && CoC.player.cor > 50 ) {
				choices.push( 13 );
				choices.push( 13 );
				choices.push( 13 );
			}
		...
		if( SceneLib.anemoneScene.kidAXP() >= 40 && CoC.player.lust >= 70 && CoC.player.gender > 0 ) {
			choices.push( 15 );
			choices.push( 15 );
			choices.push( 15 );
		}
		//Sand trap;
		if( CoC.player.sandTrapScore() >= 2 ) {
			choices.push( 16 );
			choices.push( 16 );
			choices.push( 16 );
			choices.push( 16 );
		}
		if( CoC.player.mouseScore() >= 3 ) {
			choices.push( 17 );
			choices.push( 17 );
			choices.push( 17 );
			choices.push( 17 );
		...
		if( dreamLust > 0 ) {
			EngineCore.dynStats( 'lus', dreamLust );
		}
		...
		if( dreamtemp > 5 ) {
			...
			//canine;
				...
				if( CoC.player.gender <= 1 || (CoC.player.gender === 3 && Utils.rand( 2 ) === 0) ) {
					if( Utils.rand( 2 ) === 0 ) {
						MainView.outputText( 'locking a thick knotted cock inside a female, the pheromones of her heat making your maleness twitch and flex, the knot bulging obscenely as you begin to impregnate her.  ', false );
					} else {
						MainView.outputText( 'stroking a knotted doggie-prick, gently stroking and squeezing it, yipping in pleasure as your pointed dog-cock leaks steady streams of fluids.  ', false );
					}
				}
				//female;
				else {
					...
					if( CoC.player.inHeat ) {
						MainView.outputText( 'being pregnant, your belly bulging obscenely with the fruits of all your frequent copulations, and your breasts swollen with breast milk.  You imagine your condition enhancing your libido, driving you seek out sexual partners willing to pleasure your distorted form.  ', false );
					}
					//normal dream;
					else {
						MainView.outputText( 'being in heat, your cunt sopping wet with moisture and desire, intense pheromones pouring off you to make all the males rigid and ready.  ', false );
					}
				}
			}
			//horse;
			...
			//Scylla + dom + dong;
				...
				if( CoC.player.gender <= 1 ) {
					if( Utils.rand( 2 ) === 0 ) {
						...
						if( CoC.player.hasCock() ) {
							MainView.outputText( '  [EachCock] is fully erect and throbbing.' );
						} else if( CoC.player.hasVagina() ) {
							MainView.outputText( '  Your [vagina] dripping wet with need.' );
						}
						MainView.outputText( '  You shudder as you recall the erotic dream you were just having, a hand caressing your aching loins.' );
						//end dream;
						//lust increase;
					}
				...
				//Dream about living in a borrow and having lots and lots of kids.  Many furred, big eared bodies are running around and you feel the pregnant belly of your mate, or feel your own pregnant belly.;
					...
					MainView.outputText( 'You have a great larder, able to feed everyone as much as they need.  It has never been a problem to keep it full.  It\'s a veritable paradise for a rodent such as yourself.  Then you find your way into the master bedroom and see its massive bed, the site where your great family was brought into existence.\n\n', false );
					if( CoC.player.gender === 1 || (CoC.player.gender === 3 && Utils.rand( 2 ) === 0) ) {
						MainView.outputText( 'Things blur for a moment, and you find your head resting against a pregnant belly, covered in fine fur.  The results of your virility as a father, and soon to be added to the great brood that you helped bring into this world...\n\n', false );
					} else {
						MainView.outputText( 'Things blur for a moment, and you find yourself laying on the bed with your hands wrapped around your heavily pregnant belly.  The product of your fertility and the virility of your great mate, and soon to be added to the great brood you\'ve already brought into the world...\n', false );
					}
				}
			}
	...
	Dreams.prototype.dayTenDreams = function() {
		MainView.outputText( '\n', false );
		if( CoC.player.hasCock() ) {
			if( CoC.player.cor < 50 ) {
				MainView.outputText( 'Your task completed, you return through the portal to your home.  Exhausted but happy, the sight of your village fills you with a surge of emotions.  Most prominently, and perhaps most enjoyably: pride.  You did it.\n\n', false );
				MainView.outputText( 'You drink and celebrate with your people long into the night, feasting and receiving accolades and praise.  It is a party the likes of which you have never seen, and rightfully so.  Never again will your village be threatened by the demons.  Late in the evening, you arrive at the home of your childhood and greet your mother.\n\n', false );
				MainView.outputText( '"<i>Welcome back,</i>" she smiles.\n\n', false );
				MainView.outputText( '"<i>Thank you, Mom,</i>" you respond, wrapping your arms around her and kissing her deeply.  Your hands take hold of her rear, gripping-\n\n', false );
				MainView.outputText( 'Shocked awake, you blink as the light of your dying fire flickers dimly across the campsite.  That was...\n\n', false );
				MainView.outputText( 'This place is getting to you.', false );
			} else {
				MainView.outputText( 'Your task completed, you return through the portal to your home.  Exhausted but happy, the sight of your village fills you with a surge of emotions.  Most prominently, and perhaps most enjoyably: pride.  You did it.\n\n', false );
				...
				MainView.outputText( '"<i>Welcome back,</i>" she smiles.\n\n', false );
				MainView.outputText( '"<i>Thank you, Mom,</i>" you respond, wrapping your arms around her and kissing her deeply.  Your hands take hold of her rear, gripping it in great handfuls and squeezing her possessively.  Nearly throwing her in the bed, you delight in her playful squeals.  Mounting her, you tear away her dress and plunge into her cunt.\n\n', false );
				MainView.outputText( '"<i>Oooh, god!</i>" she exclaims,  "<i>Such a good boy!  Such a big boy!</i>"\n\n', false );
				MainView.outputText( '"<i>That\'s right, Mom,</i>" you grunt, taking hold of her breast and sucking harshly on it.  "<i>All grown up.</i>"  You continue to grope her roughly as you pound into her cunt, slapping your balls into her thighs.  Her pussy squeezes you with a mother\'s love, and you soon find yourself dumping heaping loads of your spunk within her.  You rise, and bring your dick to her mouth so that she can clean you.\n\n', false );
				MainView.outputText( 'As your mother laps at your shaft, coated in her juices, you look confidently down at her belly.  Though you love your mother, you\'re looking forward to having a daughter, and training her to love you just as much as a mother does. One day, perhaps even to replace her.\n\n', false );
				MainView.outputText( 'You awaken in a sweat.  Fuck.', false );
			}
		} else {
			if( CoC.player.cor < 50 ) {
				MainView.outputText( 'Your task completed, you return through the portal to your home.  Exhausted but happy, the sight of your village fills you with a surge of emotions.  Most prominently, and perhaps most enjoyably: pride.  You did it.\n\n', false );
				...
				MainView.outputText( '"<i>Welcome back,</i>" he smiles.\n\n', false );
				MainView.outputText( '"<i>Thank you, daddy,</i>" you respond, wrapping your arms around him and kissing him deeply.  Your hands slip into his pants, lowering-\n\n', false );
				MainView.outputText( 'Shocked awake, you blink as the light of your dying fire flickers dimly across the campsite.  That was...\n\n', false );
				MainView.outputText( 'This place is getting to you.', false );
			} else {
				MainView.outputText( 'Your task completed, you return through the portal to your home.  Exhausted but happy, the sight of your village fills you with a surge of emotions.  Most prominently, and perhaps most enjoyably: pride.  You did it.\n\n', false );
				MainView.outputText( 'You drink and celebrate with your people long into the night, feasting and receiving accolades and praise.  It is a party the likes of which you have never seen, and rightfully so.  Never again will your village be threatened by the demons.  Late in the evening, you arrive at the home of your childhood and greet your father.\n\n', false );
				MainView.outputText( '"<i>Welcome back,</i>" he smiles.\n\n', false );
				MainView.outputText( '"<i>Thank you, daddy,</i>" you respond, wrapping your arms around him and kissing him deeply.  Your hands slip into his pants, lowering them and exposing his crotch.  Too excited to control yourself, you drag your lips down his chest before wrapping them around his cock and taking him into your mouth.  You pull him deep, teasing his balls with your lower lip.  Though it isn\'t necessary to gag - you could have swallowed him easily - you do anyway.  Daddy deserves a show.\n\n', false );
				MainView.outputText( 'His cum splatters into your throat.\n\n', false );
				MainView.outputText( 'You smile with glazed teeth as you straddle him in his bed and mount him.  Some disappointment lingers in you that you didn\'t just let daddy take your ass then and there, but the prospect of his cum flooding your womb and fertilizing you is too delightful to pass up.\n\n', false );
				MainView.outputText( '"<i>Do you like it?</i>" you ask, before moaning his name.\n\n', false );
				MainView.outputText( '"<i>You\'re a good girl,</i>" he answers.\n\n', false );
				MainView.outputText( 'His balls gurgle as they fill you.  You squeal in delight and lick his chest, demonstrating your affection.  Clenching him tightly, you make sure not to spill daddy\'s seed, and imagine giving birth to a sexy, busty daughter, teaching her to love him.  One day, teaching her to replace you.\n\n', false );
				MainView.outputText( 'You wake up in a sweat.  Fuck.', false );
			}
		}
		if( CoC.player.cor < 50 ) {
	...
	Dreams.prototype.displacerDreamII = function() {
		...
		if( EngineCore.silly() ) {
			MainView.outputText( 'k' );
		}
		MainView.outputText( 'not a drop of jism has escaped.' );
	...
	Dreams.prototype.displacerDreamIII = function() {
		MainView.clearOutput();
		MainView.outputText( 'The details of the dream immediately begin to slip away.  You strain your brain, trying to commit everything to memory, but it slips out of your mind like water through a sieve.' );
		...
		var x = CoC.player.cockTotal();
		while( x > 0 ) {
			x--;
			if( CoC.player.cocks[ x ].sock === 'amaranthine' && CoC.player.cocks[ x ].cockType !== CockTypesEnum.DISPLACER ) {
				if( CoC.player.cocks[ x ].cockType !== CockTypesEnum.DOG ) {
					CoC.player.cocks[ x ].knotMultiplier = 1.5;
				}
