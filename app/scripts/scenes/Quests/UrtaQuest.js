...
angular.module( 'cocjs' ).run( function( MainView, SceneLib, $log, Sirius, GnollSpearThrower, MinotaurLord, MilkySuccubus, ItemSlot, AppearanceDefs, CockTypesEnum, StatusAffects, Player, PerkLib, ItemType, Combat, WeaponLib, ArmorLib, GoblinBroodmother, PregnancyStore, Descriptors, Utils, kFLAGS, CoC, EngineCore ) {
	...
	UrtaQuest.prototype.comfortUrtaAfterFamFamTalk = function() {
		MainView.clearOutput();
		MainView.outputText( 'You lean over the table and take Urta\'s hands in your own.  Her downy fur ruffles as you give her a comforting squeeze, the tension oozing from her body at your touch.  Urta sniffles a little and wipes her moistened cheek on her shoulder, muttering, "<i>Oh, ' + CoC.player.short + ', you\'re too good for me.</i>"  She pulls back and gives you a knowing smile.  "<i>You don\'t have to worry, really.  You turned this down on her luck lush into a happy woman.</i>"' );
		...
		if( SceneLib.urta.urtaDrunk() ) {
			MainView.outputText( 'boozy ' );
		}
		MainView.outputText( 'kiss on your lips as she hugs you tight - tight enough that you feel every vein through the concealing silk of her outfit.  "<i>I\'m a lucky girl.</i>"' );
	...
	UrtaQuest.prototype.yeeshUrtaAfterFamFamTalk = function() {
		MainView.clearOutput();
		MainView.outputText( 'You loudly exhale, "<i>Yeesh,</i>" with a bemused expression.' );
	...
	UrtaQuest.prototype.whoCaresUrtaAfterFamFamTalk = function() {
		MainView.clearOutput();
		MainView.outputText( '"<i>Who cares?</i>" you ask, without a hint of compassion.' );
	...
	UrtaQuest.prototype.infertilityQuestions = function() {
		MainView.clearOutput();
		MainView.outputText( 'You ask her about her infertility - specifically, how someone could be infertile AND unable to use transformative items without adverse effects.  Urta stirs her drink before shrugging noncommittally.' );
		...
		if( CoC.player.cor < 33 ) {
			MainView.outputText( 'you have nothing to complain about there.  Your sexual romps with Urta are always a delight.' );
		} else if( CoC.player.cor < 66 ) {
			MainView.outputText( 'she\'s definitely a handful in the sack, you\'ve got to give her that.' );
		} else {
			MainView.outputText( 'she\'s a willing fuck, so no complaints there.  Now if she would just get on with it - if you wanted to flirt, you would\'ve instigated it.' );
		}
	...
	UrtaQuest.prototype.startUrtaQuest = function() {
		...
		MainView.outputText( '<b>You are Urta, a hermaphroditic fox-morph.</b>' );
	...
	UrtaQuest.prototype.towerOfTheCovanant = function() {
		MainView.clearOutput();
		MainView.outputText( 'You get up to leave the table, nervously wrapping your tail down your leg, even though ' + CoC.player2.short + ' helped you overcome that timid habit long ago.  You tell ' + CoC.player2.mf( 'him', 'her' ) + ' that ' + CoC.player2.mf( 'he', 'she' ) + ' can\'t follow.  To bring ' + CoC.player2.mf( 'him', 'her' ) + ' into the tower would require countless tests for what should only take a few brief moments if you go alone.  Secretly, you wish ' + CoC.player2.mf( 'he', 'she' ) + ' could come with you and hold you tight, to comfort you through this nerve wracking ordeal.  Lifting your chin, you marshal your courage and walk out, wiggling your bottom in ' + CoC.player2.short + '\'s direction to remind ' + CoC.player2.mf( 'him', 'her' ) + ' what ' + CoC.player2.mf( 'he', 'she' ) + '\'ll be claiming tonight if a solution can be found.' );
	...
	UrtaQuest.prototype.getHelBangedAsUrta = function() {
		...
		if( CoC.flags[ kFLAGS.HEL_HARPY_QUEEN_DEFEATED ] > 0 ) {
			MainView.outputText( ', and even how they worked together to bring down an evil sorceress and save her father. Your heart swells with pride as she recounts ' + CoC.player2.short + ' doing so a noble thing, and for a moment you know you made the right pick in a lover..' );
		}
		MainView.outputText( '.  You sit and listen for a long few minutes, sipping at the beer until the liquid courage starts to work its wonders, giving you a little tingle... and helping to open you up, too.  Soon, you\'re drawn into the conversation, joking and teasing with the other girls as if you\'d been friends for ages.' );
	...
	UrtaQuest.prototype.leaveB4HelFucksUrta = function() {
		MainView.clearOutput();
		MainView.outputText( 'As quickly as you can, you blather out some excuse and grab your gear, trying to cover your huge erection as you run toward the door, blurting an awkward thanks over your shoulder for the drink...' );
	...
	UrtaQuest.prototype.fuckHelAndKitsunesAsUrta = function() {
		...
		if( CoC.player2.hasCock() ) {
			MainView.outputText( '  "<i>I\'ve got a better one ' );
			if( SceneLib.helFollower.followerHel() ) {
				MainView.outputText( 'at home' );
			} else {
				MainView.outputText( 'at my beck and call' );
			}
			MainView.outputText( ', you know...  But we\'ve got that in common, don\'t we?  Mmm, maybe we can share it some day?  A horny girl like you\'d just love that, woudn\'t you? Getting to worship ' + CoC.player2.mf( 'his', 'her' ) + ' cock beside me while my tail fills your ass so full you scream....</i>' );
		}
	...
	UrtaQuest.prototype.visitPCPreUrtaQuest = function( truth ) {
		...
		if( CoC.flags[ kFLAGS.ANEMONE_WATCH ] > 0 && CoC.flags[ kFLAGS.ANEMONE_KID ] > 0 ) {
			MainView.outputText( 'A confused-looking anemone with a ' + ItemType.lookupItem( CoC.flags[ kFLAGS.ANEMONE_WEAPON_ID ] ).longName + ' nearly sees you, but you duck around a rock and escape her notice.  ' );
		}
		if( CoC.flags[ kFLAGS.FUCK_FLOWER_LEVEL ] >= 4 && CoC.flags[ kFLAGS.FUCK_FLOWER_KILLED ] === 0 && CoC.flags[ kFLAGS.HOLLI_DEFENSE_ON ] > 0 ) {
			MainView.outputText( 'A dangling tentacle nearly hits you in the dark.  You roll aside at the last moment, looking up to see dozens of the things idly dangling around.  When did this camp get such a corrupt tree in it?  You suppose it must have its uses in defending against foes from the sky...  ' );
		}
		if( SceneLib.camp.companionsCount() > 2 ) {
			MainView.outputText( 'There are a number of people in the camp, but you avoid them as you head towards ' + CoC.player2.short + '\'s bunk.  ' );
		}
		...
		if( truth ) {
			...
			if( CoC.player2.cor < 33 ) {
				MainView.outputText( CoC.player2.mf( 'he', 'she' ) + ' argues that ' + CoC.player2.mf( 'he', 'she' ) + ' should come anyway.  You tell ' + CoC.player2.mf( 'him', 'her' ) + ' no, firmly, even though it breaks your heart.' );
			} else if( CoC.player2.cor < 66 ) {
				MainView.outputText( CoC.player2.mf( 'he', 'she' ) + ' nods understandingly, not surprised that you have to handle this quest alone.  The two of you are alike in that way - each with their own burden to bear.' );
			} else {
				MainView.outputText( CoC.player2.mf( 'he', 'she' ) + ' nods dismissively, trusting you to handle it alone you suppose.  ' + CoC.player2.short + ' always was hard like that, even though ' + CoC.player2.mf( 'he', 'she' ) + ' clearly likes you.' );
			}
			MainView.outputText( '  You hold on to ' + CoC.player2.mf( 'him', 'her' ) + ', for comfort, dreading the long day ahead.  ' + CoC.player2.mf( 'He', 'She' ) + ' pats your back comfortingly as you cry, and when you break from the hug, ' + CoC.player2.mf( 'he', 'she' ) + '\'s looking at you expectantly.' );
			if( CoC.flags[ kFLAGS.SLEEP_WITH ] !== '' && CoC.flags[ kFLAGS.SLEEP_WITH ] !== 0 ) {
				MainView.outputText( '  ' + CoC.player2.short + ' suggests the two of you move off, so as not to wake ' + CoC.flags[ kFLAGS.SLEEP_WITH ] + ' in the night.' );
			}
		}
		//{LIE};
		else {
	...
	UrtaQuest.prototype.urtaPutsHerDickInPCsWithDicks = function( truth ) {
		...
		if( CoC.player2.tallness < 60 ) {
			MainView.outputText( ' and let your tits hang over ' + CoC.player2.mf( 'his', 'her' ) + ' ' + CoC.player2.face() );
		} else if( CoC.player2.tallness < 76 ) {
			MainView.outputText( ' and let your tits swing between you' );
		} else if( CoC.player2.tallness < 84 ) {
			MainView.outputText( ' and let your tits smash into ' + CoC.player2.mf( 'his', 'her' ) );
		} else {
			MainView.outputText( ' and let your tits sway over ' + CoC.player2.mf( 'his', 'her' ) + ' belly' );
		}
		MainView.outputText( ', giving ' + CoC.player2.mf( 'him', 'her' ) + ' something to enjoy while you begin to shift position, dragging your dick down ' + CoC.player2.mf( 'his', 'her' ) + ' length' );
		if( CoC.player2.hasVagina() ) {
			MainView.outputText( ', past ' + CoC.player2.mf( 'his', 'her' ) + ' pussy,' );
		}
		MainView.outputText( ' and towards the prize you\'ve been wanting since you got here - ' + CoC.player2.mf( 'his', 'her' ) + ' asshole.' );
		...
		if( CoC.player2.analCapacity() > 70 ) {
			MainView.outputText( '  Thankfully, ' + CoC.player2.mf( 'he', 'she' ) + '\'s nice and stretched from all the butt-fucking ' + CoC.player2.mf( 'he', 'she' ) + '\'s had to take, and you easily pop inside.  It feels so good that your flare grows wider, pulling you deeper into that welcoming, tight anal heat.' );
		} else if( CoC.player2.analCapacity() > 50 ) {
			MainView.outputText( '  Thankfully, ' + CoC.player2.mf( 'he', 'she' ) + '\'s got plenty of give in ' + CoC.player2.mf( 'his', 'her' ) + ' back-door, and you pop inside after applying a little more pressure.  It feels so good that your flare grows wider, slowing your descent into that tight, anal heat.' );
		} else {
			MainView.outputText( '  Unfortunately, there\'s not much give in ' + CoC.player2.mf( 'his', 'her' ) + ' back-door, and you have to sit there smearing pre over it, again and again, each time pushing a little harder.  After what feels like the fiftieth try, you finally pop your fat flare into ' + CoC.player2.mf( 'his', 'her' ) + ' rectum, the tight anal heat causing you to flare from the force of ' + CoC.player2.mf( 'his', 'her' ) + ' pressure.  Slowly, you push inch after throbbing inch of your swollen, stallion-like erection into ' + CoC.player2.mf( 'his', 'her' ) + ' asshole.' );
		}
		if( CoC.player2.ass.analLooseness === 0 ) {
			MainView.outputText( '  <b>You\'ve taken ' + CoC.player2.mf( 'his', 'her' ) + ' anal virginity!</b>' );
			CoC.player2.ass.analLooseness = 1;
		}
		MainView.outputText( '  ' + CoC.player2.short + ' writhes as ' + CoC.player2.mf( 'he', 'she' ) + ' takes you, ' + CoC.player2.mf( 'his', 'her' ) + ' hips lifting slightly off the ground to make sure you grind into ' + CoC.player2.mf( 'his', 'her' ) + ' prostate.  A long strand of clear excitement oozes from ' + CoC.player2.mf( 'his', 'her' ) + ' tip' );
		if( CoC.player2.cockTotal() > 1 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ', thick, viscous, and clinging to the drying mess you left behind.' );
		...
		MainView.outputText( ' between your bodies, ' + CoC.player2.mf( 'his', 'her' ) + ' liquid love exploding all over your cock-locked forms.  Each time you cum, a fresh-squeezed spurt of semen splatters into your tits, ' + CoC.player2.mf( 'his', 'her' ) + ' prostate violently being milked by the heady flows of vixen-spunk you force inside ' + CoC.player2.mf( 'his', 'her' ) + ' rectum.' );
		if( CoC.player2.hasVagina() ) {
			MainView.outputText( '  You don\'t notice the female fluids that drip from her sodden slit - they soak into your big balls, already forgotten.' );
		}
		...
		if( CoC.player2.cumQ() >= 500 ) {
			MainView.outputText( ', mixing with ' + CoC.player2.mf( 'his', 'her' ) + ' own ' );
			if( CoC.player2.cumQ() < 1000 ) {
				MainView.outputText( 'puddly' );
			} else if( CoC.player2.cumQ() < 2500 ) {
				MainView.outputText( 'small lake-sized' );
			} else {
				MainView.outputText( 'massive, lake-sized' );
			}
			MainView.outputText( ' mess - only now do you realize just how much your lover could produce!  The' );
		} else {
			MainView.outputText( ', but the' );
		}
		MainView.outputText( ' semen-inundated fabric quickly begins to dry as the parched, wasteland dirt reclaims the moisture.' );
	...
	UrtaQuest.prototype.putAPCDickInUrtaCoochies = function( truth ) {
		...
		if( CoC.player2.gender === 3 ) {
			MainView.outputText( '...well, close enough... ' );
		}
		MainView.outputText( 'to ever fuck you like a woman.  Grabbing ' + CoC.player2.mf( 'his', 'her' ) + ' face, you pull ' + CoC.player2.mf( 'him', 'her' ) + ' into a deep, passionate kiss, unthinkingly grinding your dripping cock and steadily-wettening gash into ' + CoC.player2.mf( 'his', 'her' ) + ' belly and rubbing your own pebbly onyx nipples against ' + CoC.player2.mf( 'his', 'her' ) + ' nipples.  ' + CoC.player2.mf( 'His', 'Her' ) + ' own ' + CoC.player2.cockDescript( x ) + ' begins to drool precum' );
		if( CoC.player2.hasVagina() ) {
			MainView.outputText( ', her cunt dripping wetly onto the ground from the stimulus' );
		}
		MainView.outputText( ', the fluid rolling down its length to puddle with yours on ' + CoC.player2.mf( 'his', 'her' ) + ' belly and filling your writhing, passionate embrace with hot, sticky sweetness.' );
		...
		if( CoC.player2.isTaur() || CoC.player2.isDrider() ) {
			...
			if( CoC.player2.hasVagina() ) {
				MainView.outputText( ', her female jizz spattering unnoticed onto the ground behind the pair of you' );
			}
			MainView.outputText( '.' );
			...
			if( CoC.player2.cumQ() < 1000 ) {
				MainView.outputText( 'falling face-first into the jizz-mud' );
			} else {
				MainView.outputText( 'not even bothering trying to move your swollen bulk' );
			}
			MainView.outputText( ', and you smile dopily up at ' + CoC.player2.mf( 'him', 'her' ) + ', love shining in your eyes.  Carefully, ' + CoC.player2.mf( 'he', 'she' ) + ' positions ' + CoC.player2.mf( 'him', 'her' ) + 'self beside you and strokes your face before closing ' + CoC.player2.mf( 'his', 'her' ) + ' eyes, making it quite clear ' + CoC.player2.mf( 'he', 'she' ) + ' intends to go to sleep right here.  You see no point in fighting the inevitable and instead give yourself over to slumber - wouldn\'t be the first time you woke up in dried cum-mud, after all...' );
		}
		//Else:;
		else {
			...
			if( CoC.player2.hasVagina() ) {
				MainView.outputText( ', her female jizz spattering unnoticed onto the ground beneath the pair of you' );
			}
			MainView.outputText( '.' );
			//(PC Cum=Normal or lower:;
			if( CoC.player2.cumQ() < 500 ) {
			...
			if( CoC.player2.cumQ() >= 1000 ) {
				MainView.outputText( ' not to mention the weight of your overstuffed belly' );
			}
			MainView.outputText( ' you don\'t really feel inclined to move, either.  Instead, you yawn loudly, make yourself comfortable upon your lover\'s stomach, and allow yourself to drift off to sleep - already, you can hear the quiet breathing of ' + CoC.player2.short + ' as ' + CoC.player2.mf( 'he', 'she' ) + ' does the same thing.' );
		}
		CoC.player.orgasm();
		EngineCore.dynStats( 'sen', -2 );
		MainView.menu();
	...
	UrtaQuest.prototype.putUrtasWangInPCsCunt = function( truth ) {
		MainView.clearOutput();
		MainView.outputText( 'You push ' + CoC.player2.short + ' down on her back, a task made all the more difficult by your growing arousal.  Thankfully, she\'s not wearing her usual ' + CoC.player2.armorName + '.  Instead, her form is bared to your probing fingertips, pliant and exposed.  You cannot help but dip a finger into her moist delta, parting the oozing veil to enjoy the view of her ' );
		if( CoC.player.vaginaType() !== 5 ) {
			MainView.outputText( 'pink, ' );
		}
		MainView.outputText( 'pulsing interior.  The sight is exciting in the extreme, immediately sending your heartbeat through the roof.  Your tremendous horse-cock is immediately stiffened by your ardor, along with your coal-black nipples, and your armor-plated skirt begins to rise, as if by magic.' );
		...
		if( CoC.player2.pregnancyIncubation > 0 && CoC.player.pregnancyIncubation < 150 ) {
			MainView.outputText( 'the curve of her pregnant belly' );
		} else if( CoC.player2.tone >= 65 ) {
			MainView.outputText( 'her toned abdomen' );
		} else if( CoC.player2.tone < 30 ) {
			MainView.outputText( 'her soft, forgiving middle' );
		} else {
			MainView.outputText( 'her belly' );
		}
		MainView.outputText( '.  The attentions drive your attempts to dislodge the offending garments from your lusty frame to new levels.  Metal rivets slowly give before finally snapping apart to free your left shoulder.  Your left nipple tingles with pleasant, palpable relief, and you turn to the other side, intent on ripping it off, so that you can be with your lover, skin to skin.' );
		...
		if( CoC.player2.isTaur() ) {
			MainView.outputText( 'and climb off the \'taur-woman.  She smirks up at you, until you bid her to rise, telling her that you plan to mount her like the fertile filly she is. Reluctantly, ' + CoC.player2.short + ' clatters up onto her hooves.' );
			if( CoC.player2.tailType > AppearanceDefs.TAIL_TYPE_NONE ) {
				MainView.outputText( '  You stroke her tail as you shift it aside' );
			} else {
				MainView.outputText( '  You stroke her swollen sex as you admire it' );
			}
			MainView.outputText( ', making sure you\'ve got the perfect view of her lube-slimed horse-cunt.  Unbidden, your cock thickens, disgorging a potent trail of animal-pre down its underside in anticipation.' );
			if( CoC.player2.wetness() >= 4 ) {
				MainView.outputText( '  Fuck, she\'s wetter than Edryn!  Just look at those heavy trails of slick moisture as they run down her rump and hindlegs.' );
			} else if( CoC.player2.wetness() >= 2 ) {
				MainView.outputText( '  Fuck, she\'s as wet as Edryn!  Beads of moisture are forming on her clit, ready to drop to the ground.' );
			} else {
				MainView.outputText( '  Fuck, she\'s almost as wet as Edryn!  Just getting a sniff of her horny horse-snatch has your head swimming!' );
			}
			MainView.outputText( '  You give it a lick and shudder, taking in a few deep lungfuls of her scent.' );
			if( CoC.player2.tallness >= 84 ) {
				MainView.outputText( '  Before you know what you\'re doing, you\'ve pulled her back to a rock and climbed atop it, bringing your cock level with that heavenly hole.' );
			} else {
				MainView.outputText( '  Before you know what you\'re doing, you\'ve raised yourself up to your full height, bringing your cock level with that heavenly hole.' );
			}
		}
		//{goo};
		else if( CoC.player2.isGoo() ) {
			MainView.outputText( 'and roll the gooey woman so she\'s face down.  She holds herself up off the dry dirt with her palms, sweet, slimy bottom upraised above her goopy undercarriage.  Amazingly, there\'s a slight parting in the mass of slime, right between where her legs would be, if she had them.  A narrow, barely visible divide can be spotted inside, capped with the telltale bulge of her ' );
			if( CoC.player2.clitLength < 0.5 ) {
				MainView.outputText( 'tiny ' );
			} else if( CoC.player2.clitLength >= 2.5 ) {
				MainView.outputText( 'massive ' );
			}
			MainView.outputText( 'button.  You nuzzle up to that slick slime-cunt, sniffing at her super-wet snatch, taking the pheromones as deeply inside yourself as possible.  Her femininity is so strong that it makes your head spin and your dick ache.  Glancing down as you lick the moisture from your muzzle, you see the rigid, over-engorged veins standing out in harsh relief on your equine tool.  Your legs seem to move on their own, lining you up with the moisture-laden slit just inches away.' );
		}
		//{nagas};
		else if( CoC.player2.isNaga() ) {
			MainView.outputText( 'and roll the snake-woman over so she\'s prostrate on the ground, barely holding her front half up with her arms.  Her loins are flat on the ground, undulating slightly as she grinds her moist clit against it for sensation.  Your lover\'s anatomy poses a problem for your choice of positions, but then, inspiration strikes.  You come up under the hind part of her snake-tail, and hoisting it up, you support half over your shoulder.  As expected, she has all the flexibility of the serpent she so resembles, and her tail bends down just far enough for her cunt to hang a mere inch above your rigid tool, right where her body changes from snake to person.' );
			if( CoC.player2.wetness() >= 3 ) {
				MainView.outputText( '  Her moisture drizzles from that wanton slit, basting you with her fragrant femininity, and you have a hard time not taking her here and now.' );
			}
			MainView.outputText( '  The tail you\'re holding up coils in, sliding around your arm and middle, wrapping you up in snakey snugness.  With impeccable control, she squeezes a little bit, bringing her cunt close enough to breathe foggy moisture on your equine tool.' );
		}
		//{biped};
		else {
			MainView.outputText( 'and roll her over on her hands and knees, lifting that sweet butt up in the air where you can admire it.  You kneel behind her and, pressing your nose into her fragrant quim, you extend your tongue, letting the smooth oral muscle taste her juices and tease at her engorging clit.  ' + CoC.player2.short + ' moans and presses back into you, crying out like a true bitch in heat, one in need of a thorough rutting.  The thought hits your cock like a lightning bolt, and it fills out to painful proportions, every vein and bulge standing out in stark relief, thumping obscenely to the tune of your insatiable lust.  You moan into her lovely-tasting snatch, gulping down ' );
			if( CoC.player2.wetness() < 3 ) {
				MainView.outputText( 'every drop of nectar that she gives up' );
			} else if( CoC.player2.wetness() < 4 ) {
				MainView.outputText( 'the copious drops of nectar she gives up' );
			} else if( CoC.player2.wetness() < 5 ) {
				MainView.outputText( 'the drizzling moisture that oozes from her twat' );
			} else {
				MainView.outputText( 'the rivers of girl-lube that run unimpeded from her sopping twat' );
			}
			MainView.outputText( '.  You sniff in great lungfuls of her, your head swimming with need, and before you know it, you\'ve risen behind her, your oozing flare just inches from that delectable opening.' );
		}
		...
		if( CoC.player2.pregnancyIncubation > 0 && CoC.player2.pregnancyIncubation < 150 ) {
			MainView.outputText( ' (more than it already is)' );
		}
		MainView.outputText( ' from your prodigious deposit.  The illusion of knocking her up takes hold of you, and your orgasm redoubles, fresh waves of horse-cum pouring out of you unceasingly.  ' + CoC.player2.short + ' moans, rubbing her taut' );
		if( CoC.player2.pregnancyIncubation > 0 && CoC.player2.pregnancyIncubation < 150 ) {
			MainView.outputText( ' pregnant' );
		}
		MainView.outputText( ' dome, until at last you finally spend the last of your spunk.' );
	...
	UrtaQuest.prototype.snuggleWithUrta = function( truth ) {
		MainView.clearOutput();
		MainView.outputText( 'You think it over and decide that, for once, you\'d like to focus more on love and less on lust.  "<i>Well... I\'d really just like to have a snuggle with you, ' + CoC.player2.short + ' - if that\'s alright?</i>"  You ask quietly, tail instinctively wagging in an effort to be endearing.  Your lover nods ' + CoC.player2.mf( 'his', 'her' ) + ' head and answers in the positive, opening ' + CoC.player2.mf( 'his', 'her' ) + ' arms for further effect.  You waste no time and eagerly sidle into ' + CoC.player2.mf( 'his', 'her' ) + ' arms.' );
		//[If PC Centaur or Drider:;
		if( CoC.player2.isTaur() || CoC.player2.isDrider() ) {
			MainView.outputText( '  It\'s rather awkward, thanks to ' + CoC.player2.mf( 'his', 'her' ) + ' clunky ' );
			if( CoC.player2.isTaur() ) {
				MainView.outputText( 'centaur' );
			} else {
				MainView.outputText( 'drider' );
			}
			MainView.outputText( ' body, but you manage to press up against ' + CoC.player2.mf( 'his', 'her' ) + ' ' );
			if( CoC.player2.isTaur() ) {
				MainView.outputText( 'forehoofs' );
			} else {
				MainView.outputText( 'first two pairs of spidery limbs' );
			}
			MainView.outputText( '.' );
		}
		//[if PC Naga:];
		else if( CoC.player2.isNaga() ) {
			MainView.outputText( '  ' + CoC.player2.mf( 'His', 'Her' ) + ' serpentine coils loop snugly around your waist, curling around your legs and holding you gently in place.' );
		}//[if PC Goo:;
		else if( CoC.player.isGoo() ) {
			MainView.outputText( '  ' + CoC.player2.mf( 'His', 'Her' ) + ' soft, squishy goo-body eagerly accepts you, the gelatinous mass of ' + CoC.player2.mf( 'his', 'her' ) + ' lower torso reaching out to softly envelop you in its cool, wet embrace.' );
		}//[Else:;
		else {
			MainView.outputText( '  Knees to knees, it takes you a few moments to figure out how to properly position your legs, but you eventually manage it.' );
		}
		...
		if( CoC.player2.cor < 33 ) {
			MainView.outputText( 'playful pat' );
		} else if( CoC.player2.cor < 66 ) {
			MainView.outputText( 'gentle stroke' );
		} else {
			MainView.outputText( 'lascivious caress' );
		}
	...
	UrtaQuest.prototype.morningAfterCampVisitEmbark = function( truth ) {
		...
		MainView.outputText( 'Sadly, morning comes, and your time with ' + CoC.player2.short + ' comes to an end.  You push ' + CoC.player2.mf( 'his', 'her' ) + ' shoulder gently to rouse ' + CoC.player2.mf( 'him', 'her' ) + ', smiling at ' + CoC.player2.mf( 'him', 'her' ) + ' when ' + CoC.player2.mf( 'he', 'she' ) + ' wakes.  You kiss ' + CoC.player2.mf( 'him', 'her' ) + ' on the lips before rolling out from under the blanket.  The two of you eat a quiet breakfast together, simply enjoying each other\'s company.  ' );
		if( truth ) {
			MainView.outputText( '' + CoC.player2.mf( 'He', 'She' ) + ' asks again about coming with you, but you tell ' + CoC.player2.mf( 'him', 'her' ) + ' Gul\'s instructions were quite specific.  You\'ll need to tackle this alone.  The rest of breakfast passes in uncomfortable silence, broken when you rise to leave.' );
		}
		MainView.outputText( '\n\nYou embrace ' + CoC.player2.short + ', kissing ' + CoC.player2.mf( 'him', 'her' ) + ' on the lips' );
		if( truth ) {
			MainView.outputText( ' before you promise, "<i> I\'ll come back to you, stronger and ready for kids, I promise.</i>"\n\n' );
		} else {
			MainView.outputText( '.  ' );
		}
	...
	UrtaQuest.prototype.winFuckAGoblinBroodmotherAsUrta = function() {
		MainView.clearOutput();
		MainView.outputText( 'You greedily eye the ' );
		if( CoC.monster.HP < 1 ) {
			MainView.outputText( 'submissive' );
		} else {
			MainView.outputText( 'feverishly masturbating' );
		}
		MainView.outputText( ' form of your opponent and can\'t resist licking your black lips in anticipation - any guilt about fucking someone besides ' + CoC.player2.short + ' that might make it through the haze of lust consuming you is quickly extinguished by your conviction that ' + CoC.player2.mf( 'he', 'she' ) + ' would understand.  Your growing tower of fuck-meat is already lifting the hem of your armored skirt, making it even easier to grab its lower edge and flip it up out of the way as you advance towards your assailant, eager to see what you can do about blowing off some steam...' );
	...
	UrtaQuest.prototype.loseToGoblinsPartIIAsUrta = function() {
		MainView.clearOutput();
		MainView.outputText( '<b>Hours, and many orgasms later...</b>' );
	...
	UrtaQuest.prototype.loseToGoblinsPartIIIAsUrta = function() {
		MainView.clearOutput();
		MainView.outputText( '<b>Several days later...</b>' );
	...
	UrtaQuest.prototype.loseToGoblinsPartIVAsUrta = function() {
		MainView.clearOutput();
		MainView.outputText( '<b>Several hours later...</b>' );
	...
	UrtaQuest.prototype.urtaGameOver = function() {
		MainView.clearOutput();
		MainView.outputText( '<b>Urta has been lost to her fate...  Meanwhile, back at camp...</b>' );
	...
	UrtaQuest.prototype.fuckSiriusRadioII = function() {
		MainView.clearOutput();
		MainView.outputText( '"<i>Oof... ugh... just... just one more... yeah!</i>"  You gasp, moan, and shudder as your cock explodes yet again in your lover\'s mouth.  The snake-man is sprawled on the ground, supporting a belly like a centauress full-term with twins, and doesn\'t stir as you finally go limp and pull out of his mouth, drooling cum all over his face.' );
	...
	UrtaQuest.prototype.urtaLosesToSirriusSnakeRadio = function() {
		...
		//Loss by HP:;
		if( CoC.player.HP < 1 ) {
			MainView.outputText( 'You cry out in pain as the bruises and buffets you\'ve taken overwhelm you.  You struggle to stay upright, but to no avail; your injured body has reached its limits and you collapse, helpless and at the mercy of your assailant.' );
		}
		//Loss by Lust:;
		else {
			MainView.outputText( 'With a hollow moan of anguish, you cast aside your halberd and fall to your knees, tears stinging your eyes as you desperately jerk and maul at your huge horse-prick.  The cursed thing has betrayed you again, and now you\'re too addled by lust and need to defend yourself from whatever this naga has in store for you.' );
		}
	...
	UrtaQuest.prototype.gnollAlphaBitchIntro = function() {
		MainView.clearOutput();
		//Intro*;
		MainView.outputText( 'The last two fights were wearying, but you still feel pretty good. Perhaps best of all, as your last defeated challenger fades into the distance, you become aware of the pendant glowing fairly brightly.  You must be headed in the right direction!  Setting off at a trot, you pick up your pace, covering great distance in the blink of an eye, eyes never leaving the rapidly approaching horizon.  Focus is key to travel in the uninhabited sections of Mareth, as you well know, but it also allows you to move with swiftness surpassing the fastest animal.' );
	...
	UrtaQuest.prototype.winRapeHyenaPrincess = function() {
		...
		if( CoC.monster.HP < 1 ) {
			MainView.outputText( '.  In spite of her wounded state, she seems to be quite excited by the situation' );
		}
		MainView.outputText( '.' );
	...
	UrtaQuest.prototype.useSuccubiMilkOnGnollPrincesses = function() {
		...
		if( CoC.flags[ kFLAGS.URTA_FUCKED_HEL ] === 1 ) {
			MainView.outputText( 'that salamander slut\'s cunt' );
		} else {
			MainView.outputText( 'a mino-cum addicted whore\'s sodden cunt' );
		}
		MainView.outputText( '.  As you start to pick up the pace, bucking your hips into the thick cushion of her rump, the gnoll princess starts mewling pathetically, panting and moaning like a little slut.' );
	...
	UrtaQuest.prototype.loseToGnollPrincessAndGetGangBanged = function() {
		...
		if( CoC.player.lust > 99 ) {
			MainView.outputText( 'Sinking to your knees, utterly overwhelmed by lust, you gaze up the clit-cocked amazon, licking your chops. She responds by disdainfully pushing you down with one of her soft, padded paws.  "<i>Did you really think you could trespass in my mother\'s territory and go unpunished?</i>"' );
	...
	UrtaQuest.prototype.loseToGnollAsUrtaEpilogue = function() {
		...
		//[HP];
		if( CoC.player.HP < 1 ) {
			MainView.outputText( 'Ouch... what happened to you?  You blink a few times, ignoring the buzzing throb of pain that rises up like a dormant monster in the back of your head.' );
		}
		//[Lust];
		else {
			MainView.outputText( 'Oh, gods... what happened to you?  You blink a few times, ignoring the gentle throb coming from your well-used member.' );
		}
		MainView.outputText( '  There\'s a constant, loud thrum of many voices talking all at once that makes it hard to think, and when you finally come to focus in the dim lighting, you can see why.  There are dozens, if not hundreds, of gnolls here, assembled in the faint light of some kind of feast hall, most of them male.' );
		...
		if( CoC.player.HP < 1 ) {
			MainView.outputText( 'I hope I wasn\'t too rough on you' );
		} else {
			MainView.outputText( 'I hope you enjoyed the journey here' );
		}
		...
		MainView.outputText( '\n\nYou nod your head.' );
		MainView.outputText( '"<i>Good girl,</i>" she says encouragingly.  ' );
		if( CoC.player.HP >= 1 ) {
			MainView.outputText( 'You shudder at the memory of your meeting, leaking a little more lubricant.  ' );
		}
	...
	UrtaQuest.prototype.loseToGnollAsUrtaEpilogueBAM = function() {
		MainView.clearOutput();
		MainView.outputText( 'Ouch!  Something smacks your face, and it fucking hurts!  You look up, seeing a strong, spotted paw pulling away, trailing a web of spooge in its wake.' );
	...
	UrtaQuest.prototype.urtaSleepsArmored = function() {
		MainView.clearOutput();
		CoC.time.days++;
		MainView.outputText( 'You go to sleep wearing your armor.  It turns out to have been a good idea when you\'re woken by tentacles probing at your chest-plate, seeking to undo the numerous straps holding it in place.  You roll to the side and easily squeeze out of the slippery tendrils.  Your hand falls on your halberd with practiced ease, hefting its reassuring weight in your hands as you spring up off your paws.' );
	...
	UrtaQuest.prototype.urtaFinishesWithTentacles = function() {
		MainView.clearOutput();
		MainView.outputText( 'Sighing, you pinch your nipples as your ride the dozens of tentacles, letting them completely envelop your body.  The lips that have teased your cock all this time sluggishly shift position, pressing back against your tip and slowly opening.  With sloth-like slowness, those soft lips encircle the fat, flat head of your horse-dick.  As the tight seal drifts down your length, you get your first taste of the mouth\'s interior.  The whole of the tentacle seems hollow and lubricated with some internal moisture.  Most interestingly of all, the walls are covered with wiggly little cilia, thousands of them.  Your over-aroused member is caressed and tickled from sheath to tip, buttoned up in the moist confines of the tentacle while it\'s plump and juicy lips are smacking against your balls.' );
	...
	UrtaQuest.prototype.submissiveMinoUrtaBadEndPtII = function() {
		MainView.clearOutput();
		MainView.outputText( 'You wake up feeling so blissfully bloated and utterly full.  Memories of your encounter with the minotaur come streaming back.  You submitted to the beast, and it felt... it felt goood.  Your cunny clenches at the memory, and your rigid, oozing pole pulses needily below.  You burp, tasting the salty leavings in the back of your throat for the first time.  That was good!  Gradually, you become aware of a gentle forward and back motion, making your distended gut slosh and your tits sway pendulously.' );
	...
	UrtaQuest.prototype.beatMinoLordOnToSuccubi = function() {
		MainView.clearOutput();
		MainView.outputText( 'The succubus claps slowly as she gets up from her perch on a ruined column.  "<i>I\'m impressed.</i>"  She says, stepping in the light as she walks towards you.  You gasp, this succubus looks like a cow!' );
	...
	UrtaQuest.prototype.milkyUrtaTic = function() {
		MainView.outputText( 'Your body heats up with the effects of the succubus-cow milk...  ' );
		if( CoC.player.lust < 50 ) {
			MainView.outputText( 'Thankfully it hasn\'t become too distracting yet.' );
		} else if( CoC.player.lust < 75 ) {
			MainView.outputText( 'You don\'t know how much longer you\'ll be able to withstand the effects of the milk.' );
		} else {
			MainView.outputText( 'Those orbs full of milky goodness are starting to look more attractive by the minute... if you don\'t finish this quickly you might end up giving up!' );
		}
		MainView.outputText( '\n\n' );
	...
	UrtaQuest.prototype.urtaBeatsUpCowcubi = function() {
		...
		//HP:;
		if( CoC.monster.HP < 1 ) {
			MainView.outputText( 'The succubus trips and falls on her face, dropping her whip at your feet.  You quickly take her whip and use it to bind her hands behind her back.' );
		...
		//Lust:;
			...
			MainView.outputText( 'The succubus drops her whip and begins masturbating furiously, jilling her dripping cunt with abandon.  You quickly run towards her, planting your foot on her back and sending her tumbling on her face, then you use her whip to bind her hands behind her back.' );
	...
	UrtaQuest.prototype.fuckTheCowCubi = function() {
		MainView.clearOutput();
		MainView.outputText( '"<i>Come on girl, strip down so I can see what you\'re hiding under that leather skirt of yours.  Besides, we both know you want me, I can see that tent,</i>" she laughs, licking her lips seductively.' );
	...
	UrtaQuest.prototype.urtaLosesToCowCubi = function() {
		...
		if( CoC.player.HP < CoC.player.maxHP() ) {
			MainView.outputText( ' and healing your wounds' );
		}
		MainView.outputText( '.  Your own nipples have long since become fully engorged, taut black tit-caps, pebbly and sensitive.  They drag up and down the fat bottoms of the demoness\'s swollen underbust with every swallow and gasping breath you take.' );
	...
	UrtaQuest.prototype.urtaArrivesAtCampForFukks = function() {
		MainView.clearOutput();
		MainView.outputText( '<b><u>Meanwhile, back at camp...</u></b>' );
		...
		if( CoC.player.gender === 0 ) {
			MainView.outputText( 'seeing as how you can\'t make babies' );
		} else {
			MainView.outputText( 'if you\'d rather not' );
		}
		MainView.outputText( ', I can always go and use Edryn...</i>"' );
		...
		if( CoC.player.gender > 0 ) {
			MainView.outputText( ' fuck the horny fox and help her out of her predicament, or' );
		}
		MainView.outputText( ' send her to Tel\'Adre to find a suitable partner, like Edryn?' );
		/*if(CoC.player.hasCock()) {
	...
	UrtaQuest.prototype.knockUpUrtaWithGodChild = function() {
		...
		MainView.outputText( '.  The nub of her clit is a diamond-hard button that drags across your sensitive ' + CoC.player.skin() + ', aggressively smashed into your genitals.  Still, somehow, she seems to be holding back, probing you with some last vestige of sanity for your consent.' );
		...
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( ', the mess made worse by your multiple-cocked virility.  There\'s so much that the clear juices roll off you to either side, puddling in your blankets below' );
		}
		MainView.outputText( '.  Her nipples dig into yours' );
		if( CoC.player.hasFuckableNipples() ) {
			MainView.outputText( ', kissing into the ' );
			if( CoC.player.lactationQ() > 50 ) {
				MainView.outputText( 'milky, ' );
			}
			MainView.outputText( 'wet holes on your tits, drawing fresh gasps of delight from your overwhelmed brain' );
		} else {
			MainView.outputText( ', shooting tingling excitement through your nubs as you grind against each other' );
		}
		MainView.outputText( '.  Urta\'s artless humping would be cringeworthy if she weren\'t so incredibly wet or overly tight.' );
		...
		MainView.outputText( ' tints off-white from your liquid, squirting out of her tunnel faster than ever when she climaxes with you' );
		if( CoC.player.cumQ() > 1000 ) {
			MainView.outputText( ', belly rounded with wobbly, womb-packing spunk' );
		}
		MainView.outputText( '.' );
	...
	UrtaQuest.prototype.postFuckUrtaUp = function() {
		...
		if( SceneLib.camp.followersCount() >= 2 ) {
			MainView.outputText( 'leave instructions to your fellow camp-dwellers to look after the place, ' );
		}
	...
	UrtaQuest.prototype.preggedUrtaWithGodChildEpilogue = function() {
		...
		if( CoC.player.cor > 66 ) {
			MainView.outputText( ', what do you care, anyway?' );
		} else {
			MainView.outputText( '.' );
		}
	...
	UrtaQuest.prototype.getKnockedUpWithGodChild = function() {
		MainView.clearOutput();
		MainView.outputText( 'Smiling coyly, you remove your [armor] and crawl into your bed, beckoning for the lust-crazed fox to follow.  You trust her, and you\'re more than willing to let her put a nice little bun in your' );
		if( CoC.player.isPregnant() && CoC.player.pregnancyIncubation < 150 ) {
			MainView.outputText( ' already filled' );
		}
		MainView.outputText( ' oven.  Her green eyes bulge obscenely at the sight of your naked flesh, locked tight to every curve of your shape.  The vixen takes one shuffling step after another.  It\'s clear from her crazed panting and trembling fingertips that she\'s trying to keep herself from jumping you with limited success.  She\'s like an addict eyeing a cornucopia stuffed with her favorite drugs, knowing that she must take it slow but helpless against her desire to gluttonously gorge.' );
		MainView.outputText( '\n\nYou toss a little fuel on the fire by spreading your legs' );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( ' and lifting your balls' );
		}
		MainView.outputText( ', exposing your puffy lips, and spreading them wide to expose your ' );
		if( CoC.player.vaginaType() !== 5 ) {
			MainView.outputText( 'pink, ' );
		} else {
			MainView.outputText( 'dusky, ' );
		}
		MainView.outputText( 'glistening interior.  Urta whines, high pitched and keening, and she doubles over, pumping her cock with both hands, unashamedly masturbating at the sight of your exposed, vulnerable womanhood.  She grunts, swinging her hips lewdly back and forth, actually fucking her hands now, mouth open and panting, "<i>Gonna... gonna fuck ya!  Fullofbabies!</i>"' );
		...
		MainView.outputText( 'squish, she lands nose-deep in your vagina, immediately licking at it and tickling you with her smooth, slippery tongue.  "<i>Mmmmm,</i>" she purrs as she suckles your [clit].  Inching upward, the dripping wet fox-girl licks higher and higher, her tongue smearing across the ' + CoC.player.skinFurScales() + ' that covers your ' );
		if( CoC.player.pregnancyIncubation > 0 && CoC.player.pregnancyIncubation < 150 ) {
			MainView.outputText( 'rounded ' );
		}
		MainView.outputText( 'belly.  Urta\'s rigid onyx nipples tickle your [legs] as they drag upward, each immensely erect and engorged, just a little pebbly in texture.  She reaches your [chest], kissing up the ' );
		if( CoC.player.biggestTitSize() > 1 ) {
			MainView.outputText( 'soft curve' );
		} else {
			MainView.outputText( 'flat muscles' );
		}
		MainView.outputText( ' until she hits your [nipple].  ' );
		if( CoC.player.hasFuckableNipples() ) {
			MainView.outputText( 'Her tongue plunges inside, savoring the feminine lubricants' );
		} else {
			MainView.outputText( 'Her tongue plays across the surface, the heat of her breath shooting tingles up your spine' );
		}
		MainView.outputText( '.  The further she climbs, the further up your [legs] you can feel her stallion-cock rising, a trail of clear pre-cum in its wake.' );
		...
		if( CoC.player.vaginalCapacity() < 60 ) {
			MainView.outputText( 'a little painfully' );
		} else {
			MainView.outputText( 'deliciously' );
		}
		MainView.outputText( '.  Her hands roam over the rest of your body with unrestrained ardor, groping and squeezing, pulling and tweaking.   Meanwhile, her hips smack against your own, smushing her sheath up against your puffy lips, her steamy sex dripping girl-lube down the back of her balls into a thick puddle on your blanket.  SLAP!  The sweaty nutsack slaps loud enough to be heard all over as Urta crudely fucks you, humping you artlessly, her hips beating out a confused, staccato rhythm.' );
		...
		if( CoC.player.hasCock() ) {
			MainView.outputText( ', your own forgotten length' );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ' laying on your stomach, bouncing fitfully but ultimately ignored.' );
		} else {
			MainView.outputText( ', the slippery wetness of your [vagina] and the sheer hardness of your [clit] overwhelming.' );
		}
		MainView.outputText( '  This feels better than it really ought to, perhaps in part thanks to your partner\'s copious fluids and supernaturally warm, feverish dick.' );
		...
		MainView.outputText( ' balls, slathering them with slick spunk.' );
		var ballWorship = [];
		...
		//{Bimbo Sophie ball worship here};
			...
			if( CoC.player.balls > 0 ) {
				MainView.outputText( '  She does the same to your own [balls], even giving your [asshole] a happy, sloppy kiss before returning to polishing the supple, skin-shrouded spheres just ahead.' );
			}
			MainView.outputText( '  Helia seems thrilled to be egging you both on to new heights of pleasure, and when Urta turns back to you with a blissful smile on her face, you realize she\'s enjoying it too.' );
		}
		//{Latexy};
		else if( choice === 4 ) {
		...
		if( CoC.player.tailType > AppearanceDefs.TAIL_TYPE_NONE ) {
			MainView.outputText( ', though getting your tail angled right to allow her entrance is a bit of a challenge' );
		} else if( CoC.player.isTaur() ) {
			MainView.outputText( ', though getting down on your hands and forelegs is a bit challenging for you' );
		} else {
			MainView.outputText( ', [face] buried in your blankets soaked with Urta\'s musky fluid' );
		}
		MainView.outputText( '.' );
		...
		if( CoC.player.wetness() >= 5 ) {
			MainView.outputText( 'Lady-spunk showers out of your spasming twat, glazing Urta\'s bouncy ballsack with your lusty liquid.' );
		} else if( CoC.player.wetness() >= 3 ) {
			MainView.outputText( 'Lady-spunk drools out of your spasming twat in thick streams, glazing Urta\'s bouncy ballsack with your liquid lust.' );
		} else if( CoC.player.wetness() >= 1 ) {
			MainView.outputText( 'Lady-spunk drips from your spasming twat in steady trickles, glazing Urta\'s bouncy ballsack with your liquid lust.' );
		}
		MainView.outputText( '  The hungry mouth between her legs wastes no time in cleaning them.' );
		MainView.outputText( '  Again and again, muscular contractions wiggle through your canal, wringing Urta\'s swollen staff of impregnation, milking it for more, even though your womb is STILL slightly distended from her last deposit.' );
	...
	UrtaQuest.prototype.getKnockedUpUrtaEpilogue = function() {
		MainView.clearOutput();
		MainView.outputText( 'When you wake, you\'re ' );
		if( CoC.player.pregnancyIncubation > 0 && CoC.player.pregnancyIncubation < 150 ) {
			MainView.outputText( 'much more ' );
		}
		MainView.outputText( 'pregnant ' );
		if( CoC.player.pregnancyIncubation > 0 && CoC.player.pregnancyIncubation < 150 ) {
			MainView.outputText( 'than before' );
		} else {
			MainView.outputText( '- very pregnant' );
		}
		MainView.outputText( '.  Whatever Urta just put inside you, it\'s developing fast.  Fast enough to give you a wobbling gait and a straining midriff.  You give the snoring fox a prod.  Her only reaction is to swat at your finger with her fluffy tail and roll over snoring, so this time, you jab her hard, in the butt-cheek.  She yelps, sitting upright immediately.' );
	...
	UrtaQuest.prototype.getKnockedUpByUrtaEpilogueII = function() {
		...
		if( CoC.player.pregnancyIncubation > 0 && CoC.player.pregnancyIncubation < 150 ) {
			MainView.outputText( '  Somehow, you\'re still just as pregnant with something as you were before.' );
		}
		MainView.outputText( '  You\'re filled with vigor at the touch, the stress of the birth fading immediately.  Even your poor, tortured vagina tingles, the pain fading.  You can feel the pregnant gape receding, as if the birth never happened.' );
	...
	UrtaQuest.prototype.urtaAndEdrynGodChildEpilogueII = function() {
		MainView.clearOutput();
		MainView.outputText( 'As the double doors open, you hear a painful moan from the ' );
		//{pre-pregnant };
		MainView.outputText( 'horse-girl.' );
		if( SceneLib.edryn.pregnancy.isPregnant ) {
			MainView.outputText( '  Just how did she get pregnant again when she was already knocked up?  0...Magic, maybe.' );
		}
		MainView.outputText( '  Urta leads you around a bend, squeezing your hand for comfort.  "<i>Thank you for everything, ' + CoC.player.short + ', most of all being so understanding.</i>"  The gray-furred fox pushes past the sixth pair of guards you\'ve seen since entering the tower, just inside to see Edryn splayed on the floor, groaning and heaving, her ' );
		if( SceneLib.edryn.pregnancy.isPregnant ) {
			MainView.outputText( 'massively ' );
		}
		MainView.outputText( 'pregnant flanks rippling with muscular contractions.   Something is bulging against her nether-lips, stretching the massive horse-cunt wider and wider with each passing moment.' );
		...
		if( SceneLib.edryn.pregnancy.isPregnant ) {
			MainView.outputText( '  The centaur guardswoman is still just as pregnant with your offspring as before.  Your child is unharmed by whatever just transpired.' );
		}
		MainView.outputText( '  Edryn\'s eyes immediately close, sinking into a deep, restful sleep.' );
