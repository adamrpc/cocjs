...
angular.module( 'cocjs' ).run( function( MainView, SceneLib, AppearanceDefs, ConsumableLib, CockTypesEnum, OnLoadVariables, Utils, Descriptors, kFLAGS, CoC, EngineCore ) {
	...
	XmasMisc.prototype.candyCaneTrapDiscovery = function() {
		...
		MainView.outputText( 'Shortly after embarking into the plains, your thoughts drift back to your life and home in your village.  The slight chill in the air today reminds you of a time of cheer celebrated by your former friends and family.  As days grew shorter and colder, people exchanged humble trinkets, and great feasts were prepared.  ' );
		if( CoC.player.cor < 50 ) {
			MainView.outputText( 'The memory brings a nostalgic smile to your [face].' );
		} else {
			MainView.outputText( 'The memory reminds you of how bland your world truly was in comparison to Mareth.' );
		}
	...
	//LEAVE;
	XmasMisc.prototype.leaveBeforeMeetingCandyCaneBun = function() {
		MainView.clearOutput();
		MainView.outputText( 'You\'re not willing to run the risk of getting a foreign cock stuffed up your [butt].' );
		//(If corruption or libido>50) ;
		if( CoC.player.cor > 50 || CoC.player.lib > 50 ) {
			MainView.outputText( '  At least not right now.  You turn back, navigating your way back to your camp.' );
		}
	...
	XmasMisc.prototype.investigateCandyCaneBun = function() {
		MainView.clearOutput();
		MainView.outputText( 'You make the decision to find out just what\'s going on.  You cautiously follow the small pools of goo, slightly on edge.  You approach a wall of low hanging vine which you quickly pull aside.  Sitting in the largest pudddle of semen you\'ve seen yet, a trembling bunny-like figure kneels on its knees.' );
		//(If corruption and libido<50) ;
		if( CoC.player.cor < 50 && CoC.player.lib < 50 ) {
			MainView.outputText( '  Your eyes are quickly drawn to the rabbit morph\'s stark nudity.' );
		}
		//(If corruption or libido>50) ;
		else {
			MainView.outputText( '  Your eyes are quickly drawn to the rabbit morph\'s finely curved ass.' );
		}
		MainView.outputText( '  A pair of heavily hanging balls confirm the figure to be at least partially male.' );
	...
	XmasMisc.prototype.helpWithTheCandyCane = function() {
		...
		//(If corruption<50) ;
		if( CoC.player.cor < 50 ) {
			MainView.outputText( '  Not wanting to disturb him, you make your way back to camp and attempt to ignore the burning fire in your loins.' );
		}
		//(If corruption>50) ;
		else {
			MainView.outputText( '  Despite your rising urge to take advantage of the unknowing bunny, you decide to retire and return to camp.' );
		}
	...
	XmasMisc.prototype.leaveXmasChicken = function() {
		MainView.clearOutput();
		MainView.outputText( 'Too exasperated by the plain absurdity of the situation to deal with it, you bury your face in your hands and decide to leave her to it.  Maybe some imps will shut her up for you.  You head back to camp with her piercing cries following you all the way, and prepare for the day ahead.  Her cries fade a while later; the snow thaws and the air warms soon after.' );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	//[Talk];
	XmasMisc.prototype.talkToXmasChicken = function() {
		MainView.clearOutput();
		MainView.outputText( 'Figuring you should at least try asking her to pipe down, you continue your approach.  She finally takes notice of your footsteps crunching the snow beside her and goes still and silent for a moment as she looks up at you.  Now that you\'re closer, you can make out some more of her features against the snowy ground.  She wears a light dress, almost as pale as her skin and the snow around her, thin enough that you can just see her perky nipples through its white fabric.  A shiver of your own makes you wonder why she\'s wearing something like that on a day like this.  The thick, fluffy-looking feathers of her wings even manage to stand out as somehow even whiter than the frosty backdrop of the snow.  A faint, silvery glow seems to radiate from her, but it\'s probably just a trick of the light against the snowfall.' );
		...
		//(if male);
		if( CoC.player.gender === 1 ) {
			MainView.outputText( 'How\'s a blowjob sound to you?' );
		}
		//(if female);
		else if( CoC.player.gender === 2 ) {
			MainView.outputText( 'How about I eat you out?' );
		}
		//(if herm);
		else {
			MainView.outputText( 'How about I give you a few minutes to do whatever you want with my mouth?' );
		}
		MainView.outputText( '</i>"  You raise an eyebrow, not entirely sure what to make of her suddenly lascivious nature.  It\'s certainly quite a change.  She giggles.  "<i>Oh, don\'t be surprised.  Where do you think we are' );
		//(if sillymode);
		if( EngineCore.silly() ) {
	...
	XmasMisc.prototype.acceptChristmasManBJ = function() {
		...
		if( CoC.player.cockArea( x ) >= 80 ) {
			MainView.outputText( 'You free your [cock ' + (x + 1) + '] from your [armor], signaling your acceptance.  Your monster flops free, and the harpy grabs at her throat reflexively when she looks at it.  She glances up to you with wide eyes and back down at it.  "<i>A-a promise is a promise, but...</i>"' );
		...
		//(else if cock would please Goldilocks) (area<= 30 or some shit i dunno whatever the standard is);
			...
			MainView.outputText( 'You free your [cock ' + (x + 1) + '] from your [armor], signaling your acceptance.  The immediate rush of cold air that greets it makes you hope she won\'t be wasting any time.  The pale harpy woman squeals at the sight, raising her hands to her chin and practically bouncing in place.  In a flash she\'s on her knees, stroking and licking at your meat until it\'s hard in front of her face.  Her hot breath against your shaft has you more than ready for the sanctuary from the biting cold.  She eyes your erect prick' );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ' almost hungrily, licking her lips before swiftly engulfing almost its entire length in her mouth.  Her expert technique astounds you; you certainly didn\'t expect her to have any idea what she was doing but she\'s able to take you nearly to the base of your shaft in one motion.' );
			...
			//(if only one cock);
			if( CoC.player.cockTotal() === 1 ) {
				MainView.outputText( 'She grabs onto your hips, enabling herself to move all the quicker.' );
			}//(if two cocks);
			else if( CoC.player.cockTotal() === 2 ) {
				MainView.outputText( 'With her other hands she starts to work the shaft of the second cock, pumping in time with the motions of her head.' );
			}//(if 3+ cocks);
			else {
				MainView.outputText( 'She grabs onto two of your other erect cocks with her hands and begins pumping away at them in time with the motions of her head.' );
			}
			...
			if( CoC.player.cumQ() < 250 ) {
				MainView.outputText( '  She closes her eyes and looks quite contented while she swallows your spunk.' );
			}//(else if cum output is high);
			else if( CoC.player.cumQ() < 1000 ) {
				MainView.outputText( '  Semen gushes out of you for what seems like forever, but the harpy doesn\'t seem to mind.  She closes her eyes and looks quite contented while she swallows your still-torrenting spunk.' );
			}//(else if cum output is ridonkulous);
			else {
				MainView.outputText( '  Though at first she looks like she\'ll be able to handle it, the river of semen exploding from the rip of your cock doesn\'t stop.  Her eyes widen as the almost unending torrent of spunk fills her, rushing down her throat and into her belly.  You can almost see her bloating with the stuff before it starts to spill back out, dripping white from her lips around your shaft.' );
			}
			//(if multicock);
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( '  Your cum sprays over her, coating her skin and plumage in a white to rival their own.  Strands of your semen dripping and running over her hardly even stand out on her bright body.' );
			}
			MainView.outputText( '  When you finish, you let the harpy go.  She looks just as satisfied as you are.  A trail of cum and saliva still connects your [cockHead ' + (x + 1) + '] to her; she breaks it with a lick of her lips and gives your softening member a longing glance before standing up, panting from exertion.' );
	...
	XmasMisc.prototype.approachKamiTheChristmasRoo = function() {
		...
		if( CoC.flags[ kFLAGS.KAMI_ENCOUNTER ] === 1 ) {
			...
			if( EngineCore.silly() ) {
				MainView.outputText( ' and found space to scrawl a little penis in one corner. Clever' );
			}
			MainView.outputText( '.' );
	...
	XmasMisc.prototype.skipOutAfterOpeningKamisBox = function() {
		MainView.clearOutput();
		MainView.outputText( 'You politely decline Kami\'s sexual advances, apologizing before making a turn for the door. You begin to wonder why you didn\'t buy anything at the bakery.' );
	...
	XmasMisc.prototype.chooseLetsGoKami = function() {
		MainView.clearOutput();
		MainView.outputText( 'You decide to give Kami exactly what she wants - \'tis the season for giving, after all!  You grin approvingly before heading out into the back alley, taking your time to stare at her wobbling ass.' );
		...
		EngineCore.addButton( 2, 'Give BJ', this, this.KamiBlowJob );
		if( CoC.player.cockTotal() > 1 && CoC.player.cockThatFits( 70 ) >= 0 && CoC.player.cockThatFits2( 70 ) >= 0 ) {
			EngineCore.addButton( 3, 'DoubleFuck', this, this.KamiDoubleDickFuck );
		}
	};
	...
	XmasMisc.prototype.giveKamiTheChristmasRooAnal = function() {
		...
		MainView.outputText( 'You head briskly towards the voluptuous waitress, rearing her around by the wrists and pressing her against the wall.  "<i>H-Hey, wait!</i>" Kami yelps, but you sense excitement in her delicate plea.' );
		...
		if( CoC.player.cumQ() < 1000 ) {
			MainView.outputText( '  "<i>Mmm... just like that, babe,</i>" Kami moans submissively, her asshole clenching one last time before you pull out and climb off your satiated conquest, your ' + Descriptors.cockDescript( x ) + ' still dripping creamy white spunk.' );
		}//{if cum quantity ≧ heavy};
		else {
			MainView.outputText( '  One of Kami\'s hands flies from her dick to her suddenly swollen belly as your mighty seed gushes forth.  The unexpectedly heavy girl sinks further into the ground, your load swishing back up and loosening her tunnel around you and forcing you to remove your still-spewing ' + Descriptors.cockDescript( x ) + ' before something unfortunate happens.  Semen starts tumbling out as soon as you dismount your kangaroo conquest, but judging by the slow pace, she\'s going to look heavily pregnant for a while.' );
		}
	...
	XmasMisc.prototype.takeItRooButtStyle = function() {
		...
		//[Else if have vagina];
			...
			MainView.outputText( 'your taint' );
		}
		MainView.outputText( '.  Each time, the waitress fondles your form with her eager hands.  ' );
		MainView.outputText( '\n\n' );
		//[If player has pussy] ;
		if( CoC.player.hasVagina() ) {
			MainView.outputText( 'Your cunt oozes in return, envious of the tenderized hole above it.  ' );
		}
		MainView.outputText( 'You dedicate a few more minutes to soaking up every moment of her animalistic romping, savoring the feeling of a hung rod in your punished posterior.  Your climax quickly comes to loom over you, however, the potent, ' );
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'prostate' );
		} else {
			MainView.outputText( 'ass' );
		}
		MainView.outputText( '-numbing sensations at your rear becoming too much to bear.  Slamming back into Kami\'s pole one last time, you breathe a shuddered sigh of relief as your orgasm takes over, the sensation spreading throughout your entire body like electricity.' );
		//[If male];
		if( CoC.player.hasCock() ) {
			MainView.outputText( '  Your pent-up cock ejaculates there and then, shamelessly gushing sloppy semen against the wall in front of you.' );
		}
		//[If female];
		if( CoC.player.gender === 3 ) {
			MainView.outputText( '  Meanwhile, y' );
		} else if( CoC.player.hasVagina() ) {
			MainView.outputText( 'Y' );
		}
		if( CoC.player.hasVagina() ) {
			MainView.outputText( 'our eager pussy buckles in arousal, gushing fem-juices on the floor below you.' );
		}
		MainView.outputText( '  You\'re truly spent.' );
	...
	XmasMisc.prototype.KamiBlowJob = function() {
		...
		if( CoC.player.hasCock() ) {
			MainView.outputText( '  You think you might still need to find an outlet for that itch of yours, with [eachCock] still erect and needing release.' );
		}
	...
	XmasMisc.prototype.KamiDoubleDickFuck = function() {
		...
		MainView.outputText( '\n\n"<i>I could do this all goddamn day!</i>"  Kami screams, your wicked thrashing leaving her pelvis moist with sheer bliss.  You can already feel your own orgasm approaching, so with due haste you grab Kami\'s ample bust and pick up your thrusts energetically, ramming your cocks home with as much speed as you can muster.  Your ass-engorged ' + Descriptors.cockDescript( x ) + ' cums first, packing Kami\'s tight hole with your sticky seed, shortly followed by your other ' + Descriptors.cockDescript( y ) + ' giving her delicate-yet-punished pussy the same treatment.' );
	...
	XmasMisc.prototype.getWinterPudding = function() {
		...
		if( CoC.player.gems < 35 ) {
			MainView.outputText( 'Sadly, the pudding isn\'t free, and it looks like you don\'t have the 35 gems you\'d need to try a bite!' );
			EngineCore.doNext( SceneLib.telAdre, SceneLib.bakeryScene.bakeryuuuuuu );
			return;
		}
		CoC.player.gems -= 35;
		MainView.statsView.show();
		MainView.outputText( 'You opt for a slice of the tasty treat - you\'ve only got until the festivities wade away, so you might as well take one now!' );
	...
	XmasMisc.prototype.polarPete = function() {
		MainView.clearOutput();
	...
	//Unwrap;
	XmasMisc.prototype.unwrapPolarPete = function() {
		MainView.clearOutput();
		MainView.outputText( 'The temptation too strong, you walk closer to Pete.  Sinking down to your knees before him as he adjusts his hips to your level.  Your hands on his crimson pants, pulling them down with bated breath.  His dark ebony length flops free, long and swollen with heat before your nose. Your hand rubs along it\'s smooth black surface coaxing a pleased groan from Pete.' );
		...
		if( CoC.player.hasCock() ) {
			MainView.outputText( '  [EachCock] grows stiff and soaks your [armor] with precum.' );
		}
		//(if vagina) ;
		if( CoC.player.hasVagina() ) {
			MainView.outputText( '  Your [vagina] grows moist, dripping eager juices and soaking your [armor].' );
		}
		...
		//[(if cock) ;
		if( CoC.player.hasCock() ) {
			MainView.outputText( '  His continued ramming of your prostate mixed with the addictive candy crammed in your mouth sends you over the edge, [eachCock] shooting out ropes of snow white all over your chest.  Pete eyes you over with a lurid gaze, your face covered in his cum and your body covered in your own.' );
		} else if( CoC.player.hasVagina() ) {
	...
	XmasMisc.prototype.leaveDisFukkinPolarBear = function() {
		MainView.clearOutput();
		MainView.outputText( 'You\'ve decided not to trust the stranger, with a nod of your head you walk away from the man.  "<i>Wait!  Before you go...</i>" he speaks as he moves his massive sack in front of himself with a huff.  He leans forward, his upper body rummaging through the sack before reappearing.  He hands you a brightly wrapped gift.' );
	...
	XmasMisc.prototype.nieveBuilding = function() {
		...
		if( CoC.flags[ kFLAGS.NIEVE_STAGE ] === 1 ) {
			...
			MainView.outputText( 'You head back over to the snowy patch by your camp.  The crunch of snow again makes you feel just a little nostalgic, and you think of drinking hot cocoa topped with marshmallows.  With a smile on your face, you set about rolling up the soft powder-like crystals into a huge ball.  It takes some effort, but after a while you\'re left with a large ball of snow, which will serve as the perfect base.' );
		...
		//Alters Nieve's eyes in scenes;
			...
			EngineCore.addButton( 1, 'Gems', this, this.nieveGemEyes );
			EngineCore.addButton( 4, 'Back', SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
		}
		//Fourth Step: The Nose;
		else if( CoC.flags[ kFLAGS.NIEVE_STAGE ] === 3 ) {
			MainView.outputText( 'You approach your nearly finished snowman. To others, it might be a perfectly acceptable creation, but not to you. It needs a carrot to finish it off.' );
			...
			//(Else) ;
				...
				MainView.outputText( 'Unfortunately, you\'ve yet to find one in your adventures.  You suppose you\'ll have to look more carefully.  Who knows, there might be a farm right under your nose.' );
			}
			EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
		} else {
			MainView.outputText( 'Your snowman is done!  There\'s nothing more to add to it.  It looks mighty fine however, and just looking at it brings a nostalgia-fueled smile to your lips.' );
			EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
		}
	};
	//Coal;
	...
	XmasMisc.prototype.nieveCoalEyes = function() {
		...
		MainView.outputText( 'Luckily, you happen to have two lumps of coal.' );
	...
	XmasMisc.prototype.nieveGemEyes = function() {
		...
		if( CoC.player.gems >= 9 ) {
			MainView.outputText( 'Taking a handful of gems out of your pouch, you spread them evenly around the Snowman\'s face, giving it a nice, vacant smile.  It still needs a nose, however, and for that, you\'ll need a carrot.  Perhaps there\'s a farm nearby?' );
			CoC.player.gems -= 9;
			MainView.statsView.show();
		...
		//Too broke to use the gems option? What the fuck is wrong with you?;
			...
			MainView.outputText( 'You open up your pouch, and frown.  Unfortunately, you don\'t have enough gems to create the eyes and mouth. With a sigh you march your broke ass back to camp.' );
		}
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	//Snowwoman;
	XmasMisc.prototype.nieveSnowWoman = function() {
		...
		MainView.outputText( 'You grin mischievously to yourself and set about making two more balls of powdery snow.  It takes less time than any of the others, and before you know it you\'ve attached two icy-breasts to the snowman.  They aren\'t terribly big, any heavier and you\'re sure they\'d fall off, but they get the point across.' );
	...
	//Snowman;
	//Add to existing text if possible, rather than a new window?;
	XmasMisc.prototype.nieveSnowMan = function() {
		MainView.clearOutput();
		MainView.outputText( 'You decide to leave it as is. Not everything has to have breasts, of course, even in Mareth.' );
	...
	//Reqs NIEVE_STAGE === 3;
	XmasMisc.prototype.findACarrot = function() {
		MainView.clearOutput();
		MainView.outputText( 'As you explore the farm, you come across several rows of green plants.  Getting closer, you recognize them... carrots!  You pull one of them from the ground.  It\'s expectedly dirty, but bright orange, and straight enough to be the perfect nose for your snowman.' );
	...
	XmasMisc.prototype.nieveComesToLife = function() {
		...
		if( CoC.flags[ kFLAGS.NIEVE_GENDER ] === 1 ) {
			MainView.outputText( '  It appears to be male, judging by the modestly sized cock sprouting from its loins.' );
		} else {
			MainView.outputText( '  It appears to be female, judging by the modestly sized tits on its chest, and the lack of a penis, which is quite a feat in Mareth.' );
		}
		MainView.outputText( '  Pure white hair spills down from its head, almost blending in with the surrounding snow.' );
	...
	XmasMisc.prototype.yesKeepNieve = function() {
		MainView.clearOutput();
		MainView.outputText( 'You actually laugh at the question.  Of course you want ' + this.nieveMF( 'him', 'her' ) + ' to stay!' );
		//[Silly mode: ;
		if( EngineCore.silly() ) {
			MainView.outputText( '  A free follower, no worries about buying worthless dye or raising their affection?  Fuck yeah, you\'ll take ' + this.nieveMF( 'him', 'her' ) + '!' );
		}
		MainView.outputText( '  ' + this.nieveMF( 'He', 'She' ) + ' doesn\'t seem to be a threat, and indeed seems sincere in the fact that ' + this.nieveMF( 'he', 'she' ) + ' was sent here to be your lover.' );
	...
	XmasMisc.prototype.noNoKeepNieve = function() {
		MainView.clearOutput();
		MainView.outputText( 'You shake your head.  Of course not!  While ' + this.nieveMF( 'he', 'she' ) + ' certainly seems like a nice person on the surface, you can\'t help but think that\'s just the tip of the iceberg.  For all you know, ' + this.nieveMF( 'he', 'she' ) + '\'s a frigid bitch underneath, a trap set by the demons to lure you into a false sense of security.  Clever bastards, you conclude.  They certainly know the best way to serve revenge, but you won\'t have any of it.' );
	...
	XmasMisc.prototype.nieveCampDescs = function() {
		//6:00;
		if( CoC.time.hours === 6 ) {
			MainView.outputText( 'Nieve is sitting cross-legged in the snowdrift, munching on what looks to be icicles.  As you watch, ' + this.nieveMF( 'he', 'she' ) + ' reaches down into the glittering powder surrounding ' + this.nieveMF( 'him', 'her' ) + ' and produces another one.' );
		}
		//7:00;
		else if( CoC.time.hours === 7 ) {
			MainView.outputText( 'Nieve is giggling and laughing as a fresh flurry flutters down on ' + this.nieveMF( 'him', 'her' ) + '.  You wonder idly if ' + this.nieveMF( 'he', 'she' ) + '\'s causing it to happen.' );
		}
		//8:00;
		else if( CoC.time.hours === 8 ) {
			MainView.outputText( 'Nieve is sitting in the white wintery wonderland, carefully constructing a snowman.  It strikes you as strange, and almost masturbatory in a way.  ' + this.nieveMF( 'He', 'She' ) + ' spots you and gives you a wave and a smile.  For a moment, you think the snowman has too, but you eventually conclude it\'s your imagination.' );
		}
		//9:00;
		else if( CoC.time.hours === 9 ) {
			MainView.outputText( 'Nieve is sprawled out in the icy field, creating a series of snow angels without a care in the world.  When ' + this.nieveMF( 'he', 'she' ) + ' catches sight of you, she gives a friendly wave, then dives back into the snow.' );
		}
		//10:00;
		else if( CoC.time.hours === 10 ) {
			MainView.outputText( 'Nieve, the ice spirit, sits quietly at the edge of the snowdrift, looking out at the landscape beyond.  When ' + this.nieveMF( 'he', 'she' ) + ' sees you looking, ' + this.nieveMF( 'he', 'she' ) + ' gives a somber wave.' );
		}
		//11:00;
		else if( CoC.time.hours === 11 ) {
			MainView.outputText( 'Nieve is in your camp, poking around your supplies and water barrels. You notice ' + this.nieveMF( 'he', 'she' ) + ' seems a little uncomfortable to be away from ' + this.nieveMF( 'his', 'her' ) + ' snowdrift.' );
		}
		//12:00;
		else if( CoC.time.hours === 12 ) {
			MainView.outputText( 'Nieve is carefully compressing snowballs in ' + this.nieveMF( 'his', 'her' ) + ' little camp.  To your surprise, ' + this.nieveMF( 'he', 'she' ) + ' picks one up and begins eating it like you would an apple.  When ' + this.nieveMF( 'he', 'she' ) + ' catches you looking mid-bite, ' + this.nieveMF( 'he', 'she' ) + ' gives you a grin with her mouth full, ' + this.nieveMF( 'his', 'her' ) + ' cheeks puffed out.' );
		}
		//13:00;
		else if( CoC.time.hours === 13 ) {
			MainView.outputText( 'Nieve is sitting cross-legged, in the middle of what appears to be an argument with a snowman ' + this.nieveMF( 'he', 'she' ) + '\'s just made.' );
		}
		//14:00;
		else if( CoC.time.hours === 14 ) {
			MainView.outputText( 'Nieve is in ' + this.nieveMF( 'his', 'her' ) + ' usual spot, seemingly deep in concentration.  Around ' + this.nieveMF( 'him', 'her' ) + ' snow begins to fall slowly, then quicker and quicker, whipping ' + this.nieveMF( 'his', 'her' ) + ' hair around wildly.  Although strangely, the air is still where you are.' );
		}
		//15:00;
	...
	//Appearance Screen;
	XmasMisc.prototype.approachNieve = function() {
		MainView.clearOutput();
		MainView.outputText( 'You wave at Nieve, getting ' + this.nieveMF( 'his', 'her' ) + ' attention and calling for ' + this.nieveMF( 'him', 'her' ) + '.  The icy spirit-' + this.nieveMF( 'man', 'woman' ) + ' happily walks over, smiling.  ' + this.nieveMF( 'He', 'She' ) + ' takes ' + this.nieveMF( 'his', 'her' ) + ' time, allowing you to take in ' + this.nieveMF( 'his', 'her' ) + ' naked body.' );
		...
		//[Silly Mode:];
		if( EngineCore.silly() ) {
			MainView.outputText( '  And you presume a butthole nestled between ' + this.nieveMF( 'his', 'her' ) + ' cheeks, right where it belongs.' );
		}
		MainView.outputText( '  ' + this.nieveMF( 'His', 'Her' ) + ' stomach is flat and toned, ' + this.nieveMF( 'as is his chest', 'and she possesses a pair of perky B-cup breasts' ) + '. ' + this.nieveMF( 'His', 'Her' ) + ' face is the same pale blue as the rest of ' + this.nieveMF( 'his', 'her' ) + ' body, though it is off-set by ' + this.nieveMF( 'his', 'her' ) + ' glittering, ' );
		if( CoC.flags[ kFLAGS.NIEVE_MOUTH ] === 'coal' ) {
			MainView.outputText( 'dusky black' );
		} else {
			MainView.outputText( 'vibrant purple' );
		}
	...
	XmasMisc.prototype.nieveSexMenu = function() {
		MainView.clearOutput();
		MainView.outputText( 'What will you do with your oh-so-cool lover?' );
		...
		if( CoC.flags[ kFLAGS.NIEVE_GENDER ] === 1 ) {
			EngineCore.addButton( 0, 'Suck Him', this, this.suckNieveOff );
		}
		//Fuck Her;
		//Female Nieve;
		...
		if( (CoC.player.hasCock() || (CoC.player.hasVagina() && CoC.player.clitLength >= 3.5)) && CoC.player.lust >= 33 ) {
			if( CoC.flags[ kFLAGS.NIEVE_GENDER ] === 2 ) {
				EngineCore.addButton( 1, 'Fuck Her', this, this.fuckNieve );
			}
		}
		...
		if( CoC.flags[ kFLAGS.NIEVE_GENDER ] === 1 && CoC.player.lust >= 33 ) {
			EngineCore.addButton( 1, 'Get Fucked', this, this.nieveFucksYou );
		}
		EngineCore.addButton( 9, 'Back', SceneLib.camp, SceneLib.camp.campLoversMenu );
	};
	//Lick Her;
	//Obviously for Female Nieve.;
	XmasMisc.prototype.lickNieve = function() {
		MainView.clearOutput();
		MainView.outputText( 'You explain your intentions and how you\'d like to be the one to give her pleasure.  She looks rather surprised at first, but then she smiles.  "<i>You want to go down... on me?</i>"  You nod and she giggles, "<i>That would be wonderful!</i>"' );
		...
		if( !EngineCore.silly() ) {
			...
			if( CoC.player.tongueType !== AppearanceDefs.TONUGE_DEMONIC ) {
				MainView.outputText( 'It doesn\'t get far, but elicits squeals of surprise and pleasure from your elemental slave who\'s already grasping at the snow surrounding her for support.' );
			}
			//Demon Tongue: ;
			else if( CoC.player.tongueType === AppearanceDefs.TONUGE_DEMONIC ) {
				MainView.outputText( 'Her eyes widen as you snake your impressive, demonic tongue into her. She grunts and squeals with pleasure, hands grasping piles of snow, wishing they were bedsheets.' );
			}
			MainView.outputText( '  After a minute of this, you realize you\'ve lost the feeling in your tongue!  Pulling it back into your mouth, you find it warms back up quickly, and then you\'re at it again.' );
			...
			if( CoC.player.isTaur() ) {
				MainView.outputText( ', or as close to it with your body,' );
			}
			MainView.outputText( ' with Nieve straddling your face.  "<i>Mm, this is much better.  Now you wanted to lick me, [Master]? So... lick,</i>" she whispers, lowering her chilly cunt down onto your face.' );
		...
		//Silly Mode:;
			...
			if( CoC.flags[ kFLAGS.ANEMONE_KID ] !== 1 ) {
				MainView.outputText( ', Nieve takes a ladle full of water and pours it onto her snatch' );
			}
			//Kid A: ;
			else {
				MainView.outputText( ', your little anemone spawn giggles and laughs at your predicament before finally ladling some water onto your lover\'s snatch' );
			}
			MainView.outputText( ', effectively allowing you to pull your tongue away.' );
	...
	XmasMisc.prototype.fuckNieve = function() {
		...
		if( x >= 0 ) {
			if( CoC.player.cocks[ x ].cockLength < 6 ) {
				MainView.outputText( '  "<i>It\'s so cute, [Master],</i>" Nieve says while catching a breath, "<i>Like a little toy cock.</i>"' );
			}
			//If overly large (>130): ;
			else if( CoC.player.cocks[ x ].cockLength >= 24 ) {
				MainView.outputText( '  She has to take a breath almost immediately, saying, "<i>By the fat man\'s beard this thing is huge.  You must make the rest of this world jealous.</i>"' );
			} else {
				MainView.outputText( '  "<i>Mmm,</i>" she smacks her lips and takes a quick breath, "<i>Such a nice example of a candycane right here.  Let\'s go in for another taste.</i>"' );
			}
		} else {
		...
		MainView.outputText( '  And with that she dives back down onto your ' + Descriptors.cockClit( x ) + '.  Her head bobs and bows, giving the sensation of an icecube running up and down your ' );
		if( x >= 0 ) {
			MainView.outputText( 'dick' );
		} else {
			MainView.outputText( 'clit' );
		}
		MainView.outputText( ' at great speed.' );
		//(Variable depending on dick-type);
		if( x >= 0 ) {
			//(Human) ;
			if( CoC.player.cocks[ x ].cockType === CockTypesEnum.HUMAN ) {
				MainView.outputText( '  Your normal, unaltered cock seems to take on a slight blue tint, although you can confirm it\'s not going numb.  Quite the contrary, it seems to be getting more sensitive as Nieve\'s lips roll over it.' );
			}
			//(Equine) ;
			else if( CoC.player.cocks[ x ].cockType === CockTypesEnum.HORSE ) {
				MainView.outputText( '  Nieve fawns over your horsecock like a cultist praying to a god.  She sucks and kisses, running the tip of her tongue all along the sides until she reaches the sheathe, and then right back up the other side.' );
			}
			//(Canine) ;
			else if( CoC.player.hasKnot( x ) ) {
				MainView.outputText( '  Nieve eyes the knot on your cock as though it were a challenge.  She inches down your shaft, getting ever closer.' );
				//(Cocksize >40);
				if( CoC.player.cockArea( x ) >= 40 ) {
					MainView.outputText( '  Eventually, though, she admits defeat and goes back to working your shaft, though she eyes that knot from time to time with no small amount of anger.' );
				}//(Cocksize <40) ;
				else {
					MainView.outputText( '  It takes her a few good minutes, but eventually she works her way down and takes the entire knot into her mouth.  She raises her hands in triumph and lets out a "<i>Mmmmrrrrrpphhh!</i>" before pulling back for a breath, inadvertently exposing your wet hard cock to the cold, wintery air.' );
				}
			}
			/*Cock types
			 0 - human
			...
			//(Demonic);
				...
				MainView.outputText( '  Your purple, demonic shaft pulses and throbs, seemingly immune to the unnatural cold of Nieve\'s mouth. Seemingly oblivious to its nature, Nieve slavers over each nodule, giving every ridge and bump a kiss and more than just a lick.' );
			}
			//(Tentacle) ;
			else if( CoC.player.cocks[ x ].cockType === CockTypesEnum.TENTACLE ) {
				MainView.outputText( '  Nieve does not seem put off by your tentacle cock in the least, in fact she confesses, "<i>It reminds me of a comic I read before coming here.</i>"  As she fawns over your cock, you\'re certain you hear whispers of "<i>Oh, you sexy tentacle beast...</i>" coming from her mouth when she gets her breath.' );
			}
			//(Feline) ;
			else if( CoC.player.cocks[ x ].cockType === CockTypesEnum.CAT ) {
				MainView.outputText( '  Though the barbs on your cock are soft and pliable, they reflexively harden up after Nieve\'s tongue washes over them.  She doesn\'t pay particular attention to the barbs, but she does seem rather careful not to get one hooked on her lip.' );
			}
			//(Lizard);
			else if( CoC.player.cocks[ x ].cockType === CockTypesEnum.LIZARD ) {
				MainView.outputText( '  Nieve fawns over your scaled and ridged reptilian cock, slurping and smooching up and down its length. It seems a bit more resistant to the biting cold of Nieve\'s mouth, which is lucky when she backs off, exposing your entire wet, frigid length to the wintery air surrounding you.' );
			}
			//(Anemone) ;
			else if( CoC.player.cocks[ x ].cockType === CockTypesEnum.ANEMONE ) {
				MainView.outputText( '  Nieve giggles whenever one of your wriggling tentacles stings her, seemingly immune to its effects, and kisses it back.  Soon after she devours your whole length, delighted to feel the sting of your cock in the back of her throat.' );
			}
			//(Kangaroo) ;
			else if( CoC.player.cocks[ x ].cockType === CockTypesEnum.KANGAROO ) {
				MainView.outputText( '  It takes Nieve a little while to catch on that your cock isn\'t quite human, and by the time she figures it out, she mutters, "<i>Huh, that\'s strange... I thought kangaroos had balls above the cock,</i>" before shrugging and getting back to work.' );
			}
			//(If you add dicks later, placeholder text:) ;
			else {
				MainView.outputText( '  Nieve works on your mutant cock, spending time working every unique little facet of it.' );
			}
		}
		//(Clitoris) ;
		else {
			MainView.outputText( '  Nieve nuzzles and lavishes your elongated love button, showering it with attention. "<i>I didn\'t know clits could get this big,</i>" she says with wonder in her voice. "<i>' );
			if( CoC.player.clitLength < 5 ) {
				MainView.outputText( 'It\'s just like a little cock' );
			} else if( CoC.player.clitLength < 9 ) {
				MainView.outputText( 'It\'s just like a cock' );
			} else {
				MainView.outputText( 'I bet even men get jealous of this bad girl' );
			}
			MainView.outputText( '.</i>"' );
		}
		MainView.outputText( '\n\nEventually you just can\'t handle anymore and your body tenses.  Your legs lock up, ' + Descriptors.cockClit( x ) + ' trembling, back arching slightly. Nieve notices this and latches on to the tip just in time as an orgasm rocks your body.' );
		if( x >= 0 ) {
			MainView.outputText( '  Your ' + Descriptors.cockClit( x ) + ' quivers and spurts cum directly into Nieve\'s eager mouth, who gulps it down as if she were drinking direct from the tap.' );
		}
		MainView.outputText( '\n\nAs you come down from your little orgasmic high, you glance down at the ice woman, ' );
		if( x < 0 || CoC.player.cumQ() <= 250 ) {
			MainView.outputText( 'who has a bit of cum dribbling down her chin' );
		} else if( CoC.player.cumQ() <= 400 ) {
			MainView.outputText( 'who has a steady stream of cum pouring from her mouth' );
		} else if( CoC.player.cumQ() < 600 ) {
			MainView.outputText( 'who is patting her stomach as though she\'d just had a large meal' );
		} else if( CoC.player.cumQ() < 1000 ) {
			MainView.outputText( 'whose stomach appears slightly distended from the amount of cum you\'ve poured into her' );
		} else {
			MainView.outputText( 'whose stomach is rather large, like a pregnant woman\'s, from the sheer amount of cum you\'ve pumped into her gullet' );
		}
		MainView.outputText( '.  With a smirk, you tell her it\'s not over yet.  That was just the appetizer.' );
		MainView.outputText( '\n\nWith a command Nieve falls backwards into the white, glittering powder and spreads her legs, revealing her deep blue nethers, wet and slick despite the intense cold of her body.  With a smile spreading across your face, you get down onto your knees between hers, and rub the head of your ' + Descriptors.cockClit( x ) + ' against her icy cavern.  A chill runs down it and up your spine, but you ignore it. You tease and taunt Nieve\'s cunt, flicking her joy buzzer ' );
		if( x >= 0 ) {
			MainView.outputText( 'with your ' + Descriptors.cockDescript( x ) + '' );
		} else {
			MainView.outputText( 'with yours' );
		}
		MainView.outputText( ', and just slipping the tip in before pulling out.  You do this until your snowbound lover is biting her lip, squirming with barely contained pleasure.' );
		...
		if( x >= 0 ) {
			if( CoC.player.cockArea( x ) >= 130 ) {
				MainView.outputText( 'at least as far as it will go, ' );
			}
		}
		MainView.outputText( 'with a wet "<i>schluck.</i>"  The words coming out of her mouth are lost in a loud "<i>ooohhhh</i>" as her eyes roll backwards, closely followed by her head.  Her wet pussy grips you tightly, giving you a strange tingling sensation.  Your hot rod combined with her arctic nethers actually causes some steam to rise up, and you\'re a little captivated to see more tufts of steam rise with every slow, gentle thrust you make.' );
		...
		if( x >= 0 ) {
			...
			if( CoC.player.cockArea( x ) <= 7 ) {
				MainView.outputText( 'Your meager penis isn\'t much, but at this angle you manage to hit all the right places.  Soon Nieve is wriggling around, eyes staring up at you, urging you on faster and faster.' );
			}//(Cocksize 30/Clit 12) ;
			else if( CoC.player.cockArea( x ) <= 30 ) {
				MainView.outputText( 'At this angle you manage to hit all the right places.  Soon Nieve is wriggling around, eyes tightly shut as she whispers dirty words you can\'t even make out.' );
			}
			//(Cocksize 70/Clit 24);
			else if( CoC.player.cockArea( x ) <= 70 ) {
				MainView.outputText( 'Nieve\'s cunt feels like an icy vice on your oversized cock, but even so, you manage to hit all the right places.  Soon she is wriggling around, eyes tightly shut as she pants for breath.' );
			}//(Cocksize 130+/Clit 48+);
			else {
				MainView.outputText( 'Though you can\'t fit your entire ' + Descriptors.cockClit( x ) + ' inside, you are amazed at how much she takes.  Her pale blue stomach bulges with every thrust.  Soon Nieve is wriggling, her eyes slightly open as she bites her lip in ecstasy.' );
			}
		} else {
			if( CoC.player.clitLength <= 4 ) {
				MainView.outputText( 'Your meager clit isn\'t much, but at this angle you manage to hit all the right places.  Soon Nieve is wriggling around, eyes staring up at you, urging you on faster and faster.' );
			} else if( CoC.player.clitLength <= 12 ) {
				MainView.outputText( 'At this angle you manage to hit all the right places.  Soon Nieve is wriggling around, eyes tightly shut as she whispers dirty words you can\'t even make out.' );
			} else if( CoC.player.cockArea( x ) <= 24 ) {
				MainView.outputText( 'Nieve\'s cunt feels like an icy vice on your oversized clitty, but even so, you manage to hit all the right places.  Soon she is wriggling around, eyes tightly shut as she pants for breath.' );
			}//(Cocksize 130+/Clit 48+);
			else {
				MainView.outputText( 'Though you can\'t fit your entire ' + Descriptors.cockClit( x ) + ' inside, you are amazed at how much she takes.  Her pale blue stomach bulges with every thrust.  Soon Nieve is wriggling, her eyes slightly open as she bites her lip in ecstasy.' );
			}
		...
		//((Clitfucking?);
		if( x < 0 ) {
			MainView.outputText( '  Your [clit] suddenly feels suddenly warm, like every nerve ending inside were on fire. It aches and pulses with the rest of your body, as though it were a little cock longing to cum.' );
		} else {
			MainView.outputText( '  Your ' + Descriptors.cockClit( x ) + ' aches and pulses with need before ' );
			if( CoC.player.cumQ() < 100 ) {
				MainView.outputText( 'spilling its meager offering into the ice spirit\'s womb' );
			} else if( CoC.player.cumQ() < 250 ) {
				MainView.outputText( 'ejecting streams of white hot jizz into the ice spirit\'s hungry snatch' );
			} else if( CoC.player.cumQ() < 400 ) {
				MainView.outputText( 'painting the ice spirit\'s insides with your cream until some begins to pour out' );
			} else if( CoC.player.cumQ() < 700 ) {
				MainView.outputText( 'arcs of pearly white cum shoot forth, filling the ice spirit\'s womb so much her belly visibly inflates' );
			} else {
				MainView.outputText( 'erupting inside, filling the ice spirit up with so much of your hot cum so much that she looks visibly pregnant' );
			}
			MainView.outputText( '.' );
		}
	...
	//Any Gender;
	//Nieve's cock is 9x1.5;
	XmasMisc.prototype.nieveFucksYou = function() {
		MainView.clearOutput();
		MainView.outputText( 'Your arrival to Nieve\'s part of the camp elicits a friendly smile from the ice man.  He notices the longing in your eyes with a grin, and speaks in a gentle, yet low tone, "<i>[Master], you seem to need help from me...</i>"' );
		MainView.outputText( '\n\nTo which you nod, ' );
		if( CoC.player.cor < 50 ) {
			MainView.outputText( 'telling him that you find his form... appeasing to say the least, and would like to know how his \'parts\' are working properly.' );
		} else {
			MainView.outputText( 'leting your creation know that you are interested in \'how\' it can please you, giving him no room to imagine what you\'re insinuating you then tell him bluntly that you want him to fuck you like the plaything he is.' );
		}
		...
		//{Silly Mode:};
		if( EngineCore.silly() ) {
			MainView.outputText( '   He did say that his kind were effectively love slaves, and boy does this prove it!' );
		}
		//[if (hasVagina = true) ;
		if( CoC.player.hasVagina() ) {
			MainView.outputText( '  Your [vag] is a sodden box so soon after he started, and you start to want more, much more!' );
		}
		//[if (hasCock = true) ;
		if( CoC.player.hasCock() ) {
			MainView.outputText( '  Your [cock biggest] is also rock hard in a short while and you quickly desire some more stimulation from your icy love slave.' );
		}
	...
	XmasMisc.prototype.takeNieveAnal = function() {
		...
		//[if (hasCock = true) ;
		if( CoC.player.hasCock() ) {
			MainView.outputText( '  Nieve then bends forward while keeping his pace to give you a reach-around.  Grabbing your own [cock biggest] in one hand, he starts to pump it to the speed of his thrusts, torturing you with the extra-stimulation in such a good way.' );
		}//[if (hasVagina = true);
		else if( CoC.player.hasVagina() ) {
		...
		if( CoC.player.gender > 0 ) {
			...
			if( CoC.player.hasCock() ) {
				MainView.outputText( ', ' );
				MainView.outputText( 'while your body undulates and seizes as cum starts to spew forth from [eachCock]' );
				//(normal volume: ;
				if( CoC.player.cumQ() < 200 ) {
					MainView.outputText( ', as you cum, you leave a small puddle on the floor below you' );
				} else if( CoC.player.cumQ() < 500 ) {
					MainView.outputText( ', as you cum you start to dress your arms and hands with your own cream as you shoot your stuff out onto the floor' );
				}
			}
			//[if (hasVagina = true);
			if( CoC.player.hasVagina() ) {
				MainView.outputText( '.  Your soaking vagina makes your body quake and shudder as you orgasm, splashing your femcum all over Nieve and the ground' );
			}
		}
		MainView.outputText( '.' );
	...
	XmasMisc.prototype.takeNieveVaginal = function() {
		MainView.clearOutput();
		MainView.outputText( 'You\'re already wet from the foreplay, and it shows.  Moreso, Nieve notices, with a grin similar to the one he gave you earlier.  The look on his face suggests he knows what you want, he\'s just merely waiting for his [master] to give the word.' );
		//if (corruption < 50);
		if( CoC.player.cor < 50 ) {
		...
		//[if (hasBalls = true) ;
		if( CoC.player.balls > 0 ) {
			MainView.outputText( ', gently moving your [balls] out of the way in the process' );
		}
		MainView.outputText( '.  He slams his nine inch blue rod balls deep into your sodden box.  A loud squelch fills the air at the penetration, giving Nieve the go ahead to start in top gear, pistoning in and out at a hurried pace.' );
		//[if (hasCock = true);
		if( CoC.player.hasCock() ) {
			MainView.outputText( '  During the pounding Nieve grabs your dangling [cock biggest] and starts to pump it in time with his own thrusts, quite eager to please his [master].  Every time your feminine half cums, so does your male half, splashing its happy seed into the valley that your combined bodies have made.' );
		}
		//[if (isLactating = true) ;
		if( CoC.player.lactationQ() >= 100 ) {
			MainView.outputText( '  Your ice man reaches out with a free hand to caress your [chest], twiddling his fingers around your delicate [nipples]. After some rough pulling and flicking, pounding away all the while, he feels milk dribble onto his hand.  He catches a quick glance of the liquid before diving head-first to suckle on your milk faucets, drinking heavily of your cream as he\'s about to give you his.' );
		}
	...
	XmasMisc.prototype.nieveIsOver = function() {
		...
		if( CoC.flags[ kFLAGS.NIEVE_STAGE ] < 5 ) {
			MainView.outputText( 'You wake up with a yawn and a stretch. It feels just like any other day, though a bit warmer.  Quite a bit warmer in fact.  It feels like winter\'s grasp on Mareth has slipped, and it\'s entering Spring.  Curiously, you glance over towards the snow which had been recently deposited in your camp, and note it\'s melting away before your eyes.' );
			MainView.outputText( '\n\nOh well, not a huge loss, you figure.  It was fun while it lasted.' );
			CoC.flags[ kFLAGS.NIEVE_STAGE ] = 0;
		}
		//Nieve Completed;
		else {
			MainView.outputText( 'You wake up with a yawn and a stretch.  It feels just like any other day, though a bit warmer.  Quite a bit warmer in fact.  It feels like winter\'s grasp on Mareth has slipped, and it\'s entering Spring.  You consider the fact absently at first, but then you stop dead in your tracks.  What about Nieve?!' );
			...
			if( CoC.player.cor < 50 ) {
				MainView.outputText( '  You even tear up yourself, sad to see such a good friend leave, and so abruptly.' );
			}
			//Corruption >50: ;
			else {
				MainView.outputText( '  You even feel a little sad yourself, it\'s such a pity to see a good, obedient fucktoy leave.' );
			}
			MainView.outputText( '  The snowman collapses on itself, melting away before your very eyes.  In a matter of hours, this entire area will be nothing but a dried up, dehydrated desert once more.  But as the jingling fades out into the distance, you vow to hold on to the tear.' );
			MainView.outputText( '\n\nThe tear of Nieve.' );
			if( CoC.player.cor < 50 ) {
	...
	XmasMisc.prototype.returnOfNieve = function() {
		MainView.clearOutput();
		MainView.outputText( 'As you awake in the morning you find yourself shivering slightly.  A cool breeze sweeps over your camp, while in the distance jingling bells can be heard.  How odd.  You haven\'t heard bells like that since...' );
		...
		MainView.outputText( '\n\nYou sink down into the snow, clutching Nieve\'s tear close and remember ' + this.nieveMF( 'his', 'her' ) + ' last words to you.  "<i>You were the best master I could have hoped for.</i>"  The voice rings through your head. "<i>And...  And I’ll mi--0...</i>"  Though ' + this.nieveMF( 'he', 'she' ) + ' was cut off, you knew what ' + this.nieveMF( 'he', 'she' ) + ' was saying.' );
	...
	XmasMisc.prototype.nieveReturnsPartII = function() {
		MainView.clearOutput();
		MainView.outputText( 'You sigh, resigning yourself to your companion\'s fate.  However, as you rise from the snow to return to camp, you hear a soft, muffled voice.  Perplexed, you crawl forward in the snow, frantically seeking out the source.' );
		...
		MainView.outputText( ' eyes shine with crystallizing tears, not entirely unlike the ones ' + this.nieveMF( 'he', 'she' ) + ' left with you on your last meeting.  "<i>I\'m so glad to be back!  I was worried I would be sent somewhere else, but I had so much fun here, and I told the big man that, and then Winter came and I wasn\'t sure you were still here, and then it turned out I might have gone to someone else, but oh my goodness I\'m so happy I came back!</i>" Nieve belts out without pause for breath before wrapping ' + this.nieveMF( 'his', 'her' ) + ' arms around you once again.' );
