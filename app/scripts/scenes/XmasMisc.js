...
angular.module( 'cocjs' ).run( function( SceneLib, EventParser, AppearanceDefs, ConsumableLib, CockTypesEnum, OnLoadVariables, Utils, Descriptors, kFLAGS, CoC, EngineCore ) {
	...
	XmasMisc.prototype.candyCaneTrapDiscovery = function() {
		...
		EngineCore.menu();
		EngineCore.addButton( 0, 'Investigate', this.investigateCandyCaneBun );
		EngineCore.addButton( 4, 'Leave', this.leaveBeforeMeetingCandyCaneBun );
	};
	//LEAVE;
	...
	XmasMisc.prototype.investigateCandyCaneBun = function() {
		...
		//(If corruption or libido>50) ;
			...
			EngineCore.outputText( '  Your eyes are quickly drawn to the rabbit morph\'s finely curved ass.' );
		}
		...
		//(If corruption or libido<50);
			...
			EngineCore.outputText( 'approach, ' );
		}
		...
		//(If corruption>50);
			...
			EngineCore.outputText( 'Your train of thought is derailed, but you agree to let the bunny believe whatever he wants to.' );
		}
		//(If silly mode on) ;
		if( EngineCore.silly() ) {
	...
	XmasMisc.prototype.helpWithTheCandyCane = function() {
		...
		//(If genderless) ;
			...
			EngineCore.outputText( 'sweat drips off of you.' );
		}
		...
		//(If corruption>50) ;
			...
			EngineCore.outputText( '  Despite your rising urge to take advantage of the unknowing bunny, you decide to retire and return to camp.' );
		}
	...
	XmasMisc.prototype.getAChristmasChicken = function() {
		...
		EngineCore.menu();
		EngineCore.addButton( 0, 'Talk', this.talkToXmasChicken );
		EngineCore.addButton( 4, 'Leave', this.leaveXmasChicken );
	};
	//[Leave];
	...
	XmasMisc.prototype.talkToXmasChicken = function() {
		...
		//(if herm);
			...
			EngineCore.outputText( 'How about I give you a few minutes to do whatever you want with my mouth?' );
		}
		...
		//(if sillymode);
		if( EngineCore.silly() ) {
			EngineCore.outputText( ', scrub' );
		}
		EngineCore.outputText( '?</i>"' );
		EngineCore.menu();
		EngineCore.addButton( 4, 'Decline', this.declineChickenBlowjobs );
		EngineCore.addButton( 0, 'Accept', this.acceptChristmasBJs );
	};
	//[Decline];
	...
	XmasMisc.prototype.acceptChristmasBJs = function() {
		...
		if( CoC.player.gender === 1 ) {
			...
			EngineCore.menu();
			EngineCore.addButton( 0, 'Man Parts', this.acceptChristmasManBJ );
			EngineCore.addButton( 1, 'Lady Bits', this.acceptChristmasWomanBJ );
		}
	};
	XmasMisc.prototype.acceptChristmasManBJ = function() {
		...
		//(else if cock would please Goldilocks) (area<= 30 or some shit i dunno whatever the standard is);
			...
			if( CoC.player.cockTotal() === 1 ) {
				...
				EngineCore.outputText( 'She grabs onto two of your other erect cocks with her hands and begins pumping away at them in time with the motions of her head.' );
			}
	...
	XmasMisc.prototype.approachKamiTheChristmasRoo = function() {
		...
		if( CoC.flags[ kFLAGS.KAMI_ENCOUNTER ] === 1 ) {
			...
			EngineCore.menu();
			EngineCore.addButton( 0, 'Let\'s Go', this.chooseLetsGoKami );
			EngineCore.addButton( 4, 'Leave', this.skipOutAfterOpeningKamisBox );
		}
	};
	...
	XmasMisc.prototype.takeItRooButtStyle = function() {
		...
		//[Else if have vagina];
			...
			EngineCore.outputText( 'your taint' );
		}
	...
	XmasMisc.prototype.KamiBlowJob = function() {
		...
		if( !CoC.player.isNaga() ) {
			...
			EngineCore.outputText( 'You steer your serpentine half into position, leaning your upper body within arms reach of Kami\'s wanting member.  You reach out' );
		}
	...
	XmasMisc.prototype.polarPete = function() {
		...
		EngineCore.menu();
		EngineCore.addButton( 0, 'Unwrap', this.unwrapPolarPete );
		EngineCore.addButton( 4, 'Leave', this.leaveDisFukkinPolarBear );
	};
	//Unwrap;
	...
	XmasMisc.prototype.nieveBuilding = function() {
		...
		if( CoC.flags[ kFLAGS.NIEVE_STAGE ] === 1 ) {
			...
			EngineCore.menu();
			EngineCore.addButton( 0, 'Snowwoman', this.nieveSnowWoman );
			EngineCore.addButton( 1, 'Snowman', this.nieveSnowMan );
			//[Snowwoman] [Snowman] ;
		}
	...
	XmasMisc.prototype.nieveComesToLife = function() {
		...
		//[Y/N];
		EngineCore.menu();
		EngineCore.addButton( 0, 'Yes', this.yesKeepNieve );
		EngineCore.addButton( 1, 'No', this.noNoKeepNieve );
	};
	//Yes of course, what do you look like, an idiot?;
	XmasMisc.prototype.yesKeepNieve = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'You actually laugh at the question.  Of course you want ' + this.nieveMF( 'him', 'her' ) + ' to stay!' );
		//[Silly mode: ;
		if( EngineCore.silly() ) {
			EngineCore.outputText( '  A free follower, no worries about buying worthless dye or raising their affection?  Fuck yeah, you\'ll take ' + this.nieveMF( 'him', 'her' ) + '!' );
		}
	...
	XmasMisc.prototype.approachNieve = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'You wave at Nieve, getting ' + this.nieveMF( 'his', 'her' ) + ' attention and calling for ' + this.nieveMF( 'him', 'her' ) + '.  The icy spirit-' + this.nieveMF( 'man', 'woman' ) + ' happily walks over, smiling.  ' + this.nieveMF( 'He', 'She' ) + ' takes ' + this.nieveMF( 'his', 'her' ) + ' time, allowing you to take in ' + this.nieveMF( 'his', 'her' ) + ' naked body.' );
		...
		EngineCore.outputText( ' eyes and pure white hair, which ' + this.nieveMF( 'barely goes past his ears', 'tumbles down past her shoulders' ) + '.  Much of ' + this.nieveMF( 'his', 'her' ) + ' body glimmers with a fine layer of powdered snow or ice.' );
	...
	XmasMisc.prototype.lickNieve = function() {
		...
		//Silly Mode:;
			...
			//Kid A: ;
				...
				EngineCore.outputText( ', your little anemone spawn giggles and laughs at your predicament before finally ladling some water onto your lover\'s snatch' );
			}
	...
	XmasMisc.prototype.fuckNieve = function() {
		...
		EngineCore.outputText( '  And with that she dives back down onto your ' + Descriptors.cockClit( x ) + '.  Her head bobs and bows, giving the sensation of an icecube running up and down your ' );
		if( x >= 0 ) {
		...
		if( x >= 0 ) {
			...
			//(Canine) ;
				...
				if( CoC.player.cockArea( x ) >= 40 ) {
					...
					EngineCore.outputText( '  It takes her a few good minutes, but eventually she works her way down and takes the entire knot into her mouth.  She raises her hands in triumph and lets out a "<i>Mmmmrrrrrpphhh!</i>" before pulling back for a breath, inadvertently exposing your wet hard cock to the cold, wintery air.' );
				}
			}
			/*Cock types
			 0 - human
			...
			//(If you add dicks later, placeholder text:) ;
				...
				EngineCore.outputText( '  Nieve works on your mutant cock, spending time working every unique little facet of it.' );
			}
		}
		//(Clitoris) ;
		else {
		...
		if( x >= 0 ) {
			EngineCore.outputText( 'with your ' + Descriptors.cockDescript( x ) + '' );
		} else {
			EngineCore.outputText( 'with yours' );
		}
	...
	XmasMisc.prototype.nieveIsOver = function() {
		...
		//Nieve Completed;
			...
			//Corruption >50: ;
				...
				EngineCore.outputText( '  You even feel a little sad yourself, it\'s such a pity to see a good, obedient fucktoy leave.' );
			}
	...
	XmasMisc.prototype.fixNieve = function() {
		if( CoC.flags[ kFLAGS.NIEVE_GENDER ] === 0 ) {
			...
			EngineCore.menu();
			EngineCore.addButton( 0, 'Gems', this.fixNieveMouth, 0 );
			EngineCore.addButton( 1, 'Coal', this.fixNieveMouth, 1 );
		} else {
	...
	XmasMisc.prototype.returnOfNieve = function() {
		...
		//[Next];
		EngineCore.menu();
		EngineCore.addButton( 0, 'Next', this.fixNieve );
	};
	XmasMisc.prototype.nieveReturnsPartII = function() {
		...
		EngineCore.outputText( ' eyes shine with crystallizing tears, not entirely unlike the ones ' + this.nieveMF( 'he', 'she' ) + ' left with you on your last meeting.  "<i>I\'m so glad to be back!  I was worried I would be sent somewhere else, but I had so much fun here, and I told the big man that, and then Winter came and I wasn\'t sure you were still here, and then it turned out I might have gone to someone else, but oh my goodness I\'m so happy I came back!</i>" Nieve belts out without pause for breath before wrapping ' + this.nieveMF( 'his', 'her' ) + ' arms around you once again.' );
