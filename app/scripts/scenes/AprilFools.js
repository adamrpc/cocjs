...
angular.module( 'cocjs' ).run( function( SceneLib, OnLoadVariables, AppearanceDefs, kFLAGS, CoC, EngineCore ) {
	function AprilFools() {
	}
	...
	AprilFools.prototype.leavePonies = function() {
		EngineCore.outputText( '', true );
		EngineCore.outputText( 'Deciding it must be some demonic trick, you decide to retreat from the scene before they notice your presence.', false );
	...
	AprilFools.prototype.approachPonies = function() {
		...
		EngineCore.outputText( 'The blue one looks at you in a puzzled manner while hovering above the ground, "<i>Actually, you are the first person we have seen who isn\'t a pony.</i>"\n\n', false );
		EngineCore.outputText( 'To be fair, you tell them, you haven\'t seen any one who IS a pony.  The pink one, now slightly calmer, interjects herself into the conversation "<i>Ponies, not ponies, whatever!  We have a new friend, and I say PARTY!!!</i>"\n\n', false );
		EngineCore.outputText( 'These little horses are definitely a rambunctious bunch, especially the white one, who, from the moment she spotted you, hasn\'t stop complaining about your awful sense of fashion. The only exception seems to be that yellow one hiding behind a screen of squirrels, birds and what you think are three generations of rabbits.\n\n', false );
		EngineCore.outputText( 'What should you do? You could party with the horses; after all, you haven\'t had a reason to party since getting here. You can politely decline for now or leave these oddly colored and slightly disturbing creatures for the more familiar sight of demons.\n\n', false );
		EngineCore.outputText( 'Whichever you choose, something tells you that you won\'t see these ponies again.', false );
	...
	AprilFools.prototype.derpPolitely = function() {
		EngineCore.outputText( '', true );
		EngineCore.outputText( 'You hold out your arms and stop the ponies.  Once you have their attention you let them know you have something important you need to do for now, but will come back soon.  With a wave you turn and walk back into the trees to a chorus of disappointed "<i>ahhhs</i>", mostly from the pink one.', false );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
