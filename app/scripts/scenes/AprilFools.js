...
angular.module( 'cocjs' ).run( function( SceneLib, MainView, OnLoadVariables, AppearanceDefs, kFLAGS, CoC, EngineCore ) {
	function AprilFools() {
	}
	...
	//----------Next Page-----------;
	AprilFools.prototype.leavePonies = function() {
		MainView.outputText( '', true );
		MainView.outputText( 'Deciding it must be some demonic trick, you decide to retreat from the scene before they notice your presence.', false );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	AprilFools.prototype.approachPonies = function() {
		MainView.outputText( '', true );
		MainView.outputText( 'You slowly begin your approach, though you are spotted almost immediately by the pink one.\n\n', false );
		MainView.outputText( 'You aren\'t two steps out of the forest before it seems she, and it HAS to be a she, seemingly teleports in front of you with an expression of rapture and joy pasted on her face.  You almost instinctively pull out your ' + CoC.player.weaponName + ', startled by the appearance of the pink one in your face.  Immediately she starts asking a myriad of questions, speaking at a speed which makes it impossible to understand a word.  Almost as stunning is the fact that she is speaking in a human tongue. Before you have the time to gather yourself up and figure out what the pink one is saying, or even what is going on, the rest of her group have managed to catch up to her, and begin calming her down.\n\n', false );
		MainView.outputText( 'The purple one turns to you looking apologetic and a bit exasperated, "<i>Sorry for our friend startling you. She gets excited <b>really</b> easily.</i>"\n\n', false );
		MainView.outputText( 'You respond that it\'s no problem, but she should be more careful approaching strangers', false );
		if( CoC.player.weaponName !== 'fists' ) {
			MainView.outputText( ', especially armed strangers', false );
		}
		MainView.outputText( '.\n\n', false );
		MainView.outputText( 'The blue one looks at you in a puzzled manner while hovering above the ground, "<i>Actually, you are the first person we have seen who isn\'t a pony.</i>"\n\n', false );
		MainView.outputText( 'To be fair, you tell them, you haven\'t seen any one who IS a pony.  The pink one, now slightly calmer, interjects herself into the conversation "<i>Ponies, not ponies, whatever!  We have a new friend, and I say PARTY!!!</i>"\n\n', false );
		MainView.outputText( 'These little horses are definitely a rambunctious bunch, especially the white one, who, from the moment she spotted you, hasn\'t stop complaining about your awful sense of fashion. The only exception seems to be that yellow one hiding behind a screen of squirrels, birds and what you think are three generations of rabbits.\n\n', false );
		MainView.outputText( 'What should you do? You could party with the horses; after all, you haven\'t had a reason to party since getting here. You can politely decline for now or leave these oddly colored and slightly disturbing creatures for the more familiar sight of demons.\n\n', false );
		MainView.outputText( 'Whichever you choose, something tells you that you won\'t see these ponies again.', false );
		//Option one: Leave Politely;
		//Option Two: Too creepy...;
		//Option three: Yay, party?;
		EngineCore.choices( 'Too creepy', this, this.derpCreepy, 'Yay Party!', this, this.derpyParty, '', null, null, '', null, null, 'Leave', this, this.derpPolitely );
	};
	AprilFools.prototype.derpPolitely = function() {
		MainView.outputText( '', true );
		MainView.outputText( 'You hold out your arms and stop the ponies.  Once you have their attention you let them know you have something important you need to do for now, but will come back soon.  With a wave you turn and walk back into the trees to a chorus of disappointed "<i>ahhhs</i>", mostly from the pink one.', false );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	AprilFools.prototype.derpCreepy = function() {
		MainView.outputText( '', true );
		MainView.outputText( 'Cocks, horns and slavering vaginas is one thing, but this is almost too much cute to process.  You determine to leave this grove and never EVER come back again.  Still disturbed by the mental images running through your head, as you make your way back to camp, you callously slaughter an imp. Yeah, that feels better.\n\n(+10 XP!  +5 Gems!)', false );
		CoC.player.XP += 10;
		CoC.player.gems += 5;
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	 ...
	 */
	SceneLib.registerScene( 'aprilFools', new AprilFools() );
} );