...
angular.module( 'cocjs' ).run( function( SceneLib, MainView, Descriptors, Combat, kFLAGS, CoC, EngineCore ) {
	...
	BrigidScene.prototype.pcDefeatsBrigid = function() {
		...
		MainView.outputText( 'The harpy jailer collapses, ' );
		if( CoC.monster.lust > 99 ) {
			MainView.outputText( 'too turned on' );
		} else {
			MainView.outputText( 'too badly beaten' );
		}
	...
	BrigidScene.prototype.brigitPostQueenDeathBadEndII = function() {
		...
		if( CoC.player.gender <= 1 || CoC.player.pregnancyIncubation > 0 ) {
			...
			if( !CoC.player.hasCock() ) {
				...
				if( CoC.player.pregnancyIncubation > 0 ) {
					MainView.outputText( '  Once this disgusting thing you\'re carrying falls out of you, we\'ll use you more traditionally, but...</i>"  She pauses to press uncomfortably on your pregnant belly, then resumes speaking.  "<i>I have no intention of letting you sit idle for however long that takes.' );
				}
				MainView.outputText( '  For now, I\'ve given you the most obvious modification - I can think of something else later.</i>"' );
			...
			if( CoC.player.cockTotal() > 1 || CoC.player.cumQ() > 1000 ) {
				MainView.outputText( '  ' + Descriptors.SMultiCockDesc() + ' holds forth for a while, until Kiri\'s ass, pussy, and tailfeathers are soaked with oozing, white cum.' );
			}
			MainView.outputText( '  Next to you, the salamander on the other table climaxes as well, dumping a load into Hel that sizzles when it leaks onto his fiery, iron-bound tail.  Your former lover squirms and shivers as the scalding-hot load fills her cunt, and her own prick twitches weakly, depositing a small string of bubbling jism on the stone table.' );
		...
		//(else if not visibly pregnant F or H);
			...
			if( CoC.player.hasCock() ) {
				MainView.outputText( ' as the phoenix\'s cunt slides onto your [cock smallest]' );
			}
			MainView.outputText( '.  Brigid makes her way up the stairs, leaving you and the others at the mercy of the assorted horde.  The harpy rides your mouth greedily, knocking your head into the hard stone floor, and the last thing you can see around her feathery thighs before you slip into unconsciousness again is Kiri being pulled roughly from her resting place atop Hel by a mean-looking, wide-hipped harpy matron who takes her place, sitting sideways and drawing her talons sadistically across Hel\'s vulnerable chest to smear Kiri\'s seed around...' );
