...
angular.module( 'cocjs' ).run( function( SceneLib, MainView, JeanClaude, AppearanceDefs, StatusAffects, PerkLib, Combat, kFLAGS, CoC, EngineCore ) {
	...
	JeanClaudeScenes.prototype.gogoFuckTheseBasilisksNorth = function() {
	...
	JeanClaudeScenes.prototype.gogoFuckTheseBasilisks = function() {
		MainView.clearOutput();
		MainView.outputText( 'You breathe deep, steel yourself' );
		if( CoC.player.hasKeyItem( 'Laybans' ) >= 0 ) {
			MainView.outputText( ', put your shades on' );
		}
		MainView.outputText( ' and stride out onto the gantry. The plan is simple. You’ll keep your eyes up and march across as if you belong there, as if you are on official business.' );
		if( CoC.player.demonScore() > 4 ) {
			MainView.outputText( '  You can’t be mistaken for anything but a demon, after all.' );
		}
		MainView.outputText( '  If or when the lizards realize who you are, you’ll make a break for it, and hopefully by then you’ll be in reach of the exit. It’s straight-forward, it’s elegant, it will work, whatever your hammering pulse has to say on the matter.' );
		MainView.outputText( '\n\nYou try and not look down as you proceed, back straight and as aloof as you can.' );
		if( CoC.player.isNaga() ) {
			MainView.outputText( '  You’re probably imagining things but the passage of your sinuous form over the wire surface seems absurdly loud, an endless rubbing of sandpaper which seems to fill the hall.' );
		} else if( CoC.player.isGoo() ) {
			MainView.outputText( '  Godsdamn this form... you try not to think as you slide across the wire surface, intensely aware of the slimy sound you make as you go, of the steady drip of your fluids through the mesh, trying not to think of it splashing down on curious, blunt heads.' );
		} else {
			MainView.outputText( '  Clack, clack, clack. You are intensely aware of the sound of your [feet] on the wire surface. You close your eyes and tell yourself you’re only imagining that it’s loud, that it echoes around the hall, that the basilisks would have to be deaf not to hear you.' );
		}
	...
	JeanClaudeScenes.prototype.capturedByBasilisks = function() {
		...
		if( CoC.player.hasCock() ) {
			MainView.outputText( ' [eachCock] stiffening' );
		}
		if( CoC.player.hasCock() && CoC.player.hasVagina() ) {
			MainView.outputText( ' and' );
		}
		if( CoC.player.hasVagina() ) {
			MainView.outputText( ' your [vagina] beading with excitement' );
		}
		if( !CoC.player.hasCock() && !CoC.player.hasVagina() ) {
			MainView.outputText( ' your loins aflame with excitement' );
		}
	...
	JeanClaudeScenes.prototype.capturedByBasilisksRun = function() {
		MainView.clearOutput();
	...
	JeanClaudeScenes.prototype.capturedbyBasilisksTaunt = function() {
		MainView.clearOutput();
		MainView.outputText( 'You stare into the yellow, hypnotic eyes and for a moment see beyond them, see them as part of this being rather than the whole, see the arrogance that sits behind them, see, perhaps, a way out of this. With all your willpower you force your mouth to open, work your vocal chords- and make a \'pfft\' noise.' );
	...
	JeanClaudeScenes.prototype.beatDaFuckhugeBasilisk = function( hpVictory ) {
		...
		if( CoC.player.mf( 'm', 'f' ) === 'f' && EngineCore.silly() ) {
			MainView.outputText( '  You raise your fists in glee and crow “<i>I AM THE LIZARD QUEEN!</i>” at the ceiling.' );
		}
	...
	JeanClaudeScenes.prototype.fuckhugeBasiliskFuckedYouUp = function( hpVictory ) {
		MainView.clearOutput();
		if( !hpVictory ) {
			MainView.outputText( 'You look down at the [weapon] in your hand in bleary confusion. Why are you holding it? Everything feels slow and close, your body almost painful with arousal and sensitivity, and you can’t push beyond it, can’t fathom where you are, what is happening, who you are. It’s the golden glow- it surrounds you and blots everything else out. When Jean-Claude steps in and almost kindly grabs your [weapon] out of your unresisting hands and takes you by the chin, you almost groan. So big, so powerful, so in control- everything you are not. You gladly stare into his maddening yellow eyes, gladly intensifying the feeling of delicious helplessness they give you.' );
	...
	JeanClaudeScenes.prototype.regularBadEnd = function() {
		MainView.clearOutput();
		MainView.outputText( 'The Rex’s yellow glow encompasses you, glaring out everything - your purpose, your memories, your sense of self, replaced only by the need to stare deeper and deeper into the pits of gold, the depthless lava of his will which rolls over you, heating you within and without. When he steps into you and puts his warm, smooth hand on your waist you almost flinch, you are that sensitive to his presence. He grins with the exhilaration of a conqueror as he draws you further into himself. The nagging undercurrents of your own body rise to the surface as you feel his warmth and smell his powerful musk;' );
		if( CoC.player.hasVagina() ) {
			MainView.outputText( ' your [vagina] beads with excitement' );
		}
		if( CoC.player.hasCock() && CoC.player.hasVagina() ) {
			MainView.outputText( ' and' );
		}
		if( CoC.player.hasCock() ) {
			MainView.outputText( ' [eachCock] hardens' );
		}
		if( CoC.player.hasCock() || CoC.player.hasVagina() ) {
			MainView.outputText( ' as' );
		}
		MainView.outputText( ' your body reacts uncontrollably to the presence of the being whose claws reach deeper and deeper into your mind, touching and triggering your basest desires.' );
		if( CoC.player.findStatusAffect( StatusAffects.RemovedArmor ) > 0 ) {
			MainView.outputText( '  You realize vaguely you already have taken your clothes off, undoubtedly your own subconscious preparing you for this moment.' );
		} else {
			MainView.outputText( '  You really are uncomfortably hot underneath the glare of those eyes. Hardly even aware you’re doing it, you begin to take off your [armor]. Piece after piece falls to the floor until you are naked before him.' );
		}
		...
		if( CoC.player.skinAdj !== 'smooth' || CoC.player.skinAdj !== 'latex' || CoC.player.skinAdj !== 'rubber' ) {
			...
			if( CoC.player.skinType === AppearanceDefs.SKIN_TYPE_FUR ) {
				MainView.outputText( '  Your fur drops away in ragged clumps as you drink.' );
			}
			MainView.outputText( '  Once you are finished another black egg is pushed into your hands, then another... you sigh as the viscous gloop slides down your throat and your flesh begins to thrum, your pores puckering and then shrinking into themselves as your skin begins to gleam with the clean starkness of rubber. You slowly examine yourself once the transformation is complete, marvelling at your living, latex flesh. Yes. This feels right.' );
		...
		if( CoC.player.hasVagina() ) {
			MainView.outputText( ' your [vagina] dribbling excitement' );
		}
		if( CoC.player.hasVagina() && CoC.player.hasCock() ) {
			MainView.outputText( ' and' );
		}
		if( CoC.player.hasCock() ) {
			MainView.outputText( ' your [cock] straining helplessly' );
		}
		MainView.outputText( ' to this depth of slutty surrender you have plumbed, your neck muscles seizing up and your limbs tightening as the thoughts sink into you.' );
		...
		if( CoC.player.isBiped() ) {
			MainView.outputText( '  The transformation isn’t quite complete; your mouth opens as a will not your own forces your arms and legs to open, leaving you completely exposed. It takes you a few seconds to realize that your mouth opening wasn’t your doing, either. It won’t close.' );
		} else {
			MainView.outputText( '  The transformation isn’t quite complete; your mouth opens as a will not your own forces your arms to open and your monstrous bottom half to take on a stance as close to complete submission as it will allow, your [hips] flared to the world. It takes you a few seconds to realize that your mouth opening wasn’t your doing, either. It won’t close.' );
		}
		...
		MainView.outputText( ' push roughly against your [asshole]' );
		if( CoC.player.hasVagina() ) {
			MainView.outputText( ' and [vagina]' );
		}
		MainView.outputText( ', it disappears into the depths forever. Your mind is a blank canvas of pure, submissive pleasure and it, like your limitless future, is golden.' );
		EngineCore.gameOver();
	};
	JeanClaudeScenes.prototype.bwombBadEnd = function() {
		...
		if( CoC.player.findStatusAffect( StatusAffects.RemovedArmor ) >= 0 ) {
			MainView.outputText( ' You already have taken your clothes off, undoubtedly your own subconscious preparing you for this moment, and you require no direction from Jean-Claude to sit yourself down and eagerly' );
			if( CoC.player.isBiped() ) {
				MainView.outputText( ' spread your [hips]' );
			} else {
				MainView.outputText( ' position yourself' );
			}
			MainView.outputText( ' for him, your breeder’s body aching to be taken.' );
		} else {
			MainView.outputText( ' You require no direction from Jean-Claude to take off your [armor] once you are done. Piece after piece falls to the floor until you are naked before him. Anticipation thrumming through you, you sit yourself down and eagerly' );
			if( CoC.player.isBiped() ) {
				MainView.outputText( ' spread your [hips]' );
			} else {
				MainView.outputText( ' position yourself' );
		...
		if( !CoC.player.hasCock() ) {
			MainView.outputText( '  His other dick bats into your stomach, thrusting upwards towards your [chest] as he speaks, his eyes transfixing you, drawing you further into his savage, potent, golden dreams of lust and conquest as he speaks.' );
		} else {
			MainView.outputText( '  His other dick thrusts against your own [cock], hardening it with incessant rubbing as he speaks, his eyes transfixing you, drawing you further into his savage, potent, golden dreams of lust and conquest as he speaks.' );
		}
