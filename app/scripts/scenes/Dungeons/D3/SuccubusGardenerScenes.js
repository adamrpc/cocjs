...
angular.module( 'cocjs' ).run( function( SceneLib, kFLAGS, Descriptors, Utils, StatusAffects, CockTypesEnum, AppearanceDefs, Combat, EventParser, CoC, EngineCore ) {
	...
	SuccubusGardenerScenes.prototype.gardenerDefeated = function() {
		return CoC.getInstance().flags[ kFLAGS.D3_GARDENER_DEFEATED ] > 0;
	};
	SuccubusGardenerScenes.prototype.gardenerKilled = function() {
		return CoC.getInstance().flags[ kFLAGS.D3_GARDENER_DEFEATED ] === 3;
	};
	SuccubusGardenerScenes.prototype.gardenerFucked = function() {
		return CoC.getInstance().flags[ kFLAGS.D3_GARDENER_DEFEATED ] === 2;
	};
	SuccubusGardenerScenes.prototype.gardenerLeft = function() {
		return CoC.getInstance().flags[ kFLAGS.D3_GARDENER_DEFEATED ] === 1;
	};
	...
	SuccubusGardenerScenes.prototype.tentaFail = function() {
		...
		if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( ' cunt-kissers' );
		} else {
			EngineCore.outputText( ' cock-suckers' );
		}
		...
		if( CoC.getInstance().player.hasCock() ) {
			...
			EngineCore.outputText( ' the sensitive skin of your groin' );
		}
		EngineCore.outputText( '. She eagerly goes to work, moaning from the attentions of the tentacle down below, and licking with such fervor that you\'d think this is what she wanted all along.' );
		...
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( ' blow a wad down this slut\'s throat' );
		} else if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( ' cream yourself on this slut\'s face' );
		} else {
			EngineCore.outputText( ' teach this bitch how to pleasure an asshole' );
		}
		EngineCore.outputText( '. None-too-quiet slurps hang in the steamy air, accompanied by your own ' + CoC.getInstance().player.mf( 'grunts', 'coos' ) + ' of enjoyment. You reach down to cup one of her breasts, ignorant of the shadows creeping nearer.' );
		...
		if( !CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( ' Its outline is clearly phallic, but how could such a huge thing ever fit inside you, let alone anyone?' );
		} else {
			EngineCore.outputText( ' Its tip oozes lubricants, but the gaping pussy at its tip is big enough to hold six minotaurs\' monster-cocks.' );
			if( CoC.getInstance().player.biggestCockLength() < 36 ) {
				EngineCore.outputText( ' How could you ever hope to fill it?' );
			} else {
				EngineCore.outputText( ' How long were they preparing this for you?' );
			}
		}
		...
		if( !CoC.getInstance().player.hasCock() ) // 100% gonna have a cooch by this point
		{
			EngineCore.outputText( '\n\nYou can feel your' );
			if( createdVag ) {
				EngineCore.outputText( ' new' );
			}
			EngineCore.outputText( ' entrance giving around the girthy monstrosity with each passing second. Wider and wider, your flesh opens up. It feels so big that you wonder if you\'re going to rip in half, but there is no discomfort, only the pleasure of accepting it\'s tumescent, inhuman length. You groan as your own fluids make themselves known, soaking your gaped lips in girlish cum. There\'s no way that you should be able to take this, let alone enjoy it, but you are. Oh gods, how you are!' );
		...
		//Dicks;
			...
			if( CoC.getInstance().player.biggestCockLength() <= 12 ) {
				EngineCore.outputText( ' The ease with which you slide inside is no surprise, given the disparity in size.' );
			} else if( CoC.getInstance().player.biggestCockLength() <= 30 ) {
				EngineCore.outputText( ' The ease with which you enter is no real surprise, given that even your bloated length' + ((CoC.getInstance().player.cocks.length > 1) ? 's are' : ' is') + ' small by comparison.' );
			} else if( CoC.getInstance().player.biggestCockLength() <= 48 ) {
				EngineCore.outputText( ' You slide in with ease, perfectly matched to the hungry fuck-tunnel in size.' );
			} else {
				EngineCore.outputText( ' Sliding in must take quite the effort. The orange-hued twat is drooling around you, slowly working down an inch at a time. It takes all of you, even if it has to stretch beyond all reason to do so.' );
			}
			EngineCore.outputText( ' It feels good, better' );
			if( CoC.getInstance().player.biggestCockLength() <= 12 ) {
				EngineCore.outputText( ' than a giant that looks that loose should. You\'d swear it was tight little twat from how firmly it\'s squeezing you!' );
			} else if( CoC.getInstance().player.biggestCockLength() <= 30 ) {
				EngineCore.outputText( ' than you had thought at a glance. Not only is it the perfect size, but it seems to hug and touch every part of your maleness' + ((CoC.getInstance().player.cocks.length > 1) ? 'es' : '') + ' just right.' );
			} else if( CoC.getInstance().player.biggestCockLength() > 48 ) {
				EngineCore.outputText( ' than you would expect given how tightly-stretched it looks. You\'d think it\'d be pressing down painfully, but it feels tailor made to take you instead.' );
			}
			//Dicks need to grow;
			if( CoC.getInstance().player.biggestCockLength() <= 30 ) {
				EngineCore.outputText( '\n\nGlowing orange goo leaks from the oversized slit in thick beads. You briefly wonder what purpose the odd coloration could serve when the feeling of your cock' + ((CoC.getInstance().player.cocks.length > 1) ? 's' : '') + ' stretching answers. It\'s making you grow bigger, somehow! The sensation is akin to stretching long-dormant muscles... an subtle flexing of unrealized potential that makes you aware of just how much you can do, or in this case, how big you can get. Your vision swims as your body works to keep up with the sudden change, and you close your eyes to keep from sicking up.' );
			...
			// Dicks big enough;
				...
				EngineCore.outputText( '\n\nYou don\'t go soft after you finish squirting. You don\'t even get to catch your breath. The orange channel might be holding still as it digests your load, but your dick' + ((CoC.getInstance().player.cocks.length > 1) ? 's are' : ' is') + ' still getting more sensitive. You don\'t even need motion for the clutching walls to arouse you any more, and once it starts moving... you\'re not sure you\'ll be able to handle it.' );
				EngineCore.outputText( '\n\nA few seconds later, you find out that you can\'t handle it when the tentacle strokes you. The wave of skittering, electric pleasure overloads your mind, and breakers for your consciousness shut it down while your dick' + ((CoC.getInstance().player.cocks.length > 1) ? 's throb' : 'throbs') + ', loving every moment of it.' );
	...
	SuccubusGardenerScenes.prototype.femGenderlessLoss = function( hpVictory ) {
		...
		if( hpVictory ) {
			...
			EngineCore.outputText( ', too turned on to try and stop your hands from diving into your needy crotch.' );
		}
		...
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.FactoryOverload ) >= 0 ) {
			EngineCore.outputText( ' destroy' );
		} else {
			EngineCore.outputText( ' shut down' );
		}
		EngineCore.outputText( ' our factory would fall to a simple succubus and her lowly garden?"</i> She strides over to you, hips wiggling in pleasure, and kneels alongside you, running her hand over your abdomen like a girl handling a new pet for the first time. <i>"You\'re going to be so useful in my garden!"</i>' );
		EngineCore.outputText( '\n\nHer garden?' );
		if( !hpVictory ) {
			EngineCore.outputText( ' You cast a furtive glance towards the writhing vines.' );
			if( CoC.getInstance().player.hasVagina() ) {
				EngineCore.outputText( ' Your underthings were already swampy with need, but the sight of watching those eager vines squirm elicits fresh trickles of lube to course down your [legs].' );
			} else {
				EngineCore.outputText( ' You were already horny, but you find yourself wishing you had a pussy, just so you could take even more of them inside you.' );
			}
		} else {
			EngineCore.outputText( ' You cast a worried look towards the writhing vines. They seem so eager, yet they hold back for now, perhaps waiting for their mistress to finish. No... you can\'t go down like this!' );
		}
		...
		if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( ' Against your better judgment, you\'re starting to get wet from the promises of her words. You know you\'re being given to a neverending hell of sexual stimulation, but you can\'t stop your body\'s libido from responding to the idea.' );
		}
		EngineCore.outputText( ' <i>"Yeah, you want it, doncha?"</i> Slim fingers deftly press at your' );
		if( !CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( ' bare crotch, sinking into a crease that didn\'t exist a moment before. Her skilled touches and dark powers mold your body like a sculptor\'s clay, crafting a magnificent cunt on your crotch while simultaneously bringing it to a dripping wet state.' );
			CoC.getInstance().player.createVagina();
		} else {
			if( CoC.getInstance().player.wetness() <= 2 ) {
				EngineCore.outputText( ' moistening' );
			} else {
				EngineCore.outputText( ' soaked' );
			}
			EngineCore.outputText( ' crease, and it\'s all you can do not to cry out and thrust back against her.' );
		}
		EngineCore.outputText( '\n\nThe succubus idly toys with your [vagina], sometimes rubbing her thumb' );
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( ' across your [sack],' );
		} else if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( ' across the underside of your [cock],' );
		} else {
			EngineCore.outputText( ' against your [clit],' );
		}
		EngineCore.outputText( ' confiding aloud, <i>"To be honest, I\'m a little jealous of you."</i>' );
	...
	SuccubusGardenerScenes.prototype.femGenderlessLossII = function() {
		...
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( ' and ' + Descriptors.multiCockDescriptLight() );
		}
		EngineCore.outputText( ' with foreign wetness. The vines\' lubricant is surprisingly warm on your skin, and the feeling of rivulets of it running down the crack of your [butt] and over your [asshole] is surprisingly pleasant.' );
		if( CoC.getInstance().player.isBiped() ) {
			EngineCore.outputText( ' Your [legs] are eased open, but who are you to stop them? You\'re as wet as anything, and if spreading wide is the next step towards scratching your itch, so be it.' );
		}
		EngineCore.outputText( ' Your [clit] must look like a' );
		if( CoC.getInstance().player.clitLength > 6 ) {
			EngineCore.outputText( ' glistening, cum-soaked shaft' );
		} else if( CoC.getInstance().player.clitLength > 3 ) {
			EngineCore.outputText( ' tiny, cum-soaked cock' );
		} else if( CoC.getInstance().player.clitLength > 1 ) {
			EngineCore.outputText( ' swollen, lube-lacquered gumball' );
		} else {
			EngineCore.outputText( ' glistening, pink target' );
		}
		EngineCore.outputText( ' by now. You roll your [hips] as much as your position will allow. Your passion inflamed pussy has thus far avoided the tentacles\' affections, though it\'s the one part that wants them more than anything.' );
		...
		if( CoC.getInstance().player.hasCock() ) {
			...
			EngineCore.outputText( ' over the lurid squishing sounds coming from your cunt, and the two sensations combine into a riot of overwhelming sensory data. You\'re fucking and being fucked to two different, incongruent tempos and unable to make sense of it in the slightest.' );
		}
		//Back to normal.;
		EngineCore.outputText( '\n\nMore cum splatters across the exterior of your womanhood a moment before an enclosure envelops your [clit].' );
		if( CoC.getInstance().player.clitLength > 6 ) {
			EngineCore.outputText( ' It stretches some around the cock-sized button' );
		} else if( CoC.getInstance().player.clitLength > 3 ) {
			EngineCore.outputText( ' It has no trouble handling your plus-sized button' );
		} else {
			EngineCore.outputText( ' Your tiny button seems dwarfed by the tube enveloping it' );
		}
		EngineCore.outputText( ', but that doesn\'t make it feel heavenly. It gently pulsates around your buzzer, rippling and squeezing, rhythmically whipping electric lashes of sensation into your fuck-crazed body one after another.' );
		...
		if( CoC.getInstance().player.biggestTitSize() > 2 ) {
			EngineCore.outputText( ' and jiggle your tits' );
		}
		EngineCore.outputText( ', pressing itself flat against your cervix before finally exploding, creaming your insides with a thick layer of molten-hot seed. You would groan or whimper if your throat wasn\'t crammed so full of plantcock. Instead, your eyes roll back under their fluttering lids, revealing a narrow band of white as you lose all control. Your body shakes like a seizure victim\'s, all except for the muscles down south. They\'re too busy clenching and squeezing the lengthy intrudor, wringing every drop of lust-imbued cum' );
		if( CoC.getInstance().player.isPregnant() ) {
			EngineCore.outputText( ' to splatter against your occupied womb.' );
		} else {
			EngineCore.outputText( ' to fill your womb, to make it round with life so that others can feel what you\'re feeling.' );
		}

		EngineCore.outputText( '\n\nYour girlhoney squirts, splattering over the ground.' );
		if( CoC.getInstance().player.wetness() < 4 ) {
			EngineCore.outputText( ' You may not normally be capable of such soaking orgasms, but that seems of little concern to you.' );
		}
	...
	SuccubusGardenerScenes.prototype.maleLoss = function( hpVictory ) {
		...
		if( hpVictory ) {
			...
			EngineCore.outputText( ', too turned on to try and stop your hands from rushing to your crotch. Why bother?' );
		}
	...
	SuccubusGardenerScenes.prototype.maleLossPet = function() {
		...
		if( CoC.getInstance().player.biggestCockLength() >= 18 ) {
			EngineCore.outputText( ' <i>"Oh, it\'s quite the big bone, isn\'t it? You\'re going to have an awful hard time dragging that around on all fours, [puppy]. Why, I bet the moment you see a bitch, you\'re going to be humping the ground, practically immobilized by it.' );
		} else if( CoC.getInstance().player.biggestCockLength() >= 8 ) {
			EngineCore.outputText( ' <i>"Oh, that\'s quite the nice bone you\'ve got there, isn\'t it? Why, everyone is going to see how big you are down there, even while you\'re walking on all fours. I suppose you\'d be good for breeding the bitches' );
			if( CoC.getInstance().player.cocks[ CoC.getInstance().player.biggestCockIndex() ].cockType !== CockTypesEnum.DOG ) {
				EngineCore.outputText( ', once you have an appropriately-shaped cock' );
			}
			EngineCore.outputText( '.' );
		} else {
			EngineCore.outputText( ' Oh, that\'s quite the well-formed bone you\'ve got there, isn\'t it? I could probably take you to shows, and the judges would comment on how well it matches your form. You\'d like that, wouldn\'t you? Rolling over on your back and letting some official-looking demon measure your cock.' );
		}
		...
		EngineCore.outputText( '\n\n<i>"Goood ' + CoC.getInstance().player.mf( 'boy', 'girl' ) + ',"</i> she coos, twisting her foot to rub her heel against your [sheath]. <i>"You\'re just so eager to cum that you\'ll do anything, won\'t you?"</i>' );
		...
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( ', just above your [balls],' );
		}
		EngineCore.outputText( ' once more, cooing, <i>"I knew you\'d train well. Now, I\'m going to lead you to your new home, and then you\'ll get to blow all that nasty, pent-up spunk all over my hand. But you have to be good until then, okay?"</i>' );
		...
		if( !CoC.getInstance().player.hasMuzzle() ) {
			EngineCore.outputText( ', heedless of the fact that you seem to be able to reach a little farther with every one you consume' );
		}
		EngineCore.outputText( ', slobbering some in your vigor to get more. All the while, the depression fills higher. The sound of gushing fluid tapers down to pitter-patters of the last droplets impacting the impromptu bowl, accompanied by a moan from the demon. She\'s clearly enjoyed preparing your little treat.' );
		EngineCore.outputText( '\n\n<i>"Eat up, doggy."</i> Her foot disappears.' );
		EngineCore.outputText( '\n\nYou look down at your drooling rod' );
		if( CoC.getInstance().player.cocks.length > 1 ) {
			EngineCore.outputText( 's' );
		}
		EngineCore.outputText( ', then at the puddle of sap-laced milk, then at your rod again, then finally back at the puddle before getting up onto all' );
		if( CoC.getInstance().player.isNaga() ) {
			EngineCore.outputText( ' threes' );
		} else if( CoC.getInstance().player.isTaur() ) {
			EngineCore.outputText( ' your hooves' );
		} else {
			EngineCore.outputText( ' fours' );
		}
		EngineCore.outputText( '.' );
		if( !CoC.getInstance().player.isTaur() ) {
			EngineCore.outputText( ' You scrabble over, unusued to having to move in such a way,' );
		} else {
			EngineCore.outputText( ' You gallop over and drop down on your forelegs' );
		}
		EngineCore.outputText( ' and plant your [face] right into the succubus\' milk. It\'s a little thicker than milk normally is and stickier too, like honey, but it\'s also sweeter. The ambrosial fluid fills your mouth with scintillating, indescribable flavor that sets' );
		if( CoC.getInstance().player.tailType !== 0 ) {
			EngineCore.outputText( ' your tail' );
			if( CoC.getInstance().player.tailType === 13 && CoC.getInstance().player.tailVenom > 1 ) {
				EngineCore.outputText( 's' );
			}
		} else {
			EngineCore.outputText( ' your stubby, new tail' );
		}
		EngineCore.outputText( ' wagging.' );
		if( CoC.getInstance().player.tailType !== 2 ) {
			EngineCore.outputText( ' Never mind that it gets longer and fuzzier with every gulp or that your' );
		} else {
			EngineCore.outputText( ' Never mind that your' );
		}
		EngineCore.outputText( ' tongue is swelling up, wide, flat, and perfect for panting, licking, and lapping up a drink.' );
		CoC.getInstance().player.tailType = 2;
		...
		if( CoC.getInstance().player.cocks.length > 1 ) {
			EngineCore.outputText( 's thrum' );
		} else {
			EngineCore.outputText( ' thrums' );
		}
		EngineCore.outputText( ' with ready excitement. A thin trail of pre-spooge rolls out of you, trailing on the ground behind you with each happy twitch of your bitch-' );
		if( CoC.getInstance().player.biggestCockLength() >= 12 ) {
			EngineCore.outputText( 'breaker' );
		} else {
			EngineCore.outputText( 'boner' );
		}
		EngineCore.outputText( '. You hope she doesn\'t mind, but you have a hunch that she won\'t mind you being THAT kind of messy puppy.' );
		...
		if( CoC.getInstance().player.isTaur() || CoC.getInstance().player.isDrider() ) {
			...
			if( CoC.getInstance().player.isDrider() ) {
				EngineCore.outputText( ' clip-clop of your hooves' );
			} else {
				EngineCore.outputText( ' sharp clacks of your spidery claws' );
			}
			EngineCore.outputText( ' fading into the near silent pats of padded paws. Your hands are changing too fingers are shortening, but your nails are growing. Fur grows out over them, and your spine shifts, all so that you can follow on all fours that much easier.' );
		} else {
			//Nontaurs;
			EngineCore.outputText( '\n\nThe longer you follow, the easier it gets to stay on her heels.' );
			if( CoC.getInstance().player.isGoo() ) {
				EngineCore.outputText( ' Your gooey undercarriage easily divides into two hind legs, slapping wetly with regularity as you plod along.' );
			} else if( CoC.getInstance().player.isNaga() ) {
				EngineCore.outputText( ' Your slithering tail twitches, stopping you for a second, but then it splits into two limbs, each firmer and more canine than the snake-like appendage you once had. They wobble unsteadily for a second before jumping into action.' );
			} else {
				EngineCore.outputText( ' Your [legs] shorten as you plod along, slowing you momentarily as they change. You lift your knees up so that you can stand on your [feet] once more, only they\'re soft, padded paws now, tipped with short nails.' );
			}
			EngineCore.outputText( ' Your hands follow suit. First, pads form on your palms, making it easier to support your weight. Then, your fingers shorten. It\'s not entirely an unpleasant sensation, as it makes moving towards your home that much easier.' );
	...
	SuccubusGardenerScenes.prototype.maleLossPetII = function() {
		...
		EngineCore.outputText( ' cock through. That hole is on the back wall. You can\'t see what\'s over there, but your nose can smell pussies - some of them bitch-pussies, and lots of spilled cum. Your old mind would\'ve identified it as a glory-hole, but it\'s harder to focus on concepts like that now. ' );
		...
		if( CoC.getInstance().player.skinType !== AppearanceDefs.SKIN_TYPE_FUR ) {
			EngineCore.outputText( ' Was that there before?' );
		}
		EngineCore.outputText( ' It feels very right, and in your excitement, it\'s hard not to hump her leg. It must be obvious to her, because your dick' );
		if( CoC.getInstance().player.cocks.length > 1 ) {
			EngineCore.outputText( 's are' );
		} else {
			EngineCore.outputText( ' is' );
		}
		EngineCore.outputText( ' bobbing beneath you, and she gestures for you to roll onto your back.' );
		...
		EngineCore.outputText( ' trembling and spurting out ribbons of pre. Whining in the back of your throat, you look at her, hoping she can read the affection in your eyes, wanting her to see just how completely contented you are with this state of affairs.' );
		EngineCore.outputText( '\n\n<i>"Who\'s a good doggie?"</i>' );
		EngineCore.outputText( '\n\nYou bark and cum at the same time, shooting ropes all over your belly. Mistress is so nice; she keeps stroking while you shoot, encouraging her pet to shoot every drop of ' + CoC.getInstance().player.mf( 'his', 'her' ) + ' sperm out. You must have been really good! Lances of white goop splatter off your chest, still somewhat human in shape, and tag your chin.' );
		if( CoC.getInstance().player.cumQ() >= 2500 ) {
			EngineCore.outputText( ' A few thick ropes shoot past your head to cream the wall, eliciting a pleased giggle from your owner as she squeezes yet more from your length.' );
		}
		EngineCore.outputText( ' You yip with glee as you cream for Mistress\' fingers, but eventually, your climax trails off into a dribbly white mess, some of which clings to the demoness\' nails like beads of ivory dew.' );
	...
	SuccubusGardenerScenes.prototype.maleLossFeeder = function() {
		...
		if( CoC.getInstance().player.cocks.length === 1 ) {
			...
			EngineCore.outputText( ' one\'s' );
		}
		EngineCore.outputText( ' twitching length, the succubus giggles. <i>"Can you feel it? Can you feel your body just welling up with cum? I suppose I should have mentioned that, huh?"</i> She runs her finger back the other way, and your muscles clench, squeezing out a big, fat drop of pre from your cock, dripping down to hit' );
		if( CoC.getInstance().player.biggestTitSize() >= 3 ) {
			EngineCore.outputText( ' the underside of your breasts' );
		} else {
			EngineCore.outputText( ' your chin' );
		}
		EngineCore.outputText( '. You whimper with need. It\'s impossible not to. Hell, your ' + Descriptors.multiCockDescriptLight() );
		if( CoC.getInstance().player.cocks.length === 1 ) {
			EngineCore.outputText( ' is' );
		} else {
			EngineCore.outputText( ' are' );
		}
		EngineCore.outputText( ' flexing, and if you had anything to brace off of, you\'d be thrusting into the first orifice you could find.' );
		...
		//Ball filling;
			...
			if( CoC.getInstance().player.ballSize > 15 ) {
				EngineCore.outputText( ' Thankfully, a few tentacles loop under the gigantic orbs to help support them.' );
			}
		}
		EngineCore.outputText( '\n\nYou happily gurgle into the tentacle in your mouth and arch your back, blowing the first huge rope of your enhanced load' );
		// 9999 - Check muh;
		if( CoC.getInstance().player.biggestTitSize() >= 3 ) {
			EngineCore.outputText( ' all over your body' );
		} else {
			EngineCore.outputText( ' directly onto the soil below' );
		}
	...
	SuccubusGardenerScenes.prototype.maleLossFeederII = function() {
		...
		if( CoC.getInstance().player.biggestCockLength() < 6 ) {
			EngineCore.outputText( ' swiftly engulfs your meager offering' );
			if( CoC.getInstance().player.cocks.length > 1 ) {
				EngineCore.outputText( 's' );
			}
		} else if( CoC.getInstance().player.biggestCockLength() < 12 ) {
			EngineCore.outputText( ' rapidly devours your plus-sized length' );
			if( CoC.getInstance().player.cocks.length > 1 ) {
				EngineCore.outputText( 's' );
			}
		} else if( CoC.getInstance().player.biggestCockLength() < 24 ) {
			EngineCore.outputText( ' devours your immense offering' );
			if( CoC.getInstance().player.cocks.length > 1 ) {
				EngineCore.outputText( 's' );
			}
			EngineCore.outputText( ' with steady determination' );
		} else {
			EngineCore.outputText( ' slowly encircles your cum-spouting length' );
			if( CoC.getInstance().player.cocks.length > 1 ) {
				EngineCore.outputText( 's' );
			}
			EngineCore.outputText( ' with inhuman elasticity' );
		}
		EngineCore.outputText( '. The interior is covered with tiny, wiggling nubs that wiggle into action on contact with your trembling member' );
		if( CoC.getInstance().player.cocks.length > 1 ) {
			EngineCore.outputText( 's' );
		}
		EngineCore.outputText( '. It\'s like every single nerve has its own skilled masseuse giving it a rubdown, coaxing more pleasure from every square inch of skin than you thought it capable of producing. You cry out, totally overwhelmed, only to be silenced by a familiar presense in your mouth.' );
		...
		if( CoC.getInstance().player.tailType !== 0 ) {
			EngineCore.outputText( ' past your tail to slither' );
		}
		EngineCore.outputText( ' between your cheeks.' );
		if( CoC.getInstance().player.ass.analLooseness === 0 ) {
			EngineCore.outputText( ' The virginal tightness of your pucker holds the tentacle at bay at first. Still, it\'s no match for the combination of slippery plant fluid and constant pressure.' );
		} else if( CoC.getInstance().player.ass.analLooseness <= 3 ) {
			EngineCore.outputText( ' The tentacle has little problem slipping through your tight backdoor, slick as it is with fluid and backed by constant pressure.' );
		} else {
			EngineCore.outputText( ' The tentacle has little difficulty squeezing into your well-loosened ass. Even without its constant, firm pressure and slippery fluid, it could enter with ease.' );
		}
		EngineCore.outputText( ' You gasp at the odd, room-temperature feel of an intruder squirming through your rearmost passage. It\'s entirely unexpected, but once its mass slithers across your prostate, squeezing even bigger blobs of cum from your rapt, attentive body, you forgive its intrusion.' );
		...
		if( CoC.getInstance().player.balls === 0 ) {
			...
			EngineCore.outputText( ' ball' );
		}
		EngineCore.outputText( '-draining spurts of plant-food. The cradling vines squirm over your touch-heightened skin, grinding the whole of your body. It\'s like your entire being is being fucked' );
		if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( ', and once a thick, cock-shaped plant slides inside your [vagina], you truly are' );
		}
		EngineCore.outputText( '.' );
		//Fuckable nipples;
		...
		} else if( CoC.getInstance().player.isLactating() ) {
