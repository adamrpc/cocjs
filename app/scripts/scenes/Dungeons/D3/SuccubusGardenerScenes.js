...
angular.module( 'cocjs' ).run( function( SceneLib, kFLAGS, Descriptors, Utils, StatusAffects, CockTypesEnum, AppearanceDefs, Combat, EventParser, CoC, EngineCore ) {
	...
	SuccubusGardenerScenes.prototype.tentaFail = function() {
		...
		EngineCore.outputText( '. None-too-quiet slurps hang in the steamy air, accompanied by your own ' + CoC.player.mf( 'grunts', 'coos' ) + ' of enjoyment. You reach down to cup one of her breasts, ignorant of the shadows creeping nearer.' );
		...
		//Dicks;
			...
			if( CoC.player.biggestCockLength() <= 12 ) {
				...
				EngineCore.outputText( ' The ease with which you enter is no real surprise, given that even your bloated length' + ((CoC.player.cocks.length > 1) ? 's are' : ' is') + ' small by comparison.' );
			...
			if( CoC.player.biggestCockLength() <= 12 ) {
				...
				EngineCore.outputText( ' than you had thought at a glance. Not only is it the perfect size, but it seems to hug and touch every part of your maleness' + ((CoC.player.cocks.length > 1) ? 'es' : '') + ' just right.' );
			} else if( CoC.player.biggestCockLength() > 48 ) {
