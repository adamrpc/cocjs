...
angular.module( 'cocjs' ).run( function( SceneLib, MainView, kFLAGS, Descriptors, Utils, StatusAffects, CockTypesEnum, AppearanceDefs, Combat, CoC, EngineCore ) {
	...
	var GARDENER_LEFT = 1;
	var GARDENER_KILLED = 3;
	SuccubusGardenerScenes.prototype.gardenerDefeated = function() {
		return CoC.flags[ kFLAGS.D3_GARDENER_DEFEATED ] > 0;
	};
	SuccubusGardenerScenes.prototype.gardenerKilled = function() {
		return CoC.flags[ kFLAGS.D3_GARDENER_DEFEATED ] === 3;
	};
	SuccubusGardenerScenes.prototype.gardenerFucked = function() {
		return CoC.flags[ kFLAGS.D3_GARDENER_DEFEATED ] === 2;
	};
	SuccubusGardenerScenes.prototype.gardenerLeft = function() {
		return CoC.flags[ kFLAGS.D3_GARDENER_DEFEATED ] === 1;
	};
	SuccubusGardenerScenes.prototype.fuckUpTheGardener = function() {
		MainView.clearOutput();
		MainView.outputText( 'The succubus drops to her knees, grabbing a tentacle and thrusting it into wanton sex forcefully enough to make you wince. She doesn\'t seem to mind, in fact, her lips spread into a wordless \'o\' of pleasure as she begins rocking back and forth atop, lost to her own lusts. She\'s in no state to stop you from moving on. Hell, she\'s probably going to be busy with the tentacles for a long, long time. You suppose you could try and put her mouth to use, but there are a LOT of tentacles awfully close. It might be best to move on or end her quickly.' );
		EngineCore.menu();
	...
	SuccubusGardenerScenes.prototype.tentaFail = function() {
		...
		MainView.outputText( 'There\'s no way you\'re going to pass up an opportunity to put a succubus\'s lips to use. Pillowy' );
		if( CoC.player.hasVagina() ) {
			MainView.outputText( ' cunt-kissers' );
		} else {
			MainView.outputText( ' cock-suckers' );
		}
		MainView.outputText( ' like those are one in a million.' );
		if( CoC.flags[ kFLAGS.NUMBER_OF_TIMES_MET_SCYLLA ] > 0 ) {
			MainView.outputText( ' Perhaps Scylla\'s ruby mouth could give her a run for her money, but there\'s only one way to be sure.' );
		} else if( SceneLib.dominika.fellatrixSucked() > 0 ) {
			MainView.outputText( ' Perhaps Dominika\'s all-consuming mouth could give her a run for her money, but there\'s only one way to be sure.' );
		}
		MainView.outputText( ' You grab hold of the blubbering slut by her curled horns, admiring the way the tentacle-juice on her lips shines in the light, and pull her against your crotch, pressing her against' );
		if( CoC.player.hasCock() ) {
			MainView.outputText( ' [oneCock]' );
		} else if( CoC.player.hasVagina() ) {
			MainView.outputText( ' your [vagina]' );
		} else {
			MainView.outputText( ' the sensitive skin of your groin' );
		}
		MainView.outputText( '. She eagerly goes to work, moaning from the attentions of the tentacle down below, and licking with such fervor that you\'d think this is what she wanted all along.' );
		...
		if( CoC.player.hasCock() ) {
			MainView.outputText( ' blow a wad down this slut\'s throat' );
		} else if( CoC.player.hasVagina() ) {
			MainView.outputText( ' cream yourself on this slut\'s face' );
		} else {
			MainView.outputText( ' teach this bitch how to pleasure an asshole' );
		}
		MainView.outputText( '. None-too-quiet slurps hang in the steamy air, accompanied by your own ' + CoC.player.mf( 'grunts', 'coos' ) + ' of enjoyment. You reach down to cup one of her breasts, ignorant of the shadows creeping nearer.' );
		...
		if( !CoC.player.hasCock() ) {
			MainView.outputText( ' Its outline is clearly phallic, but how could such a huge thing ever fit inside you, let alone anyone?' );
		} else {
			MainView.outputText( ' Its tip oozes lubricants, but the gaping pussy at its tip is big enough to hold six minotaurs\' monster-cocks.' );
			if( CoC.player.biggestCockLength() < 36 ) {
				MainView.outputText( ' How could you ever hope to fill it?' );
			} else {
				MainView.outputText( ' How long were they preparing this for you?' );
			}
		}
		...
		if( !CoC.player.hasCock() ) // 100% gonna have a cooch by this point
		{
			MainView.outputText( '\n\nYou can feel your' );
			if( createdVag ) {
				MainView.outputText( ' new' );
			}
			MainView.outputText( ' entrance giving around the girthy monstrosity with each passing second. Wider and wider, your flesh opens up. It feels so big that you wonder if you\'re going to rip in half, but there is no discomfort, only the pleasure of accepting it\'s tumescent, inhuman length. You groan as your own fluids make themselves known, soaking your gaped lips in girlish cum. There\'s no way that you should be able to take this, let alone enjoy it, but you are. Oh gods, how you are!' );
		...
		//Dicks;
			...
			MainView.outputText( '\n\nThat monstrous, hungry-looking pussy wastes no time in descending towards your ' + Descriptors.multiCockDescriptLight() + '.' );
			if( CoC.player.biggestCockLength() <= 12 ) {
				MainView.outputText( ' The ease with which you slide inside is no surprise, given the disparity in size.' );
			} else if( CoC.player.biggestCockLength() <= 30 ) {
				MainView.outputText( ' The ease with which you enter is no real surprise, given that even your bloated length' + ((CoC.player.cocks.length > 1) ? 's are' : ' is') + ' small by comparison.' );
			} else if( CoC.player.biggestCockLength() <= 48 ) {
				MainView.outputText( ' You slide in with ease, perfectly matched to the hungry fuck-tunnel in size.' );
			} else {
				MainView.outputText( ' Sliding in must take quite the effort. The orange-hued twat is drooling around you, slowly working down an inch at a time. It takes all of you, even if it has to stretch beyond all reason to do so.' );
			}
			MainView.outputText( ' It feels good, better' );
			if( CoC.player.biggestCockLength() <= 12 ) {
				MainView.outputText( ' than a giant that looks that loose should. You\'d swear it was tight little twat from how firmly it\'s squeezing you!' );
			} else if( CoC.player.biggestCockLength() <= 30 ) {
				MainView.outputText( ' than you had thought at a glance. Not only is it the perfect size, but it seems to hug and touch every part of your maleness' + ((CoC.player.cocks.length > 1) ? 'es' : '') + ' just right.' );
			} else if( CoC.player.biggestCockLength() > 48 ) {
				MainView.outputText( ' than you would expect given how tightly-stretched it looks. You\'d think it\'d be pressing down painfully, but it feels tailor made to take you instead.' );
			}
			//Dicks need to grow;
			if( CoC.player.biggestCockLength() <= 30 ) {
				MainView.outputText( '\n\nGlowing orange goo leaks from the oversized slit in thick beads. You briefly wonder what purpose the odd coloration could serve when the feeling of your cock' + ((CoC.player.cocks.length > 1) ? 's' : '') + ' stretching answers. It\'s making you grow bigger, somehow! The sensation is akin to stretching long-dormant muscles... an subtle flexing of unrealized potential that makes you aware of just how much you can do, or in this case, how big you can get. Your vision swims as your body works to keep up with the sudden change, and you close your eyes to keep from sicking up.' );
				MainView.outputText( '\n\nThe tentacle starts sliding up and down. At first, it\'s motions are slow and languid, giving you plenty of time to feel the supple slickness of its interior texture against you, but as your mass increases, so too does the speed of its up-and-down pumping motion. Even when it\'s pushing down, it somehow maintains a gentle suction that the velvety walls are tight against you, allowing you to subsume yourself in slippery cunt. The bigger you get, the better it feels. You aren\'t sure whether you\'re simply feeling more and more pussy at once or if you\'re somehow getting more sensitive, but it\'s enough to make your eyes roll back and your ' + Descriptors.multiCockDescriptLight() + ' practically piss pre-cum.' );
				MainView.outputText( '\n\nWhimpering from more ecstasy than you know what to do with, you feel yourself surge in size again. It\'s impossible to tell just how big you\'ve gotten with your view forced to look straight ahead. All you can be sure of is that it\'s getting bigger, and you\'re feeling a LOT of pussy. By comparison, the dozens of other tentacles surrounding you are barely there. It\'s like your cock' + ((CoC.player.cocks.length > 1) ? 's are' : ' is') + ' the only part of you worth feeling, and the rest of you is just floating in a void - a sensation that strengthens with every inch of length and girth that you gain.' );
	...
	SuccubusGardenerScenes.prototype.femGenderlessLoss = function( hpVictory ) {
		MainView.clearOutput();
		MainView.outputText( 'You drop to the ground, defeated and degraded' );
		if( hpVictory ) {
			MainView.outputText( ', not even able to push your bruised body up onto its [feet].' );
		} else {
			MainView.outputText( ', too turned on to try and stop your hands from diving into your needy crotch.' );
		}
		...
		MainView.outputText( ' our factory would fall to a simple succubus and her lowly garden?"</i> She strides over to you, hips wiggling in pleasure, and kneels alongside you, running her hand over your abdomen like a girl handling a new pet for the first time. <i>"You\'re going to be so useful in my garden!"</i>' );
		MainView.outputText( '\n\nHer garden?' );
		if( !hpVictory ) {
			MainView.outputText( ' You cast a furtive glance towards the writhing vines.' );
			if( CoC.player.hasVagina() ) {
				MainView.outputText( ' Your underthings were already swampy with need, but the sight of watching those eager vines squirm elicits fresh trickles of lube to course down your [legs].' );
			} else {
				MainView.outputText( ' You were already horny, but you find yourself wishing you had a pussy, just so you could take even more of them inside you.' );
			}
		} else {
			MainView.outputText( ' You cast a worried look towards the writhing vines. They seem so eager, yet they hold back for now, perhaps waiting for their mistress to finish. No... you can\'t go down like this!' );
		}
		...
		if( CoC.player.hasVagina() ) {
			MainView.outputText( ' Against your better judgment, you\'re starting to get wet from the promises of her words. You know you\'re being given to a neverending hell of sexual stimulation, but you can\'t stop your body\'s libido from responding to the idea.' );
		}
		MainView.outputText( ' <i>"Yeah, you want it, doncha?"</i> Slim fingers deftly press at your' );
		if( !CoC.player.hasVagina() ) {
			MainView.outputText( ' bare crotch, sinking into a crease that didn\'t exist a moment before. Her skilled touches and dark powers mold your body like a sculptor\'s clay, crafting a magnificent cunt on your crotch while simultaneously bringing it to a dripping wet state.' );
			CoC.player.createVagina();
		} else {
			if( CoC.player.wetness() <= 2 ) {
				MainView.outputText( ' moistening' );
			} else {
				MainView.outputText( ' soaked' );
			}
			MainView.outputText( ' crease, and it\'s all you can do not to cry out and thrust back against her.' );
		}
		MainView.outputText( '\n\nThe succubus idly toys with your [vagina], sometimes rubbing her thumb' );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( ' across your [sack],' );
		} else if( CoC.player.hasCock() ) {
			MainView.outputText( ' across the underside of your [cock],' );
		} else {
			MainView.outputText( ' against your [clit],' );
		}
		MainView.outputText( ' confiding aloud, <i>"To be honest, I\'m a little jealous of you."</i>' );
	...
	SuccubusGardenerScenes.prototype.femGenderlessLossII = function() {
		...
		MainView.outputText( ' with foreign wetness. The vines\' lubricant is surprisingly warm on your skin, and the feeling of rivulets of it running down the crack of your [butt] and over your [asshole] is surprisingly pleasant.' );
		if( CoC.player.isBiped() ) {
			MainView.outputText( ' Your [legs] are eased open, but who are you to stop them? You\'re as wet as anything, and if spreading wide is the next step towards scratching your itch, so be it.' );
		}
		MainView.outputText( ' Your [clit] must look like a' );
		if( CoC.player.clitLength > 6 ) {
			MainView.outputText( ' glistening, cum-soaked shaft' );
		} else if( CoC.player.clitLength > 3 ) {
			MainView.outputText( ' tiny, cum-soaked cock' );
		} else if( CoC.player.clitLength > 1 ) {
			MainView.outputText( ' swollen, lube-lacquered gumball' );
		} else {
			MainView.outputText( ' glistening, pink target' );
		}
		MainView.outputText( ' by now. You roll your [hips] as much as your position will allow. Your passion inflamed pussy has thus far avoided the tentacles\' affections, though it\'s the one part that wants them more than anything.' );
		...
		if( CoC.player.hasCock() ) {
			...
			MainView.outputText( ' over the lurid squishing sounds coming from your cunt, and the two sensations combine into a riot of overwhelming sensory data. You\'re fucking and being fucked to two different, incongruent tempos and unable to make sense of it in the slightest.' );
		}
		//Back to normal.;
		MainView.outputText( '\n\nMore cum splatters across the exterior of your womanhood a moment before an enclosure envelops your [clit].' );
		if( CoC.player.clitLength > 6 ) {
			MainView.outputText( ' It stretches some around the cock-sized button' );
		} else if( CoC.player.clitLength > 3 ) {
			MainView.outputText( ' It has no trouble handling your plus-sized button' );
		} else {
			MainView.outputText( ' Your tiny button seems dwarfed by the tube enveloping it' );
		}
		MainView.outputText( ', but that doesn\'t make it feel heavenly. It gently pulsates around your buzzer, rippling and squeezing, rhythmically whipping electric lashes of sensation into your fuck-crazed body one after another.' );
		...
		if( CoC.player.biggestTitSize() > 2 ) {
			MainView.outputText( ' and jiggle your tits' );
		}
		MainView.outputText( ', pressing itself flat against your cervix before finally exploding, creaming your insides with a thick layer of molten-hot seed. You would groan or whimper if your throat wasn\'t crammed so full of plantcock. Instead, your eyes roll back under their fluttering lids, revealing a narrow band of white as you lose all control. Your body shakes like a seizure victim\'s, all except for the muscles down south. They\'re too busy clenching and squeezing the lengthy intrudor, wringing every drop of lust-imbued cum' );
		if( CoC.player.isPregnant() ) {
			MainView.outputText( ' to splatter against your occupied womb.' );
		} else {
			MainView.outputText( ' to fill your womb, to make it round with life so that others can feel what you\'re feeling.' );
		}

		MainView.outputText( '\n\nYour girlhoney squirts, splattering over the ground.' );
		if( CoC.player.wetness() < 4 ) {
			MainView.outputText( ' You may not normally be capable of such soaking orgasms, but that seems of little concern to you.' );
		}
		MainView.outputText( ' You\'re a fountain of wetness now, fit only for cumming and taking cum in. The thought triggers a second orgasm on the back of the first while the spent tentacle exits.' );
	...
	SuccubusGardenerScenes.prototype.maleLoss = function( hpVictory ) {
		...
		if( hpVictory ) {
			...
			MainView.outputText( ', too turned on to try and stop your hands from rushing to your crotch. Why bother?' );
		}
	...
	SuccubusGardenerScenes.prototype.maleLossPet = function() {
		MainView.clearOutput();
		MainView.outputText( '<i>"I\'ll be your doggie,"</i> you gasp with an accompanying needy arch of your back.' );
		...
		if( CoC.player.biggestCockLength() >= 18 ) {
			MainView.outputText( ' <i>"Oh, it\'s quite the big bone, isn\'t it? You\'re going to have an awful hard time dragging that around on all fours, [puppy]. Why, I bet the moment you see a bitch, you\'re going to be humping the ground, practically immobilized by it.' );
		} else if( CoC.player.biggestCockLength() >= 8 ) {
			MainView.outputText( ' <i>"Oh, that\'s quite the nice bone you\'ve got there, isn\'t it? Why, everyone is going to see how big you are down there, even while you\'re walking on all fours. I suppose you\'d be good for breeding the bitches' );
			if( CoC.player.cocks[ CoC.player.biggestCockIndex() ].cockType !== CockTypesEnum.DOG ) {
				MainView.outputText( ', once you have an appropriately-shaped cock' );
			}
			MainView.outputText( '.' );
		} else {
			MainView.outputText( ' Oh, that\'s quite the well-formed bone you\'ve got there, isn\'t it? I could probably take you to shows, and the judges would comment on how well it matches your form. You\'d like that, wouldn\'t you? Rolling over on your back and letting some official-looking demon measure your cock.' );
		}
		...
		MainView.outputText( '\n\n<i>"Goood ' + CoC.player.mf( 'boy', 'girl' ) + ',"</i> she coos, twisting her foot to rub her heel against your [sheath]. <i>"You\'re just so eager to cum that you\'ll do anything, won\'t you?"</i>' );
		...
		if( CoC.player.balls > 0 ) {
			MainView.outputText( ', just above your [balls],' );
		}
		MainView.outputText( ' once more, cooing, <i>"I knew you\'d train well. Now, I\'m going to lead you to your new home, and then you\'ll get to blow all that nasty, pent-up spunk all over my hand. But you have to be good until then, okay?"</i>' );
		MainView.outputText( '\n\nYou... you\'re not sure you can! You\'re so hot and so close! Pearls of clear pre are oozing out of your ' + Descriptors.multiCockDescriptLight() + ' like water from a leaky faucet, and all you\'d need to do is grab hold of her leg and just hump away.' );
		...
		if( !CoC.player.hasMuzzle() ) {
			MainView.outputText( ', heedless of the fact that you seem to be able to reach a little farther with every one you consume' );
		}
		MainView.outputText( ', slobbering some in your vigor to get more. All the while, the depression fills higher. The sound of gushing fluid tapers down to pitter-patters of the last droplets impacting the impromptu bowl, accompanied by a moan from the demon. She\'s clearly enjoyed preparing your little treat.' );
		MainView.outputText( '\n\n<i>"Eat up, doggy."</i> Her foot disappears.' );
		MainView.outputText( '\n\nYou look down at your drooling rod' );
		if( CoC.player.cocks.length > 1 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ', then at the puddle of sap-laced milk, then at your rod again, then finally back at the puddle before getting up onto all' );
		if( CoC.player.isNaga() ) {
			MainView.outputText( ' threes' );
		} else if( CoC.player.isTaur() ) {
			MainView.outputText( ' your hooves' );
		} else {
			MainView.outputText( ' fours' );
		}
		MainView.outputText( '.' );
		if( !CoC.player.isTaur() ) {
			MainView.outputText( ' You scrabble over, unusued to having to move in such a way,' );
		} else {
			MainView.outputText( ' You gallop over and drop down on your forelegs' );
		}
		MainView.outputText( ' and plant your [face] right into the succubus\' milk. It\'s a little thicker than milk normally is and stickier too, like honey, but it\'s also sweeter. The ambrosial fluid fills your mouth with scintillating, indescribable flavor that sets' );
		if( CoC.player.tailType !== 0 ) {
			MainView.outputText( ' your tail' );
			if( CoC.player.tailType === 13 && CoC.player.tailVenom > 1 ) {
				MainView.outputText( 's' );
			}
		} else {
			MainView.outputText( ' your stubby, new tail' );
		}
		MainView.outputText( ' wagging.' );
		if( CoC.player.tailType !== 2 ) {
			MainView.outputText( ' Never mind that it gets longer and fuzzier with every gulp or that your' );
		} else {
			MainView.outputText( ' Never mind that your' );
		}
		MainView.outputText( ' tongue is swelling up, wide, flat, and perfect for panting, licking, and lapping up a drink.' );
		CoC.player.tailType = 2;
		...
		if( CoC.player.cocks.length > 1 ) {
			MainView.outputText( 's thrum' );
		} else {
			MainView.outputText( ' thrums' );
		}
		MainView.outputText( ' with ready excitement. A thin trail of pre-spooge rolls out of you, trailing on the ground behind you with each happy twitch of your bitch-' );
		if( CoC.player.biggestCockLength() >= 12 ) {
			MainView.outputText( 'breaker' );
		} else {
			MainView.outputText( 'boner' );
		}
		MainView.outputText( '. You hope she doesn\'t mind, but you have a hunch that she won\'t mind you being THAT kind of messy puppy.' );
		...
		if( CoC.player.isTaur() || CoC.player.isDrider() ) {
			...
			if( CoC.player.isDrider() ) {
				MainView.outputText( ' clip-clop of your hooves' );
			} else {
				MainView.outputText( ' sharp clacks of your spidery claws' );
			}
			MainView.outputText( ' fading into the near silent pats of padded paws. Your hands are changing too fingers are shortening, but your nails are growing. Fur grows out over them, and your spine shifts, all so that you can follow on all fours that much easier.' );
		} else {
			//Nontaurs;
			MainView.outputText( '\n\nThe longer you follow, the easier it gets to stay on her heels.' );
			if( CoC.player.isGoo() ) {
				MainView.outputText( ' Your gooey undercarriage easily divides into two hind legs, slapping wetly with regularity as you plod along.' );
			} else if( CoC.player.isNaga() ) {
				MainView.outputText( ' Your slithering tail twitches, stopping you for a second, but then it splits into two limbs, each firmer and more canine than the snake-like appendage you once had. They wobble unsteadily for a second before jumping into action.' );
			} else {
				MainView.outputText( ' Your [legs] shorten as you plod along, slowing you momentarily as they change. You lift your knees up so that you can stand on your [feet] once more, only they\'re soft, padded paws now, tipped with short nails.' );
			}
			MainView.outputText( ' Your hands follow suit. First, pads form on your palms, making it easier to support your weight. Then, your fingers shorten. It\'s not entirely an unpleasant sensation, as it makes moving towards your home that much easier.' );
	...
	SuccubusGardenerScenes.prototype.maleLossPetII = function() {
		MainView.clearOutput();
		MainView.outputText( 'Your kennel is amazing! It\'s the perfect size for you to crawl into, there\'s holes in it so that you don\'t get too hot while you wait to be let out. There\'s even one big enough for you to slip' );
		if( CoC.player.cocks.length > 1 ) {
			MainView.outputText( ' a' );
		} else {
			MainView.outputText( ' your' );
		}
		MainView.outputText( ' cock through. That hole is on the back wall. You can\'t see what\'s over there, but your nose can smell pussies - some of them bitch-pussies, and lots of spilled cum. Your old mind would\'ve identified it as a glory-hole, but it\'s harder to focus on concepts like that now. ' );
		...
		if( CoC.player.skinType !== AppearanceDefs.SKIN_TYPE_FUR ) {
			MainView.outputText( ' Was that there before?' );
		}
		MainView.outputText( ' It feels very right, and in your excitement, it\'s hard not to hump her leg. It must be obvious to her, because your dick' );
		if( CoC.player.cocks.length > 1 ) {
			MainView.outputText( 's are' );
		} else {
			MainView.outputText( ' is' );
		}
		MainView.outputText( ' bobbing beneath you, and she gestures for you to roll onto your back.' );
		...
		MainView.outputText( ' trembling and spurting out ribbons of pre. Whining in the back of your throat, you look at her, hoping she can read the affection in your eyes, wanting her to see just how completely contented you are with this state of affairs.' );
		MainView.outputText( '\n\n<i>"Who\'s a good doggie?"</i>' );
		MainView.outputText( '\n\nYou bark and cum at the same time, shooting ropes all over your belly. Mistress is so nice; she keeps stroking while you shoot, encouraging her pet to shoot every drop of ' + CoC.player.mf( 'his', 'her' ) + ' sperm out. You must have been really good! Lances of white goop splatter off your chest, still somewhat human in shape, and tag your chin.' );
		if( CoC.player.cumQ() >= 2500 ) {
			MainView.outputText( ' A few thick ropes shoot past your head to cream the wall, eliciting a pleased giggle from your owner as she squeezes yet more from your length.' );
		}
		MainView.outputText( ' You yip with glee as you cream for Mistress\' fingers, but eventually, your climax trails off into a dribbly white mess, some of which clings to the demoness\' nails like beads of ivory dew.' );
	...
	SuccubusGardenerScenes.prototype.maleLossFeeder = function() {
		...
		if( CoC.player.cocks.length === 1 ) {
			...
			MainView.outputText( ' one\'s' );
		}
		MainView.outputText( ' twitching length, the succubus giggles. <i>"Can you feel it? Can you feel your body just welling up with cum? I suppose I should have mentioned that, huh?"</i> She runs her finger back the other way, and your muscles clench, squeezing out a big, fat drop of pre from your cock, dripping down to hit' );
		if( CoC.player.biggestTitSize() >= 3 ) {
			MainView.outputText( ' the underside of your breasts' );
		} else {
			MainView.outputText( ' your chin' );
		}
		MainView.outputText( '. You whimper with need. It\'s impossible not to. Hell, your ' + Descriptors.multiCockDescriptLight() );
		if( CoC.player.cocks.length === 1 ) {
			MainView.outputText( ' is' );
		} else {
			MainView.outputText( ' are' );
		}
		MainView.outputText( ' flexing, and if you had anything to brace off of, you\'d be thrusting into the first orifice you could find.' );
		...
		MainView.outputText( '\n\nA breeze caresses your [leg] as the succubus casually removes your [armor]. It feels like oiled fingertips sliding over your flesh. Your [nipples] are like live wires, and your ' + Descriptors.multiCockDescriptLight() + '.... Well, you\'re leaking pre in thick, solid ropes now, an act that feels like halfway between a good tonguing and orgasm itself. Your lips are slick with the tentacle\'s fluid and tingling like sex-organs of their own. Sealing them tight, you moan, gurgle, and swallow. Attempts to beg for more have fallen away. This is enough, and if it keeps up, you\'re going to cum, touches or no.' );
		//Prostate swelling;
		if( CoC.player.balls === 0 ) {
		...
		//Ball filling;
			...
			if( CoC.player.ballSize > 15 ) {
				MainView.outputText( ' Thankfully, a few tentacles loop under the gigantic orbs to help support them.' );
			}
		}
		MainView.outputText( '\n\nYou happily gurgle into the tentacle in your mouth and arch your back, blowing the first huge rope of your enhanced load' );
		// 9999 - Check muh;
		if( CoC.player.biggestTitSize() >= 3 ) {
			MainView.outputText( ' all over your body' );
		} else {
			MainView.outputText( ' directly onto the soil below' );
		}
	...
	SuccubusGardenerScenes.prototype.maleLossFeederII = function() {
		...
		if( CoC.player.biggestCockLength() < 6 ) {
			MainView.outputText( ' swiftly engulfs your meager offering' );
			if( CoC.player.cocks.length > 1 ) {
				MainView.outputText( 's' );
			}
		} else if( CoC.player.biggestCockLength() < 12 ) {
			MainView.outputText( ' rapidly devours your plus-sized length' );
			if( CoC.player.cocks.length > 1 ) {
				MainView.outputText( 's' );
			}
		} else if( CoC.player.biggestCockLength() < 24 ) {
			MainView.outputText( ' devours your immense offering' );
			if( CoC.player.cocks.length > 1 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ' with steady determination' );
		} else {
			MainView.outputText( ' slowly encircles your cum-spouting length' );
			if( CoC.player.cocks.length > 1 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ' with inhuman elasticity' );
		}
		MainView.outputText( '. The interior is covered with tiny, wiggling nubs that wiggle into action on contact with your trembling member' );
		if( CoC.player.cocks.length > 1 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( '. It\'s like every single nerve has its own skilled masseuse giving it a rubdown, coaxing more pleasure from every square inch of skin than you thought it capable of producing. You cry out, totally overwhelmed, only to be silenced by a familiar presense in your mouth.' );
		...
		if( CoC.player.tailType !== 0 ) {
			MainView.outputText( ' past your tail to slither' );
		}
		MainView.outputText( ' between your cheeks.' );
		if( CoC.player.ass.analLooseness === 0 ) {
			MainView.outputText( ' The virginal tightness of your pucker holds the tentacle at bay at first. Still, it\'s no match for the combination of slippery plant fluid and constant pressure.' );
		} else if( CoC.player.ass.analLooseness <= 3 ) {
			MainView.outputText( ' The tentacle has little problem slipping through your tight backdoor, slick as it is with fluid and backed by constant pressure.' );
		} else {
			MainView.outputText( ' The tentacle has little difficulty squeezing into your well-loosened ass. Even without its constant, firm pressure and slippery fluid, it could enter with ease.' );
		}
		MainView.outputText( ' You gasp at the odd, room-temperature feel of an intruder squirming through your rearmost passage. It\'s entirely unexpected, but once its mass slithers across your prostate, squeezing even bigger blobs of cum from your rapt, attentive body, you forgive its intrusion.' );
		...
		if( CoC.player.balls === 0 ) {
			...
			MainView.outputText( ' ball' );
		}
		MainView.outputText( '-draining spurts of plant-food. The cradling vines squirm over your touch-heightened skin, grinding the whole of your body. It\'s like your entire being is being fucked' );
		if( CoC.player.hasVagina() ) {
			MainView.outputText( ', and once a thick, cock-shaped plant slides inside your [vagina], you truly are' );
		}
		MainView.outputText( '.' );
		//Fuckable nipples;
		...
		} else if( CoC.player.isLactating() ) {
