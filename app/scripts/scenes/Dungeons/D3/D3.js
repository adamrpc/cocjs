...
angular.module( 'cocjs' ).run( function( MainView, SceneLib, $log, SuccubusGardener, Combat, Utils, OnLoadVariables, kFLAGS, ConsumableLib, CoC, EngineCore ) {
	...
	D3.prototype.roomofmirrorsRoomFunc = function() {
		MainView.outputText( 'The metal door opens soundlessly onto a fairly large, unlit room, shabby and grey with disuse. It is cluttered with a great quantity of mirrors. Round hand mirrors are stacked on shelves, square wall mirrors are leant against walls, a large,' );
		if( CoC.flags[ kFLAGS.D3_MIRRORS_SHATTERED ] === 1 ) {
			MainView.outputText( ' now shattered,' );
		}
		MainView.outputText( ' ornate standing mirror dominates the center of the room, and a number of broken, jagged specimens are stacked near the back. They reflect the dull trappings of this place back at you emptily. You guess as self-centred a race as the demons probably has quite a large use for these.' );
		if( CoC.player.hasKeyItem( 'Laybans' ) >= 0 ) {
			...
			if( CoC.flags[ kFLAGS.D3_ENTERED_MAGPIEHALL ] === 1 ) {
				MainView.outputText( '  Your spirits rise. They look like they may very well be made of the same material as the screen in the basilisk hall.' );
			}
			if( CoC.player.inte >= 70 || CoC.player.sens >= 70 ) {
				MainView.outputText( '  Disquiet edges down your spine. Something about this place doesn’t feel right. The room seems faded at the corners, as if it’s not quite there.' );
			}
		...
		if( CoC.flags[ kFLAGS.D3_JEAN_CLAUDE_DEFEATED ] === 0 ) {
			if( CoC.flags[ kFLAGS.D3_ENTERED_MAGPIEHALL ] === 0 ) {
				...
				if( CoC.player.canFly() ) {
					MainView.outputText( ', even if you tried to fly it' );
				}
				MainView.outputText( '. Wouldn’t you? You can’t outrun the gaze of a thousand basilisks... could you?' );
				if( CoC.player.hasKeyItem( 'Laybans' ) >= 0 ) {
					MainView.outputText( '  You take the Laybans out of your pouch and hold them up against the glass. It’s exactly as you hoped - they are made of the same material, and are almost certainly what the demons wear when they themselves interact with the basilisks. They would surely help you get across the hall, if you were crazy enough to try.' );
				}
			} else {
				MainView.outputText( 'Again you creep up to the tinted glass, again you take in the vast hall with the army of basilisks below hard at work, and again you stare out at the metal gantry, with the exit tantalizingly visible on the other side.' );
				if( CoC.player.hasKeyItem( 'Laybans' ) < 0 ) {
					MainView.outputText( '  Are you going to try this?' );
				} else {
	...
	D3.prototype.takeEgg = function( eggMask ) {
		...
		MainView.clearOutput();
		MainView.outputText( 'You pluck out ' + item.longName + ' ' );
	...
	D3.prototype.fallbackFromMagpieHallS = function() {
		MainView.clearOutput();
		MainView.outputText( 'No, there has to be a better way.' );
	...
	D3.prototype.magpiehallnRoomFunc = function() {
		...
		MainView.outputText( 'You are back in the northern end of the Magpie Hall. Without the bustle of activity below it is a gapingly empty and quiet place, the only sound the murmur of activity from elsewhere. There is a vast amount of collected junk below but it would take, well, an army of basilisks to sort through it to find anything worthwhile. You could check out the massive pile of eggs, though.' );
		if( this.eggsAvailable() > 0 ) {
			EngineCore.addButton( 2, 'Eggs', this, this.goToEggPile );
		}
		return false;
	...
	D3.prototype.southwestwalkRoomFunc = function() {
		...
		if( CoC.flags[ kFLAGS.D3_GARDENER_DEFEATED ] === 0 ) {
			...
			if( CoC.player.lust <= 75 ) {
				...
				MainView.outputText( '.... You grope yourself absentmindedly as you consider just how long you\'ll hold out before submitting. It won\'t be long.' );
			}
	...
	D3.prototype.westwalkRoomFunc = function() {
		MainView.outputText( 'Pollen clings to you, released by the many flowering bushes in the area. They only grow thicker to the south, too. To the east, you can' );
		if( CoC.flags[ kFLAGS.D3_STATUE_DEFEATED ] === 0 ) {
			MainView.outputText( ' see a massive statue with an immense hammer' );
		} else {
			MainView.outputText( ' a mound of rubble, the scattered remains of the animated statue that you slew' );
		}
		MainView.outputText( '. The warm, sandstone bricks underfoot fork in a T-intersection, leading north, east, and south. The thick castle walls prevent passage to the west.' );
		return false;
	};
	D3.prototype.northwestwalkRoomFunc = function() {
		MainView.outputText( 'A narrow path splits from the sandstone thoroughfare towards a pair of double doors to the west. The craftsmanship of the carpenter who made them is on full display; intricate designs of dragons engaged in sexual positions of all kinds are carved around the outer edges of the frame while more mundane, eye-pleasing patterns decorate the center panels. Above, a sign designates this area as the library. Unfortunately the doors are sealed closed. Perhaps the library is not yet written. You smirk at your own joke.' );
	...
	D3.prototype.southeastwalkRoomFunc = function() {
		MainView.outputText( 'Swarms of butterflies congregate on the flowering bushes here. At first, the sight seems beautiful, almost pristine. Then, you spot the endemic corruption that Lethice has spread through the lands. They aren’t just swarms of butterflies - they’re swarms of mating butterflies, crawling all over each other in a swarm of sweet-smelling pollen and fluttering wings. You had best move on. The path leads north and south.' );
		return false;
	};
