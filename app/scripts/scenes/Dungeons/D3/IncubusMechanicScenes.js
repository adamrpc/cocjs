...
angular.module( 'cocjs' ).run( function( MainView, SceneLib, StatusAffects, IncubusMechanic, PerkLib, CockTypesEnum, PregnancyStore, Descriptors, kFLAGS, CoC, EngineCore ) {
	...
	var MECHNIC_NO_SELECTION = 0;
	var MECHANIC_PAID = 1;
	var MECHANIC_SUCKED = 2;
	var MECHANIC_FOUGHT = 3;
	var MECHANIC_KILLED = 1;
	var MECHANIC_RELEASED = 2;
	...
	IncubusMechanicScenes.prototype.meetAtElevator = function() {
		if( CoC.flags[ kFLAGS.D3_MECHANIC_LAST_GREET ] === MECHNIC_NO_SELECTION ) {
			...
			if( CoC.player.gems >= 500 ) {
				EngineCore.addButton( 2, 'Pay Toll', this, this.payDaToll );
			} else {
				MainView.outputText( '\n\n<b>You do not have enough gems to pay the required toll!</b>' );
			}
			EngineCore.addButton( 3, 'Suck Dick', this, this.suckIncubusDick );
			EngineCore.addButton( 4, 'Fight', SceneLib.combatScene, SceneLib.combatScene.startCombat, new IncubusMechanic() );
			...
			EngineCore.addButton( 2, 'Lift', this, this.useLiftPostDefeat );
		}
	};
	...
	IncubusMechanicScenes.prototype.useLiftPostDefeat = function() {
		MainView.clearOutput();
		MainView.outputText( 'You spend a minute or two fiddling with the lift controls, trying to understand how, exactly, to control the contraption. When you think you\'ve got a handle on the system, you tug on a lever with authority and hastily step onto the platform.' );
	...
	IncubusMechanicScenes.prototype.suckIncubusDick = function() {
		...
		MainView.clearOutput();
		if( CoC.player.gems < 500 ) {
			MainView.outputText( 'You spare a glance to your depleted pouches and admit, "<i>I guess I\'ll suck your dick then.</i>"' );
		} else {
			MainView.outputText( 'You shrug and smile. "<i>I guess now is as good a time as any to suck some dick, huh?</i>"' );
		}
		...
		if( CoC.player.hasVagina() ) {
			MainView.outputText( ' You find your crotch becoming' );
			if( CoC.player.wetness() <= 2 ) {
				MainView.outputText( ' even more soaked than normal' );
			} else if( CoC.player.wetness() <= 4 ) {
				MainView.outputText( ' wet with fresh lust' );
			} else {
				MainView.outputText( ' sticky with lust' );
			}
			MainView.outputText( ' at his masculine scent and appearance.' );
		} else if( CoC.player.hasCock() ) {
			MainView.outputText( ' You feel a heady, turgid warmth growing in your own crotch, mirroring the ardor displayed by the cock you\'re about to service.' );
		}
		MainView.outputText( ' A patch of neatly trimmed pubic hairs lead up his abdomen to a set well-defined muscles, half-hidden under his loose not-so-coveralls.' );
	...
	IncubusMechanicScenes.prototype.beatDaMechanic = function( hpVictory ) {
		...
		MainView.clearOutput();
		MainView.outputText( 'The mechanic collapses, his overalls' );
		if( hpVictory ) {
			MainView.outputText( ' falling off his prone from, shredded.' );
		} else {
		...
		if( CoC.player.hasCock() ) {
			EngineCore.addButton( 2, 'Buttfuck', this, this.buttfuckTheMechanic, hpVictory );
		}
		if( CoC.player.hasVagina() ) {
			EngineCore.addButton( 3, 'Ride Cock', this, this.rideMechanicsCock );
		}
	};
	IncubusMechanicScenes.prototype.killMechanic = function() {
		CoC.flags[ kFLAGS.D3_MECHANIC_FIGHT_RESULT ] = MECHANIC_KILLED;
		MainView.clearOutput();
		MainView.outputText( 'He proves blessedly easy to kill, and you roll the body off the cliffs to avoid alerting any of Lethice\'s other ilk.' );
	...
	IncubusMechanicScenes.prototype.buttfuckTheMechanic = function( hpVictory ) {
		...
		if( !hpVictory ) {
			MainView.outputText( ' throbbing hard' );
		} else {
			MainView.outputText( ' gently bobbing' );
		}
		MainView.outputText( ' dick, but you\'ve got a better idea. You got more than one look at the curvature of his butt while you were fighting, tempting your burgeoning bulge to the point of distraction. You hastily remove your [armor] and smile. [EachCock] springs up, full and hard. You take' );
		if( CoC.player.cocks.length > 1 ) {
			MainView.outputText( ' one' );
		} else {
			MainView.outputText( ' it' );
		}
		MainView.outputText( ' in hand, pumping slowly as you eye your tainted prey. He whimpers' );
		if( hpVictory ) {
			MainView.outputText( ' in need, and' );
		} else {
			MainView.outputText( ', but' );
		}
		MainView.outputText( ' his cock seems to get even bigger and harder. The poor guy doesn\'t realize what a reaming he\'s in for.' );
		...
		if( CoC.player.cumQ() <= 250 ) {
			...
			if( CoC.player.hasKnot( x ) ) {
				MainView.outputText( ', popping your knot' );
			}
			MainView.outputText( ' out after depositing your meager load into your demonic fuck-hole. One slick strand of creamy spunk is leaking out over the demon\'s ballsack.' );
		}
		//Medium cum exit;
			...
			MainView.outputText( ' out of the demon\'s creampied fuck-hole with a smile. His stretched-out ass looks virtually glazed with cum, and you can see small rivers of it trickling down over his balls and slowly shrinking cock.' );
		}
		//High cum exit;
			...
			MainView.outputText( ' out of the demon\'s well-fucked asshole with a smile. As soon as the bulk of your dick unplugs him, a river of white spills out, washing over his balls and cock to drip down into his puddle. You smirk at the white-glazed demon. His butt is still full of your spunk.' );
		}
		//Huge cum;
		else {
			MainView.outputText( '\n\nYou pull' );
			...
			MainView.outputText( ' out after depositing a massive, belly-bulging load directly into the demon\'s guts. A tidal wave of goo bursts out of his over-pressurized asshole to fall on the ground, and as it dies off, small rivers of ivory spunk escape the lake in his ass to run down his balls and cock. His belly is' );
			if( CoC.player.cumQ() <= 5000 ) {
				MainView.outputText( ' pudgy' );
			} else if( CoC.player.cumQ() <= 10000 ) {
				MainView.outputText( ' swollen' );
			} else {
				MainView.outputText( ' absolutely gravid' );
			}
			MainView.outputText( ' with the distended weight of the cum packing his body. From time to time, fresh streamers of sperm squirt out of his thoroughly cream-pied backdoor.' );
		}
		//No new paragraph;
		MainView.outputText( 'The incubus slumps down into the puddled semen, gasping in exhaustion.' );
	...
	IncubusMechanicScenes.prototype.rideMechanicsCockII = function( cType ) {
		...
		//Dogdick;
			...
			MainView.outputText( 'Giving the incubus\'s sweaty tool an affectionate squeeze, you decide, "<i>Grow a dog-cock. Let\'s see if I can take the knot, shall we, pet?</i>"' );
		...
		if( CoC.player.wetness() <= 2 ) {
			MainView.outputText( ', letting your free-flowing pussyjuice wash over it, lubricating it with your glorious girlcum' );
		} else if( CoC.player.wetness() <= 3 ) {
			MainView.outputText( ', letting your oh-so-juicy vagina drip and dribble, soaking it with your copious girlcum' );
		} else {
			MainView.outputText( ', letting your lubricant drip over it' );
		}
		MainView.outputText( '. You stuff two fingers inside and stroke deep before pulling them out and using them to spread you wide open. The ' + CoC.monster.cockDescriptShort() + ' pulsates with every beat of its demonic owner\'s heart, needing you every bit as much as you now need it.' );
		...
		if( tightFit ) {
			MainView.outputText( ' well-stretched' );
		} else {
			MainView.outputText( ' stuffed' );
		}
		...
		if( CoC.player.clitLength >= 5 ) {
			MainView.outputText( ' You grab hold of it in your hand and start to stroke it to the tempo your rising and falling body is setting, pumping the oversensitive organ like a dick. Every stroke sends your walls into fits of fluttering ecstasy. It feels so good that you don\'t think you\'ll be able to stop until you\'re cumming and gushing all over your demonic lover.' );
		} else {
			MainView.outputText( ' You thumb at it, rubbing in slow circles around it. Sometimes you push on its hood. Other times you oh-so-gently squeeze at it. It feels so good that the touches drive you crazy with ecstasy, and your walls flutter excitedly around your demonic lover.' );
		}
		MainView.outputText( ' His only reaction is to gasp and groan, balls lurching in his sack.' );
		...
		if( cType === MECHANIC_HORZ_GOG ) {
			MainView.outputText( ', the head flaring to seal itself against your cervix' );
		} else if( cType === MECHANIC_DOG_COCK ) {
			MainView.outputText( ', the knot popping in just in time to start bloating, locking you in place' );
		}
		MainView.outputText( ', and jism begins to shoot out in waves.' );
		if( !CoC.player.isPregnant() ) {
			MainView.outputText( ' Your empty womb drinks up the virile, seed-filled sludge with abandon.' );
			if( cType === MECHANIC_HORZ_GOG ) {
				MainView.outputText( ' After all, that bestial horse-cock doesn\'t really leave it anywhere else to go. The demonic animal-cunt just continues spurting straight on in to have its way with your eggs, and you\'re sure there isn\'t a contraceptive this side of Mareth that could stop you from having his imps.' );
			} else if( cType === MECHANIC_DOG_COCK ) {
				MainView.outputText( ' After all, that immense knot doesn\'t really leave it anywhere else to go. Once every fold is bathed in alabaster doggie-cream, the continued ejaculations start pouring in after your eggs. His knot is so big and stretching you so wide; you don\'t think he would pop out even if he filled your belly to look nine months pregnant. Worse, or perhaps better, you\'re sure with how thick and virile his seed feels that you\'ll be getting pregnant with his imps from this.' );
			}
			CoC.player.knockUp( PregnancyStore.PREGNANCY_IMP, PregnancyStore.INCUBATION_IMP );
		} else {
			MainView.outputText( ' Your blocked womb deflects the seed, forcing it all to stay in your channel' );
			if( cType !== MECHANIC_DOG_COCK ) {
				MainView.outputText( ' and drip out over his balls, much to his chagrin.' );
			} else {
				MainView.outputText( ' and stretch you further.' );
			}
		}
		...
		if( CoC.player.wetness() < 4 ) {
			MainView.outputText( ', even though you aren\'t normally a squirter' );
		}
		MainView.outputText( '. The pleasure is so great that you can feel electric impulses arcing between neurons, short-circuiting your throughs into involuntary ecstasy. Your eyes roll back, and your [hips] twitch and shake, vibrating around the object that brough you to this perfect' );
		if( !CoC.player.isPregnant() ) {
			MainView.outputText( ' insemination' );
		} else {
			MainView.outputText( ' tryst' );
		}
		MainView.outputText( '.' );
	...
	IncubusMechanicScenes.prototype.maleLossToMechanic = function( hpVictory ) {
		...
		if( !hpVictory ) {
			...
			//Medium cocks;
				...
				MainView.outputText( '"<i>You\'re so... average. I suppose you\'ll do, if the succubi feel like having a drab, everyday orgasm.</i>" The incubus shrugs. "<i>A dick will do...</i>"' );
			}
			//Big cocks;
			...
			//Huge cocks;
				...
				MainView.outputText( '"<i>Seriously?</i>" the incubus asks. "<i>How am I supposed to work with something so gargantuan?</i>" He disdainfully pulls a vial from his pocket and upends it over your crotch. The effect is immediate. [Eachcock] shrinks from ridiculously large down to horse-sized. <i>"There, that\'s better."</i>' );
			...
			MainView.outputText( ' length inside any hole and thrust until you\'re quivering in orgasmic release. That gorgeous demon-cock taunts you, swaying just out of reach.' );
		}
		//HP Loss;
		else {
			MainView.outputText( 'Dropping prone, you gasp for breath, struggling against your injured body to rise. Your muscles burn with the effort, exhausted from the fight, and ultimately, you fail to stand. Slumping down on the ground, you\'re forced to come to terms with your defeat. The incubus has won. You came all this way, gave your best, and got smacked down by a lower demon for all your troubles. Some champion you are.' );
		...
		if( CoC.player.cocks.length > 1 ) {
			...
			if( !allDemon ) {
				MainView.outputText( ' The skin of your transforming length is darkening to a purplish hue, and in the wake of the magical cock-ring, nodules are slowly emerging, growing into an imitation of the cock you\'re still suckling.' );
			}
			MainView.outputText( ' The incubus chuckles, easily pushing the ring the rest of the way down, smiling at the way your demonic tip pulsates and leaks pre-cum. Once it\'s nestled firmly against your groin, your new master withdraws his hands (and his cock). <b>Your singular, demonic cock is impossibly hard.</b>' );
		} else {
			//Single;
	...
	IncubusMechanicScenes.prototype.maleLossToMechanicII = function() {
		MainView.clearOutput();
		MainView.outputText( 'An agonizingly perfect pussy is suddenly pressing down on your cock, subsuming it in slick, hot depths that cling and squeeze with undulations so skillful you\'d be cumming already if it weren\'t for the magic ring.' );
	...
	IncubusMechanicScenes.prototype.maleLossToMechanicIII = function() {
		MainView.clearOutput();
		MainView.outputText( 'The champion finally got to cum once Lethice had her turn, but by that point, pussy was all ' + CoC.player.mf( 'he', 'she' ) + ' could think about. Release came more frequently with the demon queen\'s permission. It didn\'t really matter, though. The champion was addicted to riding on the edge of climax, broken into nothing more than a demonic vibrator.' );
		EngineCore.gameOver();
	};
	...
	IncubusMechanicScenes.prototype.errybodyelseLossToMechanic = function( hpVictory ) {
		...
		//Genderless HP Loss;
			...
			MainView.outputText( 'Collapsing onto the ground, you look up at the demonic aggressor with hate in your eyes, unwilling to admit defeat even when staring full-on into its face. The incubus, for his part, simply smiles down at you as he steps closer, claws clicking noisily on the smooth, tiled pavement.' );
			...
			if( CoC.player.hasFuckableNipples() ) {
				MainView.outputText( ' slipping inside a [nipple] to thrust just like your other hand. You work your upper and lower pussies like a woman possessed.' );
			} else {
				MainView.outputText( ' pinching and tugging on' );
				if( CoC.player.totalNipples() > CoC.player.totalBreasts() ) {
					MainView.outputText( ' nipple after nipple, your myriad breastly endowments providing you with oh so many buttons to tend to' );
				} else {
					MainView.outputText( 'your [nipples] one after another' );
				}
			}
			MainView.outputText( ' Alas, all good things must come to an end, and your lovely, lovely little lollipop is rudely pulled from your questing lips with a noisy pop.' );
		...
		if( CoC.player.isNaga() ) {
			MainView.outputText( 'r coils don\'t get sore' );
		} else if( !CoC.player.isBiped() ) {
			MainView.outputText( ' don\'t even need to support yourself anymore' );
		} else {
			MainView.outputText( ' knees don\'t get sore' );
		}
		MainView.outputText( '. How wonderful! If you move just right, you can make the whole contraption sway slightly to get a proper dick-sucking rhythm going. His pearly pre-cream oozes onto your tongue, and you swallow as much of it as you can.' );
		...
		if( CoC.player.clitLength > 5 ) {
			MainView.outputText( ', even if that makes it truly monstrous indeed' );
		}
		MainView.outputText( '. It drips long, sticky strands of girlish lust as you swallow, only becoming more soaked as time passes as your body becomes even more productive. A plaintive whimper escapes around the edges of your phallic gag, but your cry falls on deaf ears. The incubus is too busy fitting pipes together beneath you to care.' );
