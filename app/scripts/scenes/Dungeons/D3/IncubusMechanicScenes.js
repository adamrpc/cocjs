...
angular.module( 'cocjs' ).run( function( MainView, SceneLib, StatusAffects, IncubusMechanic, PerkLib, CockTypesEnum, PregnancyStore, Descriptors, Combat, kFLAGS, CoC, EngineCore ) {
	...
	IncubusMechanicScenes.prototype.beatDaMechanic = function( hpVictory ) {
		...
		if( hpVictory ) {
			...
			EngineCore.outputText( ' hanging open as he reaches down for his exposed, dangling cock to wildly masturbate.' );
		}
		EngineCore.outputText( ' He seems surprisingly human like that, just as mortal as anyone. You could let him go, play with him some, or cleanse the world of another demon.' );
		EngineCore.menu();
	...
	IncubusMechanicScenes.prototype.letMechanicGo = function( hpVictory ) {
		...
		EngineCore.outputText( '\n\nThe incubus' );
		if( hpVictory ) {
			EngineCore.outputText( ' slowly struggles to his feet' );
		} else {
			EngineCore.outputText( ' reluctantly stops masturbating' );
		}
	...
	IncubusMechanicScenes.prototype.buttfuckTheMechanic = function( hpVictory ) {
		...
		EngineCore.outputText( ' in hand, pumping slowly as you eye your tainted prey. He whimpers' );
		if( hpVictory ) {
			EngineCore.outputText( ' in need, and' );
		} else {
			EngineCore.outputText( ', but' );
		}
		EngineCore.outputText( ' his cock seems to get even bigger and harder. The poor guy doesn\'t realize what a reaming he\'s in for.' );
		...
		if( EngineCore.silly() ) {
			EngineCore.outputText( ' tab C with slot buttfuck' );
		} else {
		...
		//Huge cum;
			...
			EngineCore.outputText( ' with the distended weight of the cum packing his body. From time to time, fresh streamers of sperm squirt out of his thoroughly cream-pied backdoor.' );
		}
		//No new paragraph;
	...
	IncubusMechanicScenes.prototype.rideMechanicsCockII = function( cType ) {
		...
		//Demon first:;
			...
			EngineCore.outputText( 'Giving the incubus\'s sweaty tool an affectionate squeeze, you decide, "<i>Make it a horse-cock. If I\'m going to play with you, I want you to have the biggest, strongest, most inhumanly virile cock to put inside me.</i>" He starts twitching and leaking pre-cum halfway through your explanation.' );
		...
		//Dogdick;
			...
			EngineCore.outputText( 'Giving the incubus\'s sweaty tool an affectionate squeeze, you decide, "<i>Grow a dog-cock. Let\'s see if I can take the knot, shall we, pet?</i>"' );
		...
		EngineCore.outputText( ' His only reaction is to gasp and groan, balls lurching in his sack.' );
		...
		if( cType === MECHANIC_HORZ_GOG ) {
			EngineCore.outputText( ', the head flaring to seal itself against your cervix' );
		} else if( cType === MECHANIC_DOG_COCK ) {
		...
		if( !CoC.player.isPregnant() ) {
			...
			if( cType !== MECHANIC_DOG_COCK ) {
				EngineCore.outputText( ' and drip out over his balls, much to his chagrin.' );
			} else {
	...
	IncubusMechanicScenes.prototype.maleLossToMechanic = function( hpVictory ) {
		...
		if( !hpVictory ) {
			...
			EngineCore.outputText( ' length inside any hole and thrust until you\'re quivering in orgasmic release. That gorgeous demon-cock taunts you, swaying just out of reach.' );
		}
		//HP Loss;
		else {
		...
		if( CoC.player.cocks.length > 1 ) {
			...
			if( !allDemon ) {
				EngineCore.outputText( ' The skin of your transforming length is darkening to a purplish hue, and in the wake of the magical cock-ring, nodules are slowly emerging, growing into an imitation of the cock you\'re still suckling.' );
			}
			EngineCore.outputText( ' The incubus chuckles, easily pushing the ring the rest of the way down, smiling at the way your demonic tip pulsates and leaks pre-cum. Once it\'s nestled firmly against your groin, your new master withdraws his hands (and his cock). <b>Your singular, demonic cock is impossibly hard.</b>' );
		} else {
			//Single;
	...
	IncubusMechanicScenes.prototype.errybodyelseLossToMechanic = function( hpVictory ) {
		...
		//Genderless HP Loss;
			...
			EngineCore.outputText( ' Alas, all good things must come to an end, and your lovely, lovely little lollipop is rudely pulled from your questing lips with a noisy pop.' );
		...
		EngineCore.outputText( '. How wonderful! If you move just right, you can make the whole contraption sway slightly to get a proper dick-sucking rhythm going. His pearly pre-cream oozes onto your tongue, and you swallow as much of it as you can.' );
