...
angular.module( 'cocjs' ).run( function( SceneLib, MainView, Utils, PregnancyStore, Descriptors, HermCentaur, Combat, kFLAGS, CoC, EngineCore ) {
	...
	HermCentaurScenes.prototype.beatThePony = function( hpVictory ) {
		...
		MainView.outputText( 'With a heavy \'thud\', the demonic red-head slumps onto her side in the grass.  Her tail flails fitfully' );
		if( hpVictory ) {
			MainView.outputText( ' as she tries to rise' );
		} else {
			MainView.outputText( ' as she tries to stretch for her pulsating, cum-dripping member' );
		}
		MainView.outputText( ', but it\'s a futile gesture.  She doesn\'t have the power to face you on her own terms anymore, and she watches you with a' );
		if( hpVictory ) {
			MainView.outputText( ' resigned glare' );
		} else {
			MainView.outputText( ' lustful glare' );
		}
		MainView.outputText( ' once she realizes she\'s utterly spent.' );
	...
	HermCentaurScenes.prototype.maleFuckHer = function() {
		...
		if( CoC.player.cumQ() >= 1000 ) {
			MainView.outputText( ' You quake at the sensation of dumping so much of your creamy batter into her stomach, nearly closing your eyes in bliss.  Luckily, you hold onto your wits.' );
		}
		if( CoC.player.cumQ() >= 5000 ) {
			MainView.outputText( ' It doesn\'t make much difference to the demon beneath you, as her belly slowly rounds from the obscene deposit you\'re plugging her with.' );
		}
		if( CoC.player.cumQ() >= 15000 ) {
			MainView.outputText( ' Bigger and bigger, she bloats until she looks fat or pregnant, jiggling with your thick spooge.' );
		}
		...
		if( CoC.player.cumQ() >= 15000 ) {
			MainView.outputText( ', practically dragging her gut on the stones as she runs' );
		}
	...
	HermCentaurScenes.prototype.femFuckHerII = function( cockType ) {
		...
		if( cockType === TENTACOCK ) {
			MainView.outputText( 'You give her throbbing maleness an affection pat and proclaim, "<i>Make it a tentacle, one that will fit me properly.  Then we can fuck.</i>"' );
			...
			if( CoC.player.clitLength >= 6 ) {
				MainView.outputText( ' A coil of cock loops around your gigantic buzzer, effectively frotting it like another cock and chance at protest from your lips.' );
			} else {
				MainView.outputText( ' A bend in the cock grinds against your buzzer with particular intensity, effectively robbing any chance at protest from your lips.' );
			}
			...
			if( CoC.player.clitLength >= 6 ) {
				MainView.outputText( ' - you swear it spends as much time caressing your [clit] as squeezing it off' );
			}
			MainView.outputText( '.  You buck your hips, dragging the butt-piercing wang along for the ride as you start to fuck in earnest.' );
			...
			MainView.outputText( 'You give her throbbing maleness an affection pat and proclaim, "<i>Make it a dog-cock, one that fit to knot properly.  Then we can fuck.</i>"' );
			...
			MainView.outputText( '  The pleasure of being so fully stretched is enough to overwhelm any pain at such an act, and you switch over to smaller, faster strokes the leave most of the animal-cock inside you.  Bouncing atop the centaur\'s balls with a dopey grin on your [face], you sigh and moan.  This is how you\'re going to take down these demons, one fuck at a time!  The thought gives your ' + CoC.player.skin() + ' a fresh flush, and you pinch your nipple while imaging mounting a demon queen atop an orgy of her defeated lieutenants.' );
			...
			MainView.outputText( 'You give her throbbing maleness an affection pat and proclaim, "<i>Keep it equine, the flare looks delicious.  Just make sure that it will fit, and then we can fuck.</i>"' );
			...
			if( CoC.player.clitLength >= 6 ) {
				MainView.outputText( ' The hard, feminine pole twitches nicely at your touch, a far more sensitive rod than any cock.' );
			}
			MainView.outputText( ' Your fingers sink into your gash with ease and set to work with ease.  Of course, they\'re soaked in a second.' );
	...
	HermCentaurScenes.prototype.femLoss = function( hpVictory ) {
		MainView.clearOutput();
		MainView.outputText( 'Dropping down, you' );
		if( hpVictory ) {
			MainView.outputText( ' try to pull yourself up with on your palms, utterly defeated in spite of the centaur\'s complete lack of physical attacks.' );
		} else {
			MainView.outputText( 'put your rump up in the air, fully exposing your' );
			if( CoC.player.hasVagina() ) {
			...
			MainView.outputText( ' to the pervy, sexy equine.' );
		}
		MainView.outputText( ' She smirks as she trots up, commenting, "<i>That\'s more like it, slut.  I don\'t know how you got this far, but your quest ends here.</i>"  She casually reaches down and grabs you by the scruff of the neck, hauling you bodily up until your head nestles between her tits' );
		if( CoC.player.tallness >= 7 * 12 ) {
			MainView.outputText( ', even though your [feet] still touch the ground' );
		}
		MainView.outputText( '.  Brushing through your hair, a hand gently caresses your back as its owner coos, "<i>You\'ll make a fine mare for my stable, but lets get you in the right mindset first...</i>"' );
		...
		MainView.outputText( 'Your [vagina] is so puffy and inflamed with passion that it feels ready to burst out of your [armor], even as the drooling fem-cum makes it squish wetly with your every motion.' );
		if( CoC.player.hasCock() ) {
			MainView.outputText( ' [EachCock] fares little better, a turgid mess of pre-cum leaking lust that seems to grow harder and more sensitive by the moment.' );
		}
		MainView.outputText( ' That wonderful warmth excites you sexually, a deepening puddle of lust in your loins that makes you so hot and wet that you swear it must be what\'s dripping out onto the cobble below.' );
	...
	HermCentaurScenes.prototype.femLossIII = function() {
		MainView.clearOutput();
		MainView.outputText( '<b>Months later...</b>' );
	...
	HermCentaurScenes.prototype.maleLoss = function( hpVictory ) {
		...
		if( hpVictory ) {
			...
			MainView.outputText( ' try to pull [oneCock] out of your [armor] in an attempt to masturbate.  You\'re beaten, you just want to fuck or be fucked until you\'re both covered in a sheen of lady-spunk or your own thick cum.' );
		}
		MainView.outputText( ' She smirks as she trots up, commenting, "<i>That\'s more like it, slut.  I don\'t know how you got so far thinking you were some kind of ' + CoC.player.mf( 'stud', 'queen bitch' ) + '.</i>"  She casually reaches down and grabs you by the scruff of the neck, hauling you bodily up until your head nestles between her tits' );
		if( CoC.player.tallness >= 7 * 12 ) {
			MainView.outputText( ', even though your [feet] still touch the ground' );
		}
		MainView.outputText( '.  Brushing through your hair, a hand gently caresses your back as its owner coos, "<i>I\'m not sure you have what it takes to be one of my stallions...</i>"' );
		MainView.outputText( '\n\nYou\'re shifted to the side, your face dragged along the side of a pale, freckled breast as you\'re moved into position.  A thick nipple, almost as wide as your finger, pushes at your lips, smelling faintly of sugary sweetness and sex all blended together.  When you don\'t gratefully accept it, your jaw is forced open and the nearly inch-long teat is firmly inserted.  As soon as it\'s seated in your maw, the hard milk-nozzle immediately begins to leak its tasty treat - a sweet fluid that seems to fizz on your tongue and light your mouth up with pleasure.  You savor the taste as long as you can, but pleasure-inducing juice soon has your cheeks bulging.  You gulp it down reluctantly, if only to make room for more.' );
		MainView.outputText( '\n\nYour eyes open wide when the delicious demon-milk hits your stomach.  It fills your core with such warmth that you think you might melt, at least until the heat begins to diffuse outward, bringing with it a tingling sensitivity that makes the tit in your mouth taste that much better.  You draw deeply on the rosy, pink nipple, sucking down even more \'milk\' with every passing second, if only to kindle even more of that pleasant sensation.  The hands release you, but you stay firmly attached to your teat.  You need more.' );
		...
		if( CoC.player.hasVagina() ) {
			MainView.outputText( ' Your [vagina] fares no better, puffing up and drooling like its big brother while your [clit] grows taut and sensitive.' );
		}
		MainView.outputText( ' That wonderful warmth excites you sexually, moulding your loins into a simmering blaze of desire just looking for a something to cool its ache.' );
		...
		if( CoC.player.balls > 0 ) {
			MainView.outputText( ' [balls] feel fit to burst!' );
		} else {
			MainView.outputText( ' manhood feels swollen to bursting!' );
		}
		MainView.outputText( '  Realizing the only way out will be with your tongue, you open wide' );
		if( CoC.player.tongueType !== 0 ) {
			MainView.outputText( ' and uncoil your inhuman length to spear' );
		} else {
			MainView.outputText( ' and begin to lap' );
		}
		MainView.outputText( ' at her inhuman entrance.  She whinnies in delight, grinding her clydesdale-sized bulk back and forth on your face as you start to get into her eating her out.' );
		...
		if( CoC.flags[ kFLAGS.D1_OMNIBUS_KILLED ] !== 1 ) {
			MainView.outputText( ', and a familiar one at that' );
		}
		MainView.outputText( '.  She wears a black reflective bra, though you swear you can see pink protrusions under the shiny fabric, wriggling around constantly.  As expected with such dress, her nipples are hard as bullets.  A bulbous, ten-inch penis bobs between her thighs, thickening nicely as it fills to its full size, kissing your lips with a moist bead of pre as it fully erects.  The omnibus' );
		if( CoC.flags[ kFLAGS.D1_OMNIBUS_KILLED ] !== 1 ) {
			MainView.outputText( ' overseer asks, "<i>Remember me?  I thought it only proper that I thank you for letting me live.  Now, be a good boy and suck my dick - show all everyone just what a horny, helplessly fuckable toy you are.</i>"' );
		} else {
			MainView.outputText( ' asks, "<i>I hope you don\'t mind slurping down a little cock, because you\'re going to anyway.</i>"' );
		}

		MainView.outputText( '\n\nThe second herm grabs you by the' );
		if( CoC.player.hornType !== 0 ) {
			MainView.outputText( ' horns' );
		} else {
			MainView.outputText( ' head' );
		}
		MainView.outputText( ' and sidles forward, ramming her glans into your mouth and down the back of your throat.  You can feel her glans pulsing deep inside you, and as soon as the herm starts moaning, the centaur starts trotting forward again.  Amazingly, having her holding you up makes it easy to maintain your position and keep your dick bouncing in that hot twat. Fuck it! You\'re too horny to care anymore.' );
