...
angular.module( 'cocjs' ).run( function( Utils, PregnancyStore, Descriptors, HermCentaur, Combat, EventParser, kFLAGS, CoC, EngineCore ) {
	...
	HermCentaurScenes.prototype.encounterThePony = function() {
		...
		if( CoC.getInstance().player.vaginalCapacity() < 100 || CoC.getInstance().player.analCapacity() < 100 ) {
	...
	var CENTAUR_FUCKED = 3;
	HermCentaurScenes.prototype.beatThePony = function( hpVictory ) {
		EngineCore.clearOutput();
		EngineCore.outputText( 'With a heavy \'thud\', the demonic red-head slumps onto her side in the grass.  Her tail flails fitfully' );
		if( hpVictory ) {
			EngineCore.outputText( ' as she tries to rise' );
		} else {
			EngineCore.outputText( ' as she tries to stretch for her pulsating, cum-dripping member' );
		}
		EngineCore.outputText( ', but it\'s a futile gesture.  She doesn\'t have the power to face you on her own terms anymore, and she watches you with a' );
		if( hpVictory ) {
			EngineCore.outputText( ' resigned glare' );
		} else {
			EngineCore.outputText( ' lustful glare' );
		}
		EngineCore.outputText( ' once she realizes she\'s utterly spent.' );
	...
	HermCentaurScenes.prototype.maleFuckHer = function() {
		...
		var y = CoC.getInstance().player.cockThatFits( CoC.getInstance().monster.vaginalCapacity() );
		if( y < 0 ) {
			y = CoC.getInstance().player.smallestCockIndex();
		}

		EngineCore.clearOutput();
		EngineCore.outputText( 'You shrug out of your [armor] as you close with the defeated beast-woman.  Looking at her like this, she almost seems more animal than woman.  Her hundreds of pounds of muscular equine flesh twist and contort as you approach, shifting her bulk to allow you unimpeded access to either of her trembling, moist genitals.  Glancing up to her pale, freckled face, you trap her green eyes with your own and say, "<i>' + ((CoC.getInstance().player.cor > 50) ? 'I\'m gonna wreck it!' : 'We do this my way.') + '</i>"' );
		...
		if( CoC.getInstance().player.cumQ() >= 1000 ) {
			EngineCore.outputText( ' You quake at the sensation of dumping so much of your creamy batter into her stomach, nearly closing your eyes in bliss.  Luckily, you hold onto your wits.' );
		}
		if( CoC.getInstance().player.cumQ() >= 5000 ) {
			EngineCore.outputText( ' It doesn\'t make much difference to the demon beneath you, as her belly slowly rounds from the obscene deposit you\'re plugging her with.' );
		}
		if( CoC.getInstance().player.cumQ() >= 15000 ) {
			EngineCore.outputText( ' Bigger and bigger, she bloats until she looks fat or pregnant, jiggling with your thick spooge.' );
		}
		...
		if( CoC.getInstance().player.cumQ() >= 15000 ) {
			EngineCore.outputText( ', practically dragging her gut on the stones as she runs' );
		}
	...
	HermCentaurScenes.prototype.femFuckHerII = function( cockType ) {
		...
		if( cockType === TENTACOCK ) {
			EngineCore.outputText( 'You give her throbbing maleness an affection pat and proclaim, "<i>Make it a tentacle, one that will fit me properly.  Then we can fuck.</i>"' );
			...
			EngineCore.outputText( '\n\nThe demon-vine immediately thrusts forward, but in its eagerness, it misses your [vagina] entirely, instead grinding along your inner thighs.  You get a steady buzz of pleasure from the contact that makes your twat wet and your body shiver.  Involuntarily, you start to rock back and forth on it, working your soaked lips over the snake-like shaft until your [clit] is hard and poking free.  A plaintive voice whines, "<i>I can\'t... -AH! 0...can\'t control it!  Lethice\'s tits this is good... I might have to grow some more...</i>"' );
			if( CoC.getInstance().player.clitLength >= 6 ) {
				EngineCore.outputText( ' A coil of cock loops around your gigantic buzzer, effectively frotting it like another cock and chance at protest from your lips.' );
			} else {
				EngineCore.outputText( ' A bend in the cock grinds against your buzzer with particular intensity, effectively robbing any chance at protest from your lips.' );
			}
			...
			CoC.getInstance().player.buttChange( CoC.getInstance().player.analCapacity(), true, true, false );
			...
			if( CoC.getInstance().player.clitLength >= 6 ) {
				EngineCore.outputText( ' - you swear it spends as much time caressing your [clit] as squeezing it off' );
			}
			EngineCore.outputText( '.  You buck your hips, dragging the butt-piercing wang along for the ride as you start to fuck in earnest.' );
			CoC.getInstance().player.cuntChange( CoC.getInstance().player.vaginalCapacity(), true, true, false );
			...
			EngineCore.outputText( 'You give her throbbing maleness an affection pat and proclaim, "<i>Make it a dog-cock, one that fit to knot properly.  Then we can fuck.</i>"' );
			...
			CoC.getInstance().player.cuntChange( CoC.getInstance().player.vaginalCapacity() * 1.2, true, true, false );
			EngineCore.outputText( '  The pleasure of being so fully stretched is enough to overwhelm any pain at such an act, and you switch over to smaller, faster strokes the leave most of the animal-cock inside you.  Bouncing atop the centaur\'s balls with a dopey grin on your [face], you sigh and moan.  This is how you\'re going to take down these demons, one fuck at a time!  The thought gives your ' + CoC.getInstance().player.skin() + ' a fresh flush, and you pinch your nipple while imaging mounting a demon queen atop an orgy of her defeated lieutenants.' );
			...
			EngineCore.outputText( 'You give her throbbing maleness an affection pat and proclaim, "<i>Keep it equine, the flare looks delicious.  Just make sure that it will fit, and then we can fuck.</i>"' );
			...
			if( CoC.getInstance().player.clitLength >= 6 ) {
				EngineCore.outputText( ' The hard, feminine pole twitches nicely at your touch, a far more sensitive rod than any cock.' );
			}
			EngineCore.outputText( ' Your fingers sink into your gash with ease and set to work with ease.  Of course, they\'re soaked in a second.' );
	...
	HermCentaurScenes.prototype.femLoss = function( hpVictory ) {
		EngineCore.clearOutput();
		EngineCore.outputText( 'Dropping down, you' );
		if( hpVictory ) {
			EngineCore.outputText( ' try to pull yourself up with on your palms, utterly defeated in spite of the centaur\'s complete lack of physical attacks.' );
		} else {
			EngineCore.outputText( 'put your rump up in the air, fully exposing your' );
			if( CoC.getInstance().player.hasVagina() ) {
			...
			EngineCore.outputText( ' to the pervy, sexy equine.' );
		}
		EngineCore.outputText( ' She smirks as she trots up, commenting, "<i>That\'s more like it, slut.  I don\'t know how you got this far, but your quest ends here.</i>"  She casually reaches down and grabs you by the scruff of the neck, hauling you bodily up until your head nestles between her tits' );
		if( CoC.getInstance().player.tallness >= 7 * 12 ) {
			EngineCore.outputText( ', even though your [feet] still touch the ground' );
		}
		EngineCore.outputText( '.  Brushing through your hair, a hand gently caresses your back as its owner coos, "<i>You\'ll make a fine mare for my stable, but lets get you in the right mindset first...</i>"' );
		...
		EngineCore.outputText( 'Your [vagina] is so puffy and inflamed with passion that it feels ready to burst out of your [armor], even as the drooling fem-cum makes it squish wetly with your every motion.' );
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( ' [EachCock] fares little better, a turgid mess of pre-cum leaking lust that seems to grow harder and more sensitive by the moment.' );
		}
		EngineCore.outputText( ' That wonderful warmth excites you sexually, a deepening puddle of lust in your loins that makes you so hot and wet that you swear it must be what\'s dripping out onto the cobble below.' );
	...
	HermCentaurScenes.prototype.maleLoss = function( hpVictory ) {
		...
		if( hpVictory ) {
			...
			EngineCore.outputText( ' try to pull [oneCock] out of your [armor] in an attempt to masturbate.  You\'re beaten, you just want to fuck or be fucked until you\'re both covered in a sheen of lady-spunk or your own thick cum.' );
		}
		EngineCore.outputText( ' She smirks as she trots up, commenting, "<i>That\'s more like it, slut.  I don\'t know how you got so far thinking you were some kind of ' + CoC.getInstance().player.mf( 'stud', 'queen bitch' ) + '.</i>"  She casually reaches down and grabs you by the scruff of the neck, hauling you bodily up until your head nestles between her tits' );
		if( CoC.getInstance().player.tallness >= 7 * 12 ) {
			EngineCore.outputText( ', even though your [feet] still touch the ground' );
		}
		EngineCore.outputText( '.  Brushing through your hair, a hand gently caresses your back as its owner coos, "<i>I\'m not sure you have what it takes to be one of my stallions...</i>"' );
		EngineCore.outputText( '\n\nYou\'re shifted to the side, your face dragged along the side of a pale, freckled breast as you\'re moved into position.  A thick nipple, almost as wide as your finger, pushes at your lips, smelling faintly of sugary sweetness and sex all blended together.  When you don\'t gratefully accept it, your jaw is forced open and the nearly inch-long teat is firmly inserted.  As soon as it\'s seated in your maw, the hard milk-nozzle immediately begins to leak its tasty treat - a sweet fluid that seems to fizz on your tongue and light your mouth up with pleasure.  You savor the taste as long as you can, but pleasure-inducing juice soon has your cheeks bulging.  You gulp it down reluctantly, if only to make room for more.' );
		EngineCore.outputText( '\n\nYour eyes open wide when the delicious demon-milk hits your stomach.  It fills your core with such warmth that you think you might melt, at least until the heat begins to diffuse outward, bringing with it a tingling sensitivity that makes the tit in your mouth taste that much better.  You draw deeply on the rosy, pink nipple, sucking down even more \'milk\' with every passing second, if only to kindle even more of that pleasant sensation.  The hands release you, but you stay firmly attached to your teat.  You need more.' );
		...
		if( CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( ' Your [vagina] fares no better, puffing up and drooling like its big brother while your [clit] grows taut and sensitive.' );
		}
		EngineCore.outputText( ' That wonderful warmth excites you sexually, moulding your loins into a simmering blaze of desire just looking for a something to cool its ache.' );
		...
		if( CoC.getInstance().flags[ kFLAGS.D1_OMNIBUS_KILLED ] !== 1 ) {
			EngineCore.outputText( ', and a familiar one at that' );
		}
		EngineCore.outputText( '.  She wears a black reflective bra, though you swear you can see pink protrusions under the shiny fabric, wriggling around constantly.  As expected with such dress, her nipples are hard as bullets.  A bulbous, ten-inch penis bobs between her thighs, thickening nicely as it fills to its full size, kissing your lips with a moist bead of pre as it fully erects.  The omnibus' );
		if( CoC.getInstance().flags[ kFLAGS.D1_OMNIBUS_KILLED ] !== 1 ) {
			EngineCore.outputText( ' overseer asks, "<i>Remember me?  I thought it only proper that I thank you for letting me live.  Now, be a good boy and suck my dick - show all everyone just what a horny, helplessly fuckable toy you are.</i>"' );
		} else {
			EngineCore.outputText( ' asks, "<i>I hope you don\'t mind slurping down a little cock, because you\'re going to anyway.</i>"' );
		}

		EngineCore.outputText( '\n\nThe second herm grabs you by the' );
		if( CoC.getInstance().player.hornType !== 0 ) {
			EngineCore.outputText( ' horns' );
		} else {
			EngineCore.outputText( ' head' );
		}
		EngineCore.outputText( ' and sidles forward, ramming her glans into your mouth and down the back of your throat.  You can feel her glans pulsing deep inside you, and as soon as the herm starts moaning, the centaur starts trotting forward again.  Amazingly, having her holding you up makes it easy to maintain your position and keep your dick bouncing in that hot twat. Fuck it! You\'re too horny to care anymore.' );
		EngineCore.outputText( '\n\nSlurping hard, you lick and suck the dick in your throat, so busy tending to the omnibus that you barely the notice the throbbing ache in your loins.  You cast lusty, inviting eyes around at every feminine form you see during journey to the stables, located in some backwater, seemingly forgotten part of the fortress.  You quiver with need and explode in passion, moaning into the Omnibus\'s cock as your ' + Descriptors.cockDescript( y ) + ' spends its lust into the oozing horse-pussy, wasting your champion-seed to father nothing more than a batch of shiftless imps.' );
