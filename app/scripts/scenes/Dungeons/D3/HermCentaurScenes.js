...
angular.module( 'cocjs' ).run( function( SceneLib, Utils, PregnancyStore, Descriptors, HermCentaur, Combat, kFLAGS, CoC, EngineCore ) {
	...
	HermCentaurScenes.prototype.encounterThePony = function() {
		...
		if( CoC.player.vaginalCapacity() < 100 || CoC.player.analCapacity() < 100 ) {
	...
	HermCentaurScenes.prototype.femFuckHerII = function( cockType ) {
		...
		if( cockType === TENTACOCK ) {
			...
			CoC.player.buttChange( CoC.player.analCapacity(), true, true, false );
			...
			EngineCore.outputText( '\n\nYou nervelessly slide off the pole into the ground, earning a fresh bukkake of creamy cum to plaster your ' + CoC.player.skinFurScales() + '.  While you recover, your demonic lover\'s bulk shifts away, and you soon here the telltale clopping of her moving away.  It sounds like she\'s heading towards the south - the exit.  You missed your chance to do anything about that demon...' );
		}
		CoC.player.orgasm();
		EngineCore.dynStats( 'cor+', 5 );
	...
	HermCentaurScenes.prototype.maleLoss = function( hpVictory ) {
		...
		EngineCore.outputText( ' She smirks as she trots up, commenting, "<i>That\'s more like it, slut.  I don\'t know how you got so far thinking you were some kind of ' + CoC.player.mf( 'stud', 'queen bitch' ) + '.</i>"  She casually reaches down and grabs you by the scruff of the neck, hauling you bodily up until your head nestles between her tits' );
		if( CoC.player.tallness >= 7 * 12 ) {
			EngineCore.outputText( ', even though your [feet] still touch the ground' );
		}
		...
		if( CoC.flags[ kFLAGS.D1_OMNIBUS_KILLED ] !== 1 ) {
