...
angular.module( 'cocjs' ).run( function( SceneLib, MainView, PerkLib, OnLoadVariables, AppearanceDefs, kFLAGS, CoC, EngineCore ) {
	...
	Fera.prototype.isHalloween = function() {
		return ((OnLoadVariables.date.date >= 28 && OnLoadVariables.date.month === 9) || (OnLoadVariables.date.date < 2 && OnLoadVariables.date.month === 10));
	};

	Fera.prototype.pumpkinFuckEncounter = function() {
		MainView.clearOutput();
		MainView.outputText( 'Just ahead, in the middle of the path, lies a seeming innocuous gourd - a pumpkin to be precise.  The growth is oddly out of place.  Nothing else grows on the soft animal-forged path you now tread, and the pumpkin and its vines are moist, as if wet with morning dew.  They are the only plants in the vicinity to bear such moisture, gleaming dully in the muted arboreal light.' );
		if( CoC.flags[ kFLAGS.PUMPKIN_FUCK_YEAR_DONE ] > 0 ) {
			MainView.outputText( '  An old, haunting memory surfaces, and you realizes this seems... familiar.  Did you find this last year?  Yes... it could have been this very same pumpkin...' );
		}
	...
	Fera.prototype.pumpkinFuck = function() {
		...
		MainView.outputText( 'Well, fuck it.  You\'re horny, and this little pumpkin doesn\'t look like it could stop you from fucking that gushing gourd-pussy.  As you shed your [armor], a few of the rough, slime-sweating vines feebly rise up to embrace you, but you brush them away, having no time for corrupt foolishness.  You\'re going to bust a nut in this fruit\'s sweet, supple folds, not let some malformed tentacle-horror molest you.' );
		...
		if( CoC.player.cumQ() < 200 ) {
			MainView.outputText( 'Your weak load disappears into the plant\'s sucking gullet, wicked away as fast as you fire.' );
		} else if( CoC.player.cumQ() < 1000 ) {
			MainView.outputText( 'Your thick load disappears into the plant\'s sucking gullet almost as fast as you\'re firing it, though you can see a bit of white creampie forming around the edges of your cock.' );
		} else {
			MainView.outputText( 'Your massive load floods the pumping, a deluge of semen squirting around your [cock biggest] even as the pumpkin starts to enlarge.  Every titanic spooge-squirt pumps the gourd up another level.  Soon, the rind is rubbing your belly.  A few blasts later, it\'s lifting you from the ground, your still-spurting cock lodged firmly in its cunt.  The suction feels so much stronger now - you\'re not sure you could pull out now if you wanted to, and you don\'t!' );
		}
		if( CoC.player.cumQ() < 1000 ) {
			...
			if( CoC.player.weaponName === 'fists' ) {
				MainView.outputText( 'brandishing your fists' );
			} else {
				MainView.outputText( 'picking up your [weapon]' );
			}
			MainView.outputText( ' as if it would somehow protect your nude body.' );
	...
	Fera.prototype.pumpkinFuckPartII = function() {
		...
		MainView.outputText( '\n\nThe goddess circles you, idly tracing black, claw-like nails over your exposed ' + CoC.player.skin() + '.  When did she get so close?  She drops her hand low, the dagger-sharp protrusions tenderly cupping your ' );
		if( CoC.player.balls > 0 ) {
		...
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( ', her free hand working ' );
			if( CoC.player.cockTotal() === 2 ) {
				MainView.outputText( 'your other' );
			} else {
				MainView.outputText( 'another' );
			}
			MainView.outputText( ' dick with child-like glee' );
		}
		MainView.outputText( '.  "<i>' );
		if( CoC.player.race() !== 'human' ) {
			MainView.outputText( 'I know what you were, virile suitor.  ' );
		}
		MainView.outputText( 'There\'s never too much of a good thing.  Of course, my champion must have the proper train of thought...</i>"' );
	...
	Fera.prototype.pumpkinFuckPartIII = function() {
		MainView.clearOutput();
		MainView.outputText( 'SLAP!  The crack of a palm on your face rouses you, a red handprint left on your cheek.  Fera smirks at you as you stir woozily in your plant-woven prison.  "<i>Did you enjoy yourself?</i>"  You slowly nod, feeling... liberated, free.  The situation is less worrying than before.  Fera just wanted to thank you, right?' );
		MainView.outputText( '\n\nThe sultry orange-haired maiden ' );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( 'pats your sloshing sack' );
		} else {
			MainView.outputText( 'caresses a sensitive spot beneath your dick' );
		}
		MainView.outputText( ' as the vines unwind, allowing you to stand on your own [feet].  You go to wipe the cum from your nose and upper lip as soon as you\'re free.  It\'s dry.  Did any of that even happen?' );
	...
	Fera.prototype.mountPumpkin = function() {
		...
		if( CoC.player.pregnancyIncubation > 0 || (CoC.player.wetness() < 2 && CoC.player.totalFertility() < 30) ) {
			MainView.outputText( 'Well, fuck it.  You\'re horny, and this little pumpkin doesn\'t look like it could stop you from fucking that gushing stem-rod.  As you shed your [armor], a few of the rough, slime-sweating vines feebly rise up to embrace you, but you brush them away, having no time for corrupt foolishness.  You\'re going to mount this fruit\'s hard, pulsing phallus, not let some malformed tentacle-horror molest you.' );
			...
			CoC.player.cuntChange( 50, true, false, false );
		...
		//Female characters don't have an easy analogue to cum production unless we count milk production, and female gender doesn't require either breasts or lactation to play.  So I just went for high vaginal wetness, not pregnant, and likely to become pregnant if she had sex with a male creature.  I hope that works out OK.;
			...
			MainView.outputText( '\n\nThe gourd\'s oozing stem does not disappoint.  Though it lacks the warmth or heartbeat of frantic copulation with a male, the slick plant-cock is both hard and pliant.  However, the shaft seems to be pulsating against every square inch of pussy that you lower onto it, and that gentle pulsing slowly draws you down, whether you planned on going to the hilt or not.  Not that you mind - the pulsing plant seems to be doing an adequate job of stimulating your walls as it passes through your glossy gates.' );
			MainView.outputText( '\n\nYour [hips] slap into the orange \'skin\' wetly, the omnipresent ooze immediately soak your ' + CoC.player.skin() + ' with corrupted lubricant.  You hardly mind, finding your body already setting into motion as you steady your hands to either side of the heavy fruit.  The lascivious fruit doesn\'t show any signs of minding the fast fuck you\'re giving it.  Indeed, the once weakly-moving vines have stilled, their restless tremors silenced by the firm smacks of your groin against vibrant rinds and swollen lips.  You hump away at your immobile lover, spellbound by its fecund fruit-cock, too drunk on lust and pleasure to question the strangeness of it all.' );
			MainView.outputText( '\n\nWet slapping noises ring out as you piston faster and faster, violently plowing the plant-cock into your [vagina] in a rutting, mating frenzy.  Your eyes are repeatedly drawn downward to the slimy shaft each time you pull up, the ooze keeping your [vagina] well lubricated.  The green shaft seems even larger than before, the phallus stretching you wider and longer each time you slam down on it as you ride the pumpkin into orgasm.  You feel a strange compulsion to let it cum inside, to flood your womb with pie goop until you appear immensely pregnant and the overflow is spraying out around your legs.' );
			...
			if( CoC.player.weaponName !== 'fists' ) {
				...
				MainView.outputText( 'raising your guard' );
			}
			MainView.outputText( ' as if it would somehow protect your nude body.' );
	...
	Fera.prototype.pumpkinMountingLevelII = function() {
		...
		if( CoC.player.biggestTitSize() >= 2 ) {
			MainView.outputText( ', her free hand working your [fullChest] with glee' );
		}
		MainView.outputText( '.  <i>"There\'s never too much of a good thing.  Of course, my champion must have the proper train of thought..."</i>' );
	...
	Fera.prototype.mountPumpkinsIII = function() {
		MainView.clearOutput();
		MainView.outputText( 'SLAP!  The crack of a palm on your face rouses you, a red handprint left on your cheek.  Fera smirks at you as you stir woozy in your plant-woven prison.  <i>"Did you enjoy yourself?"</i>  You slowly nod, feeling... liberated... free.  The situation is less worrying than before.  Fera just wanted to thank you, right?' );
	...
	Fera.prototype.leaveFerasWonderland = function() {
		MainView.clearOutput();
		MainView.outputText( 'Nah, that whole place is probably some kind of giant venus fly trap. Fuck that.' );
	...
	Fera.prototype.exploreFerasWonderland = function() {
		...
		if( CoC.player.gender > 0 ) {
			MainView.outputText( '  Your heart is hammering' );
		}
		if( CoC.player.hasCock() && !CoC.player.hasVagina() ) {
			MainView.outputText( ', and there\'s a telltale tightness building in your groin, expanding and pressing oh-so-hard on your [armor]. You look up at those sweet lips and ponder what they would feel like wrapped around your ' + CoC.player.multiCockDescriptLight() + ', a thought that only makes your gear that much harder to bear as it constrains you.' );
		...
		if( CoC.player.inte >= 40 ) {
			...
			MainView.outputText( ', but that thought slips out of your head as it fills with more of that sinfully exciting aroma, making you feel so hot, horny, and eager.  You have to get more of it, submerse yourself deeper in its cloying clutches as you watch the pretty colors drain away your worries and concerns.' );
			if( CoC.player.hasCock() ) {
		...
		if( CoC.player.hasCock() && CoC.player.hasVagina() ) {
			...
			MainView.outputText( 'question left to you is, are you more concerned with slipping [oneCock] into those subliminally-whispering lips or guiding a writhing stamen into your [vagina] and [asshole] so that you can take in as much nectar as possible?' );
	...
	Fera.prototype.leaveAfterFlowerHuffing = function() {
		MainView.clearOutput();
		MainView.outputText( 'The farther you get from that glade, the more your head clears and the more you realize how close you came to disaster.  You resolve to avoid it in the future, lest you fall prey to that entrancing pollen and the hypnotic petals once more.' );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	//Fuck the Lips;
	Fera.prototype.fuckDemCorruptFlowerLips = function() {
		...
		if( CoC.player.longestCockLength() >= 20 ) {
			MainView.outputText( 'easily ramming your huge endowment' );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ' straight into the erotic-looking mouth' );
		} else if( CoC.player.longestCockLength() >= 8 ) {
			MainView.outputText( 'slipping your large endowments' );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ' right into the erotic-looking mouth' );
		} else if( CoC.player.longestCockLength() >= 4 ) {
			MainView.outputText( 'easing the top hal' );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 'ves' );
			} else {
				MainView.outputText( 'f' );
			}
			MainView.outputText( ' of your phallus' );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 'es' );
			}
			MainView.outputText( ' into the erotic-looking mouth' );
		} else {
			MainView.outputText( 'barely managing to slip your tip' );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ' into the erotic-looking mouth' );
		}
		MainView.outputText( '. The contact is everything you thought it would be: surprisingly warm, soaking-wet, and squeezing just tight enough to make your pillar' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ' of lust twitch in excitement.' );
		...
		if( CoC.player.balls > 0 ) {
			MainView.outputText( '[balls] against the cushy lower lip, wreathing them in syrupy sap or nectar.  You\'re not really sure what the appropriate name for such a substance is, and you\'re well past caring.' );
		} else {
			MainView.outputText( 'loins against the cushy, sap-slicked lips, compressing their hot, smooth surfaces against your ' + CoC.player.skin() + '.  You\'re not really sure if sap is the appropriate name for such a substance, but you\'re far beyond caring at this point.' );
		}
		MainView.outputText( '  The fast-diminishing light doesn\'t even register, as your eyes have closed as you subsume yourself in pleasure.' );
		...
		MainView.outputText( '\n\nYour motions slow as the ambrosial syrup is wicked into your body, frying your synapses with ecstasy until only lust, pleasure, and perfect heat are occupying your few remaining thoughts. By the time the tendril pulls free, glazing your face in sappy goo, your [hips] have stopped their thrusts entirely, leaving your ' + CoC.player.multiCockDescriptLight() + ' to pulsate and ache with need despite their owner\'s inability to remember how to hump. Best of all, the few times that you do manage to summon up a coherent thought, the flashing, undulating colors unravel it, letting you revel in unthinking arousal.' );
		MainView.outputText( '\n\nThe deluge of drugs, arousal, sexual stimulation, and mind-scattering visual patterns has made your mind as pliant and vulnerable as the lips your ' + CoC.player.multiCockDescriptLight() + ' ' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 'are' );
		} else {
			MainView.outputText( 'is' );
		}
		MainView.outputText( ' buried in, so hot, wet, and soft. With each passing moment, you become more relaxed, more open and exposed, and more aroused than before. You would probably be afraid if you were capable of any kind of reason.' );
		MainView.outputText( '\n\nA perfectly smooth, perfectly dry hand traces down your back in a comforting yet sensual gesture, followed a moment later by another upon your [butt]. It squeezes you, kneading your cheek in its supple, dextrous fingers, letting long, sharp nails just barely dig into your ' + CoC.player.skinFurScales() + '. Then the hands move down to your [legs] to continue their massage, dodging around the tentacle-like stamen so that they can reach your [feet]. There, the hands stop and separate from your body, vanishing until a familiar face leans in front of you, interrupting the slew of mind-tranquilizing colors briefly.' );
	...
	Fera.prototype.dickBoostingFuntimes = function() {
		...
		MainView.outputText( ' just in time to take the first thrust from your hips, burying yourself into them and immediately pulling back, setting an easy, average rhythm.  At the same time, the suction starts, causing your ' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 'heads' );
		} else {
			MainView.outputText( '[cockHead]' );
		}
		MainView.outputText( ' to swell up a little, and when the rest of your length is plowed through the tight, slippery ring, the tugging vacuum increases, plumping your dick' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ' bigger, harder, and fatter than ' );
		if( CoC.player.cockTotal() === 1 ) {
			MainView.outputText( 'it' );
		} else {
			MainView.outputText( 'they' );
		}
		MainView.outputText( ' should be.' );
		...
		MainView.outputText( ' so that you can start the cycle anew.  You love that, and this is the perfect place for that kind of behavior, isn\'t it? Everybody in this world wants to fuck, they just like to fight to see who gets to be on top, and you\'re good at winning, aren\'t you, pet champion?</i>" Fera ' );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( 'cups your [sack] tenderly' );
		} else {
			MainView.outputText( 'tickles your taint' );
		}
		MainView.outputText( ' to spur you to thrust harder while she fills your head with lurid thoughts.' );
		...
		MainView.outputText( ' as you do, whispering, "<i>Good ' + CoC.player.mf( 'boy', 'girl' ) + ',</i>" into your ears as dark energies flow into you, refilling your fast-emptying reservoirs faster than they can empty.  "<i>Don\'t stop yet, [name].  You\'re going to be doing this a lot, so I\'m giving you lots of practice.</i>"' );
		...
		for( var x = 0; x < CoC.player.cocks.length; x++ ) {
			CoC.player.cocks[ x ].cockLength++;
			if( CoC.player.cocks[ x ].cockThickness < CoC.player.cocks[ x ].cockLength / 5 ) {
				CoC.player.cocks[ x ].cockThickness = CoC.player.cocks[ x ].cockLength / 5;
			}
		}
		CoC.player.orgasm();
	...
	Fera.prototype.stickAStamenInIt = function() {
		...
		MainView.outputText( ', you drag yourself up until the \'floor\' flattens out and you nearly ram into the pursed lips, stopping yourself abruptly once you realize you\'ve reached your goal.' );
		...
		MainView.outputText( 'a tight, dick-milking rape-mother' );
		if( !CoC.player.hasCock() ) {
			MainView.outputText( ', or a libidinous dick-girl, sowing your seed everywhere you can and saving your cunt for only the strongest of mates' );
		}
		MainView.outputText( '?</i>"' );
	...
	Fera.prototype.gapeMePleaseFera = function() {
		...
		if( CoC.player.vaginas[ 0 ].vaginalLooseness >= AppearanceDefs.VAGINA_LOOSENESS_LEVEL_CLOWN_CAR ) {
			MainView.outputText( ', spreading its well-widened lips as wide as they\'ve ever been' );
		} else if( CoC.player.vaginas[ 0 ].vaginalLooseness >= AppearanceDefs.VAGINA_LOOSENESS_GAPING ) {
			MainView.outputText( ', spreading your lips well beyond the widest they\'ve ever been and still farther.' );
		} else if( CoC.player.vaginas[ 0 ].vaginalLooseness >= AppearanceDefs.VAGINA_LOOSENESS_LOOSE ) {
			MainView.outputText( ', meeting a bit of token resistance at your unprepared entryway and slithering stamen after stamen inside, the forerunners widening you until the entirety of the column can pass inside, gaping you almost inhumanly wide.' );
		} else {
			MainView.outputText( ', meeting more than a little resistant at your unstretched entry and finding a way in anyhow. First one tentacle slithers in, then a second, then a fourth, and so on. One after another, they spread you wider and wider, opening you until the entire wiggling column is buried deep, pulling your lips ridiculously wide.' );
		}
		...
		//STEAL VIRGINITY!;
		CoC.player.cuntChange( 90000, true, true, false );
		...
		if( CoC.player.wetness() < 4 ) {
			MainView.outputText( ' even though you\'re not a squirter' );
		}
		MainView.outputText( '.  They pump on with intense, unrelenting energy, squishing wetly with every thrust, splattering juice everywhere as they ruin your poor, convulsing cunny.' );
		CoC.player.cuntChange( 90000, true, true, false );
		...
		//(cum!);
		//Minimum size to 4;
		CoC.player.createPerk( PerkLib.FerasBoonWideOpen, 0, 0, 0, 0 );
		...
		if( CoC.player.vaginas[ 0 ].vaginalLooseness < AppearanceDefs.VAGINA_LOOSENESS_GAPING_WIDE ) {
			CoC.player.vaginas[ 0 ].vaginalLooseness = AppearanceDefs.VAGINA_LOOSENESS_GAPING_WIDE;
		}
		//Add five corruption.;
		CoC.player.orgasm();
	...
	Fera.prototype.gapeByFeraEpilogue = function() {
		MainView.clearOutput();
		MainView.outputText( 'You awaken fully dressed with a familiar ache between your legs. Damn, that was a good nap.  You rise, stretching as you rub your back, sore from lying against a tree for so long.  You suppose you could hunt down a tentacle monster and see if it\'d really give you a filling' );
		if( CoC.player.pregnancyIncubation === 0 ) {
			MainView.outputText( ', but you\'d rather fuck something that\'ll get you good and pregnant with a well-hung fuck-CoC.monster, like a minotaur!' );
		} else {
			MainView.outputText( '.' );
		}
		MainView.outputText( '  You idly reach into your [armor] and stuff four fingers into yourself before sighing and removing them.  It just isn\'t the same as being full of cock.' );
	...
	Fera.prototype.tightLikeAToigar = function() {
		MainView.clearOutput();
		MainView.outputText( 'Fera smiles.  "<i>I had hoped we might do that, little champion.  Now, you simply relax and enjoy the pleasure that only my normal weak-willed prey are allowed to experience.  You will be the first to taste it and be allowed free.  How wonderful!</i>"  She pets your forehead.' );
		...
		if( CoC.player.vaginas[ 0 ].vaginalLooseness >= AppearanceDefs.VAGINA_LOOSENESS_LEVEL_CLOWN_CAR ) {
			MainView.outputText( ', slipping past your capacitive entrance with ease' );
		} else if( CoC.player.vaginas[ 0 ].vaginalLooseness >= AppearanceDefs.VAGINA_LOOSENESS_GAPING ) {
			MainView.outputText( ', sliding into your welcoming entrance fairly easily' );
		} else if( CoC.player.vaginas[ 0 ].vaginalLooseness >= AppearanceDefs.VAGINA_LOOSENESS_LOOSE ) {
			MainView.outputText( ', slowly working its way into your decently unstretched entrance' );
		} else {
			MainView.outputText( ', slowly burrowing past your tight, restrictive entrance' );
		}
		MainView.outputText( ' and leaving a trail of slick, black corruption in its wake.  The tendril pushes the whole way up to your cervix, gently butting up against your womb\'s portal before dragging back out, sputtering more inky cum all the way.  When it pops out, the head catches the tip of your [clit], causing your [vagina] to spasm and contract, squeezing down on the foreign fluid with such strength that a geyser of sable spunk erupts from your quivering quim.' );
		CoC.player.cuntChange( 1, true, true, false );
		MainView.outputText( '\n\nFera coos, "<i>Atta girl, are you feeling it?  Can you feel the way your pussy is tightening up?  You can, I know, and your womb is getting thirstier and thirstier, just aching to squeeze the cum out of a helpless male or herm that you\'ve taken the time to beat into their place, right, my pretty?</i>"  Her fingers reach down to your black-stained cleft and rub around the hardening gunk that paints your crotch, making it look like someone painted liquid latex all across your ' + CoC.player.skin() + '.  At the same time, the tendril lunges back in to your spunk-spouting cunt, plugging it just before it starts to unload once more, its stalk undulating between your legs with each thrust, rising up and straightening with each hip-shaking thrust it delivers to your increasingly supple, stained box.' );
		...
		//(cum!);
		CoC.player.orgasm();
		EngineCore.dynStats( 'lib', 5, 'sen', 20, 'cor', 3 );
		//Maximum size to 2;
		CoC.player.createPerk( PerkLib.FerasBoonMilkingTwat, 0, 0, 0, 0 );
	...
	Fera.prototype.growADickForFera = function() {
		...
		if( CoC.player.tailType > AppearanceDefs.TAIL_TYPE_NONE ) {
			MainView.outputText( ' and press your tail to the ground' );
		}
		MainView.outputText( '.  You feel the wetness of the hollow tube as it presses against you along with a lance of pure desire and heat in the very center, undoubtedly caused by the tainted needle as it slips through your unprotected flesh.  The thing is compressed so tightly against you that it looks like the entire injector was able to bury itself in your body.' );
		...
		//(cum!);
		CoC.player.orgasm();
		EngineCore.dynStats( 'lib', 10, 'sen', 10, 'cor', 3 );
