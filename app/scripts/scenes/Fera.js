...
angular.module( 'cocjs' ).run( function( PerkLib, OnLoadVariables, AppearanceDefs, kFLAGS, CoC, EngineCore ) {
	function Fera() {
	}
	...
	Fera.prototype.pumpkinFuckEncounter = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.PUMPKIN_FUCK_YEAR_DONE ] > 0 ) {
		...
		if( CoC.getInstance().player.gender === 0 ) {
			...
			EngineCore.choices( 'Fuck It', fuck, 'Mount It', mount, '', null, '', null, 'No Way', CoC.getInstance().scenes.camp.returnToCampUseOneHour );
		}
	};
	//[HOLY SHIT YOU BE FUCKING A PUMPKIN];
	Fera.prototype.pumpkinFuck = function() {
		EngineCore.clearOutput();
		CoC.getInstance().flags[ kFLAGS.PUMPKIN_FUCK_YEAR_DONE ] = OnLoadVariables.date.fullYear;
		...
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( 'balls deep ' );
		} else {
			EngineCore.outputText( 'hilt deep ' );
		}
		...
		if( CoC.getInstance().player.cumQ() < 200 ) {
			EngineCore.outputText( 'Your weak load disappears into the plant\'s sucking gullet, wicked away as fast as you fire.' );
		} else if( CoC.getInstance().player.cumQ() < 1000 ) {
			EngineCore.outputText( 'Your thick load disappears into the plant\'s sucking gullet almost as fast as you\'re firing it, though you can see a bit of white creampie forming around the edges of your cock.' );
		} else {
		...
		if( CoC.getInstance().player.cumQ() < 1000 ) {
			...
			if( CoC.getInstance().player.weaponName === 'fists' ) {
				EngineCore.outputText( 'brandishing your fists' );
			} else {
	...
	Fera.prototype.pumpkinFuckPartII = function() {
		...
		EngineCore.outputText( '\n\nThe goddess circles you, idly tracing black, claw-like nails over your exposed ' + CoC.getInstance().player.skin() + '.  When did she get so close?  She drops her hand low, the dagger-sharp protrusions tenderly cupping your ' );
		if( CoC.getInstance().player.balls > 0 ) {
		...
		if( CoC.getInstance().player.race() !== 'human' ) {
			EngineCore.outputText( 'I know what you were, virile suitor.  ' );
		}
	...
	Fera.prototype.pumpkinFuckPartIII = function() {
		...
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( 'pats your sloshing sack' );
		} else {
			EngineCore.outputText( 'caresses a sensitive spot beneath your dick' );
		}
		...
		CoC.getInstance().player.createPerk( PerkLib.FerasBoonAlpha, 0, 0, 0, 0 );
	...
	Fera.prototype.mountPumpkin = function() {
		...
		if( CoC.getInstance().player.pregnancyIncubation > 0 || (CoC.getInstance().player.wetness() < 2 && CoC.getInstance().player.totalFertility() < 30) ) {
			...
			CoC.getInstance().player.cuntChange( 50, true, false, false );
		...
		//Female characters don't have an easy analogue to cum production unless we count milk production, and female gender doesn't require either breasts or lactation to play.  So I just went for high vaginal wetness, not pregnant, and likely to become pregnant if she had sex with a male creature.  I hope that works out OK.;
			...
			EngineCore.outputText( '\n\nThe gourd\'s oozing stem does not disappoint.  Though it lacks the warmth or heartbeat of frantic copulation with a male, the slick plant-cock is both hard and pliant.  However, the shaft seems to be pulsating against every square inch of pussy that you lower onto it, and that gentle pulsing slowly draws you down, whether you planned on going to the hilt or not.  Not that you mind - the pulsing plant seems to be doing an adequate job of stimulating your walls as it passes through your glossy gates.' );
			EngineCore.outputText( '\n\nYour [hips] slap into the orange \'skin\' wetly, the omnipresent ooze immediately soak your ' + CoC.getInstance().player.skin() + ' with corrupted lubricant.  You hardly mind, finding your body already setting into motion as you steady your hands to either side of the heavy fruit.  The lascivious fruit doesn\'t show any signs of minding the fast fuck you\'re giving it.  Indeed, the once weakly-moving vines have stilled, their restless tremors silenced by the firm smacks of your groin against vibrant rinds and swollen lips.  You hump away at your immobile lover, spellbound by its fecund fruit-cock, too drunk on lust and pleasure to question the strangeness of it all.' );
			EngineCore.outputText( '\n\nWet slapping noises ring out as you piston faster and faster, violently plowing the plant-cock into your [vagina] in a rutting, mating frenzy.  Your eyes are repeatedly drawn downward to the slimy shaft each time you pull up, the ooze keeping your [vagina] well lubricated.  The green shaft seems even larger than before, the phallus stretching you wider and longer each time you slam down on it as you ride the pumpkin into orgasm.  You feel a strange compulsion to let it cum inside, to flood your womb with pie goop until you appear immensely pregnant and the overflow is spraying out around your legs.' );
	...
	Fera.prototype.pumpkinMountingLevelII = function() {
		...
		if( CoC.getInstance().player.biggestTitSize() >= 2 ) {
			EngineCore.outputText( ', her free hand working your [fullChest] with glee' );
		}
	...
	Fera.prototype.mountPumpkinsIII = function() {
		...
		CoC.getInstance().player.createPerk( PerkLib.FerasBoonBreedingBitch, 0, 0, 0, 0 );
		EngineCore.dynStats( 'cor', 30 );
		EngineCore.doNext( CoC.getInstance().scenes.camp.returnToCampUseOneHour );
	};
	...
	Fera.prototype.leaveFerasWonderland = function() {
		...
		CoC.getInstance().flags[ kFLAGS.FERAS_GLADE_EXPLORED_YEAR ] = OnLoadVariables.date.fullYear;
		EngineCore.doNext( CoC.getInstance().scenes.camp.returnToCampUseOneHour );
	};
	//Explore Das Trees;
	Fera.prototype.exploreFerasWonderland = function() {
		EngineCore.clearOutput();
		CoC.getInstance().flags[ kFLAGS.FERAS_GLADE_EXPLORED_YEAR ] = OnLoadVariables.date.fullYear;
		EngineCore.outputText( 'Straightening to your full height, you take a few experimental sniffs and begin to walk back towards the middle of the meadow, brushing aside garish pink blooms that rise up past your waist.  That smell is getting stronger now.  How could you have missed it when you first arrived? It\'s heavenly, like a combination of every sweet treat you like with an undercurrent of passionate sexual musk blended in underneath.  You close in, ' + CoC.getInstance().player.mf( '[hips] swaggering excitedly', '[hips] sashaying excitedly' ) + ', ' + CoC.getInstance().player.skin() + ' flushing.  A few baby blue sunflowers bump your shoulders as you muscle past them to hone in your new favorite aroma.  You\'ve just got to know what bloom could possibly release such a heavenly scent!' );
		...
		if( CoC.getInstance().player.gender > 0 ) {
			EngineCore.outputText( '  Your heart is hammering' );
		}
		if( CoC.getInstance().player.hasCock() && !CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( ', and there\'s a telltale tightness building in your groin, expanding and pressing oh-so-hard on your [armor]. You look up at those sweet lips and ponder what they would feel like wrapped around your ' + CoC.getInstance().player.multiCockDescriptLight() + ', a thought that only makes your gear that much harder to bear as it constrains you.' );
	...
	Fera.prototype.fuckDemCorruptFlowerLips = function() {
		EngineCore.clearOutput();
		CoC.getInstance().flags[ kFLAGS.FERAS_TRAP_SPRUNG_YEAR ] = OnLoadVariables.date.fullYear;
		...
		if( CoC.getInstance().player.longestCockLength() >= 20 ) {
		...
		} else if( CoC.getInstance().player.longestCockLength() >= 8 ) {
		...
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( '[balls] against the cushy lower lip, wreathing them in syrupy sap or nectar.  You\'re not really sure what the appropriate name for such a substance is, and you\'re well past caring.' );
		} else {
			EngineCore.outputText( 'loins against the cushy, sap-slicked lips, compressing their hot, smooth surfaces against your ' + CoC.getInstance().player.skin() + '.  You\'re not really sure if sap is the appropriate name for such a substance, but you\'re far beyond caring at this point.' );
		}
		...
		EngineCore.outputText( '\n\nA perfectly smooth, perfectly dry hand traces down your back in a comforting yet sensual gesture, followed a moment later by another upon your [butt]. It squeezes you, kneading your cheek in its supple, dextrous fingers, letting long, sharp nails just barely dig into your ' + CoC.getInstance().player.skinFurScales() + '. Then the hands move down to your [legs] to continue their massage, dodging around the tentacle-like stamen so that they can reach your [feet]. There, the hands stop and separate from your body, vanishing until a familiar face leans in front of you, interrupting the slew of mind-tranquilizing colors briefly.' );
		...
		EngineCore.outputText( '\n\nFera rubs your ' );
		if( CoC.getInstance().player.balls === 0 ) {
			EngineCore.outputText( 'inner thighs' );
		} else {
			EngineCore.outputText( '[balls]' );
		}
		EngineCore.outputText( ', squeezing you encouragingly.  "<i>Now, I am a generous goddess. Your gift will be to aid me by being a rapacious, horny predator, one who will seed every strong person or monster ' + CoC.getInstance().player.mf( 'he', 'she' ) + ' encounters until there is an army of unstoppable fuck-monsters roaming the lands.  You will fertilize the weak again and again, until they give birth to strong offspring.  You\'ll make the males your subservient bitches, only fit to serve your pregnant slaves and to take your steaming loads in their holes whenever there\'s no suitable female around</i>"  She taps her chin thoughtfully.  "<i>How does that sound?</i>"' );
	...
	Fera.prototype.dickBoostingFuntimes = function() {
		...
		if( CoC.getInstance().player.cockTotal() > 1 ) {
			EngineCore.outputText( 'heads' );
		} else {
			EngineCore.outputText( '[cockHead]' );
		}
		...
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( 'cups your [sack] tenderly' );
		} else {
			EngineCore.outputText( 'tickles your taint' );
		}
		...
		EngineCore.outputText( ' as you do, whispering, "<i>Good ' + CoC.getInstance().player.mf( 'boy', 'girl' ) + ',</i>" into your ears as dark energies flow into you, refilling your fast-emptying reservoirs faster than they can empty.  "<i>Don\'t stop yet, [name].  You\'re going to be doing this a lot, so I\'m giving you lots of practice.</i>"' );
		...
		for( var x = 0; x < CoC.getInstance().player.cocks.length; x++ ) {
			CoC.getInstance().player.cocks[ x ].cockLength++;
			if( CoC.getInstance().player.cocks[ x ].cockThickness < CoC.getInstance().player.cocks[ x ].cockLength / 5 ) {
				CoC.getInstance().player.cocks[ x ].cockThickness = CoC.getInstance().player.cocks[ x ].cockLength / 5;
			}
		}
		CoC.getInstance().player.orgasm();
		EngineCore.dynStats( 'lib', 10, 'cor', 5 );
		CoC.getInstance().player.createPerk( PerkLib.FerasBoonSeeder, 0, 0, 0, 0 );
		EngineCore.menu();
	...
	Fera.prototype.stickAStamenInIt = function() {
		...
		if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( ', [eachCock] bobbing' );
		}
		...
		if( !CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( ', or a libidinous dick-girl, sowing your seed everywhere you can and saving your cunt for only the strongest of mates' );
		}
		EngineCore.outputText( '?</i>"' );
	...
	Fera.prototype.gapeMePleaseFera = function() {
		...
		if( CoC.getInstance().player.vaginas[ 0 ].vaginalLooseness >= AppearanceDefs.VAGINA_LOOSENESS_LEVEL_CLOWN_CAR ) {
			EngineCore.outputText( ', spreading its well-widened lips as wide as they\'ve ever been' );
		} else if( CoC.getInstance().player.vaginas[ 0 ].vaginalLooseness >= AppearanceDefs.VAGINA_LOOSENESS_GAPING ) {
			EngineCore.outputText( ', spreading your lips well beyond the widest they\'ve ever been and still farther.' );
		} else if( CoC.getInstance().player.vaginas[ 0 ].vaginalLooseness >= AppearanceDefs.VAGINA_LOOSENESS_LOOSE ) {
			EngineCore.outputText( ', meeting a bit of token resistance at your unprepared entryway and slithering stamen after stamen inside, the forerunners widening you until the entirety of the column can pass inside, gaping you almost inhumanly wide.' );
		} else {
			EngineCore.outputText( ', meeting more than a little resistant at your unstretched entry and finding a way in anyhow. First one tentacle slithers in, then a second, then a fourth, and so on. One after another, they spread you wider and wider, opening you until the entire wiggling column is buried deep, pulling your lips ridiculously wide.' );
		}
		...
		//STEAL VIRGINITY!;
		CoC.getInstance().player.cuntChange( 90000, true, true, false );
		...
		if( CoC.getInstance().player.wetness() < 4 ) {
			EngineCore.outputText( ' even though you\'re not a squirter' );
		}
		...
		//(cum!);
		//Minimum size to 4;
		CoC.getInstance().player.createPerk( PerkLib.FerasBoonWideOpen, 0, 0, 0, 0 );
		...
		if( CoC.getInstance().player.vaginas[ 0 ].vaginalLooseness < AppearanceDefs.VAGINA_LOOSENESS_GAPING_WIDE ) {
			CoC.getInstance().player.vaginas[ 0 ].vaginalLooseness = AppearanceDefs.VAGINA_LOOSENESS_GAPING_WIDE;
		}
		//Add five corruption.;
		CoC.getInstance().player.orgasm();
	...
	Fera.prototype.gapeByFeraEpilogue = function() {
		...
		if( CoC.getInstance().player.pregnancyIncubation === 0 ) {
			EngineCore.outputText( ', but you\'d rather fuck something that\'ll get you good and pregnant with a well-hung fuck-CoC.getInstance().monster, like a minotaur!' );
		} else {
			EngineCore.outputText( '.' );
		}
	...
	Fera.prototype.tightLikeAToigar = function() {
		...
		if( CoC.getInstance().player.skinAdj === 'latex' || CoC.getInstance().player.skinAdj === 'rubber' ) {
			EngineCore.outputText( ', not that a ' + CoC.getInstance().player.skinAdj + ' honey like you would notice' );
		}
		...
		if( CoC.getInstance().player.vaginas[ 0 ].vaginalLooseness >= AppearanceDefs.VAGINA_LOOSENESS_LEVEL_CLOWN_CAR ) {
			EngineCore.outputText( ', slipping past your capacitive entrance with ease' );
		} else if( CoC.getInstance().player.vaginas[ 0 ].vaginalLooseness >= AppearanceDefs.VAGINA_LOOSENESS_GAPING ) {
			EngineCore.outputText( ', sliding into your welcoming entrance fairly easily' );
		} else if( CoC.getInstance().player.vaginas[ 0 ].vaginalLooseness >= AppearanceDefs.VAGINA_LOOSENESS_LOOSE ) {
			EngineCore.outputText( ', slowly working its way into your decently unstretched entrance' );
		} else {
			EngineCore.outputText( ', slowly burrowing past your tight, restrictive entrance' );
		}
		...
		EngineCore.outputText( '\n\nFera coos, "<i>Atta girl, are you feeling it?  Can you feel the way your pussy is tightening up?  You can, I know, and your womb is getting thirstier and thirstier, just aching to squeeze the cum out of a helpless male or herm that you\'ve taken the time to beat into their place, right, my pretty?</i>"  Her fingers reach down to your black-stained cleft and rub around the hardening gunk that paints your crotch, making it look like someone painted liquid latex all across your ' + CoC.getInstance().player.skin() + '.  At the same time, the tendril lunges back in to your spunk-spouting cunt, plugging it just before it starts to unload once more, its stalk undulating between your legs with each thrust, rising up and straightening with each hip-shaking thrust it delivers to your increasingly supple, stained box.' );
		...
		//Maximum size to 2;
		CoC.getInstance().player.createPerk( PerkLib.FerasBoonMilkingTwat, 0, 0, 0, 0 );
	...
	Fera.prototype.growADickForFera = function() {
		...
		if( CoC.getInstance().player.tailType > AppearanceDefs.TAIL_TYPE_NONE ) {
			EngineCore.outputText( ' and press your tail to the ground' );
		}
	...
	CoC.getInstance().registerScene( 'fera', new Fera() );
} );