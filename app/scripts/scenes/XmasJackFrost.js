...
angular.module( 'cocjs' ).run( function( SceneLib, MainView, OnLoadVariables, StatusAffects, Utils, PregnancyStore, Descriptors, kFLAGS, CoC, EngineCore ) {
	...
	XmasJackFrost.prototype.meetJackFrostInTheMountains = function() {
		...
		//[Yes] [No];
		EngineCore.menu();
		EngineCore.addButton( 0, 'Yes', this, this.jizzSnowOnMyCampPlease );
		EngineCore.addButton( 1, 'No', this, this.noJizzingInMyCampPlease );
	};
	//[=No=];
	XmasJackFrost.prototype.noJizzingInMyCampPlease = function() {
		MainView.clearOutput();
		MainView.outputText( 'You thank the giant for his offer, but you\'ll have to refuse.  You really can\'t take even one day off your quest.' );
	...
	XmasJackFrost.prototype.processJackFrostEvent = function() {
		...
		//No followers;
			...
			MainView.outputText( 'You sigh... this really reminds you of back home... you only wish you had someone to share this feeling with... Well, there is no reason you shouldn\'t enjoy yourself while it snows, so you set about rolling a big ball of snow to make a snowman out of...' );
			//Skip to next day...;
		...
		//Izma;
			...
			MainView.outputText( 'You wonder where Izma is; as a shark, maybe she doesn\'t like the cold?  Even as the thought crosses your mind, you note a surprisingly large pile of snow sitting not too far - with a very suspicious striped fin jutting out of it.  You rush over, calling for Izma and asking if she\'s all right.' );
		...
		//Jojo;
			...
			//Corrupt;
				...
				MainView.outputText( 'Out in the outskirts of the camp you notice your pet mouse-slut Jojo, casually fucking a small mound of snow like an animal.  You laugh to yourself, now, now... doesn\'t he know that there are better things to fuck than a pitiful mound of snow on the ground...' );
				...
				if( CoC.player.hasCock() ) {
					EngineCore.addButton( 0, 'Pen Ass', this, this.penetradeCorruptJojoAss );
				}
				if( CoC.player.hasVagina() ) {
					EngineCore.addButton( 1, 'Ride Cock', this, this.rideCorruptJojoCock );
				}
				EngineCore.addButton( 2, 'Spank n Finger', this, this.spankNFingerCorruptJojoAss );
			}
			return;
		}
		//Amily;
		//less than 5!;
			...
			if( !SceneLib.amilyScene.amilyCorrupt() ) {
				MainView.outputText( 'Amily is warily prowling across the icy surface, occasionally glancing all around as if trying to spot an ambush.  Other times, she suddenly leaps up in a flurry, whirling around in mid-air as something catches her over-large ears.  All in all, it\'s quite obvious she\'s scared.' );
				...
				EngineCore.menu();
				if( CoC.player.hasCock() ) {
					EngineCore.addButton( 0, 'Penetrate', this, this.penetradePureMouseWaifu );
				}
				EngineCore.addButton( 1, 'Ride Cock', this, this.makeOutWithPureMouseWaifu );
				return;
			}
			//Corrupt;
			else {
				MainView.outputText( 'Out in the outskirts where your favorite cum-slut lives, you spot her slowly rubbing the snow over her erect nipples, the cold making her shiver in pleasure.' );
				...
				if( CoC.player.gender > 0 ) {
					...
					if( CoC.player.hasCock() ) {
						...
						if( CoC.player.balls > 0 ) {
							MainView.outputText( ', even tickling your [balls]' );
						} else if( CoC.player.hasVagina() ) {
							MainView.outputText( ', even tickling your [cunt] for added tantalization' );
						}
						MainView.outputText( '.' );
		...
		//Tainted Ember;
			...
			MainView.outputText( 'The dragon is wandering your camp in open fascination.  "<i>What is this weird white stuff?  It\'s cold and wet... but soft and fluffy, too,</i>" ' + SceneLib.emberScene.emberMF( 'he', 'she' ) + ' says, gently catching a snowflake.' );
		...
		//Marble (Written by TDM himself);
			...
			if( CoC.flags[ kFLAGS.MARBLE_KIDS ] > 0 && CoC.flags[ kFLAGS.MARBLE_NURSERY_CONSTRUCTION ] >= 100 ) {
				MainView.outputText( 'You find Marble simply staring into the sky, a look of wonder covering her face.  Dancing around her in excitement ' );
				if( CoC.flags[ kFLAGS.MARBLE_KIDS ] > 1 ) {
					MainView.outputText( 'are ' );
				} else {
					MainView.outputText( 'is ' );
				}
				MainView.outputText( 'your kid' );
				if( CoC.flags[ kFLAGS.MARBLE_KIDS ] > 1 ) {
					MainView.outputText( 's' );
				}
				MainView.outputText( ', playing in the snow, blowing the snow, and experimenting with the snow.  Exactly what ' );
				if( CoC.flags[ kFLAGS.MARBLE_KIDS ] === 1 ) {
					MainView.outputText( 'she' );
				} else {
					MainView.outputText( 'they' );
				}
				MainView.outputText( ' are up to at any given moment is really hard to say, and you\'re not even sure that the little girl' );
				if( CoC.flags[ kFLAGS.MARBLE_KIDS ] > 1 ) {
					MainView.outputText( 's' );
				}
				MainView.outputText( ' know' );
				if( CoC.flags[ kFLAGS.MARBLE_KIDS ] === 1 ) {
					MainView.outputText( 's herself.' );
				} else {
				...
				MainView.outputText( ' playing together once more.  A moment later, Marble puts her hand to her expansive backend again, before suddenly exclaiming again in realization.  "<i>Sweetie, watch our kid' );
				if( CoC.flags[ kFLAGS.MARBLE_KIDS ] > 1 ) {
					MainView.outputText( 's' );
				}
				MainView.outputText( ' for a moment, I\'ll be right back!</i>" and she leaves the camp as quickly as she can, without slipping and falling on her ass again.' );
				MainView.outputText( '\n\nYou certainly don\'t mind staying behind for now, it is certainly a lot of fun spending time with your kid' );
				if( CoC.flags[ kFLAGS.MARBLE_KIDS ] > 1 ) {
					MainView.outputText( 's' );
				}
				MainView.outputText( ' and just having fun.  Your cow-girl daughters are certainly not lacking in energy in the cold snow, their fur covered legs and built in shoes probably help a lot with this.  ' );
				if( CoC.flags[ kFLAGS.MARBLE_KIDS ] > 1 ) {
					MainView.outputText( 'They' );
				} else {
					MainView.outputText( 'She' );
				}
				MainView.outputText( ' listen' );
				if( CoC.flags[ kFLAGS.MARBLE_KIDS ] === 1 ) {
					MainView.outputText( 's' );
				}
				MainView.outputText( ' with rapt attention when you explain what snow is, and about all the things you use to do in it back home.  Hands-on demonstrations are also in high demand.' );
				...
				MainView.outputText( ' to ride on!  Happy cries of joy fill the air for some time, as you and Marble take turns in giving rides on the sled.' );
				//PC strength check:;
				//(>=85);
				if( CoC.player.str >= 85 ) {
					MainView.outputText( '\n\nIt would seem that your strength has won you more favor with ' );
					if( CoC.flags[ kFLAGS.MARBLE_KIDS ] > 1 ) {
						MainView.outputText( 'the girls' );
					} else {
						MainView.outputText( 'your daughter' );
					}
					MainView.outputText( ' when it comes to giving sled rides, Marble doesn\'t seem to mind too much though.  Your mate actually decides to join in the riding part after a while!' );
				}
				//(60-84);
					...
					MainView.outputText( ' in deciding who gives the best rides.  In the end, a clear winner is never decided, but everyone agrees that it was a lot of fun.' );
				}
				//(<=59);
				else {
		...
		//if she ain't a follower and is a lover, just have her visit.;
			...
			MainView.outputText( '"<i>Hey, champ, what\'s going on here?</i>"  Helia calls as she sees you approach.  Gentle trails of steam are wafting from the salamander\'s body as her own internal heat melts any snow that builds up on her.' );
			...
			if( CoC.player.gender > 0 ) {
				MainView.outputText( ', as evidenced by ' );
				if( CoC.player.hasCock() ) {
					MainView.outputText( 'your erect ' + CoC.player.multiCockDescriptLight() );
				}
				...
				if( CoC.player.hasVagina() ) {
					MainView.outputText( 'dripping [vagina]' );
				}
			}
			MainView.outputText( '.' );
			...
			//(If PC has a cock);
			if( CoC.player.hasCock() ) {
				MainView.outputText( '  A torrent of seed splashes against Helia\'s taut belly, as [eachCock] throbs and spews its seed while sandwiched between the two of you.' );
			}
			//(if PC has a pussy);
			if( CoC.player.hasVagina() ) {
				MainView.outputText( '  Your [vagina] quivers and spill your juices with the force of a wave.  Moisture falls on the ground to join in with Helia\'s own discharge.' );
			}
		...
		//Isabella;
			...
			MainView.outputText( 'The bovine adventurer Isabella is staring at the snow around her with a sad look on her face, one hand gently raised to catch snowflakes.  She watches them gather in her palm and heaves a huge sigh.  You quietly approach her and ask what\'s wrong; is she homesick?' );
			MainView.outputText( '\n\n"<i>' );
			if( SceneLib.isabellaFollowerScene.isabellaAccent() ) {
				MainView.outputText( 'Da, [name], I am,' );
			} else {
				MainView.outputText( 'Yes, [name], I am,' );
			}
			MainView.outputText( '</i>" she replies.  ' );
			if( SceneLib.isabellaFollowerScene.isabellaAccent() ) {
				MainView.outputText( '"<i>Back home, in mine land, it vould be snowing all over at this time of year, and ve vould be having big vinter festival to celebrate ze ending of old year and coming of new year.  This land, it is so dry and hot...  I had forgotten it, but seeing zis snow makes me remember it and all I have lost by being stuck here.</i>"  She sighs.' );
			} else {
		...
		//Kiha;
			...
			MainView.outputText( '"<i>Ah-ah-ahchoo!</i>"' );
			...
			if( CoC.player.hasCock() && CoC.player.cockThatFits( 67 ) >= 0 ) {
				EngineCore.addButton( 0, 'Fuck Her', this, this.kihaXmasFuck );
			}
			EngineCore.addButton( 1, 'Finger Her', this, this.kihaXmasFingering );
		}
		//Feast time! Just a collection of words on the pleasant day...;
		else {
			MainView.outputText( 'This small holiday of yours was much needed.  You feel reinvigorated and even more determined to put an end to this struggle with demons.' );
			...
			//(if no sexable follower);
				...
				MainView.outputText( '.  You can tell that you were not the only one needing this break... but unfortunately all good things must come to an end, and so you bid your companion' );
				if( SceneLib.camp.companionsCount() > 1 ) {
					MainView.outputText( 's' );
				}
				MainView.outputText( ' good night and retire for the night.' );
			}
			////more than one sexable follower.;
			else {
				MainView.outputText( 'You take care of the preparations and cooking, whipping up a delightful meal for you and your companions.  For a moment you look around - this small band you\'ve formed feels enough like a family that you wouldn\'t mind having more days like this in the future.  You vow to work extra hard to make this a reality.' );
	...
	XmasJackFrost.prototype.kihaXmasFuck = function() {
		...
		MainView.outputText( 'Stripping off your [armor], you tell the dragon-girl that a good workout is all she needs to get nice and warm.  She snorts in dismissal, but the way she\'s looking at you is a pretty good indication she\'s willing to try it.  "<i>So, what do you have in mind?</i>" she blusters.  You stroke your quickly erecting ' + CoC.player.multiCockDescriptLight() + ' and, smirking, ask her if your intentions aren\'t obvious?' );
	...
	XmasJackFrost.prototype.kihaXmasFingering = function() {
		MainView.clearOutput();
		MainView.outputText( 'You examine Kiha.  A small snowflake lands on one of her nipples and she yelps with a start at the sudden contact.   "<i>Hey, I asked you to help warm me up, not to stare at me!</i>" she complains. ' );
	...
	XmasJackFrost.prototype.penetradePureMouseWaifu = function() {
		MainView.clearOutput();
		MainView.outputText( 'Stripping down your [armor] and indicating Amily should do the same... you tell her that you were hoping for some good old-fashioned sex between a horny ' + CoC.player.mf( 'man', 'herm' ) + ' and ' + CoC.player.mf( 'his', 'her' ) + ' lovely mousette.  You playfully ask Amily if she has any objections?' );
		...
		if( CoC.flags[ kFLAGS.AMILY_WANG_LENGTH ] > 0 ) {
			MainView.outputText( ' and painting your belly white with futa-spooge' );
		}
		MainView.outputText( '.' );
	...
	XmasJackFrost.prototype.makeOutWithPureMouseWaifu = function() {
		...
		if( CoC.flags[ kFLAGS.AMILY_WANG_LENGTH ] > 0 ) {
			MainView.outputText( 'Smiling sheepishly you  tell her to mind the erection... though you\'re not actually looking for sex, you can\'t help with how cute she looks...  "<i>Flirt.</i>"  Amily giggles, but otherwise doesn\'t comment on it.  ' );
		}
		MainView.outputText( 'The mouse promptly walks over and sits down just in front of you, her back facing you and giving you a perfect opportunity to hug her.  Not keen on missing such an opportunity, you do so.' );
		if( CoC.player.hasCock() ) {
	...
	XmasJackFrost.prototype.penetradeCorruptJojoAss = function() {
		...
		if( CoC.player.cumQ() >= 700 ) {
			MainView.outputText( ' all over his own cum-engorged gut and ' );
		}
		MainView.outputText( 'onto the snowy earth underneath him.  He cums and cums until he\'s exhausted even his prodigious balls, creating a salty quagmire of semi-melted snow and spooge under him that he promptly collapses face-first into.' );
