...
angular.module( 'cocjs' ).run( function( MainView, SceneLib, OnLoadVariables, kFLAGS, Utils, PerkLib, ConsumableLib, AppearanceDefs, CoC, EngineCore ) {
	...
	/*Requirements:
	 ...
	 -Scene proc's at 12, overriding other scenes
	 Note on progression PC has, after each scene (Baste, Stuff, Spitroast) the option to say "<i>That's Enough</i>" or continue on to the next scene -- the scenes must be done in order, however.*/
	//const TURKEY_FUCK_YEAR_DONE:int = 566;;
	Thanksgiving.prototype.isThanksgiving = function() {
		return ((OnLoadVariables.date.date >= 21 && OnLoadVariables.date.month === 10) && (OnLoadVariables.date.date < 30 && OnLoadVariables.date.month === 10));
	};
	//Introduction: -McGirt;
	...
	Thanksgiving.prototype.shooTurkeyAway = function() {
		...
		MainView.outputText( 'You scowl and wave your arm at the strange turkey-girl, telling her to get lost.' );
	...
	Thanksgiving.prototype.noThanksTurkeyGal = function() {
		MainView.clearOutput();
		MainView.outputText( 'You reluctantly push her away.  You\'ve no need to ram your dick down some new monstrosity\'s gullet.  The girl forlornly gobbles one last time, then prances off into the fading evening light, globular ass jiggling.' );
	...
	Thanksgiving.prototype.spitRoastThatTurkey = function() {
		MainView.clearOutput();
	...
	//Main Course:;
	Thanksgiving.prototype.femaleAndTurkeyMainCourse = function() {
		MainView.clearOutput();
		MainView.outputText( 'Fully disrobed, the girl runs her hands down her chest, fingers clamping around her dribbling nipples once again.  With another squeeze, gravy is rolling down her chest once more, slathering her fingers with her own mouthwatering juices.  Unable to hold yourself back, your mouth clamps onto her breast, hands shooting to her soft, plush rear as you pull her bodily into your lap.  Her eyes almost light up in her new position, her hands running through your [hair] as you suck for all you\'re worth.  Her thick, rich gravy fills your mouth with its delicious mix of flavours.  There\'s so many different, wonderful tastes mixing together that you could almost believe you were back in Ingnam, gorging yourself on the gigantic spread that the village always strives to put on, regardless of how well the harvest went.' );
	...
	Thanksgiving.prototype.haveTurkeyGirlDesertInYourCunt = function() {
		MainView.clearOutput();
		MainView.outputText( 'Barely able to contain yourself, you nearly tear your [armor] from your burning body, desperate to feel the girl with your bare flesh.  She reclines lazily on the ground, legs spreading wide as a hand moves from her dribbling tits, sliding its way down her toned stomach to her swollen lips.  She continues to tease herself whilst you struggle with your clothes, little moans and grunts of pleasure leaving you weak at the knees as she dreamily circles her reddening snatch with a finger.  Her other hand busies itself with her chest, pinching and squeezing just hard enough to release another spurt of gravy that rolls its way down the curve of her breast, dripping onto her belly in a thick glob.' );
	...
	Thanksgiving.prototype.turkeyGirlTwoTheTurkeningBySavinWhatADickInAButt = function() {
		MainView.clearOutput();
	...
	Thanksgiving.prototype.helloAgain = function() {
		MainView.clearOutput();
		MainView.outputText( '"<i>Oh, hey, you,</i>" you say as the seasonal little near-harpy waddles up.  You pat your thigh invitingly, letting Gobbles approach.' );
		...
		if( CoC.player.cockArea( 0 ) <= 60 ) {
			MainView.outputText( 'and down to the base' );
		} else {
	...
	//Gravy Boat;
	Thanksgiving.prototype.gravyBoatDatTurkturk = function() {
		MainView.clearOutput();
		MainView.outputText( 'You cup the turkey\'s chin, pulling her up off your cock enough to get her looking up at you with her big, questioning eyes.  "<i>Gobble?</i>" she ventures nervously, straining against your hand to get back at your shaft.' );
		...
		if( CoC.player.cocks[ 0 ].cockThickness >= 5 ) {
			MainView.outputText( ', her stomach bulging obscenely with your inhuman girth' );
		}
		MainView.outputText( '.' );
	...
	Thanksgiving.prototype.turkeyDesertBitches = function() {
		MainView.clearOutput();
		MainView.outputText( '"<i>G-gobble!?</i>" Gobbles whines at your moment of hesitation, wiggling her upraised botty at you enticingly, hot flesh jiggling just close enough to brush your ' + CoC.player.multiCockDescriptLight() + ' with her lust-coated cheeks.' );
		...
		if( CoC.player.biggestTitSize() > 1 ) {
			MainView.outputText( ', cupping a tit' );
		}
		MainView.outputText( ' as the venom takes control, flushing your skin and forcing out all thoughts but one that turkey! There\'s nothing for it, now: you grab Gobbles\' hips and plunge in' );
		if( CoC.player.cockTotal() === 2 ) {
			MainView.outputText( ' with both cocks, one ramming into her abused cunny and the other right up her vulnerable little bumhole' );
		} else if( CoC.player.cockTotal() > 2 ) {
			MainView.outputText( ' with every cock you\'ve got, half in the pink and half in the stink, spreading her as wide as you are in both barrels' );
		}
		MainView.outputText( ', shocking her so much that she can\'t hold back, screaming and cumming on the spot! You groan, thrusting forward as her spasming muscles contract around your cock' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ', milking you as hard as she can while her orgasm overtakes her.  Gravy geysers out of her swollen tits as fem-cum splashes out around your deep-buried cock' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ', drenching your [hips] and [legs] in creamy girl-spunk, tinged pink by the poison you fist-fucked up her cunt.' );
		...
		if( CoC.player.cockArea( 0 ) >= 50 ) {
			MainView.outputText( 'her stomach is bulging obscenely with the sheer volume of cockflesh rammed up her cunt' );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 'and ass' );
			}
		} else {
			MainView.outputText( 'your hips are slapping into hers, making her butt bounce with the impact' );
		}
		MainView.outputText( '.  She cries her one word to the heavens as you pound into her hole' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ', thrusting your cock' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ' deep inside.  Her avian legs break free of her hold, wrapping around your [hips] and pulling you in, locking you in her spasming hole' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ' as she cums and cums, pleading, "<i>Gobble!</i>" at the top of her lungs.  You switch your grip to her bountiful bosom, roughly squeezing her tits as you hump your cock' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ' in and out of her slick slit{and quivering behind}, pinching her nipples to give the squealing turkey just a little more pleasure.' );
		...
		MainView.outputText( ', her legs spurring you on every time you withdraw, muscles clenching when you pause, never letting you rest.  Needy slut! But she\'s more than willing to do her part, thrusting her hips up to meet you, squeezing and milking your cock' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ' every time you pierce her, working your shaft' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ' so expertly that you know you won\'t be able to hold on for long - and neither will she, the way she\'s panting and gasping along with you!' );
		MainView.outputText( '\n\nYou draw nearer to the breaking point, hips unrelentingly slamming your shaft' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ' deep into Gobbles\' eager depths, urged on by her happy squawks and gleeful cries.  Lust-venom-addled as the two of you are, orgasm approaches like a wave of relief, crashing through your bodies in beautiful unison.  "<i>Gobbles!</i>" she cries as her cunt ' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 'and ass ' );
		}
		MainView.outputText( 'squirm around your cock' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ', wringing the cum from you with powerful spastic contraction.  Her arms pull you down into the valley of her gravy-laden bosom, squeezing your face between her massive mounds as she cums, drawing the spunk right out of you.  With a potent, feral roar, you cum, ' + CoC.player.multiCockDescriptLight() + ' blasting its salty load deep into Gobbles\' womb' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( ' and bowels' );
		}
		MainView.outputText( '; her back arches, teats wobbling around your head as she takes your cum, squawking and gobbling gaily as more and more of your virile spunk pours into her, mixing with her own fem-cum and the remnants of the lusty draft your fucked into her.' );
		MainView.outputText( '\n\n"<i>Gobble!</i>" she sighs happily as your cock' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 's deposit the last of their' );
		} else {
			MainView.outputText( ' deposits the last of its' );
		}
		MainView.outputText( ' cream into her, fully stuffing your turkey-girl.  Panting, you let your head rest between her tits a moment long, shaft' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ' wilting inside her until like a broken dam, her cunt' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( ' and ass' );
		}
		...
		MainView.outputText( ' to drool spunk, pooling onto the hungry ground with the gallons of spilt gravy and fem-spooge.' );
	...
	Thanksgiving.prototype.pigSlutRoastingGreet = function() {
		MainView.clearOutput();
		MainView.outputText( 'As you enter The Wet Bitch you’re accosted by an incredibly busty waitress. You can’t really discern what exactly she is as your eyes are solely focused on the massive melons jutting out from her chest and jiggling proudly with mile deep cleavage to boot.' );
	...
	Thanksgiving.prototype.getARoastPiggueOinkOinkOinkMotherfucker = function() {
		MainView.clearOutput();
		CoC.player.gems -= 1;
		MainView.outputText( 'You quickly approach the gooey bucket and toss a single, shiny gem inside. Another waitress winks at you, almost equally as busty, nodding her head in the direction of the back rooms.' );
		...
		MainView.outputText( 'our nostrils flare, taking in great amounts of the salacious stench.' );
	...
	Thanksgiving.prototype.tenderizeDatPigSloot = function() {
		MainView.clearOutput();
		MainView.outputText( '"<i>Turn over.</i>" you say rather plainly, eager to see that jiggling booty of hers.' );
		...
		if( CoC.player.tailType > AppearanceDefs.TAIL_TYPE_NONE ) {
			MainView.outputText( 'You even bring your tail from behind your back to join in, the unfamiliar texture causing the sausage slut to let out surprised, high-pitched squeaks. ' );
		}
		MainView.outputText( 'You notice the pigslut moving backwards, her ass being pushed into the air as she attempts to feel the full power of your spanking, her pussy spraying juices onto the soaked sheets.' );
	...
	Thanksgiving.prototype.tenderizeHerHardcore = function() {
		MainView.clearOutput();
		MainView.outputText( 'You can\'t help but maliciously smirk, your eyes resting on a fetish whip and some bondage equipment. You give the piggy one last double-handed spank on her ample ass, leaving her a pitifully moaning and quivering wreck. You quickly snatch up the whip, some rope, and a kinky ballgag.' );
	...
	Thanksgiving.prototype.hamRoastTenderizeHerSoftcore = function() {
		MainView.clearOutput();
		MainView.outputText( 'You can\'t help but let a salacious smile grace your lips, your eyes resting on a paddle in the corner. You give the piggy one last double-handed spank on her ample ass, leaving her a pitifully moaning and quivering wreck. You quickly snatch the somewhat heavy paddle, feeling its heft and grinning like a madman.' );
	...
	Thanksgiving.prototype.spankEpilogueColonYoureAHorriblePersonForAbusingADumbPigGirl = function() {
		MainView.clearOutput();
		MainView.outputText( 'Eventually you stop your ceaseless romping of her rear, the piggy whore blubbering into the now thoroughly torn pillow from her biting and tearing. Several strips of flesh from her reddened ass look practically raw from your activities. You lay an experimental finger on the tortured flesh, the blazing heat radiating from it telling you all you need to know.' );
	...
	Thanksgiving.prototype.getDoneEatingOutPigSlut = function() {
		MainView.clearOutput();
		MainView.outputText( 'Finally you remove yourself from the still slightly dribbling juicebox, giving the whore one last spank as she seems to have passed out from the bliss. The bed is entirely a mess, covered with her sticky juices and torn from the spanking.' );
		...
		if( CoC.player.thickness < 100 ) {
			MainView.outputText( '  You notice that you seem a bit more... jiggly, a slight paunch in your stomach now as you grow a bit more plump than you once were.' );
		...
		if( CoC.player.buttRating < 35 ) {
			MainView.outputText( '  You notice your ass filling out, gaining more mass as it visibly grows before your eyes.' );
		...
		if( CoC.player.hipRating < 35 ) {
			MainView.outputText( ' Your hips grow too, widening and giving you more sexy curves.' );
		...
		if( CoC.player.biggestTitSize() < 20 ) {
			MainView.outputText( ' Your boobs also grow, gaining size as it seems that every part of your body wants to join in on the growth.' );
		...
		MainView.outputText( '\n\nYou sigh' );
		if( changed ) {
			MainView.outputText( ', feeling your body expand as you waddle out back towards camp with belly full of sweet syrup' );
		}
		MainView.outputText( '. For only one gem, that was a pretty good time...' );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseTwoHours );
	};
