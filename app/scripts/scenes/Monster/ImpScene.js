...
angular.module( 'cocjs' ).run( function( SceneLib, MainView, ImpGang, ImpLord, EventParser, PerkLib, PregnancyStore, ImageManager, CockTypesEnum, Appearance, Combat, Descriptors, StatusAffects, AppearanceDefs, Utils, CoC, EngineCore, LustyMaidensArmor ) {
	...
	ImpScene.prototype.impVictory = function() {
		...
		var canFeed = (CoC.getInstance().player.findStatusAffect( StatusAffects.Feeder ) >= 0);
		var canBikiniTits = (CoC.getInstance().player.hasVagina() && CoC.getInstance().player.biggestTitSize() >= 4 && CoC.getInstance().player.armor instanceof LustyMaidensArmor);
		EngineCore.outputText( 'You smile in satisfaction as ' + CoC.getInstance().monster.a + CoC.getInstance().monster.short + ' collapses and begins masturbating feverishly.' );
		if( canFeed ) {
		...
		} else if( CoC.getInstance().player.lust >= 33 || canBikiniTits || CoC.getInstance().player.canOvipositBee() ) {
		...
		if( CoC.getInstance().player.lust > 33 ) {
			...
			if( CoC.getInstance().player.hasCock() ) {
				if( CoC.getInstance().player.cockThatFits( CoC.getInstance().monster.analCapacity() ) === -1 ) {
					EngineCore.outputText( '\n\n<b>You\'re too big to rape an imp with ' + CoC.getInstance().player.oMultiCockDesc() + '.</b>' );
				} else {
					maleRape = (CoC.getInstance().player.isTaur() ? this.centaurOnImpStart : this.rapeImpWithDick);
				}
			}
			...
			} else if( maleRape === null && !CoC.getInstance().player.hasFuckableNipples() && !canFeed && !canBikiniTits && !CoC.getInstance().player.canOvipositBee() ) {
				Combat.cleanupAfterCombat(); //Only happens when there's no way to fuck the imp
				return;
			}
			EngineCore.addButton( 0, (CoC.getInstance().player.isTaur() ? 'Centaur Rape' : 'Male Rape'), maleRape );
		...
		if( canBikiniTits ) {
			EngineCore.addButton( 4, 'B.Titfuck', CoC.getInstance().player.armor.lustyMaidenPaizuri );
		}
	...
	ImpScene.prototype.rapeImpWithDick = function() {
		...
		if( CoC.getInstance().player.cocks.length === 1 ) {
			...
			if( CoC.getInstance().player.lowerBody !== AppearanceDefs.LOWER_BODY_TYPE_CENTAUR ) {
				EngineCore.outputText( '  You casually unclasp your ' + CoC.getInstance().player.armorName + ' and reveal your ' + Descriptors.cockDescript( x ) + ', ', false );
				if( CoC.getInstance().player.breastRows.length > 0 && CoC.getInstance().player.breastRows[ 0 ].breastRating > 2 ) {
			...
			EngineCore.outputText( '\n\nWith no foreplay, you press your ' + Descriptors.cockDescript( x ) + ' against his tight little pucker and ram it in to the hilt.  The imp\'s eyes bulge in surprise even as a thick stream of pre leaks from his ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + '.  You grab him by his distended waist and brutally rape the little demon, whose claws stay busy adding to his pleasure.', false );
			...
			if( CoC.getInstance().player.cocks[ 0 ].cockLength >= 7 && CoC.getInstance().player.cocks[ 0 ].cockLength <= 12 ) {
			...
			if( CoC.getInstance().player.balls === 0 && CoC.getInstance().player.vaginas.length > 0 ) {
			...
			if( CoC.getInstance().player.cumQ() >= 14 && CoC.getInstance().player.cumQ() <= 30 ) {
			...
			if( CoC.getInstance().player.cumQ() > 30 && CoC.getInstance().player.cumQ() <= 100 ) {
		...
		if( CoC.getInstance().player.cocks.length >= 2 ) {
			EngineCore.outputText( 'With a demonic smile you grab the insensible imp and lift him from the ground by his neck.  The reduced airflow doesn\'t seem to slow his feverish masturbation at all, and only serves to make him harder.', true );
			if( CoC.getInstance().player.lowerBody !== AppearanceDefs.LOWER_BODY_TYPE_CENTAUR ) {
				EngineCore.outputText( '  You casually unclasp your ' + CoC.getInstance().player.armorName + ' and reveal your ' + Descriptors.multiCockDescriptLight() + ', ', false );
				if( CoC.getInstance().player.breastRows.length > 0 && CoC.getInstance().player.breastRows[ 0 ].breastRating > 2 ) {
			...
			EngineCore.outputText( '\n\nWith no foreplay, you press a ' + Descriptors.cockDescript( x ) + ' against his tight little pucker and ram it in to the hilt.  The imp\'s eyes bulge in surprise even as a thick stream of pre leaks from his ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + '.  You grab him by his distended waist and brutally rape the little demon, whose claws stay busy adding to his pleasure.', false );
			if( CoC.getInstance().player.cocks[ 0 ].cockLength >= 7 && CoC.getInstance().player.cocks[ 0 ].cockLength <= 12 ) {
				EngineCore.outputText( '  Each thrust obviously distorts the imp\'s abdomen.  It amazes you that it doesn\'t seem to be hurting him.', false );
			}
			if( CoC.getInstance().player.cocks[ 0 ].cockLength > 12 && CoC.getInstance().player.cocks[ 0 ].cockLength <= 18 ) {
				EngineCore.outputText( '  Each plunge into the imp\'s tight asshole seems to distort its entire body, bulging obscenely from its belly and chest.  Amazingly he doesn\'t seem to mind, his efforts focused solely on his sorely throbbing demon-dick.', false );
			}
			EngineCore.outputText( '\n\nThe tight confines of the imp\'s ass prove too much for you, and you feel your orgasm build.', false );
	...
	ImpScene.prototype.rapeImpWithPussy = function() {
		...
		if( CoC.getInstance().player.vaginalCapacity() < CoC.getInstance().monster.cockArea( 0 ) ) {
			...
			EngineCore.outputText( '\n\nYou lose track of time as you languidly pump against the imp\'s ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + '.  At long last you feel your ' + Descriptors.vaginaDescript( 0 ) + ' ripple and quiver.  Your ' + CoC.getInstance().player.legs() + ' give out as you lose your muscle control and collapse against the small demon.  You gasp as his ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + ' erupts against you, splattering your chest with hot demonic cum that rapidly soaks into your skin.  You giggle as you rise up from the exhausted imp, feeling totally satisfied.', false );
		}
		//Big enough!;
		else {
			EngineCore.outputText( '  You sink down his ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + ' slowly, delighting in the gradual penetration and the tingling feeling of his dripping hot pre-cum.  At last you bottom out on his balls.', false );
			CoC.getInstance().player.cuntChange( CoC.getInstance().monster.cockArea( 0 ), true );
			EngineCore.outputText( '  Your lust and desire spurs you into movement, driving you to bounce yourself up and down on the ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + '.  His exquisite member pushes you to the very height of pleasure, your ' + Descriptors.vaginaDescript( 0 ) + ' clenching tightly of its own accord each time you bottom out.  The tensing of the little demon\'s hips is the only warning you get before he cums inside you, hot demonic jizz pouring into your womb.  Your ' + CoC.getInstance().player.legs() + ' give out, pushing him deeper as he finishes filling you.', false );
			EngineCore.outputText( '\n\nThe two of you lay there a moment while you recover, at last separating as you rise up off his ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + '.  Spunk drips down your legs, quickly wicking into your skin and disappearing.', false );
	...
	ImpScene.prototype.sprocketImp = function() {
		...
		EngineCore.outputText( 'and you find yourself dragged upward with him, soaring over the tops of the trees.  The cool rush of air does nothing to abate your arousal.  If anything, the cold shock only makes your body more aware of its own need.  After just a few seconds that feel like an eternity to your lust-filled being, the imp hurls you down into a tree.  You flail as you fall, barely catching yourself on the upper branches.  Your hands and ' + CoC.getInstance().player.legs() + ' are tangled in the smooth wooden spiderweb below you, your mind torn between desire for the imp above and fear of the fall below.  You can see from the gleam in the horned creature\'s red eyes that he has you right where he wants you.\n\n', false );
		//New PG;
		...
		//Variable cocktext;
		if( CoC.getInstance().player.cocks[ 0 ].cockType === CockTypesEnum.HUMAN || CoC.getInstance().player.cocks[ 0 ].cockType === CockTypesEnum.DEMON || CoC.getInstance().player.cocks[ 0 ].cockType.Index > 4 ) {
		...
		if( CoC.getInstance().player.skinDesc === 'fur' ) {
			EngineCore.outputText( 'furry cock-sleeve', false );
		} else {
			if( CoC.getInstance().player.skinTone === 'purple' || CoC.getInstance().player.skinTone === 'blue' || CoC.getInstance().player.skinTone === 'shiny black' ) {
		...
		if( CoC.getInstance().player.breastRows.length > 0 && CoC.getInstance().player.biggestTitSize() > 2 ) {
			EngineCore.outputText( ', fucking you so hard that your ' + CoC.getInstance().player.allBreastsDescript() + ' bounce with each thrust.  ', false );
		} else {
	...
	ImpScene.prototype.centaurGirlOnImps = function() {
		...
		if( CoC.getInstance().player.hasVagina() ) {
			...
			//[Has breasts: ;
			if( CoC.getInstance().player.biggestTitSize() > 1 ) {
				EngineCore.outputText( 'He slides up to your torso and grasps your wildly flailing ' + CoC.getInstance().player.allBreastsDescript() + ', massaging them harshly. His ministrations are surprisingly crude, and you wonder how many times he\'s attempted to pleasure a woman.', false );
				...
				if( CoC.getInstance().player.biggestLactation() >= 1 && CoC.getInstance().player.lactationQ() < 50 ) {
	...
	ImpScene.prototype.centaurOnImpMale = function( vape ) {
		...
		EngineCore.outputText( 'As your shadow falls over the imp, it looks between your ' + CoC.getInstance().player.legs() + ' with a hint of fear.  ', false );
		if( CoC.getInstance().player.cockArea( x ) <= 15 ) {
			EngineCore.outputText( 'Relief washes over it followed by intense lust as is throws itself onto a mossy rock and eagerly presents its ' + Appearance.assholeDescript(CoC.getInstance().monster) + '.   The sound of your hooves moving on either side of its body seems to send the creature into a frenzy as it begins humping the air while small mewling sounds escape its lips.  ', false );
			//<<Cor <50>> ;
			if( CoC.getInstance().player.cor < 50 ) {
				EngineCore.outputText( 'You slowly rub your ' + Descriptors.cockDescript( x ) + ' between the creature\'s cheeks, letting your pre-cum oil the small hole, before slowly beginning the insertion.  Before you can get half-way the creatures drives its self back against you, impaling its ' + Appearance.assholeDescript(CoC.getInstance().monster) + ' around your ' + Descriptors.cockDescript( x ) + ' and making inhuman sounds of ecstasy. The ' + Appearance.assholeDescript(CoC.getInstance().monster) + ' relaxes around your ' + Descriptors.cockDescript( x ) + ', taking it all in while its practiced muscles grip and jerk you off internally.\n\n', false );
			}
			//<<Cor 50+>> ;
			else {
		...
		if( CoC.getInstance().player.cockTotal() > 1 && CoC.getInstance().player.cocks[ CoC.getInstance().player.biggestCockIndex() ].cockLength >= 24 ) {
			EngineCore.outputText( 'As your shadow falls over it, it looks with a hint of fear between your legs, and then its eyes widen in a mixture of apprehension and lust.  Before you can even more the little creatures scrambles forward between your hooves and wraps its hands around your ' + Descriptors.cockDescript( CoC.getInstance().player.biggestCockIndex() ) + '.  Its tongue begins to trail all along the length of it as its small hands stroke it intensely.\n\n', false );
			//<< Cor <50>>;
			if( CoC.getInstance().player.cor < 50 ) {
				EngineCore.outputText( 'You slowly undulate your ' + Descriptors.cockDescript( CoC.getInstance().player.biggestCockIndex() ) + ' against the creature\'s mouth, delighting in its eager tongue.  ', false );
				//<<GoTo I3 then return>> ;
				this.centaurOnImpResults( 3 );
				EngineCore.outputText( 'The sounds beneath you quickly take on a more intense note and you feel massive amounts of cum splashing liberally over your hooves, belly, and ' + Descriptors.cockDescript( CoC.getInstance().player.biggestCockIndex() ) + '.  The hot sensation sends you over the edge as you begin spilling yourself into the creature\'s eager mouth.\n\n', false );
				//<<GoTo I2>>;
				this.centaurOnImpResults( 2 );
				//<<End>>;
				CoC.getInstance().player.orgasm();
			...
			//<< 1 or 2 cocks, Cor 50+>>;
				...
				EngineCore.outputText( 'With an evil smile you wait for your ' + Descriptors.cockDescript( CoC.getInstance().player.smallestCockIndex() ) + ' to be at its lips before you slide it forward into its waiting mouth.  Giving it little more than a moment to catch its breath you slide your ' + Descriptors.cockDescript( CoC.getInstance().player.smallestCockIndex() ) + ' further and down the creature\'s throat.  Though you cannot see the obscene bulge it is making in the creature\'s mouth-pussy you delight in the intense tightness beneath you.  The throat muscles are massaging your ' + Descriptors.cockDescript( CoC.getInstance().player.smallestCockIndex() ) + ' as the imp desperately scrambles for air, pulling at the tentacles you have forced into it.  It cannot even begin to close its jaw as you thrust deeper and deeper, as you try to intensify the sensations.\n\n', false );
				EngineCore.outputText( 'As the imp is focused on the tentacles cutting off its air, you position your ' + Descriptors.cockDescript( CoC.getInstance().player.biggestCockIndex() ) + ' against its ' + Appearance.assholeDescript(CoC.getInstance().monster) + '.  Pausing only for a second for the pleasure of anticipation, you shove yourself deep inside the imp\'s ' + Appearance.assholeDescript(CoC.getInstance().monster) + ', only making it a few inches before having to pull back and try again.  The creature\'s throat seems to be working overtime now as it tries to divide its attention between the two invaders.  Each thrust of your ' + Descriptors.cockDescript( CoC.getInstance().player.smallestCockIndex() ) + ' makes it a little bit deeper inside of the creature, and you wonder passionately if you can get the two to meet in the middle.\n\n', false );
				...
				//<<Cor 80+>> ;
					...
					EngineCore.outputText( 'You groan in pleasure and slide your ' + Descriptors.cockDescript( CoC.getInstance().player.biggestCockIndex() ) + ' even deeper down the creature\'s throat, until you can feel its head against your ', false );
					//<<if balls>>;
					if( CoC.getInstance().player.balls > 0 ) {
					...
					EngineCore.outputText( 'you begin to cum deep and directly into the imp\'s stomach and ' + Appearance.assholeDescript(CoC.getInstance().monster) + '.  ', false );
					//<<cum multiplier: lots>>;
					...
					EngineCore.outputText( 'With on last grunt you begin extracting the tentacles back out, almost cumming again from the tightness around them.  You give your ' + Descriptors.cockDescript( CoC.getInstance().player.smallestCockIndex() ) + ' one last shake over the creature\'s face before trotting away satisfied and already thinking about the next creature you might abuse.', false );
					CoC.getInstance().player.orgasm();
					Combat.cleanupAfterCombat();
					return;
				}
			...
			//<< 3+ cocks, Cor 80+>>;
				...
				EngineCore.outputText( 'With an evil smile you wait for the creature\'s mouth to touch one of your tentacles before the other two snake their way down and wrap themselves around the imp\'s thighs.  With a tug the creatures is pulled off of it\'s feet and upside down, its eyes widening in a mixture of fear and debased lust as it sees your ' + Descriptors.cockDescript( CoC.getInstance().player.biggestCockIndex() ) + ' undulating in front of it.  You slowly move the tentacle up as your other cocks forcefully tug its legs apart, and then playfully begin sliding yourself over the imp\'s small cheeks.\n\n', false );
	...
	ImpScene.prototype.centaurOnImpFemale = function( vape ) {
		...
		CoC.getInstance().player.slimeFeed();
		CoC.getInstance().player.knockUp( PregnancyStore.PREGNANCY_IMP, PregnancyStore.INCUBATION_IMP );
		EngineCore.outputText( 'As the imp lays beaten its hands stroke its ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + ' as its eyes look over you in the hope that you might abuse it in some manner.  You lick your lips as you stare at the large member and you turn around to display your ' + Descriptors.vaginaDescript( 0 ) + '.  ', false );
		//Not gaping?;
		if( CoC.getInstance().player.vaginas[ 0 ].vaginalLooseness <= AppearanceDefs.VAGINA_LOOSENESS_GAPING ) {
			...
			//Not virgin fucking flavors;
				...
				//<<At Dicksize>>;
				if( CoC.getInstance().player.vaginalCapacity() >= CoC.getInstance().monster.cockArea( 0 ) && CoC.getInstance().player.vaginalCapacity() <= CoC.getInstance().monster.cockArea( 0 ) * 1.25 ) {
				...
				if( CoC.getInstance().player.vaginalCapacity() >= CoC.getInstance().monster.cockArea( 0 ) * 1.25 ) {
			...
			EngineCore.outputText( 'When the creature completely bottoms out inside of you, you begin trotting forward with a wicked grin.  The creature\'s hands grasp your flanks desperately, and its ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + ' bounces inside your ' + Descriptors.vaginaDescript( 0 ) + ', adding to your sensation.  The movement is causing the imp to push himself even harder against you as it tries to not fall off, and it is all you can do to keep an eye on where you are going.  Soon you can feel the imp\'s sperm filling your ' + Descriptors.vaginaDescript( 0 ) + ' and overflowing even as your cunt-muscles try to milk it of all of its seed. Unsatisfied you begin to speed up as you use its ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + ' to bring about your own orgasm.  The small creature is unable to let go without hurting itself.  It hangs on desperately while you increase the pace and begin making short jumps to force it deeper into you.  The feeling of sperm dripping out and over your ' + Descriptors.clitDescript() + ' pushes you over and cry out in intense pleasure.  When you finally slow down and clear your head the imp is nowhere to be seen.  Trotting back along the trail of sperm you left behind you find only its small satchel.', false );
			CoC.getInstance().player.cuntChange( CoC.getInstance().monster.cockArea( 0 ), true, true, false );
			CoC.getInstance().player.orgasm();
			Combat.cleanupAfterCombat();
			return;
		...
		//<<Gaping>>;
			...
			//<<Cor 50+, Breasts>>;
			if( CoC.getInstance().player.cor >= 50 && CoC.getInstance().player.biggestTitSize() >= 2 ) {
				EngineCore.outputText( 'You release the tree as you begin playing with your ' + CoC.getInstance().player.allBreastsDescript(), false );
				//<<w/ nip-pussies>>;
				if( CoC.getInstance().player.hasFuckableNipples() ) {
			...
			EngineCore.outputText( 'The imp is pushing deeper and deeper and in moments you cry out again as you feel first its hooves and then its ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + ' tearing open your cervix and bottoming out in your womb.  ', false );
			//<<Asshole large+>>;
			if( CoC.getInstance().player.analCapacity() >= 35 ) {
				...
				if( CoC.getInstance().player.ass.analLooseness < 4 && CoC.getInstance().player.cor < 80 ) {
		...
		if( iNum === 1 ) {
			...
			if( CoC.getInstance().player.cumQ() >= 250 ) {
				...
				if( CoC.getInstance().player.cocks[ x ].cockType !== CockTypesEnum.DOG ) {
					EngineCore.outputText( 'Soon the amount is overflowing from the abused ' + Appearance.assholeDescript(CoC.getInstance().monster) + ', dripping between you with no sign of stopping as you continue thrusting yourself into the imp.  ', false );
				}
				//<<knot>>;
				else {
					EngineCore.outputText( 'Soon the abused ' + Appearance.assholeDescript(CoC.getInstance().monster) + ' is full to the brim, though your knot keeps any from escaping while more and more pumps in.  Soon the creature\'s belly is distending and the imp is gasping wordlessly. ', false );
				}
			...
			//<<cum multiplier: little-normal>>;
				...
				EngineCore.outputText( 'With a last thrust into the cum receptacle you begin slowing down, even as its own ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + ' spills its seed over the ground.  ', false );
				//<<I1_1>>;
				//<<2 cocks>>;
				if( CoC.getInstance().player.cockTotal() === 2 ) {
					EngineCore.outputText( 'Your other cock drenches the imp\'s back with its own secretions that immediately start dripping down its sides.  ', false );
	...
	ImpScene.prototype.areImpsLactoseIntolerant = function() {
		EngineCore.outputText( '', true );
		EngineCore.outputText( 'You advance on the masturbating imp, baring your ' + CoC.getInstance().player.allBreastsDescript() + ' and swinging them from side to side. The little creature watches them, mesmerized as he masturbates his foot-long erection.\n\n', false );
	...
	ImpScene.prototype.impGangabangaEXPLOSIONS = function() {
		...
		CoC.getInstance().monster = new ImpGang();
		EngineCore.outputText( '\n', false );
		EngineCore.outputText( '<b>You sleep uneasily. A small sound near the edge of your camp breaks into your rest and you awaken suddenly to find yourself surrounded by ' + CoC.getInstance().monster.a + '</b>!\n\n', false );
		//CENTAUR;
		if( CoC.getInstance().player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_CENTAUR ) {
			if( Utils.rand( 2 ) === 0 && (CoC.getInstance().player.cockTotal() === 0 || CoC.getInstance().player.gender === 3) ) {
				//(First encounter);
				if( CoC.getInstance().player.findStatusAffect( StatusAffects.ImpGangBang ) < 0 ) {
					EngineCore.outputText( 'The imps stand anywhere from two to four feet tall, with scrawny builds and tiny demonic wings. Their red and orange skin is dirty, and their dark hair looks greasy. Some are naked, but most are dressed in ragged loincloths that do little to hide their groins. They all have a ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + ' as long and thick as a man\'s arm, far oversized for their bodies. Watching an imp trip over its ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + ' would be funny, if you weren\'t surrounded by a horde of leering imps closing in from all sides...\n\n', false );
					CoC.getInstance().player.createStatusAffect( StatusAffects.ImpGangBang, 0, 0, 0, 0 );
					EngineCore.outputText( 'The imps leap forward just as you start to ready your ' + CoC.getInstance().player.weaponName + ', one sweaty imp clinging to your arm', false );
					//(If the player has a weapon);
					if( CoC.getInstance().player.weaponName !== 'fists' ) {
						EngineCore.outputText( ' while another kicks your weapon out of reach', false );
					}
					EngineCore.outputText( '.  The ' + CoC.getInstance().monster.short + ' surges forward and grapples you. Imps grope your body and hump their ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + ' against your horse legs, smearing their sweat and pre-cum into your ' + CoC.getInstance().player.skinDesc + '. The rest of the ' + CoC.getInstance().monster.short + ', a dozen or more imps, all leer at you and laugh as they slap and pinch your body. The imps have sharp claws, tiny sharp teeth, and short horns on their heads. They scratch, claw, and bite at you with all of these weapons as they try to pull you down to the ground. One bold imp leaps forward and grabs your ', false );
					...
					EngineCore.outputText( ', twisting and pinching hard enough to make you yelp in pain. An imp leaps up and mounts you, grabbing your ' + Descriptors.hairDescript() + ' like reins. The long flesh of his ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + ' rubs against the small of your back. The ' + CoC.getInstance().monster.short + ' stinks of sweat and pre-cum, its moist grip and obscene smirk leaves you with no doubt as to what they will do to you if you lose this fight.\n\n', false );
				}
				EngineCore.outputText( 'The horde drags you to your knees, grappling your legs and crawling over your horse-body to pin you down. You try to buck them off but there are too many to fight. The imps drag your arms behind your back, wrapping them around your rider. Another imp whips off his loincloth to reveal his pre-cum drooling ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + ' and tosses the cloth to the imps holding your arms. They quickly tie your arms back with the sweat-damp loincloth.  ', false );
				//(If the player has breasts);
				if( CoC.getInstance().player.biggestTitSize() > 1 ) {
					EngineCore.outputText( 'Having your arms tied behind your back forces your chest out, making your ' + CoC.getInstance().player.allBreastsDescript() + ' stand out. They bounce as you struggle.  ', false );
				}
				EngineCore.outputText( 'The ' + CoC.getInstance().monster.short + ' stroke themselves and rub their hands over your outstretched chest, smearing their pre-cum into your skin. The imp riding you bounces up and down, rubbing his sweaty ' + CoC.getInstance().monster.ballsDescriptLight() + ' against your ' + CoC.getInstance().player.skinDesc + ' while he yanks your hair.  ', false );
				//(Low Corruption);
				if( CoC.getInstance().player.cor < 50 ) {
				...
				if( CoC.getInstance().player.biggestTitSize() > 1 ) {
					...
					EngineCore.outputText( '  Suddenly you feel tiny needle-sharp teeth pierce your ' + Descriptors.nippleDescript( 0 ) + '. You scream as venom pumps into your tits, red-hot poison that makes your ' + CoC.getInstance().player.allBreastsDescript() + ' feel as though they were being stung by bees. You moan in pain as your breasts start to swell, the imps continuing to pump demon-taint into them.\n\n', false );
					//Grow tits!;
					CoC.getInstance().player.growTits( 2, CoC.getInstance().player.breastRows.length, false, 1 );
					CoC.getInstance().player.boostLactation( 0.3 );
				}
				EngineCore.outputText( 'Dimly through your haze of lust and pain you see a large imp step forward from the mob. Four feet tall and broader and stronger looking than any imp you\'ve seen before, with a face as much bull as imp, this new imp has mottled grey skin, broad purple demon wings, two curving bull-horns on his head, and a ' + Appearance.cockNoun( CockTypesEnum.HORSE ) + ' big enough to choke a minotaur. The mushroom-like head of it bobs just below his mouth, and his snake-tongue darts out to flick a bit of pre-cum off the head and onto your face. You shudder as the hot fluid stings the sensitive skin of your lips. His ' + CoC.getInstance().monster.ballsDescriptLight() + ' are each the size of your fist and slick with sweat. He slaps his sweaty cock-head against your cheek, nearly scalding you with the heat.  ', false );
				//(Low corruption);
				if( CoC.getInstance().player.cor < 50 ) {
				...
				//(If the character has breasts);
				if( CoC.getInstance().player.biggestTitSize() > 2 ) {
					EngineCore.outputText( 'The big imp grabs one of your painfully distended breasts in each hand, mauling and bouncing the flesh as if weighing them. You gasp in pain as your ' + CoC.getInstance().player.allBreastsDescript() + ' swell further at his touch. ', false );
				}
				...
				//(If the character has breasts);
				if( CoC.getInstance().player.biggestTitSize() > 2 ) {
					EngineCore.outputText( 'The other imps reclaim your aching breasts, sucking your ' + Descriptors.nippleDescript( 0 ) + ' and mauling your ' + CoC.getInstance().player.allBreastsDescript() + ' so hard their fingers disappear into your swelling flesh. ', false );
				}
				...
				CoC.getInstance().player.cuntChange( CoC.getInstance().monster.cockArea( 1 ), true );
				...
				//(If the character has breasts);
				if( CoC.getInstance().player.biggestTitSize() >= 2 ) {
					EngineCore.outputText( 'your ' + CoC.getInstance().player.allBreastsDescript() + ' roll on your chest as you squirm', false );
				...
				//(If the character has breasts);
				if( CoC.getInstance().player.biggestTitSize() >= 2 ) {
					EngineCore.outputText( 'The slow fucking shakes your breasts, and the imps sucking at your nipples cling tightly to your monstrously swollen ' + CoC.getInstance().player.allBreastsDescript() + '. Your ' + Appearance.biggestBreastSizeDescript() + ' have grown three cup sizes since the imps pumped their venom into you. An ache starts deep in the base of your tits and works its way to your sore ' + Descriptors.nippleDescript( 0 ) + '. Your already bloated nipples swell as the imps suckle and you gasp as the first rush of milk spills into their mouths. Your rider reaches around and starts to milk your udders, moving his hands between your ' + CoC.getInstance().player.allBreastsDescript() + ' and forcing out more milk for his gangmates.\n\n', false );
				}
				...
				if( CoC.getInstance().player.biggestTitSize() > 3 ) {
					EngineCore.outputText( 'Your udders shake back and forth under your chest in time to the rough fucking. You arch your back to press your ' + Descriptors.nippleDescript( 0 ) + ' into eager mouths, moaning as your rider milks your distended ' + CoC.getInstance().player.allBreastsDescript() + '. ', false );
				}
				//(Low Corruption).;
				if( CoC.getInstance().player.cor < 50 ) {
				...
				EngineCore.outputText( '\n\n', false );
				EngineCore.outputText( 'The master imp pounds into you as hard as he can, driving his ' + CoC.getInstance().monster.cockDescriptShort( 1 ) + ' deeper into your cunt. His grunts come closer and closer together. Your rider grinds his cock into your back, rubbing his cock-head in your hair. He nips at your neck and shoulder as he pants. The master imp pounds into you and you can feel his ' + CoC.getInstance().monster.ballsDescriptLight() + ' swell as they slap against you. Through the haze of your approaching orgasm you realize what\'s about to happen. Those oversized balls are about to pump more cum into you than any normal man could ever produce. They\'re going to pump demonic cum right into your womb. ', false );
				//(Low Corruption);
				if( CoC.getInstance().player.cor < 50 ) {
				...
				CoC.getInstance().player.knockUp( PregnancyStore.PREGNANCY_IMP, PregnancyStore.INCUBATION_IMP - 14 ); //Bigger imp means faster pregnancy
			}
			//Scene number 2 - male possible.;
				...
				//(Low Corruption);
				if( CoC.getInstance().player.cor < 50 ) {
					EngineCore.outputText( 'swing your ' + CoC.getInstance().player.weaponName + ' wildly, determined not to let them take you', false );
				}
				//(High Corruption);
				else {
					EngineCore.outputText( 'twist and struggle in their grips, determined to make them work for their fun', false );
				}
				EngineCore.outputText( '! You kick back and feel your hooves smash into an imp\'s chest, sending him flying. But the ' + CoC.getInstance().monster.short + ' has your legs and more imps grab your arms. The pack drags you thrashing and bucking over to an old log lying on the ground.\n\n', false );
				EngineCore.outputText( 'Your human torso is dragged down to the log by ' + CoC.getInstance().monster.a + ' while two more leap onto your back. The ' + CoC.getInstance().monster.short + ' makes short work of your ' + CoC.getInstance().player.armorName + ', unbuckling straps and stripping you quickly. ', false );
				//(If the player has breasts);
				if( CoC.getInstance().player.biggestTitSize() > 0 ) {
					EngineCore.outputText( 'Your unbound ' + Appearance.biggestBreastSizeDescript() + ' bounce out over the weathered log. ', false );
				}
				EngineCore.outputText( 'The imps spread your arms wide, forcing your chest out, and tie them to the log with sweaty loincloths. Your ' + Descriptors.hipDescript() + ' are stuck high in the air. Imps rub their sweaty cocks and ' + CoC.getInstance().monster.ballsDescriptLight() + ' over your legs and grope your crotch. The two imps riding your back start stroking and licking each other. ', false );
				//(Low Corruption);
				if( CoC.getInstance().player.cor < 50 ) {
				...
				EngineCore.outputText( '\n\n', false );
				EngineCore.outputText( 'An imp quickly climbs up your body, planting his feet on your shoulders and grabbing your ' + Descriptors.hairDescript() + ' with one hand for support. He rubs his ' + CoC.getInstance().monster.ballsDescriptLight() + ' over your mouth, smearing your lips with musky sweat, while he pries at your jaw with his other hand. ', false );
				//(If the player has breasts);
				if( CoC.getInstance().player.biggestTitSize() > 2 ) {
					EngineCore.outputText( 'An imp mounts the log and slaps his ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + ' between your ' + CoC.getInstance().player.allBreastsDescript() + ', squeezing them tight over his cock as he rubs back and forth. He mauls your breasts cruelly, squeezing his fingers deep into your soft flesh.  ', false );
				}
				...
				//(If the player has breasts);
				if( CoC.getInstance().player.biggestTitSize() > 2 ) {
					EngineCore.outputText( 'The imp fucking your ' + Appearance.biggestBreastSizeDescript() + ' handles your soft flesh roughly, pressing and pulling your tits into a fuck-canal for his demon cock. Other imps slap your ' + CoC.getInstance().player.allBreastsDescript() + ' and laugh as you cry out.  ', false );
				}
				//(Low Corruption);
				if( CoC.getInstance().player.cor < 50 ) {
					EngineCore.outputText( 'You whimper as your mistreated flesh stings with dozens of pin-prick scratches and bites, and the ' + CoC.getInstance().monster.short + ' slaps your chest and flanks. The abuse falls on you from all sides, leaving you with no escape. The imp on your shoulders pries your jaws open, and you gag on his ' + CoC.getInstance().monster.ballsDescriptLight() + '.', false );
				}
				//(High Corruption);
				else {
				...
				EngineCore.outputText( ' until you shriek. The imp riding your shoulders plunges his ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + ' into your mouth, pounding at the top of your throat.\n\n', false );
				...
				//(Low Corruption);
				if( CoC.getInstance().player.cor < 50 ) {
					EngineCore.outputText( 'You try to pull away from the hot cock-head rubbing against your hole, but the ' + CoC.getInstance().monster.short + ' holds you tight.', false );
				}
				//(High Corruption);
				else {
				...
				//(If the character has breasts);
				if( CoC.getInstance().player.biggestTitSize() > 2 ) {
					EngineCore.outputText( 'The rough fucking shakes your cum-stained breasts, and the imp fucking your ' + CoC.getInstance().player.allBreastsDescript() + ' clings tightly to your red and swollen tit flesh. Your ' + Appearance.biggestBreastSizeDescript() + ' burn with agony as the ' + CoC.getInstance().monster.short + ' slaps your tits like drums.  ', false );
				}
				//(Low Corruption);
				if( CoC.getInstance().player.cor < 50 ) {
				...
				EngineCore.outputText( 'and your body rests in a cooling pool of cum. You couldn\'t move even if your ' + CoC.getInstance().player.legs() + ' felt stronger. Your hideously bloated belly weighs you down, quivering with every orgasmic twitch that passes through you. The skin of your distended belly is drum-tight and shiny. As you slip back into unconsciousness, one last thought flits across your mind. ', false );
				//(Low Corruption);
				if( CoC.getInstance().player.cor < 50 ) {
				...
				//Stretch!;
				if( CoC.getInstance().player.hasVagina() ) {
					if( CoC.getInstance().player.cuntChange( CoC.getInstance().monster.cockArea( 2 ), true ) ) {
						EngineCore.outputText( '\n', false );
					}
				} else {
					if( CoC.getInstance().player.buttChange( CoC.getInstance().monster.cockArea( 2 ), true ) ) {
						EngineCore.outputText( '\n', false );
					}
				}
			}
		...
		//NOT CENTAUR;
			...
			if( Utils.rand( 2 ) === 0 && (CoC.getInstance().player.cockTotal() === 0 || CoC.getInstance().player.gender === 3) ) {
				...
				EngineCore.outputText( '. The ' + CoC.getInstance().monster.short + ' surges forward and grapples you. Imps grope your body and hump their ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + ' against your legs, smearing their sweat and pre-cum into your ' + CoC.getInstance().player.skinDesc + '. The rest of the ' + CoC.getInstance().monster.short + ', a dozen or more imps, all leer at you and laugh as they slap and pinch your body. The imps have sharp claws, tiny sharp teeth, and short horns on their heads. They scratch, claw, and bite at you with all of these weapons as they try to pull you down to the ground. One bold imp leaps forward and grabs your ', false );
				//(If the player has a cock);
				if( CoC.getInstance().player.cockTotal() > 0 ) {
					EngineCore.outputText( Descriptors.cockDescript( 0 ), false );
				} else {
				...
				EngineCore.outputText( '\n\n', false );
				EngineCore.outputText( 'The ' + CoC.getInstance().monster.short + ' snickers lewdly as your nipples harden and your pussy moistens. One of the imps between your legs slides his shaft along your pussy lips, teasing your ' + Descriptors.clitDescript() + ' with the tip of his cock.  ', false );
				//(Low corruption);
				if( CoC.getInstance().player.cor < 50 ) {
				...
				//(High corruption);
					...
					EngineCore.outputText( 'You buck your hips, trying to capture his ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + ' with your ' + Descriptors.vaginaDescript( 0 ) + '.  ', false );
				}
				...
				EngineCore.outputText( '. His ' + CoC.getInstance().monster.ballsDescriptLight() + ' are each the size of your fist and slick with sweat. He slaps his sweaty balls against your ' + Descriptors.vaginaDescript( 0 ) + ' nearly scalding you with the heat.  ', false );
				//(Low corruption);
				...
				//(If the character has breasts);
				if( CoC.getInstance().player.biggestTitSize() >= 3 ) {
					EngineCore.outputText( 'Your ' + CoC.getInstance().player.allBreastsDescript() + ' bounce and jiggle back and forth as the master imp roughly fucks you.  ', false );
				}
				//(If the character has a dick);
				if( CoC.getInstance().player.totalCocks() > 0 ) {
					EngineCore.outputText( 'Your ' + Descriptors.multiCockDescriptLight() + ' swell painfully as the rough fucking pumps blood into your groin.  ', false );
				...
				EngineCore.outputText( '  You moan as you rise towards your orgasm.\n\n', false );
				EngineCore.outputText( 'The master imp pounds at you as hard as he can, driving his ' + CoC.getInstance().monster.cockDescriptShort( 1 ) + ' deeper into you. His grunts come closer and closer together. Your head still held up, you watch as the imps around you start to cum. They spray your body with thick globs of cum, splattering it across your belly', false );
				//(If the character has breasts);
				if( CoC.getInstance().player.biggestTitSize() >= 3 ) {
					EngineCore.outputText( ' and ' + CoC.getInstance().player.allBreastsDescript(), false );
				}
				EngineCore.outputText( '. The master imp pounds into you and you can see his ' + CoC.getInstance().monster.ballsDescriptLight() + ' swell. Through the haze of your approaching orgasm you realize what\'s about to happen. Those oversized balls are about to pump more cum into you than any normal man could ever produce. They\'re going to pump demonic cum right into your womb.  ', false );
				//(Low Corruption);
				if( CoC.getInstance().player.cor < 50 ) {
				...
				EngineCore.outputText( '. The ' + CoC.getInstance().monster.short + ' is no longer holding you down. They masturbate over you as you claw at the ground with your hands, toes curling as you clamp your thighs tight around the big imp. Another pulse of demonic cum hits your womb. You wrap your legs around your master, forcing as much of his cock into you as possible. Arching your back, your eyes roll back in your head and you shriek as your womb stretches painfully, a final orgasm crashing through your cum-bloated body. You spasm around the cock that impales you, thrashing against the ground as ', false );
				//(If the character has breasts);
				...
				if( CoC.getInstance().player.isNaga() ) {
					...
					EngineCore.outputText( CoC.getInstance().player.legs() + ' wide apart, holding them against the log.\n\n', false );
				}

				EngineCore.outputText( 'The ' + CoC.getInstance().monster.short + ' makes short work of your ' + CoC.getInstance().player.armorName + ', unbuckling straps and stripping you quickly. ', false );
				//(If the player has breasts);
				if( CoC.getInstance().player.biggestTitSize() > 0 ) {
					EngineCore.outputText( 'An imp mounts your chest and slaps his ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + ' between your ' + CoC.getInstance().player.allBreastsDescript() + ', squeezing them tight over his cock as he rubs back and forth.  ', false );
				}
				//(If the player has a SINGLE cock);
				if( CoC.getInstance().player.cockTotal() === 1 ) {
				...
				if( CoC.getInstance().player.biggestTitSize() > 0 ) {
					...
					EngineCore.outputText( '  Suddenly you feel tiny needle-sharp teeth pierce your nipple. You scream as venom pumps into your tits, red-hot poison that makes your ' + CoC.getInstance().player.allBreastsDescript() + ' feel as though they were being stung by bees. You moan in pain as your breasts start to swell, the imp rider biting into your other nipple to pump demon-taint into it.', false );
					if( CoC.getInstance().player.hasFuckableNipples() ) {
				...
				EngineCore.outputText( 'The master of this ' + CoC.getInstance().monster.short + ' steps up ' );
				if( CoC.getInstance().player.isNaga() ) {
					EngineCore.outputText( 'alongside your taut tail' );
				} else {
					EngineCore.outputText( 'between your ' + CoC.getInstance().player.legs() );
				}
				...
				EngineCore.outputText( '.  He thrusts brutally, shoving the head of his ' + CoC.getInstance().monster.cockDescriptShort( 2 ) + ' into your ', false );
				//(If the player has a vagina);
				if( CoC.getInstance().player.hasVagina() ) {
					EngineCore.outputText( Descriptors.vaginaDescript( 0 ), false );
				}
				...
				//(If the character has breasts);
				if( CoC.getInstance().player.biggestTitSize() > 0 ) {
					EngineCore.outputText( 'The big imp reaches past your tit-rider and grabs one of your painfully distended breasts in each hand, mauling and bouncing the flesh as if weighing them. You gasp in pain as your ' + CoC.getInstance().player.allBreastsDescript() + ' swell further at his touch.  ', false );
				}
				...
				//(If the character has breasts);
				if( CoC.getInstance().player.biggestTitSize() > 1 ) {
					EngineCore.outputText( 'The rough fucking shakes your breasts, and the imp sucking your nipples clings tightly to your monstrously swollen ' + CoC.getInstance().player.allBreastsDescript() + '. Your ' + Appearance.biggestBreastSizeDescript() + ' have grown three cup sizes since the imp pumped his venom into you.  ', false );
				}
				...
				EngineCore.outputText( ' as he continues to cum. You\'re pumped full of demon-cum from both ends as one imp shoots his load in your throat and another steps up to take his place. You shake and tremble in your own endless orgasm as the pleasure in your stretched hole blends with the pain of your swollen belly. Your ' + CoC.getInstance().player.legs() + ' thrash as the master imp shifts his massive knot within your monstrously stretched ', false );
				//(If the player has a vagina);
				if( CoC.getInstance().player.hasVagina() ) {
					EngineCore.outputText( Descriptors.vaginaDescript( 0 ), false );
				}
				...
				EngineCore.outputText( 'You wake up later, still trembling with small orgasms. Cum burbles in your mouth as you breathe. You haven\'t moved since you passed out. Your hips are still propped up over the log, and you rest in a cooling pool of cum, your ' + Descriptors.hairDescript() + ' plastered to the ground with drying jism. You couldn\'t move even if your ' + CoC.getInstance().player.legs() + ' felt stronger. Your hideously bloated belly weighs you down, ', false );
				//(If the player has breasts);
				if( CoC.getInstance().player.biggestTitSize() > 0 ) {
	...
	ImpScene.prototype.impRapesYou = function() {
		EngineCore.outputText( '', true );
		if( (CoC.getInstance().player.findPerk( PerkLib.BimboBrains ) >= 0 || CoC.getInstance().player.findPerk( PerkLib.FutaFaculties ) >= 0) && !CoC.getInstance().player.isTaur() && CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( ImageManager.showImage( 'imp-loss-female-fuck' ), false );
			EngineCore.outputText( 'You sink to the ground, assuming a position that feels all too natural to you now, leaning forward to let your ' + CoC.getInstance().player.allBreastsDescript() + ' hang down slightly. The imp looks you up and down, wickedly eyeing your ready, slightly open lips. He drops his loin-cloth to reveal a hardening cock. Your eyes bulge as it grows larger... and larger... and larger! The imp\'s cock finally bulges to a full twelve inches... and it\'s moving closer. You struggle to think... but you just can\'t! You want that in your mouth, like, so bad!\n\n', false );
		...
		if( CoC.getInstance().player.lust >= 100 ) {
			...
			if( CoC.getInstance().player.cocks.length >= 1 && Utils.rand( 2 ) === 0 ) {
				if( CoC.getInstance().player.cocks[ 0 ].cockThickness >= 4 ) {
					this.sprocketImp();
					return;
				}
			}
			...
			if( CoC.getInstance().player.gender === 2 || CoC.getInstance().player.gender === 3 ) {
				CoC.getInstance().player.slimeFeed();
				EngineCore.outputText( ImageManager.showImage( 'imp-loss-female-fuck' ), false );
				EngineCore.outputText( 'You sink to the ground, too overcome by lust and desire to fight.  The imp smiles, a wicked look glinting in his eyes.  He drops his loincloth to reveal a hardening cock.  Your eyes bulge a bit as it grows...and grows...and grows!  That imp has a twelve inch cock..and he\'s walking towards you.   Your ' + Descriptors.vaginaDescript( 0 ) + ' practically juices itself in anticipation, and you find yourself spreading your ' + CoC.getInstance().player.legs() + ' in preparation.', false );
				...
				EngineCore.outputText( '  The imp\'s ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + ' explodes inside you, pumping huge loads of hot demon-seed inside you with each eruption.  You swoon, feeling it fill your womb and distend your belly as the imp\'s orgasm fills you with an unnatural quantity of corrupted semen.\n\nWith a sigh, he pulls his dick free, and you flop back on your back, cum surging out onto the ground from your well-fucked hole.  ', false );
			...
			//Male or genderless;
			if( CoC.getInstance().player.gender === 0 || CoC.getInstance().player.gender === 1 ) {
				//Alternate male-only case;
				if( CoC.getInstance().player.gender === 1 && Utils.rand( 2 ) === 0 ) {
					...
					EngineCore.outputText( '.  As the imp presses a large bulk against your backside, you realize he has a massive penis!\n\nThe imp pushes his ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + ' into your ass and fucks you hard, with little regard to your pleasure.  After a rough fucking, he cums, stuffing your ass full of hot demon cum.  His orgasm lasts far longer than any human\'s, leaving your belly slightly distended.', false );
					CoC.getInstance().player.buttChange( CoC.getInstance().monster.cockArea( 0 ), true, true, false );
					EngineCore.dynStats( 'lib', 1, 'sen', 1, 'lus', 1, 'cor', 1 );
	...
	ImpScene.prototype.noogaisNippleRape = function() {
		...
		else if( CoC.getInstance().player.lactationQ() >= 50 && CoC.getInstance().player.biggestLactation() >= 1 ) {
		...
		//(if lactating) ;
		if( CoC.getInstance().player.biggestLactation() >= 1 && CoC.getInstance().player.lactationQ() < 40 ) {
	...
	ImpScene.prototype.impLordEncounter = function() {
		EngineCore.clearOutput();
		EngineCore.outputText( 'A large corrupted imp crosses your path. He flashes a cruel smile your way.  No way around it, you ready your ' + CoC.getInstance().player.weaponName + ' for the fight.' );
	...
	ImpScene.prototype.loseToAnImpLord = function() {
		EngineCore.clearOutput();
		if( CoC.getInstance().player.hasVagina() && (CoC.getInstance().player.gender === 2 || Utils.rand( 2 ) === 0) ) {
			...
			CoC.getInstance().player.takeDamage( 9999 );
			Combat.cleanupAfterCombat();
		}
	};
	//Rape;
	ImpScene.prototype.sexAnImpLord = function() {
		...
		//(No line break);
		//if(CoC.getInstance().player doesn't have centaur legs);
		if( !CoC.getInstance().player.isTaur() ) {
			EngineCore.outputText( '  You grab his hands, removing them from his ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + '. This gets his attention immediately, and you grin widely, pinning him to the ground.' );
		} else {
		...
		EngineCore.addButton( 9, 'Leave', Combat.cleanupAfterCombat );
		if( CoC.getInstance().player.lust >= 33 ) {
			if( CoC.getInstance().player.hasCock() && CoC.getInstance().player.cockThatFits( CoC.getInstance().monster.analCapacity() ) >= 0 ) {
	...
	ImpScene.prototype.impLordBumPlug = function() {
		...
		EngineCore.outputText( '  You pull the imp\'s head towards your [hips], forcing his lips against the base of your ' + Descriptors.cockDescript( x ) + '.  The imp quickly gets the idea and begins to lick and suckle at your ' + CoC.getInstance().player.cockHead( x ) + ' expertly.' );
		...
		EngineCore.outputText( '\n\nYou drape your ' + Descriptors.cockDescript( x ) + ' between the imp\'s butt cheeks.  Teasingly you begin thrusting between the muscular cleft of his ass, much to the creature\'s dismay.  The greater imp whines submissively, desperate for you to enter him.  His ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + ' drools as you tease his ass, creating a small puddle of pre between his hooves.  The poor creature becomes so desperate that he reaches back and spreads his muscular cheeks with his hands.  You catch a small glimpse of his now gaping hole as your cock continues to slide between his cheeks.  You tease him with one last empty thrust, before grabbing his shoulders and forcing your ' + Descriptors.cockDescript( x ) + ' deep inside the poor creature.' );
		EngineCore.outputText( '\n\nThe imp loses control immediately and nearly collapses from the massive orgasm.  His large, corruption-bloated balls clench up, and semen floods out of his ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + ' like a fountain.  The hot demon seed puddles around his feet, soaking the ground in his thick boy-goo.' );
		...
		if( CoC.getInstance().player.biggestTitSize() >= 1 && CoC.getInstance().player.lactationQ() >= 200 ) {
		...
		//if(CoC.getInstance().player has Fertility perk && CoC.getInstance().player.balls >= 4);
		if( CoC.getInstance().player.balls >= 4 ) {
			...
			//if(CoC.getInstance().player is a herm);
			if( CoC.getInstance().player.gender === 3 ) {
	...
	ImpScene.prototype.getBlownByAnImpLord = function() {
		...
		EngineCore.outputText( '\n\n' );
		//if(cock thickness < 7);
		if( CoC.getInstance().player.cocks[ CoC.getInstance().player.biggestCockIndex() ].cockThickness < 7 ) {
	...
	ImpScene.prototype.femaleVagRape = function() {
		...
		EngineCore.outputText( 'With little ceremony you grab the imp\'s ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + ' and begin to jerk your hand up and down roughly.  The little muscular beast begins to whine loudly, in protest to the rough and likely painful mistreatment of his ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + '.  In spite of the protests, the rough treatment goes over well, as the creature begins to leak hot demon pre across your hand, which you smear across the shaft as a natural hot lube.' );
		...
		EngineCore.outputText( '\n\nYou lower yourself, slowly, watching as the head of the demon\'s ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + ' begins to spread your ' + Descriptors.vaginaDescript( 0 ) + '.  You begin to pant as you\'re stretched wide; you can hardly believe you can take him.  It takes you several minutes of steady, shallow, downward thrusts before the imp is completely hilted inside of your body.  You sit still for a moment to adjust to the intense sensation. The muscular creature doesn\'t seem to mind, as his tongue is hanging slightly out of the side of his mouth, panting like a dog.' );
		CoC.getInstance().player.cuntChange( CoC.getInstance().monster.cockArea( 0 ), true, true, false );
		EngineCore.outputText( '\n\nAs you adjust, you get a devilish idea.  Both your hands gently begin to massage the imp\'s muscular abs and chest; before taking each of his fertite-pierced nipples between your fingers.  You pinch and tug upwards, roughly playing with his sensitive nipples.  The imp instinctively bucks his hips, slamming into you roughly, making you yelp and clench your [vagina] around his ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + '.' );
		...
		if( CoC.getInstance().player.cockTotal() === 1 ) {
			...
			EngineCore.outputText( '\n\nAs you continue your cruel torture of the imp\'s now bruised nipples, he reaches up both of his hands and takes hold of your ' + Descriptors.multiCockDescriptLight() + '.  The imp feverishly masturbates your ' + Descriptors.cockDescript( 0 ) + ' and ' + Descriptors.cockDescript( 1 ) + ' as he thrusts into your ' + Descriptors.vaginaDescript( 0 ) + '.  The intense stretching of your ' + Descriptors.vaginaDescript( 0 ) + ' by the ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + ' sends waves of pleasure coursing through you, straight into your ' + Descriptors.multiCockDescriptLight() + ', which are now leaking precum profusely onto the imp\'s muscular body.' );
		}
		EngineCore.outputText( '\n\nYou give one final rough tug on the imp\'s nipples, and that\'s all he can handle.  His cock spasms inside your ' + Descriptors.vaginaDescript( 0 ) + ' and erupts, sending wave after wave of hot, demon cum into your womb. His tainted testes spasm in their taut sack, continuing to pour their contents into you, until your belly is swollen and full of the hot seed.  You gasp in pleasure, unable to fight the pleasure of being filled so thoroughly.  Your ' + Descriptors.vaginaDescript( 0 ) + ' spasms, and you clench around the ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + ' inside of you, as your girl juices begin to flow out of you with your earth shattering orgasm.' );
		if( CoC.getInstance().player.cockTotal() === 1 ) {
	...
	ImpScene.prototype.feederBreastfeedRape = function() {
		...
		if( CoC.getInstance().player.tallness > 48 && CoC.getInstance().player.tallness < 60 && CoC.getInstance().player.lowerBody !== AppearanceDefs.LOWER_BODY_TYPE_CENTAUR ) {
			...
			//if(CoC.getInstance().player has a penis);
			if( CoC.getInstance().player.hasCock() ) {
			...
			if( CoC.getInstance().player.bRows() === 1 ) {
				...
				EngineCore.outputText( '\n\nNodding your acceptance, he continues his work much more carefully.  He\'s taking his time again instead of just sucking wildly.  You reach down curiously, and tug on the imp\'s ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + ' but find that it\'s shrinking.  As you hold it, it shrinks more and more. You wonder what will happen to him if he continues to nurse.' );
				//if(CoC.getInstance().player has only 2 rows of breasts);
	...
	ImpScene.prototype.loseToImpLord = function() {
		...
		} else if( CoC.getInstance().player.balls > 0 && CoC.getInstance().player.hasCock() ) {
		...
		EngineCore.outputText( '\n\nAfter longer than you\'d have hoped for, the painful stretching sensation begins to disappear; and the pleasurable sensation of the imp\'s ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + ' thrusting in and out of your [asshole] becomes entirely pleasurable.  The way his ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + ' fills every inch of your ass, and rubs all your most sensitive spots.  The weird sensation his warm, demonic pre-cum coats your insides.  You find your lust-blinded mind has become lost in the sensations - so lost that you don\'t even notice the imp increasing his pace.' );
		EngineCore.outputText( '\n\nWithin moments the beast is wildly thrusting in and out of your [asshole].  Pre-cum is pumping out of his ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + ' like a faucet. The hot demon pre begins to spill back out of your abused [asshole], coating your [hips], and dripping to the ground beneath.  The imp gives you a few more rough thrusts before cumming hard into your [asshole].  The little demon\'s ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + ' spasms as he continues to roughly thrust and pump you full of his burning hot demon seed.' );
		if( CoC.getInstance().player.hasCock() ) {
	...
	ImpScene.prototype.getRapedAsAGirl = function() {
		...
		EngineCore.outputText( '\n\nYou try to move, but before you can even begin to squirm a hoof presses hard between your ' + Descriptors.breastDescript( 0 ) + '.  You gasp as the air is temporarily knocked out of your lungs.  The demon chuckles at your last feeble attempt to free yourself.  He holds his ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + ' stroking it lewdly, a cruel smirk stretching across his face.  You watch as several beads of pre begin to drip from his tip onto your stomach.' );
		...
		EngineCore.outputText( ', draping his ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + ' across your wet crotch.  You groan, and unintentionally thrust against the magnificent tool between your legs.  The imp chuckles evilly as you coat his ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + ' in your girl juice, but he doesn\'t wait long before he slowly presses his head down against your [vagina].  His head slowly spreads your lips; the pleasure is unmistakable, and forces a loud moan from your lips.' );
		EngineCore.outputText( '\n\nWith a soft pop, the ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + ' pops into your [vagina], and both of you moan in unison, the demon beginning to thrust wildly into you.  His hips pumps back and forth into you.  The loud slapping sound of flesh on flesh echoes around you, drowning out the grunts of the vicious demon above you.' );
		CoC.getInstance().player.cuntChange( CoC.getInstance().monster.cockArea( 0 ), true, true, false );
		EngineCore.outputText( '\n\nYou mewl softly as you\'re viciously fucked by the beast above you.  It doesn\'t take long before your [vagina] clenches tightly around the ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + ' as you orgasm.  You scream in pleasure as your inner walls begin to milk the imp\'s ' + CoC.getInstance().monster.cockDescriptShort( 0 ) + ' of its seed.  The imp quickly succumbs and cums, his swollen balls tightening up against his crotch.  The hot jizz continues to pump into you for what feels like several painfully long minutes, until your belly bulges slightly, and your ' + Descriptors.vaginaDescript( 0 ) + ' begins to leak the white demonic fluid.' );
