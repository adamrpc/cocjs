...
angular.module( 'cocjs' ).run( function( SceneLib, MainView, ImpGang, ImpLord, PerkLib, PregnancyStore, ImageManager, CockTypesEnum, Appearance, Combat, Descriptors, StatusAffects, AppearanceDefs, Utils, CoC, EngineCore ) {
	...
	ImpScene.prototype.impVictory = function() {
		...
		if( canFeed ) {
			if( CoC.player.lust >= 33 ) {
				MainView.outputText( '  Sadly you realize your own needs have not been met.  Of course you could always rape the poor thing, but it might be more fun to force it to guzzle your breast-milk.\n\nWhat do you do?' );
			} else {
				MainView.outputText( '  You\'re not really turned on enough to rape it, but it might be fun to force it to guzzle your breast-milk.\n\nDo you breastfeed it?' );
			}
	...
	ImpScene.prototype.rapeImpWithDick = function() {
		...
		//Single cock;
		MainView.outputText( ImageManager.showImage( 'imp-win-male-fuck' ), false );
		if( CoC.player.cocks.length === 1 ) {
			MainView.outputText( 'With a demonic smile you grab the insensible imp and lift him from the ground by his neck.  The reduced airflow doesn\'t seem to slow his feverish masturbation at all, and only serves to make him harder.', true );
			if( CoC.player.lowerBody !== AppearanceDefs.LOWER_BODY_TYPE_CENTAUR ) {
				MainView.outputText( '  You casually unclasp your ' + CoC.player.armorName + ' and reveal your ' + Descriptors.cockDescript( x ) + ', ', false );
				if( CoC.player.breastRows.length > 0 && CoC.player.breastRows[ 0 ].breastRating > 2 ) {
					MainView.outputText( 'smashing him against your ' + Descriptors.breastDescript( 0 ) + ' while you jerk hard on your ' + Descriptors.cockDescript( x ) + ', bringing it to a full, throbbing erection.', false );
				} else {
					MainView.outputText( 'stroking it to full hardness languidly.', false );
				}
			}
			...
			if( CoC.player.cocks[ 0 ].cockType === CockTypesEnum.CAT ) {
				MainView.outputText( '  The tiny creature\'s claws dig into your sides at the feeling of soft, hooked barbs stroking his sensitive insides.', false );
			}
			if( CoC.player.cocks[ 0 ].cockLength >= 7 && CoC.player.cocks[ 0 ].cockLength <= 12 ) {
				MainView.outputText( '  Each thrust obviously distorts the imp\'s abdomen.  It amazes you that it doesn\'t seem to be hurting him.', false );
			}
			if( CoC.player.cocks[ 0 ].cockLength > 12 ) {
				MainView.outputText( '  Each plunge into the imp\'s tight asshole seems to distort its entire body, bulging obscenely from its belly and chest.  Amazingly he doesn\'t seem to mind, his efforts focused solely on his sorely throbbing demon-dick.', false );
			}
			...
			if( CoC.player.balls === 0 && CoC.player.vaginas.length > 0 ) {
				MainView.outputText( '  The cum seems to boil out from inside you as your ' + Descriptors.vaginaDescript( 0 ) + ' soaks itself.  With delicious slowness you fire rope after rope of cum deep into the imp\'s rectum.', false );
			}
			if( CoC.player.balls === 0 && CoC.player.vaginas.length === 0 ) {
				MainView.outputText( '  The cum seems to boil out from inside you, flowing up your ' + Descriptors.cockDescript( x ) + '.  With delicious slowness, you fire rope after rope of cum deep into the imp\'s rectum.', false );
			}
			if( CoC.player.cumQ() >= 14 && CoC.player.cumQ() <= 30 ) {
				MainView.outputText( '  Your orgasm drags on and on, until your slick jism is dripping out around your ' + Descriptors.cockDescript( x ) + '.', false );
			}
			if( CoC.player.cumQ() > 30 && CoC.player.cumQ() <= 100 ) {
				MainView.outputText( '  Your orgasm seems to last forever, jizz dripping out of the imp\'s asshole around your ' + Descriptors.cockDescript( x ) + ' as you plunder him relentlessly.', false );
			}
			if( CoC.player.cumQ() > 100 ) {
				MainView.outputText( '  Your orgasm only seems to grow more and more intense as it goes on, each spurt more powerful and copious than the last.  The imp begins to look slightly pregnant as you fill him, and tiny jets of cum squirt out around your ' + Descriptors.cockDescript( x ) + ' with each thrust.', false );
			}
		...
		if( CoC.player.cocks.length >= 2 ) {
			MainView.outputText( 'With a demonic smile you grab the insensible imp and lift him from the ground by his neck.  The reduced airflow doesn\'t seem to slow his feverish masturbation at all, and only serves to make him harder.', true );
			if( CoC.player.lowerBody !== AppearanceDefs.LOWER_BODY_TYPE_CENTAUR ) {
				MainView.outputText( '  You casually unclasp your ' + CoC.player.armorName + ' and reveal your ' + Descriptors.multiCockDescriptLight() + ', ', false );
				if( CoC.player.breastRows.length > 0 && CoC.player.breastRows[ 0 ].breastRating > 2 ) {
					MainView.outputText( 'smashing him against your ' + Descriptors.breastDescript( 0 ) + ' while you jerk hard on one of your ' + Descriptors.cockDescript( x ) + 's, bringing it to a full, throbbing erection.', false );
				} else {
					MainView.outputText( 'stroking one of your members to full hardness languidly.', false );
				}
			}
			...
			if( CoC.player.balls > 0 ) {
				MainView.outputText( 'The cum seems to boil in your balls, sending heat spreading through your ' + Descriptors.cockDescript( x ) + ' as your muscles clench reflexively, propelling hot spurts of jism deep into the imp\'s rectum.  Your other equipment pulses and dripples steady streams of its own cum.', false );
			}
			if( CoC.player.balls === 0 && CoC.player.vaginas.length > 0 ) {
				MainView.outputText( 'The cum seems to boil out from inside you as your ' + Descriptors.vaginaDescript( 0 ) + ' soaks itself.  With delicious slowness you fire rope after rope of cum deep into the imp\'s rectum.  Your other equipment drizzles small streams of jizz in sympathy.', false );
			}
			if( CoC.player.balls === 0 && CoC.player.vaginas.length === 0 ) {
				MainView.outputText( 'The cum seems to boil out from inside you, flowing up your ' + Descriptors.cockDescript( x ) + '.  With delicious slowness, you fire rope after rope of cum deep into the imp\'s rectum.  Your other equipment drizzles small streams of jizz in sympathy.', false );
			}
			if( CoC.player.cumQ() >= 14 && CoC.player.cumQ() <= 30 ) {
				MainView.outputText( '  Your orgasm drags on and on, until your slick jism is dripping out around your ' + Descriptors.cockDescript( x ) + '.', false );
			}
	...
	ImpScene.prototype.rapeImpWithPussy = function() {
		...
		//Big enough!;
			...
			MainView.outputText( '  You sink down his ' + CoC.monster.cockDescriptShort( 0 ) + ' slowly, delighting in the gradual penetration and the tingling feeling of his dripping hot pre-cum.  At last you bottom out on his balls.', false );
			CoC.player.cuntChange( CoC.monster.cockArea( 0 ), true );
			MainView.outputText( '  Your lust and desire spurs you into movement, driving you to bounce yourself up and down on the ' + CoC.monster.cockDescriptShort( 0 ) + '.  His exquisite member pushes you to the very height of pleasure, your ' + Descriptors.vaginaDescript( 0 ) + ' clenching tightly of its own accord each time you bottom out.  The tensing of the little demon\'s hips is the only warning you get before he cums inside you, hot demonic jizz pouring into your womb.  Your ' + CoC.player.legs() + ' give out, pushing him deeper as he finishes filling you.', false );
	...
	ImpScene.prototype.sprocketImp = function() {
		...
		if( CoC.player.cocks[ 0 ].cockType === CockTypesEnum.HUMAN || CoC.player.cocks[ 0 ].cockType === CockTypesEnum.DEMON || CoC.player.cocks[ 0 ].cockType.Index > 4 ) {
			MainView.outputText( 'your ' + Descriptors.cockDescript( 0 ) + ', rubbing the tip of his prick against your own, ', false );
		} else if( CoC.player.hasKnot( 0 ) ) {
			MainView.outputText( 'your ' + Descriptors.cockDescript( 0 ) + ', rubbing the tip of his prick against your point, ', false );
		} else if( CoC.player.cocks[ 0 ].cockType === CockTypesEnum.HORSE ) {
			MainView.outputText( 'your ' + Descriptors.cockDescript( 0 ) + ', rubbing the tip of his prick against your flared head, ', false );
		} else if( CoC.player.cocks[ 0 ].cockType === CockTypesEnum.TENTACLE ) {
			MainView.outputText( 'your huge green dick, rubbing the tip of his prick against your purplish cock-head, ', false );
		}
		MainView.outputText( 'smearing your pre-cum together.  You wonder if he is planning on just jerking both of you off as you shake the cloth from your face.  He flashes you an evil smile, making your eyes widen in terror as you realize what he is planning. Before you can even think to make a move to stop him, the imp ', false );
		if( CoC.player.cocks[ 0 ].cockType === CockTypesEnum.HUMAN || CoC.player.cocks[ 0 ].cockType === CockTypesEnum.DEMON || CoC.player.cocks[ 0 ].cockType.Index > 4 ) {
			MainView.outputText( 'shoves his shaft deeply into the slit in the head of your dick.  ', false );
		} else if( CoC.player.hasKnot( 0 ) ) {
			MainView.outputText( 'finds the hole in the pointed head of your cock and plunges his shaft deeply into it, literally fucking your urethra.  ', false );
		} else if( CoC.player.cocks[ 0 ].cockType === CockTypesEnum.HORSE ) {
			MainView.outputText( 'seats his dick in the flared head of your prick, and then pushes farther. His shaft plunges into yours, filling your cock more than any cum load ever could.  ', false );
		} else if( CoC.player.cocks[ 0 ].cockType === CockTypesEnum.TENTACLE ) {
			MainView.outputText( 'shoves his dick deeply into the slit in the head of your vine-like cock.  ', false );
		}
		//New PG;
		MainView.outputText( '\n\n', false );
		MainView.outputText( 'He grips your cock tightly as he fucks you, treating you like a ', false );
		//Differing cocksleeve texts;
		if( CoC.player.skinDesc === 'fur' ) {
			MainView.outputText( 'furry cock-sleeve', false );
		} else {
			if( CoC.player.skinTone === 'purple' || CoC.player.skinTone === 'blue' || CoC.player.skinTone === 'shiny black' ) {
				MainView.outputText( 'demonic cock-sleeve', false );
			} else {
				MainView.outputText( 'human cock-sleeve', false );
			}
		}
		//Bonus boob shake or period if no boobs.;
		if( CoC.player.breastRows.length > 0 && CoC.player.biggestTitSize() > 2 ) {
			MainView.outputText( ', fucking you so hard that your ' + CoC.player.allBreastsDescript() + ' bounce with each thrust.  ', false );
		} else {
			MainView.outputText( '.  ', false );
		}
		MainView.outputText( 'It briefly crosses your mind that this should be painful, but something about either his lubrication or yours makes it comfortable enough to have you writhing in pleasure.  ', false );
		MainView.outputText( 'He thrusts roughly into you for several minutes, your hips bucking upward to meet him, ', false );
		if( CoC.player.cocks.length === 2 ) {
			MainView.outputText( 'your other cock finding pleasure in rubbing against his body ', false );
		}
		if( CoC.player.cocks.length > 2 ) {
			MainView.outputText( 'your other cocks finding pleasure in rubbing against his body ', false );
		}
		//Cum;
		MainView.outputText( 'while copious amounts of sweat runs off of both your exposed forms, before he shivers and sinks deeply into you.  He cums hard, the heat of his demon seed burning your loins. His orgasm lasts longer than you think possible, forcing your own climax. Your seed mixes within your body, becoming more than you can handle and spilling out from your urethra around his intruding member.  ', false );
		//Extra cum-texts;
		if( CoC.player.cocks.length === 2 ) {
			MainView.outputText( 'Your other cock cums at the same time, liberally splattering your spunk up his back.  ', false );
		}
		if( CoC.player.cocks.length > 2 ) {
			MainView.outputText( 'The rest of your ' + Descriptors.multiCockDescriptLight() + ' twitch and release their seed at the same time, creating a shower of spunk that rains down on both you and the imp, coating both of your bodies.  ', false );
		}
		if( CoC.player.biggestLactation() >= 1 ) {
			MainView.outputText( 'At the same time, milk bursts from your ' + Descriptors.nippleDescript( 0 ) + 's, splattering him in the face.  You feel a sick sort of triumph as you get him back for cumming inside you.  ', false );
		}
		//Vagoooz;
		if( CoC.player.vaginas.length > 0 ) {
			MainView.outputText( 'Your pussy quivers, contracting furiously as your orgasm hits you - like it\'s trying to milk a phantom dick dry.  ', false );
		}
		//new PG;
		MainView.outputText( 'Satisfied, his dick slides from you and he flies away as mixed seed continues to spill from your abused body. Your limbs grow weak, and you fall from the tree with a hard thud before losing consciousness.  ', false );
		//Take some damage;
	...
	ImpScene.prototype.centaurGirlOnImps = function() {
		MainView.outputText( '', true );
		MainView.outputText( 'You stand over the thoroughly defeated demon and get an amusing idea. The tiny creatures are far from a threat, but their features seem like they might be useful. You pick the imp up and place him in a tree with explicit orders to him to stay, much to his confusion. Once you\'re sure he won\'t move, you wolf whistle and wait.\n\n', false );
		MainView.outputText( 'A goblin appears from the underbrush behind you, but a swift kick sends her flying; she\'s not what you\'re after. You\'re soon rewarded with a trio of imps, who fly up to you, cocks at the ready.  Grabbing the defeated imp by the head, you explain your need to the group and waft a bit of your scent over to them with your tail. They confer among themselves only briefly, clear on the decision, as you toss their weaker fellow underneath them. The larger of the three, evidently the leader, smiles lewdly at you and agrees to your \'demands\'.\n\n', false );
		//[Female:;
		if( CoC.player.hasVagina() ) {
			MainView.outputText( 'The imps approach you, their various genitalia glistening in the sun and drawing your attention. Their cocks swing lewdly with every flap of their wings, but you turn around, wanting their ministrations to be a surprise.\n\n', false );
			MainView.outputText( 'Hands slide over you, stroking and patting your equine form. The roving fingers find their way to your rear quickly, and begin teasing around your ' + Descriptors.vaginaDescript() + ' and ' + Descriptors.assholeDescript() + '. They probe around but don\'t penetrate and you stamp your hoof in frustration. There\'s a chuckle from behind you and all but a handful of the hands disappear.\n\n', false );
			MainView.outputText( 'A slightly larger hand smacks your ' + Descriptors.assDescript() + ' then slides up and pops a thick finger inside. Your ' + Descriptors.assholeDescript() + ' tries to suck it in deeper, but loses the opportunity as it\'s extracted before doing anything. Instead, the hand returns to your flank and slides slowly forward to your torso.\n\n', false );
			MainView.outputText( 'The \'head\' imp comes around into your vision, hovering in front of you and letting you get a good look at his long member. He pulls on it, extracting a large bead of pre onto his other hand. Opening your mouth, he wipes the salty substance onto your tongue. You swallow it happily and feel your mouth watering and your ' + Descriptors.vaginaDescript() + ' pumping out fluid.\n\n', false );
			MainView.outputText( 'The leader looks past you and gives a signal to someone you can\'t see, but you don\'t have time to turn as a huge dog cock is slipped into your slavering cunt and an even larger spined prick is inserted into your ' + Descriptors.assholeDescript() + '. They begin pumping into you hard, and you whinny in satisfaction while the demon before you watches, jerking on himself.', false );
			CoC.player.cuntChange( CoC.monster.cockArea( 0 ), true, true, false );
			CoC.player.buttChange( CoC.monster.cockArea( 0 ), true, true, false );
			MainView.outputText( '\n\n', false );
			MainView.outputText( 'He disappears behind you and gives you a slap on the haunches, yelling, "<i>Giddyup!</i>" and laughing heartily. Whether he expected you to or not, you decide to go for it and push off the ground with your forelegs, kicking them about in the air and feeling the demons aboard you scrabble to stay attached, before setting off at as fast a run as you can. You tear about in the dirt, clumps of mud and weeds flung behind you.\n\n', false );
			MainView.outputText( 'At the edge of the clearing is the leader, laughing as he watches you and still jerking himself. As if realizing that there\'s a better option available, he grabs the defeated imp and inserts himself into him, using him like a living cock sleeve who appears to not mind the position and cries out repeatedly as his ass is abused.\n\n', false );
			MainView.outputText( 'Your unexpected running momentarily paused the cocks inside you as their owners groped for holds on your ' + Descriptors.hipDescript() + ' and ' + Descriptors.assDescript() + '. With their positions relatively well established, they begin pounding at you again, causing you to nearly stumble in pleasure.\n\n', false );
			MainView.outputText( 'Managing to steady yourself, you run faster, feeling the frenetic cocks inside you explode. The hot spunk sprays about inside and you scream in ecstasy.', false );
			//[Has breasts: ;
			if( CoC.player.biggestTitSize() > 1 ) {
				MainView.outputText( '  Your hands reflexively grab your ' + Descriptors.chestDesc() + ' and mash them about.', false );
			}
			MainView.outputText( '\n\n', false );
			MainView.outputText( 'The owner of the dog-cock in your ' + Descriptors.vaginaDescript() + ' manages to insert his knot as his balls empty inside you, but the cat-cock\'s body has no such luck and his grip on you falters. He slides out of your ' + Descriptors.assholeDescript() + ' but manages to grasp the fur of your back and straddle you, all while his cock continues to spray you down with jism.\n\n', false );
			//[Has breasts: ;
			if( CoC.player.biggestTitSize() > 1 ) {
				MainView.outputText( 'He slides up to your torso and grasps your wildly flailing ' + CoC.player.allBreastsDescript() + ', massaging them harshly. His ministrations are surprisingly crude, and you wonder how many times he\'s attempted to pleasure a woman.', false );
				//[Has fuckable nipples: ;
				if( CoC.player.hasFuckableNipples() ) {
					MainView.outputText( '  His fingers slide inside your ' + Descriptors.nippleDescript( 0 ) + 's and start spreading and squishing them. Your femcum leaks out over his hands and soon your front is slick and shiny.', false );
				}//All other nipples: ;
				else {
					MainView.outputText( '  His fingers grope and grab at your nipples, stretching them uncomfortably. Before you can complain he seems to realize his mistake and releases them.', false );
				}
				//[Is lactating normally: ;
				if( CoC.player.biggestLactation() >= 1 && CoC.player.lactationQ() < 50 ) {
					MainView.outputText( '  Milk dribbles and squirts from you as his desperate squishing continues, forming small puddles on the ground.', false );
				} else if( CoC.player.biggestLactation() >= 1 ) {
					MainView.outputText( '  Milk sprays from you as his desperate squishing continues, creating massive puddles of milk that you splash through as you continue moving.', false );
				}
				MainView.outputText( '\n\n', false );
			}
			MainView.outputText( 'You stop running, spraying dirt in a massive fan and sending the imp on your back flying into a tree, where he crumples to the ground unceremoniously. The doggy-dicked imp collapses out of you and is sprayed down with your orgasm, coating him in femcum and his own semen.\n\n', false );
			MainView.outputText( 'You trot over to the leader, still using the nearly unconscious imp as a cock sleeve, and pull the abused creature off of him. He looks shocked as you grab his cock and squeeze his balls, causing him to orgasm hard and spray you down in white hot seed. He collapses onto the ground, spent, as you wipe yourself down as best you can.', false );
			MainView.outputText( '  Collecting your things, you give the assorted bodies one last look and stumble back to camp.', false );
			CoC.player.orgasm();
			EngineCore.dynStats( 'cor', 1 );
		}
		Combat.cleanupAfterCombat();
	...
	ImpScene.prototype.centaurOnImpStart = function() {
		...
		MainView.outputText( 'As the imp collapses in front of you, ', false );
		if( CoC.monster.HP === 0 ) {
			MainView.outputText( 'panting in exhaustion', false );
		} else {
			MainView.outputText( 'masturbating furiously', false );
		}
		MainView.outputText( ', you advance toward the poor creature.  The demon\'s eyes run over your powerful equine muscles as you tower above it.  It is difficult to hide your smile as you look at the tiny creature\'s engorged cock and the perpetual lust filling its beady eyes.', false );
		//OPTIONAL THOUGHTS;
		...
		if( CoC.player.cor < 50 ) {
			MainView.outputText( '  You lick your lips slightly as you begin to approach the small figure.', false );
		} else {
			MainView.outputText( 'You lick your lips obscenely as you approach the small figure.\n\n', false );
		}
		...
		if( x >= 0 && !CoC.player.hasVagina() ) {
			...
			MainView.outputText( 'Do you focus on your maleness or girl-parts?', false );
	...
	ImpScene.prototype.centaurOnImpMale = function( vape ) {
		...
		if( CoC.player.cockTotal() > 1 && CoC.player.cocks[ CoC.player.biggestCockIndex() ].cockLength >= 24 ) {
			...
			//<< Cor <50>>;
			if( CoC.player.cor < 50 ) {
				MainView.outputText( 'You slowly undulate your ' + Descriptors.cockDescript( CoC.player.biggestCockIndex() ) + ' against the creature\'s mouth, delighting in its eager tongue.  ', false );
				//<<GoTo I3 then return>> ;
				this.centaurOnImpResults( 3 );
				MainView.outputText( 'The sounds beneath you quickly take on a more intense note and you feel massive amounts of cum splashing liberally over your hooves, belly, and ' + Descriptors.cockDescript( CoC.player.biggestCockIndex() ) + '.  The hot sensation sends you over the edge as you begin spilling yourself into the creature\'s eager mouth.\n\n', false );
				//<<GoTo I2>>;
				this.centaurOnImpResults( 2 );
				//<<End>>;
				CoC.player.orgasm();
			...
			//<< 1 or 2 cocks, Cor 50+>>;
				...
				//<<Cor 80+>> ;
					...
					MainView.outputText( 'You groan in pleasure and slide your ' + Descriptors.cockDescript( CoC.player.biggestCockIndex() ) + ' even deeper down the creature\'s throat, until you can feel its head against your ', false );
					//<<if balls>>;
					if( CoC.player.balls > 0 ) {
						MainView.outputText( Descriptors.ballsDescriptLight() + '.\n\n', false );
					} else {
						MainView.outputText( 'groin.\n\n', false );
					}
					//<<GoTo I3 then return>> ;
					this.centaurOnImpResults( 3 );
					MainView.outputText( 'A guttural moan escapes your mouth as you realize the creature has completely passed out underneath you.  ', false );
					if( CoC.player.hasFuckableNipples() ) {
						MainView.outputText( 'Shoving your fingers deep into your ' + Descriptors.nippleDescript( 0 ) + 's', false );
					} else {
						MainView.outputText( 'With a fierce tug on your ' + Descriptors.nippleDescript( 0 ) + 's', false );
					}
					MainView.outputText( 'you begin to cum deep and directly into the imp\'s stomach and ' + Appearance.assholeDescript(CoC.monster) + '.  ', false );
					//<<cum multiplier: lots>>;
					if( CoC.player.cumQ() > 250 ) {
						MainView.outputText( 'Beneath you the creature\'s belly is distending more and more, and you can feel some of the overflowing cum filling back out until it is pouring out of the creature\'s unconscious mouth and overstretched ass, forming a spermy pool beneath it.', false );
					}
	...
	ImpScene.prototype.centaurOnImpFemale = function( vape ) {
		...
		CoC.player.slimeFeed();
		CoC.player.knockUp( PregnancyStore.PREGNANCY_IMP, PregnancyStore.INCUBATION_IMP );
		MainView.outputText( 'As the imp lays beaten its hands stroke its ' + CoC.monster.cockDescriptShort( 0 ) + ' as its eyes look over you in the hope that you might abuse it in some manner.  You lick your lips as you stare at the large member and you turn around to display your ' + Descriptors.vaginaDescript( 0 ) + '.  ', false );
		//Not gaping?;
		if( CoC.player.vaginas[ 0 ].vaginalLooseness <= AppearanceDefs.VAGINA_LOOSENESS_GAPING ) {
			//Penetration for non-gape cases;
			MainView.outputText( 'With a lascivious grin the imp hops forward, gripping your flanks as it drives its member forward into your ' + Descriptors.vaginaDescript( 0 ) + '.  ', false );
			//<<If Pussy Virgin>> ;
			if( CoC.player.vaginas[ 0 ].virgin ) {
				MainView.outputText( 'You cry out as your virginal pussy is torn open by the massive member and the creature cries out in pleasure as it realizes what it has taken from you.  ', false );
				//[Lose Virginity] <</Virgin>>;
			}
			//Not virgin fucking flavors;
			else {
				if( CoC.player.vaginalCapacity() < CoC.monster.cockArea( 0 ) ) {
					MainView.outputText( 'It groans in delight at your incredible tightness and shoves itself forward even harder.  ', false );
				}
				//[Increase size as needed] ;
				//<<At Dicksize>>;
				if( CoC.player.vaginalCapacity() >= CoC.monster.cockArea( 0 ) && CoC.player.vaginalCapacity() <= CoC.monster.cockArea( 0 ) * 1.25 ) {
					MainView.outputText( 'It makes a pleased sound as it slides deeply into your ' + Descriptors.vaginaDescript( 0 ) + '.  ', false );
				}
				//<<Bigger than dicksize>> ;
				if( CoC.player.vaginalCapacity() >= CoC.monster.cockArea( 0 ) * 1.25 ) {
					MainView.outputText( 'Its dick slides easily and slopping noises start sounding from your backside.  Part of you wishes that its large member was larger still, as your mind drifts to some of the monstrous cocks that have penetrated you in the past.  ', false );
				}
			}
		...
		//<<Gaping>>;
			...
			MainView.outputText( 'With a lascivious grin the imp hops forward, gripping your flanks as it drives its member forward into your ' + Descriptors.vaginaDescript( 0 ) + '.  While you might have considered him large before you came to this place, the sensation is now merely pleasant, and you can\'t help but groan in slight disappointment.  ', false );
			//<<Cor 50+>>;
			if( CoC.player.cor >= 50 ) {
				MainView.outputText( 'You take comfort in knowing that at least there is a cock inside of you, and that soon it will be filling you with its seed.  Perhaps it might even impregnate you!  ', false );
			}
			MainView.outputText( 'The imp seems to have shared your initial annoyance, and suddenly you feel strange and harsh objects prodding your ' + Descriptors.vaginaDescript( 0 ) + ' near where you are being penetrated.  Suddenly you feel yourself being forced open even wider, and you feel almost as if you are getting kicked inside of your pussy.  A second object touches near where the first had entered and you quickly brace yourself against a nearby tree.  The second jolt is even harder, feeling as if your cervix is getting stomped.  You howl out in pain as your pussy is virtually torn open, the imp using your tail to leverage not only his ' + CoC.monster.cockDescriptShort( 0 ) + ' but also his legs inside your ' + Descriptors.vaginaDescript( 0 ) + '.  ', false );
			//<<Cor <80>>;
			if( CoC.player.cor < 80 ) {
				MainView.outputText( 'Tears pour out of your eyes and you are sure you must be bleeding slightly, ', false );
			}
			//<<Cor <50>>;
			if( CoC.player.cor < 50 ) {
				MainView.outputText( 'and you hang on to the tree, afraid of the pain from even the slightest movement.  ', false );
			}
			//<<Cor 50+>>;
			else {
				MainView.outputText( 'and you hang on to the tree, grunting like a rutting animal as you delight in the intense pain.  ', false );
			}
			//<<Cor 80+>>;
			if( CoC.player.cor >= 80 ) {
				MainView.outputText( 'You howl out in pain and pleasure, bucking and hoping to intensify the sensation, hurling enticements and insults at the imp like a slut.  ', false );
			}
			//<<Cor 50+, Breasts>>;
			if( CoC.player.cor >= 50 && CoC.player.biggestTitSize() >= 2 ) {
				MainView.outputText( 'You release the tree as you begin playing with your ' + CoC.player.allBreastsDescript(), false );
				//<<w/ nip-pussies>>;
				if( CoC.player.hasFuckableNipples() ) {
					MainView.outputText( ' and shoving your fingers into your ' + Descriptors.nippleDescript( 0 ) + '.  ', false );
				} else {
					MainView.outputText( '.  ', false );
				}
				//<</Breasts>>;
			}
			MainView.outputText( 'The imp is pushing deeper and deeper and in moments you cry out again as you feel first its hooves and then its ' + CoC.monster.cockDescriptShort( 0 ) + ' tearing open your cervix and bottoming out in your womb.  ', false );
			//<<Asshole large+>>;
			if( CoC.player.analCapacity() >= 35 ) {
				MainView.outputText( 'When the imp realizes it cannot go any further you feel its hands against your asshole, and your eyes go wide in realization of what it is planning on doing.  Lubed up by your now drooling juices, the fist pushes hard into your ' + Descriptors.assholeDescript() + ', shoving past your ring-muscles.  ', false );
				//<<Assole <gaping, Cor <80>>;
				if( CoC.player.ass.analLooseness < 4 && CoC.player.cor < 80 ) {
					MainView.outputText( 'Your howl of pain leaves your throat raw.  ', false );
				} else {
					MainView.outputText( 'Your howl of perverse pleasure leaves your throat raw.  ', false );
				}
			}
			...
			if( CoC.player.cor < 80 ) {
				MainView.outputText( 'promise to yourself that you will not do that again.', false );
			} else {
	...
	ImpScene.prototype.centaurOnImpResults = function( iNum ) {
		...
		if( iNum === 1 ) {
			...
			if( CoC.player.cumQ() >= 250 ) {
				...
				if( CoC.player.cocks[ x ].cockType !== CockTypesEnum.DOG ) {
					MainView.outputText( 'Soon the amount is overflowing from the abused ' + Appearance.assholeDescript(CoC.monster) + ', dripping between you with no sign of stopping as you continue thrusting yourself into the imp.  ', false );
				}
				//<<knot>>;
				else {
					MainView.outputText( 'Soon the abused ' + Appearance.assholeDescript(CoC.monster) + ' is full to the brim, though your knot keeps any from escaping while more and more pumps in.  Soon the creature\'s belly is distending and the imp is gasping wordlessly. ', false );
				}
				MainView.outputText( 'When your ' + Descriptors.cockDescript( x ) + ' finally emerges a torrent of cum follows out of the distended hole and covering the back of the creature\'s legs.  ', false );
				//<<I1_1>>;
				//<<2 cocks>>;
				if( CoC.player.cockTotal() === 2 ) {
					MainView.outputText( 'Your other cock drenches the imp\'s back with its own secretions that immediately start dripping down its sides.  ', false );
				}
				//<<3+ cocks>>;
				if( CoC.player.cockTotal() > 2 ) {
					MainView.outputText( 'Your other cocks release their cum all over the creature\'s back and sides, leaving it a glazed mess.  ', false );
				}
				//<</I1_1>>;
				MainView.outputText( 'You leave him panting and lapping at a pool of your semen.', false );
			}//<</multiplier>>
			//<<cum multiplier: little-normal>>;
			else {
				MainView.outputText( 'With a last thrust into the cum receptacle you begin slowing down, even as its own ' + CoC.monster.cockDescriptShort( 0 ) + ' spills its seed over the ground.  ', false );
				//<<I1_1>>;
				//<<2 cocks>>;
				if( CoC.player.cockTotal() === 2 ) {
					MainView.outputText( 'Your other cock drenches the imp\'s back with its own secretions that immediately start dripping down its sides.  ', false );
				...
				//<</I1_1>>;
				MainView.outputText( 'You leave him panting and draped over the mossy boulder in a pool of your joint cum.', false );
			}
			return;
		}
		if( iNum === 2 ) {
			//<<cum multiplier: lots>>;
			if( CoC.player.cumQ() >= 250 ) {
				MainView.outputText( 'The imp\'s eyes widen in at the amount pouring in, and gobs of sperm begin overflowing down its chin.  ', false );
				//<<(lots cont.)  cum multiplier: excessive>>;
				if( CoC.player.cumQ() >= 500 ) {
					MainView.outputText( 'No matter how fast it is swallowing it does not seem to be enough, and soon its belly is distended and its skin is covered in a thick coating of cum.  ', false );
				}
				//<</multiplier>>  ;
			}
			MainView.outputText( 'Sated you trot away and leave the creature licking its lips and fingers, its eyes following you with lustful cunning.', false );
			//<</I2>>;
			return;
		}
		//<<I3>>;
		if( iNum === 3 ) {
			...
			if( CoC.player.biggestTitSize() >= 2 ) {
				MainView.outputText( 'As the sensations intensify you reach up and begin massaging your ' + Descriptors.breastDescript( 0 ) + ' and playing with your ' + Descriptors.nippleDescript( 0 ) + 's.  ', false );
				//<<(breasts cont.) nips have pussies>> ;
				if( CoC.player.hasFuckableNipples() ) {
					//<<nip-pussies and milk>>;
					if( CoC.player.biggestLactation() >= 1 ) {
						MainView.outputText( 'Milk streams out from your ' + Descriptors.nippleDescript( 0 ) + 's as if they had been recently filled with dripping cum.  ', false );
					} else {
						MainView.outputText( 'Your fingers slide faster and faster into your ' + Descriptors.nippleDescript( 0 ) + 's even as the imp begins to stroke itself under you.  ', false );
					}
				}
				//No pussies;
				else {
					...
					if( CoC.player.biggestLactation() > 0 ) {
						//<<little milk>>;
						if( CoC.player.biggestLactation() <= 1 ) {
							MainView.outputText( 'Beads of milk begin to drip down your chest and occasionally spurt outward.  ', false );
						}//<<else>>;
						else {
							MainView.outputText( 'Milk pours out of your ' + Descriptors.breastDescript( 0 ) + ' and streams down your body.  ', false );
						}
					}//<</milk>>
				}
			}//<</Breasts>>
	...
	ImpScene.prototype.areImpsLactoseIntolerant = function() {
		MainView.outputText( '', true );
		MainView.outputText( 'You advance on the masturbating imp, baring your ' + CoC.player.allBreastsDescript() + ' and swinging them from side to side. The little creature watches them, mesmerized as he masturbates his foot-long erection.\n\n', false );
		MainView.outputText( 'You sit down in front of the little creature and grab ahold of his hair. The imp squeals slightly in pain before his cries are silenced with a ' + Descriptors.nippleDescript( 0 ) + '.  It fills his mouth as he yields, defeated. At once he starts to drink down as much of your milk as he can.\n\n', false );
		MainView.outputText( 'After a moment, he takes one of his hands off his large member and puts it against your ' + Appearance.biggestBreastSizeDescript() + ' to steady himself as he continues to nurse. You give a pleased sigh and simply bask in the sensations of pleasure that being nursed gives you.  You ruffle the little imp\'s hair affectionately. "<i>These creatures are so much nicer to be around when they just take their minds off their cocks,</i>" you think as you see his other hand relax and stop rubbing his swollen, demonic member.\n\n', false );
		MainView.outputText( 'You feel the imp\'s mighty gulps start to slow down until he lets out a sigh of relief. While imps may be small, they\'re very hungry creatures. Your ' + Descriptors.nippleDescript( 0 ) + ' slips out of the imp\'s mouth, and you gently lay it down on the ground. It gives a few gentle burps before dozing off; you can see that the imp\'s erection has retracted, and its belly has expanded significantly. You smile to yourself and, feeling fully satisfied, you stand up.', false );
	...
	ImpScene.prototype.impGangabangaEXPLOSIONS = function() {
		...
		MainView.outputText( '\n', false );
		MainView.outputText( '<b>You sleep uneasily. A small sound near the edge of your camp breaks into your rest and you awaken suddenly to find yourself surrounded by ' + CoC.monster.a + '</b>!\n\n', false );
		//CENTAUR;
		if( CoC.player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_CENTAUR ) {
			if( Utils.rand( 2 ) === 0 && (CoC.player.cockTotal() === 0 || CoC.player.gender === 3) ) {
				...
				if( CoC.player.findStatusAffect( StatusAffects.ImpGangBang ) < 0 ) {
					...
					if( CoC.player.weaponName !== 'fists' ) {
						MainView.outputText( ' while another kicks your weapon out of reach', false );
					}
					MainView.outputText( '.  The ' + CoC.monster.short + ' surges forward and grapples you. Imps grope your body and hump their ' + CoC.monster.cockDescriptShort( 0 ) + ' against your horse legs, smearing their sweat and pre-cum into your ' + CoC.player.skinDesc + '. The rest of the ' + CoC.monster.short + ', a dozen or more imps, all leer at you and laugh as they slap and pinch your body. The imps have sharp claws, tiny sharp teeth, and short horns on their heads. They scratch, claw, and bite at you with all of these weapons as they try to pull you down to the ground. One bold imp leaps forward and grabs your ', false );
					...
					if( CoC.player.cockTotal() > 0 ) {
						...
						MainView.outputText( Descriptors.nippleDescript( 0 ), false );
					}
					MainView.outputText( ', twisting and pinching hard enough to make you yelp in pain. An imp leaps up and mounts you, grabbing your ' + Descriptors.hairDescript() + ' like reins. The long flesh of his ' + CoC.monster.cockDescriptShort( 0 ) + ' rubs against the small of your back. The ' + CoC.monster.short + ' stinks of sweat and pre-cum, its moist grip and obscene smirk leaves you with no doubt as to what they will do to you if you lose this fight.\n\n', false );
				}
				MainView.outputText( 'The horde drags you to your knees, grappling your legs and crawling over your horse-body to pin you down. You try to buck them off but there are too many to fight. The imps drag your arms behind your back, wrapping them around your rider. Another imp whips off his loincloth to reveal his pre-cum drooling ' + CoC.monster.cockDescriptShort( 0 ) + ' and tosses the cloth to the imps holding your arms. They quickly tie your arms back with the sweat-damp loincloth.  ', false );
				//(If the player has breasts);
				if( CoC.player.biggestTitSize() > 1 ) {
					MainView.outputText( 'Having your arms tied behind your back forces your chest out, making your ' + CoC.player.allBreastsDescript() + ' stand out. They bounce as you struggle.  ', false );
				}
				MainView.outputText( 'The ' + CoC.monster.short + ' stroke themselves and rub their hands over your outstretched chest, smearing their pre-cum into your skin. The imp riding you bounces up and down, rubbing his sweaty ' + CoC.monster.ballsDescriptLight() + ' against your ' + CoC.player.skinDesc + ' while he yanks your hair.  ', false );
				//(Low Corruption);
				if( CoC.player.cor < 50 ) {
					MainView.outputText( 'Your face flushes with humiliation. Your imp rider twists your ' + Descriptors.hairDescript() + ' hard and you whimper in pain. Imps rub their cocks along your ' + Descriptors.hipDescript() + ' while others stroke themselves and jeer at your helplessness.  ', false );
				}
				//(High Corruption);
				else {
					MainView.outputText( CoC.monster.getCapitalA() + ' swarms over your body, some stroking themselves as they watch you squirm while others rub their cocks over your flanks. Your imp rider twists your hair, pulling your head back, and you moan in pleasure at the rough handling. Your ' + CoC.player.skinDesc + ' tingles as you start to flush with desire.  ', false );
				}
				MainView.outputText( 'You yelp in shock as you feel a sharp slap on your ass. You look back to see an imp pulling your tail up. He grins at you and slaps your ' + Descriptors.hipDescript() + ' again. He yanks your tail and slaps your ass one last time, then dives down to plant his face in your ' + Descriptors.vaginaDescript( 0 ) + '. His inhumanly nimble tongue teases the folds of your pussy and flicks at your ' + Descriptors.clitDescript() + '.  ', false );
				//(If the player has balls);
				if( CoC.player.balls > 0 ) {
					MainView.outputText( 'The tongue slides over your ' + Descriptors.sackDescript() + ', coating it with warm drool.  ', false );
				}
				//(Low Corruption);
				if( CoC.player.cor < 50 ) {
					MainView.outputText( 'You shake your hips, trying to escape the demonic tongue. The imp grips your ' + Descriptors.hipDescript() + ' and pulls his face further into your cunt, sliding his nimble tongue over your lips. You grit your teeth, trying to ignore the warmth spreading from your ' + Descriptors.vaginaDescript( 0 ) + '.', false );
				}
				//(High Corruption);
				else {
					MainView.outputText( 'You let out a shuddering sigh as the heat from your cunt spreads into the rest of your body. Your ' + Descriptors.hipDescript() + ' tremble as the tongue slides over the folds of your ' + Descriptors.vaginaDescript( 0 ) + '. The imp grips your flanks harder and dives his nimble tongue into your fuck-hole.', false );
				}
				MainView.outputText( '\n\n', false );
				//(If the character has breasts);
				if( CoC.player.biggestTitSize() > 1 ) {
					MainView.outputText( 'Hands slide over your ' + CoC.player.allBreastsDescript() + ', dragging your attention back to the front of the mob. Two imps grope your ' + Appearance.biggestBreastSizeDescript() + ', mauling your flesh as they drag your tits around your chest. They lick your tit-flesh, slowly working their way up towards your ' + Descriptors.nippleDescript( 0 ) + '. The imp rider drops your hair and reaches around you, shoving his cock against your back as he squeezes your ' + Appearance.biggestBreastSizeDescript() + '. Finally the imps reach your nipples, their tongues wrapping around and pulling at the tingling flesh.  ', false );
					//(Low Corruption);
					if( CoC.player.cor < 50 ) {
						MainView.outputText( 'You can\'t escape the tongues lapping and pulling at your ' + Descriptors.nippleDescript( 0 ) + ', matching the one in your cunt. You shake your head to deny the pleasure, but your breathing comes faster and faster as lust invades your body.', false );
					}
					//(High Corruption);
					else {
						MainView.outputText( 'The tongues squeezing and tugging your nipples match the tongue working your ' + Descriptors.vaginaDescript( 0 ) + ', flooding your body with lust. You moan and arch your back, offering your tits to the imps. You can hear your pulse pounding in your ears as you pant with desire.', false );
					}
					MainView.outputText( '  Suddenly you feel tiny needle-sharp teeth pierce your ' + Descriptors.nippleDescript( 0 ) + '. You scream as venom pumps into your tits, red-hot poison that makes your ' + CoC.player.allBreastsDescript() + ' feel as though they were being stung by bees. You moan in pain as your breasts start to swell, the imps continuing to pump demon-taint into them.\n\n', false );
					//Grow tits!;
					CoC.player.growTits( 2, CoC.player.breastRows.length, false, 1 );
					CoC.player.boostLactation( 0.3 );
				}
				MainView.outputText( 'Dimly through your haze of lust and pain you see a large imp step forward from the mob. Four feet tall and broader and stronger looking than any imp you\'ve seen before, with a face as much bull as imp, this new imp has mottled grey skin, broad purple demon wings, two curving bull-horns on his head, and a ' + Appearance.cockNoun( CockTypesEnum.HORSE ) + ' big enough to choke a minotaur. The mushroom-like head of it bobs just below his mouth, and his snake-tongue darts out to flick a bit of pre-cum off the head and onto your face. You shudder as the hot fluid stings the sensitive skin of your lips. His ' + CoC.monster.ballsDescriptLight() + ' are each the size of your fist and slick with sweat. He slaps his sweaty cock-head against your cheek, nearly scalding you with the heat.  ', false );
				//(Low corruption);
				if( CoC.player.cor < 50 ) {
					MainView.outputText( 'You yelp and twist your head to escape the heat.  ', false );
				}
				//(End low corruption) ;
				MainView.outputText( 'He slowly rubs his shaft over your cheeks and along your lips, each ridge of his demonically-hot ' + Appearance.cockNoun( CockTypesEnum.HORSE ) + ' tugging at your lips. The hot pre-cum dribbles over your sensitive flesh and the musk makes your sinuses tingle. The big imp sneers as you whimper, and whips his bull-shaft back to slap your face. The other imps watch and stroke themselves as their master cock-whips you.\n\n', false );
				//(If the character has breasts);
				if( CoC.player.biggestTitSize() > 2 ) {
					MainView.outputText( 'The big imp grabs one of your painfully distended breasts in each hand, mauling and bouncing the flesh as if weighing them. You gasp in pain as your ' + CoC.player.allBreastsDescript() + ' swell further at his touch. ', false );
				}
				MainView.outputText( 'Hot pre-cum dribbles through your lips and onto your tongue. The steaming salty goo is almost too hot to stand, and you stick your tongue out to cool it. The imps jerk their cocks harder as you pant, tongue hanging out of your mouth. The master imp steps back and looks you up and down, admiring his handiwork. His snake-tongue darts out to an incredible length and wraps itself around your tongue. He licks his pre-cum from you, then forces his tongue into your mouth. The master imp\'s tongue curves back into your mouth, pressing the glob of pre-cum into your throat. ', false );
				//(Low corruption);
				if( CoC.player.cor < 50 ) {
					MainView.outputText( 'It\'s either swallow or have that demon-tongue forced all the way down your throat. Against your will you gulp back the glob.', false );
				}
				//(High Corruption);
				else {
					MainView.outputText( 'You swallow the glob of pre-cum eagerly, trying to suck the demon\'s tongue into your throat.', false );
				}
				MainView.outputText( '\n\n', false );
				MainView.outputText( 'The big imp walks around you, casting his gaze over your pinned body.  ', false );
				//(If the character has breasts);
				if( CoC.player.biggestTitSize() > 2 ) {
					MainView.outputText( 'The other imps reclaim your aching breasts, sucking your ' + Descriptors.nippleDescript( 0 ) + ' and mauling your ' + CoC.player.allBreastsDescript() + ' so hard their fingers disappear into your swelling flesh. ', false );
				}
				MainView.outputText( 'The imp rubs his hands over your sides and flanks, his ' + Appearance.cockNoun( CockTypesEnum.HORSE ) + ' bobbing as he walks. The other imps watch their master as he moves around you. Only the imp sucking your ' + Descriptors.vaginaDescript( 0 ) + ' doesn\'t notice, his tongue thrusting deeply into your folds. The big imp grabs him by the neck and easily tosses him aside, his tongue dragging through your cunt as he\'s pulled away from you. The master imp takes position behind you and grabs his ' + Appearance.cockNoun( CockTypesEnum.HORSE ) + ', bringing the mushroom-head of it down to your pussy. You shake, knowing what\'s coming next. The other imps watch and stroke themselves as their master readies his hips to push into you.\n\n', false );
				//(Low corruption);
				if( CoC.player.cor < 50 ) {
					MainView.outputText( 'You scream for help', false );
				}
				//(High corruption) ;
				else {
					MainView.outputText( 'You moan with lust', false );
				}
				MainView.outputText( ' as the inhumanly hot cock-head stretches your pussy lips, your cries vanishing into the dark skies above. Your rider grabs your hair to pull your head back, and you cry out as his master pushes his corrupted cock into you.  ', false );
				//(If the character has breasts);
				if( CoC.player.biggestTitSize() > 1 ) {
					MainView.outputText( 'The imps working your breasts suck harder, kneading your tit-flesh as though trying to milk you. ', false );
				}
				MainView.outputText( 'You squirm and twist against the imps holding you down as the hot ' + Appearance.cockNoun( CockTypesEnum.HORSE ) + ' almost burns your sensitive cunt. You can smell the sweat steaming off his shaft, and your pussy-fluids start to steam as well as he forces his cock-head into your ' + Descriptors.vaginaDescript( 0 ) + '. His huge cock-head bulges your groin, and you moan', false );
				//(Low corruption);
				if( CoC.player.cor < 50 ) {
					MainView.outputText( ' in helpless terror as you feel the bulge work up from the base of your groin towards your stomach. You let out a shuddering moan of pain as inch after inch of monstrous ' + Appearance.cockNoun( CockTypesEnum.HORSE ) + ' stretches your belly', false );
				}
				//(High corruption);
				else {
					MainView.outputText( ', panting in lust as the monstrous ' + Appearance.cockNoun( CockTypesEnum.HORSE ) + ' pushes your flesh aside to make room for itself', false );
				}
				MainView.outputText( '. ', false );
				//(This is a good place for the virginity-loss message, if needed);
				CoC.player.cuntChange( CoC.monster.cockArea( 1 ), true );
				MainView.outputText( 'You can feel every ridge and pulsing vein of his cock pulling on the lining of your stretched cunt. You tremble helplessly around the huge shaft, fully impaled on the imp\'s mutated bull-cock.\n\n', false );
				MainView.outputText( 'Every pulse of his heart makes his cock twitch, making you shake in time to the shaft pulsing in your cunt. The imps jeer at you, masturbating over your shaking body. The big imp flexes his thighs, and his cock-head throbs deep in your belly. The other imps laugh as you ', false );
				//(Low corruption);
				if( CoC.player.cor < 50 ) {
					MainView.outputText( 'whimper, spasming as the hot shaft presses against new areas', false );
				}
				//(High corruption);
				else {
					MainView.outputText( 'moan in pleasure, rotating your hips around this incredible cock', false );
				}
				MainView.outputText( ' in your stuffed ' + Descriptors.vaginaDescript( 0 ) + '. The big imp sneers and flexes his cock again, watching ', false );
				//(If the character has breasts);
				if( CoC.player.biggestTitSize() >= 2 ) {
					MainView.outputText( 'your ' + CoC.player.allBreastsDescript() + ' roll on your chest as you squirm', false );
				}//(If the character doesn't have breasts);
				else {
					MainView.outputText( 'your eyes roll back as you squirm', false );
				}
				MainView.outputText( '.\n\n', false );
				MainView.outputText( 'Finally the big imp pulls back his ' + Appearance.cockNoun( CockTypesEnum.HORSE ) + ', each ridge pulling on your pussy flesh as he slides out. You yelp and buck as the mushroom-head catches on your folds. ', false );
				//(If the character has a cock);
				if( CoC.player.cockTotal() > 0 ) {
					MainView.outputText( 'Your ' + Descriptors.multiCockDescriptLight() + ' bounces as the bulge passes over it.  ', false );
				}
				MainView.outputText( 'You moan as the mushroom-head reaches the entrance of your ' + Descriptors.vaginaDescript( 0 ) + ', your stretched pussy-flesh slowly returning to normal. The master imp pushes forward again, reclaiming your pussy for his monstrous cock. ', false );
				//(Low corruption);
				if( CoC.player.cor < 50 ) {
					MainView.outputText( 'You try to buck your ' + Descriptors.hipDescript() + ', fighting to break free as the bulge of his cock-head works its way high up into your belly. You\'re held down by too many imps. You can only writhe around the hot shaft stretching out your ' + Descriptors.vaginaDescript( 0 ) + '. The big imp grunts as his cock-head pops past your cervix, and you moan and shake in pain.  ', false );
				}
				//(High corruption);
				else {
					MainView.outputText( 'You moan in ecstasy as the hot ' + Appearance.cockNoun( CockTypesEnum.HORSE ) + ' pushes deep into your ' + Descriptors.vaginaDescript( 0 ) + ', turning every inch of your pussy into a pleasure-sheath for the big imp. You know you\'re nothing but a fuck-toy for this corrupt creature, just a wet pussy for him to fill with cum, and the thought almost makes you orgasm as he forces his huge cock-head past your cervix.  ', false );
				}
				MainView.outputText( 'Finally the corrupt cock bottoms out against your womb. The imp pulls back again, and starts to fuck you slowly.\n\n', false );
				//(If the character has breasts);
				if( CoC.player.biggestTitSize() >= 2 ) {
					MainView.outputText( 'The slow fucking shakes your breasts, and the imps sucking at your nipples cling tightly to your monstrously swollen ' + CoC.player.allBreastsDescript() + '. Your ' + Appearance.biggestBreastSizeDescript() + ' have grown three cup sizes since the imps pumped their venom into you. An ache starts deep in the base of your tits and works its way to your sore ' + Descriptors.nippleDescript( 0 ) + '. Your already bloated nipples swell as the imps suckle and you gasp as the first rush of milk spills into their mouths. Your rider reaches around and starts to milk your udders, moving his hands between your ' + CoC.player.allBreastsDescript() + ' and forcing out more milk for his gangmates.\n\n', false );
				}
				MainView.outputText( 'The big imp grinds his hips as he thrusts and pulls, rubbing his cock-ridges against every part of your ' + Descriptors.vaginaDescript( 0 ) + '. While sliding his mutated ' + Appearance.cockNoun( CockTypesEnum.HORSE ) + ' in and out of you, the imp rubs his hands along your mound, pulling it open or forcing it tight as he takes you. Your pussy juices steam off his cock as he pumps, and hot pre-cum dribbles down your crack and ', false );
				//(If the character has a cock);
				if( CoC.player.cockTotal() > 0 ) {
					MainView.outputText( 'over your ' + Descriptors.multiCockDescriptLight() + ' where it ', false );
				}
				MainView.outputText( 'drips onto the ground. ', false );
				//(Low corruption);
				if( CoC.player.cor < 50 ) {
					MainView.outputText( 'The pain as this huge cock stretches you is overwhelming, but every thrust rubs more corrupt pre-cum into your pussy walls. You start to pant as the imp rapes you, using your body for his own pleasure. You tremble as the heat of his pre-cum soaks through your body. The huge shaft forces your ' + Descriptors.clitDescript() + ' out, and the steaming fluids splashing on it make it tingle almost painfully. Your whimpers and moans of pain start to take on a different tone, and the master imp starts to fuck you faster.', false );
				}
				//(High corruption);
				else {
					MainView.outputText( 'Pain and pleasure blend into one as the huge ' + Appearance.cockNoun( CockTypesEnum.HORSE ) + ' stretches you, rubbing pre-cum into your steaming pussy. You moan as the big imp fucks you, turning you into a mindless fuck-puppet. Your ' + Descriptors.clitDescript() + ' swells painfully as hot juices splash over it. Your shaking body only adds to the master imp\'s pleasure.', false );
				}
				MainView.outputText( '\n\n', false );
				MainView.outputText( 'The other imps continue to jerk-off over you as the big imp impales you again and again on his shaft. Their pre-cum starts to splatter down on your body, and they pant as they watch your orgasm build. ', false );
				//(If the character has breasts);
				if( CoC.player.biggestTitSize() > 1 ) {
					MainView.outputText( 'Imps gulp milk from your bloated ' + Appearance.biggestBreastSizeDescript() + '. As one imp drinks his fill and staggers away with a sloshing belly, another steps up to pump your milk-spewing udders.  ', false );
				}
				//(If the character has a dick);
				if( CoC.player.cockTotal() > 0 ) {
					MainView.outputText( 'Your ' + Descriptors.multiCockDescriptLight() + ' swell painfully as the rough fucking pumps blood into your groin.  ', false );
				}
				MainView.outputText( 'The big imp\'s snake tongue flicks out and slides around your ' + Descriptors.vaginaDescript( 0 ) + ', pulling at your pussy lips. He moves his tongue back and forth along the sides of your steaming cunt, alternating between stretching and flicking the lips. ', false );
				//(If the character has a dick);
				if( CoC.player.totalCocks() > 0 ) {
					MainView.outputText( 'He draws his tongue back and wraps it around your ' + Descriptors.cockDescript( 0 ) + ', sliding its length along your shaft and flicking his tongue over your cock-head.  ', false );
				}
				MainView.outputText( 'You gasp in time to the big imp\'s thrusts, whimpering when his cock or tongue hit a sensitive point. ', false );
				//(Low Corruption);
				if( CoC.player.cor < 50 ) {
					MainView.outputText( 'You\'re being raped by a demon, milked like a cow, and you\'re about to cum hard. This corrupted land has left its mark on you.', false );
				}
				//(High corruption);
				else {
					MainView.outputText( 'This corrupted land has left its mark on you. You could never have taken a cock this big before you arrived here.', false );
				}
				MainView.outputText( ' You moan as you rise towards your orgasm.\n\n', false );
				//(If the character has breasts);
				if( CoC.player.biggestTitSize() > 3 ) {
					MainView.outputText( 'Your udders shake back and forth under your chest in time to the rough fucking. You arch your back to press your ' + Descriptors.nippleDescript( 0 ) + ' into eager mouths, moaning as your rider milks your distended ' + CoC.player.allBreastsDescript() + '. ', false );
				}
				//(Low Corruption).;
				if( CoC.player.cor < 50 ) {
					MainView.outputText( 'Some part of you can still feel shame, and you whine and clench your teeth as the urge to <i>moo</i> rises in you.', false );
				}
				//(High corruption);
				else {
					MainView.outputText( 'You moan shamelessly as you\'re fucked and milked, and the moans turn to long <i>mooos</i> of ecstasy.', false );
				}
				MainView.outputText( '\n\n', false );
				MainView.outputText( 'The master imp pounds into you as hard as he can, driving his ' + CoC.monster.cockDescriptShort( 1 ) + ' deeper into your cunt. His grunts come closer and closer together. Your rider grinds his cock into your back, rubbing his cock-head in your hair. He nips at your neck and shoulder as he pants. The master imp pounds into you and you can feel his ' + CoC.monster.ballsDescriptLight() + ' swell as they slap against you. Through the haze of your approaching orgasm you realize what\'s about to happen. Those oversized balls are about to pump more cum into you than any normal man could ever produce. They\'re going to pump demonic cum right into your womb. ', false );
				//(Low Corruption);
				if( CoC.player.cor < 50 ) {
					MainView.outputText( 'You scream as the base of his ' + Appearance.cockNoun( CockTypesEnum.HORSE ) + ' bloats with corrupted jism, the thick bulge stretching your pussy even more as it pumps along the imp\'s shaft. The bulge swells your belly and you can feel it move through your stretched cunt towards your womb. Another thick bulge forms at the base of the master imp\'s cock and you thrash wildly, yelling in protest. "<i>NOO - O - O - OOOOHhh!</i>" The hot cum floods into your womb and you reach your own orgasm, shaking as your ' + Descriptors.vaginaDescript( 0 ) + ' clamps down on his cock and milks it of waves of cum. Another orgasm hits on the heels of the first one, and you buck as more demon-cum floods your womb. Gasping for air, you continue to come as your belly swells. Even as he pumps more corrupt cum into you the big imp keeps raping you, forcing you to another peak before you\'ve come down from the last one.', false );
				}
				//(High corruption);
				else {
					MainView.outputText( 'The thought of all that demon-jism in your womb pushes you over the edge. You cum hard, bucking your hips against the ' + Appearance.cockNoun( CockTypesEnum.HORSE ) + ' pumping hot cum into your belly. Your eyes roll back in your head and you scream out in ecstasy as thick jets of cum fill your pussy. The imp keeps thrusting into his fuck-toy even as he fills your womb with his cum, forcing you to another peak before you\'ve come down from the last one. The big imp is your master now.', false );
				}
				MainView.outputText( '  You nearly black out as the orgasm blasts through you,  shrieking yourself hoarse as the orgasm wracks your body, eyes rolling back in your head as your womb swells.\n\n', false );
				//(If the character has breasts);
				if( CoC.player.biggestTitSize() > 2 ) {
				...
				//(High corruption);
				if( CoC.player.cor >= 50 ) {
					MainView.outputText( 'You lap eagerly at the salty cum, licking up and drinking as much as you can.', false );
				}
				MainView.outputText( '\n\n', false );
				MainView.outputText( 'Imp-jism rains down on your helpless spasming body. The imps spew cum into your hair, across your back and ' + Descriptors.hipDescript() + ', over your face', false );
				//(If the character has breasts);
				if( CoC.player.biggestTitSize() > 2 ) {
					MainView.outputText( ', and bouncing ' + CoC.player.allBreastsDescript(), false );
				}
				MainView.outputText( '. The ' + CoC.monster.short + ' is no longer holding you down. They masturbate over you as you claw at the ground with your hands, hooves scraping the earth as you clamp your thighs tight around the big imp. Another pulse of demonic cum hits your womb. You push back against your master, forcing as much of his cock into you as possible. Arching your back, your eyes roll back in your head and you moo as your womb stretches painfully, a final orgasm crashing through your cum-bloated body. You spasm around the cock that impales you, thrashing as ', false );
				//(If the character has breasts);
				if( CoC.player.biggestTitSize() > 2 ) {
					MainView.outputText( 'milk spurts from your ' + Descriptors.nippleDescript( 0 ) + ' and ', false );
				}
				MainView.outputText( 'steaming fluids spew from your over-filled pussy. Unconsciousness follows closely on the heels of this last orgasm, your mind shutting down even as your body still shudders.\n\n', false );
				MainView.outputText( 'You wake up later, body still twitching as tiny orgasms spark in your ' + Descriptors.vaginaDescript( 0 ) + '. It\'s still dark out. You lie on your side in a pool of cooling cum, milk, and pussy juice. Your body is covered in long ropes of drying imp-cum, and your hair is plastered to the ground. There\'s no sign of the horde of imps or their big master. Your skin is stretched and shiny over your still milk-bloated tits. Your belly is as tight and distended as a mare on the verge of giving birth. It quivers as the flesh of your ' + Descriptors.vaginaDescript( 0 ) + ' spasms. Over the swollen curve of your belly you can see steam rising from between your legs. You start to slip back into unconsciousness. ', false );
				//(Low corruption);
				if( CoC.player.cor < 50 ) {
					MainView.outputText( 'Your last coherent thought is to find a way to better hide your camp, so this never happens again.', false );
				}
				//(High corruption);
				else {
					MainView.outputText( 'Your last coherent thought is to find a way to make your own mutated master imp, maybe even a stable full of them...', false );
				}
				CoC.player.orgasm();
			...
			//Scene number 2 - male possible.;
				...
				if( CoC.player.findStatusAffect( StatusAffects.ImpGangBang ) >= 0 ) {
					...
					//(Subsequent encounters - High Corruption);
						...
						MainView.outputText( 'It\'s about time they showed up. It\'s not like there\'s a lot to do in these rocks, and you were getting bored. You grab an imp dick in either hand and spread your legs as other imps grope your thighs...\n\n', false );
					}
				}
				MainView.outputText( 'The imp mob tackles you, grabbing at your arms as you ', false );
				//(Low Corruption);
				if( CoC.player.cor < 50 ) {
					MainView.outputText( 'swing your ' + CoC.player.weaponName + ' wildly, determined not to let them take you', false );
				}
				//(High Corruption);
				else {
					MainView.outputText( 'twist and struggle in their grips, determined to make them work for their fun', false );
				}
				MainView.outputText( '! You kick back and feel your hooves smash into an imp\'s chest, sending him flying. But the ' + CoC.monster.short + ' has your legs and more imps grab your arms. The pack drags you thrashing and bucking over to an old log lying on the ground.\n\n', false );
				MainView.outputText( 'Your human torso is dragged down to the log by ' + CoC.monster.a + ' while two more leap onto your back. The ' + CoC.monster.short + ' makes short work of your ' + CoC.player.armorName + ', unbuckling straps and stripping you quickly. ', false );
				//(If the player has breasts);
				if( CoC.player.biggestTitSize() > 0 ) {
					MainView.outputText( 'Your unbound ' + Appearance.biggestBreastSizeDescript() + ' bounce out over the weathered log. ', false );
				}
				MainView.outputText( 'The imps spread your arms wide, forcing your chest out, and tie them to the log with sweaty loincloths. Your ' + Descriptors.hipDescript() + ' are stuck high in the air. Imps rub their sweaty cocks and ' + CoC.monster.ballsDescriptLight() + ' over your legs and grope your crotch. The two imps riding your back start stroking and licking each other. ', false );
				//(Low Corruption);
				if( CoC.player.cor < 50 ) {
					MainView.outputText( 'Your face flushes with humiliation as they turn their attentions on each other, each working their hands and tongue over the other\'s dick. How dare these demons use you as a bed to sate their lusts?!', false );
				}
				//(High Corruption);
				else {
					MainView.outputText( 'Your face flushes with anger as they turn their attentions on each other, each working their hands and tongue over the other\'s dick. You worked hard for this magnificent body, and now they\'re not using it?!', false );
				}
				MainView.outputText( '\n\n', false );
				MainView.outputText( 'An imp quickly climbs up your body, planting his feet on your shoulders and grabbing your ' + Descriptors.hairDescript() + ' with one hand for support. He rubs his ' + CoC.monster.ballsDescriptLight() + ' over your mouth, smearing your lips with musky sweat, while he pries at your jaw with his other hand. ', false );
				//(If the player has breasts);
				if( CoC.player.biggestTitSize() > 2 ) {
					MainView.outputText( 'An imp mounts the log and slaps his ' + CoC.monster.cockDescriptShort( 0 ) + ' between your ' + CoC.player.allBreastsDescript() + ', squeezing them tight over his cock as he rubs back and forth. He mauls your breasts cruelly, squeezing his fingers deep into your soft flesh.  ', false );
				}
				//(If the player has a SINGLE cock);
				if( CoC.player.cockTotal() === 1 ) {
					MainView.outputText( 'An imp ducks under your body and grabs your ' + Descriptors.cockDescript( 0 ) + '. His nimble tongue flicks over your cock-head while he pricks the shaft with his tiny claws.  ', false );
				}
				//(If the player has a MULTI cock);
				if( CoC.player.cockTotal() > 1 ) {
					MainView.outputText( 'Two imps duck under your body and seize your ' + Descriptors.multiCockDescriptLight() + ', licking the tips with their inhumanly flexible tongues while they stroke the shafts.  ', false );
				}
				//(Low Corruption);
				if( CoC.player.cor < 50 ) {
					MainView.outputText( 'You fight to free your hind legs and buck the imps off your back, while sweaty hands slide over your crotch. You whine through clenched teeth as sharp claws jab at your sensitive flesh.\n\n', false );
				}
				//(High Corruption);
				else {
					MainView.outputText( 'You writhe in the grasp of the imps, reveling in the sensations as tiny claws and teeth nip at your sensitive crotch. You lick salty musk off the swollen balls dangling above your mouth.\n\n', false );
				}
				MainView.outputText( '\n\n', false );
				//(If the player has breasts);
				if( CoC.player.biggestTitSize() > 2 ) {
					MainView.outputText( 'The imp fucking your ' + Appearance.biggestBreastSizeDescript() + ' handles your soft flesh roughly, pressing and pulling your tits into a fuck-canal for his demon cock. Other imps slap your ' + CoC.player.allBreastsDescript() + ' and laugh as you cry out.  ', false );
				}
				//(Low Corruption);
				if( CoC.player.cor < 50 ) {
					MainView.outputText( 'You whimper as your mistreated flesh stings with dozens of pin-prick scratches and bites, and the ' + CoC.monster.short + ' slaps your chest and flanks. The abuse falls on you from all sides, leaving you with no escape. The imp on your shoulders pries your jaws open, and you gag on his ' + CoC.monster.ballsDescriptLight() + '.', false );
				}
				//(High Corruption);
				else {
					MainView.outputText( 'You suckle eagerly at the musky balls in your mouth. Abuse falls on you from all sides, imps leaving tiny marks on your skin as they nip and scratch at you. You whimper in delight as tiny hands slap your chest and flanks.', false );
				}
				MainView.outputText( '\n\n', false );
				MainView.outputText( 'With a loud sucking sound, the imp pulls his balls out of your mouth. Spit and ball-sweat drip over your cheeks as he repositions himself, bending almost completely over on your shoulders to rub his cock-head against your lips. You nearly choke as pre-cum dribbles into your mouth and runs down the back of your throat. The ' + CoC.monster.cockDescriptShort( 0 ) + ' blocks most of your vision, but in the corners of your eyes you see the master of this imp horde step forward. Four feet tall and broader and stronger than any imp in the pack, with a face as much dog as imp, this new imp has black fur, broad red demon wings, two long demon-horns on his head, and a ' + Appearance.cockNoun( CockTypesEnum.DOG ) + ' big enough to choke a minotaur. He leers at your helpless body and grabs ', false );
				//(If the player has breasts);
				if( CoC.player.biggestTitSize() > 2 ) {
					MainView.outputText( 'one of your sore ' + Appearance.biggestBreastSizeDescript() + ' in his calloused hand, brutally pressing his fingers into your flesh', false );
				}//(If the player doesn't have breasts);
				else {
					MainView.outputText( 'your tail and yanks, brutally pulling on it', false );
				}
				MainView.outputText( ' until you shriek. The imp riding your shoulders plunges his ' + CoC.monster.cockDescriptShort( 0 ) + ' into your mouth, pounding at the top of your throat.\n\n', false );
				MainView.outputText( 'The master imp walks back to your hips, lightly dragging his sharp claws over your flanks. He kicks another imp out of the way and takes position behind your ' + Descriptors.hipDescript() + '. He pulls his monstrously long ' + Appearance.cockNoun( CockTypesEnum.DOG ) + ' down and rubs the tip over your ', false );
				...
				//(If the player has a cock);
				if( CoC.player.cockTotal() > 0 ) {
					MainView.outputText( 'Pre-cum drips from the broad tip of it, dripping down to the base of your ' + Descriptors.multiCockDescriptLight() + '.  ', false );
				}
				MainView.outputText( 'The big imp\'s hot pre-cum stings your flesh. The imps licking your crotch lap up the hot fluid, cooling you with their saliva. The big imp sneers as you whimper, and presses the head of his ' + Appearance.cockNoun( CockTypesEnum.DOG ) + ' against your ', false );
				//(If the player has a vagina);
				if( CoC.player.hasVagina() ) {
					MainView.outputText( Descriptors.vaginaDescript( 0 ), false );
				}
				...
				//(Low Corruption);
				if( CoC.player.cor < 50 ) {
					MainView.outputText( 'You try to pull away from the hot cock-head rubbing against your hole, but the ' + CoC.monster.short + ' holds you tight.', false );
				}
				//(High Corruption);
				else {
					MainView.outputText( 'The scent of musk steaming off the' + Appearance.cockNoun( CockTypesEnum.DOG ) + ' drives you wild, and you push back to try and capture the cock-tip.', false );
				}
				MainView.outputText( '\n\n', false );
				MainView.outputText( 'The pointed tip of the master imp\'s ' + Appearance.cockNoun( CockTypesEnum.DOG ) + ' plunges into your hole, splitting your ', false );
				//(If the player has a vagina);
				if( CoC.player.hasVagina() ) {
					MainView.outputText( Descriptors.vaginaDescript( 0 ), false );
				}
				...
				//(If the player has breasts);
				if( CoC.player.biggestTitSize() > 2 ) {
					MainView.outputText( '  The imp fucking your aching ' + Appearance.biggestBreastSizeDescript() + ' paints your tits with a massive load of cum. He falls off the log and another imp jumps up to take his place.', false );
				}
				MainView.outputText( '\n\n', false );
				MainView.outputText( 'The big imp fucks you roughly, clenching your ' + Descriptors.hipDescript() + ' in his clawed hands as he hammers his ' + Appearance.cockNoun( CockTypesEnum.DOG ) + ' into you. The head of his mutated shaft pounds ', false );
				//(If the player has a vagina);
				if( CoC.player.hasVagina() ) {
					MainView.outputText( 'the entrance of your womb', false );
				}//(If the player doesn't have a vagina);
				else {
					MainView.outputText( 'depths of your bowels', false );
				}
				MainView.outputText( ' as the knot slams against your ', false );
				//(If the player has a vagina);
				if( CoC.player.hasVagina() ) {
					MainView.outputText( Descriptors.vaginaDescript( 0 ), false );
				}
				...
				MainView.outputText( '. Each hard thrust pounds you against the log, and you grunt in time to the shaft pistoning in your hole.\n\n', false );
				MainView.outputText( 'The master imp fucks you for what seems like hours, beating his dog-knot against your sore ', false );
				//(If the player has a vagina);
				if( CoC.player.hasVagina() ) {
					MainView.outputText( Descriptors.vaginaDescript( 0 ), false );
				}
				...
				MainView.outputText( ' and slapping your ass every few thrusts to remind you who is in charge. Imp after imp stretches your throat with their cocks and your belly with demon-seed as the pack rapes your face. ', false );
				//(If the character has breasts);
				if( CoC.player.biggestTitSize() > 2 ) {
					MainView.outputText( 'The rough fucking shakes your cum-stained breasts, and the imp fucking your ' + CoC.player.allBreastsDescript() + ' clings tightly to your red and swollen tit flesh. Your ' + Appearance.biggestBreastSizeDescript() + ' burn with agony as the ' + CoC.monster.short + ' slaps your tits like drums.  ', false );
				}
				//(Low Corruption);
				if( CoC.player.cor < 50 ) {
					MainView.outputText( 'You\'re being raped again by demons, impaled on cocks like a roast pig on a spit, and you can feel your lust rising. This corrupted land has left its mark on you.', false );
				}
				//(High corruption);
				else {
					MainView.outputText( 'This corrupted land has left its mark on you. You could never have taken a cock this big before you arrived here.', false );
				...
				//(Low Corruption);
				if( CoC.player.cor < 50 ) {
					MainView.outputText( 'You gurgle helplessly as the cock raping your throat pours thick wads of', false );
				}
				//(High Corruption);
				else {
					MainView.outputText( 'You eagerly chug thick wads of cum from the cock stretching your throat, working your throat to force more', false );
				}
				MainView.outputText( ' cum into your swelling belly. The imp slams his cock as deep into your throat as it will go, slapping his ' + CoC.monster.ballsDescriptLight() + ' against your face. He cums for an impossibly long time, streams of jism pouring into you. You can feel your stomach stretching, but you\'re more worried about breathing. The edge of your vision starts to go red and your chest heaves as you fight for air. Finally the imp draws his cock out of your throat, spraying his last gobs of cum over your face as you gasp in huge lungfuls of air. The sudden rush of oxygen pushes you over the edge and you cum hard. Your hands clench at the air and your eyes roll back in your head as you twist around the demonic ' + Appearance.cockNoun( CockTypesEnum.DOG ) + ' pounding into you. You shriek as your ', false );
				//(If the player has a vagina);
				if( CoC.player.hasVagina() ) {
					MainView.outputText( Descriptors.vaginaDescript( 0 ), false );
				}
				...
				MainView.outputText( ' spasms on the steaming pole that impales it. Another imp shoves his cock in your mouth as you scream, throat convulsing around his cock-head.', false );
				//(If the player has a cock);
				if( CoC.player.cockTotal() > 0 ) {
					MainView.outputText( '  Your ' + Descriptors.multiCockDescriptLight() + ' shoots cum across the ground and into the waiting mouths of the imps licking your crotch.', false );
				}
				MainView.outputText( '\n\n', false );
				MainView.outputText( 'Another imp-cock spasms in your throat as its owner rams deep into you. He floods your already swollen stomach with inhuman amounts of cum. Again you feel yourself about to black out as the demon pumps jism into you. He pulls out and again you orgasm as you wheeze for air. Another imp forces his cock down your throat as you moan and gasp. Your body shakes in pleasure on the big imp\'s ' + Appearance.cockNoun( CockTypesEnum.DOG ) + '.  Tightening his grip on your ' + Descriptors.hipDescript() + ' the master imp howls and slams his shaft into your ', false );
				//(If the player has a vagina);
				if( CoC.player.hasVagina() ) {
					MainView.outputText( Descriptors.vaginaDescript( 0 ), false );
				}
				...
				MainView.outputText( '. His unnaturally huge knot stretches the entrance of your hole, and he hammers into you again. ', false );
				//(Low Corruption);
				if( CoC.player.cor < 50 ) {
					MainView.outputText( 'You howl around the imp-cock stretching your throat. The bloated knot opens your hole far beyond anything you\'ve endured before. Your violent thrashing throws the imps off your back and you buck uselessly, thrashing as the swollen ' + Appearance.cockNoun( CockTypesEnum.DOG ) + ' plunges deeper into you.', false );
				}
				//(High corruption);
				else {
					MainView.outputText( 'The master imp\'s bloated knot stretches your entrance and plunges into your hole with a loud <i>pop</i>. Another orgasm hits you as the ' + Appearance.cockNoun( CockTypesEnum.DOG ) + ' rams even deeper into you. You howl around the imp-cock stretching your throat, bucking as your orgasm shakes you. Your violent thrashing throws the imps off your back and slams your hips against the big imp, pushing him further into your hole.', false );
				}
				MainView.outputText( '  The big imp howls again as he cums, each wave of steaming demon-cum stretching his knot and shaft even more. His cum-pumping ' + Appearance.cockNoun( CockTypesEnum.DOG ) + ' is bottomed out deep in your ', false );
				//(If the player has a vagina);
				if( CoC.player.hasVagina() ) {
					MainView.outputText( 'womb', false );
				}//(If the player doesn't have a vagina);
				else {
					MainView.outputText( 'guts', false );
				}
				MainView.outputText( ' and he pumps more jism into you than his balls could possibly hold. Your belly stretches with every blast of cum and you shriek around yet another cock in your throat.\n\n', false );
				//(If the character has breasts);
				if( CoC.player.biggestTitSize() > 2 ) {
					MainView.outputText( 'The imp riding your ' + Appearance.biggestBreastSizeDescript() + ' cums, his load lost in the flood of jism dripping off your abused fuck-udders. ', false );
				}
				MainView.outputText( 'Your master isn\'t done with you yet, churning his ' + Appearance.cockNoun( CockTypesEnum.DOG ) + ' knot in your ', false );
				//(If the player has a vagina);
				if( CoC.player.hasVagina() ) {
					MainView.outputText( Descriptors.vaginaDescript( 0 ), false );
				}
				...
				MainView.outputText( '. Your legs give out as you feel more pulses of demon-cum work their way up his shaft and into your already-huge belly.\n\n', false );
				MainView.outputText( 'You pass out as another tidal wave of corrupted jism spews into your hole, another load of imp-cum pours down your throat, to meet somewhere in the middle...\n\n', false );
				MainView.outputText( 'You wake up later, still trembling with small orgasms. Cum burbles in your mouth as you breathe, and your ' + Descriptors.hairDescript() + ' is soaked with jism. You haven\'t moved since you passed out. Your arms are still tied to the log, ', false );
				//(If the player has breasts);
				if( CoC.player.biggestTitSize() > 0 ) {
					MainView.outputText( 'your bruised and throbbing tits pressed against the rough wood, ', false );
				}
				MainView.outputText( 'and your body rests in a cooling pool of cum. You couldn\'t move even if your ' + CoC.player.legs() + ' felt stronger. Your hideously bloated belly weighs you down, quivering with every orgasmic twitch that passes through you. The skin of your distended belly is drum-tight and shiny. As you slip back into unconsciousness, one last thought flits across your mind. ', false );
				//(Low Corruption);
				if( CoC.player.cor < 50 ) {
					MainView.outputText( 'How long can you last in this corrupted land, when your body can be so horribly twisted by the sick pleasures of its denizens?', false );
				}
				//(High corruption);
				else {
					MainView.outputText( 'Why bother with your silly quest, when you\'ve only scratched the surface of the pleasures this land offers you?\n', false );
				}
				CoC.player.orgasm();
				EngineCore.dynStats( 'lib', 2, 'cor', 3 );
				CoC.player.knockUp( PregnancyStore.PREGNANCY_IMP, PregnancyStore.INCUBATION_IMP - 14 ); //Bigger imp means faster pregnancy
		...
		//NOT CENTAUR;
			...
			if( Utils.rand( 2 ) === 0 && (CoC.player.cockTotal() === 0 || CoC.player.gender === 3) ) {
				...
				MainView.outputText( '. The ' + CoC.monster.short + ' surges forward and grapples you. Imps grope your body and hump their ' + CoC.monster.cockDescriptShort( 0 ) + ' against your legs, smearing their sweat and pre-cum into your ' + CoC.player.skinDesc + '. The rest of the ' + CoC.monster.short + ', a dozen or more imps, all leer at you and laugh as they slap and pinch your body. The imps have sharp claws, tiny sharp teeth, and short horns on their heads. They scratch, claw, and bite at you with all of these weapons as they try to pull you down to the ground. One bold imp leaps forward and grabs your ', false );
				//(If the player has a cock);
				if( CoC.player.cockTotal() > 0 ) {
					MainView.outputText( Descriptors.cockDescript( 0 ), false );
				} else {
				...
				MainView.outputText( ', twisting and pinching hard enough to make you yelp in pain. The ' + CoC.monster.short + ' stinks of sweat and pre-cum, and their moist grips and obscene smirks leave you with no doubts about what they will do to you if you lose this fight.\n\n', false );
				//(Bipedal, vaginal);
				MainView.outputText( 'The ' + CoC.monster.getCapitalA() + ' overwhelms you, dragging you to the ground with sheer numbers. There are at least two imps on each limb, holding you spread-eagled on the cold ground while other imps stroke your body. ', false );
				//(If the player has breasts);
				if( CoC.player.biggestTitSize() > 0 ) {
					MainView.outputText( 'Imps surround your chest, slapping their ' + CoC.monster.cockDescriptShort( 0 ) + 's on your ' + CoC.player.allBreastsDescript() + ' and rubbing their slippery pre-cum into your ' + Descriptors.nippleDescript( 0 ) + '.  ', false );
				}
				MainView.outputText( 'Others stand over your head, their cocks bobbing inches from your face as they jack off. A thick musk wafts off their cocks, and the smell of it makes your sinuses tingle. Two more imps take position between your legs, sliding their cocks along your thighs while stroking your ' + Descriptors.vaginaDescript( 0 ) + ' and flicking your ' + Descriptors.clitDescript() + '.', false );
				//(If the player has a cock);
				if( CoC.player.cockTotal() > 0 ) {
					MainView.outputText( 'An imp rubs his hand across his cock-head, smearing it with his pre-cum. He rubs his hand over your ' + Descriptors.multiCockDescriptLight() + ', making your cock-skin tingle as his fluid soaks into you.', false );
				}
				MainView.outputText( '\n\n', false );
				MainView.outputText( 'The ' + CoC.monster.short + ' snickers lewdly as your nipples harden and your pussy moistens. One of the imps between your legs slides his shaft along your pussy lips, teasing your ' + Descriptors.clitDescript() + ' with the tip of his cock.  ', false );
				//(Low corruption);
				if( CoC.player.cor < 50 ) {
					MainView.outputText( 'You renew your struggles, trying to break free of your captors. They only laugh and bear down harder on you.  ', false );
				}
				//(High corruption);
				else {
					MainView.outputText( 'You buck your hips, trying to capture his ' + CoC.monster.cockDescriptShort( 0 ) + ' with your ' + Descriptors.vaginaDescript( 0 ) + '.  ', false );
				}
				MainView.outputText( 'Before he can thrust into you, the imp is shoved aside by the biggest imp you\'ve ever seen.\n\n', false );
				MainView.outputText( 'Four feet tall and broader and healthier looking than any imp you\'ve seen before, with a face as much bull as imp, this new imp has mottled grey skin, broad purple demon wings, two curving bull-horns on his head, and a ' + Appearance.cockNoun( CockTypesEnum.HORSE ) + ' big enough to choke a minotaur. The mushroom-like head of it bobs just below his mouth, and his snake-tongue darts out to flick a bit of pre-cum off the head and onto your groin. You shudder as the hot fluid stings the sensitive skin of your ' + Descriptors.vaginaDescript( 0 ), false );
				//(If the player has a dick);
				if( CoC.player.cockTotal() > 0 ) {
					MainView.outputText( ' and ' + Descriptors.multiCockDescriptLight(), false );
				}
				MainView.outputText( '. His ' + CoC.monster.ballsDescriptLight() + ' are each the size of your fist and slick with sweat. He slaps his sweaty balls against your ' + Descriptors.vaginaDescript( 0 ) + ' nearly scalding you with the heat.  ', false );
				//(Low corruption);
				if( CoC.player.cor < 33 ) {
					MainView.outputText( 'You yelp and buck your hips to escape the heat.  ', false );
				}
				MainView.outputText( 'He grabs your hips and slowly drags his shaft down your pussy, each ridge of his demonically-hot ' + Appearance.cockNoun( CockTypesEnum.HORSE ) + ' hitting your clit and pulling at your lips. Finally the broad horse-like head of his shaft catches on your ' + Descriptors.clitDescript() + ', and the hot pre-cum dribbles over your sensitive flesh. The big imp sneers as you whimper, and drags his cock-head down to the opening of your ' + Descriptors.vaginaDescript( 0 ) + '. The other imps watch and stroke themselves as their master pulls his hips back to push into you.\n\n', false );
				//(Low corruption);
				if( CoC.player.cor < 50 ) {
					MainView.outputText( 'You scream for help', false );
				}
				...
				//(Low corruption);
				if( CoC.player.cor < 50 ) {
					MainView.outputText( 'in helpless terror as the bulge inches up from the base of your groin towards your stomach. You let out a shuddering moan of pain as inch after inch of monstrous ' + Appearance.cockNoun( CockTypesEnum.HORSE ) + ' stretches your belly', false );
				}
				//(High corruption);
				else {
					MainView.outputText( 'panting in lust as the monstrous ' + Appearance.cockNoun( CockTypesEnum.HORSE ) + ' pushes your flesh aside to make room for itself', false );
				...
				MainView.outputText( ' in your stuffed ' + Descriptors.vaginaDescript( 0 ) + '. The big imp sneers and bounces his cock again, watching ', false );
				//(If the character has breasts);
				if( CoC.player.biggestTitSize() >= 3 ) {
					MainView.outputText( 'your ' + CoC.player.allBreastsDescript() + ' roll on your chest as you squirm', false );
				}//(If the character doesn't have breasts);
				...
				//(If the character has a cock);
				if( CoC.player.cockTotal() > 0 ) {
					MainView.outputText( 'Your ' + Descriptors.multiCockDescriptLight() + ' slaps against your distended belly as you shake.', false );
				}
				MainView.outputText( '\n\n', false );
				MainView.outputText( 'Finally the big imp pulls back his ' + Appearance.cockNoun( CockTypesEnum.HORSE ) + ', each ridge pulling on your pussy flesh as he slides out. An imp reaches out and slaps the bulge as it withdraws, making you yelp and buck.  ', false );
				//(If the character has a cock);
				if( CoC.player.cockTotal() > 0 ) {
					MainView.outputText( 'Your ' + Descriptors.multiCockDescriptLight() + ' bounces as the bulge passes under it.  ', false );
				}
				MainView.outputText( 'You moan as the mushroom-head reaches the entrance of your ' + Descriptors.vaginaDescript( 0 ) + ', your stretched pussy-flesh slowly returning to normal. The master imp pushes forward again, reclaiming your pussy for his monstrous cock. ', false );
				//(Low corruption);
				if( CoC.player.cor < 50 ) {
					MainView.outputText( 'You try to pull your hips back, fighting to break free as the bulge of his cock-head works its way high up into your belly. You\'re held down by too many imps. You can only writhe around the hot shaft stretching out your ' + Descriptors.vaginaDescript( 0 ) + '. Your head is held steady by two imps, you can\'t even look away as their master rapes you. The big imp grunts as his cock-head pops past your cervix, and you moan and shake in pain.', false );
				}
				//(High corruption);
				else {
					MainView.outputText( 'You moan in ecstasy as the hot ' + Appearance.cockNoun( CockTypesEnum.HORSE ) + ' pushes deep into your ' + Descriptors.vaginaDescript( 0 ) + ', turning every inch of your pussy into a pleasure-sheath for the big imp. You know you\'re nothing but a fuck-toy for this corrupt creature, just a wet pussy for him to fill with cum, and the thought almost makes you orgasm as he forces his huge cock-head past your cervix.', false );
				...
				//(Low corruption);
				if( CoC.player.cor < 50 ) {
					MainView.outputText( 'The pain as this huge cock stretches you is overwhelming, but every thrust rubs more corrupted pre-cum into your pussy walls. You start to pant as the imp rapes you, using your body for his own pleasure. Your nipples swell as the heat of his pre-cum soaks through your body. The huge shaft forces your ' + Descriptors.clitDescript() + ' out, and the steaming fluids splashing on it make it tingle almost painfully. Your whimpers and moans of pain start to take on a different tone, and the master imp starts to fuck you faster.', false );
				}
				//(High corruption);
				else {
					MainView.outputText( 'Pain and pleasure blend into one as the huge ' + Appearance.cockNoun( CockTypesEnum.HORSE ) + ' stretches you, rubbing pre-cum into steaming pussy. You moan as the big imp fucks you, turning you into a mindless fuck-puppet. Your ' + Descriptors.clitDescript() + ' swells painfully as hot juices splash over it. Your ' + Descriptors.nippleDescript( 0 ) + ' tingle almost painfully as the heat of his pre-cum spreads through your body.', false );
				}
				MainView.outputText( '\n\n', false );
				MainView.outputText( 'The other imps continue to jerk-off over you as the big imp impales you again and again on his shaft. Their pre-cum starts to splatter down on your body, and they pant as they watch you build towards your orgasm.  ', false );
				//(If the character has breasts);
				if( CoC.player.biggestTitSize() >= 3 ) {
					MainView.outputText( 'Your ' + CoC.player.allBreastsDescript() + ' bounce and jiggle back and forth as the master imp roughly fucks you.  ', false );
				}
				//(If the character has a dick);
				if( CoC.player.totalCocks() > 0 ) {
					MainView.outputText( 'Your ' + Descriptors.multiCockDescriptLight() + ' swell painfully as the rough fucking pumps blood into your groin.  ', false );
				}
				MainView.outputText( 'The big imp\'s snake tongue lashes out to incredible length and wraps around one of your ' + Descriptors.nippleDescript( 0 ) + 's, pulling at it and stretching the flesh under it. He moves his tongue back and forth between your nipples, alternating between stretching and flicking them. ', false );
				//(If the character has a dick);
				if( CoC.player.cockTotal() > 0 ) {
					MainView.outputText( 'He draws his tongue back and wraps it around your ' + Descriptors.cockDescript( 0 ) + ', sliding its length along your shaft and flicking his tongue over your cock-head.', false );
				}//(If the character doesn't have a dick);
				else {
					MainView.outputText( 'His tongue flicks down to your ' + Descriptors.clitDescript() + ', the split ends of it teasing your clit.', false );
				}
				MainView.outputText( '  You gasp in time to the big imp\'s thrusts, whimpering when his cock or tongue hit a sensitive point.  ', false );
				//(Low Corruption);
				if( CoC.player.cor < 50 ) {
					MainView.outputText( 'You\'re being raped by a demon, forced to take an inhuman cock, and you\'re about to cum hard. This corrupted land has left its mark on you.', false );
				}
				//(High corruption);
				else {
					MainView.outputText( 'This corrupted land has left its mark on you. You could never have taken a cock this big before you arrived here.', false );
				...
				MainView.outputText( '  You moan as you rise towards your orgasm.\n\n', false );
				MainView.outputText( 'The master imp pounds at you as hard as he can, driving his ' + CoC.monster.cockDescriptShort( 1 ) + ' deeper into you. His grunts come closer and closer together. Your head still held up, you watch as the imps around you start to cum. They spray your body with thick globs of cum, splattering it across your belly', false );
				//(If the character has breasts);
				if( CoC.player.biggestTitSize() >= 3 ) {
					MainView.outputText( ' and ' + CoC.player.allBreastsDescript(), false );
				}
				MainView.outputText( '. The master imp pounds into you and you can see his ' + CoC.monster.ballsDescriptLight() + ' swell. Through the haze of your approaching orgasm you realize what\'s about to happen. Those oversized balls are about to pump more cum into you than any normal man could ever produce. They\'re going to pump demonic cum right into your womb.  ', false );
				//(Low Corruption);
				if( CoC.player.cor < 50 ) {
				...
				MainView.outputText( '  You nearly black out as the orgasm blasts through you,  arching your back off the ground as the orgasm wracks your body, eyes rolling back in your head as your womb swells.\n\n', false );
				MainView.outputText( 'Imp-jism rains down on your helpless spasming body. The imps spew cum into your hair, across your swollen belly, over your face', false );
				//(If the character has a cock) ;
				if( CoC.player.cockTotal() > 0 ) {
					MainView.outputText( ', and cum-dripping ' + Descriptors.cockDescript( 0 ), false );
				}
				//(If the character has breasts);
				if( CoC.player.biggestTitSize() >= 3 ) {
					MainView.outputText( ', and bouncing ' + CoC.player.allBreastsDescript(), false );
				}
				MainView.outputText( '. The ' + CoC.monster.short + ' is no longer holding you down. They masturbate over you as you claw at the ground with your hands, toes curling as you clamp your thighs tight around the big imp. Another pulse of demonic cum hits your womb. You wrap your legs around your master, forcing as much of his cock into you as possible. Arching your back, your eyes roll back in your head and you shriek as your womb stretches painfully, a final orgasm crashing through your cum-bloated body. You spasm around the cock that impales you, thrashing against the ground as ', false );
				//(If the character has breasts);
				if( CoC.player.biggestTitSize() >= 3 && CoC.player.biggestLactation() > 1 ) {
					MainView.outputText( 'milk spurts from your ' + Descriptors.nippleDescript( 0 ) + ' and ', false );
				}
				MainView.outputText( 'steaming fluids spew from your over-filled pussy. Unconsciousness follows close on the heels of this last orgasm, your mind shutting down even as your body still shudders.\n\n', false );
				MainView.outputText( 'You wake up later, body still twitching as tiny orgasms spark in your ' + Descriptors.vaginaDescript( 0 ) + '. It\'s still dark out. You lie in a pool of cooling cum and pussy juice. Your body is covered in long ropes of drying imp-cum, and your hair is plastered to the ground. There\'s no sign of the horde of imps or their big master. Your belly is as tight and distended as a woman on the verge of giving birth. It quivers as the flesh of your ' + Descriptors.vaginaDescript( 0 ) + ' spasms. Over the swollen curve of your belly you can see steam rising from between your legs. You start to slip back into unconsciousness. ', false );
				//(Low corruption);
				if( CoC.player.cor < 50 ) {
					MainView.outputText( 'Your last coherent thought is to find a way to better hide your camp, so this never happens again.', false );
				}
				//(High corruption);
					...
					MainView.outputText( 'Your last coherent thought is to find a way to make your own mutated master imp, one you can keep as a fuck-toy...', false );
				}
				CoC.player.orgasm();
				EngineCore.dynStats( 'lib', 2, 'cor', 3 );
				CoC.player.knockUp( PregnancyStore.PREGNANCY_IMP, PregnancyStore.INCUBATION_IMP - 14 ); //Bigger imp means faster pregnancy
				...
				MainView.outputText( 'The ' + CoC.monster.getCapitalA() + ' swarms over you, dragging you to the ground as ', false );
				//(Low Corruption);
				if( CoC.player.cor < 50 ) {
					MainView.outputText( 'you punch and kick wildly, determined not to let them take you', false );
				}
				//(High Corruption);
				else {
					MainView.outputText( 'you twist and struggle in their grips, determined to make them work for their fun', false );
				}
				MainView.outputText( '! They pull you down over a fallen log, ass resting above your head. Two imps sit on your arms, their gonads rubbing against your biceps, and rub their hands over your shoulders and chest. Others stretch your ' );
				if( CoC.player.isNaga() ) {
					MainView.outputText( 'coils out, twisting them around a log to hold you still.\n\n' );
				} else {
					MainView.outputText( CoC.player.legs() + ' wide apart, holding them against the log.\n\n', false );
				}

				MainView.outputText( 'The ' + CoC.monster.short + ' makes short work of your ' + CoC.player.armorName + ', unbuckling straps and stripping you quickly. ', false );
				//(If the player has breasts);
				if( CoC.player.biggestTitSize() > 0 ) {
					MainView.outputText( 'An imp mounts your chest and slaps his ' + CoC.monster.cockDescriptShort( 0 ) + ' between your ' + CoC.player.allBreastsDescript() + ', squeezing them tight over his cock as he rubs back and forth.  ', false );
				}
				//(If the player has a SINGLE cock);
				if( CoC.player.cockTotal() === 1 ) {
					MainView.outputText( 'Your ' + Descriptors.cockDescript( 0 ) + ' is seized by an imp, who licks the tip with his inhumanly nimble tongue while he strokes the shaft.  ', false );
				}
				//(If the player has a MULTI cock);
				if( CoC.player.cockTotal() > 1 ) {
					MainView.outputText( 'Two imps seize your ' + Descriptors.multiCockDescriptLight() + ', licking the tips with their inhumanly nimble tongues while they stroke the shafts.  ', false );
				}
				//(Low Corruption);
				if( CoC.player.cor < 50 ) {
					MainView.outputText( 'You fight to free your arms and shake the imp off your chest while tiny hands slide over your face. They tug at your lips and try to pry your jaws open', false );
				}
				//High Corruption);
				else {
					MainView.outputText( 'You writhe in the grasp of the imps, reveling in the sensations of being spread open and completely at the mercy of these demons. Tiny hands slide over your face and you lick and suck at the fingers', false );
				}
				MainView.outputText( '.\n\n', false );
				//(If the player has breasts);
				if( CoC.player.biggestTitSize() > 0 ) {
					MainView.outputText( 'Hands slide over your ' + CoC.player.allBreastsDescript() + ', pinching and pulling at your nipples. The imp riding your ' + Appearance.biggestBreastSizeDescript() + ' licks your tit-flesh, slowly working his tongue up towards your ' + Descriptors.nippleDescript( 0 ) + '. Finally the imp\'s tongue reaches your nipple, wrapping around and pulling at the tingling flesh. ', false );
					//(Low Corruption);
					if( CoC.player.cor < 50 ) {
						MainView.outputText( 'You can\'t escape the tongue lapping and pulling at your ' + Descriptors.nippleDescript( 0 ) + '. You shake your head to deny the pleasure, but your breathing comes faster and faster as lust invades your body.', false );
					}
					//(High Corruption);
					else {
						MainView.outputText( 'The tongue squeezing and tugging your nipple floods your body with lust. You moan and arch your back, offering your tits to the imp riding your chest. You can hear your pulse pounding in your ears as you pant in desire.', false );
					}
					MainView.outputText( '  Suddenly you feel tiny needle-sharp teeth pierce your nipple. You scream as venom pumps into your tits, red-hot poison that makes your ' + CoC.player.allBreastsDescript() + ' feel as though they were being stung by bees. You moan in pain as your breasts start to swell, the imp rider biting into your other nipple to pump demon-taint into it.', false );
					if( CoC.player.hasFuckableNipples() ) {
						MainView.outputText( 'With the imp\'s taint seeping into your ' + Descriptors.nippleDescript( 0 ) + ', each one\'s cunt-like shape begins swelling. The fuckable orifices engorge into larger and fatter looking labia, becoming fuller cunts each with an engorged clitoral nub the size of a golf ball. Their color deepens as the skin of your nipple-cunts becomes tighter and smoother.  The imp giggles and continues nibbling the newly swollen sensitive flesh, injecting further doses of venom.', false );
					}
					MainView.outputText( '\n\n', false );
					//Grow tits!;
					CoC.player.growTits( 2, CoC.player.breastRows.length, false, 1 );
				...
				MainView.outputText( 'The master of this ' + CoC.monster.short + ' steps up ' );
				if( CoC.player.isNaga() ) {
					MainView.outputText( 'alongside your taut tail' );
				} else {
					MainView.outputText( 'between your ' + CoC.player.legs() );
				}
				MainView.outputText( ', leering down at your trapped body. Four feet tall and broader and stronger than any imp in the pack, with a face as much dog as imp, this new imp has grey fur, broad black demon wings, two long demon-horns on his head, and a ' + Appearance.cockNoun( CockTypesEnum.DOG ) + ' big enough to choke a minotaur. Pre-cum drips from the broad tip of it, dripping down onto your ', false );
				//(If the player has a cock);
				if( CoC.player.cockTotal() > 0 ) {
				...
				MainView.outputText( '.  ', false );
				MainView.outputText( 'The heat stings your flesh. The imps licking your groin lap up the hot fluid, cooling you with their saliva. The big imp sneers as you whimper, and drags the head of his ' + Appearance.cockNoun( CockTypesEnum.DOG ) + ' down to your ', false );
				//(If the player has a vagina);
				if( CoC.player.hasVagina() ) {
					MainView.outputText( Descriptors.vaginaDescript( 0 ), false );
				}
				...
				MainView.outputText( '.  He thrusts brutally, shoving the head of his ' + CoC.monster.cockDescriptShort( 2 ) + ' into your ', false );
				//(If the player has a vagina);
				if( CoC.player.hasVagina() ) {
					MainView.outputText( Descriptors.vaginaDescript( 0 ), false );
				}
				...
				//(Low Corruption);
				if( CoC.player.cor < 50 ) {
					MainView.outputText( 'You screech in agony as the big imp forces his mutated wolf-cock into your hole, brutally shoving thick inch after inch of painfully hot ' + Appearance.cockNoun( CockTypesEnum.DOG ) + ' deeper into you than anything should ever go.  ', false );
				}
				//(High Corruption);
				else {
					MainView.outputText( 'The master imp\'s painfully hot ' + Appearance.cockNoun( CockTypesEnum.DOG ) + ' stretches your hole wider than it ever should be, and you moan in perverse ecstasy.  ', false );
				}
				MainView.outputText( 'His huge dick-knot bumps against the entrance of your ', false );
				//(If the player has a vagina);
				if( CoC.player.hasVagina() ) {
					MainView.outputText( Descriptors.vaginaDescript( 0 ), false );
				}
				...
				//(If the character has breasts);
				if( CoC.player.biggestTitSize() > 0 ) {
					MainView.outputText( 'The big imp reaches past your tit-rider and grabs one of your painfully distended breasts in each hand, mauling and bouncing the flesh as if weighing them. You gasp in pain as your ' + CoC.player.allBreastsDescript() + ' swell further at his touch.  ', false );
				}
				MainView.outputText( 'Your mouth gapes open and an imp takes the chance to stuff it full of cock.  ', false );
				MainView.outputText( 'The master imp grabs your hips and starts to fuck you hard, pistoning his steaming cock in and out of your ', false );
				//(If the player has a vagina);
				if( CoC.player.hasVagina() ) {
					MainView.outputText( Descriptors.vaginaDescript( 0 ), false );
				}
				...
				MainView.outputText( ', not caring that he\'s stretching you beyond normal human endurance. ', false );
				//(Low Corruption);
				if( CoC.player.cor < 50 ) {
					MainView.outputText( 'You\'re being raped again by demons, impaled on cocks like a roast pig on a spit, and you can feel your lust rising.  This corrupted land has left its mark on you.', false );
				}
				...
				//(If the character has breasts);
				if( CoC.player.biggestTitSize() > 0 ) {
					MainView.outputText( 'The imp riding your ' + Appearance.biggestBreastSizeDescript() + ' finally cums, painting your distended fuck-udders with his massive load.  ', false );
				}
				MainView.outputText( 'Your master isn\'t done with you yet, churning his ' + Appearance.cockNoun( CockTypesEnum.DOG ) + ' knot in your ', false );
				//(If the player has a vagina);
				if( CoC.player.hasVagina() ) {
				...
				MainView.outputText( '. Your toes curl as you feel more pulses of demon-cum work their way up his shaft and into your already-huge belly.\n\n', false );
				MainView.outputText( 'You pass out as another load of imp-cum pours down your throat, another tidal wave of corrupted jism spews into your hole, to meet somewhere in the middle...\n\n', false );
				MainView.outputText( 'You wake up later, still trembling with small orgasms. Cum burbles in your mouth as you breathe. You haven\'t moved since you passed out. Your hips are still propped up over the log, and you rest in a cooling pool of cum, your ' + Descriptors.hairDescript() + ' plastered to the ground with drying jism. You couldn\'t move even if your ' + CoC.player.legs() + ' felt stronger. Your hideously bloated belly weighs you down, ', false );
				//(If the player has breasts);
				if( CoC.player.biggestTitSize() > 0 ) {
					MainView.outputText( 'and your milk-filled udders are still swollen with imp-venom, ', false );
				}
				MainView.outputText( 'quivering with every orgasmic twitch that passes through you. The skin of your distended belly ', false );
				//(If the player has breasts);
				if( CoC.player.biggestTitSize() > 3 ) {
					MainView.outputText( 'and massive tits ', false );
				}
				MainView.outputText( 'is drum-tight and shiny, and your belly-button has popped out into an outie. As you slip back into unconsciousness, one last thought flits across your mind. ', false );
				//(Low Corruption);
				if( CoC.player.cor < 50 ) {
					MainView.outputText( 'How long can you last in this corrupted land, when your body can be so horribly twisted for the sick pleasures of its denizens?\n\n', false );
				}
				//(High corruption);
				else {
					MainView.outputText( 'Why bother with your silly quest, when you\'ve only scratched the surface of the pleasures this land offers you?\n\n', false );
	...
	ImpScene.prototype.impRapesYou = function() {
		...
		if( CoC.player.lust >= 100 ) {
			...
			if( CoC.player.gender === 2 || CoC.player.gender === 3 ) {
				...
				if( CoC.player.vaginalCapacity() < CoC.monster.cockArea( 0 ) ) {
					if( CoC.player.vaginas[ 0 ].virgin ) {
						MainView.outputText( '  He plunges in hard, breaking your hymen and stealing your virginity.  A look of surprise crosses his face, chased away by ecstasy.  If you had a rational bit left in your mind, you\'d notice he looks... stronger somehow, but you\'re too horny to care.', false );
						CoC.player.vaginas[ 0 ].virgin = false;
					} else {
						MainView.outputText( '  He pushes against your tight little pussy, struggling to penetrate you.', false );
					}
					MainView.outputText( '  His cock only sinks a few inches in, but he begins fucking you hard, each time claiming a bit more of your pussy for his demonic tool.  You feel a painful stretching as he gets half of it inside you, ruining your ' + Descriptors.vaginaDescript( 0 ) + ' for most humans.  He fucks you like this for what seems like forever, never getting much further. ', false );
					CoC.player.cuntChange( CoC.monster.cockArea( 0 ), true );
				} else {
					MainView.outputText( '  He plunges in violently, ramming his ' + CoC.monster.cockDescriptShort( 0 ) + ' in to the hilt, leaving you gasping in pain and surprise.  He leaves it there, giving you a second to get used to him, and then begins fucking you hard, slapping your ass every few thrusts to remind you who is in charge.', false );
					CoC.player.cuntChange( 12, true, true, false );
				}
				...
				MainView.outputText( '  The imp\'s ' + CoC.monster.cockDescriptShort( 0 ) + ' explodes inside you, pumping huge loads of hot demon-seed inside you with each eruption.  You swoon, feeling it fill your womb and distend your belly as the imp\'s orgasm fills you with an unnatural quantity of corrupted semen.\n\nWith a sigh, he pulls his dick free, and you flop back on your back, cum surging out onto the ground from your well-fucked hole.  ', false );
				if( CoC.player.pregnancyIncubation > 0 && CoC.player.pregnancyIncubation <= 216 ) {
					MainView.outputText( 'You wonder what this will do to whatever is growing in your womb...  ', false );
				} else {
					if( CoC.player.inHeat ) {
						MainView.outputText( 'You find yourself hoping you\'re pregnant as you swiftly lose consciousness.', false );
					} else if( CoC.player.pregnancyIncubation <= 0 ) {
						if( CoC.player.cor > 75 ) {
							MainView.outputText( 'With an appreciative moan, you bury your fingers in its slimy warmth, hoping you are pregnant with some fiendish offspring, and lose consciousness.', false );
						} else {
							MainView.outputText( 'You hope you don\'t become pregnant, but promptly lose consciousness before you can contemplate the prospect any further.', false );
						}
					}
				}
				CoC.player.knockUp( PregnancyStore.PREGNANCY_IMP, PregnancyStore.INCUBATION_IMP - 14 ); //Bigger imp means faster pregnancy
			...
			if( CoC.player.gender === 0 || CoC.player.gender === 1 ) {
				...
				if( CoC.player.gender === 1 && Utils.rand( 2 ) === 0 ) {
					MainView.outputText( ImageManager.showImage( 'imp-loss-male-fuck' ), false );
					MainView.outputText( 'Your eyes glaze over with lust as the imp\'s dark magic destroys your will to continue fighting. You sink to your ', true );
					if( CoC.player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_CENTAUR ) {
						MainView.outputText( 'hocks and knees, your ' + Descriptors.cockDescript( 0 ) + ' hurting from the massive blood pressure caused by your unbridled lust. He approaches you and stops about two feet in front of you, watching with delight your helpless state', false );
					} else {
						MainView.outputText( 'knees, pull out your ' + Descriptors.cockDescript( 0 ) + ' and begin mindlessly stroking yourself as the imp approaches you, a wicked grin on his face. Your mind races with thoughts and images of sucking the imp\'s cock. He approaches you and stops about two feet in front of you, watching with delight as you succumb to your own lust', false );
					}
					MainView.outputText( '. Your eyes glance down to his waist and see a massive bulge form under his loincloth, the sight of which causes your ' + Descriptors.cockDescript( 0 ) + ' to twitch and begin leaking pre-cum.\n\n', false );
					MainView.outputText( 'The imp drops his loincloth, revealing his huge 12-inch penis, and then forcefully grabs your head and pulls you down on to his hard throbbing demon dick. He shoves his cock past your lips and deep down your throat in one slow, forceful push. You can barely accommodate his huge cock, and yet your lust makes you hunger for more. You cough and gag while the imp proceeds to fuck your mouth hard, slapping his hot balls against your chin, disregarding your need to breathe.  ', false );
					if( CoC.player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_CENTAUR ) {
						MainView.outputText( 'Dropping down to the ground, your ' + Descriptors.cockDescript( 0 ) + ' trembles against your body to the rhythm of the imp\'s thrusts, leaving your underbelly smeared with its own pre-cum.\n\n', false );
					} else {
						MainView.outputText( 'On all fours now, your ' + Descriptors.cockDescript( 0 ) + ' bounces up and down against you to the rhythm of the imp\'s thrusts, leaving your belly smeared in your own pre-cum.\n\n', false );
					}
					if( CoC.player.ballSize >= 5 ) {
						MainView.outputText( 'Your huge ' + Descriptors.ballsDescriptLight() + ' swing heavily against you as well, responding to the force of the imp\'s thrusts, slapping your own ass and driving your ' + Descriptors.cockDescript( 0 ) + ' even stiffer with lust, the pre-cum pulsing out of your cock in time with the slapping.\n\n', false );
					}
					...
					CoC.player.slimeFeed();
					MainView.outputText( 'You sink to the ground, too overcome by lust and desire to fight.  The imp smiles and circles you, dropping his loincloth as he goes.  You are roughly shoved to the ground, your backside slapped hard.  You\'re too horny to do anything but moan from the pain ', false );
					if( CoC.player.lowerBody !== AppearanceDefs.LOWER_BODY_TYPE_CENTAUR ) {
						MainView.outputText( 'as you are disrobed', false );
					}
					MainView.outputText( '.  As the imp presses a large bulk against your backside, you realize he has a massive penis!\n\nThe imp pushes his ' + CoC.monster.cockDescriptShort( 0 ) + ' into your ass and fucks you hard, with little regard to your pleasure.  After a rough fucking, he cums, stuffing your ass full of hot demon cum.  His orgasm lasts far longer than any human\'s, leaving your belly slightly distended.', false );
					CoC.player.buttChange( CoC.monster.cockArea( 0 ), true, true, false );
					EngineCore.dynStats( 'lib', 1, 'sen', 1, 'lus', 1, 'cor', 1 );
					if( CoC.player.sens > 40 ) {
						MainView.outputText( '  You manage to orgasm from the feeling of being filled by hot cum.', false );
						if( CoC.player.gender === 1 ) {
							MainView.outputText( '  You jizz all over the ground in front of you, spraying cum in huge squirts in time with the demon\'s thrusts.', false );
						}
	...
	ImpScene.prototype.noogaisNippleRape = function() {
		MainView.outputText( '', true );
		MainView.outputText( 'You slowly walk over to the masturbating imp, your ' + Descriptors.hipDescript() + ' and ' + Descriptors.buttDescript() + ' swaying suggestively with every step.\n\n', false );
		MainView.outputText( 'Shedding your clothes you push the imp to the ground and straddle him, keeping his hands away from his twitching pecker while you quickly tie him up with his own loincloth.  The lust-addled demon utterly incapacitated, you start to use both of your hands to toy freely with your slimy nipple-holes, as well as your ', false );
		if( CoC.player.hasCock() ) {
			MainView.outputText( Descriptors.cockDescript( 0 ), false );
		}
		...
		MainView.outputText( '.\n\n', false );
		MainView.outputText( 'You gently insert a single digit into one of your nipple-cunts, ', false );
		if( CoC.player.lactationQ() >= 1000 ) {
			MainView.outputText( 'unleashing a torrent of thick, creamy milk and ', false );
		}//(if regular milky; ;
		else if( CoC.player.lactationQ() >= 50 && CoC.player.biggestLactation() >= 1 ) {
			MainView.outputText( 'releasing a steady trickle of warm milk and ', false );
		}
		MainView.outputText( 'lust-induced sex juice onto the imp\'s lap; your other hand instinctively moves down to stroke your ', false );
		//((if male/herm; ;
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'rock-hard cock', false );
			if( CoC.player.hasVagina() ) {
				MainView.outputText( ' and ', false );
			}
		}
		if( CoC.player.hasVagina() ) {
			MainView.outputText( 'dripping wet pussy', false );
		}
		if( CoC.player.gender === 0 ) {
			MainView.outputText( Descriptors.assholeDescript(), false );
		}
		MainView.outputText( ', teasing him with a lewd moan as your head rolls back in sexual ecstasy.', false );
		if( EngineCore.silly() ) {
			MainView.outputText( '  The imp is sickened, but curious.', false );
		}
		MainView.outputText( '\n\n', false );
		MainView.outputText( 'You continue finger-fucking your nipple, becoming more and more aroused as the imp gets harder and harder from watching the exotic display before him.  You soon tire of watching the imp squirm beneath you, desperate for sexual relief; you slowly move your hand away from your groin, reaching down towards his crotch, and start to toy with his apple-sized balls, fondling and squeezing them roughly.  You casually slip a second finger into your wet nipple-hole, stretch it out teasingly, and hold the gaping orifice in front of the imp\'s face, giving him a good view of the inside of your freakish, wet nipple-cunt.\n\n', false );
		//(If corrupt: ;
		if( CoC.player.cor >= 66 ) {
			MainView.outputText( '"<i>Mmm, wouldn\'t you just love to stick your fat cock into this sopping wet hole, and cum deep inside my ' + Descriptors.chestDesc() + '?</i>"  You whisper huskily into his ear, sliding your fingers away from his balls and up along the underside of his aching dick, teasing every inch of it until you reach his swollen head and start rubbing your finger around his glans in small circles.  The imp is panting heavily, his eyes firmly locked on your ', false );
			//(if normal);
			if( CoC.player.biggestLactation() < 1 ) {
				MainView.outputText( 'wet', false );
			}//(if lactating);
			else {
				MainView.outputText( 'milky', false );
			}
			MainView.outputText( ', bucking his hips upwards in desperation.\n\n', false );
		}
		MainView.outputText( 'Deciding that the poor bastard has suffered enough, you guide your stretched ' + Descriptors.nippleDescript( 0 ) + ' down to his quivering member and hold it over the tip for a moment.  The imp groans in frustration, feeling the heat of your slutty juices dripping down onto his aching rod and overfull testes, making him even more desperate to drive deep into your waiting breast.  Without warning, you forcefully shove your breast onto his swollen fuckstick, ', false );
		if( CoC.player.biggestTitSize() <= 4 ) {
			MainView.outputText( 'bottoming out halfway on his immense dick.', false );
		} else {
			MainView.outputText( 'only stopping when the flesh of your immense mammary bumps into his quaking ballsack.', false );
		}
		MainView.outputText( '\n\n', false );
		MainView.outputText( 'You shudder in ecstasy as you rise off of his drenched girth; your nipple-hole is slick with arousal, making it easier for you to slide back down until ', false );
		//((if breast size below D) ;
		if( CoC.player.biggestTitSize() <= 4 ) {
			MainView.outputText( 'you feel his swollen cock bottom out, your petite breast unable to swallow any more of his throbbing maleness', false );
		}//((over D) ;
		else {
			MainView.outputText( 'his swollen cock and desperately filled balls are entirely engulfed in tit-flesh', false );
		}
		MainView.outputText( '.  Eventually the imp starts timing his thrusts with your movements, and soon the two of you are working in a steady rhythm - thrust, retract, thrust, retract.  Minutes go by as the rhythm slowly builds towards a crescendo, with the only sounds being the lewd schlicking noise of your breast servicing the imp\'s rod, and the odd moan escaping your lips.  While one hand is furiously jilling off your vacant nipple-slit, the other one is furiously', false );
		//[(if male);
		if( CoC.player.hasCock() ) {
			MainView.outputText( ' pumping your ' + Descriptors.cockDescript( 0 ), false );
		}//(if female);
		else if( CoC.player.hasVagina() ) {
			MainView.outputText( ' fingering your hungry baby tunnel', false );
		} else {
			MainView.outputText( ' fingering your tingling anus', false );
		}
		MainView.outputText( '.\n\n', false );
		MainView.outputText( 'Eventually the rhythm becomes more sporadic as you and the imp approach climax; your tongue rolls out of your open mouth and your toes curl as you feel the imp spasm violently inside you, letting an endless stream of his searing spunk pour directly into your ' + Descriptors.chestDesc() + '.  The intense heat pushes you over the edge and ', false );
		//(if dick);
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'a ', false );
			//[(cum production < 500ml);
			if( CoC.player.cumQ() < 500 ) {
				MainView.outputText( 'jet ', false );
			}//(cum production 500-1000ml);
			else if( CoC.player.cumQ() < 1000 ) {
				MainView.outputText( 'geyser ', false );
			}//(cum production > 1000ml);
			else {
				MainView.outputText( 'volcano ', false );
			}
			MainView.outputText( 'of cum sprays from your ' + Descriptors.cockDescript( 0 ) + ' and splatters over both you and the hapless imp', false );
			if( CoC.player.hasVagina() ) {
				MainView.outputText( ', while ', false );
			}
		}
		if( CoC.player.hasVagina() ) {
			MainView.outputText( 'your pussy juices spurt out as your ' + Descriptors.vaginaDescript( 0 ) + ' twitches in orgasm', false );
		}
		if( CoC.player.gender === 0 ) {
			MainView.outputText( 'your asshole clenches tight on your finger', false );
		}
		MainView.outputText( '.\n\n', false );
		MainView.outputText( 'You collapse heavily on top of the imp, once again impaling your breast on his still-erect cock.  You lie like this for a few moments until you notice that the imp has dozed off, exhausted by the whole ordeal.  You stand up woozily as a mixture of ', false );
		//(if lactating) ;
		if( CoC.player.biggestLactation() >= 1 && CoC.player.lactationQ() < 40 ) {
			MainView.outputText( 'milk, ', false );
		}
		MainView.outputText( 'fem-spunk and hot demon cum leaks out from your gaping nipple-cunt.\n\n', false );
		//(if corruption > 60);
		if( CoC.player.cor > 60 ) {
			MainView.outputText( 'You thrust your digits into your ' + Descriptors.nippleDescript( 0 ) + ' once more, scooping out as much imp jizz as you can reach.  You happily drink up the thick goo, savoring the cloying taste before quickly getting dressed and leaving the imp to slumber.', false );
		}
		//(continue to non-corrupt text);
		//(if not);
		else {
			MainView.outputText( 'You quickly get dressed and leave the imp to his slumbering, his hands still tied together by his loincloth.', false );
		}
	...
	ImpScene.prototype.sexAnImpLord = function() {
		MainView.clearOutput();
		MainView.outputText( 'You grin evilly and walk towards the defeated corrupted creature.  He doesn\'t take notice of you even though you\'re only inches away from him.  You remove your [armor] slowly, enjoying the show the imp is giving you.  But soon it\'s time for you to have fun too.' );
		//(No line break);
		//if(CoC.player doesn't have centaur legs);
		if( !CoC.player.isTaur() ) {
			MainView.outputText( '  You grab his hands, removing them from his ' + CoC.monster.cockDescriptShort( 0 ) + '. This gets his attention immediately, and you grin widely, pinning him to the ground.' );
		} else {
	...
	ImpScene.prototype.impLordBumPlug = function() {
		MainView.clearOutput();
		MainView.outputText( ImageManager.showImage( 'implord-win-male-fuck' ), false );
		var x = CoC.player.cockThatFits( CoC.monster.analCapacity() );
		if( x < 0 ) {
			x = CoC.player.smallestCockIndex();
		}
		MainView.outputText( 'You grab the muscular creature by one of his long pointed ears, pulling him to his feet. He protests slightly, and gives a slightly defiant snarl of discomfort.  Lucky for him you\'re in a forgiving mood and ignore his whining.' );
		//(No line break);
		if( CoC.player.tallness < 72 ) {
			MainView.outputText( '  You give a powerful shove and push the imp to his knees.' );
		}
		MainView.outputText( '  You pull the imp\'s head towards your [hips], forcing his lips against the base of your ' + Descriptors.cockDescript( x ) + '.  The imp quickly gets the idea and begins to lick and suckle at your ' + CoC.player.cockHead( x ) + ' expertly.' );
		...
		//(No line break);
		if( CoC.player.balls > 0 ) {
			MainView.outputText( '  You feel the slapping of your [balls] against the imp\'s, a sensation that only spurs you on, causing you to slam into him over and over wildly.' );
		}
		if( CoC.player.biggestTitSize() >= 1 && CoC.player.lactationQ() >= 200 ) {
			MainView.outputText( '  You give a desperate groan, and grab one of your [chest] roughly.  You pinch and massage your ' );
			if( CoC.player.averageNipplesPerBreast() > 1 ) {
				MainView.outputText( '[nipples]' );
			} else {
				MainView.outputText( '[nipple]' );
			}
			MainView.outputText( ', making you moan in ecstasy and leak milk, quickly soaking your chest.' );
		}
	...
	ImpScene.prototype.feederBreastfeedRape = function() {
		...
		if( CoC.player.tallness > 48 && CoC.player.tallness < 60 && CoC.player.lowerBody !== AppearanceDefs.LOWER_BODY_TYPE_CENTAUR ) {
			...
			if( CoC.player.hasCock() ) {
				MainView.outputText( '  [EachCock] twitches violently from the stimulation of your [asshole].  Pre-cum begins to dribble out of your ' + Descriptors.cockDescript( 0 ) + '.  You can\'t help but pant desperately as the warm pleasurable sensation of arousal fills your whole being.' );
			}
			if( CoC.player.hasVagina() ) {
				MainView.outputText( '  The intense stimulation of your [chest] is causing your [vagina] to become wet with girl juice... so much so that your femcum has started to seep down your taint towards your ass.  It\'s probably a good thing, as it\'s now become a lubricant for the greater imp\'s ' + CoC.monster.cockDescriptShort( 0 ) + '.' );
			}
	...
	ImpScene.prototype.loseToImpLord = function() {
		MainView.clearOutput();
		MainView.outputText( ImageManager.showImage( 'implord-loss-male' ), false );
		MainView.outputText( 'Unable to control your lust you fall to the ground, remove your ' + CoC.player.armorName + ' and begin masturbating furiously.  The powerful imp saunters over to you smirking evilly as he towers over your fallen form. You look up at him nervously.  He grabs your chin with one of his clawed hands, while the other digs through his satchel.  He pulls out a vial filled with glowing green liquid, and pops the cork stopper off with his thumb. Before you can react, the demon forces open your mouth and pours the liquid in.  Instinct reacts faster than logic and you swallow the substance as it\'s poured down your throat.' );
		...
		if( CoC.player.hasCock() ) {
			...
			if( CoC.player.balls > 0 ) {
				MainView.outputText( 'Your [balls] clench up against your body, desperate to finally expel their contents.  ' );
			}
			MainView.outputText( 'Your seed spills across the ground, mixing with the copious amount of demon pre that had sloshed to the ground earlier.  You howl loudly in pleasure, as you\'re finally given release.' );
		}
	...
	ImpScene.prototype.getRapedAsAGirl = function() {
		...
		if( CoC.player.isBiped() ) {
			...
			MainView.outputText( ' before you' );
		}
		MainView.outputText( ', draping his ' + CoC.monster.cockDescriptShort( 0 ) + ' across your wet crotch.  You groan, and unintentionally thrust against the magnificent tool between your legs.  The imp chuckles evilly as you coat his ' + CoC.monster.cockDescriptShort( 0 ) + ' in your girl juice, but he doesn\'t wait long before he slowly presses his head down against your [vagina].  His head slowly spreads your lips; the pleasure is unmistakable, and forces a loud moan from your lips.' );
	...
	ImpScene.prototype.putBeeEggsInAnImpYouMonster = function() {
		...
		MainView.outputText( ImageManager.showImage( 'imp-egg' ), false );
		MainView.outputText( 'You glance down at the masturbating imp, feeling a twitch in your swollen, insectile abdomen.  As the red-skinned homunculus writhes on the ground, beating his meat, you smile, feeling a globule of sweet nectar oozing out of your ovipositor.' );
		...
		if( CoC.player.isGoo() ) {
			MainView.outputText( 'rippled goo edges' );
		} else if( CoC.player.isNaga() ) {
			MainView.outputText( 'trembling coils' );
		} else {
			MainView.outputText( 'straddling knees' );
		}
		MainView.outputText( ' as you turn the imp into your own, private incubator.' );
		...
		if( CoC.player.biggestTitSize() >= 1 ) {
			MainView.outputText( 'his tiny claws grab your ' );
			if( CoC.player.bRows() > 1 ) {
				MainView.outputText( 'first row of ' );
			}
			MainView.outputText( Descriptors.breastDescript( 0 ) + ', squeezing your tits as you fuck him full.' );
			if( CoC.player.lactationQ() >= 500 ) {
				MainView.outputText( '  Rivulets of your milk run down his forearms as he inexpertly milks you.' );
			}
		}
		//[If cock: ;
		else if( CoC.player.hasCock() ) {
			MainView.outputText( 'the rise of his swollen belly soon presses against [oneCock] and the rhythm of your thrusts strokes his shiny red stomach against your sensitive organ.' );
		} else if( CoC.player.hasVagina() ) {
			MainView.outputText( 'the imp’s tiny, clawed feet scrabble against you as he flails in pleasure.  By mistake, one slips between the lips of your pussy, small toes wriggling against your inner walls, and you instinctively push down against the small limb, fucking yourself with his foot.' );
		} else {
			MainView.outputText( 'you feel a firm pressure at your [asshole] as the tip of the imp’s lashing tail prods frantically against you, manically shoving in and out of your [asshole].' );
		}
