...
angular.module( 'cocjs' ).run( function( $log, ImageManager, PerkLib, CockTypesEnum, Appearance, Combat, Descriptors, StatusAffects, AppearanceDefs, Utils, CoC, EngineCore ) {
	...
	GoblinScene.prototype.goblinRapesPlayer = function() {
		...
		if( CoC.getInstance().player.cockTotal() > 0 ) {
			...
			if( CoC.getInstance().player.cocks[ 0 ].cockLength <= 7 ) {
				EngineCore.outputText( 'as your bulge begins to press between her soft ass-cheeks', false );
			} else if( CoC.getInstance().player.cocks[ 0 ].cockLength <= 14 ) {
		...
		if( CoC.getInstance().player.hasVagina() ) {
			...
			if( CoC.getInstance().player.vaginas[ 0 ].vaginalWetness <= AppearanceDefs.VAGINA_WETNESS_NORMAL ) {
				...
				EngineCore.outputText( 'Feminine lubricant soaks into the back of your ' + CoC.getInstance().player.armorName + '.  ', false );
			} else {
		...
		//[DICK VERSION];
		if( CoC.getInstance().player.totalCocks() > 0 && (!CoC.getInstance().player.hasVagina() || Utils.rand( 2 ) === 0) ) {
			//[TOO BIG];
			if( CoC.getInstance().player.cockArea( 0 ) > CoC.getInstance().monster.vaginalCapacity() ) {
				...
				EngineCore.outputText( ' before she buries it deep into your ' + CoC.getInstance().player.assholeOrPussy() + '.  Her thighs, breasts, and body surround you with a shroud of warm green flesh that wriggles and twists around you with feverish lust.  The lips of her still-partially stretched cunt do their best to devour your member\'s head when the flexible little minx curves her hips back down to grind on it.\n\n', false );
		...
		//[FEMALEZ];
			...
			CoC.getInstance().player.cuntChange( CoC.getInstance().player.vaginalCapacity(), true );
			CoC.getInstance().player.orgasm();
			Combat.cleanupAfterCombat();
		}
	};
