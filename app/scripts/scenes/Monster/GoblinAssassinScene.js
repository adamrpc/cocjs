...
angular.module( 'cocjs' ).run( function( SceneLib, $log, MainView, ImageManager, CockTypesEnum, Appearance, Combat, GoblinAssassin, Descriptors, StatusAffects, AppearanceDefs, Utils, kFLAGS, CoC, EngineCore ) {
	...
	GoblinAssassinScene.prototype.goblinAssassinEncounter = function() {
		...
		if( CoC.flags[ kFLAGS.TIMES_ENCOUNTERED_GOBLIN_ASSASSIN ] === 0 ) {
			MainView.outputText( 'A needle whizzes through the air and lands next to your foot. The fine craftsmanship along the side of the glass tube only brings you to one conclusion - it\'s owner has access to precision machinery and manufacturing capabilities uncommon in this land.' );
		...
		//Repeat Intro - Cell Chambers;
			...
			MainView.outputText( 'A familiar sight catches your attention as a shadowy blur lands in front of you - another goblin assassin appears to be lurking around these parts. You ready your [weapon] as she straps on a belt loaded with various types of needles and assumes a combat stance.' );
			//[Initiate combat encounter – goblin assassin];
		}
	...
	GoblinAssassinScene.prototype.gobboAssassinBeatYaUp = function() {
		...
		if( CoC.player.cockThatFits( CoC.monster.vaginalCapacity() ) >= 0 && CoC.player.hasCock() && (!CoC.player.hasVagina() || Utils.rand( 2 ) === 0) ) {
			...
			if( CoC.player.cockTotal() === 1 ) {
				MainView.outputText( 'that thing' );
			} else {
				MainView.outputText( 'those things' );
			}
			MainView.outputText( ' look mighty tasty.  You don’t mind if I sample just a little bit of your seed do you?</i>"  The sapphire-maned beauty kneels down next to you and lets her tiny, delicate digits dance across the underside of [oneCock], drawing a lurid moan from your lips as she wonders out loud, “I don’t hear a no, ' + CoC.player.mf( 'stud', 'sexy' ) + '.  That’s good... I’m just going to take a little bit of cum... just a few squirts.  Surely a ' );
			if( CoC.player.tallness >= 60 ) {
				MainView.outputText( 'big, ' );
			}
			MainView.outputText( CoC.player.mf( 'strapping young man', 'sexy breeder' ) + ' will have enough jizz to pay for ' + CoC.player.mf( 'his', 'her' ) + ' trespass?</i>"' );
			MainView.outputText( '  Those delightful, dancing fingers pirouette down to your ' );
			if( CoC.player.balls > 0 ) {
				MainView.outputText( '[sack], giving it a gentle caress before vanishing.' );
			} else if( CoC.player.hasVagina() ) {
				MainView.outputText( Descriptors.vaginaDescript() + ', the tips giving your soaked mound only the barest ghost of a touch.' );
			} else {
				MainView.outputText( 'taint, the tips giving your sensitive skin only the barest ghost of a touch.' );
			}
			...
			MainView.outputText( ' wild with need.  Thankfully, as soon as you submit to her puffy mound and give it a lick, a pair of soft, wrinkled feet wrap around your ' + Descriptors.cockDescript( x ) + ', drawing out a thick flood of your pre-cum.' );
			...
			MainView.outputText( '\n\n"<i>' );
			if( EngineCore.silly() ) {
				MainView.outputText( 'Stop right there, criminal scum.' );
			} else {
				MainView.outputText( 'Hold your horses.' );
			}
			MainView.outputText( '</i>"  The emerald assassin glances over her shoulder at you and urges, "<i>Come on now.  Fill me up.  Let out all that spunk I’ve been making build up ' );
			if( CoC.player.balls > 0 ) {
				MainView.outputText( 'in your balls' );
			} else {
				MainView.outputText( 'inside you' );
			}
			MainView.outputText( '.  Pay your ‘fine’, and I might let you go.</i>"' );
			if( CoC.player.balls > 0 ) {
				MainView.outputText( '  A hand squeezes your [sack] encouragingly' );
			} else {
				MainView.outputText( '  Hands squeeze your [butt] encouragingly' );
			}
			...
			if( CoC.player.cumQ() >= 750 ) {
				MainView.outputText( '  Even her near-supernatural fertility can’t keep up with your fantastic virility, and soon the goblin is sporting a small bump on her belly.' );
				if( CoC.player.cumQ() >= 2000 ) {
					MainView.outputText( '  It swells to a rounded, pregnant dome by the time you finish.' );
				}
			}
			MainView.outputText( '  All of your passion spends itself in the glorious climax, leaving you feeling like an empty, wasted husk.' );
		...
		//ii. Lust Loss - Male - No Fit;
			...
			MainView.outputText( 'You tear through your [armor] in a rush, anything to release [eachCock] from its hellish confinement.  You’re so aroused, so horny.  Any passing thoughts of modesty are immediately discarded and forgotten, washed away under the wave of your all-consuming lust as your fingers slide home around [oneCock] and begin to play with the turgid tool.  You immediately buck from the sensation of incredible, pent-up need, nearly cumming on the spot.  Looking up, you meet the emerald-skinned woman’s eyes pleadingly.' );
			...
			if( CoC.player.biggestCockLength() > CoC.player.tallness / 2 ) {
				MainView.outputText( '  Your incredibly long phallus blocks your view of the sultry goblin, and you never see the blow coming.' );
			}
			MainView.outputText( '  A none-too-tender swat connects with your ' );
			if( CoC.player.balls > 0 ) {
				MainView.outputText( '[balls]' );
			} else {
				MainView.outputText( 'cock' );
			}
			MainView.outputText( ' as the goblin grunts, "<i>Stop squirming!</i>"' );
			...
			if( CoC.player.biggestCockLength() < CoC.player.tallness / 2 ) {
				MainView.outputText( 'sitting on your face, the ' + CoC.player.cockHead( x ) + ' of your maleness jutting against her drooling lips.' );
			} else {
				MainView.outputText( 'sitting on the ground ‘above’ you, your ' + CoC.player.cockHead( x ) + ' reaching all the way up to spear her moist box, judging by what you’re feeling anyway.' );
			}
		...
		//iii. Lust Loss - Female;
			...
			MainView.outputText( 'Defeated by your own needs, your fingers dart to your [armor], rapidly disassembling it in your hurry to expose your aching, tender puss to the goblin’s eyes.  The assassin watches you with an unimpressed, bemused expression, though she remains rooted in place, her gaze locked onto you as you succumb to your lusts.  Knowing that you have an audience, you twist and contort your body to present your [chest] and [vagina] to the petite greenskin.  Glistening moisture forms on your exposed nether-lips, tempting you to slide your fingers into their slick warmth.  As if you could resist.' );
			...
			if( CoC.player.hasFuckableNipples() ) {
				MainView.outputText( 'piston in and out of your [nipple], masturbating it just as hard as the real thing.' );
			} else {
				MainView.outputText( 'squeeze around a taut nipple, twisting and pulling on it unthinkingly.' );
			}
			MainView.outputText( '  Your masturbatory reverie is interrupted sudden impacts against both your wrists.' );
