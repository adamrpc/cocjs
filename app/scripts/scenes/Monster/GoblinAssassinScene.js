...
angular.module( 'cocjs' ).run( function( $log, ImageManager, CockTypesEnum, Appearance, Combat, GoblinAssassin, Descriptors, StatusAffects, AppearanceDefs, Utils, kFLAGS, CoC, EngineCore ) {
	...
	GoblinAssassinScene.prototype.goblinAssassinEncounter = function() {
		...
		//First Time Intro;
		if( CoC.getInstance().flags[ kFLAGS.TIMES_ENCOUNTERED_GOBLIN_ASSASSIN ] === 0 ) {
		...
		CoC.getInstance().flags[ kFLAGS.TIMES_ENCOUNTERED_GOBLIN_ASSASSIN ]++;
	...
	GoblinAssassinScene.prototype.gobboAssassinBeatYaUp = function() {
		...
		if( CoC.getInstance().player.cockThatFits( CoC.getInstance().monster.vaginalCapacity() ) >= 0 && CoC.getInstance().player.hasCock() && (!CoC.getInstance().player.hasVagina() || Utils.rand( 2 ) === 0) ) {
			...
			if( CoC.getInstance().player.femininity < 40 ) {
		...
		//ii. Lust Loss - Male - No Fit;
		else if( CoC.getInstance().player.biggestCockArea() > CoC.getInstance().monster.vaginalCapacity() && CoC.getInstance().player.hasCock() && (!CoC.getInstance().player.hasVagina() || Utils.rand( 2 ) === 0) ) {
			...
			if( CoC.getInstance().player.biggestCockLength() < CoC.getInstance().player.tallness / 2 ) {
				EngineCore.outputText( 'sitting on your face, the ' + CoC.getInstance().player.cockHead( x ) + ' of your maleness jutting against her drooling lips.' );
			} else {
				EngineCore.outputText( 'sitting on the ground ‘above’ you, your ' + CoC.getInstance().player.cockHead( x ) + ' reaching all the way up to spear her moist box, judging by what you’re feeling anyway.' );
			}
			...
			} else if( CoC.getInstance().player.cumQ() <= 1750 ) {
	...
	GoblinAssassinScene.prototype.gobboAssassinRapeIntro = function() {
		...
		if( CoC.getInstance().player.lust < 30 && CoC.getInstance().player.findStatusAffect( StatusAffects.Feeder ) < 0 ) {
			Combat.cleanupAfterCombat();
			return;
		}
		var buttseks = null;
		...
		if( CoC.getInstance().player.hasCock() ) {
			...
			if( CoC.getInstance().player.cockArea( CoC.getInstance().player.biggestCockIndex() ) > CoC.getInstance().monster.vaginalCapacity() ) {
			...
			//Buttsex toggle;
			if( CoC.getInstance().player.cockThatFits( CoC.getInstance().monster.analCapacity() ) >= 0 && CoC.getInstance().player.cor > 70 ) {
				buttseks = this.gobboButtSecks;
			}
			//Spidercondom;
			if( CoC.getInstance().player.tailType === AppearanceDefs.TAIL_TYPE_SPIDER_ADBOMEN && CoC.getInstance().player.cockThatFits( CoC.getInstance().monster.vaginalCapacity() ) >= 0 ) {
		...
		if( CoC.getInstance().player.lust >= 33 && CoC.getInstance().player.gender > 0 && (fitsFuck !== null || cuntFuck !== null || tooBig !== null ||
			...
			//EngineCore.doYesNo(feeder,Combat.cleanupAfterCombat);;
		} else {
	...
	GoblinAssassinScene.prototype.giveGoblinAMilkMustache = function() {
		...
		EngineCore.outputText( 'She struggles against your grip, trying to get something, anything inside her needy pussy while yelling "<i>Come on ' + CoC.getInstance().player.mf( 'slut', 'stud' ) + ', you know you want to - mmph!</i>"  You cut her off by shoving her mouth onto your ' + Descriptors.nippleDescript( 0 ) + '. She gasps involuntarily, filling her mouth with your milk. In an instant she freezes, then slowly swallows the milk in her mouth. She relaxes in your arms a moment later, gently suckling at your nipple. Her old lust-filled self is gone, replaced with a pliant girl who now wants nothing but your milk. You slowly lower your hand and start rubbing at her still-slick pussy. In response, she puts her hand on your other ' + Descriptors.nippleDescript( 0 ) + ', playing with it and teasing you.\n\n', false );
	...
	GoblinAssassinScene.prototype.gobboButtSecks = function() {
		...
		EngineCore.outputText( 'As you pick up your ' + CoC.getInstance().player.armorName + ' and begin to get dressed, you glance at the goblin. Her hands began to dig in her now stretched out anus, desperately trying to gather up the cum you deposited in her. Smirking, you walk away nonchalantly, quite pleased with yourself.', false );
		Combat.cleanupAfterCombat();
	...
	GoblinAssassinScene.prototype.rapeAGoblinCorruptTooBig = function() {
		...
		EngineCore.outputText( 'You begin to remove your ' + CoC.getInstance().player.armorName + ', looking down on your helpless would-be-attacker and soon-to-be victim while licking your lips hungrily. Your ' + Descriptors.multiCockDescriptLight(), false );
		if( CoC.getInstance().player.cockTotal() === 1 ) {
		...
		EngineCore.outputText( 'You have a seat, legs wide, on the ground and hold the little goblin whore with her relatively tiny slit resting at the tip of your ' + Descriptors.cockDescript( x ) + '. Finally out of her stupor a look of extreme conflict crosses her face as her need for cock and cum goes to war with her survival instincts. On the brink of defeat, those instinct suddenly regroup and beat back her lust once she feels the pain of just the ' + CoC.getInstance().player.cockHead( x ) + ' of your ' + Descriptors.cockDescript( x ) + ' starting to stretch out her ' + CoC.getInstance().monster.vaginaDescript() + '. She does all she can to resist, but with the way you\'re holding this is little more than flailing wildly and pushing against your ' + Descriptors.cockDescript( x ) + ' with her feet, practically climbing it like the tree trunk it must look like from her perspective.\n\n', false );
		EngineCore.outputText( 'Both of you dripping with sweat from your respective exertions, you slowly begin to realize the combination of her furious struggling and the tightness of her ' + CoC.getInstance().monster.vaginaDescript() + ' is going to keep you from the penetration you were so looking forward to. However, as you begin to consider finishing off in her throat, a darkness stirs and another idea crosses your mind.\n\n', false );
		EngineCore.outputText( 'The goblin relaxes a little when she feels you no longer pressing her down onto your ' + Descriptors.cockDescript( x ) + '. She absolutely thrills when you bring her tiny pussy to your lips and begin to have at it. Your tongue plays around both on the inside and outside of her ' + CoC.getInstance().monster.vaginaDescript() + ' until you coax her little nub out from hiding. You wrap your lips around it and begin putting your corruption to task. As you drive the goblin slut closer and closer to orgasm, working over her clit with an expertise rarely found outside of the infernal ranks, bolts of corrupt energies travel from your tongue, through her clit, and deep into her core. As she cums, screaming, you pull off of her, admiring the outward signs of your demonic handiwork. What before was an average sized love button has swollen five times its size into a proud, engorged clit... and that\'s not the only thing you enlarged.\n\n', false );
		EngineCore.outputText( 'You reposition your fucktoy so that now she\'s facing away from you and again bring your ' + Descriptors.cockDescript( x ) + ' into contact with her ' + CoC.getInstance().monster.vaginaDescript() + '. Again she struggles, but after her orgasm she doesn\'t have the strength to put up a decent fight; all her resistance does is arouse you further, now that she\'s fighting the inevitable. You begin to press her down onto yourself.\n\n', false );
		...
		EngineCore.outputText( 'er spasming ' + CoC.getInstance().monster.vaginaDescript() + ' is clamping down so hard on your ' + Descriptors.cockDescript( x ) + ' that it can\'t release.\n\n', false );
	...
	GoblinAssassinScene.prototype.gobboGetsRapedMaleFits = function() {
		...
		if( CoC.getInstance().player.cockArea( x ) > CoC.getInstance().monster.vaginalCapacity() * 0.8 ) {
			...
			} else if( CoC.getInstance().player.cocks[ 0 ].cockType === CockTypesEnum.HORSE || CoC.getInstance().player.cocks[ 0 ].cockType.Index > 3 ) {
			...
			if( CoC.getInstance().player.biggestTitSize() >= 1 ) {
				EngineCore.outputText( CoC.getInstance().player.allBreastsDescript(), false );
			} else {
	...
	GoblinAssassinScene.prototype.goblinCondomed = function() {
		...
		EngineCore.outputText( 'Defeated, the goblin girl\'s knees give out and she sinks backward, lying on her back with her emerald ankles suspended above her head. "Use me," she begs, "humiliate, degrade, and debase me! Just, whatever you do, fill me!" As you strip off your ' + CoC.getInstance().player.armorName + ', she spreads her legs as wide as she can, the wanton girl presenting her drooling pussy to you, puffy green lips already dripping with beads of anxious sweat and eager lubrication. She wiggles in the dirt, gripping her plump rear with both hands and lifting her ass into the air for you, hopefully. You can practically feel the heat pouring off the small slut\'s cum-hungry cunt, her breeding-fever leaving her eyes glassy and unfocused. Standing over her, it\'s clear that the only things she\'s even aware of are the pulsing pussy between her legs and your burgeoning erection.\n\n', false );
		EngineCore.outputText( 'Impatiently, she thrusts her legs out and hooks her toes around your lower body, trying to pull you closer while still keeping her needy hole accessible. Her olive feet clench around your flesh, her soles firm and muscular on your ' + CoC.getInstance().player.skinFurScales() + ' as she slides up and down the outsides of your ' + Descriptors.hipDescript() + '. Dragging her heels across your thighs, the goblin pushes her feet together on either side of your ' + Descriptors.cockDescript( x ) + ', the balls of her jade skin pressing against ', false );
		if( CoC.getInstance().player.balls > 0 ) {
	...
	CoC.getInstance().registerScene( 'goblinAssassinScene', new GoblinAssassinScene() );
} );