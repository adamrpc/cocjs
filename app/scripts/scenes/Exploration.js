...
angular.module( 'cocjs' ).run( function( CoC, EngineCore, Giacomo, kFLAGS, EventParser, Utils, AppearanceDefs, Combat, Imp, Goblin, ComsumableLib, Descriptors, Appearance, StatusAffects ) {
	...
	Exploration.prototype.doExplore = function() {
		...
		if( CoC.getInstance().flags[ kFLAGS.EXPLORATION_PAGE ] === 2 ) {
			this.explorePageII();
			return;
		}
		EngineCore.menu();
	...
	Exploration.prototype.goBackToPageI = function() {
		CoC.getInstance().flags[ kFLAGS.EXPLORATION_PAGE ] = 1;
		this.doExplore();
	};

	//Try to find a new location - called from this.doExplore once the first location is found
	Exploration.prototype.tryDiscover = function() {
		...
		if( CoC.getInstance().player.explored > 1 ) {
			...
			//EXPLOOOOOOORE
			if( CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00272 ] === 0 && CoC.getInstance().flags[ kFLAGS.TIMES_EXPLORED_PLAINS ] > 0 && Utils.rand( 3 ) === 0 ) {
				CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00272 ] = 1;
				CoC.getInstance().player.explored++;
				EngineCore.outputText( '', true );
				EngineCore.outputText( 'All things considered, you decide you wouldn\'t mind a change of scenery.  Gathering up your belongings, you begin a journey into the wasteland.  The journey begins in high spirits, and you whistle a little traveling tune to pass the time.  After an hour of wandering, however, your wanderlust begins to whittle away.  Another half-hour ticks by.  Fed up with the fruitless exploration, you\'re nearly about to head back to CoC.getInstance().scenes.camp when a faint light flits across your vision.  Startled, you whirl about to take in three luminous will-o\'-the-wisps, swirling around each other whimsically.  As you watch, the three ghostly lights begin to move off, and though the thought of a trap crosses your mind, you decide to follow.\n\n', false );
				EngineCore.outputText( 'Before long, you start to detect traces of change in the environment.  The most immediate difference is the increasingly sweltering heat.  A few minutes pass, then the will-o\'-the-wisps plunge into the boundaries of a dark, murky, stagnant swamp; after a steadying breath you follow them into the bog.  Once within, however, the gaseous balls float off in different directions, causing you to lose track of them.  You sigh resignedly and retrace your steps, satisfied with your discovery.  Further exploration can wait.  For now, your CoC.getInstance().scenes.camp is waiting.\n\n', false );
			...
			if( choosey === 2 && CoC.getInstance().flags[ kFLAGS.FOUND_CATHEDRAL ] === 1 ) {
	...
	Exploration.prototype.bigJunkDesertScene = function() {
		...
		if( CoC.getInstance().player.cocks.length === 1 ) {
			EngineCore.outputText( '  As it drags along the dunes, the sensation forces you to imagine the rough textured tongue of a monstrous animal sliding along the head of your ' + Appearance.cockNoun( CoC.getInstance().player.cocks[ x ].cockType ) + '.', false );
		} else if( CoC.getInstance().player.cocks.length >= 2 ) {
			EngineCore.outputText( '  With all of your ' + Descriptors.multiCockDescriptLight() + ' dragging through the sands they begin feeling as if the rough textured tongues of ' + Utils.num2Text( CoC.getInstance().player.cockTotal() ) + ' different monstrous animals were slobbering over each one.', false );
		}
		EngineCore.outputText( '\n\n', false );
		//PARAGRAPH 2
		//FOR NON-CENTAURS]
		if( !CoC.getInstance().player.isTaur() ) {
			...
			if( CoC.getInstance().player.balls > 0 ) {
				EngineCore.outputText( '  Your ' + CoC.getInstance().player.skinTone + Descriptors.sackDescript() + ' rests beneath your raised ' + Descriptors.buttDescript() + '.  The fiery warmth of the desert caresses it, causing your ' + Descriptors.ballsDescriptLight() + ' to pulse with the need to release their sperm through your ' + Descriptors.multiCockDescriptLight() + '.', false );
			}
		...
		//FOR CENTAURS
			...
			//IF CHARACTER HAS A BALLS ADD SENTENCE
			if( CoC.getInstance().player.balls > 0 ) {
				EngineCore.outputText( '  Your ' + CoC.getInstance().player.skinTone + Descriptors.sackDescript() + ' rests beneath your raised ' + Descriptors.buttDescript() + '.  The airy warmth of the desert teases it, causing your ' + Descriptors.ballsDescriptLight() + ' pulse with the need to release their sperm through your ' + Descriptors.multiCockDescriptLight() + '.', false );
			}
			//IF CHARACTER HAS A VAGINA ADD SENTENCE
			if( CoC.getInstance().player.vaginas.length >= 1 ) {
				EngineCore.outputText( '  Your ' + Descriptors.vaginaDescript() + ' and ' + Descriptors.clitDescript() + ' are thoroughly squashed between the bulky flesh where your male genitals protrude from between your hips and the ' + Descriptors.buttDescript() + ' above.', false );
		...
		if( CoC.getInstance().player.canFly() ) {
			EngineCore.outputText( '  You extend your wings and flap as hard as you can, until at last you manage to lighten the bulk of your body somewhat - enough to allow yourself to drag your genitals across the hot sands and back to CoC.getInstance().scenes.camp.  The ordeal takes nearly an hour.', false );
		}//SCENE END IF CHARACTER HAS CENTAUR BODY
		else if( CoC.getInstance().player.isTaur() ) {
			EngineCore.outputText( '  You struggle and work your equine legs against the surface of the dune you are trapped on.  Your ' + CoC.getInstance().player.feet() + ' have consistent trouble finding footing, the soft sand failing to provide enough leverage to lift your bulk.  You breath in deeply and lean from side to side, trying to find some easier vertical leverage.  Eventually, with a crude crawl, your legs manage to push the bulk of your body onto more solid ground.  With great difficulty, you spend the next hour shuffling your genitals across the sandscape and back to CoC.getInstance().scenes.camp.', false );
