...
angular.module( 'cocjs' ).run( function( MainView, SceneLib, CoC, EngineCore, kFLAGS, Utils, AppearanceDefs, Imp, Goblin, ConsumableLib, Descriptors, Appearance, StatusAffects ) {
	...
	Exploration.prototype.doExplore = function() {
		...
		if( CoC.player.exploredForest > 0 ) {
			EngineCore.addButton( 2, 'Forest', SceneLib.forest, SceneLib.forest.exploreForest );
		}
		if( CoC.player.exploredLake > 0 ) {
			EngineCore.addButton( 3, 'Lake', SceneLib.lake, SceneLib.lake.exploreLake );
		}
		EngineCore.addButton( 4, 'Next', this, this.explorePageII );
		if( CoC.flags[ kFLAGS.TIMES_EXPLORED_PLAINS ] > 0 ) {
			EngineCore.addButton( 5, 'Plains', SceneLib.plains, SceneLib.plains.explorePlains );
		}
		...
		if( CoC.player.exploredMountain > 0 ) {
			EngineCore.addButton( 8, 'Mountain', SceneLib.mountain, SceneLib.mountain.exploreMountain );
		}
	...
	Exploration.prototype.explorePageII = function() {
		...
		if( CoC.flags[ kFLAGS.BOG_EXPLORED ] > 0 ) {
			EngineCore.addButton( 1, 'Bog', SceneLib.bog, SceneLib.bog.exploreBog );
		}
		EngineCore.addButton( 4, 'Previous', this, this.goBackToPageI );
		EngineCore.addButton( 9, 'Back', null, MainView.playerMenu );
	};
	...
	Exploration.prototype.tryDiscover = function() {
		...
		if( CoC.player.explored > 1 ) {
			...
			if( CoC.player.exploredLake >= 1 && Utils.rand( 3 ) === 0 && CoC.player.exploredDesert === 0 ) {
				MainView.outputText( 'You stumble as the ground shifts a bit underneath you.  Groaning in frustration, you straighten up and discover the rough feeling of sand ', true );
				if( CoC.player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_HUMAN ) {
					MainView.outputText( 'inside your footwear, between your toes', false );
				}
				if( CoC.player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_HOOFED ) {
					MainView.outputText( 'in your hooves', false );
				}
				if( CoC.player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_DOG ) {
					MainView.outputText( 'in your paws', false );
				}
				if( CoC.player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_NAGA ) {
					MainView.outputText( 'in your scales', false );
				}
			...
			if( CoC.player.exploredDesert >= 1 && Utils.rand( 3 ) === 0 && CoC.player.exploredMountain === 0 ) {
				MainView.outputText( 'Thunder booms overhead, shaking you out of your thoughts.  High above, dark clouds encircle a distant mountain peak.  You get an ominous feeling in your gut as you gaze up at it.\n\n<b>You have discovered the mountain!</b>', true );
				CoC.player.explored++;
			...
			if( CoC.player.exploredMountain >= 1 && Utils.rand( 3 ) === 0 && CoC.flags[ kFLAGS.TIMES_EXPLORED_PLAINS ] === 0 ) {
				...
				CoC.player.explored++;
				MainView.outputText( 'You find yourself standing in knee-high grass, surrounded by flat plains on all sides.  Though the mountain, forest, and lake are all visible from here, they seem quite distant.\n\n<b>You\'ve discovered the plains!</b>', true );
				EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
				return;
			}
			//EXPLOOOOOOORE
	...
	Exploration.prototype.bigJunkDesertScene = function() {
		...
		if( CoC.player.cocks.length === 1 ) {
			MainView.outputText( '  As it drags along the dunes, the sensation forces you to imagine the rough textured tongue of a monstrous animal sliding along the head of your ' + Appearance.cockNoun( CoC.player.cocks[ x ].cockType ) + '.', false );
		} else if( CoC.player.cocks.length >= 2 ) {
			MainView.outputText( '  With all of your ' + Descriptors.multiCockDescriptLight() + ' dragging through the sands they begin feeling as if the rough textured tongues of ' + Utils.num2Text( CoC.player.cockTotal() ) + ' different monstrous animals were slobbering over each one.', false );
		}
		MainView.outputText( '\n\n', false );
		//PARAGRAPH 2
		//FOR NON-CENTAURS]
		if( !CoC.player.isTaur() ) {
			...
			if( CoC.player.biggestTitSize() >= 35 ) {
				MainView.outputText( '  Your ' + Descriptors.allBreastsDescript() + ' hang lewdly off your torso to rest on the desert sands, seeming to bury the dunes on either side of you.  Their immense weight anchors your body, further preventing your torso from lifting itself up.  The burning heat of the desert teases your ' + Descriptors.nippleDescript( 0 ) + 's mercilessly as they grind in the sand.', false );
			}
			//IF CHARACTER HAS A BALLS ADD SENTENCE
			if( CoC.player.balls > 0 ) {
				MainView.outputText( '  Your ' + CoC.player.skinTone + Descriptors.sackDescript() + ' rests beneath your raised ' + Descriptors.buttDescript() + '.  The fiery warmth of the desert caresses it, causing your ' + Descriptors.ballsDescriptLight() + ' to pulse with the need to release their sperm through your ' + Descriptors.multiCockDescriptLight() + '.', false );
			}
			...
			if( CoC.player.vaginas.length >= 1 ) {
				...
				if( CoC.player.vaginas[ 0 ].vaginalWetness >= AppearanceDefs.VAGINA_WETNESS_DROOLING ) {
					MainView.outputText( '  Juices stream from your womanhood and begin pooling on the hot sand beneath you.  Wisps of steam rise up into the air only to tease your genitals further.  ', false );
				}
			}
		}
		//FOR CENTAURS
			...
			//IF CHARACTER HAS GIANT BREASTS ADD SENTENCE
			if( CoC.player.biggestTitSize() >= 35 ) {
				MainView.outputText( '  Your ' + Descriptors.allBreastsDescript() + ' pull your human torso forward until it also is forced to rest facedown, just like your horse half.  Your tits rest, pinned on the desert sand to either side of you.  Their immense weight anchors you, further preventing any part of your equine body from lifting itself up.  The burning heat of the desert teases your ' + Descriptors.nippleDescript( 0 ) + 's incessantly.', false );
			}
			//IF CHARACTER HAS A BALLS ADD SENTENCE
			if( CoC.player.balls > 0 ) {
				MainView.outputText( '  Your ' + CoC.player.skinTone + Descriptors.sackDescript() + ' rests beneath your raised ' + Descriptors.buttDescript() + '.  The airy warmth of the desert teases it, causing your ' + Descriptors.ballsDescriptLight() + ' pulse with the need to release their sperm through your ' + Descriptors.multiCockDescriptLight() + '.', false );
			}
			//IF CHARACTER HAS A VAGINA ADD SENTENCE
			if( CoC.player.vaginas.length >= 1 ) {
				MainView.outputText( '  Your ' + Descriptors.vaginaDescript() + ' and ' + Descriptors.clitDescript() + ' are thoroughly squashed between the bulky flesh where your male genitals protrude from between your hips and the ' + Descriptors.buttDescript() + ' above.', false );
				//IF CHARACTER HAS A DROOLING PUSSY ADD SENTENCE
				if( CoC.player.vaginas[ 0 ].vaginalWetness >= AppearanceDefs.VAGINA_WETNESS_DROOLING ) {
					MainView.outputText( '  The desert sun beats down on your body, its fiery heat inflaming the senses of your vaginal lips.  Juices stream from your womanhood and begin pooling on the hot sand beneath you.', false );
				}
			}
		}
		MainView.outputText( '\n\n', false );
		...
		if( CoC.player.canFly() ) {
			MainView.outputText( '  You extend your wings and flap as hard as you can, until at last you manage to lighten the bulk of your body somewhat - enough to allow yourself to drag your genitals across the hot sands and back to camp.  The ordeal takes nearly an hour.', false );
