...
angular.module( 'cocjs' ).run( function( MainView, SceneLib, $log, Useable, Armor, Weapon, CoC_Settings, ConsumableLib, ItemSlot, ItemType, OnLoadVariables, WeaponLib, CoC, EngineCore, kFLAGS, Utils, StatusAffects ) {
	...
	Inventory.prototype.inventoryMenu = function() {
		...
		MainView.outputText( '<b><u>Equipment:</u></b>\n' );
		MainView.outputText( '<b>Weapon</b>: ' + CoC.player.weaponName + ' (Attack - ' + CoC.player.weaponAttack + ')\n' );
		MainView.outputText( '<b>Armor : </b>' + CoC.player.armorName + ' (Defense - ' + CoC.player.armorDef + ')\n' );
		if( CoC.player.keyItems.length > 0 ) {
			MainView.outputText( '<b><u>\nKey Items:</u></b>\n' );
		}
	...
	Inventory.prototype.stash = function() {
		...
		if( CoC.player.hasKeyItem( 'Camp - Chest' ) >= 0 ) {
			MainView.outputText( 'You have a large wood and iron chest to help store excess items located near the portal entrance.\n\n' );
		...
		if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00254 ] > 0 ) {
			MainView.outputText( 'There\'s a weapon rack set up here, set up to hold up to nine various weapons.' );
		...
		if( CoC.flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00255 ] > 0 ) {
			MainView.outputText( 'Your camp has an armor rack set up to hold your various sets of gear.  It appears to be able to hold nine different types of armor.' );
	...
	Inventory.prototype.takeItem = function( itype, nextAction, overrideAbandon, source ) {
		...
		if( temp >= 0 ) {
			...
			MainView.outputText( 'You place ' + itype.longName + ' in your ' + inventorySlotName[ temp ] + ' pouch, giving you ' + CoC.player.itemSlots[ temp ].quantity + ' of them.' );
			this.itemGoNext();
			return;
		}
		...
		if( temp >= 0 ) {
			...
			MainView.outputText( 'You place ' + itype.longName + ' in your ' + inventorySlotName[ temp ] + ' pouch.' );
			this.itemGoNext();
			return;
		}
	...
	Inventory.prototype.giveHumanizer = function() {
		if( CoC.flags[ kFLAGS.TIMES_CHEATED_COUNTER ] > 0 ) {
			MainView.outputText( '<b>I was a cheater until I took an arrow to the knee...</b>', true );
			EngineCore.gameOver();
			return;
		}
		MainView.outputText( 'I AM NOT A CROOK.  BUT YOU ARE!  <b>CHEATER</b>!\n\n', true );
	...
	Inventory.prototype.useItemInInventory = function( slotNum ) {
		...
		if( CoC.player.itemSlots[ slotNum ].itype instanceof Useable ) {
			...
			MainView.outputText( 'You cannot use ' + CoC.player.itemSlots[ slotNum ].itype.longName + '!\n\n' );
		}
	...
	Inventory.prototype.replaceItem = function( itype, slotNum ) {
		...
		if( CoC.player.itemSlots[ slotNum ].itype === itype ) {
			MainView.outputText( 'You discard ' + itype.longName + ' from the stack to make room for the new one.' );
		} else {
			if( CoC.player.itemSlots[ slotNum ].quantity === 1 ) {
				MainView.outputText( 'You throw away ' + CoC.player.itemSlots[ slotNum ].itype.longName + ' and replace it with ' + itype.longName + '.' );
			} else {
				MainView.outputText( 'You throw away ' + CoC.player.itemSlots[ slotNum ].itype.longName + '(x' + CoC.player.itemSlots[ slotNum ].quantity + ') and replace it with ' + itype.longName + '.' );
			}
	...
	Inventory.prototype.armorRackDescription = function() {
		if( this.itemAnyInStorage( this.gearStorage, 9, 18 ) ) {
			...
			MainView.outputText( '  It currently holds ' + Utils.formatStringArray( itemList ) + '.' );
			return true;
		}
		return false;
	};
	...
	Inventory.prototype.placeIn = function( storage, startSlot, endSlot, slotNum ) {
		...
		for( x = startSlot; x < endSlot && qty > 0; x++ ) { //Find any slots which already hold the item that is being stored
			if( storage[ x ].itype === itype && storage[ x ].quantity < 5 ) {
				...
				MainView.outputText( 'You add ' + temp + 'x ' + itype.shortName + ' into storage slot ' + Utils.num2Text( x + 1 - startSlot ) + '.\n' );
		...
		for( x = startSlot; x < endSlot && qty > 0; x++ ) { //Find any empty slots and put the item(s) there
			if( storage[ x ].quantity === 0 ) {
				...
				MainView.outputText( 'You place ' + qty + 'x ' + itype.shortName + ' into storage slot ' + Utils.num2Text( x + 1 - startSlot ) + '.\n' );
				qty = 0;
				return;
			}
		}
		MainView.outputText( 'There is no room for ' + (orig === qty ? '' : 'the remaining ') + qty + 'x ' + itype.shortName + '.  You leave ' + (qty > 1 ? 'them' : 'it') + ' in your inventory.\n' );
