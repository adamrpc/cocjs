...
angular.module( 'cocjs' ).run( function( Descriptors, kFLAGS, CoC, EngineCore ) {
	...
	XmasGatsNotAnAngel.prototype.encounterQuoteUnquoteAngel = function() {
		...
		if( !CoC.getInstance().scenes.xmasBitch.isHolidays() ) {
			...
			if( EngineCore.silly() ) {
				EngineCore.outputText( ' (you didn\'t.)' );
			} else {
				EngineCore.outputText( '.' );
			}
		...
		//ii. Yes;
			...
			if( EngineCore.silly() ) {
				EngineCore.outputText( ', without a penis' );
			}
	...
	XmasGatsNotAnAngel.prototype.theWorstEndingEverDotJPG = function() {
		...
		if( CoC.getInstance().player.hasCock() ) {
			...
			EngineCore.outputText( 'windup key' );
		}
	...
	XmasGatsNotAnAngel.prototype.fuckTheAngelBadEnd = function() {
		...
		if( CoC.getInstance().player.hasCock() ) {
			...
			EngineCore.outputText( Descriptors.vaginaDescript() + ' throbs' );
		}
	...
	CoC.getInstance().registerScene( 'xmasGatsNotAnAngel', new XmasGatsNotAnAngel() );
} );