...
angular.module( 'cocjs' ).run( function( SceneLib, MainView, Descriptors, kFLAGS, CoC, EngineCore ) {
	...
	XmasGatsNotAnAngel.prototype.dontExploreGatsChristmasTale = function() {
		MainView.clearOutput();
		//i. No;
		MainView.outputText( 'In a place like the high mountains, there\'s no warning to be found if you happen to be confronted by danger.  Deciding not to take your chances, you continue to walk down the path you came, leaving the peaks to head straight back to camp.' );
	...
	XmasGatsNotAnAngel.prototype.encounterQuoteUnquoteAngel = function() {
		...
		if( !SceneLib.xmasBitch.isHolidays() ) {
			...
			if( EngineCore.silly() ) {
				MainView.outputText( ' (you didn\'t.)' );
			} else {
				MainView.outputText( '.' );
			}
			MainView.outputText( '  She beams a toothless grin, popping one into her mouth and blowing out a stream of candy-scented smoke.  Suddenly, you\'re glad you didn\'t take any.' );
		...
		//ii. Yes;
			...
			MainView.outputText( 'You haven\'t found anything as of yet, so you figure you might as well keep going.  Following the quiet cries from above, you trace the spiralling side of the snow-capped behemoth, eventually reaching a small cliff hanging beneath the mountain\'s peak.  The sight you\'re met with manages to take your breath away - and considering the inhabitants of the land around you, that in itself is quite a difficult feat.' );
			...
			if( EngineCore.silly() ) {
				MainView.outputText( ', without a penis' );
			}
			MainView.outputText( ' - a loving aura surrounds the feminine creature and you can\'t help but feel as if she\'s here without ill intent.  Stating your name, you question her purpose at the high mountains.' );
	...
	XmasGatsNotAnAngel.prototype.findTheWindUpKey = function() {
		...
		if( CoC.player.tallness > 60 ) {
			MainView.outputText( '  You can\'t even stand straight up below the low-hanging ceiling, forced to carefully tread along on your knees over the slick, hard rock.' );
		}
		MainView.outputText( '  Squinting around the small, deep green grotto, a pedestal in the center calls out to you.  As you approach it, it becomes evident to you why things are so chilly in here; the outcrop is made up of clear ice - and within, the bodies of the many who had found this place before you.  At least that mystery is cleared up.  Atop the frozen prop lies a small, crystal... crank? Upon closer inspection, it actually resembles a - kitty titties, it\'s a windup key!  Despite the fact that you should\'ve seen this one coming, you surmise that this is the item you\'ve come searching for.' );
	...
	XmasGatsNotAnAngel.prototype.giveThatBitchAKeyYo = function() {
		...
		if( CoC.flags[ kFLAGS.GATS_ANGEL_TIME_TO_FIND_KEY ] < 150 ) {
			...
			MainView.outputText( 'All throughout your perilous climb, you cannot help but ogle at the brilliant little crystal key.  You have a suspicion as to how this innocent little key will help the crying colossus on the peak, though you aren\'t entirely sure this is the artifact she required.  But you have a hunch that it is.' );
	...
	XmasGatsNotAnAngel.prototype.theWorstEndingEverDotJPG = function() {
		...
		if( CoC.player.cor > 49 ) {
			MainView.outputText( '  It seems like the best response to you for now.  Based on the giantess\'s sobbing soliloquy, you figure that standing in for the love of the soldier\'s life will get you straight to the action.' );
		} else {
			MainView.outputText( '  Wait, what?  It appears that in your desperation to contain the situation, your lingering doubt was enough to get you blurt out a bold faced lie.  You figure that you may as well roll with it for now.' );
		}
		...
		MainView.outputText( ' to try and convince the confused soldier.  You look North over, figuring the best way to work him under your spell.  It isn\'t a long search, as you decide that his ' );
		//[if (hasCock = true) ;
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'keyhole' );
		}//[if (hasCock = false) ;
		else {
			MainView.outputText( 'windup key' );
		}
		MainView.outputText( ' would make a worthy candidate for your first alluring act of debauchery.' );
		...
		//[if (hasCock = true) ;
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'You slide the key playfully down your wrist, rubbing your [cock] with your free hand.  You aren\'t entirely sure if he\'ll even feel a sensation from your penetration, but perhaps it\'ll at least get him in the mood.  With your manhood at full mast, you gently skirt along the edges of the keyhole.' );
		} else {
			MainView.outputText( 'A mischievous tongue traces along the smooth, perfect surface of the windup key.  You promptly lower the makeshift toy precariously in front of your [cunt]; if there was ever a time to make a wind up pun or remark as some sort of goofy foreplay, now would be the time.' );
		}
		...
		//[if (corruption > 49) ;
		if( CoC.player.cor > 49 ) {
			MainView.outputText( '  0...Well, that was a waste of time.' );
		}
		...
		//[if (corruption > 49) ;
		if( CoC.player.cor > 49 ) {
			MainView.outputText( '  More importantly, you have some unsatisfied, pent up lust that you\'d like to expend.' );
		}
	...
	XmasGatsNotAnAngel.prototype.fuckTheAngelBadEnd = function() {
		...
		MainView.outputText( ', you wander your way towards the giant\'s mouth-muscle, climbing on top of her warm, pink bed. There\'s gotta be some kinda magic involved - her mint saliva sends tingles through your body, while dragging your ' );
		if( CoC.player.hasCock() ) {
			MainView.outputText( CoC.player.multiCockDescriptLight() );
		} else {
			MainView.outputText( Descriptors.vaginaDescript() );
		}
		MainView.outputText( ' against each bump and curve is beginning to get you hard.  She winces at the taste of your crotch against her tongue, though she\'s obviously never given oral to something with a heartbeat before.  Well, there\'s a first time for everything.' );
		...
		MainView.outputText( ' along the softness of her maw causing her to twitch and drool uncomfortably.  You\'re beginning to fuck along her slowly, chuckling at every wince and moan she makes from the taste of your genitals.' );
		...
		MainView.outputText( ' on a person\'s tongue to fuck it thoroughly.  You wonder if she\'s already long forgotten the pile of bolts she was originally trying to save.  A man frozen in time, never to wake up because - whoops!  You\'re taking care of her needs now.  Maybe you\'ll invest in a giant goblin-built sex machine, who knows?  Judging by the bitch\'s aroused whines, she was probably waiting for the chance to strip for the next person to find her up the mountain.  It\'s about time she let herself go, after all.' );
		...
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'hot, white seed' );
		} else {
			MainView.outputText( 'feminine juices' );
		}
		MainView.outputText( ' all along her starving tongue, your eyes lifting up as you stretch to force as much as you can upon her.  A slick, stubby tendril fits between your [butt], while more of her taste buds rub against you in your squirting ecstasy.  Your ' );
		if( CoC.player.hasCock() ) {
			MainView.outputText( CoC.player.multiCockDescriptLight() );
			if( CoC.player.cockTotal() === 1 ) {
				MainView.outputText( 'throbs' );
			} else {
				MainView.outputText( 'throb' );
			}
		} else {
			MainView.outputText( Descriptors.vaginaDescript() + ' throbs' );
		}
	...
	XmasGatsNotAnAngel.prototype.stayForPretendNTRGoodWinjobSex = function() {
		MainView.clearOutput();
		MainView.outputText( 'You prefer to stay here, but the frosty giant has already gone back to her mechanical man, so you hold your explanation and continue to observe.' );
		...
		if( CoC.player.lust >= 100 ) {
			...
			if( CoC.player.wetness() >= 4 || CoC.player.cumQ() >= 1000 ) {
				MainView.outputText( ' - figuratively, and literally, as well, as your fluids drip off the edge to add to the rain of sexual fluids the giantess is laying in the midst of' );
			}
			MainView.outputText( '.' );
		}
	...
	XmasGatsNotAnAngel.prototype.conclusionOfGiantessGoodEnd = function() {
		MainView.clearOutput();
		MainView.outputText( 'The air is crisp, and the light powdering of snow floats through the air as if playing an invisible melody.  The warm couple are together.  Lifting her head, the giantess beams at you with a refreshing sense of spirit.' );
