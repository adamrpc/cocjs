...
angular.module( 'cocjs' ).run( function( MainView, SceneLib, $log, CoC, kFLAGS, EngineCore, PerkLib, StatusAffects, Utils, CockTypesEnum, AppearanceDefs, Descriptors ) {
	...
	Masturbation.prototype.masturbateGo = function() {
		...
		if( SceneLib.dungeonCore.isInDungeon() ) {
			MainView.outputText( 'There is no way you could get away with masturbating in a place like this!  You\'d better find your way back to camp if you want to take care of that.' );
			EngineCore.doNext( MainView, MainView.playerMenu );
			return;
		}
		...
		if( CoC.player.cor < 15 ) {
			MainView.outputText( 'You sheepishly find some rocks to hide in, where you remove your clothes.\n\n' );
		} else if( CoC.player.cor < 30 ) {
			MainView.outputText( 'You make sure you are alone and strip naked.\n\n' );
		} else if( CoC.player.cor < 60 ) {
			MainView.outputText( 'You happily remove your ' + CoC.player.armorName + ', eager to pleasure yourself.\n\n' );
		} else if( CoC.player.cor < 80 ) {
			MainView.outputText( 'You strip naked in an exaggerated fashion, hoping someone might be watching.\n\n' );
		} else {
			MainView.outputText( 'You strip naked, fondling your naughty bits as you do so and casting seductive looks around, hoping someone or something is nearby to fuck you.\n\n' );
		}
		//Tit foreplay
		this.titForeplay();
		...
		if( CoC.player.cocks.length > 0 ) {
			if( CoC.player.cocks.length === 1 ) {
				MainView.outputText( 'You stroke your ' + CoC.player.cockDescript() );
				if( CoC.player.lib < 45 ) {
					MainView.outputText( ' eagerly, quickly bringing yourself to a full, throbbing state.  ' );
				} else if( CoC.player.lib < 70 ) {
					MainView.outputText( ' languidly, reveling at its near-constant hardness.  ' );
				} else {
					MainView.outputText( ' teasingly, pre-cum running down your length from your constant state of arousal.  ' );
				}
			} else {
				MainView.outputText( 'You stroke your ' + CoC.player.cockDescript() );
				if( CoC.player.lib < 45 ) {
					MainView.outputText( ' eagerly, quickly bringing your cocks to a full, throbbing state.  ' );
				} else if( CoC.player.lib < 70 ) {
					MainView.outputText( ' languidly, reveling at their near-constant hardness.  ' );
				} else {
					MainView.outputText( ' teasingly, pre-cum running down your cocks from your constant state of arousal, pooling around you.  ' );
				}
			}
		}
		if( CoC.player.vaginas.length > 0 ) {
			if( CoC.player.vaginas.length === 1 ) {
				...
				if( CoC.player.lib < 45 ) {
					MainView.outputText( 'You touch and play with your ' + CoC.player.vaginaDescript() + ', ' );
				} else if( CoC.player.lib < 70 ) {
					MainView.outputText( 'You slap your pussy softly, ' );
				} else {
					MainView.outputText( 'You touch your enflamed and aroused ' + CoC.player.vaginaDescript() + ', ' );
				}
				switch( CoC.player.vaginas[ 0 ].vaginalWetness ) {
					case AppearanceDefs.VAGINA_WETNESS_DRY:
						MainView.outputText( 'expertly arousing your female parts.  ' );
						break;
					case AppearanceDefs.VAGINA_WETNESS_NORMAL:
						MainView.outputText( 'sighing as it quickly becomes moist.  ' );
						break;
					case AppearanceDefs.VAGINA_WETNESS_WET:
						MainView.outputText( 'giggling as your fingers get a little wet.  ' );
						break;
					case AppearanceDefs.VAGINA_WETNESS_SLICK:
						MainView.outputText( 'smiling as your fingers become coated in your slick fluids.  ' );
						break;
					case AppearanceDefs.VAGINA_WETNESS_DROOLING:
						MainView.outputText( 'slicking your fingers in the juices that constantly dribble from ' + CoC.player.vaginaDescript() + '  ' );
						break;
					default:
						MainView.outputText( 'licking your lips as a small spurt of fluid squirts from your nethers.' );
				}
			}
			if( CoC.player.vaginas.length > 1 ) {
				if( CoC.player.lib < 45 ) {
					MainView.outputText( 'You touch and play with your many folds, ' );
				} else if( CoC.player.lib < 70 ) {
					MainView.outputText( 'You slap your pussies softly, ' );
				} else {
					MainView.outputText( 'Touch your enflamed and aroused ' + CoC.player.vaginaDescript() + 's, ' );
				}
				switch( CoC.player.vaginas[ 0 ].vaginalWetness ) {
					case AppearanceDefs.VAGINA_WETNESS_DRY:
						MainView.outputText( 'expertly arousing your female parts.  ' );
						break;
					case AppearanceDefs.VAGINA_WETNESS_NORMAL:
						MainView.outputText( 'sighing as they quickly becomes moist.  ' );
						break;
					case AppearanceDefs.VAGINA_WETNESS_WET:
						MainView.outputText( 'giggling as your fingers get a little wet.  ' );
						break;
					case AppearanceDefs.VAGINA_WETNESS_SLICK:
						...
						break;
					case AppearanceDefs.VAGINA_WETNESS_DROOLING:
						MainView.outputText( 'slicking your fingers in the juices that constantly dribble from ' + CoC.player.vaginaDescript() + 's  ' );
						break;
					default:
						MainView.outputText( 'licking your lips as small spurts of fluid squirt from your nethers.' );
				}
			}
		}
		/*******************************
		...
		if( CoC.player.cocks.length === 1 ) {
			...
			if( CoC.player.cocks[ 0 ].cockThickness < 1.8 ) {
				MainView.outputText( 'You easily wrap a hand around your ' + CoC.player.cockDescript() + ' and start masturbating.  ' );
			} else if( CoC.player.cocks[ 0 ].cockThickness < 3 ) {
				MainView.outputText( 'You have some difficulty fitting your hand around your ' + CoC.player.cockDescript() + ', relishing the feelings of your ' );
				if( CoC.player.cocks[ 0 ].cockType === CockTypesEnum.HORSE ) {
					MainView.outputText( 'animalistic' );
				} else if( CoC.player.hasKnot() ) {
					MainView.outputText( 'bulbous beast' );
				} else {
					MainView.outputText( 'large' );
				}
				MainView.outputText( ' endowment as you begin masturbating.  ' );
			} else if( CoC.player.cocks[ 0 ].cockThickness < 5 ) {
				MainView.outputText( 'You use both hands to grip your ' + CoC.player.cockDescript() + ', feeling the throbbing of your ' + (CoC.player.hasKnot() ? 'knot' : 'member') + ' as you begin to masturbate.  ' );
			} else {
				MainView.outputText( 'You grasp onto your ' + CoC.player.cockDescript() + ' with both hands, but fail to encircle it fully as you begin to masturbate.  ' );
			}
			//If length > 12 proud
			...
			if( CoC.player.cocks[ 0 ].cockLength < 12 ) {
				if( CoC.player.normalCocks() >= 1 ) {
					MainView.outputText( 'You stroke quickly, pleasuring your sensitive dick, darting down to fondle the base of your cock.  ' );
				} else if( CoC.player.horseCocks() >= 1 ) {
					MainView.outputText( 'You stroke quickly, reveling in your sensitive horseflesh, darting down to fondle your sensitive sheath.  ' );
				} else if( CoC.player.dogCocks() >= 1 ) {
					MainView.outputText( 'You stroke quickly, pleasuring your sensitive, canine erection, darting down to fondle the sensitive sheath at the base of your cock.  ' );
				} else if( CoC.player.tentacleCocks() >= 1 ) {
					MainView.outputText( 'You stroke quickly, pleasuring the supple length of your tentacular endowment, fondling every inhuman nodule as you slide along every inch of twisting length.  ' );
				} else if( CoC.player.demonCocks() >= 1 ) {
					MainView.outputText( 'You stroke quickly, pleasuring the bumpy ridges of your demonic tool, fondling every inhuman nodule as you slide along the entire twitching length.  ' );
				} else if( CoC.player.catCocks() >= 1 ) {
					MainView.outputText( 'You stroke quickly, feeling the tiny \'barbs\' of your ' + CoC.player.cockDescript() + ' sliding through your fingers, even darting down to circle the sensitive skin around your sheath.  ' );
				} else if( CoC.player.lizardCocks() >= 1 ) {
					MainView.outputText( 'You stroke quickly, pleasuring your sensitive ' + CoC.player.cockDescript() + ', sliding fingers over each ridge and bump that covers its knotty length.  ' );
				} else if( CoC.player.anemoneCocks() >= 1 ) {
					MainView.outputText( 'You stroke quickly, gasping as your fingers are stung repeatedly with the aphrodisiac-laced tentacles around the base of your ' + CoC.player.cockDescript() + ' and under its crown.  ' );
				} else if( CoC.player.displacerCocks() >= 1 ) {
					MainView.outputText( 'You stroke quickly, pleasuring your sensitive, alien endowment, darting down to fondle the sensitive sheath as your pointed tip opens into a wiggling, starfish-like shape.  ' );
				} else {
					MainView.outputText( 'You stroke quickly, pleasuring your sensitive dick, darting down to fondle the base of your cock.  ' );
				}
			} else if( CoC.player.cocks[ 0 ].cockLength < 20 ) {
				if( CoC.player.normalCocks() >= 1 ) {
					MainView.outputText( 'You delight in teasing the crown of your ' + CoC.player.cockDescript() + ', rubbing it at the end of each stroke, squeezing out dollops of pre to smear over it and tease yourself with.  It seems to pulse and twitch with each stroke, responding to every touch.  ' );
				} else if( CoC.player.horseCocks() >= 1 ) {
					MainView.outputText( 'You delight in teasing the sensitive flared tip of your ' + CoC.player.cockDescript() + ', rubbing it at the end of each stroke, squeezing out dollops of pre to smear over it and tease yourself with.  It seems to pulse and ripple with each stroke, responding to every touch.  ' );
				} else if( CoC.player.dogCocks() >= 1 ) {
					MainView.outputText( 'You delight in teasing the pointed tip of your ' + CoC.player.cockDescript() + ', rubbing it at the end of each stroke, squeezing out dollops of pre to smear over it and tease yourself with.  Your knot seems to pulse and twitch with each stroke, reacting to every touch.  ' );
				} else if( CoC.player.tentacleCocks() >= 1 ) {
					MainView.outputText( 'You delight in teasing the over-sized mushroom-like tip of your ' + CoC.player.cockDescript() + ', caressing it after every stroke as you squeeze out dollops of pre to smear along its slimy length.  It writhes and twists in your hands of it\'s own volition, lengthening and shortening with each set of strokes.  ' );
				} else if( CoC.player.demonCocks() >= 1 ) {
					MainView.outputText( 'You delight in teasing the larger bumps that form a ring around the crown of your ' + CoC.player.cockDescript() + ', watching as they twitch and spasm in time with the dollops of pre you\'re squeezing out and smearing over the entire length.  ' );
				} else if( CoC.player.catCocks() >= 1 ) {
					MainView.outputText( 'You delight in teasing the sensitive nubs all along your ' + CoC.player.cockDescript() + ', circling the cock-tip at the end of each stroke to gather pre and slather it over your entire length.  Each of the tiny \'barbs\' provides bursts of pleasure with each stroke, driving you on.  ' );
				} else if( CoC.player.lizardCocks() >= 1 ) {
					MainView.outputText( 'You delight in teasing the rounded bulbs that cover your ' + CoC.player.cockDescript() + ', circling them with your fingertips before sliding up to the urethra and gathering a drop of pre.  You smear it over your sensitive reptile skin and revel in the pleasure radiating through you.  ' );
				} else if( CoC.player.anemoneCocks() >= 1 ) {
					...
					MainView.outputText( 'You delight in teasing the opened tip of your ' + CoC.player.cockDescript() + ', rubbing it at the end of each stroke, watching it squeeze out dollops of pre that you smear over it to tease yourself with.  Your knot seems to pulse and twitch with each stroke, reacting to every touch.  ' );
				} else {
					MainView.outputText( 'You delight in teasing the crown of your ' + CoC.player.cockDescript() + ', rubbing it at the end of each stroke, squeezing out dollops of pre to smear over it and tease yourself with.  It seems to pulse and twitch with each stroke, responding to every touch.  ' );
				}
			} else if( CoC.player.cocks[ 0 ].cockLength < 26 ) {
				if( CoC.player.normalCocks() >= 1 ) {
					MainView.outputText( 'The head of your ' + CoC.player.cockDescript() + ' wobbles towards your face as you masturbate, a dollop of pre slowly growing atop it.  ' );
				} else if( CoC.player.horseCocks() >= 1 ) {
					MainView.outputText( 'The flared tip of your ' + CoC.player.cockDescript() + ' wobbles towards your face as you masturbate, a dollop of pre slowly growing atop it.  ' );
				} else if( CoC.player.dogCocks() >= 1 ) {
					MainView.outputText( 'The pointed tip of your ' + CoC.player.cockDescript() + ' angles towards your face as you masturbate, a dollop of pre slowly leaking from it.  ' );
				} else if( CoC.player.tentacleCocks() >= 1 ) {
					MainView.outputText( 'The overly wide head of your ' + CoC.player.cockDescript() + ' bumps against your lips as you masturbate, waving back and forth like a snake as it searches for an orifice.  ' );
				} else if( CoC.player.demonCocks() >= 1 ) {
					MainView.outputText( 'The purplish head of your ' + CoC.player.cockDescript() + ' bumps against your lips as you masturbate, flushing darkly with every beat of your heart.  ' );
				} else if( CoC.player.catCocks() >= 1 ) {
					MainView.outputText( 'The slightly pointed tip of your ' + CoC.player.cockDescript() + ' bumps against your lips as you masturbate, flushing with blood as your spines grow thicker in your hand.  ' );
				} else if( CoC.player.lizardCocks() >= 1 ) {
					MainView.outputText( 'The pointed, purple tip of your ' + CoC.player.cockDescript() + ' bumps against your lips while its knotted surface flushes near-purple and seems to grow thicker.  ' );
				} else if( CoC.player.anemoneCocks() >= 1 ) {
					MainView.outputText( 'The tentacle-ringed tip of your ' + CoC.player.cockDescript() + ' brushes against your lips, making them tingle with artificial heat while you stroke it.  ' );
				} else if( CoC.player.displacerCocks() >= 1 ) {
					MainView.outputText( 'The blooming tip of your ' + CoC.player.cockDescript() + ' angles towards your face as you masturbate, a dollop of pre slowly leaking out of the outstretched top.  ' );
				} else {
					MainView.outputText( 'The head of your ' + CoC.player.cockDescript() + ' wobbles towards your face as you masturbate, a dollop of pre slowly growing atop it.  ' );
				}
				...
				if( CoC.player.hasFuckableNipples() && CoC.player.biggestTitSize() >= 3 ) {
					...
					MainView.outputText( 'You give in to temptation and swallow the tip, slurping greedily as you milk your ' + CoC.player.cockDescript() + ' of its pre-cum.  ' );
					autofellatio = true;
				}
				if( CoC.player.canTitFuck() && CoC.player.biggestTitSize() > 3 && !nippleFuck ) {
					MainView.outputText( 'Your hands migrate to your breasts of their own accord, wrapping your titflesh around your ' + CoC.player.cockDescript() + ', jacking it up and down in your pillowy tits.  ' );
					if( CoC.player.biggestLactation() > 0 ) {
						MainView.outputText( 'Jets of milk squirt out of your nipples with each thrust of your hips, adding to the already copious amounts of fluids coating your body. ' );
					}
				}
			} else {
				if( CoC.player.normalCocks() >= 1 ) {
					MainView.outputText( 'The head of your ' + CoC.player.cockDescript() + ' wobbles over, bumping your face and smearing your lips with its copious pre-cum.  You are unable to resist opening your mouth and sucking it down, filling your mouth with your cock.  ' );
				} else if( CoC.player.horseCocks() >= 1 ) {
					MainView.outputText( 'The flared tip of your ' + CoC.player.cockDescript() + ' wobbles over, bumping your face and smearing your lips with its copious pre-cum.  You are unable to resist opening your mouth and sucking it down, filling your mouth with horsemeat.  ' );
				} else if( CoC.player.dogCocks() >= 1 ) {
					MainView.outputText( 'The head of your ' + CoC.player.cockDescript() + ' wobbles over, bumping your face and smearing your lips with its copious pre-cum.  You are unable to resist opening your mouth and sucking it down, filling your mouth with your ' + CoC.player.cockDescript() + '.  ' );
				} else if( CoC.player.tentacleCocks() >= 1 ) {
					MainView.outputText( 'The bulbous, mushroom-like head of your ' + CoC.player.cockDescript() + ' pushes against your face eagerly, smearing pre-cum over your lips as it seeks entrance to the nearest orifice.  You\'re unable to resist opening your mouth to suck it down, filling your mouth with slick rubbery cock-tentacle.  ' );
				} else if( CoC.player.demonCocks() >= 1 ) {
					MainView.outputText( 'The tainted swollen head of your ' + CoC.player.cockDescript() + ' pushes against your face, smearing sweet pre-cum over your lips.  You\'re unable to resist opening wide and taking in the demonic member, submitting wholly to the desire to pleasure your corrupted body-parts.  ' );
				} else if( CoC.player.catCocks() >= 1 ) {
					MainView.outputText( 'The pointed tip of your ' + CoC.player.cockDescript() + ' pushes against your face, smearing pre-cum over your lips and tickling you with the many barbs.  You\'re unable to resist opening wide and taking in the ' + CoC.player.cockDescript() + ', half-humming half-purring in contentment.  ' );
				} else if( CoC.player.lizardCocks() >= 1 ) {
					MainView.outputText( 'The slightly-pointed tip of your ' + CoC.player.cockDescript() + ' pushes against your face, smearing pre-cum over your eager lips.   You can\'t resist opening wide and slipping it inside as your hands caress the reptilian bulges along your length.  ' );
				} else if( CoC.player.anemoneCocks() >= 1 ) {
					MainView.outputText( 'The rounded, tentacle-ringed tip of your ' + CoC.player.cockDescript() + ' slides against your face, smearing pre-cum over your lips and stinging them with aphrodisiacs that make you pant with lust.  You can\'t resist opening wide to greedily slurp it down, and in seconds tiny, tingling stings are erupting through your oral cavity, filling you with lust and pleasure.  ' );
				} else if( CoC.player.displacerCocks() >= 1 ) {
					MainView.outputText( 'The head of your ' + CoC.player.cockDescript() + ' wobbles over, bumping your face and smearing your lips with its copious pre-cum.  Wiggling against your lips, the various protrusions of the \'star\' at the tip smear you with your heady emissions.   You are unable to resist opening your mouth and sucking it down, filling your mouth with your ' + CoC.player.cockDescript() + '.  ' );
				} else {
					MainView.outputText( 'The head of your ' + CoC.player.cockDescript() + ' wobbles over, bumping your face and smearing your lips with its copious pre-cum.  You are unable to resist opening your mouth and sucking it down, filling your mouth with your cock.  ' );
				}
				//try to stick it in a titty!
				if( CoC.player.hasFuckableNipples() && CoC.player.biggestTitSize() >= 3 ) {
					...
					if( CoC.player.cor > 60 ) {
						if( CoC.player.normalCocks() >= 1 ) {
							MainView.outputText( 'The heady aroma of your cock fills your nostrils as you force inch after inch into your mouth, deepthroating as much of yourself as you can.  ' );
						} else if( CoC.player.horseCocks() >= 1 ) {
							MainView.outputText( 'The thick animalistic scent fills your nostrils as you force inch after inch into your mouth, deepthroating as much of yourself as you can.  ' );
						} else if( CoC.player.dogCocks() >= 1 ) {
							MainView.outputText( 'The strong animal scent of your cock fills your nostrils as you force inch after inch into your mouth, deepthroating as much of yourself as you can.  ' );
						} else if( CoC.player.tentacleCocks() >= 1 ) {
							MainView.outputText( 'The sweet scent of your ' + CoC.player.cockDescript() + ' fills your nostrils as inch after inch of tentacle forces its way down your throat.  You struggle briefly with it, wrestling to keep it from pushing the whole way into your gut, swooning with the pleasure that fighting with your own cock induces.  ' );
						} else if( CoC.player.demonCocks() >= 1 ) {
							MainView.outputText( 'The spicy demonic odor your ' + CoC.player.cockDescript() + ' radiates fills your nostrils as you force inch after inch of tainted meat down your throat.  You struggle briefly, but your ' + CoC.player.cockDescript() + ' quickly overwhelms your resistance and your gag reflex momentarily seems to disappear.  ' );
						} else if( CoC.player.catCocks() >= 1 ) {
							MainView.outputText( 'The sweet, slightly tangy scent of your cock fills your nostrils as you force inch after inch into your mouth, deepthroating as much of the prickly shaft as you can.  The soft spines that coat its surface actually feel quite pleasant as you force it deeper, denying your gag reflex.  ' );
						} else if( CoC.player.lizardCocks() >= 1 ) {
							MainView.outputText( 'The salty, dry odor of your ' + CoC.player.cockDescript() + ' fills your nostrils as you force inch after inch into your mouth, swallowing as much of the bulgy shaft as you can.  ' );
						} else if( CoC.player.anemoneCocks() >= 1 ) {
							MainView.outputText( 'The stinging, aphrodisiac-laced ' + CoC.player.cockDescript() + ' slowly works its way down your throat as you swallow it deeper and deeper, gurgling happily as your throat grows more sensitive.  ' );
						} else if( CoC.player.displacerCocks() >= 1 ) {
							MainView.outputText( 'The strong animal scent of your cock fills your nostrils as you force inch after inch into your mouth, deepthroating as much of yourself as you can.  ' );
						} else {
							MainView.outputText( 'The heady aroma of your cock fills your nostrils as you force inch after inch into your mouth, deepthroating as much of yourself as you can.  ' );
				...
				if( CoC.player.canTitFuck() && CoC.player.biggestTitSize() > 3 && !nippleFuck ) {
					MainView.outputText( 'Your hands reach up to your pillowy breasts, wrapping them around the shaft of your ' + CoC.player.cockDescript() + ', causing you to let out muffled moans of excitment. ' );
					if( CoC.player.biggestLactation() > 0 ) {
						MainView.outputText( 'Jets of milk squirt out of your nipples with each thrust of your hips, adding to the already copious amounts of fluids coating your body. ' );
					}
				}
			...
			if( CoC.player.hasSheath() ) {
				MainView.outputText( 'Your fingers trace around your sheath, drawing involuntary spasms from your twin members.  ' );
			}
			//Prep bit + variance based on thickness
			MainView.outputText( 'You grasp one of your ' + CoC.player.multiCockDescriptLight() + ' in each hand, ' );
			if( CoC.player.averageCockThickness() <= 1.8 ) {
				MainView.outputText( 'wrapping your fingers around each tool and just holding yourself, the feelings of two cocks almost too much to bear.  ' );
			} else {
				MainView.outputText( 'squeezing as many of your tools as your fingers can encircle.  The feeling of squeezing your thick twin cocks is amazing and overwhelming at the same time.  ' );
			}
			//Get into it
			//Play with it if <= 10'
			if( CoC.player.averageCockLength() <= 10 ) {
				...
				MainView.outputText( 'You look down at your trembling members, mere inches away, and tentatively lick at a cocktip.  Both dicks begin oozing pre in response.  ' );
				//Try to stick it in a titty!
				...
				if( CoC.player.canTitFuck() && CoC.player.mostBreastsPerRow() > 2 && !nippleFuck ) {
					...
					if( CoC.player.biggestLactation() > 0 ) {
						MainView.outputText( 'Jets of milk squirt out of your many nipples with each thrust of your hips, adding to the already copious amounts of fluids coating your body. ' );
					}
				}
				//No tits, jerk and lick!
				if( CoC.player.biggestTitSize() <= 3 && !nippleFuck ) {
					MainView.outputText( 'You slowly stroke yourself, pausing every few strokes to smear the streaks of pre that leak from your ' + CoC.player.multiCockDescriptLight() + ' over yourself.  As you become more and more lubed and horny, you increase the tempo, until at last you are pumping yourself hard and fast, squeezing your cocks together in pleasure.  ' );
				}
			}
			//Deepthroat yourself, maybe titfuck if equipped.
			else {
				//autofellatio only gets to be true if you have any dicks left to suck on after sticking them in your boobs
				MainView.outputText( 'Your ' + CoC.player.multiCockDescriptLight() + ' brush against your face, throbbing with need.   You open your mouth and suck one of your pricks into your mouth, taking as much of it as possible, running your tongue along the underside.  You smile and pop it free, then take a different shaft into your maw.  ' );
				//Try to stick it in a titty!
				if( CoC.player.hasFuckableNipples() ) {
					//check for dogcocks
					if( CoC.player.dogCocks() > 0 ) {
			...
			//Play with sheath if has one
			if( CoC.player.hasSheath() ) {
				MainView.outputText( 'Your fingers trace around your sheath, drawing involuntary spasms from your cocks.  ' );
			}
			//Prep bit + variance based on thickness
			MainView.outputText( 'You grasp one of your ' + CoC.player.multiCockDescriptLight() + ' in each hand, ' );
			if( CoC.player.averageCockThickness() <= 1.8 ) {
				MainView.outputText( 'wrapping your fingers around each tool and just holding yourself, the feelings of your ' + Utils.num2Text( CoC.player.cocks.length ) + ' cocks making you feel giddy.  ' );
			} else {
				MainView.outputText( 'squeezing as many of your tools as your fingers can encircle.  The feeling of rubbing your bundle of cocks together is indescribable.  Your eyes roll back in uncontrollable pleasure.  ' );
			}
			//Get into it
			//Play with it if <= 10'
			if( CoC.player.averageCockLength() <= 10 ) {
				...
				//autofellatio only gets to be true if you have any dicks left to suck on after sticking them in your boobs
				MainView.outputText( 'You look down at your trembling members, mere inches away, and tentatively lick at a cocktip.  Your dicks begin oozing pre in response.  ' );
				//Try to stick it in a titty!
				if( CoC.player.hasFuckableNipples() ) {
					//check for dogcocks
					if( CoC.player.dogCocks() > 0 ) {
			...
			if( CoC.player.averageCockLength() >= 20 ) {
				//autofellatio only gets to be true if you have any dicks left to suck on after sticking them in your boobs
				MainView.outputText( 'Your ' + CoC.player.multiCockDescriptLight() + ' brush against your face, throbbing with need.   You open your mouth and suck one of your pricks into your mouth, taking as much of it in as possible, running your tongue along the underside.  You smile and pop it free, then take a different shaft into your maw.  ' );
				//Try to stick it in a titty!
				if( CoC.player.hasFuckableNipples() ) {
					//check for dogcocks
					if( CoC.player.dogCocks() > 0 ) {
		...
		if( CoC.player.vaginas.length > 0 ) {
			...
			//Single cunt
			if( CoC.player.vaginas.length === 1 ) {
				MainView.outputText( 'You let your hand roam over your pussy-lips, slowly teasing yourself, diving deeper into your folds to arouse and expose your clit.  ' );
				//Smaller clits
				if( CoC.player.clitLength < 1.5 ) {
					MainView.outputText( 'You stroke and tease around the sensitive little pleasure bud, letting your fingers plumb the depths below.  ' );
				}
				//Big clits
				else if( CoC.player.clitLength < 4.5 ) {
					MainView.outputText( 'Your large clit is already poking out from your ' );
					if( CoC.player.vaginas[ 0 ].vaginalWetness > AppearanceDefs.VAGINA_WETNESS_DRY ) {
						MainView.outputText( 'glistening ' );
					}
					MainView.outputText( 'lips.  You gently stroke and touch it until it grows as large as a tiny cock.  ' );
				}
				//Cock-sized clits
				else {
					MainView.outputText( 'Your cock-sized clit is already fully engorged and deliciously sensitive.  You touch it softly, eliciting a quiet moan from your throat.  ' );
				}
				switch( CoC.player.vaginas[ 0 ].vaginalWetness ) {
					case AppearanceDefs.VAGINA_WETNESS_DRY:
						MainView.outputText( 'You have some difficulty with your relatively dry pussy, but you manage to gently and pleasurably masturbate by taking it slowly.  ' );
						break;
					case AppearanceDefs.VAGINA_WETNESS_NORMAL:
						MainView.outputText( 'Your horny puss is aching for attention and you oblige it, dipping your fingers into the moist honeypot, and jilling yourself vigorously.  ' );
						break;
					case AppearanceDefs.VAGINA_WETNESS_WET:
						MainView.outputText( 'The moistened cleft on your groin demands your full attention, drawing your fingers deep inside to explore the wet passage.  ' );
						break;
					case AppearanceDefs.VAGINA_WETNESS_SLICK:
						MainView.outputText( 'The sweltering heat of your slick cunt aches for something to fill it, and you oblige, dipping your fingers into your slippery cunt.  ' );
						break;
					case AppearanceDefs.VAGINA_WETNESS_DROOLING:
						MainView.outputText( 'Warm wetness runs down your legs in thick streams, pouring from your ' + CoC.player.vaginaDescript() + '.  You smile sheepishly and stroke up your now slickened legs to your pussy-lips, parting them and letting your fingers dive inside the wet channel.  ' );
						break;
					default:
						MainView.outputText( 'The heavy scent of female arousal fills the air as your steamy sexpot drizzles girl-lube everywhere.  You gasp in surprise as your fingers find their way inside, vigorously fingerfucking your passage, spurts of girlcum squirting out with each penetration.  ' );
				}
			}
		}
		//ORGASM GOES HERE
		...
		if( CoC.player.cocks.length > 0 ) {
			...
			if( CoC.player.cocks.length === 1 ) {
				if( CoC.player.lib < 30 ) {
					MainView.outputText( 'You quickly fall asleep, spent. ' );
				} else if( CoC.player.lib < 55 ) {
					MainView.outputText( 'You roll and begin to doze, your semi-erect ' + CoC.player.cockDescript() + ' flopping against you. ' );
				} else if( CoC.player.lib <= 80 ) {
					MainView.outputText( 'As you close your eyes and relax, your ' + CoC.player.cockDescript() + ' surges back to erectness, ensuring ' );
					if( CoC.player.cor < 50 ) {
						MainView.outputText( 'your dreams will be filled with sex.' );
					} else {
						MainView.outputText( 'you dream in a depraved kinky fantasia.' );
					}
				} else {
					MainView.outputText( 'You groan and drift to sleep, your rigid ' + CoC.player.cockDescript() + ' pulsing and throbbing with continual lust.' );
				}
			}
			//Multi Cock
			else {
				if( CoC.player.lib < 30 ) {
					...
					MainView.outputText( 'You roll and begin to doze for an hour, your semi-erect cocks flopping against you. ' );
				} else if( CoC.player.lib <= 80 ) {
					MainView.outputText( 'As you close your eyes and relax, your dicks surge back to erectness, ensuring ' );
					if( CoC.player.cor < 50 ) {
						MainView.outputText( 'your dreams will be filled with sex.' );
					} else {
						MainView.outputText( 'you dream in a depraved kinky fantasia.' );
					...
					MainView.outputText( 'You groan and drift into a brief catnap, your rigid erections pulsing and throbbing with continual lust.' );
				}
			}
		}
		//No cock ending
			...
			if( CoC.player.vaginas.length > 0 ) {
				MainView.outputText( 'You sigh softly and drift off into a quick nap, smelling of sex.' );
			}//Girl ending
			else {
				MainView.outputText( 'You sigh and drift off to sleep.' );
			} //Genderless ending
		}
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	...
	Masturbation.prototype.genderlessMasturbate = function() {
		...
		if( CoC.player.cor < 15 ) {
			...
			if( CoC.player.hasCock() || CoC.player.hasVagina() ) {
				...
				MainView.outputText( 'You strip naked, casting seductive looks around, hoping someone or something is nearby to fuck you.\n\n' );
			}
		}
		//Tit foreplay
		this.titForeplay();
		...
		MainView.outputText( 'Your ' + Descriptors.assholeDescript() + ' begins to twitch. It\'s practically crying out for attention.\n\n' );
		MainView.outputText( 'You lay down on your side and reach gingerly behind yourself.  The palm of your hand comes to rest on your ' + CoC.player.buttDescript() + '.  You slide your finger into your crack and find your ' + Descriptors.assholeDescript() + '.  You run your finger slowly around the sensitive ring of your hole and feel a tingle emanating from it. A smile creeps across your lips as you begin to imagine what is about to happen.\n\n' );
		...
		if( CoC.player.ass.analWetness < 2 ) {
			MainView.outputText( 'Bringing your hand up to your mouth, you coat your ' );
			if( CoC.player.ass.analLooseness <= 2 ) {
				MainView.outputText( 'middle finger' );
			} else if( CoC.player.ass.analLooseness === 3 ) {
				MainView.outputText( 'first two fingers' );
			} else {
				MainView.outputText( 'hand' );
			}
			MainView.outputText( ' in a generous helping of saliva and head back for your ' + Descriptors.assholeDescript() + '.\n\n' );
		}
		//If BioLube or continuing from no biolube -
		else {
			MainView.outputText( 'The lubrication you have created allows you to easily sink your finger' );
			if( CoC.player.ass.analLooseness >= 3 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ' into your asshole. A shiver runs up your spine as you plunge ' );
			if( CoC.player.ass.analLooseness <= 2 ) {
				MainView.outputText( 'in all the way to your knuckle' );
			} else if( CoC.player.ass.analLooseness === 3 ) {
				MainView.outputText( 'all the way to the first two knuckles' );
			} else if( CoC.player.ass.analLooseness === 4 ) {
				MainView.outputText( 'your three knuckles' );
			} else {
				MainView.outputText( 'your four fingers in deep' );
			}
			MainView.outputText( '. Slowly you begin to push and pull your finger' );
			if( CoC.player.ass.analLooseness >= 3 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ' in and out of your anus. A slight moan escapes you as you begin to pick up pace.\n\n' );
		}
		//If gaping -
		if( CoC.player.ass.analLooseness === 5 ) {
			MainView.outputText( 'A devilish thought crosses your mind. You have taken into yourself all manner of beasts and beings. There is only one real way to achieve the pleasure you have gotten from them on your own. You slowly force your whole hand into your ' + Descriptors.assholeDescript() + ' and are greeted with a fullness that you never thought you would achieve without assistance. As you move in and out you begin to slowly close your hand into a fist and open it up again over and over.\n\n' );
		}
		//All scene types -
		MainView.outputText( 'Pleasure begins to fill your body with warmth. You deliberately start to twist your hand as you pump your pleasure hole with a deep desire. Your asshole begins to violently open and close around your invading ' );
		if( CoC.player.ass.analLooseness <= 2 ) {
			MainView.outputText( 'digit' );
		} else if( CoC.player.ass.analLooseness < 5 ) {
			MainView.outputText( 'digits' );
		} else {
			MainView.outputText( 'hand' );
		}
		MainView.outputText( ' as your toes curl and an orgasm wracks your body.' );
		//If BioLube perk -
		if( CoC.player.ass.analWetness >= 2 ) {
			MainView.outputText( '  You withdraw your ' );
			if( CoC.player.ass.analLooseness <= 2 ) {
				MainView.outputText( 'finger' );
			} else if( CoC.player.ass.analLooseness < 5 ) {
				MainView.outputText( 'fingers' );
			} else {
				MainView.outputText( 'hand' );
			}
			MainView.outputText( ' and see it coated in the warm lube you produce. The scent and ecstasy you are in drive you over the edge and you begin to lick what once was inside you clean. Another orgasm drills through you and your body shakes for several seconds.' );
			//Still Horny -
		...
		//No BioLube perk -
			...
			MainView.outputText( ' off.' );
			//Still Horny -
			if( CoC.player.lib > 75 ) {
				MainView.outputText( '  Satisfied, you roll over and drift off to sleep. Your hole remains warm, ready for another round.' );
			} else {
				MainView.outputText( '  Satisfied, you roll over and drift off to sleep.' );
			}
		}
	};
	Masturbation.prototype.titForeplay = function() {
		...
		if( CoC.player.breastRows.length === 1 && CoC.player.biggestTitSize() > 3 ) {
			if( CoC.player.lib < 45 ) {
				MainView.outputText( 'You caress your ' + CoC.player.breastDescript( 0 ) + ' gently with your fingers' );
			} else if( CoC.player.lib < 70 ) {
				MainView.outputText( 'You grope your ' + CoC.player.breastDescript( 0 ) + ' agressively with both hands' );
			} else {
				MainView.outputText( 'You squeeze your ' + CoC.player.breastDescript( 0 ) + ' brutally with both hands' );
			}
		} else if( CoC.player.breastRows.length > 1 && CoC.player.biggestTitSize() > 3 ) {
			if( CoC.player.lib < 45 ) {
				MainView.outputText( 'You hesitantly run your hands across your front, caressing each of your ' + Utils.num2Text( CoC.player.totalBreasts() ) + ' breasts in turn. ' );
			} else if( CoC.player.lib < 70 ) {
				MainView.outputText( 'You run your hands across your front, squeezing each of your ' + Utils.num2Text( CoC.player.totalBreasts() ) + ' breasts in turn. ' );
			} else {
				MainView.outputText( 'You aggressively run your hands across your front, groping each of your ' + Utils.num2Text( CoC.player.totalBreasts() ) + ' breasts in turn. ' );
			}
			...
			if( CoC.player.cor <= 25 ) {
				MainView.outputText( 'You blink back tears of shame as you experience the tactile proof of your abnormal endowments, unable to resist mashing your ' + CoC.player.allBreastsDescript() + ' together' );
			} else if( CoC.player.cor < 75 ) {
				MainView.outputText( 'Though a small part of you is horrified by your abnormal endowments, it doesn\'t stop you from mashing your ' + CoC.player.allBreastsDescript() + ' together' );
			} else {
				MainView.outputText( 'The tactile reminder of your abnormal endowments gives you a delicious thrill.  You sigh and mash your tits together' );
			}
		}
		...
		if( CoC.player.hasFuckableNipples() && CoC.player.biggestTitSize() > 3 ) {
			...
			if( CoC.player.breastRows.length >= 1 ) {
				MainView.outputText( ' as you move your attention to your nipples' );
			}
			//Different attitude based on libido
			if( CoC.player.lib < 60 ) {
				MainView.outputText( '. Hesitantly, you rub your fingers across the ' );
			} else {
				MainView.outputText( '. Eagerly, you begin to roughly stroke the ' );
			}
			//Different description based on vag looseness
			if( CoC.player.averageVaginalLooseness() < 2 ) {
				MainView.outputText( 'small, delicate openings at the tip of each nipple. ' );
			} else if( CoC.player.averageVaginalLooseness() < 4 ) {
				MainView.outputText( 'puckered holes at the tip of each nipple. ' );
			}//add 'many if lots of nips
			else if( CoC.player.averageNipplesPerBreast() > 1 ) {
				MainView.outputText( 'many ' );
			} else {
				MainView.outputText( 'swollen, cunt-like lips that top each breast. ' );
			}
			//Different noise based on sensitivity
			if( CoC.player.sens < 45 ) {
				MainView.outputText( 'You sigh contentedly as ' );
			} else if( CoC.player.sens < 70 ) {
				MainView.outputText( 'You moan lewdly as ' );
			} else {
				MainView.outputText( 'You squeal with pleasure as ' );
			}
			//Different description based on wetness
			if( CoC.player.averageVaginalWetness() < 2 ) {
				MainView.outputText( 'you feel your nipples loosen up. ' );
			} else if( CoC.player.averageVaginalWetness() < 4 ) {
				MainView.outputText( 'you begin to slowly spread the slippery secretions leaking from your engorged nipples all over your ' + CoC.player.breastDescript( 0 ) + '. ' );
			} else {
				MainView.outputText( 'little spurts of ' );
				//Lactating?
				if( CoC.player.averageLactation() > 0 ) {
					MainView.outputText( ' milky ' );
				}
				MainView.outputText( 'girlcum squirt out of your engorged nipples. ' );
			}
			//Special extras for lactation
			if( CoC.player.averageLactation() > 0 && CoC.player.averageLactation() < 2 ) {
				MainView.outputText( 'Your freakish mammaries drip a sticky combination of cunt-juice and milk, spattering your ' + CoC.player.legs() + ' and crotch.' );
			} else if( CoC.player.averageLactation() < 3 ) {
				MainView.outputText( 'Your freakish mammaries leak thin streams of sticky girlcum mixed with milk all over your ' + CoC.player.legs() + ' and crotch.' );
			} else {
				MainView.outputText( 'Your freakish mammaries constantly drool a steady stream of milky cunt-juice, drenching your ' + CoC.player.legs() + ' and crotch in milky goo.' );
			}
		}
		//If nipples aren't fuckable.
		else if( CoC.player.breastRows.length > 0 && CoC.player.biggestTitSize() > 3 ) {
			MainView.outputText( ', emitting ' );
			//different noises depending on sensitivity
			if( CoC.player.sens < 45 ) {
				MainView.outputText( 'soft gasps of pleasure each time you flick one of your ' );
			} else if( CoC.player.sens < 70 ) {
				MainView.outputText( 'loud moans of pleasure each time you flick one of your ' );
			} else {
				MainView.outputText( 'squeals of pleasure each time you flick one of your ' );
			}
			...
			if( CoC.player.lust >= 50 ) {
				if( CoC.player.biggestLactation() > 2 ) {
					MainView.outputText( 'huge, swollen nipples. ' );
				} else if( CoC.player.biggestLactation() > 1 ) {
					MainView.outputText( 'fat, puckered nipples. ' );
				} else {
					MainView.outputText( 'erect nipples. ' );
				}
			} else {
				MainView.outputText( 'nipples.  ' );
			}
			//Special extras if lactating
			if( CoC.player.biggestLactation() > 1 && CoC.player.biggestLactation() < 2 ) {
				MainView.outputText( 'Droplets of milk dribble from each nipple, spattering milk onto your ' + CoC.player.legs() + ' and crotch.  ' );
			} else if( CoC.player.biggestLactation() < 3 ) {
				MainView.outputText( 'Thin squirts of milk spray from each nipple, spattering milk onto your ' + CoC.player.legs() + ' and crotch.  ' );
			} else if( CoC.player.biggestLactation() >= 3 ) {
				MainView.outputText( 'A constant stream of milk drizzles from each teat, soaking your ' + CoC.player.legs() + ' and crotch.  ' );
			}
		}
	};
	Masturbation.prototype.titFuckSingle = function() {
		if( CoC.player.lib < 45 ) {
			MainView.outputText( 'You grip your ' + CoC.player.cockDescript() + ' and begin cautiously guiding it towards ' );
		} else if( CoC.player.lib < 70 ) {
			MainView.outputText( 'Shivering with anticipation, you place the ' );
			switch( CoC.player.cocks[ 0 ].cockType ) { //Different way of choosing cock to use, results in consistent description of cock[0]
				...
				case CockTypesEnum.DISPLACER:
					MainView.outputText( 'wriggling ' );
					break;
				case CockTypesEnum.DEMON:
					MainView.outputText( 'bump-encircled ' );
					break;
				case CockTypesEnum.DOG:
				case CockTypesEnum.FOX:
					MainView.outputText( 'pointed ' );
					break;
				case CockTypesEnum.HORSE:
					MainView.outputText( 'flared ' );
					break;
				case CockTypesEnum.TENTACLE:
					MainView.outputText( 'bulbous ' );
			}
			MainView.outputText( 'tip of your ' + CoC.player.cockDescript() + ' against the opening of ' );
		} else {
			MainView.outputText( 'Without hesitation, you shove the ' );
			switch( CoC.player.cocks[ 0 ].cockType ) { //Different way of choosing cock to use, results in consistent description of cock[0]
				case CockTypesEnum.ANEMONE:
					MainView.outputText( 'tentacle-laden mushroom that is ' );
					break;
				case CockTypesEnum.DEMON:
					MainView.outputText( 'bump-ringed monstrosity that is ' );
					break;
				case CockTypesEnum.DISPLACER:
					MainView.outputText( 'opened top of ' );
					break;
				case CockTypesEnum.DOG:
				case CockTypesEnum.FOX:
					MainView.outputText( 'pointed tip of ' );
					break;
				case CockTypesEnum.HORSE:
					MainView.outputText( 'engorged flare of ' );
					break;
				case CockTypesEnum.HUMAN:
					MainView.outputText( 'tip of ' );
					break;
				case CockTypesEnum.TENTACLE:
					MainView.outputText( 'over-sized mushroom that is ' );
					break;
				default:
					MainView.outputText( CoC.player.cockHead() + ' of ' );
			}
			MainView.outputText( 'your ' + CoC.player.cockDescript() + ' into ' );
		}
		//More than one row?
		if( CoC.player.breastRows.length > 1 ) {
			MainView.outputText( 'one of the ' );
		} else {
		...
		if( CoC.player.averageVaginalLooseness() < 2 ) {
			MainView.outputText( 'painfully stretched nipples' );
		} else if( CoC.player.averageVaginalLooseness() < 4 ) {
			MainView.outputText( 'freakishly swollen nipples' );
		} else {
			MainView.outputText( 'huge, bloated cunt-nipples' );
		}
		if( CoC.player.breastRows.length > 1 ) {
			MainView.outputText( ' on one of your lower breasts' );
		}
		MainView.outputText( '. ' );
		...
		if( CoC.player.averageLactation() === 0 ) {
			if( CoC.player.averageVaginalWetness() < 2 ) {
				...
				MainView.outputText( 'Slick juices dribble ' );
				switch( CoC.player.cocks[ 0 ].cockType ) { //Different way of choosing cock to use, results in consistent description of cock[0]
					case CockTypesEnum.ANEMONE:
						MainView.outputText( 'over the nearly transparent skin of ' );
						break;
					case CockTypesEnum.CAT:
						MainView.outputText( 'over the pink, spiny protrusions that cover ' );
						break;
					case CockTypesEnum.DEMON:
						MainView.outputText( 'down the shiny purplish skin and nodules of ' );
						break;
					case CockTypesEnum.DISPLACER:
						MainView.outputText( 'over the dusky purple of ' );
						break;
					case CockTypesEnum.DOG:
					case CockTypesEnum.FOX:
						MainView.outputText( 'down the red, shiny skin of ' );
						break;
					case CockTypesEnum.HORSE:
						MainView.outputText( 'down the mottled skin of ' );
						break;
					case CockTypesEnum.LIZARD:
						MainView.outputText( 'down the bumpy purple skin of ' );
						break;
					case CockTypesEnum.TENTACLE:
						MainView.outputText( 'down the rubbery skin of ' );
						break;
					default:
						MainView.outputText( 'down the skin of ' );
				}
				MainView.outputText( 'your ' + CoC.player.cockDescript() + ' and y' );
			} else {
				MainView.outputText( 'Slick juices squirt out from around your ' + CoC.player.cockDescript() + ' and y' );
			}
		} else {
			if( CoC.player.averageLactation() < 2 ) {
				MainView.outputText( 'Rivulets of milky girlcum drizzle ' );
				switch( CoC.player.cocks[ 0 ].cockType ) { //Different way of choosing cock to use, results in consistent description of cock[0]
					case CockTypesEnum.ANEMONE:
						MainView.outputText( 'and drip from the many tiny tentacles of ' );
						break;
					case CockTypesEnum.CAT:
						MainView.outputText( 'down the spiny, pink flesh of ' );
						break;
					case CockTypesEnum.DEMON:
						MainView.outputText( 'down the shiny purplish skin and nodules of ' );
						break;
					case CockTypesEnum.DISPLACER:
						MainView.outputText( 'over the purplish, knotted flesh of ' );
						break;
					case CockTypesEnum.DOG:
					case CockTypesEnum.FOX:
						MainView.outputText( 'down the red, shiny skin of ' );
					...
					case CockTypesEnum.HUMAN:
						...
						break;
					case CockTypesEnum.LIZARD:
						MainView.outputText( 'over the purplish, bumpy flesh of ' );
						break;
					case CockTypesEnum.TENTACLE:
						MainView.outputText( 'down the rubbery skin of ' );
						break;
					default:
						MainView.outputText( 'over the sensitive skin of ' );
				}
				MainView.outputText( 'your ' + CoC.player.cockDescript() + ' and y' );
			} else {
				MainView.outputText( 'Milky girlcum squirts out from around your ' + CoC.player.cockDescript() + ', staining ' );
				switch( CoC.player.cocks[ 0 ].cockType ) { //Different way of choosing cock to use, results in consistent description of cock[0]
					case CockTypesEnum.ANEMONE:
						MainView.outputText( 'the odd aquatic shaft white.  Y' );
						break;
					case CockTypesEnum.CAT:
						MainView.outputText( 'the pink kitty-skin white.  Y' );
						break;
					case CockTypesEnum.DEMON:
						MainView.outputText( 'its purplish-skin white.  Y' );
						break;
					case CockTypesEnum.DISPLACER:
						MainView.outputText( 'purple, knotty flesh white.  Y' );
						break;
					case CockTypesEnum.DOG:
					case CockTypesEnum.FOX:
						MainView.outputText( 'its shiny skin white.  Y' );
						break;
					case CockTypesEnum.HORSE:
						MainView.outputText( 'its mottled skin white.  Y' );
						break;
					case CockTypesEnum.HUMAN:
						MainView.outputText( 'it white.  Y' );
						break;
					case CockTypesEnum.LIZARD:
						MainView.outputText( 'purple, bumpy flesh white.  Y' );
						break;
					case CockTypesEnum.TENTACLE:
						MainView.outputText( 'its rubbery skin white.  Y' );
						break;
					default:
						MainView.outputText( 'its length white.  Y' );
				}
			}
		}
		...
		if( Math.round( CoC.player.cockArea( 0 ) ) >= CoC.player.vaginalCapacity() ) {
			...
			if( CoC.player.sens < 45 ) {
				MainView.outputText( 'ou grunt with exertion as you attempt to stuff your ' + CoC.player.cockDescript() + ' into ' );
			} else if( CoC.player.sens < 70 ) {
				MainView.outputText( 'ou blink back tears as you attempt to stuff your ' + CoC.player.cockDescript() + ' into ' );
			} else {
				MainView.outputText( 'ou scream with a combination of pain and pleasure as you attempt to stuff your ' + CoC.player.cockDescript() + ' into ' );
			}
			//Different based on looseness
			if( CoC.player.averageVaginalLooseness() < 2 ) {
				MainView.outputText( 'the small, over-stretched opening of your swollen nipple. ' );
			} else if( CoC.player.averageVaginalLooseness() < 4 ) {
				MainView.outputText( 'the engorged and distended opening of your fat, swollen nipple. ' );
			} else {
				MainView.outputText( 'the gaping fuck-mouth of your inhuman nipple-cunt. ' );
			}
			//Compare cockthickness and vaglooseness more specifically
			//if it barely fits
			if( Math.round( CoC.player.cockArea( 0 ) ) === CoC.player.vaginalCapacity() ) {
				if( CoC.player.averageVaginalLooseness() < 2 ) {
					MainView.outputText( 'Your poor, tortured nipple is barely up to the task of accepting the ' + CoC.player.cockDescript() + ', but accept it it does. ' );
				} else if( CoC.player.averageVaginalLooseness() < 4 ) {
					MainView.outputText( 'The engorged opening at the end of your swollen nipple is stretched to its limit as you shove your ' + CoC.player.cockDescript() + ' home. ' );
				} else {
					switch( CoC.player.cocks[ 0 ].cockType ) { //Different way of choosing cock to use, results in consistent description of cock[0]
						case CockTypesEnum.ANEMONE:
							MainView.outputText( "The swollen tips of your bloated nipple wrap around the stinging tentacles that surround your " + CoC.player.cockDescript() + "'s tip, convulsing with wet squishing sounds as they become red and enflamed with artificial lust.  " );
							break;
						case CockTypesEnum.CAT:
							MainView.outputText( "The swollen tips of your bloated nipples stretch around the barbed tip of your " + CoC.player.cockDescript() + ", swallowing it like an enormous mouth.  " );
							break;
						case CockTypesEnum.DEMON:
							MainView.outputText( "The swollen lips of your bloated nipple stretch around the nodule-ringed tip of your " + CoC.player.cockDescript() + ", swallowing it like an enormous mouth.  " );
							break;
						case CockTypesEnum.DISPLACER:
							MainView.outputText( "The swollen tips of your bloated nipple wrap around the outstretched head of your " + CoC.player.cockDescript() + ", convulsing with wet, squishing sounds as it wriggles inside you.  " );
							break;
						case CockTypesEnum.DOG:
						case CockTypesEnum.FOX:
							MainView.outputText( "The swollen lips of your bloated nipple stretch around the pointed tip of your " + CoC.player.cockDescript() + " swallowing it like an enormous mouth.  " );
							break;
						case CockTypesEnum.HORSE:
							MainView.outputText( "The swollen lips of your bloated nipple stretch around the flared tip of your " + CoC.player.cockDescript() + " swallowing it like an enormous mouth.  " );
							break;
						case CockTypesEnum.HUMAN:
							MainView.outputText( "The swollen lips of your bloated nipple stretch around the tip of your " + CoC.player.cockDescript() + " swallowing it like an enormous mouth.  " );
							break;
						case CockTypesEnum.LIZARD:
							MainView.outputText( "The swollen tips of your bloated nipple wrap around the pointed tip of your " + CoC.player.cockDescript() + ", stretching oddly as it swallows the knot-covered appendage.  " );
							break;
						case CockTypesEnum.TENTACLE:
							MainView.outputText( "The swollen lips of your bloated nipple stretch around the rounded tip of your " + CoC.player.cockDescript() + ", swallowing it like an enormous mouth.  " );
							break;
						default:
							MainView.outputText( "tips of your bloated nipples wrap around the " + CoC.player.cockHead() + " of your " + CoC.player.cockDescript() + ", swallowing it like an enormous mouth.  " );
					}
				}
				MainView.outputText( 'With each thrust, you bury your ' + CoC.player.cockDescript() + ' deeper into your greedy tit. Overwhelmed by the combined sensations, your mind is barely able to cope with the intense feeling of fullness where no such feeling should be possible.' );
			}
			//if it doesnt really fit
			if( Math.round( CoC.player.cockArea( 0 ) ) > CoC.player.vaginalCapacity() ) {
				if( CoC.player.averageVaginalLooseness() < 2 ) {
					MainView.outputText( 'Your poor, tortured nipple is woefully insufficient compared to your ' + CoC.player.cockDescript() + ', delerious with arousal, you keep trying anyway. ' );
				} else if( CoC.player.averageVaginalLooseness() < 4 ) {
					MainView.outputText( 'The engorged opening at the end of your swollen nipple is stretched to its limits and beyond as you shove your ' + CoC.player.cockDescript() + ' home. ' );
				} else {
					switch( CoC.player.cocks[ 0 ].cockType ) { //Different way of choosing cock to use, results in consistent description of cock[0]
						case CockTypesEnum.ANEMONE:
							MainView.outputText( "The swollen lips of your bloated nipple gape wide, but the stinging tip of your " + CoC.player.cockDescript() + " spreads them wider and fills them with artificial lust.  " );
							break;
						case CockTypesEnum.CAT:
							MainView.outputText( "The swollen lips of your bloated nipple gape wide, but the barbed tip of your " + CoC.player.cockDescript() + " spreads them even wider.  " );
							break;
						case CockTypesEnum.DEMON:
							MainView.outputText( "The swollen lips of your bloated nipple gape wide, but the tip of your " + CoC.player.cockDescript() + " spreads them even wider.  " );
							break;
						case CockTypesEnum.DISPLACER:
							MainView.outputText( "The swollen lips of your bloated nipple gape wide, but the wide head of your " + CoC.player.cockDescript() + " spreads them even wider.  " );
							break;
						case CockTypesEnum.DOG:
						case CockTypesEnum.FOX:
							MainView.outputText( "The swollen lips of your bloated nipple gape wide, but the massive girth of your " + CoC.player.cockDescript() + " spreads them even wider.  " );
							break;
						case CockTypesEnum.HORSE:
							MainView.outputText( "The swollen lips of your bloated nipple gape wide, but the flared tip of your " + CoC.player.cockDescript() + " spreads them even wider.  " );
							break;
						case CockTypesEnum.HUMAN:
							MainView.outputText( "The swollen lips of your bloated nipple gape wide, but the tip of your " + CoC.player.cockDescript() + " spreads them even wider.  " );
							break;
						case CockTypesEnum.LIZARD:
							MainView.outputText( "The swollen lips of your bloated nipple gape wide, but the pointed tip of your " + CoC.player.cockDescript() + " slowly spreads them even wider.  " );
							break;
						case CockTypesEnum.TENTACLE:
							MainView.outputText( "The swollen lips of your bloated nipple gape wide, but the tip of your " + CoC.player.cockDescript() + " spreads them even wider.  " );
							break;
						default:
							MainView.outputText( "The swollen lips of your bloated nipple gape wide, but the " + CoC.player.cockHead() + " of your " + CoC.player.cockDescript() + " spreads them even wider.  " );
					}
				}
				MainView.outputText( 'Grunting and sweating with effort, you stuff as much of your ' + CoC.player.cockDescript() + ' into your overstretched nipple as you can fit. The feeling of incredible tightness around your ' + CoC.player.cockDescript() + ' combines with the pain of your distended nipple to form a mindbending sensation that makes your head spin. ' );
			}
			if( CoC.player.averageLactation() > 0 ) {
				MainView.outputText( 'Milky ' );
			} else {
				MainView.outputText( 'Slick ' );
			}
			if( CoC.player.averageVaginalWetness() < 2 ) {
				MainView.outputText( 'girl-lube quickly coats the whole length of your ' + CoC.player.cockDescript() + ' in a glistening layer of fuck juice. ' );
			} else if( CoC.player.averageVaginalWetness() < 4 ) {
				MainView.outputText( 'girl-lube drizzles down the length of your ' + CoC.player.cockDescript() + ' in thick streams, ' );
				switch( CoC.player.cocks[ 0 ].cockType ) { //Different way of choosing cock to use, results in consistent description of cock[0]
					case CockTypesEnum.ANEMONE:
						MainView.outputText( "pooling around the wriggling tentacles near your base.  " );
						break;
					case CockTypesEnum.CAT:
						MainView.outputText( "pooling around the spines near your base.  " );
						break;
					case CockTypesEnum.DEMON:
						MainView.outputText( "pooling around the base of your member.  " );
						break;
					case CockTypesEnum.DISPLACER:
					case CockTypesEnum.DOG:
					case CockTypesEnum.FOX:
						MainView.outputText( "pooling around the bulb near your base.  " );
						break;
					case CockTypesEnum.HORSE:
						MainView.outputText( "pooling in and around your sheath.  " );
						break;
					case CockTypesEnum.LIZARD:
						MainView.outputText( "pooling around the bulbs near your base.  " );
						break;
					case CockTypesEnum.TENTACLE:
						MainView.outputText( "mixing with the tentacles own lubricants.  " );
						break;
					default:
						MainView.outputText( "pooling at your crotch.  " );
				}
			} else {
				MainView.outputText( 'girl-lube spurts out of your tortured nipple with each thrust of your ' + CoC.player.cockDescript() + ', spattering your arms and face with your secretions. ' );
			}
		}
		if( Math.round( CoC.player.cockArea( 0 ) ) < CoC.player.vaginalCapacity() ) {
			//Different noises based on sensitivity
			if( CoC.player.sens < 45 ) {
				MainView.outputText( 'ou sigh with pleasure' );
			} else if( CoC.player.sens < 70 ) {
				MainView.outputText( 'ou moan with pleasure' );
			} else {
				MainView.outputText( 'ou scream with delight' );
			}
			MainView.outputText( ' as your ' + CoC.player.cockDescript() + ' slides into ' );
			//Different based on looseness
			if( CoC.player.averageVaginalLooseness() < 2 ) {
				MainView.outputText( 'the small, over-stretched opening of your swollen nipple. Your ' + CoC.player.cockDescript() + ' penetrates your swollen nipple easily, sliding halfway in on your first thrust. ' );
			} else if( CoC.player.averageVaginalLooseness() < 4 ) {
				MainView.outputText( 'the engorged and distended opening of your fat, swollen nipple. Your ' + CoC.player.cockDescript() + ' plunges deeply into your freakishly engorged nipple, penetrating it easily. ' );
			} else {
				MainView.outputText( 'the gaping fuck-mouth of your inhuman nipple-cunt.  The swollen lips of your bloated nipple engulf the ' );
				if( CoC.player.cocks[ 0 ].cockType === CockTypesEnum.HORSE ) {
					MainView.outputText( 'flared ' );
				}
				MainView.outputText( 'tip of your ' + CoC.player.cockDescript() + ' and begin to slide down its length, ' );
				switch( CoC.player.cocks[ 0 ].cockType ) { //Different way of choosing cock to use, results in consistent description of cock[0]
					case CockTypesEnum.ANEMONE:
						MainView.outputText( "swallowing it completely and leaving a searing trail of desire in its wake.  " );
						break;
					case CockTypesEnum.CAT:
						MainView.outputText( "swallowing it completely as each springy barb makes you quiver with pleasure.  " );
						break;
					case CockTypesEnum.DEMON:
						MainView.outputText( "swallowing it completely as each bump and nodule makes you quiver with unholy pleasures.  " );
						break;
					case CockTypesEnum.DISPLACER:
					case CockTypesEnum.DOG:
					case CockTypesEnum.FOX:
						MainView.outputText( "even swallowing your bulging knot without difficulty.  " );
						break;
					case CockTypesEnum.LIZARD:
						MainView.outputText( "swallowing it completely as each of the bulgy knots along its length stretch the orifice further.  " );
						break;
					case CockTypesEnum.TENTACLE:
						MainView.outputText( "swallowing it completely as it twists and pulses on its own, fucking your nipple.  " );
						break;
					default:
						MainView.outputText( "swallowing it completely.  " );
				}
			}
			MainView.outputText( 'You revel in the sensation as you slowly stroke your ' + CoC.player.cockDescript() + ' in and out of your distended nipple. Your shaft is enveloped in the warm, wet embrace of your freakish tit, and ' );
			if( CoC.player.averageLactation() > 0 ) {
				MainView.outputText( 'milky ' );
			} else {
				MainView.outputText( 'slippery ' );
			}
			if( CoC.player.averageVaginalWetness() < 2 ) {
				MainView.outputText( 'girl-lube quickly coats the whole length of your ' + CoC.player.cockDescript() + ' in a glistening layer of fuck juice. ' );
			} else if( CoC.player.averageVaginalWetness() < 4 ) {
				MainView.outputText( 'girl-lube drizzles down the length of your ' + CoC.player.cockDescript() + ' in thick streams, pooling at your crotch. ' );
			} else {
				if( CoC.player.normalCocks() > 0 ) {
					MainView.outputText( 'girl-lube pours out of your swollen nipple over your ' + CoC.player.cockDescript() + ' and hands, pooling on the ground below you. ' );
				} else if( CoC.player.hasSheath() > 0 ) {
					MainView.outputText( 'girl-lube drizzles down the length of your ' + CoC.player.cockDescript() + ' in thick streams, pooling in and around your sheath. ' );
	...
	Masturbation.prototype.multiTitFuck = function() {
		...
		if( holeTotal >= fittableCocks ) {
			...
			{
				MainView.outputText( 'Quickly, you move your other cock into position against ' );
			} else if( fittableCocks === 2 || holeTotal === 2 ) //more than two dicks but only two that fit, or if you have only two holes anyway
			{
				MainView.outputText( 'Quickly, you move another cock into position against ' );
			} else if( holeTotal >= fittableCocks ) //If you have more than two dicks to use, and enough nipples for all of them
			{
				MainView.outputText( 'Quickly, you move your remaining shafts into position against your other nipples. ' );
			}
			//How many nips?
			if( holeTotal === 2 ) {
				MainView.outputText( 'your other nipple. ' );
			} else if( fittableCocks <= 2 ) {
				MainView.outputText( 'one of your other nipples. ' );
			}
		}
		//if you dont have enough nipples for all your dicks
		if( fittableCocks > 2 && holeTotal < fittableCocks ) {
			MainView.outputText( 'You are momentarily stumped as you realize that you don\'t have enough nipples for all of your cocks. ' );
			...
			if( CoC.player.vaginalCapacity() >= 2 * CoC.player.cockArea( randomCock ) ) {
				...
				MainView.outputText( 'The words "multiple insertions" drifts through your lust-fogged brain, and you begin to position two dicks against a poor, unsuspecting nipple, preparing to double-penetrate.  ' );
				//does this give enough space?
				if( 2 * holeTotal >= fittableCocks ) {
					MainView.outputText( 'You giggle with glee as you realize that you will be able to jam every single one of your ' + CoC.player.multiCockDescriptLight() + ' into your abused nipples, thanks to double-penetration! ' );
				} else {
					MainView.outputText( 'With a flash of irritation, you realize that even if you stick two cocks in each hole, you still won\'t be able to fit all ' + Utils.num2Text( CoC.player.cocks.length ) + ' of your dicks into your abused tits. Deciding to make the best of it, you prepare to stuff in as many as you can. ' );
				}
			} else {
				MainView.outputText( 'Accepting that you can\'t do anything about it, you start pushing. ' );
			}
		}
		//How wet/milky is this procedure?
		if( CoC.player.averageLactation() === 0 ) {
			if( CoC.player.averageVaginalWetness() < 2 ) {
				...
				MainView.outputText( 'Slick juices dribble down your shafts, and y' );
			} else {
				MainView.outputText( 'Slick juices squirt out from around your shafts and y' );
			}
		} else {
			if( CoC.player.averageLactation() < 2 ) {
				MainView.outputText( 'Rivulets of milky girlcum drizzle down your shafts and y' );
			} else {
				MainView.outputText( 'Milky girlcum squirts out from around your shafts, staining them white. Y' );
			}
		}
		//Round and compare cock thickness to vag looseness
		if( Math.round( CoC.player.cockArea( randomCock ) ) >= CoC.player.vaginalCapacity() ) {
			//Different noises based on sensitivity
			if( CoC.player.sens < 45 ) {
				MainView.outputText( 'ou grunt with exertion as you attempt to stuff your cocks into ' );
			} else if( CoC.player.sens < 70 ) {
				MainView.outputText( 'ou blink back tears as you attempt to stuff your cocks into ' );
			} else {
				MainView.outputText( 'ou scream with a combination of pain and pleasure as you attempt to stuff your cocks into ' );
			}
			//Different based on looseness
			if( CoC.player.averageVaginalLooseness() < 2 ) {
				MainView.outputText( 'the small, over-stretched openings of your swollen nipples. ' );
			} else if( CoC.player.averageVaginalLooseness() < 4 ) {
				MainView.outputText( 'the engorged and distended openings of your fat, swollen nipples. ' );
			} else {
				MainView.outputText( 'the gaping fuck-mouths of your inhuman nipple-cunts. ' );
			}
			//Compare cockthickness and vaglooseness more specifically
			//if it barely fits
			if( Math.round( CoC.player.cockArea( randomCock ) ) === CoC.player.vaginalCapacity() ) {
				if( CoC.player.averageVaginalLooseness() < 2 ) {
					MainView.outputText( 'Your poor, tortured nipples are barely up to the task of accepting your swollen rods, but accept them they do. ' );
				} else if( CoC.player.averageVaginalLooseness() < 4 ) {
					MainView.outputText( 'The puckered openings that cap your engorged nipples are stretched to their limits as you shove your fat rods home. ' );
				} else {
					MainView.outputText( 'The swollen lips of your bloated nipples stretch around your throbbing cocks, swallowing them whole. ' );
				}
				MainView.outputText( 'With each thrust, you bury your shafts deeper into your greedy tits. Overwhelmed by the combined sensations, your mind is barely able to cope with the intense feelings of fullness where no such feelings should be possible. ' );
			}
			//if it doesnt really fit well
			if( Math.round( CoC.player.cockArea( randomCock ) ) > CoC.player.vaginalCapacity() ) {
				if( CoC.player.averageVaginalLooseness() < 2 ) {
					MainView.outputText( 'Your poor, tortured nipples are woefully insufficient compared to the width of your swollen rods. Delirious with arousal, you keep pushing them in anyway. ' );
				} else if( CoC.player.averageVaginalLooseness() < 4 ) {
					MainView.outputText( 'The puckered openings that cap your engorged nipples are stretched to their limits and beyond as you shove your fat rods home. ' );
				} else {
					MainView.outputText( 'The swollen lips of your bloated nipples gape wide, but the enormous girth of your throbbing members spread them even wider. ' );
				}
				MainView.outputText( 'Grunting and sweating with effort, you stuff as much cock into each of your overstretched nipples as you can fit. The feeling of incredible tightness around your tools combines with the pain of your distended nipples to form a blizzard of mindbending sensations that makes your head spin. ' );
			}
			if( CoC.player.averageLactation() > 0 ) {
				MainView.outputText( 'Milky ' );
			} else {
			...
			if( CoC.player.averageVaginalWetness() < 2 ) {
				MainView.outputText( 'girl-lube drizzles down your shafts, coating them in glistening layers of fuck juice. ' );
			} else if( CoC.player.averageVaginalWetness() < 4 ) {
				MainView.outputText( 'girl-lube drools from your tortured nipples in thick streams, pooling beneath your ass. ' );
			} else {
				MainView.outputText( 'girl-lube spurts out of your tortured nipples with each thrust of your hips, spattering your arms and face with your secretions. ' );
			}
		}
		if( Math.round( CoC.player.cockArea( randomCock ) ) < CoC.player.vaginalCapacity() ) {
			if( !doubleUp ) {
				//Different noises based on sensitivity
				if( CoC.player.sens < 45 ) {
					MainView.outputText( 'ou sigh with pleasure as your stiff cocks slide into ' );
				} else if( CoC.player.sens < 70 ) {
					MainView.outputText( 'ou moan with pleasure as your stiff cocks slide into ' );
				} else {
					MainView.outputText( 'ou scream with delight as your stiff cocks slide into ' );
				}
				//Different based on looseness
				if( CoC.player.averageVaginalLooseness() < 2 ) {
					MainView.outputText( 'the small, over-stretched openings on your swollen nipples. Your hard rods penetrate your engorged nipples easily, sliding halfway in on your first thrust. ' );
				} else if( CoC.player.averageVaginalLooseness() < 4 ) {
					MainView.outputText( 'the engorged and distended openings on your fat, swollen nipples. Your hard rods plunge deeply into your freakishly engorged nipples, penetrating them easily. ' );
				} else {
					MainView.outputText( 'the gaping fuck-mouths of your inhuman nipple-cunts. The swollen lips of your bloated nipples engulf your hard rods and begin to slide down their length, swallowing them completely. ' );
				}
				MainView.outputText( 'You revel in the sensation as you slowly stroke your shafts in and out of your distended nipples. Your cocks are enveloped in the warm, wet embrace of your freakish tits, and ' );
				if( CoC.player.averageLactation() > 0 ) {
					MainView.outputText( 'milky ' );
				} else {
					MainView.outputText( 'slippery ' );
				...
				if( CoC.player.averageVaginalWetness() < 2 ) {
					...
					MainView.outputText( 'girl-lube drools from your fat nipples in thick streams, pooling beneath your ass. ' );
				} else {
					MainView.outputText( 'girl-lube pours out of your swollen nipples in a constant deluge of sticky fluid, creating a large puddle beneath you. ' );
				}
			} else {
				//Different noises based on sensitivity
				if( CoC.player.sens < 45 ) {
				...
				if( 4 * Math.round( CoC.player.averageCockThickness() ) < CoC.player.averageVaginalLooseness() ) {
					if( CoC.player.averageVaginalLooseness() < 2 ) {
						MainView.outputText( 'the over-stretched openings on your swollen nipples. Though they are reletively small and delicate, you can easily slide two of your tiny cocks into each of their slippery passages.' );
					} else if( CoC.player.averageVaginalLooseness() < 4 ) {
						MainView.outputText( 'the engorged and distended openings on your fat, swollen nipples. Stretching easily, they can engulf two cocks each with surprising ease, stretching around your turgid rods with little difficulty. ' );
					} else {
						MainView.outputText( 'the gaping fuck-mouths of your inhuman nipple-cunts. The swollen lips of your bloated nipples squish open as they swallow your fat dicks with little difficulty. ' );
					}
					MainView.outputText( 'With each thrust, you bury your shafts deeper into your greedy tits. The feeling of your cocks rubbing together inside the warm, wet embrace of your inhuman breasts pushes you towards the edge of insanity, and you are briefly unable to comprehend the world around you. ' );
				} else { //otherwise, its a tight fit
					if( CoC.player.averageVaginalLooseness() < 2 ) {
						MainView.outputText( 'the small, over-stretched openings on your swollen nipples. Your poor, tortured nipples are stretched painfully as you cram two of your throbbing cocks into one. ' );
					} else if( CoC.player.averageVaginalLooseness() < 4 ) {
						MainView.outputText( 'the engorged and distended openings on your fat, swollen nipples. They are stretched to their limits as you shove two of your fat dicks into one. ' );
					} else {
						MainView.outputText( 'the gaping fuck-mouths of your inhuman nipple-cunts. The swollen lips of your bloated nipples are stretched to their limits as you push two of your fat dicks into one. ' );
					}
	...
	Masturbation.prototype.multiNippleFuckPrep = function( randomCock ) {
		...
		if( CoC.player.cocks[ randomCock ].cockType === CockTypesEnum.HUMAN ) {
			...
			//more than one?
			if( CoC.player.normalCocks() > 1 ) {
				MainView.outputText( 'one of your ' + Utils.num2Text( CoC.player.normalCocks() ) + ' ' + CoC.player.cockDescript( randomCock ) + 's ' );
			} else {
				MainView.outputText( 'your ' + CoC.player.cockDescript( randomCock ) + ' ' );
			}
		}
		//Applying randomization - horse cocks
		if( CoC.player.cocks[ randomCock ].cockType === CockTypesEnum.HORSE ) {
			MainView.outputText( 'flared tip of ' );
			//more than one?
			if( CoC.player.horseCocks() > 1 ) {
				MainView.outputText( 'one of your ' + Utils.num2Text( CoC.player.horseCocks() ) + ' ' + CoC.player.cockDescript( randomCock ) + 's ' );
			} else {
				MainView.outputText( 'your ' + CoC.player.cockDescript( randomCock ) + ' ' );
			}
		}
		...
		if( CoC.player.cocks[ randomCock ].cockType === CockTypesEnum.DOG ) {
			...
			if( CoC.player.dogCocks() > 1 ) {
				MainView.outputText( 'one of your ' + Utils.num2Text( CoC.player.dogCocks() ) + ' ' + CoC.player.cockDescript( randomCock ) + 's ' );
			} else {
				MainView.outputText( 'your ' + CoC.player.cockDescript( randomCock ) + ' ' );
			}
		}
		...
		if( CoC.player.lib < 70 ) {
			MainView.outputText( 'against the opening of ' );
		} else {
			MainView.outputText( 'into ' );
		}
		//More than one row?
		if( CoC.player.breastRows.length > 1 ) {
			if( CoC.player.averageNipplesPerBreast() > 1 ) {
		...
		//Different based on looseness (again)
		if( CoC.player.averageVaginalLooseness() < 2 ) {
			MainView.outputText( 'painfully stretched nipple' );
		} else if( CoC.player.averageVaginalLooseness() < 4 ) {
			MainView.outputText( 'freakishly swollen nipple' );
		} else {
			MainView.outputText( 'huge, bloated cunt-nipple' );
		}
		if( CoC.player.breastRows.length > 1 ) {
			MainView.outputText( ' on one of your lower breasts' );
		} else if( CoC.player.averageNipplesPerBreast() > 1 ) {
	...
	Masturbation.prototype.orgazmo = function( selfSucking, nippleFuck ) {
		...
		if( CoC.player.cocks.length > 0 ) {
			if( CoC.player.cocks.length === 1 ) {
				MainView.outputText( 'The sensations prove too much for you, and you feel the tightness building in your ' );
				if( CoC.player.normalCocks() === 1 ) {
					MainView.outputText( 'twitching manhood, growing rapidly.  You stroke furiously, feeling the pressure of your cum as it nears release.  ' );
				} else if( CoC.player.hasKnot() ) {
					MainView.outputText( 'swelling, bulbous knot.  You feel it growing tighter and tighter until it\'s nearly twice the width of your ' + CoC.player.cockDescript() + '.  The pressure is an agonizing pleasure that only builds higher and higher as you come closer and closer to orgasm.  ' );
				} else if( CoC.player.horseCocks() === 1 ) {
					MainView.outputText( 'swollen equine sheath, slowly beginning to work its way up your shaft.  Pre-cum begins pouring from your ' + CoC.player.cockDescript() + ', slicking your ' + CoC.player.cockDescript() + ' as you get ready to blow.  ' );
				} else if( CoC.player.tentacleCocks() === 1 ) {
					MainView.outputText( 'wiggling vine-like cock, growing rapidly.  You feel the rubbery surface of your tentacle prick contracting as it nears release.  Thick bulges of fluids collect and travel along the length of your ' + CoC.player.cockDescript() + ', the first of which is almost to your oversized tip.  ' );
				} else if( CoC.player.demonCocks() === 1 ) {
					MainView.outputText( 'tainted man-meat, growing rapidly.  You feel the nodules close to the base swell and pulse, starting a rippling wave of pleasure that migrates upwards.  The ring of bumps around your crown double in size, flaring out as your ' + CoC.player.cockDescript() + ' prepares to unload.  ' );
				} else if( CoC.player.catCocks() === 1 ) {
					MainView.outputText( 'full, cat-like sheath.  You feel it tingling and throbbing as the spines pulsate with arousal.  You feel the barbs at the bottom thickening as bulges of fluid migrate through your ' + CoC.player.cockDescript() + ' towards the tip.  ' );
				} else if( CoC.player.lizardCocks() === 1 ) {
					MainView.outputText( 'swollen member.  You can feel it tingling and bulging strangly as it begins to contract and pulse.  Pre-cum leaks from your ' + CoC.player.cockDescript() + ' in a steady stream as each \'bulb\' nearly doubles in size, and then one at a time, they deflate while your urethra dilates wide.  ' );
				} else if( CoC.player.anemoneCocks() === 1 ) {
					MainView.outputText( 'base.  The tentacles surrounding your ' + CoC.player.cockDescript() + ' go nuts, constricting around it, inadvertently firing aphrodisiacs into the orgasmic flesh as they wring a potent, hip-jerking climax from you.  ' );
				} else {
					MainView.outputText( 'twitching manhood, growing rapidly.  You stroke furiously, feeling the pressure of your cum as it nears release.  ' );
				}
				MainView.outputText( 'Pleasurable spasms overwhelm you as cum erupts from your ' + CoC.player.cockDescript() );
				MainView.outputText( '.  Your hips jerk in the air in time with your eruptions, spraying cum ' );
				if( selfSucking ) {
					MainView.outputText( 'into your eager mouth.  ' );
				}
				if( nippleFuck ) {
					MainView.outputText( 'deep inside your unnatural breast. ' );
				} else if( !selfSucking ) {
					MainView.outputText( 'in the air.  ' );
				}
				if( selfSucking ) {
					if( CoC.player.cumQ() < 25 ) {
						MainView.outputText( 'You manage to gulp down most of your cum as it bursts from your loins, the pleasure of orgasm goading you into compulsively swallowing.  ' );
					} else if( CoC.player.cumQ() < 250 ) {
						MainView.outputText( 'You sputter jism from your mouth as your orgasm becomes too much to keep up with, cum filling your mouth to capacity as you swallow and dribble in equal measure.  ' );
					} else if( CoC.player.cumQ() < 500 ) {
						MainView.outputText( 'Pulse after pulse of cum erupts from your ' + CoC.player.cockDescript() + ' into your mouth.  You swallow what you can but it\'s too much for you.  Cum runs down your cock to pool on you as your orgasm drags on.  ' );
					} else {
						MainView.outputText( 'Your orgasm never seems to end, and your world dissolves into the feelings from your ' + CoC.player.cockDescript() + ' as it erupts jet after jet of cum into your mouth.  You nearly gag, cum overflowing to spray out in a river, pooling around you.  ' );
					}
				} else if( nippleFuck ) {
					if( CoC.player.cumQ() < 25 ) {
						MainView.outputText( 'You feel multiple thick spurts of cum splash against the inside of your breast. ' );
					} else if( CoC.player.cumQ() < 250 ) {
						MainView.outputText( 'Cum dribbles down the length of your shaft as your abused tit is filled to overflowing by spurt after spurt of gooey sperm. ' );
					} else if( CoC.player.cumQ() < 500 ) {
						MainView.outputText( 'Pulse after pulse of cum splash deep within your breast, causing it to swell with pressure. Cum runs down your cock in a torrent to pool on you as your orgasm drags on.  ' );
					} else {
						MainView.outputText( 'Your orgasm never seems to end, and your world dissolves into the feelings from your ' + CoC.player.cockDescript() + ' as it erupts jet after jet of cum into your abused tit.  With a *pop*, your cock is forced free by the pressure. It continues to squirt semen everywhere, while a river of cum pours out of your dialated nipple, pooling around you.  ' );
					}
				} else {
					if( CoC.player.cumQ() < 25 ) {
						MainView.outputText( 'A few thick spurts of cum burst from your loins, splattering you liberally.  ' );
					} else if( CoC.player.cumQ() < 250 ) {
						MainView.outputText( 'The orgasm drags on and on, spurt after spurt of jism coating you.  ' );
					} else if( CoC.player.cumQ() < 500 ) {
						MainView.outputText( 'Your body spasms powerfully, each spurt making you twitch more powerfully than the last.  Rope after rope of jizz rains down.  ' );
					} else {
						MainView.outputText( 'The orgasm never seems to end, and your world dissolves to little more than the feeling of each jet of cum erupting from your cock.  Your mind dimly processes the feeling of each burst splattering over you, but it only enhances the feeling.  ' );
					}
				}
			}
			//MULTICOCK
			if( CoC.player.cocks.length > 1 ) {
				MainView.outputText( 'The sensations prove too much for you, and you feel the tightness building in your loins.  ' );
				if( CoC.player.dogCocks() > CoC.player.normalCocks() && CoC.player.dogCocks() > CoC.player.horseCocks() ) { //Primary Dog
					...
					MainView.outputText( 'Your feel your knots bulging and swelling, growing tighter and tighter until they\'re nearly double the width of a ' + CoC.player.cockDescript( dogIndex ) + '.  The agonizing pressure builds higher and tighter with every passing second as you get closer and closer to orgasm.  ' );
					...
					MainView.outputText( 'You feel a pulsing in your sheath, slowly working its way up your ' + CoC.player.cockDescript( horseIndex ) + 's.  Pre-cum pours from your ' + CoC.player.cockDescript( horseIndex ) + 's, slicking the wobbly equine shafts as they get ready to blow.  ' );
				} else { //Primary Normal
					MainView.outputText( 'Your manhoods twitch, growing to their full size.  You stroke them furiously, feeling the pressure of your cum as it nears release.  ' );
				}
				//check if all dicks are stuck in tits.
				if( nippleFuck ) {
					if( CoC.player.cumQ() < 25 ) {
						MainView.outputText( 'You feel multiple thick spurts of cum splash inside of your breasts. As you buck and thrust with orgasm, your cocks begin to pop free of your abused tits. ' );
					} else if( CoC.player.cumQ() < 250 ) {
						MainView.outputText( 'Cum dribbles down your shafts as your abused tits are filled to overflowing by spurt after spurt of gooey sperm. As you buck and thrust with orgasm, your cocks begin to pop free of your abused tits. ' );
					} else if( CoC.player.cumQ() < 500 ) {
						MainView.outputText( 'Pulse after pulse of cum splash deep within your breasts, causing them to swell with pressure. Torrents of cum run down your cocks to pool around you. As you buck and thrust with orgasm, your cocks begin to pop free of your abused tits.  ' );
					} else {
						MainView.outputText( 'With an explosive release, you finally cum.  Your orgasm never seems to end, and your world dissolves into the feelings from your ' + CoC.player.multiCockDescriptLight() + ' as they erupt jet after jet of cum into your abused tits.  With a series of wet *pops*, your cocks are forced free by the pressure. They continue to squirt semen everywhere, while rivers of cum pour out of your dilated nipples, pooling around you.  ' );
					}
				}
				//Doing the perverted sprinkler after dicks have popped out.
				MainView.outputText( 'Pleasure overwhelms your fragile mind as cum erupts from your ' + CoC.player.multiCockDescriptLight() + '.  Your hips jerk in the air in time with your eruptions, cum spraying from you as your body does its best impression of a perverted sprinkler.  ' );
				if( selfSucking ) {
					if( CoC.player.cumQ() < 25 ) {
						MainView.outputText( 'You manage to swallow most of your cum as it bursts from your loins, the pleasure of orgasm goading you into compulsively swallowing.  ' );
					} else if( CoC.player.cumQ() < 250 ) {
						MainView.outputText( 'You sputter jism from your mouth as your orgasm becomes too much to keep up with, cum filling your mouth to capacity as you swallow and dribble in equal measure.  Cum splatters on you from the rest of your equipment, making you a slimy mess.' );
					} else if( CoC.player.cumQ() < 500 ) {
						MainView.outputText( 'Pulse after pulse of cum erupts from your ' + CoC.player.cockDescript() + ' into your mouth.  You swallow what you can but it\'s too much for you.  Cum runs down your ' + CoC.player.cockDescript() + ' to pool on you as your orgasm drags on.  Jizz rains over you the entire time from the rest of your "equipment".' );
					} else {
						MainView.outputText( 'Your orgasm never seems to end, and your world dissolves into the feelings from your ' + CoC.player.cockDescript() + ' as it erupts jet after jet of cum into your mouth.  You nearly gag, cum overflowing to spray out in a river, pooling around you.  Your other \'equipment\' rains jizz upon you the whole while, soaking you in a cum-puddle.' );
					}
				}
				//These seem like they should always be displayed regardless of other factors.
				if( CoC.player.cumQ() < 5 ) {
					MainView.outputText( 'A few thick spurts of cum burst from your cocks, splattering you liberally.  ' );
				} else if( CoC.player.cumQ() < 7 ) {
					MainView.outputText( 'The orgasm drags on and on, spurt after spurt of jism coating you from each cock.  ' );
				} else if( CoC.player.cumQ() < 10 ) {
					MainView.outputText( 'Your body spasms powerfully, each spurt making you twitch more powerfully than the last.  Rope after rope of jizz rains down as the orgasms from each of your members begin to overlap.  Your nearly black out in pleasure.  ' );
				} else {
					MainView.outputText( 'The orgasm never seems to end, and your world dissolves to little more than the feeling of multiple cum eruptions spurting from your pricks.  Your mind dimly processes the feeling of each burst splattering over you, but it only enhances the feeling.  ' );
				}
			}
		}
		//Vaginal CUMMING
		if( CoC.player.vaginas.length > 0 ) {
			...
			if( CoC.player.vaginas.length === 1 ) {
				...
				MainView.outputText( 'sex quivers as the pleasure overwhelms you, robbing you of muscle control, your passage rippling and contracting around your fingers.  ' );
				//Big clitty bonus!
				if( CoC.player.clitLength >= 4.5 ) {
					MainView.outputText( 'A hand finds your cock-like clit, squeezing and caressing it as the cunt-wrenching orgasm wracks your body, the over-sized joy-buzzer nearly making you black out from the sensations it generates.  ' );
				} else if( CoC.player.clitLength > 1.5 ) {
					MainView.outputText( 'Every muscle-twitch seems to stretch your big oversensitive clitty larger, causing you to squeal with delight at every bump and touch against it.  ' );
				}
				//Wet orgasms
				if( CoC.player.vaginas[ 0 ].vaginalWetness === AppearanceDefs.VAGINA_WETNESS_SLAVERING ) {
					MainView.outputText( 'A veritable gush of fluid explodes from your nethers, pulsing in time with the ripples of your ' + CoC.player.vaginaDescript() + '.  ' );
				}
			}
	...
	Masturbation.prototype.titCum = function( cumQuantity ) {
		...
		if( CoC.player.averageLactation() >= 2 && !CoC.player.hasFuckableNipples() ) {
			MainView.outputText( 'As orgasm after orgasm thunders through your body, you feel pressure build in your breasts and then suddenly release, as ' );
			if( CoC.player.averageLactation() <= 2.6 ) {
				MainView.outputText( 'thin streams of milk spray out of your ' );
			}
			if( CoC.player.averageLactation() > 2.6 && CoC.player.averageLactation() < 3 ) {
				MainView.outputText( 'thick gouts of milk spurt out of your ' );
			}
			if( CoC.player.averageLactation() >= 3 ) {
				MainView.outputText( 'huge torrents of milk blast out of your ' );
			}
			//different description based on size of nips
			...
			if( CoC.player.averageNippleLength() >= 4 ) {
				MainView.outputText( 'huge, swollen teats, ' );
			}
			if( CoC.player.averageLactation() <= 2.6 ) {
				MainView.outputText( 'spattering milk everywhere. ' );
			}
			if( CoC.player.averageLactation() > 2.6 && CoC.player.averageLactation() < 3 ) {
				MainView.outputText( 'covering everything nearby. ' );
			}
			if( CoC.player.averageLactation() >= 3 ) {
				MainView.outputText( 'drenching the entire area.' );
			}
		}
		//Cumming with Nipplecunts!
		if( CoC.player.hasFuckableNipples() ) {
			MainView.outputText( 'A strange feeling builds in your breasts as your nipples seem to tighten and quiver with anticipation. You ' );
			//Different noise based on sensitivity
			if( CoC.player.sens < 70 ) {
				MainView.outputText( 'moan with ecstacy as your freakishly swollen nipples ' );
			}
			if( CoC.player.sens >= 70 ) {
				MainView.outputText( 'scream with ecstacy as your freakishly swollen nipples ' );
			}
			//Different description based on wetness
			if( CoC.player.averageVaginalWetness() < 2 ) {
			...
			//Lactating?
			if( CoC.player.averageLactation() > 0 ) {
				MainView.outputText( 'milk and ' );
			}
			MainView.outputText( 'pussy juice everywhere.' );
		}
	};
	...
	Masturbation.prototype.deluxeDildo = function() {
		...
		if( CoC.player.hasVagina() ) {
			...
			if( CoC.player.cor > 66 ) {
				MainView.outputText( 'You retrieve the floppy pink dildo from your possessions and strip down in the middle of your camp, shivering with the sexual thrill of your exhibitionism.' );
			}//(medcor)
			else if( CoC.player.cor > 33 ) {
				MainView.outputText( 'You retrieve your floppy dildo and sigh happily as you squeeze it, feeling the spongy surface give a little bit.  Glancing around furtively, you find a secluded spot and strip down.' );
			}//(lowcor)
			else {
				MainView.outputText( 'You blush feverishly as you grab your pink dildo.  It flops about lewdly as you run off behind some rocks and strip down to use it.  You feel like such a pervert.' );
			}
			MainView.outputText( '\n\n' );
			//(low cor)
			if( CoC.player.cor < 50 ) {
				MainView.outputText( 'You hold the fake dong away from you, aroused but still somewhat disgusted by its lewd shape.' );
			}//high cor
			else {
				MainView.outputText( 'You hold the fake dong, squeezing it and giggling at the realistic texture.  You can\'t wait to try it out.' );
			}
			MainView.outputText( '  A drop of pink aphrodisiac leaks from the tip, offering you a hint of the pleasure you\'re in for.   You make sure to catch it on your crotch, letting the fluid seep into your ' + CoC.player.vaginaDescript() + '.  Warmth radiates outwards, spreading to your thighs as your ' + CoC.player.clitDescript() );
			if( CoC.player.clitLength < 0.5 ) {
				MainView.outputText( ' becomes hard and sensitive' );
			} else if( CoC.player.clitLength < 3 ) {
				MainView.outputText( ' peeks through your folds' );
			} else if( CoC.player.clitLength < 6 ) {
				MainView.outputText( ' fills with blood, growing erect like a tiny cock' );
			} else {
				MainView.outputText( ' fills with blood, twitching and pulsating like a man\'s cock' );
			}
			MainView.outputText( '.  You ' );
			if( CoC.player.cor > 50 ) {
				MainView.outputText( 'don\'t ' );
			}
			MainView.outputText( 'hesitate ' );
			if( CoC.player.cor < 50 ) {
				MainView.outputText( 'before slowly working it inside you, gasping at the enhanced sensitivity of your ' + CoC.player.vaginaDescript() + '.' );
			} else {
				MainView.outputText( 'ramming it deep inside you, moaning as it rubs your now over-sensitive walls.' );
			}
			MainView.outputText( '  You splay your ' + CoC.player.legs() + ' and lie there with it inside you, feeling it respond to your wetness, becoming more and more turned on by the second.\n\n' );
			//(Kinda dry)
			if( CoC.player.vaginas[ 0 ].vaginalWetness < AppearanceDefs.VAGINA_WETNESS_SLICK ) {
				MainView.outputText( 'The thickness of the toy gradually increases, filling you more and more effectively as it reacts to your bodily fluids.  You grab it two-handed and start slamming it into your ' + CoC.player.vaginaDescript() + ', vigorously fucking yourself with the swelling dong.  The sensations just keep getting better and better as more and more of the goblin\'s sex-drug leaks into you.  Even your ' + CoC.player.clitDescript() + ' and cunt-lips tingle with need.  You answer that need by picking up the pace, pistoning faster and faster.\n\n' );
			}//(Pretty wet)
			else if( CoC.player.vaginas[ 0 ].vaginalWetness < AppearanceDefs.VAGINA_WETNESS_SLAVERING ) {
				MainView.outputText( 'The toy\'s girth seems to pulse and swell within you, spreading you wide open as it sops up your natural wetness and grows larger.  You grab it in a two-handed grip and begin working it in and out of your ' + CoC.player.vaginaDescript() + ', gasping and twitching as every ridge and feature of the dildo rubs you just right.  Every inch of your nethers tingles with a desire to be touched, rubbed, and squeezed. ' );
				if( CoC.player.cocks.length > 0 ) {
					MainView.outputText( 'Even your ' + CoC.player.multiCockDescript() + ' ache' );
					if( CoC.player.totalCocks() === 1 ) {
						MainView.outputText( 's' );
					}
					MainView.outputText( ' and pulse' );
					if( CoC.player.totalCocks() === 1 ) {
						MainView.outputText( 's' );
					}
					MainView.outputText( ', bouncing on your belly.  ' );
				}
				MainView.outputText( 'You answer that need by pistoning the fat juicy dick even harder into your tightly stretched box, cooing as tiny squirts of fluid erupt with every thrust.\n\n' );
			}
			//(Soaked)
			else {
				MainView.outputText( 'You can feel the dildo growing inside you, reacting to gushing feminine fluids by stretching your ' + CoC.player.vaginaDescript() + ' wide.  It doesn\'t seem to stop when you start fucking yourself with it.  If anything, it only seems to get thicker and thicker until there is barely room for your juices to squirt around it and your hips feel sore.  However, the tingling hotness of the dildo\'s aphrodisiac cum overwhelms the discomfort of the fattening fuck-tool, and you work it harder and harder, reveling in being stretched beyond your normal capacity.\n\n' );
			}
			//Sensitivity based orgasms.
			//(Low sensitivity)
			if( CoC.player.sens < 80 ) {
				MainView.outputText( 'Practically brutalizing your cunt with the swollen puss-plug, you bring yourself to orgasm.  Your ' + Descriptors.hipDescript() + ' leap off the ground, quivering in the air against your hands as you ram the toy into yourself as far as it will go.  You can feel it spurting inside you, just like a real man.  You wiggle and moan as the muscle spasms work their way through your ' + CoC.player.legs() + ', leaving you drained and exhausted.  The pink dildo suddenly shrinks back to its original size and flops free, leaving your ' + CoC.player.vaginaDescript() + ' stretched open to drool a puddle of pink cum.' );
				//(+sensitivity by 5)
				EngineCore.dynStats( 'sen', 5 );
			}
			//High sensitivity (80+)
			else {
				MainView.outputText( 'Brutalizing your stretched fuck-hole with the bloated toy, you manage to get yourself off.   Your body quakes and spasms while your ' + Descriptors.hipDescript() + ' buck into the air, fucking an imaginary lover.  The dildo sinks into your core, your arms instinctively fulfilling your desire for complete penetration.  A warm wetness suddenly soaks your womb as the fuck-stick erupts, filling you.  Just like that the orgasm you had seems like foreplay.  Your eyes roll back into your head and you begin convulsing, practically having a pleasure-seizure from the drugs and your already oversensitive twat.   You sprawl there, wiggling and cumming your brains out for what feels like an eternity, but it does eventually end, and when it does the dildo is back to its normal size, lying in a puddle of aphrodisiacs and girlcum.' );
	...
	Masturbation.prototype.onaholeUse = function() {
		...
		if( CoC.player.findStatusAffect( StatusAffects.PlainOnaholeUsed ) < 0 ) {
			...
			if( CoC.player.cocks.length > 1 ) {
				MainView.outputText( 'With your free hand, you slap your ' + CoC.player.multiCockDescriptLight() + ' to \'attention\' and ease the onahole over your cocks.  ' );
			} else {
				MainView.outputText( 'With your free hand, you slap your cock to \'attention\' and ease the onahole over your cock.' );
			}
	...
	Masturbation.prototype.deluxeOnaholeUse = function() {
		...
		if( CoC.player.findStatusAffect( StatusAffects.DeluxeOnaholeUsed ) < 0 ) {
			...
			MainView.outputText( 'As you \'deflower\' the toy, you are floored by how realistic it REALLY does feel. Giacomo must use one of these himself, as it does feel damn close to a real twat. You especially enjoy how it manages to squeeze with just the right amount of pressure.' );
	...
	Masturbation.prototype.onaholeMulticockContinuation = function() {
		MainView.outputText( 'You pull the sloppy toy from your dribbling dick and smile, shoving its slippery surface down on another of your ' + CoC.player.multiCockDescriptLight() + '.  You rapidly work it around your cocks, orgasming until ' );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( 'you pass out with aching, empty balls.' );
		} else {
			MainView.outputText( 'you pass out with ' + CoC.player.multiCockDescriptLight() + ' sore from exertion.' );
		}
	...
	Masturbation.prototype.onaholeFutaContinuation = function() {
		...
		MainView.outputText( String( Math.ceil( ((Math.random() * CoC.player.str / 12) + CoC.player.str / 6) * 10 ) / 10 ) );
		MainView.outputText( ' feet away from you. Delirious with pleasure, you continue your \'impression\' of a semen volcano, covering yourself and the area with your seed. ' );
		MainView.outputText( ' As your orgasms fade, you find yourself a well-fucked mess, and pass out.' );
		EngineCore.dynStats( 'sen', -1 );
		CoC.player.orgasm();
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	};
	...
	Masturbation.prototype.stimBeltUse = function() {
		...
		if( (CoC.player.hasKeyItem( 'Self-Stimulation Belt' ) >= 0) ) {
			...
			if( CoC.player.findStatusAffect( StatusAffects.used_self_dash_stim ) < 0 ) {
				...
				MainView.outputText( ' tits as your body convulses with multiple orgasms. Savoring every moment, you relish in the pangs of delight searing your body. Eventually, the belt moves slower and slower, until it comes to a stop, along with your fun. You realize that the gears have wound down and the box needs to be wound for your pleasure to continue. Deciding not to overwhelm yourself, you carefully remove your toy and save it for another time.' );
				CoC.player.orgasm();
				EngineCore.dynStats( 'sen', -1 );
				EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
			}
	...
	Masturbation.prototype.allNaturalSelfStimulationBeltContinuation = function() {
		MainView.clearOutput();
		MainView.outputText( 'In shock, you scream as you realize the nodule has instantly grown into a massive, organic dildo. It bottoms out easily and rests against your cervix as you recover from the initial shock of its penetration. As the pangs subside, the infernal appendage begins working itself. It begins undulating in long, slow strokes. It takes great care to adjust itself to fit every curve of your womb. Overwhelmed, your body begins reacting against your conscious thought and slowly thrusts your pelvis in tune to the thing.\n\n' );
		...
		MainView.outputText( 'The fiendish belt shifts again. It buries itself as deep as it can go and you feel pressure against the depths of your womanhood. You feel a hot fluid spray inside you. Reflexively, you shout, "<b>IT\'S CUMMING! IT\'S CUMMING INSIDE ME!</b>" Indeed, each push of the prodding member floods your box with juice. It cums... and cums... and cums... and cums...\n\n' );
		MainView.outputText( 'An eternity passes, and your pussy is sore. It is stretched and filled completely with whatever this thing shoots for cum. It retracts itself from your hole and you feel one last pang of pressure as your body now has a chance to force out all of the spunk that it cannot handle. Ooze sprays out from the sides of the belt and leaves you in a smelly, sticky mess. You feel the belt\'s tension ease up as it loosens. The machine has run its course. You immediately pass out.' );
		CoC.player.slimeFeed();
		CoC.player.orgasm();
	...
	Masturbation.prototype.allNaturalSelfStimulationBeltBadEnd = function() {
		...
		MainView.outputText( '"<i>Well, well,</i>" Giacomo says.  "<i>The Libertines are right.  The creature\'s fluids are addictive. This poor woman is a total slave to the beast!</i>"\n\n' );
		MainView.outputText( 'Giacomo contemplates the situation as you writhe in backbreaking pleasure before him.  His sharp features brighten as an idea strikes him.\n\n' );
		MainView.outputText( '"<i>AHA!</i>" the hawkish purveyor cries.  "<i>I have a new product to sell! I will call it the \'One Woman Show!\'</i>"\n\n' );
		MainView.outputText( 'Giacomo cackles smugly at his idea.  "<i>Who knows how much someone will pay me for a live woman who can\'t stop cumming!</i>"\n\n' );
		MainView.outputText( 'Giacomo loads you up onto his cart and sets off for his next sale.  You do not care.  You do not realize what has happened.  All you know is that the creature keeps cumming and it feels... sooooo GODDAMN GOOD!' );
		EngineCore.gameOver();
	};
		...
		//[Repeatable]
			...
			if( CoC.player.countCockSocks( 'gilded' ) > 0 && CoC.flags[ kFLAGS.GILDED_JERKED ] < CoC.player.countCockSocks( 'gilded' ) ) {
				...
				if( CoC.flags[ kFLAGS.TIMES_AUTOFELLATIO_DUE_TO_CAT_FLEXABILITY ] > 10 && CoC.player.balls > 1 && Utils.rand( 5 ) === 0 ) {
					MainView.outputText( 'But it\'s not enough and you are forced to come up for air.' );
	...
	Masturbation.prototype.dualBeltMasturbation = function() {
		MainView.clearOutput();
		MainView.outputText( 'You look at the thing in front of you and sigh, your ' + CoC.player.multiCockDescriptLight() + ' and ' + CoC.player.vaginaDescript() + ' dripping with fluids. With a nervous sigh you step into the underwear looking device and slip it up your legs, the cold metal feeling so good on your ' + CoC.player.skinDesc + ', sending a rush of feelings up your body. You slip your ' + CoC.player.cockDescript() + ' down and into a hole on the front of the belt, the extra length sticking out on the other side of it. Underneath the hole and right above your pussy is another metal square with what feels like a small nub on the inside rubbing against your clit. Shivering from the feeling of it, you stay there for a moment, waiting in anticipation. Finally, you reach down to the side of the belt and flick the switch to the on position.\n\n' );
		MainView.outputText( 'The belt whirs to life, shaking on your waist, sending jolts of pleasure through your clit as the small inside nub hits it. "<i>Ohh...</i>" Suddenly, the ring around your cock vibrates and then tightens hard around your cock, the belt sinking onto your body and locking in place. Worry sets in instantly as you try to wiggle and take it off, but it is no use. You see something black bubble from the edges of the metal, worried even more that it might be some sort of acid. It begins to creep across your skin at a disturbing rate, going down your ' + CoC.player.legs() + ' and encasing them in the blackness, wrapping your cock, ' );
		if( CoC.player.tailType > AppearanceDefs.TAIL_TYPE_NONE ) {
			MainView.outputText( 'covering up your tail, ' );
		}
		MainView.outputText( 'and then going up your body, covering your ' + CoC.player.allBreastsDescript() + ' and neck. The only part of your body unclad by the suit is your head. The blackness feels slick and smooth, almost cold, a strange type of feeling washes over you until you realize that it is a rubber suit.\n\n' );
		MainView.outputText( 'Before you can do anything else, the belt activates again and the latex covering of your ' + CoC.player.cockDescript() + ' begins to tighten and pulse around the meat, warming up to feel like a virgin cunt. A moan is dragged from your lips as it begins to ripple and pulse, simulating the feeling of fucking a tight hole as the entire suit molds itself to your body. Before you can get too used to the feeling of the suit milking your cock, the nub that had been teasing your clit suddenly expands and pushes out, the slick feeling of the latex pushing into your pussy.  The hardened black latex splits your tunnel and spreads you wide as it goes in deep. Your eyes widen for a moment as both stop, and then your world explodes in a flash of pleasure. The hardened lump begins to piston in and out of your ' + CoC.player.vaginaDescript() + ', vibrating wildly as a lump grows in on top in precisely the right spot to rub back and forth on your g-spot.\n\n' );
		MainView.outputText( 'Meanwhile the latex around your ' + CoC.player.cockDescript() + ' begins to pulse and ripple faster than ever before. You quake and quiver, ' + CoC.player.legs() + ' giving out as it teases and pulses around your ' + CoC.player.allBreastsDescript() + '. Your hands go down your body helplessly and start stroking at your encased cock, rubbing up and down your length. Unfortunately, all things must come to an end as the pleasure gets to be way too much and you feel yourself cum. Your hips buck wildly as you feel cum spurt into the latex, the end swelling up and filling like a ' );
		if( CoC.player.cumQ() > 200 ) {
			MainView.outputText( 'massive ' );
		}
		MainView.outputText( 'balloon. ' );
		if( CoC.player.cumQ() >= 1000 ) {
			MainView.outputText( 'It grows larger and larger until you are sure it will pop, but it doesn\'t.  It just sloshes around - a huge bubble, nearly waist high. ' );
		}
		MainView.outputText( 'Your eyes close in shivered ecstasy as your cunt spasms and clutches down around the hardened section deep inside of you. ' );
		//([If high lactation]
		if( CoC.player.biggestLactation() >= 2 ) {
			MainView.outputText( 'Milk gushes out from your ' + CoC.player.breastDescript( 0 ) + ' as you orgasm, filling the inside of the suit with a slick layer of milk and forming milk bubbles that hang lewdly off your chest. ' );
		}
		MainView.outputText( 'However, the suit is far from over as it keeps up all of its actions, keeping you on an orgasmic plateau, making sure you never stop coming. Your hands fall to the side and your body falls down, unable to keep it up as your consciousness fades, the suit still filling with all your fluids.\n\n' );
		MainView.outputText( 'When you wake, the black latex is no longer covering your body and the belt is silent around your waist. Cum drips from the tip of your cock and the top part of your ' + CoC.player.legs() + ' are coated with your feminine juices. ' );
		if( CoC.player.biggestLactation() >= 2 ) {
			MainView.outputText( 'Thin streams of creamy milk flow from your ' + CoC.player.allBreastsDescript() + ', your torso and midsection dripping wet from the stuff. ' );
		}
		MainView.outputText( 'Completely sated, you take off the belt, finding it slides off easily, and put it away in your campsite, eagerly awaiting the time you can next use it and have the suit work you over once more.' );
	...
	Masturbation.prototype.centaurNippleCunt = function() {
		MainView.outputText( 'You shrug out of your ' + CoC.player.armorName + ', ' );
		if( CoC.flags[ kFLAGS.PC_FETISH ] > 0 ) {
			MainView.outputText( 'panting lustily as you envision being caught masturbating your ' + CoC.player.nippleDescript( 0 ) + 's.' );
		} else if( CoC.player.cor < 33 ) {
			MainView.outputText( 'blushing a bit as you look down at your ' + CoC.player.nippleDescript( 0 ) + 's.' );
		} else if( CoC.player.cor < 66 ) {
			MainView.outputText( 'shivering as the air hits your exposed ' + CoC.player.nippleDescript( 0 ) + 's.' );
		} else {
			MainView.outputText( 'smiling to yourself as you blatantly oogle your ' + CoC.player.nippleDescript( 0 ) + 's.' );
		}
		MainView.outputText( '  The openings are soft, sensitive, and slick as you ease a fingertip into ' );
		if( CoC.player.totalNipples() > 2 ) {
			MainView.outputText( 'two of ' );
		}
		MainView.outputText( 'them.' );
		if( CoC.player.biggestTitSize() > 2 ) {
			MainView.outputText( '  The sudden displacement makes your ' + CoC.player.breastDescript( 0 ) + ' jiggle enticingly, and the sudden movement fires arcs of pleasure deep into your body.' );
		} else {
			MainView.outputText( '  They\'re shallow enough that you don\'t get much more than a fingertip inside them, but the flesh is extraordinarily sensitive, shooting arcs of pleasure deep into your body.' );
		}
		MainView.outputText( '  Your free fingers slowly stroke around the outer edges of your ' + CoC.player.nippleDescript( 0 ) + 's\' lips, pausing when you feel a tiny sensitive clit-like nub just inside the top of the opening.\n\n' );
		MainView.outputText( 'Shivering with pleasure, you kneel down to prevent your legs from going out from underneath you.   Your ' + CoC.player.face() + ' flushes hotter as you toy with the slippery nipple-cunts, feeling them growing hard and puffy as you turn yourself on more and more.  ' );
		if( CoC.player.biggestLactation() > 1 ) {
			MainView.outputText( 'Thick, milky lubricant races down the curvature of your breasts as your self-pleasure inadvertently lets your milk down.' );
		} else if( CoC.player.wetness() >= 3 ) {
			MainView.outputText( 'Rivulets of lubricant race down the curvature of your breasts as your chest-pussies clench around your intruding fingers.' );
		} else {
			MainView.outputText( 'Slick lubricant squishes around your fingers as you finger-fuck your chest-pussies.' );
		}
		MainView.outputText( '  You moan and lean over, shoving a second finger inside each of them and rubbing your thumbs over the miniature clits as you near orgasm.\n\n' );
		if( CoC.player.hasVagina() || CoC.player.hasCock() ) {
			var plural = false;
			MainView.outputText( 'Denied a single touch, your ' );
			if( CoC.player.hasCock() ) {
				if( CoC.player.hasVagina() ) {
					MainView.outputText( CoC.player.multiCockDescriptLight() + ' and ' + CoC.player.vaginaDescript() );
				} else {
					MainView.outputText( CoC.player.cockDescript() );
				}
				...
				MainView.outputText( CoC.player.vaginaDescript() );
			}
			if( plural ) {
				MainView.outputText( ' leak their own fluid, quivering with ache and need that you can\'t reach to satisfy.' );
			} else {
				MainView.outputText( ' leaks its own fluids, quivering with ache and need that you can\'t reach to satisfy.' );
			}
			if( CoC.player.totalCocks() > 1 ) {
				MainView.outputText( '  You feel each of your ' + CoC.player.multiCockDescriptLight() + ' tapping against your belly as your muscles clench, getting as much pleasure as possible from your male endowments.' );
			} else if( CoC.player.totalCocks() === 1 ) {
				MainView.outputText( '  You feel your ' + CoC.player.multiCockDescriptLight() + ' tapping against your belly as your muscles clench, getting as much pleasure as possible from your male endowment.' );
			}
			if( CoC.player.balls > 0 ) {
				MainView.outputText( '  Rocking slightly inside your sack, your ' + CoC.player.ballsDescriptLight() + ' repeatedly clench up against your body, ready to release.' );
			}
			MainView.outputText( '\n\n' );
		}
		//(ORGAZMO)
		MainView.outputText( 'A slow wave of contractions starts deep inside each of your ' + CoC.player.nippleDescript( 0 ) + 's.  It intensifies as it rises towards the surface of your ' + CoC.player.breastDescript( 0 ) + ', reaching a crescendo that brings you to the peak of pleasure.  Your eyes roll back as you slump down in orgasmic bliss, fingers pumping relentlessly at you slippery nipple-holes.  Noisy wet squelches and incessant moaning rouse you from your pleasure-induced coma, eventually waking you back to reality.\n\n' );
		//Optional post orgasm bits for dicks/pussies
		if( CoC.player.hasCock() ) {
			if( CoC.player.cumQ() < 50 ) {
				MainView.outputText( 'A small puddle of semen has formed under you, ' );
			} else if( CoC.player.cumQ() < 200 ) {
				MainView.outputText( 'A decent sized puddle of white seed has formed under you, ' );
			} else if( CoC.player.cumQ() < 1000 ) {
				MainView.outputText( 'A large puddle of white seed has formed underneath you and even splattered your underside, ' );
			} else if( CoC.player.cumQ() < 2000 ) {
				MainView.outputText( 'A frothing puddle of thick cum has formed below you, soaking your legs and underbelly, ' );
			} else {
				MainView.outputText( 'A small lake of semen surrounds you, ' );
			}
			MainView.outputText( 'released from your ' + CoC.player.multiCockDescriptLight() + ' by the intense climax.' );
			if( CoC.player.hasVagina() ) {
				MainView.outputText( '  ' );
			}
		}
		if( CoC.player.hasVagina() ) {
			if( CoC.player.wetness() < 2 ) {
				MainView.outputText( 'The scent of pussy hangs in the air, clinging to your moist, puffy cunt-lips.' );
			} else if( CoC.player.wetness() < 4 ) {
				MainView.outputText( 'The air is full of the scent of horny horse-pussy.  If the moist feeling between your hind-legs is any indication, your back-half probably looks and smells like a mare in heat.' );
			} else if( CoC.player.wetness() < 5 ) {
				MainView.outputText( 'The air is saturated with the heady scent of aroused horse-pussy, and if the wetness between your hind-legs is any indication, your hind-quarters would be a slip \'n slide of pleasure for any daring enough to penetrate you.' );
			} else {
				MainView.outputText( 'The air is filled with the thick musk of your bestial horse-pussy.  If the rivulets of moisture dripping down to your hooves are any indication, you might need to get used to the smell.' );
			}
		}
		if( CoC.player.hasCock() || CoC.player.hasVagina() ) {
			MainView.outputText( '\n\n' );
	...
	Masturbation.prototype.centaurHugeCock = function() {
		...
		MainView.outputText( 'You shrug out of your ' + CoC.player.armorName + ', not that it does much to impede your ' + CoC.player.multiCockDescriptLight() + ' as a centaur.   ' );
		if( CoC.player.cor < 33 ) {
			MainView.outputText( 'Sighing in disappointment and shame, ' );
		} else if( CoC.player.cor < 66 ) {
			MainView.outputText( 'Sighing in resignation, ' );
		} else {
			MainView.outputText( 'Smiling with excitement, ' );
		}
		MainView.outputText( 'you blush as ' );
		if( plural ) {
			MainView.outputText( 'each of ' );
		}
		MainView.outputText( 'your ' + CoC.player.multiCockDescriptLight() + ' swells out underneath you, hanging down from your equine hindquarters as ' );
		if( !plural ) {
			MainView.outputText( 'it grows full and hard.' );
		} else {
			MainView.outputText( 'they grow full and hard.' );
		}
		if( plural ) {
			MainView.outputText( '  After a moment they peek out from under your forelegs, proudly displaying your ' + CoC.player.multiCockDescriptLight() + '.' );
		} else {
			MainView.outputText( '  After a moment it peeks out from under your forelegs, proudly displaying your ' );
			if( CoC.player.cocks[ primary ].cockType === CockTypesEnum.HORSE ) {
				MainView.outputText( 'flared tip' );
			} else if( CoC.player.cocks[ primary ].cockType === CockTypesEnum.DEMON ) {
				MainView.outputText( 'nodule-ringed head' );
			} else if( CoC.player.cocks[ primary ].cockType === CockTypesEnum.DOG ) {
				MainView.outputText( 'pointed erection' );
			} else if( CoC.player.cocks[ primary ].cockType === CockTypesEnum.TENTACLE ) {
				MainView.outputText( 'wriggling, mushroom-like tip' );
			} else if( CoC.player.cocks[ primary ].cockType === CockTypesEnum.CAT ) {
				MainView.outputText( 'spiny head' );
			} else if( CoC.player.cocks[ primary ].cockType === CockTypesEnum.LIZARD ) {
				MainView.outputText( 'pointed, purple tip' );
			} else if( CoC.player.cocks[ primary ].cockType === CockTypesEnum.ANEMONE ) {
				MainView.outputText( 'tentacle-ringed head' );
			} else {
				MainView.outputText( 'dome-like head' );
			}
			MainView.outputText( '.' );
		}
		if( CoC.player.tallness * 1.5 < CoC.player.cocks[ CoC.player.longestCock() ].cockLength ) {
			MainView.outputText( '  In no time flat you\'re rendered virtually immobile by the length of ' );
			if( plural ) {
				MainView.outputText( 'all ' );
			}
			MainView.outputText( 'your ' + CoC.player.multiCockDescriptLight() + '.' );
		}
		MainView.outputText( '\n\n' );
		//Get started (not THAT long)
		if( CoC.player.tallness * 1.5 >= CoC.player.cocks[ CoC.player.longestCock() ].cockLength ) {
			MainView.outputText( 'Bending down and grabbing hold with both hands, you take ' );
			if( plural ) {
				MainView.outputText( 'a ' );
			} else {
				MainView.outputText( 'your ' );
			}
			MainView.outputText( CoC.player.cockDescript( primary ) + ' and lift up, pressing it tightly against your belly as you begin to stroke along the sensitive underside.  ' );
			if( CoC.player.cumQ() > 500 ) {
				MainView.outputText( 'Thick streams of pre-cum start to drizzle from your tip, forced out due to your prodigious production of sexual fluid.' );
			} else if( CoC.player.cumQ() > 100 ) {
				MainView.outputText( 'A dribble of pre-cum starts to leak from the tip as you get more into the act.' );
			} else {
				MainView.outputText( 'A tiny dollop of pre-cum slowly builds on your tip as you get into the act.' );
			}
			if( CoC.player.hasVagina() ) {
				MainView.outputText( '  Your neglected ' + CoC.player.vaginaDescript() + ' aches for penetration, but there\'s no way you\'ll be able to reach it in your current state.' );
			}
			MainView.outputText( '  You sigh, delighted that you\'re able to caress at least one of your sexual organs with this body' );
			if( CoC.player.cor < 40 ) {
				MainView.outputText( ', but you\'re also worried that you\'re falling into the perversion that lurks in this strange world' );
			}
			MainView.outputText( '.  It feels so good that you close your eyes and whinny with delight.\n\n' );
			MainView.outputText( 'You smear your pre-cum over the tip, rolling your palm across the sensitive tip in a way that makes it hard to stand.  Meanwhile your other hand is busy fondling the underside, stroking what little of your length you can reach.  Even with the limited access, you can feel your ' + CoC.player.cockDescript( primary ) + ' pulsing in your grip, growing harder with every touch and caress.' );
			if( CoC.player.totalCocks() === 2 ) {
				MainView.outputText( '  Your other dick mimics its lucky brother\'s pleasure, even though it\'s been ignored in order for you to focus on your current \'toy\'.' );
			} else if( CoC.player.totalCocks() > 2 ) {
				MainView.outputText( '  Your other ' + CoC.player.multiCockDescriptLight() + ' mimic their lucky brother\'s pleasure, even though they\'ve been ignored in order for you to focus on your current \'toy\'.' );
			}
			MainView.outputText( '  You let your hind legs give out and slump down, locking your forelegs while you tremble with what can only be the approach of an orgasm.\n\n' );
		}
		//Get started (long)
		else {
			MainView.outputText( 'Bending down and grabbing hold with both hands, you take ' );
			...
			MainView.outputText( CoC.player.cockDescript( primary ) + ' and lift it up, curving it slightly as you begin to stroke the underside with long, fluid caresses.  You even manage to bend down and give it a lick, forcing you into a pleasurable shudder.' );
			if( CoC.player.cumQ() > 500 ) {
				MainView.outputText( '  Thick streams of pre-cum start to drizzle from your tip, forced out due to your prodigious production of sexual fluid.' );
			} else if( CoC.player.cumQ() > 100 ) {
				MainView.outputText( '  A dribble of pre-cum starts to leak from the tip as you get more into the act.' );
			...
			MainView.outputText( '  You sigh, delighted that you\'re able to caress your sexual organs with this body' );
			if( CoC.player.cor < 33 ) {
				MainView.outputText( ', but you\'re also worried that you\'re falling into the perversion that lurks in this strange world' );
			}
			MainView.outputText( '.  It feels so good that you close your eyes and whinny with delight as your hands fondle your massive length.\n\n' );
			//STROKE (long)
		...
		//Cumjaculation
		if( CoC.player.tallness * 1.5 >= CoC.player.cocks[ CoC.player.longestCock() ].cockLength ) {
			MainView.outputText( 'Squeezing tightly around your ' + CoC.player.cockHead() );
		} else {
			MainView.outputText( 'Pumping hard' );
		}
		MainView.outputText( ', you orgasm, feeling relief flow through you with every pump of ' );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( 'your ' + CoC.player.ballsDescriptLight() );
		} else {
			MainView.outputText( 'release' );
		}
		MainView.outputText( '.  You arch your back as you blast out your seed, shuddering and spraying it into the air.  It splatters wetly against a rock, oozing down the now-slimy surface ' );
		if( CoC.player.cumQ() < 50 ) {
			MainView.outputText( 'as you finish unloading against it.' );
		} else if( CoC.player.cumQ() < 200 ) {
			MainView.outputText( 'as you continue to pump more and more seed onto it.' );
		} else if( CoC.player.cumQ() < 500 ) {
			MainView.outputText( 'as you continue to fountain more and more spooge over it.  By the time you finish there\'s a small puddle at the base and you\'ve thoroughly glazed the rock in question.' );
		} else if( CoC.player.cumQ() < 1500 ) {
			MainView.outputText( 'as you continue to blast it with cum.  Spooge puddles underneath it, slowly spreading while it soaks into the barren ground.' );
		} else {
			MainView.outputText( 'as you continue to erupt cum all over it.  The thick spooge forms a thick puddle at the base, spreading until your hooves are coated in it.' );
			if( CoC.player.cumQ() > 3000 ) {
				MainView.outputText( '  As you finish cumming, the puddle deepens.  It\'s inches thick and splattering everywhere whenever you take a step.' );
			}
		}
		MainView.outputText( '  You slump back, still shuddering from the pleasure as you feel your ' );
		if( !plural ) {
			MainView.outputText( 'member' );
		} else {
			MainView.outputText( 'members' );
		}
		if( CoC.player.hasSheath() ) {
			if( plural ) {
				MainView.outputText( ' slowly grow limp and slide back inside your sheath.' );
			} else {
				MainView.outputText( ' slowly growing limp and sliding back inside your sheath.' );
			}
		} else if( CoC.player.minLust() > 40 || CoC.player.lib > 50 ) {
			if( plural ) {
				MainView.outputText( ' grow slightly less hard.  As much as you seem to need sex, that doesn\'t surprise you.' );
			} else {
				MainView.outputText( ' growing slightly less hard.  As much as you seem to need sex, that doesn\'t surprise you.' );
			}
		} else {
			if( plural ) {
				MainView.outputText( ' slowly grow limp, shrinking.' );
			} else {
				MainView.outputText( ' slowly growing limp, shrinking.' );
			}
		}
		MainView.outputText( '\n\n' );
		//Cunt cums
		if( CoC.player.hasVagina() ) {
			if( CoC.player.wetness() <= 1 ) {
				MainView.outputText( 'A trickle of wetness oozes down your thighs, filling the air with the scent of centaur sex from your orgasm.' );
			} else if( CoC.player.wetness() === 2 ) {
				MainView.outputText( 'Fluids leak down your thighs, filling the air with the scent of centaur sex as you orgasm.' );
			} else if( CoC.player.wetness() === 3 ) {
				MainView.outputText( 'Thick, clear fluid soaks your thighs, filling the air with the scent of liquid centaur sex as you orgasm.' );
			} else if( CoC.player.wetness() === 4 ) {
				MainView.outputText( 'Thick fluid coats your thighs and drips into a puddle on the ground, filling the air with the pungent aroma of liquid centaur sex as you orgasm.' );
			} else {
				MainView.outputText( 'Spatters of fluids gush out from between your thighs, coating your upper legs in a thick coat of clear feminine lubricant as you orgasm.  The air is filled from the scent of your liquid centaur lust as more and more drips into a rapidly expanding puddle between your rear legs.' );
			}
			if( CoC.player.hasFuckableNipples() ) {
				if( CoC.player.totalNipples() > 2 ) {
					MainView.outputText( 'All' );
				...
				MainView.outputText( ' of your ' + Utils.num2Text( CoC.player.totalNipples() ) + ' ' + CoC.player.nippleDescript( 0 ) + 's quiver, ' );
				if( CoC.player.wetness() < 4 && CoC.player.biggestLactation() < 2 ) {
					MainView.outputText( 'leaking' );
				} else if( CoC.player.wetness() < 5 && CoC.player.biggestLactation() < 3 ) {
					MainView.outputText( 'dripping' );
				} else {
					MainView.outputText( 'squirting' );
				}
				MainView.outputText( ' equal quantities of ' );
				if( CoC.player.biggestLactation() < 1 ) {
					MainView.outputText( 'clear' );
				} else {
					MainView.outputText( 'milk-laced' );
				}
				MainView.outputText( ' lubricant.  You pause to finger their entrances, shivering unconsciously as you slowly come down.' );
			}
			MainView.outputText( '\n\n' );
		}
		MainView.outputText( 'Sated for now, you rest for an hour or so before climbing back atop your hooves.' );
		//DONE!
		CoC.player.orgasm();
		EngineCore.dynStats( 'sen', (-0.5) );
	};
	Masturbation.prototype.centaurCantMasturbate = function() {
		if( CoC.flags[ kFLAGS.UNABLE_TO_MASTURBATE_BECAUSE_CENTAUR ] === 0 ) {
			MainView.outputText( 'No matter how you twist and turn, you can\'t reach anywhere close to your ' );
			if( CoC.player.hasCock() || CoC.player.hasVagina() ) {
				MainView.outputText( 'genitalia' );
			} else {
				MainView.outputText( 'anything remotely sexual' );
			}
			MainView.outputText( '!  It seems that being a centaur has a rather crippling downside – you can\'t reach around to get yourself off and sate your lusts!\n\n' );
			CoC.flags[ kFLAGS.UNABLE_TO_MASTURBATE_BECAUSE_CENTAUR ]++;
		} else {
			MainView.outputText( 'You still can\'t reach around to masturbate yourself.  Being half-horse sure is inconvenient!\n\n' );
		}
		//(If Milker)
		if( CoC.player.hasKeyItem( 'Cock Milker - Installed At Whitney\'s Farm' ) >= 0 && CoC.player.cocks.length > 0 ) {
			MainView.outputText( 'Perhaps you could visit the cock-milker you have set up at Whitney\'s farm and drain off the arousal?  Or maybe you\'ll just have to find a willing partner somewhere.' );
		} else {
			MainView.outputText( 'It looks like you\'ll have to find a partner to relieve your pent up need, but in your current state you\'ll probably be on the receiving end of whatever you can get!' );
		}
	};
	...
	Masturbation.prototype.centaurDudesGetHorseAids = function() {
		...
		if( CoC.player.keyItemv1( 'Fake Mare' ) === 0 ) {
			if( CoC.player.cor < 50 ) {
				MainView.outputText( 'Deciding to give the mare-like cocksleeve you got from Whitney a try, you spend a few awkward minutes dragging the lump of metal off to someplace secluded and setting it up.  When you\'re done, you stand behind a wood-and-iron replica of a mare, adjusted to the perfect height for you.  Looking "<i>her</i>" over, your eyes are drawn to the slick black lips of the Onahole between her legs, craftily shaped like a horsecunt, and what looks like a second, smaller one above it simulating an anus.\n\n' );
			}//[If Med-High Corruption
			else {
				MainView.outputText( 'You decide to play with the mare-shaped cocksleeve Whitney gave you.  You pull it out of your stash and spend a few minutes setting it up in the heart of camp.  Once done, you stand behind a wood-and-iron replica of a mare, adjusted to the perfect height for you.  Looking "<i>her</i>" over, your eyes are drawn to the slick black lips of the Onahole between her legs, craftily shaped like a horsecunt, and what looks like a second, smaller one above it simulating an anus.\n\n' );
			}
		}
		MainView.outputText( 'Seeing the toy\'s exposed, gaping genitals, you feel a stirring in your ' + CoC.player.multiCockDescriptLight() + '.  You yearn to touch yourself, but your centaur lower body prevents you, as usual.  Grunting with annoyance, you trot up to the toy and give its wide cunt an experimental fisting.  ' );
		//[If small cock
		if( CoC.player.cockArea( x ) < 30 ) {
			MainView.outputText( 'Your hand slips in easily... since it\'s made for real horsecocks, it\'s a bit too big to give you any satisfaction.  Your gaze shifts upwards to the toy\'s fake anus, which seems a bit more your size.</i>"' );
		} else {
			MainView.outputText( 'Your fist slips in easily, and you give the toy a few preparatory thrusts to make sure it\'s nice and ready for your hefty cock.' );
		}
		MainView.outputText( '\n\n' );
		...
		if( CoC.player.cockArea( x ) < 30 ) {
			MainView.outputText( 'pressing against the tight ring of the toy\'s anus' );
		} else {
			MainView.outputText( 'lining up with the toy\'s gaping cunt' );
		}
		MainView.outputText( '.  You rear your ' + Descriptors.hipDescript() + ' and slam yourself into the mare\'s waiting hole.\n\n' );
		MainView.outputText( 'The toy\'s passage seems to shift and contract around your ' + CoC.player.cockDescript( x ) + ', molding itself to perfectly sheathe you.  What a marvelous little toy!  You slide on up until you hilt yourself, your crotch pressed against the mare\'s wide ass as your ' + CoC.player.chestDesc() + ' squeezes against her back.  Now fully mounted, you begin to rut on the mare toy, slapping your hips' );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( ' and ' + CoC.player.ballsDescriptLight() );
		}
		MainView.outputText( ' hard against her rump as you pound into her tight, slick ' );
		if( CoC.player.cockArea( x ) >= 30 ) {
			MainView.outputText( 'horsecunt' );
		} else {
			MainView.outputText( 'asshole' );
		}
		MainView.outputText( '.\n\n' );
		MainView.outputText( 'Finally able to get the release you weren\'t able to give yourself, you feel an orgasm mounting deep within you.  You let out a loud, equine whinny as you hump the toy faster and faster, pounding her with your ' + CoC.player.cockDescript( x ) + ' until the pleasure overwhelms you.  You cry out as you cum, squirting your centaur-cum as far into the toy as you can shoot it' );
		if( CoC.player.countCockSocks( 'gilded' ) > 0 && CoC.flags[ kFLAGS.GILDED_JERKED ] < CoC.player.countCockSocks( 'gilded' ) ) {
			CoC.flags[ kFLAGS.GILDED_JERKED ]++;
			var gems = this.midasCockJackingGemsRoll();
			...
			MainView.outputText( ' and you scramble to gather all ' + gems + ' of them up.</b> Satisfied you didn\'t miss any shinies, you disassemble the toy and haul it off back to your stash.' );
			CoC.player.gems += gems;
		} else {
			MainView.outputText( ', until she\'s full up and leaking onto the ground.\n\n' );
			MainView.outputText( 'Sated, you spend a few blissful minutes enjoying the warmth and tightness of the mare-like onahole until your ' + CoC.player.cockDescript( x ) + ' is soft inside it.  You scamper off of her, dropping back to your four equine feet.  With a contented yawn, you disassemble the toy and haul it off back to your stash, leaking your cum the entire way.' );
		}
		CoC.player.orgasm();
	...
	Masturbation.prototype.getHugeEggsInCawk = function() {
		...
		if( CoC.player.statusAffectv1( StatusAffects.Exgartuan ) === 1 && CoC.player.statusAffectv2( StatusAffects.Exgartuan ) === 0 && CoC.flags[ kFLAGS.TIMES_EGGED_IN_COCK ] === 0 ) {
			...
			MainView.outputText( 'You decide it\'s time for a little fun.' );
			MainView.outputText( '\n\nRemoving your [armor], you settle down ' );
			if( CoC.player.cor < 33 ) {
				MainView.outputText( 'in your bedroll' );
				if( SceneLib.camp.hasCompanions() ) {
					MainView.outputText( ', hoping one of your companions sees' );
				}
			} else if( CoC.player.cor < 66 ) {
				MainView.outputText( 'behind a rock to hide your sight if not sound' );
			} else {
				MainView.outputText( 'in the wastes just outside of camp to be out of sight and mind' );
			}
			MainView.outputText( ' while you take care of your endowments.' );
			...
			if( SceneLib.shouldraFollower.followerShouldra() ) {
				MainView.outputText( ', with Shouldra\'s assistance,' );
			}
			MainView.outputText( ' manage to fight him, keeping your cock in place.  With his attention distracted, you\'re able to lower your ovipositor directly to the tip of your cock, and with a flex and a shove, you manage to force it in.' );
			...
			if( CoC.player.balls > 0 ) {
				...
				if( CoC.player.fertilizedEggs() > 0 ) {
					MainView.outputText( ', and you know beyond a doubt that they\'ll be fertilized before much longer and you\'ll be able to lay your own eggs' );
				}
				MainView.outputText( '.' );
			}
			//[If no balls
			...
			MainView.outputText( ', egg after egg forcing its way into you until you have nothing left to give from your ovipositor, and it begins to withdraw from your length.  You finally pull it free with a loud \'schlick\' sound, and go limp against the ground as you recover from your ordeals.' );
			...
			MainView.outputText( 'Feeling a little bit randy, you slip ' );
			if( CoC.player.cor < 33 ) {
				MainView.outputText( 'into your bedroll' );
			} else if( CoC.player.cor < 66 ) {
				MainView.outputText( 'off behind a rock' );
			} else {
				MainView.outputText( 'into the trees outside of camp' );
			}
			MainView.outputText( ' and  strip yourself out of your [armor].  You can feel  your abdomen swaying heavily behind you, reminding you that it\'s been some time since you laid any eggs.  This presents you with a problem. You\'ll continue to feel full of eggs until you get rid of them, but you want to masturbate right now; there are no able receptors anywhere nearby.' );
			...
			if( CoC.player.gender === 3 ) {
				...
				MainView.outputText( ' as well as your pussy, and decide to try the latter first.  You flex your still-unfamiliar muscles and curl your abdomen back across yourself, straining to pull it into place, but it just isn\'t flexible enough to curl that far. Even using your arms, ' );
				if( CoC.player.str > 50 ) {
					MainView.outputText( 'even with your considerable strength, ' );
				}
				MainView.outputText( 'you cannot manage to bend it far enough to get there.  You actually start to feel a pain where your abdomen connects to the rest of your body, and you disregard this as a bad idea.' );
			}
			//[If male
			else {
			...
			if( CoC.player.cor >= 75 ) {
				MainView.outputText( 'corrupted ' );
			} else {
				MainView.outputText( 'transformed ' );
			}
			MainView.outputText( 'body.  You thrust deeper, feeling an egg beginning to slide through your ovipositor, and by now you\'re aching to feel it push itself down your cock.' );
			...
			if( CoC.player.balls > 0 ) {
				...
				if( !CoC.player.isTaur() ) {
					MainView.outputText( 'More begin to follow, and you massage your quickly-bloating balls as they fill with egg after egg after egg.  ' );
				}
				MainView.outputText( 'Your gut protests slightly, but with the tranquilizing effects of the honey and venom, the discomfort is nearly nonexistent.' );
				//[endballs]
			}
			...
			if( !CoC.player.isTaur() ) {
				MainView.outputText( ' as you idly fondle your ' );
				if( CoC.player.balls === 0 ) {
					MainView.outputText( 'swollen belly' );
				} else {
					MainView.outputText( 'bulging sack' );
				}
			}
			MainView.outputText( ', and you dream of laying your own eggs some time in the future.' );
		}
	...
	Masturbation.prototype.birthBeeEggsOutYourWang = function() {
		...
		if( CoC.player.balls > 0 ) {
			MainView.outputText( 'balls feel' );
		} else {
			MainView.outputText( 'stomach feels' );
		}
		MainView.outputText( ' heavy, and ' );
		if( CoC.player.isTaur() ) {
			MainView.outputText( 'y' );
		} else {
			MainView.outputText( 'you caress ' );
			if( CoC.player.balls > 0 ) {
				MainView.outputText( 'them' );
			} else {
				MainView.outputText( 'it' );
			}
			MainView.outputText( ' with the hand not already actively stroking your [cock biggest].  Y' );
		}
		MainView.outputText( 'ou feel something within you shifting; it\'s time to lay your eggs!' );
		MainView.outputText( '\n\nYou lie back on the ground and groan as your [cock biggest] quickly grows to full hardness in your hand, feeling your ' );
		if( CoC.player.balls === 0 ) {
			MainView.outputText( 'stomach' );
		} else {
			MainView.outputText( 'balls' );
		}
		MainView.outputText( ' shift as your eggs begin to attempt their journey outward.  Your strokes speed up; your lust only rises while the eggs move, and before much longer you\'re arching your back and thrusting your hips, practically fucking your hand, losing yourself to the pleasure of the act.' );
		...
		if( CoC.player.cor < 50 ) {
			...
			MainView.outputText( 'somewhat annoyed' );
		}
		MainView.outputText( ' as it manages to land directly on your nose.  When you helpfully pluck it off and place it on the ground, it looks up at you with a wave and a smile.' );
	...
	Masturbation.prototype.layEggsInYerTits = function() {
		...
		if( CoC.player.statusAffectv1( StatusAffects.Exgartuan ) === 2 && CoC.player.statusAffectv2( StatusAffects.Exgartuan ) === 0 ) {
			...
			MainView.outputText( 'Smiling mischieviously to yourself, you look down to your possessed [chest] and tell Exgartuan that you have something you very much would like to do for her.' );
			...
			//[if (cocks > 1)
			if( CoC.player.cocks.length > 1 ) {
				MainView.outputText( '  Your cocks, while still erect, are doing little more than dribbling cum over your breasts and tubes, devoid of the force and power they usually have.' );
			}//[if (cocks = 1)]
			else if( CoC.player.cocks.length === 1 ) {
				MainView.outputText( '  Your [cock], while still erect, does little more than dribble cum over your breasts and tubes, devoid of the force and power that usually comes with orgasm.' );
			}
			//[if (hasVagina = true)]
			if( CoC.player.hasVagina() ) {
				MainView.outputText( '  Your [vagina] lets sticky fluids out slowly around your thighs; you are only able to tell by the damp feel of your [legs].  It feels fantastic!' );
			}
			...
			MainView.outputText( 'Having decided to give in to your baser urges, you see no reason not to solve all your problems and lay the orbs that have been burdening you at the same time.  You look around a moment before beginning, [if (corruption < 50) concerned that you may be observed.][if (corruption > 50) hopeful for a target on which to unburden yourself instead.]' );
			...
			if( CoC.player.hasVagina() ) {
				if( CoC.player.balls > 0 ) {
					MainView.outputText( '  Behind your [balls], y' );
				} else {
					MainView.outputText( 'Y' );
				}
				MainView.outputText( 'our [vagina] begins to quietly drip and pull open slightly, and you wish for the flexibility to plant your eggs in it.' );
			}
			...
			if( CoC.player.canOvipositBee() ) {
				...
				MainView.outputText( 'green fluid ' );
			}
			MainView.outputText( 'squirts out of you under the sudden pressure from your penetration.' );
			...
			if( CoC.player.hasVagina() ) {
				MainView.outputText( '  Meanwhile, your [vagina] has fully parted and begun to drool ' );
				if( CoC.player.wetness() >= 4 ) {
					MainView.outputText( 'liberally ' );
				}
				MainView.outputText( 'onto your [legs] and abdomen.' );
			}
			...
			if( CoC.player.eggs() < 20 ) {
				...
				MainView.outputText( '.  Quickly, you thrust it into your other breast and let the egging continue.  Suddenly conscious of your situation, you hold your previous [nipple] closed.' );
				if( CoC.player.hasCock() ) {
					MainView.outputText( '  Your [chest] and ovipositor meanwhile are slowly being coated in white from your languid pleasure.' );
				}
	...
	Masturbation.prototype.tentacleSelfFuck = function() {
		...
		MainView.outputText( 'Almost immediately, your ' + Descriptors.cockDescript( x ) + ' perks up like a pet expecting to be fed, and you have to admit that you plan to give that squirming tentacle exactly what it desires - a hot, slippery slit to nestle inside of.  Already, your [vagina] has grown ' );
		if( CoC.player.wetness() <= 2 ) {
			MainView.outputText( 'moist' );
		} else if( CoC.player.wetness() <= 3 ) {
			MainView.outputText( 'wet' );
		} else if( CoC.player.wetness() <= 4 ) {
			MainView.outputText( 'sloppy and wet' );
		} else {
			MainView.outputText( 'beyond soaked' );
		}
		MainView.outputText( '.  The slick slit is slowly parting as you reach to wrangle the wiggly cock' );
		if( CoC.player.cocks.length > 1 ) {
			MainView.outputText( ', brushing your hand against your other ' );
			if( CoC.player.lust <= 70 ) {
				MainView.outputText( 'half-hard' );
			} else {
				MainView.outputText( 'erect' );
			}
			MainView.outputText( ' penis' );
			if( CoC.player.cocks.length > 2 ) {
				MainView.outputText( 'es' );
			}
			MainView.outputText( ' on the way' );
		}
		MainView.outputText( '.  Moaning out loud, you try your best to handle the flood of alien sensations, but the pleasure-sparking tendril in your grip feels so different from a human penis.  It\'s almost like you\'re compelled to thrust it inside of some orifice, any orifice, so long as it\'s somewhere warm and tight.' );
		if( y >= 0 ) {
			MainView.outputText( '  It loops around your ' + Descriptors.cockDescript( y ) + ' while you fight with it, strangling the other phallus in tight coils of squeezing, floral friction.' );
		}
		...
		if( y >= 0 ) {
			MainView.outputText( '  Your ' + Descriptors.cockDescript( y ) + ' is getting jacked off by the engorged cock-coil\'s motions and slowly leaks creamy pre over the jerking length.' );
			if( CoC.player.cocks[ y ].cockType === CockTypesEnum.TENTACLE ) {
				MainView.outputText( '  In no time flat the second tendril has gotten the idea, and it elongates to reach for your unoccupied asshole.  There\'s a moment of token resistance before it violates your [asshole], but then, there\'s only the warm heat of a torrid butt-fuck.' );
		...
		if( CoC.player.biggestTitSize() >= 1 ) {
			MainView.outputText( ', kneading the soft mammary' );
		}
		MainView.outputText( ' and twisting your [nipple] as your body goes white-hot with pleasure' );
		if( CoC.player.hasFuckableNipples() ) {
			MainView.outputText( ', even stuffing a finger inside a sloppy, dripping nipplecunt' );
		}
		MainView.outputText( '.' );
		if( CoC.player.lactationQ() >= 250 ) {
			MainView.outputText( '  Milk squirts from your engorged teat almost immediately to fall in a moist, creamy rain across your writhing form.' );
		}
		//{no new PG}  Three + Tentalce fork - one in mouth
		if( CoC.player.tentacleCocks() >= 3 ) {
			MainView.outputText( '  The pleasured noises that have been issuing forth from your \'O\'-gaped lips are cut off by a sudden intrusion from another of your arboreal phalluses.   This one punches straight into your throat without pause, sliding so smoothly across your tongue that you barely care about gagging when it feels so good.  Trickles of your sweet pre-cum are dribbling out from [eachCock] into your holes' );
			if( CoC.player.tentacleCocks() < CoC.player.cocks.length ) {
				MainView.outputText( ' and the open air' );
			}
			MainView.outputText( '.' );
			if( CoC.player.biggestTitSize() >= 4 ) {
				MainView.outputText( '  Laying right between your boobs, it doesn\'t take long for the shaft to extend far enough to loop around each tit, sliding slowly encircling each curvy mound as it presses them together around itself, getting a titfuck while it plows your unresisting mouth.' );
			}
		}
		//No bonus cawks!
		else {
			MainView.outputText( '  The pleasured noises issuing forth from your \'O\'-gaped lips get higher and higher pitched with each passing second, and for a split second, you find yourself wishing you had a third tentacle so you could suck it while you fuck yourself.' );
			if( y >= 0 && CoC.player.cocks[ y ].cockType === CockTypesEnum.TENTACLE && CoC.player.cocks[ y ].cockLength >= 20 ) {
				MainView.outputText( '  Luckily, you\'re so big down there that a juicy cock-head is right there within reach, and you bend to slurp it up without thought, busily self-sucking with reckless abandon.' );
			}
		}
		//MORE
		if( CoC.player.tentacleCocks() >= 6 ) {
			...
			if( CoC.player.tentacleCocks() >= 7 ) {
				MainView.outputText( '  They pile in there, really squirming against each other as much as you, frotting against your sweat-slick flesh.  ' );
			}
			MainView.outputText( '  You\'re fucking your armpits, and it feels divine, so good that your pits are swampy pits of sex within moments.' );
		}
		//One more
		...
		MainView.outputText( '.  Orgasm is rapidly closing in, and there\'s no slowing your frenzied flora at this point.  You ' );
		if( CoC.player.tentacleCocks() >= 3 ) {
			MainView.outputText( 'gurgle' );
		} else {
			MainView.outputText( 'moan' );
		}
		MainView.outputText( ' as your inner muscles begin to contract into tight knots, the pressure building to a turgid, throbbing peak.  Then, as you' );
		if( CoC.player.tentacleCocks() >= 3 ) {
			MainView.outputText( 'r multitude of penises' );
		}
		MainView.outputText( ' thrust deep inside yourself, a volcano of pleasure erupts, pumping thick flows of white goo straight into your snatch' );
		if( CoC.player.tentacleCocks() === 2 ) {
			MainView.outputText( ' and ass' );
		} else if( CoC.player.tentacleCocks() > 2 ) {
			MainView.outputText( ', ass, and mouth' );
		}
		MainView.outputText( '.' );
		if( CoC.player.tentacleCocks() >= 3 ) {
			MainView.outputText( '  You gulp the salty flow down as best you are able' );
			if( CoC.player.cumQ() >= 500 ) {
				MainView.outputText( '.  That doesn\'t really count for much with how heavy your load is, ' );
				if( CoC.player.cumQ() < 1000 ) {
					MainView.outputText( 'and your cheeks bulge with the size of each squirt' );
				} else {
					MainView.outputText( 'and the cum is soon squirting out the corners of your mouth while your cheeks bulge cartoonishly' );
				}
			}
			MainView.outputText( '.' );
		}
		MainView.outputText( '  Your birth canal is quickly flooded with white spooge' );
		if( CoC.player.tentacleCocks() >= 2 ) {
			MainView.outputText( ', while your [ass]\'s interior is painted bright white' );
		}
		MainView.outputText( '.' );
		if( CoC.player.tentacleCocks() === 2 ) {
			MainView.outputText( '  Your untended ' + Descriptors.cockDescript( y ) + ' blows spunk over you ' );
			if( CoC.player.cumQ() < 50 ) {
				MainView.outputText( 'in small spurts' );
			} else if( CoC.player.cumQ() < 200 ) {
				MainView.outputText( 'in thick ropes' );
			} else if( CoC.player.cumQ() < 500 ) {
				MainView.outputText( 'in huge blobs' );
			} else {
				MainView.outputText( 'like a virile sprinkler' );
			}
			MainView.outputText( '.' );
			if( CoC.player.cocks.length > 2 ) {
				MainView.outputText( '  The other one' );
				if( CoC.player.cocks.length > 3 ) {
					MainView.outputText( 's match' );
				} else {
					MainView.outputText( ' matches' );
				}
				MainView.outputText( ' it in output, even though you haven\'t done anything to stimulate ' );
				if( CoC.player.cocks.length === 3 ) {
					MainView.outputText( 'it' );
				} else {
					MainView.outputText( 'them' );
				}
				MainView.outputText( '.  The sensations coming from your prehensile penises are just so overwhelming that it\'s like a whole-body-gasm.' );
			}
		} else if( CoC.player.tentacleCocks() >= 4 ) {
			MainView.outputText( '  Meanwhile, the purple tip' );
			if( CoC.player.tentacleCocks() === 4 ) {
				MainView.outputText( 's swell' );
			} else {
				MainView.outputText( ' swells' );
			}
			MainView.outputText( ' in your armpit' );
			if( CoC.player.tentacleCocks() >= 5 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ', engorging immensely as they press right into the soft, concave flesh within, the semen squirting out in pressured, arm-soaking jets.' );
		}
		MainView.outputText( '\n\nAt once, your whole body sags back, limp and drained.  The elongated tentacle' );
		if( CoC.player.tentacleCocks() > 1 ) {
			MainView.outputText( 's retract' );
		} else {
			MainView.outputText( ' retracts' );
		}
		MainView.outputText( ' back to ' );
		if( CoC.player.tentacleCocks() === 1 ) {
			MainView.outputText( 'its' );
		} else {
			MainView.outputText( 'their' );
		}
		MainView.outputText( ' normal length, popping out of your vagina' );
		if( CoC.player.tentacleCocks() === 2 ) {
			MainView.outputText( ' and ass' );
		} else if( CoC.player.tentacleCocks() > 2 ) {
			MainView.outputText( ', ass, and mouth' );
		}
		MainView.outputText( ', leaving your orifice' );
		if( CoC.player.tentacleCocks() > 1 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ' to dribble the spent passion on the ground.  Damn, that was satisfying.' );
	...
	Masturbation.prototype.tentacleGoesUpYerPooperNewsAtEleven = function() {
		...
		if( CoC.player.cumQ() >= 1000 ) {
			MainView.outputText( ', making [eachCock] bulge.  The extra feeling sends you over the edge and you quickly reach your climax as you cum and cum' );
		}
		MainView.outputText( '.' );
		//[if cum production is moderate]
		if( CoC.player.cumQ() >= 500 ) {
			MainView.outputText( '  Your belly swells a bit from all the semen being packed inside you.' );
		}
		//[if cum production is massive]
		if( CoC.player.cumQ() >= 1500 ) {
			MainView.outputText( '  Your poor insides cannot handle the enormous cumshot being unloaded in your [asshole] and a significant volume of spunk dribbles outside, carelessly polluting the floor.' );
		}
		//[Standard text for other cocks cumming goes here.]
		MainView.outputText( '  You groan and lazily remove your ' + CoC.player.cockDescript( tentacle ) + ' from your anus as you give in to your pleasure-induced drowsiness.' );
		if( CoC.player.cocks.length > 0 ) {
			//Single Cock
			if( CoC.player.cocks.length === 1 ) {
				if( CoC.player.lib < 30 ) {
	...
	Masturbation.prototype.gildedCockTurbate = function() {
		...
		MainView.outputText( 'You disrobe, shivering softly.  Biting your lip, you look down, realizing that though the day is warm the gleaming metallic sleeve on your cock is cool, almost chill.' );
		...
		if( CoC.player.cocks[ gildedCock ].cArea() < 6 ) {
			MainView.outputText( '  Enveloped in the cocksock, you can barely see the tip of your tiny dick as your head peeks out on every downstroke.  The rounded edge of the metallic sleeve rubs maddeningly against the head of your cock, cool and smooth.' );
		}//[medium dicks]
		else if( CoC.player.cocks[ gildedCock ].cArea() < 20 ) {
			MainView.outputText( '  The head of your cock abuts against the edge of the cocksock, bumping against it with every stroke.  With your thickness, the cocksock is a perfect fit, tugging on your cock with every upstroke, stretching your dick lightly as you jack yourself.' );
		}//[large dicks]
		else {
			MainView.outputText( '  Clamped around your dick, the metal casing looks more like a gigantic cock ring. The golden sheath actually allows you a better grip on your massive member, a ring of coolness around your hot meat.' );
		}
		...
		if( CoC.player.cumQ() < 25 ) {
			MainView.outputText( 'a sprinkle of' );
		} else if( CoC.player.cumQ() < 250 ) {
			MainView.outputText( 'a rain of' );
		} else {
			MainView.outputText( 'a torrent of' );
		}
		MainView.outputText( ' gems falls down upon your body instead of cum, bouncing off your ' + CoC.player.skinFurScales() + '.' );
		var gems = this.midasCockJackingGemsRoll();
	...
	Masturbation.prototype.stickADildoInYourVagooSlut = function() {
		...
		if( CoC.player.hasVirginVagina() ) { //LOW CORRUPTION DEFLOWER
			if( CoC.player.cor <= 50 ) {
				MainView.outputText( 'You blush nervously as you grasp your phallic toy. The firm dildo bends slightly as you hold it in your hand. You feel perverse to consider doing this, and shakingly undo the bottom of your [armor].' );
				...
				if( CoC.player.wetness() <= 2 ) {
					MainView.outputText( 'The toy slides easier as your juices cover it.' );
				} else {
					MainView.outputText( 'The toy slides effortlessly thanks to your copious, almost inhuman lubrication.' );
				}
		...
		//Repeat Dildo For Ladyboners
			...
			MainView.outputText( 'You remove your [armor] and sit yourself down behind a rock not far from camp, being sure to bring your toy with you.' );
	...
	Masturbation.prototype.dildoButts = function() {
		MainView.clearOutput();
		MainView.outputText( 'A kinky idea crosses your mind, and you grab your dildo. Finding a safe spot a short distance from camp, you undo your [armor] and rest on your [butt].' );
		...
		if( CoC.player.analCapacity() <= 8 ) {
			MainView.outputText( 'Your tight hole offers resistance at first, but soon relaxes with a tender, lubricated fingering.' );
		} else {
			MainView.outputText( 'Your experienced hole readily accepts your saliva-coated finger.' );
		}
		MainView.outputText( ' As you gently rub your insides, you use your free hand and reach up to pinch and rub your [nipples]. As the motions become easier, you push a second finger inside as well. You soon find your sensitive anal entrance begging for something more appropriate, and you happily oblige.' );
		...
		MainView.outputText( 'Your body shakes and rocks from the anal orgasm before slumping onto your back. Happily tightening around the toy with each beat of your hammering heart, you rest.' );
	...
	Masturbation.prototype.onaholeRepeatUse = function( corrupted ) {
		...
		if( corrupted ) {
			...
			if( gemsCreated > 0 ) {
				MainView.outputText( '. You pop the toy from your rod and let your reflexes hump the sky.' );
				...
				if( CoC.player.cumQ() < 25 ) {
					MainView.outputText( 'sprinkle' );
				} else if( CoC.player.cumQ() < 250 ) {
					MainView.outputText( 'rain' );
				} else {
					MainView.outputText( 'torrent' );
				}
				MainView.outputText( ' of gems falls down upon your body instead of cum, bouncing off your ' + CoC.player.skinFurScales() + '.\n\n<b>You force yourself to pick up the ' + gemsCreated + ' gems before passing out.</b>' );
			} else {
				MainView.outputText( 'as multiple streams of semen erupt from your dong, creating an impressive mess about you. Soaked in your own fluids, you take a moment to clean yourself up before replacing the toy in your bag and going to sleep, happy to be relieved of your urges.' );
			}
		}
	};
