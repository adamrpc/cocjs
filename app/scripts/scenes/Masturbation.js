...
angular.module( 'cocjs' ).run( function( SceneLib, $log, CoC, kFLAGS, EngineCore, PerkLib, StatusAffects, Utils, EventParser, CockTypesEnum, AppearanceDefs, Descriptors ) {
	...
	Masturbation.prototype.masturbateMenu = function() {
		EngineCore.menu();
		if( CoC.getInstance().player.hasCock() && (CoC.getInstance().player.cocks[ 0 ].cockType === CockTypesEnum.BEE) ) {
		...
		if( (CoC.getInstance().player.findPerk( PerkLib.HistoryReligious ) >= 0 && CoC.getInstance().player.cor <= 66) || (CoC.getInstance().player.findPerk( PerkLib.Enlightened ) >= 0 && CoC.getInstance().player.cor < 10) ) {
			if( CoC.getInstance().player.findStatusAffect( StatusAffects.Exgartuan ) >= 0 && CoC.getInstance().player.statusAffectv2( StatusAffects.Exgartuan ) === 0 ) {
				EngineCore.addButton( button++, 'Masturbate', this.masturbateGo );
			} else {
				EngineCore.addButton( button++, 'Meditate', this.meditate );
			}
		} else {
			EngineCore.addButton( button++, 'Masturbate', this.masturbateGo );
		}
		//catofellato
		if( CoC.getInstance().player.hasCock() && (CoC.getInstance().player.findPerk( PerkLib.Flexibility ) >= 0 || CoC.getInstance().flags[ kFLAGS.TIMES_AUTOFELLATIO_DUE_TO_CAT_FLEXABILITY ] > 0) ) {
			EngineCore.addButton( button++, 'Lick Cock', this.catAutoLick );
		}
		if( CoC.getInstance().player.hasVagina() && (CoC.getInstance().player.findPerk( PerkLib.Flexibility ) >= 0 || CoC.getInstance().flags[ kFLAGS.TIMES_AUTOFELLATIO_DUE_TO_CAT_FLEXABILITY ] > 0) ) {
			EngineCore.addButton( button++, 'Lick \'Gina', this.lickYerGirlParts );
		}
		if( CoC.getInstance().player.tentacleCocks() > 0 && CoC.getInstance().player.hasVagina() ) {
			EngineCore.addButton( button++, 'Tentapussy', this.tentacleSelfFuck );
		}
		if( CoC.getInstance().player.tentacleCocks() > 0 ) {
			EngineCore.addButton( button++, 'Tentabutt', this.tentacleGoesUpYerPooperNewsAtEleven );
		}
		if( CoC.getInstance().player.canOvipositBee() && CoC.getInstance().player.lust >= 33 && CoC.getInstance().player.biggestCockArea() > 100 ) {
			EngineCore.addButton( button++, 'LayInCock', this.getHugeEggsInCawk );
		}
		if( CoC.getInstance().player.canOviposit() && CoC.getInstance().player.hasFuckableNipples() && CoC.getInstance().player.lust >= 33 && CoC.getInstance().player.biggestTitSize() >= 21 ) {
			EngineCore.addButton( button++, 'LayInTits', this.layEggsInYerTits );
		}
		if( this.fappingItems( false ) ) {
			EngineCore.addButton( 8, 'Items', this.fappingItems );
	...
	Masturbation.prototype.fappingItems = function( menus ) {
		...
		var canReachCock = CoC.getInstance().player.cocks.length > 0 && (!CoC.getInstance().player.isTaur() || CoC.getInstance().player.cocks[ CoC.getInstance().player.longestCock() ].cockLength >= CoC.getInstance().player.tallness * (5 / 6));
		if( CoC.getInstance().player.hasKeyItem( 'Deluxe Dildo' ) >= 0 && CoC.getInstance().player.hasVagina() && !CoC.getInstance().player.isTaur() ) {
			if( menus ) {
				EngineCore.addButton( button, 'D. Dildo', this.deluxeDildo );
			}
			button++;
		}
		if( CoC.getInstance().player.hasKeyItem( 'All-Natural Onahole' ) >= 0 && canReachCock ) {
			if( menus ) {
				EngineCore.addButton( button, 'AN Onahole', this.allNaturalOnaholeUse );
			}
			button++;
		}
		if( CoC.getInstance().player.hasKeyItem( 'Deluxe Onahole' ) >= 0 && canReachCock ) {
			if( menus ) {
				EngineCore.addButton( button, 'D Onahole', this.deluxeOnaholeUse );
			}
			button++;
		}
		if( CoC.getInstance().player.hasKeyItem( 'Plain Onahole' ) >= 0 && canReachCock ) {
			if( menus ) {
				EngineCore.addButton( button, 'Onahole', this.onaholeUse );
			}
			button++;
		}
		if( CoC.getInstance().player.hasKeyItem( 'Self-Stimulation Belt' ) >= 0 && CoC.getInstance().player.vaginas.length > 0 && !CoC.getInstance().player.isTaur() ) {
			if( menus ) {
				EngineCore.addButton( button, 'Stim-Belt', this.stimBeltUse );
			}
			button++;
		}
		if( CoC.getInstance().player.hasKeyItem( 'All-Natural Self-Stimulation Belt' ) >= 0 && CoC.getInstance().player.vaginas.length > 0 && !CoC.getInstance().player.isTaur() ) {
			if( menus ) {
				EngineCore.addButton( button, 'AN Stim-Belt', this.allNaturalStimBeltUse );
			}
			button++;
		}
		if( CoC.getInstance().player.hasKeyItem( 'Dual Belt' ) >= 0 && CoC.getInstance().player.gender === 3 && !CoC.getInstance().player.isTaur() ) {
			if( menus ) {
				EngineCore.addButton( button, 'Dual Belt', this.dualBeltMasturbation );
			}
			button++;
		}
		if( CoC.getInstance().player.hasKeyItem( 'Fake Mare' ) >= 0 && CoC.getInstance().player.hasCock() && CoC.getInstance().player.isTaur() ) {
			if( menus ) {
				EngineCore.addButton( button, 'Fake Mare', this.centaurDudesGetHorseAids );
			}
			button++;
		}
		if( CoC.getInstance().player.hasKeyItem( 'Centaur Pole' ) >= 0 && CoC.getInstance().player.hasVagina() && CoC.getInstance().player.isTaur() ) {
			if( menus ) {
				EngineCore.addButton( button, 'C. Pole', this.centaurGirlsGetHorseAids );
			}
			button++;
		}
		if( CoC.getInstance().player.hasKeyItem( 'Dildo' ) >= 0 ) {
			if( menus ) {
				EngineCore.addButton( button, 'Anal Dildo', this.dildoButts );
			}
			button++;
			if( CoC.getInstance().player.hasVagina() ) {
				EngineCore.addButton( button, 'Dildo', this.stickADildoInYourVagooSlut );
			}
		}
		if( menus ) {
			EngineCore.addButton( 9, 'Back', this.masturbateMenu );
		}
		return button > 0;
	};
	...
	Masturbation.prototype.masturbateGo = function() {
		...
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.Exgartuan ) >= 0 && CoC.getInstance().player.statusAffectv2( StatusAffects.Exgartuan ) === 0 ) {
			if( CoC.getInstance().player.isNaga() && Utils.rand( 2 ) === 0 && CoC.getInstance().player.statusAffectv1( StatusAffects.Exgartuan ) === 1 ) {
				CoC.getInstance().exgartuan.exgartuanNagaStoleMyMasturbation();
			} else {
				CoC.getInstance().exgartuan.exgartuanMasturbation();
			}
			return;
		}
		if( CoC.getInstance().player.countCockSocks( 'gilded' ) > 0 && CoC.getInstance().flags[ kFLAGS.GILDED_JERKED ] < CoC.getInstance().player.countCockSocks( 'gilded' ) ) {
		...
		} else if( CoC.getInstance().player.cor < 30 ) {
			EngineCore.outputText( 'You make sure you are alone and strip naked.\n\n' );
		} else if( CoC.getInstance().player.cor < 60 ) {
			EngineCore.outputText( 'You happily remove your ' + CoC.getInstance().player.armorName + ', eager to pleasure yourself.\n\n' );
		} else if( CoC.getInstance().player.cor < 80 ) {
		...
		if( CoC.getInstance().player.cocks.length > 0 ) {
			if( CoC.getInstance().player.cocks.length === 1 ) {
				EngineCore.outputText( 'You stroke your ' + CoC.getInstance().player.cockDescript() );
				if( CoC.getInstance().player.lib < 45 ) {
		...
		if( CoC.getInstance().player.vaginas.length > 0 ) {
			if( CoC.getInstance().player.vaginas.length === 1 ) {
				...
				if( CoC.getInstance().player.lib < 45 ) {
					EngineCore.outputText( 'You touch and play with your ' + CoC.getInstance().player.vaginaDescript() + ', ' );
				} else if( CoC.getInstance().player.lib < 70 ) {
					EngineCore.outputText( 'You slap your pussy softly, ' );
				} else {
					EngineCore.outputText( 'You touch your enflamed and aroused ' + CoC.getInstance().player.vaginaDescript() + ', ' );
				}
				switch( CoC.getInstance().player.vaginas[ 0 ].vaginalWetness ) {
					...
					case AppearanceDefs.VAGINA_WETNESS_DROOLING:
						EngineCore.outputText( 'slicking your fingers in the juices that constantly dribble from ' + CoC.getInstance().player.vaginaDescript() + '  ' );
						break;
					default:
			...
			if( CoC.getInstance().player.vaginas.length > 1 ) {
				if( CoC.getInstance().player.lib < 45 ) {
					...
					EngineCore.outputText( 'Touch your enflamed and aroused ' + CoC.getInstance().player.vaginaDescript() + 's, ' );
				}
				switch( CoC.getInstance().player.vaginas[ 0 ].vaginalWetness ) {
					case AppearanceDefs.VAGINA_WETNESS_DRY:
						EngineCore.outputText( 'expertly arousing your female parts.  ' );
					...
					case AppearanceDefs.VAGINA_WETNESS_SLICK:
						...
						break;
					case AppearanceDefs.VAGINA_WETNESS_DROOLING:
						EngineCore.outputText( 'slicking your fingers in the juices that constantly dribble from ' + CoC.getInstance().player.vaginaDescript() + 's  ' );
						break;
					default:
		...
		if( CoC.getInstance().player.cocks.length === 1 ) {
			...
			if( CoC.getInstance().player.cocks[ 0 ].cockThickness < 1.8 ) {
				EngineCore.outputText( 'You easily wrap a hand around your ' + CoC.getInstance().player.cockDescript() + ' and start masturbating.  ' );
			} else if( CoC.getInstance().player.cocks[ 0 ].cockThickness < 3 ) {
				EngineCore.outputText( 'You have some difficulty fitting your hand around your ' + CoC.getInstance().player.cockDescript() + ', relishing the feelings of your ' );
				if( CoC.getInstance().player.cocks[ 0 ].cockType === CockTypesEnum.HORSE ) {
					EngineCore.outputText( 'animalistic' );
				} else if( CoC.getInstance().player.hasKnot() ) {
					EngineCore.outputText( 'bulbous beast' );
				} else {
					EngineCore.outputText( 'large' );
				}
				EngineCore.outputText( ' endowment as you begin masturbating.  ' );
			} else if( CoC.getInstance().player.cocks[ 0 ].cockThickness < 5 ) {
				EngineCore.outputText( 'You use both hands to grip your ' + CoC.getInstance().player.cockDescript() + ', feeling the throbbing of your ' + (CoC.getInstance().player.hasKnot() ? 'knot' : 'member') + ' as you begin to masturbate.  ' );
			} else {
				EngineCore.outputText( 'You grasp onto your ' + CoC.getInstance().player.cockDescript() + ' with both hands, but fail to encircle it fully as you begin to masturbate.  ' );
			}
			//If length > 12 proud
			...
			if( CoC.getInstance().player.cocks[ 0 ].cockLength < 12 ) {
				...
				} else if( CoC.getInstance().player.horseCocks() >= 1 ) {
				...
				} else if( CoC.getInstance().player.dogCocks() >= 1 ) {
				...
				} else if( CoC.getInstance().player.tentacleCocks() >= 1 ) {
				...
				} else if( CoC.getInstance().player.demonCocks() >= 1 ) {
				...
				} else if( CoC.getInstance().player.catCocks() >= 1 ) {
					EngineCore.outputText( 'You stroke quickly, feeling the tiny \'barbs\' of your ' + CoC.getInstance().player.cockDescript() + ' sliding through your fingers, even darting down to circle the sensitive skin around your sheath.  ' );
				} else if( CoC.getInstance().player.lizardCocks() >= 1 ) {
					EngineCore.outputText( 'You stroke quickly, pleasuring your sensitive ' + CoC.getInstance().player.cockDescript() + ', sliding fingers over each ridge and bump that covers its knotty length.  ' );
				} else if( CoC.getInstance().player.anemoneCocks() >= 1 ) {
					EngineCore.outputText( 'You stroke quickly, gasping as your fingers are stung repeatedly with the aphrodisiac-laced tentacles around the base of your ' + CoC.getInstance().player.cockDescript() + ' and under its crown.  ' );
				} else if( CoC.getInstance().player.displacerCocks() >= 1 ) {
				...
				if( CoC.getInstance().player.normalCocks() >= 1 ) {
					EngineCore.outputText( 'You delight in teasing the crown of your ' + CoC.getInstance().player.cockDescript() + ', rubbing it at the end of each stroke, squeezing out dollops of pre to smear over it and tease yourself with.  It seems to pulse and twitch with each stroke, responding to every touch.  ' );
				} else if( CoC.getInstance().player.horseCocks() >= 1 ) {
					EngineCore.outputText( 'You delight in teasing the sensitive flared tip of your ' + CoC.getInstance().player.cockDescript() + ', rubbing it at the end of each stroke, squeezing out dollops of pre to smear over it and tease yourself with.  It seems to pulse and ripple with each stroke, responding to every touch.  ' );
				} else if( CoC.getInstance().player.dogCocks() >= 1 ) {
					EngineCore.outputText( 'You delight in teasing the pointed tip of your ' + CoC.getInstance().player.cockDescript() + ', rubbing it at the end of each stroke, squeezing out dollops of pre to smear over it and tease yourself with.  Your knot seems to pulse and twitch with each stroke, reacting to every touch.  ' );
				} else if( CoC.getInstance().player.tentacleCocks() >= 1 ) {
					EngineCore.outputText( 'You delight in teasing the over-sized mushroom-like tip of your ' + CoC.getInstance().player.cockDescript() + ', caressing it after every stroke as you squeeze out dollops of pre to smear along its slimy length.  It writhes and twists in your hands of it\'s own volition, lengthening and shortening with each set of strokes.  ' );
				} else if( CoC.getInstance().player.demonCocks() >= 1 ) {
					EngineCore.outputText( 'You delight in teasing the larger bumps that form a ring around the crown of your ' + CoC.getInstance().player.cockDescript() + ', watching as they twitch and spasm in time with the dollops of pre you\'re squeezing out and smearing over the entire length.  ' );
				} else if( CoC.getInstance().player.catCocks() >= 1 ) {
					EngineCore.outputText( 'You delight in teasing the sensitive nubs all along your ' + CoC.getInstance().player.cockDescript() + ', circling the cock-tip at the end of each stroke to gather pre and slather it over your entire length.  Each of the tiny \'barbs\' provides bursts of pleasure with each stroke, driving you on.  ' );
				} else if( CoC.getInstance().player.lizardCocks() >= 1 ) {
					EngineCore.outputText( 'You delight in teasing the rounded bulbs that cover your ' + CoC.getInstance().player.cockDescript() + ', circling them with your fingertips before sliding up to the urethra and gathering a drop of pre.  You smear it over your sensitive reptile skin and revel in the pleasure radiating through you.  ' );
				} else if( CoC.getInstance().player.anemoneCocks() >= 1 ) {
					EngineCore.outputText( 'You delight in grabbing hold of the tiny, stinging tentacles around the base and squeezing them between your ' + CoC.getInstance().player.cockDescript() + ' and hand.  Aphrodisiac pours into your blood as you release them and stroke along the length, gathering dollops of pre to coat yourself with and \'accidentally\' bumping the other tentacles at the crown.  ' );
				} else if( CoC.getInstance().player.displacerCocks() >= 1 ) {
					EngineCore.outputText( 'You delight in teasing the opened tip of your ' + CoC.getInstance().player.cockDescript() + ', rubbing it at the end of each stroke, watching it squeeze out dollops of pre that you smear over it to tease yourself with.  Your knot seems to pulse and twitch with each stroke, reacting to every touch.  ' );
				} else {
					EngineCore.outputText( 'You delight in teasing the crown of your ' + CoC.getInstance().player.cockDescript() + ', rubbing it at the end of each stroke, squeezing out dollops of pre to smear over it and tease yourself with.  It seems to pulse and twitch with each stroke, responding to every touch.  ' );
				}
			} else if( CoC.getInstance().player.cocks[ 0 ].cockLength < 26 ) {
				if( CoC.getInstance().player.normalCocks() >= 1 ) {
					EngineCore.outputText( 'The head of your ' + CoC.getInstance().player.cockDescript() + ' wobbles towards your face as you masturbate, a dollop of pre slowly growing atop it.  ' );
				} else if( CoC.getInstance().player.horseCocks() >= 1 ) {
					EngineCore.outputText( 'The flared tip of your ' + CoC.getInstance().player.cockDescript() + ' wobbles towards your face as you masturbate, a dollop of pre slowly growing atop it.  ' );
				} else if( CoC.getInstance().player.dogCocks() >= 1 ) {
					EngineCore.outputText( 'The pointed tip of your ' + CoC.getInstance().player.cockDescript() + ' angles towards your face as you masturbate, a dollop of pre slowly leaking from it.  ' );
				} else if( CoC.getInstance().player.tentacleCocks() >= 1 ) {
					EngineCore.outputText( 'The overly wide head of your ' + CoC.getInstance().player.cockDescript() + ' bumps against your lips as you masturbate, waving back and forth like a snake as it searches for an orifice.  ' );
				} else if( CoC.getInstance().player.demonCocks() >= 1 ) {
					EngineCore.outputText( 'The purplish head of your ' + CoC.getInstance().player.cockDescript() + ' bumps against your lips as you masturbate, flushing darkly with every beat of your heart.  ' );
				} else if( CoC.getInstance().player.catCocks() >= 1 ) {
					EngineCore.outputText( 'The slightly pointed tip of your ' + CoC.getInstance().player.cockDescript() + ' bumps against your lips as you masturbate, flushing with blood as your spines grow thicker in your hand.  ' );
				} else if( CoC.getInstance().player.lizardCocks() >= 1 ) {
					EngineCore.outputText( 'The pointed, purple tip of your ' + CoC.getInstance().player.cockDescript() + ' bumps against your lips while its knotted surface flushes near-purple and seems to grow thicker.  ' );
				} else if( CoC.getInstance().player.anemoneCocks() >= 1 ) {
					EngineCore.outputText( 'The tentacle-ringed tip of your ' + CoC.getInstance().player.cockDescript() + ' brushes against your lips, making them tingle with artificial heat while you stroke it.  ' );
				} else if( CoC.getInstance().player.displacerCocks() >= 1 ) {
					EngineCore.outputText( 'The blooming tip of your ' + CoC.getInstance().player.cockDescript() + ' angles towards your face as you masturbate, a dollop of pre slowly leaking out of the outstretched top.  ' );
				} else {
					EngineCore.outputText( 'The head of your ' + CoC.getInstance().player.cockDescript() + ' wobbles towards your face as you masturbate, a dollop of pre slowly growing atop it.  ' );
				}
				//try to stick it in a titty!
				if( CoC.getInstance().player.hasFuckableNipples() && CoC.getInstance().player.biggestTitSize() >= 3 ) {
					...
					EngineCore.outputText( 'You give in to temptation and swallow the tip, slurping greedily as you milk your ' + CoC.getInstance().player.cockDescript() + ' of its pre-cum.  ' );
					autofellatio = true;
				}
				if( CoC.getInstance().player.canTitFuck() && CoC.getInstance().player.biggestTitSize() > 3 && !nippleFuck ) {
					EngineCore.outputText( 'Your hands migrate to your breasts of their own accord, wrapping your titflesh around your ' + CoC.getInstance().player.cockDescript() + ', jacking it up and down in your pillowy tits.  ' );
				...
				if( CoC.getInstance().player.normalCocks() >= 1 ) {
					EngineCore.outputText( 'The head of your ' + CoC.getInstance().player.cockDescript() + ' wobbles over, bumping your face and smearing your lips with its copious pre-cum.  You are unable to resist opening your mouth and sucking it down, filling your mouth with your cock.  ' );
				} else if( CoC.getInstance().player.horseCocks() >= 1 ) {
					EngineCore.outputText( 'The flared tip of your ' + CoC.getInstance().player.cockDescript() + ' wobbles over, bumping your face and smearing your lips with its copious pre-cum.  You are unable to resist opening your mouth and sucking it down, filling your mouth with horsemeat.  ' );
				} else if( CoC.getInstance().player.dogCocks() >= 1 ) {
					EngineCore.outputText( 'The head of your ' + CoC.getInstance().player.cockDescript() + ' wobbles over, bumping your face and smearing your lips with its copious pre-cum.  You are unable to resist opening your mouth and sucking it down, filling your mouth with your ' + CoC.getInstance().player.cockDescript() + '.  ' );
				} else if( CoC.getInstance().player.tentacleCocks() >= 1 ) {
					EngineCore.outputText( 'The bulbous, mushroom-like head of your ' + CoC.getInstance().player.cockDescript() + ' pushes against your face eagerly, smearing pre-cum over your lips as it seeks entrance to the nearest orifice.  You\'re unable to resist opening your mouth to suck it down, filling your mouth with slick rubbery cock-tentacle.  ' );
				} else if( CoC.getInstance().player.demonCocks() >= 1 ) {
					EngineCore.outputText( 'The tainted swollen head of your ' + CoC.getInstance().player.cockDescript() + ' pushes against your face, smearing sweet pre-cum over your lips.  You\'re unable to resist opening wide and taking in the demonic member, submitting wholly to the desire to pleasure your corrupted body-parts.  ' );
				} else if( CoC.getInstance().player.catCocks() >= 1 ) {
					EngineCore.outputText( 'The pointed tip of your ' + CoC.getInstance().player.cockDescript() + ' pushes against your face, smearing pre-cum over your lips and tickling you with the many barbs.  You\'re unable to resist opening wide and taking in the ' + CoC.getInstance().player.cockDescript() + ', half-humming half-purring in contentment.  ' );
				} else if( CoC.getInstance().player.lizardCocks() >= 1 ) {
					EngineCore.outputText( 'The slightly-pointed tip of your ' + CoC.getInstance().player.cockDescript() + ' pushes against your face, smearing pre-cum over your eager lips.   You can\'t resist opening wide and slipping it inside as your hands caress the reptilian bulges along your length.  ' );
				} else if( CoC.getInstance().player.anemoneCocks() >= 1 ) {
					EngineCore.outputText( 'The rounded, tentacle-ringed tip of your ' + CoC.getInstance().player.cockDescript() + ' slides against your face, smearing pre-cum over your lips and stinging them with aphrodisiacs that make you pant with lust.  You can\'t resist opening wide to greedily slurp it down, and in seconds tiny, tingling stings are erupting through your oral cavity, filling you with lust and pleasure.  ' );
				} else if( CoC.getInstance().player.displacerCocks() >= 1 ) {
					EngineCore.outputText( 'The head of your ' + CoC.getInstance().player.cockDescript() + ' wobbles over, bumping your face and smearing your lips with its copious pre-cum.  Wiggling against your lips, the various protrusions of the \'star\' at the tip smear you with your heady emissions.   You are unable to resist opening your mouth and sucking it down, filling your mouth with your ' + CoC.getInstance().player.cockDescript() + '.  ' );
				} else {
					EngineCore.outputText( 'The head of your ' + CoC.getInstance().player.cockDescript() + ' wobbles over, bumping your face and smearing your lips with its copious pre-cum.  You are unable to resist opening your mouth and sucking it down, filling your mouth with your cock.  ' );
				}
				//try to stick it in a titty!
				if( CoC.getInstance().player.hasFuckableNipples() && CoC.getInstance().player.biggestTitSize() >= 3 ) {
					...
					if( CoC.getInstance().player.cor > 60 ) {
						if( CoC.getInstance().player.normalCocks() >= 1 ) {
							...
							EngineCore.outputText( 'The sweet scent of your ' + CoC.getInstance().player.cockDescript() + ' fills your nostrils as inch after inch of tentacle forces its way down your throat.  You struggle briefly with it, wrestling to keep it from pushing the whole way into your gut, swooning with the pleasure that fighting with your own cock induces.  ' );
						} else if( CoC.getInstance().player.demonCocks() >= 1 ) {
							EngineCore.outputText( 'The spicy demonic odor your ' + CoC.getInstance().player.cockDescript() + ' radiates fills your nostrils as you force inch after inch of tainted meat down your throat.  You struggle briefly, but your ' + CoC.getInstance().player.cockDescript() + ' quickly overwhelms your resistance and your gag reflex momentarily seems to disappear.  ' );
						} else if( CoC.getInstance().player.catCocks() >= 1 ) {
							...
							EngineCore.outputText( 'The salty, dry odor of your ' + CoC.getInstance().player.cockDescript() + ' fills your nostrils as you force inch after inch into your mouth, swallowing as much of the bulgy shaft as you can.  ' );
						} else if( CoC.getInstance().player.anemoneCocks() >= 1 ) {
							EngineCore.outputText( 'The stinging, aphrodisiac-laced ' + CoC.getInstance().player.cockDescript() + ' slowly works its way down your throat as you swallow it deeper and deeper, gurgling happily as your throat grows more sensitive.  ' );
						} else if( CoC.getInstance().player.displacerCocks() >= 1 ) {
							EngineCore.outputText( 'The strong animal scent of your cock fills your nostrils as you force inch after inch into your mouth, deepthroating as much of yourself as you can.  ' );
						} else {
							EngineCore.outputText( 'The heady aroma of your cock fills your nostrils as you force inch after inch into your mouth, deepthroating as much of yourself as you can.  ' );
				...
				if( CoC.getInstance().player.canTitFuck() && CoC.getInstance().player.biggestTitSize() > 3 && !nippleFuck ) {
					EngineCore.outputText( 'Your hands reach up to your pillowy breasts, wrapping them around the shaft of your ' + CoC.getInstance().player.cockDescript() + ', causing you to let out muffled moans of excitment. ' );
					if( CoC.getInstance().player.biggestLactation() > 0 ) {
						EngineCore.outputText( 'Jets of milk squirt out of your nipples with each thrust of your hips, adding to the already copious amounts of fluids coating your body. ' );
					}
				}
			}
		} else if( CoC.getInstance().player.cocks.length === 2 ) { //Pair of cocks
			//Grab it
			...
			EngineCore.outputText( 'You grasp one of your ' + CoC.getInstance().player.multiCockDescriptLight() + ' in each hand, ' );
			if( CoC.getInstance().player.averageCockThickness() <= 1.8 ) {
			...
			//Get into it
			//Play with it if <= 10'
			if( CoC.getInstance().player.averageCockLength() <= 10 ) {
			...
			else if( CoC.getInstance().player.averageCockLength() < 20 ) {
				...
				if( CoC.getInstance().player.hasFuckableNipples() ) {
					...
					if( CoC.getInstance().player.dogCocks() > 0 ) {
						...
						} else if( CoC.getInstance().player.dogCocks() !== CoC.getInstance().player.cocks.length ) { //if you have something that will fit
							nippleFuck = true;
							autofellatio = true;
							//UNFINISHED
							if( CoC.getInstance().player.normalCocks() > 0 ) {
								this.titFuckSingle();
							}
				...
				if( CoC.getInstance().player.canTitFuck() && !nippleFuck ) {
					EngineCore.outputText( 'You wrap your ' + CoC.getInstance().player.breastCup( 0 ) + ' tits around your ' + CoC.getInstance().player.multiCockDescriptLight() + ', sandwiching them inside your pillowy mounds.  With delicious slowness you begin jacking yourself off with your tits, your shafts rubbing together between your luxurious breasts, tongue running figure eights across the tips. ' );
					if( CoC.getInstance().player.biggestLactation() > 0 ) {
						EngineCore.outputText( 'Jets of milk squirt out of your nipples with each thrust of your hips, adding to the already copious amounts of fluids coating your body. ' );
					}
				}
				//Titfuck (enough breasts for both!)
				if( CoC.getInstance().player.canTitFuck() && CoC.getInstance().player.mostBreastsPerRow() > 2 && !nippleFuck ) {
					EngineCore.outputText( 'You wrap your ' + CoC.getInstance().player.breastCup( 0 ) + ' tits around your ' + CoC.getInstance().player.multiCockDescriptLight() + ', squeezing them between your ' + CoC.getInstance().player.totalBreasts() + ' breasts.  With delicious slowness you begin jacking yourself off with your tits, smooth breastflesh massaging your members as you aggressively lick and suck at the tips.  ' );
					if( CoC.getInstance().player.biggestLactation() > 0 ) {
				...
				if( CoC.getInstance().player.biggestTitSize() <= 3 && !nippleFuck ) {
					EngineCore.outputText( 'You slowly stroke yourself, pausing every few strokes to smear the streaks of pre that leak from your ' + CoC.getInstance().player.multiCockDescriptLight() + ' over yourself.  As you become more and more lubed and horny, you increase the tempo, until at last you are pumping yourself hard and fast, squeezing your cocks together in pleasure.  ' );
				}
			}
			//Deepthroat yourself, maybe titfuck if equipped.
			else {
				//autofellatio only gets to be true if you have any dicks left to suck on after sticking them in your boobs
				EngineCore.outputText( 'Your ' + CoC.getInstance().player.multiCockDescriptLight() + ' brush against your face, throbbing with need.   You open your mouth and suck one of your pricks into your mouth, taking as much of it as possible, running your tongue along the underside.  You smile and pop it free, then take a different shaft into your maw.  ' );
				//Try to stick it in a titty!
				if( CoC.getInstance().player.hasFuckableNipples() ) {
					//check for dogcocks
					if( CoC.getInstance().player.dogCocks() > 0 ) {
						...
						if( CoC.getInstance().player.hasFuckableNipples() ) {
							...
							nippleFuck = true;
							autofellatio = true;
							if( CoC.getInstance().player.normalCocks() > 0 || CoC.getInstance().player.horseCocks() > 0 ) {
								this.titFuckSingle();
							}
						}  //if you only have dogcocks that are too big, no nipple fuck! Otherwise, run normal multiTitFuck
					} else {
				...
				//Titfuck (squeeze em both in between 2!
				if( CoC.getInstance().player.canTitFuck() && !nippleFuck ) {
					EngineCore.outputText( 'You wrap your ' + CoC.getInstance().player.breastCup( 0 ) + ' tits around your ' + CoC.getInstance().player.multiCockDescriptLight() + ', sandwiching them inside your pillowy mounds.  With delicious slowness you begin jacking yourself off with your tits, your shafts rubbing together between your luxurious breasts, mouthfucking yourself with each of your cocks in turn. ' );
					if( CoC.getInstance().player.biggestLactation() > 0 ) {
						EngineCore.outputText( 'Jets of milk squirt out of your nipples with each thrust of your hips, adding to the already copious amounts of fluids coating your body. ' );
					}
				}
				//Titfuck (enough breasts for both!)
				if( CoC.getInstance().player.mostBreastsPerRow() > 2 && CoC.getInstance().player.biggestTitSize() > 3 && !nippleFuck ) {
		...
		} else if( CoC.getInstance().player.cockTotal() >= 3 ) { //Three or more cocks
			//Grab it
			//Play with sheath if has one
			if( CoC.getInstance().player.hasSheath() ) {
			...
			EngineCore.outputText( 'You grasp one of your ' + CoC.getInstance().player.multiCockDescriptLight() + ' in each hand, ' );
			if( CoC.getInstance().player.averageCockThickness() <= 1.8 ) {
				EngineCore.outputText( 'wrapping your fingers around each tool and just holding yourself, the feelings of your ' + Utils.num2Text( CoC.getInstance().player.cocks.length ) + ' cocks making you feel giddy.  ' );
			} else {
			...
			if( CoC.getInstance().player.averageCockLength() <= 10 ) {
				...
				if( CoC.getInstance().player.hasFuckableNipples() ) {
					...
					if( CoC.getInstance().player.dogCocks() > 0 ) {
						...
						if( CoC.getInstance().player.hasFuckableNipples() ) {
							...
							nippleFuck = true;
							autofellatio = true;
							if( (CoC.getInstance().player.normalCocks() > 0 || CoC.getInstance().player.horseCocks() > 0) && CoC.getInstance().player.cocks.length - CoC.getInstance().player.dogCocks() === 1 ) {
								this.titFuckSingle();
							}
							if( CoC.getInstance().player.cocks.length - CoC.getInstance().player.dogCocks() > 1 ) {
								this.multiTitFuck();
							}
						} //if you only have dogcocks that are too big, no nipple fuck! Otherwise, run normal multiTitFuck
					} else {
				...
				//Titfuck (squeeze em both in between 2!
				if( CoC.getInstance().player.canTitFuck() && !nippleFuck ) {
					EngineCore.outputText( 'You wrap your ' + CoC.getInstance().player.breastCup( 0 ) + ' tits around your ' + CoC.getInstance().player.multiCockDescriptLight() + ', sandwiching them inside your pillowy mounds.  With delicious slowness you begin jacking yourself off with your tits, your shafts rubbing together between your luxurious breasts, your tongue running figure eights across the tips. ' );
					if( CoC.getInstance().player.biggestLactation() > 0 ) {
						EngineCore.outputText( 'Jets of milk squirt out of your nipples with each thrust of your hips, adding to the already copious amounts of fluids coating your body. ' );
					}
				}
			...
			//Deepthroat yourself, maybe titfuck if equipped.
			if( CoC.getInstance().player.averageCockLength() >= 20 ) {
				//autofellatio only gets to be true if you have any dicks left to suck on after sticking them in your boobs
				EngineCore.outputText( 'Your ' + CoC.getInstance().player.multiCockDescriptLight() + ' brush against your face, throbbing with need.   You open your mouth and suck one of your pricks into your mouth, taking as much of it in as possible, running your tongue along the underside.  You smile and pop it free, then take a different shaft into your maw.  ' );
				//Try to stick it in a titty!
				if( CoC.getInstance().player.hasFuckableNipples() ) {
					//check for dogcocks
					if( CoC.getInstance().player.dogCocks() > 0 ) {
			...
			if( CoC.getInstance().player.vaginas.length === 1 ) {
				...
				switch( CoC.getInstance().player.vaginas[ 0 ].vaginalWetness ) {
					...
					case AppearanceDefs.VAGINA_WETNESS_SLICK:
						...
						break;
					case AppearanceDefs.VAGINA_WETNESS_DROOLING:
						EngineCore.outputText( 'Warm wetness runs down your legs in thick streams, pouring from your ' + CoC.getInstance().player.vaginaDescript() + '.  You smile sheepishly and stroke up your now slickened legs to your pussy-lips, parting them and letting your fingers dive inside the wet channel.  ' );
						break;
					default:
		...
		if( CoC.getInstance().player.cocks.length > 0 ) {
			...
			if( CoC.getInstance().player.cocks.length === 1 ) {
				if( CoC.getInstance().player.lib < 30 ) {
				...
				} else if( CoC.getInstance().player.lib < 55 ) {
					EngineCore.outputText( 'You roll and begin to doze, your semi-erect ' + CoC.getInstance().player.cockDescript() + ' flopping against you. ' );
				} else if( CoC.getInstance().player.lib <= 80 ) {
					EngineCore.outputText( 'As you close your eyes and relax, your ' + CoC.getInstance().player.cockDescript() + ' surges back to erectness, ensuring ' );
					...
					EngineCore.outputText( 'You groan and drift to sleep, your rigid ' + CoC.getInstance().player.cockDescript() + ' pulsing and throbbing with continual lust.' );
				}
			}
			//Multi Cock
			else {
	...
	Masturbation.prototype.genderlessMasturbate = function() {
		...
		//Tit foreplay
		this.titForeplay();
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.FappedGenderless ) < 0 ) { //first time as a genderless person
			...
			CoC.getInstance().player.createStatusAffect( StatusAffects.FappedGenderless, 0, 0, 0, 0 );
		}
		...
		EngineCore.outputText( 'You lay down on your side and reach gingerly behind yourself.  The palm of your hand comes to rest on your ' + CoC.getInstance().player.buttDescript() + '.  You slide your finger into your crack and find your ' + Descriptors.assholeDescript() + '.  You run your finger slowly around the sensitive ring of your hole and feel a tingle emanating from it. A smile creeps across your lips as you begin to imagine what is about to happen.\n\n' );
		...
		//If no BioLube perk -
		if( CoC.getInstance().player.ass.analWetness < 2 ) {
		...
		if( CoC.getInstance().player.ass.analWetness >= 2 ) {
			...
			//Still Horny -
			if( CoC.getInstance().player.lib >= 75 ) {
	...
	Masturbation.prototype.titForeplay = function() {
		...
		if( CoC.getInstance().player.breastRows.length === 1 && CoC.getInstance().player.biggestTitSize() > 3 ) {
			if( CoC.getInstance().player.lib < 45 ) {
				EngineCore.outputText( 'You caress your ' + CoC.getInstance().player.breastDescript( 0 ) + ' gently with your fingers' );
			} else if( CoC.getInstance().player.lib < 70 ) {
				EngineCore.outputText( 'You grope your ' + CoC.getInstance().player.breastDescript( 0 ) + ' agressively with both hands' );
			} else {
				EngineCore.outputText( 'You squeeze your ' + CoC.getInstance().player.breastDescript( 0 ) + ' brutally with both hands' );
			}
		} else if( CoC.getInstance().player.breastRows.length > 1 && CoC.getInstance().player.biggestTitSize() > 3 ) {
			if( CoC.getInstance().player.lib < 45 ) {
				EngineCore.outputText( 'You hesitantly run your hands across your front, caressing each of your ' + Utils.num2Text( CoC.getInstance().player.totalBreasts() ) + ' breasts in turn. ' );
			} else if( CoC.getInstance().player.lib < 70 ) {
				EngineCore.outputText( 'You run your hands across your front, squeezing each of your ' + Utils.num2Text( CoC.getInstance().player.totalBreasts() ) + ' breasts in turn. ' );
			} else {
				EngineCore.outputText( 'You aggressively run your hands across your front, groping each of your ' + Utils.num2Text( CoC.getInstance().player.totalBreasts() ) + ' breasts in turn. ' );
			}
			...
			if( CoC.getInstance().player.cor <= 25 ) {
				EngineCore.outputText( 'You blink back tears of shame as you experience the tactile proof of your abnormal endowments, unable to resist mashing your ' + CoC.getInstance().player.allBreastsDescript() + ' together' );
			} else if( CoC.getInstance().player.cor < 75 ) {
				EngineCore.outputText( 'Though a small part of you is horrified by your abnormal endowments, it doesn\'t stop you from mashing your ' + CoC.getInstance().player.allBreastsDescript() + ' together' );
			} else {
		...
		if( CoC.getInstance().player.hasFuckableNipples() && CoC.getInstance().player.biggestTitSize() > 3 ) {
			...
			} else if( CoC.getInstance().player.averageVaginalLooseness() < 4 ) {
				EngineCore.outputText( 'puckered holes at the tip of each nipple. ' );
			}//add 'many if lots of nips
			else if( CoC.getInstance().player.averageNipplesPerBreast() > 1 ) {
			...
			if( CoC.getInstance().player.sens < 45 ) {
				EngineCore.outputText( 'You sigh contentedly as ' );
			} else if( CoC.getInstance().player.sens < 70 ) {
			...
			if( CoC.getInstance().player.averageVaginalWetness() < 2 ) {
				EngineCore.outputText( 'you feel your nipples loosen up. ' );
			} else if( CoC.getInstance().player.averageVaginalWetness() < 4 ) {
				EngineCore.outputText( 'you begin to slowly spread the slippery secretions leaking from your engorged nipples all over your ' + CoC.getInstance().player.breastDescript( 0 ) + '. ' );
			} else {
				EngineCore.outputText( 'little spurts of ' );
				//Lactating?
				if( CoC.getInstance().player.averageLactation() > 0 ) {
					EngineCore.outputText( ' milky ' );
				}
				EngineCore.outputText( 'girlcum squirt out of your engorged nipples. ' );
			}
			//Special extras for lactation
			if( CoC.getInstance().player.averageLactation() > 0 && CoC.getInstance().player.averageLactation() < 2 ) {
				EngineCore.outputText( 'Your freakish mammaries drip a sticky combination of cunt-juice and milk, spattering your ' + CoC.getInstance().player.legs() + ' and crotch.' );
			} else if( CoC.getInstance().player.averageLactation() < 3 ) {
				EngineCore.outputText( 'Your freakish mammaries leak thin streams of sticky girlcum mixed with milk all over your ' + CoC.getInstance().player.legs() + ' and crotch.' );
			} else {
				EngineCore.outputText( 'Your freakish mammaries constantly drool a steady stream of milky cunt-juice, drenching your ' + CoC.getInstance().player.legs() + ' and crotch in milky goo.' );
			}
		}
		//If nipples aren't fuckable.
		else if( CoC.getInstance().player.breastRows.length > 0 && CoC.getInstance().player.biggestTitSize() > 3 ) {
			...
			if( CoC.getInstance().player.biggestLactation() > 1 && CoC.getInstance().player.biggestLactation() < 2 ) {
				EngineCore.outputText( 'Droplets of milk dribble from each nipple, spattering milk onto your ' + CoC.getInstance().player.legs() + ' and crotch.  ' );
			} else if( CoC.getInstance().player.biggestLactation() < 3 ) {
				EngineCore.outputText( 'Thin squirts of milk spray from each nipple, spattering milk onto your ' + CoC.getInstance().player.legs() + ' and crotch.  ' );
			} else if( CoC.getInstance().player.biggestLactation() >= 3 ) {
				EngineCore.outputText( 'A constant stream of milk drizzles from each teat, soaking your ' + CoC.getInstance().player.legs() + ' and crotch.  ' );
			}
		}
	};
	Masturbation.prototype.titFuckSingle = function() {
		if( CoC.getInstance().player.lib < 45 ) {
			EngineCore.outputText( 'You grip your ' + CoC.getInstance().player.cockDescript() + ' and begin cautiously guiding it towards ' );
		} else if( CoC.getInstance().player.lib < 70 ) {
			EngineCore.outputText( 'Shivering with anticipation, you place the ' );
			switch( CoC.getInstance().player.cocks[ 0 ].cockType ) { //Different way of choosing cock to use, results in consistent description of cock[0]
			...
			EngineCore.outputText( 'tip of your ' + CoC.getInstance().player.cockDescript() + ' against the opening of ' );
		} else {
			...
			switch( CoC.getInstance().player.cocks[ 0 ].cockType ) { //Different way of choosing cock to use, results in consistent description of cock[0]
				...
				case CockTypesEnum.TENTACLE:
					...
					break;
				default:
					EngineCore.outputText( CoC.getInstance().player.cockHead() + ' of ' );
			}
			EngineCore.outputText( 'your ' + CoC.getInstance().player.cockDescript() + ' into ' );
		}
		//More than one row?
		if( CoC.getInstance().player.breastRows.length > 1 ) {
		...
		if( CoC.getInstance().player.averageLactation() === 0 ) {
			if( CoC.getInstance().player.averageVaginalWetness() < 2 ) {
				...
				EngineCore.outputText( 'your ' + CoC.getInstance().player.cockDescript() + ' and y' );
			} else {
				EngineCore.outputText( 'Slick juices squirt out from around your ' + CoC.getInstance().player.cockDescript() + ' and y' );
			}
		} else {
			if( CoC.getInstance().player.averageLactation() < 2 ) {
				...
				EngineCore.outputText( 'Milky girlcum squirts out from around your ' + CoC.getInstance().player.cockDescript() + ', staining ' );
				switch( CoC.getInstance().player.cocks[ 0 ].cockType ) { //Different way of choosing cock to use, results in consistent description of cock[0]
					case CockTypesEnum.ANEMONE:
		...
		if( Math.round( CoC.getInstance().player.cockArea( 0 ) ) >= CoC.getInstance().player.vaginalCapacity() ) {
			//Different noises based on sensitivity
			if( CoC.getInstance().player.sens < 45 ) {
				EngineCore.outputText( 'ou grunt with exertion as you attempt to stuff your ' + CoC.getInstance().player.cockDescript() + ' into ' );
			} else if( CoC.getInstance().player.sens < 70 ) {
				EngineCore.outputText( 'ou blink back tears as you attempt to stuff your ' + CoC.getInstance().player.cockDescript() + ' into ' );
			} else {
				EngineCore.outputText( 'ou scream with a combination of pain and pleasure as you attempt to stuff your ' + CoC.getInstance().player.cockDescript() + ' into ' );
			}
			...
			if( Math.round( CoC.getInstance().player.cockArea( 0 ) ) === CoC.getInstance().player.vaginalCapacity() ) {
				if( CoC.getInstance().player.averageVaginalLooseness() < 2 ) {
					EngineCore.outputText( 'Your poor, tortured nipple is barely up to the task of accepting the ' + CoC.getInstance().player.cockDescript() + ', but accept it it does. ' );
				} else if( CoC.getInstance().player.averageVaginalLooseness() < 4 ) {
					EngineCore.outputText( 'The engorged opening at the end of your swollen nipple is stretched to its limit as you shove your ' + CoC.getInstance().player.cockDescript() + ' home. ' );
				} else {
					switch( CoC.getInstance().player.cocks[ 0 ].cockType ) { //Different way of choosing cock to use, results in consistent description of cock[0]
						case CockTypesEnum.ANEMONE:
							EngineCore.outputText( "The swollen tips of your bloated nipple wrap around the stinging tentacles that surround your " + CoC.getInstance().player.cockDescript() + "'s tip, convulsing with wet squishing sounds as they become red and enflamed with artificial lust.  " );
							break;
						case CockTypesEnum.CAT:
							EngineCore.outputText( "The swollen tips of your bloated nipples stretch around the barbed tip of your " + CoC.getInstance().player.cockDescript() + ", swallowing it like an enormous mouth.  " );
							break;
						case CockTypesEnum.DEMON:
							EngineCore.outputText( "The swollen lips of your bloated nipple stretch around the nodule-ringed tip of your " + CoC.getInstance().player.cockDescript() + ", swallowing it like an enormous mouth.  " );
							break;
						case CockTypesEnum.DISPLACER:
							EngineCore.outputText( "The swollen tips of your bloated nipple wrap around the outstretched head of your " + CoC.getInstance().player.cockDescript() + ", convulsing with wet, squishing sounds as it wriggles inside you.  " );
							break;
						case CockTypesEnum.DOG:
						case CockTypesEnum.FOX:
							EngineCore.outputText( "The swollen lips of your bloated nipple stretch around the pointed tip of your " + CoC.getInstance().player.cockDescript() + " swallowing it like an enormous mouth.  " );
							break;
						case CockTypesEnum.HORSE:
							EngineCore.outputText( "The swollen lips of your bloated nipple stretch around the flared tip of your " + CoC.getInstance().player.cockDescript() + " swallowing it like an enormous mouth.  " );
							break;
						case CockTypesEnum.HUMAN:
							EngineCore.outputText( "The swollen lips of your bloated nipple stretch around the tip of your " + CoC.getInstance().player.cockDescript() + " swallowing it like an enormous mouth.  " );
							break;
						case CockTypesEnum.LIZARD:
							EngineCore.outputText( "The swollen tips of your bloated nipple wrap around the pointed tip of your " + CoC.getInstance().player.cockDescript() + ", stretching oddly as it swallows the knot-covered appendage.  " );
							break;
						case CockTypesEnum.TENTACLE:
							EngineCore.outputText( "The swollen lips of your bloated nipple stretch around the rounded tip of your " + CoC.getInstance().player.cockDescript() + ", swallowing it like an enormous mouth.  " );
							break;
						default:
							EngineCore.outputText( "tips of your bloated nipples wrap around the " + CoC.getInstance().player.cockHead() + " of your " + CoC.getInstance().player.cockDescript() + ", swallowing it like an enormous mouth.  " );
					}
				}
				EngineCore.outputText( 'With each thrust, you bury your ' + CoC.getInstance().player.cockDescript() + ' deeper into your greedy tit. Overwhelmed by the combined sensations, your mind is barely able to cope with the intense feeling of fullness where no such feeling should be possible.' );
			}
			//if it doesnt really fit
			if( Math.round( CoC.getInstance().player.cockArea( 0 ) ) > CoC.getInstance().player.vaginalCapacity() ) {
				if( CoC.getInstance().player.averageVaginalLooseness() < 2 ) {
					EngineCore.outputText( 'Your poor, tortured nipple is woefully insufficient compared to your ' + CoC.getInstance().player.cockDescript() + ', delerious with arousal, you keep trying anyway. ' );
				} else if( CoC.getInstance().player.averageVaginalLooseness() < 4 ) {
					EngineCore.outputText( 'The engorged opening at the end of your swollen nipple is stretched to its limits and beyond as you shove your ' + CoC.getInstance().player.cockDescript() + ' home. ' );
				} else {
					switch( CoC.getInstance().player.cocks[ 0 ].cockType ) { //Different way of choosing cock to use, results in consistent description of cock[0]
						case CockTypesEnum.ANEMONE:
							EngineCore.outputText( "The swollen lips of your bloated nipple gape wide, but the stinging tip of your " + CoC.getInstance().player.cockDescript() + " spreads them wider and fills them with artificial lust.  " );
							break;
						case CockTypesEnum.CAT:
							EngineCore.outputText( "The swollen lips of your bloated nipple gape wide, but the barbed tip of your " + CoC.getInstance().player.cockDescript() + " spreads them even wider.  " );
							break;
						case CockTypesEnum.DEMON:
							EngineCore.outputText( "The swollen lips of your bloated nipple gape wide, but the tip of your " + CoC.getInstance().player.cockDescript() + " spreads them even wider.  " );
							break;
						case CockTypesEnum.DISPLACER:
							EngineCore.outputText( "The swollen lips of your bloated nipple gape wide, but the wide head of your " + CoC.getInstance().player.cockDescript() + " spreads them even wider.  " );
							break;
						case CockTypesEnum.DOG:
						case CockTypesEnum.FOX:
							EngineCore.outputText( "The swollen lips of your bloated nipple gape wide, but the massive girth of your " + CoC.getInstance().player.cockDescript() + " spreads them even wider.  " );
							break;
						case CockTypesEnum.HORSE:
							EngineCore.outputText( "The swollen lips of your bloated nipple gape wide, but the flared tip of your " + CoC.getInstance().player.cockDescript() + " spreads them even wider.  " );
							break;
						case CockTypesEnum.HUMAN:
							EngineCore.outputText( "The swollen lips of your bloated nipple gape wide, but the tip of your " + CoC.getInstance().player.cockDescript() + " spreads them even wider.  " );
							break;
						case CockTypesEnum.LIZARD:
							EngineCore.outputText( "The swollen lips of your bloated nipple gape wide, but the pointed tip of your " + CoC.getInstance().player.cockDescript() + " slowly spreads them even wider.  " );
							break;
						case CockTypesEnum.TENTACLE:
							EngineCore.outputText( "The swollen lips of your bloated nipple gape wide, but the tip of your " + CoC.getInstance().player.cockDescript() + " spreads them even wider.  " );
							break;
						default:
							EngineCore.outputText( "The swollen lips of your bloated nipple gape wide, but the " + CoC.getInstance().player.cockHead() + " of your " + CoC.getInstance().player.cockDescript() + " spreads them even wider.  " );
					}
				}
				EngineCore.outputText( 'Grunting and sweating with effort, you stuff as much of your ' + CoC.getInstance().player.cockDescript() + ' into your overstretched nipple as you can fit. The feeling of incredible tightness around your ' + CoC.getInstance().player.cockDescript() + ' combines with the pain of your distended nipple to form a mindbending sensation that makes your head spin. ' );
			}
			if( CoC.getInstance().player.averageLactation() > 0 ) {
				EngineCore.outputText( 'Milky ' );
			} else {
			...
			if( CoC.getInstance().player.averageVaginalWetness() < 2 ) {
				EngineCore.outputText( 'girl-lube quickly coats the whole length of your ' + CoC.getInstance().player.cockDescript() + ' in a glistening layer of fuck juice. ' );
			} else if( CoC.getInstance().player.averageVaginalWetness() < 4 ) {
				EngineCore.outputText( 'girl-lube drizzles down the length of your ' + CoC.getInstance().player.cockDescript() + ' in thick streams, ' );
				switch( CoC.getInstance().player.cocks[ 0 ].cockType ) { //Different way of choosing cock to use, results in consistent description of cock[0]
					case CockTypesEnum.ANEMONE:
				...
				EngineCore.outputText( 'girl-lube spurts out of your tortured nipple with each thrust of your ' + CoC.getInstance().player.cockDescript() + ', spattering your arms and face with your secretions. ' );
			}
		}
		if( Math.round( CoC.getInstance().player.cockArea( 0 ) ) < CoC.getInstance().player.vaginalCapacity() ) {
			//Different noises based on sensitivity
			...
			EngineCore.outputText( ' as your ' + CoC.getInstance().player.cockDescript() + ' slides into ' );
			//Different based on looseness
			if( CoC.getInstance().player.averageVaginalLooseness() < 2 ) {
				EngineCore.outputText( 'the small, over-stretched opening of your swollen nipple. Your ' + CoC.getInstance().player.cockDescript() + ' penetrates your swollen nipple easily, sliding halfway in on your first thrust. ' );
			} else if( CoC.getInstance().player.averageVaginalLooseness() < 4 ) {
				EngineCore.outputText( 'the engorged and distended opening of your fat, swollen nipple. Your ' + CoC.getInstance().player.cockDescript() + ' plunges deeply into your freakishly engorged nipple, penetrating it easily. ' );
			} else {
				...
				EngineCore.outputText( 'tip of your ' + CoC.getInstance().player.cockDescript() + ' and begin to slide down its length, ' );
				switch( CoC.getInstance().player.cocks[ 0 ].cockType ) { //Different way of choosing cock to use, results in consistent description of cock[0]
					case CockTypesEnum.ANEMONE:
			...
			EngineCore.outputText( 'You revel in the sensation as you slowly stroke your ' + CoC.getInstance().player.cockDescript() + ' in and out of your distended nipple. Your shaft is enveloped in the warm, wet embrace of your freakish tit, and ' );
			if( CoC.getInstance().player.averageLactation() > 0 ) {
				EngineCore.outputText( 'milky ' );
			} else {
			...
			if( CoC.getInstance().player.averageVaginalWetness() < 2 ) {
				...
				EngineCore.outputText( 'girl-lube drizzles down the length of your ' + CoC.getInstance().player.cockDescript() + ' in thick streams, pooling at your crotch. ' );
			} else {
				if( CoC.getInstance().player.normalCocks() > 0 ) {
					EngineCore.outputText( 'girl-lube pours out of your swollen nipple over your ' + CoC.getInstance().player.cockDescript() + ' and hands, pooling on the ground below you. ' );
				} else if( CoC.getInstance().player.hasSheath() > 0 ) {
					EngineCore.outputText( 'girl-lube drizzles down the length of your ' + CoC.getInstance().player.cockDescript() + ' in thick streams, pooling in and around your sheath. ' );
				} else if( CoC.getInstance().player.tentacleCocks() > 0 ) {
					EngineCore.outputText( 'girl-lube pours out of your swollen nipple over your ' + CoC.getInstance().player.cockDescript() + ' and hands, pooling on the ground below you. ' );
				} else if( CoC.getInstance().player.demonCocks() > 0 ) {
					EngineCore.outputText( 'girl-lube pours out of your swollen nipple over your ' + CoC.getInstance().player.cockDescript() + ' and hands, pooling on the ground below you. ' );
				} else {
					EngineCore.outputText( 'girl-lube pours out of your swollen nipple over your ' + CoC.getInstance().player.cockDescript() + ' and hands, pooling on the ground below you. ' );
				}
	...
	Masturbation.prototype.multiTitFuck = function() {
		var holeTotal = CoC.getInstance().player.totalNipples();
		var fittableCocks = CoC.getInstance().player.cocks.length;
		var doubleUp = false;
		...
		if( fittableCocks > 2 && holeTotal < fittableCocks ) {
			...
			if( CoC.getInstance().player.vaginalCapacity() >= 2 * CoC.getInstance().player.cockArea( randomCock ) ) {
				...
				if( 2 * holeTotal >= fittableCocks ) {
					EngineCore.outputText( 'You giggle with glee as you realize that you will be able to jam every single one of your ' + CoC.getInstance().player.multiCockDescriptLight() + ' into your abused nipples, thanks to double-penetration! ' );
				} else {
					EngineCore.outputText( 'With a flash of irritation, you realize that even if you stick two cocks in each hole, you still won\'t be able to fit all ' + Utils.num2Text( CoC.getInstance().player.cocks.length ) + ' of your dicks into your abused tits. Deciding to make the best of it, you prepare to stuff in as many as you can. ' );
				}
			} else {
		...
		//Round and compare cock thickness to vag looseness
		if( Math.round( CoC.getInstance().player.cockArea( randomCock ) ) >= CoC.getInstance().player.vaginalCapacity() ) {
			//Different noises based on sensitivity
			if( CoC.getInstance().player.sens < 45 ) {
		...
		if( Math.round( CoC.getInstance().player.cockArea( randomCock ) ) < CoC.getInstance().player.vaginalCapacity() ) {
			if( !doubleUp ) {
				...
				if( 4 * Math.round( CoC.getInstance().player.averageCockThickness() ) < CoC.getInstance().player.averageVaginalLooseness() ) {
					if( CoC.getInstance().player.averageVaginalLooseness() < 2 ) {
	...
	Masturbation.prototype.multiNippleFuckPrep = function( randomCock ) {
		...
		if( CoC.getInstance().player.lib < 70 ) {
			EngineCore.outputText( 'Shivering with anticipation, you place the ' );
		} else {
			EngineCore.outputText( 'Without hesitation, you shove the ' );
		}
		...
		if( CoC.getInstance().player.cocks[ randomCock ].cockType === CockTypesEnum.HUMAN ) {
			EngineCore.outputText( 'tip of ' );
			//more than one?
			if( CoC.getInstance().player.normalCocks() > 1 ) {
				EngineCore.outputText( 'one of your ' + Utils.num2Text( CoC.getInstance().player.normalCocks() ) + ' ' + CoC.getInstance().player.cockDescript( randomCock ) + 's ' );
			} else {
				EngineCore.outputText( 'your ' + CoC.getInstance().player.cockDescript( randomCock ) + ' ' );
			}
		}
		//Applying randomization - horse cocks
		if( CoC.getInstance().player.cocks[ randomCock ].cockType === CockTypesEnum.HORSE ) {
			EngineCore.outputText( 'flared tip of ' );
			//more than one?
			if( CoC.getInstance().player.horseCocks() > 1 ) {
				EngineCore.outputText( 'one of your ' + Utils.num2Text( CoC.getInstance().player.horseCocks() ) + ' ' + CoC.getInstance().player.cockDescript( randomCock ) + 's ' );
			} else {
				EngineCore.outputText( 'your ' + CoC.getInstance().player.cockDescript( randomCock ) + ' ' );
			}
		}
		//Applying randomization - dog cocks
		if( CoC.getInstance().player.cocks[ randomCock ].cockType === CockTypesEnum.DOG ) {
			EngineCore.outputText( 'pointed tip of ' );
			//more than one?
			if( CoC.getInstance().player.dogCocks() > 1 ) {
				EngineCore.outputText( 'one of your ' + Utils.num2Text( CoC.getInstance().player.dogCocks() ) + ' ' + CoC.getInstance().player.cockDescript( randomCock ) + 's ' );
			} else {
				EngineCore.outputText( 'your ' + CoC.getInstance().player.cockDescript( randomCock ) + ' ' );
			}
		}
		//Applying randomization - everything else
		if( CoC.getInstance().player.cocks[ randomCock ].cockType.Index >= 3 ) {
			EngineCore.outputText( 'tip of ' );
			//more than one?
			if( CoC.getInstance().player.normalCocks() > 1 ) {
				EngineCore.outputText( 'one of your ' + Utils.num2Text( CoC.getInstance().player.normalCocks() ) + ' ' + CoC.getInstance().player.cockDescript( randomCock ) + 's ' );
	...
	Masturbation.prototype.orgazmo = function( selfSucking, nippleFuck ) {
		...
		if( CoC.getInstance().player.cocks.length > 0 ) {
			if( CoC.getInstance().player.cocks.length === 1 ) {
				...
				if( CoC.getInstance().player.normalCocks() === 1 ) {
					...
					EngineCore.outputText( 'swelling, bulbous knot.  You feel it growing tighter and tighter until it\'s nearly twice the width of your ' + CoC.getInstance().player.cockDescript() + '.  The pressure is an agonizing pleasure that only builds higher and higher as you come closer and closer to orgasm.  ' );
				} else if( CoC.getInstance().player.horseCocks() === 1 ) {
					EngineCore.outputText( 'swollen equine sheath, slowly beginning to work its way up your shaft.  Pre-cum begins pouring from your ' + CoC.getInstance().player.cockDescript() + ', slicking your ' + CoC.getInstance().player.cockDescript() + ' as you get ready to blow.  ' );
				} else if( CoC.getInstance().player.tentacleCocks() === 1 ) {
					EngineCore.outputText( 'wiggling vine-like cock, growing rapidly.  You feel the rubbery surface of your tentacle prick contracting as it nears release.  Thick bulges of fluids collect and travel along the length of your ' + CoC.getInstance().player.cockDescript() + ', the first of which is almost to your oversized tip.  ' );
				} else if( CoC.getInstance().player.demonCocks() === 1 ) {
					EngineCore.outputText( 'tainted man-meat, growing rapidly.  You feel the nodules close to the base swell and pulse, starting a rippling wave of pleasure that migrates upwards.  The ring of bumps around your crown double in size, flaring out as your ' + CoC.getInstance().player.cockDescript() + ' prepares to unload.  ' );
				} else if( CoC.getInstance().player.catCocks() === 1 ) {
					EngineCore.outputText( 'full, cat-like sheath.  You feel it tingling and throbbing as the spines pulsate with arousal.  You feel the barbs at the bottom thickening as bulges of fluid migrate through your ' + CoC.getInstance().player.cockDescript() + ' towards the tip.  ' );
				} else if( CoC.getInstance().player.lizardCocks() === 1 ) {
					EngineCore.outputText( 'swollen member.  You can feel it tingling and bulging strangly as it begins to contract and pulse.  Pre-cum leaks from your ' + CoC.getInstance().player.cockDescript() + ' in a steady stream as each \'bulb\' nearly doubles in size, and then one at a time, they deflate while your urethra dilates wide.  ' );
				} else if( CoC.getInstance().player.anemoneCocks() === 1 ) {
					EngineCore.outputText( 'base.  The tentacles surrounding your ' + CoC.getInstance().player.cockDescript() + ' go nuts, constricting around it, inadvertently firing aphrodisiacs into the orgasmic flesh as they wring a potent, hip-jerking climax from you.  ' );
				} else {
					EngineCore.outputText( 'twitching manhood, growing rapidly.  You stroke furiously, feeling the pressure of your cum as it nears release.  ' );
				}
				EngineCore.outputText( 'Pleasurable spasms overwhelm you as cum erupts from your ' + CoC.getInstance().player.cockDescript() );
				...
				if( selfSucking ) {
					if( CoC.getInstance().player.cumQ() < 25 ) {
						...
						EngineCore.outputText( 'Pulse after pulse of cum erupts from your ' + CoC.getInstance().player.cockDescript() + ' into your mouth.  You swallow what you can but it\'s too much for you.  Cum runs down your cock to pool on you as your orgasm drags on.  ' );
					} else {
						EngineCore.outputText( 'Your orgasm never seems to end, and your world dissolves into the feelings from your ' + CoC.getInstance().player.cockDescript() + ' as it erupts jet after jet of cum into your mouth.  You nearly gag, cum overflowing to spray out in a river, pooling around you.  ' );
					}
				} else if( nippleFuck ) {
					if( CoC.getInstance().player.cumQ() < 25 ) {
						...
						EngineCore.outputText( 'Your orgasm never seems to end, and your world dissolves into the feelings from your ' + CoC.getInstance().player.cockDescript() + ' as it erupts jet after jet of cum into your abused tit.  With a *pop*, your cock is forced free by the pressure. It continues to squirt semen everywhere, while a river of cum pours out of your dialated nipple, pooling around you.  ' );
					}
				} else {
					if( CoC.getInstance().player.cumQ() < 25 ) {
			...
			if( CoC.getInstance().player.cocks.length > 1 ) {
				...
				if( CoC.getInstance().player.dogCocks() > CoC.getInstance().player.normalCocks() && CoC.getInstance().player.dogCocks() > CoC.getInstance().player.horseCocks() ) { //Primary Dog
					var dogIndex;
					for( dogIndex = 0; dogIndex < CoC.getInstance().player.cocks.length; dogIndex++ ) {
						if( CoC.getInstance().player.cocks[ dogIndex ].cockType === CockTypesEnum.DOG ) {
							break;
						}
					}
					EngineCore.outputText( 'Your feel your knots bulging and swelling, growing tighter and tighter until they\'re nearly double the width of a ' + CoC.getInstance().player.cockDescript( dogIndex ) + '.  The agonizing pressure builds higher and tighter with every passing second as you get closer and closer to orgasm.  ' );
				} else if( CoC.getInstance().player.horseCocks() > CoC.getInstance().player.normalCocks() ) { //Primary Horse
					var horseIndex;
					for( horseIndex = 0; horseIndex < CoC.getInstance().player.cocks.length; horseIndex++ ) {
						if( CoC.getInstance().player.cocks[ horseIndex ].cockType === CockTypesEnum.HORSE ) {
							break;
						}
					}
					EngineCore.outputText( 'You feel a pulsing in your sheath, slowly working its way up your ' + CoC.getInstance().player.cockDescript( horseIndex ) + 's.  Pre-cum pours from your ' + CoC.getInstance().player.cockDescript( horseIndex ) + 's, slicking the wobbly equine shafts as they get ready to blow.  ' );
				} else { //Primary Normal
				...
				if( nippleFuck ) {
					if( CoC.getInstance().player.cumQ() < 25 ) {
						...
						EngineCore.outputText( 'With an explosive release, you finally cum.  Your orgasm never seems to end, and your world dissolves into the feelings from your ' + CoC.getInstance().player.multiCockDescriptLight() + ' as they erupt jet after jet of cum into your abused tits.  With a series of wet *pops*, your cocks are forced free by the pressure. They continue to squirt semen everywhere, while rivers of cum pour out of your dilated nipples, pooling around you.  ' );
					}
				}
				//Doing the perverted sprinkler after dicks have popped out.
				EngineCore.outputText( 'Pleasure overwhelms your fragile mind as cum erupts from your ' + CoC.getInstance().player.multiCockDescriptLight() + '.  Your hips jerk in the air in time with your eruptions, cum spraying from you as your body does its best impression of a perverted sprinkler.  ' );
				if( selfSucking ) {
					if( CoC.getInstance().player.cumQ() < 25 ) {
						...
						EngineCore.outputText( 'Pulse after pulse of cum erupts from your ' + CoC.getInstance().player.cockDescript() + ' into your mouth.  You swallow what you can but it\'s too much for you.  Cum runs down your ' + CoC.getInstance().player.cockDescript() + ' to pool on you as your orgasm drags on.  Jizz rains over you the entire time from the rest of your "equipment".' );
					} else {
						EngineCore.outputText( 'Your orgasm never seems to end, and your world dissolves into the feelings from your ' + CoC.getInstance().player.cockDescript() + ' as it erupts jet after jet of cum into your mouth.  You nearly gag, cum overflowing to spray out in a river, pooling around you.  Your other \'equipment\' rains jizz upon you the whole while, soaking you in a cum-puddle.' );
					}
				}
				//These seem like they should always be displayed regardless of other factors.
				if( CoC.getInstance().player.cumQ() < 5 ) {
					EngineCore.outputText( 'A few thick spurts of cum burst from your cocks, splattering you liberally.  ' );
				} else if( CoC.getInstance().player.cumQ() < 7 ) {
					EngineCore.outputText( 'The orgasm drags on and on, spurt after spurt of jism coating you from each cock.  ' );
				} else if( CoC.getInstance().player.cumQ() < 10 ) {
		...
		if( CoC.getInstance().player.vaginas.length > 0 ) {
			...
			if( CoC.getInstance().player.vaginas.length === 1 ) {
				...
				} else if( CoC.getInstance().player.clitLength > 1.5 ) {
				...
				if( CoC.getInstance().player.vaginas[ 0 ].vaginalWetness === AppearanceDefs.VAGINA_WETNESS_SLAVERING ) {
					EngineCore.outputText( 'A veritable gush of fluid explodes from your nethers, pulsing in time with the ripples of your ' + CoC.getInstance().player.vaginaDescript() + '.  ' );
				}
			}
		...
		if( CoC.getInstance().player.averageLactation() >= 2 || CoC.getInstance().player.hasFuckableNipples() ) {
			//FUCK ANOTHER SHODDILY WRITTEN FUNCTION TO DEBUG/PORT
			//WHYYYYY
			this.titCum( CoC.getInstance().player.cumQ() );
		}
		//DONE!
	...
	Masturbation.prototype.titCum = function( cumQuantity ) {
		...
		if( CoC.getInstance().player.averageLactation() >= 2 && !CoC.getInstance().player.hasFuckableNipples() ) {
			...
			if( CoC.getInstance().player.averageLactation() <= 2.6 ) {
				EngineCore.outputText( 'thin streams of milk spray out of your ' );
			}
			if( CoC.getInstance().player.averageLactation() > 2.6 && CoC.getInstance().player.averageLactation() < 3 ) {
				EngineCore.outputText( 'thick gouts of milk spurt out of your ' );
			}
			if( CoC.getInstance().player.averageLactation() >= 3 ) {
				EngineCore.outputText( 'huge torrents of milk blast out of your ' );
			}
			//different description based on size of nips
			if( CoC.getInstance().player.averageNippleLength() <= 1 ) {
				EngineCore.outputText( 'erect nipples, ' );
			}
			if( CoC.getInstance().player.averageNippleLength() > 1 && CoC.getInstance().player.averageNippleLength() < 4 ) {
				EngineCore.outputText( 'fat, puckered nipples, ' );
			}
			if( CoC.getInstance().player.averageNippleLength() >= 4 ) {
		...
		if( CoC.getInstance().player.hasFuckableNipples() ) {
			...
			if( CoC.getInstance().player.averageVaginalWetness() >= 2 && CoC.getInstance().player.averageVaginalWetness() < 4 ) {
	...
	Masturbation.prototype.deluxeDildo = function() {
		...
		if( CoC.getInstance().player.hasVagina() ) {
			...
			EngineCore.outputText( '  A drop of pink aphrodisiac leaks from the tip, offering you a hint of the pleasure you\'re in for.   You make sure to catch it on your crotch, letting the fluid seep into your ' + CoC.getInstance().player.vaginaDescript() + '.  Warmth radiates outwards, spreading to your thighs as your ' + CoC.getInstance().player.clitDescript() );
			...
			} else if( CoC.getInstance().player.clitLength < 6 ) {
			...
			if( CoC.getInstance().player.cor < 50 ) {
				EngineCore.outputText( 'before slowly working it inside you, gasping at the enhanced sensitivity of your ' + CoC.getInstance().player.vaginaDescript() + '.' );
			} else {
				EngineCore.outputText( 'ramming it deep inside you, moaning as it rubs your now over-sensitive walls.' );
			}
			EngineCore.outputText( '  You splay your ' + CoC.getInstance().player.legs() + ' and lie there with it inside you, feeling it respond to your wetness, becoming more and more turned on by the second.\n\n' );
			//(Kinda dry)
			if( CoC.getInstance().player.vaginas[ 0 ].vaginalWetness < AppearanceDefs.VAGINA_WETNESS_SLICK ) {
				EngineCore.outputText( 'The thickness of the toy gradually increases, filling you more and more effectively as it reacts to your bodily fluids.  You grab it two-handed and start slamming it into your ' + CoC.getInstance().player.vaginaDescript() + ', vigorously fucking yourself with the swelling dong.  The sensations just keep getting better and better as more and more of the goblin\'s sex-drug leaks into you.  Even your ' + CoC.getInstance().player.clitDescript() + ' and cunt-lips tingle with need.  You answer that need by picking up the pace, pistoning faster and faster.\n\n' );
			}//(Pretty wet)
			else if( CoC.getInstance().player.vaginas[ 0 ].vaginalWetness < AppearanceDefs.VAGINA_WETNESS_SLAVERING ) {
				EngineCore.outputText( 'The toy\'s girth seems to pulse and swell within you, spreading you wide open as it sops up your natural wetness and grows larger.  You grab it in a two-handed grip and begin working it in and out of your ' + CoC.getInstance().player.vaginaDescript() + ', gasping and twitching as every ridge and feature of the dildo rubs you just right.  Every inch of your nethers tingles with a desire to be touched, rubbed, and squeezed. ' );
				if( CoC.getInstance().player.cocks.length > 0 ) {
					EngineCore.outputText( 'Even your ' + CoC.getInstance().player.multiCockDescript() + ' ache' );
	...
	Masturbation.prototype.onaholeUse = function() {
		...
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.PlainOnaholeUsed ) < 0 ) {
			CoC.getInstance().player.createStatusAffect( StatusAffects.PlainOnaholeUsed, 0, 0, 0, 0 );
			...
			if( CoC.getInstance().player.cocks.length > 1 ) {
				EngineCore.outputText( 'With your free hand, you slap your ' + CoC.getInstance().player.multiCockDescriptLight() + ' to \'attention\' and ease the onahole over your cocks.  ' );
			} else {
		...
		//If CoC.getInstance().player is already flagged, show repeated use text!
	...
	Masturbation.prototype.deluxeOnaholeUse = function() {
		EngineCore.clearOutput();
		//Flag after first use!
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.DeluxeOnaholeUsed ) < 0 ) {
			CoC.getInstance().player.createStatusAffect( StatusAffects.DeluxeOnaholeUsed, 0, 0, 0, 0 );
	...
	Masturbation.prototype.onaholeContinuation = function() {
		if( CoC.getInstance().player.cocks.length > 1 ) {
			if( CoC.getInstance().player.gender === 3 && Utils.rand( 2 ) === 0 ) {
				EngineCore.doNext( this.onaholeFutaContinuation );
			} else {
				EngineCore.doNext( this.onaholeMulticockContinuation );
			}
	...
	Masturbation.prototype.onaholeMulticockContinuation = function() {
		EngineCore.outputText( 'You pull the sloppy toy from your dribbling dick and smile, shoving its slippery surface down on another of your ' + CoC.getInstance().player.multiCockDescriptLight() + '.  You rapidly work it around your cocks, orgasming until ' );
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( 'you pass out with aching, empty balls.' );
		} else {
			EngineCore.outputText( 'you pass out with ' + CoC.getInstance().player.multiCockDescriptLight() + ' sore from exertion.' );
		}
	...
	Masturbation.prototype.onaholeFutaContinuation = function() {
		...
		EngineCore.outputText( String( Math.ceil( ((Math.random() * CoC.getInstance().player.str / 12) + CoC.getInstance().player.str / 6) * 10 ) / 10 ) );
	...
	Masturbation.prototype.allNaturalOnaholeUse = function() {
		EngineCore.clearOutput();
		//First use!
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.AllNaturalOnaholeUsed ) < 0 ) {
			CoC.getInstance().player.createStatusAffect( StatusAffects.AllNaturalOnaholeUsed, 0, 0, 0, 0 );
	...
	Masturbation.prototype.stimBeltUse = function() {
		EngineCore.clearOutput();
		//FIRST TIME USAGE
		if( (CoC.getInstance().player.hasKeyItem( 'Self-Stimulation Belt' ) >= 0) ) {
			//First use! Flag after first use!
			if( CoC.getInstance().player.findStatusAffect( StatusAffects.used_self_dash_stim ) < 0 ) {
				CoC.getInstance().player.createStatusAffect( StatusAffects.used_self_dash_stim, 0, 0, 0, 0 );
				...
				EngineCore.outputText( CoC.getInstance().player.breastCup( 0 ) );
	...
	Masturbation.prototype.allNaturalStimBeltUse = function() {
		EngineCore.clearOutput();
		if( CoC.getInstance().player.hasKeyItem( 'All-Natural Self-Stimulation Belt' ) >= 0 ) {
			//First time!
			if( CoC.getInstance().player.findStatusAffect( StatusAffects.UsedNaturalSelfStim ) < 0 ) {
				//Flag as used!
				CoC.getInstance().player.createStatusAffect( StatusAffects.UsedNaturalSelfStim, 0, 0, 0, 0 );
				...
				EngineCore.doNext( this.allNaturalSelfStimulationBeltContinuation );
			}
			//Multiple uses
			else {
				//Low corruption variant
		...
		CoC.getInstance().player.cuntChange( 1, true );
		CoC.getInstance().player.slimeFeed();
	};
	...
	Masturbation.prototype.lickYerGirlParts = function() { //Female cat masturbation
		...
		if( CoC.getInstance().flags[ kFLAGS.TIMES_AUTOFELLATIO_DUE_TO_CAT_FLEXABILITY ] === 0 ) {
			CoC.getInstance().flags[ kFLAGS.TIMES_AUTOFELLATIO_DUE_TO_CAT_FLEXABILITY ]++;
			EngineCore.outputText( 'You take off your ' + CoC.getInstance().player.armorName + ' and take a seat on the ground. You spread your legs and look down at your sex. It\'s aching for something more than just your fingers, and you have a craving to taste the lustful juices leaking out. A very perverted idea flashes through your brain, putting a smile on your face. You lay on your side and spread your legs again, giving you a perfect view of your ' + CoC.getInstance().player.vaginaDescript() + '. You lean your head down with your tongue sticking out; closer and closer you come to your own cunt, feeling the heat from your puss flowing against your face as your own hot breath returns the warmth. You\'re only a small distance away from tasting it before you can\'t bend any farther.  Your cunny can almost feel your tongue wriggling its slimy warm wetness only a few centimeters away. You pull your head back and let out a frustrated sigh before you remember how the cats in your village got to those hard to reach places stretched one of their legs straight up. Following their example, you point one leg straight to the sky and close your eyes as you plunge your head down. You slowly open one eye to see that you\'re face to face with your ' + CoC.getInstance().player.vaginaDescript() + '; you\'re amazed that you are actually able to do it. You begin lapping your tongue up and down the slutty snatch.\n\n' );
			EngineCore.outputText( 'The feeling is amazing, as you flick your tongue across your swollen ' + CoC.getInstance().player.clitDescript() + '. Juices leak from your moist hole, filling your mouth with the sweet taste of girlcum. You can feel your entire sex pulsing and throbbing around your tongue as you plumb the depths of your ' + CoC.getInstance().player.vaginaDescript() + '.  The vibrations from your moans and screams of pleasure cause the intense feelings to increase, making you a slave to your own needs. Your juices flow down your chin as you try to swallow every last drop. You crane your neck, giving you deeper access to your wanting honey pot. You can feel yourself tightening around your tongue as it rams into your pussy, soaking up the juices as you slowly reach your peak.  You lick and suck hard around your ' + CoC.getInstance().player.clitDescript() + ', using both your hands to spread your cunt farther open. You move your head in an up and down motion just like a cat when they groom themselves. Your lapping and tongue play continues until you can feel your body tense up, ready to cum.\n\n' );
	...
	Masturbation.prototype.catAutoLick = function() { //Male cat masturbation
		...
		if( CoC.getInstance().flags[ kFLAGS.TIMES_AUTOFELLATIO_DUE_TO_CAT_FLEXABILITY ] === 0 ) {
			...
			if( CoC.getInstance().player.countCockSocks( 'gilded' ) > 0 && CoC.getInstance().flags[ kFLAGS.GILDED_JERKED ] < CoC.getInstance().player.countCockSocks( 'gilded' ) ) {
				...
				EngineCore.outputText( ' gems falls down upon your body instead of cum, bouncing off your ' + CoC.getInstance().player.skinFurScales() + ' as you arc your back higher and higher until the only thing touching the ground is the top of your head and the tips of your toes! Your hips continue jerking in the air from the intense orgasm for a little while after the cum stops flowing.\n\n' );
		...
		//[Repeatable]
			...
			if( CoC.getInstance().player.countCockSocks( 'gilded' ) > 0 && CoC.getInstance().flags[ kFLAGS.GILDED_JERKED ] < CoC.getInstance().player.countCockSocks( 'gilded' ) ) {
				...
				CoC.getInstance().player.gems += gemsCreated;
			} else {
				if( CoC.getInstance().flags[ kFLAGS.TIMES_AUTOFELLATIO_DUE_TO_CAT_FLEXABILITY ] > 10 && CoC.getInstance().player.balls > 1 && Utils.rand( 5 ) === 0 ) {
					EngineCore.outputText( 'But it\'s not enough and you are forced to come up for air.' );
					EngineCore.outputText( '\n\nWhile gasping for air, you scowl at your ' + CoC.getInstance().player.cockDescript() + ' in disapproval. That\'s when your ' + CoC.getInstance().player.sackDescript() + ' catches your attention. It\'s gleaming with your sex sheen and you watch one of your balls slowly slide off to one side.' );
					EngineCore.outputText( '\n\nYou think you can make it! You throw your other leg over your head and both feet come to rest on your back. You push your mouth towards your ' + CoC.getInstance().player.sackDescript() + ', slowly walking your toes down your back. You are only an inch from your ' + CoC.getInstance().player.ballsDescriptLight() + ' now and your own aroma fills your nostrils, spurring your on. Then, finally, you are there! And it nearly knocks you out! The sensation of your own balls in your mouth is incredible--you can feel them churning in your mouth--and your [feet] start kneeding your back. You bring your hands up to massage your ' + CoC.getInstance().player.ballsDescriptLight() + ' as well, making them take turns in your mouth. Completely intoxicated by your own scent you loose all track of time--there is only the bliss of sucking, licking, and massaging your own balls...' );
					EngineCore.outputText( '\n\nAbrubtly you realize that your ' + CoC.getInstance().player.sMultiCockDesc() + ' have soaked your torso in precum--apparently you\'ve been on the edge for some time. In one swift motion, you pick your head up and slam your mouth down over your ' + CoC.getInstance().player.cockDescript() + ' leaving your hands to continue their ball massage.' );
					//add cum quantity conditional text...
					EngineCore.outputText( '\n\nThe release is immediate. You feel the contractions of your climax against your [face], your [feet] involuntarly start massaging your back, and you feel your ' + CoC.getInstance().player.cockDescript() + ' expand and contract in your mouth as seed pumps through it into your stomach. It\'s more relaxing than anything else--each contraction makes you feel like you might be melting a little, like you might remain in this position forever.' );
					EngineCore.outputText( '\n\nYour [feet] and hands are still masagging their respective charges when you realize you are starting to go flaccid! You make an attempt to massage every last drop of cum from your ' + CoC.getInstance().player.ballsDescriptLight() + ', sucking on your ' + CoC.getInstance().player.cockDescript() + ' continuously as it goes down. It slowly slips from your mouth once it\'s completely deflated, causing you to frown slightly. Still feeling great in this position, you go down on your ' + CoC.getInstance().player.sackDescript() + ' again--alternating between licking and sucking them with your mouth and massaging them with your hands.' );
					...
					CoC.getInstance().player.modCumMultiplier( 0.3 );
				} else {
	...
	Masturbation.prototype.meditate = function() {
		...
		EngineCore.dynStats( 'lus', -50, 'cor', -0.3 - 0.3 * CoC.getInstance().player.countCockSocks( 'alabaster' ) );
	...
	Masturbation.prototype.dualBeltMasturbation = function() {
		...
		//([If high lactation]
		if( CoC.getInstance().player.biggestLactation() >= 2 ) {
			EngineCore.outputText( 'Milk gushes out from your ' + CoC.getInstance().player.breastDescript( 0 ) + ' as you orgasm, filling the inside of the suit with a slick layer of milk and forming milk bubbles that hang lewdly off your chest. ' );
		}
		...
		EngineCore.outputText( 'When you wake, the black latex is no longer covering your body and the belt is silent around your waist. Cum drips from the tip of your cock and the top part of your ' + CoC.getInstance().player.legs() + ' are coated with your feminine juices. ' );
		if( CoC.getInstance().player.biggestLactation() >= 2 ) {
			EngineCore.outputText( 'Thin streams of creamy milk flow from your ' + CoC.getInstance().player.allBreastsDescript() + ', your torso and midsection dripping wet from the stuff. ' );
		}
		...
		if( CoC.getInstance().player.sens > 40 ) {
			CoC.getInstance().player.sens -= 1;
		}
		if( CoC.getInstance().player.sens > 60 ) {
			CoC.getInstance().player.sens -= 1;
		}
		if( CoC.getInstance().player.sens > 80 ) {
			CoC.getInstance().player.sens -= 1;
	...
	Masturbation.prototype.centaurMasturbation = function() {
		EngineCore.clearOutput();
		var canMasturbateHugeCock = CoC.getInstance().player.hasCock() && (CoC.getInstance().player.tallness * (5 / 6) < CoC.getInstance().player.cocks[ CoC.getInstance().player.longestCock() ].cockLength);
		if( CoC.getInstance().player.hasFuckableNipples() ) {
	...
	Masturbation.prototype.centaurNippleCunt = function() {
		EngineCore.outputText( 'You shrug out of your ' + CoC.getInstance().player.armorName + ', ' );
		if( CoC.getInstance().flags[ kFLAGS.PC_FETISH ] > 0 ) {
			EngineCore.outputText( 'panting lustily as you envision being caught masturbating your ' + CoC.getInstance().player.nippleDescript( 0 ) + 's.' );
		} else if( CoC.getInstance().player.cor < 33 ) {
			EngineCore.outputText( 'blushing a bit as you look down at your ' + CoC.getInstance().player.nippleDescript( 0 ) + 's.' );
		} else if( CoC.getInstance().player.cor < 66 ) {
			EngineCore.outputText( 'shivering as the air hits your exposed ' + CoC.getInstance().player.nippleDescript( 0 ) + 's.' );
		} else {
			EngineCore.outputText( 'smiling to yourself as you blatantly oogle your ' + CoC.getInstance().player.nippleDescript( 0 ) + 's.' );
		}
		...
		if( CoC.getInstance().player.biggestTitSize() > 2 ) {
			EngineCore.outputText( '  The sudden displacement makes your ' + CoC.getInstance().player.breastDescript( 0 ) + ' jiggle enticingly, and the sudden movement fires arcs of pleasure deep into your body.' );
		} else {
		...
		EngineCore.outputText( '  Your free fingers slowly stroke around the outer edges of your ' + CoC.getInstance().player.nippleDescript( 0 ) + 's\' lips, pausing when you feel a tiny sensitive clit-like nub just inside the top of the opening.\n\n' );
		EngineCore.outputText( 'Shivering with pleasure, you kneel down to prevent your legs from going out from underneath you.   Your ' + CoC.getInstance().player.face() + ' flushes hotter as you toy with the slippery nipple-cunts, feeling them growing hard and puffy as you turn yourself on more and more.  ' );
		...
		if( CoC.getInstance().player.hasVagina() || CoC.getInstance().player.hasCock() ) {
			...
			if( CoC.getInstance().player.hasCock() ) {
				if( CoC.getInstance().player.hasVagina() ) {
					EngineCore.outputText( CoC.getInstance().player.multiCockDescriptLight() + ' and ' + CoC.getInstance().player.vaginaDescript() );
				} else {
					EngineCore.outputText( CoC.getInstance().player.cockDescript() );
				}
				//Set as plural if multi dick or dick and vag.
				if( CoC.getInstance().player.hasVagina() || CoC.getInstance().player.totalCocks() > 1 ) {
					plural = true;
				}
			} else {
				EngineCore.outputText( CoC.getInstance().player.vaginaDescript() );
			}
			if( plural ) {
			...
			if( CoC.getInstance().player.totalCocks() > 1 ) {
				EngineCore.outputText( '  You feel each of your ' + CoC.getInstance().player.multiCockDescriptLight() + ' tapping against your belly as your muscles clench, getting as much pleasure as possible from your male endowments.' );
			} else if( CoC.getInstance().player.totalCocks() === 1 ) {
				EngineCore.outputText( '  You feel your ' + CoC.getInstance().player.multiCockDescriptLight() + ' tapping against your belly as your muscles clench, getting as much pleasure as possible from your male endowment.' );
			}
			if( CoC.getInstance().player.balls > 0 ) {
				EngineCore.outputText( '  Rocking slightly inside your sack, your ' + CoC.getInstance().player.ballsDescriptLight() + ' repeatedly clench up against your body, ready to release.' );
			}
			EngineCore.outputText( '\n\n' );
		}
		//(ORGAZMO)
		EngineCore.outputText( 'A slow wave of contractions starts deep inside each of your ' + CoC.getInstance().player.nippleDescript( 0 ) + 's.  It intensifies as it rises towards the surface of your ' + CoC.getInstance().player.breastDescript( 0 ) + ', reaching a crescendo that brings you to the peak of pleasure.  Your eyes roll back as you slump down in orgasmic bliss, fingers pumping relentlessly at you slippery nipple-holes.  Noisy wet squelches and incessant moaning rouse you from your pleasure-induced coma, eventually waking you back to reality.\n\n' );
		...
		if( CoC.getInstance().player.hasCock() ) {
			...
			EngineCore.outputText( 'released from your ' + CoC.getInstance().player.multiCockDescriptLight() + ' by the intense climax.' );
			if( CoC.getInstance().player.hasVagina() ) {
				EngineCore.outputText( '  ' );
			}
		}
		...
		//Real aftermath
		EngineCore.outputText( 'Judging by the sky, at least an hour has passed.  You sigh and pry your cramped fingers from your aching ' + CoC.getInstance().player.nippleDescript( 0 ) + 's, rubbing the sore entrances with your palm before you climb back up onto your ' + CoC.getInstance().player.feet() + '.  As you get dressed, you\'re very conscious of how much better you feel from the wonderful finger-fuck.  ' );
		if( CoC.getInstance().player.cor > 66 ) {
	...
	Masturbation.prototype.centaurHugeCock = function() {
		...
		var primary = CoC.getInstance().player.longestCock();
		var plural = (CoC.getInstance().player.cocks.length > 1);
		EngineCore.outputText( 'You shrug out of your ' + CoC.getInstance().player.armorName + ', not that it does much to impede your ' + CoC.getInstance().player.multiCockDescriptLight() + ' as a centaur.   ' );
		...
		EngineCore.outputText( 'your ' + CoC.getInstance().player.multiCockDescriptLight() + ' swells out underneath you, hanging down from your equine hindquarters as ' );
		if( !plural ) {
		...
		if( plural ) {
			EngineCore.outputText( '  After a moment they peek out from under your forelegs, proudly displaying your ' + CoC.getInstance().player.multiCockDescriptLight() + '.' );
		} else {
			EngineCore.outputText( '  After a moment it peeks out from under your forelegs, proudly displaying your ' );
			if( CoC.getInstance().player.cocks[ primary ].cockType === CockTypesEnum.HORSE ) {
				EngineCore.outputText( 'flared tip' );
			} else if( CoC.getInstance().player.cocks[ primary ].cockType === CockTypesEnum.DEMON ) {
				EngineCore.outputText( 'nodule-ringed head' );
			} else if( CoC.getInstance().player.cocks[ primary ].cockType === CockTypesEnum.DOG ) {
				EngineCore.outputText( 'pointed erection' );
			} else if( CoC.getInstance().player.cocks[ primary ].cockType === CockTypesEnum.TENTACLE ) {
				EngineCore.outputText( 'wriggling, mushroom-like tip' );
			} else if( CoC.getInstance().player.cocks[ primary ].cockType === CockTypesEnum.CAT ) {
				EngineCore.outputText( 'spiny head' );
			} else if( CoC.getInstance().player.cocks[ primary ].cockType === CockTypesEnum.LIZARD ) {
				EngineCore.outputText( 'pointed, purple tip' );
			} else if( CoC.getInstance().player.cocks[ primary ].cockType === CockTypesEnum.ANEMONE ) {
		...
		if( CoC.getInstance().player.tallness * 1.5 < CoC.getInstance().player.cocks[ CoC.getInstance().player.longestCock() ].cockLength ) {
			EngineCore.outputText( '  In no time flat you\'re rendered virtually immobile by the length of ' );
			if( plural ) {
				EngineCore.outputText( 'all ' );
			}
			EngineCore.outputText( 'your ' + CoC.getInstance().player.multiCockDescriptLight() + '.' );
		}
		EngineCore.outputText( '\n\n' );
		...
		if( CoC.getInstance().player.tallness * 1.5 >= CoC.getInstance().player.cocks[ CoC.getInstance().player.longestCock() ].cockLength ) {
			...
			EngineCore.outputText( CoC.getInstance().player.cockDescript( primary ) + ' and lift up, pressing it tightly against your belly as you begin to stroke along the sensitive underside.  ' );
			...
			if( CoC.getInstance().player.hasVagina() ) {
				EngineCore.outputText( '  Your neglected ' + CoC.getInstance().player.vaginaDescript() + ' aches for penetration, but there\'s no way you\'ll be able to reach it in your current state.' );
			}
			...
			if( CoC.getInstance().player.totalCocks() === 2 ) {
				...
				EngineCore.outputText( '  Your other ' + CoC.getInstance().player.multiCockDescriptLight() + ' mimic their lucky brother\'s pleasure, even though they\'ve been ignored in order for you to focus on your current \'toy\'.' );
			}
		...
		//Get started (long)
			...
			EngineCore.outputText( CoC.getInstance().player.cockDescript( primary ) + ' and lift it up, curving it slightly as you begin to stroke the underside with long, fluid caresses.  You even manage to bend down and give it a lick, forcing you into a pleasurable shudder.' );
			if( CoC.getInstance().player.cumQ() > 500 ) {
				EngineCore.outputText( '  Thick streams of pre-cum start to drizzle from your tip, forced out due to your prodigious production of sexual fluid.' );
			} else if( CoC.getInstance().player.cumQ() > 100 ) {
				EngineCore.outputText( '  A dribble of pre-cum starts to leak from the tip as you get more into the act.' );
			...
			//STROKE (long)
			EngineCore.outputText( 'You smear your pre-cum over the tip, rolling your palm across the sensitive tip in a way that makes it hard to stand.  Meanwhile your other hand is making great use of your incredible length, jacking you off with long fluid strokes.  In no time flat, you can feel your ' + CoC.getInstance().player.cockDescript() + ' pulsing in your grip, growing harder in time with your fevered stroking.' );
			if( CoC.getInstance().player.totalCocks() === 2 ) {
				EngineCore.outputText( '  Your other dick mimics its lucky brother\'s pleasure, even though it\'s been ignored in order for you to focus on your current \'toy\'.' );
			} else if( CoC.getInstance().player.totalCocks() > 2 ) {
				EngineCore.outputText( '  Your other ' + CoC.getInstance().player.multiCockDescriptLight() + ' mimic their lucky brother\'s pleasure, even though they\'ve been ignored in order for you to focus on your current \'toy\'.' );
		...
		//Cumjaculation
		if( CoC.getInstance().player.tallness * 1.5 >= CoC.getInstance().player.cocks[ CoC.getInstance().player.longestCock() ].cockLength ) {
			EngineCore.outputText( 'Squeezing tightly around your ' + CoC.getInstance().player.cockHead() );
		} else {
			EngineCore.outputText( 'Pumping hard' );
		}
		EngineCore.outputText( ', you orgasm, feeling relief flow through you with every pump of ' );
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( 'your ' + CoC.getInstance().player.ballsDescriptLight() );
		} else {
		...
		} else if( CoC.getInstance().player.minLust() > 40 || CoC.getInstance().player.lib > 50 ) {
			if( plural ) {
		...
		if( CoC.getInstance().player.hasVagina() ) {
			...
			if( CoC.getInstance().player.hasFuckableNipples() ) {
				...
				EngineCore.outputText( ' of your ' + Utils.num2Text( CoC.getInstance().player.totalNipples() ) + ' ' + CoC.getInstance().player.nippleDescript( 0 ) + 's quiver, ' );
				if( CoC.getInstance().player.wetness() < 4 && CoC.getInstance().player.biggestLactation() < 2 ) {
					EngineCore.outputText( 'leaking' );
				} else if( CoC.getInstance().player.wetness() < 5 && CoC.getInstance().player.biggestLactation() < 3 ) {
	...
	Masturbation.prototype.centaurCantMasturbate = function() {
		if( CoC.getInstance().flags[ kFLAGS.UNABLE_TO_MASTURBATE_BECAUSE_CENTAUR ] === 0 ) {
			...
			CoC.getInstance().flags[ kFLAGS.UNABLE_TO_MASTURBATE_BECAUSE_CENTAUR ]++;
		} else {
			EngineCore.outputText( 'You still can\'t reach around to masturbate yourself.  Being half-horse sure is inconvenient!\n\n' );
		}
		//(If Milker)
		if( CoC.getInstance().player.hasKeyItem( 'Cock Milker - Installed At Whitney\'s Farm' ) >= 0 && CoC.getInstance().player.cocks.length > 0 ) {
	...
	Masturbation.prototype.centaurDudesGetHorseAids = function() {
		...
		EngineCore.clearOutput();
		if( CoC.getInstance().player.keyItemv1( 'Fake Mare' ) === 0 ) {
			if( CoC.getInstance().player.cor < 50 ) {
		...
		EngineCore.outputText( 'Seeing the toy\'s exposed, gaping genitals, you feel a stirring in your ' + CoC.getInstance().player.multiCockDescriptLight() + '.  You yearn to touch yourself, but your centaur lower body prevents you, as usual.  Grunting with annoyance, you trot up to the toy and give its wide cunt an experimental fisting.  ' );
		//[If small cock
		...
		EngineCore.outputText( 'The toy\'s passage seems to shift and contract around your ' + CoC.getInstance().player.cockDescript( x ) + ', molding itself to perfectly sheathe you.  What a marvelous little toy!  You slide on up until you hilt yourself, your crotch pressed against the mare\'s wide ass as your ' + CoC.getInstance().player.chestDesc() + ' squeezes against her back.  Now fully mounted, you begin to rut on the mare toy, slapping your hips' );
		if( CoC.getInstance().player.balls > 0 ) {
			EngineCore.outputText( ' and ' + CoC.getInstance().player.ballsDescriptLight() );
		}
		...
		EngineCore.outputText( '.\n\n' );
		EngineCore.outputText( 'Finally able to get the release you weren\'t able to give yourself, you feel an orgasm mounting deep within you.  You let out a loud, equine whinny as you hump the toy faster and faster, pounding her with your ' + CoC.getInstance().player.cockDescript( x ) + ' until the pleasure overwhelms you.  You cry out as you cum, squirting your centaur-cum as far into the toy as you can shoot it' );
		if( CoC.getInstance().player.countCockSocks( 'gilded' ) > 0 && CoC.getInstance().flags[ kFLAGS.GILDED_JERKED ] < CoC.getInstance().player.countCockSocks( 'gilded' ) ) {
			CoC.getInstance().flags[ kFLAGS.GILDED_JERKED ]++;
			var gems = this.midasCockJackingGemsRoll();
			EngineCore.outputText( '.\n\nSated, you spend a few blissful minutes enjoying the warmth and tightness of the mare-like onahole until your ' + CoC.getInstance().player.cockDescript( x ) + ' is soft inside it.  You scamper off of her, dropping back to your four equine feet.  <b>You are startled to see glittering gems pouring from her ' );
			if( CoC.getInstance().player.cockArea( x ) >= 30 ) {
				EngineCore.outputText( 'horsecunt' );
			} else {
				EngineCore.outputText( 'asshole' );
			...
			EngineCore.outputText( 'Sated, you spend a few blissful minutes enjoying the warmth and tightness of the mare-like onahole until your ' + CoC.getInstance().player.cockDescript( x ) + ' is soft inside it.  You scamper off of her, dropping back to your four equine feet.  With a contented yawn, you disassemble the toy and haul it off back to your stash, leaking your cum the entire way.' );
		}
		CoC.getInstance().player.orgasm();
	...
	Masturbation.prototype.centaurGirlsGetHorseAids = function() {
		EngineCore.clearOutput();
		if( CoC.getInstance().player.keyItemv1( 'Centaur Pole' ) === 0 ) {
			//[If low Corruption
			if( CoC.getInstance().player.cor < 50 ) {
		...
		EngineCore.outputText( 'Your ' + Descriptors.hipDescript() + ' wiggle in anticipation as you work to get your ready ' + CoC.getInstance().player.vaginaDescript() + ' lined up with the horse dildo.  Unable to see the toy past your equine rump, it\'s a long, desperate minute until finally you feel its flared head against your horsecunt.  You shimmy back, gasping in delight as the meaty horsecock pushes into you.  It seems to inflate and expand inside you as you take it, until you\'re completely and utterly full of purple rubber -- and then some.  You grunt as the cock continues to grow, stretching your ' + CoC.getInstance().player.vaginaDescript() + ' until you let out a pained whimper.' );
		CoC.getInstance().player.cuntChange( CoC.getInstance().player.vaginalCapacity() - 3, true, true, false );
		EngineCore.outputText( '\n\n' );
	...
	Masturbation.prototype.getHugeEggsInCawk = function() {
		...
		if( CoC.getInstance().player.statusAffectv1( StatusAffects.Exgartuan ) === 1 && CoC.getInstance().player.statusAffectv2( StatusAffects.Exgartuan ) === 0 && CoC.getInstance().flags[ kFLAGS.TIMES_EGGED_IN_COCK ] === 0 ) {
		...
		if( CoC.getInstance().player.fertilizedEggs() > 0 && CoC.getInstance().flags[ kFLAGS.DICK_EGG_INCUBATION ] === 0 ) {
			CoC.getInstance().flags[ kFLAGS.DICK_EGG_INCUBATION ] = 48;
		}
	...
	Masturbation.prototype.birthBeeEggsOutYourWang = function() {
		...
		CoC.getInstance().flags[ kFLAGS.DICK_EGG_INCUBATION ] = 0;
		CoC.getInstance().player.orgasm();
	};
	...
	Masturbation.prototype.layEggsInYerTits = function() {
		EngineCore.clearOutput();
		if( CoC.getInstance().player.statusAffectv1( StatusAffects.Exgartuan ) === 2 && CoC.getInstance().player.statusAffectv2( StatusAffects.Exgartuan ) === 0 ) {
			...
			else if( CoC.getInstance().player.cocks.length === 1 ) {
		...
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.Eggchest ) < 0 ) {
			CoC.getInstance().player.createStatusAffect( StatusAffects.Eggchest, 3 + Utils.rand( 10 ), 1 + Utils.rand( 4 ), 0, 0 );
		}
		CoC.getInstance().player.orgasm();
		EngineCore.dynStats( 'sen', 1 );
		CoC.getInstance().player.dumpEggs();
	...
	Masturbation.prototype.tentacleSelfFuck = function() {
		var x = _.indexOf( CoC.getInstance().player.cocks, _.find( CoC.getInstance().player.cocks, function( cock ) {
			return cock.cockType === CockTypesEnum.TENTACLE;
		} ) );
		var y = _.indexOf( CoC.getInstance().player.cocks, _.find( _.shuffle( _.filter( CoC.getInstance().player.cocks, function( cock, index ) {
			return index !== x;
		} ) ), function( cock ) {
			return cock.cockType === CockTypesEnum.TENTACLE;
		} ) );
		...
		if( y >= 0 ) {
			...
			if( CoC.getInstance().player.cocks[ y ].cockType === CockTypesEnum.TENTACLE ) {
				...
				CoC.getInstance().player.buttChange( CoC.getInstance().player.cockArea( y ), true, true, false );
			}
		}
		...
		EngineCore.outputText( '.' );
		if( CoC.getInstance().player.lactationQ() >= 250 ) {
		...
		if( CoC.getInstance().player.tentacleCocks() >= 3 ) {
			...
			if( CoC.getInstance().player.tentacleCocks() < CoC.getInstance().player.cocks.length ) {
		...
		//No bonus cawks!
			...
			if( y >= 0 && CoC.getInstance().player.cocks[ y ].cockType === CockTypesEnum.TENTACLE && CoC.getInstance().player.cocks[ y ].cockLength >= 20 ) {
		...
		//One more
		else if( CoC.getInstance().player.tentacleCocks() >= 4 ) {
			EngineCore.outputText( '\n\nThe last green tool rises up above you.  It surveys the view before it in a crude approximation of a sentient surveyor, seeming to take in the shifting, squiggling view of your ardent masturbation.  In truth, you\'re just trying to think of where to stick it.  A droplet of liquid lust leaks from the tip in sympathetic pleasure, and it gives you an idea for just what to do with it.  It stretches out towards your torso but veers left at the last second.  Dripping onto your shoulder, the pulsating plant-prick slowly pushes into your armpit, lubricating your ' + CoC.getInstance().player.skinFurScales() + ' with its amorous liquid.  You\'re fucking your armpit and it feels divine, so good that you quickly slick your pit and twist the spare cock around yourself so that it can double-fuck both sides.' );
		}
		//JIZZBOMB
		...
		} else if( CoC.getInstance().player.tentacleCocks() > 2 ) {
	...
	Masturbation.prototype.tentacleGoesUpYerPooperNewsAtEleven = function() {
		var tentacle;
		for( tentacle = 0; tentacle < CoC.getInstance().player.cocks.length; tentacle++ ) {
			if( CoC.getInstance().player.cocks[ tentacle ].cockType === CockTypesEnum.TENTACLE ) {
				break;
			}
		}
		for( var x = tentacle + 1; x < CoC.getInstance().player.cocks.length; x++ ) { //Find the biggest tentacle cock you've got
			if( CoC.getInstance().player.cocks[ x ].cockType === CockTypesEnum.TENTACLE && CoC.getInstance().player.cocks[ x ].cArea() > CoC.getInstance().player.cocks[ tentacle ].cArea() ) {
				tentacle = x;
			}
		}
		EngineCore.clearOutput();
		...
		EngineCore.outputText( '  You groan and lazily remove your ' + CoC.getInstance().player.cockDescript( tentacle ) + ' from your anus as you give in to your pleasure-induced drowsiness.' );
		if( CoC.getInstance().player.cocks.length > 0 ) {
			//Single Cock
			if( CoC.getInstance().player.cocks.length === 1 ) {
				if( CoC.getInstance().player.lib < 30 ) {
	...
	Masturbation.prototype.gildedCockTurbate = function() {
		EngineCore.clearOutput();
		var gildedCock;
		for( gildedCock = 0; gildedCock < CoC.getInstance().player.cocks.length; gildedCock++ ) {
			if( CoC.getInstance().player.cocks[ gildedCock ].sock === 'gilded' ) {
				break;
			}
		}
		for( var x = gildedCock + 1; x < CoC.getInstance().player.cocks.length; x++ ) { //Find the biggest gilded cock you've got
			if( CoC.getInstance().player.cocks[ x ].sock === 'gilded' && CoC.getInstance().player.cocks[ x ].cArea() > CoC.getInstance().player.cocks[ gildedCock ].cArea() ) {
				gildedCock = x;
			}
		}
		...
		if( CoC.getInstance().player.cocks[ gildedCock ].cArea() < 6 ) {
		...
		else if( CoC.getInstance().player.cocks[ gildedCock ].cArea() < 20 ) {
		...
		EngineCore.outputText( '\n\nPressure builds, and tremors of pleasure run through your body as you stroke faster and faster.  You let out a small moan, as you pump the cocksleeve up and down your ' + CoC.getInstance().player.cockDescript( gildedCock ) + '.  Your cock swells within the confines of the sleeve, throbbing against the metallic confines of the sheath.  You shudder, arching your back, as you climax, your ' + Math.round( CoC.getInstance().player.cumQ() ) + 'mLs of cum arcing up high into the air overhead.' );
		...
		EngineCore.outputText( ' gems falls down upon your body instead of cum, bouncing off your ' + CoC.getInstance().player.skinFurScales() + '.' );
		var gems = this.midasCockJackingGemsRoll();
	...
	Masturbation.prototype.stickADildoInYourVagooSlut = function() {
		EngineCore.clearOutput();
		if( CoC.getInstance().player.hasVirginVagina() ) { //LOW CORRUPTION DEFLOWER
			if( CoC.getInstance().player.cor <= 50 ) {
				...
				CoC.getInstance().player.cuntChange( 8, true, true, false );
	...
	Masturbation.prototype.dildoButts = function() {
		...
		if( CoC.getInstance().player.analCapacity() <= 8 ) {
		...
		CoC.getInstance().player.buttChange( 8, true, true, false );
		if( CoC.getInstance().player.hasCock() && CoC.getInstance().player.hasVagina() ) {
			EngineCore.outputText( '\n\nYou moan in ecstasy while your [vagina] and ' + CoC.getInstance().player.multiCockDescriptLight() + ' erupt with sex juices. ' );
		} else if( CoC.getInstance().player.hasCock() ) {
			EngineCore.outputText( '\n\nYou moan in ecstasy while cum spurts from your ' + CoC.getInstance().player.multiCockDescriptLight() + '. ' );
	...
	Masturbation.prototype.midasCockJackingGemsRoll = function() {
		...
		if( CoC.getInstance().player.cumQ() < 1000 * cockSocks ) {
			gems += CoC.getInstance().player.cumQ() / 10;
		} else if( CoC.getInstance().player.cumQ() < 2500 * cockSocks ) {
			gems += CoC.getInstance().player.cumQ() / 50;
		} else if( CoC.getInstance().player.cumQ() < 5000 * cockSocks ) {
			gems += CoC.getInstance().player.cumQ() / 150;
		} else if( CoC.getInstance().player.cumQ() < 10000 * cockSocks ) {
			gems += CoC.getInstance().player.cumQ() / 450;
		} else {
			gems += CoC.getInstance().player.cumQ() / 1350;
		}
	...
	Masturbation.prototype.onaholeRepeatUse = function( corrupted ) {
		...
		if( corrupted ) {
			...
			EngineCore.outputText( 'Much to your annoyance and embarrassment, you feel the need to relieve yourself of your tension. Sighing in reservation, you remove the used onahole from your sack. Operating with a mind of its own, your ' + CoC.getInstance().player.cockDescript() + ' springs to attention, anticipating the coming stimulation and release. Your member easily pushes past the opening in the toy and you begin working your penis with some vigor. Pleasure begins to pulse through your ' + CoC.getInstance().player.cockDescript() + ' as you build yourself up. The force of the building fluids pushes against your inner organs, creating the paradoxical sensation of pain and pleasure. Pure instict takes over, and your hips begin to buck reflexively. With a sudden, sharp pinch at the base of your member, the need to cum takes over your body' );
			if( gemsCreated > 0 ) {
				...
				EngineCore.outputText( ' of gems falls down upon your body instead of cum, bouncing off your ' + CoC.getInstance().player.skinFurScales() + '.\n\n<b>You force yourself to pick up the ' + gemsCreated + ' gems before passing out.</b>' );
			} else {
