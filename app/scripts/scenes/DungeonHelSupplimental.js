...
angular.module( 'cocjs' ).run( function( SceneLib, GooArmorMonster, ArmorLib, WeaponLib, PregnancyStore, MainView, Combat, ConsumableLib, Utils, PerkLib, StatusAffects, OnLoadVariables, kFLAGS, CoC, EngineCore ) {
	...
	DungeonHelSupplimental.prototype.heliaDiscovery = function() {
		...
		if( CoC.flags[ kFLAGS.HEL_ISABELLA_THREESOME_ENABLED ] === 0 ) {
			...
			if( CoC.player.cor < 50 ) {
				MainView.outputText( 'quickly rush over and stop Hel before she loses a leg to one of your traps' );
			}
			//(dick);
			else {
				MainView.outputText( 'lazily point out your traps to the not-quite-intruder' );
			}
			MainView.outputText( ', and guide her over to the camp proper.' );
		}
		//(Else);
		else {
		...
		if( !CoC.player.isTaur() ) {
			MainView.outputText( '  Her hand slides over to rest on your thigh.' );
		} else {
			MainView.outputText( '  "<i>She gives your flank a slow, affectionate stroke.</i>"' );
		}
		MainView.outputText( '  You ask her if that\'s true. With a little wink, she answers, "<i>Well, it\'s not entirely untrue...</i>"' );
		...
		EngineCore.doYesNo( this, this.agreeToHelpHeliaDungeon, this, this.noDungeon );
	};
	//Intro Scene -- No;
	DungeonHelSupplimental.prototype.noDungeon = function() {
		MainView.clearOutput();
		MainView.outputText( 'You consider for a few moments, but ultimately decide that this is a venture you\'d rather not participate in.' );
	...
	DungeonHelSupplimental.prototype.agreeToHelpHeliaDungeon = function() {
		MainView.clearOutput();
		MainView.outputText( 'You mull the salamander\'s proposition over and eventually agree to assist her.  Not only will you be stopping a new race of monsters from spawning into the mountains, but you\'ll be getting into the lovely Helia\'s good graces - a win-win if ever there was one.' );
		...
		//[if has lap: ;
		if( !CoC.player.isTaur() ) {
			MainView.outputText( ' and cuddling up in your lap' );
		}
		MainView.outputText( '.  She nuzzles your neck and whispers, "<i>Thanks, Champ. It means a lot to know I can count on you to... watch my back.</i>"' );
	...
	DungeonHelSupplimental.prototype.morningAfterHeliaDungeonAgreements = function() {
		...
		if( CoC.player.biggestTitSize() > 3 ) {
			MainView.outputText( 'her head nestled between your soft tits and ' );
		}
		MainView.outputText( 'snoring boorishly.  The air around you smells like hot booze and sex, yet you awaken feeling as spirited and lively as you ever have.  You give Hel a little shake, waking her.' );
	...
	DungeonHelSupplimental.prototype.notYet = function() {
		MainView.clearOutput();
		MainView.outputText( 'You tell Hel you were only checking on her, and that you\'ve still got some things to do.  She sighs and quietly asks you to hurry.' );
		EngineCore.doNext( MainView, MainView.playerMenu );
	};
	//Introduction -- Dungeon;
	DungeonHelSupplimental.prototype.goToHeliaDungeon = function() {
		MainView.clearOutput();
		MainView.outputText( 'You tell Helia that yeah, you\'re as ready as you\'ll ever be.  She beams and grabs you in a tight hug.  "<i>Thanks again, [name].  You\'re a real goddamn champion, you know that?</i>"  You laugh it off, but the salamander gives you a sultry wink and starts off toward the mountains.  You\'re quick to follow her.' );
		//(NEXT);
		EngineCore.doNext( this, this.goToHeliaDungeon2 );
	};
	DungeonHelSupplimental.prototype.goToHeliaDungeon2 = function() {
		MainView.clearOutput();
		MainView.outputText( 'Within the hour, you and Helia are hiking up the narrow ledges and crevices of the high mountains, slowly but steadily climbing toward a snow-capped peak.  Hel certainly seems to know where she\'s going - she blazes a certain and steady trail, as if she knows every path and shortcut up the mountain.  By the time you near the peak, you\'re convinced she\'s been up here before - many times, even.' );
	...
	DungeonHelSupplimental.prototype.takeGooArmor = function() {
		MainView.clearOutput();
		MainView.outputText( 'You approach the armor rack.  A suit of heavy plated armor sits upon it, overlaying a flexible chain vest.  Contrasting against the rotting room, the armor seems to be in pristine condition, even shining.  Perhaps someone uses this heavy equipment - but surely not a harpy? You suppose you could take it.' );
	...
	DungeonHelSupplimental.prototype.gooArmorAttackPhysical = function() {
		if( Combat.combatMiss() ) {
			MainView.outputText( 'The goo-armor rushes forward and swings her sword in a mighty arc, but you dodge it!' );
		} else if( Combat.combatEvade() ) {
			MainView.outputText( 'The goo-armor rushes forward and swings her sword in a mighty arc, but you evade the attack!' );
		} else if( Combat.combatFlexibility() ) {
			MainView.outputText( 'The goo-armor swings a greatsword at you in a mighty arc, but your cat-like flexibility makes it easy to twist out of the way.' );
		} else if( Combat.combatMisdirect() ) {
			MainView.outputText( 'The goo-armor swings a sword at you in a mighty arc, but your training with Raphael allows you to misdirect her into a miss!' );
		}
		//HIT!;
		else {
			MainView.outputText( 'The goo-armor rushes forward and swings her sword in a mighty arc.  You aren\'t quite quick enough to dodge her blow, and the goopy sword slams into you, throwing you back and leaving a nasty welt.' );
	...
	DungeonHelSupplimental.prototype.struggleAtGooBind = function() {
		...
		if( Utils.rand( 10 ) > 0 && CoC.player.str / 5 + Utils.rand( 20 ) < 23 ) {
			MainView.outputText( 'You try and get out of the goo\'s grasp, but every bit of goop you pull off you seems to be replaced by twice as much!' );
	...
	DungeonHelSupplimental.prototype.gooArmorBeatsUpPC = function() {
		...
		if( CoC.player.hasVagina() ) {
			...
			if( CoC.player.biggestTitSize() > 1 ) {
				MainView.outputText( '  As she hammers your cunny, bits of her goo swirl around your [chest], squeezing and massaging your tits.  You squirm as she roughly teases your boobs, pinching at your nipples and squeezing your tender flesh roughly.' );
				//[if PC is lactating: "<i>;
				if( CoC.player.lactationQ() > 0 ) {
					MainView.outputText( '  To her delight, a spray of warm milk jets out of your sore nipples, milky white mixing into blue goo like oil in water. "<i>Mmm, tasty!</i>" she teases, massaging more and more from you.</i>"' );
				}
			}
			...
			MainView.outputText( ' girl ready to cum?  Well, go on then. Feed me!</i>"' );
			...
			if( CoC.player.biggestTitSize() > 1 && CoC.player.lactationQ() > 0 ) {
				MainView.outputText( 'and milk ' );
			}
			MainView.outputText( 'swirling around inside her.  "<i>Well, I suppose since you fed me so well, I\'ll let you go.  This time! See you around, tasty!</i>"' );
		...
		//(PC has Dick);
			...
			MainView.outputText( 'She begins to use her goo to peel back your armor, soon revealing your defenseless, half-erect cock' );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( '.  She makes a show of licking her lips as tendrils of goo wrap tightly around [eachCock] like a warm, wet onahole. You writhe and struggle against your gooey bonds, but your efforts are futile.  The goo-girl inside the armor only shakes her head at you, and squeezes [eachCock] tighter.' );
			...
			MainView.outputText( ' ' + CoC.player.mf( 'boy', 'girl' ) + ' ready to cum? Well, go on then. Feed me!</i>"' );
	...
	DungeonHelSupplimental.prototype.beatUpGooArmor = function() {
		...
		if( CoC.monster.lust > 99 ) {
			MainView.outputText( 'erotic abilities' );
		} else {
			MainView.outputText( 'skill in battle' );
		}
		MainView.outputText( ', the armored goo slumps backwards against the wall, unable to stand.  You loom over her, grinning as you contemplate what to do with your helpless opponent.' );
		...
		EngineCore.choices( 'Take Her', this, this.takeGooArmorAndWearIt, 'Refuse Her', this, this.refuseGooArmorOffer, '', null, null, '', null, null, '', null, null );
	...
	DungeonHelSupplimental.prototype.harpyHordeClawFlurry = function() {
		MainView.outputText( 'The harpies lunge at you, a veritable storm of talons and claws raining down around you.  You stumble back, trying desperately to deflect some of the attacks, but there are simply too many to block them all!  Only a single harpy in the brood seems to be holding back...\n' );
	...
	DungeonHelSupplimental.prototype.harpyHordeGangBangAttack = function() {
		MainView.outputText( 'Suddenly, a pair of harpies grabs you from behind, holding your arms to keep you from fighting back! Taking advantage of your open state, the other harpies leap at you, hammering your chest with punches and kicks - only one hangs back from the gang assault.\n\n' );
	...
	DungeonHelSupplimental.prototype.pcLosesToHarpyHorde = function() {
		...
		if( CoC.player.HP < 1 ) {
			MainView.outputText( 'brutal assault' );
		} else {
			MainView.outputText( 'raw sexuality' );
		}
	...
	DungeonHelSupplimental.prototype.talkToKiri = function() {
		MainView.clearOutput();
		MainView.outputText( 'You ask Kiri if she wouldn\'t mind sharing a bit of information with you.' );
		...
		EngineCore.choices( 'Hel', this, this.askKirkAboutHel, 'Harpies', this, this.askKiriAboutHarpies, 'Salamander', this, this.askKiriAboutSalamander, 'Kiri', this, this.askKiriAboutKiri, 'Nevermind', null, MainView.playerMenu );
	};
	//Kiri -- [Talk] -- [Hel];
	DungeonHelSupplimental.prototype.askKirkAboutHel = function() {
		MainView.clearOutput();
		MainView.outputText( 'You ask the harpy girl how she knows Hel, exactly.' );
		...
		EngineCore.doNext( this, this.talkToKiri );
	};
	//Kiri -- [Talk] -- [Harpies];
	DungeonHelSupplimental.prototype.askKiriAboutHarpies = function() {
		MainView.clearOutput();
		MainView.outputText( 'You ask Kiri about the harpies remaining in the tower and their relative strength and position - anything to give you an advantage.' );
	...
	DungeonHelSupplimental.prototype.askKiriAboutSalamander = function() {
		MainView.clearOutput();
		MainView.outputText( 'You ask her about the salamander prisoner you\'re here to help free.' );
	...
	DungeonHelSupplimental.prototype.askKiriAboutKiri = function() {
		MainView.clearOutput();
		MainView.outputText( 'You ask Kiri to tell you a little about herself.' );
	...
	DungeonHelSupplimental.prototype.kiriSexIntro = function() {
		MainView.clearOutput();
		MainView.outputText( 'You ask Kiri if she could help you blow off some steam.' );
		...
		if( CoC.player.hasCock() ) {
			EngineCore.addButton( 0, 'Anal', this, this.kiriSexAnal );
		}
		if( CoC.player.hasVagina() ) {
			EngineCore.addButton( 1, 'Get Licked', this, this.kiriSexGetLicked );
		}
	...
	DungeonHelSupplimental.prototype.kiriSexAnal = function() {
		...
		var y = x + 1;
		MainView.outputText( 'You whip your [cock ' + (y) + '] out of your [armor] and tell Kiri to get on all fours.  She grimaces, but does as you ask.  You hike up her shift to reveal her large, egg-laying pussy and her tight little pucker.' );
		...
		if( CoC.player.cockArea( x ) > 60 ) {
			MainView.outputText( 'as far in as you can manage' );
		} else {
			MainView.outputText( 'filling her completely' );
		}
		MainView.outputText( '.  Your cock squirts a thick load inside her, shooting creamy ropes of jizz deep into her bowels ' );
		//[if High Cum Production: ;
		if( CoC.player.cumQ() > 500 ) {
			MainView.outputText( 'until your cream squelches back out around your cock and onto the floor' );
		}
		MainView.outputText( '.  With a scream of delight, Kiri clamps down on your [cock ' + y + '] and climaxes too, leaking a pool of fem-spunk onto the ground.  She starts to bounce on your cock, riding out her anal orgasm until she\'s exhausted and you\'re deflated inside her.' );
	...
	DungeonHelSupplimental.prototype.kiriSexGetLicked = function() {
		MainView.clearOutput();
		MainView.outputText( 'You ask Kiri to eat you out. She grimaces but drops to her knees and undoes the bottom of your [armor], revealing your lusty [vagina]' );
		if( CoC.player.hasCock() ) {
			MainView.outputText( ' and ' + CoC.player.multiCockDescriptLight() );
		}
		MainView.outputText( '.  With a word of encouragement from you, she leans forward and presses her face into your groin, letting her tongue loose to explore your lower lips.' );
	...
	DungeonHelSupplimental.prototype.talkToValeria = function() {
		MainView.clearOutput();
		MainView.outputText( 'Now that you have a few moments to catch your breath, you ask your goo-armor what she thinks about the situation.' );
	...
	DungeonHelSupplimental.prototype.tortureGear = function() {
		...
		if( CoC.flags[ kFLAGS.HEL_DUNGEON_TAKEN_WHIP ] === 0 || CoC.flags[ kFLAGS.HEL_DUNGEON_TAKEN_STRAPS ] === 0 || CoC.flags[ kFLAGS.HEL_DUNGEON_TAKEN_DAGGER ] === 0 ) {
			...
			if( CoC.flags[ kFLAGS.HEL_DUNGEON_TAKEN_WHIP ] === 0 ) {
				MainView.outputText( 'A whip' );
				EngineCore.addButton( 0, 'Succ. Whip', this, this.takeWhip );
			}
			if( CoC.flags[ kFLAGS.HEL_DUNGEON_TAKEN_STRAPS ] === 0 ) {
				if( CoC.flags[ kFLAGS.HEL_DUNGEON_TAKEN_WHIP ] === 0 ) {
					MainView.outputText( ', ' );
				}
				MainView.outputText( 'some leather straps' );
				EngineCore.addButton( 1, 'BondageStraps', this, this.takeStraps );
			}
			if( CoC.flags[ kFLAGS.HEL_DUNGEON_TAKEN_DAGGER ] === 0 ) {
				...
				MainView.outputText( 'a lust-draft coated dagger' );
				EngineCore.addButton( 2, 'Lust Dagger', this, this.takeDagger );
			}
			MainView.outputText( '.  ' );
		}
		EngineCore.addButton( 4, 'Back', null, MainView.playerMenu );
	...
	DungeonHelSupplimental.prototype.helDungeonPrisonerTalk = function() {
		...
		if( CoC.flags[ kFLAGS.HEL_PC_TALKED_WITH_HAKON ] === 0 ) {
			MainView.outputText( 'You approach the Salamander strapped to the table.  He looks at you with his one good eye, warily gauging you as you approach.' );
	...
	DungeonHelSupplimental.prototype.phoenixPlatoonRush = function() {
		MainView.outputText( 'You fall back under a hail of scimitar attacks.  The sheer number of phoenixes attacking is bad enough, but their attacks are perfectly coordinated, leaving virtually no room for escape or maneuver without getting hit!\n' );
	...
	DungeonHelSupplimental.prototype.phoenixPlatoonMurdersPC = function() {
		MainView.clearOutput();
		MainView.outputText( 'You collapse, too ' );
		if( CoC.player.lust > 99 ) {
			MainView.outputText( 'turned on' );
		} else {
			MainView.outputText( 'badly injured' );
		}
	...
	DungeonHelSupplimental.prototype.checkOutDemBirdBitches = function() {
		MainView.clearOutput();
		MainView.outputText( 'You loom over the defeated heavy infantry, marveling at them.  The half-breeds were probably the most organized and efficient fighting unit you\'ve ever come across here in Mareth, and though you defeated them, you know most denizens of the region wouldn\'t have stood a chance.' );
		var missionary = null;
		var wanked = null;
		var rideAnal = null;
		...
		EngineCore.choices( 'Missionary', this, missionary, 'Get Wanked', this, wanked, 'Ride Anal', this, rideAnal, 'Ride Vaginal', this, rideVaginal, '', null, null, '', null, null, '', null, null, '', null, null, '', null, null, 'Back', null, MainView.playerMenu );
	};
	...
	DungeonHelSupplimental.prototype.phoenixMissionary = function() {
		...
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( ' and the tight ring of her pucker' );
		}
		MainView.outputText( '.  You grasp her wide flanks and, lining your [cock ' + y + ']' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( ' and extra boner up with her holes' );
		} else {
			MainView.outputText( ' up with her hole' );
		}
		MainView.outputText( ', push in, penetrating her cunt' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( ' and ass' );
		}
		MainView.outputText( ' and sliding into her warm, wet channel' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( '.' );
		...
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 's drive' );
		} else {
			MainView.outputText( ' drives' );
		}
		MainView.outputText( ' into her; you roll your hips back and forth for short but powerful strokes into her blazing hot innards.  As the pace picks up, you pull the phoenix-girl into a long, tender kiss.  The kiss soon turns into her moaning into your mouth as you fuck her cunt ' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 'and ass ' );
		}
		MainView.outputText( 'hard, slamming your hips into hers.' );
		MainView.outputText( '\n\nShe cums first' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( ', your double penetration too much for her to handle' );
		}
		MainView.outputText( '.  The phoenix grips your shoulders tight enough for her claws to cut into you as her tight pussy ' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 'and tighter sphincter spasm' );
		} else {
			MainView.outputText( 'spasms' );
		}
		MainView.outputText( ' around your dick' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( '.  With her squirming in your embrace and squeezing down so hard, you can\'t help but blow your load.  You press your lips hard into hers and cum, pumping thick ropes of steaming jizz into her wet box' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( ' and hot asshole' );
		}
		MainView.outputText( '.  As you cum into her, you feel her reptile prick shoot off, squirting a long white rope onto her chest and yours until her tits are soaked with her spunk.' );
		...
		MainView.outputText( ' out of her ravaged hole' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ' and gather your gear.' );
	...
	DungeonHelSupplimental.prototype.phoenixAginal = function() {
		...
		MainView.outputText( ' and strip out of your [armor]. The phoenix, a bit more dominant than you might have liked, roughly grabs your [chest], pinching your nipples as she takes over wringing her cock from you. Oh well. You decide to roll with it and slide a hand down to your [vagina], stroking your pussy as your lover warms up.' );
	...
	DungeonHelSupplimental.prototype.harpyQueenBeatsUpPCBadEnd = function( clearS ) {
		...
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'sperm bank' );
		} else {
			MainView.outputText( 'incubator' );
		}
		MainView.outputText( ' in my harem.  Yes, you\'ll do nicely...</i>"' );
		MainView.outputText( '\n\n<b>SIX MONTHS LATER...</b>\n' );
		MainView.outputText( 'You groan, your wrists chafing in the manacles hanging overhead.  The harpy slut riding your huge, engorged dick crosses her eyes, screams, and cums.  Another wave of seed lurches out of your cripplingly-large balls, so massive that they drag on the floor between your [legs].  You shudder slightly, but the act of ejaculation has lost all meaning' );
		if( !CoC.player.hasCock() ) {
			MainView.outputText( ', even if the sudden new sensations nearly broke your mind when the harpies used their magics to grow these huge male implements on your body' );
		}
		MainView.outputText( '.  Indeed, as soon as that slut\'s gotten her fill, one of her sisters pushes her roughly off your shaft and mounts you, bending over and backing into your massive schlong.  A little shudder of pleasure courses up your body as the twenty-ninth slut you\'ve serviced today starts to milk you for your magically-enhanced seed.  The tubes pumping enchanted drugs directly into the flesh of your testes goes into overtime, flooding your system with lust and the strange concoction that creates the Phoenixes.' );
	...
	DungeonHelSupplimental.prototype.harpyQueenDefeatedByPC = function() {
		MainView.clearOutput();
		MainView.outputText( 'With a final, loud gasp, the Harpy Queen collapses into her throne, unable to oppose you further.  Seeing their broodmother defeated, the other harpies that had been swarming around the room suddenly break off their attack and retreat, edging back to the fringes of the throne room.  Behind you, Hel stumbles out of the melee, covered in little cuts and bruises, but seeming otherwise unhurt.' );
	...
	DungeonHelSupplimental.prototype.HeliaThroneRoom = function() {
		MainView.clearOutput();
		MainView.outputText( 'You turn your attentions to the salamander, who is currently pinning down the harpy bitch to prevent her from flying off or doing something drastic.' );
		...
		EngineCore.choices( 'Hakon', this, hakon, 'Kiri', this, this.heliaKiriTalk, 'Queen', this, queen, '', null, null, 'Back', null, MainView.playerMenu );
	};
	//Throne Room -- [Helia] -- [Hakon];
	DungeonHelSupplimental.prototype.heliaHakonTalk = function() {
		MainView.clearOutput();
		MainView.outputText( 'You decide to tell Hel who, exactly, is chained up downstairs.  When you do, she simply stares at you, wide-eyed.' );
		...
		if( CoC.flags[ kFLAGS.HARPY_QUEEN_EXECUTED ] === 0 ) {
			MainView.outputText( '  "<i>You\'re going to pay for what you did to my father, you bitch.  I promise you that.</i>"' );
		}
	...
	DungeonHelSupplimental.prototype.heliaKiriTalk = function() {
		MainView.clearOutput();
		MainView.outputText( 'You ask Hel about the half-breed girl that helped the two of you find this place, and offered you information below.' );
	...
	DungeonHelSupplimental.prototype.heliaQueenTalk = function() {
		MainView.clearOutput();
		MainView.outputText( 'You nod your head toward the great big-booty broodmother.  "<i>Ah, the queen cunt herself,</i>" Hel says ruefully, cracking her knuckles.  "<i>We\'re gonna have some fun with you, little birdie...  Aren\'t we, [name]?</i>"' );
		...
		if( CoC.flags[ kFLAGS.HEL_KNOWS_ABOUT_HAKON ] === 1 ) {
			MainView.outputText( '  "<i>Maybe snap her neck afterwards.</i>"' );
		}
		EngineCore.doNext( MainView, MainView.playerMenu );
	};
	...
	DungeonHelSupplimental.prototype.harpyQueenAdvantage = function() {
		MainView.clearOutput();
		MainView.outputText( 'You loom over the defeated Harpy Queen, who squirms underneath Hel\'s foot on her chest.' );
		...
		EngineCore.choices( 'Fuck Her', this, fuck, 'Interrogate', this, this.harpyQueenInterrogate, 'Kill Her', this, this.killHarpyQueen, 'Let Her Go', this, this.letHarpyQueenGo, 'Back', null, MainView.playerMenu );
	};
	//Throne Room -- [Harpy Queen] -- [Let Her Go];
	DungeonHelSupplimental.prototype.letHarpyQueenGo = function() {
		MainView.clearOutput();
		MainView.outputText( 'You tell Hel to let up. You\'re letting the bitch go.' );
		...
		if( CoC.flags[ kFLAGS.HEL_KNOWS_ABOUT_HAKON ] === 1 ) {
			...
			EngineCore.choices( 'Forgive', this, this.harpyQueenLetHerGoForgive, 'Berate', this, this.harpyQueenLetHerGoBerate, '', null, null, '', null, null, '', null, null );
	...
	DungeonHelSupplimental.prototype.harpyQueenLetHerGoForgive = function() {
		MainView.clearOutput();
		MainView.outputText( 'You and Hel stare each other down, the dead harpy\'s body the only thing separating you.  Two dozen enraged harpies screech and caw around you, demanding justice for their fallen queen.' );
		...
		if( CoC.flags[ kFLAGS.HEL_KNOWS_ABOUT_HAKON ] === 1 ) {
			MainView.outputText( 'Dad ' );
		} else {
			MainView.outputText( 'that poor salamander ' );
		}
		MainView.outputText( 'out of the dungeon.  I\'ll make sure the phoenixes and harpies don\'t give you two trouble on the way out.</i>"' );
	...
	DungeonHelSupplimental.prototype.killHarpyQueen = function() {
		MainView.clearOutput();
		MainView.outputText( 'You look from Hel to the Harpy Queen.  This bitch could have bred an entire army - and might try it again. You can\'t allow that.' );
	...
	DungeonHelSupplimental.prototype.takeQueensStaff = function() {
		MainView.clearOutput();
		MainView.outputText( 'You pick up the Harpy Queen\'s staff.  It is a tall whitewood staff, nearly six feet in length, and covered in glowing eldritch runes, with a singular shimmering sphere of crystal at its head, which seems to have a swirling mist within.' );
	...
	DungeonHelSupplimental.prototype.fuckHarpyQueen = function() {
		MainView.clearOutput();
		MainView.outputText( 'You decide that the queen bitch deserves to be taught a lesson and you\'ll use her to get you off in the process.  You whisper this to Hel, who seems quite amused by the idea.' );
	...
	DungeonHelSupplimental.prototype.harpyQueenSexAnal = function() {
		...
		if( CoC.player.cockArea( 0 ) < 100 ) {
			MainView.outputText( 'until your hips sink into her pillowy ass' );
		} else {
			MainView.outputText( 'until your massive ' + CoC.player.cockDescript( 0 ) + ' can go no further inside her' );
		}
		MainView.outputText( '.  Sunk in as far as you\'ll go, you leave your cock where it is and instead give the broodmother a hard slap.  She screeches and clamps down, wringing your ' + CoC.player.cockDescript( 0 ) + ' hard.  But it\'s more pleasurable this time, now that she\'s nice and stretched by your anal intruder.' );
	...
	DungeonHelSupplimental.prototype.vaginalHarpyQueenSex = function() {
		MainView.clearOutput();
		MainView.outputText( 'You roughly toss the harpy queen to the floor and, grinning, tell her that since you\'re wrecking her plans to breed phoenixes, you\'ll just give her a brood of champion-spawn instead.  She gapes at you, wide-eyed in confusion and fear. Hel keeps her pinned down for you as you toss your [armor] aside, revealing your ' + CoC.player.cockDescript( 0 ) + '.' );
		...
		if( CoC.player.cockArea( 0 ) < 48 ) {
			MainView.outputText( '  Not to be outdone, you slam your hips forward, ramming your ' + CoC.player.cockDescript( 0 ) + ' into the queen\'s depths until you catch up with Hel, pounding the entrance to her womb.' );
		}
		...
		if( CoC.player.cumQ() < 300 ) {
			MainView.outputText( 'you slam your [hips] into the harpy\'s groin and ejaculate, launching thick, sperm-filled globs right into her waiting womb.\n\nYou cum and cum, filling the queen with all your seed until your ' );
			if( CoC.player.balls > 0 ) {
				MainView.outputText( '[balls] feel' );
			} else {
				MainView.outputText( 'crotch feels' );
			}
			MainView.outputText( ' hollow and empty.  Shuddering, you and Hel both withdraw, your cock and her tail a spunk and juice-covered mess.' );
		} else {
			MainView.outputText( 'you slam your [hips] into the harpy\'s groin and ejaculate, releasing a massive torrent of spunk deep inside the queen\'s womb, causing the harpy to shudder at the sheer amount of sperm you let out.  You continue to coat the harpy\'s walls for a minute, until your ' );
			if( CoC.player.balls > 0 ) {
				MainView.outputText( '[balls] feel' );
			} else {
				MainView.outputText( 'crotch feels' );
			}
			MainView.outputText( ' hollow and empty.  You and Hel slowly withdraw, causing some of your semen to leak out of the harpy\'s massive canal, leaving your cock and Hel\'s tail a spunk-and-juice-covered mess.' );
		}
	...
	DungeonHelSupplimental.prototype.harpyQueenInterrogate = function() {
		MainView.clearOutput();
		MainView.outputText( 'Leaning over the defeated Harpy Queen, you decide to get some answers.  First, you ask her why, exactly, she kidnapped a salamander in the first place.  That\'s what brought you here, after all.' );
