...
angular.module( 'cocjs' ).run( function( SceneLib, MainView, Zetaz, HarpyMob, Brigid, PhoenixPlatoon, HarpyQueen, SandWitchMob, CumWitch, SecretarialSuccubus, IncubusMechanic, OmnibusOverseer, Combat, Appearance, ConsumableLib, Vala, Utils, PerkLib, StatusAffects, Descriptors, CockTypesEnum, EventParser, OnLoadVariables, AppearanceDefs, kFLAGS, CoC, EngineCore ) {
	...
	DungeonCore.prototype.dungeonMenu = function() {
		...
		if( OnLoadVariables.dungeonLoc === DungeonCore.DUNGEON_FACTORY_PUMP_ROOM ) {
			...
			EngineCore.addButton( 0, 'North', this.dungeonEnterRoom, DungeonCore.DUNGEON_FACTORY_REPAIR_CLOSET );
			EngineCore.addButton( 1, 'East', this.dungeonEnterRoom, DungeonCore.DUNGEON_FACTORY_FURNACE_ROOM );
			EngineCore.addButton( 5, 'West', this.dungeonEnterRoom, DungeonCore.DUNGEON_FACTORY_MAIN_CHAMBER );
			EngineCore.addButton( 6, 'South', this.dungeonEnterRoom, DungeonCore.DUNGEON_FACTORY_FOYER );
		}
		//Break Room;
		if( OnLoadVariables.dungeonLoc === DungeonCore.DUNGEON_FACTORY_BREAK_ROOM ) {
			EngineCore.spriteSelect( 96 );
			EngineCore.outputText( 'Stepping through the dark red doorway, you wander into an expansive break room. Tables surrounded by crude wooden chairs fill most of the floor space. Along the far eastern wall sits a small counter, complete with a strange ebony sculpture of a busty woman with \'Mrs. Coffee\' printed on the side. Below the sculpture is a pot of steaming hot coffee, giving off an invigoratingly rich smell.', true );
			//Hooks for succubi encounter;
			//(if succubus gone/defeated);
			if( CoC.getInstance().player.findStatusAffect( StatusAffects.FactorySuccubusDefeated ) >= 0 ) {
				if( CoC.getInstance().player.hasKeyItem( 'Iron Key' ) < 0 ) {
					EngineCore.outputText( '  It seems your opponent dropped a small iron key as she fled.', false );
					EngineCore.addButton( 2, 'Iron Key', this.takeIronKey );
				}
				EngineCore.addButton( 4, 'Coffee', this.drinkCoffee );
				EngineCore.addButton( 5, 'West', this.dungeonEnterRoom, DungeonCore.DUNGEON_FACTORY_FOYER );
			} else {
				...
				if( CoC.getInstance().player.demonScore() >= 4 && CoC.getInstance().player.cor > 75 ) {
					EngineCore.outputText( 'The busty succubus turns, her barely contained breasts jiggling obscenely as she notices you, "<i>Oh, like hi there ', false );
					if( CoC.getInstance().player.gender === 1 ) {
						EngineCore.outputText( 'stud', false );
					} else {
						EngineCore.outputText( 'sexy', false );
					}
					EngineCore.outputText( '!</i>"  She stops, sniffing the air, a curious expression on her face as she slowly circles you, her heals clicking loudly on the floor.  A knowing grin blooms across her face as understanding hits her.\n\n', false );
					EngineCore.outputText( 'She exclaims, "<i>Omigawsh!  You\'re the champion!  Your, like, soul is still there and everything!  But, you\'re like, completely corrupt an\' stuff!  Ya know what\'d be fun?  I could fuck you \'til you cum so hard your soul melts out an\' you turn into a demon.  Wouldn\'t that be great?</i>"\n\n', false );
					EngineCore.outputText( 'The secretarial demoness pulls out a file and fiddles with her nails, murmuring, "<i>I guess if you don\'t wanna, we could just hook you up in the factory.  What\'s it gonna be?</i>"', false );
					EngineCore.addButton( 0, 'Fight', this.succubusCombatStart );
					EngineCore.addButton( 1, 'Go Demon', this.demonBadEnd );
					EngineCore.addButton( 2, 'Hook Up', this.succubusBadEnd );
					return; //This prevents the masturbate and item menus showing
				}
				//Not recognized;
				else if( CoC.getInstance().player.humanScore() <= 3 ) {
					EngineCore.outputText( 'The busty succubus turns, her barely contained breasts jiggling obscenely as she notices you, "<i>Oh, like hi there ', false );
					if( CoC.getInstance().player.gender === 1 ) {
						EngineCore.outputText( 'stud', false );
					} else {
					...
					EngineCore.addButton( 0, 'Fight', this.succubusCombatStart );
					EngineCore.addButton( 1, 'It\'s Me!', this.secretarialSuccubusInsult );
					EngineCore.addButton( 2, 'Leave', this.dungeonEnterRoom, DungeonCore.DUNGEON_FACTORY_FOYER );
					return; //This prevents the masturbate and item menus showing
				} else {
					EngineCore.outputText( 'The busty succubus turns, her barely contained breasts jiggling obscenely as she notices you, "<i>Oh, like hi there ', false );
					if( CoC.getInstance().player.gender === 1 ) {
					...
					EngineCore.outputText( '!  What\'s a cute little morsel like you doing by yourself out here?</i>"', false );
					EngineCore.addButton( 0, 'Fight', this.succubusCombatStart );
					EngineCore.addButton( 1, 'Talk', this.succubusTalkOne );
					EngineCore.addButton( 2, 'Run', this.dungeonEnterRoom, DungeonCore.DUNGEON_FACTORY_FOYER );
					return; //This prevents the masturbate and item menus showing
				}
			}
		}
		...
		if( OnLoadVariables.dungeonLoc === DungeonCore.DUNGEON_FACTORY_FURNACE_ROOM ) {
			if( CoC.getInstance().player.findStatusAffect( StatusAffects.DungeonShutDown ) < 0 ) {
				EngineCore.outputText( '<b><u>Furnace Room</u></b>\nThe air inside this room is hot enough to coat your ' + CoC.getInstance().player.skinTone + ' ' + CoC.getInstance().player.skinDesc + ' in a fine sheen of sweat.  The eastern side of the chamber is more machine than wall, a solid mass of iron piping covered in small metal blast-doors through which fuel is to be fed.  A small transparent plate is riveted into the wall, allowing you to see some kind of pink crystalline fuel being burned by purple-white fire.  The few visible controls and gauges don\'t seem to be linked into anything important, and the machinery looks far too durable to damage with what you have.  The only exit is a heavy iron door on the west wall.  ', true );
			} else {
				EngineCore.outputText( '<b><u>Furnace Room</u></b>\nDespite the machinery being shut down, the air in this room is still hot enough to coat your ' + CoC.getInstance().player.skinTone + ' ' + CoC.getInstance().player.skinDesc + ' in a fine sheen of sweat.  The eastern side of the chamber is more machine than wall, a solid mass of iron piping covered in small metal blast-doors through which fuel is to be fed.  A small transparent plate is riveted into the wall, allowing you to see some the ashes of a previous fuel source.  The few visible controls and gauges don\'t seem to be linked into anything important, and the machinery looks far too durable to damage with what you have.  The only exit is a heavy iron door on the west wall.  ', true );
			}
			// If the players found D3, hide him entirely to avoid two-places-at-once syndrome.;
			if( CoC.getInstance().player.findStatusAffect( StatusAffects.FactoryIncubusDefeated ) >= 0 || CoC.getInstance().flags[ kFLAGS.D3_DISCOVERED ] === 1 ) {
				EngineCore.addButton( 5, 'West', this.openFactoryDoor );
			}
			//Incubus is ALLLLIVE;
			else {
				...
				if( CoC.getInstance().player.findStatusAffect( StatusAffects.IncubusBribed ) >= 0 ) {
					...
					EngineCore.addButton( 1, 'Fight', this.startIncubusFight );
					EngineCore.addButton( 5, 'West', this.openFactoryDoor );
				} else {
					...
					EngineCore.addButton( 0, 'Fight', this.startIncubusFight );
					EngineCore.addButton( 1, 'Talk', this.talkToIncubus );
				}
			}
		}
		//Repair Closet;
		if( OnLoadVariables.dungeonLoc === DungeonCore.DUNGEON_FACTORY_REPAIR_CLOSET ) {
			EngineCore.outputText( '<b><u>Repair Closet</u></b>\nAs you carefully slip inside the room, you note with some relief that it seems to be an empty storage closet. The room is tiny, barely 6\' by 8\' and almost entirely empty.  The one piece of furniture inside the closet is a simple wooden cabinet, placed against the far wall.  ', true );
			if( CoC.getInstance().player.findStatusAffect( StatusAffects.BuiltMilker ) >= 0 ) {
				EngineCore.outputText( 'The shelves are empty.  ', false );
			} else {
				EngineCore.outputText( 'The shelves of the cabinet hold various pieces of pump machinery, probably used to repair complete machines further into the factory.  ', false );
				if( CoC.getInstance().player.inte >= 40 ) {
					EngineCore.outputText( 'You realize there are enough pieces here to put together a breast-milking pump or a cock-milker.  ', false );
					if( CoC.getInstance().player.hasKeyItem( 'Cock Milker' ) >= 0 ) {
						EngineCore.outputText( '\nYou already have a cock milker.\n', false );
					} else {
						EngineCore.addButton( 3, 'C. Milker', this.takeCockMilker );
					}
					if( CoC.getInstance().player.hasKeyItem( 'Breast Milker' ) >= 0 ) {
						EngineCore.outputText( '\nYou already have a breast milker.\n', false );
					} else {
						EngineCore.addButton( 2, 'B. Milker', this.takeBreastMilker );
					}
				}
			}
			EngineCore.outputText( 'The only exit is back to the south.', false );
			EngineCore.addButton( 6, 'South', this.openFactoryDoor );
		}
		//Main Chamber;
		if( OnLoadVariables.dungeonLoc === DungeonCore.DUNGEON_FACTORY_MAIN_CHAMBER ) {
			//Dungeon still operational;
			if( CoC.getInstance().player.findStatusAffect( StatusAffects.DungeonShutDown ) < 0 ) {
				EngineCore.outputText( '<b><u>Main Chamber</u></b>\nThis cavernous chamber is filled with a cacophony of sexual moans.  Rows of harnesses are spaced evenly throughout this room, nearly all of them filled with delirious-looking humans.  Each is over-endowed with huge breasts and a penis of elephantine proportions.  The source of their delirium hangs down from the ceiling - groups of hoses that end with needles buried deep into the poor \'girls\' flesh, pumping them full of demonic chemicals.  Constant sucking and slurping noises emanate from nipple and cock pumps as they keep the victims in a state of near-constant orgasm.  ', true );
				if( CoC.getInstance().player.cor < 50 ) {
					EngineCore.outputText( 'You wish you could free them, but it would take the better part of a day to get them all free.  It\'d be better to find the control room and shut down the infernal machinery.  ', false );
				} else {
					EngineCore.outputText( 'You wish you had some machinery like this for yourself.  It looks so fun!  Still, you suppose you should find the control panel to shut this down and free these people.  ', false );
				}
				EngineCore.outputText( 'There is a doorway to the east marked with an \'exit\' sign above it.  Along the southern wall is a stairwell that leads up to some kind of foreman\'s office.  Perhaps the controls are in there?', false );
			}
			//Dungeon shut down.;
			else {
				EngineCore.outputText( 'The chamber is significantly emptier since you\'ve shut down this factory.  Roughly half the girls appear to have left.  The rest seem to be pre-occupied by fucking each other in a massive orgy.  A few enterprising ladies have found leather outfits and appear to be helping to manually administer the chemical cocktails to those engaged in rampant sexual exploits.  It seems some of them preferred a life of near-constant orgasm to their freedom.  There is a door to the east marked as \'EXIT\', and a stairwell along the south wall that leads to an overseer\'s office.', true );
		...
		if( OnLoadVariables.dungeonLoc === DungeonCore.DUNGEON_FACTORY_PUMP_CONTROL ) {
			//PUMP CONTROL ROOM;
			EngineCore.outputText( '<b><u>Pump Control Room</u></b>\n', true );
			if( CoC.getInstance().player.findStatusAffect( StatusAffects.DungeonShutDown ) < 0 ) {
				EngineCore.outputText( 'This room is little more than a closet in reality.  There is a simple set of mechanical controls on a finely crafted terminal against the far wall.  You spend a moment looking over them, and realize you have three options to deal with this place.\n\n', true );
				EngineCore.outputText( '-You could close the storage vent valves and overload the fluid storage systems.  The storage tanks along the back portion of the building would rupture, releasing thousands of gallons of tainted fluids into the surrounding area, but the facility\'s systems would suffer catastrophic failures and shut down forever.\n', false );
				//(Consequences - lake goddess becomes tainted!);
				EngineCore.outputText( '-You could perform a system shutdown and then smash the controls.  It\'d let the girls go and keep the factory shut down in the short term.  However most of the equipment would be undamaged and the place could be re-opened without too much work on the demons\' part.\n', false );
				//(Consequences - If Marcus is a demon he takes over running the factory forever.  If not, nothing bad happens);
				EngineCore.outputText( '-You could leave the equipment to continue running.  After all, the girls downstairs did seem to be enjoying themselves...\n', false );
				//(Consequences - Marcus takes over if demonic choice taken, if not he shuts down the equipment & things continue as per #3).;
				EngineCore.addButton( 3, 'Valves', this.factoryOverload );
				EngineCore.addButton( 4, 'Shutdown', this.factoryShutdown );
			} else {
				EngineCore.outputText( 'This room is little more than a closet in reality.  There is a simple set of mechanical controls on the a finely crafted terminal against the far wall.  The controls are now inoperable, due to the damage your actions have caused.', false );
			}
			EngineCore.addButton( 5, 'West', this.dungeonEnterRoom, DungeonCore.DUNGEON_FACTORY_FOREMANS_OFFICE );
		}
		//Premium Products;
		if( OnLoadVariables.dungeonLoc === DungeonCore.DUNGEON_FACTORY_STORE_ROOM ) {
			EngineCore.outputText( '<b><u>Premium Products</u></b>\nThis store room is filled with a few opened crates, meant to store the various substances in the factory.  It looks as if the current overseer has allowed supplies to run low, as there is not much to be gleaned from this meager stash.\n\n', true );
			EngineCore.addButton( 0, 'North', this.dungeonEnterRoom, DungeonCore.DUNGEON_FACTORY_FOREMANS_OFFICE );
			if( CoC.getInstance().player.findStatusAffect( StatusAffects.TakenLactaid ) >= 0 ) {
				if( CoC.getInstance().player.statusAffectv1( StatusAffects.TakenLactaid ) > 0 ) {
					EngineCore.outputText( 'There is a crate with ' + Utils.num2Text( CoC.getInstance().player.statusAffectv1( StatusAffects.TakenLactaid ) ) + ' bottles of something called \'Lactaid\' inside.\n\n', false );
					EngineCore.addButton( 2, 'Lactaid', this.storageTakeLactaid );
				}
			} else {
				EngineCore.outputText( 'There is a crate with five bottles of something called \'Lactaid\' inside.\n\n', false );
				EngineCore.addButton( 2, 'Lactaid', this.storageTakeLactaid );
			}
			if( CoC.getInstance().player.findStatusAffect( StatusAffects.TakenGroPlus ) >= 0 ) {
				if( CoC.getInstance().player.statusAffectv1( StatusAffects.TakenGroPlus ) > 0 ) {
					EngineCore.outputText( 'There is a crate with ' + Utils.num2Text( CoC.getInstance().player.statusAffectv1( StatusAffects.TakenGroPlus ) ) + ' bottles of something called \'Gro+\' inside.\n\n', false );
					EngineCore.addButton( 3, 'GroPlus', this.storageTakeGroPlus );
				}
			} else {
				EngineCore.outputText( 'There is a crate with five bottles of something called \'Gro+\' inside.\n\n', false );
				EngineCore.addButton( 3, 'GroPlus', this.storageTakeGroPlus );
			}
		}
		...
		if( OnLoadVariables.dungeonLoc === SceneLib.dungeon2Supplimental.DUNGEON_CAVE_SECRET_TUNNEL ) {
			...
			if( CoC.getInstance().flags[ kFLAGS.DEFEATED_ZETAZ ] === 0 ) {
				EngineCore.outputText( 'You\'re going to find Zetaz and pay him back for drugging you on your first day here.  ', false );
			}
		...
		if( OnLoadVariables.dungeonLoc === SceneLib.dungeon2Supplimental.DUNGEON_CAVE_ZETAZ_CHAMBER ) {
			...
			if( CoC.getInstance().flags[ kFLAGS.ZETAZ_DOOR_UNLOCKED ] === 0 ) {
				EngineCore.outputText( '  <b>There\'s a bolt holding a door to the south closed, but you give it a gentle tug and it comes unlocked.</b>', false );
				CoC.getInstance().flags[ kFLAGS.ZETAZ_DOOR_UNLOCKED ] = 1;
			}
			EngineCore.outputText( '\n\n', false );
			if( CoC.getInstance().flags[ kFLAGS.DEFEATED_ZETAZ ] === 0 ) {
				EngineCore.outputText( 'A familiar imp is looking at you with a bewildered expression painted across his face.  You recognize his face immediately – this is Zetaz!  Oddly, he seems to have grown much larger in the time since your previous meeting.  He\'s over four feet tall and much more solidly built!\n\n', false );
				EngineCore.outputText( 'Zetaz whines, "<i>Seriously?  You show up here!?  First you make me lose my job, and now you beat up my friends and track dirt in my bedroom!?  I\'ve had enough!</i>"', false );
		...
		if( OnLoadVariables.dungeonLoc === SceneLib.dungeonHelSupplimental.DUNGEON_HEL_GUARD_HALL ) {
			...
			//Room Description:;
			EngineCore.outputText( 'You stand in what might have been a guard room once upon a time.  Now it is a ruined, ransacked mess.  It seems not to have been used in years, and the table, chairs, and spears lined up against the wall have all rotted away to almost nothing.' );
		...
		if( OnLoadVariables.dungeonLoc === SceneLib.dungeonHelSupplimental.DUNGEON_HEL_WINE_CELLAR ) {
			...
			if( CoC.getInstance().player.tallness < 60 ) {
				EngineCore.outputText( 'just enough room to stand up in' );
			} else {
				EngineCore.outputText( 'a ceiling so low you have to crouch' );
			}
		...
		if( OnLoadVariables.dungeonLoc === SceneLib.dungeonHelSupplimental.DUNGEON_HEL_STAIR_WELL ) {
			...
			//(Upon clicking in:);
			if( CoC.getInstance().flags[ kFLAGS.HEL_HARPIES_DEFEATED ] === 0 ) {
				EngineCore.outputText( 'You open the heavy double doors and cringe as a loud "<i>SCREECH!</i>" echoes out and up the next room - a wide open stairwell, it seems, with minimal cover.  The perfect place for a harpy to fight... Oh, shit!' );
		...
		if( OnLoadVariables.dungeonLoc === SceneLib.dungeonHelSupplimental.DUNGEON_HEL_DUNGEON ) {
			...
			if( CoC.getInstance().flags[ kFLAGS.HEL_BRIGID_DEFEATED ] === 0 ) {
				...
				EngineCore.outputText( '\n\n"<i>\'Bout time you made it down here, you ' + CoC.getInstance().player.mf( 'bastard', 'bitch' ) + '.  Mama Brigid\'s been waiting a loooong time for someone to try and break out one of her toys.</i>"  She pats the hefty keyring on the underside of her shield and leers at you.' );
		...
		if( OnLoadVariables.dungeonLoc === SceneLib.dungeonHelSupplimental.DUNGEON_HEL_MEZZANINE ) {
			...
			if( CoC.getInstance().flags[ kFLAGS.HEL_PHOENIXES_DEFEATED ] === 0 ) {
				EngineCore.outputText( 'You ascend the heavy stone steps, circling the tower\'s walls as you ascend.  You are stopped perhaps half-way to the second main floor on a small terrace level with a wide open view overlooking the vale beneath the high mountains.  As you step onto the mezzanine, you watch with a scowl as a number of tall, muscular hermaphrodites step out from the shadows.  Each is clad in heavy chainmail and wields a scimitar and a blood-red shield, but is otherwise nude, revealing their reptilian pricks and slick pussies.  The soldiers standing before you look like harpies, but they have scaled, humanoid legs, long, fiery tails and their wings are the darkest crimson.  These are phoenixes - the dread half-breed warriors you and Hel are here to stop!' );
		...
		if( OnLoadVariables.dungeonLoc === SceneLib.dungeonHelSupplimental.DUNGEON_HEL_THRONE_ROOM ) {
			...
			if( CoC.getInstance().flags[ kFLAGS.HEL_HARPY_QUEEN_DEFEATED ] === 0 ) {
				...
				//Room Description:;
				EngineCore.outputText( 'You stand in the harpy throne room - a long, circular room dominated by a high throne surrounded by cushions and drapes.  A single long carpet flows from the heavy double doors to the throne, reminding you of a castle\'s great hall in days gone by.  A number of harpies cower in the shadows, afraid to oppose you further now that their mighty leader is defeated.' );
			...
			if( CoC.getInstance().flags[ kFLAGS.SANURA_DISABLED ] > 0 ) {
				...
				if( CoC.getInstance().flags[ kFLAGS.TIMES_SUBMITTED_TO_SANURA ] + CoC.getInstance().flags[ kFLAGS.TIMES_WINFUCKED_SANURA ] > 0 ) {
					...
					if( CoC.getInstance().flags[ kFLAGS.BEATEN_SANURA_COUNT ] > 0 ) {
						EngineCore.outputText( '  Or would you rather skip the formalities?  We both know who\'s got the sharper wit, I should think.' );
					}
					EngineCore.outputText( '</i>"' );
				} else {
		...
		if( OnLoadVariables.dungeonLoc === SceneLib.dungeonSandWitch.DUNGEON_WITCH_CAVERNOUS_COMMONS ) {
			...
			if( CoC.getInstance().flags[ kFLAGS.SANDWITCH_MOB_DEFEATED ] === 0 ) {
				EngineCore.outputText( 'Worst of all, a huge assortment of spellcasters is assembling into a mob, obviously hostile.' );
				Combat.startCombat( new SandWitchMob(), true );
				return;
			} else {
				EngineCore.outputText( 'The women you defeated before have returned to their tasks, casting wary glances your way from time to time but no longer threatening.' );
			}
			EngineCore.outputText( '  Cave tunnels lead in to the east and west into more underground chambers.  A path leads south towards the exit.' );
		...
		if( OnLoadVariables.dungeonLoc === SceneLib.dungeonSandWitch.DUNGEON_WITCH_WEST_WARRENS_WEST ) {
			...
			if( CoC.getInstance().flags[ kFLAGS.ESSRAYLE_ESCAPED_DUNGEON ] === 0 && CoC.getInstance().flags[ kFLAGS.MET_ESSY ] > 0 ) {
			...
			if( CoC.getInstance().flags[ kFLAGS.SAND_WITCHES_FRIENDLY ] > 0 ) {
				...
				if( CoC.getInstance().flags[ kFLAGS.CUM_WITCH_DEFEATED ] === 0 ) {
					...
					if( CoC.getInstance().player.hasVagina() ) {
						EngineCore.outputText( 'but then, maybe you can come to serve us as a mother.  Our tribe is not wasteful.' );
					} else if( CoC.getInstance().player.hasCock() ) {
						EngineCore.outputText( 'but perhaps, once you have been disabused of your notions of freedom, you could serve as my loyal cum-pump.  It does get so tiring inseminating all these girls alone.' );
					} else {
						EngineCore.outputText( 'but then, perhaps you could be made to serve in other ways.' );
					}
					EngineCore.outputText( '</i>"' );
	...
	DungeonCore.prototype.factoryShutdown = function() {
		...
		CoC.getInstance().player.createStatusAffect( StatusAffects.DungeonShutDown, 0, 0, 0, 0 );
	};
	DungeonCore.prototype.factoryOverload = function() {
		...
		EngineCore.doNext( EventParser.playerMenu );
		CoC.getInstance().player.createStatusAffect( StatusAffects.DungeonShutDown, 0, 0, 0, 0 );
		CoC.getInstance().player.createStatusAffect( StatusAffects.FactoryOverload, 0, 0, 0, 0 );
	};
	DungeonCore.prototype.relieveTension = function() {
		EngineCore.clearOutput();
		//First time...;
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.TensionReleased ) < 0 ) {
			...
			if( CoC.getInstance().player.biggestTitSize() < 10 ) {
				CoC.getInstance().player.growTits( 1, (2 + Utils.rand( 3 )), true, 1 );
				EngineCore.outputText( '  ' );
			}
			EngineCore.outputText( 'You glance over to the pile of glistening entwined bodies as they writhe in pleasure, and find yourself drawn in to the mass.  You spend the next four hours suckling tainted breast milk, fucking gaping pussies, and doing your damnedest to milk as much cum from the dick-girls around you.  Eventually the drugs work their way out of your system, leaving you to recover on the floor.  Cum, milk, and sweat drip from your nude form as you try to clean up and get dressed.' );
			CoC.getInstance().player.orgasm();
			EngineCore.dynStats( 'int', -2, 'lib', 4, 'cor', 4 );
			CoC.getInstance().player.slimeFeed();
			CoC.getInstance().player.createStatusAffect( StatusAffects.TensionReleased, 0, 0, 0, 0 );
		}
		//Second/third times...;
		else {
			//[[2nd time]] ;
			if( CoC.getInstance().player.statusAffectv1( StatusAffects.TensionReleased ) === 0 ) {
				EngineCore.outputText( 'You eagerly put on the modified harness and let them inject you with more of those body-altering chemicals.  As they fill you with artificial lust and desire, you cry out and beg for more.  They oblige you and give you a larger dose than the first time.  ' );
				//Grow dick!;
				if( CoC.getInstance().player.cocks.length > 0 ) {
					CoC.getInstance().player.lengthChange( CoC.getInstance().player.increaseCock( 0, 5 ), CoC.getInstance().player.cocks.length );
					if( CoC.getInstance().player.averageCockLength() >= 9 && CoC.getInstance().player.averageCockThickness() < 2 ) {
						EngineCore.outputText( 'You feel yourself gain in thickness as well, to match your new length.  ' );
						_.forEach( _.filter( CoC.getInstance().player.cocks, function( cock ) {
							return cock.cockThickness < 2;
						} ), function( cock ) {
							cock.cockThickness++;
						} );
					} else if( CoC.getInstance().player.averageCockLength() >= 15 && CoC.getInstance().player.averageCockThickness() < 3 ) {
						EngineCore.outputText( 'You feel yourself gain in thickness as well, to match your new length.  ' );
						_.forEach( _.filter( CoC.getInstance().player.cocks, function( cock ) {
				...
				if( CoC.getInstance().player.breastRows.length === 0 ) {
					...
					EngineCore.outputText( 'Your chest tingles, revealing a pair of pink nipples on your new mammory glands.  ' );
				}
				CoC.getInstance().player.growTits( 1, (2 + Utils.rand( 3 )), true, 1 );
				EngineCore.outputText( '  ' );
				EngineCore.outputText( 'Your ' + Descriptors.nippleDescript( 0 ) + 's ' );
				if( CoC.getInstance().player.cocks.length > 0 ) {
					EngineCore.outputText( 'and ' + CoC.getInstance().player.multiCockDescript() );
				}
				EngineCore.outputText( ' become rock hard, leaking fluids constantly.  ' );
				//MALE;
				if( CoC.getInstance().player.cocks.length > 0 && CoC.getInstance().player.vaginas.length === 0 ) {
					EngineCore.outputText( 'Glancing over into the sea of sex, you find yourself drawn to the nearest pussy, as if it was the only thing in the world to matter.  You lose track of the time as you fuck hard dozens of gaping cunts, each of them overflowing with cum from all the participants in this infernal orgy.  ' );
				}
				//FEMALE;
				if( CoC.getInstance().player.vaginas.length > 0 && CoC.getInstance().player.cocks.length === 0 ) {
					...
					CoC.getInstance().player.cuntChange( 150, true );
				}
				//HERM;
				if( CoC.getInstance().player.vaginas.length > 0 && CoC.getInstance().player.cocks.length > 0 ) {
					EngineCore.outputText( 'You feel your ' + CoC.getInstance().player.multiCockDescript() + ' getting milked by many wet holes, though you are too busy sucking cocks and moaning in ecstasy to notice who they belong to.  ' );
				}
				EngineCore.outputText( 'The next eight hours are lost to your desires as you cum over and over, feeling mind-shattering pleasure.  You recover a while on the floor, soaked with a mixture of milk, cum, and pussy-juice.  Getting dressed is a bit troublesome with the recent changes, but you manage to squeeze back into your ' + CoC.getInstance().player.armorName + '.  You walk away while still feeling horny, and the moaning of the girls behind you doesn\'t help.  Maybe you could stay for another round...' );
				CoC.getInstance().player.orgasm();
				EngineCore.dynStats( 'int', -2, 'lib', 4, 'cor', 4 );
				CoC.getInstance().player.createStatusAffect( StatusAffects.TensionReleased, 0, 0, 0, 0 );
				CoC.getInstance().player.addStatusValue( StatusAffects.TensionReleased, 1, 1 );
				CoC.getInstance().player.slimeFeed();
			}
			//Third time;
			else {
				EngineCore.outputText( 'Desperate for more of the demon-drugs, you slide into the now-familiar harness and let the needles sink into your skin.   Panting in lust, you beg for them to increase the dosage again.   Desire burns through your veins as the cocktail surges through them' );
				if( CoC.getInstance().player.cocks.length > 0 ) {
					EngineCore.outputText( ', filling your ' + CoC.getInstance().player.multiCockDescriptLight() );
					EngineCore.outputText( ' with sensation' );
					if( CoC.getInstance().player.cockTotal() === 1 ) {
						EngineCore.outputText( 's' );
					}
					EngineCore.outputText( ' as ', false );
					if( CoC.getInstance().player.cocks.length > 1 ) {
						EngineCore.outputText( 'they' );
					} else {
						EngineCore.outputText( 'it', false );
					}
					EngineCore.outputText( ' grow', false );
					if( CoC.getInstance().player.cocks.length === 1 ) {
						EngineCore.outputText( 's' );
					}
					EngineCore.outputText( ' massive and engorged.  ' );
				} else {
					EngineCore.outputText( '.  ' );
				}
				EngineCore.outputText( 'Your ' + Descriptors.nippleDescript( 0 ) + 's throb, becoming hard, puffy, and starting to dribble milk.  ' );
				if( CoC.getInstance().player.vaginas.length > 0 ) {
					EngineCore.outputText( 'Your pussy is instantaneously soaked, filling the air with the scent of sex.  ' );
				}
	...
	DungeonCore.prototype.factoryFinisher = function() {
		...
		if( CoC.getInstance().player.cocks.length === 0 || CoC.getInstance().player.biggestTitSize() <= 1 ) {
			EngineCore.outputText( 'A small inner voice pipes up to remind you that you don\'t have ', false );
			if( CoC.getInstance().player.cocks.length === 0 ) {
				EngineCore.outputText( 'anything in your groin to suck on', false );
				if( CoC.getInstance().player.biggestTitSize() <= 1 ) {
					EngineCore.outputText( ' or ', false );
				}
			}
			if( CoC.getInstance().player.biggestTitSize() <= 1 ) {
				EngineCore.outputText( 'any adornments on your chest', false );
			}
			EngineCore.outputText( '.  That voice trails off as that feeling of perfect pleasure and rightness sweeps it away with the last remnants of sleep.\n\n', false );
		} else {
			EngineCore.outputText( 'A small inner voice tries to warn you of something, only to be swept away in the feelings of perfect pleasure and rightness that wash away the last remnants of your sleep.\n\n', false );
		}
		EngineCore.outputText( 'You realize that the moaning voice is your own, and find that the thought just turns you on more.\n\n', false );
		EngineCore.outputText( '\'<i>You\'re such a horny slut!</i>\' echoes a voice in your head.  You want to nod and smile, but are prevented by something.  You realize you\'re strapped into some kind of chair and harness so securely that you can\'t even move.  Tiny soothing fingers massage your temples, rubbing away the fears that moments ago threatened to interrupt your pleasure.  You can see a ', false );
		if( CoC.getInstance().player.totalBreasts() === 2 ) {
			EngineCore.outputText( 'pair of ', false );
		} else {
			EngineCore.outputText( 'multitude of ', false );
		}
		EngineCore.outputText( ' clear hoses coming away from your cow-like chest udders.  ', false );
		if( CoC.getInstance().player.biggestLactation() <= 1.5 ) {
			EngineCore.outputText( 'Creamy white milk is flowing in a steady stream up the tubes and away from you.  ', false );
		} else {
			EngineCore.outputText( 'The hoses bulge obscenely as they struggle to keep up with the torrents of creamy-white milk you\'re producing.  ', false );
		}
		EngineCore.outputText( 'Even more wanton moans erupt from your disobedient lips now that you know what\'s going on.  You\'re not just a horny slut.  You\'re a horny cow-slut who\'s getting off on having her tits pumped.  The massage you\'re getting feels so good once you realize that.\n\n', false );
		EngineCore.outputText( 'A snap echoes through the pumping room, nearly drowned out by the moans of the other milk-sluts around you.  You look around as you realize the band to restrain your head has been unlatched.  You take advantage of your newfound freedom and look around.  Rows and rows of other girls are there, just like you.  Almost all of them have bigger tits and fuller milk-tubes.  In addition, they all have enormous members that would drag on the floor were it not for the gigantic tubes encapsulating each and every one.  ', false );
		EngineCore.outputText( 'The girl next to you squirms and cums, wriggling inside her harness as waves of sticky goop are pumped down her cock-tube into a floor-socket.  She just keeps going and going, making you wonder how she can make so much of the stuff.  As the sight excites you, the pleasure in your own crotch redoubles.  Looking down thanks to your newfound freedom, you see your own giant encapsulated member; though not as large as your neighbor\'s, it still looks and feels wonderful.\n\n', false );
		EngineCore.outputText( 'The lining of the tube squeezes and massages your trapped prick expertly, even as those hands continue to work on your mind.  Some part of you suspects that your thoughts are being manipulated, but the carnal pleasure you are experiencing is so amazing that you have no intention of resisting. If being a cumslut for your sexy demonic masters is what it takes, so be it. Cramming a massive demon-cock in your throat, getting a few others up your holes to keep you pregnant all the time, and being their busty hermaphrodite breeding tool would be your joy and privilege.  ', false );
	...
	DungeonCore.prototype.succubusTalkOne = function() {
		...
		EngineCore.choices( 'For what?', this.succubusTalkTwo, 'Yes', this.succubusBadEnd, 'No', this.succubusRefuseOffer, '', null, '', null );
	};
	DungeonCore.prototype.succubusTalkTwo = function() {
		...
		EngineCore.choices( 'Sick!', this.succubusRefuseOffer, 'Sounds Fun', this.succubusBadEnd, '', null, '', null, '', null );
	};
	DungeonCore.prototype.succubusCombatStart = function() {
		EngineCore.spriteSelect( 55 );
		CoC.getInstance().player.createStatusAffect( StatusAffects.FactorySuccubusDefeated, 0, 0, 0, 0 );
	...
	DungeonCore.prototype.succubusRefuseOffer = function() {
		...
		EngineCore.doNext( this.succubusCombatStart );
	};
	...
	DungeonCore.prototype.succubusBadEnd = function() {
		...
		EngineCore.doNext( this.succubusBadEndPartTwo );
	};
	DungeonCore.prototype.succubusBadEndPartTwo = function() {
		...
		if( CoC.getInstance().player.biggestLactation() < 1 ) { //Tits – regular
			EngineCore.outputText( 'Your ' + Descriptors.nippleDescript( 0 ) + 's begin prodding painfully against your ' + CoC.getInstance().player.armorName + ', every touch serving to make them harder and more erect.  ' );
		} else if( CoC.getInstance().player.biggestLactation() < 3 ) { //Tits – lactating
			EngineCore.outputText( 'Your ' + Descriptors.nippleDescript( 0 ) + 's get painfully hard as you feel milk begin backing up inside your ' + Descriptors.allBreastsDescript() + '.   The succubus glances down mischieviously as her hands begin to grope you through your ' + CoC.getInstance().player.armorName + ', squeezing out a few drops of milk.  ' );
		} else {
			{ //Tits – megalactating
			}
			EngineCore.outputText( 'Your ' + Descriptors.nippleDescript( 0 ) + 's get painfully hard as milk begins drooling down your over-productive chest, making your ' + CoC.getInstance().player.armorName + ' slide across your leaky milk-spouts in an agonizingly pleasurable way.  ' );
		}
		if( CoC.getInstance().player.cocks.length === 1 ) {
			{ //Cock – single
			}
			if( CoC.getInstance().player.cockArea( 0 ) < 30 ) {
				EngineCore.outputText( 'Swooning from sudden blood loss, you struggle to maintain the kiss as your body takes your ' + Descriptors.cockDescript( 0 ) + ' to full hardness in seconds.  ' );
			} else if( CoC.getInstance().player.cockArea( 0 ) < 100 ) {
				EngineCore.outputText( 'Nearly blacking out, you struggle to stay awake as your body shifts your blood to your disproportionate ' + Appearance.cockNoun( CockTypesEnum.HUMAN ) + '.  ' );
			} else {
				EngineCore.outputText( 'As you struggle not to lose consciousness, you realize your over-aroused body had pumped most of your blood to your over-sized ' + Appearance.cockNoun( CockTypesEnum.HUMAN ) + ', which now droops to the floor, pulsing hotly.  ' ); //Cock -megahuge
			}
		}
		//DO MULTIZ;
		if( CoC.getInstance().player.cocks.length > 1 ) {
			EngineCore.outputText( 'The feeling of light-headedness nearly robs you of consciousness as your ' + CoC.getInstance().player.multiCockDescript() + ' fill with blood, pulsating with arousal as they reach full size.  ' );
		}
		if( CoC.getInstance().player.vaginas.length > 0 ) {
			{ //Vagooooz
			}
			if( CoC.getInstance().player.clitLength >= 4.5 ) {
				...
				EngineCore.outputText( 'Popping from between your thighs, your ' + Descriptors.clitDescript() + ' responds to the sheer hotness of the situation by making itself known.   You squeeze your legs tightly together, hungry for additional sensation.  ' );
			} else if( CoC.getInstance().player.clitLength > 1 ) {
				{ //big clit
				}
				EngineCore.outputText( 'A wave of pleasure erupts from between your legs as your ' + Descriptors.clitDescript() + ' pops free.    You squeeze your legs tightly together, hungry for the additional sensations.  ' );
			} else if( CoC.getInstance().player.vaginas[ 0 ].vaginalWetness >= AppearanceDefs.VAGINA_WETNESS_SLICK ) {
				{ //slick
				}
				EngineCore.outputText( 'Squishing wetly, your bottoms become soggy with the flood of fluids leaking from your ' + Descriptors.vaginaDescript( 0 ) + '.   Your legs spread apart on their own, begging for any kind of intrusion.  ' );
			} else {
				EngineCore.outputText( 'Groaning softly, you feel yourself getting wetter and wetter with arousal.  You wish your sticky bottoms were off so you could let something into your ' + Descriptors.vaginaDescript( 0 ) + '.  ' ); //normal
			}
		}
		EngineCore.outputText( '\n\n' ); //New PG
		EngineCore.outputText( 'No longer caring about modesty, etiquette, or even your own safety, you squirm against your bindings, lewdly putting on a display for your captor as you suck her tongue as if your life depended on it.   She breaks the kiss with a smile, "<i>I told you I\'d make sure it was pleasurable.  Now you sit tight while I get you hooked up, and we\'ll have you cumming what\'s left of your brains out in no time,</i>" she promises.\n\n' );
		EngineCore.outputText( 'The succubus pushes a button on the wall, and a number of strange looking suction tubes and hoses drop down from above you.   Moving with practiced efficiency, she hooks a ' );
		if( CoC.getInstance().player.breastRows.length === 1 ) {
			EngineCore.outputText( 'pair of' );
		} else {
			EngineCore.outputText( 'number of' );
		}
		EngineCore.outputText( ' hoses to your breasts, ' );
		if( CoC.getInstance().player.cocks.length === 0 ) {
			if( CoC.getInstance().player.vaginas.length > 0 ) {
				EngineCore.outputText( 'and a vacuum pump to your clit.  ' );
			} else {
				EngineCore.outputText( 'and another vacuum pump to your bare groin.  In seconds a wet fleshy growth erupts.  You have grown a cock!  ' );
				CoC.getInstance().player.createCock();
			}
		} else if( CoC.getInstance().player.vaginas.length > 0 ) {
			EngineCore.outputText( 'a vacuum pump to your clit, and a pump many times bigger than your ' + Descriptors.cockDescript( 0 ) + ' to it.  ' );
		} else {
			EngineCore.outputText( 'and a pump many times bigger than your ' + Descriptors.cockDescript( 0 ) + ' to it.  ' );
		}
		EngineCore.outputText( 'At first there is only a gentle suction, you assume in order to keep them in place.  Unfinished, your captor places something large and hollow against your backdoor' );
		if( CoC.getInstance().player.vaginas.length === 0 ) {
			EngineCore.outputText( '.' );
		} else {
			EngineCore.outputText( ' and an ever larger dildo against your ' + Descriptors.vaginaDescript( 0 ) + '.  It seems to pulse and wiggle with a life of its own, rubbing the bumps of its lumpy head against your lips.' );
		}
		EngineCore.outputText( '  You swoon as you hear the solid click of a button being pushed, and all at once all devices attached to you leap to life.' );
		EngineCore.doNext( this.succubusBadEndPartThree );
	};
	DungeonCore.prototype.succubusBadEndPartThree = function() {
		...
		//Dick  ;
		if( CoC.getInstance().player.cocks.length > 0 ) {
			EngineCore.outputText( 'The vacuum-pump on your cock noisily sucks down all your spoo, ' );
			//High cum variant;
			if( CoC.getInstance().player.cumQ() > 300 ) {
				EngineCore.outputText( 'struggling with the amount you put off.  Grinning, the succubus reaches over and flips a lever.  You feel the suction increase as the machine is turned up to accommodate your altered physique.  ' );
			}//else;
			else {
				EngineCore.outputText( 'the suction and drugs make it so easy to just keep cumming and cumming.  ' );
			}
			//either or:;
			EngineCore.outputText( 'Dimly, you feel a needle lodged in your taint, pumping your prostate full of something.  Your ' + Descriptors.cockDescript( 0 ) + ' begins growing mid-orgasm, the skin turning a deep purple even as small nodule-like bumps form all over it, rapidly becoming a bloated parody of its demonic counterparts.  ' );
		}
		//Puss Orgasmz;
		if( CoC.getInstance().player.vaginas.length > 0 ) {
			EngineCore.outputText( 'Clenching tightly, your ' + Descriptors.vaginaDescript( 0 ) + ' squeezes tightly on its intruder as its repeatedly violated by the machines.  ' );
		}
		//End;
		...
		if( CoC.getInstance().player.statusAffectv3( StatusAffects.Marble ) === 1 ) {
			EngineCore.outputText( 'Later on, you are briefly pulled out of your reverie by a familiar warm fluid flowing down your throat.  You come to your senses and see Marble looking down at you with an odd expression on her face.  ' );
			EngineCore.outputText( 'She seems to be in a state of bliss. Looking down, you see that she is wearing some kind of pair of pink panties.  Marble gasps and the surface of the panties ripples; it\'s a living thing!\n\nYou look around and realize you aren\'t alone.  ' );
		} else if( CoC.getInstance().player.findStatusAffect( StatusAffects.CampMarble ) >= 0 ) {
			EngineCore.outputText( 'You are given a brief moment of clarity as you see Marble standing in front of you.  ' );
			EngineCore.outputText( 'She seems to be in a state of bliss. Looking down, you see that she is wearing some kind of pair of pink panties.  Marble gasps and the surface of the panties ripples; it\'s a living thing!\n\nYou look around and realize you aren\'t alone.  ' );
		} else {
			EngineCore.outputText( 'Later on, in a moment of clarity, you look around and realize you aren\'t alone.  ' );
		}
		EngineCore.outputText( 'In rows alongside you are a large number of other captives, every single one endowed with freakishly sized breasts, and nearly all gifted with throbbing demonic dicks.  Some small analytical part of you notes that the farther down the line they are, the older and larger they have become.   You look down and see your own massive tits, shiny tainted nipples still pumping out streams of milk.  The huge throbbing demon-cock between your legs begins to get hard as the machines crank back up, filling you full of happy horniness.' );
		if( CoC.getInstance().player.statusAffectv3( StatusAffects.Marble ) === 1 || CoC.getInstance().player.findStatusAffect( StatusAffects.CampMarble ) >= 0 ) {
	...
	DungeonCore.prototype.succubusLossRape = function() {
		...
		if( CoC.getInstance().player.cocks.length > 0 ) {
			if( CoC.getInstance().player.lust > 99 ) {
				EngineCore.outputText( 'Driven half mad with lust, you drop to your knees. Your fingers fly over your body as you pry off every last piece of your ' + CoC.getInstance().player.armorName + ', displaying just how hard your alluring opponent has gotten you.  The succubus saunters over, every sinuous step radiating the inhuman sexuality that pours off her skin like heat from a bonfire.\n\n', false );
			} else {
				EngineCore.outputText( 'Exhausted, you collapse before the succubus.  She effortlessly slices away your ' + CoC.getInstance().player.armorName + ', peeling your possessions away with practiced ease.  In moments you are stark naked and wholly exposed to your captor.  In spite of yourself, your body begins to respond to her sultry aura, displaying the hardness of your desire and shame immediately.\n\n', false );
			}
			EngineCore.outputText( '"<i>Awww, did I get you all <b>HOT</b> and bothered?</i>" She croons, poising a stocking clad foot above you as her high-heels seem to fade away.  Warm silk begins to press against your groin as slender toes curl around the head of your throbbing maleness, your foe having her way with your desire-saturated form.  You mewl pitifully at the sensation, your hips twitching involuntarily against her demonic sole. The slippery surface of her foot squeezes as she expertly strokes you with her foot, delighting in her complete dominance over your easily controlled member.\n\n', false );
			//balls or pussy play;
			if( CoC.getInstance().player.balls > 0 ) {
				//[[balls]];
				if( CoC.getInstance().player.ballSize < 6 ) {
					EngineCore.outputText( 'Your sultry captor leans low over you, her luscious tits wobbling enticingly as she reaches down and caresses your ' + Descriptors.ballsDescriptLight() + ' with soft touches.  Almost immediately you feel them clench with boiling heat, growing heavy and churning with a load big enough to satisfy a thirsty succubus.', false );
				}
				//[[huge balls]];
				else {
					EngineCore.outputText( 'Your sultry captor leans low, marveling at the size of your ' + Descriptors.ballsDescriptLight() + ' as she reaches down to caress them.  Her tits swing lewdly above you, bouncing in hypnotic motions. Her hands work gently, taking each one of your ' + Descriptors.ballsDescriptLight() + ' and hefting it gently.  Almost immediately you feel them fill with an unnatural heat that spreads everywhere her slender fingers touch.  They begin to feel full and heavy, practically sloshing as the pent up need inside you is channeled into liquid form.  "So ripe... and full," she whispers to herself as she caresses them, her silken foot still sliding all over your ' + Descriptors.cockDescript( 0 ) + ', pumping stroke after stroke of pleasure into your lust-weakened form.', false );
				}
			} else {
				//[[no balls no pussy]];
				if( CoC.getInstance().player.vaginas.length === 0 ) {
					EngineCore.outputText( 'Your sultry captor leans low over you, her luscious tits wobbling enticingly as she reaches down and caresses the skin between your ' + Descriptors.cockDescript( 0 ) + ' and ' + Descriptors.assholeDescript() + ' with a slender finger.  Almost immediately you feel your groin clench with the boiling heat of a growing orgasm, thick cum churning out from your prostate as your body readies a load large enough to satisfy a thirsty succubus.', false );
				}
				//[[no balls + pussy]];
				else {
					EngineCore.outputText( 'Your sultry captor leans low over you, her luscious tits wobbling enticingly as she reaches down and caresses the slick skin of your slit with a single digit.  Other fingers circle your ' + Descriptors.clitDescript() + ', teasing it from between the folds as it grows hard, peeking out from the hood and turning cherry-red.  Almost immediately you feel your groin clench with the boiling heat of a growing orgasm, thick cum churning in your prostate as your body readies a load large enough to satisfy a thirsty succubus.', false );
				}
			}
			EngineCore.outputText( '\n\n', false );
			//[[Cum]];
			EngineCore.outputText( 'The succubus licks her lips in anticipation as she curls her silk-clad toes tighter around you, making you bulge and twitch in obscene pleasure.  With a naughty smile, she caresses your ass with her bulbous demonic tail.  Before you can react, it plunges inside you, easily slipping through your ' + Descriptors.assholeDescript() + ' and pressing tightly against your prostate.  The suddenness pushes you over the edge, but she immediately wraps her fingers around you, pinching tightly, bottling your cum inside you.  You cry out in pain and surprise as your entire thick load is trapped inside you.  After nearly a full minute, your groin aches with the discomfort of it all.\n\n', false );
			//More cum paragraph.  HAHA! TINY BABY CUM! ;
			EngineCore.outputText( 'She wastes no time, and caresses you again.  You instantly feel another surge of heat and desire as a fresh load of cum brews behind your first strangled orgasm.  You need to cum so bad, her foot still stroking and squeezing you full of perverted desire.  She slaps your ', false );
			if( CoC.getInstance().player.balls > 0 ) {
				EngineCore.outputText( 'balls', false );
			} else {
				EngineCore.outputText( 'ass', false );
			}
			EngineCore.outputText( ' as she releases your ' + Descriptors.cockDescript( 0 ) + ', shouting, "<i>CUM!  Feed me!</i>"  You are all too happy to oblige.  ', false );
			//[[normal volume]];
			if( CoC.getInstance().player.cumQ() < 50 ) {
				EngineCore.outputText( 'Freed at last, your body clenches tightly as it squirts the first jet of cum from your ' + Descriptors.cockDescript( 0 ) + '.  She smears her foot over the head, catching the cum and using it to lubricate her silken foot as it massages your member with merciless strokes, alternatively catching your spooge and milking more from your obedient maleness.  Your orgasm lasts many times longer than normal as your dual loads feed her demonic hunger.', false );
			}
			//[[big volume]];
			if( CoC.getInstance().player.cumQ() >= 50 && CoC.getInstance().player.cumQ() < 400 ) {
	...
	DungeonCore.prototype.succubusVictoryRape = function() {
		...
		if( CoC.getInstance().player.cocks.length > 0 && (CoC.getInstance().player.gender !== 3 || Utils.rand( 2 )) ) {
			...
			if( CoC.getInstance().player.cockArea( 0 ) > CoC.getInstance().monster.vaginalCapacity() ) {
				...
				EngineCore.outputText( 'Still, your possessed maleness is far from finished as it continues to slide along her belly and between her still-dripping tits, and before long you feel another orgasm building.  You hang on for dear life, hoping just to stay conscious through the ordeal...\n\n\nHours later you pull away, sated.  For now.', false );
			}
			//((IT FITS));
			else {
				EngineCore.outputText( 'You plunge in to her velvety depths and feel her rippling cunt-muscles contract tightly around you for a perfect fit.  She gasps as each cock-distorting nodule bounces her two-inch clitty, making your mirror image moan like a bitch in heat.  The corrupted fluids dripping from her snatch squelch loudly, making your groin burn with pleasure.  ', false );
				if( CoC.getInstance().player.balls > 0 ) {
					EngineCore.outputText( 'Looking down, you even see the veins on your sack darkening to solid black as the corruption begins tainting your sperm-factories.  ', false );
				}
				EngineCore.outputText( 'You pull back, letting the ring of pointed fleshy barbs spring free before plunging them back in. The pleasure makes you swoon, nearly forcing you to orgasm on the spot.  ', false );
				if( CoC.getInstance().player.biggestTitSize() > 0 ) {
					EngineCore.outputText( 'Your female double reaches down to pinch your nipple, spurring you on, "<i>Please, could you like, cum for me?</i>"', false );
				}
				//New PG;
				EngineCore.outputText( '\n\n', false );
				CoC.getInstance().player.cocks[ 0 ].cockType = CockTypesEnum.DEMON;
				EngineCore.outputText( 'The succubus bucks her hips weakly, still clearly defeated, but egging on your orgasm as dark fluids squelch free from her quim.  "<i>Please... can\'t you see how hot you\'re making me?  I\'ve made your cock perfect, so please make me cum!</i>" she begs as she quivers with delight.  ', false );
				EngineCore.outputText( 'You pause to think about it, not noticing that your groin is pounding away with furious energy, splattering cum and pre over her thighs as your new cock\'s instincts take control from your waist down.  Gasping with sudden pleasure, you feel a flash of heat pass through your tainted meatstick as an orgasm builds.', false );
				//New PG;
				EngineCore.outputText( 'Each of the new bumps and rounded spines of your ' + Descriptors.cockDescript( 0 ) + ' flood with blood, doubling in size as orgasm overtakes you.  ', false );
				EngineCore.outputText( 'Your hips bury your entire length inside her, acting on their own as your ' + Descriptors.cockDescript( 0 ) + ' clenches, pouring more and more spunk inside her', false );
				if( CoC.getInstance().player.balls > 0 ) {
					EngineCore.outputText( ' as your balls empty their load queue', false );
				}
				EngineCore.outputText( '.  ', false );
				//Big cum;
				if( CoC.getInstance().player.cumQ() >= 50 && CoC.getInstance().player.cumQ() < 400 ) {
					EngineCore.outputText( 'You inhale as black cum spatters from her entrance, her belly distending slightly as you empty what feels like a gallon inside her.  ', false );
				}
				//((Ginormohuge)) ;
				if( CoC.getInstance().player.cumQ() >= 400 ) {
					EngineCore.outputText( 'Gasping in pleasure and surprise, you marvel as her belly visibly expands with each eruption of your dark load.  At first it looks like a tiny belly, but by the time the orgasm finishes, your girlish double looks like a woman in her ninth month of pregnancy – with twins.', false );
				}
		...
		//FEMSAUCE;
			...
			if( CoC.getInstance().monster.HP < 1 ) {
				EngineCore.outputText( 'Your foe staggers and falls hard on her ass, utterly defeated.  Her bruises and lacerations slowly fade and heal, regenerating with the aid of her demonic powers.  You easily tear through her clothes, leaving only the damaged stockings that gird her legs. It doesn\'t take much to force her down on her back and straddle her as you disrobe, ready to take your pleasure.\n\n', false );
			} else {
				EngineCore.outputText( 'Your foe drops to her knees, stuffing three digits into her greedy snatch as arousal overcomes her desire to subdue you.  With great care, you approach your insensible enemy and tear off her clothes, leaving her wearing only the remains of her stockings as you force her down on her back.  As if possessing a will of their own, her legs lewdly spread as you disrobe.\n\n', false );
			}
			EngineCore.outputText( 'Her budding clit rises from between her folds, hardening like a tiny three inch dick.\n\n', false );
			if( CoC.getInstance().player.biggestLactation() > 1 ) {
				EngineCore.outputText( '<b>You could scissor with her, or maybe force-feed her some of the milk you\'ve backed up.  Which will it be?</b>', false );
				EngineCore.choices( 'Scissor', this.dungeonSuccubusForceScissor, 'ForceFeed', this.dungeonSuccubusForceFeed, '', null, '', null, '', null );
			}
	...
	DungeonCore.prototype.dungeonSuccubusForceFeed = function() {
		...
		EngineCore.outputText( '.  You jiggle back and forth, lining up a ' + Descriptors.nippleDescript( 0 ) + ' with the demon\'s parted lips.  You press your weight down threateningly, making it clear you could suffocate her with a boob.\n\n', false );
		EngineCore.outputText( '"<i>Drink up bitch, these tits are full!</i>" you command.\n\n', false );
		EngineCore.outputText( 'The succubus tentatively takes a ' + Descriptors.nippleDescript( 0 ) + ' into her mouth, sighing at the tangy taste of your sweat-drenched skin.  Her lips gently suckle, wrapping around the perky hardening nub as it fills with arousal and milk.  You feel something let go inside your ' + Descriptors.breastDescript( 0 ) + ' and the succubus smiles, now working to free more of your trapped cream.\n\n', false );
		if( CoC.getInstance().player.biggestLactation() < 2 ) {
			EngineCore.outputText( 'Her flexible tongue easily curls around one of your ' + Descriptors.nippleDescript( 0 ) + '\'s, letting her pull and tug on it as she increases the suction from her lips.  Your body rewards her efforts with a faster flow of milk that she sucks down as if she were starving.\n\n', false );
		}
		if( CoC.getInstance().player.biggestLactation() >= 2 && CoC.getInstance().player.biggestLactation() < 3 ) {
			EngineCore.outputText( 'Her flexible tongue wraps around your milk-engorged nipple, pulling it tightly as she increases the suction of her lips.  Your body wastes no time rewarding her and she begins gulping down a steady supply of your breastmilk with a pleased expression on her face. You muse to yourself that perhaps succubi are masochists as breast-milk runs freely from your un-milked ', false );
			if( CoC.getInstance().player.totalBreasts() > 2 ) {
				EngineCore.outputText( 'tits.\n\n', false );
			} else {
				EngineCore.outputText( 'tit.\n\n', false );
			}
		}
		if( CoC.getInstance().player.biggestLactation() >= 3 && CoC.getInstance().player.biggestLactation() < 4 ) {
			EngineCore.outputText( 'Her flexible tongue wraps around a milk-swollen nipple, immediately squeezing out a jet of thick breast-milk.  The tongue squeezes and tugs while the succubus ramps up the suction between her thick bee-stung lips.  Your body rewards her with fountains of milk, forcing her to visibly gulp and struggle to keep up with the heavy flow.  Milk runs freely down the ' + CoC.getInstance().player.skinDesc + ' on your chest, pooling around the succubus\' groin and your own as the unattended nipple', false );
			if( CoC.getInstance().player.totalBreasts() > 2 ) {
				EngineCore.outputText( 's', false );
			}
			EngineCore.outputText( ' can\'t help but dribble in sympathy.\n\n', false );
		}
		if( CoC.getInstance().player.biggestLactation() >= 4 ) {
			EngineCore.outputText( 'Her flexible tongue wraps around a milk-bloated nipple, immediately releasing a massive spray of cream that pours into her gullet, nearly choking her.  You stifle a giggle and pull her closer.  Thankfully, her determined tongue manages to stay in place and start tugging your nipple about, releasing even more of your over-large milk production.  She struggles, her throat and cheeks bulging from your explosive output of milk, until it overwhelms her and begin to pour out of her nose.  More milk pours from your unoccupied nipple', false );
			if( CoC.getInstance().player.totalBreasts() > 2 ) {
				EngineCore.outputText( 's', false );
			}
			EngineCore.outputText( ' in sympathy, drenching your ' + CoC.getInstance().player.skinDesc + ' with creamy goodness until it puddles on your captive demon.\n\n', false );
		}
		if( CoC.getInstance().player.totalNipples() === 2 ) {
			EngineCore.outputText( 'As your first nipple drains, you move her to your other breast, being sure to motivate her cunt by grinding it with your ' + CoC.getInstance().player.foot() + '.  She squeals and sucks harder, emptying the last of your milk with a cute burp.  ', false );
		}
		if( CoC.getInstance().player.totalNipples() > 2 && CoC.getInstance().player.totalNipples() < 6 ) {
			EngineCore.outputText( 'As your first ' + Descriptors.nippleDescript( 0 ) + ' drains, you forcibly move her to the next, unleashing a fresh batch of milk for her to feast upon.  Eventually, it too dries up, so you migrate her onwards to your next ' + Descriptors.nippleDescript( 0 ) + '.  After she drains each of your ' + Utils.num2Text( CoC.getInstance().player.totalNipples() ) + ', you smile happily at your now emptied breasts.  ', false );
		}
		if( CoC.getInstance().player.totalNipples() >= 6 ) {
		...
		if( CoC.getInstance().player.averageLactation() * CoC.getInstance().player.totalBreasts() < 6 ) {
			EngineCore.outputText( 'Her belly bulges slightly from all the breast-milk she\'s consumed.\n\n', false );
		} else {
			EngineCore.outputText( 'The succubus looks bloated and pregnant from all the milk you\'ve forced into her.  She sloshes and moans incoherently from the strain of it all.\n\n', false );
		}
		EngineCore.outputText( 'Despite the relief your ' + Descriptors.allBreastsDescript() + ' now feel, your ' + Descriptors.vaginaDescript( 0 ), false );
		if( CoC.getInstance().player.cocks.length > 0 ) {
			EngineCore.outputText( ' and ' + CoC.getInstance().player.multiCockDescriptLight() + ' feel hungrier than ever.  ', false );
		} else {
			EngineCore.outputText( ' feels hungrier than ever.  ', false );
		}
		EngineCore.outputText( 'You shove your crotch into your milk-dazed foe\'s white-stained visage, grinding your ' + Descriptors.vaginaDescript( 0 ) + ' into her mouth until you cum all over her', false );
		if( CoC.getInstance().player.cocks.length === 0 ) {
			if( CoC.getInstance().player.vaginas[ 0 ].vaginalWetness >= AppearanceDefs.VAGINA_WETNESS_SLAVERING ) {
				EngineCore.outputText( ', soaking her with girl-cum.', false );
			} else {
				if( CoC.getInstance().player.vaginas[ 0 ].vaginalWetness <= AppearanceDefs.VAGINA_WETNESS_WET ) {
					EngineCore.outputText( ', slicking her face with girlish cum.', false );
				} else {
					EngineCore.outputText( ', drenching her with a deluge of girlcum.', false );
				}
			}
		} else {
			EngineCore.outputText( ', and unloading a wave of hot spunk into her hair.', false );
		}
		...
		if( CoC.getInstance().player.cocks.length > 0 ) {
			EngineCore.outputText( ' and a thick layer of spunk', false );
		}
		EngineCore.outputText( '.', false );
		EngineCore.outputText( '\n\nYou turn away with a bemused sigh.  When you glance back, she has vanished!', false );
	...
	DungeonCore.prototype.takeIronKey = function() {
		...
		CoC.getInstance().player.createKeyItem( 'Iron Key', 0, 0, 0, 0 );
		EngineCore.doNext( EventParser.playerMenu );
	};
	...
	DungeonCore.prototype.takeCockMilker = function() {
		...
		CoC.getInstance().player.createKeyItem( 'Cock Milker', 0, 0, 0, 0 );
		CoC.getInstance().player.createStatusAffect( StatusAffects.BuiltMilker, 0, 0, 0, 0 );
		EngineCore.doNext( EventParser.playerMenu );
	};
	DungeonCore.prototype.takeBreastMilker = function() {
		...
		EngineCore.outputText( '\n\nYou\'ll need a little help to use it though.' );
		CoC.getInstance().player.createKeyItem( 'Breast Milker', 0, 0, 0, 0 );
		CoC.getInstance().player.createStatusAffect( StatusAffects.BuiltMilker, 0, 0, 0, 0 );
		EngineCore.doNext( EventParser.playerMenu );
	};
	DungeonCore.prototype.talkToIncubus = function() {
		EngineCore.spriteSelect( 30 );
		EngineCore.clearOutput();
		if( CoC.getInstance().player.hasKeyItem( 'Hentai Comic' ) >= 0 ) {
			...
			EngineCore.choices( 'Fight', this.startIncubusFight, 'Trade', this.tradeComic, 'Submit', this.submitToIncubus, '', null, '', null );
		} else {
			EngineCore.outputText( 'The incubus speaks to you with a calm, deep voice, "<i>And so the insect, unaware of its path, stumbles directly into the spider\'s web.  Tiny insect, you have little to offer me, but everything to offer our facility.  Why don\'t you come along quietly?</i>"' );
			EngineCore.choices( 'Fight', this.startIncubusFight, 'Submit', this.submitToIncubus, '', null, '', null, '', null );
		}
	};
	DungeonCore.prototype.startIncubusFight = function() {
		EngineCore.spriteSelect( 30 );
		CoC.getInstance().player.createStatusAffect( StatusAffects.FactoryIncubusDefeated, 0, 0, 0, 0 ); //Won't matter if you lose
		Combat.startCombat( new IncubusMechanic(), true );
	};
	DungeonCore.prototype.submitToIncubus = function() {
		EngineCore.spriteSelect( 30 );
		EngineCore.outputText( '"<i>It is good to see the insect accept its fate as the spider closes in,</i>" intones the strange demonic mechanic as he takes you by the arm and leads you deeper into the facility.  ' );
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.DungeonShutDown ) >= 0 ) {
	...
	DungeonCore.prototype.tradeComic = function() {
		...
		CoC.getInstance().player.removeKeyItem( 'Hentai Comic' );
		CoC.getInstance().player.createStatusAffect( StatusAffects.IncubusBribed, 0, 0, 0, 0 );
		EngineCore.doNext( EventParser.playerMenu );
	};
	DungeonCore.prototype.incubusLossRape = function() {
		...
		//Tentacle gangbang;
			...
			if( CoC.getInstance().player.lust > 99 ) {
				EngineCore.outputText( 'Molten arousal pumps through your veins, burning away your reason with an unquenchable desire to mate. You drop your top, exposing your ' + Descriptors.allBreastsDescript() + ' to your foe in a submissive display, ', false );
			} else {
				EngineCore.outputText( 'You lower your top, exposing your nubile form to your foe in a submissive display, ', false );
			}
			EngineCore.outputText( 'lowering your eyes and fixating on his now-rigid demonic member.  Right before your eyes, it begins splitting and dividing into thinner prehensile penises that squirm about in the air, each one reminding you of a snake on the prowl.  ', false );
			if( CoC.getInstance().player.cor < 80 ) {
				EngineCore.outputText( 'In a disgusting display', false );
			} else {
				EngineCore.outputText( 'As you grope yourself noisily with your hand into your undergarments, a salacious smile on your lips', false );
			}
			EngineCore.outputText( ', you watch as his pricks pulse and thicken out until their masses are as wide as his original dick.\n\n', false );
			if( CoC.getInstance().player.cor >= 80 ) {
				EngineCore.outputText( 'As you realize their size and number, you open your eyes wide and smile broadly, reflexively spreading your legs wide, practically begging him to fuck you.  ', false );
			}
			EngineCore.outputText( 'In a flash, each fat tentacle-cock whips out and surrounds your body in slick demon-flesh.  The tentacles constrict, working in pairs to take off every piece of your ' + CoC.getInstance().player.armorName + '.  ', false );
			if( CoC.getInstance().player.biggestTitSize() >= 2 ) {
				EngineCore.outputText( 'They make sure to rub each of your breasts, spending a few seconds smearing slick pre-cum into your ' + Descriptors.nippleDescript( 0 ) + '.  ', false );
			}
			EngineCore.outputText( 'A pair of them slides into your undergarments, pressing against your needy crotch and teasing your ' + Descriptors.assholeDescript() + ' with more slick demonic cum.  ', false );
			if( CoC.getInstance().player.cor < 80 ) {
				EngineCore.outputText( 'You wriggle and whine,', false );
			} else {
				EngineCore.outputText( 'You grab the thick tentacle-cock working on your cunt with both hands, as you can barely grab it with one. Then,  while letting out moans fit for a bitch in heat, which you are, you begin', false );
			}
			if( CoC.getInstance().player.vaginas.length > 0 ) {
				EngineCore.outputText( 'squeezing your legs around them and grinding your ' + Descriptors.clitDescript() + ' against the oddly textured demon-cock.', false );
			} else {
				if( CoC.getInstance().player.balls > 0 ) {
					EngineCore.outputText( 'grinding down against the ribbed and textured demonic cock as it slides between your ' + Descriptors.ballsDescriptLight() + '.', false );
				} else if( CoC.getInstance().player.cocks.length > 0 ) {
					EngineCore.outputText( 'squeezing your legs around the thick demonic flesh as a steady dribble of pre-cum drips from your ' + Descriptors.cockDescript( 0 ) + '.', false );
				}
				if( CoC.getInstance().player.gender === 0 ) {
					EngineCore.outputText( 'grinding suggestively on your captor\'s rods, barely noticing as the last of your ' + CoC.getInstance().player.armorName + ' falls away.', false );
				}
			}
			EngineCore.outputText( '\n\n', false );
			//FUCKKKING;
			//Female paragraph;
			if( CoC.getInstance().player.vaginas.length > 0 && (CoC.getInstance().player.cocks.length === 0 || Utils.rand( 2 )) ) {
				EngineCore.outputText( 'The incubus at last decides to tend to your over-aroused body and pulls you off the ground with his tentacles, suspending you in mid-air.  ', false );
				if( CoC.getInstance().player.cor < 80 ) {
					EngineCore.outputText( 'You feel your ' + CoC.getInstance().player.legs() + ' lifted and pulled tight as countless demonic cocks encircle your body, binding and constraining you further.  You whimper as a demonic tentacle probes your back door while a thicker one lines itself up just below your ' + Descriptors.clitDescript() + '.  ', false );
				} else {
				...
				EngineCore.outputText( 'You then giggle and try to wiggle your ' + Descriptors.hipDescript() + ' forward, begging for him to take you and quench the fire burning in your ' + Descriptors.vaginaDescript( 0 ) + '.  The incubus obliges, pressing forth with both drooling members and simultaneously plugging your front and back doors.  ', false );
				if( CoC.getInstance().player.cor < 80 ) {
					EngineCore.outputText( 'You gasp from pleasure and surprise, ', false );
				} else {
					EngineCore.outputText( 'You open your mouth wide, as the enormous tentacle-cocks force their way in, dripping demonic pre-cum all over. Quickly, it acts as additional lubricant, and the pain largely subsides.  You find that these massive demonic cocks\' size is perfect to stimulate, in an incredibly pleasurable way, all sides of your cunt at once, and to stretch your ass just a bit over your preferred size.  As the incubus starts moving his appendages in rhythm, you\'re lost in heavenly pleasure, eyes closed, letting out deafening moans of lust, your legs and arms dangling without any thought for dignity.  You are deeply ', false );
				}
				EngineCore.outputText( 'enjoying the knobbed texture of his shafts as you\'re double-penetrated by a single demon. The incubus smirks as another cock-tentacle wraps itself up around your neck like a shiny student collar and plugs your noisy little mouth.  You groan into his member as you\'re ', false );
				if( CoC.getInstance().player.cor < 80 ) {
					EngineCore.outputText( 'squeezed and caressed by the writhing tentacle-pricks in and around your body, lost in the pleasure and taste of demonic pre-cum.\n\n', false );
				} else {
					EngineCore.outputText( 'getting roughly fucked by the two tentacle-cocks at the same time.  Taking the tentacle-cock in your mouth with both hands, you eagerly swallow every bit of demonic pre-cum, then suckle on the huge cock-slit. \n\n', false );
				}

				//FemCum;
				if( CoC.getInstance().player.clitLength > 3 ) {
					EngineCore.outputText( 'You nearly cum on the spot when the cock fucking your pussy loops its length around your ' + Descriptors.clitDescript() + ', the cum-slickened coils driving you mad with pleasure as they coil, slide, and jerk around your clit as if it was a cock.  ', false );
				} else {
					EngineCore.outputText( 'You nearly cum on the spot when the cock fucking your pussy curves up to rub its textured nodules against your ' + Descriptors.clitDescript() + '.  ', false );
				}
				CoC.getInstance().player.cuntChange( CoC.getInstance().player.vaginalCapacity() * 0.8, true );
				if( CoC.getInstance().player.cor >= 80 ) {
					EngineCore.outputText( 'You cum more times than you are able to count, each time causing a tightening of your fuckholes, which increases the rubbing against the demonic nodules and sends another wave of pleasure to your dazed brain.  You begin to drool freely, reveling in this most unholy mating.  ', false );
				}
				EngineCore.outputText( 'The prick in your mouth surges forward, sliding deep into your throat.  The coils around your neck tighten in response, choking your neck into a tight cock-sleeve as you feel bulges of cum moving along its length.  In moments you feel your belly starting to grow full, sloshing with cum as you become desperate to breathe.  The tentacles lodged in your ' + Descriptors.assholeDescript() + ' and ' + Descriptors.vaginaDescript( 0 ) + ' react in similar fashion, stretching you wide as they begin pumping your body full of vast quantities of spunk.  A few free tentacles begin spurting gobs of the white stuff onto your ' + CoC.getInstance().player.skinDesc + ', soaking you in the stuff as you black out from a combination of oxygen deprivation and pleasure.', false );
				CoC.getInstance().player.orgasm();
			...
			//Male/Genderless;
				...
				if( CoC.getInstance().player.balls > 0 ) {
					EngineCore.outputText( '  You groan around the dick plugging your throat as another tentacle-like appendage wraps around your ' + Descriptors.sackDescript() + ', pulling your ' + Descriptors.ballsDescriptLight() + ' down and gently squeezing them.', false );
				}
				EngineCore.outputText( '\n\n', false );
				//Genderless Orgasm;
				if( CoC.getInstance().player.gender === 0 ) {
					EngineCore.outputText( 'The tainted cum mixed with the sensation of fullness provide you with pleasures beyond what your genderless body could accomplish on its own.  You writhe as the demon face-fucks you with one tendril while another continues to bury itself ever-deeper into your abused ' + Descriptors.assholeDescript() + '.  ', false );
				}
				if( CoC.getInstance().player.cocks.length > 0 ) {
					EngineCore.outputText( 'The cock-tentacle around your ' + Descriptors.cockDescript( 0 ) + '  increases the pace of its stimulation as it begins to spurt hot wet cum over you, giving it lubrication as it jacks you off while staying tight around your base to prevent you from an orgasming.  ', false );
				}
				EngineCore.outputText( 'You feel cum pulse through the tentacles encircling you as the incubus loses control of his tentacles.  Cum pumps into your belly, suffusing you with drug-like warmth as the tentacle around your neck pulls tight enough to squeeze the cock inside your throat.  You squirm and gasp for oxygen as spooge begins unloading into and around your body to the point where you aren\'t sure where your body begins and the demonic-spunk ends.  You twitch in what you assume is orgasm as you fight to breathe; all the while more cum is squeezed into your stuffed belly and ruined anus. The tentacle in your ass backs out slowly, having filled every inch of your intestines with cum, until it pops free with a splatter.', false );
				EngineCore.outputText( '\n\n', false );
				if( CoC.getInstance().player.cocks.length > 0 ) {
					EngineCore.outputText( 'Being so thoroughly used and stimulated pushes you over the edge of orgasm, and your ', false );
					if( CoC.getInstance().player.balls > 0 ) {
						EngineCore.outputText( 'balls', false );
					} else {
						EngineCore.outputText( 'prostate', false );
					}
					EngineCore.outputText( ' unloads with enough force to squeeze past the constrictor clutching at your groin.\n\n', false );
					//Small cum;
					if( CoC.getInstance().player.cumQ() < 50 ) {
						EngineCore.outputText( 'You groan and orgasm with enough force to splatter a few ropes of cum into the sea of demon-spunk that soaks you from head to toe.  ', false );
					}
					//Big cum;
					if( CoC.getInstance().player.cumQ() >= 50 && CoC.getInstance().player.cumQ() < 400 ) {
						EngineCore.outputText( 'Your orgasm goes off like a shotgun blast, splattering the incubus with a huge wad of cum.  It\'s but the first of many, and though each load of jizz is of comparable size, the force behind them diminishes until the last few blasts drip down your body and soak your ' + CoC.getInstance().player.legs() + ' and ' + CoC.getInstance().player.feet() + '.  ', false );
					}
					//Huge cum;
					if( CoC.getInstance().player.cumQ() >= 400 ) {
						EngineCore.outputText( 'Your orgasm goes off like a volcano, visibly distending your ' + Descriptors.cockDescript( 0 ) + ' as a huge wave of cum erupts from your groin, painting the incubus and floor with your spoo.  You cry from the sheer pleasure as the next wave builds and erupts, nearly as large as the last.  The demon-cock controlling your prick aims this blast up, forcing you to soak your ' + Descriptors.hairDescript() + ' and face with slick goo. The orgasming drags on and on while you slowly turn blue from oxygen deprivation. Before long, both you and the incubus are buried under a wave of white.  ', false );
					}
				}
				EngineCore.outputText( 'The feeling is so intense that your ' + Descriptors.hipDescript() + ' twitch and move of their own volition while your eyes roll back in pleasure.\n\n', false );
				EngineCore.outputText( 'You black out just as you feel the cock-tentacle in your throat retracting. You dully feel your body drop to the ground, your pregnant-looking belly sloshing with demon jizz.', false );
				CoC.getInstance().player.buttChange( CoC.getInstance().monster.cockArea( 0 ), true );
				CoC.getInstance().player.orgasm();
				EngineCore.dynStats( 'cor', 25 );
				if( CoC.getInstance().player.findStatusAffect( StatusAffects.DungeonShutDown ) < 0 ) {
	...
	DungeonCore.prototype.incubusVictoryRapeSex = function() {
		...
		if( CoC.getInstance().player.cocks.length > 0 ) {
			EngineCore.outputText( 'With a few deft motions, you shift your ' + CoC.getInstance().player.armorName + ' to expose your ', false );
			if( CoC.getInstance().player.gender === 3 ) {
				if( CoC.getInstance().player.balls > 0 ) {
					EngineCore.outputText( CoC.getInstance().player.multiCockDescriptLight() + ', ' + Descriptors.ballsDescriptLight() + ', and ' + Descriptors.vaginaDescript( 0 ), false );
				} else {
					EngineCore.outputText( CoC.getInstance().player.multiCockDescriptLight() + ' and ' + Descriptors.vaginaDescript( 0 ), false );
				}
			} else {
				if( CoC.getInstance().player.balls > 0 ) {
					EngineCore.outputText( CoC.getInstance().player.multiCockDescriptLight() + ' and ' + Descriptors.ballsDescriptLight(), false );
				} else {
					EngineCore.outputText( CoC.getInstance().player.multiCockDescriptLight(), false );
				}
			}
			EngineCore.outputText( '.  Having resolved to take the demon\'s backdoor, you approach his weakened form with brimming confidence.  He looks up, clearly hoping your plan is to squat on his throbbing member.  You dispel his misguided notion when you grab him by the horns and shove his face against the floor. He struggles weakly until you press down harder, making it clear he is to stay in position - on his knees with his head down and his ass in the air.  Circling your prey, you inspect his flawless body and carefully note that the hole at his crotch actually exposes a fair portion of his very supple and surprisingly feminine-looking backside.\n\n', false );
			EngineCore.outputText( 'You don\'t waste any time, gripping your ' + Descriptors.cockDescript( 0 ) + ' in one hand and ', false );
			if( CoC.getInstance().player.cocks[ 0 ].cockType === CockTypesEnum.HORSE ) {
				EngineCore.outputText( 'pressing your thick flare ', false );
			}
			if( CoC.getInstance().player.cocks[ 0 ].cockType === CockTypesEnum.DOG ) {
				EngineCore.outputText( 'pressing your pointed tip ', false );
			}
			if( CoC.getInstance().player.cocks[ 0 ].cockType === CockTypesEnum.HUMAN || CoC.getInstance().player.cocks[ 0 ].cockType.Index > 2 ) {
				EngineCore.outputText( 'pressing your head ', false );
			}
			EngineCore.outputText( 'between the incubus\' cheeks towards his inhumanly smooth rear-passage.  You gasp in delight at the tight ribbed texture of his asshole as you slide ', false );
			if( CoC.getInstance().player.cocks[ 0 ].cockLength > 10 ) {
				EngineCore.outputText( 'deep inside ', false );
			} else {
				EngineCore.outputText( 'inside ', false );
			}
			EngineCore.outputText( '.  The demon underneath you grunts in an attempt to sound displeased, but it\'s plain to see the pre-cum he\'s dripping all over the floor.  What a slut!  You slap his ass and begin roughly butt-fucking him, panting with each plunge into the depths of his ridged passage, gradually increasing your tempo until your ' + Descriptors.hipDescript() + ' fill the room with loud slapping noises.', false );
			if( CoC.getInstance().player.balls > 0 ) {
				EngineCore.outputText( '  Your ' + Descriptors.ballsDescriptLight() + ' swing freely, smacking into the demon\'s own and making both of you squeal and dribble more pre-cum.', false );
			}
			EngineCore.outputText( '\n\n', false );
			//(CUM)  ;
			if( CoC.getInstance().player.balls > 0 ) {
				EngineCore.outputText( 'You feel your ' + Descriptors.ballsDescriptLight() + ' draw up tight against your body.  ', false );
			}
			EngineCore.outputText( 'Warm heat begins to build inside your groin, pooling under the base of your ' + Descriptors.cockDescript( 0 ) + '.  You realize you\'re about to paint this demon\'s gut with white, the thought only turning you on more.  ', false );
			if( CoC.getInstance().player.cumQ() > 200 ) {
				EngineCore.outputText( 'You groan as you feel your urethra being stretched by the sheer volume of fluid beginning to shoot through it.  ', false );
			}
			EngineCore.outputText( 'You throw back your head and cum, slapping the incubus\' ass with one hand while you grip and squeeze the jiggling flesh of his other cheek.  ', false );
			if( CoC.getInstance().player.cumQ() < 50 ) {
				EngineCore.outputText( 'A few thick spurts later and y', false );
			}
			if( CoC.getInstance().player.cumQ() >= 50 && CoC.getInstance().player.cumQ() < 400 ) {
				EngineCore.outputText( 'Thick jets of cum pump into the demon\'s plump backside, soon building up a wave of pressure that pushes back against you.  Y', false );
			}
			if( CoC.getInstance().player.cumQ() >= 400 ) {
				EngineCore.outputText( 'A massive cock-distending bulge of cum works through your shaft, splashing into the demon\'s rectum in an explosive burst of pleasure. Unfortunately for your victim, it is only the first of many such cum-blasts. In no time flat, jism is spurting from his overfilled rectum while his belly looks a few months pregnant. You feel weak from discharging so much fluid, and y', false );
			}
			EngineCore.outputText( 'ou fall back, the fluid of your orgasm dripping from your ' + Descriptors.cockDescript( 0 ) + ' and the gaping asshole of your latest conquest.\n\nYou turn to gather your ' + CoC.getInstance().player.armorName + ', and when you look back the demon is gone, leaving only a small puddle of male fluids in his wake.', false );
			CoC.getInstance().player.orgasm();
			Combat.cleanupAfterCombat();
			return;
		}
	...
	DungeonCore.prototype.omnibusStartCombat = function() {
		EngineCore.spriteSelect( 16 );
		EngineCore.clearOutput();
		EngineCore.outputText( 'You strike a combat pose and prepare your ' + CoC.getInstance().player.weaponName + '.  She smiles and saunters around the desk, letting something bulbous and fleshy drop free from between her nether-lips.  You watch in shock as it hardens into a dick, growing right from where her clit should be.\n\nShe taunts, "<i>Like what you see cow?  I\'ll be sure to visit you in the pens.</i>\'"' );
		CoC.getInstance().player.createStatusAffect( StatusAffects.FactoryOmnibusDefeated, 0, 0, 0, 0 ); //This won't matter if you lose to her
	...
	DungeonCore.prototype.omnibusVictoryEvent = function() {
		...
		if( CoC.getInstance().monster.lust > 99 ) {
			...
			EngineCore.outputText( 'The omnibus trembles and drops to her knees, utterly defeated.\n\n' );
			EngineCore.outputText( '"<i>Please, if you\'ll let me go I could use my magics to give you nearly anything you want!  Just please don\'t tell the other demons what happened here, I\'d never live it down,</i>" she begs.\n\n' );
		}
		EngineCore.outputText( 'What do you do?  You could use her boon increase the size of your endowments or maybe regain some of your lost humanity!  Or you could play it safe and turn down her offer.  Although then you\'d have to decide to let her go or kill her.' );
		EngineCore.menu();
		EngineCore.addButton( 0, 'GrowBreasts', this.omnibusVictoryGrowBreasts );
		EngineCore.addButton( 1, 'Grow Dick', this.omnibusVictoryGrowDick );
		EngineCore.addButton( 2, 'Normal-Face', this.omnibusVictoryNormalFace );
		EngineCore.addButton( 3, 'Normal-Chest', this.omnibusVictoryNormalChest );
		EngineCore.addButton( 4, 'Normal-Groin', this.omnibusVictoryNormalGroin );
		EngineCore.addButton( 5, 'Normal-Legs', this.omnibusVictoryNormalLegs );
		EngineCore.addButton( 8, 'No (Let go)', this.omnibusVictoryLetGo );
		EngineCore.addButton( 9, 'No (Kill Her)', this.omnibusVictoryKillHer );
	};
	DungeonCore.prototype.omnibusVictoryGrowBreasts = function() {
		...
		else if( CoC.getInstance().player.breastRows.length <= 1 ) {
			EngineCore.outputText( 'Your ' + Descriptors.allBreastsDescript() + ' tingle pleasantly as the magic takes effect.  You watch with fascination as they begin to swell up, like sponges exposed to water.  The top of your ' + CoC.getInstance().player.armorName + ' is pulled tight by change, until your chest seems ready to burst free.  <b>You\'ve gained 3 cup sizes!</b>  ' );
			CoC.getInstance().player.breastRows[ 0 ].breastRating += 3;
			if( CoC.getInstance().player.nippleLength < 0.75 ) {
				CoC.getInstance().player.nippleLength += 0.5;
				EngineCore.outputText( 'Your ' + Descriptors.nippleDescript( 0 ) + 's grow hard and sensitive, becoming much more noticeable inside your ' + CoC.getInstance().player.armorName + '.  It appears your nipples have grown larger to match.' );
			}
		}
		//Multiple Rows...;
		else {
			//Top row + 3, all other rows brought up to par.;
			EngineCore.outputText( 'Your top ' + Descriptors.breastDescript( 0 ) + ' tingle pleasantly as the magic takes effect.  You watch with fascination as they begin to swell up, like sponges exposed to water.  The top of your ' + CoC.getInstance().player.armorName + ' is pulled tight by change, until your chest seems ready to burst free.  <b>You\'ve gained 4 cup sizes!</b>  ' );
			CoC.getInstance().player.breastRows[ 0 ].breastRating += 4;
			EngineCore.outputText( 'The next row of ' + Descriptors.breastDescript( 1 ) + ' jiggle and tingle with even more pleasure than the first.  They pulsate for a few seconds, shrinking and growing rapidly until they settle at a size just below your top ' + Descriptors.breastDescript( 0 ) + '.  ' );
			CoC.getInstance().player.breastRows[ 1 ].breastRating = CoC.getInstance().player.breastRows[ 0 ].breastRating - 1;
			if( CoC.getInstance().player.breastRows.length >= 3 ) {
				EngineCore.outputText( 'Your third group of ' + Descriptors.breastDescript( 2 ) + ' seem to follow their sister\'s example, tingling briefly before settling at a size just below the breasts above.  ', false );
				CoC.getInstance().player.breastRows[ 2 ].breastRating = CoC.getInstance().player.breastRows[ 1 ].breastRating - 1;
			}
			if( CoC.getInstance().player.breastRows.length >= 4 ) {
				EngineCore.outputText( 'Your remaining ' + Descriptors.breastDescript( 3 ) + ' feel so wonderful that you just can\'t resist cupping and squeezing them as they reshape to fit in perfectly with the rest of your breasts.  ' );
				CoC.getInstance().player.breastRows[ 3 ].breastRating = CoC.getInstance().player.breastRows[ 2 ].breastRating - 1;
				if( CoC.getInstance().player.breastRows.length === 5 ) {
					CoC.getInstance().player.breastRows[ 4 ].breastRating = CoC.getInstance().player.breastRows[ 3 ].breastRating - 1;
				}
			}
			if( CoC.getInstance().player.nippleLength < 0.75 ) {
				CoC.getInstance().player.nippleLength += 0.5;
				EngineCore.outputText( 'Your ' + Descriptors.nippleDescript( 0 ) + 's grow hard and sensitive, becoming much more noticable inside your ' + CoC.getInstance().player.armorName + '.  It appears your nipples are have grown larger to match.' );
			}
		}
		EngineCore.outputText( '\n\n' );
	...
	DungeonCore.prototype.omnibusVictoryGrowDick = function() {
		...
		if( CoC.getInstance().player.cocks.length === 1 ) {
			if( CoC.getInstance().player.demonCocks() < 1 ) {
				EngineCore.outputText( 'You smile, watching your ' + Descriptors.cockDescript( 0 ) + ' grow longer.  Inches of newfound dick-flesh erupt from your crotch in response to omnibus\' dark magics.  Before you can play with your transformed tool, a wave of burning desire passes through you.  ' );
				if( CoC.getInstance().player.cor < 80 ) {
					EngineCore.outputText( 'You watch in horror as the skin of your ' + Descriptors.cockDescript( 0 ) + ' turns shiny and purplish-black.  ' );
				} else {
					EngineCore.outputText( 'Curious, you watch the skin of your ' + Descriptors.cockDescript( 0 ) + ' turn a shiny-dark purple.  ' );
				}
				CoC.getInstance().player.cocks[ 0 ].cockLength += 3 + Utils.rand( 5 );
				CoC.getInstance().player.cocks[ 0 ].thickenCock( 2 );
				CoC.getInstance().player.cocks[ 0 ].cockType = CockTypesEnum.DEMON;
				if( CoC.getInstance().player.cor < 50 ) {
					EngineCore.outputText( 'Corrupt nodules begin to spring up over its entire length.  <b>Your penis is transforming into a ' + Descriptors.cockDescript( 0 ) + '!<\b>  The new nubs wriggle about as they sprout over every inch of surface, save for the head.  Unable to do anything but groan with forced pleasure and horror, you can only watch.  One last batch of nodules forms in a ring around the crown of your ' + Descriptors.cockDescript( 0 ) + ', seemingly completing its transformation, until you notice, almost throwing up, that your testicles are also getting covered in black veins under your powerless eyes!  ' );
				} else {
				...
				EngineCore.outputText( 'Your ' + Descriptors.cockDescript( 0 ) + ' leaps forwards, taking to the dark magic with ease.  Inch after inch of new length erupts from your groin as your ' + Descriptors.cockDescript( 0 ) + ' gets longer and thicker.  It pulsates, as if promising dark pleasure as it settles into its new enhanced size.' );
				CoC.getInstance().player.cocks[ 0 ].cockLength += 6 + Utils.rand( 10 );
				CoC.getInstance().player.cocks[ 0 ].thickenCock( 3 );
			}
		}
		if( CoC.getInstance().player.cocks.length > 1 ) {
			var temp = CoC.getInstance().player.cocks.length;
			//Already has demoncocks;
			if( CoC.getInstance().player.demonCocks() === CoC.getInstance().player.cockTotal() ) {
				EngineCore.outputText( 'Your ' + CoC.getInstance().player.multiCockDescriptLight() + ' leap forwards, taking to the dark magic with ease.  Inch after inch of new length erupts from your groin as your ' + CoC.getInstance().player.multiCockDescriptLight() + ' get longer and thicker.  They pulsate, as if promising dark pleasure as they settle into their new enhanced size.' );
				while( temp > 0 ) {
					temp--;
					CoC.getInstance().player.cocks[ temp ].cockLength += 6 + Utils.rand( 10 );
					CoC.getInstance().player.cocks[ temp ].thickenCock( 3 );
				}
			}
			//Not yet full of demoncocks...;
			else {
				EngineCore.outputText( 'You smile, watching your ' + CoC.getInstance().player.multiCockDescriptLight() + ' grow longer.  Inches of newfound dick-flesh erupt from your crotch in response to omnibus\' dark magics.  Before you can play with your transformed pleasure tools, a wave of burning desire passes through you.  You watch' );
				if( CoC.getInstance().player.cor < 80 ) {
					EngineCore.outputText( ' in horror' );
				} else {
					EngineCore.outputText( ' curiously' );
				}
				EngineCore.outputText( ' as the skin of your ' + CoC.getInstance().player.multiCockDescriptLight() + ' turns shiny and purplish-black.  Corrupt nodules begin to spring up over the entire length of each dick.  ' );
				while( temp > 0 ) {
					temp--;
					CoC.getInstance().player.cocks[ temp ].cockLength += 3 + Utils.rand( 5 );
					CoC.getInstance().player.cocks[ temp ].thickenCock( 2 );
					CoC.getInstance().player.cocks[ temp ].cockType = CockTypesEnum.DEMON;
				}
				if( CoC.getInstance().player.cor < 50 ) {
					EngineCore.outputText( '<b>Your dicks are transforming into ' + CoC.getInstance().player.multiCockDescriptLight() + '!</b>  The new nubs wriggle about as they sprout over every inch of surface, save for the heads.  Unable to do anything but groan with forced pleasure and horror, you can only watch.  One last batch of nodules forms in a ring around the crowns of your ' + CoC.getInstance().player.multiCockDescriptLight() + ', seemingly completing its transformation, until you notice, almost throwing up, that your testicles are also getting covered in black veins under your powerless eyes!  ' );
				} else {
					EngineCore.outputText( '<b>Your dicks are transforming into ' + CoC.getInstance().player.multiCockDescriptLight() + '!</b>  The new nubs wriggle about as they sprout over every inch of surface, save for the heads.  You pant and moan in happiness as they lengthen one last time.  As you stroke all of their amazing length with both hands, the excitement of possessing such a magnificent pleasure tool makes you cum. You lick your fingers eagerly, tasting your new cum, while a last ring of nodules forms around the crowns of your beautiful ' + CoC.getInstance().player.multiCockDescriptLight() + '.   Your new ' + CoC.getInstance().player.multiCockDescriptLight() + ' pulsate darkly with each beat of your heart, but the thick, throbbing veins that are finishing to cover your testicles do not contain blood, but a black liquid which apparently has perverted them. You ponder what its purpose might be, but then you decide, as you stroke a huge, dark, bumpy shaft, that if they feel as good as they look, it doesn\'t really matter.  ' );
				}
			}
		}
		this.omnibusVictoryPostBoon();
	...
	DungeonCore.prototype.omnibusVictoryNormalFace = function() {
		...
		EngineCore.clearOutput();
		var changed = false;
		if( CoC.getInstance().player.horns > 0 || CoC.getInstance().player.antennae > AppearanceDefs.ANTENNAE_NONE ) {
			...
			if( CoC.getInstance().player.horns > 0 ) {
				EngineCore.outputText( 'Your horns fall off, landing on the floor with a heavy thud.  ' );
				CoC.getInstance().player.horns = 0;
				CoC.getInstance().player.hornType = AppearanceDefs.HORNS_NONE;
			}
			if( CoC.getInstance().player.antennae > AppearanceDefs.ANTENNAE_NONE ) {
				EngineCore.outputText( 'Antennae pop free, and float lightly down towards the floor.  ' );
				CoC.getInstance().player.antennae = AppearanceDefs.ANTENNAE_NONE;
			}
			changed = true;
		}
		//EARS;
		if( CoC.getInstance().player.earType !== AppearanceDefs.EARS_HUMAN ) {
			EngineCore.outputText( 'Pain erupts from both sides of your head as your ears reform and move, returning to look like your old human ears!  ' );
			CoC.getInstance().player.earType = AppearanceDefs.EARS_HUMAN;
			changed = true;
		}
		//Face;
		if( CoC.getInstance().player.faceType !== AppearanceDefs.FACE_HUMAN ) {
			EngineCore.outputText( 'Your facial structure rearranges itself into a normal human visage, exactly like yours was before you came to this horrid place.' );
			CoC.getInstance().player.faceType = AppearanceDefs.FACE_HUMAN;
			changed = true;
		}
		//Nothing changed;
		if( !changed ) {
			EngineCore.outputText( 'You tingle briefly but feel no obvious change.  Your face was already fairly human.' );
		}
		this.omnibusVictoryPostBoon();
	};
	DungeonCore.prototype.omnibusVictoryNormalChest = function() {
		...
		var changed = false;
		if( CoC.getInstance().player.breastRows.length > 1 ) {
			CoC.getInstance().player.removeBreastRow( 1, CoC.getInstance().player.breastRows.length - 1 );
			EngineCore.outputText( 'Your chest tingles and begins to feel lighter.  You hastily pull open your ' + CoC.getInstance().player.armorName + ' and realize you only have ' + Descriptors.allBreastsDescript() + ' now!  ' );
			changed = true;
		}
		//Size!;
		if( CoC.getInstance().player.breastRows[ 0 ].breastRating > 7 ) {
			EngineCore.outputText( 'The weighty flesh that constantly hangs from your chest gets lighter and lighter, vanishing rapidly.  ' );
			CoC.getInstance().player.breastRows[ 0 ].breastRating = 3 + Utils.rand( 5 );
			EngineCore.outputText( 'You now have ' + Descriptors.allBreastsDescript() + '.  ' );
			changed = true;
		}
		//Fix nips;
		if( CoC.getInstance().player.nippleLength > 1 ) {
			EngineCore.outputText( 'Your nipples shrink down to a more normal size.  ' );
			CoC.getInstance().player.nippleLength = 0.75;
			changed = true;
		}
		if( CoC.getInstance().player.hasFuckableNipples() ) {
			EngineCore.outputText( 'The vagina-like openings in your nipples close, sealing themselves shut.  ' );
			CoC.getInstance().player.breastRows[ 0 ].fuckable = false;
			changed = true;
		}
		//Normal chest, normal skin;
		if( CoC.getInstance().player.skinType !== AppearanceDefs.SKIN_TYPE_PLAIN ) {
			EngineCore.outputText( 'The skin on your body itches intensely as it sheds its ' + CoC.getInstance().player.skinDesc + ', revealing ' + CoC.getInstance().player.skinTone + ' skin.  ' );
		...
		//Nothing changed;
		if( !changed ) {
			EngineCore.outputText( 'You tingle briefly but feel no obvious change.  Your chest is already fairly human.' );
		}
		this.omnibusVictoryPostBoon();
	};
	DungeonCore.prototype.omnibusVictoryNormalGroin = function() {
		...
		//Remove multiple.;
		if( CoC.getInstance().player.cocks.length > 1 ) {
			EngineCore.outputText( 'Your ' + CoC.getInstance().player.multiCockDescriptLight() + ' shiver and retract back towards your body.  When the process finishes you are left with only your ' + Descriptors.cockDescript( 0 ) + '.  ' );
			CoC.getInstance().player.removeCock( 1, CoC.getInstance().player.cocks.length - 1 );
			CoC.getInstance().player.genderCheck();
			changed = true;
		}
		//Super long nerf;
		if( CoC.getInstance().player.hasCock() ) {
			if( CoC.getInstance().player.cocks[ 0 ].cockLength > 12 ) {
				EngineCore.outputText( 'A tingling sensation worms through your ' + Descriptors.cockDescript( 0 ) + ' as it shrinks down to a more modest eleven inches.  ' );
				CoC.getInstance().player.cocks[ 0 ].cockLength = 11;
				changed = true;
			}
			//Super thick nerf;
			if( CoC.getInstance().player.cocks[ 0 ].cockThickness > 2 ) {
				EngineCore.outputText( 'Your ' + Descriptors.cockDescript( 0 ) + '\'s obscene thickness withers down to roughly two inches of girth.  ' );
				CoC.getInstance().player.cocks[ 0 ].cockThickness = 2;
				changed = true;
			}
			//Humanitize;
			...
			if( CoC.getInstance().player.cocks[ 0 ].cockType === CockTypesEnum.DEMON ) {
				EngineCore.outputText( 'Your ' + Descriptors.cockDescript( 0 ) + ' tingles as the bumps begin to fade.  After a moment the flesh darkens, and every single nodule reappears.  <b>Your corrupt penis resisted the magic!</b>  ' );
				changed = true;
			} else if( CoC.getInstance().player.cocks[ 0 ].cockType !== CockTypesEnum.HUMAN ) {
				EngineCore.outputText( 'The inhuman appearance of your ' + Descriptors.cockDescript( 0 ) + ' shifts, the flesh rearranging itself into a more human configuration.  After a few seconds you have a very normal looking penis.  ' );
		...
		if( CoC.getInstance().player.ballSize > 5 ) {
			EngineCore.outputText( 'The ' + Descriptors.ballsDescriptLight() + ' that constantly pull so heavily on your groin tingle and shrink down to a more managable size.  ' );
			CoC.getInstance().player.ballSize = 2 + Utils.rand( 3 );
			changed = true;
		}
		if( changed ) {
			EngineCore.outputText( '\n\n' );
		...
		if( CoC.getInstance().player.vaginas.length > 0 ) {
			if( CoC.getInstance().player.vaginas[ 0 ].vaginalWetness >= AppearanceDefs.VAGINA_WETNESS_SLICK ) {
				EngineCore.outputText( 'The constant fluids leaking from your ' + Descriptors.vaginaDescript( 0 ) + ' slow down, then stop.  ' );
		...
		//Nothing changed;
		if( !changed ) {
			EngineCore.outputText( 'You tingle briefly but feel no obvious change.  Your crotch isn\'t really in need of becoming more human.' );
		}
		this.omnibusVictoryPostBoon();
	};
	DungeonCore.prototype.omnibusVictoryNormalLegs = function() {
		EngineCore.spriteSelect( 16 );
		EngineCore.clearOutput();
		if( CoC.getInstance().player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_HUMAN ) {
			...
			EngineCore.outputText( 'You collapse as your ' + CoC.getInstance().player.legs() + ' are unable to support you.  The sounds of bones breaking and reshaping fills the room, but oddly you feel no pain, only mild arousal.  You blink your eyes and sigh, and when you look down again <b>you have normal human legs</b>!' );
		}
		...
		if( CoC.getInstance().player.tailType > AppearanceDefs.TAIL_TYPE_NONE ) {
			EngineCore.outputText( '  A moment later, your feel something detach from above your backside.  <b>You no longer have a tail!</b>' );
	...
	DungeonCore.prototype.omnibusVictoryPostBoon = function() {
		...
		CoC.getInstance().player.createPerk( PerkLib.OmnibusGift, 0, 0, 0, 0 );
		CoC.getInstance().player.orgasm();
		EngineCore.dynStats( 'cor', 2 );
		Combat.cleanupAfterCombat();
	};
	...
	DungeonCore.prototype.omnibusVictoryKillHer = function() {
		...
		CoC.getInstance().flags[ kFLAGS.D1_OMNIBUS_KILLED ] = 1;
		Combat.cleanupAfterCombat();
	};
	DungeonCore.prototype.omnibusLossRape = function() {
		...
		if( CoC.getInstance().player.HP < 1 ) {
			...
			EngineCore.outputText( 'Forgetting about the fight, you lean on the desk and slide your free hand under your ' + CoC.getInstance().player.armorName + ', seeking any pleasure you can get.  ', false );
		}
		EngineCore.outputText( 'Sensing victory, the demoness sidles up next to you and pushes you into a chair.  Incapable of resisting, all you can do is watch as she opens your ' + CoC.getInstance().player.armorName + ' for easier access.  She steps back and admires her handiwork as she gives you a thorough looking over.\n\n', false );
		EngineCore.outputText( '"<i>I have just the thing for a ', false );
		if( CoC.getInstance().player.gender <= 1 ) {
			EngineCore.outputText( 'man', false );
		} else {
			EngineCore.outputText( 'woman', false );
		}
		EngineCore.outputText( ' such as you.  I\'ve been crossbreeding the parasites that developed in the deep jungle, trying to create the PERFECT slave-maker.  You get to be my first test subject,</i>" she says.\n\n', false );
		EngineCore.outputText( ' She sees the look of fear creeping into your eyes and pats you comfortingly, "<i>Awww don\'t worry. It\'ll feel REALLY good.  If anything you should feel honored to be assisting an Omnibus in her experiments.</i>"\n\n', false );
		EngineCore.outputText( ' She opens one of the desk drawers, and searches briefly before her eyes light up with recognition.  "<i>Here we are,</i>" she says as she pulls something free...', false );
		EngineCore.doNext( this.omnibusLossRape2 );
	};
	DungeonCore.prototype.omnibusLossRape2 = function() {
		...
		if( CoC.getInstance().player.cocks.length > 1 ) {
			...
			EngineCore.outputText( ', sliding up your abdomen, and oozing down over your hips.  As it spreads the colors fill more and more of your head, clearing away your thoughts of resistance.\n\n', false );
			EngineCore.outputText( 'A soothing female voice talks to you from somewhere, "<i>Did I mention it\'s specifically tuned to ensnare the conscious mind with it\'s pretty colors?  I must have forgot.  Well, I see you\'ve discovered it on your own.  The colors are just so perfect for opening your mind to me, aren\'t they?  They just chase away your thoughts and let my words slip deep into your subconscious.  I bet it feels nice to just focus on the colors and let my pet tease your cocks, doesn\'t it?</i>"\n\n', false );
			EngineCore.outputText( 'You nod without any awareness of the act.\n\nThe voice laughs and continues while the creature reaches around your ' + Descriptors.hipDescript() + ' and slides a feeler between your cheeks, completing the tight loop around your groin, "<i>That\'s good.  You want to let the creature cover as much of you as it wants.  Being sex-food for a symbiotic plant is arousing beyond measure.</i>"  You feel the creature licking at your ' + Descriptors.assholeDescript() + ' until it relaxes, and then slides something inside.  A warm wetness spreads through your bowels as something begins caressing your prostate from inside you.\n\n', false );
			EngineCore.outputText( 'Overloaded with pleasure, you feel your ' + CoC.getInstance().player.multiCockDescriptLight() + ' pulse and cum, creating translucent green cum balloons the size of ', false );
			if( CoC.getInstance().player.cumQ() < 50 ) {
				EngineCore.outputText( 'apples ', false );
			}
			if( CoC.getInstance().player.cumQ() >= 50 && CoC.getInstance().player.cumQ() < 300 ) {
				EngineCore.outputText( 'cantaloupes ', false );
			}
			if( CoC.getInstance().player.cumQ() >= 300 ) {
				EngineCore.outputText( 'watermelons ', false );
			}
			EngineCore.outputText( 'at the end of each of your dicks.  The creature\'s flashing intensifies while your hips quake uncontrollably, pumping the last of your load feeds into the wonderful plant.  The light-show grows brighter, totally emptying any remaining stray thoughts and leaving you feeling wonderfully open.\n\n', false );
			EngineCore.outputText( '"<i>Being used for your cum is great,</i>" says the voice and you agree, it is great.\n\n', false );
			EngineCore.outputText( '"<i>Your greatest fetish is allowing demonic creatures to feed on your cum,</i>" she says, and it feels so right.  Your cum is meant for demons and plants to feast on.  Just the thought makes you want to orgasm again.\n\n', false );
			EngineCore.outputText( '"<i>Since you provide food-source, that must make you livestock.  You like being livestock.  Livestock don\'t have to think.  Livestock follow orders.  Best of all, as livestock you can live your favorite fetish of being milked of all your cum, every hour of every day,</i>" the voice says, filling your mind with new thoughts.  Of course it\'s right, you can just let a demon or tentacle plant milk you and do all the hard stuff, like thinking.  All you have to do is cum.  The thought makes you shiver as the plant-suit absorbs the encapsulated bubbles of jizz.  The dazzling lights grow even brighter as it takes in the nutrients.\n\n', false );
			EngineCore.outputText( '*FLASH* "<i>You want to cum for the plant.</i>"\n\n', false );
			EngineCore.outputText( 'Tendrils of plant crawl up your belly, coating you in slime as they massage every inch of you.\n\n', false );
			EngineCore.outputText( '*FLASH* "<i>You need to cum for the plant.</i>"\n\n', false );
			if( CoC.getInstance().player.breastRows.length === 1 ) {
				EngineCore.outputText( 'They reach the lower curve of your breasts.\n\n', false );
			}
			if( CoC.getInstance().player.breastRows.length > 1 ) {
				EngineCore.outputText( 'They slide over your lowest pair of breasts, encapsulating them in wriggling tightness.\n\n', false );
			}
			EngineCore.outputText( '*FLASH* "<i>You love cumming for anything and anyone.</i>"\n\n', false );
			if( CoC.getInstance().player.breastRows.length === 3 ) {
				EngineCore.outputText( 'Your middle breasts tingle with absolute pleasure as they too become engulfed in tightness.\n\n', false );
			}
			if( CoC.getInstance().player.breastRows.length === 2 ) {
				EngineCore.outputText( 'You groan as the plant grows up the summit of your top breasts, coating the bottom half of your aureola.\n\n', false );
			}
			if( CoC.getInstance().player.breastRows.length === 1 ) {
				EngineCore.outputText( 'Your ' + Descriptors.nippleDescript( 0 ) + 's become hard as steel as the wave of slick pleasure washes over them.\n\n', false );
			}
			EngineCore.outputText( '*FLASH* "<i>You love being told to orgasm.</i>"\n\n', false );
			if( CoC.getInstance().player.breastRows.length === 1 ) {
				EngineCore.outputText( 'The wriggling mass slides up the top-most parts of your breasts, narrowing into two tiny tendrils that loop around your neck.\n\n', false );
			}
			if( CoC.getInstance().player.breastRows.length >= 2 ) {
				EngineCore.outputText( 'The wriggling mass climbs your top pair of breasts with ease, wrapping your diamond-hard nipples in slime and sensation.  It continues climbing upward, narrowing into two bands that loop around the back of your neck.\n\n', false );
			}
			EngineCore.outputText( '*FLASH* "<i>To orgasm is to obey.  You love to orgasm.  You love to obey.  You love to obey my voice more than any other.  Obeying my voice gave you these orgasms.  Since you love to obey me, you must be my pet.</i>"\n\n', false );
			EngineCore.outputText( 'Your mistress\' OTHER pet wraps around your neck, forming a choker comprised of shifting green colors.  You smile as you realize it is done - you\'ve become one of her pet cattle.  Your body is wrapped in an emerald sea of shifting pleasure, just like your mistress wanted.  If it weren\'t for the obvious bulges of your ' + CoC.getInstance().player.multiCockDescriptLight() + ', you\'d look to be wearing an extraordinarily revealing one piece swim-suit.  The constant teasing at your crotch continues, and you stay rock-hard, even though you just came.  The idea of being milked to feed your new clothing just turns you on so much that you bet you\'re leaking constant streams of pre-cum for your new green master.\n\n', false );
			EngineCore.outputText( 'The flashing subsides, and your new thoughts rush into the void.  You immediately begin masturbating your encapsulated members as you seek to obey.  To orgasm is to obey.  To obey is to orgasm.  You discover that you can feel every touch through the skin of your \'clothing\'.  You increase the tempo, knowing that your orgasm will be feeding the creature that now lives on you, fulfilling your deepest darkest desires.  You cum again, just as hard as before, inflating ' + Utils.num2Text( CoC.getInstance().player.cocks.length ) + ' shiny green balloons with the proof of your obedience.\n\n', false );
			if( CoC.getInstance().player.findStatusAffect( StatusAffects.CampMarble ) >= 0 ) {
				EngineCore.outputText( 'Suddenly, a loud scream is heard down on the factory floor. You and your mistress turn to see Marble dashing up the stairs to the foremen\'s office.  Your mistress looks over at her and says with some amusement, "<i>Oh ho!  So another cow has come to join in the fun.</i>"\n\n"<i>Sweetie! What has she done to you?</i>" Marble exclaims, "<i>What has she put on you?!</i>"\n\n"<i>Oh, so you knew this girl?</i>" your mistress asks you, "<i>It\'s a Lacta Bovine from the looks of it, so it seems this time I\'ll be adding a real cow to the pens.</i>"  Marble turns to your mistress and brandishes her hammer, but the horror from the thought of your mistress being hurt causes you to spring forward and grab Marble.  The brief distraction gives your mistress a chance to sink a syringe into Marble\'s shoulder, and within moments she slumps onto the ground unconscious."\n\n', false );
				EngineCore.outputText( 'Your mistress turns back to you and smiles.\n\n"<i>Well, she should make a fine replacement for you in the pens,</i>" she says before tapping her chin thoughtfully and looking back at you, "<i>Really is convenient that I don\'t have to worry about my new pet dying on me now, hun.</i>"  Then she pushes you back into the chair and says "<i>But first..."\n\n', false );
			} else {
				EngineCore.outputText( 'Your mistress looks down with approval and speaks, "<i>Very good.  ', false );
			}
		...
		//Dick version;
		if( CoC.getInstance().player.cocks.length === 1 ) {
			EngineCore.outputText( 'In her hand is a squirming purplish mass.  It has a smooth outer surface, spotted with dark shades of iridescent purple. The opposite side is comprised of a smooth mucusy membrane covered with wriggling pink cilia.\n\n', false );
			EngineCore.outputText( 'She leans over you with a predatory smile, "<i>This little guy is my favorite.  I\'ve even given him a bit of \'field testing\'.</i>"  She gestures towards a small dripping orifice, explaining, "<i>You see, once I put this on you, it\'ll open up niiice and wide.  It\'ll suck your nice little cock into its mouth and starting squeezing and massaging you with each of its tiny tentacles until you can\'t help but release all your ', false );
			if( CoC.getInstance().player.cor < 33 ) {
				EngineCore.outputText( 'sweet ', false );
			}
			if( CoC.getInstance().player.cor >= 66 ) {
				EngineCore.outputText( 'tainted ', false );
			}
			EngineCore.outputText( 'sexual energies deep into its gullet.  And that\'s just the start!</i>"  Her hands let go of the squirming mass, dropping it squarely into your lap.\n\n', false );
			if( CoC.getInstance().player.averageCockLength() < 15 ) {
				EngineCore.outputText( 'With one swift motion, the beast engulfs your ' + Descriptors.cockDescript( 0 ) + ' in its slimy maw.  ', false );
			} else {
				EngineCore.outputText( 'Distending obscenely, the beast starts engulfing your ' + Descriptors.cockDescript( 0 ) + ' in its slimy maw, progressing along its entire length until you can no longer see your pleasure tool.  ', false );
			}
			EngineCore.outputText( 'The slimy tentacles waste no time, massaging you with mechanical precision.  You groan in helpless pleasure, growing to painful hardness within the squirming confines of the creature.  Three protrusions sprout from the creature\'s core, dripping with slime of their own, and covered on the inside with the same wriggling protrusions that now massage your trapped member.  Two curl around your ' + Descriptors.hipDescript() + ', while the last one', false );
			if( CoC.getInstance().player.balls > 0 ) {
				EngineCore.outputText( ' smothers your balls, entrapping them in sticky sensation as it continues across your taint between your butt-cheeks.  ', false );
			} else {
				EngineCore.outputText( ' journeys over your taint before travelling between your butt-cheeks.  ', false );
			}
			EngineCore.outputText( 'The three tendrils join together in the back, forming a seemless tiny purple triangle.  It really rides up high, tickling your ' + Descriptors.assholeDescript() + ' with constant teasing.  You\'re wearing an organic purple thong!\n\n', false );
			EngineCore.outputText( 'You try to endure, but the humiliation is too much for you to take.  The pleasure and shame push you past your limit.  You let out a squeal of mixed agony and delight as the proof of your pleasure boils out into the creature.  You pant and twitch, helpless to resist the strength of your orgasm as your jism fills the creature, distorting it visibly around your member.  Sighing, you relax as the assault winds down, the squirming tentacles relaxing noticeably as they work to digest their \'meal\'.\n\n', false );
			EngineCore.outputText( '"<i>Enjoy yourself?  The best part is about to start,</i>" she says with an evil glint in her eye.  You sit bolt upright as your living thong squirms and shifts, pressing something rigid against the ring of your ' + Descriptors.assholeDescript() + '.  You reach down, trying to pull the creature off, but its outer covering is surprisingly hard, and seals almost perfectly against your ' + CoC.getInstance().player.skinDesc + '.  You look up with terror in your eyes, a pleading look painted across your face.\n\n', false );
			EngineCore.outputText( 'She cocks her head to the side with an inquisitive look and asks, "<i>So it\'s found your back door I take it?</i>"  You nod sheepishly, squealing as the rigid growth pushes through your sphincter, violating you completely.  She continues with a nonchalant tone, though her eyes seem to be drinking in the scene, "<i>That thing you feel drilling into your ass?  It\'s a carefully evolved injection appendage.  Don\'t worry, once it settles in it won\'t move much.  It\'s just going to get nice and cozy with your prostate and a few major blood vessels.  Then it\'s going to reward you for cumming!</i>"\n\n', false );
			EngineCore.outputText( 'You feel it burrow a little deeper, and then curve up.  It presses against something inside of you in a way that makes your ' + Descriptors.cockDescript( 0 ) + ' twitch uncontrollably.  You\'re sure that if it weren\'t for the greedy tentacle-panties around your dick you would\'ve seen a huge dollop of pre-cum squeeze out.  Filled with angst and worry as to what is to come, you ask, "<i>Ummm, h-h-how is it going to reward me?</i>"\n\n', false );
			EngineCore.outputText( 'She winks, petting the mottled surface of your purple-cock-prison as you feel a sensation of warm wetness in your backside.  At the same time you nearly jump as you feel a painful pinch in your prostate.  The demoness licks her lips and answers, "<i>Well, it rewards you in two ways pet.  One:  It empties a specially designed cocktail of drugs directly into your bowels, where they\'ll be absorbed slowly into the body.</i>"  As if on cue a gentle warmth spreads through your torso, radiating out into your limbs, and settling like calming mist in your head.  You relax utterly, enjoying the feeling in spite of your worries.\n\n', false );
			EngineCore.outputText( 'She coos, petting your still-hard member and the creature around it.  Miraculously you can feel both the touch of her silky fingers and the constant pleasurable squirming of the panties themselves.  You twitch your ' + Descriptors.cockDescript( 0 ) + ' against her hand, giggling happily.\n\n', false );
			EngineCore.outputText( '"<i>That\'s right, it\'s a good reward isn\'t it?</i>" she asks as she continues to fondle you, "<i>those drugs are making you docile and extraordinarily suggestible.  For instance – every time I talk you can feel my hands caressing and fondling your member', false );
			if( CoC.getInstance().player.balls > 0 ) {
				EngineCore.outputText( ' and teasing your balls', false );
			}
			EngineCore.outputText( '.  You see?  I\'m not even touching you anymore and you\'re still twitching.  My my, what an obedient slave you\'re going to be.</i>"\n\n', false );
			EngineCore.outputText( 'You pant and groan while she talks to you, still feeling the combined efforts of the panty-creature and your master\'s wonderful hands, "<i>And I haven\'t even told you about the second part of your reward.  If you want me to tell you, you\'ll need to admit out loud what we both already know – that you\'re my obedient slave-toy.  Say it toy.</i>"\n\n', false );
			EngineCore.outputText( '"<i>I-I-I\'m your obedia—ahhh-nt s-s-lave toy,</i>" you moan.  As soon as the words leave your mouth, you know it\'s true, but that settles in the back of your mind.  You\'re eager to know how the creature and your mistress will reward you for being such an obedient-toy.  And of course, to get her talking again so you can feel those smooth fingertips caress you once more.\n\n', false );
			EngineCore.outputText( '"<i>You really are my good toy already, aren\'t you?</i>" she muses, "<i>You just love pleasing, me don\'t you toy?</i>"  You nod feverishly, eliciting a happy laugh from your mistress as she lectures you, "<i>The second part of your reward is an injection of its venom directly into your prostate.  You may not have noticed with the constant teasing your cock is enduring, but by now your prostate should have doubled in size.  If I ever separate you and your training-suit, you\'ll notice you\'re producing so much pre-cum that it\'s dribbling out ALL the time.  Your orgasms won\'t get much bigger, but you\'ll find yourself pouring out pre as you get more and more turned on.  After all, my baby here needs to feed.</i>"\n\n', false );
			EngineCore.outputText( 'Your mistress pats your obscene purple panties tenderly and whispers in your ear, "<i>Be a good toy and cum for mistress.</i>"  You smile broadly as your hips piston in the air, as if fucking an imaginary twat.  Cum boils out from your ', false );
			if( CoC.getInstance().player.balls > 0 ) {
				EngineCore.outputText( Descriptors.ballsDescriptLight() + ' and ', false );
			}
			EngineCore.outputText( ' over-sized prostate, filling the chamber around your cock with thick blasts of seed.  You smile happily as the tentacle-chamber distorts to hold your load, bulging out into a more spherical appearance.  You slump down as your orgasm finishes and you begin to feel even more \'reward\' fill your now greedy-hole.\n\n', false );
			if( CoC.getInstance().player.findStatusAffect( StatusAffects.CampMarble ) >= 0 ) {
				EngineCore.outputText( 'Suddenly, a loud scream is heard down on the factory floor. You and your mistress turn to see Marble dashing up the stairs to the foremen\'s office.  Your mistress looks over at her and says with some amusement, "<i>Oh ho!  So another cow has come to join in the fun.</i>"\n\n"<i>Sweetie! What has she done to you?</i>" Marble exclaims, "<i>What has she put on you?!</i>"\n\n"<i>Oh, so you knew this girl?</i>" your mistress asks you, "<i>It\'s a Lacta Bovine from the looks of it, so it seems this time I\'ll be adding a real cow to the pens.</i>"  Marble turns to your mistress and brandishes her hammer, but the horror from the thought of your mistress being hurt causes you to spring forward and grab Marble.  The brief distraction gives your mistress a chance to sink a syringe into Marble\'s shoulder, and within moments she slumps onto the ground unconscious."\n\n', false );
		...
		//(Female) ;
		if( CoC.getInstance().player.vaginas.length === 1 || CoC.getInstance().player.gender === 0 ) {
			EngineCore.outputText( 'In her hand is a seamless pair of panties.  Their surface reflects light perfectly, as if its bright pink surface were coated in slippery oil or made from latex.  ', false );
			if( CoC.getInstance().player.gender === 0 ) {
				EngineCore.outputText( 'The demoness smiles with wicked intent and yanks the bottoms of your ' + CoC.getInstance().player.armorName + ' the rest of the way off.  Your lack of genetalia does not faze her, and she responds by swiftly pulling out a needle and injecting your groin.  In seconds your crotch splits open, revealing a fresh virgin vagina.  Licking her perfect lips with anticipation, she inverts the panties and holds them up for you to see.\n\n', false );
				CoC.getInstance().player.createVagina();
			} else {
				EngineCore.outputText( 'The demoness smiles with wicked intent and yanks your ' + CoC.getInstance().player.armorName + '\'s bottom the rest of the way off.  She leans close, smiling and inhaling the scent of your sex, savoring it like a aroma of a fine wine.  Licking her perfect lips with anticipation, she inverts the panties and holds them up for you to see.\n\n', false );
			}
			EngineCore.outputText( 'They aren\'t panties at all, but instead some living creature.  The entire inside surface of the living garment is covered with fleshy pink nodules that wriggle constantly, practically dripping with a pungent lubricant that smells not unlike your own juices.  Horrifyingly, there is a large lump of flesh towards the front.  Its surface is ribbed and pulses, constantly swelling and shrinking.  It\'s clearly designed to enter the passage of anyone who wears it.  Worse yet is a smaller narrower protrusion on the backside.  This... creature... will certainly do its best to plug both your holes.\n\n', false );
			EngineCore.outputText( 'Your captor pulls it back and leans closer, letting the scent of her own fragrant puss fill the air.  It smells tangy and sweet and makes you ', false );
			if( CoC.getInstance().player.vaginas[ 0 ].vaginalWetness <= AppearanceDefs.VAGINA_WETNESS_WET ) {
			...
			EngineCore.outputText( 'It takes less than a second for you to moan out, "<i>Taaaaake meeeee!</i>"\n\n', false );
			EngineCore.outputText( 'She smiles and lifts you up from the chair with her strong arms, and sits down on the desk.  She lowers your symbiote-covered lips onto her demon dick and coos with delight as you slide down her length, taking the entire thing in your greedy depths.  If anything, the creature inside you makes sex even better - you feel a combination of her nub-covered cock fucking you and the ridges of the panty-creature as it continues to stimulate you.  It drives you mad with pleasure, and you begin bouncing yourself up and down, spearing your ' + Descriptors.vaginaDescript( 0 ) + ' with the demon\'s pole.\n\n', false );
			EngineCore.outputText( 'She giggles and reaches down.  Too aroused to care, you just keep fucking her and moaning in delight.  Her hands come up and begin to massage and rub your ' + Descriptors.allBreastsDescript() + ' taking special care to tweak and tug on your nipples.  They become as hard as ', false );
			if( CoC.getInstance().player.nippleLength < 0.5 ) {
				EngineCore.outputText( 'erasers ', false );
			} else if( CoC.getInstance().player.nippleLength < 3 ) {
				EngineCore.outputText( 'bullets ', false );
			} else {
				EngineCore.outputText( 'tiny cocks ', false );
			}
			EngineCore.outputText( 'in moments', false );
			if( CoC.getInstance().player.biggestLactation() > 2 ) {
				EngineCore.outputText( ' and start to drip with milk', false );
			}
			EngineCore.outputText( '.  You sigh with disappointment when her hands drop away.  You were so close to orgasm.  She reaches back up and places something wet and warm on ', false );
			if( CoC.getInstance().player.breastRows.length <= 1 ) {
				EngineCore.outputText( 'your ' + Descriptors.nippleDescript( 0 ), false );
			}
			if( CoC.getInstance().player.breastRows.length > 1 ) {
				EngineCore.outputText( 'your top ' + Descriptors.nippleDescript( 0 ), false );
			}
			EngineCore.outputText( '.  You look down and see two star-shaped creatures made of glistening pink (just like your panties!) attached to your ' + Descriptors.nippleDescript( 0 ) + 's.  They pulse and ripple as they constantly massage and suck.  ', false );
			if( CoC.getInstance().player.biggestLactation() > 1 ) {
				EngineCore.outputText( 'Your milk erupts, spraying out from a tiny hole in the center of the star.  In response the creature increases the force of its sucking action, making you fountain milk constantly.  ', false );
			}
			if( CoC.getInstance().player.breastRows.length > 1 ) {
				EngineCore.outputText( 'While you continue to fuck that meat pole and watch the creatures squirming on your nipples, more are affixed to each of your remaining ' + Descriptors.nippleDescript( 0 ) + 's, until every single one is covered with its own tiny pleasure-creature.\n\n', false );
			}
			EngineCore.outputText( 'A flood of hot demonic spunk unloads into your aching ' + Descriptors.vaginaDescript( 0 ) + ', filling you with warmth.  The panty-plug in your backside seems to erupt as well, dumping a flood of undiluted aphrodisiacs into your body.  You squeal and cum, your eyes rolling back with pleasure as you sink down on the quivering member of your captor.  You twitch and moan, orgasming for far longer than the dick inside of you.  The pleasure goes on for minute after minute.  Your ' + Descriptors.nippleDescript( 0 ) + 's each radiate satisfaction and pleasure as they manage to provide you with miniature orgasms of their own.  You moan, feeling relief at last and losing yourself in the wave of satisfaction that fills your body.\n\n', false );
	...
	DungeonCore.prototype.demonBadEnd = function() {
		EngineCore.outputText( '', true );
		EngineCore.outputText( 'Advancing slowly, the succubus gives you a hungry look.  She extends a hand, allowing her fingers to lengthen into razor-sharp claws.  With a single expert slash, she cuts away everything holding together your ' + CoC.getInstance().player.armorName + '.  They fall into a discarded pile, already forgotten as your ', false );
		//[genderless];
		if( CoC.getInstance().player.gender === 0 ) {
			EngineCore.outputText( 'entire body blushes read before the sexy seductress.  She looks at you, frowning as she murmers, "<i>Now this just won\'t do.  You look more like a ' + CoC.getInstance().player.boyGirl() + ' to me, so why don\'t I make the plumbing match the exterior?</i>"\n\n', false );
			EngineCore.outputText( 'Her palm caresses your crotch, stoking the warmth inside you until it blazes white-hot with new sensation.  Your skin ripples and parts, ', false );
			if( CoC.getInstance().player.biggestTitSize() <= 1 ) {
				EngineCore.outputText( 'pushed apart the thick flesh of a powerful demonic member, complete with two swollen balls.', false );
				...
				EngineCore.outputText( 'gushing with fluids as it shapes itself into a hungry demonic cunt.', false );
		...
		if( CoC.getInstance().player.gender === 1 ) {
			//Multispoooooo;
			if( CoC.getInstance().player.cockTotal() > 1 ) {
				EngineCore.outputText( CoC.getInstance().player.multiCockDescriptLight() + ' pulsate, straining for just a touch of the succubus\' hand.  She paces around you, giggling and toying with you as your ' + CoC.getInstance().player.multiCockDescript() + ' seem to follow her, twitching and thickening any time she takes a step closer.\n\n', false );
				EngineCore.outputText( 'She reaches out, cupping the underside of a shaft, slowly stroking your most sensitive places while she stops the bimbo-like voice and teases, "<i>Awww, so hard and ready.  It looks to me like you\'re already a slave to your desires.  You\'re twitching and dripping, just from the soft touches of your enemy\'s fingers.  Are you truly so in need of release as to willingly offer it to me?  No, don\'t answer, your ' + Descriptors.cockDescript( 0 ) + ' already has.</i>"\n\n', false );
				EngineCore.outputText( 'You glance down, seeing just how hard her words have made you.  You squirm your ' + Descriptors.hipDescript() + 's pathetically, trying to hump her hand and increase the stimulation.  The succubus immediately releases you and draws back, shedding her secretary\'s clothes like a snake sheds its skin.  Now clad only in a tight leather corset and thigh-high stockings with garters, the succubus tosses you onto a table, surprising you with her raw strength.  Seemingly from nowhere, she produces a whip, winding it tightly around ', false );
				if( CoC.getInstance().player.balls > 0 ) {
					EngineCore.outputText( 'your ' + Descriptors.ballsDescriptLight() + ' and ', false );
				}
				EngineCore.outputText( 'the bases of your ' + CoC.getInstance().player.multiCockDescriptLight() + '.\n\n', false );
				EngineCore.outputText( '"<i>There, that\'ll make sure you feel every squeeze and caress of my velvet walls, and keep you from getting off until you\'re ready,</i>" says the succubus as she climbs the table and straddles your eager form.\n\n', false );
				EngineCore.outputText( 'She lifts herself up with her shapely legs and spreads her thighs, proudly revealing her puffy pierced folds.  They drip with demonic nectar as she wiggles over you, spattering you with demon-honey.  Slowly, nearly imperceptibly, she swivels the lewd opening closer and closer, and your cocks, as if possessed, angle themselves upward towards the juicy target.  The grinning succubus looks over her shoulder and asks, "<i>Ready are we? Ok, I won\'t keep you waiting.</i>"\n\n', false );
				EngineCore.outputText( 'Marvelous heat and wetness sweeps over you in a fluid motion, wrapping your ' + Descriptors.cockDescript( 0 ) + ' tightly.  You sigh happily, already lost in the feeling of having a succubus\' tight walls wriggling around you.  Were you not already so corrupt, you would probably be coming already, but as it is, you can lie there and enjoy it, reveling in the sensations your unholy lover is spreading through your body.  You shiver, finally approaching your climax, but as it nears you find yourself denied by the whip binding your ' + CoC.getInstance().player.multiCockDescriptLight() + '.  It isn\'t just the physical tightness either – something else about it keeps your release buried deep, inaccessible.\n\n', false );
				EngineCore.outputText( '"<i>Have you hit it yet?</i>" the succubus asks as she rocks on top of you, "<i>I\'ve placed a block inside you.  Don\'t worry, it\'s temporary, it\'ll only stop you from orgasming for a few days...</i>"\n\n', false );
				EngineCore.outputText( 'You moan pitifully, begging for her to remove it and allow you to cum.\n\n', false );
				EngineCore.outputText( '"<i>Oh I can\'t remove it,</i>" she says, "<i>The only way you\'ll be rid of it with any sort of certainty is to melt through it with something stronger.  Something, like, I don\'t know, the focused remains of your soul and humanity.  Now you think on that while I melt away any doubts you might have.</i>"\n\n', false );
				EngineCore.outputText( 'She resumes fucking you, driving you insane with need, all the while fiddling with her clit and pulling up a nipple to lick.  It feels so good, but you NEED to cum.  She fucks you like that for hours, until the table collapses under the pair of you and dumps you both on the floor. More than anything you crave release, and over time you cave in further and further to the need.  In time, you can feel the block weakening, melting, and eroding.  Your life has been distilled down into this one moment, this one desire, this need for release.  The block shatters, melting away under the force of your need.\n\n', false );
				EngineCore.outputText( 'A look of shock and pleasure spreads over the succubus\'s face as you release into her hot snatch, cumming with a force unlike anything you\'ve felt before.  Her walls squeeze and caress in time with your orgasm, milking you of every drop.  Your body clenches and squeezes, shuddering as the orgasm continues for far longer than normal.  Though you don\'t feel like you\'re pushing out as much fluid as normal, somehow it feels even better, like a slow drip of pleasure and release.  When at last your ' + Descriptors.cockDescript( 0 ) + ' empties, you feel drained and strangely energized at the same time, and your entire torso is spattered with rapidly hardening pink crystals.\n\n', false );
				EngineCore.outputText( 'The slutty succubus stands up, her puffy vulva coated in a shining pink fluid.  Did that just come out of you?  She grunts, her eyes glowing for a moment as the pink goop disappears into her skin, vanishing entirely.\n\n', false );
				EngineCore.outputText( '"<i>Ahhhhh,</i>" she sighs, "<i>nothing like fresh Lethicite.  Mmmm, yours was potent!</i>"\n\n', false );
				EngineCore.outputText( 'You stand up, dissatisfied at the sudden lack of sensation you\'re forced to endure.  The gloating demoness looks rather pleased with herself, and brimming with newfound power.  You resolve to ', false );
				if( CoC.getInstance().player.findStatusAffect( StatusAffects.MaraesLethicite ) < 0 ) {
					EngineCore.outputText( 'gather some yourself at the next opportunity...', false );
				} else {
					EngineCore.outputText( 'devour Marae\'s as soon as you get a chance.', false );
				}
				EngineCore.outputText( 'You greedily gather up the lethicite splattered on your body and devour it, turning it into raw demonic power.  You really do need to get more of this... but first you know a certain demoness you intend to wrap around your ', false );
				if( CoC.getInstance().player.demonCocks() > 0 ) {
					EngineCore.outputText( 'growing', false );
				} else {
					EngineCore.outputText( 'new', false );
				}
				EngineCore.outputText( ' demon-cock for a few more orgasms.', false );
			}
			//SINGLEZ NITE;
			else {
				EngineCore.outputText( CoC.getInstance().player.multiCockDescriptLight() + ' pulsates, straining for just a touch of the succubus\' hand.  She paces around you, giggling and toying with you as your ' + CoC.getInstance().player.multiCockDescript() + ' seems to follow her, twitching and thickening anytime she takes a step closer.\n\n', false );
				EngineCore.outputText( 'She reaches out, cupping the underside of the shaft, slowly stroking your most sensitive places while she stops the bimbo-like voice and teases, "<i>Awww, so hard and ready.  It looks to me like you\'re already a slave to your desires.  You\'re twitching and dripping, just from the soft touches of your enemy\'s fingers.  Are you truly so in need of release as to willingly offer it to me?  No, don\'t answer, your ' + Descriptors.cockDescript( 0 ) + ' already has.</i>"\n\n', false );
				EngineCore.outputText( 'You glance down, seeing just how hard her words have made you.  You squirm your ' + Descriptors.hipDescript() + 's pathetically, trying to hump her hand and increase the stimulation.  The succubus immediately releases you and draws back, shedding her secretary\'s clothes like a snake sheds its skin.  Now clad only in a tight leather corset and thigh-high stockings with garters, the succubus tosses you onto a table, surprising you with her raw strength.  Seemingly from nowhere, she produces a whip, winding it tightly around ', false );
				if( CoC.getInstance().player.balls > 0 ) {
					EngineCore.outputText( 'your ' + Descriptors.ballsDescriptLight() + ' and ', false );
				}
				EngineCore.outputText( 'the base of your ' + Descriptors.cockDescript( 0 ) + '.\n\n', false );
				EngineCore.outputText( '"<i>There, that\'ll make sure you feel every squeeze and caress of my velvet walls, and keep you from getting off until you\'re ready,</i>" says the succubus as she climbs the table and straddles your eager form.\n\n', false );
				...
				EngineCore.outputText( 'The slutty succubus stands up, her puffy vulva coated in a shining pink fluid.  Did that just come out of you?  She grunts, her eyes glowing for a moment as the pink goop disappears into her skin, vanishing entirely.\n\n', false );
				EngineCore.outputText( '"<i>Ahhhhh,</i>" she sighs, "<i>nothing like fresh Lethicite.  Mmmm your\'s was potent!</i>"\n\n', false );
				EngineCore.outputText( 'You stand up, dissatisfied at the sudden lack of sensation you\'re forced to endure.  The gloating demoness looks rather pleased with herself, and brimming with newfound power.  You resolve to ', false );
				if( CoC.getInstance().player.findStatusAffect( StatusAffects.MaraesLethicite ) < 0 ) {
					EngineCore.outputText( 'gather some yourself at the next opportunity...', false );
				} else {
				...
				EngineCore.outputText( '  But first you know a certain demoness you intend to wrap around your ', false );
				if( CoC.getInstance().player.demonCocks() > 0 ) {
					EngineCore.outputText( 'growing', false );
				} else {
					EngineCore.outputText( 'new', false );
		...
		//[female];
			...
			EngineCore.outputText( Descriptors.vaginaDescript( 0 ) + ' grows wet and ready, practically juicing itself as the demoness\' hand caresses your inner thigh.  She teases, "<i>Oh my! You\'re so wet and ready and I haven\'t even touched your moist little cum-receptacle.  You\'re a slut aren\'t you?  Who else would be so turned on by the idea of cumming until all your humanity drips out?</i>"\n\n', false );
			EngineCore.outputText( 'The words make you blush hard, shaming you and stoking the growing fire between your ' + CoC.getInstance().player.legs() + '.  You know two things for certain: she\'s right and you\'re more turned on that ever.  You don\'t resist as the demoness easily lifts you up, setting you down on a table with your ' + CoC.getInstance().player.legs() + ' spread.  "<i>There,</i>" she comments, "<i>now your juicy snatch is on display, just like you\'ve always wanted.</i>"\n\n', false );
			EngineCore.outputText( 'She effortlessly swings her lissomelegs onto the table as she pulls herself up, mounting you as a man might.  You can feel waves of heat rolling off her sex, bathing your own slit in her warmth.  ', false );
			if( CoC.getInstance().player.clitLength >= 2 ) {
				EngineCore.outputText( 'Your ' + Descriptors.clitDescript() + ' pushes free, nuzzling against her hairless cunt and slipping inside, as if drawn in by its desire.  She openly moans, and begins rocking on top of you.  You gasp in delight as she rides your ' + Descriptors.clitDescript() + ', fucking and grinding against it.  ', false );
			} else {
				EngineCore.outputText( 'She lowers herself down, rubbing smooth hairless netherlips over you, smearing you with her fragrant demon-honey.  You feel her clit grinding on your own, drawing out gasps of delight from both of your mouths as she relentlessly scissors against you.  ', false );
			}
			EngineCore.outputText( 'In no time flat you feel your climax building.  Your ' + Descriptors.vaginaDescript( 0 ) + ' quivers and grows wetter in anticipation.  Tugging on your aching ' + Descriptors.nippleDescript( 0 ) + 's and aching for release, you squirm under your demonic mistress, smashing your ' + Descriptors.vaginaDescript( 0 ) + ' against her in a lewd attempt to find your orgasm.  It does not happen, and you moan in disappointment as the pleasure continues to build, oblivious to your desire for orgasm.\n\n', false );
			EngineCore.outputText( '"<i>Have you hit it yet?</i>" the succubus asks as she rocks on top of you, "<i>I\'ve placed a block inside you.  Don\'t worry, it\'s temporary, it\'ll only stop you from orgasming for a few days...</i>"\n\n', false );
			EngineCore.outputText( 'You moan pitifully, begging for her to remove it and allow you to cum.\n\n', false );
			EngineCore.outputText( '"<i>Oh I can\'t remove it,</i>" she says, "<i>The only way you\'ll be rid of it with any sort of certainty is to melt through it with something stronger.  Something, like, oh I don\'t know, the focused remains of your soul and humanity.  Now you think on that while I grind away any remaining doubts you might have.</i>"\n\n', false );
			EngineCore.outputText( 'The beautiful succubus on top of you picks up the pace, grinding harder against your ' + Descriptors.vaginaDescript( 0 ) + ' and ' + Descriptors.clitDescript() + ', pushing you beyond anything you\'ve ever felt before.  She leans down over you, licking her lips to reveal an inhumanly long tongue.  It snakes down, curving around you ' + Descriptors.allBreastsDescript() + ' before it arches up to flick a ' + Descriptors.nippleDescript( 0 ) + '.  ', false );
			if( CoC.getInstance().player.hasFuckableNipples() && CoC.getInstance().player.biggestTitSize() > 2 ) {
				EngineCore.outputText( 'You gasp as the tongue slides inside each of your breasts, violating them in turn thanks to your strange anatomy.\n\n', false );
			} else {
				EngineCore.outputText( 'You gasp as it curls around each of your ' + Descriptors.nippleDescript( 0 ) + 's in turn, tugging them lewdly.\n\n', false );
			}
			EngineCore.outputText( 'She fucks you like that for hours, until the table collapses under the pair of you and dumps you both on the floor. More than anything you find yourself craving release, and over time you cave in further and further to the need.  You start to feel the block weakening, melting, and eroding.  Your life has been distilled down into this one moment, this one desire, and this need for release.  The block shatters, melting away under the force of your need as you explosively orgasm.\n\n', false );
			EngineCore.outputText( 'Sparkling pink fluid splatters between the two of you as you cum, squirting hard', false );
			if( CoC.getInstance().player.vaginas[ 0 ].vaginalWetness < AppearanceDefs.VAGINA_WETNESS_SLAVERING ) {
				EngineCore.outputText( ' for the first time', false );
			}
			EngineCore.outputText( '.  The succubus throws back her head and lets loose a moan of ecstasy, her entire body shivering with your own as both of your heads fill with fireworks of pleasure.  Nervelessly, she rolls off of you, her tail contracting hard around your leg while the two of you share the moment.\n\n', false );
			EngineCore.outputText( 'The succubus interrupts your delight by recovering far faster than you, rolling up to a standing position and watching something between your legs.  You prop yourself up on your elbows to see what the fuss is about.  Between your legs something curious is happening – a trickle of pinkish fluid is still escaping your nethers, rolling towards a rapidly expanding pool, along with every other drop of the pink goop.  Before your very eyes the pool grows until every drop of pink fluid has collected together, and it grows upwards, solidifying into a sparkling crystalline shape.\n\n', false );
			EngineCore.outputText( 'Before you can react, she grasps the newly-formed lethicite and noisily begins eating it, her eyes glowing with newfound power.  Watching her makes you more than a little jealous and angry with yourself.  You should\'ve taken the lethicite and gained its power!  No use fretting about it, you can still fuck this succubus for a few hours before you go out in search of your own victims...\n\n', false );
		}
		//[HERM ENDING];
		else {
			//Buh.  Zombie fen need brains.;
			EngineCore.outputText( CoC.getInstance().player.multiCockDescript() + ' and ' + Descriptors.vaginaDescript( 0 ) + ' grow wet and ready, both starting to leak fluids as the succubus\' hand traces your inner thigh.  She teases, "<i>Oh my! You\'re so wet and ready and I haven\'t even touched your moist little cum-receptacle.  And that throbbing cock!  How obscene!  You\'re a slut aren\'t you?  Who else would be so turned on by the idea of cumming until your humanity is splattered between my legs?</i>"\n\n', false );
			EngineCore.outputText( 'The words make you blush hard, shaming you and stoking the growing fire between your legs.  You know two things for certain: she\'s right and you\'re more turned on that ever.  You don\'t resist as the demoness easily lifts you up, setting you down on a table with your legs spread.  "<i>There,</i>" she comments, "<i>now all of your fun-parts are on display.  Maybe I should call in an incubus and a few imps to watch.  I bet you\'d like that wouldn\'t you?</i>"\n\n', false );
			EngineCore.outputText( 'She effortlessly swings her lissomelegs onto the table as she pulls herself up, mounting you in a single swift motion.  You can feel waves of heat rolling off her sex, bathing your ' + Descriptors.cockDescript( 0 ) + ' in her warmth.  ', false );
			if( CoC.getInstance().player.clitLength >= 2 ) {
				EngineCore.outputText( 'Your ' + Descriptors.clitDescript() + ' pushes free, nuzzling against her tight asshole and slipping inside, as if drawn in by its desire.  She openly moans, and begins rocking on top of you.  You gasp in delight as she rides your ' + Descriptors.clitDescript() + ', fucking her ass and grinding against it.', false );
			} else {
				EngineCore.outputText( 'She lowers herself down, rubbing smooth hairless netherlips over your crotch and vulva, smearing you with her fragrant demon-honey.  You feel her clit grinding on your belly, drawing out gasps of delight from both of your mouths as she relentlessly works her body against your own.', false );
			}
			...
			EngineCore.outputText( 'A look of shock and pleasure spreads over the succubus\' face as you release into her hot snatch, cumming with a force unlike anything you\'ve felt before.  Her walls squeeze and caress in time with your orgasm, milking you of every drop.  Your body clenches and squeezes, shuddering as the orgasm continues for far longer than normal.  Though you don\'t feel like you\'re pushing out as much fluid as normal, somehow it feels even better, like a slow drip of pleasure and release.  When at last your ' + Descriptors.cockDescript( 0 ) + ' empties, you feel drained and strangely energized at the same time.\n\n', false );
			EngineCore.outputText( 'The slutty succubus stands up, her puffy vulva coated in a shining pink fluid.  Did that just come out of you?  She grunts, her eyes glowing for a moment as the pink goop disappears into her skin, vanishing entirely.\n\n', false );
			EngineCore.outputText( '"<i>Ahhhhh,</i>" she sighs, "<i>nothing like fresh Lethicite.  Mmmm, your\'s was soooo potent!</i>"\n\n', false );
			EngineCore.outputText( 'You stand up, dissatisfied at the sudden lack of sensation you\'re forced to endure.  The gloating demoness looks rather pleased with herself, and brimming with her new-found power.  You resolve to ', false );
			if( CoC.getInstance().player.findStatusAffect( StatusAffects.MaraesLethicite ) < 0 ) {
				EngineCore.outputText( 'gather some yourself at the next opportunity...', false );
			} else {
				EngineCore.outputText( 'devour Marae\'s as soon as you get a chance.', false );
			...
			EngineCore.outputText( ' demon-cock for a few more orgasms.', false );
			EngineCore.outputText( '  Before you get into that, you spy a small piece of pink crystal on the floor between your legs.  You snatch it and devour it before the succubus has a chance and eat it, turning part of your soul into new-found demonic strength before you return to a long night of sex...', false );
		}
		CoC.getInstance().player.orgasm();
		EngineCore.dynStats( 'str', 2, 'tou', 2, 'spe', 2, 'int', 2, 'lib', 2, 'sen', 2, 'cor', 100 );
		EngineCore.doNext( this.demonBadEnd2 );
	};
	//epilogues;
	...
	DungeonCore.prototype.takeSupervisorsKey = function() {
		...
		CoC.getInstance().player.createKeyItem( 'Supervisor\'s Key', 0, 0, 0, 0 );
		EngineCore.doNext( EventParser.playerMenu );
	};
	...
	DungeonCore.prototype.storageTakeLactaid = function() {
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.TakenLactaid ) >= 0 ) {
			CoC.getInstance().player.addStatusValue( StatusAffects.TakenLactaid, 1, -1 );
		} else {
	...
	DungeonCore.prototype.storageTakeGroPlus = function() {
		if( CoC.getInstance().player.findStatusAffect( StatusAffects.TakenGroPlus ) >= 0 ) {
			CoC.getInstance().player.addStatusValue( StatusAffects.TakenGroPlus, 1, -1 );
		} else {
