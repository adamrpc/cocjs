...
angular.module( 'cocjs' ).run( function( SceneLib, MainView, Zetaz, HarpyMob, Brigid, PhoenixPlatoon, HarpyQueen, SandWitchMob, CumWitch, SecretarialSuccubus, IncubusMechanic, OmnibusOverseer, Combat, Appearance, ConsumableLib, Vala, Utils, PerkLib, StatusAffects, Descriptors, CockTypesEnum, EventParser, OnLoadVariables, AppearanceDefs, kFLAGS, CoC, EngineCore ) {
	...
	DungeonCore.prototype.dungeonMenu = function() {
		...
		if( OnLoadVariables.dungeonLoc === DungeonCore.DUNGEON_FACTORY_STORE_ROOM ) {
			...
			if( CoC.player.findStatusAffect( StatusAffects.TakenLactaid ) >= 0 ) {
				if( CoC.player.statusAffectv1( StatusAffects.TakenLactaid ) > 0 ) {
					EngineCore.outputText( 'There is a crate with ' + Utils.num2Text( CoC.player.statusAffectv1( StatusAffects.TakenLactaid ) ) + ' bottles of something called \'Lactaid\' inside.\n\n', false );
			...
			if( CoC.player.findStatusAffect( StatusAffects.TakenGroPlus ) >= 0 ) {
				if( CoC.player.statusAffectv1( StatusAffects.TakenGroPlus ) > 0 ) {
					EngineCore.outputText( 'There is a crate with ' + Utils.num2Text( CoC.player.statusAffectv1( StatusAffects.TakenGroPlus ) ) + ' bottles of something called \'Gro+\' inside.\n\n', false );
		...
		if( OnLoadVariables.dungeonLoc === SceneLib.dungeonHelSupplimental.DUNGEON_HEL_WINE_CELLAR ) {
			...
			if( CoC.flags[ kFLAGS.HEL_DUNGEON_MEAD_LOOTED ] < 5 ) {
				EngineCore.outputText( ' (There are ' + (5 - CoC.flags[ kFLAGS.HEL_DUNGEON_MEAD_LOOTED ]) + 'x God\'s Mead horns here to take.)\n\n' );
	...
	DungeonCore.prototype.relieveTension = function() {
		...
		//Second/third times...;
			...
			if( CoC.player.statusAffectv1( StatusAffects.TensionReleased ) === 0 ) {
				...
				if( CoC.player.cocks.length > 0 ) {
					EngineCore.outputText( 'and ' + CoC.player.multiCockDescript() );
				}
				...
				//HERM;
				if( CoC.player.vaginas.length > 0 && CoC.player.cocks.length > 0 ) {
					EngineCore.outputText( 'You feel your ' + CoC.player.multiCockDescript() + ' getting milked by many wet holes, though you are too busy sucking cocks and moaning in ecstasy to notice who they belong to.  ' );
				}
			...
			//Third time;
				...
				if( CoC.player.cocks.length > 0 ) {
					EngineCore.outputText( ', filling your ' + CoC.player.multiCockDescriptLight() );
	...
	DungeonCore.prototype.succubusBadEndPartTwo = function() {
		...
		if( CoC.player.biggestLactation() < 1 ) { //Tits – regular
			...
			EngineCore.outputText( 'Your ' + Descriptors.nippleDescript( 0 ) + 's get painfully hard as you feel milk begin backing up inside your ' + Descriptors.allBreastsDescript() + '.   The succubus glances down mischieviously as her hands begin to grope you through your ' + CoC.player.armorName + ', squeezing out a few drops of milk.  ' );
		} else {
			{ //Tits – megalactating
			}
			EngineCore.outputText( 'Your ' + Descriptors.nippleDescript( 0 ) + 's get painfully hard as milk begins drooling down your over-productive chest, making your ' + CoC.player.armorName + ' slide across your leaky milk-spouts in an agonizingly pleasurable way.  ' );
		}
		if( CoC.player.cocks.length === 1 ) {
			{ //Cock – single
			}
		...
		//DO MULTIZ;
		if( CoC.player.cocks.length > 1 ) {
			EngineCore.outputText( 'The feeling of light-headedness nearly robs you of consciousness as your ' + CoC.player.multiCockDescript() + ' fill with blood, pulsating with arousal as they reach full size.  ' );
		}
		if( CoC.player.vaginas.length > 0 ) {
			{ //Vagooooz
			}
	...
	DungeonCore.prototype.succubusLossRape = function() {
		...
		if( CoC.player.cocks.length > 0 ) {
			...
			if( CoC.player.lust > 99 ) {
				EngineCore.outputText( 'Driven half mad with lust, you shake yourself free from the trappings of your ' + CoC.player.armorName + ', first revealing your ' + Descriptors.allBreastsDescript() + ', then ' + Descriptors.hipDescript() + ' and finally your ', false );
				if( CoC.player.vaginas.length > 0 ) {
	...
	DungeonCore.prototype.succubusVictoryRape = function() {
		...
		if( CoC.player.cocks.length > 0 && (CoC.player.gender !== 3 || Utils.rand( 2 )) ) {
			...
			EngineCore.outputText( ', shifting to ' + CoC.player.hairColor + '.  The bone structures of her cheeks, nose, and face shift ever so slightly, and you suddenly realize you are looking down at a slutty version of yourself!  You aren\'t sure if it\'s the growing pool of succubus fluid below you or how hot your female twin is, but your ' + Descriptors.cockDescript( 0 ) + ' is as hard as a rock.\n\n', false );
	...
	DungeonCore.prototype.dungeonSuccubusForceFeed = function() {
		...
		if( CoC.player.biggestLactation() >= 4 ) {
			...
			EngineCore.outputText( ' in sympathy, drenching your ' + CoC.player.skinDesc + ' with creamy goodness until it puddles on your captive demon.\n\n', false );
		}
		if( CoC.player.totalNipples() === 2 ) {
			EngineCore.outputText( 'As your first nipple drains, you move her to your other breast, being sure to motivate her cunt by grinding it with your ' + CoC.player.foot() + '.  She squeals and sucks harder, emptying the last of your milk with a cute burp.  ', false );
		}
		...
		if( CoC.player.cocks.length > 0 ) {
			EngineCore.outputText( ' and ' + CoC.player.multiCockDescriptLight() + ' feel hungrier than ever.  ', false );
		} else {
	...
	DungeonCore.prototype.incubusLossRape = function() {
		...
		//Tentacle gangbang;
			...
			EngineCore.outputText( 'In a flash, each fat tentacle-cock whips out and surrounds your body in slick demon-flesh.  The tentacles constrict, working in pairs to take off every piece of your ' + CoC.player.armorName + '.  ', false );
			...
			if( CoC.player.vaginas.length > 0 ) {
				...
				if( CoC.player.gender === 0 ) {
					EngineCore.outputText( 'grinding suggestively on your captor\'s rods, barely noticing as the last of your ' + CoC.player.armorName + ' falls away.', false );
				}
			}
			EngineCore.outputText( '\n\n', false );
			//FUCKKKING;
			...
			if( CoC.player.vaginas.length > 0 && (CoC.player.cocks.length === 0 || Utils.rand( 2 )) ) {
				...
				if( CoC.player.cor < 80 ) {
					EngineCore.outputText( 'You feel your ' + CoC.player.legs() + ' lifted and pulled tight as countless demonic cocks encircle your body, binding and constraining you further.  You whimper as a demonic tentacle probes your back door while a thicker one lines itself up just below your ' + Descriptors.clitDescript() + '.  ', false );
				} else {
			...
			//Male/Genderless;
				...
				if( CoC.player.cocks.length > 0 ) {
					...
					//Big cum;
					if( CoC.player.cumQ() >= 50 && CoC.player.cumQ() < 400 ) {
						EngineCore.outputText( 'Your orgasm goes off like a shotgun blast, splattering the incubus with a huge wad of cum.  It\'s but the first of many, and though each load of jizz is of comparable size, the force behind them diminishes until the last few blasts drip down your body and soak your ' + CoC.player.legs() + ' and ' + CoC.player.feet() + '.  ', false );
					}
					//Huge cum;
					if( CoC.player.cumQ() >= 400 ) {
	...
	DungeonCore.prototype.incubusVictoryRapeSex = function() {
		...
		if( CoC.player.cocks.length > 0 ) {
			EngineCore.outputText( 'With a few deft motions, you shift your ' + CoC.player.armorName + ' to expose your ', false );
			if( CoC.player.gender === 3 ) {
				if( CoC.player.balls > 0 ) {
					EngineCore.outputText( CoC.player.multiCockDescriptLight() + ', ' + Descriptors.ballsDescriptLight() + ', and ' + Descriptors.vaginaDescript( 0 ), false );
				} else {
					EngineCore.outputText( CoC.player.multiCockDescriptLight() + ' and ' + Descriptors.vaginaDescript( 0 ), false );
				}
			} else {
				if( CoC.player.balls > 0 ) {
					EngineCore.outputText( CoC.player.multiCockDescriptLight() + ' and ' + Descriptors.ballsDescriptLight(), false );
				} else {
	...
	DungeonCore.prototype.incubusVictoryService = function() {
		...
		if( CoC.player.hairLength > 10 ) {
			EngineCore.outputText( 'and brushing a strand of ' + CoC.player.hairColor + ' out of the way ', false );
		}
	...
	DungeonCore.prototype.omnibusVictoryGrowBreasts = function() {
		...
		//Singular row - cup size + 3.  Nipple size to 1" if smaller.;
			...
			if( CoC.player.nippleLength < 0.75 ) {
				...
				EngineCore.outputText( 'Your ' + Descriptors.nippleDescript( 0 ) + 's grow hard and sensitive, becoming much more noticeable inside your ' + CoC.player.armorName + '.  It appears your nipples have grown larger to match.' );
			}
		}
		//Multiple Rows...;
		else {
			...
			if( CoC.player.nippleLength < 0.75 ) {
				CoC.player.nippleLength += 0.5;
				EngineCore.outputText( 'Your ' + Descriptors.nippleDescript( 0 ) + 's grow hard and sensitive, becoming much more noticable inside your ' + CoC.player.armorName + '.  It appears your nipples are have grown larger to match.' );
			}
		}
		EngineCore.outputText( '\n\n' );
	...
	DungeonCore.prototype.omnibusVictoryGrowDick = function() {
		...
		if( CoC.player.cocks.length > 1 ) {
			...
			//Not yet full of demoncocks...;
				...
				EngineCore.outputText( 'You smile, watching your ' + CoC.player.multiCockDescriptLight() + ' grow longer.  Inches of newfound dick-flesh erupt from your crotch in response to omnibus\' dark magics.  Before you can play with your transformed pleasure tools, a wave of burning desire passes through you.  You watch' );
				if( CoC.player.cor < 80 ) {
					EngineCore.outputText( ' in horror' );
				} else {
					EngineCore.outputText( ' curiously' );
				}
				EngineCore.outputText( ' as the skin of your ' + CoC.player.multiCockDescriptLight() + ' turns shiny and purplish-black.  Corrupt nodules begin to spring up over the entire length of each dick.  ' );
				while( temp > 0 ) {
					temp--;
	...
	DungeonCore.prototype.omnibusVictoryNormalChest = function() {
		...
		if( CoC.player.breastRows.length > 1 ) {
			...
			EngineCore.outputText( 'Your chest tingles and begins to feel lighter.  You hastily pull open your ' + CoC.player.armorName + ' and realize you only have ' + Descriptors.allBreastsDescript() + ' now!  ' );
			changed = true;
		}
		//Size!;
		...
		if( CoC.player.skinType !== AppearanceDefs.SKIN_TYPE_PLAIN ) {
			EngineCore.outputText( 'The skin on your body itches intensely as it sheds its ' + CoC.player.skinDesc + ', revealing ' + CoC.player.skinTone + ' skin.  ' );
	...
	DungeonCore.prototype.omnibusVictoryNormalGroin = function() {
		...
		//Remove multiple.;
		if( CoC.player.cocks.length > 1 ) {
			EngineCore.outputText( 'Your ' + CoC.player.multiCockDescriptLight() + ' shiver and retract back towards your body.  When the process finishes you are left with only your ' + Descriptors.cockDescript( 0 ) + '.  ' );
	...
	DungeonCore.prototype.omnibusVictoryNormalLegs = function() {
		...
		if( CoC.player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_HUMAN ) {
			...
			EngineCore.outputText( 'You collapse as your ' + CoC.player.legs() + ' are unable to support you.  The sounds of bones breaking and reshaping fills the room, but oddly you feel no pain, only mild arousal.  You blink your eyes and sigh, and when you look down again <b>you have normal human legs</b>!' );
		}
	...
	DungeonCore.prototype.omnibusLossRape = function() {
		...
		if( CoC.player.HP < 1 ) {
			...
			EngineCore.outputText( 'Forgetting about the fight, you lean on the desk and slide your free hand under your ' + CoC.player.armorName + ', seeking any pleasure you can get.  ', false );
		}
		EngineCore.outputText( 'Sensing victory, the demoness sidles up next to you and pushes you into a chair.  Incapable of resisting, all you can do is watch as she opens your ' + CoC.player.armorName + ' for easier access.  She steps back and admires her handiwork as she gives you a thorough looking over.\n\n', false );
	...
	DungeonCore.prototype.omnibusLossRape2 = function() {
		...
		if( CoC.player.cocks.length > 1 ) {
			...
			EngineCore.outputText( 'Overloaded with pleasure, you feel your ' + CoC.player.multiCockDescriptLight() + ' pulse and cum, creating translucent green cum balloons the size of ', false );
			if( CoC.player.cumQ() < 50 ) {
		...
		if( CoC.player.vaginas.length === 1 || CoC.player.gender === 0 ) {
			...
			if( CoC.player.gender === 0 ) {
				EngineCore.outputText( 'The demoness smiles with wicked intent and yanks the bottoms of your ' + CoC.player.armorName + ' the rest of the way off.  Your lack of genetalia does not faze her, and she responds by swiftly pulling out a needle and injecting your groin.  In seconds your crotch splits open, revealing a fresh virgin vagina.  Licking her perfect lips with anticipation, she inverts the panties and holds them up for you to see.\n\n', false );
				CoC.player.createVagina();
			} else {
				EngineCore.outputText( 'The demoness smiles with wicked intent and yanks your ' + CoC.player.armorName + '\'s bottom the rest of the way off.  She leans close, smiling and inhaling the scent of your sex, savoring it like a aroma of a fine wine.  Licking her perfect lips with anticipation, she inverts the panties and holds them up for you to see.\n\n', false );
			}
	...
	DungeonCore.prototype.demonBadEnd = function() {
		...
		//[genderless];
		if( CoC.player.gender === 0 ) {
			EngineCore.outputText( 'entire body blushes read before the sexy seductress.  She looks at you, frowning as she murmers, "<i>Now this just won\'t do.  You look more like a ' + CoC.player.boyGirl() + ' to me, so why don\'t I make the plumbing match the exterior?</i>"\n\n', false );
		...
		if( CoC.player.gender === 1 ) {
			//Multispoooooo;
			if( CoC.player.cockTotal() > 1 ) {
				EngineCore.outputText( CoC.player.multiCockDescriptLight() + ' pulsate, straining for just a touch of the succubus\' hand.  She paces around you, giggling and toying with you as your ' + CoC.player.multiCockDescript() + ' seem to follow her, twitching and thickening any time she takes a step closer.\n\n', false );
				...
				EngineCore.outputText( 'the bases of your ' + CoC.player.multiCockDescriptLight() + '.\n\n', false );
			...
			//SINGLEZ NITE;
				...
				EngineCore.outputText( CoC.player.multiCockDescriptLight() + ' pulsates, straining for just a touch of the succubus\' hand.  She paces around you, giggling and toying with you as your ' + CoC.player.multiCockDescript() + ' seems to follow her, twitching and thickening anytime she takes a step closer.\n\n', false );
		...
		//[female];
			...
			EngineCore.outputText( 'The words make you blush hard, shaming you and stoking the growing fire between your ' + CoC.player.legs() + '.  You know two things for certain: she\'s right and you\'re more turned on that ever.  You don\'t resist as the demoness easily lifts you up, setting you down on a table with your ' + CoC.player.legs() + ' spread.  "<i>There,</i>" she comments, "<i>now your juicy snatch is on display, just like you\'ve always wanted.</i>"\n\n', false );
		...
		//[HERM ENDING];
			...
			EngineCore.outputText( CoC.player.multiCockDescript() + ' and ' + Descriptors.vaginaDescript( 0 ) + ' grow wet and ready, both starting to leak fluids as the succubus\' hand traces your inner thigh.  She teases, "<i>Oh my! You\'re so wet and ready and I haven\'t even touched your moist little cum-receptacle.  And that throbbing cock!  How obscene!  You\'re a slut aren\'t you?  Who else would be so turned on by the idea of cumming until your humanity is splattered between my legs?</i>"\n\n', false );
