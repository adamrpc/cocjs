...
angular.module( 'cocjs' ).run( function( MainView, SceneLib, $log, ArmorLib, WeaponLib, PregnancyStore, EncapsulationPod, Appearance, ConsumableLib, ImpHorde, Vala, Utils, Combat, StatusAffects, Descriptors, CockTypesEnum, EventParser, OnLoadVariables, AppearanceDefs, kFLAGS, CoC, EngineCore ) {
	...
	Dungeon2Supplimental.prototype.leaveZetazsLair = function() {
		...
		MainView.outputText( 'You leave the cave behind and take off through the deepwoods back towards camp.' );
	...
	Dungeon2Supplimental.prototype.imtacularMultiHitzilla = function() {
		...
		if( !CoC.monster.findStatusAffect( StatusAffects.Blind ) ) {
			...
			MainView.outputText( 'In spite of their blindness, most of them manage to find you, aided by the clutching claws of their brothers.\n', false );
		}
		...
		while( hits > 0 ) {
			...
			//Determine if dodged!;
				...
				if( CoC.player.spe - CoC.monster.spe < 8 ) {
					...
					MainView.outputText( 'You dodge ' + CoC.monster.a + CoC.monster.short + '\'s ' + CoC.monster.weaponVerb + ' with superior quickness!\n', false );
				} else if( CoC.player.spe - CoC.monster.spe >= 20 ) {
					MainView.outputText( 'You deftly avoid ' + CoC.monster.a + CoC.monster.short + '\'s slow ' + CoC.monster.weaponVerb + '.\n', false );
				}
			}
			//Determine if evaded;
			...
			//OH SHIT SON YOU GOT REAPED;
				...
				if( hits === 6 ) {
					MainView.outputText( 'You\'re clawed viciously by an imp!', false );
				}
				if( hits === 5 ) {
					MainView.outputText( 'One bites your ankle!', false );
				}
				if( hits === 4 ) {
					MainView.outputText( 'An imp rakes his claws down your back.', false );
				}
				if( hits === 3 ) {
					MainView.outputText( 'One of the little bastards manages to scratch up your legs!', false );
				}
				if( hits === 2 ) {
					MainView.outputText( 'Another imp punches you in the gut, hard!', false );
				}
				if( hits === 1 ) {
					MainView.outputText( 'Your arm is mauled by the clawing!', false );
				}
				damage = 20 - Utils.rand( CoC.player.tou / 10 );
				if( damage <= 0 ) {
					damage = 1;
				}
	...
	Dungeon2Supplimental.prototype.impGangBukkake = function() {
		MainView.outputText( 'Many of the imps are overcome by the lust you\'ve inspired.  They hover in the air around you, pumping their many varied demonic rods as they bring themselves to orgasm.\n', false );
		//(2-6 hits);
		var hits = Utils.rand( 5 ) + 2;
		//Initial damage variable.;
		var damage = 0;
		...
		while( hits > 0 ) {
			...
			//Determine if dodged!;
				...
				if( damage === 0 ) {
					...
					MainView.outputText( 'One of the imps launches his seed so hard it passes clean over you, painting the wall white.\n', false );
				} else if( damage === 2 ) {
					MainView.outputText( 'You duck a glob of spooge and it passes harmlessly by.  A muffled grunt of disgust can be heard from behind you.\n', false );
				} else if( damage === 3 ) {
			...
			//(2-6 hits for +10 lust each!) (-5 lust per successful hit);
				...
				damage = Utils.rand( 6 );
				if( damage === 0 ) {
					MainView.outputText( 'A squirt of hot demonic cum splatters into your face!\n', false );
				}
				if( damage === 1 ) {
					MainView.outputText( 'Your ' + Descriptors.allBreastsDescript() + ' are coated with thick demonic spunk!\n', false );
				}
				if( damage === 2 ) {
					MainView.outputText( 'Some of the fluid splatters into your midriff and drools down to your waist, making your ' + CoC.player.armorName + ' slimy and wet.\n', false );
				}
				if( damage === 3 ) {
					MainView.outputText( 'Seed lands in your ' + Descriptors.hairDescript() + ', slicking you with demonic fluid.\n', false );
				}
				if( damage === 4 ) {
					MainView.outputText( 'Another blast of jizz splatters against your face, coating your lips and forcing a slight taste of it into your mouth.\n', false );
				}
				if( damage === 5 ) {
					MainView.outputText( 'The last eruption of cum soaks your thighs and the lower portions of your ' + CoC.player.armorName + ', turning it a sticky white.\n', false );
				}
				EngineCore.dynStats( 'lus', (7 + Math.ceil( CoC.player.lib / 40 + CoC.player.cor / 40 )) );
			}
			CoC.monster.lust -= 5;
			hits--;
		}
	...
	Dungeon2Supplimental.prototype.impGangUber = function() {
		if( !CoC.monster.findStatusAffect( StatusAffects.ImpUber ) ) {
			...
			MainView.outputText( 'The imps in the back finish their spell-casting, and point at you in unison.  A wave of pure arousal hits you with the force of a freight train.   Your equipment rubs across your suddenly violently sensitive ' + Descriptors.nippleDescript( 0 ), false );
			if( CoC.player.biggestLactation() > 1 ) {
				MainView.outputText( ' as they begin to drip milk', false );
			}
			MainView.outputText( '.  The lower portions of your coverings ', false );
			if( CoC.player.cockTotal() > 0 ) {
				MainView.outputText( 'are pulled tight by your ' + CoC.player.multiCockDescript() + ', ', false );
				if( CoC.player.totalCocks() > 1 ) {
					MainView.outputText( 'each ', false );
				}
				MainView.outputText( 'beading a drop of pre-cum at the tip', false );
				if( CoC.player.hasVagina() ) {
					MainView.outputText( ', and in addition, the clothes around your groin ', false );
				}
			}
			if( CoC.player.hasVagina() ) {
				MainView.outputText( 'become stained with feminine moisture', false );
				if( CoC.player.clitLength > 3 ) {
					MainView.outputText( ' as your clit swells up in a more sensitive imitation of a cock', false );
				}
			}
			if( CoC.player.gender === 0 ) {
				MainView.outputText( 'rub the sensitive skin of your thighs and featureless groin in a way that makes you wish you had a sex of some sort', false );
			}
			MainView.outputText( '.\n', false );
			CoC.monster.removeStatusAffect( StatusAffects.ImpUber );
		}
	};
	Dungeon2Supplimental.prototype.loseToImpMob = function() {
		...
		//(HP) ;
		if( CoC.player.HP < 1 ) {
			MainView.outputText( 'Unable to handle your myriad wounds, you collapse with your strength exhausted.\n\n', false );
		}
		//(LUST);
		else {
			MainView.outputText( 'Unable to handle the lust coursing through your body, you give up and collapse, hoping the mob will get you off.\n\n', false );
		}

		MainView.outputText( 'In seconds, the squirming red bodies swarm over you, blotting the rest of the room from your vision.  You can feel their scrabbling fingers and hands tearing off your ' + CoC.player.armorName + ', exposing your body to their always hungry eyes.   Their loincloths disappear as their growing demonic members make themselves known, pushing the tiny flaps of fabric out of the way or outright tearing through them.   You\'re groped, touched, and licked all over, drowning in a sea of long tongues and small nude bodies.\n\n', false );
		MainView.outputText( 'You\'re grabbed by the chin, and your jaw is pried open to make room for a swollen dog-dick.   It\'s shoved in without any warmup or fan-fare, and you\'re forced to taste his pre in the back of your throat.  You don\'t dare bite down or resist in such a compromised position, and you\'re forced to try and suppress your gag reflex and keep your teeth back as he pushes the rest of the way in, burying his knot behind your lips.\n\n', false );
		//(tits);
		if( CoC.player.biggestTitSize() > 1 ) {
			MainView.outputText( 'A sudden weight drops onto your chest as one of the demons straddles your belly, allowing his thick, tainted fuck-stick to plop down between your ' + Descriptors.allBreastsDescript() + '.  The hot fluid leaking from his nodule-ringed crown  swiftly lubricates your cleavage.  In seconds the little devil is squeezing your ' + Descriptors.breastDescript( 0 ) + ' around himself as he starts pounding his member into your tits.  The purplish tip peeks out between your jiggling flesh mounds, dripping with tainted moisture.', false );
			if( CoC.player.biggestLactation() > 1 ) {
				MainView.outputText( '  Milk starts to squirt from the pressure being applied to your ' + Descriptors.breastDescript( 0 ) + ', which only encourages the imp to squeeze even harder.', false );
			}
			MainView.outputText( '\n\n', false );
		}
		//(NIPPLECUNTS!);
		if( CoC.player.hasFuckableNipples() ) {
			MainView.outputText( 'A rough tweak on one of your nipples startles you, but your grunt of protest is turned into a muffled moan when one of the imp\'s tiny fingers plunges inside your ' + Descriptors.nippleDescript( 0 ) + '.  He pulls his hand out, marveling at the sticky mess, and wastes no time grabbing the top of your tit with both hands and plunging himself in.', false );
			if( CoC.player.biggestTitSize() < 7 ) {
				MainView.outputText( '  He can only get partway in, but it doesn\'t seem to deter him.', false );
			} else {
				MainView.outputText( '  Thanks to your massive bust, he is able to fit his entire throbbing prick inside you.', false );
			}
			MainView.outputText( '  The demon starts pounding your tit with inhuman vigor, making the entire thing wobble enticingly.  The others, seeing their brother\'s good time, pounce on ', false );
			if( CoC.player.totalNipples() > 2 ) {
				MainView.outputText( 'each of ', false );
			}
			MainView.outputText( 'your other ' + Descriptors.nippleDescript( 0 ), false );
			if( CoC.player.totalNipples() > 2 ) {
				MainView.outputText( 's', false );
			}
			MainView.outputText( ', fighting over the opening', false );
			if( CoC.player.totalNipples() > 2 ) {
				MainView.outputText( 's', false );
			}
			MainView.outputText( '.  A victor quickly emerges, and in no time ', false );
			if( CoC.player.totalNipples() === 2 ) {
				MainView.outputText( 'both', false );
			} else {
				MainView.outputText( 'all the', false );
			}
			MainView.outputText( ' openings on your chest are plugged with a tumescent demon-cock.\n\n', false );
		}
		//(SINGLE PEN) ;
		if( !CoC.player.hasVagina() ) {
			MainView.outputText( 'Most of the crowd centers itself around your lower body, taking a good long look at your ' + Descriptors.assholeDescript() + '.  An intrepid imp steps forwards and pushes his member into the unfilled orifice.  You\'re stretched wide by the massive and unexpectedly forceful intrusion.  The tiny corrupted nodules stroke every inch of your interior, eliciting uncontrollable spasms from your inner muscles.  The unintentional dick massage gives your rapist a wide smile, and he reaches down to smack your ass over and over again throughout the ordeal.', false );
		...
		if( CoC.player.totalCocks() > 0 ) {
			MainView.outputText( 'Some of the other imps, feeling left out, fish out your ' + CoC.player.multiCockDescript() + '.  They pull their own members alongside yours and begin humping against you, frotting as their demonic lubricants coat the bundle of cock with slippery slime.   Tiny hands bundle the dicks together and you find yourself enjoying the stimulation in spite of the brutal fucking you\'re forced to take.  Pre bubbles up, mixing with the demonic seed that leaks from your captors members until your crotch is sticky with frothing pre.\n\n', false );
		}
		//(ORGAZMO);
		MainView.outputText( 'As one, the crowd of demons orgasm.  Hot spunk gushes into your ass, filling you with uncomfortable pressure.  ', false );
		if( CoC.player.hasVagina() ) {
			MainView.outputText( 'A thick load bastes your pussy with whiteness, and you can feel it seeping deeper inside your fertile womb.  ', false );
		}
		MainView.outputText( 'Your mouth is filled with a wave of thick cream.  Plugged as you are by the demon\'s knot, you\'re forced to guzzle down the stuff, lest you choke on his tainted baby-batter.', false );
		if( CoC.player.biggestTitSize() > 1 ) {
			MainView.outputText( '  More and more hits your chin as the dick sandwiched between your tits unloads, leaving the whitish juice to dribble down to your neck.', false );
			if( CoC.player.hasFuckableNipples() ) {
				if( CoC.player.totalNipples() === 2 ) {
					MainView.outputText( '  The pair', false );
				} else {
					MainView.outputText( '  The group', false );
				}
				MainView.outputText( ' of cocks buried in your ' + Descriptors.nippleDescript( 0 ) + ' pull free before they cum, dumping the spooge into the gaping holes they\'ve left behind.  It tingles hotly, making you quiver with pleasure.', false );
			}
		}
		MainView.outputText( '  Finally, your own orgasm arrives, ', false );
		if( CoC.player.cockTotal() === 0 ) {
			MainView.outputText( 'and you clench tightly around the uncomfortable intrusion.', false );
		} else {
			MainView.outputText( 'and ' + CoC.player.sMultiCockDesc() + ' unloads, splattering the many demons with a bit of your own seed.  You\'d smile if your mouth wasn\'t so full of cock.  At least you got to make a mess of them!', false );
		}
		if( CoC.player.hasVagina() ) {
			MainView.outputText( '  Your cunt clenches around the invading cock as orgasm takes you, massaging the demonic tool with its instinctual desire to breed.  Somehow you get him off again, and take another squirt of seed into your waiting cunt.', false );
		}
		MainView.outputText( '\n\n', false );
		MainView.outputText( 'Powerless and in the throes of post-coital bliss, you don\'t object as you\'re lifted on the table', false );
		if( !CoC.player.hasVagina() ) {
			MainView.outputText( ' and forced to start drinking bottle after bottle of succubi milk', false );
		}
		MainView.outputText( '.  You pass out just as round two is getting started, but the demons don\'t seem to mind....', false );
		EngineCore.doNext( this, this.loseToImpMobII );
	};
	//[IMP GANGBANG VOL 2];
	Dungeon2Supplimental.prototype.loseToImpMobII = function() {
		MainView.outputText( '', true );
		MainView.outputText( 'You wake up, sore from the previous activity and a bit groggy.  You try to move, but find yourself incapable.  Struggling futilely, you thrash around until you realize your arms and legs are strapped down with heavy iron restraints.  You gasp out loud when you look down and discover your ', false );
		if( CoC.player.biggestTitSize() < 1 ) {
			MainView.outputText( 'new', false );
		} else {
			MainView.outputText( 'much larger', false );
		}
		MainView.outputText( ' tits, wobbling with every twist and movement you make.  You\'re stark naked, save for a sheer and somewhat perverse nurse\'s outfit.   The room around you looks to be empty, though you can see a number of blankets piled in the corners and a few cages full of spooge-covered faeries, all snoring contently.\n\n', false );

		MainView.outputText( 'Eventually a lone imp enters the room.  It\'s Zetaz!  He looks you up and down and decrees, "<i>You\'re ready.</i>"  You struggle to shout him down, but all that escapes the gag in your mouth is incomprehensible gibberish.  He chuckles and flips a switch on the wall, and suddenly the most heavenly vibration begins within your sopping twat.', false );
		if( !CoC.player.hasVagina() ) {
			MainView.outputText( '...Wait, your what?  You have a cunt now!?', false );
		}
		MainView.outputText( '  Your eyes cross at the pleasure as your mind struggles to figure out why it feels so good.\n\n', false );
		MainView.outputText( 'Zetaz pours a few bottles into a larger container and connects a tube to an opening on the bottom of the bottle.  Your eyes trace the tube back to the gag in your mouth, and after feeling around with your tongue, you realize it\'s been threaded through the gag and down your throat.   Zetaz lifts up the bottle and hangs it from a hook on the ceiling, and you watch in horror as the fluid flows through the tube, helpless to stop it.  You shake your head desperately, furious at having fallen into the little fucker\'s hands at last.\n\n', false );
		MainView.outputText( 'Zetaz walks up and paws at your ', false );
		if( CoC.player.biggestTitSize() < 1 ) {
			MainView.outputText( 'new', false );
		} else {
			MainView.outputText( 'larger', false );
		}
		MainView.outputText( ' mounds, flitting into the air to bring himself to eye-level.  He rambles, "<i>It\'s so good to see you again, ' + CoC.player.short + '.  Because of you, I had to flee from my honored place by Lethice\'s side.  I\'ve had to hide in this fetid forest.  I\'ll admit, it hasn\'t been all bad.  We\'ve caught a few faeries to play with, and with you here, the boys and I will have lots of fun.  We just need to reshape that troubled mind a little bit.</i>"\n\n', false );
		MainView.outputText( 'You barely register his monologue.  You\'re far too busy cumming hard on the vibrating intruder that\'s currently giving your stuffed snatch the workout of a lifetime.  Zetaz chuckles at your vacant stare and massages your temples gently, and you feel the touch of his dark magic INSIDE you.  It feels warm and wet, matching the feel of your body\'s other intrusion.   You try to fight it, and for a moment you feel like you might push the demon out of your mind.  Then your body cums, and your resistance melts away.  You violently thrash against your restraints, caving in to the pleasure as the imp rapes your body and mind as one.\n\n', false );
		MainView.outputText( 'The desire to protect your village drips out between your legs, and thoughts of your independence are fucked away into nothing.  It feels good to cum, and your eyes cross when you see the bulge at your master\'s crotch, indicative of how well you\'re pleasing him.  It feels so good to obey!  Zetaz suddenly kisses you, and you enthusiastically respond in between orgasms.\n\n', false );
		MainView.outputText( 'You gladly live out the rest of your life, fucking and birthing imps over and over as their live-in broodmother.', false );
		CoC.player.orgasm();
		CoC.player.HP += 100;
		EngineCore.gameOver();
	};
	...
	Dungeon2Supplimental.prototype.impGangVICTORY = function() {
		MainView.outputText( '', true );
		//Flag them defeated!;
		CoC.flags[ kFLAGS.ZETAZ_IMP_HORDE_DEFEATED ] = 1;
		if( CoC.monster.HP < 1 ) {
			MainView.outputText( 'The last of the imps collapses into the pile of his defeated comrades.  You\'re not sure how you managed to win a lopsided fight, but it\'s a testament to your new-found prowess that you succeeded at all.', false );
		} else {
			MainView.outputText( 'The last of the imps collapses, pulling its demon-prick free from the confines of its loincloth.  Surrounded by masturbating imps, you sigh as you realize how enslaved by their libidos the foul creatures are.', false );
		}
	...
	Dungeon2Supplimental.prototype.impGangGetsRapedByMale = function() {
		MainView.outputText( '', true );
		MainView.outputText( 'You walk around and pick out three of the demons with the cutest, girliest faces.  You set them on a table and pull aside your ' + CoC.player.armorName + ', revealing your ' + CoC.player.multiCockDescriptLight() + '.  You say, "<i>Lick,</i>" in a tone that brooks no argument.  The feminine imps nod and open wide, letting their long tongues free.   Narrow and slightly forked at the tips, the slippery tongues wrap around your ' + Descriptors.cockDescript( 0 ) + ', slurping wetly as they pass over each other in their attempts to please you.\n\n', false );
		MainView.outputText( 'Grabbing the center one by his horns, you pull him forwards until your shaft is pressed against the back of his throat.  He gags audibly, but you pull him back before it can overwhelm him, only to slam it in deep again.  ', false );
		MainView.outputText( 'The girly imp to your left, seeing how occupied your ' + Descriptors.cockDescript( 0 ) + ' is, shifts his attention down to your ', false );
		...
		MainView.outputText( ', licking with care', false );
		if( CoC.player.balls === 0 ) {
			MainView.outputText( ' and plunging deep inside', false );
		}
		MainView.outputText( '.  The imp to the right wraps his tongue around the base ', false );
		if( CoC.player.hasSheath() ) {
			MainView.outputText( 'just above your sheath ', false );
		}
		MainView.outputText( ' and pulls it tight, acting as an organic cock-ring.\n\n', false );
		MainView.outputText( 'Fucking the little bitch of a demon is just too good, and you quickly reach orgasm.  ', false );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( 'Cum boils in your balls, ready to paint your foe white.  ', false );
		}
		MainView.outputText( 'With a mighty heave, you yank the imp forward, ramming your cock deep into his throat.  He gurgles noisily as you unload directly into his belly.   Sloshing wet noises echo in the room as his belly bulges slightly from the load, and his nose dribbles cum.   You pull him off and push him away.  He coughs and sputters, but immediately starts stroking himself, too turned on to care.', false );
		if( CoC.player.cumQ() > 1000 ) {
			MainView.outputText( '  You keep cumming while the other two imps keep licking and servicing you.   By the time you finish, they\'re glazed in spooge and masturbating as well.', false );
		}
		MainView.outputText( '\n\n', false );
	...
	Dungeon2Supplimental.prototype.impGangGetsRapedByFemale = function() {
		MainView.outputText( '', true );
		MainView.outputText( 'You walk around to one of the demons and push him onto his back.  Your ' + CoC.player.armorName + ' falls to the ground around you as you disrobe, looking over your tiny conquest.  A quick ripping motion disposes of his tiny loincloth, leaving his thick demon-tool totally unprotected. You grab and squat down towards it, rubbing the corrupted tool between your legs ', false );
		if( CoC.player.vaginas[ 0 ].vaginalWetness >= AppearanceDefs.VAGINA_WETNESS_SLICK ) {
			MainView.outputText( 'and coating it with feminine drool ', false );
		}
		MainView.outputText( 'as you become more and more aroused.  It parts your lips and slowly slides in.  The ring of tainted nodules tickles you just right as you take the oddly textured member further and further into your willing depths.', false );
		CoC.player.cuntChange( 15, true, true, false );
		MainView.outputText( '\n\n', false );
		MainView.outputText( 'At last you feel it bottom out, bumping against your cervix with the tiniest amount of pressure.  Grinning like a cat with the cream, you swivel your hips, grinding your ' + Descriptors.clitDescript() + ' against him in triumph.  ', false );
		if( CoC.player.clitLength > 3 ) {
			MainView.outputText( 'You stroke the cock-like appendage in your hand, trembling with delight.  ', false );
		}
		MainView.outputText( 'You begin riding the tiny demon, lifting up, and then dropping down, feeling each of the nodes gliding along your sex-lubed walls.   As time passes and your pleasure mounts, you pick up the pace, until you\'re bouncing happily atop your living demon-dildo.\n\n', false );
		MainView.outputText( 'The two of you cum together, though the demon\'s pleasure starts first.  A blast of his tainted seed pushes you over the edge.  You sink the whole way down, feeling him bump your cervix and twitch inside you, the bumps on his dick swelling in a pulsating wave in time with each explosion of fluid.  ', false );
		if( CoC.player.vaginas[ 0 ].vaginalWetness >= AppearanceDefs.VAGINA_WETNESS_SLAVERING ) {
			MainView.outputText( 'Cunt juices splatter him as you squirt explosively, leaving a puddle underneath him.  ', false );
		} else {
			MainView.outputText( 'Cunt juices drip down his shaft, oozing off his balls to puddle underneath him.  ', false );
		}
		MainView.outputText( 'The two of you lie together, trembling happily as you\'re filled to the brim with tainted fluids.\n\n', false );
		MainView.outputText( 'Sated for now, you rise up, your body dripping gooey whiteness.  Though in retrospect it isn\'t nearly as much as was pumped into your womb.', false );
		if( CoC.player.pregnancyIncubation === 0 ) {
			MainView.outputText( '  You\'ll probably get pregnant.', false );
		}
		EngineCore.dynStats( 'cor', 1 );
		CoC.player.orgasm();
	...
	Dungeon2Supplimental.prototype.enterZetazsRoomFromTheSouth = function() {
		if( CoC.flags[ kFLAGS.ZETAZ_DOOR_UNLOCKED ] === 0 ) {
			MainView.clearOutput();
			MainView.outputText( 'The door won\'t budge.' );
	...
	Dungeon2Supplimental.prototype.getSwordAndGetTrapped = function() {
		MainView.outputText( '', true );
		MainView.outputText( 'You start to walk over to the corpse and its discarded weapon, but halfway through your journey, the unexpected happens.   The leaf-like petals shift underfoot, snapping up with lightning-quick speed.  You ', false );
		if( CoC.player.spe < 50 ) {
			MainView.outputText( 'fall flat on your ' + Descriptors.assDescript() + ', slipping on the slick, shifting surface.', false );
		} else {
			MainView.outputText( 'stumble and nearly fall, slipping on the shifting, slick surface.', false );
		}
		//[ADD 'CONTINUE' ON];
	...
	Dungeon2Supplimental.prototype.encapsulationPodAI = function() {
		...
		if( !CoC.monster.findStatusAffect( StatusAffects.Round ) ) {
			...
			MainView.outputText( 'as they climb ever-further up your body.  In spite of yourself, you feel the touch of arousal licking at your thoughts.\n', false );
		...
		//[Round 2 Action];
		else if( CoC.monster.statusAffectv1( StatusAffects.Round ) === 2 ) {
			MainView.outputText( 'The tentacles under your ' + CoC.player.armorName + ' squirm against you, seeking out openings to penetrate and genitalia to caress.  ', false );
			if( CoC.player.balls > 0 ) {
				MainView.outputText( 'One of them wraps itself around the top of your ' + Descriptors.sackDescript() + ' while its tip slithers over your ' + Descriptors.ballsDescriptLight() + '.  Another ', false );
			} else {
				MainView.outputText( 'One ', false );
			}
			if( CoC.player.cockTotal() > 0 ) {
				MainView.outputText( 'prods your ' + Descriptors.cockDescript( 0 ) + ' for a second before it begins slithering around it, snake-like.  Once it has you encircled from ' + CoC.player.cockHead() + ' to ', false );
				if( !CoC.player.hasSheath() ) {
					MainView.outputText( 'base', false );
				} else {
					MainView.outputText( 'sheath', false );
				}
				MainView.outputText( ', it begins to squeeze and relax to a pleasant tempo.  ', false );
			} else {
				if( CoC.player.hasVagina() ) {
					MainView.outputText( 'prods at your groin, circling around your ' + Descriptors.vaginaDescript( 0 ) + ' deliberately, as if seeking other toys to play with.  ', false );
					if( CoC.player.clitLength > 4 ) {
						MainView.outputText( 'It brushes your ' + Descriptors.clitDescript() + ' then curls around it, squeezing and gently caressing it with a slow, pleasing rhythm.  ', false );
					}
				} else {
					MainView.outputText( 'prods your groin before curling around to circle your ' + Descriptors.assholeDescript() + ' playfully.  The entire tendril pulses in a pleasant, relaxing way.  ', false );
				}
			}
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 'Your other ', false );
				if( CoC.player.cockTotal() === 2 ) {
					MainView.outputText( Descriptors.cockDescript( 1 ) + ' gets the same treatment, and soon both of your ' + CoC.player.multiCockDescriptLight() + ' are quite happy to be here.  ', false );
				} else {
					MainView.outputText( CoC.player.multiCockDescriptLight() + ' get the same treatment and soon feel quite happy to be here.  ', false );
				}
			}
			if( CoC.player.hasVagina() ) {
				MainView.outputText( 'The violation of your ' + Descriptors.vaginaDescript( 0 ) + ' is swift and painless.  The fungus\' slippery lubricants make it quite easy for it to slip inside, and you find your ' + Descriptors.vaginaDescript( 0 ) + ' engorging with pleasure in spite of your need to escape.  The tentacle folds up so that it can rub its stalk over your ' + Descriptors.clitDescript() + ', ', false );
				if( CoC.player.clitLength > 3 ) {
					MainView.outputText( 'and once it discovers how large it is, it wraps around it and squeezes.  It feels good!  ', false );
				} else {
					MainView.outputText( 'and it has quite an easy time making your bud grow hard and sensitive.  The constant rubbing feels good!  ', false );
				}
			}
			MainView.outputText( 'One \'lucky\' stalk manages to find your ' + Descriptors.assholeDescript() + '.  As soon as it touches your rear \'entrance\', it lunges forward to penetrate you.  The fluids coating the tentacle make your muscles relax, allowing it to slide inside you with ease.\n\n', false );
			MainView.outputText( 'The rest of the mass continues to crawl up you.  They tickle at your ', false );
			if( CoC.player.pregnancyIncubation > 0 && CoC.player.pregnancyIncubation < 120 ) {
				MainView.outputText( 'pregnant ', false );
			}
			MainView.outputText( 'belly as they get closer and closer to ', false );
			if( CoC.player.biggestTitSize() < 1 ) {
				MainView.outputText( 'your chest', false );
			} else {
				MainView.outputText( 'the underside of your ' + Descriptors.allBreastsDescript(), false );
			}
			MainView.outputText( '.  Gods above, this is turning you on!  Your lower body is being violated in every conceivable way and it\'s only arousing you more.  Between the mind-numbing smell and the sexual assault you\'re having a hard time focusing.\n', false );
		...
		//[Round 3 Action];
		else if( CoC.monster.statusAffectv1( StatusAffects.Round ) === 3 ) {
			MainView.outputText( 'The wet, warm pressure of the fungus\' protrusion working their way up your body feels better than it has any right to be.  It\'s like a combination of a warm bath and a gentle massage, and when combined with the thought-numbing scent in the air, it\'s nigh-impossible to resist relaxing a little.  In seconds the mass of tentacles is underneath your ' + CoC.player.armorName + ' and rubbing over your chest and ' + Descriptors.nippleDescript( 0 ) + 's.  You swoon from the sensation and lean back against the wall while they stroke and caress you, teasing your sensitive ' + Descriptors.nippleDescript( 0 ) + '.', false );
			if( CoC.player.hasFuckableNipples() ) {
				MainView.outputText( '  Proof of your arousal leaks from each ' + Descriptors.nippleDescript( 0 ) + ' as their entrances part for the probing tentacles.  They happily dive inside to begin fucking your breasts, doubling your pleasure.', false );
			}
			MainView.outputText( '  Moans escape your mouth as your hips begin to rock in time with the tentacles and the pulsing luminance of your fungus-pod.  It would be easy to lose yourself here.  You groan loudly enough to startle yourself back to attention.  You\'ve got to get out!\n\n', false );
			MainView.outputText( 'The tentacles that aren\'t busy with your ' + Descriptors.allBreastsDescript() + ' are already climbing higher, and the slime has reached your waist.  If anything it actually makes the constant violation more intense and relaxing.  You start to sink down into it, but catch yourself and pull yourself back up.  No! You\'ve got to fight!\n', false );
		...
		//[Round 4 Action];
			...
			MainView.outputText( 'What\'s happening to you definitely isn\'t rape.  Not any more.  You like it too much.  You lean back against a wall of the pod and thrust your ' + Descriptors.hipDescript() + ' pitifully against a phantom lover, moaning lewdly as you\'re forcibly pleasured.  You grab hold of the fleshy walls with your hands and try to hold yourself up, but your ' + CoC.player.legs() + ' have the consistency of jello.   They fold neatly underneath you as you slide into the ooze and begin to float inside it.  It\'s comforting in an odd way, and while you\'re gasping in between moans, your balance finally gives out.  You sink deeper into the fluid and lose all sense of direction.  Up and down become meaningless constructs that no longer matter to you.\n\n', false );
			MainView.outputText( 'The thick slime passes over your lips and nose as you sink into the rising tide of bliss, and you find yourself wondering how you\'ll breathe.  Instinctively, you hold your breath.  Even riddled with sexual bliss and thought-obliterating drugs, you won\'t let yourself open your mouth when \'underwater\'.  The lack of oxygen makes your heart hammer in your chest', false );
			if( CoC.player.totalCocks() > 0 ) {
				MainView.outputText( ', and ' + CoC.player.sMultiCockDesc() + ' bloats with blood, getting larger than ever', false );
			}
			MainView.outputText( '.  Before you can pass out, the constant penetration forces a moan from your lips.\n\n', false );
			MainView.outputText( 'A tentacle takes the opportunity to slip into your mouth along with a wave of the slime.  You try to cough out the fluid, but there isn\'t any air left in your lungs to push it out.  The orally-fixated tendril widens and begins to pour more of it inside you, and with nowhere else to go, it packs your goo-filled lungs to the brim before you start to swallow.  You relax and exhale the last of your air from your nose as your body calms itself.  Somehow you can breathe the fungus-pod\'s fluids!\n\n', false );
			MainView.outputText( 'You\'re floating in pure liquid bliss.  Thoughts melt away before they can form, and every inch of your body is being caressed, squeezed, or penetrated by the warm, slime-slicked tentacles.  Nearly every muscle in your body goes completely slack as you\'re cradled with bliss.  Without your thoughts or stress bothering you, the pleasure swiftly builds to a crescendo.\n\n', false );
			MainView.outputText( 'The wave of need starts out inside your crotch, begging to be let out, but you can\'t even be bothered to move your ' + Descriptors.hipDescript() + ' anymore.  Without your help, release stays just out of reach, but the tentacles working your body seem intent on spurring it on.  The one inside your ' + Descriptors.assholeDescript() + ' begins to pump more quickly, and with the added pressure, you cum quickly.  ', false );
			if( !CoC.player.hasVagina() ) {
				MainView.outputText( 'Your body twitches weakly, too relaxed to move while it gets off from anal penetration.', false );
			} else {
				MainView.outputText( 'Your body twitches weakly, too relaxed to move while it gets off from being double-penetrated.', false );
			}
			if( CoC.player.hasFuckableNipples() ) {
				MainView.outputText( '  Your ' + Descriptors.nippleDescript( 0 ) + 's squirt around their phallic partners, leaking sexual lubricant ', false );
				if( CoC.player.biggestLactation() > 1 ) {
					MainView.outputText( 'and milk ', false );
				}
				MainView.outputText( 'while the fucking continues.', false );
			}
			if( CoC.player.cockTotal() > 0 ) {
				MainView.outputText( '  The tentacles around ' + CoC.player.sMultiCockDesc() + ' squeeze and rotate, screwing you silly through your orgasm while cum dribbles in a steady stream from your loins.  Normally it would be squirting out in thick ropes, but the muscle-relaxing drugs in your system make the spurts a steady, weak flow.', false );
				if( CoC.player.cumQ() > 800 ) {
					MainView.outputText( '  Of course with all the semen you produce, the flesh-pod\'s ooze clouds over quite quickly, blocking your vision with a purple-white haze.', false );
				}
			}
			if( CoC.player.biggestLactation() > 1 ) {
				MainView.outputText( 'Milk leaks out too, ', false );
				if( CoC.player.biggestLactation() < 2 ) {
					MainView.outputText( 'though the slight dribble is barely noticeable to you.', false );
				} else if( CoC.player.biggestLactation() < 3 ) {
					MainView.outputText( 'coloring things a little more white.', false );
				} else {
					MainView.outputText( 'thickening your fluid-filled prison with nutrients.', false );
				}
			}
	...
	Dungeon2Supplimental.prototype.loseToThisShitPartII = function() {
		...
		if( CoC.player.cumQ() > 3500 ) {
			MainView.outputText( 'Your orgasm drags on for so long that you begin to feel pressure from the cum-slime surrounding you.  It doesn\'t seem to matter to ' + CoC.player.sMultiCockDesc() + ', which is too busy sending bliss to your brain and squirting cum for the tentacles to care.  It actually kind of hurts.  The oscillating purple ambiance flashes brighter in protest for a second, and then everything releases all at once.  The pressure is gone and you\'re sliding down on a wave of fungal-slime cum, feeling the tentacles being pulled from you by the sudden shift of position.  Moist cave air tickles at your ' + CoC.player.skinDesc + ' as you come to rest on another spongy petal and begin to cough out the sludge.\n\n', false );
			MainView.outputText( 'Over the next minute your head clears and your strength returns.  You push yourself up on something hard, then glance down and realize you washed up next to the skeleton!  The bleached bone leers up at you knowingly, and everything you can see is covered in a thick layer of your spooge.  ' + CoC.player.SMultiCockDesc() + ' is still dripping more spunk.  Clearly your ruined orgasm didn\'t pump it ALL out.  You look down at the rapier and pick it up out of your mess, examining it.  The blade shines keenly, and the sword is balanced to perfection.  Though you succumbed to the same fate as its owner, your warped body saved you from sharing his fate.  Thankfully potential pods that carpet the floor don\'t even twitch at you.  Perhaps your orgasm was enough to sate them all?  Or maybe they\'ve learned their lesson.', false );
			//(switch from loss to victory, sword loot);
			CoC.monster.lust = 100;
			CoC.player.orgasm();
		}
		//[OPTIONAL MILK ESCAPE];
		else if( CoC.player.lactationQ() > 3500 || (CoC.player.lactationQ() + CoC.player.cumQ() > 4500) ) {
			MainView.outputText( 'Your milk-spouting ' + Descriptors.nippleDescript( 0 ) + 's continuously pour your breast-milk into the soupy fluids surrounding you.  Once you let down your milk, there was no stopping it.  Pressure backs up inside the flesh-pod, pressing down on you with near painful intensity, but your ' + Descriptors.allBreastsDescript() + ' refuse to give up or slow down.  Even though each squirt jacks up the force on your body, your unholy milk production will not relent.  The oscillating purple ambience flashes bright in protest, then gives out entirely, along with the pressure.  At once you\'re pulled away by a wave of milk-laced fungus-slime, yanking the tentacles away from your body with the change in position.\n\n', false );
		...
		//[BAD-END GO];
		//(MALE)  ;
		if( CoC.player.gender === 1 || (CoC.player.gender === 3 && Utils.rand( 2 ) === 0) ) {
			MainView.outputText( 'The orgasm squirts and drips from ' + CoC.player.sMultiCockDesc() + ' for what seems like forever.  It feels so right, so perfect, that you actually whine in disappointment when it finally does end.  You can\'t even be bothered to reach down and stroke yourself.  The softening in your loins is nothing compared to your flaccid, listless muscles.  You couldn\'t make your arms reach down to touch yourself even if you could work up the motivation to try.  Thankfully the slippery tentacles curl back around your ', false );
			if( !CoC.player.hasSheath() ) {
				MainView.outputText( 'base', false );
			} else {
				MainView.outputText( 'sheath', false );
			}
			MainView.outputText( ' and squeeze, forcing ' + CoC.player.sMultiCockDesc() + ' to inflate to readiness.  Deep inside your ' + Descriptors.assholeDescript() + ', the tentacle starts to rub against your prostate.  It caresses the male organ on each side and pauses to squeeze the center of it, pushing a few drops of sticky cum from your trembling ' + Appearance.cockNoun( CockTypesEnum.HUMAN ) + '.\n\n', false );
			MainView.outputText( 'The vine-like stalks currently hugging ' + CoC.player.sMultiCockDesc() + ' constrict the base and begin to swirl around it in a circular motion.  Warm fungi-flesh and viscous, drugged ooze work together to send hot spikes of pleasure up your spinal-cord.  Despite your recent orgasm, you aren\'t being given any chance to recover or refill your ', false );
			if( CoC.player.balls > 0 ) {
				MainView.outputText( 'balls', false );
			} else {
				MainView.outputText( 'prostate', false );
			}
			MainView.outputText( '.  Things like logic and rest don\'t matter in this warm, soupy environment, at least not to your poor, unthinking mind and erect, sensitive dick', false );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 's', false );
			}
			MainView.outputText( '.  With such stimulation coming so closely on the heels of your last orgasm, [eachCock] is suffering painful levels of pleasure.  Your whole body shakes from the sensory overload; though with your muscles so completely shut down, it\'s more of a shiver.\n\n', false );
			MainView.outputText( 'Another wave of sperm begins the slow escape from your helpless, pinned form, drawn out by the fungus\' constant sexual ministrations.  The fluid inside your pod gurgles noisily as the fluids are exchanged, but the sensory input doesn\'t register to your overloaded, drugged-out shell of a mind.  You\'ve lost yourself to mindless pleasure, and repeated, endless orgasms.  The rest of your life is spent floating in an artificial womb, orgasming over and over to feed your fungus prison, and enjoying the pleasure that long ago eroded your ability to reason.', false );
			EngineCore.gameOver();
		}
		//(FEM);
		else {
			MainView.outputText( 'You orgasm around the tentacles in your ' + Descriptors.vaginaDescript( 0 ) + ' and ' + Descriptors.assholeDescript() + ' for what feels like hours, though some dim, half forgotten whisper of your mind tells you it can\'t possibly have gone on for that long.  It feels so right and so perfect that resistance is almost a foreign concept to you at this point.  How could you have tried to fight off this heaven?  You\'re completely limp, totally helpless, and happier than you ever remember.  The pulsing lights of your womb-like prison continue their steady beat in time with the tentacles buried in your snatch, soothing you while your body is played like a violin heading towards its latest crescendo.\n\n', false );
			MainView.outputText( 'The steady rhythm of your penetration sends rockets of bliss-powered pleasure up your spinal cord and straight into your brain, where it explodes in orgasm.  Your body barely twitches, too relaxed to work up any muscle response, involuntary or otherwise.  A moment to rest never presents itself.  The cruel fungus never relents.  It never slows, unless it\'s only the briefest pause to intensify the next thrust.  Were you in the open air, away from the strange fluid you\'re now breathing, you\'d be twisting and screaming with pleasure.  Instead you float and cum in silence.\n\n', false );
			MainView.outputText( 'Fluids gurgle and shift inside the pod as they are exchanged.  If you were capable of noticing the sound or change, you might wonder if it\'s harvesting your sexual fluids, but even those thoughts are beyond you now. You\'ve lost yourself to mindless pleasure, and repeated, endless orgasms.  The rest of your life is spent floating in an artificial womb, orgasming over and over to feed your fungus prison, and enjoying the pleasure that long ago eroded your ability to reason.', false );
			EngineCore.gameOver();
		}
	};
	...
	Dungeon2Supplimental.prototype.freeValazLooseCoochie = function() {
		MainView.spriteSelect( 85 );
		MainView.outputText( '', true );
		MainView.outputText( 'You search the room for a way to free the fairy from her shackles and find an ugly, iron key that looks like a promising candidate. Opening the rusted metal with a teeth-clenching screech, the girl slumps to the ground in an ungainly heap. The fall seems to have roused her, at least, because she blinks, slowly, several times before lifting her head to stare blankly at you. You try to explain that she\'s free, but it\'s clear that thoughts are travelling through a pretty thick haze of abuse, so you take a moment to let her gather herself. When she\'s managed to assemble what wits are left to her, she slowly curls her mouth into a hopeful smile. "<i>How can Bitch please Master?</i>" she asks in an innocent voice tainted by husky desire.\n\n', false );
		MainView.outputText( 'You bend down to comfort the girl and offer her a shoulder to lean on as you help her to her feet. As you expected, the weight of her milky tits nearly surpasses the rest of her body. She clings to you happily, stroking and rubbing her bare skin against your body. She is adamantly ignoring your efforts to dissuade her amorous advances, merely cooing "<i>master</i>" and "<i>pleasure</i>" over and over again. If you had the right materials, you might be able to mix something to heal the damage her captors have done to the fairy\'s mind.\n\n', false );
		//[Heal (if the player has Pure Honey)] [Sex] [Reject];
		var heal = null;
		if( CoC.player.hasItem( ConsumableLib.PURHONY, 1 ) ) {
			heal = this.healVala;
		}
		if( CoC.player.hasItem( ConsumableLib.P_PEARL, 1 ) ) {
			heal = this.healVala;
		}
	...
	Dungeon2Supplimental.prototype.healVala = function() {
		...
		MainView.outputText( '', true );
		if( CoC.player.hasItem( ConsumableLib.PURHONY, 1 ) ) {
			CoC.player.consumeItem( ConsumableLib.PURHONY, 1 );
			CoC.flags[ kFLAGS.VALA_HEALED_HONEY ] = 1;
			MainView.outputText( 'You\'re not sure if Pure Honey will do the trick, but it seems like the most likely candidate. You set the broken girl down and step over to the alchemy table. She clings onto your ', false );
			if( CoC.player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_NAGA ) {
				MainView.outputText( 'tail', false );
			} else {
				MainView.outputText( CoC.player.leg(), false );
			}
	...
	Dungeon2Supplimental.prototype.rejectFuckingVala = function() {
		...
		if( CoC.flags[ kFLAGS.TIMES_PC_DEFEATED_VALA ] === 0 ) {
			MainView.outputText( 'The fairy\'s weak insistence has begun to get obnoxious. What kind of prisoner dry humps her rescuer? Actually, if the heavy flow of lubricating girl cum dripping out of her pussy is any indication, it\'s the wettest humping you\'ve had with your ' + CoC.player.armorName + ' still on. You seize the girl\'s shoulders and hold her up, pushing her away from your goo-stained lower body. You assure the girl that you won\'t be having sex with her here. It\'s far too dangerous, you tell her, to leave yourself vulnerable right now. You\'ll take her to safety when the demons are defeated. You try to impress on her the importance of speed and stealth, but you might as well be talking to a big-breasted brick wall. When she makes a grab at your crotch, you\'ve had enough and throw her back.\n\n', false );
			MainView.outputText( 'The fairy gathers herself and raises her eyes to you, mad passion whirling in their pink depths. "<i>But Bitch is horny!</i>" she demands, madly. Her wings gain sudden life, flapping rapidly to pull her frail body off the floor. Hovering before you, she curls her fingers into desperate claws and rakes at you. She\'s too far gone, you realize. You\'re going to have to fight the broken fairy!', false );
		} else {
			MainView.outputText( 'What\'s wrong with this fairy? You\'ve already beaten her once, but she\'s still dripping and grinding against you. If anything, it\'s even wetter than the last humping the sex-addicted faerie forced on you. You seize the girl\'s shoulders and hold her up, pushing her away from your goo-stained lower body once again and re-iterate that you won\'t be having sex with her right now – you have other tasks that need your attention.  It doesn\'t work, and once again she makes a move towards your crotch.', false );
			if( CoC.flags[ kFLAGS.TIMES_PC_DEFEATED_VALA_AND_RAEPED ] > 0 ) {
				MainView.outputText( '  You smirk in wonder at the horny little slut.  You guess you\'ll have to put her back into her place and give her another dose of \'love\'.', false );
			}
			MainView.outputText( '\n\n', false );
	...
	Dungeon2Supplimental.prototype.loseToVala = function() {
		...
		if( CoC.player.gender === 0 ) {
			MainView.outputText( '', true );
			MainView.outputText( 'Vala forces a bottle into your throat before your defeated form has a chance to react, and you grunt with pleasure as a new gash opens between your ' + CoC.player.legs() + '!', false );
	...
	Dungeon2Supplimental.prototype.loseToValaAsMale = function() {
		...
		MainView.outputText( 'You are powerless to stop the fairy as she drags you to the south wall and up to the wooden rail secured a couple of feet off the ground. "<i>When she was still growing, Bitch was too small and tight for the masters,</i>" your captor tells you. "<i>They blessed her with this ladder to make us big enough. You will feel their generosity.</i>" Gripping you under the arms, the fairy\'s lust-fuelled strength lifts you off the ground and flies you directly over the bristling peg ladder.\n\n', false );
		//[Next];
	...
	Dungeon2Supplimental.prototype.fightValaVictory = function() {
		MainView.spriteSelect( 85 );
		MainView.outputText( '', true );
		MainView.outputText( 'The fairy girl collapses, well-drilled obedience robbing her limbs of their fight. She squirms to a crouching bow, fully accepting you as her new ' + CoC.player.mf( 'Master', 'Mistress' ) + '. The warped fae\'s empty eyes look up at you, her face a mask of rapture as she anxiously awaits her punishment, wagging her butt in the air as lubrication gushes down her thighs. It seems being defeated has excited the broken creature to a breeding frenzy. Her endurance must be incredible to be this frisky after your battle.', false );
		CoC.flags[ kFLAGS.TIMES_PC_DEFEATED_VALA ]++;
		//[Fuck] [Leave];
		if( CoC.player.gender > 0 ) {
	...
	Dungeon2Supplimental.prototype.tryToHealVala = function() {
		...
		if( !(CoC.player.hasItem( ConsumableLib.PURHONY, 1 ) || CoC.player.hasItem( ConsumableLib.P_PEARL, 1 )) ) {
			MainView.outputText( 'You try your best with what you\'ve got, but nothing seems to restore the broken fairy\'s mind to her sex-addled  body. You\'re going to have to go out and gather more materials. Surely there\'s something that can break the damage the imps have done to Vala.', false );
			EngineCore.doNext( MainView, MainView.playerMenu );
		}
		//(With Pure Honey);
		else if( CoC.player.hasItem( ConsumableLib.PURHONY, 1 ) ) {
			CoC.player.consumeItem( ConsumableLib.PURHONY, 1 );
			MainView.outputText( 'You\'re not sure if Pure Honey will do the trick, but it seems like the most likely candidate. You set the broken girl down and step over to the alchemy table. Vala clings onto your leg as you walk, and you end up dragging her across the dungeon floor. Before things can get too out of hand with the needy girl, you pull out the vial of Pure Honey and arrange the equipment in front of you. Using the cleanest of the pipettes, you take a small portion of the honey and mix it with what you hope to be water, diluting the rich mixture to a more viscous state. Working quickly, you manage to produce a draught that the weak girl can tolerate. By now, she\'s managed to work her way to a sitting position and is grinding her dripping sex against your shin. You lean down and hold her nose to make her open her mouth. She gleefully opens wide, tongue thrashing about in anticipation. You pour the sweet-smelling concoction down her anxious throat and begin to re-cork the rest of your honey.\n\n', false );
	...
	Dungeon2Supplimental.prototype.useValaOHYEAHKOOLAIDPTII = function() {
		...
		//f2;
			...
			MainView.outputText( 'You quickly recover and thrust backwards, driving more of the dildo inside your clenching walls while also slamming the fairy\'s slick box in lusty punishment for her overeagerness. She giggles and humps right back, driving another inch into you, your ' + Descriptors.vaginaDescript( 0 ) + ' aflame with the delicious stimulation of the bumpy, uneven veins carved into the smooth cock-base. Before long, the two of you have a rhythm, driving the makeshift double-ended dildo into each other until you end up ass-to-ass, your ' + Descriptors.buttDescript() + ' smacking heavily against the fey girl\'s supple rear. A large puddle of girl cum has begun forming on the floor, each wet slap of your cunts splashing more of the warm, clear liquid between the two of you. You can feel your orgasm building a knot in your gut and you bite your lower lips as you make your thrusts more forceful, more insistent, slamming yourself down on the veiny cock, driving the flared head deeper into the fairy\'s body. She, in turn, is just as consumed by lust and tries to jackhammer it right back, until you can feel the rounded base pushing against your cervix, blissful pain coursing through your lower body.\n\n', false );
			MainView.outputText( 'You clutch your ' + Descriptors.allBreastsDescript() + ' and squeeze your ' + Descriptors.nippleDescript( 0 ) + ' until it hurts, the agony giving you strength to drive the dildo back into the fairy, but it\'s a losing battle. Vala cums before you do, her pulsing walls locking down and driving the dildo out, inch by painful inch, deeper into your body until the base is so far against your gut that it is pushed into your womb with a toe-curling pop. You silently scream in ecstasy and agony, unable to believe that the frail fairy managed to fuck your womb with your own toy. Your strength falters and you slump down, sliding off the shaft with soothing regret, your cervix clenching wildly, still spasming from its penetration. You roll onto your back, fairy cum all around you, even now dripping down the lube-slick cock still sticking out of the girl\'s pussy, flared tip keeping it locked inside of her. You jill your ' + Descriptors.clitDescript() + ' for a few minutes afterward, just enjoying the afterglow, lapping at the dripping sprite cum that dribbles onto your face from the fairy\'s shivering, dick-stuffed cunt. Gradually, your strength returns and you rise, skin slick with the cum pool you\'ve been basking in. You give Vala\'s ass a slap on your way out- a reward for your favorite brainless fairy.', false );
		}
		//h2;
		else {
	...
	Dungeon2Supplimental.prototype.wakeValaUpBeforeYouGoGo = function() {
		...
		if( CoC.player.gender === 1 ) {
			...
			MainView.outputText( 'The fairy\'s grinding and the sweet scent leaking out of her honey pot is as tempting as ever. You push the girl back gently and nod your head, stripping your ' + CoC.player.armorName + ' piece by piece, teasing the fairy with your slowness. She curls her lips into a wanton smile. "<i>Now fuck Vala!</i>" she begs. You chuckle and show your satisfaction at the small restoration of her mind by grabbing one of her supple tits and leaning down to suck out some of her chest-bloating milk. She arches her back under your touch and clenches her muscles. Her hands find your genitals, eager fingers sliding across your sensitive ' + CoC.player.skinDesc + '. Her hand grabs your ' + Descriptors.cockDescript( 0 ) + ', thumb and pinkie forming a tight circle at your base while her other fingers stroke up and down your shaft. The fairy\'s touch is surprisingly light for the rough treatment she\'s endured, and you\'re quickly brought to hardness under her caresses.\n\n', false );
			MainView.outputText( 'Whatever small parts of her mind may be returning, she clearly hasn\'t conquered her sex-madness, because she simply cannot restrain herself any longer. Fluttering her wings rapidly, the girl lifts out of your embrace and rises above you, lining her sex up with yours, thick beads of wetness trickling down from her gaping pussy. With a mad giggle, she stops flapping and drops down, impaling herself on your length.\n\n', false );
	...
	Dungeon2Supplimental.prototype.wakeMeUpBeforeValaGoGosPtII = function() {
		...
		if( CoC.player.gender === 1 ) {
			MainView.outputText( '', true );
			MainView.outputText( 'You spot an unsecured set of chained manacles on the floor and an idea strikes you. Vala still sliding along your shaft, you bend down and grab the fetters, snapping one around the girl\'s neck like a collar. With a shudder at the cold, you lift the fairy up and lock the other end of the shackles around the root of your ' + Descriptors.cockDescript( 0 ) + ', steel snapping around your base tightly. The makeshift cock-ring works perfectly as your ' + Descriptors.hipDescript() + ' quiver, your body trying to orgasm but denied release by the metal loop. The fairy, meanwhile, thrashes atop your groin, the chain of her collar swinging between her tits, buffeting them with enough force to spray small white streams as she rides you. Her purple hair glimmers pink in the dull light of the dungeon as it bounces right along with her rocking hips. Even the diminished pulses of sunlight streaming from her stained and tattooed skin seems brighter as she is filled labia to cervix by your straining fuckpole.\n\n', false );
	...
	Dungeon2Supplimental.prototype.isValaAtBar = function() {
		return (CoC.flags[ kFLAGS.FREED_VALA ] !== 0 && CoC.time.hours >= 12 && CoC.time.hours <= 21);
	};
	//Purified Fairy;
	Dungeon2Supplimental.prototype.purifiedFaerieBitchBar = function() {
		...
		if( CoC.flags[ kFLAGS.FUCKED_VALA_AT_BAR_TODAY ] > 0 ) {
		...
		if( CoC.player.gender > 0 ) {
			MainView.outputText( '  You could speak with her if you wished to.' );
			return true;
		}
		MainView.outputText( '  Speaking to her might be better after you get a gender...' );
		return false;
	};
	//[Vala];
	Dungeon2Supplimental.prototype.chooseValaInBar = function() {
		MainView.spriteSelect( 60 );
		var cumBath = null;
		...
		} else if( CoC.time.hours === 20 && Utils.rand( 2 ) === 0 && CoC.player.hasCock() ) {
	...
	Dungeon2Supplimental.prototype.cleansedValaFuckHerBrainsOutPtII = function() {
		...
		if( CoC.player.totalCocks() > 0 ) {
			...
			//[Tall characters- 6'3"+];
				...
				MainView.outputText( 'Your limbs entwined, you let the helpless, orgasming fairy\'s hands fondle your shoulders and back as you draw your knees up, enveloping Vala in a full body embrace, her loins milking with every ounce of strength left to her. A flush of pride and nurturing compassion fills your heart even as your own crescendo erupts, linking the two of you in a creamy bond of shivering passion. She nestles in the hollow of your lap, panting with every pulse of jizz you pump into her, nuzzling your chest, perfectly at ease in your ', false );
				//;
				if( CoC.player.biggestTitSize() < 1 ) {
					MainView.outputText( 'chest', false );
				} else {
	...
	Dungeon2Supplimental.prototype.cleansedValaRepeatBrainFucking = function() {
		MainView.spriteSelect( 60 );
		MainView.outputText( '', true );
		MainView.outputText( 'She flashes a beautiful smile and tells the bartender that she\'ll be taking her break early. Grabbing you by the hand, she rushes to the stockroom and sheds her dress without any trace of her former shame about her tattooed back. When you\'re too slow taking off your ' + CoC.player.armorName + ', she helps, deft hands made all the quicker by eagerness. When the two of you are naked, she pushes you onto the well-worn stool and sits in your lap, staring into your eyes with a small smile.', false );
	...
	Dungeon2Supplimental.prototype.faerieOrgyFuck = function() {
		...
		MainView.outputText( 'You thank Vala for introducing you, but you\'ve really got to be going, you claim. Demons to defeat, maidens to rescue, all that. The large fairy chuckles and gives you a squeeze. "<i>You\'re so cute when you\'re flustered. Don\'t worry, we\'re not going to all jump you at once- you\'d probably end up like I did! No, I asked my sisters here to help me with a little forest magic. Don\'t you want to see how fairies masturbate?</i>" You\'re a little taken aback by the question, but you nod all the same. "<i>All right girls, the petals please.</i>" The cloud of fairies in front of you disperses, each winged vixen scattering to different corners of the room to retrieve hidden flower petals. Each blossom, you note, matches the hair color of the fairy holding it, creating a dizzying array of  hues as they form circles around the two of you. Vala guides you to the bed and gently removes your ' + CoC.player.armorName + ' before instructing you to lie down.\n\n', false );
		CoC.flags[ kFLAGS.WEEKLY_FAIRY_ORGY_COUNTDOWN ] = 6;
		CoC.flags[ kFLAGS.VALA_TIMES_CONSENSUAL_SEX ]++;
		//[Herm];
		if( CoC.player.gender === 3 ) {
	...
	Dungeon2Supplimental.prototype.faerieOrgyFuckFemaleContinue = function() {
		MainView.spriteSelect( 60 );
		MainView.outputText( '', true );
		MainView.outputText( 'The fairy girl lands at the edge of the bed, settling on her knees and resting her weighty chest on the mattress. Leaning in, she spreads your ' + CoC.player.legs() + ' with warm, delicate hands, fingertips stroking the insides of your thighs. You shiver and hug your arms under your breasts, a little nervous about this \'forest magic.\' One of the fairies flutters down to Vala and hands her a pale white lily petal with a tittering giggle. The larger fairy takes the soft white bloom and, whispering a silken word to it, licks the cream-colored surface. Slithering further between your legs, she reaches your ' + Descriptors.vaginaDescript( 0 ) + ' and brushes the petal against your ' + Descriptors.clitDescript() + ' with a teasing caress. Then, much to your surprise, she places it on the tip of her forefinger and slides it into your pussy. You gasp, the petal\'s soft surface rubbing your inner walls with a warmth you were unprepared for. Vala presses lightly on a spot inside your vagina and you lose your breath as the tremor of a small orgasm rumbles through your gut. Without explanation, the fairy barmaid takes a rose petal and repeats the process, another orgasm seizing you and leaving you panting. "<i>Normally we only need the one petal, but since you and I are so much larger than the average fairy, I had my sisters bring lots. All for you,</i>" she whispers, coyly.\n\n', false );
		//[If the Player has unlocked Scylla's Addiction Counseling event] ;
		if( CoC.flags[ kFLAGS.TIMES_SCYLLA_ADDICT_GROUP_EXPLOITED ] + CoC.flags[ kFLAGS.SCYLLA_TIMES_SHARED_IN_ADDICT_GROUP ] + CoC.flags[ kFLAGS.SCYLLA_MILK_THERAPY_TIMES ] + CoC.flags[ kFLAGS.SCYLLA_CUM_THERAPY_TIMES ] + CoC.flags[ kFLAGS.SCYLLA_SEX_THERAPY_TIMES ] > 0 ) {
			MainView.outputText( 'Another fairy flies toward Vala and you blink through the building lusty haze when you recognize the little girl. Wearing a slightly slutty crystalline gown of translucent gossamer, you recognize Pastie, the fairy from Scylla\'s addiction counseling meeting. When you call out to her, she bashfully waves back. "<i>Hey, uh, ' + CoC.player.short + '. It\'s kinda weird seeing you outside of the meetings. How are you? How have things been?</i>" She looks at your naked, reclined form on the bed and blushes. "<i>Oh, right, right. Um. I\'m just helping out Vala, you know. Sisters and all. Uh. I\'m... I\'m going to just give her this petal and get something to drink, okay? All this sobriety is making this really uncomfortable for me.</i>" The other fairies laugh and Pastie hands off her blossom before zipping out of the room, probably to steal someone\'s drink downstairs.\n\n', false );
		}
	...
	Dungeon2Supplimental.prototype.zetazTaunt = function() {
		if( !CoC.monster.findStatusAffect( StatusAffects.round ) ) {
			...
			if( CoC.player.humanScore() >= 4 ) {
				MainView.outputText( 'human', false );
			} else {
				MainView.outputText( '\'human\'', false );
			}
			MainView.outputText( '?  No, of course not.  That\'s the kind of attitude I\'d expect from one of you!</i>"', false );
		} else {
			CoC.monster.addStatusValue( StatusAffects.round, 1, 1 );
			if( CoC.monster.statusAffectv1( StatusAffects.round ) === 2 ) {
				MainView.outputText( '"<i>I lost my post!  And when you screwed up the factory?  I barely escaped with my life!  You ruined EVERYTHING!</i>" screams Zetaz.', false );
			} else if( CoC.monster.statusAffectv1( StatusAffects.round ) === 3 ) {
				MainView.outputText( 'Zetaz snarls, "<i>Do you know how hard it is to hide from Lethice?  DO YOU HAVE ANY IDEA!?  I\'ve had to live in this fetid excuse for a jungle, and just when I found some friends and made it livable, you show up and DESTROY EVERYTHING!</i>"', false );
			} else if( CoC.monster.statusAffectv1( StatusAffects.round ) === 4 ) {
				MainView.outputText( 'Zetaz explains, "<i>I won\'t let you go.  I\'m going to break you.</i>"', false );
			} else if( CoC.monster.statusAffectv1( StatusAffects.round ) === 5 ) {
				MainView.outputText( '"<i>Would it have been that bad to go along with me?  You\'ve seen the factory.  We would\'ve kept you fed, warm, and provided you with limitless pleasure.  You would\'ve tasted heaven and served a greater purpose.  It\'s not too late.  If you come willingly I can make sure they find a good machine to milk you with,</i>" offers the imp lord.', false );
			} else if( CoC.monster.statusAffectv1( StatusAffects.round ) === 6 ) {
				MainView.outputText( '"<i>Why won\'t you fall?</i>" questions Zetaz incredulously.', false );
			} else if( CoC.monster.statusAffectv1( StatusAffects.round ) === 7 ) {
				MainView.outputText( 'The imp lord suggests, "<i>If you give up and let me fuck your ass maybe I\'ll let you go.</i>"', false );
			} else if( CoC.monster.statusAffectv1( StatusAffects.round ) === 8 ) {
				MainView.outputText( 'Zetaz pants, "<i>Just give up!  I\'m nothing like the weakling you met so long ago!  I\'ve been through hell to get here and I\'m not giving it up just because you\'ve shown up to destroy my hard work!</i>"', false );
			} else {
				MainView.outputText( 'He glares at you silently.', false );
			}
		}
	};
	//ZETAZ AI:;
	Dungeon2Supplimental.prototype.zetazAI = function() {
		...
		if( CoC.monster.findStatusAffect( StatusAffects.Constricted ) ) {
			...
			if( CoC.monster.statusAffectv1( StatusAffects.Constricted ) <= 0 ) {
				MainView.outputText( '  ' + CoC.monster.getCapitalA() + CoC.monster.short + ' proves to be too much for your tail to handle, breaking free of your tightly bound coils.', false );
		...
		if( CoC.monster.lust > 50 && CoC.monster.HPRatio() <= 0.5 ) {
			...
			if( attackChoice === 0 ) {
				...
				MainView.outputText( 'The imp leaps into the air with a powerful spring, beating his wings hard to suspend himself in the center of his bedchamber.  Dust kicks up into the air from the force of his flight and turns the room into a blinding tornado!  Small objects smack off of you, ', false );
				//(causing little damage/;
				if( CoC.player.tou > 60 ) {
					MainView.outputText( 'causing little damage', false );
				} else {
					var dmg = 1 + Utils.rand( 6 );
					dmg = CoC.player.takeDamage( dmg );
					MainView.outputText( 'wounding you slightly (' + dmg + ')', false );
				}
				MainView.outputText( ' while the dust gets into your eyes, temporarily blinding you!', false );
	...
	Dungeon2Supplimental.prototype.gigaArouse = function() {
		...
		EngineCore.dynStats( 'lus', Utils.rand( CoC.player.lib / 10 ) + CoC.player.cor / 10 + 15 );
		if( CoC.player.lust < 30 ) {
			MainView.outputText( 'Your nethers pulse with pleasant warmth that brings to mind pleasant sexual memories.  ', false );
		}
		if( CoC.player.lust >= 30 && CoC.player.lust < 60 ) {
			MainView.outputText( 'Blood rushes to your groin in a rush as your body is hit by a tidal-wave of arousal.  ', false );
		}
		if( CoC.player.lust >= 60 ) {
			MainView.outputText( 'Your mouth begins to drool as you close your eyes and imagine yourself sucking off Zetaz, then riding him, letting him sate his desires in your inviting flesh.  The unnatural visions send pulses of lust through you so strongly that your body shivers.  ', false );
		}
		if( CoC.player.cocks.length > 0 ) {
			if( CoC.player.lust >= 60 && CoC.player.cocks.length > 0 ) {
				MainView.outputText( 'You feel ' + CoC.player.sMultiCockDesc() + ' dribble pre-cum, bouncing with each beat of your heart and aching to be touched.  ', false );
			}
			if( CoC.player.lust >= 30 && CoC.player.lust < 60 && CoC.player.cocks.length === 1 ) {
				MainView.outputText( CoC.player.SMultiCockDesc() + ' hardens and twitches, distracting you further.  ', false );
			}
		}
		if( CoC.player.vaginas.length > 0 ) {
			if( CoC.player.lust >= 60 && CoC.player.vaginas[ 0 ].vaginalWetness === AppearanceDefs.VAGINA_WETNESS_NORMAL && CoC.player.vaginas.length === 1 ) {
				MainView.outputText( 'Your ' + Descriptors.allVaginaDescript() + ' dampens perceptibly, feeling very empty.  ', false );
			}
			if( CoC.player.lust >= 60 && CoC.player.vaginas[ 0 ].vaginalWetness === AppearanceDefs.VAGINA_WETNESS_WET && CoC.player.vaginas.length > 0 ) {
				MainView.outputText( 'Your crotch becomes sticky with girl-lust, making it clear to ' + CoC.monster.a + CoC.monster.short + ' just how welcome your body finds the spell.  ', false );
			}
			if( CoC.player.lust >= 60 && CoC.player.vaginas[ 0 ].vaginalWetness === AppearanceDefs.VAGINA_WETNESS_SLICK && CoC.player.vaginas.length === 1 ) {
				MainView.outputText( 'Your ' + Descriptors.allVaginaDescript() + ' becomes sloppy and wet, dribbling with desire to be mounted and fucked.  ', false );
			}
			if( CoC.player.lust >= 60 && CoC.player.vaginas[ 0 ].vaginalWetness === AppearanceDefs.VAGINA_WETNESS_DROOLING && CoC.player.vaginas.length > 0 ) {
				MainView.outputText( 'Thick runners of girl-lube stream down the insides of your thighs as your crotch gives into the demonic magics.  You wonder what ' + CoC.monster.a + CoC.monster.short + '\'s cock would feel like inside you?  ', false );
			}
			if( CoC.player.lust >= 60 && CoC.player.vaginas[ 0 ].vaginalWetness === AppearanceDefs.VAGINA_WETNESS_SLAVERING && CoC.player.vaginas.length === 1 ) {
				MainView.outputText( 'Your ' + Descriptors.allVaginaDescript() + ' instantly soaks your groin with the heady proof of your need.  You wonder just how slippery you could ' + CoC.monster.a + CoC.monster.short + '\'s dick when it\'s rammed inside you?  ', false );
			}
		}
	...
	Dungeon2Supplimental.prototype.defeatZetaz = function() {
		CoC.flags[ kFLAGS.DEFEATED_ZETAZ ]++;
		MainView.outputText( '', true );
		//[VICTORY HP];
		if( CoC.monster.HP < 1 ) {
			MainView.outputText( 'Zetaz sinks down on his knees, too wounded to continue.  He looks up at you with helpless rage in his eyes and asks, "<i>Are you satisfied now?  Go ahead then, kill me.  My life hasn\'t been worth living since I met you anyway.</i>"\n\n', false );
		}
		//[VICTORY LUST] ;
		else {
			MainView.outputText( 'Zetaz sinks down on his knees and fishes his massive, pre-drooling member from under his loincloth.  He looks up at you, nearly crying and moans, "<i>Why? Ruining my life wasn\'t enough?  You had to make me jerk off at your feet too?  Just kill me, I don\'t want to live anymore.</i>"\n\n', false );
		}

		//[Both] ;
		MainView.outputText( 'He can\'t die yet.  You need to know where his master, this \'Lethice\', is.  It sounds like she\'s the queen-bitch of the demons, and if you\'re going to break this vicious cycle', false );
		//( or take her place);
		if( CoC.player.cor > 66 ) {
			MainView.outputText( ' or take her place', false );
		}
		MainView.outputText( ', you need to find her and bring her down.  What do you do?', false );
	...
	Dungeon2Supplimental.prototype.releaseZForInfo = function() {
		MainView.outputText( '', true );
		MainView.outputText( 'You look the pathetic imp up and down and smirk.  He closes his eyes, expecting a summary execution, but you present him with an offer instead.  If he gives you more information on Lethice and where to find her, you\'ll let him go scot-free and avoid him if he doesn\'t make a nuisance of himself.\n\n', false );
		MainView.outputText( '"<i>Really?</i>" questions Zetaz in a voice laced with suspicion. "<i>For fuck\'s sake, I\'m already a renegade.  I\'ll take your deal.  It\'s not like it costs me anything I wouldn\'t give away for free anyway.</i>"\n\n', false );
		MainView.outputText( 'Invigorated by the promise of safety and freedom, Zetaz pulls himself up and ', false );
		if( CoC.monster.HP < 1 ) {
			MainView.outputText( 'staggers', false );
		} else {
			MainView.outputText( 'nearly stumbles over his lust-filled cock', false );
		}
		MainView.outputText( ' towards a desk.  His dextrous fingers twist the knob on the top drawer expertly until a quiet \'click\' comes from the furniture.  He reaches down to the divider between the drawers and pulls on it, revealing a tiny, hidden compartment.  In the center of it is a detailed map of the mountain and its upper reaches.  Though the secret diagram is quite crude, it depicts a winding trail that bypasses numerous harpy nests, minotaur caves, and various unrecognizable pitfalls to reach the cloud-shrouded mountain peak.  The drawing loses much of its detail once it gets to the demon fortifications at the top, but it can\'t be that hard to track down Lethice once you\'ve entered the seat of her power, can it?\n\n', false );
		MainView.outputText( 'A loincloth flies across the room and deposits itself on your shoulder, startling you from your planning.  You glance back and see Zetaz tearing through his possessions, tossing his most prized items into a burlap sack with reckless abandon.  His whole body is trembling, as he ties it to a wooden pole, never once looking up at you.  Perhaps he fears you might change your mind?  ', false );
		if( CoC.player.cor > 66 ) {
			MainView.outputText( 'You smirk down at him and fold your arms over your ', false );
			if( CoC.player.biggestTitSize() < 1 ) {
				MainView.outputText( 'chest', false );
			} else {
				MainView.outputText( Descriptors.breastDescript( 0 ), false );
			}
			MainView.outputText( ', relishing his fear while you consider the possibilities', false );
		} else if( CoC.player.cor > 33 ) {
			MainView.outputText( 'You chuckle with amusement and watch the little bastard scrabble to pack up his life, relishing the chance to pay him back for your previous encounter', false );
		} else {
			MainView.outputText( 'You sigh and rub at your temples as the little jerk scrabbles to pack his life away.  In spite of yourself, you actually feel a little bad about the situation', false );
		}
	...
	Dungeon2Supplimental.prototype.sexualInterrogation = function() {
		MainView.outputText( '', true );
		MainView.outputText( 'You lean down until your face hovers over Zetaz, looking him square in the eyes, and explain, "<i>I can\'t have someone who knows the way to the demons\' headquarters dying before they tell me how to get there, can I?</i>"\n\n', false );
		MainView.outputText( '"<i>Piss off!  You won\'t get shit from me,</i>" retorts the defeated demon, "<i>You may as well finish me off – I\'ll NEVER help a ' + CoC.player.mf( 'jackass', 'bitch' ) + ' like you!</i>"\n\n', false );
		MainView.outputText( 'Smirking, you grab a strip of leather from Zetaz\'s dresser and dangle it over his nose.\n\n', false );
		MainView.outputText( 'You whisper, "<i>This is all I\'ll need.</i>"\n\n', false );
		MainView.outputText( 'The imp looks up with his face shrouded in confusion as he asks, "<i>I don\'t think a string is going to help you much, ' + CoC.player.mf( 'dork', 'skank' ) + '.</i>"\n\n', false );
		MainView.outputText( '"<i>Give me a moment, my stupid little snitch,</i>" you taunt as you lift his loincloth, exposing the hardness concealed within.  It pulses, growing harder and an inch longer just from your brief touch and exposure to the air.  Perhaps the imp isn\'t as in control of his libido as he\'d like you to think?  You twirl the leather strip around his base and swiftly knot it, getting a tight enough seal to make Zetaz grunt in discomfort. "<i>Ungh! What the fuck!? Ow, goddamnit!</i>"\n\n', false );
		MainView.outputText( 'Even with his protests and cries, you watch his cock inflate further, until it looks stuffed far beyond his normal capacity.  It twitches and drools corrupted pre-seed as you slide your finger along his urethra, watching the member bob and twitch from the slight, soft touches.  That must feel quite good.  Zetaz confirms your hunch by lifting his hips off the ground, shaking them lewdly to try to grind against your hand.  You don\'t deny him the friction he craves, wrapping your hand around as much of his meat as your fingers will encircle until steady dribbles of fluid escape from his urethra.  The tainted nodules of his demonic dick begin to flare and pulse, signaling that his orgasm is almost upon him.\n\n', false );
		MainView.outputText( 'He\'s NOT allowed to cum – not until you get the information you need!   You slide your fingers down to his base in one fluid stroke, slamming your hand against his crotch as his orgasm starts to bubble up.  Before your opponent can attain release, you squeeze hard with one hand and tighten the leather cord with the other, clamping the base until the imp\'s cum is bottled up in his abdomen.  Zetaz cries, "<i>No-no-no, let me cum, please let me cum, need-ta-cum-nooowwww.</i>"\n\n', false );
		MainView.outputText( 'No such luck.  You wait for his body to stop convulsing and return to your task, one hand sealed around his base while the other begins to stroke him with firm, steady motions, sliding over the pebbly surface of his dick\'s nubs.  Your victim continues his begging and crying, but you don\'t let up as you pause to gather his escaped pre-cum and smear it over his tip.  Zetaz pants and groans, trembling and swelling in your hand from your efficient hand-job.   Spitting on your palm, you bump up the tempo and begin to stroke him hard and fast, sliding over his cockring-swollen prick with practiced, deliberate motions.\n\n', false );
		MainView.outputText( '"<i>Tell me how to find the head demon and I\'ll let you cum.  Don\'t make this any \'harder\' than it has to be,</i>" you whisper.\n\n', false );
		MainView.outputText( 'The demon\'s voice starts to crack in spite of his efforts to remain defiant. "<i>No!  I-uh-won\'t let yo-oooooh-control meeeeee!</i>"\n\n', false );
		MainView.outputText( 'His protests trail into incoherent squeals and babbles as you bottle up his second orgasm behind the tightly tied strap.  Again, his body twists and writhes in your grip, tortured with the ever-increasing sexual tension.  Zetaz looks up at you with a pleading, cross-eyed expression as he tries to regain his wits, but you just keep pumping away.  His balls are visibly pulsing and quivering, desperately needing to release the building pressure.  You meet his gaze calmly, your hands continuing their work on the bloated imp-cock, and you break into a knowing smile as he thickens in your grip for the third time.\n\n', false );
		MainView.outputText( '"<i>Well Zetaz?  Is three the lucky number, or do I have to switch hands and keep backing you up until you go mad?</i>" you ask.\n\n', false );
		MainView.outputText( 'His hands claw the rug underneath him as he gasps, "<i>You win, you win!  The desk has a-ah ah ahh-hidden drawer with a map to Lethice\'s hideout.  Please justletmecomeletmecomeletmecomePLEAAAAASE!</i>"\n\n', false );
	...
	Dungeon2Supplimental.prototype.loseToZetaz = function() {
		MainView.outputText( '', true );
		MainView.outputText( '"<i>Well, isn\'t this familiar?</i>" asks Zetaz as he watches your ', false );
		if( CoC.player.lust > 99 ) {
			MainView.outputText( 'masturbating', false );
		} else {
			MainView.outputText( 'prone', false );
		}
		MainView.outputText( ' form with an amused expression, "<i>The first champion in ages to retain ' + CoC.player.mf( 'his', 'her' ) + ' free will for more than a few minutes, and ' + CoC.player.mf( 'he', 'she' ) + '\'s brought to ' + CoC.player.mf( 'his', 'her' ) + ' knees by the very imp ' + CoC.player.mf( 'he', 'she' ) + ' escaped!  Once you\'ve learned your proper place, you\'ll guarantee my safe return to my rightful station.  Perhaps I\'ll even get a promotion?  After all, you\'ve defeated so many higher ranking demons already.</i>"\n\n', false );
		//'Fix' genderless folks.;
		if( CoC.player.gender === 0 ) {
			MainView.outputText( 'He squints down at you with a bemused look and laughs, "<i>How did you lose your gender anyhow?  Never mind, we\'ve got to do something about that!</i>"\n\n', false );
			MainView.outputText( 'Zetaz grabs a bottle, uncorks it, and crams it against your lips while you\'re still too dazed to resist.  He massages your throat to make you swallow the milk-like fluid, and in seconds the skin of your groin splits to form a new, virgin pussy.\n\n', false );
			CoC.player.createVagina();
			CoC.player.gender = 2;
		}
	...
	Dungeon2Supplimental.prototype.femaleZetazOver = function() {
		...
		MainView.outputText( 'With your resistance ', false );
		if( CoC.player.HP < 1 ) {
			MainView.outputText( 'beaten out of you', false );
		} else {
			MainView.outputText( 'moistening the delta of your legs', false );
		}
		...
		if( CoC.player.cor < 33 ) {
			MainView.outputText( 'shame', false );
		} else if( CoC.player.cor < 66 ) {
			MainView.outputText( 'confuse', false );
		} else {
			MainView.outputText( 'arouse', false );
		}
		MainView.outputText( ' you as you\'re thrown atop one of the tables.   You grunt as leather straps are produced and laid over your form to restrain you.  In the span of a minute you\'re completely immobilized from the neck down, and your ' + CoC.player.legs() + ' are kept spread to allow easy access to your ' + Descriptors.vaginaDescript( 0 ) + '.\n\n', false );
		MainView.outputText( 'Shuffling as they remove their garments, the entire gang of imps, as well as Zetaz, are completely nude.  They\'ve all grown full and hard from the sight of your nubile, restrained body, and in spite of yourself you get ', false );
		if( CoC.player.vaginas[ 0 ].vaginalWetness >= AppearanceDefs.VAGINA_WETNESS_DROOLING ) {
			MainView.outputText( 'even more wet ', false );
		} else {
			MainView.outputText( 'a little wet ', false );
		}
		MainView.outputText( 'from the masculine scent the aroused penises are producing.  ', false );
		if( CoC.player.cor < 33 ) {
			MainView.outputText( 'How could you be turned on by such a repulsive situation?  You\'re going to be raped, brainwashed, and either kept as a pet or tossed in a milking tube for the rest of your life and your body is acting like some horny slut!', false );
		} else if( CoC.player.cor < 66 ) {
			MainView.outputText( 'You marvel at just how turned on you\'re getting from the strange situation.  You know you\'ll be raped, drugged, and used as a toy or milk cow, but your loins are thrumming with warm, wet desire.', false );
		} else {
			MainView.outputText( 'How did you wind up in such an arousing situation?  You\'re going to be raped, drugged, and probably milked in a factory for the rest of your life.  Your body is so fucking turned on that you know you\'ll love every second of it, but your desire to triumph and dominate mourns the loss of your freedom.', false );
		}
		MainView.outputText( '  The crowd draws close, but Zetaz\'s voice rings out, thick with the tone of command, "<i>Not yet, my brothers; this one will be mine first.  I\'ll claim each of her holes, then you may each have your fill of her.</i>"\n\n', false );
		MainView.outputText( 'The imps draw back, clearing a path for their leader to emerge, and the new, much more imposing Zetaz climbs atop the table.   He glances at your ' + Descriptors.vaginaDescript( 0 ) + ' with a knowing eye and smiles, walking further forward until he\'s standing next to your face with his tainted, corruption-filled cock dangling overhead.  You\'re so distracted by the purplish-black demon-cock swinging above your lips that the sharp pain takes you completely off-guard. As soon as the discomfort passes you twist your head around to try and find the source of your irritation.\n\n', false );
		MainView.outputText( 'Zetaz turns away from you, holding a spent needle in one of his clawed hands as he exchanges it with one of his kin for another injector, only this one is filled with viscous white fluid.  He glances down at you, watching you intently for some kind of reaction, but you won\'t give him the satisfaction!  Even so, the room is getting so bright that your eyes start tearing up, and you blink repeatedly to rid yourself of them before half-closing your eyelids to shield your poor pupils.  Maybe that\'s what he\'s looking for?  The room spins and you find yourself thankful to be strapped down; even if only seated, you would probably tumble from your chair.\n\n', false );
		MainView.outputText( 'Your lips start to tingle, and you run your tongue over them reflexively.  A shiver of pleasure worms through your body, and you instinctively press your ' + CoC.player.legs() + ' against the straps in an effort to spread them further.  Worse yet, your lips feel much plumper and fuller than a few moments ago.  ', false );
		if( CoC.flags[ kFLAGS.NUMBER_OF_TIMES_MET_SCYLLA ] > 0 ) {
			MainView.outputText( 'Unbidden, Scylla\'s face comes to mind, and you realize the drugs coursing through your veins must be doing something similar to you!  Her visage changes to your own, though the thick, cock-sucking lips remain behind, eager to be penetrated.', false );
		} else {
			MainView.outputText( 'Unbidden, you imagine yourself with thick, cock-sucking lips, so swollen and bloated that they\'re slightly pursed and ready to be penetrated.', false );
		}
		MainView.outputText( '  Warm slipperiness slides over your lips again, feeling nearly as good as it would on your lower lips, and you pull your rebellious tongue back into your mouth with a gasp of pleasure.\n\n', false );
		MainView.outputText( 'This must be what Zetaz was waiting for, and the imp carefully injects the next chemical cocktail into the other side of your neck while you\'re distracted by orally masturbating your new mouth.  Your ' + Descriptors.vaginaDescript( 0 ) + ' ', false );
		if( CoC.player.vaginas[ 0 ].vaginalWetness >= AppearanceDefs.VAGINA_WETNESS_DROOLING ) {
			MainView.outputText( 'gushes fresh fluids into a puddle on the table', false );
		} else if( CoC.player.vaginas[ 0 ].vaginalWetness >= AppearanceDefs.VAGINA_WETNESS_WET ) {
			MainView.outputText( 'drools a heavy flow of liquid arousal onto the hardwood table', false );
		} else {
			MainView.outputText( 'begins to dribble a steady flow of liquid on to the table\'s girl-slicked boards', false );
		}
		MainView.outputText( '.  ', false );
		if( CoC.player.inHeat ) {
			MainView.outputText( 'D', false );
		} else {
			MainView.outputText( 'Foreign d', false );
		}
		MainView.outputText( 'esires wash through your doped up body, and your hungry slit practically demands to be filled with cock and injected with semen.  It wants to be filled with... with males, and with their hot, sticky cum. No, your hot little pussy doesn\'t want that – you do.  Gods above and below, you want to feel your belly pumped full of imp sperm until their offspring are wriggling in your womb.  And then you want them to come in you some more!\n\n', false );
		MainView.outputText( 'That sexy-0... no, that bastard\'s dick is so hard, and he\'s starting to squat down now that you\'re feeling so randy.  The artificial needs coursing through your body make it hard to resist, but you\'ve got to try!  You can\'t open your mouth and... mmm, it feels so good when those nubs touch your bee-stung lips.  Giving in isn\'t an option, even if you can\'t stop him from fucking your mouth, you aren\'t going to curl your tongue around his member and lick it, just like that, sliding it over his bumpy surface until corrupted pre is dripping onto your tongue. Yes, you won\'t let him out of your mouth until you can get his seed inside you, what are the other imps waiting for?  Your other hole is soooo hungry!\n\n', false );
		MainView.outputText( 'The mental incongruities in your thoughts are subsumed in a wave of hot, sticky fuck that\'s slowly rising over your thought processes with each lick and suck of Zetaz\'s thick, sexy dick.  He plunges down, stuffing your greedy gullet with the full length of his elephantine member and letting you know just how much he\'s enjoying your oral cum-hole.  You stick your tongue out to slurp at his desire-filled balls, swooning at the feeling of so much cock-flesh and slippery tongue sliding between your sensitive-as-a-pussy lips.  They twitch and pull tightly against his groin as he grabs your ' + Descriptors.hairDescript() + ' and hilts himself, allowing your lips to seal around his base as his urethra rhythmically bulges with orgasm.  A feeling of warm fullness grows in your gut with each pulse of cum, and you work your throat muscles to squeeze his tip of every last drop while you try to get off on the feelings coming from your mouth.\n\n', false );
		MainView.outputText( 'Once finished, the imp yanks himself up and pulls his orgasm-distended member from your lips with such force that it feels like each of his nubs is flicking your lips.  The orgy of oral pleasure sets off fireworks in your head strong enough to cross your eyes and make you babble incoherent \'thank-you\'s and moans.  You pant happily and lick the residue of Zetaz\'s love from your lips, shivering from the sensitivity and trying to come to grips with what happened.  It doesn\'t do much good – you\'re already getting horny again, and you still haven\'t been knocked up.  Even though you know something about the situation is deeply wrong, you\'re horny as hell and desperately desire to be a mother.  Maybe it\'s just that there\'s all these strong, handsome males here but none of them are fucking your horny, wet twat.  There\'s something wrong with that!', false );
	...
	Dungeon2Supplimental.prototype.femaleZetazOverPtII = function() {
		MainView.outputText( '', true );
		EngineCore.hideUpDown();
		MainView.outputText( 'While you\'re gathering your thoughts, Zetaz staggers back down the table and accepts a flask from one of his lackeys.  He guzzles down the bubbling pink fluid in seconds, and the effect is immediate and greatly pleasing to your fuck-happy worldview.  The imp\'s cock, which had been slowly retracting, thickens at the base and rapidly fills until it\'s hard and twitching with sexual need.  He glances down at your exposed ' + Descriptors.vaginaDescript( 0 ) + ' with a hungry look and drops to his knees, lining the nodule-ringed crown of his wondrous dick up with your lust-juiced slit.\n\n', false );
		MainView.outputText( 'You look down at the male and moan, "<i>Please, hurry up... I need your cum... your babies.  Put your cock inside me!</i>"\n\n', false );
		MainView.outputText( 'Zetaz looks surprised at your words, and you start to wonder why, but the heat and pleasure of his long, thick member spearing your love-canal interrupts your thought process.  He reaches up, and begins to ', false );
		if( CoC.player.biggestTitSize() < 1 ) {
			MainView.outputText( 'tweak your ' + Descriptors.nippleDescript( 0 ) + 's roughly, pulling and yanking on them as', false );
		} else {
			MainView.outputText( 'maul at your ' + Descriptors.allBreastsDescript() + ', slapping and squeezing them as', false );
		}
		MainView.outputText( ' he begins to repeatedly thrust against your ' + Descriptors.vaginaDescript( 0 ) + ', fucking you in earnest.  The wet slap of his balls on your juice-slimed body fills the chamber and sends ripples of pleasure down your ' + CoC.player.legs() + '.  With your eyelids half-closed, your tongue masturbating your lips, and your pussy practically squirting lubricants at the end of each thrust, you must look like every male\'s wet dream.\n\n', false );
		MainView.outputText( 'Looking around, you see a large number of the imps are masturbating, and one of the larger ones has the audacity to speak while his boss is plowing your quim with savage strokes. "<i>Since you already got to use her mouth, I\'m going to put that fuck-hole to use.</i>"\n\n', false );
		MainView.outputText( 'Zetaz waves his hand, though you aren\'t sure if it\'s meant to be a dismissal or permission.  He\'s far too busy sawing away, sending bliss up your spine that makes you giggle and moan with desire.  You\'re already getting close to cumming!  Before you can vocalize just how great it feels, the imp that spoke is straddling your neck and dangling his own member towards the bloated, bimbo-like cum-receptacle that was once your mouth.\n\n', false );
		MainView.outputText( 'The pointed tip of the new imps dick slides through your sensitive orifice with ease, at least until you feel the curvature of his knot pushing apart your jaw.  The utter wrongness of being double-teamed by tiny, huge-cocked demons rears its ugly head, and you knit your brows together as you try to puzzle it out.  What could be wrong?  Your lips feel so good and you\'re about to be pregnant.  Wasn\'t there a reason not to, though?  Something about saving something?  You unconsciously lick at the new invader as his knot finally gets past your lips, humming and sucking while your drug-dulled mind tries to refocus on something other than getting knocked up.\n\n', false );
		MainView.outputText( 'Zetaz grunts and bottoms out, punching his tip into your cervix and blasting a thick rope of seed into your empty, ready womb.  You climax immediately from the act, and moan into the dog-cock that fills your mouth, using it like a ballgag.  There wasn\'t any natural buildup, just spunk hitting your womb and then a climax strong enough to make you see white.  Your ' + Descriptors.vaginaDescript( 0 ) + ' clenches tightly, hugging and squeezing Zetaz\'s potent prick as it dumps more and more of his corrupt demon-spoo into your fertile breeding grounds.  The thick goop tingles in a way that makes you sure you\'ll be giving him a litter of horny little sons before long.  Maybe they\'ll fuck you like they do Vala?\n\n', false );
		MainView.outputText( 'The knot in your mouth pops out, and your belly gurgles, feeling very full.  The second imp must have come while his master was fertilizing your pussy.  You sigh and sag against your restraints as Zetaz steps away and lines begin to form.  In a few seconds, you\'ve got a rubbery, spined cat-cock twitching inside your cunt, and are wrapping your sensitive lips around a horse-cock.  This must be what nirvana feels like.', false );
		CoC.player.orgasm();
	...
	Dungeon2Supplimental.prototype.zetazBadEndEpilogue = function() {
		MainView.outputText( '', true );
		if( CoC.player.gender === 2 ) {
			MainView.outputText( 'The once-champion, ' + CoC.player.short + ' was raped repeatedly by every imp that survived her initial assault.  Her mind never recovered from the initial orgy, and she found herself happy to be named \'Fuck-cow\'.  She quickly became a favorite of Zetaz\'s ever growing brood, and surprised them all with her fertility and rapidly decreasing incubation times.  Within a few months, she was popping out litters of tiny masters even faster than Vala.  Within a year, her body was so well trained and her womb so stretched that she could keep multiple litters growing within at all times.\n\n', false );
			MainView.outputText( 'It was rare for Fuck-cow\'s cunt or mouth to be empty, and she delighted in servicing any male she was presented with.  Her masters even captured bee-girls, so that fuck-cow\'s ass could be kept as pregnant as her belly.  Fuck-cow came to love her masters dearly, and with her constantly growing ability to birth imps, she was able to incubate enough troops for Zetaz to challenge Lethice\'s armies.  The imps never managed to overthrow the old demon lord, but the land was eventually divided in half, split between two growing demonic empires.', false );
		} else if( CoC.player.gender === 3 ) {
			//[Epilogue];
			MainView.outputText( 'The champion was fucked and brainwashed repeatedly for a few more days until Zetaz was sure she understood her place in the world.  Once rendered completely obedient, they released her from her bindings.  It was time she was turned over to Lethice.  ', false );
			if( CoC.player.wingType !== AppearanceDefs.WING_TYPE_BAT_LIKE_TINY || CoC.player.wingType !== AppearanceDefs.WING_TYPE_BAT_LIKE_LARGE ) {
				MainView.outputText( 'Zetaz gave her one of the weaker imps to penetrate and taught her to fly with her new, demonic wings.  ', false );
			} else {
				MainView.outputText( 'Zetaz gave her one of the weaker imps to penetrate during the journey.  ', false );
			}
			MainView.outputText( 'With preparations complete, Zetaz, the champion, and a few dozen imps flew to the mountain peak.\n\n', false );
			MainView.outputText( 'The champion was presented to Lethice, and the demonic mistress was so pleased with Zetaz\'s gift that she gave him a pair of nubile slave-girls and promoted him over a small army of his own kind.  Once the imps departed, Lethice put the champion through her paces, using her as a fucktoy whenever the mood took her.  The rest of the time the champion was kept bound and unable to orgasm, tortured with unholy levels of arousal, but she didn\'t mind.  When Lethice allowed her to cum, the champion\'s orgasms were long and intense enough for her to love her mistress in spite of having to be so pent up.', false );
		} else {
			MainView.outputText( 'The imps never released the champion from that chamber after that.  \'He\' gave birth to a healthy litter of imps a few weeks later, and the hormones from the pregnancy ', false );
			if( CoC.player.biggestTitSize() < 1 ) {
				MainView.outputText( 'created a decent set of chest-bumps.', false );
			} else {
				MainView.outputText( 'swelled her already impressive rack with milk.', false );
			}
			MainView.outputText( '  After that, the imps really took a liking to her, and she was let down from her restraints.  She never got much chance to get up though; she was well and truly fucked at every opportunity.  She was already hooked.  With her incredible libido and the constant fucking, staying was the easy choice.\n\n', false );
			MainView.outputText( 'After a few months the champion started to become acclimated to her new life, and began birthing imps in larger broods with shorter gestations.  She had become the ideal broodmother, and her worldview shrank down to two powerful priorities: acquiring cum, and birthing.', false );
		}
		CoC.player.orgasm();
		CoC.player.HP += 150;
		EngineCore.gameOver();
	...
	Dungeon2Supplimental.prototype.hermZetazOver = function() {
		...
		MainView.outputText( ' you as you\'re thrown atop one of the tables.   You grunt as leather straps are produced and laid over your form to restrain you.  In the span of a minute you\'re completely immobilized from the neck down, and your ' + CoC.player.legs() + ' are kept spread to allow easy access to your ' + Descriptors.vaginaDescript( 0 ) + '.\n\n', false );
		MainView.outputText( 'Your willpower starts to come back, and you struggle in vain against the tight leather straps, accomplishing nothing.  Zetaz leers down at your double-sexed form and roughly manhandles both your male and female organs as he taunts, "<i>I don\'t remember ', false );
		if( CoC.player.cockTotal() === 1 ) {
			MainView.outputText( 'both', false );
		} else {
			MainView.outputText( 'all', false );
		}
		MainView.outputText( ' of these being here when we met.  Did you sample some incubi draft?  Or did you guzzle some succubi milk?  Perhaps both?  In any event, I think you could do with a little more of each.</i>"\n\n', false );
		MainView.outputText( 'Oh no.  Your eyes widen in fear at his bold declaration, but Zetaz only throws back his head and laughs, "<i>Oh yes!</i>"  He turns to the mob and orders something in a tongue you don\'t understand, then returns to fondling your ' + Descriptors.cockDescript( 0 ) + '.  "<i>How perverse.  Why would you have something like this when you have such a beautiful pussy hiding below it?</i>" asks the imp lord.  Despite his questioning words, he doesn\'t stop stroking you until you\'re full, hard and twitching.  Your poor ' + Descriptors.vaginaDescript( 0 ) + ' is aching from being ignored with all this building sexual tension.\n\n', false );
		MainView.outputText( 'The sounds of numerous footfalls and clinking glass signal that the mob of imps has returned, bringing what sounds like hundreds of vials worth of their foul concoctions.  Zetaz releases your tumescent member and reaches over for something, then returns to your view bearing a ring gag.  Even turned on, defeated, and immobilized on a table, you try your best to fight him, but all that gets you is slapped.  The imp\'s palm smacks you hard enough to stun you and leave your ears ringing, and when you blink the stars from your eyes, your mouth is forced open with your tongue hanging out lewdly.\n\n', false );
		MainView.outputText( 'Another of Zetaz\'s brothers, or perhaps sons, hands him a tube with a funnel, and he easily threads the funnel\'s tube through the ring gag.  Foul remnants of whatever it was used for last leave a sour taste on your tongue, but worse yet is the knowledge that you\'re going to be force-fed tainted, body-altering, mind-melting drugs.  A drop of pre-cum hits your belly and your thighs grow ', false );
		if( CoC.player.vaginas[ 0 ].vaginalWetness < AppearanceDefs.VAGINA_WETNESS_DROOLING ) {
			MainView.outputText( 'sticky', false );
		} else {
			MainView.outputText( 'soaked', false );
		}
		MainView.outputText( ' from the thoughts.  ', false );
		if( CoC.player.cor < 33 ) {
			MainView.outputText( 'Are you really being turned on by such lewd, debased thoughts?', false );
		} else if( CoC.player.cor < 66 ) {
			MainView.outputText( 'Are you this much of a pervert?  Yes, it\'ll feel good, but you\'re a little ashamed of your body\'s immediate and lewd reaction.', false );
		} else {
			MainView.outputText( 'Are you really this much of a submissive?  Yeah, sucking down drinks like this is hot as hell, but you\'d like to be doing it on your own terms.  At least you\'ll probably start cumming after a few bottles worth of the stuff.', false );
		}
		MainView.outputText( '\n\n', false );
		MainView.outputText( '"<i>Hey boss!  She\'s already starting to drip!  To think she tried to fight us.  She\'s showing us her true nature – that of a pervert-slut,</i>" raves one of the horde.  You can\'t pick out the source of his voice in the crowd, but the words sting enough to make your whole body blush with ', false );
		if( CoC.player.cor < 33 ) {
			MainView.outputText( 'shame', false );
		} else if( CoC.player.cor < 66 ) {
			MainView.outputText( 'confusion', false );
		} else {
			MainView.outputText( 'arousal', false );
		}
		MainView.outputText( '.  The imp lord nods in agreement and upends the first bottle over the funnel, channeling fragrant white fluid into your mouth.  It tastes fantastic!  Your throat instinctively gulps down the creamy delight before you can make a conscious decision.  The effect is immediate and strong.  Warmth builds on your chest as weight is added to your ' + Descriptors.allBreastsDescript() + ' while a gush of fluid squirts from your ' + Descriptors.vaginaDescript( 0 ) + '.\n\n', false );
		MainView.outputText( 'Zetaz is just getting started.  Before you have time to react to your predicament, the next bottle is empty and thicker cream is flooding your mouth.  You don\'t swallow for a moment, so the imp pours another bottle in, backing up more of the fluid.  Faced with a choice between corruption and drowning, you try to gulp down enough liquid to breathe.  ' + CoC.player.SMultiCockDesc() + ' puffs and swells, spurting thick ropes of cum as it adds a half-dozen inches to its length.  Your eyes cross from the sudden change, but you get a fresh breath before the imps begin to pour several bottles in at once.\n\n', false );
		MainView.outputText( 'You swallow in loud, greedy gulps as your body is slowly warped by the fluids you\'re consuming.  Though your ' + Descriptors.allBreastsDescript() + ' and ' + CoC.player.multiCockDescriptLight() + ' sometimes shrink, they grow far more often, and after a few minutes of force-feeding, you\'re pleading for more each time they stop to let you breath.  You\'re a mess of sexual fluids, your tits are squirting milk, and your pussy squirts from every touch.  Demon horns are swelling from your brow, curling back over your ears', false );
		if( CoC.player.horns > 0 ) {
			MainView.outputText( ' and adding to your existing pair', false );
		} else {
			MainView.outputText( ' and giving you an exotic, tainted appearance', false );
		}
		MainView.outputText( '.  ', false );
		if( CoC.player.lowerBody !== AppearanceDefs.LOWER_BODY_TYPE_DEMONIC_HIGH_HEELS ) {
			MainView.outputText( 'Your ' + CoC.player.feet() + ' have been changing throughout the ordeal, but you didn\'t notice your ' + CoC.player.legs() + ' becoming such lissom, lengthy legs, or your heels growing long, high-heel-like spikes.  ', false );
		}
		if( CoC.player.tailType !== AppearanceDefs.TAIL_TYPE_DEMONIC ) {
			MainView.outputText( 'A tail snakes around your leg and begins to caress your ' + Descriptors.vaginaDescript( 0 ) + ', then plunges in to fuck the squirting orifice while you drink.  ', false );
		} else {
			MainView.outputText( 'Your tail snakes around your leg and begins to caress your ' + Descriptors.vaginaDescript( 0 ) + ', then plunges in to fuck the squirting orifice while you drink.  ', false );
		}
		MainView.outputText( 'The imps start hooting and cat-calling, laughing and prodding your body with their twisted demonic members as your mind starts to come apart in the seething oven of unnatural lust.\n\n', false );
		//NEXT;
		EngineCore.dynStats( 'lib', 100, 'sen', 100, 'lus=', 1000, 'cor', 50 );
	...
	Dungeon2Supplimental.prototype.hermZetazOverPtII = function() {
		EngineCore.hideUpDown();
		MainView.outputText( '', true );
		MainView.outputText( 'You awaken midway through a loud moan and nearly jump out of your ' + CoC.player.skinDesc + ' in surprise, but the fire of your unnaturally stoked libido immediately reasserts yourself.  You twitch your hips to and fro, thrusting against a ', false );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 'number of ', false );
		}
		MainView.outputText( 'mechanical milking device', false );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 's', false );
		}
		MainView.outputText( '.  ' + CoC.player.SMultiCockDesc() + ' is sucked rhythmically, producing a loud, wet, slurping noise that echoes around the small room.  You\'re suspended from a set of shackles on the wall, next to Vala.  The sexy faerie is chained up in a similar manner, but she\'s locked in coitus with a well-endowed imp, and making no secret of her enjoyment.  The sexual sight stirs your well-stimulated loins and you groan, filling the milker with what feels like gallons of male cream over the next minute and a half.\n\n', false );
		MainView.outputText( 'Slumping forwards, you hang there, but the corruption and lust in your blood refuses to be sated.  ' + CoC.player.SMultiCockDesc() + ' is already hard again, and after sucking your cum down some tubes, the milker begins its oh-so-pleasurable work again.  Still, you estimate it will be a few minutes before it gets you off again, so you look around the room.  A platform is set up in front of you, about knee-height and poorly built.  Judging by its height, it\'s probably there so that the imps can use it to fuck you without having to fly.  There\'s also a pair of platforms built into the walls next to each of your shoulders, though their function is less clear.\n\n', false );
		MainView.outputText( 'The door to the room bangs open, and Zetaz steps in, followed by two scrawnier-than-usual imps.  He smiles when he sees you awake and flushed, and steps up onto the platform, rubbing his palms together in excitement.\n\n', false );
		MainView.outputText( '"<i>You took quite well to our little experiment,</i>" he announces, "<i>In fact, your body is a demonic fucking machine.  I won\'t be transforming you into an actual demon though.  But we\'re going to have to have a little training to get you ready to meet Lethice.  After all the trouble you\'ve caused her, she might want to turn you herself, or maybe hook you up in a factory?  I can\'t say for sure.</i>"\n\n', false );
		MainView.outputText( 'With a flourish, the imp lord discards his loincloth, tossing it over his shoulder to reveal his erect demon dick.  He taunts, "<i>Like what you see?</i>" and orders his lackeys, "<i>Go on, you know what to do.</i>"  The pair of scrawny imps flit up to their perches while Zetaz advances and strokes himself, preparing for penetration.  Dozens of unanswered questions swarm through your mind, actually distracting you from your pending orgasm enough to ask, "<i>Wha-what are you going to do to me?</i>"\n\n', false );
		MainView.outputText( '"<i>Shhhh, shhh,</i>" responds Zetaz, "<i>just relax my pet.</i>"  He ', false );
		if( CoC.player.balls > 0 ) {
			MainView.outputText( 'gently shifts your ' + Descriptors.ballsDescript() + ' out of the way and ', false );
		}
		MainView.outputText( 'lines up with your drooling fuck-hole, and with a long smooth stroke, he\'s inside you.  You cum immediately and hard, barely noticing the chanting that has started up on the adjacent platforms.  Each squirt of cum is accompanied by a thrust from Zetaz, sliding over your lube-leaking walls with ease.  The orgasm lasts nearly twice as long as your last one.  It never seems to end, but when it slowly trails off, you find yourself wondering how soon you can cum again.\n\n', false );
		MainView.outputText( 'You envision yourself on all fours, being taken in both openings by a pair of imps while you suck off a shadowy figure that your mind recognizes as your lord and master.  ' + CoC.player.SMultiCockDesc() + ' spurts and squirts with each penetration as your twin violators get off and stuff you full of their yummy imp cum, glazing your insides with corrupted white goo.  Maybe you\'ll get pregnant this time?  It\'s been a few weeks since your last litter.  You suck harder on your master\'s penis and caress his balls until he shows his affection by giving you a salty treat.  He pulls out and blasts a few ropes over your face and hair, so you do your best to look slutty to encourage him.  When he finishes, you lick your lips and beam at your master, Zetaz.\n\n', false );
		MainView.outputText( 'Wait- what!?  You shake your head and clear away the fantasy, though your sexual organs\' constant delightful throbbings aren\'t doing much to help.  Zetaz is still fucking your pussy, taking it with long slow strokes that would\'ve made your demonic legs give out ages ago if you weren\'t hanging on a wall.  The chanting is so loud, so insidious.  You can feel it snaking through your brain, twisting your thoughts and ideas.  You close your eyes, desperate to fight it, but it only enhances the sensation of dick-milking and being fucked by your- no, by that demon!\n\n', false );
		MainView.outputText( 'Glancing down at him, you remark that the little bastard is quite handsome for an imp.  With his perfect jawline and marvelous cock, you find yourself hard-pressed to justify resisting him so long ago.    How did you resist his charms?  His cock feels soooo fucking good inside you.  With an explosive burst, ' + CoC.player.sMultiCockDesc() + ' erupts again, squirting thick arousal and submission into the milker while your ' + Descriptors.vaginaDescript( 0 ) + ' wrings Zetaz\'s nodule-ringed cock incessantly.  His turgid member bulges obscenely, and he starts to cum inside you, squirting master\'s thick seed into your breeding hole.  Breeding hole?  Why would you call your slutty fuck-hole a breeding hole?  Something seems off about that last thought, but you can\'t place it.\n\n', false );
		MainView.outputText( 'Your master finishes squirting inside you and withdraws, pawing at your milk-leaking teats for a moment as you continue to shudder and cum like a good bitch.  Wow, you really are a good bitch aren\'t you?  Pride wells in your breast as the imp\'s chanting reaches a crescendo and a relaxed smile forms on your ' + CoC.player.face() + '.  Yes, you\'re a good, breeding bitch.   Master is smiling up at you and you know you\'ve made him feel very happy.  Hopefully he\'ll come back soon and fuck you some more.  Your pussy feels so empty without him.', false );
		CoC.player.orgasm();
		EngineCore.dynStats( 'cor', 50 );
		EngineCore.doNext( this, this.zetazBadEndEpilogue );
	};
	...
	Dungeon2Supplimental.prototype.malesZetazOver = function() {
		...
		MainView.outputText( ' you as you\'re thrown atop one of the tables and rolled onto your side.   You grunt as leather straps are produced and laid over your form to restrain you.  In the span of a minute you\'re completely immobilized from the neck down, and your ' + CoC.player.legs() + ' are kept spread to allow easy access to ' + CoC.player.sMultiCockDesc() + ' and ' + Descriptors.assholeDescript() + '.\n\n', false );
		MainView.outputText( 'Zetaz leaps atop the table in a single bound, the barely concealed bulge in his loincloth dangling freely underneath.  You begin to struggle, fearful of the cruel imp\'s intentions and ', false );
		if( CoC.player.ass.analLooseness < 4 ) {
			MainView.outputText( 'worried he\'ll try to force the mammoth between his thighs into your backdoor', false );
		} else {
			MainView.outputText( 'worried he\'ll take advantage of your well-stretched backdoor', false );
		}
		MainView.outputText( ', but your feverish efforts are in vain – the restraints are too strong!  The imps start to laugh at your predicament, and Zetaz pushes the humiliation a step further by stepping squarely on your groin, painfully squeezing your ' + Descriptors.cockDescript( 0 ) + ' with his heel.  He throws his arms up in the air and shouts, "<i>I am your champion!  I have brought the scourge of our kind to his knees, and ground him under my heel!</i>"\n\n', false );
		MainView.outputText( 'You whine plaintively and squirm under the imp\'s heel, utterly humiliated and helpless.  Zetaz smirks down at your taunts, "<i>What\'s the matter?  Is something bothering you?</i>"  He raises his foot, letting you gasp, "<i>Thank you,</i>" before he delivers a kick to your gut, knocking the wind out of you.  Restrained as you are, your body convulses underneath the leather, trying to curl up while your diaphragm spasms repeatedly.  A strap is fastened around your head, and a ring gag is slipped into your mouth, holding it open and ready for whatever sick plans the imps have devised.\n\n', false );
		MainView.outputText( 'The imp lord gestures at his underlings with an irritated scowl while you catch your breath, and the horde scrambles to satisfy him before they can draw his ire.  A funnel with a clear tube suspended from the bottle is passed from the mass of bodies up to Zetaz, along with a few bottles filled with roiling pink and red fluids.   The funnel\'s exit-tube is threaded into your ring gag and there\'s nothing you can do but grunt in wide-eyed panic while it\'s secured in place.  The first bottle of what you assume to be lust-draft is upended into the funnel, and there\'s nothing you can do but drink or drown.\n\n', false );
		MainView.outputText( 'It has a bubblegum-like taste that makes your tongue tingle as it passes into your belly, but the more pressing sensation of ' + CoC.player.sMultiCockDesc() + ' getting rock-hard lets you know exactly what you just drank.  Even though you just finished chugging down that foul drink, the imps uncork another pair of potions and dump them into the funnel.  The sweet fluids flood your mouth, and once again you swallow and chug rather than drown.   After you finish the last swallow, you pant, completely out of breath and getting hotter by the moment.  Your ' + CoC.player.skinDesc + ' tingles and sweats, growing more and more sensitive with every passing second while ' + CoC.player.sMultiCockDesc() + ' begins to drip and drool.\n', false );
		MainView.outputText( 'Zetaz hands the funnel to an underling with a knowing laugh and repositions himself over your ' + CoC.player.legs() + '.  Warm pressure pushes at your ' + Descriptors.assholeDescript() + ', forcing your clenching flesh to yield around the intruder.  Normally such an instant penetration would be irritating, or perhaps painful, but the sudden pressure on your prostate only serves to release a copious squirt of pre-cum.  An unwelcome moan slips past your lips and sends a titter of laughter through the mob.  As if losing wasn\'t bad enough – they all know you\'re getting off on having your ' + Descriptors.assDescript() + ' penetrated.  The worst part is that the humiliation is just making the situation hotter and ' + CoC.player.sMultiCockDesc() + ' harder.\n\n', false );
		MainView.outputText( 'You nearly choke as an unexpected wave of potions washes through the funnel into your mouth, but you start swallowing and gulp down what feels like a half-dozen lust potions before you can breathe again.  ' + CoC.player.SMultiCockDesc() + ' starts squirting and spurting, dumping heavy loads of cum onto the table and your belly from the effects of the potions alone.  Zetaz gathers a massive dollop in his hand and smears it over himself, using it as lubricant to penetrate your poor, beleaguered asshole with savage, rough strokes that smash against your prostate at the apex of each thrust.  You moan loudly and lewdly through the tube in your mouth, wriggling against your restraints and spurting helplessly as you\'re penetrated over and over.\n\n', false );
	...
	Dungeon2Supplimental.prototype.incubusDeal = function() {
		MainView.spriteSelect( 52 );
		MainView.clearOutput();
		MainView.outputText( '"<i>Excellent!  Give me a few moments to gather my things and I\'ll be open for business!</i>" exclaims the strange demon.  If his story is true it\'s no wonder he doesn\'t get along with the rest of his kind.' );
		//[Next – to room];
	...
	Dungeon2Supplimental.prototype.incubusNoDeal = function() {
		...
		MainView.clearOutput();
		CoC.flags[ kFLAGS.ZETAZ_LAIR_DEMON_VENDOR_PRESENT ] = -1;
		MainView.outputText( 'Sean nods, grabs a pack, and takes off running before you have a chance to kill him.' );
		EngineCore.doNext( MainView, MainView.playerMenu );
	};
	Dungeon2Supplimental.prototype.incubusShop = function() {
		...
		if( CoC.player.hasItem( ConsumableLib.BIMBOCH ) && CoC.flags[ kFLAGS.NIAMH_SEAN_BREW_BIMBO_LIQUEUR_COUNTER ] === 0 ) {
			...
			if( CoC.player.hasItem( ConsumableLib.BIMBOCH, 5 ) ) {
				MainView.outputText( '  Luckily, you do!' );
	...
	Dungeon2Supplimental.prototype.incubusBuy = function( itype ) {
		MainView.spriteSelect( 52 );
		MainView.clearOutput();
		MainView.outputText( 'The incubus lifts ' + itype.longName + ' from his shelves and says, "<i>That will be ' + (itype.value * 3) + ' gems.  Are you sure you want to buy it?</i>"' );
	...
	Dungeon2Supplimental.prototype.valaBigYou = function() {
		...
		CoC.flags[ kFLAGS.VALA_TIMES_CONSENSUAL_SEX ]++;
		//{FIRST TIME};
		if( CoC.flags[ kFLAGS.TIMES_VALA_CONSENSUAL_BIG ] === 1 ) {
			MainView.outputText( 'Tilting your head, eyes glowing with ethereal power, you answer, "<i>I want you... big.</i>"' );
		...
		//{REPEAT};
			...
			MainView.outputText( 'You tell Vala just how HUGE your need for her is, which earns you a blush from the petite faerie\'s fair cheeks.  She calls in a favor from another waitress - looks like Vala covered for this one\'s tryst with Chad as well.  That Chad guy must get around!' );
			...
			if( EngineCore.silly() ) {
				MainView.outputText( 'The last thing you need is a bunch of winged archers taking potshots at Vala while she clings to a tower with you in her hand...  ' );
			}
			MainView.outputText( 'She seems to know the way directly to your camp, and all too soon, you\'ve reached your place of residence.  Vala cheekily pokes your [chest] and asks, "<i>Shouldra?</i>"' );
		}
		//{MERGE};
	...
	Dungeon2Supplimental.prototype.bigValaDomsPC = function() {
		MainView.clearOutput();
		MainView.outputText( 'Vala tilts her head to the side, causing a gust of displaced air to blow your ' + Descriptors.hairDescript() + ' wildly.  "<i>Giving the maiden a chance to lead?  You\'re so sweet!</i>"  She tickles your chin with one immense fingertip.  "<i>But you had better give it your all, you adorable rascal, you.</i>"' );
		...
		if( CoC.flags[ kFLAGS.VALA_HEALED_HONEY ] === 0 ) {
			MainView.outputText( '"<i>If you do a good job, I\'ll let you taste a pure pearl of my own,</i>" she giggles.' );
		}
		...
		if( CoC.player.gender === 1 ) {
			...
			if( CoC.player.lust >= 70 ) {
				MainView.outputText( ', already hard, begins to bead with pre-cum, mixing with the slut-leakings that have found their way under your equipment.' );
			} else {
				MainView.outputText( ' stiffens immediately to a full, throbbing hardness.' );
			}
			MainView.outputText( '  Your body is imminently ready for breeding, erect and wanting so badly to be freed from your constricting garb, but you know that with what you asked, you won\'t get to sate it.' );
			if( CoC.player.biggestCockArea() >= 150 ) {
				MainView.outputText( '  If only you weren\'t so big - you can\'t expand to your full size, and the tightness is almost emasculating at how it constrains your size, keeping you small enough to fit inside your [armor] but still hard as a stone, only this stone is having the pre squeezed out of it.' );
			}
		}
		//{Female:};
		else if( CoC.player.gender === 2 ) {
			...
			if( CoC.player.wetness() >= 3 ) {
				MainView.outputText( 'Your [vagina] seems to always be wet, but now, with this sight before you, your girl-cum is running in rivers down your [legs] to mix into the puddling lady-spunk below.  ' );
			}
			MainView.outputText( 'Your armor seems tight and constricting against your rock-hard [nipples].  It\'s something you can\'t escape from, and with each passing moment, the sensations coming from your chest seem more and more worrisome, stoking your fiery lust to an incredible degree.' );
		}
		//{Herm:};
		else if( CoC.player.gender === 3 ) {
			MainView.outputText( '\n\nShuddering from the onslaught of lusty pheromones, you find [eachCock] responding even before you realize what\'s going on.  ' );
			if( CoC.player.lust >= 70 ) {
				MainView.outputText( 'You may have already been uncomfortably hard, but you start leaking pre-cum all over the inside of your [armor].  Of course, the copious fem-jizz that\'s already seeped inside your gear absorbs the extra fluid with little to no complaint.' );
			} else {
				MainView.outputText( 'You get hard with alarming rapidity.  Being so close to such a gloriously large quim seems to overpower your better sense, and you\'re forced to give in to your body\'s imperative.' );
			}
			MainView.outputText( '  Inside your undergarments, everything is already turning into a swampy, slimy mess thanks to Vala\'s secretions, and your own feminine drippings don\'t help any.  Your lips and dick' );
			if( CoC.player.cockTotal() > 1 ) {
				MainView.outputText( 's' );
			}
			MainView.outputText( ' are smothered with the stuff, making you moan without meaning to.' );
			if( CoC.player.biggestCockArea() >= 150 ) {
				MainView.outputText( '  If only you weren\'t so big!  You\'re compressed so tightly by your enclosing [armor], and even though you\'re rock-hard, you\'re a fraction of your full size.  If only you could get out of your gear and fuck her!' );
			}
		}
		//{Genderless};
		else {
	...
	Dungeon2Supplimental.prototype.bigValaLicksOffDudes = function() {
		MainView.clearOutput();
		MainView.outputText( 'Giant Vala cocks an eyebrow at your request in surprise.  "<i>But, what if I accidentally swallow?  I couldn\'t live with myself if something should happen to my big... er, little Hero.</i>"  She pats you on the head affectionately, unsure of how to handle herself with all of her newfound size.' );
		...
		if( CoC.player.hasFuckableNipples() ) {
			MainView.outputText( '  It even pauses at one to slip a tiny bit inside, too big to penetrate your nipplecunt properly but teasing it all the same.' );
		}
		MainView.outputText( '  Finally, it encircles the back of your neck and curls up to slide across your chin.  You greedily slurp the tip of her oral muscle into your own mouth, and the effect is immediate.' );
		...
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 'each' );
		} else {
			MainView.outputText( 'your' );
		}
		MainView.outputText( ' length, unholy smoothness sliding ever so slowly back and forth as she moans.  The giant faerie drags you further inside, much to your delight.  Agonizingly sensuous pressure mounts all along your length' );
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 's' );
		}
		MainView.outputText( ' until ' );
		if( CoC.player.cockTotal() === 1 ) {
			MainView.outputText( 'it slides' );
		} else {
			MainView.outputText( 'they slide' );
		}
		MainView.outputText( ' inside her' );
		if( CoC.player.biggestCockArea() >= 200 ) {
			MainView.outputText( ', the immense length sucked in by a surprising burst of suction' );
		}
		MainView.outputText( '.' );
		...
		if( CoC.player.biggestCockArea() < 200 ) {
			MainView.outputText( 'smothering your relatively tiny pecker' );
		} else {
			MainView.outputText( 'wrapping up your immense pecker' );
		}
		if( CoC.player.biggestCockArea() >= 400 ) {
			MainView.outputText( ', even as it hangs partway down her throat' );
		}
		MainView.outputText( ', you begin to moan as well, subsumed in steamy pleasure.' );
		...
		if( CoC.player.balls > 0 ) {
			MainView.outputText( ', [balls] clenching against Vala\'s tongue' );
		}
		MainView.outputText( ' as you explode into ecstatic, jerking pumps against her slippery, sensitive tongue.  Vala\'s voice rises with yours the moment the first spurt of seed stains her tongue, and her fem-cum sprays out to soak her thighs with enough force to carry some smaller boulders away in the tide.' );
		if( CoC.player.cumQ() < 250 ) {
			MainView.outputText( '  She barely notices the spunk you shoot against her tongue, her drooling saliva washing it away almost immediately, but you\'re sure she got a nice, salty thrill from it regardless.' );
		} else if( CoC.player.cumQ() < 1000 ) {
			MainView.outputText( '  She has to noisily gulp down the liquid deposit you leave in her mouth, but with her enhanced size, it\'s still no more than a creamy morsel.' );
		} else if( CoC.player.cumQ() < 4000 ) {
			MainView.outputText( '  You slowly flood her mouth with spooge until her cheeks bulge.  The spooge inundates her maw until your lower body floats in it, forcing the girl to swallow hard or risk having the teeming seed spill out around you.' );
		} else {
			MainView.outputText( '  You immediately flood her mouth, bulging her cheeks with obscene amounts of spunk.  She noisily gulps, mouth working futilely to handle it all, but it\'s not enough, and jizz begins to burst from the corners of her mouth, almost pushing you out with it.' );
		}
		MainView.outputText( '  Her tongue grinds back and forth underneath you, encouraging you to deposit every single drop into her.' );
	...
	Dungeon2Supplimental.prototype.valaVapulaThreesome = function() {
		...
		//{Repeat}e;
			...
			MainView.outputText( 'You call for Vapula, which brings an amused smile to your giant faerie\'s face.' );
	...
	Dungeon2Supplimental.prototype.valaDommyVapula3Some = function() {
		...
		if( !CoC.player.hasCock() ) {
			MainView.outputText( 'dildo' );
		} else {
			MainView.outputText( '[cock smallest]' );
		}
		MainView.outputText( ' slides into the demoness\'s waiting snatch, which slurps it down with a hungry contraction, slapping your [hips] against hers, the slippery, demonic folds wriggling around you with such insistence that you couldn\'t pull out if you want to.' );
		...
		if( CoC.player.hasCock() ) {
			MainView.outputText( 'seed' );
		} else {
			MainView.outputText( 'fake seed' );
		}
		MainView.outputText( ' without meaning to, the lustiness of the situation just far beyond your ability to bear.' );
		if( CoC.player.cumQ() > 500 ) {
			MainView.outputText( '  The demon\'s purple belly balloons with cum, swelling against your own' );
		}
		if( CoC.player.cumQ() >= 1500 ) {
			MainView.outputText( ' as her midriff turns taut and dome-like' );
		}
		if( CoC.player.cumQ() > 500 ) {
			MainView.outputText( '.' );
		}
		if( CoC.player.cumQ() > 1000 ) {
			MainView.outputText( '  Waves of spunk pour out from Vapula\'s lust-swollen lips thanks to your exceedingly potent orgasm.' );
		}
		if( CoC.player.lactationQ() >= 100 ) {
			MainView.outputText( '  Even better, your [fullChest] lets down your milk, letting it run out of you in ' );
			if( CoC.player.lactationQ() <= 200 ) {
				MainView.outputText( 'thick' );
			} else if( CoC.player.lactationQ() <= 2000 ) {
				MainView.outputText( 'powerful' );
			} else {
				MainView.outputText( 'enormous' );
			}
			MainView.outputText( ' streams, tinting Vala\'s secretions white.' );
		}
	...
	Dungeon2Supplimental.prototype.valaLoveyVapula3Some = function() {
		...
		if( !CoC.player.hasCock() ) {
			MainView.outputText( ', and you buckle on your demonic strap-on, just in case' );
		}
		MainView.outputText( '.  Holding your arms out to either side, you let Vala\'s cunt-soaked hand come out to lift you, a finger under each armpit and one between your legs, slickly sliding against the sensitive, hard prong there.' );
		MainView.outputText( '\n\nYou\'re lifted up high, which only serves to slide ' );
		if( !CoC.player.hasCock() ) {
			MainView.outputText( 'the dildo' );
		} else {
			MainView.outputText( '[oneCock]' );
		}
		MainView.outputText( ' even harder into your flawless faerie-skin, you moan out loud, matching Vapula\'s lascivious cries as she\'s mercilessly teased.  Your [feet] are brought up against Vala\'s immense lip, quickly sliding over the oral protrusion and into the steamy, warm cavern behind it.  The demon\'s shoulder brushes against your own, enough to rouse her from her sexual stupor and have her turn her head to face you.  Mischief twinkles in her lust-lidded eyes as she asks, "<i>Oh, are you going to feed me, [Master]?</i>"' );
		...
		if( !CoC.player.hasCock() ) {
			MainView.outputText( 'that exotic dildo' );
		} else {
			MainView.outputText( 'your [cock smallest]' );
		}
		MainView.outputText( '.  You grab hold of her ass and the tongue that\'s slipping through her crack and hammer your hips home, slapping against Vapula so hard that the violet woman\'s cries of bliss are interrupted by the breath-stealing impacts. ' );
		...
		if( !CoC.player.hasCock() ) {
			MainView.outputText( 'spurting dildo' );
		} else {
			MainView.outputText( '[cock smallest]' );
		}
		MainView.outputText( ', ensuring that every single drop is passed into her ravenous gash.' );
	...
	Dungeon2Supplimental.prototype.kinathisValaStuff = function() {
		...
		MainView.outputText( 'With it so late at night, there are barely any patrons at the bar.  It\'s likely the bar will close up soon.  Lounging in one of the empty, comfortable booths off to the side, you wave over the short, busty bar-fairy.  Vala flutters over to you, a frothy mug in hand, smiling brightly at the sight of you.  The lovely fairy girl always seems happy to see you; rescuing her from the caves really did endear you to her a great deal.' );
		...
		if( CoC.player.tallness > 75 ) {
			MainView.outputText( '  She looks up at you as you seem to tower over her, a' );
		} else {
			MainView.outputText( '  A' );
		}
		MainView.outputText( ' faint blush on her cheeks as she smiles and hugs your arm, beckoning you to follow.' );
		...
		if( CoC.player.cockTotal() > 1 ) {
			MainView.outputText( 'most appealing phallus' );
		} else {
			MainView.outputText( Descriptors.cockDescript( x ) );
		}
		MainView.outputText( ' and starts to stroke it.  Leaning down, she licks her lips and gets down between your legs, her eyes and body set on tasting your hard-on.' );
		//[cock area <17:];
		if( CoC.player.cockArea( x ) < 17 ) {
		...
		MainView.outputText( '\n\nHer diminutive tongue sucks around your ' + CoC.player.cockHead( x ) + ', teasing and dancing circles around you.  Her silky hair feels good around your fingers, making the perfect hand-hold as you keep her in place as she sucks and plays with you.  Pulling back with a slurp and a gasp, Vala licks her lips before kissing her way down your shaft.  ' );
		if( CoC.player.cocks[ x ].cockType === CockTypesEnum.HORSE ) {
			MainView.outputText( 'Her tongue licks around the throbbing glands, the heady animal musk filling her nose and mind with sweet lust.  ' );
		} else if( CoC.player.cocks[ x ].cockType === CockTypesEnum.DOG ) {
			MainView.outputText( 'Her tongue licks around the hot building knot that gently throbs at your base.  ' );
		}
		//[anemone cock:;
		else if( CoC.player.cocks[ x ].cockType === CockTypesEnum.ANEMONE ) {
			MainView.outputText( 'Her tongue plays with the squirming tentacles that line the drone of your throbbing phallus.  She shivers and groans as the tentacles sting her full of pleasurable aphrodisiac.  ' );
		}
		MainView.outputText( 'Playing her way down your cock, the fey-girl gently licks up and down, sucking along your cumvein before coming back to the ' + CoC.player.cockHead( x ) + ' of your meaty prick.' );
		...
		MainView.outputText( '\n\nNot wanting to wait a moment longer, you surge forward, pushing your ' + Descriptors.cockDescript( x ) + ' between her  thighs, the ' + CoC.player.cockHead( x ) + ' gently probing the small tight sex that lay between them, her fey-honey oozing from the womanly flower like sweet nectar from the most exotic of plants.  The touch of your meaty masculinity against her puffy, sensitive nethers draws a gentle gasp from the excited woman.  Taking hold of her hips, you push forward and sink your aching, throbbing spear into her tender, needy body.  ' );
		//[cockarea <17:;
		if( CoC.player.cockArea( x ) < 17 ) {
			MainView.outputText( 'Despite her size, you slide in easily, her wet, slippery pussy taking you in and squeezing you in a velvety embrace.  "<i>Yessss, it\'s in, it\'s in!  Mmmm, come to me hero, come to Vala,</i>" she croons to you, beckoning  you to take her.' );
		} else if( CoC.player.cockArea( x ) < 30 ) {
			MainView.outputText( '"<i>Ahhh...  yesss, it\'s in!  Ohh...  come to me hero, come to Vala and fill her with your love!</i>" she croons to you, beckoning  you to take her.' );
		} else {
			MainView.outputText( 'Ohhh! Ahhh...  Y-Yessss...  Vala’s big strong hero, oh you feel so good inside me!</i>" she moans and groans as you stretch her tight little fairy cunt with your pulsing prick.  Her stretchy flesh distends as more and more of your cock is fed into her hungry pussy.' );
		}
		...
		if( CoC.player.cockArea( x ) > 17 ) {
			MainView.outputText( '  Her hot little body stretches around your girthy prick, distending her belly as you sheath your throbbing arousal into her.' );
		}
		MainView.outputText( '\n\nYour hands roam over her as you buck your hips into your fairy lover, your hands groping her round ass, her massive milky mammaries swaying and jiggling from your hard thrusts.  The bounce and ripple of her creamy udders is too much to bear, their hypnotic shakes drawing your hands to them, squeezing and manhandling those luscious orbs.  Spurts of rich milk splash across your ' + CoC.player.skinFurScales() + ' as you pinch and squeeze her nipples.  The rough touches, the pinching, the squeezes and gropes draw long moans and groans from the panting, lusty fairy.  Your fey-lover murmurs your name as you take her, beckoning you for more, for you to keep fucking her and to make her scream.  "<i>Haa haa, ohhhh, yessss fuck me, fuck Vala’s tight little cunt!  Give me all your burning love!</i>" she pants and cries out as she bucks her hips up to you, her arms reaching up to hold onto you as you start to speed up your thrusts.' );
		...
		if( CoC.player.balls > 0 ) {
			...
			if( CoC.player.ballSize < 3 ) {
				MainView.outputText( '  With each hard quaking thrust your balls slap against Vala’s expansive ass.  Your nuts ache under their pent up load and demand you to shoot it into the sweet fey-girl soon.' );
			} else if( CoC.player.ballSize < 6 ) {
				MainView.outputText( '  With each hard quaking thrust, your balls smack against Vala’s cushiony ass.  Your hefty, cum-bloated sack, so full, aches for the sweet release into your tender fairy lover.' );
			} else {
				MainView.outputText( '  With each hard quaking thrust, your fat balls slam against Vala’s pale squeezable ass.  The huge, virile spunk-factories go into overtime, their already taut skin growing tighter as they swell and bloat with your pent up load.  Your body\'s natural instincts demand you do your duty as the breeder you are and pack your small lover’s womb full of your spunk until her belly is swollen and seeded with your young.' );
			}
		}
