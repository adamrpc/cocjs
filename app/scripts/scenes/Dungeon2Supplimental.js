...
angular.module( 'cocjs' ).run( function( MainView, SceneLib, $log, ArmorLib, WeaponLib, Combat, PregnancyStore, EncapsulationPod, Appearance, ConsumableLib, ImpHorde, Vala, Utils, PerkLib, StatusAffects, Descriptors, CockTypesEnum, EventParser, OnLoadVariables, AppearanceDefs, kFLAGS, CoC, EngineCore ) {
	...
	Dungeon2Supplimental.prototype.imtacularMultiHitzilla = function() {
		...
		while( hits > 0 ) {
			...
			if( CoC.monster.findStatusAffect( StatusAffects.Blind ) >= 0 && Utils.rand( 3 ) < 2 ) {
				EngineCore.outputText( CoC.monster.getCapitalA() + CoC.monster.short + ' completely misses you with a blind attack!\n', false );
			}
			//Determine if dodged!;
				...
				if( CoC.player.spe - CoC.monster.spe < 8 ) {
					EngineCore.outputText( 'You narrowly avoid ' + CoC.monster.a + CoC.monster.short + '\'s ' + CoC.monster.weaponVerb + '!\n', false );
				} else if( CoC.player.spe - CoC.monster.spe >= 8 && CoC.player.spe - CoC.monster.spe < 20 ) {
					EngineCore.outputText( 'You dodge ' + CoC.monster.a + CoC.monster.short + '\'s ' + CoC.monster.weaponVerb + ' with superior quickness!\n', false );
				} else if( CoC.player.spe - CoC.monster.spe >= 20 ) {
					EngineCore.outputText( 'You deftly avoid ' + CoC.monster.a + CoC.monster.short + '\'s slow ' + CoC.monster.weaponVerb + '.\n', false );
				}
			}
			//Determine if evaded;
			else if( CoC.player.findPerk( PerkLib.Evade ) >= 0 && Utils.rand( 100 ) < 10 ) {
				EngineCore.outputText( 'Using your skills at evading attacks, you anticipate and sidestep ' + CoC.monster.a + CoC.monster.short + '\'s attack.\n', false );
			} else if( CoC.player.findPerk( PerkLib.Misdirection ) >= 0 && Utils.rand( 100 ) < 10 && CoC.player.armorName === 'red, high-society bodysuit' ) {
				EngineCore.outputText( 'With the easy movement afforded by your bodysuit and Raphael\'s teachings, you easily avoid ' + CoC.monster.a + CoC.monster.short + '\'s attack.\n', false );
			}
			//Determine if cat'ed;
			else if( CoC.player.findPerk( PerkLib.Flexibility ) >= 0 && Utils.rand( 100 ) < 6 ) {
				EngineCore.outputText( 'With your incredible flexibility, you squeeze out of the way of ' + CoC.monster.a + CoC.monster.short + '', false );
	...
	Dungeon2Supplimental.prototype.impGangBukkake = function() {
		...
		while( hits > 0 ) {
			...
			//Blind dodge change;
			if( CoC.monster.findStatusAffect( StatusAffects.Blind ) >= 0 && Utils.rand( 3 ) < 2 ) {
				EngineCore.outputText( CoC.monster.getCapitalA() + CoC.monster.short + '\' misguided spooge flies everyone.  A few bursts of it don\'t even land anywhere close to you!\n', false );
			}
			//Determine if dodged!;
			else if( CoC.player.spe - CoC.monster.spe > 0 && Utils.rand( ((CoC.player.spe - CoC.monster.spe) / 4) + 90 ) > 80 ) {
				damage = Utils.rand( 4 );
			...
			//(2-6 hits for +10 lust each!) (-5 lust per successful hit);
				...
				if( damage === 2 ) {
					EngineCore.outputText( 'Some of the fluid splatters into your midriff and drools down to your waist, making your ' + CoC.player.armorName + ' slimy and wet.\n', false );
				}
				if( damage === 3 ) {
				...
				if( damage === 5 ) {
					EngineCore.outputText( 'The last eruption of cum soaks your thighs and the lower portions of your ' + CoC.player.armorName + ', turning it a sticky white.\n', false );
				}
	...
	Dungeon2Supplimental.prototype.impGangUber = function() {
		if( CoC.monster.findStatusAffect( StatusAffects.ImpUber ) < 0 ) {
			...
			if( CoC.player.cockTotal() > 0 ) {
				EngineCore.outputText( 'are pulled tight by your ' + CoC.player.multiCockDescript() + ', ', false );
	...
	Dungeon2Supplimental.prototype.loseToImpMob = function() {
		...
		EngineCore.outputText( 'In seconds, the squirming red bodies swarm over you, blotting the rest of the room from your vision.  You can feel their scrabbling fingers and hands tearing off your ' + CoC.player.armorName + ', exposing your body to their always hungry eyes.   Their loincloths disappear as their growing demonic members make themselves known, pushing the tiny flaps of fabric out of the way or outright tearing through them.   You\'re groped, touched, and licked all over, drowning in a sea of long tongues and small nude bodies.\n\n', false );
		...
		if( CoC.player.cockTotal() === 0 ) {
			...
			EngineCore.outputText( 'and ' + CoC.player.sMultiCockDesc() + ' unloads, splattering the many demons with a bit of your own seed.  You\'d smile if your mouth wasn\'t so full of cock.  At least you got to make a mess of them!', false );
		}
		if( CoC.player.hasVagina() ) {
	...
	Dungeon2Supplimental.prototype.getTrappedContinuation = function() {
		...
		EngineCore.outputText( 'Meanwhile, a number of tentacles have sprung up from below, and are crawling up your ' + CoC.player.legs() + '.  It\'s becoming fairly clear how the skeleton wound up in this cave...  You\'ve got to escape!', false );
		//[FIGHT];
	...
	Dungeon2Supplimental.prototype.encapsulationPodAI = function() {
		...
		if( CoC.monster.findStatusAffect( StatusAffects.Round ) < 0 ) {
			EngineCore.outputText( 'You shiver from the feeling of warm wetness crawling up your ' + CoC.player.legs() + '.   Tentacles brush against your ', false );
			if( CoC.player.balls > 0 ) {
				EngineCore.outputText( Descriptors.ballsDescriptLight() + ' ', false );
				if( CoC.player.hasVagina() ) {
		...
		//[Round 2 Action];
			...
			EngineCore.outputText( 'The tentacles under your ' + CoC.player.armorName + ' squirm against you, seeking out openings to penetrate and genitalia to caress.  ', false );
			if( CoC.player.balls > 0 ) {
			...
			if( CoC.player.cockTotal() > 0 ) {
				EngineCore.outputText( 'prods your ' + Descriptors.cockDescript( 0 ) + ' for a second before it begins slithering around it, snake-like.  Once it has you encircled from ' + CoC.player.cockHead() + ' to ', false );
				if( !CoC.player.hasSheath() ) {
			...
			if( CoC.player.cockTotal() > 1 ) {
				...
				if( CoC.player.cockTotal() === 2 ) {
					EngineCore.outputText( Descriptors.cockDescript( 1 ) + ' gets the same treatment, and soon both of your ' + CoC.player.multiCockDescriptLight() + ' are quite happy to be here.  ', false );
				} else {
					EngineCore.outputText( CoC.player.multiCockDescriptLight() + ' get the same treatment and soon feel quite happy to be here.  ', false );
				}
			}
			if( CoC.player.hasVagina() ) {
		...
		//[Round 4 Action];
			...
			if( CoC.player.totalCocks() > 0 ) {
				EngineCore.outputText( ', and ' + CoC.player.sMultiCockDesc() + ' bloats with blood, getting larger than ever', false );
			}
	...
	Dungeon2Supplimental.prototype.loseToThisShitPartII = function() {
		...
		if( CoC.player.cumQ() > 3500 ) {
			EngineCore.outputText( 'Your orgasm drags on for so long that you begin to feel pressure from the cum-slime surrounding you.  It doesn\'t seem to matter to ' + CoC.player.sMultiCockDesc() + ', which is too busy sending bliss to your brain and squirting cum for the tentacles to care.  It actually kind of hurts.  The oscillating purple ambiance flashes brighter in protest for a second, and then everything releases all at once.  The pressure is gone and you\'re sliding down on a wave of fungal-slime cum, feeling the tentacles being pulled from you by the sudden shift of position.  Moist cave air tickles at your ' + CoC.player.skinDesc + ' as you come to rest on another spongy petal and begin to cough out the sludge.\n\n', false );
		...
		if( CoC.player.gender === 1 || (CoC.player.gender === 3 && Utils.rand( 2 ) === 0) ) {
			EngineCore.outputText( 'The orgasm squirts and drips from ' + CoC.player.sMultiCockDesc() + ' for what seems like forever.  It feels so right, so perfect, that you actually whine in disappointment when it finally does end.  You can\'t even be bothered to reach down and stroke yourself.  The softening in your loins is nothing compared to your flaccid, listless muscles.  You couldn\'t make your arms reach down to touch yourself even if you could work up the motivation to try.  Thankfully the slippery tentacles curl back around your ', false );
			if( !CoC.player.hasSheath() ) {
				EngineCore.outputText( 'base', false );
			} else {
				EngineCore.outputText( 'sheath', false );
			}
			EngineCore.outputText( ' and squeeze, forcing ' + CoC.player.sMultiCockDesc() + ' to inflate to readiness.  Deep inside your ' + Descriptors.assholeDescript() + ', the tentacle starts to rub against your prostate.  It caresses the male organ on each side and pauses to squeeze the center of it, pushing a few drops of sticky cum from your trembling ' + Appearance.cockNoun( CockTypesEnum.HUMAN ) + '.\n\n', false );
			EngineCore.outputText( 'The vine-like stalks currently hugging ' + CoC.player.sMultiCockDesc() + ' constrict the base and begin to swirl around it in a circular motion.  Warm fungi-flesh and viscous, drugged ooze work together to send hot spikes of pleasure up your spinal-cord.  Despite your recent orgasm, you aren\'t being given any chance to recover or refill your ', false );
			if( CoC.player.balls > 0 ) {
	...
	Dungeon2Supplimental.prototype.healVala = function() {
		...
		if( CoC.player.hasItem( ConsumableLib.PURHONY, 1 ) ) {
			...
			if( CoC.player.lowerBody === AppearanceDefs.LOWER_BODY_TYPE_NAGA ) {
				...
				EngineCore.outputText( CoC.player.leg(), false );
			}
		...
		//Pearl!;
			...
			EngineCore.outputText( 'A soft, warm breeze rustles your ' + Descriptors.hairDescript() + ' and for a moment the foul stench of the dungeon vanishes, setting your mind at ease and draining the tension that has been building in your gut. In a moment of clarity, you remember the beautiful Pure Pearl that Marae granted you as a reward for shutting down the demons\' factory. It seems only right to use the goddess\' gift to rescue one of her wayward children. Perhaps she gave it to you for this very reason? The oblivious girl has managed to work her way to a sitting position and is grinding her dripping sex against your ' + CoC.player.foot() + '. You lean down and gently lift her chin up, bringing her empty, pink eyes to stare into yours. Mistaking the gentle motion for a command, she gleefully opens wide, tongue thrashing about in anticipation. You place the pink pearl at the fairy\'s lips and she wraps her mouth around the pale jewel, trying obediently to swallow it. However, the little fairy\'s mouth is far smaller than you would\'ve thought and it seems to get stuck just behind her teeth, like a pink ball-gag. She coos a muffled moan and begins to masturbate without breaking eye contact with you.\n\n', false );
	...
	Dungeon2Supplimental.prototype.fightValaVictory = function() {
		EngineCore.spriteSelect( 85 );
		EngineCore.outputText( '', true );
		EngineCore.outputText( 'The fairy girl collapses, well-drilled obedience robbing her limbs of their fight. She squirms to a crouching bow, fully accepting you as her new ' + CoC.player.mf( 'Master', 'Mistress' ) + '. The warped fae\'s empty eyes look up at you, her face a mask of rapture as she anxiously awaits her punishment, wagging her butt in the air as lubrication gushes down her thighs. It seems being defeated has excited the broken creature to a breeding frenzy. Her endurance must be incredible to be this frisky after your battle.', false );
	...
	Dungeon2Supplimental.prototype.valaFightVictoryFuck = function() {
		...
		if( CoC.player.gender === 2 ) {
			...
			if( CoC.player.wingType > AppearanceDefs.WING_TYPE_NONE ) {
				EngineCore.outputText( 'You decide to give her a hand, and flap your ' + CoC.player.wingDesc + ' as hard as you can, the added thrust giving her the start she needs.  ', false );
			}
		...
		//(Male);
			...
			EngineCore.outputText( 'If the girl\'s got energy to spare, you\'d better try to wear her down or she might alert more demons after you leave. Commanding her to bow before you, the broken fairy is all too happy to comply, falling on her face, her arms spread wide, panting with desire. You circle around her and strip your ' + CoC.player.armorName + ' off, admiring her plump little ass, all the plumper for how whip-thin her waist and legs are. You bend down to grab her waist and haul her ass upward, spearing her on your stiffening ' + Descriptors.cockDescript( 0 ) + ' without further ceremony. It\'s important she knows that foreplay is for obedient pets, you note to yourself with a grin. With a vicious thrust, you fill the girl\'s stretched pussy and bend her over at a right angle. Smacking her ass, you command the little bitch to fly and she shivers with delight.\n\n', false );
		...
		//(Herm);
			...
			EngineCore.outputText( 'underside of your ' + CoC.player.foot() + ' against the back of her head and apply pressure, forcing her face toward the seething meal. "<i>Eat up,</i>" you command the fairy, and her resistance crumbles, allowing her face to be fully submerged in the spunk, bubbles rising from the slimy bowl as she begins to suck it down with mindless obedience.\n\n', false );
	...
	Dungeon2Supplimental.prototype.faerieOrgyFuckMaleContinue = function() {
		...
		if( CoC.flags[ kFLAGS.TIMES_SCYLLA_ADDICT_GROUP_EXPLOITED ] + CoC.flags[ kFLAGS.SCYLLA_TIMES_SHARED_IN_ADDICT_GROUP ] + CoC.flags[ kFLAGS.SCYLLA_MILK_THERAPY_TIMES ] + CoC.flags[ kFLAGS.SCYLLA_CUM_THERAPY_TIMES ] + CoC.flags[ kFLAGS.SCYLLA_SEX_THERAPY_TIMES ] > 0 ) {
			EngineCore.outputText( 'Another fairy flies toward Vala and you blink through the building lusty haze when you recognize the little girl. Wearing a slightly slutty crystalline gown of translucent gossamer, it seems to be Pastie, the fairy from Scylla\'s addiction counseling meeting. When you call out to her, she bashfully waves back. "<i>Hey, uh, ' + CoC.player.short + '. It\'s kinda weird seeing you outside of the meetings. How are you? How have things been?</i>" She looks at your naked, reclined form on the bed and blushes. "<i>Oh, right, right. Um. I\'m just helping out Vala, you know. Sisters and all. Uh. I\'m... I\'m going to just give her this petal and get something to drink, okay? All this sobriety is making this really uncomfortable for me.</i>" The other fairies laugh and Pastie hands off her blossom before zipping out of the room, probably to steal someone\'s drink downstairs.\n\n', false );
		}
	...
	Dungeon2Supplimental.prototype.faerieOrgyFuckFemaleContinue = function() {
		EngineCore.spriteSelect( 60 );
		EngineCore.outputText( '', true );
		EngineCore.outputText( 'The fairy girl lands at the edge of the bed, settling on her knees and resting her weighty chest on the mattress. Leaning in, she spreads your ' + CoC.player.legs() + ' with warm, delicate hands, fingertips stroking the insides of your thighs. You shiver and hug your arms under your breasts, a little nervous about this \'forest magic.\' One of the fairies flutters down to Vala and hands her a pale white lily petal with a tittering giggle. The larger fairy takes the soft white bloom and, whispering a silken word to it, licks the cream-colored surface. Slithering further between your legs, she reaches your ' + Descriptors.vaginaDescript( 0 ) + ' and brushes the petal against your ' + Descriptors.clitDescript() + ' with a teasing caress. Then, much to your surprise, she places it on the tip of her forefinger and slides it into your pussy. You gasp, the petal\'s soft surface rubbing your inner walls with a warmth you were unprepared for. Vala presses lightly on a spot inside your vagina and you lose your breath as the tremor of a small orgasm rumbles through your gut. Without explanation, the fairy barmaid takes a rose petal and repeats the process, another orgasm seizing you and leaving you panting. "<i>Normally we only need the one petal, but since you and I are so much larger than the average fairy, I had my sisters bring lots. All for you,</i>" she whispers, coyly.\n\n', false );
		//[If the Player has unlocked Scylla's Addiction Counseling event] ;
		if( CoC.flags[ kFLAGS.TIMES_SCYLLA_ADDICT_GROUP_EXPLOITED ] + CoC.flags[ kFLAGS.SCYLLA_TIMES_SHARED_IN_ADDICT_GROUP ] + CoC.flags[ kFLAGS.SCYLLA_MILK_THERAPY_TIMES ] + CoC.flags[ kFLAGS.SCYLLA_CUM_THERAPY_TIMES ] + CoC.flags[ kFLAGS.SCYLLA_SEX_THERAPY_TIMES ] > 0 ) {
			EngineCore.outputText( 'Another fairy flies toward Vala and you blink through the building lusty haze when you recognize the little girl. Wearing a slightly slutty crystalline gown of translucent gossamer, you recognize Pastie, the fairy from Scylla\'s addiction counseling meeting. When you call out to her, she bashfully waves back. "<i>Hey, uh, ' + CoC.player.short + '. It\'s kinda weird seeing you outside of the meetings. How are you? How have things been?</i>" She looks at your naked, reclined form on the bed and blushes. "<i>Oh, right, right. Um. I\'m just helping out Vala, you know. Sisters and all. Uh. I\'m... I\'m going to just give her this petal and get something to drink, okay? All this sobriety is making this really uncomfortable for me.</i>" The other fairies laugh and Pastie hands off her blossom before zipping out of the room, probably to steal someone\'s drink downstairs.\n\n', false );
		}
	...
	Dungeon2Supplimental.prototype.zetazAI = function() {
		...
		if( CoC.monster.findStatusAffect( StatusAffects.Constricted ) >= 0 ) {
			...
			if( CoC.monster.statusAffectv1( StatusAffects.Constricted ) <= 0 ) {
				EngineCore.outputText( '  ' + CoC.monster.getCapitalA() + CoC.monster.short + ' proves to be too much for your tail to handle, breaking free of your tightly bound coils.', false );
		...
		if( CoC.monster.lust > 50 && CoC.monster.HPRatio() <= 0.5 ) {
			...
			if( attackChoice === 0 ) {
				...
				if( (CoC.player.findPerk( PerkLib.Evade ) >= 0 && Utils.rand( 4 ) === 0) ||
					...
					EngineCore.outputText( 'You try to avoid it, but the fragile glass shatters against you, coating you in sticky red liquid.  It seeps into your ' + CoC.player.skinDesc + ' and leaves a pleasant, residual tingle in its wake.  Oh no...', false );
	...
	Dungeon2Supplimental.prototype.gigaArouse = function() {
		EngineCore.outputText( 'You see ' + CoC.monster.a + CoC.monster.short + ' make familiar arcane gestures at you, but his motions seem a lot more over the top than you\'d expect from an imp.\n\n', false );
		...
		if( CoC.player.cocks.length > 0 ) {
			if( CoC.player.lust >= 60 && CoC.player.cocks.length > 0 ) {
				EngineCore.outputText( 'You feel ' + CoC.player.sMultiCockDesc() + ' dribble pre-cum, bouncing with each beat of your heart and aching to be touched.  ', false );
			}
			if( CoC.player.lust >= 30 && CoC.player.lust < 60 && CoC.player.cocks.length === 1 ) {
				EngineCore.outputText( CoC.player.SMultiCockDesc() + ' hardens and twitches, distracting you further.  ', false );
			}
		}
		if( CoC.player.vaginas.length > 0 ) {
			...
			if( CoC.player.lust >= 60 && CoC.player.vaginas[ 0 ].vaginalWetness === AppearanceDefs.VAGINA_WETNESS_WET && CoC.player.vaginas.length > 0 ) {
				EngineCore.outputText( 'Your crotch becomes sticky with girl-lust, making it clear to ' + CoC.monster.a + CoC.monster.short + ' just how welcome your body finds the spell.  ', false );
			}
			...
			if( CoC.player.lust >= 60 && CoC.player.vaginas[ 0 ].vaginalWetness === AppearanceDefs.VAGINA_WETNESS_DROOLING && CoC.player.vaginas.length > 0 ) {
				EngineCore.outputText( 'Thick runners of girl-lube stream down the insides of your thighs as your crotch gives into the demonic magics.  You wonder what ' + CoC.monster.a + CoC.monster.short + '\'s cock would feel like inside you?  ', false );
			}
			if( CoC.player.lust >= 60 && CoC.player.vaginas[ 0 ].vaginalWetness === AppearanceDefs.VAGINA_WETNESS_SLAVERING && CoC.player.vaginas.length === 1 ) {
				EngineCore.outputText( 'Your ' + Descriptors.allVaginaDescript() + ' instantly soaks your groin with the heady proof of your need.  You wonder just how slippery you could ' + CoC.monster.a + CoC.monster.short + '\'s dick when it\'s rammed inside you?  ', false );
			}
		}
	...
	Dungeon2Supplimental.prototype.sexualInterrogation = function() {
		...
		EngineCore.outputText( '"<i>Piss off!  You won\'t get shit from me,</i>" retorts the defeated demon, "<i>You may as well finish me off – I\'ll NEVER help a ' + CoC.player.mf( 'jackass', 'bitch' ) + ' like you!</i>"\n\n', false );
		...
		EngineCore.outputText( 'The imp looks up with his face shrouded in confusion as he asks, "<i>I don\'t think a string is going to help you much, ' + CoC.player.mf( 'dork', 'skank' ) + '.</i>"\n\n', false );
	...
	Dungeon2Supplimental.prototype.loseToZetaz = function() {
		...
		EngineCore.outputText( ' form with an amused expression, "<i>The first champion in ages to retain ' + CoC.player.mf( 'his', 'her' ) + ' free will for more than a few minutes, and ' + CoC.player.mf( 'he', 'she' ) + '\'s brought to ' + CoC.player.mf( 'his', 'her' ) + ' knees by the very imp ' + CoC.player.mf( 'he', 'she' ) + ' escaped!  Once you\'ve learned your proper place, you\'ll guarantee my safe return to my rightful station.  Perhaps I\'ll even get a promotion?  After all, you\'ve defeated so many higher ranking demons already.</i>"\n\n', false );
	...
	Dungeon2Supplimental.prototype.femaleZetazOverPtII = function() {
		...
		EngineCore.outputText( ' he begins to repeatedly thrust against your ' + Descriptors.vaginaDescript( 0 ) + ', fucking you in earnest.  The wet slap of his balls on your juice-slimed body fills the chamber and sends ripples of pleasure down your ' + CoC.player.legs() + '.  With your eyelids half-closed, your tongue masturbating your lips, and your pussy practically squirting lubricants at the end of each thrust, you must look like every male\'s wet dream.\n\n', false );
	...
	Dungeon2Supplimental.prototype.hermZetazOver = function() {
		...
		if( CoC.player.lowerBody !== AppearanceDefs.LOWER_BODY_TYPE_DEMONIC_HIGH_HEELS ) {
			EngineCore.outputText( 'Your ' + CoC.player.feet() + ' have been changing throughout the ordeal, but you didn\'t notice your ' + CoC.player.legs() + ' becoming such lissom, lengthy legs, or your heels growing long, high-heel-like spikes.  ', false );
		}
	...
	Dungeon2Supplimental.prototype.hermZetazOverPtII = function() {
		EngineCore.hideUpDown();
		EngineCore.outputText( '', true );
		EngineCore.outputText( 'You awaken midway through a loud moan and nearly jump out of your ' + CoC.player.skinDesc + ' in surprise, but the fire of your unnaturally stoked libido immediately reasserts yourself.  You twitch your hips to and fro, thrusting against a ', false );
		if( CoC.player.cockTotal() > 1 ) {
	...
	Dungeon2Supplimental.prototype.malesZetazOverPtII = function() {
		...
		EngineCore.outputText( 'While you observe the room, you realize that the waves of pleasure sliding up your spinal cord haven\'t stopped, and that your entire body is being shaken rhythmically.  You look down with a look of incredible, still-drugged confusion and behold the last thing you expected to see.  Somehow ' + CoC.player.sMultiCockDesc() + ' has been shrunk to less than half of its previous size', false );
		if( CoC.player.balls > 0 ) {
	...
	Dungeon2Supplimental.prototype.kinathisValaStuff = function() {
		...
		EngineCore.outputText( 'Playing her way down your cock, the fey-girl gently licks up and down, sucking along your cumvein before coming back to the ' + CoC.player.cockHead( x ) + ' of your meaty prick.' );
