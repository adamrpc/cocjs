...
angular.module( 'cocjs' ).run( function( $rootScope, Descriptors, CoC, kFLAGS, Utils, StatusAffects, EventParser, EngineCore, ConsumableLib ) {
	...
	Giacomo.prototype.timeChange = function() {
		...
		if( CoC.getInstance().time.hours > 23 ) {
			if( CoC.getInstance().flags[ kFLAGS.CERULEAN_POTION_BAD_END_FUTA_COUNTER ] > 0 ) {
				CoC.getInstance().flags[ kFLAGS.CERULEAN_POTION_BAD_END_FUTA_COUNTER ] -= 0.5; //Reduce bad-end for cerulean herms number
			}
		}
		return false;
	};
	Giacomo.prototype.timeChangeLarge = function() {
		if( this.checkedSuccubi++ === 0 && CoC.getInstance().time.hours === 4 && CoC.getInstance().player.findStatusAffect( StatusAffects.SuccubiNight ) >= 0 && (CoC.getInstance().player.hasCock() || CoC.getInstance().player.gender === 0) ) { //Call secksins!
			if( CoC.getInstance().player.findStatusAffect( StatusAffects.RepeatSuccubi ) >= 0 ) {
				...
				CoC.getInstance().player.createStatusAffect( StatusAffects.RepeatSuccubi, 0, 0, 0, 0 );
			}
			//Lower count if multiples stacked up.;
			if( CoC.getInstance().player.statusAffectv1( StatusAffects.SuccubiNight ) > 1 ) {
				CoC.getInstance().player.addStatusValue( StatusAffects.SuccubiNight, 1, -1 );
			} else {
				CoC.getInstance().player.removeStatusAffect( StatusAffects.SuccubiNight );
			}
			return true;
		}
		return false;
	...
	Giacomo.prototype.giacomoEncounter = function() {
		...
		if( CoC.getInstance().giacomo === 0 ) {
			this.firstEncounter();
		} else if( CoC.getInstance().player.findStatusAffect( StatusAffects.WormOffer ) < 0 && CoC.getInstance().player.findStatusAffect( StatusAffects.Infested ) >= 0 ) { //If infested && no worm offer yet
			EngineCore.outputText( 'Upon walking up to Giacomo\'s wagon, he turns to look at you and cocks an eyebrow in curiosity and mild amusement.\n\n' );
			EngineCore.outputText( '"<i>Been playing with creatures best left alone, I see</i>," he chuckles.  "<i>Infestations of any kind are annoying, yet your plight is quite challenging given the magnitude of corrupt creatures around here.  It is not the first time I have seen one infested with THOSE worms.</i>"\n\n' );
			EngineCore.outputText( 'You ask how he knows of your change and the merchant giggles heartily.\n\n' );
			EngineCore.outputText( '"<i>Do not look at me as if I am a mystic,</i>" Giacomo heckles lightly.  "<i>Your crotch is squirming.</i>"\n\n' );
			EngineCore.outputText( 'Looking down, you realize how right he is and attempt to cover yourself in embarrassment.\n\n' );
			EngineCore.outputText( '"<i>Fear not!</i>" the purveyor jingles.  "<i>I have something that will cure you of those little bastards.  Of course, there is also a chance that it will purge your system in general.  This potion is not cheap.  I will trade it for 175 gems.</i>"\n\n' );
			CoC.getInstance().player.createStatusAffect( StatusAffects.WormOffer, 0, 0, 0, 0 );
			if( CoC.getInstance().player.gems < 175 ) {
				EngineCore.outputText( 'You realize you don\'t have enough gems for such a pricey potion, but perhaps there is something else in his inventory you can buy.' );
			} else {
				EngineCore.outputText( 'Do you purchase his cure?' );
				//Remove/No;
				EngineCore.doYesNo( this.wormRemoval, this.giacomoEncounter );
				return;
			}
		} else {
			EngineCore.outputText( 'You spy the merchant Giacomo in the distance.  He makes a beeline for you, setting up his shop in moments.  ' );
	...
	Giacomo.prototype.firstEncounter = function() {
		...
		EngineCore.outputText( '"<i>Greetings, traveler! My name is Giacomo.  I am, as you can see, a humble purveyor of items, curios and other accoutrements.  While I am not in a position to show you my full wares as my shop is packed on this push-cart, I do offer some small trinkets for travelers I meet.</i>"\n\n' );
		EngineCore.outputText( 'The merchant looks at you sharply and cracks a wide, toothy smile you find... unnerving.  The merchant twists his way around to access a sack he has around his back.  After a moment, he swings the sack from his back to have better access to its contents.  Inquisitively, the merchant turns back to you.\n' );
		EngineCore.outputText( '"<i>So stranger, be you interested in some drafts to aid you in your travels, some quick pamphlets to warn you of dangers on journeys or...</i>"\n\n' );
		EngineCore.outputText( 'Giacomo pauses and turns his head in both directions in a mocking gesture of paranoid observation.  His little bit of theatrics does make you wonder what he is about to offer.\n' );
		EngineCore.outputText( '"<i>...maybe you would be interested in some items that enhance the pleasures of the flesh?  Hmmm?</i>"\n\n' );
		EngineCore.outputText( 'Giacomo\'s grin is nothing short of creepy as he offers his wares to you.  What are you interested in?' );
		CoC.getInstance().giacomo = 1;
	};
	Giacomo.prototype.potionMenu = function() {
		EngineCore.spriteSelect( 23 );
		EngineCore.clearOutput();
		EngineCore.outputText( 'Which potion or tincture will you examine?' );
		EngineCore.choices( 'Vitality T.', this.pitchVitailtyTincture, 'Scholars T.', this.pitchScholarsTea,
			'Cerulean P.', (CoC.getInstance().player.gender !== 2 ? this.pitchCeruleanPotion : null), '', null, 'Back', this.giacomoEncounter );
		EngineCore.statScreenRefresh();
	};
	Giacomo.prototype.bookMenu = function() {
		...
		EngineCore.choices( 'Dangerous Plants', this.pitchDangerousPlantsBook, 'Traveler\'s Guide', this.pitchTravellersGuide, 'Hentai Comic', this.pitchHentaiComic,
			'Yoga Guide', (CoC.getInstance().flags[ kFLAGS.COTTON_UNUSUAL_YOGA_BOOK_TRACKER ] > 0 ? this.pitchYogaGuide : null), 'Back', this.giacomoEncounter );
		EngineCore.statScreenRefresh();
	};
	Giacomo.prototype.eroticaMenu = function() {
		...
		if( CoC.getInstance().player.gender === 1 ) {
			EngineCore.choices( 'Dildo', this.pitchDildo, 'Onahole', this.pitchOnahole, 'D Onahole', this.pitchDeluxeOnahole, '', null, 'Back', this.giacomoEncounter );
		}
		if( CoC.getInstance().player.gender === 2 ) {
			EngineCore.choices( 'Dildo', this.pitchDildo, 'Stim-Belt', this.pitchSelfStimulationBelt, 'AN Stim-Belt', this.pitchAllNaturalSelfStimulationBelt, '', null, 'Back', this.giacomoEncounter );
		}
		if( CoC.getInstance().player.gender === 3 ) {
			EngineCore.choices( 'Onahole', this.pitchOnahole, 'D Onahole', this.pitchDeluxeOnahole, 'AN Onahole', this.pitchAllNaturalOnahole, 'Stim-Belt', this.pitchSelfStimulationBelt, 'AN Stim-Belt', this.pitchAllNaturalSelfStimulationBelt, 'Dual Belt', this.pitchDualStimulationBelt, '', null, '', null, 'Dildo', this.pitchDildo, 'Back', this.giacomoEncounter );
		}
		if( CoC.getInstance().player.gender === 0 ) {
			EngineCore.choices( 'Dildo', this.pitchDildo, 'Onahole', this.pitchOnahole, 'Stim-Belt', this.pitchSelfStimulationBelt, '', null, 'Back', this.giacomoEncounter );
		}
		EngineCore.statScreenRefresh();
	};
	Giacomo.prototype.pitchVitailtyTincture = function() {
		EngineCore.spriteSelect( 23 );
		EngineCore.clearOutput();
		EngineCore.outputText( 'Giacomo holds up the item and says, "<i>Ah, yes!  The quintessential elixir for all travelers, this little bottle of distilled livelihood will aid you in restoring your energy on your journey and, should you be hurt or injured, will aid the body\'s ability to heal itself.  Yes ' + CoC.getInstance().player.mf( 'sir', 'madam' ) + ', this is liquid gold for pilgrim and adventurer alike.  Interested?  It is <b>15 gems</b></i>."  ' );
		EngineCore.doYesNo( this.buyVitailtyTincture, this.potionMenu );
	};
	Giacomo.prototype.buyVitailtyTincture = function() {
		EngineCore.spriteSelect( 23 );
		if( CoC.getInstance().player.gems < 15 ) {
			EngineCore.clearOutput();
			EngineCore.outputText( '\n\nGiacomo sighs, indicating you need ' + String( 15 - CoC.getInstance().player.gems ) + ' more gems to purchase this item.' );
			EngineCore.doNext( this.potionMenu );
		} else {
			CoC.getInstance().player.gems -= 15;
			CoC.getInstance().inventory.takeItem( ConsumableLib.VITAL_T, this.potionMenu );
			EngineCore.statScreenRefresh();
		}
	};
	Giacomo.prototype.pitchScholarsTea = function() {
		...
		EngineCore.doYesNo( this.buyScholarsTea, this.potionMenu );
	};
	Giacomo.prototype.buyScholarsTea = function() {
		...
		if( CoC.getInstance().player.gems < 15 ) {
			...
			CoC.getInstance().player.gems -= 15;
			CoC.getInstance().inventory.takeItem( ConsumableLib.SMART_T, this.potionMenu );
			EngineCore.statScreenRefresh();
		}
	};
	Giacomo.prototype.pitchCeruleanPotion = function() {
		...
		EngineCore.doYesNo( this.buyCeruleanPotion, this.potionMenu );
	};
	Giacomo.prototype.buyCeruleanPotion = function() {
		...
		if( CoC.getInstance().player.gems < 75 ) {
			...
			CoC.getInstance().inventory.takeItem( ConsumableLib.CERUL_P, this.potionMenu );
	...
	Giacomo.prototype.buyDangerousPlantsBook = function() {
		...
		if( CoC.getInstance().player.gems < 10 ) {
			...
			CoC.getInstance().player.createKeyItem( 'Dangerous Plants', 0, 0, 0, 0 );
			EngineCore.statScreenRefresh();
		}
	};
	...
	Giacomo.prototype.buyTravellersGuide = function() {
		...
		if( CoC.getInstance().player.gems < 1 ) {
			...
			EngineCore.outputText( 'The crazy merchant said you might not need this and he was right.  Written at a simple level, this was obviously intended for a city-dweller who never left the confines of their walls.  Littered with childish illustrations and silly phrases, the book is informative in the sense that it does tell a person what they need and what to do, but naively downplays the dangers of the forest and from bandits.  Were it not so cheap, you would be pissed at the merchant.  However, he is right in the fact that giving this to some idiot ignorant of the dangers of the road saves time from having to answer a bunch of stupid questions.' );
			EngineCore.doNext( this.bookMenu );
			CoC.getInstance().player.gems -= 1;
			CoC.getInstance().player.createKeyItem( 'Traveler\'s Guide', 0, 0, 0, 0 );
			EngineCore.statScreenRefresh();
		}
	};
	...
	Giacomo.prototype.buyHentaiComic = function() {
		...
		if( CoC.getInstance().player.gems < 10 ) {
			...
			EngineCore.outputText( 'You peruse the erotic book.  The story is one of a group of sisters who are all impossibly heavy-chested and equally horny getting into constant misadventures trying to satisfy their lust.  While the comic was entertaining and erotic to the highest degree, you cannot help but laugh at how over-the-top the story and all of the characters are.  Were the world as it was in the book, nothing would get done as humanity would be fucking like jackrabbits in heat for the rest of their lives.  While certainly a tempting proposition, everyone gets worn out sometime.  You place the book in your sack, well entertained and with a head filled with wilder perversions than what you woke up with this morning.' );
			EngineCore.doNext( this.bookMenu );
			CoC.getInstance().player.gems -= 10;
			CoC.getInstance().player.createKeyItem( 'Hentai Comic', 0, 0, 0, 0 );
			EngineCore.statScreenRefresh();
		}
	};
	Giacomo.prototype.pitchYogaGuide = function() {
		...
		EngineCore.doYesNo( this.buyYogaGuide, this.bookMenu );
	};
	Giacomo.prototype.buyYogaGuide = function() {
		EngineCore.clearOutput();
		if( CoC.getInstance().player.hasKeyItem( 'Yoga Guide' ) >= 0 ) {
			EngineCore.outputText( '<b>You already own a yoga guide!</b>' );
		} else if( CoC.getInstance().player.gems < 100 ) {
			EngineCore.outputText( 'You cannot afford a yoga guide!' );
		} else {
			EngineCore.outputText( 'You exchange 100 gems for the tome.  Now you can finally enjoy a workout with Cotton!' );
			CoC.getInstance().player.createKeyItem( 'Yoga Guide', 0, 0, 0, 0 );
	...
	Giacomo.prototype.pitchDildo = function() {
		...
		if( CoC.getInstance().player.hasKeyItem( 'Dildo' ) >= 0 ) {
			...
			EngineCore.doNext( this.eroticaMenu );
			return;
		}
		EngineCore.outputText( 'Giacomo takes out a slender tube roughly over half a foot in length.  "<i>Since you seek pleasure, this is as simple and effective as it gets.  This dildo is a healthy seven inches long and is suitable for most women and even adventurous men.  Pick a hole, stick it in and work it to your heart\'s content or your partner\'s pleasure.  The single-piece construction makes it solid, sturdy and straightforward.  For 20 gems, you can take matters into your own hands.  How about it?</i>"' );
		EngineCore.doYesNo( this.buyDildo, this.eroticaMenu );
	};
	Giacomo.prototype.buyDildo = function() {
		EngineCore.spriteSelect( 23 );
		EngineCore.clearOutput();
		if( CoC.getInstance().player.gems < 20 ) {
			EngineCore.outputText( '\n\nGiacomo sighs, indicating you need ' + String( 20 - CoC.getInstance().player.gems ) + ' more gems to purchase this item.' );
			EngineCore.doNext( this.eroticaMenu );
		} else {
			EngineCore.outputText( 'After making the payment, Giacomo hands you the Dildo' );
			EngineCore.doNext( this.eroticaMenu );
			CoC.getInstance().player.gems -= 20;
			CoC.getInstance().player.createKeyItem( 'Dildo', 0, 0, 0, 0 );
			EngineCore.statScreenRefresh();
		}
	};
	...
	Giacomo.prototype.pitchAllNaturalSelfStimulationBelt = function() {
		...
		if( CoC.getInstance().player.hasKeyItem( 'Dangerous Plants' ) >= 0 && CoC.getInstance().player.inte > 39 ) {
			...
			if( CoC.getInstance().player.inte > 29 ) {
		...
		EngineCore.doYesNo( this.buyAllNaturalSelfStimulationBelt, this.eroticaMenu );
	};
	Giacomo.prototype.buyAllNaturalSelfStimulationBelt = function() {
		EngineCore.spriteSelect( 23 );
		EngineCore.clearOutput();
		if( CoC.getInstance().player.gems < 40 ) {
			EngineCore.outputText( '\n\nGiacomo sighs, indicating you need ' + String( 40 - CoC.getInstance().player.gems ) + ' more gems to purchase this item.' );
			EngineCore.doNext( this.eroticaMenu );
			return;
		}
		EngineCore.outputText( 'After making the payment, Giacomo hands you the All-Natural Self-Stimulation Belt' );
		CoC.getInstance().player.createKeyItem( 'All-Natural Self-Stimulation Belt', 0, 0, 0, 0 );
		EngineCore.doNext( this.eroticaMenu );
	...
	Giacomo.prototype.buyOnahole = function() {
		...
		EngineCore.outputText( 'After making the payment, Giacomo hands you the Plain Onahole' );
		CoC.getInstance().player.createKeyItem( 'Plain Onahole', 0, 0, 0, 0 );
		EngineCore.doNext( this.eroticaMenu );
		CoC.getInstance().player.gems -= 20;
		EngineCore.statScreenRefresh();
	};
	...
	Giacomo.prototype.buyDualStimulationBelt = function() {
		...
		if( CoC.getInstance().player.gems < 50 ) {
			...
			CoC.getInstance().player.createKeyItem( 'Dual Belt', 0, 0, 0, 0 );
			CoC.getInstance().player.gems -= 50;
			EngineCore.statScreenRefresh();
		}
		EngineCore.doNext( this.eroticaMenu );
	...
	Giacomo.prototype.wormRemoval = function() {
		...
		EngineCore.outputText( 'The merchant puts away his prize and comes back to help you up. "<i>Here.</i>", he says as he shoves a couple of bottles into your hand. "<i>This is on the house. You probably need it after the shock of getting those things out.</i>"\n\n' );
		//Add 1 tincture of vitality to inventory;
		//Infestation purged. Hit Points reduced to 10% of MAX. Corruption -20.;
		if( CoC.getInstance().player.HP > Math.ceil( CoC.getInstance().player.maxHP() * 0.15 ) ) {
			CoC.getInstance().player.HP = Math.ceil( CoC.getInstance().player.maxHP() * 0.15 );
		}
	...
	Giacomo.prototype.wormRemovalOffer = function() {
		...
		//Broke as a joke;
		if( CoC.getInstance().player.gems < 175 ) {
			EngineCore.outputText( 'You realize you don\'t have enough gems for such a pricey potion, but perhaps there is something else in his inventory you can buy.', false );
			EngineCore.doNext( this.giacomoEncounter );
		}
		//Can afford;
		else {
			EngineCore.outputText( 'Do you purchase his cure?', false );
			//Remove/No;
			EngineCore.doYesNo( this.wormRemoval, this.giacomoEncounter );
		}
	};
	Giacomo.prototype.nightSuccubiFirstTime = function() {
		...
		if( CoC.getInstance().player.gender === 0 ) {
			EngineCore.outputText( 'She quickly moves down to your crotch... only to discover no organs down there.\n\n' );
			EngineCore.outputText( '*record scratch*\n\n' );
			EngineCore.outputText( '"<i>Wait a fucking minute,</i>" the Succubus says, "<i>Where\'s your dick?!</i>"\n\n' );
			EngineCore.outputText( 'As you state your genderless nature, the succubus hops off and from nowhere pulls out a large folder marked "<i>Corruption of Champions-Script</i>" and begins thumbing through the pages.  After finding the page she is looking for, she reads it and looks off into the distance in disgust.\n\n' );
			EngineCore.outputText( '"<i>Hey Fenoxo and Dxasmodeus!!!!!!</i>" the Succubus crows, "<i>The goddamn script says that I should be milking someone\'s DICK!!!  Man, futa, herm, I don\'t give a shit.  YOUR OWN FUCKING SCRIPT SAYS I SHOULD BE MOUNTING AND MILKING A COCK!!!!  THIS IS A SEX GAME!!!!!!  THAT MEANS FUCKING!  WHAT THE HELL AM I SUPPOSED TO FUCK???!!!</i>"\n\n' );
			EngineCore.outputText( 'The Succubus looks at you with utter contempt, "<i>THIS motherfucker doesn\'t have a DAMN thing!  What am I supposed to do?!  I can\'t exactly order a fucking Happy Meal!!!!!</i>"\n\n' );
			EngineCore.outputText( 'Throwing the script down in an utter rage, the tantrum continues, "<i>Goddammit!  I can\'t believe this shit!  HEY!!!!!  INTERN!!!!  Bring me my robe, aspirins and cancer sticks!!!!</i>"\n\n' );
			EngineCore.outputText( 'The Succubus walks a few paces away where a plain-dressed woman with a clipboard hands the Succubus a pack of cigarettes and a small bottle of aspirin.  She takes a fistful of the painkillers and immediately lights up a smoke.  The Succubus takes a couple of drags off the cig and rubs her temples.\n\n' );
			EngineCore.outputText( '"<i>You two are killing me!</i>" she groans in clear frustration, "<i>I come to work for you perverts based off the promise of MORE perverts to feed from and you do THIS to me!  I can\'t work like this!</i>"\n\n' );
			EngineCore.outputText( 'The plain woman hands the Succubus a robe, which she crudely puts on as she storms off into the night.\n\n' );
			EngineCore.outputText( '"<i>I will discuss this horseshit with my agent,</i>" the Succubus continues bitching, "<i>THIS was NOT in my contract.</i>"\n\n' );
		...
		if( CoC.getInstance().player.gender === 1 ) {
			...
			EngineCore.outputText( '"<i>Quit whimpering,</i>" the Succubus orders.  "<i>I hope the rumors about you futas are true.  I need a good, fiery load of cum to get me going.  I haven\'t had one in a while and as much as I LOVE men, they can only feed me so much.</i>"\n\n' );
			EngineCore.outputText( 'You quickly try to struggle, but find the Succubus to be utterly dominating.  She wraps her arms around your back and entwines her lean legs around your hips.  The Succubus playfully licks your lips and grins.\n\n' );
			EngineCore.outputText( '"<i>You are getting your dick milked,</i>" the Succubus says flatly, "<i>Accept it.  Trust me, when I am done, you will want more of me, anyway.</i>"\n\n' );
			EngineCore.outputText( 'As the Succubus finishes her ultimatum, you feel churning vaginal contractions stroking your massive cock.  Heavy, powerful, coordinated undulations work your dick as surely as the best handjob.  You quickly moan in shock and pleasure at such rough treatment.' );
		}
		EngineCore.dynStats( 'lus', 35 );
		EngineCore.doNext( this.ceruleanSuccubusEncounterPart2 );
	};
	...
	Giacomo.prototype.ceruleanSuccubusEncounterPart3 = function() {
		...
		if( CoC.getInstance().player.gender === 1 ) {
			...
			EngineCore.outputText( 'The Succubus rears up and pins your shoulders firmly to the ground.  The speed of her vaginal contractions becomes impossibly fast.  As you look down at the Succubus\' crotch, you can clearly see her contractions working your penis despite the massive growth of fur between your legs.  The pressure deep behind your cock is crushing.  With one last spasmodic push, you release a thick, superhuman gout of semen deep in the Succubus\' cunt.  She lets out an inhuman howl of pleasure as her womb clamps down upon you like a vise.  She literally squeezes each stream and drop of cum out of you.\n\n' );
			EngineCore.outputText( 'After a couple of minutes, you feel your dick weaken and begin going limp.  Satisfied, you feel the Succubus\' grip on your prick release and you quickly fall out of her womb.  Much to your surprise, despite the massive load you released, not a single drop of cum falls out or is left on your prick.  The Succubus begins rubbing herself all over, clearly in post-orgasmic ecstasy from absorbing your fluids.  After a couple of chain-orgasms, the Succubus sits down next to the empty jar.\n\n' );
			EngineCore.outputText( 'She places the mouth of the jar next to one of her fat nipples and begins milking her tit into the jar, filling it back up with her fluids.  She places the jar next to you and stands up.\n\n' );
			EngineCore.outputText( '"<i>It has been a long time since I soaked up a load that good,</i>" she says, "<i>Anytime you want more of that, just drink my milk again.  It\'s only fair.  I take your milk, you take mine.</i>"\n\n' );
			EngineCore.outputText( 'She smiles and flies off, leaving you with a fresh bottle of "Cerulean Potion".  As pleasing as the experience was, it has left you thoroughly exhausted.' );
		...
		EngineCore.doNext( this.ceruleanSuccubusEncounterPart4 );
	};
	...
	Giacomo.prototype.nightSuccubiRepeat = function() {
		EngineCore.spriteSelect( 8 );
		if( CoC.getInstance().player.gender === 0 ) {
			if( CoC.getInstance().flags[ kFLAGS.CERULEAN_POTION_NEUTER_ATTEMPTED ] === 0 ) {
				EngineCore.outputText( '\nAs you sleep, your rest becomes increasingly disturbed. You feel a great weight on top of you and you find it difficult to breathe. Stirred to consciousness, your eyes are greeted by an enormous pair of blue-tinged breasts. The nipples are quite long and thick and are surrounded by large, round areola. A deep, feminine voice breaks the silence, "<i>I was wondering if you would wake up.</i>" You turn your head to the voice to see the visage of a sharp featured, attractive woman. The woman grins mischievously and speaks again, "<i>I was hoping that idiot, Giacomo, did not dilute the \'potion\' again.</i>" Your campfire reflects off the woman\'s face and her beauty contains some sharply contrasting features. The pupils of her eyes are slit like a cat\'s. As she grins, she bares her teeth, which contain two pairs of long and short fangs. This woman is clearly NOT human! In shock, you attempt to get up, only prompting the woman to prove her inhuman nature by grabbing your shoulders and pinning you to the ground. You see that each finger on her hand also contains a fourth joint, further proving her status. Before you can speak a word, the woman begins mocking your fear and places her face in front of yours. Her face is almost certainly demonic in nature.\n\n', false );
				EngineCore.outputText( 'She quickly moves down to your crotch...only to discover no organs down there.\n\n', false );
				EngineCore.outputText( '*record scratch*\n\n', false );
				EngineCore.outputText( '"<i>Wait a fucking minute.</i>", the Succubus says, "<i>Where\'s your dick?!</i>"\n\n', false );
				...
				EngineCore.outputText( '"<i>I will discuss this horseshit with my agent.</i>", the Succubus continues bitching, "<i>THIS was NOT in my contract.</i>"\n\n', false );
				EngineCore.outputText( 'The Succubus stops, turns and points to you in derision. "<i>And YOU! You no-cock, no-cunt having pissant! Take your ass back to the lab before they find out you escaped!!!!!</i>"\n\n', false );
				EngineCore.outputText( 'The Succubus resumes her stormy exit. You look at the bottle of Cerulean Potion and wonder if it REALLY had some psychotropics in it. What the hell just happened?!', false );
				CoC.getInstance().flags[ kFLAGS.CERULEAN_POTION_NEUTER_ATTEMPTED ] = 1;
			}
			//REPEAT;
			else {
				...
				EngineCore.outputText( 'You turn around, hoping for an exciting encounter only to find a rather short, plain-faced woman with horned-rim glasses and a purple dress on. She appears to be holding a stack of papers in her hand.\n\n', false );
				EngineCore.outputText( '"<i>Ahem.</i>", the woman says meekly, "<i>I hate to bother you, but I was sent by the CoC writers and staff to hand you this.</i>"\n\n', false );
				EngineCore.outputText( 'Scratching your head, you inquire what the document is. The woman smiles shyly and hands it to you.\n\n', false );
				EngineCore.outputText( '"<i>This is the script and production notes for Corruption of Champions,</i>" she says with a small bit of pride, "<i>Apparently, you need to read the highlighted sections. They are important.</i>"\n\n', false );
				EngineCore.outputText( 'You take the script, scratching your head at the surreal nature of the moment. You thumb through the pages, finding virtually every aspect of your life and encounters written as if foreseen by great mystics. The accuracy is nothing short of horrifying. You find a highlighted section that appears to be what the woman is referring to. The note is terse and outright blunt.\n\n', false );
				EngineCore.outputText( '"<i>GENDER NEUTRAL CHARACTERS ARE BUTT-MONKEYS. IF THE ENCOUNTER INVOLVES SEX, EXPECT SOMETHING FUCKED UP TO HAPPEN INSTEAD. ACTORS WHO PLAY NEUTER CHARACTERS SHOULD EXPECT TO PLAY ONLY FOR LULZ</i>."\n\n', false );
				EngineCore.outputText( 'The shock is overwhelming. The script basically says that you will never catch a break. As this reality drapes about you, the script disappears and you hear a cacophony of mocking laughter in all directions. The woman is nowhere to be found.\n\n', false );
		...
		if( CoC.getInstance().player.gender === 1 ) {
			if( CoC.getInstance().player.cor < 66 ) {
				...
				if( CoC.getInstance().player.isTaur() ) {
					EngineCore.outputText( ' crouch between your legs and impale herself on your ' + Descriptors.cockDescript( 0 ) + ' with a wet sound caused by her well-lubricated vulva. Y', false );
				} else {
					EngineCore.outputText( ' open her womb and quickly consume your ' + Descriptors.cockDescript( 0 ) + '. She embraces you, entrapping your head in her cleavage as y', false );
				}
				EngineCore.outputText( 'ou quickly feel her superhuman vaginal muscles work and stroke your ' + Descriptors.cockDescript( 0 ) + ' better than any human woman or pair of hands could ever hope to accomplish. You are helpless as your unholy embrace milks the both of you in an infernal symphony of debauchery. The familiar cramp of an impending ejaculation grips you and your twitching signals the succubus of your approaching climax.\n\n', false );
				if( CoC.getInstance().player.isTaur() ) {
					EngineCore.outputText( 'Pushing on your forelegs, she engulfs even more of your ' + Descriptors.cockDescript( 0 ), false );
				} else {
					EngineCore.outputText( 'Almost crushing your pelvis, she wraps her legs around your body', false );
				}
				EngineCore.outputText( ' and her muscles churn mercilessly demanding that you release your \'milk\' as freely as she has released hers into you. Stimulated beyond any human ability to maintain control, you bear down and release a milky flood of your own inside the succubus. Moaning in ecstasy, she ', false );
				if( CoC.getInstance().player.isTaur() ) {
					EngineCore.outputText( 'arches under your belly as you feel your ' + Descriptors.cockDescript( 0 ) + ' bending pleasurably inside her, and', false );
				} else {
					EngineCore.outputText( 'releases you from her grip, allowing you to finally breathe deeply, and leans back, arching high to reveal your joined genitals in the moonlight. You visibly see', false );
				}
				EngineCore.outputText( ' her contractions milking your ' + Descriptors.cockDescript( 0 ) + ' as fiercely as a maid milks a cow! Another torrent of cum pushes its way out of your body and you let out a moan of pleasure and exhaustion.\n\n', false );
				EngineCore.outputText( 'As you are passing out, you feel a deep kiss upon your lips from the succubus. "You taste better each time we join. Call upon me soon, lest I take what I want from you anyway,", says the lustful creature.\n\n', false );
				EngineCore.outputText( 'Fatigue takes you and you collapse into a deep sleep.  ', false );
			} else {
				...
				EngineCore.outputText( 'The succubus comes, as you predicted. Despite her obvious strength and size difference to you, you grab her and push her down to the ground and immediately push your angry cock into her hairy hole. The succubus, surprised and enthralled, laughs at your aggression.\n\n', false );
				EngineCore.outputText( '"<i>I thought I was the hungry one.</i>", she chuckles. "I am all yours, little man. FEED ME!"\n\n', false );
				EngineCore.outputText( 'You begin bucking your ', false );
				if( CoC.getInstance().player.isTaur() ) {
					EngineCore.outputText( 'flanks', false );
				} else {
					EngineCore.outputText( 'hips', false );
				}
				EngineCore.outputText( ' in the all-too-familiar rhythm, hammering away at the succubus\' cunt. Impressed with your initiative, she chooses to remain submissive as you work up an impressive load of spunk. Trying with all of your might, you continue to hold off your orgasm, painfully, as you continue stimulating your inhuman lover.\n\n', false );
				EngineCore.outputText( 'However, she senses your control and immediately brings her own muscles into the little love game. With one good squeeze, she breaks down any control and resistance you have. Sensing you are about to explode, she ', false );
				if( CoC.getInstance().player.isTaur() ) {
					EngineCore.outputText( 'pushes on your forelegs, impaling herself even deeper on your ' + Descriptors.cockDescript( 0 ), false );
				} else {
					EngineCore.outputText( 'wraps her legs around your hips and bears down', false );
				}
				EngineCore.outputText( '. You feel the head of your prick push past the dilated opening in her cervix, which immediately contracts around your head. Your penis is literally trapped and caught in her womb!\n\n', false );
				EngineCore.outputText( 'Groaning loudly, long muscle spasms release the painfully stored semen into the deepest parts of the succubus. The sensation of your hot cum so deep inside her body triggers her peak. ', false );
				if( CoC.getInstance().player.isTaur() ) {
					EngineCore.outputText( 'She moans inhumanly, and reflexively digs her claws into your forelegs. Searing with lust, the pain means little to you as you only feel the sensation of your body forcing your fluids out of your body and into hers. You press your ' + Descriptors.cockDescript( 0 ) + ' into her', false );
				} else {
					EngineCore.outputText( 'She embraces you, moaning inhumanly, and reflexively digs her claws into your back. Searing with lust, the pain means little to you as you only feel the sensation of your body forcing your fluids out of your body and into hers. You slam your pelvis into hers', false );
				}
				EngineCore.outputText( ', as if to force yourself to cum harder than you already are capable of, prompting an equally pleasurable reaction from her.\n\n', false );
				EngineCore.outputText( 'For the first time since you have had your \'visits\', the succubus appears winded. Without another word, her muscles release your manhood, which she quickly licks clean of your intermingled juices.  She tongues your face in lustful approval and flies away. You quickly fall asleep, utterly spent.  ', false );
			...
			if( CoC.getInstance().flags[ kFLAGS.CERULEAN_POTION_BAD_END_FUTA_COUNTER ] > 10 && CoC.getInstance().player.cor > 50 ) {
				...
				if( CoC.getInstance().player.balls > 0 ) {
					EngineCore.outputText( '  Even your nuts are unbearably sore.', false );
				}
				...
				EngineCore.outputText( 'After a short time, you wake up sore from head to toe. The Succubus is sitting next to you with an utterly satisfied look on her face.\n\n', false );
				EngineCore.outputText( '"<i>Well, this was unexpected.</i>", she says, "<i>I did not expect you to change. Normally, men are susceptible to my milk, but apparently it works on herms, too.</i>"\n\n', false );
				...
				if( CoC.getInstance().player.horseScore() + CoC.getInstance().player.dogScore() + CoC.getInstance().player.nagaScore() + CoC.getInstance().player.goblinScore() + CoC.getInstance().player.sharkScore() + CoC.getInstance().player.minoScore() + CoC.getInstance().player.cowScore() > 5 ) {
					EngineCore.outputText( '  All of the other corruptions and changes to your body have faded away as your new form has taken shape.', false );
				}
				...
				CoC.getInstance().flags[ kFLAGS.UNKNOWN_FLAG_NUMBER_00111 ]++;
				CoC.getInstance().flags[ kFLAGS.CERULEAN_POTION_BAD_END_FUTA_COUNTER ]++;
				...
				EngineCore.outputText( 'The Succubus restrains you without missing a stroke or disrupting your breastfeeding as the pangs of orgasmic pleasure swell up at the base of your cock. You wrap your arms forcefully around the succubus as you bear down upon your crotch, releasing the painfully stockpiled load of lust into the demoness\' cunt for her own sustenance. The succubus lets out an inhuman howl of pleasure as her own orgasm begins to crush your cock, draining every last drop out of you.\n\n', false );
				EngineCore.outputText( 'Your consciousness begins to fade as the orgasm subsides. The succubus pops her tit out of your mouth and squeezes more of her essence into the empty bottle. She licks your lips and flies away just in time for you to pass out.  ', false );
				//Clear out any queue'ed events if bad-end;
				//coming.  PC has to dig his own grave.;
				if( CoC.getInstance().flags[ kFLAGS.CERULEAN_POTION_BAD_END_FUTA_COUNTER ] > 10 ) {
					CoC.getInstance().player.removeStatusAffect( StatusAffects.SuccubiNight );
				}
				EngineCore.fatigue( 20 );
