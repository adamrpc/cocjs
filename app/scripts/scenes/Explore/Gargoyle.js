...
angular.module( 'cocjs' ).run( function( SceneLib, Descriptors, kFLAGS, CoC, EngineCore, Utils, MainView, StatusAffects ) {
	...
	Gargoyle.prototype.gargoyleConfidence = function( arg ) {
		if( arg !== undefined && arg !== 0 ) {
			CoC.flags[ kFLAGS.GAR_CONFIDENCE ] += arg;
			if( CoC.flags[ kFLAGS.GAR_CONFIDENCE ] > 100 ) {
				CoC.flags[ kFLAGS.GAR_CONFIDENCE ] = 100;
			} else if( CoC.flags[ kFLAGS.GAR_CONFIDENCE ] < 0 ) {
				CoC.flags[ kFLAGS.GAR_CONFIDENCE ] = 0;
			}
		}
		return CoC.flags[ kFLAGS.GAR_CONFIDENCE ];
	};
	...
	Gargoyle.prototype.gargoylesTheShowNowOnWBNetwork = function() {
		...
		MainView.outputText( 'You set off in search of new horizons, setting off from camp in a completely new direction than you\'ve ever tried before.  Away from the parts of Mareth you have thus far discovered, much of the world seems to be a barren wasteland' );
		//if Desert is discovered:;
		if( CoC.player.exploredDesert > 0 ) {
			MainView.outputText( ', making even the desert seem healthy and full of life' );
		}
		MainView.outputText( '.  Your trip soon begins to seem unproductive, having found no new areas of Mareth or any contact with its inhabitants.  You sigh and turn back towards camp.' );
		...
		if( CoC.player.statusAffectv1( StatusAffects.TelAdre ) >= 1 ) {
			MainView.outputText( 'or the ruins of one of Tel\'Adre\'s more distant neighbors, ' );
		}
	...
	Gargoyle.prototype.gargoyleMeeting2 = function() {
		...
		if( CoC.player.lib >= 40 ) {
			MainView.outputText( '  Maybe you could take this with you as a life-sized sex toy?' );
		}
	...
	Gargoyle.prototype.dontBreakThatShit = function() {
		MainView.clearOutput();
		MainView.outputText( 'Oh, no.  Nope.  Nuh-uh.  Not going to happen.  You\'re not falling for that trick!  No, siree.  As soon as you chop those chains, it\'ll probably just come to life and try to kill - or rape - you.  Feeling rather smart, you turn on a heel and exit the cathedral – there\'s nothing else to see right now.' );
	...
	Gargoyle.prototype.breakZeChains = function() {
		...
		MainView.outputText( '\n\n"<i>I am your humble servant, Master.  Your tool.  Your plaything.  Your toy.  Ask, and I shall obey.</i>"  She seems to pause for a moment, and adds in an emotionless whisper, "<i>this one would be honored to pleasure Master, if ' + CoC.player.mf( 'he', 'she' ) + ' desires.</i>"' );
	...
	Gargoyle.prototype.nameZeGargoyle = function() {
		if( MainView.nameBox.value === '' || MainView.nameBox.value === '0' ) {
			...
			MainView.clearOutput();
			MainView.outputText( '<b>You must name her.</b>', false );
			MainView.nameBox.value = '';
		...
		MainView.clearOutput();
		MainView.outputText( '"<i>' + CoC.flags[ kFLAGS.GAR_NAME ] + ',</i>" she purrs, "<i>' + CoC.flags[ kFLAGS.GAR_NAME ] + ', ' + CoC.flags[ kFLAGS.GAR_NAME ] + '.  Master has interesting taste.</i>"' );
	...
	Gargoyle.prototype.giveGargoyleAFunnyOrder = function() {
		MainView.clearOutput();
		MainView.outputText( 'If she\'s so intent on playing the servant, you suppose you can play along.  Summoning up your most commanding voice, you snap, "<i>ROLL OVER!</i>"' );
	...
	Gargoyle.prototype.carnalOrder = function() {
		MainView.clearOutput();
		MainView.outputText( 'Well, well. ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' seems willing to follow any order you give, no matter how carnal, and you could not help but notice her rather <i>arousing</i> attributes earlier.  Smirking wickedly, you command your new gargoyle to finger herself.' );
		...
		if( CoC.player.cor < 40 ) {
			MainView.outputText( 'flustered' );
		} else {
			MainView.outputText( 'excited' );
		}
		MainView.outputText( ' heartbeat.  The way she watches you, unblinking, as she fingers herself is rather unnerving, and you soon command her to stop.  She does so immediately, though with what you think might have been a sigh, and assumes a low crouch to await your next command.' );
		this.gargoyleStarterMenu();
	};
	//[b]Talk (First Time)[/b];
	Gargoyle.prototype.firstGargoyleTalk = function() {
		MainView.clearOutput();
		MainView.outputText( 'You decide against giving ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' an order for now.  Instead, you right a nearby toppled pew and seat yourself across from the crouching gargoyle, returning her intense stare with one of your own.' );
	...
	Gargoyle.prototype.berateTheGargoyle = function() {
		...
		MainView.outputText( 'A pillar of rage boils up inside you.  You are [name], Champion of Ingnam, you tell her, rising to your feet, fists clenched.  Your duty is the same as hers was, to protect your home.  And she failed.  She let all those people down.  You tell her that she\'s worthless, disgusting, a failure.' );
	...
	Gargoyle.prototype.returnToCathedral = function( woken ) {
		...
		if( CoC.time.hours <= 9 || CoC.time.hours >= 18 || woken ) {
			if( !woken ) {
				MainView.outputText( '\n\nIn the dim ' );
				if( CoC.time.hours <= 9 ) {
					MainView.outputText( 'early morning' );
				} else {
					MainView.outputText( 'late evening' );
				}
				MainView.outputText( ' light streaming in from the broken windows, you see ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' poking around some of the rubble, slowly but surely making efforts to repair the damage done to the Cathedral.  Seeing you, however, she immediately drops what she was doing and ' );
				if( this.gargoyleConfidence() < 70 ) {
					MainView.outputText( 'kneels before you, head bowed low.' );
				} else if( this.gargoyleConfidence() < 99 ) {
					MainView.outputText( 'bows low before you, a small smile on her stone lips.' );
				} else {
					MainView.outputText( 'with a sexy sway of her hips, approaches you and lays a small but tender kiss upon your cheek before bowing her head to you.' );
				}

				MainView.outputText( '\n\n"<i>Master has returned,</i>" ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' says ' );
				if( this.gargoyleConfidence() > 50 ) {
					MainView.outputText( 'happily.' );
				} else {
					MainView.outputText( 'solemnly.' );
				}
				MainView.outputText( '  "<i>What would you have of me?</i>"' );
			}
		...
		//[b]Cathedral Interior –> 17:00[/b];
			...
			if( !woken ) {
				MainView.outputText( '\n\nWithin the ruins, you can see ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' perched atop her pedestal in a low, predatory crouch, much as you found her.  Waving a hand in front of her eyes, you confirm she\'s in some sort of dormant mode.  A simple touch from her Master could revive her' );
				if( CoC.player.lust >= 33 ) {
					MainView.outputText( ', or you could use her to satisfy your own urges, not unlike a life-size sex toy' );
				}
				MainView.outputText( '.' );
			}
	...
	Gargoyle.prototype.gargoyleSexMenu = function() {
		MainView.clearOutput();
		MainView.outputText( 'You approach ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' and tell her you have some... pressure that needs relieving.' );
		//(if Confidence =< 69:;
		if( this.gargoyleConfidence() <= 69 ) {
			MainView.outputText( '  The gargoyle bows low before you, her face expressionless.  "<i>By your command, Master.  How would you have me?</i>"' );
		} else {
	...
	Gargoyle.prototype.gargoyleAnal = function() {
		MainView.clearOutput();
		MainView.outputText( 'You tell her to get on all fours as you shed your [armor], unleashing your already-hardening cock.  Nervously, the gargoyle does as you command, quickly finding a clear bit of floor and getting on her hands and knees.  You kneel down behind her, peeling her taut ass-cheeks apart to reveal her tight slit and the marble ring of her tiny asshole.  Grinning, you slather up your forefinger with your tongue, lubing it up as best you can before pressing it to her backdoor.' );
		...
		if( CoC.player.cocks[ CoC.player.biggestCockIndex() ].cockLength >= 50 ) {
			MainView.outputText( '\n\nKnowing full well the living statue doesn\'t have much in the way of anatomy, you continue to push more and more of monstrous manhood into ' + CoC.flags[ kFLAGS.GAR_NAME ] + '\'s asshole, watching with glee as her stomach begins to distend with the massive size of your prick.  Then her chest bulges, making her gasp in pain and clutch at her breast; that doesn\'t stop you, though.  With one mighty, massive thrust, you slam yourself up to the hilt in ' + CoC.flags[ kFLAGS.GAR_NAME ] + '\'s ass, and watch with glee as the head bursts out of her mouth, squirting pre and drool all over the floor.  The poor gargoyle makes a muffled scream and starts flailing around helplessly, trying to do <i>something</i> about the massive member now protruding from her mouth.' );
			MainView.outputText( '\n\nWith gusto, you start hammering away at your living cock-sleeve, using her entire body as one great big ona-hole to take your gargantuan girth.  Resigned to her fate, ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' slumps forward and grasps the head of your [cock biggest], giving you a handy through her mouth as her innards milk the rest of your shaft.' );
	...
	Gargoyle.prototype.gargoyleCoochiiGetsPlowed = function() {
		...
		MainView.outputText( 'You gently push ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' onto her back and spread her long, marble legs, giving you a clear view of her tight little pussy.  You brush your hand along her thighs, making her gasp with the sensation as your knuckles run along her smooth, cool flesh.  Your fingers come to her small clit, and you caress her tiny pleasure buzzer, eliciting a tiny moan from the gargoyle.  Grinning, you give it another flick; ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' squirms, trying to either enhance or end the sensation, you can\'t be sure.  You start to finger her clitty harder, creating little circles around it as you slip another finger into her tight passage.' );
		MainView.outputText( '\n\nYou\'re rewarded for your efforts by ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' bucking her hips into your hands, moaning, "<i>M-Master... please...</i>"  Not done teasing her yet, you put your other hand on her flat belly and start to crawl it up, tickling her as you make your way toward her sizable breasts.  You glomp onto one of them, giving it a rough squeeze as you drive your finger into her a little further than before.  "<i>Masterrrrr...</i>" ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' whines, reaching up to stroke your cheek.  Deciding you\'ve teased the poor girl enough, you prepare for the main course.' );
		MainView.outputText( '\n\nYou pull your ' + Descriptors.cockDescript( x ) + ' from your [armor], letting it flop down atop her crotch.  Embarrassed, ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' looks away as you rub the underside of your cock across her mons, spitting on it for a bit of extra lubricant.  You lean back, lining the tip of your cockhead with the gargoyle\'s tight slit, and press forward.  She lets out a sharp gasp as your cock presses against her, pushing it past her lips and finally slipping into her depths.' );
		...
		if( this.gargoyleConfidence() >= 70 ) {
			MainView.outputText( '  Before you can stand, though, ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' hugs her arms tightly around you pulling you down on top of her and mashing your face between her soft tits.' );
		}
	...
	Gargoyle.prototype.titFuckTheGargoyle = function() {
		...
		MainView.outputText( '\n\nSuddenly, you feel an overwhelming wetness on your cock.  You look down, expecting to see pre flowing freely from you, but are instead greeted with the sight of ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' sliding the tip of your cock into her mouth, starting to fellate you on top of the tittyfuck.  You give her a few more words of encouragement and press her head down further onto your shaft, forcing her to take all that\'s left over from her breasts.  Despite her marble nature, her mouth is moist and surprisingly warm, and her long tongue is quick to wrap around your [cock biggest], wringing it in time with her chest-thrusts.' );
		...
		MainView.outputText( '\n\nYou grab ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' and push her down hard, slamming your cock down her throat as you cum.  A human would surely have choked with this maneuver – instead, ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' squirms and flails, but you can see and feel her swallow your cum as it pours into her mouth, her throat massaging your flaring head, milking it harder than any tit-job ever could have.  Once her face has been ground into your pubes, you buck your hips sharply, thrusting into her throat as the last of your jizz spurts out and down her gizzard.' );
		MainView.outputText( '\n\nBy the time you\'re done, ' + CoC.flags[ kFLAGS.GAR_NAME ] + '\'s face is an utter mess, with white lines trickling down her cheeks.  You drag your [cock biggest] out of her with a wet POP, unleashing a dam of jizz that quickly leaks out of her agape mouth to stain her breasts and belly.  You chuckle at the sight' );
		if( this.gargoyleConfidence() >= 70 ) {
			MainView.outputText( ', and to your surprise, so does ' + CoC.flags[ kFLAGS.GAR_NAME ] + '' );
		}
		MainView.outputText( '.' );
	...
	Gargoyle.prototype.strapOnGargoyle = function() {
		MainView.clearOutput();
		MainView.outputText( 'Giving a quick look around the ruins, you tell ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' to go find something that can bring you two a little <i>closer</i>.' );
		...
		MainView.outputText( '\n\n"<i>Well, someone\'s enjoying herself,</i>" you tease, and immediately ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' freezes, her mouth agape with embarrassment.  With a little laugh, you give her a pat on the thigh and start to strip out of your [armor].  But you can\'t just leave ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' without SOME stimulation: ' );
		//(If Human Legs/Feet [ie, needs toes], as new paragraph): ;
		if( !CoC.player.isTaur() && !CoC.player.isGoo() && !CoC.player.isDrider() && !CoC.player.isNaga() ) {
			MainView.outputText( 'you reveal your leg first, moving slowly and sensually, giving her a tantalizing look at your nethers before hiding them again.  With a wide grin, you creep your foot up to her thighs and gently run your heel along her smooth, tender flesh.  You come to the base of her rod, standing stark straight in the air like a flag-pole.  As you strip, you grasp her cock between your big and second toe, sliding them up the smooth shaft to the tip.  ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' moans, rolling her head back as you start to foot-fuck her cock, slowly putting more and more pressure on the steel member until it\'s pressed against her belly.  Your heel takes over, running quickly along the underside of her cock as you finally rid yourself of your [armor].' );
		}//Else, continued from above paragraph:;
		else {
			MainView.outputText( 'you giver her a good show, putting on a brief striptease for the gargoyle.  You strip your [armor] slowly, sensually, showing only the barest hints of your privates until the last moment, when the last piece of your clothing falls to the floor.' );
		}
		//(From Both, if has Vagina:) ;
		if( CoC.player.hasVagina() ) {
			MainView.outputText( '\n\n' + CoC.flags[ kFLAGS.GAR_NAME ] + ' is panting with lust by the time you\'re done teasing and stripping for her. Now nude, you drop back to your knees over her holy rod-cock, straddling her hips and lining her up with your [vagina]. You lower yourself until the tip of the rod is pressing against your lower lips – you\'re pleasantly surprised by the warm tingling sensation that quickly spreads out around the point of contact, and your cunny responds immediately by letting loose a little trickle of moisture that beads down on the gargoyle\'s shaft.' );
			MainView.outputText( '\n\nWithout further ado, you drop down on the strap-on, lowering yourself onto the tingling metal rod until your groin is pressed against ' + CoC.flags[ kFLAGS.GAR_NAME ] + '\'s.  She, all the while, is biting her lower lip and whimpering with pleasure as your [vagina] swallows her length. Once you\'ve taken all she has to offer, you begin grinding your hips against her, rocking back and forth atop the gargoyle.  To your surprise, ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' grabs you, squeezing her long fingers into your [hips] and pulling you up, almost off her rod. You decide to go with it; you grab her tits for support and start to bounce on her cock.' );
			CoC.player.cuntChange( 15, true, true, false );
			MainView.outputText( '\n\nSoon, you\'re both moaning with pleasure, bucking and thrusting your hips to meet each other as you continue to ride ' + CoC.flags[ kFLAGS.GAR_NAME ] + '\'s strap-on.  However, you can feel the clenching sensation of a coming orgasm building in your gut, and if the strained panting of your partner is any indication, she isn\'t far behind.  You start bouncing faster, increasing to a wild pace as your cunt contracts on the metal rod inside it, trying to milk the fake cock as waves of pleasure roll over you.  You don\'t let up throughout the orgasm, quite literally riding it out until ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' lets out a high-pitched shriek and grabs your back, smashing your face into her tits and holding you tight as she, too cums.  Suddenly she\'s the one pounding you, thrusting her hips into your [vagina].' );
			MainView.outputText( '\n\nBy the time she\'s done, both your hips and thighs are spattered with your sweat and girlcum. Still panting from the ordeal, you slowly drag yourself off of ' + CoC.flags[ kFLAGS.GAR_NAME ] + '\'s cock and roll onto your back, exhausted. By the time you\'ve recovered enough to stumble to your feet, ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' is fast asleep beside you, snoring cutely, her metal cock still standing straight up in the air.  You give her a gentle pat on the thigh and get dressed, leaving the gargoyle snoozing peacefully as you head back to camp.' );
		}
		//(ELSE, PC has no vagina:);
		else {
			MainView.outputText( '\n\n' + CoC.flags[ kFLAGS.GAR_NAME ] + ' is panting with lust by the time you\'re done teasing and stripping for her. Now nude, you drop back to your knees over her holy rod-cock, straddling her hips and lining her up with your [asshole].  You lower yourself until the tip of the rod is pressing against your clenched backdoor – and you\'re pleasantly surprised by the warm tingling sensation that quickly spreads out around the point of contact. Your anus responds immediately by utterly relaxing, letting just the tip of ' + CoC.flags[ kFLAGS.GAR_NAME ] + '\'s strap-on slide inside you.' );
			MainView.outputText( '\n\nWithout further ado, you drop down on the strap-on, lowering yourself onto the tingling metal rod until your ass is pressed against ' + CoC.flags[ kFLAGS.GAR_NAME ] + '\'s thighs.  She, all the while, is biting her lower lip and whimpering with pleasure as your ass swallows her length.  Once you\'ve taken all she has to offer, you begin grinding your hips against her, rocking back and forth atop the gargoyle.  To your surprise, ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' grabs you, squeezing her long fingers into your [hips] and pulling you up, almost off her rod.  You decide to go with it; you grab her tits for support and start to bounce on her cock.' );
			CoC.player.buttChange( 15, true, true, false );
			MainView.outputText( '\n\nSoon, you\'re both moaning with pleasure, bucking and thrusting your hips to meet each other as you continue to ride ' + CoC.flags[ kFLAGS.GAR_NAME ] + '\'s strap-on.  However, you can feel the clenching sensation of a coming anal orgasm building in your gut, and if the strained panting of your partner is any indication, she isn\'t far behind.  You start bouncing faster, increasing to a wild pace as your hole contracts on the metal rod inside it, instinctively trying to force out the intruder, but only serving to let waves of pleasure roll over you.  You don\'t let up throughout the orgasm, quite literally riding it out until ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' lets out a high-pitched shriek and grabs your back, smashing your face into her tits and holding you tight as she, too cums.  Suddenly she\'s the one pounding you, thrusting her hips into your [asshole] until you\'ve both cum, hard, clutching each other as orgasmic pleasure slams through you.' );
			MainView.outputText( '\n\nStill panting from the ordeal, you slowly drag yourself off of ' + CoC.flags[ kFLAGS.GAR_NAME ] + '\'s cock and roll onto your back, exhausted. By the time you\'ve recovered enough to stumble to your feet, ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' is fast asleep beside you, snoring cutely, her metal cock still standing straight up in the air.  You give her a gentle pat on the thigh and get dressed, leaving the gargoyle snoozing peacefully as you head back to camp.' );
		}
		CoC.player.orgasm();
		EngineCore.dynStats( 'sen', -1 );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	...
	Gargoyle.prototype.tailFuckGargoyleScene = function() {
		...
		MainView.outputText( '\n\nBefore you can get used to the anal intruder, your eyes have rolled back in your head with pleasure.  Girlcum spurts from your cunt, splattering ' + CoC.flags[ kFLAGS.GAR_NAME ] + '\'s face with your juices as you cum.  Your holes squeeze down on her appendages, trapping them inside you as your ride out the orgasm, bouncing on her rigid, rapidly-moistening tail until your body settles down.' );
		MainView.outputText( '\n\nWhen you\'ve finished, you carefully extricate yourself from the gargoyle\'s tail and, a bit bow-legged, gather up your [armor].  By the time you\'re dressed, ' + CoC.flags[ kFLAGS.GAR_NAME ] + '\'s cleaned herself off and is eagerly awaiting some token of your approval.' );
		MainView.outputText( '\n\n"<i>Was Master satisfied?</i>" she asks' );
		if( this.gargoyleConfidence() > 70 ) {
			MainView.outputText( ' with just a hint of a smile' );
		}
		MainView.outputText( '.' );
	...
	Gargoyle.prototype.useGargoyleMaleHerm = function() {
		MainView.clearOutput();
		MainView.outputText( 'You approach the motionless statue with a lustful look in your eye. Quickly, you strip out of your [armor] and circle around the frozen girl.  With some effort, you\'re able to lift her heavy, spiked tail out of the way, revealing the tight cunt hidden between her legs.  You drop to your knees behind the gargoyle and bury your face in her crack, slurping at her vag, getting it nice and moist in preparation for your cock.' );
		...
		MainView.outputText( '\n\nYou return to thrusting into her, going fast and furious as your pleasure climbs steadily towards climax.  Only when you\'re bursting over the threshold of orgasm do you lay your hands on ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' again, grabbing her ass and giving one last massive thrust into her, shoving your [cock biggest] in to the hilt.  You cum inside her just as she awakens, moaning lustily into her ear as spunk drizzles out around your member.' );
		MainView.outputText( '\n\n"<i>M-Master?</i>" ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' gasps, giving a full-body shudder as you slowly pull out from inside her.  Your cum drools out of her snatch and pools beneath her.  Playfully, you jam your fingers up between her legs, damming your sperm inside her.  To your glee, this simple act sends ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' right over the edge, she collapses on the floor with a heavy THUNK!' );
	...
	Gargoyle.prototype.ritualGargoyle = function() {
		...
		if( CoC.flags[ kFLAGS.RITUAL_INTRO ] === 0 ) {
			...
			MainView.outputText( 'You note that the sanctuary\'s altar has been cleared of rubble, and a number of candles have been lit upon it. Resting atop the flat stone surface is a simple iron bowl filled with what appears to be water. Curious, you ask ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' what she\'s been up to.' );
	...
	Gargoyle.prototype.bodyRitual = function() {
		MainView.clearOutput();
		MainView.outputText( 'You tell ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' that you could stand to reverse some of the corruption that has affected your body.  With an solemn nod, she takes you to a side alcove near the altar and instructs you to spread your arms and legs.  Hesitantly, you do so, and from the floor and ceiling ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' produces leather straps with rough looped straps.  Before you can react, she binds your arms and legs, spreading you eagle against the cold stone wall.' );
	...
	Gargoyle.prototype.noReactionToBodyRitual = function() {
		MainView.clearOutput();
		MainView.outputText( 'You simply lie still and try to recover from ' + CoC.flags[ kFLAGS.GAR_NAME ] + '\'s "gentle ministrations."  After a few minutes, you\'ve caught your breath and, gathering your possessions, make your way out of the Cathedral without another word.  Damn, you\'re sore.' );
		EngineCore.doNext( SceneLib.camp, SceneLib.camp.returnToCampUseOneHour );
	...
	Gargoyle.prototype.mindGargoyleRitual = function() {
		...
		MainView.outputText( '  ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' brings the invocation to a quiet close, and rests the bowl back upon the altar. "<i>Drink of this, Master,</i>" she says, offering you the bowl, "<i>and be healed of mind.</i>"' );
		MainView.outputText( '\n\nYou do so, taking a long draught of the holy water.  By the time you\'ve set the bowl down, you feel light-headed, as if a fog has receded from your mind.  You gasp happily, able to think more clearly than you have in days.  You thank ' + CoC.flags[ kFLAGS.GAR_NAME ] + ', give her a pat on the head, and head back to camp with a skip in your step.' );
	...
	Gargoyle.prototype.banishmentGargoyleRitual = function() {
		...
		MainView.outputText( ' out of your system.' );
		MainView.outputText( '\n\nFrom beneath the altar, ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' withdraws what looks like a perfectly cylindrical dildo, covered with dozens of dark, dimly-glowing blue runes.  Slowly, she makes her way around you and kneels behind your [butt].  "<i>Take a deep breath, Master, and relax yourself.  It will be better this way.</i>"' );
		...
		MainView.outputText( '\n\n"<i>Do not worry, Master,</i>" ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' says' );
		if( this.gargoyleConfidence() >= 50 ) {
			MainView.outputText( ' soothingly, using her free hand to stroke your thighs' );
		}
		...
		if( CoC.player.findStatusAffect( StatusAffects.Exgartuan ) >= 0 ) {
			...
			MainView.outputText( '  Gratifyingly, however, you can feel the infestation within you squirming and writhing, desperately seeking escape.' );
		}
	...
	Gargoyle.prototype.kinkyBodyRitual = function() {
		...
		//(Confidence +2);
		//(PC decreases Libido and Sensitivity. Suffers 20% Max HP damage, to a minimum of 1 pt. remaining);
		MainView.outputText( 'You tell ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' that you could stand to reverse some of the corruption that has affected your body.  With an eager nod, she strips you down, takes you to a side alcove near the altar and instructs you to spread your arms and legs.  Putting your trust in your little gargoyle, you do so, allowing her to bind you spread-eagle with long leather straps.' );
		...
		if( CoC.player.biggestTitSize() >= 1 ) {
			MainView.outputText( ', cupping your [chest] in her cold stone hands' );
		}
		MainView.outputText( '.  Your [nipples] go rock hard from her cold touch, and you shudder as she gently nips your ear and drags the length of the crop along your thighs.' );
		MainView.outputText( '\n\n"<i>You\'ve been a very naughty ' + CoC.player.mf( 'boy', 'girl' ) + ', to have a body so warped,</i>" she says, again running the crop along your incredibly sensitive inner thighs, making you tremble with the sensation.  You\'re acutely aware of her large, soft breasts pressing into your sweat-slicked back.  Huskily, ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' whispers, "<i>Don\'t worry, Master. I\'ll take good care of you...</i>"  CRACK!  She lays on with the crop, a short, stinging stoke across your thigh.  You gasp, as much in pleasure as in pain, still reveling in the sensation of her embrace.' );
		...
		if( CoC.player.hasCock() ) {
			...
			MainView.outputText( 'barren crotch' );
		}
		MainView.outputText( '.  She breaks the kiss and steps back, laying on one last time – CRACK! with the crop, stinging you again with an intense mix of pain and pleasure.  Gasping and panting from stimulation, you nearly collapse when ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' unbinds you, though she\'s quick to catch you before you fall.' );
		MainView.outputText( '\n\nShe guides you over to the altar and helps you lie atop it to rest.  Once you\'re comfortably situated, ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' slips up beside you, resting her cheek ' );
		if( CoC.player.biggestTitSize() < 1 ) {
			MainView.outputText( 'on your chest' );
		} else {
			MainView.outputText( 'between your [chest]' );
		}
		MainView.outputText( '.  Already, you feel more thick-skinned and less lusty, and so give an appreciative grin to your cute little gargoyle.  In response, she nuzzles against you and wraps her bat-like wings protectively around the two of you.' );
		MainView.outputText( '\n\nYou spend a few minutes cuddling with ' + CoC.flags[ kFLAGS.GAR_NAME ] + ', but sadly, you know you can\'t leave your camp unattended for too long.  Giving her a pat on the head and a kiss on the brow, you gather up your clothes and head on back to camp.' );
	...
	Gargoyle.prototype.mindRitualPervy = function() {
		...
		MainView.outputText( '\n\n' + CoC.flags[ kFLAGS.GAR_NAME ] + ' quickly slips her small toes into your mouth, letting you suckle the holy water from them like an infant.  She lets you remain like that for nearly a minute, nursing from the rivulets that come your way, before pulling her leg back and taking your head with it.  She leans backwards as you approach, balancing upon her bat-like wings as she finishes the chant blessed is ' + CoC.player.mf( 'he', 'she' ) + ' who lusts after holiness and virtue before pleasures of the flesh.</i>"  She pours from the basin once again, releasing the holy water directly onto the large, shapely mounds of her breasts.  The water runs down her chest and belly, creating a tiny waterfall over the nub of her clitoris to which she guides you.' );
		MainView.outputText( '\n\nYou do as she so obviously wishes, and spread her legs to get a good look at her now-sopping wet slit. Her vagina seems small, but an experimental touch proves it to be quite malleable – and your single touch causes ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' to arch her back, gasping with the sudden pleasure.  Smiling at the adorable, enraptured look on her face, you start lapping with a vengeance, grasping her thighs and burying your face between her legs.  You flick your tongue into her smooth, cool slit, lapping up a few drops of water with each flick of your dexterous tongue, and eliciting a gasp or moan each time.' );
		MainView.outputText( '\n\nSoon, ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' is clutching at her breasts and quickly fingering her cherry-nub clit as you lick her out, trying her best to keep an even rhythm with you, even as the waterfall comes to an end and you begin to feel light-headed, as if a fog has lifted from your mind.  By way of thanks, you redouble your efforts to pleasure her, ramming your tongue into her depths and fucking her mercilessly.  Quickly, she begins to tense, stopping her own actions to grasp at your ' + Descriptors.hairDescript() + ' and shove your face further into her crotch, locking her legs and tail around your back as she approaches orgasm.' );
		MainView.outputText( '\n\nScreaming, ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' arches her back and thrusts out her bat-wings to their full expanse, clamping down on your tongue as she cums.  You slow your pace, giving her a last few licks as she pants, gasps, and moans, slowly coming down from an orgasmic high.  She collapses backwards, clutching her chest and staring into the rafters.  Chuckling, you stand and give her a pleased pat on the inner thigh, letting her know she did a good job for you.  You leave her smiling atop the altar, slowly getting her breath under control.' );
		//(Return PC to camp, advance time 1 hour);
	...
	Gargoyle.prototype.banishPervRitual = function() {
		...
		MainView.outputText( '.  She gives you an understanding nod and tells you to strip.  As you do so, she dips beneath the altar, reappearing with a silver, blue-rune covered rod nestled into what looks like a leather thong.  ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' slips the thong on, letting a slight gasp escape her lips as the inner end of the rod enters her.' );
		MainView.outputText( '\n\nShe gives the shaft of the strap-on a few experimental strokes and, satisfied, says, "<i>I\'ve turned down the power of the runes, so it will not hurt you so much... though it will take longer than normal.  I suggest you lubricate it, Master.</i>"  Smiling, she sits herself atop the alter and presents her silver “cock” to you.  You kneel and slip your face between her spread legs, giving the rod a quick lick.  It\'s incredibly smooth and quite cold, much like its wearer, but your tongue comes away with a warm tingle spreading through it – not pain, but more like numbness.  You take the rod into your mouth, running your tongue across its length and thrusting your face against it, blowing it like a cock while getting it sopping wet with your saliva.  You\'ve only been at it for a minute when you notice ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' is bucking her hips slightly and moaning quietly, obviously getting more than a little enjoyment from the sensation.  Perhaps she can feel through it, like a real dick?  Grinning, you redouble your efforts to suck her off.' );
		MainView.outputText( '\n\n"<i>Enough, Master,</i>" she gasps, pushing you back.  Breathing heavily, she takes you by the hand and gets you to brace yourself against the altar.  Your entire mouth is numb now, tingling from the strange sensation given off by the holy rod-cock.  ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' slips behind you, putting a cool, soft hand on your [hips] as she lines up for penetration.  You grip down hard on the edge of the altar, and try your best to relax your sphincter as you begin to feel a tingling pressure against your back door.' );
		if( CoC.player.ass.analWetness > 0 ) {
			MainView.outputText( '  She slips in easily, sliding her tingling holy rod into you until her hips are pressed against your [butt]' );
		} else {
			MainView.outputText( '  She slowly but surely coaxes the holy rod into your asshole, making long and slow thrusts into you until she finally bottoms out' );
		}
		MainView.outputText( '.' );
		...
		MainView.outputText( ' been absorbed and purged by the rod; and you can\'t help but hope she has more... just in case.' );
	...
	Gargoyle.prototype.talkToGargoyle = function( output ) {
		if( output === undefined || output ) {
			MainView.clearOutput();
			MainView.outputText( 'You take a seat on one of the sanctuary\'s pews, motioning for ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' to join you.' );
			//If Confidence <50:;
			if( this.gargoyleConfidence() < 50 ) {
				MainView.outputText( '  She meekly approaches her Master and kneels before you, head bowed submissively.' );
			}//if Confidence = 51-69: ;
			else if( this.gargoyleConfidence() < 70 ) {
				MainView.outputText( '  She nearly skips to follow you, assuming her low crouch at your feet and looking up at your expectantly.' );
			} else {
				MainView.outputText( '  Happily, she takes a seat beside you and slips an arm around your shoulders, locking you in companionable closeness.' );
			}
			MainView.outputText( '  You tell her you\'d like to talk for a few minutes, which immediately causes her eyes to brighten excitedly.  "<i>Master honors me...  What would you like to talk about?</i>"' );
		}
	...
	Gargoyle.prototype.historyGo = function() {
		if( CoC.flags[ kFLAGS.GAR_HISTORY ] === 0 ) {
			CoC.flags[ kFLAGS.GAR_HISTORY ] = 1;
			this.talkToGargoyleHistoryA();
		} else if( CoC.flags[ kFLAGS.GAR_HISTORY ] === 1 ) {
			this.historyOfGargoylesB();
			CoC.flags[ kFLAGS.GAR_HISTORY ] = 2;
		} else {
			CoC.flags[ kFLAGS.GAR_HISTORY ] = 3;
	...
	Gargoyle.prototype.historyOfGargoylesB = function() {
		MainView.clearOutput();
		MainView.outputText( 'You ask ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' to tell you a little bit about the demons coming to Mareth' );
		if( this.gargoyleConfidence() >= 70 ) {
			MainView.outputText( ' if she\'s up to it' );
		}
		MainView.outputText( '.  ' );
		MainView.outputText( 'Hesitantly, she nods her assent, though she says, "<i>I\'m sorry, Master, but I must warn you was greatly abused by the demons.  They are responsible for everything that is wrong with my world' );
		//if Confidence >70: ;
		if( this.gargoyleConfidence() > 70 ) {
			MainView.outputText( ', and I despise them with every fiber of my being' );
		}
		MainView.outputText( '.  I may not be the most fair nor most balanced source of information.</i>"' );
	...
	Gargoyle.prototype.gargoyleHistoryC = function() {
		...
		if( SceneLib.izmaScene.izmaFollower() && SceneLib.isabellaFollowerScene.isabellaFollower() ) {
			...
			MainView.outputText( '\n\nWhoa, there. You stop ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' and explain to her that some of your lovers – mates, even – are just such beasts.  You tell her a little bit about Izma and Isabella, enforcing the fact that they\'re both not only quite friendly, but sweet as well.  They\'re just regular people' );
			if( this.gargoyleConfidence() > 50 ) {
				MainView.outputText( ' just like her' );
			}
			MainView.outputText( '.  If she could, you\'re sure ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' would be blushing bright crimson as you educate her on a little inter-special tolerance and understanding.  Chastened, she nods her comprehension when you\'re finished and, after a long pause, asks, "<i>I-If Master really has many... girlfriends... does... no, it\'s silly, of course not...</i>"' );
	...
	Gargoyle.prototype.gargoyleDoesntCountAsAWaifu = function() {
		MainView.clearOutput();
		MainView.outputText( 'You scoff and tell her not to worry – it\'s not like you\'re cheating on Marble by being around her, after all.  She\'s just a statue – a toy, really. Her head sinks low, and the little gargoyle lets out a shudder that might just have been a sob.  You shake your head and change the subject.' );
	...
	Gargoyle.prototype.gargoylesDontCountAsSharkWaifus = function() {
		...
		//(Confidence -5);
		this.gargoyleConfidence( -5 );
		MainView.outputText( 'You scoff and tell her not to worry – it\'s not like you\'re cheating on Izma by being around her, after all.  She\'s just a statue – a toy, really.  Her head sinks low, and the little gargoyle lets out a shudder that might just have been a sob.  You shake your head and change the subject.' );
		//(Return PC to ' + CoC.flags[kFLAGS.GAR_NAME] + ''s main menu);
		this.returnToCathedral( true );
	};
	...
	Gargoyle.prototype.izmaAndIsabellaDodge = function() {
		...
		//(Confidence -5);
		this.gargoyleConfidence( -5 );
		MainView.outputText( 'You make a shitty joke and laugh awkwardly, trying to avoid the topic altogether.  ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' makes a pitiful little sniffle, but takes the hint and falls silent, allowing you to change the subject.' );
	...
	Gargoyle.prototype.cathedralTalks = function() {
		if( CoC.flags[ kFLAGS.GAR_CATHEDRAL ] === 0 ) {
			CoC.flags[ kFLAGS.GAR_CATHEDRAL ]++;
			this.talkCathedralA();
		} else if( CoC.flags[ kFLAGS.GAR_CATHEDRAL ] === 1 ) {
			this.cathedralBTalk();
			CoC.flags[ kFLAGS.GAR_CATHEDRAL ] = 2;
		} else {
			this.cathedralC();
			CoC.flags[ kFLAGS.GAR_CATHEDRAL ] = 3;
		}
	};
	...
	Gargoyle.prototype.cathedralC = function() {
		MainView.clearOutput();
		MainView.outputText( 'As you sit down with ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' to talk, you remember her words when you first met indicating that she could not – or would not – leave the Cathedral, even if you asked her to.   You ask her about that.' );
		...
		if( this.gargoyleConfidence() > 69 ) {
			MainView.outputText( ' unless it were to be with you, that is' );
		}
		MainView.outputText( '.  But I am bound to this place, Master. I could not leave even if I wanted to.  My makers were wary of their creation, and chose to bind me here; they thought that if I ever rebelled, I would not do much harm if contained within the Cathedral.</i>"' );
	...
	Gargoyle.prototype.comfortGargoyle = function() {
		...
		MainView.outputText( '\n\nTo your surprise, ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' follows you, running to catch up with you.  She takes your hand in hers and wraps one of her wings around you as you walk, holding you close as you make your way out the doors and onto the grounds.  Soon, though, you see ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' slowing and grimacing – approaching the edge of her boundary.  The two of you stop at the wrought-iron fence surrounding the Cathedral, and face each other.' );
	...
	Gargoyle.prototype.talkAboutGarName = function() {
		if( CoC.flags[ kFLAGS.GAR_NAME_TALKS ] === 0 ) {
			CoC.flags[ kFLAGS.GAR_NAME_TALKS ] = 1;
			this.garNameA();
		} else if( CoC.flags[ kFLAGS.GAR_NAME_TALKS ] === 1 ) {
			CoC.flags[ kFLAGS.GAR_NAME_TALKS ] = 2;
			this.garNameB();
		} else {
			CoC.flags[ kFLAGS.GAR_NAME_TALKS ] = 3;
			this.garNameC();
		}
	};
	...
	Gargoyle.prototype.garNameA = function() {
		MainView.clearOutput();
		MainView.outputText( 'Curious about your gargoyle, you ask ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' to tell you a little bit about her life before you arrived.  She makes a nervous chuckle at the request, and shyly says, "<i>Master is too kind...  I am no one special...</i>"' );
		...
		if( CoC.player.race() === 'human' ) {
			...
			MainView.outputText( 'once were' );
		}
		MainView.outputText( ', Master, and more.  We were to be their guardians, protectors – their perfect servants.</i>"' );
		...
		if( this.gargoyleConfidence() > 69 ) {
			MainView.outputText( '  She continues, "<i>though even were I not compelled to, I would still serve you,</i>" she says with a playful wink.' );
		}
		...
		if( this.gargoyleConfidence() <= 69 ) {
			...
			MainView.outputText( 'After what she did to me, I never thought I\'d enjoy sex again.</i>"  With a loving smile, ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' takes your hand in hers and brushes it against her cheek.  "<i>But for you, Master, I think I could make an exception.</i>"' );
		}
		...
		//If Confidence =<69:;
		if( this.gargoyleConfidence() <= 69 ) {
			MainView.outputText( '  "<i>I\'m sorry, Master, I can\'t... it hurts too much to think about it.</i>"' );
		} else {
			MainView.outputText( '  "<i>Sorry, Master. I just....Don\'t want to talk about it right now.  Ask me again some time.</i>"' );
		}
	...
	Gargoyle.prototype.garNameB = function() {
		...
		MainView.outputText( '\n\nYou don\'t need to prompt ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' this time.  "<i>She left, perhaps ten years ago.  One day I was following orders to Master\'s pleasure, and the next I was bound to my pedestal, wearing the chains that Master broke to set me free.</i>"' );
		MainView.outputText( '\n\nWhile she still hasn\'t answered the whole of your question, you nod and move on.' );
		this.gargoyleConfidence( 5 );
		this.talkToGargoyle( false );
	};
	//[b]' + CoC.flags[kFLAGS.GAR_NAME] + ' C[/b];
	Gargoyle.prototype.garNameC = function() {
		//(Confidence +3);
		this.gargoyleConfidence( 5 );
		MainView.clearOutput();
		MainView.outputText( 'You ask ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' if she\'s finally ready to talk to you about what happened to her previous Master, and what happened to her since.  Hesitantly, she agrees to your request.  "<i>Master was a dark sorceress, gifted with the skill of manipulating the bodies of others, though it caused them incredible pain.  I think... perhaps her fascination with me came from my immutability, that she couldn\'t change my body to her whims through her magic.  So she sought to change my mind, breaking my will and shaping me to her desires.</i>"  With a rueful smile, she adds, "<i>You could say that she made me the creature I am today.</i>"' );
	...
	Gargoyle.prototype.comfortGarNameC = function() {
		MainView.clearOutput();
		MainView.outputText( 'You pull ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' onto your lap and run your hand through her hair.  You tell her that she\'s wrong – dead wrong.  She is a person, and she is not helpless.  She\'s anything but.' );
		...
		MainView.outputText( '\n\nShe smiles weakly, and rests her head on your chest.  "<i>As you command, Master,</i>" she says, and falls silent.  You sit there for a long while, stroking the gargoyle\'s soft hair and hoping you have made an impression on her.  But, eventually, you know you have to part.  You give ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' a little kiss on the forehead and tell her to do whatever she wants while you\'re gone.' );
		...
		MainView.outputText( '\n\nSighing, you pat ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' on the head and tell her you\'d be happy to stay for a while longer.  Beaming, she asks you what you\'d like to do.' );
	...
	Gargoyle.prototype.gargoyleAppearance = function() {
		MainView.clearOutput();
		MainView.outputText( '' + CoC.flags[ kFLAGS.GAR_NAME ] + ' is a gray marble statue that, through magics you can\'t quite grasp, has been animated.' );
		...
		if( this.gargoyleConfidence() < 70 ) {
			MainView.outputText( 'glow timidly, reflecting a submissive and servile nature in the gargoyle.' );
		} else {
			MainView.outputText( 'have a cool fire in them, showing her steadily-growing confidence and warmth toward you.' );
		}
		MainView.outputText( '  Her skin is cold and smooth to the touch, a consequence of her marble material, yet she is quite soft, almost squishy.  ' + CoC.flags[ kFLAGS.GAR_NAME ] + ' has a pair of cute little horns upon her brow, and has bat-like wings sprouting from her back which give a gentle flap every few seconds, an unconscious gesture on her part as you stare at her.  She has a long, spiked tail that curls ' );
		if( this.gargoyleConfidence() < 70 ) {
			MainView.outputText( 'timidly' );
		} else {
			MainView.outputText( 'playfully' );
		}
		MainView.outputText( ' about her thighs.  She has two normal human legs sculpted to end in normal human feet.' );
